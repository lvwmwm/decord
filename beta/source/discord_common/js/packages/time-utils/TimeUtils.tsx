...
// Function ID: 4788
// Name: TimeUtils
// Dependencies: [5, 4788, 573, 4789, 2]
// Exports: convertMinutesToGivenTimeUnit, getTimeAndUnit, getTimeUnit

// Module 4787 (TimeUtils)
import u from "u" /* 573 */;
import findLastIndexDefault from "findLastIndex" /* 4789 */;
...
class StopWatch {
  constructor() {
    ...
    merged = Object.assign({ startTime: "duration", timePassed: false });
...
class TimeOut {
  constructor(arg0) {
    if (typeof StopWatch === "function") {
      ...
      tmp5 = closure_6;
      merged1 = Object.assign({ startTime: "duration", timePassed: false });
...
export const getTimeUnit = function getTimeUnit(arg0, arg1) {
  closure_0 = arg0;
  importDefault = arg1;
  const tmp2 = findLastIndexDefault(items, (unit) => f89435(unit.unit), items.findIndex((max) => {
    max = max.max;
  ...
  if (null != tmp2) {
    ...
    const found = arr.find((unit) => f89435(unit.unit));
    let unit = null;
    if (null != found) {
      unit = found.unit;
    }
...
export const getTimeAndUnit = function getTimeAndUnit(rounded, items) {
  ...
  if (null == rounded) {
    ...
    closure_0 = rounded;
    const f89435 = (dependencyMap) => closure_0.includes(dependencyMap);
    const tmp12 = f89435(4789)(items, (unit) => f89435(unit.unit), items.findIndex((max) => {
      max = max.max;
      let tmp = max.unit === obj.NONE;
      if (tmp) {
        tmp = closure_0 === max;
