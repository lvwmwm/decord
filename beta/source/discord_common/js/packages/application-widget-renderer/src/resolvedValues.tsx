...
// Function ID: 9208
// Name: resolvedValues
// Dependencies: [9208, 9209, 2]
// Exports: bindResolveFieldValue

// Module 9207 (resolvedValues)
...
function resolveFieldValue(image, items, applicationAssets) {
  ...
  if (null == image) {
    ...
    return tmp10;
  } else if (image.value_type === tmp19(9209).ApplicationWidgetFieldValueType.CUSTOM_STRING) {
    let tmp6 = null;
    if (image.presentation_type === tmp19(9208).ApplicationWidgetFieldPresentationType.TEXT) {
      tmp6 = null;
      if (items.includes(obj.STRING)) {
        const obj5 = { type: tmp7.STRING, value: image.value, presentationType: tmp19(9208).ApplicationWidgetFieldPresentationType.TEXT };
        tmp6 = obj5;
      }
      tmp7 = obj;
    }
    return tmp6;
  } else if (image.value_type === tmp19(9209).ApplicationWidgetFieldValueType.APPLICATION_ASSET) {
    if (items.includes(obj.MEDIA)) {
      ...
      if (null != found) {
        ...
        obj.media = size;
        obj.presentationType = tmp19(9208).ApplicationWidgetFieldPresentationType.IMAGE;
        tmp5 = obj;
      }
      return tmp5;
    } else {
