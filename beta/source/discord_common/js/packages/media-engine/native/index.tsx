...
// Function ID: 4815
// Name: destroy
// Dependencies: [32, 5, 4783, 4815, 4816, 4817, 4821, 4, 1998, 4813, 4822, 1344, 1369, 4824, 4884, 2]
...
import CameraDefault from "Camera" /* 4821 */;
import ConnectionDefault from "Connection" /* 4824 */;
import Devices from "Devices" /* 4884 */;
import _slicedToArray from "module_32" /* 32 */;
import asyncGeneratorStep from "asyncGeneratorStep" /* 5 */;
import TypedEventEmitter from "TypedEventEmitter" /* 4816 */;

const pollConnectionStatsDefault = tmp10(4822);
require = fn;
let Constants = fn(4783);
...
class MediaEngineNative extends tmp4 {
  constructor() {
    ...
    obj.handleVolumeChange = function handleVolumeChange(arg0, arg1) {
      obj.emit(BaseConnectionEvent.MediaEngineEvent.VolumeChange, arg0 * __initData2, arg1 * __initData2);
    };
    ...
    obj.handleRemoveListener = function handleRemoveListener(arg0) {
      if (arg0 === BaseConnectionEvent.MediaEngineEvent.VoiceActivity) {
        if (null != tmpResult.getVoiceEngine().setEmitVADLevel2) {
          const voiceEngine = tmp(1998).getVoiceEngine();
          voiceEngine.setEmitVADLevel2(obj.listenerCount(tmp(4813).MediaEngineEvent.VoiceActivity) > 0);
          const tmpResult3 = tmp(1998);
        } else {
          const voiceEngine1 = tmp(1998).getVoiceEngine();
          voiceEngine1.setEmitVADLevel(obj.listenerCount(tmp(4813).MediaEngineEvent.VoiceActivity) > 0, false, {});
          const tmpResult4 = tmp(1998);
        }
        tmpResult = tmp(1998);
      }
    };
          ...
          if (0 === dependencyMap) {
            if (arg0 === 1) {
              ...
              if (importDefault) {
                ...
                voiceEngine = v0(1998).getVoiceEngine();
            ...
            closure_129_0.emit(v0(4813).MediaEngineEvent.VoiceQueueMetrics, closure_128_1);
    ...
    if (tmp.ATTENUATION !== arg0) {
      if (tmp.VIDEO_HOOK !== arg0) {
        if (tmp.EXPERIMENTAL_SOUNDSHARE === arg0) {
          ...
          if (isDesktopResult) {
            isDesktopResult = tmp50(1998).supportsFeature(constants6.MEDIAPIPE);
            const tmp50Result = tmp50(1998);
          }
          if (!isDesktopResult) {
            isDesktopResult = tmp50(1998).supportsFeature(constants6.VIDEO_BACKGROUND_FILTER);
            const tmp50Result2 = tmp50(1998);
          }
...
prototype["connect"] = function connect(arg0, arg1, videoSupported) {
  ...
  obj3 = self(4824).create(arg0, arg1, videoSupported, flag);
  obj3.on(obj3(4813).BaseConnectionEvent.Destroy, (arg0) => {
  ...
  obj3.on(obj3(4813).BaseConnectionEvent.Connected, () => {
    obj3.setVideoBroadcast(self.shouldConnectionBroadcastVideo(obj3));
  });
  obj3.on(obj3(4813).BaseConnectionEvent.Silence, (arg0) => {
  ...
  obj3(1998).setProcessPriority(HIGH);
  if (null != videoSupported.threadPriorityConfiguration) {
    let voiceEngine = tmp(1998).getVoiceEngine();
    ...
    const tmpResult2 = tmp(1998);
  }
  self.emit(obj3(4813).MediaEngineEvent.Connection, obj3);
  return obj3;
};
prototype["shouldConnectionBroadcastVideo"] = function shouldConnectionBroadcastVideo(context) {
  ...
  if (hasDesktopSourceResult) {
    const self = this;
    hasDesktopSourceResult = this.videoInputDeviceId !== closure_1_15;
  }
  if (!hasDesktopSourceResult) {
...
prototype["setInputVolume"] = function setInputVolume(arg0) {
  ...
  voiceEngine.setInputVolume(tmp / __initData2);
};
prototype["setOutputVolume"] = function setOutputVolume(arg0) {
  ...
  voiceEngine.setOutputVolume(tmp / __initData2);
};
...
prototype["setAudioInputDevice"] = function setAudioInputDevice(audioInputDeviceId) {
  ...
  if (obj.supportsFeature(constants6.SET_AUDIO_DEVICE_BY_ID)) {
    ...
    const tmpResult = tmp(1998);
  } else {
    const audioInputDevices = tmp(4884).getAudioInputDevices();
...
prototype["setAudioOutputDevice"] = function setAudioOutputDevice(audioOutputDeviceId) {
  ...
  if (obj.supportsFeature(constants6.SET_AUDIO_DEVICE_BY_ID)) {
    ...
    const audioOutputDevices = tmp(4884).getAudioOutputDevices();
        ...
        if (0 === dependencyMap) {
          ...
          if (closure_128_1 === closure_129_1.videoInputDeviceId) {
            ...
            if (!obj9.supportsFeature(constants.SET_VIDEO_DEVICE_BY_ID)) {
              const voiceEngine = tmp2(1998).getVoiceEngine();
              let num3 = -1;
            ...
            if (forEach == closure_128_0) {
              ...
              const voiceEngine1 = tmp2(1998).getVoiceEngine();
              voiceEngine1.setVideoInputDevice(closure_128_2);
              const obj3 = tmp2(1998);
            }
...
prototype["setLoopback"] = function setLoopback(arg0, arg1) {
  ...
  if (null != tmp2Result4.getVoiceEngine().setEmitVADLevel2) {
    const voiceEngine1 = tmp2(1998).getVoiceEngine();
    if (!tmp) {
      tmp = self.listenerCount(tmp2(4813).MediaEngineEvent.VoiceActivity) > 0;
    }
    voiceEngine1.setEmitVADLevel2(tmp);
    const tmp2Result5 = tmp2(1998);
  } else {
    const voiceEngine2 = tmp2(1998).getVoiceEngine();
    let tmp6 = tmp;
    if (!tmp) {
      tmp6 = self.listenerCount(tmp2(4813).MediaEngineEvent.VoiceActivity) > 0;
    }
...
prototype["createReplayConnection"] = function createReplayConnection(arg0, arg1) {
  ...
  let tmp2 = null;
  if (null != replay) {
    replay.on(self(4813).BaseConnectionEvent.Destroy, (arg0) => {
      const connections = self.connections;
      connections.delete(arg0);
      if (self.connectionsEmpty()) {
        inject.setProcessPriority(constants.NORMAL);
    ...
    self(1998).setProcessPriority(constants.HIGH);
    self.emit(self(4813).MediaEngineEvent.Connection, replay);
    tmp2 = replay;
    const obj3 = self(1998);
  }
  return tmp2;
};
...
prototype["stopLocalAudioRecording"] = function stopLocalAudioRecording(arg0) {
  ...
  let tmp5 = this.listenerCount(tmp(4813).MediaEngineEvent.VoiceActivity) > 0;
  if (tmp5) {
    tmp5 = null != tmp(1998).getVoiceEngine().setEmitVADLevel2;
    const tmpResult = tmp(1998);
  }
  if (tmp5) {
    const voiceEngine1 = tmp(1998).getVoiceEngine();
