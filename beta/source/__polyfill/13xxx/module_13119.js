// Module ID: 13119
// Function ID: 13120
// Dependencies: [32, 13112, 13118, 13107, 13120, 13090, 13102, 13103, 13104, 13097, 13095, 13113, 13085, 13117, 13121, 13123, 13114, 13124, 13126, 13100]
...
function createChildOrRootSpan(forceTransaction) {
  ...
  if (obj.hasTracingEnabled()) {
    const isolationScope = tmp(13112).getIsolationScope();
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (!tmp9) {
          spanIsSampledResult = tmp(13090).spanIsSampled(parentSpan);
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(13126).SentrySpan(obj2);
        } else {
          const obj3 = { traceId };
          sentrySpan = new tmp(13118).SentryNonRecordingSpan(obj3);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(13090).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult12 = tmp(13090);
        const client = tmp(13112).getClient();
        if (client) {
        ...
        const tmpResult13 = tmp(13112);
        tmp(13090).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult14 = tmp(13090);
      }
      tmp(13123).logSpanStart(sentrySpan);
      const tmpResult15 = tmp(13123);
      const result = tmp(13114).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(13121).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult17 = tmp(13121);
      ...
      const tmpResult18 = tmp(13090);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj4, scope, tmp(13090).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(13090).spanIsSampled(parentSpan);
      tmp(13121).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult19 = tmp(13121);
    } else {
      const obj5 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(13121).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult20 = tmp(13121);
      }
    }
    const tmpResult = tmp(13112);
  } else {
    const sentryNonRecordingSpan = new tmp(13118).SentryNonRecordingSpan();
...
function _startRootSpan(name, arg1, parentSampled) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    sampleSpanResult = tmp(13124).sampleSpan(options, obj2);
    const tmpResult = tmp(13124);
  }
  ...
  obj4.attributes = { [_mod13100.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj4.sampled = tmp6;
  const sentrySpan = new tmp(13126).SentrySpan(obj4);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(13100).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  _require = arg1;
  const mainCarrier = require("module_13103").getMainCarrier();
  const obj = require("module_13103");
  const tmp = _require;
  const asyncContextStrategy = require("module_13104").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(13112).withScope((setPropagationContext) => {
      const result = setPropagationContext.setPropagationContext(_mod13097.propagationContextFromHeaders(dependencyMap, _slicedToArray));
      return closure_0();
    });
  }
  const obj2 = require("module_13104");
};
export const startInactiveSpan = function startInactiveSpan(experimental) {
  _require = experimental;
  let mainCarrier = require("module_13103").getMainCarrier();
  let obj = require("module_13103");
  const tmp3 = _require;
  const tmp4 = obj4;
  let asyncContextStrategy = require("module_13104").getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.startInactiveSpan) {
    ...
    if (experimental.scope) {
      let fn = (arg0) => _mod13112.withScope(experimental.scope, arg0);
    } else {
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = _mod13112.withScope((arg0) => {
            _null(dependencyMap[3])._setSpanForScope(arg0, c0);
            return closure_1(arg0);
          });
          const tmp2Result = _mod13112;
        }
    ...
    return fn(() => {
      ...
      const _getSpanForScopeResult = _mod13107._getSpanForScope(currentScope);
      let tmp5;
      if (_getSpanForScopeResult) {
        ...
        if (options.parentSpanIsAlwaysRootSpan) {
          rootSpan = tmp(13090).getRootSpan(_getSpanForScopeResult);
      ...
      if (experimental.onlyIfParent) {
        if (!tmp5) {
          let sentryNonRecordingSpan = new tmp(13118).SentryNonRecordingSpan();
        }
        return sentryNonRecordingSpan;
      }
...
export const startNewTrace = function startNewTrace(arg0) {
  _require = arg0;
  return require("module_13112").withScope((setPropagationContext) => {
    ...
    const tmp6 = _null;
    const mainCarrier = _mod13103.getMainCarrier();
    const tmpResult = _mod13103;
    const asyncContextStrategy = _mod13104.getAsyncContextStrategy(mainCarrier);
    if (asyncContextStrategy.withActiveSpan) {
      let withActiveSpanResult = asyncContextStrategy.withActiveSpan(null, tmp6);
    } else {
      withActiveSpanResult = tmp(13112).withScope((arg0) => {
        _null(dependencyMap[3])._setSpanForScope(arg0, c0);
        return closure_1(arg0);
      });
      const tmpResult4 = tmp(13112);
    }
    return withActiveSpanResult;
  });
};
export const startSpan = function startSpan(experimental, arg1) {
  ...
  if (asyncContextStrategy.startSpan) {
    ...
    if (experimental.startTime) {
      let obj4 = {};
      const merged1 = Object.assign(obj3);
      obj4.startTimestamp = tmp3(13090).spanTimeInputToSeconds(experimental.startTime);
      delete tmp[tmp2];
      tmp10 = obj4;
      const tmp3Result = tmp3(13090);
    }
    obj4 = tmp10;
    ({ forceTransaction: __SENTRY_SUPPRESS_TRACING__, parentSpan: createChildOrRootSpan } = experimental);
    return tmp3(13112).withScope(experimental.scope, () => {
      closure_0 = createChildOrRootSpan;
      return undefined !== createChildOrRootSpan ? ((arg0) => {
        dependencyMap = arg0;
        const mainCarrier = closure_0(13103).getMainCarrier();
        const obj = closure_0(13103);
        const tmp = closure_0;
        const tmp2 = closure_0;
        const asyncContextStrategy = closure_0(13104).getAsyncContextStrategy(mainCarrier);
        if (asyncContextStrategy.withActiveSpan) {
          let withActiveSpanResult = asyncContextStrategy.withActiveSpan(tmp, arg0);
        } else {
          withActiveSpanResult = tmp2(13112).withScope((arg0) => {
            _null(dependencyMap[3])._setSpanForScope(arg0, c0);
            return closure_1(arg0);
          });
          const tmp2Result = tmp2(13112);
        }
        return withActiveSpanResult;
      }) : ((fn) => fn())(() => {
        const currentScope = sentryNonRecordingSpan(13112).getCurrentScope();
        const obj = sentryNonRecordingSpan(13112);
        const _getSpanForScopeResult = sentryNonRecordingSpan(13107)._getSpanForScope(currentScope);
        let tmp5;
        if (_getSpanForScopeResult) {
          const client = tmp(13112).getClient();
          if (client) {
        ...
        if (sentryNonRecordingSpan.onlyIfParent) {
          if (!tmp5) {
            sentryNonRecordingSpan = new tmp(13118).SentryNonRecordingSpan();
          }
          tmp(13107)._setSpanForScope(currentScope, sentryNonRecordingSpan);
          const tmpResult5 = tmp(13107);
          return tmp(13120).handleCallbackErrors(() => dependencyMap(sentryNonRecordingSpan), () => {
    ...
    return tmp3(13112).withScope(experimental.scope, () => {
      ...
      return undefined !== createChildOrRootSpan ? ((arg0) => {
        ...
        if (sentryNonRecordingSpan.onlyIfParent) {
          ...
          tmp(13107)._setSpanForScope(currentScope, sentryNonRecordingSpan);
          const tmpResult5 = tmp(13107);
          return tmp(13120).handleCallbackErrors(() => dependencyMap(sentryNonRecordingSpan, finishAndSetSpan), () => {
            const status = sentryNonRecordingSpan(dependencyMap[5]).spanToJSON(sentryNonRecordingSpan).status;
            const isRecordingResult = sentryNonRecordingSpan.isRecording();
            let tmp4 = !isRecordingResult;
            if (isRecordingResult) {
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(13112).withScope((setSDKProcessingMetadata) => {
