// Module ID: 13105
// Function ID: 13106
// Dependencies: [32, 41, 42, 13097, 13078, 13050, 13104, 13061, 13057, 13073, 13074, 13106, 13093, 13094, 13077, 13099, 13086, 13107, 13090, 13108, 13055]
    ...
    value: function _processEvent(type, arg1, arg2) {
      ...
      return self._prepareEvent(type, arg1, arg2, type.sdkProcessingMetadata || {}.capturedSpanIsolationScope).then((result) => {
        ...
        if (originalException instanceof BaseClient(13107).SentryError) {
          ...
          const _HermesInternal = HermesInternal;
          const sentryError = new tmp(13107).SentryError("Event processing pipeline threw an error, original event will not be sent. Details have been sent as a new event.\nReason: " + originalException);
          throw sentryError;
        }
        tmp = BaseClient;
      });
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(13078).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(13050).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tunnel = self._options.tunnel;
          if (tunnel) {
            tunnel = tmp(13097).dsnToString(self._dsn);
            const tmpResult2 = tmp(13097);
          }
          self.sendEnvelope(tmp(13108).createClientReportEnvelope(_clearOutcomesResult, tunnel));
