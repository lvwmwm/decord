// Module ID: 10756
// Function ID: 10757
// Dependencies: [41, 42, 93, 95, 98, 10754, 10692, 10693, 10699]
...
const regExp = new RegExp("(?:on\\s*?)?(" + _mod10754.ORDINAL_NUMBER_PATTERN + ")(?:\\s*(?:au|\\-|\\\u2013|jusqu'au?|\\s)\\s*(" + _mod10754.ORDINAL_NUMBER_PATTERN + "))?(?:-|/|\\s*(?:de)?\\s*)(" + repeatedTimeunitPattern.matchAnyPattern(_mod10754.MONTH_DICTIONARY) + ")(?:(?:-|/|,?\\s*)(" + _mod10754.YEAR_PATTERN + "(?![^\\s]\\d)))?(?=\\W|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = FRMonthNameLittleEndianParser(10754).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      const result = FRMonthNameLittleEndianParser(10754).parseOrdinalNumberPattern(index[1]);
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", tmp2(10754).parseYear(index[4]));
        } else {
        ...
        if (index[2]) {
          ...
          const result1 = tmp2(10754).parseOrdinalNumberPattern(index[2]);
