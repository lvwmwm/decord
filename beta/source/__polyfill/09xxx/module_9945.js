// Module ID: 9945
// Function ID: 9946
// Dependencies: [41, 42, 93, 95, 98, 9939, 9940]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9940).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9939).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9940).exists(this);
      HashMD(9940).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9939).createView(content);
