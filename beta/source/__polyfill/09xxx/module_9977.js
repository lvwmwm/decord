// Module ID: 9977
// Function ID: 9978
// Dependencies: [41, 42, 93, 95, 98, 9971, 9972]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9972).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9971).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9972).exists(this);
      HashMD(9972).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9971).createView(content);
