// Module ID: 8196
// Function ID: 8197
// Dependencies: [32, 19, 17, 21, 1489, 1619, 5880, 8197, 5150, 8198, 8200, 8201, 8202, 8203]
...
function SceneView(arg0) {
  ...
  const callback = obj3.useCallback(num6(8197).debounce(tmp18, 100), []);
  let tmp21 = "usesNewAndroidHeaderHeightImplementation" in tmp8(5150).compatibilityFlags;
  if (tmp21) {
    tmp21 = true === tmp8(5150).compatibilityFlags.usesNewAndroidHeaderHeightImplementation;
  }
  _slicedToArray = 0;
  let num8 = 0;
  if (null == header) {
  ...
  let eventResult;
  const headerConfigProps = num6(8198).useHeaderConfigProps(obj5);
  if (null == header) {
  ...
  obj10.children = closure_10(num6(8200).AnimatedHeaderHeightContext.Provider, obj12);
  obj9.children = closure_10(num6(5150).ScreenStackItem, obj10);
...
export const NativeStackView = function NativeStackView(state) {
  ...
  const setNextDismissedKey = require("module_8201").useDismissedRouteError(state).setNextDismissedKey;
  let obj = require("module_8201");
  const invalidPreventRemoveError = require("module_8202").useInvalidPreventRemoveError(descriptors);
  const obj2 = require("module_8202");
  const modalRouteKeys = require("module_8203").getModalRouteKeys(state.routes, descriptors);
