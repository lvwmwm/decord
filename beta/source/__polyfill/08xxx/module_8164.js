// Module ID: 8164
// Function ID: 8165
// Dependencies: [32, 19, 17, 21, 1489, 1619, 5848, 8165, 5118, 8166, 8168, 8169, 8170, 8171]
...
function SceneView(arg0) {
  ...
  safeAreaInsets = num6(1619).useSafeAreaInsets();
  ...
  let num7 = 2;
  const tmp8Result5 = num6(1489);
  [tmp17, tmp18] = noop.useState(frameSize1);
  dependencyMap = tmp18;
  const tmp16 = _slicedToArray(noop.useState(frameSize1), 2);
  const callback = obj3.useCallback(num6(8165).debounce(tmp18, 100), []);
  let tmp21 = "usesNewAndroidHeaderHeightImplementation" in tmp8(5118).compatibilityFlags;
  if (tmp21) {
    tmp21 = true === tmp8(5118).compatibilityFlags.usesNewAndroidHeaderHeightImplementation;
  }
  _slicedToArray = 0;
  let num8 = 0;
  if (null == header) {
  ...
  let eventResult;
  const headerConfigProps = num6(8166).useHeaderConfigProps(obj5);
  if (null == header) {
  ...
  obj12.children = tmp38(num6(5848).HeaderHeightContext.Provider, obj13);
  obj10.children = closure_10(num6(8168).AnimatedHeaderHeightContext.Provider, obj12);
  obj9.children = closure_10(num6(5118).ScreenStackItem, obj10);
  return closure_10(num6(1489).NavigationProvider, obj9);
}
...
export const NativeStackView = function NativeStackView(state) {
  ...
  const setNextDismissedKey = require("module_8169").useDismissedRouteError(state).setNextDismissedKey;
  let obj = require("module_8169");
  const invalidPreventRemoveError = require("module_8170").useInvalidPreventRemoveError(descriptors);
  const obj2 = require("module_8170");
  const modalRouteKeys = require("module_8171").getModalRouteKeys(state.routes, descriptors);
