...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.getScrollResponder = () => f132869;
    tmp3Result.getScrollableNode = () => renderElement.findNodeHandle(f132869.getNativeScrollRef());
    tmp3Result.getInnerViewNode = () => renderElement.findNodeHandle(f132869._innerView.nativeInstance);
    tmp3Result.getInnerViewRef = () => f132869._innerView.nativeInstance;
    tmp3Result.getNativeScrollRef = () => f132869._scrollView.nativeInstance;
    tmp3Result.scrollTo = (num, arg1, arg2) => {
      ...
      const nativeScrollRef = f132869.getNativeScrollRef();
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      f132869._additionalScrollOffset = num;
      f132869._preventNegativeScrollOffset = _preventNegativeScrollOffset;
      if (null != f132869._innerView.nativeInstance) {
    ...
    tmp3Result.scrollResponderZoomTo = (animated, arg1) => {
      ...
      if ("animated" in animated) {
        f132869._animated = animated.animated;
        delete tmp[tmp2];
      ...
      if (null == _keyboardMetrics._keyboardMetrics) {
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f132869._keyboardMetrics) {
              screenY = obj._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + obj._additionalScrollOffset;
            let bound = sum;
            if (true === f132869._preventNegativeScrollOffset) {
              const _Math = Math;
              bound = Math.max(0, sum);
            }
            const point = { x: 0, y: bound, animated: true };
            f132869.scrollTo(point);
            f132869._additionalScrollOffset = 0;
            f132869._preventNegativeScrollOffset = false;
          } else {
    ...
    tmp3Result._handleScroll = (arg0) => {
      f132869._observedScrollSinceBecomingResponder = true;
      if (f132869.props.onScroll) {
        const props = tmp.props;
        props.onScroll(arg0);
      }
    };
    tmp3Result._handleLayout = (nativeEvent) => {
      if (true === f132869.props.invertStickyHeaders) {
        const obj2 = { layoutHeight: nativeEvent.nativeEvent.layout.height };
        obj.setState(obj2);
      }
      if (f132869.props.onLayout) {
        const props = obj.props;
        props.onLayout(nativeEvent);
      }
    };
    tmp3Result._handleContentOnLayout = (arg0) => {
      if (f132869.props.onContentSizeChange) {
        const props = f132869.props;
    ...
    f132869 = (arg0) => Object.assign(arg0, { getScrollResponder: f132869.getScrollResponder, getScrollableNode: f132869.getScrollableNode, getInnerViewNode: f132869.getInnerViewNode, getInnerViewRef: f132869.getInnerViewRef, getNativeScrollRef: f132869.getNativeScrollRef, scrollTo: f132869.scrollTo, scrollToEnd: f132869.scrollToEnd, flashScrollIndicators: f132869.flashScrollIndicators, scrollResponderZoomTo: f132869.scrollResponderZoomTo, scrollResponderScrollNativeHandleToKeyboard: f132869.scrollResponderScrollNativeHandleToKeyboard });
    closure_1 = undefined;
    obj4 = {
      getForwardingRef: c2(closure_4[29])((arg0) => {
            closure_0 = arg0;
    ...
    tmp3Result.scrollResponderKeyboardWillShow = (endCoordinates) => {
      f132869._keyboardMetrics = endCoordinates.endCoordinates;
      if (f132869.props.onKeyboardWillShow) {
        const props = f132869.props;
        props.onKeyboardWillShow(endCoordinates);
      }
    };
    tmp3Result.scrollResponderKeyboardWillHide = (arg0) => {
      f132869._keyboardMetrics = null;
      if (f132869.props.onKeyboardWillHide) {
        const props = tmp.props;
    ...
    tmp3Result.scrollResponderKeyboardDidShow = (endCoordinates) => {
      f132869._keyboardMetrics = endCoordinates.endCoordinates;
      if (f132869.props.onKeyboardDidShow) {
        const props = f132869.props;
    ...
    tmp3Result.scrollResponderKeyboardDidHide = (arg0) => {
      f132869._keyboardMetrics = null;
      if (f132869.props.onKeyboardDidHide) {
        const props = tmp.props;
    ...
    tmp3Result._handleMomentumScrollBegin = (arg0) => {
      ...
      f132869._lastMomentumScrollBeginTime = _performance.now();
      if (f132869.props.onMomentumScrollBegin) {
        const props = f132869.props;
    ...
    tmp3Result._handleMomentumScrollEnd = (arg0) => {
      ...
      const _performance = ScrollView.performance;
      f132869._lastMomentumScrollEndTime = _performance.now();
      if (f132869.props.onMomentumScrollEnd) {
        const props = f132869.props;
    ...
    tmp3Result._handleScrollBeginDrag = (arg0) => {
      ...
      if ("on-drag" === f132869.props.keyboardDismissMode) {
        dismissKeyboardDefault();
      }
      if (f132869.props.onScrollBeginDrag) {
    ...
    tmp3Result._handleScrollEndDrag = (nativeEvent) => {
      ...
      let _isAnimatingResult = f132869._isAnimating();
      ...
      if (f132869.props.onScrollEndDrag) {
        const props = tmp.props;
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return _performance.now() - f132869._lastMomentumScrollEndTime < 16 || f132869._lastMomentumScrollEndTime < f132869._lastMomentumScrollBeginTime;
    };
    tmp3Result._handleResponderGrant = (arg0) => {
      f132869._observedScrollSinceBecomingResponder = false;
      if (f132869.props.onResponderGrant) {
        const props = obj.props;
        props.onResponderGrant(arg0);
      }
      f132869._becameResponderWhileAnimating = f132869._isAnimating();
    };
    tmp3Result._handleResponderReject = () => {

    };
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      f132869._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      if (f132869.props.onResponderRelease) {
        const props = obj.props;
    ...
    tmp3Result._handleResponderTerminationRequest = () => !f132869._observedScrollSinceBecomingResponder;
    tmp3Result._handleScrollShouldSetResponder = () => true !== f132869.props.disableScrollViewPanResponder && f132869._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f132869.props.disableScrollViewPanResponder) {
        return false;
      } else {
    ...
    tmp3Result._softKeyboardIsDetached = () => {
      let tmp = null != f132869._keyboardMetrics;
      if (tmp) {
        tmp = 0 === f132869._keyboardMetrics.height;
      }
      return tmp;
    };
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      f132869._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      const keyboardShouldPersistTaps = f132869.props.keyboardShouldPersistTaps;
      let tmp = !keyboardShouldPersistTaps;
      if (keyboardShouldPersistTaps) {
        tmp = "never" === keyboardShouldPersistTaps;
      }
