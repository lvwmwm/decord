// Module ID: 7282
// Function ID: 7283
// Dependencies: [109, 41, 42, 93, 95, 98, 19, 17, 21, 7283, 7286, 1489, 5880, 1619, 7290, 7292]
...
const jsx = fn(21).jsx;
let GestureHandlerRootView = fn(7283).GestureHandlerRootView;
...
class StackView {
  constructor() {
    ...
    obj1 = { routes: [], previousState: "y", openingRouteKeys: "button", closingRouteKeys: null, replacingRouteKeys: null, descriptors: null };
    ...
    tmp3Result.renderHeader = (arg0) => {
      ...
      return jsx(closure_0(7286).HeaderContainer, {});
    };
          ...
          if (routeNames.includes(route.name)) {
            ...
            if (!routes.some((key) => key.key === route.key)) {
              navigation.dispatch((routes) => {
                ...
                const CommonActions = key(1489).CommonActions;
                const obj = {};
    ...
    tmp3Result.handleCloseRoute = (route) => {
      ...
      if (routes.some((key) => key.key === route.key)) {
        const obj2 = {};
        const StackActions = StackView(1489).StackActions;
            ...
            children(arg0) {
              ...
              return self(closure_0(state[12]).HeaderShownContext.Consumer, {
                children(isParentHeaderShown) {
                  ...
                  return jsx(StackView(7292).CardStack, { insets: preloadedDescriptors, isParentHeaderShown, isParentModal: preloadedDescriptors, getPreviousRoute: self.getPreviousRoute, routes, openingRouteKeys, closingRouteKeys, onOpenRoute: self.handleOpenRoute, onCloseRoute: self.handleCloseRoute, onTransitionStart: self.handleTransitionStart, onTransitionEnd: self.handleTransitionEnd, renderHeader: self.renderHeader, state, descriptors, onGestureStart: self.handleGestureStart, onGestureEnd: self.handleGestureEnd, onGestureCancel: self.handleGestureCancel, preloadedDescriptors });
                }
              });
            }
          });
        ...
        if (tmp6.key !== tmp7.key) {
          if (arr3.some((key) => key.key === closure_7.key)) {
            if (!substr1.some((key) => key.key === closure_6.key)) {
              ...
              tmp23 = arr3(7292).getAnimationEnabled(animation) && !found2.includes(tmp6.key);
            }
          }
          let animation1;
      ...
      if (tmp8) {
        ...
        HermesBuiltin.arraySpread(routes5.filter((key) => {
          if (typeof isAnimationEnabled === "function") {
            ...
            let animationEnabled = StackView(7292).getAnimationEnabled(animation);
            if (animationEnabled) {
