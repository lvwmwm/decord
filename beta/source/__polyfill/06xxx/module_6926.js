// Module ID: 6926
// Function ID: 6927
// Dependencies: [17, 6927, 6929, 6930, 6931]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6929).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6929).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6929).State.BEGAN) {
        if (handlerTag.state === tmp(6929).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6929).State.END) {
          if (handlerTag.oldState === tmp(6929).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6929).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6929).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6930).GestureStateManager;
      ...
      if (tmp(6931).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6931).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6931).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6931).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6927).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
