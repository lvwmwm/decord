// Module ID: 6947
// Function ID: 6948
// Dependencies: [32, 19, 6948, 6959, 6950, 6963, 6934]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6950).SingleGestureName.Tap !== type) {
      if (tmp(6950).SingleGestureName.LongPress !== type) {
        if (tmp(6950).SingleGestureName.Fling !== type) {
          if (tmp(6950).SingleGestureName.Native !== type) {
            if (tmp(6950).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6948).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6963").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6934).Reanimated;
    const enabledSharedValues = gesture(6948).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6963.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
