// Module ID: 6894
// Function ID: 6895
// Dependencies: [17, 6895, 6897, 6898, 6899]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6897).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6897).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6897).State.BEGAN) {
        if (handlerTag.state === tmp(6897).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6897).State.END) {
          if (handlerTag.oldState === tmp(6897).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6897).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6897).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6898).GestureStateManager;
      ...
      if (tmp(6899).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6899).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6899).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6899).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6895).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
