// Module ID: 6918
// Function ID: 6919
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6919, 6897, 6913, 6920, 6922, 6895, 6924, 6923, 6925, 6926, 6896, 6910]
// Exports: default

// Module 6918
import handlerIDToTag2 from "handlerIDToTag" /* 6895 */;
...
const jsx = fn(21).jsx;
fn(6919).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6897).State.UNDETERMINED]: undefined, [fn(6897).State.BEGAN]: "onBegan", [fn(6897).State.FAILED]: "onFailed", [fn(6897).State.CANCELLED]: "onCancelled", [fn(6897).State.ACTIVE]: "onActivated", [fn(6897).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6913.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6924).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6924).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6922).scheduleFlushOperations();
          const tmp2Result = tmp2(6922);
          tmp2(6923).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6924).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6923).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6923);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6922);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6926)(self.viewNode));
        } else {
          const _Error = Error;
