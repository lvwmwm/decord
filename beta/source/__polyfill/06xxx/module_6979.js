// Module ID: 6979
// Function ID: 6980
// Dependencies: [32, 19, 6980, 6991, 6982, 6995, 6966]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6982).SingleGestureName.Tap !== type) {
      if (tmp(6982).SingleGestureName.LongPress !== type) {
        if (tmp(6982).SingleGestureName.Fling !== type) {
          if (tmp(6982).SingleGestureName.Native !== type) {
            if (tmp(6982).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6980).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6995").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6966).Reanimated;
    const enabledSharedValues = gesture(6980).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6995.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
