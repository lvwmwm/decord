// Module ID: 6950
// Function ID: 6951
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6951, 6929, 6945, 6952, 6954, 6927, 6956, 6955, 6957, 6958, 6928, 6942]
// Exports: default

// Module 6950
import handlerIDToTag2 from "handlerIDToTag" /* 6927 */;
...
const jsx = fn(21).jsx;
fn(6951).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6929).State.UNDETERMINED]: undefined, [fn(6929).State.BEGAN]: "onBegan", [fn(6929).State.FAILED]: "onFailed", [fn(6929).State.CANCELLED]: "onCancelled", [fn(6929).State.ACTIVE]: "onActivated", [fn(6929).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6945.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6956).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6956).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6954).scheduleFlushOperations();
          const tmp2Result = tmp2(6954);
          tmp2(6955).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6956).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6955).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6955);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6954);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6958)(self.viewNode));
        } else {
          const _Error = Error;
