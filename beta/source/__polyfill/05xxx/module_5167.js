// Module ID: 5167
// Function ID: 5168
// Dependencies: [109, 19, 17, 21, 5168, 5169, 5170, 5166, 5171, 5172, 5174, 5157, 5175]

// Module 5167
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 5157 */;
import _modDef5168 from "module_5168" /* 5168 */;
import animatedComponentDefault from "animatedComponent" /* 5169 */;
import _mod5170 from "module_5170" /* 5170 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 5172 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef5168);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod5170.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(5166).screensEnabled();
    const tmp2Result = tmp2(5166);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5166).freezeEnabled();
    const tmp2Result6 = tmp2(5166);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5171).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5166).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5171).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5171);
      const sheetLargestUndimmedDetent = tmp2(5171).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5171);
      ...
      const sheetInitialDetentIndex1 = tmp2(5171).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5174).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5175).Provider, obj7);
      }
