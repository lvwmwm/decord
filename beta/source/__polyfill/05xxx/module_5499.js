// Module ID: 5499
// Function ID: 5500
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 5464, 5500, 5501, 5502]

// Module 5499
import _wrapNativeSuperDefault from "_wrapNativeSuper" /* 158 */;
import _modDef5500 from "module_5500" /* 5500 */;
import calculateGPSValueDefault from "calculateGPSValue" /* 5502 */;
...
function readTags(_raw, buffer, arg2) {
  try {
    ...
    ParseError(5464).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5501).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(5501);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
    ...
    for (const key10002 in arg0) {
      ...
      let item = arr2.forEach((attributes) => {
        ...
        if (typeof attributes.value === "object") {
          ParseError(5464).objectAssign(obj, parseNodeChildrenAsTags(attributes.value));
          const tmpResult = ParseError(5464);
        }
      });
      continue;
    }
  ...
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      ...
      if (typeof calculateGPSValueDefault[tmp2] === "function") {
        tmp12 = tmp13(5502)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(5502);
      }
      tmp13 = importDefault;
    }
    return tmp12;
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(5464).objectAssign(obj, obj4, obj6);
  const prop = iter.value["rdf:value"];
  const obj3 = ParseError(5464);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(5464).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(5464);
    const obj6 = {};
    ...
    ParseError(5464).objectAssign(obj.attributes, obj6);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(5464);
  }
  ParseError(5464).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
