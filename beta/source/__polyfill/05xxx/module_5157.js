// Module ID: 5157
// Function ID: 5158
// Dependencies: [109, 19, 17, 21, 5145, 5155, 5154, 5158, 5159, 5162, 5144, 5164, 5135]

// Module 5157
import _modDef5135 from "module_5135" /* 5135 */;
import _modDef5154 from "module_5154" /* 5154 */;
import styleDefault from "style" /* 5159 */;
...
export default noop.forwardRef(function ScreenStackItem(arg0, ref) {
  ...
  dependencyMap = flag5.useContext(tmp4(5155).RNSScreensRefContext);
  ...
  const effect = obj2.useEffect(() => {
    _modDef5154(false, "Dynamically changing header's visibility in modals will result in remounting the screen and losing all local state.");
  ...
  let obj = screenId(5145);
  const tmp3 = !hidden;
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp10;
  }
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp4(5158).isIOS26OrHigher;
  }
  _modDef5154(isIOS26OrHigher, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  if (screenId(5158).isIOS26OrHigher) {
    const obj5 = { edges: {}, children };
    tmp19Result = tmp19(tmp4(5162).SafeAreaView, obj5);
  }
  obj4.children = tmp19Result;
  obj3.children = closure_4(styleDefault, obj4);
  const items1 = [closure_4(screenId(5145).EdgeInsetApplicationContext.Provider, obj3), , ];
  const merged1 = Object.assign(headerConfig);
  items1[1] = closure_4(screenId(5144).ScreenStackHeaderConfig, {});
  let tmp19Result2 = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp19Result2) {
    const obj7 = { children: unstable_sheetFooter() };
    tmp19Result2 = tmp19(tmp4(5164).FooterComponent, obj7);
  }
  ...
  return closure_4(_modDef5135, obj8);
});
