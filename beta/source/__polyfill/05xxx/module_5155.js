// Module ID: 5155
// Function ID: 5156
// Dependencies: [109, 19, 17, 21, 5143, 5153, 5152, 5156, 5157, 5160, 5142, 5162, 5133]

// Module 5155
import _modDef5133 from "module_5133" /* 5133 */;
import _modDef5152 from "module_5152" /* 5152 */;
import styleDefault from "style" /* 5157 */;
...
export default noop.forwardRef(function ScreenStackItem(arg0, ref) {
  ...
  dependencyMap = flag5.useContext(tmp4(5153).RNSScreensRefContext);
  ...
  const effect = obj2.useEffect(() => {
    _modDef5152(false, "Dynamically changing header's visibility in modals will result in remounting the screen and losing all local state.");
  ...
  let obj = screenId(5143);
  const tmp3 = !hidden;
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp10;
  }
  if (isIOS26OrHigher) {
    isIOS26OrHigher = tmp4(5156).isIOS26OrHigher;
  }
  _modDef5152(isIOS26OrHigher, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  if (screenId(5156).isIOS26OrHigher) {
    const obj5 = { edges: {}, children };
    tmp19Result = tmp19(tmp4(5160).SafeAreaView, obj5);
  }
  obj4.children = tmp19Result;
  obj3.children = closure_4(styleDefault, obj4);
  const items1 = [closure_4(screenId(5143).EdgeInsetApplicationContext.Provider, obj3), , ];
  const merged1 = Object.assign(headerConfig);
  items1[1] = closure_4(screenId(5142).ScreenStackHeaderConfig, {});
  let tmp19Result2 = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp19Result2) {
    const obj7 = { children: unstable_sheetFooter() };
    tmp19Result2 = tmp19(tmp4(5162).FooterComponent, obj7);
  }
  ...
  return closure_4(_modDef5133, obj8);
});
