...
fn = function n(toValue, userConfig, callback) {
  ...
  __initData = callback;
  const fn = function u() {
    let obj = { damping: 10, mass: 1, stiffness: 100, overshootClamping: false, restDisplacementThreshold: 0.01, restSpeedThreshold: 2, velocity: 0, duration: 2000, dampingRatio: 0.5, reduceMotion: "unicodeVersion", clamp: "v" };
    ...
    obj.skipAnimation = !_mod1724.checkIfConfigIsValid(obj);
    if (0 === obj.duration) {
    ...
    let obj3 = {
      onFrame: function springOnFrame(toValue, lastTimestamp) {
        ...
        if (obj.skipAnimation) {
          ...
          if (zeta < 1) {
            ...
            let result1 = closure_0(1724).underDampedSpringCalculations(toValue, obj2);
            const obj3 = closure_0(1724);
          } else {
            obj = closure_0(1724);
          ...
          const result2 = closure_0(1724).isAnimationTerminatingCalculation(toValue, tmp2);
      ...
      onStart(toValue, current, arg2, lastTimestamp) {
        ...
        if (lastTimestamp) {
          ...
          const obj2 = closure_0(1724);
          ({ zeta: toValue.zeta, omega0: toValue.omega0, omega1: toValue.omega1 } = closure_0(1724).initialCalculations(mass, tmp));
          if (undefined !== tmp.clamp) {
            toValue.zeta = closure_0(1724).scaleZetaToMatchClamps(toValue, tmp.clamp);
            const obj3 = closure_0(1724);
          }
          const initialCalculationsResult = closure_0(1724).initialCalculations(mass, tmp);
        }
        let lastTimestamp1;
        if (lastTimestamp != null) {
    ...
    obj3.reduceMotion = _mod1681.getReduceMotionForAnimation(obj.reduceMotion);
    return obj3;
  };
  let obj = require("module_1681");
  fn.__closure = { userConfig, checkIfConfigIsValid: require("module_1724").checkIfConfigIsValid, underDampedSpringCalculations: require("module_1724").underDampedSpringCalculations, criticallyDampedSpringCalculations: require("module_1724").criticallyDampedSpringCalculations, isAnimationTerminatingCalculation: require("module_1724").isAnimationTerminatingCalculation, calculateNewMassToMatchDuration: require("module_1724").calculateNewMassToMatchDuration, initialCalculations: require("module_1724").initialCalculations, scaleZetaToMatchClamps: require("module_1724").scaleZetaToMatchClamps, toValue, callback, getReduceMotionForAnimation: require("module_1681").getReduceMotionForAnimation };
  fn.__workletHash = 3229069592929;
  fn.__initData = __initData;
  return obj.defineAnimation(toValue, fn);
};
fn.__closure = { defineAnimation: fn(1681).defineAnimation, checkIfConfigIsValid: fn(1724).checkIfConfigIsValid, underDampedSpringCalculations: fn(1724).underDampedSpringCalculations, criticallyDampedSpringCalculations: fn(1724).criticallyDampedSpringCalculations, isAnimationTerminatingCalculation: fn(1724).isAnimationTerminatingCalculation, calculateNewMassToMatchDuration: fn(1724).calculateNewMassToMatchDuration, initialCalculations: fn(1724).initialCalculations, scaleZetaToMatchClamps: fn(1724).scaleZetaToMatchClamps, getReduceMotionForAnimation: fn(1681).getReduceMotionForAnimation };
