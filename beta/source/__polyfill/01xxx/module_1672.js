// Module ID: 1672
// Function ID: 1673
// Dependencies: [41, 42, 93, 95, 98, 1673, 19, 17, 21, 1640, 1691, 38, 1732, 1733, 1741, 1743, 1681, 1662, 1744, 1746, 1740, 1724, 1677, 1666, 1648, 1750, 1665, 1734, 1736, 1751, 1752, 1753, 1775, 1776]
...
import NativeEventsManager from "NativeEventsManager" /* 1744 */;
...
const startObservingProgress = fn(1673);
...
module_1640.isWeb();
module_1640 = fn(1640);
let closure_14 = module_1640.isJest();
module_1640 = fn(1640);
let closure_15 = module_1640.isReact19();
module_1640 = fn(1640);
let closure_16 = module_1640.shouldBeUseWeb();
if (module_1640) {
  let result = fn(1691).configureWebLayoutAnimations();
  let obj5 = fn(1691);
}
let c17 = 0;
...
export const createAnimatedComponent = function createAnimatedComponent(name, arg1) {
        ...
        obj1 = {
          ...
          setLocalRef(arg0) {
                  if (arg0) {
                    ...
                    if (!closure_2_16) {
                      const result = AnimatedComponent(1681).enableLayoutAnimations(true, false);
                    ...
                    if (entering) {
                      isFabricResult = AnimatedComponent(1640).isFabric();
                      const obj3 = AnimatedComponent(1640);
                    }
                    if (!isFabricResult) {
                      isFabricResult = current;
                    }
                    if (!isFabricResult) {
                      isFabricResult = module_1640;
                    }
                    if (!isFabricResult) {
                      const result2 = obj._configureLayoutAnimation(AnimatedComponent(1662).LayoutAnimationType.ENTERING, obj.props.entering);
                    }
                  }
                }
        };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      let isFabricResult = _mod1640.isFabric();
      if (isFabricResult) {
        isFabricResult = -1 !== componentViewTag;
      }
      if (isFabricResult) {
        const PropsRegistryGarbageCollector = tmp17(1746).PropsRegistryGarbageCollector;
        PropsRegistryGarbageCollector.registerView(componentViewTag, self);
      }
      if (-1 !== componentViewTag) {
        const ComponentRegistry = tmp17(1740).ComponentRegistry;
        ComponentRegistry.register(componentViewTag, self);
      }
      const result2 = self._configureLayoutAnimation(tmp17(1662).LayoutAnimationType.LAYOUT, self.props.layout);
      const result3 = self._configureLayoutAnimation(tmp17(1662).LayoutAnimationType.EXITING, self.props.exiting);
      let tmp25 = tmp17;
      if (module_1640) {
        tmp25 = tmp17;
          ...
          if (self.props.entering) {
            if (obj3.getReducedMotionFromConfig(self.props.entering)) {
              ...
              if (current) {
                ...
                const result4 = tmp32(1691).startWebLayoutAnimation(self.props, _componentDOMRef, tmp32(1662).LayoutAnimationType.ENTERING);
                tmp25 = tmp32;
                const tmp32Result = tmp32(1691);
              }
            }
      ...
      if (!closure_16) {
        _willUnmount = tmp25(1640).isFabric();
        const tmp25Result = tmp25(1640);
      }
      if (_willUnmount) {
      ...
      if (_willUnmount) {
        const result5 = tmp25(1681).unmarkNodeAsRemovable(componentViewTag);
        const tmp25Result2 = tmp25(1681);
      }
      self._isFirstRender = false;
    }
  };
  let items = [
    entry,
    {
      ...
      value: function componentWillUnmount() {
        ...
        const componentViewTag = self.getComponentViewTag();
        let isFabricResult = AnimatedComponent(1640).isFabric();
        if (isFabricResult) {
          isFabricResult = -1 !== componentViewTag;
        }
        if (isFabricResult) {
          const PropsRegistryGarbageCollector = tmp4(1746).PropsRegistryGarbageCollector;
        ...
        if (-1 !== componentViewTag) {
          const ComponentRegistry = tmp4(1740).ComponentRegistry;
          ...
          if (self._componentDOMRef) {
            if (exiting) {
              if (tmp4Result.getReducedMotionFromConfig(exiting)) {
                ...
                const result2 = tmp4(1724).addHTMLMutationObserver();
                const tmp4Result8 = tmp4(1724);
                ({ props, _componentDOMRef } = self);
                const result3 = tmp4(1691).startWebLayoutAnimation(props, _componentDOMRef, tmp4(1662).LayoutAnimationType.EXITING);
                const tmp4Result9 = tmp4(1691);
              }
              tmp4Result = tmp4(1691);
            }
            const _viewInfo = self._viewInfo;
            let isFabricResult1 = !closure_1_16;
            if (!closure_1_16) {
              isFabricResult1 = tmp4(1640).isFabric();
              const tmp4Result10 = tmp4(1640);
            }
            if (isFabricResult1) {
              isFabricResult1 = shadowNodeWrapper;
            }
            if (isFabricResult1) {
              tmp4(1681).markNodeAsRemovable(shadowNodeWrapper);
              const tmp4Result11 = tmp4(1681);
            }
            ...
            if (!tmp4Result12.isFabric()) {
              if ("getReduceMotion" in exiting) {
                if (typeof exiting.getReduceMotion === "function") {
                  let reduceMotionFromConfig = tmp4(1677).getReduceMotionFromConfig(exiting.getReduceMotion());
                  const tmp4Result13 = tmp4(1677);
                }
                if (!reduceMotionFromConfig) {
                  const result4 = self._configureLayoutAnimation(tmp4(1662).LayoutAnimationType.EXITING, exiting);
                }
              }
              reduceMotionFromConfig = tmp4(1677).getReduceMotionFromConfig();
              const tmp4Result14 = tmp4(1677);
            }
            tmp4Result12 = tmp4(1640);
          }
        }
      }
    },
      ...
      value: function _getViewInfo() {
        ...
        if (undefined !== this._viewInfo) {
          ...
          if (closure_1_16) {
            ...
            const findHostInstanceResult = AnimatedComponent(1666).findHostInstance(self);
            if (findHostInstanceResult) {
              const viewInfo = tmp(1750).getViewInfo(findHostInstanceResult);
              ({ viewTag, viewName, viewConfig } = viewInfo);
              const tmpResult = tmp(1750);
              shadowNodeWrapperFromRef = null;
              if (tmpResult3.isFabric()) {
                shadowNodeWrapperFromRef = tmp(1665).getShadowNodeWrapperFromRef(self, findHostInstanceResult);
                const tmpResult4 = tmp(1665);
              }
              _componentDOMRef = null;
              tmpResult3 = tmp(1640);
            } else {
              const reanimatedError = new tmp(1648).ReanimatedError("Cannot find host instance for this component. Maybe it renders nothing?");
              throw reanimatedError;
            }
            const obj = AnimatedComponent(1666);
          }
    ...
    },
    {
      ...
      value: function componentDidUpdate(layout, arg1, arg2) {
        const self = this;
        const result = this._configureLayoutAnimation(AnimatedComponent(1662).LayoutAnimationType.LAYOUT, this.props.layout, layout.layout);
        const result1 = this._configureLayoutAnimation(AnimatedComponent(1662).LayoutAnimationType.EXITING, this.props.exiting, layout.exiting);
        if (!tmp5) {
        ...
        const _InlinePropManager = self._InlinePropManager;
        _InlinePropManager.attachInlineProps(self, self._getViewInfo());
        let _componentDOMRef = module_1640;
        if (module_1640) {
        ...
        if (_componentDOMRef) {
          tmp(1691).saveSnapshot(self._componentDOMRef);
          const tmpResult = tmp(1691);
        }
        if (module_1640) {
          if (arg2) {
            if (self.props.layout) {
              if (tmpResult3.getReducedMotionFromConfig(self.props.layout)) {
                ...
                const result4 = tmp(1691).tryActivateLayoutTransition(self.props, self._componentDOMRef, arg2);
                const tmpResult4 = tmp(1691);
              }
              tmpResult3 = tmp(1691);
            }
          }
    ...
    },
    {
      ...
      value: function _configureLayoutAnimation(EXITING, exiting, exiting2) {
        if (!map1) {
          if (exiting !== exiting2) {
            const self = this;
            const obj = AnimatedComponent(1751);
            if (obj2.isFabric()) {
              if (EXITING === tmp5(1662).LayoutAnimationType.ENTERING) {
                let reanimatedID = self.reanimatedID;
              }
              let maybeBuildResult = tmp3;
              if (tmp3) {
                let tmp8;
                if (EXITING !== tmp5(1662).LayoutAnimationType.LAYOUT) {
                ...
                maybeBuildResult = tmp5(1752).maybeBuild(tmp3, tmp8, AnimatedComponent.displayName);
                const tmp5Result = tmp5(1752);
              }
              const result = obj.updateLayoutAnimations(reanimatedID, EXITING, maybeBuildResult);
            }
            reanimatedID = self.getComponentViewTag();
            obj2 = AnimatedComponent(1640);
          }
        }
      }
    },
      ...
      value: function _configureSharedTransition(flag) {
        ...
        if (!module_1640) {
          ...
          if (sharedTransitionTag) {
            ...
            if (_sharedElementTransition2 == null) {
              _sharedElementTransition2 = new AnimatedComponent(1753).SharedTransition();
            }
        ...
        if (getReduceMotion) {
          if ("getReduceMotion" in getReduceMotion) {
            if (typeof getReduceMotion.getReduceMotion === "function") {
              let reduceMotionFromConfig = AnimatedComponent(1677).getReduceMotionFromConfig(getReduceMotion.getReduceMotion());
              const obj2 = AnimatedComponent(1677);
            }
            return reduceMotionFromConfig;
          }
        }
        reduceMotionFromConfig = AnimatedComponent(1677).getReduceMotionFromConfig();
      }
    },
    {
    ...
    },
    {
      ...
      value: function render() {
        ...
        let tmp3 = self._isFirstRender && module_1640 && result.entering;
        if (tmp3) {
  ...
  dependencyMap = tmp9;
  tmp9.contextType = AnimatedComponent(1775).SkipEnteringContext;
  tmp9.displayName = "AnimatedComponent(" + name.displayName || name.name || "Component" + ")";
  const componentWithRefResult = AnimatedComponent(1776).componentWithRef((arg0, forwardedRef) => {
