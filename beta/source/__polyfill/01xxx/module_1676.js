// Module ID: 1676
// Function ID: 1677
// Dependencies: [41, 42, 93, 95, 98, 1677, 19, 17, 21, 1644, 1695, 38, 1736, 1737, 1745, 1747, 1685, 1666, 1748, 1750, 1744, 1728, 1681, 1670, 1652, 1754, 1669, 1738, 1740, 1755, 1756, 1757, 1779, 1780]
// Exports: createAnimatedComponent

// Module 1676
import _modDef38 from "module_38" /* 38 */;
import _mod1644 from "module_1644" /* 1644 */;
import startWebLayoutAnimation from "startWebLayoutAnimation" /* 1695 */;
import NativeEventsManager from "NativeEventsManager" /* 1748 */;
...
const startObservingProgress = fn(1677);
...
module_1644.isWeb();
module_1644 = fn(1644);
let closure_14 = module_1644.isJest();
module_1644 = fn(1644);
let closure_15 = module_1644.isReact19();
module_1644 = fn(1644);
let closure_16 = module_1644.shouldBeUseWeb();
if (module_1644) {
  let result = fn(1695).configureWebLayoutAnimations();
  let obj5 = fn(1695);
}
let c17 = 0;
...
export const createAnimatedComponent = function createAnimatedComponent(name, arg1) {
        ...
        obj1 = {
          ...
          setLocalRef(arg0) {
                  if (arg0) {
                    ...
                    if (!closure_2_16) {
                      const result = AnimatedComponent(1685).enableLayoutAnimations(true, false);
                      const obj2 = AnimatedComponent(1685);
                    }
                    ...
                    if (entering) {
                      isFabricResult = AnimatedComponent(1644).isFabric();
                      const obj3 = AnimatedComponent(1644);
                    }
                    if (!isFabricResult) {
                      isFabricResult = current;
                    }
                    if (!isFabricResult) {
                      isFabricResult = module_1644;
                    }
                    if (!isFabricResult) {
                      const result2 = obj._configureLayoutAnimation(AnimatedComponent(1666).LayoutAnimationType.ENTERING, obj.props.entering);
                    }
                  }
                }
        };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      let isFabricResult = _mod1644.isFabric();
      if (isFabricResult) {
        isFabricResult = -1 !== componentViewTag;
      }
      if (isFabricResult) {
        const PropsRegistryGarbageCollector = tmp17(1750).PropsRegistryGarbageCollector;
        PropsRegistryGarbageCollector.registerView(componentViewTag, self);
      }
      if (-1 !== componentViewTag) {
        const ComponentRegistry = tmp17(1744).ComponentRegistry;
        ComponentRegistry.register(componentViewTag, self);
      }
      const result2 = self._configureLayoutAnimation(tmp17(1666).LayoutAnimationType.LAYOUT, self.props.layout);
      const result3 = self._configureLayoutAnimation(tmp17(1666).LayoutAnimationType.EXITING, self.props.exiting);
      let tmp25 = tmp17;
      if (module_1644) {
        tmp25 = tmp17;
          ...
          if (self.props.entering) {
            if (obj3.getReducedMotionFromConfig(self.props.entering)) {
              ...
              if (current) {
                ...
                const result4 = tmp32(1695).startWebLayoutAnimation(self.props, _componentDOMRef, tmp32(1666).LayoutAnimationType.ENTERING);
                tmp25 = tmp32;
                const tmp32Result = tmp32(1695);
              }
            }
      ...
      if (!closure_16) {
        _willUnmount = tmp25(1644).isFabric();
        const tmp25Result = tmp25(1644);
      }
      if (_willUnmount) {
      ...
      if (_willUnmount) {
        const result5 = tmp25(1685).unmarkNodeAsRemovable(componentViewTag);
        const tmp25Result2 = tmp25(1685);
      }
      self._isFirstRender = false;
    }
  };
  let items = [
    entry,
    {
      ...
      value: function componentWillUnmount() {
        ...
        const componentViewTag = self.getComponentViewTag();
        let isFabricResult = AnimatedComponent(1644).isFabric();
        if (isFabricResult) {
          isFabricResult = -1 !== componentViewTag;
        }
        if (isFabricResult) {
          const PropsRegistryGarbageCollector = tmp4(1750).PropsRegistryGarbageCollector;
        ...
        if (-1 !== componentViewTag) {
          const ComponentRegistry = tmp4(1744).ComponentRegistry;
          ...
          if (self._componentDOMRef) {
            if (exiting) {
              if (tmp4Result.getReducedMotionFromConfig(exiting)) {
                ...
                const result2 = tmp4(1728).addHTMLMutationObserver();
                const tmp4Result8 = tmp4(1728);
                ({ props, _componentDOMRef } = self);
                const result3 = tmp4(1695).startWebLayoutAnimation(props, _componentDOMRef, tmp4(1666).LayoutAnimationType.EXITING);
                const tmp4Result9 = tmp4(1695);
              }
              tmp4Result = tmp4(1695);
            }
            const _viewInfo = self._viewInfo;
            let isFabricResult1 = !closure_1_16;
            if (!closure_1_16) {
              isFabricResult1 = tmp4(1644).isFabric();
              const tmp4Result10 = tmp4(1644);
            }
            if (isFabricResult1) {
              isFabricResult1 = shadowNodeWrapper;
            }
            if (isFabricResult1) {
              tmp4(1685).markNodeAsRemovable(shadowNodeWrapper);
              const tmp4Result11 = tmp4(1685);
            }
            ...
            if (!tmp4Result12.isFabric()) {
              if ("getReduceMotion" in exiting) {
                if (typeof exiting.getReduceMotion === "function") {
                  let reduceMotionFromConfig = tmp4(1681).getReduceMotionFromConfig(exiting.getReduceMotion());
                  const tmp4Result13 = tmp4(1681);
                }
                if (!reduceMotionFromConfig) {
                  const result4 = self._configureLayoutAnimation(tmp4(1666).LayoutAnimationType.EXITING, exiting);
                }
              }
              reduceMotionFromConfig = tmp4(1681).getReduceMotionFromConfig();
              const tmp4Result14 = tmp4(1681);
            }
            tmp4Result12 = tmp4(1644);
          }
        }
      }
    },
      ...
      value: function _getViewInfo() {
        ...
        if (undefined !== this._viewInfo) {
          ...
          if (closure_1_16) {
            ...
            const findHostInstanceResult = AnimatedComponent(1670).findHostInstance(self);
            if (findHostInstanceResult) {
              const viewInfo = tmp(1754).getViewInfo(findHostInstanceResult);
              ({ viewTag, viewName, viewConfig } = viewInfo);
              const tmpResult = tmp(1754);
              shadowNodeWrapperFromRef = null;
              if (tmpResult3.isFabric()) {
                shadowNodeWrapperFromRef = tmp(1669).getShadowNodeWrapperFromRef(self, findHostInstanceResult);
                const tmpResult4 = tmp(1669);
              }
              _componentDOMRef = null;
              tmpResult3 = tmp(1644);
            } else {
              const reanimatedError = new tmp(1652).ReanimatedError("Cannot find host instance for this component. Maybe it renders nothing?");
              throw reanimatedError;
            }
            const obj = AnimatedComponent(1670);
          }
    ...
    },
    {
      ...
      value: function componentDidUpdate(layout, arg1, arg2) {
        const self = this;
        const result = this._configureLayoutAnimation(AnimatedComponent(1666).LayoutAnimationType.LAYOUT, this.props.layout, layout.layout);
        const result1 = this._configureLayoutAnimation(AnimatedComponent(1666).LayoutAnimationType.EXITING, this.props.exiting, layout.exiting);
        if (!tmp5) {
        ...
        const _InlinePropManager = self._InlinePropManager;
        _InlinePropManager.attachInlineProps(self, self._getViewInfo());
        let _componentDOMRef = module_1644;
        if (module_1644) {
        ...
        if (_componentDOMRef) {
          tmp(1695).saveSnapshot(self._componentDOMRef);
          ...
          if (arg2) {
            if (self.props.layout) {
              if (tmpResult3.getReducedMotionFromConfig(self.props.layout)) {
                ...
                const result4 = tmp(1695).tryActivateLayoutTransition(self.props, self._componentDOMRef, arg2);
    ...
    },
    {
      ...
      value: function _configureLayoutAnimation(EXITING, exiting, exiting2) {
        if (!map1) {
          if (exiting !== exiting2) {
            const self = this;
            const obj = AnimatedComponent(1755);
            if (obj2.isFabric()) {
              if (EXITING === tmp5(1666).LayoutAnimationType.ENTERING) {
                let reanimatedID = self.reanimatedID;
              }
              let maybeBuildResult = tmp3;
              if (tmp3) {
                let tmp8;
                if (EXITING !== tmp5(1666).LayoutAnimationType.LAYOUT) {
                ...
                maybeBuildResult = tmp5(1756).maybeBuild(tmp3, tmp8, AnimatedComponent.displayName);
                const tmp5Result = tmp5(1756);
              }
              const result = obj.updateLayoutAnimations(reanimatedID, EXITING, maybeBuildResult);
            }
            reanimatedID = self.getComponentViewTag();
            obj2 = AnimatedComponent(1644);
          }
        }
      }
    },
      ...
      value: function _configureSharedTransition(flag) {
        ...
        if (!module_1644) {
          ...
          if (sharedTransitionTag) {
            ...
            if (_sharedElementTransition2 == null) {
              _sharedElementTransition2 = new AnimatedComponent(1757).SharedTransition();
            }
        ...
        if (getReduceMotion) {
          if ("getReduceMotion" in getReduceMotion) {
            if (typeof getReduceMotion.getReduceMotion === "function") {
              let reduceMotionFromConfig = AnimatedComponent(1681).getReduceMotionFromConfig(getReduceMotion.getReduceMotion());
              const obj2 = AnimatedComponent(1681);
            }
            return reduceMotionFromConfig;
          }
        }
        reduceMotionFromConfig = AnimatedComponent(1681).getReduceMotionFromConfig();
      }
    },
    {
    ...
    },
    {
      ...
      value: function render() {
        ...
        let tmp3 = self._isFirstRender && module_1644 && result.entering;
        if (tmp3) {
  ...
  dependencyMap = tmp9;
  tmp9.contextType = AnimatedComponent(1779).SkipEnteringContext;
  tmp9.displayName = "AnimatedComponent(" + name.displayName || name.name || "Component" + ")";
  const componentWithRefResult = AnimatedComponent(1780).componentWithRef((arg0, forwardedRef) => {
