// Module ID: 1677
// Function ID: 1678
// Dependencies: [1640, 1662, 1648, 1678, 1679, 1680, 1644]
    ...
    if (!module_1640) {
      ...
      if (!factory) {
        if (!obj.isWorkletFunction(factory)) {
          const reanimatedError = new tmp5(1648).ReanimatedError("The easing function is not a worklet. Please make sure you import `Easing` from react-native-reanimated.");
...
assertEasingIsWorklet.__closure = { SHOULD_BE_USE_WEB: module_1640, isWorkletFunction: _mod1662.isWorkletFunction };
...
function recognizePrefixSuffix(current) {
  if (typeof current === "string") {
    ...
    if (match) {
      ...
      const reanimatedError = new _mod1648.ReanimatedError("Couldn't parse animation value.");
      throw reanimatedError;
    }
  } else {
...
const uiValue = _mod1678.ReducedMotionManager.uiValue;
function getReduceMotionFromConfig(reduceMotion) {
  if (reduceMotion) {
    if (reduceMotion !== _mod1662.ReduceMotion.System) {
      value = reduceMotion === _mod1662.ReduceMotion.Always;
    }
    return value;
  }
  value = uiValue.value;
}
let obj = { SHOULD_BE_USE_WEB: module_1640, isWorkletFunction: _mod1662.isWorkletFunction };
getReduceMotionFromConfig.__closure = { ReduceMotion: _mod1662.ReduceMotion, isReduceMotionOnUI: uiValue };
...
function applyProgressToMatrix(arg0, arr, arr) {
  const obj = _mod1679;
  const obj2 = _mod1679;
  return obj.addMatrices(arr, obj2.scaleMatrix(_mod1679.subtractMatrices(arr, arr), arg0));
}
let obj2 = { ReduceMotion: _mod1662.ReduceMotion, isReduceMotionOnUI: uiValue };
applyProgressToMatrix.__closure = { addMatrices: _mod1679.addMatrices, scaleMatrix: _mod1679.scaleMatrix, subtractMatrices: _mod1679.subtractMatrices };
...
function decorateAnimation(isHigherOrder) {
  ...
  if (isHigherOrder.isHigherOrder) {
    ...
    function colorOnFrame(nonscaledCurrent, arg1) {
      ...
      nonscaledCurrent(1680).clampRGBA(items);
      nonscaledCurrent.nonscaledCurrent = items;
      const obj = nonscaledCurrent(1680);
      const obj2 = nonscaledCurrent(1680);
      nonscaledCurrent.current = obj2.rgbaArrayToRGBAColor(nonscaledCurrent(1680).toGammaSpace(items));
      return c3;
    }
    function transformationMatrixOnFrame(arg0, arg1) {
      ...
      const item = items1.forEach((item, index) => {
        if (typeof objectOnFrame === "function") {
          const obj = fn(1679);
          const obj2 = fn(1679);
          return tmp2(obj.addMatrices(tmp4, obj2.scaleMatrix(fn(1679).subtractMatrices(tmp5, tmp4), tmp3)));
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      });
      ...
      const item1 = items3.forEach((item, index) => {
        if (typeof closure_2_9 === "function") {
          ...
          items2.push(fn(1679).getRotationMatrix(sum, item));
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      });
      ...
      let obj2 = stopMatrices(1679);
      const obj3 = stopMatrices(1679);
      const multiplyMatricesResult = obj2.multiplyMatrices(tmp7, stopMatrices(1679).multiplyMatrices(tmp8, tmp9));
      const obj4 = stopMatrices(1679);
      const obj5 = stopMatrices(1679);
      const obj6 = stopMatrices(1679);
      arg0.current = obj4.flatten(obj5.multiplyMatrices(obj6.multiplyMatrices(tmp4, stopMatrices(1679).multiplyMatrices(tmp5, multiplyMatricesResult)), tmp3));
      ...
      if (reduceMotion.reduceMotion) {
        ...
        if (obj.isColor(current)) {
          ...
          if (tmp3Result.isColor(current)) {
            const tmp3Result13 = tmp3(1680);
            closure_129_4 = tmp3Result13.toLinearSpace(tmp3(1680).convertToRGBA(reduceMotion.current));
            const tmp3Result14 = tmp3(1680);
            const tmp3Result15 = tmp3(1680);
            closure_129_3 = tmp3Result15.toLinearSpace(tmp3(1680).convertToRGBA(current));
            if (reduceMotion.toValue) {
              const tmp3Result17 = tmp3(1680);
              closure_129_5 = tmp3Result17.toLinearSpace(tmp3(1680).convertToRGBA(reduceMotion.toValue));
              const tmp3Result18 = tmp3(1680);
            }
            const tmp3Result16 = tmp3(1680);
          }
          ...
          tmp3Result = tmp3(1680);
          tmp3(1680).clampRGBA(items);
          const tmp3Result19 = tmp3(1680);
          const tmp3Result20 = tmp3(1680);
          reduceMotion.current = tmp3Result20.rgbaArrayToRGBAColor(tmp3(1680).toGammaSpace(items));
          reduceMotion.onFrame = colorOnFrame;
          const tmp3Result21 = tmp3(1680);
        } else {
          if (tmp3Result22.isAffineMatrixFlat(current)) {
            reduceMotion.startMatrices = tmp3(1679).decomposeMatrixIntoMatricesAndAngles(current);
            const tmp3Result23 = tmp3(1679);
            reduceMotion.stopMatrices = tmp3(1679).decomposeMatrixIntoMatricesAndAngles(reduceMotion.toValue);
            const _Object = Object;
              ...
              if (typeof colorOnFrame === "function") {
                if (typeof current === "string") {
                  ...
                  if (match) {
                    ...
                    const reanimatedError = new tmp3(1648).ReanimatedError("Couldn't parse animation value.");
                    throw reanimatedError;
                  }
                } else {
                ...
                if (typeof tmp21 === "function") {
                  if (typeof str3 === "string") {
                    ...
                    if (match1) {
                      ...
                      const reanimatedError1 = new tmp3(1648).ReanimatedError("Couldn't parse animation value.");
                      ...
                      if (typeof tmp21 === "function") {
                        if (typeof str7 === "string") {
                          ...
                          if (match2) {
                            ...
                            const reanimatedError2 = new tmp3(1648).ReanimatedError("Couldn't parse animation value.");
...
let obj3 = { addMatrices: _mod1679.addMatrices, scaleMatrix: _mod1679.scaleMatrix, subtractMatrices: _mod1679.subtractMatrices };
decorateAnimation.__closure = { getReduceMotionFromConfig, recognizePrefixSuffix, isColor: _mod1680.isColor, toLinearSpace: _mod1680.toLinearSpace, convertToRGBA: _mod1680.convertToRGBA, clampRGBA: _mod1680.clampRGBA, rgbaArrayToRGBAColor: _mod1680.rgbaArrayToRGBAColor, toGammaSpace: _mod1680.toGammaSpace, decomposeMatrixIntoMatricesAndAngles: _mod1679.decomposeMatrixIntoMatricesAndAngles, applyProgressToMatrix, applyProgressToNumber, getRotationMatrix: _mod1679.getRotationMatrix, multiplyMatrices: _mod1679.multiplyMatrices, flatten: _mod1679.flatten, isAffineMatrixFlat: _mod1679.isAffineMatrixFlat };
...
defineAnimation.__closure = { IN_STYLE_UPDATER: React2, decorateAnimation, SHOULD_BE_USE_WEB: module_1640 };
...
let obj4 = { getReduceMotionFromConfig, recognizePrefixSuffix, isColor: _mod1680.isColor, toLinearSpace: _mod1680.toLinearSpace, convertToRGBA: _mod1680.convertToRGBA, clampRGBA: _mod1680.clampRGBA, rgbaArrayToRGBAColor: _mod1680.rgbaArrayToRGBAColor, toGammaSpace: _mod1680.toGammaSpace, decomposeMatrixIntoMatricesAndAngles: _mod1679.decomposeMatrixIntoMatricesAndAngles, applyProgressToMatrix, applyProgressToNumber, getRotationMatrix: _mod1679.getRotationMatrix, multiplyMatrices: _mod1679.multiplyMatrices, flatten: _mod1679.flatten, isAffineMatrixFlat: _mod1679.isAffineMatrixFlat };
let obj5 = { IN_STYLE_UPDATER: React2, decorateAnimation, SHOULD_BE_USE_WEB: module_1640 };
