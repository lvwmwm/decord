...
fn = function n(config, _animationToClamp) {
  ...
  const fn = function u() {
    ...
    if (undefined !== config.min) {
      strippedValue = config(1681).recognizePrefixSuffix(range.min).strippedValue;
      const obj = config(1681);
    }
    let strippedValue1;
    if (undefined !== range.max) {
      strippedValue1 = config(1681).recognizePrefixSuffix(range.max).strippedValue;
      const obj2 = config(1681);
    }
    const obj3 = {
      isHigherOrder: true,
      onFrame: function clampOnFrame(arg0, arg1) {
        if (undefined === previousAnimation.current) {
          ...
          const result = _mod1681.recognizePrefixSuffix(tmp.current);
      ...
      reduceMotion: config(1681).getReduceMotionForAnimation(range.reduceMotion)
    };
    return obj3;
  };
  let obj = require("module_1681");
  fn.__closure = { _animationToClamp, config, recognizePrefixSuffix: require("module_1681").recognizePrefixSuffix, logger: require("module_1645").logger, getReduceMotionForAnimation: require("module_1681").getReduceMotionForAnimation };
  fn.__workletHash = 9293031098818;
  fn.__initData = __initData;
  return obj.defineAnimation(_animationToClamp, fn);
};
fn.__closure = { defineAnimation: fn(1681).defineAnimation, recognizePrefixSuffix: fn(1681).recognizePrefixSuffix, logger: fn(1645).logger, getReduceMotionForAnimation: fn(1681).getReduceMotionForAnimation };
