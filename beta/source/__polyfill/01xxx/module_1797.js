// Module ID: 1797
// Function ID: 1798
// Dependencies: [19, 1666, 1685, 1648]
...
function adjustRotationToInterfaceOrientation(pitch) {
  ...
  if (interfaceOrientation === _mod1666.InterfaceOrientation.ROTATION_90) {
    pitch.pitch = roll;
    pitch.roll = -pitch;
    const _Math2 = Math;
    pitch.yaw = yaw - Math.PI / 2;
  } else if (interfaceOrientation === tmp(1666).InterfaceOrientation.ROTATION_270) {
    pitch.pitch = -roll;
    pitch.roll = pitch;
    const _Math = Math;
    pitch.yaw = yaw + Math.PI / 2;
  } else if (interfaceOrientation === tmp(1666).InterfaceOrientation.ROTATION_180) {
...
adjustRotationToInterfaceOrientation.__closure = { InterfaceOrientation: _mod1666.InterfaceOrientation, eulerToQuaternion };
...
let obj = { InterfaceOrientation: _mod1666.InterfaceOrientation, eulerToQuaternion };
adjustVectorToInterfaceOrientation.__closure = { InterfaceOrientation: _mod1666.InterfaceOrientation };
...
export const useAnimatedSensor = function useAnimatedSensor(arg0, adjustToInterfaceOrientation) {
  ...
  const tmp13 = tmpResult(() => {
    ...
    return { interval: "auto", adjustToInterfaceOrientation: true, iosReferenceFrame: _mod1666.IOSReferenceFrame.Auto };
  }, items);
  let config = tmp13;
  let obj2 = {
  ...
  config(() => {
    ...
    const fn = function n(pitch) {
      if (adjustToInterfaceOrientation) {
        if (closure_0 === _mod1666.SensorType.ROTATION) {
          ...
          if (interfaceOrientation === _mod1666.InterfaceOrientation.ROTATION_90) {
            pitch.x = -y;
            pitch.y = x;
          } else if (interfaceOrientation === _mod1666.InterfaceOrientation.ROTATION_270) {
            pitch.x = y;
            pitch.y = -x;
          } else if (interfaceOrientation === _mod1666.InterfaceOrientation.ROTATION_180) {
            pitch.x = pitch.x * -1;
            pitch.y = pitch.y * -1;
          }
        } else {
