// Module ID: 1558
// Function ID: 1559
// Dependencies: [32, 109, 19, 21, 1488, 1531, 1532, 1559, 1525, 1516, 1507, 1501, 1514, 1506, 1548, 1489, 1560, 1561, 1505, 1512, 1563, 1508, 1509, 1564, 1566, 1517, 1567, 1568, 1569, 1570, 1574, 1575, 1553, 1550, 1576, 1526, 1555]
// Exports: useNavigationBuilder

// Module 1558
import deepFreeze from "deepFreeze" /* 1505 */;
...
export const useNavigationBuilder = function useNavigationBuilder(StackRouter, UNSTABLE_routeNamesChangeBehavior) {
  ...
  dependencyMap = require("module_1559").useRegisterNavigator();
  const context = noop.useContext(require("module_1525").NavigationRouteContext);
  ...
  if (mapped.length) {
    ...
    const tmp35 = _modDef1506((current) => {
      if (ref.current) {
    ...
    let found = keys.filter((item) => {
      ...
      if (tmp3) {
        tmp3 = !_mod1548.isArrayEqual(tmp, tmp2);
      }
      return tmp3;
    });
    ...
    if (null != tmp43) {
          ...
          if (isNavigationState(context.params.state)) {
            if (!tmp5) {
              ...
              if (null != tmp61) {
                ...
                const CommonActions = tmp30(1489).CommonActions;
          ...
          if (typeof context.params.screen === "string") {
            ...
            if ("lastUnhandled" === UNSTABLE_routeNamesChangeBehavior.UNSTABLE_routeNamesChangeBehavior) {
              if (!mapped.includes(context.params.screen)) {
                ...
                tmp67 = null == tmp66 || tmp34(1560)(tmp66, tmp43);
              }
            }
      ...
      const scheduleUpdate = tmp30(1561).useScheduleUpdate(() => {
        if (closure_38) {
      ...
      const tmp30Result = tmp30(1561);
      const eventEmitter = tmp30(1512).useEventEmitter((target) => {
        const items = [];
      ...
      const tmp30Result14 = tmp30(1512);
      const obj11 = { state: tmp58, emitter: eventEmitter };
      const focusEvents = tmp30(1563).useFocusEvents(obj11);
      ...
      const tmp30Result15 = tmp30(1563);
      const childListeners = tmp30(1508).useChildListeners();
      ({ listeners, addListener } = childListeners);
      const tmp30Result16 = tmp30(1508);
      const keyedChildListeners = tmp30(1509).useKeyedChildListeners();
      ({ keyedListeners, addKeyedListener } = keyedChildListeners);
      const tmp30Result17 = tmp30(1509);
      ...
      const onAction = tmp30(1564).useOnAction(obj12);
      const tmp30Result18 = tmp30(1564);
      ...
      const onRouteFocus = tmp30(1566).useOnRouteFocus(obj14);
      closure_41 = obj10.useContext(tmp30(1517).UnhandledActionContext);
      const tmp30Result19 = tmp30(1566);
      const tmp85 = tmp34(1506)((type) => {
        if ("lastUnhandled" === UNSTABLE_routeNamesChangeBehavior.UNSTABLE_routeNamesChangeBehavior) {
      ...
      const navigationHelpers = tmp30(1567).useNavigationHelpers(obj15);
      const tmp30Result20 = tmp30(1567);
      const obj16 = { navigation: navigationHelpers, focusedListeners: listeners.focus };
      const focusedListenersChildrenAdapter = tmp30(1568).useFocusedListenersChildrenAdapter(obj16);
      const tmp30Result21 = tmp30(1568);
      const obj17 = { getState: tmp75, getStateListeners: keyedListeners.getState };
      const onGetState = tmp30(1569).useOnGetState(obj17);
      const tmp30Result22 = tmp30(1569);
      ...
      const descriptors1 = tmp30(1570).useDescriptors(obj18);
      const descriptors = descriptors1.descriptors;
      const tmp30Result23 = tmp30(1570);
      const obj19 = { state: tmp58, navigation: navigationHelpers, descriptors };
      const currentRender = tmp30(1574).useCurrentRender(obj19);
      const tmp30Result24 = tmp30(1574);
      const obj20 = {
        state: tmp58,
        navigation: navigationHelpers,
        describe: descriptors1.describe,
        descriptors,
        NavigationContent: tmp30(1575).useComponent((children) => {
    ...
    const tmp30Result26 = require("module_1548");
    rehydratedState = tmp39;
    if (!tmp51) {
    ...
    tmp51 = require("module_1548").isArrayEqual(tmp39.routeNames, mapped) && 0 === found.length;
  } else {
    let _Error = Error;
