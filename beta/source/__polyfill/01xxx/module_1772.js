  ...
  constructor() {
    ...
    tmp3Result.build = () => {
      ...
      const fn = function t(originX) {
        ...
        const withTimingResult = delayFunction(1713).withTiming(0, { duration: result });
        size.opacity = delayFunction(delay, obj3.withSequence(withTimingResult, delayFunction(1713).withTiming(1, { duration: result })));
        const obj5 = { duration: result };
        const obj6 = delayFunction(1713);
        const sum = delay + result;
        const obj8 = delayFunction(1713);
        size.originX = obj8.withDelay(sum, delayFunction(1713).withTiming(originX.targetOriginX, { duration: 0 }));
        const obj9 = delayFunction(1713);
        const sum1 = delay + result;
        const obj10 = delayFunction(1713);
        size.originY = obj10.withDelay(sum1, delayFunction(1713).withTiming(originX.targetOriginY, { duration: 0 }));
        const obj11 = delayFunction(1713);
        const sum2 = delay + result;
        const obj12 = delayFunction(1713);
        size.width = obj12.withDelay(sum2, delayFunction(1713).withTiming(originX.targetWidth, { duration: 0 }));
        const obj13 = delayFunction(1713);
        const sum3 = delay + result;
        const obj14 = delayFunction(1713);
        size.height = obj14.withDelay(sum3, delayFunction(1713).withTiming(originX.targetHeight, { duration: 0 }));
        obj.animations = size;
        obj.callback = callbackV;
        return obj;
      };
      fn.__closure = { delayFunction, delay, withSequence: FadingTransition(1713).withSequence, withTiming: FadingTransition(1713).withTiming, halfDuration: result, withDelay: FadingTransition(1713).withDelay, callback: callbackV };
...
_inherits(FadingTransition, fn(1711).BaseAnimationBuilder);
const entry = {
  key: "createInstance",
