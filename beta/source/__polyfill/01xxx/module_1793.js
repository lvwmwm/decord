// Module ID: 1793
// Function ID: 1794
// Dependencies: [19, 1662, 1681, 1644]
...
function adjustRotationToInterfaceOrientation(pitch) {
  ...
  if (interfaceOrientation === _mod1662.InterfaceOrientation.ROTATION_90) {
    pitch.pitch = roll;
    pitch.roll = -pitch;
    const _Math2 = Math;
    pitch.yaw = yaw - Math.PI / 2;
  } else if (interfaceOrientation === tmp(1662).InterfaceOrientation.ROTATION_270) {
    pitch.pitch = -roll;
    pitch.roll = pitch;
    const _Math = Math;
    pitch.yaw = yaw + Math.PI / 2;
  } else if (interfaceOrientation === tmp(1662).InterfaceOrientation.ROTATION_180) {
...
adjustRotationToInterfaceOrientation.__closure = { InterfaceOrientation: _mod1662.InterfaceOrientation, eulerToQuaternion };
...
let obj = { InterfaceOrientation: _mod1662.InterfaceOrientation, eulerToQuaternion };
adjustVectorToInterfaceOrientation.__closure = { InterfaceOrientation: _mod1662.InterfaceOrientation };
...
export const useAnimatedSensor = function useAnimatedSensor(arg0, adjustToInterfaceOrientation) {
  ...
  const tmp13 = tmpResult(() => {
    ...
    return { interval: "auto", adjustToInterfaceOrientation: true, iosReferenceFrame: _mod1662.IOSReferenceFrame.Auto };
  }, items);
  let config = tmp13;
  let obj2 = {
  ...
  config(() => {
    ...
    const fn = function n(pitch) {
      if (adjustToInterfaceOrientation) {
        if (closure_0 === _mod1662.SensorType.ROTATION) {
          ...
          if (interfaceOrientation === _mod1662.InterfaceOrientation.ROTATION_90) {
            pitch.x = -y;
            pitch.y = x;
          } else if (interfaceOrientation === _mod1662.InterfaceOrientation.ROTATION_270) {
            pitch.x = y;
            pitch.y = -x;
          } else if (interfaceOrientation === _mod1662.InterfaceOrientation.ROTATION_180) {
            pitch.x = pitch.x * -1;
            pitch.y = pitch.y * -1;
          }
        } else {
