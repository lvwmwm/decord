...
// Function ID: 9943
// Name: pbkdf2Init
// Dependencies: [5, 9940, 9939, 9943]
// Exports: pbkdf2, pbkdf2Async

// Module 9942 (pbkdf2Init)
import _asyncLoop from "_asyncLoop" /* 9939 */;
import number from "number" /* 9940 */;
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9939).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9943).hmac;
    const obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9939).toBytes(B);
