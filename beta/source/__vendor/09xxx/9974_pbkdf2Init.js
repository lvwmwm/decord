...
// Function ID: 9975
// Name: pbkdf2Init
// Dependencies: [5, 9972, 9971, 9975]
// Exports: pbkdf2, pbkdf2Async

// Module 9974 (pbkdf2Init)
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9971).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9975).hmac;
    const obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9971).toBytes(B);
