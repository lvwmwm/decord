...
// Function ID: 6981
// Name: hash
// Dependencies: [32, 6967, 6966, 6981, 6990]
...
import allowedNativeProps from "allowedNativeProps" /* 6981 */;
...
let c4 = 1.618;
fn(6967).NativeProxy.updateGestureHandlerConfig;
...
function maybeUnpackValue(enabled) {
  const Reanimated = _mod6966.Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: fn(6966).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6966.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6966).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6966").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let tmp8 = tmp7;
      let tmp10 = _require;
      let Reanimated = require("module_6966").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6966).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6966.Reanimated;
          const fn = function t() {
