...
// Function ID: 6949
// Name: hash
// Dependencies: [32, 6935, 6934, 6949, 6958]
...
import allowedNativeProps from "allowedNativeProps" /* 6949 */;
...
let c4 = 1.618;
fn(6935).NativeProxy.updateGestureHandlerConfig;
...
function maybeUnpackValue(enabled) {
  const Reanimated = _mod6934.Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: fn(6934).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6934.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6934).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6934").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let tmp8 = tmp7;
      let tmp10 = _require;
      let Reanimated = require("module_6934").Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6934).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6934.Reanimated;
          const fn = function t() {
