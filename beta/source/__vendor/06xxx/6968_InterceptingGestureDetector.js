...
// Dependencies: [32, 19, 17, 21, 6909, 6969, 6905, 6964, 6906, 6896, 6934, 6965, 6966, 6958, 6967]
// Exports: InterceptingGestureDetector

// Module 6968 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6909).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6909);
  ...
  if (prop) {
    let REANIMATED = tmp(6969).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6969).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6969).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6969).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6905).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6964).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6906);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6969.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6969.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6965).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6965);
    const gestureRelationsUpdater = tmp(6966).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6966);
    const detectorAttachmentGuard = tmp(6967).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6905).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6969).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6896).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
