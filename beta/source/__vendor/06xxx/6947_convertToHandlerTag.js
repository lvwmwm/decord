...
// Dependencies: [32, 19, 6948, 6949, 6959, 6960, 6961, 6963, 6964, 6965, 6944, 6928, 6966, 6926]
// Exports: checkGestureCallbacksForWorklets, extractGestureRelations, useForceRender, useWebEventHandlers

// Module 6947 (convertToHandlerTag)
...
function convertToHandlerTag(handlerTag) {
  if (typeof handlerTag === "number") {
    return handlerTag;
  } else if (handlerTag instanceof _mod6944.BaseGesture) {
...
let items = [...fn(6948).baseGestureHandlerWithDetectorProps, ...fn(6949).tapGestureHandlerProps, ...fn(6959).panGestureHandlerProps, ...fn(6959).panGestureHandlerCustomNativeProps, ...fn(6960).longPressGestureHandlerProps, ...fn(6961).forceTouchGestureHandlerProps, ...fn(6963).flingGestureHandlerProps, ...fn(6964).hoverGestureHandlerProps, ...fn(6965).nativeViewGestureHandlerProps];
...
export const useWebEventHandlers = function useWebEventHandlers() {
  return React4({
    onGestureHandlerEvent(nativeEvent) {
      const result = _mod6926.onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
