...
// Function ID: 6984
// Name: eventHandler
// Dependencies: [6965, 6903, 6918, 6905, 6967]
// Exports: eventHandler

// Module 6983 (eventHandler)
import _mod6903 from "module_6903" /* 6903 */;
import TouchEventType from "TouchEventType" /* 6905 */;
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6903.State.UNDETERMINED) {
    if (state === tmp(6903).State.BEGAN) {
      tmp(6965).runCallback(tmp(6918).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6965);
    }
  }
  if (oldState === _mod6903.State.BEGAN) {
    if (state === tmp(6903).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6965).runCallback(tmp(6918).CALLBACK_TYPE.START, arg1, result);
      const tmpResult4 = tmp(6965);
    }
  }
  if (oldState !== state) {
    const obj2 = {};
    const merged = Object.assign(result);
    obj2.canceled = state === tmp(6903).State.FAILED || state === tmp(6903).State.CANCELLED;
    if (oldState === tmp(6903).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6965).runCallback(tmp(6918).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6965);
    }
    const tmp7 = state === tmp(6903).State.FAILED || state === tmp(6903).State.CANCELLED;
    tmp(6965).runCallback(tmp(6918).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6965);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6965).flattenAndFilterEvent, State: fn(6903).State, runCallback: fn(6965).runCallback, CALLBACK_TYPE: fn(6918).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6918.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6965).flattenAndFilterEvent, State: fn(6903).State, runCallback: fn(6965).runCallback, CALLBACK_TYPE: fn(6918).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6965).flattenAndFilterEvent, runCallback: fn(6965).runCallback, CALLBACK_TYPE: fn(6918).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6965);
    tmpResult.runCallback(tmp(6965).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6965);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6965).flattenAndFilterEvent, runCallback: fn(6965).runCallback, CALLBACK_TYPE: fn(6918).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6905).TouchEventType, runCallback: fn(6965).runCallback, touchEventTypeToCallbackType: fn(6965).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6905).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6965);
            tmpResult7.runCallback(tmp(6965).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6965);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6965).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6965);
          tmp(6965).runCallback(tmp(6918).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
...
const obj3 = { TouchEventType: fn(6905).TouchEventType, runCallback: fn(6965).runCallback, touchEventTypeToCallbackType: fn(6965).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6965).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6965).isEventForHandlerWithTag, isStateChangeEvent: fn(6967).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6967).isTouchEvent, handleTouchEvent, handleUpdateEvent };
