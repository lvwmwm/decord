...
// Dependencies: [32, 41, 42, 1197, 1195, 1198]
// Exports: binaryReadOptions

// Module 1196 (binaryReadOptions)
...
let items = [
  entry,
  {
    ...
    value: function skip(arg0) {
      ...
      let tmp14;
      const self = this;
      if (BinaryReader(1195).WireType.Varint === arg0) {
        self.pos = +self.pos + 1;
        ...
        if (tmp(1195).WireType.Bit64 === arg0) {
          self.pos = self.pos + 4;
        } else if (tmp(1195).WireType.Bit32 !== arg0) {
          if (tmp(1195).WireType.LengthDelimited === arg0) {
            self.pos = self.pos + self.uint32();
          } else if (tmp(1195).WireType.StartGroup === arg0) {
            const tmp8 = self.tag()[1];
            if (tmp8 !== tmp(1195).WireType.EndGroup) {
              do {
                let skipResult = self.skip(self.tag()[1]);
                EndGroup = BinaryReader(1195).WireType.EndGroup;
              } while (tmp8 !== EndGroup);
            }
          } else {
            const _Error = Error;
  ...
  },
  {
    ...
    value: function int64() {
      return BinaryReader(1198).PbLong(...this.varint64());
    }
  },
  {
    key: "uint64",
    value: function uint64() {
      return BinaryReader(1198).PbULong(...this.varint64());
    }
  },
  {
    key: "sint64",
    value: function sint64() {
      ...
      const pbLong = new BinaryReader(1198).PbLong((tmp2 >>> 1 | (1 & tmp3) << 31) ^ tmp4, tmp3 >>> 1 ^ tmp4);
      return pbLong;
    }
  },
  {
  ...
  },
  {
    ...
    value: function fixed64() {
      const pbULong = new BinaryReader(1198).PbULong(this.sfixed32(), this.sfixed32());
      return pbULong;
    }
  },
  {
    ...
    value: function sfixed64() {
      const pbLong = new BinaryReader(1198).PbLong(this.sfixed32(), this.sfixed32());
      return pbLong;
    }
  },
  {
