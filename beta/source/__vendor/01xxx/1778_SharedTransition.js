...
// Dependencies: [41, 42, 1666, 1681, 1755, 1652, 1713, 1777]

// Module 1778 (SharedTransition)
...
const items = [
        ...
        if (!self._defaultTransitionType) {
          if (self._customAnimationFactory) {
            if (!self._customProgressAnimation) {
              self._defaultTransitionType = obj(1666).SharedTransitionType.ANIMATION;
            }
          }
          self._defaultTransitionType = obj(1666).SharedTransitionType.PROGRESS_ANIMATION;
        }
        if (self._defaultTransitionType === obj(1666).SharedTransitionType.ANIMATION) {
          let SHARED_ELEMENT_TRANSITION_PROGRESS = obj(1666).LayoutAnimationType.SHARED_ELEMENT_TRANSITION;
        } else {
          SHARED_ELEMENT_TRANSITION_PROGRESS = obj(1666).LayoutAnimationType.SHARED_ELEMENT_TRANSITION_PROGRESS;
        }
        obj = obj(1755);
  ...
  },
  {
    ...
    value: function unregisterTransition(componentViewTag, flag) {
      ...
      if (this._defaultTransitionType === require("module_1666").SharedTransitionType.ANIMATION) {
        let SHARED_ELEMENT_TRANSITION_PROGRESS = tmp(1666).LayoutAnimationType.SHARED_ELEMENT_TRANSITION;
      } else {
        SHARED_ELEMENT_TRANSITION_PROGRESS = tmp(1666).LayoutAnimationType.SHARED_ELEMENT_TRANSITION_PROGRESS;
      }
      const result = require("module_1755").updateLayoutAnimations(componentViewTag, SHARED_ELEMENT_TRANSITION_PROGRESS, undefined, undefined, flag);
          ...
          if (keys !== undefined) {
            ...
            while (keys[tmp] !== undefined) {
              if (closure_4.includes(tmp22)) {
                ...
                let reanimatedError = new _mod1652.ReanimatedError("The prop '" + tmp22 + "' is not supported yet.");
...
const entry1 = {
  ...
  value: function custom(arg0) {
    ...
    obj._reduceMotion = require("module_1666").ReduceMotion.System;
...
const progressTransitionManager = new fn(1777).ProgressTransitionManager();
