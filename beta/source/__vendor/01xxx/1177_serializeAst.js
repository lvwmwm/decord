...
// Function ID: 1178
// Name: serializeAst
// Dependencies: [32, 41, 42, 1163]

// Module 1177 (serializeAst)
...
function serializeAst(ast, value) {
  ...
  while (iter !== undefined) {
    ...
    if (typeof nextResult !== "string") {
      ...
      if (InternalIntlMessage(1163).FormatJsNodeType.Argument === first) {
        value.value = `${value.value}{${tmp2[1]}}`;
      } else if (tmp5(1163).FormatJsNodeType.Date === first) {
        value.value = `${value.value}{${tmp2[1]}, date`;
        if (null != tmp2[2]) {
          value.value = `${value.value}, ${tmp2[2]}`;
        }
        value.value = `${value.value}}`;
      } else if (tmp5(1163).FormatJsNodeType.Time === first) {
        value.value = `${value.value}{${tmp2[1]}, time`;
        if (null != tmp2[2]) {
          value.value = `${value.value}, ${tmp2[2]}`;
        }
        value.value = `${value.value}}`;
      } else if (tmp5(1163).FormatJsNodeType.Number === first) {
        value.value = `${value.value}{${tmp2[1]}, number`;
        if (null != tmp2[2]) {
          value.value = `${value.value}, ${tmp2[2]}`;
        }
        value.value = `${value.value}}`;
      } else if (tmp5(1163).FormatJsNodeType.Plural === first) {
        let str = "plural";
        if ("ordinal" == tmp2[4]) {
          str = "selectordinal";
        }
        ...
        value.value = `${value.value}}`;
      } else if (tmp5(1163).FormatJsNodeType.Pound === first) {
        value.value = `${value.value}#`;
      } else if (tmp5(1163).FormatJsNodeType.Select === first) {
        ...
        value.value = `${value.value}}`;
      } else if (tmp5(1163).FormatJsNodeType.Tag === first) {
