...
function rubberBandDecay(current, lastTimestamp, deceleration) {
  ...
  const diff = velocity * Math.exp(result * _mod1717.SLOPE_FACTOR) - num2 * deceleration.rubberBandFactor;
  if (Math.abs(num2) > 0.1) {
    ...
    if (absolute1 < _mod1717.VELOCITY_EPS) {
      return true;
    }
  }
...
rubberBandDecay.__closure = { SLOPE_FACTOR: fn(1717).SLOPE_FACTOR, DERIVATIVE_EPS: 0.1, VELOCITY_EPS: fn(1717).VELOCITY_EPS };
