...
// Dependencies: [32, 41, 42, 1195, 1204, 1212, 1209]

// Module 1211 (ReflectionBinaryReader)
...
let items = [
      ...
      if (len.pos < len) {
        while (true) {
          ...
          if (value) {
            ...
            if ("enum" == value.kind) {
              let T = ReflectionBinaryReader(1204).ScalarType.INT32;
            } else {
              T = value.T;
            }
            ...
            if (repeat) {
              ...
              if (tmp5 == ReflectionBinaryReader(1195).WireType.LengthDelimited) {
                if (T != tmp29(1204).ScalarType.STRING) {
                  if (T != tmp29(1204).ScalarType.BYTES) {
            ...
            if ("throw" == onRead) {
              ...
              if (false !== onRead) {
                if (true === onRead) {
                  onRead = ReflectionBinaryReader(1195).UnknownFieldHandler.onRead;
                }
  ...
  },
  {
    ...
    value: function mapEntry(V, pos, arg2) {
      ...
      if (pos.pos < sum) {
        ...
        if (V.K == ReflectionBinaryReader(1204).ScalarType.BOOL) {
          let str1 = pos.bool().toString();
          const str5 = pos.bool();
        } else {
          str1 = self.scalar(pos, V.K, tmp21(1204).LongType.STRING);
        }
        tmp21 = ReflectionBinaryReader;
        const tmp7 = _slicedToArray(pos.tag(), 2);
      }
      if (undefined === tmp5) {
        const str6 = ReflectionBinaryReader(1212).reflectionScalarDefault(V.K);
        let str7 = str6;
        if (V.K == ReflectionBinaryReader(1204).ScalarType.BOOL) {
          str7 = str6.toString();
        }
        tmp5 = str7;
        const obj = ReflectionBinaryReader(1212);
      }
      let num = tmp4;
      if (undefined === tmp4) {
        const kind2 = V.V.kind;
        if ("scalar" === kind2) {
          num = ReflectionBinaryReader(1212).reflectionScalarDefault(V.V.T, V.V.L);
          const obj3 = ReflectionBinaryReader(1212);
        } else if ("enum" === kind2) {
          num = 0;
        } else {
          num = tmp4;
  ...
  },
  {
    ...
    value: function scalar(int32, arg1, STRING) {
      if (ReflectionBinaryReader(1204).ScalarType.INT32 === arg1) {
        return int32.int32();
      } else if (tmp(1204).ScalarType.STRING === arg1) {
        return int32.string();
      } else if (tmp(1204).ScalarType.BOOL === arg1) {
        return int32.bool();
      } else if (tmp(1204).ScalarType.DOUBLE === arg1) {
        return int32.double();
      } else if (tmp(1204).ScalarType.FLOAT === arg1) {
        return int32.float();
      } else if (tmp(1204).ScalarType.INT64 === arg1) {
        return tmp(1209).reflectionLongConvert(int32.int64(), STRING);
      } else if (tmp(1204).ScalarType.UINT64 === arg1) {
        return tmp(1209).reflectionLongConvert(int32.uint64(), STRING);
      } else if (tmp(1204).ScalarType.FIXED64 === arg1) {
        return tmp(1209).reflectionLongConvert(int32.fixed64(), STRING);
      } else if (tmp(1204).ScalarType.FIXED32 === arg1) {
        return int32.fixed32();
      } else if (tmp(1204).ScalarType.BYTES === arg1) {
        return int32.bytes();
      } else if (tmp(1204).ScalarType.UINT32 === arg1) {
        return int32.uint32();
      } else if (tmp(1204).ScalarType.SFIXED32 === arg1) {
        return int32.sfixed32();
      } else if (tmp(1204).ScalarType.SFIXED64 === arg1) {
        return tmp(1209).reflectionLongConvert(int32.sfixed64(), STRING);
      } else if (tmp(1204).ScalarType.SINT32 === arg1) {
        return int32.sint32();
      } else if (tmp(1204).ScalarType.SINT64 === arg1) {
        return tmp(1209).reflectionLongConvert(int32.sint64(), STRING);
      }
    }
  }
];
