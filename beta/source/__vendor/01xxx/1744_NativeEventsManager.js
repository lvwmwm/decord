...
// Dependencies: [41, 42, 90, 91, 1745, 1734, 1742]
...
const entry = {
  ...
  value: function attachEvents() {
    ...
    for (const key10010 in props) {
      ...
      if (hasItem) {
        hasItem = tmp6.workletEventHandler instanceof tmp7(1742).WorkletEventHandler;
      }
      if (!hasItem) {
        continue;
      } else {
    ...
    value: function updateEvents(current) {
      ...
      if (_classPrivateFieldBase(this, closure_6)[closure_6] !== eventViewTag) {
        for (const key10071 in arg0) {
          ...
          if (hasItem) {
            hasItem = tmp37.workletEventHandler instanceof tmp38(1742).WorkletEventHandler;
          }
          if (!hasItem) {
            continue;
          } else {
        ...
        for (const key10010 in arg0) {
          ...
          if (hasItem1) {
            hasItem1 = tmp29.workletEventHandler instanceof tmp30(1742).WorkletEventHandler;
          }
          if (!hasItem1) {
            continue;
          } else {
            ...
            if (tmp5) {
              let tmp30Result = tmp30(1734);
              let hasItem2 = tmp30Result.has("workletEventHandler", tmp5);
              if (hasItem2) {
                hasItem2 = tmp5.workletEventHandler instanceof tmp30(1742).WorkletEventHandler;
              }
              if (hasItem2) {
        ...
        for (const key10054 in props) {
          ...
          if (hasItem3) {
            hasItem3 = tmp33.workletEventHandler instanceof tmp34(1742).WorkletEventHandler;
          }
          if (!hasItem3) {
            continue;
          } else {
  ...
  },
  {
    ...
    value: function getEventViewTag(arg0) {
      ...
      if (getScrollableNode) {
        ...
        if (setNativeProps) {
          let num3 = NativeEventsManager(1745).findNodeHandle(tmp(self, tmp2)[tmp2]);
          if (num3 == null) {
            num3 = -1;
          }
