...
// Dependencies: [32, 19, 21, 1493, 1550, 1525, 1554, 1506]
// Exports: PreventRemoveProvider

// Module 1555 (transformPreventedRoutes)
...
export const PreventRemoveProvider = function PreventRemoveProvider(children) {
  ...
  const first = _slicedToArray(noop.useState(() => first(1493).nanoid()), 1)[0];
  [first1, dependencyMap] = noop.useState(() => new Map());
  _slicedToArray = noop.useRef(new Map());
  noop = noop.useContext(first(1550).NavigationHelpersContext);
  const context = noop.useContext(first(1525).NavigationRouteContext);
  const context1 = noop.useContext(first(1554).PreventRemoveContext);
  ...
  const tmp11 = first1(1506)((arg0, arg1, arg2) => {
    closure_0 = arg1;
    if (arg2) {
      if (null != closure_4) {
        let everyResult;
  ...
  return context(first(1554).PreventRemoveContext.Provider, { value, children: children.children });
};
