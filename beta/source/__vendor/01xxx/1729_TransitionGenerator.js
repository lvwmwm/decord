...
// Dependencies: [109, 32, 1699, 1728, 1697, 1730, 1731, 1732, 1733, 1734, 1735]
// Exports: TransitionGenerator, createAnimationWithInitialValues, createCustomKeyFrameAnimation

// Module 1729 (TransitionGenerator)
import TransitionType from "TransitionType" /* 1697 */;
import convertAnimationObjectToKeyframes from "convertAnimationObjectToKeyframes" /* 1699 */;
...
export const TransitionGenerator = function TransitionGenerator(ENTRY_EXIT, easingY) {
  ...
  if (TransitionType.TransitionType.LINEAR === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1730).LinearTransition(`REA${tmp}`, easingY);
    const tmp3Result = tmp3(1730);
  } else if (tmp3(1697).TransitionType.SEQUENCED === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1731).SequencedTransition(`REA${tmp}`, easingY);
    const tmp3Result10 = tmp3(1731);
  } else if (tmp3(1697).TransitionType.FADING === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1732).FadingTransition(`REA${tmp}`, easingY);
    const tmp3Result11 = tmp3(1732);
  } else if (tmp3(1697).TransitionType.JUMPING === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1733).JumpingTransition(`REA${tmp}`, easingY);
    const tmp3Result12 = tmp3(1733);
  } else if (tmp3(1697).TransitionType.CURVED === ENTRY_EXIT) {
    closure_6 = tmp7 + 1;
    const text1 = `REA${tmp7}`;
    const tmp3Result13 = tmp3(1734);
    ({ firstKeyframeObj, secondKeyframeObj } = tmp3(1734).CurvedTransition(`REA${tmp}`, `REA${+closure_6}`, easingY));
    const CurvedTransitionResult = tmp3(1734).CurvedTransition(`REA${tmp}`, `REA${+closure_6}`, easingY);
    const result = tmp3(1699).convertAnimationObjectToKeyframes(secondKeyframeObj);
    const tmp3Result14 = tmp3(1699);
    tmp3(1728).insertWebAnimation(`REA${+closure_6}`, result);
    const dummyTransitionKeyframeName = text1;
    const tmp3Result15 = tmp3(1728);
  } else if (tmp3(1697).TransitionType.ENTRY_EXIT === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1735).EntryExitTransition(`REA${tmp}`, easingY);
    const tmp3Result16 = tmp3(1735);
  }
