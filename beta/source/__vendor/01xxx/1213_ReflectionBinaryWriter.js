...
// Dependencies: [32, 41, 42, 1204, 1200, 1195, 1198]

// Module 1213 (ReflectionBinaryWriter)
...
let items = [
  entry,
  {
    ...
    value: function write(arg0, tag, writeUnknownFields) {
      ...
      while (iter !== undefined) {
        ...
        if ("scalar" !== kind) {
          if ("enum" !== kind) {
            if ("message" === kind) {
              if (repeat) {
                let obj2 = ReflectionBinaryWriter(1200);
                let _Array = Array;
              ...
              let obj5 = ReflectionBinaryWriter(1200);
        ...
        if ("enum" == obj.kind) {
          let T = ReflectionBinaryWriter(1204).ScalarType.INT32;
        } else {
          T = obj.T;
        }
        let tmp45 = T;
        if (repeat) {
          let obj4 = ReflectionBinaryWriter(1200);
          let _Array2 = Array;
          let assertResult2 = obj4.assert(Array.isArray(tmp4));
          if (repeat == ReflectionBinaryWriter(1204).RepeatType.PACKED) {
          ...
          let obj3 = ReflectionBinaryWriter(1200);
      ...
      if (false !== onWrite) {
        if (true === onWrite) {
          onWrite = ReflectionBinaryWriter(1195).UnknownFieldHandler.onWrite;
        }
  ...
  },
  {
    ...
    value: function mapEntry(tag, arg1, no, match, arg4) {
      tag.tag(no.no, ReflectionBinaryWriter(1195).WireType.LengthDelimited);
      tag.fork();
      const K = no.K;
      if (ReflectionBinaryWriter(1204).ScalarType.INT32 !== K) {
        if (tmp(1204).ScalarType.FIXED32 !== K) {
          if (tmp(1204).ScalarType.UINT32 !== K) {
            if (tmp(1204).ScalarType.SFIXED32 !== K) {
              if (tmp(1204).ScalarType.SINT32 !== K) {
                let parsed = match;
                if (tmp(1204).ScalarType.BOOL === K) {
                  let tmp6 = "true" == match;
                  if (!tmp6) {
                    tmp6 = "false" == match;
                  }
                  tmp(1200).assert(tmp6);
                  parsed = "true" == match;
              ...
              if ("scalar" === kind) {
                ...
                self.scalar(tag, tmp(1204).ScalarType.INT32, 2, arg4, true);
  ...
  },
  {
    ...
    value: function message(tag, arg1, internalBinaryWrite, arg3, arg4) {
      if (undefined !== arg4) {
        internalBinaryWrite.internalBinaryWrite(arg4, tag.tag(arg3, ReflectionBinaryWriter(1195).WireType.LengthDelimited).fork(), arg1);
        const joined = tag.join();
        const tagResult = tag.tag(arg3, ReflectionBinaryWriter(1195).WireType.LengthDelimited);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function packed(tag, arg1, no, arg3) {
      let length;
      if (arg3.length) {
        const obj = ReflectionBinaryWriter(1200);
        const self = this;
        obj.assert(arg1 !== ReflectionBinaryWriter(1204).ScalarType.BYTES && arg1 !== ReflectionBinaryWriter(1204).ScalarType.STRING);
        tag.tag(no, ReflectionBinaryWriter(1195).WireType.LengthDelimited);
        tag.fork();
        let num2 = 0;
        if (0 < arg3.length) {
          do {
        ...
        const joined = tag.join();
        const tmp4 = arg1 !== ReflectionBinaryWriter(1204).ScalarType.BYTES && arg1 !== ReflectionBinaryWriter(1204).ScalarType.STRING;
      }
    }
  },
  {
    ...
    value: function scalarInfo(arg0, byteLength) {
      const Varint = ReflectionBinaryWriter(1195).WireType.Varint;
      let tmp3 = undefined === byteLength;
      let str = "int32";
      let tmp5 = tmp4;
      let Bit64 = Varint;
      if (ReflectionBinaryWriter(1204).ScalarType.INT32 !== arg0) {
        if (tmp(1204).ScalarType.STRING === arg0) {
          let tmp12 = tmp3;
          if (!tmp3) {
            tmp12 = !byteLength.length;
          }
          Bit64 = tmp(1195).WireType.LengthDelimited;
          str = "string";
          tmp5 = tmp12;
        } else if (tmp(1204).ScalarType.BOOL === arg0) {
          ...
          tmp5 = tmp4;
          Bit64 = Varint;
          if (tmp(1204).ScalarType.UINT32 !== arg0) {
            if (tmp(1204).ScalarType.DOUBLE === arg0) {
              Bit64 = tmp(1195).WireType.Bit64;
              str = "double";
              tmp5 = tmp4;
            } else if (tmp(1204).ScalarType.FLOAT === arg0) {
              Bit64 = tmp(1195).WireType.Bit32;
              str = "float";
              tmp5 = tmp4;
            } else if (tmp(1204).ScalarType.INT64 === arg0) {
              let isZeroResult = tmp3;
              if (!tmp3) {
                const PbLong3 = tmp(1198).PbLong;
              ...
              tmp5 = isZeroResult;
              Bit64 = Varint;
            } else if (tmp(1204).ScalarType.UINT64 === arg0) {
              let isZeroResult1 = tmp3;
              if (!tmp3) {
                const PbULong2 = tmp(1198).PbULong;
              ...
              tmp5 = isZeroResult1;
              Bit64 = Varint;
            } else if (tmp(1204).ScalarType.FIXED64 === arg0) {
              let isZeroResult2 = tmp3;
              if (!tmp3) {
                const PbULong = tmp(1198).PbULong;
              ...
              str = "fixed64";
              tmp5 = isZeroResult2;
            } else if (tmp(1204).ScalarType.BYTES === arg0) {
              let tmp8 = tmp3;
              if (!tmp3) {
              ...
              str = "bytes";
              tmp5 = tmp8;
            } else if (tmp(1204).ScalarType.FIXED32 === arg0) {
              Bit64 = tmp(1195).WireType.Bit32;
              str = "fixed32";
              tmp5 = tmp4;
            } else if (tmp(1204).ScalarType.SFIXED32 === arg0) {
              Bit64 = tmp(1195).WireType.Bit32;
              str = "sfixed32";
              tmp5 = tmp4;
            } else if (tmp(1204).ScalarType.SFIXED64 === arg0) {
              let isZeroResult3 = tmp3;
              if (!tmp3) {
                const PbLong2 = tmp(1198).PbLong;
              ...
              tmp5 = tmp4;
              Bit64 = Varint;
              if (tmp(1204).ScalarType.SINT32 !== arg0) {
                tmp5 = tmp4;
                Bit64 = Varint;
                if (tmp(1204).ScalarType.SINT64 === arg0) {
                  let isZeroResult4 = tmp3;
                  if (!tmp3) {
                    const PbLong = tmp(1198).PbLong;
