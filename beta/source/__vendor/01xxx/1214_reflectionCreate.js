...
// Dependencies: [1202, 1212]
// Exports: reflectionCreate

// Module 1214 (reflectionCreate)
import MESSAGE_TYPE from "MESSAGE_TYPE" /* 1202 */;
import reflectionScalarDefault from "reflectionScalarDefault" /* 1212 */;
...
export const reflectionCreate = function reflectionCreate(value) {
  ...
  while (iter !== undefined) {
    ...
    if (!nextResult.opt) {
      if (tmp3.oneof) {
        obj[tmp3.oneof] = { oneofKind: "emoji" };
      } else if (tmp3.repeat) {
        obj[localName] = [];
      } else {
        let kind = tmp3.kind;
