...
// Dependencies: [5, 32, 19, 893, 1018, 1019, 686, 1011, 1009]
...
// Module 1017 (computeLocationKey)
...
function processResolvedRoutes(arr, children, arg2, activeRootSpan) {
  ...
  if (activeRootSpan) {
    const spanToJSONResult = _mod686.spanToJSON(activeRootSpan);
    if (spanToJSONResult.timestamp) {
      if (tmp12(1011).DEBUG_BUILD) {
        const debug = tmp12(686).debug;
      ...
      if (!tmp) {
        ...
        if (!activeRootSpan) {
          tmp15 = tmp;
          if (undefined !== tmp12(893).WINDOW) {
            const _location = tmp12(893).WINDOW.location;
            let pathname;
            if (_location != null) {
...
function updateNavigationSpan(activeRootSpan, _location, routes, arg3, fn) {
  ...
  if (description) {
    result = tmp(1018).transactionNameHasWildcard(description);
    const tmpResult = tmp(1018);
  }
  if (!spanToJSONResult.timestamp) {
    const tmp9 = fn(routes, _location);
    const tmpResult4 = tmp(1018);
    let items = tmp9;
    if (!tmp9) {
      items = [];
    }
    ...
    if (data != null) {
      const tmp19 = data[tmp(undefined, 686).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
    }
    let tmp20 = tmp17;
    if (tmp17) {
    ...
    if (tmp20) {
      ...
      const attr = activeRootSpan.setAttribute(tmp(686).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, tmp18);
      let result1 = tmp(1018).transactionNameHasWildcard(tmp17);
      if (!result1) {
        result1 = "route" !== tmp18;
      }
      if (!result1) {
        const result2 = tmp(686).addNonEnumerableProperty(activeRootSpan, "__sentry_navigation_name_set__", true);
        const tmpResult6 = tmp(686);
      }
      const tmpResult5 = tmp(1018);
    }
...
function setupRouterSubscription(subscribe, routes, arg2, basename, activeRootSpan) {
  ...
  if (tmp) {
    tmp = "pageload" === require("module_686").spanToJSON(activeRootSpan).op;
    let obj = require("module_686");
  }
  closure_4 = tmp;
  c5 = false;
  c6 = null;
  ...
  const subscription = subscribe.subscribe((historyAction) => {
    ...
    if (!c3) {
      const activeRootSpan = routes(1018).getActiveRootSpan();
      if (activeRootSpan) {
    ...
    if ("PUSH" === historyAction.historyAction) {
      ...
      if ("idle" !== historyAction.navigation.state) {
        ...
        const WINDOW5 = routes(893).WINDOW;
        let prop;
        if (WINDOW5 != null) {
        ...
        if (prop) {
          const WINDOW6 = routes(893).WINDOW;
...
function handleNavigation(arg0) {
  ...
  if (client) {
    if (weakSet.has(client)) {
      const activeRootSpan = tmp6(1018).getActiveRootSpan();
      if (activeRootSpan) {
        const tmp6Result6 = tmp6(686);
      }
      if ("PUSH" === navigationType) {
        if (tmp4Result) {
          const tmp6Result7 = tmp6(1018);
          let tmp11 = allRoutes;
          if (!allRoutes) {
            tmp11 = routes;
          }
          ...
          if (!isPlaceholder) {
            timestamp = tmp6(686).spanToJSON(value.span).timestamp;
            const tmp6Result8 = tmp6(686);
          }
          ...
          if (tmp26Result.skip) {
            if (tmp26Result.shouldUpdate) {
              if (value) {
                ...
                if (value.isPlaceholder) {
                  ...
                  if (tmp6(1011).DEBUG_BUILD) {
                    const debug3 = tmp6(686).debug;
                  ...
                  const attr = span2.setAttribute(tmp6(686).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, tmp22);
                  const result = tmp6(686).addNonEnumerableProperty(value.span, "__sentry_navigation_name_set__", true);
                  value.routeName = tmp21;
                  if (tmp6(1011).DEBUG_BUILD) {
                  ...
                  const tmp6Result9 = tmp6(686);
                }
              }
            }
            if (tmp6(1011).DEBUG_BUILD) {
              const debug = tmp6(686).debug;
            ...
            try {
              ...
              const obj7 = {};
              obj7[tmp6(686).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] = tmp22;
              obj7[tmp6(686).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "navigation";
              const _HermesInternal = HermesInternal;
              obj7[tmp6(686).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = "auto.navigation.react.reactrouter_v" + tmp2;
              obj5.attributes = obj7;
              const result2 = tmp6(893).startBrowserTracingNavigationSpan(client, obj5);
              if (result2) {
                ...
                if (value.size > 0) {
                  if (description) {
                    if (0 === num) {
                      ...
                      racePromise.then(() => {
                        const spanToJSONResult = _mod686.spanToJSON(closure_0);
...
function tryUpdateSpanNameBeforeEnd(updateName, spanToJSONResult, description, _location, arg4, basename, arg6, set) {
  try {
    ...
    let tmp4;
    if (data != null) {
      tmp4 = data[_mod686.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
    }
    if ("route" === tmp4) {
    }
    let tmp15 = arg4;
    ...
    if (tmp23) {
      ...
      if (tmp46) {
        ...
        const attr = updateName.setAttribute(_mod686.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, tmp40);
      }
    }
    tmp10 = tmp4;
  } catch (tmp54) {
    if (_mod1011.DEBUG_BUILD) {
      const debug = tmp55(686).debug;
      const _HermesInternal = HermesInternal;
...
function patchSpanEnd(result2, _location, routes, basename, allRoutes, navigation) {
  ...
  if (!tmp2) {
    if (result2.end) {
      ...
      const result = require("module_686").addNonEnumerableProperty(result2, combined, true);
      let obj = require("module_686");
    }
  }
}
...
export const createReactRouterV6CompatibleTracingIntegration = function createReactRouterV6CompatibleTracingIntegration(instrumentPageLoad, _6) {
  ...
  obj3.afterAllSetup = function afterAllSetup(f133600) {
    ...
    if (tmp4) {
      ...
      const obj2 = {};
      obj2[tmp2(686).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] = "url";
      obj2[tmp2(686).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "pageload";
      const _HermesInternal = HermesInternal;
      obj2[tmp2(686).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = "auto.pageload.react.reactrouter_v" + closure_1;
      obj.attributes = obj2;
      result = tmp2(893).startBrowserTracingPageLoadSpan(f133600, obj);
      const tmp2Result = tmp2(893);
    }
    if (closure_11) {
      weakSet.add(f133600);
    }
...
export const createV6CompatibleWithSentryReactRouterRouting = function createV6CompatibleWithSentryReactRouterRouting(arg0, _6) {
      ...
      if (closure_6) {
        if (closure_7) {
          if (closure_8) {
            ...
            require("module_1009").hoistNonReactStatics(SentryRoutes, arg0);
            return SentryRoutes;
          }
        }
      }
  ...
  if (require("module_1011").DEBUG_BUILD) {
    class SentryRoutes {
      constructor(arg0) {
        closure_0 = arg0;
        closure_1 = closure_1_9.useRef(true);
