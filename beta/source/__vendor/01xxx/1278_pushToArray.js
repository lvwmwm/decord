...
// Function ID: 1279
// Name: pushToArray
// Dependencies: [1277, 1275, 1279]

// Module 1278 (pushToArray)
import _mod1275 from "module_1275" /* 1275 */;
import arrayToObject from "arrayToObject" /* 1277 */;
...
let obj2 = {
  ...
  filter: "r",
  format: _mod1275.default,
  formatter: _mod1275.formatters[_mod1275.default],
  indices: false,
  serializeDate(arg0) {
      ...
      if (undefined === str2) {
        ...
        if ("comma" === fn) {
          if (isArray(str2)) {
            ...
            if (tmp17) {
              maybeMapResult = tmp10(1277).maybeMap(str2, fn2);
              const tmp10Result = tmp10(1277);
            }
            let tmp18;
...
export default (arg0, allowEmptyArrays) => {
  if (allowEmptyArrays) {
    ...
    let format = _mod1275.default;
    if (undefined !== allowEmptyArrays.format) {
      const call = hasOwnProperty.call;
      const formatters = tmp3(1275).formatters;
    ...
    obj.filter = filter;
    obj.format = format;
    obj.formatter = _mod1275.formatters[format];
