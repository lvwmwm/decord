...
// Function ID: 1727
// Name: InnerKeyframe
// Dependencies: [41, 42, 1666, 1713, 1693, 1652, 1681]

// Module 1726 (InnerKeyframe)
  ...
  constructor(arg0) {
    ...
    this.build = () => {
      ...
      if (!self.parsedAnimation) {
            ...
            if (0 !== first.length) {
              if (1 === first.length) {
                ...
                if (first[0].easing) {
                  ...
                  linear = self(1693).Easing.linear;
                }
                obj2.easing = linear;
                obj.withTiming(first[0].value, obj2);
              } else {
                const tmp3 = self(1713);
        ...
        let obj = { keyframes, delayFunction, delay: delayV, withTiming: InnerKeyframe(1713).withTiming, Easing: InnerKeyframe(1693).Easing, withSequence: InnerKeyframe(1713).withSequence, initialValues, makeKeyframeKey, callback: callbackV };
        fn.__closure = obj;
            ...
            item = keys.forEach((item) => {
              if ("transform" === item) {
                ...
                if (item in obj) {
                  ...
                  let reanimatedError = new InnerKeyframe(1652).ReanimatedError("Keyframe can contain only that set of properties that were provide with initial values (keyframe 0 or 'from')");
                  throw reanimatedError;
                }
                tmp3 = obj;
              } else {
    ...
    value: function getDelayFunction() {
      ...
      if (this.delayV) {
        const fn2 = function t(arg0, arg1) {
          return InnerKeyframe(1713).withDelay(arg0, arg1, reduceMotionV);
        };
        const obj2 = { withDelay: reduceMotionV(1713).withDelay, reduceMotion: reduceMotionV };
        fn2.__closure = obj2;
        ...
        fn = function n(arg0, arg1) {
          arg1.reduceMotion = InnerKeyframe(1681).getReduceMotionFromConfig(reduceMotionV);
          return arg1;
        };
        const obj = { getReduceMotionFromConfig: reduceMotionV(1681).getReduceMotionFromConfig, reduceMotion: reduceMotionV };
        fn.__closure = obj;
