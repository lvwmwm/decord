...
// Dependencies: [1160, 1161]
// Exports: makeDataFormatters

// Module 1159 (DEFAULT_FORMAT_CONFIG)
import _mod1160 from "module_1160" /* 1160 */;
import dataFormatterCache2 from "dataFormatterCache" /* 1161 */;
...
export function makeDataFormatters(items, formatConfig, _forceLookupMatcher) {
  ...
  return {
    formatDate(arg0, format) {
      const formatConfigOptions = _mod1160.resolveFormatConfigOptions(closure_1.date, format);
    ...
    formatDuration(arg0, format) {
      const formatConfigOptions = _mod1160.resolveFormatConfigOptions(closure_1.time, format);
      const dataFormatterCache = dataFormatterCache2.dataFormatterCache;
      let merged = formatConfigOptions;
      if (flag) {
        const _Object = Object;
    ...
    formatNumber(result2, parseNumberSkeletonResult) {
      const formatConfigOptions = _mod1160.resolveFormatConfigOptions(closure_1.number, parseNumberSkeletonResult);
      const dataFormatterCache = dataFormatterCache2.dataFormatterCache;
      let merged = formatConfigOptions;
      if (flag) {
        const _Object = Object;
    ...
    formatList(arg0, format) {
      const formatConfigOptions = _mod1160.resolveFormatConfigOptions(closure_1.list, format);
      const dataFormatterCache = dataFormatterCache2.dataFormatterCache;
      let merged = formatConfigOptions;
      if (flag) {
        const _Object = Object;
    ...
    formatRelativeTime(arg0, day, format) {
      const formatConfigOptions = _mod1160.resolveFormatConfigOptions(closure_1.relativeTime, format);
      const dataFormatterCache = dataFormatterCache2.dataFormatterCache;
      let merged = formatConfigOptions;
      if (flag) {
        const _Object = Object;
