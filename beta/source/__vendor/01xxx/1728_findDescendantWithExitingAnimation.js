...
// Dependencies: [1644, 1645, 1697, 1652, 1727]
...
// Module 1728 (findDescendantWithExitingAnimation)
  ...
  if (arr instanceof globalThis.HTMLElement) {
    ...
    if (isDummy) {
      ...
      if (value) {
        ...
        tmp2(1727).setElementPosition(arr, value);
        ...
        const tmp2Result = tmp2(1727);
      } else {
        const logger = tmp2(1645).logger;
...
export const configureWebLayoutAnimations = function configureWebLayoutAnimations() {
  if (obj.isWindowAvailable()) {
    ...
    if (null === document.getElementById(ReanimatedPredefinedWebAnimationsStyle)) {
      ...
      element.onload = () => {
        if (element.sheet) {
          for (const key10016 in tmp(1697).Animations) {
  ...
  if (obj.isWindowAvailable()) {
    ...
    if (element.sheet) {
      ...
      if (1 < closure_5.length) {
        ...
        const reanimatedError = new _mod1652.ReanimatedError("Failed to obtain animation index.");
    ...
    if (obj.isWindowAvailable()) {
      ...
      if (undefined === sum) {
        ...
        if (sum < closure_5.length) {
          ...
          const reanimatedError1 = new _mod1652.ReanimatedError("Failed to obtain animation index.");
...
export const addHTMLMutationObserver = function addHTMLMutationObserver() {
  ...
  if (!c6) {
    isWindowAvailableResult = _mod1644.isWindowAvailable();
  }
