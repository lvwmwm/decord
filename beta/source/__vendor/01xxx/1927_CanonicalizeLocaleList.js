...
// Dependencies: [1928, 1929]

// Module 1927 (CanonicalizeLocaleList)
import expBCP47Syntax2 from "expBCP47Syntax" /* 1928 */;
          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "string") {
                ...
                if (isMatch1) {
                  let expVariantDupes = tmp5(1928).expVariantDupes;
                  let isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(1928).expSingletonDupes;
                ...
                if (isMatch1) {
                  ...
                  if (!tmp20) {
                    ...
                    let RegExpResult = RegExp(`(?:${tmp17(1928).expExtSequences.source})+`, "i");
...
function InitializeDateTimeFormat(prototype, arg1, arg2) {
  ...
  if (true === result["[[initializedIntlObject]]"]) {
    ...
    if ("[object Array]" !== (typeof call6 === "unknown" ? toString() : call6(formats))) {
      dateTimeFormats = _mod1929.createDateTimeFormats(formats);
    }
    obj10 = Object.create(List.prototype);
    List("basic", "best fit");
...
class List {
  constructor() {
    tmp = f25632(this, "length", { writable: true, value: 0 });
    if (!arguments.length) {
      return;
    } else {
      tmp3 = slice;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    if (isMatch1) {
      const expVariantDupes = tmp(1928).expVariantDupes;
      const isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(1928).expSingletonDupes;
