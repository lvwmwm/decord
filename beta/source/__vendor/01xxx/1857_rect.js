...
// Function ID: 1858
// Name: rect
// Dependencies: [32, 109, 19, 17, 21, 1858, 1859, 1835, 1860, 1861, 1631, 1862, 1866, 1850]

// Module 1857 (rect)
import nativeEventEmitter from "nativeEventEmitter" /* 1631 */;
import TEST_ID_KEYBOARD_TOOLBAR from "TEST_ID_KEYBOARD_TOOLBAR" /* 1859 */;
import Background from "Background" /* 1862 */;
      ...
      item = Children.forEach(children, (type) => {
        if (noop.isValidElement(type)) {
          ...
          if (type === Background.Background) {
            closure_13 = type;
          } else if (type === tmp(1862).Content) {
            closure_11 = type;
          } else if (type === tmp(1862).Prev) {
            closure_9 = type;
          } else if (type === tmp(1862).Next) {
            closure_10 = type;
          } else if (type === tmp(1862).Done) {
            closure_12 = type;
          }
        }
      });
...
let rect = { position: "absolute", left: 0, right: 0, bottom: 0, height: fn(1859).KEYBOARD_TOOLBAR_HEIGHT };
obj.sticky = rect;
const size = { position: "absolute", bottom: 0, alignItems: "center", width: "100%", flexDirection: "row", height: fn(1859).KEYBOARD_TOOLBAR_HEIGHT };
obj.toolbar = size;
...
KeyboardToolbar.Background = fn(1862).Background;
KeyboardToolbar.Content = fn(1862).Content;
KeyboardToolbar.Prev = fn(1862).Prev;
KeyboardToolbar.Next = fn(1862).Next;
KeyboardToolbar.Done = fn(1862).Done;
KeyboardToolbar.Group = fn(1631).RCTKeyboardToolbarGroupView;

export default KeyboardToolbar;
export const DefaultKeyboardToolbarTheme = fn(1858).colors;
