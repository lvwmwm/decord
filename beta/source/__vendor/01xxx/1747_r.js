...
PlatformColor.__closure = { IS_IOS: fn(1748).IS_IOS };
...
function processColorNumber(semantic) {
  const result = _mod1680.processColorInitially(semantic);
  let tmp3 = result;
  if (tmp2) {
    tmp3 = result | 0;
  }
...
let obj = { IS_IOS: fn(1748).IS_IOS };
processColorNumber.__closure = { processColorInitially: fn(1680).processColorInitially, IS_ANDROID: fn(1748).IS_ANDROID };
...
function processColor(semantic, target) {
  if (typeof processColorNumber === "function") {
    ...
    if (tmp6) {
      ...
      let tmp29 = target !== tmp2(1749).ValueProcessorTarget.CSS;
      if (!tmp29) {
        ...
        if (tmp8) {
          ...
          if (isDynamicColorObjectIOS(semantic)) {
            if (tmp2(1748).IS_IOS) {
              ...
              const reanimatedError = new tmp2(1648).ReanimatedError(obj2.dynamicNotAvailableOnPlatform());
              throw reanimatedError;
            }
          }
          if (null === tmp13) {
            const reanimatedError1 = new tmp2(1648).ReanimatedError(obj2.invalidColor(semantic));
...
const obj3 = { processColorInitially: fn(1680).processColorInitially, IS_ANDROID: fn(1748).IS_ANDROID };
processColor.__closure = { processColorNumber, ValueProcessorTarget: fn(1749).ValueProcessorTarget, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: fn(1748).IS_IOS, ERROR_MESSAGES: obj2, processDynamicColorObjectIOS };
    ...
    if (typeof fn === "function") {
      ...
      if (tmp2) {
        ...
        if (tmp7(1748).IS_IOS) {
          ...
          const reanimatedError = new tmp7(1648).ReanimatedError(obj2.dynamicNotAvailableOnPlatform());
          throw reanimatedError;
        }
      } else {
        const reanimatedError1 = new tmp7(1648).ReanimatedError(obj2.invalidProcessedColor(arr));
        throw reanimatedError1;
      }
    } else {
      throw new TypeError("Trying to call a non-function");
...
const obj4 = { processColorNumber, ValueProcessorTarget: fn(1749).ValueProcessorTarget, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: fn(1748).IS_IOS, ERROR_MESSAGES: obj2, processDynamicColorObjectIOS };
unprocessColor.__closure = { unprocessColorNumber, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: fn(1748).IS_IOS, ERROR_MESSAGES: obj2, unprocessDynamicColorObjectIOS };
...
function processColorsInProps(obj) {
  for (const key10005 in arg0) {
    let ColorProperties = _mod1680.ColorProperties;
...
const obj5 = { unprocessColorNumber, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: fn(1748).IS_IOS, ERROR_MESSAGES: obj2, unprocessDynamicColorObjectIOS };
processColorsInProps.__closure = { ColorProperties: fn(1680).ColorProperties, processColor };
...
const obj6 = { ColorProperties: fn(1680).ColorProperties, processColor };
unprocessColorsInProps.__closure = { ColorProperties: fn(1680).ColorProperties, unprocessColor };
