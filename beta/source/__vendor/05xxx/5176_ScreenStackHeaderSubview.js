...
// Dependencies: [109, 19, 17, 21, 5177, 5174, 5178, 5179, 5157, 5180]
...
// Module 5176 (ScreenStackHeaderSubview)
...
import _modDef5179 from "module_5179" /* 5179 */;
...
const forwardRefResult = noop.forwardRef((hidden, ref) => {
  ...
  const edgeInsetApplication = _mod5177.useEdgeInsetApplication(!hidden.hidden, flag, flag2, flag3, flag4);
  ...
  if (headerLeftBarButtonItems) {
    if (tmp(5174).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result = tmp(5178).prepareHeaderBarButtonItems(headerLeftBarButtonItems, "left");
      const tmpResult = tmp(5178);
    }
  }
  require = result;
  let result1;
  if (headerRightBarButtonItems) {
    if (tmp(5174).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result1 = tmp(5178).prepareHeaderBarButtonItems(headerRightBarButtonItems, "right");
      const tmpResult2 = tmp(5178);
    }
  }
  let isHeaderBarButtonsAvailableForCurrentPlatform = tmp(5174).isHeaderBarButtonsAvailableForCurrentPlatform;
...
export const ScreenStackHeaderSubview = _modDef5180;
...
export const ScreenStackHeaderBackButtonImage = (arg0) => {
  ...
  return jsx(_modDef5180, { type: "back", style: closure_9.headerSubview, synchronousShadowStateUpdatesEnabled: get_synchronousScreenUpdatesEnabledDefault.experiment.synchronousHeaderSubviewUpdatesEnabled, children: null });
};
