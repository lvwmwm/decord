...
// Dependencies: [41, 42, 5436, 5439, 5445, 5446, 5447, 5448, 5449]

// Module 5438 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5445 */;
import ImageTypes from "ImageTypes" /* 5446 */;
import VideoTypes from "VideoTypes" /* 5447 */;
import CompressedTypes from "CompressedTypes" /* 5448 */;
import OtherTypes from "OtherTypes" /* 5449 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5436).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5436).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5439).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5439).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5439).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5436).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5439).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5436).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
