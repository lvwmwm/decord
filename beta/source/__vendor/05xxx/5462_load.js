...
// Function ID: 5463
// Name: load
// Dependencies: [5463, 5464, 5465, 5466, 5467, 5480, 5482, 5483, 5495, 5499, 5503, 5505, 5507, 5508, 5509, 5510, 5511, 5512, 5513, 5514, 5515, 5516, 5489]
// Exports: load
...
function load(response, response) {
  ...
  if (typeof response === "string") {
    ...
    if (obj2.test(response)) {
      ...
      if (obj3.test(response)) {
        let resolved = Promise.resolve(response(5464).dataUriToBuffer(response));
        const obj5 = response(5464);
      } else {
        let obj7 = then;
        if (then === undefined) {
          obj7 = {};
...
function loadView(byteLength, response) {
  ...
  function addPngTextTags(readTags) {
    if (flag) {
      ...
      if (closure_1.png) {
        objectAssignResult3 = _mod5464.objectAssign({}, closure_1.png, readTags);
      }
      closure_1.png = objectAssignResult3;
      let objectAssignResult4 = readTags;
      if (closure_1.pngText) {
        objectAssignResult4 = _mod5464.objectAssign({}, closure_1.png, readTags);
      }
  ...
  let obj2 = require("module_5466");
  ({ fileType, fileDataOffset, jfifDataOffset, tiffHeaderOffset, iptcDataOffset, xmpChunks, iccChunks, mpfDataOffset, pngHeaderOffset, pngTextChunks, pngChunkOffsets, vp8xChunkOffset, gifHeaderOffset } = require("module_5466").parseAppMarkers(byteLength, flag2));
  let flag4 = false;
  if (require("module_5467").USE_JPEG) {
    flag4 = false;
        ...
        if (readResult11 instanceof Promise) {
          items1.push(readResult11.then(function addIccTags(icc) {
            if (flag) {
              ...
              closure_1 = _mod5464.objectAssign({}, closure_1, icc);
            }
          }));
          flag9 = true;
        } else if (flag) {
  ...
  const parseAppMarkersResult = require("module_5466").parseAppMarkers(byteLength, flag2);
...
export default { load, loadView, errors: _modDef5463 };
export const errors = _modDef5463;
export { load };
export { loadView };
