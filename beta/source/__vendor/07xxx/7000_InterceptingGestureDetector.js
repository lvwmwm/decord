...
// Dependencies: [32, 19, 17, 21, 6941, 7001, 6937, 6996, 6938, 6928, 6966, 6997, 6998, 6990, 6999]
// Exports: InterceptingGestureDetector

// Module 7000 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6941).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6941);
  ...
  if (prop) {
    let REANIMATED = tmp(7001).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(7001).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(7001).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(7001).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6937).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6996).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6938);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod7001.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod7001.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6997).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6997);
    const gestureRelationsUpdater = tmp(6998).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6998);
    const detectorAttachmentGuard = tmp(6999).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6937).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(7001).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6928).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
