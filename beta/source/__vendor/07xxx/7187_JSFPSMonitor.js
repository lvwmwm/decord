...
// Function ID: 7188
// Name: JSFPSMonitor
// Dependencies: [7134, 7135, 7122, 7188]

// Module 7187 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(7122).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(7188).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(7188);
      obj.maxFPS = JSFPSMonitor(7188).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(7188);
      obj.averageFPS = JSFPSMonitor(7188).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef7135(JSFPSMonitor, items);
