...
// Function ID: 14505
// Name: ToRawPrecision
// Dependencies: [1161, 14485, 14486, 14488, 14498]
// Exports: ToRawPrecision

// Module 14504 (ToRawPrecision)
...
export const ToRawPrecision = function ToRawPrecision(ZERO, minimumSignificantDigits, maximumSignificantDigits, result) {
  ...
  let tmp4;
  if (ZERO.isZero()) {
    let repeatResult = _mod14486.repeat("0", maximumSignificantDigits);
  ...
  if (roundingMagnitude >= maximumSignificantDigits - 1) {
    let sum = repeatResult + tmp16(14486).repeat("0", roundingMagnitude - maximumSignificantDigits + 1);
    ...
    tmp16(14486).invariant(roundingMagnitude < 0, "e should be less than 0");
    sum = `0.${tmp16(14486).repeat("0", -num3 - 1)}${arr}`;
