...
// Dependencies: [14489, 14486, 14491, 14490]
// Exports: SetNumberFormatDigitOptions

// Module 14533 (SetNumberFormatDigitOptions)
...
export const SetNumberFormatDigitOptions = function SetNumberFormatDigitOptions(internalSlots, result1, minimumFractionDigits, arg3, GetOptionResult1) {
  ...
  _mod14486.invariant(hasItem, combined.concat(Array.from(set).join(", "), "."));
  ...
  if (flag2) {
    if (tmp11) {
      internalSlots.minimumSignificantDigits = tmp2(14490).DefaultNumberOption(minimumSignificantDigits, 1, 21, 1);
      internalSlots.maximumSignificantDigits = tmp2(14490).DefaultNumberOption(maximumSignificantDigits, internalSlots.minimumSignificantDigits, 21, 21);
    } else {
  ...
  if (flag) {
    if (tmp12) {
      const DefaultNumberOptionResult = tmp2(14490).DefaultNumberOption(minimumFractionDigits, 0, 100, undefined);
      const DefaultNumberOptionResult1 = tmp2(14490).DefaultNumberOption(maximumFractionDigits, 0, 100, undefined);
      if (undefined === DefaultNumberOptionResult) {
        tmp2(14486).invariant(undefined !== DefaultNumberOptionResult1, "maximumFractionDigits must be defined");
        const _Math2 = Math;
  ...
  if (!flag2) {
    ...
    if (tmp10) {
      ...
      tmp2(14486).invariant("fractionDigits" === internalSlots.roundingType, "Invalid roundingType", TypeError);
      const _RangeError = RangeError;
      tmp2(14486).invariant(internalSlots.maximumFractionDigits === internalSlots.minimumFractionDigits, "With roundingIncrement > 1, maximumFractionDigits and minimumFractionDigits must be equal.", RangeError);
    }
  }
