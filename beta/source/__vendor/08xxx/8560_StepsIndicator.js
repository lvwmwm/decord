...
// Function ID: 8561
// Name: StepsIndicator
// Dependencies: [19, 17, 21, 8558, 8559, 8561, 8562]
// Exports: StepsIndicator

// Module 8560 (StepsIndicator)
import _mod8558 from "module_8558" /* 8558 */;
import _mod8559 from "module_8559" /* 8559 */;
import SliderTrackMark from "SliderTrackMark" /* 8561 */;
import StepNumber from "StepNumber" /* 8562 */;
...
export const StepsIndicator = function StepsIndicator(options) {
  ...
  const memo = currentValue.useMemo(() => {
    if (options.length > 9) {
      let fontSize = _mod8558.constants.STEP_NUMBER_TEXT_FONT_SMALL;
    } else {
      fontSize = _mod8558.constants.STEP_NUMBER_TEXT_FONT_BIG;
    }
    return { fontSize };
  }, items);
  ...
  const memo1 = currentValue.useMemo(() => {
    if ("web" === get_ActivityIndicator.Platform.OS) {
      let stepsIndicator = _mod8559.styles.stepsIndicator;
      let tmp6 = require;
    } else {
      const _Object = Object;
      const obj = { marginHorizontal: sliderWidth * _mod8558.constants.MARGIN_HORIZONTAL_PADDING };
      stepsIndicator = Object.assign({}, _mod8559.styles.stepsIndicator, obj);
      tmp6 = require;
    }
    const obj2 = { stepIndicatorContainerStyle: stepsIndicator, stepIndicatorElementStyle: null };
    if ("web" === get_ActivityIndicator.Platform.OS) {
      const _Object2 = Object;
      const obj3 = { width: tmp6(8558).constants.THUMB_SIZE, justifyContent: "space-between" };
      let stepIndicatorElement = Object.assign({}, tmp6(8559).styles.stepIndicatorElement, obj3);
    } else {
      stepIndicatorElement = tmp6(8559).styles.stepIndicatorElement;
    }
