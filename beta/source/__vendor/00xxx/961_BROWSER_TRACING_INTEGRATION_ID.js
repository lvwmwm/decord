...
let obj = {};
let merged = Object.assign(_mod686.TRACING_DEFAULTS);
...
export const browserTracingIntegration = () => {
    ...
    if (flag) {
      ...
      let obj3 = {
        ...
        beforeSpanEnd(setAttribute) {
            ...
            const result1 = _mod686.addNonEnumerableProperty(closure_0, _sentry_idleSpan, undefined);
            ...
            obj5.sampled = _mod686.spanIsSampled(_undefined);
            obj5.dsc = _mod686.getDynamicSamplingContextFromSpan(setAttribute);
  ...
  closure_3 = { name: "state", source: "toCharArray$esjava$1" };
    ...
    setup(client) {
      ...
      const dependencyMap2 = function maybeEndActiveSpan() {
        ...
        if (dependencyMap[_sentry_idleSpan]) {
          tmp = !_mod686.spanToJSON(obj).timestamp;
        }
        if (tmp) {
          if (_mod941.DEBUG_BUILD) {
            const debug = tmp4(686).debug;
            const _HermesInternal = HermesInternal;
            debug.log("[Tracing] Finishing current active span with op: " + tmp4(686).spanToJSON(obj).op);
            const tmp4Result = tmp4(686);
          }
          const attr = obj.setAttribute(tmp4(686).SEMANTIC_ATTRIBUTE_SENTRY_IDLE_SPAN_FINISH_REASON, "cancelled");
          obj.end();
        }
      };
          ...
          if (globalThis.PerformanceObserver.supportedEntryTypes) {
            ...
            if (c26) {
              if (document) {
                function interactionHandler() {
                  closure_1 = dependencyMap(686).timestampInSeconds();
                }
            ...
            client.on("startNavigationSpan", (arg0, isRedirect) => {
              if (obj.getClient() === closure_0) {
                ...
                if (isRedirect) {
                  ...
                  obj4.traceId = tmp(686).generateTraceId();
                  const _Math2 = Math;
                  obj4.sampleRand = Math.random();
                  const tmpResult12 = tmp(686);
                  let spanId1;
                  if (!tmpResult13.hasSpansEnabled()) {
                    spanId1 = tmp(686).generateSpanId();
                    const tmpResult14 = tmp(686);
                  }
                  obj4.propagationSpanId = spanId1;
                  const result1 = currentScope.setPropagationContext(obj4);
                  const result2 = currentScope.setSDKProcessingMetadata({ normalizedRequest: "emoji" });
            ...
            client.on("startPageLoadSpan", (arg0) => {
              ...
              if (obj2.getClient() === closure_0) {
                ...
                if (!baggage) {
                  const _document2 = tmp(897).WINDOW.document;
                  let element1;
                  if (_document2 != null) {
                ...
                const result = tmp(686).propagationContextFromHeaders(sentryTrace, baggage);
                const tmpResult = tmp(686);
                const currentScope = tmp(686).getCurrentScope();
            ...
            client.on("endPageloadSpan", () => {
              ...
              if (tmp) {
                const attr = closure_1_2.setAttribute(dependencyMap(686).SEMANTIC_ATTRIBUTE_SENTRY_IDLE_SPAN_FINISH_REASON, "reportPageLoaded");
                closure_1_2.end();
              }
            });
          }
      ...
      if (_undefined(_undefined2[2]).WINDOW.location) {
        ...
        if (c25) {
          let result2 = tmp(tmp2[3]).addHistoryInstrumentationHandler((to) => {
            ...
            idleTimeout = undefined;
            const result = _mod686.parseStringToURLObject(to);
            let tmp8 = tmp7;
            if (emit[_sentry_idleSpan]) {
              tmp8 = c26;
            }
            if (tmp8) {
              const tmp3Result = tmp3(686);
              const spanToJSONResult = tmp3(686).spanToJSON(tmp7);
              const result1 = tmp3(686).dateTimestampInSeconds();
              let flag = false;
            ...
            if (!pathname) {
              pathname = tmp3(897).WINDOW.location.pathname;
            }
            const obj3 = { name: pathname, attributes: { [_mod686.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "url", [_mod686.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN]: "auto.navigation.browser" } };
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f133600, tmp2Result, arg2) {
  f133600.emit("startPageLoadSpan", tmp2Result, arg2);
  const currentScope = _mod686.getCurrentScope();
  currentScope.setTransactionName(tmp2Result.name);
  if (f133600[_sentry_idleSpan]) {
    f133600.emit("afterStartPageLoadSpan", tmp3);
  }
  return f133600[_sentry_idleSpan];
};
