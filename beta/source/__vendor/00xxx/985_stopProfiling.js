...
function stopProfiling(arg0) {
  ...
  if (stopProfilingResult) {
    ...
    const result1 = tmp(993).convertToSentryProfile(stopProfilingResult.hermesProfile);
    if (result1) {
      const hermesProfilingEvent = tmp(987).createHermesProfilingEvent(result1);
      ...
      const tmpResult2 = tmp(987);
    } else {
      return null;
    }
    const tmpResult = tmp(993);
  } else {
    return null;
  }
}
  ...
  function _startCurrentProfile(activeSpan) {
    if (obj.isRootSpan(activeSpan)) {
      ...
      if (typeof _shouldStartProfiling === "function") {
        ...
        spanIsSampledResult = tmp(686).spanIsSampled(activeSpan);
      } else {
  ...
  return {
    ...
    setupOnce() {
      if (!c3) {
        ...
        if (isHermesEnabledResult) {
          ...
          if (tmp5) {
            if (typeof _startCurrentProfileForActiveTransaction === "function") {
              if (!c0) {
                const activeSpan = tmp(686).getActiveSpan();
                if (activeSpan) {
              ...
              client.on("beforeEnvelope", (arg0) => {
                const PROFILE_QUEUE = c0(986).PROFILE_QUEUE;
                if (PROFILE_QUEUE.size()) {
                  const result = tmp(987).findProfiledTransactionsFromEnvelope(arg0);
                  if (result.length) {
                    const items = [];
        ...
        isHermesEnabledResult = _mod871.isHermesEnabled();
      }
    }
  };
};
