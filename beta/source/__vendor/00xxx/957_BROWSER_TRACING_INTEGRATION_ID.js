...
export const browserTracingIntegration = () => {
  ...
  closure_3 = { name: "paths", source: "scales" };
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f110545, tmp2Result, arg2) {
  f110545.emit("startPageLoadSpan", tmp2Result, arg2);
  const currentScope = _mod682.getCurrentScope();
  currentScope.setTransactionName(tmp2Result.name);
  if (f110545[_sentry_idleSpan]) {
    f110545.emit("afterStartPageLoadSpan", tmp3);
  }
  return f110545[_sentry_idleSpan];
};
