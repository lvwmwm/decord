...
// Dependencies: [13055, 13059, 13061, 13063, 13064, 13065, 13066, 13067, 13068, 13071, 13076, 13049]
...
// Module 13054 (spanTimeInputToSeconds)
  ...
  if (typeof getTime === "number") {
    ...
    if (Array.isArray(getTime)) {
      ...
      if (getTime instanceof Date) {
        ...
        sum = _mod13063.timestampInSeconds();
      }
    }
  }
  return sum;
  ...
  if ((function spanIsSentrySpan(getSpanJSON) {
    ...
    try {
      ...
      if ((function spanIsOpenTelemetrySdkTraceBaseSpan(attributes) {
        ...
        obj3.op = attributes[_mod13064.SEMANTIC_ATTRIBUTE_SENTRY_OP];
        obj3.origin = attributes[_mod13064.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
        const obj2 = _mod13055;
        obj3._metrics_summary = _mod13065.getMetricSummaryJsonForSpan(getSpanJSON);
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== _mod13066.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== tmp(13066).SPAN_STATUS_OK) {
...
export const addChildSpanToSpan = function addChildSpanToSpan(arg0, arg1) {
  ...
  const result = _mod13055.addNonEnumerableProperty(arg1, _sentryRootSpan, tmp2);
  if (arg0[_sentryChildSpans]) {
    ...
    const result1 = _mod13055.addNonEnumerableProperty(arg0, tmp6, set);
...
export const getActiveSpan = function getActiveSpan() {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    activeSpan = tmpResult._getSpanForScope(tmp(13076).getCurrentScope());
...
export const spanToTraceContext = function spanToTraceContext(spanContext) {
  ...
  return _mod13055.dropUndefinedKeys({ parent_span_id, span_id, trace_id: trace_id.traceId });
};
export const spanToTraceHeader = function spanToTraceHeader(spanContext) {
  ...
  return _mod13061.generateSentryTraceHeader(traceId, spanId, 1 === spanContext.spanContext().traceFlags);
};
export const spanToTransactionTraceContext = function spanToTransactionTraceContext(spanContext) {
  ...
  return _mod13055.dropUndefinedKeys({ parent_span_id, span_id, trace_id, data, op, status, origin });
};
export const updateMetricSummaryOnActiveSpan = function updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, tags, bucketKey) {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    const tmpResult = tmp(13071);
    activeSpan = tmpResult._getSpanForScope(tmp(13076).getCurrentScope());
    const tmpResult3 = tmp(13076);
  }
  if (activeSpan) {
    const tmpResult4 = tmp(13065);
