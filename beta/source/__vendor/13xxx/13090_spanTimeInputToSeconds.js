...
// Dependencies: [13091, 13095, 13097, 13099, 13100, 13101, 13102, 13103, 13104, 13107, 13112, 13085]
...
// Module 13090 (spanTimeInputToSeconds)
  ...
  if (typeof getTime === "number") {
    ...
    if (Array.isArray(getTime)) {
      ...
      if (getTime instanceof Date) {
        ...
        sum = _mod13099.timestampInSeconds();
      }
    }
  }
  return sum;
  ...
  if ((function spanIsSentrySpan(getSpanJSON) {
    ...
    try {
      ...
      if ((function spanIsOpenTelemetrySdkTraceBaseSpan(attributes) {
        ...
        obj3.op = attributes[_mod13100.SEMANTIC_ATTRIBUTE_SENTRY_OP];
        obj3.origin = attributes[_mod13100.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
        const obj2 = _mod13091;
        obj3._metrics_summary = _mod13101.getMetricSummaryJsonForSpan(getSpanJSON);
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== _mod13102.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== tmp(13102).SPAN_STATUS_OK) {
...
export const addChildSpanToSpan = function addChildSpanToSpan(arg0, arg1) {
  ...
  const result = _mod13091.addNonEnumerableProperty(arg1, _sentryRootSpan, tmp2);
  if (arg0[_sentryChildSpans]) {
    ...
    const result1 = _mod13091.addNonEnumerableProperty(arg0, tmp6, set);
...
export const getActiveSpan = function getActiveSpan() {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    activeSpan = tmpResult._getSpanForScope(tmp(13112).getCurrentScope());
...
export const spanToTraceContext = function spanToTraceContext(spanContext) {
  ...
  return _mod13091.dropUndefinedKeys({ parent_span_id, span_id, trace_id: trace_id.traceId });
};
export const spanToTraceHeader = function spanToTraceHeader(spanContext) {
  ...
  return _mod13097.generateSentryTraceHeader(traceId, spanId, 1 === spanContext.spanContext().traceFlags);
};
export const spanToTransactionTraceContext = function spanToTransactionTraceContext(spanContext) {
  ...
  return _mod13091.dropUndefinedKeys({ parent_span_id, span_id, trace_id, data, op, status, origin });
};
