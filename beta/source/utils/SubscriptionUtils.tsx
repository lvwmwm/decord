...
// Dependencies: [32, 19, 4423, 1078, 1378, 38, 11574, 4418, 7501, 558, 568, 504, 11576, 4352, 2]
...
// Module 11573 (SubscriptionUtils)
...
import PauseDuration from "PauseDuration" /* 11576 */;
...
export const getSubscriptionPauseDurations = function getSubscriptionPauseDurations(status) {
  ...
  if (status.status !== constants.PAUSED) {
    ...
    const tmp6 = _modDef4352(status.currentPeriodStart);
    const _Math = Math;
    const rounded = Math.round(_modDef4352(status.pauseEndsAt).diff(tmp6, "days", true));
    const items = [];
...
export const didBeginPurchaseFlowOnFractionalPremium = function didBeginPurchaseFlowOnFractionalPremium(isSameOrAfter) {
  ...
  if (isMomentResult) {
    isMomentResult = _modDef4352.isMoment(isSameOrAfter);
  }
  if (isMomentResult) {
    isMomentResult = isSameOrAfter.isSameOrAfter(_modDef4352());
  }
