...
// Dependencies: [2049, 2067, 1078, 8677, 17302, 7495, 5231, 17303, 1119, 2112, 7513, 5636, 17304, 7512, 17305, 9833, 8014, 2]

// Module 17301 (PermissionSpecUtils)
...
import useGuildEligibleForStageChannels from "useGuildEligibleForStageChannels" /* 17302 */;
import HangoutWindowExperiment from "HangoutWindowExperiment" /* 17303 */;
import permissions_PermissionUtilsAll from "permissions/PermissionUtils" /* 17305 */;
...
export default {
  generateChannelPermissionSpec(guild_id, stateFromStores, arg2, arg3) {
    ...
    let result = tmp4(17304).canCurrentUserManageMessageFilters(guild_id);
    ...
    if (constants2.GUILD_CATEGORY === type) {
      ...
      if (tmp.showStageChannelPermissions) {
        ...
        const items1 = [obj33(17305).generateChannelStageSection(tmp3, intl28.string(tmp4(1119).t.yniauk))];
        let items2 = items1;
        const obj33Result = obj33(17305);
      } else {
        items2 = [];
      }
      obj33 = obj33(17305);
      ...
      if (tmp16.GUILD_VOICE === type) {
        ...
        items4[6] = tmp27(17305).generateChannelAppsSection(tmp3, intl15.string(tmp4(1119).t["rrh/W6"]), { showActivities: false });
        items5 = items4;
        const tmp27Result = tmp27(17305);
      } else {
        if (tmp16.GUILD_FORUM !== type) {
          if (tmp16.GUILD_MEDIA !== type) {
            ...
            if (enabled) {
              ...
              const items6 = [tmp38(17305).generateChannelVoiceSection(tmp3, intl.string(tmp4(1119).t["46Ra1b"]))];
              let items7 = items6;
              const tmp38Result = tmp38(17305);
            } else {
              items7 = [];
            }
        ...
        if (enabled) {
          ...
          const items9 = [tmp22(17305).generateChannelVoiceSection(tmp3, intl7.string(tmp4(1119).t["46Ra1b"]))];
          let items10 = items9;
          const tmp22Result2 = tmp22(17305);
        } else {
          items10 = [];
        }
