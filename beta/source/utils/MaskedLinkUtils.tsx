...
// Dependencies: [8679, 2045, 2067, 8680, 5010, 4441, 2099, 1078, 1933, 5142, 1119, 1245, 8681, 8682, 8683, 8685, 4481, 13280, 13284, 5246, 13285, 13287, 2]
// Exports: handleClick, isLinkTrusted

// Module 8678 (MaskedLinkUtils)
import openURLDefault from "openURL" /* 4481 */;
import LinkAnalyticsUtilsDefault from "LinkAnalyticsUtils" /* 8683 */;
import BlockedDomainStore from "BlockedDomainStore" /* 8679 */;
import ChannelStore from "ChannelStore" /* 2045 */;
import GuildStore from "GuildStore" /* 2067 */;
import MaskedLinkStore from "MaskedLinkStore" /* 8680 */;
...
export const handleClick = function handleClick(href, preventDefault, arg2, contextKey) {
  ...
  const sanitizeUrlResult = channelId(1933).sanitizeUrl(href.href);
  if (null == sanitizeUrlResult) {
    ...
    try {
      ...
      if (null != channelId) {
        ...
        if (tmp3(8682)(channelId)) {
          const obj6 = { cta_type: "inline_link", target: tmp8 };
          tmp3(1245).track(constants2.CHANGE_LOG_CTA_CLICKED, obj6);
          const tmp3Result9 = tmp3(1245);
        }
      }
      tmp3(8683).trackLinkClicked(tmp8);
      if (null == onClick) {
      ...
      if (null !== guild_id1.isBlockedDomain(tmp8)) {
        ...
        if (!tmp55) {
          ...
          if (tmp56) {
            const obj8 = { url: tmp8, trustUrl: tmp53(13287).trustProtocol, onConfirm: handleConfirm, onCancel, isProtocol: true, contextKey };
            tmp3(13285).show(obj8);
            const tmp3Result12 = tmp3(13285);
          } else {
            const punycodeLinkResult = tmp53(5246).punycodeLink(tmp8);
            let displayTarget = tmp8;
            if (null != punycodeLinkResult) {
              displayTarget = punycodeLinkResult.displayTarget;
            }
            const tmp53Result = tmp53(5246);
            const obj9 = { url: displayTarget, trustUrl: tmp53(13287).trustDomain, onConfirm: handleConfirm, onCancel, isProtocol: false, contextKey };
            tmp3(13285).show(obj9);
            const tmp3Result13 = tmp3(13285);
          }
        }
        if (null == preventDefault) {
          handleConfirm();
          ...
          let result = tmp3(8683).trackAnnouncementMessageLinkClicked(obj11);
          const tmp3Result14 = tmp3(8683);
        }
        const obj10 = require("MaskedLinkStoreMethodsAdditional");
      }
      const tmp3Result10 = tmp3(8683);
    } catch (err) {
