...
// Function ID: 4739
// Name: parseURL
// Dependencies: [32, 1078, 1080, 4739, 4740, 1476, 1933, 1372, 4741, 4746, 13241, 4996, 14120, 1370, 4912, 9327, 7650, 1613, 1368, 9958, 1245, 1258, 14121, 2]
// Exports: default

// Module 4738 (parseURL)
...
import urlPartToSettingsEnumDefault from "urlPartToSettingsEnum" /* 14121 */;
...
export default function parseURL(arg0) {
  ...
  if (null == sanitizeUrlResult) {
    ...
    if (null != findCodedLinkResult) {
      ...
      if (tmp5(4746).CodedLinkType.INVITE === type) {
        ...
        obj5.payload = obj6;
        return obj5;
      } else if (tmp5(4746).CodedLinkType.TEMPLATE === type) {
        ...
        if (tmp5(4746).CodedLinkType.BUILD_OVERRIDE !== type) {
          if (tmp5(4746).CodedLinkType.MANUAL_BUILD_OVERRIDE !== type) {
            if (tmp5(4746).CodedLinkType.EXPERIMENT !== type) {
              if (tmp5(4746).CodedLinkType.EVENT !== type) {
                if (tmp5(4746).CodedLinkType.CHANNEL_LINK !== type) {
                  if (tmp5(4746).CodedLinkType.APP_DIRECTORY_PROFILE !== type) {
                    if (tmp5(4746).CodedLinkType.ACTIVITY_BOOKMARK === type) {
                      ...
                      obj9.payload = obj10;
                      return obj9;
                    } else if (tmp5(4746).CodedLinkType.EMBEDDED_ACTIVITY_INVITE !== type) {
                      if (tmp5(4746).CodedLinkType.GUILD_PRODUCT !== type) {
                        if (tmp5(4746).CodedLinkType.SERVER_SHOP !== type) {
                          if (tmp5(4746).CodedLinkType.SOCIAL_LAYER_STOREFRONT !== type) {
                            if (tmp5(4746).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP !== type) {
                              if (tmp5(4746).CodedLinkType.QUESTS_EMBED !== type) {
                                if (tmp5(4746).CodedLinkType.GAME_PROFILE === type) {
                                  ...
                                  obj11.payload = obj12;
                                  return obj11;
                                } else if (tmp5(4746).CodedLinkType.APP_DIRECTORY_STOREFRONT !== type) {
                                  if (tmp5(4746).CodedLinkType.APP_DIRECTORY_STOREFRONT_SKU !== type) {
                                    if (tmp5(4746).CodedLinkType.APP_OAUTH2_LINK !== type) {
                                      if (tmp5(4746).CodedLinkType.COLLECTIBLES_SHOP === type) {
                                        const tmp5Result = tmp5(13241);
                                      ...
                                      } else if (tmp5(4746).CodedLinkType.GAME_SERVER_SHARE !== type) {
                                        if (tmp5(4746).CodedLinkType.USER_PROFILE !== type) {
                                          const _Error2 = Error;
    ...
    if (findGiftCodesResult.length > 0) {
      ...
      const result = tmp5(14120).findRemoteAuthFingerprint(host, pathname);
      if (null != result) {
        if (result.length > 0) {
      ...
      if (null != pathname) {
        const tryParseDiceRollLinkResult = tmp5(4912).tryParseDiceRollLink(pathname);
        if (null != tryParseDiceRollLinkResult) {
          ...
          const tryParseChannelPathResult = tmp5(4912).tryParseChannelPath(pathname);
          if (null != tryParseChannelPathResult) {
            ...
            if (null != pathname.match(re27)) {
              ...
              let ad_creative_ids1 = tmp(1476).parse(str24).ad_creative_ids;
              ...
              if (null != pathname.match(re17)) {
                  ...
                  if (match5.length > 1) {
                    ...
                    switch (match5[1]) {
                      ...
                      case "mobile-web-redirect-checkout":
                        let result2 = tmp5(7650).isMobileWebRedirectCheckoutEnabled();
                        if (result2) {
                ...
                if (null != result3) {
                  ...
                  obj86.fingerprint = tmp65(1258).maybeExtractId(fingerprint3);
          ...
          const tmp5Result11 = tmp5(4912);
        }
        const tmp5Result10 = tmp5(4912);
      }
      tmpResult6 = tmp(1370);
    }
  }
