...
// Function ID: 9940
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9940, 9941]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      let arr2 = items[num];
      let bytesResult = Hash(9940).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9941).crypto) {
    if (typeof tmp(9941).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9941).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9941).crypto) {
    if (typeof tmp(9941).crypto.randomBytes === "function") {
      const _crypto = tmp(9941).crypto;
