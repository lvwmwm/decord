...
// Function ID: 9962
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9962, 9963]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      let arr2 = items[num];
      let bytesResult = Hash(9962).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9963).crypto) {
    if (typeof tmp(9963).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9963).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9963).crypto) {
    if (typeof tmp(9963).crypto.randomBytes === "function") {
      const _crypto = tmp(9963).crypto;
