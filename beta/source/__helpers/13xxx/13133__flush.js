...
// Function ID: 13134
// Name: _flush
// Dependencies: [5, 13112, 13134, 13113, 13085, 13096, 13099, 13092, 13122, 13086, 13108]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        const client = require("module_13112").getClient();
        if (client) {
          client.flush(tmp13);
        } else {
        ...
        c1 = 3;
        const obj3 = require("module_13112");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(13108).closeSession(tmp3);
    const tmpResult = tmp(13108);
  }
  const isolationScope1 = _mod13112.getIsolationScope();
  const tmpResult4 = _mod13112;
  const currentScope1 = _mod13112.getCurrentScope();
  const tmpResult5 = _mod13112;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod13112.getCurrentScope();
  return currentScope.captureException(arg0, _mod13134.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(13122).DEFAULT_ENVIRONMENT;
  }
  const userAgent = _mod13086.GLOBAL_OBJ.navigator || {}.userAgent;
  const tmp3 = _mod13086.GLOBAL_OBJ.navigator || {};
  const obj2 = { release, environment, user: null };
  const tmpResult = _mod13108;
  ...
  if (tmp10) {
    tmp(13108).updateSession(session1, { status: "exited" });
    const tmpResult2 = tmp(13108);
  }
  endSession();
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod13099.timestampInSeconds() - closure_3 };
    const currentScope = _mod13112.getCurrentScope();
    const client = _mod13112.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_13112").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_13112");
  let client = require("module_13112").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(13113).DEBUG_BUILD) {
      let logger2 = tmp(13085).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    asyncGeneratorStep = captureCheckInResult;
    closure_3 = tmp(13099).timestampInSeconds();
    const tmpResult = tmp(13099);
    return tmp(13112).withIsolationScope(() => {
      try {
    ...
    let logger = tmp(13085).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = require("module_13112");
  captureCheckInResult = require("module_13096").uuid4();
};
