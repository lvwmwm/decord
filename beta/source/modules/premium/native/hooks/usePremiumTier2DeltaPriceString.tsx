...
// Dependencies: [19, 7666, 7482, 1374, 7651, 7485, 1364, 7479, 4429, 504, 2]
// Exports: usePremiumTier2DeltaPriceString

// Module 13814 (usePremiumTier2DeltaPriceString)
...
function getViewerProductId(subscription) {
  if (null == subscription) {
    ...
    try {
      ...
      try {
        const productIdFromSubscription1 = tmp3(7651).getProductIdFromSubscription(subscription, true);
        const tmp8 = tmp3(7485).AppStorePremiumProductIdsToPremiumBundledItems[productIdFromSubscription];
        const tmp10 = tmp3(7485).AppStorePremiumProductIdsToPremiumBundledItems[productIdFromSubscription1];
        if (null != tmp8) {
          if (null != tmp10) {
...
export const usePremiumTier2DeltaPriceString = function usePremiumTier2DeltaPriceString(plan, subscription, currencyCode, flag4) {
  ...
  if (flag4) {
    ...
    if (plan.premiumTier === PremiumTypes.TIER_2) {
      ...
      if (plan.numPremiumGuild >= 1) {
        ...
        let tmp11 = null;
        if (null != tmp2Result) {
          tmp11 = tmp4(7485).AppStorePremiumProductIdsToPremiumBundledItems[tmp2Result];
        }
  ...
  if (flag) {
          ...
          if (currencyCode.currencyCode !== stateFromStores.currencyCode) {
            ...
            if (diff > 0) {
              if (diff < currencyCode.price) {
                ...
                obj5.priceString = tmp4(7479).formatPrice(result, currencyCode.currencyCode, { convertToMajorUnits: false });
                obj6 = obj5;
                const tmp4Result8 = tmp4(7479);
              }
            }
    ...
    if (null == checkoutContext) {
      ...
      const availablePlanForItems = checkoutContext.getAvailablePlanForItems(tmp4(7651).getSubscriptionItemsForProduct(plan.productId));
      if (null == availablePlanForItems) {
        ...
        if (null != addOnPrice) {
          if (addOnPrice.majorUnits > 0) {
            let tmp17 = null;
            if (null != tmp3) {
              tmp17 = tmp4(7485).AppStorePremiumProductIdsToPremiumBundledItems[tmp3];
            }
            if (null != tmp3) {
              if (null != tmp17) {
                if (0 !== tmp17.numPremiumGuild) {
                  const availablePlanForItems1 = checkoutContext.getAvailablePlanForItems(tmp4(7651).getSubscriptionItemsForProduct(tmp3));
                  let addOnPrice1;
                  ...
                  if (null != addOnPrice1) {
                    ...
                    if (diff1 > 0) {
                      const obj8 = { priceString: tmp4(7479).formatPrice(diff1, addOnPrice.currency, { convertToMajorUnits: false }), failure: null };
                      const tmp4Result11 = tmp4(7479);
                    }
                  }
                  const tmp4Result10 = tmp4(7651);
                }
              }
            }
            const obj9 = { priceString: tmp4(7479).formatPrice(addOnPrice.majorUnits, addOnPrice.currency, { convertToMajorUnits: false }), failure: null };
            const tmp4Result12 = tmp4(7479);
          }
        }
      }
      const tmp4Result9 = tmp4(7651);
    }
  } else {
