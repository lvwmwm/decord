...
// Dependencies: [1078, 5234, 8101, 38, 1098, 2]
// Exports: getAnalyticsPage, getUpsellType, isSoundboardSectionNitroLocked

// Module 10220 (PremiumFeatureUpsellUtils)
...
export const getAnalyticsPage = function getAnalyticsPage(featureName) {
  ...
  } else if (tmp(8101).EntitlementFeatureNames.SOUNDBOARD_EVERYWHERE === featureName) {
    return AnalyticsPages.PREMIUM_UPSELL_SOUNDBOARD_EVERYWHERE;
  } else if (tmp(8101).EntitlementFeatureNames.CLIENT_THEMES === featureName) {
  ...
  } else if (tmp(8101).EntitlementFeatureNames.SAVED_MESSAGES === featureName) {
    return AnalyticsPages.PREMIUM_UPSELL_FOR_LATER;
  } else if (tmp(8101).EntitlementFeatureNames.SCHEDULED_MESSAGES === featureName) {
...
export const getUpsellType = function getUpsellType(EMOJIS_EVERYWHERE) {
  ...
  } else if (tmp(8101).EntitlementFeatureNames.EMOJIS_EVERYWHERE === EMOJIS_EVERYWHERE) {
    return tmp(1098).UpsellTypes.GLOBAL_EMOJI;
  } else if (tmp(8101).EntitlementFeatureNames.STICKERS_EVERYWHERE === EMOJIS_EVERYWHERE) {
    return tmp(1098).UpsellTypes.GLOBAL_STICKER;
  } else if (tmp(8101).EntitlementFeatureNames.INCREASED_FILE_UPLOAD_SIZE === EMOJIS_EVERYWHERE) {
    return tmp(1098).UpsellTypes.UPLOAD;
  } else if (tmp(8101).EntitlementFeatureNames.SOUNDBOARD_EVERYWHERE === EMOJIS_EVERYWHERE) {
    return tmp(1098).UpsellTypes.SOUNDBOARD;
  } else if (tmp(8101).EntitlementFeatureNames.CLIENT_THEMES === EMOJIS_EVERYWHERE) {
    return tmp(1098).UpsellTypes.CLIENT_THEMES;
  } else if (tmp(8101).EntitlementFeatureNames.APP_ICONS === EMOJIS_EVERYWHERE) {
    return tmp(1098).UpsellTypes.APP_ICONS;
  } else if (tmp(8101).EntitlementFeatureNames.SAVED_MESSAGES === EMOJIS_EVERYWHERE) {
    return tmp(1098).UpsellTypes.FOR_LATER;
  } else if (tmp(8101).EntitlementFeatureNames.SCHEDULED_MESSAGES === EMOJIS_EVERYWHERE) {
    return tmp(1098).UpsellTypes.SCHEDULED_MESSAGES;
  } else if (tmp(8101).EntitlementFeatureNames.STREAM_HIGH_QUALITY === EMOJIS_EVERYWHERE) {
    return tmp(1098).UpsellTypes.STREAM_HIGH_QUALITY;
  } else if (tmp(8101).EntitlementFeatureNames.SHOP_MEMBER_PRICING === EMOJIS_EVERYWHERE) {
