...
export const isXboxEntry = function isXboxEntry(content_type) {
  ...
  if (!tmp4) {
    ...
    tmp4 = content_type1 === tmp2(8399).ContentInventoryEntryType.TOP_GAME;
  }
  let tmp6 = tmp4;
  if (tmp6) {
...
export const isConsoleEntry = function isConsoleEntry(content_type) {
  ...
  if (!tmp6) {
    ...
    let tmp8 = content_type2 === tmp2(8399).ContentInventoryEntryType.PLAYED_GAME;
    if (!tmp8) {
      let content_type3;
      if (content_type != null) {
        content_type3 = content_type.content_type;
      }
      tmp8 = content_type3 === tmp2(8399).ContentInventoryEntryType.TOP_GAME;
    }
    let tmp10 = tmp8;
    if (tmp10) {
...
export const isRecentActivityEntry = function isRecentActivityEntry(length) {
  ...
  if (!tmp4) {
    ...
    tmp4 = content_type2 === tmp2(8399).ContentInventoryEntryType.LISTENED_SESSION;
  }
  if (!tmp4) {
    let content_type3;
    if (length != null) {
    ...
    tmp4 = content_type3 === tmp2(8399).ContentInventoryEntryType.WATCHED_MEDIA;
  }
  if (!tmp4) {
    let extra;
    if (length != null) {
