...
// Dependencies: [32, 19, 17, 5844, 4755, 21, 5734, 4756, 576, 5822, 4454, 7446, 504, 5904, 16004, 7226, 16021, 5184, 16022, 7295, 2]
// Exports: default

// Module 16020 (DevToolsTogglesScreen)
...
function fuzzySearchToggle(str, str2, str3) {
  ...
  if (!tmp) {
    ...
    if (!tmp3ResultResult) {
      ...
      tmp3ResultResult = tmp3(5734)(formatted1, str3.toLowerCase());
      const tmp3Result = tmp3(5734);
    }
...
function ToggleTableRow(toggleName) {
  ...
  return closure_9(toggleName(5822).TableRow, {
    ...
    trailing: closure_9(toggleName(7446).FormSwitch, { value, onValueChange })
  }, toggleName);
}
function DevTogglesForCategory(title) {
  ...
  if (0 !== stateFromStores.length) {
    ...
    tmp3 = closure_9(category(5904).TableRowGroup, obj2);
  }
  return tmp3;
}
...
export default function DevToolsTogglesScreen() {
  ...
  const query = tmp3[0];
  const manaTextMigrationHighlightRestartNotice = query(16021).useManaTextMigrationHighlightRestartNotice();
  let obj = query(16021);
  const tmp5 = query;
  ...
  const stateFromStores = query(504).useStateFromStores(items, () => DesignTogglesStore.allWithDescriptions().filter((item) => {
    ...
    if (!tmp) {
      ...
      if (!tmp2ResultResult) {
        ...
        tmp2ResultResult = tmp2(5734)(formatted1, str2.toLowerCase());
        const tmp2Result = tmp2(5734);
      }
  ...
  const items3 = [
    closure_9(query(5822).TableRow, {
      ...
      variant: "danger",
      onPress() {
        first(16022).clearAll();
        const obj = first(16022);
        first(16004).clearAll();
      },
      arrow: true
    }),
  ...
  items3[1] = closure_9(query(5822).TableRow, { label: closure_9(query(7295).SearchField, { size: "md", placeholder: "Search design toggles", onChange: tmp3[1] }) });
  obj5.children = items3;
  const items4 = [closure_10(query(5904).TableRowGroup, obj5), , ];
  let tmp7Result = null;
    ...
    const obj8 = {
      ...
      children: stateFromStores.map((item) => {
          ...
          return closure_9(closure_13, {
            ...
            value: tmp2,
            onValueChange(arg0) {
              return first(16022).toggle(query, arg0);
            }
          }, tmp);
        })
    };
    tmp7Result = tmp7(tmp5(5904).TableRowGroup, obj8);
  }
