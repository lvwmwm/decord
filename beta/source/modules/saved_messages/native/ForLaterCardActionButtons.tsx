...
// Dependencies: [19, 17, 21, 4758, 4725, 11836, 1984, 11830, 11835, 1119, 11864, 5846, 13599, 10626, 8182, 8178, 8181, 2]
// Exports: default

// Module 13598 (ForLaterCardActionButtons)
...
export default function ForLaterCardActionButtons(savedMessage) {
  ...
  const callback = noop.useCallback(() => ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11836, dependencyMap.paths), "MessageReminderDurationActionSheet", {
    createReminder(dueAt) {
      const obj2 = {};
      const merged = Object.assign(closure_1_0.saveData);
      obj2.dueAt = dueAt;
      obj2.source = savedMessage(11835).SavedMessageSources.FOR_LATER_LIST;
      return savedMessage(11830).addOrUpdateSavedMessage(obj2);
    },
    removeReminder() {
      return savedMessage(11830).removeSavedMessage({ channelId: closure_1_0.saveData.channelId, messageId: closure_1_0.saveData.messageId, displayToast: true, isReminder: true });
    },
  ...
  obj.IconComponent = savedMessage(11864).ChatArrowRightIcon;
  ...
  items1[1] = {
    ...
    IconComponent: savedMessage(5846).XSmallIcon,
    action() {
  ...
  if (null == savedMessage.saveData.dueAt) {
    ...
    obj3.children = jsx(tmp3(8182).ContextMenu, {
      items: items1,
      keyboardShouldPersistTaps: "handled",
      triggerOnTap: true,
      children(ref) {
    ...
    if (savedMessage.throttledNow > savedMessage.saveData.dueAt) {
      let PencilIcon = tmp3(13599).BellZIcon;
    } else {
      PencilIcon = tmp3(10626).PencilIcon;
    }
