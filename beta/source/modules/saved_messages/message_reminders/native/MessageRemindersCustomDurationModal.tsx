...
// Dependencies: [32, 19, 17, 21, 4758, 580, 1616, 1879, 4725, 9780, 1984, 1119, 5848, 8112, 1368, 5839, 5341, 4754, 2]

// Module 11839 (MessageRemindersCustomDurationModal)
...
export default noop.memo((onClose) => {
  ...
  function handleOpenDatePicker(date) {
    ...
    obj2.openLazy(asyncRequireImpl(9780, dependencyMap.paths), "DatePicker", obj3);
  }
  const tmp = onPress();
  ...
  obj5 = onClose(1368);
  obj3.headerLeft = onClose(5839).getHeaderCloseButton(onClose);
  ...
  const items1 = [error(onClose(5848).Header, obj3), ];
  ...
  const items2 = [error(onClose(4754).Text, obj8), ];
  ...
  obj9.children = error(onClose(4754).Text, { variant: "text-md/medium", children: formatResult });
  items2[1] = error(onClose(5341).PressableOpacity, obj9);
  ...
  const items4 = [error(onClose(4754).Text, obj10), , ];
  ...
  obj11.children = error(onClose(4754).Text, { variant: "text-md/medium", children: formatResult1 });
  items4[1] = error(onClose(5341).PressableOpacity, obj11);
  ...
  if (tmp10Result) {
    ...
    tmp10Result = tmp10(tmp11(4754).Text, obj12);
  }
