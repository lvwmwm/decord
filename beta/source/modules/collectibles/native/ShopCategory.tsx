...
// Function ID: 16135
// Name: ShopCategory
// Dependencies: [19, 17, 1080, 1078, 21, 9074, 4790, 580, 558, 568, 7441, 1488, 15341, 16130, 9027, 16135, 9077, 4757, 8482, 7820, 7461, 1119, 5834, 7488, 5373, 2]

// Module 16134 (ShopCategory)
import c from "c" /* 568 */;
...
const sum = 100 + fn(9074).COLLECTIBLES_SHOP_CARD_HEIGHT;
...
export const ShopCategory = ReactCompilerGating.isReactCompilerEnabled() ? ((arg0) => {
  ...
  const filteredAndSortedProducts = category(15341).useFilteredAndSortedProducts({ products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive });
  const mobileBannerUrl = category.mobileBannerUrl;
  let obj2 = category(15341);
  let obj3 = { products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive };
  const collectiblesShopDeepLinkProps = category(16130).useCollectiblesShopDeepLinkProps({ products: filteredAndSortedProducts });
  ({ productIndex, initialProductSkuId } = collectiblesShopDeepLinkProps);
  const initialVariantIndex = collectiblesShopDeepLinkProps.initialVariantIndex;
  const ref = unpublishedAt.useRef(null);
  const obj4 = category(16130);
  let items = [category.storeListingId];
  const recyclingState = category(9027).useRecyclingState(null, items, () => {
    const current = ref.current;
    if (current != null) {
      current.scrollToOffset({ offset: 0, animated: false });
    }
  });
  const obj6 = category(9027);
  let tmp9 = null != productIndex;
  if (tmp9) {
    tmp9 = productIndex > 0;
  }
  const obj7 = category(16135);
  const scrollToInitialIndexOnce = obj7.useScrollToInitialIndexOnce({ shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(16135).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId });
  const obj8 = { shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(16135).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId };
  collectiblesAnalyticsContext = category(9077).useCollectiblesAnalyticsContext();
  ...
  const tmp4Result = category(9077);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(5834), obj14);
  }
  const items4 = [tmp14Result, ];
  const obj13 = { radius: analyticsLocations(580).radii.lg };
  items4[1] = ref(filteredAndSortedProducts, { style: tmp3.viewAllIcon, children: ref(category(7488).ChevronSmallRightIcon, { size: "sm", color: "white" }) });
  obj11.children = items4;
  const items5 = [collectiblesAnalyticsContext(category(5373).PressableOpacity, obj11, category.storeListingId), ];
  ...
  obj17.snapToInterval = category(9074).COLLECTIBLES_SHOP_CARD_WIDTH + 12;
  ...
  items5[1] = ref(category(9027).FlashList, obj17);
  obj10.children = items5;
  obj9.children = collectiblesAnalyticsContext(filteredAndSortedProducts, obj10);
  return ref(category(9077).CollectiblesAnalyticsProvider, obj9);
});
