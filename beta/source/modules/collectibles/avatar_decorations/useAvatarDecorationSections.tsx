...
// Module 13475 (useAvatarDecorationSections)
...
export default ReactCompilerGating.isReactCompilerEnabled() ? (() => {
    ...
    if (cResult[5] === tmp13) {
      if (cResult[6] === stateFromStores) {
        ...
        if (cResult[13] === Symbol.for("react.memo_cache_sentinel")) {
          ...
          const stringResult = intl.string(tmp(1119).t.VqmVqE);
  ...
  let avatarDecorations = stateFromStores(7801).getAvatarDecorations(stateFromStores, tmp12);
  if (cResult[10] === tmp13) {
    if (cResult[11] === stateFromStores) {
      let tmp17 = cResult[12];
    }
    const obj5 = { purchase: [], premium_purchase: [], preview: [] };
    const reduced = avatarDecorations.reduce(tmp17, obj5);
    const PURCHASE = obj.PURCHASE;
    const items3 = [obj2, ];
    avatarDecorations = obj3;
    items3[1] = obj3;
