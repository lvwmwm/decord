...
export const getCardProductName = function getCardProductName(product) {
  if (null == product) {
    ...
    if (null != product) {
      ...
      if (product.type === CollectiblesItemType.CollectiblesItemType.BUNDLE) {
        ...
        if (product.items.length > 0) {
          ...
          str2 = intl2.format(tmp3(1119).t.UTc0ny, obj2);
        }
      }
    }
    return str2;
    ...
    if ("baseVariantName" in product) {
      const intl = BZN5k2(1119).intl;
      BZN5k2 = BZN5k2(1119).t.BZN5k2;
    ...
    if ("baseVariantName" in product) {
      if (null != product.baseVariantName) {
        if (product.variantLabel !== tmp3) {
          ...
          ({ baseVariantName: obj.baseVariantName, variantLabel: obj.variantLabel } = product);
          let name = intl.formatToPlainString(tmp(1119).t.BZN5k2, obj);
        }
        return name;
      }
    }
...
export const getProductNameAndTypeLabel = function getProductNameAndTypeLabel(product) {
  if (null == product) {
    ...
    if (CollectiblesItemType.CollectiblesItemType.AVATAR_DECORATION === productType) {
      ...
      const obj3 = { product: "" };
      return intl5.formatToPlainString(tmp3(1119).t.lvBzLi, obj3);
    } else if (tmp3(1977).CollectiblesItemType.PROFILE_EFFECT === productType) {
      const intl4 = tmp3(1119).intl;
      const obj4 = { product: "" };
      return intl4.formatToPlainString(tmp3(1119).t.eR7moP, obj4);
    } else if (tmp3(1977).CollectiblesItemType.NAMEPLATE === productType) {
      const intl3 = tmp3(1119).intl;
      const obj5 = { product: "" };
      return intl3.formatToPlainString(tmp3(1119).t.YFOwHj, obj5);
    } else if (tmp3(1977).CollectiblesItemType.PROFILE_FRAME === productType) {
      const intl2 = tmp3(1119).intl;
      const obj6 = { product: "" };
      return intl2.formatToPlainString(tmp3(1119).t.vov8LX, obj6);
    } else {
      return "";
    }
  } else {
