...
// Dependencies: [5, 32, 19, 17, 6866, 4774, 9049, 16290, 16291, 1078, 21, 4790, 580, 4529, 7217, 16287, 504, 16313, 16315, 16298, 1119, 16306, 16289, 16305, 7245, 16316, 16317, 5220, 7214, 7319, 2]
// Exports: default

// Module 16312 (RegisterAccountInformation)
...
import AuthFormViewDefault from "AuthFormView" /* 7245 */;
import useInitialRegistrationStepDefault from "useInitialRegistrationStep" /* 16305 */;
import useAuthFlowBackHandlerDefault from "useAuthFlowBackHandler" /* 16306 */;
...
obj2.layout = LinearTransition.easing(Easing.inOut(fn(4529).Easing.quad)).duration(300);
const size = fn(2);
...
export default function RegisterAccountInformation() {
  ...
  context = noop.useContext(context(16287).TrackRegistrationContext);
  ...
  const passwordRegistrationStep = context(16313).usePasswordRegistrationStep();
  ({ password, validatePassword: _slicedToArray, setPassword, passwordScore, preventSubmitPassword } = passwordRegistrationStep);
  let obj4 = context(16313);
  const usernameRegistrationStep = context(16315).useUsernameRegistrationStep(AuthStates.REGISTER_ACCOUNT_INFORMATION);
  ...
  const result = context(16298).hasAllRegistrationFieldsCompleted(obj6, { isConsentRequired: true === stateFromStores1 });
  ConsentStore = result;
  ...
  const tmp5Result3 = context(16298);
  const tmpResult = useAuthFlowBackHandlerDefault;
  tmpResult(context(16289).getPreviousRegistrationTransitionStep(AuthStates.REGISTER_ACCOUNT_INFORMATION));
  ...
  const tmp5Result4 = context(16289);
  const intl2 = tmp5(1119).intl;
  obj8.headerText = intl2.string(context(1119).t.jec90v);
  ...
  const items4 = [closure_17(closure_6, { style: tmp4.container, children: closure_17(context(16316).RegisterUsernameInput, { username, setUsername, onSubmitEditing: callback, usernameStatus, submitBehavior: "submit", autoFocus: true }) }), , ];
  ...
  obj11.children = closure_17(context(16317).RegisterPasswordInput, obj12);
  ...
  const items5 = [closure_17(context(5220).Button, obj14), ];
  let tmp27Result = null;
  if (null != tmp11.message) {
    tmp27Result = null;
    if ("" !== tmp11.message) {
      const obj15 = { style: tmp4.errors, children: tmp11.message };
      tmp27Result = tmp27(tmp(7214), obj15);
    }
  }
  ...
  if (!tmp3) {
    ...
    tmp27Result4 = tmp27(tmp(7319), obj16);
  }
  return tmp27Result4;
};
