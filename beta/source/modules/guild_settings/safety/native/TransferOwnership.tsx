...
// Dependencies: [5, 32, 19, 17, 1376, 12013, 1078, 21, 4790, 580, 4494, 1488, 504, 38, 9865, 12011, 4489, 4942, 5831, 4786, 12015, 1181, 1119, 4635, 5935, 5851, 5220, 2]
// Exports: default

// Module 12014 (TransferOwnership)
...
export default function TransferOwnership(guild) {
          ...
          if (arg0 === 1) {
            ...
            if (!mfaEnabled.mfaEnabled) {
              if (null != tmp27.email) {
                v1 = 1;
                dependencyMap = 1;
                const obj6 = { value: v1(9865).sendTransferOwnershipPincode(user.id), done: false };
                return obj6;
              }
            }
            let MFA = null;
            ...
            const obj4 = v1(9865);
            v1(9865).transferOwnership(user.id, id.id, MFA).then(() => {
              v1(12011).close();
              const obj = v1(12011);
              v1(9865).close();
              const obj2 = v1(9865);
              const result = closure_1_0(4489).showTransferOwnershipSuccess();
            }, NOOP);
            dependencyMap = 3;
            const transferOwnershipResult = v1(9865).transferOwnership(user.id, id.id, MFA);
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
          throw value;
  ...
  const token = guild(4494).useToken(toUser(580).modules.mobile.TABLE_ROW_PADDING);
  let tmp5 = closure_16();
  let obj = guild(4494);
  ...
  nickname = toUser(4942).getNickname(guild.id, undefined, toUser);
  ...
  let obj4 = toUser(4942);
  obj6.style = { paddingTop: toUser(580).space.PX_16, paddingHorizontal: token };
  const obj8 = { style: tmp5.header, children: null };
  const obj7 = { paddingTop: toUser(580).space.PX_16, paddingHorizontal: token };
  const tmp12 = closure_8;
  const items1 = [closure_13(c7, { source: toUser(12015), style: tmp5.arrow }), , , ];
  const obj10 = { style: tmp5.avatarsWrapper, children: null };
  const obj9 = { source: toUser(12015), style: tmp5.arrow };
  const items2 = [closure_13(guild(1181).Avatar, { user: stateFromStores, guildId: guild.id, size: guild(1181).AvatarSizes.XXLARGE }), ];
  const obj12 = { style: tmp5.avatarFauxBorder, children: null };
  const obj13 = { user: toUser, guildId: "r", size: guild(1181).AvatarSizes.XXLARGE, style: tmp5.otherUserAvatar };
  obj12.children = closure_13(guild(1181).Avatar, obj13);
  items2[1] = closure_13(nickname, obj12);
  obj10.children = items2;
  items1[1] = closure_15(nickname, obj10);
  items1[2] = closure_13(guild(4786).Text, { variant: "text-xs/medium", color: "text-default", children: guild.name });
  if (null == nickname) {
    if (!hasAvatarForGuildResult) {
      const intl = tmp(1119).intl;
      const obj15 = { GuildHook: GuildWithSmallIcon, user: tmp3(4635).getUserTag(toUser) };
      let formatResult = intl.format(tmp(1119).t["2XLnG0"], obj15);
    ...
    const obj19 = { username: tmp3(4635).getUserTag(toUser) };
    obj18.label = intl3.format(tmp(1119).t.xm6ACJ, obj19);
    obj18.onPress = function handleConfirmToggle(arg0) {
      _undefined(arg0);
    };
    obj17.children = tmp11(tmp(5851).TableCheckboxRow, obj18);
    items3[1] = tmp11(tmp(5935).TableRowGroup, obj17);
    const obj20 = {
    ...
    obj20.text = intl4.string(tmp(1119).t.jqqLb6);
    obj20.disabled = !tmp8;
    items3[2] = tmp11(tmp(5220).Button, obj20);
  ...
  const obj11 = { user: stateFromStores, guildId: guild.id, size: guild(1181).AvatarSizes.XXLARGE };
  const obj14 = { variant: "text-xs/medium", color: "text-default", children: guild.name };
  obj21.user = toUser(4635).getUserTag(toUser);
  obj21.AKAHook = function NicknameAKA() {
    ...
    let tmp6 = null;
    if (c7) {
      const obj2 = { style: tmp.miniAvatar, user: toUser, guildId: guild.id, size: tmp4(1181).AvatarSizes.XXSMALL };
      tmp6 = __initData2(tmp4(1181).Avatar, obj2);
    }
    items[1] = tmp6;
    let str = nickname;
    if (nickname == null) {
  ...
  formatResult = intl2.format(tmp(1119).t.E90vgp, obj21);
};
