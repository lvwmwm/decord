...
// Dependencies: [5, 32, 19, 17, 9845, 2067, 9866, 1078, 8239, 21, 4790, 580, 1488, 9868, 504, 9846, 8471, 9847, 9865, 4489, 1119, 12678, 4683, 5871, 7653, 1489, 5143, 1181, 7317, 10029, 4786, 5935, 7478, 5854, 5218, 14224, 8908, 14222, 18035, 2]
// Exports: default

// Module 18034 (GuildSettingsModalServerTag)
...
export default function GuildSettingsModalServerTag(guildId) {
  ...
  let obj = guildId(1488);
  const result = guildId(9868).canUseMobileServerTagSettings(guildId);
  dependencyMap = result;
  let obj2 = guildId(9868);
  const items = [result1];
  ...
  if (guildSupportsTagsResult) {
    guildSupportsTagsResult = tmp2(8471).guildSupportsTags(stateFromStores);
  ...
  result1 = guildId(9868).isServerTagDraftDirty(stateFromStores1, stateFromStores2);
    ...
    if (c3 === 2) {
      ...
      try {
        ...
        if (0 === dependencyMap) {
              ...
              if (!ref.current) {
                if (closure_9) {
                  if (dependencyMap) {
                    ...
                    dependencyMap = 1;
                    c3 = 1;
                    const obj7 = { value: tmp2(9847).saveGuildProfile(guildId, { tag: null, badge: null, badgeColorPrimary: null, badgeColorSecondary: null }), done: false };
                    return obj7;
                  }
                }
              }
          ...
          tmp2(4489).presentError(intl.string(tmp2(1119).t.RTSuVn));
          const obj = tmp2(4489);
        }
        c3 = 3;
      } catch (tmp27) {
  ...
  const effect2 = obj7.useEffect(() => {
    if (c2) {
      if (closure_12) {
        if (c7) {
          let obj = {
            headerRight: first ? (() => closure_1_13(guildId(5871).HeaderSubmittingIndicator, {})) : (() => {
                    ...
                    return closure_13(guildId(7653).HeaderActionButton, obj);
                  })
          };
  ...
  const tmp2Result3 = guildId(9868);
  let tmp26 = result1;
  if (result1) {
    tmp26 = !first;
  }
  const preventRemove = guildId(1489).usePreventRemove(tmp26, (data) => {
  ...
  if (result) {
    if (tmp24) {
      if (null == stateFromStores) {
        ...
        return closure_13(tmp2(7317).SceneLoadingIndicator, {});
      } else {
        let tmp29;
        if (tmp14) {
        ...
        let tmp31Result;
        if (null != tmp29) {
          const obj13 = { direction: "horizontal", align: "center", spacing: navigation(580).space.PX_4, children: null };
        ...
        const obj15 = { spacing: navigation(580).space.PX_16, children: null };
        if (guildSupportsTagsResult) {
          const obj17 = { variant: "text-sm/medium", color: "text-subtle", style: tmp.description, children: null };
          let intl4 = tmp2(1119).intl;
          obj17.children = intl4.string(tmp2(1119).t["qVCnq+"]);
          const items14 = [tmp32(tmp2(4786).Text, obj17), tmp31Result2, , ];
          const obj18 = { variant: "eyebrow", color: "text-muted", style: tmp.description, accessibilityRole: "header", children: null };
          const intl5 = tmp2(1119).intl;
          obj18.children = intl5.string(tmp2(1119).t.SKNnqq);
          items14[2] = tmp32(tmp2(4786).Text, obj18);
          ...
          items14[3] = tmp32(tmp39(14222), obj19);
          obj15.children = items14;
          let tmp40 = obj15;
        } else {
          const obj20 = { guildId, onUnlockPress: callback2 };
          const items15 = [tmp32(tmp39(18035), obj20), tmp31Result2];
          obj15.children = items15;
          tmp40 = obj15;
        }
        obj14.children = closure_14(tmp2(5218).Stack, tmp40);
        return closure_13(tmp2(8908).Form, obj14);
      }
    } else {
      return closure_13(tmp2(7317).SceneLoadingIndicator, {});
    }
  ...
  const tmp2Result4 = guildId(1489);
};
