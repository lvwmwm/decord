...
// Function ID: 13519
// Name: GroupDMInvite
// Dependencies: [2045, 4409, 1376, 7983, 8208, 10595, 1119, 13326, 1404, 4911, 2]
// Exports: createGroupDMInvite

// Module 13518 (GroupDMInvite)
...
export const createGroupDMInvite = function createGroupDMInvite(invite, arg1, theme) {
  ...
  if (recipients_.length > 0) {
    ...
    formatToPlainStringResult = intl3.formatToPlainString(tmp8(1119).t.zRl6XR, obj);
  }
  let channelIconSource = null;
  if (null != channel) {
    channelIconSource = tmp8(13326).getChannelIconSource(channel);
    const tmp8Result = tmp8(13326);
  }
  let uri = null;
  if (null != channelIconSource) {
    uri = tmp8(1404).ensureAvatarSource(channelIconSource).uri;
    const tmp8Result4 = tmp8(1404);
  }
  let channelName = null;
  if (flag) {
    channelName = null;
    if (null != channel) {
      channelName = tmp8(4911).computeChannelName(channel, UserStore, RelationshipStore);
      const tmp8Result5 = tmp8(4911);
    }
  }
  if (!channelName) {
  ...
  if (!channelName) {
    ...
    channelName = intl4.string(tmp8(1119).t.LJpTRF);
  }
  if (flag) {
    ...
    let stringResult = intl6.string(tmp8(1119).t.cEnaWx);
  } else {
    ({ acceptLabelGreenColor, acceptLabelGreenBackgroundColor } = colors);
    const intl5 = tmp8(1119).intl;
    stringResult = intl5.string(tmp8(1119).t.XpeFYr);
  }
  const obj2 = {};
  ...
  if (flag) {
    ...
    if (null != channel) {
      channelName1 = tmp8(4911).computeChannelName(channel, UserStore, RelationshipStore);
      const tmp8Result6 = tmp8(4911);
    }
  }
