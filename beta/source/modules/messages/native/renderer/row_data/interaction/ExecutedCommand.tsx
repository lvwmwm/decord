...
// Dependencies: [17, 1390, 2045, 1376, 1078, 1404, 1401, 5022, 11777, 580, 7800, 1982, 8256, 8258, 9628, 1119, 9433, 2]
// Exports: createExecutedCommand

// Module 13531 (ExecutedCommand)
...
export const createExecutedCommand = function createExecutedCommand(message, channel, roleStyle, forcedTheme, defaultUsernameColor) {
  ...
  if (null != message.interaction) {
    ...
    if (null == user2) {
      ...
      if (result1) {
        processColorStringsResult = tmp16(8256).processColorStrings(userAuthor.colorStrings);
        const tmp16Result13 = tmp16(8256);
      }
      let processColorStringsResult1 = null;
      if (result2) {
        processColorStringsResult1 = tmp16(8256).processColorStrings(userAuthor1.colorStrings);
        const tmp16Result14 = tmp16(8256);
      }
      ...
      if (null != tmp24) {
        ...
        const displayNameFontIdForMobileUser1 = tmp16(8258).getDisplayNameFontIdForMobileUser(user3, guildId);
        const tmp16Result16 = tmp16(8258);
      }
      ...
      if (tmp) {
        if (!result) {
          const result3 = tmp16(9628).isActivitiesInTextEnabled(channel);
          ...
          const tmp16Result18 = tmp16(9628);
        }
      ...
      if (result) {
        result4 = tmp16(9433).formatPrimaryEntryPointCommandName(displayName);
        const tmp16Result19 = tmp16(9433);
      }
      ...
      if (null != tmp24) {
        ...
        obj17.targetUsername = tmp16(5022).getUserAuthor(tmp24, channel).nick;
        let id3;
        if (tmp24 != null) {
          id3 = tmp24.id;
        }
        ...
        const tmp16Result20 = tmp16(5022);
      }
