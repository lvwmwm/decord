...
// Dependencies: [17, 5017, 2113, 1376, 2008, 8436, 4942, 1404, 8437, 8438, 8439, 4640, 8441, 1095, 1119, 8443, 8444, 5278, 8445, 8447, 8448, 8442, 2]
// Exports: transformToRowGeneratedContentInventoryEntryComponent

// Module 8435 (transformContentInventoryEntryMessageComponent)
...
import _modDef8437 from "module_8437" /* 8437 */;
...
export const transformToRowGeneratedContentInventoryEntryComponent = function transformToRowGeneratedContentInventoryEntryComponent(component) {
  ...
  if (ContentInventoryEntryType.ContentInventoryEntryType.PLAYED_GAME !== content_type) {
    if (tmp(8436).ContentInventoryEntryType.TOP_GAME !== content_type) {
      if (tmp(8436).ContentInventoryEntryType.WATCHED_MEDIA === content_type) {
        items = [, ];
        ({ LARGE: arr7[0], LARGE: arr7[1] } = ImageSizes);
        const assetImage = tmp(8445).getAssetImage(contentInventoryEntry.extra.application_id, contentInventoryEntry.extra.media_assets_large_image, items);
        ...
        if (iconURL == null) {
          iconURL = Image.resolveAssetSource(_modDef8437).uri;
        }
        let tmp45 = assetImage;
        if (assetImage == null) {
          tmp45 = iconURL;
        }
        if (tmp45 === Image.resolveAssetSource(_modDef8437).uri) {
          let tmp47 = items;
        } else {
          if (tmpResult29.hasFetchedColors(tmp45)) {
            const heroColors = tmp(8439).getHeroColors(tmp45);
            ({ primaryColor: primaryColor4, secondaryColor: secondaryColor4 } = heroColors);
            const tmpResult30 = tmp(8439);
            const items1 = [tmp(4640).hexToRgba(primaryColor4), ];
            const tmpResult31 = tmp(4640);
            items1[1] = tmp(4640).hexToRgba(secondaryColor4);
            tmp47 = items1;
            const tmpResult32 = tmp(4640);
          }
          tmpResult29 = tmp(8438);
        }
        let tmp49;
        if (null != tmp47) {
          ...
          if (null != match) {
            ...
            const formatToPlainStringResult = intl4.formatToPlainString(tmp(1119).t.ijVm6y, obj2);
          ...
          const obj4 = { imageUrl: tmp45, title: contentInventoryEntry.extra.media_title, subtitles: items2, gradientColors: tmp47, platformIconUrl: obj28.resolveAssetSource(tmp46(8447)).uri };
          tmp49 = obj4;
          const obj33 = /\w+ (\d+), \w+ (\d+)/;
        }
        let tmp20 = tmp49;
      ...
      } else if (tmp(8436).ContentInventoryEntryType.TOP_ARTIST === content_type) {
        const image_url2 = contentInventoryEntry.extra.media.image_url;
        let tmp30;
        if (null != image_url2) {
          if (tmpResult33.hasFetchedColors(image_url2)) {
            const heroColors1 = tmp(8439).getHeroColors(image_url2);
            ({ primaryColor: primaryColor3, secondaryColor: secondaryColor3 } = heroColors1);
            const tmpResult34 = tmp(8439);
            const items3 = [tmp(4640).hexToRgba(primaryColor3), ];
            const tmpResult35 = tmp(4640);
            items3[1] = tmp(4640).hexToRgba(secondaryColor3);
            const tmpResult36 = tmp(4640);
            const trait = tmp(8441).getTrait(contentInventoryEntry, tmp(8442).ContentInventoryTraitType.AGGREGATE_COUNT);
            let count;
            if (trait != null) {
              count = trait.count;
            }
            if (null != count) {
              ...
              const obj5 = { count };
              const obj6 = { badgeUrl: Image.resolveAssetSource(_modDef8443).uri, text: intl.formatToPlainString(tmp(1119).t.HtifnG, obj5) };
              items4.push(obj6);
              const obj7 = { imageUrl: image_url2, title: contentInventoryEntry.extra.artist.name, subtitles: items4, gradientColors: items3, platformIconUrl: Image.resolveAssetSource(_modDef8448).uri };
              tmp30 = obj7;
              const formatToPlainStringResult1 = intl.formatToPlainString(tmp(1119).t.HtifnG, obj5);
            }
            const tmpResult37 = tmp(8441);
          }
          tmpResult33 = tmp(8438);
        }
        tmp20 = tmp30;
      } else if (tmp(8436).ContentInventoryEntryType.LISTENED_SESSION === content_type) {
        ...
        if (null != image_url) {
          if (tmpResult38.hasFetchedColors(image_url)) {
            const heroColors2 = tmp(8439).getHeroColors(image_url);
            ({ primaryColor: primaryColor2, secondaryColor: secondaryColor2 } = heroColors2);
            const tmpResult39 = tmp(8439);
            const items5 = [tmp(4640).hexToRgba(primaryColor2), ];
            const tmpResult40 = tmp(4640);
            items5[1] = tmp(4640).hexToRgba(secondaryColor2);
            ...
            obj8.platformIconUrl = Image.resolveAssetSource(_modDef8448).uri;
            tmp25 = obj8;
            const tmpResult41 = tmp(4640);
          }
          tmpResult38 = tmp(8438);
        }
        tmp20 = tmp25;
      } else if (tmp(8436).ContentInventoryEntryType.LAUNCHED_ACTIVITY === content_type) {
        ...
        if (null != application1) {
          ...
          if (uri == null) {
            uri = Image.resolveAssetSource(_modDef8437).uri;
          }
          if (uri === Image.resolveAssetSource(_modDef8437).uri) {
            let tmp7 = items;
          } else {
            if (tmpResult42.hasFetchedColors(uri)) {
              const heroColors3 = tmp(8439).getHeroColors(uri);
              ({ primaryColor, secondaryColor } = heroColors3);
              const tmpResult43 = tmp(8439);
              const items7 = [tmp(4640).hexToRgba(primaryColor), ];
              const tmpResult44 = tmp(4640);
              items7[1] = tmp(4640).hexToRgba(secondaryColor);
              tmp7 = items7;
              const tmpResult45 = tmp(4640);
            }
            tmpResult42 = tmp(8438);
          }
          if (null != tmp7) {
            const items8 = [];
            const obj10 = { badgeUrl: obj.resolveAssetSource(tmp6(5278)).uri };
            const timestamp = message.timestamp;
            const time = timestamp.getTime();
            const obj11 = {};
            const merged = Object.assign(obj10);
            obj11.text = tmp(8441).formatEntryTimestamp(contentInventoryEntry, LocaleStore.locale, time);
            const tmpResult47 = tmp(8441);
            const obj12 = { formatSet: tmp(8441).A11Y_FORMAT_SET };
    ...
    if (null != user) {
      ...
      const ensureAvatarSourceResult = tmp(1404).ensureAvatarSource(user.getAvatarSource(null, undefined, 80));
      let uri1;
  ...
  if (null != application2) {
    ...
    if (game_name == tmp54) {
      game_name = Image.resolveAssetSource(_modDef8437).uri;
    }
    if (game_name === Image.resolveAssetSource(_modDef8437).uri) {
      let tmp60 = items;
    } else {
      if (tmpResult49.hasFetchedColors(game_name)) {
        const heroColors4 = tmp(8439).getHeroColors(game_name);
        ({ primaryColor: primaryColor5, secondaryColor: secondaryColor5 } = heroColors4);
        const tmpResult50 = tmp(8439);
        const items9 = [tmp(4640).hexToRgba(primaryColor5), ];
        const tmpResult51 = tmp(4640);
        items9[1] = tmp(4640).hexToRgba(secondaryColor5);
        tmp60 = items9;
        const tmpResult52 = tmp(4640);
      }
      tmpResult49 = tmp(8438);
    }
    if (tmp54 != tmp60) {
      const items10 = [];
      if (tmpResult53.isEntryTopGame(contentInventoryEntry)) {
        const entryDuration = tmp(8441).getEntryDuration(contentInventoryEntry);
        tmp54 = tmp54 == entryDuration;
        if (!tmp54) {
          const _Math = Math;
          const rounded = Math.round(entryDuration / tmp59(1095).Seconds.HOUR);
          const intl2 = tmp(1119).intl;
          const intl3 = tmp(1119).intl;
          const obj17 = { hours: rounded };
          const _HermesInternal2 = HermesInternal;
          const obj18 = { badgeUrl: null, text: null };
          const combined = "" + intl2.string(tmp(1119).t["/50eHi"]) + " - " + intl3.formatToPlainString(tmp(1119).t.C0AxoR, obj17);
          obj18.badgeUrl = obj37.resolveAssetSource(tmp59(8443)).uri;
          obj18.text = combined;
          items10.push(obj18);
          const stringResult = intl2.string(tmp(1119).t["/50eHi"]);
        }
        const tmpResult54 = tmp(8441);
      } else {
        const obj19 = { badgeUrl: obj37.resolveAssetSource(tmp59(8444)).uri };
        const timestamp2 = message.timestamp;
        const time1 = timestamp2.getTime();
        const obj20 = {};
        const merged3 = Object.assign(obj19);
        obj20.text = tmp(8441).formatEntryTimestamp(contentInventoryEntry, LocaleStore.locale, time1);
        const tmpResult56 = tmp(8441);
        const obj21 = { formatSet: tmp(8441).A11Y_FORMAT_SET };
        obj20.ariaDescription = tmpResult56.formatEntryTimestamp(contentInventoryEntry, LocaleStore.locale, time1, obj21);
        items10.push(obj20);
        const tmpResult55 = tmp(8441);
      }
