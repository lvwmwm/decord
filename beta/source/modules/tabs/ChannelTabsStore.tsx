...
// Dependencies: [32, 2099, 4580, 2052, 11289, 1369, 504, 577, 2]

// Module 11288 (ChannelTabsStore)
import initializeDefault from "initialize" /* 504 */;
import DispatcherDefault from "Dispatcher" /* 577 */;
import utils_PlatformUtils from "utils/PlatformUtils" /* 1369 */;
import TabsExperimentDefault from "TabsExperiment" /* 11289 */;
...
function handleChannelDelete(channel) {
  ...
  if (enabled) {
    enabled = channel(1369).isDesktop();
    const obj2 = channel(1369);
  }
  if (enabled) {
    if (0 === found.filter((kind) => {
      ...
      if (enabled2) {
        enabled2 = channel(1369).isDesktop();
        const obj4 = channel(1369);
      }
      let tmp10 = !enabled2;
      if (!enabled2) {
...
const channelTabsStore = new ChannelTabsStore(DispatcherDefault, {
  ...
  CHANNEL_TABS_NAVIGATE_ROUTE: function handleNavigateRoute(routePath) {
    ...
    if (enabled) {
      enabled = obj3(1369).isDesktop();
      const obj2 = obj3(1369);
    }
    if (enabled) {
      if (0 !== tabs.length) {
        if (null != id) {
