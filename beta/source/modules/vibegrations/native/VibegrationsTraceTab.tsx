...
// Dependencies: [32, 19, 17, 9339, 21, 4790, 580, 558, 568, 17086, 17088, 1119, 3714, 17087, 5856, 4786, 1616, 504, 17089, 17090, 4757, 17091, 8511, 11532, 17083, 7329, 16952, 4738, 9027, 2]

// Module 17085 (VibegrationsTraceTab)
import c from "c" /* 568 */;
...
import VibegrationsTraceFormat from "VibegrationsTraceFormat" /* 17086 */;
import vibegrations_VibegrationsTraceFormat from "vibegrations/VibegrationsTraceFormat" /* 17087 */;
import VibegrationsTraceUtils from "VibegrationsTraceUtils" /* 17088 */;
import VibegrationsTraceDetailSheet from "VibegrationsTraceDetailSheet" /* 17091 */;
...
let closure_10 = ReactCompilerGating.isReactCompilerEnabled() ? ((entry) => {
                ...
                if (cResult[36] === cResult[7]) {
                  if (cResult[37] === tmp10) {
                    if (cResult[38] === tmp12) {
                      ...
                      if (cResult[41] === tmp4.rowTitle) {
                        ...
                        if (cResult[44] !== tmp21) {
                          ...
                          if (null != tmp21) {
                            ...
                            tmp59 = React5(tmp(4786).Text, obj3);
                          }
                          cResult[44] = tmp21;
                          cResult[45] = tmp59;
                          let tmp58 = tmp59;
                                ...
                                if (cResult[50] === tmp14) {
                                  ...
                                  if (cResult[53] === entry.kind) {
                                    ...
                                    if (cResult[56] !== entry.error) {
                                      ...
                                      if (null != entry.error) {
                                        ...
                                        tmp69 = React5(tmp(4786).Text, obj4);
                                      }
                                  ...
                                  if ("tool" === entry.kind) {
                                    ...
                                    if (null != entry.summary) {
                                      ...
                                      tmp66 = React5(tmp(4786).Text, obj8);
                                    }
                                  }
                      ...
                      const tmp57 = React5(tmp(4786).Text, obj10);
        ...
        if (cResult[28] === tmp4.rowSlot) {
          ...
          if (cResult[31] === entry) {
            ...
            if (cResult[34] !== entry.status) {
              ...
              const tmp34 = React5(tmp(17086).TraceStatusDot, obj12);
            ...
            const categoryLabelResult = tmp(17087).categoryLabel(traceCategoryResult);
            cResult[0] = str2;
            cResult[1] = entry;
            cResult[2] = str;
  ...
  if ("model" !== entry.kind) {
    ...
    if (null != entry.durationMs) {
      formatDurationResult = tmp(17087).formatDuration(entry.durationMs);
      const tmpResult5 = tmp(17087);
    }
  ...
  formatDurationResult = intl.formatToPlainString(_modDef3714["PYO+Jv"], obj13);
}) : ((entry) => {
  entry = entry.entry;
  ...
  if ("model" === entry.kind) {
    if (null != entry.promptTokens) {
      ...
      const obj3 = { tokens: tmp2(17087).formatTokens(entry.promptTokens) };
      let formatToPlainStringResult = intl.formatToPlainString(_modDef3714["PYO+Jv"], obj3);
      const tmp2Result = tmp2(17087);
    }
    ...
    const items1 = [React5(tmp2(17086).TraceStatusDot, obj8), , , ];
    const obj9 = { variant: "text-xs/semibold", style: traceCategoryTextStyles[traceCategoryResult], children: tmp2(17087).categoryLabel(traceCategoryResult) };
    items1[1] = React5(tmp2(4786).Text, obj9);
    const obj10 = { variant: "text-xs/semibold", color: "text-default", style: tmp.rowTitle, lineClamp: 1, children: tmp6 };
    items1[2] = React5(tmp2(4786).Text, obj10);
    ...
    if (null != formatToPlainStringResult) {
      ...
      tmp10Result = tmp10(tmp2(4786).Text, obj11);
    }
    ...
    if ("tool" === entry.kind) {
      ...
      if (null != entry.summary) {
        ...
        tmp10Result3 = tmp10(tmp2(4786).Text, obj12);
      }
    }
    ...
    if (null != entry.error) {
      ...
      tmp10Result4 = tmp10(tmp2(4786).Text, obj13);
    }
  ...
  if (null != entry.durationMs) {
    formatToPlainStringResult = tmp2(17087).formatDuration(entry.durationMs);
    const tmp2Result4 = tmp2(17087);
  }
});
...
let closure_11 = ReactCompilerGating.isReactCompilerEnabled() ? ((entries) => {
    ...
    if (cResult[4] === reduced) {
      ...
      if (cResult[7] === tmp4.overviewBar) {
        ...
        const mapped = TRACE_CATEGORIES.map((item) => {
          ...
          let tmp6Result = null;
          if (0 !== num) {
            const obj7 = { variant: "text-xs/normal", color: "text-subtle", children: tmp7(17087).formatDuration(num) };
            tmp6Result = tmp6(tmp7(4786).Text, obj7);
            const tmp7Result = tmp7(17087);
          }
          items1[4] = tmp6Result;
          obj.children = items1;
  ...
  let tmp = closure_9();
  closure_1 = tmp;
  dependencyMap = entries(17086).useTraceCategoryFillStyles();
  let items = [entries];
  ...
  const TRACE_CATEGORIES = entries(17088).TRACE_CATEGORIES;
...
export default ReactCompilerGating.isReactCompilerEnabled() ? ((projectId) => {
  ...
  let item = projectId(stateFromStoresArray[10]).groupTraceByTurn(stateFromStoresArray).forEach((turnId, index) => {
    ...
    if (0 !== filterTraceResult.length) {
      ...
      obj2.started = tmp(17090).formatClockTime(turnId.startedAt);
  ...
  const callback1 = onPress.useCallback((item) => {
    ...
    if ("entry" === item.kind) {
      ...
      if (null != item.started) {
        ...
        tmp2 = React5(tmp13(4786).Text, obj);
      }
      items[1] = tmp2;
      let tmp3 = null;
      if (null != item.spanMs) {
        const obj5 = { variant: "text-xs/normal", color: "text-subtle", children: tmp13(17087).formatDuration(item.spanMs) };
        tmp3 = React5(tmp13(4786).Text, obj5);
        const tmp13Result = tmp13(17087);
      }
      items[2] = tmp3;
      obj3.children = items;
