...
export default ReactCompilerGating.isReactCompilerEnabled() ? ((markAsDismissed) => {
  ...
  const enabled = noop.useContext(markAsDismissed(4481).AccessibilityPreferencesContext).reducedMotion.enabled;
  const obj = markAsDismissed(568);
  const isThemeDarkResult = markAsDismissed(4610).isThemeDark(ref(4693)());
  const tmp7 = closure_11();
  const obj2 = markAsDismissed(4610);
  const typeConsolidationTextTransform = markAsDismissed(7222).useTypeConsolidationTextTransform("DisplayNameStylesFlywheel");
  ...
  const obj3 = markAsDismissed(7222);
  const stateFromStores = markAsDismissed(504).useStateFromStores(tmp9, tmp10);
  if (cResult[2] !== stateFromStores) {
    let result = tmp5(4418).canUsePremiumProfileCustomization(stateFromStores);
  ...
  if (cResult[4] !== tmp13) {
    ...
    if (cResult[6] === tmp13) {
        ...
        class O {
          constructor() {
            ...
            openUserSettingsResult = obj.openUserSettings({ screen: tmp2 }, () => {
              markAsDismissed(7284).runAfterInteractions(() => { ... });
            });
            return;
          }
        }
      ...
      if (cResult[13] !== markAsDismissed) {
        ...
        const tmp22 = closure_9(tmp(7401).ActionSheetHeaderBar, { onPress: null });
      ...
      if (cResult[15] !== enabled) {
        let tmp24 = enabled;
        if (enabled) {
          tmp24 = closure_9(tmp(5907).DisplayNameStylesV2AbstractUI, { resizeMode: "contain" });
        }
        class O {
          constructor() {
            tmp = UserSettingsSections;
          ...
          if (cResult[21] === tmp23) {
            if (cResult[24] === tmp7.title) {
              ...
              if (cResult[28] === tmp35) {
                ...
                const tmp44 = closure_9(tmp(4754).Text, obj5);
              ...
              const tmp41 = closure_9(tmp(4754).Text, obj6);
              cResult[28] = tmp35;
              cResult[29] = str2;
              cResult[30] = tmp41;
            }
  ...
  let result = ref(4418).canUsePremiumProfileCustomization(stateFromStores);
  ...
  const callback = obj.useCallback(() => {
    openUserSettings.openUserSettings({ screen: c2 ? timestampProducer.PROFILE_CUSTOMIZATION : timestampProducer.PROFILE_CUSTOMIZATION_TRY_IT_OUT }, () => {
      markAsDismissed(7284).runAfterInteractions(() => {
        markAsDismissed(7626).openUserSettings({ screen: constants.DISPLAY_NAME_STYLES }, () => {
          closure_1_0(constants.TAKE_ACTION);
          const result = closure_2_0(4579).UNSAFE_markDismissibleContentAsDismissed(closure_2_0(2031).DismissibleContent.DISPLAY_NAME_STYLES_FLYWHEEL_MOBILE_PROFILE_COACHMARK, { dismissAction: constants.INDIRECT_ACTION });
        });
      });
    });
  }, items1);
  ...
  if (enabled) {
    tmp15Result = tmp15(tmp2(5907).DisplayNameStylesV2AbstractUI, { resizeMode: "contain" });
  }
  ...
  if (enabled) {
    ...
    items4[2] = tmp15(tmp2(4754).Text, obj10);
    ...
    items4[3] = tmp15(tmp2(4754).Text, obj11);
    ...
    const items7 = [tmp15(tmp2(5188).Button, obj14), ];
    ...
    items7[1] = tmp15(tmp2(5188).Button, obj15);
    obj13.children = items7;
    items4[4] = tmp16(tmp17, obj13);
    obj7.children = items4;
    obj12.children = tmp16(tmp17, obj7);
    obj6.children = tmp15(tmp2(7371).SafeAreaPaddingView, obj12);
    return tmp15(tmp2(7397).BottomSheet, obj6);
  } else {
    if (tmp2Result.isIOS()) {
