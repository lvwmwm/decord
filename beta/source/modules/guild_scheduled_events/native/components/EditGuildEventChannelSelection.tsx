...
// Dependencies: [19, 17, 4399, 4409, 1376, 7773, 1078, 21, 4758, 580, 6856, 9789, 9774, 4911, 504, 5241, 9776, 9777, 1119, 4754, 5341, 1879, 9792, 9760, 4725, 9536, 1984, 1181, 9773, 2]
// Exports: default

// Module 9791 (EditGuildEventChannelSelection)
import nativeDefault from "native" /* 580 */;
import util from "util" /* 1119 */;
import KeyboardManagerUtilsAll from "KeyboardManagerUtils" /* 1879 */;
import asyncRequireImpl from "asyncRequireImpl" /* 1984 */;
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4725 */;
import StageChannelUpsellDefault from "StageChannelUpsell" /* 9792 */;
...
export default function EditGuildEventChannelSelection(guild) {
  ...
  let tmp = closure_13();
  const inputStyles = guild(6856).useInputStyles({ hasLeadingIcon: true });
  closure_5 = tmp5;
  let obj = guild(6856);
  closure_6 = guild(9789).useGetEventChannelsByType(guild.id, channelType);
  let obj2 = guild(9789);
  guild(9774).useChannelsUserCanStartStageIn(guild);
  const tmp7 = channel(4911)(channel);
  let obj3 = guild(9774);
  ...
  if (null != channel) {
    let channelIcon = tmp2(5241).getChannelIcon(channel);
    const tmp2Result = tmp2(5241);
  } else {
    channelIcon = tmp6(9776);
  }
  if (null != channel) {
    let LocationIcon = tmp2(5241).getChannelIconComponent(channel);
    const tmp2Result2 = tmp2(5241);
  } else {
    LocationIcon = tmp2(9777).LocationIcon;
  }
  ...
  obj6.style = items3;
  const items4 = [closure_11(guild(4754).Heading, { style: tmp.channelTypeText, variant: "text-sm/semibold", color: "text-subtle", children: stringResult }), ];
  ...
  obj8.onPress = function onPress() {
      ...
      if (closure_8) {
        let obj2 = {
          ...
          onCreate(channel) {
                ...
                const result = guild(9760).openCreateOrEditGuildEventModal(closure_1_0, obj2);
              }
        };
    ...
    obj3.openLazy(asyncRequireImpl(9536, tmp.paths), "SelectUpdatesChannel", obj4);
  };
  ...
  const items6 = [tmp12Result, closure_11(guild(4754).Text, { style: tmp.channelNameText, variant: "text-md/medium", color: "interactive-text-active", children: tmp7 }), ];
  ...
  const tmp11 = View;
  items6[2] = closure_11(guild(1181).Icon, { source: channel(9773) });
  obj8.children = items6;
  items4[1] = closure_12(guild(5341).PressableOpacity, obj8);
