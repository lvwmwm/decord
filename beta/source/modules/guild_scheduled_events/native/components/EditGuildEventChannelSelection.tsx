...
// Dependencies: [19, 17, 4431, 4441, 1376, 7805, 1078, 21, 4790, 580, 6888, 9821, 9806, 4943, 504, 5273, 9808, 9809, 1119, 4786, 5373, 1879, 9824, 9792, 4757, 9568, 1984, 1181, 9805, 2]
// Exports: default

// Module 9823 (EditGuildEventChannelSelection)
import nativeDefault from "native" /* 580 */;
import util from "util" /* 1119 */;
import KeyboardManagerUtilsAll from "KeyboardManagerUtils" /* 1879 */;
import asyncRequireImpl from "asyncRequireImpl" /* 1984 */;
import ActionSheetActionCreatorsDefault from "ActionSheetActionCreators" /* 4757 */;
import StageChannelUpsellDefault from "StageChannelUpsell" /* 9824 */;
...
export default function EditGuildEventChannelSelection(guild) {
  ...
  let tmp = closure_13();
  const inputStyles = guild(6888).useInputStyles({ hasLeadingIcon: true });
  closure_5 = tmp5;
  let obj = guild(6888);
  closure_6 = guild(9821).useGetEventChannelsByType(guild.id, channelType);
  let obj2 = guild(9821);
  guild(9806).useChannelsUserCanStartStageIn(guild);
  const tmp7 = channel(4943)(channel);
  let obj3 = guild(9806);
  ...
  if (null != channel) {
    let channelIcon = tmp2(5273).getChannelIcon(channel);
    const tmp2Result = tmp2(5273);
  } else {
    channelIcon = tmp6(9808);
  }
  if (null != channel) {
    let LocationIcon = tmp2(5273).getChannelIconComponent(channel);
    const tmp2Result2 = tmp2(5273);
  } else {
    LocationIcon = tmp2(9809).LocationIcon;
  }
  ...
  obj6.style = items3;
  const items4 = [closure_11(guild(4786).Heading, { style: tmp.channelTypeText, variant: "text-sm/semibold", color: "text-subtle", children: stringResult }), ];
  ...
  obj8.accessibilityHint = intl2.string(guild(1119).t.AaXbMD);
  ...
  obj8.onPress = function onPress() {
      ...
      if (closure_8) {
        let obj2 = {
          ...
          onCreate(channel) {
                ...
                const result = guild(9792).openCreateOrEditGuildEventModal(closure_1_0, obj2);
              }
        };
    ...
    obj3.openLazy(asyncRequireImpl(9568, tmp.paths), "SelectUpdatesChannel", obj4);
  };
  if (null != LocationIcon) {
    ...
    tmp12Result = tmp12(tmp2(1181).Icon, obj10);
  }
  const items6 = [tmp12Result, closure_11(guild(4786).Text, { style: tmp.channelNameText, variant: "text-md/medium", color: "interactive-text-active", children: tmp7 }), ];
  ...
  const tmp11 = View;
  items6[2] = closure_11(guild(1181).Icon, { source: channel(9805) });
  obj8.children = items6;
  items4[1] = closure_12(guild(5373).PressableOpacity, obj8);
