...
// Dependencies: [19, 1482, 11469, 10347, 12, 4497, 2]
// Exports: useBottomSheetFlashListBottomViewabilityInset

// Module 12238 (useBottomSheetFlashListBottomViewabilityInset)
...
export const useBottomSheetFlashListBottomViewabilityInset = function useBottomSheetFlashListBottomViewabilityInset() {
  const context = bottomVisibilityInsetRef.useContext(bottomSheetPosition(11469).AppLauncherContext);
  bottomSheetPosition = undefined;
  if (context != null) {
    bottomSheetPosition = context.bottomSheetPosition;
  }
  const flashListRef = obj.useRef(null);
  const diff = flashListRef(1482)().height - flashListRef(10347)().maximum;
  dependencyMap = diff;
  ...
  const tmp4Result = bottomSheetPosition(4497);
  fn2.__closure = { distanceBetweenExpandedScreenTopAndSheetTop: diff, runOnJS: bottomSheetPosition(4497).runOnJS, handleBottomViewabilityInsetDebounced: memo };
