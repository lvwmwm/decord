...
// Dependencies: [19, 21, 558, 568, 1489, 5876, 5879, 1119, 8210, 2]

// Module 12307 (AppLauncherBackButton)
import c from "c" /* 568 */;
...
export default ReactCompilerGating.isReactCompilerEnabled() ? ((onPress) => {
  ...
  const tmp6 = importDefault(tmp4 ? 5876 : 5879);
  if (cResult[2] !== tmp4) {
    ...
    const tmp12 = jsx(tmp(8210).IconButton, { size: "sm", variant: "secondary-overlay", icon: tmp6, onPress, accessibilityLabel: cResult[3], maxFontSizeMultiplier: 1.5 });
    cResult[4] = onPress;
    cResult[5] = tmp6;
    cResult[6] = cResult[3];
    cResult[7] = tmp12;
  ...
  const navigation = Link.useNavigation();
  const canGoBackResult = navigation.canGoBack();
  const obj2 = { size: "sm", variant: "secondary-overlay", icon: importDefault(canGoBackResult ? 5876 : 5879), onPress: onPress.onPress, accessibilityLabel: null, maxFontSizeMultiplier: 1.5 };
  const intl = tmp(1119).intl;
  const t = tmp(1119).t;
  obj2.accessibilityLabel = intl.string(canGoBackResult ? t["13/7kX"] : t.cpT0Cq);
  return jsx(IconButton.IconButton, { size: "sm", variant: "secondary-overlay", icon: importDefault(canGoBackResult ? 5876 : 5879), onPress: onPress.onPress, accessibilityLabel: null, maxFontSizeMultiplier: 1.5 });
});
