...
// Dependencies: [19, 17, 5695, 15361, 21, 580, 4790, 558, 568, 15368, 15362, 4529, 5219, 15358, 5834, 4786, 7352, 15460, 2]

// Module 15469 (QuestDockBountyHeader)
...
const QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED = fn(15361).QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED;
const jsxProd = fn(21);
...
export default noop.memo(ReactCompilerGating.isReactCompilerEnabled() ? (() => {
  ...
  const questDockBounty = activeQuestDockMode(15368).useQuestDockBounty();
  const tmp5 = closure_9();
  let str = questDockBounty.productName;
  if (str == null) {
    str = "";
  }
  activeQuestDockMode = noop.useContext(tmp(15362).QuestDockGestureContext).activeQuestDockMode;
  const obj2 = activeQuestDockMode(15368);
  const fn = function n() {
    let num = 1;
  ...
  const tmpResult = activeQuestDockMode(4529);
  ...
  const obj3 = { withSpring: activeQuestDockMode(5219).withSpring, activeQuestDockMode, QuestDockMode, QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED };
  const bountyCreative = activeQuestDockMode(15368).useBountyCreative(questDockBounty);
  const tmpResult3 = activeQuestDockMode(15368);
  const actionSheetPressHandler = activeQuestDockMode(15358).useActionSheetPressHandler(bountyCreative);
  if (cResult[0] === questDockBounty.productIcon) {
    ...
    if (cResult[3] === animatedStyle) {
      ...
      if (cResult[6] !== str) {
        ...
        const tmp16 = closure_7(tmp(4786).Text, obj4);
        cResult[6] = str;
        cResult[7] = tmp16;
        let tmp14 = tmp16;
      } else {
  ...
  activeQuestDockMode = noop.useContext(tmp(15362).QuestDockGestureContext).activeQuestDockMode;
  let obj = activeQuestDockMode(15368);
  const fn = function n() {
    let num = 1;
    if (activeQuestDockMode.get() === QuestDockMode.EXPANDED) {
      num = 0;
  ...
  const animatedStyle = tmpResult.useAnimatedStyle(fn);
  const obj2 = { withSpring: activeQuestDockMode(5219).withSpring, activeQuestDockMode, QuestDockMode, QUEST_DOCK_MODE_CHANGE_PHYSICS_CLAMPED };
  const bountyCreative = activeQuestDockMode(15368).useBountyCreative(questDockBounty);
  const tmpResult3 = activeQuestDockMode(15368);
  const tmpResult4 = activeQuestDockMode(15358);
  const obj3 = { onSubmenuPress: activeQuestDockMode(15358).useActionSheetPressHandler(bountyCreative), hideBlurWhenCollapsed: true, children: null };
  ...
  if (tmp8Result) {
    ...
    obj5.source = obj6;
    tmp8Result = tmp8(tmp9(5834), obj5);
  }
  ...
  obj8.children = closure_7(activeQuestDockMode(4786).Text, { variant: "text-md/medium", color: "text-strong", lineClamp: 1, accessible: false, children: str });
