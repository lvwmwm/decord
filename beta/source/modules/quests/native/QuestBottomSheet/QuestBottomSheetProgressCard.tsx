...
// Dependencies: [19, 17, 1372, 21, 4756, 576, 11738, 11751, 504, 11553, 7958, 1115, 4752, 5669, 5824, 15398, 11548, 15390, 12020, 15385, 5339, 8578, 5198, 5804, 15428, 8545, 2]
...
// Module 15427 (QuestBottomSheetProgressCard)
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = closure_9();
  questTaskDetails = questTaskDetails(11738).useQuestTaskDetails(quest);
  let obj = questTaskDetails(11738);
  let isQuestProgressing = questTaskDetails(11738).useIsQuestProgressing(quest);
  ...
  noop = tmp9;
  let obj2 = questTaskDetails(11738);
  const result = questTaskDetails(11751).supportedTaskPlatforms(quest);
  c4 = result;
  const tmp2Result = questTaskDetails(11751);
  questFormattedDate = questTaskDetails(11738).useQuestFormattedDate(quest.config.rewardsConfig.rewardsExpireAt);
  gameTitle = quest.config.messages.gameTitle;
  const tmp2Result5 = questTaskDetails(11738);
  ...
  defaultRewardName = questTaskDetails(11553).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result7 = questTaskDetails(11553);
  const isSponsoredPlayQuestResult = questTaskDetails(7958).isSponsoredPlayQuest(quest);
  ...
  const tmp2Result8 = questTaskDetails(7958);
  if (!tmp7) {
    tmp7 = isQuestProgressing;
  }
  obj5.loading = !tmp7;
  const items2 = [defaultRewardName(isQuestProgressing(15398), obj5), ];
...
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  const obj = quest(11738);
  let isHeroVideoSupportedResult = null != memo;
  const obj2 = quest(15390);
  const obj3 = { questId: quest.id, sourceQuestContent: quest.sourceQuestContent };
  if (isHeroVideoSupportedResult) {
    isHeroVideoSupportedResult = tmp2(12020).isHeroVideoSupported(memo.mimetype);
    const tmp2Result = tmp2(12020);
  }
  const userStatus = quest.userStatus;
  let completedAt;
  if (userStatus != null) {
  ...
  const watchTaskPressHandler = quest(15390).useWatchTaskPressHandler({ questId: quest.id, sourceQuestContent: quest.sourceQuestContent });
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj7.source = obj8;
    const items4 = [tmp14(tmp2(8578).VideoComponent, obj7), ];
  ...
  tmp2Result2 = quest(11738);
  items7[1] = closure_7(quest(8545).PlayIcon, { color: nativeDefault.colors.WHITE });
  ...
  return closure_7(quest(5339).PressableOpacity, obj4);
};
