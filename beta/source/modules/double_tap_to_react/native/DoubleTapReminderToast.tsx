...
// Dependencies: [19, 2042, 21, 4758, 580, 558, 568, 4754, 1119, 4579, 2031, 2023, 8231, 4458, 10449, 2]
// Exports: maybeShowDoubleTapReminderToast

// Module 10448 (DoubleTapReminderToast)
import c from "c" /* 568 */;
import nativeDefault from "native" /* 580 */;
import util from "util" /* 1119 */;
import ToastActionCreatorsDefault from "ToastActionCreators" /* 4458 */;
import Text_Text from "Text/Text" /* 4754 */;
import DoubleTapEmojiUpdatedToast from "DoubleTapEmojiUpdatedToast" /* 10449 */;
...
let closure_6 = ReactCompilerGating.isReactCompilerEnabled() ? ((emoji) => {
  ...
  if (cResult[0] !== name.name) {
    ...
    const tmp13 = jsx(tmp(4754).Text, { variant: "text-sm/normal", style: tmp5, children: cResult[1] });
...
export const maybeShowDoubleTapReminderToast = function maybeShowDoubleTapReminderToast(emoji) {
  ...
  if (!obj.UNSAFE_isDismissibleContentDismissed(require("dismissible_content").DismissibleContent.DOUBLE_TAP_TO_REACT_REMINDER)) {
    ...
    if (areEmojisEqualResult) {
      areEmojisEqualResult = tmp(8231).areEmojisEqual(result, emoji);
      const tmpResult3 = tmp(8231);
    }
    if (areEmojisEqualResult) {
      const obj2 = {
      ...
      const result1 = tmp(4579).UNSAFE_markDismissibleContentAsDismissed(tmp(2031).DismissibleContent.DOUBLE_TAP_TO_REACT_REMINDER, obj3);
