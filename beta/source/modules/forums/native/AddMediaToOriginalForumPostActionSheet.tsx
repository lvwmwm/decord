...
// Dependencies: [32, 5, 19, 17, 2045, 5139, 2067, 5010, 1078, 21, 4790, 580, 8118, 5384, 5412, 4757, 9454, 9451, 11, 5379, 9453, 8044, 1275, 12165, 7735, 7879, 5143, 1119, 558, 568, 504, 7441, 8046, 5388, 12166, 4786, 5221, 7429, 2]

// Module 12164 (AddMediaToOriginalForumPostActionSheet)
      ...
      if (0 === c8) {
        ...
        if (arg0 === 1) {
          ...
          closure_132_5.on("progress", (currentSize) => {
            const maxFileSizeResult = closure_0(5384).maxFileSize(id.id);
            const obj = closure_0(5384);
            const tmp2 = id;
            const effectiveUploadLimit = closure_0(5412).getEffectiveUploadLimit(maxFileSizeResult);
            if (currentSize.currentSize > effectiveUploadLimit) {
              closure_1_5.cancel();
              dependencyMap(false);
              closure_1(4757).hideActionSheet();
          ...
          closure_132_5.on("complete", () => {
            dependencyMap(false);
            closure_1(9451).clearAll(closure_1_0, ChannelMessage.ChannelMessage);
            const obj = closure_1(9451);
            closure_1(4757).hideActionSheet();
          });
        ...
        if (arg0 === 1) {
          ...
          const mapped = closure_132_9.map((item, index) => closure_1_0(5379).getAttachmentPayload(item, index));
