...
// Dependencies: [9002, 1119, 9003, 9004, 9005, 9006, 2]
// Exports: getOpenCriticCircleRatingColor, getOpenCriticTierImage, getOpenCriticTierText

// Module 9001 (useGameProfileOpenCritic)
import OpenCriticTier from "OpenCriticTier" /* 9002 */;
...
export const getOpenCriticTierText = function getOpenCriticTierText(tier) {
  ...
  } else if (tmp(9002).OpenCriticTier.STRONG === tier) {
    const intl3 = tmp(1119).intl;
    return intl3.string(tmp(1119).t.MLxnSg);
  } else if (tmp(9002).OpenCriticTier.FAIR === tier) {
    const intl2 = tmp(1119).intl;
    return intl2.string(tmp(1119).t["3f19KA"]);
  } else if (tmp(9002).OpenCriticTier.WEAK === tier) {
...
export const getOpenCriticCircleRatingColor = function getOpenCriticCircleRatingColor(tier) {
  ...
  if (OpenCriticTier.OpenCriticTier.MIGHTY !== tier) {
    ...
    if (tmp(9002).OpenCriticTier.STRONG !== tier) {
      foregroundColor = "#4aa1ce";
      if (tmp(9002).OpenCriticTier.FAIR !== tier) {
        foregroundColor = "";
        if (tmp(9002).OpenCriticTier.WEAK === tier) {
