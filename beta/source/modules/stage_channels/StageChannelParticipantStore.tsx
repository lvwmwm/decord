...
// Dependencies: [4780, 502, 2045, 2067, 4399, 4409, 5638, 1376, 4777, 4782, 5640, 2050, 4395, 12, 5644, 4810, 1335, 504, 577, 2]
...
function updateParticipant(arg0) {
  ...
  f90697 = (dependencyMap) => dependencyMap.updateParticipant(f90697);
  if (mapped === undefined) {
...
function handleUserUpdate(user) {
  ...
  const f90697 = (dependencyMap) => dependencyMap.updateParticipant(f90697);
  let mapped1 = mapped;
  if (mapped === undefined) {
    const values2 = secondaryIndexMap.values(undefined, true);
...
function handleStreamClose(streamKey) {
  const decodeStreamKeyResult = f90697(4810).decodeStreamKey(streamKey.streamKey);
...
const secondaryIndexMap = new fn(4395).SecondaryIndexMap((getGuildId) => {
...
const stageChannelParticipantStore = new StageChannelParticipantStore(DispatcherDefault, {
  ...
  CHANNEL_UPDATES: function handleChannelUpdate(channels) {
    ...
    const f90701 = (rebuild) => rebuild.rebuild();
    let mapped = reduced;
