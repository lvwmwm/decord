...
// Dependencies: [19, 17, 21, 4790, 580, 558, 568, 1119, 4786, 5220, 4487, 2]

// Module 13309 (MediaModalYoutubeEmbedError)
...
export default noop.memo(ReactCompilerGating.isReactCompilerEnabled() ? ((videoId) => {
  ...
  if (cResult[1] !== tmp4.text) {
    ...
    const tmp9 = closure_4(tmp(4786).Text, obj2);
    cResult[1] = tmp4.text;
    cResult[2] = tmp9;
    let tmp7 = tmp9;
  } else {
  ...
  if (cResult[4] !== videoId) {
    ...
    const tmp14 = closure_4(tmp(5220).Button, obj3);
    cResult[4] = videoId;
    cResult[5] = tmp14;
    let tmp12 = tmp14;
  } else {
  ...
  const items = [closure_4(videoId(4786).Text, obj2), ];
  ...
  items[1] = closure_4(videoId(5220).Button, obj3);
  obj.children = items;
