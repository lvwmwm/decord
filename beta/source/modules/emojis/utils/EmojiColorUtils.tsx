...
// Dependencies: [4608, 4609, 676, 8030, 2]
// Exports: buildEmojiColorPalette

// Module 8220 (EmojiColorUtils)
import ColorUtils from "ColorUtils" /* 4608 */;
import utils_ColorDefault from "utils/Color" /* 4609 */;
import EmojiUtilsPlatformedDefault from "EmojiUtilsPlatformed" /* 8030 */;
...
function buildPlatformedThemedEmojiColorPalette(shouldProcessMobileColors) {
  ...
  if (null != colors) {
    ...
    if (colors.length >= 1) {
      ...
      palette = null;
      if (null != color) {
        const rawRgbToHslResult = tmp2(4608).rawRgbToHsl(color.red, color.blue, color.green);
        const tmp2Result = tmp2(4608);
        const obj3 = { foreground: null, background: null, ratio: 3, saturationFactor: null };
        const tmp2Result9 = tmp2(4608);
        const tmp5Result = tmp5(676);
        const colorLightnessAdjusted = tmp2(4608).getColorLightnessAdjusted(color, 0.6, true);
        obj3.foreground = tmp5Result(colorLightnessAdjusted.toHexString());
        obj3.background = tmp5(676)(c3);
        ...
        const tmp2Result10 = tmp2(4608);
        const tmp8 = c3;
        const obj4 = { foreground: null, background: null, ratio: 5, saturationFactor: null };
        const tmp2Result11 = tmp2(4608);
        const tmp5Result2 = tmp5(676);
        const colorLightnessAdjusted1 = tmp2(4608).getColorLightnessAdjusted(color, 0.6, false);
        obj4.foreground = tmp5Result2(colorLightnessAdjusted1.toHexString());
        obj4.background = tmp5(676)(c4);
        ...
        const tmp11 = c4;
        const tmp2Result12 = tmp2(4608);
        const obj5 = { foreground: tmp5(676)(findColorByHsvResult), background: tmp5(676)(tmp8), ratio: 7, saturationFactor };
        const accessibleForegroundColor2 = tmp2(4608).getAccessibleForegroundColor(obj5);
        const tmp2Result13 = tmp2(4608);
        const obj6 = { foreground: tmp5(676)(findColorByHsvResult), background: tmp5(676)(tmp11), ratio: 7, saturationFactor };
        const accessibleForegroundColor3 = tmp2(4608).getAccessibleForegroundColor(obj6);
        let hexResult;
        ...
        const tmp2Result14 = tmp2(4608);
        const obj8 = { colorRGB: accessibleForegroundColor, saturationFactor };
        obj7.backgroundColor = tmp2(4608).getSaturatedColorHex(obj8);
        ...
        const tmp2Result15 = tmp2(4608);
        const obj11 = { colorRGB: accessibleForegroundColor1, saturationFactor };
        obj10.backgroundColor = tmp2(4608).getSaturatedColorHex(obj11);
