...
// Dependencies: [32, 4789, 2045, 5010, 4441, 1376, 7569, 7571, 1374, 7573, 1982, 5020, 1389, 1190, 7578, 2]
...
// Module 7568 (ObscuredMediaUtils)
  ...
  if (null != currentUser) {
    if (id !== currentUser.id) {
      ...
      if (null == tmp10) {
        ...
        if (null == tmp10) {
          NONE = tmp12(7571).ContentHarmTypeBitMask.NONE;
        } else {
          ...
          NONE = contentHarmTypesToFlags(mapped.filter(tmp12(1374).isNotNullish));
        }
      }
      return NONE;
    }
...
function findComponentMedia(components) {
  ...
  return obj.flatMap((type) => {
    ...
    if (require("Server").ComponentType.MEDIA_GALLERY === type) {
      ...
      return type.media;
    } else if (tmp(1982).ComponentType.FILE === type) {
      return type.file;
    } else {
      if (tmp(1982).ComponentType.SECTION !== type) {
        if (tmp(1982).ComponentType.ACTION_ROW !== type) {
          if (tmp(1982).ComponentType.CONTAINER !== type) {
            return [];
          }
        }
      }
...
function isMediaScanPending(type, NONE) {
  if (NONE === media(7571).ContentHarmTypeBitMask.NONE) {
    return false;
    ...
    if (0 === arr.length) {
      ...
      type = type.type;
      if (tmp(7573).ObscuredMediaTypes.Embed === type) {
            ...
            if (0 !== arr.filter((item) => !isMediaFlaggedForHarmType(item, { type: ExplicitMediaRedactionModels.ObscuredMediaTypes.Embed, media })).length) {
              ...
              if (!("images" in media3)) {
                ...
                if (-1 === content_scan_version) {
                  ...
                  if (!arr.includes(tmp(7571).ContentHarmType.GORE)) {
                    if (!arr.includes(tmp(7571).ContentHarmType.SELF_HARM)) {
        ...
        return flag3;
      } else if (tmp(7573).ObscuredMediaTypes.Attachment === type) {
        ...
        return tmp11;
      } else if (tmp(7573).ObscuredMediaTypes.GenericMedia === type) {
        media = type.media;
        let flag2 = false;
        if (0 !== arr.length) {
          flag2 = false;
...
function isMediaFlaggedForHarmType(EXPLICIT, type) {
  if (null == EXPLICIT) {
    ...
    if (ExplicitMediaRedactionModels.ObscuredMediaTypes.Embed === type) {
      ...
      return tmp3(1389).hasFlag(num3, tmp5.embedFlag);
    } else if (tmp3(7573).ObscuredMediaTypes.Attachment === type) {
      let num2 = type.media.flags;
      if (num2 == null) {
        num2 = 0;
      }
      return tmp3(1389).hasFlag(num2, tmp5.attachmentFlag);
    } else if (tmp3(7573).ObscuredMediaTypes.GenericMedia === type) {
      ...
      return tmp3(1389).hasFlag(num, tmp5.genericMediaFlag);
    } else {
      return false;
    }
  }
...
function contentHarmTypesToFlags(arr6) {
  ...
  while (iter !== undefined) {
    ...
    if (HarmTypeConfiguration.ContentHarmType.EXPLICIT === nextResult) {
      NONE = NONE | tmp3(7571).ContentHarmTypeBitMask.EXPLICIT;
    } else if (tmp3(7571).ContentHarmType.GORE === nextResult) {
      NONE = NONE | tmp3(7571).ContentHarmTypeBitMask.GORE;
    } else if (tmp3(7571).ContentHarmType.SELF_HARM === nextResult) {
      NONE = NONE | tmp3(7571).ContentHarmTypeBitMask.SELF_HARM;
    }
    continue;
  }
  return NONE;
...
export const getEnabledHarmTypesBitmaskForChannelType = function getEnabledHarmTypesBitmaskForChannelType(GUILD) {
  ...
  if (null == GUILD) {
    ...
    NONE = contentHarmTypesToFlags(mapped.filter(tmp(1374).isNotNullish));
  }
  return NONE;
};
...
export const getUnscannedMediaIds = function getUnscannedMediaIds(message) {
  ...
  if (null != channelId) {
    ...
    if (NONE === NONE(7571).ContentHarmTypeBitMask.NONE) {
  ...
  NONE = NONE(7571).ContentHarmTypeBitMask.NONE;
};
