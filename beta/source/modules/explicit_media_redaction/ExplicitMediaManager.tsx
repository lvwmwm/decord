...
// Dependencies: [7835, 502, 7522, 2041, 4976, 2095, 4577, 7845, 1074, 1084, 7842, 7534, 7537, 7539, 573, 7846, 5084, 5089, 7847, 7848, 11, 4978, 4865, 7363, 2]
    ...
    if (null != message1.channel_id) {
      ...
      if (null != dependencyMap[combined]) {
        ...
        if (UPDATE === ExplicitMediaRedactionUtils.TimeoutCancelSource.UPDATE) {
          ...
          const result = tmp5(7842).trackExplicitMediaScanComplete(obj);
  ...
  const item = found1.forEach((channel_id) => {
    ...
    if (null == closure_14[combined]) {
      ...
      obj3.timeout = setTimeout(() => {
        if (maybeCancelTimeout(closure_0, found1(7842).TimeoutCancelSource.TIMEOUT)) {
          message = message.getMessage(tmp.channel_id, tmp.id);
          if (null != message) {
            const unscannedMediaIds = tmp2(7534).getUnscannedMediaIds(message);
            ...
            const result = tmp2(7842).trackScanningTimedOut(obj);
    ...
    if (null != message.id) {
      if (obj6.getChannelIdAndAuthorIdFromMessage(message).authorId !== AuthenticationStore.getId()) {
        ...
        if (!tmp20Result6.hasUnscannedMedia(message)) {
          ...
          if (!hasUnscannedMediaResult) {
            maybeCancelTimeout(message1, tmp20(7842).TimeoutCancelSource.UPDATE);
          }
        }
