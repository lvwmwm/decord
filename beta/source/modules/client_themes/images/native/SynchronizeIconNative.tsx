...
// Module 15536 (SynchronizeIconNative)
import c from "c" /* 568 */;
...
export default ReactCompilerGating.isReactCompilerEnabled() ? ((arg0) => {
  ...
  if (cResult[0] === Symbol.for("react.memo_cache_sentinel")) {
    const obj2 = { id: "Frame_-_24px", children: React3(tmp(8733).Rect, { y: "0", fill: "none", width: "24", height: "24" }) };
    const tmp6 = React3(tmp(8733).G, obj2);
    cResult[0] = tmp6;
    let first = tmp6;
  } else {
    first = cResult[0];
  ...
  if (cResult[1] !== fill) {
    ...
    const items = [React3(tmp(8733).Path, obj5), ];
    ...
    items[1] = React3(tmp(8733).Path, obj6);
    obj4.children = items;
    obj3.children = React4(tmp(8733).G, obj4);
    const tmp10 = React3(tmp(8733).G, obj3);
    cResult[1] = fill;
    cResult[2] = tmp10;
    let tmp7 = tmp10;
  } else {
