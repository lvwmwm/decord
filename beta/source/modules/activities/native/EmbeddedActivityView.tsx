...
// Dependencies: [109, 32, 19, 17, 2040, 2004, 1349, 21, 4756, 9722, 1478, 573, 9724, 9725, 9727, 9720, 504, 9729, 9576, 9730, 9738, 2]

// Module 9723 (EmbeddedActivityView)
import DispatcherDefault from "Dispatcher" /* 573 */;
import EmbeddedActivitiesNativeManagerDefault from "EmbeddedActivitiesNativeManager" /* 9576 */;
import doesOrientationMatchLockStateDefault from "doesOrientationMatchLockState" /* 9724 */;
import WakeLockDefault from "WakeLock" /* 9727 */;
...
const memoResult = noop.memo(function EmbeddedActivityViewInner(portraitSafeAreasConfig) {
  ...
  const tmp4 = currentEmbeddedActivity(9720)();
  dependencyMap = tmp4;
  ...
  if (null == currentEmbeddedActivity) {
    ...
    const discordEnvQueryParams = tmp5(9725).getDiscordEnvQueryParams();
  ...
  currentEmbeddedActivity(9729)({ connectedEmbeddedActivity: currentEmbeddedActivity });
  ...
  const obj5 = { orientationLockState: stateFromStores, showLoadingIndicator: tmp22, setShowLoadingStateForLockingOrientation: tmp9, application: tmp4, setOrientationLockState: layoutMode(9722).setOrientationLockState };
  setIsResetting = useBaseActivityView({ orientationLockState: stateFromStores, showLoadingIndicator: tmp22, setShowLoadingStateForLockingOrientation: tmp9, application: tmp4, setOrientationLockState: layoutMode(9722).setOrientationLockState }).setIsResetting;
  ...
  if (null != currentEmbeddedActivity) {
    ...
    if (null != id) {
      ...
      const tmp2Result = tmp2(9730);
      obj7.allowPopups = tmp5(9738).allowPopups(tmp4);
      obj7.isPipOrGridMode = layoutMode === constants.PIP || layoutMode === constants.GRID;
      obj7.webViewKey = tmp5(9576).EMBEDDED_ACTIVITY_WEB_VIEW_KEY;
      if (tmp26) {
