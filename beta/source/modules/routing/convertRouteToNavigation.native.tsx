...
// Dependencies: [1078, 4651, 4648, 4649, 4617, 4630, 2]
// Exports: convertRouteToNavigation

// Module 13079 (convertRouteToNavigation)
  ...
  if (null != rootNavigationRef) {
    if (rootNavigationRef.isReady()) {
      if (pathname.startsWith("/channels/")) {
        ...
        const RouteParam2 = tmp(4630).RouteParam;
        ...
        if (null != tmpResult.matchPath(pathname, obj2)) {
          ...
          const RouteParam6 = tmp(4630).RouteParam;
          const tmpResult15 = tmp(4617);
          const RouteParam7 = tmp(4630).RouteParam;
            ...
            if (tmpResult16.getChatLayout().isChatLockedOpen) {
              if (null != channelId) {
                if (false === navigationReplace) {
                  const tmpResult17 = tmp(4648);
                  const rootNavigationRef1 = tmp(4649).getRootNavigationRef();
                  let currentRoute;
                  ...
                  if (channelId1 === channelId) {
                    ...
                    tmp(4648).navigateToRootTab(obj4);
                    const tmpResult19 = tmp(4648);
                  } else {
                    const obj6 = { channelId, guildId, messageId, replaceChannelAndFixRoot: navigationReplace };
                    tmp(4648).navigateToChannel(obj6);
                    const tmpResult20 = tmp(4648);
                  }
                  const tmpResult18 = tmp(4649);
                }
              }
              const obj7 = { screen: "guilds", guildId, channelId, resetRoot: navigationReplace };
              tmp(4648).navigateToRootTab(obj7);
              const tmpResult21 = tmp(4648);
      ...
      if (pathname.startsWith("/member-verification/")) {
        ...
        const RouteParam5 = tmp(4630).RouteParam;
        obj12.path = Routes.GUILD_MEMBER_VERIFICATION(RouteParam5.guildId());
        const matchPathResult1 = tmp(4617).matchPath(pathname, obj12);
        if (null != matchPathResult1) {
          const result = tmp(4648).navigateToMemberVerification(matchPathResult1.params.guildId, matchPathResult1.params.inviteCode);
          const tmpResult27 = tmp(4648);
        }
        return true;
      } else {
        ...
        tmp(4648).resetToAuthRoute();
        flag = true;
        const tmpResult28 = tmp(4648);
      }
    }
  }
  return true;
