...
// Dependencies: [1078, 4619, 4616, 4617, 4585, 4598, 2]
// Exports: convertRouteToNavigation

// Module 13043 (convertRouteToNavigation)
  ...
  if (null != rootNavigationRef) {
    if (rootNavigationRef.isReady()) {
      if (pathname.startsWith("/channels/")) {
        ...
        const RouteParam2 = tmp(4598).RouteParam;
        ...
        if (null != tmpResult.matchPath(pathname, obj2)) {
          ...
          const RouteParam6 = tmp(4598).RouteParam;
          const tmpResult15 = tmp(4585);
          const RouteParam7 = tmp(4598).RouteParam;
            ...
            if (tmpResult16.getChatLayout().isChatLockedOpen) {
              if (null != channelId) {
                if (false === navigationReplace) {
                  const tmpResult17 = tmp(4616);
                  const rootNavigationRef1 = tmp(4617).getRootNavigationRef();
                  let currentRoute;
                  ...
                  if (channelId1 === channelId) {
                    ...
                    tmp(4616).navigateToRootTab(obj4);
                    const tmpResult19 = tmp(4616);
                  } else {
                    const obj6 = { channelId, guildId, messageId, replaceChannelAndFixRoot: navigationReplace };
                    tmp(4616).navigateToChannel(obj6);
                    const tmpResult20 = tmp(4616);
                  }
                  const tmpResult18 = tmp(4617);
                }
              }
              const obj7 = { screen: "guilds", guildId, channelId, resetRoot: navigationReplace };
              tmp(4616).navigateToRootTab(obj7);
              const tmpResult21 = tmp(4616);
      ...
      if (pathname.startsWith("/member-verification/")) {
        ...
        const RouteParam5 = tmp(4598).RouteParam;
        obj12.path = Routes.GUILD_MEMBER_VERIFICATION(RouteParam5.guildId());
        const matchPathResult1 = tmp(4585).matchPath(pathname, obj12);
        if (null != matchPathResult1) {
          const result = tmp(4616).navigateToMemberVerification(matchPathResult1.params.guildId, matchPathResult1.params.inviteCode);
          const tmpResult27 = tmp(4616);
        }
        return true;
      } else {
        ...
        tmp(4616).resetToAuthRoute();
        flag = true;
        const tmpResult28 = tmp(4616);
      }
    }
  }
  return true;
