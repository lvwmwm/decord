...
// Dependencies: [109, 9636, 2049, 2045, 4584, 2100, 2099, 1078, 9637, 3, 4598, 13035, 5630, 1105, 13040, 4961, 4965, 5739, 4616, 577, 4959, 4689, 13041, 2]
...
// Module 13034 (RouteManagerUtils)
...
import MemoryRouter from "MemoryRouter" /* 13035 */;
import DefaultRouteActionCreators from "DefaultRouteActionCreators" /* 13040 */;
...
function updateSelectedChannelListener(location) {
  const matchPathResult = channel2(13035).matchPath(location.pathname, { path: items, strict: false, exact: false });
  let params;
  if (matchPathResult != null) {
  ...
  if (null == voiceChannelId2) {
    if (null == voiceGuildId2) {
      ...
      if (isGuildVoiceResult) {
        ...
        obj21.popWithKey(tmp(4965).getVoiceChannelKey(channel.id));
...
function extractParams(arg0) {
  ...
  if (null == match) {
    ...
    if (_location.jumpType === Client.JumpType.INSTANT) {
      let ANIMATED = tmp5(4689).JumpType.INSTANT;
    } else {
      ANIMATED = tmp5(4689).JumpType.ANIMATED;
    }
