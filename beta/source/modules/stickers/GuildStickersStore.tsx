...
// Dependencies: [32, 2061, 2068, 2067, 5519, 4445, 2071, 2]

// Module 5754 (GuildStickersStore)
...
function deriveStickerMetadata(arg0, tags) {
  const items = [];
  const obj = { type: items(5519).StickerMetadataTypes.STICKER_NAME, value: null };
  const trimmed = tags.name.trim();
  obj.value = trimmed.toLocaleLowerCase();
  items.push(obj);
  if (null != tags.tags) {
    const obj2 = { type: tmp(5519).StickerMetadataTypes.TAG, value: null };
    ...
    if (null != guild) {
      ...
      if (tmp5) {
        const obj3 = { type: tmp(5519).StickerMetadataTypes.GUILD_NAME, value: toLocaleLowerCaseResult };
        items.push(obj3);
      }
    }
    const byName = UnicodeEmojisDefault.getByName(str);
    if (null != byName) {
      const obj4 = { type: tmp(5519).StickerMetadataTypes.CORRELATED_EMOJI, value: byName.surrogates };
      items.push(obj4);
