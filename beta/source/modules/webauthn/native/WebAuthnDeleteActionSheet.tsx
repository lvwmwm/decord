...
// Dependencies: [19, 17, 21, 4790, 580, 558, 568, 4757, 6868, 4490, 1119, 10986, 4749, 9740, 8903, 7476, 7428, 4786, 5220, 7429, 2]

// Module 14972 (WebAuthnDeleteActionSheet)
...
export default ReactCompilerGating.isReactCompilerEnabled() ? ((credential) => {
  ...
  const fn2 = function y() {
    ...
    const nextPromise = result.then(() => {
      ...
      obj2.icon = setDeleting(10986);
      obj2.IconComponent = credential(4749).CircleCheckIcon;
      setDeleting(4490).open(obj2);
    });
    result.then(() => {
      const obj2 = { key: "WEBAUTHN_CREDENTIAL_DELETE_SUCCESS_TOAST_KEY", content: null, icon: null, IconComponent: null, iconColor: "status-positive" };
      const intl = credential(1119).intl;
      ...
      const obj = setDeleting(4490);
      obj.open({ key: "WEBAUTHN_CREDENTIAL_DELETE_ERROR_TOAST_KEY", content: error.message, icon: setDeleting(9740), IconComponent: credential(8903).WarningIcon, iconColor: "icon-feedback-critical" });
    }).finally(() => {
      setDeleting(false);
    });
  };
  ...
  obj2.trailing = closure_4(credential(7476).ActionSheetCloseButton, { onPress: handleClose });
  const items = [closure_4(credential(7428).BottomSheetTitleHeader, obj2), , , ];
  ...
  obj4.children = closure_4(credential(4786).Text, obj5);
  ...
  obj6.children = closure_4(credential(5220).Button, obj7);
  ...
  obj9.onPress = function onPress() {
    ...
    result.then(() => {
      ...
      const obj = closure_1_1(4490);
      obj.open({ key: "WEBAUTHN_CREDENTIAL_DELETE_ERROR_TOAST_KEY", content: error.message, icon: closure_1_1(9740), IconComponent: credential(8903).WarningIcon, iconColor: "icon-feedback-critical" });
    }).finally(() => {
      closure_1_1(false);
    });
  };
  ...
  obj8.children = closure_4(credential(5220).Button, obj9);
  items[3] = closure_4(View, obj8);
  obj.children = items;
  return closure_5(credential(7429).BottomSheet, obj);
});
