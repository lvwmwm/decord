...
// Dependencies: [4706, 7805, 4442, 4441, 1376, 1078, 7913, 5012, 7914, 11, 504, 577, 2]

// Module 7912 (NotificationCenterItemsStore)
...
function toNotificationCenterItem(item_enum) {
  ...
  if (tmp3) {
    tmp3 = item_enum.type === tmp(7913).NotificationCenterItems.LIFECYCLE_ITEM;
  }
  if (tmp3) {
  ...
  if (null != item_enum.message) {
    messageRecord = tmp(5012).createMessageRecord(item_enum.message);
...
function handleAddItem(type) {
  if ("NOTIFICATION_CENTER_ITEM_CREATE" === type.type) {
    ...
    if (tmp3) {
      tmp3 = item2.type === tmp(7913).NotificationCenterItems.LIFECYCLE_ITEM;
    }
    if (tmp3) {
    ...
    if (null != item2.message) {
      messageRecord = tmp(5012).createMessageRecord(item2.message);
      const tmpResult = tmp(5012);
    }
    obj.message = messageRecord;
    let id;
...
function handleRelationshipAddOrUpdate(relationship) {
  ...
  if (tmp14) {
    ...
    obj.notifCenterLocalItems = prop1.filter((type) => {
      ...
      if (!tmp4) {
        let tmp7 = type.type === tmp(7913).NotificationCenterLocalItems.INCOMING_FRIEND_REQUESTS_ACCEPTED;
        if (tmp7) {
      ...
      if (!tmp4) {
        let tmp10 = type.type === tmp(7913).NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS;
        if (tmp10) {
      ...
      if (!tmp4) {
        let tmp13 = type.type === tmp(7913).NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS_ACCEPTED;
        if (tmp13) {
...
const isGuildEventEnded = fn(7805).isGuildEventEnded;
const RelationshipTypes = fn(1078).RelationshipTypes;
let obj = { loading: false, initialized: false, errored: false, isDataStale: false, notifCenterItems: [], staleNotifCenterItems: [], notifCenterIds: new Set(), notifCenterLocalItems: [], paginationHasMore: true, paginationCursor: "Symbol", notifCenterActive: "M16 4H15V1H16V4Z", notifCenterTabFocused: "black" };
...
const notificationCenterItemsStore = new NotificationCenterItemsStore(DispatcherDefault, {
  ...
  LOGOUT() {
    ...
    obj = { loading: false, initialized: false, errored: false, isDataStale: false, notifCenterItems: [], staleNotifCenterItems: [], notifCenterIds: new Set(), notifCenterLocalItems: null, paginationHasMore: true, paginationCursor: "Symbol", notifCenterActive: "M16 4H15V1H16V4Z", notifCenterTabFocused: "black" };
    if (flag) {
