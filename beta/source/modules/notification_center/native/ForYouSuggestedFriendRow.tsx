...
// Dependencies: [19, 17, 4750, 4409, 1078, 21, 4758, 12214, 580, 1368, 7409, 5195, 565, 8453, 1984, 4603, 16382, 1119, 4497, 16782, 5341, 16783, 1181, 4754, 16784, 16785, 1245, 2]
// Exports: default

// Module 16781 (ForYouSuggestedFriendRow)
...
export default function ForYouSuggestedFriendRow(suggestedFriend) {
  ...
  let sharedValue;
  let stateFromStores;
  const messagesTabLayout = suggestedFriend(12214).useMessagesTabLayout(panelVariant);
  const tmp4 = closure_12(messagesTabLayout);
  const analyticsLocations = useAnalyticsLocationsDefault().analyticsLocations;
  const obj = suggestedFriend(12214);
  const layoutStyles = suggestedFriend(12214).getLayoutStyles(messagesTabLayout);
  const obj2 = suggestedFriend(12214);
  const fontScale = suggestedFriend(5195).useFontScale();
  const obj3 = suggestedFriend(5195);
  ...
  if (null != suggestedFriend.friendSuggestionName) {
    ...
    const suggestedContactNameForSuggestion = tmp(16382).getSuggestedContactNameForSuggestion(friendSuggestionName, suggestedFriend);
    let str2 = "";
    ...
    if (null != suggestedFriend.mutualFriendsCount) {
      ...
      const items6 = [closure_9(tmp(4754).Text, obj12), ];
      const tmp20 = closure_11;
      const tmpResult11 = tmp(16783);
      let num3 = 0;
      ...
      obj13.children = closure_9(tmp(16784).ActionStatusSubLabel, obj15);
      ...
      items5[2] = closure_9(tmp(16785).ContactSuggestionActions, obj16);
    ...
    formatToPlainStringResult = tmp5(4603).getName(suggestedFriend.user);
