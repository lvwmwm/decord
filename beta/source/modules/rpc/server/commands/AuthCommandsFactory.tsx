...
// Dependencies: [32, 5, 5017, 2006, 1376, 4695, 1078, 9616, 1095, 510, 9609, 9165, 9348, 9633, 9366, 9362, 9368, 4436, 1090, 1275, 577, 14798, 8647, 1476, 2]
// Exports: default

// Module 14837 (AuthCommandsFactory)
...
import LeakyBucket from "LeakyBucket" /* 9616 */;
        ...
        if (null != currentUser) {
          if (user) {
            if (currentUser.id === user.id) {
              const items = [];
              items[HermesBuiltin.arraySpread(scopes, HermesBuiltin.arraySpread(tmp.authorization.scopes, 0))] = v65535;
    ...
    handler(socket) {
      ...
      if (null == access_token) {
        if (socket.transport === constants.IPC) {
          ...
          if (null == id) {
            let obj2 = { errorCode: constants2.INVALID_COMMAND };
            const tmp29 = new closure_1(9609)(obj2, "No application.");
            throw tmp29;
          } else {
            IDENTIFY = closure_0(8647).OAuth2Scopes.IDENTIFY;
                ...
                if (typeof _authorize === "function") {
                  ...
                  return authorizeWithPrompt(obj2, closure_0, closure_1).then((result) => {
                    if (null == result) {
                      ...
                      const tmp22 = new closure_1(9609)(obj, "Unknown error occurred");
                      throw tmp22;
                    } else {
                      const parts = result.split(/#|\?/);
                      const parsed = closure_1(1476).parse(parts[parts.length - 1]);
                      if (null != parsed.error) {
                        ...
                        const _HermesInternal = HermesInternal;
                        const tmp82 = new closure_1(9609)(obj2, "OAuth2 Error: " + parsed.error + ": " + str);
                        throw tmp82;
                      } else {
      ...
      if (null == access_token) {
        ...
        const tmp11 = new closure_1(9609)(obj, "No access token provided");
        throw tmp11;
      } else {
          ...
          if ("token" === args.response_type) {
            ...
            return authorizeWithPrompt(obj6, closure_0, hasItem).then((location) => {
              ...
              if (null == location) {
                ...
                const tmp25 = new hasItem(9609)(obj2, "Unknown error occurred");
                throw tmp25;
              } else {
                const _URL = URL;
                ...
                if (hasItem) {
                  ...
                  if (null != value2) {
                    if ("" !== value2) {
                      ...
                      const _HermesInternal = HermesInternal;
                      const tmp122 = new hasItem(9609)(obj4, "OAuth2 Error: " + value2 + ": " + str5);
                      throw tmp122;
                    }
                  }
                  if (null == value) {
                    const obj5 = { errorCode: constants2.OAUTH2_ERROR };
                    const tmp8 = new hasItem(9609)(obj5, "OAuth2 Error: Unable to find auth code");
                    throw tmp8;
                  } else {
