...
// Function ID: 11280
// Name: formatResults
// Dependencies: [5, 10069, 2045, 4399, 1376, 11280, 1078, 10070, 4771, 10079, 1119, 1374, 12, 2]
  ...
  if (hasQuery.hasQuery) {
    ...
    if (null != pinnedDestinations) {
      if (pinnedDestinations.length > 0) {
        let mapped = pinnedDestinations.map((type) => {
          if ("user" === type.type) {
            ...
            let tmp6 = null;
            if (null != user) {
              const obj = { type: queryMode(10070).AutocompleterResultTypes.USER, record: user, score: 0 };
              tmp6 = obj;
            }
            let tmp3 = tmp6;
          } else {
            tmp3 = originDestination(10079)(type.id);
          }
          return tmp3;
        });
      }
      ...
      if (channelHistory.length > 0) {
        let mapped1 = channelHistory.map((item) => originDestination(10079)(item));
      } else {
        mapped1 = [];
      }
      if (frequentChannels.length > 0) {
        let mapped2 = frequentChannels.map((id) => originDestination(10079)(id.id));
      } else {
        mapped2 = [];
      }
      const items = [];
      ...
      if (null == targetDestination) {
        ...
        if (null != user) {
          let obj = { type: queryMode(10070).AutocompleterResultTypes.USER, record: user, score: 0 };
          targetDestination = obj;
        }
      } else {
        const tmp9 = originDestination(10079)(targetDestination.id);
      }
    }
    mapped = [];
  }
...
export const formatResultsWithHeaders = function formatResultsWithHeaders(hasNonEmptyQuery) {
  ...
  const intl = queryMode(1119).intl;
  const headerResult = queryMode(10070).createHeaderResult(intl.string(queryMode(1119).t.qm9dSj));
  if (hasNonEmptyQuery.hasNonEmptyQuery) {
    ...
    if (null != queryMode) {
      ...
      items4 = [tmp(10070).createHeaderResult(intl2.string(tmp(1119).t["80lOZ1"])), , ];
      let tmp13 = null;
      if (null != selectedChannelId) {
        ...
        if (null != tmp7) {
          ...
          if (isAllowedType(tmp7)) {
            let canResult = tmp7.type === tmp(10070).AutocompleterResultTypes.USER;
            if (!canResult) {
