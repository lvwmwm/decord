...
// Dependencies: [19, 1078, 21, 4790, 558, 568, 4757, 9865, 7658, 18160, 1119, 4786, 18164, 7429, 2]

// Module 18159 (EligibilityActionSheet)
...
import EligibilityChecklistDefault from "EligibilityChecklist" /* 18164 */;
...
export default ReactCompilerGating.isReactCompilerEnabled() ? ((onRequireModeratorMFAClick) => {
  ...
  if (cResult[0] === Symbol.for("react.memo_cache_sentinel")) {
    const fn = function o() {
      ...
      onRequireModeratorMFAClick(7658).openUserSettings({ screen: constants.ACCOUNT });
    };
    cResult[0] = fn;
    let first = fn;
  } else {
  ...
  if (cResult[4] !== tmp4.title) {
    ...
    const tmp13 = closure_5(tmp(4786).Heading, obj4);
  ...
  if (cResult[6] === tmp8) {
    ...
    obj5.children = items;
    const tmp18 = closure_6(tmp(7429).BottomSheet, obj5);
    cResult[9] = tmp11;
    cResult[10] = tmp14;
    cResult[11] = tmp18;
    tmp16 = tmp18;
  ...
  const memo = noop.useMemo(() => ({
    actions: {
      onEnableMFAClick() {
        closure_1_1(4757).hideActionSheet(closure_1_7);
        const obj = closure_1_1(4757);
        closure_1_1(9865).close();
        const obj2 = closure_1_1(9865);
        onRequireModeratorMFAClick(7658).openUserSettings({ screen: constants.ACCOUNT });
      },
      onRequireModeratorMFAClick() {
        ActionSheetActionCreatorsDefault.hideActionSheet(EligibilityActionSheet);
  ...
  const items1 = [closure_5(onRequireModeratorMFAClick(4786).Heading, obj2), ];
  ...
  obj.children = items1;
  return closure_6(onRequireModeratorMFAClick(7429).BottomSheet, obj);
});
