...
// Dependencies: [32, 5139, 8059, 1078, 5245, 1612, 12158, 12157, 8057, 7800, 9558, 7802, 9554, 1982, 9451, 4659, 1614, 12331, 12, 12194, 12160, 4777, 12333, 12159, 4758, 4970, 10675, 2]

// Module 12591 (ApplicationCommandManager)
...
import ApplicationCommandActionCreators from "ApplicationCommandActionCreators" /* 8057 */;
...
import ChatInputParser from "ChatInputParser" /* 12158 */;
import ApplicationCommandOptionValueParser from "ApplicationCommandOptionValueParser" /* 12159 */;
import application_commands_ApplicationCommandValidationUtils from "application_commands/ApplicationCommandValidationUtils" /* 12333 */;
  ...
  constructor(arg0) {
    ...
    obj.setPastedCommand = function setPastedCommand(arg0, channel) {
      ...
      if (null != command) {
        let tmp8 = null;
        if (null != application) {
          const obj5 = { type: tmp2(7802).ApplicationCommandSectionType.APPLICATION, id: null, icon: null, name: null, application: null };
        ...
        const initialValuesFromInteractionOptions = tmp2(9554).getInitialValuesFromInteractionOptions(command, interactionOptions);
        ...
        const obj6 = { channelId: channel.id, command, section: tmp8, location: tmp2(7802).ApplicationCommandTriggerLocations.PASTE, commandText: null };
        ...
        obj.setCommand(obj6);
        const tmp2Result = tmp2(9554);
      } else {
        ({ id, name } = parsed);
        obj.setPartialCommand(id, name, tmp2(7802).ApplicationCommandTriggerLocations.PASTE);
      }
    };
    ...
    obj.updateStyles = function updateStyles(styles) {
      ...
      obj.chatInputNodes = chatInputNodes.map((style) => {
        if (null == style.style) {
          ...
          if (obj(12158).ChatInputNodeType.COMMAND_OPTION !== type) {
            if (tmp30(12158).ChatInputNodeType.COMMAND_OPTION_WITH_VALUE !== type) {
              if (tmp30(12158).ChatInputNodeType.GAME_HIGHLIGHT === type) {
                const obj2 = {};
                const merged = Object.assign(style);
                const styles4 = closure_1_0.styles;
                obj2.style = styles4.gameMention();
                return obj2;
              } else if (tmp30(12158).ChatInputNodeType.GAME_MENTION_INPUT === type) {
                const obj3 = {};
                const merged1 = Object.assign(style);
                const styles3 = closure_1_0.styles;
                obj3.style = styles3.commandOption();
                return obj3;
              } else if (tmp30(12158).ChatInputNodeType.ROLE_HIGHLIGHT === type) {
                ...
                if (tmp30(12158).ChatInputNodeType.EMOJI_HIGHLIGHT !== type) {
                  if (tmp30(12158).ChatInputNodeType.USER_HIGHLIGHT !== type) {
                    if (tmp30(12158).ChatInputNodeType.CHANNEL_HIGHLIGHT !== type) {
                      if (tmp30(12158).ChatInputNodeType.SILENT_HIGHLIGHT !== type) {
                        return style;
                      }
                    }
                  }
    ...
    obj.addCommandOptionParserRules = function addCommandOptionParserRules() {
      ...
      obj = {
        ...
        matchFunction(c22, activeCommand) {
          return obj(12157).getMatchedOptions(c22, activeCommand);
        },
        style() {
      ...
      parser2.addRule({
        ...
        matchFunction(arg0, arg1) {
          return obj(12157).getMatchedOptionsWithValue(arg0, arg1);
        },
        style() {
          const styles = obj.styles;
          return styles.commandOption();
        ...
        editDisabled(data) {
          ...
          return type === obj(1982).ApplicationCommandOptionType.ATTACHMENT;
        }
      });
      ...
      parser7.addRule({
        ...
        matchFunction(arg0) {
          return obj(12157).getSilentHighlightNodes(arg0);
        },
        style() {
          const styles = obj.styles;
          return styles.autocomplete();
      ...
      parser9.addRule({
        ...
        matchFunction(arr) {
          return obj(12157).getGameMentionInputNodes(arr);
        },
        style() {
          const styles = obj.styles;
          return styles.commandOption();
      ...
      parser11.addRule({
        ...
        matchFunction(arr) {
          return obj(12157).getTimestampMentionInputNodes(arr);
        },
        style() {
          const styles = obj.styles;
          return styles.commandOption();
        ...
        if (!obj.props.commandsDisabled) {
          if (text.startsWith(closure_2_11)) {
            ...
            if (null == tmp.contextCommands) {
              ...
              const commandContext = tmp3(12194).getCommandContext(obj3);
              ...
              if (preferredCommandType === constants.PARTIAL_COMMAND) {
                ...
                if (null != found) {
                  ...
                  let cachedApplicationSection = ApplicationCommandQueryApiAll.getCachedApplicationSection(obj4, tmp3(1982).ApplicationCommandType.CHAT, found.applicationId);
                  let tmp20 = null;
                ...
                const draftCommand = tmp3(12160).resolveDraftCommand(channel, text, DraftStore.getDraftCommand(channel.id, DraftType.ChannelMessage));
                  ...
                  if (0 < parts.length) {
                    function _loop() {
                      ...
                      if (found.length > 0) {
                        ...
                        const cachedApplicationSection = ApplicationCommandQueryApiAll.getCachedApplicationSection(obj, tmp3(1982).ApplicationCommandType.CHAT, first.applicationId);
                        if (null == cachedApplicationSection) {
                          let obj5 = { v: null };
                        } else {
                          obj5 = { v: null };
...
prototype["mergePropsAndUpdate"] = function mergePropsAndUpdate(editId) {
  ...
  if (text === editId.text) {
    ...
    if (tmp84) {
      ...
      if (!focused2) {
        const keyboardType = obj10(4659).getKeyboardType();
        focused2 = keyboardType !== obj10(1614).KeyboardTypes.SYSTEM;
        const obj6 = obj10(4659);
      }
      ...
      self.chatInputNodes = chatInputNodes.map((type) => {
        if (type.type === ChatInputParser.ChatInputNodeType.COMMAND_OPTION) {
          if (null != type.data) {
            ...
            if (type.type === tmp(12158).ChatInputNodeType.COMMAND_OPTION_WITH_VALUE) {
              let name;
            ...
            if (option.name !== name1) {
              if (undefined !== tmp15) {
                ...
                if (success) {
                  success = option.type === tmp(1982).ApplicationCommandOptionType.ATTACHMENT;
                }
                if (success) {
    ...
    if (name1 !== name3) {
      ...
      if (type === tmp113(1982).ApplicationCommandOptionType.ATTACHMENT) {
        if (!self.optionValidationResults[activeOption.name].success) {
          const current2 = self.ref.current;
          let obj2 = { type: tmp113(1614).KeyboardTypes.MEDIA, context: null };
    ...
    if (tmp140) {
      tmp140 = activeOption.type !== tmp113(1982).ApplicationCommandOptionType.ATTACHMENT;
    }
    if (tmp140) {
      const keyboardType1 = tmp113(4659).getKeyboardType();
      tmp140 = keyboardType1 !== tmp113(1614).KeyboardTypes.SYSTEM;
      const tmp113Result2 = tmp113(4659);
    }
    if (tmp140) {
  ...
  if (editId.commandsDisabled) {
    ...
    const item1 = chatInputNodes1.forEach((type) => {
      if (type.type === ChatInputParser.ChatInputNodeType.COMMAND_OPTION) {
        ...
        if (type === tmp(12158).ChatInputParseResultDataType.COMMAND_OPTION) {
          const optionsToNodes = self.optionsToNodes;
          const result = optionsToNodes.set(type.data.option.name, type);
        }
      }
      let tmp5 = type.type === tmp(12158).ChatInputNodeType.COMMAND_OPTION_WITH_VALUE;
      if (tmp5) {
        ...
        tmp5 = type1 === tmp(12158).ChatInputParseResultDataType.COMMAND_OPTION;
      }
      if (tmp5) {
    ...
    if (tmp5) {
      ...
      if (preferredCommandType11 === constants3.FULL_COMMAND) {
        ...
        while (iter !== undefined) {
          ...
          if (!tmp60) {
            delete tmp[tmp2];
            if (tmp56.type === obj10(1982).ApplicationCommandOptionType.ATTACHMENT) {
