...
// Dependencies: [32, 19, 17, 2113, 13361, 21, 4790, 580, 1181, 558, 568, 4529, 565, 5219, 9124, 1885, 4786, 16430, 2]

// Module 16429 (HappeningNowAvatarStack)
...
export const HappeningNowAvatarStack = ReactCompilerGating.isReactCompilerEnabled() ? ((arg0) => {
  ...
  const sharedValue = guildId(4529).useSharedValue(num6);
  ...
  const tmpResult4 = guildId(4529);
  J.__closure = { interpolate: guildId(4529).interpolate, typingValue: sharedValue, ELLIPSIS_WIDTH: 28 };
  ...
  if (cResult[4] === (undefined !== isTyping && isTyping)) {
    ...
    const mapped = substr.map((user, index) => {
      ...
      if (index !== diff) {
        ...
        const point = { shape: tmp3(9124).CutoutShape.Circle, x: tmp3(1181).AVATAR_SIZE_MAP[tmp5] - num3 - num2, y: -num2, size: tmp3(1181).AVATAR_SIZE_MAP[tmp5] + 2 * num2 };
        const items = [point];
        obj3.cutouts = items;
        obj3.children = tmp6;
