...
// Dependencies: [5, 32, 2102, 2045, 2109, 1376, 7347, 1078, 4386, 4940, 1275, 577, 1235, 5771, 12, 7353, 1374, 7356, 1245, 4938, 7357, 11, 1389, 7360, 7367, 2]

// Module 7352 (GuildOnboardingActionCreators)
...
obj.updateRolesLocal = function updateRolesLocal(guildId, addedRoleIds, removedRoleIds) {
  ...
  if (ImpersonateStore.isViewingRoles(guildId)) {
    ...
    const obj6 = apply;
    const result = obj5.updateImpersonatedRoles(guildId, obj6.difference(apply.union(roles, addedRoleIds), removedRoleIds));
  } else {
    if (tmp2) {
...
obj.completeOnboarding = function completeOnboarding(guildId, prompts) {
  ...
  const selectedRoleIds = items1(7353).getSelectedRoleIds(selectedOptions);
  const obj2 = items1(7353);
  const selectedChannelIds = items1(7353).getSelectedChannelIds(selectedOptions);
  if (GuildOnboardingPromptsStore.getEnabled(guildId)) {
    let defaultChannelIds = obj.getDefaultChannelIds(guildId);
  } else {
    defaultChannelIds = [];
  }
  const obj3 = items1(7353);
  const tmp2Result = items1(7353);
  [arr3, arr4] = items1(7353).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds);
  const items = [...defaultChannelIds];
  const mapped = items.map((item) => channel.getChannel(item));
  const found = mapped.filter(tmp2(1374).isNotNullish);
  const tmp6 = _slicedToArray(items1(7353).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds), 2);
  const tmp2Result11 = items1(7356);
  if (null == tmp) {
    items1 = [];
  } else {
    options = tmp.options;
  ...
  const providerConnectionState = items1(7353).getProviderConnectionState(connections);
  const tmp2Result12 = items1(7353);
  const applicationConnectionState = items1(7353).getApplicationConnectionState(connections);
  const tmp2Result13 = items1(7353);
  const obj4 = {};
  const obj8 = AnalyticsUtilsDefault;
  const merged = Object.assign(items1(4938).collectGuildAnalyticsMetadata(guildId));
  ...
  const tmp2Result14 = items1(4938);
  const tmp2Result15 = items1(7357);
  ...
  if (ImpersonateStore.isFullServerPreview(guildId)) {
    const result = tmp2(5771).updateImpersonatedChannels(guildId, items, []);
    const tmp2Result16 = tmp2(5771);
    const result1 = tmp2(5771).updateImpersonatedData(guildId, { optInEnabled: true });
    const tmp2Result17 = tmp2(5771);
    const _Array = Array;
    const result2 = tmp2(5771).updateImpersonatedRoles(guildId, Array.from(selectedRoleIds));
    ...
    if (null != currentUser) {
      ...
      const tmp2Result19 = tmp2(5771);
      obj6.flags = tmp2(1389).setFlag(num3, GuildMemberFlags.COMPLETED_ONBOARDING, true);
      obj5.memberOptions = obj6;
      const result3 = tmp2Result19.updateImpersonatedData(guildId, obj5);
      const tmp2Result20 = tmp2(1389);
    }
    const tmp2Result18 = tmp2(5771);
  }
};
        ...
        if (0 === dependencyMap) {
          if (arg0 === 1) {
            ...
            if (null != currentUser) {
              ...
              obj6.flags = v0(1389).setFlag(v0, constants.COMPLETED_ONBOARDING, false);
              dependencyMap = 1;
              c1 = 1;
