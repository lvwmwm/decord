...
// Dependencies: [19, 17, 2045, 2100, 2067, 2099, 12213, 1078, 21, 4758, 580, 12446, 1984, 5112, 558, 568, 1488, 12447, 12448, 12486, 7205, 5827, 8178, 10285, 1119, 9862, 5188, 7299, 12518, 4497, 10058, 504, 10055, 12519, 12520, 12522, 2]

// Module 12445 (GuildSearchAndInvite)
...
import useEventsButtonPropsDefault from "useEventsButtonProps" /* 12522 */;
...
const SEARCH_BAR_MARGIN_BOTTOM = fn(12213).SEARCH_BAR_MARGIN_BOTTOM;
...
let closure_16 = ReactCompilerGating.isReactCompilerEnabled() ? ((guildId) => {
  ...
  const memo = onInvitePress.useMemo(() => {
    let tmp = null;
    if (canInvite) {
      const obj = { variant: "secondary", size: "sm", icon: _modDef10285, onPress: onInvitePress, onPressDisabled: handleInviteDisabledPress, accessibilityLabel: null, disabled: null, maxFontSizeMultiplier: 2 };
...
export default noop.memo(ReactCompilerGating.isReactCompilerEnabled() ? ((guild) => {
  ...
  const shouldShowInvitesDisabledNotif = guild(12519).useShouldShowInvitesDisabledNotif(guild);
  useCanSeeEventsInChannelListDefault(guild.id);
  const tmpResult2 = guild(12519);
