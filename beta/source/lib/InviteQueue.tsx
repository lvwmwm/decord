...
// Function ID: 10131
// Name: InviteQueue
// Dependencies: [2045, 1095, 8082, 3, 7703, 4771, 2]

// Module 10130 (InviteQueue)
import LoggerDefault from "Logger" /* 3 */;
import DurationsDefault from "Durations" /* 1095 */;
import MessageActionCreatorsDefault from "MessageActionCreators" /* 7703 */;
import ChannelStore from "ChannelStore" /* 2045 */;
import Queue from "Queue" /* 8082 */;
...
function drain(location, sum) {
  ...
  if (self.GROUP_DM !== type) {
    if (tmp.CHANNEL !== type) {
      if (tmp.USER === type) {
        const obj = inviteAnalyticsMetadata(4771);
        inviteAnalyticsMetadata(4771).ensurePrivateChannel(location.user.id).then((result) => {
        ...
        const ensurePrivateChannelResult = inviteAnalyticsMetadata(4771).ensurePrivateChannel(location.user.id);
      }
    }
  }
