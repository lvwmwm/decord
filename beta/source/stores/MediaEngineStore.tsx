...
// Dependencies: [32, 5, 1997, 2002, 4674, 2003, 4803, 1224, 502, 14268, 9890, 2045, 4781, 1376, 14269, 1078, 4800, 4967, 14270, 1088, 14271, 4783, 3, 1095, 1368, 4813, 13571, 2040, 5357, 14272, 14273, 14274, 10194, 14275, 12, 14276, 14087, 14277, 14278, 14279, 14280, 14281, 14282, 9681, 1245, 577, 10249, 14283, 14284, 14285, 5639, 14286, 14331, 14332, 14333, 14334, 14335, 14336, 4893, 4752, 4381, 510, 14337, 10236, 5784, 14341, 14342, 1119, 5080, 14343, 14344, 14345, 38, 14099, 14098, 14346, 14081, 1235, 13942, 9696, 504, 14347, 14348, 14349, 14350, 2]
...
import getEffectiveNoiseCancellationDefault from "getEffectiveNoiseCancellation" /* 10249 */;
import VideoGuardExperiment2 from "VideoGuardExperiment" /* 13571 */;
import KrispUtilsDefault from "KrispUtils" /* 14081 */;
...
import GoLiveHdrExperiment from "GoLiveHdrExperiment" /* 14272 */;
import StreamZeroVadLeadingExperiment2 from "StreamZeroVadLeadingExperiment" /* 14273 */;
import AGC2MobileExperimentDefault from "AGC2MobileExperiment" /* 14274 */;
import PTTEducationExperimentDefault from "PTTEducationExperiment" /* 14275 */;
import MuteAwareNoiseCancellationExperiment from "MuteAwareNoiseCancellationExperiment" /* 14276 */;
import HookAll from "Hook" /* 14277 */;
import AudioFidelityExperiment from "AudioFidelityExperiment" /* 14283 */;
import _modDef14286 from "module_14286" /* 14286 */;
import AudioEffectsExperimentDefault from "AudioEffectsExperiment" /* 14341 */;
import SilenceWarningExperimentDefault from "SilenceWarningExperiment" /* 14346 */;
import InputWatcherDefault from "InputWatcher" /* 14347 */;
import DisableCameraSimulcastExperiment2 from "DisableCameraSimulcastExperiment" /* 14350 */;
        ...
        if (null != tmp2) {
          ...
          if (null != tmp2.desktopSource) {
            ...
            if (isWindowsResult) {
              ...
              isWindowsResult = _modDef14286.satisfies(release, closure_1_29);
            }
            if (!isWindowsResult) {
              ...
              if (!isWindowsResult1) {
                ...
                if (videoHook2) {
                  const VideoHookDX12Experiment = tmp69(14280).VideoHookDX12Experiment;
                  enabled2 = VideoHookDX12Experiment.getConfig({ location: "updateVideo" }).enabled;
                }
                const UpscaleSmallCapturedFramesExperiment = tmp69(14281).UpscaleSmallCapturedFramesExperiment;
                ...
                if (isMacResult) {
                  ...
                  isMacResult = _modDef14286.satisfies(release1, closure_1_24);
                }
                ...
                obj7.enableGlobalFramePoolLock = tmp69(14282).getGlobalFramePoolLockExperimentConfig({ location: "updateVideo" }).enabled;
                ...
                const tmp69Result7 = tmp69(14282);
              } else if (true === c134) {
                const WGCDirtyRegionsExperiment = tmp69(14278).WGCDirtyRegionsExperiment;
                enabled = WGCDirtyRegionsExperiment.getConfig({ location: "updateVideo" }).enabled;
              } else {
                const WGCDirtyRegionsAllExperiment = tmp69(14279).WGCDirtyRegionsAllExperiment;
              ...
              if (isWindowsResult2) {
                ...
                isWindowsResult2 = _modDef14286.satisfies(release2, closure_1_27);
              }
...
function handleVideoFilterError(code, arg1) {
  ...
  if (arg1 !== constants10.LIVE) {
    ...
    if (constants9.UNSUPPORTED === code) {
      AVUnderlyingError = tmp4(9681).AVUnderlyingError;
      let VideoBackgroundInitFailed = AVUnderlyingError.VideoBackgroundUnsupported;
    } else if (tmp5.INIT_FAILED === code) {
      VideoBackgroundInitFailed = tmp4(9681).AVUnderlyingError.VideoBackgroundInitFailed;
    }
...
function updateConnectionVoiceProcessing(setEchoCancellation) {
  ...
  const tmp10ResultResult = tmp10(10249)(noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
  ...
  const tmp10Result6 = tmp10(14274);
  ...
  const tmp10Result = tmp10(10249);
  let supportsResult = true === tmp2.audioMixerSettings.enabled && tmp10(14098).getConfig({ location: "MediaEngineStore" }).enabled;
  if (supportsResult) {
  ...
  const tmp10Result7 = tmp10(14098);
  ...
  const tmp10Result8 = tmp10(10249);
  const tmp10Result3Result = tmp10(10249)(tmpResult.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
  ...
  let isWindowsResult = PlatformUtils.isWindows();
  if (isWindowsResult) {
    const tmp10Result10 = tmp10(4381);
    let release;
    if (tmp10Result10 != null) {
      release = tmp10Result10.os.release;
    }
    isWindowsResult = tmp10(14286).satisfies(release, c156);
    const tmp10Result9 = tmp10(14286);
  }
  if (isWindowsResult) {
    if (setEchoCancellation.context === MediaEngineContextTypes.DEFAULT) {
      const systemwideEchoCancellationExperimentConfig = tmp30(14284).getSystemwideEchoCancellationExperimentConfig({ location: "updateConnectionVoiceProcessing" });
...
function setLoopback() {
  ...
  const tmp10Result = tmp10(10249);
  const tmp10ResultResult = tmp10(10249)(tmpResult.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
...
const NativePermissionConstants = fn(4967);
({ NativePermissionStates: closure_32, NativePermissionTypes } = NativePermissionConstants);
let closure_34 = fn(14270).DEFAULT_AUDIO_MIXER_SETTINGS;
...
const InputProfile = fn(14271).InputProfile;
Constants = fn(4783);
...
fn(4813);
const BaseConnectionEvent = fn(4813);
...
const outputDevices = { [DEFAULT_DEVICE_ID]: { id: DEFAULT_DEVICE_ID, deviceType: DeviceTypes.AUDIO_OUTPUT, index: 0, name: "No Output Devices", disabled: true, guid: "code", hardwareId: "transform", containerId: "WireType" } };
let dependencyMap = { [DEFAULT_DEVICE_ID]: { id: DEFAULT_DEVICE_ID, deviceType: DeviceTypes.VIDEO_INPUT, index: 0, name: "No Video Devices", disabled: true, guid: "unicodeVersion", hardwareId: "width", containerId: "toCharArray$esjava$1" } };
...
prototype["initialize"] = function initialize() {
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.Connection, (setAttenuation) => {
    ...
    tmp9Result19 = pollMetrics(14285);
    ...
    if (tmp9Result20.isWindows()) {
      ...
      if (tmp9Result21.isMac()) {
        ...
        if (satisfiesResult) {
          ...
          satisfiesResult = tmp25(14286).satisfies(release, closure_39);
          const tmp25Result = tmp25(14286);
        }
    ...
    if (tmp9Result26.isWeb()) {
      setAttenuation.setExperimentFlag(tmp22.BROWSER_HEVC, tmp25(14332).getConfig({ location: "MediaEngineStore" }).enabled);
      const tmp25Result2 = tmp25(14332);
    }
    ...
    if (enabled) {
      enabled = tmp9(14333).getWmfGpuEncode("MediaEngineStore").enabled;
      const tmp9Result28 = tmp9(14333);
    }
    if (enabled) {
    ...
    if (enabled2) {
      enabled2 = tmp9(14334).getWmfGpuEncodeIntel("MediaEngineStore").enabled;
      const tmp9Result30 = tmp9(14334);
    }
    if (enabled2) {
      setAttenuation.setExperimentFlag(tmp22.WMF_GPU_ENCODE, true);
    ...
    if (enabled3) {
      enabled3 = tmp9(14335).getWmfCpuEncodeIntel("MediaEngineStore").enabled;
      const tmp9Result32 = tmp9(14335);
    }
    if (enabled3) {
      setAttenuation.setExperimentFlag(tmp22.INTEL_GPU_DISABLE, true);
    }
    ...
    if (enabled4) {
      enabled4 = tmp9(14333).getWmfGpuEncode("MediaEngineStore").enabled;
      const tmp9Result34 = tmp9(14333);
    }
    if (enabled4) {
      setAttenuation.setExperimentFlag(tmp22.WMF_GPU_ENCODE, true);
    }
    const result2 = closure_73.setHasFullbandPerformance(tmp25(14336)());
    ...
    setAttenuation.on(pollMetrics(4813).BaseConnectionEvent.Speaking, (userId, speakingFlags, arg2, voiceDb) => {
    ...
    if (setAttenuation.context === constants4.DEFAULT) {
      c99 = false;
      setAttenuation.on(tmp84(4813).BaseConnectionEvent.SpeakingWhileMuted, () => {
        c99 = true;
        if (!c99) {
          closure_1_70.emitChange();
        }
    ...
    setAttenuation.on(pollMetrics(4813).BaseConnectionEvent.DesktopSourceEnd, (endReason, errorCode) => {
    ...
    setAttenuation.on(pollMetrics(4813).BaseConnectionEvent.InteractionRequired, (required) => {
      closure_1_1(577).dispatch({ type: "MEDIA_ENGINE_INTERACTION_REQUIRED", required });
    });
    setAttenuation.on(pollMetrics(4813).BaseConnectionEvent.VideoHookInitialize, (backend, format, framebuffer_format, sample_count, success, reinitialization) => {
      ...
      if (null != desktopSource) {
        ...
        const merged = Object.assign(closure_1_1(4893)(desktopSource1));
        obj.track(constants.VIDEOHOOK_INITIALIZED, obj2);
        const tmp15 = closure_1_1(4893);
      }
    });
    setAttenuation.on(pollMetrics(4813).BaseConnectionEvent.NoiseCancellationError, closure_153);
    setAttenuation.on(pollMetrics(4813).BaseConnectionEvent.VoiceActivityDetectorError, closure_153);
    setAttenuation.on(pollMetrics(4813).BaseConnectionEvent.SdpError, (operation, error, type, sdp) => {
      closure_1_1(1245).track(constants.SDP_ERROR, { operation, error, type, sdp });
    });
    setAttenuation.on(pollMetrics(4813).BaseConnectionEvent.VideoState, (videoState) => {
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.DeviceChange, (inputDevices, outputDevices, videoDevices) => {
    timeout.stop();
    closure_1(577).dispatch({ type: "MEDIA_ENGINE_DEVICES", inputDevices, outputDevices, videoDevices });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.VolumeChange, (inputVolume, outputVolume) => {
    closure_1(577).dispatch({ type: "AUDIO_VOLUME_CHANGE", inputVolume, outputVolume });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.DesktopSourceEnd, (endReason, errorCode) => {
    closure_1(577).dispatch({ type: "MEDIA_ENGINE_SET_GO_LIVE_SOURCE", settings: null, endReason, errorCode });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.AudioPermission, (granted) => {
    c127 = true;
    closure_1(577).dispatch({ type: "MEDIA_ENGINE_PERMISSION", kind: "audio", granted });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.VideoPermission, (granted) => {
    closure_1(577).dispatch({ type: "MEDIA_ENGINE_PERMISSION", kind: "video", granted });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.WatchdogTimeout, asyncGeneratorStep(async (arg0, value) => {
    if (c6 === 2) {
      c6 = 3;
      throw new TypeError("Generator functions may not be called on executing generators");
    } else if (tmp6 === 3) {
        ...
        if (0 === c5) {
          if (arg0 === 1) {
            ...
            if ("canary" === window.GLOBAL_ENV.RELEASE_CHANNEL) {
              ...
              c5 = 2;
              c6 = 1;
              const obj6 = { value: closure_1(4752).submitLiveCrashReport(obj4), done: false };
              return obj6;
            }
          }
        } else if (1 === tmp7) {
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.VideoInputInitialized, (description) => {
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.AudioInputInitialized, (description) => {
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.ClipsRecordingRestartNeeded, () => {
    closure_1(577).dispatch({ type: "CLIPS_RESTART" });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.ClipsInitFailure, (errMsg, applicationName) => {
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.ClipsRecordingHealthy, () => {
    if (0 !== c83) {
      const _HermesInternal = HermesInternal;
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.ClipsRecordingReadyChanged, (arg0) => {
    if (closure_84 !== arg0) {
      const _HermesInternal = HermesInternal;
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.ClipsBridgeIdleShutdown, () => {
    logger.info("Clips bridge idle shutdown");
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.ClipsRecordingEnded, (arg0, soundshareId) => {
    let id;
    if (_null != null) {
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.NativeScreenSharePickerUpdate, (existing, content) => {
    closure_1(577).dispatch({ type: "NATIVE_SCREEN_SHARE_PICKER_UPDATE", existing, content });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.NativeScreenSharePickerCancel, (existing) => {
    closure_1(577).dispatch({ type: "NATIVE_SCREEN_SHARE_PICKER_CANCEL", existing });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.NativeScreenSharePickerError, (error) => {
    closure_1(577).dispatch({ type: "NATIVE_SCREEN_SHARE_PICKER_ERROR", error });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.AudioDeviceModuleError, (audio_device_module, code, device_name) => {
    closure_1(1245).track(constants.AUDIO_DEVICE_MODULE_ERROR, { audio_device_module, code, device_name });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.VideoCodecError, (mode) => {
    if ("encode" === mode.mode) {
      let VIDEO_DECODE_ERROR = pollMetrics(9681).AVError.VIDEO_ENCODE_ERROR;
      let tmp3 = pollMetrics;
    } else {
      VIDEO_DECODE_ERROR = pollMetrics(9681).AVError.VIDEO_DECODE_ERROR;
      tmp3 = pollMetrics;
    }
    const obj = { videoCodec: mode.codecStandard, errorMessage: mode.message };
    if (VIDEO_DECODE_ERROR === tmp3(9681).AVError.VIDEO_ENCODE_ERROR) {
    ...
    tmp3(9681).reportAVError(obj3);
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.ConnectionStats, (arr) => {
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.VoiceProcessingError, handleVoiceProcessingError);
  closure_73.on(pollMetrics(4813).MediaEngineEvent.VideoFilterError, handleVideoFilterError);
  closure_73.on(pollMetrics(4813).MediaEngineEvent.SpatialAudioStatus, handleSpatialAudioStatus);
  closure_73.on(pollMetrics(4813).MediaEngineEvent.VoiceQueueMetrics, (arg0) => {
  ...
  closure_73.on(pollMetrics(4813).MediaEngineEvent.DeviceHardwareMutedChange, (deviceGuid, hardwareMuted) => {
    closure_1(577).dispatch({ type: "AUDIO_INPUT_DEVICE_HARDWARE_MUTED_CHANGED", deviceGuid, hardwareMuted });
  });
  closure_73.on(pollMetrics(4813).MediaEngineEvent.SystemMicrophoneModeChange, (arg0) => {
    closure_131 = arg0;
  ...
  _modDef12.each(value2, (modeOptions) => {
    ...
    if (tmp8) {
      modeOptions.modeOptions.shortcut = tmp2(14337).toCombo(modeOptions.modeOptions.shortcut);
      const tmp2Result = tmp2(14337);
    }
...
prototype["supportsExperimentalSoundshare"] = function supportsExperimentalSoundshare() {
  ...
  if (supportsResult) {
    ...
    supportsResult = _modDef14286.satisfies(release, __initData4);
  }
  return supportsResult;
};
prototype["supportsHookSoundshare"] = function supportsHookSoundshare() {
  ...
  if (isWindowsResult) {
    ...
    isWindowsResult = _modDef14286.satisfies(release, __initData3);
  }
...
prototype["getVideoEncoderExperiments"] = function getVideoEncoderExperiments(STREAM, streamer) {
  ...
  if (isWindowsResult) {
    ...
    const VideoCaptureDeviceNoReuseExperiment = tmp6(14349).VideoCaptureDeviceNoReuseExperiment;
...
prototype["supportsScreenSoundshare"] = function supportsScreenSoundshare() {
  if (obj.isMac()) {
    ...
    if (supportsResult) {
      ...
      supportsResult = _modDef14286.satisfies(release, closure_1_25);
    }
    if (supportsResult) {
      ...
      if (satisfiesResult) {
        ...
        satisfiesResult = _modDef14286.satisfies(release1, closure_1_24);
      }
...
const mediaEngineStore = new MediaEngineStore(DispatcherDefault, {
  ...
  CONNECTION_OPEN: function handleConnectionOpen(sessionId) {
    ...
    if (enabled) {
      enabled = tmp2(14344).getIOSAudioInterruptExperimentConfig("handleConnectionOpen").enabled;
      const tmp2Result4 = tmp2(14344);
    }
    if (enabled) {
    ...
    if (tmp2Result5.isIOS()) {
      ...
      if (setNcModels != null) {
        setNcModels(tmp2(14345).KRISP_NC_MODELS);
      }
      mediaEngineStore.emitChange();
    }
    ...
    if (tmp2Result6.isWindows()) {
      if (obj8.getConfig({ location: "MediaEngineStore.setInputDevice" }).probeAudioEffects) {
        ...
        if (null != guid) {
          tmp21(14342)(tmp23.guid, inputDeviceId, obj3);
        }
      }
  ...
  AUDIO_SET_LOCAL_VIDEO_DISABLED: function handleSetLocalVideoDisabled(arg0) {
    ...
    if (!tmp22) {
      ...
      if (isAutomatic) {
        tmp3(14099)(userId, tmp12 ? constants12.AUTO_DISABLE : constants12.AUTO_ENABLE, tmp11);
        if (tmp12) {
          obj.add(userId);
        } else {
          obj.delete(userId);
        ...
        if (hasItem) {
          if (!tmp12) {
            ...
            tmp3(14099)(userId, constants12.MANUAL_REENABLE, tmp11);
          }
        }
        tmp3(14099)(userId, tmp12 ? constants12.MANUAL_DISABLE : constants12.MANUAL_ENABLE, tmp11);
      }
      if (tmp19) {
        tmp19 = !tmp12;
      }
  ...
  MEDIA_ENGINE_DEVICES: function handleDevices(videoDevices) {
    ...
    if (0 === arr.length) {
      let obj = { id: DEFAULT_DEVICE_ID, deviceType: AUDIO_INPUT, index: 0, name: tmp5, disabled: true, guid: "__packager_asset", hardwareId: "right", containerId: "Symbol" };
      obj2 = {};
      obj2[obj.id] = obj;
    ...
    if (0 === arr3.length) {
      const obj3 = { id: DEFAULT_DEVICE_ID, deviceType: AUDIO_OUTPUT, index: 0, name: tmp21, disabled: true, guid: "__packager_asset", hardwareId: "right", containerId: "Symbol" };
      const obj6 = {};
      obj6[obj3.id] = obj3;
    ...
    if (0 === arr5.length) {
      const obj7 = { id: DEFAULT_DEVICE_ID, deviceType: VIDEO_INPUT, index: 0, name: tmp38, disabled: true, guid: "__packager_asset", hardwareId: "right", containerId: "Symbol" };
      const obj8 = {};
      obj8[obj7.id] = obj7;
    ...
    if (closure_95) {
      if (!tmp9Result2.isEqual(valueResult4, tmp37)) {
        ...
        let tmp49 = "Firefox" === tmp9(5080).name;
        if (tmp49) {
          tmp49 = "" === closure_96;
        }
        if (tmp49) {
  ...
  CLIPS_INIT: function handleClipsInit(applicationName) {
    ...
    if (obj.isClipsEnabled()) {
      if (null != DiscordNativeDefault) {
        ...
        const pidFromDesktopSource = tmp3(4752).getPidFromDesktopSource(sourceId);
        const tmp3Result = tmp3(4752);
        ...
        enabled = videoHook2;
        if (videoHook2) {
          const VideoHookDX12Experiment = tmp(14280).VideoHookDX12Experiment;
          enabled = VideoHookDX12Experiment.getConfig({ location: "handleClipsInit" }).enabled;
        }
        const UpscaleSmallCapturedFramesExperiment = tmp(14281).UpscaleSmallCapturedFramesExperiment;
        ...
        ({ minCaptureWidth, minCaptureHeight } = config);
        const obj8 = result;
        const tmpResult4 = tmp(14272);
        let isWindowsResult = tmp(1368).isWindows();
        if (isWindowsResult) {
          const tmp3Result6 = tmp3(4381);
          let release;
          if (tmp3Result6 != null) {
            release = tmp3Result6.os.release;
          }
          isWindowsResult = tmp3(14286).satisfies(release, closure_1_29);
          const tmp3Result5 = tmp3(14286);
        }
        ...
        if (isMacResult) {
          const tmp3Result8 = tmp3(4381);
          let release1;
          if (tmp3Result8 != null) {
            release1 = tmp3Result8.os.release;
          }
          isMacResult = tmp3(14286).satisfies(release1, closure_1_24);
          const tmp3Result7 = tmp3(14286);
        }
