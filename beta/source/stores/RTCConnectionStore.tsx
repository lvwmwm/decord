...
// Dependencies: [4807, 502, 4814, 1078, 4815, 3, 4817, 4844, 577, 14133, 2040, 4819, 5662, 14134, 504, 14136, 1245, 1984, 2]

// Module 4813 (RTCConnectionStore)
import LoggerDefault from "Logger" /* 3 */;
import initializeDefault from "initialize" /* 504 */;
import DispatcherDefault from "Dispatcher" /* 577 */;
import TimeUtils from "TimeUtils" /* 4819 */;
import VoiceStateAnalyticsDefault from "VoiceStateAnalytics" /* 14133 */;
import useSpatialAudioControlState from "useSpatialAudioControlState" /* 14134 */;
import trackVideoToggle from "trackVideoToggle" /* 14136 */;
...
function createRTCConnection(guildId, channelId, createdTime) {
  if (null == sessionId) {
    ...
    _default = new _default(4817).default(obj);
    _default.on(_default(4844).RTCConnectionEvent.State, (state, arg1, arg2) => {
      closure_1 = arg1;
      dependencyMap = arg2;
      closure_1(577).wait(() => {
    ...
    _default.on(_default(4844).RTCConnectionEvent.Video, (guildId, channelId, userId, streamId, rtcServerId) => {
    ...
    _default.on(_default(4844).RTCConnectionEvent.Ping, (pings, quality) => {
      quality(577).wait(() => DispatcherDefault.dispatch({ type: "RTC_CONNECTION_PING", pings, quality }));
    });
    _default.on(_default(4844).RTCConnectionEvent.OutboundLossRate, (lossRate) => {
      closure_1(577).wait(() => DispatcherDefault.dispatch({ type: "RTC_CONNECTION_LOSS_RATE", lossRate }));
    });
    _default.on(_default(4844).RTCConnectionEvent.Speaking, (userId, speaking) => {
      if (speaking != null) {
        speaking.setSpeaking(userId, speaking);
      }
    });
    _default.on(_default(4844).RTCConnectionEvent.Flags, (userId, flags) => {
      DispatcherDefault.wait(() => {
    ...
    _default.on(_default(4844).RTCConnectionEvent.UsersMerged, (userIds, context) => {
      DispatcherDefault.dispatch({ type: "RTC_CONNECTION_USERS_MERGED", userIds, context });
    });
    _default.on(_default(4844).RTCConnectionEvent.ClientConnect, (userIds) => {
      DispatcherDefault.wait(() => {
    ...
    _default.on(_default(4844).RTCConnectionEvent.ClientDisconnect, (userId) => {
      DispatcherDefault.wait(() => {
    ...
    _default.on(_default(4844).RTCConnectionEvent.Platform, (userId, platform, channelId) => {
      dependencyMap = channelId;
      platform(577).wait(() => {
        DispatcherDefault.dispatch({ type: "RTC_CONNECTION_PLATFORM", platform, userId, channelId });
      });
    });
    _default.on(_default(4844).RTCConnectionEvent.SecureFramesUpdate, () => {
      DispatcherDefault.wait(() => {
        closure_1_1(dependencyMap[8]).dispatch({ type: "RTC_CONNECTION_SECURE_FRAMES_UPDATE" });
      });
    });
    _default.on(_default(4844).RTCConnectionEvent.RosterMapUpdate, (userIds) => {
      closure_1(577).wait(() => {
