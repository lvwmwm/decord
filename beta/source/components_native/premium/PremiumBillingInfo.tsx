...
// Dependencies: [32, 19, 17, 1078, 21, 4790, 580, 558, 568, 4463, 1119, 4450, 4786, 7441, 7461, 13695, 7680, 13699, 2]

// Module 13698 (PremiumBillingInfo)
import c from "c" /* 568 */;
...
let tmp5 = ReactCompilerGating.isReactCompilerEnabled() ? ((arg0) => {
  ...
  if (obj.isGooglePlayBillingSupported()) {
    ...
    if (subscription.isPurchasedViaGoogle) {
      ...
      const obj3 = { onClick: tmp(4450).getExternalSubscriptionMethodUrl(subscription.paymentGateway, "SUBSCRIPTION_MANAGEMENT") };
      obj2.children = intl.format(tmp(1119).t["9NPc+O"], obj3);
      tmp3 = timestampProducer(tmp(4786).Text, obj2);
...
export default ReactCompilerGating.isReactCompilerEnabled() ? ((arg0) => {
  ...
  if (cResult[0] === subscription.id) {
    ...
    const first = _slicedToArray(tmp(13695).useFetchSubscriptionInvoicePreview(tmp7), 1)[0];
    if (cResult[3] === subscription.id) {
      if (cResult[4] === tmp11) {
        let tmp12 = cResult[5];
      }
      const first1 = tmp8(tmp(13695).useGetSubscriptionInvoice(tmp12), 1)[0];
      if (null == first) {
        return null;
      } else {
        if (cResult[6] !== subscription) {
          const externalManagementMessage = tmp(7680).getExternalManagementMessage(subscription, { shouldAllowExternalManagement: true });
          cResult[6] = subscription;
          cResult[7] = externalManagementMessage;
          let tmp15 = externalManagementMessage;
          const tmpResult5 = tmp(7680);
        } else {
          tmp15 = cResult[7];
        }
        ...
        if (cResult[9] !== tmp4.title) {
          ...
          const tmp22 = timestampProducer(tmp(4786).Text, obj2);
        ...
        if (cResult[11] === Symbol.for("react.memo_cache_sentinel")) {
          ...
          const tmp25 = timestampProducer(tmp(4786).Text, obj3);
          cResult[11] = tmp25;
          let tmp23 = tmp25;
        } else {
          tmp23 = cResult[11];
        ...
        if (cResult[12] === first1) {
          if (cResult[13] === first) {
                ...
                if (cResult[22] === tmp4.billingContainer) {
                  if (cResult[23] === tmp29) {
                    ...
                    if (null != tmp15) {
                      ...
                      tmp41 = timestampProducer(tmp(4786).Text, obj5);
                    }
                    cResult[26] = tmp15;
            ...
            const tmp31 = timestampProducer(tmp(4786).Text, obj8);
        ...
        const billingInformationStringNative = tmp(13699).getBillingInformationStringNative(subscription, first, first1);
        cResult[12] = first1;
        cResult[13] = first;
        cResult[14] = subscription;
        cResult[15] = billingInformationStringNative;
        tmp27 = billingInformationStringNative;
        const tmpResult6 = tmp(13699);
      }
      const tmpResult4 = tmp(13695);
    }
  ...
  if (null == first) {
    ...
    const externalManagementMessage = tmp2(7680).getExternalManagementMessage(subscription, { shouldAllowExternalManagement: true });
    ...
    const items = [timestampProducer(tmp2(4786).Text, obj4), , ];
    ...
    const items1 = [timestampProducer(tmp2(4786).Text, obj6), , ];
    const obj7 = { style: tmp.billingRenewalInfo, variant: "text-sm/medium", children: null };
    const tmp10 = View;
    const tmp11 = timestampProducer;
    const tmp2Result = tmp2(7680);
    const tmp9 = React5;
    obj7.children = tmp2(13699).getBillingInformationStringNative(subscription, first, tmp6);
    items1[1] = timestampProducer(tmp2(4786).Text, obj7);
    ...
    if (null != externalManagementMessage) {
      ...
      tmp11Result = tmp11(tmp2(4786).Text, obj9);
    }
    items[2] = tmp11Result;
    obj3.children = items;
