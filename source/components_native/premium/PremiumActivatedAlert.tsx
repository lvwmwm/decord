...
// Dependencies: [19, 17, 676, 21, 4285, 5125, 3931, 7301, 7302, 7303, 7304, 7305, 7251, 7252, 7306, 7307, 7308, 7309, 7310, 7311, 7312, 4131, 7313, 7314, 7315, 7316, 7317, 7318, 7319, 7320, 7321, 1236, 4221, 4713, 7322, 7323, 1297, 2]
// Exports: default
...
let closure_11 = createCacheKey.createStyles((arg0) => {
  if (require(3931) /* getPremiumPlanItem */.Branding.BUNDLE === arg0) {
    let obj = { animation: null };
    obj[0] = { borderRadius: 6 };
    return obj;
  } else {
    if (tmp(3931).Branding.TIER_0 !== arg0) {
      if (tmp(3931).Branding.TIER_1 !== arg0) {
        if (tmp(3931).Branding.TIER_2 !== arg0) {
          if (tmp(3931).Branding.PREMIUM_GUILD === arg0) {
            obj = { animation: null };
...
export default function PremiumActivatedAlert(onClose) {
  ...
  if (require(3931) /* getPremiumPlanItem */.Branding.TIER_0 === premiumBranding) {
    obj = { logo: null };
    ...
    if (tmp7(3931).Branding.BUNDLE !== premiumBranding) {
      if (tmp7(3931).Branding.TIER_2 !== premiumBranding) {
        if (tmp7(3931).Branding.PREMIUM_GUILD === premiumBranding) {
  ...
  } else if (tmp7(3931).Branding.TIER_2 === premiumBranding) {
    tmp4Result = tmp4(7303);
  } else if (tmp7(3931).Branding.BUNDLE === premiumBranding) {
    tmp4Result = tmp4(7304);
  } else if (tmp7(3931).Branding.PREMIUM_GUILD === premiumBranding) {
  ...
  if (premiumBranding === require(3931) /* getPremiumPlanItem */.Branding.BUNDLE) {
  ...
  if (require(3931) /* getPremiumPlanItem */.Branding.TIER_0 === premiumBranding) {
    ...
    if (tmp7(3931).Branding.TIER_1 === premiumBranding) {
    ...
    if (tmp7(3931).Branding.TIER_0 !== premiumBranding) {
      if (tmp7(3931).Branding.TIER_1 !== premiumBranding) {
        if (tmp7(3931).Branding.TIER_2 === premiumBranding) {
          ...
          obj10[0] = tmp7(3931).getExternalPlanDisplayName({ planId: null, additionalPlans: null });
          stringResult = intl2.format(tmp7(1236).t.YJUUH3, obj10);
          const obj11 = { planId: null, additionalPlans: null };
          const tmp7Result3 = tmp7(3931);
        }
      }
      obj9[1] = stringResult;
