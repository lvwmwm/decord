...
function GlobalEmojiUpsell(arg0) {
  ...
  if (null != sku_id) {
    ...
    if (useTier0Description) {
      ...
      obj1[0] = tmp(3835).getPremiumTypeDisplayName(closure_13.TIER_0);
...
function AnimatedEmojiUpsell(alertWidth) {
  ...
  if (alertWidth.useTier0Description) {
    obj = { planName: null };
    obj[0] = require(3835) /* getPremiumPlanItem */.getPremiumTypeDisplayName(closure_13.TIER_0);
    let formatResult = format(t["1a36ee"], obj);
    const tmp4Result = require(3835) /* getPremiumPlanItem */;
  } else {
...
function UploadUpsell(arg0) {
  ...
  if (callback(importAllResult.useState(!stateFromStores), 1)[0]) {
    ...
    tmp6Result = callback2(tmp(5555).TableSwitchRow, obj);
  }
...
UpsellItem.prototype["render"] = function render() {
  ...
  items1[2] = callback2(require(4185) /* Text */.Text, { style: tmp.upsellDescription, variant: "text-sm/medium", children: description });
  obj[1] = items1;
