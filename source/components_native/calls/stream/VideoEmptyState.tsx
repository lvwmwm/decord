...
// Dependencies: [19, 17, 1372, 676, 21, 4255, 712, 10568, 1297, 1236, 10571, 9063, 589, 4681, 4434, 4318, 4251, 4666, 2]
// Exports: default
...
export default function VideoEmptyState(style) {
  ...
  if (obj.STREAM_ENDED === type) {
    ...
    if (!removeSplashImage) {
      ...
      tmp16 = callback(stream(10568).StreamEnded, obj);
    }
    obj = { children: null };
    const items = [tmp16, ];
    ...
    if (!removeSplashImage) {
      obj = { style: null };
      obj[0] = tmp2.placeholderImage;
      tmp7 = callback(stream(10571).StreamFailed, obj);
    }
  ...
  if (null != avError) {
    let obj7 = stream(9063);
    let errorCode = obj7.getErrorInfo(avError).errorCode;
  } else {
    errorCode = null;
    if (type === tmp3.STREAM_FAILED) {
      let obj6 = stream(9063);
      errorCode = obj6.getErrorInfo(stream(9063).AVError.STREAM_FAILED_TO_START).errorCode;
    }
  }
  let obj8 = stream(589);
  ...
  if (!removeCloseButton) {
    ...
    obj7[1] = callback(tmp28(4666).BaseTextButton, obj8);
