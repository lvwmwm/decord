...
// Dependencies: [19, 17, 1372, 676, 21, 4285, 712, 10623, 1297, 1236, 10626, 9092, 589, 4711, 4464, 4348, 4281, 4696, 2]
// Exports: default

// Module 10622 (VideoEmptyState)
import "noop";
...
export default function VideoEmptyState(style) {
  ...
  if (obj.STREAM_ENDED === type) {
    ...
    if (!removeSplashImage) {
      ...
      tmp16 = callback(stream(10623).StreamEnded, obj);
    }
    obj = { children: null };
    const items = [tmp16, ];
    ...
    if (!removeSplashImage) {
      obj = { style: null };
      obj[0] = tmp2.placeholderImage;
      tmp7 = callback(stream(10626).StreamFailed, obj);
    }
  ...
  if (null != avError) {
    let obj7 = stream(9092);
    let errorCode = obj7.getErrorInfo(avError).errorCode;
  } else {
    errorCode = null;
    if (type === tmp3.STREAM_FAILED) {
      let obj6 = stream(9092);
      errorCode = obj6.getErrorInfo(stream(9092).AVError.STREAM_FAILED_TO_START).errorCode;
    }
  }
  let obj8 = stream(589);
