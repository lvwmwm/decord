...
// Dependencies: [19, 17, 1391, 676, 21, 4444, 712, 10844, 1297, 1236, 10847, 9385, 589, 4896, 4627, 4510, 4440, 4880, 2]
// Exports: default

// Module 10843 (VideoEmptyState)
...
export default function VideoEmptyState(style) {
  ...
  if (obj.STREAM_ENDED === type) {
    ...
    if (!removeSplashImage) {
      ...
      tmp16 = callback(stream(10844).StreamEnded, obj);
    }
    obj = { children: null };
    const items = [tmp16, ];
    ...
    if (!removeSplashImage) {
      obj = { style: null };
      obj[0] = tmp2.placeholderImage;
      tmp7 = callback(stream(10847).StreamFailed, obj);
    }
  ...
  if (null != avError) {
    let obj7 = stream(9385);
    let errorCode = obj7.getErrorInfo(avError).errorCode;
  } else {
    errorCode = null;
    if (type === tmp3.STREAM_FAILED) {
      let obj6 = stream(9385);
      errorCode = obj6.getErrorInfo(stream(9385).AVError.STREAM_FAILED_TO_START).errorCode;
    }
  }
  let obj8 = stream(589);
  ...
  if (tmp34) {
    ...
    tmp34 = callback(tmp28(4440).Text, obj5);
  }
  items5[1] = tmp34;
  ...
  if (!removeCloseButton) {
    ...
    obj7[1] = callback(tmp28(4880).BaseTextButton, obj8);
