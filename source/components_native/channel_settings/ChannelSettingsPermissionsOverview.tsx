...
function ChannelPermissionSyncModule(channel) {
  ...
  obj3[2] = callback;
  obj2[2] = closure_16(channel(5315).TableRow, obj3);
  obj[1] = closure_16(channel(5649).TableRowGroup, obj2);
...
function AddPermission(isEditing) {
  ...
  if (isEditing.isEditing) {
    ...
    items[1] = callback(tmp2(5315).TableRow, obj2);
    obj[2] = items;
    obj[1] = callback2(tmp2(5649).TableRowGroup, obj);
...
function RoleOverwrites(guild) {
  ...
  if (null == obj[guild.id]) {
    obj[guild.id] = importAll(3918).makeEveryoneOverwrite(guild.id);
  ...
  obj[1] = callback(guild(5649).TableRowGroup, obj1);
  return callback(View, obj);
}
...
function MemberOverwrites(channel) {
  ...
  if (valueResult.length > 0) {
    ...
    obj[1] = callback(tmp3(5649).TableRowGroup, obj);
