...
// Function ID: 11065
// Name: onTapMention
// Dependencies: [5, 11062, 9751, 8943, 4017, 11025, 8789, 5748, 4271, 11065, 2007, 11, 3993, 1236, 5756, 4020, 4642, 8740, 2]

// Module 11064 (onTapMention)
...
obj[3] = function onTapMention(closure_0) {
  ...
  if (null != userId) {
    ...
    obj[0] = userId;
    obj[1] = channelId;
    const items = [importDefault(5748).USER_MENTION];
    obj[2] = items;
    importDefault(8789)(obj);
    const tmp15 = importDefault(8789);
  } else {
    if (null != roleId) {
      if (null != guildId) {
        ...
        obj[1] = roleId;
        obj[2] = channelId;
        importDefault(4271).openLazy(tmp(2007)(11065, tmp2.paths), "RoleMembersActionSheet", obj);
...
obj[4] = function onTapTimestamp(nativeEvent) {
  require(4020) /* presentAddedFriendToast */.presentTimestamp(nativeEvent.nativeEvent.node.full);
};
obj[5] = function onTapInlineCode(nativeEvent) {
  const node = nativeEvent.nativeEvent.node;
  if (tmp) {
    require(5756) /* _copy */.copy(node.content);
    const obj = require(5756) /* _copy */;
    const result = require(4020) /* presentAddedFriendToast */.presentCopiedToClipboard();
    const obj2 = require(4020) /* presentAddedFriendToast */;
  }
};
obj[6] = function onTapEmoji(emojiNode) {
  importDefault(4271).openLazy(require(2007) /* asyncRequireImpl */(8740, dependencyMap.paths), "MessageEmojiActionSheet", { emojiNode: emojiNode.nativeEvent.node });
};
