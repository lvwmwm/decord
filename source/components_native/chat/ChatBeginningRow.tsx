...
// Dependencies: [32, 5, 19, 17, 4440, 1386, 6092, 9690, 5443, 7334, 1391, 1430, 1387, 1982, 1992, 1909, 4091, 4100, 4669, 1922, 676, 1394, 12000, 11974, 9019, 21, 4448, 712, 4988, 1236, 4990, 4995, 4997, 4883, 4444, 4160, 4167, 5036, 11159, 1297, 12001, 8715, 10288, 589, 4292, 7838, 4642, 9668, 6090, 6087, 10383, 4383, 4438, 12002, 12003, 38, 11, 12005, 4163, 9689, 5050, 12009, 6342, 9870, 6754, 1995, 12120, 12123, 8719, 4461, 4896, 12127, 4827, 12128, 5929, 9414, 698, 10200, 7826, 5455, 5450, 4164, 4415, 12130, 2009, 5921, 12131, 8083, 8859, 5949, 9193, 8527, 6154, 4691, 9231, 5972, 9021, 9037, 9033, 9031, 12132, 8333, 4302, 8617, 4268, 1373, 11155, 6036, 10610, 10076, 4391, 12136, 12140, 2]
// Exports: default

// Module 11999 (ChatBeginningRowIcon)
...
import ChannelAccessInfoDefault from "ChannelAccessInfo" /* 12002 */;
import registerAssetDefault2 from "registerAsset" /* 12127 */;
import PortalAccessibilityWorkaroundViewDefault from "PortalAccessibilityWorkaroundView" /* 12140 */;
...
import closure_12 from "recomputeFromAppTokens" /* 5443 */;
import { FetchState } from "recomputeFromAppTokens" /* 5443 */;
import closure_14 from "createUserWidgetFromServer" /* 7334 */;
...
import closure_21 from "getUncachedChannelPermissions" /* 4091 */;
import closure_22 from "markAllUserIdListsStale" /* 4100 */;
...
import { MOBILE_MESSAGE_REQUESTS_MODAL_KEY as closure_40 } from "MESSAGE_REQUEST_ACCEPT_CONFIRMATION_MODAL" /* 11974 */;
import { BLOCK_CONFIRMATION_ACTION_SHEET_KEY as closure_41 } from "IGNORE_CONFIRMATION_ACTION_SHEET_KEY" /* 9019 */;
...
function ChatBeginningRowIcon(arg0) {
  ...
  if (isForumPost) {
    obj[0] = tmp2(4988).ChatIcon;
    obj[1] = tmp2(1236).t.Y4REmB;
    let tmp5 = tmp2;
    let tmp6 = obj;
  } else if (isThread) {
    obj[0] = tmp2(4990).ThreadIcon;
    obj[1] = tmp2(1236).t["7Xm5QI"];
    tmp5 = tmp2;
    tmp6 = obj;
  } else if (isPrivate) {
    obj[0] = tmp2(4995).TextLockIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  } else {
    obj[0] = tmp2(4997).TextIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  }
...
function ChatBeginningRowHeader(arg0) {
  ...
  if (!isGameInvitesPost) {
    ...
    obj1[0] = items1;
    obj1[4] = title;
    items[1] = callback2(tmp2(4444).Text, obj1);
    obj[0] = items;
  ...
  if (tmp11Result) {
    ...
    obj3[2] = tmp.subtitle;
    obj3[5] = subtitle;
    tmp11Result = callback2(tmp2(4444).Text, obj3);
...
function LinkManageButtons(arg0) {
  ...
  if (isPrivate) {
    ...
    items1[1] = callback2(tmp2(4444).Text, obj2);
    obj[3] = items1;
    isPrivate = tmp6(tmp2(5036).PressableOpacity, obj);
  }
  ...
  if (canEdit) {
    ...
    obj4[1] = token;
    const items5 = [callback2(tmp2(10288).PencilIcon, obj4), ];
    ...
    items5[1] = callback2(tmp2(4444).Text, obj5);
    obj3[3] = items5;
    canEdit = tmp6(tmp2(5036).PressableOpacity, obj3);
  }
  items3[1] = canEdit;
  obj[1] = items3;
...
function ThreadOwner(arg0) {
  ...
  if ("username" === stateFromStores2) {
    ...
    obj[1] = tmp16;
    obj[4] = str;
    return callback2(tmp2(4444).Text, obj);
  } else {
    ...
    obj3[2] = str;
    items3[1] = callback2(tmp2(4444).Text, obj3);
    obj2[0] = items3;
...
function ChatBeginningRowThread(channel) {
  ...
  if (null != channel.threadMetadata) {
    ...
    if (!isForumPostResult) {
      ...
      tmp12Result = tmp12(tmp4(4444).Text, obj1);
    }
    items[2] = tmp12Result;
    ...
    if (channel.isForumPost()) {
      ...
      if (null == tmp8) {
        ...
        tmp12Result1 = tmp12(tmp4(4444).Text, obj3);
      }
    }
...
function DMSpamButton(channel) {
  ...
  if (!channel.showingSpamBanner) {
    if (dMMessageToReport.isReportable) {
      ...
      return callback2(tmp(4896).Button, obj);
    }
  }
  return null;
}
function ManageAppButton(application) {
  ...
  if (application.user.bot) {
    ...
    if (null != application) {
      ...
      tmp4 = callback2(tmp2(4896).Button, obj);
    }
  }
  return tmp4;
}
...
function ChatBeginningRowDM(channel) {
  ...
  const tmp = callback3();
  dependencyMap = tmp;
  analyticsLocations = user(5921)().analyticsLocations;
  let id;
  if (user != null) {
    id = user.id;
  }
  ...
  if (null == user) {
    ...
    const items5 = [callback2(tmp7(5036).PressableOpacity, obj1), , , , , , ];
    ...
    obj4[2] = name;
    obj3[3] = callback2(tmp7(4444).Text, obj4);
    items5[1] = callback2(tmp7(5036).PressableOpacity, obj3);
    let tmp15Result = null;
    if (!user.isProvisional) {
      ...
      obj6[2] = userTag;
      obj5[3] = tmp15(tmp7(4444).Text, obj6);
      tmp15Result = tmp15(tmp7(5036).PressableOpacity, obj5);
    }
    ...
    obj7[3] = stringResult;
    items5[3] = callback2(tmp7(4444).Text, obj7);
    tmp15Result = null;
    if (user.isProvisional) {
      let obj8 = { style: null, userId: null, iconSize: 14 };
      obj8[0] = tmp.provisionalAccountExplainer;
      obj8[1] = user.id;
      tmp15Result = tmp15(tmp7(12132).ChatProvisionalAccountExplainerCard, obj8);
    }
    ...
    if (null != stateFromStores) {
      if (stateFromStores.length >= 1) {
        ...
        function handleOpenMutualGuilds() {
          if (null != user) {
            ...
            obj[1] = function onPressMutualGuild(arg0) {
              ...
              callback2(4691).popWithKey(closure_40);
            };
        ...
        obj10[0] = tmp7(5972).GuildIconSizes.SMALL;
        ...
        const items6 = [tmp15(tmp7(9231).GuildIconPile, obj10), ];
        ...
        items6[1] = tmp15(tmp7(4444).Text, obj11);
        obj9[3] = items6;
        tmp18Result = tmp18(tmp7(5036).PressableOpacity, obj9);
      }
    }
...
function ChatBeginningRowGroupDM(channel) {
  ...
  callback = React.useCallback(() => {
    channel(4302).navigateToNewGroupDM(channel.id, closure_1_27.CHANNEL_TEXT_AREA);
  }, items1);
  ...
  const callback2 = React.useCallback(() => {
    ...
    if (obj.UNSAFE_isDismissibleContentDismissed(channel(1373).DismissibleContent.GDM_INVITE_REMINDER)) {
      callback();
    } else {
      obj = { onClick: null };
      obj[0] = callback;
  ...
  const items5 = [callback2(first(10610), obj), , , ];
  ...
  const tmp17 = channel.id === channel(6036).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID;
  obj1[1] = str;
  obj1[3] = arr;
  items5[1] = callback2(channel(4444).Text, obj1);
  ...
  obj2[3] = formatResult;
  items5[2] = callback2(channel(4444).Text, obj2);
  ...
  if (!tmp17) {
    ...
    if (stateFromStores) {
      ...
      obj4[1] = callback2;
      obj4[2] = tmp7(10076).GroupPlusIcon;
    ...
    if (channel.hasFlag(ChannelFlags.IS_JOIN_REQUEST_INTERVIEW_CHANNEL)) {
      ...
      obj6[0] = channel.id;
      tmp15Result1 = tmp15(first(12136), obj6);
    }
          ...
          try {
            ...
            if (0 === dependencyMap) {
              if (arg0 === 1) {
                ...
                let base64 = tmp2;
                base64 = undefined;
                closure_1_1(9689).init(dependencyMap);
                const obj7 = closure_1_1(9689);
                obj1 = { size: null };
                obj1[0] = closure_1_35;
                dependencyMap = 1;
                c3 = 1;
