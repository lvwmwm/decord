...
// Dependencies: [32, 5, 19, 17, 4436, 1390, 6068, 9237, 5422, 7293, 1395, 1434, 1391, 1982, 1992, 1910, 4089, 4098, 4653, 1922, 676, 1398, 12102, 12076, 10501, 21, 4444, 712, 4968, 1236, 4970, 4975, 4977, 4866, 4440, 4158, 4165, 5015, 11231, 1297, 12103, 8234, 9889, 589, 4288, 8289, 4638, 9212, 6066, 10220, 4379, 4434, 12104, 12105, 38, 11, 12107, 4161, 9236, 5029, 12111, 6316, 9280, 6720, 1995, 12220, 12223, 8238, 4457, 4879, 12227, 4810, 12228, 5908, 10758, 698, 9955, 8276, 5434, 5429, 4162, 4411, 12230, 2009, 5900, 12231, 8530, 8998, 5928, 12232, 9004, 6132, 4675, 12249, 5951, 9459, 9849, 12251, 10502, 12258, 8188, 4298, 7970, 4265, 1377, 11225, 6015, 9880, 9939, 4387, 12262, 12266, 2]
// Exports: default

// Module 12101 (ChatBeginningRowIcon)
...
import registerAssetDefault from "registerAsset" /* 12103 */;
import ChannelAccessInfoDefault from "ChannelAccessInfo" /* 12104 */;
import registerAssetDefault2 from "registerAsset" /* 12227 */;
import PortalAccessibilityWorkaroundViewDefault from "PortalAccessibilityWorkaroundView" /* 12266 */;
...
import closure_10 from "handleLoadThreadsSuccess" /* 6068 */;
import closure_11 from "handleFormInit" /* 9237 */;
import closure_12 from "recomputeFromAppTokens" /* 5422 */;
import { FetchState } from "recomputeFromAppTokens" /* 5422 */;
import closure_14 from "createUserWidgetFromServer" /* 7293 */;
...
import { MOBILE_MESSAGE_REQUESTS_MODAL_KEY as closure_40 } from "MESSAGE_REQUEST_ACCEPT_CONFIRMATION_MODAL" /* 12076 */;
import { BLOCK_CONFIRMATION_ACTION_SHEET_KEY as closure_41 } from "IGNORE_CONFIRMATION_ACTION_SHEET_KEY" /* 10501 */;
...
function ChatBeginningRowIcon(arg0) {
  ...
  if (isForumPost) {
    obj[0] = tmp2(4968).ChatIcon;
    obj[1] = tmp2(1236).t.Y4REmB;
    let tmp5 = tmp2;
    let tmp6 = obj;
  } else if (isThread) {
    obj[0] = tmp2(4970).ThreadIcon;
    obj[1] = tmp2(1236).t["7Xm5QI"];
    tmp5 = tmp2;
    tmp6 = obj;
  } else if (isPrivate) {
    obj[0] = tmp2(4975).TextLockIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  } else {
    obj[0] = tmp2(4977).TextIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  }
...
function ChatBeginningRowHeader(arg0) {
  ...
  children[1] = callback2(subtitleLink(4440).Text, { style: items1, accessibilityRole: "header", variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", children: title });
...
function LinkManageButtons(arg0) {
  ...
  if (isPrivate) {
    ...
    items1[1] = callback2(tmp2(4440).Text, obj2);
    obj[3] = items1;
    isPrivate = tmp6(tmp2(5015).PressableOpacity, obj);
  }
  ...
  if (canEdit) {
    ...
    obj4[1] = token;
    const items5 = [callback2(tmp2(9889).PencilIcon, obj4), ];
    ...
    items5[1] = callback2(tmp2(4440).Text, obj5);
    obj3[3] = items5;
    canEdit = tmp6(tmp2(5015).PressableOpacity, obj3);
  }
  items3[1] = canEdit;
  obj[1] = items3;
...
function ThreadOwner(arg0) {
  ...
  if ("username" === stateFromStores2) {
    ...
    obj[1] = tmp16;
    obj[4] = str;
    return callback2(tmp2(4440).Text, obj);
  } else {
    ...
    obj3[2] = str;
    items3[1] = callback2(tmp2(4440).Text, obj3);
    obj2[0] = items3;
...
function ChatBeginningRowThread(channel) {
  ...
  if (null != channel.threadMetadata) {
    ...
    if (!isForumPostResult) {
      ...
      tmp11Result = tmp11(tmp4(4440).Text, obj1);
    }
    items[2] = tmp11Result;
    ...
    if (channel.isForumPost()) {
      ...
      if (null == tmp7) {
        ...
        tmp11Result1 = tmp11(tmp4(4440).Text, obj3);
      }
    }
...
function DMSpamButton(channel) {
  ...
  if (!channel.showingSpamBanner) {
    if (dMMessageToReport.isReportable) {
      ...
      return callback2(tmp(4879).Button, obj);
    }
  }
  return null;
}
function ManageAppButton(application) {
  ...
  if (application.user.bot) {
    ...
    if (null != application) {
      ...
      tmp4 = callback2(tmp2(4879).Button, obj);
    }
  }
  return tmp4;
}
...
function ChatBeginningRowDM(channel) {
  ...
  const tmp = callback3();
  dependencyMap = tmp;
  analyticsLocations = user(5900)().analyticsLocations;
  let id;
  if (user != null) {
    id = user.id;
  }
  ...
  if (null == user) {
    ...
    const items5 = [callback2(tmp7(5015).PressableOpacity, obj1), , , , , , ];
    ...
    obj4[2] = name;
    obj3[3] = callback2(tmp7(4440).Text, obj4);
    items5[1] = callback2(tmp7(5015).PressableOpacity, obj3);
    let tmp15Result = null;
    if (!user.isProvisional) {
      ...
      obj6[2] = userTag;
      obj5[3] = tmp15(tmp7(4440).Text, obj6);
      tmp15Result = tmp15(tmp7(5015).PressableOpacity, obj5);
    }
    ...
    obj7[3] = stringResult;
    items5[3] = callback2(tmp7(4440).Text, obj7);
    tmp15Result = null;
    if (user.isProvisional) {
      let obj8 = { style: null, userId: null, iconSize: 14 };
      obj8[0] = tmp.provisionalAccountExplainer;
      obj8[1] = user.id;
      tmp15Result = tmp15(tmp7(12258).ChatProvisionalAccountExplainerCard, obj8);
    }
    ...
    if (null != stateFromStores) {
      if (stateFromStores.length >= 1) {
        ...
        function handleOpenMutualGuilds() {
          if (null != user) {
            ...
            obj[1] = function onPressMutualGuild(arg0) {
              ...
              callback2(4675).popWithKey(closure_40);
            };
        ...
        obj10[0] = tmp7(5951).GuildIconSizes.SMALL;
        ...
        const items6 = [tmp15(tmp7(12249).GuildIconPile, obj10), ];
        ...
        items6[1] = tmp15(tmp7(4440).Text, obj11);
        obj9[3] = items6;
        tmp18Result = tmp18(tmp7(5015).PressableOpacity, obj9);
      }
    }
...
function ChatBeginningRowGroupDM(channel) {
  ...
  callback = React.useCallback(() => {
    channel(4298).navigateToNewGroupDM(channel.id, closure_1_27.CHANNEL_TEXT_AREA);
  }, items1);
  ...
  const items5 = [callback2(first(9880), obj), , , ];
  ...
  const tmp17 = channel.id === channel(6015).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID;
  obj1[1] = str;
  obj1[3] = arr;
  items5[1] = callback2(channel(4440).Text, obj1);
  ...
  obj2[3] = formatResult;
  items5[2] = callback2(channel(4440).Text, obj2);
  ...
  if (!tmp17) {
    ...
    if (stateFromStores) {
      ...
      obj4[1] = callback2;
      obj4[2] = tmp7(9939).GroupPlusIcon;
    ...
    if (channel.hasFlag(ChannelFlags.IS_JOIN_REQUEST_INTERVIEW_CHANNEL)) {
      ...
      obj6[0] = channel.id;
      tmp15Result1 = tmp15(first(12262), obj6);
    }
          ...
          try {
            ...
            if (0 === dependencyMap) {
              if (arg0 === 1) {
                ...
                let base64 = tmp2;
                base64 = undefined;
                closure_1_1(9236).init(dependencyMap);
                const obj7 = closure_1_1(9236);
                obj1 = { size: null };
                obj1[0] = closure_1_35;
                dependencyMap = 1;
                c3 = 1;
