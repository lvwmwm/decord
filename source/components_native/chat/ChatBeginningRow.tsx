...
// Dependencies: [32, 5, 19, 17, 4437, 1390, 6076, 9650, 5427, 7299, 1395, 1434, 1391, 1982, 1992, 1910, 4089, 4098, 4654, 1922, 676, 1398, 11948, 11922, 8978, 21, 4445, 712, 4972, 1236, 4974, 4979, 4981, 4867, 4441, 4158, 4165, 5020, 11112, 1297, 11949, 8677, 10246, 589, 4289, 7803, 4639, 9628, 6074, 6071, 10341, 4380, 4435, 11950, 11951, 38, 11, 11953, 4161, 9649, 5034, 11957, 6322, 9829, 6726, 1995, 12068, 12071, 8681, 4458, 4880, 12075, 4811, 12076, 5913, 9374, 698, 10159, 7791, 5439, 5434, 4162, 4412, 12078, 2009, 5905, 12079, 8047, 8819, 5933, 9153, 8491, 6138, 4676, 9191, 5956, 8980, 8996, 8992, 8990, 12080, 8297, 4299, 8580, 4266, 1377, 11108, 6020, 10568, 10035, 4388, 12084, 12088, 2]
// Exports: default

// Module 11947 (ChatBeginningRowIcon)
...
import registerAssetDefault from "registerAsset" /* 11949 */;
import ChannelAccessInfoDefault from "ChannelAccessInfo" /* 11950 */;
import registerAssetDefault2 from "registerAsset" /* 12075 */;
import PortalAccessibilityWorkaroundViewDefault from "PortalAccessibilityWorkaroundView" /* 12088 */;
...
import closure_10 from "handleLoadThreadsSuccess" /* 6076 */;
import closure_11 from "handleFormInit" /* 9650 */;
import closure_12 from "recomputeFromAppTokens" /* 5427 */;
import { FetchState } from "recomputeFromAppTokens" /* 5427 */;
import closure_14 from "createUserWidgetFromServer" /* 7299 */;
...
import closure_18 from "comparator" /* 1982 */;
import closure_19 from "trackCommunicationDisabled" /* 1992 */;
import closure_20 from "createGuildRecordFromRust" /* 1910 */;
import closure_21 from "getUncachedChannelPermissions" /* 4089 */;
import closure_22 from "markAllUserIdListsStale" /* 4098 */;
...
import { MOBILE_MESSAGE_REQUESTS_MODAL_KEY as closure_40 } from "MESSAGE_REQUEST_ACCEPT_CONFIRMATION_MODAL" /* 11922 */;
import { BLOCK_CONFIRMATION_ACTION_SHEET_KEY as closure_41 } from "IGNORE_CONFIRMATION_ACTION_SHEET_KEY" /* 8978 */;
...
function ChatBeginningRowIcon(arg0) {
  ...
  if (isForumPost) {
    obj[0] = tmp2(4972).ChatIcon;
    obj[1] = tmp2(1236).t.Y4REmB;
    let tmp5 = tmp2;
    let tmp6 = obj;
  } else if (isThread) {
    obj[0] = tmp2(4974).ThreadIcon;
    obj[1] = tmp2(1236).t["7Xm5QI"];
    tmp5 = tmp2;
    tmp6 = obj;
  } else if (isPrivate) {
    obj[0] = tmp2(4979).TextLockIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  } else {
    obj[0] = tmp2(4981).TextIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  }
...
function ChatBeginningRowHeader(arg0) {
  ...
  if (!isGameInvitesPost) {
    ...
    obj1[0] = items1;
    obj1[4] = title;
    items[1] = callback2(tmp2(4441).Text, obj1);
    obj[0] = items;
  ...
  if (tmp11Result) {
    ...
    obj3[2] = tmp.subtitle;
    obj3[5] = subtitle;
    tmp11Result = callback2(tmp2(4441).Text, obj3);
...
function LinkManageButtons(arg0) {
  ...
  if (isPrivate) {
    ...
    items1[1] = callback2(tmp2(4441).Text, obj2);
    obj[3] = items1;
    isPrivate = tmp6(tmp2(5020).PressableOpacity, obj);
  }
  ...
  if (canEdit) {
    ...
    obj4[1] = token;
    const items5 = [callback2(tmp2(10246).PencilIcon, obj4), ];
    ...
    items5[1] = callback2(tmp2(4441).Text, obj5);
    obj3[3] = items5;
    canEdit = tmp6(tmp2(5020).PressableOpacity, obj3);
  }
  items3[1] = canEdit;
  obj[1] = items3;
...
function ThreadOwner(arg0) {
  ...
  if ("username" === stateFromStores2) {
    ...
    obj[1] = tmp16;
    obj[4] = str;
    return callback2(tmp2(4441).Text, obj);
  } else {
    ...
    obj3[2] = str;
    items3[1] = callback2(tmp2(4441).Text, obj3);
    obj2[0] = items3;
...
function ChatBeginningRowThread(channel) {
  ...
  if (null != channel.threadMetadata) {
    ...
    if (!isForumPostResult) {
      ...
      tmp12Result = tmp12(tmp4(4441).Text, obj1);
    }
    items[2] = tmp12Result;
    ...
    if (channel.isForumPost()) {
      ...
      if (null == tmp8) {
        ...
        tmp12Result1 = tmp12(tmp4(4441).Text, obj3);
      }
    }
...
function DMSpamButton(channel) {
  ...
  if (!channel.showingSpamBanner) {
    if (dMMessageToReport.isReportable) {
      ...
      return callback2(tmp(4880).Button, obj);
    }
  }
  return null;
}
function ManageAppButton(application) {
  ...
  if (application.user.bot) {
    ...
    if (null != application) {
      ...
      tmp4 = callback2(tmp2(4880).Button, obj);
    }
  }
  return tmp4;
}
...
function ChatBeginningRowDM(channel) {
  ...
  const tmp = callback3();
  dependencyMap = tmp;
  analyticsLocations = user(5905)().analyticsLocations;
  let id;
  if (user != null) {
    id = user.id;
  }
  ...
  if (null == user) {
    ...
    const items5 = [callback2(tmp7(5020).PressableOpacity, obj1), , , , , , ];
    ...
    obj4[2] = name;
    obj3[3] = callback2(tmp7(4441).Text, obj4);
    items5[1] = callback2(tmp7(5020).PressableOpacity, obj3);
    let tmp15Result = null;
    if (!user.isProvisional) {
      ...
      obj6[2] = userTag;
      obj5[3] = tmp15(tmp7(4441).Text, obj6);
      tmp15Result = tmp15(tmp7(5020).PressableOpacity, obj5);
    }
    ...
    obj7[3] = stringResult;
    items5[3] = callback2(tmp7(4441).Text, obj7);
    tmp15Result = null;
    if (user.isProvisional) {
      let obj8 = { style: null, userId: null, iconSize: 14 };
      obj8[0] = tmp.provisionalAccountExplainer;
      obj8[1] = user.id;
      tmp15Result = tmp15(tmp7(12080).ChatProvisionalAccountExplainerCard, obj8);
    }
    ...
    if (null != stateFromStores) {
      if (stateFromStores.length >= 1) {
        ...
        function handleOpenMutualGuilds() {
          if (null != user) {
            ...
            obj[1] = function onPressMutualGuild(arg0) {
              ...
              callback2(4676).popWithKey(closure_40);
            };
        ...
        obj10[0] = tmp7(5956).GuildIconSizes.SMALL;
        ...
        const items6 = [tmp15(tmp7(9191).GuildIconPile, obj10), ];
        ...
        items6[1] = tmp15(tmp7(4441).Text, obj11);
        obj9[3] = items6;
        tmp18Result = tmp18(tmp7(5020).PressableOpacity, obj9);
      }
    }
...
function ChatBeginningRowGroupDM(channel) {
  ...
  callback = React.useCallback(() => {
    channel(4299).navigateToNewGroupDM(channel.id, closure_1_27.CHANNEL_TEXT_AREA);
  }, items1);
  ...
  const items5 = [callback2(first(10568), obj), , , ];
  ...
  const tmp17 = channel.id === channel(6020).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID;
  obj1[1] = str;
  obj1[3] = arr;
  items5[1] = callback2(channel(4441).Text, obj1);
  ...
  obj2[3] = formatResult;
  items5[2] = callback2(channel(4441).Text, obj2);
  ...
  if (!tmp17) {
    ...
    if (stateFromStores) {
      ...
      obj4[1] = callback2;
      obj4[2] = tmp7(10035).GroupPlusIcon;
    ...
    if (channel.hasFlag(ChannelFlags.IS_JOIN_REQUEST_INTERVIEW_CHANNEL)) {
      ...
      obj6[0] = channel.id;
      tmp15Result1 = tmp15(first(12084), obj6);
    }
          ...
          try {
            ...
            if (0 === dependencyMap) {
              if (arg0 === 1) {
                ...
                let base64 = tmp2;
                base64 = undefined;
                closure_1_1(9649).init(dependencyMap);
                const obj7 = closure_1_1(9649);
                obj1 = { size: null };
                obj1[0] = closure_1_35;
                dependencyMap = 1;
                c3 = 1;
