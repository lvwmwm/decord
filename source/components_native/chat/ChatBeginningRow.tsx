...
// Dependencies: [32, 5, 19, 17, 4438, 1390, 6089, 9668, 5440, 7313, 1395, 1434, 1391, 1983, 1993, 1910, 4090, 4099, 4667, 1923, 676, 1398, 11967, 11941, 8996, 21, 4446, 712, 4985, 1236, 4987, 4992, 4994, 4880, 4442, 4159, 4166, 5033, 11130, 1297, 11968, 8693, 10264, 589, 4290, 7817, 4640, 9646, 6087, 6084, 10359, 4381, 4436, 11969, 11970, 38, 11, 11972, 4162, 9667, 5047, 11976, 6335, 9847, 6739, 1996, 12087, 12090, 8697, 4459, 4893, 12094, 4824, 12095, 5926, 9392, 698, 10177, 7805, 5452, 5447, 4163, 4413, 12097, 2010, 5918, 12098, 8061, 8836, 5946, 9171, 8505, 6151, 4689, 9209, 5969, 8998, 9014, 9010, 9008, 12099, 8311, 4300, 8595, 4267, 1377, 11126, 6033, 10586, 10053, 4389, 12103, 12107, 2]
// Exports: default

// Module 11966 (ChatBeginningRowIcon)
...
import registerAssetDefault from "registerAsset" /* 11968 */;
import ChannelAccessInfoDefault from "ChannelAccessInfo" /* 11969 */;
import registerAssetDefault2 from "registerAsset" /* 12094 */;
import PortalAccessibilityWorkaroundViewDefault from "PortalAccessibilityWorkaroundView" /* 12107 */;
...
import closure_10 from "handleLoadThreadsSuccess" /* 6089 */;
import closure_11 from "handleFormInit" /* 9668 */;
import closure_12 from "recomputeFromAppTokens" /* 5440 */;
import { FetchState } from "recomputeFromAppTokens" /* 5440 */;
import closure_14 from "createUserWidgetFromServer" /* 7313 */;
...
import closure_18 from "comparator" /* 1983 */;
import closure_19 from "trackCommunicationDisabled" /* 1993 */;
import closure_20 from "createGuildRecordFromRust" /* 1910 */;
import closure_21 from "getUncachedChannelPermissions" /* 4090 */;
import closure_22 from "markAllUserIdListsStale" /* 4099 */;
import closure_23 from "updateUserGuildSettingsInternal" /* 4667 */;
import closure_24 from "mergeGuildAvatar" /* 1923 */;
import ME from "ME" /* 676 */;
import { ChannelFlags } from "set" /* 1398 */;
import Steps from "Steps" /* 11967 */;
import { MOBILE_MESSAGE_REQUESTS_MODAL_KEY as closure_40 } from "MESSAGE_REQUEST_ACCEPT_CONFIRMATION_MODAL" /* 11941 */;
import { BLOCK_CONFIRMATION_ACTION_SHEET_KEY as closure_41 } from "IGNORE_CONFIRMATION_ACTION_SHEET_KEY" /* 8996 */;
...
function ChatBeginningRowIcon(arg0) {
  ...
  if (isForumPost) {
    obj[0] = tmp2(4985).ChatIcon;
    obj[1] = tmp2(1236).t.Y4REmB;
    let tmp5 = tmp2;
    let tmp6 = obj;
  } else if (isThread) {
    obj[0] = tmp2(4987).ThreadIcon;
    obj[1] = tmp2(1236).t["7Xm5QI"];
    tmp5 = tmp2;
    tmp6 = obj;
  } else if (isPrivate) {
    obj[0] = tmp2(4992).TextLockIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  } else {
    obj[0] = tmp2(4994).TextIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  }
...
function ChatBeginningRowHeader(arg0) {
  ...
  if (!isGameInvitesPost) {
    ...
    obj1[0] = items1;
    obj1[4] = title;
    items[1] = callback2(tmp2(4442).Text, obj1);
    obj[0] = items;
  ...
  if (tmp11Result) {
    ...
    obj3[2] = tmp.subtitle;
    obj3[5] = subtitle;
    tmp11Result = callback2(tmp2(4442).Text, obj3);
...
function LinkManageButtons(arg0) {
  ...
  if (isPrivate) {
    ...
    items1[1] = callback2(tmp2(4442).Text, obj2);
    obj[3] = items1;
    isPrivate = tmp6(tmp2(5033).PressableOpacity, obj);
  }
  ...
  if (canEdit) {
    ...
    obj4[1] = token;
    const items5 = [callback2(tmp2(10264).PencilIcon, obj4), ];
    ...
    items5[1] = callback2(tmp2(4442).Text, obj5);
    obj3[3] = items5;
    canEdit = tmp6(tmp2(5033).PressableOpacity, obj3);
  }
  items3[1] = canEdit;
  obj[1] = items3;
...
function ThreadOwner(arg0) {
  ...
  if ("username" === stateFromStores2) {
    ...
    obj[1] = tmp16;
    obj[4] = str;
    return callback2(tmp2(4442).Text, obj);
  } else {
    ...
    obj3[2] = str;
    items3[1] = callback2(tmp2(4442).Text, obj3);
    obj2[0] = items3;
...
function ChatBeginningRowThread(channel) {
  ...
  if (null != channel.threadMetadata) {
    ...
    if (!isForumPostResult) {
      ...
      tmp12Result = tmp12(tmp4(4442).Text, obj1);
    }
    items[2] = tmp12Result;
    ...
    if (channel.isForumPost()) {
      ...
      if (null == tmp8) {
        ...
        tmp12Result1 = tmp12(tmp4(4442).Text, obj3);
      }
    }
    const obj4 = { children: null };
    items[3] = tmp12Result1;
    obj4[0] = items;
    tmp10Result = closure_44(closure_43, obj4);
...
function ChatBeginningRowGuildNonDefaultChannel(channel) {
  ...
  if (result) {
    ...
    obj[1] = function topicHook() {
      let obj = closure_1_1(closure_1_2[52]);
  ...
  if (!channel.showingSpamBanner) {
    if (dMMessageToReport.isReportable) {
      ...
      obj[4] = function handleShowReportModal() {
        if (null != message) {
          const result = channel(closure_1_2[68]).showReportModalForFirstDM(tmp, () => {
            closure_1_1(closure_1_2[69]).closePrivateChannel(id.id, true);
          });
          const obj = channel(closure_1_2[68]);
        }
      };
      return callback2(tmp(4893).Button, obj);
    }
  }
  return null;
}
  ...
  if (application.user.bot) {
    ...
    if (null != application) {
      ...
      obj[4] = function onPress() {
        let obj = application(closure_1_2[72]);
        obj = {
          application,
          scopes: scopes.scopes,
          onDelete() {
            let obj = closure_1_1(closure_1_2[74]);
            obj.delete(id2.id);
            ...
            closure_1_1(closure_1_2[75]).leaveActivity(obj);
          }
        };
        obj.openAlert("confirm-delete-authed-app", closure_1_42(closure_1_1(closure_1_2[73]), obj));
        obj = { application_id: application.id, channel_id: closure_1.id, channel_type: closure_1.type };
        closure_1_1(closure_1_2[76]).track(closure_1_25.APP_MANAGE_CTA_CLICKED, obj);
      };
      tmp4 = callback2(tmp2(4893).Button, obj);
    }
  }
  return tmp4;
}
function MuteAppButton(channel) {
  ...
  if (channel.user.bot) {
    ...
    obj[1] = closure_1(stateFromStores ? tmp3[77] : tmp3[78]);
    let WHITE;
    if (!stateFromStores) {
    ...
    obj[4] = function onPress() {
      if (stateFromStores) {
        ...
        obj[2] = function icon() {
          ...
          obj = { source: closure_1_1(closure_1_2[78]), color: closure_1_1(closure_1_2[27]).unsafe_rawColors.WHITE, style: closure_1.unmutedNotification };
          obj[1] = closure_1_42(closure_1_0(closure_1_2[39]).Icon, obj);
          return closure_1_42(closure_1_7, obj);
        };
        callback(stateFromStores[81]).open(obj);
        const obj4 = callback(stateFromStores[81]);
      } else {
...
function ChatBeginningRowDM(channel) {
  ...
  const tmp = callback3();
  dependencyMap = tmp;
  analyticsLocations = user(5918)().analyticsLocations;
  let id;
  if (user != null) {
    id = user.id;
  }
  ...
  const effect = authorizedAppsFetchState.useEffect(() => {
    ...
    user(closure_2[87])(id, getAvatarURL(guild_id, 80), { withMutualGuilds: true, dispatchWait: true });
  }, items3);
  let bot;
  if (user != null) {
    bot = user.bot;
  ...
  const effect1 = authorizedAppsFetchState.useEffect(() => {
    ...
    if (bot) {
      const response = user(closure_2[74]).fetch();
      const obj = user(closure_2[74]);
    }
  }, items4);
  if (null == user) {
    return null;
    ...
    function handleCopyUserTag() {
      channel(closure_2[89]).copy(userTag);
      const obj = channel(closure_2[89]);
      const result = channel(closure_2[58]).presentUsernameCopied();
    }
    ...
    obj1[1] = function handleOpenProfile() {
      user(closure_2[88])({ userId: user.id, channelId: channel.id, sourceAnalyticsLocations: analyticsLocations });
    };
    ...
    const items5 = [callback2(tmp7(5033).PressableOpacity, obj1), , , , , , ];
    ...
    obj4[2] = name;
    obj3[3] = callback2(tmp7(4442).Text, obj4);
    items5[1] = callback2(tmp7(5033).PressableOpacity, obj3);
    let tmp15Result = null;
    if (!user.isProvisional) {
      ...
      obj6[2] = userTag;
      obj5[3] = tmp15(tmp7(4442).Text, obj6);
      tmp15Result = tmp15(tmp7(5033).PressableOpacity, obj5);
    }
    ...
    obj7[3] = stringResult;
    items5[3] = callback2(tmp7(4442).Text, obj7);
    tmp15Result = null;
    if (user.isProvisional) {
      let obj8 = { style: null, userId: null, iconSize: 14 };
      obj8[0] = tmp.provisionalAccountExplainer;
      obj8[1] = user.id;
      tmp15Result = tmp15(tmp7(12099).ChatProvisionalAccountExplainerCard, obj8);
    }
    ...
    if (null != stateFromStores) {
      if (stateFromStores.length >= 1) {
        ...
        function handleOpenMutualGuilds() {
          if (null != user) {
            ...
            obj[1] = function onPressMutualGuild(arg0) {
              ...
              callback2(4689).popWithKey(closure_40);
            };
            obj.openLazy(channel(closure_2[84])(closure_2[90], closure_2.paths), "MutualGuildsActionSheet", obj);
          }
        }
        ...
        obj10[0] = tmp7(5969).GuildIconSizes.SMALL;
        obj10[1] = substr.map((guild) => guild.guild.name);
        obj10[2] = stateFromStores.length;
        obj10[3] = substr.map((guild) => {
          guild = guild.guild;
          const obj = { guild, size: channel(closure_2[95]).GuildIconSizes.SMALL };
          return callback(user(closure_2[95]), obj, guild.id);
        });
        const items6 = [tmp15(tmp7(9209).GuildIconPile, obj10), ];
        ...
        items6[1] = tmp15(tmp7(4442).Text, obj11);
        obj9[3] = items6;
        tmp18Result = tmp18(tmp7(5033).PressableOpacity, obj9);
      }
    }
    ...
    if (user.isNonUserBot()) {
      ...
      tmp18Result1 = tmp15(function RelationshipButtons(reportButton) {
        ...
        function handleBlock() {
          let obj = closure_1_1(closure_1_2[82]);
          obj = { userId: user.id, channelId: id.id };
          obj.openLazy(closure_1_0(closure_1_2[84])(closure_1_2[99], closure_1_2.paths), closure_1_41, obj);
        }
        ...
        const Button = channel(closure_2[70]).Button;
        ...
        if (stateFromStores === closure_1_34.PENDING_INCOMING) {
          ...
          obj4[3] = function handleAcceptFriend() {
            let obj = closure_1_1(closure_1_2[97]);
          ...
          if (tmp6.NONE === stateFromStores) {
            ...
            function handleRemoveFriend() {
              let obj = closure_1_0(closure_1_2[98]);
              obj = {
...
function ChatBeginningRowGroupDM(channel) {
  ...
  callback = React.useCallback(() => {
    channel(4300).navigateToNewGroupDM(channel.id, closure_1_27.CHANNEL_TEXT_AREA);
  }, items1);
  ...
  const items5 = [callback2(first(10586), obj), , , ];
  ...
  const tmp17 = channel.id === channel(6033).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID;
  obj1[1] = str;
  obj1[3] = arr;
  items5[1] = callback2(channel(4442).Text, obj1);
  ...
  obj2[3] = formatResult;
  items5[2] = callback2(channel(4442).Text, obj2);
  ...
  if (!tmp17) {
    ...
    if (stateFromStores) {
      ...
      obj4[1] = callback2;
      obj4[2] = tmp7(10053).GroupPlusIcon;
    ...
    if (channel.hasFlag(ChannelFlags.IS_JOIN_REQUEST_INTERVIEW_CHANNEL)) {
      ...
      obj6[0] = channel.id;
      tmp15Result1 = tmp15(first(12103), obj6);
    }
          ...
          try {
            ...
            if (0 === dependencyMap) {
              if (arg0 === 1) {
                ...
                let base64 = tmp2;
                base64 = undefined;
                closure_1_1(9667).init(dependencyMap);
                const obj7 = closure_1_1(9667);
                obj1 = { size: null };
                obj1[0] = closure_1_35;
                dependencyMap = 1;
                c3 = 1;
  ...
  if (canInvite) {
    ...
    obj1[0] = function inviteFriends() {
      if (null != guild.vanityURLCode) {
        const result = guild(id[63]).showVanityUrlInviteActionSheet(tmp, channel, closure_1_32.WELCOME_MESSAGE);
