...
// Dependencies: [32, 5, 19, 17, 4372, 1390, 6002, 9169, 5356, 7227, 1395, 1434, 1391, 1982, 1992, 1910, 4025, 4034, 4589, 1922, 676, 1398, 12029, 12003, 10433, 21, 4380, 712, 4902, 1236, 4904, 4909, 4911, 4802, 4376, 4094, 4101, 4949, 11159, 1297, 12030, 8167, 9822, 589, 4224, 8222, 4574, 9144, 6000, 10153, 4315, 4370, 12031, 12032, 38, 11, 12034, 4097, 9168, 4963, 12038, 6250, 9212, 6654, 1995, 12147, 12150, 8171, 4393, 4815, 12154, 4746, 12155, 5842, 10686, 698, 9888, 8209, 5368, 5363, 4098, 4347, 12157, 2009, 5834, 12158, 8463, 8930, 5862, 12159, 8936, 6066, 4611, 12176, 5885, 9391, 9782, 12178, 10434, 12185, 8121, 4234, 7903, 4201, 1377, 11153, 5949, 9813, 9872, 4323, 12189, 12193, 2]
// Exports: default

// Module 12028 (ChatBeginningRowIcon)
...
import registerAssetDefault from "registerAsset" /* 12030 */;
import ChannelAccessInfoDefault from "ChannelAccessInfo" /* 12031 */;
import registerAssetDefault2 from "registerAsset" /* 12154 */;
import PortalAccessibilityWorkaroundViewDefault from "PortalAccessibilityWorkaroundView" /* 12193 */;
...
import closure_10 from "handleLoadThreadsSuccess" /* 6002 */;
import closure_11 from "handleFormInit" /* 9169 */;
import closure_12 from "recomputeFromAppTokens" /* 5356 */;
import { FetchState } from "recomputeFromAppTokens" /* 5356 */;
import closure_14 from "createUserWidgetFromServer" /* 7227 */;
...
import closure_21 from "getUncachedChannelPermissions" /* 4025 */;
import closure_22 from "markAllUserIdListsStale" /* 4034 */;
...
import { MOBILE_MESSAGE_REQUESTS_MODAL_KEY as closure_40 } from "MESSAGE_REQUEST_ACCEPT_CONFIRMATION_MODAL" /* 12003 */;
import { BLOCK_CONFIRMATION_ACTION_SHEET_KEY as closure_41 } from "IGNORE_CONFIRMATION_ACTION_SHEET_KEY" /* 10433 */;
...
function ChatBeginningRowIcon(arg0) {
  ...
  if (isForumPost) {
    obj[0] = tmp2(4902).ChatIcon;
    obj[1] = tmp2(1236).t.Y4REmB;
    let tmp5 = tmp2;
    let tmp6 = obj;
  } else if (isThread) {
    obj[0] = tmp2(4904).ThreadIcon;
    obj[1] = tmp2(1236).t["7Xm5QI"];
    tmp5 = tmp2;
    tmp6 = obj;
  } else if (isPrivate) {
    obj[0] = tmp2(4909).TextLockIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  } else {
    obj[0] = tmp2(4911).TextIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  }
...
function ChatBeginningRowHeader(arg0) {
  ...
  children[1] = callback2(subtitleLink(4376).Text, { style: items1, accessibilityRole: "header", variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", children: title });
...
function LinkManageButtons(arg0) {
  ...
  if (isPrivate) {
    ...
    items1[1] = callback2(tmp2(4376).Text, obj2);
    obj[3] = items1;
    isPrivate = tmp6(tmp2(4949).PressableOpacity, obj);
  }
  ...
  if (canEdit) {
    ...
    obj4[1] = token;
    const items5 = [callback2(tmp2(9822).PencilIcon, obj4), ];
    ...
    items5[1] = callback2(tmp2(4376).Text, obj5);
    obj3[3] = items5;
    canEdit = tmp6(tmp2(4949).PressableOpacity, obj3);
  }
  items3[1] = canEdit;
  obj[1] = items3;
...
function ThreadOwner(arg0) {
  ...
  if ("username" === stateFromStores2) {
    ...
    obj[1] = tmp16;
    obj[4] = str;
    return callback2(tmp2(4376).Text, obj);
  } else {
    ...
    obj3[2] = str;
    items3[1] = callback2(tmp2(4376).Text, obj3);
    obj2[0] = items3;
...
function ChatBeginningRowThread(channel) {
  ...
  if (null != channel.threadMetadata) {
    ...
    if (!isForumPostResult) {
      ...
      tmp11Result = tmp11(tmp4(4376).Text, obj1);
    }
    items[2] = tmp11Result;
    ...
    if (channel.isForumPost()) {
      ...
      if (null == tmp7) {
        ...
        tmp11Result1 = tmp11(tmp4(4376).Text, obj3);
      }
    }
...
function DMSpamButton(channel) {
  ...
  if (!channel.showingSpamBanner) {
    if (dMMessageToReport.isReportable) {
      ...
      return callback2(tmp(4815).Button, obj);
    }
  }
  return null;
}
function ManageAppButton(application) {
  ...
  if (application.user.bot) {
    ...
    if (null != application) {
      ...
      tmp4 = callback2(tmp2(4815).Button, obj);
    }
  }
  return tmp4;
}
...
function ChatBeginningRowDM(channel) {
  ...
  const tmp = callback3();
  dependencyMap = tmp;
  analyticsLocations = user(5834)().analyticsLocations;
  let id;
  if (user != null) {
    id = user.id;
  }
  ...
  if (null == user) {
    ...
    const items5 = [callback2(tmp7(4949).PressableOpacity, obj1), , , , , , ];
    ...
    obj4[2] = name;
    obj3[3] = callback2(tmp7(4376).Text, obj4);
    items5[1] = callback2(tmp7(4949).PressableOpacity, obj3);
    let tmp15Result = null;
    if (!user.isProvisional) {
      ...
      obj6[2] = userTag;
      obj5[3] = tmp15(tmp7(4376).Text, obj6);
      tmp15Result = tmp15(tmp7(4949).PressableOpacity, obj5);
    }
    ...
    obj7[3] = stringResult;
    items5[3] = callback2(tmp7(4376).Text, obj7);
    tmp15Result = null;
    if (user.isProvisional) {
      let obj8 = { style: null, userId: null, iconSize: 14 };
      obj8[0] = tmp.provisionalAccountExplainer;
      obj8[1] = user.id;
      tmp15Result = tmp15(tmp7(12185).ChatProvisionalAccountExplainerCard, obj8);
    }
    ...
    if (null != stateFromStores) {
      if (stateFromStores.length >= 1) {
        ...
        function handleOpenMutualGuilds() {
          if (null != user) {
            ...
            obj[1] = function onPressMutualGuild(arg0) {
              ...
              callback2(4611).popWithKey(closure_40);
            };
        ...
        obj10[0] = tmp7(5885).GuildIconSizes.SMALL;
        ...
        const items6 = [tmp15(tmp7(12176).GuildIconPile, obj10), ];
        ...
        items6[1] = tmp15(tmp7(4376).Text, obj11);
        obj9[3] = items6;
        tmp18Result = tmp18(tmp7(4949).PressableOpacity, obj9);
      }
    }
...
function ChatBeginningRowGroupDM(channel) {
  ...
  callback = React.useCallback(() => {
    channel(4234).navigateToNewGroupDM(channel.id, closure_1_27.CHANNEL_TEXT_AREA);
  }, items1);
  ...
  const items5 = [callback2(first(9813), obj), , , ];
  ...
  const tmp17 = channel.id === channel(5949).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID;
  obj1[1] = str;
  obj1[3] = arr;
  items5[1] = callback2(channel(4376).Text, obj1);
  ...
  obj2[3] = formatResult;
  items5[2] = callback2(channel(4376).Text, obj2);
  ...
  if (!tmp17) {
    ...
    if (stateFromStores) {
      ...
      obj4[1] = callback2;
      obj4[2] = tmp7(9872).GroupPlusIcon;
    ...
    if (channel.hasFlag(ChannelFlags.IS_JOIN_REQUEST_INTERVIEW_CHANNEL)) {
      ...
      obj6[0] = channel.id;
      tmp15Result1 = tmp15(first(12189), obj6);
    }
          ...
          try {
            ...
            if (0 === dependencyMap) {
              if (arg0 === 1) {
                ...
                let base64 = tmp2;
                base64 = undefined;
                closure_1_1(9168).init(dependencyMap);
                const obj7 = closure_1_1(9168);
                obj1 = { size: null };
                obj1[0] = closure_1_35;
                dependencyMap = 1;
                c3 = 1;
