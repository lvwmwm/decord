...
// Dependencies: [32, 5, 19, 17, 4334, 1390, 5957, 9085, 5311, 7150, 1395, 1434, 1391, 1980, 1990, 1910, 3989, 3998, 4550, 1922, 676, 1398, 11909, 11883, 10338, 21, 4342, 712, 4863, 1236, 4865, 4870, 4872, 4763, 4338, 4058, 4065, 4909, 11062, 1297, 11910, 8082, 9732, 589, 4187, 8136, 4535, 9060, 5955, 10058, 4278, 4332, 11911, 11912, 38, 11, 11914, 4061, 9084, 4923, 11918, 6201, 9128, 6573, 1993, 12027, 12030, 8086, 4355, 4777, 12034, 4707, 12035, 5797, 10591, 698, 9793, 8124, 5323, 5318, 4062, 4310, 12037, 2007, 5789, 12038, 8378, 8846, 5817, 12039, 8852, 6021, 4572, 12056, 5840, 9307, 9692, 12058, 10339, 12065, 8036, 4197, 7824, 4164, 1377, 11056, 5904, 9723, 9782, 4286, 12069, 12073, 2]
// Exports: default

// Module 11908 (ChatBeginningRowIcon)
...
function ChatBeginningRowIcon(arg0) {
  ...
  if (isForumPost) {
    obj[0] = tmp2(4863).ChatIcon;
    obj[1] = tmp2(1236).t.Y4REmB;
    let tmp5 = tmp2;
    let tmp6 = obj;
  } else if (isThread) {
    obj[0] = tmp2(4865).ThreadIcon;
    obj[1] = tmp2(1236).t["7Xm5QI"];
    tmp5 = tmp2;
    tmp6 = obj;
  } else if (isPrivate) {
    obj[0] = tmp2(4870).TextLockIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  } else {
    obj[0] = tmp2(4872).TextIcon;
    obj[1] = tmp2(1236).t.GK18KJ;
    tmp5 = tmp2;
    tmp6 = obj;
  }
...
function ChatBeginningRowHeader(arg0) {
  ...
  children[1] = callback2(subtitleLink(4338).Text, { style: items1, accessibilityRole: "header", variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", children: title });
...
function LinkManageButtons(arg0) {
  ...
  if (isPrivate) {
    ...
    items1[1] = callback2(tmp2(4338).Text, obj2);
    obj[3] = items1;
    isPrivate = tmp6(tmp2(4909).PressableOpacity, obj);
  }
  ...
  if (canEdit) {
    ...
    obj4[1] = token;
    const items5 = [callback2(tmp2(9732).PencilIcon, obj4), ];
    ...
    items5[1] = callback2(tmp2(4338).Text, obj5);
    obj3[3] = items5;
    canEdit = tmp6(tmp2(4909).PressableOpacity, obj3);
  }
  items3[1] = canEdit;
  obj[1] = items3;
...
function ThreadOwner(arg0) {
  ...
  if ("username" === stateFromStores2) {
    ...
    obj[1] = tmp16;
    obj[4] = str;
    return callback2(tmp2(4338).Text, obj);
  } else {
    ...
    obj3[2] = str;
    items3[1] = callback2(tmp2(4338).Text, obj3);
    obj2[0] = items3;
...
function ChatBeginningRowThread(channel) {
  ...
  if (null != channel.threadMetadata) {
    ...
    if (!isForumPostResult) {
      ...
      tmp11Result = tmp11(tmp4(4338).Text, obj1);
    }
    items[2] = tmp11Result;
    ...
    if (channel.isForumPost()) {
      ...
      if (null == tmp7) {
        ...
        tmp11Result1 = tmp11(tmp4(4338).Text, obj3);
      }
    }
...
function ChatBeginningRowGuildNonDefaultChannel(channel) {
  ...
  const tmp10 = importDefault(4278)();
  ...
  if (result) {
    ...
    if (canResult) {
      ...
      tmp15Result = callback2(tmp(11911), obj2);
    }
  }
  ...
  if (!channel.showingSpamBanner) {
    if (dMMessageToReport.isReportable) {
      ...
      obj[4] = function handleShowReportModal() {
        if (null != message) {
          const result = channel(outer1_2[67]).showReportModalForFirstDM(tmp, () => {
            outer1_1(outer1_2[68]).closePrivateChannel(id.id, true);
          });
          const obj = channel(outer1_2[67]);
        }
      };
      return callback2(tmp(4777).Button, obj);
    }
  }
  return null;
}
function ManageAppButton(application) {
  ...
  if (application.user.bot) {
    ...
    if (null != application) {
      ...
      obj[1] = importDefault(12034);
      obj[2] = tmp.appDMButtonIcon;
      obj[3] = callback2(tmp2(1297).Icon, obj);
      obj[4] = function onPress() {
        let obj = application(outer1_2[71]);
        obj = {
          application,
          scopes: scopes.scopes,
          onDelete() {
            let obj = outer1_1(outer1_2[73]);
            obj.delete(id2.id);
            ...
            outer1_1(outer1_2[74]).leaveActivity(obj);
          }
        };
        obj.openAlert("confirm-delete-authed-app", outer1_42(outer1_1(outer1_2[72]), obj));
        obj = { application_id: application.id, channel_id: closure_1.id, channel_type: closure_1.type };
        outer1_1(outer1_2[75]).track(outer1_25.APP_MANAGE_CTA_CLICKED, obj);
      };
      tmp4 = callback2(tmp2(4777).Button, obj);
    }
  }
  return tmp4;
}
function MuteAppButton(channel) {
  ...
  if (channel.user.bot) {
    ...
    obj[1] = c1(stateFromStores ? tmp3[76] : tmp3[77]);
    let WHITE;
    if (!stateFromStores) {
    ...
    obj[4] = function onPress() {
      if (stateFromStores) {
        ...
        _undefined(stateFromStores[80]).open(obj);
        const obj4 = _undefined(stateFromStores[80]);
      } else {
...
function ChatBeginningRowDM(channel) {
  ...
  const tmp = createCacheKey();
  dependencyMap = tmp;
  analyticsLocations = user(5789)().analyticsLocations;
  let id;
  if (user != null) {
    id = user.id;
  }
  ...
  const effect = authorizedAppsFetchState.useEffect(() => {
    ...
    user(_undefined[86])(id, getAvatarURL(guild_id, 80), { withMutualGuilds: true, dispatchWait: true });
  }, items3);
  let bot;
  if (user != null) {
    bot = user.bot;
  ...
  const effect1 = authorizedAppsFetchState.useEffect(() => {
    ...
    if (bot) {
      const response = user(_undefined[73]).fetch();
      const obj = user(_undefined[73]);
    }
  }, items4);
  if (null == user) {
    return null;
    ...
    function handleCopyUserTag() {
      channel(_undefined[88]).copy(userTag);
      const obj = channel(_undefined[88]);
      const result = channel(_undefined[57]).presentUsernameCopied();
    }
    ...
    obj1[1] = function handleOpenProfile() {
      user(_undefined[87])({ userId: user.id, channelId: channel.id, sourceAnalyticsLocations: analyticsLocations });
    };
    ...
    const items5 = [callback2(tmp7(4909).PressableOpacity, obj1), , , , , , ];
    ...
    obj4[2] = name;
    obj3[3] = callback2(tmp7(4338).Text, obj4);
    items5[1] = callback2(tmp7(4909).PressableOpacity, obj3);
    let tmp15Result = null;
    if (!user.isProvisional) {
      ...
      obj6[2] = userTag;
      obj5[3] = tmp15(tmp7(4338).Text, obj6);
      tmp15Result = tmp15(tmp7(4909).PressableOpacity, obj5);
    }
    ...
    obj7[3] = stringResult;
    items5[3] = callback2(tmp7(4338).Text, obj7);
    tmp15Result = null;
    if (user.isProvisional) {
      let obj8 = { style: null, userId: null, iconSize: 14 };
      obj8[0] = tmp.provisionalAccountExplainer;
      obj8[1] = user.id;
      tmp15Result = tmp15(tmp7(12065).ChatProvisionalAccountExplainerCard, obj8);
    }
    ...
    if (null != stateFromStores) {
      if (stateFromStores.length >= 1) {
        ...
        function handleOpenMutualGuilds() {
          if (null != user) {
            ...
            obj[1] = function onPressMutualGuild(arg0) {
              ...
              callback2(4572).popWithKey(closure_40);
            };
            obj.openLazy(channel(_undefined[83])(_undefined[89], _undefined.paths), "MutualGuildsActionSheet", obj);
          }
        }
        ...
        obj10[0] = tmp7(5840).GuildIconSizes.SMALL;
        ...
        const items6 = [tmp15(tmp7(12056).GuildIconPile, obj10), ];
        ...
        items6[1] = tmp15(tmp7(4338).Text, obj11);
        obj9[3] = items6;
        tmp18Result = tmp18(tmp7(4909).PressableOpacity, obj9);
      }
    }
    ...
    if (user.isNonUserBot()) {
      ...
      tmp18Result1 = tmp15(function RelationshipButtons(reportButton) {
        ...
        function handleBlock() {
          let obj = outer1_1(outer1_2[81]);
          obj = { userId: user.id, channelId: id.id };
          obj.openLazy(outer1_0(outer1_2[83])(outer1_2[98], outer1_2.paths), outer1_41, obj);
        }
        ...
        const Button = channel(_undefined[69]).Button;
...
function ChatBeginningRowButton(style) {
  ...
  obj[1] = callback2(require(8036) /* RowButtonWrapper */.RowButton.Icon, { IconComponent, variant: iconVariant });
  obj[2] = title;
  obj[3] = subtitle;
  obj[4] = trailing;
  obj[1] = callback2(require(8036) /* RowButtonWrapper */.RowButton, obj);
  return callback2(closure_7, obj);
}
function ChatBeginningRowGroupDM(channel) {
  ...
  callback = React.useCallback(() => {
    channel(_undefined[101]).navigateToNewGroupDM(channel.id, outer1_27.CHANNEL_TEXT_AREA);
  }, items1);
  ...
  const callback2 = React.useCallback(() => {
    let obj = channel(_undefined[103]);
    if (obj.UNSAFE_isDismissibleContentDismissed(channel(_undefined[104]).DismissibleContent.GDM_INVITE_REMINDER)) {
      callback();
    } else {
      obj = { onClick: null };
      obj[0] = callback;
      first(_undefined[105])(obj);
    }
  }, items3);
  ...
  const items5 = [callback2(first(9723), obj), , , ];
  ...
  const tmp17 = channel.id === channel(5904).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID;
  obj1[1] = str;
  obj1[3] = arr;
  items5[1] = callback2(channel(4338).Text, obj1);
  ...
  obj2[3] = formatResult;
  items5[2] = callback2(channel(4338).Text, obj2);
  ...
  if (!tmp17) {
    ...
    if (stateFromStores) {
      ...
      obj4[1] = callback2;
      obj4[2] = tmp7(9782).GroupPlusIcon;
    ...
    if (channel.hasFlag(ChannelFlags.IS_JOIN_REQUEST_INTERVIEW_CHANNEL)) {
      ...
      obj6[0] = channel.id;
      tmp15Result1 = tmp15(first(12069), obj6);
    }
          ...
          try {
            ...
            if (0 === dependencyMap) {
              if (arg0 === 1) {
                ...
                let base64 = tmp2;
                base64 = undefined;
                outer1_1(9084).init(dependencyMap);
                const obj7 = outer1_1(9084);
                let obj1 = { size: null };
                obj1[0] = outer1_35;
                dependencyMap = 1;
                c3 = 1;
  ...
  if (canInvite) {
    ...
    obj1[0] = function inviteFriends() {
      if (null != guild.vanityURLCode) {
        const result = guild(id[62]).showVanityUrlInviteActionSheet(tmp, channel, outer1_32.WELCOME_MESSAGE);
...
export default function ChatBeginningRow(channelId) {
  ...
  if (!shouldRender) {
    ...
    obj[1] = tmp19Result;
    return callback2(importDefault(12073), obj);
