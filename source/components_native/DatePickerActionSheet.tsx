...
// Dependencies: [32, 19, 17, 21, 4285, 712, 4253, 500, 5376, 5686, 8899, 1236, 4691, 4145, 1297, 4286, 4281, 4696, 4221, 8414, 3866, 5377, 8900, 4130, 2]
// Exports: default

// Module 8898 (ActionSheetHeader)
...
function ActionSheetHeader(handleSubmit) {
  ...
  const BottomSheetTitleHeader = require(5376) /* RedesignBottomSheetTitleHeaderBase */.BottomSheetTitleHeader;
  if (isAndroidResult) {
    obj = { title: null, trailing: null };
    obj[0] = title;
    obj = { onPress: null };
    obj[0] = handleCancel;
    obj[1] = tmp4(tmp(5686).ActionSheetCloseButton, obj);
    ...
    obj1[1] = tmp4(tmp(8899).ActionSheetHeaderPressableText, obj2);
    const obj3 = { onPress: null, label: null };
    obj3[0] = handleSubmit.handleSubmit;
    const intl2 = tmp(1236).intl;
    obj3[1] = intl2.string(tmp(1236).t["R3BPH+"]);
    obj1[2] = tmp4(tmp(8899).ActionSheetHeaderPressableText, obj3);
...
function DateRangeError(children) {
  ...
  obj = { STANDARD_EASING: show(1297).STANDARD_EASING, show, withTiming: show(4286).withTiming };
  A.__closure = obj;
  ...
  obj[2] = str;
  obj[3] = closure_6(View, { ref, accessible: true, accessibilityRole: "alert", style: tmp.rangeError, children: closure_6(show(4281).Text, { variant: "text-md/medium", color: "text-overlay-light", children: children.errorText }) });
  return closure_6(ref(4145).View, obj);
}
function ActionSheetFooter(arg0) {
  ...
  if (obj.isAndroid()) {
    ...
    obj[3] = callback(tmp2(4281).Text, obj1);
    const intl2 = tmp2(1236).intl;
    obj[4] = intl2.string(tmp2(1236).t["ETE/oC"]);
    obj[5] = tmp.actionButton;
    obj[6] = handleCancel;
    const items = [callback(tmp2(4696).BaseTextButton, obj), ];
    ...
    obj2[4] = callback(tmp2(4281).Text, obj3);
    const intl4 = tmp2(1236).intl;
    obj2[5] = intl4.string(tmp2(1236).t["cY+Oob"]);
    obj2[6] = tmp.actionButton;
    obj2[7] = handleSubmit;
    items[1] = callback(tmp2(4696).BaseTextButton, obj2);
    obj[1] = items;
...
export default function DatePickerActionSheet(mode) {
  ...
  const tmp27 = minimumDate(8414)((getTime) => {
    if (null != getTime) {
  ...
  const tmp28 = first1;
  obj1[0] = minimumDate(3866)(minimumDate).format("L");
  ...
  const obj6 = minimumDate(3866)(minimumDate);
  obj3[0] = minimumDate(3866)(maximumDate).format("L");
  ...
  const obj9 = minimumDate(3866)(maximumDate);
  const tmp37 = first;
  const tmp13Result = minimumDate(8900);
  let str2 = "dark";
  ...
  obj[2] = items1;
  return tmp28(maximumDate(5377).BottomSheet, obj);
};
