...
// Dependencies: [32, 19, 17, 21, 4285, 712, 4253, 500, 5337, 5647, 8898, 1236, 4691, 4146, 1297, 4286, 4281, 4696, 4221, 8302, 3867, 5338, 8899, 4131, 2]
// Exports: default

// Module 8897 (ActionSheetHeader)
...
function ActionSheetHeader(handleSubmit) {
  ...
  const BottomSheetTitleHeader = require(5337) /* RedesignBottomSheetTitleHeaderBase */.BottomSheetTitleHeader;
  if (isAndroidResult) {
    obj = { title: null, trailing: null };
    obj[0] = title;
    obj = { onPress: null };
    obj[0] = handleCancel;
    obj[1] = tmp4(tmp(5647).ActionSheetCloseButton, obj);
    ...
    obj1[1] = tmp4(tmp(8898).ActionSheetHeaderPressableText, obj2);
    const obj3 = { onPress: null, label: null };
    obj3[0] = handleSubmit.handleSubmit;
    const intl2 = tmp(1236).intl;
    obj3[1] = intl2.string(tmp(1236).t["R3BPH+"]);
    obj1[2] = tmp4(tmp(8898).ActionSheetHeaderPressableText, obj3);
...
function DateRangeError(children) {
  ...
  return closure_6(ref(4146).View, obj);
}
...
export default function DatePickerActionSheet(mode) {
  ...
  const tmp27 = minimumDate(8302)((getTime) => {
    if (null != getTime) {
  ...
  const tmp28 = first1;
  obj1[0] = minimumDate(3867)(minimumDate).format("L");
  ...
  const obj6 = minimumDate(3867)(minimumDate);
  obj3[0] = minimumDate(3867)(maximumDate).format("L");
  ...
  const obj9 = minimumDate(3867)(maximumDate);
  const tmp37 = first;
  const tmp13Result = minimumDate(8899);
  let str2 = "dark";
  ...
  obj[2] = items1;
  return tmp28(maximumDate(5338).BottomSheet, obj);
};
