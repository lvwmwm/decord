...
// Function ID: 7555
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 7546, 7527, 7499, 7553, 7510, 7506, 7522, 7523, 7555, 7542, 7543, 7526, 7548, 7535, 7556, 7539, 7557, 7504]

// Module 7554 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(7527).DEBUG_BUILD) {
          const logger = tmp(7499).logger;
        ...
        BaseClient(7522).updateSession(release, { init: false });
        const obj = BaseClient(7522);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(7555).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(7555).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(7555);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(7542).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(7527).DEBUG_BUILD) {
        let logger = tmp2(7499).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(7523).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(7555).setupIntegrations(this, integrations);
      const obj = BaseClient(7555);
      const result = BaseClient(7555).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(7539).parseSampleRate(sampleRate);
        let obj = str(7539);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(7556).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(7523).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(7527).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(7499).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(7557);
