...
// Dependencies: [77, 57, 65, 12016, 12074]
...
export const toJSONSchema = function toJSONSchema(_idmap, uri) {
  ...
  const initializeContextResult = require(12074) /* initializeContext */.initializeContext(Object.assign({}, uri, obj));
  if (tmp) {
    obj = {};
    ...
    while (tmp10 !== undefined) {
      ...
      let processResult = require(12074) /* initializeContext */.process(tmp14[1], tmp7);
      continue;
    }
    obj = {};
    ...
    for (const item10069 of entries1) {
      ...
      let extractDefsResult = require(12074) /* initializeContext */.extractDefs(tmp7, tmp27);
      let tmp32 = obj;
      obj[tmp26[0]] = require(12074) /* initializeContext */.finalize(tmp7, tmp27);
      continue;
    }
    ...
    tmp2(12074).process(_idmap, initializeContextResult);
    tmp2(12074).extractDefs(initializeContextResult, _idmap);
    return tmp2(12074).finalize(initializeContextResult, _idmap);
  }
...
export const enumProcessor = function enumProcessor(closure_0, arg1, arg2, arg3) {
  const enumValues = require(12016) /* cached */.getEnumValues(closure_0._zod.def.entries);
...
export const arrayProcessor = function arrayProcessor(closure_0, arg1, arg2, path) {
  ...
  arg2.items = require(12074) /* initializeContext */.process(closure_0._zod.def.element, arg1, Object.assign({}, path, obj));
};
export const objectProcessor = function objectProcessor(closure_0, io, properties, path) {
  ...
  for (const key10014 in shape) {
    ...
    arg2.properties[key10014] = _require(12074).process(shape[key10014], arg1, Object.assign({}, arg3, obj));
    continue;
  }
  ...
  if ("never" === type) {
    ...
    if (def.catchall) {
      ...
      properties.additionalProperties = _require(12074).process(def.catchall, io, Object.assign({}, path, obj));
    }
...
export const intersectionProcessor = function intersectionProcessor(closure_0, arg1, arg2, path) {
  ...
  const processResult = require(12074) /* initializeContext */.process(def.left, arg1, Object.assign({}, path, obj));
  obj = {};
  const items1 = [];
  obj.path = items1.concat(_toConsumableArray(path.path), ["allOf", 1]);
  const processResult1 = require(12074) /* initializeContext */.process(def.right, arg1, Object.assign({}, path, obj));
...
export const recordProcessor = function recordProcessor(closure_0, target, arg2, path) {
  ...
  if ("loose" === def.mode) {
    if (patterns) {
      if (patterns.size > 0) {
        ...
        const processResult = require(12074) /* initializeContext */.process(def.valueType, target, Object.assign({}, path, obj));
  ...
  if (!tmp2) {
    ...
    tmp.propertyNames = require(12074) /* initializeContext */.process(def.keyType, target, Object.assign({}, path, obj));
  }
  obj = {};
  const items2 = [];
  obj.path = items2.concat(_toConsumableArray(path.path), ["additionalProperties"]);
  arg2.additionalProperties = require(12074) /* initializeContext */.process(def.valueType, target, Object.assign({}, path, obj));
};
...
export const nonoptionalProcessor = function nonoptionalProcessor(closure_0, seen, arg2, arg3) {
  const def = closure_0._zod.def;
  require(12074) /* initializeContext */.process(def.innerType, seen, arg3);
  seen = seen.seen;
...
export const pipeProcessor = function pipeProcessor(closure_0, io, arg2, arg3) {
  ...
  if ("input" !== io.io) {
    const out = def.out;
    require(12074) /* initializeContext */.process(out, io, arg3);
    const seen = io.seen;
...
export const lazyProcessor = function lazyProcessor(closure_0, seen, arg2, arg3) {
  ...
  require(12074) /* initializeContext */.process(innerType, seen, arg3);
  seen = seen.seen;
