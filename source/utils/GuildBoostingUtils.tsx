...
// Dependencies: [1862, 1874, 4082, 3841, 676, 1877, 1876, 3849, 4083, 1236, 1351, 4084, 1945, 12, 3771, 4085, 2805, 3835, 1208, 4096, 4115, 2]
...
// Module 4081 (getGuildTierFromGuild)
  ...
  if (null != guild) {
    ...
    if (features.has(closure_11.MORE_STICKERS)) {
      if (premiumTier === BoostedGuildTiers.TIER_3) {
        let MAX_STICKER_SLOTS = require(4083) /* PremiumGuildOverrides */.PremiumGuildOverrides.MAX_STICKER_SLOTS;
      }
...
export const getAppliedGuildBoostMonths = function getAppliedGuildBoostMonths(arg0) {
  let num = importDefault(3771)().diff(importDefault(3771)(arg0), "months");
  if (num == null) {
    num = 1;
  }
  return num;
...
export const generateBlockGuildSubscriptionPurchasesNode = function generateBlockGuildSubscriptionPurchasesNode(fractionalState) {
  ...
  if (!tmp3) {
    ...
    const obj2 = require(4085) /* _fetchAppliedGuildBoostsForGuild */;
  }
  ...
  if (isPremiumGroupMemberResult) {
    ...
    obj[0] = callback();
    return intl7.formatToPlainString(tmp7(2805)["5xN/C1"], obj);
  } else {
    ...
    if (null != premiumTypeSubscription) {
      if (reduced.numAvailableGuildBoostSlots <= 0) {
        if (premiumTypeSubscription.status === constants2.PAST_DUE) {
          ...
          const numPremiumGuildSubscriptions = importAll(3835).getNumPremiumGuildSubscriptions(premiumTypeSubscription.renewalMutations.additionalPlans);
          const obj5 = importAll(3835);
...
export const getNumberOfAppliedBoostsNeededForTier = function getNumberOfAppliedBoostsNeededForTier(id) {
  return Math.max(0, AppliedGuildBoostsRequiredForBoostedGuildTier[arg1] - require(4096) /* useGuildAppliedBoostCount */.getGuildPowerupsBoostCount(id.id).available);
};
...
export const getGuildBoostingProgressBarFillFactor = function getGuildBoostingProgressBarFillFactor(guild) {
  totalAvailableBoostsCount = totalAvailableBoostsCount(4115).getGuildPowerupBoostLevelProgress(guild.id);
  let NONE = reversed.find((arg0) => totalAvailableBoostsCount >= outer1_9[arg0]);
  if (NONE == null) {
    NONE = BoostedGuildTiers.NONE;
  }
