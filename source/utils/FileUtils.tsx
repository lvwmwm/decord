...
// Dependencies: [1862, 1874, 676, 1876, 12, 4728, 4116, 3835, 4084, 1236, 2]
...
// Module 4727 (getUploadFileSizeSum)
...
export const sizeString = function sizeString(createGuildRecordFromRust) {
  return importDefault(4728).filesize(createGuildRecordFromRust);
};
export const maxFileSize = function maxFileSize(guildId) {
  const currentUser = authStore.getCurrentUser();
  const userMaxFileSize = enabled(3835).getUserMaxFileSize(currentUser);
  if (null == guildId) {
    return userMaxFileSize;
  } else {
    const guild = store.getGuild(guildId);
    if (null != guild) {
      const FileUploadPowerupHoldoutExperiment = guild(4116).FileUploadPowerupHoldoutExperiment;
...
export const fileUploadLimitRoadblockDescription = function fileUploadLimitRoadblockDescription(onClick) {
  ...
  if (maxSize == null) {
    ...
    if (null == guildId) {
      ...
      if (null != _Math) {
        const FileUploadPowerupHoldoutExperiment = tmp(4116).FileUploadPowerupHoldoutExperiment;
        enabled = FileUploadPowerupHoldoutExperiment.getConfig({ location: "getGuildMaxFileSize" }).enabled;
        let reduced = items2.reduce((arg0, arg1) => {
          let tmp;
          let tmp2;
    ...
    const obj3 = enabled(3835);
  }
  maxSize = _Math(4084).formatSize(maxSize / 1024, { useKibibytes: true });
  const obj = _Math(4084);
  const premiumMaxSize = _Math(4084).formatSize(closure_7 / 1024, { useKibibytes: true });
