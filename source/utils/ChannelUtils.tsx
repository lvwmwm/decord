...
// Dependencies: [1376, 1372, 1932, 3821, 1931, 4266, 676, 1876, 3826, 506, 1906, 4377, 4378, 1236, 4032, 4379, 11, 2, 4386]
...
function allowChannelAccess(id, channelType, MEMBER) {
  ...
  return { id, type: MEMBER, deny: importAll(3826).NONE, allow: addResult2 };
}
...
export const denyChannelAccessForNonPaidUsers = function denyChannelAccessForNonPaidUsers(arg0, arg1) {
  if (arg1 === ChannelTypes.GUILD_STAGE_VOICE) {
    ...
    obj[2] = importAll(3826).NONE;
    obj[3] = obj.add(importAll(3826).NONE, constants.CONNECT);
    return obj;
  } else {
    const _Error = Error;
...
export const permissionOverwritesForAnnouncement = function permissionOverwritesForAnnouncement(id) {
  const items = [{ id, type: require(1906) /* PermissionOverwriteType */.PermissionOverwriteType.ROLE, deny: constants.SEND_MESSAGES, allow: importAll(3826).NONE }];
  return items;
};
export const isChannelFull = function isChannelFull(channel, outer1_11, handleConnectionOpen) {
  ...
  if (channel.type === closure_23) {
    ...
    if (tmp8) {
      ...
      if (!hasVideoResult) {
        hasVideoResult = require(4377) /* hasStream */.hasStream(voiceStatesForChannel);
        const obj2 = require(4377) /* hasStream */;
      }
      tmp8 = hasVideoResult;
    }
    if (tmp8) {
      ...
      if (null != tmp10) {
        ...
        if (obj.isGuildStageVoice()) {
          ...
          for (const item10017 of tmp10) {
            ...
            let obj3 = require(4378) /* useAudienceRequestToSpeakState */;
            let audienceRequestToSpeakState = obj3.getAudienceRequestToSpeakState(item10017.voiceState);
            if (audienceRequestToSpeakState === require(4378) /* useAudienceRequestToSpeakState */.RequestToSpeakStates.ON_STAGE) {
              let tmp7 = obj2;
              obj2.return();
              let tmp8 = iter;
              iter.return();
