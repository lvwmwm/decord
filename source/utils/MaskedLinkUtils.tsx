...
// Function ID: 8512
// Name: isLinkTrusted
// Dependencies: [8512, 1372, 1862, 8513, 4473, 3892, 1931, 676, 3954, 4594, 1236, 698, 8514, 8515, 8516, 8518, 3946, 12406, 12410, 4691, 12411, 12413, 2]
// Exports: handleClick, isLinkTrusted

// Module 8511 (isLinkTrusted)
  ...
  if (null == sanitizeUrlResult) {
    ...
    try {
      ...
      if (null !== guild_id.isBlockedDomain(tmp8)) {
        ...
        tmp3(12406).show(tmp8);
      } else {
        let trustedResult = trusted;
        if (typeof trusted !== "HAS_APPLICATION") {
          trustedResult = trusted();
        }
        if (!trustedResult) {
          const TRUSTED_URLS = _require(12410).TRUSTED_URLS;
          trustedResult = TRUSTED_URLS.has(tmp8);
        }
        const protocol = _require(8514).getProtocol(tmp8);
        ...
        if (null == preventDefault) {
          ...
          obj6[3] = tmp12;
          obj6[4] = tmp11;
          let result = tmp3(8516).trackAnnouncementMessageLinkClicked(obj6);
          const tmp3Result6 = tmp3(8516);
        }
        const obj10 = _require(8514);
      }
      const tmp3Result2 = tmp3(8516);
    } catch (err) {
