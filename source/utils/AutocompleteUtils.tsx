...
function getMatchValue(toLocaleLowerCaseResult1, nextResult, flag) {
  ...
  try {
    if (exactQuery.test(toLocaleLowerCaseResult1)) {
      ...
      if (flag) {
        if (importDefault(5102)(queryLower, toLocaleLowerCaseResult1)) {
          return 1;
        }
      }
      return 0;
...
function queryMemberList(arg0) {
                        ...
                        if (str5.substring(0, normalizeResult.length) !== normalizeResult) {
                          ...
                          if (substr4 !== normalizeResult) {
                            ...
                            if (substr5 !== normalizeResult) {
                              let tmp33 = num < 50;
                              if (num < 50) {
                                let tmp28 = tmp12(5102)(toLocaleLowerCaseResult, str4) || tmp12(5102)(normalizeResult, str5);
                                if (!tmp28) {
                                  let tmp29 = null != str6 && tmp12(5102)(toLocaleLowerCaseResult, str6);
                                  tmp28 = tmp29;
                                }
                                if (!tmp28) {
                                  let tmp30 = null != str7 && tmp12(5102)(normalizeResult, str7);
                                  tmp28 = tmp30;
                                }
                                if (!tmp28) {
                                  let tmp31 = null != str8 && tmp12(5102)(toLocaleLowerCaseResult, str8);
                                  tmp28 = tmp31;
                                }
                                if (!tmp28) {
                                  let tmp32 = null != str9 && tmp12(5102)(normalizeResult, str9);
                                  tmp28 = tmp32;
                                }
                                tmp33 = tmp28;
                              }
  ...
  const sorted = items.sort(importDefault(5103));
  let combined = items;
  if (items.length < limit) {
    const sorted1 = items1.sort(importDefault(5103));
    const _Math = Math;
...
function getPriorityForStickerMetadataType(arg0) {
  if (require(4856) /* StickerFormat */.StickerMetadataTypes.STICKER_NAME === arg0) {
    return 11;
  } else if (tmp(4856).StickerMetadataTypes.CORRELATED_EMOJI === arg0) {
    return 6;
  } else if (tmp(4856).StickerMetadataTypes.TAG === arg0) {
    return 1;
  } else {
    if (tmp(4856).StickerMetadataTypes.GUILD_NAME !== arg0) {
      if (tmp(4856).StickerMetadataTypes.PACK_NAME !== arg0) {
        return 1;
      }
    }
    return 8;
...
let closure_48 = tmp7;
let closure_49 = [];
const MENTION_EVERYONE = require("module_5101").default.MENTION_EVERYONE;
const MENTION_HERE = require("module_5101").default.MENTION_HERE;
const MENTION_GAME = require("module_5101").default.MENTION_GAME;
const MENTION_TIMESTAMP = require("module_5101").default.MENTION_TIMESTAMP;
const LAUNCHABLE_APPLICATIONS = require("module_5101").default.LAUNCHABLE_APPLICATIONS;
...
areArraysShallowlyEqual = {
    ...
    if (null == channel) {
      ...
      if (null == channel1) {
        ...
        if (channel1.isPrivate()) {
          ...
          tmp6 = found1;
          if (flag) {
            const members1 = importDefault(5104).requestMembers(channel1.guild_id, query, limit);
            tmp6 = found1;
            const obj3 = importDefault(5104);
          }
        }
  ...
  queryChannels(guildId) {
    ...
    (function getSeparatedQueries(query, flag2) {
      ...
      const mapped = found.map((toLocaleLowerCase) => {
        ...
        const regExp = new RegExp("^" + callback(4177).escape(toLocaleLowerCaseResult), "i");
        obj[1] = regExp;
        const obj2 = callback(4177);
        const regExp1 = new RegExp(callback(4177).escape(toLocaleLowerCaseResult), "i");
        obj[2] = regExp1;
        return obj;
      });
      if (query.includes(" ")) {
        ...
        let obj2 = callback(4177);
        const _HermesInternal = HermesInternal;
        let regExp = new RegExp("^" + callback(4177).escape(toLocaleLowerCaseResult).replace(" ", "( |-)"), "i");
        obj[1] = regExp;
        const _RegExp2 = RegExp;
        const str = callback(4177).escape(toLocaleLowerCaseResult);
        const obj3 = callback(4177);
        let regExp1 = new RegExp(callback(4177).escape(toLocaleLowerCaseResult).replace(" ", "( |-)"), "i");
        obj[2] = regExp1;
        mapped.unshift(obj);
        const str6 = callback(4177).escape(toLocaleLowerCaseResult);
      }
      return mapped;
    })(guildId.query, flag2);
  ...
  queryGuilds(fuzzy) {
    ...
    const regExp = new RegExp("^" + importDefault(4177).escape(str), "i");
    obj[0] = regExp;
    const obj3 = importDefault(4177);
    const regExp1 = new RegExp(importDefault(4177).escape(str), "i");
    obj[1] = regExp1;
    obj[2] = str;
    const items = [];
  ...
  queryGroupDMs(fuzzy) {
    ...
    const regExp = new RegExp("^" + importDefault(4177).escape(stripDiacriticsResult), "i");
    obj[0] = regExp;
    const obj5 = importDefault(4177);
    const regExp1 = new RegExp(importDefault(4177).escape(stripDiacriticsResult), "i");
    obj[1] = regExp1;
    obj[2] = stripDiacriticsResult;
    const obj6 = importDefault(4177);
    ...
    while (iter2 !== undefined) {
      ...
      if (nextResult.isMultiUserDM()) {
        ...
        if (filter(tmp6)) {
          ...
          let obj9 = require(4380) /* computeChannelName */;
          ...
          if (tmp17 > 0) {
            ...
            let obj16 = require(4380) /* computeChannelName */;
  ...
  queryApplications(fuzzy) {
    ...
    const toLocaleLowerCaseResult = query.toLocaleLowerCase();
    let obj = { exactQuery: null, containQuery: null, queryLower: null };
    const regExp = new RegExp("^" + importDefault(4177).escape(toLocaleLowerCaseResult), "i");
    obj[0] = regExp;
    const obj2 = importDefault(4177);
    const regExp1 = new RegExp(importDefault(4177).escape(toLocaleLowerCaseResult), "i");
    obj[1] = regExp1;
  ...
  queryInAppNavigations(fuzzy) {
    ...
    const sorted = items7.sort(importDefault(5103));
  ...
  queryStaticRouteChannels(arg0) {
    ...
    const toLocaleLowerCaseResult = query.toLocaleLowerCase();
    let obj = { exactQuery: null, containQuery: null, queryLower: null };
    let obj1 = importDefault(4177);
    const regExp = new RegExp("^" + obj1.escape(toLocaleLowerCaseResult), "i");
    obj[0] = regExp;
    let obj2 = importDefault(4177);
    const regExp1 = new RegExp(obj2.escape(toLocaleLowerCaseResult), "i");
    obj[1] = regExp1;
    obj[2] = toLocaleLowerCaseResult;
    let canSeeOnboardingHomeResult = require(5658) /* useCanSeeOnboardingHome */.canSeeOnboardingHome(guild.id);
  ...
  queryStickers(items, arg1, items1) {
    ...
    function _loop() {
      if ("" === closure_6) {
        ...
        const _HermesInternal = HermesInternal;
        const regExp = new RegExp("^" + nextResult(4177).escape(stripDiacriticsResult), "i");
        const _RegExp2 = RegExp;
        const obj3 = nextResult(4177);
        const _HermesInternal2 = HermesInternal;
        const regExp1 = new RegExp("" + nextResult(4177).escape(stripDiacriticsResult), "i");
