...
// Dependencies: [32, 19, 3971, 676, 1924, 38, 10425, 3966, 5877, 589, 10427, 3902, 2]
...
// Module 10424 (getSubscriptionPlans)
...
export const subscriptionCanSwitchImmediately = function subscriptionCanSwitchImmediately(getCurrentSubscriptionPlanIdForGroup, newPlanId, arr) {
  ...
  if (null == currentSubscriptionPlanIdForGroup) {
    ...
    obj[1] = obj;
    const checkoutError = new require(10425) /* prototype */.CheckoutError(obj);
    throw checkoutError;
  } else {
    if (currentSubscriptionPlanIdForGroup === closure_8.PREMIUM_YEAR_TIER_1) {
      if (newPlanId === closure_8.PREMIUM_MONTH_TIER_2) {
        ...
        obj[1] = obj;
        const checkoutError1 = new require(10425) /* prototype */.CheckoutError(obj);
...
export const getOrFetchSubscriptionPlan = function getOrFetchSubscriptionPlan(subscriptionPlanId, closure_1) {
  ...
  if (null == value) {
    ...
    const result = require(3966) /* getPremiumPlanItem */.castPremiumSubscriptionAsSkuId(tmp5.skuId);
...
export const getSubscriptionPauseDurations = function getSubscriptionPauseDurations(status) {
  const keys = Object.keys(require(10427) /* PauseDuration */.PauseDuration);
  ...
  if (status.status !== constants.PAUSED) {
    ...
    const tmp6 = importDefault(3902)(status.currentPeriodStart);
    const _Math = Math;
    const rounded = Math.round(importDefault(3902)(status.pauseEndsAt).diff(tmp6, "days", true));
    const items = [];
    for (const item10042 of found) {
      ...
      let tmp10 = item10042;
      if (require(10427) /* PauseDuration */.PauseDuration[item10042] > rounded) {
        let tmp14 = item10042;
...
export const didBeginPurchaseFlowOnFractionalPremium = function didBeginPurchaseFlowOnFractionalPremium(isSameOrAfter) {
  ...
  if (isMomentResult) {
    isMomentResult = importDefault(3902).isMoment(isSameOrAfter);
    const obj = importDefault(3902);
  }
  if (isMomentResult) {
    isMomentResult = isSameOrAfter.isSameOrAfter(importDefault(3902)());
  }
