...
// Dependencies: [32, 19, 3935, 676, 1905, 38, 10337, 3930, 5798, 589, 10339, 3866, 2]
...
// Module 10336 (getSubscriptionPlans)
...
export const subscriptionCanSwitchImmediately = function subscriptionCanSwitchImmediately(getCurrentSubscriptionPlanIdForGroup, newPlanId, arr) {
  ...
  if (null == currentSubscriptionPlanIdForGroup) {
    ...
    obj[1] = obj;
    const checkoutError = new require(10337) /* prototype */.CheckoutError(obj);
    throw checkoutError;
  } else {
    if (currentSubscriptionPlanIdForGroup === closure_8.PREMIUM_YEAR_TIER_1) {
      if (newPlanId === closure_8.PREMIUM_MONTH_TIER_2) {
        ...
        obj[1] = obj;
        const checkoutError1 = new require(10337) /* prototype */.CheckoutError(obj);
...
export const getOrFetchSubscriptionPlan = function getOrFetchSubscriptionPlan(subscriptionPlanId, closure_1) {
  ...
  if (null == value) {
    ...
    const result = require(3930) /* getPremiumPlanItem */.castPremiumSubscriptionAsSkuId(tmp5.skuId);
    if (!addSubscriptionPlan.isFetchingForSKU(result)) {
      const subscriptionPlansForSKU = tmp12(5798).fetchSubscriptionPlansForSKU(result, closure_1);
      const tmp12Result = tmp12(5798);
    }
...
export const getSubscriptionPauseDurations = function getSubscriptionPauseDurations(status) {
  const keys = Object.keys(require(10339) /* PauseDuration */.PauseDuration);
  ...
  if (status.status !== constants.PAUSED) {
    ...
    const tmp6 = importDefault(3866)(status.currentPeriodStart);
    const _Math = Math;
    const rounded = Math.round(importDefault(3866)(status.pauseEndsAt).diff(tmp6, "days", true));
    const items = [];
    for (const item10042 of found) {
      ...
      let tmp10 = item10042;
      if (require(10339) /* PauseDuration */.PauseDuration[item10042] > rounded) {
        let tmp14 = item10042;
...
export const didBeginPurchaseFlowOnFractionalPremium = function didBeginPurchaseFlowOnFractionalPremium(isSameOrAfter) {
  ...
  if (isMomentResult) {
    isMomentResult = importDefault(3866).isMoment(isSameOrAfter);
    const obj = importDefault(3866);
  }
  if (isMomentResult) {
    isMomentResult = isSameOrAfter.isSameOrAfter(importDefault(3866)());
  }
