...
// Dependencies: [5, 17, 4301, 3982, 1903, 5798, 676, 1905, 7362, 3, 38, 500, 7381, 5801, 12, 514, 3962, 589, 7382, 1606, 4282, 7383, 2]
// Exports: makeIAPRequest, manageSubscription, shouldMockIAPForceEnable, useCanPurchaseIAP

// Module 7361 (serializePurchaseResponse)
...
function remapStorefront(countryCode) {
  ...
  if (null == countryCode.currency) {
    if (null == require(7382) /* GeneratedPaymentCurrencies */.GeneratedPaymentCurrenciesSets.APPLE_STORE_COUNTRY_CURRENCIES[country]) {
      const _Error = Error;
...
let _default = {
  loadProducts() {
    if (obj.isIOS()) {
      ...
      let items = [importDefault(7381).fetchSubscriptions(Object.values(tmp(5801).ProductIds)), ];
      const obj2 = importDefault(7381);
      const _Object2 = Object;
      items[1] = importDefault(7381).fetchProducts(Object.values(tmp(5801).ProductIds));
      const obj3 = importDefault(7381);
...
function shouldMockIAPForceEnable() {
  ...
  if (isIOSResult) {
    ...
    let isRunningOnSimulator = Identifier.startsWith("" + IOS_BUNDLE_ID + ".local");
    if (!isRunningOnSimulator) {
      isRunningOnSimulator = require(4282) /* DCDDeviceManager */.getIsRunningOnSimulator();
      const tmpResult = require(4282) /* DCDDeviceManager */;
    }
