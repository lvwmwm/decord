...
function getCodeToKeyLanguageCorrection(keyCode, key, arg2) {
  ...
  const BACKTICK_CODES = require(12890) /* _isNativeReflectConstruct */.BACKTICK_CODES;
  if (BACKTICK_CODES.has(keyCode)) {
    let obj = require(12890) /* _isNativeReflectConstruct */;
    ...
    if (key === value) {
      ...
      const exactKeyboardEventMatchFromAny = require(12890) /* _isNativeReflectConstruct */.getExactKeyboardEventMatchFromAny(obj);
      if ("\\" === key) {
        if ("`" === value) {
          return key;
        }
      ...
      const obj3 = require(12890) /* _isNativeReflectConstruct */;
...
function codeToKey(arg0) {
  ...
  if (KeyboardEnvs.LINUX === tmp6) {
    ...
    if (KeyboardEnvs.WINDOWS === tmp6) {
      ...
      const tmp15 = importDefault(12889)(tmp5);
      let tmp17 = null;
      if (null != tmp15) {
    ...
    if (null != tmp17) {
      ...
      const keyboardEventShapeFromKeycode = require(12890) /* _isNativeReflectConstruct */.getKeyboardEventShapeFromKeycode(tmp2);
      let tmp29 = null;
...
function keyToCode(key) {
    ...
    if (tmp10 === true) {
      ...
      if (tmp13 === true) {
        ...
        if (tmp15 === true) {
          ...
          if (tmp17 === true) {
            parsed = importDefault(12889)(getKeyConversionForBrowser(key));
            const tmp30 = importDefault(12889);
          } else if (tmp8 === true) {
            parsed = set[key];
  ...
  if (null != tmp6) {
    ...
    const keyboardEventShapeFromKey = require(12890) /* _isNativeReflectConstruct */.getKeyboardEventShapeFromKey(key);
    let keyCode = null;
...
export const getRawCodeFromKey = function getRawCodeFromKey(arg0) {
  ...
  if (tmp === KeyboardEnvs.BROWSER) {
    let tmp4 = importDefault(12889)(arg0);
  } else {
    tmp4 = set[arg0];
  }
  return tmp4;
...
export const toCombo = function toCombo(shortcut) {
  ...
  const parts = shortcut.replace(/numpad plus/i, "").replace(/NUMPAD \+/i, "numpad plus").replace(/mod/i, KEYBOARD_KEY(12891).modKey).split("+");
