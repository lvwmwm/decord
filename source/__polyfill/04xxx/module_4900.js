...
export default importAllResult.forwardRef(function ScreenStackItem(unstable_sheetFooter, ref) {
  ...
  dependencyMap = flag4.useContext(screenId(4898).RNSScreensRefContext);
  ...
  obj = { value: obj1.useEdgeInsetApplication(flag, flag2, flag3).nextContextValue, children: callback(tmp10(4901), { contentStyle, style: container, stackPresentation, children }) };
  const items1 = [callback(screenId(4889).EdgeInsetApplicationContext.Provider, obj), , ];
  obj = {};
  const merged1 = Object.assign(headerConfig);
  items1[1] = callback(screenId(4888).ScreenStackHeaderConfig, obj);
  let tmp18Result = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp18Result) {
    obj1 = { children: null };
    obj1[0] = unstable_sheetFooter();
    tmp18Result = tmp18(tmp2(4904).FooterComponent, obj1);
  }
