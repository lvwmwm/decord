...
arg5.default = {
  read(byteLength, sum) {
    ...
    if (sum + 9 + 1 <= byteLength.byteLength) {
      const byteAt1 = module(5126).getByteAt(byteLength, sum + 9);
      ...
      tmp16 = obj3;
      const obj8 = module(5126);
    }
    obj[3] = tmp16;
    let tmp20;
    if (sum + 10 + 1 <= byteLength.byteLength) {
      const byteAt2 = module(5126).getByteAt(byteLength, sum + 10);
      ...
      obj4[1] = str6;
      tmp20 = obj4;
      const obj10 = module(5126);
    }
    obj[4] = tmp20;
    let tmp24;
    if (sum + 11 + 1 <= byteLength.byteLength) {
      const byteAt3 = module(5126).getByteAt(byteLength, sum + 11);
