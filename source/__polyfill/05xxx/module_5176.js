// Module ID: 5176
// Function ID: 5177
// Dependencies: [5177, 5178, 5180, 5181, 5182, 5186, 5187, 5188, 5189, 5174]
...
arg5.default = {
  parseAppMarkers(byteLength, flag2) {
    if (module(5177).USE_TIFF) {
      let tmpResult = tmp(5178);
    ...
    if (module(5177).USE_JPEG) {
      if (tmpResult1.isJpegFile(byteLength)) {
        const tmpResult2 = tmp(5180);
        const findJpegOffsetsResult = tmp(5180).findJpegOffsets(byteLength);
        obj = { fileType: null };
    ...
    if (module(5177).USE_PNG) {
      if (tmpResult3.isPngFile(byteLength)) {
        const tmpResult4 = tmp(5181);
        const findPngOffsetsResult = tmp(5181).findPngOffsets(byteLength, flag2);
        obj1 = { fileType: null };
    ...
    if (module(5177).USE_HEIC) {
      if (tmpResult5.isHeicFile(byteLength)) {
        const tmpResult6 = tmp(5182);
        const findHeicOffsetsResult = tmp(5182).findHeicOffsets(byteLength);
    ...
    if (module(5177).USE_AVIF) {
      if (tmpResult7.isAvifFile(byteLength)) {
        const tmpResult8 = tmp(5186);
        const findAvifOffsetsResult = tmp(5186).findAvifOffsets(byteLength);
    ...
    if (module(5177).USE_WEBP) {
      if (tmpResult9.isWebpFile(byteLength)) {
        const tmpResult10 = tmp(5187);
        const findOffsetsResult = tmp(5187).findOffsets(byteLength);
