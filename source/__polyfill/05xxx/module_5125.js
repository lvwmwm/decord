...
arg5.default = {
  read(dataView, sum) {
    ...
    let tmp8;
    if (5 <= shortAt) {
      const shortAt1 = tmp(5126).getShortAt(dataView, sum + 3);
    ...
    let tmp11;
    if (7 <= shortAt) {
      const shortAt2 = tmp(5126).getShortAt(dataView, sum + 5);
