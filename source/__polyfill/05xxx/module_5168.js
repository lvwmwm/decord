// Module ID: 5168
// Function ID: 5169
// Dependencies: [5169, 5170, 5172, 5173, 5174, 5178, 5179, 5180, 5181, 5166]
...
arg5.default = {
  parseAppMarkers(byteLength, flag2) {
    if (module(5169).USE_TIFF) {
      let tmpResult = tmp(5170);
    ...
    if (module(5169).USE_JPEG) {
      if (tmpResult1.isJpegFile(byteLength)) {
        const tmpResult2 = tmp(5172);
        const findJpegOffsetsResult = tmp(5172).findJpegOffsets(byteLength);
        obj = { fileType: null };
    ...
    if (module(5169).USE_PNG) {
      if (tmpResult3.isPngFile(byteLength)) {
        const tmpResult4 = tmp(5173);
        const findPngOffsetsResult = tmp(5173).findPngOffsets(byteLength, flag2);
        obj1 = { fileType: null };
    ...
    if (module(5169).USE_HEIC) {
      if (tmpResult5.isHeicFile(byteLength)) {
        const tmpResult6 = tmp(5174);
        const findHeicOffsetsResult = tmp(5174).findHeicOffsets(byteLength);
    ...
    if (module(5169).USE_AVIF) {
      if (tmpResult7.isAvifFile(byteLength)) {
        const tmpResult8 = tmp(5178);
        const findAvifOffsetsResult = tmp(5178).findAvifOffsets(byteLength);
    ...
    if (module(5169).USE_WEBP) {
      if (tmpResult9.isWebpFile(byteLength)) {
        const tmpResult10 = tmp(5179);
        const findOffsetsResult = tmp(5179).findOffsets(byteLength);
