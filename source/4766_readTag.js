...
function readTag(byteLength, arg1, arg2, sum, byteOrder) {
  ...
  sum = typeSize + module(4763).getTypeSize("SHORT");
  const obj2 = module(4763);
  const sum1 = sum + module(4763).getTypeSize("LONG");
  const obj3 = module(4763);
  const shortAt = module(4763).getShortAt(byteLength, sum, byteOrder);
  const obj4 = module(4763);
  const shortAt1 = module(4763).getShortAt(byteLength, sum + typeSize, byteOrder);
  const obj5 = module(4763);
  const longAt = module(4763).getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== module(4763).typeSizes[shortAt1]) {
    const result = module(4763).typeSizes[shortAt1] * longAt;
    if (result <= obj7.getTypeSize("LONG")) {
      const sum2 = sum + sum1;
      let str = getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder);
      let tmp15 = sum2;
    } else {
      const longAt1 = module(4763).getLongAt(byteLength, sum + sum1, byteOrder);
      const sum3 = arg2 + longAt1;
      if (sum3 + module(4763).typeSizes[shortAt1] * longAt <= byteLength.byteLength) {
      ...
      const obj10 = module(4763);
    }
    if (shortAt1 === module(4763).tagTypes.ASCII) {
      const items = [];
      let num3 = 0;
      let num4 = 0;
      if (0 < str.length) {
    ...
    if (undefined !== module(4767)[arg1][shortAt]) {
      if (undefined !== module(4767)[arg1][shortAt].name) {
        if (undefined !== module(4767)[arg1][shortAt].description) {
          name = module(4767)[arg1][shortAt].name;
          descriptionResult = module(4767)[arg1][shortAt].description(str);
          const obj8 = module(4767)[arg1][shortAt];
        }
      }
      if (shortAt1 !== module(4763).tagTypes.RATIONAL) {
        if (shortAt1 !== module(4763).tagTypes.SRATIONAL) {
          name = module(4767)[arg1][shortAt];
...
function getTagValue(arg0, arg1, arg2, arg3, arg4) {
  ...
  let result = arg3;
  if (tmp2) {
    result = arg3 * module(4763).typeSizes[arg2];
    BYTE = module(4763).tagTypes.BYTE;
  }
  for (let num2 = 0; num2 < result; num2 = num2 + 1) {
    ...
    sum = sum + module(4763).typeSizes[BYTE];
  }
  if (BYTE === module(4763).tagTypes.ASCII) {
    obj = module(4763);
...
arg5.get0thIfdOffset = function get0thIfdOffset(dataView, sum1, byteOrder) {
  return sum1 + module(4763).getLongAt(dataView, sum1 + 4, byteOrder);
};
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5, arg6) {
  ...
  if (sum + obj3.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = module(4763).getShortAt(byteLength, sum, byteOrder);
    const obj4 = module(4763);
  }
  sum = sum + typeSize;
  let tmp5 = sum;
      ...
      while (true) {
        ...
        if (undefined !== tmp13) {
          ...
          if (!tmp15) {
            ...
            let tmp18 = IFD_TYPE_0TH === require(4767) /* IFD_TYPE_0TH */.IFD_TYPE_PENTAX && "LevelInfo" === tmp13.name;
            tmp15 = tmp18;
          }
          if (tmp15) {
  ...
  if (module(4749).USE_THUMBNAIL) {
    if (tmp5 < byteLength.byteLength - obj6.getTypeSize("LONG")) {
      const longAt = module(4763).getLongAt(byteLength, tmp5, byteOrder);
      let tmp25 = 0 !== longAt;
      if (tmp25) {
        tmp25 = IFD_TYPE_0TH === require(4767) /* IFD_TYPE_0TH */.IFD_TYPE_0TH;
      }
      if (tmp25) {
        obj.Thumbnail = readIfd(byteLength, require(4767) /* IFD_TYPE_0TH */.IFD_TYPE_1ST, sum, sum + longAt, byteOrder, arg5);
      }
      const obj7 = module(4763);
    }
    obj6 = module(4763);
  }
  return obj;
