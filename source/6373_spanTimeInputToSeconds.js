...
// Dependencies: [6374, 6378, 6380, 6382, 6383, 6384, 6385, 6386, 6387, 6390, 6395, 6368]

// Module 6373 (spanTimeInputToSeconds)
const require = arg1;
  ...
  if (typeof getTime === "Object") {
    ...
    if (Array.isArray(getTime)) {
      ...
      if (getTime instanceof Date) {
        ...
        sum = require(6382) /* dateTimestampInSeconds */.timestampInSeconds();
        const obj = require(6382) /* dateTimestampInSeconds */;
      }
    }
  }
  return sum;
  ...
  if ((function spanIsSentrySpan(getSpanJSON) {
    ...
    try {
      ...
      if ((function spanIsOpenTelemetrySdkTraceBaseSpan(attributes) {
        ...
        obj[8] = attributes[require(undefined, 6383).SEMANTIC_ATTRIBUTE_SENTRY_OP];
        obj[9] = attributes[require(undefined, 6383).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
        const obj2 = require(6374) /* addNonEnumerableProperty */;
        obj[10] = require(6384) /* getMetricSummaryJsonForSpan */.getMetricSummaryJsonForSpan(getSpanJSON);
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== require(6385) /* getSpanStatusFromHttpCode */.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== tmp(6385).SPAN_STATUS_OK) {
...
arg5.addChildSpanToSpan = function addChildSpanToSpan(arg0, arg1) {
  ...
  const result = require(6374) /* addNonEnumerableProperty */.addNonEnumerableProperty(arg1, _sentryRootSpan, tmp2);
  if (arg0[_sentryChildSpans]) {
    ...
    const result1 = require(6374) /* addNonEnumerableProperty */.addNonEnumerableProperty(arg0, tmp6, set);
    const tmp3Result = require(6374) /* addNonEnumerableProperty */;
  }
};
arg5.getActiveSpan = function getActiveSpan() {
  ...
  const asyncContextStrategy = require(6387) /* getAsyncContextStrategy */.getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.getActiveSpan) {
    let activeSpan = asyncContextStrategy.getActiveSpan();
  } else {
    let tmpResult = tmp(6390);
    tmpResult = tmp(6395);
...
arg5.showSpanDropWarning = function showSpanDropWarning() {
  if (!c2) {
    require(6368) /* consoleSandbox */.consoleSandbox(() => {
...
arg5.spanToTraceContext = function spanToTraceContext(spanContext) {
  ...
  if (isRemote) {
    span_id = require(6378) /* generatePropagationContext */.generateSpanId();
    const obj = require(6378) /* generatePropagationContext */;
  }
  return require(6374) /* addNonEnumerableProperty */.dropUndefinedKeys({ parent_span_id, span_id, trace_id: trace_id.traceId });
};
arg5.spanToTraceHeader = function spanToTraceHeader(spanContext) {
  ...
  return require(6380) /* regExp */.generateSentryTraceHeader(traceId, spanId, 1 === spanContext.spanContext().traceFlags);
};
arg5.spanToTransactionTraceContext = function spanToTransactionTraceContext(spanContext) {
  ...
  return require(6374) /* addNonEnumerableProperty */.dropUndefinedKeys({ parent_span_id, span_id, trace_id, data, op, status, origin });
};
