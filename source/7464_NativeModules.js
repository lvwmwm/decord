...
// Function ID: 7465
// Name: NativeModules
// Dependencies: [5, 17, 7465, 7466, 7471, 7472, 7473, 7474, 7475]
...
const subs = require("module_7465").ProductType.subs;
const inapp = require("module_7465").ProductType.inapp;
...
export const isIosStorekit2 = require("module_7466").isIosStorekit2;
export const setup = () => {
  let obj = arg0;
  if (arg0 === undefined) {
    obj = {};
  ...
  if ("STOREKIT1_MODE" === str) {
    require(7466).storekit1Mode();
    const obj4 = require(7466);
  } else if ("STOREKIT2_MODE" === str) {
    require(7466).storekit2Mode();
    const obj3 = require(7466);
  } else if ("STOREKIT_HYBRID_MODE" === str) {
    require(7466).storekitHybridMode();
    const obj2 = require(7466);
  }
};
export const initConnection = () => {
  const nativeModule = require(7466).getNativeModule();
...
export const flushFailedPurchasesCachedAsPendingAndroid = () => {
  const androidModule = require(7466).getAndroidModule();
...
export const getProducts = (skus) => {
  ...
  if (length) {
    closure_1 = callback(function*() {
      let closure_1 = tmp5;
      let closure_0 = tmp2;
      const obj3 = outer1_0(7466);
      const androidModule = obj3.getAndroidModule();
      yield androidModule.getItemsByType(outer1_7, outer1_0);
      closure_0 = arg1.map(outer1_0(7471).singleProductAndroidMap);
      const obj = outer1_0(7466);
...
export const getSubscriptions = (skus) => {
  ...
  if (length) {
    closure_1 = callback(function*() {
      ...
      androidModuleType = outer1_0(7466).getAndroidModuleType();
      const obj10 = outer1_0(7466);
      const androidModule = outer1_0(7466).getAndroidModule();
      const outer1_1 = yield androidModule.getItemsByType(outer1_6, outer1_0);
      if ("android" === outer1_0) {
        return outer1_8(outer1_1, outer1_0(7465).SubscriptionPlatform.android);
      }
      if ("amazon" !== tmp35) {
        const _Error = Error;
      ...
      const obj2 = outer1_0(7466);
      let closure_2 = yield obj2.fillProductsWithAdditionalData(dependencyMap);
      return outer1_8(closure_2, outer1_0(7465).SubscriptionPlatform.amazon);
    });
    let rejectResult = (function android() {
      const self = this;
      const apply = closure_1.apply;
