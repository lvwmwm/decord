...
// Function ID: 5042
// Name: constructTag
// Dependencies: [4997, 5013, 5025, 4994, 5001, 5027]

// Module 5041 (constructTag)
const require = arg1;
const module = arg2;
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (decompressResult instanceof DataView) {
    let obj = require(4994) /* getDataView */;
    stringFromDataView = obj.getStringFromDataView(decompressResult, 0, decompressResult.byteLength);
  }
  const stringValueFromArray = require(4994) /* getDataView */.getStringValueFromArray(items1);
  let combined = stringValueFromArray;
  if (type !== require(5001).TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(4994).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(4994);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === require(5001).TYPE_ITXT) {
    stringFromDataView = module(5027).decode("UTF-8", decompressResult);
    const obj5 = module(5027);
  }
