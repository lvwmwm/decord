...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let dependencyMap;
  let iter;
  const tmp4 = iter(scrollableGesture(5411).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  let obj = scrollableGesture(5404);
  ...
  obj = { animatedScrollableState: iter, SCROLLABLE_STATE: tmp2(5400).SCROLLABLE_STATE };
  fn.__closure = obj;
  ...
  if (tmp7) {
    ...
    let tmp8Result = tmp8(tmp2(5427).GestureDetector, obj);
  } else {
    const obj2 = {};
