...
// Dependencies: [5441, 5394, 31, 5459, 5391, 5460]
// Exports: useBenchmark

// Module 5458 (getFormattedString)
...
export const useBenchmark = function useBenchmark(arg0, arg1) {
  ...
  if (arguments.length > 2) {
    ...
    let tmp8 = callback4(() => {
      if (!first) {
        const Cancellable = callback(5459).Cancellable;
        ...
        if (cancellable.current) {
          ...
          if (Number(length) <= 0) {
            const _Error = Error;
            const error = new Error(callback(5391).ErrorMessages.dataEmptyCannotRunBenchmark);
            throw error;
          }
        }
        callback2(true);
        (() => {
          let closure_0 = obj(async () => {
            let tmp9;
            const jSFPSMonitor = new lib(5460).JSFPSMonitor();
