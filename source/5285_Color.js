...
obj = {
  ...
  string(num) {
    const self = this;
    let self2 = this;
    if (!(this.model in require(5290) /* hasOwnProperty */.to)) {
      self2 = self.rgb();
    }
    num = 1;
    ...
    const to = require(5290) /* hasOwnProperty */.to;
  ...
  percentString(num) {
    ...
    const rgb = require(5290) /* hasOwnProperty */.to.rgb;
  ...
  object() {
    const self = this;
    const obj = {};
    const channels = require(5286) /* keys */[this.model].channels;
    for (let num = 0; num < channels; num = num + 1) {
      obj[require(5286) /* keys */[this.model].labels[num]] = self.color[num];
    }
    if (1 !== self.valpha) {
  ...
  keyword(color) {
    if (undefined !== color) {
      ...
      const self = this;
      keywordResult = require(5286) /* keys */[this.model].keyword(this.color);
      const obj = require(5286) /* keys */[this.model];
    }
