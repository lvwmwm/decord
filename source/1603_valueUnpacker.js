...
let closure_2 = () => {
  function valueUnpacker(__workletHash, arg1, self) {
    let __handleCache;
    let __workletsCache;
    ({ __workletsCache, __handleCache } = self);
    if (undefined === __workletsCache) {
      const _Map = Map;
      const map = new Map();
      self.__workletsCache = map;
      const _WeakMap = WeakMap;
      const weakMap = new WeakMap();
      self.__handleCache = weakMap;
      __workletsCache = map;
    ...
    if (undefined !== __workletHash) {
      ...
      if (undefined !== value) {
        ...
        if (self.evalWithSourceMap) {
          ...
          evalWithSourceMapResult = self.evalWithSourceUrl(`${`(${__initData.code}`}
