...
// Dependencies: [32, 19, 4049]
// Exports: useRiveProperty

// Module 4048 (useRiveProperty)
...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f28801) {
  const _require = instance;
  const dependencyMap = FillColor;
  const callback = f28801;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = _require(4049).useDisposableMemo(() => {
    if (closure_0) {
