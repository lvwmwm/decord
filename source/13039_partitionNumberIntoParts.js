...
// Module 13039 (partitionNumberIntoParts)
...
const regExp = new RegExp("^".concat(require("result").S_UNICODE_REGEX.source));
const regExp1 = new RegExp("".concat(require("result").S_UNICODE_REGEX.source, "$"));
let closure_5 = /[#0](?:[\.,][#0]+)*/g;
  ...
  if ("currency" === style) {
    if ("name" === currencyDisplay.currencyDisplay) {
      if (numbers.currencies[currencyDisplay.currency]) {
        ...
        let currency3 = selectPlural(arg2, roundedNumber4.times(result.default.pow(10, exponent)).toNumber(), tmp70.displayName);
        const _default3 = result.default;
        const timesResult = roundedNumber4.times(result.default.pow(10, exponent));
      } else {
    ...
    if (numbers.units.simple[unit]) {
      ...
      let str30 = selectPlural(arg2, roundedNumber3.times(result.default.pow(10, exponent)).toNumber(), numbers.units.simple[unit][unitDisplay]);
      const _default2 = result.default;
      const timesResult1 = roundedNumber3.times(result.default.pow(10, exponent));
    } else {
      const parts2 = unit.split("-per-");
      [tmp54, tmp55] = parts2;
      const roundedNumber2 = magnitude.roundedNumber;
      const tmp59 = selectPlural(arg2, roundedNumber2.times(result.default.pow(10, exponent)).toNumber(), numbers.units.simple[tmp54][unitDisplay]);
      ...
      const timesResult2 = roundedNumber2.times(result.default.pow(10, exponent));
    }
    const items2 = [];
