...
// Dependencies: [7767, 7768, 7769]

// Module 7766 (appendTransformProps)
const require = arg1;
...
function appendTransformProps(arg0) {
  ...
  require(7767) /* append */.appendTransform(x + originX, y + originY, scaleX, scaleY, rotation, skewX, skewY, originX, originY);
}
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        tmp3Result = tmp3(7768);
        const parsed = tmp3Result.parse(arr);
        const tmp3Result1 = tmp3(7767);
  ...
  const obj = require(7767) /* append */;
  return require(7767) /* append */.toArray();
}
arg5.default = function extractTransform(arr) {
  ...
  if (typeof arr === "string") {
    try {
      const parsed = require(7768) /* peg$SyntaxError */.parse(arr);
      const items = [, , , , , ];
      ...
      const _console = tmp.console;
      _console.error(tmp6);
      return require(7767) /* append */.identity;
    }
  } else {
    let transform;
    if (arr != null) {
...
arg5.extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "string") {
    transform = require(7769) /* peg$SyntaxError */.parse(transform.transform);
    const obj = require(7769) /* peg$SyntaxError */;
  } else {
