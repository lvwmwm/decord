...
// Dependencies: [32, 7526, 7532, 7521, 7534, 7504, 7516, 7517, 7518, 7511, 7509, 7527, 7499, 7531, 7535, 7537, 7528, 7538, 7540, 7514]
// Exports: continueTrace, startInactiveSpan, startNewTrace, startSpan, startSpanManual, suppressTracing, withActiveSpan

// Module 7533 (createChildOrRootSpan)
...
function createChildOrRootSpan(forceTransaction) {
  ...
  let obj = require(7531) /* hasTracingEnabled */;
  if (obj.hasTracingEnabled()) {
    let tmpResult = tmp(7526);
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(7540).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(7532).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(7504).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(7504);
        const client = tmp(7526).getClient();
        if (client) {
      ...
      tmp(7537).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(7537);
      const result = tmp(7528).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(7535).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(7535);
      ...
      obj1[0] = traceId2;
      obj1[1] = spanId2;
      const tmpResult7 = tmp(7504);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj1, scope, tmp(7504).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(7504).spanIsSampled(parentSpan);
      tmp(7535).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = tmp(7535);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(7535).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = tmp(7535);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(7532).SentryNonRecordingSpan();
...
function _startRootSpan(name) {
  let tmp6;
  let tmp7;
  let obj = require(7526) /* getClient */;
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(7538).sampleSpan(options, obj);
  ...
  obj1.attributes = { [require(7514).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(7540).SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(7514).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  ...
  let dependencyMap;
  const _require = arg1;
  const mainCarrier = _require(7517).getMainCarrier();
  const obj = _require(7517);
  const tmp = _require;
  const asyncContextStrategy = _require(7518).getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(7526).withScope((setPropagationContext) => {
...
export const startInactiveSpan = function startInactiveSpan(experimental) {
  ...
  let asyncContextStrategy = _require(7518).getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.startInactiveSpan) {
    ...
    if (experimental.startTime) {
      ...
      obj.startTimestamp = tmp3(7504).spanTimeInputToSeconds(experimental.startTime);
      delete tmp[tmp2];
      tmp10 = obj;
      const tmp3Result = tmp3(7504);
    }
      ...
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = tmp2(tmp3[1]).withScope((arg0) => {
            _null(7521)._setSpanForScope(arg0, _null);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return _require(7526).withScope((setPropagationContext) => {
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(7526).withScope((setSDKProcessingMetadata) => {
      const result = setSDKProcessingMetadata.setSDKProcessingMetadata({ [outer1_3]: true });
      return callback();
    });
    const tmpResult = tmp(7526);
  }
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(7526).withScope((arg0) => {
      _null(7521)._setSpanForScope(arg0, _null);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(7526);
