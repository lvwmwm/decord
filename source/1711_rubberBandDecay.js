...
function rubberBandDecay(current, lastTimestamp, deceleration) {
  ...
  const diff = velocity * Math.exp(result * require(1710) /* isValidRubberBandConfig */.SLOPE_FACTOR) - num2 * deceleration.rubberBandFactor;
  if (Math.abs(num2) > 0.1) {
    ...
    if (absolute1 < require(1710) /* isValidRubberBandConfig */.VELOCITY_EPS) {
      return true;
    }
  }
