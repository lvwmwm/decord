...
// Dependencies: [32, 19, 5524, 5535, 5526, 5539, 5510]
// Exports: useJSResponderHandler

// Module 5523 (isSupportedGesture)
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(5526).SingleGestureName.Tap !== type) {
      if (tmp(5526).SingleGestureName.LongPress !== type) {
        if (tmp(5526).SingleGestureName.Fling !== type) {
          if (tmp(5526).SingleGestureName.Native !== type) {
            if (tmp(5526).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  ...
  let tmp = callback2(_require(5539).JSResponderContext);
