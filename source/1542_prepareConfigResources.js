...
// Dependencies: [32, 1543, 1544, 1545, 1541, 1507, 1536]
// Exports: getStateFromPath

// Module 1542 (prepareConfigResources)
...
function checkForDuplicatedConfigs(map, routeNames2, joined) {
  if (map) {
    ...
    if (routeNames.length > routeNames1.length) {
      let arrayStartsWithResult = require(1545) /* arrayStartsWith */.arrayStartsWith(routeNames, routeNames1);
      const obj2 = require(1545) /* arrayStartsWith */;
    } else {
      arrayStartsWithResult = require(1545) /* arrayStartsWith */.arrayStartsWith(routeNames1, routeNames);
      const obj = require(1545) /* arrayStartsWith */;
    }
...
function getStaticSegmentPattern(arg0) {
  return Array.from(arg0, (str) => {
    ...
    return "(?:" + callback(1543)(str) + "|" + callback(1543)(encodeURIComponentResult) + ")";
  }).join("");
}
...
function createNestedStateObject(str, items, initialRoutes, config) {
  ...
  if (0 === items.length) {
    ...
    const findFocusedRouteResult = require(1507) /* findFocusedRoute */.findFocusedRoute(obj);
...
function parseQueryParams(arr) {
  ...
  if (substr) {
    parsed = importAll(1536).parse(substr);
    const obj2 = importAll(1536);
  } else {
    parsed = {};
  }
  set[Symbol.iterator]();
