...
// Function ID: 9352
// Name: pbkdf2Init
// Dependencies: [5, 9349, 9348, 9352]
// Exports: pbkdf2, pbkdf2Async

// Module 9351 (pbkdf2Init)
import _asyncLoop from "_asyncLoop" /* 9348 */;
import number from "number" /* 9349 */;
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9348).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9352).hmac;
    let obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9348).toBytes(B);
