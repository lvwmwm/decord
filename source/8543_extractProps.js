...
// Function ID: 8544
// Name: extractProps
// Dependencies: [8544, 8535, 8545, 8548, 8536, 8534]

// Module 8543 (extractProps)
const require = arg1;
const module = arg2;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(8535)(obj, markerMid, arg1);
  module(8545)(obj, markerMid, items);
  ...
  const tmp6 = module(8536)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(8534)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
  ...
  if (markerStart) {
    let tmp7;
    if (markerStart) {
      const match = markerStart.match(require(8544) /* pickNotNil */.idPattern);
      let tmp10;
      if (match) {
        tmp10 = match[1];
      }
  ...
  if (str) {
    let tmp11;
    if (str) {
      const match1 = str.match(require(8544) /* pickNotNil */.idPattern);
      let tmp14;
      if (match1) {
        tmp14 = match1[1];
      }
  ...
  if (str2) {
    let tmp15;
    if (str2) {
      const match2 = str2.match(require(8544) /* pickNotNil */.idPattern);
      let tmp18;
      if (match2) {
        tmp18 = match2[1];
      }
  ...
  if (clipPath) {
    const match3 = clipPath.match(require(8544) /* pickNotNil */.idPattern);
    if (match3) {
  ...
  if (mask) {
    const match4 = mask.match(require(8544) /* pickNotNil */.idPattern);
    if (match4) {
      obj.mask = match4[1];
    } else {
  ...
  if (filter) {
    const match5 = filter.match(require(8544) /* pickNotNil */.idPattern);
    if (match5) {
      obj.filter = match5[1];
    } else {
