...
// Function ID: 6427
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 6418, 6399, 6371, 6425, 6382, 6378, 6394, 6395, 6427, 6414, 6415, 6398, 6420, 6407, 6428, 6411, 6429, 6376]
...
let items = [
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release === "__FORMATJS_LISTFORMAT_DATA__") {
        if (BaseClient(6399).DEBUG_BUILD) {
          const logger = tmp(6371).logger;
        ...
        BaseClient(6394).updateSession(release, { init: false });
        const obj = BaseClient(6394);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(6427).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(6427).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(6427);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(6414).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(6399).DEBUG_BUILD) {
        let logger = tmp2(6371).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(6395).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(6427).setupIntegrations(this, integrations);
      const obj = BaseClient(6427);
      const result = BaseClient(6427).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(6411).parseSampleRate(sampleRate);
        let obj = str(6411);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult !== "__REMOTEDEV__") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(6428).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(6395).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(6399).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(6371).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(6429);
