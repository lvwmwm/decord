...
// Function ID: 5558
// Name: map
// Dependencies: [32, 19, 5555, 5545, 5558, 5531, 5546, 5493]
...
function isGestureEnabled(closure_0) {
  if (obj.isComposedGesture(closure_0)) {
    ...
    someResult = false !== require(5545) /* hash */.maybeUnpackValue(closure_0.config.enabled);
    const tmpResult = require(5545) /* hash */;
  }
  return someResult;
}
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = require(5558) /* isNativeEvent */.isNativeAnimatedEvent(useAnimated.onUpdate);
    const obj = require(5558) /* isNativeEvent */;
  }
  ...
  if (!disableReanimated) {
    result = undefined !== require(5531).Reanimated;
  }
  if (result) {
    result = require(5545) /* hash */.hasWorkletEventHandlers(useAnimated);
    const obj2 = require(5545) /* hash */;
  }
  if (result) {
    result = !useAnimated.dispatchesAnimatedEvents;
  }
  useAnimated.shouldUseReanimatedDetector = result;
  useAnimated.needsPointerData = require(5558) /* isNativeEvent */.shouldHandleTouchEvents(useAnimated);
};
export const prepareConfigForNativeSide = function prepareConfigForNativeSide(closure_0, closure_1) {
  ...
  const PropsWhiteLists = require(5546) /* items2 */.PropsWhiteLists;
  let EMPTY_WHITE_LIST = PropsWhiteLists.get(closure_0);
  if (EMPTY_WHITE_LIST == null) {
    EMPTY_WHITE_LIST = require(5546) /* items2 */.EMPTY_WHITE_LIST;
  }
  ...
  while (tmp12 !== undefined) {
    ...
    let allowedNativeProps = require(5546) /* items2 */.allowedNativeProps;
    if (!allowedNativeProps.has(first)) {
      let tmp22 = first;
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let tmp23 = tmp18;
        let tmp24 = tmp20;
        let PropsToFilter = tmp19(5546).PropsToFilter;
        let tmp25 = first;
    ...
    let tmp32 = tmp18;
    let tmp33 = tmp20;
    let Reanimated = tmp19(5531).Reanimated;
