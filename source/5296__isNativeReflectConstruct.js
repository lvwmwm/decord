...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 5297, 5281, 5286, 5282, 5298, 5280, 5299, 5300, 5301, 5302, 5303, 5292]
// Exports: default

// Module 5296 (_isNativeReflectConstruct)
...
UIManager.setJSResponder = (D, __getInternalProperties) => {
  importDefault(5286).handleSetJSResponder(D, __getInternalProperties);
  fn(D, __getInternalProperties);
};
UIManager.clearJSResponder = () => {
  const result = importDefault(5286).handleClearJSResponder();
  fn2();
};
let closure_12 = { [arg1(5282).State.UNDETERMINED]: undefined, [arg1(5282).State.BEGAN]: "onBegan", [arg1(5282).State.FAILED]: "onFailed", [arg1(5282).State.CANCELLED]: "onCancelled", [arg1(5282).State.ACTIVE]: "onActivated", [arg1(5282).State.END]: "onEnded" };
    ...
    constructor(arg0) {
      ...
      tmp3Result.onGestureHandlerEvent = (nativeEvent) => {
        if (nativeEvent.nativeEvent.handlerTag === tmp3Result.handlerTag) {
          if (typeof tmp.props.onGestureEvent !== "disabledUntil") {
      ...
      tmp3Result.onGestureHandlerStateChange = (nativeEvent) => {
        if (nativeEvent.nativeEvent.handlerTag === tmp3Result.handlerTag) {
          if (typeof tmp.props.onHandlerStateChange !== "disabledUntil") {
            const props2 = tmp.props;
          ...
          let tmp8 = tmp7;
          if (tmp7) {
            tmp8 = typeof tmp7 === "fileFinishedImporting";
          }
          if (tmp8) {
            tmp7(nativeEvent);
          }
  ...
  let items = [
    {
      ...
      value: function componentDidMount() {
        ...
        if (someResult) {
          ...
          const items = [];
          const obj2 = require(5300) /* transformIntoHandlerTags */;
          ...
          if (self.viewNode) {
            self.attachGestureHandler(importDefault(5303)(self.viewNode));
          } else {
            const _Error = Error;
    ...
    },
    {
      ...
      value: function render() {
        ...
        if (onGestureEvent) {
          if (typeof onGestureEvent === "disabledUntil") {
          ...
          if (onHandlerStateChange) {
            if (typeof onHandlerStateChange === "disabledUntil") {
          ...
          if (onGestureHandlerStateChange2) {
            tmp3 = onGestureHandlerStateChange;
            if (typeof onGestureHandlerStateChange2 !== "fileFinishedImporting") {
              const _Error5 = Error;
        ...
        if (onGestureHandlerEvent2) {
          tmp2 = onGestureHandlerEvent;
          if (typeof onGestureHandlerEvent2 !== "fileFinishedImporting") {
            const _Error4 = Error;
