...
// Dependencies: [853, 7495, 7498, 7499, 7513, 7523, 7527, 7531, 7581, 7504, 7533, 7514, 7532, 7570, 7516, 7512, 7506]
// Exports: addTracingHeadersToFetchRequest, instrumentFetchRequest

// Module 7604 (_addTracingHeadersToFetchRequest)
...
function _addTracingHeadersToFetchRequest(headers, headers2, span) {
  let baggage;
  let tmp4;
  let obj = require(7570) /* getTraceData */;
  obj = { span };
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(7516).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(7516);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(7516).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(7581).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(7581);
    }
    const activeSpan = setHttpStatus(7504).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(7514).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(7514).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(7533).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(7533);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(7531);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(7532).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(7504);
  }
};
