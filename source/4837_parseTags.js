...
// Function ID: 4838
// Name: parseTags
// Dependencies: [4838, 4839]

// Module 4837 (parseTags)
const module = arg2;
    ...
    if (tmp < byteLength.byteLength) {
      while (true) {
        ...
        if (byteLength.getUint8(tmp) !== 28) {
          ...
          let tmp15 = obj;
          let tmp16 = encoding;
          obj[3] = getTagDescription(tmp8(4838).iptc[uint16], items, obj, encoding);
          let tmp17 = tmp8(4838).iptc[uint16] && tmp8(4838).iptc[uint16].repeatable;
          if (tmp17) {
            obj.repeatable = true;
          }
          let tmp18 = tmp8(4838).iptc[uint16] && undefined !== tmp8(4838).iptc[uint16].encoding_name;
          if (tmp18) {
            let obj4 = tmp8(4838).iptc[uint16];
      ...
      if (typeof obj3.name === "fileFinishedImporting") {
...
function getTagDescription(description, items, arg2, encoding) {
  if (!(function hasDescriptionProperty(description) {
    ...
    if ((function tagValueIsText(description, items) {
      ...
      decodeResult = module(4839).decode(encoding, items);
      const obj = module(4839);
    }
    return decodeResult;
  } else {
    try {
