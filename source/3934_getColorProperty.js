...
// Dependencies: [57, 31, 3916, 3929, 3924]
// Exports: useRiveColor

// Module 3934 (getColorProperty)
...
export const useRiveColor = function useRiveColor(FillColor, instance) {
  ...
  const tmp2 = callback(require(3929) /* useRiveProperty */.useRiveProperty(instance, FillColor, getColorProperty), 3);
  const first = tmp2[0];
  require = tmp4;
  if (cResult[0] !== first) {
    let fromIntResult;
    if (undefined !== first) {
      let RiveColor = require(3924) /* RiveColor */.RiveColor;
