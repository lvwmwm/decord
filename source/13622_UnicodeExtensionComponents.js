...
// Module 13622 (UnicodeExtensionComponents)
const require = arg1;
const dependencyMap = arg6;
arg5.UnicodeExtensionComponents = function UnicodeExtensionComponents(str) {
  let obj;
  obj(13617).invariant(str === str.toLowerCase(), "Expected extension to be lowercase");
  let num = 3;
  obj(13617).invariant("-u-" === str.slice(0, 3), "Expected extension to be a Unicode locale extension");
  const attributes = [];
  const keywords = [];
  if (3 < str.length) {
    while (true) {
      ...
      if (2 === tmp6) {
        ...
        if ("" === value) {
          ...
          let invariantResult3 = tmp8(13617).invariant(undefined !== iter, "Expected keyword to be defined");
