...
// Dependencies: [57, 65, 6358, 6361, 6374, 6397, 6398, 6384, 6356, 6399, 6392, 6368]
// Exports: parseEventHintOrCaptureContext, prepareEvent

// Module 6396 (applyClientOptions)
...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = require(6356) /* isMatchingPattern */.truncate(environment.message, num);
    const obj = require(6356) /* isMatchingPattern */;
  }
  ...
  if (value) {
    iter.value = require(6356) /* isMatchingPattern */.truncate(iter.value, num);
  ...
  if (url) {
    request.url = require(6356) /* isMatchingPattern */.truncate(request.url, num);
    const obj3 = require(6356) /* isMatchingPattern */;
  }
}
function applyDebugIds(exception) {
  _require = _require(6399).getFilenameToDebugIdMap(arg1);
...
export const parseEventHintOrCaptureContext = function parseEventHintOrCaptureContext(captureContext) {
  if (captureContext) {
    let tmp3 = captureContext instanceof require(6368) /* ScopeClass */.Scope;
    if (!tmp3) {
