...
function getMessageForObject(name) {
  ...
  const result = require(dependencyMap[2]).extractExceptionKeysForMessage(name);
...
function getException(getOptions, arg1, arg2, syntheticException) {
  if (obj.isError(arg2)) {
    ...
    if (obj2.isPlainObject(arg2)) {
      ...
      const tmp13 = callback2({}, "__serialized__", require(dependencyMap[4]).normalizeToSize(arg2, normalizeDepth));
      let tmp16;
...
export const eventFromMessage = function eventFromMessage(arg0, value) {
  ...
  const obj7 = require(dependencyMap[3]);
};
