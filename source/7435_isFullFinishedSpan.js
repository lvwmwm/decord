...
// Dependencies: [41, 42, 7404, 7408, 7409, 7399, 7432, 7400, 7410, 7436, 7422, 7394, 7421, 7437, 7423, 7430]

// Module 7435 (isFullFinishedSpan)
...
class SentrySpan {
  constructor() {
    ...
    obj[require("module_7409").SEMANTIC_ATTRIBUTE_SENTRY_OP] = obj.op;
...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(7399).spanTimeInputToSeconds(arg0);
    }
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(7409).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    }
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(7399).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(7399);
        SentrySpan(7432).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(7432);
      }
    }
  },
  {
    ...
    value: function getSpanJSON() {
      let _attributes;
      const self = this;
      let obj = SentrySpan(7400);
      obj = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 7409).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: null, timestamp: null, trace_id: null, origin: null, _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      obj[6] = SentrySpan(7399).getStatusMessage(this._status);
      ({ _endTime: obj2[7], _traceId: obj2[8], _attributes } = this);
      obj[9] = _attributes[SentrySpan(undefined, 7409).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
      const obj3 = SentrySpan(7399);
      obj[10] = SentrySpan(7410).getMetricSummaryJsonForSpan(this);
      obj[11] = this._attributes[SentrySpan(undefined, 7409).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj[12] = this._attributes[SentrySpan(undefined, 7409).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(7410);
      obj[13] = SentrySpan(7436).timedEventsToMeasurements(this._events);
  ...
  },
  {
    ...
    value: function addEvent(name, num) {
      if (SentrySpan(7422).DEBUG_BUILD) {
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(7421).getClient();
      if (client) {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          ...
          if (result) {
            let scope = tmp(7423).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(7421).getCurrentScope();
              const tmpResult2 = tmp(7421);
            }
            scope.captureEvent(result);
            const tmpResult1 = tmp(7423);
          }
        }
      } else {
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        ...
        if (true !== self._sampled) {
          ...
          const spanDescendants = tmp3(7399).getSpanDescendants(self);
          ...
          const mapped = found.map((arg0) => self(7399).spanToJSON(arg0));
          const found1 = mapped.filter(isFullFinishedSpan);
          const tmp24 = self._attributes[tmp3(undefined, 7409).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
          const _attributes = self._attributes;
          let SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = tmp3(7409).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
          delete tmp2[tmp];
          const item = found1.forEach((data) => {
            if (data.data) {
              data = data.data;
              const SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = self(7409).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
              delete tmp2[tmp];
            }
          });
          ...
          obj[0] = tmp3(7399).spanToTransactionTraceContext(self);
          obj[0] = obj;
          let substr = found1;
          ...
          const tmp3Result4 = tmp3(7400);
          obj2[0] = tmp3(7430).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result4.dropUndefinedKeys(obj2));
          obj[6] = obj1;
          const tmp3Result5 = tmp3(7430);
          obj[7] = tmp3(7410).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
          ...
          const tmp3Result6 = tmp3(7410);
          const result = tmp3(7436).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
          ...
          if (length) {
            if (tmp3(7422).DEBUG_BUILD) {
              const logger2 = tmp3(7394).logger;
              const _JSON = JSON;
