...
// Function ID: 4854
// Name: constructTag
// Dependencies: [4809, 4825, 4837, 4806, 4813, 4839]

// Module 4853 (constructTag)
const require = arg1;
const module = arg2;
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  const stringValueFromArray = require(4806) /* getDataView */.getStringValueFromArray(items1);
  let combined = stringValueFromArray;
  if (type !== require(4813).TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(4806).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(4806);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === require(4813).TYPE_ITXT) {
    stringFromDataView = module(4839).decode("UTF-8", decompressResult);
    const obj5 = module(4839);
  }
