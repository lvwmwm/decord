// Module ID: 109
// Function ID: 1603
// Dependencies: []

// Module 109
let obj = { topAccessibilityAction: { registrationName: "onAccessibilityAction" } };
const _module = require(dependencyMap[0]);
obj.onGestureHandlerEvent = _module.DynamicallyInjectedByGestureHandler({ registrationName: "onGestureHandlerEvent" });
const _module1 = require(dependencyMap[0]);
obj.onGestureHandlerStateChange = _module1.DynamicallyInjectedByGestureHandler({ registrationName: "onGestureHandlerStateChange" });
obj.topContentSizeChange = { registrationName: "onContentSizeChange" };
obj.topScrollBeginDrag = { registrationName: "onScrollBeginDrag" };
obj.topMessage = { registrationName: "onMessage" };
obj.topSelectionChange = { registrationName: "onSelectionChange" };
obj.topLoadingFinish = { registrationName: "onLoadingFinish" };
obj.topMomentumScrollEnd = { registrationName: "onMomentumScrollEnd" };
obj.topLoadingStart = { registrationName: "onLoadingStart" };
obj.topLoadingError = { registrationName: "onLoadingError" };
obj.topMomentumScrollBegin = { registrationName: "onMomentumScrollBegin" };
obj.topScrollEndDrag = { registrationName: "onScrollEndDrag" };
obj.topScroll = { registrationName: "onScroll" };
obj.topLayout = { registrationName: "onLayout" };
obj = { <string:2195412908>: null, <string:1365265948>: null, <string:1449460740>: null };
obj = { process: require(dependencyMap[1]).default };
obj.backgroundColor = obj;
obj.experimental_backgroundImage = { process: require(dependencyMap[2]).default };
const obj1 = { process: require(dependencyMap[2]).default };
const tmp2 = !importAll(dependencyMap[3]).enableNativeCSSParsing();
let tmp3 = !tmp2;
if (tmp2) {
  const obj2 = { process: require(dependencyMap[4]).default };
  tmp3 = obj2;
}
obj.boxShadow = tmp3;
const importAllResult = importAll(dependencyMap[3]);
const tmp4 = !importAll(dependencyMap[3]).enableNativeCSSParsing();
let tmp5 = !tmp4;
if (tmp4) {
  const obj3 = { process: require(dependencyMap[5]).default };
  tmp5 = obj3;
}
obj.filter = tmp5;
obj.mixBlendMode = true;
obj.isolation = true;
obj.opacity = true;
obj.elevation = true;
const importAllResult1 = importAll(dependencyMap[3]);
obj.shadowColor = { process: require(dependencyMap[1]).default };
obj.zIndex = true;
obj.renderToHardwareTextureAndroid = true;
obj.testID = true;
obj.nativeID = true;
obj.accessibilityLabelledBy = true;
obj.accessibilityLabel = true;
obj.accessibilityHint = true;
obj.accessibilityRole = true;
obj.accessibilityCollection = true;
obj.accessibilityCollectionItem = true;
obj.accessibilityState = true;
obj.accessibilityActions = true;
obj.accessibilityValue = true;
obj.experimental_accessibilityOrder = true;
obj.importantForAccessibility = true;
obj.screenReaderFocusable = true;
obj.role = true;
obj.rotation = true;
obj.scaleX = true;
obj.scaleY = true;
obj.translateX = true;
obj.translateY = true;
obj.accessibilityLiveRegion = true;
obj.width = true;
obj.minWidth = true;
obj.collapsable = true;
obj.collapsableChildren = true;
obj.maxWidth = true;
obj.height = true;
obj.minHeight = true;
obj.maxHeight = true;
obj.flex = true;
obj.flexGrow = true;
obj.rowGap = true;
obj.columnGap = true;
obj.gap = true;
obj.flexShrink = true;
obj.flexBasis = true;
obj.aspectRatio = true;
obj.flexDirection = true;
obj.flexWrap = true;
obj.alignSelf = true;
obj.alignItems = true;
obj.alignContent = true;
obj.justifyContent = true;
obj.overflow = true;
obj.display = true;
obj.boxSizing = true;
obj.margin = true;
obj.marginBlock = true;
obj.marginBlockEnd = true;
obj.marginBlockStart = true;
obj.marginBottom = true;
obj.marginEnd = true;
obj.marginHorizontal = true;
obj.marginInline = true;
obj.marginInlineEnd = true;
obj.marginInlineStart = true;
obj.marginLeft = true;
obj.marginRight = true;
obj.marginStart = true;
obj.marginTop = true;
obj.marginVertical = true;
obj.padding = true;
obj.paddingBlock = true;
obj.paddingBlockEnd = true;
obj.paddingBlockStart = true;
obj.paddingBottom = true;
obj.paddingEnd = true;
obj.paddingHorizontal = true;
obj.paddingInline = true;
obj.paddingInlineEnd = true;
obj.paddingInlineStart = true;
obj.paddingLeft = true;
obj.paddingRight = true;
obj.paddingStart = true;
obj.paddingTop = true;
obj.paddingVertical = true;
obj.borderWidth = true;
obj.borderStartWidth = true;
obj.borderEndWidth = true;
obj.borderTopWidth = true;
obj.borderBottomWidth = true;
obj.borderLeftWidth = true;
obj.borderRightWidth = true;
const obj4 = { process: require(dependencyMap[1]).default };
obj.outlineColor = { process: require(dependencyMap[1]).default };
obj.outlineOffset = true;
obj.outlineStyle = true;
obj.outlineWidth = true;
obj.start = true;
obj.end = true;
obj.left = true;
obj.right = true;
obj.top = true;
obj.bottom = true;
obj.inset = true;
obj.insetBlock = true;
obj.insetBlockEnd = true;
obj.insetBlockStart = true;
obj.insetInline = true;
obj.insetInlineEnd = true;
obj.insetInlineStart = true;
obj.position = true;
obj.style = importDefault(dependencyMap[6]);
obj.removeClippedSubviews = true;
obj.accessible = true;
obj.hasTVPreferredFocus = true;
obj.nextFocusDown = true;
obj.nextFocusForward = true;
obj.nextFocusLeft = true;
obj.nextFocusRight = true;
obj.nextFocusUp = true;
obj.borderRadius = true;
obj.borderTopLeftRadius = true;
obj.borderTopRightRadius = true;
obj.borderBottomRightRadius = true;
obj.borderBottomLeftRadius = true;
obj.borderTopStartRadius = true;
obj.borderTopEndRadius = true;
obj.borderBottomStartRadius = true;
obj.borderBottomEndRadius = true;
obj.borderEndEndRadius = true;
obj.borderEndStartRadius = true;
obj.borderStartEndRadius = true;
obj.borderStartStartRadius = true;
obj.borderStyle = true;
obj.hitSlop = true;
obj.pointerEvents = true;
obj.nativeBackgroundAndroid = true;
obj.nativeForegroundAndroid = true;
obj.needsOffscreenAlphaCompositing = true;
const obj5 = { process: require(dependencyMap[1]).default };
obj.borderColor = { process: require(dependencyMap[1]).default };
const obj6 = { process: require(dependencyMap[1]).default };
obj.borderLeftColor = { process: require(dependencyMap[1]).default };
const obj7 = { process: require(dependencyMap[1]).default };
obj.borderRightColor = { process: require(dependencyMap[1]).default };
const obj8 = { process: require(dependencyMap[1]).default };
obj.borderTopColor = { process: require(dependencyMap[1]).default };
const obj9 = { process: require(dependencyMap[1]).default };
obj.borderBottomColor = { process: require(dependencyMap[1]).default };
const obj10 = { process: require(dependencyMap[1]).default };
obj.borderStartColor = { process: require(dependencyMap[1]).default };
const obj11 = { process: require(dependencyMap[1]).default };
obj.borderEndColor = { process: require(dependencyMap[1]).default };
const obj12 = { process: require(dependencyMap[1]).default };
obj.borderBlockColor = { process: require(dependencyMap[1]).default };
const obj13 = { process: require(dependencyMap[1]).default };
obj.borderBlockEndColor = { process: require(dependencyMap[1]).default };
const obj14 = { process: require(dependencyMap[1]).default };
obj.borderBlockStartColor = { process: require(dependencyMap[1]).default };
obj.focusable = true;
obj.backfaceVisibility = true;
const obj15 = { process: require(dependencyMap[1]).default };

export default { directEventTypes: obj, bubblingEventTypes: { topChange: { phasedRegistrationNames: { display: 0, alignItems: "red" } }, topSelect: { phasedRegistrationNames: { display: 2, alignItems: null } }, topTouchEnd: { phasedRegistrationNames: { display: "Button", alignItems: "Math" } }, topTouchCancel: { phasedRegistrationNames: { display: true, alignItems: true } }, topTouchStart: { phasedRegistrationNames: { display: "<string:827438790>", alignItems: "<string:3545309543>" } }, topTouchMove: { phasedRegistrationNames: { display: "stack", alignItems: null } }, topPointerCancel: { phasedRegistrationNames: {} }, topPointerDown: { phasedRegistrationNames: { display: false, alignItems: false } }, topPointerEnter: { phasedRegistrationNames: { "Null": null, "Null": "v2", "Null": null } }, topPointerLeave: { phasedRegistrationNames: {} }, topPointerMove: { phasedRegistrationNames: { display: null, alignItems: null } }, topPointerUp: { phasedRegistrationNames: { display: null, alignItems: null } }, topPointerOut: { phasedRegistrationNames: {} }, topPointerOver: { phasedRegistrationNames: { display: "a", alignItems: "isArray" } }, topClick: { phasedRegistrationNames: { display: "petrol varili", alignItems: "<string:257032192>" } }, topBlur: { phasedRegistrationNames: {} }, topFocus: { phasedRegistrationNames: { display: null, alignItems: null } } }, validAttributes: Object.assign({}, obj, {}) };
