...
// Dependencies: [7292, 7237, 7328]

// Module 7327 (generateIteratee)
...
export const rewriteFramesIntegration = setupIntegration.defineIntegration(() => {
  ...
  if (arguments.length > 0) {
    ...
    let tmp3 = "window" in iteratee(7237).GLOBAL_OBJ;
    if (tmp3) {
      tmp3 = undefined !== iteratee(7237).GLOBAL_OBJ.window;
    }
              ...
              obj = {
                values: values.map((stacktrace) => {
                  ...
                  if (stacktrace) {
                    ...
                    if (frames) {
                      frames = stacktrace2.frames;
                      frames = frames.map((arg0) => outer3_0(arg0));
                    }
                    obj.frames = frames;
