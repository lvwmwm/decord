...
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 4802, 4838, 4839, 4840]
...
function readTags(_raw, buffer, arg2) {
  ...
  try {
    ...
    ParseError(4802).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(4839).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(4839);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
...
function getDescription(arr) {
  if (Array.isArray(arr)) {
    ...
    try {
      if (tmp2) {
        if (typeof importDefault(4840)[tmp2] === "find") {
          let decodeURIComponentResult = importDefault(4840)[tmp2](arr);
          const tmp4Result = importDefault(4840);
        }
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(4802).objectAssign(obj, obj, obj);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(4802).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(4802);
    obj = {};
    ...
    ParseError(4802).objectAssign(obj.attributes, obj);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(4802);
  }
  ParseError(4802).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
