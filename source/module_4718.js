// Module ID: 4718
// Function ID: 4719
// Dependencies: [109, 19, 17, 21, 4716, 4707, 4700, 4719, 4706, 4722, 4695]
...
export default importAllResult.forwardRef(function ScreenStackItem(unstable_sheetFooter, ref) {
  ...
  dependencyMap = flag4.useContext(screenId(4716).RNSScreensRefContext);
  const imperativeHandle = flag4.useImperativeHandle(ref, () => ref.current);
  let obj1 = screenId(4707);
  let flag;
  ...
  importDefault(4700)(flag5, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  obj[1] = callback(importDefault(4719), { contentStyle, style: container, stackPresentation, children });
  const items1 = [callback(screenId(4707).EdgeInsetApplicationContext.Provider, obj), , ];
  obj = {};
  const merged1 = Object.assign(headerConfig);
  items1[1] = callback(screenId(4706).ScreenStackHeaderConfig, obj);
  let tmp18Result = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp18Result) {
    obj1 = { children: null };
    obj1[0] = unstable_sheetFooter();
    tmp18Result = tmp18(tmp2(4722).FooterComponent, obj1);
  }
  items1[2] = tmp18Result;
  const tmp11 = importDefault(4700);
  ...
  return callback(importDefault(4695), obj2);
});
