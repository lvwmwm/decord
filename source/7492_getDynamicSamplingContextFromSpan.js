...
// Dependencies: [7462, 7493, 7483, 7461, 7469, 7471, 7488]

// Module 7492 (getDynamicSamplingContextFromSpan)
const require = arg1;
const dependencyMap = arg6;
function getDynamicSamplingContextFromSpan(spanContext) {
  let obj = require(7483) /* getClient */;
    ...
    if (rootSpan[_frozenDsc]) {
      ...
      if (result) {
        ...
        if (!DEFAULT_ENVIRONMENT) {
          DEFAULT_ENVIRONMENT = tmp(7493).DEFAULT_ENVIRONMENT;
        }
        ...
        const dropUndefinedKeysResult = tmp(7462).dropUndefinedKeys(obj);
        client.emit("createDsc", dropUndefinedKeysResult);
        const tmpResult1 = tmp(7462);
        const spanToJSONResult = tmp(7461).spanToJSON(rootSpan);
        const tmp13 = spanToJSONResult.data || {};
        const tmp14 = tmp13[tmp(undefined, 7471).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE];
        if (null != tmp14) {
        ...
        tmp17 = "url" !== tmp13[tmp(undefined, 7471).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] && description;
        if (tmpResult3.hasTracingEnabled()) {
          const _String = String;
          dropUndefinedKeysResult.sampled = String(tmp(7461).spanIsSampled(rootSpan));
          const tmpResult4 = tmp(7461);
        }
...
arg5.freezeDscOnSpan = function freezeDscOnSpan(arg0, arg1) {
  const result = require(7462) /* addNonEnumerableProperty */.addNonEnumerableProperty(arg0, _frozenDsc, arg1);
};
arg5.getDynamicSamplingContextFromClient = function getDynamicSamplingContextFromClient(trace_id, getOptions) {
  ...
  let DEFAULT_ENVIRONMENT = options.environment;
  if (!DEFAULT_ENVIRONMENT) {
    DEFAULT_ENVIRONMENT = require(7493).DEFAULT_ENVIRONMENT;
  }
...
arg5.getDynamicSamplingContextFromScope = function getDynamicSamplingContextFromScope(getOptions, getPropagationContext) {
  ...
  if (!dsc) {
    ...
    let DEFAULT_ENVIRONMENT = options.environment;
    if (!DEFAULT_ENVIRONMENT) {
      DEFAULT_ENVIRONMENT = tmp5(7493).DEFAULT_ENVIRONMENT;
    }
    obj = { environment: null, release: null, public_key: null, trace_id: null };
    obj[0] = DEFAULT_ENVIRONMENT;
    obj[1] = options.release;
...
arg5.spanToBaggageHeader = function spanToBaggageHeader(arg0) {
  ...
  return require(7469) /* baggageHeaderToDynamicSamplingContext */.dynamicSamplingContextToSentryBaggageHeader(tmp);
};
