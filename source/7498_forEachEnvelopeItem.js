...
// Dependencies: [32, 7455, 7499, 7460, 7501]
...
// Module 7498 (forEachEnvelopeItem)
...
export const createAttachmentEnvelopeItem = function createAttachmentEnvelopeItem(data) {
  if (typeof data.data === "string") {
    ...
    if (!require(7455) /* getGlobalSingleton */.GLOBAL_OBJ.__SENTRY__) {
      const _TextEncoder = TextEncoder;
      const textEncoder = new TextEncoder();
      let encodeResult = textEncoder.encode(data);
    }
    __SENTRY__ = __SENTRY__(7455).GLOBAL_OBJ.__SENTRY__;
    ...
    const items = [require(7460) /* addNonEnumerableProperty */.dropUndefinedKeys(obj), data];
    return items;
  }
};
...
export const createEventEnvelopeHeaders = function createEventEnvelopeHeaders(event_id) {
  ...
  if (tmp4) {
    obj = { dsn: null };
    obj[0] = require(7501) /* dsnFromString */.dsnToString(arg3);
    tmp4 = obj;
    const obj5 = require(7501) /* dsnFromString */;
  }
  ...
  if (event_id.sdkProcessingMetadata && event_id.sdkProcessingMetadata.dynamicSamplingContext) {
    ...
    obj1[0] = require(7460) /* addNonEnumerableProperty */.dropUndefinedKeys(obj2);
    tmp8 = obj1;
    const obj7 = require(7460) /* addNonEnumerableProperty */;
  }
...
export const parseEnvelope = function parseEnvelope(arr) {
  if (typeof arr !== "string") {
    ...
    let encodePolyfill = dependencyMap;
    if (!_require(7455).GLOBAL_OBJ.__SENTRY__) {
      const _TextEncoder = TextEncoder;
      const textEncoder = new TextEncoder();
    ...
    if (typeof tmp8[1] !== "string") {
      ...
      if (!(tmp10 instanceof Uint8Array)) {
        try {
          ...
          json = _JSON3.stringify(_require(7499).normalize(tmp3));
          const obj = _require(7499);
        }
      }
    }
    append(tmp8[1]);
