// Module ID: 13030
// Function ID: 13031
// Dependencies: [12993, 13022, 13013, 13018, 13014, 13017, 13024, 13021]
...
export default (arg0, arg1, arg2, arg3) => {
  let obj;
  let _require = arg3;
  const tmp3 = new _require(12993)(arg0, arg3);
  const tmp4 = new _require(13022)(arg1, arg3);
  if (">" === arg2) {
    let dependencyMap = tmp(13013);
    let tmpResult = tmp(13018);
    tmpResult = tmp(13014);
    ...
    str = "<";
    if ("<" === arg2) {
      dependencyMap = tmp(13014);
      tmpResult = tmp(13017);
      tmpResult1 = tmp(13013);
  ...
  if (_require(13024)(tmp3, tmp4, arg3)) {
    return false;
  } else {
    let num = 0;
    let num3 = 0;
    if (0 < tmp4.set.length) {
      while (true) {
        ...
        let item = arr.forEach((semver) => {
          let tmp = semver;
          if (semver.semver === require(13021) /* Comparator */.ANY) {
            tmp = new require(13021) /* Comparator */(">=0.0.0");
          }
          let tmp6 = _null;
          if (!_null) {
            tmp6 = tmp;
