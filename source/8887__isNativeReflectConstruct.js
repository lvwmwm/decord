...
// Module 8887 (_isNativeReflectConstruct)
...
export const HashMD = (Hash) => {
  class HashMD {
    constructor(arg0, arg1, arg2, arg3) {
      self = this;
      tmp = closure_3(this, HashMD);
      obj = closure_6(HashMD);
      tmp2 = closure_5;
      if (closure_8()) {
        tmp4 = globalThis;
        _Reflect = Reflect;
      ...
      tmp2Result.view = HashMD(closure_1[6]).createView(tmp2Result.buffer);
      return tmp2Result;
    }
  }
  const require = HashMD;
  callback2(HashMD, Hash);
  let obj = {
    key: "update",
    value: function update(B) {
      ...
      let tmp9;
      const self = this;
      HashMD(closure_1[7]).exists(this);
      ({ buffer, blockLen } = this);
      const toBytesResult = HashMD(closure_1[6]).toBytes(B);
      let num = 0;
      if (0 < toBytesResult.length) {
        do {
          ...
          if (bound !== blockLen) {
            ...
            let tmp5 = closure_0;
            let tmp6 = closure_1;
            let view = closure_0(closure_1[6]).createView(toBytesResult);
            let tmp8 = num;
            tmp9 = num;
            tmp10 = view;
  ...
  obj = {
    ...
    value: function digestInto(content) {
      ...
      const self = this;
      HashMD(closure_1[7]).exists(this);
      HashMD(closure_1[7]).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      view = HashMD(closure_1[6]).createView(content);
  ...
  items[3] = {
    ...
    value: function _cloneInto(arg0) {
      ...
      constructor.set.apply(constructor, callback(self.get()));
