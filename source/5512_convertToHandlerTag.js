...
// Dependencies: [32, 19, 5513, 5514, 5524, 5525, 5526, 5528, 5529, 5530, 5509, 5493, 5531, 5491]
// Exports: checkGestureCallbacksForWorklets, extractGestureRelations, useForceRender, useWebEventHandlers

// Module 5512 (convertToHandlerTag)
...
function convertToHandlerTag(num) {
  if (typeof num === "number") {
    return num;
  } else if (num instanceof require(5509) /* _isNativeReflectConstruct */.BaseGesture) {
    num = num.handlerTag;
  } else {
...
export const useWebEventHandlers = function useWebEventHandlers() {
  return callback3({
    onGestureHandlerEvent(nativeEvent) {
      const result = callback(5491).onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
