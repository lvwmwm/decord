...
// Function ID: 4209
// Name: reset
// Dependencies: [4203, 4209, 1218, 1372, 1862, 4236, 3817, 4261, 1931, 4205, 676, 4254, 687, 4253, 12964, 12965, 12966, 1959, 6008, 589, 12852, 709, 2]
...
prototype["getLastActiveStream"] = function getLastActiveStream() {
  let tmp = null;
  if (importDefault(12852)(_detectH265HardwareDecode)) {
    const _Array = Array;
  ...
  if (importDefault(12852)(_detectH265HardwareDecode)) {
    ...
    if (null != tmp5) {
      ...
      if (!tmp10) {
        ...
        let first = null != basicChannel1;
        if (first) {
          const obj2 = require(6008) /* canStreamInChannel */;
          first = obj2.canWatchStream(basicChannel1, updateVoiceState, createGuildRecordFromRust, getUncachedChannelPermissions, set)[0];
        }
        flag = first;
      }
...
const applicationStreamingStore = new ApplicationStreamingStore(require("dispatcher"), {
  ...
  NATIVE_SCREEN_SHARE_PICKER_UPDATE: function handleNativePickerUpdate(content) {
    ...
    if (null == content) {
      ...
      if (tmp2) {
        const tmp6 = importDefault(12965)(content);
  ...
  STREAM_DELETE: function handleStreamDelete(unavailable) {
    ...
    if (null == value) {
      ...
      if (unavailable.unavailable) {
        ...
        guildId(1959)(12966, dependencyMap.paths).then((arg0) => {
          arg0.default(guildId);
        });
        FAILED = tmp22.ENDED;
        const promise = guildId(1959)(12966, dependencyMap.paths);
      } else {
        if (tmp9) {
          FAILED = tmp22.FAILED;
        }
