...
// Dependencies: [32, 5, 1963, 4503, 1975, 1218, 1372, 4969, 1961, 1971, 1891, 3912, 3921, 1960, 4101, 1903, 676, 3, 11, 4970, 4975, 1936, 4973, 4504, 1384, 12, 6980, 4513, 3923, 7214, 12944, 589, 11062, 1935, 709, 2]

// Module 4502 (reinjectEphemerals)
...
function handleConnectionOpen() {
  const item = importDefault(4970).forEach((mutate) => {
...
function receiveMediaMentionMessage(item10037) {
  ...
  if (null != message_id) {
    ...
    const orCreate = importDefault(4970).getOrCreate(attachment_id);
    let obj = {};
...
function handleCleanup() {
  const item = importDefault(4970).forEach((channelId) => {
...
function handleRelationshipUpdate() {
  let c0 = false;
  const item = importDefault(4970).forEach((reset) => {
...
function handleReaction(optimistic) {
  ...
  const value = importDefault(4970).get(channelId);
  if (null == value) {
    return false;
  } else {
...
function handleMessageSendFailedAutomod(arg0) {
  ...
  const failedMessageId = require(7214) /* items */.getFailedMessageId(messageData);
  const obj = require(7214) /* items */;
  const tmp3 = importDefault;
  const orCreate = importDefault(4970).getOrCreate(messageData.message.channelId);
  ...
  if (initialize.hasViewingRoles()) {
    ...
    if (initialize.isViewingRoles(guildId)) {
      if (!getUncachedChannelPermissions.can(constants4.VIEW_CHANNEL, channel)) {
        const tmp11 = new importDefault(4970)(arg0);
        return tmp11;
      }
    }
  }
  return importDefault(4970).getOrCreate(arg0);
};
prototype["getMessage"] = function getMessage(arg0, arg1) {
  const orCreate = importDefault(4970).getOrCreate(arg0);
...
prototype["jumpedMessageId"] = function jumpedMessageId(arg0) {
  const value = importDefault(4970).get(arg0);
  let jumpTargetId;
  if (value != null) {
...
prototype["isReady"] = function isReady(arg0) {
  return importDefault(4970).getOrCreate(arg0).ready;
};
...
prototype["isLoadingMessages"] = function isLoadingMessages(channelId) {
  return importDefault(4970).getOrCreate(channelId).loadingMore;
};
...
const messageStore = new MessageStore(require("dispatcher"), {
  BACKGROUND_SYNC_CHANNEL_MESSAGES: function handleBackgroundSyncChannelMessages(changesByChannelId) {
    ...
    for (const key10012 in changesByChannelId) {
      ...
      let obj = importDefault(4970);
      let value = obj.get(key10012);
      if (null == value) {
        continue;
      } else {
        let tmp = require;
        let _default = require(4975) /* _handleConnectionOpen */.default;
  ...
  LOAD_MESSAGES_SUCCESS: function handleLoadMessagesSuccess(arg0) {
    ...
    const orCreate = importDefault(4970).getOrCreate(channelId);
  ...
  LOAD_MESSAGES_FAILURE: function handleLoadMessagesFailure(channelId) {
    const orCreate = importDefault(4970).getOrCreate(channelId.channelId);
    const obj = importDefault(4970);
    importDefault(4970).commit(orCreate.mutate({ loadingMore: false, error: true }));
  },
  ...
  LOCAL_MESSAGES_LOADED: function handleLocalMessagesLoaded(channelId) {
    ...
    importDefault(4970).commit(mutation);
  },
  ...
  MESSAGE_CREATE: function handleIncomingMessage(isPushNotification) {
    ...
    const obj = importDefault(4970);
    const tmp3 = require;
    const isConnectedResult = require(4975) /* _handleConnectionOpen */.default.isConnected();
  ...
  MESSAGE_SEND_FAILED: function handleSendFailed(channelId) {
    ...
    if (null != orCreate) {
      if (orCreate.has(messageId)) {
        ...
        importDefault(4970).commit(removeResult);
      }
    }
    return false;
  },
  ...
  MESSAGE_UPDATE: function handleMessageUpdate(message) {
    ...
    const orCreate = importDefault(4970).getOrCreate(message.message.channel_id);
    if (null != orCreate) {
      if (orCreate.has(id)) {
        let tmpResult = tmp(4970);
  ...
  MESSAGE_EXPLICIT_CONTENT_SCAN_TIMEOUT: function handleMessageExplicitContentScanTimeout(messageId) {
    ...
    if (null != value) {
      if (value.has(messageId)) {
        const updateResult = value.update(messageId, require(6980) /* redactionSettingToRenderedString */.handleExplicitMediaScanTimeoutForMessage);
        importDefault(4970).commit(updateResult);
      }
    }
    return false;
  ...
  MESSAGE_REVEAL: function handleMessageReveal(arg0) {
    ...
    const orCreate = importDefault(4970).getOrCreate(channelId);
    const obj = importDefault(4970);
    importDefault(4970).commit(orCreate.mutate({ revealedMessageId: messageId }));
  },
  THREAD_CREATE_LOCAL: function handleThreadCreateLocal(channelId) {
    ...
    const obj = importDefault(4970);
    importDefault(4970).commit(complete);
  },
  ...
  THREAD_MEMBER_LIST_UPDATE: function handleThreadMemberListUpdate(guildId) {
    ...
    let item = importDefault(4970).forEach((channelId) => {
      const channel = outer1_9.getChannel(channelId.channelId);
      let guild_id;
      if (channel != null) {
        guild_id = channel.guild_id;
  ...
  UPLOAD_FAIL: function handleUploadFail(messageId) {
    ...
    if (null == messageId) {
      ...
      if (set.has(messageId)) {
        const orCreate = importDefault(4970).getOrCreate(tmp);
