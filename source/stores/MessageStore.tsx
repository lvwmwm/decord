...
function handleConnectionOpen() {
  const item = importDefault(4860).forEach((mutate) => {
    ...
    return (function*(arg0, arg1, arg2) {
      if (c7 === 2) {
        ...
        try {
          ...
          lib(4860).commit(orCreate.receivePushNotification(lib, dependencyMap));
...
function receiveMediaMentionMessage(item10037) {
  ...
  if (null != message_id) {
    ...
    const orCreate = importDefault(4860).getOrCreate(attachment_id);
    let obj = {};
    ...
    tmp5(4860).commit(mutation);
    const tmp5Result = tmp5(4860);
  }
}
function handleCleanup() {
  const item = importDefault(4860).forEach((channelId) => {
...
function handleRelationshipUpdate() {
  let c0 = false;
  const item = importDefault(4860).forEach((reset) => {
...
function handleReaction(optimistic) {
  ...
  const value = importDefault(4860).get(channelId);
  if (null == value) {
    return false;
  } else {
    if (obj3.shouldApplyReaction(optimistic)) {
      ...
      tmp(4860).commit(updateResult);
    } else {
      return false;
    }
    obj3 = _require(3828);
  }
  const obj = importDefault(4860);
  tmp = importDefault;
}
  ...
  if (initialize.hasViewingRoles()) {
    ...
    if (initialize.isViewingRoles(guildId)) {
      if (!getUncachedChannelPermissions.can(constants4.VIEW_CHANNEL, channel)) {
        const tmp11 = new importDefault(4860)(arg0);
        return tmp11;
      }
    }
  }
  return importDefault(4860).getOrCreate(arg0);
};
prototype["getMessage"] = function getMessage(arg0, arg1) {
  const orCreate = importDefault(4860).getOrCreate(arg0);
...
prototype["jumpedMessageId"] = function jumpedMessageId(arg0) {
  const value = importDefault(4860).get(arg0);
  let jumpTargetId;
  if (value != null) {
...
prototype["isReady"] = function isReady(arg0) {
  return importDefault(4860).getOrCreate(arg0).ready;
};
...
prototype["isLoadingMessages"] = function isLoadingMessages(channelId) {
  return importDefault(4860).getOrCreate(channelId).loadingMore;
};
...
const messageStore = new MessageStore(require("dispatcher"), {
  BACKGROUND_SYNC_CHANNEL_MESSAGES: function handleBackgroundSyncChannelMessages(changesByChannelId) {
    ...
    for (const key10012 in changesByChannelId) {
      ...
      if (null == value) {
        ...
        let tmp = require;
        let _default = require(4865) /* _handleConnectionOpen */.default;
  ...
  LOAD_MESSAGES_SUCCESS: function handleLoadMessagesSuccess(arg0) {
    ...
    const orCreate = importDefault(4860).getOrCreate(channelId);
  ...
  LOAD_MESSAGES_FAILURE: function handleLoadMessagesFailure(channelId) {
    const orCreate = importDefault(4860).getOrCreate(channelId.channelId);
    const obj = importDefault(4860);
    importDefault(4860).commit(orCreate.mutate({ loadingMore: false, error: true }));
  },
  ...
  LOCAL_MESSAGES_LOADED: function handleLocalMessagesLoaded(channelId) {
    ...
    importDefault(4860).commit(mutation);
  },
  ...
  MESSAGE_CREATE: function handleIncomingMessage(isPushNotification) {
    ...
    const obj = importDefault(4860);
    const tmp3 = require;
    const isConnectedResult = require(4865) /* _handleConnectionOpen */.default.isConnected();
  ...
  MESSAGE_SEND_FAILED: function handleSendFailed(channelId) {
    ...
    if (null != orCreate) {
      if (orCreate.has(messageId)) {
        ...
        importDefault(4860).commit(removeResult);
      }
    }
    return false;
  },
  ...
  MESSAGE_UPDATE: function handleMessageUpdate(message) {
    ...
    const orCreate = importDefault(4860).getOrCreate(message.message.channel_id);
    if (null != orCreate) {
      if (orCreate.has(id)) {
        let tmpResult = tmp(4860);
        ...
        if (null != message_id) {
          if ("content" in message) {
            ...
            tmp(4860).commit(updateResult1);
            const tmpResult1 = tmp(4860);
          }
        }
      }
    }
  ...
  MESSAGE_REVEAL: function handleMessageReveal(arg0) {
    ...
    const orCreate = importDefault(4860).getOrCreate(channelId);
    const obj = importDefault(4860);
    importDefault(4860).commit(orCreate.mutate({ revealedMessageId: messageId }));
  },
  THREAD_CREATE_LOCAL: function handleThreadCreateLocal(channelId) {
    ...
    const obj = importDefault(4860);
    importDefault(4860).commit(complete);
  },
  ...
  THREAD_MEMBER_LIST_UPDATE: function handleThreadMemberListUpdate(guildId) {
    ...
    let item = importDefault(4860).forEach((channelId) => {
      const channel = outer1_9.getChannel(channelId.channelId);
      let guild_id;
      if (channel != null) {
        guild_id = channel.guild_id;
  ...
  UPLOAD_FAIL: function handleUploadFail(messageId) {
    ...
    if (null == messageId) {
      ...
      if (set.has(messageId)) {
        const orCreate = importDefault(4860).getOrCreate(tmp);
