...
// Dependencies: [1390, 4357, 9798, 5297, 7038, 1340, 1391, 1910, 9799, 4356, 1979, 1385, 1922, 676, 8165, 8163, 3943, 12, 9800, 9802, 5985, 5929, 9803, 9812, 4563, 9813, 9814, 9816, 9808, 9817, 9818, 9822, 4535, 4034, 1306, 8419, 7095, 9823, 4538, 11, 589, 709, 2]

// Module 9797 (handleAlertMessage)
...
function handleEnqueueNotification(notification) {
  ...
  const trackWithMetadata = importDefault(4538).trackWithMetadata;
  if (closure_23) {
...
function trackDismissed(type) {
  ...
  importDefault(4538).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj);
}
...
prototype["dismissNotification"] = function dismissNotification(guild) {
  guild = guild.guild;
  let obj = require(8163) /* DATE_CONFIG */;
  ...
  if (null != incidentAlertType) {
    ...
    obj[0] = guild;
    obj[1] = guild.channel;
    const result = this.dissmissedAlertsMap[incidentAlertType].set(this.key(obj), importDefault(3943)());
...
prototype["wasRecentlyDismissed"] = function wasRecentlyDismissed(guild) {
  ...
  if (null == incidentAlertType) {
    ...
    if (tmp4) {
      tmp4 = importDefault(3943)().diff(importDefault(3943)(value), self.threshold.unitOfTime) < self.threshold.amount;
...
const inAppNotificationStore = new InAppNotificationStore(require("dispatcher"), {
  POST_CONNECTION_OPEN: function handlePostConnectionOpen() {
    ...
    let closure_25 = require(9800) /* isOnlyDayLoss */.toScheduleSnapshot(tmp3);
    handleAlertMessage();
  },
  LOGOUT: function handleLogout() {
    let c24 = false;
    const EMPTY_SCHEDULE_SNAPSHOT = require(9800) /* isOnlyDayLoss */.EMPTY_SCHEDULE_SNAPSHOT;
    let c21 = null;
    obj1.removeAll(() => true);
  },
  ...
  MESSAGE_CREATE: function handleIncomingMessage(message) {
    ...
    let obj = require(9802) /* canViewPotentiallyNSFWChannel */;
  ...
  MESSAGE_REQUEST_NOTIFICATION_SENT: function handleMessageRequest(triggeringUserId) {
    ...
    if (obj.getConfig({ location: "inAppNotificationStore" }).enabled) {
      ...
      if (null == user) {
        ...
        obj[4] = require(9814) /* extractMetadataFromNotification */.getNotificationDuration(MESSAGE_REQUEST);
        obj[5] = function onDismiss() {
          callback(table[27]).clearNotification();
        };
        const obj4 = require(9814) /* extractMetadataFromNotification */;
        obj[6] = require(9814) /* extractMetadataFromNotification */.generateInAppNotificationId();
        obj[0] = obj;
        handleEnqueueNotification(obj);
      }
    } else {
  ...
  RESTRICTED_HOURS_STATE_CHANGE: function handleRestrictedHoursStateChange(isInRestrictedHours) {
    ...
    if (isInRestrictedHours) {
      if (null != _null) {
        const result = require(9814) /* extractMetadataFromNotification */.extractMetadataFromNotification(_null);
        ({ guildId, channelId, messageId } = result);
  ...
  USER_SETTINGS_PROTO_UPDATE: function handleUserSettingsUpdate() {
    ...
    let guildId;
    let messageId;
    let obj = require(9803) /* shouldNotifyBase */;
    const result = obj.allowInAppNotifications();
    let flag = !result;
    if (!result) {
      if (null != _null) {
        const result1 = require(9814) /* extractMetadataFromNotification */.extractMetadataFromNotification(_null);
        ({ guildId, channelId, messageId } = result1);
        const tmpResult = require(9814) /* extractMetadataFromNotification */;
