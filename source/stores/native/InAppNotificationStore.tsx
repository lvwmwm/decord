...
// Dependencies: [1390, 4316, 9757, 5258, 6999, 1340, 1391, 1910, 9758, 4315, 1979, 1385, 1922, 676, 8126, 8124, 3902, 12, 9759, 9761, 5946, 5890, 9762, 9771, 4523, 9772, 9773, 9775, 9767, 9776, 9777, 9781, 4495, 3993, 1306, 8380, 7056, 9782, 4498, 11, 589, 709, 2]

// Module 9756 (handleAlertMessage)
...
prototype["dismissNotification"] = function dismissNotification(guild) {
  guild = guild.guild;
  let obj = require(8124) /* DATE_CONFIG */;
  ...
  if (null != incidentAlertType) {
    ...
    obj[0] = guild;
    obj[1] = guild.channel;
    const result = this.dissmissedAlertsMap[incidentAlertType].set(this.key(obj), importDefault(3902)());
...
prototype["wasRecentlyDismissed"] = function wasRecentlyDismissed(guild) {
  ...
  if (null == incidentAlertType) {
    ...
    if (tmp4) {
      tmp4 = importDefault(3902)().diff(importDefault(3902)(value), self.threshold.unitOfTime) < self.threshold.amount;
...
const inAppNotificationStore = new InAppNotificationStore(require("dispatcher"), {
  POST_CONNECTION_OPEN: function handlePostConnectionOpen() {
    ...
    let closure_25 = require(9759) /* isOnlyDayLoss */.toScheduleSnapshot(tmp3);
    handleAlertMessage();
  },
  LOGOUT: function handleLogout() {
    let c24 = false;
    const EMPTY_SCHEDULE_SNAPSHOT = require(9759) /* isOnlyDayLoss */.EMPTY_SCHEDULE_SNAPSHOT;
    let c21 = null;
    obj1.removeAll(() => true);
  },
  ...
  MESSAGE_CREATE: function handleIncomingMessage(message) {
    ...
    let obj = require(9761) /* canViewPotentiallyNSFWChannel */;
  ...
  MESSAGE_REQUEST_NOTIFICATION_SENT: function handleMessageRequest(triggeringUserId) {
    ...
    if (obj.getConfig({ location: "inAppNotificationStore" }).enabled) {
      ...
      if (null == user) {
        ...
        obj[4] = require(9773) /* extractMetadataFromNotification */.getNotificationDuration(MESSAGE_REQUEST);
        obj[5] = function onDismiss() {
          callback(table[27]).clearNotification();
        };
        const obj4 = require(9773) /* extractMetadataFromNotification */;
        obj[6] = require(9773) /* extractMetadataFromNotification */.generateInAppNotificationId();
        obj[0] = obj;
        handleEnqueueNotification(obj);
      }
    } else {
  ...
  RESTRICTED_HOURS_STATE_CHANGE: function handleRestrictedHoursStateChange(isInRestrictedHours) {
    ...
    if (isInRestrictedHours) {
      if (null != _null) {
        const result = require(9773) /* extractMetadataFromNotification */.extractMetadataFromNotification(_null);
        ({ guildId, channelId, messageId } = result);
  ...
  USER_SETTINGS_PROTO_UPDATE: function handleUserSettingsUpdate() {
    ...
    let guildId;
    let messageId;
    let obj = require(9762) /* shouldNotifyBase */;
    const result = obj.allowInAppNotifications();
    let flag = !result;
    if (!result) {
      if (null != _null) {
        const result1 = require(9773) /* extractMetadataFromNotification */.extractMetadataFromNotification(_null);
        ({ guildId, channelId, messageId } = result1);
        const tmpResult = require(9773) /* extractMetadataFromNotification */;
