...
// Dependencies: [1371, 4316, 9751, 5258, 6996, 1340, 1372, 1891, 9752, 4315, 1960, 1366, 1903, 676, 8124, 8122, 3883, 12, 9753, 9755, 5944, 5890, 9756, 9765, 4523, 9766, 9767, 9769, 9761, 9770, 9771, 9775, 4495, 3974, 1306, 8375, 7053, 9776, 4498, 11, 589, 709, 2]

// Module 9750 (handleAlertMessage)
...
function handleEnqueueNotification(notification) {
  ...
  const trackWithMetadata = importDefault(4498).trackWithMetadata;
  if (closure_23) {
...
function trackDismissed(type) {
  ...
  importDefault(4498).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj);
}
...
prototype["dismissNotification"] = function dismissNotification(guild) {
  guild = guild.guild;
  let obj = require(8122) /* DATE_CONFIG */;
...
const inAppNotificationStore = new InAppNotificationStore(require("dispatcher"), {
  POST_CONNECTION_OPEN: function handlePostConnectionOpen() {
    ...
    let closure_25 = require(9753) /* isOnlyDayLoss */.toScheduleSnapshot(tmp3);
    handleAlertMessage();
  },
  LOGOUT: function handleLogout() {
    let c24 = false;
    const EMPTY_SCHEDULE_SNAPSHOT = require(9753) /* isOnlyDayLoss */.EMPTY_SCHEDULE_SNAPSHOT;
    let c21 = null;
    obj1.removeAll(() => true);
  },
  ...
  MESSAGE_CREATE: function handleIncomingMessage(message) {
    ...
    let obj = require(9755) /* canViewPotentiallyNSFWChannel */;
  ...
  MESSAGE_REQUEST_NOTIFICATION_SENT: function handleMessageRequest(triggeringUserId) {
    ...
    if (obj.getConfig({ location: "inAppNotificationStore" }).enabled) {
      ...
      if (null == user) {
        ...
        obj[4] = require(9767) /* extractMetadataFromNotification */.getNotificationDuration(MESSAGE_REQUEST);
        obj[5] = function onDismiss() {
          callback(table[27]).clearNotification();
        };
        const obj4 = require(9767) /* extractMetadataFromNotification */;
        obj[6] = require(9767) /* extractMetadataFromNotification */.generateInAppNotificationId();
        obj[0] = obj;
        handleEnqueueNotification(obj);
      }
    } else {
  ...
  MESSAGE_REMINDER_DUE: function handleMessageReminderDue(savedMessage) {
    ...
    let obj = require(8375) /* ForLaterFreemiumConfig */;
  ...
  RESTRICTED_HOURS_STATE_CHANGE: function handleRestrictedHoursStateChange(isInRestrictedHours) {
    ...
    if (isInRestrictedHours) {
      if (null != _null) {
        const result = require(9767) /* extractMetadataFromNotification */.extractMetadataFromNotification(_null);
        ({ guildId, channelId, messageId } = result);
  ...
  USER_SETTINGS_PROTO_UPDATE: function handleUserSettingsUpdate() {
    ...
    let guildId;
    let messageId;
    let obj = require(9756) /* shouldNotifyBase */;
    const result = obj.allowInAppNotifications();
    let flag = !result;
    if (!result) {
      if (null != _null) {
        const result1 = require(9767) /* extractMetadataFromNotification */.extractMetadataFromNotification(_null);
        ({ guildId, channelId, messageId } = result1);
        const tmpResult = require(9767) /* extractMetadataFromNotification */;
