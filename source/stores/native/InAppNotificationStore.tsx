...
// Dependencies: [1390, 4773, 10014, 6796, 5296, 1340, 1391, 1910, 10015, 4772, 1979, 1385, 1922, 676, 8236, 8234, 3975, 12, 10016, 10018, 5267, 5385, 10019, 10027, 4803, 10028, 10029, 10031, 10023, 10032, 10033, 10037, 4488, 4066, 1306, 8484, 5364, 10038, 5042, 11, 589, 709, 2]
...
function handleEnqueueNotification(notification) {
  ...
  const trackWithMetadata = importDefault(5042).trackWithMetadata;
  if (closure_23) {
...
function trackDismissed(type) {
  ...
  importDefault(5042).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj);
}
...
prototype["dismissNotification"] = function dismissNotification(guild) {
  guild = guild.guild;
  let obj = require(8234) /* DATE_CONFIG */;
  ...
  if (null != incidentAlertType) {
    ...
    obj[0] = guild;
    obj[1] = guild.channel;
    const result = this.dissmissedAlertsMap[incidentAlertType].set(this.key(obj), importDefault(3975)());
...
prototype["wasRecentlyDismissed"] = function wasRecentlyDismissed(guild) {
  ...
  if (null == incidentAlertType) {
    ...
    if (tmp4) {
      tmp4 = importDefault(3975)().diff(importDefault(3975)(value), self.threshold.unitOfTime) < self.threshold.amount;
...
const inAppNotificationStore = new InAppNotificationStore(require("dispatcher"), {
  POST_CONNECTION_OPEN: function handlePostConnectionOpen() {
    ...
    let closure_25 = require(10016) /* isOnlyDayLoss */.toScheduleSnapshot(tmp3);
    handleAlertMessage();
  },
  LOGOUT: function handleLogout() {
    let c24 = false;
    const EMPTY_SCHEDULE_SNAPSHOT = require(10016) /* isOnlyDayLoss */.EMPTY_SCHEDULE_SNAPSHOT;
    let c21 = null;
    obj1.removeAll(() => true);
  },
  ...
  MESSAGE_CREATE: function handleIncomingMessage(message) {
    ...
    let obj = require(10018) /* canViewPotentiallyNSFWChannel */;
      ...
      if (tmpResult.shouldShowSpoilerGateForChannelId(channel_id)) {
        ...
        if (!message.optimistic) {
          ...
          if (tmpResult.allowInAppNotifications()) {
            ...
            tmp3Result = importDefault(10027);
          }
        }
        return false;
      }
  ...
  MESSAGE_REQUEST_NOTIFICATION_SENT: function handleMessageRequest(triggeringUserId) {
    ...
    if (obj.getConfig({ location: "inAppNotificationStore" }).enabled) {
      ...
      if (null == user) {
        ...
        obj[4] = require(10029) /* isReactionMilestoneNotification */.getNotificationDuration(MESSAGE_REQUEST);
        obj[5] = function onDismiss() {
          callback(table[27]).clearNotification();
        };
        const obj4 = require(10029) /* isReactionMilestoneNotification */;
        obj[6] = require(10029) /* isReactionMilestoneNotification */.generateInAppNotificationId();
        obj[0] = obj;
        handleEnqueueNotification(obj);
      }
    } else {
  ...
  RESTRICTED_HOURS_STATE_CHANGE: function handleRestrictedHoursStateChange(isInRestrictedHours) {
    ...
    if (isInRestrictedHours) {
      if (null != _null) {
        const result = require(10029) /* isReactionMilestoneNotification */.extractMetadataFromNotification(_null);
        ({ guildId, channelId, messageId } = result);
  ...
  USER_SETTINGS_PROTO_UPDATE: function handleUserSettingsUpdate() {
    ...
    let guildId;
    let messageId;
    let obj = require(10019) /* shouldNotifyBase */;
    const result = obj.allowInAppNotifications();
    let flag = !result;
    if (!result) {
      if (null != _null) {
        const result1 = require(10029) /* isReactionMilestoneNotification */.extractMetadataFromNotification(_null);
        ({ guildId, channelId, messageId } = result1);
        const tmpResult = require(10029) /* isReactionMilestoneNotification */;
