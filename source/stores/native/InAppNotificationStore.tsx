...
// Dependencies: [1385, 4497, 10009, 7055, 7470, 1339, 1386, 1908, 10010, 4496, 1980, 1380, 1921, 673, 7948, 7946, 4074, 12, 10011, 10013, 7267, 7210, 10014, 9539, 4745, 10022, 10023, 10025, 10018, 10026, 10027, 10031, 4682, 4166, 1305, 7787, 7527, 10032, 4708, 11, 586, 706, 2]

// Module 10008 (handleAlertMessage)
...
import isOnlyDayLoss from "isOnlyDayLoss" /* 10011 */;
import canViewPotentiallyNSFWChannel from "canViewPotentiallyNSFWChannel" /* 10013 */;
import shouldNotifyBase from "shouldNotifyBase" /* 10014 */;
...
import playInAppMessageSound from "playInAppMessageSound" /* 10032 */;
import closure_3 from "participantFromServer" /* 1385 */;
import closure_4 from "getParticipants" /* 4497 */;
import closure_5 from "computeAlertSettings" /* 10009 */;
import { trackMessageNotificationTimestamps as closure_6 } from "initialize" /* 7055 */;
import importDefaultResult from "freshTeenActivityWithMap" /* 7470 */;
...
import { EMPTY_SCHEDULE_SNAPSHOT } from "isOnlyDayLoss" /* 10011 */;

require = arg1;
function handleAlertMessage() {
  obj = guild(10014);
  if (obj.allowInAppNotifications()) {
    let tmpResult = tmp(10027);
    ...
    if (result.show) {
      if (null != guildId) {
        ...
        if (null == guild) {
          ...
          channel = store2.getChannel(channel(10031)(guild));
          if (null == channel) {
            return false;
            ...
            if (obj.wasRecentlyDismissed(obj)) {
              ...
              obj1[4] = channel;
              obj1[5] = guild;
              obj1[6] = tmp(10023).generateInAppNotificationId();
              obj[0] = obj1;
...
const inAppNotificationStore = new InAppNotificationStore(dispatcherDefault, {
          ...
          if (tmpResult.allowInAppNotifications()) {
            if (!tmp3Result.isEnabled()) {
              if (!uiStore.getChatOpen(channel_id)) {
                const result = tmp(10014).shouldIncludeSelectedChannel();
                const tmpResult1 = tmp(10014);
                if (tmpResult2.shouldNotify(message, channel_id, result)) {
                  const channel = store2.getChannel(channel_id);
                  if (null == channel) {
                    return false;
                  } else {
                    const messageRecord = tmp(4745).createMessageRecord(message);
                    const tmpResult3 = tmp(4745);
                    if (tmpResult4.isMessageContentPreviewable(messageRecord)) {
                      ...
                      obj[0] = MESSAGE;
                      const notificationDuration = tmp(10023).getNotificationDuration(MESSAGE);
                      ...
                      const tmpResult5 = tmp(10023);
                      obj[8] = tmp(10023).generateInAppNotificationId();
                    ...
                    tmpResult4 = tmp(10022);
                  }
                  obj7 = store2;
                } else {
                  return false;
                }
                tmpResult2 = tmp(10014);
              }
            }
  ...
  MESSAGE_ACK: function handleMessageAck(channelId) {
    ...
    if (tmp) {
      const result = channelId(10023).extractMetadataFromNotification(_null);
      ({ guildId, channelId: channelId2, messageId: messageId2 } = result);
      const obj2 = channelId(10023);
      ...
      messageId(4708).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj);
      const obj3 = messageId(4708);
    }
                  ...
                  if (null != channel) {
                    if (!obj4.isEnabled()) {
                      if (!uiStore.getChatOpen(channelId)) {
                        const result1 = tmp19(10014).shouldIncludeSelectedChannel();
                        const tmp19Result1 = tmp19(10014);
                        ...
                        if (tmp19Result2.shouldNotifyForReaction(obj)) {
                          const messageRecord = tmp19(4745).createMessageRecord(message);
                          const tmp19Result3 = tmp19(4745);
                          if (tmp19Result4.isMessageContentPreviewable(messageRecord)) {
                            ...
                            obj1[0] = REACTION;
                            obj1[1] = channelId;
                            obj1[2] = tmp19(10023).getNotificationDuration(REACTION);
                            obj1[3] = function onDismiss() {
                              callback(table[27]).clearNotification();
                            };
                            obj1[4] = channel;
                            ...
                            obj1[9] = found;
                            const tmp19Result5 = tmp19(10023);
                            obj1[10] = tmp19(10023).generateInAppNotificationId();
                            obj[0] = obj1;
                            handleEnqueueNotification(obj);
                          } else {
                            return false;
                          }
                          tmp19Result4 = tmp19(10022);
                        } else {
                          return false;
                        }
                        tmp19Result2 = tmp19(10014);
                      }
                    }
                    return false;
                  }
            ...
            if (tmpResult.shouldNotifyForForumThreadCreation(channel, channel, !tmpResult.isChannelFocused())) {
              ...
              if (null == user) {
                ...
                if (null == guild) {
                  ...
                  obj[4] = guild;
                  obj[5] = channel.id;
                  obj[6] = tmp(10023).getNotificationDuration(FORUM_THREAD_CREATED);
                  obj[7] = function onDismiss() {
                    callback(table[27]).clearNotification();
                  };
                  const tmpResult1 = tmp(10023);
                  obj[8] = tmp(10023).generateInAppNotificationId();
                  obj[0] = obj;
                  handleEnqueueNotification(obj);
                }
  ...
  CHANNEL_RTC_UPDATE_CHAT_OPEN: function handleVoiceChatOpen(channelId) {
    ...
    if (tmp) {
      ...
      obj[3] = messageId;
      obj[5] = _null.inAppNotificationId;
      chatOpen(4708).trackWithMetadata(constants.IN_APP_NOTIFICATION_DISMISSED, obj);
      const obj2 = chatOpen(4708);
    }
    obj1.removeAll(function predicate(type) {
      let tmp = type.type === closure_1_18.MESSAGE;
      if (tmp) {
        ...
        if (null != diffSchedulesResult) {
          ...
          if (EnableScreenDowntimeScheduleNotifications.getSetting()) {
            if (tmpResult1.allowInAppNotifications()) {
              ...
              const notificationDuration = tmp(10023).getNotificationDuration(RESTRICTED_SCHEDULE_UPDATED);
              const tmpResult2 = tmp(10023);
              obj[1] = tmp(10011).restrictedScheduleNotificationKey(diffSchedulesResult.kind);
              obj[2] = notificationDuration;
              obj[3] = function onDismiss() {
                callback(table[27]).clearNotification();
              };
              const tmpResult3 = tmp(10011);
              obj[4] = tmp(10011).getRestrictedScheduleNotificationTitle(diffSchedulesResult.kind);
              const tmpResult4 = tmp(10011);
              obj[5] = tmp(10011).getRestrictedScheduleNotificationSubtitle(diffSchedulesResult.rule);
              const tmpResult5 = tmp(10011);
              obj[6] = tmp(10023).generateInAppNotificationId();
              obj[0] = obj;
              handleEnqueueNotification(obj);
              const tmpResult6 = tmp(10023);
            }
            tmpResult1 = tmp(10014);
          }
        }
      } else {
        return false;
