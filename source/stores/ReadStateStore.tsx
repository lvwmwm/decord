...
// Dependencies: [5, 32, 1390, 4358, 1981, 4256, 7055, 5902, 5903, 7168, 5285, 3991, 1340, 1395, 1218, 5960, 1391, 5051, 4704, 1910, 5190, 4584, 3989, 3998, 1979, 4550, 1922, 13264, 676, 9820, 1398, 1397, 4551, 1235, 3, 13265, 13266, 11, 687, 709, 4600, 13267, 530, 4227, 10462, 9823, 3997, 13268, 7064, 7165, 13269, 2007, 1403, 3943, 12, 1370, 5055, 4198, 9822, 3996, 1404, 589, 6944, 8999, 2]
...
function shouldBadgeMessage(channel_id, id) {
  ...
  if (tmp) {
    ...
    if (!result) {
      ...
      let tmp8Result = importDefault(4600)(obj);
      if (!tmp8Result) {
          ...
          if (null != messages) {
            if (messages.ready) {
              if (!messages.loadingMore) {
                const result = require(9823) /* getFocusedChannelId */.isChannelFocusedForReadStateAck(value.channelId, arg1);
                ...
                const obj10 = require(9823) /* getFocusedChannelId */;
              }
              return flag6;
            }
          }
  ...
  if (this.type !== ReadStateTypes.CHANNEL) {
    ...
    if (self._isResourceChannel) {
      ...
      if (null == basicPermissions) {
        ...
        let hasItem = importDefault(3997).has(basicPermissions, BasicPermissions.VIEW_CHANNEL);
...
prototype2["canBeUnread"] = function canBeUnread() {
  ...
  obj = require(13268) /* filterOutMessageRequestsAndSpam */;
  tmp = require;
};
prototype2["canHaveMentions"] = function canHaveMentions() {
  ...
  if (tmp) {
    ...
    if (!(self._isThread && !self._isJoinedThread)) {
      const items = [processChannel, closure_12];
      const result = require(13268) /* filterOutMessageRequestsAndSpam */.isMessageRequestOrSpamRequest(self.channelId, items);
      let tmp9 = !result;
      if (!result) {
      ...
      tmp3 = tmp9;
      const obj = require(13268) /* filterOutMessageRequestsAndSpam */;
      tmp4 = require;
    }
    tmp = tmp3;
  }
...
prototype2["takeSnapshot"] = function takeSnapshot() {
  const _default = require(7165) /* updateGuildUnreadSentinel */.default;
...
prototype2["_ack"] = function _ack(closure_1, c0) {
  ...
  if (null != outgoingAck) {
    ...
    let promise = importDefault(13267)(() => {
      const HTTP = tmp4(530).HTTP;
      obj = { url: outer1_39.MESSAGE_ACK(self.channelId, outgoingAck), body: obj, oldFormErrors: true, rejectWithError: true };
      obj = { token: outer1_63, last_viewed: self.lastViewed, flags: tmp4 };
      return HTTP.post(obj);
...
prototype3["initialize"] = function initialize() {
  ...
  items.push(require(6944) /* getState */.default);
...
obj = {
  ...
  LOCAL_MESSAGES_LOADED: function handleLocalMessagesLoaded(messages) {
    ...
    obj = require(5055) /* isIOSPushNotificationRawPayloadFixExperimentEnabled */;
  },
