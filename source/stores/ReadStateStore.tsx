...
// Dependencies: [5, 32, 1371, 4298, 1962, 4199, 6906, 5762, 5763, 7021, 5207, 3914, 1340, 1376, 1218, 5819, 1372, 4969, 4622, 1891, 5110, 4502, 3912, 3921, 1960, 4480, 1903, 13099, 676, 9676, 1379, 1378, 4481, 1235, 3, 13100, 13101, 11, 687, 709, 4518, 13102, 530, 4170, 10387, 9679, 3920, 13103, 6915, 7018, 13104, 1988, 1384, 3866, 12, 1351, 4973, 4136, 9678, 3919, 1385, 589, 6797, 8859, 2]

// Module 4297 (generateOldThreadCutoff)
          ...
          if (null != messages) {
            if (messages.ready) {
              if (!messages.loadingMore) {
                const result = require(9679) /* getFocusedChannelId */.isChannelFocusedForReadStateAck(value.channelId, arg1);
                ...
                const obj10 = require(9679) /* getFocusedChannelId */;
              }
              return flag6;
            }
          }
  ...
  if (this.type !== ReadStateTypes.CHANNEL) {
    ...
    if (self._isResourceChannel) {
      ...
      if (null == basicPermissions) {
        ...
        let hasItem = importDefault(3920).has(basicPermissions, BasicPermissions.VIEW_CHANNEL);
...
prototype2["canBeUnread"] = function canBeUnread() {
  ...
  obj = require(13103) /* filterOutMessageRequestsAndSpam */;
  tmp = require;
};
prototype2["canHaveMentions"] = function canHaveMentions() {
  ...
  if (tmp) {
    ...
    if (!(self._isThread && !self._isJoinedThread)) {
      const items = [processChannel, closure_12];
      const result = require(13103) /* filterOutMessageRequestsAndSpam */.isMessageRequestOrSpamRequest(self.channelId, items);
      let tmp9 = !result;
      if (!result) {
      ...
      tmp3 = tmp9;
      const obj = require(13103) /* filterOutMessageRequestsAndSpam */;
      tmp4 = require;
    }
    tmp = tmp3;
  }
...
prototype2["takeSnapshot"] = function takeSnapshot() {
  const _default = require(7018) /* updateGuildUnreadSentinel */.default;
...
prototype2["_ack"] = function _ack(closure_1, c0) {
  ...
  if (null != outgoingAck) {
    ...
    let promise = importDefault(13102)(() => {
      const HTTP = tmp4(530).HTTP;
      obj = { url: outer1_39.MESSAGE_ACK(self.channelId, outgoingAck), body: obj, oldFormErrors: true, rejectWithError: true };
      obj = { token: outer1_63, last_viewed: self.lastViewed, flags: tmp4 };
      return HTTP.post(obj);
...
prototype3["initialize"] = function initialize() {
  ...
  items.push(require(6797) /* getState */.default);
...
obj = {
  ...
  LOCAL_MESSAGES_LOADED: function handleLocalMessagesLoaded(messages) {
    ...
    obj = require(4973) /* isIOSPushNotificationRawPayloadFixExperimentEnabled */;
  },
  MESSAGE_CREATE: function handleIncomingMessage(isPushNotification) {
    ...
    let obj6 = require(4136) /* getRootNavigationRef */;
