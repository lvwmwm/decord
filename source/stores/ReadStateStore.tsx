...
// Dependencies: [5, 32, 1371, 4315, 1962, 4216, 6925, 5781, 5782, 7040, 5223, 3931, 1340, 1376, 1218, 5838, 1372, 4986, 4639, 1891, 5127, 4519, 3929, 3938, 1960, 4497, 1903, 13113, 676, 9692, 1379, 1378, 4498, 1235, 3, 13114, 13115, 11, 687, 709, 4535, 13116, 530, 4187, 10403, 9695, 3937, 13117, 6934, 7037, 13118, 1988, 1384, 3883, 12, 1351, 4990, 4153, 9694, 3936, 1385, 589, 6816, 8876, 2]

// Module 4314 (generateOldThreadCutoff)
...
function shouldBadgeMessage(channel_id, id) {
  ...
  if (tmp) {
    ...
    if (!result) {
      ...
      let tmp8Result = importDefault(4535)(obj);
      if (!tmp8Result) {
          ...
          if (null != messages) {
            if (messages.ready) {
              if (!messages.loadingMore) {
                const result = require(9695) /* getFocusedChannelId */.isChannelFocusedForReadStateAck(value.channelId, arg1);
                ...
                const obj10 = require(9695) /* getFocusedChannelId */;
              }
              return flag6;
            }
          }
...
prototype2["canBeUnread"] = function canBeUnread() {
  ...
  obj = require(13117) /* filterOutMessageRequestsAndSpam */;
  tmp = require;
};
prototype2["canHaveMentions"] = function canHaveMentions() {
  ...
  if (tmp) {
    ...
    if (!(self._isThread && !self._isJoinedThread)) {
      const items = [processChannel, closure_12];
      const result = require(13117) /* filterOutMessageRequestsAndSpam */.isMessageRequestOrSpamRequest(self.channelId, items);
      let tmp9 = !result;
      if (!result) {
      ...
      tmp3 = tmp9;
      const obj = require(13117) /* filterOutMessageRequestsAndSpam */;
      tmp4 = require;
    }
    tmp = tmp3;
  }
...
prototype2["takeSnapshot"] = function takeSnapshot() {
  const _default = require(7037) /* updateGuildUnreadSentinel */.default;
...
prototype2["_ack"] = function _ack(closure_1, c0) {
  ...
  if (null != outgoingAck) {
    ...
    let promise = importDefault(13116)(() => {
      const HTTP = tmp4(530).HTTP;
      obj = { url: outer1_39.MESSAGE_ACK(self.channelId, outgoingAck), body: obj, oldFormErrors: true, rejectWithError: true };
      obj = { token: outer1_63, last_viewed: self.lastViewed, flags: tmp4 };
      return HTTP.post(obj);
...
prototype3["initialize"] = function initialize() {
  ...
  items.push(require(6816) /* getState */.default);
...
obj = {
  ...
  LOCAL_MESSAGES_LOADED: function handleLocalMessagesLoaded(messages) {
    ...
    obj = require(4990) /* isIOSPushNotificationRawPayloadFixExperimentEnabled */;
  },
  MESSAGE_CREATE: function handleIncomingMessage(isPushNotification) {
    ...
    let obj6 = require(4153) /* getRootNavigationRef */;
