...
// Dependencies: [32, 5, 4333, 4338, 4199, 4305, 4353, 1340, 1218, 4354, 4355, 1372, 4356, 1903, 13011, 676, 4349, 4497, 13012, 685, 13013, 4345, 3, 687, 500, 4381, 4170, 4842, 13014, 13015, 13016, 10232, 13017, 12, 12998, 13018, 13019, 13020, 13021, 13022, 13023, 13024, 9092, 698, 709, 10783, 13025, 13026, 13027, 5120, 13028, 13073, 13074, 13075, 13076, 13077, 13078, 4344, 4460, 4279, 3895, 595, 13079, 10770, 5331, 13083, 13084, 1236, 4603, 13085, 13086, 13087, 13088, 38, 13010, 13009, 13089, 12993, 1208, 4342, 9683, 589, 13090, 13091, 13092, 13093, 2]

// Module 4332 (_detectH265HardwareDecode)
...
function getSettings() {
  ...
  if (null == obj1.vadDuringPreProcess) {
    obj1.vadDuringPreProcess = require(13015) /* getVADBeforeProcessingExperimentConfig */.getVADBeforeProcessingExperimentConfig({ location: "getSettings" }).enabled;
    const obj7 = require(13015) /* getVADBeforeProcessingExperimentConfig */;
  }
  ...
  const aGC2ExperimentConfig = require(13016) /* getAGC2ExperimentConfig */.getAGC2ExperimentConfig({ location: "getSettings" });
...
function setInputMode(context) {
  ...
  if (context.context === MediaEngineContextTypes.DEFAULT) {
    let obj = require(10232) /* setPushToTalkState */;
    obj.setPushToTalkState(false, false);
  }
  let showPTTSpeakingIndicator = importDefault(13017).getConfig({ location: "setInputMode" }).showPTTSpeakingIndicator;
...
function updateConnectionMuteDeaf(context) {
  ...
  if (MediaEngineContextTypes.DEFAULT === context) {
    ...
    if (!tmp3) {
      tmp3 = !importDefault(4842).didHavePermission(NativePermissionTypes.AUDIO);
      const obj = importDefault(4842);
    }
    let flag = tmp3;
  } else {
    flag = true;
  ...
  if (context.context === MediaEngineContextTypes.DEFAULT) {
    importDefault(12998).updateNativeMute();
    const obj2 = importDefault(12998);
  }
}
      ...
      if (!tmp) {
        ...
        if (null != tmp2) {
          ...
          if (null != tmp2.desktopSource) {
            ...
            const obj20 = require(13014) /* HdrCaptureMode */;
            let isWindowsResult = require(500) /* set */.isWindows();
            if (isWindowsResult) {
              const tmp41 = importDefault(3895);
              let release;
              if (tmp41 != null) {
                release = tmp41.os.release;
              }
              isWindowsResult = importDefault(13028).satisfies(release, closure_29);
              const obj6 = importDefault(13028);
            }
            if (!isWindowsResult) {
              ...
              if (!isWindowsResult1) {
                ...
                if (isMacResult) {
                  const tmp61 = importDefault(3895);
                  let release1;
                  if (tmp61 != null) {
                    release1 = tmp61.os.release;
                  }
                  isMacResult = importDefault(13028).satisfies(release1, closure_24);
                  const obj14 = importDefault(13028);
                }
              ...
              if (isWindowsResult2) {
                const tmp47 = importDefault(3895);
                let release2;
                if (tmp47 != null) {
                  release2 = tmp47.os.release;
                }
                isWindowsResult2 = importDefault(13028).satisfies(release2, closure_27);
                const obj8 = importDefault(13028);
              }
...
function noiseCancellerErrorToAVUnderlyingError(noise_canceller_error) {
  if (NoiseCancellerError.KRISP_CPU_OVERUSE === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispCpuOveruse;
  } else if (tmp.KRISP_FAILED === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispFailed;
  } else if (tmp.KRISP_VAD_CPU_OVERUSE === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispVadCpuOveruse;
  } else if (tmp.KRISP_INIT_ERROR === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispInitError;
  } else if (tmp.KRISP_INIT_ERROR_NATIVE === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispInitErrorNative;
  } else if (tmp.KRISP_INIT_ERROR_SSE4_NOT_SUPPORTED === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispInitErrorSse4NotSupported;
  } else if (tmp.KRISP_INIT_ERROR_AVX2_NOT_SUPPORTED === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispInitErrorAvx2NotSupported;
  } else if (tmp.KRISP_INIT_ERROR_UNSIGNED === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispInitErrorUnsigned;
  } else if (tmp.KRISP_INIT_ERROR_GLOBAL_INIT === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispInitErrorGlobalInit;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_8K === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispInitErrorWeight8k;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_16K === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispInitErrorWeight16k;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_32K === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispInitErrorWeight32k;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_VAD === noise_canceller_error) {
    return require(9092) /* mapped */.AVUnderlyingError.KrispInitErrorWeightVad;
  }
}
function handleVoiceProcessingError(noise_canceller_error) {
  obj.warn("Voice processing error: " + noise_canceller_error);
  obj = require(9092) /* mapped */;
  obj = { type: require(9092) /* mapped */.AVError.NOISE_CANCELLER_ERROR, underlyingError: noiseCancellerErrorToAVUnderlyingError(noise_canceller_error) };
...
function handleVideoFilterError(arg0, arg1) {
  ...
  if (arg1 !== constants10.LIVE) {
    ...
    obj[0] = require(9092) /* mapped */.AVError.VIDEO_BACKGROUND_UNAVAILABLE;
...
function updateConnectionVoiceProcessing(setEchoCancellation) {
  ...
  const merged = Object.assign(require(13016) /* getAGC2ExperimentConfig */.getAGC2ExperimentConfig(obj).noiseCancellationConfig);
  const result = setEchoCancellation.setAutomaticGainControl(obj);
  const noiseCancellation = tmp2.noiseCancellation;
  const obj3 = require(13016) /* getAGC2ExperimentConfig */;
  const tmp13Result = importDefault(10783)(noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
  ...
  const tmp13 = importDefault(10783);
...
function trackVoiceProcessing(location) {
  ...
  const tmp11 = importDefault(10783);
  const tmp11Result = importDefault(10783)(tmp.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
...
function setLoopback() {
  ...
  const tmp15 = importDefault(10783);
  const tmp15Result = importDefault(10783)(tmpResult.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
...
prototype["isSelfMute"] = function isSelfMute(DEFAULT) {
  ...
  if (!mute) {
    mute = !importDefault(4842).didHavePermission(NativePermissionTypes.AUDIO);
    const obj = importDefault(4842);
  }
  if (!mute) {
...
prototype["supportsExperimentalSoundshare"] = function supportsExperimentalSoundshare() {
  ...
  if (supportsResult) {
    const tmp4 = importDefault(3895);
    let release;
    if (tmp4 != null) {
      release = tmp4.os.release;
    }
    supportsResult = importDefault(13028).satisfies(release, closure_31);
    const obj = importDefault(13028);
  }
  return supportsResult;
};
prototype["supportsHookSoundshare"] = function supportsHookSoundshare() {
  ...
  if (isWindowsResult) {
    const tmp6 = importDefault(3895);
    let release;
    if (tmp6 != null) {
      release = tmp6.os.release;
    }
    isWindowsResult = importDefault(13028).satisfies(release, closure_30);
    const obj2 = importDefault(13028);
  }
...
prototype["supportsScreenSoundshare"] = function supportsScreenSoundshare() {
  if (obj.isMac()) {
    ...
    if (supportsResult) {
      const tmp12 = importDefault(3895);
      let release;
      if (tmp12 != null) {
        release = tmp12.os.release;
      }
      supportsResult = importDefault(13028).satisfies(release, closure_25);
      const obj5 = importDefault(13028);
    }
    if (supportsResult) {
      let tmpResult = tmp(500);
      let satisfiesResult = tmpResult.isMac() && obj4.supports(tmp9.SCREEN_CAPTURE_KIT);
      if (satisfiesResult) {
        const tmp18 = importDefault(3895);
        let release1;
        if (tmp18 != null) {
          release1 = tmp18.os.release;
        }
        satisfiesResult = importDefault(13028).satisfies(release1, closure_24);
        const obj7 = importDefault(13028);
      }
...
prototype["getVideoStreamParameters"] = function getVideoStreamParameters(context) {
  ...
  if (this.supports(Features.VIDEO)) {
    ...
    if (enableSimulcast) {
      const DisableCameraSimulcastExperiment = require(13093) /* apexExperiment */.DisableCameraSimulcastExperiment;
...
const mediaEngineStore = new MediaEngineStore(require("dispatcher"), {
  ...
  CONNECTION_OPEN: function handleConnectionOpen(sessionId) {
    ...
    if (tmp17) {
      const obj7 = importDefault(5331);
      const setMklZenOverrideEnabled = importDefault(5331).requireModule("discord_krisp").setMklZenOverrideEnabled;
      if (setMklZenOverrideEnabled != null) {
      ...
      const requireModuleResult = importDefault(5331).requireModule("discord_krisp");
    }
  ...
  AUDIO_SET_AUDIO_MIXER_SETTINGS: function handleSetAudioMixerSettings(arg0) {
    ...
    const Storage = require(595) /* Storage */.Storage;
    const result = Storage.set(MediaEngineStore, dependencyMap);
    let supportsResult = true === settings.enabled && importDefault(13009).getConfig({ location: "MediaEngineStore" }).enabled;
    if (supportsResult) {
      supportsResult = store3.supports(Features.SPATIAL_AUDIO);
    }
    let c120 = supportsResult;
  ...
  AUDIO_SET_KRISP_MODEL_OVERRIDE: function handleSetKrispModelOverride(model) {
    const result = importDefault(12993).setKrispModelOverride(model.model);
    model = model.model;
    setLoopback();
  },
  ...
  AUDIO_SET_KRISP_SUPPRESSION_LEVEL: function handleSetKrispSuppressionLevel(level) {
    level = level.level;
    const result = importDefault(12993).setKrispSuppressionLevel(level);
  },
  ...
  AUDIO_INPUT_DETECTED: function handleInputDetected(inputDetected) {
    ...
    if (null == inputDetected) {
      ...
      if (inputDetected) {
        ...
        if (c102) {
          const config = importDefault(13089).getConfig({ location: "MediaEngineStore.handleInputDetected" });
  ...
  AUDIO_SET_BYPASS_SYSTEM_INPUT_PROCESSING: function handleBypassSystemInputProcessing(bypassEnabled) {
    ...
    const tmp19 = importDefault(10783);
    const tmp19Result = importDefault(10783)(tmp16.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
  ...
  MEDIA_ENGINE_SET_VIDEO_ENABLED: function handleSetVideoEnabled(enabled) {
    const permission = importDefault(4842).requestPermission(NativePermissionTypes.CAMERA);
  ...
  MEDIA_ENGINE_SET_OPENH264_ENABLED: function handleSetOpenH264Enabled(enabled) {
    ...
    const Storage = require(595) /* Storage */.Storage;
    const result = Storage.set(MediaEngineStore, dependencyMap);
    const tmp13 = importDefault(5331);
    if (tmp13 != null) {
  ...
  MEDIA_ENGINE_SET_DEVICE_AUDIO_EFFECTS: function handleSetDeviceAudioEffects(active) {
    ...
    const tmp4 = importDefault(10783);
    const tmp4Result = importDefault(10783)(tmp.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
    const voiceFidelityCaps = require(13025) /* getAudioFidelityExperimentConfig */.getVoiceFidelityCaps({ location: "updateVoiceFidelityCaps" }, { krispEnabled: tmp4Result, noiseSuppressionEnabled: tmp3, echoCancellationEnabled: tmp2 });
