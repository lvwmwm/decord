...
// Dependencies: [32, 5, 4352, 4357, 4217, 4323, 4372, 1340, 1218, 4373, 4374, 1391, 4375, 1922, 13106, 676, 4368, 4516, 13107, 685, 13108, 4364, 3, 687, 500, 4400, 4188, 4861, 13109, 13110, 13111, 10320, 13112, 12, 13093, 13113, 13114, 13115, 13116, 13117, 13118, 13119, 9175, 698, 709, 10867, 13120, 13121, 13122, 5138, 13123, 13168, 13169, 13170, 13171, 13172, 13173, 4363, 4479, 4297, 3931, 595, 13174, 10854, 5351, 13178, 13179, 1236, 4622, 13180, 13181, 13182, 13183, 38, 13105, 13104, 13184, 13088, 1208, 4361, 9771, 589, 13185, 13186, 13187, 2]

// Module 4351 (_detectH265HardwareDecode)
import apexExperiment from "apexExperiment";
import initialize from "initialize";
import module_4352 from "module_4352";
...
function getSettings() {
  ...
  if (null == obj1.vadDuringPreProcess) {
    obj1.vadDuringPreProcess = require(13110) /* getVADBeforeProcessingExperimentConfig */.getVADBeforeProcessingExperimentConfig({ location: "getSettings" }).enabled;
    const obj7 = require(13110) /* getVADBeforeProcessingExperimentConfig */;
  }
  ...
  const aGC2ExperimentConfig = require(13111) /* getAGC2ExperimentConfig */.getAGC2ExperimentConfig({ location: "getSettings" });
...
function setInputMode(context) {
  ...
  if (context.context === MediaEngineContextTypes.DEFAULT) {
    let obj = require(10320) /* setPushToTalkState */;
    obj.setPushToTalkState(false, false);
  }
  let showPTTSpeakingIndicator = importDefault(13112).getConfig({ location: "setInputMode" }).showPTTSpeakingIndicator;
...
function updateConnectionMuteDeaf(context) {
  ...
  if (MediaEngineContextTypes.DEFAULT === context) {
    ...
    if (!tmp3) {
      tmp3 = !importDefault(4861).didHavePermission(NativePermissionTypes.AUDIO);
      const obj = importDefault(4861);
    }
    let flag = tmp3;
  } else {
    flag = true;
  ...
  if (context.context === MediaEngineContextTypes.DEFAULT) {
    importDefault(13093).updateNativeMute();
    const obj2 = importDefault(13093);
  }
}
      ...
      if (!tmp) {
        ...
        if (null != tmp2) {
          ...
          if (null != tmp2.desktopSource) {
            ...
            const obj20 = require(13109) /* HdrCaptureMode */;
            let isWindowsResult = require(500) /* set */.isWindows();
            if (isWindowsResult) {
              const tmp41 = importDefault(3931);
              let release;
              if (tmp41 != null) {
                release = tmp41.os.release;
              }
              isWindowsResult = importDefault(13123).satisfies(release, closure_29);
              const obj6 = importDefault(13123);
            }
            if (!isWindowsResult) {
              ...
              if (!isWindowsResult1) {
                ...
                if (isMacResult) {
                  const tmp61 = importDefault(3931);
                  let release1;
                  if (tmp61 != null) {
                    release1 = tmp61.os.release;
                  }
                  isMacResult = importDefault(13123).satisfies(release1, closure_24);
                  const obj14 = importDefault(13123);
                }
              ...
              if (isWindowsResult2) {
                const tmp47 = importDefault(3931);
                let release2;
                if (tmp47 != null) {
                  release2 = tmp47.os.release;
                }
                isWindowsResult2 = importDefault(13123).satisfies(release2, closure_27);
                const obj8 = importDefault(13123);
              }
...
function noiseCancellerErrorToAVUnderlyingError(noise_canceller_error) {
  if (NoiseCancellerError.KRISP_CPU_OVERUSE === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispCpuOveruse;
  } else if (tmp.KRISP_FAILED === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispFailed;
  } else if (tmp.KRISP_VAD_CPU_OVERUSE === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispVadCpuOveruse;
  } else if (tmp.KRISP_INIT_ERROR === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispInitError;
  } else if (tmp.KRISP_INIT_ERROR_NATIVE === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispInitErrorNative;
  } else if (tmp.KRISP_INIT_ERROR_SSE4_NOT_SUPPORTED === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispInitErrorSse4NotSupported;
  } else if (tmp.KRISP_INIT_ERROR_AVX2_NOT_SUPPORTED === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispInitErrorAvx2NotSupported;
  } else if (tmp.KRISP_INIT_ERROR_UNSIGNED === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispInitErrorUnsigned;
  } else if (tmp.KRISP_INIT_ERROR_GLOBAL_INIT === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispInitErrorGlobalInit;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_8K === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispInitErrorWeight8k;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_16K === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispInitErrorWeight16k;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_32K === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispInitErrorWeight32k;
  } else if (tmp.KRISP_INIT_ERROR_WEIGHT_VAD === noise_canceller_error) {
    return require(9175) /* mapped */.AVUnderlyingError.KrispInitErrorWeightVad;
  }
}
function handleVoiceProcessingError(noise_canceller_error) {
  obj.warn("Voice processing error: " + noise_canceller_error);
  obj = require(9175) /* mapped */;
  obj = { type: require(9175) /* mapped */.AVError.NOISE_CANCELLER_ERROR, underlyingError: noiseCancellerErrorToAVUnderlyingError(noise_canceller_error) };
...
function handleVideoFilterError(arg0, arg1) {
  ...
  if (arg1 !== constants10.LIVE) {
    ...
    obj[0] = require(9175) /* mapped */.AVError.VIDEO_BACKGROUND_UNAVAILABLE;
...
function updateConnectionVoiceProcessing(setEchoCancellation) {
  ...
  const merged = Object.assign(require(13111) /* getAGC2ExperimentConfig */.getAGC2ExperimentConfig(obj).noiseCancellationConfig);
  const result = setEchoCancellation.setAutomaticGainControl(obj);
  const noiseCancellation = tmp2.noiseCancellation;
  const obj3 = require(13111) /* getAGC2ExperimentConfig */;
  const tmp13Result = importDefault(10867)(noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
  ...
  const tmp13 = importDefault(10867);
  ...
  if (sound) {
    ...
    if (null != mediaEngineStore) {
      ...
      if (!obj.getExperimentalSoundshare()) {
        let obj2 = importDefault(4297);
        audioPid = obj2.getAudioPid(pidFromDesktopSource);
      }
      let str2 = "";
      if (null != audioPid) {
        str2 = importDefault(4297).generateSessionFromPid(audioPid);
        const obj4 = importDefault(4297);
      }
...
function trackVoiceProcessing(location) {
  ...
  const tmp11 = importDefault(10867);
  const tmp11Result = importDefault(10867)(tmp.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
...
function setLoopback() {
  ...
  const tmp15 = importDefault(10867);
  const tmp15Result = importDefault(10867)(tmpResult.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
...
prototype["isSelfMute"] = function isSelfMute(DEFAULT) {
  ...
  if (!mute) {
    mute = !importDefault(4861).didHavePermission(NativePermissionTypes.AUDIO);
    const obj = importDefault(4861);
  }
  if (!mute) {
...
prototype["supportsExperimentalSoundshare"] = function supportsExperimentalSoundshare() {
  ...
  if (supportsResult) {
    ...
    supportsResult = importDefault(13123).satisfies(release, closure_31);
    const obj = importDefault(13123);
  }
  return supportsResult;
};
prototype["supportsHookSoundshare"] = function supportsHookSoundshare() {
  ...
  if (isWindowsResult) {
    const tmp6 = importDefault(3931);
    let release;
    if (tmp6 != null) {
      release = tmp6.os.release;
    }
    isWindowsResult = importDefault(13123).satisfies(release, closure_30);
    const obj2 = importDefault(13123);
  }
...
prototype["getVideoEncoderExperiments"] = function getVideoEncoderExperiments(STREAM, streamer) {
  ...
  if (isWindowsResult) {
    ...
    const VideoCaptureDeviceNoReuseExperiment = require(13186) /* apexExperiment */.VideoCaptureDeviceNoReuseExperiment;
...
prototype["supportsScreenSoundshare"] = function supportsScreenSoundshare() {
  if (obj.isMac()) {
    ...
    if (supportsResult) {
      const tmp12 = importDefault(3931);
      let release;
      if (tmp12 != null) {
        release = tmp12.os.release;
      }
      supportsResult = importDefault(13123).satisfies(release, closure_25);
      const obj5 = importDefault(13123);
    }
    if (supportsResult) {
      let tmpResult = tmp(500);
      let satisfiesResult = tmpResult.isMac() && obj4.supports(tmp9.SCREEN_CAPTURE_KIT);
      if (satisfiesResult) {
        const tmp18 = importDefault(3931);
        let release1;
        if (tmp18 != null) {
          release1 = tmp18.os.release;
        }
        satisfiesResult = importDefault(13123).satisfies(release1, closure_24);
        const obj7 = importDefault(13123);
      }
...
prototype["getVideoStreamParameters"] = function getVideoStreamParameters(context) {
  ...
  if (this.supports(Features.VIDEO)) {
    ...
    if (enableSimulcast) {
      const DisableCameraSimulcastExperiment = require(13187) /* apexExperiment */.DisableCameraSimulcastExperiment;
...
const mediaEngineStore = new MediaEngineStore(require("dispatcher"), {
  ...
  CONNECTION_OPEN: function handleConnectionOpen(sessionId) {
    ...
    if (tmp17) {
      const obj7 = importDefault(5351);
      const setMklZenOverrideEnabled = importDefault(5351).requireModule("discord_krisp").setMklZenOverrideEnabled;
      if (setMklZenOverrideEnabled != null) {
      ...
      const requireModuleResult = importDefault(5351).requireModule("discord_krisp");
    }
  ...
  AUDIO_SET_AUDIO_MIXER_SETTINGS: function handleSetAudioMixerSettings(arg0) {
    ...
    const Storage = require(595) /* Storage */.Storage;
    const result = Storage.set(MediaEngineStore, dependencyMap);
    let supportsResult = true === settings.enabled && importDefault(13104).getConfig({ location: "MediaEngineStore" }).enabled;
    if (supportsResult) {
      supportsResult = store3.supports(Features.SPATIAL_AUDIO);
    }
    let c120 = supportsResult;
  ...
  AUDIO_SET_KRISP_MODEL_OVERRIDE: function handleSetKrispModelOverride(model) {
    const result = importDefault(13088).setKrispModelOverride(model.model);
    model = model.model;
    setLoopback();
  },
  ...
  AUDIO_SET_KRISP_SUPPRESSION_LEVEL: function handleSetKrispSuppressionLevel(level) {
    level = level.level;
    const result = importDefault(13088).setKrispSuppressionLevel(level);
  },
  ...
  AUDIO_INPUT_DETECTED: function handleInputDetected(inputDetected) {
    ...
    if (null == inputDetected) {
      ...
      if (inputDetected) {
        ...
        if (c102) {
          const config = importDefault(13184).getConfig({ location: "MediaEngineStore.handleInputDetected" });
  ...
  AUDIO_SET_BYPASS_SYSTEM_INPUT_PROCESSING: function handleBypassSystemInputProcessing(bypassEnabled) {
    ...
    const tmp19 = importDefault(10867);
    const tmp19Result = importDefault(10867)(tmp16.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
  ...
  MEDIA_ENGINE_SET_GO_LIVE_SOURCE: function handleSetGoLiveSource(settings) {
    ...
    if (null != desktopSettings) {
      ...
      const pidFromDesktopSource = importDefault(4297).getPidFromDesktopSource(sourceId);
  ...
  MEDIA_ENGINE_SET_OPENH264_ENABLED: function handleSetOpenH264Enabled(enabled) {
    ...
    const Storage = require(595) /* Storage */.Storage;
    const result = Storage.set(MediaEngineStore, dependencyMap);
    const tmp13 = importDefault(5351);
    if (tmp13 != null) {
  ...
  CLIPS_INIT: function handleClipsInit(applicationName) {
    ...
    let obj = require(4361) /* isClipsEnabled */;
    if (obj.isDecoupledClipsEnabled()) {
      if (null != importDefault(3931)) {
        let id;
        if (obj != null) {
  ...
  MEDIA_ENGINE_SET_DEVICE_AUDIO_EFFECTS: function handleSetDeviceAudioEffects(active) {
    ...
    const tmp4 = importDefault(10867);
    const tmp4Result = importDefault(10867)(tmp.noiseCancellation, mediaEngineStore.getSystemMicrophoneMode());
    const voiceFidelityCaps = require(13120) /* getAudioFidelityExperimentConfig */.getVoiceFidelityCaps({ location: "updateVoiceFidelityCaps" }, { krispEnabled: tmp4Result, noiseSuppressionEnabled: tmp3, echoCancellationEnabled: tmp2 });
