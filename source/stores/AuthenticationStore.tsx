...
// Dependencies: [1219, 1220, 676, 1221, 3, 707, 595, 1222, 698, 13367, 709, 13368, 8398, 4185, 513, 13369, 1208, 11702, 589, 10376, 13370, 1352, 1935, 2]
      ...
      if (flag) {
        if (!handoffAvailable.isHandoffAvailable()) {
          ...
          nextPromise = experiments.then((body) => {
            ...
            callback(13368).onExperimentsLoaded();
          }, () => {
            let c33 = null;
...
prototype["initialize"] = function initialize() {
  ...
  if (null == obj.getToken()) {
    ...
    if (tmp7) {
      function fireApex() {
        const installationExperiments = callback(10376).fetchInstallationExperiments(null);
      }
      promise.then(fireApex, fireApex);
    }
    promise = fetchFingerprint();
  }
  this.addChangeListener(() => callback(13370).setClientState(closure_17));
};
...
const authenticationStore = new AuthenticationStore(require("dispatcher"), {
  CONNECTION_OPEN: function handleConnectionOpen(arg0) {
    ...
    importDefault(1208).setUser(id, username, email, importDefault(11702)(user));
  ...
  OVERLAY_INITIALIZE: function handleOverlayInitialize(arg0) {
    ...
    ({ id, username, email } = user);
    obj.setUser(id, username, email, importDefault(11702)(user));
    const id2 = user.id;
  ...
  EXPERIMENTS_FETCH(withGuildExperiments) {
    ...
    obj = { location: null };
    let obj3 = require(13367) /* fetchExperiments */;
  ...
  PASSWORDLESS_FAILURE: function handlePasswordlessFailure(error) {
    ...
    if (error instanceof importDefault(4185)) {
      ...
      obj = require(8398) /* getAuthenticationErrorsFromAPIError */;
    } else {
      NONE = LoginStates.NONE;
    }
  },
