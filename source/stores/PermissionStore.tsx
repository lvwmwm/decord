...
// Dependencies: [1984, 4092, 1392, 4093, 1391, 1430, 1930, 1387, 1992, 1909, 1922, 676, 4095, 4096, 12, 4098, 1426, 589, 506, 4099, 709, 2]

// Module 4091 (getUncachedChannelPermissions)
...
import doesThreadMembersActionAffectMe from "doesThreadMembersActionAffectMe" /* 4098 */;
import hasDefault from "has" /* 4099 */;
...
import { MemberSafetyPagePermissions as closure_18 } from "MemberSafetyPagePermissions" /* 4095 */;
...
function computePermissions(context, overwrites, roles, excludeGuildPermissions) {
  ...
  if (context instanceof closure_8) {
    if (set.has(context.type)) {
      ...
      if (null == channel) {
        let NONE4 = tmp(4096).NONE;
      } else {
        let tmpResult = tmp(4096);
...
prototype["canAccessMemberSafetyPage"] = function canAccessMemberSafetyPage(id) {
  ...
  if (null == NONE) {
    ...
    if (null == currentUser) {
      ...
      const guild = store2.getGuild(id);
      if (null == guild) {
        let NONE2 = tmp(4096).NONE;
      } else {
        obj = { user: null, context: null, checkElevated: null };
        obj[0] = currentUser;
        obj[1] = guild;
        obj[2] = true;
        NONE2 = tmp(4096).computePermissions(obj);
        dependencyMap[id] = NONE2;
        const tmpResult = tmp(4096);
      }
      NONE = NONE2;
    }
  }
