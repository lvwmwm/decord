...
// Dependencies: [4303, 4121, 1339, 5399, 4863, 1991, 1908, 1217, 4709, 1921, 5400, 656, 38, 1394, 1990, 2]

// Module 5398 (insertUnsortedGuilds)
...
import closure_7 from "set" /* 5399 */;
...
function convertNodeToGuildFolder(type) {
  type = type.type;
  if (areArraysShallowlyEqual(5400).GuildsNodeType.FOLDER === type) {
    ...
    return obj;
  } else if (tmp(5400).GuildsNodeType.GUILD === type) {
...
function rebuildTree(arg0, arg1) {
  guildsTree = new areArraysShallowlyEqual(5400).GuildsTree();
  if (0 === arg0.length) {
    if (arg1.length > 0) {
      ...
      while (tmp18 !== undefined) {
        ...
        let obj3 = areArraysShallowlyEqual(5400);
    ...
    while (iter2 !== undefined) {
      ...
      let tmp36 = nextResult.type === areArraysShallowlyEqual(5400).GuildsNodeType.GUILD;
      if (tmp36) {
        let tmp37 = closure_5;
    ...
    for (const item10123 of values) {
      ...
      let tmp58 = item10123.type === areArraysShallowlyEqual(5400).GuildsNodeType.FOLDER;
      if (tmp58) {
        let tmp59 = item10123;
  ...
  while (iter !== undefined) {
    ...
    if (0 !== nextResult1.guildIds.length) {
      ...
      if (null == tmp5.folderId) {
        ...
        let obj2 = areArraysShallowlyEqual(5400);
        ...
        let obj4 = areArraysShallowlyEqual(5400);
        ...
        for (const item10030 of guildIds) {
          ...
          let obj = areArraysShallowlyEqual(5400);
...
function handleMoveById(sourceId) {
  ...
  if (null != node) {
    if (null != node1) {
      let tmp4 = combine;
      if (combine) {
        tmp4 = node.type === areArraysShallowlyEqual(5400).GuildsNodeType.FOLDER;
      }
      ...
      tmp22Result(!(node.type === areArraysShallowlyEqual(5400).GuildsNodeType.FOLDER && null != node1.parentId), "[SORTED GUILDS] Can't move a folder " + node.id + " to inside of another folder " + node1.parentId);
      if (combine) {
        let convertToFolderResult = node1;
        if (node1.type !== tmp13(5400).GuildsNodeType.FOLDER) {
...
function handleGuildFolderDeleteLocal(targetId) {
  ...
  if (tmp) {
    const tmp4 = element.type === element(5400).GuildsNodeType.FOLDER;
    if (tmp4) {
      const children = element.children;
      const mapped = children.map((id) => id.id);
      const found = mapped.filter(tmp2(1394).isNotNullish);
...
function handleGuildFolderExpand(folderId) {
  ...
  if (null != node) {
    if (node.type === areArraysShallowlyEqual(5400).GuildsNodeType.FOLDER) {
...
function handleCollapseAll() {
  ...
  while (iter !== undefined) {
    ...
    let expanded = nextResult.type === areArraysShallowlyEqual(5400).GuildsNodeType.FOLDER;
    if (expanded) {
      let tmp6 = nextResult;
