...
// Dependencies: [4007, 3818, 1340, 5028, 4527, 1942, 1862, 1220, 4385, 1874, 5029, 659, 38, 1351, 1941, 2]

// Module 5027 (insertUnsortedGuilds)
...
function convertNodeToGuildFolder(type) {
  let children;
  type = type.type;
  if (areArraysShallowlyEqual(5029).GuildsNodeType.FOLDER === type) {
    ...
    return obj;
  } else if (tmp(5029).GuildsNodeType.GUILD === type) {
...
function rebuildTree(arg0, arg1) {
  let guildsTree = new areArraysShallowlyEqual(5029).GuildsTree();
  if (0 === arg0.length) {
    if (arg1.length > 0) {
      ...
      while (tmp18 !== undefined) {
        ...
        let obj3 = areArraysShallowlyEqual(5029);
    ...
    while (iter2 !== undefined) {
      ...
      let tmp36 = nextResult.type === areArraysShallowlyEqual(5029).GuildsNodeType.GUILD;
      if (tmp36) {
    ...
    for (const item10123 of values) {
      ...
      let tmp58 = item10123.type === areArraysShallowlyEqual(5029).GuildsNodeType.FOLDER;
      if (tmp58) {
        let tmp59 = item10123;
  ...
  while (iter !== undefined) {
    ...
    if (0 !== nextResult1.guildIds.length) {
      ...
      if (null == tmp5.folderId) {
        ...
        let obj2 = areArraysShallowlyEqual(5029);
        ...
        let obj4 = areArraysShallowlyEqual(5029);
        ...
        for (const item10030 of guildIds) {
          ...
          let obj = areArraysShallowlyEqual(5029);
...
function handleMoveById(sourceId) {
  ...
  if (null != node) {
    if (null != node1) {
      let tmp4 = combine;
      if (combine) {
        tmp4 = node.type === areArraysShallowlyEqual(5029).GuildsNodeType.FOLDER;
      }
      ...
      tmp22Result(!(node.type === areArraysShallowlyEqual(5029).GuildsNodeType.FOLDER && null != node1.parentId), "[SORTED GUILDS] Can't move a folder " + node.id + " to inside of another folder " + node1.parentId);
      if (combine) {
        let convertToFolderResult = node1;
        if (node1.type !== tmp13(5029).GuildsNodeType.FOLDER) {
...
function handleGuildFolderDeleteLocal(targetId) {
  ...
  if (tmp) {
    const tmp4 = element.type === element(5029).GuildsNodeType.FOLDER;
    if (tmp4) {
...
function handleGuildFolderExpand(folderId) {
  ...
  if (null != node) {
    if (node.type === areArraysShallowlyEqual(5029).GuildsNodeType.FOLDER) {
...
function handleCollapseAll() {
  ...
  while (iter !== undefined) {
    ...
    let expanded = nextResult.type === areArraysShallowlyEqual(5029).GuildsNodeType.FOLDER;
    if (expanded) {
      let tmp6 = nextResult;
