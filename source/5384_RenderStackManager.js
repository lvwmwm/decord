...
// Module 5384 (RenderStackManager)
...
export const RenderStackManager = () => {
  class RenderStackManager {
    constructor() {
      ...
      if (arguments.length > 0) {
        ...
        tmp3 = closure_1(self, RenderStackManager);
        flag = false;
  ...
  let closure_0 = RenderStackManager;
  let obj = {
    key: "sync",
    value: function sync(arg0, arg1, items) {
      let self = this;
      const RenderStackManager = arg0;
      self = this;
      this.clearRecyclePool();
      const unProcessedIndices = this.unProcessedIndices;
      ...
      const item = keyMap.forEach((index) => {
        ...
        if (index >= arg3) {
          ...
          if (arg2.includes(index)) {
            const tmp10 = index(index);
            if (!tmp12) {
              self.recycleKey(arg1);
            }
            const tmp12 = tmp === index(index) && tmp2 === arg1(index);
          } else {
            self.recycleKey(arg1);
          }
        }
  ...
  items[7] = {
    ...
    value: function generateKey() {
      const str = +this.keyCounter;
      this.keyCounter = str + 1;
      return str.toString();
    }
  };
  items[8] = {
