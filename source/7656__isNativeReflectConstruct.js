...
// Dependencies: [32, 41, 42, 93, 95, 96, 98, 7596, 7611, 7593, 7657, 7621, 7624, 7625, 7597, 7650, 7608, 7658, 7619, 7602, 7633, 7652]

// Module 7656 (_isNativeReflectConstruct)
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7657).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7657);
      return ServerRuntimeClient(7621).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7621);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7657).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7624).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7624);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7650).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7625).DEBUG_BUILD) {
        const logger = tmp2(7597).logger;
    ...
    value: function captureCheckIn(checkInId) {
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7658);
          ...
          if (ServerRuntimeClient(7625).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7597).logger;
          ...
          if (ServerRuntimeClient(7625).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7597).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7608);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7619)._getSpanForScope(arg0);
        ...
        const tmpResult1 = ServerRuntimeClient(7633);
        if (_getSpanForScopeResult) {
