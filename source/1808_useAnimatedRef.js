...
export const useAnimatedRef = isJest ? (function useAnimatedRefWeb() {
  const f71964 = (getScrollableNode) => {
  ...
  if (!tmp2.current) {
    fun = function fun(map) {
      ...
      if (map) {
        ...
        fun.getTag = () => f71967(current[3]).findNodeHandle(closure_0);
        fun.current = map;
        if (tag.size) {
          tag = undefined;
          if (tmp3 != null) {
  ...
  const first = callback(callback2(() => {
    ...
    if (!obj.isFabric()) {
      mutable = null;
      if (closure_5) {
        mutable = f71967(current[4]).makeMutable(null);
        const tmpResult = f71967(current[4]);
      }
    }
    return mutable;
  }), 1)[0];
  let f71967 = first;
  const first1 = callback(callback2(() => f71967(current[4]).makeMutable(null)), 1)[0];
  let current = first1;
  f71967 = (viewConfig) => {
    if (obj.isFabric()) {
      let fn = f71967(current[5]).getShadowNodeWrapperFromRef;
    } else {
  ...
  const shareableMappingCache = f71967(current[6]).shareableMappingCache;
