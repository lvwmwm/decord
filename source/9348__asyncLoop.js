...
// Function ID: 9349
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9349, 9350]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(9349).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9350).crypto) {
    if (typeof tmp(9350).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9350).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9350).crypto) {
    if (typeof tmp(9350).crypto.randomBytes === "function") {
      const _crypto = tmp(9350).crypto;
