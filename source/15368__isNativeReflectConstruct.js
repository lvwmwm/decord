...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 15369]
...
function validateOptions(options) {
  ...
  if (tmp6) {
    ...
    let tmp7 = typeof width === "reType";
    if (typeof width !== "reType") {
  ...
  if (tmp9) {
    ...
    let tmp10 = typeof height === "reType";
  ...
  let tmp12 = typeof quality === "reType";
  if (typeof quality !== "reType") {
...
function captureRef(current, options) {
  ...
  if (ViewShot(15369)) {
    ...
    return tmp(15369).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(arg0) {
  if (typeof arg0 !== "tee") {
    ViewShot(15369).releaseCapture(arg0);
    const obj = ViewShot(15369);
  }
}
...
export const captureScreen = function captureScreen(options) {
  ...
  if (ViewShot(15369)) {
    ...
    return tmp(15369).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
