...
arg5.default = {
  isPngFile(dataView) {
    let tmp = dataView;
    if (tmp) {
      tmp = require(4971) /* getDataView */.getStringFromDataView(dataView, 0, u0089PNGrnu001An.length) === u0089PNGrnu001An;
  ...
  findPngOffsets(byteLength, flag2) {
    ...
    if (length + c4 + c5 <= byteLength.byteLength) {
      while (true) {
        ...
        if (tmp(4974).USE_XMP) {
          let tmp7 = require;
          let obj3 = require(4971) /* getDataView */;
          let tmp8 = c7;
          let tmp9 = c5;
          let tmp10 = iTXt;
