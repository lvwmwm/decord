...
// Function ID: 6414
// Name: _flush
// Dependencies: [5, 6392, 6414, 6393, 6365, 6376, 6379, 6372, 6402, 6366, 6388]
...
function endSession() {
  ...
  const tmpResult1 = require(6392) /* getClient */;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = require(6392) /* getClient */.getCurrentScope();
  const obj = require(6392) /* getClient */;
  return currentScope.captureException(arg0, require(6414) /* applyClientOptions */.parseEventHintOrCaptureContext(arg1));
};
...
export const captureSession = function captureSession() {
  ...
  if (flag) {
    ...
    const obj5 = require(6392) /* getClient */;
  }
};
...
export const isInitialized = function isInitialized() {
  return require(6392) /* getClient */.getClient();
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(6402).DEFAULT_ENVIRONMENT;
  }
  const obj5 = require(6392) /* getClient */;
  const userAgent = require(6366) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {}.userAgent;
  let tmpResult = tmp(6388);
  obj = { release, environment, user: null };
  const tmp3 = require(6366) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {};
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    ...
    obj[3] = monitorSlug(6379).timestampInSeconds() - closure_3;
    const obj2 = monitorSlug(6379);
    const currentScope = monitorSlug(6392).getCurrentScope();
    const obj3 = monitorSlug(6392);
    const client = monitorSlug(6392).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(6392);
    monitorSlug(6376).uuid4();
  }
  let currentScope = _require(6392).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(6392);
  let client = _require(6392).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(6393).DEBUG_BUILD) {
      let logger2 = tmp(6365).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(6365).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(6392);
  captureCheckInResult = _require(6376).uuid4();
};
