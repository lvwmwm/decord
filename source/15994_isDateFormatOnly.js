...
function createDateTimeFormat(expandFormatResult) {
  ...
  if (!regex.test(expandFormatResult)) {
    const obj = {};
    obj.pattern = expandFormatResult.replace(obj, (str) => {
      const charAtResult = str.charAt(0);
      if ("E" !== charAtResult) {
        if ("e" !== charAtResult) {
          if ("c" !== charAtResult) {
            if ("G" === charAtResult) {
              obj.era = outer1_2.era[str.length - 1];
              return "{era}";
            } else {
              if ("y" !== charAtResult) {
                if ("Y" !== charAtResult) {
                  if ("u" !== charAtResult) {
                    if ("U" !== charAtResult) {
                      ...
                      obj.month = outer1_2.month[str.length - 1];
                      return "{month}";
                    }
                  }
                }
      ...
      obj.weekday = outer1_2.weekday[str.length - 1];
      return "{weekday}";
    });
...
arg5.createDateTimeFormats = function createDateTimeFormats(formats) {
  ...
  for (const key10079 in dateFormats) {
    ...
    if (hasOwnPropertyResult2) {
      let tmp37 = createDateTimeFormat;
      let expandFormatResult2 = expandFormat(key10079, dateFormats[key10079]);
      hasOwnPropertyResult2 = createDateTimeFormat(expandFormatResult2);
