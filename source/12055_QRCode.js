    ...
    while (true) {
      ...
      if (0 < self.dataList.length) {
        do {
          ...
          num5 = num5 + 1;
          let length2 = self.dataList.length;
        } while (num5 < length2);
      }
      tmp14 = num;
...
prototype.setupPositionAdjustPattern = function() {
  const self = this;
  const patternPosition = require(dependencyMap[3]).getPatternPosition(this.typeNumber);
...
prototype.setupTypeNumber = function(arg0) {
  let num2;
  const self = this;
  const bCHTypeNumber = require(dependencyMap[3]).getBCHTypeNumber(this.typeNumber);
  let num = 0;
  do {
...
prototype.setupTypeInfo = function(arg0, arg1) {
  let num2;
  const self = this;
  const bCHTypeInfo = require(dependencyMap[3]).getBCHTypeInfo(this.errorCorrectLevel << 3 | arg1);
  let num = 0;
  do {
...
QRCode.createData = (arg0, arg1, arg2) => {
  ...
  let length2;
  let result;
  const rSBlocks = require(dependencyMap[1]).getRSBlocks(arg0, arg1);
  let tmp = require(dependencyMap[2]);
  tmp = new tmp();
  let num = 0;
  if (0 < arg2.length) {
    do {
...
QRCode.createBytes = (arg0, arg1) => {
  ...
  if (0 < arg1.length) {
    do {
      ...
      let tmp13 = closure_0(closure_1[4]);
