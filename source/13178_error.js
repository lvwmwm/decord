...
// Module 13178 (error)
function error(arg0) {
  const rangeError = new RangeError(closure_4[arg0]);
  throw rangeError;
}
...
let closure_2 = /[^\0-\x7F]/;
let closure_3 = /[\x2E\u3002\uFF0E\uFF61]/g;
let closure_4 = { "Bool(true)": "TypeMismatchError", "Bool(true)": "DEFAULT_NAVIGATION_SPAN_NAME", "Bool(true)": "nameToEmoji" };
  ...
  if (num4 < arr.length) {
    ...
    do {
      while (true) {
        ...
        if (num4 >= length) {
          let tmp8 = closure_12;
          let tmp9 = closure_12("invalid-input");
        }
        let tmp10 = closure_7;
        let tmp11 = +num4;
        sum = tmp11 + 1;
        let tmp13 = closure_7(arr.charCodeAt(tmp11));
        if (tmp13 >= 36) {
          let tmp14 = closure_12;
          let tmp15 = closure_12("invalid-input");
        }
        let tmp16 = closure_5;
        if (tmp13 > closure_5((2147483647 - tmp6) / num8)) {
          let tmp17 = closure_12;
          let tmp18 = closure_12("overflow");
        }
        ...
        if (tmp13 < num10) {
          ...
          let diff = 36 - num10;
          let tmp20 = closure_5;
          if (num8 > closure_5(2147483647 / diff)) {
            let tmp21 = closure_12;
            let tmp22 = closure_12("overflow");
          }
          num8 = num8 * diff;
          num9 = num9 + 36;
          tmp6 = sum1;
      ...
      let tmp25 = closure_9(sum1 - num7, sum2, 0 === num7);
      if (closure_5(result) > 2147483647 - num2) {
        let tmp28 = closure_12;
        let tmp29 = closure_12("overflow");
      }
      let tmp30 = closure_5;
      let sum3 = num2 + closure_5(result);
...
function encode(arg0) {
  ...
  for (const item10026 of arr1) {
    if (item10026 < 128) {
      ...
      let tmp3 = closure_6;
      let tmp4 = item10026;
      let arr = items.push(closure_6(tmp));
    }
  }
  let sum2 = length2;
  if (items.length) {
  ...
  if (sum2 < arr1.length) {
    ...
    do {
      ...
      let tmp25 = num2;
      let diff = num4 - num;
      if (diff > closure_5((2147483647 - num2) / sum)) {
        let tmp26 = closure_12;
        let tmp27 = closure_12("overflow");
      }
      let tmp28 = num2;
      let tmp29 = num4;
      let tmp30 = num;
      ...
      for (const item10085 of arr1) {
        ...
        if (tmp36) {
          let tmp37 = closure_12;
          let tmp38 = closure_12("overflow");
        }
        let tmp39 = item10085;
        let tmp40 = tmp29;
        if (tmp73 === tmp32) {
          ...
          while (true) {
            ...
            if (tmp58 < num6) {
              ...
              let tmp54 = closure_6;
              let tmp55 = closure_8;
              arr1 = items.push(closure_6(closure_8(tmp47 + diff1 % diff2, 0)));
              let tmp57 = closure_5;
              tmp58 = closure_5(diff1 / diff2);
              let tmp59 = num8;
              num8 = num8 + 36;
              // continue
            }
          ...
          let tmp62 = closure_8;
          let tmp63 = tmp58;
          let arr2 = items.push(closure_6(closure_8(tmp58, 0)));
          let tmp65 = closure_9;
          let tmp66 = num5;
          let tmp67 = sum;
          let tmp68 = sum2;
          let tmp69 = length2;
          num3 = closure_9(num5, tmp20, sum2 === length2);
          num5 = 0;
          sum2 = sum2 + 1;
        }
      }
...
obj = {
  ...
  encode: function ucs2encode(arg0) {
    return fromCodePoint.apply(String, callback(arg0));
  }
};
