...
// Function ID: 5978
// Name: CardContainer
// Dependencies: [19, 17, 21, 1501, 5449, 5978, 5979, 5980, 5967]

// Module 5977 (CardContainer)
...
export const CardContainer = importAllResult.memo(function CardContainerInner(active) {
  ...
  obj1 = active(1501);
  let num = importAllResult.useContext(active(5449).HeaderHeightContext);
  let tmp4 = focused;
  if (focused) {
    tmp4 = false !== scene.descriptor.options.keyboardHandlingEnabled;
  }
  let tmp2Result = tmp2(5978);
  ...
  if (previousScene) {
    ...
    headerTitle = tmp2(5449).getHeaderTitle(options, route.name);
    href = tmp2Result1.useLinkBuilder().buildHref(route.name, route.params);
    tmp8 = href;
    tmp9 = headerTitle;
    const tmp2Result2 = tmp2(5449);
  }
  closure_15 = tmp12;
  ...
  obj8[1] = closure_4(active(5449).HeaderHeightContext.Provider, { value: headerHeight, children: descriptor.render() });
  obj7[1] = closure_4(active(5449).HeaderShownContext.Provider, obj8);
  obj6[1] = closure_4(active(5449).HeaderBackContext.Provider, obj7);
  items4[1] = closure_4(closure_3, obj6);
  obj4[1] = items4;
  obj3[1] = tmp22(active(5967).ModalPresentationContext.Provider, obj4);
  obj1[29] = closure_4(closure_3, obj3);
  obj[6] = closure_4(active(5980).Card, obj1);
  return closure_4(active(5979).CardA11yWrapper, obj);
});
