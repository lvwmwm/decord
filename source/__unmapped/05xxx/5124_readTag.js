...
// Function ID: 5125
// Name: readTag
// Dependencies: [5121, 5125, 5107]
// Exports: get0thIfdOffset

// Module 5124 (readTag)
import _modDef5107 from "module_5107" /* 5107 */;
import _modDef5121 from "module_5121" /* 5121 */;
import importDefaultResult1 from "importDefaultResult1" /* 5125 */;

require = arg1;
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  obj = _modDef5121;
  const typeSize = obj.getTypeSize("SHORT");
  sum = typeSize + _modDef5121.getTypeSize("SHORT");
  const obj2 = _modDef5121;
  const sum1 = sum + _modDef5121.getTypeSize("LONG");
  const obj3 = _modDef5121;
  const shortAt = _modDef5121.getShortAt(byteLength, sum, byteOrder);
  const obj4 = _modDef5121;
  const shortAt1 = _modDef5121.getShortAt(byteLength, sum + typeSize, byteOrder);
  const obj5 = _modDef5121;
  const longAt = _modDef5121.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== _modDef5121.typeSizes[shortAt1]) {
    if ((function tagValueFitsInOffsetSlot(shortAt1, longAt) {
      const result = callback(5121).typeSizes[shortAt1] * longAt;
      return result <= callback(5121).getTypeSize("LONG");
    })(shortAt1, longAt)) {
      const sum2 = sum + sum1;
      let str = getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder);
      let tmp21 = sum2;
    } else {
      const longAt1 = tmp4(5121).getLongAt(byteLength, sum + sum1, byteOrder);
      str = "<faulty value>";
      tmp21 = longAt1;
      if ((function tagValueFitsInDataView(byteLength, sum, longAt1, shortAt1, longAt) {
        sum = sum + longAt1;
        return sum + callback(5121).typeSizes[shortAt1] * longAt <= byteLength.byteLength;
    ...
    let tmp34 = str;
    if (shortAt1 === tmp4(5121).tagTypes.ASCII) {
    ...
    if (undefined !== tmp4(5125)[IFD_TYPE_0TH][shortAt]) {
      if (undefined !== tmp4(5125)[IFD_TYPE_0TH][shortAt].name) {
        if (undefined !== tmp4(5125)[IFD_TYPE_0TH][shortAt].description) {
          try {
            descriptionResult = tmp4(5125)[IFD_TYPE_0TH][shortAt].description(tmp34);
            combined = tmp39;
            const obj8 = tmp4(5125)[IFD_TYPE_0TH][shortAt];
          } catch (err) {
      ...
      if (shortAt1 !== tmp4(5121).tagTypes.RATIONAL) {
        if (shortAt1 !== tmp4(5121).tagTypes.SRATIONAL) {
          combined = tmp4(5125)[IFD_TYPE_0TH][shortAt];
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * _modDef5121.typeSizes[shortAt1];
    BYTE = _modDef5121.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    ...
    sum = sum + _modDef5121.typeSizes[BYTE];
  }
  if (BYTE === _modDef5121.tagTypes.ASCII) {
    let asciiValue = _modDef5121.getAsciiValue(items);
    const tmp9Result = _modDef5121;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
let obj = { 1: null, 2: _modDef5121.getByteAt, 3: _modDef5121.getAsciiAt, 4: _modDef5121.getShortAt, 5: _modDef5121.getLongAt, 7: _modDef5121.getRationalAt, 9: null, 10: _modDef5121.getUndefinedAt, 13: null };
obj[9] = _modDef5121.getSlongAt;
obj[10] = _modDef5121.getSrationalAt;
obj[13] = _modDef5121.getIfdPointerAt;
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5, arg6) {
  obj = _modDef5121;
  const typeSize = obj.getTypeSize("SHORT");
  let num = 0;
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = _modDef5121.getShortAt(byteLength, sum, byteOrder);
    const tmpResult = _modDef5121;
  }
  obj = {};
  sum = sum + typeSize;
  let tmp5 = sum;
  ...
  if (_modDef5107.USE_THUMBNAIL) {
    let tmp23Result = tmp23(5121);
    if (tmp5 < byteLength.byteLength - tmp23Result.getTypeSize("LONG")) {
      tmp23Result = tmp23(5121);
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, sum, byteOrder) {
  return sum + _modDef5121.getLongAt(buffer, sum + 4, byteOrder);
};
export { readIfd };
