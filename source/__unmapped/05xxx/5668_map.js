...
// Function ID: 5669
// Name: map
// Dependencies: [32, 19, 5666, 5656, 5669, 5642, 5657, 5604]
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!disableReanimated) {
    result = undefined !== _mod5642.Reanimated;
  }
  if (result) {
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      ...
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let tmp23 = tmp18;
        let tmp24 = tmp20;
        let PropsToFilter = tmp19(5657).PropsToFilter;
        let tmp25 = first;
    ...
    let tmp32 = tmp18;
    let tmp33 = tmp20;
    let Reanimated = tmp19(5642).Reanimated;
