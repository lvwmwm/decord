...
// Function ID: 9417
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9417, 9418]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(9417).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9418).crypto) {
    if (typeof tmp(9418).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9418).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9418).crypto) {
    if (typeof tmp(9418).crypto.randomBytes === "function") {
      const _crypto = tmp(9418).crypto;
