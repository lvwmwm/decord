...
// Dependencies: [41, 42, 7673, 7677, 7678, 7668, 7701, 7669, 7679, 7705, 7691, 7663, 7690, 7706, 7692, 7699]

// Module 7704 (isFullFinishedSpan)
...
class SentrySpan {
  constructor() {
    ...
    obj[require("module_7678").SEMANTIC_ATTRIBUTE_SENTRY_OP] = obj.op;
...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(7668).spanTimeInputToSeconds(arg0);
    }
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(7678).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    }
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(7668).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(7668);
        SentrySpan(7701).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(7701);
      }
    }
  },
  {
    ...
    value: function getSpanJSON() {
      const self = this;
      let obj = SentrySpan(7669);
      obj = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 7678).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: SentrySpan(7668).getStatusMessage(this._status), timestamp: null, trace_id: null, origin: _attributes[SentrySpan(undefined, 7678).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN], _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      ({ _endTime: obj2[7], _traceId: obj2[8], _attributes } = this);
      const obj3 = SentrySpan(7668);
      obj[10] = SentrySpan(7679).getMetricSummaryJsonForSpan(this);
      obj[11] = this._attributes[SentrySpan(undefined, 7678).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj[12] = this._attributes[SentrySpan(undefined, 7678).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(7679);
      obj[13] = SentrySpan(7705).timedEventsToMeasurements(this._events);
  ...
  },
  {
    ...
    value: function addEvent(name, num) {
      if (SentrySpan(7691).DEBUG_BUILD) {
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(7690).getClient();
      if (client) {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          ...
          if (result) {
            let scope = tmp(7692).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(7690).getCurrentScope();
              const tmpResult2 = tmp(7690);
            }
            scope.captureEvent(result);
            const tmpResult1 = tmp(7692);
          }
        }
      } else {
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        ...
        if (true !== self._sampled) {
          ...
          const spanDescendants = tmp3(7668).getSpanDescendants(self);
          ...
          const mapped = found.map((arg0) => self(7668).spanToJSON(arg0));
          const found1 = mapped.filter(isFullFinishedSpan);
          const tmp24 = self._attributes[tmp3(undefined, 7678).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
          const _attributes = self._attributes;
          let SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = tmp3(7678).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
          delete tmp2[tmp];
          const item = found1.forEach((data) => {
            if (data.data) {
              data = data.data;
              const SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = self(7678).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
              delete tmp2[tmp];
            }
          });
          ...
          obj = { trace: null };
          const tmp3Result2 = tmp3(7668);
          obj[0] = tmp3(7668).spanToTransactionTraceContext(self);
          obj[0] = obj;
          let substr = found1;
          ...
          const tmp3Result3 = tmp3(7668);
          const obj2 = { dynamicSamplingContext: null };
          const tmp3Result4 = tmp3(7669);
          obj2[0] = tmp3(7699).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result4.dropUndefinedKeys(obj2));
          obj[6] = obj1;
          const tmp3Result5 = tmp3(7699);
          obj[7] = tmp3(7679).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
          ...
          const tmp3Result6 = tmp3(7679);
          const result = tmp3(7705).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
          ...
          if (length) {
            if (tmp3(7691).DEBUG_BUILD) {
              const logger2 = tmp3(7663).logger;
              const _JSON = JSON;
