...
// Function ID: 7702
// Name: logSpanEnd
// Dependencies: [7691, 7668, 7663]

// Module 7701 (logSpanEnd)
...
arg5.logSpanStart = function logSpanStart(spanContext) {
  if (__SENTRY_DEBUG__.DEBUG_BUILD) {
    ...
    const rootSpan = tmp(7668).getRootSpan(spanContext);
    let str3 = "unsampled";
    ...
    if (rootSpan !== spanContext) {
      ...
      ({ op: op2, description: description2 } = tmp(7668).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(7668).spanToJSON(rootSpan);
    }
    const logger = tmp(7663).logger;
