...
// Dependencies: [32, 7690, 7696, 7685, 7698, 7668, 7680, 7681, 7682, 7675, 7673, 7691, 7663, 7695, 7699, 7701, 7692, 7702, 7704, 7678]
// Exports: continueTrace, startInactiveSpan, startNewTrace, startSpan, startSpanManual, suppressTracing, withActiveSpan

// Module 7697 (createChildOrRootSpan)
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(7704).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(7696).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(7668).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(7668);
        const client = tmp(7690).getClient();
        if (client) {
      ...
      tmp(7701).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(7701);
      const result = tmp(7692).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(7699).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(7699);
      ...
      const tmp42 = _startRootSpan(obj1, scope, tmp(7668).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(7668).spanIsSampled(parentSpan);
      tmp(7699).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = tmp(7699);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(7699).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = tmp(7699);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(7696).SentryNonRecordingSpan();
...
function _startRootSpan(name) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(7702).sampleSpan(options, obj);
  ...
  obj1.attributes = { [_mod7678.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(7704).SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(7678).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  const _require = arg1;
  const mainCarrier = _require(7681).getMainCarrier();
  const obj = _require(7681);
  const tmp = _require;
  const asyncContextStrategy = _require(7682).getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(7690).withScope((setPropagationContext) => {
      ...
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = experimental(obj[1]).withScope((arg0) => {
            _null(7685)._setSpanForScope(arg0, _null);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return _require(7690).withScope((setPropagationContext) => {
...
export const startSpanManual = function startSpanManual(experimental) {
  ...
  let obj = _require(7681);
  let mainCarrier = obj.getMainCarrier();
  let asyncContextStrategy = _require(7682).getAsyncContextStrategy(mainCarrier);
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(7690).withScope((setSDKProcessingMetadata) => {
      const result = setSDKProcessingMetadata.setSDKProcessingMetadata({ [closure_1_3]: true });
      return callback();
    });
    const tmpResult = tmp(7690);
  }
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(7690).withScope((arg0) => {
      _null(7685)._setSpanForScope(arg0, _null);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(7690);
