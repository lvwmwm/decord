...
// Dependencies: [7688, 7669, 7668, 7699]

// Module 7713 (applyScopeDataToEvent)
import addNonEnumerableProperty from "addNonEnumerableProperty" /* 7669 */;
import merge from "merge" /* 7688 */;
...
arg5.applyScopeDataToEvent = function applyScopeDataToEvent(extra) {
  ...
  if (span) {
    ...
    obj3[0] = tmp3(7668).spanToTraceContext(span);
    const merged8 = Object.assign(extra.contexts);
    extra.contexts = obj3;
    const obj4 = { dynamicSamplingContext: null };
    const tmp3Result2 = tmp3(7668);
    obj4[0] = tmp3(7699).getDynamicSamplingContextFromSpan(span);
    const merged9 = Object.assign(extra.sdkProcessingMetadata);
    extra.sdkProcessingMetadata = obj4;
    const tmp3Result3 = tmp3(7699);
    const rootSpan = tmp3(7668).getRootSpan(span);
    const tmp3Result4 = tmp3(7668);
    const description = tmp3(7668).spanToJSON(rootSpan).description;
