...
// Function ID: 70324
// Name: pbkdf2Init
// Dependencies: [5, 8928, 8927, 8931]
// Exports: pbkdf2, pbkdf2Async

// Module 8930 (pbkdf2Init)
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  let c;
  let dkLen;
  require(8928) /* number */.hash(sha256);
  ({ c, dkLen, asyncTick } = require(8927) /* asciiToBase16 */.checkOpts({ dkLen: 32, asyncTick: 10 }, arg3));
  require(8928) /* number */.number(c);
  require(8928) /* number */.number(dkLen);
  require(8928) /* number */.number(asyncTick);
  if (c < 1) {
    const _Error = Error;
    ...
    const toBytesResult = require(8927) /* asciiToBase16 */.toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = require(8931) /* _isNativeReflectConstruct */.hmac;
    let obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = require(8927) /* asciiToBase16 */.toBytes(B);
    obj = { c, dkLen, asyncTick, DK: uint8Array };
    obj.PRF = obj;
    obj.PRFSalt = obj._cloneInto().update(toBytesResult1);
    return obj;
  }
  const checkOptsResult = require(8927) /* asciiToBase16 */.checkOpts({ dkLen: 32, asyncTick: 10 }, arg3);
}
...
export const pbkdf2 = function pbkdf2(sha256, B, B, arg3) {
  ...
  const view = require(8927) /* asciiToBase16 */.createView(uint8Array);
