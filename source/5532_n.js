...
let obj = {
  createGestureHandler(closure_0, handlerTag, config) {
    ...
    const result = _require(5519).scheduleOperationToBeFlushed(() => {
  ...
  setGestureHandlerConfig(handlerTag, closure_1) {
    ...
    let result = _require(5519).scheduleOperationToBeFlushed(() => {
...
const fn = function n(arg0, arg1) {
  const result = module(5517).updateGestureHandlerConfig(arg0, arg1);
  const obj = module(5517);
  module(5517).flushOperations();
};
...
obj[5] = function installUIRuntimeBindings() {
  return module(5517).installUIRuntimeBindings();
};
