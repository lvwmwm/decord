...
// Function ID: 45811
// Name: Pressable
// Dependencies: [57, 29, 31, 27, 33, 5255, 5327, 5328, 5329, 5330, 5302, 5331, 5287, 5314]
// Exports: default

// Module 5326 (Pressable)
...
export default function Pressable(pressRetentionOffset) {
  ...
  const items7 = [tmp14, tmp15, tmp16, isScreenReaderEnabled];
  onHoverOut(() => {
    tmp16.setStates(hitSlop(delayHoverIn[9]).getStatesConfig(closure_28, closure_29, isScreenReaderEnabled));
  }, items7);
  let closure_32 = onPressIn(null);
  let closure_33 = onPressIn(null);
  const items8 = [delayHoverIn, delayHoverOut, onHoverIn, onHoverOut];
  const items9 = [tmp16, tmp13, tmp15, isScreenReaderEnabled];
  const tmp7 = onPress(() => {
    if ("number" === typeof pressRetentionOffset) {
      let numberAsInsetResult = hitSlop(delayHoverIn[6]).numberAsInset(pressRetentionOffset);
      const obj = hitSlop(delayHoverIn[6]);
  ...
  const items11 = [
    onPress(() => {
      ...
      pnpm_PressableTsx8.__closure = { Platform: onLongPress, stateMachine: closure_30, StateMachineEvent: hitSlop(delayHoverIn[9]).StateMachineEvent };
      ...
      pnpm_PressableTsx7.__closure = { Platform: onLongPress, stateMachine: closure_30, StateMachineEvent: hitSlop(delayHoverIn[9]).StateMachineEvent, handleFinalize: closure_27 };
    ...
    onPress(() => {
      ...
      /* worklet (recovered source) */ function pnpm_PressableTsx5(){const{Platform,isScreenReaderEnabled,stateMachine,handleFinalize}=this.__closure;if(Platform.OS==='android'&&!isScreenReaderEnabled){stateMachine.reset();handleFinalize();}}
      ...
      pnpm_PressableTsx5.__workletHash = 8055694403599;
      ...
      pnpm_PressableTsx3.__closure = { Platform: onLongPress, stateMachine: closure_30, StateMachineEvent: hitSlop(delayHoverIn[9]).StateMachineEvent, handleFinalize: closure_27 };
  ...
  function _loop(item10114) {
    let closure_0 = item10114;
    item10114.enabled(closure_34);
    item10114.runOnJS(true);
    item10114.hitSlop(closure_22);
    ...
    const item = entries.forEach((arg0) => {
      ...
      hitSlop(delayHoverIn[11]).applyRelationProp(closure_0, tmp2, tmp3);
    });
  }
  for (const item10114 of items11) {
    let tmp22 = _loop;
    let _loopResult = _loop(item10114);
    continue;
  }
  const Simultaneous = hitSlop(delayHoverIn[10]).GestureObjects.Simultaneous;
  const tmp20 = onPress(() => {
    const GestureObjects = hitSlop(delayHoverIn[10]).GestureObjects;
    const LongPressResult = GestureObjects.LongPress();
    const minDurationResult = GestureObjects.LongPress().minDuration(hitSlop(delayHoverIn[5]).INT32_MAX);
  ...
  const applyResult = Simultaneous.apply(hitSlop(delayHoverIn[10]).GestureObjects, items11);
  const tmp27 = onPress(() => {
    if (null === closure_16) {
  ...
  if (null != android_disableSound) {
    tmp32 = android_disableSound;
  }
  obj2.touchSoundDisabled = tmp32;
  ...
  obj1.children = tmp30(pressRetentionOffset(delayHoverIn[13]), Object.assign({}, tmp, obj2));
  return tmp29(hitSlop(delayHoverIn[12]).GestureDetector, obj1);
};
