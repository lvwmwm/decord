...
// Dependencies: [5555, 5547, 5493]

// Module 5562 (traverseAndConfigureRelations)
const require = arg1;
...
function traverseAndConfigureRelations(gestures, map, set) {
  ...
  if (obj.isComposedGesture(gestures)) {
    ...
    gestures.gestureRelations = tmp(5555).prepareRelations(gestures.config, gestures.handlerTag);
...
arg5.configureRelations = function configureRelations(externalSimultaneousHandlers) {
  ...
  if (obj2.isComposedGesture(externalSimultaneousHandlers)) {
    ...
    if (externalSimultaneousHandlers.type === set(5547).ComposedGestureName.Simultaneous) {
...
arg5.ensureNativeDetectorComponent = function ensureNativeDetectorComponent(ReanimatedNativeDetector) {
  if (!ReanimatedNativeDetector) {
    const _Error = Error;
    const error = new Error(require(5493) /* tagMessage */.tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
};
