...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16343]
// Exports: captureScreen, ensureModuleIsLoaded

// Module 16342 (_isNativeReflectConstruct)
import noopDefault from "noop" /* 19 */;
import _inheritsDefault from "_inherits" /* 98 */;
import get_ActivityIndicatorDefault from "get ActivityIndicator" /* 16343 */;
...
function captureRef(obj, options) {
  if (ViewShot(16343)) {
    ...
    return tmp(16343).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16343).releaseCapture(str);
    const obj = ViewShot(16343);
  }
}
noopDefault;
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16343)) {
    ...
    return tmp(16343).captureScreen(options);
  } else {
    const _Error = Error;
    error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
