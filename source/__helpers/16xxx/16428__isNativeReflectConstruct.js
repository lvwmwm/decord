...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16429]
// Exports: captureScreen, ensureModuleIsLoaded

// Module 16428 (_isNativeReflectConstruct)
import noopDefault from "noop" /* 19 */;
import _inheritsDefault from "_inherits" /* 98 */;
import get_ActivityIndicatorDefault from "get ActivityIndicator" /* 16429 */;
...
function captureRef(obj, options) {
  if (ViewShot(16429)) {
    ...
    return tmp(16429).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16429).releaseCapture(str);
    const obj = ViewShot(16429);
  }
}
noopDefault;
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16429)) {
    ...
    return tmp(16429).captureScreen(options);
  } else {
    const _Error = Error;
    error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
