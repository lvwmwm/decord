...
// Function ID: 9792
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9792, 9793]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(9792).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9793).crypto) {
    if (typeof tmp(9793).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9793).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9793).crypto) {
    if (typeof tmp(9793).crypto.randomBytes === "function") {
      const _crypto = tmp(9793).crypto;
