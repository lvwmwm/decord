...
// Function ID: 9752
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9752, 9753]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(9752).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9753).crypto) {
    if (typeof tmp(9753).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9753).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9753).crypto) {
    if (typeof tmp(9753).crypto.randomBytes === "function") {
      const _crypto = tmp(9753).crypto;
