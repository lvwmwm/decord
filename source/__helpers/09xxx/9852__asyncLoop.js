...
// Function ID: 9853
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9853, 9854]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(9853).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9854).crypto) {
    if (typeof tmp(9854).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9854).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9854).crypto) {
    if (typeof tmp(9854).crypto.randomBytes === "function") {
      const _crypto = tmp(9854).crypto;
