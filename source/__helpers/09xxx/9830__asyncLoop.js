...
// Function ID: 9831
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9831, 9832]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(9831).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9832).crypto) {
    if (typeof tmp(9832).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9832).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9832).crypto) {
    if (typeof tmp(9832).crypto.randomBytes === "function") {
      const _crypto = tmp(9832).crypto;
