...
// Dependencies: [41, 42, 93, 95, 98, 9852, 9853]
// Exports: Chi, Maj
...
let items = [
  {
    ...
    value: function update(B) {
      let tmp8;
      const self = this;
      HashMD(9853).exists(this);
      ({ buffer, blockLen, view } = this);
      const toBytesResult = HashMD(9852).toBytes(B);
      let num = 0;
  ...
  },
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9853).exists(this);
      HashMD(9853).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      view = HashMD(9852).createView(content);
