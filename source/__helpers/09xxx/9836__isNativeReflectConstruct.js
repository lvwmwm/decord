...
// Dependencies: [41, 42, 93, 95, 98, 9830, 9831]
// Exports: Chi, Maj

// Module 9836 (_isNativeReflectConstruct)
...
let items = [
  {
    ...
    value: function update(B) {
      let tmp8;
      const self = this;
      HashMD(9831).exists(this);
      ({ buffer, blockLen, view } = this);
      const toBytesResult = HashMD(9830).toBytes(B);
      let num = 0;
  ...
  },
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9831).exists(this);
      HashMD(9831).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      view = HashMD(9830).createView(content);
