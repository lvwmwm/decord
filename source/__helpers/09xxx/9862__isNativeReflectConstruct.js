...
// Dependencies: [41, 42, 93, 95, 98, 9856, 9857]
// Exports: Chi, Maj

// Module 9862 (_isNativeReflectConstruct)
...
let items = [
  {
    ...
    value: function update(B) {
      let tmp8;
      const self = this;
      HashMD(9857).exists(this);
      ({ buffer, blockLen, view } = this);
      const toBytesResult = HashMD(9856).toBytes(B);
      let num = 0;
  ...
  },
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9857).exists(this);
      HashMD(9857).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      view = HashMD(9856).createView(content);
