...
// Function ID: 9071
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9071, 9072]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(9071).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9072).crypto) {
    if (typeof tmp(9072).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9072).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9072).crypto) {
    if (typeof tmp(9072).crypto.randomBytes === "function") {
      const _crypto = tmp(9072).crypto;
