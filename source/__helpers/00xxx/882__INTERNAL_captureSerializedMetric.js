...
// Dependencies: [32, 878, 841, 835, 880, 845, 820, 821, 868, 883, 822]
// Exports: _INTERNAL_captureMetric, _INTERNAL_getMetricBuffer

// Module 882 (_INTERNAL_captureSerializedMetric)
...
import createMetricContainerEnvelopeItem from "createMetricContainerEnvelopeItem" /* 883 */;
...
export const _INTERNAL_captureMetric = function _INTERNAL_captureMetric(attributes, scope) {
  ...
  if (client) {
    ...
    if (beforeSendMetricResult) {
      ...
      obj1[0] = tmp17(835).timestampInSeconds();
      if (str10 == null) {
        str10 = "";
      }
      obj1[1] = str10;
      ...
      obj2 = {};
      const tmp17Result1 = tmp17(835);
      const merged2 = Object.assign(tmp17(880).serializeAttributes(attributes));
      const tmp17Result2 = tmp17(880);
      const merged3 = Object.assign(tmp17(880).serializeAttributes(beforeSendMetricResult.attributes, "skip-undefined"));
      obj1[7] = obj2;
      if (tmp17(820).DEBUG_BUILD) {
        const debug4 = tmp17(821).debug;
        debug4.log("[Metric]", obj1);
      }
      prop(client, obj1);
      client.emit("afterCaptureMetric", beforeSendMetricResult);
      const tmp17Result3 = tmp17(880);
    } else if (tmp17(820).DEBUG_BUILD) {
      const debug3 = tmp17(821).debug;
      debug3.log("`beforeSendMetric` returned `null`, will not send metric.");
    }
    const obj4 = getClient;
  } else {
    if (__SENTRY_DEBUG__.DEBUG_BUILD) {
      const debug = tmp8(821).debug;
