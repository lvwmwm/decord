...
// Dependencies: [850, 12567, 12570, 12571, 12585, 12595, 12599, 12603, 12653, 12576, 12605, 12586, 12604, 12642, 12588, 12584, 12578]
// Exports: addTracingHeadersToFetchRequest, instrumentFetchRequest

// Module 12676 (_addTracingHeadersToFetchRequest)
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(12588).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(12588);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(12588).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(12653).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(12653);
    }
    const activeSpan = setHttpStatus(12576).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(12586).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(12586).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(12605).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(12605);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(12603);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(12604).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(12576);
  }
};
