...
// Function ID: 12620
// Name: _flush
// Dependencies: [5, 12598, 12620, 12599, 12571, 12582, 12585, 12578, 12608, 12572, 12594]
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12608).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: closure_2, duration: monitorSlug(12585).timestampInSeconds() - closure_3 };
    const obj2 = monitorSlug(12585);
    const currentScope = monitorSlug(12598).getCurrentScope();
    const obj3 = monitorSlug(12598);
    const client = monitorSlug(12598).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(12598);
    monitorSlug(12582).uuid4();
  }
  let currentScope = _require(12598).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(12598);
  let client = _require(12598).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(12599).DEBUG_BUILD) {
      let logger2 = tmp(12571).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    closure_2 = captureCheckInResult;
    let tmpResult = tmp(12585);
    ...
    let logger = tmp(12571).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(12598);
  captureCheckInResult = _require(12582).uuid4();
};
