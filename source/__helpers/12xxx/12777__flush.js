...
// Function ID: 12778
// Name: _flush
// Dependencies: [5, 12756, 12778, 12757, 12729, 12740, 12743, 12736, 12766, 12730, 12752]
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12766).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: closure_2, duration: monitorSlug(12743).timestampInSeconds() - closure_3 };
    const obj2 = monitorSlug(12743);
    const currentScope = monitorSlug(12756).getCurrentScope();
    const obj3 = monitorSlug(12756);
    const client = monitorSlug(12756).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(12756);
    monitorSlug(12740).uuid4();
  }
  let currentScope = _require(12756).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(12756);
  let client = _require(12756).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(12757).DEBUG_BUILD) {
      let logger2 = tmp(12729).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(12729).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(12756);
  captureCheckInResult = _require(12740).uuid4();
};
