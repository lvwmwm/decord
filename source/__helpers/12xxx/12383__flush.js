...
// Function ID: 12384
// Name: _flush
// Dependencies: [5, 12362, 12384, 12363, 12335, 12346, 12349, 12342, 12372, 12336, 12358]
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12372).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    ...
    const client = monitorSlug(12362).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(12362);
    monitorSlug(12346).uuid4();
  }
  let currentScope = _require(12362).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(12362);
  let client = _require(12362).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(12363).DEBUG_BUILD) {
      let logger2 = tmp(12335).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(12335).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(12362);
  captureCheckInResult = _require(12346).uuid4();
};
