...
// Dependencies: [853, 12298, 12301, 12302, 12316, 12326, 12330, 12334, 12384, 12307, 12336, 12317, 12335, 12373, 12319, 12315, 12309]
// Exports: addTracingHeadersToFetchRequest, instrumentFetchRequest

// Module 12407 (_addTracingHeadersToFetchRequest)
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(12319).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(12319);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(12319).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(12384).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(12384);
    }
    const activeSpan = setHttpStatus(12307).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(12317).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(12317).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(12336).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(12336);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(12334);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(12335).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(12307);
  }
};
