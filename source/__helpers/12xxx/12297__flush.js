...
// Function ID: 12298
// Name: _flush
// Dependencies: [5, 12276, 12298, 12277, 12249, 12260, 12263, 12256, 12286, 12250, 12272]
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(12286).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: closure_2, duration: monitorSlug(12263).timestampInSeconds() - closure_3 };
    const obj2 = monitorSlug(12263);
    const currentScope = monitorSlug(12276).getCurrentScope();
    const obj3 = monitorSlug(12276);
    const client = monitorSlug(12276).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(12276);
    monitorSlug(12260).uuid4();
  }
  let currentScope = _require(12276).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(12276);
  let client = _require(12276).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(12277).DEBUG_BUILD) {
      let logger2 = tmp(12249).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(12249).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(12276);
  captureCheckInResult = _require(12260).uuid4();
};
