...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 814, 1189, 1162, 1152, 1155]
...
let items = [
  {
    ...
    value: function componentDidMount() {
      const client = TouchEventBoundary(814).getClient();
      let addIntegration;
      if (null != client) {
      ...
      if (!tmp5) {
        ...
        const integration = TouchEventBoundary(1189).createIntegration(this.name);
        if (typeof call === "unknown") {
          addIntegration(integration);
        } else {
          call(client, integration);
        }
        const tmpResult = TouchEventBoundary(1189);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function _logTouchEvent(items, label) {
      ...
      if (first) {
        ...
        TouchEventBoundary(814).addBreadcrumb(obj);
        const debug2 = TouchEventBoundary(814).debug;
        const _HermesInternal4 = HermesInternal;
        debug2.log("[TouchEvents] " + obj.message);
        const obj3 = TouchEventBoundary(814);
      } else {
        const debug = TouchEventBoundary(814).debug;
  ...
  },
  {
    ...
    value: function _onTouchStart(_targetInst) {
      ...
      if (_targetInst._targetInst) {
        ...
        obj[0] = label;
        obj[1] = TouchEventBoundary(1152).UI_ACTION_TOUCH;
        const result = TouchEventBoundary(1162).startUserInteractionSpan(obj);
        if (result) {
          const attr = result.setAttribute(TouchEventBoundary(814).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, TouchEventBoundary(1155).SPAN_ORIGIN_AUTO_INTERACTION);
        }
        const obj4 = TouchEventBoundary(1162);
      }
    }
  },
  {
