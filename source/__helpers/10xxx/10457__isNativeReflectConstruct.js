...
// Dependencies: [41, 42, 93, 95, 98, 10455, 10361, 10362, 10368]
...
const items = [
  ...
  },
  {
    ...
    value: function innerExtract(createParsingComponents, index) {
      const tmp3 = NLMonthNameMiddleEndianParser(10455).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      const result = NLMonthNameMiddleEndianParser(10455).parseOrdinalNumberPattern(index[1]);
      if (result > 31) {
        ...
        if (index[4]) {
          obj = parsingComponents.assign("year", tmp(10455).parseYear(index[4]));
        } else {
          parsingComponents.imply("year", tmp(10362).findYearClosestToRef(createParsingComponents.refDate, result, tmp3));
        }
        if (index[2]) {
          const result1 = tmp(10455).parseOrdinalNumberPattern(index[2]);
