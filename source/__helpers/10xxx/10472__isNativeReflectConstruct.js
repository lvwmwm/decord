...
// Dependencies: [41, 42, 93, 95, 98, 10446, 10445, 10451, 10473, 10453]
...
const items = [
  ...
  },
  {
    ...
    value: function innerExtract(reference) {
      ...
      if (undefined !== ENWeekdayParser(10445).WEEKDAY_DICTIONARY[formatted1]) {
        let sum = tmp3(10445).WEEKDAY_DICTIONARY[formatted1];
      } else if ("weekend" == formatted1) {
        if ("last" == str2) {
          let SATURDAY = tmp3(10451).Weekday.SUNDAY;
        } else {
          SATURDAY = tmp3(10451).Weekday.SATURDAY;
        }
        sum = SATURDAY;
        ...
        if (day != tmp3(10451).Weekday.SUNDAY) {
          if (day != tmp3(10451).Weekday.SATURDAY) {
            const diff = day - 1;
            sum = ("last" == str2 ? diff - 1 : diff + 1) % 5 + 1;
          }
        }
        if ("last" == str2) {
          let MONDAY = tmp3(10451).Weekday.FRIDAY;
        } else {
          MONDAY = tmp3(10451).Weekday.MONDAY;
        }
        sum = MONDAY;
      }
      return ENWeekdayParser(10473).createParsingComponentsAtWeekday(reference.reference, sum, str2);
    }
  }
];
