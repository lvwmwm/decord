...
// Dependencies: [41, 42, 93, 95, 98, 10361, 10360, 10366, 10388, 10368]

// Module 10387 (_isNativeReflectConstruct)
...
const items = [
  ...
  },
  {
    ...
    value: function innerExtract(reference) {
      ...
      if (undefined !== ENWeekdayParser(10360).WEEKDAY_DICTIONARY[formatted1]) {
        let sum = tmp3(10360).WEEKDAY_DICTIONARY[formatted1];
      } else if ("weekend" == formatted1) {
        if ("last" == str2) {
          let SATURDAY = tmp3(10366).Weekday.SUNDAY;
        } else {
          SATURDAY = tmp3(10366).Weekday.SATURDAY;
        }
        sum = SATURDAY;
        ...
        if (day != tmp3(10366).Weekday.SUNDAY) {
          if (day != tmp3(10366).Weekday.SATURDAY) {
            const diff = day - 1;
            sum = ("last" == str2 ? diff - 1 : diff + 1) % 5 + 1;
          }
        }
        if ("last" == str2) {
          let MONDAY = tmp3(10366).Weekday.FRIDAY;
        } else {
          MONDAY = tmp3(10366).Weekday.MONDAY;
        }
        sum = MONDAY;
      }
      return ENWeekdayParser(10388).createParsingComponentsAtWeekday(reference.reference, sum, str2);
    }
  }
];
