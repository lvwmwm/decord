...
// Module 10412 (_isNativeReflectConstruct)
...
const items = [
  obj,
  {
    ...
    value: function innerExtract(createParsingComponents) {
      ...
      DECasualTimeParser(10367).implySimilarTime(parsingComponents, createParsingComponents.refDate);
...
obj = {
  ...
  value: function extractTimeComponents(nowResult, formatted) {
    if ("morgen" === formatted) {
      ...
      nowResult.imply("meridiem", DECasualTimeParser(10366).Meridiem.AM);
      ...
      if ("mittag" !== formatted) {
        if ("mittags" !== formatted) {
          if ("nachmittag" === formatted) {
            ...
            nowResult.imply("minute", 0);
            nowResult.imply("second", 0);
            nowResult.imply("meridiem", DECasualTimeParser(10366).Meridiem.PM);
