...
// Dependencies: [41, 42, 93, 95, 98, 10412, 10385, 10363, 10367, 10368]
...
import _isNativeReflectConstruct2 from "_isNativeReflectConstruct" /* 10412 */;
  ...
  if (__setModuleDefault) {
    ...
    obj[1] = function innerExtract(reference) {
      ...
      if ("jetzt" === str2) {
        ...
        const addDurationResult = DECasualDateParser(10363).addDuration(dateWithAdjustedTimezone, { day: 1 });
        DECasualDateParser(10367).assignSimilarDate(parsingComponents, addDurationResult);
        DECasualDateParser(10367).implySimilarTime(parsingComponents, addDurationResult);
        nowResult = parsingComponents;
      } else {
        if ("\u00FCbermorgen" !== str2) {
          if ("uebermorgen" !== str2) {
            if ("gestern" === str2) {
              const addDurationResult1 = DECasualDateParser(10363).addDuration(dateWithAdjustedTimezone, { day: -1 });
              DECasualDateParser(10367).assignSimilarDate(parsingComponents, addDurationResult1);
              DECasualDateParser(10367).implySimilarTime(parsingComponents, addDurationResult1);
              nowResult = parsingComponents;
            } else if ("vorgestern" === str2) {
              const addDurationResult2 = DECasualDateParser(10363).addDuration(dateWithAdjustedTimezone, { day: -2 });
              DECasualDateParser(10367).assignSimilarDate(parsingComponents, addDurationResult2);
              DECasualDateParser(10367).implySimilarTime(parsingComponents, addDurationResult2);
              nowResult = parsingComponents;
            } else {
              nowResult = parsingComponents;
              if (str2.match(/letzte\s*nacht/)) {
                ...
                if (dateWithAdjustedTimezone.getHours() > 6) {
                  addDurationResult3 = DECasualDateParser(10363).addDuration(dateWithAdjustedTimezone, { day: -1 });
                }
                DECasualDateParser(10367).assignSimilarDate(parsingComponents, addDurationResult3);
        ...
        const addDurationResult4 = DECasualDateParser(10363).addDuration(dateWithAdjustedTimezone, { day: 2 });
        DECasualDateParser(10367).assignSimilarDate(parsingComponents, addDurationResult4);
        DECasualDateParser(10367).implySimilarTime(parsingComponents, addDurationResult4);
        nowResult = parsingComponents;
      }
