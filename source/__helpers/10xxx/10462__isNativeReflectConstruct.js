...
// Dependencies: [41, 42, 93, 95, 98, 10367, 10366, 10368]
  ...
  },
  {
    ...
    value: function innerExtract(createParsingComponents) {
      ...
      if ("gisteren" === formatted) {
        ...
        NLCasualDateTimeParser(10367).assignSimilarDate(parsingComponents, date);
      } else if ("van" === formatted) {
        NLCasualDateTimeParser(10367).assignSimilarDate(parsingComponents, refDate);
      } else if ("morgen" === formatted) {
        const _Date2 = Date;
        const date1 = new Date(refDate.getTime());
        date1.setDate(date1.getDate() + 1);
        NLCasualDateTimeParser(10367).assignSimilarDate(parsingComponents, date1);
        NLCasualDateTimeParser(10367).implySimilarTime(parsingComponents, date1);
      }
      if ("ochtend" === formatted1) {
        parsingComponents.imply("meridiem", NLCasualDateTimeParser(10366).Meridiem.AM);
        parsingComponents.imply("hour", 6);
      } else if ("middag" === formatted1) {
        parsingComponents.imply("meridiem", NLCasualDateTimeParser(10366).Meridiem.AM);
        parsingComponents.imply("hour", 12);
      } else if ("namiddag" === formatted1) {
        parsingComponents.imply("meridiem", NLCasualDateTimeParser(10366).Meridiem.PM);
