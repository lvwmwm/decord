...
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 5165, 5201, 5202, 5203]

// Module 5200 (_isNativeReflectConstruct)
import _inheritsDefault from "_inherits" /* 98 */;
import _wrapNativeSuperDefault from "_wrapNativeSuper" /* 158 */;
import calculateGPSValueDefault from "calculateGPSValue" /* 5203 */;
...
function readTags(_raw, dataView, arg2) {
  try {
    ...
    ParseError(5165).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5202).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(5202);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
  ...
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      ...
      if (typeof calculateGPSValueDefault[tmp2] === "function") {
        tmp12 = tmp13(5203)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(5203);
      }
      tmp13 = importDefault;
    }
    return tmp12;
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(5165).objectAssign(obj, obj, obj);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(5165).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(5165);
    obj = {};
    ...
    ParseError(5165).objectAssign(obj.attributes, obj);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(5165);
  }
  ParseError(5165).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
