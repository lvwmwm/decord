...
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 5174, 5210, 5211, 5212]

// Module 5209 (_isNativeReflectConstruct)
import _inheritsDefault from "_inherits" /* 98 */;
import _wrapNativeSuperDefault from "_wrapNativeSuper" /* 158 */;
import calculateGPSValueDefault from "calculateGPSValue" /* 5212 */;
...
function readTags(_raw, dataView, arg2) {
  try {
    ...
    ParseError(5174).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5211).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(5211);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
  ...
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      ...
      if (typeof calculateGPSValueDefault[tmp2] === "function") {
        tmp12 = tmp13(5212)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(5212);
      }
      tmp13 = importDefault;
    }
    return tmp12;
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(5174).objectAssign(obj, obj, obj);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(5174).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(5174);
    obj = {};
    ...
    ParseError(5174).objectAssign(obj.attributes, obj);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(5174);
  }
  ParseError(5174).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
