...
// Dependencies: [13861, 13887, 13886, 13885, 13874]

// Module 13889 (__exportStarResult25)
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13861).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13887).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13887).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13886).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13885).FormatApproximately(internalSlots, result);
    ...
    return tmp(13874).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13886).FormatNumeric(internalSlots, isNaN);
};
