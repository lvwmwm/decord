...
// Dependencies: [13775, 13801, 13800, 13799, 13788]

// Module 13803 (__exportStarResult25)
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13775).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13801).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13801).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13800).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13799).FormatApproximately(internalSlots, result);
    ...
    return tmp(13788).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13800).FormatNumeric(internalSlots, isNaN);
};
