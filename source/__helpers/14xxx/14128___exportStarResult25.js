...
// Dependencies: [14100, 14126, 14125, 14124, 14113]

// Module 14128 (__exportStarResult25)
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(14100).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(14126).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(14126).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(14125).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(14124).FormatApproximately(internalSlots, result);
    ...
    return tmp(14113).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(14125).FormatNumeric(internalSlots, isNaN);
};
