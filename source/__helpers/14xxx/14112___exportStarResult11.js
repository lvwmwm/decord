...
// Module 14112 (__exportStarResult11)
  ...
  if (eq.eq(timesResult)) {
    ...
    if ("zero" === result) {
      ...
      if (minusResult.lessThan(minusResult1)) {
        ...
        tmp2(14100).invariant(minusResult.eq(minusResult1), "d1 should be equal to d2");
        if ("half-zero" === result) {
          return timesResult;
        } else if ("half-infinity" === result) {
          return roundedNumber;
        } else {
          tmp2(14100).invariant("half-even" === result, "unsignedRoundingMode should be half-even");
