...
arg5.default = {
  isPngFile(dataView) {
    let tmp = dataView;
    if (tmp) {
      tmp = require(4802) /* getDataView */.getStringFromDataView(dataView, 0, u0089PNGrnu001An.length) === u0089PNGrnu001An;
  ...
  findPngOffsets(byteLength, flag2) {
    ...
    if (length + c4 + c5 <= byteLength.byteLength) {
      while (true) {
        ...
        let tmp3 = length;
        if (module(4805).USE_PNG_FILE) {
          let tmp4 = require;
          let obj1 = require(4802) /* getDataView */;
          let tmp5 = c7;
          let tmp6 = c5;
