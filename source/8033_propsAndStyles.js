...
// Function ID: 63795
// Name: propsAndStyles
// Dependencies: [65, 8034, 8025, 8035, 8038, 8026, 8024]
// Exports: extract, withoutXY

// Module 8033 (propsAndStyles)
...
function getMarker(markerEnd) {
  if (markerEnd) {
    let tmp3;
    const match = markerEnd.match(require(8034) /* pickNotNil */.idPattern);
    if (match) {
      tmp3 = match[1];
    }
    return tmp3;
...
function extractProps(markerMid, self) {
  ...
  const obj = {};
  const items = [];
  importDefault(8025)(obj, markerMid, self);
  importDefault(8035)(obj, markerMid, items);
  importDefault(8038)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp4 = importDefault(8026)(markerMid);
  if (null !== tmp4) {
    obj.matrix = tmp4;
  }
  if (null != opacity) {
    obj.opacity = importDefault(8024)(opacity);
  }
  if (null != display) {
    let str2;
    if ("none" === display) {
  ...
  if (clipPath) {
    const match = clipPath.match(require(8034) /* pickNotNil */.idPattern);
    if (match) {
  ...
  if (mask) {
    const match1 = mask.match(require(8034) /* pickNotNil */.idPattern);
    if (match1) {
      obj.mask = match1[1];
    } else {
  ...
  if (filter) {
    const match2 = filter.match(require(8034) /* pickNotNil */.idPattern);
    if (match2) {
      obj.filter = match2[1];
    } else {
