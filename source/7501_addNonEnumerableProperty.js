...
// Dependencies: [7494, 7495, 7502, 7503, 7504]

// Module 7501 (addNonEnumerableProperty)
const require = arg1;
...
function serializeEventTarget(arg0) {
  try {
    if (obj.isElement(arg0)) {
      let htmlTreeAsStringResult = require(7503) /* _htmlElementAsString */.htmlTreeAsString(arg0);
      const tmp2Result = require(7503) /* _htmlElementAsString */;
    } else {
      const _Object = Object;
...
arg5.extractExceptionKeysForMessage = function extractExceptionKeysForMessage(arg0) {
  ...
  if (first) {
    if (first.length >= num) {
      return require(7504) /* isMatchingPattern */.truncate(first, num);
    } else {
      ...
      if (length > 0) {
        ...
        if (length !== keys.length) {
          truncateResult = require(7504) /* isMatchingPattern */.truncate(joined, num);
