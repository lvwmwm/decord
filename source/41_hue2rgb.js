...
export default function normalizeColor(arg0) {
  ...
  if ("number" === tmp) {
    ...
    if (undefined === obj) {
      const text = `${call("[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+")}|`;
      const text1 = `${`${call("[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+")}|`}${commaSeparatedCall("[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+")}|${callWithSlashSeparator("[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+")}`;
      obj = {};
      const _RegExp = RegExp;
      const regExp = new RegExp("rgb(" + text1 + ")");
      obj.rgb = regExp;
      const _RegExp2 = RegExp;
      const regExp1 = new RegExp("rgba(" + text1 + ")");
      obj.rgba = regExp1;
      const _RegExp3 = RegExp;
      const regExp2 = new RegExp("hsl" + call("[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+%", "[-+]?\\d*\\.?\\d+%"));
      obj.hsl = regExp2;
      const _RegExp4 = RegExp;
      const tmp74 = commaSeparatedCall("[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+");
      ...
      const text2 = `hwb(${callModern("[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+%", "[-+]?\\d*\\.?\\d+%")}`;
      const regExp4 = new RegExp(`hwb(${callModern("[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+%", "[-+]?\\d*\\.?\\d+%")}` + "|" + callWithSlashSeparator("[-+]?\\d*\\.?\\d+", "[-+]?\\d*\\.?\\d+%", "[-+]?\\d*\\.?\\d+%", "[-+]?\\d*\\.?\\d+") + ")");
      tmp = regExp4;
      obj.hwb = regExp4;
    ...
    if (match) {
      ...
      if (null != num) {
        ...
        let match1 = rgba.exec(arg0);
        if (!match1) {
          const rgb = tmp2.rgb;
          match1 = rgb.exec(arg0);
        }
        if (!match1) {
          const hex3 = tmp2.hex3;
          const match2 = hex3.exec(arg0);
          if (match2) {
            const _parseInt3 = parseInt;
            let tmp12 = parseInt(`${tmp6[1]}${tmp6[1]}${tmp6[2]}${tmp6[2]}${tmp6[3]}${tmp6[3]}ff`, 16) >>> 0;
          } else {
            const hex8 = tmp2.hex8;
            const match3 = hex8.exec(arg0);
            if (match3) {
              const _parseInt2 = parseInt;
              tmp12 = parseInt(match3[1], 16) >>> 0;
            } else {
              const hex4 = tmp2.hex4;
              const match4 = hex4.exec(arg0);
              if (match4) {
                const _parseInt = parseInt;
                tmp12 = parseInt(match4[1] + match4[1] + match4[2] + match4[2] + match4[3] + match4[3] + match4[4] + match4[4], 16) >>> 0;
              } else {
                const hsl = tmp2.hsl;
                const match5 = hsl.exec(arg0);
                if (match5) {
                  const tmp45 = parse360(match5[1]);
                  tmp12 = (255 | hslToRgb(tmp45, parsePercentage(match5[2]), parsePercentage(match5[3]))) >>> 0;
                  const tmp47 = parsePercentage(match5[2]);
                } else {
                  const hsla = tmp2.hsla;
                  const match6 = hsla.exec(arg0);
                  if (match6) {
                    let num11 = 6;
                    if (undefined !== match6[6]) {
                      const tmp37 = parse360(match6[num11]);
                      const tmp39 = parsePercentage(match6[7]);
                      num11 = 0;
                      let tmp34 = (hslToRgb(tmp37, parsePercentage(match6[7]), parsePercentage(match6[8])) | parse1(match6[9])) >>> 0;
                      const tmp40 = hslToRgb(tmp37, parsePercentage(match6[7]), parsePercentage(match6[8]));
                    } else {
                      const tmp29 = parse360(match6[2]);
                      const tmp31 = parsePercentage(match6[3]);
                      tmp34 = (hslToRgb(tmp29, parsePercentage(match6[3]), parsePercentage(match6[4])) | parse1(match6[5])) >>> 0;
                      const tmp32 = hslToRgb(tmp29, parsePercentage(match6[3]), parsePercentage(match6[4]));
                    }
                  } else {
                    const hwb = tmp2.hwb;
                    const match7 = hwb.exec(arg0);
                    tmp12 = null;
                    if (match7) {
                      let num2 = 5;
                      if (undefined !== match7[5]) {
                        const tmp21 = parse360(match7[num2]);
                        const tmp23 = parsePercentage(match7[6]);
                        num2 = 0;
                        let tmp18 = (hwbToRgb(tmp21, parsePercentage(match7[6]), parsePercentage(match7[7])) | parse1(match7[8])) >>> 0;
                        const tmp24 = hwbToRgb(tmp21, parsePercentage(match7[6]), parsePercentage(match7[7]));
                      } else {
                        const tmp15 = parse360(match7[2]);
                        tmp18 = (255 | hwbToRgb(tmp15, parsePercentage(match7[3]), parsePercentage(match7[4]))) >>> 0;
                        const tmp17 = parsePercentage(match7[3]);
                      }
                    }
                  }
                }
        ...
        if (undefined !== match1[9]) {
          const tmp61 = parse255(match1[num39]) << 24;
          const tmp62 = parse255(match1[10]) << 16;
          num39 = 0;
          let tmp54 = (tmp61 | tmp62 | parse255(match1[11]) << 8 | parse1(match1[12])) >>> 0;
          const tmp63 = parse255(match1[11]) << 8;
        } else if (undefined !== match1[5]) {
          const tmp56 = parse255(match1[5]) << 24;
          const tmp57 = parse255(match1[6]) << 16;
          tmp54 = (tmp56 | tmp57 | parse255(match1[7]) << 8 | parse1(match1[8])) >>> 0;
          const tmp58 = parse255(match1[7]) << 8;
        } else {
          const tmp52 = parse255(match1[2]) << 24;
          tmp54 = (tmp52 | parse255(match1[3]) << 16 | parse255(match1[4]) << 8 | 255) >>> 0;
          const tmp53 = parse255(match1[3]) << 16;
        }
      }
    }
  }
