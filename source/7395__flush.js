...
// Function ID: 7396
// Name: _flush
// Dependencies: [5, 7374, 7396, 7375, 7347, 7358, 7361, 7354, 7384, 7348, 7370]
...
function endSession() {
  const isolationScope = require(7374) /* getClient */.getIsolationScope();
  const obj = require(7374) /* getClient */;
  const currentScope = require(7374) /* getClient */.getCurrentScope();
  ...
  const obj3 = require(7374) /* getClient */;
  const currentScope1 = require(7374) /* getClient */.getCurrentScope();
  const tmpResult1 = require(7374) /* getClient */;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = require(7374) /* getClient */.getCurrentScope();
  const obj = require(7374) /* getClient */;
  return currentScope.captureException(arg0, require(7396) /* applyClientOptions */.parseEventHintOrCaptureContext(arg1));
};
...
export const captureSession = function captureSession() {
  ...
  if (flag) {
    ...
    const obj5 = require(7374) /* getClient */;
  }
};
...
export const isInitialized = function isInitialized() {
  return require(7374) /* getClient */.getClient();
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(7384).DEFAULT_ENVIRONMENT;
  }
  const obj5 = require(7374) /* getClient */;
  const userAgent = require(7348) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {}.userAgent;
  let tmpResult = tmp(7370);
  obj = { release, environment, user: null };
  const tmp3 = require(7348) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {};
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    ...
    obj[3] = monitorSlug(7361).timestampInSeconds() - closure_3;
    const obj2 = monitorSlug(7361);
    const currentScope = monitorSlug(7374).getCurrentScope();
    const obj3 = monitorSlug(7374);
    const client = monitorSlug(7374).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(7374);
    monitorSlug(7358).uuid4();
  }
  let currentScope = _require(7374).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(7374);
  let client = _require(7374).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(7375).DEBUG_BUILD) {
      let logger2 = tmp(7347).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(7347).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(7374);
  captureCheckInResult = _require(7358).uuid4();
};
