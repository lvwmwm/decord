...
// Dependencies: [1257, 13247, 13270, 13265, 13271, 13267]
// Exports: default

// Module 13269 (partitionNumberIntoParts)
...
function partitionNumberIntoParts(currencyGroup, arg1, arg2, arg3, arg4, arg5, arg6, arg7, roundingIncrement, result) {
  ...
  if (roundedNumber.isNaN()) {
    ...
    const tmp4 = require(13271).digitMapping[arg4];
    require = tmp4;
    ...
    if (undefined === substr1) {
      if ("scientific" === arg2) {
        if (roundedNumber.isFinite()) {
          ...
          const obj5 = { type: "exponentInteger", value: require(13267) /* __exportStarResult28 */.ToRawFixed(_default, 0, 0, roundingIncrement, result).formattedString };
          items.push(obj5);
        }
      }
      return items;
...
const regExp = new RegExp("^".concat(require("module_13270").S_UNICODE_REGEX.source));
const regExp1 = new RegExp("".concat(require("module_13270").S_UNICODE_REGEX.source, "$"));
const re5 = /[#0](?:[\.,][#0]+)*/g;
    ...
    do {
      ...
      if (str22) {
        if ("{0}" === str22) {
          ...
          let tmp40 = !tmp3;
          let tmp39 = partitionNumberIntoParts;
          if (tmp40) {
          ...
          let tmp50 = first2;
          let tmp51 = style;
          let applyResult = push2.apply(items, tmp39(tmp29, magnitude, notation, exponent, numberingSystem, tmp40, first2, style, currencyDisplay.roundingIncrement, require(13265) /* __exportStarResult22 */.GetUnsignedRoundingMode(currencyDisplay.roundingMode, tmp30)));
          tmp32 = tmp31;
        } else if ("-" === str22) {
  ...
  if ("currency" === style) {
    if ("name" === currencyDisplay.currencyDisplay) {
      if (numbers.currencies[currencyDisplay.currency]) {
        ...
        let currency3 = selectPlural(arg2, roundedNumber4.times(digitsToString.default.pow(10, exponent)).toNumber(), tmp70.displayName);
        const _default3 = digitsToString.default;
        const timesResult = roundedNumber4.times(digitsToString.default.pow(10, exponent));
      } else {
    ...
    if (numbers.units.simple[unit]) {
      ...
      let str30 = selectPlural(arg2, roundedNumber3.times(digitsToString.default.pow(10, exponent)).toNumber(), numbers.units.simple[unit][unitDisplay]);
      const _default2 = digitsToString.default;
      const timesResult1 = roundedNumber3.times(digitsToString.default.pow(10, exponent));
    } else {
      const parts2 = unit.split("-per-");
      [tmp54, tmp55] = parts2;
      const roundedNumber2 = magnitude.roundedNumber;
      const tmp59 = selectPlural(arg2, roundedNumber2.times(digitsToString.default.pow(10, exponent)).toNumber(), numbers.units.simple[tmp54][unitDisplay]);
      const str24 = numbers.units.simple[tmp55].perUnit[unitDisplay];
      if (str24) {
      ...
      const timesResult2 = roundedNumber2.times(digitsToString.default.pow(10, exponent));
    }
    const items2 = [];
