  ...
  if (num4 < arr.length) {
    ...
    do {
      while (true) {
        ...
        if (tmp13 >= 36) {
          let tmp14 = error;
          let tmp15 = error("invalid-input");
        }
        let tmp16 = floor;
        if (tmp13 > floor((2147483647 - tmp6) / num8)) {
          let tmp17 = error;
          let tmp18 = error("overflow");
        }
      ...
      let tmp25 = adapt(sum1 - num7, sum2, 0 === num7);
      if (floor(result) > 2147483647 - num2) {
        let tmp28 = error;
        let tmp29 = error("overflow");
      }
      let tmp30 = floor;
