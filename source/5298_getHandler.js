...
// Function ID: 45518
// Name: getHandler
// Dependencies: [5290, 5259, 5255, 5258, 5256, 5257]

// Module 5298 (getHandler)
const require = arg1;
...
let closure_3 = (() => {
  function getHandler(arg0, onBegin) {
    if (outer1_0(outer1_1[0]).CALLBACK_TYPE.BEGAN === arg0) {
      return onBegin.onBegin;
    } else if (outer1_0(outer1_1[0]).CALLBACK_TYPE.START === arg0) {
      return onBegin.onStart;
    } else if (outer1_0(outer1_1[0]).CALLBACK_TYPE.UPDATE === arg0) {
      return onBegin.onUpdate;
    } else if (outer1_0(outer1_1[0]).CALLBACK_TYPE.CHANGE === arg0) {
      return onBegin.onChange;
    } else if (outer1_0(outer1_1[0]).CALLBACK_TYPE.END === arg0) {
      return onBegin.onEnd;
    } else if (outer1_0(outer1_1[0]).CALLBACK_TYPE.FINALIZE === arg0) {
      return onBegin.onFinalize;
    } else if (outer1_0(outer1_1[0]).CALLBACK_TYPE.TOUCHES_DOWN === arg0) {
      return onBegin.onTouchesDown;
    } else if (outer1_0(outer1_1[0]).CALLBACK_TYPE.TOUCHES_MOVE === arg0) {
      return onBegin.onTouchesMove;
    } else if (outer1_0(outer1_1[0]).CALLBACK_TYPE.TOUCHES_UP === arg0) {
      return onBegin.onTouchesUp;
    } else if (outer1_0(outer1_1[0]).CALLBACK_TYPE.TOUCHES_CANCELLED === arg0) {
      return onBegin.onTouchesCancelled;
    }
  }
  getHandler.__closure = { CALLBACK_TYPE: require(5290) /* _callSuper */.CALLBACK_TYPE };
...
let closure_5 = (() => {
  function touchEventTypeToCallbackType(arg0) {
    if (outer1_0(outer1_1[1]).TouchEventType.TOUCHES_DOWN === arg0) {
      return outer1_0(outer1_1[0]).CALLBACK_TYPE.TOUCHES_DOWN;
    } else if (outer1_0(outer1_1[1]).TouchEventType.TOUCHES_MOVE === arg0) {
      return outer1_0(outer1_1[0]).CALLBACK_TYPE.TOUCHES_MOVE;
    } else if (outer1_0(outer1_1[1]).TouchEventType.TOUCHES_UP === arg0) {
      return outer1_0(outer1_1[0]).CALLBACK_TYPE.TOUCHES_UP;
    } else if (outer1_0(outer1_1[1]).TouchEventType.TOUCHES_CANCELLED === arg0) {
      return outer1_0(outer1_1[0]).CALLBACK_TYPE.TOUCHES_CANCELLED;
    } else {
      return outer1_0(outer1_1[0]).CALLBACK_TYPE.UNDEFINED;
    }
  }
  touchEventTypeToCallbackType.__closure = { TouchEventType: require(5259).TouchEventType, CALLBACK_TYPE: require(5290) /* _callSuper */.CALLBACK_TYPE };
...
let closure_7 = (() => {
  function runWorklet(arg0, arg1, arg2) {
    ...
    if (arg1.isWorklet[arg0]) {
      ...
      console.warn(outer1_0(outer1_1[2]).tagMessage("Animated gesture callback must be a worklet"));
      const obj2 = outer1_0(outer1_1[2]);
    }
  }
  runWorklet.__closure = { getHandler: closure_3, tagMessage: require(5255) /* hasProperty */.tagMessage };
...
arg5.useAnimatedGesture = function useAnimatedGesture(current2, needsToReattachResult) {
  if (sharedValue(sharedValue1[3]).Reanimated) {
    const Reanimated = sharedValue(sharedValue1[3]).Reanimated;
    sharedValue = Reanimated.useSharedValue(null);
    const Reanimated2 = sharedValue(sharedValue1[3]).Reanimated;
    ...
    let obj = { sharedHandlersCallbacks: sharedValue, isStateChangeEvent: closure_9, State: sharedValue(sharedValue1[4]).State, runWorklet: closure_7, CALLBACK_TYPE: sharedValue(sharedValue1[0]).CALLBACK_TYPE, lastUpdateEvent: sharedValue1, isTouchEvent: closure_11, stateControllers: items, GestureStateManager: sharedValue(sharedValue1[5]).GestureStateManager, TouchEventType: sharedValue(sharedValue1[1]).TouchEventType, touchEventTypeToCallbackType: closure_5 };
    ...
    const Reanimated3 = sharedValue(sharedValue1[3]).Reanimated;
    current2.animatedEventHandler = Reanimated3.useEvent(pnpm_useAnimatedGestureTs6, ["onGestureHandlerStateChange", "onGestureHandlerEvent"], needsToReattachResult);
