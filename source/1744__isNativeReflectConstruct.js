...
// Dependencies: [41, 42, 93, 95, 98, 19, 21, 1609, 1714, 1650]

// Module 1744 (_isNativeReflectConstruct)
  ...
  isJest,
  {
    ...
    value: function setShouldAnimateExiting() {
      ...
      if (1 === closure_5.count(this.props.children)) {
        const findNodeHandleResult = LayoutAnimationConfig(1714).findNodeHandle(self);
        if (findNodeHandleResult) {
          const result = tmp(1650).setShouldAnimateExitingForTag(findNodeHandleResult, !self.props.skipExiting);
          const tmpResult = tmp(1650);
        }
        const obj = LayoutAnimationConfig(1714);
