...
// Module 7335 (_addTracingHeadersToFetchRequest)
    ...
    if (headers) {
      ...
      if (isInstanceOfResult1) {
        ...
        if (Array.isArray(headers)) {
          ...
          const items1 = [null, tmp2];
          const items2 = [items1];
          const combined1 = items.concat(callback(found.map((arg0) => {
            let tmp2;
            let tmp3;
...
const _module5 = require(dependencyMap[8]);
    ...
    while (true) {
      ...
      if (hasTracingEnabledResult) {
        if (!tmp17) {
          ...
          let num5 = 13;
          let tmp22 = callback2(obj1, require(dependencyMap[13]).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, str);
          let str5 = "http.client";
          obj.attributes = callback2(tmp22, require(dependencyMap[13]).SEMANTIC_ATTRIBUTE_SENTRY_OP, "http.client");
