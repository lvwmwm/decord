...
// Function ID: 7548
// Name: _flush
// Dependencies: [5, 7526, 7548, 7527, 7499, 7510, 7513, 7506, 7536, 7500, 7522]
...
function endSession() {
  const isolationScope = require(7526) /* getClient */.getIsolationScope();
  const obj = require(7526) /* getClient */;
  const currentScope = require(7526) /* getClient */.getCurrentScope();
  ...
  const currentScope1 = require(7526) /* getClient */.getCurrentScope();
  const tmpResult1 = require(7526) /* getClient */;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = require(7526) /* getClient */.getCurrentScope();
  const obj = require(7526) /* getClient */;
  return currentScope.captureException(arg0, require(7548) /* applyClientOptions */.parseEventHintOrCaptureContext(arg1));
};
...
export const captureSession = function captureSession() {
  ...
  if (flag) {
    ...
    const obj5 = require(7526) /* getClient */;
  }
};
...
export const isInitialized = function isInitialized() {
  return require(7526) /* getClient */.getClient();
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(7536).DEFAULT_ENVIRONMENT;
  }
  const obj5 = require(7526) /* getClient */;
  const userAgent = require(7500) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {}.userAgent;
  let tmpResult = tmp(7522);
  obj = { release, environment, user: null };
  const tmp3 = require(7500) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {};
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    ...
    obj[3] = monitorSlug(7513).timestampInSeconds() - closure_3;
    const obj2 = monitorSlug(7513);
    const currentScope = monitorSlug(7526).getCurrentScope();
    const obj3 = monitorSlug(7526);
    const client = monitorSlug(7526).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(7526);
    monitorSlug(7510).uuid4();
  }
  let currentScope = _require(7526).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(7526);
  let client = _require(7526).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(7527).DEBUG_BUILD) {
      let logger2 = tmp(7499).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(7499).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(7526);
  captureCheckInResult = _require(7510).uuid4();
};
