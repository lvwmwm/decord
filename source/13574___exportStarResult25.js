...
// Dependencies: [13546, 13572, 13571, 13570, 13559]

// Module 13574 (__exportStarResult25)
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13546).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13572).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13572).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13571).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13570).FormatApproximately(internalSlots, result);
    ...
    return tmp(13559).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13571).FormatNumeric(internalSlots, isNaN);
};
