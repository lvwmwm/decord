...
// Function ID: 9073
// Name: pbkdf2Init
// Dependencies: [5, 9070, 9069, 9073]
// Exports: pbkdf2, pbkdf2Async

// Module 9072 (pbkdf2Init)
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  let c;
  let dkLen;
  require(9070) /* number */.hash(sha256);
  ({ c, dkLen, asyncTick } = require(9069) /* _asyncLoop */.checkOpts({ dkLen: 32, asyncTick: 10 }, arg3));
  require(9070) /* number */.number(c);
  require(9070) /* number */.number(dkLen);
  require(9070) /* number */.number(asyncTick);
  if (c < 1) {
    const _Error = Error;
    ...
    const toBytesResult = tmp(9069).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9073).hmac;
    let obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9069).toBytes(B);
  ...
  const checkOptsResult = require(9069) /* _asyncLoop */.checkOpts({ dkLen: 32, asyncTick: 10 }, arg3);
}
...
export const pbkdf2 = function pbkdf2(sha256, B, B, arg3) {
  ...
  const uint8Array = new Uint8Array(4);
  const view = require(9069) /* _asyncLoop */.createView(uint8Array);
