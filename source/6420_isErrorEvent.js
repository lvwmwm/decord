...
// Function ID: 6421
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 6412, 6393, 6365, 6419, 6376, 6372, 6388, 6389, 6421, 6408, 6409, 6392, 6414, 6401, 6422, 6405, 6423, 6370]

// Module 6420 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release === "tee") {
        if (BaseClient(6393).DEBUG_BUILD) {
          const logger = tmp(6365).logger;
        ...
        BaseClient(6388).updateSession(release, { init: false });
        const obj = BaseClient(6388);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(6421).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(6421).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(6421);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(6408).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(6393).DEBUG_BUILD) {
        let logger = tmp2(6365).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(6389).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(6421).setupIntegrations(this, integrations);
      const obj = BaseClient(6421);
      const result = BaseClient(6421).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(6405).parseSampleRate(sampleRate);
        let obj = str(6405);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult !== "reType") {
          const _Math = Math;
          if (Math.random() > parseSampleRateResult) {
            self.recordDroppedEvent("sample_rate", "error", type);
            let _HermesInternal = HermesInternal;
            let sentryError = new str(6422).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(6389).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(6393).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(6365).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(6423);
