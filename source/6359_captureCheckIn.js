...
function captureCheckIn(arg0, arg1) {
  ...
  const obj = require(6338) /* getCurrentScope */;
...
function endSession() {
  const isolationScope = require(6338) /* getCurrentScope */.getIsolationScope();
  const obj = require(6338) /* getCurrentScope */;
  const currentScope = require(6338) /* getCurrentScope */.getCurrentScope();
  const tmp = currentScope.getSession() || isolationScope.getSession();
  if (tmp) {
    require(6334) /* updateSession */.closeSession(tmp);
    const obj5 = require(6334) /* updateSession */;
  }
  _sendSessionUpdate();
...
function _sendSessionUpdate() {
  ...
  const obj = require(6338) /* getCurrentScope */;
  const currentScope = require(6338) /* getCurrentScope */.getCurrentScope();
  const obj3 = require(6338) /* getCurrentScope */;
  const client = require(6338) /* getCurrentScope */.getClient();
  const tmp = currentScope.getSession() || isolationScope.getSession();
  let tmp2 = tmp;
  if (tmp) {
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = require(6338) /* getCurrentScope */.getCurrentScope();
  const obj = require(6338) /* getCurrentScope */;
  return currentScope.captureException(arg0, require(6360) /* applyClientOptions */.parseEventHintOrCaptureContext(arg1));
};
...
export const isInitialized = function isInitialized() {
  return require(6338) /* getCurrentScope */.getClient();
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = require(6348).DEFAULT_ENVIRONMENT;
  }
  const obj5 = require(6338) /* getCurrentScope */;
  const userAgent = require(6312) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {}.userAgent;
  const tmp3 = require(6312) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {};
  obj = { release, environment };
  const obj8 = require(6334) /* updateSession */;
  ...
  if (tmp8) {
    ...
    require(6334) /* updateSession */.updateSession(session1, obj1);
    const obj11 = require(6334) /* updateSession */;
  }
  endSession();
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: closure_2, duration: monitorSlug(6325).timestampInSeconds() - callback };
    callback(obj);
  }
  let closure_2 = captureCheckIn({ monitorSlug, status: "in_progress" }, arg2);
  captureCheckIn = _require(6325).timestampInSeconds();
  let obj = { monitorSlug, status: "in_progress" };
  const obj2 = _require(6325);
  return _require(6338).withIsolationScope(() => {
