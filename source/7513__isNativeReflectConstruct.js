...
// Dependencies: [32, 41, 42, 93, 95, 96, 98, 7453, 7468, 7450, 7514, 7478, 7481, 7482, 7454, 7507, 7465, 7515, 7476, 7459, 7490, 7509]

// Module 7513 (_isNativeReflectConstruct)
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7514).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7514);
      return ServerRuntimeClient(7478).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7478);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7514).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7481).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7481);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7507).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7482).DEBUG_BUILD) {
        const logger = tmp2(7454).logger;
    ...
    value: function captureCheckIn(checkInId) {
      ...
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7515);
          ...
          if (ServerRuntimeClient(7482).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7454).logger;
          ...
          if (ServerRuntimeClient(7482).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7454).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7465);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7476)._getSpanForScope(arg0);
        ...
        const tmpResult1 = ServerRuntimeClient(7490);
        if (_getSpanForScopeResult) {
