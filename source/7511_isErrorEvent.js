...
// Function ID: 7512
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 7503, 7484, 7456, 7510, 7467, 7463, 7479, 7480, 7512, 7499, 7500, 7483, 7505, 7492, 7513, 7496, 7514, 7461]

// Module 7511 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(7484).DEBUG_BUILD) {
          const logger = tmp(7456).logger;
        ...
        BaseClient(7479).updateSession(release, { init: false });
        const obj = BaseClient(7479);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(7512).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(7512).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(7512);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(7499).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(7484).DEBUG_BUILD) {
        let logger = tmp2(7456).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(7480).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(7512).setupIntegrations(this, integrations);
      const obj = BaseClient(7512);
      const result = BaseClient(7512).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(7496).parseSampleRate(sampleRate);
        let obj = str(7496);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(7513).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(7480).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(7484).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(7456).logger;
