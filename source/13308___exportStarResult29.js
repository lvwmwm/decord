...
// Dependencies: [1281, 13289, 13290, 13292, 13302]
...
export const ToRawPrecision = function ToRawPrecision(ZERO, minimumSignificantDigits, maximumSignificantDigits, result) {
  ...
  let tmp4;
  if (ZERO.isZero()) {
    let repeatResult = require(13290) /* getMultiInternalSlots */.repeat("0", maximumSignificantDigits);
    let roundedNumber = require(13292) /* TEN */.ZERO;
    ...
    result = require(13302) /* __exportStarResult11 */.ApplyUnsignedRoundingMode(ZERO, timesResult, roundedNumber, result);
  ...
  if (roundingMagnitude >= maximumSignificantDigits - 1) {
    let sum = repeatResult + tmp16(13290).repeat("0", roundingMagnitude - maximumSignificantDigits + 1);
    ...
    tmp16(13290).invariant(roundingMagnitude < 0, "e should be less than 0");
    sum = `0.${tmp16(13290).repeat("0", -num3 - 1)}${arr}`;
