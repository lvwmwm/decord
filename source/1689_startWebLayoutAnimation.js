...
// Dependencies: [1659, 1690, 1638, 1703, 1704, 1722, 1720, 1721, 1686]

// Module 1689 (startWebLayoutAnimation)
const require = arg1;
const dependencyMap = arg6;
function startWebLayoutAnimation(props, _componentDOMRef, ENTERING, easingY) {
  if (ENTERING === require(1659) /* isWorkletFunction */.LayoutAnimationType.ENTERING) {
    let entering = props.entering;
  } else if (ENTERING === tmp(1659).LayoutAnimationType.EXITING) {
    entering = props.exiting;
  } else {
    entering = null;
    if (ENTERING === tmp(1659).LayoutAnimationType.LAYOUT) {
      entering = props.layout;
    }
  }
  let processedConfig = null;
  if (entering) {
    const tmp5 = entering instanceof tmp(1704).Keyframe;
    if (tmp5) {
      let tmpResult = tmp(1722);
    ...
    let flag = !(animationWithInitialValues in tmp(1690).Animations) && !(ENTERING === tmp(1659).LayoutAnimationType.LAYOUT || tmp5 || undefined !== entering.initialValues);
    if (flag) {
      let logger = tmp(1638).logger;
    ...
    if (!flag) {
      ...
      processedConfig = tmp(1703).getProcessedConfig(animationWithInitialValues, ENTERING, entering);
      const tmpResult1 = tmp(1703);
    }
    const tmp8 = ENTERING === tmp(1659).LayoutAnimationType.LAYOUT || tmp5 || undefined !== entering.initialValues;
  }
  const result = require(1703) /* setElementAnimation */.maybeModifyStyleForKeyframe(_componentDOMRef, props.entering);
  let animationName;
  if (processedConfig != null) {
    animationName = processedConfig.animationName;
  }
  if (animationName in require(1690) /* TransitionType */.Animations) {
    let animationName1;
    if (processedConfig != null) {
    ...
    })(tmp(1690).Animations[animationName1].style, _componentDOMRef.style);
  }
  if (processedConfig) {
    if (tmp(1659).LayoutAnimationType.ENTERING === ENTERING) {
      tmp(1703).setElementAnimation(_componentDOMRef, processedConfig, true);
      const tmpResult3 = tmp(1703);
    } else if (tmp(1659).LayoutAnimationType.LAYOUT === ENTERING) {
      easingY.reversed = processedConfig.reversed;
      const result1 = tmp(1703).handleLayoutTransition(_componentDOMRef, processedConfig, easingY);
      const tmpResult4 = tmp(1703);
    } else if (tmp(1659).LayoutAnimationType.EXITING === ENTERING) {
      const result2 = tmp(1703).handleExitingAnimation(_componentDOMRef, processedConfig);
      const tmpResult5 = tmp(1703);
    }
  } else {
    const elementVisible = tmp(1720).makeElementVisible(_componentDOMRef, 0);
    const tmpResult6 = tmp(1720);
  }
}
...
arg5.tryActivateLayoutTransition = function tryActivateLayoutTransition(props, _componentDOMRef, arg2) {
  if (props.layout) {
    ...
    let obj = require(1721) /* findDescendantWithExitingAnimation */;
    if (!obj.areDOMRectsEqual(size, arg2)) {
      ...
      let str;
      if (easingXV != null) {
        str = easingXV[tmp3(undefined, 1686).EasingNameSymbol];
      }
      if (str == null) {
        str = "ease";
      }
      ...
      let str2;
      if (easingYV != null) {
        str2 = easingYV[tmp3(undefined, 1686).EasingNameSymbol];
      }
      if (str2 == null) {
        str2 = "ease";
      }
      ...
      obj[7] = presetName;
      obj[8] = presetName1;
      startWebLayoutAnimation(props, _componentDOMRef, tmp3(1659).LayoutAnimationType.LAYOUT, obj);
    }
  }
};
