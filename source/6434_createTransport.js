...
// Dependencies: [6435, 6415, 6436, 6395, 6399, 6371, 6428]
  ...
  obj = {
    send(arg0) {
      ...
      bufferSize(table[1]).forEachEnvelopeItem(arg0, (arg0, arg1) => {
        const result = items(6415).envelopeItemTypeToDataCategory(arg1);
        const obj = items(6415);
      ...
      if (0 === items.length) {
        ...
        return recordEnvelopeLoss.add(() => {
          const obj = { body: null };
          obj[0] = items(6415).serializeEnvelope(dependencyMap);
          const obj2 = items(6415);
          ...
          if (arg0 instanceof items(6428).SentryError) {
            if (tmp(6399).DEBUG_BUILD) {
              const logger = tmp(6371).logger;
              logger.error("Skipped sending event because buffer is full.");
            }
            if (typeof recordEnvelopeLoss !== "fileFinishedImporting") {
              HermesBuiltin.throwTypeError();
            }
            const queue_overflow = "queue_overflow";
            let tmpResult = tmp(6415);
