...
// Function ID: 4850
// Name: constructTag
// Dependencies: [4805, 4821, 4833, 4802, 4809, 4835]

// Module 4849 (constructTag)
const require = arg1;
const module = arg2;
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (decompressResult instanceof DataView) {
    let obj = require(4802) /* getDataView */;
    stringFromDataView = obj.getStringFromDataView(decompressResult, 0, decompressResult.byteLength);
  }
  const stringValueFromArray = require(4802) /* getDataView */.getStringValueFromArray(items1);
  let combined = stringValueFromArray;
  if (type !== require(4809).TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(4802).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(4802);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === require(4809).TYPE_ITXT) {
    stringFromDataView = module(4835).decode("UTF-8", decompressResult);
    const obj5 = module(4835);
  }
