...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      ...
      if (handlerTag.oldState === tmp(5358).State.BEGAN) {
        if (handlerTag.state === tmp(5358).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(5358).State.END) {
          if (handlerTag.oldState === tmp(5358).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(5358).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(5358).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(5359).GestureStateManager;
      ...
      if (tmp(5361).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(5361).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(5361).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(5361).TouchEventType.TOUCHES_CANCELLED === eventType) {
    ...
    const result2 = tmp(5356).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
