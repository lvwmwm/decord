...
// Dependencies: [13748, 13774, 13773, 13772, 13761]

// Module 13776 (__exportStarResult25)
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13748).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13774).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13774).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13773).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13772).FormatApproximately(internalSlots, result);
    ...
    return tmp(13761).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13773).FormatNumeric(internalSlots, isNaN);
};
