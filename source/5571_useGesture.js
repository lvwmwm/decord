...
// Function ID: 5572
// Name: useGesture
// Dependencies: [19, 5510, 5493, 5572, 5556, 5532, 5519, 5492]
// Exports: useGesture

// Module 5571 (useGesture)
...
export const useGesture = function useGesture(Fling, clonedAndRemappedConfig) {
  ...
  if (clonedAndRemappedConfig.disableReanimated !== jsEventHandler(() => clonedAndRemappedConfig.disableReanimated, [])) {
    const _Error2 = Error;
    const error = new Error(_require(5493).tagMessage("The \"disableReanimated\" property must not be changed after the handler is created."));
    throw error;
  } else {
    const gestureCallbacks = _require(5572).useGestureCallbacks(tmp2, clonedAndRemappedConfig);
    ...
    if (clonedAndRemappedConfig.shouldUseReanimatedDetector) {
      if (!reanimatedEventHandler) {
        const _Error = Error;
        const error1 = new Error(_require(5493).tagMessage("Failed to create reanimated event handlers."));
        throw error1;
      }
    }
    const items = [tmp2, , , ];
