...
// Function ID: 8212
// Name: extractProps
// Dependencies: [8212, 8203, 8213, 8216, 8204, 8202]

// Module 8211 (extractProps)
const require = arg1;
const module = arg2;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(8203)(obj, markerMid, arg1);
  module(8213)(obj, markerMid, items);
  module(8216)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(8204)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(8202)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
  ...
  if (markerStart) {
    let tmp7;
    if (markerStart) {
      const match = markerStart.match(require(8212) /* pickNotNil */.idPattern);
      let tmp10;
      if (match) {
        tmp10 = match[1];
      }
  ...
  if (str) {
    let tmp11;
    if (str) {
      const match1 = str.match(require(8212) /* pickNotNil */.idPattern);
      let tmp14;
      if (match1) {
        tmp14 = match1[1];
      }
  ...
  if (str2) {
    let tmp15;
    if (str2) {
      const match2 = str2.match(require(8212) /* pickNotNil */.idPattern);
      let tmp18;
      if (match2) {
        tmp18 = match2[1];
      }
  ...
  if (clipPath) {
    const match3 = clipPath.match(require(8212) /* pickNotNil */.idPattern);
    if (match3) {
  ...
  if (mask) {
    const match4 = mask.match(require(8212) /* pickNotNil */.idPattern);
    if (match4) {
      obj.mask = match4[1];
    } else {
  ...
  if (filter) {
    const match5 = filter.match(require(8212) /* pickNotNil */.idPattern);
    if (match5) {
      obj.filter = match5[1];
    } else {
