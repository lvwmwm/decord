...
// Dependencies: [3423, 3553]

// Module 3552 (parseNumericPattern)
const require = arg1;
...
arg5.parseAnyDigitsSigned = function parseAnyDigitsSigned(arg0) {
  return parseNumericPattern(require(3553).numericPatterns.anyDigitsSigned, arg0);
};
arg5.parseNDigits = function parseNDigits(length, arg1) {
  if (1 === length) {
    return parseNumericPattern(require(3553).numericPatterns.singleDigit, arg1);
  } else if (2 === length) {
    return parseNumericPattern(require(3553).numericPatterns.twoDigits, arg1);
  } else if (3 === length) {
    return parseNumericPattern(require(3553).numericPatterns.threeDigits, arg1);
  } else if (4 === length) {
    return parseNumericPattern(require(3553).numericPatterns.fourDigits, arg1);
  } else {
...
arg5.parseNDigitsSigned = function parseNDigitsSigned(length, arg1) {
  if (1 === length) {
    return parseNumericPattern(require(3553).numericPatterns.singleDigitSigned, arg1);
  } else if (2 === length) {
    return parseNumericPattern(require(3553).numericPatterns.twoDigitsSigned, arg1);
  } else if (3 === length) {
    return parseNumericPattern(require(3553).numericPatterns.threeDigitsSigned, arg1);
  } else if (4 === length) {
    return parseNumericPattern(require(3553).numericPatterns.fourDigitsSigned, arg1);
  } else {
    const _RegExp = RegExp;
    const regExp = new RegExp("^-?\\d{1," + length + "}");
    return parseNumericPattern(regExp, arg1);
...
arg5.parseTimezonePattern = function parseTimezonePattern(basic, str) {
  ...
  if (match) {
    if ("Z" === match[0]) {
      ...
      obj = {};
      const result = num4 * require(3423) /* keys */.millisecondsInHour;
      const sum = result + num7 * require(3423) /* keys */.millisecondsInMinute;
      obj.value = num2 * (sum + num10 * require(3423) /* keys */.millisecondsInSecond);
      obj.rest = str.slice(match[0].length);
      return obj;
    }
  } else {
