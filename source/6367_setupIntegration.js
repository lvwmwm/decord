...
function setupIntegration(on, name) {
  const _require = on;
  if (arg2[name.name]) {
    if (_require(6339).DEBUG_BUILD) {
      const logger2 = _require(6311).logger;
    ...
    if (_require(6339).DEBUG_BUILD) {
      const logger = _require(6311).logger;
