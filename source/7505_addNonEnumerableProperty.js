...
// Dependencies: [7498, 7499, 7506, 7507, 7508]

// Module 7505 (addNonEnumerableProperty)
const require = arg1;
...
function serializeEventTarget(arg0) {
  try {
    if (obj.isElement(arg0)) {
      let htmlTreeAsStringResult = require(7507) /* _htmlElementAsString */.htmlTreeAsString(arg0);
      const tmp2Result = require(7507) /* _htmlElementAsString */;
    } else {
      const _Object = Object;
...
arg5.extractExceptionKeysForMessage = function extractExceptionKeysForMessage(arg0) {
  ...
  if (first) {
    if (first.length >= num) {
      return require(7508) /* isMatchingPattern */.truncate(first, num);
    } else {
      ...
      if (length > 0) {
        ...
        if (length !== keys.length) {
          truncateResult = require(7508) /* isMatchingPattern */.truncate(joined, num);
