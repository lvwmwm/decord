...
// Dependencies: [32, 41, 42, 93, 95, 96, 98, 6364, 6379, 6361, 6425, 6389, 6392, 6393, 6365, 6418, 6376, 6426, 6387, 6370, 6401, 6420]

// Module 6424 (_isNativeReflectConstruct)
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(6425).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(6425);
      return ServerRuntimeClient(6389).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(6389);
      return obj.resolvedSyncPromise(ServerRuntimeClient(6425).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(6392).getIsolationScope();
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(6418).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(6393).DEBUG_BUILD) {
        const logger = tmp2(6365).logger;
    ...
    value: function captureCheckIn(checkInId) {
      ...
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(6426);
          ...
          if (ServerRuntimeClient(6393).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(6365).logger;
          ...
          if (ServerRuntimeClient(6393).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(6365).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(6376);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(6387)._getSpanForScope(arg0);
        if (_getSpanForScopeResult) {
          let tmpResult = tmp(6370);
        ...
        const tmpResult1 = ServerRuntimeClient(6401);
        if (_getSpanForScopeResult) {
