...
// Function ID: 39724
// Name: Animated
// Dependencies: [29, 31, 27, 33, 4518, 4519, 4520, 4516, 4521, 4522, 4523, 4525, 4526, 4527]

// Module 4517 (Animated)
...
const forwardRefResult = importDefaultResult.forwardRef(function InnerScreen(activityState) {
  ...
  setRef = function setRef(current) {
    ...
    if (null != activityState.onComponentRef) {
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = _require(setRef[8]).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (_require(setRef[7]).isNativePlatformSupported) {
      let obj5 = _require(setRef[8]);
      const sheetAllowedDetents1 = obj5.resolveSheetAllowedDetents(sheetAllowedDetents);
      let obj6 = _require(setRef[8]);
      ...
      importDefault(setRef[9])(false, "[RNScreens] Using featureFlags `ios26AllowInteractionsDuringTransition` with `iosPreventReattachmentOfDismissedScreens` disabled is discouraged and will result in visual bugs on screen transitions. See flags description for details.");
      if (isNativeStack) {
      ...
      const tmp31 = __INTERNAL_VIEW_CONFIG;
      ...
      const tmp44 = importDefault(setRef[10]);
      obj.fullScreenSwipeEnabled = _require(setRef[11]).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      obj2 = {};
      let start;
      ...
      obj.synchronousShadowStateUpdatesEnabled = importDefault(setRef[12]).experiment.synchronousScreenUpdatesEnabled;
      obj.androidResetScreenShadowStateOnOrientationChangeEnabled = importDefault(setRef[12]).experiment.androidResetScreenShadowStateOnOrientationChangeEnabled;
      obj.ios26AllowInteractionsDuringTransition = importDefault(setRef[12]).experiment.ios26AllowInteractionsDuringTransition;
      let tmp63 = children;
      if (isNativeStack) {
        obj6 = {};
        ...
        tmp63 = jsx(importDefault(setRef[13]).Provider, {});
      }
      obj.children = tmp63;
      obj.children = <tmp31 {...Object.assign({}, tmp34, obj)} />;
      return <tmp44 onAppear={tmp18} onDisappear={tmp19} onWillAppear={tmp20} onWillDisappear={tmp21} />;
    }
  }
...
const forwardRefResult1 = importDefaultResult.forwardRef((arg0, ref) => jsx(importDefaultResult.useContext(context) || closure_11, Object.assign({}, arg0, { ref })));
