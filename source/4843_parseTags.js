...
// Function ID: 4844
// Name: parseTags
// Dependencies: [4802, 4844]

// Module 4843 (parseTags)
const require = arg1;
...
function parseTags(buffer) {
  ...
  if (buffer.byteLength !== buffer.getUint32()) {
    ...
    const keys = Object.keys(require(4844).iccProfile);
    for (let num = 0; num < keys.length; num = num + 1) {
      let tmp = keys[num];
      let tmp2 = require;
      let tmp3 = dependencyMap;
      let iter = require(4844).iccProfile[tmp];
      ...
      if (buffer.length < 132) {
        ...
        if (0 < uint32) {
          while (buffer.length >= num6 + c8) {
            ...
            let obj17 = require(4802) /* getDataView */;
            ...
            if (uint321 > buffer.length) {
              ...
              if (stringFromDataView1 === desc) {
                ...
                if (uint323 > uint322) {
                  ...
                  if (tmp55(4844).iccTags[stringFromDataView]) {
                    obj = { value: null, description: null };
                    obj[0] = applyResult;
                    obj[1] = applyResult;
                    obj[tmp55(4844).iccTags[stringFromDataView].name] = obj;
                  } else {
                ...
                if (stringFromDataView1 === mluc) {
                  ...
                  if (0 < uint324) {
                    do {
                      ...
                      let obj5 = require(4802) /* getDataView */;
                      let stringFromDataView2 = obj5.getStringFromDataView(buffer, sum, 2);
                      let obj6 = require(4802) /* getDataView */;
                      ...
                      let obj7 = require(4802) /* getDataView */;
                  ...
                  if (1 === uint324) {
                    ...
                    if (tmp25(4844).iccTags[stringFromDataView]) {
                      let obj3 = { value: null, description: null };
                      obj3[0] = text;
                      obj3[1] = text;
                      obj[tmp25(4844).iccTags[stringFromDataView].name] = obj3;
                    } else {
                    ...
                    if (require(4844).iccTags[stringFromDataView]) {
                      obj6 = { value: null, description: null };
                      obj6[0] = obj5;
                      obj6[1] = obj5;
                      obj[tmp33(4844).iccTags[stringFromDataView].name] = obj6;
                    } else {
                  ...
                  if (stringFromDataView1 === text) {
                    ...
                    if (tmp55(4844).iccTags[stringFromDataView]) {
                      ...
                      obj8[0] = applyResult1;
                      obj8[1] = applyResult1;
                      obj[tmp55(4844).iccTags[stringFromDataView].name] = obj8;
                    } else {
                    ...
                    if (stringFromDataView1 === c7) {
                      ...
                      if (tmp55(4844).iccTags[stringFromDataView]) {
                        ...
                        obj[tmp55(4844).iccTags[stringFromDataView].name] = obj10;
                      } else {
...
arg5.default = {
  read(buffer, arr) {
    if (arg2) {
      if (arr[0].compressionMethod !== require(4802) /* getDataView */.COMPRESSION_METHOD_NONE) {
        if (arr[0].compressionMethod === tmp(4802).COMPRESSION_METHOD_DEFLATE) {
          let _DataView = DataView;
          buffer = buffer.buffer;
          let dataView = new DataView(buffer.slice(arr[0].offset, arr[0].offset + arr[0].length));
          const tmpResult = tmp(4802);
