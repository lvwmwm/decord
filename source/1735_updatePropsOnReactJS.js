...
// Dependencies: [1637, 1644, 1677, 1736, 1737, 1641, 1645]
// Exports: updatePropsJestWrapper

// Module 1735 (updatePropsOnReactJS)
import isJest from "isJest";
import isJest from "isJest";
import isJest from "isJest";

function updatePropsOnReactJS(arg0, arg1) {
  const ComponentRegistry = require(1737) /* map */.ComponentRegistry;
...
if (isJest.shouldBeUseWeb()) {
  ...
  fn = function s(value, transformOrigin) {
    ...
    require(1677) /* call */.processColorsInProps(transformOrigin);
    if ("transformOrigin" in transformOrigin) {
      transformOrigin.transformOrigin = require(1736) /* validateTransformOrigin */.processTransformOrigin(transformOrigin.transformOrigin);
      const tmp2Result = require(1736) /* validateTransformOrigin */;
    }
      ...
      if (__frameTimestamp) {
        if (tmp2) {
          if (__frameTimestamp - tmp2 >= 20) {
            require(1641) /* runWorkletOnJS */.runOnJS(updatePropsOnReactJS)(arg0, tmp.lastUpdateByTag[arg0]);
