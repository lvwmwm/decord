...
// Dependencies: [77, 57, 4780, 4797]

// Module 4820 (parseBezierKnot)
...
function getFixedPointNumber(dataView, sum2, arg2) {
  const longAt = importDefault(4797).getLongAt(dataView, sum2);
  let num = -1;
  if (longAt >>> 31 === 0) {
    num = 1;
  }
  const obj = importDefault(4797);
  const obj2 = require(4780) /* getStringFromDataView */;
  const str2 = longAt & parseInt(require(4780) /* getStringFromDataView */.strRepeat("1", 32 - arg2), 2);
  const text = `${str.toString(2)}.`;
  const obj3 = require(4780) /* getStringFromDataView */;
  return num * obj3.parseFloatRadix(`${(2130706432 & longAt) >>> 32 - arg2.toString(2)}.` + require(4780) /* getStringFromDataView */.padStart(str2.toString(2), 32 - arg2, "0"), 2);
}
...
obj = {
  ...
  description(getUint8) {
    return callback(require(4780) /* getStringFromDataView */.getPascalStringFromDataView(getUint8, 0), 2)[1];
  }
};
let closure_4 = importDefaultResult(importDefaultResult(importDefaultResult(importDefaultResult(importDefaultResult(importDefaultResult(importDefaultResult(importDefaultResult(importDefaultResult({}, obj.CLOSED_SUBPATH_LENGTH, {
  ...
  path(dataView, sum) {
    const items = [importDefault(4797).getShortAt(dataView, sum)];
    return items;
  }
