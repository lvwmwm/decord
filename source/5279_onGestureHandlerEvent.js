...
// Dependencies: [17, 5280, 5282, 5283, 5285]
...
function onGestureHandlerEvent(handlerTag) {
  let obj = require(5280) /* map */;
  const findHandlerResult = obj.findHandler(handlerTag.handlerTag);
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(5282).State.UNDETERMINED) {
        if (handlerTag.state === tmp(5282).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(5282).State.BEGAN) {
        if (handlerTag.state === tmp(5282).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(5282).State.END) {
          if (handlerTag.oldState === tmp(5282).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(5282).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(5282).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(5283).GestureStateManager;
      ...
      if (tmp(5285).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(5285).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(5285).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(5285).TouchEventType.TOUCHES_CANCELLED === eventType) {
    ...
    const result2 = tmp(5280).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
