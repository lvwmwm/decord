...
// Module 12983 (getInputRangeFromIndexes)
...
export const defaultScrollInterpolator = function defaultScrollInterpolator(_getCustomIndexResult, props) {
  const items = [true, true, -1];
  return { inputRange: getInputRangeFromIndexes(items, _getCustomIndexResult, props), outputRange: [255, 235, 205] };
};
...
export const stackAnimatedStyles = function stackAnimatedStyles(index, interpolate, props, layoutCardOffset) {
  ...
  let obj = 1 === props.inactiveSlideOpacity ? [] : ["marginBottom", "transform", "formatToPlainString", "construct"];
  obj = {};
  obj = { inputRange: items, outputRange: obj.reverse(), extrapolate: "clamp" };
  const items = [-3, -2, -1, 0];
  obj.opacity = interpolate.interpolate(obj);
  const items1 = [-2, -1, 0, 1];
  const items2 = [{ scale: interpolate.interpolate({ inputRange: items1, outputRange: ["opacity", "variant", "formatToPlainString", "LengthDelimited"], extrapolate: "clamp" }) }, ];
...
export const tinderAnimatedStyles = function tinderAnimatedStyles(index, interpolate, props, layoutCardOffset) {
  ...
  let obj = { opacity: interpolate.interpolate({ inputRange: items, outputRange: ["GiftTrollIllocon", "EmbeddedActivitiesStore", "onFastRefresh", "NOTIFICATION_CENTER_ITEM_CREATE", "DEVICE_VOIP_TOKEN"], extrapolate: "clamp" }) };
  ...
  const items3 = [obj, , , ];
  obj = { rotate: interpolate.interpolate({ inputRange: [77601039, 1612144654], outputRange: [], extrapolate: "clamp" }) };
  items3[1] = obj;
