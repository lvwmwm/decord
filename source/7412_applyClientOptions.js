...
// Dependencies: [7374, 7377, 7390, 7413, 7414, 7400, 7372, 7415, 7408, 7384]

// Module 7412 (applyClientOptions)
const require = arg1;
...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = require(7372) /* isMatchingPattern */.truncate(environment.message, num);
    const obj = require(7372) /* isMatchingPattern */;
  }
  let value = iter;
  ...
  if (value) {
    iter.value = require(7372) /* isMatchingPattern */.truncate(iter.value, num);
  ...
  if (url) {
    request.url = require(7372) /* isMatchingPattern */.truncate(request.url, num);
    const obj3 = require(7372) /* isMatchingPattern */;
  }
}
function applyDebugIds(exception) {
  _require = _require(7415).getFilenameToDebugIdMap(arg1);
  try {
          ...
          if (breadcrumbs) {
            ...
            if (breadcrumbs) {
              ...
              obj[0] = breadcrumbs.map((data) => {
                ...
                if (data) {
                  obj = { data: null };
                  obj[0] = dependencyMap(7408).normalize(data.data, dependencyMap, dependencyMap);
                  data = obj;
                  const obj3 = dependencyMap(7408);
                }
