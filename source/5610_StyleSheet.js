...
// Function ID: 47502
// Name: StyleSheet
// Dependencies: [57, 31, 27, 33, 5561, 5611, 1457, 5591, 5587]

// Module 5610 (StyleSheet)
...
export default importAllResult.memo(function CardContainer(active) {
  ...
  const context = importAllResult.useContext(active(5561).HeaderHeightContext);
  const items = [scene.descriptor];
  const tmp2 = importDefault(5611);
  ({ onPageChangeStart: closure_9, onPageChangeCancel: closure_10, onPageChangeConfirm: closure_11 } = importDefault(5611)(importAllResult.useCallback(() => {
    let navigation;
    let options;
  ...
  const tmp2Result = importDefault(5611)(importAllResult.useCallback(() => {
    let navigation;
    let options;
    ({ navigation, options } = scene.descriptor);
    let isFocusedResult = navigation.isFocused();
  ...
  if (false === animationEnabled) {
    if (false === isNextScreenTransparent) {
      ...
      obj8.children = closure_7(active(5561).HeaderHeightContext.Provider, obj9);
      obj7.children = closure_7(active(5561).HeaderShownContext.Provider, obj8);
      obj6.children = closure_7(active(5561).HeaderBackContext.Provider, obj7);
      ...
      obj4.children = scene(importDefault(5587).Provider, obj5);
