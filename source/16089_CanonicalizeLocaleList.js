...
// Dependencies: [16090, 16091]

// Module 16089 (CanonicalizeLocaleList)
let require = arg1;
          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "y") {
                ...
                let expBCP47Syntax = require(16090) /* RegExpResult1 */.expBCP47Syntax;
                let isMatch = expBCP47Syntax.test(str);
                if (isMatch) {
                  let expVariantDupes = tmp5(16090).expVariantDupes;
                  isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(16090).expSingletonDupes;
                ...
                if (isMatch) {
                  ...
                  let match = str8.match(require(16090) /* RegExpResult1 */.expExtSequences);
                  let tmp19 = str8;
                  let tmp20 = match;
                  if (match) {
                  ...
                  if (!tmp20) {
                    ...
                    let RegExpResult = RegExp(`(?:${tmp17(16090).expExtSequences.source})+`, "i");
...
function InitializeDateTimeFormat(prototype) {
  ...
  if (true === result["[[initializedIntlObject]]"]) {
    ...
    if ("[object Array]" !== (typeof call6 === "unknown" ? toString() : call6(formats))) {
      obj2 = require(16091) /* createDateTimeFormat */;
...
class List {
  constructor() {
    tmp = f63005(this, "length", { writable: true, value: 0 });
    if (!arguments.length) {
      return;
    } else {
      tmp3 = slice;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    const expBCP47Syntax = require(16090) /* RegExpResult1 */.expBCP47Syntax;
    let isMatch = expBCP47Syntax.test(locale);
    if (isMatch) {
      const expVariantDupes = tmp(16090).expVariantDupes;
      isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(16090).expSingletonDupes;
