...
// Dependencies: [7374, 7375, 7419, 7422, 7411, 7380]
// Exports: createEventEnvelope, createSessionEnvelope, createSpanEnvelope

// Module 7418 (createEventEnvelope)
...
export const createEventEnvelope = function createEventEnvelope(type, arg1, sdk) {
  const sdkMetadataForEnvelopeHeader = require(7419) /* forEachEnvelopeItem */.getSdkMetadataForEnvelopeHeader(sdk);
  let str = "event";
  if (type.type) {
    str = "event";
...
export const createSessionEnvelope = function createSessionEnvelope(toJSON) {
  let obj = require(7419) /* forEachEnvelopeItem */;
...
export const createSpanEnvelope = function createSpanEnvelope(arg0, getDsn) {
  ...
  if (tmp9) {
    const obj1 = { dsn: null };
    obj1[0] = beforeSendSpan(7422).dsnToString(dsn);
    tmp9 = obj1;
    const tmp2Result = beforeSendSpan(7422);
  }
