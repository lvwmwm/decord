...
({ useEffect: closure_2, useMemo: closure_3, useRef: closure_4 } = _module);
...
let closure_8 = () => {
  function adjustRotationToInterfaceOrientation(pitch) {
    ...
    if (interfaceOrientation === callback(closure_1[1]).InterfaceOrientation.ROTATION_90) {
      pitch.pitch = roll;
      pitch.roll = -pitch;
      const _Math2 = Math;
      pitch.yaw = yaw - Math.PI / 2;
    } else if (interfaceOrientation === callback(closure_1[1]).InterfaceOrientation.ROTATION_270) {
      pitch.pitch = -roll;
      pitch.roll = pitch;
      const _Math = Math;
      pitch.yaw = yaw + Math.PI / 2;
    } else if (interfaceOrientation === callback(closure_1[1]).InterfaceOrientation.ROTATION_180) {
      pitch.pitch = pitch.pitch * -1;
      pitch.roll = pitch.roll * -1;
      pitch.yaw = pitch.yaw * -1;
    }
    const tmp5 = callback2(pitch.pitch, pitch.roll, pitch.yaw);
    [pitch.qx, pitch.qy] = tmp5;
    pitch.qz = tmp5[2];
    pitch.qw = tmp5[3];
    return pitch;
  }
  adjustRotationToInterfaceOrientation.__closure = { InterfaceOrientation: require(dependencyMap[1]).InterfaceOrientation, eulerToQuaternion: closure_6 };
...
let closure_10 = () => {
  ...
  adjustVectorToInterfaceOrientation.__closure = { InterfaceOrientation: require(dependencyMap[1]).InterfaceOrientation };
...
export const useAnimatedSensor = function useAnimatedSensor(arg0, adjustToInterfaceOrientation) {
  const require = arg0;
  const tmp = callback(adjustToInterfaceOrientation);
  ...
  const tmp10 = tmp11(() => {
    const obj = { 0: 1895956479, 9223372036854775807: 1, iosReferenceFrame: arg0(tmp[1]).IOSReferenceFrame.Auto };
    return Object.assign(obj, tmp.current);
  }, items);
  const obj = {
    sensor: require(dependencyMap[2]).initializeSensor(arg0, tmp10),
    unregister() {
  ...
  const tmp11 = callback(obj);
  const items1 = [arg0, tmp10];
  tmp10(() => {
    let obj = {
      sensor: arg0(tmp[2]).initializeSensor(arg0, tmp10),
      unregister() {
    ...
    tmp11.current = obj;
    const sensor = tmp11.current.sensor;
    const arg0 = sensor;
    ...
    obj = { adjustToInterfaceOrientation, sensorType: arg0, SensorType: arg0(tmp[1]).SensorType, adjustRotationToInterfaceOrientation: closure_8, adjustVectorToInterfaceOrientation: closure_10, sensorData: sensor, callMicrotasks: arg0(tmp[3]).callMicrotasks };
    pnpm_useAnimatedSensorTs4.__closure = obj;
    pnpm_useAnimatedSensorTs4.__workletHash = 6807952122364;
    pnpm_useAnimatedSensorTs4.__initData = closure_11;
    const registerSensorResult = arg0(adjustToInterfaceOrientation[2]).registerSensor(arg0, tmp10, pnpm_useAnimatedSensorTs4);
    if (-1 !== registerSensorResult) {
      tmp11.current.unregister = () => sensor(adjustToInterfaceOrientation[2]).unregisterSensor(registerSensorResult);
