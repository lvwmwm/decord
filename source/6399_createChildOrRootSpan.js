...
// Dependencies: [32, 6392, 6398, 6387, 6400, 6370, 6382, 6383, 6384, 6377, 6375, 6393, 6365, 6397, 6401, 6403, 6394, 6404, 6406, 6380]
// Exports: continueTrace, startInactiveSpan, startNewTrace, startSpan, startSpanManual, suppressTracing, withActiveSpan

// Module 6399 (createChildOrRootSpan)
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(6406).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(6398).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(6370).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(6370);
        const client = tmp(6392).getClient();
        if (client) {
        ...
        const tmpResult2 = tmp(6392);
        tmp(6370).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult3 = tmp(6370);
      }
      tmp(6403).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(6403);
      const result = tmp(6394).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(6401).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(6401);
      ...
      obj1[0] = traceId2;
      obj1[1] = spanId2;
      const tmpResult7 = tmp(6370);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj1, scope, tmp(6370).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(6370).spanIsSampled(parentSpan);
      tmp(6401).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = tmp(6401);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(6401).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = tmp(6401);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(6398).SentryNonRecordingSpan();
...
function _startRootSpan(name) {
  let tmp6;
  let tmp7;
  let obj = require(6392) /* getClient */;
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(6404).sampleSpan(options, obj);
    const tmpResult = tmp(6404);
  }
  ...
  obj1.attributes = { [require(6380).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(6406).SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(6380).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  ...
  let dependencyMap;
  const _require = arg1;
  const mainCarrier = _require(6383).getMainCarrier();
  const obj = _require(6383);
  const tmp = _require;
  const asyncContextStrategy = _require(6384).getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(6392).withScope((setPropagationContext) => {
...
export const startInactiveSpan = function startInactiveSpan(experimental) {
  ...
  if (asyncContextStrategy.startInactiveSpan) {
    ...
    if (experimental.startTime) {
      ...
      obj.startTimestamp = tmp3(6370).spanTimeInputToSeconds(experimental.startTime);
      delete tmp[tmp2];
      tmp10 = obj;
      const tmp3Result = tmp3(6370);
    }
      ...
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = tmp2(tmp3[1]).withScope((arg0) => {
            _null(6387)._setSpanForScope(arg0, _null);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return _require(6392).withScope((setPropagationContext) => {
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(6392).withScope((setSDKProcessingMetadata) => {
      const result = setSDKProcessingMetadata.setSDKProcessingMetadata({ [outer1_3]: true });
      return callback();
    });
    const tmpResult = tmp(6392);
  }
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(6392).withScope((arg0) => {
      _null(6387)._setSpanForScope(arg0, _null);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(6392);
