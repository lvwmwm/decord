...
function setupIntegration(on, name) {
  const _require = on;
  if (arg2[name.name]) {
    if (_require(6375).DEBUG_BUILD) {
      const logger2 = _require(6347).logger;
    ...
    if (_require(6375).DEBUG_BUILD) {
      const logger = _require(6347).logger;
...
export const addIntegration = function addIntegration(name) {
  ...
  } else if (require(6375).DEBUG_BUILD) {
