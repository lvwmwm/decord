...
function getCompressionMethod(arg0) {
  ...
  if (type === require(4753) /* isPngImageHeaderChunk */.TYPE_ITXT) {
    if (dataView.getUint8(offset) === c10) {
      return dataView.getUint8(offset + 1);
    }
  } else if (type === require(4753) /* isPngImageHeaderChunk */.TYPE_ZTXT) {
    return dataView.getUint8(offset);
  }
  return require(4746) /* getStringFromDataView */.COMPRESSION_METHOD_NONE;
}
function moveToNextState(type, arg1) {
  if (arg1 === STATE_KEYWORD) {
    const items = [require(4753) /* isPngImageHeaderChunk */.TYPE_ITXT, require(4753) /* isPngImageHeaderChunk */.TYPE_ZTXT];
  ...
  if (arg1 === STATE_COMPRESSION) {
    type === require(4753) /* isPngImageHeaderChunk */.TYPE_ITXT ? STATE_LANG : STATE_TEXT;
  } else {
...
function getName(arg0, items, items) {
  const stringValueFromArray = require(4746) /* getStringFromDataView */.getStringValueFromArray(items);
  let combined = stringValueFromArray;
  if (arg0 !== require(4753) /* isPngImageHeaderChunk */.TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + require(4746) /* getStringFromDataView */.getStringValueFromArray(items) + ")";
...
function getDescription(arg0) {
  return importDefault(4779).decode("UTF-8", arg0);
}
