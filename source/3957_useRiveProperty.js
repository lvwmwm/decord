...
// Dependencies: [32, 19, 3958]
// Exports: useRiveProperty

// Module 3957 (useRiveProperty)
...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f28569) {
  const _require = instance;
  const dependencyMap = FillColor;
  const callback = f28569;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = _require(3958).useDisposableMemo(() => {
    if (closure_0) {
