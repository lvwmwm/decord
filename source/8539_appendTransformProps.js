...
// Dependencies: [8540, 8541, 8542]

// Module 8539 (appendTransformProps)
const require = arg1;
...
function appendTransformProps(arg0) {
  ...
  require(8540) /* append */.appendTransform(x + originX, y + originY, scaleX, scaleY, rotation, skewX, skewY, originX, originY);
}
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        tmp3Result = tmp3(8541);
        const parsed = tmp3Result.parse(arr);
        const tmp3Result1 = tmp3(8540);
  ...
  const obj = require(8540) /* append */;
  return require(8540) /* append */.toArray();
}
arg5.default = function extractTransform(transform) {
  ...
  if (typeof transform === "y") {
    try {
      const parsed = require(8541) /* peg$SyntaxError */.parse(transform);
      const items = [, , , , , ];
      ...
      const _console = tmp.console;
      _console.error(tmp6);
      return require(8540) /* append */.identity;
    }
  } else {
    transform = undefined;
    if (transform != null) {
...
arg5.extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "y") {
    transform = require(8542) /* peg$SyntaxError */.parse(transform.transform);
    const obj = require(8542) /* peg$SyntaxError */;
  } else {
    transform = transform.transform;
  }
  return transform;
