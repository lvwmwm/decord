...
// Dependencies: [7455, 7463, 7460]

// Module 7465 (addContextToFrame)
const require = arg1;
...
arg5.addContextToFrame = function addContextToFrame(arr, lineno) {
  ...
  if (undefined !== lineno.lineno) {
    ...
    lineno.context_line = require(7463) /* isMatchingPattern */.snipLine(arr[bound1], num2);
    const _Math = Math;
...
arg5.checkOrSetAlreadyCaught = function checkOrSetAlreadyCaught(__sentry_captured__) {
  if ((function isAlreadyCaptured(__sentry_captured__) {
    ...
    try {
      const result = require(7460) /* addNonEnumerableProperty */.addNonEnumerableProperty(__sentry_captured__, "__sentry_captured__", true);
      return false;
    } catch (err) {
    }
  }
