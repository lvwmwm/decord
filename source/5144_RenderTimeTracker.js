...
// Dependencies: [5159, 5145, 5146, 5150]

// Module 5144 (RenderTimeTracker)
...
export const RenderTimeTracker = () => {
  class RenderTimeTracker {
    constructor() {
      tmp = _toConsumableArray(this, RenderTimeTracker);
      averageWindow = new RenderTimeTracker(closure_1[2]).AverageWindow(5);
  ...
  const arg1 = RenderTimeTracker;
  let obj = {
    key: "startTracking",
    value: function startTracking() {
      const self = this;
      this.rendersWithoutCommit = this.rendersWithoutCommit + 1;
      let trackAverageRenderTimeForOffsetProjection = RenderTimeTracker(closure_1[3]).PlatformConfig.trackAverageRenderTimeForOffsetProjection;
  ...
  items[4] = {
    ...
    value: function getAverageRenderTime() {
      const self = this;
      if (RenderTimeTracker(closure_1[3]).PlatformConfig.trackAverageRenderTimeForOffsetProjection) {
        const _Math = Math;
        const _Math2 = Math;
        const _Math3 = Math;
