...
// Dependencies: [32, 1903, 676, 9163, 5322, 4623, 5217, 1236, 9164, 9165, 9166, 530, 4123, 4130, 709, 9167, 7707, 2]

// Module 9162 (handleRelationshipAddError)
...
function handleRelationshipAddError(arg0, arg1, substr) {
  ...
  if (429 === status) {
    if (arg1 === obj.SHOW_ALWAYS) {
      ...
      require(5322) /* openContextMenu */.closeContextMenu();
      const obj11 = require(5322) /* openContextMenu */;
      importDefault(4623).show(obj);
      const obj12 = importDefault(4623);
    }
  } else {
    if (403 === status) {
      if (num === constants.EMAIL_VERIFICATION_REQUIRED) {
        ...
        require(5322) /* openContextMenu */.closeContextMenu();
        const obj7 = require(5322) /* openContextMenu */;
        importDefault(4623).show(obj);
        const obj8 = importDefault(4623);
      }
    }
    if (num === constants.USER_QUARANTINED) {
      require(5322) /* openContextMenu */.closeContextMenu();
      importDefault(5217)();
      const obj5 = require(5322) /* openContextMenu */;
    } else {
      if (!obj9.isLimitedAccessErrorCode(status, num)) {
        if (num !== tmp2.RELATIONSHIP_INVALID_NO_CONFIRMATION) {
          if (arg1 === obj.SHOW_ALWAYS) {
            if (null != substr) {
              let tmp18Result = tmp18(9166);
              if (!num) {
                num = 0;
              }
      ...
      obj9 = require(9165) /* isLimitedAccessErrorCode */;
    }
  }
  throw arg0;
}
...
obj = {
  ...
  ignoreUser(closure_3, IGNORE_CONFIRMATION_ACTION_SHEET, channelId) {
    ...
    return HTTP.put(obj).then(() => {
      ...
      channelId(7707).showFailedToast();
      const AccessibilityAnnouncer = callback(4130).AccessibilityAnnouncer;
      const intl = callback(1236).intl;
      AccessibilityAnnouncer.announce(intl.string(callback(1236).t.n6Jo3E));
    });
  ...
  unignoreUser(id, UserProfileRemediatedNotice, id2) {
    ...
    return HTTP.del(obj).then(() => {
      ...
      id2(7707).showFailedToast();
      const AccessibilityAnnouncer = id(4130).AccessibilityAnnouncer;
