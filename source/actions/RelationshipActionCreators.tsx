...
// Dependencies: [32, 1874, 676, 9134, 5268, 4593, 5187, 1236, 9135, 9136, 9137, 530, 4094, 4101, 709, 9138, 7679, 2]

// Module 9133 (handleRelationshipAddError)
...
function handleRelationshipAddError(arg0, arg1, substr) {
  ...
  if (429 === status) {
    if (arg1 === obj.SHOW_ALWAYS) {
      ...
      require(5268) /* openContextMenu */.closeContextMenu();
      const obj11 = require(5268) /* openContextMenu */;
      importDefault(4593).show(obj);
      const obj12 = importDefault(4593);
    }
  } else {
    if (403 === status) {
      if (num === constants.EMAIL_VERIFICATION_REQUIRED) {
        ...
        require(5268) /* openContextMenu */.closeContextMenu();
        const obj7 = require(5268) /* openContextMenu */;
        importDefault(4593).show(obj);
        const obj8 = importDefault(4593);
      }
    }
    if (num === constants.USER_QUARANTINED) {
      require(5268) /* openContextMenu */.closeContextMenu();
      importDefault(5187)();
      const obj5 = require(5268) /* openContextMenu */;
    } else {
      if (!obj9.isLimitedAccessErrorCode(status, num)) {
        if (num !== tmp2.RELATIONSHIP_INVALID_NO_CONFIRMATION) {
          if (arg1 === obj.SHOW_ALWAYS) {
            if (null != substr) {
              let tmp18Result = tmp18(9137);
              if (!num) {
                num = 0;
              }
      ...
      obj9 = require(9136) /* isLimitedAccessErrorCode */;
    }
  }
  throw arg0;
}
...
obj = {
  ...
  ignoreUser(closure_3, IGNORE_CONFIRMATION_ACTION_SHEET, channelId) {
    ...
    return HTTP.put(obj).then(() => {
      ...
      channelId(7679).showFailedToast();
      const AccessibilityAnnouncer = callback(4101).AccessibilityAnnouncer;
      const intl = callback(1236).intl;
      AccessibilityAnnouncer.announce(intl.string(callback(1236).t.n6Jo3E));
    });
  ...
  unignoreUser(id, UserProfileRemediatedNotice, id2) {
    ...
    return HTTP.del(obj).then(() => {
      ...
      id2(7679).showFailedToast();
      const AccessibilityAnnouncer = id(4101).AccessibilityAnnouncer;
