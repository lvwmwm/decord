...
// Dependencies: [32, 1874, 676, 9105, 5254, 4594, 5173, 1236, 9106, 9107, 9108, 530, 4094, 4101, 709, 9109, 7695, 2]

// Module 9104 (handleRelationshipAddError)
...
function handleRelationshipAddError(arg0, arg1, substr) {
  ...
  if (429 === status) {
    if (arg1 === obj.SHOW_ALWAYS) {
      ...
      require(5254) /* openContextMenu */.closeContextMenu();
      const obj11 = require(5254) /* openContextMenu */;
      importDefault(4594).show(obj);
      const obj12 = importDefault(4594);
    }
  } else {
    if (403 === status) {
      if (num === constants.EMAIL_VERIFICATION_REQUIRED) {
        ...
        require(5254) /* openContextMenu */.closeContextMenu();
        const obj7 = require(5254) /* openContextMenu */;
        importDefault(4594).show(obj);
        const obj8 = importDefault(4594);
      }
    }
    if (num === constants.USER_QUARANTINED) {
      require(5254) /* openContextMenu */.closeContextMenu();
      importDefault(5173)();
      const obj5 = require(5254) /* openContextMenu */;
    } else {
      if (!obj9.isLimitedAccessErrorCode(status, num)) {
        if (num !== tmp2.RELATIONSHIP_INVALID_NO_CONFIRMATION) {
          if (arg1 === obj.SHOW_ALWAYS) {
            if (null != substr) {
              let tmp18Result = tmp18(9108);
              if (!num) {
                num = 0;
              }
      ...
      obj9 = require(9107) /* isLimitedAccessErrorCode */;
    }
  }
  throw arg0;
}
...
obj = {
  ...
  ignoreUser(closure_3, IGNORE_CONFIRMATION_ACTION_SHEET, channelId) {
    ...
    return HTTP.put(obj).then(() => {
      ...
      channelId(7695).showFailedToast();
      const AccessibilityAnnouncer = callback(4101).AccessibilityAnnouncer;
      const intl = callback(1236).intl;
      AccessibilityAnnouncer.announce(intl.string(callback(1236).t.n6Jo3E));
    });
