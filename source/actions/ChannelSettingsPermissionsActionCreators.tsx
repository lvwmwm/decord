    ...
    function chain() {
      ...
      if (chain.length > 0) {
        ...
        if (null == arr) {
          ...
          const result = chain(4175).updatePermissionOverwrite(callback, arr);
          result.then(chain, chain);
          const obj2 = chain(4175);
        }
      } else {
        arr = outer1_2.pop();
        if (null == arr) {
          ...
          const result1 = chain(4175).clearPermissionOverwrite(callback, arr);
          result1.then(chain, chain);
          const obj = chain(4175);
        }
      }
    }
    !chain();
