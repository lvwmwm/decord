...
// Dependencies: [1995, 10294, 673, 682, 4708, 10296, 10297, 527, 706, 12, 511, 1485, 9871, 1369, 1340, 4865, 1233, 695, 2]
...
function doSearchRequest(q, arg1, limit) {
  ...
  value.then((body) => {
    ...
    callback(4708).trackWithMetadata(closure_1_6.SEARCH_RESULT_VIEWED, obj2);
    const obj6 = callback(4708);
    const tmp2 = q;
...
export const gifUrlKey = function gifUrlKey(uri) {
  ...
  if (null != toURLSafeResult) {
    if (obj2.isAttachmentPathUrl(toURLSafeResult)) {
      str = tmp5(9871).removeSignedUrlParameters(toURLSafeResult);
      str = str.toString();
      const tmp5Result = tmp5(9871);
    }
    tmp4 = str;
