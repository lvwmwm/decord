...
export default {
  ...
  updateGuildNotificationSettings(guildId, muteSettings, NotificationLabel, location) {
    let obj = require(5257) /* UserNotificationSettings */;
    const currentGuildSettings = obj.getCurrentGuildSettings(guildId);
    const result = importDefault(5259).saveUserGuildSettings(guildId, muteSettings);
    const obj2 = importDefault(5259);
    obj = { type: "USER_GUILD_SETTINGS_GUILD_UPDATE", guildId, settings: muteSettings };
    importDefault(709).dispatch(obj);
    const obj3 = importDefault(709);
    const result1 = require(5257) /* UserNotificationSettings */.trackGuildNotificationSettingsUpdate(guildId, muteSettings, currentGuildSettings, NotificationLabel, location);
  },
  updateGuildAndChannelNotificationSettings(guildId, channel_overrides, OptedIn, location) {
    ...
    const currentGuildSettings = _require(5257).getCurrentGuildSettings(guildId);
    const obj2 = _require(5257);
    const manyCurrentChannelSettings = _require(5257).getManyCurrentChannelSettings(guildId, keys);
    const obj3 = _require(5257);
    let result = importDefault(5259).saveUserGuildSettings(guildId, channel_overrides);
    const obj4 = importDefault(5259);
    obj = { type: "USER_GUILD_SETTINGS_GUILD_AND_CHANNELS_UPDATE", guildId, settings: channel_overrides };
    importDefault(709).dispatch(obj);
    const obj5 = importDefault(709);
    const result1 = _require(5257).trackGuildNotificationSettingsUpdate(guildId, channel_overrides, currentGuildSettings, OptedIn, location);
    const obj7 = _require(5257);
  ...
  updateChannelOverrideSettings(guildId, id, muteSettings, NotificationLabel, location) {
    ...
    const result1 = require(5257) /* UserNotificationSettings */.trackChannelNotificationSettingsUpdate(obj1);
  },
  updateChannelOverrideSettingsBulk(guildId, channel_overrides, OptedOut) {
    ...
    const result = importDefault(5259).saveUserGuildSettings(guildId, obj);
    const obj3 = importDefault(5259);
