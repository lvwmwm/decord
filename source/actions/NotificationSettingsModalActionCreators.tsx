// Module ID: 5261
// Function ID: 5262
// Dependencies: [5, 4497, 676, 3941, 685, 709, 5256, 5258, 11, 4147, 1236, 1384, 530, 2]
...
export default {
  ...
  updateGuildNotificationSettings(guildId, muteSettings, NotificationLabel, location) {
    let obj = require(5256) /* UserNotificationSettings */;
    const currentGuildSettings = obj.getCurrentGuildSettings(guildId);
    const result = importDefault(5258).saveUserGuildSettings(guildId, muteSettings);
    const obj2 = importDefault(5258);
    obj = { type: "USER_GUILD_SETTINGS_GUILD_UPDATE", guildId, settings: muteSettings };
    importDefault(709).dispatch(obj);
    const obj3 = importDefault(709);
    const result1 = require(5256) /* UserNotificationSettings */.trackGuildNotificationSettingsUpdate(guildId, muteSettings, currentGuildSettings, NotificationLabel, location);
  },
  updateGuildAndChannelNotificationSettings(guildId, channel_overrides, OptedIn, location) {
    ...
    const currentGuildSettings = _require(5256).getCurrentGuildSettings(guildId);
    const obj2 = _require(5256);
    const manyCurrentChannelSettings = _require(5256).getManyCurrentChannelSettings(guildId, keys);
    const obj3 = _require(5256);
    let result = importDefault(5258).saveUserGuildSettings(guildId, channel_overrides);
    const obj4 = importDefault(5258);
    obj = { type: "USER_GUILD_SETTINGS_GUILD_AND_CHANNELS_UPDATE", guildId, settings: channel_overrides };
    importDefault(709).dispatch(obj);
    const obj5 = importDefault(709);
    const result1 = _require(5256).trackGuildNotificationSettingsUpdate(guildId, channel_overrides, currentGuildSettings, OptedIn, location);
    const obj7 = _require(5256);
  ...
  updateChannelOverrideSettings(guildId, id, muteSettings, NotificationLabel, location) {
    ...
    const result1 = require(5256) /* UserNotificationSettings */.trackChannelNotificationSettingsUpdate(obj1);
  },
  updateChannelOverrideSettingsBulk(guildId, channel_overrides, OptedOut) {
    ...
    const result = importDefault(5258).saveUserGuildSettings(guildId, obj);
    const obj3 = importDefault(5258);
