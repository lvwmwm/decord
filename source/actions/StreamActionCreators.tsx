...
// Function ID: 4435
// Name: watchStream
// Dependencies: [5, 4269, 4435, 4436, 4274, 1218, 1372, 1862, 1931, 4271, 676, 4319, 709, 4318, 38, 4437, 4448, 4459, 530, 687, 5079, 503, 9062, 5081, 4265, 9132, 2]
...
function watchStream(stream, forceMultiple) {
  if (null == remoteSessionId.getRemoteSessionId()) {
    ...
    if (null == stream.guildId) {
      const encodeStreamKeyResult = require(4318) /* isStreamKey */.encodeStreamKey(stream);
      ...
      if (!isInChannelResult) {
        obj = require(4437) /* allowChannelAccess */;
...
export const watchStreamAndTransitionToStream = function watchStreamAndTransitionToStream(stream, forceMultiple) {
  ...
  if (null == stream.guildId) {
    ...
    let isChannelFullResult = !isInChannelResult;
    if (!isInChannelResult) {
      isChannelFullResult = require(4437) /* allowChannelAccess */.isChannelFull(channel, tmp6, createGuildRecordFromRust);
...
export const updateStreamSettings = function updateStreamSettings(noTrack) {
  if (true !== noTrack.noTrack) {
    let obj = require(9062) /* isPremiumResolution */;
...
export const joinPrivateChannelAndWatchStream = function joinPrivateChannelAndWatchStream(arg0, streamKey) {
  const _require = arg0;
  id = id.getId();
  const importDefault = _require(4318).decodeStreamKey(streamKey);
  const voiceChannelId = store2.getVoiceChannelId();
  if (tmp4) {
    importDefault(5081).disconnect();
    const obj2 = importDefault(5081);
  }
  let obj = _require(4318);
