...
function watchStream(stream, forceMultiple) {
  if (null == remoteSessionId.getRemoteSessionId()) {
    ...
    if (null == stream.guildId) {
      const encodeStreamKeyResult = require(4367) /* isStreamKey */.encodeStreamKey(stream);
      ...
      if (!tmp21) {
        const participant = tmp18(4497).selectParticipant(stream.channelId, encodeStreamKeyResult);
        const tmp18Result = tmp18(4497);
      }
...
export const watchStreamAndTransitionToStream = function watchStreamAndTransitionToStream(stream, forceMultiple) {
  ...
  if (null == stream.guildId) {
    ...
    let isChannelFullResult = !isInChannelResult;
    if (!isInChannelResult) {
      isChannelFullResult = require(4486) /* allowChannelAccess */.isChannelFull(channel, tmp6, createGuildRecordFromRust);
...
export const joinPrivateChannelAndWatchStream = function joinPrivateChannelAndWatchStream(arg0, streamKey) {
  const _require = arg0;
  id = id.getId();
  const importDefault = _require(4367).decodeStreamKey(streamKey);
  const voiceChannelId = store2.getVoiceChannelId();
  if (tmp4) {
    importDefault(5129).disconnect();
    const obj2 = importDefault(5129);
  }
  let obj = _require(4367);
  tmp4 = null != voiceChannelId && voiceChannelId !== arg0;
  importDefault(4313).addRecipient(arg0, id, undefined, () => {
