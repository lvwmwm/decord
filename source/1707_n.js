...
let fn = function n(config, _animationToClamp) {
  ...
  const fn = function u() {
    ...
    if (undefined !== config.max) {
      strippedValue1 = config(1674).recognizePrefixSuffix(tmp3.max).strippedValue;
      const obj2 = config(1674);
    }
    obj = {
      isHigherOrder: true,
    ...
    obj[6] = config(1674).getReduceMotionForAnimation(config.reduceMotion);
    return obj;
  };
  obj = { _animationToClamp, config, recognizePrefixSuffix: _require(1674).recognizePrefixSuffix, logger: _require(1638).logger, getReduceMotionForAnimation: _require(1674).getReduceMotionForAnimation };
  fn.__closure = obj;
