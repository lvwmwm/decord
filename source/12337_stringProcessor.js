...
export const toJSONSchema = function toJSONSchema(_idmap, uri) {
  ...
  const initializeContextResult = require(12336) /* initializeContext */.initializeContext(obj);
  if (tmp) {
    ...
    while (tmp10 !== undefined) {
      ...
      let processResult = require(12336) /* initializeContext */.process(tmp15[1], initializeContextResult);
      continue;
    }
    obj = {};
    ...
    for (const item10061 of entries1) {
      ...
      let extractDefsResult = require(12336) /* initializeContext */.extractDefs(initializeContextResult, tmp27);
      obj[tmp26[0]] = require(12336) /* initializeContext */.finalize(initializeContextResult, tmp27);
      continue;
    }
    ...
    tmp2(12336).process(_idmap, initializeContextResult);
    tmp2(12336).extractDefs(initializeContextResult, _idmap);
    return tmp2(12336).finalize(initializeContextResult, _idmap);
  }
...
export const arrayProcessor = (_zod, arg1, arg2, path) => {
  ...
  obj.path = items;
  arg2.items = require(12336) /* initializeContext */.process(_zod._zod.def.element, arg1, obj);
};
export const objectProcessor = (_zod, io, properties, path) => {
  ...
  for (const key10015 in shape) {
    ...
    obj.path = items;
    arg2.properties[key10015] = _require(12336).process(shape[key10015], arg1, obj);
    continue;
  }
  ...
  if ("never" === type) {
    ...
    if (def.catchall) {
      ...
      obj.path = items2;
      properties.additionalProperties = _require(12336).process(def.catchall, io, obj);
    }
...
export const intersectionProcessor = (_zod, arg1, arg2, path) => {
  ...
  obj.path = items;
  const processResult = require(12336) /* initializeContext */.process(def.left, arg1, obj);
  obj = {};
  const merged1 = Object.assign(path);
  const items1 = [...path.path, "allOf", 1];
  obj.path = items1;
  const processResult1 = require(12336) /* initializeContext */.process(def.right, arg1, obj);
...
export const recordProcessor = (_zod, target, patternProperties, path) => {
  ...
  if ("loose" === def.mode) {
    if (patterns) {
      if (patterns.size > 0) {
        let obj = {};
        const _process = require(12336) /* initializeContext */.process;
  ...
  if (!tmp) {
    ...
    obj.path = items2;
    patternProperties.propertyNames = require(12336) /* initializeContext */.process(def.keyType, target, obj);
  }
  obj = {};
  ...
  obj.path = items3;
  patternProperties.additionalProperties = require(12336) /* initializeContext */.process(def.valueType, target, obj);
};
...
export const nonoptionalProcessor = (_zod, seen) => {
  const def = _zod._zod.def;
  require(12336) /* initializeContext */.process(def.innerType, seen, arg3);
  seen = seen.seen;
...
export const pipeProcessor = (_zod, io) => {
  ...
  if ("input" !== io.io) {
    const out = def.out;
    require(12336) /* initializeContext */.process(out, io, arg3);
    const seen = io.seen;
...
export const lazyProcessor = (_zod, seen) => {
  ...
  require(12336) /* initializeContext */.process(innerType, seen, arg3);
  seen = seen.seen;
