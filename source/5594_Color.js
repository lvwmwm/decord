...
function getset(cmyk, arg1, arg2) {
  ...
  for (const item10015 of tmp3) {
    ...
    let tmp6 = dependencyMap[item10015];
    let tmp7 = tmp6;
    if (!tmp6) {
      let tmp8 = dependencyMap;
      let tmp9 = item10015;
      let items1 = [];
      dependencyMap[tmp4] = items1;
      tmp7 = items1;
    }
    let tmp10 = tmp7;
    let tmp11 = arg1;
...
while (iter !== undefined) {
  ...
  let _moduleResult = _toConsumableArray(require("keys")[nextResult].labels);
...
obj = {
  ...
  string(arg0) {
    const self = this;
    let self2 = this;
    if (!(this.model in require(5599) /* clamp */.to)) {
      self2 = self.rgb();
    }
    let num = 1;
    ...
    const to = require(5599) /* clamp */.to;
  ...
  percentString(arg0) {
    ...
    const rgb = require(5599) /* clamp */.to.rgb;
  ...
  object() {
    const self = this;
    const obj = {};
    const channels = require(5595) /* keys */[this.model].channels;
    for (let num = 0; num < channels; num = num + 1) {
      obj[require(5595) /* keys */[this.model].labels[num]] = self.color[num];
    }
    if (1 !== self.valpha) {
  ...
  round(arg0) {
    ...
    const items1 = [self.valpha];
    return new Color(items.concat(_toConsumableArray(color.map((function roundToPlace(arg0) {
      let closure_0 = arg0;
      return (toFixed) => Number(toFixed.toFixed(closure_0));
  ...
  alpha(self, arg1) {
    ...
    if (undefined !== self) {
      ...
      let valpha = new Color(items.concat(_toConsumableArray(self.color), items1), self.model);
      const tmp3 = _toConsumableArray(self.color);
    } else {
      valpha = self.valpha;
    }
    return valpha;
  ...
  keyword(color) {
    ...
    if (undefined !== color) {
      ...
      keywordResult = require(5595) /* keys */[self.model].keyword(self.color);
      const obj = require(5595) /* keys */[self.model];
    }
...
function _loop(item10145) {
  const _require = item10145;
  if (closure_4.includes(item10145)) {
    return 1;
  } else {
    channels = _require(channels[2])[item10145].channels;
    Color.prototype[item10145] = function() {
      const self = this;
      if (this.model === item10145) {
        const prototype3 = outer1_7.prototype;
        const tmp15 = new outer1_7(self);
        return tmp15;
      } else {
        ...
        if (array.length > 0) {
          ...
          let tmp22 = new outer1_7(array, item10145);
        } else {
          tmp22 = outer1_7;
          const items = [];
          const rawResult = item10145(channels[2])[self.model][item10145].raw(self.color);
          const _Array2 = Array;
          let tmp = rawResult;
          ...
          tmp22 = new tmp22(items.concat(outer1_3(tmp), items2), tmp26);
          const obj = item10145(channels[2])[self.model][item10145];
          const tmp23 = outer1_3;
        }
        return tmp22;
      }
    };
    Color[item10145] = () => {
      ...
      if ("number" === typeof first) {
        outer1_8(array, channels);
        first = array;
      }
      return new outer1_7(first, closure_0);
    };
  }
}
