...
// Function ID: 5485
// Name: hash
// Dependencies: [32, 5471, 5470, 5485, 5494]
...
function maybeUnpackValue(enabled) {
  const Reanimated = require(5470).Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: require("module_5470").Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === require(5470).Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(5470).Reanimated;
...
export const bindSharedValues = function bindSharedValues(closure_1, _slicedToArray) {
  ...
  if (undefined !== _require(5470).Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let Reanimated = _require(5470).Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let tmp13 = tmp9;
        let tmp14 = tmp11;
        let Reanimated2 = tmp10(5470).Reanimated;
        let tmp15 = tmp7;
        let tmp16 = tmp6;
...
export const unbindSharedValues = function unbindSharedValues(closure_1, _slicedToArray) {
  if (undefined !== _require(5470).Reanimated) {
    ...
    function _loop(iter) {
      const callback = iter;
      const Reanimated = callback(5470).Reanimated;
      if (Reanimated.isSharedValue(iter)) {
        ...
        dependencyMap = sum;
        const Reanimated2 = callback(5470).Reanimated;
        const fn = function t() {
...
export const hasWorkletEventHandlers = function hasWorkletEventHandlers(useAnimated) {
  const iter = require(5485) /* items2 */.HandlerCallbacks[Symbol.iterator]();
