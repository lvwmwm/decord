...
// Dependencies: [32, 19, 17, 21, 5445, 5505, 5441, 5500, 5442, 5432, 5470, 5501, 5502, 5494, 5503]
// Exports: InterceptingGestureDetector

// Module 5504 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  if (prop) {
    let REANIMATED = tmp(5505).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(5505).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(5505).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(5505).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(5441).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(5500).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(5442);
  }
  ...
  c5(() => {
    ...
    if (prop) {
      _undefined.setMode(gesture(5505).InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        _undefined.setMode(gesture(5505).InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const detectorAttachmentGuard = tmp(5503).useDetectorAttachmentGuard(tmp8Result1);
    ...
    obj1[10] = prop6;
    obj1[11] = tmp8Result1;
    obj1[12] = tmp(5441).nativeDetectorStyles.detector;
    obj1[13] = tmp9;
    const _globalThis = globalThis;
    obj1[14] = globalThis._RNGH_MODULE_ID;
    obj1[15] = children;
    obj[1] = c8(ReanimatedNativeDetector, obj1);
    return c8(tmp(5505).InterceptingDetectorContext, obj);
  } else {
    const _Error = Error;
    let error = new Error(tmp(5432).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
  const tmp20 = c5;
