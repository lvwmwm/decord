...
// Module 13558 (__exportStarResult11)
const require = arg1;
...
arg5.ApplyUnsignedRoundingMode = function ApplyUnsignedRoundingMode(eq, timesResult, roundedNumber, result) {
  if (eq.eq(timesResult)) {
    ...
    require(13546) /* getMultiInternalSlots */.invariant(timesResult.lessThan(eq) && eq.lessThan(roundedNumber), combined1.concat(roundedNumber));
    if ("zero" === result) {
      return timesResult;
