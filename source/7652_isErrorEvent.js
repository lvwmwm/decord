...
// Function ID: 7653
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 7644, 7625, 7597, 7651, 7608, 7604, 7620, 7621, 7653, 7640, 7641, 7624, 7646, 7633, 7654, 7637, 7655, 7602]

// Module 7652 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(7625).DEBUG_BUILD) {
          const logger = tmp(7597).logger;
        ...
        BaseClient(7620).updateSession(release, { init: false });
        const obj = BaseClient(7620);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(7653).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(7653).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(7653);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(7640).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(7625).DEBUG_BUILD) {
        let logger = tmp2(7597).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(7621).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(7653).setupIntegrations(this, integrations);
      const obj = BaseClient(7653);
      const result = BaseClient(7653).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(7637).parseSampleRate(sampleRate);
        let obj = str(7637);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(7654).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(7621).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(7625).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(7597).logger;
