...
// Function ID: 1729
// Name: validateConfig
// Dependencies: [1664, 1693, 1729, 1730, 1731]

// Module 1728 (validateConfig)
const require = arg1;
  ...
  if (clamp.clamp) {
    ...
    if (Array.isArray(clamp.clamp)) {
      if (2 !== clamp.clamp.length) {
        ...
        const reanimatedError = new require(1664) /* t */.ReanimatedError("`clamp array` must contain 2 items but is given " + clamp.clamp.length + ".");
        throw reanimatedError;
      }
    } else {
      const _HermesInternal = HermesInternal;
      const reanimatedError1 = new require(1664) /* t */.ReanimatedError("`config.clamp` must be an array but is " + typeof clamp.clamp + ".");
      throw reanimatedError1;
    }
  }
  if (clamp.velocityFactor <= 0) {
    const _HermesInternal2 = HermesInternal;
    const reanimatedError2 = new require(1664) /* t */.ReanimatedError("`config.velocityFactor` must be greater then 0 but is " + clamp.velocityFactor + ".");
    throw reanimatedError2;
  } else if (clamp.rubberBandEffect) {
    if (!clamp.clamp) {
      const reanimatedError3 = new require(1664) /* t */.ReanimatedError("You need to set `clamp` property when using `rubberBandEffect`.");
...
let fn = function n(userConfig, callback) {
  ...
  obj = { userConfig, isValidRubberBandConfig: _require(1729).isValidRubberBandConfig, rubberBandDecay: _require(1730).rubberBandDecay, rigidDecay: _require(1731).rigidDecay, validateConfig, callback, getReduceMotionForAnimation: _require(1693).getReduceMotionForAnimation };
  fn.__closure = obj;
