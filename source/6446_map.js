...
// Function ID: 6447
// Name: map
// Dependencies: [32, 19, 6444, 6434, 6447, 6420, 6435, 6382]
...
function isGestureEnabled(closure_0) {
  if (obj.isComposedGesture(closure_0)) {
    ...
    someResult = false !== require(6434) /* hash */.maybeUnpackValue(closure_0.config.enabled);
    const tmpResult = require(6434) /* hash */;
  }
  return someResult;
}
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = require(6447) /* isNativeEvent */.isNativeAnimatedEvent(useAnimated.onUpdate);
    const obj = require(6447) /* isNativeEvent */;
  }
  ...
  if (!disableReanimated) {
    result = undefined !== require(6420).Reanimated;
  }
  if (result) {
    result = require(6434) /* hash */.hasWorkletEventHandlers(useAnimated);
    const obj2 = require(6434) /* hash */;
  }
  if (result) {
    result = !useAnimated.dispatchesAnimatedEvents;
  }
  useAnimated.shouldUseReanimatedDetector = result;
  useAnimated.needsPointerData = require(6447) /* isNativeEvent */.shouldHandleTouchEvents(useAnimated);
};
export const prepareConfigForNativeSide = function prepareConfigForNativeSide(closure_0, closure_1) {
  ...
  const PropsWhiteLists = require(6435) /* items2 */.PropsWhiteLists;
  let EMPTY_WHITE_LIST = PropsWhiteLists.get(closure_0);
  if (EMPTY_WHITE_LIST == null) {
    EMPTY_WHITE_LIST = require(6435) /* items2 */.EMPTY_WHITE_LIST;
  }
  ...
  while (tmp12 !== undefined) {
    ...
    let allowedNativeProps = require(6435) /* items2 */.allowedNativeProps;
