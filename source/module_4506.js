// Module ID: 4506
// Function ID: 39723
// Dependencies: [29, 31, 27, 33, 4504, 4495, 4488, 4507, 4494, 4510, 4483]
...
export default importAllResult.forwardRef(function ScreenStackItem(unstable_sheetFooter) {
  ...
  const dependencyMap = importAllResult.useContext(screenId(4504).RNSScreensRefContext);
  const imperativeHandle = importAllResult.useImperativeHandle(arg1, () => ref.current);
  let obj = screenId(4495);
  let prop;
  ...
  importDefault(4488)(flag, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  if ("formSheet" !== str) {
  ...
  obj.children = callback2(importDefault(4507), obj1);
  const items1 = [callback2(screenId(4495).EdgeInsetApplicationContext.Provider, obj), callback2(screenId(4494).ScreenStackHeaderConfig, Object.assign({}, headerConfig)), ];
  let tmp23 = "formSheet" === str && unstable_sheetFooter;
  if (tmp23) {
    const obj2 = { children: unstable_sheetFooter() };
    tmp23 = callback2(screenId(4510).FooterComponent, obj2);
  }
  items1[2] = tmp23;
  obj.children = items1;
  ...
  let largeTitle;
  const tmp16 = importDefault(4488);
  ...
  return tmp28(importDefault(4483), Object.assign(obj3, tmp, { children }));
});
