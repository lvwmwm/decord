...
export const RVLayoutManager = () => {
  class RVLayoutManager {
    constructor(arg0, arg1) {
      self = this;
      tmp = closure_3(this, RVLayoutManager);
      ...
      multiTypeAverageWindow = new RVLayoutManager(closure_1[3]).MultiTypeAverageWindow(5, 200);
      this.heightAverageWindow = multiTypeAverageWindow;
      multiTypeAverageWindow1 = new RVLayoutManager(closure_1[3]).MultiTypeAverageWindow(5, 200);
  ...
  items[4] = {
    ...
    value: function getVisibleLayouts(bound1, bound1) {
      const result = RVLayoutManager(closure_1[4]).findFirstVisibleIndex(this.layouts, bound1, this.horizontal);
      const obj = RVLayoutManager(closure_1[4]);
      const findLastVisibleIndexResult = RVLayoutManager(closure_1[4]).findLastVisibleIndex(this.layouts, bound1, this.horizontal);
      if (-1 !== result) {
        if (-1 !== findLastVisibleIndexResult) {
          const ConsecutiveNumbers = RVLayoutManager(closure_1[5]).ConsecutiveNumbers;
          const prototype = ConsecutiveNumbers.prototype;
          let EMPTY = new ConsecutiveNumbers(result, findLastVisibleIndexResult);
        }
        return EMPTY;
      }
      EMPTY = RVLayoutManager(closure_1[5]).ConsecutiveNumbers.EMPTY;
    }
  };
  items[5] = {
    key: "deleteLayout",
    value: function deleteLayout(arr) {
      ...
      const applyResult = min.apply(Math, callback(arr));
  ...
  items[6] = {
    ...
    value: function modifyLayout(arr, arg1) {
      ...
      if (this.layouts.length > arg1) {
        ...
        MAX_VALUE = arg1 - 1;
        found = arr.filter((index) => index.index < arg1);
      }
  ...
  items[7] = {
    ...
    value: function getLayout(arg0) {
      ...
      if (arg0 >= this.layouts.length) {
        const _Error = Error;
        const error = new Error(RVLayoutManager(closure_1[6]).ErrorMessages.indexOutOfBounds);
        throw error;
      } else {
        let tmp = self.layouts[arg0];
        if (!tmp) {
          const obj = { <string:80505088>: null, <string:80502272>: 4, <string:80550656>: 4, <string:80553216>: "rgba(0, 0, 0, 0.75)" };
    ...
    value: function computeEstimatesAndMinMaxChangedLayout(found) {
      ...
      while (iter !== undefined) {
        ...
        if (!tmp6) {
          ...
          let tmp10 = closure_1;
          let obj = RVLayoutManager(closure_1[7]);
          let tmp11 = size;
        ...
        if (!result) {
          ...
          let tmp15 = closure_1;
          let obj2 = RVLayoutManager(closure_1[7]);
          let tmp16 = size;
