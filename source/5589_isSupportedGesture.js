...
// Dependencies: [32, 19, 5590, 5601, 5592, 5605, 5576]
// Exports: useJSResponderHandler

// Module 5589 (isSupportedGesture)
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(5592).SingleGestureName.Tap !== type) {
      if (tmp(5592).SingleGestureName.LongPress !== type) {
        if (tmp(5592).SingleGestureName.Fling !== type) {
          if (tmp(5592).SingleGestureName.Native !== type) {
            if (tmp(5592).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  ...
  let tmp = callback2(_require(5605).JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  callback4(() => {
    const Reanimated = gesture(5576).Reanimated;
    let obj = gesture(5590);
  ...
  const tmp7 = callback3(() => {
    let some = gesture;
    const isGestureEnabledResult = gesture(5601).isGestureEnabled(gesture);
  ...
  let handleStartShouldSetResponder = callback3(() => {
    if (callback()) {
      const result = gesture(5605).updateResponderEventValue(dependencyMap, true);
      const obj = gesture(5605);
    }
    return false;
  }, items2);
  if (null == tmp) {
