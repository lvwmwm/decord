...
// Function ID: 6417
// Name: _flush
// Dependencies: [5, 6395, 6417, 6396, 6368, 6379, 6382, 6375, 6405, 6369, 6391]
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = require(6395) /* getClient */.getCurrentScope();
  const obj = require(6395) /* getClient */;
  return currentScope.captureException(arg0, require(6417) /* applyClientOptions */.parseEventHintOrCaptureContext(arg1));
};
...
export const captureSession = function captureSession() {
  ...
  if (flag) {
    ...
    const obj5 = require(6395) /* getClient */;
  }
};
...
export const isInitialized = function isInitialized() {
  return require(6395) /* getClient */.getClient();
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(6405).DEFAULT_ENVIRONMENT;
  }
  const obj5 = require(6395) /* getClient */;
  const userAgent = require(6369) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {}.userAgent;
  let tmpResult = tmp(6391);
  obj = { release, environment, user: null };
  const tmp3 = require(6369) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {};
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    ...
    obj[3] = monitorSlug(6382).timestampInSeconds() - closure_3;
    const obj2 = monitorSlug(6382);
    const currentScope = monitorSlug(6395).getCurrentScope();
    const obj3 = monitorSlug(6395);
    const client = monitorSlug(6395).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(6395);
    monitorSlug(6379).uuid4();
  }
  let currentScope = _require(6395).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(6395);
  let client = _require(6395).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(6396).DEBUG_BUILD) {
      let logger2 = tmp(6368).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(6368).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(6395);
  captureCheckInResult = _require(6379).uuid4();
};
