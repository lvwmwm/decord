...
function convertToPlainObject(type) {
  let obj = require(6354) /* isBuiltin */;
  if (obj.isError(type)) {
    ...
    if (obj2.isEvent(type)) {
      ...
      if (isInstanceOfResult) {
        isInstanceOfResult = require(6354) /* isBuiltin */.isInstanceOf(type, globalThis.CustomEvent);
...
function serializeEventTarget(arg0) {
  if (obj.isElement(arg0)) {
    let htmlTreeAsStringResult = require(6355) /* _htmlElementAsString */.htmlTreeAsString(arg0);
    const obj2 = require(6355) /* _htmlElementAsString */;
  } else {
    const _Object = Object;
...
export const extractExceptionKeysForMessage = function extractExceptionKeysForMessage(arg0) {
  ...
  if (first) {
    if (first.length >= num) {
      return require(6356) /* isMatchingPattern */.truncate(first, num);
    } else {
      ...
      if (length > 0) {
        ...
        if (length !== keys.length) {
          truncateResult = require(6356) /* isMatchingPattern */.truncate(joined, num);
          const obj2 = require(6356) /* isMatchingPattern */;
        }
