  ...
  if (tmp8) {
    ...
    if (iter !== undefined) {
      try {
        ...
        if (require(6370) /* __SENTRY_DEBUG__ */.DEBUG_BUILD) {
          const logger = tmp19(6371).logger;
          logger.error(tmp2 + tmp6 + tmp3 + tmp19(6374).getFunctionName(tmp7) + tmp4, tmp18);
          const tmp19Result = tmp19(6374);
        }
      }
    }
