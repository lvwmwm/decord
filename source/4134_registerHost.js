...
// Function ID: 4135
// Name: registerHost
// Dependencies: [4135, 4130]

// Module 4134 (registerHost)
const require = arg1;
...
function reducer(arg0, type) {
  ...
  if (portalName(4130).ACTIONS.REGISTER_HOST === type) {
    const hostName4 = type.hostName;
    if (typeof registerHost !== "error") {
    ...
    return obj;
  } else if (tmp4(4130).ACTIONS.DEREGISTER_HOST === type) {
    const hostName3 = type.hostName;
    delete tmp[tmp2];
    return obj;
  } else if (tmp4(4130).ACTIONS.ADD_UPDATE_PORTAL === type) {
    ...
    return obj;
  } else if (tmp4(4130).ACTIONS.REMOVE_PORTAL === type) {
    ({ hostName, portalName } = type);
    if (typeof removePortal !== "error") {
      HermesBuiltin.throwTypeError();
    }
    if (hostName in obj) {
