...
for (const item10022 of keys) {
  ...
  let tmp6 = arg1;
  let tmp7 = arg6;
  obj[require("keys")[item10022]] = item10022;
  continue;
}
...
while (iter !== undefined) {
  ...
  let tmp37 = obj;
  if ("channels" in tmp8[nextResult]) {
    let tmp16 = obj;
    let tmp17 = nextResult;
    if ("labels" in tmp8[tmp36]) {
      let tmp23 = obj;
      let tmp24 = nextResult;
      if (tmp8[tmp36].labels.length !== tmp8[tmp36].channels) {
        let _Error3 = Error;
        let tmp31 = nextResult;
        let str3 = "channel and label counts mismatch: ";
        let tmp32 = new.target;
        let tmp33 = new.target;
        let error = new Error("channel and label counts mismatch: " + tmp36);
        let tmp35 = error;
        throw error;
      } else {
        let tmp25 = obj;
        let tmp26 = nextResult;
        let tmp27 = tmp8[tmp36];
        ({ channels, labels } = tmp8[tmp36]);
        delete tmp.channels;
        let tmp28 = tmp8[tmp36];
        delete tmp.labels;
        let _Object = Object;
        obj = { value: channels };
        let definePropertyResult = Object.defineProperty(tmp8[tmp36], "channels", obj);
        let _Object2 = Object;
        let obj1 = { value: labels };
        let definePropertyResult1 = Object.defineProperty(tmp8[tmp36], "labels", obj1);
        continue;
      }
    } else {
      let _Error2 = Error;
      ...
      let error1 = new Error("missing channel labels property: " + tmp36);
      let tmp22 = error1;
      throw error1;
    }
  } else {
    ...
    let error2 = new Error("missing channels property: " + tmp36);
    let tmp15 = error2;
    throw error2;
  }
}
...
obj.rgb.keyword = (arg0) => {
  ...
  if (obj[arg0]) {
    ...
    const keys = Object.keys(require(5564) /* keys */);
    ...
    while (iter !== undefined) {
      ...
      let tmp14 = require(5564) /* keys */[nextResult];
      let tmp15 = arg0;
      let num5 = 2;
...
obj.keyword.rgb = (arg0) => require(5564) /* keys */[arg0];
obj.rgb.xyz = (arg0) => {
  const result = arg0[0] / 255;
  const result1 = arg0[1] / 255;
  const result2 = arg0[2] / 255;
...
obj.rgb.ansi16 = (value) => {
  ...
  [tmp3, tmp4, tmp5] = _slicedToArray(value, 3);
  if (null === tmp) {
    const rgb = obj.rgb;
    tmp = rgb.hsv(value)[2];
  }
  ...
  const tmp2 = _slicedToArray(value, 3);
};
obj.hsv.ansi16 = (red) => {
  const rgb = obj.rgb;
  const hsv = obj.hsv;
