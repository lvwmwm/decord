...
arg5.computeOffsetIfDataChanged = (() => {
  function computeOffsetIfDataChanged(arg0) {
    ...
    if (direction < 0) {
      ...
      let parsed = Number.parseInt(String(outer1_1(previousLength, result / previousLength)));
      let result1 = result;
    } else {
      result1 = (absolute - size) / size;
      const _Number = Number;
      const _String = String;
      parsed = Number.parseInt(String(outer1_1(previousLength, result1 / previousLength))) + 1;
    }
    const tmp8 = outer1_1(previousLength, result1 % previousLength);
    let diff = tmp8;
    if (direction >= 0) {
  ...
  computeOffsetIfDataChanged.__closure = { omitZero: closure_1 };
