// Module ID: 4392
// Function ID: 38845
// Name: ToPrimitive
// Dependencies: [1378, 4393, 4394, 4395]

// Module 4392 (ToPrimitive)
let tmp = "function" === typeof Symbol;
if (tmp) {
  let _Symbol = Symbol;
  tmp = "symbol" === typeof Symbol.iterator;
}
let closure_2 = tmp;

export default function ToPrimitive(arg0) {
  let callResult;
  if (require(4393) /* isPrimitive */(arg0)) {
    return arg0;
  } else {
    let str2 = "default";
    if (arguments.length > 1) {
      const _String = String;
      let str3 = "string";
      if (arguments[1] !== String) {
        const _Number = Number;
        let str4 = "default";
        if (arguments[1] === Number) {
          str4 = "number";
        }
        str3 = str4;
      }
      str2 = str3;
    }
    if (!closure_2) {
      {
        let tmp22 = "default" === str2;
        if (tmp22) {
          let tmp19 = require(4395) /* isDateObject */(arg0);
          if (!tmp19) {
            tmp19 = require(4394)(arg0);
          }
          tmp22 = tmp19;
        }
        let str8 = str2;
        if (tmp22) {
          str8 = "string";
        }
        let str10 = "number";
        if ("default" !== str8) {
          str10 = str8;
        }
        if (null == arg0) {
          const _TypeError4 = TypeError;
          const typeError = new TypeError("Cannot call method on " + arg0);
          throw typeError;
        } else {
          if ("number" !== str10) {
            if ("string" !== str10) {
              const _TypeError3 = TypeError;
              const typeError1 = new TypeError("hint must be \"string\" or \"number\"");
              throw typeError1;
            }
          }
          const arr = "string" === str10 ? ["toString", "valueOf"] : ["valueOf", "toString"];
          let num6 = 0;
          if (0 < arr.length) {
            while (true) {
              let obj = arg0[arr[num6]];
              let tmp24 = require;
              let tmp25 = dependencyMap;
              if (require(1378) /* apply */(obj)) {
                callResult = obj.call(arg0);
                let tmp27 = require;
                let tmp28 = dependencyMap;
                let tmp29 = callResult;
                if (require(4393) /* isPrimitive */(callResult)) {
                  break;
                }
              }
              num6 = num6 + 1;
            }
            return callResult;
          }
          const _TypeError2 = TypeError;
          const typeError2 = new TypeError("No default value");
          throw typeError2;
        }
      }
    } else {
      const _Symbol = Symbol;
      if (Symbol.toPrimitive) {
        const _Symbol3 = Symbol;
        let tmp8;
        if (null != arg0[toPrimitive]) {
          tmp8 = tmp6;
          if (!require(1378) /* apply */(tmp6)) {
            const _TypeError = TypeError;
            const _String2 = String;
            const text = `${tmp6} returned for property `;
            const typeError3 = new TypeError(`${tmp6} returned for property ` + String(toPrimitive) + " of object " + arg0 + " is not a function");
            throw typeError3;
          }
        }
        let valueOf = tmp8;
      } else if (require(4394)(arg0)) {
        const _Symbol2 = Symbol;
        valueOf = Symbol.prototype.valueOf;
      }
    }
  }
};
