...
// Dependencies: [27, 31, 33, 5411, 5448, 5449, 5450]
// Exports: useSecondaryProps

// Module 5447 (useSecondaryProps)
...
let closure_2;
let closure_3;
({ Animated: closure_2, RefreshControl: closure_3 } = get_ActivityIndicator);
...
export const useSecondaryProps = function useSecondaryProps(ListHeaderComponent) {
  ...
  const tmp7 = ListEmptyComponentStyle(() => {
    let tmp = null;
  ...
  let tmp9 = ListEmptyComponentStyle(() => {
    if (ListEmptyComponent) {
      let obj = ListHeaderComponent(ListHeaderComponentStyle[5]);
      const validComponent = obj.getValidComponent(ListEmptyComponent);
      if (invertedTransformStyle) {
  ...
  obj.CompatScrollView = ListEmptyComponentStyle(() => {
    if ("function" === typeof renderScrollComponent) {
      if (!obj.isComponentClass(renderScrollComponent)) {
        let CompatAnimatedScroller = ListEmptyComponent.forwardRef((arg0, ref) => outer1_6(Object.assign({}, arg0, { ref })));
