...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 15494]
// Exports: captureScreen, ensureModuleIsLoaded

// Module 15493 (_isNativeReflectConstruct)
...
function captureRef(obj, options) {
  ...
  if (ViewShot(15494)) {
    ...
    return tmp(15494).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(15494).releaseCapture(str);
    const obj = ViewShot(15494);
  }
}
...
export const captureScreen = function captureScreen(options) {
  ...
  if (ViewShot(15494)) {
    ...
    return tmp(15494).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
