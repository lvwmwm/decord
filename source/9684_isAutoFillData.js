...
// Module 9684 (isAutoFillData)
const SINGLE_ITEM = require(dependencyMap[0]).DATA_LENGTH.SINGLE_ITEM;
const DOUBLE_ITEM = require(dependencyMap[0]).DATA_LENGTH.DOUBLE_ITEM;
...
export const convertToSharedIndex = () => {
  function convertToSharedIndex(loop) {
    ...
    if (callback(obj)) {
      if (closure_0 === rawDataLength) {
        return 0;
      } else if (closure_1 === rawDataLength) {
        return index % 2;
      }
    }
    return index;
