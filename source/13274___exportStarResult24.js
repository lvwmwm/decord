...
// Dependencies: [1257, 13247, 13269, 13248, 13262, 13264]
// Exports: PartitionNumberPattern

// Module 13274 (__exportStarResult24)
...
export const PartitionNumberPattern = function PartitionNumberPattern(internalSlots, isNaN) {
  ...
  if (isNaN.isNaN()) {
    ...
    let num6 = 0;
    if (!isNaN.isZero()) {
      require(13248) /* setInternalSlot */.invariant(isNaN.isFinite(), "Input must be a mathematical value");
      let timesResult = isNaN;
      if ("percent" == internalSlots.style) {
        timesResult = isNaN.times(100);
      }
      const ComputeExponentResult = require(13262) /* __exportStarResult13 */.ComputeExponent(internalSlots, timesResult);
    ...
    const result = require(13264) /* __exportStarResult21 */.FormatNumericToString(internalSlots, timesResult1);
  ...
  if ("never" !== signDisplay) {
    if ("auto" === signDisplay) {
      ...
      require(13248) /* setInternalSlot */.invariant("negative" === signDisplay, "signDisplay must be \"negative\"");
      let num15 = 0;
      if (roundedNumber.isNegative()) {
        num15 = 0;
        if (!roundedNumber.isZero()) {
