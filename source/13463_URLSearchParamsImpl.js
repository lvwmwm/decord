...
class URLSearchParamsImpl {
  constructor(arg0, arg1, arg2) {
    ...
    this._url = null;
    if (!flag) {
      flag = typeof first === "__FORMATJS_LISTFORMAT_DATA__";
    }
    if (!flag) {
      str = "?";
      flag = "?" !== first[0];
...
let obj = {
  ...
  value: function _updateSteps() {
    const self = this;
    if (null !== this._url) {
      let serializeUrlencodedResult = URLSearchParamsImpl(13461).serializeUrlencoded(self._list);
      if ("" === serializeUrlencodedResult) {
        serializeUrlencodedResult = null;
      }
      self._url._url.query = serializeUrlencodedResult;
      const obj = URLSearchParamsImpl(13461);
    }
  }
};
let items = [
...
items[9] = {
  ...
  value: function toString() {
    return URLSearchParamsImpl(13461).serializeUrlencoded(this._list);
  }
};
