...
obj = Object.create(null);
obj.%ArrayBufferPrototype% = ["status", "Text"];
obj.%ArrayPrototype% = [];
obj.%ArrayProto_entries% = [];
obj.%ArrayProto_forEach% = ["<string:1470103553>", "<string:345636866>", "<string:5308417>"];
obj.%ArrayProto_keys% = [false, false, false];
obj.%ArrayProto_values% = ["<string:1555081692>", "<string:1996064986>", "<string:1073741878>"];
obj.%AsyncFunctionPrototype% = [];
obj.%AsyncGenerator% = [false, false];
obj.%AsyncGeneratorPrototype% = [null, null, null];
obj.%BooleanPrototype% = ["isFreeForStaffSelfPurchase", "Text"];
obj.%DataViewPrototype% = ["<string:2426404864>", "<string:4317774>"];
obj.%DatePrototype% = ["<string:4317774>", "<string:4245954560>"];
obj.%ErrorPrototype% = ["<string:337641472>", 16868];
obj.%EvalErrorPrototype% = [];
obj.%Float32ArrayPrototype% = ["<string:3378511872>", "<string:4318467>"];
obj.%Float64ArrayPrototype% = ["<string:4318467>", "<string:1903034368>"];
obj.%FunctionPrototype% = [];
obj.%Generator% = [];
obj.%GeneratorPrototype% = [];
obj.%Int8ArrayPrototype% = [];
obj.%Int16ArrayPrototype% = [];
obj.%Int32ArrayPrototype% = [null, null];
obj.%JSONParse% = [null, null];
obj.%JSONStringify% = [-1069530870, 760283138];
...
obj.%PromisePrototype% = ["person_kneeling", true];
obj.%PromiseProto_then% = [true, true, true];
obj.%Promise_all% = [];
obj.%Promise_reject% = ["r", "isShowingAlert"];
obj.%Promise_resolve% = [];
obj.%RangeErrorPrototype% = [true, true];
obj.%ReferenceErrorPrototype% = [null, null];
obj.%RegExpPrototype% = [];
obj.%SetPrototype% = [null, null];
obj.%SharedArrayBufferPrototype% = [];
obj.%StringPrototype% = [];
obj.%SymbolPrototype% = [null, null];
obj.%SyntaxErrorPrototype% = [false, false];
obj.%TypedArrayPrototype% = ["RN$UnifiedNativeModuleProxy", "chart"];
obj.%TypeErrorPrototype% = [];
obj.%Uint8ArrayPrototype% = [];
obj.%Uint8ClampedArrayPrototype% = [];
obj.%Uint16ArrayPrototype% = [];
obj.%Uint32ArrayPrototype% = [-180289534, 1090519586];
obj.%URIErrorPrototype% = [-1539244030, 1090519327];
obj.%WeakMapPrototype% = [false, false];
obj.%WeakSetPrototype% = [true, true];
  ...
  if ("string" === typeof arg0) {
    if (0 !== arg0.length) {
      ...
      if (null === callback4(/^%?[^%]*%?$/, arg0)) {
        ...
        if (alias) {
          str2 = alias[0];
          callback2(arr, callback([null, null], alias));
        }
        let tmp8 = value;
        let flag = false;
        let num3 = 1;
