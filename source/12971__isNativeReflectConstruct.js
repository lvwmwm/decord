...
({ Easing: closure_11, FlatList, Platform, View: closure_12 } = tmp3);
({ I18nManager, ScrollView } = tmp3);
...
const tmp5 = (Component) => {
  class Carousel {
    constructor(arg0) {
      self = this;
      tmp = closure_5(this, Carousel);
      items = [];
      items[0] = Component;
      obj = closure_8(Carousel);
      tmp2 = closure_7;
      if (closure_17()) {
        tmp4 = globalThis;
        _Reflect = Reflect;
      ...
      if (!Carousel(closure_2[10]).ViewPropTypes) {
        tmp12 = globalThis;
        _console = console;
  ...
  let obj = {
    ...
    value: function componentDidMount() {
      ...
      let closure_3 = this._getFirstItem(props.firstItem);
      ...
      this._mounted = true;
      const result = this._initPositionsAndInterpolators();
      const animationFrame = requestAnimationFrame(() => {
        if (self._mounted) {
          self._snapToItem(closure_3, false, false, true, false);
          const result = self._hackActiveSlideAnimation(closure_3, "start", true);
          if (closure_1) {
  ...
  obj = {
    ...
    value: function shouldComponentUpdate(arg0, arg1) {
      ...
      if (!tmp2) {
        tmp2 = callback(closure_2[11])(tmp, arg0, arg1);
      }
      return tmp2;
    }
  };
            ...
            if (!tmp4) {
              if (!tmp5) {
                if (!tmp6) {
                  ...
                  const tmp8 = _getFirstItemResult !== self._previousFirstItem && _getFirstItemResult !== self._activeItem;
                }
    ...
    value: function _setScrollHandler(props) {
      ...
      if (props.onScroll) {
        ...
        if (Array.isArray(props.onScroll._argMapping)) {
          items1.pop();
          const first = callback3(props.onScroll._argMapping, 1)[0];
          ...
          push.apply(items1, callback2(props.onScroll._argMapping));
        }
      }
      self._onScrollHandler = RN.event(items1, obj);
    }
  };
  items[8] = {
  ...
  items[13] = {
    ...
    value: function _shouldUseCustomAnimation() {
      const self = this;
      const tmp = !this.props.activeAnimationOptions;
      let tmp2 = !tmp;
      if (!tmp) {
          ...
          if (self._enableLoop()) {
            ...
            if (loopClonesPerSide > length) {
              ...
              for (let num = 0; num < rounded; num = num + 1) {
                let push = items.push;
                let tmp5 = callback2;
                let applyResult = push.apply(items, callback2(data));
                let push2 = items1.push;
                let applyResult1 = push2.apply(items1, callback2(data));
              }
              const unshift = items.unshift;
              unshift.apply(items, callback2(data.slice(-result)));
              const push3 = items1.push;
              push3.apply(items1, callback2(data.slice(0, result)));
              let substr = items;
              let substr1 = items1;
            } else {
        ...
        if (length) {
          if (_getActiveItemResult >= length + loopClonesPerSide) {
            ...
            if (loopClonesPerSide > length) {
              ...
              unshift.apply(items1, callback2(items.slice(-result1)));
              ...
              if (self._shouldAnimateSlides(props)) {
                if (self._shouldUseCustomAnimation()) {
                  ...
                  if (callback) {
                    ...
                    result = self(callback[12]).stackScrollInterpolator(_getCustomIndexResult, props);
                    const obj3 = self(callback[12]);
                  } else if (self._shouldUseTinderLayout()) {
                    result = self(callback[12]).tinderScrollInterpolator(_getCustomIndexResult, props);
                    const obj2 = self(callback[12]);
                  }
                  ...
                  if (!outputRange) {
                    result = self(callback[12]).defaultScrollInterpolator(_getCustomIndexResult, props);
                    const obj4 = self(callback[12]);
                  }
  ...
  items[33] = {
    ...
    value: function _getSlideAnimation(_activeItem, toValue) {
      ...
      let obj = { toValue };
      const merged = Object.assign({ "Bool(true)": null, "Bool(true)": null }, activeAnimationOptions, obj);
      obj = { easing: linear.linear };
      ({ parallel, timing } = closure_10);
      const items = [timing(tmp, Object.assign({}, merged, obj)), closure_10[activeAnimationType](tmp, Object.assign({}, merged))];
  ...
  items[34] = {
    ...
    value: function _playCustomSlideAnimation(_activeItem, _getActiveItemResult) {
      ...
      const _getDataIndexResult1 = this._getDataIndex(this._getCustomIndex(_getActiveItemResult));
      RN.parallel(items, { stopTogether: false }).start();
    }
  };
  items[35] = {
      ...
      if (self._mounted) {
        if (self._carouselRef) {
          if (self._positions[_activeItem]) {
            if (tmp) {
              const tmp2 = self._positions[_activeItem] && self._positions[_activeItem].start;
              if (tmp2) {
                let length = data;
                if (data) {
                  length = data.length;
  ...
  items[40] = {
    ...
    value: function _onScroll(nativeEvent) {
      ...
      if (enableMomentum) {
        ...
        const tmp5 = self._activeItem !== _getActiveItemResult && tmp2;
      }
      if (tmp12) {
  ...
  items[44] = {
    ...
    value: function _onScrollBeginDrag(nativeEvent) {
      const self = this;
      const onScrollBeginDrag = this.props.onScrollBeginDrag;
      if (this._getScrollEnabled()) {
        self._scrollStartOffset = self._getScrollOffset(nativeEvent);
  ...
  items[45] = {
    ...
    value: function _onScrollEndDrag(arg0) {
      const self = this;
      ...
      if (onScrollEndDrag) {
        onScrollEndDrag(arg0);
      }
    }
  };
  items[46] = {
    key: "_onMomentumScrollEnd",
    value: function _onMomentumScrollEnd(nativeEvent) {
      const self = this;
  ...
  items[47] = {
    ...
    value: function _onScrollEnd(arg0) {
      ...
      if (this._ignoreNextMomentum) {
        ...
        const tmp4 = self._autoplay && !self._autoplaying;
      }
    }
  };
  items[48] = {
  ...
  items[51] = {
    ...
    value: function _snapToItem(_scrollEndActive, first, arg2, arg3, arg4) {
      ...
      let self = this;
      let closure_0 = _scrollEndActive;
      self = this;
      const tmp = arguments.length > 1 && undefined !== arguments[1];
      let tmp2 = !tmp;
      if (tmp) {
      ...
      if (_getWrappedRefResult) {
        if (num) {
          if (num >= 0) {
            ...
            const tmp10 = _getCustomDataLengthResult > 0 && num >= _getCustomDataLengthResult;
          }
            ...
            if (enableMomentum) {
              ...
              if (!tmp14) {
                ...
                self._edgeItemTimeout = setTimeout(() => {
                  let _onScrollTriggered = tmp5;
                  if (!tmp5) {
                    _onScrollTriggered = closure_0 !== self._activeItem;
                  }
              ...
              const tmp14 = 0 !== num && num !== _getCustomDataLengthResult - 1;
            }
          }
        }
        closure_0 = 0;
      ...
      if (!this._autoplaying) {
        ...
        self._autoplayTimeout = setTimeout(() => {
          ...
          self._autoplayInterval = setInterval(() => {
            if (closure_0._autoplaying) {
              closure_0.snapToNext();
            }
          }, autoplayInterval);
        }, tmp);
      }
  ...
  items[62] = {
    ...
    value: function _renderItem(arg0) {
      ...
      if (self._needsScrollView()) {
        ...
        return callback6(View, Object.assign(obj2, {}, obj3));
      }
    }
  };
  items[63] = {
      ...
      if (props.data) {
        if (props.renderItem) {
          ...
          if (self._needsScrollView()) {
            ...
            tmp4Result = tmp4(closure_14, Object.assign({}, merged));
          }
          return tmp4Result;
        }
      }
...
let obj = { data: importDefault(dependencyMap[13]).array.isRequired, renderItem: importDefault(dependencyMap[13]).func.isRequired, itemWidth: importDefault(dependencyMap[13]).number, itemHeight: importDefault(dependencyMap[13]).number, sliderWidth: importDefault(dependencyMap[13]).number, sliderHeight: importDefault(dependencyMap[13]).number, activeAnimationType: importDefault(dependencyMap[13]).string, activeAnimationOptions: importDefault(dependencyMap[13]).object, activeSlideAlignment: importDefault(dependencyMap[13]).oneOf([null, null, null]), activeSlideOffset: importDefault(dependencyMap[13]).number, apparitionDelay: importDefault(dependencyMap[13]).number, autoplay: importDefault(dependencyMap[13]).bool, autoplayDelay: importDefault(dependencyMap[13]).number, autoplayInterval: importDefault(dependencyMap[13]).number, callbackOffsetMargin: importDefault(dependencyMap[13]).number, containerCustomStyle: arg1(dependencyMap[10]).ViewPropTypes.style, contentContainerCustomStyle: arg1(dependencyMap[10]).ViewPropTypes.style, enableMomentum: importDefault(dependencyMap[13]).bool, enableSnap: importDefault(dependencyMap[13]).bool, firstItem: importDefault(dependencyMap[13]).number, hasParallaxImages: importDefault(dependencyMap[13]).bool, inactiveSlideOpacity: importDefault(dependencyMap[13]).number, inactiveSlideScale: importDefault(dependencyMap[13]).number, inactiveSlideShift: importDefault(dependencyMap[13]).number };
const importDefaultResult1 = importDefault(dependencyMap[13]);
obj.layout = importDefault(dependencyMap[13]).oneOf([402653697, false, false]);
obj.layoutCardOffset = importDefault(dependencyMap[13]).number;
obj.lockScrollTimeoutDuration = importDefault(dependencyMap[13]).number;
obj.lockScrollWhileSnapping = importDefault(dependencyMap[13]).bool;
obj.loop = importDefault(dependencyMap[13]).bool;
obj.loopClonesPerSide = importDefault(dependencyMap[13]).number;
obj.scrollEnabled = importDefault(dependencyMap[13]).bool;
obj.scrollInterpolator = importDefault(dependencyMap[13]).func;
obj.slideInterpolatedStyle = importDefault(dependencyMap[13]).func;
obj.slideStyle = arg1(dependencyMap[10]).ViewPropTypes.style;
obj.shouldOptimizeUpdates = importDefault(dependencyMap[13]).bool;
obj.swipeThreshold = importDefault(dependencyMap[13]).number;
const importDefaultResult2 = importDefault(dependencyMap[13]);
const items = [importDefault(dependencyMap[13]).bool, importDefault(dependencyMap[13]).func];
obj.useScrollView = importDefault(dependencyMap[13]).oneOfType(items);
obj.vertical = importDefault(dependencyMap[13]).bool;
obj.onBeforeSnapToItem = importDefault(dependencyMap[13]).func;
obj.onSnapToItem = importDefault(dependencyMap[13]).func;
tmp5.propTypes = obj;
obj = { containerCustomStyle: {}, contentContainerCustomStyle: {}, slideStyle: {}, useScrollView: !animatedComponent, vertical: false };
