...
function hydrateArray(value) {
  ...
  if (0 < value.length) {
    do {
      let tmp = hydrateSingle;
      value[num] = hydrateSingle(value[num]);
      num = num + 1;
      length = value.length;
    } while (num < length);
  }
...
function hydrateSingle(value) {
  ...
  if ("string" === typeof value) {
    ...
    const first = _slicedToArray(value, 1)[0];
      ...
      if (obj.Number !== first) {
        if (obj.Date !== first) {
          if (obj.Time !== first) {
            if (obj.Select !== first) {
              if (obj.Plural !== first) {
                if (obj.Pound === first) {
                  ...
                  const tmp7 = _slicedToArray(value, 4);
                  [tmp8, tmp9] = tmp7;
                  hydrateArray(tmp7[2]);
                  if (null != tmp7[3]) {
                    hydrateArray(tmp11);
            ...
            const tmp19 = _slicedToArray(value, 5);
            [tmp20, tmp21] = tmp19;
            for (const key10044 in tmp22) {
              let tmp36 = key10044;
              let tmp37 = hydrateArray;
              let tmp38 = hydrateArray(tmp22[key10044]);
...
function compressFormatJsToAst(value) {
  if (Array.isArray(value)) {
    return value.map((arg0) => outer1_5(arg0));
  } else {
