...
// Function ID: 5537
// Name: map
// Dependencies: [32, 19, 5534, 5524, 5537, 5510, 5525, 5472]
...
function isGestureEnabled(closure_0) {
  if (obj.isComposedGesture(closure_0)) {
    ...
    someResult = false !== require(5524) /* hash */.maybeUnpackValue(closure_0.config.enabled);
    const tmpResult = require(5524) /* hash */;
  }
  return someResult;
}
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = require(5537) /* isNativeEvent */.isNativeAnimatedEvent(useAnimated.onUpdate);
    const obj = require(5537) /* isNativeEvent */;
  }
  ...
  if (!disableReanimated) {
    result = undefined !== require(5510).Reanimated;
  }
  if (result) {
    result = require(5524) /* hash */.hasWorkletEventHandlers(useAnimated);
    const obj2 = require(5524) /* hash */;
  }
  if (result) {
    result = !useAnimated.dispatchesAnimatedEvents;
  }
  useAnimated.shouldUseReanimatedDetector = result;
  useAnimated.needsPointerData = require(5537) /* isNativeEvent */.shouldHandleTouchEvents(useAnimated);
};
export const prepareConfigForNativeSide = function prepareConfigForNativeSide(closure_0, closure_1) {
  ...
  const PropsWhiteLists = require(5525) /* items2 */.PropsWhiteLists;
  let EMPTY_WHITE_LIST = PropsWhiteLists.get(closure_0);
  if (EMPTY_WHITE_LIST == null) {
    EMPTY_WHITE_LIST = require(5525) /* items2 */.EMPTY_WHITE_LIST;
  }
  ...
  while (tmp12 !== undefined) {
    ...
    let allowedNativeProps = require(5525) /* items2 */.allowedNativeProps;
