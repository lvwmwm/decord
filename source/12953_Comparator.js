...
// Function ID: 12954
// Name: Comparator
// Dependencies: [41, 42, 12926, 12928, 12929, 12925, 12951, 12954]
...
class Comparator {
  constructor(arg0, arg1) {
    ...
    parts = str2.split(/\s+/);
    joined = parts.join(" ");
    tmp6 = require("module_12928")("comparator", joined, tmp4);
    self.options = tmp4;
    ...
    tmp8 = require("module_12928")("comp", self);
    return;
  }
}
let obj = {
  ...
  value: function parse(str) {
    const self = this;
    const safeRe = Comparator(12929).safeRe;
    const t = Comparator(12929).t;
    ...
    if (match) {
      ...
      if (match[2]) {
        const tmp15 = new tmp5(12925)(match[2], self.options.loose);
        self.semver = tmp15;
      } else {
...
const items = [
  ...
  },
  {
    ...
    value: function test(arg0) {
      const self = this;
      let tmp = arg0;
      Comparator(12928)("Comparator.test", arg0, this.options.loose);
      if (this.semver !== closure_3) {
        if (tmp !== tmp5) {
          if (typeof tmp !== "__FORMATJS_LISTFORMAT_DATA__") {
            try {
              tmp = new tmp2(12925)(tmp, self.options);
            } catch (err) {
              return false;
            }
          }
          return tmp2(12951)(tmp, self.operator, self.semver, self.options);
        }
      }
      return true;
    }
    ...
    value: function intersects(value) {
      if (value instanceof Comparator) {
        ...
        if ("" === this.operator) {
          ...
          if (!isMatch) {
            const obj2 = new Comparator(12954)(value.value, arg1);
          ...
          if (!isMatch1) {
            const obj = new Comparator(12954)(self.value, arg1);
            isMatch1 = obj.test(value.semver);
          }
          tmp8 = isMatch1;
        } else {
          const tmp45 = Comparator(12926)(arg1);
            ...
            if (!tmp9) {
              ...
              if (tmp13) {
                ...
                if (tmp16) {
                  ...
                  let tmp19 = !tmp18;
                  if (tmp18) {
                    let startsWithResult3 = Comparator(12951)(self.semver, "<", value.semver, tmp45);
                    ...
                    if (!tmp24) {
                      let startsWithResult4 = Comparator(12951)(self.semver, ">", value.semver, tmp45);
