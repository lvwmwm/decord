...
// Function ID: 4914
// Name: load
// Dependencies: [4914, 4915, 4916, 4917, 4918, 4931, 4933, 4934, 4946, 4950, 4954, 4956, 4958, 4959, 4960, 4961, 4962, 4963, 4964, 4965, 4966, 4967, 4940]
// Exports: load

// Module 4913 (load)
import errorDefault from "error" /* 4914 */;
  ...
  if (typeof response === "string") {
    ...
    if (obj1.test(response)) {
      ...
      if (obj2.test(response)) {
        let resolved = Promise.resolve(response(4915).dataUriToBuffer(response));
        const obj5 = response(4915);
      } else {
        obj2 = then;
        if (then === undefined) {
          obj2 = {};
