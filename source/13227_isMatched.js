...
// Function ID: 101838
// Name: isMatched
// Dependencies: [13228, 1257, 13229]

// Module 13227 (isMatched)
const require = arg1;
  ...
  if (parts[2]) {
    if ("$" === arr[0]) {
      ...
      const mapped = arr2.map((arg0) => {
        let tmp = outer1_0(outer1_1[2]).regions[arg0];
        if (!tmp) {
          const items = [arg0];
          tmp = items;
        }
      ...
      const reduced = mapped.reduce((items, items) => {
        ...
        return obj.__spreadArray(outer1_0(outer1_1[1]).__spreadArray([], items, true), items, true);
      }, []);
...
function findMatchingDistance(arg0, arg1) {
  ...
  const tmp7 = (function processData() {
    if (!outer1_2) {
      let first = outer1_0(outer1_1[0]).data.supplemental.languageMatching["written-new"][0];
      let paradigmLocales;
      if (null !== first) {
        if (undefined !== first) {
      ...
      const prop = outer1_0(outer1_1[0]).data.supplemental.languageMatching["written-new"];
      let substr = prop.slice(1, 5);
      const prop1 = outer1_0(outer1_1[0]).data.supplemental.languageMatching["written-new"];
      ...
      obj.paradigmLocales = obj2.__spreadArray(outer1_0(outer1_1[1]).__spreadArray([], parts, true), parts.map((str) => {
        const locale = new Intl.Locale(str.replace(/_/g, "-"));
        return locale.maximize().toString();
      }), true);
      outer1_2 = obj;
      const __spreadArrayResult = outer1_0(outer1_1[1]).__spreadArray([], parts, true);
    }
    return outer1_2;
  })();
  let num = 0;
  if (obj.language !== obj.language) {
    ...
    let obj2 = { language: maximizeResult1.language, script: "", region: "" };
    num = findMatchingDistanceForLSR(obj1, obj2, tmp7);
  }
  let sum = num;
  if (obj.script !== obj.script) {
    let obj3 = { language: maximizeResult.language, script: obj.script, region: "" };
...
arg5.findBestMatch = function findBestMatch(arr) {
  ...
  let item = arr.forEach((arg0, arg1) => {
    ...
    const item = closure_0.forEach((matchedSupportedLocale) => {
      const sum = outer2_7(closure_0, matchedSupportedLocale) + 40 * closure_1;
      outer1_2.distances[closure_0][matchedSupportedLocale] = sum;
      if (sum < closure_1) {
        closure_1 = sum;
        outer1_2.matchedDesiredLocale = closure_0;
        outer1_2.matchedSupportedLocale = matchedSupportedLocale;
      }
    });
  });
  if (Infinity >= tmp) {
