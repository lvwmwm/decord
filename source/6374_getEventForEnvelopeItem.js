...
// Module 6374 (getEventForEnvelopeItem)
const require = arg1;
...
arg5.createTransport = function createTransport(bufferSize) {
  ...
  if (arguments.length > 2) {
    ...
    let obj = {
      send(arg0) {
          let items = [];
          bufferSize(6355).forEachEnvelopeItem(arg0, (arg0, arg1) => {
            const result = items(6355).envelopeItemTypeToDataCategory(arg1);
            const obj = items(6355);
          ...
          if (0 === items.length) {
            return bufferSize(6335).resolvedSyncPromise({});
          } else {
            const dependencyMap = bufferSize(6355).createEnvelope(arg0[0], items);
            function recordEnvelopeLoss(arg0) {
              const items = arg0;
              items(6355).forEachEnvelopeItem(dependencyMap, (arg0, arg1) => {
            ...
            let obj2 = bufferSize(6355);
            return recordEnvelopeLoss.add(function requestTask() {
              const obj = { body: items(6355).serializeEnvelope(dependencyMap) };
              const obj2 = items(6355);
              return dependencyMap(obj).then((statusCode) => {
                ...
                if (DEBUG_BUILD) {
                  DEBUG_BUILD = callback(6339).DEBUG_BUILD;
                }
                if (DEBUG_BUILD) {
                  const logger = callback(6311).logger;
                  const _HermesInternal = HermesInternal;
                  logger.warn("Sentry responded with status code " + statusCode.statusCode + " to sent event.");
                }
                outer2_3 = callback(6376).updateRateLimits(outer2_3, statusCode);
                return statusCode;
              }, (arg0) => {
              ...
              if (arg0 instanceof items(6368).SentryError) {
                if (items(6339).DEBUG_BUILD) {
                  const logger = items(6311).logger;
                  logger.error("Skipped sending event because buffer is full.");
                }
                recordEnvelopeLoss("queue_overflow");
                return items(6335).resolvedSyncPromise({});
              } else {
                throw arg0;
              }
            });
          }
          let obj = bufferSize(6355);
        },
      flush(arg0) {
