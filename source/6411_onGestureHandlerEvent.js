...
// Dependencies: [17, 6412, 6414, 6415, 6416]
// Exports: startListening, stopListening

// Module 6411 (onGestureHandlerEvent)
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import map2 from "map" /* 6412 */;
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6414).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6414).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6414).State.BEGAN) {
        if (handlerTag.state === tmp(6414).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6414).State.END) {
          if (handlerTag.oldState === tmp(6414).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6414).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6414).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6415).GestureStateManager;
      ...
      if (tmp(6416).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6416).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6416).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6416).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6412).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
