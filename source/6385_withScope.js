...
// Function ID: 6386
// Name: withScope
// Dependencies: [41, 42, 6386, 6372, 6383, 6391]
// Exports: getStackAsyncContextStrategy

// Module 6385 (withScope)
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(6383).getMainCarrier();
  const obj = AsyncContextStack(6383);
  const sentryCarrier = AsyncContextStack(6383).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(6383).getMainCarrier();
      let obj = callback(6383);
      let sentryCarrier = callback(6383).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(6391);
    ...
    getCurrentScope() {
      const mainCarrier = callback(6383).getMainCarrier();
      const obj = callback(6383);
      const sentryCarrier = callback(6383).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(6391);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
