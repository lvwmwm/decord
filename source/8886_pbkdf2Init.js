...
// Function ID: 8887
// Name: pbkdf2Init
// Dependencies: [5, 8884, 8883, 8887]
// Exports: pbkdf2, pbkdf2Async

// Module 8886 (pbkdf2Init)
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  let c;
  let dkLen;
  require(8884) /* number */.hash(sha256);
  ({ c, dkLen, asyncTick } = require(8883) /* _asyncLoop */.checkOpts({ dkLen: 32, asyncTick: 10 }, arg3));
  require(8884) /* number */.number(c);
  require(8884) /* number */.number(dkLen);
  require(8884) /* number */.number(asyncTick);
  if (c < 1) {
    const _Error = Error;
    ...
    const toBytesResult = tmp(8883).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(8887).hmac;
    let obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(8883).toBytes(B);
  ...
  const checkOptsResult = require(8883) /* _asyncLoop */.checkOpts({ dkLen: 32, asyncTick: 10 }, arg3);
}
...
export const pbkdf2 = function pbkdf2(sha256, B, B, arg3) {
  ...
  const uint8Array = new Uint8Array(4);
  const view = require(8883) /* _asyncLoop */.createView(uint8Array);
