...
// Function ID: 6623
// Name: extractProps
// Dependencies: [6623, 6614, 6624, 6627, 6615, 6613]

// Module 6622 (extractProps)
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(6614)(obj, markerMid, arg1);
  module(6624)(obj, markerMid, items);
  module(6627)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(6615)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(6613)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
