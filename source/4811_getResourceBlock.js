...
function readTag(getUint8, offset) {
  if (getUint8.getUint8(offset) !== 28) {
    ...
    obj = { id: uint16, name: getTagName(module(4812).iptc[uint16], uint16, items), value: items, description: getTagDescription(module(4812).iptc[uint16], items, arg2, arg3) };
    let repeatable = module(4812).iptc[uint16];
    if (repeatable) {
      repeatable = module(4812).iptc[uint16].repeatable;
    }
    if (repeatable) {
      obj.repeatable = true;
    }
    let tmp16 = module(4812).iptc[uint16];
    if (tmp16) {
      tmp16 = undefined !== module(4812).iptc[uint16].encoding_name;
    }
    if (tmp16) {
      obj.encoding = module(4812).iptc[uint16].encoding_name(items);
      const obj3 = module(4812).iptc[uint16];
    }
    obj = {};
    obj.tag = obj;
    obj.tagSize = uint161;
