...
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 4972, 5008, 5009, 5010]

// Module 5007 (_isNativeReflectConstruct)
...
function getDescription(arr) {
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      tmp12 = tmp11;
      if (typeof importDefault(5010)[tmp2] === "function") {
    ...
    try {
      if (tmp2) {
        if (typeof importDefault(5010)[tmp2] === "function") {
          let decodeURIComponentResult = importDefault(5010)[tmp2](arr);
          const tmp4Result = importDefault(5010);
        }
