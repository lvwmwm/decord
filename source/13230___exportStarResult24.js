...
// Dependencies: [1257, 13203, 13225, 13204, 13218, 13220]
// Exports: PartitionNumberPattern

// Module 13230 (__exportStarResult24)
...
export const PartitionNumberPattern = function PartitionNumberPattern(internalSlots, isNaN) {
  ...
  if (isNaN.isNaN()) {
    ...
    let num6 = 0;
    if (!isNaN.isZero()) {
      require(13204) /* setInternalSlot */.invariant(isNaN.isFinite(), "Input must be a mathematical value");
      let timesResult = isNaN;
      if ("percent" == internalSlots.style) {
        timesResult = isNaN.times(100);
      }
      const ComputeExponentResult = require(13218) /* __exportStarResult13 */.ComputeExponent(internalSlots, timesResult);
    ...
    const result = require(13220) /* __exportStarResult21 */.FormatNumericToString(internalSlots, timesResult1);
  ...
  if ("never" !== signDisplay) {
    if ("auto" === signDisplay) {
      ...
      require(13204) /* setInternalSlot */.invariant("negative" === signDisplay, "signDisplay must be \"negative\"");
      let num15 = 0;
      if (roundedNumber.isNegative()) {
        num15 = 0;
        if (!roundedNumber.isZero()) {
