...
// Dependencies: [7499, 7546, 7543, 7600]

// Module 7602 (createMetricEnvelope)
const require = arg1;
...
function createMetricEnvelope(arg0, arg1, sdk) {
  ...
  if (tmp) {
    obj.dsn = require(7546) /* dsnFromString */.dsnToString(arg1);
    const obj4 = require(7546) /* dsnFromString */;
  }
  const date = new Date();
  const result = require(7600) /* items */.serializeMetricBuckets(arg0);
  obj = { type: "statsd", length: result.length };
  const items = [obj, result];
  const obj5 = require(7600) /* items */;
  const items1 = [items];
...
arg5.captureAggregateMetrics = function captureAggregateMetrics(_client, arr) {
  const logger = require(7499) /* consoleSandbox */.logger;
