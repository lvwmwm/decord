...
// Dependencies: [41, 42, 6375, 6379, 6380, 6370, 6403, 6371, 6381, 6407, 6393, 6365, 6392, 6408, 6394, 6401]

// Module 6406 (isFullFinishedSpan)
...
class SentrySpan {
  constructor() {
    ...
    obj[require("module_6380").SEMANTIC_ATTRIBUTE_SENTRY_OP] = obj.op;
...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(6370).spanTimeInputToSeconds(arg0);
    }
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(6380).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    }
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(6370).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(6370);
        SentrySpan(6403).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(6403);
      }
    }
  },
  {
    ...
    value: function getSpanJSON() {
      let _attributes;
      const self = this;
      let obj = SentrySpan(6371);
      obj = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 6380).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: null, timestamp: null, trace_id: null, origin: null, _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      obj[6] = SentrySpan(6370).getStatusMessage(this._status);
      ({ _endTime: obj2[7], _traceId: obj2[8], _attributes } = this);
      obj[9] = _attributes[SentrySpan(undefined, 6380).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
      const obj3 = SentrySpan(6370);
      obj[10] = SentrySpan(6381).getMetricSummaryJsonForSpan(this);
      obj[11] = this._attributes[SentrySpan(undefined, 6380).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj[12] = this._attributes[SentrySpan(undefined, 6380).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(6381);
      obj[13] = SentrySpan(6407).timedEventsToMeasurements(this._events);
  ...
  },
  {
    ...
    value: function addEvent(name) {
      if (SentrySpan(6393).DEBUG_BUILD) {
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(6392).getClient();
      if (client) {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          ...
          if (result) {
            let scope = tmp(6394).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(6392).getCurrentScope();
              const tmpResult2 = tmp(6392);
            }
            scope.captureEvent(result);
            const tmpResult1 = tmp(6394);
          }
        }
      } else {
  ...
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        ...
        if (!client) {
          client = tmp3(6392).getClient();
          const tmp3Result1 = tmp3(6392);
        }
        if (true !== self._sampled) {
          if (tmp3(6393).DEBUG_BUILD) {
            const logger3 = tmp3(6365).logger;
          ...
          const spanDescendants = tmp3(6370).getSpanDescendants(self);
          ...
          const mapped = found.map((arg0) => self(6370).spanToJSON(arg0));
          const found1 = mapped.filter(isFullFinishedSpan);
          const tmp24 = self._attributes[tmp3(undefined, 6380).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
          const _attributes = self._attributes;
          let SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = tmp3(6380).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
          delete tmp2[tmp];
          const item = found1.forEach((data) => {
            if (data.data) {
              data = data.data;
              const SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = self(6380).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
              delete tmp2[tmp];
            }
          });
          ...
          obj[0] = tmp3(6370).spanToTransactionTraceContext(self);
          obj[0] = obj;
          let substr = found1;
          ...
          const tmp3Result3 = tmp3(6370);
          const obj2 = { dynamicSamplingContext: null };
          const tmp3Result4 = tmp3(6371);
          obj2[0] = tmp3(6401).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result4.dropUndefinedKeys(obj2));
          obj[6] = obj1;
          const tmp3Result5 = tmp3(6401);
          obj[7] = tmp3(6381).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
          ...
          const tmp3Result6 = tmp3(6381);
          const result = tmp3(6407).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
          ...
          if (length) {
            if (tmp3(6393).DEBUG_BUILD) {
              const logger2 = tmp3(6365).logger;
              const _JSON = JSON;
