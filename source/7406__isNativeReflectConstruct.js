...
// Dependencies: [32, 41, 42, 93, 95, 96, 98, 7346, 7361, 7343, 7407, 7371, 7374, 7375, 7347, 7400, 7358, 7408, 7369, 7352, 7383, 7402]

// Module 7406 (_isNativeReflectConstruct)
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7407).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7407);
      return ServerRuntimeClient(7371).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7371);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7407).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7374).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7374);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7400).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7375).DEBUG_BUILD) {
        const logger = tmp2(7347).logger;
    ...
    value: function captureCheckIn(checkInId) {
      ...
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7408);
          ...
          if (ServerRuntimeClient(7375).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7347).logger;
          ...
          if (ServerRuntimeClient(7375).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7347).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7358);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7369)._getSpanForScope(arg0);
        ...
        const tmpResult1 = ServerRuntimeClient(7383);
        if (_getSpanForScopeResult) {
