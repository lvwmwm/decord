...
// Function ID: 5387
// Name: getHandler
// Dependencies: [5378, 5347, 5343, 5346, 5344, 5345]

// Module 5386 (getHandler)
const require = arg1;
const dependencyMap = arg6;
function getHandler(arg0, onBegin) {
  if (require(5378) /* _isNativeReflectConstruct */.CALLBACK_TYPE.BEGAN === arg0) {
    return onBegin.onBegin;
  } else if (tmp(5378).CALLBACK_TYPE.START === arg0) {
    return onBegin.onStart;
  } else if (tmp(5378).CALLBACK_TYPE.UPDATE === arg0) {
    return onBegin.onUpdate;
  } else if (tmp(5378).CALLBACK_TYPE.CHANGE === arg0) {
    return onBegin.onChange;
  } else if (tmp(5378).CALLBACK_TYPE.END === arg0) {
    return onBegin.onEnd;
  } else if (tmp(5378).CALLBACK_TYPE.FINALIZE === arg0) {
    return onBegin.onFinalize;
  } else if (tmp(5378).CALLBACK_TYPE.TOUCHES_DOWN === arg0) {
    return onBegin.onTouchesDown;
  } else if (tmp(5378).CALLBACK_TYPE.TOUCHES_MOVE === arg0) {
    return onBegin.onTouchesMove;
  } else if (tmp(5378).CALLBACK_TYPE.TOUCHES_UP === arg0) {
    return onBegin.onTouchesUp;
  } else if (tmp(5378).CALLBACK_TYPE.TOUCHES_CANCELLED === arg0) {
...
function touchEventTypeToCallbackType(arg0) {
  if (require(5347).TouchEventType.TOUCHES_DOWN === arg0) {
    return tmp(5378).CALLBACK_TYPE.TOUCHES_DOWN;
  } else if (tmp(5347).TouchEventType.TOUCHES_MOVE === arg0) {
    return tmp(5378).CALLBACK_TYPE.TOUCHES_MOVE;
  } else if (tmp(5347).TouchEventType.TOUCHES_UP === arg0) {
    return tmp(5378).CALLBACK_TYPE.TOUCHES_UP;
  } else if (tmp(5347).TouchEventType.TOUCHES_CANCELLED === arg0) {
    return tmp(5378).CALLBACK_TYPE.TOUCHES_CANCELLED;
  } else {
    return tmp(5378).CALLBACK_TYPE.UNDEFINED;
  }
}
obj = { TouchEventType: require("module_5347").TouchEventType, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
...
function runWorklet(arg0, arg1, arg2) {
  ...
  if (arg1.isWorklet[arg0]) {
    ...
    console.warn(require(5343) /* toArray */.tagMessage("Animated gesture callback must be a worklet"));
    const obj = require(5343) /* toArray */;
  }
}
