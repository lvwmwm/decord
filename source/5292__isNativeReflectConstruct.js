...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 5293, 5277, 5282, 5278, 5294, 5276, 5295, 5296, 5297, 5298, 5299, 5288]
// Exports: default

// Module 5292 (_isNativeReflectConstruct)
...
UIManager.setJSResponder = (D, __getInternalProperties) => {
  importDefault(5282).handleSetJSResponder(D, __getInternalProperties);
  fn(D, __getInternalProperties);
};
UIManager.clearJSResponder = () => {
  const result = importDefault(5282).handleClearJSResponder();
  fn2();
};
let closure_12 = { [arg1(5278).State.UNDETERMINED]: undefined, [arg1(5278).State.BEGAN]: "onBegan", [arg1(5278).State.FAILED]: "onFailed", [arg1(5278).State.CANCELLED]: "onCancelled", [arg1(5278).State.ACTIVE]: "onActivated", [arg1(5278).State.END]: "onEnded" };
  ...
  let items = [
    {
      ...
      value: function componentDidMount() {
        ...
        if (someResult) {
          if (someResult) {
            require(5297).ghQueueMicrotask(() => {
              self.update(1);
            });
            const obj = require(5297);
          }
          const props2 = self.props;
          const items = [];
          const obj2 = require(5296) /* transformIntoHandlerTags */;
          ...
          if (self.viewNode) {
            self.attachGestureHandler(importDefault(5299)(self.viewNode));
          } else {
            const _Error = Error;
