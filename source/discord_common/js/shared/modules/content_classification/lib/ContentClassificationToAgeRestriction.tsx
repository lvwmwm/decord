...
// Dependencies: [5075, 5076, 503, 5077, 5078, 5079, 5080, 5081, 5082, 2]
// Exports: contentClassificationToAgeRestriction

// Module 5074 (contentClassificationToAgeRestrictionConclusion)
import set from "set" /* 2 */;
import fromStringAll from "fromString" /* 503 */;
import AgeRestrictionSource from "AgeRestrictionSource" /* 5075 */;
import AgeRestrictionStatus8 from "AgeRestrictionStatus" /* 5076 */;
import frozen from "frozen" /* 5077 */;
import set2 from "set" /* 5078 */;
import set3 from "set" /* 5079 */;
import set4 from "set" /* 5080 */;
    ...
    if (data.type === obj.MINIMAL) {
      if (null != data.discord_classifications) {
        ...
        if (obj15.has(deserializeResult, frozen.DiscordContentClassificationFlags.EMERGENCY_ONLY_USE_IF_YOU_HAVE_TO_FORCE_MARK_AGE_RESTRICTED)) {
          ADULT = tmp28(5076).AgeRestrictionStatus.ADULT;
          obj[1] = ADULT;
          let tmp12 = obj;
        } else {
          const tmp29Result = tmp29(503);
          const AgeRestrictionStatus3 = tmp28(5076).AgeRestrictionStatus;
          obj[1] = tmp29(503).hasAny(deserializeResult, tmp28(5077).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT) ? AgeRestrictionStatus3.ADULT : AgeRestrictionStatus3.EVERYONE;
          tmp12 = obj;
          const hasAnyResult = tmp29(503).hasAny(deserializeResult, tmp28(5077).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT);
        }
        items.push(tmp12);
        obj15 = fromStringAll;
      }
      ...
      if (obj2.has(deserializeResult1, frozen.DiscordContentClassificationFlags.EMERGENCY_ONLY_USE_IF_YOU_HAVE_TO_FORCE_MARK_AGE_RESTRICTED)) {
        obj[1] = tmp4(5076).AgeRestrictionStatus.ADULT;
        let tmp9 = obj;
      } else {
        const tmp6Result = tmp6(503);
        const AgeRestrictionStatus2 = tmp4(5076).AgeRestrictionStatus;
        obj[1] = tmp6(503).hasAny(deserializeResult1, tmp4(5077).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT) ? AgeRestrictionStatus2.ADULT : AgeRestrictionStatus2.EVERYONE;
        tmp9 = obj;
        const hasAnyResult1 = tmp6(503).hasAny(deserializeResult1, tmp4(5077).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT);
      }
      items.push(tmp9);
      tmp6 = importAll;
      ...
      if (obj12.has(deserializeResult2, frozen.DiscordContentClassificationFlags.EMERGENCY_ONLY_USE_IF_YOU_HAVE_TO_FORCE_MARK_AGE_RESTRICTED)) {
        obj1[1] = tmp24(5076).AgeRestrictionStatus.ADULT;
        let tmp2 = obj1;
      } else {
        const tmp26Result = tmp26(503);
        const AgeRestrictionStatus = tmp24(5076).AgeRestrictionStatus;
        obj1[1] = tmp26(503).hasAny(deserializeResult2, tmp24(5077).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT) ? AgeRestrictionStatus.ADULT : AgeRestrictionStatus.EVERYONE;
        tmp2 = obj1;
        const hasAnyResult2 = tmp26(503).hasAny(deserializeResult2, tmp24(5077).DiscordContentClassificationFlagMasks.RESTRICTED_TO_ADULT);
      }
      items.push(tmp2);
      obj12 = fromStringAll;
    }
        ...
        if (null == data.agency_ratings.pegi) {
          if (null == data.agency_ratings.gop) {
            if (null != data.agency_ratings.igdb) {
              ...
              obj2[0] = tmp21(5075).AgeRestrictionSource.AGENCY_CLASSIFICATION_IGDB;
              obj2[1] = EVERYONE;
              AgeRestrictionStatus7 = items.push(obj2);
              someResult = themes.some((arg0) => {
                const ADULT_THEMES = callback(5081).ContentRatingIGDBThemeSets.ADULT_THEMES;
