...
// Dependencies: [12269, 12270, 2]
// Exports: bindResolveFieldValue

// Module 12268 (resolveFieldValue)
function resolveFieldValue(image, items, applicationAssets) {
  ...
  if (null == image) {
    return null;
  } else if (image.value_type === _require(12270).ApplicationWidgetFieldValueType.DATA) {
    ...
    return tmp10;
  } else if (image.value_type === tmp19(12270).ApplicationWidgetFieldValueType.CUSTOM_STRING) {
    let tmp6 = null;
    if (image.presentation_type === tmp19(12269).ApplicationWidgetFieldPresentationType.TEXT) {
      tmp6 = null;
      if (items.includes(obj.STRING)) {
        const obj1 = { type: null, value: null, presentationType: null };
        obj1[0] = tmp7.STRING;
        obj1[1] = image.value;
        obj1[2] = tmp19(12269).ApplicationWidgetFieldPresentationType.TEXT;
        tmp6 = obj1;
      }
      tmp7 = obj;
    }
    return tmp6;
  } else if (image.value_type === tmp19(12270).ApplicationWidgetFieldValueType.APPLICATION_ASSET) {
    if (items.includes(obj.MEDIA)) {
      ...
      if (null != found) {
        ...
        obj[1] = obj2;
        obj[2] = tmp19(12269).ApplicationWidgetFieldPresentationType.IMAGE;
        tmp5 = obj;
      }
      return tmp5;
    } else {
...
let closure_3 = { [require(12269).ApplicationWidgetFieldPresentationType.TEXT]: items, [require(12269).ApplicationWidgetFieldPresentationType.NUMBER]: items1, [require(12269).ApplicationWidgetFieldPresentationType.IMAGE]: items2, [require(12269).ApplicationWidgetFieldPresentationType.DURATION]: items3 };
