...
// Dependencies: [32, 12633, 12716, 2]
// Exports: default

// Module 12715 (isVisualUnfurledMedia)
import resolveFieldValue from "resolveFieldValue" /* 12633 */;
import ProfileDataDynamicType from "ProfileDataDynamicType" /* 12716 */;
  ...
  if (null == dynamic) {
    ...
    while (iter2 !== undefined) {
      ...
      if (nextResult.type === ProfileDataDynamicType.ProfileDataDynamicType.STRING) {
        let tmp9 = nextResult;
        obj = { type: null, value: null };
        obj[0] = tmp5(12633).ResolvedValueType.STRING;
        obj[1] = iter.value;
        obj[iter.name] = obj;
      } else {
        let tmp11 = nextResult;
        if (iter.type === tmp5(12716).ProfileDataDynamicType.NUMBER) {
          let tmp8 = nextResult;
          obj = { type: null, value: null };
          obj[0] = tmp5(12633).ResolvedValueType.NUMBER;
          obj[1] = iter.value;
          obj[iter.name] = obj;
        } else {
          let tmp12 = nextResult;
          if (iter.type === tmp5(12716).ProfileDataDynamicType.MEDIA) {
            let tmp13 = isVisualUnfurledMedia;
            let tmp14 = nextResult;
            if (isVisualUnfurledMedia(iter.value)) {
              let tmp7 = nextResult;
              obj1 = { type: null, media: null };
              obj1[0] = tmp5(12633).ResolvedValueType.MEDIA;
              let obj2 = { url: null, width: null, height: null };
