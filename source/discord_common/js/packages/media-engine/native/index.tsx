...
// Function ID: 4549
// Name: destroy
// Dependencies: [32, 5, 4512, 4549, 4550, 4551, 4555, 4, 4501, 4547, 4556, 669, 501, 4558, 4618, 2]
...
import VideoDefault from "Video" /* 4551 */;
import createDefault from "create" /* 4558 */;
import sanitizeDevices from "sanitizeDevices" /* 4618 */;
...
class MediaEngineNative extends tmp4 {
  constructor() {
    ...
    obj.handleDeviceChange = function handleDeviceChange(items, items, items) {
      ...
      obj = obj(pollMetrics[14]);
      const sanitizeDevicesResult = obj.sanitizeDevices(closure_1_14.AUDIO_INPUT, items);
      const obj2 = obj(pollMetrics[14]);
      const sanitizeDevicesResult1 = obj(pollMetrics[14]).sanitizeDevices(closure_1_14.AUDIO_OUTPUT, items1);
      obj.emit(obj(pollMetrics[9]).MediaEngineEvent.DeviceChange, sanitizeDevicesResult, sanitizeDevicesResult1, obj(pollMetrics[14]).sanitizeDevices(closure_1_14.VIDEO_INPUT, items2));
    };
    obj.handleVolumeChange = function handleVolumeChange(arg0, arg1) {
      obj.emit(obj(pollMetrics[9]).MediaEngineEvent.VolumeChange, arg0 * closure_1_13, arg1 * closure_1_13);
    };
        ...
        if (c3 === 2) {
          ...
          try {
            ...
            if (0 === dependencyMap) {
              ...
              voiceEngine.emit(closure_1_0(4547).MediaEngineEvent.VoiceQueueMetrics, closure_1);
    ...
    if (tmp.ATTENUATION !== arg0) {
      if (tmp.VIDEO_HOOK !== arg0) {
        if (tmp.EXPERIMENTAL_SOUNDSHARE === arg0) {
          ...
          let isDesktopResult = PlatformTypes.isDesktop();
          if (isDesktopResult) {
            let tmp50Result = tmp50(4501);
            isDesktopResult = tmp50Result.supportsFeature(constants6.MEDIAPIPE);
          }
          if (!isDesktopResult) {
            tmp50Result = tmp50(4501);
...
prototype["connect"] = function connect(arg0, arg1, videoSupported) {
  ...
  obj = self(4558).create(arg0, arg1, videoSupported, flag);
  obj.on(obj(4547).BaseConnectionEvent.Destroy, (arg0) => {
    const connections = self.connections;
    connections.delete(arg0);
    if (self.connectionsEmpty()) {
      obj = obj(closure_1_2[8]);
      obj.setProcessPriority(closure_1_10.NORMAL);
      const voiceEngine = obj(closure_1_2[8]).getVoiceEngine();
  ...
  self.emit(obj(4547).MediaEngineEvent.Connection, obj);
  return obj;
};
...
prototype["getAudioInputDevices"] = function getAudioInputDevices() {
  return sanitizeDevices.getAudioInputDevices();
};
...
prototype["setAudioInputDevice"] = function setAudioInputDevice(audioInputDeviceId) {
  ...
  if (obj.supportsFeature(constants6.SET_AUDIO_DEVICE_BY_ID)) {
    ...
    audioInputDevices.then((arr) => {
      ...
      if (null != found) {
        const voiceEngine = audioInputDeviceId(closure_1_2[8]).getVoiceEngine();
        voiceEngine.setInputDevice(found.index);
        const obj = audioInputDeviceId(closure_1_2[8]);
      }
    });
  }
  self.emit(_require(4547).MediaEngineEvent.SelectedDeviceChange, constants3.AUDIO_INPUT, this.audioInputDeviceId, audioInputDeviceId);
};
...
prototype["setAudioOutputDevice"] = function setAudioOutputDevice(audioOutputDeviceId) {
  ...
  if (obj.supportsFeature(constants6.SET_AUDIO_DEVICE_BY_ID)) {
    ...
    audioOutputDevices.then((arr) => {
      ...
      if (null != found) {
        const voiceEngine = audioOutputDeviceId(closure_1_2[8]).getVoiceEngine();
        voiceEngine.setOutputDevice(found.index);
        const obj = audioOutputDeviceId(closure_1_2[8]);
      }
    });
  }
  self.emit(_require(4547).MediaEngineEvent.SelectedDeviceChange, constants3.AUDIO_OUTPUT, this.audioOutputDeviceId, audioOutputDeviceId);
};
      ...
      try {
        ...
        if (0 === c2) {
          if (arg0 === 1) {
            ...
            c3 = 1;
            obj1 = { value: null, done: false };
            obj1[0] = closure_1_1.getVideoInputDevices();
            return obj1;
          }
        } else if (arg0 === 1) {
          c3 = 3;
          ...
          if (id === closure_1_1.videoInputDeviceId) {
            ...
            if (!obj9.supportsFeature(closure_1_18.SET_VIDEO_DEVICE_BY_ID)) {
              ...
              const connections = closure_1_1.connections;
    ...
    if (null != voiceEngine.setOnClipsRecordingEvent) {
      if (null != voiceEngine.applyClipsSettings) {
        if (null != quality) {
          ...
          const first = tmp8[0];
          obj1 = { id: null, soundshareId: null };
          obj1[0] = tmp8[1];
          let num10 = 0;
...
prototype["getScreenPreviews"] = function getScreenPreviews(arg0, arg1, arg2) {
  ...
  let obj = _require(4501);
  return new Promise((arg0) => {
    const callback = arg0;
    if (null != obj.getVoiceEngine().getScreenPreviews) {
      const voiceEngine = callback(closure_1_2[8]).getVoiceEngine();
      ...
      const tmpResult = callback(closure_1_2[8]);
    } else {
      arg0([]);
    }
  });
...
prototype["registerClipsRecordingEventHandler"] = function registerClipsRecordingEventHandler() {
  let self = this;
  self = this;
  const voiceEngine = self(4501).getVoiceEngine();
  if (!tmp) {
    ...
    const result = voiceEngine.setOnClipsRecordingEvent((arg0, arg1) => {
      ...
      logger.info("Clips recording event: " + closure_1_12[arg0] + " received for stream " + id + " and sound " + soundshareId + ".");
      if (arg0 === closure_1_12.GoLiveEnded) {
        obj.emit(self(closure_1_2[9]).MediaEngineEvent.ClipsRecordingRestartNeeded);
        ...
        obj.emit(self(closure_1_2[9]).MediaEngineEvent.ClipsInitFailure, str2, clipsRecordingEventContext.applicationName);
      } else if (arg0 === tmp.IdleShutdown) {
        obj.emit(self(closure_1_2[9]).MediaEngineEvent.ClipsBridgeIdleShutdown);
      } else if (arg0 === tmp.RecordingHealthy) {
        obj.emit(self(closure_1_2[9]).MediaEngineEvent.ClipsRecordingHealthy);
      } else if (arg0 === tmp.RecordingActive) {
        obj.emit(self(closure_1_2[9]).MediaEngineEvent.ClipsRecordingReadyChanged, true);
      } else if (arg0 === tmp.RecordingInactive) {
        obj.emit(self(closure_1_2[9]).MediaEngineEvent.ClipsRecordingReadyChanged, false);
      } else {
        if (!tmp3) {
          obj.emit(self(closure_1_2[9]).MediaEngineEvent.ClipsRecordingEnded, id, soundshareId);
        }
...
prototype["hasSetClipsRecordingEnabled"] = function hasSetClipsRecordingEnabled() {
  return null != inject.getVoiceEngine().setClipsRecordingEnabled;
};
...
prototype["saveClipEx"] = function saveClipEx(arg0) {
  const _require = arg0;
  const voiceEngine = _require(4501).getVoiceEngine();
  let obj = _require(4501);
      ...
      if (tmp5) {
        ...
        if (tmp4) {
          if (null != obj.saveClipForUserWithTime) {
            ...
            if (obj3.supportsFeature(closure_1_18.CLIPS_THUMBNAIL)) {
          ...
          if (obj2.supportsFeature(closure_1_18.CLIPS_THUMBNAIL)) {
            if (saveClip != null) {
...
prototype["hasExportClipToFile"] = function hasExportClipToFile() {
  return null != inject.getVoiceEngine().exportClipToFile;
};
...
prototype["exportClipToFile"] = function exportClipToFile(arg0, arg1, arg2) {
  ...
  closure_1 = arg1;
  dependencyMap = arg2;
  const exportClipToFile = _require(4501).getVoiceEngine().exportClipToFile;
      ...
      try {
        ...
        if (0 === c1) {
          if (arg0 === 1) {
            ...
            let length = tmp2;
            length = undefined;
            const voiceEngine = closure_1_0(table[8]).getVoiceEngine();
            if (null != voiceEngine.setPreviewsUseWgc) {
              voiceEngine.setPreviewsUseWgc(closure_1_3);
            }
            ...
            if (null != tmp22Result.getVoiceEngine().getSingleWindowPreview) {
              ...
              table = 1;
              obj1 = { value: null, done: false };
              obj1[0] = voiceEngine1.getSingleWindowPreview(closure_1_0, c1, table);
              return obj1;
            } else {
              table = 3;
              return { value: null, done: true };
            }
            const obj9 = closure_1_0(table[8]);
          }
        } else if (arg0 === 1) {
          table = 3;
          throw arg1;
...
prototype["setLoopback"] = function setLoopback(arg0, arg1) {
  ...
  const self = this;
  tmp2Result = tmp2(4501);
  if (null != tmp2Result.getVoiceEngine().setEmitVADLevel2) {
    const voiceEngine1 = tmp2(4501).getVoiceEngine();
    if (!tmp) {
      tmp = self.listenerCount(tmp2(4547).MediaEngineEvent.VoiceActivity) > 0;
    }
    voiceEngine1.setEmitVADLevel2(tmp);
    const tmp2Result1 = tmp2(4501);
  } else {
    const voiceEngine2 = tmp2(4501).getVoiceEngine();
    let tmp6 = tmp;
    if (!tmp) {
      tmp6 = self.listenerCount(tmp2(4547).MediaEngineEvent.VoiceActivity) > 0;
    }
...
prototype["getCodecSurvey"] = function getCodecSurvey() {
  ...
  if (null != this.codecSurvey) {
    ...
    resolved = new Promise((arg0, arg1) => {
      closure_0 = arg0;
      const voiceEngine = self(closure_1_2[8]).getVoiceEngine();
      if (null != voiceEngine.getCodecSurvey) {
        ...
        const _Error = Error;
        error = new Error("getCodecSurvey is not implemented.");
        arg1(error);
      }
    });
  }
...
prototype["rankRtcRegions"] = function rankRtcRegions(arg0) {
  ...
  return new Promise((arg0, arg1) => {
    const callback = arg0;
    const rankRtcRegions = callback(closure_1_2[8]).getVoiceEngine().rankRtcRegions;
...
prototype["createReplayConnection"] = function createReplayConnection(arg0, arg1) {
  ...
  let tmp2 = null;
  if (null != replay) {
    replay.on(self(4547).BaseConnectionEvent.Destroy, (arg0) => {
      const connections = self.connections;
      connections.delete(arg0);
      if (self.connectionsEmpty()) {
        self(closure_1_2[8]).setProcessPriority(closure_1_10.NORMAL);
        const obj = self(closure_1_2[8]);
      }
    });
    let connections = self.connections;
    connections.add(replay);
    self(4501).setProcessPriority(constants.HIGH);
    self.emit(self(4547).MediaEngineEvent.Connection, replay);
    tmp2 = replay;
    const obj3 = self(4501);
  }
  return tmp2;
};
...
prototype["startLocalAudioRecording"] = function startLocalAudioRecording(arg0) {
  ...
  return new Promise((arg0, arg1) => {
    const callback = arg0;
    closure_1 = arg1;
    const startLocalAudioRecording = callback(closure_1_2[8]).getVoiceEngine().startLocalAudioRecording;
    if (null != startLocalAudioRecording) {
      const result = startLocalAudioRecording(callback, (arg0) => {
        if (arg0) {
          ...
          const _Error = Error;
          error = new Error("Failed to start local audio recording.");
          callback2(error);
        }
      });
    } else {
...
prototype["stopLocalAudioRecording"] = function stopLocalAudioRecording(arg0) {
  ...
  let tmp5 = this.listenerCount(tmp(4547).MediaEngineEvent.VoiceActivity) > 0;
  if (tmp5) {
    let tmpResult = tmp(4501);
...
prototype["getSystemMicrophoneMode"] = function getSystemMicrophoneMode() {
  return callback2(function*() {
    const obj2 = closure_1_0(table[8]);
...
prototype["watchdogTick"] = function watchdogTick() {
  ...
  const timerId = setTimeout(() => {
    if (!callback) {
      ...
      if (sum > 1) {
        self.emit(callback(closure_1_2[9]).MediaEngineEvent.WatchdogTimeout);
      }
    }
    self.watchdogTick();
  }, closure_19);
...
let result = require("set").fileFinishedImporting("../discord_common/js/packages/media-engine/native/index.tsx");
