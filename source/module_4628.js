// Module ID: 4628
// Function ID: 4629
// Dependencies: [109, 19, 17, 21, 4626, 4617, 4610, 4629, 4616, 4632, 4605]
...
export default importAllResult.forwardRef(function ScreenStackItem(unstable_sheetFooter, ref) {
  ...
  dependencyMap = flag4.useContext(screenId(4626).RNSScreensRefContext);
  const imperativeHandle = flag4.useImperativeHandle(ref, () => ref.current);
  let obj1 = screenId(4617);
  let flag;
  ...
  importDefault(4610)(flag5, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  obj[1] = callback(importDefault(4629), { contentStyle, style: container, stackPresentation, children });
  const items1 = [callback(screenId(4617).EdgeInsetApplicationContext.Provider, obj), , ];
  obj = {};
  const merged1 = Object.assign(headerConfig);
  items1[1] = callback(screenId(4616).ScreenStackHeaderConfig, obj);
  let tmp18Result = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp18Result) {
    obj1 = { children: null };
    obj1[0] = unstable_sheetFooter();
    tmp18Result = tmp18(tmp2(4632).FooterComponent, obj1);
  }
  items1[2] = tmp18Result;
  const tmp11 = importDefault(4610);
  ...
  return callback(importDefault(4605), obj2);
});
