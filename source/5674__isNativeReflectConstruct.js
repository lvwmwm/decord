...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 5675, 5645, 5690, 5677, 5692, 5688, 5693, 5678, 5694]
...
function getProgressFromGesture(interpolate, width, options) {
  ...
  if (typeof getDistanceFromOptions !== "error") {
  ...
  if (undefined !== gestureDirection) {
    const tmp10 = importDefault(5677)(obj, gestureDirection);
    if (tmp10 > 0) {
    ...
    let gestureDirection2 = CardStack(5690).ModalTransition.gestureDirection;
  } else {
    gestureDirection2 = CardStack(5690).DefaultTransition.gestureDirection;
  }
}
class CardStack {
  constructor(arg0) {
...
let obj = {
  ...
  value: function render() {
        ...
        if (undefined === detachPreviousScreen) {
          ...
          if (tmp7) {
            ...
            if (isModalPresentation) {
              ...
              tmp11 = diff !== importDefault(5692)(scenes, (descriptor) => {
    ...
    children[1] = closure_9(CardStack(5693).MaybeScreenContainer, obj2);
    let tmp23 = null;
    if (someResult) {
      tmp23 = tmp15Result;
    }
    children[2] = tmp23;
    return closure_10(CardStack(5645).Background, { children });
  }
};
let items = [obj];
obj = {
  ...
  value: function getDerivedStateFromProps(routes, routes2) {
    ...
    if (typeof getHeaderHeights !== "error") {
      HermesBuiltin.throwTypeError();
    }
