...
// Function ID: 7469
// Name: NativeModules
// Dependencies: [5, 17, 7469, 7470, 7475, 7476, 7477, 7478, 7479]
...
const subs = require("module_7469").ProductType.subs;
const inapp = require("module_7469").ProductType.inapp;
...
export const isIosStorekit2 = require("module_7470").isIosStorekit2;
export const setup = () => {
  let obj = arg0;
  if (arg0 === undefined) {
    obj = {};
  ...
  if ("STOREKIT1_MODE" === str) {
    require(7470).storekit1Mode();
    const obj4 = require(7470);
  } else if ("STOREKIT2_MODE" === str) {
    require(7470).storekit2Mode();
    const obj3 = require(7470);
  } else if ("STOREKIT_HYBRID_MODE" === str) {
    require(7470).storekitHybridMode();
    const obj2 = require(7470);
  }
};
export const initConnection = () => {
  const nativeModule = require(7470).getNativeModule();
...
export const flushFailedPurchasesCachedAsPendingAndroid = () => {
  const androidModule = require(7470).getAndroidModule();
...
export const getProducts = (skus) => {
  ...
  if (length) {
    closure_1 = callback(function*() {
      let closure_1 = tmp5;
      let closure_0 = tmp2;
      const obj3 = outer1_0(7470);
      const androidModule = obj3.getAndroidModule();
      yield androidModule.getItemsByType(outer1_7, outer1_0);
      closure_0 = arg1.map(outer1_0(7475).singleProductAndroidMap);
      const obj = outer1_0(7470);
...
export const getSubscriptions = (skus) => {
  ...
  if (length) {
    closure_1 = callback(function*() {
      ...
      androidModuleType = outer1_0(7470).getAndroidModuleType();
      const obj10 = outer1_0(7470);
      const androidModule = outer1_0(7470).getAndroidModule();
      const outer1_1 = yield androidModule.getItemsByType(outer1_6, outer1_0);
      if ("android" === outer1_0) {
        return outer1_8(outer1_1, outer1_0(7469).SubscriptionPlatform.android);
      }
      if ("amazon" !== tmp35) {
        const _Error = Error;
      ...
      const obj2 = outer1_0(7470);
      let closure_2 = yield obj2.fillProductsWithAdditionalData(dependencyMap);
      return outer1_8(closure_2, outer1_0(7469).SubscriptionPlatform.amazon);
    });
    let rejectResult = (function android() {
      const self = this;
      const apply = closure_1.apply;
