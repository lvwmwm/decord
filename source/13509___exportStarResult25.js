...
// Dependencies: [13481, 13507, 13506, 13505, 13494]

// Module 13509 (__exportStarResult25)
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13481).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13507).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13507).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13506).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13505).FormatApproximately(internalSlots, result);
    ...
    return tmp(13494).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13506).FormatNumeric(internalSlots, isNaN);
};
