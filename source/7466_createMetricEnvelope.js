...
// Dependencies: [7363, 7410, 7407, 7464]

// Module 7466 (createMetricEnvelope)
const require = arg1;
...
function createMetricEnvelope(arg0, arg1, sdk) {
  ...
  if (tmp) {
    obj.dsn = require(7410) /* dsnFromString */.dsnToString(arg1);
    const obj4 = require(7410) /* dsnFromString */;
  }
  const date = new Date();
  const result = require(7464) /* items */.serializeMetricBuckets(arg0);
  obj = { type: "statsd", length: result.length };
  const items = [obj, result];
  const obj5 = require(7464) /* items */;
  const items1 = [items];
