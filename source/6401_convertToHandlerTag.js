...
// Dependencies: [32, 19, 6402, 6403, 6413, 6414, 6415, 6417, 6418, 6419, 6398, 6382, 6420, 6380]
...
function convertToHandlerTag(num) {
  if (typeof num === "number") {
    return num;
  } else if (num instanceof require(6398) /* _isNativeReflectConstruct */.BaseGesture) {
    num = num.handlerTag;
  } else {
...
export const useWebEventHandlers = function useWebEventHandlers() {
  return callback3({
    onGestureHandlerEvent(nativeEvent) {
      const result = callback(6380).onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
