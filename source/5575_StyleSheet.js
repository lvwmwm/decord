...
// Function ID: 47369
// Name: StyleSheet
// Dependencies: [57, 31, 27, 33, 5526, 5576, 1457, 5556, 5552]

// Module 5575 (StyleSheet)
...
export default importAllResult.memo(function CardContainer(active) {
  ...
  const context = importAllResult.useContext(active(5526).HeaderHeightContext);
  const items = [scene.descriptor];
  const tmp2 = importDefault(5576);
  ({ onPageChangeStart: closure_9, onPageChangeCancel: closure_10, onPageChangeConfirm: closure_11 } = importDefault(5576)(importAllResult.useCallback(() => {
    let navigation;
    let options;
  ...
  const effect = importAllResult.useEffect(() => {
    ...
    return () => {
      if (closure_0) {
        ...
        if (!tmp3) {
          next.removeListener(closure_0);
        }
        tmp3 = null == next || null == next.removeListener;
      }
    };
  }, items1);
  ...
  const tmp2Result = importDefault(5576)(importAllResult.useCallback(() => {
    let navigation;
    let options;
    ({ navigation, options } = scene.descriptor);
    let isFocusedResult = navigation.isFocused();
  ...
  if (false === animationEnabled) {
    if (false === isNextScreenTransparent) {
      ...
      obj8.children = closure_7(active(5526).HeaderHeightContext.Provider, obj9);
      obj7.children = closure_7(active(5526).HeaderShownContext.Provider, obj8);
      obj6.children = closure_7(active(5526).HeaderBackContext.Provider, obj7);
      ...
      obj4.children = scene(importDefault(5552).Provider, obj5);
