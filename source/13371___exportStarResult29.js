...
// Dependencies: [1281, 13352, 13353, 13355, 13365]
// Exports: ToRawPrecision

// Module 13371 (__exportStarResult29)
import e from "e";
...
export const ToRawPrecision = function ToRawPrecision(ZERO, minimumSignificantDigits, maximumSignificantDigits, result) {
  ...
  let tmp4;
  if (ZERO.isZero()) {
    let repeatResult = require(13353) /* getMultiInternalSlots */.repeat("0", maximumSignificantDigits);
    let roundedNumber = require(13355) /* TEN */.ZERO;
    ...
    result = require(13365) /* __exportStarResult11 */.ApplyUnsignedRoundingMode(ZERO, timesResult, roundedNumber, result);
  ...
  if (roundingMagnitude >= maximumSignificantDigits - 1) {
    let sum = repeatResult + tmp16(13353).repeat("0", roundingMagnitude - maximumSignificantDigits + 1);
    ...
    tmp16(13353).invariant(roundingMagnitude < 0, "e should be less than 0");
    sum = `0.${tmp16(13353).repeat("0", -num3 - 1)}${arr}`;
