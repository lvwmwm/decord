...
// Function ID: 13180
// Name: SemVer
// Dependencies: [41, 42, 13180, 13181, 13182, 13183, 13184]
...
class SemVer {
  constructor(arg0, arg1) {
    ...
    tmp2 = SemVer;
    tmp3 = closure_1;
    tmp4 = require("module_13180")(require);
    ...
    if (version.length > require("module_13181").MAX_LENGTH) {
      tmp28 = globalThis;
      ...
      tmp29 = new.target;
      tmp30 = new.target;
      typeError1 = new TypeError("version is longer than " + require("module_13181").MAX_LENGTH + " characters");
      tmp32 = typeError1;
      throw typeError1;
    } else {
      str13 = "SemVer";
      tmp38 = require("module_13182")("SemVer", version, tmp4);
      self.options = tmp4;
      ...
      if (match) {
        ...
        self.minor = +match[2];
        self.patch = +match[3];
        if (self.major <= require("module_13181").MAX_SAFE_INTEGER) {
          num = 0;
          if (self.major >= 0) {
            if (self.minor <= require("module_13181").MAX_SAFE_INTEGER) {
              if (self.minor >= 0) {
                if (self.patch <= require("module_13181").MAX_SAFE_INTEGER) {
                  if (self.patch >= 0) {
                    if (match[4]) {
                      str2 = match[4];
                      str3 = ".";
...
let items = [
  ...
  },
  {
    ...
    value: function compare(tmp2Result) {
      const self = this;
      SemVer(13182)("SemVer.compare", this.version, this.options, tmp2Result);
  ...
  },
  {
    ...
    value: function compareMain(tmp2Result) {
      ...
      let compareIdentifiersResult = SemVer(13184).compareIdentifiers(self.major, tmpResult.major);
      if (!compareIdentifiersResult) {
        let tmp3Result = tmp3(13184);
        compareIdentifiersResult = tmp3Result.compareIdentifiers(self.minor, tmpResult.minor);
      }
      if (!compareIdentifiersResult) {
        tmp3Result = tmp3(13184);
  ...
  },
  {
    ...
    value: function comparePre(tmp2Result) {
      ...
      while (true) {
        ...
        let tmp8 = tmp3;
        let tmp9 = tmp4;
        let tmp10 = SemVer(13182)("prerelease compare", num3, tmp3, tmp4);
  ...
  },
  {
    ...
    value: function compareBuild(arg0) {
      ...
      while (true) {
        ...
        let tmp8 = tmp3;
        let tmp9 = tmp4;
        let tmp10 = SemVer(13182)("build compare", num, tmp3, tmp4);
        let tmp11 = undefined === tmp3;
        let tmp12 = num;
        if (tmp11) {
          if (undefined === tmp4) {
