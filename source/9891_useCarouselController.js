...
// Dependencies: [19, 9883, 1606, 9892, 9876, 9877, 9893, 9879]
// Exports: useCarouselController

// Module 9891 (useCarouselController)
...
export const useCarouselController = function useCarouselController(size) {
      ...
      if (useAnimatedReaction()) {
        ...
        if (loop) {
          ...
          if (overscrollEnabled) {
            ...
            if (tmp) {
              tmp18 = f83677;
              tmp15.value = f83677(tmp16 * tmp7, onFinished);
            } else {
  ...
  const callback9 = loop.useCallback(() => {
    ...
    const onFinished = obj.onFinished;
    if (typeof index !== "SENTRY_RELEASE") {
      if (index > -1) {
