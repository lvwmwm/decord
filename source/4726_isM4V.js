...
function isM4V(fileChunk) {
  ...
  if (!tmp2) {
    isftypStringIncludedResult = require(4718) /* getFileChunk */.isftypStringIncluded(fileChunk);
  }
...
arg5.isFLV = function isFLV(fileChunk) {
  ...
  if (!tmp2) {
    isFlvStringIncludedResult = require(4718) /* getFileChunk */.isFlvStringIncluded(fileChunk);
  }
  return isFlvStringIncludedResult;
};
arg5.isM4V = isM4V;
arg5.isMKV = function isMKV(fileChunk) {
  fileChunk = require(4718) /* getFileChunk */.getFileChunk(fileChunk, 64);
  const FileTypes = require(4719).FileTypes;
  const tmp2 = !FileTypes.checkByFileType(fileChunk, "mkv");
  let tmp3 = !tmp2;
  if (!tmp2) {
    tmp3 = "mkv" === require(4718) /* getFileChunk */.findMatroskaDocTypeElements(fileChunk);
  }
  return tmp3;
};
...
arg5.isWEBM = function isWEBM(fileChunk) {
  ...
  let tmp3 = !tmp2;
  if (!tmp2) {
    tmp3 = "webm" === require(4718) /* getFileChunk */.findMatroskaDocTypeElements(fileChunk);
  }
  return tmp3;
};
