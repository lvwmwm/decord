...
// Dependencies: [32, 41, 42, 93, 95, 96, 98, 7393, 7408, 7390, 7454, 7418, 7421, 7422, 7394, 7447, 7405, 7455, 7416, 7399, 7430, 7449]

// Module 7453 (_isNativeReflectConstruct)
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7454).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7454);
      return ServerRuntimeClient(7418).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7418);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7454).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7421).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7421);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7447).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7422).DEBUG_BUILD) {
        const logger = tmp2(7394).logger;
    ...
    value: function captureCheckIn(checkInId) {
      ...
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7455);
          ...
          if (ServerRuntimeClient(7422).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7394).logger;
          ...
          if (ServerRuntimeClient(7422).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7394).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7405);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7416)._getSpanForScope(arg0);
        ...
        const tmpResult1 = ServerRuntimeClient(7430);
        if (_getSpanForScopeResult) {
