...
// Function ID: 8884
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 8884, 8885]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(8884).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(8885).crypto) {
    if (typeof tmp(8885).crypto.getRandomValues === "find") {
      const _crypto2 = tmp(8885).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(8885).crypto) {
    if (typeof tmp(8885).crypto.randomBytes === "find") {
      const _crypto = tmp(8885).crypto;
