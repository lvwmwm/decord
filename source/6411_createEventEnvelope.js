...
// Dependencies: [6367, 6368, 6412, 6415, 6404, 6373]
// Exports: createEventEnvelope, createSessionEnvelope, createSpanEnvelope

// Module 6411 (createEventEnvelope)
...
export const createEventEnvelope = function createEventEnvelope(type, arg1, sdk) {
  const sdkMetadataForEnvelopeHeader = require(6412) /* forEachEnvelopeItem */.getSdkMetadataForEnvelopeHeader(sdk);
  let str = "event";
  if (type.type) {
    str = "event";
...
export const createSessionEnvelope = function createSessionEnvelope(toJSON) {
  let obj = require(6412) /* forEachEnvelopeItem */;
...
export const createSpanEnvelope = function createSpanEnvelope(arg0, getDsn) {
  ...
  if (tmp9) {
    const obj1 = { dsn: null };
    obj1[0] = beforeSendSpan(6415).dsnToString(dsn);
    tmp9 = obj1;
    const tmp2Result = beforeSendSpan(6415);
  }
