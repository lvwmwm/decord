...
// Dependencies: [7495, 7542, 7539, 7596]

// Module 7598 (createMetricEnvelope)
const require = arg1;
...
function createMetricEnvelope(arg0, arg1, sdk) {
  ...
  if (tmp) {
    obj.dsn = require(7542) /* dsnFromString */.dsnToString(arg1);
    const obj4 = require(7542) /* dsnFromString */;
  }
  const date = new Date();
  const result = require(7596) /* items */.serializeMetricBuckets(arg0);
  obj = { type: "statsd", length: result.length };
  const items = [obj, result];
  const obj5 = require(7596) /* items */;
  const items1 = [items];
...
arg5.captureAggregateMetrics = function captureAggregateMetrics(_client, arr) {
  const logger = require(7495) /* consoleSandbox */.logger;
