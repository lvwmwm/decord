...
// Dependencies: [32, 19, 5443, 5444, 5456, 5455, 5453, 5459, 5464, 5461, 5465, 5430, 5433, 5428, 5466]
// Exports: checkGestureCallbacksForWorklets, extractGestureRelations, useForceRender, useWebEventHandlers

// Module 5463 (convertToHandlerTag)
...
function convertToHandlerTag(num) {
  if (typeof num === "number") {
    return num;
  } else if (num instanceof require(5465) /* _isNativeReflectConstruct */.BaseGesture) {
    num = num.handlerTag;
  } else {
...
export const useWebEventHandlers = function useWebEventHandlers() {
  const obj = {
    onGestureHandlerEvent(nativeEvent) {
      const result = callback(5428).onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
