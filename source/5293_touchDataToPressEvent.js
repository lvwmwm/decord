...
// Module 5293 (touchDataToPressEvent)
...
arg5.isTouchWithinInset = function isTouchWithinInset(current, closure_21, changedTouches) {
  let locationX;
  ...
  if (tmp2) {
    ...
    const bottom = closure_21.bottom;
    let num4 = 0;
    if (null != bottom) {
      num4 = bottom;
    }
...
arg5.gestureToPressableEvent = function gestureToPressableEvent(closure_0) {
  ...
  let nativeEvent = { identifier: closure_0.handlerTag, locationX: closure_0.x, locationY: closure_0.y, pageX: closure_0.absoluteX, pageY: closure_0.absoluteY, target: 0, timestamp, touches: [], changedTouches: [] };
  nativeEvent = { touches: items, changedTouches: items1, identifier: nativeEvent.identifier, locationX: closure_0.x, locationY: closure_0.y, pageX: closure_0.absoluteX, pageY: closure_0.absoluteY, target: 0, timestamp, force: undefined };
  items = [nativeEvent];
  items1 = [nativeEvent];
  return { nativeEvent };
};
arg5.gestureTouchToPressableEvent = function gestureTouchToPressableEvent(handlerTag) {
  ...
  const timestamp = Date.now();
  let obj = {};
  obj = { touches: allTouches.map((arg0) => timestamp(arg0, timestamp, 0)), changedTouches: changedTouches.map((arg0) => timestamp(arg0, timestamp, 0)), identifier: handlerTag.handlerTag };
