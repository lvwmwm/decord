...
// Dependencies: [32, 19, 4066]
// Exports: useRiveProperty

// Module 4065 (useRiveProperty)
...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f28837) {
  const _require = instance;
  const dependencyMap = FillColor;
  const callback = f28837;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = _require(4066).useDisposableMemo(() => {
    if (closure_0) {
