...
// Function ID: 47372
// Name: StyleSheet
// Dependencies: [57, 31, 27, 33, 5528, 5578, 1457, 5558, 5554]

// Module 5577 (StyleSheet)
...
export default importAllResult.memo(function CardContainer(active) {
  ...
  const context = importAllResult.useContext(active(5528).HeaderHeightContext);
  const items = [scene.descriptor];
  const tmp2 = importDefault(5578);
  ({ onPageChangeStart: closure_9, onPageChangeCancel: closure_10, onPageChangeConfirm: closure_11 } = importDefault(5578)(importAllResult.useCallback(() => {
    let navigation;
    let options;
  ...
  const tmp2Result = importDefault(5578)(importAllResult.useCallback(() => {
    let navigation;
    let options;
    ({ navigation, options } = scene.descriptor);
    let isFocusedResult = navigation.isFocused();
  ...
  if (false === animationEnabled) {
    if (false === isNextScreenTransparent) {
      ...
      obj8.children = closure_7(active(5528).HeaderHeightContext.Provider, obj9);
      obj7.children = closure_7(active(5528).HeaderShownContext.Provider, obj8);
      obj6.children = closure_7(active(5528).HeaderBackContext.Provider, obj7);
      ...
      obj4.children = scene(importDefault(5554).Provider, obj5);
