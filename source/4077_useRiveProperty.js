...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f28990) {
  const _require = instance;
  const dependencyMap = FillColor;
  const callback = f28990;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = _require(4078).useDisposableMemo(() => {
    if (closure_0) {
