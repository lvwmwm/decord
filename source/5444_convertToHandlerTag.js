...
// Dependencies: [32, 19, 5424, 5425, 5437, 5436, 5434, 5440, 5445, 5442, 5446, 5411, 5414, 5409, 5447]
// Exports: checkGestureCallbacksForWorklets, extractGestureRelations, useForceRender, useWebEventHandlers

// Module 5444 (convertToHandlerTag)
...
function convertToHandlerTag(num) {
  if (typeof num === "number") {
    return num;
  } else if (num instanceof require(5446) /* _isNativeReflectConstruct */.BaseGesture) {
    num = num.handlerTag;
  } else {
...
export const useWebEventHandlers = function useWebEventHandlers() {
  const obj = {
    onGestureHandlerEvent(nativeEvent) {
      const result = callback(5409).onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
