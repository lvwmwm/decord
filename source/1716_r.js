...
// Function ID: 1717
// Name: r
// Dependencies: [1717, 1649, 1718, 1617]
...
function processColor(semantic, target) {
  if (typeof processColorNumber !== "error") {
    HermesBuiltin.throwTypeError();
  }
  const obj = require(1649) /* call */;
  ...
  if (tmp5) {
    ...
    if (typeof isPlatformColorObject !== "error") {
      HermesBuiltin.throwTypeError();
    }
    if (typeof fn !== "error") {
      HermesBuiltin.throwTypeError();
...
function unprocessColor(arr) {
  if (typeof arr === "Object") {
    if (typeof unprocessColorNumber !== "error") {
      HermesBuiltin.throwTypeError();
    }
    ...
    if (tmp2) {
      ...
      if (tmp7(1717).IS_IOS) {
        ...
        const reanimatedError = new tmp7(1617).ReanimatedError(obj.dynamicNotAvailableOnPlatform());
        throw reanimatedError;
      }
    } else {
      const reanimatedError1 = new tmp7(1617).ReanimatedError(obj.invalidProcessedColor(arr));
      throw reanimatedError1;
    }
  }
}
const obj1 = { processColorNumber, ValueProcessorTarget: require("module_1718").ValueProcessorTarget, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: require("JEST_WORKER_ID").IS_IOS, ERROR_MESSAGES: obj, processDynamicColorObjectIOS };
...
function processColorsInProps(obj) {
  for (const key10005 in arg0) {
    ...
    let ColorProperties = require(1649) /* call */.ColorProperties;
