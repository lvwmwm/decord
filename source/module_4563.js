// Module ID: 4563
// Function ID: 4564
// Dependencies: [109, 19, 17, 21, 4561, 4552, 4545, 4564, 4551, 4567, 4540]
...
export default importAllResult.forwardRef(function ScreenStackItem(unstable_sheetFooter, ref) {
  ...
  dependencyMap = flag4.useContext(screenId(4561).RNSScreensRefContext);
  const imperativeHandle = flag4.useImperativeHandle(ref, () => ref.current);
  let obj1 = screenId(4552);
  let flag;
  ...
  importDefault(4545)(flag5, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  obj[1] = callback(importDefault(4564), { contentStyle, style: container, stackPresentation, children });
  const items1 = [callback(screenId(4552).EdgeInsetApplicationContext.Provider, obj), , ];
  obj = {};
  const merged1 = Object.assign(headerConfig);
  items1[1] = callback(screenId(4551).ScreenStackHeaderConfig, obj);
  let tmp18Result = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp18Result) {
    obj1 = { children: null };
    obj1[0] = unstable_sheetFooter();
    tmp18Result = tmp18(tmp2(4567).FooterComponent, obj1);
  }
  items1[2] = tmp18Result;
  const tmp11 = importDefault(4545);
  ...
  return callback(importDefault(4540), obj2);
});
