...
// Dependencies: [6, 7, 4700, 507, 4708, 1845, 2]
// Exports: calculateProgress, canUploadNatively, doesImageMatchUpload

// Module 4707 (DefaultHttpClient)
import set from "set";
...
export const doesImageMatchUpload = function doesImageMatchUpload(image, closure_0) {
  if (closure_0.id !== image.uri) {
    if (closure_0.item.platform === require(4700) /* _isNativeReflectConstruct */.UploadPlatform.REACT_NATIVE) {
...
export const canUploadNatively = function canUploadNatively(item) {
  let fileIsInAppDirResult = item.platform === require(4700) /* _isNativeReflectConstruct */.UploadPlatform.REACT_NATIVE;
  if (fileIsInAppDirResult) {
    fileIsInAppDirResult = null != item.uri;
  }
  if (fileIsInAppDirResult) {
    fileIsInAppDirResult = require(4708) /* _getAttachmentFile */.fileIsInAppDir(item.uri);
    const obj = require(4708) /* _getAttachmentFile */;
  }
  if (fileIsInAppDirResult) {
