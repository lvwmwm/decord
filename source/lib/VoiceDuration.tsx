...
prototype["start"] = function start(flag, flag2) {
  ...
  connection.on(self(4413).BaseConnectionEvent.Speaking, (arg0, arg1) => {
    if (self.userId === arg0) {
  ...
  connection2.on(self(4413).BaseConnectionEvent.Mute, (flag) => {
    self.onMuted(flag);
  });
  const connection3 = self.connection;
  connection3.on(self(4413).BaseConnectionEvent.Deafen, (flag2) => {
    self.onDeafened(flag2);
  });
};
