...
// Function ID: 5098
// Name: mergeMessage
// Dependencies: [676, 3, 4625, 12, 4311, 11, 5098, 5099, 5100, 5101, 2]
// Exports: flatMapChannelMessages

// Module 5097 (mergeMessage)
...
prototype2["mergeDelta"] = function mergeDelta(new_messages, modified_messages, deleted_message_ids) {
  ...
  return this.mutate((_before) => {
    ...
    const mapped = set.map((message) => set(4625).createMessageRecord(message));
    const combined = found.concat(mapped, items1.map((message) => set(4625).createMessageRecord(message)));
...
prototype2["receiveMessage"] = function receiveMessage(nonce) {
    ...
    if (id === id1) {
      if (null != nonce.nonce) {
        if (value.id === nonce.nonce) {
          const messageRecord = messageRecord1(4625).createMessageRecord(nonce);
  ...
  if (self.hasMoreAfter) {
    ...
    messageRecord1 = messageRecord1(4625).createMessageRecord(nonce);
    const lastResult = self.last();
    if (null != lastResult) {
