...
// Function ID: 5185
// Name: mergeMessage
// Dependencies: [676, 3, 4705, 12, 4379, 11, 5185, 5186, 5187, 5188, 2]
// Exports: flatMapChannelMessages

// Module 5184 (mergeMessage)
...
prototype2["mergeDelta"] = function mergeDelta(new_messages, modified_messages, deleted_message_ids) {
  ...
  return this.mutate((_before) => {
    ...
    const mapped = set.map((message) => set(4705).createMessageRecord(message));
    const combined = found.concat(mapped, items1.map((message) => set(4705).createMessageRecord(message)));
...
prototype2["receiveMessage"] = function receiveMessage(nonce) {
    ...
    if (id === id1) {
      if (null != nonce.nonce) {
        if (value.id === nonce.nonce) {
          const messageRecord = messageRecord1(4705).createMessageRecord(nonce);
  ...
  if (self.hasMoreAfter) {
    ...
    messageRecord1 = messageRecord1(4705).createMessageRecord(nonce);
    const lastResult = self.last();
    if (null != lastResult) {
