...
// Function ID: 5234
// Name: mergeMessage
// Dependencies: [673, 3, 4745, 12, 4412, 11, 5234, 5235, 5236, 5237, 2]
// Exports: flatMapChannelMessages

// Module 5233 (mergeMessage)
...
import isIOSPushNotificationRawPayloadFixExperimentEnabled from "isIOSPushNotificationRawPayloadFixExperimentEnabled" /* 5236 */;
...
prototype2["mergeDelta"] = function mergeDelta(new_messages, modified_messages, deleted_message_ids) {
  ...
  return this.mutate((_before) => {
    ...
    const mapped = set.map((message) => set(4745).createMessageRecord(message));
    const combined = found.concat(mapped, items1.map((message) => set(4745).createMessageRecord(message)));
...
prototype2["receiveMessage"] = function receiveMessage(nonce) {
    ...
    if (id === id1) {
      if (null != nonce.nonce) {
        if (value.id === nonce.nonce) {
          const messageRecord = messageRecord1(4745).createMessageRecord(nonce);
  ...
  if (self.hasMoreAfter) {
    ...
    messageRecord1 = messageRecord1(4745).createMessageRecord(nonce);
    const lastResult = self.last();
    if (null != lastResult) {
