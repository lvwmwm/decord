...
// Function ID: 5164
// Name: mergeMessage
// Dependencies: [676, 3, 4689, 12, 4375, 11, 5164, 5165, 5166, 5167, 2]
// Exports: flatMapChannelMessages

// Module 5163 (mergeMessage)
...
prototype2["mergeDelta"] = function mergeDelta(new_messages, modified_messages, deleted_message_ids) {
  ...
  return this.mutate((_before) => {
    ...
    const mapped = set.map((message) => set(4689).createMessageRecord(message));
    const combined = found.concat(mapped, items1.map((message) => set(4689).createMessageRecord(message)));
...
prototype2["receiveMessage"] = function receiveMessage(nonce) {
    ...
    if (id === id1) {
      if (null != nonce.nonce) {
        if (value.id === nonce.nonce) {
          const messageRecord = messageRecord1(4689).createMessageRecord(nonce);
  ...
  if (self.hasMoreAfter) {
    ...
    messageRecord1 = messageRecord1(4689).createMessageRecord(nonce);
    const lastResult = self.last();
    if (null != lastResult) {
