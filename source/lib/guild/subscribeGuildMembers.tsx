...
// Dependencies: [109, 19, 21, 12, 5821, 643, 1208, 2]
// Exports: default, useSubscribeGuildMembers

// Module 5820 (subscribeGuildMembers)
      ...
      if (!WrappedComponent(WrappedComponent[5])(this.props, arg0)) {
        ...
        if (!isEqualResult) {
          if (null != self._subscriptions) {
            ...
            const item = tmpResult.forEach(self._subscriptions, (userIds, guildId) => callback(5821).unsubscribeMembers(guildId, userIds));
          }
          const item1 = tmp(tmp2[3]).forEach(tmp4, (userIds, guildId) => callback(5821).subscribeMembers(guildId, userIds));
