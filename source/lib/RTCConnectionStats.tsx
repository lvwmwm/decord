...
// Function ID: 13080
// Name: reset
// Dependencies: [676, 4379, 12, 13080, 2]
...
prototype["update"] = function update(current) {
  ...
  if (this.current !== current) {
    ...
    obj[0] = current;
    obj[1] = require(4379) /* sleep */.now();
    history.push(obj);
    const obj2 = require(4379) /* sleep */;
  }
};
...
prototype["getStateDurations"] = function getStateDurations(nowResult) {
  ...
  if (0 === this.history.length) {
    ...
    let arr = importDefault(13080)(self.history, (state, startTime) => ({ state: state.state, durationMs: startTime.startTime - state.startTime }));
