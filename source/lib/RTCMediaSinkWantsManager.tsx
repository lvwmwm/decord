...
// Dependencies: [32, 1218, 676, 4315, 687, 4572, 12950, 4354, 4364, 4140, 10579, 11, 12, 500, 643, 4351, 2]

// Module 12949 (getDefaultWants)
...
function getDefaultWants(arg0) {
  let obj = require(12950) /* getBrowserInvertedWantsConfig */;
...
prototype["setConnection"] = function setConnection(c3, arg1) {
  ...
  const connection = this.connection;
  if (connection != null) {
    connection.removeListener(require(4351) /* BaseConnectionEvent */.BaseConnectionEvent.LocalVideoDisabled, self.handleLocalVideoDisabled);
  }
  const connection2 = self.connection;
  if (connection2 != null) {
    connection2.removeListener(require(4351) /* BaseConnectionEvent */.BaseConnectionEvent.LocalMute, self.handleLocalMute);
  }
  const connection3 = self.connection;
  if (connection3 != null) {
    connection3.removeListener(require(4351) /* BaseConnectionEvent */.BaseConnectionEvent.ActiveSinksChange, self.delayedUpdate);
  }
  self.connection = c3;
  const connection4 = self.connection;
  if (connection4 != null) {
    connection4.addListener(require(4351) /* BaseConnectionEvent */.BaseConnectionEvent.LocalVideoDisabled, self.handleLocalVideoDisabled);
  }
  const connection5 = self.connection;
  if (connection5 != null) {
    connection5.addListener(require(4351) /* BaseConnectionEvent */.BaseConnectionEvent.LocalMute, self.handleLocalMute);
  }
  const connection6 = self.connection;
  if (connection6 != null) {
    connection6.addListener(require(4351) /* BaseConnectionEvent */.BaseConnectionEvent.ActiveSinksChange, self.delayedUpdate);
  }
  if (flag) {
    self.update();
  }
...
prototype["reset"] = function reset() {
  ...
  WindowVisibilityVideoManager.off(require(10579) /* isIncomingVideoEnabled */.WindowVisibilityEvent.IncomingVideoEnabledChanged, self.incomingVideoEnabledChanged);
};
