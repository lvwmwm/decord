...
// Function ID: 8880
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 8880, 8881]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      ...
      let bytesResult = Hash(8880).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(8881).crypto) {
    if (typeof tmp(8881).crypto.getRandomValues === "find") {
      const _crypto2 = tmp(8881).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(8881).crypto) {
    if (typeof tmp(8881).crypto.randomBytes === "find") {
      const _crypto = tmp(8881).crypto;
