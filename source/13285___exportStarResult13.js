...
// Dependencies: [1281, 13270, 13286, 13287]
// Exports: ComputeExponent

// Module 13285 (__exportStarResult13)
import e from "e";
...
export const ComputeExponent = function ComputeExponent(internalSlots, timesResult) {
  if (timesResult.isZero()) {
    ...
    const result = require(13286) /* __exportStarResult14 */.ComputeExponentForMagnitude(internalSlots, floorResult);
    const _default = digitsToString.default;
    const logResult = negatedResult.log(10);
    const tmp2 = require;
    const result1 = require(13287) /* __exportStarResult21 */.FormatNumericToString(internalSlots, negatedResult.times(digitsToString.default.pow(10, -result)));
    ...
    if (roundedNumber.isZero()) {
      ...
      if (floorResult1.eq(floorResult.minus(result))) {
        ...
        const items2 = [tmp2(13286).ComputeExponentForMagnitude(internalSlots, floorResult.plus(1)), floorResult.plus(1).toNumber()];
        return items2;
      }
