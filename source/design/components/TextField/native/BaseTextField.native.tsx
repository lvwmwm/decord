...
// Dependencies: [31, 33, 477, 1324, 6683, 3840, 6686, 3839, 2]

// Module 6685 (collapseNewlines)
...
const forwardRefResult = importAllResult.forwardRef((size, photosButtonExternalRef) => {
  ...
  const merged2 = Object.assign(ref(3839).mergeProps(size, focusProps));
  obj2["value"] = collapseNewlines(size.value);
  obj2["defaultValue"] = collapseNewlines(size.defaultValue);
  obj2["onChangeText"] = callback;
  const obj10 = ref(3839);
  obj2["ref"] = ref(3839).mergeRefs(ref, photosButtonExternalRef);
  const items2 = [, , ];
  ...
  items1[1] = callback(ref(6686).NativeTextInput, obj2);
  items1[2] = size.trailing;
  obj1["children"] = items1;
  return callback2(ref(6683).InputFieldContainer, obj1);
});
