...
export const BackgroundBlurFillWithPress = function BackgroundBlurFillWithPress(pressed) {
  ...
  class T {
    constructor() {
      ...
      items[0] = closure_3;
      items[1] = closure_4;
      interpolateColorResult = obj3.interpolateColor(pressed.get(), [null, null], items);
  ...
  const fn = function p() {
    ...
    if (!isBlurDisabledResult) {
      ...
      withSpringResult = obj2.withSpring(pressed(tmp7[9]).interpolateColor(pressed.get(), [null, null], items), pressed(tmp7[11]).ON_PRESS_SPRING, "animate-always");
      const interpolateColorResult = pressed(tmp7[9]).interpolateColor(pressed.get(), [null, null], items);
    }
