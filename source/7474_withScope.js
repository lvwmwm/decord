...
// Function ID: 7475
// Name: withScope
// Dependencies: [41, 42, 7475, 7461, 7472, 7480]
// Exports: getStackAsyncContextStrategy

// Module 7474 (withScope)
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(7472).getMainCarrier();
  const obj = AsyncContextStack(7472);
  const sentryCarrier = AsyncContextStack(7472).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7472).getMainCarrier();
      let obj = callback(7472);
      let sentryCarrier = callback(7472).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7480);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7472).getMainCarrier();
      const obj = callback(7472);
      const sentryCarrier = callback(7472).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7480);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
