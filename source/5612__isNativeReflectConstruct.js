...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 5613, 5583, 5628, 5615, 5630, 5626, 5631, 5616, 5632]
...
function getProgressFromGesture(interpolate, width, options) {
  ...
  if (typeof getDistanceFromOptions !== "fileFinishedImporting") {
  ...
  if (undefined !== gestureDirection) {
    const tmp10 = importDefault(5615)(obj, gestureDirection);
    if (tmp10 > 0) {
    ...
    let gestureDirection2 = CardStack(5628).ModalTransition.gestureDirection;
  } else {
    gestureDirection2 = CardStack(5628).DefaultTransition.gestureDirection;
  }
}
class CardStack {
  constructor(arg0) {
...
let obj = {
  ...
  value: function render() {
        ...
        if (undefined === detachPreviousScreen) {
          ...
          if (tmp7) {
            ...
            if (isModalPresentation) {
              ...
              tmp11 = diff !== importDefault(5630)(scenes, (descriptor) => {
    ...
    children[1] = closure_9(CardStack(5631).MaybeScreenContainer, obj2);
    let tmp23 = null;
    if (someResult) {
      tmp23 = tmp15Result;
    }
    children[2] = tmp23;
    return closure_10(CardStack(5583).Background, { children });
  }
};
let items = [obj];
obj = {
  ...
  value: function getDerivedStateFromProps(routes, routes2) {
    ...
    if (typeof getHeaderHeights !== "fileFinishedImporting") {
      HermesBuiltin.throwTypeError();
    }
