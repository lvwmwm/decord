...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 15862]
// Exports: captureScreen, ensureModuleIsLoaded

// Module 15861 (_isNativeReflectConstruct)
import noopDefault from "noop" /* 19 */;
import _inheritsDefault from "_inherits" /* 98 */;
import get_ActivityIndicatorDefault from "get ActivityIndicator" /* 15862 */;
...
function captureRef(obj, options) {
  if (ViewShot(15862)) {
    ...
    return tmp(15862).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(15862).releaseCapture(str);
    const obj = ViewShot(15862);
  }
}
noopDefault;
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(15862)) {
    ...
    return tmp(15862).captureScreen(options);
  } else {
    const _Error = Error;
    error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
