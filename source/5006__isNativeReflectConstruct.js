...
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 4971, 5007, 5008, 5009]

// Module 5006 (_isNativeReflectConstruct)
...
function readTags(_raw, dataView, arg2) {
  ...
  try {
    ...
    ParseError(4971).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5008).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(5008);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
...
function getDescription(arr) {
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      tmp12 = tmp11;
      if (typeof importDefault(5009)[tmp2] === "function") {
        tmp12 = tmp13(5009)[tmp2](arr, tmp11);
    ...
    try {
      if (tmp2) {
        if (typeof importDefault(5009)[tmp2] === "function") {
          let decodeURIComponentResult = importDefault(5009)[tmp2](arr);
          const tmp4Result = importDefault(5009);
        }
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(4971).objectAssign(obj, obj, obj);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(4971).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(4971);
    obj = {};
    ...
    ParseError(4971).objectAssign(obj.attributes, obj);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(4971);
  }
  ParseError(4971).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
