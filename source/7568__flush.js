...
// Function ID: 7569
// Name: _flush
// Dependencies: [5, 7547, 7569, 7548, 7520, 7531, 7534, 7527, 7557, 7521, 7543]
...
function endSession() {
  const isolationScope = require(7547) /* getClient */.getIsolationScope();
  const obj = require(7547) /* getClient */;
  ...
  const obj3 = require(7547) /* getClient */;
  const currentScope1 = require(7547) /* getClient */.getCurrentScope();
  const tmpResult1 = require(7547) /* getClient */;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = require(7547) /* getClient */.getCurrentScope();
  const obj = require(7547) /* getClient */;
  return currentScope.captureException(arg0, require(7569) /* applyClientOptions */.parseEventHintOrCaptureContext(arg1));
};
...
export const captureSession = function captureSession() {
  ...
  if (flag) {
    ...
    const obj5 = require(7547) /* getClient */;
  }
};
...
export const isInitialized = function isInitialized() {
  return require(7547) /* getClient */.getClient();
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(7557).DEFAULT_ENVIRONMENT;
  }
  const obj5 = require(7547) /* getClient */;
  const userAgent = require(7521) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {}.userAgent;
  let tmpResult = tmp(7543);
  obj = { release, environment, user: null };
  const tmp3 = require(7521) /* getGlobalSingleton */.GLOBAL_OBJ.navigator || {};
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    ...
    obj[3] = monitorSlug(7534).timestampInSeconds() - closure_3;
    const obj2 = monitorSlug(7534);
    const currentScope = monitorSlug(7547).getCurrentScope();
    const obj3 = monitorSlug(7547);
    const client = monitorSlug(7547).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(7547);
    monitorSlug(7531).uuid4();
  }
  let currentScope = _require(7547).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(7547);
  let client = _require(7547).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(7548).DEBUG_BUILD) {
      let logger2 = tmp(7520).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(7520).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(7547);
  captureCheckInResult = _require(7531).uuid4();
};
