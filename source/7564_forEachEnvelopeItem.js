...
// Dependencies: [32, 7521, 7565, 7526, 7567]
...
// Module 7564 (forEachEnvelopeItem)
...
export const createAttachmentEnvelopeItem = function createAttachmentEnvelopeItem(data) {
  if (typeof data.data === "string") {
    ...
    if (!require(7521) /* getGlobalSingleton */.GLOBAL_OBJ.__SENTRY__) {
      const _TextEncoder = TextEncoder;
      const textEncoder = new TextEncoder();
      let encodeResult = textEncoder.encode(data);
    }
    __SENTRY__ = __SENTRY__(7521).GLOBAL_OBJ.__SENTRY__;
    ...
    const items = [require(7526) /* addNonEnumerableProperty */.dropUndefinedKeys(obj), data];
    return items;
  }
};
...
export const createEventEnvelopeHeaders = function createEventEnvelopeHeaders(event_id) {
  ...
  if (tmp4) {
    obj = { dsn: null };
    obj[0] = require(7567) /* dsnFromString */.dsnToString(arg3);
    tmp4 = obj;
    const obj5 = require(7567) /* dsnFromString */;
  }
  ...
  if (event_id.sdkProcessingMetadata && event_id.sdkProcessingMetadata.dynamicSamplingContext) {
    ...
    obj1[0] = require(7526) /* addNonEnumerableProperty */.dropUndefinedKeys(obj2);
    tmp8 = obj1;
    const obj7 = require(7526) /* addNonEnumerableProperty */;
  }
...
export const parseEnvelope = function parseEnvelope(arr) {
  if (typeof arr !== "string") {
    ...
    let encodePolyfill = dependencyMap;
    if (!_require(7521).GLOBAL_OBJ.__SENTRY__) {
      const _TextEncoder = TextEncoder;
      const textEncoder = new TextEncoder();
    ...
    if (typeof tmp8[1] !== "string") {
      ...
      if (!(tmp10 instanceof Uint8Array)) {
        try {
          ...
          json = _JSON3.stringify(_require(7565).normalize(tmp3));
          const obj = _require(7565);
        }
      }
    }
    append(tmp8[1]);
