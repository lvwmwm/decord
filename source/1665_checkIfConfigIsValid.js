...
let tmp3 = (() => {
  export function bisectRoot(arg0) {
    let func;
    let max;
    let maxIterations;
    let min;
...
let tmp2 = (() => {
  export function checkIfConfigIsValid(merged) {
  ...
  checkIfConfigIsValid.__closure = { logger: require(1586) /* noop */.logger };
...
let tmp4 = (() => {
  export function initialCalculations(mass, merged) {
    let stiffness;
    let num = 0;
...
let tmp5 = (() => {
  export function scaleZetaToMatchClamps(toValue, clamp) {
    let startValue;
    let zeta;
    ...
    if (NumberResult === startValue) {
      ...
      const _Math10 = Math;
      const items3 = [zeta];
      return max.apply(Math, outer1_2(items2.filter((arg0) => undefined !== arg0)).concat(items3));
    }
  }
...
const tmp6 = (() => {
  export function calculateNewMassToMatchDuration(diff, merged, velocity) {
    let closure_0 = diff;
    ...
    if (merged.skipAnimation) {
      ...
      pnpm_springUtilsTs6.__initData = outer1_10;
      obj = { min: 0, max: 100, func: pnpm_springUtilsTs6 };
      return outer1_6(obj);
    }
  }
  calculateNewMassToMatchDuration.__closure = { bisectRoot: closure_6 };
...
const tmp7 = (() => {
  export function criticallyDampedSpringCalculations(toValue, arg1) {
    let omega0;
    let t;
    let v0;
    let x0;
