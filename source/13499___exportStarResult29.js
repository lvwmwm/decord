...
// Dependencies: [1281, 13480, 13481, 13483, 13493]
// Exports: ToRawPrecision

// Module 13499 (__exportStarResult29)
import e from "e";
...
export const ToRawPrecision = function ToRawPrecision(ZERO, minimumSignificantDigits, maximumSignificantDigits, result) {
  ...
  let tmp4;
  if (ZERO.isZero()) {
    let repeatResult = require(13481) /* getMultiInternalSlots */.repeat("0", maximumSignificantDigits);
    let roundedNumber = require(13483) /* TEN */.ZERO;
    ...
    result = require(13493) /* __exportStarResult11 */.ApplyUnsignedRoundingMode(ZERO, timesResult, roundedNumber, result);
  ...
  if (roundingMagnitude >= maximumSignificantDigits - 1) {
    let sum = repeatResult + tmp16(13481).repeat("0", roundingMagnitude - maximumSignificantDigits + 1);
    ...
    tmp16(13481).invariant(roundingMagnitude < 0, "e should be less than 0");
    sum = `0.${tmp16(13481).repeat("0", -num3 - 1)}${arr}`;
