...
// Module 13559 (__exportStarResult11)
const require = arg1;
...
arg5.ApplyUnsignedRoundingMode = function ApplyUnsignedRoundingMode(eq, timesResult, roundedNumber, result) {
  if (eq.eq(timesResult)) {
    ...
    require(13547) /* getMultiInternalSlots */.invariant(timesResult.lessThan(eq) && eq.lessThan(roundedNumber), combined1.concat(roundedNumber));
    if ("zero" === result) {
      return timesResult;
      ...
      if (minusResult.lessThan(minusResult1)) {
        ...
        tmp2(13547).invariant(minusResult.eq(minusResult1), "d1 should be equal to d2");
        if ("half-zero" === result) {
          return timesResult;
        } else if ("half-infinity" === result) {
          return roundedNumber;
        } else {
          tmp2(13547).invariant("half-even" === result, "unsignedRoundingMode should be half-even");
