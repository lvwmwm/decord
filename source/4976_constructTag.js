...
// Function ID: 4977
// Name: constructTag
// Dependencies: [4932, 4948, 4960, 4929, 4936, 4962]

// Module 4976 (constructTag)
const require = arg1;
const module = arg2;
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  const stringValueFromArray = require(4929) /* getDataView */.getStringValueFromArray(items1);
  let combined = stringValueFromArray;
  if (type !== require(4936).TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(4929).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(4929);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === require(4936).TYPE_ITXT) {
    stringFromDataView = module(4962).decode("UTF-8", decompressResult);
    const obj5 = module(4962);
  }
