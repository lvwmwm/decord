...
// Dependencies: [32, 41, 42, 7472, 7389, 7476, 7477, 7380, 7478]

// Module 7475 (MetricsAggregator)
...
class MetricsAggregator {
  constructor(arg0) {
    ...
    tmp4 = closure_1;
    this._interval = setInterval(() => self._flush(), require("module_7472").DEFAULT_FLUSH_INTERVAL);
    if (this._interval.unref) {
      _interval = self._interval;
      unrefResult = _interval.unref();
    }
    random = Math.random();
    self._flushShift = Math.floor(random * require("module_7472").DEFAULT_FLUSH_INTERVAL / 1000);
...
const items = [
  {
    ...
    value: function add(arg0, arg1, diff) {
      ...
      if (arg5 === undefined) {
        timestampInSecondsResult = MetricsAggregator(7389).timestampInSeconds();
        const obj2 = MetricsAggregator(7389);
      }
      const self = this;
      const rounded = Math.floor(timestampInSecondsResult);
      const sanitizeMetricKeyResult = MetricsAggregator(7476).sanitizeMetricKey(arg1);
      const obj3 = MetricsAggregator(7476);
      const sanitizeTagsResult = MetricsAggregator(7476).sanitizeTags(obj);
      const obj4 = MetricsAggregator(7476);
      const sanitizeUnitResult = MetricsAggregator(7476).sanitizeUnit(str);
      const obj5 = MetricsAggregator(7476);
      const bucketKey = MetricsAggregator(7476).getBucketKey(arg0, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
      ...
      if (value) {
        ...
        const tmp15 = new tmp5(7477).METRIC_MAP[arg0](diff);
        obj[0] = tmp15;
        obj[1] = rounded;
        obj[2] = arg0;
      ...
      const result1 = MetricsAggregator(7380).updateMetricSummaryOnActiveSpan(arg0, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
      self._bucketsTotalWeight = self._bucketsTotalWeight + obj.metric.weight;
      if (self._bucketsTotalWeight >= MetricsAggregator(7472).MAX_WEIGHT) {
        self.flush();
      }
    }
  },
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(7389).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(7472).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(7478).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(7478);
      }
    }
  }
];
