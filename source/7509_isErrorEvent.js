...
// Function ID: 7510
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 7501, 7482, 7454, 7508, 7465, 7461, 7477, 7478, 7510, 7497, 7498, 7481, 7503, 7490, 7511, 7494, 7512, 7459]

// Module 7509 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(7482).DEBUG_BUILD) {
          const logger = tmp(7454).logger;
        ...
        BaseClient(7477).updateSession(release, { init: false });
        const obj = BaseClient(7477);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(7510).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(7510).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(7510);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(7497).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(7482).DEBUG_BUILD) {
        let logger = tmp2(7454).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(7478).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(7510).setupIntegrations(this, integrations);
      const obj = BaseClient(7510);
      const result = BaseClient(7510).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(7494).parseSampleRate(sampleRate);
        let obj = str(7494);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(7511).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(7478).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(7482).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(7454).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(7512);
