...
const memoResult = importAllResult.memo(function ChatPlaceholder(screenIndex) {
  ...
  const insets = importDefault(dependencyMap[7])({ body: true, color: true }).insets;
  ...
  const style = [tmp.placeholder, memo, importDefault(dependencyMap[9])({ "Bool(false)": null, "Bool(false)": null })];
