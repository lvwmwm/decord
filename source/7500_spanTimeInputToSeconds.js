...
// Dependencies: [7501, 7505, 7507, 7509, 7510, 7511, 7512, 7513, 7514, 7517, 7522, 7495]

// Module 7500 (spanTimeInputToSeconds)
const require = arg1;
  ...
  if (typeof num === "number") {
    ...
    if (Array.isArray(num)) {
      ...
      if (num instanceof Date) {
        ...
        sum = require(7509) /* dateTimestampInSeconds */.timestampInSeconds();
        const obj = require(7509) /* dateTimestampInSeconds */;
      }
    }
  }
  return sum;
  ...
  if ((function spanIsSentrySpan(getSpanJSON) {
    ...
    try {
      ...
      if ((function spanIsOpenTelemetrySdkTraceBaseSpan(attributes) {
        ...
        obj[8] = attributes[require(undefined, 7510).SEMANTIC_ATTRIBUTE_SENTRY_OP];
        obj[9] = attributes[require(undefined, 7510).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
        const obj2 = require(7501) /* addNonEnumerableProperty */;
        obj[10] = require(7511) /* getMetricSummaryJsonForSpan */.getMetricSummaryJsonForSpan(getSpanJSON);
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== require(7512) /* getSpanStatusFromHttpCode */.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== tmp(7512).SPAN_STATUS_OK) {
...
arg5.addChildSpanToSpan = function addChildSpanToSpan(arg0, arg1) {
  ...
  const result = require(7501) /* addNonEnumerableProperty */.addNonEnumerableProperty(arg1, _sentryRootSpan, tmp2);
  if (arg0[_sentryChildSpans]) {
    ...
    const result1 = require(7501) /* addNonEnumerableProperty */.addNonEnumerableProperty(arg0, tmp6, set);
    const tmp3Result = require(7501) /* addNonEnumerableProperty */;
  }
};
arg5.getActiveSpan = function getActiveSpan() {
  ...
  const asyncContextStrategy = require(7514) /* getAsyncContextStrategy */.getAsyncContextStrategy(mainCarrier);
...
arg5.showSpanDropWarning = function showSpanDropWarning() {
  if (!c2) {
    require(7495) /* consoleSandbox */.consoleSandbox(() => {
...
arg5.spanToTraceContext = function spanToTraceContext(spanContext) {
  ...
  if (isRemote) {
    span_id = require(7505) /* generatePropagationContext */.generateSpanId();
    const obj = require(7505) /* generatePropagationContext */;
  }
  return require(7501) /* addNonEnumerableProperty */.dropUndefinedKeys({ parent_span_id, span_id, trace_id: trace_id.traceId });
};
arg5.spanToTraceHeader = function spanToTraceHeader(spanContext) {
  ...
  return require(7507) /* regExp */.generateSentryTraceHeader(traceId, spanId, 1 === spanContext.spanContext().traceFlags);
};
arg5.spanToTransactionTraceContext = function spanToTransactionTraceContext(spanContext) {
  ...
  return require(7501) /* addNonEnumerableProperty */.dropUndefinedKeys({ parent_span_id, span_id, trace_id, data, op, status, origin });
};
