...
// Dependencies: [853, 6361, 6364, 6365, 6379, 6389, 6393, 6397, 6447, 6370, 6399, 6380, 6398, 6436, 6382, 6378, 6372]
// Exports: addTracingHeadersToFetchRequest, instrumentFetchRequest

// Module 6470 (_addTracingHeadersToFetchRequest)
...
function _addTracingHeadersToFetchRequest(headers, headers2, span) {
  let baggage;
  let tmp4;
  let obj = require(6436) /* getTraceData */;
  obj = { span };
  ...
  if (tmp4) {
    ...
    if (!headers) {
      ...
      let isInstanceOfResult = typeof Request === "__INTERNAL_SLOT_MAP__";
      if (typeof Request !== "Array") {
        let tmpResult = tmp(6372);
    ...
    if (headers) {
      ...
      let isInstanceOfResult1 = typeof Headers === "__INTERNAL_SLOT_MAP__";
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(6382).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(6382);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(6382).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(6447).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(6447);
    }
    const activeSpan = setHttpStatus(6370).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(6380).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(6380).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(6399).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(6399);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(6397);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(6398).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(6370);
  }
};
