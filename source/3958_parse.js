  ...
  if (null != match) {
    do {
      ...
      if (tmp16) {
        ...
        if (tmp31) {
          let tmp36 = closure_7;
          let str5 = closure_7(tmp31);
        } else {
          str5 = ".*";
          if (!tmp25) {
            let tmp35 = closure_6;
            str5 = `${"[^" + closure_6(tmp30)}]+?`;
          }
        }
        obj.pattern = str5;
        arr = items.push(obj);
...
function tokensToFunction(arg0) {
  ...
  return (arg0, arg1) => {
    ...
    if (obj.pretty) {
      let _encodeURIComponent = closure_4;
    } else {
...
function tokensToRegExp(arg0, items, id) {
  ...
  let tmp4;
  let obj = id;
  if (!require(dependencyMap[0])(items)) {
    let tmp = items;
    if (!items) {
      tmp = id;
    }
...
function pathToRegexp(source, items, id) {
  ...
  } else if (require(dependencyMap[0])(source)) {
    const items1 = [];
    let num = 0;
    if (0 < source.length) {
      do {
        let tmp7 = closure_11;
        arr = items1.push(closure_11(source[num], items, obj).source);
        num = num + 1;
...
module.exports.compile = function compile(flag) {
  return tokensToFunction(parse(flag, arg1));
};
