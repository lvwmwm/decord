...
// Function ID: 7759
// Name: withScope
// Dependencies: [41, 42, 7759, 7745, 7756, 7764]
// Exports: getStackAsyncContextStrategy

// Module 7758 (withScope)
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7756).getMainCarrier();
      let obj = callback(7756);
      let sentryCarrier = callback(7756).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7764);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7756).getMainCarrier();
      const obj = callback(7756);
      const sentryCarrier = callback(7756).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7764);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
