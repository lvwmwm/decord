...
arg5.isFLV = function isFLV(fileChunk) {
  fileChunk = require(4901) /* getFileChunk */.getFileChunk(fileChunk);
  const FileTypes = require(4902).FileTypes;
  return FileTypes.checkByFileType(fileChunk, "flv") && require(4901) /* getFileChunk */.isFlvStringIncluded(fileChunk);
};
arg5.isM4V = function isM4V(fileChunk) {
  fileChunk = require(4901) /* getFileChunk */.getFileChunk(fileChunk);
  const FileTypes = require(4902).FileTypes;
  return FileTypes.checkByFileType(fileChunk, "m4v") && require(4901) /* getFileChunk */.isftypStringIncluded(fileChunk);
};
arg5.isMKV = function isMKV(fileChunk) {
  fileChunk = require(4901) /* getFileChunk */.getFileChunk(fileChunk, 64);
  const FileTypes = require(4902).FileTypes;
  return FileTypes.checkByFileType(fileChunk, "mkv") && "mkv" === require(4901) /* getFileChunk */.findMatroskaDocTypeElements(fileChunk);
};
...
arg5.isMP4 = function isMP4(fileChunk, excludeSimilarTypes) {
  ...
  if (!checkByFileTypeResult) {
    ...
    if (!excludeSimilarTypes) {
      const fileChunk1 = tmp(4901).getFileChunk(fileChunk);
      const FileTypes2 = tmp(4902).FileTypes;
      tmp8 = FileTypes2.checkByFileType(fileChunk1, "m4v") && tmp(4901).isftypStringIncluded(fileChunk1);
      const tmp10 = FileTypes2.checkByFileType(fileChunk1, "m4v") && tmp(4901).isftypStringIncluded(fileChunk1);
    }
...
arg5.isWEBM = function isWEBM(fileChunk) {
  fileChunk = require(4901) /* getFileChunk */.getFileChunk(fileChunk, 64);
  const FileTypes = require(4902).FileTypes;
  return FileTypes.checkByFileType(fileChunk, "webm") && "webm" === require(4901) /* getFileChunk */.findMatroskaDocTypeElements(fileChunk);
};
