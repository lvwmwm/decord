...
let closure_5 = (() => {
    ...
    function extractInputs(items, arr) {
      if (Array.isArray(items)) {
        for (const item10034 of arg0) {
          if (item10034) {
            let tmp12 = extractInputs;
            let tmp13 = item10034;
            let tmp14 = arg1;
            let tmp15 = extractInputs(tmp11, arg1);
          }
          continue;
        }
      } else {
        if (obj.isSharedValue(items)) {
          ...
          if (Object.getPrototypeOf(items) === Object.prototype) {
            ...
            for (const item10021 of values) {
              if (item10021) {
                let tmp4 = extractInputs;
                let tmp5 = item10021;
                let tmp6 = arg1;
                let tmp7 = extractInputs(tmp3, arg1);
              }
              continue;
            }
          }
        }
        obj = outer2_1(outer2_2[1]);
      }
      return arr;
    }
    let map = new Map();
  ...
  createMapperRegistry.__closure = { IS_JEST: isJest, isSharedValue: require(1631) /* isSharedValue */.isSharedValue };
...
export const startMapper = function startMapper(pnpm_InlinePropManagerTs2, combined, items) {
  let closure_0 = pnpm_InlinePropManagerTs2;
...
export const stopMapper = function stopMapper(_inlinePropsMapperId) {
  ...
  require(1589) /* runWorkletOnJS */.runOnUI(pnpm_mappersTs3)();
};
