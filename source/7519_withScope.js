...
// Function ID: 7520
// Name: withScope
// Dependencies: [41, 42, 7520, 7506, 7517, 7525]
// Exports: getStackAsyncContextStrategy

// Module 7519 (withScope)
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(7517).getMainCarrier();
  const obj = AsyncContextStack(7517);
  const sentryCarrier = AsyncContextStack(7517).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7517).getMainCarrier();
      let obj = callback(7517);
      let sentryCarrier = callback(7517).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7525);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7517).getMainCarrier();
      const obj = callback(7517);
      const sentryCarrier = callback(7517).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7525);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
