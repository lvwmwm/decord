...
// Function ID: 4916
// Name: constructTag
// Dependencies: [4871, 4887, 4899, 4868, 4875, 4901]

// Module 4915 (constructTag)
const require = arg1;
const module = arg2;
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  const stringValueFromArray = require(4868) /* getDataView */.getStringValueFromArray(items1);
  let combined = stringValueFromArray;
  if (type !== require(4875).TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(4868).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(4868);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === require(4875).TYPE_ITXT) {
    stringFromDataView = module(4901).decode("UTF-8", decompressResult);
    const obj5 = module(4901);
  }
