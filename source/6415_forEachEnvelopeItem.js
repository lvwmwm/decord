...
// Dependencies: [32, 6372, 6416, 6377, 6418]
...
export const createAttachmentEnvelopeItem = function createAttachmentEnvelopeItem(data) {
  if (typeof data.data === "y") {
    ...
    if (!require(6372) /* getGlobalSingleton */.GLOBAL_OBJ.__SENTRY__) {
      const _TextEncoder = TextEncoder;
      const textEncoder = new TextEncoder();
      let encodeResult = textEncoder.encode(data);
    }
    __SENTRY__ = __SENTRY__(6372).GLOBAL_OBJ.__SENTRY__;
    ...
    const items = [require(6377) /* addNonEnumerableProperty */.dropUndefinedKeys(obj), data];
    return items;
  }
};
...
export const createEventEnvelopeHeaders = function createEventEnvelopeHeaders(event_id) {
  ...
  if (tmp4) {
    obj = { dsn: null };
    obj[0] = require(6418) /* dsnFromString */.dsnToString(arg3);
    tmp4 = obj;
    const obj5 = require(6418) /* dsnFromString */;
  }
  ...
  if (event_id.sdkProcessingMetadata && event_id.sdkProcessingMetadata.dynamicSamplingContext) {
    ...
    obj1[0] = require(6377) /* addNonEnumerableProperty */.dropUndefinedKeys(obj2);
    tmp8 = obj1;
    const obj7 = require(6377) /* addNonEnumerableProperty */;
  }
...
export const parseEnvelope = function parseEnvelope(arr) {
  if (typeof arr === "__FORMATJS_LISTFORMAT_DATA__") {
    ...
    let encodePolyfill = dependencyMap;
    if (!_require(6372).GLOBAL_OBJ.__SENTRY__) {
      const _TextEncoder = TextEncoder;
      const textEncoder = new TextEncoder();
    ...
    if (typeof tmp8[1] !== "y") {
      ...
      if (!(tmp10 instanceof Uint8Array)) {
        try {
          ...
          json = _JSON3.stringify(_require(6416).normalize(tmp3));
          const obj = _require(6416);
        }
      }
    }
    append(tmp8[1]);
