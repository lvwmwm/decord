...
// Dependencies: [7885, 7886, 7887]

// Module 7884 (appendTransformProps)
const require = arg1;
...
function appendTransformProps(arg0) {
  ...
  require(7885) /* append */.appendTransform(x + originX, y + originY, scaleX, scaleY, rotation, skewX, skewY, originX, originY);
}
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        tmp3Result = tmp3(7886);
        const parsed = tmp3Result.parse(arr);
        const tmp3Result1 = tmp3(7885);
  ...
  const obj = require(7885) /* append */;
  return require(7885) /* append */.toArray();
}
arg5.default = function extractTransform(arr) {
  ...
  if (typeof arr === "string") {
    try {
      const parsed = require(7886) /* peg$SyntaxError */.parse(arr);
      const items = [, , , , , ];
      ...
      const _console = tmp.console;
      _console.error(tmp6);
      return require(7885) /* append */.identity;
    }
  } else {
    let transform;
    if (arr != null) {
...
arg5.extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "string") {
    transform = require(7887) /* peg$SyntaxError */.parse(transform.transform);
    const obj = require(7887) /* peg$SyntaxError */;
  } else {
