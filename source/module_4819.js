...
obj = {
  ...
  getShortAt(dataView, sum, byteOrder) {
    return dataView.getUint16(sum, byteOrder === module(4807).LITTLE_ENDIAN);
  },
  getLongAt(dataView, sum, byteOrder) {
    return dataView.getUint32(sum, byteOrder === module(4807).LITTLE_ENDIAN);
  },
  getRationalAt(getUint32, sum) {
    const items = [getUint32.getUint32(sum, arg2 === module(4807).LITTLE_ENDIAN), ];
    sum = sum + 4;
    items[1] = getUint32.getUint32(sum, arg2 === module(4807).LITTLE_ENDIAN);
    return items;
  },
  ...
  getSlongAt(getInt32, sum) {
    return getInt32.getInt32(sum, arg2 === module(4807).LITTLE_ENDIAN);
  },
  getSrationalAt(getInt32, sum) {
    const items = [getInt32.getInt32(sum, arg2 === module(4807).LITTLE_ENDIAN), ];
    sum = sum + 4;
    items[1] = getInt32.getInt32(sum, arg2 === module(4807).LITTLE_ENDIAN);
    return items;
  },
  getIfdPointerAt(getUint32, sum) {
    return getUint32.getUint32(sum, arg2 === module(4807).LITTLE_ENDIAN);
  },
  typeSizes: obj,
  tagTypes: obj,
  getTypeSize(LONG) {
