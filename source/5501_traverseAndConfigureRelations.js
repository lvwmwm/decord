...
// Dependencies: [5494, 5486, 5432]

// Module 5501 (traverseAndConfigureRelations)
const require = arg1;
...
arg5.ensureNativeDetectorComponent = function ensureNativeDetectorComponent(ReanimatedNativeDetector) {
  if (!ReanimatedNativeDetector) {
    const _Error = Error;
    const error = new Error(require(5432) /* tagMessage */.tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
};
