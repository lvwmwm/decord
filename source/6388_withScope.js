...
// Function ID: 6389
// Name: withScope
// Dependencies: [41, 42, 6389, 6375, 6386, 6394]
// Exports: getStackAsyncContextStrategy

// Module 6388 (withScope)
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(6386).getMainCarrier();
  const obj = AsyncContextStack(6386);
  const sentryCarrier = AsyncContextStack(6386).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(6386).getMainCarrier();
      let obj = callback(6386);
      let sentryCarrier = callback(6386).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(6394);
    ...
    getCurrentScope() {
      const mainCarrier = callback(6386).getMainCarrier();
      const obj = callback(6386);
      const sentryCarrier = callback(6386).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(6394);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
