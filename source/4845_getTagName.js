...
// Function ID: 4846
// Name: getTagName
// Dependencies: [32, 4806, 4823, 4846]
...
export default {
  read(arg0, arg1) {
    ...
    if (0 < arg0.length) {
      ...
      const stringFromDataView = require(4806) /* getDataView */.getStringFromDataView(dataView, num, closure_7);
      const obj3 = require(4806) /* getDataView */;
      const shortAt = importDefault(4823).getShortAt(dataView, sum);
      const sum1 = sum + c5;
      const tmp15 = getTagName(dataView, sum1);
      let name = tmp15.tagName;
      const sum2 = sum1 + tmp15.tagNameSize;
      const obj4 = importDefault(4823);
      const longAt = importDefault(4823).getLongAt(dataView, sum2);
      const sum3 = sum2 + c6;
      if (stringFromDataView === c4) {
        let tmp5Result = tmp5(4806);
        const dataView1 = tmp5Result.getDataView(dataView.buffer, sum3, longAt);
        obj = { id: null, value: null };
        obj[0] = shortAt;
        tmp5Result = tmp5(4806);
        obj[1] = tmp5Result.getStringFromDataView(dataView1, 0, longAt);
        if (tmp10(4846)[shortAt]) {
          try {
            obj.description = tmp10(4846)[shortAt].description(dataView1);
            if (!name) {
              name = tmp10(4846)[shortAt].name;
            }
            obj[name] = obj;
            const obj6 = tmp10(4846)[shortAt];
          } catch (err) {
