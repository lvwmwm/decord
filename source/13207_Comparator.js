...
// Function ID: 13208
// Name: Comparator
// Dependencies: [41, 42, 13180, 13182, 13183, 13179, 13205, 13208]

// Module 13207 (Comparator)
...
class Comparator {
  constructor(arg0, arg1) {
    ...
    parts = str2.split(/\s+/);
    joined = parts.join(" ");
    tmp6 = require("module_13182")("comparator", joined, tmp4);
    self.options = tmp4;
    ...
    tmp8 = require("module_13182")("comp", self);
    return;
  }
}
let obj = {
  ...
  value: function parse(str) {
    const self = this;
    const safeRe = Comparator(13183).safeRe;
    const t = Comparator(13183).t;
    ...
    if (match) {
      ...
      if (match[2]) {
        const tmp15 = new tmp5(13179)(match[2], self.options.loose);
        self.semver = tmp15;
      } else {
...
const items = [
  ...
  },
  {
    ...
    value: function test(arg0) {
      const self = this;
      let tmp = arg0;
      Comparator(13182)("Comparator.test", arg0, this.options.loose);
      if (this.semver !== closure_3) {
        if (tmp !== tmp5) {
          if (typeof tmp === "string") {
            try {
              tmp = new tmp2(13179)(tmp, self.options);
            } catch (err) {
              return false;
            }
          }
          return tmp2(13205)(tmp, self.operator, self.semver, self.options);
        }
      }
      return true;
    }
    ...
    value: function intersects(value) {
      if (value instanceof Comparator) {
        ...
        if ("" === this.operator) {
          ...
          if (!isMatch) {
            const obj2 = new Comparator(13208)(value.value, arg1);
          ...
          if (!isMatch1) {
            const obj = new Comparator(13208)(self.value, arg1);
            isMatch1 = obj.test(value.semver);
          }
          tmp8 = isMatch1;
        } else {
          const tmp45 = Comparator(13180)(arg1);
            ...
            if (!tmp9) {
              ...
              if (tmp13) {
                ...
                if (tmp16) {
                  ...
                  let tmp19 = !tmp18;
                  if (tmp18) {
                    let startsWithResult3 = Comparator(13205)(self.semver, "<", value.semver, tmp45);
                    ...
                    if (!tmp24) {
                      let startsWithResult4 = Comparator(13205)(self.semver, ">", value.semver, tmp45);
