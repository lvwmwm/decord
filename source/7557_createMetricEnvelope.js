...
// Dependencies: [7454, 7501, 7498, 7555]

// Module 7557 (createMetricEnvelope)
const require = arg1;
...
function createMetricEnvelope(arg0, arg1, sdk) {
  ...
  if (tmp) {
    obj.dsn = require(7501) /* dsnFromString */.dsnToString(arg1);
    const obj4 = require(7501) /* dsnFromString */;
  }
  const date = new Date();
  const result = require(7555) /* items */.serializeMetricBuckets(arg0);
  obj = { type: "statsd", length: result.length };
  const items = [obj, result];
  const obj5 = require(7555) /* items */;
  const items1 = [items];
...
arg5.captureAggregateMetrics = function captureAggregateMetrics(_client, arr) {
  const logger = require(7454) /* consoleSandbox */.logger;
