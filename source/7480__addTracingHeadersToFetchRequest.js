...
// Dependencies: [853, 7371, 7374, 7375, 7389, 7399, 7403, 7407, 7457, 7380, 7409, 7390, 7408, 7446, 7392, 7388, 7382]
// Exports: addTracingHeadersToFetchRequest, instrumentFetchRequest

// Module 7480 (_addTracingHeadersToFetchRequest)
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(7392).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(7392);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(7392).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(7457).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(7457);
    }
    const activeSpan = setHttpStatus(7380).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(7390).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(7390).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(7409).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(7409);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(7407);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(7408).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(7380);
  }
};
