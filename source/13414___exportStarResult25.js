...
// Dependencies: [13386, 13412, 13411, 13410, 13399]

// Module 13414 (__exportStarResult25)
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13386).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13412).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13412).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13411).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13410).FormatApproximately(internalSlots, result);
    ...
    return tmp(13399).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13411).FormatNumeric(internalSlots, isNaN);
};
