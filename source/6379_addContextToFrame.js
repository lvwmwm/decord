...
// Dependencies: [6369, 6377, 6374]

// Module 6379 (addContextToFrame)
const require = arg1;
...
arg5.addContextToFrame = function addContextToFrame(arr, lineno) {
  ...
  if (undefined !== lineno.lineno) {
    ...
    lineno.pre_context = substr.map((arg0) => callback(6377).snipLine(arg0, 0));
    const _Math5 = Math;
    ...
    lineno.context_line = require(6377) /* isMatchingPattern */.snipLine(arr[bound1], num2);
    const _Math = Math;
    const substr1 = arr.slice(Math.min(bound + 1, length), bound + 1 + num);
    lineno.post_context = substr1.map((arg0) => callback(6377).snipLine(arg0, 0));
...
arg5.checkOrSetAlreadyCaught = function checkOrSetAlreadyCaught(__sentry_captured__) {
  if ((function isAlreadyCaptured(__sentry_captured__) {
    ...
    try {
      const result = require(6374) /* addNonEnumerableProperty */.addNonEnumerableProperty(__sentry_captured__, "__sentry_captured__", true);
      return false;
    } catch (err) {
    }
  }
