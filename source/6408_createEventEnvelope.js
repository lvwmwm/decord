...
// Dependencies: [6364, 6365, 6409, 6412, 6401, 6370]
// Exports: createEventEnvelope, createSessionEnvelope, createSpanEnvelope

// Module 6408 (createEventEnvelope)
...
export const createEventEnvelope = function createEventEnvelope(type, arg1, sdk) {
  const sdkMetadataForEnvelopeHeader = require(6409) /* forEachEnvelopeItem */.getSdkMetadataForEnvelopeHeader(sdk);
  let str = "event";
  if (type.type) {
    str = "event";
...
export const createSessionEnvelope = function createSessionEnvelope(toJSON) {
  let obj = require(6409) /* forEachEnvelopeItem */;
...
export const createSpanEnvelope = function createSpanEnvelope(arg0, getDsn) {
  ...
  if (tmp9) {
    const obj1 = { dsn: null };
    obj1[0] = beforeSendSpan(6412).dsnToString(dsn);
    tmp9 = obj1;
    const tmp2Result = beforeSendSpan(6412);
  }
