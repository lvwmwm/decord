...
// Function ID: 13422
// Name: isMatched
// Dependencies: [13422, 1281, 13423]

// Module 13421 (isMatched)
const require = arg1;
  ...
  if (arr) {
    if ("$" === arr[0]) {
      ...
      const mapped = arr2.map((arg0) => {
        let tmp = callback(13423).regions[arg0];
        if (!tmp) {
          const items = [arg0];
          tmp = items;
        }
...
function findMatchingDistance(arg0, arg1) {
  ...
  if (!obj1) {
    ...
    const prop = tmp8(13422).data.supplemental.languageMatching["written-new"];
    let substr = prop.slice(1, 5);
    const prop1 = tmp8(13422).data.supplemental.languageMatching["written-new"];
