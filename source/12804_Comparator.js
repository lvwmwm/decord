...
// Function ID: 99554
// Name: Comparator
// Dependencies: [6, 7, 12777, 12779, 12780, 12776, 12802, 12805]

// Module 12804 (Comparator)
import _classCallCheck from "cmp";
import _defineProperties from "Range";
...
export default (() => {
  class Comparator {
    constructor(arg0, arg1) {
      ...
      tmp2 = Comparator(outer1_1[2])(arg1);
      str = arg0;
      ...
      parts = str2.split(/\s+/);
      joined = parts.join(" ");
      tmp4 = Comparator(outer1_1[3])("comparator", joined, tmp2);
      self.options = tmp2;
      self.loose = tmp2.loose;
      parsed = self.parse(joined);
      if (self.semver === outer1_4) {
        str3 = "";
        self.value = "";
      } else {
        self.value = self.operator + self.semver.version;
      }
      tmp6 = Comparator(outer1_1[3])("comp", self);
      return;
    }
  }
  let obj = {
    ...
    value: function parse(str) {
      const self = this;
      const safeRe = Comparator(outer1_1[4]).safeRe;
      const t = Comparator(outer1_1[4]).t;
  ...
  obj = {
    ...
    value: function test(arg0) {
      const self = this;
      let tmp = arg0;
      Comparator(outer1_1[3])("Comparator.test", arg0, this.options.loose);
      if (this.semver !== outer1_4) {
        if (tmp !== outer1_4) {
          if ("string" === typeof tmp) {
            const tmp6 = Comparator(outer1_1[5]);
            const prototype = tmp6.prototype;
            tmp = new tmp6(tmp, self.options);
          }
          return Comparator(outer1_1[6])(tmp, self.operator, self.semver, self.options);
        }
      }
      return true;
    }
    ...
    value: function intersects(value) {
      ...
      if (value instanceof Comparator) {
        if ("" === self.operator) {
          ...
          if (!isMatch) {
            let tmp38 = Comparator(outer1_1[7]);
          ...
          if (!isMatch1) {
            let tmp30 = Comparator(outer1_1[7]);
          ...
          const tmp45 = Comparator(outer1_1[2])(arg1);
            ...
            if (tmp10) {
              ...
              if (tmp11) {
                ...
                if (tmp13) {
                  ...
                  let tmp16 = !tmp15;
                  if (tmp15) {
                    let startsWithResult1 = Comparator(outer1_1[6])(self.semver, "<", value.semver, tmp45);
                    ...
                    let tmp22 = !tmp21;
                    if (!startsWithResult1) {
                      let startsWithResult2 = Comparator(outer1_1[6])(self.semver, ">", value.semver, tmp45);
  ...
  return _defineProperties(Comparator, items, items1);
})();
