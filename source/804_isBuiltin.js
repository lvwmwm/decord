...
arg5.isRequest = function isRequest(first) {
  let tmp = "undefined" !== typeof Request;
  if (tmp) {
    const _Request = Request;
    tmp = isInstanceOf(first, Request);
  }
  return tmp;
};
...
arg5.isVueViewModel = function isVueViewModel(__isVue) {
  ...
  if (!tmp) {
    ...
    const tmp3 = __isVue.__isVue || __isVue._isVue || __isVue.__v_isVNode;
  }
  return !tmp;
};
