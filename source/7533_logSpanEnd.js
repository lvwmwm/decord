...
// Function ID: 7534
// Name: logSpanEnd
// Dependencies: [7523, 7500, 7495]

// Module 7533 (logSpanEnd)
const require = arg1;
...
arg5.logSpanStart = function logSpanStart(spanContext) {
  ...
  if (require(7523) /* __SENTRY_DEBUG__ */.DEBUG_BUILD) {
    ...
    const rootSpan = tmp(7500).getRootSpan(spanContext);
    let str3 = "unsampled";
    ...
    if (rootSpan !== spanContext) {
      ...
      ({ op: op2, description: description2 } = tmp(7500).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(7500).spanToJSON(rootSpan);
    }
    const logger = tmp(7495).logger;
    const _HermesInternal9 = HermesInternal;
    logger.log("" + combined + "\n  " + items.join("\n  "));
    const tmpResult1 = tmp(7500);
  }
};
