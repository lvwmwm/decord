...
// Dependencies: [41, 42, 4943, 4946, 4952, 4953, 4954, 4955, 4956]

// Module 4945 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
...
let items = [
  {
    ...
    value: function getInfoByName(key10027) {
      return FileTypes(4943).fetchFromObject(FileTypes, key10027.toUpperCase());
    }
  },
  {
    key: "getSignaturesByName",
    value: function getSignaturesByName(item10079) {
      return FileTypes(4943).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((arg0) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(4946).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(4946).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(4946).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(4943).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(4946).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((arg0) => {
          ...
          if (FileTypes(4943).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
