...
for (const key10016 in require("keys")) {
  let tmp2 = key10016;
  if (!hasOwnProperty.call(require("keys"), key10016)) {
    continue;
  } else {
    obj[require("keys")[key10016]] = key10016;
    continue;
  }
  continue;
}
...
obj.get.rgb = (str) => {
  if (str) {
        ...
        if (match2) {
          ...
          if (match3) {
            ...
            if (!match4) {
              ...
              items1 = null;
              if (hasOwnProperty.call(require(5564) /* keys */, match4[1])) {
                const tmp15 = require(5564) /* keys */[match4[1]];
                tmp15[3] = 1;
                items1 = tmp15;
              }
            }
    ...
    do {
      ...
      let num25 = 0;
      let num26 = 255;
      items[num8] = clamp(items[num8], 0, 255);
      num8 = num8 + 1;
      let num27 = 3;
    } while (num8 < 3);
...
obj.to.hex = () => {
  const tmp = require(5567) /* swizzle */(arguments);
  const text = `#${hexDouble(tmp[0])}`;
  const text1 = `#${hexDouble(tmp[0])}${hexDouble(tmp[1])}`;
  let str = "";
  const text2 = `#${hexDouble(tmp[0])}${hexDouble(tmp[1])}${hexDouble(tmp[2])}`;
  if (tmp[3] < 1) {
    const _Math = Math;
...
obj.to.rgb = () => {
  const arr = require(5567) /* swizzle */(arguments);
  if (arr.length >= 4) {
    if (1 !== arr[3]) {
      const _Math = Math;
      const _Math2 = Math;
