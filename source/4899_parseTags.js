...
// Function ID: 4900
// Name: parseTags
// Dependencies: [4900, 4901]

// Module 4899 (parseTags)
const module = arg2;
      ...
      while (true) {
        ...
        if (byteLength.getUint8(tmp) !== 28) {
          ...
          if (!arg3) {
            ...
            if (!module(4900).iptc[uint16]) {
              obj = { tag: "Array", tagSize: true };
              obj[1] = uint161;
            }
          }
          items = [];
          ...
          let tmp15 = obj;
          let tmp16 = encoding;
          obj[3] = getTagDescription(tmp8(4900).iptc[uint16], items, obj, encoding);
          let tmp17 = tmp8(4900).iptc[uint16] && tmp8(4900).iptc[uint16].repeatable;
          if (tmp17) {
            obj.repeatable = true;
          }
          let tmp18 = tmp8(4900).iptc[uint16] && undefined !== tmp8(4900).iptc[uint16].encoding_name;
          if (tmp18) {
            let obj4 = tmp8(4900).iptc[uint16];
...
function getTagDescription(description, items, arg2, encoding) {
  if (!(function hasDescriptionProperty(description) {
    ...
    if ((function tagValueIsText(description, items) {
      ...
      decodeResult = module(4901).decode(encoding, items);
      const obj = module(4901);
    }
    return decodeResult;
  } else {
    try {
