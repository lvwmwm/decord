...
// Dependencies: [13353, 13379, 13378, 13377, 13366]

// Module 13381 (__exportStarResult25)
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13353).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13379).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13379).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13378).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13377).FormatApproximately(internalSlots, result);
    ...
    return tmp(13366).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13378).FormatNumeric(internalSlots, isNaN);
};
