...
export const purchaseUpdatedListener = (arg0, arg1) => {
  ...
  const _require = arg0;
  dependencyMap = arg1;
  const obj = _require(7568);
  const obj2 = new NativeEventEmitter(_require(7568).getNativeModule());
  ...
  obj3 = _require(7566);
  if (_require(7568).isAndroid) {
    const androidModule = tmp(7568).getAndroidModule();
...
export const purchaseErrorListener = (arg0) => {
  const obj = _mod7568;
  return new NativeEventEmitter(_mod7568.getNativeModule()).addListener("purchase-error", arg0);
};
