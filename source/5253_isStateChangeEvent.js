...
// Dependencies: [27, 5254, 5256, 5257, 5259]
// Exports: startListening

// Module 5253 (isStateChangeEvent)
...
function onGestureHandlerEvent(handlerTag) {
  let obj = require(5254) /* findHandler */;
  const findHandlerResult = obj.findHandler(handlerTag.handlerTag);
  if (findHandlerResult) {
    if (isStateChangeEvent(handlerTag)) {
      if (handlerTag.oldState === require(5256).State.UNDETERMINED) {
        if (handlerTag.state === require(5256).State.BEGAN) {
      ...
      if (handlerTag.oldState === require(5256).State.BEGAN) {
        if (handlerTag.state === require(5256).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === require(5256).State.END) {
          if (handlerTag.oldState === require(5256).State.ACTIVE) {
      ...
      let tmp51 = handlerTag.state !== require(5256).State.FAILED;
      if (tmp51) {
        tmp51 = handlerTag.state !== require(5256).State.CANCELLED;
      }
      if (!tmp51) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = require(5257) /* setGestureState */.GestureStateManager;
      ...
      if (require(5259).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (require(5259).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (require(5259).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (require(5259).TouchEventType.TOUCHES_CANCELLED === eventType) {
    ...
    const result2 = require(5254) /* findHandler */.findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
    ...
    const obj2 = require(5254) /* findHandler */;
  }
}
