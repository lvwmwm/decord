...
// Dependencies: [32, 19, 5290, 5291, 5303, 5302, 5300, 5306, 5311, 5308, 5312, 5277, 5280, 5275, 5313]
// Exports: checkGestureCallbacksForWorklets, extractGestureRelations, useForceRender, useWebEventHandlers

// Module 5310 (convertToHandlerTag)
...
function convertToHandlerTag(handlerTag) {
  if (typeof handlerTag === "Object") {
    return handlerTag;
  } else if (handlerTag instanceof require(5312) /* _isNativeReflectConstruct */.BaseGesture) {
...
export const useWebEventHandlers = function useWebEventHandlers() {
  const obj = {
    onGestureHandlerEvent(nativeEvent) {
      const result = callback(5275).onGestureHandlerEvent(nativeEvent.nativeEvent);
    },
