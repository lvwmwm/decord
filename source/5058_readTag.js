...
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  ...
  sum = typeSize + _modDef5055.getTypeSize("SHORT");
  const obj2 = _modDef5055;
  const sum1 = sum + _modDef5055.getTypeSize("LONG");
  const obj3 = _modDef5055;
  const shortAt = _modDef5055.getShortAt(byteLength, sum, byteOrder);
  const obj4 = _modDef5055;
  const shortAt1 = _modDef5055.getShortAt(byteLength, sum + typeSize, byteOrder);
  const obj5 = _modDef5055;
  const longAt = _modDef5055.getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== _modDef5055.typeSizes[shortAt1]) {
    if ((function tagValueFitsInOffsetSlot(shortAt1, longAt) {
      const result = callback(5055).typeSizes[shortAt1] * longAt;
      return result <= callback(5055).getTypeSize("LONG");
    })(shortAt1, longAt)) {
      const sum2 = sum + sum1;
      let str = getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder);
      let tmp21 = sum2;
    } else {
      const longAt1 = tmp4(5055).getLongAt(byteLength, sum + sum1, byteOrder);
      str = "<faulty value>";
      tmp21 = longAt1;
      if ((function tagValueFitsInDataView(byteLength, sum, longAt1, shortAt1, longAt) {
        sum = sum + longAt1;
        return sum + callback(5055).typeSizes[shortAt1] * longAt <= byteLength.byteLength;
      })(byteLength, sum, longAt1, shortAt1, longAt)) {
        str = getTagValue(byteLength, sum + longAt1, shortAt1, longAt, byteOrder, 33723 === shortAt);
        tmp21 = longAt1;
      }
      const tmp4Result = tmp4(5055);
    }
    let tmp34 = str;
    if (shortAt1 === tmp4(5055).tagTypes.ASCII) {
    ...
    if (undefined !== tmp4(5059)[IFD_TYPE_0TH][shortAt]) {
      if (undefined !== tmp4(5059)[IFD_TYPE_0TH][shortAt].name) {
        if (undefined !== tmp4(5059)[IFD_TYPE_0TH][shortAt].description) {
          try {
            descriptionResult = tmp4(5059)[IFD_TYPE_0TH][shortAt].description(tmp34);
            combined = tmp39;
            const obj8 = tmp4(5059)[IFD_TYPE_0TH][shortAt];
          } catch (err) {
      ...
      if (shortAt1 !== tmp4(5055).tagTypes.RATIONAL) {
        if (shortAt1 !== tmp4(5055).tagTypes.SRATIONAL) {
          combined = tmp4(5059)[IFD_TYPE_0TH][shortAt];
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * _modDef5055.typeSizes[shortAt1];
    BYTE = _modDef5055.tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    ...
    sum = sum + _modDef5055.typeSizes[BYTE];
  }
  if (BYTE === _modDef5055.tagTypes.ASCII) {
    let asciiValue = _modDef5055.getAsciiValue(items);
    const tmp9Result = _modDef5055;
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
let obj = { 1: null, 2: _modDef5055.getByteAt, 3: _modDef5055.getAsciiAt, 4: _modDef5055.getShortAt, 5: _modDef5055.getLongAt, 7: _modDef5055.getRationalAt, 9: null, 10: _modDef5055.getUndefinedAt, 13: null };
obj[9] = _modDef5055.getSlongAt;
obj[10] = _modDef5055.getSrationalAt;
obj[13] = _modDef5055.getIfdPointerAt;
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5, arg6) {
  obj = _modDef5055;
  const typeSize = obj.getTypeSize("SHORT");
  let num = 0;
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = _modDef5055.getShortAt(byteLength, sum, byteOrder);
    const tmpResult = _modDef5055;
  }
  obj = {};
  sum = sum + typeSize;
  let tmp5 = sum;
  ...
  if (_modDef5041.USE_THUMBNAIL) {
    let tmp23Result = tmp23(5055);
    if (tmp5 < byteLength.byteLength - tmp23Result.getTypeSize("LONG")) {
      tmp23Result = tmp23(5055);
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, sum, byteOrder) {
  return sum + _modDef5055.getLongAt(buffer, sum + 4, byteOrder);
};
export { readIfd };
