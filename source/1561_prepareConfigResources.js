...
// Dependencies: [32, 1562, 1563, 1564, 1560, 1526, 1555]
// Exports: getStateFromPath

// Module 1561 (prepareConfigResources)
...
function checkForDuplicatedConfigs(map, routeNames2, joined) {
  if (map) {
    ...
    if (routeNames.length > routeNames1.length) {
      let arrayStartsWithResult = require(1564) /* arrayStartsWith */.arrayStartsWith(routeNames, routeNames1);
      const obj2 = require(1564) /* arrayStartsWith */;
    } else {
      arrayStartsWithResult = require(1564) /* arrayStartsWith */.arrayStartsWith(routeNames1, routeNames);
      const obj = require(1564) /* arrayStartsWith */;
    }
...
function getStaticSegmentPattern(arg0) {
  return Array.from(arg0, (str) => {
    ...
    return "(?:" + callback(1562)(str) + "|" + callback(1562)(encodeURIComponentResult) + ")";
  }).join("");
}
...
function createNestedStateObject(str, items, initialRoutes, config) {
  ...
  if (0 === items.length) {
    ...
    const findFocusedRouteResult = require(1526) /* findFocusedRoute */.findFocusedRoute(obj);
...
function parseQueryParams(arr) {
  ...
  if (substr) {
    parsed = importAll(1555).parse(substr);
    const obj2 = importAll(1555);
  } else {
    parsed = {};
  }
  set[Symbol.iterator]();
