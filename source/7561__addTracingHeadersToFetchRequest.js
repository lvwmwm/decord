...
// Dependencies: [853, 7452, 7455, 7456, 7470, 7480, 7484, 7488, 7538, 7461, 7490, 7471, 7489, 7527, 7473, 7469, 7463]
// Exports: addTracingHeadersToFetchRequest, instrumentFetchRequest

// Module 7561 (_addTracingHeadersToFetchRequest)
...
function _addTracingHeadersToFetchRequest(headers, headers2, span) {
  let baggage;
  let tmp4;
  let obj = require(7527) /* getTraceData */;
  obj = { span };
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
    ...
    let obj = require(7488) /* hasTracingEnabled */;
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(7473).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(7473);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(7473).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(7538).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(7538);
    }
    const activeSpan = setHttpStatus(7461).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(7471).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(7471).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(7490).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(7490);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(7488);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(7489).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(7461);
  }
};
