...
// Dependencies: [32, 41, 42, 7444, 7361, 7448, 7449, 7352, 7450]

// Module 7447 (MetricsAggregator)
...
class MetricsAggregator {
  constructor(arg0) {
    ...
    tmp4 = closure_1;
    this._interval = setInterval(() => self._flush(), require("module_7444").DEFAULT_FLUSH_INTERVAL);
    if (this._interval.unref) {
      _interval = self._interval;
      unrefResult = _interval.unref();
    }
    random = Math.random();
    self._flushShift = Math.floor(random * require("module_7444").DEFAULT_FLUSH_INTERVAL / 1000);
...
const items = [
  {
    ...
    value: function add(arg0, arg1, diff) {
      ...
      if (arg5 === undefined) {
        timestampInSecondsResult = MetricsAggregator(7361).timestampInSeconds();
        const obj2 = MetricsAggregator(7361);
      }
      const self = this;
      const rounded = Math.floor(timestampInSecondsResult);
      const sanitizeMetricKeyResult = MetricsAggregator(7448).sanitizeMetricKey(arg1);
      const obj3 = MetricsAggregator(7448);
      const sanitizeTagsResult = MetricsAggregator(7448).sanitizeTags(obj);
      const obj4 = MetricsAggregator(7448);
      const sanitizeUnitResult = MetricsAggregator(7448).sanitizeUnit(str);
      const obj5 = MetricsAggregator(7448);
      const bucketKey = MetricsAggregator(7448).getBucketKey(arg0, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
      ...
      if (value) {
        ...
        const tmp15 = new tmp5(7449).METRIC_MAP[arg0](diff);
        obj[0] = tmp15;
        obj[1] = rounded;
        obj[2] = arg0;
      ...
      const obj6 = MetricsAggregator(7448);
      const result1 = MetricsAggregator(7352).updateMetricSummaryOnActiveSpan(arg0, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
      self._bucketsTotalWeight = self._bucketsTotalWeight + obj.metric.weight;
      if (self._bucketsTotalWeight >= MetricsAggregator(7444).MAX_WEIGHT) {
        self.flush();
      }
    }
  },
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(7361).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(7444).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(7450).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(7450);
      }
    }
  }
];
