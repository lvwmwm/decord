...
// Dependencies: [109, 32, 19, 17, 21, 4640, 4647, 4613, 4605, 4615]
// Exports: default

// Module 4646 (mapItemAppearanceToNativeProp)
...
export default function TabsScreen(onWillAppear) {
  ...
  if (obj1.freezeEnabled()) {
    ...
    if (undefined === freezeContents) {
      let tmp10 = !tmp5;
      if (tmp8(4615).featureFlags.experiment.controlledBottomTabs) {
        if (!tmp5) {
          tmp10 = !tmp6;
        }
        flag = tmp10;
  ...
  ref(4640).bottomTabsDebugLog("TabsScreen [" + num + "] render; tabKey: " + tmp7.tabKey + " shouldFreeze: " + flag + ", isFocused: " + undefined !== isFocused && isFocused + " nativeViewIsVisible: " + tmp5);
  let android;
  if (icon != null) {
  ...
  obj3.children = jsx(ref(4613).Freeze, { freeze: flag, placeholder: tmp7.placeholder, children: tmp7.children });
  return jsx(ref1(4647), { collapsable: false, style: items4, onWillAppear: callback, onDidAppear: callback1, onWillDisappear: callback2, onDidDisappear: callback3, isFocused: undefined !== isFocused && isFocused });
};
