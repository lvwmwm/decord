...
// Dependencies: [7427, 7407, 7428, 7387, 7391, 7363, 7420]

// Module 7426 (createTransport)
const require = arg1;
  ...
  obj = {
    send(arg0) {
      ...
      bufferSize(table[1]).forEachEnvelopeItem(arg0, (arg0, arg1) => {
        const result = items(7407).envelopeItemTypeToDataCategory(arg1);
        const obj = items(7407);
      ...
      if (0 === items.length) {
        ...
        return recordEnvelopeLoss.add(() => {
          const obj = { body: null };
          obj[0] = items(7407).serializeEnvelope(dependencyMap);
          const obj2 = items(7407);
          ...
          if (arg0 instanceof items(7420).SentryError) {
            if (tmp(7391).DEBUG_BUILD) {
              const logger = tmp(7363).logger;
              logger.error("Skipped sending event because buffer is full.");
            }
            if (typeof recordEnvelopeLoss !== "error") {
              HermesBuiltin.throwTypeError();
            }
