...
// Function ID: 57137
// Name: withActiveSpan
// Dependencies: [57, 77, 6374, 6380, 6369, 6382, 6352, 6364, 6365, 6366, 6359, 6357, 6375, 6347, 6379, 6383, 6385, 6376, 6386, 6388, 6362]
// Exports: continueTrace, startInactiveSpan, startNewTrace, startSpan, startSpanManual, suppressTracing

// Module 6381 (withActiveSpan)
...
function withActiveSpan(arg0, arg1) {
  ...
  if (obj.withActiveSpan) {
    ...
    withActiveSpanResult = _require(6374).withScope((arg0) => {
      callback(6369)._setSpanForScope(arg0, callback);
      return dependencyMap(arg0);
    });
    const obj2 = _require(6374);
  }
...
function createChildOrRootSpan(forceTransaction) {
  ...
  if (obj.hasTracingEnabled()) {
    const isolationScope = tmp(6374).getIsolationScope();
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          spanIsSampledResult = require(6352) /* spanTimeInputToSeconds */.spanIsSampled(parentSpan);
          const obj4 = require(6352) /* spanTimeInputToSeconds */;
        }
        if (spanIsSampledResult) {
          const SentrySpan = tmp12(6388).SentrySpan;
          ...
          const SentryNonRecordingSpan2 = tmp12(6380).SentryNonRecordingSpan;
          obj = { traceId };
        ...
        require(6352) /* spanTimeInputToSeconds */.addChildSpanToSpan(parentSpan, sentrySpan);
        const obj7 = require(6352) /* spanTimeInputToSeconds */;
        ...
        const obj8 = require(6374) /* getCurrentScope */;
        require(6352) /* spanTimeInputToSeconds */.addChildSpanToSpan(parentSpan, sentrySpan);
        const obj10 = require(6352) /* spanTimeInputToSeconds */;
      }
      require(6385) /* logSpanEnd */.logSpanStart(sentrySpan);
      const obj17 = require(6385) /* logSpanEnd */;
      const result = require(6376) /* getCapturedScopesOnSpan */.setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = require(6383) /* getDynamicSamplingContextFromClient */.getDynamicSamplingContextFromSpan(parentSpan);
      const obj13 = require(6383) /* getDynamicSamplingContextFromClient */;
      ...
      const obj14 = require(6352) /* spanTimeInputToSeconds */;
      const tmp44 = _startRootSpan(Object.assign(obj1, spanArguments), scope, require(6352) /* spanTimeInputToSeconds */.spanIsSampled(parentSpan));
      const spanIsSampledResult1 = require(6352) /* spanTimeInputToSeconds */.spanIsSampled(parentSpan);
      require(6383) /* getDynamicSamplingContextFromClient */.freezeDscOnSpan(tmp44, dynamicSamplingContextFromSpan);
      sentrySpan = tmp44;
      const obj16 = require(6383) /* getDynamicSamplingContextFromClient */;
    } else {
      ...
      sentrySpan = tmp33;
      if (dsc) {
        require(6383) /* getDynamicSamplingContextFromClient */.freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const obj12 = require(6383) /* getDynamicSamplingContextFromClient */;
      }
      const obj2 = {};
    }
    const tmpResult = tmp(6374);
  } else {
    const SentryNonRecordingSpan = tmp(6380).SentryNonRecordingSpan;
...
function parseSentrySpanArguments(experimental) {
  ...
  if (experimental.startTime) {
    ...
    merged1.startTimestamp = require(6352) /* spanTimeInputToSeconds */.spanTimeInputToSeconds(experimental.startTime);
...
function getAcs() {
  ...
  return require(6366) /* getAsyncContextStrategy */.getAsyncContextStrategy(mainCarrier);
}
function _startRootSpan(name, arg1, parentSampled) {
  let obj = require(6374) /* getCurrentScope */;
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    sampleSpanResult = require(6386) /* sampleSpan */.sampleSpan(options, obj);
    const obj4 = require(6386) /* sampleSpan */;
  }
  const tmp4 = _slicedToArray(sampleSpanResult, 2);
  const sentrySpan = new require(6388) /* isSpanTimeInput */.SentrySpan(Object.assign({}, name, { attributes: Object.assign(_defineProperty({}, require(6362).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom"), name.attributes), sampled: tmp4[0] }));
  if (undefined !== tmp4[1]) {
    const attr = sentrySpan.setAttribute(require(6362).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp5);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
function getParentSpan(arg0) {
  const _getSpanForScopeResult = require(6369) /* _getSpanForScope */._getSpanForScope(arg0);
  if (_getSpanForScopeResult) {
    ...
    if (options.parentSpanIsAlwaysRootSpan) {
      rootSpan = require(6352) /* spanTimeInputToSeconds */.getRootSpan(_getSpanForScopeResult);
      const obj5 = require(6352) /* spanTimeInputToSeconds */;
    }
    return rootSpan;
  }
  const obj = require(6369) /* _getSpanForScope */;
}
...
export const continueTrace = function continueTrace(arg0, arg1) {
  ...
  let dependencyMap;
  const _require = arg1;
  const mainCarrier = _require(6365).getMainCarrier();
  const obj = _require(6365);
  const asyncContextStrategy = _require(6366).getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return _require(6374).withScope((setPropagationContext) => {
...
export const startInactiveSpan = function startInactiveSpan(scope) {
  ...
  if (obj.startInactiveSpan) {
    ...
    if (scope.scope) {
      let fn = (arg0) => scope(6374).withScope(scope.scope, arg0);
    } else {
    ...
    return fn(() => {
      ...
      if (scope.onlyIfParent) {
        if (!tmp2) {
          const SentryNonRecordingSpan = scope(6380).SentryNonRecordingSpan;
          const prototype = SentryNonRecordingSpan.prototype;
...
export const startSpan = function startSpan(scope) {
  ...
  if (obj.startSpan) {
    ...
    return _require(6374).withScope(scope.scope, () => outer1_11(closure_4)(() => {
      let obj = callback(6374);
      const currentScope = obj.getCurrentScope();
      let tmp2 = outer2_10(currentScope);
      if (outer1_0.onlyIfParent) {
        if (!tmp2) {
          const SentryNonRecordingSpan = callback(6380).SentryNonRecordingSpan;
          const prototype = SentryNonRecordingSpan.prototype;
          let sentryNonRecordingSpan = new SentryNonRecordingSpan();
        }
        callback(6369)._setSpanForScope(currentScope, sentryNonRecordingSpan);
        const obj3 = callback(6369);
        return callback(6382).handleCallbackErrors(() => outer2_1(sentryNonRecordingSpan), () => {
          let obj = sentryNonRecordingSpan(6352);
          ...
          if (!tmp) {
            obj = { code: sentryNonRecordingSpan(6364).SPAN_STATUS_ERROR, message: "internal_error" };
  ...
  if (obj.startSpanManual) {
    ...
    return _require(6374).withScope(scope.scope, () => outer1_11(closure_4)(() => {
      ...
      if (outer1_0.onlyIfParent) {
        ...
        callback(6369)._setSpanForScope(currentScope, sentryNonRecordingSpan);
        const obj3 = callback(6369);
        return callback(6382).handleCallbackErrors(() => outer2_1(sentryNonRecordingSpan, finishAndSetSpan), () => {
          let obj = sentryNonRecordingSpan(6352);
          const status = obj.spanToJSON(sentryNonRecordingSpan).status;
          let tmp = !sentryNonRecordingSpan.isRecording();
          if (!tmp) {
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (obj.suppressTracing) {
    ...
    suppressTracingResult = _require(6374).withScope((setSDKProcessingMetadata) => {
