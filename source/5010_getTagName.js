...
// Function ID: 5011
// Name: getTagName
// Dependencies: [32, 4971, 4988, 5011]

// Module 5010 (getTagName)
...
export default {
  read(arg0, arg1) {
    ...
    if (0 < arg0.length) {
      ...
      const stringFromDataView = require(4971) /* getDataView */.getStringFromDataView(dataView, num, closure_7);
      const obj3 = require(4971) /* getDataView */;
      const shortAt = importDefault(4988).getShortAt(dataView, sum);
      const sum1 = sum + c5;
      const tmp15 = getTagName(dataView, sum1);
      let name = tmp15.tagName;
      const sum2 = sum1 + tmp15.tagNameSize;
      const obj4 = importDefault(4988);
      const longAt = importDefault(4988).getLongAt(dataView, sum2);
      const sum3 = sum2 + c6;
      if (stringFromDataView === c4) {
        let tmp5Result = tmp5(4971);
        const dataView1 = tmp5Result.getDataView(dataView.buffer, sum3, longAt);
        obj = { id: null, value: null };
        obj[0] = shortAt;
        tmp5Result = tmp5(4971);
        obj[1] = tmp5Result.getStringFromDataView(dataView1, 0, longAt);
        if (tmp10(5011)[shortAt]) {
          try {
            obj.description = tmp10(5011)[shortAt].description(dataView1);
            if (!name) {
              name = tmp10(5011)[shortAt].name;
            }
            obj[name] = obj;
            const obj6 = tmp10(5011)[shortAt];
          } catch (err) {
