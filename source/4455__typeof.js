...
const combined = "".concat("https://js.stripe.com", "/");
let closure_1 = combined.concat("basil", "/stripe.js");
let closure_2 = /^https:\/\/js\.stripe\.com\/v3\/?(\?.*)?$/;
let closure_3 = /^https:\/\/js\.stripe\.com\/(v3|[a-z]+)\/stripe\.js(\?.*)?$/;
        ...
        if ("undefined" !== typeof document) {
          ...
          if (window.Stripe) {
            ...
            if (0 < elements.length) {
              tmp5 = tmp6;
              while (!callback(elements[num].src)) {
                num = num + 1;
                tmp5 = null;
            ...
            if (tmp5) {
              ...
              let closure_8 = function onLoad(arg0, arg1) {
                return () => {
                  if (window.Stripe) {
                    const _window = window;
                    arg0(window.Stripe);
              ...
              let closure_7 = function onError(arg0) {
                return (cause) => {
                  const error = new Error("Failed to load Stripe.js", { cause });
                  cause(error);
                };
              }(arg1);
              const listener = obj.addEventListener("load", closure_8);
              const listener1 = obj.addEventListener("error", closure_7);
            }
            if (obj) {
              if (obj) {
                if (null !== closure_8) {
                  if (null !== closure_7) {
                    const removed = obj.removeEventListener("load", closure_8);
                    const removed1 = obj.removeEventListener("error", closure_7);
