...
// Dependencies: [13538, 13564, 13563, 13562, 13551]

// Module 13566 (__exportStarResult25)
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13538).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13564).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13564).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13563).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13562).FormatApproximately(internalSlots, result);
    ...
    return tmp(13551).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13563).FormatNumeric(internalSlots, isNaN);
};
