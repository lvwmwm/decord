...
// Dependencies: [853, 6364, 6367, 6368, 6382, 6392, 6396, 6400, 6450, 6373, 6402, 6383, 6401, 6439, 6385, 6381, 6375]
// Exports: addTracingHeadersToFetchRequest, instrumentFetchRequest

// Module 6473 (_addTracingHeadersToFetchRequest)
...
function _addTracingHeadersToFetchRequest(headers, headers2, span) {
  let baggage;
  let tmp4;
  let obj = require(6439) /* getTraceData */;
  obj = { span };
  ...
  if (tmp4) {
    ...
    if (!headers) {
      ...
      let isInstanceOfResult = typeof Request === "ta";
      if (typeof Request !== "Array") {
        let tmpResult = tmp(6375);
    ...
    if (headers) {
      ...
      let isInstanceOfResult1 = typeof Headers === "ta";
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData) {
  ...
  if (fetchData.fetchData) {
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(6385).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(6385);
            } else if (endResult.error) {
              obj = { code: null, message: "internal_error" };
              obj[0] = setHttpStatus(6385).SPAN_STATUS_ERROR;
              obj10.setStatus(obj);
            }
            endResult = obj10.end();
            delete tmp2[tmp];
    ...
    if (tmp7) {
      const host = setHttpStatus(6450).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(6450);
    }
    const activeSpan = setHttpStatus(6373).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj1[3] = tmp7;
        obj1[4] = host;
        obj1[setHttpStatus(6383).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj1[setHttpStatus(6383).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj[1] = obj1;
        let startInactiveSpanResult = setHttpStatus(6402).startInactiveSpan(obj);
        const setHttpStatusResult4 = setHttpStatus(6402);
      }
      ...
      if (arg2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(6400);
        const tmp13 = _addTracingHeadersToFetchRequest;
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(6401).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(6373);
  }
};
