...
// Function ID: 7646
// Name: _flush
// Dependencies: [5, 7624, 7646, 7625, 7597, 7608, 7611, 7604, 7634, 7598, 7620]
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(7634).DEFAULT_ENVIRONMENT;
  }
  const obj5 = getClient;
...
export const withMonitor = function withMonitor(monitorSlug) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: closure_2, duration: monitorSlug(7611).timestampInSeconds() - closure_3 };
    const obj2 = monitorSlug(7611);
    const currentScope = monitorSlug(7624).getCurrentScope();
    const obj3 = monitorSlug(7624);
    const client = monitorSlug(7624).getClient();
    if (client) {
      if (client.captureCheckIn) {
    ...
    const obj4 = monitorSlug(7624);
    monitorSlug(7608).uuid4();
  }
  let currentScope = _require(7624).getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = _require(7624);
  let client = _require(7624).getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(7625).DEBUG_BUILD) {
      let logger2 = tmp(7597).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    ...
    let logger = tmp(7597).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = _require(7624);
  captureCheckInResult = _require(7608).uuid4();
};
