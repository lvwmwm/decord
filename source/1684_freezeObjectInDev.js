...
// Dependencies: [32, 1657, 1685, 1665, 1679, 1670, 1658]
// Exports: makeShareableCloneOnUIRecursive

// Module 1684 (freezeObjectInDev)
...
let fn = function u() {
  const proxy = new Proxy({}, {
    get(arg0, arg1) {
      if ("_isReanimatedSharedValue" !== arg1) {
        if ("__remoteFunction" !== arg1) {
          ...
          const reanimatedError = new callback(1665).ReanimatedError("Trying to access property `" + String(arg1) + "` of an object which cannot be sent to the UI runtime.");
    ...
    set() {
      const reanimatedError = new callback(1665).ReanimatedError("Trying to write to an object which cannot be sent to the UI runtime.");
      throw reanimatedError;
    }
  });
  return proxy;
...
export const makeShareable = isJest ? (function makeShareableJS(arg0) {
  ...
  let closure_0 = arg0;
  const shareableMappingCache = require(1685) /* weakMap */.shareableMappingCache;
  if (shareableMappingCache.get(arg0)) {
    ...
    obj[0] = fn;
    const shareableMappingCache2 = tmp(1685).shareableMappingCache;
    const result = shareableMappingCache2.set(arg0, tmp3(obj));
    return arg0;
  }
  tmp = require;
});
