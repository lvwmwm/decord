...
// Function ID: 7403
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 7394, 7375, 7347, 7401, 7358, 7354, 7370, 7371, 7403, 7390, 7391, 7374, 7396, 7383, 7404, 7387, 7405, 7352]

// Module 7402 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(7375).DEBUG_BUILD) {
          const logger = tmp(7347).logger;
        ...
        BaseClient(7370).updateSession(release, { init: false });
        const obj = BaseClient(7370);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(7403).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(7403).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(7403);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(7390).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(7375).DEBUG_BUILD) {
        let logger = tmp2(7347).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(7371).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(7403).setupIntegrations(this, integrations);
      const obj = BaseClient(7403);
      const result = BaseClient(7403).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(7387).parseSampleRate(sampleRate);
        let obj = str(7387);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(7404).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(7371).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(7375).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(7347).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(7405);
