...
function getThreeByteValue(getUint8, sum) {
  ...
  sum = byteAt + 256 * module(4797).getByteAt(getUint8, sum + 1);
  const obj2 = module(4797);
  const sum1 = sum + 65536 * module(4797).getByteAt(getUint8, sum + 2) + 1;
...
arg5.default = {
  read(getUint8, sum) {
    const obj = {};
    const byteAt = module(4797).getByteAt(getUint8, sum);
