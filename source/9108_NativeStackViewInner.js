...
// Module 9108 (NativeStackViewInner)
let Platform;
let StyleSheet;
function NativeStackViewInner(state) {
  ...
  state = state.state;
  const arg1 = state;
  ({ navigation: closure_1, descriptors } = state);
  const dependencyMap = descriptors;
  const setNextDismissedKey = importDefault(dependencyMap[11])(state).setNextDismissedKey;
  importDefault(dependencyMap[12])(descriptors);
  const obj = { style: closure_11.container };
  ...
  obj.children = routes.map((key, index) => {
    ...
    const obj = {
      ...
      onWillDisappear(arg0) {
        closure_1.emit({ type: "transitionStart", data: { closing: true }, target: arg0.key });
      },
      onWillAppear(arg0) {
        closure_1.emit({ type: "transitionStart", data: { closing: false }, target: arg0.key });
      },
      onAppear() {
        closure_1.emit({ type: "transitionEnd", data: { closing: false }, target: arg0.key });
      },
      onDisappear() {
        closure_1.emit({ type: "transitionEnd", data: { closing: true }, target: arg0.key });
      },
      onDismissed(nativeEvent) {
        const StackActions = nativeEvent(closure_2[4]).StackActions;
        closure_1.dispatch(Object.assign({}, StackActions.pop(nativeEvent.nativeEvent.dismissCount), { source: nativeEvent.key, target: nativeEvent.key }));
        callback(nativeEvent.key);
      },
      onHeaderBackButtonClicked() {
        const StackActions = arg0(closure_2[4]).StackActions;
        closure_1.dispatch(Object.assign({}, StackActions.pop(), { source: arg0.key, target: arg0.key }));
      },
      ...
      onGestureCancel() {
        closure_1.emit({ type: "gestureCancel", target: arg0.key });
      }
    };
    return callback(closure_10, obj, key.key);
  });
  return callback3(arg1(dependencyMap[7]).ScreenStack, obj);
}
let closure_3 = importDefault(dependencyMap[0]);
let closure_4 = importAll(dependencyMap[1]);
const tmp2 = arg1(dependencyMap[2]);
({ Platform, StyleSheet } = tmp2);
...
function MaybeNestedStack(children) {
  ...
  closure_2 = React.useRef(tmp);
  const items = [tmp, presentation, route.name];
  const effect = React.useEffect(() => {
    const tmp = tmp(closure_2[5]);
  ...
  return closure_7(importDefault(closure_2[6]), obj);
}
function SceneView(arg0) {
  ...
  let obj2 = callback(dependencyMap[8]);
  ...
  let context = React.useContext(callback(dependencyMap[9]).HeaderShownContext);
  const context1 = React.useContext(callback(dependencyMap[9]).HeaderHeightContext);
  let num2 = 0;
  const context2 = React.useContext(callback(dependencyMap[9]).HeaderBackContext);
  if (!context) {
    num2 = safeAreaInsets.top;
  }
  let obj3 = callback(dependencyMap[4]);
  let obj4 = callback(dependencyMap[9]);
  let defaultHeaderHeight = obj4.getDefaultHeaderHeight(safeAreaFrame, tmp10, num2);
  const tmp15 = callback2(React.useState(defaultHeaderHeight), 2);
  const callback = tmp15[1];
  ...
  if (previousDescriptor) {
    obj = {};
    let obj6 = callback(dependencyMap[9]);
  ...
  obj = { 9223372036854775807: null, -9223372036854775808: null, style: StyleSheet.absoluteFill };
  ...
  if (null != headerBackground) {
    ...
    tmp31Result = callback3(View, obj5);
  ...
  obj9.children = callback3(callback(dependencyMap[7]).EdgeInsetApplicationContext.Provider, obj10);
  obj8.children = callback3(callback(dependencyMap[9]).HeaderBackContext.Provider, obj9);
  const items2 = [callback3(MaybeNestedStack, obj8), ];
  ...
  if (undefined !== header) {
    ...
    if (tmp28) {
      ...
      tmp39Result = callback3(View, obj11);
  ...
  items1[2] = callback3(importDefault(dependencyMap[10]), Object.assign({}, options, obj13));
  obj4.children = items1;
  obj3.children = closure_8(callback(dependencyMap[9]).HeaderHeightContext.Provider, obj4);
  obj2.children = callback3(callback(dependencyMap[9]).HeaderShownContext.Provider, obj3);
  obj1.children = callback3(callback(dependencyMap[4]).NavigationRouteContext.Provider, obj2);
...
const styles = StyleSheet.create({ container: { flex: 1 }, scene: { marginLeft: false, marginVertical: "linear" }, absolute: { "Null": 131420590, "Null": -1717986767, "Null": -644245095, "Null": 94527 }, translucent: {}, background: { overflow: "hidden" } });

export default function NativeStackView(arg0) {
  const obj = { children: callback3(NativeStackViewInner, Object.assign({}, arg0)) };
  return callback3(arg1(dependencyMap[9]).SafeAreaProviderCompat, obj);
};
