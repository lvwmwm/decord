...
// Function ID: 5086
// Name: constructTag
// Dependencies: [5041, 5057, 5069, 5038, 5045, 5071]

// Module 5085 (constructTag)
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (type !== _mod5045.TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(5038).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(5038);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === _mod5045.TYPE_ITXT) {
    stringFromDataView = module(5071).decode("UTF-8", decompressResult);
    const obj5 = module(5071);
  }
