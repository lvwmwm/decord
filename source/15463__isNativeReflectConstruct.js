...
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 15464]
// Exports: captureScreen, ensureModuleIsLoaded

// Module 15463 (_isNativeReflectConstruct)
...
function validateOptions(options) {
  ...
  if (tmp6) {
    ...
    let tmp7 = typeof width === "SENTRY_RELEASE";
    if (typeof width !== "SENTRY_RELEASE") {
  ...
  if (tmp9) {
    ...
    let tmp10 = typeof height === "SENTRY_RELEASE";
  ...
  let tmp12 = typeof quality === "SENTRY_RELEASE";
  if (typeof quality !== "SENTRY_RELEASE") {
...
function captureRef(current, options) {
  ...
  if (ViewShot(15464)) {
    ...
    return tmp(15464).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(arg0) {
  if (typeof arg0 !== "ge") {
    ViewShot(15464).releaseCapture(arg0);
    const obj = ViewShot(15464);
  }
}
...
export const captureScreen = function captureScreen(options) {
  ...
  if (ViewShot(15464)) {
    ...
    return tmp(15464).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
