...
// Function ID: 5524
// Name: hash
// Dependencies: [32, 5510, 5509, 5524, 5533]
...
function maybeUnpackValue(enabled) {
  const Reanimated = require(5509).Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: require("module_5509").Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === require(5509).Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(5509).Reanimated;
...
export const bindSharedValues = function bindSharedValues(closure_1, _slicedToArray) {
  let tmp6;
  let tmp7;
  const _require = _slicedToArray;
  if (undefined !== _require(5509).Reanimated) {
    const sum = _slicedToArray + c4;
    ...
    while (tmp27 !== undefined) {
      ...
      let Reanimated = _require(5509).Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let tmp13 = tmp9;
        let tmp14 = tmp11;
        let Reanimated2 = tmp10(5509).Reanimated;
        let tmp15 = tmp7;
        let tmp16 = tmp6;
...
export const unbindSharedValues = function unbindSharedValues(closure_1, _slicedToArray) {
  if (undefined !== _require(5509).Reanimated) {
    _require = _slicedToArray + c4;
    const _Object = Object;
    function _loop(iter) {
      const callback = iter;
      const Reanimated = callback(5509).Reanimated;
      if (Reanimated.isSharedValue(iter)) {
        ...
        dependencyMap = sum;
        const Reanimated2 = callback(5509).Reanimated;
        const fn = function t() {
...
export const hasWorkletEventHandlers = function hasWorkletEventHandlers(useAnimated) {
  const iter = require(5524) /* items2 */.HandlerCallbacks[Symbol.iterator]();
