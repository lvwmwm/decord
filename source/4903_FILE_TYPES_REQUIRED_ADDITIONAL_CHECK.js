...
// Dependencies: [41, 42, 4901, 4904, 4910, 4911, 4912, 4913, 4914]

// Module 4903 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
...
let items = [
  {
    ...
    value: function getInfoByName(key10027) {
      return FileTypes(4901).fetchFromObject(FileTypes, key10027.toUpperCase());
    }
  },
  {
    key: "getSignaturesByName",
    value: function getSignaturesByName(item10079) {
      return FileTypes(4901).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((arg0) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(4904).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(4904).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(4904).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(4901).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(4904).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((arg0) => {
          ...
          if (FileTypes(4901).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
