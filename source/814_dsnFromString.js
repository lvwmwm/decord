...
function extractOrgIdFromDsnHost(host) {
  const match = host.match(closure_3);
  let tmp2;
  if (null != match) {
    tmp2 = match[1];
  }
...
export const extractOrgIdFromClient = function extractOrgIdFromClient(getOptions) {
  const options = getOptions.getOptions();
  const host = getOptions.getDsn() || {}.host;
  if (options.orgId) {
    const _String = String;
    let StringResult = String(options.orgId);
  } else if (host) {
    StringResult = extractOrgIdFromDsnHost(host);
  }
  return StringResult;
};
export { extractOrgIdFromDsnHost };
