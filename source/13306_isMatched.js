...
// Function ID: 13307
// Name: isMatched
// Dependencies: [13307, 1281, 13308]

// Module 13306 (isMatched)
const require = arg1;
  ...
  if (arr) {
    if ("$" === arr[0]) {
      ...
      const mapped = arr2.map((arg0) => {
        let tmp = callback(13308).regions[arg0];
        if (!tmp) {
          const items = [arg0];
          tmp = items;
        }
...
function findMatchingDistance(arg0, arg1) {
  ...
  let tmp7 = obj1;
  if (!obj1) {
    let first = require(13307).data.supplemental.languageMatching["written-new"][0];
    let paradigmLocales;
    if (null !== first) {
      if (undefined !== first) {
    ...
    const prop = tmp8(13307).data.supplemental.languageMatching["written-new"];
    let substr = prop.slice(1, 5);
    const prop1 = tmp8(13307).data.supplemental.languageMatching["written-new"];
