...
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 4882, 4918, 4919, 4920]

// Module 4917 (_isNativeReflectConstruct)
...
function readTags(_raw, dataView, arg2) {
  ...
  try {
    ...
    ParseError(4882).objectAssign(_raw, parseXMPObject(convertToObject(tmp6, true)));
    return true;
  } catch (err) {
    return false;
  }
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(4919).addMissingNamespaces(tmp), true);
        }
      }
      obj2 = ParseError(4919);
      tmp13 = ParseError;
    }
    throw tmp12;
  }
...
function getDescription(arr) {
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      tmp12 = tmp11;
      if (typeof importDefault(4920)[tmp2] === "function") {
        tmp12 = tmp13(4920)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(4920);
      }
      tmp13 = importDefault;
    }
    return tmp12;
    ...
    try {
      if (tmp2) {
        if (typeof importDefault(4920)[tmp2] === "function") {
          let decodeURIComponentResult = importDefault(4920)[tmp2](arr);
          const tmp4Result = importDefault(4920);
        }
...
function parseNodeAsSimpleRdfDescription(attributes, key10005) {
  ...
  ParseError(4882).objectAssign(obj, obj, obj);
...
function parseNodeAsStructureRdfDescription(value, key10005) {
  ...
  if (undefined !== value.value["rdf:Description"]) {
    ParseError(4882).objectAssign(obj.value, parseNodeAttributesAsTags(value.value["rdf:Description"].attributes));
    const obj4 = ParseError(4882);
    obj = {};
    ...
    ParseError(4882).objectAssign(obj.attributes, obj);
    iter = value.value["rdf:Description"];
    const obj5 = ParseError(4882);
  }
  ParseError(4882).objectAssign(obj.value, parseNodeChildrenAsTags(iter.value));
