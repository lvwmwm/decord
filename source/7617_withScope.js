...
// Function ID: 7618
// Name: withScope
// Dependencies: [41, 42, 7618, 7604, 7615, 7623]
// Exports: getStackAsyncContextStrategy

// Module 7617 (withScope)
import closure_2 from "_classCallCheck" /* 41 */;
import _createClass from "_createClass" /* 42 */;

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(7615).getMainCarrier();
  const obj = AsyncContextStack(7615);
  const sentryCarrier = AsyncContextStack(7615).getSentryCarrier(mainCarrier);
  let stack = sentryCarrier.stack;
  if (!stack) {
    let tmpResult = tmp(7623);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7615).getMainCarrier();
      let obj = callback(7615);
      let sentryCarrier = callback(7615).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7623);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7615).getMainCarrier();
      const obj = callback(7615);
      const sentryCarrier = callback(7615).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7623);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
