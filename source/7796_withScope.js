...
// Function ID: 7797
// Name: withScope
// Dependencies: [41, 42, 7797, 7783, 7794, 7802]
// Exports: getStackAsyncContextStrategy

// Module 7796 (withScope)
import closure_2 from "_classCallCheck" /* 41 */;
import _createClass from "_createClass" /* 42 */;

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(7794).getMainCarrier();
  const obj = AsyncContextStack(7794);
  const sentryCarrier = AsyncContextStack(7794).getSentryCarrier(mainCarrier);
  let stack = sentryCarrier.stack;
  if (!stack) {
    let tmpResult = tmp(7802);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7794).getMainCarrier();
      let obj = callback(7794);
      let sentryCarrier = callback(7794).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7802);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7794).getMainCarrier();
      const obj = callback(7794);
      const sentryCarrier = callback(7794).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7802);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
