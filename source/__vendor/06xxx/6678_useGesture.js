...
// Function ID: 6679
// Name: useGesture
// Dependencies: [19, 6617, 6600, 6679, 6663, 6639, 6626, 6599]
// Exports: useGesture

// Module 6678 (useGesture)
...
export const useGesture = function useGesture(Fling, clonedAndRemappedConfig) {
  ...
  if (clonedAndRemappedConfig.disableReanimated !== jsEventHandler(() => clonedAndRemappedConfig.disableReanimated, [])) {
    const _Error2 = Error;
    error = new Error(_require(6600).tagMessage("The \"disableReanimated\" property must not be changed after the handler is created."));
    throw error;
  } else {
    const gestureCallbacks = _require(6679).useGestureCallbacks(tmp2, clonedAndRemappedConfig);
    ...
    if (clonedAndRemappedConfig.shouldUseReanimatedDetector) {
      if (!reanimatedEventHandler) {
        const _Error = Error;
        const error1 = new Error(_require(6600).tagMessage("Failed to create reanimated event handlers."));
        throw error1;
      }
    }
    const items = [tmp2, , , ];
