...
// Function ID: 6665
// Name: map
// Dependencies: [32, 19, 6662, 6652, 6665, 6638, 6653, 6600]
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!disableReanimated) {
    result = undefined !== _mod6638.Reanimated;
  }
  if (result) {
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      ...
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let tmp23 = tmp18;
        let tmp24 = tmp20;
        let PropsToFilter = tmp19(6653).PropsToFilter;
        let tmp25 = first;
    ...
    let tmp32 = tmp18;
    let tmp33 = tmp20;
    let Reanimated = tmp19(6638).Reanimated;
