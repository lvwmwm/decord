...
// Function ID: 6647
// Name: getHandler
// Dependencies: [6616, 6603, 6600, 6638, 6601, 6602]

// Module 6646 (getHandler)
...
function getHandler(arg0, onBegin) {
  ...
  } else if (tmp(6616).CALLBACK_TYPE.START === arg0) {
    return onBegin.onStart;
  } else if (tmp(6616).CALLBACK_TYPE.UPDATE === arg0) {
    return onBegin.onUpdate;
  } else if (tmp(6616).CALLBACK_TYPE.CHANGE === arg0) {
    return onBegin.onChange;
  } else if (tmp(6616).CALLBACK_TYPE.END === arg0) {
    return onBegin.onEnd;
  } else if (tmp(6616).CALLBACK_TYPE.FINALIZE === arg0) {
    return onBegin.onFinalize;
  } else if (tmp(6616).CALLBACK_TYPE.TOUCHES_DOWN === arg0) {
    return onBegin.onTouchesDown;
  } else if (tmp(6616).CALLBACK_TYPE.TOUCHES_MOVE === arg0) {
    return onBegin.onTouchesMove;
  } else if (tmp(6616).CALLBACK_TYPE.TOUCHES_UP === arg0) {
    return onBegin.onTouchesUp;
  } else if (tmp(6616).CALLBACK_TYPE.TOUCHES_CANCEL === arg0) {
...
function touchEventTypeToCallbackType(arg0) {
  if (_mod6603.TouchEventType.TOUCHES_DOWN === arg0) {
    return tmp(6616).CALLBACK_TYPE.TOUCHES_DOWN;
  } else if (tmp(6603).TouchEventType.TOUCHES_MOVE === arg0) {
    return tmp(6616).CALLBACK_TYPE.TOUCHES_MOVE;
  } else if (tmp(6603).TouchEventType.TOUCHES_UP === arg0) {
    return tmp(6616).CALLBACK_TYPE.TOUCHES_UP;
  } else if (tmp(6603).TouchEventType.TOUCHES_CANCEL === arg0) {
    return tmp(6616).CALLBACK_TYPE.TOUCHES_CANCEL;
  } else {
    return tmp(6616).CALLBACK_TYPE.UNDEFINED;
  }
}
obj = { TouchEventType: require("module_6603").TouchEventType, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
