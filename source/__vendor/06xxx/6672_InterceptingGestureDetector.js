...
// Dependencies: [32, 19, 17, 21, 6613, 6673, 6609, 6668, 6610, 6600, 6638, 6669, 6670, 6662, 6671]
// Exports: InterceptingGestureDetector

// Module 6672 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  if (prop) {
    let REANIMATED = tmp(6673).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6673).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6673).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6673).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6609).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6668).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6610);
  }
  ...
  closure_5(() => {
    ...
    if (prop) {
      store.setMode(gesture(6673).InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        store.setMode(gesture(6673).InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const Reanimated = tmp(6638).Reanimated;
    ...
    const detectorAttachmentGuard = tmp(6671).useDetectorAttachmentGuard(tmp8Result1);
    ...
    obj1[10] = prop6;
    obj1[11] = tmp8Result1;
    obj1[12] = tmp(6609).nativeDetectorStyles.detector;
    obj1[13] = tmp9;
    const _globalThis = globalThis;
    obj1[14] = globalThis._RNGH_MODULE_ID;
    obj1[15] = children;
    obj[1] = closure_8(ReanimatedNativeDetector, obj1);
    return closure_8(tmp(6673).InterceptingDetectorContext, obj);
  } else {
    const _Error = Error;
    error = new Error(tmp(6600).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
