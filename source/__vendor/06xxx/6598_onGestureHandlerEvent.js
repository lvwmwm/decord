...
// Dependencies: [17, 6599, 6601, 6602, 6603]
// Exports: startListening, stopListening

// Module 6598 (onGestureHandlerEvent)
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import map2 from "map" /* 6599 */;
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6601).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6601).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6601).State.BEGAN) {
        if (handlerTag.state === tmp(6601).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6601).State.END) {
          if (handlerTag.oldState === tmp(6601).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6601).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6601).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6602).GestureStateManager;
      ...
      if (tmp(6603).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6603).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6603).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6603).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6599).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
