...
// Dependencies: [6663, 6601, 6616, 6603, 6665]

// Module 6681 (handleStateChangeEvent)
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, arg3) {
  ...
  if (oldState === _mod6601.State.UNDETERMINED) {
    if (state === tmp(6601).State.BEGAN) {
      let tmpResult = tmp(6663);
      tmpResult.runCallback(tmp(6616).CALLBACK_TYPE.BEGAN, arg1, result);
    }
  }
  if (oldState === _mod6601.State.BEGAN) {
    if (state === tmp(6601).State.ACTIVE) {
      if (arg3 != null) {
        arg3(result);
      }
      tmpResult = tmp(6663);
      tmpResult.runCallback(tmp(6616).CALLBACK_TYPE.START, arg1, result);
    }
  }
  if (oldState !== state) {
    obj = {};
    const merged = Object.assign(result);
    obj.canceled = state === tmp(6601).State.FAILED || state === tmp(6601).State.CANCELLED;
    if (oldState === tmp(6601).State.ACTIVE) {
      if (arg3 != null) {
        arg3(obj);
      }
      tmp(6663).runCallback(tmp(6616).CALLBACK_TYPE.END, arg1, obj);
      const tmpResult1 = tmp(6663);
    }
    const tmp7 = state === tmp(6601).State.FAILED || state === tmp(6601).State.CANCELLED;
    tmp(6663).runCallback(tmp(6616).CALLBACK_TYPE.FINALIZE, arg1, obj);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult2 = tmp(6663);
  }
}
let obj = { flattenAndFilterEvent: require("isGestureEnabled").flattenAndFilterEvent, State: require("module_6601").State, runCallback: require("isGestureEnabled").runCallback, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
...
function handleTouchEvent(eventType) {
  if (eventType.eventType !== _mod6603.TouchEventType.UNDETERMINED) {
    let tmpResult = tmp(6663);
    tmpResult = tmp(6663);
    tmpResult.runCallback(tmpResult.touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
  }
}
obj = { TouchEventType: require("module_6603").TouchEventType, runCallback: require("isGestureEnabled").runCallback, touchEventTypeToCallbackType: require("isGestureEnabled").touchEventTypeToCallbackType };
  ...
  if (obj2.isEventForHandlerWithTag(arg0, result)) {
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult.isTouchEvent(result)) {
        ...
        if (result.eventType !== tmp(6603).TouchEventType.UNDETERMINED) {
          const tmpResult1 = tmp(6663);
          tmpResult1.runCallback(tmp(6663).touchEventTypeToCallbackType(result.eventType), arg2, result);
          const tmpResult2 = tmp(6663);
        }
      } else if (!arg5) {
        ...
        const result1 = tmp(6663).flattenAndFilterEvent(tmp9);
        const tmpResult3 = tmp(6663);
        tmp(6663).runCallback(tmp(6616).CALLBACK_TYPE.UPDATE, arg2, result1);
        if (lastUpdateEvent) {
          lastUpdateEvent.lastUpdateEvent = result;
        }
        const tmpResult4 = tmp(6663);
      }
    }
  }
}
