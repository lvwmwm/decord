...
// Dependencies: [32, 19, 6652, 6663, 6654, 6667, 6638]
// Exports: useJSResponderHandler

// Module 6651 (isSupportedGesture)
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6654).SingleGestureName.Tap !== type) {
      if (tmp(6654).SingleGestureName.LongPress !== type) {
        if (tmp(6654).SingleGestureName.Fling !== type) {
          if (tmp(6654).SingleGestureName.Native !== type) {
            if (tmp(6654).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  ...
  let tmp = callback2(_require(6667).JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  callback4(() => {
    const Reanimated = gesture(6638).Reanimated;
    let obj = gesture(6652);
  ...
  const tmp7 = callback3(() => {
    let some = gesture;
    const isGestureEnabledResult = gesture(6663).isGestureEnabled(gesture);
  ...
  let handleStartShouldSetResponder = callback3(() => {
    if (callback()) {
      const result = gesture(6667).updateResponderEventValue(dependencyMap, true);
      const obj = gesture(6667);
    }
    return false;
  }, items2);
  if (null == tmp) {
