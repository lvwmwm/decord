...
// Function ID: 6653
// Name: hash
// Dependencies: [32, 6639, 6638, 6653, 6662]
...
function maybeUnpackValue(enabled) {
  const Reanimated = _mod6638.Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: require("module_6638").Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6638.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6638).Reanimated;
...
export const bindSharedValues = function bindSharedValues(closure_1, closure_2) {
  ...
  if (undefined !== _require(6638).Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      if (Reanimated.isSharedValue(tmp7)) {
        let tmp13 = tmp9;
        let tmp14 = tmp11;
        let Reanimated2 = tmp10(6638).Reanimated;
        let tmp15 = tmp7;
        let tmp16 = tmp6;
...
export const unbindSharedValues = function unbindSharedValues(closure_1, closure_2) {
  if (undefined !== _require(6638).Reanimated) {
    ...
    function _loop(iter) {
      const callback = iter;
      const Reanimated = callback(6638).Reanimated;
      if (Reanimated.isSharedValue(iter)) {
        ...
        dependencyMap = sum;
        const Reanimated2 = callback(6638).Reanimated;
        const fn = function t() {
