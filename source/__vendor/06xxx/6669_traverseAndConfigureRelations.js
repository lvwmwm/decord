...
// Dependencies: [6662, 6654, 6600]

// Module 6669 (traverseAndConfigureRelations)
...
function traverseAndConfigureRelations(gestures, map, set) {
  ...
  if (obj.isComposedGesture(gestures)) {
    ...
    gestures.gestureRelations = tmp(6662).prepareRelations(gestures.config, gestures.handlerTag);
...
arg5.configureRelations = function configureRelations(externalSimultaneousHandlers) {
  ...
  if (obj2.isComposedGesture(externalSimultaneousHandlers)) {
    ...
    if (externalSimultaneousHandlers.type === set(6654).ComposedGestureName.Simultaneous) {
