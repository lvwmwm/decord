...
let obj = {
  createGestureHandler(closure_0, handlerTag, config) {
    ...
    const result = _require(6626).scheduleOperationToBeFlushed(() => {
  ...
  setGestureHandlerConfig(handlerTag, closure_1) {
    ...
    let result = _require(6626).scheduleOperationToBeFlushed(() => {
...
const fn = function n(arg0, arg1) {
  const result = module(6624).updateGestureHandlerConfig(arg0, arg1);
  const obj = module(6624);
  module(6624).flushOperations();
};
...
obj[5] = function installUIRuntimeBindings() {
  return module(6624).installUIRuntimeBindings();
};
