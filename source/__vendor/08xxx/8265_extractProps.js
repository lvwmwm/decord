...
// Function ID: 8266
// Name: extractProps
// Dependencies: [8266, 8257, 8267, 8270, 8258, 8256]

// Module 8265 (extractProps)
import pickNotNil from "pickNotNil" /* 8266 */;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(8257)(obj, markerMid, arg1);
  module(8267)(obj, markerMid, items);
  module(8270)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(8258)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(8256)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
