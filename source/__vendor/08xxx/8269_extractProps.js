...
// Function ID: 8270
// Name: extractProps
// Dependencies: [8270, 8261, 8271, 8274, 8262, 8260]

// Module 8269 (extractProps)
import pickNotNil from "pickNotNil" /* 8270 */;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(8261)(obj, markerMid, arg1);
  module(8271)(obj, markerMid, items);
  module(8274)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(8262)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(8260)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
