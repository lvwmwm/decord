...
// Function ID: 8203
// Name: extractProps
// Dependencies: [8203, 8194, 8204, 8207, 8195, 8193]

// Module 8202 (extractProps)
import pickNotNil from "pickNotNil" /* 8203 */;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(8194)(obj, markerMid, arg1);
  module(8204)(obj, markerMid, items);
  module(8207)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(8195)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(8193)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
