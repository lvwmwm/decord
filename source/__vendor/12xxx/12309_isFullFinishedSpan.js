...
// Dependencies: [41, 42, 12278, 12282, 12283, 12273, 12306, 12274, 12284, 12310, 12296, 12268, 12295, 12311, 12297, 12304]

// Module 12309 (isFullFinishedSpan)
...
class SentrySpan {
  constructor() {
    ...
    obj[require("module_12283").SEMANTIC_ATTRIBUTE_SENTRY_OP] = obj.op;
...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(12273).spanTimeInputToSeconds(arg0);
    }
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(12283).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    }
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(12273).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(12273);
        SentrySpan(12306).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(12306);
      }
    }
  },
  {
    ...
    value: function getSpanJSON() {
      const self = this;
      let obj = SentrySpan(12274);
      obj = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 12283).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: SentrySpan(12273).getStatusMessage(this._status), timestamp: null, trace_id: null, origin: _attributes[SentrySpan(undefined, 12283).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN], _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      ({ _endTime: obj2[7], _traceId: obj2[8], _attributes } = this);
      const obj3 = SentrySpan(12273);
      obj[10] = SentrySpan(12284).getMetricSummaryJsonForSpan(this);
      obj[11] = this._attributes[SentrySpan(undefined, 12283).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj[12] = this._attributes[SentrySpan(undefined, 12283).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(12284);
      obj[13] = SentrySpan(12310).timedEventsToMeasurements(this._events);
  ...
  },
  {
    ...
    value: function addEvent(name, num) {
      if (SentrySpan(12296).DEBUG_BUILD) {
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(12295).getClient();
      if (client) {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          ...
          if (result) {
            let scope = tmp(12297).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(12295).getCurrentScope();
              const tmpResult2 = tmp(12295);
            }
            scope.captureEvent(result);
            const tmpResult1 = tmp(12297);
          }
        }
      } else {
  ...
  },
  {
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        ...
        if (!client) {
          client = tmp3(12295).getClient();
          const tmp3Result1 = tmp3(12295);
        }
        if (true !== self._sampled) {
          if (tmp3(12296).DEBUG_BUILD) {
            const logger3 = tmp3(12268).logger;
          ...
          const spanDescendants = tmp3(12273).getSpanDescendants(self);
          ...
          const mapped = found.map((arg0) => self(12273).spanToJSON(arg0));
          const found1 = mapped.filter(isFullFinishedSpan);
          const tmp24 = self._attributes[tmp3(undefined, 12283).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
          const _attributes = self._attributes;
          let SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = tmp3(12283).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
          delete tmp2[tmp];
          const item = found1.forEach((data) => {
            if (data.data) {
              data = data.data;
              const SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = self(12283).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
              delete tmp2[tmp];
            }
          });
          ...
          obj = { trace: null };
          const tmp3Result2 = tmp3(12273);
          obj[0] = tmp3(12273).spanToTransactionTraceContext(self);
          obj[0] = obj;
          let substr = found1;
          ...
          const tmp3Result3 = tmp3(12273);
          const obj2 = { dynamicSamplingContext: null };
          const tmp3Result4 = tmp3(12274);
          obj2[0] = tmp3(12304).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result4.dropUndefinedKeys(obj2));
          obj[6] = obj1;
          const tmp3Result5 = tmp3(12304);
          obj[7] = tmp3(12284).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
          ...
          const tmp3Result6 = tmp3(12284);
          const result = tmp3(12310).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
          ...
          if (length) {
            if (tmp3(12296).DEBUG_BUILD) {
              const logger2 = tmp3(12268).logger;
              const _JSON = JSON;
