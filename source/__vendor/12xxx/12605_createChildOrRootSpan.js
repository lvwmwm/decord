...
// Dependencies: [32, 12598, 12604, 12593, 12606, 12576, 12588, 12589, 12590, 12583, 12581, 12599, 12571, 12603, 12607, 12609, 12600, 12610, 12612, 12586]
// Exports: continueTrace, startInactiveSpan, startNewTrace, startSpan, startSpanManual, suppressTracing, withActiveSpan

// Module 12605 (createChildOrRootSpan)
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(12612).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(12604).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(12576).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(12576);
        const client = tmp(12598).getClient();
        if (client) {
        ...
        const tmpResult2 = tmp(12598);
        tmp(12576).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult3 = tmp(12576);
      }
      tmp(12609).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(12609);
      const result = tmp(12600).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(12607).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(12607);
      ...
      obj1[0] = traceId2;
      obj1[1] = spanId2;
      const tmpResult7 = tmp(12576);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj1, scope, tmp(12576).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(12576).spanIsSampled(parentSpan);
      tmp(12607).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = tmp(12607);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(12607).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = tmp(12607);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(12604).SentryNonRecordingSpan();
...
function _startRootSpan(name) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(12610).sampleSpan(options, obj);
    const tmpResult = tmp(12610);
  }
  ...
  obj1.attributes = { [_mod12586.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(12612).SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(12586).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  const _require = arg1;
  const mainCarrier = _require(12589).getMainCarrier();
  const obj = _require(12589);
  const tmp = _require;
  const asyncContextStrategy = _require(12590).getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(12598).withScope((setPropagationContext) => {
      ...
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = experimental(obj[1]).withScope((arg0) => {
            _null(12593)._setSpanForScope(arg0, _null);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return _require(12598).withScope((setPropagationContext) => {
...
export const startSpanManual = function startSpanManual(experimental) {
  ...
  let obj = _require(12589);
  let mainCarrier = obj.getMainCarrier();
  let asyncContextStrategy = _require(12590).getAsyncContextStrategy(mainCarrier);
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(12598).withScope((setSDKProcessingMetadata) => {
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(12598).withScope((arg0) => {
      _null(12593)._setSpanForScope(arg0, _null);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(12598);
