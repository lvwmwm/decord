...
// Function ID: 12580
// Name: withScope
// Dependencies: [41, 42, 12580, 12566, 12577, 12585]
// Exports: getStackAsyncContextStrategy

// Module 12579 (withScope)
import closure_2 from "_classCallCheck" /* 41 */;
import _createClass from "_createClass" /* 42 */;

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(12577).getMainCarrier();
  const obj = AsyncContextStack(12577);
  const sentryCarrier = AsyncContextStack(12577).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(12577).getMainCarrier();
      let obj = callback(12577);
      let sentryCarrier = callback(12577).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12585);
    ...
    getCurrentScope() {
      const mainCarrier = callback(12577).getMainCarrier();
      const obj = callback(12577);
      const sentryCarrier = callback(12577).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12585);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
