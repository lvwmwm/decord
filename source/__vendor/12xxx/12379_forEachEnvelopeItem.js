...
// Dependencies: [32, 12336, 12380, 12341, 12382]
...
// Module 12379 (forEachEnvelopeItem)
...
export const createAttachmentEnvelopeItem = function createAttachmentEnvelopeItem(data) {
  if (typeof data.data === "string") {
    ...
    __SENTRY__ = __SENTRY__(12336).GLOBAL_OBJ.__SENTRY__;
...
export const parseEnvelope = function parseEnvelope(arr) {
  if (typeof arr !== "string") {
    ...
    let encodePolyfill = dependencyMap;
    if (!_require(12336).GLOBAL_OBJ.__SENTRY__) {
      const _TextEncoder = TextEncoder;
      const textEncoder = new TextEncoder();
    ...
    if (typeof tmp8[1] !== "string") {
      ...
      if (!(tmp10 instanceof Uint8Array)) {
        try {
          ...
          json = _JSON3.stringify(_require(12380).normalize(tmp3));
          const obj = _require(12380);
        }
      }
    }
    append(tmp8[1]);
