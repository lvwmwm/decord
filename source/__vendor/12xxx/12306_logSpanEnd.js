...
// Function ID: 12307
// Name: logSpanEnd
// Dependencies: [12296, 12273, 12268]

// Module 12306 (logSpanEnd)
...
arg5.logSpanStart = function logSpanStart(spanContext) {
  if (__SENTRY_DEBUG__.DEBUG_BUILD) {
    ...
    const rootSpan = tmp(12273).getRootSpan(spanContext);
    let str3 = "unsampled";
    ...
    if (rootSpan !== spanContext) {
      ...
      ({ op: op2, description: description2 } = tmp(12273).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(12273).spanToJSON(rootSpan);
    }
    const logger = tmp(12268).logger;
