...
// Dependencies: [41, 42, 12259, 12263, 12264, 12254, 12287, 12255, 12265, 12291, 12277, 12249, 12276, 12292, 12278, 12285]

// Module 12290 (isFullFinishedSpan)
...
class SentrySpan {
  constructor() {
    ...
    obj[require("module_12264").SEMANTIC_ATTRIBUTE_SENTRY_OP] = obj.op;
...
let items = [
  ...
  },
  {
    ...
    value: function updateStartTime(arg0) {
      this._startTime = SentrySpan(12254).spanTimeInputToSeconds(arg0);
    }
  },
  {
    key: "setStatus",
  ...
  },
  {
    ...
    value: function updateName(_name) {
      this._name = _name;
      const attr = this.setAttribute(SentrySpan(12264).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom");
      return this;
    }
  },
  {
    ...
    value: function end(arg0) {
      const self = this;
      if (!this._endTime) {
        self._endTime = SentrySpan(12254).spanTimeInputToSeconds(arg0);
        const obj = SentrySpan(12254);
        SentrySpan(12287).logSpanEnd(self);
        self._onSpanEnded();
        const obj2 = SentrySpan(12287);
      }
    }
  },
  {
    ...
    value: function getSpanJSON() {
      const self = this;
      let obj = SentrySpan(12255);
      obj = { data: this._attributes, description: this._name, op: this._attributes[SentrySpan(undefined, 12264).SEMANTIC_ATTRIBUTE_SENTRY_OP], parent_span_id: this._parentSpanId, span_id: this._spanId, start_timestamp: this._startTime, status: SentrySpan(12254).getStatusMessage(this._status), timestamp: null, trace_id: null, origin: _attributes[SentrySpan(undefined, 12264).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN], _metrics_summary: null, profile_id: null, exclusive_time: null, measurements: null, is_segment: null, segment_id: null };
      ({ _endTime: obj2[7], _traceId: obj2[8], _attributes } = this);
      const obj3 = SentrySpan(12254);
      obj[10] = SentrySpan(12265).getMetricSummaryJsonForSpan(this);
      obj[11] = this._attributes[SentrySpan(undefined, 12264).SEMANTIC_ATTRIBUTE_PROFILE_ID];
      obj[12] = this._attributes[SentrySpan(undefined, 12264).SEMANTIC_ATTRIBUTE_EXCLUSIVE_TIME];
      const obj4 = SentrySpan(12265);
      obj[13] = SentrySpan(12291).timedEventsToMeasurements(this._events);
  ...
  },
  {
    ...
    value: function addEvent(name, num) {
      if (SentrySpan(12277).DEBUG_BUILD) {
  ...
  },
  {
    ...
    value: function _onSpanEnded() {
      const self = this;
      const client = SentrySpan(12276).getClient();
      if (client) {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          ...
          if (result) {
            let scope = tmp(12278).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(12276).getCurrentScope();
              const tmpResult2 = tmp(12276);
            }
            scope.captureEvent(result);
            const tmpResult1 = tmp(12278);
          }
        }
      } else {
    ...
    value: function _convertSpanToTransaction() {
      ...
      if (tmp6) {
        ...
        if (true !== self._sampled) {
          ...
          const spanDescendants = tmp3(12254).getSpanDescendants(self);
          ...
          const mapped = found.map((arg0) => self(12254).spanToJSON(arg0));
          const found1 = mapped.filter(isFullFinishedSpan);
          const tmp24 = self._attributes[tmp3(undefined, 12264).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
          const _attributes = self._attributes;
          let SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = tmp3(12264).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
          delete tmp2[tmp];
          const item = found1.forEach((data) => {
            if (data.data) {
              data = data.data;
              const SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME = self(12264).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME;
              delete tmp2[tmp];
            }
          });
          ...
          obj = { trace: null };
          const tmp3Result2 = tmp3(12254);
          obj[0] = tmp3(12254).spanToTransactionTraceContext(self);
          obj[0] = obj;
          let substr = found1;
          ...
          const tmp3Result3 = tmp3(12254);
          const obj2 = { dynamicSamplingContext: null };
          const tmp3Result4 = tmp3(12255);
          obj2[0] = tmp3(12285).getDynamicSamplingContextFromSpan(self);
          const merged = Object.assign(tmp3Result4.dropUndefinedKeys(obj2));
          obj[6] = obj1;
          const tmp3Result5 = tmp3(12285);
          obj[7] = tmp3(12265).getMetricSummaryJsonForSpan(self);
          let tmp12 = tmp24;
          if (tmp24) {
          ...
          const tmp3Result6 = tmp3(12265);
          const result = tmp3(12291).timedEventsToMeasurements(self._events);
          let length = result;
          if (result) {
          ...
          if (length) {
            if (tmp3(12277).DEBUG_BUILD) {
              const logger2 = tmp3(12249).logger;
              const _JSON = JSON;
