...
// Dependencies: [32, 41, 42, 12432, 12349, 12436, 12437, 12340, 12438]

// Module 12435 (MetricsAggregator)
...
class MetricsAggregator {
  constructor(arg0) {
    ...
    tmp4 = closure_1;
    this._interval = setInterval(() => self._flush(), require("module_12432").DEFAULT_FLUSH_INTERVAL);
    if (this._interval.unref) {
      _interval = self._interval;
      unrefResult = _interval.unref();
    }
    random = Math.random();
    self._flushShift = Math.floor(random * require("module_12432").DEFAULT_FLUSH_INTERVAL / 1000);
...
const items = [
  {
    ...
    value: function add(arg0, arg1, diff) {
      ...
      if (arg5 === undefined) {
        timestampInSecondsResult = MetricsAggregator(12349).timestampInSeconds();
        const obj2 = MetricsAggregator(12349);
      }
      const self = this;
      const rounded = Math.floor(timestampInSecondsResult);
      const sanitizeMetricKeyResult = MetricsAggregator(12436).sanitizeMetricKey(arg1);
      const obj3 = MetricsAggregator(12436);
      const sanitizeTagsResult = MetricsAggregator(12436).sanitizeTags(obj);
      ...
      if (value) {
        ...
        const tmp15 = new tmp5(12437).METRIC_MAP[arg0](diff);
        obj[0] = tmp15;
        obj[1] = rounded;
        obj[2] = arg0;
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(12349).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(12432).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
