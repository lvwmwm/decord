...
// Function ID: 12391
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 12382, 12363, 12335, 12389, 12346, 12342, 12358, 12359, 12391, 12378, 12379, 12362, 12384, 12371, 12392, 12375, 12393, 12340]

// Module 12390 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureMessage(arg0, arg1, arg2, arg3) {
      ...
      self = this;
      dependencyMap = arg3;
      obj = { event_id: obj(12346).uuid4() };
      const merged = Object.assign(arg2);
      const obj2 = obj(12346);
      const tmp = obj;
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(12363).DEBUG_BUILD) {
          const logger = tmp(12335).logger;
        ...
        BaseClient(12358).updateSession(release, { init: false });
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(12391).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(12391).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(12391);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(12378).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(12363).DEBUG_BUILD) {
        let logger = tmp2(12335).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(12359).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(12391).setupIntegrations(this, integrations);
      const obj = BaseClient(12391);
      const result = BaseClient(12391).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(12375).parseSampleRate(sampleRate);
        let obj = str(12375);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(12392).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(12359).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(12363).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(12335).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(12393);
