...
// Function ID: 12750
// Name: withScope
// Dependencies: [41, 42, 12750, 12736, 12747, 12755]
// Exports: getStackAsyncContextStrategy

// Module 12749 (withScope)
import closure_2 from "_classCallCheck" /* 41 */;
import _createClass from "_createClass" /* 42 */;

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(12747).getMainCarrier();
  const obj = AsyncContextStack(12747);
  const sentryCarrier = AsyncContextStack(12747).getSentryCarrier(mainCarrier);
  let stack = sentryCarrier.stack;
  if (!stack) {
    let tmpResult = tmp(12755);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(12747).getMainCarrier();
      let obj = callback(12747);
      let sentryCarrier = callback(12747).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12755);
    ...
    getCurrentScope() {
      const mainCarrier = callback(12747).getMainCarrier();
      const obj = callback(12747);
      const sentryCarrier = callback(12747).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12755);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
