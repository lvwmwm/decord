...
// Function ID: 12270
// Name: withScope
// Dependencies: [41, 42, 12270, 12256, 12267, 12275]
// Exports: getStackAsyncContextStrategy

// Module 12269 (withScope)
import closure_2 from "_classCallCheck" /* 41 */;
import _createClass from "_createClass" /* 42 */;

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(12267).getMainCarrier();
  const obj = AsyncContextStack(12267);
  const sentryCarrier = AsyncContextStack(12267).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(12267).getMainCarrier();
      let obj = callback(12267);
      let sentryCarrier = callback(12267).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12275);
    ...
    getCurrentScope() {
      const mainCarrier = callback(12267).getMainCarrier();
      const obj = callback(12267);
      const sentryCarrier = callback(12267).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12275);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
