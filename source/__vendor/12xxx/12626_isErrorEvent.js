...
// Function ID: 12627
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 12618, 12599, 12571, 12625, 12582, 12578, 12594, 12595, 12627, 12614, 12615, 12598, 12620, 12607, 12628, 12611, 12629, 12576]

// Module 12626 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureMessage(arg0, arg1, arg2, arg3) {
      ...
      self = this;
      dependencyMap = arg3;
      obj = { event_id: obj(12582).uuid4() };
      const merged = Object.assign(arg2);
      const obj2 = obj(12582);
      const tmp = obj;
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(12599).DEBUG_BUILD) {
          const logger = tmp(12571).logger;
        ...
        BaseClient(12594).updateSession(release, { init: false });
        const obj = BaseClient(12594);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(12627).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(12627).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(12627);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(12614).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(12599).DEBUG_BUILD) {
        let logger = tmp2(12571).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(12595).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(12627).setupIntegrations(this, integrations);
      const obj = BaseClient(12627);
      const result = BaseClient(12627).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(12611).parseSampleRate(sampleRate);
        let obj = str(12611);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(12628).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(12595).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(12599).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(12571).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(12629);
