...
// Function ID: 12358
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 12349, 12330, 12302, 12356, 12313, 12309, 12325, 12326, 12358, 12345, 12346, 12329, 12351, 12338, 12359, 12342, 12360, 12307]

// Module 12357 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureMessage(arg0, arg1, arg2, arg3) {
      ...
      self = this;
      dependencyMap = arg3;
      obj = { event_id: obj(12313).uuid4() };
      const merged = Object.assign(arg2);
      const obj2 = obj(12313);
      const tmp = obj;
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(12330).DEBUG_BUILD) {
          const logger = tmp(12302).logger;
        ...
        BaseClient(12325).updateSession(release, { init: false });
        const obj = BaseClient(12325);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(12358).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(12358).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(12358);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(12345).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(12330).DEBUG_BUILD) {
        let logger = tmp2(12302).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(12326).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(12358).setupIntegrations(this, integrations);
      const obj = BaseClient(12358);
      const result = BaseClient(12358).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(12342).parseSampleRate(sampleRate);
        let obj = str(12342);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(12359).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(12326).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(12330).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(12302).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(12360);
