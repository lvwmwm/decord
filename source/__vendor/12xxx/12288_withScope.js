...
// Function ID: 12289
// Name: withScope
// Dependencies: [41, 42, 12289, 12275, 12286, 12294]
// Exports: getStackAsyncContextStrategy

// Module 12288 (withScope)
import closure_2 from "_classCallCheck" /* 41 */;
import _createClass from "_createClass" /* 42 */;

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(12286).getMainCarrier();
  const obj = AsyncContextStack(12286);
  const sentryCarrier = AsyncContextStack(12286).getSentryCarrier(mainCarrier);
  let stack = sentryCarrier.stack;
  if (!stack) {
    let tmpResult = tmp(12294);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(12286).getMainCarrier();
      let obj = callback(12286);
      let sentryCarrier = callback(12286).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12294);
    ...
    getCurrentScope() {
      const mainCarrier = callback(12286).getMainCarrier();
      const obj = callback(12286);
      const sentryCarrier = callback(12286).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(12294);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
