...
// Dependencies: [32, 12276, 12282, 12271, 12284, 12254, 12266, 12267, 12268, 12261, 12259, 12277, 12249, 12281, 12285, 12287, 12278, 12288, 12290, 12264]
// Exports: continueTrace, startInactiveSpan, startNewTrace, startSpan, startSpanManual, suppressTracing, withActiveSpan

// Module 12283 (createChildOrRootSpan)
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(12290).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(12282).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(12254).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(12254);
        const client = tmp(12276).getClient();
        if (client) {
      ...
      tmp(12287).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(12287);
      const result = tmp(12278).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(12285).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(12285);
      ...
      obj1[0] = traceId2;
      obj1[1] = spanId2;
      const tmpResult7 = tmp(12254);
      const merged1 = Object.assign(spanArguments);
      const tmp42 = _startRootSpan(obj1, scope, tmp(12254).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(12254).spanIsSampled(parentSpan);
      tmp(12285).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = tmp(12285);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(12285).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = tmp(12285);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(12282).SentryNonRecordingSpan();
...
function _startRootSpan(name) {
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(12288).sampleSpan(options, obj);
    const tmpResult = tmp(12288);
  }
  ...
  obj1.attributes = { [_mod12264.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(12290).SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(12264).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  const _require = arg1;
  const mainCarrier = _require(12267).getMainCarrier();
  const obj = _require(12267);
  const tmp = _require;
  const asyncContextStrategy = _require(12268).getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(12276).withScope((setPropagationContext) => {
      ...
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = experimental(obj[1]).withScope((arg0) => {
            _null(12271)._setSpanForScope(arg0, _null);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return _require(12276).withScope((setPropagationContext) => {
...
export const startSpanManual = function startSpanManual(experimental) {
  ...
  let obj = _require(12267);
  let mainCarrier = obj.getMainCarrier();
  let asyncContextStrategy = _require(12268).getAsyncContextStrategy(mainCarrier);
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(12276).withScope((setSDKProcessingMetadata) => {
      const result = setSDKProcessingMetadata.setSDKProcessingMetadata({ [closure_1_3]: true });
      return callback();
    });
    const tmpResult = tmp(12276);
  }
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(12276).withScope((arg0) => {
      _null(12271)._setSpanForScope(arg0, _null);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(12276);
