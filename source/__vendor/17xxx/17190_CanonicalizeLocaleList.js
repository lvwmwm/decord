...
// Dependencies: [17191, 17192]

// Module 17190 (CanonicalizeLocaleList)
import RegExpResult1 from "RegExpResult1" /* 17191 */;
import createDateTimeFormat from "createDateTimeFormat" /* 17192 */;
          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "string") {
                ...
                if (isMatch) {
                  let expVariantDupes = tmp5(17191).expVariantDupes;
                  isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(17191).expSingletonDupes;
                ...
                if (isMatch) {
                  ...
                  if (!tmp20) {
                    ...
                    let RegExpResult = RegExp(`(?:${tmp17(17191).expExtSequences.source})+`, "i");
...
class List {
  constructor() {
    tmp = f66774(this, "length", { writable: true, value: 0 });
    if (!arguments.length) {
      return;
    } else {
      tmp3 = slice;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    if (isMatch) {
      const expVariantDupes = tmp(17191).expVariantDupes;
      isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(17191).expSingletonDupes;
