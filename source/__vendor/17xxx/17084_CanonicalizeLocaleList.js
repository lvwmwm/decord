...
// Dependencies: [17085, 17086]

// Module 17084 (CanonicalizeLocaleList)
import RegExpResult1 from "RegExpResult1" /* 17085 */;
import createDateTimeFormat from "createDateTimeFormat" /* 17086 */;
          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "string") {
                ...
                if (isMatch) {
                  let expVariantDupes = tmp5(17085).expVariantDupes;
                  isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(17085).expSingletonDupes;
                ...
                if (isMatch) {
                  ...
                  if (!tmp20) {
                    ...
                    let RegExpResult = RegExp(`(?:${tmp17(17085).expExtSequences.source})+`, "i");
...
class List {
  constructor() {
    tmp = f66410(this, "length", { writable: true, value: 0 });
    if (!arguments.length) {
      return;
    } else {
      tmp3 = slice;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    if (isMatch) {
      const expVariantDupes = tmp(17085).expVariantDupes;
      isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(17085).expSingletonDupes;
