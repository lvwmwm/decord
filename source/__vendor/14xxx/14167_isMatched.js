...
// Function ID: 14168
// Name: isMatched
// Dependencies: [14168, 1280, 14169]

// Module 14167 (isMatched)
  ...
  if (arr) {
    if ("$" === arr[0]) {
      ...
      const mapped = arr2.map((arg0) => {
        let tmp = callback(14169).regions[arg0];
        if (!tmp) {
          const items = [arg0];
          tmp = items;
        }
      ...
      const reduced = mapped.reduce((arg0, arg1) => {
        const obj = callback(1280);
        return obj.__spreadArray(callback(1280).__spreadArray([], arg0, true), arg1, true);
      }, []);
...
function findMatchingDistance(arg0, arg1) {
  ...
  let tmp7 = obj1;
  if (!obj1) {
    let first = _mod14168.data.supplemental.languageMatching["written-new"][0];
    let paradigmLocales;
    if (null !== first) {
      if (undefined !== first) {
    ...
    const prop = tmp8(14168).data.supplemental.languageMatching["written-new"];
    let substr = prop.slice(1, 5);
    const prop1 = tmp8(14168).data.supplemental.languageMatching["written-new"];
    ...
    let tmp8Result = tmp8(1280);
    tmp8Result = tmp8(1280);
