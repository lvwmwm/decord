...
// Function ID: 14166
// Name: ResolveLocale
// Dependencies: [14166, 14171, 14172, 14167, 14173, 14174]

// Module 14165 (ResolveLocale)
const require = arg1;
...
arg5.ResolveLocale = function ResolveLocale(arg0, arg1, localeMatcher) {
  ...
  if ("lookup" === localeMatcher.localeMatcher) {
    const _Array2 = Array;
    let LookupMatcherResult = str(14166).LookupMatcher(Array.from(arg0), arg1, arg5);
    let tmp5 = str;
    let tmp7 = str;
  } else {
    const _Array = Array;
    tmp5 = str;
    tmp7 = str;
    LookupMatcherResult = str(14171).BestFitMatcher(Array.from(arg0), arg1, arg5);
  }
  ...
  if (LookupMatcherResult.extension) {
    let keywords = tmp7(14172).UnicodeExtensionComponents(LookupMatcherResult.extension).keywords;
  } else {
    keywords = [];
  }
  const items = [];
  ...
  if (0 < arg3.length) {
    do {
      ...
      let invariantResult = str(14167).invariant(isArray, "keyLocaleData for ".concat(str, " must be an array"));
      let first = items1[0];
      ...
      let invariantResult1 = str(14167).invariant(tmp19, "value must be a string or undefined");
      ...
      let invariantResult2 = tmp14(14167).invariant(tmp24, "optionsValue must be a string or undefined");
      let str3 = tmp23;
      if (typeof tmp23 === "string") {
        let formatted = str.toLowerCase();
        str3 = tmp14(14173).CanonicalizeUValue(formatted, tmp23);
        if ("" === str3) {
          str3 = "true";
        }
      }
  ...
  let result = locale;
  if (items.length > 0) {
    result = tmp5(14174).InsertUnicodeExtensionAndCanonicalize(locale, [], items);
  }
  obj.locale = result;
  return obj;
};
