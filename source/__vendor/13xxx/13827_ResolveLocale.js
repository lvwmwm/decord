...
// Function ID: 13828
// Name: ResolveLocale
// Dependencies: [13828, 13833, 13834, 13829, 13835, 13836]

// Module 13827 (ResolveLocale)
const require = arg1;
...
arg5.ResolveLocale = function ResolveLocale(arg0, arg1, localeMatcher) {
  ...
  if ("lookup" === localeMatcher.localeMatcher) {
    const _Array2 = Array;
    let LookupMatcherResult = str(13828).LookupMatcher(Array.from(arg0), arg1, arg5);
    let tmp5 = str;
    let tmp7 = str;
  } else {
    const _Array = Array;
    tmp5 = str;
    tmp7 = str;
    LookupMatcherResult = str(13833).BestFitMatcher(Array.from(arg0), arg1, arg5);
  }
  ...
  if (LookupMatcherResult.extension) {
    let keywords = tmp7(13834).UnicodeExtensionComponents(LookupMatcherResult.extension).keywords;
  } else {
    keywords = [];
  }
  const items = [];
  ...
  if (0 < arg3.length) {
    do {
      ...
      let invariantResult = str(13829).invariant(isArray, "keyLocaleData for ".concat(str, " must be an array"));
      let first = items1[0];
      ...
      let invariantResult1 = str(13829).invariant(tmp19, "value must be a string or undefined");
      ...
      let invariantResult2 = tmp14(13829).invariant(tmp24, "optionsValue must be a string or undefined");
      let str3 = tmp23;
      if (typeof tmp23 === "string") {
        let formatted = str.toLowerCase();
        str3 = tmp14(13835).CanonicalizeUValue(formatted, tmp23);
        if ("" === str3) {
          str3 = "true";
        }
      }
  ...
  let result = locale;
  if (items.length > 0) {
    result = tmp5(13836).InsertUnicodeExtensionAndCanonicalize(locale, [], items);
  }
  obj.locale = result;
  return obj;
};
