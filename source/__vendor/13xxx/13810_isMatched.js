...
// Function ID: 13811
// Name: isMatched
// Dependencies: [13811, 1281, 13812]

// Module 13810 (isMatched)
  ...
  if (arr) {
    if ("$" === arr[0]) {
      ...
      const mapped = arr2.map((arg0) => {
        let tmp = callback(13812).regions[arg0];
        if (!tmp) {
          const items = [arg0];
          tmp = items;
        }
...
function findMatchingDistance(arg0, arg1) {
  ...
  let tmp7 = obj1;
  if (!obj1) {
    let first = _mod13811.data.supplemental.languageMatching["written-new"][0];
    let paradigmLocales;
    if (null !== first) {
      if (undefined !== first) {
    ...
    const prop = tmp8(13811).data.supplemental.languageMatching["written-new"];
    let substr = prop.slice(1, 5);
    const prop1 = tmp8(13811).data.supplemental.languageMatching["written-new"];
