...
// Dependencies: [32, 19, 4221]
// Exports: useRiveProperty

// Module 4220 (useRiveProperty)
...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f29608) {
  const _require = instance;
  dependencyMap = FillColor;
  const callback = f29608;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = _require(4221).useDisposableMemo(() => {
    if (closure_0) {
