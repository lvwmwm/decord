...
// Dependencies: [32, 19, 4220]
// Exports: useRiveProperty

// Module 4219 (useRiveProperty)
...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f29579) {
  const _require = instance;
  dependencyMap = FillColor;
  const callback = f29579;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = _require(4220).useDisposableMemo(() => {
    if (closure_0) {
