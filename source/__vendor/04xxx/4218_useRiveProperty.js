...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f29557) {
  const _require = instance;
  dependencyMap = FillColor;
  const callback = f29557;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = _require(4219).useDisposableMemo(() => {
    if (closure_0) {
