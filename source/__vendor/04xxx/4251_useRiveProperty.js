...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f30035) {
  const _require = instance;
  dependencyMap = FillColor;
  const callback = f30035;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = _require(4252).useDisposableMemo(() => {
    if (closure_0) {
