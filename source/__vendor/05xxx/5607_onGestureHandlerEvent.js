...
// Dependencies: [17, 5608, 5610, 5611, 5612]
// Exports: startListening, stopListening

// Module 5607 (onGestureHandlerEvent)
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import map2 from "map" /* 5608 */;
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(5610).State.UNDETERMINED) {
        if (handlerTag.state === tmp(5610).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(5610).State.BEGAN) {
        if (handlerTag.state === tmp(5610).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(5610).State.END) {
          if (handlerTag.oldState === tmp(5610).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(5610).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(5610).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(5611).GestureStateManager;
      ...
      if (tmp(5612).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(5612).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(5612).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(5612).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(5608).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
