...
// Dependencies: [5719, 5711, 5657]

// Module 5726 (traverseAndConfigureRelations)
...
function traverseAndConfigureRelations(gestures, map, set) {
  ...
  if (obj.isComposedGesture(gestures)) {
    ...
    gestures.gestureRelations = tmp(5719).prepareRelations(gestures.config, gestures.handlerTag);
...
arg5.configureRelations = function configureRelations(externalSimultaneousHandlers) {
  ...
  if (obj2.isComposedGesture(externalSimultaneousHandlers)) {
    ...
    if (externalSimultaneousHandlers.type === set(5711).ComposedGestureName.Simultaneous) {
