...
// Function ID: 5736
// Name: useGesture
// Dependencies: [19, 5674, 5657, 5736, 5720, 5696, 5683, 5656]
// Exports: useGesture

// Module 5735 (useGesture)
...
export const useGesture = function useGesture(Fling, clonedAndRemappedConfig) {
  ...
  if (clonedAndRemappedConfig.disableReanimated !== jsEventHandler(() => clonedAndRemappedConfig.disableReanimated, [])) {
    const _Error2 = Error;
    error = new Error(_require(5657).tagMessage("The \"disableReanimated\" property must not be changed after the handler is created."));
    throw error;
  } else {
    const gestureCallbacks = _require(5736).useGestureCallbacks(tmp2, clonedAndRemappedConfig);
    ...
    if (clonedAndRemappedConfig.shouldUseReanimatedDetector) {
      if (!reanimatedEventHandler) {
        const _Error = Error;
        const error1 = new Error(_require(5657).tagMessage("Failed to create reanimated event handlers."));
        throw error1;
      }
    }
    const items = [tmp2, , , ];
