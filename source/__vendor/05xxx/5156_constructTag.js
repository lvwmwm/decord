...
// Function ID: 5157
// Name: constructTag
// Dependencies: [5112, 5128, 5140, 5109, 5116, 5142]

// Module 5156 (constructTag)
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (type !== _mod5116.TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(5109).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(5109);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === _mod5116.TYPE_ITXT) {
    stringFromDataView = module(5142).decode("UTF-8", decompressResult);
    const obj5 = module(5142);
  }
