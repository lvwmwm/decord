...
// Function ID: 5851
// Name: memo
// Dependencies: [19, 17, 21, 1652, 5644, 5637, 5633, 5660]
...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let iter;
  const tmp4 = iter(scrollableGesture(5644).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  let obj = scrollableGesture(5637);
  ...
  const fn = function f() {
    return { enabled: iter.value === scrollableGesture(5633).SCROLLABLE_STATE.UNLOCKED };
  };
  obj = { animatedScrollableState: iter, SCROLLABLE_STATE: tmp2(5633).SCROLLABLE_STATE };
  fn.__closure = obj;
  ...
  const tmp7 = callback(() => {
    let result;
    if (dependencyMap) {
      const Gesture = scrollableGesture(5660).Gesture;
  ...
  if (tmp7) {
    ...
    let tmp8Result = tmp8(tmp2(5660).GestureDetector, obj);
  } else {
    const obj2 = {};
