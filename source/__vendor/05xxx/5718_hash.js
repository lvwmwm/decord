...
// Function ID: 5719
// Name: hash
// Dependencies: [32, 5705, 5704, 5719, 5728]
...
function maybeUnpackValue(enabled) {
  const Reanimated = _mod5704.Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: require("module_5704").Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod5704.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(5704).Reanimated;
...
export const bindSharedValues = function bindSharedValues(closure_1, closure_2) {
  ...
  if (undefined !== _require(5704).Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let Reanimated = _require(5704).Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let tmp13 = tmp9;
        let tmp14 = tmp11;
        let Reanimated2 = tmp10(5704).Reanimated;
        let tmp15 = tmp7;
        let tmp16 = tmp6;
...
export const unbindSharedValues = function unbindSharedValues(closure_1, closure_2) {
  if (undefined !== _require(5704).Reanimated) {
    ...
    function _loop(iter) {
      const callback = iter;
      const Reanimated = callback(5704).Reanimated;
      if (Reanimated.isSharedValue(iter)) {
        ...
        dependencyMap = sum;
        const Reanimated2 = callback(5704).Reanimated;
        const fn = function t() {
