...
// Dependencies: [32, 19, 17, 21, 5679, 5739, 5675, 5734, 5676, 5666, 5704, 5735, 5736, 5728, 5737]
// Exports: InterceptingGestureDetector

// Module 5738 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  if (prop) {
    let REANIMATED = tmp(5739).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(5739).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(5739).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(5739).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(5675).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(5734).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(5676);
  }
  ...
  closure_5(() => {
    ...
    if (prop) {
      store.setMode(gesture(5739).InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        store.setMode(gesture(5739).InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const detectorAttachmentGuard = tmp(5737).useDetectorAttachmentGuard(tmp8Result1);
    ...
    obj1[10] = prop6;
    obj1[11] = tmp8Result1;
    obj1[12] = tmp(5675).nativeDetectorStyles.detector;
    obj1[13] = tmp9;
    const _globalThis = globalThis;
    obj1[14] = globalThis._RNGH_MODULE_ID;
    obj1[15] = children;
    obj[1] = closure_8(ReanimatedNativeDetector, obj1);
    return closure_8(tmp(5739).InterceptingDetectorContext, obj);
  } else {
    const _Error = Error;
    error = new Error(tmp(5666).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
