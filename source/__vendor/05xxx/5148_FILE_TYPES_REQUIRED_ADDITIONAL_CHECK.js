...
// Dependencies: [41, 42, 5146, 5149, 5155, 5156, 5157, 5158, 5159]

// Module 5148 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5155 */;
import ImageTypes from "ImageTypes" /* 5156 */;
import VideoTypes from "VideoTypes" /* 5157 */;
import CompressedTypes from "CompressedTypes" /* 5158 */;
import OtherTypes from "OtherTypes" /* 5159 */;
...
let items = [
  {
    ...
    value: function getInfoByName(key10027) {
      return FileTypes(5146).fetchFromObject(FileTypes, key10027.toUpperCase());
    }
  },
  {
    key: "getSignaturesByName",
    value: function getSignaturesByName(item10079) {
      return FileTypes(5146).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((arg0) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5149).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5149).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5149).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5146).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5149).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((arg0) => {
          ...
          if (FileTypes(5146).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
