...
// Dependencies: [32, 19, 17, 21, 5635, 5695, 5631, 5690, 5632, 5622, 5660, 5691, 5692, 5684, 5693]
// Exports: InterceptingGestureDetector

// Module 5694 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  if (prop) {
    let REANIMATED = tmp(5695).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(5695).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(5695).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(5695).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(5631).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(5690).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(5632);
  }
  ...
  closure_5(() => {
    ...
    if (prop) {
      store.setMode(gesture(5695).InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        store.setMode(gesture(5695).InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const detectorAttachmentGuard = tmp(5693).useDetectorAttachmentGuard(tmp8Result1);
    ...
    obj1[10] = prop6;
    obj1[11] = tmp8Result1;
    obj1[12] = tmp(5631).nativeDetectorStyles.detector;
    obj1[13] = tmp9;
    const _globalThis = globalThis;
    obj1[14] = globalThis._RNGH_MODULE_ID;
    obj1[15] = children;
    obj[1] = closure_8(ReanimatedNativeDetector, obj1);
    return closure_8(tmp(5695).InterceptingDetectorContext, obj);
  } else {
    const _Error = Error;
    error = new Error(tmp(5622).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
