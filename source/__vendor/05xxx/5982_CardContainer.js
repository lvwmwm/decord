...
// Function ID: 5983
// Name: CardContainer
// Dependencies: [19, 17, 21, 1501, 5454, 5983, 5984, 5985, 5972]

// Module 5982 (CardContainer)
...
export const CardContainer = importAllResult.memo(function CardContainerInner(active) {
  ...
  obj1 = active(1501);
  let num = importAllResult.useContext(active(5454).HeaderHeightContext);
  let tmp4 = focused;
  if (focused) {
    tmp4 = false !== scene.descriptor.options.keyboardHandlingEnabled;
  }
  let tmp2Result = tmp2(5983);
  ...
  if (previousScene) {
    ...
    headerTitle = tmp2(5454).getHeaderTitle(options, route.name);
    href = tmp2Result1.useLinkBuilder().buildHref(route.name, route.params);
    tmp8 = href;
    tmp9 = headerTitle;
    const tmp2Result2 = tmp2(5454);
  }
  closure_15 = tmp12;
  ...
  obj8[1] = closure_4(active(5454).HeaderHeightContext.Provider, { value: headerHeight, children: descriptor.render() });
  obj7[1] = closure_4(active(5454).HeaderShownContext.Provider, obj8);
  obj6[1] = closure_4(active(5454).HeaderBackContext.Provider, obj7);
  items4[1] = closure_4(closure_3, obj6);
  obj4[1] = items4;
  obj3[1] = tmp22(active(5972).ModalPresentationContext.Provider, obj4);
  obj1[29] = closure_4(closure_3, obj3);
  obj[6] = closure_4(active(5985).Card, obj1);
  return closure_4(active(5984).CardA11yWrapper, obj);
});
