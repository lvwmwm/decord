  ...
  findJpegOffsets(byteLength) {
    ...
    if (c5 + c6 + 5 <= byteLength.byteLength) {
      while (true) {
        ...
        let tmp28 = tmp8;
        let sum5 = tmp9;
        if (module(5112).USE_FILE) {
          let tmp30 = c19;
          if (byteLength.getUint16(sum) === c19) {
            break;
          }
        }
        if (tmp19(5112).USE_FILE) {
          let tmp31 = c20;
