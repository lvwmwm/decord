...
// Dependencies: [5684, 5676, 5622]

// Module 5691 (traverseAndConfigureRelations)
...
function traverseAndConfigureRelations(gestures, map, set) {
  ...
  if (obj.isComposedGesture(gestures)) {
    ...
    gestures.gestureRelations = tmp(5684).prepareRelations(gestures.config, gestures.handlerTag);
...
arg5.configureRelations = function configureRelations(externalSimultaneousHandlers) {
  ...
  if (obj2.isComposedGesture(externalSimultaneousHandlers)) {
    ...
    if (externalSimultaneousHandlers.type === set(5676).ComposedGestureName.Simultaneous) {
