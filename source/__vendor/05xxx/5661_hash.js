...
// Function ID: 5662
// Name: hash
// Dependencies: [32, 5648, 5647, 5662, 5671]
...
function maybeUnpackValue(enabled) {
  const Reanimated = _mod5647.Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: require("module_5647").Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod5647.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(5647).Reanimated;
...
export const bindSharedValues = function bindSharedValues(closure_1, closure_2) {
  ...
  if (undefined !== _require(5647).Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      let Reanimated = _require(5647).Reanimated;
      if (Reanimated.isSharedValue(tmp7)) {
        let tmp13 = tmp9;
        let tmp14 = tmp11;
        let Reanimated2 = tmp10(5647).Reanimated;
        let tmp15 = tmp7;
        let tmp16 = tmp6;
...
export const unbindSharedValues = function unbindSharedValues(closure_1, closure_2) {
  if (undefined !== _require(5647).Reanimated) {
    ...
    function _loop(iter) {
      const callback = iter;
      const Reanimated = callback(5647).Reanimated;
      if (Reanimated.isSharedValue(iter)) {
        ...
        dependencyMap = sum;
        const Reanimated2 = callback(5647).Reanimated;
        const fn = function t() {
