...
// Dependencies: [5863, 5864, 5876, 5878, 5879, 5851, 5881, 5892, 5893, 5894]

// Module 5875 (RecyclerViewManager)
import _createClassDefault from "_createClass" /* 5864 */;
import ViewabilityManagerDefault from "ViewabilityManager" /* 5879 */;
import closure_3 from "_classCallCheck" /* 5863 */;
    ...
    value: function updateLayoutParams(windowSize, firstItemOffset) {
      ...
      if (this.layoutManager) {
        ...
        if (BooleanResult !== Boolean(self.propsRef.horizontal)) {
          const _Error = Error;
          error = new Error(RecyclerViewManager(5851).ErrorMessages.horizontalPropCannotBeToggled);
          throw error;
        }
    ...
    value: function getLayoutManagerClass() {
      ...
      if (this.propsRef.masonry) {
        if (self.propsRef.horizontal) {
          const _Error2 = Error;
          error = new Error(RecyclerViewManager(5851).ErrorMessages.masonryAndHorizontalIncompatible);
          throw error;
        }
      }
      if (self.numColumns > 1) {
        if (self.propsRef.horizontal) {
          const _Error = Error;
          const error1 = new Error(RecyclerViewManager(5851).ErrorMessages.numColumnsAndHorizontalIncompatible);
          throw error1;
        }
      }
      if (self.propsRef.masonry) {
        let RVLinearLayoutManagerImpl = RecyclerViewManager(5881).RVMasonryLayoutManagerImpl;
      } else {
        if (self.numColumns > 1) {
          if (!self.propsRef.horizontal) {
            RVLinearLayoutManagerImpl = RecyclerViewManager(5892).RVGridLayoutManagerImpl;
          }
        }
