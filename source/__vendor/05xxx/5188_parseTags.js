...
// Function ID: 5189
// Name: parseTags
// Dependencies: [5189, 5190]

// Module 5188 (parseTags)
const module = arg2;
    ...
    if (tmp < byteLength.byteLength) {
      while (true) {
        ...
        if (byteLength.getUint8(tmp) !== 28) {
          ...
          let tmp15 = obj;
          let tmp16 = encoding;
          obj[3] = getTagDescription(tmp8(5189).iptc[uint16], items, obj, encoding);
          let tmp17 = tmp8(5189).iptc[uint16] && tmp8(5189).iptc[uint16].repeatable;
          if (tmp17) {
            obj.repeatable = true;
          }
          let tmp18 = tmp8(5189).iptc[uint16] && undefined !== tmp8(5189).iptc[uint16].encoding_name;
          if (tmp18) {
            let obj4 = tmp8(5189).iptc[uint16];
