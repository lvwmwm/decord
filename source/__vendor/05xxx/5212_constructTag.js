...
// Function ID: 5213
// Name: constructTag
// Dependencies: [5168, 5184, 5196, 5165, 5172, 5198]

// Module 5212 (constructTag)
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (type !== _mod5172.TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(5165).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(5165);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === _mod5172.TYPE_ITXT) {
    stringFromDataView = module(5198).decode("UTF-8", decompressResult);
    const obj5 = module(5198);
  }
