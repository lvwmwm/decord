...
// Dependencies: [41, 42, 5138, 5141, 5147, 5148, 5149, 5150, 5151]

// Module 5140 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5147 */;
import ImageTypes from "ImageTypes" /* 5148 */;
import VideoTypes from "VideoTypes" /* 5149 */;
import CompressedTypes from "CompressedTypes" /* 5150 */;
import OtherTypes from "OtherTypes" /* 5151 */;
...
let items = [
  {
    ...
    value: function getInfoByName(key10027) {
      return FileTypes(5138).fetchFromObject(FileTypes, key10027.toUpperCase());
    }
  },
  {
    key: "getSignaturesByName",
    value: function getSignaturesByName(item10079) {
      return FileTypes(5138).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((arg0) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5141).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5141).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5141).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5138).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5141).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((arg0) => {
          ...
          if (FileTypes(5138).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
