...
// Dependencies: [5685, 5623, 5638, 5625, 5687]

// Module 5703 (handleStateChangeEvent)
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, arg3) {
  ...
  if (oldState === _mod5623.State.UNDETERMINED) {
    if (state === tmp(5623).State.BEGAN) {
      let tmpResult = tmp(5685);
      tmpResult.runCallback(tmp(5638).CALLBACK_TYPE.BEGAN, arg1, result);
    }
  }
  if (oldState === _mod5623.State.BEGAN) {
    if (state === tmp(5623).State.ACTIVE) {
      if (arg3 != null) {
        arg3(result);
      }
      tmpResult = tmp(5685);
      tmpResult.runCallback(tmp(5638).CALLBACK_TYPE.START, arg1, result);
    }
  }
  if (oldState !== state) {
    obj = {};
    const merged = Object.assign(result);
    obj.canceled = state === tmp(5623).State.FAILED || state === tmp(5623).State.CANCELLED;
    if (oldState === tmp(5623).State.ACTIVE) {
      if (arg3 != null) {
        arg3(obj);
      }
      tmp(5685).runCallback(tmp(5638).CALLBACK_TYPE.END, arg1, obj);
      const tmpResult1 = tmp(5685);
    }
    const tmp7 = state === tmp(5623).State.FAILED || state === tmp(5623).State.CANCELLED;
    tmp(5685).runCallback(tmp(5638).CALLBACK_TYPE.FINALIZE, arg1, obj);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult2 = tmp(5685);
  }
}
let obj = { flattenAndFilterEvent: require("isGestureEnabled").flattenAndFilterEvent, State: require("module_5623").State, runCallback: require("isGestureEnabled").runCallback, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
...
function handleTouchEvent(eventType) {
  if (eventType.eventType !== _mod5625.TouchEventType.UNDETERMINED) {
    let tmpResult = tmp(5685);
    tmpResult = tmp(5685);
    tmpResult.runCallback(tmpResult.touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
  }
}
obj = { TouchEventType: require("module_5625").TouchEventType, runCallback: require("isGestureEnabled").runCallback, touchEventTypeToCallbackType: require("isGestureEnabled").touchEventTypeToCallbackType };
  ...
  if (obj2.isEventForHandlerWithTag(arg0, result)) {
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult.isTouchEvent(result)) {
        ...
        if (result.eventType !== tmp(5625).TouchEventType.UNDETERMINED) {
          const tmpResult1 = tmp(5685);
          tmpResult1.runCallback(tmp(5685).touchEventTypeToCallbackType(result.eventType), arg2, result);
          const tmpResult2 = tmp(5685);
        }
      } else if (!arg5) {
        ...
        const result1 = tmp(5685).flattenAndFilterEvent(tmp9);
        const tmpResult3 = tmp(5685);
        tmp(5685).runCallback(tmp(5638).CALLBACK_TYPE.UPDATE, arg2, result1);
        if (lastUpdateEvent) {
          lastUpdateEvent.lastUpdateEvent = result;
        }
        const tmpResult4 = tmp(5685);
      }
    }
  }
}
