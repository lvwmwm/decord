...
// Dependencies: [32, 19, 17, 21, 5670, 5730, 5666, 5725, 5667, 5657, 5695, 5726, 5727, 5719, 5728]
// Exports: InterceptingGestureDetector

// Module 5729 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  if (prop) {
    let REANIMATED = tmp(5730).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(5730).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(5730).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(5730).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(5666).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(5725).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(5667);
  }
  ...
  closure_5(() => {
    ...
    if (prop) {
      store.setMode(gesture(5730).InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        store.setMode(gesture(5730).InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const detectorAttachmentGuard = tmp(5728).useDetectorAttachmentGuard(tmp8Result1);
    ...
    obj1[10] = prop6;
    obj1[11] = tmp8Result1;
    obj1[12] = tmp(5666).nativeDetectorStyles.detector;
    obj1[13] = tmp9;
