...
// Function ID: 5164
// Name: load
// Dependencies: [5164, 5165, 5166, 5167, 5168, 5181, 5183, 5184, 5196, 5200, 5204, 5206, 5208, 5209, 5210, 5211, 5212, 5213, 5214, 5215, 5216, 5217, 5190]
// Exports: load

// Module 5163 (load)
import errorDefault from "error" /* 5164 */;
  ...
  if (typeof response === "string") {
    ...
    if (obj1.test(response)) {
      ...
      if (obj2.test(response)) {
        let resolved = Promise.resolve(response(5165).dataUriToBuffer(response));
        const obj5 = response(5165);
      } else {
        obj2 = then;
        if (then === undefined) {
          obj2 = {};
