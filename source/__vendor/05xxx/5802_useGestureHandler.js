...
// Dependencies: [1652, 5661, 5634]

// Module 5802 (useGestureHandler)
const require = arg1;
...
arg5.useGestureHandler = (source, state, gestureSource, onStart, onChange, onEnd, onFinalize) => {
  ...
  obj = { state, State: _require(5661).State, gestureSource, source, onStart };
  R.__closure = obj;
  ...
  obj[1] = _require(1652).useWorkletCallback(U, items1);
  const obj4 = _require(1652);
  class C {
    constructor(arg0) {
      if (closure_2.value === closure_0) {
        tmp3 = source;
  ...
  obj = { gestureSource, source, state, GESTURE_SOURCE: _require(5634).GESTURE_SOURCE, onEnd };
  C.__closure = obj;
  C.__workletHash = 10682034812271;
  C.__initData = closure_4;
  const items2 = [state, gestureSource, source, onEnd];
  obj[2] = _require(1652).useWorkletCallback(C, items2);
  ...
  obj1 = { gestureSource, source, state, GESTURE_SOURCE: _require(5634).GESTURE_SOURCE, onFinalize };
  T.__closure = obj1;
  T.__workletHash = 9696716573416;
  T.__initData = closure_5;
  const items3 = [state, gestureSource, source, onFinalize];
  obj[3] = _require(1652).useWorkletCallback(T, items3);
  return obj;
};
