...
// Dependencies: [41, 42, 5081, 5084, 5090, 5091, 5092, 5093, 5094]

// Module 5083 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5090 */;
import ImageTypes from "ImageTypes" /* 5091 */;
import VideoTypes from "VideoTypes" /* 5092 */;
import CompressedTypes from "CompressedTypes" /* 5093 */;
import OtherTypes from "OtherTypes" /* 5094 */;
...
let items = [
  {
    ...
    value: function getInfoByName(key10027) {
      return FileTypes(5081).fetchFromObject(FileTypes, key10027.toUpperCase());
    }
  },
  {
    key: "getSignaturesByName",
    value: function getSignaturesByName(item10079) {
      return FileTypes(5081).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((arg0) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5084).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5084).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5084).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5081).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5084).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((arg0) => {
          ...
          if (FileTypes(5081).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
