...
// Dependencies: [41, 42, 5097, 5100, 5106, 5107, 5108, 5109, 5110]

// Module 5099 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5106 */;
import ImageTypes from "ImageTypes" /* 5107 */;
import VideoTypes from "VideoTypes" /* 5108 */;
import CompressedTypes from "CompressedTypes" /* 5109 */;
import OtherTypes from "OtherTypes" /* 5110 */;
...
let items = [
  {
    ...
    value: function getInfoByName(key10027) {
      return FileTypes(5097).fetchFromObject(FileTypes, key10027.toUpperCase());
    }
  },
  {
    key: "getSignaturesByName",
    value: function getSignaturesByName(item10079) {
      return FileTypes(5097).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((arg0) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5100).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5100).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5100).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5097).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5100).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((arg0) => {
          ...
          if (FileTypes(5097).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
