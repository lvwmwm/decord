...
arg5.isAVI = function isAVI(fileChunk) {
  ...
  const FileTypes = _mod5147.FileTypes;
...
arg5.isMP4 = function isMP4(fileChunk, excludeSimilarTypes) {
  ...
  if (!checkByFileTypeResult) {
    ...
    if (!excludeSimilarTypes) {
      const fileChunk1 = tmp(5146).getFileChunk(fileChunk);
      const FileTypes2 = tmp(5147).FileTypes;
      tmp8 = FileTypes2.checkByFileType(fileChunk1, "m4v") && tmp(5146).isftypStringIncluded(fileChunk1);
      const tmp10 = FileTypes2.checkByFileType(fileChunk1, "m4v") && tmp(5146).isftypStringIncluded(fileChunk1);
    }
