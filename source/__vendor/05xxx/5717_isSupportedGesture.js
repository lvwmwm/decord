...
// Dependencies: [32, 19, 5718, 5729, 5720, 5733, 5704]
// Exports: useJSResponderHandler

// Module 5717 (isSupportedGesture)
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(5720).SingleGestureName.Tap !== type) {
      if (tmp(5720).SingleGestureName.LongPress !== type) {
        if (tmp(5720).SingleGestureName.Fling !== type) {
          if (tmp(5720).SingleGestureName.Native !== type) {
            if (tmp(5720).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  ...
  let tmp = callback2(_require(5733).JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  callback4(() => {
    const Reanimated = gesture(5704).Reanimated;
    let obj = gesture(5718);
  ...
  const tmp7 = callback3(() => {
    let some = gesture;
    const isGestureEnabledResult = gesture(5729).isGestureEnabled(gesture);
  ...
  let handleStartShouldSetResponder = callback3(() => {
    if (callback()) {
      const result = gesture(5733).updateResponderEventValue(dependencyMap, true);
      const obj = gesture(5733);
    }
    return false;
  }, items2);
  if (null == tmp) {
