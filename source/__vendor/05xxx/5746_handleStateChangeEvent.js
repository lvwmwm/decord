...
// Dependencies: [5728, 5666, 5681, 5668, 5730]

// Module 5746 (handleStateChangeEvent)
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, arg3) {
  ...
  if (oldState === _mod5666.State.UNDETERMINED) {
    if (state === tmp(5666).State.BEGAN) {
      let tmpResult = tmp(5728);
      tmpResult.runCallback(tmp(5681).CALLBACK_TYPE.BEGAN, arg1, result);
    }
  }
  if (oldState === _mod5666.State.BEGAN) {
    if (state === tmp(5666).State.ACTIVE) {
      if (arg3 != null) {
        arg3(result);
      }
      tmpResult = tmp(5728);
      tmpResult.runCallback(tmp(5681).CALLBACK_TYPE.START, arg1, result);
    }
  }
  if (oldState !== state) {
    obj = {};
    const merged = Object.assign(result);
    obj.canceled = state === tmp(5666).State.FAILED || state === tmp(5666).State.CANCELLED;
    if (oldState === tmp(5666).State.ACTIVE) {
      if (arg3 != null) {
        arg3(obj);
      }
      tmp(5728).runCallback(tmp(5681).CALLBACK_TYPE.END, arg1, obj);
      const tmpResult1 = tmp(5728);
    }
    const tmp7 = state === tmp(5666).State.FAILED || state === tmp(5666).State.CANCELLED;
    tmp(5728).runCallback(tmp(5681).CALLBACK_TYPE.FINALIZE, arg1, obj);
    if (lastUpdateEvent) {
...
let obj = { flattenAndFilterEvent: require("isGestureEnabled").flattenAndFilterEvent, State: require("module_5666").State, runCallback: require("isGestureEnabled").runCallback, CALLBACK_TYPE: require("_isNativeReflectConstruct").CALLBACK_TYPE };
...
function handleTouchEvent(eventType) {
  if (eventType.eventType !== _mod5668.TouchEventType.UNDETERMINED) {
    let tmpResult = tmp(5728);
    tmpResult = tmp(5728);
    tmpResult.runCallback(tmpResult.touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
  }
}
obj = { TouchEventType: require("module_5668").TouchEventType, runCallback: require("isGestureEnabled").runCallback, touchEventTypeToCallbackType: require("isGestureEnabled").touchEventTypeToCallbackType };
  ...
  if (obj2.isEventForHandlerWithTag(arg0, result)) {
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult.isTouchEvent(result)) {
        ...
        if (result.eventType !== tmp(5668).TouchEventType.UNDETERMINED) {
          const tmpResult1 = tmp(5728);
          tmpResult1.runCallback(tmp(5728).touchEventTypeToCallbackType(result.eventType), arg2, result);
          const tmpResult2 = tmp(5728);
        }
      } else if (!arg5) {
        ...
        const result1 = tmp(5728).flattenAndFilterEvent(tmp9);
        const tmpResult3 = tmp(5728);
        tmp(5728).runCallback(tmp(5681).CALLBACK_TYPE.UPDATE, arg2, result1);
        if (lastUpdateEvent) {
          lastUpdateEvent.lastUpdateEvent = result;
        }
        const tmpResult4 = tmp(5728);
      }
    }
  }
}
