...
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  ...
  const obj2 = _modDef5230;
  const sum1 = sum + _modDef5230.getTypeSize("LONG");
  const obj3 = _modDef5230;
  ...
  if (undefined !== _modDef5230.typeSizes[shortAt1]) {
    if ((function tagValueFitsInOffsetSlot(shortAt1, longAt) {
      const result = callback(5230).typeSizes[shortAt1] * longAt;
      return result <= callback(5230).getTypeSize("LONG");
    })(shortAt1, longAt)) {
      const sum2 = sum + sum1;
      ...
      if ((function tagValueFitsInDataView(byteLength, sum, longAt1, shortAt1, longAt) {
        sum = sum + longAt1;
        return sum + callback(5230).typeSizes[shortAt1] * longAt <= byteLength.byteLength;
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, sum, byteOrder) {
  return sum + _modDef5230.getLongAt(buffer, sum + 4, byteOrder);
};
export { readIfd };
