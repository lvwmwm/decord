...
// Function ID: 5745
// Name: useGesture
// Dependencies: [19, 5683, 5666, 5745, 5729, 5705, 5692, 5665]
// Exports: useGesture

// Module 5744 (useGesture)
...
export const useGesture = function useGesture(Fling, clonedAndRemappedConfig) {
  ...
  if (clonedAndRemappedConfig.disableReanimated !== jsEventHandler(() => clonedAndRemappedConfig.disableReanimated, [])) {
    const _Error2 = Error;
    error = new Error(_require(5666).tagMessage("The \"disableReanimated\" property must not be changed after the handler is created."));
    throw error;
  } else {
    const gestureCallbacks = _require(5745).useGestureCallbacks(tmp2, clonedAndRemappedConfig);
    ...
    if (clonedAndRemappedConfig.shouldUseReanimatedDetector) {
      if (!reanimatedEventHandler) {
        const _Error = Error;
        const error1 = new Error(_require(5666).tagMessage("Failed to create reanimated event handlers."));
        throw error1;
      }
    }
    const items = [tmp2, , , ];
