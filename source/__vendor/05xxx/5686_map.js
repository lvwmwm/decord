...
// Function ID: 5687
// Name: map
// Dependencies: [32, 19, 5684, 5674, 5687, 5660, 5675, 5622]
...
import hash from "hash" /* 5674 */;
import items2 from "items2" /* 5675 */;
import isNativeEvent from "isNativeEvent" /* 5687 */;
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!disableReanimated) {
    result = undefined !== _mod5660.Reanimated;
  }
  if (result) {
  ...
  while (tmp12 !== undefined) {
    ...
    if (!allowedNativeProps.has(first)) {
      ...
      if (!EMPTY_WHITE_LIST.has(tmp17)) {
        let tmp23 = tmp18;
        let tmp24 = tmp20;
        let PropsToFilter = tmp19(5675).PropsToFilter;
        let tmp25 = first;
    ...
    let tmp32 = tmp18;
    let tmp33 = tmp20;
    let Reanimated = tmp19(5660).Reanimated;
