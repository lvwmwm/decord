...
// Dependencies: [32, 19, 5674, 5685, 5676, 5689, 5660]
// Exports: useJSResponderHandler

// Module 5673 (isSupportedGesture)
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(5676).SingleGestureName.Tap !== type) {
      if (tmp(5676).SingleGestureName.LongPress !== type) {
        if (tmp(5676).SingleGestureName.Fling !== type) {
          if (tmp(5676).SingleGestureName.Native !== type) {
            if (tmp(5676).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  ...
  let tmp = callback2(_require(5689).JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  callback4(() => {
    const Reanimated = gesture(5660).Reanimated;
    let obj = gesture(5674);
  ...
  const tmp7 = callback3(() => {
    let some = gesture;
    const isGestureEnabledResult = gesture(5685).isGestureEnabled(gesture);
  ...
  let handleStartShouldSetResponder = callback3(() => {
    if (callback()) {
      const result = gesture(5689).updateResponderEventValue(dependencyMap, true);
      const obj = gesture(5689);
    }
    return false;
  }, items2);
  if (null == tmp) {
