...
// Dependencies: [32, 19, 5677, 5688, 5679, 5692, 5663]
// Exports: useJSResponderHandler

// Module 5676 (isSupportedGesture)
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(5679).SingleGestureName.Tap !== type) {
      if (tmp(5679).SingleGestureName.LongPress !== type) {
        if (tmp(5679).SingleGestureName.Fling !== type) {
          if (tmp(5679).SingleGestureName.Native !== type) {
            if (tmp(5679).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  ...
  let tmp = callback2(_require(5692).JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  callback4(() => {
    const Reanimated = gesture(5663).Reanimated;
    let obj = gesture(5677);
  ...
  const tmp7 = callback3(() => {
    let some = gesture;
    const isGestureEnabledResult = gesture(5688).isGestureEnabled(gesture);
  ...
  let handleStartShouldSetResponder = callback3(() => {
    if (callback()) {
      const result = gesture(5692).updateResponderEventValue(dependencyMap, true);
      const obj = gesture(5692);
    }
    return false;
  }, items2);
  if (null == tmp) {
