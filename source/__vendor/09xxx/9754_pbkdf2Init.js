...
// Function ID: 9755
// Name: pbkdf2Init
// Dependencies: [5, 9752, 9751, 9755]
// Exports: pbkdf2, pbkdf2Async

// Module 9754 (pbkdf2Init)
import _asyncLoop from "_asyncLoop" /* 9751 */;
import number from "number" /* 9752 */;
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9751).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9755).hmac;
    let obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9751).toBytes(B);
