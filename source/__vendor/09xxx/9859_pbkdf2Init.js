...
// Function ID: 9860
// Name: pbkdf2Init
// Dependencies: [5, 9857, 9856, 9860]
// Exports: pbkdf2, pbkdf2Async

// Module 9859 (pbkdf2Init)
import _asyncLoop from "_asyncLoop" /* 9856 */;
import number from "number" /* 9857 */;
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9856).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9860).hmac;
    let obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9856).toBytes(B);
