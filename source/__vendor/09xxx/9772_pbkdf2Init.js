...
// Function ID: 9773
// Name: pbkdf2Init
// Dependencies: [5, 9770, 9769, 9773]
// Exports: pbkdf2, pbkdf2Async

// Module 9772 (pbkdf2Init)
import _asyncLoop from "_asyncLoop" /* 9769 */;
import number from "number" /* 9770 */;
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9769).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9773).hmac;
    let obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9769).toBytes(B);
