...
// Function ID: 9074
// Name: pbkdf2Init
// Dependencies: [5, 9071, 9070, 9074]
// Exports: pbkdf2, pbkdf2Async

// Module 9073 (pbkdf2Init)
import _asyncLoop from "_asyncLoop" /* 9070 */;
import number from "number" /* 9071 */;
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9070).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9074).hmac;
    let obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9070).toBytes(B);
