...
// Function ID: 9856
// Name: pbkdf2Init
// Dependencies: [5, 9853, 9852, 9856]
// Exports: pbkdf2, pbkdf2Async

// Module 9855 (pbkdf2Init)
import _asyncLoop from "_asyncLoop" /* 9852 */;
import number from "number" /* 9853 */;
...
function pbkdf2Init(sha256, B, B, arg3) {
  ...
  if (c < 1) {
    ...
    const toBytesResult = tmp(9852).toBytes(B);
    const uint8Array = new Uint8Array(dkLen);
    const hmac = tmp(9856).hmac;
    let obj = hmac.create(sha256, toBytesResult);
    const toBytesResult1 = tmp(9852).toBytes(B);
