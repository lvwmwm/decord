...
// Module 808 (getNotificationAttributes)
...
function getNotificationAttributes(arg0, requestId) {
  ...
  if ("notifications/cancelled" === arg0) {
    ...
    if (level) {
      ...
      obj[_mod805.MCP_LOGGING_LEVEL_ATTRIBUTE] = String(requestId.level);
    }
    let logger;
    if (requestId != null) {
    ...
    if (logger) {
      ...
      obj[_mod805.MCP_LOGGING_LOGGER_ATTRIBUTE] = String(requestId.logger);
    }
    let data;
    if (requestId != null) {
      data = requestId.data;
    ...
    if (undefined !== data) {
      obj[_mod805.MCP_LOGGING_DATA_TYPE_ATTRIBUTE] = typeof requestId.data;
      if (arg2) {
        data = requestId.data;
        let json = data;
    ...
    if (uri) {
      ...
      obj[_mod805.MCP_RESOURCE_URI_ATTRIBUTE] = String(requestId.uri);
...
arg5.buildTypeSpecificAttributes = function buildTypeSpecificAttributes(request, message, params, recordInputs) {
  ...
  if ("request" === request) {
    ...
    if (tmp5) {
      obj = {};
      const _String = String;
      obj[tmp2(805).MCP_REQUEST_ID_ATTRIBUTE] = String(message.id);
      tmp5 = obj;
    }
    obj1 = {};
