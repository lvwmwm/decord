...
// Dependencies: [32, 822, 818, 862, 834]
...
// Module 861 (forEachEnvelopeItem)
...
export const createAttachmentEnvelopeItem = function createAttachmentEnvelopeItem(data) {
  if (typeof data.data === "string") {
    ...
    const sentryCarrier = obj.getSentryCarrier(_mod818.GLOBAL_OBJ);
...
export const parseEnvelope = function parseEnvelope(arr) {
  if (typeof arr !== "string") {
    ...
    let sentryCarrier = _require(822).getSentryCarrier(_require(818).GLOBAL_OBJ);
    if (sentryCarrier.encodePolyfill) {
    ...
    if (typeof tmp8[1] !== "string") {
      ...
      if (!(tmp10 instanceof Uint8Array)) {
        try {
          ...
          json = _JSON3.stringify(_require(862).normalize(tmp3));
          const obj = _require(862);
        }
      }
    }
    append(tmp8[1]);
