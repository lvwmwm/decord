...
// Dependencies: [955, 975]

// Module 979 (extractToolCalls)
import _mod955 from "module_955" /* 955 */;
import asString from "asString" /* 975 */;
...
function extractModelMetadata(setAttribute, item10050) {
  if (item10050.response_metadata) {
    if (typeof item10050.response_metadata === "object") {
      const response_metadata = item10050.response_metadata;
      if (tmp) {
        const attr = setAttribute.setAttribute(_mod955.GEN_AI_RESPONSE_MODEL_ATTRIBUTE, response_metadata.model_name);
      }
      if (tmp6) {
        const items = [response_metadata.finish_reason];
        const attr1 = setAttribute.setAttribute(_mod955.GEN_AI_RESPONSE_FINISH_REASONS_ATTRIBUTE, items);
      }
  ...
  if (messages) {
    ...
    if (Array.isArray(messages)) {
      ...
      if (0 !== substr.length) {
        ...
        if (tmp34) {
          const _JSON = JSON;
          const attr = setAttribute.setAttribute(_mod955.GEN_AI_RESPONSE_TOOL_CALLS_ATTRIBUTE, JSON.stringify(tmp34));
        }
        const result = asString.normalizeLangChainMessages(substr);
        const _JSON2 = JSON;
        const attr1 = setAttribute.setAttribute(_mod955.GEN_AI_RESPONSE_TEXT_ATTRIBUTE, JSON.stringify(result));
        let num3 = 0;
        let num4 = 0;
        let num5 = 0;
        ...
        if (num3 > 0) {
          const attr2 = setAttribute.setAttribute(_mod955.GEN_AI_USAGE_INPUT_TOKENS_ATTRIBUTE, num3);
        }
        if (num4 > 0) {
          const attr3 = setAttribute.setAttribute(_mod955.GEN_AI_USAGE_OUTPUT_TOKENS_ATTRIBUTE, num4);
        }
        if (num5 > 0) {
          const attr4 = setAttribute.setAttribute(_mod955.GEN_AI_USAGE_TOTAL_TOKENS_ATTRIBUTE, num5);
        }
        const obj = asString;
      }
    }
