...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(690).getMainCarrier();
      let obj = callback(690);
      let sentryCarrier = callback(690).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(712);
    ...
    getCurrentScope() {
      const mainCarrier = callback(690).getMainCarrier();
      const obj = callback(690);
      const sentryCarrier = callback(690).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(712);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
