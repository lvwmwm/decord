...
// Dependencies: [937, 897, 939]

// Module 940 (getNotificationAttributes)
...
function getNotificationAttributes(arg0, requestId) {
  ...
  if ("notifications/cancelled" === arg0) {
    ...
    if (level) {
      ...
      obj[_mod937.MCP_LOGGING_LEVEL_ATTRIBUTE] = String(requestId.level);
    }
    let logger;
    if (requestId != null) {
    ...
    if (logger) {
      ...
      obj[_mod937.MCP_LOGGING_LOGGER_ATTRIBUTE] = String(requestId.logger);
    }
    let data;
    if (requestId != null) {
      data = requestId.data;
    ...
    if (undefined !== data) {
      obj[_mod937.MCP_LOGGING_DATA_TYPE_ATTRIBUTE] = typeof requestId.data;
      if (arg2) {
        data = requestId.data;
        let json = data;
        ...
        obj[tmp33(937).MCP_LOGGING_MESSAGE_ATTRIBUTE] = json;
      }
      tmp33 = require;
    }
    ...
    if (uri) {
      ...
      obj[_mod937.MCP_RESOURCE_URI_ATTRIBUTE] = String(requestId.uri);
      const _String2 = String;
      const result = getHttpSpanDetailsFromUrlObject.parseStringToURLObject(String(requestId.uri));
      let tmp7 = result;
      if (result) {
        tmp7 = !tmp3(897).isURLObjectRelative(result);
        const tmp3Result = tmp3(897);
      }
      if (tmp7) {
...
arg5.buildTypeSpecificAttributes = function buildTypeSpecificAttributes(request, message, params, recordInputs) {
  ...
  if ("request" === request) {
    ...
    if (tmp5) {
      obj = {};
      const _String = String;
      obj[tmp2(937).MCP_REQUEST_ID_ATTRIBUTE] = String(message.id);
      tmp5 = obj;
    }
    obj1 = {};
    ...
    if (recordInputs) {
      ...
      let requestArguments = tmp2(939).getRequestArguments(message.method, obj);
      const tmp2Result = tmp2(939);
    } else {
      requestArguments = {};
    }
