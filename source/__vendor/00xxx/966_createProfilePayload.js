...
// Module 966 (createProfilePayload)
...
function convertJSSelfProfileToSampledFormat(samples) {
  ...
  if (first) {
    ...
    let result = _require(682).browserPerformanceTimeOrigin();
...
export const createProfileChunkPayload = function createProfileChunkPayload(closure_0, _client, _profilerId) {
  if (null == closure_0) {
    ...
    if (stackParser) {
      let debugImagesForResources = tmp24(682).getDebugImagesForResources(stackParser, closure_0.resources);
