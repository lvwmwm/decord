...
let items = [
  obj,
  {
    ...
    value: function captureMessage(arg0, arg1, arg2, arg3) {
      ...
      const obj = { event_id: callback(695).uuid4() };
      const merged = Object.assign(arg2);
      const obj2 = callback(695);
      const tmp = callback;
  ...
  },
  {
    ...
    value: function captureSession(arg0) {
      ...
      callback(710).updateSession(arg0, { init: false });
    }
  },
  {
    key: "getDsn",
...
items[14] = {
  ...
  value: function addIntegration(arg0) {
    callback(752).setupIntegration(this, arg0, this._integrations);
    if (!this._integrations[arg0.name]) {
      const items = [arg0];
      const result = callback(752).afterSetupIntegrations(this, items);
      const tmpResult = callback(752);
    }
  }
};
items[15] = {
...
items[16] = {
  ...
  value: function sendSession(attrs) {
    ...
    if (undefined === environment) {
      environment = callback(723).DEFAULT_ENVIRONMENT;
    }
    if ("aggregates" in attrs) {
      const tmp7 = attrs.attrs || {};
      if (!tmp7.release) {
        if (!release) {
          if (callback(688).DEBUG_BUILD) {
    ...
    self.sendEnvelope(callback(728).createSessionEnvelope(attrs, self._dsn, self._options._metadata, self._options.tunnel));
  }
};
items[17] = {
...
items[21] = {
  ...
  value: function _setupIntegrations() {
    ...
    this._integrations = callback(752).setupIntegrations(this, integrations);
    const obj = callback(752);
    const result = callback(752).afterSetupIntegrations(this, integrations);
  }
};
items[22] = {
...
items[25] = {
  ...
  value: function _prepareEvent(type, integrations, arg2, setLastEventId) {
    ...
    let obj = callback(735);
    return callback(735).prepareEvent(options, type, integrations, arg2, self, setLastEventId).then((contexts) => {
      if (null === contexts) {
        return contexts;
      } else {
...
items[26] = {
  ...
  value: function _captureEvent(type) {
    ...
    if (arg2 === undefined) {
      currentScope = callback(713).getCurrentScope();
      const obj2 = callback(713);
    }
    let isolationScope = arg3;
    if (arg3 === undefined) {
      isolationScope = callback(713).getIsolationScope();
    ...
    tmp9 = callback(688).DEBUG_BUILD && undefined === type.type;
