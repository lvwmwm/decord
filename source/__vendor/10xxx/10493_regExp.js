  ...
  value: function extractTimeComponent(assign) {
    ...
    if (num < 60) {
      if (parsed <= 24) {
        let PM = null;
        if (parsed >= 12) {
          PM = DESpecificTimeExpressionParser(10451).Meridiem.PM;
        }
        let tmp5 = PM;
        let tmp6 = parsed;
        if (null != arg1[5]) {
          if (parsed > 12) {
            ...
            if (str8.match(/morgen|vormittag/)) {
              ...
              tmp9 = num2;
              PM = DESpecificTimeExpressionParser(10451).Meridiem.AM;
            }
            let tmp10 = tmp9;
        ...
        if (null !== tmp5) {
          ...
          assign.imply("meridiem", DESpecificTimeExpressionParser(10451).Meridiem.AM);
        } else {
          assign.imply("meridiem", DESpecificTimeExpressionParser(10451).Meridiem.PM);
        }
        if (null != arg1[4]) {
