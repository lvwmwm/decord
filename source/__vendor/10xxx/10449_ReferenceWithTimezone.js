...
// Dependencies: [377, 41, 42, 10450, 10448, 10452]

// Module 10449 (ReferenceWithTimezone)
  ...
  obj,
  {
    ...
    value: function fromInput(instant, timezones) {
      if (instant instanceof Date) {
        ...
        const toTimezoneOffsetResult = ParsingComponents(10450).toTimezoneOffset(timezone, instant, timezones);
        const obj = Object.create(ParsingComponents.prototype);
        callback(obj, ParsingComponents);
        if (null == instant) {
          const _Date2 = Date;
...
const items2 = [
  ...
  },
  {
    ...
    value: function addDurationAsImplied(EmptyDuration) {
      ...
      const addDurationResult = ParsingComponents(10448).addDuration(result, EmptyDuration);
...
const items3 = [
  {
    ...
    value: function createRelativeFromReference(reference, reverseDurationResult) {
      ...
      if (reverseDurationResult === undefined) {
        EmptyDuration = ParsingComponents(10448).EmptyDuration;
      }
      const addDurationResult = ParsingComponents(10448).addDuration(reference.getDateWithAdjustedTimezone(), EmptyDuration);
        ...
        if (!("minute" in EmptyDuration)) {
          if (!("second" in EmptyDuration)) {
            if (!("millisecond" in EmptyDuration)) {
              ParsingComponents(10452).implySimilarTime(obj, addDurationResult);
      ...
      ParsingComponents(10452).assignSimilarTime(obj, addDurationResult);
      ParsingComponents(10452).assignSimilarDate(obj, addDurationResult);
