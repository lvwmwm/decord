...
// Dependencies: [377, 41, 42, 10365, 10363, 10367]

// Module 10364 (ReferenceWithTimezone)
  ...
  obj,
  {
    ...
    value: function fromInput(instant, timezones) {
      if (instant instanceof Date) {
        ...
        const toTimezoneOffsetResult = ParsingComponents(10365).toTimezoneOffset(timezone, instant, timezones);
        const obj = Object.create(ParsingComponents.prototype);
        callback(obj, ParsingComponents);
        if (null == instant) {
          const _Date2 = Date;
...
const items2 = [
  ...
  },
  {
    ...
    value: function addDurationAsImplied(EmptyDuration) {
      ...
      const addDurationResult = ParsingComponents(10363).addDuration(result, EmptyDuration);
...
const items3 = [
  {
    ...
    value: function createRelativeFromReference(reference, reverseDurationResult) {
      ...
      if (reverseDurationResult === undefined) {
        EmptyDuration = ParsingComponents(10363).EmptyDuration;
      }
      const addDurationResult = ParsingComponents(10363).addDuration(reference.getDateWithAdjustedTimezone(), EmptyDuration);
        ...
        if (!("minute" in EmptyDuration)) {
          if (!("second" in EmptyDuration)) {
            if (!("millisecond" in EmptyDuration)) {
              ParsingComponents(10367).implySimilarTime(obj, addDurationResult);
      ...
      ParsingComponents(10367).assignSimilarTime(obj, addDurationResult);
      ParsingComponents(10367).assignSimilarDate(obj, addDurationResult);
