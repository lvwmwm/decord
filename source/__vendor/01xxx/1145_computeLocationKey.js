...
// Dependencies: [5, 32, 19, 1021, 1146, 1147, 814, 1139, 1137]
...
// Module 1145 (computeLocationKey)
import registerSpanErrorInstrumentation from "registerSpanErrorInstrumentation" /* 814 */;
import __SENTRY_DEBUG__ from "__SENTRY_DEBUG__" /* 1139 */;
import pickSplat from "pickSplat" /* 1146 */;
...
function processResolvedRoutes(arr, children, arg2, activeRootSpan) {
  ...
  if (activeRootSpan) {
    ...
    if (spanToJSONResult.timestamp) {
      if (tmp12(1139).DEBUG_BUILD) {
        const debug = tmp12(814).debug;
      ...
      if (!tmp) {
        ...
        if (!activeRootSpan) {
          tmp15 = tmp;
          if (undefined !== tmp12(1021).WINDOW) {
            const _location = tmp12(1021).WINDOW.location;
            let pathname;
            if (_location != null) {
...
function updateNavigationSpan(activeRootSpan, _location, arg2, arg3, closure_8) {
  ...
  if (!spanToJSONResult.timestamp) {
    ...
    if (data != null) {
      const tmp19 = data[tmp(undefined, 814).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
    }
    let tmp20 = tmp17;
    if (tmp17) {
    ...
    if (tmp20) {
      ...
      const attr = activeRootSpan.setAttribute(tmp(814).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, tmp18);
      let result1 = tmp(1146).transactionNameHasWildcard(tmp17);
      if (!result1) {
        result1 = "route" !== tmp18;
      }
      if (!result1) {
        const result2 = tmp(814).addNonEnumerableProperty(activeRootSpan, "__sentry_navigation_name_set__", true);
        const tmpResult2 = tmp(814);
      }
      const tmpResult1 = tmp(1146);
    }
...
function setupRouterSubscription(subscribe) {
  ...
  let tmp = arg4;
  if (tmp) {
    tmp = "pageload" === _require(814).spanToJSON(arg4).op;
  ...
  const subscription = subscribe.subscribe((historyAction) => {
    ...
    if ("PUSH" === historyAction.historyAction) {
      ...
      if ("idle" !== historyAction.navigation.state) {
        ...
        const WINDOW5 = callback(1021).WINDOW;
        let prop;
        if (WINDOW5 != null) {
        ...
        if (prop) {
          const WINDOW6 = callback(1021).WINDOW;
                      ...
                      try {
                        ...
                        if (0 === c4) {
                          if (arg0 === 1) {
                            ...
                            let activeRootSpan2;
                            c3 = undefined;
                            navigationContext = callback(1146).setNavigationContext(closure_1_1, closure_1_2);
                            c3 = 1;
                            c4 = 2;
                            c5 = 1;
                          ...
                          c3 = 0;
                          const result = callback(1146).clearNavigationContext(dependencyMap);
                          ...
                          callback = arg1;
                          c3 = 0;
                          const result2 = callback(1146).clearNavigationContext(navigationContext);
                          const obj11 = callback(1146);
                          activeRootSpan2 = callback(1146).getActiveRootSpan();
                          if (activeRootSpan2) {
                            obj = callback(814);
                            if ("navigation" === obj.spanToJSON(activeRootSpan2).op) {
                              if (closure_1_0) {
                                ...
                                tmp14 = tmp73;
                                if (!tmp73) {
                                  const _location = callback(1021).WINDOW.location;
                                  pathname = undefined;
                                  if (_location != null) {
                                    pathname = _location.pathname;
                                  }
    ...
    if (weakSet.has(client)) {
      ...
      if ("PUSH" === navigationType) {
        if (tmp4Result) {
          ...
          if (!isPlaceholder) {
            timestamp = tmp6(814).spanToJSON(value.span).timestamp;
            const tmp6Result2 = tmp6(814);
          }
          ...
          if (tmp26Result.skip) {
            if (tmp26Result.shouldUpdate) {
              if (value) {
                ...
                if (value.isPlaceholder) {
                  ...
                  if (tmp6(1139).DEBUG_BUILD) {
                    const debug3 = tmp6(814).debug;
                  ...
                  const attr = span2.setAttribute(tmp6(814).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, tmp22);
                  const result = tmp6(814).addNonEnumerableProperty(value.span, "__sentry_navigation_name_set__", true);
                  value.routeName = tmp21;
                  if (tmp6(1139).DEBUG_BUILD) {
                    const debug2 = tmp6(814).debug;
                    const _HermesInternal3 = HermesInternal;
                    debug2.log("[Tracing] Updated navigation span name from \"" + routeName + "\" to \"" + tmp21 + "\"");
                  }
                  const tmp6Result3 = tmp6(814);
                }
              }
            }
            if (tmp6(1139).DEBUG_BUILD) {
              const debug = tmp6(814).debug;
            ...
            try {
              ...
              const obj2 = {};
              obj2[tmp6(814).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] = tmp22;
              obj2[tmp6(814).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "navigation";
              const _HermesInternal = HermesInternal;
              obj2[tmp6(814).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = "auto.navigation.react.reactrouter_v" + tmp2;
              obj1[1] = obj2;
              const result2 = tmp6(1021).startBrowserTracingNavigationSpan(client, obj1);
              if (result2) {
                ...
                if (value.size > 0) {
                  if (description) {
                    ...
                    if (0 === closure_1_11) {
                      ...
                      racePromise.then(() => {
                        const spanToJSONResult = callback(814).spanToJSON(first);
                        ...
                        const client = callback(814).getClient();
                        if (client) {
                          if ("navigation" === tmp5) {
      ...
      let tmp46 = (function shouldUpdateWildcardSpanName(description, arg1, first, arg3, arg4) {
        ...
        if (tmp) {
          ...
          if (tmp3) {
            let result = !description;
            if (description) {
              result = !callback(1146).transactionNameHasWildcard(description);
              const obj = callback(1146);
            }
            if (!result) {
              result = "route" !== arg3;
            }
            if (!result) {
              result = callback(1146).transactionNameHasWildcard(first);
              const obj2 = callback(1146);
            }
            tmp5 = !result;
          }
          if (!tmp5) {
...
function patchSpanEnd(result2, _location, routes, basename, allRoutes, navigation) {
  ...
  if (!tmp2) {
    if (result2.end) {
      ...
      const result = _require(814).addNonEnumerableProperty(result2, combined, true);
      let obj = _require(814);
    }
  }
}
...
export const createReactRouterV6CompatibleTracingIntegration = function createReactRouterV6CompatibleTracingIntegration(instrumentPageLoad, _6) {
  ...
  obj.setup = function setup(arg0) {
    ...
    if (tmp4 === Infinity) {
      if (instrumentPageLoad(1139).DEBUG_BUILD) {
        const debug3 = instrumentPageLoad(814).debug;
    ...
    const result1 = instrumentPageLoad(1146).initializeRouterUtils(closure_7, flag);
  };
  obj.afterAllSetup = function afterAllSetup(f107606) {
    closure_2.afterAllSetup(f107606);
    const _location = instrumentPageLoad(1021).WINDOW.location;
    if (_location != null) {
    ...
    if (tmp4) {
      ...
      obj[0] = pathname;
      obj = {};
      obj[tmp2(814).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] = "url";
      obj[tmp2(814).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "pageload";
      const _HermesInternal = HermesInternal;
      obj[tmp2(814).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = "auto.pageload.react.reactrouter_v" + dependencyMap;
      obj[1] = obj;
      result = tmp2(1021).startBrowserTracingPageLoadSpan(f107606, obj);
      const tmp2Result = tmp2(1021);
    }
    if (num) {
      lazyRouteTimeout.add(f107606);
    }
  };
  return obj;
};
      ...
      if (closure_6) {
        if (closure_7) {
          if (closure_8) {
            ...
            _require(1137).hoistNonReactStatics(SentryRoutes, arg0);
            return SentryRoutes;
          }
        }
      }
      ...
      if (closure_6) {
        if (closure_8) {
          let fn = (arg0, basename) => {
            ...
            const activeRootSpan = callback(1146).getActiveRootSpan();
            if (tmp14) {
