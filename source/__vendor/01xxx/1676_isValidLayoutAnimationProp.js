...
// Module 1676 (isValidLayoutAnimationProp)
import runWorkletOnJS from "runWorkletOnJS" /* 1643 */;
import t from "t" /* 1647 */;
import isWorkletFunction from "isWorkletFunction" /* 1661 */;
import isReducedMotionEnabledInSystem from "isReducedMotionEnabledInSystem" /* 1677 */;
import isAffineMatrixFlat from "isAffineMatrixFlat" /* 1678 */;
  ...
  if (isHigherOrder.isHigherOrder) {
    ...
    function transformationMatrixOnFrame(arg0, arg1) {
      ...
      const item = items1.forEach((arg0, arg1) => {
        ...
        const obj2 = lib(1678);
        return items.push(obj.addMatrices(lib.startMatrices[arg0], obj2.scaleMatrix(lib(1678).subtractMatrices(lib.stopMatrices[arg0], lib.startMatrices[arg0]), tmp2)));
      });
      const items2 = [];
      const items3 = ["x", "y", "z"];
      ...
      const item1 = items3.forEach((arg0, arg1) => {
        ...
        items2.push(lib(1678).getRotationMatrix(sum, arg0));
      });
      [tmp7, tmp8, tmp9] = items2;
      const onFrameResult = first.onFrame(arg0[0], arg1);
      let obj2 = callback(1678);
      const obj3 = callback(1678);
      const multiplyMatricesResult = obj2.multiplyMatrices(tmp7, callback(1678).multiplyMatrices(tmp8, tmp9));
      const obj4 = callback(1678);
      const obj5 = callback(1678);
      const obj6 = callback(1678);
      arg0.current = obj4.flatten(obj5.multiplyMatrices(obj6.multiplyMatrices(tmp4, callback(1678).multiplyMatrices(tmp5, multiplyMatricesResult)), tmp3));
