...
// Dependencies: [1711, 1704, 1656, 1693, 1677, 1708, 1722, 1738, 1662, 1739, 1740, 1745]

// Module 1721 (setElementAnimation)
...
import BaseAnimationBuilder from "BaseAnimationBuilder" /* 1722 */;
import weakMap from "weakMap" /* 1738 */;
...
function setElementAnimation(dummy, dummyAnimationConfig, arg2) {
  ...
  if (dummyAnimationConfig.animationType === _require(1677).LayoutAnimationType.ENTERING) {
  ...
  if (!(animationName in _require(1708).Animations)) {
    let result = tmp2(1739).scheduleAnimationCleanup(animationName, duration + delay, () => {
      if (flag) {
        const snapshots = dummy(dummyAnimationConfig[7]).snapshots;
...
arg5.getReducedMotionFromConfig = function getReducedMotionFromConfig(entering) {
  if (entering.reduceMotionV) {
    ...
    if (isWorkletFunction.ReduceMotion.Never === reduceMotionV) {
      return false;
    } else if (tmp3(1677).ReduceMotion.Always === reduceMotionV) {
      return true;
    } else {
      return tmp3(1693).ReducedMotionManager.jsValue;
    }
  } else {
...
arg5.getProcessedConfig = function getProcessedConfig(animationWithInitialValues, animationType, entering) {
  ...
  if (animationWithInitialValues in TransitionType.Animations) {
    num = tmp(1708).Animations[animationWithInitialValues].duration;
  }
  ...
  if (!entering.delayV) {
    obj[3] = num3;
    if (entering.easingV) {
      const tmp8 = entering.easingV[tmp(undefined, 1704).EasingNameSymbol];
      let tmpResult = tmp(1711);
      if (tmp9) {
      ...
      tmp9 = tmp8 in tmp(1711).WebEasings;
    } else {
      easingByName = tmp(1711).getEasingByName("linear");
      const tmpResult1 = tmp(1711);
    }
    obj[4] = easingByName;
    let callbackV = null;
...
arg5.handleLayoutTransition = function handleLayoutTransition(_componentDOMRef, processedConfig, easingY) {
  ...
  if (ENTRY_EXIT === tmp4(1708).TransitionType.CURVED) {
    tmp4Result = tmp4(1745);
...
arg5.handleExitingAnimation = function handleExitingAnimation(offsetParent, processedConfig) {
  ...
  const snapshots2 = tmp6(1738).snapshots;
  const result1 = snapshots2.set(cloneNodeResult, rect);
  _require(1738).setElementPosition(cloneNodeResult, rect);
