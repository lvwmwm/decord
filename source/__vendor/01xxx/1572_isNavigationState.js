...
// Dependencies: [32, 109, 19, 21, 1502, 1545, 1546, 1573, 1539, 1530, 1521, 1515, 1528, 1520, 1562, 1503, 1574, 1575, 1519, 1526, 1577, 1522, 1523, 1578, 1580, 1531, 1581, 1582, 1583, 1584, 1588, 1589, 1567, 1564, 1590, 1540, 1569]
// Exports: useNavigationBuilder

// Module 1572 (isNavigationState)
import useEffectDefault from "useEffect" /* 1520 */;
import closure_4 from "_slicedToArray" /* 32 */;
import closure_5 from "_objectWithoutProperties" /* 109 */;
import closure_6 from "noop" /* 19 */;
import { jsx } from "jsxProd" /* 21 */;
import { PrivateValueStore } from "PrivateValueStore" /* 1502 */;
...
export const useNavigationBuilder = function useNavigationBuilder(StackRouter, UNSTABLE_routeNamesChangeBehavior) {
  ...
  let obj = _require(1573);
  dependencyMap = obj.useRegisterNavigator();
  const context = React.useContext(_require(1539).NavigationRouteContext);
  const context1 = React.useContext(_require(1530).ConsumedParamsContext);
  let params;
  if (context != null) {
  ...
  const lazyValue = _require(1521).useLazyValue(() => {
  ...
  if (mapped.length) {
    ...
    const context2 = React.useContext(_require(1515).NavigationStateContext);
    ...
    const context3 = React.useContext(_require(1528).NavigationBuilderContext);
    ...
    if (null != tmp43) {
          ...
          if (isNavigationState(context.params.state)) {
            if (!tmp5) {
              ...
              if (null != tmp61) {
                ...
                const CommonActions = tmp30(1503).CommonActions;
          ...
          if (typeof context.params.screen === "string") {
            ...
            if ("lastUnhandled" === UNSTABLE_routeNamesChangeBehavior.UNSTABLE_routeNamesChangeBehavior) {
              if (!mapped.includes(context.params.screen)) {
                ...
                tmp67 = null == tmp66 || tmp34(1574)(tmp66, tmp43);
              }
            }
      ...
      const tmp75 = tmp34(1520)(() => {
        let tmp = callback2();
        if (!callback1(tmp)) {
          tmp = closure_30;
        }
        return StackRouter(1519).deepFreeze(tmp);
      });
      tmp30Result = tmp30(1526);
      ...
      obj8[0] = tmp58;
      obj8[1] = eventEmitter;
      const focusEvents = tmp30(1577).useFocusEvents(obj8);
      const items5 = [eventEmitter, tmp58];
      const effect3 = obj9.useEffect(() => {
        eventEmitter.emit({ type: "state", data: { state: rehydratedState } });
      }, items5);
      const tmp30Result1 = tmp30(1577);
      const childListeners = tmp30(1522).useChildListeners();
      ({ listeners, addListener } = childListeners);
      const tmp30Result2 = tmp30(1522);
      const keyedChildListeners = tmp30(1523).useKeyedChildListeners();
      ({ keyedListeners, addKeyedListener } = keyedChildListeners);
      const tmp30Result3 = tmp30(1523);
      ...
      obj9[6] = obj10;
      obj9[7] = eventEmitter;
      const onAction = tmp30(1578).useOnAction(obj9);
      const tmp30Result4 = tmp30(1578);
      ...
      obj11[2] = tmp75;
      obj11[3] = tmp35;
      const onRouteFocus = tmp30(1580).useOnRouteFocus(obj11);
      closure_41 = obj9.useContext(tmp30(1531).UnhandledActionContext);
      const tmp30Result5 = tmp30(1580);
      const tmp85 = tmp34(1520)((type) => {
        if ("lastUnhandled" === UNSTABLE_routeNamesChangeBehavior.UNSTABLE_routeNamesChangeBehavior) {
      ...
      obj12[6] = lazyValue;
      const navigationHelpers = tmp30(1581).useNavigationHelpers(obj12);
      const tmp30Result6 = tmp30(1581);
      const obj13 = { navigation: null, focusedListeners: null };
      obj13[0] = navigationHelpers;
      obj13[1] = listeners.focus;
      const focusedListenersChildrenAdapter = tmp30(1582).useFocusedListenersChildrenAdapter(obj13);
      const tmp30Result7 = tmp30(1582);
      const obj14 = { getState: null, getStateListeners: null };
      obj14[0] = tmp75;
      obj14[1] = keyedListeners.getState;
      const onGetState = tmp30(1583).useOnGetState(obj14);
      const tmp30Result8 = tmp30(1583);
      ...
      let descriptors = tmp30(1584).useDescriptors(obj15);
      descriptors = descriptors.descriptors;
      const tmp30Result9 = tmp30(1584);
      const obj16 = { state: null, navigation: null, descriptors: null };
      obj16[0] = tmp58;
      obj16[1] = navigationHelpers;
      obj16[2] = descriptors;
      const currentRender = tmp30(1588).useCurrentRender(obj16);
      const tmp30Result10 = tmp30(1588);
      ...
      obj17[3] = descriptors;
      obj17[4] = tmp30(1589).useComponent((arg0) => {
        let tmpResult = arg0;
        if (null != closure_6) {
        ...
        obj1 = { state: rehydratedState, getState: navigationHelpers.getState, children: closure_1_7(StackRouter(1540).FocusedRouteKeyContext.Provider, { value: rehydratedState.routes[rehydratedState.index].key, children: closure_1_7(StackRouter(1569).PreventRemoveProvider, { children: tmpResult }) }) };
        obj[1] = closure_1_7(StackRouter(1590).NavigationStateListenerProvider, obj1);
        obj[1] = closure_1_7(StackRouter(1564).NavigationHelpersContext.Provider, obj);
        return closure_1_7(StackRouter(1567).NavigationMetaContext.Provider, obj);
      });
      return obj17;
    }
    let everyResult;
    ...
    const tmp30Result12 = _require(1562);
    rehydratedState = tmp39;
    if (!tmp51) {
    ...
    tmp51 = _require(1562).isArrayEqual(tmp39.routeNames, mapped) && 0 === found.length;
  } else {
    let _Error = Error;
