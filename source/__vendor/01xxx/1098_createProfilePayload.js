...
// Module 1098 (createProfilePayload)
...
function convertJSSelfProfileToSampledFormat(samples) {
  ...
  if (first) {
    ...
    let result = _require(814).browserPerformanceTimeOrigin();
...
export const createProfileChunkPayload = function createProfileChunkPayload(closure_0, _client, _profilerId) {
  if (null == closure_0) {
    ...
    if (stackParser) {
      let debugImagesForResources = tmp24(814).getDebugImagesForResources(stackParser, closure_0.resources);
      const tmp24Result1 = tmp24(814);
    } else {
      debugImagesForResources = [];
    }
...
export const findProfiledTransactionsFromEnvelope = function findProfiledTransactionsFromEnvelope(arg0) {
  const items = [];
  items(814).forEachEnvelopeItem(arg0, (arg0, arg1) => {
    if ("transaction" === arg1) {
...
export const shouldProfileSpanLegacy = function shouldProfileSpanLegacy(rootSpan) {
  if (c14) {
    ...
    if (rootSpan.isRecording()) {
      const client = log(814).getClient();
      let options;
      if (client != null) {
        options = client.getOptions();
      }
      if (options) {
        ...
        if (isValidSampleRate(concat)) {
          if (concat) {
            ...
            if (!flag6) {
              flag6 = false;
              if (log(1069).DEBUG_BUILD) {
                const debug5 = log(814).debug;
                log = debug5.log;
        ...
        if (log(1069).DEBUG_BUILD) {
          const debug2 = log(814).debug;
          debug2.log("[Profiling] Profiling disabled, no options found.");
        }
        return false;
      }
      const logResult1 = log(814);
    } else {
      if (log(1069).DEBUG_BUILD) {
        const debug = log(814).debug;
...
export const startJSSelfProfile = function startJSSelfProfile() {
  ...
  } else if (tmp4(1069).DEBUG_BUILD) {
    const debug = tmp4(814).debug;
