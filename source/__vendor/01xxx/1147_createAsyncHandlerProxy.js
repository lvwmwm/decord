...
// Dependencies: [1146, 1021, 1139, 814]

// Module 1147 (createAsyncHandlerProxy)
const require = arg1;
...
function createAsyncHandlerProxy(arg0, item10034, item10008, closure_1_18) {
    ...
    apply(apply) {
      let tmp = (function captureCurrentLocation() {
        ...
        if (targetPath) {
          ...
          if (undefined !== tmp4(1021).WINDOW) {
            try {
              const _location = tmp4(1021).WINDOW.location;
              if (_location) {
  ...
  const result = _require(814).addNonEnumerableProperty(proxy, "__sentry_proxied__", true);
  return proxy;
}
