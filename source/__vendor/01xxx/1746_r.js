...
function processColor(arr, target) {
  ...
  if (tmp5) {
    ...
    let tmp28 = target !== tmp(1748).ValueProcessorTarget.CSS;
    if (!tmp28) {
    ...
    if (tmp7) {
      ...
      if (isDynamicColorObjectIOS(arr)) {
        if (tmp(1747).IS_IOS) {
          ...
          const reanimatedError = new tmp(1647).ReanimatedError(obj.dynamicNotAvailableOnPlatform());
          throw reanimatedError;
        }
      }
      if (null === tmp12) {
        const reanimatedError1 = new tmp(1647).ReanimatedError(obj.invalidColor(arr));
...
processColor.__closure = { processColorNumber, ValueProcessorTarget: require("module_1748").ValueProcessorTarget, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: require("JEST_WORKER_ID").IS_IOS, ERROR_MESSAGES: obj, processDynamicColorObjectIOS };
...
const obj1 = { processColorNumber, ValueProcessorTarget: require("module_1748").ValueProcessorTarget, isPlatformColorObject, isDynamicColorObjectIOS, IS_IOS: require("JEST_WORKER_ID").IS_IOS, ERROR_MESSAGES: obj, processDynamicColorObjectIOS };
