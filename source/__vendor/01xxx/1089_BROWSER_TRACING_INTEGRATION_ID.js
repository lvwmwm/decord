...
export const browserTracingIntegration = () => {
  ...
  closure_3 = { name: "left", source: "call" };
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f107925, result) {
  f107925.emit("startPageLoadSpan", result, arg2);
  const currentScope = registerSpanErrorInstrumentation.getCurrentScope();
  currentScope.setTransactionName(result.name);
  if (f107925[_sentry_idleSpan]) {
    f107925.emit("afterStartPageLoadSpan", tmp3);
  }
  return f107925[_sentry_idleSpan];
};
