...
import shouldAttachHeaders from "shouldAttachHeaders" /* 1087 */;
...
export const browserTracingIntegration = () => {
  ...
  function _createRouteSpan(emit, op) {
    ...
    if (flag) {
      ...
      obj[4] = function beforeSpanEnd(setAttribute) {
        ...
        const result1 = emit(814).addNonEnumerableProperty(closure_0, document, undefined);
        const obj3 = emit(814);
        const currentScope = emit(814).getCurrentScope();
        obj = {};
        const merged = Object.assign(currentScope.getPropagationContext());
        obj.traceId = _undefined.spanContext().traceId;
        const obj4 = emit(814);
        obj.sampled = emit(814).spanIsSampled(_undefined);
        const obj7 = emit(814);
        obj.dsc = emit(814).getDynamicSamplingContextFromSpan(setAttribute);
  ...
  closure_3 = { name: "disabled", source: "Date" };
  document = _require(1025).WINDOW.document;
  obj = {};
      ...
      if (_undefined(_undefined2[2]).WINDOW.location) {
        ...
        if (c25) {
          let result2 = tmp(tmp2[3]).addHistoryInstrumentationHandler((to) => {
            ...
            if (!pathname) {
              pathname = tmp3(1025).WINDOW.location.pathname;
            }
            obj = { name: pathname, attributes: { [tmp3(814).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "url", [tmp3(814).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN]: "auto.navigation.browser" } };
            ...
            if (tmp17) {
              obj = { normalizedRequest: null };
              obj1 = {};
              const merged = Object.assign(tmp3(1025).getHttpRequestData());
              obj1.url = url;
              obj[0] = obj1;
              const result2 = currentScope.setSDKProcessingMetadata(obj);
              const tmp3Result2 = tmp3(1025);
            }
          });
      ...
      if (c9) {
        ...
        if (tmp(tmp2[2]).WINDOW.document) {
          const listener = globalThis.addEventListener("click", function registerInteractionTransaction() {
            ...
            obj1 = closure_5;
            if (closure_5) {
              const attr = obj1.setAttribute(emit(814).SEMANTIC_ATTRIBUTE_SENTRY_IDLE_SPAN_FINISH_REASON, "interactionInterrupted");
              closure_5.end();
              closure_5 = undefined;
            }
            if (name.name) {
              ...
              obj = {};
              obj[tmp10(814).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] = str5;
              obj[2] = obj;
              obj1 = { idleTimeout: null, finalTimeout: null, childSpanTimeout: null };
              obj1[0] = dependencyMap;
              obj1[1] = closure_2;
              obj1[2] = closure_3;
              closure_5 = tmp10(814).startIdleSpan(obj, obj1);
              const tmp10Result = tmp10(814);
            } else if (tmp10(1069).DEBUG_BUILD) {
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f107389, result) {
  f107389.emit("startPageLoadSpan", result, arg2);
  const currentScope = registerSpanErrorInstrumentation.getCurrentScope();
  currentScope.setTransactionName(result.name);
  if (f107389[_sentry_idleSpan]) {
    f107389.emit("afterStartPageLoadSpan", tmp3);
  }
  return f107389[_sentry_idleSpan];
};
