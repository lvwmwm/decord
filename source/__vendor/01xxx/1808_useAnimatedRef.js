...
export const useAnimatedRef = isJest ? (function useAnimatedRefWeb() {
  const f72400 = (getScrollableNode) => {
  ...
  if (!tmp2.current) {
    fun = function fun(map) {
      ...
      if (map) {
        ...
        fun.getTag = () => f72403(current[3]).findNodeHandle(closure_0);
        fun.current = map;
        if (tag.size) {
          tag = undefined;
          if (tmp3 != null) {
  ...
  const first = callback(callback2(() => {
    ...
    if (!obj.isFabric()) {
      mutable = null;
      if (closure_5) {
        mutable = f72403(current[4]).makeMutable(null);
        const tmpResult = f72403(current[4]);
      }
    }
    return mutable;
  }), 1)[0];
  let f72403 = first;
  const first1 = callback(callback2(() => f72403(current[4]).makeMutable(null)), 1)[0];
  let current = first1;
  f72403 = (viewConfig) => {
    if (obj.isFabric()) {
      let fn = f72403(current[5]).getShadowNodeWrapperFromRef;
    } else {
  ...
  const shareableMappingCache = f72403(current[6]).shareableMappingCache;
