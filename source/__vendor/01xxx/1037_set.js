...
// Function ID: 1038
// Name: set
// Dependencies: [1036, 1038, 1039]

// Module 1037 (set)
import addPageListener from "addPageListener" /* 1038 */;
...
arg5.getVisibilityWatcher = () => {
  if (_require(1036).WINDOW.document) {
    if (num2 < 0) {
      let tmpResult = tmp(1039);
      _require = tmpResult.getActivationStart();
      let tmp8;
      if (!tmp(1036).WINDOW.document.prerendering) {
      ...
      tmp(1038).addPageListener("pagehide", onVisibilityUpdate, true);
      const tmpResult1 = tmp(1038);
      tmp(1038).addPageListener("prerenderingchange", onVisibilityUpdate, true);
      const tmpResult2 = tmp(1038);
    }
  }
  const obj = {};
