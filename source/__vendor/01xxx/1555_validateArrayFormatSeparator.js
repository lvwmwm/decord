...
// Dependencies: [32, 1556, 1557, 1558, 1559]
// Exports: exclude, extract, parseUrl, pick, stringify, stringifyUrl

// Module 1555 (validateArrayFormatSeparator)
import _mod1556 from "module_1556" /* 1556 */;
import decodeComponents from "decodeComponents" /* 1557 */;
    ...
    const mapped = keys1.map((arg0) => {
      if (undefined === dependencyMap[arg0]) {
        ...
        if (!merged.encode) {
          ...
          let encodeURIComponentResult = dependencyMap(1556)(arg0);
        } else {
          const _encodeURIComponent3 = encodeURIComponent;
        ...
        if (Array.isArray(arr)) {
          ...
          if (!tmp13.encode) {
            ...
            let encodeURIComponentResult1 = dependencyMap(1556)(arg0);
          } else {
          ...
          if (!tmp.encode) {
            ...
            let encodeURIComponentResult2 = dependencyMap(1556)(arr);
          } else {
            const _encodeURIComponent2 = encodeURIComponent;
            encodeURIComponentResult2 = encodeURIComponent(arr);
          }
        } else if (tmp.strict) {
          let encodeURIComponentResult3 = dependencyMap(1556)(arg0);
        } else {
          const _encodeURIComponent = encodeURIComponent;
...
export const parseUrl = (arr) => {
  ...
  [str, tmp5] = callback(_mod1558(arr, "#"), 2);
  const tmp2 = require;
  const tmp4 = callback(_mod1558(arr, "#"), 2);
  ...
  if (!url.fragmentIdentifier) {
    ...
    if (merged[tmp]) {
      if (!merged.encode) {
        ...
        let encodeURIComponentResult = _mod1556(fragmentIdentifier);
      } else {
        const _encodeURIComponent = encodeURIComponent;
...
export const pick = (arg0, arg1, arg2) => {
  ...
  obj = { url: parseUrlResult.url, query: _mod1559(query, arg1), fragmentIdentifier };
