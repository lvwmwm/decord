...
// Module 1557 (isNavigationState)
import useEffectDefault from "useEffect" /* 1505 */;
import closure_4 from "_slicedToArray" /* 32 */;
import closure_5 from "_objectWithoutProperties" /* 109 */;
import closure_6 from "noop" /* 19 */;
import { jsx } from "jsxProd" /* 21 */;
import { PrivateValueStore } from "PrivateValueStore" /* 1487 */;
...
export const useNavigationBuilder = function useNavigationBuilder(StackRouter, UNSTABLE_routeNamesChangeBehavior) {
  ...
  let obj = _require(1558);
  dependencyMap = obj.useRegisterNavigator();
  const context = React.useContext(_require(1524).NavigationRouteContext);
  const context1 = React.useContext(_require(1515).ConsumedParamsContext);
  let params;
  if (context != null) {
  ...
  const lazyValue = _require(1506).useLazyValue(() => {
  ...
  if (mapped.length) {
    ...
    const context2 = React.useContext(_require(1500).NavigationStateContext);
    ...
    const context3 = React.useContext(_require(1513).NavigationBuilderContext);
    ...
    if (null != tmp43) {
      ...
      const tmp75 = tmp34(1505)(() => {
        ...
        return StackRouter(1504).deepFreeze(tmp);
      });
      ...
      obj8[0] = tmp58;
      obj8[1] = eventEmitter;
      const focusEvents = tmp30(1562).useFocusEvents(obj8);
      ...
      obj9[6] = obj10;
      obj9[7] = eventEmitter;
      const onAction = tmp30(1563).useOnAction(obj9);
      ...
      obj11[2] = tmp75;
      obj11[3] = tmp35;
      const onRouteFocus = tmp30(1565).useOnRouteFocus(obj11);
      ...
      obj12[6] = lazyValue;
      const navigationHelpers = tmp30(1566).useNavigationHelpers(obj12);
      ...
      const focusedListenersChildrenAdapter = tmp30(1567).useFocusedListenersChildrenAdapter(obj13);
      const tmp30Result7 = tmp30(1567);
      const obj14 = { getState: null, getStateListeners: null };
      obj14[0] = tmp75;
      obj14[1] = keyedListeners.getState;
      const onGetState = tmp30(1568).useOnGetState(obj14);
      ...
      let descriptors = tmp30(1569).useDescriptors(obj15);
      ...
      obj16[2] = descriptors;
      const currentRender = tmp30(1573).useCurrentRender(obj16);
      ...
      obj17[3] = descriptors;
      obj17[4] = tmp30(1574).useComponent((arg0) => {
        let tmpResult = arg0;
        if (null != closure_6) {
        ...
        obj1 = { state: rehydratedState, getState: navigationHelpers.getState, children: closure_1_7(StackRouter(1525).FocusedRouteKeyContext.Provider, { value: rehydratedState.routes[rehydratedState.index].key, children: closure_1_7(StackRouter(1554).PreventRemoveProvider, { children: tmpResult }) }) };
        obj[1] = closure_1_7(StackRouter(1575).NavigationStateListenerProvider, obj1);
        obj[1] = closure_1_7(StackRouter(1549).NavigationHelpersContext.Provider, obj);
        return closure_1_7(StackRouter(1552).NavigationMetaContext.Provider, obj);
      });
      return obj17;
    }
    let everyResult;
    ...
    const tmp30Result12 = _require(1547);
    rehydratedState = tmp39;
    if (!tmp51) {
    ...
    tmp51 = _require(1547).isArrayEqual(tmp39.routeNames, mapped) && 0 === found.length;
  } else {
    let _Error = Error;
