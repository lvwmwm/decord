...
// Function ID: 1799
// Name: isAnimated
// Dependencies: [1677, 1663]

// Module 1798 (isAnimated)
let isAnimated = arg1;
...
function validateAnimatedStyles(obj) {
  if (typeof obj !== "object") {
    ...
    const reanimatedError = new isAnimated(1663).ReanimatedError("`useAnimatedStyle` has to return an object, found " + typeof obj + " instead.");
    throw reanimatedError;
  } else {
    const _Array = Array;
    if (Array.isArray(obj)) {
      const reanimatedError1 = new isAnimated(1663).ReanimatedError("`useAnimatedStyle` has to return an object and cannot return static styles combined with dynamic ones. Please do merging where a component receives props.");
        ...
        if (0 < savedDependencies.length) {
          while (true) {
            ...
            if (is(tmp2, tmp3)) {
              ...
              let obj = isAnimated(1677);
              flag = false;
              if (!obj.isWorkletFunction(tmp2)) {
                break;
              } else {
                let tmp5Result = tmp5(1677);
                flag = false;
