...
// Dependencies: [16780, 16781]

// Module 16779 (CanonicalizeLocaleList)
import RegExpResult1 from "RegExpResult1" /* 16780 */;
import createDateTimeFormat from "createDateTimeFormat" /* 16781 */;
          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "string") {
                ...
                if (isMatch) {
                  let expVariantDupes = tmp5(16780).expVariantDupes;
                  isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(16780).expSingletonDupes;
                ...
                if (isMatch) {
                  ...
                  if (!tmp20) {
                    ...
                    let RegExpResult = RegExp(`(?:${tmp17(16780).expExtSequences.source})+`, "i");
...
class List {
  constructor() {
    tmp = f65298(this, "length", { writable: true, value: 0 });
    if (!arguments.length) {
      return;
    } else {
      tmp3 = slice;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    if (isMatch) {
      const expVariantDupes = tmp(16780).expVariantDupes;
      isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(16780).expSingletonDupes;
