...
// Dependencies: [16762, 16763]

// Module 16761 (CanonicalizeLocaleList)
import RegExpResult1 from "RegExpResult1" /* 16762 */;
import createDateTimeFormat from "createDateTimeFormat" /* 16763 */;
          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "string") {
                ...
                if (isMatch) {
                  let expVariantDupes = tmp5(16762).expVariantDupes;
                  isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(16762).expSingletonDupes;
                ...
                if (isMatch) {
                  ...
                  if (!tmp20) {
                    ...
                    let RegExpResult = RegExp(`(?:${tmp17(16762).expExtSequences.source})+`, "i");
...
class List {
  constructor() {
    tmp = f65244(this, "length", { writable: true, value: 0 });
    if (!arguments.length) {
      return;
    } else {
      tmp3 = slice;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    if (isMatch) {
      const expVariantDupes = tmp(16762).expVariantDupes;
      isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(16762).expSingletonDupes;
