...
// Function ID: 5546
// Name: hash
// Dependencies: [32, 5532, 5531, 5546, 5555]
...
function maybeUnpackValue(enabled) {
  const Reanimated = require(5531).Reanimated;
...
maybeUnpackValue.__closure = { Reanimated: require("module_5531").Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === require(5531).Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(5531).Reanimated;
...
export const bindSharedValues = function bindSharedValues(closure_1, _slicedToArray) {
  ...
  if (undefined !== _require(5531).Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      if (Reanimated.isSharedValue(tmp7)) {
        let tmp13 = tmp9;
        let tmp14 = tmp11;
        let Reanimated2 = tmp10(5531).Reanimated;
        let tmp15 = tmp7;
        let tmp16 = tmp6;
...
export const unbindSharedValues = function unbindSharedValues(closure_1, _slicedToArray) {
  if (undefined !== _require(5531).Reanimated) {
    ...
    function _loop(iter) {
      const callback = iter;
      const Reanimated = callback(5531).Reanimated;
      if (Reanimated.isSharedValue(iter)) {
        ...
        dependencyMap = sum;
        const Reanimated2 = callback(5531).Reanimated;
        const fn = function t() {
...
export const hasWorkletEventHandlers = function hasWorkletEventHandlers(useAnimated) {
  const iter = require(5546) /* items2 */.HandlerCallbacks[Symbol.iterator]();
