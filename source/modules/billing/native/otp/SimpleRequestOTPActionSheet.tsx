...
// Dependencies: [5, 32, 19, 17, 1922, 4527, 676, 1924, 21, 3, 1628, 10477, 589, 8492, 7286, 1946, 8222, 10758, 4445, 4858, 7299, 4926, 4474, 5601, 4928, 10601, 514, 10456, 5622, 10594, 2]
// Exports: default

// Module 15293 (GiftPurchaseSKUView)
...
function GiftPurchaseSKUView(selectedSkuId) {
  ...
  const effect = React.useEffect(() => {
    if (null != selectedSkuId) {
      const collectiblesProduct = selectedSkuId(7286).fetchCollectiblesProduct(tmp);
      const obj = selectedSkuId(7286);
    }
  }, items1);
...
function SimpleRequestOTPActionSheet(giftMessage) {
  ...
  if ("giftSku" === requestType) {
    if (null != selectedSkuId) {
      if (null != giftRecipientId) {
        ...
        let tmp6Result = callback2(tmp(10477).NativeGiftContextProvider, obj);
        let tmp8 = callback2;
      }
      obj1 = { children: null };
      obj1[0] = tmp6Result;
      return tmp8(tmp(5622).BottomSheet, obj1);
    }
  }
  tmp8 = callback2;
  const items = [callback2(_require(4474).Text, { variant: "text-lg/bold", color: "text-feedback-warning", children: "Gift purchasing is the only supported feature on Android in this version." }), ];
  let str = "none";
  if (null != requestType) {
    str = requestType;
  }
  ...
  items[1] = closure_11(_require(4474).Text, { variant: "text-md/normal", color: "text-feedback-warning", children: items1 });
  obj2[0] = items;
