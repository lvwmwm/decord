...
// Dependencies: [57, 4219, 653, 4226, 4703, 4222, 675, 4704, 2]
// Exports: getClipCreatedAt, getClipEventsTimeline, getClipParticipantIds

// Module 4702 (_createForOfIteratorHelperLoose)
      ...
      if (0 !== sorted.length) {
        ...
        if (!iter.done) {
          while (true) {
            ...
            if (value.signal.type !== _require(4222).ClipSignalTypes.SPEAKING) {
              let iter2 = tmp16();
              iter = iter2;
              if (iter2.done) {
                break;
        ...
        if (!iter5.done) {
          while (true) {
            ...
            if (value.signal.type === _require(4222).ClipSignalTypes.SPEAKING) {
              ...
              tmp44 = tmp33;
              tmp45 = tmp34;
              if (value.signal.type === _require(4222).ClipSignalTypes.GAME_EVENT) {
                tmp41 = tmp30;
                tmp42 = tmp31;
                tmp43 = tmp32;
                tmp44 = tmp33;
                ...
                if (value.timestamp >= sum) {
                  if (value.timestamp > sum1) {
                    ...
                    if (_require(4222).GameEventType.KILL === eventType) {
                      let tmp40 = obj;
                      let MULTIKILL = obj.KILL;
                    } else {
                      let tmp37 = _require;
                      let tmp38 = dependencyMap;
                      if (_require(4222).GameEventType.MULTIKILL === eventType) {
                        let tmp39 = obj;
        ...
        if (substr.length !== items.length) {
          ...
          obj6.clip_runtime = _require(4704).getClipsRuntime("getClipEventsTimeline");
          obj12.track(AnalyticEvents.CLIP_TIMELINE_TRIMMED, obj6);
          const obj14 = _require(4704);
        }
        let tmp58;
        if (substr.length > 0) {
          tmp58 = substr;
