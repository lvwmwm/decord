...
// Dependencies: [17, 676, 4406, 9217, 12, 1208, 589, 709, 2]

// Module 9216 (nativeEventEmitter)
...
prototype["initialize"] = function initialize() {
  const self = this;
  const audioDevices = importDefault(9217).getAudioDevices();
  ...
  const obj = importDefault(9217);
  const activeAudioDevice = importDefault(9217).getActiveAudioDevice();
  ...
  const obj2 = importDefault(9217);
  importDefault(9217).setSCORetryCount(4);
};
  ...
  RTC_CONNECTION_STATE: function handleRTCConnectionStateUpdate(context) {
    if (context.context !== MediaEngineContextTypes.DEFAULT) {
      ...
      if (RTCConnectionStates.CONNECTING === state) {
        let c9 = true;
        const result = importDefault(9217).setCommunicationModeOn(true);
        let tmp8 = enforcing !== enforcing;
        if (tmp8) {
          tmp8 = enforcing.simpleDeviceType !== require(9217) /* enforcing */.AudioDeviceType.INVALID;
        }
        if (tmp8) {
          ...
          if (tmp4Result.isString(enforcing)) {
            ...
            tmp4(9217).setActiveAudioDevice(tmp10);
            const tmp4Result1 = tmp4(9217);
          }
        }
        const obj2 = importDefault(9217);
