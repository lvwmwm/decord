...
// Dependencies: [17, 676, 4364, 9159, 12, 1208, 589, 709, 2]

// Module 9158 (nativeEventEmitter)
...
prototype["initialize"] = function initialize() {
  const self = this;
  const audioDevices = importDefault(9159).getAudioDevices();
  ...
  const obj = importDefault(9159);
  const activeAudioDevice = importDefault(9159).getActiveAudioDevice();
  ...
  const obj2 = importDefault(9159);
  importDefault(9159).setSCORetryCount(4);
};
  ...
  RTC_CONNECTION_STATE: function handleRTCConnectionStateUpdate(context) {
    if (context.context !== MediaEngineContextTypes.DEFAULT) {
      ...
      if (RTCConnectionStates.CONNECTING === state) {
        let c9 = true;
        const result = importDefault(9159).setCommunicationModeOn(true);
        let tmp8 = enforcing !== enforcing;
        if (tmp8) {
          tmp8 = enforcing.simpleDeviceType !== require(9159) /* enforcing */.AudioDeviceType.INVALID;
        }
        if (tmp8) {
          ...
          if (tmp4Result.isString(enforcing)) {
            ...
            tmp4(9159).setActiveAudioDevice(tmp10);
            const tmp4Result1 = tmp4(9159);
          }
        }
        const obj2 = importDefault(9159);
