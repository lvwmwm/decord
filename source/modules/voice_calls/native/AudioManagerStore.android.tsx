...
// Dependencies: [17, 676, 4364, 9153, 12, 1208, 589, 709, 2]

// Module 9152 (nativeEventEmitter)
...
prototype["initialize"] = function initialize() {
  const self = this;
  const audioDevices = importDefault(9153).getAudioDevices();
  ...
  const obj = importDefault(9153);
  const activeAudioDevice = importDefault(9153).getActiveAudioDevice();
  ...
  const obj2 = importDefault(9153);
  importDefault(9153).setSCORetryCount(4);
};
  ...
  RTC_CONNECTION_STATE: function handleRTCConnectionStateUpdate(context) {
    if (context.context !== MediaEngineContextTypes.DEFAULT) {
      ...
      if (RTCConnectionStates.CONNECTING === state) {
        let c9 = true;
        const result = importDefault(9153).setCommunicationModeOn(true);
        let tmp8 = enforcing !== enforcing;
        if (tmp8) {
          tmp8 = enforcing.simpleDeviceType !== require(9153) /* enforcing */.AudioDeviceType.INVALID;
        }
        if (tmp8) {
          ...
          if (tmp4Result.isString(enforcing)) {
            ...
            tmp4(9153).setActiveAudioDevice(tmp10);
            const tmp4Result1 = tmp4(9153);
          }
        }
        const obj2 = importDefault(9153);
