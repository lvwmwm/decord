...
// Dependencies: [57, 5, 31, 27, 4184, 1348, 4212, 1907, 4181, 8808, 8811, 4379, 8813, 4506, 1212, 4710, 8814, 8828, 1820, 4376, 4979, 22, 8829, 8830, 8831, 477, 566, 8832, 8812, 9031, 8809, 2]
...
// Module 8807 (_handleToggleVideo)
...
export const handleToggleSelfMute = function handleToggleSelfMute() {
  const permission = importDefault(4710).requestPermission(NativePermissionTypes.AUDIO, { showAuthorizationError: true });
  const obj = importDefault(4710);
  importDefault(8814).toggleSelfMute();
};
...
export const showMinOSScreenshareRequirementAlert = function showMinOSScreenshareRequirementAlert() {
  const intl = require(1212) /* getSystemLocale */.intl;
  let obj = {};
  const errorInfo = require(8828) /* validateUniqueErrorCodes */.getErrorInfo(require(8828) /* validateUniqueErrorCodes */.AVError.SCREENSHARE_OS_NOT_SUPPORTED);
  let errorCode;
  if (null != errorInfo) {
    errorCode = errorInfo.errorCode;
  }
  obj.errorCode = errorCode;
  const obj2 = require(8828) /* validateUniqueErrorCodes */;
...
export const handleDisconnect = function handleDisconnect(channel) {
  ...
  const result1 = require(4376) /* getVoiceChannelKeyByChannelId */.dismissVoiceChannelScreens(channel, () => {
...
export const useMaskedSpeakerStates = set.isAndroid() ? (() => {
  ...
  const stateFromStoresObject = obj.useStateFromStoresObject(items, () => {
    ...
    let tmp3 = currentRouteType === isEnabled(8809).RouteTypes.SPEAKER;
    const tmp4 = currentRouteType === isEnabled(8809).RouteTypes.BLUETOOTH;
    obj = {};
    if (!tmp3) {
      tmp3 = tmp4;
    }
...
export const useImmediateMaskedSpeakerStates = () => {
  ...
  const tmp2 = importDefault(9031)();
  require = tmp2;
  let tmp3 = stateFromStores === require(8809) /* RouteTypes */.RouteTypes.SPEAKER;
  const tmp4 = stateFromStores === require(8809) /* RouteTypes */.RouteTypes.BLUETOOTH;
  if (!tmp3) {
    tmp3 = tmp4;
  }
  if (!tmp3) {
