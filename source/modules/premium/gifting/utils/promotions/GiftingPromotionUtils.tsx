...
// Dependencies: [32, 19, 7487, 1924, 589, 8434, 9762, 9763, 9764, 9765, 7791, 4201, 1377, 2]
...
// Module 9761 (useFetchClaimableGiftingPromotionRewardSkuIds)
import apexExperiment from "apexExperiment" /* 9764 */;
import apexExperiment2 from "apexExperiment" /* 9765 */;
...
export const useFetchClaimableGiftingPromotionRewardSkuIds = function useFetchClaimableGiftingPromotionRewardSkuIds() {
  ...
  const obj = first(589);
  const fetchPurchases = first(8434).useFetchPurchases();
  ...
  const someResult = stateFromStoresArray.some((arg0) => arg0 === first(9762).GiftingPromotionRewardSKUIds.SUMMER_2026_GOGO_FAKE_SKU_ID);
  closure_7 = someResult;
  const obj3 = first(8434);
  const fetchSummer2026GogoPromoEligibility = first(9763).useFetchSummer2026GogoPromoEligibility(someResult);
...
export const shouldShowGiftPromotionReminderNotice = function shouldShowGiftPromotionReminderNotice() {
  ...
  if (GiftPromotionReminderExperiment.getConfig({ location: "shouldShowGiftPromotionReminderNotice" }).enabled) {
    if (null == marketingComponentByType.getMarketingComponentByType(tmp(7791).MarketingComponentType.GIFT_REMINDER_NAGBAR)) {
      return false;
    } else {
