...
// Dependencies: [9960, 15918, 2]
// Exports: default

// Module 15917 (useGiftingPromotionAssetsReady)
...
export default function useGiftingPromotionAssetsReady(asset, asset2) {
  let obj = require(9960) /* useThemeAndReducedMotionAwareAssetUrl */;
  asset = undefined;
  if (asset != null) {
    asset = asset.asset;
  }
  const themeAndReducedMotionAwareAssetUrl = obj.useThemeAndReducedMotionAwareAssetUrl(asset);
  const status = importDefault(15918)(themeAndReducedMotionAwareAssetUrl).status;
  let asset1;
  if (asset2 != null) {
    asset1 = asset2.asset;
  }
  const themeAndReducedMotionAwareAssetUrl1 = require(9960) /* useThemeAndReducedMotionAwareAssetUrl */.useThemeAndReducedMotionAwareAssetUrl(asset1);
  const status2 = importDefault(15918)(themeAndReducedMotionAwareAssetUrl1).status;
