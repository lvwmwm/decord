...
// Dependencies: [32, 19, 4013, 6955, 1924, 676, 5767, 6927, 4663, 7633, 4061, 1236, 4207, 2]
// Exports: default

// Module 8773 (usePremiumFeatureUpsellGetNitro)
...
export default function usePremiumFeatureUpsellGetNitro(arg0, arg1, arg2, arg3) {
  ...
  callback = loading[1];
  analyticsLocations = importDefault(5767)(items).analyticsLocations;
  ...
  onPress = analyticsLocations.useCallback(() => {
    ...
    if (result) {
      if (tmp3) {
        if (tmp5) {
          ...
          callback2(6927)(obj, dependencyMap);
        }
      }
    }
    if (!result) {
      if (ref.current < 5) {
        ...
        if (result) {
          ...
          resolved = callback(4663).fetchSubscriptions();
          const obj2 = callback(4663);
        }
        const items = [resolved, ];
        if (isFetchingOfferResult) {
          let resolved1 = Promise.resolve();
        } else {
          resolved1 = callback(7633).fetchUserOffer("usePremiumFeatureUpsellGetNitro");
          const obj3 = callback(7633);
        }
        items[1] = resolved1;
    ...
    const bestActiveInput = callback(4207).getBestActiveInput();
