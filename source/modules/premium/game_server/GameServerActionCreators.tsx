...
// Dependencies: [1946, 1874, 11575, 4078, 676, 709, 11576, 4427, 530, 11578, 4099, 698, 11579, 11577, 2]
...
// Module 11574 (fetchGameServerCatalog)
...
let result = require("handleGameServerUpsert").fileFinishedImporting("modules/premium/game_server/GameServerActionCreators.tsx");
...
export const fetchGameServerGlobalCatalog = function fetchGameServerGlobalCatalog() {
  ...
  const obj2 = require(4427) /* _httpGetWithCountryCodeQuery */;
...
export const fetchGameServerRegions = function fetchGameServerRegions(arg0) {
  ...
  return value.then((body) => {
    ...
    body = body.body;
    const mapped = body.map(callback(11579));
...
export const fetchMyGameServerRegions = function fetchMyGameServerRegions() {
  ...
  return value.then((body) => {
    ...
    const mapped = regions.map(callback(11579));
    obj[1] = mapped.sort((name, name2) => {
      name = name.name;
      return name.localeCompare(name2.name);
    });
...
export const fetchMyGameServers = function fetchMyGameServers() {
  ...
  return value.then((body) => {
    ...
    callback(table[5]).dispatch({ type: "GAME_SERVER_FETCH_MY_SERVERS_SUCCESS", gameServers: game_servers });
  });
};
export const optimisticallyMarkGameServerResizing = function optimisticallyMarkGameServerResizing(arg0) {
  ...
  if (null != found) {
    let obj = importDefault(709);
    obj = { type: "GAME_SERVER_UPDATE", guildId: "Array", gameServer: "none" };
    obj = {};
    const merged = Object.assign(found);
    obj.status = _require(11577).GameServerStatus.STARTING;
    obj[2] = obj;
    obj.dispatch(obj);
  }
};
