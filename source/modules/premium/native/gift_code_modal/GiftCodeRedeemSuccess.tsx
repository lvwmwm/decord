...
// Dependencies: [32, 19, 17, 4494, 21, 4445, 712, 589, 8420, 5911, 8424, 7232, 1950, 8425, 10750, 5505, 6025, 8435, 5915, 8438, 4658, 1297, 8458, 10777, 10778, 12763, 4441, 1236, 4705, 4880, 4676, 2]
// Exports: default

// Module 12767 (GiftCodeRedeemSuccess)
  ...
  if (null == first) {
    if (null != getOrFetchApplication) {
      if (tmp2Result1.isGameItemSKU(stateFromStores)) {
        ...
        obj3[1] = tmp24(user(8435), obj4);
        let tmp24Result = tmp24(closure_5, obj3);
      } else {
        const obj5 = { game: null, size: null, skuId: null };
        obj5[0] = getOrFetchApplication;
        obj5[1] = tmp2(5915).GameIconSizes.LARGE;
        obj5[2] = giftCode.skuId;
        tmp24Result = tmp24(user(5915), obj5);
        const tmp26 = user(5915);
      }
      tmp2Result1 = tmp2(6025);
    }
  }
  if (type === giftCode(1950).CollectiblesItemType.BUNDLE) {
    if (null != product) {
      ...
      if (tmp20Result) {
        ...
        obj8[6] = tmp15;
        tmp20Result = tmp20(user(8438), obj8);
      }
      obj7[2] = tmp20Result;
    ...
    if (null == stateFromStores) {
      ...
      let tmp31 = callback2(tmp2(4441).Text, obj9);
    } else {
      if (tmp2Result2.isGameItemSKU(stateFromStores)) {
        ...
        tmp31 = callback2(tmp2(4441).Text, obj10);
      } else {
        if (giftCode.isSubscription) {
          if (null != getOrFetchSubscriptionPlan) {
            ...
            tmp31 = callback2(tmp2(4441).Text, obj11);
          }
        }
        if (null != first) {
          ...
          tmp31 = callback2(tmp2(4441).Text, obj13);
        }
      }
      tmp2Result2 = tmp2(6025);
    }
    items1[1] = tmp31;
    if (tmp2Result3.isGameItemSKU(stateFromStores)) {
      if (null != getOrFetchApplication) {
        ...
        let tmp36Result = tmp42(tmp2(4441).Text, obj14);
        let tmp36 = tmp42;
      }
    ...
    if (giftCode.isSubscription) {
      if (null != getOrFetchSubscriptionPlan) {
        ...
        obj19[1] = tmp.message;
        obj19[2] = tmp2(4705).getSubscriptionGiftSuccessText(getOrFetchSubscriptionPlan);
        tmp36Result = callback2(tmp2(4441).Text, obj19);
        tmp36 = callback2;
        const tmp2Result4 = tmp2(4705);
      }
    }
    tmp36 = callback2;
    ...
    tmp2Result3 = tmp2(6025);
  }
  const tmp14 = callback(React.useState(), 2);
  tmp18 = closure_6;
  const match = giftCode(4658).match(first);
  const str = giftCode(4658);
