...
// Dependencies: [32, 19, 17, 4300, 21, 4255, 712, 589, 8377, 5587, 8381, 6910, 1901, 8382, 9955, 5243, 5728, 8387, 5591, 8390, 4455, 1297, 8410, 8931, 9982, 12444, 4251, 1236, 4490, 4666, 4461, 2]
// Exports: default

// Module 12448 (GiftCodeRedeemSuccess)
  ...
  if (null == first) {
    if (null != getOrFetchApplication) {
      if (tmp2Result1.isGameItemSKU(stateFromStores)) {
        ...
        obj3[1] = tmp24(user(8387), obj4);
        let tmp24Result = tmp24(closure_5, obj3);
      } else {
        const obj5 = { game: null, size: null, skuId: null };
        obj5[0] = getOrFetchApplication;
        obj5[1] = tmp2(5591).GameIconSizes.LARGE;
        obj5[2] = giftCode.skuId;
        tmp24Result = tmp24(user(5591), obj5);
        const tmp26 = user(5591);
      }
      tmp2Result1 = tmp2(5728);
    }
  }
  if (type === giftCode(1901).CollectiblesItemType.BUNDLE) {
    if (null != product) {
      ...
      if (tmp20Result) {
        ...
        obj8[6] = tmp15;
        tmp20Result = tmp20(user(8390), obj8);
      }
      obj7[2] = tmp20Result;
    ...
    if (null == stateFromStores) {
      ...
      let tmp31 = callback2(tmp2(4251).Text, obj9);
    } else {
      if (tmp2Result2.isGameItemSKU(stateFromStores)) {
        ...
        tmp31 = callback2(tmp2(4251).Text, obj10);
      } else {
        if (giftCode.isSubscription) {
          if (null != getOrFetchSubscriptionPlan) {
            ...
            tmp31 = callback2(tmp2(4251).Text, obj11);
          }
        }
        if (null != first) {
          ...
          tmp31 = callback2(tmp2(4251).Text, obj13);
        }
      }
      tmp2Result2 = tmp2(5728);
    }
    items1[1] = tmp31;
    if (tmp2Result3.isGameItemSKU(stateFromStores)) {
      if (null != getOrFetchApplication) {
        ...
        let tmp36Result = tmp42(tmp2(4251).Text, obj14);
        let tmp36 = tmp42;
      }
    ...
    if (giftCode.isSubscription) {
      if (null != getOrFetchSubscriptionPlan) {
        ...
        obj19[1] = tmp.message;
        obj19[2] = tmp2(4490).getSubscriptionGiftSuccessText(getOrFetchSubscriptionPlan);
        tmp36Result = callback2(tmp2(4251).Text, obj19);
        tmp36 = callback2;
        const tmp2Result4 = tmp2(4490);
      }
    }
    tmp36 = callback2;
    ...
    tmp2Result3 = tmp2(5728);
  }
  const tmp14 = callback(React.useState(), 2);
  tmp18 = closure_6;
  const match = giftCode(4455).match(first);
  const str = giftCode(4455);
