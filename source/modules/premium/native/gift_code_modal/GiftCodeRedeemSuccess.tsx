...
// Dependencies: [32, 19, 17, 4495, 21, 4446, 712, 589, 8434, 5924, 8438, 7246, 1951, 8439, 10768, 5518, 6038, 8449, 5928, 8452, 4671, 1297, 8472, 10795, 10796, 12782, 4442, 1236, 4718, 4893, 4689, 2]
// Exports: default

// Module 12786 (GiftCodeRedeemSuccess)
import ThemesDefault from "Themes" /* 712 */;
import closure_3 from "_slicedToArray" /* 32 */;
import closure_4 from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_7 from "addSku" /* 4495 */;
...
export default function GiftCodeRedeemSuccess(giftCode) {
  ...
  if (product == null) {
    ...
    obj[1] = tmp2(1951).CollectiblesItemType.BUNDLE;
    obj[2] = [];
    tmp12 = obj;
  }
  ...
  if (null == first) {
    if (null != getOrFetchApplication) {
      if (tmp2Result1.isGameItemSKU(stateFromStores)) {
        ...
        obj3[1] = tmp24(user(8449), obj4);
        let tmp24Result = tmp24(closure_5, obj3);
      } else {
        const obj5 = { game: null, size: null, skuId: null };
        obj5[0] = getOrFetchApplication;
        obj5[1] = tmp2(5928).GameIconSizes.LARGE;
        obj5[2] = giftCode.skuId;
        tmp24Result = tmp24(user(5928), obj5);
        const tmp26 = user(5928);
      }
      tmp2Result1 = tmp2(6038);
    }
  }
  if (type === giftCode(1951).CollectiblesItemType.BUNDLE) {
    if (null != product) {
      ...
      if (tmp20Result) {
        ...
        obj8[6] = tmp15;
        tmp20Result = tmp20(user(8452), obj8);
      }
      obj7[2] = tmp20Result;
    ...
    if (null == stateFromStores) {
      ...
      let tmp31 = callback2(tmp2(4442).Text, obj9);
    } else {
      if (tmp2Result2.isGameItemSKU(stateFromStores)) {
        ...
        tmp31 = callback2(tmp2(4442).Text, obj10);
      } else {
        if (giftCode.isSubscription) {
          if (null != getOrFetchSubscriptionPlan) {
            ...
            tmp31 = callback2(tmp2(4442).Text, obj11);
          }
        }
        if (null != first) {
          ...
          tmp31 = callback2(tmp2(4442).Text, obj13);
        }
      }
      tmp2Result2 = tmp2(6038);
    }
    items1[1] = tmp31;
    if (tmp2Result3.isGameItemSKU(stateFromStores)) {
      if (null != getOrFetchApplication) {
        ...
        let tmp36Result = tmp42(tmp2(4442).Text, obj14);
        let tmp36 = tmp42;
      }
    ...
    if (giftCode.isSubscription) {
      if (null != getOrFetchSubscriptionPlan) {
        ...
        obj19[1] = tmp.message;
        obj19[2] = tmp2(4718).getSubscriptionGiftSuccessText(getOrFetchSubscriptionPlan);
        tmp36Result = callback2(tmp2(4442).Text, obj19);
        tmp36 = callback2;
        const tmp2Result4 = tmp2(4718);
      }
    }
    tmp36 = callback2;
    ...
    tmp2Result3 = tmp2(6038);
  }
  const tmp14 = callback(React.useState(), 2);
  tmp18 = closure_6;
  const match = giftCode(4671).match(first);
  const str = giftCode(4671);
  const obj22 = { type: giftCode(1951).CollectiblesItemType.AVATAR_DECORATION };
  const withResult = match.with({ type: giftCode(1951).CollectiblesItemType.AVATAR_DECORATION }, (avatarDecoration) => {
    let obj = user;
    let avatarSource;
    if (user != null) {
  ...
  const obj23 = { type: giftCode(1951).CollectiblesItemType.PROFILE_EFFECT };
  const withResult1 = withResult.with({ type: giftCode(1951).CollectiblesItemType.PROFILE_EFFECT }, (profileEffect) => closure_1_8(user(nameplateContainer[22]), { user, profileEffect }));
  const obj24 = { type: giftCode(1951).CollectiblesItemType.PROFILE_FRAME };
  const withResult2 = withResult1.with({ type: giftCode(1951).CollectiblesItemType.PROFILE_FRAME }, (profileFrame) => closure_1_8(user(nameplateContainer[23]), { user, profileFrame }));
  const obj25 = { type: giftCode(1951).CollectiblesItemType.NAMEPLATE };
  tmp20Result = withResult2.with({ type: giftCode(1951).CollectiblesItemType.NAMEPLATE }, (nameplate) => {
