...
// Dependencies: [32, 19, 17, 4497, 21, 4448, 712, 589, 8456, 5927, 8460, 7267, 1950, 8461, 10792, 5521, 6041, 8471, 5931, 8474, 4673, 1297, 8494, 10819, 10820, 12817, 4444, 1236, 4720, 4896, 4691, 2]
// Exports: default

// Module 12821 (GiftCodeRedeemSuccess)
...
export default function GiftCodeRedeemSuccess(giftCode) {
  ...
  tmp2Result = tmp2(10792);
  let tmp12 = product;
  if (product == null) {
    obj = { skuId: "", type: null, items: null };
    obj[1] = tmp2(1950).CollectiblesItemType.BUNDLE;
    obj[2] = [];
    tmp12 = obj;
  }
  ...
  if (null == first) {
    if (null != getOrFetchApplication) {
      if (tmp2Result1.isGameItemSKU(stateFromStores)) {
        ...
        obj3[1] = tmp24(user(8471), obj4);
        let tmp24Result = tmp24(closure_5, obj3);
      } else {
        const obj5 = { game: null, size: null, skuId: null };
        obj5[0] = getOrFetchApplication;
        obj5[1] = tmp2(5931).GameIconSizes.LARGE;
        obj5[2] = giftCode.skuId;
        tmp24Result = tmp24(user(5931), obj5);
        const tmp26 = user(5931);
      }
      tmp2Result1 = tmp2(6041);
    }
  }
  if (type === giftCode(1950).CollectiblesItemType.BUNDLE) {
    if (null != product) {
      ...
      if (tmp20Result) {
        ...
        obj8[6] = tmp15;
        tmp20Result = tmp20(user(8474), obj8);
      }
      obj7[2] = tmp20Result;
    ...
    if (null == stateFromStores) {
      ...
      let tmp31 = callback2(tmp2(4444).Text, obj9);
    } else {
      if (tmp2Result2.isGameItemSKU(stateFromStores)) {
        ...
        tmp31 = callback2(tmp2(4444).Text, obj10);
      } else {
        if (giftCode.isSubscription) {
          if (null != getOrFetchSubscriptionPlan) {
            ...
            tmp31 = callback2(tmp2(4444).Text, obj11);
          }
        }
        if (null != first) {
          ...
          tmp31 = callback2(tmp2(4444).Text, obj13);
        }
      }
      tmp2Result2 = tmp2(6041);
    }
    items1[1] = tmp31;
    if (tmp2Result3.isGameItemSKU(stateFromStores)) {
      if (null != getOrFetchApplication) {
        ...
        let tmp36Result = tmp42(tmp2(4444).Text, obj14);
        let tmp36 = tmp42;
      }
    ...
    if (giftCode.isSubscription) {
      if (null != getOrFetchSubscriptionPlan) {
        ...
        obj19[1] = tmp.message;
        obj19[2] = tmp2(4720).getSubscriptionGiftSuccessText(getOrFetchSubscriptionPlan);
        tmp36Result = callback2(tmp2(4444).Text, obj19);
        tmp36 = callback2;
        const tmp2Result4 = tmp2(4720);
      }
    }
    tmp36 = callback2;
    ...
    tmp2Result3 = tmp2(6041);
  }
  const tmp14 = callback(React.useState(), 2);
  tmp18 = closure_6;
  const match = giftCode(4673).match(first);
  const str = giftCode(4673);
