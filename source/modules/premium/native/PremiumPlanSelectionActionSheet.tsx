...
// Function ID: 7116
// Name: Header
// Dependencies: [109, 5, 32, 19, 17, 7116, 1997, 6049, 7113, 1925, 676, 1301, 4428, 505, 21, 4446, 712, 4108, 4442, 1236, 38, 7123, 1898, 589, 12893, 6046, 4893, 4880, 4889, 10418, 7109, 4381, 7139, 7915, 8184, 8026, 5918, 5938, 5906, 10393, 7096, 4910, 10520, 4413, 698, 7095, 1625, 4823, 10419, 4803, 7099, 1363, 10437, 10438, 10439, 10440, 10441, 10442, 500, 1996, 5587, 8428, 5902, 5458, 8318, 12894, 1297, 8166, 10422, 10516, 2]
...
import PremiumPlanActionSheetHeaderDefault from "PremiumPlanActionSheetHeader" /* 7123 */;
...
function Header(arg0) {
  ...
  if (tmp9) {
    ...
    tmp9 = skuId === tmp6(4108).getSkuIdForPremiumType(premiumType);
    const tmp6Result = tmp6(4108);
  }
  let tmp11 = tmp9;
  if (tmp11) {
...
function PlanOption(premiumItem) {
  ...
  if (null != discountedPriceString) {
    tmp4Result = tmp4(6046);
    formatRateResult = tmp4Result.formatRate(discountedPriceString, tmp24.interval, tmp24.intervalCount);
  }
  if (tmp12) {
    const intl = tmp4(1236).intl;
    obj = { price: null };
    obj[0] = tmp4(6046).formatPrice(0, USD, { minimumFractionDigits: 0, maximumFractionDigits: 0 });
  ...
  const tmp4Result2 = premiumItem(6046);
  if (first > 0) {
  ...
  obj3[2] = combined;
  const items3 = [closure_35(premiumItem(4442).Text, obj3), ];
  ...
  items4[1] = closure_35(premiumItem(4442).Text, obj9);
  obj5[1] = items4;
  ...
  if (!tmp12) {
    ...
    if (null != formatRateResult) {
      ...
      if (null != discountOffer) {
        ...
        if (null == premiumItem.selectedPremiumType) {
          ...
          tmp32Result = tmp32(tmp4(4442).Text, obj11);
        }
      }
    }
    ...
    if (tmp32Result1) {
      ...
      tmp32Result1 = tmp32(tmp4(4442).Text, obj13);
    }
          ...
          if (0 === c5) {
            ...
            if (1 === tmp7) {
              ...
              if (2 === tmp7) {
                c4 = 0;
                c5 = c3;
                if (c5 instanceof closure_1_1(10419)) {
                  obj1 = callback(4803);
                  const subscriptions = obj1.fetchSubscriptions();
                  obj2 = closure_1_1(4823);
...
export default function PremiumPlanSelectionActionSheetWithOrderCTX(predicate) {
  const NitroACOMSubscriptionExperiment = TIER_2(8166).NitroACOMSubscriptionExperiment;
  let obj = TIER_2(500);
  if (obj.isIOS()) {
    ...
    if (null == fn3) {
      let premiumBundlesWithPredicate = tmp(7099).getPremiumBundlesWithPredicate(fn);
      const tmpResult1 = tmp(7099);
    } else {
      const premiumBundlesWithPredicate1 = tmp(7099).getPremiumBundlesWithPredicate(fn);
    ...
    if (null != found) {
      const subscriptionItemsForProduct = tmp(7099).getSubscriptionItemsForProduct(found.productId);
      let mapped = subscriptionItemsForProduct.map((planId) => {
        const obj = { subscriptionPlanId: planId.planId, skuId: null, quantity: null };
        const obj2 = TIER_2(4108);
        obj[1] = obj2.castPremiumSubscriptionAsSkuId(callback(4108).getSkuIdForPlan(planId.planId));
      ...
      obj1[1] = tmp(4108).castPremiumSubscriptionAsSkuId(TIER_22.TIER_2);
      mapped = [obj1];
      const tmpResult4 = tmp(4108);
    }
    ...
    obj2[7] = function onOrderRetryCancellation() {
      return callback(4413).hideActionSheet(closure_24);
    };
    const obj3 = {};
