...
// Function ID: 6809
// Name: Header
// Dependencies: [109, 5, 32, 19, 17, 6809, 1975, 5779, 6806, 1905, 676, 1301, 4268, 505, 21, 4285, 712, 3930, 4281, 1236, 38, 6816, 1879, 589, 7253, 5776, 4695, 7254, 6802, 4221, 6832, 7262, 7324, 7325, 5649, 5669, 5637, 7295, 6789, 4711, 7296, 4253, 698, 6788, 1605, 4624, 7256, 4604, 6792, 4130, 7313, 7314, 7315, 7316, 7317, 7318, 500, 1974, 5377, 7326, 5633, 5251, 7329, 7332, 1297, 7333, 7264, 7334, 2]
...
function Header(arg0) {
  ...
  if (tmp9) {
    ...
    let tmp11 = sku_id === tmp6(3930).getSkuIdForPremiumType(premiumType);
    if (tmp11) {
    ...
    tmp9 = tmp11;
    const tmp6Result = tmp6(3930);
  }
  trialOffer = null;
  return callback3(importDefault(6816), { premiumType, trialOffer, discountOffer });
}
...
function PlanOption(premiumItem) {
  ...
  const tmp4Result2 = premiumItem(5776);
  if (first > 0) {
  ...
  obj3[2] = combined;
  const items3 = [closure_35(premiumItem(4281).Text, obj3), ];
  ...
  items4[1] = closure_35(premiumItem(4281).Text, obj9);
  obj5[1] = items4;
  ...
  if (!tmp9) {
    ...
    if (null != formatRateResult) {
      ...
      if (null != discountOffer) {
        ...
        if (null == premiumItem.selectedPremiumType) {
          ...
          tmp32Result = tmp32(tmp4(4281).Text, obj11);
        }
      }
    }
    ...
    if (tmp32Result1) {
      ...
      tmp32Result1 = tmp32(tmp4(4281).Text, obj13);
    }
...
export default function PremiumPlanSelectionActionSheetWithOrderCTX(predicate) {
  const NitroACOMSubscriptionExperiment = TIER_2(7333).NitroACOMSubscriptionExperiment;
  let obj = TIER_2(500);
  if (obj.isIOS()) {
    ...
    if (null == fn3) {
      let premiumBundlesWithPredicate = tmp(6792).getPremiumBundlesWithPredicate(fn);
      const tmpResult1 = tmp(6792);
    } else {
      const premiumBundlesWithPredicate1 = tmp(6792).getPremiumBundlesWithPredicate(fn);
      premiumBundlesWithPredicate = premiumBundlesWithPredicate1.sort(fn3);
      const tmpResult2 = tmp(6792);
    }
    ...
    if (null != found) {
      const subscriptionItemsForProduct = tmp(6792).getSubscriptionItemsForProduct(found.productId);
      let mapped = subscriptionItemsForProduct.map((planId) => {
        const obj = { subscriptionPlanId: planId.planId, skuId: null, quantity: null };
        const obj2 = TIER_2(3930);
        obj[1] = obj2.castPremiumSubscriptionAsSkuId(callback(3930).getSkuIdForPlan(planId.planId));
        obj[2] = planId.quantity;
        return obj;
      });
      const tmpResult3 = tmp(6792);
    } else {
      const obj1 = { subscriptionPlanId: null, skuId: null, quantity: 1 };
      obj1[0] = closure_21.PREMIUM_YEAR_TIER_2;
      obj1[1] = tmp(3930).castPremiumSubscriptionAsSkuId(TIER_22.TIER_2);
      mapped = [obj1];
      const tmpResult4 = tmp(3930);
    }
    ...
    obj2[7] = function onOrderRetryCancellation() {
      return callback(4253).hideActionSheet(closure_24);
    };
    const obj3 = {};
