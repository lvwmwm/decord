...
// Function ID: 6782
// Name: Header
// Dependencies: [109, 5, 32, 19, 17, 6782, 1946, 5752, 6779, 1876, 676, 1301, 4238, 505, 21, 4255, 712, 3901, 4251, 1236, 38, 6789, 1851, 589, 7226, 5749, 4665, 7227, 6775, 4191, 6805, 7235, 7296, 7297, 5595, 5615, 5583, 7267, 6762, 4681, 7268, 4223, 698, 6761, 1577, 4594, 7229, 4574, 6765, 4101, 7285, 7286, 7287, 7288, 7289, 7290, 500, 1945, 5323, 7298, 5579, 5221, 7301, 7304, 1297, 7305, 7237, 7306, 2]
...
function Header(arg0) {
  ...
  trialOffer = null;
  return callback3(importDefault(6789), { premiumType, trialOffer, discountOffer });
}
...
function PlanOption(premiumItem) {
  ...
  const tmp4Result2 = premiumItem(5749);
  if (first > 0) {
...
function PremiumPlanSelectionActionSheetCTA(isPaymentSuccess) {
  ...
  if (isPaymentSuccess.isPaymentSuccess) {
    ...
    obj[3] = tmp;
    return callback3(require(4665) /* Button */.Button, obj);
  } else {
...
export default function PremiumPlanSelectionActionSheetWithOrderCTX(predicate) {
  const NitroACOMSubscriptionExperiment = TIER_2(7305).NitroACOMSubscriptionExperiment;
  let obj = TIER_2(500);
  if (obj.isIOS()) {
    ...
    if (null == fn3) {
      let premiumBundlesWithPredicate = tmp(6765).getPremiumBundlesWithPredicate(fn);
      const tmpResult1 = tmp(6765);
    } else {
      const premiumBundlesWithPredicate1 = tmp(6765).getPremiumBundlesWithPredicate(fn);
      premiumBundlesWithPredicate = premiumBundlesWithPredicate1.sort(fn3);
      const tmpResult2 = tmp(6765);
    }
    ...
    if (null != found) {
      const subscriptionItemsForProduct = tmp(6765).getSubscriptionItemsForProduct(found.productId);
    ...
    return callback3(importDefault(7306), obj2);
  }
};
