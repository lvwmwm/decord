...
function AccountCreditTier(arg0) {
  ...
  if (hasPremiumGroup) {
    ...
    let stringResult = intl3.string(tmp4(2805)["5asczk"]);
  } else {
    if (null != currentSubscription) {
      if (currentSubscription.planId === planId) {
        if (currentSubscription.status === SubscriptionStatusTypes.PAUSED) {
          ...
          let num = tmp2(3835).extendDateWithUnconsumedFractionalPremium(date, unconsumedFractionalPremiumUnits);
  ...
  obj2[0] = require(5525) /* GameIcon */.GameIconSizes.SMALL;
  obj2[1] = result;
  const items1 = [callback(importDefault(5525), obj2), , ];
  ...
  const items2 = [callback(require(4185) /* Text */.Text, obj4), ];
  ...
  if (!(null != currentSubscription && currentSubscription.isPurchasedExternally)) {
    ...
    obj5[0] = tmp.subText;
    obj5[3] = stringResult;
    tmp21Result = tmp21(tmp2(4185).Text, obj5);
  }
  ...
  items1[2] = callback(require(4185) /* Text */.Text, obj6);
  obj1[1] = items1;
...
export default function PremiumAccountCredit(currentSubscription) {
  ...
  if (null != entitlements) {
    if (obj8.hasAccountCredit(entitlements)) {
      ...
      items1[2] = callback(tmp2(4185).Text, obj2);
      let tmp9Result = null;
      if (null != currentSubscription) {
        tmp9Result = null;
        if (currentSubscription.isPurchasedExternally) {
          ...
          tmp9Result = callback(tmp2(4185).Text, obj3);
        }
      }
      items1[3] = tmp9Result;
      obj[1] = items1;
