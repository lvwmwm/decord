...
// Dependencies: [19, 17, 1924, 21, 4448, 712, 10438, 5567, 8333, 10487, 4175, 4444, 1236, 2]
// Exports: default

// Module 10753 (PremiumGiftDurationButton)
...
function PremiumGiftDurationButton(arg0) {
  ...
  let setPlanInterval;
  let obj = planInterval(10438);
  const nativeGiftContext = obj.useNativeGiftContext();
  setPlanInterval = nativeGiftContext.setPlanInterval;
  const tmp4 = callback3();
  if (selected) {
    let RowButton = tmp(5567).TableRow;
  } else {
    RowButton = tmp(8333).RowButton;
  }
  let combined = null;
  ...
  const tmp9 = setPlanInterval(10487)(nativeGiftContext.premiumType, planInterval);
  const radioA11yNative = planInterval(4175).useRadioA11yNative({ selected });
  ...
  const items1 = [closure_6(planInterval(4444).Text, { variant: "text-md/semibold", children: stringResult }), ];
...
export default function PremiumGiftDuration() {
  ...
  items = [callback(planInterval(4444).Text, obj), ];
