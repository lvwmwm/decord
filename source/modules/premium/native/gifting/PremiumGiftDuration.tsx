...
// Dependencies: [19, 17, 1924, 21, 4444, 712, 9824, 5546, 8188, 10262, 4173, 4440, 1236, 2]
// Exports: default

// Module 10338 (PremiumGiftDurationButton)
...
function PremiumGiftDurationButton(arg0) {
  ...
  let setPlanInterval;
  let obj = planInterval(9824);
  const nativeGiftContext = obj.useNativeGiftContext();
  setPlanInterval = nativeGiftContext.setPlanInterval;
  const tmp4 = callback3();
  if (selected) {
    let RowButton = tmp(5546).TableRow;
  } else {
    RowButton = tmp(8188).RowButton;
  }
  let combined = null;
  ...
  const tmp9 = setPlanInterval(10262)(nativeGiftContext.premiumType, planInterval);
  ...
  const items1 = [closure_6(planInterval(4440).Text, { variant: "text-md/semibold", children: stringResult }), ];
  ...
  if (tmp11Result) {
    ...
    obj1[1] = tmp11(tmp(4440).Text, obj2);
  ...
  obj[1] = closure_6(RowButton, {
    ...
    trailing: closure_6(planInterval(4440).Text, { variant: "text-md/semibold", children: tmp9 }),
    onPress() {
...
export default function PremiumGiftDuration() {
  ...
  items = [callback(planInterval(4440).Text, obj), ];
