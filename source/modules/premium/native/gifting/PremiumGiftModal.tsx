...
export default function PremiumGiftModal(analyticsLocations) {
  ...
  const tmp3 = importDefault(5503)(() => recipientUserId(_undefined2[21]).v4());
  callback = tmp3;
  let obj = c4;
  ...
  if (initialRoute != null) {
    ...
    if (initialRoute === obj.REWARD_SELECT) {
      tmp11Result = tmp11(5143);
      let headerCloseButton = tmp11Result.getHeaderCloseButton(callback);
    } else {
      headerCloseButton = tmp11(5143).getHeaderBackButton();
      const tmp11Result1 = tmp11(5143);
    }
    ...
    if (initialRoute === obj.CUSTOMIZATION) {
      let headerCloseButton1 = tmp11(5143).getHeaderCloseButton(callback);
      const tmp11Result2 = tmp11(5143);
    } else {
      headerCloseButton1 = tmp11(5143).getHeaderBackButton();
      const tmp11Result3 = tmp11(5143);
    }
    ...
    obj5[1] = recipientUserId(5143).getHeaderCloseButton(callback);
    obj5[2] = tmp10.header;
    ...
    obj6[0] = intl2.string(tmp(2319).roVAey);
    const tmp11Result4 = recipientUserId(5143);
    obj6[1] = recipientUserId(5143).getHeaderCloseButton(callback);
    ...
    c4 = tmp22;
    const tmp11Result5 = recipientUserId(5143);
    if (tmp11Result6.isPremiumGiftingSupported()) {
      const obj7 = { value: null, children: null };
      obj7[0] = importDefault(5515)(analyticsLocations).analyticsLocations;
