...
// Dependencies: [32, 19, 1921, 21, 10592, 4481, 709, 586, 8706, 1233, 5579, 10593, 10971, 10974, 11003, 2464, 10959, 10590, 7122, 7102, 5553, 511, 4731, 4152, 10756, 10628, 11153, 6940, 2]
// Exports: default

// Module 10591 (PremiumGiftModal)
...
export default function PremiumGiftModal(analyticsLocations) {
  ...
  if (initialRoute != null) {
    ...
    if (initialRoute === obj.REWARD_SELECT) {
      tmp11Result = tmp11(5579);
      let headerCloseButton = tmp11Result.getHeaderCloseButton(callback);
    } else {
      headerCloseButton = tmp11(5579).getHeaderBackButton();
      const tmp11Result1 = tmp11(5579);
    }
    ...
    if (initialRoute === obj.CUSTOMIZATION) {
      let headerCloseButton1 = tmp11(5579).getHeaderCloseButton(callback);
      const tmp11Result2 = tmp11(5579);
    } else {
      headerCloseButton1 = tmp11(5579).getHeaderBackButton();
      const tmp11Result3 = tmp11(5579);
    }
    ...
    obj5[1] = recipientUserId(5579).getHeaderCloseButton(callback);
    obj5[2] = tmp10.header;
    ...
    const tmp11Result4 = recipientUserId(5579);
    obj6[1] = recipientUserId(5579).getHeaderCloseButton(callback);
    ...
    obj6[4] = function render(currentProgress) {
      return callback2(_undefined(_undefined2[16]), {
        ...
        onSendGift(arg0) {
          ...
          const items = [callback2(7122).GIFTING_BADGE_POST_PURCHASE];
          obj[0] = items;
    ...
    closure_4 = tmp22;
    const tmp11Result5 = recipientUserId(5579);
    if (tmp11Result6.isPremiumGiftingSupported()) {
      ...
      obj9[1] = tmp23(tmp11(6940).Navigator, obj10);
      obj8[7] = tmp23(tmpResult, obj9);
      obj7[1] = tmp23(tmp11(10628).NativeGiftContextProvider, obj8);
      let tmp23Result = tmp23(tmp11(7102).AnalyticsLocationProvider, obj7);
    } else {
