// Module ID: 9959
// Function ID: 9960
// Dependencies: [32, 19, 17, 7265, 1905, 505, 21, 712, 4285, 5219, 589, 7568, 9960, 8548, 9961, 9531, 9962, 5251, 8214, 9963, 8552, 8550, 4281, 1236, 8557, 4695, 2]
...
let closure_17 = createCacheKey.createStyles(() => {
  ...
  const merged1 = Object.assign(importDefault(5219)(Fonts.PRIMARY_NORMAL, importDefault(712).colors.WHITE, 16));
  obj9.marginStart = -8;
  obj[12] = obj9;
...
const memoResult = importAllResult.memo(function PremiumGiftFeaturesCard(variant) {
  ...
  const themeAndReducedMotionAwareAssetUrl1 = _require(9960).useThemeAndReducedMotionAwareAssetUrl(backgroundAsset);
  let tmp9 = null != claimableRewards;
  if (tmp9) {
    tmp9 = 1 === claimableRewards.length;
  }
  const tmp10 = callback3(tmp9);
  const tmp2Result = _require(9960);
  let stringResult1 = importDefault(9961)(premiumType, constants.MONTH);
  const tmp12 = importDefault(8548)(premiumType);
  let obj3 = importDefault(9531);
  const config = obj3.useConfig({ location: "PremiumGiftFeaturesCard" });
  ({ showGiftPrice, useNewCopy } = config);
  let promotionalButton = importDefault(9962)(premiumType) && null != claimableRewards;
  ...
  const tmp14 = importDefault(9961)(premiumType, constants.YEAR);
