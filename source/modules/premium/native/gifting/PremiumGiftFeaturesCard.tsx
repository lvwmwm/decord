// Module ID: 9928
// Function ID: 9929
// Dependencies: [32, 19, 17, 7238, 1876, 505, 21, 712, 4255, 5189, 589, 7540, 9929, 8407, 9930, 9502, 9931, 5221, 8073, 9932, 8523, 8521, 4251, 1236, 8528, 4665, 2]
...
let closure_17 = createCacheKey.createStyles(() => {
  ...
  const merged1 = Object.assign(importDefault(5189)(Fonts.PRIMARY_NORMAL, importDefault(712).colors.WHITE, 16));
  obj9.marginStart = -8;
  obj[12] = obj9;
...
const memoResult = importAllResult.memo(function PremiumGiftFeaturesCard(variant) {
  ...
  const themeAndReducedMotionAwareAssetUrl1 = _require(9929).useThemeAndReducedMotionAwareAssetUrl(backgroundAsset);
  let tmp9 = null != claimableRewards;
  if (tmp9) {
    tmp9 = 1 === claimableRewards.length;
  }
  const tmp10 = callback3(tmp9);
  const tmp2Result = _require(9929);
  let stringResult1 = importDefault(9930)(premiumType, constants.MONTH);
  const tmp12 = importDefault(8407)(premiumType);
  let obj3 = importDefault(9502);
  const config = obj3.useConfig({ location: "PremiumGiftFeaturesCard" });
  ({ showGiftPrice, useNewCopy } = config);
  let promotionalButton = importDefault(9931)(premiumType) && null != claimableRewards;
  ...
  const tmp14 = importDefault(9930)(premiumType, constants.YEAR);
