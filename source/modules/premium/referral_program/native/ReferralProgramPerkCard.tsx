...
// Dependencies: [19, 17, 6855, 676, 21, 4302, 712, 1297, 4298, 12648, 1236, 1974, 589, 5668, 5688, 6856, 698, 4270, 12649, 1988, 12656, 4712, 2]
// Exports: ReferralProgramPerkCard

// Module 12647 (ReferredFriendAvatar)
...
function AvailableReferralSlot(children) {
  return callback(View, { style: createCacheKey().availableReferralSlot, children: callback(require(4298) /* Text */.Text, { variant: "text-xs/medium", color: "text-strong", children: children.slotIndex }) });
}
function ProgressIndicator(referralSentUsers) {
  referralSentUsers = referralSentUsers.referralSentUsers;
  const items = [];
  let num = 0;
  if (0 < require(12648) /* MAX_REFERRALS_SENT */.MAX_REFERRALS_SENT) {
    do {
      let tmp2 = num;
    ...
    } while (num < require(12648) /* MAX_REFERRALS_SENT */.MAX_REFERRALS_SENT);
  }
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  analyticsLocations = importDefault(5668)(importDefault(5688).PREMIUM_MARKETING_REFERALL_PROGRAM_PROGRESS_BAR).analyticsLocations;
