...
// Dependencies: [31, 27, 6192, 653, 33, 4165, 689, 1273, 4161, 12452, 1212, 1921, 566, 5497, 5517, 6193, 675, 4133, 12453, 1935, 12460, 4578, 2]
// Exports: ReferralProgramPerkCard

// Module 12451 (ReferredFriendAvatar)
...
function AvailableReferralSlot(slotIndex) {
  obj = { style: _createForOfIteratorHelperLoose().availableReferralSlot, children: callback(require(4161) /* Text */.Text, obj) };
...
function ProgressIndicator(referralSentUsers) {
  ...
  const items = [];
  let num = 0;
  if (0 < require(12452) /* MAX_REFERRALS_SENT */.MAX_REFERRALS_SENT) {
    do {
    ...
    } while (num < require(12452) /* MAX_REFERRALS_SENT */.MAX_REFERRALS_SENT);
  }
...
function getBodyText(arg0, arg1, arg2) {
  if (arg0) {
    if (arg1 !== tmp(12452).MAX_REFERRALS_SENT) {
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  let obj = analyticsLocations(12452);
  ...
  analyticsLocations = importDefault(5497)(importDefault(5517).PREMIUM_MARKETING_REFERALL_PROGRAM_PROGRESS_BAR).analyticsLocations;
  let everyResult = stateFromStores.size === analyticsLocations(12452).MAX_REFERRALS_SENT;
  if (everyResult) {
    const items2 = [];
  ...
  const items4 = [callback(importDefault(12460), obj), callback(ProgressIndicator, { referralSentUsers }), , ];
  ...
  const items5 = [callback(analyticsLocations(4161).Text, obj2), ];
  ...
  items5[1] = callback(analyticsLocations(4161).Text, obj3);
  ...
  obj4.children = callback(analyticsLocations(4578).Button, obj5);
