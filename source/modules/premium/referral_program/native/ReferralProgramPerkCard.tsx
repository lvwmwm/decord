...
// Dependencies: [19, 17, 6984, 676, 21, 4342, 712, 1297, 4338, 12802, 1236, 1993, 589, 5789, 5809, 6985, 698, 4310, 12803, 2007, 12810, 4777, 2]
// Exports: ReferralProgramPerkCard

// Module 12801 (ReferredFriendAvatar)
...
function AvailableReferralSlot(children) {
  return callback(View, { style: createCacheKey().availableReferralSlot, children: callback(require(4338) /* Text */.Text, { variant: "text-xs/medium", color: "text-strong", children: children.slotIndex }) });
}
function ProgressIndicator(referralSentUsers) {
  referralSentUsers = referralSentUsers.referralSentUsers;
  const items = [];
  let num = 0;
  if (0 < require(12802) /* MAX_REFERRALS_SENT */.MAX_REFERRALS_SENT) {
    do {
      let tmp2 = num;
    ...
    } while (num < require(12802) /* MAX_REFERRALS_SENT */.MAX_REFERRALS_SENT);
  }
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  analyticsLocations = importDefault(5789)(importDefault(5809).PREMIUM_MARKETING_REFERALL_PROGRAM_PROGRESS_BAR).analyticsLocations;
