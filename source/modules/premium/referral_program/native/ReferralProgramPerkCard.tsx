...
// Dependencies: [19, 17, 6918, 676, 21, 4303, 712, 1297, 4299, 12724, 1236, 1993, 589, 5728, 5748, 6919, 698, 4271, 12725, 2007, 12732, 4714, 2]
// Exports: ReferralProgramPerkCard

// Module 12723 (ReferredFriendAvatar)
...
function AvailableReferralSlot(children) {
  return callback(View, { style: createCacheKey().availableReferralSlot, children: callback(require(4299) /* Text */.Text, { variant: "text-xs/medium", color: "text-strong", children: children.slotIndex }) });
}
function ProgressIndicator(referralSentUsers) {
  referralSentUsers = referralSentUsers.referralSentUsers;
  const items = [];
  let num = 0;
  if (0 < require(12724) /* MAX_REFERRALS_SENT */.MAX_REFERRALS_SENT) {
    do {
      let tmp2 = num;
    ...
    } while (num < require(12724) /* MAX_REFERRALS_SENT */.MAX_REFERRALS_SENT);
  }
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  analyticsLocations = importDefault(5728)(importDefault(5748).PREMIUM_MARKETING_REFERALL_PROGRAM_PROGRESS_BAR).analyticsLocations;
