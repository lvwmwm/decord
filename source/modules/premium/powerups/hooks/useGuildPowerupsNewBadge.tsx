...
// Dependencies: [32, 19, 4231, 4232, 1388, 1377, 589, 11924, 6063, 1379, 2]
// Exports: default, useAutoDismissGuildPowerupsNewBadge

// Module 11931 (useGuildPowerupsNewBadge)
...
export default function useGuildPowerupsNewBadge(arg0) {
  ...
  const tmp4Result = importDefault(11924)(arg0, stateFromStores);
  let num = 0;
  if (tmp4Result >= constants.GUILD_THEME) {
    num = tmp4Result;
  }
  const tmp = _require;
  const tmp4 = importDefault(11924);
  let tmp6 = null;
  if (num > 0) {
    tmp6 = null;
    if (!flag) {
  ...
  const tmp7 = callback(_require(6063).useSelectedVersionedDismissibleContent(tmp6, num), 2);
  _require = tmp8;
  importDefault = tmp9;
