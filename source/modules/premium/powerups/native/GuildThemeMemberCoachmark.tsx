...
// Dependencies: [19, 4438, 4332, 4333, 1388, 21, 4446, 712, 589, 4336, 12018, 15689, 4352, 5342, 1236, 2402, 12021, 8913, 2]
// Exports: default

// Module 15688 (GuildThemeMemberCoachmark)
...
import { GUILD_THEME_POWERUP_BOOST_PRICE as closure_6 } from "BoostedGuildTiers" /* 4333 */;
...
export default function GuildThemeMemberCoachmark(guildId) {
  ...
  const stateFromStores = guildId(589).useStateFromStores(items, () => {
    ...
    if (stateForGuild != null) {
      tmp2 = stateForGuild.allPowerups[guildId(undefined, 4336).GUILD_POWERUP_GUILD_THEME_SKU_ID];
    }
    return tmp2;
  }, items1);
  let obj = guildId(589);
  ...
  guildPowerupBannerImage = guildId(12018).getGuildPowerupBannerImage(stateFromStores, stateFromStores1, true);
  if (guildPowerupBannerImage == null) {
    guildPowerupBannerImage = markAsDismissed(15689);
  }
  const diff = callback - markAsDismissed(4352)(guildId).available;
  c5 = diff;
  ...
  callback1 = stateFromStores1.useCallback(() => {
    ...
    if (c5 > 0) {
      guildId(5342).openApplyBoostModal(guildId);
      const obj = guildId(5342);
    }
  }, items4);
  ...
  const memo = stateFromStores1.useMemo(() => {
    ...
    obj[0] = intl.string(markAsDismissed(2402).RK6NbY);
    const intl2 = guildId(1236).intl;
    obj[1] = intl2.string(markAsDismissed(2402).xlAqGk);
    obj[5] = callback;
  ...
  const obj3 = guildId(12018);
  const coachmark = tmp2(8913).useCoachmark(guildId.targetRef, memo);
  return null;
};
