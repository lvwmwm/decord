...
// Dependencies: [19, 17, 10201, 21, 12059, 7708, 712, 1297, 4478, 15761, 15602, 5942, 12055, 12074, 11948, 12043, 5973, 5940, 1236, 2401, 15807, 11858, 2]
// Exports: default

// Module 15806 (GuildPowerupsChannelRowIndicator)
import ThemesDefault from "Themes" /* 712 */;
import add from "add" /* 5942 */;
import GUILD_POWERUP_NOTIFICATION_COOLDOWN from "GUILD_POWERUP_NOTIFICATION_COOLDOWN" /* 12059 */;
import SidebarCoachmarkOverlayLayer from "SidebarCoachmarkOverlayLayer" /* 15602 */;
import useGuildPowerupsCoachmarkDefault from "useGuildPowerupsCoachmark" /* 15761 */;
...
function GuildPowerupsChannelRowIndicator(indicator) {
  ...
  if (null == indicator) {
    ...
    if (GUILD_POWERUP_NOTIFICATION_COOLDOWN.GuildPowerupNotificationIndicatorType.WARNING === type) {
      ...
      return callback(tmp4(7708).CircleErrorIcon, obj);
    } else if (tmp4(12059).GuildPowerupNotificationIndicatorType.UNREAD === type) {
...
export default function GuildPowerupsChannelRow(guildId) {
  ...
  const tmp5 = dismissNewBadgeIfShown(12055)(guildId);
  let indicator;
  const tmp = callback2();
  if (tmp5 != null) {
  ...
  const tmp6Result = dismissNewBadgeIfShown(12074)(guildId, tmp8);
  ...
  const tmp6 = dismissNewBadgeIfShown(12074);
  ...
  obj1[8] = closure_5(tmp14(11948).BaseChannelIcon, { mode: DEFAULT, IconComponent: tmp14(15807).BoostTier2Icon });
