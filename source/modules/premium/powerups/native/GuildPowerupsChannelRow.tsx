...
// Dependencies: [19, 17, 10162, 21, 12026, 7676, 712, 1297, 4448, 15725, 15567, 5910, 12022, 12041, 11915, 12010, 5941, 5908, 1236, 2401, 15771, 11825, 2]
// Exports: default

// Module 15770 (GuildPowerupsChannelRowIndicator)
import ThemesDefault from "Themes" /* 712 */;
import add from "add" /* 5910 */;
import GUILD_POWERUP_NOTIFICATION_COOLDOWN from "GUILD_POWERUP_NOTIFICATION_COOLDOWN" /* 12026 */;
import SidebarCoachmarkOverlayLayer from "SidebarCoachmarkOverlayLayer" /* 15567 */;
import useGuildPowerupsCoachmarkDefault from "useGuildPowerupsCoachmark" /* 15725 */;
...
function GuildPowerupsChannelRowIndicator(indicator) {
  ...
  if (null == indicator) {
    ...
    if (GUILD_POWERUP_NOTIFICATION_COOLDOWN.GuildPowerupNotificationIndicatorType.WARNING === type) {
      ...
      return callback(tmp4(7676).CircleErrorIcon, obj);
    } else if (tmp4(12026).GuildPowerupNotificationIndicatorType.UNREAD === type) {
...
export default function GuildPowerupsChannelRow(guildId) {
  ...
  const tmp5 = dismissNewBadgeIfShown(12022)(guildId);
  let indicator;
  const tmp = callback2();
  if (tmp5 != null) {
  ...
  const tmp6Result = dismissNewBadgeIfShown(12041)(guildId, tmp8);
  dismissNewBadgeIfShown = tmp6Result.dismissNewBadgeIfShown;
  let showUnread;
  if (tmp5 != null) {
    showUnread = tmp5.showUnread;
  }
  const ChannelModes = guildId(11915).ChannelModes;
  if (true === showUnread) {
  ...
  const tmp6 = dismissNewBadgeIfShown(12041);
  ...
  obj2[1] = DEFAULT;
  obj1[7] = closure_5(tmp14(11915).BaseChannelName, obj2);
  const tmp3Result = dismissNewBadgeIfShown(11915);
  obj1[8] = closure_5(tmp14(11915).BaseChannelIcon, { mode: DEFAULT, IconComponent: tmp14(15771).BoostTier2Icon });
  if (tmp6Result.showNewBadgeOnRow) {
    let tmp16Result = tmp16(tmp14(11825).NewBadge, {});
  } else {
    let indicator1;
    if (tmp5 != null) {
  ...
  return closure_5(tmp14(5908).LayerScope, obj5);
};
