...
// Dependencies: [19, 17, 9841, 21, 11916, 7834, 712, 1297, 4344, 15369, 15214, 5756, 11912, 11931, 11813, 11900, 5787, 5754, 1236, 2335, 15413, 11723, 2]
// Exports: default

// Module 15412 (GuildPowerupsChannelRowIndicator)
...
function GuildPowerupsChannelRowIndicator(indicator) {
  ...
  if (null == indicator) {
    ...
    if (require(11916) /* GUILD_POWERUP_NOTIFICATION_COOLDOWN */.GuildPowerupNotificationIndicatorType.WARNING === type) {
      let obj = { color: null, size: "sm" };
      obj[0] = importDefault(712).colors.STATUS_WARNING;
      return callback(tmp4(7834).CircleErrorIcon, obj);
    } else if (tmp4(11916).GuildPowerupNotificationIndicatorType.UNREAD === type) {
...
function GuildPowerupsCoachmarkHost(arg0) {
  ...
  importDefault(15369)(targetRef, guildId, popout);
  return null;
}
function GuildPowerupsCoachmark(arg0) {
  ...
  let targetRef;
  ({ targetRef, guildId, popout } = arg0);
  const context = React.useContext(require(15214) /* SidebarCoachmarkOverlayLayer */.SidebarCoachmarkOverlayContext);
  ...
  if (null != context) {
    ...
    obj[0] = context;
    obj[1] = tmp5;
    tmp4Result = callback(require(5756) /* add */.LayerContext.Provider, obj);
  }
  return tmp4Result;
}
...
export default function GuildPowerupsChannelRow(guildId) {
  ...
  const tmp5 = dismissNewBadgeIfShown(11912)(guildId);
  let indicator;
  ...
  const tmp6Result = dismissNewBadgeIfShown(11931)(guildId, tmp8);
  dismissNewBadgeIfShown = tmp6Result.dismissNewBadgeIfShown;
  let showUnread;
  if (tmp5 != null) {
    showUnread = tmp5.showUnread;
  }
  const ChannelModes = guildId(11813).ChannelModes;
  if (true === showUnread) {
  ...
  const tmp6 = dismissNewBadgeIfShown(11931);
  ...
  obj2[1] = DEFAULT;
  obj1[7] = closure_5(tmp14(11813).BaseChannelName, obj2);
  const obj3 = { mode: DEFAULT, IconComponent: null };
  obj3[1] = tmp14(15413).BoostTier2Icon;
  obj1[8] = closure_5(tmp14(11813).BaseChannelIcon, obj3);
  if (tmp6Result.showNewBadgeOnRow) {
    let tmp16Result = tmp16(tmp14(11723).NewBadge, {});
  } else {
    let indicator1;
    if (tmp5 != null) {
  ...
  obj1[9] = tmp16Result;
  items1[1] = closure_5(dismissNewBadgeIfShown(11813), obj1);
  obj[2] = items1;
  obj5[1] = tmp17(tmp18, obj);
  return closure_5(tmp14(5754).LayerScope, obj5);
};
