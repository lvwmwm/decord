...
// Dependencies: [19, 17, 9800, 21, 11859, 7795, 712, 1297, 4303, 15312, 15157, 5717, 11855, 11874, 11756, 11843, 5748, 5715, 1236, 2335, 15356, 11666, 2]
// Exports: default

// Module 15355 (GuildPowerupsChannelRowIndicator)
...
function GuildPowerupsChannelRowIndicator(indicator) {
  ...
  if (null == indicator) {
    ...
    if (require(11859) /* GUILD_POWERUP_NOTIFICATION_COOLDOWN */.GuildPowerupNotificationIndicatorType.WARNING === type) {
      let obj = { color: null, size: "sm" };
      obj[0] = importDefault(712).colors.STATUS_WARNING;
      return callback(tmp4(7795).CircleErrorIcon, obj);
    } else if (tmp4(11859).GuildPowerupNotificationIndicatorType.UNREAD === type) {
...
function GuildPowerupsCoachmarkHost(arg0) {
  ...
  importDefault(15312)(targetRef, guildId, popout);
  return null;
}
function GuildPowerupsCoachmark(arg0) {
  ...
  let targetRef;
  ({ targetRef, guildId, popout } = arg0);
  const context = React.useContext(require(15157) /* SidebarCoachmarkOverlayLayer */.SidebarCoachmarkOverlayContext);
...
export default function GuildPowerupsChannelRow(guildId) {
  ...
  const tmp5 = dismissNewBadgeIfShown(11855)(guildId);
  let indicator;
  ...
  const tmp6Result = dismissNewBadgeIfShown(11874)(guildId, tmp8);
  dismissNewBadgeIfShown = tmp6Result.dismissNewBadgeIfShown;
  let showUnread;
  if (tmp5 != null) {
    showUnread = tmp5.showUnread;
  }
  const ChannelModes = guildId(11756).ChannelModes;
  if (true === showUnread) {
  ...
  const tmp6 = dismissNewBadgeIfShown(11874);
  ...
  obj2[1] = DEFAULT;
  obj1[7] = closure_5(tmp14(11756).BaseChannelName, obj2);
  const obj3 = { mode: DEFAULT, IconComponent: null };
  obj3[1] = tmp14(15356).BoostTier2Icon;
  obj1[8] = closure_5(tmp14(11756).BaseChannelIcon, obj3);
  if (tmp6Result.showNewBadgeOnRow) {
    let tmp16Result = tmp16(tmp14(11666).NewBadge, {});
  } else {
    let indicator1;
    if (tmp5 != null) {
  ...
  obj1[9] = tmp16Result;
  items1[1] = closure_5(dismissNewBadgeIfShown(11756), obj1);
  obj[2] = items1;
