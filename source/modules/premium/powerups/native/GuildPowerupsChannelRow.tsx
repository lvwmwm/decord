...
// Dependencies: [19, 17, 9794, 21, 11855, 7793, 712, 1297, 4303, 15293, 15146, 5717, 11851, 11870, 11752, 11839, 5748, 5715, 1236, 2316, 15334, 11662, 2]
// Exports: default

// Module 15333 (GuildPowerupsChannelRowIndicator)
...
function GuildPowerupsChannelRowIndicator(indicator) {
  ...
  if (null == indicator) {
    ...
    if (require(11855) /* GUILD_POWERUP_NOTIFICATION_COOLDOWN */.GuildPowerupNotificationIndicatorType.WARNING === type) {
      let obj = { color: null, size: "sm" };
      obj[0] = importDefault(712).colors.STATUS_WARNING;
      return callback(tmp4(7793).CircleErrorIcon, obj);
    } else if (tmp4(11855).GuildPowerupNotificationIndicatorType.UNREAD === type) {
...
function GuildPowerupsCoachmarkHost(arg0) {
  ...
  importDefault(15293)(targetRef, guildId, popout);
  return null;
}
function GuildPowerupsCoachmark(arg0) {
  ...
  let targetRef;
  ({ targetRef, guildId, popout } = arg0);
  const context = React.useContext(require(15146) /* SidebarCoachmarkOverlayLayer */.SidebarCoachmarkOverlayContext);
  ...
  if (null != context) {
    ...
    obj[0] = context;
    obj[1] = tmp5;
    tmp4Result = callback(require(5717) /* add */.LayerContext.Provider, obj);
  }
  return tmp4Result;
}
...
export default function GuildPowerupsChannelRow(guildId) {
  ...
  const tmp5 = dismissNewBadgeIfShown(11851)(guildId);
  let indicator;
  ...
  const tmp6Result = dismissNewBadgeIfShown(11870)(guildId, tmp8);
  dismissNewBadgeIfShown = tmp6Result.dismissNewBadgeIfShown;
  let showUnread;
  if (tmp5 != null) {
    showUnread = tmp5.showUnread;
  }
  const ChannelModes = guildId(11752).ChannelModes;
  if (true === showUnread) {
  ...
  const tmp6 = dismissNewBadgeIfShown(11870);
  ...
  obj2[1] = DEFAULT;
  obj1[7] = closure_5(tmp14(11752).BaseChannelName, obj2);
  const obj3 = { mode: DEFAULT, IconComponent: null };
  obj3[1] = tmp14(15334).BoostTier2Icon;
  obj1[8] = closure_5(tmp14(11752).BaseChannelIcon, obj3);
  if (tmp6Result.showNewBadgeOnRow) {
    let tmp16Result = tmp16(tmp14(11662).NewBadge, {});
  } else {
    let indicator1;
    if (tmp5 != null) {
  ...
  obj1[9] = tmp16Result;
  items1[1] = closure_5(dismissNewBadgeIfShown(11752), obj1);
  obj[2] = items1;
  obj5[1] = tmp17(tmp18, obj);
  return closure_5(tmp14(5715).LayerScope, obj5);
};
