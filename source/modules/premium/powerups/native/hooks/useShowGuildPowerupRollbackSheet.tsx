...
// Dependencies: [57, 31, 1345, 11580, 8417, 11582, 4133, 2]
// Exports: default

// Module 11579 (useShowGuildPowerupRollbackSheet)
...
export default function useShowGuildPowerupRollbackSheet(arg0, arg1) {
  ...
  let dependencyMap;
  let callback;
  ({ shouldShow, modalConfig } = first(11580)(arg0, arg1));
  const items = [];
  if (shouldShow) {
  ...
  const tmp = first(11580)(arg0, arg1);
  const tmp4 = callback(modalConfig(8417).useSelectedDismissibleContent(items), 2);
  first = tmp4[0];
  dependencyMap = tmp6;
  ...
  const effect = React.useEffect(() => {
    ...
    if (!current) {
      ...
      obj.onCtaPress = function onCtaPress() {
        ...
        first(4133).hideActionSheet(modalConfig(11582).GUILD_POWERUP_ROLLBACK_SHEET_KEY);
      };
