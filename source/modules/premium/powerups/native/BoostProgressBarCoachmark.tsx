...
// Dependencies: [19, 17, 1388, 21, 4446, 9667, 1236, 2402, 4173, 8913, 2]
// Exports: default

// Module 15687 (BoostProgressBarCoachmark)
...
export default function BoostProgressBarCoachmark(guild) {
  ...
  callback1 = callback.useCallback(() => {
    ...
    markAsDismissed(9667).saveGuild(guild.id, { premiumProgressBarEnabled: true });
  }, items1);
  ...
  const memo = callback.useMemo(() => {
    ...
    obj[0] = intl.string(markAsDismissed(2402).uwV2dH);
    const intl2 = guild(1236).intl;
    obj[1] = intl2.string(markAsDismissed(2402).MIwlcR);
    obj[5] = callback;
  ...
  const coachmark = guild(8913).useCoachmark(guild.targetRef, memo);
  return null;
};
