...
// Dependencies: [17, 4334, 4230, 4231, 21, 4342, 712, 11939, 11936, 11959, 11960, 589, 11961, 4072, 11963, 4338, 11964, 11966, 11967, 4298, 1236, 2335, 11953, 11971, 4233, 9087, 11972, 11973, 11977, 11982, 11983, 4777, 11981, 5458, 2]
// Exports: default

// Module 11958 (GuildPowerupsBottomSheetHeader)
...
function GuildPowerupsBottomSheetHeader(arg0) {
  ...
  const tmp = callback3();
  const tmp4 = importDefault(11939)(guildId, powerup);
  let obj = require(11959) /* useCalculatePowerupCardStatus */;
  const calculatePowerupCardStatus = obj.useCalculatePowerupCardStatus(powerup, tmp4, importDefault(11936)(guildId, powerup, "GuildPowerupsBottomSheet"));
  let str = importDefault(11960)(powerup, true);
  if (str == null) {
    str = "";
  }
  const tmp5 = importDefault(11936)(guildId, powerup, "GuildPowerupsBottomSheet");
  ...
  if (powerup.type === constants.LEVEL) {
    ...
    let tmp15 = callback(tmp2(11961), obj);
    let tmp14 = callback;
    ...
    obj1[1] = obj2;
    obj[1] = callback(tmp6(4072).GameServerHostingRive, obj1);
    ...
    obj3[1] = tmp.image;
    tmp14 = callback;
    tmp15 = callback(tmp2(11963), obj3);
  }
  const obj4 = { children: null };
  const items1 = [tmp15, ];
  const obj5 = { style: tmp.headerContainer, children: null };
  const items2 = [tmp14(require(4338) /* Text */.Text, { variant: "heading-xl/bold", accessibilityRole: "header", children: powerup.title }), tmp14(require(11964) /* GuildPowerupCardFooterActive */.GuildPowerupsCardFooter, { cost: powerup.cost, costDecorator: str2, status: calculatePowerupCardStatus, style: tmp.statusContainer })];
  obj5[1] = items2;
...
function GuildPowerupsBottomSheetLevelBody(powerup) {
  ...
  const tmp = callback3();
  c0 = tmp;
  const arr = importDefault(11966)(powerup.powerup);
  return callback(View, {
    style: tmp.levelContainer,
    children: importDefault(11966)(powerup.powerup).map((children) => {
...
function GuildPowerupsBottomSheetBody(powerup) {
  ...
  if (constants.PERK === type) {
    ...
    if (tmp5Result) {
      ...
      const items1 = [tmp7(tmp8(4298).CircleInformationIcon, obj1), ];
      ...
      items1[1] = tmp7(tmp8(4338).Text, obj2);
      obj[1] = items1;
...
function GuildPowerupsBottomSheetFooter(arg0) {
  ...
  let dependencyMap;
  const tmp = callback3();
  const tmp4 = importDefault(11953)(guildId);
  ({ showToggleButton, showConfigureButton, isPowerupActive } = importDefault(11971)(guildId, powerup));
  if (showConfigureButton) {
    let result = powerup.skuId !== isPowerupActive(4233).GUILD_POWERUP_TAG_SKU_ID;
    if (!result) {
      result = tmp6(9087).canUseMobileServerTagSettings(guildId);
      const tmp6Result = tmp6(9087);
    }
    showConfigureButton = result;
    tmp6 = isPowerupActive;
  }
  const tmp5 = importDefault(11971)(guildId, powerup);
  ({ disabled, reason } = importDefault(11972)(guildId, powerup, isPowerupActive));
  const tmp8 = importDefault(11972)(guildId, powerup, isPowerupActive);
  ({ onActivate: c1, isLoading } = importDefault(11973)(guildId, powerup));
  dependencyMap = tmp2(11977)(guildId, powerup);
  if (tmp4) {
    ...
    if (tmp14) {
      ...
      tmp14 = callback(isPowerupActive(4338).Text, obj);
    }
    const items = [tmp14, , , ];
    let tmp21 = disabled;
    if (disabled) {
    ...
    if (tmp21) {
      ...
      obj1[0] = reason;
      tmp21 = callback(tmp2(11983), obj1);
    }
    items[1] = tmp21;
    if (showConfigureButton) {
      ...
      obj2[2] = tmp10;
      showConfigureButton = callback(isPowerupActive(4777).Button, obj2);
    }
    ...
    if (!showToggleButton) {
      ...
      callback(isPowerupActive(4777).Button, obj3);
...
export default function GuildPowerupsBottomSheet(arg0) {
  ...
  let powerup;
  ({ guildId, powerup } = arg0);
  let obj = require(11981) /* ModalType */;
  const logPowerupModalOpened = obj.useLogPowerupModalOpened(guildId, powerup, require(11981) /* ModalType */.ModalType.DETAIL);
