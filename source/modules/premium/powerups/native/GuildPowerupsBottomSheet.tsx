...
// Dependencies: [17, 4247, 4143, 4144, 21, 4255, 712, 11711, 11708, 11731, 11732, 589, 11733, 3967, 11735, 4251, 11736, 11738, 11739, 4211, 1236, 2287, 11725, 11743, 4146, 8616, 11744, 11745, 11749, 11754, 11755, 4666, 11753, 5309, 2]
// Exports: default

// Module 11730 (GuildPowerupsBottomSheetHeader)
...
function GuildPowerupsBottomSheetHeader(arg0) {
  ...
  const calculatePowerupCardStatus = obj.useCalculatePowerupCardStatus(powerup, tmp4, importDefault(11708)(guildId, powerup, "GuildPowerupsBottomSheet"));
  let str = importDefault(11732)(powerup, true);
  if (str == null) {
    str = "";
  }
  const tmp5 = importDefault(11708)(guildId, powerup, "GuildPowerupsBottomSheet");
  ...
  if (powerup.type === constants.LEVEL) {
    ...
    let tmp15 = callback(tmp2(11733), obj);
    let tmp14 = callback;
    ...
    obj3[1] = tmp.image;
    tmp14 = callback;
    tmp15 = callback(tmp2(11735), obj3);
  }
  const obj4 = { children: null };
  const items1 = [tmp15, ];
  const obj5 = { style: tmp.headerContainer, children: null };
  const items2 = [tmp14(require(4251) /* Text */.Text, { variant: "heading-xl/bold", accessibilityRole: "header", children: powerup.title }), tmp14(require(11736) /* GuildPowerupCardFooterActive */.GuildPowerupsCardFooter, { cost: powerup.cost, costDecorator: str2, status: calculatePowerupCardStatus, style: tmp.statusContainer })];
  obj5[1] = items2;
...
function GuildPowerupsBottomSheetLevelBody(powerup) {
  ...
  const tmp = callback3();
  c0 = tmp;
  const arr = importDefault(11738)(powerup.powerup);
  return callback(View, {
    style: tmp.levelContainer,
    children: importDefault(11738)(powerup.powerup).map((children) => {
...
function GuildPowerupsBottomSheetFooter(arg0) {
  ...
  ({ showToggleButton, showConfigureButton, isPowerupActive } = importDefault(11743)(guildId, powerup));
  if (showConfigureButton) {
    let result = powerup.skuId !== isPowerupActive(4146).GUILD_POWERUP_TAG_SKU_ID;
    if (!result) {
      result = tmp6(8616).canUseMobileServerTagSettings(guildId);
      const tmp6Result = tmp6(8616);
    }
    showConfigureButton = result;
    tmp6 = isPowerupActive;
  }
  const tmp5 = importDefault(11743)(guildId, powerup);
  ({ disabled, reason } = importDefault(11744)(guildId, powerup, isPowerupActive));
  const tmp8 = importDefault(11744)(guildId, powerup, isPowerupActive);
  ({ onActivate: c1, isLoading } = importDefault(11745)(guildId, powerup));
  dependencyMap = tmp2(11749)(guildId, powerup);
  if (tmp4) {
    ...
    if (tmp21) {
      ...
      obj1[0] = reason;
      tmp21 = callback(tmp2(11755), obj1);
    }
    items[1] = tmp21;
    if (showConfigureButton) {
      ...
      obj2[2] = tmp10;
      showConfigureButton = callback(isPowerupActive(4666).Button, obj2);
    }
    ...
    if (!showToggleButton) {
      ...
      callback(isPowerupActive(4666).Button, obj3);
...
export default function GuildPowerupsBottomSheet(arg0) {
  ...
  let powerup;
  ({ guildId, powerup } = arg0);
  let obj = require(11753) /* ModalType */;
  const logPowerupModalOpened = obj.useLogPowerupModalOpened(guildId, powerup, require(11753) /* ModalType */.ModalType.DETAIL);
