...
// Dependencies: [17, 4662, 4262, 4263, 21, 4661, 712, 11668, 11665, 11688, 11689, 589, 11690, 4104, 11692, 4734, 11693, 11695, 11696, 4330, 1236, 2367, 11682, 11702, 4265, 8877, 11703, 11704, 11708, 11713, 11714, 4745, 11712, 6950, 2]
// Exports: default

// Module 11687 (GuildPowerupsBottomSheetHeader)
...
function GuildPowerupsBottomSheetHeader(arg0) {
  ...
  const tmp = callback3();
  const tmp4 = importDefault(11668)(guildId, powerup);
  let obj = require(11688) /* useCalculatePowerupCardStatus */;
  const calculatePowerupCardStatus = obj.useCalculatePowerupCardStatus(powerup, tmp4, importDefault(11665)(guildId, powerup, "GuildPowerupsBottomSheet"));
  let str = importDefault(11689)(powerup, true);
  if (str == null) {
    str = "";
  }
  const tmp5 = importDefault(11665)(guildId, powerup, "GuildPowerupsBottomSheet");
  ...
  const items2 = [tmp14(require(4734) /* Text */.Text, { variant: "heading-xl/bold", accessibilityRole: "header", children: powerup.title }), tmp14(require(11693) /* GuildPowerupCardFooterActive */.GuildPowerupsCardFooter, { cost: powerup.cost, costDecorator: str2, status: calculatePowerupCardStatus, style: tmp.statusContainer })];
  obj5[1] = items2;
...
function GuildPowerupsBottomSheetLevelBody(powerup) {
  ...
  const tmp = callback3();
  c0 = tmp;
  const arr = importDefault(11695)(powerup.powerup);
  return callback(View, {
    style: tmp.levelContainer,
    children: importDefault(11695)(powerup.powerup).map((children) => {
...
function GuildPowerupsBottomSheetBody(powerup) {
  ...
  if (constants.PERK === type) {
    ...
    if (tmp5Result) {
      ...
      obj2[2] = intl.formatToPlainString(importDefault(2367).GMhQcE, obj3);
...
function GuildPowerupsBottomSheetFooter(arg0) {
  ...
  let dependencyMap;
  const tmp = callback3();
  const tmp4 = importDefault(11682)(guildId);
  ({ showToggleButton, showConfigureButton, isPowerupActive } = importDefault(11702)(guildId, powerup));
  if (showConfigureButton) {
    let result = powerup.skuId !== isPowerupActive(4265).GUILD_POWERUP_TAG_SKU_ID;
    if (!result) {
      result = tmp6(8877).canUseMobileServerTagSettings(guildId);
      const tmp6Result = tmp6(8877);
    }
    showConfigureButton = result;
    tmp6 = isPowerupActive;
  }
  const tmp5 = importDefault(11702)(guildId, powerup);
  ({ disabled, reason } = importDefault(11703)(guildId, powerup, isPowerupActive));
  const tmp8 = importDefault(11703)(guildId, powerup, isPowerupActive);
  ({ onActivate: c1, isLoading } = importDefault(11704)(guildId, powerup));
  ...
  if (tmp4) {
    ...
    if (tmp14) {
      ...
      obj[2] = intl.string(tmp2(2367)["jo5++h"]);
      tmp14 = callback(isPowerupActive(4734).Text, obj);
    }
    const items = [tmp14, , , ];
    let tmp21 = disabled;
    if (disabled) {
    ...
    if (showConfigureButton) {
      ...
      obj2[1] = intl2.string(tmp2(2367).g5Ds69);
      obj2[2] = tmp10;
      showConfigureButton = callback(isPowerupActive(4745).Button, obj2);
    }
...
export default function GuildPowerupsBottomSheet(arg0) {
  ...
  let powerup;
  ({ guildId, powerup } = arg0);
  let obj = require(11712) /* ModalType */;
  const logPowerupModalOpened = obj.useLogPowerupModalOpened(guildId, powerup, require(11712) /* ModalType */.ModalType.DETAIL);
