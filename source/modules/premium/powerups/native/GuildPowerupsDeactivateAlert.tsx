...
// Dependencies: [17, 21, 4285, 712, 11818, 11819, 11820, 4628, 7716, 1236, 2316, 4628, 4281, 2]
// Exports: default

// Module 11817 (GuildPowerupsDeactivateAlert)
...
export default function GuildPowerupsDeactivateAlert(arg0) {
  ...
  const tmp = callback2();
  _require = tmp;
  ({ onDeactivate: c1, error } = importDefault(11818)(guildId, powerup));
  const tmp2 = importDefault;
  const tmp4 = importDefault(11818)(guildId, powerup);
  let obj = _require(11820);
  const logPowerupModalOpened = obj.useLogPowerupModalOpened(guildId, powerup, _require(11820).ModalType.DEACTIVATE);
  ...
  const arr = importDefault(11819)(guildId, powerup);
  ...
  obj[4] = callback(tmp8, {
    ...
    children: importDefault(11819)(guildId, powerup).map((critical) => {
