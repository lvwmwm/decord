...
// Dependencies: [17, 21, 4344, 712, 11960, 11961, 11962, 4687, 7834, 1236, 2335, 4687, 4340, 2]
// Exports: default

// Module 11959 (GuildPowerupsDeactivateAlert)
...
export default function GuildPowerupsDeactivateAlert(arg0) {
  ...
  const tmp = callback2();
  _require = tmp;
  ({ onDeactivate: c1, error } = importDefault(11960)(guildId, powerup));
  const tmp2 = importDefault;
  const tmp4 = importDefault(11960)(guildId, powerup);
  let obj = _require(11962);
  const logPowerupModalOpened = obj.useLogPowerupModalOpened(guildId, powerup, _require(11962).ModalType.DEACTIVATE);
  ...
  obj[1] = callback(_require(7834).CircleErrorIcon, obj1);
  obj[0] = callback(View, obj);
  const intl = _require(1236).intl;
  obj[1] = intl.formatToPlainString(importDefault(2335).iEBw1M, { perk: powerup.title });
  const intl2 = _require(1236).intl;
  obj[2] = intl2.formatToPlainString(importDefault(2335)["7o0K+2"], { perk: powerup.title });
  ...
  items[1] = callback(_require(4687).AlertActionButton, obj6, "deactivate");
  const obj7 = {
    onPress() {
  ...
  items[2] = callback(_require(4687).AlertActionButton, obj7, "cancel");
  obj5[0] = items;
  obj[3] = closure_5(_require(4687).AlertActions, obj5);
  const arr = importDefault(11961)(guildId, powerup);
  ...
  obj[4] = callback(tmp8, {
    ...
    children: importDefault(11961)(guildId, powerup).map((critical) => {
  ...
  return callback(_require(4687).AlertModal, obj);
};
