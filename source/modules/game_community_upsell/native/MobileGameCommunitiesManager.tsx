...
// Dependencies: [12971, 14941, 5261, 14942, 15385, 12974, 12972, 2]

// Module 16580 (_fetchGameCommunities)
...
prototype["_fetchGameCommunities"] = function _fetchGameCommunities(result) {
  const gameIdsForDetectedGames = require(14942) /* DETECTABLE_GAME_TO_APPLICATION_ID_MAP */.getGameIdsForDetectedGames(result);
  ...
  if (GameCommunityUpsellMultiExperiment.getConfig({ location: "MobileGameCommunitiesManager" }).enabled) {
    if (0 === store.getLastFetchedAt()) {
      ...
      if (values.some((lastScannedAt) => {
        ...
        const obj = {};
        const ALL_DETECTABLE_APP_NAMES = require(12972) /* DetectableAppNames */.ALL_DETECTABLE_APP_NAMES;
