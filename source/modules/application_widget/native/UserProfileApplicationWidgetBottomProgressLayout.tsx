...
// Dependencies: [19, 17, 21, 4302, 712, 12285, 12373, 4298, 2]
// Exports: default

// Module 12376 (UserProfileApplicationWidgetBottomProgressLayout)
import "noop";
...
export default function UserProfileApplicationWidgetBottomProgressLayout(arg0) {
  ...
  const items = [require(12285) /* resolvedValuesFromUserApplicationIdentityProfile */.ResolvedValueType.MEDIA];
  const fieldValue = resolveFieldValue(image, items);
  let obj = require(12285) /* resolvedValuesFromUserApplicationIdentityProfile */;
  const singleStringOrSkeleton = obj.resolveSingleStringOrSkeleton(objective, "name", resolveFieldValue);
  let obj1 = require(12285) /* resolvedValuesFromUserApplicationIdentityProfile */;
  ...
  const items1 = [require(12285) /* resolvedValuesFromUserApplicationIdentityProfile */.ResolvedValueType.NUMBER];
  let iter = resolveFieldValue(current, items1);
  let max;
  if (progress != null) {
    max = progress.fields.max;
  }
  const items2 = [require(12285) /* resolvedValuesFromUserApplicationIdentityProfile */.ResolvedValueType.NUMBER];
  let iter2 = resolveFieldValue(max, items2);
  let tmp3Result = tmp3(12285);
  ...
  if (null != fieldValue) {
    ...
    obj2[0] = tmp.image;
    tmp14 = callback(tmp3(12373).ImageSkeleton, obj2);
    tmp15 = callback;
  }
  const items3 = [tmp14, ];
  ...
  if ("value" === singleStringOrSkeleton.status) {
    ...
    let tmp15Result = tmp15(tmp3(4298).Text, obj10);
  } else {
    tmp15Result = tmp15(tmp3(12373).TextSkeleton, { variant: "heading-sm/medium" });
  }
  ...
  if ("value" === singleStringOrSkeleton1.status) {
    ...
    tmp15Result = tmp15(tmp3(4298).Text, obj11);
  } else {
    tmp15Result = tmp15(tmp3(12373).TextSkeleton, { variant: "text-xs/medium" });
  }
  ...
  if (null != iter) {
    ...
    if (null != iter2) {
      ...
      tmp3Result = tmp3(12285);
      const _HermesInternal = HermesInternal;
      iter2Result = "" + tmp3Result.decimalToClampedPercentage(iter.value) + "%";
    }
    obj12[3] = iter2Result;
    tmp15(tmp3(4298).Text, obj12);
  } else {
    items7[1] = tmp15(tmp3(12373).TextSkeleton, { variant: "text-sm/medium", widthChars: 4 });
    obj8[1] = items7;
