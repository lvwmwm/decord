...
// Dependencies: [19, 17, 21, 4380, 712, 12559, 12647, 4376, 2]
// Exports: default

// Module 12650 (UserProfileApplicationWidgetBottomProgressLayout)
import noopAll from "noop" /* 19 */;
import ThemesDefault from "Themes" /* 712 */;
import resolvedValuesFromUserApplicationIdentityProfile from "resolvedValuesFromUserApplicationIdentityProfile" /* 12559 */;
...
export default function UserProfileApplicationWidgetBottomProgressLayout(arg0) {
  ...
  if (null != fieldValue) {
    ...
    obj2[0] = tmp.image;
    tmp14 = callback(tmp3(12647).ImageSkeleton, obj2);
    tmp15 = callback;
  }
  const items3 = [tmp14, ];
  ...
  if ("value" === singleStringOrSkeleton.status) {
    ...
    let tmp15Result = tmp15(tmp3(4376).Text, obj10);
  } else {
    tmp15Result = tmp15(tmp3(12647).TextSkeleton, { variant: "heading-sm/medium" });
  }
  ...
  if ("value" === singleStringOrSkeleton1.status) {
    ...
    tmp15Result = tmp15(tmp3(4376).Text, obj11);
  } else {
    tmp15Result = tmp15(tmp3(12647).TextSkeleton, { variant: "text-xs/medium" });
  }
  ...
  if (null != iter) {
    ...
    if (null != iter2) {
      ...
      tmp3Result = tmp3(12559);
      const _HermesInternal = HermesInternal;
      iter2Result = "" + tmp3Result.decimalToClampedPercentage(iter.value) + "%";
    }
    obj12[3] = iter2Result;
    tmp15(tmp3(4376).Text, obj12);
  } else {
    items7[1] = tmp15(tmp3(12647).TextSkeleton, { variant: "text-sm/medium", widthChars: 4 });
    obj8[1] = items7;
