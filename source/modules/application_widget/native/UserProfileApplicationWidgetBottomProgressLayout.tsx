...
// Dependencies: [19, 17, 21, 4255, 712, 12178, 12266, 4251, 2]
// Exports: default

// Module 12269 (UserProfileApplicationWidgetBottomProgressLayout)
import "noop";
...
export default function UserProfileApplicationWidgetBottomProgressLayout(arg0) {
  ...
  const items1 = [require(12178) /* resolvedValuesFromUserApplicationIdentityProfile */.ResolvedValueType.NUMBER];
  let iter = resolveFieldValue(current, items1);
  let max;
  if (progress != null) {
    max = progress.fields.max;
  }
  const items2 = [require(12178) /* resolvedValuesFromUserApplicationIdentityProfile */.ResolvedValueType.NUMBER];
  ...
  if (null != fieldValue) {
    ...
    obj2[0] = tmp.image;
    tmp14 = callback(tmp3(12266).ImageSkeleton, obj2);
    tmp15 = callback;
  }
  const items3 = [tmp14, ];
  ...
  if ("value" === singleStringOrSkeleton.status) {
    ...
    tmp15Result = tmp15(tmp3(12266).TextSkeleton, { variant: "heading-sm/medium" });
  }
  ...
  if ("value" === singleStringOrSkeleton1.status) {
    ...
    tmp15Result = tmp15(tmp3(12266).TextSkeleton, { variant: "text-xs/medium" });
  }
  ...
  if (null != iter) {
    ...
    items7[1] = tmp15(tmp3(12266).TextSkeleton, { variant: "text-sm/medium", widthChars: 4 });
    obj8[1] = items7;
