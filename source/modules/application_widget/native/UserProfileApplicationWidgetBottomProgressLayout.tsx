...
// Dependencies: [31, 27, 33, 4131, 689, 12002, 12090, 4127, 2]
// Exports: default

// Module 12093 (UserProfileApplicationWidgetBottomProgressLayout)
import "result";
...
export default function UserProfileApplicationWidgetBottomProgressLayout(arg0) {
  ...
  const items1 = [require(12002) /* resolvedValuesFromUserApplicationIdentityProfile */.ResolvedValueType.NUMBER];
  let iter = resolveFieldValue(current, items1);
  let max;
  if (null != progress) {
    max = progress.fields.max;
  }
  const items2 = [require(12002) /* resolvedValuesFromUserApplicationIdentityProfile */.ResolvedValueType.NUMBER];
  ...
  if (null != fieldValue) {
    ...
    tmp14 = callback(require(12090) /* ImageSkeleton */.ImageSkeleton, obj2);
  }
  const items3 = [tmp14, ];
  ...
  if ("value" === singleStringOrSkeleton.status) {
    ...
    let tmp27 = callback(require(4127) /* Text */.Text, obj10);
  } else {
    const obj11 = { variant: "heading-sm/medium" };
    tmp27 = callback(require(12090) /* ImageSkeleton */.TextSkeleton, obj11);
  }
  const items6 = [tmp27, ];
  if ("value" === singleStringOrSkeleton1.status) {
    ...
    let tmp34 = callback(require(4127) /* Text */.Text, obj12);
  } else {
    const obj13 = { variant: "text-xs/medium" };
    tmp34 = callback(require(12090) /* ImageSkeleton */.TextSkeleton, obj13);
  }
  items6[1] = tmp34;
  ...
  if (null != iter) {
    ...
    if (null != iter2) {
      ...
      iter2Result = "" + require(12002) /* resolvedValuesFromUserApplicationIdentityProfile */.decimalToClampedPercentage(iter.value) + "%";
      const obj20 = require(12002) /* resolvedValuesFromUserApplicationIdentityProfile */;
    }
    obj14.children = iter2Result;
    callback(require(4127) /* Text */.Text, obj14);
    const tmp41 = callback;
  } else {
    items7[1] = callback(require(12090) /* ImageSkeleton */.TextSkeleton, { variant: "text-sm/medium", widthChars: 4 });
