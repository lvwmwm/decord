...
// Dependencies: [32, 19, 17, 676, 5697, 21, 4285, 712, 12270, 12357, 12358, 5293, 4706, 2]
// Exports: default

// Module 12269 (UserProfileApplicationWidgetTopHeroLayout)
...
export default function UserProfileApplicationWidgetTopHeroLayout(header) {
  ...
  const items = [_require(12270).ResolvedValueType.MEDIA];
  const fieldValue = resolveFieldValue(image, items);
  obj = { style: tmp.root, children: null };
  const items1 = [header.header, , ];
  obj = { style: tmp.contentRow, children: null };
  obj1 = { style: tmp.heroText, children: null };
  const items2 = [callback2(_require(12357).FieldText, { field: textComponentValues, variant: "text-lg/medium", color: "text-default" }), callback2(_require(12357).FieldText, { field: textComponentValues1, variant: "text-sm/normal", color: "text-muted" }), callback2(_require(12357).FieldText, { field: textComponentValues2, variant: "text-sm/normal", color: "text-muted" }), callback2(_require(12357).FieldText, { field: textComponentValues3, variant: "text-sm/normal", color: "text-muted" })];
  obj1[1] = items2;
  ...
  if (tmp14Result1) {
    ...
    if (tmp14Result) {
      ...
      tmp14Result = tmp14(importDefault(5293), obj5);
      const tmp20 = importDefault(5293);
    }
    obj4[3] = tmp14Result;
