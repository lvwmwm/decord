...
// Dependencies: [32, 19, 17, 676, 5716, 21, 4302, 712, 12285, 12372, 12373, 5309, 4723, 2]
// Exports: default

// Module 12284 (UserProfileApplicationWidgetTopHeroLayout)
...
export default function UserProfileApplicationWidgetTopHeroLayout(header) {
  ...
  const items = [_require(12285).ResolvedValueType.MEDIA];
  const fieldValue = resolveFieldValue(image, items);
  obj = { style: tmp.root, children: null };
  const items1 = [header.header, , ];
  obj = { style: tmp.contentRow, children: null };
  obj1 = { style: tmp.heroText, children: null };
  const items2 = [callback2(_require(12372).FieldText, { field: textComponentValues, variant: "text-lg/medium", color: "text-default" }), callback2(_require(12372).FieldText, { field: textComponentValues1, variant: "text-sm/normal", color: "text-muted" }), callback2(_require(12372).FieldText, { field: textComponentValues2, variant: "text-sm/normal", color: "text-muted" }), callback2(_require(12372).FieldText, { field: textComponentValues3, variant: "text-sm/normal", color: "text-muted" })];
  obj1[1] = items2;
  ...
  if (tmp14Result1) {
    ...
    if (tmp14Result) {
      ...
      const items4 = [tmp14(importDefault(4723), obj8), ];
      ...
      tmp14Result = tmp14(importDefault(5309), obj5);
      const tmp20 = importDefault(5309);
    }
    obj4[3] = tmp14Result;
