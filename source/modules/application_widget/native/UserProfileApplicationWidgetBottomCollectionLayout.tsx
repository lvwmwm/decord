...
function CollectionItem(arg0) {
  ...
  if (null != fieldValue) {
    ...
    tmp11 = callback(tmp3(12266).ImageSkeleton, obj2);
    tmp12 = callback;
  }
  const items1 = [tmp11, ];
  ...
  if ("value" === singleStringOrSkeleton.status) {
    ...
    let tmp12Result = tmp12(tmp3(4251).Text, obj4);
  } else {
    tmp12Result = tmp12(tmp3(12266).TextSkeleton, { variant: "text-xs/medium", widthChars: 6 });
  }
  ...
  if ("value" === singleStringOrSkeleton1.status) {
    ...
    tmp12Result = tmp12(tmp3(4251).Text, obj5);
  } else {
    tmp12Result = tmp12(tmp3(12266).TextSkeleton, { variant: "text-xxs/medium", widthChars: 10 });
  }
