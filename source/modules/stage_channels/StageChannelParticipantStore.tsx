...
// Dependencies: [4503, 1215, 1386, 1908, 4120, 4130, 5380, 1921, 4500, 4558, 5382, 1391, 4116, 12, 5385, 4547, 656, 586, 706, 2]

// Module 5379 (getActiveStageChannelIds)
import applyDefault from "apply" /* 12 */;
import initializeDefault from "initialize" /* 586 */;
import dispatcherDefault from "dispatcher" /* 706 */;
import sortKeyDefault from "sortKey" /* 5385 */;
...
import closure_13 from "buildStageChannelUserRoles" /* 5382 */;
...
function updateParticipant(arg0) {
  ...
  f79290 = (updateParticipant) => updateParticipant.updateParticipant(f79290);
  if (mapped === undefined) {
...
function handleRelationshipUpdate(relationship) {
  let f79290 = relationship.relationship.id;
  let values = secondaryIndexMap.values(undefined, true);
  const mapped = values.map((id) => id.id);
  f79290 = (updateParticipant) => updateParticipant.updateParticipant(f79290);
  let mapped1 = mapped;
...
function handleStreamClose(streamKey) {
  const decodeStreamKeyResult = f79290(4547).decodeStreamKey(streamKey.streamKey);
...
const stageChannelParticipantStore = new StageChannelParticipantStore(dispatcherDefault, {
  ...
  CHANNEL_UPDATES: function handleChannelUpdate(channels) {
    ...
    const f79294 = (rebuild) => rebuild.rebuild();
    let mapped = reduced;
