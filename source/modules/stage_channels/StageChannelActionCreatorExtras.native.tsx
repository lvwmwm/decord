...
// Dependencies: [7920, 5239, 4347, 7922, 2009, 8115, 8156, 8163, 12865, 4617, 4615, 4611, 10737, 8930, 8158, 2]
...
export const openStageChannelSettings = function openStageChannelSettings(closure_0) {
  ...
  obj.openLazy(asyncRequireImpl(7922, dependencyMap.paths), closure_5, obj);
};
...
export const openStageChannelAudienceNoticeModal = function openStageChannelAudienceNoticeModal(channelId) {
  let obj = ACTION_SHEET_HEIGHT_HALFDefault;
  obj = { channelId };
  obj.openLazy(asyncRequireImpl(8115, dependencyMap.paths), closure_4, obj);
};
export const openStageBlockedUsersSheet = function openStageBlockedUsersSheet(channel, onAccept) {
  let obj = ACTION_SHEET_HEIGHT_HALFDefault;
  obj = { channel, onAccept };
  obj.openLazy(asyncRequireImpl(8156, dependencyMap.paths), closure_6, obj);
};
export const openStageSettingsSheet = function openStageSettingsSheet(closure_0, closure_1) {
  ...
  obj.openLazy(asyncRequireImpl(8163, dependencyMap.paths), closure_7, obj);
};
export const openEndStageModal = function openEndStageModal(closure_0) {
  let obj = ACTION_SHEET_HEIGHT_HALFDefault;
  obj = { channel: closure_0 };
  obj.openLazy(asyncRequireImpl(12865, dependencyMap.paths), closure_8, obj);
};
export const openStageChannel = function openStageChannel(closure_0) {
  if (closure_0.isGuildStageVoice()) {
    ...
    if (obj.isStageVoicePanelEnabled("stage_channel_action_creator_extras")) {
      ...
      tmpResult = tmp(4615);
      obj2.pushLazy(tmp(2009)(10737, tmp2.paths), obj, tmpResult.getVoiceChannelKey(closure_0.id));
      const tmp4 = tmp(2009)(10737, tmp2.paths);
    }
    tmp2 = dependencyMap;
  }
};
...
export const navigateToStage = function navigateToStage(id) {
  ...
  if (id.isGuildStageVoice()) {
    let obj = useIsStageVoicePanelEnabled;
    if (obj.isStageVoicePanelEnabled("stage_channel_action_creator_extras")) {
      let tmp3Result = tmp3(4615);
      tmp3Result.openGuildVoiceModal(id);
    } else {
      obj = { channel: null };
      obj[0] = id;
      const obj2 = _modDef4611;
      tmp3Result = tmp3(4615);
      obj2.pushLazy(tmp3(2009)(10737, tmp4.paths), obj, tmp3Result.getVoiceChannelKey(id.id));
      const tmp6 = tmp3(2009)(10737, tmp4.paths);
    }
    tmp4 = dependencyMap;
  }
};
