...
// Dependencies: [8634, 5326, 4415, 8636, 2009, 8701, 8708, 8711, 12757, 4697, 4695, 4691, 9487, 8859, 8710, 2]
...
// Module 8633 (openStageChannelSettings)
import set from "set" /* 2 */;
import asyncRequireImpl from "asyncRequireImpl" /* 2009 */;
import ACTION_SHEET_HEIGHT_HALFDefault from "ACTION_SHEET_HEIGHT_HALF" /* 4415 */;
import _modDef4691 from "module_4691" /* 4691 */;
import useIsStageVoicePanelEnabled from "useIsStageVoicePanelEnabled" /* 4697 */;
import setIsOnStartStageScreen2 from "setIsOnStartStageScreen" /* 8634 */;
...
export const openStageChannelSettings = function openStageChannelSettings(closure_0) {
  ...
  obj.openLazy(asyncRequireImpl(8636, dependencyMap.paths), closure_5, obj);
};
...
export const openStageChannelAudienceNoticeModal = function openStageChannelAudienceNoticeModal(channelId) {
  let obj = ACTION_SHEET_HEIGHT_HALFDefault;
  obj = { channelId };
  obj.openLazy(asyncRequireImpl(8701, dependencyMap.paths), closure_4, obj);
};
export const openStageBlockedUsersSheet = function openStageBlockedUsersSheet(channel, onAccept) {
  let obj = ACTION_SHEET_HEIGHT_HALFDefault;
  obj = { channel, onAccept };
  obj.openLazy(asyncRequireImpl(8708, dependencyMap.paths), closure_6, obj);
};
export const openStageSettingsSheet = function openStageSettingsSheet(closure_0, closure_1) {
  ...
  obj.openLazy(asyncRequireImpl(8711, dependencyMap.paths), closure_7, obj);
};
export const openEndStageModal = function openEndStageModal(closure_0) {
  let obj = ACTION_SHEET_HEIGHT_HALFDefault;
  obj = { channel: closure_0 };
  obj.openLazy(asyncRequireImpl(12757, dependencyMap.paths), closure_8, obj);
};
export const openStageChannel = function openStageChannel(closure_0) {
  if (closure_0.isGuildStageVoice()) {
    ...
    if (obj.isStageVoicePanelEnabled("stage_channel_action_creator_extras")) {
      ...
      tmpResult = tmp(4695);
      obj2.pushLazy(tmp(2009)(9487, tmp2.paths), obj, tmpResult.getVoiceChannelKey(closure_0.id));
      const tmp4 = tmp(2009)(9487, tmp2.paths);
    }
    tmp2 = dependencyMap;
  }
};
...
export const navigateToStage = function navigateToStage(id) {
  ...
  if (id.isGuildStageVoice()) {
    let obj = useIsStageVoicePanelEnabled;
    if (obj.isStageVoicePanelEnabled("stage_channel_action_creator_extras")) {
      let tmp3Result = tmp3(4695);
      tmp3Result.openGuildVoiceModal(id);
    } else {
      obj = { channel: null };
      obj[0] = id;
      const obj2 = _modDef4691;
      tmp3Result = tmp3(4695);
      obj2.pushLazy(tmp3(2009)(9487, tmp4.paths), obj, tmp3Result.getVoiceChannelKey(id.id));
      const tmp6 = tmp3(2009)(9487, tmp4.paths);
    }
    tmp4 = dependencyMap;
  }
};
