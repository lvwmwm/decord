...
// Dependencies: [8325, 5375, 4448, 8327, 2008, 8520, 8548, 8551, 9011, 4737, 4735, 4731, 9395, 8110, 8550, 2]
...
// Module 8324 (openStageChannelSettings)
...
import setIsOnStartStageScreen2 from "setIsOnStartStageScreen" /* 8325 */;
import useStageBlockedUsersCount from "useStageBlockedUsersCount" /* 8550 */;
import MAX_STAGE_TOPIC_LENGTH from "MAX_STAGE_TOPIC_LENGTH" /* 5375 */;
...
export const openStageChannelSettings = function openStageChannelSettings(closure_0) {
  ...
  obj.openLazy(asyncRequireImpl(8327, dependencyMap.paths), closure_5, obj);
};
...
export const openStageChannelAudienceNoticeModal = function openStageChannelAudienceNoticeModal(channelId) {
  let obj = ACTION_SHEET_HEIGHT_HALFDefault;
  obj = { channelId };
  obj.openLazy(asyncRequireImpl(8520, dependencyMap.paths), closure_4, obj);
};
export const openStageBlockedUsersSheet = function openStageBlockedUsersSheet(channel, onAccept) {
  let obj = ACTION_SHEET_HEIGHT_HALFDefault;
  obj = { channel, onAccept };
  obj.openLazy(asyncRequireImpl(8548, dependencyMap.paths), closure_6, obj);
};
export const openStageSettingsSheet = function openStageSettingsSheet(closure_0, closure_1) {
  ...
  obj.openLazy(asyncRequireImpl(8551, dependencyMap.paths), closure_7, obj);
};
export const openEndStageModal = function openEndStageModal(closure_0) {
  let obj = ACTION_SHEET_HEIGHT_HALFDefault;
  obj = { channel: closure_0 };
  obj.openLazy(asyncRequireImpl(9011, dependencyMap.paths), closure_8, obj);
};
export const openStageChannel = function openStageChannel(closure_0) {
  if (closure_0.isGuildStageVoice()) {
    ...
    if (obj.isStageVoicePanelEnabled("stage_channel_action_creator_extras")) {
      ...
      tmpResult = tmp(4735);
      obj2.pushLazy(tmp(2008)(9395, tmp2.paths), obj, tmpResult.getVoiceChannelKey(closure_0.id));
      const tmp4 = tmp(2008)(9395, tmp2.paths);
    }
    tmp2 = dependencyMap;
  }
};
...
export const navigateToStage = function navigateToStage(id) {
  ...
  if (id.isGuildStageVoice()) {
    let obj = useIsStageVoicePanelEnabled;
    if (obj.isStageVoicePanelEnabled("stage_channel_action_creator_extras")) {
      let tmp3Result = tmp3(4735);
      tmp3Result.openGuildVoiceModal(id);
    } else {
      obj = { channel: null };
      obj[0] = id;
      const obj2 = _modDef4731;
      tmp3Result = tmp3(4735);
      obj2.pushLazy(tmp3(2008)(9395, tmp4.paths), obj, tmp3Result.getVoiceChannelKey(id.id));
      const tmp6 = tmp3(2008)(9395, tmp4.paths);
    }
    tmp4 = dependencyMap;
  }
};
