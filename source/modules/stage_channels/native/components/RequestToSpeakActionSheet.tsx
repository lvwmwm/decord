...
// Dependencies: [32, 19, 17, 1218, 1372, 5114, 21, 4285, 712, 10715, 5689, 1236, 589, 4469, 10716, 10718, 5354, 5122, 7720, 7722, 4253, 7701, 1297, 10720, 3988, 5649, 5669, 5130, 5124, 5377, 5379, 4693, 5688, 4281, 10721, 2]
// Exports: default

// Module 10714 (RequestToSpeakRow)
...
function RequestToSpeakRow(channel) {
  ...
  return closure_9(_require(5689).TableSwitchRow, obj);
}
function ManageSelfSpeakerRow(channel) {
  ...
  const tmp5 = importDefault(4469);
  const tmp8 = importDefault(4469)(stateFromStores, id) === channel(4469).RequestToSpeakStates.ON_STAGE;
  importDefault = tmp8;
  ...
  if (tmp8) {
    let MicrophoneArrowRightIcon = tmp(10716).GroupArrowDownIcon;
  } else {
    MicrophoneArrowRightIcon = tmp(10718).MicrophoneArrowRightIcon;
  }
  obj = {
    onPress() {
      if (!_undefined) {
  ...
  return callback2(channel(5354).TableRow, obj);
}
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  let obj2 = channelId(5130);
  const stageParticipantsCount = obj2.useStageParticipantsCount(channelId, channelId(5124).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK);
  ...
  if (null != stateFromStores) {
    ...
    obj4[1] = items2;
    const items3 = [callback3(tmp(5688).TableRowGroup, obj4), ];
    ...
    obj7[1] = callback2(tmp(4281).Text, obj9);
    items3[1] = callback2(View, obj7);
    obj3[2] = items3;
    const items4 = [callback3(tmp(4693).Stack, obj3), ];
    const obj11 = { channel: null, height: null };
    obj11[0] = stateFromStores;
    const _Math = Math;
    obj11[1] = Math.max(first1 - first - 8, 0);
    items4[1] = callback2(tmp3(10721), obj11);
    obj2[1] = items4;
    obj1[2] = callback3(tmp(4693).Stack, obj2);
    obj[2] = callback2(tmp(5379).BottomSheetScrollView, obj1);
    obj[1] = callback2(tmp(5377).BottomSheet, obj);
    tmp14 = callback2(tmp(5649).AnalyticsLocationProvider, obj);
    const tmp3Result = tmp3(10721);
  }
  return tmp14;
};
