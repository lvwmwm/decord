...
// Dependencies: [32, 19, 17, 1218, 1372, 5070, 21, 4255, 712, 10836, 5621, 1236, 589, 4440, 10837, 10839, 5286, 5078, 7708, 7710, 4223, 7689, 1297, 10841, 3959, 5581, 5601, 5086, 5080, 5309, 5311, 4664, 5620, 4251, 10842, 2]
// Exports: default

// Module 10835 (RequestToSpeakRow)
...
function RequestToSpeakRow(channel) {
  ...
  return closure_9(_require(5621).TableSwitchRow, obj);
}
function ManageSelfSpeakerRow(channel) {
  ...
  const tmp5 = importDefault(4440);
  const tmp8 = importDefault(4440)(stateFromStores, id) === channel(4440).RequestToSpeakStates.ON_STAGE;
  importDefault = tmp8;
  ...
  if (tmp8) {
    let MicrophoneArrowRightIcon = tmp(10837).GroupArrowDownIcon;
  } else {
    MicrophoneArrowRightIcon = tmp(10839).MicrophoneArrowRightIcon;
  }
  obj = {
    onPress() {
      if (!_undefined) {
  ...
  return callback2(channel(5286).TableRow, obj);
}
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  let obj2 = channelId(5086);
  const stageParticipantsCount = obj2.useStageParticipantsCount(channelId, channelId(5080).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK);
  ...
  if (null != stateFromStores) {
    ...
    obj4[1] = items2;
    const items3 = [callback3(tmp(5620).TableRowGroup, obj4), ];
    ...
    obj7[1] = callback2(tmp(4251).Text, obj9);
    items3[1] = callback2(View, obj7);
    obj3[2] = items3;
    const items4 = [callback3(tmp(4664).Stack, obj3), ];
    const obj11 = { channel: null, height: null };
    obj11[0] = stateFromStores;
    const _Math = Math;
    obj11[1] = Math.max(first1 - first - 8, 0);
    items4[1] = callback2(tmp3(10842), obj11);
    obj2[1] = items4;
    obj1[2] = callback3(tmp(4664).Stack, obj2);
    obj[2] = callback2(tmp(5311).BottomSheetScrollView, obj1);
    obj[1] = callback2(tmp(5309).BottomSheet, obj);
    tmp14 = callback2(tmp(5581).AnalyticsLocationProvider, obj);
    const tmp3Result = tmp3(10842);
  }
  return tmp14;
};
