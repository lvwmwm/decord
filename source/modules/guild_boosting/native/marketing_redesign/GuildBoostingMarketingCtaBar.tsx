...
// Dependencies: [32, 19, 17, 1903, 4178, 676, 1905, 21, 687, 4285, 712, 1297, 4145, 4286, 5649, 5669, 647, 12646, 8089, 4192, 3930, 1921, 12700, 12702, 4706, 12763, 4281, 1236, 5700, 4827, 12687, 6785, 12766, 12767, 4695, 5133, 9951, 12768, 8545, 2]
// Exports: default

// Module 12762 (GuildBoostingMarketingCtaBar)
...
export default function GuildBoostingMarketingCtaBar(premiumGroupRole) {
  ...
  analyticsLocations = first(5649)(first(5669).BOOSTED_GUILD_PERKS_MODAL).analyticsLocations;
  let obj = first(647);
  ...
  obj = { withTiming: first(4286).withTiming, isVisible: first };
  fn.__closure = obj;
  ...
  let obj7 = first(3930);
  const tmp14Result = first(12646)(fractionalPremiumInfo.endsAt, first(12646).CountDownMessageTypes.LONG_TIME_LEFT);
  if (premiumGroupRole === first(1921).PremiumSubscriptionGroupRole.MEMBER) {
    obj1 = { style: null };
    obj1[0] = tmp.boostingUnavailablePill;
    let tmp20 = callback(tmp4(12700), obj1);
  } else {
    tmp20 = null;
  ...
  tmp4Result = tmp4(4706);
  const items5 = [callback(first(12763), { guild }), , , ];
  ...
  const items6 = [callback(first(4281).Heading, obj6), , , , ];
  ...
  obj7[3] = first(5700).GuildIconSizes.LARGE;
  items6[1] = callback(first(5700), obj7);
  const obj8 = {
    onPress() {
      window.clearTimeout(ref.current);
  ...
  const items7 = [callback(first(4281).Text, { style: tmp.guildName, color: "text-overlay-light", variant: "text-md/bold", children: guild.name }), ];
  ...
  obj13[4] = intl2.format(first(1236).t["pob/cL"], { subscriptions: first(4192)(premiumGroupRole.guild.id).total });
  items9[1] = callback(first(4281).Text, obj13);
  obj11[1] = items9;
  const items10 = [callback2(first(4145).View, obj11), ];
  ...
  obj14[1] = callback(first(4281).Text, obj15);
  items10[1] = callback(first(4145).View, obj14);
  obj10[1] = items10;
  items7[1] = callback2(memo, obj10);
  obj8[1] = items7;
  items6[2] = callback2(first(4827).PressableOpacity, obj8);
  items6[3] = tmp20;
  const items13 = [, ];
  ({ cta: arr14[0], ctaPrimary: arr14[1] } = tmp);
  items6[4] = callback(first(6785), { styles: items13, guild, previousGuildSubscriptionSlot, analyticsSection: constants.HEADER, fractionalPremiumState: fractionalPremiumInfo.fractionalState, premiumGroupRole, intent, onResult });
  obj5[1] = items6;
  items5[1] = callback2(memo, obj5);
  items5[2] = callback(first(12766), { style: tmp.headerStars });
  items5[3] = callback(first(12767), { style: tmp.headerWave });
  obj4[6] = items5;
  ...
  obj19[1] = callback(first(4695).Button, obj22);
