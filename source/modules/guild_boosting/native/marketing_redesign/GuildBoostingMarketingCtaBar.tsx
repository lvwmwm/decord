...
// Dependencies: [32, 19, 17, 1923, 4338, 676, 1925, 21, 687, 4446, 712, 1297, 4186, 4447, 5918, 5938, 647, 13051, 7920, 4352, 4108, 1939, 13105, 13107, 4905, 13167, 4442, 1236, 5969, 5033, 13092, 7092, 13170, 13171, 4893, 5342, 10391, 13172, 8339, 2]
// Exports: default

// Module 13166 (GuildBoostingMarketingCtaBar)
...
import closure_7 from "handleGuildBoostsUpdate" /* 4338 */;
...
export default function GuildBoostingMarketingCtaBar(premiumGroupRole) {
  ...
  analyticsLocations = first(5918)(first(5938).BOOSTED_GUILD_PERKS_MODAL).analyticsLocations;
  ...
  obj = { withTiming: _require(4447).withTiming, isVisible: first };
  fn.__closure = obj;
  ...
  obj = { withTiming: _require(4447).withTiming, isVisible: tmp12 };
  fn2.__closure = obj;
  fn2.__workletHash = 6895237370657;
  fn2.__initData = closure_16;
  const animatedStyle1 = obj4.useAnimatedStyle(fn2);
  const tmp6 = first(5918);
  const tmp14 = first(13051);
  let obj6 = _require(7920);
  ...
  let obj7 = first(4108);
  const tmp14Result = first(13051)(fractionalPremiumInfo.endsAt, _require(13051).CountDownMessageTypes.LONG_TIME_LEFT);
  if (premiumGroupRole === _require(1939).PremiumSubscriptionGroupRole.MEMBER) {
    obj1 = { style: null };
    obj1[0] = tmp.boostingUnavailablePill;
    let tmp20 = callback(tmp4(13105), obj1);
  } else {
    tmp20 = null;
  ...
  const items5 = [callback(first(13167), { guild }), , , ];
  ...
  const items6 = [callback(_require(4442).Heading, obj6), , , , ];
  ...
  obj7[3] = _require(5969).GuildIconSizes.LARGE;
  items6[1] = callback(first(5969), obj7);
  const obj8 = {
    onPress() {
      window.clearTimeout(ref.current);
  ...
  const items7 = [callback(_require(4442).Text, { style: tmp.guildName, color: "text-overlay-light", variant: "text-md/bold", children: guild.name }), ];
  ...
  const tmp4Result1 = first(5969);
  const items9 = [callback(_require(1297).Icon, { style: tmp.guildBoostCountIcon, source: first(13092), color: first(712).unsafe_rawColors.GUILD_BOOSTING_PINK, size: _require(1297).Icon.Sizes.SMALL }), ];
  ...
  obj13[4] = intl2.format(_require(1236).t["pob/cL"], { subscriptions: first(4352)(premiumGroupRole.guild.id).total });
  items9[1] = callback(_require(4442).Text, obj13);
  obj11[1] = items9;
  const items10 = [callback2(first(4186).View, obj11), ];
  ...
  obj14[1] = callback(_require(4442).Text, obj15);
  items10[1] = callback(first(4186).View, obj14);
  obj10[1] = items10;
  items7[1] = callback2(memo, obj10);
  obj8[1] = items7;
  items6[2] = callback2(_require(5033).PressableOpacity, obj8);
  items6[3] = tmp20;
  const items13 = [, ];
  ({ cta: arr14[0], ctaPrimary: arr14[1] } = tmp);
  items6[4] = callback(first(7092), { styles: items13, guild, previousGuildSubscriptionSlot, analyticsSection: constants.HEADER, fractionalPremiumState: fractionalPremiumInfo.fractionalState, premiumGroupRole, intent, onResult });
  obj5[1] = items6;
  items5[1] = callback2(memo, obj5);
  items5[2] = callback(first(13170), { style: tmp.headerStars });
  items5[3] = callback(first(13171), { style: tmp.headerWave });
  obj4[6] = items5;
  ...
  if (isPremiumResult) {
    ...
    obj20[2] = function onPress() {
      ...
      callback(10391).openGiftModal(obj);
    };
  ...
  obj19[1] = callback(_require(4893).Button, obj22);
