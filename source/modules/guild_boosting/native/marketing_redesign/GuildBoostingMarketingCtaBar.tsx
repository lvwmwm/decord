...
// Dependencies: [32, 19, 17, 1874, 4148, 676, 1876, 21, 687, 4255, 712, 1297, 4116, 4256, 5581, 5601, 647, 12604, 7964, 4162, 3901, 1892, 12658, 12660, 4677, 12721, 4251, 1236, 5632, 4783, 12645, 6761, 12724, 12725, 4666, 5089, 9858, 12726, 8316, 2]
// Exports: default

// Module 12720 (GuildBoostingMarketingCtaBar)
...
export default function GuildBoostingMarketingCtaBar(premiumGroupRole) {
  ...
  analyticsLocations = first(5581)(first(5601).BOOSTED_GUILD_PERKS_MODAL).analyticsLocations;
  let obj = first(647);
  ...
  const tmp6 = first(5581);
  const tmp14 = first(12604);
  let obj6 = first(7964);
  ...
  let obj7 = first(3901);
  const tmp14Result = first(12604)(fractionalPremiumInfo.endsAt, first(12604).CountDownMessageTypes.LONG_TIME_LEFT);
  if (premiumGroupRole === first(1892).PremiumSubscriptionGroupRole.MEMBER) {
    obj1 = { style: null };
    obj1[0] = tmp.boostingUnavailablePill;
    let tmp20 = callback(tmp4(12658), obj1);
  } else {
    tmp20 = null;
  ...
  tmp4Result = tmp4(4677);
  const items5 = [callback(first(12721), { guild }), , , ];
  ...
  obj7[3] = first(5632).GuildIconSizes.LARGE;
  items6[1] = callback(first(5632), obj7);
  const obj8 = {
    onPress() {
      window.clearTimeout(ref.current);
  ...
  obj8[1] = items7;
  items6[2] = callback2(first(4783).PressableOpacity, obj8);
  items6[3] = tmp20;
  const items13 = [, ];
  ({ cta: arr14[0], ctaPrimary: arr14[1] } = tmp);
  items6[4] = callback(first(6761), { styles: items13, guild, previousGuildSubscriptionSlot, analyticsSection: constants.HEADER, fractionalPremiumState: fractionalPremiumInfo.fractionalState, premiumGroupRole, intent, onResult });
  obj5[1] = items6;
  items5[1] = callback2(memo, obj5);
  items5[2] = callback(first(12724), { style: tmp.headerStars });
  items5[3] = callback(first(12725), { style: tmp.headerWave });
  obj4[6] = items5;
  ...
  obj19[1] = callback(first(4666).Button, obj22);
