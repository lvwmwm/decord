...
// Dependencies: [32, 19, 17, 1922, 4337, 676, 1924, 21, 687, 4445, 712, 1297, 4185, 4446, 5905, 5925, 647, 13032, 7906, 4351, 4107, 1938, 13086, 13088, 4892, 13148, 4441, 1236, 5956, 5020, 13073, 7079, 13151, 13152, 4880, 5329, 10373, 13153, 8325, 2]
// Exports: default

// Module 13147 (GuildBoostingMarketingCtaBar)
...
import closure_7 from "handleGuildBoostsUpdate" /* 4337 */;
...
export default function GuildBoostingMarketingCtaBar(premiumGroupRole) {
  ...
  analyticsLocations = first(5905)(first(5925).BOOSTED_GUILD_PERKS_MODAL).analyticsLocations;
  ...
  obj = { withTiming: _require(4446).withTiming, isVisible: first };
  fn.__closure = obj;
  ...
  obj = { withTiming: _require(4446).withTiming, isVisible: tmp12 };
  fn2.__closure = obj;
  fn2.__workletHash = 6895237370657;
  fn2.__initData = closure_16;
  const animatedStyle1 = obj4.useAnimatedStyle(fn2);
  const tmp6 = first(5905);
  const tmp14 = first(13032);
  let obj6 = _require(7906);
  ...
  let obj7 = first(4107);
  const tmp14Result = first(13032)(fractionalPremiumInfo.endsAt, _require(13032).CountDownMessageTypes.LONG_TIME_LEFT);
  if (premiumGroupRole === _require(1938).PremiumSubscriptionGroupRole.MEMBER) {
    obj1 = { style: null };
    obj1[0] = tmp.boostingUnavailablePill;
    let tmp20 = callback(tmp4(13086), obj1);
  } else {
    tmp20 = null;
  ...
  tmp4Result = tmp4(4892);
  const items5 = [callback(first(13148), { guild }), , , ];
  ...
  const items6 = [callback(_require(4441).Heading, obj6), , , , ];
  ...
  obj7[3] = _require(5956).GuildIconSizes.LARGE;
  items6[1] = callback(first(5956), obj7);
  const obj8 = {
    onPress() {
      window.clearTimeout(ref.current);
  ...
  const items7 = [callback(_require(4441).Text, { style: tmp.guildName, color: "text-overlay-light", variant: "text-md/bold", children: guild.name }), ];
  ...
  const tmp4Result1 = first(5956);
  const items9 = [callback(_require(1297).Icon, { style: tmp.guildBoostCountIcon, source: first(13073), color: first(712).unsafe_rawColors.GUILD_BOOSTING_PINK, size: _require(1297).Icon.Sizes.SMALL }), ];
  ...
  obj13[4] = intl2.format(_require(1236).t["pob/cL"], { subscriptions: first(4351)(premiumGroupRole.guild.id).total });
  items9[1] = callback(_require(4441).Text, obj13);
  obj11[1] = items9;
  const items10 = [callback2(first(4185).View, obj11), ];
  ...
  obj14[1] = callback(_require(4441).Text, obj15);
  items10[1] = callback(first(4185).View, obj14);
  obj10[1] = items10;
  items7[1] = callback2(memo, obj10);
  obj8[1] = items7;
  items6[2] = callback2(_require(5020).PressableOpacity, obj8);
  items6[3] = tmp20;
  const items13 = [, ];
  ({ cta: arr14[0], ctaPrimary: arr14[1] } = tmp);
  items6[4] = callback(first(7079), { styles: items13, guild, previousGuildSubscriptionSlot, analyticsSection: constants.HEADER, fractionalPremiumState: fractionalPremiumInfo.fractionalState, premiumGroupRole, intent, onResult });
  obj5[1] = items6;
  items5[1] = callback2(memo, obj5);
  items5[2] = callback(first(13151), { style: tmp.headerStars });
  items5[3] = callback(first(13152), { style: tmp.headerWave });
  obj4[6] = items5;
  ...
  if (isPremiumResult) {
    ...
    obj20[2] = function onPress() {
      ...
      callback(10373).openGiftModal(obj);
    };
  ...
  obj19[1] = callback(_require(4880).Button, obj22);
