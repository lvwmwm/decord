...
// Dependencies: [32, 19, 17, 1922, 4340, 676, 1924, 21, 687, 4448, 712, 1297, 4187, 4449, 5921, 5941, 647, 13085, 7942, 4354, 4109, 1938, 13139, 13141, 4908, 13200, 4444, 1236, 5972, 5036, 13126, 7113, 13203, 13204, 4896, 5345, 10415, 13205, 8361, 2]
// Exports: default

// Module 13199 (GuildBoostingMarketingCtaBar)
...
import closure_7 from "handleGuildBoostsUpdate" /* 4340 */;
...
export default function GuildBoostingMarketingCtaBar(premiumGroupRole) {
  ...
  analyticsLocations = first(5921)(first(5941).BOOSTED_GUILD_PERKS_MODAL).analyticsLocations;
  ...
  obj = { withTiming: _require(4449).withTiming, isVisible: first };
  fn.__closure = obj;
  ...
  obj = { withTiming: _require(4449).withTiming, isVisible: tmp12 };
  fn2.__closure = obj;
  fn2.__workletHash = 6895237370657;
  fn2.__initData = closure_16;
  const animatedStyle1 = obj4.useAnimatedStyle(fn2);
  const tmp6 = first(5921);
  const tmp14 = first(13085);
  let obj6 = _require(7942);
  ...
  let obj7 = first(4109);
  const tmp14Result = first(13085)(fractionalPremiumInfo.endsAt, _require(13085).CountDownMessageTypes.LONG_TIME_LEFT);
  if (premiumGroupRole === _require(1938).PremiumSubscriptionGroupRole.MEMBER) {
    obj1 = { style: null };
    obj1[0] = tmp.boostingUnavailablePill;
    let tmp20 = callback(tmp4(13139), obj1);
  } else {
    tmp20 = null;
  ...
  tmp4Result = tmp4(4908);
  const items5 = [callback(first(13200), { guild }), , , ];
  ...
  obj6[3] = intl.string(_require(1236).t["AF+Tyh"]);
  const items6 = [callback(_require(4444).Heading, obj6), , , , ];
  ...
  obj7[3] = _require(5972).GuildIconSizes.LARGE;
  items6[1] = callback(first(5972), obj7);
  const obj8 = {
    onPress() {
      window.clearTimeout(ref.current);
  ...
  const items7 = [callback(_require(4444).Text, { style: tmp.guildName, color: "text-overlay-light", variant: "text-md/bold", children: guild.name }), ];
  ...
  const tmp4Result1 = first(5972);
  const items9 = [callback(_require(1297).Icon, { style: tmp.guildBoostCountIcon, source: first(13126), color: first(712).unsafe_rawColors.GUILD_BOOSTING_PINK, size: _require(1297).Icon.Sizes.SMALL }), ];
  ...
  obj13[4] = intl2.format(_require(1236).t["pob/cL"], { subscriptions: first(4354)(premiumGroupRole.guild.id).total });
  items9[1] = callback(_require(4444).Text, obj13);
  obj11[1] = items9;
  const items10 = [callback2(first(4187).View, obj11), ];
  ...
  obj15[3] = intl3.format(_require(1236).t.xXb78j, { numSubscriptions: memo });
  obj14[1] = callback(_require(4444).Text, obj15);
  items10[1] = callback(first(4187).View, obj14);
  obj10[1] = items10;
  items7[1] = callback2(memo, obj10);
  obj8[1] = items7;
  items6[2] = callback2(_require(5036).PressableOpacity, obj8);
  items6[3] = tmp20;
  const items13 = [, ];
  ({ cta: arr14[0], ctaPrimary: arr14[1] } = tmp);
  items6[4] = callback(first(7113), { styles: items13, guild, previousGuildSubscriptionSlot, analyticsSection: constants.HEADER, fractionalPremiumState: fractionalPremiumInfo.fractionalState, premiumGroupRole, intent, onResult });
  obj5[1] = items6;
  items5[1] = callback2(memo, obj5);
  items5[2] = callback(first(13203), { style: tmp.headerStars });
  items5[3] = callback(first(13204), { style: tmp.headerWave });
  obj4[6] = items5;
  ...
  if (isPremiumResult) {
    ...
    obj20[2] = function onPress() {
      ...
      callback(10415).openGiftModal(obj);
    };
  ...
  obj19[1] = callback(_require(4896).Button, obj22);
