...
// Dependencies: [32, 19, 17, 1874, 4086, 676, 1876, 21, 687, 4193, 712, 1297, 4054, 4194, 5519, 5539, 647, 12511, 7846, 4100, 3839, 1892, 12565, 12567, 4615, 12658, 4189, 1236, 5570, 4721, 12552, 11694, 12661, 12662, 4604, 5027, 9713, 12663, 9332, 2]
// Exports: default
...
export default function GuildBoostingMarketingCtaBar(premiumGroupRole) {
  ...
  analyticsLocations = first(5519)(first(5539).BOOSTED_GUILD_PERKS_MODAL).analyticsLocations;
  let obj = first(647);
  ...
  obj = { withTiming: first(4194).withTiming, isVisible: first };
  fn.__closure = obj;
  ...
  let obj7 = first(3839);
  const tmp14Result = first(12511)(fractionalPremiumInfo.endsAt, first(12511).CountDownMessageTypes.LONG_TIME_LEFT);
  if (premiumGroupRole === first(1892).PremiumSubscriptionGroupRole.MEMBER) {
    obj1 = { style: null };
    obj1[0] = tmp.boostingUnavailablePill;
    let tmp20 = callback(tmp4(12565), obj1);
  } else {
    tmp20 = null;
  ...
  tmp4Result = tmp4(4615);
  const items5 = [callback(first(12658), { guild }), , , ];
  ...
  const items6 = [callback(first(4189).Heading, obj6), , , , ];
  ...
  obj7[3] = first(5570).GuildIconSizes.LARGE;
  items6[1] = callback(first(5570), obj7);
  const obj8 = {
    onPress() {
      window.clearTimeout(ref.current);
  ...
  const items7 = [callback(first(4189).Text, { style: tmp.guildName, color: "text-overlay-light", variant: "text-md/bold", children: guild.name }), ];
  ...
  obj13[4] = intl2.format(first(1236).t["pob/cL"], { subscriptions: first(4100)(premiumGroupRole.guild.id).total });
  items9[1] = callback(first(4189).Text, obj13);
  obj11[1] = items9;
  const items10 = [callback2(first(4054).View, obj11), ];
  ...
  obj14[1] = callback(first(4189).Text, obj15);
  items10[1] = callback(first(4054).View, obj14);
  obj10[1] = items10;
  items7[1] = callback2(memo, obj10);
  obj8[1] = items7;
  items6[2] = callback2(first(4721).PressableOpacity, obj8);
  items6[3] = tmp20;
  const items13 = [, ];
  ({ cta: arr14[0], ctaPrimary: arr14[1] } = tmp);
  items6[4] = callback(first(11694), { styles: items13, guild, previousGuildSubscriptionSlot, analyticsSection: constants.HEADER, fractionalPremiumState: fractionalPremiumInfo.fractionalState, premiumGroupRole, intent, onResult });
  obj5[1] = items6;
  items5[1] = callback2(memo, obj5);
  items5[2] = callback(first(12661), { style: tmp.headerStars });
  items5[3] = callback(first(12662), { style: tmp.headerWave });
  obj4[6] = items5;
  ...
  obj19[1] = callback(first(4604).Button, obj22);
