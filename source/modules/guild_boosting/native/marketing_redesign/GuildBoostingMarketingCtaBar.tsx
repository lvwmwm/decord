...
// Dependencies: [32, 19, 17, 1922, 4271, 676, 1924, 21, 687, 4668, 712, 1297, 4119, 4671, 7177, 7197, 647, 12948, 8318, 4285, 4042, 1940, 13001, 13003, 4761, 13064, 4739, 1236, 7226, 5438, 12988, 7410, 13067, 13068, 4750, 6747, 10334, 13069, 9362, 2]
// Exports: default

// Module 13063 (GuildBoostingMarketingCtaBar)
...
import closure_7 from "handleGuildBoostsUpdate" /* 4271 */;
...
export default function GuildBoostingMarketingCtaBar(premiumGroupRole) {
  ...
  analyticsLocations = first(7177)(first(7197).BOOSTED_GUILD_PERKS_MODAL).analyticsLocations;
  ...
  obj = { withTiming: _require(4671).withTiming, isVisible: first };
  fn.__closure = obj;
  ...
  obj = { withTiming: _require(4671).withTiming, isVisible: tmp12 };
  fn2.__closure = obj;
  ...
  const tmp14 = first(12948);
  let obj6 = _require(8318);
  ...
  let obj7 = first(4042);
  const tmp14Result = first(12948)(fractionalPremiumInfo.endsAt, _require(12948).CountDownMessageTypes.LONG_TIME_LEFT);
  if (premiumGroupRole === _require(1940).PremiumSubscriptionGroupRole.MEMBER) {
    obj1 = { style: null };
    obj1[0] = tmp.boostingUnavailablePill;
    let tmp20 = callback(tmp4(13001), obj1);
  } else {
    tmp20 = null;
  ...
  tmp4Result = tmp4(4761);
  const items5 = [callback(first(13064), { guild }), , , ];
  ...
  const items6 = [callback(_require(4739).Heading, obj6), , , , ];
  ...
  obj7[3] = _require(7226).GuildIconSizes.LARGE;
  items6[1] = callback(first(7226), obj7);
  const obj8 = {
    onPress() {
      window.clearTimeout(ref.current);
  ...
  const items7 = [callback(_require(4739).Text, { style: tmp.guildName, color: "text-overlay-light", variant: "text-md/bold", children: guild.name }), ];
  ...
  const tmp4Result1 = first(7226);
  const items9 = [callback(_require(1297).Icon, { style: tmp.guildBoostCountIcon, source: first(12988), color: first(712).unsafe_rawColors.GUILD_BOOSTING_PINK, size: _require(1297).Icon.Sizes.SMALL }), ];
  ...
  obj13[4] = intl2.format(_require(1236).t["pob/cL"], { subscriptions: first(4285)(premiumGroupRole.guild.id).total });
  items9[1] = callback(_require(4739).Text, obj13);
  obj11[1] = items9;
  const items10 = [callback2(first(4119).View, obj11), ];
  ...
  obj14[1] = callback(_require(4739).Text, obj15);
  items10[1] = callback(first(4119).View, obj14);
  obj10[1] = items10;
  items7[1] = callback2(memo, obj10);
  obj8[1] = items7;
  items6[2] = callback2(_require(5438).PressableOpacity, obj8);
  items6[3] = tmp20;
  const items13 = [, ];
  ({ cta: arr14[0], ctaPrimary: arr14[1] } = tmp);
  items6[4] = callback(first(7410), { styles: items13, guild, previousGuildSubscriptionSlot, analyticsSection: constants.HEADER, fractionalPremiumState: fractionalPremiumInfo.fractionalState, premiumGroupRole, intent, onResult });
  obj5[1] = items6;
  items5[1] = callback2(memo, obj5);
  items5[2] = callback(first(13067), { style: tmp.headerStars });
  items5[3] = callback(first(13068), { style: tmp.headerWave });
  obj4[6] = items5;
  ...
  if (isPremiumResult) {
    ...
    obj20[2] = function onPress() {
      ...
      callback(10334).openGiftModal(obj);
    };
  ...
  obj19[1] = callback(_require(4750).Button, obj22);
