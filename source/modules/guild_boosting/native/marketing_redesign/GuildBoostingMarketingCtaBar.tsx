...
// Dependencies: [32, 19, 17, 1922, 4272, 676, 1924, 21, 687, 4380, 712, 1297, 4120, 4381, 5834, 5854, 647, 12935, 8319, 4286, 4043, 1938, 12989, 12991, 4826, 13052, 4376, 1236, 5885, 4949, 12976, 7007, 13055, 13056, 4815, 5258, 10185, 13057, 8769, 2]
// Exports: default

// Module 13051 (GuildBoostingMarketingCtaBar)
...
import closure_7 from "handleGuildBoostsUpdate" /* 4272 */;
...
export default function GuildBoostingMarketingCtaBar(premiumGroupRole) {
  ...
  analyticsLocations = first(5834)(first(5854).BOOSTED_GUILD_PERKS_MODAL).analyticsLocations;
  ...
  obj = { withTiming: _require(4381).withTiming, isVisible: first };
  fn.__closure = obj;
  ...
  obj = { withTiming: _require(4381).withTiming, isVisible: tmp12 };
  fn2.__closure = obj;
  fn2.__workletHash = 6895237370657;
  fn2.__initData = closure_16;
  const animatedStyle1 = obj4.useAnimatedStyle(fn2);
  const tmp6 = first(5834);
  const tmp14 = first(12935);
  let obj6 = _require(8319);
  ...
  let obj7 = first(4043);
  const tmp14Result = first(12935)(fractionalPremiumInfo.endsAt, _require(12935).CountDownMessageTypes.LONG_TIME_LEFT);
  if (premiumGroupRole === _require(1938).PremiumSubscriptionGroupRole.MEMBER) {
    obj1 = { style: null };
    obj1[0] = tmp.boostingUnavailablePill;
    let tmp20 = callback(tmp4(12989), obj1);
  } else {
    tmp20 = null;
  ...
  const items5 = [callback(first(13052), { guild }), , , ];
  ...
  const items6 = [callback(_require(4376).Heading, obj6), , , , ];
  ...
  obj7[3] = _require(5885).GuildIconSizes.LARGE;
  items6[1] = callback(first(5885), obj7);
  const obj8 = {
    onPress() {
      window.clearTimeout(ref.current);
  ...
  const items7 = [callback(_require(4376).Text, { style: tmp.guildName, color: "text-overlay-light", variant: "text-md/bold", children: guild.name }), ];
  ...
  const tmp4Result1 = first(5885);
  const items9 = [callback(_require(1297).Icon, { style: tmp.guildBoostCountIcon, source: first(12976), color: first(712).unsafe_rawColors.GUILD_BOOSTING_PINK, size: _require(1297).Icon.Sizes.SMALL }), ];
  ...
  obj13[4] = intl2.format(_require(1236).t["pob/cL"], { subscriptions: first(4286)(premiumGroupRole.guild.id).total });
  items9[1] = callback(_require(4376).Text, obj13);
  obj11[1] = items9;
  const items10 = [callback2(first(4120).View, obj11), ];
  ...
  obj14[1] = callback(_require(4376).Text, obj15);
  items10[1] = callback(first(4120).View, obj14);
  obj10[1] = items10;
  items7[1] = callback2(memo, obj10);
  obj8[1] = items7;
  items6[2] = callback2(_require(4949).PressableOpacity, obj8);
  items6[3] = tmp20;
  const items13 = [, ];
  ({ cta: arr14[0], ctaPrimary: arr14[1] } = tmp);
  items6[4] = callback(first(7007), { styles: items13, guild, previousGuildSubscriptionSlot, analyticsSection: constants.HEADER, fractionalPremiumState: fractionalPremiumInfo.fractionalState, premiumGroupRole, intent, onResult });
  obj5[1] = items6;
  items5[1] = callback2(memo, obj5);
  items5[2] = callback(first(13055), { style: tmp.headerStars });
  items5[3] = callback(first(13056), { style: tmp.headerWave });
  obj4[6] = items5;
  ...
  if (isPremiumResult) {
    ...
    obj20[2] = function onPress() {
      ...
      callback(10185).openGiftModal(obj);
    };
  ...
  obj19[1] = callback(_require(4815).Button, obj22);
