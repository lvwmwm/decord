...
// Dependencies: [32, 19, 17, 1874, 4082, 676, 1876, 21, 687, 4189, 712, 1297, 4050, 4190, 5515, 5535, 647, 12485, 7838, 4096, 3835, 1892, 12539, 12541, 4611, 12633, 4185, 1236, 5566, 4717, 12526, 11676, 12636, 12637, 4600, 5023, 9696, 12638, 9317, 2]
// Exports: default

// Module 12632 (GuildBoostingMarketingCtaBar)
...
export default function GuildBoostingMarketingCtaBar(premiumGroupRole) {
  ...
  analyticsLocations = first(5515)(first(5535).BOOSTED_GUILD_PERKS_MODAL).analyticsLocations;
  let obj = first(647);
  ...
  obj = { withTiming: first(4190).withTiming, isVisible: first };
  fn.__closure = obj;
  ...
  let obj7 = first(3835);
  const tmp14Result = first(12485)(fractionalPremiumInfo.endsAt, first(12485).CountDownMessageTypes.LONG_TIME_LEFT);
  if (premiumGroupRole === first(1892).PremiumSubscriptionGroupRole.MEMBER) {
    obj1 = { style: null };
    obj1[0] = tmp.boostingUnavailablePill;
    let tmp20 = callback(tmp4(12539), obj1);
  } else {
    tmp20 = null;
  ...
  tmp4Result = tmp4(4611);
  const items5 = [callback(first(12633), { guild }), , , ];
  ...
  const items6 = [callback(first(4185).Heading, obj6), , , , ];
  ...
  obj7[3] = first(5566).GuildIconSizes.LARGE;
  items6[1] = callback(first(5566), obj7);
  const obj8 = {
    onPress() {
      window.clearTimeout(ref.current);
  ...
  const items7 = [callback(first(4185).Text, { style: tmp.guildName, color: "text-overlay-light", variant: "text-md/bold", children: guild.name }), ];
  ...
  obj13[4] = intl2.format(first(1236).t["pob/cL"], { subscriptions: first(4096)(premiumGroupRole.guild.id).total });
  items9[1] = callback(first(4185).Text, obj13);
  obj11[1] = items9;
  const items10 = [callback2(first(4050).View, obj11), ];
  ...
  obj14[1] = callback(first(4185).Text, obj15);
  items10[1] = callback(first(4050).View, obj14);
  obj10[1] = items10;
  items7[1] = callback2(memo, obj10);
  obj8[1] = items7;
  items6[2] = callback2(first(4717).PressableOpacity, obj8);
  items6[3] = tmp20;
  const items13 = [, ];
  ({ cta: arr14[0], ctaPrimary: arr14[1] } = tmp);
  items6[4] = callback(first(11676), { styles: items13, guild, previousGuildSubscriptionSlot, analyticsSection: constants.HEADER, fractionalPremiumState: fractionalPremiumInfo.fractionalState, premiumGroupRole, intent, onResult });
  obj5[1] = items6;
  items5[1] = callback2(memo, obj5);
  items5[2] = callback(first(12636), { style: tmp.headerStars });
  items5[3] = callback(first(12637), { style: tmp.headerWave });
  obj4[6] = items5;
  ...
  obj19[1] = callback(first(4600).Button, obj22);
