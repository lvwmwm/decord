...
// Dependencies: [19, 1364, 5158, 8000, 1369, 1906, 21, 3925, 3957, 8001, 3923, 4254, 7144, 8002, 3929, 8004, 1988, 4625, 4100, 1358, 2]
...
// Module 7999 (getFallbackDoubleTapDisambiguatedEmoji)
...
export const getFallbackDoubleTapDisambiguatedEmoji = function getFallbackDoubleTapDisambiguatedEmoji() {
  let byName = importDefault(3925).getByName("heart");
  if (byName == null) {
    byName = null;
  }
  return byName;
...
export const disambiguatedEmojiFromSettingsValue = function disambiguatedEmojiFromSettingsValue(setting) {
  ...
  if (null == customEmojiById) {
    let byName = null;
    if (null != tmp2) {
      byName = importDefault(3925).getByName(tmp2);
      const obj = importDefault(3925);
    }
...
export const handleAddDefaultDoubleTapReaction = function handleAddDefaultDoubleTapReaction(message, channel) {
  ...
  let emojiName;
  let emojiName2;
  const DoubleTapReactionEmoji = obj(3957).DoubleTapReactionEmoji;
  ...
  if (true !== disableDoubleTap) {
    ...
    if (tmpResult.canReactToMessage(message, channel)) {
      ...
      if (reactions.some((emoji) => {
        ...
        tmpResult = tmp(4254);
        const result2 = tmpResult.triggerHapticFeedback(tmp(4254).HapticFeedbackTypes.IMPACT_LIGHT);
        obj1 = { channelId: null, messageId: null, emoji: null, location: null };
        obj1[0] = channel.id;
        obj1[1] = message.id;
        obj1[2] = tmp8;
        obj1[3] = tmp(7144).ReactionLocations.DOUBLE_TAP;
        tmp(7144).removeReaction(obj1);
      } else {
        if (flag) {
          let obj7 = importDefault(3925);
        ...
        if (null != customEmojiById) {
          if (null != customEmojiById) {
            ...
            const emojiUnavailableReason = importDefault(3929).getEmojiUnavailableReason(obj3);
            if (emojiUnavailableReason === constants.PREMIUM_LOCKED) {
              ...
              tmp(4625).openAlert(NITRO_UPSELL_ALERT_KEY, <lazyResult emojiName={null} />);
              ...
              const result3 = tmp(8002).showDoubleTapErrorToast(obj5);
            }
            const obj12 = importDefault(3929);
          }
          const result4 = tmp(4254).triggerHapticFeedback(tmp(4254).HapticFeedbackTypes.IMPACT_LIGHT);
          const tmpResult5 = tmp(7144);
          const id = channel.id;
          tmpResult5.addReaction(id, message.id, tmp8, tmp(7144).ReactionLocations.DOUBLE_TAP);
          const tmpResult4 = tmp(4254);
          obj6 = { dismissAction: null };
          obj6[0] = ContentDismissActionType.INDIRECT_ACTION;
          const result5 = tmp(4100).UNSAFE_markDismissibleContentAsDismissed(tmp(1358).DismissibleContent.DOUBLE_TAP_TO_REACT_REMINDER, obj6);
          if (isContentShown(tmp(1358).DismissibleContent.DOUBLE_TAP_TO_REACT_EXPANDED_UPSELL)) {
            ...
            const result6 = tmp(4100).UNSAFE_markDismissibleContentAsDismissed(tmp(1358).DismissibleContent.DOUBLE_TAP_TO_REACT_EXPANDED_UPSELL, obj7);
            const tmpResult7 = tmp(4100);
          }
          tmp31 = ContentDismissActionType;
          const tmpResult6 = tmp(4100);
        } else if (!flag) {
          emojiName = undefined;
          if (setting != null) {
          ...
          obj8[0] = emojiName;
          const result7 = tmp(8002).showDoubleTapErrorToast(obj8);
          const tmpResult8 = tmp(8002);
        }
      }
    }
  }
