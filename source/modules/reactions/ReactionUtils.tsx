...
// Function ID: 3960
// Name: MAX_REACTIONS
// Dependencies: [1218, 676, 3960, 3961, 1236, 7223, 3993, 698, 2]
...
export const getReactionEmojiName = function getReactionEmojiName(emoji) {
  if (null == emoji.id) {
    let result = importDefault(3961).convertSurrogateToName(emoji.name);
...
export const getAccessibleEmojiDisplayName = function getAccessibleEmojiDisplayName(me, count, emoji, arg3) {
  ...
  if (arg3) {
    ...
    obj[0] = count;
    if (null == emoji.id) {
      let str2 = importDefault(3961).convertSurrogateToName(emoji.name);
...
export const isMeReaction = function isMeReaction(me, me_burst, arg2) {
  let tmp3 = arg2 === require(7223) /* ReactionTypes */.ReactionTypes.BURST;
  if (tmp3) {
    tmp3 = true === me_burst;
  }
  if (!tmp3) {
    let tmp5 = arg2 === require(7223) /* ReactionTypes */.ReactionTypes.NORMAL;
    if (tmp5) {
      tmp5 = true === me;
    }
    tmp3 = tmp5;
...
export const updateReactionNotificationsSetting = function updateReactionNotificationsSetting(NumberResult, setting) {
  const ReactionNotifications = require(3993) /* explicitContentFromProto */.ReactionNotifications;
