...
// Function ID: 11193
// Name: EmojiRow
// Dependencies: [19, 17, 5459, 21, 4342, 4310, 4311, 7290, 4000, 6986, 8712, 4065, 712, 11193, 11194, 8676, 11195, 5809, 2]
// Exports: default

// Module 11192 (EmojiRow)
import "noop";
...
export default function EmojiRow(arg0) {
  ...
  token = require(4065) /* map */.useToken(channel(712).modules.mobile.EMOJI_ROW_EMOJI_CONTAINER_SIZE);
  let obj4 = require(4065) /* map */;
  const token1 = require(4065) /* map */.useToken(channel(712).modules.mobile.EMOJI_ROW_EMOJI_MIN_SPACING);
  const obj5 = require(4065) /* map */;
  const emojisForReactionRow = require(11193) /* useEmojisForReactionRow */.useEmojisForReactionRow(channel, closure_4, token + token1);
  ...
  const items = [
    emojisForReactionRow.map((emoji) => {
      let closure_0 = emoji;
      return token(outer1_0(11194).EmojiReactionRowButton, {
        emoji,
        onPress() {
          let channel_id;
          let id;
      ...
      }, outer1_0(11194).getEmojiKey(emoji, arg1));
    }),
  ...
  obj1 = {
    emojiContainerSize: token,
    onPress() {
      return outer1_0(8676).handleAddNewReactions(channel, id.id, outer1_0(7290).ReactionLocations.MESSAGE);
    }
  };
  items[1] = token(require(11194) /* EmojiPickerRowButton */.EmojiPickerRowButton, obj1);
  obj[1] = items;
  const items1 = [callback(closure_3, obj), ];
  obj2 = { location: null };
  obj2[0] = channel(5809).MESSAGE_LONG_PRESS_MENU;
  items1[1] = token(require(11195) /* DoubleTapEmojiEditNudgeInner */.DoubleTapEmojiEditNudge, obj2);
  obj[1] = items1;
