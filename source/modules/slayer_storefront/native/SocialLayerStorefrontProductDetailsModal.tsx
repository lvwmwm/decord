...
// Function ID: 9977
// Name: HeroMedia
// Dependencies: [32, 19, 17, 6782, 4300, 5744, 9977, 676, 21, 689, 712, 4255, 5220, 9978, 8764, 5221, 4251, 1236, 9979, 1297, 1856, 4492, 1581, 7544, 589, 5601, 5598, 5742, 5615, 5583, 514, 4681, 698, 9975, 7650, 500, 9980, 5746, 5230, 5255, 4676, 9287, 9982, 3193, 4665, 7644, 7965, 9983, 7305, 7306, 9460, 2]
// Exports: default

// Module 9976 (HeroMedia)
...
function HeroMedia(mediaItem) {
  ...
  if ("video" === mediaItem.type) {
    ...
    obj[4] = tmp.heroImage;
    obj[1] = callback(require(8764) /* VideoComponent */.VideoComponent, obj);
    ...
    obj[1] = callback(importDefault(5221), obj7);
...
function ProductPriceSection(sku) {
  const tmp = callback3();
  let obj = require(9979) /* useFormattedSKUPrice */;
...
function SKUNameAndDescriptionSection(sku) {
  ...
  if (!obj.isNullOrEmpty(sku.name)) {
    ...
    obj[2] = sku.name;
    const items = [callback(tmp2(4251).Heading, obj), ];
    ...
    if (!isNullOrEmptyResult) {
      ...
      tmp7Result = tmp7(tmp2(4251).Text, obj1);
    }
    items[1] = tmp7Result;
    obj[1] = items;
...
function ItemDetailsSection(selectedItem) {
  ...
  if (null != selectedItem.labelIconAssetId) {
    let obj = require(4492) /* _httpGetWithCountryCodeQuery */;
  ...
  if (!isNullOrEmptyResult) {
    ...
    obj[2] = trimmed;
    tmp14 = callback(tmp8(4251).Heading, obj);
  }
  const items = [tmp14, , ];
  ...
  if (!isNullOrEmptyResult1) {
    ...
    if (tmp18) {
      ...
      tmp18 = callback(importDefault(5221), obj2);
    }
    const items1 = [tmp18, ];
    const obj4 = { variant: "text-sm/medium", color: "text-muted", children: null };
    obj4[2] = trimmed1;
    items1[1] = callback(tmp8(4251).Text, obj4);
    obj1[1] = items1;
  ...
  if (!isNullOrEmptyResult2) {
    ...
    obj5[2] = trimmed2;
    tmp23 = callback(tmp8(4251).Text, obj5);
  }
  items[2] = tmp23;
  obj[1] = items;
...
function SocialLayerStorefrontProductDetailsModal(skuId) {
  ...
  let obj = analyticsLocations(7544);
  ...
  const tmp17 = analyticsLocations(5583)(() => skuId(_undefined[30]).v4());
  ...
  if (null == stateFromStores) {
    ...
    let result = tmp4(5746).isSlayerSkuAvailableOnThisPlatform(stateFromStores);
    const intl4 = tmp4(1236).intl;
    const stringResult = intl4.string(tmp4(1236).t.boqtTA);
    const items12 = [tmp.container, ];
    const tmp4Result1 = tmp4(5746);
    let num3 = 0;
    ...
    obj3[0] = tmp4(9975).closeSocialLayerStorefrontProductDetailsModal;
    ...
    const items13 = [callback1(tmp4(5230).HeaderBackButton, obj3), ];
    ...
    obj4[3] = name;
    items13[1] = callback1(tmp4(4251).Heading, obj4);
    obj2[1] = items13;
    ...
    if (exclusive) {
      ...
      obj6[1] = items15;
      exclusive = tmp42(tmp2(4676), obj6);
    }
    ...
    if (null != tmp29) {
      ...
      if (tmp42Result) {
        ...
        tmp42Result = tmp42(tmp2(9287), obj9);
      }
    }
    ...
    if (exclusive2) {
      ...
      obj10[1] = tmp42(tmp4(9982).ExclusiveBadge, {});
    ...
    if (!result) {
      ...
      obj19[3] = intl2.string(tmp2(3193).gndWN7);
      tmp42Result2 = tmp42(tmp4(4251).Text, obj19);
    }
    ...
    obj22[4] = tmp59;
    obj22[5] = callback2;
    obj21[1] = callback1(tmp4(4665).Button, obj22);
    ...
    obj23[5] = tmp41;
    items21[1] = callback1(tmp4(7644).IconButton, obj23);
    obj20[1] = items21;
    ...
    if (result) {
      ...
      const mobileFinePrintMessageForApplication = tmp4(9983).getMobileFinePrintMessageForApplication(getOrFetchApplication, stringResult, obj25);
...
function SocialLayerStorefrontProductDetailsModalWithOrderCTX(skuId) {
  ...
  const OTPACOMOrderExperiment = tmp(7305).OTPACOMOrderExperiment;
  ...
  obj[2] = items;
  obj[5] = require(9975) /* SOCIAL_LAYER_STOREFRONT_PRODUCT_DETAILS_MODAL_KEY */.closeSocialLayerStorefrontProductDetailsModal;
  obj = {};
  const merged = Object.assign(skuId);
  obj[6] = closure_15(SocialLayerStorefrontProductDetailsModal, obj);
  return closure_15(importDefault(7306), obj, skuId.skuId);
}
...
export default function WrappedSocialLayerStorefrontProductDetailsModal(arg0) {
  ...
  return callback(require(9460) /* NativePaymentContextProvider */.NativePaymentContextProvider, obj);
};
