...
// Dependencies: [5, 1304, 1372, 4638, 1960, 4637, 1604, 676, 4514, 4849, 8297, 4856, 4858, 4859, 698, 4870, 500, 4282, 9943, 1231, 9967, 4708, 10097, 10098, 12, 2]
...
// Module 9942 (handleLimitedPickerDialog)
import map from "map";
...
function handleSelectKeyboardItem(channelId, item, isIncluded, arg3) {
  ...
  if (isIncluded) {
    ...
    if (null != findUploadResult) {
      importDefault(8297).remove(channelId, findUploadResult.id, tmp13.ChannelMessage);
      const obj5 = importDefault(8297);
    }
    tmp13 = DraftType;
  } else {
    let obj = importDefault(8297);
...
function showSimpleMediaKeyboard(channel) {
  ...
  obj = {
    ...
    onClose: _require(9943).hideMediaKeyboardActionSheet,
    onBack: _require(9943).hideMediaKeyboardActionSheet
  };
...
export const addImagesFromPicker = function addImagesFromPicker(id, items, IMAGE_PICKER) {
  ...
  if (IMAGE_PICKER !== _require(4849).UploadOrigin.FILE_ATTACHMENT) {
    ...
    obj[1] = id;
    obj[2] = DraftType.ChannelMessage;
    importDefault(8297).addFiles(obj);
    const obj3 = importDefault(8297);
  } else {
    obj = importDefault(8297);
    obj = { files: null, channelId: null, draftType: null };
    obj[0] = items;
    obj[1] = id;
    obj[2] = DraftType.ChannelMessage;
...
export const addAttachmentForCommand = function addAttachmentForCommand(channelId, onSelectFiles, result1, outer1_1, IMAGE_PICKER) {
  ...
  importDefault(8297).remove(channelId, outer1_1.option.name, InteractionModal);
  const obj4 = importDefault(8297);
  ...
  importDefault(8297).addFile(obj);
};
...
export const handleViewAllDialog = function handleViewAllDialog(arg0) {
  ...
  obj = require(500) /* set */;
  const tmp = require;
  resolved = importDefault(4859).requestPermission(NativePermissionTypes.PHOTOS);
};
...
export const handleSelectGift = function handleSelectGift(analyticsLocations, chatInput, current2) {
  ...
  obj1[4] = fn;
  _require(9967).openGiftModal(obj1);
};
export const handleSelectThread = function handleSelectThread(channel, chatInput) {
  ...
  if (null != channel) {
    ...
    const tmp6 = require;
    const result = require(10097) /* navigateToThreadCreation */.navigateToThreadCreation(channel, "Plus Button");
    const tmp6Result = require(10097) /* navigateToThreadCreation */;
  }
};
...
export const mediaNodeToUploadItem = function mediaNodeToUploadItem(item) {
  ...
  let obj = { id: uri, origin: require(4849) /* cancel */.UploadOrigin.IMAGE_PICKER, uri: image.uri, originalUri: image.uri, mimeType: image.mimeType, width: image.width, height: image.height, filename: image.filename, playableDuration: image.playableDuration, platform: require(4849) /* cancel */.UploadPlatform.REACT_NATIVE };
  let tmp = null != arg1;
  if (tmp) {
...
export const cropResultToUploadItem = function cropResultToUploadItem(path) {
  ...
  obj[6] = arr;
  obj[7] = require(4849) /* cancel */.UploadPlatform.REACT_NATIVE;
  return obj;
};
