// Module ID: 9695
// Function ID: 9696
// Dependencies: [19, 17, 21, 4193, 712, 500, 4004, 689, 3897, 1581, 1348, 4615, 4721, 4189, 2]
...
const memoResult = importAllResult.memo(function MediaKeyboardBottomSheetActions(onHeightChange) {
  ...
  let importAllResult;
  let closure_4;
  let obj = onHeightChange(4004);
  const gradientValue = obj.useGradientValue(onHeightChange(4004).GradientPercentage.END);
  let hexResult = null;
  ...
  tmpResult1 = onHeightChange(3897);
  dependencyMap = onHeightChange(3897).useToken(tmp6(712).modules.mobile.MEDIA_KEYBOARD_BUTTON_ICON_COLOR_ACTIVE);
  const tmpResult2 = onHeightChange(3897);
  importAllResult = onHeightChange(3897).useToken(tmp6(712).modules.mobile.MEDIA_KEYBOARD_BUTTON_TEXT_COLOR_ACTIVE);
  const tmpResult3 = onHeightChange(3897);
  closure_4 = onHeightChange(3897).useToken(tmp6(712).modules.mobile.MEDIA_KEYBOARD_BUTTON_TEXT_VARIANT);
  ...
  const tmpResult4 = onHeightChange(3897);
  const merged = Object.assign(memo);
  obj.pointerEvents = "none";
  const items2 = [callback(importDefault(4615), obj), ];
  obj1 = { style: tmp9Result.container, children: null };
  const tmp6Result = importDefault(4615);
  obj1[1] = callback(closure_5, {
    ...
    children: overflowButtons.map((accessibilityLabel) => {
      ...
      items[1] = outer1_6(onHeightChange(4189).Text, obj);
      obj[6] = items;
      return outer1_7(onHeightChange(4721).PressableOpacity, obj, arg1);
    })
  });
