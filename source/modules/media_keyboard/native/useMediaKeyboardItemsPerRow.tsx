...
// Module 9938 (useMediaKeyboardItemsPerRow)
...
export const useMediaKeyboardItemsPerRow = function useMediaKeyboardItemsPerRow() {
  const tmp2 = itemsPageSizeRef(4139)();
  let itemsPerRow = 8;
  if (itemsPerRow(4139).WindowSizeClassifier.XLARGE !== tmp2) {
    itemsPerRow = 6;
    if (tmp3(4139).WindowSizeClassifier.LARGE !== tmp2) {
      itemsPerRow = 4;
      if (tmp3(4139).WindowSizeClassifier.NORMAL !== tmp2) {
        itemsPerRow = 3;
        if (tmp3(4139).WindowSizeClassifier.SMALL !== tmp2) {
          const _Error = Error;
