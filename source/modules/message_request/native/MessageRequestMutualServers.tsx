...
// Dependencies: [19, 17, 21, 4481, 5539, 16802, 1233, 12550, 4477, 5084, 2]
// Exports: default

// Module 16801 (MessageRequestMutualServers)
...
export default function MessageRequestMutualServers(textVariant) {
  ...
  if (iconSize === undefined) {
    iconSize = iconSize(5539).GuildIconSizes.XXSMALL_12;
  }
  ...
  if (mutualGuildsForMessageRequests.length > 0) {
    ...
    obj = { count: null };
    obj[0] = length;
    let formatResult = intl2.format(tmp4(1233).t.eE3oep, obj);
  } else {
    const intl = tmp4(1233).intl;
    formatResult = intl.string(tmp4(1233).t.jpY0X5);
  }
  ...
  if (tmp9) {
    ...
    tmp9 = callback(tmp4(12550).GuildIconPile, obj1);
  }
  const items1 = [tmp9, ];
  ...
  obj2[4] = combined;
  items1[1] = callback(iconSize(4477).Text, obj2);
  obj[1] = items1;
  ...
  if (null != onPress) {
    ...
    if (length > 0) {
      ...
      obj3[1] = onPress;
      obj3[2] = tmp7Result;
      tmp11Result = tmp11(tmp4(5084).PressableOpacity, obj3);
    }
  }
  return tmp11Result;
};
