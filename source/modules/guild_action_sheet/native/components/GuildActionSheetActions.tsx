...
// Dependencies: [19, 17, 7245, 1340, 1430, 7349, 1985, 4463, 4669, 1922, 676, 5432, 7821, 4670, 21, 4448, 712, 589, 5959, 1236, 10663, 6147, 4268, 1373, 7241, 4415, 4691, 11111, 2009, 11825, 4641, 5921, 5941, 8994, 1297, 9732, 13617, 1306, 1370, 9692, 13618, 8043, 8970, 4444, 4136, 6210, 5455, 10191, 5450, 13666, 10186, 13667, 9603, 5319, 9667, 9629, 11131, 13668, 4827, 38, 13615, 10143, 10142, 7248, 11117, 13669, 8719, 7894, 11364, 5948, 5949, 4163, 1431, 2]
...
// Module 13616 (HideMutedChannelsOption)
...
import experimentDefault from "experiment" /* 5948 */;
...
import closure_12 from "updateUserGuildSettingsInternal" /* 4669 */;
...
function HideMutedChannelsOption(guild) {
  ...
  return callback2(guild(5959).ActionSheetSwitchRow, obj);
}
function BrowseChannelsOption(guild) {
  ...
  if (!features.has(constants.COMMUNITY)) {
    ...
    tmp8Result = tmp8(tmp3(11825).NewBadge, {});
  }
}
function ChangeIdentityOption(arg0) {
  ...
  return closure_21(_require(5959).ActionSheetRow, obj);
}
function GuildThemePreferenceOption(guild) {
  ...
  if (tmp2) {
    ...
    obj[2] = tmp5;
    tmp6 = callback2(tmp3(5959).ActionSheetSwitchRow, obj);
  }
  return tmp6;
}
function ServerTagOption(guild) {
  ...
  if (result) {
    ...
    const guildTagBadgeUrl = tmp2(8043).getGuildTagBadgeUrl(guild.id, badge, GuildTagBadgeSize.SIZE_16);
    obj = { style: null, children: null };
    ...
    obj1[2] = closure_21(tmp2(8970).BaseGuildTagChiplet, obj2);
    ...
    items[1] = closure_21(tmp2(4444).Text, obj3);
    obj[1] = items;
    obj[0] = closure_22(View, obj);
    obj[1] = tmp5;
    return closure_21(tmp2(5959).ActionSheetRow, obj);
  } else {
    return null;
  }
  tmp = callback4();
...
export const GuildActionSheetPrimaryActions = function GuildActionSheetPrimaryActions(guild) {
  ...
  if (obj.useGuildActionSheetPermissions(guild).canManageChannels) {
    ...
    items1.push(callback2(tmp(5959).ActionSheetRow, obj));
    obj = { label: null, onPress: null };
  ...
  if (tmp3) {
    ...
    items1.push(callback2(tmp(5959).ActionSheetRow, obj1));
  }
  if (stateFromStores) {
...
export const GuildActionSheetDirectoryActions = function GuildActionSheetDirectoryActions(guild) {
  ...
  if (tmp4) {
    ...
    if (0 !== items.length) {
      ...
      tmp5Result = tmp5(tmp9(5959).ActionSheetRow.Group, obj);
    }
    return tmp5Result;
  } else {
    const features = guild.features;
    ...
    t = items.push(tmp5(tmp9(5959).ActionSheetRow, obj));
  }
};
export const GuildActionSheetSecondaryActions = function GuildActionSheetSecondaryActions(guild) {
  ...
  if (features.has(constants.COMMUNITY)) {
    ...
    items.push(tmp8(tmp3(5959).ActionSheetSwitchRow, obj));
  }
  items.push(callback2(HideMutedChannelsOption, { guild }));
  items.push(callback2(RestrictedGuildPrivacyOption, { guild }));
  const messageRequestPrivacyOption = guild(13615).useMessageRequestPrivacyOption({ guild });
  if (null != messageRequestPrivacyOption) {
    items.push(messageRequestPrivacyOption);
  }
  if (canReportRaid) {
    ...
    items.push(tmp8(tmp3(5959).ActionSheetRow, obj));
  }
  if (!tmp2) {
    obj1 = { label: null, onPress: null };
    const intl3 = tmp3(1236).intl;
    obj1[0] = intl3.string(tmp3(1236).t.Aen9eh);
    obj1[1] = function onPress() {
      closure_1_1(closure_1_2[25]).hideActionSheet();
      const obj = closure_1_1(closure_1_2[25]);
      const result = guild(closure_1_2[66]).showReportModalForGuild(guild);
    };
    items.push(tmp8(tmp3(5959).ActionSheetRow, obj1));
  }
  if (!shouldShowIncidentActions) {
    if (tmp2) {
      ...
      if (0 !== items.length) {
        ...
        tmp8Result = tmp8(tmp3(5959).ActionSheetRow.Group, obj2);
      }
      return tmp8Result;
    } else {
      ...
      t2 = items.push(tmp8(tmp3(5959).ActionSheetRow, obj3));
    }
  } else {
    ...
    t = items.push(tmp8(tmp3(5959).ActionSheetRow, obj4));
  }
};
export const GuildDeveloperOptionAction = function GuildDeveloperOptionAction(guild) {
  guild = guild.guild;
  const DeveloperMode = guild(4136).DeveloperMode;
  ...
  if (setting) {
    ...
    items.push(callback2(tmp(5959).ActionSheetRow, obj));
    if (tmp5) {
      if (null != guild.icon) {
        obj = { label: null, onPress: null };
        const intl2 = tmp(1236).intl;
        ...
        items.push(tmp7(tmp(5959).ActionSheetRow, obj));
      }
    ...
    return callback2(tmp(5959).ActionSheetRow.Group, obj1);
  } else {
    return null;
  }
};
