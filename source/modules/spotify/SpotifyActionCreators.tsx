...
export const fetchIsSpotifyProtocolRegistered = function fetchIsSpotifyProtocolRegistered() {
  if (!protocolRegistered.isProtocolRegistered()) {
    if (obj.isDesktop()) {
      const obj2 = importDefault(9281);
      importDefault(9281).isProtocolRegistered(closure_4).then((isRegistered) => {
        let obj = callback(table[5]);
        obj = { type: "SPOTIFY_SET_PROTOCOL_REGISTERED", isRegistered };
        obj.dispatch(obj);
      });
      const isProtocolRegisteredResult = importDefault(9281).isProtocolRegistered(closure_4);
    }
    obj = require(500) /* set */;
  }
};
