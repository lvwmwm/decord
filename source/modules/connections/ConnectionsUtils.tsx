...
// Dependencies: [1910, 1838, 3758, 4940, 653, 1443, 44, 1212, 22, 4941, 566, 3946, 1334, 2]
...
// Module 4939 (officialApplicationIds)
...
let closure_9;
const require = arg1;
({ MetadataFields: closure_6, OperatorTypes: closure_7 } = OperatorTypes);
({ Permissions: closure_8, PlatformTypes: closure_9 } = ME);
let result = require("_isNativeReflectConstruct").fileFinishedImporting("modules/connections/ConnectionsUtils.tsx");
...
export const getCallbackParamsFromURL = function getCallbackParamsFromURL(outer1_1) {
  let code;
  let error;
  let error_description;
  let state;
  let obj = importDefault(1443);
  let toURLSafeResult = obj.toURLSafe(outer1_1);
  ...
  importDefault(44)(!Array.isArray(code), "Received multiple query param values for code");
  const tmp8 = importDefault(44);
  importDefault(44)(!Array.isArray(state), "Received multiple query param values for state");
  const tmp10 = importDefault(44);
  importDefault(44)(!Array.isArray(error), "Received multiple query param values for error");
  const tmp12 = importDefault(44);
  importDefault(44)(!Array.isArray(errorDescription), "Received multiple query param values for error_description");
...
export const getConnectionsCheckText = function getConnectionsCheckText(value) {
  ...
  if (constants.EQUAL === operator) {
    ...
    let H97H4S = _require(1212).t.H97H4S;
    if (tmp14) {
      H97H4S = _require(1212).t["N95b+f"];
    }
  } else if (constants.NOT_EQUAL === operator) {
    H97H4S = _require(1212).t["D9B/q2"];
  } else if (constants.LESS_THAN === operator) {
    H97H4S = _require(1212).t["3ru8/N"];
    const _Math2 = Math;
    _require = Math.max(0, rounded - 1);
  } else if (constants.GREATER_THAN === operator) {
    H97H4S = _require(1212).t.wCVDHn;
    const _Math = Math;
    _require = Math.max(0, rounded + 1);
  } else {
      ...
      if (constants2.REDDIT === connectionType) {
          ...
          if (closure_6.REDDIT_TOTAL_KARMA === connectionMetadataField) {
            ...
            if (closure_6.REDDIT_GOLD === connectionMetadataField) {
              ...
              const obj1 = {
                platformQuantityHook() {
                              const intl = callback(outer1_2[7]).intl;
                              return intl.string(callback(outer1_2[7]).t["+/5TCx"]);
                            }
              };
              ...
              const obj2 = {
                platformQuantityHook() {
                              const intl = callback(outer1_2[7]).intl;
                              return intl.string(callback(outer1_2[7]).t["9rPbEs"]);
                            }
              };
        ...
        if (closure_6.CREATED_AT === connectionMetadataField) {
          ...
          const obj11 = {
            platformQuantityHook() {
                      const intl = callback(outer1_2[7]).intl;
                      return intl.string(callback(outer1_2[7]).t.xRygZL);
                    }
          };
        ...
        if (closure_6.CREATED_AT === connectionMetadataField) {
          ...
          const obj15 = {
            platformQuantityHook() {
                      const intl = callback(outer1_2[7]).intl;
                      return intl.string(callback(outer1_2[7]).t.slSQuB);
                    }
          };
        ...
        if (closure_6.CREATED_AT === connectionMetadataField) {
          ...
          const obj17 = {
            platformQuantityHook() {
                      const intl = callback(outer1_2[7]).intl;
                      return intl.string(callback(outer1_2[7]).t.TEEYwa);
                    }
          };
        ...
        if (closure_6.TIKTOK_VERIFIED === connectionMetadataField) {
          ...
          const obj21 = {
            platformQuantityHook() {
                      const intl = callback(outer1_2[7]).intl;
                      return intl.string(callback(outer1_2[7]).t.uv7ety);
                    }
          };
...
export const getVisibleConnectionsRole = function getVisibleConnectionsRole(guildMember) {
  ...
  if (null == guildMember) {
    ...
    if (null == guild) {
      ...
      const intersectionResult = importDefault(22).intersection(found, importDefault(4941)(channel));
