...
// Dependencies: [19, 17, 21, 4189, 712, 4050, 4190, 4395, 11439, 4185, 1236, 2]
// Exports: default

// Module 16189 (NewMemberActionsCompleted)
...
export default function NewMemberActionsCompleted(arg0) {
  ...
  obj = { withDelay: sharedValue(4050).withDelay, withTiming: sharedValue(4190).withTiming, barWidth: sharedValue };
  fn.__closure = obj;
  fn.__workletHash = 7643178959760;
  fn.__initData = closure_8;
  const animatedStyle = obj1.useAnimatedStyle(fn);
  const effect1 = React.useEffect(() => {
    const timerId = setTimeout(() => callback2(4395).popWithKey(callback(11439).NEW_MEMBER_ACTION_COMPLETE_MODAL_KEY), 2500);
  }, []);
  ...
  const items1 = [callback(sharedValue(4185).Text, obj1), ];
  const items2 = [tmp.progressForeground, animatedStyle];
  items1[1] = callback(View, { style: tmp.progressBackground, children: callback(importDefault(4050).View, { style: items2 }) });
  obj[1] = items1;
