...
// Function ID: 15642
// Name: MessageContent
// Dependencies: [19, 17, 4247, 1942, 1874, 5816, 676, 505, 21, 4255, 712, 589, 7134, 4693, 5185, 4251, 1236, 4094, 5820, 4483, 11, 1297, 7983, 7852, 4119, 2]
// Exports: ThreadSubtext

// Module 15641 (MessageContent)
...
function MessageContent(arg0) {
  ...
  ({ nick: c2, colorString: c3, colorStrings: c4 } = importDefault(4483)(message));
  let tmp4 = importDefault(4483)(message);
  const extractTimestampResult = importDefault(11).extractTimestamp(message.id);
  const obj4 = importDefault(11);
  const timestampString = message(7134).getTimestampString(extractTimestampResult);
  const obj5 = message(7134);
  const timestampAccessibilityLabel = message(7134).getTimestampAccessibilityLabel(extractTimestampResult);
  maybeApplyNoTextColorForLightCustomTheme = importDefault(4693)(thread.guild_id, stateFromStores.id);
...
export const ThreadSubtext = function ThreadSubtext(thread) {
  ...
  const lastMessageTimestamp = id(7134).useLastMessageTimestamp(thread);
  if (null != stateFromStores) {
