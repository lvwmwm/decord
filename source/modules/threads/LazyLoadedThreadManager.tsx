...
function loadThread(channelId) {
  ...
  if (null == channelId) {
  ...
  } else if (channelId === _require(5657).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID) {
    return Promise.resolve();
    ...
    if (_handleConnectionOpen.isConnected()) {
      if (null != dependencyMap[channelId]) {
        ...
        let tmp13Result = tmp13(4011);
        const _location = location;
        obj = { path: null, exact: true };
        const RouteParam = tmp13(4024).RouteParam;
        const RouteParam2 = tmp13(4024).RouteParam;
  ...
  loadThreadsBulk(arr) {
    ...
    if (_handleConnectionOpen.isConnected()) {
      ...
      while (iter !== undefined) {
        ...
        if (nextResult !== items1(5657).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID) {
