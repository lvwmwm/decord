...
// Function ID: 5223
// Name: rebuild
// Dependencies: [1376, 1372, 1891, 4314, 1960, 5223, 3931, 1379, 11, 4515, 5224, 709, 12, 589, 2]
        ...
        if (null == channel) {
          ...
          if (null != joinTimestampResult) {
            ...
            if (!isTimedRelevant) {
              ...
              let _Date = Date;
              let tmp51 = importDefault(5224)(channel);
      ...
      if (handleThreadCreateOrUpdate.isActive(guild_id, parent_id, id)) {
        if (null != joinTimestampResult) {
          ...
          if (isTimedRelevant) {
            ...
            clearTimeoutResult = importDefault(5224)(_Date);
            _Date = Date;
  ...
  if (null == parent_id) {
    ...
    if (null != tmp) {
      for (const key10006 in tmp) {
        ...
        let obj = require(4515) /* shouldShowAgeGateForVoiceChannel */;
    ...
    if (null != tmp5) {
      for (const key10015 in tmp5) {
        ...
        let obj2 = require(4515) /* shouldShowAgeGateForVoiceChannel */;
      ...
      if (null == parent_id4) {
        ...
        if (tmp10) {
          ...
          if (tmp23.isTimedRelevant) {
            ...
            }, importDefault(5224)(channel) - Date.now() + 1);
            const tmp33 = importDefault(5224)(channel);
          }
        ...
        if (keys1 === undefined) {
          ...
          while (tmp6 !== undefined) {
            ...
            if (!isTimedRelevant) {
              ...
              let _Date = Date;
              let tmp22 = importDefault(5224)(channel);
              dependencyMap7[channel.id] = setTimeout(() => {
                channel = outer1_5.getChannel(channel.id);
                if (null != channel) {
                  let obj = outer1_1(outer1_2[11]);
...
function parseThreadState(channel) {
  ...
  if (isActiveThreadResult) {
    const _Date = Date;
    tmp6 = importDefault(5224)(channel) > Date.now();
    const tmp9 = importDefault(5224)(channel);
  }
