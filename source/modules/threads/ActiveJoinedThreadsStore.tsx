...
// Function ID: 5264
// Name: rebuild
// Dependencies: [1395, 1391, 1910, 4357, 1979, 5264, 3991, 1398, 11, 4558, 5265, 709, 12, 589, 2]
        ...
        if (null == channel) {
          ...
          if (null != joinTimestampResult) {
            ...
            if (!isTimedRelevant) {
              ...
              let _Date = Date;
              let tmp51 = importDefault(5265)(channel);
      ...
      if (handleThreadCreateOrUpdate.isActive(guild_id, parent_id, id)) {
        if (null != joinTimestampResult) {
          ...
          if (isTimedRelevant) {
            ...
            clearTimeoutResult = importDefault(5265)(_Date);
            _Date = Date;
  ...
  if (null == parent_id) {
    ...
    if (null != tmp5) {
      for (const key10015 in tmp5) {
        ...
        let obj2 = require(4558) /* shouldShowAgeGateForVoiceChannel */;
      ...
      if (null == parent_id4) {
        ...
        if (tmp10) {
          ...
          if (tmp23.isTimedRelevant) {
            ...
            }, importDefault(5265)(channel) - Date.now() + 1);
            const tmp33 = importDefault(5265)(channel);
          }
        ...
        if (keys1 === undefined) {
          ...
          while (tmp6 !== undefined) {
            ...
            if (!isTimedRelevant) {
              ...
              let _Date = Date;
              let tmp22 = importDefault(5265)(channel);
              dependencyMap7[channel.id] = setTimeout(() => {
                channel = outer1_5.getChannel(channel.id);
                if (null != channel) {
                  let obj = outer1_1(outer1_2[11]);
...
function parseThreadState(channel) {
  ...
  if (isActiveThreadResult) {
    const _Date = Date;
    tmp6 = importDefault(5265)(channel) > Date.now();
    const tmp9 = importDefault(5265)(channel);
  }
