...
// Dependencies: [19, 5128, 1372, 1931, 676, 1877, 1338, 8539, 8540, 4449, 3900, 2]
// Exports: useTrackOnEmojiPickerOpenedForReactions

// Module 8571 (trackOnEmojiPickerOpened)
...
function trackOnEmojiPickerOpened(arg0) {
  ...
  const topAndNewlyAddedEmojis = require(8539) /* useTopAndNewlyAddedEmojis */.getTopAndNewlyAddedEmojis({ guildId: guildId2, pickerIntention: intention });
  ({ topEmojis, newlyAddedEmojis } = topAndNewlyAddedEmojis);
  const obj4 = require(8539) /* useTopAndNewlyAddedEmojis */;
  const tmp6 = require;
  const emojiHotrail = require(8540) /* useEmojiHotrail */.getEmojiHotrail({ topEmojis, newlyAddedEmojis, rowSize });
  ({ visibleTopEmojis, visibleNewlyAddedEmojis } = emojiHotrail);
  const tmp6Result = require(8540) /* useEmojiHotrail */;
  if (intention === EmojiIntention.REACTION) {
  ...
  let obj = {
    ...
    num_custom_expressions_favorites: prop.filter(tmp11(3900).isCustomEmoji).length,
    ...
    num_custom_expressions_frecent: substr1.filter(tmp11(3900).isCustomEmoji).length,
  ...
  importDefault(4449).trackWithMetadata(EXPRESSION_PICKER_OPENED, obj);
}
