...
// Dependencies: [19, 5158, 1372, 1960, 676, 1906, 1338, 8568, 8569, 4479, 3929, 2]
// Exports: useTrackOnEmojiPickerOpenedForReactions

// Module 8600 (trackOnEmojiPickerOpened)
...
function trackOnEmojiPickerOpened(arg0) {
  ...
  const topAndNewlyAddedEmojis = require(8568) /* useTopAndNewlyAddedEmojis */.getTopAndNewlyAddedEmojis({ guildId: guildId2, pickerIntention: intention });
  ({ topEmojis, newlyAddedEmojis } = topAndNewlyAddedEmojis);
  const obj4 = require(8568) /* useTopAndNewlyAddedEmojis */;
  const tmp6 = require;
  const emojiHotrail = require(8569) /* useEmojiHotrail */.getEmojiHotrail({ topEmojis, newlyAddedEmojis, rowSize });
  ({ visibleTopEmojis, visibleNewlyAddedEmojis } = emojiHotrail);
  const tmp6Result = require(8569) /* useEmojiHotrail */;
  if (intention === EmojiIntention.REACTION) {
  ...
  let obj = {
    ...
    num_custom_expressions_favorites: prop.filter(tmp11(3929).isCustomEmoji).length,
    ...
    num_custom_expressions_frecent: substr1.filter(tmp11(3929).isCustomEmoji).length,
