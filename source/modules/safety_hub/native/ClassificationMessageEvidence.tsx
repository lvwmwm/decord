...
// Dependencies: [32, 19, 17, 1302, 1874, 11165, 7700, 676, 21, 7823, 4255, 589, 4101, 11175, 11176, 7699, 4442, 8738, 8765, 11177, 8371, 4474, 11, 2]
// Exports: default

// Module 11174 (ClassificationEvidence)
...
export default function ClassificationEvidence(arg0) {
  ...
  if (obj2.useStateFromStores(items1, () => ref(4101).isThemeLight(id.theme))) {
    let assetSource = resolveAssetSource(tmp11(11175));
    let tmp13 = tmp11;
  } else {
    assetSource = resolveAssetSource(tmp11(11176));
    tmp13 = tmp11;
  }
  ...
  const callback = obj1.useCallback((nativeEvent) => {
    ...
    ref(8738).openMediaModal({ initialIndex: index, initialSources: mapped, disableDownload: true, disableMediaOverlayButton: true, shareable: false, originViewOrOriginLayout: layout });
  }, items4);
  ...
  if (first3 > 0) {
    ...
    first2 = attachments.filter((filename) => ref(4442).isVideoFile(filename.filename));
    obj[1] = first2.map((uri) => {
      ...
      obj = { uri: uri.url };
      return outer1_18(first(8765), obj, arg1);
    });
    ...
    obj1[0] = ref;
    obj1[1] = callback;
    const items9 = [callback(tmp13(11177), obj1), ];
    obj2 = { rowGenerator: null, message: null, modifyRow: null };
    obj2[0] = closure_20;
    let tmp13Result = tmp13(8371);
    tmpResult = tmp(4474);
