...
// Dependencies: [19, 17, 8017, 21, 4444, 712, 4173, 8185, 8016, 11519, 709, 4411, 11541, 2009, 1236, 11534, 4440, 2]
// Exports: default

// Module 11540 (AppealSignalRadioRow)
...
function AppealSignalRadioRow(signal) {
  ...
  obj = { text: signal(8016).getAppealSignalDisplayText(signal) };
  obj[1] = callback(signal(8185).FormRow.Label, obj);
  obj[2] = function onPress() {
    return callback(signal);
  };
  obj[3] = callback(signal(8185).FormRow.Radio, { selected });
  obj[4] = accessibilityRole;
  obj[5] = accessibilityState;
  return callback(signal(8185).FormRow, obj);
}
noopAll;
...
export default function AppealIngestionCollectSignal(isDsaEligible) {
  ...
  function handleAppealSignalSelect(signal) {
    ...
    if (tmp4) {
      ...
      obj[0] = function onSave(userInput) {
        ...
        obj.dispatch(obj);
        callback(4411).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
      };
      obj[1] = function onClose() {
        return callback(4411).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
      };
  ...
  obj = { children: null };
  const items = [callback(isDsaEligible(11534).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(isDsaEligible(1236).t.VEcRhw) }), ];
  obj = { style: tmp.container, children: null };
  obj1 = { style: tmp.form, children: null };
  const stringResult1 = intl2.string(isDsaEligible(1236).t.VEcRhw);
  const items1 = [callback(isDsaEligible(8185).FormSection, { sectionBodyStyle: tmp.formSection, accessibilityRole: "radiogroup", children: closure_5.map((signal) => closure_1_6(closure_1_9, { signal, selected: signal === closure_2, rowStyle: lib.formRow, onSelect: handleAppealSignalSelect }, "formrow-" + arg1)) }), ];
  ...
  obj3[1] = callback(isDsaEligible(4440).Text, obj4);
  items1[1] = callback(handleAppealSignalSelect, obj3);
  obj1[1] = items1;
  obj[1] = callback2(isDsaEligible(8185).Form, obj1);
  items[1] = callback(handleAppealSignalSelect, obj);
  obj[0] = items;
  return callback2(isDsaEligible(11534).AppealIngestionModalScreen, obj);
};
