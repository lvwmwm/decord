...
// Dependencies: [19, 17, 7871, 21, 4342, 712, 4073, 8033, 7870, 11353, 709, 4310, 11375, 2007, 1236, 11368, 4338, 2]
// Exports: default

// Module 11374 (AppealSignalRadioRow)
import "noop";
...
function AppealSignalRadioRow(signal) {
  ...
  obj = { text: null };
  obj[0] = signal(7870).getAppealSignalDisplayText(signal);
  obj[1] = callback(signal(8033).FormRow.Label, obj);
  obj[2] = function onPress() {
    return callback(signal);
  };
  obj[3] = callback(signal(8033).FormRow.Radio, { selected });
  obj[4] = accessibilityRole;
  obj[5] = accessibilityState;
  return callback(signal(8033).FormRow, obj);
}
...
export default function AppealIngestionCollectSignal(isDsaEligible) {
  ...
  obj = { children: null };
  const items = [callback(isDsaEligible(11368).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(isDsaEligible(1236).t.VEcRhw) }), ];
  ...
  const items1 = [callback(isDsaEligible(8033).FormSection, obj2), ];
  ...
  obj3[1] = callback(isDsaEligible(4338).Text, obj4);
  items1[1] = callback(handleAppealSignalSelect, obj3);
  obj1[1] = items1;
  obj[1] = callback2(isDsaEligible(8033).Form, obj1);
  items[1] = callback(handleAppealSignalSelect, obj);
  obj[0] = items;
  return callback2(isDsaEligible(11368).AppealIngestionModalScreen, obj);
};
