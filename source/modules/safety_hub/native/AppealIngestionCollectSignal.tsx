...
// Dependencies: [19, 17, 7745, 21, 4302, 712, 4019, 7907, 7744, 11219, 709, 4270, 11241, 1988, 1236, 11234, 4298, 2]
// Exports: default

// Module 11240 (AppealSignalRadioRow)
import "noop";
...
function AppealSignalRadioRow(signal) {
  ...
  obj = { text: null };
  obj[0] = signal(7744).getAppealSignalDisplayText(signal);
  obj[1] = callback(signal(7907).FormRow.Label, obj);
  obj[2] = function onPress() {
    return callback(signal);
  };
  obj[3] = callback(signal(7907).FormRow.Radio, { selected });
  obj[4] = accessibilityRole;
  obj[5] = accessibilityState;
  return callback(signal(7907).FormRow, obj);
}
...
export default function AppealIngestionCollectSignal(isDsaEligible) {
  ...
  function handleAppealSignalSelect(signal) {
    ...
    if (tmp4) {
      ...
      obj[0] = function onSave(userInput) {
        ...
        obj.dispatch(obj);
        callback(4270).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
      };
      obj[1] = function onClose() {
        return callback(4270).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
      };
  ...
  obj = { children: null };
  const items = [callback(isDsaEligible(11234).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(isDsaEligible(1236).t.VEcRhw) }), ];
  ...
  const items1 = [callback(isDsaEligible(7907).FormSection, obj2), ];
  ...
  obj3[1] = callback(isDsaEligible(4298).Text, obj4);
  items1[1] = callback(handleAppealSignalSelect, obj3);
  obj1[1] = items1;
  obj[1] = callback2(isDsaEligible(7907).Form, obj1);
  items[1] = callback(handleAppealSignalSelect, obj);
  obj[0] = items;
  return callback2(isDsaEligible(11234).AppealIngestionModalScreen, obj);
};
