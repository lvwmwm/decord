...
// Dependencies: [19, 17, 8636, 21, 4446, 712, 4174, 8309, 8635, 11392, 709, 4413, 11414, 2010, 1236, 11407, 4442, 2]
// Exports: default

// Module 11413 (AppealSignalRadioRow)
...
function AppealSignalRadioRow(signal) {
  ...
  obj = { text: signal(8635).getAppealSignalDisplayText(signal) };
  obj[1] = callback(signal(8309).FormRow.Label, obj);
  obj[2] = function onPress() {
    return callback(signal);
  };
  obj[3] = callback(signal(8309).FormRow.Radio, { selected });
  obj[4] = accessibilityRole;
  obj[5] = accessibilityState;
  return callback(signal(8309).FormRow, obj);
}
noopAll;
...
export default function AppealIngestionCollectSignal(isDsaEligible) {
  ...
  function handleAppealSignalSelect(signal) {
    ...
    if (tmp4) {
      ...
      obj[0] = function onSave(userInput) {
        ...
        obj.dispatch(obj);
        callback(4413).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
      };
      obj[1] = function onClose() {
        return callback(4413).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
      };
  ...
  obj = { children: null };
  const items = [callback(isDsaEligible(11407).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(isDsaEligible(1236).t.VEcRhw) }), ];
  obj = { style: tmp.container, children: null };
  obj1 = { style: tmp.form, children: null };
  const stringResult1 = intl2.string(isDsaEligible(1236).t.VEcRhw);
  const items1 = [callback(isDsaEligible(8309).FormSection, { sectionBodyStyle: tmp.formSection, accessibilityRole: "radiogroup", children: closure_5.map((signal) => closure_1_6(closure_1_9, { signal, selected: signal === closure_2, rowStyle: lib.formRow, onSelect: handleAppealSignalSelect }, "formrow-" + arg1)) }), ];
  ...
  obj3[1] = callback(isDsaEligible(4442).Text, obj4);
  items1[1] = callback(handleAppealSignalSelect, obj3);
  obj1[1] = items1;
  obj[1] = callback2(isDsaEligible(8309).Form, obj1);
  items[1] = callback(handleAppealSignalSelect, obj);
  obj[0] = items;
  return callback2(isDsaEligible(11407).AppealIngestionModalScreen, obj);
};
