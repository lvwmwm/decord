...
// Dependencies: [19, 17, 7846, 21, 4344, 712, 4073, 8008, 7845, 11345, 709, 4312, 11367, 2007, 1236, 11360, 4340, 2]
// Exports: default

// Module 11366 (AppealSignalRadioRow)
import "noop";
...
function AppealSignalRadioRow(signal) {
  ...
  obj = { text: null };
  obj[0] = signal(7845).getAppealSignalDisplayText(signal);
  obj[1] = callback(signal(8008).FormRow.Label, obj);
  obj[2] = function onPress() {
    return callback(signal);
  };
  obj[3] = callback(signal(8008).FormRow.Radio, { selected });
  obj[4] = accessibilityRole;
  obj[5] = accessibilityState;
  return callback(signal(8008).FormRow, obj);
}
...
export default function AppealIngestionCollectSignal(isDsaEligible) {
  ...
  obj = { children: null };
  const items = [callback(isDsaEligible(11360).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(isDsaEligible(1236).t.VEcRhw) }), ];
  ...
  const items1 = [callback(isDsaEligible(8008).FormSection, obj2), ];
  ...
  obj3[1] = callback(isDsaEligible(4340).Text, obj4);
  items1[1] = callback(handleAppealSignalSelect, obj3);
  obj1[1] = items1;
  obj[1] = callback2(isDsaEligible(8008).Form, obj1);
  items[1] = callback(handleAppealSignalSelect, obj);
  obj[0] = items;
  return callback2(isDsaEligible(11360).AppealIngestionModalScreen, obj);
};
