...
// Dependencies: [19, 17, 8349, 21, 4481, 709, 4206, 8523, 8348, 11812, 706, 4448, 11834, 2008, 1233, 11818, 4477, 2]
// Exports: default

// Module 11833 (AppealSignalRadioRow)
...
function AppealSignalRadioRow(signal) {
  ...
  obj = { text: signal(8348).getAppealSignalDisplayText(signal) };
  obj[1] = callback(signal(8523).FormRow.Label, obj);
  obj[2] = function onPress() {
    return callback(signal);
  };
  obj[3] = callback(signal(8523).FormRow.Radio, { selected });
  obj[4] = accessibilityRole;
  obj[5] = accessibilityState;
  return callback(signal(8523).FormRow, obj);
}
noopAll;
...
export default function AppealIngestionCollectSignal(isDsaEligible) {
  ...
  function handleAppealSignalSelect(signal) {
    ...
    if (tmp4) {
      ...
      obj[0] = function onSave(userInput) {
        ...
        obj.dispatch(obj);
        callback(4448).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
      };
      obj[1] = function onClose() {
        return callback(4448).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
      };
  ...
  obj = { children: null };
  const items = [callback(isDsaEligible(11818).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(isDsaEligible(1233).t.VEcRhw) }), ];
  obj = { style: tmp.container, children: null };
  obj1 = { style: tmp.form, children: null };
  const stringResult1 = intl2.string(isDsaEligible(1233).t.VEcRhw);
  const items1 = [callback(isDsaEligible(8523).FormSection, { sectionBodyStyle: tmp.formSection, accessibilityRole: "radiogroup", children: closure_5.map((signal) => closure_1_6(closure_1_9, { signal, selected: signal === closure_2, rowStyle: lib.formRow, onSelect: handleAppealSignalSelect }, "formrow-" + arg1)) }), ];
  ...
  obj4[1] = intl3.format(isDsaEligible(1233).t["8k9GCW"], {});
  obj3[1] = callback(isDsaEligible(4477).Text, obj4);
  items1[1] = callback(handleAppealSignalSelect, obj3);
  obj1[1] = items1;
  obj[1] = callback2(isDsaEligible(8523).Form, obj1);
  items[1] = callback(handleAppealSignalSelect, obj);
  obj[0] = items;
  return callback2(isDsaEligible(11818).AppealIngestionModalScreen, obj);
};
