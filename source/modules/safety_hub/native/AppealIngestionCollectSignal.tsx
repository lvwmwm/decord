...
// Dependencies: [19, 17, 6759, 21, 4193, 712, 3911, 7639, 6758, 11091, 709, 4161, 11108, 1959, 1236, 11101, 4189, 2]
// Exports: default
...
function AppealSignalRadioRow(signal) {
  ...
  obj = { text: null };
  obj[0] = signal(6758).getAppealSignalDisplayText(signal);
  obj[1] = callback(signal(7639).FormRow.Label, obj);
  obj[2] = function onPress() {
    return callback(signal);
  };
  obj[3] = callback(signal(7639).FormRow.Radio, { selected });
  obj[4] = accessibilityRole;
  obj[5] = accessibilityState;
  return callback(signal(7639).FormRow, obj);
}
...
export default function AppealIngestionCollectSignal(isDsaEligible) {
  ...
  function handleAppealSignalSelect(signal) {
    ...
    if (tmp4) {
      ...
      obj[0] = function onSave(userInput) {
        ...
        obj.dispatch(obj);
        callback(4161).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
      };
      obj[1] = function onClose() {
        return callback(4161).hideActionSheet("AppealIngestionFreeTextAppealReasonActionSheet");
      };
  ...
  obj = { children: null };
  const items = [callback(isDsaEligible(11101).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(isDsaEligible(1236).t.VEcRhw) }), ];
  ...
  const items1 = [callback(isDsaEligible(7639).FormSection, obj2), ];
  ...
  obj3[1] = callback(isDsaEligible(4189).Text, obj4);
  items1[1] = callback(handleAppealSignalSelect, obj3);
  obj1[1] = items1;
  obj[1] = callback2(isDsaEligible(7639).Form, obj1);
  items[1] = callback(handleAppealSignalSelect, obj);
  obj[0] = items;
  return callback2(isDsaEligible(11101).AppealIngestionModalScreen, obj);
};
