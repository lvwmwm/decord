...
// Dependencies: [19, 17, 7716, 21, 4255, 712, 3973, 7765, 7715, 11207, 709, 4223, 11224, 1959, 1236, 11217, 4251, 2]
// Exports: default

// Module 11223 (AppealSignalRadioRow)
import "noop";
...
function AppealSignalRadioRow(signal) {
  ...
  obj = { text: null };
  obj[0] = signal(7715).getAppealSignalDisplayText(signal);
  obj[1] = callback(signal(7765).FormRow.Label, obj);
  obj[2] = function onPress() {
    return callback(signal);
  };
  obj[3] = callback(signal(7765).FormRow.Radio, { selected });
  obj[4] = accessibilityRole;
  obj[5] = accessibilityState;
  return callback(signal(7765).FormRow, obj);
}
...
export default function AppealIngestionCollectSignal(isDsaEligible) {
  ...
  obj = { children: null };
  const items = [callback(isDsaEligible(11217).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(isDsaEligible(1236).t.VEcRhw) }), ];
  ...
  const items1 = [callback(isDsaEligible(7765).FormSection, obj2), ];
  ...
  obj1[1] = items1;
  obj[1] = callback2(isDsaEligible(7765).Form, obj1);
  items[1] = callback(handleAppealSignalSelect, obj);
  obj[0] = items;
  return callback2(isDsaEligible(11217).AppealIngestionModalScreen, obj);
};
