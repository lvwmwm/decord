...
// Dependencies: [19, 17, 11280, 7805, 676, 21, 4303, 589, 11285, 11302, 1236, 11300, 11303, 11304, 11305, 4299, 2]
// Exports: default

// Module 11301 (AppealIngestionSpeedBump)
import "noop";
...
export default function AppealIngestionSpeedBump(arg0) {
  ...
  const tmp = callback2();
  _require = _require(11302).useEmitAppealIngestionEvent();
  ...
  const tmp2Result = _require(11302);
  const intl2 = tmp2(1236).intl;
  const stringResult = intl.string(_require(1236).t["C5q+pW"]);
  const items1 = [callback(_require(11300).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(_require(1236).t.URt7VI) }), ];
  obj = { style: tmp.container, children: null };
  let tmp9Result = flagged_content.length > 0;
  if (tmp9Result) {
    obj = { flaggedContent: null };
    obj[0] = flagged_content;
    tmp9Result = tmp9(importDefault(11303), obj);
  }
  const items2 = [tmp9Result, , , , , ];
  obj1 = { classification: safetyHubClassification.classification };
  items2[1] = callback(importDefault(11304), obj1);
  tmp9Result = isCoppa;
  if (isCoppa) {
  ...
  if (!isCoppa) {
    ...
    tmp9Result1 = tmp9(tmp13(11305), obj4);
    const tmp13Result1 = tmp13(11305);
  }
  ...
  obj6[0] = items1;
  return closure_9(_require(11300).AppealIngestionModalScreen, obj6);
};
