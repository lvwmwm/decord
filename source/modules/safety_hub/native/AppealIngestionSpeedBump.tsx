...
// Dependencies: [19, 17, 11068, 6753, 676, 21, 4189, 589, 11073, 11085, 1236, 11083, 11086, 11087, 11088, 4185, 2]
// Exports: default

// Module 11084 (AppealIngestionSpeedBump)
import "noop";
...
export default function AppealIngestionSpeedBump(arg0) {
  ...
  const tmp = callback2();
  _require = _require(11085).useEmitAppealIngestionEvent();
  ...
  const tmp2Result = _require(11085);
  const intl2 = tmp2(1236).intl;
  const stringResult = intl.string(_require(1236).t["C5q+pW"]);
  const items1 = [callback(_require(11083).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(_require(1236).t.URt7VI) }), ];
  obj = { style: tmp.container, children: null };
  let tmp9Result = flagged_content.length > 0;
  if (tmp9Result) {
    obj = { flaggedContent: null };
    obj[0] = flagged_content;
    tmp9Result = tmp9(importDefault(11086), obj);
  }
  const items2 = [tmp9Result, , , , , ];
  obj1 = { classification: safetyHubClassification.classification };
  items2[1] = callback(importDefault(11087), obj1);
  tmp9Result = isCoppa;
  if (isCoppa) {
    const obj2 = { text: null, url: null, onPress: null };
    let tmp13Result = tmp13(11088);
  ...
  if (!isCoppa) {
    ...
    tmp9Result1 = tmp9(tmp13(11088), obj4);
    const tmp13Result1 = tmp13(11088);
  }
  ...
  obj6[0] = items1;
  return closure_9(_require(11083).AppealIngestionModalScreen, obj6);
};
