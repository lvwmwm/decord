...
// Dependencies: [19, 17, 676, 21, 4380, 15348, 5929, 4376, 1236, 4815, 1297, 4224, 1500, 4638, 698, 15349, 15353, 8654, 2]
// Exports: CompanionRemoteAuth

// Module 15347 (CompanionRemoteAuth)
...
export const CompanionRemoteAuth = function CompanionRemoteAuth() {
  ...
  obj1 = React;
  const context = React.useContext(fingerprint(4638));
  ...
  fingerprint = null;
  if (state.step === navigation(15348).RemoteAuthStep.PENDING_REMOTE_INIT) {
  ...
  let tmp5Result = tmp5(8654);
  const intl = tmp2(1236).intl;
  obj[0] = intl.string(navigation(1236).t["7fNJgA"]);
  obj = { style: tmp.statusContainer, children: null };
  const step = state.step;
  if (navigation(15348).RemoteAuthStep.INITIALIZING !== step) {
    if (tmp2(15348).RemoteAuthStep.PENDING_REMOTE_INIT !== step) {
      if (tmp2(15348).RemoteAuthStep.PENDING_TICKET === step) {
        ...
        items2[1] = tmp11(tmp2(4376).Text, obj3);
        ...
        items2[2] = tmp11(tmp2(4376).Text, obj4);
        ...
        obj7[3] = callback1;
        obj6[1] = tmp11(tmp2(4815).Button, obj7);
        items2[3] = tmp11(tmp13, obj6);
        obj1[0] = items2;
        let tmp11Result = callback(closure_8, obj1);
      } else if (tmp2(15348).RemoteAuthStep.PENDING_LOGIN === step) {
        tmp11Result = tmp11(tmp2(5929).ActivityIndicator, {});
      }
    }
    obj[1] = tmp11Result;
  ...
  const items3 = [closure_7(navigation(5929).ActivityIndicator, {}), , ];
  ...
  items3[1] = closure_7(navigation(4376).Text, obj9);
  ...
  obj11[3] = callback1;
  obj10[1] = closure_7(navigation(4815).Button, obj11);
