...
// Dependencies: [19, 17, 676, 21, 4448, 15544, 6016, 4444, 1236, 4896, 1297, 4292, 1499, 4718, 698, 15545, 15549, 7713, 2]
// Exports: CompanionRemoteAuth

// Module 15543 (CompanionRemoteAuth)
...
export const CompanionRemoteAuth = function CompanionRemoteAuth() {
  ...
  obj1 = React;
  const context = React.useContext(fingerprint(4718));
  ...
  fingerprint = null;
  if (state.step === navigation(15544).RemoteAuthStep.PENDING_REMOTE_INIT) {
  ...
  let tmp5Result = tmp5(7713);
  const intl = tmp2(1236).intl;
  obj[0] = intl.string(navigation(1236).t["7fNJgA"]);
  obj = { style: tmp.statusContainer, children: null };
  const step = state.step;
  if (navigation(15544).RemoteAuthStep.INITIALIZING !== step) {
    if (tmp2(15544).RemoteAuthStep.PENDING_REMOTE_INIT !== step) {
      if (tmp2(15544).RemoteAuthStep.PENDING_TICKET === step) {
        ...
        items2[1] = tmp11(tmp2(4444).Text, obj3);
        ...
        items2[2] = tmp11(tmp2(4444).Text, obj4);
        ...
        obj7[3] = callback1;
        obj6[1] = tmp11(tmp2(4896).Button, obj7);
        items2[3] = tmp11(tmp13, obj6);
        obj1[0] = items2;
        let tmp11Result = callback(closure_8, obj1);
      } else if (tmp2(15544).RemoteAuthStep.PENDING_LOGIN === step) {
        tmp11Result = tmp11(tmp2(6016).ActivityIndicator, {});
      }
    }
    obj[1] = tmp11Result;
  ...
  const items3 = [closure_7(navigation(6016).ActivityIndicator, {}), , ];
  ...
  items3[1] = closure_7(navigation(4444).Text, obj9);
  ...
  obj11[3] = callback1;
  obj10[1] = closure_7(navigation(4896).Button, obj11);
