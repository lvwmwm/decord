...
// Dependencies: [32, 5, 19, 17, 4247, 4945, 1875, 1946, 1302, 1304, 1874, 7072, 676, 11944, 1876, 10225, 1305, 21, 4255, 712, 11985, 709, 11943, 8592, 13589, 1355, 1358, 4071, 4460, 14861, 1959, 10270, 13905, 13907, 12838, 3956, 14622, 1581, 589, 3928, 14877, 5634, 5635, 13725, 1297, 14378, 4101, 14437, 14878, 9562, 1236, 5300, 14481, 5307, 8268, 14879, 8278, 1208, 1869, 500, 2]
// Exports: default

// Module 14860 (handleNewUserOnboarding)
import module_13905 from "module_13905";
...
function handleNewUserOnboarding() {
  require(11985) /* setNewUser */.setNewUser(NewUserTypes.ORGANIC_REGISTERED);
  const obj = require(11985) /* setNewUser */;
  importDefault(709).wait(require(11943) /* _startContactSyncForDiscoverability */.startOnboarding);
}
function handleThemeChange(arg0) {
  importDefault(8592).updateTheme(arg0 ? ThemeTypes.LIGHT : ThemeTypes.DARK);
}
...
function showVibingWumpus() {
  ...
  obj.pushLazy(require(1959) /* asyncRequireImpl */(10270, dependencyMap.paths), obj, VIBING_WUMPUS_MODAL_KEY);
}
...
function launchTotpSetupSuccess() {
  ...
  arr = arr.pop();
  const items = [require(1959) /* asyncRequireImpl */(13905, dependencyMap.paths), require(1959) /* asyncRequireImpl */(13907, dependencyMap.paths)];
...
function launchMFA() {
  let arr = importDefault(4460);
  arr = arr.pop();
  require(1959) /* asyncRequireImpl */(14861, dependencyMap.paths).then((openMFAModal) => {
