...
// Function ID: 13929
// Name: CacheLogEntry
// Dependencies: [17, 1391, 21, 4342, 712, 5435, 4775, 5828, 7017, 4338, 2]
// Exports: default

// Module 13928 (CacheLogEntry)
...
function CacheLogEntry(entry) {
  ...
  return tmp4(require(5435) /* TableRowInner */.TableRow, { label: "" + name, subLabel: items.join("\n") });
}
...
export default function DevToolsLocalMessageCache() {
  ...
  obj1[1] = importDefault(7017).channelsFetchStarted.size;
  const items = [callback(require(5435) /* TableRowInner */.TableRow, obj1), , , ];
  const obj2 = { label: "Cache Hits", subLabel: null };
  obj2[1] = importDefault(7017).channelsFetchedWithLocalMessages.size;
  items[1] = callback(require(5435) /* TableRowInner */.TableRow, obj2);
  const obj3 = { label: "Cache Misses", subLabel: null };
  obj3[1] = importDefault(7017).channelsFetchedNetwork.size - importDefault(7017).channelsFetchedWithLocalMessages.size;
  items[2] = callback(require(5435) /* TableRowInner */.TableRow, obj3);
  const obj4 = { label: "Incomplete Fetches", subLabel: null };
  obj4[1] = importDefault(7017).channelsFetchStarted.size - importDefault(7017).channelsFetchedNetwork.size;
  items[3] = callback(require(5435) /* TableRowInner */.TableRow, obj4);
  obj[2] = items;
  const items1 = [callback2(require(5828) /* TableRowGroupTitle */.TableRowGroup, obj), callback(require(4338) /* Text */.Text, { variant: "text-sm/normal", color: "text-muted", children: "Cumulative since app launch. Does not update dynamically." }), ];
  const obj5 = { title: "Fetch Log (Reversed)", hasIcons: false, children: null };
  const fetchLogs = importDefault(7017).fetchLogs;
  const reversed = Array.from(fetchLogs.values()).reverse();
  obj5[2] = reversed.map((entry) => callback(closure_8, { entry }, arg1));
  items1[2] = callback(require(5828) /* TableRowGroupTitle */.TableRowGroup, obj5);
  obj[1] = items1;
