...
// Function ID: 13772
// Name: CacheLogEntry
// Dependencies: [17, 1372, 21, 4285, 712, 5354, 4693, 5688, 6868, 4281, 2]
// Exports: default

// Module 13771 (CacheLogEntry)
...
function CacheLogEntry(entry) {
  ...
  return tmp4(require(5354) /* TableRowInner */.TableRow, { label: "" + name, subLabel: items.join("\n") });
}
...
export default function DevToolsLocalMessageCache() {
  ...
  obj1[1] = importDefault(6868).channelsFetchStarted.size;
  const items = [callback(require(5354) /* TableRowInner */.TableRow, obj1), , , ];
  const obj2 = { label: "Cache Hits", subLabel: null };
  obj2[1] = importDefault(6868).channelsFetchedWithLocalMessages.size;
  items[1] = callback(require(5354) /* TableRowInner */.TableRow, obj2);
  const obj3 = { label: "Cache Misses", subLabel: null };
  obj3[1] = importDefault(6868).channelsFetchedNetwork.size - importDefault(6868).channelsFetchedWithLocalMessages.size;
  items[2] = callback(require(5354) /* TableRowInner */.TableRow, obj3);
  const obj4 = { label: "Incomplete Fetches", subLabel: null };
  obj4[1] = importDefault(6868).channelsFetchStarted.size - importDefault(6868).channelsFetchedNetwork.size;
  items[3] = callback(require(5354) /* TableRowInner */.TableRow, obj4);
  obj[2] = items;
  const items1 = [callback2(require(5688) /* TableRowGroupTitle */.TableRowGroup, obj), callback(require(4281) /* Text */.Text, { variant: "text-sm/normal", color: "text-muted", children: "Cumulative since app launch. Does not update dynamically." }), ];
  const obj5 = { title: "Fetch Log (Reversed)", hasIcons: false, children: null };
  const fetchLogs = importDefault(6868).fetchLogs;
  const reversed = Array.from(fetchLogs.values()).reverse();
  obj5[2] = reversed.map((entry) => callback(closure_8, { entry }, arg1));
  items1[2] = callback(require(5688) /* TableRowGroupTitle */.TableRowGroup, obj5);
  obj[1] = items1;
