...
// Function ID: 13786
// Name: CacheLogEntry
// Dependencies: [17, 1372, 21, 4302, 712, 5373, 4710, 5707, 6887, 4298, 2]
// Exports: default

// Module 13785 (CacheLogEntry)
...
function CacheLogEntry(entry) {
  ...
  return tmp4(require(5373) /* TableRowInner */.TableRow, { label: "" + name, subLabel: items.join("\n") });
}
...
export default function DevToolsLocalMessageCache() {
  ...
  obj1[1] = importDefault(6887).channelsFetchStarted.size;
  const items = [callback(require(5373) /* TableRowInner */.TableRow, obj1), , , ];
  const obj2 = { label: "Cache Hits", subLabel: null };
  obj2[1] = importDefault(6887).channelsFetchedWithLocalMessages.size;
  items[1] = callback(require(5373) /* TableRowInner */.TableRow, obj2);
  const obj3 = { label: "Cache Misses", subLabel: null };
  obj3[1] = importDefault(6887).channelsFetchedNetwork.size - importDefault(6887).channelsFetchedWithLocalMessages.size;
  items[2] = callback(require(5373) /* TableRowInner */.TableRow, obj3);
  const obj4 = { label: "Incomplete Fetches", subLabel: null };
  obj4[1] = importDefault(6887).channelsFetchStarted.size - importDefault(6887).channelsFetchedNetwork.size;
  items[3] = callback(require(5373) /* TableRowInner */.TableRow, obj4);
  obj[2] = items;
  const items1 = [callback2(require(5707) /* TableRowGroupTitle */.TableRowGroup, obj), callback(require(4298) /* Text */.Text, { variant: "text-sm/normal", color: "text-muted", children: "Cumulative since app launch. Does not update dynamically." }), ];
  const obj5 = { title: "Fetch Log (Reversed)", hasIcons: false, children: null };
  const fetchLogs = importDefault(6887).fetchLogs;
  const reversed = Array.from(fetchLogs.values()).reverse();
  obj5[2] = reversed.map((entry) => callback(closure_8, { entry }, arg1));
  items1[2] = callback(require(5707) /* TableRowGroupTitle */.TableRowGroup, obj5);
  obj[1] = items1;
  obj[2] = callback2(require(4710) /* Stack */.Stack, obj);
