...
// Dependencies: [32, 19, 17, 5445, 4443, 21, 5401, 4444, 712, 5546, 4162, 5941, 589, 5939, 15317, 5541, 15334, 4877, 15335, 6167, 2]
// Exports: default

// Module 15333 (fuzzySearchToggle)
...
import closure_6 from "getUserAgnosticState" /* 5445 */;
import closure_7 from "getUserAgnosticState" /* 4443 */;
import { CATEGORY_LABELS } from "getUserAgnosticState" /* 4443 */;
...
function fuzzySearchToggle(str, str2, str3) {
  ...
  if (!tmp) {
    ...
    if (!tmp3ResultResult) {
      ...
      tmp3ResultResult = tmp3(5401)(formatted1, str3.toLowerCase());
      const tmp3Result = tmp3(5401);
    }
...
function ToggleTableRow(toggleName) {
  ...
  return callback2(toggleName(5546).TableRow, {
    label: description,
    labelLineClamp: 1,
    subLabel: toggleName,
    subLabelLineClamp: 1,
    ...
    trailing: callback2(toggleName(5941).FormSwitch, { value, onValueChange })
  }, toggleName);
}
function DevTogglesForCategory(title) {
  ...
  if (0 !== stateFromStores.length) {
    ...
    tmp3 = callback2(category(5939).TableRowGroup, obj);
  }
  return tmp3;
}
...
export default function DevToolsTogglesScreen() {
  ...
  const items3 = [
    callback2(first(5546).TableRow, {
      ...
      variant: "danger",
      onPress() {
        first(15335).clearAll();
        const obj = first(15335);
        first(15317).clearAll();
      },
      arrow: true
    }),
    callback2(first(5546).TableRow, { label: callback2(first(6167).SearchField, { size: "md", placeholder: "Search design toggles", onChange: tmp3[1] }) })
  ];
  obj1[2] = items3;
  const items4 = [callback3(first(5939).TableRowGroup, obj1), , ];
  let tmp7Result = null;
  if (stateFromStores.length > 0) {
    ...
    tmp7Result = tmp7(first(5939).TableRowGroup, obj4);
  }
  ...
  obj5[1] = items4;
  obj[2] = callback3(first(4877).Stack, obj5);
