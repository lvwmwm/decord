...
// Dependencies: [32, 19, 17, 5334, 4341, 21, 5290, 4342, 712, 5435, 4062, 5830, 589, 5828, 15095, 5430, 15112, 4775, 15113, 6056, 2]
// Exports: default

// Module 15111 (fuzzySearchToggle)
...
function fuzzySearchToggle(str, str2, str3) {
  ...
  if (!tmp) {
    ...
    let tmp3ResultResult = importDefault(5290)(formatted, str2.toLowerCase());
    if (!tmp3ResultResult) {
      const formatted1 = str.toLowerCase();
      tmp3ResultResult = tmp3(5290)(formatted1, str3.toLowerCase());
      const tmp3Result = tmp3(5290);
    }
    tmp = tmp3ResultResult;
    tmp3 = importDefault;
    const tmp5 = importDefault(5290);
  }
  return tmp;
}
function ToggleTableRow(toggleName) {
  ...
  obj[5] = callback2(toggleName(5830).FormSwitch, { value, onValueChange });
  return callback2(toggleName(5435).TableRow, obj, toggleName);
}
function DevTogglesForCategory(title) {
  ...
  if (0 !== stateFromStores.length) {
    ...
    tmp3 = callback2(category(5828).TableRowGroup, obj);
  }
  return tmp3;
}
...
export default function DevToolsTogglesScreen() {
  ...
  obj = { paddingBottom: importDefault(712).space.PX_16 + importDefault(5430)({ includeKeyboardHeight: true }).insets.bottom };
  items2[1] = obj;
  obj[1] = items2;
  ...
  const items3 = [
    callback2(first(5435).TableRow, {
      ...
      variant: "danger",
      onPress() {
        first(15113).clearAll();
        const obj = first(15113);
        first(15095).clearAll();
      },
      arrow: true
    }),
  ...
  obj3[0] = callback2(first(6056).SearchField, { size: "md", placeholder: "Search design toggles", onChange: tmp3[1] });
  items3[1] = callback2(first(5435).TableRow, obj3);
  obj1[2] = items3;
  const items4 = [callback3(first(5828).TableRowGroup, obj1), , ];
  let tmp7Result = null;
  if (stateFromStores.length > 0) {
    ...
    tmp7Result = tmp7(first(5828).TableRowGroup, obj4);
  }
  ...
  obj5[1] = items4;
  obj[2] = callback3(first(4775).Stack, obj5);
