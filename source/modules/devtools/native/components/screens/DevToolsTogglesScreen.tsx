...
// Dependencies: [32, 19, 17, 5294, 4737, 21, 6775, 4661, 712, 6291, 4094, 7179, 589, 6286, 15144, 5441, 15161, 4733, 15162, 7337, 2]
// Exports: default
...
function fuzzySearchToggle(str, str2, str3) {
  ...
  if (!tmp) {
    ...
    let tmp3ResultResult = importDefault(6775)(formatted, str2.toLowerCase());
    if (!tmp3ResultResult) {
      const formatted1 = str.toLowerCase();
      tmp3ResultResult = tmp3(6775)(formatted1, str3.toLowerCase());
      const tmp3Result = tmp3(6775);
    }
...
function ToggleTableRow(toggleName) {
  ...
  obj[5] = callback2(toggleName(7179).FormSwitch, { value, onValueChange });
  return callback2(toggleName(6291).TableRow, obj, toggleName);
}
...
export default function DevToolsTogglesScreen() {
  ...
  obj = { paddingBottom: importDefault(712).space.PX_16 + importDefault(5441)({ includeKeyboardHeight: true }).insets.bottom };
  items2[1] = obj;
  obj[1] = items2;
  ...
  obj3[0] = callback2(first(7337).SearchField, { size: "md", placeholder: "Search design toggles", onChange: tmp3[1] });
  items3[1] = callback2(first(6291).TableRow, obj3);
  obj1[2] = items3;
  ...
  if (stateFromStores.length > 0) {
    ...
    tmp7Result = tmp7(first(6286).TableRowGroup, obj4);
  }
