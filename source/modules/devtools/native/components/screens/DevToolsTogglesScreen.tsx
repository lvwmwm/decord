...
// Dependencies: [32, 19, 17, 5256, 4284, 21, 5212, 4285, 712, 5354, 3985, 5690, 589, 5688, 14925, 5349, 14942, 4693, 14943, 5913, 2]
// Exports: default

// Module 14941 (fuzzySearchToggle)
...
function fuzzySearchToggle(str, str2, str3) {
  ...
  if (!tmp) {
    ...
    let tmp3ResultResult = importDefault(5212)(formatted, str2.toLowerCase());
    if (!tmp3ResultResult) {
      const formatted1 = str.toLowerCase();
      tmp3ResultResult = tmp3(5212)(formatted1, str3.toLowerCase());
      const tmp3Result = tmp3(5212);
    }
...
function ToggleTableRow(toggleName) {
  ...
  obj[5] = callback2(toggleName(5690).FormSwitch, { value, onValueChange });
  return callback2(toggleName(5354).TableRow, obj, toggleName);
}
function DevTogglesForCategory(title) {
  ...
  if (0 !== stateFromStores.length) {
    ...
    tmp3 = callback2(category(5688).TableRowGroup, obj);
  }
  return tmp3;
}
...
export default function DevToolsTogglesScreen() {
  ...
  obj = { paddingBottom: importDefault(712).space.PX_16 + importDefault(5349)({ includeKeyboardHeight: true }).insets.bottom };
  items2[1] = obj;
  obj[1] = items2;
  ...
  const items3 = [
    callback2(first(5354).TableRow, {
      ...
      variant: "danger",
      onPress() {
        first(14943).clearAll();
        const obj = first(14943);
  ...
  obj3[0] = callback2(first(5913).SearchField, { size: "md", placeholder: "Search design toggles", onChange: tmp3[1] });
  items3[1] = callback2(first(5354).TableRow, obj3);
  obj1[2] = items3;
  const items4 = [callback3(first(5688).TableRowGroup, obj1), , ];
  let tmp7Result = null;
  if (stateFromStores.length > 0) {
    ...
    tmp7Result = tmp7(first(5688).TableRowGroup, obj4);
  }
