...
// Function ID: 13780
// Name: jsx
// Dependencies: [32, 19, 17, 10341, 21, 4302, 712, 7670, 13780, 5368, 589, 11119, 4710, 5707, 5373, 4250, 5696, 4001, 13777, 4261, 7925, 7924, 7730, 4712, 1351, 2]
...
const memoResult = importAllResult.memo(() => {
  ...
  const stateFromStores = obj.useStateFromStores(items, () => {
    ...
    let tmp;
    if (overrides != null) {
      tmp = overrides[first(undefined, 11119).DEVICE_FIELD];
    }
    return tmp;
  });
  ...
  obj.paddingBottom = tmp.contentContainer.padding + first(5368)({ includeKeyboardHeight: true }).insets.bottom;
  obj[1] = obj;
  let tmp10Result = null;
  if (null != stateFromStores) {
    const obj1 = { icon: null, label: null, subLabel: null, onPress: null };
    obj1[0] = tmp7(tmp3(4250).CopyIcon, {});
    first = stateFromStores;
    ...
    obj1[3] = function onPress() {
      first(5696).copy(first.id);
      const obj = first(5696);
      const result = first(4001).presentCopiedToClipboard();
    };
    const items1 = [tmp7(tmp3(5373).TableRow, obj1), , ];
    let obj3 = { icon: null, label: "Refresh Override", onPress: null, arrow: true };
    obj3[0] = tmp7(tmp3(13777).RefreshIcon, {});
    obj3[2] = tmp3(11119).refreshBuildOverride;
    items1[1] = tmp7(tmp3(5373).TableRow, obj3);
    const obj4 = { icon: null, label: "Clear Override", variant: "danger", onPress: null, arrow: true };
    obj4[0] = tmp7(tmp3(4261).TrashIcon, { color: "text-feedback-critical" });
    obj4[3] = tmp3(11119).clearBuildOverride;
    items1[2] = tmp7(tmp3(5373).TableRow, obj4);
    obj2[2] = items1;
  ...
  items2[1] = jsx(first(7925).TableRadioGroup, {
    ...
    children: items.map((value) => {
      ...
      value = value.value;
      ({ icon, label } = value);
      return callback(first(7924).TableRadioRow, { value, label, icon }, value);
    })
  });
  ...
  obj7[1] = jsx(first(7730).TextInput, {
    size: "md",
  ...
  obj6[2] = jsx(first(5373).TableRow, { icon, label: null });
  items2[2] = jsx(first(5707).TableRowGroup, { title: label1, hasIcons: true, children: null });
  items2[3] = jsx(first(4712).Button, {
    text: "Apply Build Override",
    disabled: "" === first.id,
    onPress() {
      const type = first.type;
      if ("branch" === type) {
        const result = first(11119).setBuildOverrideForBranch(tmp.id);
        const obj3 = first(11119);
      } else if ("id" === type) {
        const result1 = first(11119).setBuildOverrideForId(tmp.id);
        const obj2 = first(11119);
      } else {
  ...
  obj8[1] = items2;
  obj[2] = jsxs(first(4710).Stack, { spacing: 16, children: null });
  return <tmp8 />;
});
