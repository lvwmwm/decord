...
function Select(children) {
  ...
  if (type === obj.type) {
    ...
    state[1] = items1;
    tmp6(tmp4(4185).Text, state);
  } else {
    items[2] = tmp7(tmp4(4185).Text, { variant: "text-md/normal", children: "Nothing selected" });
    let str3 = "off";
    if (tmp2) {
      str3 = "on";
    }
    ...
    items[3] = tmp7(tmp4(4600).Button, obj2);
    obj[1] = items;
    return tmp6(_require(5222).Card, obj);
  }
}
...
export default function DevToolsComponentsTestingScreen() {
  ...
  const items = [callback(require(4185) /* Text */.Text, { variant: "text-md/normal", children: "Test screen for embedding native components in RN" }), , ];
