...
// Function ID: 14828
// Name: Select
// Dependencies: [32, 19, 17, 1931, 21, 4193, 712, 4415, 1906, 7896, 5226, 4189, 14828, 14831, 4604, 4602, 14832, 709, 2]
// Exports: default
...
function Select(children) {
  ...
  obj.type = type;
  obj.id = String(type);
  let obj1 = _require(7896);
  let state = obj1.useComponentState(obj).state;
  obj = { style: { gap: 8 }, children: null };
  const items = [callback(_require(4189).Text, { variant: "heading-lg/medium", children: children.title }), , , ];
  if (obj.type === _require(1906).ComponentType.STRING_SELECT) {
    obj = {};
    const merged1 = Object.assign(obj);
    let tmp7Result = tmp7(importDefault(14828), obj);
    const tmp15 = importDefault(14828);
  } else {
    obj1 = {};
    const merged2 = Object.assign(obj);
    obj1.type = obj.type;
    tmp7Result = tmp7(importDefault(14831), obj1);
    const tmp9 = importDefault(14831);
  }
  items[1] = tmp7Result;
  type = undefined;
  if (state != null) {
  ...
  if (type === obj.type) {
    ...
    state[1] = items1;
    tmp6(tmp4(4189).Text, state);
  } else {
    items[2] = tmp7(tmp4(4189).Text, { variant: "text-md/normal", children: "Nothing selected" });
    let str3 = "off";
    if (tmp2) {
      str3 = "on";
    }
    ...
    items[3] = tmp7(tmp4(4604).Button, obj2);
    obj[1] = items;
    return tmp6(_require(5226).Card, obj);
  }
}
...
export default function DevToolsComponentsTestingScreen() {
  ...
  const items = [callback(require(4189) /* Text */.Text, { variant: "text-md/normal", children: "Test screen for embedding native components in RN" }), , ];
  ...
  const items1 = [callback(importDefault(14832), {}), , , , , ];
  const obj2 = {};
  const tmp2 = importDefault(14832);
  ...
  obj[1] = items1;
  items[1] = callback2(require(7896) /* isInteractionComponent */.ComponentStateContextProvider, obj);
  items[2] = callback(require(4604) /* Button */.Button, {
    onPress() {
