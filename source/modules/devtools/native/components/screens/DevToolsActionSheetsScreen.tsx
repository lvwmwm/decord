...
// Dependencies: [32, 19, 17, 21, 4302, 712, 12155, 12153, 4507, 14972, 1988, 4270, 5396, 5395, 5707, 5373, 7902, 4254, 4710, 5375, 4298, 2]
// Exports: default

// Module 14971 (ActionSheetSelector)
...
function ActionSheetSelector(arg0) {
  ...
  dependencyMap = React.useCallback((type) => {
    onSelect(4270).hideActionSheet("action-sheet-selector");
    onSelect(type.type);
    type.show();
  }, items);
  ...
  obj2[1] = items.map((type) => {
    ...
    obj[0] = outer1_7(outer1_0(7902).WarningIcon, { size: "md" });
    ...
    if (closure_0 === type.type) {
      tmpResult = tmp(outer1_0(4254).CheckmarkLargeIcon, { size: "md", color: "text-feedback-positive" });
    }
    obj[4] = tmpResult;
    obj[5] = 0 === arg1;
    obj[6] = arg1 === outer1_10.length - 1;
    return outer1_7(outer1_0(5373).TableRow, obj, type.type);
  });
  obj[1] = callback2(require(5707) /* TableRowGroupTitle */.TableRowGroup, obj2);
...
let items = [
  {
    ...
    show() {
      return importDefault(12155).show("https://example-phishing-site.com/malicious-page");
    }
  },
  {
    ...
    show() {
      return importDefault(12153).show("https://suspicious-file.com/dangerous-file.exe");
    }
  },
  {
    ...
    show() {
      return importDefault(4507).pushLazy(require(1988) /* asyncRequireImpl */(14972, dependencyMap.paths), { warningId: "test-warning-123", warningType: "inappropriate_conversation", senderId: "123456789", channelId: "987654321" }, "INAPPROPRIATE_CONVERSATION_TAKEOVER_MODAL");
    }
  }
];
...
export default function DevToolsActionSheetsScreen() {
  ...
  obj = { children: null };
  const items1 = [callback2(first(4298).Text, { variant: "heading-lg/medium", children: "Action Sheets" }), ];
  const obj1 = { description: "Tap an option to launch the action sheet immediately", hasIcons: false, children: null };
  obj1[2] = callback2(first(5373).TableRow, { label: found.label, subLabel: found.description, arrow: true, onPress: callback });
  items1[1] = callback2(first(5707).TableRowGroup, obj1);
  obj[0] = items1;
  obj[1] = callback3(first(5375).Card, obj);
  obj[2] = callback2(first(4710).Stack, obj);
