...
// Dependencies: [32, 19, 17, 21, 4189, 712, 12282, 12280, 4395, 14815, 1959, 4157, 5243, 5242, 5554, 5220, 7626, 4141, 4598, 5222, 4185, 2]
// Exports: default

// Module 14814 (ActionSheetSelector)
...
function ActionSheetSelector(arg0) {
  ...
  dependencyMap = React.useCallback((type) => {
    onSelect(4157).hideActionSheet("action-sheet-selector");
    onSelect(type.type);
    type.show();
  }, items);
  ...
  obj2[1] = items.map((type) => {
    ...
    obj[0] = outer1_7(outer1_0(7626).WarningIcon, { size: "md" });
    ...
    if (closure_0 === type.type) {
      tmpResult = tmp(outer1_0(4141).CheckmarkLargeIcon, { size: "md", color: "text-feedback-positive" });
    }
    obj[4] = tmpResult;
    obj[5] = 0 === arg1;
    obj[6] = arg1 === outer1_10.length - 1;
    return outer1_7(outer1_0(5220).TableRow, obj, type.type);
  });
  obj[1] = callback2(require(5554) /* TableRowGroupTitle */.TableRowGroup, obj2);
...
let items = [
  {
    ...
    show() {
      return importDefault(12282).show("https://example-phishing-site.com/malicious-page");
    }
  },
  {
    ...
    show() {
      return importDefault(12280).show("https://suspicious-file.com/dangerous-file.exe");
    }
  },
  {
    ...
    show() {
      return importDefault(4395).pushLazy(require(1959) /* asyncRequireImpl */(14815, dependencyMap.paths), { warningId: "test-warning-123", warningType: "inappropriate_conversation", senderId: "123456789", channelId: "987654321" }, "INAPPROPRIATE_CONVERSATION_TAKEOVER_MODAL");
    }
  }
];
...
export default function DevToolsActionSheetsScreen() {
  ...
  obj = { children: null };
  const items1 = [callback2(first(4185).Text, { variant: "heading-lg/medium", children: "Action Sheets" }), ];
  const obj1 = { description: "Tap an option to launch the action sheet immediately", hasIcons: false, children: null };
  obj1[2] = callback2(first(5220).TableRow, { label: found.label, subLabel: found.description, arrow: true, onPress: callback });
  items1[1] = callback2(first(5554).TableRowGroup, obj1);
  obj[0] = items1;
  obj[1] = callback3(first(5222).Card, obj);
  obj[2] = callback2(first(4598).Stack, obj);
