...
// Dependencies: [32, 19, 6201, 4110, 1388, 505, 6202, 589, 1377, 6203, 4790, 709, 6207, 2009, 4412, 2]
// Exports: useGiftCardMobileConsumptionHalfsheet

// Module 6200 (useGiftCardMobileConsumptionHalfsheet)
import closure_3 from "_slicedToArray" /* 32 */;
import closure_4 from "noop" /* 19 */;
import closure_5 from "set" /* 6201 */;
...
export const useGiftCardMobileConsumptionHalfsheet = function useGiftCardMobileConsumptionHalfsheet() {
  enabled = enabled(6202).useGiftCardsExperimentConfig({ location: "useGiftCardMobileConsumptionHalfsheet" }).enabled;
  obj = enabled(6202);
  let tmp = enabled;
  let items = [callback];
  let items1 = [enabled];
  ...
  const tmp10 = first(tmp(6203).useSelectedDismissibleContent(memo, undefined, true), 2);
  first = tmp10[0];
  React = tmp12;
  ...
  const effect1 = React.useEffect(() => {
    if (enabled) {
      const paymentSources = enabled(4790).fetchPaymentSources();
      obj = enabled(4790);
    }
  }, items8);
  ...
  const effect2 = React.useEffect(() => {
    if (null != stateFromStores) {
      const walletInformation = enabled(4790).fetchWalletInformation(tmp);
      obj = enabled(4790);
    }
  }, items9);
  ...
  const effect3 = React.useEffect(() => {
    ...
    if (first === enabled(1377).DismissibleContent.GIFT_CARD_MOBILE_CONSUMPTION_UNAVAILABLE_HALFSHEET) {
      if (Opening === closure_1_10.Idle) {
        ...
        const promise = enabled(2009)(6207, dependencyMap.paths);
        promise.catch(() => {
          let tmp = c0;
          if (!c0) {
        ...
        const obj2 = stateFromStores(4412);
