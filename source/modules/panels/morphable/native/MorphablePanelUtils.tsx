...
const tmp4 = () => {
  function getPIPWindowDimensions(width, left) {
    if (arg2) {
      const _Math = Math;
      let bound = Math.max(left.left, closure_1);
    } else {
      bound = closure_1;
    }
    const diff = width.width - bound;
    let diff1 = width.width - left.left - left.right - 2 * closure_1;
    if (arg2) {
      diff1 = diff - Math.max(left.right, closure_1);
    }
    const bound1 = Math.max(left.top, closure_1);
    const obj = { xOffset: bound, yOffset: bound1, xRange: diff1 };
    const diff2 = width.height - bound1;
    obj.yRange = diff2 - Math.max(left.bottom, closure_1);
    return obj;
  }
...
const tmp3 = () => {
  function calculateXYDiff(state, closure_10) {
    let absoluteX;
    let absoluteY;
    ({ absoluteY, absoluteX } = state.changedTouches[0]);
    const diff = closure_10.get().absoluteYStart - absoluteY;
    const diff1 = closure_10.get().absoluteXStart - absoluteX;
...
const tmp6 = () => {
  function pipXYtoAbsoluteXY(windowDimensions) {
    ...
    const tmp = callback(windowDimensions.windowDimensions, windowDimensions.safeArea, windowDimensions.disableHorizontalSafeAreas);
    return { x: tmp.xOffset + pipX * tmp.xRange, y: tmp.yOffset + pipY * tmp.yRange };
  }
  pipXYtoAbsoluteXY.__closure = { getPIPWindowDimensions: tmp4 };
...
const tmp5 = () => {
  function calculatePIPPositionFromVelocity(windowDimensions) {
    ...
    const tmp = callback2(windowDimensions.windowDimensions, windowDimensions.safeArea, windowDimensions.disableHorizontalSafeAreas);
    ({ xRange, yRange } = tmp);
    const diff = absoluteX - tmp.xOffset;
    const diff1 = absoluteY - tmp.yOffset;
    const absolute = Math.abs(velocityY);
    if (Math.max(absolute, Math.abs(velocityX)) < closure_0) {
  ...
  calculatePIPPositionFromVelocity.__closure = { getPIPWindowDimensions: tmp4, MIN_PIP_TOSS_VELOCITY: closure_0, clamp: closure_3 };
...
export const calculatePIPPositionFromVelocity = tmp5;
export const pipXYtoAbsoluteXY = tmp6;
export const getClampedPIPPosition = () => {
  function getClampedPIPPosition(topAvoidanceRegion) {
    ...
    const tmp5 = callback2(windowDimensions, safeArea, disableHorizontalSafeAreas);
  ...
  getClampedPIPPosition.__closure = { pipXYtoAbsoluteXY: tmp6, getPIPWindowDimensions: tmp4, clamp: closure_3 };
