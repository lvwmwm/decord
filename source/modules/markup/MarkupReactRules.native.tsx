...
// Function ID: 4247
// Name: MarkupText
// Dependencies: [19, 17, 4247, 1372, 1942, 1935, 1862, 676, 1379, 505, 21, 4250, 4251, 4255, 712, 4263, 4447, 4107, 4461, 8668, 4594, 1236, 5609, 3955, 5632, 1297, 1467, 8511, 3952, 9667, 9704, 10147, 589, 4099, 7868, 4223, 11042, 1959, 11, 3928, 5207, 10164, 13063, 11057, 4686, 10444, 13064, 13066, 4199, 4688, 500, 4716, 13068, 10446, 2]
...
function MarkupLink(arg0) {
  ...
  obj.inLink = true;
  obj[5] = node(10147).smartOutput(node, output, obj);
...
function MarkupBlockQuote(state) {
  ...
  obj[1] = textColor;
  obj[2] = require(10147) /* smartOutput */.smartOutput(node, output, state);
...
function MarkupInlineCode(arg0) {
  ...
  obj[4] = node(10147).smartOutput(node, output, state);
...
function MarkupCodeBlock(state) {
  ...
  obj[1] = textColor;
  const items = [require(10147) /* smartOutput */.smartOutput(node, output, state), "\n"];
  obj[2] = items;
...
function MarkupCustomEmoji(styles) {
  ...
  if (node.src) {
    ...
    obj1[2] = tmp13;
    let tmp4Result = tmp4(importDefault(5207), obj1, state.key);
...
function MarkupChannelMention(state) {
  ...
  if (null == node.inContent) {
    items[1] = null;
    items[2] = tmp3(10147).smartOutput(node, output, state);
    obj[2] = items;
...
function MarkupAttachmentLink(state) {
  ...
  obj[2] = SMALL;
  const items = [closure_16(state(1297).ThemedIcon, obj), state(10147).smartOutput(node, output, state)];
  obj[2] = items;
  const tmp2Result = closure_17(state(4251).Text, obj, state.key);
...
function MarkupCommandMention(state) {
  ...
  obj[3] = mention;
  return tmp2(importDefault(11057), obj, state.key);
}
  ...
  return {
    [closure_0(closure_2[44]).AST_KEY.TEXT]: {
      react(content, output, textColor) {
        if (typeof content.content === "y") {
          ...
          obj[0] = textColor;
          obj[1] = obj(10147).smartOutput(content, output, textColor);
          content = closure_16(closure_20, obj, textColor.key);
          const obj2 = obj(10147);
          const tmp6 = closure_16;
    ...
    [closure_0(closure_2[44]).AST_KEY.STRIKETHROUGH]: {
      react(node, output, textColor) {
        ...
        obj[3] = obj(10147).smartOutput(node, output, textColor);
    ...
    [closure_0(closure_2[44]).AST_KEY.HIGHLIGHT]: {
      react(node, output, textColor) {
        ...
        obj[1] = obj(10147).smartOutput(node, output, textColor);
        return closure_16(closure_20, obj, textColor.key);
      }
    },
    ...
    [closure_0(closure_2[44]).AST_KEY.CODE_BLOCK]: {
      parse(arg0, arg1, arg2) {
        obj = str(4688).RULES[obj(undefined, 4686).AST_KEY.CODE_BLOCK];
    ...
    [closure_0(closure_2[44]).AST_KEY.SOUNDBOARD]: {
      react(node, output, key) {
        ...
        const items = ["<sound:", obj(10147).smartOutput(node, output, key), ">"];
        obj[1] = items;
    ...
    [closure_0(closure_2[44]).AST_KEY.GUILD]: {
      react(icon, output, textColor) {
        ...
        if (null == icon.icon) {
          ...
          obj1[0] = textColor1;
          obj1[1] = tmp(10147).smartOutput(icon, output, textColor);
          ...
          const fontScale = store.getFontScale();
          if (fontScale < 1) {
            let XXSMALL = tmp(5632).GuildIconSizes.XXXSMALL;
          } else if (fontScale < 1.25) {
            XXSMALL = tmp(5632).GuildIconSizes.XXSMALL_12;
          } else {
            XXSMALL = tmp(5632).GuildIconSizes.XXSMALL;
          }
          obj2[2] = XXSMALL;
          callback(str(5632), obj2);
          const tmp4 = callback;
          const tmp6 = str(5632);
        }
      }
    ...
    [closure_0(closure_2[44]).AST_KEY.CHANNEL]: {
      react(iconType, output, textColor) {
        ...
        obj[1] = obj(4716).getChannelMentionIcon(str);
        const fontScale = store.getFontScale();
        if (fontScale < 1) {
        ...
        items[1] = obj(10147).smartOutput(iconType, output, textColor);
        obj[1] = items;
    ...
    [closure_0(closure_2[44]).AST_KEY.GAME_MENTION]: {
      react(node, arg1, key) {
        return callback(str(13068), { node, state: key }, key.key);
      }
    },
    ...
    [closure_0(closure_2[44]).AST_KEY.HEADING]: {
      react(level, output, formatInline) {
        if (formatInline.formatInline) {
          ...
          const items = [obj(10147).smartOutput(level, output, obj), " "];
          obj[2] = items;
          ...
          const items1 = [obj(10147).smartOutput(level, output, obj1), "\n"];
          obj[2] = items1;
    ...
    [closure_0(closure_2[44]).AST_KEY.SUBTEXT]: {
      react(node, output, key) {
        ...
        const items = [obj(10147).smartOutput(node, output, key), "\n"];
        obj[2] = items;
        return callback2(obj(4251).Text, obj, key.key);
      }
    },
