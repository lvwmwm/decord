...
// Function ID: 4438
// Name: MarkupText
// Dependencies: [19, 17, 4438, 1391, 1986, 1910, 676, 1398, 505, 21, 4441, 4442, 4446, 712, 4457, 4641, 4301, 4689, 8836, 4822, 1236, 5946, 4162, 5969, 1297, 1487, 8587, 4159, 11134, 11117, 7951, 589, 4294, 7817, 4413, 11120, 2010, 11, 4135, 5458, 11098, 13512, 8984, 4914, 10146, 13513, 13515, 4389, 4916, 500, 4946, 13517, 10148, 2]
// Exports: default, plainMentionRenderer, plainSpoilerRenderer

// Module 4437 (MarkupText)
...
import handleTapCommandMentionDefault from "handleTapCommandMention" /* 8984 */;
...
function MarkupLink(arg0) {
  ...
  obj.inLink = true;
  obj[5] = node(7951).smartOutput(node, output, obj);
  return tmp2(node(4442).Text, obj, state.key);
}
...
function MarkupInlineCode(arg0) {
  ...
  obj[4] = node(7951).smartOutput(node, output, state);
...
function MarkupChannelMention(state) {
  ...
  if (null == node.inContent) {
    items[1] = null;
    items[2] = tmp3(7951).smartOutput(node, output, state);
    obj[2] = items;
    const tmp2Result = closure_16(state(4442).Text, obj, state.key);
...
function MarkupAttachmentLink(state) {
  ...
  let obj = { variant: str2, style: tmp.channelMentionText, children: null };
  obj = { themedColor: node(712).colors.MENTION_FOREGROUND, source: node(13512), size: null };
  const fontScale = store.getFontScale();
  if (fontScale < 1) {
  ...
  obj[2] = SMALL;
  const items = [closure_15(state(1297).ThemedIcon, obj), state(7951).smartOutput(node, output, state)];
  obj[2] = items;
  const tmp2Result = closure_16(state(4442).Text, obj, state.key);
  ...
  return {
    [closure_0(closure_2[43]).AST_KEY.TEXT]: {
      react(content, output, textColor) {
        if (typeof content.content === "string") {
          ...
          obj = { color: null, children: null };
          obj[0] = textColor;
          obj[1] = obj(7951).smartOutput(content, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.STRIKETHROUGH]: {
      react(node, output, textColor) {
        ...
        obj[3] = obj(7951).smartOutput(node, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.HIGHLIGHT]: {
      react(node, output, textColor) {
        ...
        obj = { color: textColor, children: obj(7951).smartOutput(node, output, textColor) };
        return closure_15(closure_19, obj, textColor.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.CODE_BLOCK]: {
      parse(arg0, arg1, arg2) {
        obj = str(4916).RULES[obj(undefined, 4914).AST_KEY.CODE_BLOCK];
    ...
    [closure_0(closure_2[43]).AST_KEY.SOUNDBOARD]: {
      react(node, output, key) {
        ...
        const items = ["<sound:", obj(7951).smartOutput(node, output, key), ">"];
        obj[1] = items;
        return callback2(obj(4442).Text, obj, key.key);
      }
    },
    [closure_0(closure_2[43]).AST_KEY.GUILD]: {
      react(icon, output, textColor) {
        ...
        if (null == icon.icon) {
          ...
          obj1[0] = textColor1;
          obj1[1] = tmp(7951).smartOutput(icon, output, textColor);
          ...
          const fontScale = store.getFontScale();
          if (fontScale < 1) {
            let XXSMALL = tmp(5969).GuildIconSizes.XXXSMALL;
          } else if (fontScale < 1.25) {
            XXSMALL = tmp(5969).GuildIconSizes.XXSMALL_12;
          } else {
            XXSMALL = tmp(5969).GuildIconSizes.XXSMALL;
          }
          obj2[2] = XXSMALL;
          callback(str(5969), obj2);
          const tmp4 = callback;
          const tmp6 = str(5969);
        }
      }
    ...
    [closure_0(closure_2[43]).AST_KEY.CHANNEL]: {
      react(iconType, output, textColor) {
        ...
        obj = { themedColor: str(712).colors.MENTION_FOREGROUND, source: obj(4946).getChannelMentionIcon(str), size: null, style: null };
        const fontScale = store.getFontScale();
        if (fontScale < 1) {
        ...
        items[1] = obj(7951).smartOutput(iconType, output, textColor);
        obj[1] = items;
    ...
    [closure_0(closure_2[43]).AST_KEY.GAME_MENTION]: {
      react(node, arg1, key) {
        return callback(str(13517), { node, state: key }, key.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.LIST]: {
      react(start, arg1, level) {
        ...
        if (level.formatInline) {
          ...
          return callback(obj(4442).Text, obj, "list-" + level.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.HEADING]: {
      react(level, output, formatInline) {
        if (formatInline.formatInline) {
          ...
          const items = [obj(7951).smartOutput(level, output, obj), " "];
          obj[2] = items;
          ...
          const items1 = [obj(7951).smartOutput(level, output, obj1), "\n"];
          obj[2] = items1;
          return closure_16(obj(4442).Text, obj, formatInline.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.SUBTEXT]: {
      react(node, output, key) {
        ...
        const items = [obj(7951).smartOutput(node, output, key), "\n"];
        obj[2] = items;
        return callback2(obj(4442).Text, obj, key.key);
      }
    },
