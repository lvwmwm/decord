...
// Function ID: 4473
// Name: MarkupText
// Dependencies: [19, 17, 4473, 1386, 1984, 1908, 673, 1393, 502, 21, 4476, 4477, 4481, 709, 4492, 4682, 4336, 4731, 8110, 4865, 1233, 7130, 4193, 5539, 1296, 1485, 8300, 4190, 11546, 11516, 8027, 586, 4329, 7892, 4448, 11519, 2008, 11, 4166, 5542, 11497, 13850, 11144, 4957, 10056, 13851, 13853, 4424, 4959, 1234, 4989, 13855, 10058, 2]
// Exports: default, plainMentionRenderer, plainSpoilerRenderer

// Module 4472 (MarkupText)
import initialize from "initialize" /* 586 */;
import ThemesDefault from "Themes" /* 709 */;
import explicitContentFromProto from "explicitContentFromProto" /* 4166 */;
import Text from "Text" /* 4477 */;
import preloadDefault from "preload" /* 5542 */;
import smartOutput from "smartOutput" /* 8027 */;
import handleTapCommandMentionDefault from "handleTapCommandMention" /* 11144 */;
...
function MarkupLink(arg0) {
  ...
  obj.inLink = true;
  obj[5] = node(8027).smartOutput(node, output, obj);
  return tmp2(node(4477).Text, obj, state.key);
}
...
function MarkupInlineCode(arg0) {
  ...
  obj[4] = node(8027).smartOutput(node, output, state);
...
function MarkupChannelMention(state) {
  ...
  if (null == node.inContent) {
    items[1] = null;
    items[2] = tmp3(8027).smartOutput(node, output, state);
    obj[2] = items;
    const tmp2Result = closure_16(state(4477).Text, obj, state.key);
    ...
    obj2[0] = node(709).colors.MENTION_FOREGROUND;
    ...
    obj2[1] = obj3;
    obj2[2] = node(11497);
    obj2[3] = tmp3(1296).Icon.Sizes.CUSTOM;
    closure_15(tmp3(1296).ThemedIcon, obj2);
...
function MarkupAttachmentLink(state) {
  ...
  let obj = { variant: str2, style: tmp.channelMentionText, children: null };
  obj = { themedColor: node(709).colors.MENTION_FOREGROUND, source: node(13850), size: null };
  const fontScale = store.getFontScale();
  if (fontScale < 1) {
    let SMALL = tmp3(1296).Icon.Sizes.EXTRA_SMALL_10;
  } else if (fontScale < 1.25) {
    SMALL = tmp3(1296).Icon.Sizes.EXTRA_SMALL;
  } else {
    SMALL = tmp3(1296).Icon.Sizes.SMALL;
  }
  obj[2] = SMALL;
  const items = [closure_15(state(1296).ThemedIcon, obj), state(8027).smartOutput(node, output, state)];
  obj[2] = items;
  const tmp2Result = closure_16(state(4477).Text, obj, state.key);
  ...
  return {
    [closure_0(closure_2[43]).AST_KEY.TEXT]: {
      react(content, output, textColor) {
        if (typeof content.content === "string") {
          ...
          obj = { color: null, children: null };
          obj[0] = textColor;
          obj[1] = obj(8027).smartOutput(content, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.STRIKETHROUGH]: {
      react(node, output, textColor) {
        ...
        obj[3] = obj(8027).smartOutput(node, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.HIGHLIGHT]: {
      react(node, output, textColor) {
        ...
        obj = { color: textColor, children: obj(8027).smartOutput(node, output, textColor) };
        return closure_15(closure_19, obj, textColor.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.CODE_BLOCK]: {
      parse(arg0, arg1, arg2) {
        obj = str(4959).RULES[obj(undefined, 4957).AST_KEY.CODE_BLOCK];
    ...
    [closure_0(closure_2[43]).AST_KEY.SOUNDBOARD]: {
      react(node, output, key) {
        ...
        const items = ["<sound:", obj(8027).smartOutput(node, output, key), ">"];
        obj[1] = items;
        return callback2(obj(4477).Text, obj, key.key);
      }
    },
    [closure_0(closure_2[43]).AST_KEY.GUILD]: {
      react(icon, output, textColor) {
        ...
        if (null == icon.icon) {
          ...
          obj1[0] = textColor1;
          obj1[1] = tmp(8027).smartOutput(icon, output, textColor);
          ...
          const fontScale = store.getFontScale();
          if (fontScale < 1) {
            let XXSMALL = tmp(5539).GuildIconSizes.XXXSMALL;
          } else if (fontScale < 1.25) {
            XXSMALL = tmp(5539).GuildIconSizes.XXSMALL_12;
          } else {
            XXSMALL = tmp(5539).GuildIconSizes.XXSMALL;
          }
          obj2[2] = XXSMALL;
          callback(str(5539), obj2);
          const tmp4 = callback;
          const tmp6 = str(5539);
        }
      }
    ...
    [closure_0(closure_2[43]).AST_KEY.CHANNEL]: {
      react(iconType, output, textColor) {
        ...
        obj = { themedColor: str(709).colors.MENTION_FOREGROUND, source: obj(4989).getChannelMentionIcon(str), size: null, style: null };
        const fontScale = store.getFontScale();
        if (fontScale < 1) {
          let SMALL = tmp2(1296).Icon.Sizes.EXTRA_SMALL_10;
        } else if (fontScale < 1.25) {
          SMALL = tmp2(1296).Icon.Sizes.EXTRA_SMALL;
        } else {
          SMALL = tmp2(1296).Icon.Sizes.SMALL;
        }
        obj[2] = SMALL;
        obj[3] = { top: 1 };
        textColor = undefined;
        ...
        const items = [closure_15(closure_6, { style: { paddingEnd: num }, children: closure_15(obj(1296).ThemedIcon, obj) }), ];
        const tmp6 = closure_16;
        const tmp7 = closure_19;
        const tmpResult = closure_15(obj(1296).ThemedIcon, obj);
        items[1] = obj(8027).smartOutput(iconType, output, textColor);
        obj[1] = items;
    ...
    [closure_0(closure_2[43]).AST_KEY.GAME_MENTION]: {
      react(node, arg1, key) {
        return callback(str(13855), { node, state: key }, key.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.LIST]: {
      react(start, arg1, level) {
        ...
        if (level.formatInline) {
          ...
          return callback(obj(4477).Text, obj, "list-" + level.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.HEADING]: {
      react(level, output, formatInline) {
        if (formatInline.formatInline) {
          ...
          const items = [obj(8027).smartOutput(level, output, obj), " "];
          obj[2] = items;
          ...
          const items1 = [obj(8027).smartOutput(level, output, obj1), "\n"];
          obj[2] = items1;
          return closure_16(obj(4477).Text, obj, formatInline.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.SUBTEXT]: {
      react(node, output, key) {
        ...
        const items = [obj(8027).smartOutput(node, output, key), "\n"];
        obj[2] = items;
        return callback2(obj(4477).Text, obj, key.key);
      }
    },
