...
// Function ID: 4440
// Name: MarkupText
// Dependencies: [19, 17, 4440, 1387, 1985, 1909, 676, 1394, 505, 21, 4443, 4444, 4448, 712, 4459, 4643, 4303, 4691, 8859, 4825, 1236, 5949, 4163, 5972, 1297, 1486, 8609, 4160, 11163, 11146, 7973, 589, 4296, 7838, 4415, 11149, 2009, 11, 4136, 5461, 11127, 13546, 9007, 4917, 10169, 13547, 13549, 4391, 4919, 500, 4949, 13551, 10171, 2]
// Exports: default, plainMentionRenderer, plainSpoilerRenderer

// Module 4439 (MarkupText)
...
import handleTapCommandMentionDefault from "handleTapCommandMention" /* 9007 */;
...
function MarkupLink(arg0) {
  ...
  obj.inLink = true;
  obj[5] = node(7973).smartOutput(node, output, obj);
  return tmp2(node(4444).Text, obj, state.key);
}
...
function MarkupInlineCode(arg0) {
  ...
  obj[4] = node(7973).smartOutput(node, output, state);
...
function MarkupChannelMention(state) {
  ...
  if (null == node.inContent) {
    items[1] = null;
    items[2] = tmp3(7973).smartOutput(node, output, state);
    obj[2] = items;
    const tmp2Result = closure_16(state(4444).Text, obj, state.key);
...
function MarkupAttachmentLink(state) {
  ...
  let obj = { variant: str2, style: tmp.channelMentionText, children: null };
  obj = { themedColor: node(712).colors.MENTION_FOREGROUND, source: node(13546), size: null };
  const fontScale = store.getFontScale();
  if (fontScale < 1) {
  ...
  obj[2] = SMALL;
  const items = [closure_15(state(1297).ThemedIcon, obj), state(7973).smartOutput(node, output, state)];
  obj[2] = items;
  const tmp2Result = closure_16(state(4444).Text, obj, state.key);
  ...
  return {
    [closure_0(closure_2[43]).AST_KEY.TEXT]: {
      react(content, output, textColor) {
        if (typeof content.content === "string") {
          ...
          obj = { color: null, children: null };
          obj[0] = textColor;
          obj[1] = obj(7973).smartOutput(content, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.STRIKETHROUGH]: {
      react(node, output, textColor) {
        ...
        obj[3] = obj(7973).smartOutput(node, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.HIGHLIGHT]: {
      react(node, output, textColor) {
        ...
        obj = { color: textColor, children: obj(7973).smartOutput(node, output, textColor) };
        return closure_15(closure_19, obj, textColor.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.CODE_BLOCK]: {
      parse(arg0, arg1, arg2) {
        obj = str(4919).RULES[obj(undefined, 4917).AST_KEY.CODE_BLOCK];
    ...
    [closure_0(closure_2[43]).AST_KEY.SOUNDBOARD]: {
      react(node, output, key) {
        ...
        const items = ["<sound:", obj(7973).smartOutput(node, output, key), ">"];
        obj[1] = items;
        return callback2(obj(4444).Text, obj, key.key);
      }
    },
    [closure_0(closure_2[43]).AST_KEY.GUILD]: {
      react(icon, output, textColor) {
        ...
        if (null == icon.icon) {
          ...
          obj1[0] = textColor1;
          obj1[1] = tmp(7973).smartOutput(icon, output, textColor);
          ...
          const fontScale = store.getFontScale();
          if (fontScale < 1) {
            let XXSMALL = tmp(5972).GuildIconSizes.XXXSMALL;
          } else if (fontScale < 1.25) {
            XXSMALL = tmp(5972).GuildIconSizes.XXSMALL_12;
          } else {
            XXSMALL = tmp(5972).GuildIconSizes.XXSMALL;
          }
          obj2[2] = XXSMALL;
          callback(str(5972), obj2);
          const tmp4 = callback;
          const tmp6 = str(5972);
        }
      }
    ...
    [closure_0(closure_2[43]).AST_KEY.CHANNEL]: {
      react(iconType, output, textColor) {
        ...
        obj = { themedColor: str(712).colors.MENTION_FOREGROUND, source: obj(4949).getChannelMentionIcon(str), size: null, style: null };
        const fontScale = store.getFontScale();
        if (fontScale < 1) {
        ...
        items[1] = obj(7973).smartOutput(iconType, output, textColor);
        obj[1] = items;
    ...
    [closure_0(closure_2[43]).AST_KEY.GAME_MENTION]: {
      react(node, arg1, key) {
        return callback(str(13551), { node, state: key }, key.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.LIST]: {
      react(start, arg1, level) {
        ...
        if (level.formatInline) {
          ...
          return callback(obj(4444).Text, obj, "list-" + level.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.HEADING]: {
      react(level, output, formatInline) {
        if (formatInline.formatInline) {
          ...
          const items = [obj(7973).smartOutput(level, output, obj), " "];
          obj[2] = items;
          ...
          const items1 = [obj(7973).smartOutput(level, output, obj1), "\n"];
          obj[2] = items1;
          return closure_16(obj(4444).Text, obj, formatInline.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.SUBTEXT]: {
      react(node, output, key) {
        ...
        const items = [obj(7973).smartOutput(node, output, key), "\n"];
        obj[2] = items;
        return callback2(obj(4444).Text, obj, key.key);
      }
    },
