...
// Function ID: 4772
// Name: MarkupText
// Dependencies: [19, 17, 4669, 1391, 1984, 1910, 676, 1398, 505, 21, 4772, 4739, 4668, 712, 4773, 4492, 4234, 5265, 8966, 4832, 1236, 7205, 4096, 7226, 1297, 1487, 8812, 4093, 10839, 10820, 8348, 589, 4227, 8226, 4346, 10823, 2008, 11, 4069, 5454, 10796, 13122, 9597, 4770, 10101, 13123, 13125, 4322, 6846, 500, 6870, 13127, 10103, 2]
// Exports: default, plainMentionRenderer, plainSpoilerRenderer

// Module 4771 (MarkupText)
...
import handleTapCommandMentionDefault from "handleTapCommandMention" /* 9597 */;
...
function MarkupLink(arg0) {
  ...
  obj.inLink = true;
  obj[5] = node(8348).smartOutput(node, output, obj);
  return tmp2(node(4739).Text, obj, state.key);
}
...
function MarkupInlineCode(arg0) {
  ...
  obj[4] = node(8348).smartOutput(node, output, state);
...
function MarkupChannelMention(state) {
  ...
  if (null == node.inContent) {
    items[1] = null;
    items[2] = tmp3(8348).smartOutput(node, output, state);
    obj[2] = items;
    const tmp2Result = closure_16(state(4739).Text, obj, state.key);
...
function MarkupAttachmentLink(state) {
  ...
  let obj = { variant: str2, style: tmp.channelMentionText, children: null };
  obj = { themedColor: node(712).colors.MENTION_FOREGROUND, source: node(13122), size: null };
  const fontScale = store.getFontScale();
  if (fontScale < 1) {
  ...
  obj[2] = SMALL;
  const items = [closure_15(state(1297).ThemedIcon, obj), state(8348).smartOutput(node, output, state)];
  obj[2] = items;
  const tmp2Result = closure_16(state(4739).Text, obj, state.key);
  ...
  return {
    [closure_0(closure_2[43]).AST_KEY.TEXT]: {
      react(content, output, textColor) {
        if (typeof content.content === "string") {
          ...
          obj = { color: null, children: null };
          obj[0] = textColor;
          obj[1] = obj(8348).smartOutput(content, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.STRIKETHROUGH]: {
      react(node, output, textColor) {
        ...
        obj[3] = obj(8348).smartOutput(node, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.HIGHLIGHT]: {
      react(node, output, textColor) {
        ...
        obj = { color: textColor, children: obj(8348).smartOutput(node, output, textColor) };
        return closure_15(closure_19, obj, textColor.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.CODE_BLOCK]: {
      parse(arg0, arg1, arg2) {
        obj = str(6846).RULES[obj(undefined, 4770).AST_KEY.CODE_BLOCK];
    ...
    [closure_0(closure_2[43]).AST_KEY.SOUNDBOARD]: {
      react(node, output, key) {
        ...
        const items = ["<sound:", obj(8348).smartOutput(node, output, key), ">"];
        obj[1] = items;
        return callback2(obj(4739).Text, obj, key.key);
      }
    },
    [closure_0(closure_2[43]).AST_KEY.GUILD]: {
      react(icon, output, textColor) {
        ...
        if (null == icon.icon) {
          ...
          obj1[0] = textColor1;
          obj1[1] = tmp(8348).smartOutput(icon, output, textColor);
          ...
          const fontScale = store.getFontScale();
          if (fontScale < 1) {
            let XXSMALL = tmp(7226).GuildIconSizes.XXXSMALL;
          } else if (fontScale < 1.25) {
            XXSMALL = tmp(7226).GuildIconSizes.XXSMALL_12;
          } else {
            XXSMALL = tmp(7226).GuildIconSizes.XXSMALL;
          }
          obj2[2] = XXSMALL;
          callback(str(7226), obj2);
          const tmp4 = callback;
          const tmp6 = str(7226);
        }
      }
    ...
    [closure_0(closure_2[43]).AST_KEY.CHANNEL]: {
      react(iconType, output, textColor) {
        ...
        obj = { themedColor: str(712).colors.MENTION_FOREGROUND, source: obj(6870).getChannelMentionIcon(str), size: null, style: null };
        const fontScale = store.getFontScale();
        if (fontScale < 1) {
        ...
        items[1] = obj(8348).smartOutput(iconType, output, textColor);
        obj[1] = items;
    ...
    [closure_0(closure_2[43]).AST_KEY.GAME_MENTION]: {
      react(node, arg1, key) {
        return callback(str(13127), { node, state: key }, key.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.LIST]: {
      react(start, arg1, level) {
        ...
        if (level.formatInline) {
          ...
          return callback(obj(4739).Text, obj, "list-" + level.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.HEADING]: {
      react(level, output, formatInline) {
        if (formatInline.formatInline) {
          ...
          const items = [obj(8348).smartOutput(level, output, obj), " "];
          obj[2] = items;
          ...
          const items1 = [obj(8348).smartOutput(level, output, obj1), "\n"];
          obj[2] = items1;
          return closure_16(obj(4739).Text, obj, formatInline.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.SUBTEXT]: {
      react(node, output, key) {
        ...
        const items = [obj(8348).smartOutput(node, output, key), "\n"];
        obj[2] = items;
        return callback2(obj(4739).Text, obj, key.key);
      }
    },
