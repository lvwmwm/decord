...
// Function ID: 4437
// Name: MarkupText
// Dependencies: [19, 17, 4437, 1391, 1985, 1910, 676, 1398, 505, 21, 4440, 4441, 4445, 712, 4456, 4640, 4300, 4676, 8819, 4809, 1236, 5933, 4161, 5956, 1297, 1487, 8572, 4158, 11116, 11099, 7937, 589, 4293, 7803, 4412, 11102, 2009, 11, 4134, 5445, 11080, 13493, 8966, 4901, 10128, 13494, 13496, 4388, 4903, 500, 4933, 13498, 10130, 2]
// Exports: default, plainMentionRenderer, plainSpoilerRenderer

// Module 4436 (MarkupText)
...
import handleTapCommandMentionDefault from "handleTapCommandMention" /* 8966 */;
...
function MarkupLink(arg0) {
  ...
  obj.inLink = true;
  obj[5] = node(7937).smartOutput(node, output, obj);
  return tmp2(node(4441).Text, obj, state.key);
}
...
function MarkupInlineCode(arg0) {
  ...
  obj[4] = node(7937).smartOutput(node, output, state);
...
function MarkupChannelMention(state) {
  ...
  if (null == node.inContent) {
    items[1] = null;
    items[2] = tmp3(7937).smartOutput(node, output, state);
    obj[2] = items;
    const tmp2Result = closure_16(state(4441).Text, obj, state.key);
...
function MarkupAttachmentLink(state) {
  ...
  let obj = { variant: str2, style: tmp.channelMentionText, children: null };
  obj = { themedColor: node(712).colors.MENTION_FOREGROUND, source: node(13493), size: null };
  const fontScale = store.getFontScale();
  if (fontScale < 1) {
  ...
  obj[2] = SMALL;
  const items = [closure_15(state(1297).ThemedIcon, obj), state(7937).smartOutput(node, output, state)];
  obj[2] = items;
  const tmp2Result = closure_16(state(4441).Text, obj, state.key);
  ...
  return {
    [closure_0(closure_2[43]).AST_KEY.TEXT]: {
      react(content, output, textColor) {
        if (typeof content.content === "string") {
          ...
          obj = { color: null, children: null };
          obj[0] = textColor;
          obj[1] = obj(7937).smartOutput(content, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.STRIKETHROUGH]: {
      react(node, output, textColor) {
        ...
        obj[3] = obj(7937).smartOutput(node, output, textColor);
    ...
    [closure_0(closure_2[43]).AST_KEY.HIGHLIGHT]: {
      react(node, output, textColor) {
        ...
        obj = { color: textColor, children: obj(7937).smartOutput(node, output, textColor) };
        return closure_15(closure_19, obj, textColor.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.CODE_BLOCK]: {
      parse(arg0, arg1, arg2) {
        obj = str(4903).RULES[obj(undefined, 4901).AST_KEY.CODE_BLOCK];
    ...
    [closure_0(closure_2[43]).AST_KEY.SOUNDBOARD]: {
      react(node, output, key) {
        ...
        const items = ["<sound:", obj(7937).smartOutput(node, output, key), ">"];
        obj[1] = items;
        return callback2(obj(4441).Text, obj, key.key);
      }
    },
    [closure_0(closure_2[43]).AST_KEY.GUILD]: {
      react(icon, output, textColor) {
        ...
        if (null == icon.icon) {
          ...
          obj1[0] = textColor1;
          obj1[1] = tmp(7937).smartOutput(icon, output, textColor);
          ...
          const fontScale = store.getFontScale();
          if (fontScale < 1) {
            let XXSMALL = tmp(5956).GuildIconSizes.XXXSMALL;
          } else if (fontScale < 1.25) {
            XXSMALL = tmp(5956).GuildIconSizes.XXSMALL_12;
          } else {
            XXSMALL = tmp(5956).GuildIconSizes.XXSMALL;
          }
          obj2[2] = XXSMALL;
          callback(str(5956), obj2);
          const tmp4 = callback;
          const tmp6 = str(5956);
        }
      }
    ...
    [closure_0(closure_2[43]).AST_KEY.CHANNEL]: {
      react(iconType, output, textColor) {
        ...
        obj = { themedColor: str(712).colors.MENTION_FOREGROUND, source: obj(4933).getChannelMentionIcon(str), size: null, style: null };
        const fontScale = store.getFontScale();
        if (fontScale < 1) {
        ...
        items[1] = obj(7937).smartOutput(iconType, output, textColor);
        obj[1] = items;
    ...
    [closure_0(closure_2[43]).AST_KEY.GAME_MENTION]: {
      react(node, arg1, key) {
        return callback(str(13498), { node, state: key }, key.key);
      }
    },
    ...
    [closure_0(closure_2[43]).AST_KEY.LIST]: {
      react(start, arg1, level) {
        ...
        if (level.formatInline) {
          ...
          return callback(obj(4441).Text, obj, "list-" + level.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.HEADING]: {
      react(level, output, formatInline) {
        if (formatInline.formatInline) {
          ...
          const items = [obj(7937).smartOutput(level, output, obj), " "];
          obj[2] = items;
          ...
          const items1 = [obj(7937).smartOutput(level, output, obj1), "\n"];
          obj[2] = items1;
          return closure_16(obj(4441).Text, obj, formatInline.key);
        }
      }
    },
    [closure_0(closure_2[43]).AST_KEY.SUBTEXT]: {
      react(node, output, key) {
        ...
        const items = [obj(7937).smartOutput(node, output, key), "\n"];
        obj[2] = items;
        return callback2(obj(4441).Text, obj, key.key);
      }
    },
