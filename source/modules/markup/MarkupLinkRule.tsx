...
// Dependencies: [32, 1386, 4691, 4685, 1351, 4239, 1467, 4692, 3954, 12, 2]

// Module 4690 (validateContentTypes)
  ...
  while (iter !== undefined) {
    ...
    if (undefined !== nextResult) {
      ...
      if (items.includes(tmp3.type)) {
        ...
        if (tmp3.type === require(4685) /* AST_KEY */.AST_KEY.INLINE_CODE) {
          items2 = [];
          let tmp7 = items2;
          let tmp8 = items;
          let num = 0;
...
function getRawText(content) {
  ...
  for (; iter !== undefined; str = str + tmp2.content) {
    ...
    if (require(4685) /* AST_KEY */.AST_KEY.TEXT !== type) {
      if (tmp3(4685).AST_KEY.INLINE_CODE !== type) {
        if (tmp3(4685).AST_KEY.CUSTOM_EMOJI === type) {
          let tmp16 = str;
          let tmp17 = nextResult;
          str = `${tmp2.name}`;
        } else if (tmp3(4685).AST_KEY.EMOJI === type) {
          let tmp14 = str;
          let tmp15 = nextResult;
          str = `${tmp2.name}${tmp2.surrogate}`;
        } else if (tmp3(4685).AST_KEY.LINE_BREAK === type) {
          let tmp13 = str;
          str = `${tmp2.name}${tmp2.surrogate}
  `;
        } else {
          if (tmp3(4685).AST_KEY.STRONG !== type) {
            if (tmp3(4685).AST_KEY.ITALICS !== type) {
              if (tmp3(4685).AST_KEY.UNDERLINE !== type) {
                if (tmp3(4685).AST_KEY.STRIKETHROUGH !== type) {
                  if (tmp3(4685).AST_KEY.SPOILER !== type) {
                    if (tmp3(4685).AST_KEY.TIMESTAMP === type) {
                      let tmp9 = str;
                      str = `${tmp2.name}${tmp2.surrogate}
  <timestamp>`;
                    } else {
                      if (tmp3(4685).AST_KEY.BLOCK_QUOTE !== type) {
                        if (tmp3(4685).AST_KEY.LIST !== type) {
                          if (tmp3(4685).AST_KEY.HEADING !== type) {
                            if (tmp3(4685).AST_KEY.SUBTEXT !== type) {
  ...
  try {
    if (isSuspiciousUrl(url)) {
      ...
      if (items.includes(str.toLowerCase())) {
        ...
        let obj = importDefault(4692);
        tmp9.hostname = obj.toASCII(tmp9.hostname.toLowerCase());
        tmp9.username = "";
        tmp9.password = "";
        obj = { target: null, displayTarget: null };
        obj[0] = require(4691) /* safelyPartiallyDecodeURIComponent */.safelyMakeUrlHumanReadable(tmp9);
        const obj2 = require(4691) /* safelyPartiallyDecodeURIComponent */;
        const str6 = tmp9.hostname;
        obj[1] = require(4691) /* safelyPartiallyDecodeURIComponent */.safelyMakeUrlHumanReadable(tmp9);
        return obj;
      } else {
        const _Error = Error;
...
obj.match = function match(arr, allowLinks) {
  if (allowLinks.allowLinks) {
    if (-1 === arr.indexOf("](")) {
      ...
      return importDefault(3954).defaultRules.link.match(arr, allowLinks, arg2);
    }
  } else {
    return null;
  }
...
obj.parse = function parse(arg0, rules, allowEmojiLinks) {
  ...
  if (isSuspiciousUrl(tmp4)) {
    ...
    obj[0] = require(4685) /* AST_KEY */.AST_KEY.TEXT;
    obj[1] = tmp2;
    return obj;
  } else {
    obj = tmp2;
    ...
    if (null == value) {
      let obj1 = require(4691) /* safelyPartiallyDecodeURIComponent */;
      const sanitizeWhitespaceResult = obj1.sanitizeWhitespace(tmp4);
      obj = { whitespaceSanitized: null, fullySanitized: null };
      obj[0] = sanitizeWhitespaceResult;
      let obj3 = require(4691) /* safelyPartiallyDecodeURIComponent */;
    ...
    value = obj.get(tmp3);
    if (null == value) {
      let obj4 = require(4691) /* safelyPartiallyDecodeURIComponent */;
      ...
      let obj6 = require(4691) /* safelyPartiallyDecodeURIComponent */;
    ...
    if (null == value1) {
      let obj7 = require(4691) /* safelyPartiallyDecodeURIComponent */;
      ...
      obj2[1] = require(4691) /* safelyPartiallyDecodeURIComponent */.sanitizeUnicodeConfusables(sanitizeWhitespaceResult2);
      const result2 = obj.set(str, obj2);
      value1 = obj2;
      const obj10 = require(4691) /* safelyPartiallyDecodeURIComponent */;
    }
    const trimmed = str3.trim();
    if (0 !== str2.trim().length) {
      if (0 !== trimmed.length) {
        const tmp53 = punycodeLink(importDefault(3954).unescapeUrl(tmp4));
        const obj20 = importDefault(3954);
        if (null != tmp53) {
          if (!tmp23) {
            obj3 = {};
            ...
            items2 = [require(4685) /* AST_KEY */.AST_KEY.EMOJI];
            ...
            if (null != tmp43) {
              if (null != validateContentTypes(rules(value1.fullySanitized, obj3), items1)) {
                if (0 === str4.trim().length) {
                  ...
                  obj4[0] = tmp42(4685).AST_KEY.TEXT;
                  obj4[1] = tmp2;
                  return obj4;
                } else {
                  let tmp51Result = tmp51(12);
                  tmp51Result = tmp51(3954);
            ...
            obj6[0] = require(4685) /* AST_KEY */.AST_KEY.TEXT;
            obj6[1] = tmp2;
            return obj6;
          }
        }
        obj7 = { type: null, content: null };
        obj7[0] = require(4685) /* AST_KEY */.AST_KEY.TEXT;
        obj7[1] = tmp2;
        return obj7;
      }
    }
    const obj8 = { type: null, content: null };
    obj8[0] = require(4685) /* AST_KEY */.AST_KEY.TEXT;
    obj8[1] = tmp2;
    return obj8;
  }
