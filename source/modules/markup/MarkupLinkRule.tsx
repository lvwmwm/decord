...
// Dependencies: [32, 1386, 4721, 4715, 1351, 4269, 1467, 4722, 3983, 12, 2]
...
obj.match = function match(arr, allowLinks) {
  if (allowLinks.allowLinks) {
    if (-1 === arr.indexOf("](")) {
      ...
      return importDefault(3983).defaultRules.link.match(arr, allowLinks, arg2);
    }
  } else {
    return null;
  }
...
obj.parse = function parse(arg0, rules, allowEmojiLinks) {
  ...
  if (isSuspiciousUrl(tmp4)) {
    ...
    if (0 !== str2.trim().length) {
      if (0 !== trimmed.length) {
        const tmp53 = punycodeLink(importDefault(3983).unescapeUrl(tmp4));
        const obj20 = importDefault(3983);
        if (null != tmp53) {
          if (!tmp23) {
            obj3 = {};
