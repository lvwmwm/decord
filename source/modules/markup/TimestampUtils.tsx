...
// Dependencies: [3954, 3866, 687, 2]
// Exports: formatTimestampMention, parseTimestamp, unparseTimestamp

// Module 4739 (TIMESTAMP_FORMATS)
let obj = {
  t(date) {
    return require(3954) /* resetCache */.dateFormat(date, "LT");
  },
  T(date) {
    return require(3954) /* resetCache */.dateFormat(date, "LTS");
  },
  d(date) {
    return require(3954) /* resetCache */.dateFormat(date, "L");
  },
  D(date) {
    return require(3954) /* resetCache */.dateFormat(date, "LL");
  },
  f(date) {
    return require(3954) /* resetCache */.dateFormat(date, "LLL");
  },
  F(date) {
    return require(3954) /* resetCache */.dateFormat(date, "LLLL");
  },
  s(date) {
    return require(3954) /* resetCache */.dateFormat(date, "L LT");
  },
  S(date) {
    return require(3954) /* resetCache */.dateFormat(date, "L LTS");
  },
  R(toDate) {
    const result = importDefault(3866).relativeTimeThreshold("s");
    const obj = importDefault(3866);
    const result1 = importDefault(3866).relativeTimeThreshold("s", 60);
    const obj2 = importDefault(3866);
    const result2 = importDefault(3866).relativeTimeThreshold("ss");
    const obj3 = importDefault(3866);
    const result3 = importDefault(3866).relativeTimeThreshold("ss", -1);
    const obj4 = importDefault(3866);
    const result4 = importDefault(3866).relativeTimeThreshold("m");
    const obj5 = importDefault(3866);
    const result5 = importDefault(3866).relativeTimeThreshold("m", 60);
    try {
      ...
      const result7 = tmp(3866).relativeTimeThreshold("ss", result2);
      const tmpResult1 = tmp(3866);
      const result8 = tmp(3866).relativeTimeThreshold("m", result4);
      if (fromNowResult == null) {
        const tmpResult3 = tmp(3866);
        fromNowResult = tmp(3866)(toDate.toDate()).fromNow();
        const tmpResult4Result = tmp(3866)(toDate.toDate());
      }
