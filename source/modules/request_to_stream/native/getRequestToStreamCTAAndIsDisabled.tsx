...
// Dependencies: [57, 1194, 10896, 21, 10883, 1212, 2653, 2]
// Exports: default

// Module 12318 (getRequestToStreamCTAAndIsDisabled)
...
export default function getRequestToStreamCTAAndIsDisabled(id) {
  let obj = require(10896) /* canFulfillStreamRequest */;
  const tmp = callback(obj.canFulfillStreamRequest(id, true), 2);
  id = id.getId();
  const obj2 = importDefault(21);
  const sum = importDefault(21).extractTimestamp(id.id) + require(10883) /* result */.EMBED_LIFETIME;
  ...
  if (tmp6) {
    ...
    flag = false;
    if (!tmp[0]) {
      if (require(10896) /* canFulfillStreamRequest */.StreamRequestUnfulfillableReason.ALREADY_STREAMING === tmp2) {
        const intl4 = require(1212) /* getSystemLocale */.intl;
        stringResult1 = intl4.string(importDefault(2653).P0wwmM);
        flag = true;
      } else if (require(10896) /* canFulfillStreamRequest */.StreamRequestUnfulfillableReason.NOT_RUNNING_GAME === tmp2) {
        const intl3 = require(1212) /* getSystemLocale */.intl;
        stringResult1 = intl3.string(importDefault(2653)["43zohO"]);
        flag = true;
      } else if (require(10896) /* canFulfillStreamRequest */.StreamRequestUnfulfillableReason.NOT_IN_VOICE_CHANNEL === tmp2) {
        ...
        stringResult1 = stringResult;
        flag = false;
        if (require(10896) /* canFulfillStreamRequest */.StreamRequestUnfulfillableReason.NO_PERMISSION === tmp2) {
