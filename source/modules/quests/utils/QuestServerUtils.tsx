...
// Dependencies: [32, 4503, 7169, 7166, 2]
...
// Module 7168 (progressFromServer)
...
function getSimpleRewardFromServer(type) {
  if (type.type === require(7166) /* QuestRewardTypes */.QuestRewardTypes.VIRTUAL_CURRENCY) {
...
function _questsEntitlementFromServer(skuId) {
  ...
  if (null != quest_rewards) {
    ...
    if (require(7166) /* QuestRewardTypes */.QuestRewardTypes.IN_GAME === tag) {
...
export const isQuestWithKnownConfigVersion = function isQuestWithKnownConfigVersion(config) {
  try {
    const match = require(4503) /* t */.match(config.config);
...
export const questConfigFromServer = function questConfigFromServer(body) {
  const match = require(4503) /* t */.match(body);
  const str = require(4503) /* t */;
...
export const questWithUserStatusFromServer = function questWithUserStatusFromServer(body) {
  ...
  const match = require(4503) /* t */.match(body.config);
  const str = require(4503) /* t */;
...
export const getClaimedQuestWithUserStatusFromServer = function getClaimedQuestWithUserStatusFromServer(id) {
  ...
  obj[8] = require(7169) /* questFromServerV2 */.questCosponsorMetadataFromServer(config.cosponsor_metadata);
  obj[1] = obj;
  let tmp = null;
