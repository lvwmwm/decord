...
// Dependencies: [32, 4454, 7062, 7059, 2]
...
// Module 7061 (progressFromServer)
...
function getSimpleRewardFromServer(type) {
  if (type.type === require(7059) /* QuestRewardTypes */.QuestRewardTypes.VIRTUAL_CURRENCY) {
...
function _questsEntitlementFromServer(skuId) {
  ...
  if (null != quest_rewards) {
    ...
    if (require(7059) /* QuestRewardTypes */.QuestRewardTypes.IN_GAME === tag) {
...
export const isQuestWithKnownConfigVersion = function isQuestWithKnownConfigVersion(config) {
  try {
    const match = require(4454) /* t */.match(config.config);
...
export const questConfigFromServer = function questConfigFromServer(body) {
  const match = require(4454) /* t */.match(body);
  const str = require(4454) /* t */;
...
export const questWithUserStatusFromServer = function questWithUserStatusFromServer(body) {
  ...
  const match = require(4454) /* t */.match(body.config);
  const str = require(4454) /* t */;
...
export const getClaimedQuestWithUserStatusFromServer = function getClaimedQuestWithUserStatusFromServer(id) {
  ...
  obj[8] = require(7062) /* questFromServerV2 */.questCosponsorMetadataFromServer(config.cosponsor_metadata);
  obj[1] = obj;
  let tmp = null;
