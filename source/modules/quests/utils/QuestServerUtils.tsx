...
// Dependencies: [32, 4543, 7208, 7205, 2]
...
// Module 7207 (progressFromServer)
...
function getSimpleRewardFromServer(type) {
  if (type.type === require(7205) /* QuestRewardTypes */.QuestRewardTypes.VIRTUAL_CURRENCY) {
...
function _questsEntitlementFromServer(skuId) {
  ...
  if (null != quest_rewards) {
    ...
    if (require(7205) /* QuestRewardTypes */.QuestRewardTypes.IN_GAME === tag) {
...
export const isQuestWithKnownConfigVersion = function isQuestWithKnownConfigVersion(config) {
  try {
    const match = require(4543) /* t */.match(config.config);
...
export const questConfigFromServer = function questConfigFromServer(body) {
  const match = require(4543) /* t */.match(body);
  const str = require(4543) /* t */;
...
export const questWithUserStatusFromServer = function questWithUserStatusFromServer(body) {
  ...
  const match = require(4543) /* t */.match(body.config);
  const str = require(4543) /* t */;
...
export const getClaimedQuestWithUserStatusFromServer = function getClaimedQuestWithUserStatusFromServer(id) {
  ...
  obj[8] = require(7208) /* questFromServerV2 */.questCosponsorMetadataFromServer(config.cosponsor_metadata);
  obj[1] = obj;
  let tmp = null;
