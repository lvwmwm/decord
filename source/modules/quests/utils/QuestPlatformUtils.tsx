...
// Dependencies: [5113, 676, 7077, 5120, 9405, 9406, 9410, 7082, 7071, 10378, 1236, 500, 10368, 4221, 10470, 1231, 514, 10471, 3946, 5910, 709, 2]
...
// Module 10377 (supportedConsoles)
...
function supportedConsoles(quest) {
  ...
  for (const item10013 of keys) {
    ...
    if (require(5120) /* set */.FirstPartyQuestTaskTypes.PLAY_ON_XBOX === item10013) {
      let tmp6 = constants2;
      let arr = items.push(constants2.XBOX);
    } else if (tmp2(5120).FirstPartyQuestTaskTypes.PLAY_ON_PLAYSTATION === item10013) {
      let tmp4 = constants2;
...
export const supportedTaskPlatforms = function supportedTaskPlatforms(closure_0) {
  ...
  const items = [];
  const tmpResult1 = require(7077) /* getApplicationIdsByTaskTypes */;
  if (hasPlayOnDesktopTaskResult) {
...
export const isQuestSupportedOnWeb = function isQuestSupportedOnWeb(userStatus) {
  let hasWatchVideoTasksResult = require(7077) /* getApplicationIdsByTaskTypes */.hasWatchVideoTasks(userStatus);
  const obj = require(7077) /* getApplicationIdsByTaskTypes */;
...
export const openAuthorizationConnectionModal = function openAuthorizationConnectionModal(platformType, ctaContent) {
  ...
  let obj = require(9405) /* apexExperiment */;
  if (obj.shouldMigrateToAdAnalyticsInterface(require(9405) /* apexExperiment */.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "open_authorization_connection_modal")) {
  ...
  importDefault(10378)({ platformType: platformType.platformType, location: ctaContent.ctaContent });
};
...
export const openGameLinkDirectly = function openGameLinkDirectly(quest, arg1) {
  ...
  if (tmpResult1.shouldMigrateToAdAnalyticsInterface(f84851(9405).AdAnalyticsInterfaceExperimentStep.STEP_3_CLICKED_EXTERNAL, "open_game_link_directly")) {
    ...
    obj1[6] = tmp7;
    let result = tmp(7071).trackQuestContentClicked(obj1);
    const tmpResult3 = tmp(7071);
  }
  const ComponentDispatch = tmp(1231).ComponentDispatch;
  ComponentDispatch.dispatch(constants.QUEST_GAME_LINK_OPENED);
  const AppStoreBottomSheetOverlayFeatureGate = tmp(10471).AppStoreBottomSheetOverlayFeatureGate;
  if (!AppStoreBottomSheetOverlayFeatureGate.getConfig({ location: "quest_open_game_link" }).enabled) {
    if (null != tmp4) {
      ...
      if (null != ctaConfig2) {
        ...
        obj2[0] = tmp(10368).getCtaLink(quest.config);
        ({ android: obj10[1], ios: obj10[2] } = ctaConfig2);
        tmp13 = getInlineStoreParamsFromCta(obj2);
        const tmpResult4 = tmp(10368);
      }
      if (null != tmp13) {
        f84851 = (event, arg1, time_spent_ms) => f84851(outer1_2[8]).trackAppStoreOverlayEvent(f84851, closure_1, arg1, event, time_spent_ms);
        ({ clearAppStoreOverlayOpen: closure_1, setAppStoreOverlayOpen: dependencyMap } = tmp(10470));
        let str = tmp13.appId;
        str = undefined;
        if (str != null) {
          str = str.toString();
        }
        const result1 = tmp(4221).openPlayStoreInlineInstall(tmp13.url, str, (arg0) => {
          adCreativeType();
          f84852(outer1_4.QUEST_APP_STORE_OVERLAY_CLOSED, str, arg0);
          const ComponentDispatch = f84852(1231).ComponentDispatch;
          ComponentDispatch.dispatch(outer1_5.QUEST_APP_STORE_OVERLAY_FINISHED);
        });
        const tmpResult5 = tmp(10470);
        result1.then((arg0) => {
          if (arg0) {
            ...
            dependencyMap(obj);
            f84852(outer1_4.QUEST_APP_STORE_OVERLAY_OPEN_SUCCEEDED, str);
          } else {
            adCreativeType();
            f84852(outer1_4.QUEST_APP_STORE_OVERLAY_OPEN_FAILED, str);
            const ComponentDispatch = f84852(1231).ComponentDispatch;
            ComponentDispatch.dispatch(outer1_5.QUEST_APP_STORE_OVERLAY_FINISHED);
          }
        }).catch(() => {
...
export const openAdGameLinkDirectly = function openAdGameLinkDirectly(adContentId, arg1) {
  ...
  let obj = f84852(9405);
  if (obj.shouldMigrateToAdAnalyticsInterface(f84852(9405).AdAnalyticsInterfaceExperimentStep.STEP_3_CLICKED_EXTERNAL, "open_ad_game_link_directly")) {
    let tmp2Result = tmp2(9406);
    ...
    obj[0] = tmp2(9410).AdUserActionType.CLICK_EXTERNAL_ADVERTISER_CTA;
  ...
  ComponentDispatch.dispatch(constants.QUEST_GAME_LINK_OPENED);
  const AppStoreBottomSheetOverlayFeatureGate = tmp2(10471).AppStoreBottomSheetOverlayFeatureGate;
  if (!AppStoreBottomSheetOverlayFeatureGate.getConfig({ location: "quest_open_game_link" }).enabled) {
    if (null != tmp) {
      const tmp11 = getInlineStoreParamsFromCta(cta);
      if (null != tmp11) {
        f84852 = (event, arg1, time_spent_ms) => {
          let obj = f84852(7071);
          obj = { adContentId: f84852, adCreativeType };
          return obj.trackAdContentAppStoreOverlayEvent(obj, dependencyMap, arg1, event, time_spent_ms);
        };
        ({ clearAppStoreOverlayOpen: adCreativeType, setAppStoreOverlayOpen: closure_2 } = tmp2(10470));
        let str = tmp11.appId;
        str = undefined;
        if (str != null) {
          str = str.toString();
        }
        const result1 = tmp2(4221).openPlayStoreInlineInstall(tmp11.url, str, (arg0) => {
          adCreativeType();
          f84852(outer1_4.QUEST_APP_STORE_OVERLAY_CLOSED, str, arg0);
          const ComponentDispatch = f84852(1231).ComponentDispatch;
          ComponentDispatch.dispatch(outer1_5.QUEST_APP_STORE_OVERLAY_FINISHED);
        });
        const tmp2Result1 = tmp2(10470);
        result1.then((arg0) => {
          if (arg0) {
            const obj = { trackOverlayEvent: null };
            obj[0] = function trackOverlayEvent(QUEST_APP_STORE_OVERLAY_BACKGROUNDED, arg1) {
  ...
  adCreativeType(3946)(url);
};
export const openConsoleConnectionSettings = function openConsoleConnectionSettings(quest, arg1) {
  quest = quest.quest;
  let obj = require(9405) /* apexExperiment */;
  if (obj.shouldMigrateToAdAnalyticsInterface(require(9405) /* apexExperiment */.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "open_console_connection_settings")) {
    let tmpResult = tmp(9406);
  ...
  require(5910) /* openUserSettings */.openUserSettings({ screen: constants3.CONNECTIONS });
};
export const openAddConsoleConnectionModal = function openAddConsoleConnectionModal(quest, arg1) {
  quest = quest.quest;
  let obj = require(9405) /* apexExperiment */;
  if (obj.shouldMigrateToAdAnalyticsInterface(require(9405) /* apexExperiment */.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "open_add_console_connection_modal")) {
    let tmpResult = tmp(9406);
  ...
  if (1 === arr.length) {
    ...
    obj1[0] = arr.at(0);
    return importDefault(10378)(obj1);
  } else {
...
export const openSingleConsoleConnectionModal = function openSingleConsoleConnectionModal(quest, arg1, platformType) {
  quest = quest.quest;
  let obj = require(9405) /* apexExperiment */;
  if (obj.shouldMigrateToAdAnalyticsInterface(require(9405) /* apexExperiment */.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "open_single_console_connection_modal")) {
    let tmpResult = tmp(9406);
    obj = { type: null, adCreativeType: null, adCreativeId: null, questContentCTA: null, surfaceId: null, sourceQuestContent: null, impressionId: null, questContentPosition: null, questContentRowIndex: null };
    obj[0] = tmp(9410).AdUserActionType.CLICK_INTERNAL;
    obj[1] = tmp(7082).AdCreativeType.QUEST;
  ...
  return importDefault(10378)({ platformType });
};
