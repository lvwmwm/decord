...
// Dependencies: [7148, 10548, 7310, 7303, 3622, 1236, 10549, 10550, 10551, 10552, 10553, 5271, 2]
...
// Module 10547 (_getDefaultRewardName)
...
import getQuestOrbMultiplierEligibilityForUser from "getQuestOrbMultiplierEligibilityForUser" /* 10549 */;
import getDurationStringOfFractionalPremium from "getDurationStringOfFractionalPremium" /* 10551 */;
import getContextualEntrypointHeading from "getContextualEntrypointHeading" /* 10552 */;
import QuestRewardAssignmentMethods from "QuestRewardAssignmentMethods" /* 10553 */;
...
function _getDefaultRewardName(rewardsConfig, stateFromStores, arg2) {
  ...
  if (rewards.some((type) => type.type === callback(7310).QuestRewardTypes.FRACTIONAL_PREMIUM)) {
    ...
    if (rewards2.some((type) => type.type === callback(7310).QuestRewardTypes.VIRTUAL_CURRENCY)) {
      rewards = rewardsConfig.rewardsConfig.rewards;
      const found = rewards.find((type) => type.type === callback(7310).QuestRewardTypes.VIRTUAL_CURRENCY);
      let num = null;
      if (null != found) {
        if (null == stateFromStores) {
          ...
          const found1 = rewards1.find((type) => type.type === callback(7310).QuestRewardTypes.VIRTUAL_CURRENCY);
      ...
      if (num == null) {
        ...
        const found2 = rewards2.find((type) => type.type === callback(7310).QuestRewardTypes.VIRTUAL_CURRENCY);
...
export const hasFractionalPremiumQuestReward = function hasFractionalPremiumQuestReward(rewardsConfig) {
  const rewards = rewardsConfig.rewardsConfig.rewards;
  return rewards.some((type) => type.type === callback(7310).QuestRewardTypes.FRACTIONAL_PREMIUM);
};
export const hasVirtualCurrencyReward = function hasVirtualCurrencyReward(config) {
  const rewards = config.rewardsConfig.rewards;
  return rewards.some((type) => type.type === callback(7310).QuestRewardTypes.VIRTUAL_CURRENCY);
};
...
export const isCollectibleQuestRewardPermanentWithPremiumSubscription = function isCollectibleQuestRewardPermanentWithPremiumSubscription(config) {
  ...
  if (hasItem) {
    ...
    let tmp11 = null;
    if (type1 === tmp3(7310).QuestRewardTypes.COLLECTIBLE) {
      tmp11 = found1;
    }
    let expirationMode1;
    if (tmp11 != null) {
    ...
    hasItem = expirationMode1 === tmp3(10548).QuestRewardExpirationMode.PREMIUM_PERMANENT;
  }
  return hasItem;
};
...
export const getPlatformString = function getPlatformString(arg0) {
  ...
  } else if (tmp(5271).QuestRewardCodePlatforms.PLAYSTATION === arg0) {
    const intl4 = tmp(1236).intl;
    return intl4.string(tmp(1236).t["6IeKx2"]);
  } else if (tmp(5271).QuestRewardCodePlatforms.SWITCH === arg0) {
    const intl3 = tmp(1236).intl;
    return intl3.string(tmp(1236).t["1pp0su"]);
  } else if (tmp(5271).QuestRewardCodePlatforms.PC === arg0) {
    const intl2 = tmp(1236).intl;
    return intl2.string(tmp(1236).t["YK+wUg"]);
  } else if (tmp(5271).QuestRewardCodePlatforms.CROSS_PLATFORM === arg0) {
