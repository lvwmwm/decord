...
// Function ID: 14729
// Name: StyleSheet
// Dependencies: [19, 17, 1922, 21, 4448, 712, 11046, 11059, 589, 10889, 7435, 1236, 4444, 5362, 5569, 14699, 10884, 14691, 11297, 14686, 5036, 8570, 4908, 5461, 14729, 8546, 2]
...
import openQuestAccessSuspendedBottomSheetDefault from "openQuestAccessSuspendedBottomSheet" /* 14686 */;
import COMPLETION_GLOW_SHADOW_RADIUSDefault from "COMPLETION_GLOW_SHADOW_RADIUS" /* 14699 */;
import _modDef14729 from "module_14729" /* 14729 */;
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = callback();
  let obj = questTaskDetails(11046);
  questTaskDetails = obj.useQuestTaskDetails(quest);
  obj1 = questTaskDetails(11046);
  ...
  defaultRewardName = questTaskDetails(10889).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result2 = questTaskDetails(10889);
  const isSponsoredPlayQuestResult = questTaskDetails(7435).isSponsoredPlayQuest(quest);
  ...
  const memo = React.useMemo(() => {
    ...
    if (isQuestProgressing) {
      ...
      if (c8) {
      ...
      } else if (questTaskDetails.taskType === questTaskDetails(5362).FirstPartyQuestTaskTypes.STREAM_ON_DESKTOP) {
        const intl4 = questTaskDetails(1236).intl;
        obj1 = { gameName: null };
        obj1[0] = gameTitle;
        stringResult = intl4.formatToPlainString(questTaskDetails(1236).t.boMftC, obj1);
      } else if (tmp41.taskType === questTaskDetails(5362).FirstPartyQuestTaskTypes.PLAY_ON_DESKTOP) {
        ...
        const CONSOLE = questTaskDetails(5362).FirstPartyQuestTaskTypesSets.CONSOLE;
        stringResult = null;
  ...
  const tmp2Result3 = questTaskDetails(7435);
  if (!tmp7) {
    tmp7 = isQuestProgressing;
  }
  obj1[3] = !tmp7;
  const items2 = [defaultRewardName(isQuestProgressing(14699), obj1), ];
  ...
  if (tmp18Result) {
    ...
    obj2[3] = memo;
    tmp18Result = tmp18(tmp2(4444).Text, obj2);
  }
  ...
  if (isQuestProgressing) {
    ...
    obj3[1] = tmp18(tmp2(4444).Text, obj4);
    isQuestProgressing = tmp18(tmp17, obj3);
  }
  items3[1] = isQuestProgressing;
  obj[2] = items3;
  return c8(questTaskDetails(5569).Card, obj);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj4[0] = memo.url;
    obj3[3] = obj4;
    const items4 = [tmp13(tmp2(8570).VideoComponent, obj3), ];
  ...
  const items7 = [closure_7(_modDef14729, { blurTheme: "light" }), ];
  const watchTaskPressHandler = obj1.useWatchTaskPressHandler(obj);
  items7[1] = closure_7(quest(8546).PlayIcon, { color: ThemesDefault.colors.WHITE });
  obj10[1] = items7;
  ...
  obj1[2] = items5;
  obj[3] = closure_8(quest(5569).Card, obj1);
  return closure_7(quest(5036).PressableOpacity, obj);
};
