...
// Function ID: 14610
// Name: StyleSheet
// Dependencies: [19, 17, 1922, 21, 4380, 712, 10531, 10564, 589, 10547, 7326, 1236, 4376, 5275, 5482, 14581, 10542, 14573, 11326, 14568, 4949, 9017, 4826, 5374, 14610, 9002, 2]
...
import openQuestAccessSuspendedBottomSheetDefault from "openQuestAccessSuspendedBottomSheet" /* 14568 */;
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = callback();
  let obj = questTaskDetails(10531);
  questTaskDetails = obj.useQuestTaskDetails(quest);
  obj1 = questTaskDetails(10531);
  ...
  defaultRewardName = questTaskDetails(10547).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result2 = questTaskDetails(10547);
  const isSponsoredPlayQuestResult = questTaskDetails(7326).isSponsoredPlayQuest(quest);
  ...
  const memo = React.useMemo(() => {
    ...
    if (isQuestProgressing) {
      ...
      if (c8) {
      ...
      } else if (questTaskDetails.taskType === questTaskDetails(5275).FirstPartyQuestTaskTypes.STREAM_ON_DESKTOP) {
        const intl4 = questTaskDetails(1236).intl;
        obj1 = { gameName: null };
        obj1[0] = gameTitle;
        stringResult = intl4.formatToPlainString(questTaskDetails(1236).t.boMftC, obj1);
      } else if (tmp41.taskType === questTaskDetails(5275).FirstPartyQuestTaskTypes.PLAY_ON_DESKTOP) {
        ...
        const CONSOLE = questTaskDetails(5275).FirstPartyQuestTaskTypesSets.CONSOLE;
        stringResult = null;
  ...
  const tmp2Result3 = questTaskDetails(7326);
  if (!tmp7) {
    tmp7 = isQuestProgressing;
  }
  obj1[3] = !tmp7;
  const items2 = [defaultRewardName(isQuestProgressing(14581), obj1), ];
  ...
  if (tmp18Result) {
    ...
    obj2[3] = memo;
    tmp18Result = tmp18(tmp2(4376).Text, obj2);
  }
  ...
  if (isQuestProgressing) {
    ...
    obj3[1] = tmp18(tmp2(4376).Text, obj4);
    isQuestProgressing = tmp18(tmp17, obj3);
  }
  items3[1] = isQuestProgressing;
  obj[2] = items3;
  return c8(questTaskDetails(5482).Card, obj);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj4[0] = memo.url;
    obj3[3] = obj4;
    const items4 = [tmp13(tmp2(9017).VideoComponent, obj3), ];
  ...
  const items7 = [closure_7(_modDef14610, { blurTheme: "light" }), ];
  const watchTaskPressHandler = obj1.useWatchTaskPressHandler(obj);
  items7[1] = closure_7(quest(9002).PlayIcon, { color: ThemesDefault.colors.WHITE });
  obj10[1] = items7;
  ...
  obj1[2] = items5;
  obj[3] = closure_8(quest(5482).Card, obj1);
  return closure_7(quest(4949).PressableOpacity, obj);
};
