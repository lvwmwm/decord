...
// Function ID: 15085
// Name: StyleSheet
// Dependencies: [19, 17, 1921, 21, 4481, 709, 11415, 11428, 586, 11222, 7655, 1233, 4477, 5412, 5562, 15055, 11217, 15047, 11699, 15042, 5084, 8237, 4948, 5542, 15085, 8204, 2]
...
import openQuestAccessSuspendedBottomSheetDefault from "openQuestAccessSuspendedBottomSheet" /* 15042 */;
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = callback();
  let obj = questTaskDetails(11415);
  questTaskDetails = obj.useQuestTaskDetails(quest);
  obj1 = questTaskDetails(11415);
  ...
  defaultRewardName = questTaskDetails(11222).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result2 = questTaskDetails(11222);
  const isSponsoredPlayQuestResult = questTaskDetails(7655).isSponsoredPlayQuest(quest);
  ...
  const memo = React.useMemo(() => {
    ...
    if (isQuestProgressing) {
      ...
      if (c8) {
      ...
      } else if (questTaskDetails.taskType === questTaskDetails(5412).FirstPartyQuestTaskTypes.STREAM_ON_DESKTOP) {
        const intl4 = questTaskDetails(1233).intl;
        obj1 = { gameName: null };
        obj1[0] = gameTitle;
        stringResult = intl4.formatToPlainString(questTaskDetails(1233).t.boMftC, obj1);
      } else if (tmp41.taskType === questTaskDetails(5412).FirstPartyQuestTaskTypes.PLAY_ON_DESKTOP) {
        ...
        const CONSOLE = questTaskDetails(5412).FirstPartyQuestTaskTypesSets.CONSOLE;
        stringResult = null;
  ...
  const tmp2Result3 = questTaskDetails(7655);
  if (!tmp7) {
    tmp7 = isQuestProgressing;
  }
  obj1[3] = !tmp7;
  const items2 = [defaultRewardName(isQuestProgressing(15055), obj1), ];
  ...
  if (tmp18Result) {
    ...
    obj2[3] = memo;
    tmp18Result = tmp18(tmp2(4477).Text, obj2);
  }
  ...
  if (isQuestProgressing) {
    ...
    obj3[1] = tmp18(tmp2(4477).Text, obj4);
    isQuestProgressing = tmp18(tmp17, obj3);
  }
  items3[1] = isQuestProgressing;
  obj[2] = items3;
  return c8(questTaskDetails(5562).Card, obj);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj4[0] = memo.url;
    obj3[3] = obj4;
    const items4 = [tmp13(tmp2(8237).VideoComponent, obj3), ];
  ...
  const items7 = [closure_7(_modDef15085, { blurTheme: "light" }), ];
  const watchTaskPressHandler = obj1.useWatchTaskPressHandler(obj);
  items7[1] = closure_7(quest(8204).PlayIcon, { color: ThemesDefault.colors.WHITE });
  obj10[1] = items7;
  ...
  obj1[2] = items5;
  obj[3] = closure_8(quest(5562).Card, obj1);
  return closure_7(quest(5084).PressableOpacity, obj);
};
