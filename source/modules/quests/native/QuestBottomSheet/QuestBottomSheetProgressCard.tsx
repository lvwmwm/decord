...
// Function ID: 14695
// Name: StyleSheet
// Dependencies: [19, 17, 1923, 21, 4446, 712, 11021, 11026, 589, 10865, 7414, 1236, 4442, 5359, 5566, 14665, 10860, 14657, 11268, 14652, 5033, 8548, 4905, 5458, 14695, 8524, 2]
...
import openQuestAccessSuspendedBottomSheetDefault from "openQuestAccessSuspendedBottomSheet" /* 14652 */;
...
export const QuestBottomSheetProgressCardPlayStreamTask = function QuestBottomSheetProgressCardPlayStreamTask(quest) {
  ...
  c8 = undefined;
  const tmp = callback();
  let obj = questTaskDetails(11021);
  questTaskDetails = obj.useQuestTaskDetails(quest);
  obj1 = questTaskDetails(11021);
  ...
  defaultRewardName = questTaskDetails(10865).getDefaultRewardName(quest.config, stateFromStores);
  const tmp2Result2 = questTaskDetails(10865);
  const isSponsoredPlayQuestResult = questTaskDetails(7414).isSponsoredPlayQuest(quest);
  ...
  const memo = React.useMemo(() => {
    ...
    if (isQuestProgressing) {
      ...
      if (c8) {
      ...
      } else if (questTaskDetails.taskType === questTaskDetails(5359).FirstPartyQuestTaskTypes.STREAM_ON_DESKTOP) {
        const intl4 = questTaskDetails(1236).intl;
        obj1 = { gameName: null };
        obj1[0] = gameTitle;
        stringResult = intl4.formatToPlainString(questTaskDetails(1236).t.boMftC, obj1);
      } else if (tmp41.taskType === questTaskDetails(5359).FirstPartyQuestTaskTypes.PLAY_ON_DESKTOP) {
        ...
        const CONSOLE = questTaskDetails(5359).FirstPartyQuestTaskTypesSets.CONSOLE;
        stringResult = null;
  ...
  const tmp2Result3 = questTaskDetails(7414);
  if (!tmp7) {
    tmp7 = isQuestProgressing;
  }
  obj1[3] = !tmp7;
  const items2 = [defaultRewardName(isQuestProgressing(14665), obj1), ];
  ...
  if (tmp18Result) {
    ...
    obj2[3] = memo;
    tmp18Result = tmp18(tmp2(4442).Text, obj2);
  }
  ...
  if (isQuestProgressing) {
    ...
    obj3[1] = tmp18(tmp2(4442).Text, obj4);
    isQuestProgressing = tmp18(tmp17, obj3);
  }
  items3[1] = isQuestProgressing;
  obj[2] = items3;
  return c8(questTaskDetails(5566).Card, obj);
};
export const QuestBottomSheetProgressCardWatchTask = function QuestBottomSheetProgressCardWatchTask(quest) {
  ...
  if (isHeroVideoSupportedResult) {
    ...
    obj4[0] = memo.url;
    obj3[3] = obj4;
    const items4 = [tmp13(tmp2(8548).VideoComponent, obj3), ];
  ...
  const items7 = [closure_7(_modDef14695, { blurTheme: "light" }), ];
  const watchTaskPressHandler = obj1.useWatchTaskPressHandler(obj);
  items7[1] = closure_7(quest(8524).PlayIcon, { color: ThemesDefault.colors.WHITE });
  obj10[1] = items7;
  ...
  obj1[2] = items5;
  obj[3] = closure_8(quest(5566).Card, obj1);
  return closure_7(quest(5033).PressableOpacity, obj);
};
