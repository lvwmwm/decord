...
// Dependencies: [32, 19, 17, 7054, 5113, 21, 4255, 712, 7060, 10348, 3956, 1236, 8228, 10346, 10377, 5116, 14210, 589, 11062, 7077, 5323, 14237, 14238, 11058, 10375, 7081, 9405, 9406, 9410, 7082, 14275, 14277, 14278, 7075, 7744, 4251, 2]
// Exports: default

// Module 14236 (QuestBottomSheet)
...
function QuestBottomSheet(initialStep) {
  ...
  obj[0] = callback2(importDefault(14237), obj1);
  let tmp9Result = null;
  if (!isInGameQuestResult) {
    if (!hasWatchVideoTasksResult) {
      ...
      tmp9Result = tmp9(importDefault(14238), obj2);
    } else {
      tmp9Result = null;
    }
  }
  ...
  obj[1] = callback2(_require(5323).BottomSheet, obj);
...
function useEnrolledQuestContentProps(quest) {
  ...
  let obj1 = quest(10375);
  callback = obj1.useQuestImpressionId();
  let obj2 = quest(10346);
  const questTaskDetails = obj2.useQuestTaskDetails(quest);
  const isQuestProgressing = quest(10346).useIsQuestProgressing(quest);
  const obj4 = quest(10346);
  let tmp6 = callback(quest(10346).useTaskPlatformScreen(quest, questTaskDetails), 3);
  importDefaultResult = tmp6[2];
  const obj5 = quest(10346);
  ...
  let completedAt;
  const hasWatchVideoOnMobileTasks = quest(14210).useHasWatchVideoOnMobileTasks(quest.config);
  if (userStatus != null) {
  ...
  const obj6 = quest(14210);
  isQuestProgressing1 = quest(10346).useIsQuestProgressing(quest);
  const tmpResult1 = quest(10346);
  let tmp5Result = tmp5(quest(10346).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  if (tmp16) {
    tmp16 = first === tmp(5116).TaskPlatformScreen.CONSOLE;
  }
  if (!tmp16) {
  ...
  tmp21 = 0 !== memo.length || null != completedAt || first !== quest(5116).TaskPlatformScreen.CONSOLE || first1;
  hasWatchVideoOnMobileTasks1 = quest(14210).useHasWatchVideoOnMobileTasks(quest.config);
  const tmpResult3 = quest(14210);
  isMobileActivityQuest = quest(14210).useMobileActivityQuest(quest).isMobileActivityQuest;
  ...
  questLogger = quest(7060).getQuestLogger(obj);
  ...
  const tmpResult5 = quest(7060);
  obj2 = {
    quest,
    defibrillator: obj1,
    step: tmp32,
    stepActions: tmp33,
    showMicrophone: null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5116).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks,
...
function MicrophoneUnit(arg0) {
  ...
  if (num > 0) {
    ...
    obj1[0] = str;
    const items1 = [closure_10(tmp2(7744).WarningIcon, obj1), ];
    ...
    items1[1] = closure_10(tmp2(4251).Text, obj3);
    obj[1] = items1;
  ...
  const tmp2Result = require(7075) /* isSponsoredPlayQuest */;
