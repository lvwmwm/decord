...
// Dependencies: [32, 19, 17, 7416, 5355, 21, 4448, 712, 7422, 10875, 4164, 1236, 7648, 11046, 11059, 5358, 14659, 589, 11307, 7437, 5590, 14689, 14690, 11303, 11049, 7441, 8961, 8962, 8966, 7414, 14728, 14730, 14731, 7435, 8699, 4444, 2]
// Exports: default

// Module 14688 (QuestBottomSheet)
...
function QuestBottomSheet(initialStep) {
  ...
  let obj2 = _require(7437);
  const tmp = callback3();
  const tmp2 = constants;
  let obj3 = _require(7437);
  ...
  obj[1] = callback2(_require(5590).BottomSheet, obj);
...
function useEnrolledQuestContentProps(quest) {
  ...
  const isQuestProgressing = quest(11046).useIsQuestProgressing(quest);
  const obj4 = quest(11046);
  let tmp6 = callback(quest(11046).useTaskPlatformScreen(quest, questTaskDetails), 3);
  importDefaultResult = tmp6[2];
  const obj5 = quest(11046);
  ...
  let completedAt;
  const hasWatchVideoOnMobileTasks = quest(14659).useHasWatchVideoOnMobileTasks(quest.config);
  if (userStatus != null) {
  ...
  const memo = importDefaultResult.useMemo(() => {
    obj = quest(11059);
    return quest(11059).supportedConsoles(quest).filter((arg0) => {
      closure_0 = arg0;
  ...
  const obj6 = quest(14659);
  isQuestProgressing1 = quest(11046).useIsQuestProgressing(quest);
  const tmpResult1 = quest(11046);
  let tmp5Result = tmp5(quest(11046).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  if (tmp16) {
    tmp16 = first === tmp(5358).TaskPlatformScreen.CONSOLE;
  }
  if (!tmp16) {
  ...
  memo3 = obj8.useMemo(() => {
    obj = { type: memo3.TASK_SELECT, shouldShow: dependencyMap === quest(5358).TaskPlatformScreen.SELECT };
    const items = [obj, , ];
    obj = { type: memo3.CONSOLE_CONNECT, shouldShow: dependencyMap === quest(5358).TaskPlatformScreen.CONSOLE && first1, onBack: callback, onNext: hideConsoleSelect };
    let tmp6 = callback;
    items[1] = obj;
    obj = { type: memo3.TASK_STATUS, shouldShow: true, onBack: null };
    let tmp7;
    if (!isQuestProgressing1) {
      if (dependencyMap === quest(5358).TaskPlatformScreen.CONSOLE) {
  ...
  tmp21 = 0 !== memo.length || null != completedAt || first !== quest(5358).TaskPlatformScreen.CONSOLE || first1;
  hasWatchVideoOnMobileTasks1 = quest(14659).useHasWatchVideoOnMobileTasks(quest.config);
  const tmpResult3 = quest(14659);
  isMobileActivityQuest = quest(14659).useMobileActivityQuest(quest).isMobileActivityQuest;
  ...
  questLogger = quest(7422).getQuestLogger(obj);
  ...
  obj1 = {
    ...
    start: obj8.useCallback(() => {
      lib(true);
      const result = quest(10875).manuallyStartConsoleQuest(quest.id);
      obj = quest(10875);
  ...
  const tmpResult5 = quest(7422);
  obj2 = {
    quest,
    defibrillator: obj1,
    step: tmp32,
    stepActions: tmp33,
    showMicrophone: null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5358).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks,
    handleTaskSelect(arg0) {
      if (arg0 === showConsoleSelect.CONSOLE) {
        let DESELECT_PLATFORM = quest(7441).QuestContentCTA.SELECT_CONSOLE_PLATFORM;
        let tmp4 = quest;
      } else if (arg0 === tmp.DESKTOP) {
        DESELECT_PLATFORM = quest(7441).QuestContentCTA.SELECT_DESKTOP_PLATFORM;
        tmp4 = quest;
      } else {
        DESELECT_PLATFORM = quest(7441).QuestContentCTA.DESELECT_PLATFORM;
        tmp4 = quest;
      }
      let tmp4Result = tmp4(8961);
      if (tmp4Result.shouldMigrateToAdAnalyticsInterface(tmp4(8961).AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet")) {
        ...
        obj[0] = tmp4(8966).AdUserActionType.CLICK_INTERNAL;
        obj[1] = tmp4(7414).AdCreativeType.QUEST;
        obj[2] = quest.id;
        obj[3] = DESELECT_PLATFORM;
        obj[4] = tmp4(5358).QuestContent.QUEST_BOTTOM_SHEET;
        obj[5] = questLogger;
        obj[6] = closure_3;
        ...
        obj[0] = quest.id;
        obj[1] = tmp4(5358).QuestContent.QUEST_BOTTOM_SHEET;
...
function MicrophoneUnit(arg0) {
  ...
  if (num > 0) {
    ...
    obj1 = { color: null };
    obj1[0] = str;
    const items1 = [closure_10(tmp2(8699).WarningIcon, obj1), ];
    ...
    items1[1] = closure_10(tmp2(4444).Text, obj3);
    obj[1] = items1;
