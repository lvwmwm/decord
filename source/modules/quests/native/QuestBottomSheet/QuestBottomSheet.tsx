...
// Dependencies: [32, 19, 17, 7636, 5404, 21, 4481, 709, 7642, 11208, 4194, 1233, 5552, 11415, 11428, 5407, 15015, 586, 11710, 7657, 7090, 15045, 15046, 11706, 11418, 7661, 11124, 11125, 11129, 5411, 15084, 15086, 15087, 7655, 8518, 4477, 2]
// Exports: default

// Module 15044 (QuestBottomSheet)
import ThemesDefault from "Themes" /* 709 */;
import isSponsoredPlayQuest from "isSponsoredPlayQuest" /* 7655 */;
import useQuests from "useQuests" /* 11415 */;
import QuestBottomSheetHeaderDefault from "QuestBottomSheetHeader" /* 15045 */;
import useQuestRewardClaimHandlerDefault from "useQuestRewardClaimHandler" /* 15046 */;
...
function QuestBottomSheet(initialStep) {
  ...
  let obj2 = _require(7657);
  const tmp = callback3();
  const tmp2 = constants;
  let obj3 = _require(7657);
  ...
  obj[1] = callback2(_require(7090).BottomSheet, obj);
...
function useEnrolledQuestContentProps(quest) {
  ...
  const isQuestProgressing = quest(11415).useIsQuestProgressing(quest);
  const obj4 = quest(11415);
  let tmp6 = callback(quest(11415).useTaskPlatformScreen(quest, questTaskDetails), 3);
  importDefaultResult = tmp6[2];
  const obj5 = quest(11415);
  ...
  let completedAt;
  const hasWatchVideoOnMobileTasks = quest(15015).useHasWatchVideoOnMobileTasks(quest.config);
  if (userStatus != null) {
  ...
  const memo = importDefaultResult.useMemo(() => {
    obj = quest(11428);
    return quest(11428).supportedConsoles(quest).filter((arg0) => {
      closure_0 = arg0;
  ...
  const obj6 = quest(15015);
  isQuestProgressing1 = quest(11415).useIsQuestProgressing(quest);
  const tmpResult1 = quest(11415);
  let tmp5Result = tmp5(quest(11415).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  if (tmp16) {
    tmp16 = first === tmp(5407).TaskPlatformScreen.CONSOLE;
  }
  if (!tmp16) {
  ...
  memo3 = obj8.useMemo(() => {
    obj = { type: memo3.TASK_SELECT, shouldShow: dependencyMap === quest(5407).TaskPlatformScreen.SELECT };
    const items = [obj, , ];
    obj = { type: memo3.CONSOLE_CONNECT, shouldShow: dependencyMap === quest(5407).TaskPlatformScreen.CONSOLE && first1, onBack: callback, onNext: hideConsoleSelect };
    let tmp6 = callback;
    items[1] = obj;
    obj = { type: memo3.TASK_STATUS, shouldShow: true, onBack: null };
    let tmp7;
    if (!isQuestProgressing1) {
      if (dependencyMap === quest(5407).TaskPlatformScreen.CONSOLE) {
  ...
  tmp21 = 0 !== memo.length || null != completedAt || first !== quest(5407).TaskPlatformScreen.CONSOLE || first1;
  hasWatchVideoOnMobileTasks1 = quest(15015).useHasWatchVideoOnMobileTasks(quest.config);
  const tmpResult3 = quest(15015);
  isMobileActivityQuest = quest(15015).useMobileActivityQuest(quest).isMobileActivityQuest;
  ...
  questLogger = quest(7642).getQuestLogger(obj);
  ...
  obj1 = {
    ...
    start: obj8.useCallback(() => {
      lib(true);
      const result = quest(11208).manuallyStartConsoleQuest(quest.id);
      obj = quest(11208);
  ...
  const tmpResult5 = quest(7642);
  obj2 = {
    quest,
    defibrillator: obj1,
    step: tmp32,
    stepActions: tmp33,
    showMicrophone: null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5407).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks,
    handleTaskSelect(arg0) {
      if (arg0 === showConsoleSelect.CONSOLE) {
        let DESELECT_PLATFORM = quest(7661).QuestContentCTA.SELECT_CONSOLE_PLATFORM;
        let tmp4 = quest;
      } else if (arg0 === tmp.DESKTOP) {
        DESELECT_PLATFORM = quest(7661).QuestContentCTA.SELECT_DESKTOP_PLATFORM;
        tmp4 = quest;
      } else {
        DESELECT_PLATFORM = quest(7661).QuestContentCTA.DESELECT_PLATFORM;
        tmp4 = quest;
      }
      let tmp4Result = tmp4(11124);
      if (tmp4Result.shouldMigrateToAdAnalyticsInterface(tmp4(11124).AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet")) {
        tmp4Result = tmp4(11125);
        ...
        obj[0] = tmp4(11129).AdUserActionType.CLICK_INTERNAL;
        obj[1] = tmp4(5411).AdCreativeType.QUEST;
        obj[2] = quest.id;
        obj[3] = DESELECT_PLATFORM;
        obj[4] = tmp4(5407).QuestContent.QUEST_BOTTOM_SHEET;
        obj[5] = questLogger;
        obj[6] = closure_3;
        ...
        obj[0] = quest.id;
        obj[1] = tmp4(5407).QuestContent.QUEST_BOTTOM_SHEET;
...
function MicrophoneUnit(arg0) {
  ...
  if (num > 0) {
    ...
    obj1 = { color: null };
    obj1[0] = str;
    const items1 = [closure_10(tmp2(8518).WarningIcon, obj1), ];
    const intl2 = tmp2(1233).intl;
    if (tmp4) {
      const obj2 = { gameTitle: null };
      obj2[0] = quest.config.messages.gameTitle;
      let formatToPlainStringResult = intl2.formatToPlainString(tmp2(1233).t["28Ql27"], obj2);
    } else {
      formatToPlainStringResult = intl2.string(tmp2(1233).t.YstzGO);
    }
    const obj3 = { variant: "text-md/medium", color: "mobile-text-heading-primary", children: null };
    obj3[2] = formatToPlainStringResult;
    items1[1] = closure_10(tmp2(4477).Text, obj3);
    obj[1] = items1;
  ...
  if (isSponsoredPlayQuestResult) {
    let stringResult = intl.string(tmp2(1233).t.bUyEZZ);
  } else {
    const obj4 = { gameTitle: null };
    obj4[0] = quest.config.messages.gameTitle;
    stringResult = intl.formatToPlainString(tmp2(1233).t.GXqvC1, obj4);
  }
  items3 = [stringResult];
}
