...
// Dependencies: [32, 19, 17, 7395, 5352, 21, 4446, 712, 7401, 10851, 4163, 1236, 7627, 11021, 11026, 5355, 14625, 589, 11278, 7416, 5587, 14655, 14656, 11274, 11024, 7420, 8938, 8939, 8943, 7393, 14694, 14696, 14697, 7414, 8677, 4442, 2]
// Exports: default

// Module 14654 (QuestBottomSheet)
...
function QuestBottomSheet(initialStep) {
  ...
  let obj2 = _require(7416);
  const tmp = callback3();
  const tmp2 = constants;
  let obj3 = _require(7416);
  ...
  obj[1] = callback2(_require(5587).BottomSheet, obj);
...
function useEnrolledQuestContentProps(quest) {
  ...
  const isQuestProgressing = quest(11021).useIsQuestProgressing(quest);
  const obj4 = quest(11021);
  let tmp6 = callback(quest(11021).useTaskPlatformScreen(quest, questTaskDetails), 3);
  importDefaultResult = tmp6[2];
  const obj5 = quest(11021);
  ...
  let completedAt;
  const hasWatchVideoOnMobileTasks = quest(14625).useHasWatchVideoOnMobileTasks(quest.config);
  if (userStatus != null) {
  ...
  const memo = importDefaultResult.useMemo(() => {
    obj = quest(11026);
    return quest(11026).supportedConsoles(quest).filter((arg0) => {
      closure_0 = arg0;
  ...
  const obj6 = quest(14625);
  isQuestProgressing1 = quest(11021).useIsQuestProgressing(quest);
  const tmpResult1 = quest(11021);
  let tmp5Result = tmp5(quest(11021).useTaskPlatformScreen(quest, questTaskDetails1), 3);
  ...
  if (tmp16) {
    tmp16 = first === tmp(5355).TaskPlatformScreen.CONSOLE;
  }
  if (!tmp16) {
  ...
  memo3 = obj8.useMemo(() => {
    obj = { type: memo3.TASK_SELECT, shouldShow: dependencyMap === quest(5355).TaskPlatformScreen.SELECT };
    const items = [obj, , ];
    obj = { type: memo3.CONSOLE_CONNECT, shouldShow: dependencyMap === quest(5355).TaskPlatformScreen.CONSOLE && first1, onBack: callback, onNext: hideConsoleSelect };
    let tmp6 = callback;
    items[1] = obj;
    obj = { type: memo3.TASK_STATUS, shouldShow: true, onBack: null };
    let tmp7;
    if (!isQuestProgressing1) {
      if (dependencyMap === quest(5355).TaskPlatformScreen.CONSOLE) {
  ...
  tmp21 = 0 !== memo.length || null != completedAt || first !== quest(5355).TaskPlatformScreen.CONSOLE || first1;
  hasWatchVideoOnMobileTasks1 = quest(14625).useHasWatchVideoOnMobileTasks(quest.config);
  const tmpResult3 = quest(14625);
  isMobileActivityQuest = quest(14625).useMobileActivityQuest(quest).isMobileActivityQuest;
  ...
  questLogger = quest(7401).getQuestLogger(obj);
  ...
  obj1 = {
    ...
    start: obj8.useCallback(() => {
      lib(true);
      const result = quest(10851).manuallyStartConsoleQuest(quest.id);
      obj = quest(10851);
  ...
  const tmpResult5 = quest(7401);
  obj2 = {
    quest,
    defibrillator: obj1,
    step: tmp32,
    stepActions: tmp33,
    showMicrophone: null == completedAt1 && !isQuestProgressing && tmp6[0] === quest(5355).TaskPlatformScreen.CONSOLE && !hasWatchVideoOnMobileTasks,
    handleTaskSelect(arg0) {
      if (arg0 === showConsoleSelect.CONSOLE) {
        let DESELECT_PLATFORM = quest(7420).QuestContentCTA.SELECT_CONSOLE_PLATFORM;
        let tmp4 = quest;
      } else if (arg0 === tmp.DESKTOP) {
        DESELECT_PLATFORM = quest(7420).QuestContentCTA.SELECT_DESKTOP_PLATFORM;
        tmp4 = quest;
      } else {
        DESELECT_PLATFORM = quest(7420).QuestContentCTA.DESELECT_PLATFORM;
        tmp4 = quest;
      }
      let tmp4Result = tmp4(8938);
      if (tmp4Result.shouldMigrateToAdAnalyticsInterface(tmp4(8938).AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_bottom_sheet")) {
        tmp4Result = tmp4(8939);
        ...
        obj[0] = tmp4(8943).AdUserActionType.CLICK_INTERNAL;
        obj[1] = tmp4(7393).AdCreativeType.QUEST;
        obj[2] = quest.id;
        obj[3] = DESELECT_PLATFORM;
        obj[4] = tmp4(5355).QuestContent.QUEST_BOTTOM_SHEET;
        obj[5] = questLogger;
        obj[6] = closure_3;
        ...
        obj[0] = quest.id;
        obj[1] = tmp4(5355).QuestContent.QUEST_BOTTOM_SHEET;
...
function MicrophoneUnit(arg0) {
  ...
  if (num > 0) {
    ...
    obj1 = { color: null };
    obj1[0] = str;
    const items1 = [closure_10(tmp2(8677).WarningIcon, obj1), ];
    ...
    items1[1] = closure_10(tmp2(4442).Text, obj3);
    obj[1] = items1;
