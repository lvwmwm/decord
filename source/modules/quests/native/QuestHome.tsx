...
// Dependencies: [32, 19, 17, 4436, 11399, 7371, 7393, 5334, 676, 21, 4444, 712, 589, 1500, 14530, 709, 4662, 4670, 14587, 4879, 1236, 4675, 6191, 10599, 11140, 4440, 5015, 14589, 4399, 14591, 10600, 1629, 7369, 10601, 7387, 4162, 8670, 698, 9565, 503, 14611, 12499, 14612, 1501, 5337, 7392, 14594, 14604, 11408, 14613, 14691, 7957, 2]

// Module 14529 (EmptyStateNoQuestsAvailable)
import ThemesDefault from "Themes" /* 712 */;
import getSystemLocale from "getSystemLocale" /* 1236 */;
import Text from "Text" /* 4440 */;
import Button from "Button" /* 4879 */;
import useQuests from "useQuests" /* 10599 */;
import useBountiesExperience from "useBountiesExperience" /* 11140 */;
import QuestHomeEmptyStateDefault from "QuestHomeEmptyState" /* 14587 */;
import openBountiesNuxPromoSheetDefault from "openBountiesNuxPromoSheet" /* 14589 */;
import EmptyStateDefault from "EmptyState" /* 14591 */;
import closure_3 from "_slicedToArray" /* 32 */;
import importAllResult from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_7 from "maybeApplyNoTextColorForLightCustomTheme" /* 4436 */;
import closure_8 from "zustandStore" /* 11399 */;
import closure_9 from "initializeState" /* 7371 */;
import closure_10 from "keys" /* 7393 */;
...
function EmptyStateNoQuestsAvailable() {
  ...
  obj[2] = callback;
  obj[0] = callback2(navigation(4879).Button, obj);
...
function HeaderPreviewButton() {
  ...
  if (obj.useShouldShowPreviewToolTab()) {
    ...
    obj[1] = callback2(tmp3(4879).Button, obj);
...
function HeaderWithBounties(arg0) {
  ...
  if (!verticalScrollEnabled) {
    ...
    const items2 = [tmp6(tmp2(4440).Text, obj), ];
    ...
    obj1[4] = tmp6(tmp2(4399).CircleInformationIcon, obj2);
    items2[1] = tmp6(tmp2(5015).PressableOpacity, obj1);
    obj[1] = items2;
...
const memoResult = importAllResult.memo(function QuestHome(filters) {
  ...
  const filteredQuests = obj1.useFilteredQuests(_require(10599).QuestTabs.ALL, importAllResult.useMemo(() => ({ filters: navigation, sortMethod: closure_2 }), items));
  ...
  obj = { utm_source_current: tmp24.utmSourceCurrent, utm_medium_current: tmp24.utmMediumCurrent, utm_campaign_current: tmp24.utmCampaignCurrent, utm_content_current: tmp24.utmContentCurrent, tab: _require(10599).QuestTabs.ALL };
  obj[2] = obj;
  navigation(9565)(obj);
  ...
  const tmp25 = navigation(9565);
  let tmp5 = navigation;
  showBounties = _require(12499).useVirtualCurrencyMobileEnabled().enabled;
  const obj10 = _require(12499);
  const bountiesExperience = _require(11140).useBountiesExperience(callback4.QUEST_HOME_MOBILE);
  verticalScrollEnabled = bountiesExperience.verticalScrollEnabled;
  const OrbsHoldoutExperiment = _require(14612).OrbsHoldoutExperiment;
  obj2 = { location: callback4.QUEST_HOME_MOBILE };
  const obj11 = _require(11140);
  ...
  const BountiesShopCarouselExperiment = tmp(14594).BountiesShopCarouselExperiment;
  ...
  const questHomeOrbShopCarouselData = _require(14604).useQuestHomeOrbShopCarouselData({ enabled, variation });
  ...
  _require = tmp37;
  const tmpResult1 = _require(14604);
  ...
  if (!isEligibleForQuests) {
    ...
    obj6[10] = callback1;
    obj6[11] = callback3;
    tmp63Result = verticalScrollEnabled(tmp(7957).FlashList, obj6);
