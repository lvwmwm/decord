...
function Footer(eligibleToReceivePremiumRewards) {
  ...
  ({ loading, onPress } = importDefault(8650)(false, callback, constants.QUEST_ORB_MULTIPLIER_PERK_INFO));
  const callback1 = React.useCallback(() => {
    callback2(4253).hideActionSheet();
    const obj = callback2(4253);
    const obj2 = callback2(3982);
...
function SheetContent(arg0) {
  ...
  const items = [callback(require(5594) /* ActionSheetHeaderBar */.ActionSheetHeaderBar, { variant: "floating" }), ];
  obj = { style: items1, children: null };
  items1 = [tmp.container, { marginBottom: importDefault(1581)().bottom }];
  obj = { style: tmp.contentContainer, children: null };
  const items2 = [callback(View, { style: tmp.riveContainer, children: callback(require(3997) /* ManaContext */.NitroQuestOrbsMultiplierRive, {}) }), , , ];
  const items3 = [, ];
