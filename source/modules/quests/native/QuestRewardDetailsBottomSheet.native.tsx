...
// Dependencies: [19, 17, 1922, 21, 4445, 712, 11250, 4412, 11003, 10847, 7386, 1236, 589, 5574, 4878, 11253, 4441, 2]
// Exports: default

// Module 11252 (QuestRewardDetailsBottomSheet)
...
function QuestRewardDetailsBottomSheet(quest) {
  ...
  const items3 = [callback(memo(11253), { quest, height: 56, width: 56, withAnimation: true }), ];
  ...
  const items4 = [callback(quest(4441).Text, obj2), callback(quest(4441).Text, { variant: "heading-lg/semibold", color: "text-strong", children: defaultRewardName })];
  obj1[3] = items4;
  items3[1] = callback2(quest(4878).Stack, obj1);
  obj[3] = items3;
  const items5 = [callback2(quest(4878).Stack, obj), ];
  ...
  if (tmp9Result) {
    ...
    obj5[2] = memo1;
    items6[1] = tmp8(quest(4441).Text, obj5);
    obj3[0] = items6;
    tmp9Result = tmp9(closure_8, obj3);
  }
  items5[1] = tmp9Result;
  obj[3] = items5;
  return callback(quest(5574).BottomSheet, { startExpanded: true, children: callback2(quest(4878).Stack, obj) });
}
...
export default function QuestRewardDetailsBottomSheetConnected(questId) {
  const callback = React.useCallback(() => {
    const result = callback(11250).showQuestUnavailableAlert();
    const obj = callback(11250);
