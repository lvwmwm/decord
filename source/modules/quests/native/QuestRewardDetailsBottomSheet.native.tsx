...
// Dependencies: [19, 17, 1923, 21, 4446, 712, 11268, 4413, 11021, 10865, 7400, 1236, 589, 5587, 4891, 11271, 4442, 2]
// Exports: default

// Module 11270 (QuestRewardDetailsBottomSheet)
...
function QuestRewardDetailsBottomSheet(quest) {
  ...
  const items3 = [callback(memo(11271), { quest, height: 56, width: 56, withAnimation: true }), ];
  ...
  const items4 = [callback(quest(4442).Text, obj2), callback(quest(4442).Text, { variant: "heading-lg/semibold", color: "text-strong", children: defaultRewardName })];
  obj1[3] = items4;
  items3[1] = callback2(quest(4891).Stack, obj1);
  obj[3] = items3;
  const items5 = [callback2(quest(4891).Stack, obj), ];
  ...
  if (tmp9Result) {
    ...
    obj5[2] = memo1;
    items6[1] = tmp8(quest(4442).Text, obj5);
    obj3[0] = items6;
    tmp9Result = tmp9(closure_8, obj3);
  }
  items5[1] = tmp9Result;
  obj[3] = items5;
  return callback(quest(5587).BottomSheet, { startExpanded: true, children: callback2(quest(4891).Stack, obj) });
}
...
export default function QuestRewardDetailsBottomSheetConnected(questId) {
  const callback = React.useCallback(() => {
    const result = callback(11268).showQuestUnavailableAlert();
    const obj = callback(11268);
