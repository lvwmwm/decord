...
// Dependencies: [19, 17, 1922, 21, 4444, 712, 11398, 4411, 10599, 10615, 7376, 1236, 589, 5569, 4877, 11401, 4440, 2]
// Exports: default

// Module 11400 (QuestRewardDetailsBottomSheet)
...
function QuestRewardDetailsBottomSheet(quest) {
  ...
  const items3 = [callback(memo(11401), { quest, height: 56, width: 56, withAnimation: true }), ];
  ...
  const items4 = [callback(quest(4440).Text, obj2), callback(quest(4440).Text, { variant: "heading-lg/semibold", color: "text-strong", children: defaultRewardName })];
  obj1[3] = items4;
  items3[1] = callback2(quest(4877).Stack, obj1);
  obj[3] = items3;
  const items5 = [callback2(quest(4877).Stack, obj), ];
  ...
  if (tmp9Result) {
    ...
    obj5[2] = memo1;
    items6[1] = tmp8(quest(4440).Text, obj5);
    obj3[0] = items6;
    tmp9Result = tmp9(closure_8, obj3);
  }
  items5[1] = tmp9Result;
  obj[3] = items5;
  return callback(quest(5569).BottomSheet, { startExpanded: true, children: callback2(quest(4877).Stack, obj) });
}
...
export default function QuestRewardDetailsBottomSheetConnected(questId) {
  const callback = React.useCallback(() => {
    const result = callback(11398).showQuestUnavailableAlert();
