...
// Dependencies: [5, 57, 31, 27, 14098, 5011, 14100, 653, 482, 33, 4165, 689, 14182, 14101, 14104, 14097, 4563, 4026, 8094, 1557, 14105, 3869, 14184, 4577, 14099, 4581, 4560, 14185, 7656, 1212, 14186, 14193, 14196, 14197, 4566, 10468, 5014, 14198, 566, 10469, 14096, 8800, 675, 5849, 5854, 5993, 9445, 10946, 3877, 2]

// Module 14183 (QuestDockWithGestureAnimation)
import closure_3 from "QuestsExperimentLocations";
import _slicedToArray from "_slicedToArray";
import importAllResult from "module_14193";
...
function QuestDockModeChangeTracker(arg0) {
  ...
  obj = { mode, questContent: require(5014) /* QuestsVisibleMessagesChangedSource */.QuestContent.QUEST_BAR_MOBILE, questId, sourceQuestContent: require(5014) /* QuestsVisibleMessagesChangedSource */.QuestContent.QUEST_BAR_MOBILE };
...
class QuestDockWithEntranceAnimation {
  constructor() {
    ...
    quest = context.quest;
    f108907 = _slicedToArray(c5.useState(() => performance.now()), 1)[0];
    c3 = c5.useRef(false);
    tmp2 = f108925();
    ...
    items = [];
    items[0] = staticUrl;
    f108908 = obj.useStateFromStores(items, () => staticUrl.prevRestingQuestDockMode);
    ...
    fn = function c() {
      ...
      obj.opacity = isRendered(4577).withSpring(num, outer1_32, "animate-always");
      obj = {};
      const obj2 = isRendered(4577);
      let num2 = 0;
      if (!isRendered) {
        num2 = first.height;
      }
      obj.translateY = isRendered(4577).withSpring(num2, outer1_32);
      const items = [obj];
      obj.transform = items;
      return obj;
    };
    obj = { withSpring: require("withSpring").withSpring, isRendered, ENTRANCE_ANIMATION_SPING_CONFIG: f108925, componentDimensions: first };
    fn.__closure = obj;
    fn.__workletHash = 15545726338295;
    fn.__initData = f108925;
    ...
    effect1 = c5.useEffect(() => {
      if (false === first1) {
        let obj = quest(5849);
        obj = { name: isRendered(5854).MetricEvents.QUEST_CONTENT_RENDERING_FAILURE };
        const _HermesInternal = HermesInternal;
        const items = ["quest_id:" + quest.id, , ];
        const _HermesInternal2 = HermesInternal;
        items[1] = "quest_content:" + isRendered(5993).getQuestContentName(isRendered(5014).QuestContent.QUEST_BAR_MOBILE);
        items[2] = "reason:asset_loading_error";
        obj.tags = items;
        obj.increment(obj);
        const obj3 = isRendered(5993);
      }
    }, items2);
    ...
    if (!tmp17) {
      ...
      obj1.onLayout = function onLayout(height) {
        ...
        if (!ref.current) {
          ...
          if (Math.random() < 0.1) {
            ...
            obj = { name: isRendered(5854).MetricEvents.QUEST_BAR_MOBILE_TIME_TO_FIRST_PAINT };
            const _HermesInternal = HermesInternal;
            const items = ["quest_id:" + quest.id];
            obj.tags = items;
            quest(5849).distribution(obj, rounded);
            const obj2 = quest(5849);
          }
        }
      };
      tmp23 = jsx;
      ...
      obj2.children = function children() {
        ...
        obj.children = outer1_19(isRendered(3877).ThemeContextProvider, obj1);
        items[2] = outer1_19(isRendered(14101).QuestDockGestureContextProvider, obj);
        obj.children = items;
        return outer1_20(outer1_21, obj);
      };
      obj1.children = jsx(require("_createForOfIteratorHelperLoose").BillableAdPlacementImpressionTrackerNative, obj2);
      tmp19 = jsx(require("module_4026").View, obj1);
    }
    return tmp19;
  }
}
...
const memoResult = importAllResult.memo(function QuestDockWithQuestContext() {
  let obj = require(14096) /* useIsMobileQuestDockRenderedBase */;
  const mobileQuestDock = obj.useMobileQuestDock();
  const isMobileQuestDockRenderedBase = require(14096) /* useIsMobileQuestDockRenderedBase */.useIsMobileQuestDockRenderedBase(mobileQuestDock);
  require(14096) /* useIsMobileQuestDockRenderedBase */;
  let tmp5 = null;
  if (null != mobileQuestDock) {
    ...
    tmp5 = callback(importDefault(14182).Provider, obj);
  }
  return tmp5;
});
