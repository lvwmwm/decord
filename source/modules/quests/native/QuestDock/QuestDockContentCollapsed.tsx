// Module ID: 15110
// Function ID: 15111
// Dependencies: [19, 17, 5404, 15019, 21, 4481, 15020, 4218, 4935, 7013, 2]

// Module 15110
import importAllResult from "noop" /* 19 */;
import { QuestDockMode } from "QuestsExperimentLocations" /* 5404 */;
import { QUEST_DOCK_MODE_CHANGE_PHYSICS as closure_5 } from "QUEST_DOCK_COLLAPSED_HEIGHT" /* 15019 */;
...
const memoResult = importAllResult.memo(function QuestDockContentCollapsed(children) {
  ...
  hideOnExpand = tmp;
  const activeQuestDockMode = importAllResult.useContext(hideOnExpand(15020).QuestDockGestureContext).activeQuestDockMode;
  let obj = hideOnExpand(4218);
  const fn = function l() {
  ...
  obj = { withSpring: hideOnExpand(4935).withSpring, activeQuestDockMode, QuestDockMode, hideOnExpand: tmp, QUEST_DOCK_MODE_CHANGE_PHYSICS: closure_5 };
  fn.__closure = obj;
  ...
  const animatedProps = hideOnExpand(4218).useAnimatedProps(C);
  const style = [tmp2.questDockContentCollapsed, animatedStyle];
  return jsx(activeQuestDockMode(7013), { style, animatedProps, children: children.children });
});
