...
// Module 13809 (_createForOfIteratorHelperLoose)
...
export const useBountiesModalVideoAnalytics = function useBountiesModalVideoAnalytics(bountyId) {
  ...
  const items4 = [callback1, rewardDurationMs];
  ...
  callback4 = React.useCallback((arg0, arg1) => {
    ...
    if (!current.has("end")) {
      ...
      current2.add("end");
      const _Math = Math;
      const obj = { 1986250873: null, -77429767: null, 1536053920: null, videoTimestampSeconds: Math.max(arg0, arg1) };
      callback1(obj);
    }
  }, items5);
  const items6 = [callback1, rewardDurationMs];
  ...
  const items7 = [callback, callback2, callback3, callback5, rewardDurationMs];
  const items8 = [callback, callback2, callback3, callback5, callback4];
  const callback6 = React.useCallback((arg0, current, current2) => {
  ...
  const callback7 = React.useCallback((arg0, current, current2) => {
    closure_13.current = current2;
    closure_14.current = current;
    callback(current);
    if (0 === arg0) {
  ...
  const callback8 = React.useCallback((loop_count) => {
  ...
  const callback9 = React.useCallback((video_timestamp_seconds) => {
  ...
  const callback10 = React.useCallback((video_timestamp_seconds) => {
    if (arg1 === bountyId(rewardDurationMs[8]).PlaybackTriggerSource.USER_INTERACTION) {
      let obj = bountyId(rewardDurationMs[6]);
  ...
  const items13 = [bountyId, memo, questImpressionId, memo1, sourceQuestContent];
  const callback11 = React.useCallback((error) => {
    let obj = bountyId(rewardDurationMs[6]);
    obj = { adContentId: bountyId, adCreativeType: bountyId(rewardDurationMs[7]).AdCreativeType.BOUNTY, event: flag.AD_VIDEO_ERROR };
    obj = { "Null": false, "Null": false, "Null": null, video_progress: bountyId(rewardDurationMs[5]).formatVideoProgressRatio(ref2.current, ref3.current), video_session_id: memo };
    let code;
    if (null != error) {
    ...
    if (obj5.isSourceError(error)) {
      const obj1 = { name: bountyId(rewardDurationMs[11]).MetricEvents.QUEST_VIDEO_ERROR, tags: [] };
  ...
  callback12 = React.useCallback(() => {
    if (null != ref8.current) {
      ...
      obj = { video_session_id: memo, impression_id: questImpressionId, duration: diff, network_connection_speed: num3.getEffectiveConnectionSpeed(), buffer_index: ref9.current };
  ...
  const items14 = [bountyId, memo, questImpressionId, memo1, sourceQuestContent];
  callback13 = React.useCallback(() => {
    let current = ref6.current;
    if (!current) {
      current = null == ref4.current;
    }
    if (!current) {
      current = null == ref5.current;
    }
    if (!current) {
      ref6.current = true;
      let obj = bountyId(rewardDurationMs[6]);
      ...
      obj = { video_session_id: memo, impression_id: questImpressionId, duration_ms: ref5.current - ref4.current, network_connection_speed: num3.getEffectiveConnectionSpeed() };
      const merged = Object.assign(memo1);
      obj.properties = obj;
      obj.sourceQuestContent = sourceQuestContent;
      obj.trackAdContentEvent(obj);
  ...
  const items15 = [callback12];
  const effect = React.useEffect(() => () => callback(), items15);
  const items16 = [flag2, callback13, callback12];
  ...
  obj = {
    handleVideoProgressAnalytics: callback6,
    handleVideoEndAnalytics: callback7,
    handleVideoLoopedAnalytics: callback8,
    handleVideoPausedAnalytics: callback9,
    handleVideoResumedAnalytics: callback10,
    handleVideoErrorAnalytics: callback11,
      ...
      if (ref7.current) {
        if (flag2) {
          if (arg0) {
            ...
            obj = { video_session_id: memo, impression_id: questImpressionId, network_connection_speed: 7.getEffectiveConnectionSpeed(), buffer_index: ref9.current };
            const merged = Object.assign(memo1);
            obj.properties = obj;
            obj.sourceQuestContent = sourceQuestContent;
            obj.trackAdContentEvent(obj);
  ...
  const items17 = [callback12];
  const items18 = [callback13, flag2];
  const items19 = [bountyId, memo, questImpressionId, callback12, flag2, memo1, sourceQuestContent];
  return obj;
};
