...
// Dependencies: [17, 1302, 7228, 5223, 676, 7263, 500, 1236, 7251, 11239, 5226, 1363, 10447, 1624, 12697, 7226, 7245, 7249, 8119, 2]
// Exports: createQuestsEmbed

// Module 12696 (createQuestsEmbed)
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  let acceptLabelGreenColor = importDefault(8119)(theme);
  ...
  if (obj.isMetaQuest()) {
    ...
    obj.thumbnailUrl = Image.resolveAssetSource(merged5(12697)).uri;
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7245).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1236).intl;
        const tmp2Result2 = tmp2(7245);
        ...
        obj4.thumbnailUrl = Image.resolveAssetSource(merged5(12697)).uri;
      ...
      const result2 = tmp2(7245).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1236).intl;
      const tmp2Result4 = tmp2(7245);
      ...
      obj6.thumbnailUrl = Image.resolveAssetSource(merged5(12697)).uri;
      ...
      if (tmp2Result6.isShareableQuest(result.config)) {
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7251).getQuestTaskDetails(result);
          const tmp2Result8 = tmp2(7251);
          const thirdPartyTaskDetails = tmp2(7251).getThirdPartyTaskDetails(result);
          const tmp2Result9 = tmp2(7251);
          ...
          obj10[3] = tmp2(5226).QuestContent.QUEST_EMBED_MOBILE;
          obj10[4] = thirdPartyTaskDetails;
          obj10[6] = currentUser;
          formatToPlainStringResult1 = tmp2(11239).getQuestsInstructionsToWinReward(obj10);
        ...
        obj11.thumbnailUrl = tmp2(10447).getQuestAsset(result, tmp2(10447).QuestAssetType.GAME_TILE, tmp29).url;
        obj11.embedCanBeTapped = true;
        obj11.canBeAccepted = true;
        obj11.type = InviteTypes.GUILD;
        const tmp2Result11 = tmp2(10447);
        const obj12 = {};
        ...
        const result3 = tmp2(7245).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1236).intl;
        const tmp2Result13 = tmp2(7245);
        ...
        obj14.thumbnailUrl = Image.resolveAssetSource(merged5(12697)).uri;
