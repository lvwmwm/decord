...
// Dependencies: [17, 1302, 7081, 5143, 676, 7117, 500, 1236, 7104, 11091, 5146, 4130, 10389, 1605, 12528, 7079, 7098, 7102, 7976, 2]
// Exports: createQuestsEmbed

// Module 12527 (createQuestsEmbed)
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  let acceptLabelGreenColor = importDefault(7976)(theme);
  let obj = require(1605) /* isMetaQuest */;
  if (obj.isMetaQuest()) {
    ...
    obj.thumbnailUrl = Image.resolveAssetSource(merged5(12528)).uri;
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7098).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1236).intl;
        const tmp2Result2 = tmp2(7098);
        ...
        obj4.thumbnailUrl = Image.resolveAssetSource(merged5(12528)).uri;
      ...
      const result2 = tmp2(7098).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1236).intl;
      const tmp2Result4 = tmp2(7098);
      ...
      obj6.thumbnailUrl = Image.resolveAssetSource(merged5(12528)).uri;
      ...
      if (tmp2Result6.isShareableQuest(result.config)) {
        ...
        const _Date2 = Date;
        const tmp29 = tmp2(4130).isThemeDark(state.getState().theme) ? ThemeTypes.DARK : ThemeTypes.LIGHT;
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7104).getQuestTaskDetails(result);
          const tmp2Result8 = tmp2(7104);
          const thirdPartyTaskDetails = tmp2(7104).getThirdPartyTaskDetails(result);
          const tmp2Result9 = tmp2(7104);
          ...
          obj10[3] = tmp2(5146).QuestContent.QUEST_EMBED_MOBILE;
          obj10[4] = thirdPartyTaskDetails;
          obj10[6] = currentUser;
          formatToPlainStringResult1 = tmp2(11091).getQuestsInstructionsToWinReward(obj10);
          const tmp2Result10 = tmp2(11091);
        }
        ...
        obj11.thumbnailUrl = tmp2(10389).getQuestAsset(result, tmp2(10389).QuestAssetType.GAME_TILE, tmp29).url;
        obj11.embedCanBeTapped = true;
        obj11.canBeAccepted = true;
        obj11.type = InviteTypes.GUILD;
        const tmp2Result11 = tmp2(10389);
        const obj12 = {};
        ...
        const result3 = tmp2(7098).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1236).intl;
        const tmp2Result13 = tmp2(7098);
        ...
        obj14.thumbnailUrl = Image.resolveAssetSource(merged5(12528)).uri;
