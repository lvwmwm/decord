...
// Dependencies: [17, 1302, 7305, 5268, 676, 7340, 500, 1236, 7328, 11333, 5271, 1363, 10542, 1625, 12817, 7303, 7322, 7326, 8204, 2]
// Exports: createQuestsEmbed

// Module 12816 (createQuestsEmbed)
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  if (obj.isMetaQuest()) {
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7322).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1236).intl;
        const tmp2Result2 = tmp2(7322);
      ...
      const result2 = tmp2(7322).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1236).intl;
      const tmp2Result4 = tmp2(7322);
      ...
      if (tmp2Result6.isShareableQuest(result.config)) {
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7328).getQuestTaskDetails(result);
          const tmp2Result8 = tmp2(7328);
          const thirdPartyTaskDetails = tmp2(7328).getThirdPartyTaskDetails(result);
          const tmp2Result9 = tmp2(7328);
          ...
          obj10[3] = tmp2(5271).QuestContent.QUEST_EMBED_MOBILE;
          obj10[4] = thirdPartyTaskDetails;
          obj10[6] = currentUser;
          formatToPlainStringResult1 = tmp2(11333).getQuestsInstructionsToWinReward(obj10);
          const tmp2Result10 = tmp2(11333);
        }
        ...
        obj11.thumbnailUrl = tmp2(10542).getQuestAsset(result, tmp2(10542).QuestAssetType.GAME_TILE, tmp29).url;
        ...
        const result3 = tmp2(7322).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1236).intl;
        const tmp2Result13 = tmp2(7322);
