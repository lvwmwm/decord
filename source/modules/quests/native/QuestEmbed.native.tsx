...
// Dependencies: [17, 1302, 7448, 5387, 676, 7481, 500, 1236, 7469, 11342, 5390, 1363, 10922, 1624, 12890, 7443, 7463, 7467, 7854, 2]
// Exports: createQuestsEmbed

// Module 12889 (createQuestsEmbed)
...
import registerAssetDefault from "registerAsset" /* 12890 */;
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  if (obj.isMetaQuest()) {
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7463).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1236).intl;
        const tmp2Result2 = tmp2(7463);
      ...
      const result2 = tmp2(7463).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1236).intl;
      const tmp2Result4 = tmp2(7463);
      ...
      if (tmp2Result6.isShareableQuest(result.config)) {
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7469).getQuestTaskDetails(result);
          ...
          obj10[3] = tmp2(5390).QuestContent.QUEST_EMBED_MOBILE;
          obj10[4] = thirdPartyTaskDetails;
          obj10[6] = currentUser;
          formatToPlainStringResult1 = tmp2(11342).getQuestsInstructionsToWinReward(obj10);
          const tmp2Result10 = tmp2(11342);
        }
        ...
        const result3 = tmp2(7463).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1236).intl;
        const tmp2Result13 = tmp2(7463);
