...
// Dependencies: [17, 1302, 7371, 5334, 676, 7406, 500, 1236, 7394, 11405, 5337, 1363, 10610, 1625, 12891, 7369, 7388, 7392, 8271, 2]
// Exports: createQuestsEmbed

// Module 12890 (createQuestsEmbed)
...
import registerAssetDefault from "registerAsset" /* 12891 */;
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  if (obj.isMetaQuest()) {
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7388).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1236).intl;
        const tmp2Result2 = tmp2(7388);
      ...
      const result2 = tmp2(7388).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1236).intl;
      const tmp2Result4 = tmp2(7388);
      ...
      if (tmp2Result6.isShareableQuest(result.config)) {
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7394).getQuestTaskDetails(result);
          const tmp2Result8 = tmp2(7394);
          const thirdPartyTaskDetails = tmp2(7394).getThirdPartyTaskDetails(result);
          const tmp2Result9 = tmp2(7394);
          ...
          obj10[3] = tmp2(5337).QuestContent.QUEST_EMBED_MOBILE;
          obj10[4] = thirdPartyTaskDetails;
          obj10[6] = currentUser;
          formatToPlainStringResult1 = tmp2(11405).getQuestsInstructionsToWinReward(obj10);
          const tmp2Result10 = tmp2(11405);
        }
        ...
        obj11.thumbnailUrl = tmp2(10610).getQuestAsset(result, tmp2(10610).QuestAssetType.GAME_TILE, tmp29).url;
        obj11.embedCanBeTapped = true;
        obj11.canBeAccepted = true;
        obj11.type = InviteTypes.GUILD;
        const tmp2Result11 = tmp2(10610);
        const obj12 = {};
        ...
        const result3 = tmp2(7388).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1236).intl;
        const tmp2Result13 = tmp2(7388);
