...
// Dependencies: [17, 1302, 7200, 5201, 676, 7236, 500, 1236, 7223, 11231, 5204, 1363, 10518, 1624, 12678, 7198, 7217, 7221, 8094, 2]
// Exports: createQuestsEmbed

// Module 12677 (createQuestsEmbed)
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  let acceptLabelGreenColor = importDefault(8094)(theme);
  ...
  if (obj.isMetaQuest()) {
    ...
    obj.thumbnailUrl = Image.resolveAssetSource(merged5(12678)).uri;
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7217).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1236).intl;
        const tmp2Result2 = tmp2(7217);
        ...
        obj4.thumbnailUrl = Image.resolveAssetSource(merged5(12678)).uri;
      ...
      const result2 = tmp2(7217).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1236).intl;
      const tmp2Result4 = tmp2(7217);
      ...
      obj6.thumbnailUrl = Image.resolveAssetSource(merged5(12678)).uri;
      ...
      if (tmp2Result6.isShareableQuest(result.config)) {
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7223).getQuestTaskDetails(result);
          const tmp2Result8 = tmp2(7223);
          const thirdPartyTaskDetails = tmp2(7223).getThirdPartyTaskDetails(result);
          const tmp2Result9 = tmp2(7223);
          ...
          obj10[3] = tmp2(5204).QuestContent.QUEST_EMBED_MOBILE;
          obj10[4] = thirdPartyTaskDetails;
          obj10[6] = currentUser;
          formatToPlainStringResult1 = tmp2(11231).getQuestsInstructionsToWinReward(obj10);
          const tmp2Result10 = tmp2(11231);
        }
        ...
        obj11.thumbnailUrl = tmp2(10518).getQuestAsset(result, tmp2(10518).QuestAssetType.GAME_TILE, tmp29).url;
        obj11.embedCanBeTapped = true;
        obj11.canBeAccepted = true;
        obj11.type = InviteTypes.GUILD;
        const tmp2Result11 = tmp2(10518);
        const obj12 = {};
        ...
        const result3 = tmp2(7217).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1236).intl;
        const tmp2Result13 = tmp2(7217);
        ...
        obj14.thumbnailUrl = Image.resolveAssetSource(merged5(12678)).uri;
