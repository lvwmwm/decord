...
// Dependencies: [17, 1302, 7054, 5113, 676, 7090, 500, 1236, 7077, 11059, 5116, 4101, 10358, 1577, 12491, 7052, 7071, 7075, 7835, 2]
// Exports: createQuestsEmbed

// Module 12490 (createQuestsEmbed)
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  let acceptLabelGreenColor = importDefault(7835)(theme);
  ...
  if (obj.isMetaQuest()) {
    ...
    obj.thumbnailUrl = Image.resolveAssetSource(merged5(12491)).uri;
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7071).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1236).intl;
        const tmp2Result2 = tmp2(7071);
        ...
        obj4.thumbnailUrl = Image.resolveAssetSource(merged5(12491)).uri;
      ...
      const result2 = tmp2(7071).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1236).intl;
      const tmp2Result4 = tmp2(7071);
      ...
      obj6.thumbnailUrl = Image.resolveAssetSource(merged5(12491)).uri;
      ...
      if (tmp2Result6.isShareableQuest(result.config)) {
        ...
        const _Date2 = Date;
        const tmp29 = tmp2(4101).isThemeDark(state.getState().theme) ? ThemeTypes.DARK : ThemeTypes.LIGHT;
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7077).getQuestTaskDetails(result);
          const tmp2Result8 = tmp2(7077);
          const thirdPartyTaskDetails = tmp2(7077).getThirdPartyTaskDetails(result);
          const tmp2Result9 = tmp2(7077);
          ...
          obj10[3] = tmp2(5116).QuestContent.QUEST_EMBED_MOBILE;
          obj10[4] = thirdPartyTaskDetails;
          obj10[6] = currentUser;
          formatToPlainStringResult1 = tmp2(11059).getQuestsInstructionsToWinReward(obj10);
          const tmp2Result10 = tmp2(11059);
        }
        ...
        obj11.thumbnailUrl = tmp2(10358).getQuestAsset(result, tmp2(10358).QuestAssetType.GAME_TILE, tmp29).url;
        obj11.embedCanBeTapped = true;
        obj11.canBeAccepted = true;
        obj11.type = InviteTypes.GUILD;
        const tmp2Result11 = tmp2(10358);
        const obj12 = {};
        ...
        const result3 = tmp2(7071).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1236).intl;
        const tmp2Result13 = tmp2(7071);
        ...
        obj14.thumbnailUrl = Image.resolveAssetSource(merged5(12491)).uri;
