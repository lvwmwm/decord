...
// Dependencies: [17, 1302, 5983, 5033, 676, 6019, 500, 1236, 6006, 10963, 5036, 4035, 9475, 1577, 12362, 5981, 6000, 6004, 7722, 2]
// Exports: createQuestsEmbed

// Module 12361 (createQuestsEmbed)
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  let acceptLabelGreenColor = importDefault(7722)(theme);
  ...
  if (obj.isMetaQuest()) {
    ...
    obj.thumbnailUrl = Image.resolveAssetSource(merged5(12362)).uri;
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(6000).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1236).intl;
        const tmp2Result2 = tmp2(6000);
        ...
        obj4.thumbnailUrl = Image.resolveAssetSource(merged5(12362)).uri;
      ...
      const result2 = tmp2(6000).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1236).intl;
      const tmp2Result4 = tmp2(6000);
      ...
      obj6.thumbnailUrl = Image.resolveAssetSource(merged5(12362)).uri;
      ...
      if (tmp2Result6.isShareableQuest(result.config)) {
        ...
        const _Date2 = Date;
        const tmp29 = tmp2(4035).isThemeDark(state.getState().theme) ? ThemeTypes.DARK : ThemeTypes.LIGHT;
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(6006).getQuestTaskDetails(result);
          const tmp2Result8 = tmp2(6006);
          const thirdPartyTaskDetails = tmp2(6006).getThirdPartyTaskDetails(result);
          const tmp2Result9 = tmp2(6006);
          ...
          obj10[3] = tmp2(5036).QuestContent.QUEST_EMBED_MOBILE;
          obj10[4] = thirdPartyTaskDetails;
          obj10[6] = currentUser;
          formatToPlainStringResult1 = tmp2(10963).getQuestsInstructionsToWinReward(obj10);
          const tmp2Result10 = tmp2(10963);
        }
        ...
        obj11.thumbnailUrl = tmp2(9475).getQuestAsset(result, tmp2(9475).QuestAssetType.GAME_TILE, tmp29).url;
        obj11.embedCanBeTapped = true;
        obj11.canBeAccepted = true;
        obj11.type = InviteTypes.GUILD;
        const tmp2Result11 = tmp2(9475);
        const obj12 = {};
        ...
        const result3 = tmp2(6000).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1236).intl;
        const tmp2Result13 = tmp2(6000);
        ...
        obj14.thumbnailUrl = Image.resolveAssetSource(merged5(12362)).uri;
