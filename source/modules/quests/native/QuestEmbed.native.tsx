...
// Dependencies: [17, 1302, 7381, 5339, 676, 7414, 500, 1236, 7402, 11257, 5342, 1363, 10842, 1625, 12803, 7376, 7396, 7400, 7787, 2]
// Exports: createQuestsEmbed

// Module 12802 (createQuestsEmbed)
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  if (obj.isMetaQuest()) {
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7396).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1236).intl;
        const tmp2Result2 = tmp2(7396);
      ...
      const result2 = tmp2(7396).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1236).intl;
      const tmp2Result4 = tmp2(7396);
      ...
      if (tmp2Result6.isShareableQuest(result.config)) {
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7402).getQuestTaskDetails(result);
          const tmp2Result8 = tmp2(7402);
          const thirdPartyTaskDetails = tmp2(7402).getThirdPartyTaskDetails(result);
          const tmp2Result9 = tmp2(7402);
          ...
          obj10[3] = tmp2(5342).QuestContent.QUEST_EMBED_MOBILE;
          obj10[4] = thirdPartyTaskDetails;
          obj10[6] = currentUser;
          formatToPlainStringResult1 = tmp2(11257).getQuestsInstructionsToWinReward(obj10);
          const tmp2Result10 = tmp2(11257);
        }
        ...
        obj11.thumbnailUrl = tmp2(10842).getQuestAsset(result, tmp2(10842).QuestAssetType.GAME_TILE, tmp29).url;
        obj11.embedCanBeTapped = true;
        obj11.canBeAccepted = true;
        obj11.type = InviteTypes.GUILD;
        const tmp2Result11 = tmp2(10842);
        const obj12 = {};
        ...
        const result3 = tmp2(7396).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1236).intl;
        const tmp2Result13 = tmp2(7396);
