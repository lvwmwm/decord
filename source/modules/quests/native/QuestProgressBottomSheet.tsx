...
// Dependencies: [5, 19, 17, 9439, 7636, 5404, 9434, 21, 4481, 709, 586, 11710, 5407, 8028, 8603, 1233, 5087, 7108, 7657, 9581, 11415, 11707, 15046, 9436, 4448, 11699, 8291, 11227, 7090, 5542, 4948, 688, 11702, 8984, 15075, 4934, 4477, 4936, 15042, 2]
// Exports: default

// Module 16956 (contextMenuButton)
...
function QuestProgressBottomSheet(quest) {
  ...
  const url = claim(9581)({ applicationId: id, size: 600, names: ["embedded_cover"] }).url;
  let tmpResult = tmp(11415);
  const questTaskDetails = tmpResult.useQuestTaskDetails(quest);
  const intl = tmp(1233).intl;
  obj = { questName: quest.config.messages.questName };
  const tmp7 = claim(9581);
  tmpResult = tmp(11707);
  obj = { quest, taskDetails: questTaskDetails, location: QuestsExperimentLocations.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: tmp(5407).QuestContent.RUNNING_ACTIVITY };
  ...
  const isQuestAccessSuspended = quest(11415).useIsQuestAccessSuspended();
  const tmpResult1 = quest(11415);
  obj1 = { quest, questContent: tmp(5407).QuestContent.RUNNING_ACTIVITY, sourceQuestContent: tmp(5407).QuestContent.RUNNING_ACTIVITY };
  const questRewardClaimHandler = quest(15046).useQuestRewardClaimHandler(obj1);
  ...
  const obj9 = { icon: claim(8984), onPress: callback2, variant: "secondary-overlay", size: "sm", accessibilityLabel: null };
  const intl2 = tmp(1233).intl;
  obj9[4] = intl2.string(quest(1233).t.RDE0Sc);
  const items5 = [callback2(quest(8028).IconButton, obj9), ];
  const obj10 = { quest, showShareLink: true, location: tmp11.QUEST_ACTIVITY_BOTTOM_SHEET, sourceQuestContent: null, children: null };
  const tmpResult2 = quest(15046);
  obj10[3] = quest(5407).QuestContent.RUNNING_ACTIVITY;
  obj10[4] = contextMenuButton;
  items5[1] = callback2(claim(15075), obj10);
  obj8[1] = items5;
  ...
  const items7 = [callback2(quest(4477).Text, { variant: str, color: "mobile-text-heading-primary", children: formatToPlainStringResult }), ];
  ...
  items7[1] = callback2(quest(4477).Text, obj13);
  obj12[3] = items7;
  const items8 = [closure_12(quest(4934).Stack, obj12), ];
...
export default function QuestProgressBottomSheetConnected(questId) {
  ...
  if (null != stateFromStores) {
    ...
    obj[2] = tmp(5407).QuestContent.RUNNING_ACTIVITY;
    obj[3] = tmp(5407).QuestContent.RUNNING_ACTIVITY;
    obj[4] = function children() {
      return closure_1_11(closure_1_16, { quest: stateFromStores });
    };
    tmp4 = callback2(tmp(11710).QuestContentImpressionTrackerNative, obj);
  }
  return tmp4;
};
