...
// Dependencies: [19, 676, 11006, 7396, 7376, 7407, 7406, 7355, 514, 500, 7412, 5342, 7379, 698, 2]
...
// Module 11256 (useTrackQuestEventWithImpression)
...
export const useTrackQuestEventWithImpression = function useTrackQuestEventWithImpression() {
  getQuestImpressionId = getQuestImpressionId(11006).useGetQuestImpressionId();
...
export const useTrackQuestContentClickedWithImpression = function useTrackQuestContentClickedWithImpression() {
  const getQuestImpressionId = callback(11006).useGetQuestImpressionId();
...
export const useQuestHomeEntrypointAnalyticsEvents = function useQuestHomeEntrypointAnalyticsEvents(questHomeHero) {
  ...
  let callback3;
  let callback4;
  const QuestContent = questHomeHero(5342).QuestContent;
  const tmp = shouldShowQuestHomeHeroContent ? QuestContent.QUEST_HOME_ENTRYPOINT_THEMED : QuestContent.QUEST_HOME_ENTRYPOINT;
  dependencyMap = tmp;
  const items = [tmp];
  memo = memo.useMemo(() => {
    const contentProperties = questHomeHero(7406).getContentProperties(dependencyMap);
    delete tmp[tmp2];
  ...
  callback = memo.useCallback((arg0) => {
    if (null != questHomeHero) {
      if (shouldShowQuestHomeHeroContent) {
        ...
        obj[0] = tmp.id;
        obj[1] = questHomeHero(7379).AdCreativeType.QUEST_HOME_HERO;
        obj[2] = arg0;
        obj[3] = memo;
        obj[4] = dependencyMap;
        questHomeHero(7396).trackAdContentEvent(obj);
        const obj2 = questHomeHero(7396);
      }
    }
