...
// Function ID: 9054
// Name: _toPropertyKey
// Dependencies: [5, 109, 32, 19, 17, 1990, 1983, 1910, 1922, 7826, 505, 21, 4342, 712, 3994, 5409, 4330, 589, 9046, 1351, 1236, 1297, 4338, 9054, 5439, 7790, 9058, 5271, 9063, 4535, 4527, 9047, 4061, 4310, 5437, 5436, 4755, 2]
// Exports: default

// Module 9053 (_toPropertyKey)
...
class AddMembersBody {
  constructor(arg0) {
    ...
    tmp8Result = require("module_9058");
...
export default function AddMembersActionSheet(channel) {
      ...
      const item = values.forEach((row) => {
        ...
        if (tmp) {
          if (row.rowType === outer2_14.ROLE) {
            ...
            items.push(outer2_0(4527).permissionOverwriteForRole(row.id, items.type));
            const obj = outer2_0(4527);
          } else if (row.rowType === tmp2.MEMBER) {
            closure_1 = closure_1 + 1;
            items.push(outer2_0(4527).permissionOverwriteForUser(row.id, items.type));
            const obj2 = outer2_0(4527);
          }
        }
      });
      let dependencyMap = 1;
      yield outer1_0(9047).savePermissionUpdates(outer1_0.id, items);
      if (1 === tmp7) {
        dependencyMap = 0;
        let c5 = 3;
      } else if (arg0 === 1) {
  ...
  let str = first(4535)(channel, true);
  if (str == null) {
    str = "";
  }
  if (null == stateFromStores) {
    ...
    if (channel.canSkip) {
      ...
      obj1[1] = tmp11(tmp4(5436).BottomSheetTitleHeader, obj);
      ...
      return tmp11(tmp4(5437).BottomSheet, obj1);
    }
