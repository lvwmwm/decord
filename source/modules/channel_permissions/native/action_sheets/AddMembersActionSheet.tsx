...
// Function ID: 9050
// Name: _toPropertyKey
// Dependencies: [5, 109, 32, 19, 17, 1990, 1983, 1910, 1922, 7822, 505, 21, 4344, 712, 3994, 5408, 4332, 589, 9042, 1351, 1236, 1297, 4340, 9050, 5438, 7786, 9054, 5270, 9059, 4534, 4526, 9043, 4061, 4312, 5436, 5435, 4754, 2]
// Exports: default

// Module 9049 (_toPropertyKey)
...
class AddMembersBody {
  constructor(arg0) {
    ...
    tmp8Result = require("module_9054");
...
export default function AddMembersActionSheet(channel) {
      ...
      const item = values.forEach((row) => {
        ...
        if (tmp) {
          if (row.rowType === outer2_14.ROLE) {
            ...
            items.push(outer2_0(4526).permissionOverwriteForRole(row.id, items.type));
            const obj = outer2_0(4526);
          } else if (row.rowType === tmp2.MEMBER) {
            closure_1 = closure_1 + 1;
            items.push(outer2_0(4526).permissionOverwriteForUser(row.id, items.type));
            const obj2 = outer2_0(4526);
          }
        }
      });
      let dependencyMap = 1;
      yield outer1_0(9043).savePermissionUpdates(outer1_0.id, items);
      if (1 === tmp7) {
        dependencyMap = 0;
        let c5 = 3;
      } else if (arg0 === 1) {
        ...
        let obj = outer1_0(4061);
        const result = obj.memberOrRoleAddedToast(c2, c1);
        const obj1 = outer1_1(4312);
  ...
  let str = first(4534)(channel, true);
  if (str == null) {
    str = "";
  }
  if (null == stateFromStores) {
    ...
    if (channel.canSkip) {
      ...
      obj1[1] = tmp11(tmp4(5435).BottomSheetTitleHeader, obj);
      ...
      return tmp11(tmp4(5436).BottomSheet, obj1);
    }
