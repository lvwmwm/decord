...
// Function ID: 8921
// Name: getRoleRowData
// Dependencies: [1376, 1415, 1965, 1971, 1903, 7704, 676, 1968, 1236, 506, 11, 4123, 1351, 3917, 8921, 1935, 4467, 2]
...
function getMemberRowData(id, id2) {
  ...
  if (nick == null) {
    nick = importDefault(4123).getName(id);
    const obj2 = importDefault(4123);
  }
  obj[1] = nick;
  obj[2] = importDefault(4123).getUserTag(id);
  obj[3] = id.id;
...
export const toggleChannelEveryonePermission = function toggleChannelEveryonePermission(guild_id) {
  ...
  require(8921) /* _updatePermission */.updatePermission(guild_id, obj.id, obj.allow, obj.deny);
};
export const grantUserChannelAccess = function grantUserChannelAccess(outer1_0, accessPermissions) {
  ...
  if (null != currentUser) {
    if (null == outer1_0.permissionOverwrites[currentUser.id]) {
      ...
      obj[2] = importAll(506).add(importAll(3917).NONE, accessPermissions);
      obj[3] = importAll(3917).NONE;
      const obj4 = importAll(506);
      const items = [obj];
      return require(8921) /* _updatePermission */.savePermissionUpdates(outer1_0.id, items, true);
    } else {
      ({ allow, deny } = tmp4);
      obj = importAll(506);
      const obj2 = require(8921) /* _updatePermission */;
