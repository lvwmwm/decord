...
// Function ID: 8892
// Name: getRoleRowData
// Dependencies: [1376, 1415, 1936, 1942, 1874, 7676, 676, 1939, 1236, 506, 11, 4094, 1351, 3888, 8892, 1906, 4437, 2]
...
function getMemberRowData(id, id2) {
  ...
  if (nick == null) {
    nick = importDefault(4094).getName(id);
    const obj2 = importDefault(4094);
  }
  obj[1] = nick;
  obj[2] = importDefault(4094).getUserTag(id);
  obj[3] = id.id;
...
export const toggleChannelEveryonePermission = function toggleChannelEveryonePermission(guild_id) {
  ...
  require(8892) /* _updatePermission */.updatePermission(guild_id, obj.id, obj.allow, obj.deny);
};
export const grantUserChannelAccess = function grantUserChannelAccess(outer1_0, accessPermissions) {
  ...
  if (null != currentUser) {
    if (null == outer1_0.permissionOverwrites[currentUser.id]) {
      ...
      obj[2] = importAll(506).add(importAll(3888).NONE, accessPermissions);
      obj[3] = importAll(3888).NONE;
      const obj4 = importAll(506);
      const items = [obj];
      return require(8892) /* _updatePermission */.savePermissionUpdates(outer1_0.id, items, true);
    } else {
      ({ allow, deny } = tmp4);
      obj = importAll(506);
      const obj2 = require(8892) /* _updatePermission */;
