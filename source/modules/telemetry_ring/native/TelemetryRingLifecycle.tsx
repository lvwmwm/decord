...
// Dependencies: [1212, 1903, 6816, 676, 4480, 13385, 7075, 709, 13389, 2]
...
class TelemetryRingLifecycleImpl extends tmp2 {
  constructor() {
    ...
    applyArgumentsResult._handleLogout = function _handleLogout() {
      applyArgumentsResult(13389).clear();
      const obj = applyArgumentsResult(13389);
      applyArgumentsResult(13385).reset();
    };
...
prototype["_updateZoomedInExport"] = function _updateZoomedInExport() {
  ...
  if (shouldRunResult) {
    shouldRunResult = importDefault(13385).shouldRun();
    const obj = importDefault(13385);
  }
  const result = importDefault(7075).setShouldCollectHermesInstrumentedStats(shouldRunResult);
  if (state === AppStates.ACTIVE) {
    let tmp6Result = tmp6(13385);
    tmp6Result.start();
  } else {
    tmp6Result = tmp6(13385);
    tmp6Result.stop();
  }
};
prototype["_initialize"] = function _initialize() {
  ...
  if (!this._initialized) {
    ...
    const obj = self(709);
    self(13385).initialize();
    const result = self._updateZoomedInExport();
    const obj2 = self(13385);
  }
};
