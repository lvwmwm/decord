...
// Dependencies: [1212, 1922, 7091, 676, 4627, 13751, 7352, 709, 13755, 2]
...
class TelemetryRingLifecycleImpl extends tmp2 {
  constructor() {
    ...
    applyArgumentsResult._handleLogout = function _handleLogout() {
      applyArgumentsResult(13755).clear();
      const obj = applyArgumentsResult(13755);
      applyArgumentsResult(13751).reset();
    };
...
prototype["_updateZoomedInExport"] = function _updateZoomedInExport() {
  ...
  if (state === AppStates.ACTIVE) {
    let tmp6Result = tmp6(13751);
    tmp6Result.start();
  } else {
    tmp6Result = tmp6(13751);
    tmp6Result.stop();
  }
};
prototype["_initialize"] = function _initialize() {
  ...
  if (!this._initialized) {
    ...
    const obj = self(709);
    self(13751).initialize();
    const result = self._updateZoomedInExport();
    const obj2 = self(13751);
  }
};
