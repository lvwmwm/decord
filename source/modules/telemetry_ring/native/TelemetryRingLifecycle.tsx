...
// Dependencies: [1212, 1922, 6944, 676, 4523, 13526, 7203, 709, 13530, 2]
...
class TelemetryRingLifecycleImpl extends tmp2 {
  constructor() {
    ...
    applyArgumentsResult._handleLogout = function _handleLogout() {
      applyArgumentsResult(13530).clear();
      const obj = applyArgumentsResult(13530);
      applyArgumentsResult(13526).reset();
    };
...
prototype["_updateZoomedInExport"] = function _updateZoomedInExport() {
  ...
  if (shouldRunResult) {
    shouldRunResult = importDefault(13526).shouldRun();
    const obj = importDefault(13526);
  }
  const result = importDefault(7203).setShouldCollectHermesInstrumentedStats(shouldRunResult);
  if (state === AppStates.ACTIVE) {
    let tmp6Result = tmp6(13526);
    tmp6Result.start();
  } else {
    tmp6Result = tmp6(13526);
    tmp6Result.stop();
  }
};
prototype["_initialize"] = function _initialize() {
  ...
  if (!this._initialized) {
    ...
    const obj = self(709);
    self(13526).initialize();
    const result = self._updateZoomedInExport();
    const obj2 = self(13526);
  }
};
