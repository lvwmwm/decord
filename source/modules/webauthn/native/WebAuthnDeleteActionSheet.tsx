...
// Dependencies: [19, 17, 21, 4380, 712, 4347, 5503, 5502, 1236, 5871, 14189, 4376, 4815, 5937, 4098, 10176, 4340, 9210, 8113, 2]
// Exports: default

// Module 14188 (WebAuthnDeleteActionSheet)
import noopAll from "noop" /* 19 */;
import ThemesDefault from "Themes" /* 712 */;
import registerAssetDefault from "registerAsset" /* 14189 */;
...
export default function WebAuthnDeleteActionSheet(credential) {
  ...
  obj[1] = callback(credential(5871).ActionSheetCloseButton, { onPress: handleClose });
  const items = [callback(credential(5502).BottomSheetTitleHeader, obj), , , ];
  ...
  items1[1] = callback(credential(4376).Text, obj3);
  obj1[1] = items1;
  ...
  obj5[1] = handleClose;
  obj4[0] = callback(credential(4815).Button, obj5);
  ...
  obj7[1] = function onPress() {
    ...
    const nextPromise = result.then(() => {
      ...
      obj[2] = callback2(10176);
      obj[3] = callback(4340).CircleCheckIcon;
      obj.open(obj);
    });
    result.then(() => {
      let obj = callback2(4098);
      ...
      let obj = callback2(4098);
      obj = { key: "WEBAUTHN_CREDENTIAL_DELETE_ERROR_TOAST_KEY", content: content.message, icon: callback2(9210), IconComponent: callback(8113).WarningIcon, iconColor: "icon-feedback-critical" };
      obj.open(obj);
    }).finally(() => {
      callback2(false);
    });
  ...
  obj7[3] = deleting;
  obj7[4] = deleting;
  obj6[0] = callback(credential(4815).Button, obj7);
  items[3] = callback(closure_4, obj6);
  obj[2] = items;
  return callback2(credential(5503).BottomSheet, obj);
};
