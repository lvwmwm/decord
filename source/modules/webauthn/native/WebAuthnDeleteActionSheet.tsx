...
// Dependencies: [19, 17, 21, 4445, 712, 4412, 5574, 5573, 1236, 5942, 14288, 4441, 4880, 6008, 4162, 10364, 4405, 9515, 8661, 2]
// Exports: default

// Module 14287 (WebAuthnDeleteActionSheet)
import noopAll from "noop" /* 19 */;
import ThemesDefault from "Themes" /* 712 */;
import registerAssetDefault from "registerAsset" /* 14288 */;
...
export default function WebAuthnDeleteActionSheet(credential) {
  ...
  obj[1] = callback(credential(5942).ActionSheetCloseButton, { onPress: handleClose });
  const items = [callback(credential(5573).BottomSheetTitleHeader, obj), , , ];
  ...
  items1[1] = callback(credential(4441).Text, obj3);
  obj1[1] = items1;
  ...
  obj5[1] = handleClose;
  obj4[0] = callback(credential(4880).Button, obj5);
  ...
  obj7[1] = function onPress() {
    ...
    const nextPromise = result.then(() => {
      ...
      obj[2] = callback2(10364);
      obj[3] = callback(4405).CircleCheckIcon;
      obj.open(obj);
    });
    result.then(() => {
      let obj = callback2(4162);
      ...
      let obj = callback2(4162);
      obj = { key: "WEBAUTHN_CREDENTIAL_DELETE_ERROR_TOAST_KEY", content: content.message, icon: callback2(9515), IconComponent: callback(8661).WarningIcon, iconColor: "icon-feedback-critical" };
      obj.open(obj);
    }).finally(() => {
      callback2(false);
    });
  ...
  obj7[3] = deleting;
  obj7[4] = deleting;
  obj6[0] = callback(credential(4880).Button, obj7);
  items[3] = callback(closure_4, obj6);
  obj[2] = items;
  return callback2(credential(5574).BottomSheet, obj);
};
