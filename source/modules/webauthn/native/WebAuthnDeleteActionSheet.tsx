...
export default function WebAuthnDeleteActionSheet(credential) {
  ...
  obj5[1] = handleClose;
  obj4[0] = callback(credential(4755).Button, obj5);
  ...
  obj7[1] = function onPress() {
    ...
    result.then(() => {
      ...
      let obj = callback2(4062);
      obj = { key: "WEBAUTHN_CREDENTIAL_DELETE_ERROR_TOAST_KEY", content: content.message, icon: callback2(9112), IconComponent: callback(8007).WarningIcon, iconColor: "icon-feedback-critical" };
      obj.open(obj);
    }).finally(() => {
      callback2(false);
    });
  ...
  obj7[3] = deleting;
  obj7[4] = deleting;
  obj6[0] = callback(credential(4755).Button, obj7);
  items[3] = callback(closure_4, obj6);
  obj[2] = items;
  return callback2(credential(5437).BottomSheet, obj);
};
