...
// Dependencies: [19, 17, 21, 4342, 712, 4310, 5458, 5457, 1236, 5826, 14059, 4338, 4777, 5892, 4062, 10081, 4303, 9126, 8028, 2]
// Exports: default

// Module 14058 (WebAuthnDeleteActionSheet)
import "set";
...
export default function WebAuthnDeleteActionSheet(credential) {
  ...
  obj[1] = callback(credential(5826).ActionSheetCloseButton, { onPress: handleClose });
  const items = [callback(credential(5457).BottomSheetTitleHeader, obj), , , ];
  const obj1 = { style: tmp.content, children: null };
  const items1 = [callback(closure_3, { source: importDefault(14059) }), ];
  ...
  items1[1] = callback(credential(4338).Text, obj3);
  obj1[1] = items1;
  ...
  obj5[1] = handleClose;
  obj4[0] = callback(credential(4777).Button, obj5);
  ...
  obj7[1] = function onPress() {
    ...
    const nextPromise = result.then(() => {
      ...
      obj[2] = callback2(10081);
      obj[3] = callback(4303).CircleCheckIcon;
      obj.open(obj);
    });
    result.then(() => {
      let obj = callback2(4062);
      ...
      let obj = callback2(4062);
      obj = { key: "WEBAUTHN_CREDENTIAL_DELETE_ERROR_TOAST_KEY", content: content.message, icon: callback2(9126), IconComponent: callback(8028).WarningIcon, iconColor: "icon-feedback-critical" };
      obj.open(obj);
    }).finally(() => {
      callback2(false);
    });
  ...
  obj7[3] = deleting;
  obj7[4] = deleting;
  obj6[0] = callback(credential(4777).Button, obj7);
  items[3] = callback(closure_4, obj6);
  obj[2] = items;
  return callback2(credential(5458).BottomSheet, obj);
};
