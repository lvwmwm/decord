...
// Dependencies: [19, 17, 21, 4285, 712, 4253, 5377, 5376, 1236, 5686, 13895, 4281, 4695, 5752, 3985, 9942, 4246, 8986, 7885, 2]
// Exports: default

// Module 13894 (WebAuthnDeleteActionSheet)
import "set";
...
export default function WebAuthnDeleteActionSheet(credential) {
  ...
  obj[1] = callback(credential(5686).ActionSheetCloseButton, { onPress: handleClose });
  const items = [callback(credential(5376).BottomSheetTitleHeader, obj), , , ];
  const obj1 = { style: tmp.content, children: null };
  const items1 = [callback(closure_3, { source: importDefault(13895) }), ];
  ...
  items1[1] = callback(credential(4281).Text, obj3);
  obj1[1] = items1;
  ...
  obj5[1] = handleClose;
  obj4[0] = callback(credential(4695).Button, obj5);
  ...
  obj7[1] = function onPress() {
    ...
    const nextPromise = result.then(() => {
      ...
      obj[2] = callback2(9942);
      obj[3] = callback(4246).CircleCheckIcon;
      obj.open(obj);
    });
    result.then(() => {
      let obj = callback2(3985);
      ...
      let obj = callback2(3985);
      obj = { key: "WEBAUTHN_CREDENTIAL_DELETE_ERROR_TOAST_KEY", content: content.message, icon: callback2(8986), IconComponent: callback(7885).WarningIcon, iconColor: "icon-feedback-critical" };
      obj.open(obj);
    }).finally(() => {
      callback2(false);
    });
  ...
  obj7[3] = deleting;
  obj7[4] = deleting;
  obj6[0] = callback(credential(4695).Button, obj7);
  items[3] = callback(closure_4, obj6);
  obj[2] = items;
  return callback2(credential(5377).BottomSheet, obj);
};
