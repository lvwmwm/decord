...
// Dependencies: [19, 17, 21, 4344, 712, 4312, 5436, 5435, 1236, 5804, 14046, 4340, 4754, 5870, 4062, 10071, 4305, 9108, 8003, 2]
// Exports: default
...
export default function WebAuthnDeleteActionSheet(credential) {
  ...
  obj[1] = callback(credential(5804).ActionSheetCloseButton, { onPress: handleClose });
  const items = [callback(credential(5435).BottomSheetTitleHeader, obj), , , ];
  ...
  items1[1] = callback(credential(4340).Text, obj3);
  obj1[1] = items1;
  ...
  obj5[1] = handleClose;
  obj4[0] = callback(credential(4754).Button, obj5);
  ...
  obj7[1] = function onPress() {
    ...
    const nextPromise = result.then(() => {
      ...
      obj[2] = callback2(10071);
      obj[3] = callback(4305).CircleCheckIcon;
      obj.open(obj);
    });
    result.then(() => {
      let obj = callback2(4062);
      ...
      let obj = callback2(4062);
      obj = { key: "WEBAUTHN_CREDENTIAL_DELETE_ERROR_TOAST_KEY", content: content.message, icon: callback2(9108), IconComponent: callback(8003).WarningIcon, iconColor: "icon-feedback-critical" };
      obj.open(obj);
    }).finally(() => {
      callback2(false);
    });
  ...
  obj7[3] = deleting;
  obj7[4] = deleting;
  obj6[0] = callback(credential(4754).Button, obj7);
  items[3] = callback(closure_4, obj6);
  obj[2] = items;
  return callback2(credential(5436).BottomSheet, obj);
};
