...
function _handlePollGifAttachmentAdd() {
  ...
  const _handlePollGifAttachmentAdd = obj;
  return obj(...arguments);
}
function _removePollUploadAttachment() {
  // CreateGeneratorClosureLongIndex (0x67)
  const obj = callback(tmp);
  const _removePollUploadAttachment = obj;
  return obj(...arguments);
}
function _removeAllPollUploadAttachments() {
  // CreateGeneratorClosureLongIndex (0x67)
  const obj = callback(tmp);
  const _removeAllPollUploadAttachments = obj;
  return obj(...arguments);
}
let closure_3 = importDefault(dependencyMap[0]);
const DraftType = arg1(dependencyMap[1]).DraftType;
const POLL_ATTACHMENT_FOLDER = arg1(dependencyMap[2]).POLL_ATTACHMENT_FOLDER;
const result = arg1(dependencyMap[8]).fileFinishedImporting("modules/polls/PollUploadAttachmentActionCreators.native.tsx");
