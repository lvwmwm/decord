...
// Function ID: 4657
// Name: isBlurDisabled
// Dependencies: [19, 17, 4657, 21, 500, 4658, 3959, 712, 4661, 4099, 2]
// Exports: isBlurDisabled

// Module 4656 (isBlurDisabled)
...
export default require("noop").forwardRef(function VisualEffectView(blurAmount) {
  ...
  if (set) {
    ...
    if (!tmp15) {
      if (android_softwareBlurDisabled) {
        android_softwareBlurDisabled = !tmp3(4661).MODERN_ANDROID_BLURRING_AVAILABLE;
      }
  ...
  if (tmp14) {
    ...
    if (blurEffectNameOverride != null) {
      ...
      if (!tmp3(4658).MODERN_IOS_BLURS_EFFECTS_AVAILABLE) {
...
export const isBlurDisabled = function isBlurDisabled(merged) {
  ...
  if (set) {
    ...
    if (!tmp4) {
      if (android_softwareBlurDisabled) {
        android_softwareBlurDisabled = !require(4661) /* MODERN_ANDROID_BLURRING_AVAILABLE */.MODERN_ANDROID_BLURRING_AVAILABLE;
      }
