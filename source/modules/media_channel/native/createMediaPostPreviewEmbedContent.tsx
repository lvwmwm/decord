...
// Dependencies: [17, 4443, 1391, 1910, 4266, 1922, 10547, 4633, 4639, 1236, 8288, 8290, 712, 4635, 12911, 7272, 4685, 4634, 2]
// Exports: default

// Module 12910 (createMediaPostPreviewEmbedContent)
import set from "set" /* 2 */;
import ThemesDefault from "Themes" /* 712 */;
import getMediaPostEmbedCommonData from "getMediaPostEmbedCommonData" /* 4633 */;
...
import closure_10 from "getMediaPostEmbed" /* 10547 */;
        ...
        if (null == mediaPostEmbedCommonData) {
          ...
          if (null != mediaPostEmbedCommonData.authorName) {
            if (null != mediaPostEmbedCommonData.channelName) {
              if (null != user) {
                const userAuthorWithProcessedColor = tmp(8288).getUserAuthorWithProcessedColor(user, mediaPostEmbedCommonData.postThread);
              ...
              if (isAnimatedImageUrlResult) {
                isAnimatedImageUrlResult = tmp(4635).isAnimatedImageUrl(mediaPostEmbedCommonData.coverImage);
              ...
              if (mediaPostEmbedCommonData.shouldShowBlurredThumbnailImage) {
                ...
                obj2.blurredCoverImage = closure_3.resolveAssetSource(tmp10(12911)).uri;
                ...
                const isPendingScanVersionResult = tmp(7272).isPendingScanVersion(mediaPostEmbedCommonData.contentScanVersion);
                let result = value;
                if (value) {
                  result = tmp(7272).shouldAgeVerifyForExplicitMedia();
                  const tmpResult4 = tmp(7272);
                }
                let isVerifiedTeenResult = value;
                if (value) {
                  isVerifiedTeenResult = tmp(4685).isVerifiedTeen();
                  const tmpResult5 = tmp(4685);
                }
                if (mediaPostEmbedCommonData.shouldContainMediaWithBackground) {
                  if (null != mediaPostEmbedCommonData.coverImage) {
                    ...
                    obj3.backgroundImage = tmp(4634).getBackgroundImageUrl(mediaPostEmbedCommonData.coverImage);
                    let obj4 = obj3;
                    const tmpResult6 = tmp(4634);
                  }
                  return obj4;
                }
                obj4 = {};
