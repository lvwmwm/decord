...
// Dependencies: [17, 4254, 1372, 1862, 4072, 1874, 10296, 4440, 4446, 1236, 7851, 7853, 712, 4442, 12511, 6953, 4470, 4441, 2]
// Exports: default

// Module 12510 (createMediaPostPreviewEmbedContent)
...
export default function createMediaPostPreviewEmbedContent(arg0, arg1, url) {
  ...
  let obj = require(4440) /* getMediaPostEmbedCommonData */;
        ...
        if (null == mediaPostEmbedCommonData) {
          ...
          if (null != mediaPostEmbedCommonData.authorName) {
            if (null != mediaPostEmbedCommonData.channelName) {
              if (null != user) {
                const userAuthorWithProcessedColor = tmp(7851).getUserAuthorWithProcessedColor(user, mediaPostEmbedCommonData.postThread);
              ...
              if (isAnimatedImageUrlResult) {
                isAnimatedImageUrlResult = tmp(4442).isAnimatedImageUrl(mediaPostEmbedCommonData.coverImage);
              ...
              if (mediaPostEmbedCommonData.shouldShowBlurredThumbnailImage) {
                ...
                obj2.blurredCoverImage = closure_3.resolveAssetSource(tmp10(12511)).uri;
                ...
                const isPendingScanVersionResult = tmp(6953).isPendingScanVersion(mediaPostEmbedCommonData.contentScanVersion);
                let result = value;
                if (value) {
                  result = tmp(6953).shouldAgeVerifyForExplicitMedia();
                  const tmpResult4 = tmp(6953);
                }
                let isVerifiedTeenResult = value;
                if (value) {
                  isVerifiedTeenResult = tmp(4470).isVerifiedTeen();
                  const tmpResult5 = tmp(4470);
                }
                if (mediaPostEmbedCommonData.shouldContainMediaWithBackground) {
                  if (null != mediaPostEmbedCommonData.coverImage) {
                    ...
                    obj3.backgroundImage = tmp(4441).getBackgroundImageUrl(mediaPostEmbedCommonData.coverImage);
                    let obj4 = obj3;
                    const tmpResult6 = tmp(4441);
                  }
                  return obj4;
                }
                obj4 = {};
