...
// Dependencies: [17, 4254, 1372, 1862, 4072, 1874, 10620, 4441, 4447, 1236, 7867, 7869, 712, 4443, 12506, 6956, 4471, 4442, 2]
// Exports: default

// Module 12505 (createMediaPostPreviewEmbedContent)
...
export default function createMediaPostPreviewEmbedContent(arg0, arg1, url) {
  ...
  let obj = require(4441) /* getMediaPostEmbedCommonData */;
        ...
        if (null == mediaPostEmbedCommonData) {
          ...
          if (null != mediaPostEmbedCommonData.authorName) {
            if (null != mediaPostEmbedCommonData.channelName) {
              if (null != user) {
                const userAuthorWithProcessedColor = tmp(7867).getUserAuthorWithProcessedColor(user, mediaPostEmbedCommonData.postThread);
              ...
              if (isAnimatedImageUrlResult) {
                isAnimatedImageUrlResult = tmp(4443).isAnimatedImageUrl(mediaPostEmbedCommonData.coverImage);
                const tmpResult2 = tmp(4443);
              }
              if (tmp15) {
              ...
              if (mediaPostEmbedCommonData.shouldShowBlurredThumbnailImage) {
                ...
                obj2.blurredCoverImage = closure_3.resolveAssetSource(tmp10(12506)).uri;
                ...
                const isPendingScanVersionResult = tmp(6956).isPendingScanVersion(mediaPostEmbedCommonData.contentScanVersion);
                let result = value;
                if (value) {
                  result = tmp(6956).shouldAgeVerifyForExplicitMedia();
                  const tmpResult4 = tmp(6956);
                }
                let isVerifiedTeenResult = value;
                if (value) {
                  isVerifiedTeenResult = tmp(4471).isVerifiedTeen();
                ...
                if (mediaPostEmbedCommonData.shouldContainMediaWithBackground) {
                  if (null != mediaPostEmbedCommonData.coverImage) {
                    ...
                    obj3.backgroundImage = tmp(4442).getBackgroundImageUrl(mediaPostEmbedCommonData.coverImage);
                    let obj4 = obj3;
                    const tmpResult6 = tmp(4442);
                  }
                  return obj4;
                }
                obj4 = {};
