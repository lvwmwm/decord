...
// Dependencies: [17, 4379, 1391, 1910, 4202, 1922, 10479, 4569, 4575, 1236, 8221, 8223, 712, 4571, 12837, 7206, 4621, 4570, 2]
// Exports: default

// Module 12836 (createMediaPostPreviewEmbedContent)
import set from "set" /* 2 */;
import ThemesDefault from "Themes" /* 712 */;
import getMediaPostEmbedCommonData from "getMediaPostEmbedCommonData" /* 4569 */;
...
import closure_10 from "getMediaPostEmbed" /* 10479 */;
        ...
        if (null == mediaPostEmbedCommonData) {
          ...
          if (null != mediaPostEmbedCommonData.authorName) {
            if (null != mediaPostEmbedCommonData.channelName) {
              if (null != user) {
                const userAuthorWithProcessedColor = tmp(8221).getUserAuthorWithProcessedColor(user, mediaPostEmbedCommonData.postThread);
              ...
              if (isAnimatedImageUrlResult) {
                isAnimatedImageUrlResult = tmp(4571).isAnimatedImageUrl(mediaPostEmbedCommonData.coverImage);
              ...
              if (mediaPostEmbedCommonData.shouldShowBlurredThumbnailImage) {
                ...
                obj2.blurredCoverImage = closure_3.resolveAssetSource(tmp10(12837)).uri;
                ...
                if (value) {
                  ...
                  const tmpResult4 = tmp(7206);
                }
                let isVerifiedTeenResult = value;
                if (value) {
                  isVerifiedTeenResult = tmp(4621).isVerifiedTeen();
                  const tmpResult5 = tmp(4621);
                }
                if (mediaPostEmbedCommonData.shouldContainMediaWithBackground) {
                  if (null != mediaPostEmbedCommonData.coverImage) {
                    ...
                    obj3.backgroundImage = tmp(4570).getBackgroundImageUrl(mediaPostEmbedCommonData.coverImage);
                    let obj4 = obj3;
