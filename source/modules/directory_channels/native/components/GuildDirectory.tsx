...
// Function ID: 11876
// Name: keyExtractor
// Dependencies: [19, 17, 1862, 4267, 10219, 10212, 10217, 676, 21, 4255, 712, 11876, 11877, 10249, 10250, 4251, 11878, 1236, 4666, 5905, 10203, 11879, 10214, 4783, 10215, 11966, 589, 1581, 6831, 9, 5192, 4083, 10223, 698, 11967, 10207, 11971, 2]
// Exports: default

// Module 11875 (keyExtractor)
...
function keyExtractor(type) {
  ...
  if (type === require(11877) /* RowType */.RowType.ENTRY) {
...
function renderItem(item) {
  ...
  if (require(11877) /* RowType */.RowType.HEADER === type) {
    let obj = { children: null };
    obj[0] = item.header;
    return callback(GuildDirectoryHeaderRowItem, obj);
  } else if (tmp2(11877).RowType.ENTRY === type) {
    obj = { entry: null };
    obj[0] = item.entry;
    return callback(importDefault(10249), obj);
  } else {
    return callback(importDefault(10250), {});
  }
  tmp2 = require;
}
...
function GuildDirectoryHeader(guild) {
  ...
  obj[0] = require(11878) /* registerAsset */;
  ...
  obj3[1] = callback(require(5905) /* MagnifyingGlassIcon */.MagnifyingGlassIcon, { size: "sm", color: "text-strong" });
  ...
  items1[3] = callback(require(10203) /* TTIFirstContentfulPaint */.TTIFirstContentfulPaint, { label: "hub_directory" });
  obj[1] = items1;
  ...
  if (hasItem) {
    ...
    obj4[0] = guild;
    tmp5Result = callback(importDefault(11879), obj4);
  }
...
function GuildDirectoryFooter(hideFooter) {
  ...
  const tmp = callback3();
  let obj = require(10214) /* useCanManageGuildDirectoryEntry */;
  let tmp4 = null;
  if (obj.useCanCreateOrAddGuildInDirectory(channel)) {
    ...
    if (!hideFooter.hideFooter) {
      ...
      obj1[0] = tmp.addIcon;
      obj1[1] = callback(tmp2(11966).PlusMediumIcon, {});
      ...
      tmp4 = callback(tmp2(4783).PressableOpacity, obj);
    }
  }
  return tmp4;
}
  ...
  if (null == onPressSearch) {
    ...
    if (!directoryIsFetching) {
      if (0 === allEntriesCount) {
        ...
        obj2[1] = channel;
        items9[1] = callback(guildId(11967), obj2);
        items9[2] = callback(tmp2(10203).TTIFirstContentfulPaint, { label: "guild_directory_empty" });
        obj[1] = items9;
