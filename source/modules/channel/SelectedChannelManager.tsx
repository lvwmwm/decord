...
// Dependencies: [4349, 1960, 4118, 676, 5260, 5897, 5128, 1222, 709, 2]

// Module 16483 (handleGuildCreate)
...
prototype["handleGuildCreate"] = function handleGuildCreate(guild) {
  ...
  if (guild.id === channelId) {
    require(5897) /* transitionToGuild */.transitionToGuild(guild.id);
    const obj = require(5897) /* transitionToGuild */;
  }
  ...
  if (tmp6) {
    const voiceChannel = importDefault(5128).selectVoiceChannel(findFirstVoiceChannelId(guild.id));
...
prototype["handleChannelCreate"] = function handleChannelCreate(channel) {
  ...
  if (channel.type === constants.GROUP_DM) {
    ...
    if (tmp7) {
      const voiceChannel = importDefault(5128).selectVoiceChannel(channel.id, videoEnabled.isVideoEnabled());
      const obj2 = importDefault(5128);
    }
