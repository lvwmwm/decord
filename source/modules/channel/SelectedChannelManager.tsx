...
// Dependencies: [4332, 1960, 4101, 676, 5244, 5878, 5111, 1222, 709, 2]

// Module 16456 (handleGuildCreate)
...
prototype["handleGuildCreate"] = function handleGuildCreate(guild) {
  ...
  if (guild.id === channelId) {
    require(5878) /* transitionToGuild */.transitionToGuild(guild.id);
    const obj = require(5878) /* transitionToGuild */;
  }
  ...
  if (tmp6) {
    const voiceChannel = importDefault(5111).selectVoiceChannel(findFirstVoiceChannelId(guild.id));
...
prototype["handleChannelCreate"] = function handleChannelCreate(channel) {
  ...
  if (channel.type === constants.GROUP_DM) {
    ...
    if (tmp7) {
      const voiceChannel = importDefault(5111).selectVoiceChannel(channel.id, videoEnabled.isVideoEnabled());
      const obj2 = importDefault(5111);
    }
