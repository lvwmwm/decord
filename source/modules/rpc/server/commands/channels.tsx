...
// Function ID: 13842
// Name: items
// Dependencies: [1395, 1391, 1910, 4021, 1979, 4542, 4277, 676, 4375, 8752, 8757, 12, 13838, 8755, 4975, 4979, 1222, 8042, 2]
...
obj[1] = function handler(args) {
  ...
  if (null == channel) {
    ...
    if (channel.isPrivate()) {
      ...
      if (!scopes.includes(require(4375) /* set */.OAuth2Scopes.RPC)) {
        if (!scopes.includes(tmp(4375).OAuth2Scopes.DM_CHANNELS_READ)) {
          obj = { errorCode: null };
          obj[0] = constants2.INVALID_PERMISSIONS;
          const tmp8 = new importDefault(8752)(obj, "Invalid scope");
          throw tmp8;
        }
      }
      tmp = require;
    }
    const obj3 = require(8757) /* recurseReplaceContentTree */;
    return obj3.transformChannel(channel, require(8757) /* recurseReplaceContentTree */.hasMessageReadPermission(channel, socket.application.id, socket.authorization.scopes));
  }
};
  ...
  handler(args) {
    ...
    if (guild_id) {
      ...
      if (null == guild) {
        ...
        const _HermesInternal = HermesInternal;
        let tmpResult = importDefault(8752);
...
obj2[1] = function handler() {
  const tmp3 = importDefault(13838)();
  if (null == tmp3) {
    let obj = { errorCode: null };
    obj[0] = constants2.INVALID_CHANNEL;
...
obj5[1] = function handler(socket) {
  ...
  if (null != channel) {
    ...
    transformChannelResult = obj.transformChannel(channel, require(8757) /* recurseReplaceContentTree */.hasMessageReadPermission(channel, socket.application.id, socket.authorization.scopes));
...
obj[RPCCommands.CREATE_CHANNEL_INVITE] = {
  ...
  handler(args) {
    ...
    const invite = importDefault(8042).createInvite(channel_id, merged, "RPC");
    return invite.catch(() => {
