...
// Function ID: 13618
// Name: items
// Dependencies: [1376, 1372, 1862, 3883, 1931, 4271, 4158, 676, 8796, 10515, 10520, 12, 13614, 10518, 5081, 4437, 1222, 7651, 2]
...
obj[1] = function handler(args) {
  ...
  if (null == channel) {
    ...
    if (channel.isPrivate()) {
      ...
      if (!scopes.includes(require(8796) /* set */.OAuth2Scopes.RPC)) {
        if (!scopes.includes(tmp(8796).OAuth2Scopes.DM_CHANNELS_READ)) {
          obj = { errorCode: null };
          obj[0] = constants2.INVALID_PERMISSIONS;
          const tmp8 = new importDefault(10515)(obj, "Invalid scope");
          throw tmp8;
        }
      }
      tmp = require;
    }
    const obj3 = require(10520) /* recurseReplaceContentTree */;
    return obj3.transformChannel(channel, require(10520) /* recurseReplaceContentTree */.hasMessageReadPermission(channel, socket.application.id, socket.authorization.scopes));
  }
};
  ...
  handler(args) {
    ...
    if (guild_id) {
      ...
      if (null == guild) {
        ...
        const _HermesInternal = HermesInternal;
        let tmpResult = importDefault(10515);
...
obj2[1] = function handler() {
  const tmp3 = importDefault(13614)();
  if (null == tmp3) {
    let obj = { errorCode: null };
    obj[0] = constants2.INVALID_CHANNEL;
...
obj5[1] = function handler(socket) {
  ...
  if (null != channel) {
    ...
    transformChannelResult = obj.transformChannel(channel, require(10520) /* recurseReplaceContentTree */.hasMessageReadPermission(channel, socket.application.id, socket.authorization.scopes));
...
obj[RPCCommands.CREATE_CHANNEL_INVITE] = {
  ...
  handler(args) {
    ...
    const invite = importDefault(7651).createInvite(channel_id, merged, "RPC");
    return invite.catch(() => {
