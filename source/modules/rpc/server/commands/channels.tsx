...
// Function ID: 13806
// Name: items
// Dependencies: [1395, 1391, 1910, 3989, 1979, 4360, 4247, 676, 8947, 10795, 10800, 12, 13802, 10798, 5169, 4526, 1222, 7799, 2]
...
obj[1] = function handler(args) {
  ...
  if (null == channel) {
    ...
    if (channel.isPrivate()) {
      ...
      if (!scopes.includes(require(8947) /* set */.OAuth2Scopes.RPC)) {
        if (!scopes.includes(tmp(8947).OAuth2Scopes.DM_CHANNELS_READ)) {
          obj = { errorCode: null };
          obj[0] = constants2.INVALID_PERMISSIONS;
          const tmp8 = new importDefault(10795)(obj, "Invalid scope");
          throw tmp8;
        }
      }
      tmp = require;
    }
    const obj3 = require(10800) /* recurseReplaceContentTree */;
    return obj3.transformChannel(channel, require(10800) /* recurseReplaceContentTree */.hasMessageReadPermission(channel, socket.application.id, socket.authorization.scopes));
  }
};
  ...
  handler(args) {
    ...
    if (guild_id) {
      ...
      if (null == guild) {
        ...
        const _HermesInternal = HermesInternal;
        let tmpResult = importDefault(10795);
...
obj2[1] = function handler() {
  const tmp3 = importDefault(13802)();
  if (null == tmp3) {
    let obj = { errorCode: null };
    obj[0] = constants2.INVALID_CHANNEL;
...
obj5[1] = function handler(socket) {
  ...
  if (null != channel) {
    ...
    transformChannelResult = obj.transformChannel(channel, require(10800) /* recurseReplaceContentTree */.hasMessageReadPermission(channel, socket.application.id, socket.authorization.scopes));
...
obj[RPCCommands.CREATE_CHANNEL_INVITE] = {
  ...
  handler(args) {
    ...
    const invite = importDefault(7799).createInvite(channel_id, merged, "RPC");
    return invite.catch(() => {
