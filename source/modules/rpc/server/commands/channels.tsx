...
// Function ID: 13526
// Name: items
// Dependencies: [1376, 1372, 1862, 3817, 1931, 4205, 4092, 676, 8171, 10622, 10627, 12, 13522, 10625, 5001, 4372, 1222, 6690, 2]
...
obj[1] = function handler(args) {
  ...
  if (null == channel) {
    ...
    if (channel.isPrivate()) {
      ...
      if (!scopes.includes(require(8171) /* set */.OAuth2Scopes.RPC)) {
        if (!scopes.includes(tmp(8171).OAuth2Scopes.DM_CHANNELS_READ)) {
          obj = { errorCode: null };
          obj[0] = constants2.INVALID_PERMISSIONS;
          const tmp8 = new importDefault(10622)(obj, "Invalid scope");
          throw tmp8;
        }
      }
      tmp = require;
    }
    const obj3 = require(10627) /* recurseReplaceContentTree */;
    return obj3.transformChannel(channel, require(10627) /* recurseReplaceContentTree */.hasMessageReadPermission(channel, socket.application.id, socket.authorization.scopes));
  }
};
  ...
  handler(args) {
    ...
    if (guild_id) {
      ...
      if (null == guild) {
        ...
        const _HermesInternal = HermesInternal;
        let tmpResult = importDefault(10622);
...
obj2[1] = function handler() {
  const tmp3 = importDefault(13522)();
  if (null == tmp3) {
    let obj = { errorCode: null };
    obj[0] = constants2.INVALID_CHANNEL;
    const tmp9 = new tmp(10622)(obj, "Invalid channel");
    throw tmp9;
  } else {
...
obj5[1] = function handler(socket) {
  ...
  if (null != channel) {
    const obj = require(10627) /* recurseReplaceContentTree */;
    transformChannelResult = obj.transformChannel(channel, require(10627) /* recurseReplaceContentTree */.hasMessageReadPermission(channel, socket.application.id, socket.authorization.scopes));
...
obj[RPCCommands.SELECT_TEXT_CHANNEL] = {
  ...
  handler(args) {
    ...
    if (channel_id) {
      const storeWaitResult = server.storeWait(socket, () => outer1_5.getChannel(channel_id), num);
      const catchPromise = server.storeWait(socket, () => outer1_5.getChannel(channel_id), num).catch(() => {
        throw new channel_id(10622)({ errorCode: constants2.SELECT_CHANNEL_TIMED_OUT }, "Request to select text channel timed out.");
      });
      let nextPromise1 = server.storeWait(socket, () => outer1_5.getChannel(channel_id), num).catch(() => {
        ...
        if (tmp2.guild_id) {
          if (!getUncachedChannelPermissions.can(constants.VIEW_CHANNEL, tmp)) {
            ...
            obj[0] = constants2.INVALID_CHANNEL;
            const tmp11 = new channel_id(10622)(obj, "No permission to see channel");
            throw tmp11;
          }
        }
        if (tmp2.guild_id) {
          ...
          const privateChannel = channel_id(5001).selectPrivateChannel(tmp.id);
          const obj2 = channel_id(5001);
        }
        return tmp2;
      });
...
obj[RPCCommands.CREATE_CHANNEL_INVITE] = {
  ...
  handler(args) {
    ...
    const invite = importDefault(6690).createInvite(channel_id, merged, "RPC");
    return invite.catch(() => {
