...
// Dependencies: [5, 4188, 676, 13676, 13677, 10536, 7508, 7506, 13678, 6783, 2]

// Module 13675 (getSubscriptionSkusViaListings)
        ...
        try {
          ...
          if (0 === constants) {
            ...
            if (arg0 === 1) {
              ...
              const result = socket(13677).validateTransportType(socket.transport);
              callback = socket.application.id;
              if (null == callback) {
                const obj2 = { errorCode: null };
                obj2[0] = constants.INVALID_COMMAND;
                const tmp32 = new callback(10536)(obj2, "No application.");
                throw tmp32;
              } else {
...
function getEntitlementsHandler(socket) {
  socket = socket.socket;
  let obj = require(13677) /* validateTransportType */;
  const result = obj.validateTransportType(socket.transport);
  const id = socket.application.id;
  if (null == id) {
    obj = { errorCode: null };
    obj[0] = constants.INVALID_COMMAND;
    const tmp9 = new importDefault(10536)(obj, "No application.");
    throw tmp9;
  } else {
    return importAll(6783).fetchUserEntitlementsForApplication(id);
  }
}
