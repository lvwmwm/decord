...
// Dependencies: [4226, 4209, 1415, 4208, 1372, 1942, 1862, 4236, 4261, 1874, 4205, 4092, 676, 10625, 10627, 10622, 4403, 8171, 12, 13518, 506, 643, 10628, 13571, 13572, 2]

// Module 13570 (messageEventsValidation)
  ...
  if (null != channel) {
    if (obj4.hasMessageReadPermission(channel, socket.application.id, socket.authorization.scopes)) {
      if (tmp13Result.userCannotSeeNSFWContent(channel)) {
        ...
        let tmp3 = importDefault(10622);
        tmp3 = new tmp3(obj, "Invalid nsfw channel id: " + channel.id);
        throw tmp3;
      }
      tmp13Result = tmp13(4403);
    }
    obj4 = require(10627) /* recurseReplaceContentTree */;
    tmp13 = require;
  }
  obj = { errorCode: constants2.INVALID_CHANNEL };
  let tmp11 = importDefault(10622);
...
obj[RPCEvents.RELATIONSHIP_UPDATE] = {
  ...
  handler(socket) {
    ...
    if (obj.has(deserializeResult, tmpResult.deserialize(constants.DISABLE_RELATIONSHIPS_ACCESS))) {
      ...
      const tmp9 = new importDefault(10622)(obj, "Missing Permissions");
      throw tmp9;
    }
    tmpResult = tmp(506);
  }
...
obj38[1] = function handler() {
  return (prevState) => {
    ...
    if (!tmp) {
      prevState.dispatch(callback(10628)(obj.currentUser));
    }
    return obj;
  };
};
...
obj44[1] = function handler() {
  return (arg0) => {
    ...
    let tmp8 = null;
    if (null != application) {
      tmp8 = callback(13572)(application);
    }
    if (streamerActiveStreamMetadata != null) {
