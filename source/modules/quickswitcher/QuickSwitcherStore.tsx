...
// Dependencies: [4813, 5206, 1302, 1218, 1372, 4621, 1961, 1971, 1891, 3912, 4297, 1960, 4101, 4480, 676, 7229, 12, 7240, 5141, 1236, 7241, 1351, 595, 589, 709, 2]

// Module 7228 (handleConnectionOpen)
import set from "set";
...
function generateResultFromId(arg0) {
  const tmp = importDefault(7240)(arg0);
  let tmp2 = null;
  if (null != tmp) {
    if (null == c26) {
      tmp2 = tmp;
...
function handleQuickSwitcherShow(arg0) {
  ...
  let tmp13 = store;
  if (store == null) {
    tmp13 = importDefault(7229);
    const items1 = [require(7229) /* sortByMatchScore */.AutocompleterResultTypes.USER, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.GROUP_DM, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.TEXT_CHANNEL, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.GUILD, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.APPLICATION, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.GAME_PROFILE, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.LINK, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.IN_APP_NAVIGATION];
    let num = 5;
    if (null != queryMode) {
      num = 100;
    }
...
function handleUserSearchUpdate(arg0, str) {
  ...
  if (!flag) {
    let DOWN = require;
    if (require(7229) /* sortByMatchScore */.AutocompleterResultTypes.USER === c26) {
      ...
      arr = arr.unshift(DOWN(7229).createHeaderResult(formatToPlainStringResult));
      let items = arr;
      const DOWNResult = DOWN(7229);
    } else {
      if (DOWN(7229).AutocompleterResultTypes.TEXT_CHANNEL === tmp7) {
        const intl5 = DOWN(1236).intl;
        arr = arr.unshift(DOWN(7229).createHeaderResult(intl5.string(DOWN(1236).t.W26k4V)));
        items = arr;
        items = arr;
        const DOWNResult1 = DOWN(7229);
      } else if (DOWN(7229).AutocompleterResultTypes.VOICE_CHANNEL === tmp7) {
        const intl4 = DOWN(1236).intl;
        arr.unshift(DOWN(7229).createHeaderResult(intl4.string(DOWN(1236).t.zUoI5C)));
        items = arr;
        items = arr;
        const DOWNResult2 = DOWN(7229);
      } else if (DOWN(7229).AutocompleterResultTypes.GUILD === tmp7) {
        const intl3 = DOWN(1236).intl;
        arr.unshift(DOWN(7229).createHeaderResult(intl3.string(DOWN(1236).t.olADPs)));
        items = arr;
        items = arr;
        const DOWNResult3 = DOWN(7229);
      } else if (DOWN(7229).AutocompleterResultTypes.APPLICATION === tmp7) {
        const intl2 = DOWN(1236).intl;
        arr.unshift(DOWN(7229).createHeaderResult(intl2.string(DOWN(1236).t.VwK1ld)));
        items = arr;
        items = arr;
        const DOWNResult4 = DOWN(7229);
      } else if (DOWN(7229).AutocompleterResultTypes.GAME_PROFILE === tmp7) {
        const intl = DOWN(1236).intl;
        arr.unshift(DOWN(7229).createHeaderResult(intl.string(DOWN(1236).t.gEp2SG)));
        items = arr;
        items = arr;
        const DOWNResult5 = DOWN(7229);
      } else {
        const found = arr.filter((type) => type.type === callback(7229).AutocompleterResultTypes.GAME_PROFILE);
        const substr = found.slice(0, 3);
        items = [];
        HermesBuiltin.arraySpread(substr, HermesBuiltin.arraySpread(arr.filter((type) => type.type !== callback(7229).AutocompleterResultTypes.GAME_PROFILE), 0));
      }
      if (str !== str) {
        const _Math = Math;
        closure_29 = Math.max(str.length, closure_29);
        str = DOWN(7229).findNextSelectedResult;
        DOWN = DOWN(7229).FindResultDirections.DOWN;
        let closure_23 = str(DOWN, -1, items);
        const DOWNResult6 = DOWN(7229);
      } else {
        let tmp29 = null != tmp27;
        if (tmp29) {
          tmp29 = tmp27.type === DOWN(7229).AutocompleterResultTypes.HEADER;
        }
        if (tmp29) {
          closure_23 = DOWN(7229).findNextSelectedResult(DOWN(7229).FindResultDirections.DOWN, closure_23, items);
          const DOWNResult7 = DOWN(7229);
        }
      }
    ...
    if (null == store) {
      ...
      if (queryMode !== queryMode) {
        if (null != queryMode) {
          ...
          items1 = [require(7229) /* sortByMatchScore */.AutocompleterResultTypes.USER, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.GROUP_DM, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.TEXT_CHANNEL, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.GUILD, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.APPLICATION, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.GAME_PROFILE, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.LINK, require(7229) /* sortByMatchScore */.AutocompleterResultTypes.IN_APP_NAVIGATION];
        }
        ...
        store.setLimit(num);
        const guildId = store4.getGuildId();
        if (queryMode === require(7229) /* sortByMatchScore */.AutocompleterResultTypes.USER) {
          if (null != guildId) {
        ...
        if (queryMode === tmp7(7229).AutocompleterResultTypes.VOICE_CHANNEL) {
