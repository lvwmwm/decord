...
// Dependencies: [5, 32, 5338, 1395, 1391, 4484, 1981, 4267, 9843, 676, 1398, 678, 9844, 698, 8578, 709, 6138, 4456, 4674, 5307, 4458, 10644, 8731, 8737, 6043, 10645, 7219, 5925, 10646, 8580, 2]
// Exports: hide, search, selectResult, switchToResultInNewTab, toggle, trackOpen

// Module 10643 (getQuickSwitcherOptions)
import expandEventPropertiesDefault from "expandEventProperties" /* 698 */;
import dispatcherDefault from "dispatcher" /* 709 */;
import setDefault from "set" /* 8578 */;
import sortByMatchScore from "sortByMatchScore" /* 9844 */;
import closure_3 from "asyncGeneratorStep" /* 5 */;
import closure_4 from "_slicedToArray" /* 32 */;
import { InAppNavigationType } from "fromType" /* 5338 */;
import { ChannelRecordBase } from "createChannelRecord" /* 1395 */;
import closure_7 from "ensureGuildLoaded" /* 1391 */;
import closure_8 from "setLibraryApplications" /* 4484 */;
import closure_9 from "handleConnectionOpen" /* 1981 */;
import closure_10 from "handleConnectionOpen" /* 4267 */;
import closure_11 from "handleConnectionOpen" /* 9843 */;
...
function getQuickSwitcherOptions(str) {
  ...
  if (charAtResult === sortByMatchScore.AutocompleterQuerySymbols.USER) {
    if (charAtResult1 === tmp2(9844).AutocompleterQuerySymbols.USER) {
      const items = [str.slice(2), tmp2(9844).AutocompleterResultTypes.USER_GLOBAL];
      let items1 = items;
    }
...
function trackClose(QUICKSWITCHER_RESULT_SELECTED, type) {
  ...
  if (null == tmp6) {
    ...
    obj[13] = obj.getResultTotals(tmp4(9844).AutocompleterResultTypes.USER);
    obj[14] = obj.getResultTotals(tmp4(9844).AutocompleterResultTypes.TEXT_CHANNEL);
    obj[15] = obj.getResultTotals(tmp4(9844).AutocompleterResultTypes.VOICE_CHANNEL);
    obj[16] = obj.getResultTotals(tmp4(9844).AutocompleterResultTypes.GUILD);
    obj[17] = obj.getResultTotals(tmp4(9844).AutocompleterResultTypes.GROUP_DM);
    if (null != channelId) {
    ...
    if (null != type) {
      ...
      if (null == type) {
        ...
        if (tmp4(9844).AutocompleterResultTypes.GUILD === type3) {
          obj.selected_guild_id = record.id;
        } else {
          if (tmp4(9844).AutocompleterResultTypes.TEXT_CHANNEL !== type3) {
            if (tmp4(9844).AutocompleterResultTypes.VOICE_CHANNEL !== type3) {
              if (tmp4(9844).AutocompleterResultTypes.GROUP_DM === type3) {
                obj.selected_channel_id = record.id;
              } else if (tmp4(9844).AutocompleterResultTypes.USER === type3) {
      ...
      } else if (type.type === tmp4(9844).AutocompleterResultTypes.IN_APP_NAVIGATION) {
  ...
  } else if (tmp6.type === tmp4(9844).AutocompleterResultTypes.IN_APP_NAVIGATION) {
...
function switchToResult(record) {
  ...
  if (sortByMatchScore.AutocompleterResultTypes.GUILD === type) {
  ...
  } else if (tmp5(9844).AutocompleterResultTypes.VOICE_CHANNEL === type) {
    ...
    if (null != channel1) {
      ...
      obj3[0] = obj;
      obj2[0] = obj3;
      tmp5(4456).transitionToChannel(channel1.id, obj2);
      const tmp5Result1 = tmp5(4456);
    }
  } else if (tmp5(9844).AutocompleterResultTypes.USER === type) {
    const obj4 = { recipientIds: null, location: "Quickswitcher" };
    const items = [record.id];
    obj4[0] = items;
    tmp(4458).openPrivateChannel(obj4);
    const tmpResult1 = tmp(4458);
    tmp(10644).channelListScrollTo(closure_12, authStore.getDMFromUserId(record.id));
    const tmpResult2 = tmp(10644);
  } else if (tmp5(9844).AutocompleterResultTypes.GROUP_DM === type) {
    tmp5(4456).transitionToChannel(record.id, { navigationReplace: true });
    const tmp5Result2 = tmp5(4456);
    tmp(10644).channelListScrollTo(closure_12, record.id);
    const tmpResult3 = tmp(10644);
  } else if (tmp5(9844).AutocompleterResultTypes.APPLICATION === type) {
  ...
  } else if (tmp5(9844).AutocompleterResultTypes.GAME_PROFILE === type) {
    const obj5 = { gameId: null, gameProfileModalChecks: null, source: null };
    obj5[0] = record.id;
    const obj6 = { shouldOpenGameProfile: true, gameId: null };
    obj6[1] = record.id;
    obj5[1] = obj6;
    obj5[2] = tmp5(8737).GameProfileSources.QuickSwitcher;
    tmp(8731).openGameProfileModal(obj5);
    const tmpResult4 = tmp(8731);
  } else if (tmp5(9844).AutocompleterResultTypes.LINK === type) {
    if (null != record.inviteCode) {
      ...
      tmp(6043)(record.path, { navigationReplace: true });
    }
  } else if (tmp5(9844).AutocompleterResultTypes.IN_APP_NAVIGATION === type) {
    if (record.record.type !== InAppNavigationType.SETTINGS) {
      if (record.record.type === tmp37.PLAYGROUND) {
        const tmp5Result3 = tmp5(10645);
        ...
        const items1 = [tmp(5925).QUICK_SWITCHER];
        obj7[1] = items1;
        obj7[2] = tmp(5925).QUICK_SWITCHER;
        tmp5(7219).openCollectiblesShop(obj7);
        const tmp5Result4 = tmp5(7219);
      } else {
        tmp(6043)(record.path, { navigationReplace: true });
      }
    }
...
export const switchToResultInNewTab = function switchToResultInNewTab(type) {
  const _require = type;
  type = type.type;
  if (_require(9844).AutocompleterResultTypes.TEXT_CHANNEL !== type) {
    if (tmp(9844).AutocompleterResultTypes.VOICE_CHANNEL !== type) {
      if (tmp(9844).AutocompleterResultTypes.GROUP_DM !== type) {
        if (tmp(9844).AutocompleterResultTypes.DM !== type) {
          if (tmp(9844).AutocompleterResultTypes.USER === type) {
            let obj = dispatcherDefault;
            obj.dispatch({ type: "QUICKSWITCHER_HIDE" });
  ...
  if (null == channel) {
    ...
    tmp(10646).openChannelTabActive(channel.id, guildId);
    const tmpResult = tmp(10646);
  }
};
