...
// Function ID: 10254
// Name: trackClose
// Dependencies: [5, 32, 5160, 1395, 1391, 4339, 1979, 4126, 7308, 676, 1398, 678, 7309, 698, 8944, 709, 5959, 4311, 4497, 5129, 4313, 10254, 9283, 9289, 5865, 10255, 7003, 5748, 7760, 2]
// Exports: getQuickSwitcherOptions, hide, search, selectResult, switchToResult, toggle, trackOpen

// Module 10253 (trackClose)
...
function trackClose(QUICKSWITCHER_CLOSED, type) {
  ...
  const obj2 = require(7309) /* sortByMatchScore */;
  const tmp6 = results[obj2.findNextSelectedResult(obj2, require(undefined, 7309) /* sortByMatchScore */.FindResultDirections.DOWN, -1, results)];
  const isEmailResult = importDefault(8944).isEmail(query);
  const obj3 = importDefault(8944);
  const tmp7 = importDefault;
  const isPhoneNumberResult = importDefault(8944).isPhoneNumber(query);
  const obj4 = importDefault(8944);
  const isUserTagLikeResult = importDefault(8944).isUserTagLike(query);
  ...
  if (null == tmp6) {
    ...
    obj[13] = obj.getResultTotals(tmp4(7309).AutocompleterResultTypes.USER);
    obj[14] = obj.getResultTotals(tmp4(7309).AutocompleterResultTypes.TEXT_CHANNEL);
    obj[15] = obj.getResultTotals(tmp4(7309).AutocompleterResultTypes.VOICE_CHANNEL);
    obj[16] = obj.getResultTotals(tmp4(7309).AutocompleterResultTypes.GUILD);
    obj[17] = obj.getResultTotals(tmp4(7309).AutocompleterResultTypes.GROUP_DM);
    if (null != channelId) {
    ...
    if (null != type) {
      ...
      if (null == type) {
        ...
        if (tmp4(7309).AutocompleterResultTypes.GUILD === type3) {
          obj.selected_guild_id = record.id;
        } else {
          if (tmp4(7309).AutocompleterResultTypes.TEXT_CHANNEL !== type3) {
            if (tmp4(7309).AutocompleterResultTypes.VOICE_CHANNEL !== type3) {
              if (tmp4(7309).AutocompleterResultTypes.GROUP_DM === type3) {
                obj.selected_channel_id = record.id;
              } else if (tmp4(7309).AutocompleterResultTypes.USER === type3) {
      ...
      } else if (type.type === tmp4(7309).AutocompleterResultTypes.IN_APP_NAVIGATION) {
  ...
  } else if (tmp6.type === tmp4(7309).AutocompleterResultTypes.IN_APP_NAVIGATION) {
...
export const switchToResult = function switchToResult(record) {
  ...
  if (require(7309) /* sortByMatchScore */.AutocompleterResultTypes.GUILD === type) {
    let tmp5Result = tmp5(5959);
    tmp5Result.transitionToGuild(record.id, { navigationReplace: true });
  } else if (tmp5(7309).AutocompleterResultTypes.TEXT_CHANNEL === type) {
  ...
  } else if (tmp5(7309).AutocompleterResultTypes.VOICE_CHANNEL === type) {
  ...
  } else if (tmp5(7309).AutocompleterResultTypes.USER === type) {
    ...
    tmp(10254).channelListScrollTo(closure_12, authStore.getDMFromUserId(record.id));
    const tmpResult2 = tmp(10254);
  } else if (tmp5(7309).AutocompleterResultTypes.GROUP_DM === type) {
    tmp5(4311).transitionToChannel(record.id, { navigationReplace: true });
    const tmp5Result2 = tmp5(4311);
    tmp(10254).channelListScrollTo(closure_12, record.id);
    const tmpResult3 = tmp(10254);
  } else if (tmp5(7309).AutocompleterResultTypes.APPLICATION === type) {
  ...
  } else if (tmp5(7309).AutocompleterResultTypes.GAME_PROFILE === type) {
    const obj5 = { gameId: null, gameProfileModalChecks: null, source: null };
    obj5[0] = record.id;
    const obj6 = { shouldOpenGameProfile: true, gameId: null };
    obj6[1] = record.id;
    obj5[1] = obj6;
    obj5[2] = tmp5(9289).GameProfileSources.QuickSwitcher;
    tmp(9283).openGameProfileModal(obj5);
    const tmpResult4 = tmp(9283);
  } else if (tmp5(7309).AutocompleterResultTypes.LINK === type) {
  ...
  } else if (tmp5(7309).AutocompleterResultTypes.IN_APP_NAVIGATION === type) {
    if (record.record.type !== InAppNavigationType.SETTINGS) {
      if (record.record.type === tmp37.PLAYGROUND) {
        const tmp5Result3 = tmp5(10255);
        ...
        tmp5(7003).openCollectiblesShop(obj7);
        const tmp5Result4 = tmp5(7003);
      } else {
        tmp(5865)(record.path, { navigationReplace: true });
      }
    }
