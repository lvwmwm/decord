...
// Function ID: 10249
// Name: trackClose
// Dependencies: [5, 32, 5160, 1376, 1372, 4339, 1960, 4124, 7307, 676, 1379, 678, 7308, 698, 8938, 709, 5957, 4311, 4497, 5129, 4313, 10249, 9277, 9283, 5865, 10250, 7000, 5748, 7758, 2]
// Exports: getQuickSwitcherOptions, hide, search, selectResult, switchToResult, toggle, trackOpen

// Module 10248 (trackClose)
...
function trackClose(QUICKSWITCHER_CLOSED, type) {
  ...
  const obj2 = require(7308) /* sortByMatchScore */;
  const tmp6 = results[obj2.findNextSelectedResult(obj2, require(undefined, 7308) /* sortByMatchScore */.FindResultDirections.DOWN, -1, results)];
  const isEmailResult = importDefault(8938).isEmail(query);
  const obj3 = importDefault(8938);
  const tmp7 = importDefault;
  const isPhoneNumberResult = importDefault(8938).isPhoneNumber(query);
  const obj4 = importDefault(8938);
  const isUserTagLikeResult = importDefault(8938).isUserTagLike(query);
  ...
  if (null == tmp6) {
    ...
    obj[13] = obj.getResultTotals(tmp4(7308).AutocompleterResultTypes.USER);
    obj[14] = obj.getResultTotals(tmp4(7308).AutocompleterResultTypes.TEXT_CHANNEL);
    obj[15] = obj.getResultTotals(tmp4(7308).AutocompleterResultTypes.VOICE_CHANNEL);
    obj[16] = obj.getResultTotals(tmp4(7308).AutocompleterResultTypes.GUILD);
    obj[17] = obj.getResultTotals(tmp4(7308).AutocompleterResultTypes.GROUP_DM);
    if (null != channelId) {
    ...
    if (null != type) {
      ...
      if (null == type) {
        ...
        if (tmp4(7308).AutocompleterResultTypes.GUILD === type3) {
          obj.selected_guild_id = record.id;
        } else {
          if (tmp4(7308).AutocompleterResultTypes.TEXT_CHANNEL !== type3) {
            if (tmp4(7308).AutocompleterResultTypes.VOICE_CHANNEL !== type3) {
              if (tmp4(7308).AutocompleterResultTypes.GROUP_DM === type3) {
                obj.selected_channel_id = record.id;
              } else if (tmp4(7308).AutocompleterResultTypes.USER === type3) {
      ...
      } else if (type.type === tmp4(7308).AutocompleterResultTypes.IN_APP_NAVIGATION) {
  ...
  } else if (tmp6.type === tmp4(7308).AutocompleterResultTypes.IN_APP_NAVIGATION) {
...
export const switchToResult = function switchToResult(record) {
  ...
  if (require(7308) /* sortByMatchScore */.AutocompleterResultTypes.GUILD === type) {
    let tmp5Result = tmp5(5957);
    tmp5Result.transitionToGuild(record.id, { navigationReplace: true });
  } else if (tmp5(7308).AutocompleterResultTypes.TEXT_CHANNEL === type) {
    const channel = authStore.getChannel(record.id);
    if (null != channel) {
      tmp5Result = tmp5(4311);
  ...
  } else if (tmp5(7308).AutocompleterResultTypes.VOICE_CHANNEL === type) {
    ...
    if (null != channel1) {
      ...
      obj3[0] = obj;
      obj2[0] = obj3;
      tmp5(4311).transitionToChannel(channel1.id, obj2);
      const tmp5Result1 = tmp5(4311);
    }
  } else if (tmp5(7308).AutocompleterResultTypes.USER === type) {
    const obj4 = { recipientIds: null, location: "Quickswitcher" };
    const items = [record.id];
    obj4[0] = items;
    tmp(4313).openPrivateChannel(obj4);
    const tmpResult1 = tmp(4313);
    tmp(10249).channelListScrollTo(closure_12, authStore.getDMFromUserId(record.id));
    const tmpResult2 = tmp(10249);
  } else if (tmp5(7308).AutocompleterResultTypes.GROUP_DM === type) {
    tmp5(4311).transitionToChannel(record.id, { navigationReplace: true });
    const tmp5Result2 = tmp5(4311);
    tmp(10249).channelListScrollTo(closure_12, record.id);
    const tmpResult3 = tmp(10249);
  } else if (tmp5(7308).AutocompleterResultTypes.APPLICATION === type) {
  ...
  } else if (tmp5(7308).AutocompleterResultTypes.GAME_PROFILE === type) {
    const obj5 = { gameId: null, gameProfileModalChecks: null, source: null };
    obj5[0] = record.id;
    const obj6 = { shouldOpenGameProfile: true, gameId: null };
    obj6[1] = record.id;
    obj5[1] = obj6;
    obj5[2] = tmp5(9283).GameProfileSources.QuickSwitcher;
    tmp(9277).openGameProfileModal(obj5);
    const tmpResult4 = tmp(9277);
  } else if (tmp5(7308).AutocompleterResultTypes.LINK === type) {
    if (null != record.inviteCode) {
      ...
      tmp(5865)(record.path, { navigationReplace: true });
    }
  } else if (tmp5(7308).AutocompleterResultTypes.IN_APP_NAVIGATION === type) {
    if (record.record.type !== InAppNavigationType.SETTINGS) {
      if (record.record.type === tmp37.PLAYGROUND) {
        const tmp5Result3 = tmp5(10250);
        ...
        const items1 = [tmp(5748).QUICK_SWITCHER];
        obj7[1] = items1;
        obj7[2] = tmp(5748).QUICK_SWITCHER;
        tmp5(7000).openCollectiblesShop(obj7);
        const tmp5Result4 = tmp5(7000);
      } else {
        tmp(5865)(record.path, { navigationReplace: true });
      }
    }
