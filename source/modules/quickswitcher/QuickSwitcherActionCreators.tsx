...
// Function ID: 10164
// Name: trackClose
// Dependencies: [5, 32, 5127, 1376, 1372, 4320, 1931, 4102, 7227, 676, 1379, 678, 7230, 698, 8860, 709, 5866, 4293, 4478, 5096, 4295, 10164, 9194, 9200, 5774, 10165, 6909, 5630, 7679, 2]
// Exports: getQuickSwitcherOptions, hide, search, selectResult, switchToResult, toggle, trackOpen

// Module 10163 (trackClose)
...
function trackClose(QUICKSWITCHER_CLOSED, type) {
  ...
  if (null == tmp6) {
    ...
    obj[13] = obj.getResultTotals(tmp4(7230).AutocompleterResultTypes.USER);
    obj[14] = obj.getResultTotals(tmp4(7230).AutocompleterResultTypes.TEXT_CHANNEL);
    obj[15] = obj.getResultTotals(tmp4(7230).AutocompleterResultTypes.VOICE_CHANNEL);
    obj[16] = obj.getResultTotals(tmp4(7230).AutocompleterResultTypes.GUILD);
    obj[17] = obj.getResultTotals(tmp4(7230).AutocompleterResultTypes.GROUP_DM);
    if (null != channelId) {
    ...
    if (null != type) {
      ...
      if (null == type) {
        ...
        if (tmp4(7230).AutocompleterResultTypes.GUILD === type3) {
          obj.selected_guild_id = record.id;
        } else {
          if (tmp4(7230).AutocompleterResultTypes.TEXT_CHANNEL !== type3) {
            if (tmp4(7230).AutocompleterResultTypes.VOICE_CHANNEL !== type3) {
              if (tmp4(7230).AutocompleterResultTypes.GROUP_DM === type3) {
                obj.selected_channel_id = record.id;
              } else if (tmp4(7230).AutocompleterResultTypes.USER === type3) {
      ...
      } else if (type.type === tmp4(7230).AutocompleterResultTypes.IN_APP_NAVIGATION) {
  ...
  } else if (tmp6.type === tmp4(7230).AutocompleterResultTypes.IN_APP_NAVIGATION) {
...
export const switchToResult = function switchToResult(record) {
  ...
  if (require(7230) /* sortByMatchScore */.AutocompleterResultTypes.GUILD === type) {
  ...
  } else if (tmp5(7230).AutocompleterResultTypes.VOICE_CHANNEL === type) {
  ...
  } else if (tmp5(7230).AutocompleterResultTypes.USER === type) {
    ...
    tmp(10164).channelListScrollTo(closure_12, authStore.getDMFromUserId(record.id));
    const tmpResult2 = tmp(10164);
  } else if (tmp5(7230).AutocompleterResultTypes.GROUP_DM === type) {
    tmp5(4293).transitionToChannel(record.id, { navigationReplace: true });
    const tmp5Result2 = tmp5(4293);
    tmp(10164).channelListScrollTo(closure_12, record.id);
    const tmpResult3 = tmp(10164);
  } else if (tmp5(7230).AutocompleterResultTypes.APPLICATION === type) {
  ...
  } else if (tmp5(7230).AutocompleterResultTypes.GAME_PROFILE === type) {
    const obj5 = { gameId: null, gameProfileModalChecks: null, source: null };
    obj5[0] = record.id;
    const obj6 = { shouldOpenGameProfile: true, gameId: null };
    obj6[1] = record.id;
    obj5[1] = obj6;
    obj5[2] = tmp5(9200).GameProfileSources.QuickSwitcher;
    tmp(9194).openGameProfileModal(obj5);
    const tmpResult4 = tmp(9194);
  } else if (tmp5(7230).AutocompleterResultTypes.LINK === type) {
    if (null != record.inviteCode) {
      ...
      tmp(5774)(record.path, { navigationReplace: true });
    }
  } else if (tmp5(7230).AutocompleterResultTypes.IN_APP_NAVIGATION === type) {
    if (record.record.type !== InAppNavigationType.SETTINGS) {
      if (record.record.type === tmp37.PLAYGROUND) {
        const tmp5Result3 = tmp5(10165);
        ...
        const items1 = [tmp(5630).QUICK_SWITCHER];
        obj7[1] = items1;
        obj7[2] = tmp(5630).QUICK_SWITCHER;
        tmp5(6909).openCollectiblesShop(obj7);
        const tmp5Result4 = tmp5(6909);
      } else {
        tmp(5774)(record.path, { navigationReplace: true });
      }
    }
