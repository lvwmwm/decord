...
// Function ID: 10295
// Name: trackClose
// Dependencies: [5, 32, 5200, 1395, 1391, 4380, 1979, 4167, 7347, 676, 1398, 678, 7348, 698, 8983, 709, 5998, 4352, 4537, 5169, 4354, 10295, 9323, 9329, 5904, 10296, 7042, 5787, 7799, 2]
// Exports: getQuickSwitcherOptions, hide, search, selectResult, switchToResult, toggle, trackOpen

// Module 10294 (trackClose)
...
function trackClose(QUICKSWITCHER_CLOSED, type) {
  ...
  const obj2 = require(7348) /* sortByMatchScore */;
  const tmp6 = results[obj2.findNextSelectedResult(obj2, require(undefined, 7348) /* sortByMatchScore */.FindResultDirections.DOWN, -1, results)];
  const isEmailResult = importDefault(8983).isEmail(query);
  const obj3 = importDefault(8983);
  const tmp7 = importDefault;
  const isPhoneNumberResult = importDefault(8983).isPhoneNumber(query);
  const obj4 = importDefault(8983);
  const isUserTagLikeResult = importDefault(8983).isUserTagLike(query);
  ...
  if (null == tmp6) {
    ...
    obj[13] = obj.getResultTotals(tmp4(7348).AutocompleterResultTypes.USER);
    obj[14] = obj.getResultTotals(tmp4(7348).AutocompleterResultTypes.TEXT_CHANNEL);
    obj[15] = obj.getResultTotals(tmp4(7348).AutocompleterResultTypes.VOICE_CHANNEL);
    obj[16] = obj.getResultTotals(tmp4(7348).AutocompleterResultTypes.GUILD);
    obj[17] = obj.getResultTotals(tmp4(7348).AutocompleterResultTypes.GROUP_DM);
    if (null != channelId) {
    ...
    if (null != type) {
      ...
      if (null == type) {
        ...
        if (tmp4(7348).AutocompleterResultTypes.GUILD === type3) {
          obj.selected_guild_id = record.id;
        } else {
          if (tmp4(7348).AutocompleterResultTypes.TEXT_CHANNEL !== type3) {
            if (tmp4(7348).AutocompleterResultTypes.VOICE_CHANNEL !== type3) {
              if (tmp4(7348).AutocompleterResultTypes.GROUP_DM === type3) {
                obj.selected_channel_id = record.id;
              } else if (tmp4(7348).AutocompleterResultTypes.USER === type3) {
      ...
      } else if (type.type === tmp4(7348).AutocompleterResultTypes.IN_APP_NAVIGATION) {
  ...
  } else if (tmp6.type === tmp4(7348).AutocompleterResultTypes.IN_APP_NAVIGATION) {
...
export const switchToResult = function switchToResult(record) {
  ...
  if (require(7348) /* sortByMatchScore */.AutocompleterResultTypes.GUILD === type) {
    let tmp5Result = tmp5(5998);
    tmp5Result.transitionToGuild(record.id, { navigationReplace: true });
  } else if (tmp5(7348).AutocompleterResultTypes.TEXT_CHANNEL === type) {
    const channel = authStore.getChannel(record.id);
    if (null != channel) {
      tmp5Result = tmp5(4352);
  ...
  } else if (tmp5(7348).AutocompleterResultTypes.VOICE_CHANNEL === type) {
    ...
    if (null != channel1) {
      ...
      obj3[0] = obj;
      obj2[0] = obj3;
      tmp5(4352).transitionToChannel(channel1.id, obj2);
      const tmp5Result1 = tmp5(4352);
    }
  } else if (tmp5(7348).AutocompleterResultTypes.USER === type) {
    const obj4 = { recipientIds: null, location: "Quickswitcher" };
    const items = [record.id];
    obj4[0] = items;
    tmp(4354).openPrivateChannel(obj4);
    const tmpResult1 = tmp(4354);
    tmp(10295).channelListScrollTo(closure_12, authStore.getDMFromUserId(record.id));
    const tmpResult2 = tmp(10295);
  } else if (tmp5(7348).AutocompleterResultTypes.GROUP_DM === type) {
    tmp5(4352).transitionToChannel(record.id, { navigationReplace: true });
    const tmp5Result2 = tmp5(4352);
    tmp(10295).channelListScrollTo(closure_12, record.id);
    const tmpResult3 = tmp(10295);
  } else if (tmp5(7348).AutocompleterResultTypes.APPLICATION === type) {
  ...
  } else if (tmp5(7348).AutocompleterResultTypes.GAME_PROFILE === type) {
    const obj5 = { gameId: null, gameProfileModalChecks: null, source: null };
    obj5[0] = record.id;
    const obj6 = { shouldOpenGameProfile: true, gameId: null };
    obj6[1] = record.id;
    obj5[1] = obj6;
    obj5[2] = tmp5(9329).GameProfileSources.QuickSwitcher;
    tmp(9323).openGameProfileModal(obj5);
    const tmpResult4 = tmp(9323);
  } else if (tmp5(7348).AutocompleterResultTypes.LINK === type) {
    if (null != record.inviteCode) {
      ...
      tmp(5904)(record.path, { navigationReplace: true });
    }
  } else if (tmp5(7348).AutocompleterResultTypes.IN_APP_NAVIGATION === type) {
    if (record.record.type !== InAppNavigationType.SETTINGS) {
      if (record.record.type === tmp37.PLAYGROUND) {
        const tmp5Result3 = tmp5(10296);
        ...
        const items1 = [tmp(5787).QUICK_SWITCHER];
        obj7[1] = items1;
        obj7[2] = tmp(5787).QUICK_SWITCHER;
        tmp5(7042).openCollectiblesShop(obj7);
        const tmp5Result4 = tmp5(7042);
      } else {
        tmp(5904)(record.path, { navigationReplace: true });
      }
    }
