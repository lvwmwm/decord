...
// Dependencies: [5, 32, 5403, 1390, 1386, 4520, 1980, 4302, 9763, 673, 1393, 675, 9764, 695, 8306, 706, 7280, 4492, 4729, 5372, 4494, 10918, 8610, 8616, 7184, 10919, 7474, 7122, 10920, 8308, 2]
// Exports: hide, search, selectResult, switchToResultInNewTab, toggle, trackOpen

// Module 10917 (getQuickSwitcherOptions)
import expandEventPropertiesDefault from "expandEventProperties" /* 695 */;
import dispatcherDefault from "dispatcher" /* 706 */;
import setDefault from "set" /* 8306 */;
import sortByMatchScore from "sortByMatchScore" /* 9764 */;
import closure_3 from "asyncGeneratorStep" /* 5 */;
import closure_4 from "_slicedToArray" /* 32 */;
import { InAppNavigationType } from "fromType" /* 5403 */;
import { ChannelRecordBase } from "createChannelRecord" /* 1390 */;
import closure_7 from "ensureGuildLoaded" /* 1386 */;
import closure_8 from "setLibraryApplications" /* 4520 */;
import closure_9 from "handleConnectionOpen" /* 1980 */;
import closure_10 from "handleConnectionOpen" /* 4302 */;
import closure_11 from "handleConnectionOpen" /* 9763 */;
...
function getQuickSwitcherOptions(str) {
  ...
  if (charAtResult === sortByMatchScore.AutocompleterQuerySymbols.USER) {
    if (charAtResult1 === tmp2(9764).AutocompleterQuerySymbols.USER) {
      const items = [str.slice(2), tmp2(9764).AutocompleterResultTypes.USER_GLOBAL];
      let items1 = items;
    }
...
function trackClose(QUICKSWITCHER_RESULT_SELECTED, type) {
  ...
  if (null == tmp6) {
    ...
    obj[13] = obj.getResultTotals(tmp4(9764).AutocompleterResultTypes.USER);
    obj[14] = obj.getResultTotals(tmp4(9764).AutocompleterResultTypes.TEXT_CHANNEL);
    obj[15] = obj.getResultTotals(tmp4(9764).AutocompleterResultTypes.VOICE_CHANNEL);
    obj[16] = obj.getResultTotals(tmp4(9764).AutocompleterResultTypes.GUILD);
    obj[17] = obj.getResultTotals(tmp4(9764).AutocompleterResultTypes.GROUP_DM);
    if (null != channelId) {
    ...
    if (null != type) {
      ...
      if (null == type) {
        ...
        if (tmp4(9764).AutocompleterResultTypes.GUILD === type3) {
          obj.selected_guild_id = record.id;
        } else {
          if (tmp4(9764).AutocompleterResultTypes.TEXT_CHANNEL !== type3) {
            if (tmp4(9764).AutocompleterResultTypes.VOICE_CHANNEL !== type3) {
              if (tmp4(9764).AutocompleterResultTypes.GROUP_DM === type3) {
                obj.selected_channel_id = record.id;
              } else if (tmp4(9764).AutocompleterResultTypes.USER === type3) {
      ...
      } else if (type.type === tmp4(9764).AutocompleterResultTypes.IN_APP_NAVIGATION) {
  ...
  } else if (tmp6.type === tmp4(9764).AutocompleterResultTypes.IN_APP_NAVIGATION) {
...
function switchToResult(record) {
  ...
  if (sortByMatchScore.AutocompleterResultTypes.GUILD === type) {
    let tmp5Result = tmp5(7280);
    tmp5Result.transitionToGuild(record.id, { navigationReplace: true });
  } else if (tmp5(9764).AutocompleterResultTypes.TEXT_CHANNEL === type) {
  ...
  } else if (tmp5(9764).AutocompleterResultTypes.VOICE_CHANNEL === type) {
    ...
    if (null != channel1) {
      ...
      obj3[0] = obj;
      obj2[0] = obj3;
      tmp5(4492).transitionToChannel(channel1.id, obj2);
      const tmp5Result1 = tmp5(4492);
    }
  } else if (tmp5(9764).AutocompleterResultTypes.USER === type) {
    const obj4 = { recipientIds: null, location: "Quickswitcher" };
    const items = [record.id];
    obj4[0] = items;
    tmp(4494).openPrivateChannel(obj4);
    const tmpResult1 = tmp(4494);
    tmp(10918).channelListScrollTo(closure_12, authStore.getDMFromUserId(record.id));
    const tmpResult2 = tmp(10918);
  } else if (tmp5(9764).AutocompleterResultTypes.GROUP_DM === type) {
    tmp5(4492).transitionToChannel(record.id, { navigationReplace: true });
    const tmp5Result2 = tmp5(4492);
    tmp(10918).channelListScrollTo(closure_12, record.id);
    const tmpResult3 = tmp(10918);
  } else if (tmp5(9764).AutocompleterResultTypes.APPLICATION === type) {
  ...
  } else if (tmp5(9764).AutocompleterResultTypes.GAME_PROFILE === type) {
    const obj5 = { gameId: null, gameProfileModalChecks: null, source: null };
    obj5[0] = record.id;
    const obj6 = { shouldOpenGameProfile: true, gameId: null };
    obj6[1] = record.id;
    obj5[1] = obj6;
    obj5[2] = tmp5(8616).GameProfileSources.QuickSwitcher;
    tmp(8610).openGameProfileModal(obj5);
    const tmpResult4 = tmp(8610);
  } else if (tmp5(9764).AutocompleterResultTypes.LINK === type) {
    if (null != record.inviteCode) {
      ...
      tmp(7184)(record.path, { navigationReplace: true });
    }
  } else if (tmp5(9764).AutocompleterResultTypes.IN_APP_NAVIGATION === type) {
    if (record.record.type !== InAppNavigationType.SETTINGS) {
      if (record.record.type === tmp37.PLAYGROUND) {
        const tmp5Result3 = tmp5(10919);
        ...
        const items1 = [tmp(7122).QUICK_SWITCHER];
        obj7[1] = items1;
        obj7[2] = tmp(7122).QUICK_SWITCHER;
        tmp5(7474).openCollectiblesShop(obj7);
        const tmp5Result4 = tmp5(7474);
      } else {
        tmp(7184)(record.path, { navigationReplace: true });
      }
    }
...
export const switchToResultInNewTab = function switchToResultInNewTab(type) {
  const _require = type;
  type = type.type;
  if (_require(9764).AutocompleterResultTypes.TEXT_CHANNEL !== type) {
    if (tmp(9764).AutocompleterResultTypes.VOICE_CHANNEL !== type) {
      if (tmp(9764).AutocompleterResultTypes.GROUP_DM !== type) {
        if (tmp(9764).AutocompleterResultTypes.DM !== type) {
          if (tmp(9764).AutocompleterResultTypes.USER === type) {
            let obj = dispatcherDefault;
            obj.dispatch({ type: "QUICKSWITCHER_HIDE" });
  ...
  if (null == channel) {
    ...
    tmp(10920).openChannelTabActive(channel.id, guildId);
    const tmpResult = tmp(10920);
  }
};
