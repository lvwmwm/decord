...
// Function ID: 9136
// Name: SHARE_APPS_KEY
// Dependencies: [17, 9116, 676, 9136, 9139, 712, 1236, 9110, 9140, 4289, 9141, 1624, 9142, 4312, 9143, 2007, 7258, 500, 9160, 9161, 4517, 6959, 4683, 9162, 9163, 9164, 9165, 9166, 9167, 9168, 9169, 2]

// Module 9135 (SHARE_APPS_KEY)
...
obj = {
  ...
  onPress(code) {
    ...
    return require(9110) /* showInstantInviteActionSheet */.handleOpenShareSheet(code.code, channel, message, _location);
  }
};
const items = [obj, , , , , , , , , , ];
obj = {
  ...
  onPress(arg0) {
    ...
    return require(9110) /* showInstantInviteActionSheet */.handleCopy(code, channel, _location);
  }
};
items[1] = obj;
...
obj1[5] = function onPress(code) {
  ...
  obj[0] = importDefault(7258)(code);
  obj[1] = _location;
  obj[2] = channel;
  obj.openLazy(require(2007) /* asyncRequireImpl */(9143, dependencyMap.paths), combined, obj, "stack");
};
items[2] = obj1;
...
obj2[5] = function onPress(channel) {
  ...
  channel(9110).trackOptionClicked(code, channel, constants.SMS, _location);
  let obj = channel(9110);
  const tmp = channel;
  if (obj2.isIOS()) {
    code(4312).hideActionSheet();
    const obj3 = code(4312);
  }
  obj2 = channel(500);
  tmp(4517).sendSMS({ body: message }, (arg0, arg1, arg2) => {
    if (arg0) {
...
obj3[5] = function onPress(channel) {
  ...
  const code = channel.code;
  ({ message, location: _location } = channel);
  channel(9110).trackOptionClicked(code, channel, constants.EMAIL, _location);
  let obj = channel(9110);
  const tmp = channel;
  if (obj2.isIOS()) {
    code(4312).hideActionSheet();
  ...
  obj2 = channel(500);
  tmp(4517).sendMail({ subject: "", body: message }, (arg0, arg1, arg2) => {
    if (arg0) {
      let obj = code(outer1_2[21]);
...
obj4[4] = function onPress(code) {
  ...
  code = code.code;
  ({ channel, location: _location } = code);
  const tmp = importDefault(7258)(code);
  require(9110) /* showInstantInviteActionSheet */.trackOptionClicked(code, channel, constants.MESSENGER, _location);
...
obj5[4] = function onPress(code) {
  ...
  let message;
  ({ channel, message, location: _location } = code);
  require(9110) /* showInstantInviteActionSheet */.trackOptionClicked(code.code, channel, constants.GMAIL, _location);
...
obj6[4] = function onPress(code) {
  ...
  ({ channel, message, location: _location } = code);
  const tmp = importDefault(7258)(code);
  require(9110) /* showInstantInviteActionSheet */.trackOptionClicked(code, channel, constants.TELEGRAM, _location);
...
obj7[4] = function onPress(code) {
  ...
  let message;
  ({ channel, message, location: _location } = code);
  require(9110) /* showInstantInviteActionSheet */.trackOptionClicked(code.code, channel, constants.TWITTER, _location);
...
obj8[4] = function onPress(code) {
  ...
  let message;
  ({ channel, message, location: _location } = code);
  require(9110) /* showInstantInviteActionSheet */.trackOptionClicked(code.code, channel, constants.WHATSAPP, _location);
...
obj9[4] = function onPress(code) {
  ...
  let message;
  ({ channel, message, location: _location } = code);
  require(9110) /* showInstantInviteActionSheet */.trackOptionClicked(code.code, channel, constants.LINE, _location);
