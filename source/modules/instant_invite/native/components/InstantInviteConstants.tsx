...
// Function ID: 95448
// Name: SHARE_APPS_KEY
// Dependencies: [27, 8330, 653, 10912, 12214, 689, 1212, 8324, 10179, 4110, 12215, 1553, 12216, 4133, 12217, 1935, 6026, 477, 12234, 12235, 4339, 5695, 4506, 12236, 11816, 12237, 12238, 12239, 12240, 12241, 12242, 2]

// Module 12213 (SHARE_APPS_KEY)
...
obj = {
  ...
  onPress(code) {
    ...
    return require(8324) /* showInstantInviteActionSheet */.handleOpenShareSheet(code.code, channel, message, _location);
  }
};
const items = [obj, , , , , , , , , , ];
obj = {
  ...
  onPress(arg0) {
    ...
    return require(8324) /* showInstantInviteActionSheet */.handleCopy(code, channel, _location);
  }
};
items[1] = obj;
...
obj1.onPress = function onPress(code) {
  ...
  obj.link = importDefault(6026)(code);
  obj.location = _location;
  obj.channel = channel;
  obj.openLazy(require(1935) /* maybeLoadBundle */(12217, dependencyMap.paths), combined, obj, "stack");
};
items[2] = obj1;
...
obj2.onPress = function onPress(channel) {
  ...
  channel(8324).trackOptionClicked(code, channel, constants.SMS, _location);
  let obj = channel(8324);
  if (obj2.isIOS()) {
    code(4133).hideActionSheet();
    const obj3 = code(4133);
  }
  obj2 = channel(477);
  channel(4339).sendSMS({ body: message }, (arg0, arg1, arg2) => {
    if (arg0) {
      let id;
...
obj3.onPress = function onPress(channel) {
  ...
  const code = channel.code;
  ({ message, location: _location } = channel);
  channel(8324).trackOptionClicked(code, channel, constants.EMAIL, _location);
  let obj = channel(8324);
  if (obj2.isIOS()) {
    code(4133).hideActionSheet();
    const obj3 = code(4133);
  ...
  obj2 = channel(477);
  channel(4339).sendMail({ subject: "", body: message }, (arg0, arg1, arg2) => {
    if (arg0) {
      let id;
      let obj = code(outer1_2[21]);
      obj = { inviteKey: code };
...
obj4.onPress = function onPress(code) {
  ...
  require(8324) /* showInstantInviteActionSheet */.trackOptionClicked(code, channel, constants.MESSENGER, _location);
...
obj5.onPress = function onPress(code) {
  ...
  let message;
  ({ channel, message, location: _location } = code);
  require(8324) /* showInstantInviteActionSheet */.trackOptionClicked(code.code, channel, constants.GMAIL, _location);
...
obj6.onPress = function onPress(code) {
  ...
  ({ channel, message, location: _location } = code);
  const tmp = importDefault(6026)(code);
  require(8324) /* showInstantInviteActionSheet */.trackOptionClicked(code, channel, constants.TELEGRAM, _location);
...
obj7.onPress = function onPress(code) {
  ...
  let message;
  ({ channel, message, location: _location } = code);
  require(8324) /* showInstantInviteActionSheet */.trackOptionClicked(code.code, channel, constants.TWITTER, _location);
...
obj8.onPress = function onPress(code) {
  ...
  let message;
  ({ channel, message, location: _location } = code);
  require(8324) /* showInstantInviteActionSheet */.trackOptionClicked(code.code, channel, constants.WHATSAPP, _location);
...
obj9.onPress = function onPress(code) {
  ...
  let message;
  ({ channel, message, location: _location } = code);
  require(8324) /* showInstantInviteActionSheet */.trackOptionClicked(code.code, channel, constants.LINE, _location);
