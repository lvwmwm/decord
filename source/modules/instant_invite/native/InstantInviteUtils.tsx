...
// Dependencies: [6813, 1376, 1372, 8960, 1932, 4240, 4341, 1874, 8964, 7090, 8965, 676, 698, 7092, 8966, 8967, 4241, 4223, 8817, 3955, 7112, 5623, 4428, 38, 8962, 2]
...
// Module 8959 (showInstantInviteActionSheet)
...
function showInstantInviteActionSheet(channel, source) {
  ...
  if (null != stream) {
    ...
    const streamerApplication = require(7092) /* _findPlayingActivity */.getStreamerApplication(stream, sortActivity);
    ...
    const obj3 = require(7092) /* _findPlayingActivity */;
  } else {
...
function trackOptionClicked(code, channel, COPY, _location) {
  ...
  const tmp = require;
  obj[3] = require(4241) /* readSnowflake */.parseInviteCodeFromInviteKey(code);
  let type;
  if (channel != null) {
    type = channel.type;
  }
...
export const showVanityUrlInviteActionSheet = function showVanityUrlInviteActionSheet(guild, channel, GUILD_SCHEDULED_EVENT, guildScheduledEventId) {
  ...
  importDefault(8966).init(guild.id, channel.id, { skipCreateInvite: true });
...
export const handleOpenShareSheet = function handleOpenShareSheet(code, channel, intl, ADD_FRIENDS_WIDGET) {
  ...
  if (null != code) {
    const result = require(4241) /* readSnowflake */.parseExtraDataFromInviteKey(code);
    const invite = store3.getInvite(result.baseCode);
    const obj6 = require(4241) /* readSnowflake */;
    const tmp21 = require;
    ...
    tmp26(4223).hideAllActionSheets();
    const tmp26Result = tmp26(4223);
    ...
    tmp21(8817).showShareActionSheet(obj, ADD_FRIENDS_WIDGET);
    const tmp21Result = tmp21(8817);
  }
};
export const handleCopy = function handleCopy(code, channel, GROUP_DM, arg3) {
  ...
  if (null != code) {
    const result = require(4241) /* readSnowflake */.parseExtraDataFromInviteKey(code);
    const obj4 = require(4241) /* readSnowflake */;
    const tmp13 = require;
    const tmp17 = importDefault(7112)(code);
    require(5623) /* _copy */.copy(tmp17);
    const invite = store3.getInvite(result.baseCode);
    const obj5 = require(5623) /* _copy */;
    if (channel instanceof ChannelRecordBase) {
      let guild_id = channel.guild_id;
    } else {
      let id;
    ...
    tmp13(3955).presentLinkCopied();
    const tmp13Result = tmp13(3955);
  }
};
export const handlePressSettings = function handlePressSettings(channel, closure_1, handleGuildTemplateResolveSuccess) {
  ...
  importDefault(8966).openSettings(guild_id, id, str, () => {
    if (null != closure_1) {
      tmp();
    } else {
      let targetApplicationId;
...
export const isAppInstalled = function isAppInstalled(handleGuildTemplateResolveSuccess) {
  return require(4428) /* sendSMS */.canOpenUrlScheme(handleGuildTemplateResolveSuccess);
};
export const handleOpenInviteActionsheet = function handleOpenInviteActionsheet(guild, id, channels, GUILD_HEADER) {
  ...
  let obj1 = require(8962) /* get label */;
