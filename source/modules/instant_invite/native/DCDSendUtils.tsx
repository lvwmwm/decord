...
export const sendSMS = function sendSMS(body, recipients) {
  if (obj.isAndroid()) {
    ...
    importDefault(4478).sendSMS(str, recipients);
...
export const sendMail = function sendMail(subject, subject, recipients) {
  if (obj.isAndroid()) {
    ...
    importDefault(4478).sendMail(str, str2, recipients);
    const obj2 = importDefault(4478);
  } else {
    const DCDSend = closure_4.DCDSend;
...
export const canSendSMS = function canSendSMS() {
  if (obj.isAndroid()) {
    let resolved = Promise.resolve(importDefault(4478).canSendSMS());
    const obj2 = importDefault(4478);
  } else {
    const DCDSend = closure_4.DCDSend;
...
export const canSendMail = function canSendMail() {
  if (obj.isAndroid()) {
    let resolved = Promise.resolve(importDefault(4478).canSendMail());
    const obj2 = importDefault(4478);
  } else {
    const DCDSend = closure_4.DCDSend;
...
export const canOpenUrlScheme = function canOpenUrlScheme(closure_3) {
  ...
  if (obj.isAndroid()) {
    try {
      return Promise.resolve(importDefault(4478).canOpenUrlScheme(closure_3));
    } catch (err) {
