...
export const sendSMS = function sendSMS(body, recipients) {
  if (obj.isAndroid()) {
    ...
    importDefault(4476).sendSMS(str, recipients);
    const obj2 = importDefault(4476);
  } else {
...
export const sendMail = function sendMail(subject, subject, recipients) {
  if (obj.isAndroid()) {
    ...
    importDefault(4476).sendMail(str, str2, recipients);
    const obj2 = importDefault(4476);
  } else {
    const DCDSend = closure_4.DCDSend;
...
export const canSendSMS = function canSendSMS() {
  if (obj.isAndroid()) {
    let resolved = Promise.resolve(importDefault(4476).canSendSMS());
    const obj2 = importDefault(4476);
  } else {
    const DCDSend = closure_4.DCDSend;
...
export const canSendMail = function canSendMail() {
  if (obj.isAndroid()) {
    let resolved = Promise.resolve(importDefault(4476).canSendMail());
    const obj2 = importDefault(4476);
  } else {
    const DCDSend = closure_4.DCDSend;
...
export const canOpenUrlScheme = function canOpenUrlScheme(closure_3) {
  ...
  if (obj.isAndroid()) {
    try {
      return Promise.resolve(importDefault(4476).canOpenUrlScheme(closure_3));
    } catch (err) {
