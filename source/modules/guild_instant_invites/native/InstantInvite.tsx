...
// Dependencies: [19, 17, 1390, 673, 21, 4481, 7108, 10864, 10865, 4866, 1233, 10866, 1219, 10868, 5562, 4934, 7112, 4477, 8596, 8028, 10872, 10873, 10874, 10877, 10879, 2]
// Exports: LinkedChannelInvite

// Module 10863 (LinkedChannelInvite)
import memoResult1Default from "memoResult1" /* 10873 */;
import InviteRolesDisplayDefault from "InviteRolesDisplay" /* 10877 */;
import InstantInviteUsesLabelDefault from "InstantInviteUsesLabel" /* 10879 */;
...
const memoResult = importAllResult.memo((onInviteRevoked) => {
  ...
  const items2 = [
    callback(invite(4477).Text, obj),
    callback(invite(8596).ContextMenu, {
      items: inviteActions,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: callback(10868).more, accessibilityLabel: null, ref: null };
        const intl = invite(1233).intl;
        obj[3] = intl.string(invite(1233).t.DEoVWZ);
        obj[4] = ref.ref;
        const merged1 = Object.assign(merged);
        return callback2(invite(8028).IconButton, obj);
      }
    })
  ];
  obj[2] = items2;
  const items3 = [callback2(invite(4934).Stack, obj), , , ];
  obj1 = {
    items: inviteActions,
    children(ref) {
      const merged = Object.assign(ref, Object.create(null));
  ...
  const tmp = callback3();
  items3[1] = callback(invite(10872).InstantInviteDetails, { channel: memo, expiresAt: invite.getExpiresAt() });
  if (tmp9Result) {
  ...
  obj7[2] = items4;
  items3[3] = callback2(invite(4934).Stack, obj7);
  obj6[0] = items3;
  return callback2(invite(5562).Card, obj6);
});
...
export const LinkedChannelInvite = function LinkedChannelInvite(channel) {
  ...
  canUnlinkLobbyChannel = channel(10864).useCanUnlinkLobbyChannel(channel);
  let str;
  const tmp3Result = channel(10864);
  ...
  const tmp9Result = canUnlinkLobbyChannel(10865)(channel.id, str);
  ...
  callback = callback.useCallback(() => {
    if (canUnlinkLobbyChannel) {
      ...
      let obj = canUnlinkLobbyChannel(4866);
      obj = { title: null, body: null };
      const intl = channel(1233).intl;
      obj[0] = intl.string(channel(1233).t.JmUENg);
      const intl2 = channel(1233).intl;
      obj[1] = intl2.string(channel(1233).t.SrvsML);
      obj.show(obj);
    }
  }, items);
  const items1 = [, , ];
  ...
  const memo = callback.useMemo(() => {
    ...
    const intl = channel(1233).intl;
    obj[0] = intl.string(channel(1233).t.aW2YlJ);
    obj[1] = channel(10866).ArrowSmallRightIcon;
    ...
    const intl2 = channel(1233).intl;
    obj[0] = intl2.string(channel(1233).t.JmUENg);
    obj[1] = canUnlinkLobbyChannel(10868).revoke;
    obj[3] = callback;
    items[1] = obj;
    return items;
  }, items1);
  ...
  const tmp9 = canUnlinkLobbyChannel(10865);
  obj[1] = channel(7112).GameIconSizes.SIZE_24;
  const items2 = [callback(canUnlinkLobbyChannel(7112), obj), ];
  ...
  obj1[4] = name;
  items2[1] = callback(channel(4477).Text, obj1);
  obj[1] = items2;
  const items3 = [
    closure_8(View, obj),
    callback(channel(8596).ContextMenu, {
      items: memo,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: canUnlinkLobbyChannel(10868).more, accessibilityLabel: null, ref: null };
        const intl = channel(1233).intl;
        obj[3] = intl.string(channel(1233).t.DEoVWZ);
        obj[4] = ref.ref;
        const merged1 = Object.assign(merged);
        return callback(channel(8028).IconButton, obj);
      }
    })
  ];
  obj2[2] = items3;
  const items4 = [closure_8(channel(4934).Stack, obj2), callback(channel(10872).InstantInviteDetails, { channel }), ];
  const obj4 = { style: tmp.creatorWrapper, children: null };
  ...
  if (null != linked_by) {
    ...
    tmp15Result = tmp15(tmp8(10873), obj5);
  }
  const obj6 = { children: null };
  const obj3 = {
    items: memo,
  ...
  const tmp8Result = canUnlinkLobbyChannel(7112);
  obj4[1] = tmp15Result;
  items4[2] = callback(channel(4934).Stack, { direction: "horizontal", align: "flex-end", children: callback(View, obj4) });
  obj6[0] = items4;
  return closure_8(channel(5562).Card, obj6);
};
