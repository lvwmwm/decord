...
// Dependencies: [19, 17, 1395, 676, 21, 4445, 5911, 10590, 10591, 4810, 1236, 10592, 1222, 10594, 5553, 4878, 5915, 4441, 8717, 7938, 10598, 10599, 10600, 10603, 10605, 2]
// Exports: LinkedChannelInvite

// Module 10589 (LinkedChannelInvite)
...
const memoResult = importAllResult.memo((onInviteRevoked) => {
  ...
  const items2 = [
    callback(invite(4441).Text, obj),
    callback(invite(8717).ContextMenu, {
      items: inviteActions,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: callback(10594).more, accessibilityLabel: null, ref: null };
        const intl = invite(1236).intl;
        obj[3] = intl.string(invite(1236).t.DEoVWZ);
        obj[4] = ref.ref;
        const merged1 = Object.assign(merged);
        return callback2(invite(7938).IconButton, obj);
      }
    })
  ];
  obj[2] = items2;
  const items3 = [callback2(invite(4878).Stack, obj), , , ];
  obj1 = {
    items: inviteActions,
    children(ref) {
      const merged = Object.assign(ref, Object.create(null));
  ...
  const tmp = callback3();
  items3[1] = callback(invite(10598).InstantInviteDetails, { channel: memo, expiresAt: invite.getExpiresAt() });
  if (tmp9Result) {
  ...
  obj7[2] = items4;
  items3[3] = callback2(invite(4878).Stack, obj7);
  obj6[0] = items3;
  return callback2(invite(5553).Card, obj6);
});
...
export const LinkedChannelInvite = function LinkedChannelInvite(channel) {
  ...
  canUnlinkLobbyChannel = channel(10590).useCanUnlinkLobbyChannel(channel);
  let str;
  const tmp3Result = channel(10590);
  ...
  const tmp9Result = canUnlinkLobbyChannel(10591)(channel.id, str);
  ...
  callback = callback.useCallback(() => {
    if (canUnlinkLobbyChannel) {
      ...
      let obj = canUnlinkLobbyChannel(4810);
  ...
  const memo = callback.useMemo(() => {
    ...
    obj[1] = channel(10592).ArrowSmallRightIcon;
    ...
    obj[1] = canUnlinkLobbyChannel(10594).revoke;
    obj[3] = callback;
    items[1] = obj;
    return items;
  }, items1);
  ...
  const tmp9 = canUnlinkLobbyChannel(10591);
  obj[1] = channel(5915).GameIconSizes.SIZE_24;
  const items2 = [callback(canUnlinkLobbyChannel(5915), obj), ];
  ...
  obj1[4] = name;
  items2[1] = callback(channel(4441).Text, obj1);
  obj[1] = items2;
  const items3 = [
    closure_8(View, obj),
    callback(channel(8717).ContextMenu, {
      items: memo,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: canUnlinkLobbyChannel(10594).more, accessibilityLabel: null, ref: null };
        const intl = channel(1236).intl;
        obj[3] = intl.string(channel(1236).t.DEoVWZ);
        obj[4] = ref.ref;
        const merged1 = Object.assign(merged);
        return callback(channel(7938).IconButton, obj);
      }
    })
  ];
  obj2[2] = items3;
  const items4 = [closure_8(channel(4878).Stack, obj2), callback(channel(10598).InstantInviteDetails, { channel }), ];
  const obj4 = { style: tmp.creatorWrapper, children: null };
  ...
  if (null != linked_by) {
    ...
    tmp15Result = tmp15(tmp8(10599), obj5);
  }
  const obj6 = { children: null };
  const obj3 = {
    items: memo,
  ...
  const tmp8Result = canUnlinkLobbyChannel(5915);
  obj4[1] = tmp15Result;
  items4[2] = callback(channel(4878).Stack, { direction: "horizontal", align: "flex-end", children: callback(View, obj4) });
  obj6[0] = items4;
  return closure_8(channel(5553).Card, obj6);
};
