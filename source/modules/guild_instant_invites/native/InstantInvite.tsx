...
// Dependencies: [19, 17, 1376, 676, 21, 4255, 5601, 9576, 9577, 4594, 1236, 9578, 1222, 9580, 5302, 4663, 5605, 4251, 8380, 7644, 9584, 9585, 9586, 9589, 9591, 2]
// Exports: LinkedChannelInvite

// Module 9575 (LinkedChannelInvite)
...
const memoResult = importAllResult.memo((onInviteRevoked) => {
  ...
  if (null != id1) {
    ...
    obj5[2] = id2;
    obj5[3] = id;
    tmp9Result = tmp9(importDefault(9585), obj5);
    const tmp15 = importDefault(9585);
  }
  const obj6 = { children: null };
  const obj7 = { direction: "horizontal", align: "flex-end", children: null };
  obj4[1] = tmp9Result;
  const items4 = [callback(View, obj4), callback(importDefault(9591), { uses, maxUses })];
  obj7[2] = items4;
  items3[3] = callback2(invite(4663).Stack, obj7);
  obj6[0] = items3;
  return callback2(invite(5302).Card, obj6);
});
...
export const LinkedChannelInvite = function LinkedChannelInvite(channel) {
  ...
  canUnlinkLobbyChannel = channel(9576).useCanUnlinkLobbyChannel(channel);
  let str;
  const tmp3Result = channel(9576);
  ...
  const tmp9Result = canUnlinkLobbyChannel(9577)(channel.id, str);
  ...
  const tmp9 = canUnlinkLobbyChannel(9577);
  obj[1] = channel(5605).GameIconSizes.SIZE_24;
  const items2 = [callback(canUnlinkLobbyChannel(5605), obj), ];
  ...
  const items3 = [
    closure_8(View, obj),
    callback(channel(8380).ContextMenu, {
      items: memo,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: null, accessibilityLabel: null, ref: null };
  ...
  obj2[2] = items3;
  const items4 = [closure_8(channel(4663).Stack, obj2), callback(channel(9584).InstantInviteDetails, { channel }), ];
  ...
  if (null != linked_by) {
    ...
    tmp15Result = tmp15(tmp8(9585), obj5);
  }
  const obj6 = { children: null };
  const obj3 = {
    items: memo,
  ...
  const tmp8Result = canUnlinkLobbyChannel(5605);
  obj4[1] = tmp15Result;
  items4[2] = callback(channel(4663).Stack, { direction: "horizontal", align: "flex-end", children: callback(View, obj4) });
  obj6[0] = items4;
  return closure_8(channel(5302).Card, obj6);
};
