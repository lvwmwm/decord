...
// Dependencies: [19, 17, 1390, 673, 21, 4478, 5967, 10891, 10892, 4858, 1233, 10893, 1219, 10895, 5609, 4926, 5971, 4474, 8807, 8015, 10899, 10900, 10901, 10904, 10906, 2]
// Exports: LinkedChannelInvite

// Module 10890 (LinkedChannelInvite)
import memoResult1Default from "memoResult1" /* 10900 */;
import InviteRolesDisplayDefault from "InviteRolesDisplay" /* 10904 */;
import InstantInviteUsesLabelDefault from "InstantInviteUsesLabel" /* 10906 */;
...
const memoResult = importAllResult.memo((onInviteRevoked) => {
  ...
  const items2 = [
    ...
    callback(invite(8807).ContextMenu, {
      items: inviteActions,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: callback(10895).more, accessibilityLabel: null, ref: null };
        const intl = invite(1233).intl;
        obj[3] = intl.string(invite(1233).t.DEoVWZ);
        obj[4] = ref.ref;
        const merged1 = Object.assign(merged);
        return callback2(invite(8015).IconButton, obj);
      }
    })
  ];
  obj[2] = items2;
  ...
  const tmp = callback3();
  items3[1] = callback(invite(10899).InstantInviteDetails, { channel: memo, expiresAt: invite.getExpiresAt() });
  if (tmp9Result) {
  ...
  obj6[0] = items3;
  return callback2(invite(5609).Card, obj6);
});
...
export const LinkedChannelInvite = function LinkedChannelInvite(channel) {
  ...
  canUnlinkLobbyChannel = channel(10891).useCanUnlinkLobbyChannel(channel);
  let str;
  const tmp3Result = channel(10891);
  ...
  const tmp9Result = canUnlinkLobbyChannel(10892)(channel.id, str);
  ...
  const memo = callback.useMemo(() => {
    ...
    obj[1] = channel(10893).ArrowSmallRightIcon;
    ...
    obj[1] = canUnlinkLobbyChannel(10895).revoke;
    obj[3] = callback;
    items[1] = obj;
    return items;
  }, items1);
  ...
  const tmp9 = canUnlinkLobbyChannel(10892);
  obj[1] = channel(5971).GameIconSizes.SIZE_24;
  const items2 = [callback(canUnlinkLobbyChannel(5971), obj), ];
  ...
  const items3 = [
    closure_8(View, obj),
    callback(channel(8807).ContextMenu, {
      items: memo,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: canUnlinkLobbyChannel(10895).more, accessibilityLabel: null, ref: null };
        const intl = channel(1233).intl;
        obj[3] = intl.string(channel(1233).t.DEoVWZ);
        obj[4] = ref.ref;
        const merged1 = Object.assign(merged);
        return callback(channel(8015).IconButton, obj);
      }
    })
  ];
  obj2[2] = items3;
  const items4 = [closure_8(channel(4926).Stack, obj2), callback(channel(10899).InstantInviteDetails, { channel }), ];
  const obj4 = { style: tmp.creatorWrapper, children: null };
  ...
  if (null != linked_by) {
    ...
    tmp15Result = tmp15(tmp8(10900), obj5);
  }
  const obj6 = { children: null };
  const obj3 = {
    items: memo,
