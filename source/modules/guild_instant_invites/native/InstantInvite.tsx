...
// Dependencies: [19, 17, 1395, 676, 21, 4668, 7183, 9988, 9989, 4662, 1236, 9990, 1222, 9992, 6323, 4738, 7187, 4739, 9997, 8074, 10003, 10004, 10006, 10009, 10011, 2]
// Exports: LinkedChannelInvite

// Module 9987 (LinkedChannelInvite)
import memoResult1Default from "memoResult1" /* 10004 */;
import InviteRolesDisplayDefault from "InviteRolesDisplay" /* 10009 */;
import InstantInviteUsesLabelDefault from "InstantInviteUsesLabel" /* 10011 */;
...
const memoResult = importAllResult.memo((onInviteRevoked) => {
  ...
  const items2 = [
    callback(invite(4739).Text, obj),
    callback(invite(9997).ContextMenu, {
      items: inviteActions,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: callback(9992).more, accessibilityLabel: null, ref: null };
        const intl = invite(1236).intl;
        obj[3] = intl.string(invite(1236).t.DEoVWZ);
        obj[4] = ref.ref;
        const merged1 = Object.assign(merged);
        return callback2(invite(8074).IconButton, obj);
      }
    })
  ];
  obj[2] = items2;
  const items3 = [callback2(invite(4738).Stack, obj), , , ];
  obj1 = {
    items: inviteActions,
    children(ref) {
      const merged = Object.assign(ref, Object.create(null));
  ...
  const tmp = callback3();
  items3[1] = callback(invite(10003).InstantInviteDetails, { channel: memo, expiresAt: invite.getExpiresAt() });
  if (tmp9Result) {
  ...
  obj7[2] = items4;
  items3[3] = callback2(invite(4738).Stack, obj7);
  obj6[0] = items3;
  return callback2(invite(6323).Card, obj6);
});
...
export const LinkedChannelInvite = function LinkedChannelInvite(channel) {
  ...
  canUnlinkLobbyChannel = channel(9988).useCanUnlinkLobbyChannel(channel);
  let str;
  const tmp3Result = channel(9988);
  ...
  const tmp9Result = canUnlinkLobbyChannel(9989)(channel.id, str);
  ...
  callback = callback.useCallback(() => {
    if (canUnlinkLobbyChannel) {
      ...
      let obj = canUnlinkLobbyChannel(4662);
  ...
  const memo = callback.useMemo(() => {
    ...
    obj[1] = channel(9990).ArrowSmallRightIcon;
    ...
    obj[1] = canUnlinkLobbyChannel(9992).revoke;
    obj[3] = callback;
    items[1] = obj;
    return items;
  }, items1);
  ...
  const tmp9 = canUnlinkLobbyChannel(9989);
  obj[1] = channel(7187).GameIconSizes.SIZE_24;
  const items2 = [callback(canUnlinkLobbyChannel(7187), obj), ];
  ...
  obj1[4] = name;
  items2[1] = callback(channel(4739).Text, obj1);
  obj[1] = items2;
  const items3 = [
    closure_8(View, obj),
    callback(channel(9997).ContextMenu, {
      items: memo,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: canUnlinkLobbyChannel(9992).more, accessibilityLabel: null, ref: null };
        const intl = channel(1236).intl;
        obj[3] = intl.string(channel(1236).t.DEoVWZ);
        obj[4] = ref.ref;
        const merged1 = Object.assign(merged);
        return callback(channel(8074).IconButton, obj);
      }
    })
  ];
  obj2[2] = items3;
  const items4 = [closure_8(channel(4738).Stack, obj2), callback(channel(10003).InstantInviteDetails, { channel }), ];
  const obj4 = { style: tmp.creatorWrapper, children: null };
  ...
  if (null != linked_by) {
    ...
    tmp15Result = tmp15(tmp8(10004), obj5);
  }
  const obj6 = { children: null };
  const obj3 = {
    items: memo,
  ...
  const tmp8Result = canUnlinkLobbyChannel(7187);
  obj4[1] = tmp15Result;
  items4[2] = callback(channel(4738).Stack, { direction: "horizontal", align: "flex-end", children: callback(View, obj4) });
  obj6[0] = items4;
  return closure_8(channel(6323).Card, obj6);
};
