...
// Dependencies: [19, 17, 1395, 676, 21, 4380, 5840, 9836, 9837, 4745, 1236, 9838, 1222, 9840, 5482, 4813, 5844, 4376, 8745, 7896, 9844, 9845, 9846, 9849, 9851, 2]
// Exports: LinkedChannelInvite

// Module 9835 (LinkedChannelInvite)
import memoResult1Default from "memoResult1" /* 9845 */;
import InviteRolesDisplayDefault from "InviteRolesDisplay" /* 9849 */;
import InstantInviteUsesLabelDefault from "InstantInviteUsesLabel" /* 9851 */;
...
const memoResult = importAllResult.memo((onInviteRevoked) => {
  ...
  const items2 = [
    callback(invite(4376).Text, obj),
    callback(invite(8745).ContextMenu, {
      items: inviteActions,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: callback(9840).more, accessibilityLabel: null, ref: null };
        const intl = invite(1236).intl;
        obj[3] = intl.string(invite(1236).t.DEoVWZ);
        obj[4] = ref.ref;
        const merged1 = Object.assign(merged);
        return callback2(invite(7896).IconButton, obj);
      }
    })
  ];
  obj[2] = items2;
  const items3 = [callback2(invite(4813).Stack, obj), , , ];
  obj1 = {
    items: inviteActions,
    children(ref) {
      const merged = Object.assign(ref, Object.create(null));
  ...
  const tmp = callback3();
  items3[1] = callback(invite(9844).InstantInviteDetails, { channel: memo, expiresAt: invite.getExpiresAt() });
  if (tmp9Result) {
  ...
  obj7[2] = items4;
  items3[3] = callback2(invite(4813).Stack, obj7);
  obj6[0] = items3;
  return callback2(invite(5482).Card, obj6);
});
...
export const LinkedChannelInvite = function LinkedChannelInvite(channel) {
  ...
  canUnlinkLobbyChannel = channel(9836).useCanUnlinkLobbyChannel(channel);
  let str;
  const tmp3Result = channel(9836);
  ...
  const tmp9Result = canUnlinkLobbyChannel(9837)(channel.id, str);
  ...
  callback = callback.useCallback(() => {
    if (canUnlinkLobbyChannel) {
      ...
      let obj = canUnlinkLobbyChannel(4745);
  ...
  const memo = callback.useMemo(() => {
    ...
    obj[1] = channel(9838).ArrowSmallRightIcon;
    ...
    obj[1] = canUnlinkLobbyChannel(9840).revoke;
    obj[3] = callback;
    items[1] = obj;
    return items;
  }, items1);
  ...
  const tmp9 = canUnlinkLobbyChannel(9837);
  obj[1] = channel(5844).GameIconSizes.SIZE_24;
  const items2 = [callback(canUnlinkLobbyChannel(5844), obj), ];
  ...
  obj1[4] = name;
  items2[1] = callback(channel(4376).Text, obj1);
  obj[1] = items2;
  const items3 = [
    closure_8(View, obj),
    callback(channel(8745).ContextMenu, {
      items: memo,
      children(ref) {
        const merged = Object.assign(ref, Object.create(null));
        const obj = { size: "sm", variant: "secondary", icon: canUnlinkLobbyChannel(9840).more, accessibilityLabel: null, ref: null };
        const intl = channel(1236).intl;
        obj[3] = intl.string(channel(1236).t.DEoVWZ);
        obj[4] = ref.ref;
        const merged1 = Object.assign(merged);
        return callback(channel(7896).IconButton, obj);
      }
    })
  ];
  obj2[2] = items3;
  const items4 = [closure_8(channel(4813).Stack, obj2), callback(channel(9844).InstantInviteDetails, { channel }), ];
  const obj4 = { style: tmp.creatorWrapper, children: null };
  ...
  if (null != linked_by) {
    ...
    tmp15Result = tmp15(tmp8(9845), obj5);
  }
  const obj6 = { children: null };
  const obj3 = {
    items: memo,
  ...
  const tmp8Result = canUnlinkLobbyChannel(5844);
  obj4[1] = tmp15Result;
  items4[2] = callback(channel(4813).Stack, { direction: "horizontal", align: "flex-end", children: callback(View, obj4) });
  obj6[0] = items4;
  return closure_8(channel(5482).Card, obj6);
};
