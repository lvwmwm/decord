...
// Function ID: 112243
// Name: SmsScreen
// Dependencies: [5, 57, 31, 27, 653, 33, 9169, 14746, 1212, 507, 14750, 14745, 6706, 4578, 14748, 2]
// Exports: default

// Module 14753 (SmsScreen)
...
export default function SmsScreen(mfaChallenge) {
  ...
  let obj = finish(14746);
  const screenStyles = obj.useScreenStyles(finish(9169)());
  const tmp3 = first(React.useState(null), 2);
  dependencyMap = tmp3[1];
  const tmp = finish(9169)();
  ...
  const obj1 = { autoFocus: true, autoCapitalize: "characters", maxLength: mfaChallenge(14750).SMS_CODE_LENGTH, autoComplete: "sms-otp", textContentType: "oneTimeCode", keyboardType: "number-pad", onChange: handleChange };
  ...
  const items1 = [handleChange(mfaChallenge(6706).TextInput, obj1), ];
  ...
  items1[1] = handleChange(mfaChallenge(4578).Button, obj2);
  obj.children = items1;
  obj.input = _handleChange(c6, obj);
  const obj3 = { variant: "primary" };
  const tmp13 = finish(14745);
  ...
  if (!tmp5) {
    tmp5 = first.length !== mfaChallenge(14750).SMS_CODE_LENGTH;
  }
  obj3.disabled = tmp5;
  obj.submit = handleChange(finish(14748), obj3);
