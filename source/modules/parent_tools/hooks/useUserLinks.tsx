...
// Dependencies: [19, 1922, 7066, 7067, 647, 7404, 7405, 7122, 2]
...
// Module 7403 (useUserIdsForLinkStatus)
...
export const useActivityWindowTimeStamp = function useActivityWindowTimeStamp(activityWindowTimestampFormatter) {
  ...
  let closure_1 = _require(7405).useSelectedTeenId();
  const obj = _require(7405);
  const tmp = _require;
  const items = [freshTeenActivityWithMap];
...
export const useLinkTimestampText = function useLinkTimestampText(id, status) {
  ...
  if (null == stateFromStores) {
    ...
    const _Date = Date;
    _require(7122).formatLinkTimestamp(Date.parse(stateFromStores), status === constants.PENDING ? closure_10 : closure_6);
    const tmpResult = _require(7122);
  }
};
