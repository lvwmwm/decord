...
// Dependencies: [19, 1922, 7038, 7039, 647, 7378, 7379, 7094, 2]
...
// Module 7377 (useUserIdsForLinkStatus)
...
export const useActivityWindowTimeStamp = function useActivityWindowTimeStamp(activityWindowTimestampFormatter) {
  ...
  let closure_1 = _require(7379).useSelectedTeenId();
  const obj = _require(7379);
  const tmp = _require;
  const items = [freshTeenActivityWithMap];
...
export const useLinkTimestampText = function useLinkTimestampText(id, status) {
  ...
  if (null == stateFromStores) {
    ...
    const _Date = Date;
    _require(7094).formatLinkTimestamp(Date.parse(stateFromStores), status === constants.PENDING ? closure_10 : closure_6);
    const tmpResult = _require(7094);
  }
};
