...
// Dependencies: [19, 1922, 6999, 7000, 647, 7339, 7340, 7055, 2]
...
// Module 7338 (useUserIdsForLinkStatus)
...
export const useActivityWindowTimeStamp = function useActivityWindowTimeStamp(activityWindowTimestampFormatter) {
  ...
  let closure_1 = _require(7340).useSelectedTeenId();
  const obj = _require(7340);
  const tmp = _require;
  const items = [freshTeenActivityWithMap];
...
export const useLinkTimestampText = function useLinkTimestampText(id, status) {
  ...
  if (null == stateFromStores) {
    ...
    const _Date = Date;
    _require(7055).formatLinkTimestamp(Date.parse(stateFromStores), status === constants.PENDING ? closure_10 : closure_6);
    const tmpResult = _require(7055);
  }
};
