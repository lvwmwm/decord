...
// Dependencies: [32, 19, 17, 7210, 21, 4444, 712, 7547, 7265, 14422, 4440, 11557, 1236, 2369, 14428, 5015, 2]
// Exports: default

// Module 14427 (FamilyCenterActivitySectionHeader)
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    obj1[0] = displayType;
    const items1 = [callback2(FamilyCenterActivitySectionHeader, obj1), substr.map((action) => callback(loadMore(14428), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj5[4] = tmp17(tmp(4440).Text, obj6);
        tmp17Result = tmp17(tmp(5015).PressableOpacity, obj5);
      }
      obj2[1] = tmp17Result;
