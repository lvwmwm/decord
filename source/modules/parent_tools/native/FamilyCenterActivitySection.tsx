...
// Dependencies: [32, 19, 17, 6937, 21, 4302, 712, 7278, 6992, 14069, 4298, 11257, 1236, 2284, 14075, 4844, 2]
// Exports: default

// Module 14074 (FamilyCenterActivitySectionHeader)
...
function FamilyCenterActivitySectionHeader(displayType) {
  ...
  let flag = importDefault(7278)();
  let obj = require(6992) /* getEmptyActivityFormatter */;
  const activityTypeTextConfigs = obj.getActivityTypeTextConfigs();
  const value = activityTypeTextConfigs.get(displayType);
  const actionsForDisplayType = require(14069) /* useActionsForDisplayType */.useActionsForDisplayType(displayType);
  const obj4 = require(14069) /* useActionsForDisplayType */;
  const formattedTotalForDisplayType = require(14069) /* useActionsForDisplayType */.useFormattedTotalForDisplayType(displayType);
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    obj1[0] = displayType;
    const items1 = [callback2(FamilyCenterActivitySectionHeader, obj1), substr.map((action) => callback(loadMore(14075), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj5[4] = tmp17(tmp(4298).Text, obj6);
        tmp17Result = tmp17(tmp(4844).PressableOpacity, obj5);
      }
      obj2[1] = tmp17Result;
