...
// Dependencies: [32, 19, 17, 7144, 21, 4380, 712, 7481, 7199, 14349, 4376, 11485, 1236, 2337, 14355, 4949, 2]
// Exports: default

// Module 14354 (FamilyCenterActivitySectionHeader)
...
import useActionsForDisplayType from "useActionsForDisplayType" /* 14349 */;
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    const formatToPlainStringResult = intl.formatToPlainString(loadMore(2337)["7dMmJY"], obj);
    obj = { style: null, children: null };
    obj[0] = loadMoreButton.container;
    obj1 = { displayType: null };
    obj1[0] = displayType;
    const items1 = [callback2(FamilyCenterActivitySectionHeader, obj1), substr.map((action) => callback(loadMore(14355), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj5[4] = tmp17(tmp(4376).Text, obj6);
        tmp17Result = tmp17(tmp(4949).PressableOpacity, obj5);
      }
      obj2[1] = tmp17Result;
