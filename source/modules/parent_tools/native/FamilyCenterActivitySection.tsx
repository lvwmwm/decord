...
// Dependencies: [32, 19, 17, 5302, 21, 4668, 712, 7660, 5368, 14319, 4739, 11143, 1236, 2336, 14325, 5438, 2]
// Exports: default

// Module 14324 (FamilyCenterActivitySectionHeader)
...
import useActionsForDisplayType from "useActionsForDisplayType" /* 14319 */;
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    const formatToPlainStringResult = intl.formatToPlainString(loadMore(2336)["7dMmJY"], obj);
    obj = { style: null, children: null };
    obj[0] = loadMoreButton.container;
    obj1 = { displayType: null };
    obj1[0] = displayType;
    const items1 = [callback2(FamilyCenterActivitySectionHeader, obj1), substr.map((action) => callback(loadMore(14325), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj5[4] = tmp17(tmp(4739).Text, obj6);
        tmp17Result = tmp17(tmp(5438).PressableOpacity, obj5);
      }
      obj2[1] = tmp17Result;
