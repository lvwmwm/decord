...
// Dependencies: [32, 19, 17, 7216, 21, 4445, 712, 8698, 7271, 14449, 4441, 11408, 1236, 2369, 14455, 5020, 2]
// Exports: default

// Module 14454 (FamilyCenterActivitySectionHeader)
...
import useActionsForDisplayType from "useActionsForDisplayType" /* 14449 */;
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    obj1[0] = displayType;
    const items1 = [callback2(FamilyCenterActivitySectionHeader, obj1), substr.map((action) => callback(loadMore(14455), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj5[4] = tmp17(tmp(4441).Text, obj6);
        tmp17Result = tmp17(tmp(5020).PressableOpacity, obj5);
      }
      obj2[1] = tmp17Result;
