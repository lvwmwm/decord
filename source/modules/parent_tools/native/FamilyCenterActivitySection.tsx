...
// Dependencies: [32, 19, 17, 7471, 21, 4481, 709, 8576, 7526, 14819, 4477, 11848, 1233, 2368, 14825, 5084, 2]
// Exports: default

// Module 14824 (FamilyCenterActivitySectionHeader)
...
import useActionsForDisplayType from "useActionsForDisplayType" /* 14819 */;
...
export default function FamilyCenterActivitySection(displayType) {
  ...
  if (0 === actionsForDisplayType.length) {
    ...
    obj1[0] = displayType;
    const items1 = [callback2(FamilyCenterActivitySectionHeader, obj1), substr.map((action) => callback(loadMore(14825), { action }, action.event_id)), ];
    if (substr.length >= actionTotalsForDisplayType) {
      ...
      if (familyCenterActions.isMoreLoading) {
        ...
        obj5[4] = tmp17(tmp(4477).Text, obj6);
        tmp17Result = tmp17(tmp(5084).PressableOpacity, obj5);
      }
      obj2[1] = tmp17Result;
