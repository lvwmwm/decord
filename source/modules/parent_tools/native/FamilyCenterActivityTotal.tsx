...
// Dependencies: [19, 17, 21, 4255, 712, 14018, 6946, 4251, 2]
// Exports: default

// Module 14019 (FamilyCenterActivityTotal)
import "noop";
...
export default function FamilyCenterActivityTotal(displayType) {
  ...
  let obj = require(14018) /* useActionsForDisplayType */;
  const actionTotalsForDisplayType = obj.useActionTotalsForDisplayType(displayType);
  const tmp = createCacheKey();
  let num = require(14018) /* useActionsForDisplayType */.useFormattedTotalForDisplayType(displayType);
  let str = "text-muted";
  if (actionTotalsForDisplayType > 0) {
    str = "text-brand";
  }
  const obj2 = require(14018) /* useActionsForDisplayType */;
  const activityTypeTextConfigs = require(6946) /* getEmptyActivityFormatter */.getActivityTypeTextConfigs();
