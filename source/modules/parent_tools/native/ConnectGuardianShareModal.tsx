...
// Dependencies: [19, 17, 5793, 21, 4189, 712, 1236, 2223, 3889, 4395, 11110, 647, 13911, 6783, 6784, 4598, 4185, 13912, 5637, 5143, 11122, 2]
// Exports: default

// Module 16296 (ConnectGuardianShareScreen)
...
function ConnectGuardianShareScreen() {
  ...
  const syncMessages = obj.useSyncMessages(getLinkCode(2223).messagesLoader);
  const callback = React.useCallback(() => {
    const intl = getLinkCode(1236).intl;
    getLinkCode(3889).presentFailedToast(intl.string(getLinkCode(1236).t.R0RpRX));
    let arr = callback(4395);
    arr = arr.pop();
  }, []);
  let obj1 = getLinkCode(11110);
  ...
  importDefault(13911)(importDefault(4395).pop);
  ...
  obj1[4] = intl.string(importDefault(2223).ITlV6p);
  const items2 = [callback(getLinkCode(4185).Text, obj1), ];
  obj2 = { style: tmp.body, variant: "text-sm/medium", color: "text-muted", children: null };
  const intl2 = getLinkCode(1236).intl;
  obj2[3] = intl2.format(importDefault(2223).F4GT2S, { link: "https://support.discord.com/hc/articles/14155060633623" });
  items2[1] = callback(getLinkCode(4185).Text, obj2);
  obj[1] = items2;
  const items3 = [callback2(getLinkCode(4598).Stack, obj), ];
  ...
  obj4[3] = intl3.string(importDefault(2223).pojgfk);
  const items4 = [callback(getLinkCode(4185).Text, obj4), ];
  if (null != stateFromStores) {
    if (null != stateFromStores1) {
      ...
      obj5[3] = getLinkCode;
      let tmp11Result = tmp11(tmp2(13912).ConnectGuardianCard, obj5);
    }
    const obj6 = { children: null };
    const obj7 = { children: null };
    items4[1] = tmp11Result;
    obj3[2] = items4;
    items3[1] = tmp12(getLinkCode(4598).Stack, obj3);
    obj[1] = items3;
    obj7[0] = tmp12(getLinkCode(4598).Stack, obj);
    obj6[0] = tmp11(getLinkCode(6784).ModalContent, obj7);
    return tmp11(getLinkCode(6783).ModalScreen, obj6);
  }
  const tmp9 = importDefault(13911);
  tmp11Result = tmp11(View, { style: tmp.loading, children: callback(getLinkCode(5637).ActivityIndicator, {}) });
}
...
export default function ConnectGuardianShareModal() {
  const memo = React.useMemo(() => {
    ...
    obj[1] = callback(5143).getHeaderBackButton(callback2(4395).pop);
  ...
  return callback(require(11122) /* Modal */.Modal, obj);
};
