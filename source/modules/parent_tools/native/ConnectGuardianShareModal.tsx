...
// Dependencies: [19, 17, 7470, 21, 4481, 709, 1233, 2368, 4193, 4731, 11848, 644, 14805, 8351, 8352, 4934, 4477, 14806, 5532, 5579, 11860, 2]
// Exports: default

// Module 17383 (ConnectGuardianShareScreen)
...
function ConnectGuardianShareScreen() {
  ...
  useOnNewPendingRequestDefault(_modDef4731.pop);
  ...
  const items2 = [callback(getLinkCode(4477).Text, obj1), ];
  ...
  items2[1] = callback(getLinkCode(4477).Text, obj2);
  obj[1] = items2;
  const items3 = [callback2(getLinkCode(4934).Stack, obj), ];
  ...
  const items4 = [callback(getLinkCode(4477).Text, obj4), ];
  if (null != stateFromStores) {
    if (null != stateFromStores1) {
      ...
      obj5[3] = getLinkCode;
      let tmp11Result = tmp11(tmp2(14806).ConnectGuardianCard, obj5);
    }
    const obj6 = { children: null };
    const obj7 = { children: null };
    items4[1] = tmp11Result;
    obj3[2] = items4;
    items3[1] = tmp12(getLinkCode(4934).Stack, obj3);
    obj[1] = items3;
    obj7[0] = tmp12(getLinkCode(4934).Stack, obj);
    obj6[0] = tmp11(getLinkCode(8352).ModalContent, obj7);
    return tmp11(getLinkCode(8351).ModalScreen, obj6);
  }
  const tmp9 = useOnNewPendingRequestDefault;
  tmp11Result = tmp11(View, { style: tmp.loading, children: callback(getLinkCode(5532).ActivityIndicator, {}) });
}
...
export default function ConnectGuardianShareModal() {
  const memo = React.useMemo(() => {
    ...
    obj = {
      headerShown: true,
      headerLeft: callback(5579).getHeaderBackButton(callback2(4731).pop),
      headerTitle() {
        return null;
      },
      render() {
