...
// Dependencies: [19, 17, 21, 4668, 712, 7660, 5265, 4096, 1236, 11143, 38, 5445, 5450, 14347, 8182, 4739, 2336, 14317, 11153, 6723, 4750, 6345, 11154, 2]
// Exports: default

// Module 14352 (FamilyCenterModalCancelScreen)
...
function FamilyCenterModalCancelScreen(otherUser) {
  ...
  const callback = React.useCallback(() => {
    let arr = cancelLinkRequest(5265);
    arr = arr.pop();
  }, []);
  ...
  cancelLinkRequest(38)(cancelLinkRequest(7660)(), "FamilyCenterCancelModal should only be rendered for parents.");
  obj = { children: null };
  obj = { children: null };
  obj1 = { style: tmp.header, children: null };
  const obj2 = { otherUser, iconSrc: null };
  const tmp2 = cancelLinkRequest(7660)();
  obj2[1] = cancelLinkRequest(8182);
  const items1 = [callback(cancelLinkRequest(14347), obj2), , ];
  const obj3 = { style: tmp.headerText, variant: "text-lg/bold", children: null };
  let intl = otherUser(1236).intl;
  obj3[2] = intl.string(cancelLinkRequest(2336).HynllX);
  items1[1] = callback(otherUser(4739).Text, obj3);
  items1[2] = callback(cancelLinkRequest(14317), { user: otherUser });
  obj1[1] = items1;
  obj[0] = callback2(View, obj1);
  const items2 = [callback(otherUser(5450).ModalContent, obj), ];
  ...
  obj6[3] = intl2.string(cancelLinkRequest(2336).mK40bk);
  obj6[4] = callback2;
  const items3 = [callback(otherUser(4750).Button, obj6), ];
  const obj7 = { variant: "tertiary", text: null, onPress: null };
  const intl3 = otherUser(1236).intl;
  obj7[1] = intl3.string(cancelLinkRequest(2336).czincX);
  obj7[2] = cancelLinkRequest(5265).pop;
  items3[1] = callback(otherUser(4750).Button, obj7);
  obj5[0] = items3;
  obj4[0] = callback2(otherUser(6723).ButtonGroup, obj5);
  items2[1] = callback(otherUser(11153).ModalFooter, obj4);
  obj[0] = items2;
