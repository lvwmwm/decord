...
// Dependencies: [19, 17, 21, 4342, 712, 7404, 4572, 4061, 1236, 11391, 38, 7873, 7874, 14247, 8093, 4338, 2303, 14217, 11401, 5212, 4777, 5331, 11403, 2]
// Exports: default

// Module 14252 (FamilyCenterModalCancelScreen)
...
function FamilyCenterModalCancelScreen(otherUser) {
  ...
  const callback = React.useCallback(() => {
    let arr = cancelLinkRequest(4572);
    arr = arr.pop();
  }, []);
  ...
  cancelLinkRequest(38)(cancelLinkRequest(7404)(), "FamilyCenterCancelModal should only be rendered for parents.");
  obj = { children: null };
  obj = { children: null };
  const obj1 = { style: tmp.header, children: null };
  const obj2 = { otherUser, iconSrc: null };
  const tmp2 = cancelLinkRequest(7404)();
  obj2[1] = cancelLinkRequest(8093);
  const items1 = [callback(cancelLinkRequest(14247), obj2), , ];
  ...
  items1[2] = callback(cancelLinkRequest(14217), { user: otherUser });
  obj1[1] = items1;
  obj[0] = callback2(View, obj1);
  const items2 = [callback(otherUser(7874).ModalContent, obj), ];
  ...
  obj6[4] = callback2;
  const items3 = [callback(otherUser(4777).Button, obj6), ];
  ...
  obj7[2] = cancelLinkRequest(4572).pop;
  items3[1] = callback(otherUser(4777).Button, obj7);
  obj5[0] = items3;
  obj4[0] = callback2(otherUser(5212).ButtonGroup, obj5);
  items2[1] = callback(otherUser(11401).ModalFooter, obj4);
  obj[0] = items2;
