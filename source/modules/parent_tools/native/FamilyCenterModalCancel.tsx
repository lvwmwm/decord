...
// Dependencies: [19, 17, 21, 4255, 712, 7248, 4461, 3955, 1236, 11237, 38, 7745, 7746, 14039, 7825, 4251, 2255, 14009, 11247, 5088, 4666, 5209, 11249, 2]
// Exports: default

// Module 14044 (FamilyCenterModalCancelScreen)
...
function FamilyCenterModalCancelScreen(otherUser) {
  ...
  const callback = React.useCallback(() => {
    let arr = cancelLinkRequest(4461);
    arr = arr.pop();
  }, []);
  ...
  cancelLinkRequest(38)(cancelLinkRequest(7248)(), "FamilyCenterCancelModal should only be rendered for parents.");
  obj = { children: null };
  obj = { children: null };
  const obj1 = { style: tmp.header, children: null };
  const obj2 = { otherUser, iconSrc: null };
  const tmp2 = cancelLinkRequest(7248)();
  obj2[1] = cancelLinkRequest(7825);
  const items1 = [callback(cancelLinkRequest(14039), obj2), , ];
  ...
  items1[2] = callback(cancelLinkRequest(14009), { user: otherUser });
  obj1[1] = items1;
  obj[0] = callback2(View, obj1);
  const items2 = [callback(otherUser(7746).ModalContent, obj), ];
  ...
  obj6[4] = callback2;
  const items3 = [callback(otherUser(4666).Button, obj6), ];
  ...
  obj7[2] = cancelLinkRequest(4461).pop;
  items3[1] = callback(otherUser(4666).Button, obj7);
  obj5[0] = items3;
  obj4[0] = callback2(otherUser(5088).ButtonGroup, obj5);
  items2[1] = callback(otherUser(11247).ModalFooter, obj4);
  obj[0] = items2;
