...
// Dependencies: [19, 17, 21, 4661, 712, 7622, 5260, 4093, 1236, 11104, 38, 5440, 5445, 14279, 8143, 4734, 2335, 14249, 11114, 6687, 4745, 6314, 11115, 2]
// Exports: default
...
function FamilyCenterModalCancelScreen(otherUser) {
  ...
  const tmp2 = cancelLinkRequest(7622)();
  obj2[1] = cancelLinkRequest(8143);
  const items1 = [callback(cancelLinkRequest(14279), obj2), , ];
  const obj3 = { style: tmp.headerText, variant: "text-lg/bold", children: null };
  let intl = otherUser(1236).intl;
  obj3[2] = intl.string(cancelLinkRequest(2335).HynllX);
  items1[1] = callback(otherUser(4734).Text, obj3);
  items1[2] = callback(cancelLinkRequest(14249), { user: otherUser });
  obj1[1] = items1;
  obj[0] = callback2(View, obj1);
  const items2 = [callback(otherUser(5445).ModalContent, obj), ];
  ...
  obj6[3] = intl2.string(cancelLinkRequest(2335).mK40bk);
  obj6[4] = callback2;
  const items3 = [callback(otherUser(4745).Button, obj6), ];
  const obj7 = { variant: "tertiary", text: null, onPress: null };
  const intl3 = otherUser(1236).intl;
  obj7[1] = intl3.string(cancelLinkRequest(2335).czincX);
  obj7[2] = cancelLinkRequest(5260).pop;
  items3[1] = callback(otherUser(4745).Button, obj7);
  obj5[0] = items3;
  obj4[0] = callback2(otherUser(6687).ButtonGroup, obj5);
  items2[1] = callback(otherUser(11114).ModalFooter, obj4);
  obj[0] = items2;
