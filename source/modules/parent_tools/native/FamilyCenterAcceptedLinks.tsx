...
// Dependencies: [19, 17, 6894, 21, 4255, 712, 7248, 7247, 11240, 1236, 2255, 4251, 14033, 14035, 4783, 4461, 14038, 1959, 1297, 14040, 2]
// Exports: default

// Module 14032 (FamilyCenterAcceptedLinkRow)
...
function FamilyCenterAcceptedLinkRow(otherUser) {
  ...
  if (undefined !== str) {
    ...
    obj = { name: null };
    obj[0] = str;
    obj[1] = intl.formatToPlainString(importDefault(2255).T7DUoU, obj);
    ...
    obj1[2] = importDefault(14040);
    obj[4] = closure_6(str(1297).Icon, obj1);
    obj[1] = closure_6(str(4783).PressableOpacity, obj);
    tmp4Result = tmp4(importDefault(14035), obj);
    const tmp5 = importDefault;
    const tmp7 = importDefault(14035);
  }
  return tmp4Result;
}
...
export default function FamilyCenterAcceptedLinks() {
  ...
  obj = { count: activeLinkUsers.length, max: importDefault(7248)() ? closure_4 : closure_5 };
  const tmp4 = importDefault(7248)();
  const intl = tmp5(1236).intl;
  const tmp5Result = require(11240) /* useAgeSpecificText */;
  const intl2 = tmp5(1236).intl;
  obj = { style: tmp.container, children: null };
  const ageSpecificText = tmp5Result.useAgeSpecificText(intl.formatToPlainString(importDefault(2255)["+tnO34"], obj), intl2.formatToPlainString(tmp2(2255)["pu6/U0"], obj));
  const items = [callback(require(4251) /* Text */.Text, { style: tmp.header, variant: "eyebrow", color: "text-default", children: ageSpecificText }), ];
  ...
  if (0 === activeLinkUsers.length) {
    ...
    obj3[1] = tmp10(tmp2(14033), obj4);
    let mapped = tmp10(tmp9, obj3);
    const tmp2Result = tmp2(14033);
  } else {
