...
// Function ID: 14156
// Name: memoResult1
// Dependencies: [19, 17, 1922, 6999, 7000, 676, 21, 4303, 712, 1297, 38, 647, 11, 4299, 4148, 7055, 5779, 8444, 1236, 2303, 14156, 14159, 14160, 2]
// Exports: default

// Module 14155 (memoResult1)
...
const memoResult = importAllResult.memo((action) => {
  ...
  if (null == stateFromStores) {
    ...
    const obj1 = { avatarStyle: null, user: null, guildId: "HermesInternal", disablePlaceholder: null, avatarDecoration: "fatal" };
    obj1[0] = tmp.avatar;
    ...
    const items2 = [callback(tmp6(4299).Text, obj3), ];
    const obj4 = { variant: "text-xs/medium", color: "channels-default", children: null };
    const _Date = Date;
    const date = new Date(extractTimestampResult);
    obj4[2] = tmp6(7055).formatUserActivityTimestamp(date.getTime(), value.timestampFormatter);
    items2[1] = callback(tmp6(4299).Text, obj4);
    obj2[1] = items2;
...
const memoResult1 = importAllResult.memo((action) => {
  ...
  if (undefined === stateFromStores) {
    ...
    let tmp6Result = tmp6(5779);
    obj[3] = tmp2(5779).GuildIconSizes.NORMAL;
    ...
    if (hasItem) {
      ...
      tmp6Result = tmp6(8444);
      obj4[2] = tmp6(8444).Sizes.SMALL;
    ...
    items2[1] = callback(tmp2(4299).Text, obj5);
    obj3[1] = items2;
    ...
    if (undefined !== stateFromStores.approximateMemberCount) {
      ...
      tmp11Result = tmp11(tmp2(4299).Text, obj6);
    }
...
export default function FamilyCenterActivityRow(action) {
  ...
  if (tmpResult2.isPurchase(action)) {
    ...
    if (null != purchaseInfo) {
      ...
      tmp14 = callback(importDefault(14156), obj);
    }
    return tmp14;
  } else {
    if (tmpResult3.isGift(action)) {
      ...
      if (null == giftInfo) {
        ...
        const giftRowDisplayInfo = tmp(14159).getGiftRowDisplayInfo(giftInfo);
        ...
        obj[5] = offeredAt;
        obj[6] = claimedAt;
        return callback(importDefault(14160), obj);
      }
    } else {
  ...
  tmpResult2 = require(7055) /* getEmptyActivityFormatter */;
};
