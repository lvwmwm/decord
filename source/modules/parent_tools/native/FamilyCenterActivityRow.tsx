...
// Function ID: 13927
// Name: memoResult1
// Dependencies: [19, 17, 1874, 5793, 5794, 676, 21, 4189, 712, 1297, 38, 647, 11, 4185, 4028, 5853, 5566, 9163, 1236, 2223, 13927, 13930, 13931, 2]
// Exports: default

// Module 13926 (memoResult1)
...
const memoResult = importAllResult.memo((action) => {
  ...
  if (null == stateFromStores) {
    ...
    const items2 = [callback(tmp6(4185).Text, obj3), ];
    const obj4 = { variant: "text-xs/medium", color: "channels-default", children: null };
    const _Date = Date;
    const date = new Date(extractTimestampResult);
    obj4[2] = tmp6(5853).formatUserActivityTimestamp(date.getTime(), value.timestampFormatter);
    items2[1] = callback(tmp6(4185).Text, obj4);
    obj2[1] = items2;
...
const memoResult1 = importAllResult.memo((action) => {
  ...
  if (undefined === stateFromStores) {
    ...
    let tmp6Result = tmp6(5566);
    obj[3] = tmp2(5566).GuildIconSizes.NORMAL;
    ...
    if (hasItem) {
      ...
      tmp6Result = tmp6(9163);
      obj4[2] = tmp6(9163).Sizes.SMALL;
    ...
    items2[1] = callback(tmp2(4185).Text, obj5);
    obj3[1] = items2;
    ...
    if (undefined !== stateFromStores.approximateMemberCount) {
      ...
      obj6[2] = intl.format(tmp6(2223)["5JmNgg"], obj7);
      tmp11Result = tmp11(tmp2(4185).Text, obj6);
    }
...
export default function FamilyCenterActivityRow(action) {
  ...
  if (tmpResult2.isPurchase(action)) {
    ...
    if (null != purchaseInfo) {
      ...
      tmp14 = callback(importDefault(13927), obj);
    }
    return tmp14;
  } else {
    if (tmpResult3.isGift(action)) {
      ...
      if (null == giftInfo) {
        ...
        const giftRowDisplayInfo = tmp(13930).getGiftRowDisplayInfo(giftInfo);
        ...
        obj[5] = offeredAt;
        obj[6] = claimedAt;
        return callback(importDefault(13931), obj);
      }
    } else {
  ...
  tmpResult2 = require(5853) /* getEmptyActivityFormatter */;
};
