...
// Function ID: 14474
// Name: memoResult1
// Dependencies: [19, 17, 1922, 7250, 7251, 676, 21, 4448, 712, 1297, 38, 647, 11, 4444, 4292, 7306, 5972, 7641, 1236, 2369, 14474, 14477, 14478, 2]
// Exports: default

// Module 14473 (memoResult1)
import _modDef38 from "module_38" /* 38 */;
import ThemesDefault from "Themes" /* 712 */;
import getEmptyActivityFormatter from "getEmptyActivityFormatter" /* 7306 */;
import FamilyCenterActivityPurchaseRowDefault from "FamilyCenterActivityPurchaseRow" /* 14474 */;
import FamilyCenterActivityGiftRowDefault from "FamilyCenterActivityGiftRow" /* 14478 */;
import importAllResult from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import closure_4 from "mergeGuildAvatar" /* 1922 */;
import closure_5 from "freshTeenActivityWithMap" /* 7250 */;
import { ACTION_TO_TEXT } from "items" /* 7251 */;
...
const memoResult = importAllResult.memo((action) => {
  ...
  if (null == stateFromStores) {
    ...
    const items2 = [callback(tmp6(4444).Text, obj3), ];
    const obj4 = { variant: "text-xs/medium", color: "channels-default", children: null };
    const _Date = Date;
    const date = new Date(extractTimestampResult);
    obj4[2] = tmp6(7306).formatUserActivityTimestamp(date.getTime(), value.timestampFormatter);
    items2[1] = callback(tmp6(4444).Text, obj4);
    obj2[1] = items2;
...
const memoResult1 = importAllResult.memo((action) => {
  ...
  if (undefined === stateFromStores) {
    ...
    let tmp6Result = tmp6(5972);
    obj[3] = tmp2(5972).GuildIconSizes.NORMAL;
    ...
    if (hasItem) {
      ...
      tmp6Result = tmp6(7641);
      obj4[2] = tmp6(7641).Sizes.SMALL;
    ...
    items2[1] = callback(tmp2(4444).Text, obj5);
    obj3[1] = items2;
    ...
    if (undefined !== stateFromStores.approximateMemberCount) {
      ...
      tmp11Result = tmp11(tmp2(4444).Text, obj6);
    }
  ...
  if (tmpResult2.isPurchase(action)) {
    ...
    if (tmpResult3.isGift(action)) {
      ...
      if (null == giftInfo) {
        ...
        const giftRowDisplayInfo = tmp(14477).getGiftRowDisplayInfo(giftInfo);
