...
// Function ID: 14222
// Name: memoResult1
// Dependencies: [19, 17, 1922, 7044, 7045, 676, 21, 4342, 712, 1297, 38, 647, 11, 4338, 4187, 7100, 5819, 8487, 1236, 2303, 14222, 14225, 14226, 2]
// Exports: default

// Module 14221 (memoResult1)
...
const memoResult = importAllResult.memo((action) => {
  ...
  if (null == stateFromStores) {
    ...
    const items2 = [callback(tmp6(4338).Text, obj3), ];
    const obj4 = { variant: "text-xs/medium", color: "channels-default", children: null };
    const _Date = Date;
    const date = new Date(extractTimestampResult);
    obj4[2] = tmp6(7100).formatUserActivityTimestamp(date.getTime(), value.timestampFormatter);
    items2[1] = callback(tmp6(4338).Text, obj4);
    obj2[1] = items2;
...
const memoResult1 = importAllResult.memo((action) => {
  ...
  if (undefined === stateFromStores) {
    ...
    let tmp6Result = tmp6(5819);
    obj[3] = tmp2(5819).GuildIconSizes.NORMAL;
    ...
    if (hasItem) {
      ...
      tmp6Result = tmp6(8487);
      obj4[2] = tmp6(8487).Sizes.SMALL;
    ...
    items2[1] = callback(tmp2(4338).Text, obj5);
    obj3[1] = items2;
    ...
    if (undefined !== stateFromStores.approximateMemberCount) {
      ...
      tmp11Result = tmp11(tmp2(4338).Text, obj6);
    }
...
export default function FamilyCenterActivityRow(action) {
  ...
  if (tmpResult2.isPurchase(action)) {
    ...
    if (null != purchaseInfo) {
      ...
      tmp14 = callback(importDefault(14222), obj);
    }
    return tmp14;
  } else {
    if (tmpResult3.isGift(action)) {
      ...
      if (null == giftInfo) {
        ...
        const giftRowDisplayInfo = tmp(14225).getGiftRowDisplayInfo(giftInfo);
        ...
        obj[5] = offeredAt;
        obj[6] = claimedAt;
        return callback(importDefault(14226), obj);
      }
    } else {
  ...
  tmpResult2 = require(7100) /* getEmptyActivityFormatter */;
};
