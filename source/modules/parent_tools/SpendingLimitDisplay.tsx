...
// Dependencies: [1340, 6890, 1876, 589, 13951, 5749, 5750, 1236, 2255, 2]
// Exports: useSpendingLimitDisplayState, useSpendingLimitFromUserSettings

// Module 14033 (getSpendingLimitDisplayState)
...
function getSpendingLimitDisplayState(amount) {
  if (null == amount) {
    ...
    const obj6 = require(5749) /* formatSingleCurrencyPrice */;
    const formatRateResult = obj6.formatRate(require(5749) /* formatSingleCurrencyPrice */.formatPrice(amount.amount, currency), SubscriptionIntervalTypes.MONTH, 1);
    if (arg1 >= amount.amount) {
      let obj = { kind: "spent", monthlyText: null };
      obj[1] = formatRateResult;
      return obj;
    } else {
      let num = tmp5(5750).CurrencyExponents[amount.currency];
      if (num == null) {
        num = 2;
      }
      ...
      if (diff <= 10 * 10 ** num) {
        ...
        obj1[0] = tmp5(5749).formatPrice(diff, currency);
        obj[2] = intl.formatToPlainString(importDefault(2255)["+Q+bU1"], obj1);
    ...
    const obj7 = require(5749) /* formatSingleCurrencyPrice */;
  }
}
...
export const useSpendingLimitFromUserSettings = function useSpendingLimitFromUserSettings() {
  ...
  return require(589) /* initialize */.useStateFromStores(items, () => {
    ...
    return tmp2;
  }, undefined, require(13951) /* spendingLimitEqual */.spendingLimitEqual);
};
