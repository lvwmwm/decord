...
// Function ID: 10672
// Name: Scope
// Dependencies: [19, 17, 21, 4444, 1236, 6171, 712, 4404, 4440, 5888, 10652, 9759, 9117, 2]
// Exports: default
...
function Scope(arg0) {
  ...
  if (isFake) {
    ...
    let tmp10Result = tmp10(tmp6(6171).CircleXIcon, obj);
  } else {
    obj1 = { color: null, size: "refresh_sm" };
    obj1[0] = ThemesDefault.colors.TEXT_MUTED;
    tmp10Result = tmp10(tmp6(4404).CircleCheckIcon, obj1);
  }
  obj[3] = tmp10Result;
  ...
  const items1 = [closure_4(tmp6(4440).Text, { variant: "text-md/normal", color: str3, children: text }), ];
  tmp10Result = null;
  if (null != error) {
    const obj3 = { variant: "text-xs/normal", children: null };
    obj3[1] = error;
    tmp10Result = tmp10(tmp6(4440).Text, obj3);
  }
...
export default function AuthorizeScopes(accountScopes) {
  ...
  if (0 === accountScopes.length) {
    ...
    let hasItem = integrationType === tmp4(9759).ApplicationIntegrationType.USER_INSTALL;
    if (hasItem) {
      hasItem = requestedScopes.includes(tmp4(9117).OAuth2Scopes.APPLICATIONS_COMMANDS);
    }
    ...
    const items = [callback(tmp4(4440).Text, obj), ];
