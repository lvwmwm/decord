...
// Function ID: 10577
// Name: Scope
// Dependencies: [19, 17, 21, 4344, 1236, 6037, 712, 4305, 4340, 5755, 10557, 9587, 8947, 2]
// Exports: default
...
function Scope(arg0) {
  ...
  if (isFake) {
    ...
    let tmp10Result = tmp10(tmp6(6037).CircleXIcon, obj);
  } else {
    const obj1 = { color: null, size: "refresh_sm" };
    obj1[0] = importDefault(712).colors.TEXT_MUTED;
    tmp10Result = tmp10(tmp6(4305).CircleCheckIcon, obj1);
  }
  obj[3] = tmp10Result;
  ...
  const items1 = [closure_4(tmp6(4340).Text, { variant: "text-md/normal", color: str3, children: text }), ];
  tmp10Result = null;
  if (null != error) {
    const obj3 = { variant: "text-xs/normal", children: null };
    obj3[1] = error;
    tmp10Result = tmp10(tmp6(4340).Text, obj3);
  }
...
export default function AuthorizeScopes(accountScopes) {
  ...
  if (0 === accountScopes.length) {
    ...
    let hasItem = integrationType === tmp4(9587).ApplicationIntegrationType.USER_INSTALL;
    if (hasItem) {
      hasItem = requestedScopes.includes(tmp4(8947).OAuth2Scopes.APPLICATIONS_COMMANDS);
    }
    ...
    const items = [callback(tmp4(4340).Text, obj), ];
