...
// Function ID: 9302
// Name: Scope
// Dependencies: [19, 17, 21, 4446, 1236, 6190, 712, 4406, 4442, 5906, 9227, 8379, 8556, 2]
// Exports: default
...
function Scope(arg0) {
  ...
  if (isFake) {
    ...
    let tmp10Result = tmp10(tmp6(6190).CircleXIcon, obj);
  } else {
    obj1 = { color: null, size: "refresh_sm" };
    obj1[0] = ThemesDefault.colors.TEXT_MUTED;
    tmp10Result = tmp10(tmp6(4406).CircleCheckIcon, obj1);
  }
  obj[3] = tmp10Result;
  ...
  const items1 = [closure_4(tmp6(4442).Text, { variant: "text-md/normal", color: str3, children: text }), ];
  tmp10Result = null;
  if (null != error) {
    const obj3 = { variant: "text-xs/normal", children: null };
    obj3[1] = error;
    tmp10Result = tmp10(tmp6(4442).Text, obj3);
  }
...
export default function AuthorizeScopes(accountScopes) {
  ...
  if (0 === accountScopes.length) {
    ...
    let hasItem = integrationType === tmp4(8379).ApplicationIntegrationType.USER_INSTALL;
    if (hasItem) {
      hasItem = requestedScopes.includes(tmp4(8556).OAuth2Scopes.APPLICATIONS_COMMANDS);
    }
    ...
    const items = [callback(tmp4(4442).Text, obj), ];
