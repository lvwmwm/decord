...
function Scope(arg0) {
  ...
  if (isFake) {
    ...
    let tmp10Result = tmp10(tmp6(6039).CircleXIcon, obj);
  } else {
...
export default function AuthorizeScopes(accountScopes) {
  ...
  if (0 === accountScopes.length) {
    ...
    let hasItem = integrationType === tmp4(9591).ApplicationIntegrationType.USER_INSTALL;
    if (hasItem) {
      hasItem = requestedScopes.includes(tmp4(8951).OAuth2Scopes.APPLICATIONS_COMMANDS);
    }
