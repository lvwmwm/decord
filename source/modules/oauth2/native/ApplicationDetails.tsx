...
// Dependencies: [19, 17, 21, 4302, 712, 10446, 10469, 10471, 11, 10444, 4246, 1236, 4821, 10448, 10473, 4266, 8842, 10475, 10166, 4298, 2]
// Exports: default

// Module 10468 (ApplicationDetailsEntry)
import "Text";
...
export default function ApplicationDetails(arg0) {
  ...
  tmp18Result = null;
  if (scopes.includes(require(8842) /* set */.OAuth2Scopes.BOT)) {
    tmp18Result = null;
    if (null != approximateGuildCount) {
      const obj5 = { iconComponent: null, text: null };
      obj5[0] = tmp5(10475).RobotIcon;
  ...
  items[4] = tmp18Result;
  const tmp5Result = require(10448) /* getApplicationDetailsText */;
  items[5] = callback(ApplicationDetailsEntry, { iconComponent: require(10166) /* ShieldIcon */.ShieldIcon, text: securityMessage });
  let mapped = null;
  if (null != disclosures) {
    mapped = disclosures.map((toFixed) => {
      let obj = callback(10446);
      const textForDisclosure = obj.getTextForDisclosure(toFixed);
      if (callback(10446).ApplicationDisclosure.IP_LOCATION === toFixed) {
        obj = { iconComponent: null };
        obj[0] = tmp(10469).GlobeEarthIcon;
        let tmp4 = obj;
      } else {
        tmp4 = null;
        if (tmp(10446).ApplicationDisclosure.DISPLAYS_ADVERTISEMENTS === toFixed) {
          obj = { iconComponent: null };
          obj[0] = tmp(10471).EmbedIcon;
          tmp4 = obj;
        }
      }
      let tmp5 = null;
