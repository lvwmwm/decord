...
// Dependencies: [19, 17, 21, 4446, 712, 9229, 9308, 9310, 11, 9227, 4389, 1236, 5009, 9231, 9312, 4409, 8556, 9314, 8373, 4442, 2]
// Exports: default

// Module 9307 (ApplicationDetailsEntry)
...
import items2 from "items" /* 9227 */;
import getApplicationDetailsText from "getApplicationDetailsText" /* 9231 */;
...
export default function ApplicationDetails(arg0) {
  ...
  if (null != disclosures) {
    mapped = disclosures.map((toFixed) => {
      let obj = callback(9229);
      const textForDisclosure = obj.getTextForDisclosure(toFixed);
      if (callback(9229).ApplicationDisclosure.IP_LOCATION === toFixed) {
        obj = { iconComponent: null };
        obj[0] = tmp(9308).GlobeEarthIcon;
        let tmp4 = obj;
      } else {
        tmp4 = null;
        if (tmp(9229).ApplicationDisclosure.DISPLAYS_ADVERTISEMENTS === toFixed) {
          obj = { iconComponent: null };
          obj[0] = tmp(9310).EmbedIcon;
          tmp4 = obj;
        }
      }
      let tmp5 = null;
