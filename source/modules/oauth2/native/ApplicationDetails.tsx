...
// Dependencies: [19, 17, 21, 4255, 712, 10399, 10422, 10424, 11, 10397, 4199, 1236, 4774, 10401, 10426, 4219, 8796, 10428, 10119, 4251, 2]
// Exports: default

// Module 10421 (ApplicationDetailsEntry)
import "Text";
...
export default function ApplicationDetails(arg0) {
  ...
  tmp18Result = null;
  if (scopes.includes(require(8796) /* set */.OAuth2Scopes.BOT)) {
    tmp18Result = null;
    if (null != approximateGuildCount) {
      const obj5 = { iconComponent: null, text: null };
      obj5[0] = tmp5(10428).RobotIcon;
  ...
  items[4] = tmp18Result;
  const tmp5Result = require(10401) /* getApplicationDetailsText */;
  items[5] = callback(ApplicationDetailsEntry, { iconComponent: require(10119) /* ShieldIcon */.ShieldIcon, text: securityMessage });
  let mapped = null;
  if (null != disclosures) {
    mapped = disclosures.map((toFixed) => {
      let obj = callback(10399);
      const textForDisclosure = obj.getTextForDisclosure(toFixed);
      if (callback(10399).ApplicationDisclosure.IP_LOCATION === toFixed) {
        obj = { iconComponent: null };
        obj[0] = tmp(10422).GlobeEarthIcon;
        let tmp4 = obj;
      } else {
        tmp4 = null;
        if (tmp(10399).ApplicationDisclosure.DISPLAYS_ADVERTISEMENTS === toFixed) {
          obj = { iconComponent: null };
          obj[0] = tmp(10424).EmbedIcon;
          tmp4 = obj;
        }
      }
      let tmp5 = null;
