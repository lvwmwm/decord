...
// Dependencies: [19, 17, 21, 4285, 712, 10430, 10453, 10455, 11, 10428, 4229, 1236, 4804, 10432, 10457, 4249, 8825, 10459, 10150, 4281, 2]
// Exports: default

// Module 10452 (ApplicationDetailsEntry)
import "Text";
...
export default function ApplicationDetails(arg0) {
  ...
  tmp18Result = null;
  if (scopes.includes(require(8825) /* set */.OAuth2Scopes.BOT)) {
    tmp18Result = null;
    if (null != approximateGuildCount) {
      const obj5 = { iconComponent: null, text: null };
      obj5[0] = tmp5(10459).RobotIcon;
  ...
  items[4] = tmp18Result;
  const tmp5Result = require(10432) /* getApplicationDetailsText */;
  items[5] = callback(ApplicationDetailsEntry, { iconComponent: require(10150) /* ShieldIcon */.ShieldIcon, text: securityMessage });
  let mapped = null;
  if (null != disclosures) {
    mapped = disclosures.map((toFixed) => {
      let obj = callback(10430);
      const textForDisclosure = obj.getTextForDisclosure(toFixed);
      if (callback(10430).ApplicationDisclosure.IP_LOCATION === toFixed) {
        obj = { iconComponent: null };
        obj[0] = tmp(10453).GlobeEarthIcon;
        let tmp4 = obj;
      } else {
        tmp4 = null;
        if (tmp(10430).ApplicationDisclosure.DISPLAYS_ADVERTISEMENTS === toFixed) {
          obj = { iconComponent: null };
          obj[0] = tmp(10455).EmbedIcon;
          tmp4 = obj;
        }
      }
      let tmp5 = null;
