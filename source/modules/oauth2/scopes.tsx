...
export const getScopeNames = function getScopeNames(nextResult, c2) {
  ...
  if (require(8951) /* set */.OAuth2Scopes.IDENTIFY === nextResult) {
    ...
    if (!c2.includes(tmp(8951).OAuth2Scopes.APPLICATION_IDENTITIES_WRITE)) {
      const intl11 = tmp(1236).intl;
      items45.push(intl11.string(tmp(1236).t["1zioRF"]));
    }
    return items45;
  } else if (tmp(8951).OAuth2Scopes.SDK_SOCIAL_LAYER === nextResult) {
...
export const isSocialLayerUmbrellaScope = function isSocialLayerUmbrellaScope(arg0) {
  return arg0 === require(8951) /* set */.OAuth2Scopes.SDK_SOCIAL_LAYER_PRESENCE || arg0 === require(8951) /* set */.OAuth2Scopes.SDK_SOCIAL_LAYER;
};
export const getSecurityMessage = function getSecurityMessage(scopes) {
  if (!scopes.includes(require(8951) /* set */.OAuth2Scopes.DM_CHANNELS_MESSAGES_WRITE)) {
    if (!scopes.includes(tmp(8951).OAuth2Scopes.SDK_SOCIAL_LAYER)) {
      if (!scopes.includes(tmp(8951).OAuth2Scopes.MESSAGES_READ)) {
        if (!scopes.includes(tmp(8951).OAuth2Scopes.DM_CHANNELS_MESSAGES_READ)) {
          const intl = tmp(1236).intl;
