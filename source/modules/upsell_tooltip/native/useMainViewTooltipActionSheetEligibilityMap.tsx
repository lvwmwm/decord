...
// Dependencies: [32, 19, 8123, 16854, 10594, 1339, 1380, 673, 1923, 682, 586, 16871, 1623, 7991, 7381, 16872, 13280, 10671, 10670, 13498, 8128, 8115, 4301, 1372, 16873, 16868, 16851, 13394, 16862, 9106, 11899, 2]
// Exports: useMainViewTooltipActionSheetMap

// Module 16870 (PREMIUM_TIER_2_LIKELIHOOD_TRIAL_ID)
import apexExperimentDefault from "apexExperiment" /* 16871 */;
import useGiftingPromotionAssetsReadyDefault from "useGiftingPromotionAssetsReady" /* 16873 */;
import closure_3 from "_slicedToArray" /* 32 */;
import closure_4 from "noop" /* 19 */;
import closure_5 from "initialize" /* 8123 */;
import closure_6 from "onInitializeSync" /* 16854 */;
...
export const useMainViewTooltipActionSheetMap = function useMainViewTooltipActionSheetMap() {
  ...
  const premiumDiscountOffer = isEligibleToShowGiftingBadgeCoachmark(7991).usePremiumDiscountOffer();
  const tmpResult1 = isEligibleToShowGiftingBadgeCoachmark(7991);
  const premiumTrialOffer = isEligibleToShowGiftingBadgeCoachmark(7381).usePremiumTrialOffer();
  const PremiumTrialOfferActionSheetKillSwitchExperiment = tmp(16872).PremiumTrialOfferActionSheetKillSwitchExperiment;
  const tmpResult2 = isEligibleToShowGiftingBadgeCoachmark(7381);
  const promotionMarketingComponent = isEligibleToShowGiftingBadgeCoachmark(13280).usePromotionMarketingComponent(tmp(10671).MarketingComponentType.MOBILE_BOTTOM_SHEET);
  let oneofKind;
  ...
  const tmpResult3 = isEligibleToShowGiftingBadgeCoachmark(13280);
  ...
  const stateFromStores3 = isEligibleToShowGiftingBadgeCoachmark(586).useStateFromStores(items4, () => {
    const marketingComponentByType = store.getMarketingComponentByType(isEligibleToShowGiftingBadgeCoachmark(10671).MarketingComponentType.GIFT_ICON_COACHMARK);
  ...
  const stateFromStores4 = isEligibleToShowGiftingBadgeCoachmark(586).useStateFromStores(items5, () => {
    const marketingComponentByType = store.getMarketingComponentByType(isEligibleToShowGiftingBadgeCoachmark(10671).MarketingComponentType.GIFT_REMINDER_COACHMARK);
    let prop = null;
    if (null != marketingComponentByType) {
      prop = null;
  ...
  const GiftPromotionReminderExperiment = tmp(10670).GiftPromotionReminderExperiment;
  const tmpResult6 = isEligibleToShowGiftingBadgeCoachmark(586);
  isEligibleToShowGiftingBadgeCoachmark = isEligibleToShowGiftingBadgeCoachmark(13498).useIsEligibleToShowGiftingBadgeCoachmark({ platform: "native", location: tmp5 });
  ...
  const tmpResult7 = isEligibleToShowGiftingBadgeCoachmark(13498);
  const items7 = [closure_5];
  let isDismissed = null != stateFromStores2;
  const stateFromStores5 = isEligibleToShowGiftingBadgeCoachmark(586).useStateFromStores(items7, () => null != badgeById.getBadgeById(isEligibleToShowGiftingBadgeCoachmark(8115).BadgeId.GIFTING));
  if (isDismissed) {
    isDismissed = tmp(4301).UNSAFE_isSnowflakeBoundDismissibleContentDismissed(tmp(1372).DismissibleContent.GIFTING_PROMOTION_MOBILE_FIRST_TIME_HALFSHEET, stateFromStores2).isDismissed;
    const tmpResult9 = tmp(4301);
  }
  let isDismissed2 = null != stateFromStores2;
  if (isDismissed2) {
    isDismissed2 = tmp(4301).UNSAFE_isSnowflakeBoundDismissibleContentDismissed(tmp(1372).DismissibleContent.GIFTING_PROMOTION_REMINDER, stateFromStores2).isDismissed;
    const tmpResult10 = tmp(4301);
  }
  let tmp22 = null;
  ...
  const tmpResult11 = isEligibleToShowGiftingBadgeCoachmark(16868);
  const shouldShowRobloxConnectionCoachmark = isEligibleToShowGiftingBadgeCoachmark(16851).useShouldShowRobloxConnectionCoachmark();
  const tmpResult12 = isEligibleToShowGiftingBadgeCoachmark(16851);
  const items8 = [, ];
  ({ LEAGUE_OF_LEGENDS: arr9[0], RIOT_GAMES: arr9[1] } = PlatformTypes);
  const isEligibleForBogoPromotion = isEligibleToShowGiftingBadgeCoachmark(13394).useIsEligibleForBogoPromotion();
  const tmp27 = PlatformTypes;
  const tmpResult13 = isEligibleToShowGiftingBadgeCoachmark(13394);
  const shouldShowConnectionDeprecationBottomSheet = isEligibleToShowGiftingBadgeCoachmark(16862).useShouldShowConnectionDeprecationBottomSheet({ deprecatedPlatformTypes: items8 });
  const tmpResult14 = isEligibleToShowGiftingBadgeCoachmark(16862);
  const items9 = [PlatformTypes.BATTLENET];
  const shouldShowConnectionDeprecationBottomSheet1 = isEligibleToShowGiftingBadgeCoachmark(16862).useShouldShowConnectionDeprecationBottomSheet({ deprecatedPlatformTypes: items9 });
  const tmpResult15 = isEligibleToShowGiftingBadgeCoachmark(16862);
  const isDisplayNameStylesFlywheelSettersEnabled = isEligibleToShowGiftingBadgeCoachmark(9106).useIsDisplayNameStylesFlywheelSettersEnabled(tmp5);
  isEligibleToShowGiftingBadgeCoachmark(11899);
  obj = {};
  if (stateFromStores) {
