...
// Dependencies: [19, 17, 21, 8816, 8827, 5607, 9034, 4478, 9038, 8088, 1233, 4445, 9022, 2008, 5630, 5632, 6000, 2]
// Exports: default

// Module 9039 (GuildProfileGameRow)
import useOpenGameProfileModalDefault from "useOpenGameProfileModal" /* 8816 */;
import stylesDefault from "styles" /* 9034 */;
...
function GuildProfileGameRow(activityLevel) {
  ...
  let _require;
  let obj = { gameId: game.id, source: _require(8827).GameProfileSources.GuildProfileGames, trackEntryPointImpression: true };
  ...
  obj[3] = fn;
  return jsx(_require(5607).TableRow, { icon: jsx(stylesDefault, { game, activityLevel: activityLevel.activityLevel }), label: game.name, arrow: tmp3, onPress: null });
}
...
export default function GuildProfileGamesActionSheet(profile) {
  ...
  const tmp = callback();
  obj1[1] = jsx(id(6000).TableRowGroup, { title: str, hasIcons: true, children: allGuildProfileGames.map((game) => closure_1_5(closure_1_6, { game, activityLevel: gameActivity[game.id] }, game.id)) });
