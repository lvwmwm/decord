...
// Dependencies: [19, 17, 21, 8605, 8616, 5560, 9125, 4481, 9129, 8101, 1233, 4448, 9114, 2008, 7090, 6567, 5642, 2]
// Exports: default

// Module 9130 (GuildProfileGameRow)
import useOpenGameProfileModalDefault from "useOpenGameProfileModal" /* 8605 */;
import stylesDefault from "styles" /* 9125 */;
...
function GuildProfileGameRow(activityLevel) {
  ...
  let _require;
  let obj = { gameId: game.id, source: _require(8616).GameProfileSources.GuildProfileGames, trackEntryPointImpression: true };
  ...
  obj[3] = fn;
  return jsx(_require(5560).TableRow, { icon: jsx(stylesDefault, { game, activityLevel: activityLevel.activityLevel }), label: game.name, arrow: tmp3, onPress: null });
}
...
export default function GuildProfileGamesActionSheet(profile) {
  ...
  const tmp = callback();
  obj1[1] = jsx(id(5642).TableRowGroup, { title: str, hasIcons: true, children: allGuildProfileGames.map((game) => closure_1_5(closure_1_6, { game, activityLevel: gameActivity[game.id] }, game.id)) });
  obj[0] = <View style={callback().container}>{null}</View>;
  obj[4] = jsx(id(6567).BottomSheetScrollView, { children: null });
  return jsx(id(7090).BottomSheet, { children: null });
};
