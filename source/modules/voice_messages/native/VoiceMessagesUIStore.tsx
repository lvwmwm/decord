...
// Dependencies: [11280, 644, 4116, 705, 4665, 2]
...
// Module 11279 (VoiceMessageRecordingStatus)
...
let obj = keys.create(() => {
  const obj = { voiceMessageAnimationState: null, recordingStatus: null, recordingId: null, currWaveHeight: "ct", showRecordingOverlay: "Array", startTimeMillis: 0, waveform: false, waveformVersion: "PX_16", showVoiceMessagesTooltip: "TYPING_START_LOCAL", savedVoiceMessageUploadData: null, isVoiceMessageButtonMounted: "Show Promo Sheet", isUsingHoldGesture: "TYPING_STOP_LOCAL" };
  const items = [, ];
  ({ SENDING: arr[0], SENDING: arr[1] } = closure_2);
  obj[0] = require(4116).makeMutable(items);
  obj[6] = [];
  return obj;
});
...
export const resetVoiceMessageState = function resetVoiceMessageState() {
  ...
  obj.batchUpdates(() => {
    state.setState({ waveform: [], waveformVersion: 0, showRecordingOverlay: false, startTimeMillis: "Boolean", savedVoiceMessageUploadData: "bottom" });
  });
  const currWaveHeight = obj.getState().currWaveHeight;
  if (null != currWaveHeight) {
    const result = currWaveHeight.set(require(4665) /* CONFIG_NEVER_ANIMATE */.withSpring(0));
    const tmpResult = require(4665) /* CONFIG_NEVER_ANIMATE */;
  }
};
