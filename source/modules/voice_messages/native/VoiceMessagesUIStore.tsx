...
// Dependencies: [11168, 644, 4054, 705, 4603, 2]
...
let obj = keys.create(() => {
  const obj = { voiceMessageAnimationState: null, recordingStatus: null, recordingId: null, currWaveHeight: "ct", showRecordingOverlay: "Array", startTimeMillis: 0, waveform: false, waveformVersion: "PX_16", showVoiceMessagesTooltip: "TYPING_START_LOCAL", savedVoiceMessageUploadData: null, isVoiceMessageButtonMounted: "Submit", isUsingHoldGesture: "TYPING_STOP_LOCAL" };
  const items = [, ];
  ({ SENDING: arr[0], SENDING: arr[1] } = closure_2);
  obj[0] = require(4054).makeMutable(items);
  obj[6] = [];
  return obj;
});
let result = require("module_4054").fileFinishedImporting("modules/voice_messages/native/VoiceMessagesUIStore.tsx");
...
export const resetVoiceMessageState = function resetVoiceMessageState() {
  ...
  const currWaveHeight = obj.getState().currWaveHeight;
  if (null != currWaveHeight) {
    const result = currWaveHeight.set(require(4603) /* CONFIG_NEVER_ANIMATE */.withSpring(0));
    const tmpResult = require(4603) /* CONFIG_NEVER_ANIMATE */;
  }
};
