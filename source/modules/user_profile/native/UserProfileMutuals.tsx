...
// Dependencies: [19, 17, 8524, 5969, 21, 4448, 8526, 9189, 9191, 4415, 9199, 2009, 8859, 6154, 5036, 9388, 1297, 4444, 9222, 9231, 5972, 9194, 2]
// Exports: default

// Module 12572 (UserProfileMutuals)
import noopAll from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import { UserProfileSections } from "USER_PROFILE_TOOLTIP_DELAY" /* 8524 */;
import { DIVIDER_DOT } from "ARBITRARY_LARGE_OFFSET" /* 5969 */;
...
export default function UserProfileMutuals(user) {
  ...
  ({ mutualFriends, mutualGuilds } = guildId(9189)(user));
  if (guildId(9191)(user)) {
    ...
    if (tmp7) {
      ...
      const items = [callback(tmp2(9388).AvatarPile, obj1), ];
      const obj2 = { variant: null, color: null, children: null };
      obj2[0] = c8;
      obj2[1] = c9;
      obj2[2] = tmp5(9222)(mutualFriends.length);
      items[1] = callback(tmp2(4444).Text, obj2);
      obj[3] = items;
      tmp9Result = tmp9(tmp2(5036).PressableOpacity, obj);
    }
    ...
    if (tmp15) {
      ...
      obj3[1] = c9;
      obj3[4] = DIVIDER_DOT;
      tmp15 = callback(tmp2(4444).Text, obj3);
    }
    items1[1] = tmp15;
    tmp9Result = null;
    if (tmp8) {
      ...
      if (!tmp7) {
        ...
        obj5[0] = tmp2(5972).GuildIconSizes.XXSMALL;
        ...
        tmp21 = callback(tmp2(9231).GuildIconPile, obj5);
      }
      const items2 = [tmp21, ];
      const obj6 = { variant: null, color: null, children: null };
      obj6[0] = c8;
      obj6[1] = c9;
      obj6[2] = tmp5(9194)(mutualGuilds.length);
      items2[1] = callback(tmp2(4444).Text, obj6);
      obj4[3] = items2;
      tmp9Result = tmp9(tmp2(5036).PressableOpacity, obj4);
    }
    items1[2] = tmp9Result;
    obj[1] = items1;
