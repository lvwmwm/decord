...
// Dependencies: [19, 17, 8502, 5966, 21, 4446, 8504, 9167, 9169, 4413, 9177, 2010, 8836, 6151, 5033, 9366, 1297, 4442, 9200, 9209, 5969, 9172, 2]
// Exports: default

// Module 12538 (UserProfileMutuals)
import noopAll from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import { UserProfileSections } from "USER_PROFILE_TOOLTIP_DELAY" /* 8502 */;
import { DIVIDER_DOT } from "ARBITRARY_LARGE_OFFSET" /* 5966 */;
...
export default function UserProfileMutuals(user) {
  ...
  ({ mutualFriends, mutualGuilds } = guildId(9167)(user));
  if (guildId(9169)(user)) {
    ...
    if (tmp7) {
      ...
      const items = [callback(tmp2(9366).AvatarPile, obj1), ];
      const obj2 = { variant: null, color: null, children: null };
      obj2[0] = c8;
      obj2[1] = c9;
      obj2[2] = tmp5(9200)(mutualFriends.length);
      items[1] = callback(tmp2(4442).Text, obj2);
      obj[3] = items;
      tmp9Result = tmp9(tmp2(5033).PressableOpacity, obj);
    }
    ...
    if (tmp15) {
      ...
      obj3[1] = c9;
      obj3[4] = DIVIDER_DOT;
      tmp15 = callback(tmp2(4442).Text, obj3);
    }
    items1[1] = tmp15;
    tmp9Result = null;
    if (tmp8) {
      ...
      if (!tmp7) {
        ...
        obj5[0] = tmp2(5969).GuildIconSizes.XXSMALL;
        ...
        tmp21 = callback(tmp2(9209).GuildIconPile, obj5);
      }
      const items2 = [tmp21, ];
      const obj6 = { variant: null, color: null, children: null };
      obj6[0] = c8;
      obj6[1] = c9;
      obj6[2] = tmp5(9172)(mutualGuilds.length);
      items2[1] = callback(tmp2(4442).Text, obj6);
      obj4[3] = items2;
      tmp9Result = tmp9(tmp2(5033).PressableOpacity, obj4);
    }
    items1[2] = tmp9Result;
    obj[1] = items1;
