// Module ID: 8774
// Function ID: 8775
// Dependencies: [19, 17, 5716, 5397, 21, 8775, 8728, 4491, 4140, 8776, 8765, 8777, 1609, 8763, 8772, 5696, 4001, 8778, 4162, 8782, 9404, 1236, 9465, 2]
...
const memoResult = require("OpenableUserProfileAvatar").memo(function NonUserBotProfileContent(scrollPosition) {
  ...
  let obj = trackUserProfileAction(8728);
  trackUserProfileAction = obj.useUserProfileAnalyticsContext().trackUserProfileAction;
  let obj1 = importDefault(4491);
  let guild_id;
  if (channel != null) {
  ...
  const tmp9 = importDefault(8765)(ACTION_SHEET_MAX_WIDTH);
  const tmp8 = importDefault(8776)(displayProfile);
  ({ bannerAnimatedStyle, bannerImageAnimatedStyle, contentAnimatedStyle, blurAnimatedProps, showBlur } = importDefault(8777)({ scrollPosition: scrollPosition.scrollPosition, bannerHeight: tmp9 }));
  const tmp10 = importDefault(8777)({ scrollPosition: scrollPosition.scrollPosition, bannerHeight: tmp9 });
  ({ theme, primaryColor, secondaryColor } = importDefault(8763)({ user, displayProfile }));
  const tmp11 = importDefault(8763)({ user, displayProfile });
  const userProfileColors = trackUserProfileAction(8772).useUserProfileColors({ theme, primaryColor, secondaryColor });
  ...
  if (null == user) {
    ...
    obj[6] = showBlur;
    const items = [callback(tmp(8778), obj), ];
    ...
    items4[1] = obj9;
    obj8[3] = items4;
    obj7[1] = callback(tmp(9465), obj8);
    items3[1] = callback(View, obj7);
    obj2[1] = items3;
    items1[1] = closure_8(View, obj2);
    obj[1] = items1;
    items[1] = closure_8(tmp(4162).View, obj);
    obj6[0] = items;
    return closure_8(closure_9, obj6);
  }
  const tmp4Result = trackUserProfileAction(8772);
});
