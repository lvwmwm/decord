...
// Dependencies: [5, 19, 17, 8091, 673, 21, 4481, 709, 7102, 4139, 4448, 5099, 14518, 6929, 7138, 7089, 1233, 8592, 5642, 5560, 8523, 14519, 586, 8100, 8117, 8179, 685, 8095, 14520, 14522, 4477, 1296, 14527, 2]
// Exports: default

// Module 14517 (ChangeBannerColorRow)
...
function ChangeBannerColorRow(user) {
  ...
  let tmp2Result = tmp2(8179);
  tmp2Result = tmp2(685);
  const memoizedImageSourceResult = tmp2Result.memoizedImageSource(pendingAvatarSrc);
  const rgb2intResult = tmp2Result.rgb2int(_require(8179).useDominantColorFromImage(pendingAvatarSrc, memoizedImageSourceResult));
  ...
  dependencyMap = React.useCallback((arg0) => {
    ...
    _undefined(8095).setPendingChanges({ accentColor: tmp });
  }, items1);
  obj = { label: null, trailing: null, onPress: null };
  obj1 = { style: tmp.label, text: null };
  const intl = tmp2(1233).intl;
  obj1[1] = intl.string(_require(1233).t.xzNfPz);
  obj[0] = callback(_require(8523).FormLabel, obj1);
  const obj2 = { style: tmp.selectedColor, children: null };
  const items2 = [callback(pendingAccentColor(14522), { style: tmp.bannerColor, color: pendingAccentColor }), , ];
  ...
  const tmp2Result1 = _require(8179);
  obj4[3] = _require(685).int2hex(pendingAccentColor);
  items2[1] = callback(_require(4477).Text, obj4);
  const tmp2Result2 = _require(685);
  items2[2] = callback(_require(1296).Icon, { style: tmp.rowArrow, size: _require(1296).Icon.Sizes.CUSTOM, source: pendingAccentColor(14527) });
  obj2[1] = items2;
  obj[1] = callback2(View, obj2);
  obj[2] = function handleChangeColor() {
    pendingAccentColor(14520)({ color: pendingAccentColor, onSelect: dependencyMap });
  };
  return callback(_require(5560).TableRow, obj);
}
...
export default function ChangeBannerActionSheet(isTryItOut) {
        ...
        try {
          ...
          if (0 === originalMd5) {
            if (arg0 === 1) {
              ...
              let obj3 = closure_1_1(4448);
              obj3.hideActionSheet();
              originalMd5 = 1;
              c3 = 1;
              obj1 = { value: null, done: false };
              obj1[0] = closure_1_1(5099).openImagePicker(closure_1_8);
              return obj1;
            }
          } else if (arg0 === 1) {
            c3 = 3;
            ...
            if (null != base64) {
              ...
              obj3 = { assetOrigin: null, imageUri: null, description: "", originalAsset: "Array", originalMd5: "none" };
              obj3[0] = closure_1_0(6929).AssetOriginTypes.NEW_ASSET;
              obj3[1] = base64;
              obj3[4] = originalMd5;
  ...
  let obj = { value: _handleBannerUploadSelect(7102)(isTryItOut.analyticsLocations).analyticsLocations, children: null };
  ...
  let tmp4Result = flag;
  if (flag) {
    tmp4Result = tmp4(tmp5(8592).NitroWheelIcon, {});
  }
  obj[1] = tmp4Result;
  ...
  if (!flag) {
    ...
    tmp4Result1 = tmp4(tmp5(8592).NitroWheelIcon, obj3);
  }
