...
// Function ID: 12053
// Name: WidgetRenderer
// Dependencies: [32, 19, 17, 1218, 8102, 21, 4189, 712, 8472, 8483, 8104, 12053, 4185, 12057, 1856, 12058, 589, 1348, 12, 5895, 1236, 5562, 5898, 12060, 5916, 12061, 12165, 2]
// Exports: default

// Module 12052 (WidgetRenderer)
...
let closure_17 = importAllResult.memo((game) => {
  ...
  let tmp = callback4();
  ({ gameName, coverImageUrl } = importDefault(12057)(game.applicationId));
  let obj = { location: "UserProfileWidgetsBoard", applicationId: game.applicationId, source: null, sourceUserId: null, trackEntryPointImpression: true, stackingBehavior: "stack" };
  const tmp2 = importDefault;
  const tmp4 = importDefault(12057)(game.applicationId);
  obj[2] = _require(8483).GameProfileSources.UserProfile;
  obj[3] = userId;
  let tmp5Result;
  ...
  const items2 = [
    callback2(closure_6, {
      ...
      accessibilityRole: "button",
      accessibilityLabel: gameName,
      children: callback2(_require(4185).Text, { variant: "text-md/semibold", color: "text-default", lineClamp: 2, children: gameName })
    }),
  ,
  ...
  const isNullOrEmptyResult = _require(1856).isNullOrEmpty(trimmed);
...
let closure_18 = importAllResult.memo((game) => {
  ...
  ({ gameName, coverImageUrl } = importDefault(12057)(game.applicationId));
  let obj = { location: "UserProfileWidgetsBoard", applicationId: game.applicationId, source: null, sourceUserId: null, trackEntryPointImpression: true, stackingBehavior: "stack" };
  const tmp3 = importDefault(12057)(game.applicationId);
  obj[2] = _require(8483).GameProfileSources.UserProfile;
  obj[3] = userId;
  let tmp4Result;
  if (!disableInteraction) {
  ...
  const items2 = [
    callback2(closure_6, {
      ...
      accessibilityRole: "button",
      accessibilityLabel: gameName,
      children: callback2(_require(4185).Text, { variant: "text-md/medium", color: "text-default", lineClamp: 2, children: gameName })
    }),
...
let closure_19 = importAllResult.memo((game) => {
  ...
  ({ userId, coverWidth, disableInteraction } = game);
  let tmp = callback4();
  ({ coverImageUrl, gameName } = importDefault(12057)(game.applicationId));
  let obj = { location: "UserProfileWidgetsBoard", applicationId: game.applicationId, source: null, sourceUserId: null, trackEntryPointImpression: true, stackingBehavior: "stack" };
  const tmp2 = importDefault(12057)(game.applicationId);
  obj[2] = _require(8483).GameProfileSources.UserProfile;
  obj[3] = userId;
  let tmp3Result;
  if (!disableInteraction) {
...
const memoResult = importAllResult.memo((style) => {
  ...
  if (userId(disableInteraction[19]).WidgetType.FAVORITE_GAMES === type) {
    ...
    let mapped = tmp10 > 0;
    if (mapped) {
      mapped = substr.map((applicationId) => outer1_10(outer1_18, { userId, game: applicationId, coverWidth: c5, disableInteraction }, applicationId.applicationId));
    }
    obj[1] = mapped;
    ...
    let mapped1 = tmp10 > 0;
    if (mapped1) {
      mapped1 = substr1.map((applicationId) => outer1_10(outer1_19, { userId, game: applicationId, coverWidth: c5, disableInteraction }, applicationId.applicationId));
    }
    obj5[1] = mapped1;
...
export default function UserProfileWidgetsBoard(userId) {
  ...
  let obj = flag(trackUserProfileAction[26]);
