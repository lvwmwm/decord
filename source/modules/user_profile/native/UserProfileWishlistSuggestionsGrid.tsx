...
// Dependencies: [19, 17, 4473, 8707, 8114, 7148, 673, 675, 21, 709, 4481, 13017, 586, 13023, 8121, 13018, 7102, 5553, 511, 13024, 8706, 4448, 7474, 7122, 4936, 12070, 1233, 13025, 4218, 4477, 8028, 5635, 13026, 2]
// Exports: default

// Module 13022 (UserProfileWishlistSuggestionsGridContent)
import ThemesDefault from "Themes" /* 709 */;
import contextDefault from "context" /* 7102 */;
import useIsMobileWishlistSuggestionsEnabled from "useIsMobileWishlistSuggestionsEnabled" /* 13017 */;
import useCardGridLayoutDefault from "useCardGridLayout" /* 13018 */;
import useWishlistSuggestionsDismissibleContentDefault from "useWishlistSuggestionsDismissibleContent" /* 13023 */;
import closure_3 from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import closure_5 from "maybeApplyNoTextColorForLightCustomTheme" /* 4473 */;
import closure_6 from "get" /* 8707 */;
import { TrackUserProfileWishlistActions as closure_7 } from "USER_PROFILE_TOOLTIP_DELAY" /* 8114 */;
...
function WishlistSuggestionsGridContents(arg0) {
  ...
  let obj3 = stateFromStores(13024);
  obj = { userId, wishlist, numWishlistItemsToRecommend: 15, maxWishlistItemsToShow: 9, source: stateFromStores(8706).WishlistFetchSource.USER_PROFILE };
  ...
  if (0 === items2.length) {
    ...
    obj2[2] = callback(tmp(12070).ShopIcon, { size: "sm" });
    const intl3 = tmp(1233).intl;
    obj2[3] = intl3.string(tmp(1233).t.RSyoZu);
    obj2[4] = callback;
    obj1[1] = callback(tmp(4936).Button, obj2);
    ...
    const items4 = [callback(tmp(4477).Text, obj7), ];
    ...
    obj9[2] = callback(tmp(5635).XSmallIcon, obj10);
    obj9[3] = markAsDismissed;
    const intl = tmp(1233).intl;
    obj9[4] = intl.string(tmp(1233).t.WAI6xu);
    obj8[4] = callback(tmp(8028).IconButton, obj9);
    ...
    items6[1] = callback(tmp6(13026), obj11);
    const obj12 = { style: null, children: null };
    obj12[0] = tmp3.shopButtonContainer;
    const obj13 = { size: "md", variant: "secondary", icon: null, text: null, onPress: null };
    obj13[2] = callback(tmp(12070).ShopIcon, { size: "sm" });
    const intl2 = tmp(1233).intl;
    obj13[3] = intl2.string(tmp(1233).t.RSyoZu);
    obj13[4] = callback;
    obj12[1] = callback(tmp(4936).Button, obj13);
    items6[2] = callback(View, obj12);
    obj5[4] = items6;
    obj3[1] = closure_13(tmp6(4218).View, obj5);
    tmp12Result = tmp12(tmp(13025).WishlistAnalyticsProvider, obj3);
  }
  return tmp12Result;
}
