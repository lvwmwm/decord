...
// Dependencies: [31, 33, 8084, 9032, 4510, 1212, 4510, 2]
// Exports: default

// Module 11692 (UserProfileConfirmRemoveGameFriend)
...
export default function UserProfileConfirmRemoveGameFriend(gameName) {
  ...
  const items1 = [callback(userId(4510).AlertActionButton, obj, "confirm-remove"), ];
  const obj1 = { variant: "secondary" };
  const intl4 = userId(1212).intl;
  obj1.text = intl4.string(userId(1212).t["eN6+rI"]);
  items1[1] = callback(userId(4510).AlertActionButton, obj1, "nevermind");
  obj.children = items1;
  obj.actions = callback2(userId(4510).AlertActions, obj);
  return callback(userId(4510).AlertModal, obj);
};
