...
export default function UserProfileEditTheme(pendingThemeColors) {
  ...
  let callback;
  const tmp = callback3();
  let tmp4 = f94839(primaryColor[11])(user.id, guildId);
  f94839 = tmp4;
  const tmp5 = f94839(primaryColor[12])({ user, displayProfile: tmp4, pendingThemeColors: pendingThemeColors.pendingThemeColors, isPreview: flag });
  ...
  callback = obj.useAvatarColors(pendingAvatarSrc, f94839(tmp3[4]).unsafe_rawColors.PRIMARY_530, false);
    ...
    if (null != secondaryColor) {
      ...
      if (showResetMenu) {
        ...
        obj2[2] = function onPress() {
          let obj = f94839(primaryColor[18]);
          obj = {
            onResetTheme(arg0) {
              let themeColors;
              if (themeColors != null) {
      ...
      f94839 = (arg0) => {
        if (arg0 !== primaryColor) {
          ...
          let themeColors;
          if (f94839 != null) {
            themeColors = f94839.themeColors;
          }
          let tmp8;
      ...
      obj5[0] = () => {
        ...
        obj = { color: secondaryColor, onSelect: f94839, suggestedColors: closure_4 };
