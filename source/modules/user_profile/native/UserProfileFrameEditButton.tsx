...
// Dependencies: [32, 19, 17, 5969, 1384, 505, 21, 712, 4448, 12497, 6219, 1373, 8044, 8460, 1950, 4415, 14252, 2009, 1236, 14239, 6016, 10711, 1297, 8060, 2]
// Exports: default

// Module 14251 (COLLECTIBLES_PREVIEW_SIZE)
...
export default function UserProfileFrameEditButton(arg0) {
  ...
  if (type === user(1950).CollectiblesItemType.PROFILE_FRAME) {
    const first1 = product.items[0];
  }
  tmp2Result = tmp2(8044);
  ...
  if (canViewProfileFramesInCollectiblesShop) {
    ...
    if (fetchCollectiblesProduct.isFetching) {
      ...
      obj1[2] = NOOP;
      obj1[3] = tmp19(tmp2(6016).ActivityIndicator, { animating: true, size: "large" });
      obj2 = obj1;
    } else {
      ...
      obj3[0] = tmp5[0] === tmp2(1373).DismissibleContent.PROFILE_FRAME_USER_PROFILE_NEW_BADGE;
      obj2[1] = tmp19(tmp2(14239).UserProfileEditFormLabelBadges, obj3);
      ...
      if (null != first1) {
        ...
        obj5[1] = tmp19(guildId(10711), obj6);
        let tmp19Result = tmp19(View, obj5);
        const tmp24 = guildId(10711);
      } else {
    ...
    return jsx(tmp2(14239).UserProfileEditFormButton, obj2);
  } else {
    return null;
  }
};
