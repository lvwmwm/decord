...
// Dependencies: [19, 17, 1923, 676, 21, 4446, 712, 589, 4108, 5954, 5586, 1236, 8042, 5957, 5564, 8309, 14181, 2]
// Exports: default

// Module 14197 (ChangeAvatarActionSheet)
...
import UserProfileUpsellButtonDefault from "UserProfileUpsellButton" /* 14181 */;
...
export default function ChangeAvatarActionSheet(showRemoveAvatar) {
  ...
  if (isPremiumResult) {
    isPremiumResult = tmp8(tmp2(8042).NitroWheelIcon, {});
  }
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp2(5564).TableRow, obj1);
  }
  items2[1] = tmp8Result;
  if (showAnimatedAvatarUpsell) {
    ...
    const items3 = [tmp8(tmp2(8309).FormLabel, obj4), ];
    const obj5 = { style: null, size: "sm" };
    obj5[0] = tmp.nitroWheel;
    items3[1] = tmp8(tmp2(8042).NitroWheelIcon, obj5);
    obj3[1] = items3;
    ...
    const items4 = [tmp8(tmp2(8309).FormSubLabel, obj7), ];
    ...
    showAnimatedAvatarUpsell = tmp8(tmp2(5564).TableRow, obj2);
  ...
  if (tmp8Result) {
    ...
    obj11[1] = tmp8(tmp2(8309).FormLabel, obj12);
    obj10[0] = tmp8(View, obj11);
    obj10[1] = handleEditAvatarDecorationSelect;
    tmp8Result = tmp8(tmp2(5564).TableRow, obj10);
  }
  items2[3] = tmp8Result;
  if (flag) {
    ...
    obj13[0] = tmp8(tmp2(8309).FormLabel, obj14);
    obj13[1] = handleRemoveAvatarSelect;
    flag = tmp8(tmp2(5564).TableRow, obj13);
  }
