...
// Dependencies: [19, 17, 1922, 676, 21, 4448, 712, 589, 4109, 5957, 5589, 1236, 8064, 5960, 5567, 8331, 14215, 2]
// Exports: default

// Module 14231 (ChangeAvatarActionSheet)
...
import UserProfileUpsellButtonDefault from "UserProfileUpsellButton" /* 14215 */;
...
export default function ChangeAvatarActionSheet(showRemoveAvatar) {
  ...
  if (isPremiumResult) {
    isPremiumResult = tmp8(tmp2(8064).NitroWheelIcon, {});
  }
  ...
  if (tmp8Result) {
    ...
    tmp8Result = tmp8(tmp2(5567).TableRow, obj1);
  }
  items2[1] = tmp8Result;
  if (showAnimatedAvatarUpsell) {
    ...
    const items3 = [tmp8(tmp2(8331).FormLabel, obj4), ];
    const obj5 = { style: null, size: "sm" };
    obj5[0] = tmp.nitroWheel;
    items3[1] = tmp8(tmp2(8064).NitroWheelIcon, obj5);
    obj3[1] = items3;
    ...
    const items4 = [tmp8(tmp2(8331).FormSubLabel, obj7), ];
    ...
    showAnimatedAvatarUpsell = tmp8(tmp2(5567).TableRow, obj2);
  ...
  if (tmp8Result) {
    ...
    obj11[1] = tmp8(tmp2(8331).FormLabel, obj12);
    obj10[0] = tmp8(View, obj11);
    obj10[1] = handleEditAvatarDecorationSelect;
    tmp8Result = tmp8(tmp2(5567).TableRow, obj10);
  }
  items2[3] = tmp8Result;
  if (flag) {
    ...
    obj13[0] = tmp8(tmp2(8331).FormLabel, obj14);
    obj13[1] = handleRemoveAvatarSelect;
    flag = tmp8(tmp2(5567).TableRow, obj13);
  }
