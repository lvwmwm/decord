...
// Dependencies: [32, 19, 17, 7312, 1947, 1991, 673, 21, 4478, 709, 8088, 5961, 5981, 695, 8082, 5630, 4474, 1233, 8089, 8092, 586, 14514, 8084, 14515, 8111, 8112, 8090, 8534, 4940, 11078, 2]
// Exports: default

// Module 14513 (EditNameplateInner)
...
function NameplateActionSheetPreview(arg0) {
  ...
  const tmp = callback4();
  const tmp4 = purchase(8090)(previewSkuId);
  ...
  const items1 = [callback2(tmp10(8534).NameplateDummyUserPreview, { width: 100 }), callback2(tmp10(8534).NameplateDummyUserPreview, { width: 140 }), ];
  ...
  let tmp2Result = tmp2(4940);
  items2[1] = "" + tmp.nameplatePreviewGradient.color + "00";
  obj1[3] = items2;
  items1[2] = callback2(tmp2Result, obj1);
  obj[1] = items1;
  const items3 = [closure_11(View, obj), callback2(tmp10(11078).NameplatePreview, { nameplate: memo, user, guildId, animate: true, "aria-hidden": true }), ];
  const obj2 = { style: tmp.nameplateGradientContainer, children: null };
  const items4 = [callback2(tmp10(8534).NameplateDummyUserPreview, { width: 140 }), callback2(tmp10(8534).NameplateDummyUserPreview, { width: 100 }), ];
...
export default function EditNameplateActionSheet(arg0) {
  ...
  const analyticsLocations = first(5961)(first(5981).EDIT_NAMEPLATE_SHEET).analyticsLocations;
  const items = [first, guildId];
  callback = obj1.useCallback(() => {
    let obj = first(695);
    obj = { type: first(5981).EDIT_NAMEPLATE_SHEET, is_fullscreen: true };
  ...
  obj4[5] = tmp7(5981).EDIT_NAMEPLATE_SHEET;
  items2[1] = callback2(first(8089), obj4);
  obj[4] = items2;
  obj[1] = tmp12(guildId(5630).BottomSheet, obj);
  return callback2(guildId(5961).AnalyticsLocationProvider, obj);
};
