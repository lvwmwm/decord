...
// Dependencies: [19, 17, 676, 21, 9395, 2]
// Exports: default

// Module 9394 (UserProfileActivityCardBadges)
...
export default function UserProfileActivityCardBadges(activity) {
  ...
  if (activity.type === ActivityTypes.PLAYING) {
    const items = [activity(9395).PartyBadge, activity(9395).TimestampBadge];
    let items3 = items;
  } else if (activity.type === tmp.LISTENING) {
    const items1 = [activity(9395).TimestampBadge];
    items3 = items1;
  } else if (activity.type === tmp.WATCHING) {
    const items2 = [activity(9395).TimestampBadge, activity(9395).EpisodeBadge];
    items3 = items2;
  } else {
    items3 = [];
  }
