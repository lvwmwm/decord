...
// Dependencies: [1994, 1930, 1218, 1910, 1220, 4431, 5217, 676, 7151, 12, 7152, 7162, 7159, 7158, 1370, 4227, 1949, 1900, 7163, 1236, 7164, 5063, 2]

// Module 7150 (createUserWidgetFromServer)
...
function createUserWidgetFromServer(data) {
  ...
  if (require(7151) /* WidgetType */.WidgetType.CURRENT_GAMES !== type) {
    if (tmp(7151).WidgetType.FAVORITE_GAMES !== type) {
      if (tmp(7151).WidgetType.PLAYED_GAMES !== type) {
        if (tmp(7151).WidgetType.WANT_TO_PLAY_GAMES !== type) {
          if (tmp(7151).WidgetType.APPLICATION === type) {
            let obj = { id: null, applicationId: null };
            obj[0] = data.id;
            obj[1] = data.data.application_id;
            const applicationWidget = new tmp(7162).ApplicationWidget(obj);
            return applicationWidget;
          } else if (tmp(7151).WidgetType.PERSONAL === type) {
            obj = { id: null, header: null, sections: null };
            obj[0] = data.id;
            const header = data.data.header;
            obj[1] = header;
            obj[2] = tmp(7159).parsePersonalWidgetSections(data.data.sections);
            const userProfilePersonalWidget = new tmp(7159).UserProfilePersonalWidget(obj);
            return userProfilePersonalWidget;
          } else if (tmp(7151).WidgetType.CLIPS_GALLERY === type) {
            ...
            const mapped = clips.map((id) => {
              ...
              if (null != id.id) {
                tmp = null;
                if (null != id.file_id) {
                  const obj = { status: "saved", id: null, fileId: null, gameId: null, title: null, tags: null };
            ...
            const clipsGalleryWidget = new tmp(7158).ClipsGalleryWidget(obj);
  ...
  const baseGameWidget = new tmp(7152).BaseGameWidget({ id: data.id, type, games: importDefault(12).uniqBy(mapped1, "gameId") });
...
function createUserWidgetFromSnapshot(type) {
  ...
  const baseGameWidget = new tmp(7152).BaseGameWidget({ id: id4, type: type3, games });
  return baseGameWidget;
}
...
function handleProfileFetchFailure(arg0) {
  ...
  if (value == null) {
    const obj = { connectedAccounts: null, applicationRoleConnections: null, premiumSince: null, premiumGuildSince: null, application: null, legacyUsername: null, userId: null, banner: null, accentColor: null, bio: "", pronouns: "", premiumType: null, fetchStartedAt: 0, fetchEndedAt: 0, fetchError: "diversity" };
    obj[0] = [];
    obj[1] = [];
    obj[6] = userId;
    value = obj;
...
function handleProfileUpdateSuccess(guild_id) {
  ...
  if (null != guild_id.guild_id) {
    ...
    if (null != guild_id) {
      if (null != value) {
        ...
        if (null != value) {
          ...
          obj[0] = collectibles;
          const merged1 = Object.assign(importDefault(7164)(obj));
          obj.accentColor = accent_color;
          obj.banner = banner;
          obj.bio = bio;
    ...
    if (null != value1) {
      ...
      const merged3 = Object.assign(importDefault(7164)(obj2));
...
function handleWidgetsUpdateSuccess(arg0) {
  ...
  if (null == value) {
    ...
    obj.widgets = mapped.filter(require(1370) /* isDiscordFrontendDevelopment */.isNotNullish);
...
class UserProfileStore extends tmp2 {
  constructor() {
    ...
    tmp.loadCache = function loadCache() {
      const snapshot = _undefined.readSnapshot(outer1_45.LATEST_SNAPSHOT_VERSION);
      if (null != snapshot) {
