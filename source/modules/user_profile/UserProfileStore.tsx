...
// Dependencies: [1995, 1934, 1215, 1908, 1217, 4569, 5390, 673, 7377, 12, 7378, 7388, 7385, 7384, 1470, 4362, 1949, 1898, 7389, 1233, 7390, 5236, 2]

// Module 7376 (createUserWidgetFromServer)
import applyDefault from "apply" /* 12 */;
import clearAllDefault from "clearAll" /* 1217 */;
import isDiscordFrontendDevelopment from "isDiscordFrontendDevelopment" /* 1470 */;
import WidgetType from "WidgetType" /* 7377 */;
import parseUserProfileCollectiblesDefault from "parseUserProfileCollectibles" /* 7390 */;
...
function createUserWidgetFromServer(data) {
  ...
  if (WidgetType.WidgetType.CURRENT_GAMES !== type) {
    if (tmp(7377).WidgetType.FAVORITE_GAMES !== type) {
      if (tmp(7377).WidgetType.PLAYED_GAMES !== type) {
        if (tmp(7377).WidgetType.WANT_TO_PLAY_GAMES !== type) {
          if (tmp(7377).WidgetType.APPLICATION === type) {
            let obj = { id: null, applicationId: null };
            obj[0] = data.id;
            obj[1] = data.data.application_id;
            const applicationWidget = new tmp(7388).ApplicationWidget(obj);
            return applicationWidget;
          } else if (tmp(7377).WidgetType.PERSONAL === type) {
            ...
            obj[1] = str;
            obj[2] = tmp(7385).parsePersonalWidgetSections(data.data.sections);
            const userProfilePersonalWidget = new tmp(7385).UserProfilePersonalWidget(obj);
            return userProfilePersonalWidget;
          } else if (tmp(7377).WidgetType.CLIPS_GALLERY === type) {
            ...
            const clipsGalleryWidget = new tmp(7384).ClipsGalleryWidget(obj);
  ...
  const baseGameWidget = new tmp(7378).BaseGameWidget({ id: data.id, type, games: applyDefault.uniqBy(mapped1, "gameId") });
...
function createUserWidgetFromSnapshot(type) {
  ...
  const baseGameWidget = new tmp(7378).BaseGameWidget({ id: id4, type: type3, games });
  return baseGameWidget;
}
...
function handleProfileFetchFailure(arg0) {
  ...
  if (value == null) {
    const obj = { connectedAccounts: null, applicationRoleConnections: null, premiumSince: null, premiumGuildSince: null, application: null, legacyUsername: null, userId: null, banner: null, accentColor: null, bio: "", pronouns: "", premiumType: null, fetchStartedAt: 0, fetchEndedAt: 0, fetchError: "accessible" };
    obj[0] = [];
    obj[1] = [];
    obj[6] = userId;
    value = obj;
