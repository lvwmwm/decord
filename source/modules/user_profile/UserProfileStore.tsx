...
// Dependencies: [1994, 1930, 1218, 1910, 1220, 4431, 5196, 676, 7129, 12, 7130, 7140, 7137, 7136, 1370, 4227, 1949, 1900, 7141, 1236, 7142, 5041, 2]

// Module 7128 (createUserWidgetFromServer)
...
function createUserWidgetFromServer(data) {
  ...
  if (require(7129) /* WidgetType */.WidgetType.CURRENT_GAMES !== type) {
    if (tmp(7129).WidgetType.FAVORITE_GAMES !== type) {
      if (tmp(7129).WidgetType.PLAYED_GAMES !== type) {
        if (tmp(7129).WidgetType.WANT_TO_PLAY_GAMES !== type) {
          if (tmp(7129).WidgetType.APPLICATION === type) {
            let obj = { id: null, applicationId: null };
            obj[0] = data.id;
            obj[1] = data.data.application_id;
            const applicationWidget = new tmp(7140).ApplicationWidget(obj);
            return applicationWidget;
          } else if (tmp(7129).WidgetType.PERSONAL === type) {
            obj = { id: null, header: null, sections: null };
            obj[0] = data.id;
            const header = data.data.header;
            obj[1] = header;
            obj[2] = tmp(7137).parsePersonalWidgetSections(data.data.sections);
            const userProfilePersonalWidget = new tmp(7137).UserProfilePersonalWidget(obj);
            return userProfilePersonalWidget;
          } else if (tmp(7129).WidgetType.CLIPS_GALLERY === type) {
            ...
            const clipsGalleryWidget = new tmp(7136).ClipsGalleryWidget(obj);
  ...
  const baseGameWidget = new tmp(7130).BaseGameWidget({ id: data.id, type, games: importDefault(12).uniqBy(mapped1, "gameId") });
...
function createUserWidgetFromSnapshot(type) {
  ...
  const baseGameWidget = new tmp(7130).BaseGameWidget({ id: id4, type: type3, games });
  return baseGameWidget;
}
...
function handleProfileUpdateSuccess(guild_id) {
  ...
  if (null != guild_id.guild_id) {
    ...
    if (null != guild_id) {
      if (null != value) {
        ...
        if (null != value) {
          ...
          obj[0] = collectibles;
          const merged1 = Object.assign(importDefault(7142)(obj));
          obj.accentColor = accent_color;
          obj.banner = banner;
          obj.bio = bio;
    ...
    if (null != value1) {
      ...
      const merged3 = Object.assign(importDefault(7142)(obj2));
