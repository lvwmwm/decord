...
// Dependencies: [32, 4277, 8747, 676, 4221, 589, 711, 8158, 688, 8236, 4130, 2]
// Exports: default

// Module 8746 (useProfileTheme)
...
export default function useProfileTheme(arg0) {
  ...
  if (null != tmp3) {
    ...
    if (!stateFromStores) {
      tmp16 = tmp2;
      if (!forceUserTheme) {
        let profileTheme = tmp4(8236).getProfileTheme(first);
    ...
    if (tmp16 !== ThemeTypes.DARK) {
      ...
      if (isThemeLightResult) {
        isThemeLightResult = tmp4(4130).isThemeLight(tmp2);
        const tmp4Result4 = tmp4(4130);
      }
      let DARKER = tmp16;
      if (isThemeLightResult) {
        DARKER = tmp18.DARKER;
