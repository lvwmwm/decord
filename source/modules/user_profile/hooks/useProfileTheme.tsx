...
// Dependencies: [32, 4294, 8764, 676, 4238, 589, 711, 8175, 688, 8253, 4147, 2]
// Exports: default

// Module 8763 (useProfileTheme)
...
export default function useProfileTheme(arg0) {
  ...
  if (null != tmp3) {
    ...
    if (!stateFromStores) {
      tmp16 = tmp2;
      if (!forceUserTheme) {
        let profileTheme = tmp4(8253).getProfileTheme(first);
    ...
    if (tmp16 !== ThemeTypes.DARK) {
      ...
      if (isThemeLightResult) {
        isThemeLightResult = tmp4(4147).isThemeLight(tmp2);
        const tmp4Result4 = tmp4(4147);
      }
      let DARKER = tmp16;
      if (isThemeLightResult) {
        DARKER = tmp18.DARKER;
