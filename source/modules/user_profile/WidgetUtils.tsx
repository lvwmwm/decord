...
// Function ID: 7132
// Name: findGameWidget
// Dependencies: [32, 1922, 7128, 7132, 7133, 1388, 7134, 4875, 1236, 7130, 7129, 4164, 1377, 7135, 7136, 7137, 7139, 5967, 4877, 2]
...
function findGameWidget(widgetType) {
  ...
  const found = widgets.filter(_require(7130).isGameWidget);
...
export const getGameWidgetSubtitle = function getGameWidgetSubtitle(games, showEditingControls) {
  if (showEditingControls.showEditingControls) {
    if (games.games.length > 0) {
      if (1 === require(4875) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_LIMITS_BY_TYPE[games.type]) {
...
export const widgetMaxGames = function widgetMaxGames(arg0) {
  let num = 0;
  if (arg0 in require(4875) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_LIMITS_BY_TYPE) {
    num = require(4875) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_LIMITS_BY_TYPE[arg0];
  }
  return num;
};
...
export const addWidgetToPending = function addWidgetToPending(type) {
  ...
  if (null == widgets.find((getUniqueKey) => {
    ...
    if (type.type === _require(7129).WidgetType.PERSONAL) {
    ...
    importDefault(7135).setPendingWidgets(items);
    const obj4 = importDefault(7135);
  }
};
export const removeWidgetFromPending = function removeWidgetFromPending(arg0) {
  ...
  importDefault(7135).setPendingWidgets(found);
};
export const addPendingClipToClipsGalleryWidget = function addPendingClipToClipsGalleryWidget(arg0) {
  ...
  if (clips.length < closure_11) {
    ...
    obj[1] = items;
    const clipsGalleryWidget = new require(7136) /* convertClip */.ClipsGalleryWidget(obj);
    importDefault(7135).setPendingWidgets(replaceWidgetInList(clipsGalleryWidget));
    const obj3 = importDefault(7135);
  }
};
export const updateClipTitleInClipsGalleryWidget = function updateClipTitleInClipsGalleryWidget(arg0, str) {
  ...
  if (null != found) {
    ...
    const clipsGalleryWidget = new _require(7136).ClipsGalleryWidget(obj);
    importDefault(7135).setPendingWidgets(replaceWidgetInList(clipsGalleryWidget));
    const obj3 = importDefault(7135);
  }
};
...
export const updatePersonalWidget = function updatePersonalWidget(arg0) {
  ...
  if (found == null) {
    found = require(7137) /* isFieldEmpty */.createDefaultPersonalWidget();
    const obj2 = require(7137) /* isFieldEmpty */;
  }
  obj = initialize;
  const tmp = replaceWidgetInList;
  const tmpResult = replaceWidgetInList(arg0(found));
  importDefault(7135).setPendingWidgets(tmpResult);
};
export const updatePendingGameTags = function updatePendingGameTags(widgetType, arg1, tags) {
  const _require = arg1;
  if (tags.length <= Object.values(_require(7139).WidgetGameTag).length) {
    ...
    if (null != tmp5) {
      ...
      if (null != found) {
        ...
        obj.games = mapped;
        const baseGameWidget = new _require(7130).BaseGameWidget(obj);
    ...
    if (null != found) {
      if (null != found.tags) {
        if (0 !== found.tags.length) {
          ...
          if (found1.length <= Object.values(_require(7139).WidgetGameTag).length) {
...
export const addPendingGameToWidget = function addPendingGameToWidget(ignoreMaxGames) {
  ...
  importDefault(7135).setPendingWidgets(tmp7Result);
          ...
          if (arg1 < items.length) {
            if (arg2 >= 0) {
              if (arg2 < items.length) {
                ...
                obj.games = items;
                const baseGameWidget = new require(7130) /* items */.BaseGameWidget(obj);
                const tmp19 = replaceWidgetInList(baseGameWidget);
                importDefault(7135).setPendingWidgets(tmp19);
                const obj2 = importDefault(7135);
              }
            }
          }
        }
...
export const removePendingGameFromWidget = function removePendingGameFromWidget(widgetType) {
  ...
  if (null != merged) {
    ...
    importDefault(7135).setPendingWidgets(tmp13);
    const obj2 = importDefault(7135);
  }
};
export const isGameLimitReached = function isGameLimitReached(type) {
  type = type.type;
  let num = 0;
  if (type in require(4875) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_LIMITS_BY_TYPE) {
    num = require(4875) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_LIMITS_BY_TYPE[type];
  }
...
export const isGameAllowedInGameWidgets = function isGameAllowedInGameWidgets(contentClassification) {
  const result = require(4877) /* isAgeRestrictedContentClassification */.isAgeRestrictedContentClassification(contentClassification.contentClassification);
  let tmp4 = !result;
  if (!result) {
    const GAME_WIDGET_BANNED_APPLICATION_IDS = require(4875) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_BANNED_APPLICATION_IDS;
