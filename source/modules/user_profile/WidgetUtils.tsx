...
// Function ID: 7154
// Name: findGameWidget
// Dependencies: [32, 1922, 7150, 7154, 7155, 1388, 7156, 4897, 1236, 7152, 7151, 4164, 1377, 7157, 7158, 7159, 7161, 5988, 4899, 2]
// Exports: addPendingClipToClipsGalleryWidget, addPendingGameToWidget, addWidgetToPending, areWidgetGamesEqual, getGameWidgetSubtitle, getRandomElement, getRandomElements, getSavedWidgets, getWidgetTitle, isGameAllowedInGameWidgets, isGameLimitReached, removeClipFromClipsGalleryWidget, removePendingGameFromWidget, removeTagFromGame, removeWidgetFromPending, reorderGamesInWidget, reorderWidgets, updateClipTitleInClipsGalleryWidget, updatePendingGameComment, updatePendingGameTags, updatePersonalWidget, widgetMaxGames, widgetSupportsComment, widgetSupportsTags

// Module 7153 (findGameWidget)
...
function findGameWidget(widgetType) {
  ...
  const found = widgets.filter(_require(7152).isGameWidget);
...
export const getGameWidgetSubtitle = function getGameWidgetSubtitle(games, showEditingControls) {
  if (showEditingControls.showEditingControls) {
    if (games.games.length > 0) {
      if (1 === require(4897) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_LIMITS_BY_TYPE[games.type]) {
...
export const widgetMaxGames = function widgetMaxGames(arg0) {
  let num = 0;
  if (arg0 in require(4897) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_LIMITS_BY_TYPE) {
    num = require(4897) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_LIMITS_BY_TYPE[arg0];
  }
  return num;
};
...
export const addWidgetToPending = function addWidgetToPending(type) {
  ...
  if (null == widgets.find((getUniqueKey) => {
    ...
    if (type.type === _require(7151).WidgetType.PERSONAL) {
    ...
    importDefault(7157).setPendingWidgets(items);
    const obj4 = importDefault(7157);
  }
};
export const removeWidgetFromPending = function removeWidgetFromPending(arg0) {
  ...
  importDefault(7157).setPendingWidgets(found);
};
export const addPendingClipToClipsGalleryWidget = function addPendingClipToClipsGalleryWidget(arg0) {
  ...
  if (clips.length < closure_11) {
    ...
    obj[1] = items;
    const clipsGalleryWidget = new require(7158) /* convertClip */.ClipsGalleryWidget(obj);
    importDefault(7157).setPendingWidgets(replaceWidgetInList(clipsGalleryWidget));
    const obj3 = importDefault(7157);
  }
};
export const updateClipTitleInClipsGalleryWidget = function updateClipTitleInClipsGalleryWidget(arg0, str) {
  ...
  if (null != found) {
    ...
    const clipsGalleryWidget = new _require(7158).ClipsGalleryWidget(obj);
    importDefault(7157).setPendingWidgets(replaceWidgetInList(clipsGalleryWidget));
    const obj3 = importDefault(7157);
  }
};
...
export const updatePersonalWidget = function updatePersonalWidget(arg0) {
  ...
  if (found == null) {
    found = require(7159) /* isFieldEmpty */.createDefaultPersonalWidget();
    const obj2 = require(7159) /* isFieldEmpty */;
  }
  obj = initialize;
  const tmp = replaceWidgetInList;
  const tmpResult = replaceWidgetInList(arg0(found));
  importDefault(7157).setPendingWidgets(tmpResult);
};
export const updatePendingGameTags = function updatePendingGameTags(widgetType, arg1, tags) {
  const _require = arg1;
  if (tags.length <= Object.values(_require(7161).WidgetGameTag).length) {
    ...
    if (null != tmp5) {
      ...
      if (null != found) {
        ...
        obj.games = mapped;
        const baseGameWidget = new _require(7152).BaseGameWidget(obj);
    ...
    if (null != found) {
      if (null != found.tags) {
        if (0 !== found.tags.length) {
          ...
          if (found1.length <= Object.values(_require(7161).WidgetGameTag).length) {
...
export const addPendingGameToWidget = function addPendingGameToWidget(ignoreMaxGames) {
  ...
  importDefault(7157).setPendingWidgets(tmp7Result);
          ...
          if (arg1 < items.length) {
            if (arg2 >= 0) {
              if (arg2 < items.length) {
                ...
                obj.games = items;
                const baseGameWidget = new require(7152) /* items */.BaseGameWidget(obj);
                const tmp19 = replaceWidgetInList(baseGameWidget);
                importDefault(7157).setPendingWidgets(tmp19);
                const obj2 = importDefault(7157);
              }
            }
          }
        }
...
export const removePendingGameFromWidget = function removePendingGameFromWidget(widgetType) {
  ...
  if (null != merged) {
    ...
    importDefault(7157).setPendingWidgets(tmp13);
    const obj2 = importDefault(7157);
  }
};
export const isGameLimitReached = function isGameLimitReached(type) {
  type = type.type;
  let num = 0;
  if (type in require(4897) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_LIMITS_BY_TYPE) {
    num = require(4897) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_LIMITS_BY_TYPE[type];
  }
...
export const isGameAllowedInGameWidgets = function isGameAllowedInGameWidgets(contentClassification) {
  const result = require(4899) /* isAgeRestrictedContentClassification */.isAgeRestrictedContentClassification(contentClassification.contentClassification);
  let tmp4 = !result;
  if (!result) {
    const GAME_WIDGET_BANNED_APPLICATION_IDS = require(4897) /* GAME_WIDGET_LIMITS_BY_TYPE */.GAME_WIDGET_BANNED_APPLICATION_IDS;
