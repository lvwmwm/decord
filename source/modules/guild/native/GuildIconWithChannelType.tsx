...
// Dependencies: [19, 17, 21, 5972, 4448, 712, 4949, 9232, 8066, 1297, 2]
// Exports: GuildIconWithChannelType

// Module 10698 (GuildIconWithChannelTypeSizes)
...
export const GuildIconWithChannelType = function GuildIconWithChannelType(arg0) {
  ...
  const gap = tmp4.gap;
  const tmp5 = typeIconSize(5972).ImageSizes[guildIconSize];
  dependencyMap = tmp5;
  ...
  obj = typeIconSize(4949);
  const channelIcon = obj.getChannelIcon(channel);
  obj = { "aria-label": tmp, shape: typeIconSize(8066).CutoutShape.Circle, size: memo, gap, depthX: sum1, depthY: sum1, children: null };
  obj = {};
  const tmp3 = callback3();
  const merged1 = Object.assign(merged);
  obj.size = guildIconSize;
  const items3 = [callback(typeIconPadding(5972), obj), ];
  const tmp12 = typeIconPadding(5972);
  ...
  obj[6] = items3;
  return callback2(typeIconSize(9232).Pile, obj);
};
