...
// Dependencies: [19, 17, 21, 4116, 4255, 712, 1348, 1865, 1236, 15134, 13133, 15132, 5305, 3959, 8638, 4251, 9152, 1297, 5645, 15137, 11553, 15142, 2]
// Exports: default

// Module 15133 (ChannelListStickyHeader)
...
let closure_7 = require("module_4116").createAnimatedComponent(Pressable);
...
export default function ChannelListStickyHeader(guild) {
  ...
  const tmp10 = importDefault(15132)(guild);
  let tmp6Result = tmp6(5305);
  ...
  const token = guild(3959).useToken(tmp(712).modules.mobile.CHANNEL_LIST_TITLE_TEXT_STYLE);
  const tmp6Result1 = guild(3959);
  const tmp8 = isFavoritesGuildIdResult ? t.hW8QDk : t["Gpyp/e"];
  const token1 = guild(3959).useToken(tmp(712).modules.mobile.CHANNEL_LIST_SUBTITLE_TEXT_STYLE);
  ...
  if (isFavoritesGuildIdResult) {
    ...
    tmp22 = callback(tmp6(8638).StarIcon, obj1);
  }
  const items2 = [tmp22, , , ];
  const tmp16 = closure_7;
  const tmp6Result2 = guild(3959);
  items2[1] = callback(closure_4, { ref, collapsable: false, style: tmp4.flex, children: callback(guild(4251).Text, { experimental_useNativeText: true, color: "mobile-text-heading-primary", variant: token, lineClamp: 1, children: favoritesAwareGuildName }) });
  const obj3 = { guild, size: null, style: null };
  const obj2 = { ref, collapsable: false, style: tmp4.flex, children: callback(guild(4251).Text, { experimental_useNativeText: true, color: "mobile-text-heading-primary", variant: token, lineClamp: 1, children: favoritesAwareGuildName }) };
  ...
  if (flag2) {
    ...
    obj4[2] = tmp4.chevron;
    tmp24Result = tmp24(tmp6(5645).ChevronSmallRightIcon, obj4);
  }
  ...
  if (null != tmp10) {
    ...
    if (tmp10 > 0) {
      ...
      const items4 = [tmp24(tmp6(4251).Text, obj6), , ];
      ...
      items4[2] = tmp24(tmp6(4251).Text, obj9);
      obj5[1] = items4;
  ...
  tmp24Result = null;
  if (isFavoritesGuildIdResult) {
    tmp24Result = tmp24(tmp6(15137).FavoritesGuildHeaderActionButton, {});
  }
  ...
  if (flag) {
    ...
    obj12[0] = guild;
    tmp24Result1 = tmp24(tmp(11553), obj12);
  }
  ...
  if (flag3) {
    ...
    obj14[0] = ref;
    obj14[1] = guild;
    tmp24Result3 = tmp24(tmp(15142), obj14);
  }
