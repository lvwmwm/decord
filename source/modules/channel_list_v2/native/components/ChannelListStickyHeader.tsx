...
// Dependencies: [19, 17, 676, 21, 4186, 4446, 712, 1367, 15623, 1913, 1236, 15655, 13579, 15653, 5569, 4166, 9119, 4442, 8806, 1297, 5968, 15669, 11800, 15674, 15675, 2]
// Exports: default

// Module 15654 (ChannelListStickyHeader)
import ThemesDefault from "Themes" /* 712 */;
import useIsMobileVisualRefreshExperimentEnabledDefault from "useIsMobileVisualRefreshExperimentEnabled" /* 1367 */;
import GuildBadgeV2Default from "GuildBadgeV2" /* 8806 */;
import useIsGameCommunityServerPreviewDefault from "useIsGameCommunityServerPreview" /* 15623 */;
import useStickyServerHeaderSubtitleDefault from "useStickyServerHeaderSubtitle" /* 15653 */;
...
export default function ChannelListStickyHeader(guild) {
  ...
  const token = guild(4166).useToken(tmp(712).modules.mobile.CHANNEL_LIST_TITLE_TEXT_STYLE);
  const tmp7Result1 = guild(4166);
  const tmp9 = isFavoritesGuildIdResult ? t.hW8QDk : t["Gpyp/e"];
  const token1 = guild(4166).useToken(tmp(712).modules.mobile.CHANNEL_LIST_SUBTITLE_TEXT_STYLE);
  ...
  if (isFavoritesGuildIdResult) {
    ...
    tmp23 = callback(tmp7(9119).StarIcon, obj1);
  }
  const items2 = [tmp23, , , ];
  const tmp17 = closure_8;
  const tmp7Result2 = guild(4166);
  items2[1] = callback(closure_4, { ref, collapsable: false, style: tmp4.flex, children: callback(guild(4442).Text, { experimental_useNativeText: true, color: "mobile-text-heading-primary", variant: token, lineClamp: 1, children: favoritesAwareGuildName }) });
  const obj3 = { guild, size: null, style: null };
  const obj2 = { ref, collapsable: false, style: tmp4.flex, children: callback(guild(4442).Text, { experimental_useNativeText: true, color: "mobile-text-heading-primary", variant: token, lineClamp: 1, children: favoritesAwareGuildName }) };
  ...
  if (flag2) {
    ...
    obj4[2] = tmp4.chevron;
    tmp25Result = tmp25(tmp7(5968).ChevronSmallRightIcon, obj4);
  }
  ...
  if (null != tmp11) {
    ...
    if (tmp11 > 0) {
      ...
      const items4 = [tmp25(tmp7(4442).Text, obj6), , ];
      ...
      items4[2] = tmp25(tmp7(4442).Text, obj9);
      obj5[1] = items4;
  ...
  tmp25Result = null;
  if (isFavoritesGuildIdResult) {
    tmp25Result = tmp25(tmp7(15669).FavoritesGuildHeaderActionButton, {});
  }
  ...
  if (flag) {
    ...
    obj12[0] = guild;
    tmp25Result1 = tmp25(tmp(11800), obj12);
  }
  ...
  if (flag3) {
    ...
    obj16[0] = ref;
    obj16[1] = guild;
    tmp25Result4 = tmp25(tmp(15675), obj16);
  }
