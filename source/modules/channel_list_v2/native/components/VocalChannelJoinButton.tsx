// Module ID: 11549
// Function ID: 11550
// Dependencies: [19, 17, 3883, 4271, 676, 21, 4255, 712, 500, 8175, 4191, 4101, 9630, 647, 5100, 5094, 5087, 11550, 4251, 1236, 4665, 4776, 4780, 2]
...
const memoResult = importAllResult.memo(function VocalChannelJoinButton(channel) {
  ...
  let obj3 = channel(5100);
  const stageParticipantsCount = obj3.useStageParticipantsCount(channel.id, channel(5094).StageChannelParticipantNamedIndex.AUDIENCE);
  const isGuildStageVoiceResult = channel.isGuildStageVoice();
  const tmp2 = voiceStates(4191)();
  const obj5 = channel(5087);
  ...
  tmp3Result = tmp3(11550);
  const connectedUserLimit = tmp3Result.useConnectedUserLimit({ channel, video: tmp12 });
  tmp8 = channel(5087).useStageHasMedia(channel.id) && isGuildStageVoiceResult;
  let connectedUserLimitFormatted = channel(11550).useConnectedUserLimitFormatted({ channel, video: tmp12, userCount: sum });
    ...
    if (!noIcon) {
      if (!isConnectedToVoiceChannel) {
        ...
        if (isGuildStageVoiceResult) {
          let VoiceNormalIcon = tmp3(4776).StageIcon;
        } else {
          VoiceNormalIcon = tmp3(4780).VoiceNormalIcon;
        }
