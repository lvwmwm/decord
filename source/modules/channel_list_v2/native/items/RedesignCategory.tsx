...
// Dependencies: [19, 17, 7209, 5437, 4654, 10121, 21, 4445, 500, 712, 4441, 12205, 5020, 8858, 11073, 10571, 1367, 589, 4639, 10632, 15608, 15609, 5433, 1236, 5939, 11074, 6177, 2]
// Exports: CategoryChannel, RecentlyActiveCategory, SuggestedCategory, useCategoryPressEvents

// Module 15607 (renderCategoryItem)
import set from "set" /* 500 */;
import ThemesDefault from "Themes" /* 712 */;
import closure_3 from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import closure_5 from "set" /* 7209 */;
import closure_6 from "incrementVersion" /* 5437 */;
...
function renderCategoryItem(longPressAction) {
  ...
  if (typeof name === "string") {
    ...
    obj[5] = name;
    tmp35Result = callback(trailingAction(4441).Text, obj);
  ...
  if (Icon == null) {
    Icon = trailingAction(12205).PlusMediumIcon;
  }
  if (null == trailingAction) {
    ...
    if (tmp) {
      ...
      obj5[6] = items;
      obj5[7] = tmp18Result;
      let tmp29Result = tmp29(trailingAction(5020).PressableHighlight, obj5);
    } else {
    ...
    obj8[6] = colors;
    obj8 = tmp9(trailingAction(5020).PressableOpacity, obj8);
    obj7[1] = obj8;
    callback(View, obj7);
    const tmp10 = View;
  }
...
export const CategoryChannel = function CategoryChannel(channel) {
  ...
  const tmp9 = collapsed(4639)(channel);
  const tmp = collapsed;
  const tmp4 = styles(tmp3);
  const tmp11 = collapsed(15608)(channel, tmp9);
  const tmp12 = collapsed(15609)(channel);
  obj = { name: tmp9, muted: stateFromStoresObject.muted, collapsed, onPress: callback, onLongPress: null, withMarginTop: null, styles: null, isRefreshEnabled: null, note: null, trailingAction: null, longPressAction: null };
  let perform;
  if (tmp12 != null) {
    perform = tmp12.perform;
  }
  if (perform == null) {
    perform = callback1;
  }
  ...
  if (null != tmp11) {
    ...
    ({ tooltip: obj3[2], label: obj3[3] } = tmp11);
    tmp14 = callback(tmp5(4441).Text, obj);
  }
  obj[8] = tmp14;
  obj[9] = collapsed(10632)(channel);
  obj[10] = tmp12;
  ...
  obj1.icon = callback(id(8858).ChevronSmallDownIcon, { size: "xxs", color: merged.muted ? colors.ICON_MUTED : colors.TEXT_SUBTLE, style: collapsed2 ? createCacheKey : obj1 });
  obj1.accessibilityState = { expanded: !collapsed2 };
  obj1.isRefreshEnabled = isRefreshEnabled;
...
export const RecentlyActiveCategory = function RecentlyActiveCategory(guildId) {
  ...
  const callback = React.useCallback(() => guildId(closure_1_2[22]).setRecentlyActiveCollapsed(guildId, !stateFromStores), items1);
  ...
  obj = {};
  const merged1 = Object.assign(merged);
  obj.icon = callback(guildId(8858).ChevronSmallDownIcon, { size: "xxs", color: merged.muted ? colors.ICON_MUTED : colors.TEXT_SUBTLE, style: collapsed ? createCacheKey : obj1 });
...
export const SuggestedCategory = function SuggestedCategory(guildId) {
  ...
  const callback = React.useCallback(() => {
    let obj = guildId(closure_1_2[24]);
    obj = { key: "REDESIGN_SUGGESTED_CHANNELS_CLEAR", options: null, hasIcons: true };
    obj = { label: null, onPress: null, IconComponent: null };
    const intl = guildId(closure_1_2[23]).intl;
    obj[0] = intl.string(guildId(closure_1_2[23]).t.VkKicb);
    obj[1] = function onPress() {
      closure_1_0(closure_1_2[25]).clearRecentChannels(closure_0, closure_1);
    };
    obj[2] = guildId(closure_1_2[26]).CircleXIcon;
    const items = [obj];
    obj[1] = items;
