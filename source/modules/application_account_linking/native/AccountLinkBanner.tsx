...
// Dependencies: [19, 17, 1922, 1388, 21, 712, 7149, 15456, 9288, 4749, 4661, 647, 7139, 7159, 6292, 5433, 6368, 1297, 4734, 1236, 4745, 9147, 2]
...
const memoResult = importAllResult.memo((arg0) => {
  let obj = importDefault(15456);
  const config = obj.useConfig({ location: "AccountLinkBanner" });
  const bannerSize = config.bannerSize;
  let tmp5Result = null;
  if (config.enabled) {
    let tmp8 = bannerSize === require(15456) /* apexExperiment */.MobileAccountLinkingBannerSize.LARGE;
    if (tmp8) {
      obj = {};
    ...
    const items = [tmp8, ];
    let tmp14 = bannerSize === require(15456) /* apexExperiment */.MobileAccountLinkingBannerSize.MEDIUM;
    if (tmp14) {
      obj = {};
...
export const getScaledAccountLinkBannerHeight = function getScaledAccountLinkBannerHeight(fontScale) {
  ...
  const config = importDefault(15456).getConfig({ location: "getScaledAccountLinkBannerHeight" });
  const bannerSize = config.bannerSize;
  if (!config.enabled) {
    return 0;
  } else if (bannerSize === require(15456) /* apexExperiment */.MobileAccountLinkingBannerSize.LARGE) {
