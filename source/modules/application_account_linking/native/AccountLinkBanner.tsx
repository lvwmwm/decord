...
// Dependencies: [31, 27, 1850, 1345, 33, 689, 5472, 15092, 9849, 4548, 4131, 624, 5462, 5482, 5166, 4661, 5119, 1273, 4127, 1212, 4544, 8577, 2]
// Exports: getScaledAccountLinkBannerHeight

// Module 15091 (AccountLinkLargeBanner)
...
const memoResult = importAllResult.memo((arg0) => {
  let obj = importDefault(15092);
  const config = obj.useConfig({ location: "AccountLinkBanner" });
  const bannerSize = config.bannerSize;
  let tmp3Result = null;
  if (config.enabled) {
    obj = {};
    let tmp7 = bannerSize === require(15092) /* apexExperiment */.MobileAccountLinkingBannerSize.LARGE;
    if (tmp7) {
      obj = {};
    ...
    const items = [tmp7, ];
    let tmp15 = bannerSize === require(15092) /* apexExperiment */.MobileAccountLinkingBannerSize.MEDIUM;
    if (tmp15) {
      const obj1 = {};
...
export const getScaledAccountLinkBannerHeight = function getScaledAccountLinkBannerHeight(fontScale) {
  const config = importDefault(15092).getConfig({ location: "getScaledAccountLinkBannerHeight" });
  const bannerSize = config.bannerSize;
  if (!config.enabled) {
    return 0;
  } else if (bannerSize === require(15092) /* apexExperiment */.MobileAccountLinkingBannerSize.LARGE) {
    ...
    num = 0;
    if (bannerSize === require(15092) /* apexExperiment */.MobileAccountLinkingBannerSize.MEDIUM) {
