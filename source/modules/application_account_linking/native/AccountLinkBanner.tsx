...
// Dependencies: [31, 27, 1850, 1345, 33, 689, 5507, 15132, 9889, 4582, 4165, 624, 5497, 5517, 5200, 4695, 5153, 1273, 4161, 1212, 4578, 8621, 2]
// Exports: getScaledAccountLinkBannerHeight

// Module 15131 (AccountLinkLargeBanner)
...
const memoResult = importAllResult.memo((arg0) => {
  let obj = importDefault(15132);
  const config = obj.useConfig({ location: "AccountLinkBanner" });
  const bannerSize = config.bannerSize;
  let tmp3Result = null;
  if (config.enabled) {
    obj = {};
    let tmp7 = bannerSize === require(15132) /* apexExperiment */.MobileAccountLinkingBannerSize.LARGE;
    if (tmp7) {
      obj = {};
    ...
    const items = [tmp7, ];
    let tmp15 = bannerSize === require(15132) /* apexExperiment */.MobileAccountLinkingBannerSize.MEDIUM;
    if (tmp15) {
      const obj1 = {};
...
export const getScaledAccountLinkBannerHeight = function getScaledAccountLinkBannerHeight(fontScale) {
  const config = importDefault(15132).getConfig({ location: "getScaledAccountLinkBannerHeight" });
  const bannerSize = config.bannerSize;
  if (!config.enabled) {
    return 0;
  } else if (bannerSize === require(15132) /* apexExperiment */.MobileAccountLinkingBannerSize.LARGE) {
    ...
    let num = sum2 + require(4582) /* getButtonPadding */.MEDIUM_BUTTON_HEIGHT + PX_12 + PX_82;
    const obj3 = require(9889) /* scaleLineHeight */;
  } else {
    num = 0;
    if (bannerSize === require(15132) /* apexExperiment */.MobileAccountLinkingBannerSize.MEDIUM) {
      const sum3 = PX_8 + PX_12 + 48 + PX_122;
      num = sum3 + require(4582) /* getButtonPadding */.SMALL_BUTTON_HEIGHT + PX_12 + PX_82;
    }
  }
};
