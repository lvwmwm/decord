...
// Dependencies: [19, 17, 1874, 1369, 21, 712, 5525, 15165, 9911, 4604, 4189, 647, 5515, 5535, 5222, 4717, 5175, 1297, 4185, 1236, 4600, 8645, 2]
// Exports: getScaledAccountLinkBannerHeight

// Module 15164 (AccountLinkLargeBanner)
...
const memoResult = importAllResult.memo((arg0) => {
  let obj = importDefault(15165);
  const config = obj.useConfig({ location: "AccountLinkBanner" });
  const bannerSize = config.bannerSize;
  let tmp5Result = null;
  if (config.enabled) {
    let tmp8 = bannerSize === require(15165) /* apexExperiment */.MobileAccountLinkingBannerSize.LARGE;
    if (tmp8) {
      obj = {};
    ...
    const items = [tmp8, ];
    let tmp14 = bannerSize === require(15165) /* apexExperiment */.MobileAccountLinkingBannerSize.MEDIUM;
    if (tmp14) {
      obj = {};
...
export const getScaledAccountLinkBannerHeight = function getScaledAccountLinkBannerHeight(fontScale) {
  ...
  const config = importDefault(15165).getConfig({ location: "getScaledAccountLinkBannerHeight" });
  const bannerSize = config.bannerSize;
  if (!config.enabled) {
    return 0;
  } else if (bannerSize === require(15165) /* apexExperiment */.MobileAccountLinkingBannerSize.LARGE) {
    const sum = PX_8 + PX_12 + closure_16 + PX_16;
    let tmp3Result = tmp3(9911);
    const sum1 = sum + tmp3Result.scaleTextLineHeight(c17, fontScale) + PX_4;
    tmp3Result = tmp3(9911);
    const sum2 = sum1 + 2 * tmp3Result.scaleTextLineHeight(c18, fontScale) + PX_162;
    sum3 = sum2 + tmp3(4604).MEDIUM_BUTTON_HEIGHT + PX_12;
    let num = sum3 + PX_82;
  } else {
    num = 0;
    if (bannerSize === tmp3(15165).MobileAccountLinkingBannerSize.MEDIUM) {
