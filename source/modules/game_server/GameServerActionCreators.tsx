...
// Dependencies: [1946, 1874, 11571, 4078, 676, 709, 11572, 4427, 530, 11574, 4099, 698, 11575, 11573, 2]
...
// Module 11570 (fetchGameServerCatalog)
...
export const fetchGameServerRegions = function fetchGameServerRegions(arg0) {
  ...
  return value.then((body) => {
    ...
    body = body.body;
    const mapped = body.map(callback(11575));
...
export const fetchMyGameServerRegions = function fetchMyGameServerRegions() {
  ...
  return value.then((body) => {
    ...
    const mapped = regions.map(callback(11575));
    obj[1] = mapped.sort((name, name2) => {
      name = name.name;
      return name.localeCompare(name2.name);
    });
...
export const optimisticallyMarkGameServerResizing = function optimisticallyMarkGameServerResizing(arg0) {
  ...
  if (null != found) {
    let obj = importDefault(709);
    obj = { type: "GAME_SERVER_UPDATE", guildId: "Array", gameServer: 3 };
    obj = {};
    const merged = Object.assign(found);
    obj.status = _require(11573).GameServerStatus.STARTING;
    obj[2] = obj;
    obj.dispatch(obj);
  }
};
