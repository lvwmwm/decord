...
// Dependencies: [19, 17, 8945, 676, 21, 4285, 712, 16536, 16576, 4981, 1416, 4130, 5688, 5354, 5689, 1236, 8944, 4624, 4713, 3988, 1480, 589, 4221, 5253, 5929, 5937, 1974, 7890, 4693, 4281, 5635, 2]
// Exports: default

// Module 16575 (getDerivedStateFromProps)
...
IntegrationItem.prototype["render"] = function render() {
  ...
  const SUPPORTED_SETTINGS_INTEGRATION_PLATFORMS = integration(16536).SUPPORTED_SETTINGS_INTEGRATION_PLATFORMS;
  if (SUPPORTED_SETTINGS_INTEGRATION_PLATFORMS.includes(integration.type)) {
    const type = integration.type;
    if (tmp(16576).IntegrationTypes.YOUTUBE === type) {
      ...
      let combined = name;
    } else if (tmp(16576).IntegrationTypes.TWITCH === type) {
    ...
    if (null == value) {
      ...
      const items = [closure_11(tmp(5354).TableRow, obj), ];
      ...
      items[1] = closure_11(tmp(5689).TableSwitchRow, obj2);
      obj1[1] = items;
      return closure_12(tmp(5688).TableRowGroup, obj1);
    } else {
...
export default function GuildSettingsModalIntegrationPlatform(platformType) {
  ...
  if (null == guild) {
    ...
    const items3 = [closure_11(tmp(7890).Form, obj2), closure_11(tmp(5635).NavScrim, {})];
    obj5[0] = items3;
