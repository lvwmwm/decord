...
// Dependencies: [19, 17, 8994, 673, 21, 4481, 709, 17480, 17520, 5244, 1431, 1362, 5642, 5560, 7141, 1233, 8993, 4866, 4955, 4197, 1498, 586, 4416, 5579, 7315, 7320, 1994, 8523, 4934, 4477, 6980, 2]
// Exports: default

// Module 17519 (getDerivedStateFromProps)
...
IntegrationItem.prototype["render"] = function render() {
  ...
  const SUPPORTED_SETTINGS_INTEGRATION_PLATFORMS = integration(17480).SUPPORTED_SETTINGS_INTEGRATION_PLATFORMS;
  if (SUPPORTED_SETTINGS_INTEGRATION_PLATFORMS.includes(integration.type)) {
    ...
    if (null == value) {
      ...
      const items = [closure_11(tmp(5560).TableRow, obj), ];
      ...
      items[1] = closure_11(tmp(7141).TableSwitchRow, obj2);
      obj1[1] = items;
      return closure_12(tmp(5642).TableRowGroup, obj1);
    } else {
...
export default function GuildSettingsModalIntegrationPlatform(platformType) {
  ...
  function onSave() {
    if (null != guild) {
      let obj = closeGuildSettings(8993);
      obj = { features: null };
      obj[0] = tmp.features;
      obj.saveGuild(tmp.id, obj);
    }
  }
  let obj = platformType(4197);
  const token = obj.useToken(closeGuildSettings(709).modules.mobile.TABLE_ROW_PADDING);
  const tmp5 = callback();
  dependencyMap = tmp5;
  obj1 = platformType(1498);
  navigation = obj1.useNavigation();
  let obj3 = platformType(586);
  const items = [guild];
  ...
  closure_7 = closeGuildSettings(4416)();
  let obj4 = platformType(586);
  const items1 = [guild];
  ...
  if (null == guild) {
    ...
    obj[2] = function headerRight() {
      if (submitting) {
        let tmp2 = closure_1_11(platformType(5579).HeaderSubmittingIndicator, {});
      } else {
        tmp2 = null;
        if (c5) {
          ...
          obj[1] = onSave;
          tmp2 = closure_1_11(platformType(7315).HeaderActionButton, obj);
        }
      }
      return tmp2;
    };
    ...
    if (constants.YOUTUBE === platformType) {
      ...
      obj[0] = onConectTap;
      tmp3Result = tmp3(1994);
      obj[1] = tmp3Result.getArticleURL(onSave.YOUTUBE_INTEGRATION);
      let formatResult = intl.format(tmp(1233).t["4OSAQ9"], obj);
    } else if (tmp10.TWITCH === platformType) {
      const intl2 = tmp(1233).intl;
      obj1 = { connectAction: null, helpdeskArticle: null };
      obj1[0] = onConectTap;
      obj1[1] = tmp3(1994).getArticleURL(onSave.TWITCH_INTEGRATION);
      formatResult = intl2.format(tmp(1233).t.ro1jEN, obj1);
      const tmp3Result1 = tmp3(1994);
    }
    ...
    obj6[2] = formatResult;
    items2[1] = closure_11(tmp(4477).Text, obj6);
    obj3[2] = items2;
    obj2[2] = closure_12(tmp(4934).Stack, obj3);
    const items3 = [closure_11(tmp(8523).Form, obj2), closure_11(tmp(6980).NavScrim, {})];
    obj5[0] = items3;
