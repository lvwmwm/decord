// Module ID: 16379
// Function ID: 127358
// Name: showGuildSettingsStickerError
// Dependencies: [3866, 6709, 1212, 4121, 2]
// Exports: showGuildSettingsStickerError, showGuildSettingsStickerSuccess

// Module 16379 (showGuildSettingsStickerError)
const result = require("getSystemLocale").fileFinishedImporting("modules/guild_settings/server_monetization/stickers/native/guildSettingsStickerToasts.tsx");

export const showGuildSettingsStickerError = function showGuildSettingsStickerError() {
  let obj = importDefault(3866);
  obj = { key: "GUILD_SETTINGS_STICKER_ERROR", IconComponent: require(6709) /* CircleErrorIcon */.CircleErrorIcon };
  const intl = require(1212) /* getSystemLocale */.intl;
  obj.content = intl.string(require(1212) /* getSystemLocale */.t["5NMPSS"]);
  obj.open(obj);
};
export const showGuildSettingsStickerSuccess = function showGuildSettingsStickerSuccess() {
  let obj = importDefault(3866);
  obj = { key: "GUILD_SETTINGS_STICKER_SUCCESS", IconComponent: require(4121) /* CircleInformationIcon */.CircleInformationIcon };
  const intl = require(1212) /* getSystemLocale */.intl;
  obj.content = intl.string(require(1212) /* getSystemLocale */.t["+c5xtT"]);
  obj.open(obj);
};
