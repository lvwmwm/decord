...
// Module 5038 (_createForOfIteratorHelperLoose)
import set from "set";
...
let tmp4 = (() => {
  class GuildStickers {
    constructor() {
      self = this;
      tmp = outer1_3(this, GuildStickers);
      this.actions = {
  ...
  let closure_0 = callback(async (arg0) => {
    ...
    const arr = yield callback(GuildStickers[4]).stickers(arg0).getMapEntries();
    ...
    value(promisesForBackgroundSyncToWaitOn) {
      ...
      prop.push(Promise.all(stickers.map((dataMode) => {
        if ("unavailable" === dataMode.dataMode) {
          ...
          outer2_5.verbose("Replacing " + dataMode.entities.length + " stickers for " + dataMode.guildId);
          const replaced = self.replace(dataMode.guildId, dataMode.entities, closure_1);
        } else {
          if (tmp) {
            const _HermesInternal = HermesInternal;
            outer2_5.verbose("Updating " + dataMode.updatedEntities.length + " and deleting " + dataMode.deletedEntityIds.length + " stickers for " + dataMode.guildId);
...
tmp4 = new tmp4();
const result = require("_defineProperties").fileFinishedImporting("modules/app_database/modules/GuildStickers.tsx");
