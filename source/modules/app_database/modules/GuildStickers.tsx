...
// Module 5308 (_createForOfIteratorHelperLoose)
...
importDefaultResult = new importDefaultResult("GuildStickers");
let tmp4 = () => {
  class GuildStickers {
    constructor() {
      f45029 = this;
  ...
  const dependencyMap = GuildStickers;
  let obj = { key: "getAsync" };
  let closure_0 = callback(async (arg0) => {
    const nowResult = performance.now();
    const obj = callback(closure_1[4]);
    const arr = yield callback(closure_1[4]).stickers(arg0).getMapEntries();
  ...
  obj = {
    ...
    value(unavailableGuilds) {
      ...
      const obj = callback(GuildStickers[4]);
      callback(GuildStickers[4]).stickersTransaction(arg1).deleteAllExcept(items);
    ...
    value(promisesForBackgroundSyncToWaitOn) {
      ...
      prop.push(Promise.all(stickers.map((dataMode) => {
        if ("unavailable" === dataMode.dataMode) {
          ...
          closure_5.verbose("Replacing " + dataMode.entities.length + " stickers for " + dataMode.guildId);
          const replaced = self.replace(dataMode.guildId, dataMode.entities, arg1);
        } else {
          if (tmp) {
            const _HermesInternal = HermesInternal;
            closure_5.verbose("Updating " + dataMode.updatedEntities.length + " and deleting " + dataMode.deletedEntityIds.length + " stickers for " + dataMode.guildId);
  ...
  items[9] = {
    ...
    value(arg0, arg1, arg2) {
      const obj = callback(GuildStickers[4]);
      const replaced = callback(GuildStickers[4]).stickersTransaction(arg2).replaceAll(arg0, arg1);
    }
  };
  items[10] = {
    key: "delete",
    value(arg0, arg1) {
      const obj = callback(GuildStickers[4]);
      callback(GuildStickers[4]).stickersTransaction(arg1).delete(arg0);
    }
  };
  items[11] = {
    key: "update",
    value(arg0, arg1, arg2, arg3) {
      let done;
      const stickersTransactionResult = callback(GuildStickers[4]).stickersTransaction(arg3);
      stickersTransactionResult.putAll(arg0, arg1);
      const tmp2 = callback3(arg2);
      let iter = tmp2();
      if (!iter.done) {
        do {
          let deleteResult = stickersTransactionResult.delete(arg0, iter.value);
          let iter2 = tmp2();
          iter = iter2;
          done = iter2.done;
        } while (!done);
  ...
  return callback2(GuildStickers, items);
}();
tmp4 = new tmp4();
const result = arg1(dependencyMap[5]).fileFinishedImporting("modules/app_database/modules/GuildStickers.tsx");
