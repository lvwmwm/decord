...
// Dependencies: [5, 32, 5189, 1387, 7191, 3, 5188, 1956, 7197, 7200, 1957, 2]
// Exports: isLikelyNotDelta

// Module 7190 (computeUsersAndMembers)
...
import closure_7 from "handleSelectedChannelStoreChanged" /* 7191 */;
...
prototype["computeUsersAndMembers"] = function computeUsersAndMembers(arr) {
  const self = this;
  const result = set(5188).requireSortedDescending(arr);
  const map = new Map();
  map1 = new Map();
...
prototype2["handleMessageUpdate"] = function handleMessageUpdate(message, database) {
  ...
  if (isReadableChannelIdResult) {
    isReadableChannelIdResult = set(7197).isReadableChannelId(message.message.channel_id);
    const obj = set(7197);
  }
  if (isReadableChannelIdResult) {
...
prototype2["insertStale"] = function insertStale(guildId, channel_id, item10009, database) {
  ...
  const KvMessage = set(7200).KvMessage;
...
prototype2["upsertMany"] = function upsertMany(guild_id, channelId, messages, database) {
  ...
  while (iter !== undefined) {
    let tmp3 = set;
    let tmp4 = dependencyMap;
    let KvMessage = set(7200).KvMessage;
    let tmp5 = KvMessage;
    let tmp6 = guild_id;
    let tmp7 = channelId;
    let tmp8 = nextResult;
