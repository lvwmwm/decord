...
// Dependencies: [1391, 4263, 1220, 1979, 7008, 7009, 7010, 7012, 7013, 7014, 7015, 2]

// Module 7007 (handleSelectedChannelStoreChanged)
...
function handleConnectionOpenSupplemental() {
  ...
  SaveableChannelsStore.replaceLru(prototype(7015).withFallbacks(extendedMemoryLru, 1250));
}
function handleChannelUpdate(id) {
  id = id.id;
  const isReadableChannelResult = prototype(7013).isReadableChannel(id);
...
function handleThreadUpdate(channel) {
  ...
  const id = channel.id;
  const isReadableChannelResult = prototype(7013).isReadableChannel(channel);
  let tmp2 = isReadableChannelResult;
  if (isReadableChannelResult) {
    tmp2 = id === store2.getChannelId();
  }
...
SaveableChannelsStore["mergeSnapshot"] = function mergeSnapshot(snapshot) {
  extendedMemoryLru = new prototype(7009).ExtendedMemoryLru(extendedMemoryLru.primaryCapacity, extendedMemoryLru.extendedCapacity);
  lru = new prototype(7010).Lru(lru.capacity);
  if (lastChannel == null) {
