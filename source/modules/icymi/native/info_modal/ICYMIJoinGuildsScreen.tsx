...
// Dependencies: [5, 32, 19, 17, 4185, 1862, 15375, 676, 21, 15344, 712, 5570, 9013, 4054, 4194, 9250, 589, 1416, 7974, 5145, 4721, 4189, 4604, 4154, 1236, 1581, 8198, 8189, 3894, 4399, 15359, 6679, 2]
// Exports: default
...
function SelectedServerIcon(guild) {
  ...
  if (null != guild) {
    ...
    obj[1] = guild;
    obj[2] = require(5570) /* GuildIconSizes */.GuildIconSizes.LARGE;
    let tmp3Result = tmp3(importDefault(5570), obj);
    ...
    if (tmp2) {
      ...
      tmp3Result = tmp3(require(9013) /* ServerIcon */.ServerIcon, obj);
    }
  }
  obj[1] = tmp3Result;
...
function SelectedServersRow(selectedGuilds) {
  ...
  let obj = selectedGuilds(4054);
  const fn = function _() {
    let obj = selectedGuilds(4194);
    let num = 0;
    if (selectedGuilds.length > 0) {
      num = 1;
    }
    obj = { opacity: obj.withTiming(num), height: null, marginTop: null };
    let tmpResult = tmp(4194);
    let num2 = 0;
    if (selectedGuilds.length > 0) {
      num2 = outer1_15;
    }
  ...
  obj = { withTiming: selectedGuilds(4194).withTiming, selectedGuilds, SELECTED_SERVER_SIZE_WITH_BORDER: c15, tokens: first(712) };
  fn.__closure = obj;
  ...
  obj[1] = closure_12(first(9250), obj1);
  return closure_12(first(4054).View, obj);
}
function CutoutGuildBanner(guild) {
  ...
  obj1[0] = guild(7974).CutoutShape.RoundedRect;
  const items1 = [obj1];
  obj[0] = items1;
  if (null != guild.banner) {
    ...
    let tmp11Result = tmp11(tmp12(5145), obj2);
  } else {
  ...
  obj[1] = tmp11Result;
  const items3 = [closure_12(importDefault(7974), obj), ];
  const obj4 = { style: items4, guild, size: null, animate: null };
  items4 = [, ];
  ({ guildIcon: arr5[0], guildIconBorder: arr5[1] } = tmp);
  const tmp13 = importDefault(7974);
  const tmp9 = closure_13;
  obj4[2] = guild(5570).GuildIconSizes.LARGE;
  obj4[3] = !stateFromStores;
  items3[1] = closure_12(importDefault(5570), obj4);
  obj[1] = items3;
...
function FeaturedServer(guild) {
  ...
  const items3 = [callback2(guild(4189).Text, { maxFontSizeMultiplier: 1, style: tmp.featuredServerTitle, variant: "text-md/semibold", color: "mobile-text-heading-primary", children: guild.name }), callback2(guild(4189).Text, { maxFontSizeMultiplier: 1, lineClamp: 2, variant: "text-xs/normal", color: "text-default", children: guild.description }), ];
  ...
  if (selected) {
    ...
    tmp7Result = tmp7(tmp2(4154).CircleCheckIcon, obj5);
  }
  obj4[3] = tmp7Result;
  if (stateFromStores) {
    if (!guild.loading) {
      ...
      obj4[7] = str;
      obj3[1] = tmp7(guild(4604).Button, obj4);
      items3[2] = tmp7(tmp8, obj3);
      obj[1] = items3;
      items2[1] = tmp6(tmp8, obj);
      obj[4] = items2;
      return tmp6(guild(4721).PressableHighlight, obj);
    }
  }
      ...
      try {
        ...
        if (0 === v0) {
          if (arg0 === 1) {
            ...
            obj1[0] = v0(8189).gravityJoinGuild(Array.from(outer1_3), "icymi_info_modal");
            return obj1;
          }
        } else if (1 === tmp4) {
          if (arg0 === 1) {
            ...
            const recommendedGuilds = v0(8189).getRecommendedGuilds();
            const obj9 = v0(8189);
            v0(4399).popWithKey(outer1_0(15359).ICYMI_INFO_MODAL_KEY);
            dependencyMap = 3;
          ...
          const guildChannelScores1 = v0(8189).getGuildChannelScores();
          const obj19 = v0(8189);
          const recommendedGuilds1 = v0(8189).getRecommendedGuilds();
          const obj20 = v0(8189);
          v0(4399).popWithKey(outer1_0(15359).ICYMI_INFO_MODAL_KEY);
          dependencyMap = 3;
          return { value: "HermesInternal", done: null };
