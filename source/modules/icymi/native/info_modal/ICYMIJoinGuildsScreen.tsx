...
// Dependencies: [5, 32, 19, 17, 4295, 1891, 15590, 676, 21, 15559, 712, 5779, 9704, 4036, 4304, 8526, 589, 1416, 8293, 5268, 4846, 4299, 4714, 4264, 1236, 1609, 8921, 8913, 4002, 4509, 15574, 7745, 2]
// Exports: default

// Module 15591 (SelectedServerIcon)
...
function SelectedServerIcon(guild) {
  ...
  if (null != guild) {
    ...
    obj[1] = guild;
    obj[2] = require(5779) /* GuildIconSizes */.GuildIconSizes.LARGE;
    let tmp3Result = tmp3(importDefault(5779), obj);
    const tmp12 = importDefault(5779);
  } else {
    tmp3Result = null;
    if (tmp2) {
      obj = { size: "md", color: null };
      obj[1] = importDefault(712).colors.ICON_MUTED;
      tmp3Result = tmp3(require(9704) /* ServerIcon */.ServerIcon, obj);
    }
  }
  obj[1] = tmp3Result;
...
function SelectedServersRow(selectedGuilds) {
  ...
  obj = { withTiming: selectedGuilds(4304).withTiming, selectedGuilds, SELECTED_SERVER_SIZE_WITH_BORDER: c15, tokens: first(712) };
  fn.__closure = obj;
...
function CutoutGuildBanner(guild) {
  ...
  if (null != guild.banner) {
    ...
    let tmp11Result = tmp11(tmp12(5268), obj2);
  } else {
  ...
  obj[1] = tmp11Result;
  const items3 = [closure_12(importDefault(8293), obj), ];
  const obj4 = { style: items4, guild, size: null, animate: null };
  items4 = [, ];
  ({ guildIcon: arr5[0], guildIconBorder: arr5[1] } = tmp);
  const tmp13 = importDefault(8293);
  const tmp9 = closure_13;
  obj4[2] = guild(5779).GuildIconSizes.LARGE;
  obj4[3] = !stateFromStores;
  items3[1] = closure_12(importDefault(5779), obj4);
  obj[1] = items3;
...
function FeaturedServer(guild) {
  ...
  const items3 = [callback2(guild(4299).Text, { maxFontSizeMultiplier: 1, style: tmp.featuredServerTitle, variant: "text-md/semibold", color: "mobile-text-heading-primary", children: guild.name }), callback2(guild(4299).Text, { maxFontSizeMultiplier: 1, lineClamp: 2, variant: "text-xs/normal", color: "text-default", children: guild.description }), ];
  ...
  if (selected) {
    ...
    tmp7Result = tmp7(tmp2(4264).CircleCheckIcon, obj5);
  }
  obj4[3] = tmp7Result;
  if (stateFromStores) {
    if (!guild.loading) {
      ...
      obj4[7] = str;
      obj3[1] = tmp7(guild(4714).Button, obj4);
      items3[2] = tmp7(tmp8, obj3);
      obj[1] = items3;
      items2[1] = tmp6(tmp8, obj);
      obj[4] = items2;
      return tmp6(guild(4846).PressableHighlight, obj);
    }
  }
