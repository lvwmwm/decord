...
function SelectedServerIcon(guild) {
  ...
  if (null != guild) {
    ...
    obj[1] = guild;
    obj[2] = require(5661) /* GuildIconSizes */.GuildIconSizes.LARGE;
    let tmp3Result = tmp3(importDefault(5661), obj);
...
function SelectedServersRow(selectedGuilds) {
  ...
  obj[1] = closure_12(first(8337), obj1);
  return closure_12(first(4146).View, obj);
}
function CutoutGuildBanner(guild) {
  ...
  obj1[0] = guild(8103).CutoutShape.RoundedRect;
  const items1 = [obj1];
  obj[0] = items1;
  if (null != guild.banner) {
    ...
    let tmp11Result = tmp11(tmp12(5236), obj2);
  } else {
...
function FeaturedServer(guild) {
  ...
  if (stateFromStores) {
    if (!guild.loading) {
      ...
      obj[4] = items2;
      return tmp6(guild(4812).PressableHighlight, obj);
    }
  }
      ...
      try {
        ...
        if (0 === v0) {
          if (arg0 === 1) {
            ...
            obj1[0] = v0(8835).gravityJoinGuild(Array.from(outer1_3), "icymi_info_modal");
            return obj1;
          }
        } else if (1 === tmp4) {
          if (arg0 === 1) {
          ...
          const guildChannelScores1 = v0(8835).getGuildChannelScores();
          const obj19 = v0(8835);
