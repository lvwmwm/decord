...
// Dependencies: [5, 32, 19, 17, 4247, 1862, 15442, 676, 21, 15411, 712, 5632, 9190, 4116, 4256, 9422, 589, 1416, 8091, 5207, 4783, 4251, 4666, 4216, 1236, 1581, 8505, 8496, 3956, 4461, 15426, 7654, 2]
// Exports: default

// Module 15443 (SelectedServerIcon)
...
function SelectedServerIcon(guild) {
  ...
  if (null != guild) {
    ...
    obj[1] = guild;
    obj[2] = require(5632) /* GuildIconSizes */.GuildIconSizes.LARGE;
    let tmp3Result = tmp3(importDefault(5632), obj);
    const tmp12 = importDefault(5632);
  } else {
    tmp3Result = null;
    if (tmp2) {
      obj = { size: "md", color: null };
      obj[1] = importDefault(712).colors.ICON_MUTED;
      tmp3Result = tmp3(require(9190) /* ServerIcon */.ServerIcon, obj);
    }
  }
  obj[1] = tmp3Result;
...
function SelectedServersRow(selectedGuilds) {
  ...
  let obj = selectedGuilds(4116);
  const fn = function _() {
    let obj = selectedGuilds(4256);
    let num = 0;
    if (selectedGuilds.length > 0) {
      num = 1;
    }
    obj = { opacity: obj.withTiming(num), height: null, marginTop: null };
    let tmpResult = tmp(4256);
    let num2 = 0;
    if (selectedGuilds.length > 0) {
      num2 = outer1_15;
    }
  ...
  obj = { withTiming: selectedGuilds(4256).withTiming, selectedGuilds, SELECTED_SERVER_SIZE_WITH_BORDER: c15, tokens: first(712) };
  fn.__closure = obj;
  ...
  obj[1] = closure_12(first(9422), obj1);
  return closure_12(first(4116).View, obj);
}
function CutoutGuildBanner(guild) {
  ...
  obj1[0] = guild(8091).CutoutShape.RoundedRect;
  const items1 = [obj1];
  obj[0] = items1;
  if (null != guild.banner) {
    ...
    let tmp11Result = tmp11(tmp12(5207), obj2);
  } else {
  ...
  obj[1] = tmp11Result;
  const items3 = [closure_12(importDefault(8091), obj), ];
  const obj4 = { style: items4, guild, size: null, animate: null };
  items4 = [, ];
  ({ guildIcon: arr5[0], guildIconBorder: arr5[1] } = tmp);
  const tmp13 = importDefault(8091);
  const tmp9 = closure_13;
  obj4[2] = guild(5632).GuildIconSizes.LARGE;
  obj4[3] = !stateFromStores;
  items3[1] = closure_12(importDefault(5632), obj4);
  obj[1] = items3;
...
function FeaturedServer(guild) {
  ...
  const items3 = [callback2(guild(4251).Text, { maxFontSizeMultiplier: 1, style: tmp.featuredServerTitle, variant: "text-md/semibold", color: "mobile-text-heading-primary", children: guild.name }), callback2(guild(4251).Text, { maxFontSizeMultiplier: 1, lineClamp: 2, variant: "text-xs/normal", color: "text-default", children: guild.description }), ];
  ...
  if (selected) {
    ...
    tmp7Result = tmp7(tmp2(4216).CircleCheckIcon, obj5);
  }
  obj4[3] = tmp7Result;
  if (stateFromStores) {
    if (!guild.loading) {
      ...
      obj4[7] = str;
      obj3[1] = tmp7(guild(4666).Button, obj4);
      items3[2] = tmp7(tmp8, obj3);
      obj[1] = items3;
      items2[1] = tmp6(tmp8, obj);
      obj[4] = items2;
      return tmp6(guild(4783).PressableHighlight, obj);
    }
  }
      ...
      try {
        ...
        if (0 === v0) {
          if (arg0 === 1) {
            ...
            obj1[0] = v0(8496).gravityJoinGuild(Array.from(outer1_3), "icymi_info_modal");
            return obj1;
          }
        } else if (1 === tmp4) {
          if (arg0 === 1) {
            ...
            const recommendedGuilds = v0(8496).getRecommendedGuilds();
            const obj9 = v0(8496);
            v0(4461).popWithKey(outer1_0(15426).ICYMI_INFO_MODAL_KEY);
            dependencyMap = 3;
          ...
          const guildChannelScores1 = v0(8496).getGuildChannelScores();
          const obj19 = v0(8496);
          const recommendedGuilds1 = v0(8496).getRecommendedGuilds();
          const obj20 = v0(8496);
          v0(4461).popWithKey(outer1_0(15426).ICYMI_INFO_MODAL_KEY);
          dependencyMap = 3;
          return { value: "HermesInternal", done: null };
