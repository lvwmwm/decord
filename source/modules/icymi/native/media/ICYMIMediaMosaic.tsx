...
// Dependencies: [32, 19, 17, 4277, 1372, 3921, 1903, 8821, 676, 21, 4285, 712, 4472, 691, 589, 8793, 4145, 4286, 15506, 5251, 4281, 1236, 8778, 4841, 4827, 8836, 10775, 4810, 15461, 8769, 7225, 1351, 12, 5236, 8767, 4475, 2]
// Exports: default

// Module 15505 (MediaMosaicVideo)
...
function MediaMosaicVideo(source) {
  ...
  obj[9] = isGIFV;
  return closure_15(importDefault(8793), obj);
}
function MediaMosaicImage(source) {
  ...
  obj = { withTiming: source(4286).withTiming, imageFinishedLoading: first };
  fn.__closure = obj;
  ...
  const memo = React.useMemo(() => {
    if (null != source.placeholder) {
      ...
      obj[0] = source(15506).createThumbhashImageFromPlaceholder(tmp.placeholder);
  ...
  obj[1] = callback2(first(5251), { source: memo, style: items2 });
  const items3 = [callback2(first(4145).View, obj), ];
  const obj1 = {
    source,
    style: items4,
    onLoadEnd() {
  ...
  obj1[3] = num;
  items3[1] = callback2(first(4145).Image, obj1, source.uri);
  obj2[0] = items3;
...
function OneImageRow(widthOverride) {
  ...
  const context = React.useContext(require(15461) /* context */.ICYMIContext);
...
function SingleImage(source) {
  ...
  const tmp = callback4();
  let obj = React;
  const context = React.useContext(source(15461).ICYMIContext);
  if (null == widthOverride) {
    let width;
    if (context != null) {
      width = context.width;
