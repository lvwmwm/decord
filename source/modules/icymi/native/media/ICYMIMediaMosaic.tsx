...
// Dependencies: [32, 19, 17, 4247, 1372, 3892, 1874, 8792, 676, 21, 4255, 712, 4442, 691, 589, 8764, 4116, 4256, 15468, 5221, 4251, 1236, 8749, 4811, 4797, 8807, 10720, 4780, 15423, 8740, 7198, 1351, 12, 5206, 8738, 4445, 2]
// Exports: default

// Module 15467 (MediaMosaicVideo)
...
function MediaMosaicVideo(source) {
  ...
  obj[9] = isGIFV;
  return closure_15(importDefault(8764), obj);
}
function MediaMosaicImage(source) {
  ...
  const memo = React.useMemo(() => {
    if (null != source.placeholder) {
      ...
      obj[0] = source(15468).createThumbhashImageFromPlaceholder(tmp.placeholder);
  ...
  obj[1] = callback2(first(5221), { source: memo, style: items2 });
...
function OneImageRow(widthOverride) {
  ...
  const context = React.useContext(require(15423) /* context */.ICYMIContext);
...
function SingleImage(source) {
  ...
  const tmp = callback4();
  let obj = React;
  const context = React.useContext(source(15423).ICYMIContext);
  if (null == widthOverride) {
    let width;
    if (context != null) {
      width = context.width;
