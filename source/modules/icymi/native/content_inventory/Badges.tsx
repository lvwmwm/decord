...
// Dependencies: [19, 17, 1995, 21, 4481, 709, 12938, 8066, 4477, 586, 9152, 11537, 12941, 1233, 12943, 12945, 9127, 10109, 8647, 684, 2]
...
import TimerIcon from "TimerIcon" /* 11537 */;
...
function ContentTimestamp(entry) {
  ...
  if (isEntryActiveResult) {
    ...
    obj[1] = tmp.text;
    obj[2] = tmp2(8066).formatEndedTimestamp(entry, tmp6);
    tmp7Result = tmp7(tmp2(4477).Text, obj);
    const tmp2Result = tmp2(8066);
  }
  return tmp7Result;
}
...
export const TrendingBadge = function TrendingBadge(entry) {
  ...
  if (null != trendingType) {
    tmp4 = null;
    if (trendingType !== tmp(12945).TrendingType.TRENDING_TYPE_UNSPECIFIED) {
      obj = { Icon: null, text: null, iconColor: null };
      obj[0] = tmp(9127).FireIcon;
      const intl = tmp(1233).intl;
