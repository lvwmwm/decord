...
// Dependencies: [5, 19, 17, 4297, 8833, 8821, 676, 21, 4285, 712, 589, 8837, 5685, 5688, 1236, 5689, 8836, 5354, 7225, 692, 11, 5236, 4253, 8835, 4490, 15463, 1988, 15472, 2]
// Exports: default

// Module 15462 (ICYMISettingsActionSheet)
...
export default function ICYMISettingsActionSheet() {
  ...
  const ICYMIStaffDebuggingUtilityExperiment = stateFromStoresObject(8837).ICYMIStaffDebuggingUtilityExperiment;
  ...
  if (ICYMIStaffDebuggingUtilityExperiment.useConfig({ location: "settings action sheet" }).enabled) {
    ...
    const items1 = [callback2(tmp(5689).TableSwitchRow, obj), , ];
    ...
    items1[1] = callback2(tmp(5354).TableRow, obj2);
    ...
    items1[2] = callback2(tmp(5354).TableRow, obj3);
    obj1[0] = items1;
  ...
  items2[1] = callback2(stateFromStoresObject(5354).TableRow, obj5);
  obj[2] = items2;
  const items3 = [closure_12(stateFromStoresObject(5688).TableRowGroup, obj), ];
  const tmp4 = createCacheKey();
  items3[1] = callback2(View, { style: createCacheKey().padding });
  obj4[2] = items3;
  return closure_12(stateFromStoresObject(5685).ActionSheet, obj4);
};
