...
// Dependencies: [32, 8168, 4103, 5779, 1218, 1372, 8177, 1862, 4407, 3817, 4201, 3826, 4385, 8179, 8180, 676, 8190, 687, 8181, 6154, 7906, 7911, 8173, 8191, 6686, 4409, 589, 709, 2]

// Module 8167 (filterStaffGuild)
...
function filterStaffGuild(data) {
  if (closure_19.filterStaffContent()) {
    if (obj.isGuildItem(data)) {
      if (data.data.guild_id === require(6154) /* MessageEmbedTypes */.GAME_CONTENT_GUILD_ID) {
        return true;
      } else {
...
function injectRecommendedGuildsRow() {
  items1 = items1.filter((type) => type.type !== callback(6154).ICYMIItemTypes.RECOMMENDED_GUILDS);
  items = items.filter((type) => type.type !== callback(6154).ICYMIItemTypes.RECOMMENDED_GUILDS);
  if (0 !== length.length) {
    ...
    obj[1] = require(6154) /* MessageEmbedTypes */.ICYMIItemTypes.RECOMMENDED_GUILDS;
...
function getNewUnreadItems(arr9, channelId) {
  ...
  while (iter !== undefined) {
    ...
    let tmp3 = require;
    let tmp4 = dependencyMap;
    if (nextResult.type !== require(6154) /* MessageEmbedTypes */.ICYMIItemTypes.RECOMMENDED_GUILDS) {
      ...
      if (!set.has(tmp2.id)) {
        ...
        if (tmp7) {
          let tmp8 = nextResult;
          let tmp9 = tmp2.type !== tmp3(6154).ICYMIItemTypes.MESSAGE;
          if (!tmp9) {
...
function maybeFilterChannelItems(arg0, stateFromStores1) {
  const obj = _require(8181);
  if (numberToCustomScoreResult === _require(8181).ICYMICustomScore.MUTED) {
    _require = arg0;
...
function handleReaction(colors) {
  ...
  if (null == dependencyMap[colors.messageId]) {
    return false;
  } else if (tmp3.type !== require(6154) /* MessageEmbedTypes */.ICYMIItemTypes.MESSAGE) {
    return false;
  } else {
...
prototype["getMessage"] = function getMessage(arg0) {
  ...
  if (null != dependencyMap[arg0]) {
    message = null;
    if (tmp.type === require(6154) /* MessageEmbedTypes */.ICYMIItemTypes.MESSAGE) {
      message = tmp.message;
    }
  }
  return message;
...
prototype["customMuted"] = function customMuted(id, id) {
  ...
  let tmp4 = customGuildScore === require(8181) /* generateHydrationId */.ICYMICustomScore.MUTED;
  if (!tmp4) {
    const customChannelScore = self.getCustomChannelScore(id, id);
    tmp4 = customChannelScore === require(8181) /* generateHydrationId */.ICYMICustomScore.MUTED;
  }
  return tmp4;
};
prototype["getCustomChannelScore"] = function getCustomChannelScore(guild_id, id) {
  if (null != dependencyMap2[guild_id]) {
    if (null != dependencyMap2[guild_id][id]) {
      let UNKNOWN = require(8181) /* generateHydrationId */.numberToCustomScore(dependencyMap2[guild_id][id]);
      const obj = require(8181) /* generateHydrationId */;
    }
    return UNKNOWN;
  }
  UNKNOWN = require(8181) /* generateHydrationId */.ICYMICustomScore.UNKNOWN;
};
...
const iCYMIStore = new ICYMIStore(require("dispatcher"), {
  ...
  LOAD_ICYMI_HYDRATED: function handleLoadHydratedItems(arg0) {
    ...
    set.delete(_require(8181).generateHydrationId(startingIndex, endingIndex));
  },
  ...
  MESSAGE_REACTION_ADD_MANY: function handleReactionBatch(arg0) {
    if (null == dependencyMap[arg0.messageId]) {
      return false;
    } else if (tmp2.type !== require(6154) /* MessageEmbedTypes */.ICYMIItemTypes.MESSAGE) {
      return false;
    } else {
  ...
  MESSAGE_REACTION_REMOVE_ALL: function handleRemoveAllReactions(arg0) {
    let tmp2 = null != tmp;
    if (tmp2) {
      const tmp5 = tmp.type === require(6154) /* MessageEmbedTypes */.ICYMIItemTypes.MESSAGE;
      if (tmp5) {
  ...
  MESSAGE_REACTION_REMOVE_EMOJI: function handleRemoveEmojiReactions(arg0) {
    let tmp3 = null != tmp2;
    if (tmp3) {
      const tmp6 = tmp2.type === require(6154) /* MessageEmbedTypes */.ICYMIItemTypes.MESSAGE;
      if (tmp6) {
        const message = tmp2.message;
