...
// Function ID: 64990
// Name: listItemToType
// Dependencies: [8146, 653, 7884, 6138, 675, 2]

// Module 8170 (listItemToType)
...
function listItemToType(data) {
  ...
  if ("end" === kind) {
    ...
    if ("message" === kind) {
      ...
      if (data.data.content.content_type === require(7884) /* ContentInventoryEntryType */.ContentInventoryEntryType.CUSTOM_STATUS) {
...
function dehydratedItemToType(type) {
  type = type.type;
  if (require(6138) /* MessageEmbedTypes */.ICYMIItemTypes.MESSAGE === type) {
    let str5 = "message";
    if (type.data.channel_type === constants.GUILD_ANNOUNCEMENT) {
      str5 = "announcement";
    }
    return str5;
  } else if (require(6138) /* MessageEmbedTypes */.ICYMIItemTypes.ACTIVITY === type) {
    return "hotwheels_gaming_activity";
  } else if (require(6138) /* MessageEmbedTypes */.ICYMIItemTypes.CUSTOM_STATUS === type) {
    return "hotwheels_custom_status";
  } else if (require(6138) /* MessageEmbedTypes */.ICYMIItemTypes.GUILD_EVENT === type) {
    return "guild_event";
  } else if (require(6138) /* MessageEmbedTypes */.ICYMIItemTypes.RECOMMENDED_GUILDS === type) {
    return "recommended_guilds";
  }
}
({ ChannelTypes: closure_4, AnalyticEvents: closure_5 } = ME);
...
export const ICYMIAnalytics = {
  ...
  trackItemShortImpression(viewableItems, viewableFeedItemsArray, stateFromStores) {
    let obj = importDefault(675);
    obj = { load_id: store.getLoadId(), home_session_id: "gravity", feed_item_ids: viewableItems.map((item) => item.item.id), feed_item_types: viewableItems.map((item) => outer1_6(item.item)), num_items: viewableItems.length, all_feed_item_ids: viewableFeedItemsArray.map((id) => id.id), all_feed_item_types: viewableFeedItemsArray.map((type) => type.type), num_all_items: viewableFeedItemsArray.length, all_feed_item_indices: viewableFeedItemsArray.map((arg0, arg1) => arg1), feed_version: stateFromStores, version: 3 };
  ...
  trackFeedFeedbackPromptViewed() {
    importDefault(675).track(closure_5.HOME_FEEDBACK_PROMPT_VIEWED);
  },
