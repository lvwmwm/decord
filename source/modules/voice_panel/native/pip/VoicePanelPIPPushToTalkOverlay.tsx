...
// Dependencies: [32, 19, 17, 12205, 21, 4218, 5544, 1296, 4481, 709, 9625, 17014, 4935, 17009, 6595, 17090, 2]
// Exports: default

// Module 17089 (VoicePanelPIPPushToTalkOverlay)
import ThemesDefault from "Themes" /* 709 */;
import setDefault from "set" /* 5544 */;
import closure_3 from "_slicedToArray" /* 32 */;
import closure_4 from "noop" /* 19 */;
import { PUSH_TO_TALK_PIP_PHYSICS } from "VoicePanelModes" /* 12205 */;
...
export default function VoicePanelPIPPushToTalkOverlay() {
  ...
  const items1 = [
    ...
    BLACK.useCallback((current) => {
      if (current !== first.current) {
        ...
        sharedValue(9625).setPushToTalkState(current);
        const result = sharedValue.set(current);
        const obj = sharedValue(9625);
      }
    }, items)
  ];
  ...
  let fn = function o() {
    ...
    obj = { right: sharedValue(4935).withSpring(num, closure_1_5), bottom: null, transform: null, backgroundColor: null };
    const obj3 = sharedValue(4935);
    const tmp = sharedValue;
    obj[1] = sharedValue(4935).withSpring(num, closure_1_5);
    const obj4 = sharedValue(4935);
    let num2 = 1;
    if (obj.get()) {
      num2 = 1.5;
    }
    obj = { scale: sharedValue(4935).withSpring(num2, tmp3) };
    const items = [obj];
    obj[2] = items;
    const obj5 = sharedValue(4935);
    let str = "rgba(0, 0, 0, 0.54)";
    if (obj.get()) {
      str = WHITE;
    }
    obj[3] = tmp(4935).withSpring(str, closure_1_5);
    return obj;
  };
  obj = { isPushingToTalk: first, EXPANDED_ICON_SIZE: 48, BASE_ICON_SIZE: 32, withSpring: sharedValue(4935).withSpring, PUSH_TO_TALK_PIP_PHYSICS, white: WHITE };
  fn.__closure = obj;
  fn.__workletHash = 3936373516983;
  fn.__initData = closure_13;
  const animatedStyle = obj2.useAnimatedStyle(fn);
  let obj4 = sharedValue(4218);
  class P {
    constructor() {
  ...
  obj = { withSpring: sharedValue(4935).withSpring, isPushingToTalk: first, black: BLACK, white: WHITE, PUSH_TO_TALK_PIP_PHYSICS };
  P.__closure = obj;
  ...
  obj1 = { withSpring: sharedValue(4935).withSpring, isPushingToTalk: first, PUSH_TO_TALK_PIP_PHYSICS, getVoicePanelPIPBorderRadius: sharedValue(17009).getVoicePanelPIPBorderRadius, pipState: pIPState };
  H.__closure = obj1;
  H.__workletHash = 450590017248;
  H.__initData = closure_15;
  const items2 = [tmp4[1]];
  const animatedStyle2 = sharedValue(4218).useAnimatedStyle(H);
  obj2 = { children: null };
  const items3 = [tmp2.overlay, animatedStyle2];
  const memo = BLACK.useMemo(() => {
    const Gesture = sharedValue(6595).Gesture;
    const Gesture2 = sharedValue(6595).Gesture;
    ...
    let obj = { runOnJS: sharedValue(4218).runOnJS, handlePushToTalk: dependencyMap };
    fn.__closure = obj;
    fn.__workletHash = 13736796804739;
    fn.__initData = closure_1_16;
    const maxDistanceResult = Gesture2.Tap().maxDistance(30);
    const Gesture3 = sharedValue(6595).Gesture;
    ...
    obj = { runOnJS: sharedValue(4218).runOnJS, handlePushToTalk: dependencyMap };
    fn2.__closure = obj;
    ...
    fn3.__closure = { runOnJS: sharedValue(4218).runOnJS, handlePushToTalk: dependencyMap };
  ...
  const obj7 = sharedValue(4218);
  obj4[2] = callback(closure_10, { style: animatedStyle1, size: sharedValue(1296).Icon.Sizes.SMALL_20, source: first(17090), disableColor: true });
  obj3[1] = callback(closure_9, obj4);
  items4[1] = callback(sharedValue(6595).GestureDetector, obj3);
  obj2[0] = items4;
