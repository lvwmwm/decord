...
// Dependencies: [4461, 4467, 1218, 11757, 16582, 4466, 11758, 4879, 9512, 13638, 712, 2]
...
// Module 16581 (PIPReferenceDimensions)
import ThemesDefault from "Themes" /* 712 */;
import CONFIG_NEVER_ANIMATE from "CONFIG_NEVER_ANIMATE" /* 4879 */;
import canRenderParticipantVideo from "canRenderParticipantVideo" /* 9512 */;
  ...
  if (type1 !== ParticipantTypes.ACTIVITY) {
    if (!showSecondaryPIP.showSecondaryPIP) {
      ...
      for (const item10060 of activityParticipants) {
        ...
        if (participants.some((userId) => callback(13638).isActivityParticipantCurrentUserCurrentSession(userId))) {
          let tmp18 = item10060;
  ...
  if (null != participant2) {
    if (!blockList.has(participant2.id)) {
      ...
      if (ParticipantTypes.STREAM === type) {
        ...
        if (participants2.some((userId) => callback(13638).isActivityParticipantCurrentUserCurrentSession(userId))) {
