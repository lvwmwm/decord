...
// Dependencies: [4460, 4466, 1218, 11914, 16516, 4465, 11915, 4878, 10866, 13611, 712, 2]
...
// Module 16515 (PIPReferenceDimensions)
import ThemesDefault from "Themes" /* 712 */;
import CONFIG_NEVER_ANIMATE from "CONFIG_NEVER_ANIMATE" /* 4878 */;
import canRenderParticipantVideo from "canRenderParticipantVideo" /* 10866 */;
  ...
  if (type1 !== ParticipantTypes.ACTIVITY) {
    if (!showSecondaryPIP.showSecondaryPIP) {
      ...
      for (const item10060 of activityParticipants) {
        ...
        if (participants.some((userId) => callback(13611).isActivityParticipantCurrentUserCurrentSession(userId))) {
          let tmp18 = item10060;
  ...
  if (null != participant2) {
    if (!blockList.has(participant2.id)) {
      ...
      if (ParticipantTypes.STREAM === type) {
        ...
        if (participants2.some((userId) => callback(13611).isActivityParticipantCurrentUserCurrentSession(userId))) {
