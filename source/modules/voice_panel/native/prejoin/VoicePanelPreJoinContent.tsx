...
import { StyleSheet } from "CARD_SIZE";
...
import ME from "ME";
import { VoiceChannelWarningSurfaces as closure_23 } from "GdmWarningMedium";
...
function StreamPreview(channelId) {
  ...
  const stateFromStores = obj1.useStateFromStores(items2, () => outer1_13.can(outer1_22.CONNECT, set));
...
function ActivityInfo(activity) {
  ...
  const fn2 = function c(arg0, arg1) {
    if (arg0 !== arg1) {
      activity(first[38]).runOnJS(c5)(arg0 - 2 * (outer1_19 + 16));
...
function RoomMembersSection(title) {
  ...
  obj[2] = (() => {
    ...
    while (iter !== undefined) {
      let tmp4 = outer1_15;
      let tmp3 = nextResult;
      let user = outer1_15.getUser(nextResult);
      let tmp6 = user;
      if (null != user) {
        let tmp7 = outer1_25;
        let tmp8 = outer1_0;
        ...
        obj[2] = closure_2;
        let tmp13 = outer1_1;
        let obj2 = outer1_1(outer1_2[49]);
        obj[3] = obj2.getName(closure_2, closure_1, tmp6);
        let tmp14 = nextResult;
        let arr = items.push(outer1_25(outer1_0(outer1_2[48]).MemberRowItem, obj, tmp3));
      }
      continue;
    }
    return items;
...
function RoomMembers(members) {
  ...
  const children = [
    ...
    streamingMembers.map((arg0) => {
      ...
      let tmp2;
      [tmp, tmp2] = arg0;
      return outer1_25(outer1_29, { channelId, voiceState: tmp, stream: tmp2 }, tmp2.ownerId);
    }),
  ,
  ,
          ...
          for (const item10007 of members) {
            ...
            if (items.length >= first) {
              ...
              if (!hasItem) {
                ...
                let arr = items.push(outer1_25(members(ignoredMembers[48]).MemberRowItem, obj, tmp.user.id));
              }
              continue;
            }
            return items;
...
function PreJoinTransitioner(transitionState) {
  ...
  let fn = function l() {
    ...
    const fn = function o() {
      ...
      if (flag) {
        flag = closure_0 === outer1_0(outer1_2[59]).TransitionStates.YEETED;
      }
      if (flag) {
        outer1_0(outer1_2[38]).runOnJS(closure_1)();
        const obj = outer1_0(outer1_2[38]);
      }
    };
    ...
    fn.__initData = outer1_40;
    obj[0] = tmp4Result.withSpring(tmp4Result.interpolate(num, [0, 1], [0, 400]), outer1_18, "respect-motion-settings", fn);
    const items = [obj];
    obj[2] = items;
    return obj;
  };
...
let closure_37 = importAllResult.memo((channelId) => {
  ...
  if (obj.useStateFromStores(items, () => {
    ...
    const voiceStateForSession = outer1_16.getVoiceStateForSession(id, outer1_7.getRemoteSessionId());
    ...
    if (!tmp6) {
      ...
      if (tmp8) {
        ...
        tmp8 = null != outer1_14.getSessionById(str);
      }
      tmp6 = tmp8;
    }
    return tmp6;
...
let closure_38 = importAllResult.memo(function VoicePanelPreJoinContentInner(members) {
  ...
  const effect = analyticsLocations.useEffect(() => {
    ...
    obj.track(outer1_20.VIEW_VOICE_CHANNEL, obj);
  }, items1);
  ...
  items3[2] = activities.map((activity) => outer1_25(outer1_33, { activity, analyticsLocations }, activity.launchId));
...
const memoResult = importAllResult.memo(function VoicePanelPreJoinWrapper() {
  ...
  const stateFromStores = obj.useStateFromStores(items, () => {
    if (!closure_2) {
      const voiceStatesForChannelAlt = outer1_17.getVoiceStatesForChannelAlt(tmp2, guildId);
      ...
      obj[2] = (() => {
        const items = [];
        if (outer2_12.supports(outer2_24.VIDEO)) {
          ...
          while (iter !== undefined) {
            ...
            if (nextResult.voiceState.selfStream) {
              ...
              let tmp9 = nextResult;
              let tmp10 = outer1_1;
              let streamForUser = outer2_9.getStreamForUser(tmp7.user.id, outer1_1);
