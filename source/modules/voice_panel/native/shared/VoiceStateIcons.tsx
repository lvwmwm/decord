...
// Dependencies: [31, 27, 33, 4131, 689, 8794, 8795, 8797, 8799, 8801, 1327, 1273, 8463, 8803, 8804, 2]

// Module 8793 (VideoDisabledSvgIcon)
...
function VideoDisabledSvgIcon(size) {
  ...
  const items1 = [callback(require(8463) /* inlineStyles */.Path, obj1), , ];
  ...
  items1[1] = callback(require(8463) /* inlineStyles */.Path, obj2);
  items1[2] = callback(require(8463) /* inlineStyles */.Path, { d: "M21.2785 2.70712C20.888 2.31659 20.2549 2.31659 19.8643 2.70711L2.70711 19.8643C2.31658 20.2549 2.31658 20.888 2.70711 21.2785L2.72146 21.2929C3.11199 21.6834 3.74515 21.6834 4.13567 21.2929L21.2929 4.13568C21.6834 3.74515 21.6834 3.11199 21.2929 2.72147L21.2785 2.70712Z", fill: tmp5.redTint.tintColor });
  obj["children"] = items1;
...
const memoResult = importAllResult.memo((arg0) => {
  ...
  const tmp = callback3();
  if (require(8794) /* isStableVoiceStateEqual */.MuteDeafenIconState.DEAFENED_SERVER === state) {
    let obj = {};
    const items = [style, ];
    let redTint = null;
    if (!alwaysWhite) {
    ...
    obj.style = items;
    obj.size = "xs";
    return callback(require(8795) /* HeadphonesDenyIcon */.HeadphonesDenyIcon, obj);
  } else if (require(8794) /* isStableVoiceStateEqual */.MuteDeafenIconState.DEAFENED === state) {
    obj = {};
    const items1 = [style];
    obj.style = items1;
    obj.size = "xs";
    return callback(require(8797) /* HeadphonesSlashIcon */.HeadphonesSlashIcon, obj);
  } else if (require(8794) /* isStableVoiceStateEqual */.MuteDeafenIconState.MUTED_SERVER === state) {
    const obj1 = {};
    const items2 = [style, ];
    let redTint1 = null;
    if (!alwaysWhite) {
    ...
    obj1.style = items2;
    obj1.size = "xs";
    return callback(require(8799) /* MicrophoneDenyIcon */.MicrophoneDenyIcon, obj1);
  } else if (require(8794) /* isStableVoiceStateEqual */.MuteDeafenIconState.MUTED_LOCAL === state) {
    const obj2 = {};
    const items3 = [style];
    obj2.style = items3;
    obj2.size = "xs";
    return callback(require(8799) /* MicrophoneDenyIcon */.MicrophoneDenyIcon, obj2);
  } else if (require(8794) /* isStableVoiceStateEqual */.MuteDeafenIconState.MUTED === state) {
    const obj3 = {};
    const items4 = [style];
    obj3.style = items4;
    obj3.size = "xs";
    return callback(require(8801) /* MicrophoneSlashIcon */.MicrophoneSlashIcon, obj3);
  } else {
...
const memoResult1 = importAllResult.memo((state) => {
  ...
  if (require(8794) /* isStableVoiceStateEqual */.VideoIconState.VIDEO_DISABLED_LOCAL_AUTO === state) {
    obj = {};
    const merged1 = Object.assign(merged);
    obj["source"] = importDefault(8803);
    const items = [merged.style, tmp3.noTint];
    obj["style"] = items;
    return callback(require(1273) /* Button */.Icon, obj);
  } else if (require(8794) /* isStableVoiceStateEqual */.VideoIconState.VIDEO_DISABLED_LOCAL === state) {
    const obj1 = {};
    const merged2 = Object.assign(merged);
    return callback(VideoDisabledSvgIcon, obj1);
  } else if (require(8794) /* isStableVoiceStateEqual */.VideoIconState.VIDEO_ACTIVE === state) {
    const obj2 = {};
    const merged3 = Object.assign(merged);
    obj2["source"] = importDefault(8804);
