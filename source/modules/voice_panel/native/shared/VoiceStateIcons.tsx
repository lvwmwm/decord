...
// Dependencies: [19, 17, 21, 4303, 712, 9185, 9186, 9188, 9190, 9192, 1370, 1297, 7837, 9194, 9195, 2]

// Module 9184 (VideoDisabledSvgIcon)
...
function VideoDisabledSvgIcon(size) {
  ...
  items1[2] = callback(require(7837) /* inlineStyles */.Path, obj);
  obj.children = items1;
...
const memoResult = importAllResult.memo((arg0) => {
  ...
  const tmp = callback3();
  if (require(9185) /* isStableVoiceStateEqual */.MuteDeafenIconState.DEAFENED_SERVER === state) {
    const items = [style, ];
    let redTint = null;
    if (!alwaysWhite) {
      redTint = tmp.redTint;
    ...
    items[1] = redTint;
    obj[0] = items;
    return callback(tmp2(9186).HeadphonesDenyIcon, obj);
  } else if (tmp2(9185).MuteDeafenIconState.DEAFENED === state) {
    obj = { style: null, size: "xs" };
    const items1 = [style];
    obj[0] = items1;
    return callback(tmp2(9188).HeadphonesSlashIcon, obj);
  } else if (tmp2(9185).MuteDeafenIconState.MUTED_SERVER === state) {
    const items2 = [style, ];
    let redTint1 = null;
    if (!alwaysWhite) {
    ...
    items2[1] = redTint1;
    obj1[0] = items2;
    return callback(tmp2(9190).MicrophoneDenyIcon, obj1);
  } else if (tmp2(9185).MuteDeafenIconState.MUTED_LOCAL === state) {
    const obj2 = { style: null, size: "xs" };
    const items3 = [style];
    obj2[0] = items3;
    return callback(tmp2(9190).MicrophoneDenyIcon, obj2);
  } else if (tmp2(9185).MuteDeafenIconState.MUTED === state) {
    const obj3 = { style: null, size: "xs" };
    const items4 = [style];
    obj3[0] = items4;
    return callback(tmp2(9192).MicrophoneSlashIcon, obj3);
  } else {
...
const memoResult1 = importAllResult.memo((state) => {
  ...
  if (require(9185) /* isStableVoiceStateEqual */.VideoIconState.VIDEO_DISABLED_LOCAL_AUTO === state) {
    let obj = {};
    const merged1 = Object.assign(merged);
    obj.source = importDefault(9194);
    const items = [merged.style, tmp2.noTint];
    obj.style = items;
    return callback(tmp3(1297).Icon, obj);
  } else if (tmp3(9185).VideoIconState.VIDEO_DISABLED_LOCAL === state) {
    obj = {};
    const merged2 = Object.assign(merged);
    return callback(VideoDisabledSvgIcon, obj);
  } else if (tmp3(9185).VideoIconState.VIDEO_ACTIVE === state) {
    const obj1 = {};
    const merged3 = Object.assign(merged);
    obj1.source = importDefault(9195);
