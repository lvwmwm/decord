...
// Function ID: 16930
// Name: NOOP
// Dependencies: [32, 19, 17, 4494, 8604, 4729, 12069, 12067, 4499, 12070, 21, 3, 4928, 4478, 709, 4217, 1623, 12068, 1627, 5661, 9227, 11215, 16930, 4446, 16923, 4197, 12073, 9665, 16928, 16916, 10955, 9596, 7358, 7808, 5962, 5982, 702, 16931, 5949, 16932, 16933, 9479, 16934, 16936, 16967, 16996, 17000, 17005, 2]
...
import closure_8 from "module_8604" /* 8604 */;
...
import { POP_RESISTANCE } from "MIN_PAN_GESTURE_MOVE" /* 12070 */;
...
function AnimatedWrapper(children) {
  ...
  let animatedStyle;
  let animatedStyle1;
  obj = wrapperOffset(9665);
  height = obj.useGlobalStatusIndicatorState().height;
  const tmp3 = callback3();
  dependencyMap = tmp3;
  obj1 = connected;
  const context = connected.useContext(height(12068));
  ...
  obj = { cheapWorkletShallowEqual: wrapperOffset(9227).cheapWorkletShallowEqual, VoicePanelModes: animatedStyle1, wrapperDimensions, updateSharedValueIfChanged: height(11215), wrapperOffset, getMaxPanelWidth: wrapperOffset(16916).getMaxPanelWidth, getPanelX: wrapperOffset(16916).getPanelX, roundToNearestPixel: height(10955), windowDimensions };
  fn3.__closure = obj;
  ...
  obj = { useReducedMotion, wrapperDimensions, wrapperOffset, connected, mode, VoicePanelModes: animatedStyle1, runOnJS: wrapperOffset(4217).runOnJS, updateSourceTrackingView: height(9596).updateSourceTrackingView, withSpring: wrapperOffset(4928).withSpring, DRAWER_SPRING_PHYSICS_GESTURE_ACTIVE: closure_17, DRAWER_SIZE_PHYSICS: obj2 };
  C.__closure = obj;
  ...
  obj1 = { computeBorderRadii, mode, connected, wrapperDimensions, withSpring: wrapperOffset(4928).withSpring, BORDER_RADIUS_PHYSICS: windowDimensions, VoicePanelModes: animatedStyle1, styles: tmp3 };
  M.__closure = obj1;
  M.__workletHash = 1768699814221;
  M.__initData = closure_60;
  animatedStyle1 = obj8.useAnimatedStyle(M);
  if (!wrapperOffset(7358).isStable) {
  ...
  obj7[2] = callback2(height(7808), obj8);
...
let closure_68 = importAllResult.memo((onPress) => {
  ...
  obj = { withSpring: opacity(4928).withSpring, opacity, DrawerShadeOpacityPhysics: closure_66 };
  fn.__closure = obj;
