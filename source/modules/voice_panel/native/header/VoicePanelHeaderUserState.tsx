...
// Dependencies: [19, 4268, 21, 4116, 8557, 4255, 712, 16031, 16029, 9072, 5648, 11529, 589, 4256, 2]

// Module 16030 (useVoicePanelHeaderUserStateIcons)
...
function useVoicePanelHeaderUserStateIcons(participant, guildId, userIcons) {
  ...
  let obj = require(16029) /* areStableParticipantsEqual */;
  let id;
  if (obj.isStableParticipantWithUser(participant)) {
    id = participant.user.id;
  }
  const tmp4 = importDefault(16031);
  const items = [];
  const iter = importDefault(16031)(type, id, guildId)[Symbol.iterator]();
  ...
  while (iter !== undefined) {
    ...
    if (nextResult.type === require(16031) /* useVoicePanelCardUserStateIcons */.VoicePanelCardUserStateIconType.USER_VIDEO_ICON) {
      let tmp29 = jsx;
      let tmp30 = jsx;
      let tmp31 = tmp10;
      let tmp32 = tmp12;
      ...
      obj[2] = jsx(tmp11(9072).VideoIcon, { style: null, state: null });
      let arr = items.push(jsx(tmp11(8557).BackgroundBlurView, { style: null, state: null }, "video"));
    }
    let tmp14 = nextResult;
    let tmp15 = tmp10;
    let tmp16 = tmp12;
    if (tmp9.type === tmp11(16031).VoicePanelCardUserStateIconType.MUTE_DEAFEN_ICON) {
      let tmp35 = jsx;
      let tmp37 = tmp10;
      let tmp38 = tmp12;
      let tmp36 = jsx;
      ...
      obj1[2] = tmp36(tmp11(9072).MuteDeafenIcon, obj2);
      arr = items.push(tmp35(tmp11(8557).BackgroundBlurView, obj1, "mute-deafen"));
    }
    continue;
  }
  if (0 !== items.length) {
    ...
    obj3[0] = items2;
    obj3[1] = items;
    return jsx(importDefault(5648), { style: null, children: null });
  }
  const tmp4Result = importDefault(16031)(type, id, guildId);
}
let c3 = importAllResult;
let closure_6 = require("module_4116").createAnimatedComponent(require("TwinButtons").BackgroundBlurView);
...
const memoResult = importAllResult.memo(function VoicePanelHeaderUserState(isHeaderHidden) {
  ...
  const tmp4 = useVoicePanelHeaderUserStateIcons(channelId(16029)(stateFromStores, channelId, guildId), guildId);
  isHeaderHidden(4116);
  const fn = function h() {
  ...
  obj = { withTiming: isHeaderHidden(4256).withTiming, isHeaderHidden, OPACITY_TIMING: closure_7 };
  fn.__closure = obj;
