...
// Dependencies: [19, 4357, 21, 4083, 8708, 4344, 712, 16254, 16252, 9224, 5820, 11705, 589, 4345, 2]

// Module 16253 (useVoicePanelHeaderUserStateIcons)
...
function useVoicePanelHeaderUserStateIcons(participant, guildId, userIcons) {
  ...
  let obj = require(16252) /* areStableParticipantsEqual */;
  let id;
  if (obj.isStableParticipantWithUser(participant)) {
    id = participant.user.id;
  }
  const tmp4 = importDefault(16254);
  const items = [];
  const iter = importDefault(16254)(type, id, guildId)[Symbol.iterator]();
  ...
  while (iter !== undefined) {
    ...
    if (nextResult.type === require(16254) /* useVoicePanelCardUserStateIcons */.VoicePanelCardUserStateIconType.USER_VIDEO_ICON) {
      let tmp29 = jsx;
      let tmp30 = jsx;
      let tmp31 = tmp10;
      let tmp32 = tmp12;
      ...
      obj[2] = jsx(tmp11(9224).VideoIcon, { style: null, state: null });
      let arr = items.push(jsx(tmp11(8708).BackgroundBlurView, { style: null, state: null }, "video"));
    }
    let tmp14 = nextResult;
    let tmp15 = tmp10;
    let tmp16 = tmp12;
    if (tmp9.type === tmp11(16254).VoicePanelCardUserStateIconType.MUTE_DEAFEN_ICON) {
      let tmp35 = jsx;
      let tmp37 = tmp10;
      let tmp38 = tmp12;
      let tmp36 = jsx;
      ...
      obj1[2] = tmp36(tmp11(9224).MuteDeafenIcon, obj2);
      arr = items.push(tmp35(tmp11(8708).BackgroundBlurView, obj1, "mute-deafen"));
    }
    continue;
  }
  if (0 !== items.length) {
    ...
    obj3[0] = items2;
    obj3[1] = items;
    return jsx(importDefault(5820), { style: null, children: null });
  }
  const tmp4Result = importDefault(16254)(type, id, guildId);
}
let c3 = importAllResult;
let closure_6 = require("module_4083").createAnimatedComponent(require("TwinButtons").BackgroundBlurView);
...
const memoResult = importAllResult.memo(function VoicePanelHeaderUserState(isHeaderHidden) {
  ...
  const tmp4 = useVoicePanelHeaderUserStateIcons(channelId(16252)(stateFromStores, channelId, guildId), guildId);
  isHeaderHidden(4083);
  const fn = function h() {
  ...
  obj = { withTiming: isHeaderHidden(4345).withTiming, isHeaderHidden, OPACITY_TIMING: closure_7 };
  fn.__closure = obj;
