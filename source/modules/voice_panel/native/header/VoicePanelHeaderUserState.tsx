...
// Dependencies: [19, 4461, 21, 4185, 8906, 4445, 712, 16600, 16598, 9729, 5958, 11756, 589, 4446, 2]

// Module 16599 (useVoicePanelHeaderUserStateIcons)
import ThemesDefault from "Themes" /* 712 */;
import setDefault from "set" /* 5958 */;
import areStableParticipantsEqual from "areStableParticipantsEqual" /* 16598 */;
import useVoicePanelCardUserStateIcons from "useVoicePanelCardUserStateIcons" /* 16600 */;
import useVoicePanelCardUserStateIconsDefault from "useVoicePanelCardUserStateIcons" /* 16600 */;
...
function useVoicePanelHeaderUserStateIcons(participant, guildId, userIcons) {
  ...
  while (iter !== undefined) {
    ...
    if (nextResult.type === useVoicePanelCardUserStateIcons.VoicePanelCardUserStateIconType.USER_VIDEO_ICON) {
      ...
      obj[2] = jsx(tmp11(9729).VideoIcon, { style: null, state: null });
      let arr = items.push(jsx(tmp11(8906).BackgroundBlurView, { style: null, state: null }, "video"));
    }
    let tmp14 = nextResult;
    let tmp15 = tmp10;
    let tmp16 = tmp12;
    if (tmp9.type === tmp11(16600).VoicePanelCardUserStateIconType.MUTE_DEAFEN_ICON) {
      let tmp35 = jsx;
      let tmp37 = tmp10;
      let tmp38 = tmp12;
      let tmp36 = jsx;
      ...
      obj1[2] = tmp36(tmp11(9729).MuteDeafenIcon, obj2);
      arr = items.push(tmp35(tmp11(8906).BackgroundBlurView, obj1, "mute-deafen"));
    }
    continue;
  }
  if (0 !== items.length) {
...
const memoResult = importAllResult.memo(function VoicePanelHeaderUserState(isHeaderHidden) {
  ...
  const tmp4 = useVoicePanelHeaderUserStateIcons(channelId(16598)(stateFromStores, channelId, guildId), guildId);
  isHeaderHidden(4185);
  const fn = function h() {
  ...
  obj = { withTiming: isHeaderHidden(4446).withTiming, isHeaderHidden, OPACITY_TIMING: closure_7 };
  fn.__closure = obj;
