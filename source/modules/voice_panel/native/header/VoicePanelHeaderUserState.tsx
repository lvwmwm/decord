...
// Dependencies: [19, 4298, 21, 4146, 8585, 4285, 712, 16051, 16049, 9100, 5663, 11557, 589, 4286, 2]

// Module 16050 (useVoicePanelHeaderUserStateIcons)
...
function useVoicePanelHeaderUserStateIcons(participant, guildId, userIcons) {
  ...
  let obj = require(16049) /* areStableParticipantsEqual */;
  let id;
  if (obj.isStableParticipantWithUser(participant)) {
    id = participant.user.id;
  }
  const tmp4 = importDefault(16051);
  const items = [];
  const iter = importDefault(16051)(type, id, guildId)[Symbol.iterator]();
  ...
  while (iter !== undefined) {
    ...
    if (nextResult.type === require(16051) /* useVoicePanelCardUserStateIcons */.VoicePanelCardUserStateIconType.USER_VIDEO_ICON) {
      let tmp29 = jsx;
      let tmp30 = jsx;
      let tmp31 = tmp10;
      let tmp32 = tmp12;
      ...
      obj[2] = jsx(tmp11(9100).VideoIcon, { style: null, state: null });
      let arr = items.push(jsx(tmp11(8585).BackgroundBlurView, { style: null, state: null }, "video"));
    }
    let tmp14 = nextResult;
    let tmp15 = tmp10;
    let tmp16 = tmp12;
    if (tmp9.type === tmp11(16051).VoicePanelCardUserStateIconType.MUTE_DEAFEN_ICON) {
      let tmp35 = jsx;
      let tmp37 = tmp10;
      let tmp38 = tmp12;
      let tmp36 = jsx;
      ...
      obj1[2] = tmp36(tmp11(9100).MuteDeafenIcon, obj2);
      arr = items.push(tmp35(tmp11(8585).BackgroundBlurView, obj1, "mute-deafen"));
    }
    continue;
  }
  if (0 !== items.length) {
  ...
  const tmp4Result = importDefault(16051)(type, id, guildId);
}
...
const memoResult = importAllResult.memo(function VoicePanelHeaderUserState(isHeaderHidden) {
  ...
  const tmp4 = useVoicePanelHeaderUserStateIcons(channelId(16049)(stateFromStores, channelId, guildId), guildId);
  isHeaderHidden(4146);
  const fn = function h() {
