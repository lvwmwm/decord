...
// Dependencies: [19, 1301, 21, 4445, 712, 4293, 11629, 11546, 5958, 16642, 1896, 9374, 7201, 9542, 8429, 5020, 4172, 16641, 11575, 1297, 16645, 16643, 4638, 12230, 4441, 11633, 1236, 2]
// Exports: default

// Module 16644 (ActivityActionOverlay)
...
import isTestModeForApplication2 from "isTestModeForApplication" /* 8429 */;
...
import useActivityAction from "useActivityAction" /* 11546 */;
import useActivityActionDefault from "useActivityAction" /* 11546 */;
import useLaunchingActivityButtonStateDefault from "useLaunchingActivityButtonState" /* 11629 */;
import ParticipantsSummaryDefault from "ParticipantsSummary" /* 16642 */;
...
function ActivityActionOverlay(arg0) {
  ...
  if (useActivityAction.ActivityAction.JOIN !== action) {
    if (tmp4(11546).ActivityAction.LEAVE !== action) {
      return null;
    }
  }
  let tmp8 = action === tmp4(11546).ActivityAction.LEAVE;
  if (tmp8) {
    let obj = { style: null };
    obj[0] = tmp3.ongoingActivityJoinedContainer;
    tmp8 = callback(tmp(5958), obj);
  }
  const items = [tmp8, ];
  id = undefined;
...
function ParticipantsText(arg0) {
  ...
  obj = { source: tmp2(12230), size: Button.Icon.Sizes.EXTRA_SMALL, color: "white" };
  ...
  if (action === useActivityAction.ActivityAction.START) {
    ...
    let itemSubtitleForMaxPlayersShort = tmp9(11633).getItemSubtitleForMaxPlayersShort(num2);
    const tmp9Result = tmp9(11633);
  } else {
...
export default function ActivityShelfItem(arg0) {
  ...
  if (!disableBadges) {
    const items = [tmp8(11546).ActivityAction.LEAVE, tmp8(11546).ActivityAction.JOIN];
  ...
  obj = { activeOpacity: 0.7, onPress: onActivityItemSelected2, disabled: activityAction === tmp8(11546).ActivityAction.LEAVE, androidRippleConfig: ANDROID_FOREGROUND_RIPPLE, style: items1, children: null };
  items1 = [tmp.container, { width, height }];
  obj1 = { style: tmp.imageOuterContainer, children: null };
  let tmp3Result = tmp3(5958);
  const obj2 = { accessibilityLabel: activityItem.application.name, imageBackground: null, aspectRatio: null };
  tmp3Result = tmp3(16641);
  ...
  if (tmp11) {
    ...
    obj4[0] = labelType;
    tmp15Result = tmp15(tmp3(11575), obj4);
  }
  ...
  if (tmp11) {
    ...
    if (isTestModeForApplication) {
      ...
      tmp15Result = tmp15(tmp3(5958), obj5);
      const tmp3Result1 = tmp3(5958);
    }
  }
  items3[2] = tmp15Result;
  const items4 = [closure_7(ManaContext.ThemeContextProvider, { theme: "dark", children: items3 }), ];
  let tmp15Result1 = activityAction === tmp8(11546).ActivityAction.START;
  if (tmp15Result1) {
