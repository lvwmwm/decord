...
// Dependencies: [19, 1301, 21, 4255, 712, 4099, 11413, 11328, 5648, 16073, 1849, 10510, 6876, 10615, 7480, 4797, 3967, 16072, 11356, 1297, 16076, 16074, 4444, 12013, 4251, 11418, 1236, 2]
// Exports: default

// Module 16075 (ActivityActionOverlay)
...
function ActivityActionOverlay(arg0) {
  ...
  if (require(11328) /* useActivityAction */.ActivityAction.JOIN !== action) {
    if (tmp4(11328).ActivityAction.LEAVE !== action) {
      return null;
    }
  }
  let tmp8 = action === tmp4(11328).ActivityAction.LEAVE;
  if (tmp8) {
    let obj = { style: null };
    obj[0] = tmp3.ongoingActivityJoinedContainer;
    tmp8 = callback(tmp(5648), obj);
  }
  const items = [tmp8, ];
  id = undefined;
  ...
  obj = { children: null };
  items[1] = tmp10(importDefault(16073), { channelId: id, applicationId: id, applicationName: name, submitting: importDefault(11413)({ applicationId, context, launchingComponentId }).submitting });
  obj[0] = items;
...
function ParticipantsText(arg0) {
  ...
  const arr = importDefault(16074)(activityItem.application.id, channelId);
  ...
  if (action === require(11328) /* useActivityAction */.ActivityAction.START) {
    let num2 = activityItem.application.maxParticipants;
    if (num2 == null) {
      num2 = 0;
    }
    let itemSubtitleForMaxPlayersShort = tmp9(11418).getItemSubtitleForMaxPlayersShort(num2);
    const tmp9Result = tmp9(11418);
  } else {
  ...
  obj[1] = items1;
  return closure_7(importDefault(5648), obj);
}
...
export default function ActivityShelfItem(arg0) {
  ...
  const result = width * importDefault(1849)();
  ...
  obj[5] = importDefault(10510);
  obj[6] = result;
  obj[7] = ["embedded_cover"];
  obj[8] = id;
  obj[9] = require(6876) /* ApplicationCommandSectionType */.CommandOrigin.VOICE_UI;
  const tmp7 = importDefault(11328);
  ({ activityAction, imageBackground, onActivityItemSelected: onActivityItemSelected2, labelType } = importDefault(11328)(obj));
  obj = { applicationId: activityItem.application.id, size: result, names: ["embedded_background"] };
  let tmp10 = importDefault(10615)(obj);
  let tmp11 = !disableBadges;
  if (!disableBadges) {
    const items = [tmp8(11328).ActivityAction.LEAVE, tmp8(11328).ActivityAction.JOIN];
    tmp11 = !items.includes(activityAction);
  }
  const tmp7Result = importDefault(11328)(obj);
  const isTestModeForApplication = require(7480) /* isTestModeForApplication */.useIsTestModeForApplication(activityItem.application.id);
  obj = { activeOpacity: 0.7, onPress: onActivityItemSelected2, disabled: null, androidRippleConfig: null, style: null, children: null };
  obj[2] = activityAction === require(11328) /* useActivityAction */.ActivityAction.LEAVE;
  ...
  let tmp3Result = tmp3(5648);
  const obj2 = { accessibilityLabel: activityItem.application.name, imageBackground: null, aspectRatio: null };
  tmp3Result = tmp3(16072);
  if (activityAction === require(11328) /* useActivityAction */.ActivityAction.START) {
  ...
  if (tmp11) {
    ...
    obj4[0] = labelType;
    tmp15Result = tmp15(tmp3(11356), obj4);
  }
  ...
  if (tmp11) {
    ...
    if (isTestModeForApplication) {
      ...
      tmp15Result = tmp15(tmp3(5648), obj5);
      const tmp3Result1 = tmp3(5648);
    }
  }
  items3[2] = tmp15Result;
  const items4 = [closure_7(require(3967) /* ManaContext */.ThemeContextProvider, { theme: "dark", children: items3 }), ];
  let tmp15Result1 = activityAction === tmp8(11328).ActivityAction.START;
  if (tmp15Result1) {
  ...
  obj[5] = items4;
  return closure_7(require(4797) /* PressableBase */.PressableOpacity, obj);
};
