...
// Dependencies: [19, 1301, 21, 4661, 712, 4223, 11319, 11230, 7190, 16364, 1896, 8713, 5248, 11231, 7871, 5433, 4104, 16363, 11262, 1297, 16367, 16365, 4796, 11953, 4734, 11324, 1236, 2]
// Exports: default

// Module 16366 (ActivityActionOverlay)
...
function ActivityActionOverlay(arg0) {
  ...
  if (require(11230) /* useActivityAction */.ActivityAction.JOIN !== action) {
    if (tmp4(11230).ActivityAction.LEAVE !== action) {
      return null;
    }
  }
  let tmp8 = action === tmp4(11230).ActivityAction.LEAVE;
  if (tmp8) {
    let obj = { style: null };
    obj[0] = tmp3.ongoingActivityJoinedContainer;
    tmp8 = callback(tmp(7190), obj);
  }
  const items = [tmp8, ];
  id = undefined;
  ...
  obj = { children: null };
  items[1] = tmp10(importDefault(16364), { channelId: id, applicationId: id, applicationName: name, submitting: importDefault(11319)({ applicationId, context, launchingComponentId }).submitting });
  obj[0] = items;
...
function ParticipantsText(arg0) {
  ...
  const arr = importDefault(16365)(activityItem.application.id, channelId);
  ...
  if (action === require(11230) /* useActivityAction */.ActivityAction.START) {
    let num2 = activityItem.application.maxParticipants;
    if (num2 == null) {
      num2 = 0;
    }
    let itemSubtitleForMaxPlayersShort = tmp9(11324).getItemSubtitleForMaxPlayersShort(num2);
    const tmp9Result = tmp9(11324);
  } else {
  ...
  obj[1] = items1;
  return closure_7(importDefault(7190), obj);
}
...
export default function ActivityShelfItem(arg0) {
  ...
  obj[5] = importDefault(8713);
  obj[6] = result;
  obj[7] = ["embedded_cover"];
  obj[8] = id;
  obj[9] = require(5248) /* ApplicationCommandSectionType */.CommandOrigin.VOICE_UI;
  const tmp7 = importDefault(11230);
  ({ activityAction, imageBackground, onActivityItemSelected: onActivityItemSelected2, labelType } = importDefault(11230)(obj));
  obj = { applicationId: activityItem.application.id, size: result, names: ["embedded_background"] };
  let tmp10 = importDefault(11231)(obj);
  ...
  const tmp7Result = importDefault(11230)(obj);
  const isTestModeForApplication = require(7871) /* isTestModeForApplication */.useIsTestModeForApplication(activityItem.application.id);
  obj = { activeOpacity: 0.7, onPress: onActivityItemSelected2, disabled: null, androidRippleConfig: null, style: null, children: null };
  obj[2] = activityAction === require(11230) /* useActivityAction */.ActivityAction.LEAVE;
  ...
  if (activityAction === require(11230) /* useActivityAction */.ActivityAction.START) {
  ...
  if (tmp11) {
    ...
    obj4[0] = labelType;
    tmp15Result = tmp15(tmp3(11262), obj4);
  }
  ...
  if (tmp11) {
    ...
    if (isTestModeForApplication) {
      ...
      tmp15Result = tmp15(tmp3(7190), obj5);
      const tmp3Result1 = tmp3(7190);
    }
  }
  items3[2] = tmp15Result;
  const items4 = [closure_7(require(4104) /* ManaContext */.ThemeContextProvider, { theme: "dark", children: items3 }), ];
  let tmp15Result1 = activityAction === tmp8(11230).ActivityAction.START;
  if (tmp15Result1) {
