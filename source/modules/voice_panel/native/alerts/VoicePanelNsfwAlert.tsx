...
// Dependencies: [31, 1391, 1838, 33, 4476, 4476, 1212, 5049, 4945, 2]
// Exports: default

// Module 15938 (VoicePanelNsfwAlert)
import "result";
...
export default function VoicePanelNsfwAlert(guildId) {
  ...
  const obj1 = {
    variant: "primary",
    onPress() {
      channelId(5049).nsfwAgree(guildId);
      const obj = channelId(5049);
      const voiceChannel = channelId(4945).selectVoiceChannel(channelId);
      dependencyMap();
    }
  };
  const intl3 = guildId(1212).intl;
  obj1.text = intl3.string(guildId(1212).t.wVq7uo);
  const items = [callback(guildId(4476).AlertActionButton, obj1, "confirm"), ];
  const obj2 = {
    variant: "secondary",
    onPress() {
      channelId(5049).nsfwReturnToSafety(guildId);
      dependencyMap();
    }
  };
  const intl4 = guildId(1212).intl;
  obj2.text = intl4.string(guildId(1212).t["/g10LC"]);
  items[1] = callback(guildId(4476).AlertActionButton, obj2, "add-profile-picture");
  obj.children = items;
  obj.actions = callback2(guildId(4476).AlertActions, obj);
  return callback(guildId(4476).AlertModal, obj);
};
