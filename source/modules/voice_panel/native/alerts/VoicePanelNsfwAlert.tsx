...
// Dependencies: [19, 1415, 1862, 21, 4598, 4598, 1236, 5185, 5081, 2]
// Exports: default

// Module 16111 (VoicePanelNsfwAlert)
import "noop";
...
export default function VoicePanelNsfwAlert(guildId) {
  ...
  const obj1 = {
    variant: "primary",
    onPress() {
      channelId(5185).nsfwAgree(guildId);
      const obj = channelId(5185);
      const voiceChannel = channelId(5081).selectVoiceChannel(channelId);
      dependencyMap();
    },
    text: null
  };
  ...
  const items = [closure_5(guildId(4598).AlertActionButton, obj1, "confirm"), ];
  const obj2 = {
    variant: "secondary",
    onPress() {
      channelId(5185).nsfwReturnToSafety(guildId);
      dependencyMap();
    },
    text: null
  };
  ...
  items[1] = closure_5(guildId(4598).AlertActionButton, obj2, "add-profile-picture");
  obj[0] = items;
  obj[2] = callback(guildId(4598).AlertActions, obj);
  return closure_5(guildId(4598).AlertModal, obj);
};
