...
// Dependencies: [19, 1434, 1910, 21, 4688, 4688, 1236, 5272, 5170, 2]
// Exports: default

// Module 16345 (VoicePanelNsfwAlert)
import "noop";
...
export default function VoicePanelNsfwAlert(guildId) {
  ...
  const obj1 = {
    variant: "primary",
    onPress() {
      channelId(5272).nsfwAgree(guildId);
      const obj = channelId(5272);
      const voiceChannel = channelId(5170).selectVoiceChannel(channelId);
      dependencyMap();
    },
    text: null
  };
  ...
  const items = [closure_5(guildId(4688).AlertActionButton, obj1, "confirm"), ];
  const obj2 = {
    variant: "secondary",
    onPress() {
      channelId(5272).nsfwReturnToSafety(guildId);
      dependencyMap();
    },
    text: null
  };
  ...
  items[1] = closure_5(guildId(4688).AlertActionButton, obj2, "add-profile-picture");
  obj[0] = items;
  obj[2] = callback(guildId(4688).AlertActions, obj);
  return closure_5(guildId(4688).AlertModal, obj);
};
