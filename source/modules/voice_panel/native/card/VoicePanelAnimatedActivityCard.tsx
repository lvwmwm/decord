...
// Dependencies: [32, 19, 1371, 10514, 1372, 10034, 10032, 676, 4214, 21, 4189, 712, 10033, 10380, 589, 15963, 5696, 5521, 10598, 5515, 5535, 9403, 13118, 4050, 10507, 5273, 1474, 10652, 7676, 15964, 10653, 15965, 15966, 15970, 3901, 2]

// Module 15962 (VoicePanelAnimatedActivityCardInner)
...
function VoicePanelAnimatedActivityCardInner(applicationId) {
  ...
  const context = focused.useContext(sharedVisible(10033));
  ...
  const tmp7 = sharedVisible(10380)();
  c10 = tmp7;
  ...
  const effect = obj.useEffect(() => {
    let obj = applicationId(5696);
    obj = { guildId: guild_id };
    const shelf = obj.fetchShelf(obj);
  }, items2);
  const items3 = [applicationId];
  first = tmp2(tmp4(5521)(items3), 1)[0];
  tmp8Result = tmp8(589);
  ...
  const tmp17 = sharedVisible(10598)();
  let tmp = callback();
  ...
  let tmp4Result = tmp4(5515);
  const tmp8Result1 = applicationId(589);
  let tmp21 = null != tmp17;
  const analyticsContext = applicationId(9403).useAnalyticsContext();
  if (tmp21) {
    let id;
    if (first != null) {
      id = first.id;
  ...
  if (!tmp21) {
    ...
    if (stateFromStores1 != null) {
      ...
      if (participants != null) {
        found = participants.find((userId) => applicationId(13118).isActivityParticipantCurrentUserCurrentSession(userId));
      }
    }
    tmp21 = null != found;
  }
  ...
  const tmp8Result2 = applicationId(9403);
  let fn = function q() {
  ...
  const fn2 = function $(visible, visible2) {
    ...
    if (tmp) {
      applicationId(4050).runOnJS(callback)();
      const obj = applicationId(4050);
    }
  };
  obj = { VoicePanelModes: controlsSpecs, runOnJS: tmp8(4050).runOnJS, incrementActivityKey: callback };
  fn2.__closure = obj;
  fn2.__workletHash = 9732208421749;
  fn2.__initData = c19;
  const animatedReaction = applicationId(4050).useAnimatedReaction(fn, fn2);
  ...
  if (null != stateFromStores1) {
    ...
    embeddedActivityParticipantId = tmp8(10507).getEmbeddedActivityParticipantId(obj);
    const tmp8Result4 = tmp8(10507);
  }
  const tmp25 = controlsSpecs;
  const tmp8Result3 = applicationId(4050);
  function de() {
  ...
  function re(arg0, arg1) {
    if (arg0 !== arg1) {
      applicationId(4050).runOnJS(c19)(arg0);
      const obj = applicationId(4050);
    }
  }
  obj1 = { runOnJS: tmp8(4050).runOnJS, setIsActivityFocused: tmp29 };
  re.__closure = obj1;
  re.__workletHash = 12291590020155;
  re.__initData = callback1;
  const animatedReaction1 = applicationId(4050).useAnimatedReaction(de, re);
  const items8 = [layoutManager, applicationId];
  const memo1 = obj.useMemo(() => {
    const Gesture = applicationId(5273).Gesture;
    return Gesture.Tap();
  }, []);
  ...
  const tmp8Result5 = applicationId(4050);
  class Se {
    constructor() {
  ...
  function he(landscape, landscape2) {
    ...
    if (landscape.landscape !== landscape) {
      applicationId(4050).runOnJS(callback1)(landscape.landscape, gridOrientationLockState, c16);
      const obj = applicationId(4050);
    }
    applicationId(4050).runOnJS(callback2)();
  }
  const tmp8Result6 = applicationId(4050);
  he.__closure = { runOnJS: applicationId(4050).runOnJS, handleTargetAspectRatioParams: callback1, gridOrientationLockState, hasJoined: tmp21, updateNotJoinedActivityDimensions: callback2 };
  he.__workletHash = 13125606009235;
  he.__initData = callback2;
  const animatedReaction2 = tmp8Result6.useAnimatedReaction(Se, he);
  const obj2 = { runOnJS: applicationId(4050).runOnJS, handleTargetAspectRatioParams: callback1, gridOrientationLockState, hasJoined: tmp21, updateNotJoinedActivityDimensions: callback2 };
  isTabletActivitySurface = applicationId(10652).getIsTabletActivitySurface();
  backgroundColor = tmp.activityContainerBackground.backgroundColor;
  const tmp8Result7 = applicationId(10652);
  function fe() {
    let num = 0;
    if (hideControls) {
      num = _undefined.get();
  ...
  const animatedStyle = applicationId(4050).useAnimatedStyle(fe);
  ...
  const tmp8Result8 = applicationId(4050);
  const items11 = [mode];
  ...
  if (null == stateFromStores) {
    ...
    if (tmp21) {
      ...
      obj7[1] = tmp43;
      obj6[3] = focusedOrientationLockState(tmp4(10653), obj7, tmp3[0]);
      const items13 = [focusedOrientationLockState(tmp4(15964), obj6), ];
      let tmp47Result = null;
      if (stateFromStores2) {
        tmp47Result = tmp47(tmp4(15965), {});
      }
      items13[1] = tmp47Result;
      obj5[3] = items13;
      obj4[1] = c16(tmp4Result, obj5);
      tmp47Result = tmp47(tmp8(5273).GestureDetector, obj4);
      let tmp44 = tmp47;
      const tmp48 = c16;
      const tmp4Result1 = tmp4(15964);
    } else if (null == activityShelfItemData) {
      const obj8 = { activity: null, application: null };
      obj8[0] = stateFromStores1;
      obj8[1] = first;
      tmp47Result = focusedOrientationLockState(tmp4(15966), obj8);
      ...
      obj9[1] = focusedOrientationLockState(tmp4(15970), obj10);
      tmp47Result = focusedOrientationLockState(tmp8(5273).GestureDetector, obj9);
    }
    const obj11 = { value: null, children: null };
    obj11[0] = tmp4Result(tmp4(5535).ACTIVITY_TILE).analyticsLocations;
    obj11[1] = tmp47Result;
    return tmp44(tmp8(5515).AnalyticsLocationProvider, obj11);
  }
