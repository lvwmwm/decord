...
// Dependencies: [687, 11, 1236, 3771, 7912, 3414, 7906, 2]
...
// Module 7911 (calculateTimestampDurations)
...
export const formatEntryTimestamp = function formatEntryTimestamp(contentInventoryEntry, locale, time, arg3) {
  ...
  IS_LIVE = IS_LIVE(7912).ContentInventoryTraitType.IS_LIVE;
...
export const isEntryNew = function isEntryNew(entry) {
  FIRST_TIME = FIRST_TIME(7912).ContentInventoryTraitType.FIRST_TIME;
  const traits = entry.traits;
  const found = traits.find((type) => type.type === TRENDING_CONTENT);
  let flag;
  if (found != null) {
...
export const getEntryDuration = function getEntryDuration(contentInventoryEntry) {
  DURATION_SECONDS = DURATION_SECONDS(7912).ContentInventoryTraitType.DURATION_SECONDS;
  const traits = contentInventoryEntry.traits;
  const found = traits.find((type) => type.type === TRENDING_CONTENT);
...
export const getAggregateRange = function getAggregateRange(traits) {
  AGGREGATE_RANGE = AGGREGATE_RANGE(7912).ContentInventoryTraitType.AGGREGATE_RANGE;
  traits = traits.traits;
  const found = traits.find((type) => type.type === TRENDING_CONTENT);
  let range;
  if (found != null) {
...
export const isEntryMarathon = function isEntryMarathon(entry) {
  MARATHON = MARATHON(7912).ContentInventoryTraitType.MARATHON;
  const traits = entry.traits;
  const found = traits.find((type) => type.type === TRENDING_CONTENT);
  let marathon;
  if (found != null) {
...
export const getResurrectedEntryLastPlayTime = function getResurrectedEntryLastPlayTime(entry) {
  RESURRECTED = RESURRECTED(7912).ContentInventoryTraitType.RESURRECTED;
  const traits = entry.traits;
  const found = traits.find((type) => type.type === TRENDING_CONTENT);
  let prop;
  if (found != null) {
...
export const isEntryTopGame = function isEntryTopGame(contentInventoryEntry) {
  return contentInventoryEntry.content_type === require(7906) /* ContentInventoryEntryType */.ContentInventoryEntryType.TOP_GAME;
};
export const getStreakCount = function getStreakCount(entry) {
  STREAK_DAYS = STREAK_DAYS(7912).ContentInventoryTraitType.STREAK_DAYS;
  const traits = entry.traits;
  const found = traits.find((type) => type.type === TRENDING_CONTENT);
...
export const getTrendingType = function getTrendingType(entry) {
  TRENDING_CONTENT = TRENDING_CONTENT(7912).ContentInventoryTraitType.TRENDING_CONTENT;
  const traits = entry.traits;
  const found = traits.find((type) => type.type === TRENDING_CONTENT);
  let trending;
  if (found != null) {
