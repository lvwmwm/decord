...
function isMatchingListeningActivity(closure_0, party) {
  if (obj.isListenedSessionEntry(closure_0)) {
    ...
    let isTopArtistEntryResult = provider === tmp(8175).ContentInventoryListenedMediaProvider.SPOTIFY;
  } else {
    isTopArtistEntryResult = tmp(8173).isTopArtistEntry(closure_0);
    if (isTopArtistEntryResult) {
      isTopArtistEntryResult = closure_0.extra.media.provider === tmp(8175).ContentInventoryListenedMediaProvider.SPOTIFY;
    }
...
export const isSpotifyEntry = function isSpotifyEntry(extra) {
  if (obj.isListenedSessionEntry(extra)) {
    ...
    isTopArtistEntryResult = tmp(8173).isTopArtistEntry(extra);
    if (isTopArtistEntryResult) {
      isTopArtistEntryResult = extra.extra.media.provider === tmp(8175).ContentInventoryListenedMediaProvider.SPOTIFY;
    }
    const tmpResult = tmp(8173);
  }
...
export const isCrunchyrollEntry = function isCrunchyrollEntry(extra) {
  let isWatchedMediaEntryResult = require(8173) /* isPlayedGameEntry */.isWatchedMediaEntry(extra);
...
export const isMatchingWatchActivity = function isMatchingWatchActivity(closure_0, details) {
  const tmp2 = importDefault(8176)(details);
  let tmp3 = !tmp2;
  if (tmp2) {
    let isWatchedMediaEntryResult = require(8173) /* isPlayedGameEntry */.isWatchedMediaEntry(closure_0);
    if (isWatchedMediaEntryResult) {
      isWatchedMediaEntryResult = closure_0.extra.application_id === CRUNCHYROLL_CLIENT_ID;
    }
    tmp3 = !isWatchedMediaEntryResult;
    const obj = require(8173) /* isPlayedGameEntry */;
  }
  let tmp7 = !tmp3;
  if (!tmp3) {
...
export const findMatchingEntry = function findMatchingEntry(entries, activity) {
  ...
  const found = entries.filter(_require(7911).isEntryActive);
  if (activity.type === ActivityTypes.PLAYING) {
    const found1 = found.filter(tmp(8173).isGamingLikeEntry);
    ...
    const found3 = found.filter(tmp(8173).isListenedSessionEntry);
    found2 = found3.find((arg0) => outer1_6(arg0, closure_0));
  } else if (activity.type === tmp3.WATCHING) {
    const found4 = entries.filter(tmp(8173).isWatchedMediaEntry);
