...
// Dependencies: [32, 1922, 8397, 4421, 1499, 10678, 11079, 5835, 11509, 8204, 1236, 11510, 8493, 8396, 10680, 11511, 11512, 1435, 1487, 11513, 5854, 11067, 7129, 10681, 4242, 1627, 10705, 5862, 4097, 1370, 2]
// Exports: createAppMessageEmbed, getAppLinkGateResult, handleTapAppMessageEmbed

// Module 11508 (createAppMessageEmbed)
...
import installApplication from "installApplication" /* 10705 */;
import _launchActivityInBotDM from "_launchActivityInBotDM" /* 11067 */;
import ContentClassificationVisibility from "ContentClassificationVisibility" /* 11509 */;
import _joinOrStartActivityInChannel from "_joinOrStartActivityInChannel" /* 11513 */;
...
import closure_7 from "addApplication" /* 4421 */;
...
export const getAppLinkGateResult = function getAppLinkGateResult(arg0) {
  ...
  if (null == application) {
    ...
    if (contentClassificationVisibility !== ContentClassificationVisibility.ContentClassificationVisibility.DISPLAY) {
      ...
      if (contentClassificationVisibility === tmp3(11509).ContentClassificationVisibility.BLOCK_UNDERAGE) {
      ...
      obj.displayType = tmp3(11510).AppMessageEmbedDisplayType.BLOCKED;
      obj.appId = "";
  ...
  if (null != maxParticipants) {
    ...
    if (isEmbeddedAppResult) {
      ...
      if (tmp2Result.canLaunchFrame(app)) {
        ...
        const playInContext = tmp2(11511).getPlayInContext(id, message.channel_id);
        ...
        const tmp2Result1 = tmp2(11511);
      }
    }
    ({ id: id2, bot: bot2 } = app);
    const joined = tags.join(" \u2219 ");
    if (tmp2Result2.isEmbeddedApp(app)) {
      let assetIds1 = tmp2(8396).getAssetIds(id2, closure_11);
      if (assetIds1 == null) {
        assetIds1 = [];
      }
      ...
      let assetImage = null;
      if (null != first) {
        assetImage = tmp2(8396).getAssetImage(id2, first, c12);
        const tmp2Result4 = tmp2(8396);
      }
      if (null != assetImage) {
      ...
      let appIconSrc = null;
      if (null != icon) {
        appIconSrc = tmp2(11512).getAppIconSrc(id, icon, bot);
        const tmp2Result5 = tmp2(11512);
      }
      let staticBannerSrc = appIconSrc;
      if (appIconSrc == null) {
        staticBannerSrc = obj7.staticBannerSrc;
      }
      const tmp2Result3 = tmp2(8396);
      const obj4 = {};
      const appGradientColors = tmp2(11512).getAppGradientColors(staticBannerSrc);
      const merged = Object.assign(baseColors);
      const merged1 = Object.assign(obj7);
      obj4.displayType = tmp2(11510).AppMessageEmbedDisplayType.DISPLAY;
      obj4.appId = app.id;
...
export const handleTapAppMessageEmbed = function handleTapAppMessageEmbed(appId) {
  ...
  if ("play_in_channel" === actionId) {
    ...
    const items = [tmp2(5854).APP_MESSAGE_EMBED];
    obj[2] = items;
    obj[3] = id;
    obj[4] = value;
    ...
    if (null != bot) {
      ...
      const items1 = [tmp2(5854).APP_MESSAGE_EMBED];
      obj[2] = items1;
