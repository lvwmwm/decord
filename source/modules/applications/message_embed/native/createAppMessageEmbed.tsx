...
// Dependencies: [32, 1922, 8464, 4485, 1499, 10750, 11151, 5901, 11581, 8271, 1236, 11582, 8560, 8463, 10752, 11583, 11584, 1435, 1487, 11585, 5920, 11139, 7195, 10753, 4306, 1627, 10777, 5928, 4161, 1370, 2]
// Exports: createAppMessageEmbed, getAppLinkGateResult, handleTapAppMessageEmbed

// Module 11580 (createAppMessageEmbed)
...
import installApplication from "installApplication" /* 10777 */;
import _launchActivityInBotDM from "_launchActivityInBotDM" /* 11139 */;
import ContentClassificationVisibility from "ContentClassificationVisibility" /* 11581 */;
import _joinOrStartActivityInChannel from "_joinOrStartActivityInChannel" /* 11585 */;
...
import closure_7 from "addApplication" /* 4485 */;
...
export const getAppLinkGateResult = function getAppLinkGateResult(arg0) {
  ...
  if (null == application) {
    ...
    if (contentClassificationVisibility !== ContentClassificationVisibility.ContentClassificationVisibility.DISPLAY) {
      ...
      if (contentClassificationVisibility === tmp3(11581).ContentClassificationVisibility.BLOCK_UNDERAGE) {
      ...
      obj.displayType = tmp3(11582).AppMessageEmbedDisplayType.BLOCKED;
      obj.appId = "";
  ...
  if (null != maxParticipants) {
    ...
    if (isEmbeddedAppResult) {
      ...
      if (tmp2Result.canLaunchFrame(app)) {
        ...
        const playInContext = tmp2(11583).getPlayInContext(id, message.channel_id);
        ...
        const tmp2Result1 = tmp2(11583);
      }
    }
    ({ id: id2, bot: bot2 } = app);
    const joined = tags.join(" \u2219 ");
    if (tmp2Result2.isEmbeddedApp(app)) {
      let assetIds1 = tmp2(8463).getAssetIds(id2, closure_11);
      if (assetIds1 == null) {
        assetIds1 = [];
      }
      ...
      let assetImage = null;
      if (null != first) {
        assetImage = tmp2(8463).getAssetImage(id2, first, c12);
        const tmp2Result4 = tmp2(8463);
      }
      if (null != assetImage) {
      ...
      let appIconSrc = null;
      if (null != icon) {
        appIconSrc = tmp2(11584).getAppIconSrc(id, icon, bot);
        const tmp2Result5 = tmp2(11584);
      }
      let staticBannerSrc = appIconSrc;
      if (appIconSrc == null) {
        staticBannerSrc = obj7.staticBannerSrc;
      }
      const tmp2Result3 = tmp2(8463);
      const obj4 = {};
      const appGradientColors = tmp2(11584).getAppGradientColors(staticBannerSrc);
      const merged = Object.assign(baseColors);
      const merged1 = Object.assign(obj7);
      obj4.displayType = tmp2(11582).AppMessageEmbedDisplayType.DISPLAY;
      obj4.appId = app.id;
...
export const handleTapAppMessageEmbed = function handleTapAppMessageEmbed(appId) {
  ...
  if ("play_in_channel" === actionId) {
    ...
    const items = [tmp2(5920).APP_MESSAGE_EMBED];
    obj[2] = items;
    obj[3] = id;
    obj[4] = value;
    ...
    if (null != bot) {
      ...
      const items1 = [tmp2(5920).APP_MESSAGE_EMBED];
      obj[2] = items1;
    ...
    obj1[0] = appId.appId;
    obj1[1] = MAIN_SURFACE;
    tmp2(10753).launchFrame(obj1);
    const tmp2Result = tmp2(10753);
