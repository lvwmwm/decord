...
// Dependencies: [32, 1922, 8018, 4489, 1498, 9426, 10908, 5922, 11481, 7822, 1236, 11482, 8113, 8017, 9431, 11483, 11484, 1431, 1486, 11485, 5941, 11080, 7236, 9422, 4310, 1626, 9238, 5949, 4163, 1471, 2]
// Exports: createAppMessageEmbed, getAppLinkGateResult, handleTapAppMessageEmbed

// Module 11480 (createAppMessageEmbed)
...
import installApplication from "installApplication" /* 9238 */;
import _launchActivityInBotDM from "_launchActivityInBotDM" /* 11080 */;
import ContentClassificationVisibility from "ContentClassificationVisibility" /* 11481 */;
import _joinOrStartActivityInChannel from "_joinOrStartActivityInChannel" /* 11485 */;
...
import closure_7 from "addApplication" /* 4489 */;
...
export const getAppLinkGateResult = function getAppLinkGateResult(arg0) {
  ...
  if (null == application) {
    ...
    if (contentClassificationVisibility !== ContentClassificationVisibility.ContentClassificationVisibility.DISPLAY) {
      ...
      if (contentClassificationVisibility === tmp3(11481).ContentClassificationVisibility.BLOCK_UNDERAGE) {
      ...
      obj.displayType = tmp3(11482).AppMessageEmbedDisplayType.BLOCKED;
      obj.appId = "";
  ...
  if (null != maxParticipants) {
    ...
    if (isEmbeddedAppResult) {
      ...
      if (tmp2Result.canLaunchFrame(app)) {
        ...
        const playInContext = tmp2(11483).getPlayInContext(id, message.channel_id);
        ...
        const tmp2Result1 = tmp2(11483);
      }
    }
    ({ id: id2, bot: bot2 } = app);
    const joined = tags.join(" \u2219 ");
    if (tmp2Result2.isEmbeddedApp(app)) {
      let assetIds1 = tmp2(8017).getAssetIds(id2, closure_11);
      if (assetIds1 == null) {
        assetIds1 = [];
      }
      ...
      let assetImage = null;
      if (null != first) {
        assetImage = tmp2(8017).getAssetImage(id2, first, c12);
        const tmp2Result4 = tmp2(8017);
      }
      if (null != assetImage) {
      ...
      let appIconSrc = null;
      if (null != icon) {
        appIconSrc = tmp2(11484).getAppIconSrc(id, icon, bot);
        const tmp2Result5 = tmp2(11484);
      }
      let staticBannerSrc = appIconSrc;
      if (appIconSrc == null) {
        staticBannerSrc = obj7.staticBannerSrc;
      }
      const tmp2Result3 = tmp2(8017);
      const obj4 = {};
      const appGradientColors = tmp2(11484).getAppGradientColors(staticBannerSrc);
      const merged = Object.assign(baseColors);
      const merged1 = Object.assign(obj7);
      obj4.displayType = tmp2(11482).AppMessageEmbedDisplayType.DISPLAY;
      obj4.appId = app.id;
...
export const handleTapAppMessageEmbed = function handleTapAppMessageEmbed(appId) {
  ...
  if ("play_in_channel" === actionId) {
    ...
    const items = [tmp2(5941).APP_MESSAGE_EMBED];
    obj[2] = items;
    obj[3] = id;
    obj[4] = value;
    ...
    if (null != bot) {
      ...
      const items1 = [tmp2(5941).APP_MESSAGE_EMBED];
      obj[2] = items1;
    ...
    obj1[0] = appId.appId;
    obj1[1] = MAIN_SURFACE;
    tmp2(9422).launchFrame(obj1);
    const tmp2Result = tmp2(9422);
