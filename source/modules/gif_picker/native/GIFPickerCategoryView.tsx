...
// Dependencies: [19, 17, 676, 21, 4444, 712, 1236, 9228, 5440, 10171, 8959, 4440, 2]
// Exports: default

// Module 10170 (GIFPickerCategoryView)
...
export default function GIFPickerCategoryView(onSelectCategory) {
  ...
  const merged = Object.assign(item(9228)(callback, formatToPlainStringResult));
  obj = { style: tmp.gifImage, source: obj1 };
  const items1 = [callback(item(5440), obj), callback(closure_4, { style: tmp.gifOverlay }), ];
  ...
  if (item.type === GIFPickerResultTypes.TRENDING_GIFS) {
    ...
    let tmp10Result = tmp10(tmp3(10171).AnalyticsIcon, obj4);
  } else {
    tmp10Result = null;
    if (item.type === tmp12.FAVORITES) {
      ...
      tmp10Result = tmp10(tmp3(8959).StarIcon, obj5);
    }
  }
  const items2 = [tmp10Result, callback(onSelectCategory(4440).Text, { variant: "text-sm/semibold", color: "text-overlay-light", maxFontSizeMultiplier: 2, accessible: false, children: item.name })];
  obj3[2] = items2;
