...
// Dependencies: [19, 17, 676, 21, 4255, 712, 1236, 8604, 5207, 9776, 9577, 4251, 2]
// Exports: default

// Module 9775 (GIFPickerCategoryView)
...
export default function GIFPickerCategoryView(onSelectCategory) {
  ...
  const merged = Object.assign(item(8604)(callback, formatToPlainStringResult));
  obj = { style: tmp.gifImage, source: obj1 };
  const items1 = [callback(item(5207), obj), callback(closure_4, { style: tmp.gifOverlay }), ];
  ...
  if (item.type === GIFPickerResultTypes.TRENDING_GIFS) {
    ...
    let tmp10Result = tmp10(tmp3(9776).AnalyticsIcon, obj4);
  } else {
    tmp10Result = null;
    if (item.type === tmp12.FAVORITES) {
      ...
      tmp10Result = tmp10(tmp3(9577).StarIcon, obj5);
    }
  }
