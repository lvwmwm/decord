...
// Dependencies: [19, 17, 676, 4489, 1384, 5849, 5854, 1473, 8259, 8141, 4634, 7995, 4523, 1935, 8154, 8170, 1351, 8672, 691, 4001, 8787, 4641, 1236, 8788, 2]
...
// Module 8786 (isValidImageAttachment)
...
function isValidImageAttachment(filename) {
  ...
  if (null == filename) {
    ...
    let tmp = require(4489) /* urlMatchesFileExtension */.isImageFile(filename.filename) && null != height;
    if (tmp) {
      tmp = height > 0;
    }
    if (tmp) {
  ...
  if (null != width.width) {
    if (width.width > 0) {
      if (null != width.height) {
        if (width.height > 0) {
          const enabledHarmTypesForMessage = require(5849) /* getEligibleHarmTypesConfigsForContext */.getEnabledHarmTypesForMessage(closure_0);
          const obj12 = require(5849) /* getEligibleHarmTypesConfigsForContext */;
          let obj = { type: null, media: null };
          obj[0] = require(5854) /* ContentHarmTypeChannel */.ObscuredMediaTypes.Attachment;
          obj[1] = width;
          const result = require(5849) /* getEligibleHarmTypesConfigsForContext */.isMediaObscuredForHarmTypes(obj, enabledHarmTypesForMessage);
          const obj13 = require(5849) /* getEligibleHarmTypesConfigsForContext */;
          const isVideoFileResult = require(4489) /* urlMatchesFileExtension */.isVideoFile(width.filename);
          ...
          url = width.url;
          const obj15 = require(4489) /* urlMatchesFileExtension */;
        }
      }
    }
  }
...
function extractMediaFromEmbed(image, closure_0, contentMessage, arg3, closure_02) {
  ...
  if (null != thumbnail) {
    if (null == image.video) {
      ...
      const hasSpoilerEmbeds = importDefault(8141)(closure_0, obj).hasSpoilerEmbeds;
      let obj2 = require(5849) /* getEligibleHarmTypesConfigsForContext */;
      const enabledHarmTypesForMessage = obj2.getEnabledHarmTypesForMessage(closure_0);
      let obj3 = require(5849) /* getEligibleHarmTypesConfigsForContext */;
      ...
      const result1 = require(7995) /* frozen */.shouldPlayVideoInline(url3);
      let tmp26 = result1;
      if (result1) {
      ...
      tmp13 = importDefault;
      const tmp16Result1 = require(7995) /* frozen */;
    } else {
...
function toMediaSourceFromUnfurledMedia(id, guild_id, media, description, spoiler) {
  ...
  let width;
  let width2;
  let obj = require(8154) /* transformToRowGeneratedComponent */;
  const unfurledMediaItemType = obj.getUnfurledMediaItemType(media);
  if (unfurledMediaItemType === require(8170) /* MediaGalleryItemType */.MediaGalleryItemType.VISUAL_PLACEHOLDER) {
    return null;
  } else {
...
export const isValidVideoAttachment = function isValidVideoAttachment(filename) {
  ...
  if (tmp) {
    ...
    if (isVideoFileResult) {
      isVideoFileResult = require(4489) /* urlMatchesFileExtension */.isVideoFile(filename.filename);
      const obj = require(4489) /* urlMatchesFileExtension */;
    }
    if (isVideoFileResult) {
...
export const extractMediaFromMessageComponents = function extractMediaFromMessageComponents(getContentMessage, contentMessage, closure_0) {
  if (0 === contentMessage.components.length) {
    ...
    let items = [];
    const obj2 = require(4523) /* flattenComponents */;
    const items1 = [];
    HermesBuiltin.arraySpread(require(4523) /* flattenComponents */.flattenComponents(contentMessage.components).values(), 0);
...
export const extractMediaSourcesFromEmbed = function extractMediaSourcesFromEmbed(arg0, contentMessage, images, index, guild_id) {
  ...
  const hasSpoilerEmbeds = importDefault(8141)(arg0, { contentMessage }).hasSpoilerEmbeds;
      ...
      if (isValidImageAttachment(tmp)) {
        ...
        if (tmp5) {
          ...
          if (isVideoFileResult) {
            let tmp7 = require;
            let tmp8 = dependencyMap;
            let obj = require(4489) /* urlMatchesFileExtension */;
...
export const useSelectedMediaSource = function useSelectedMediaSource(syncer) {
  ...
  const tmp = importDefault(8672)(syncer.index);
  importDefault = tmp;
...
export const getVideoSourceType = function getVideoSourceType(source) {
  ...
  if (null != source.portal) {
    ...
    obj2 = require(8788) /* MediaPlayerManager */;
  }
...
export const isAnimatedWebpSource = function isAnimatedWebpSource(sourceURI) {
  ...
  if (result) {
    result = require(4489) /* urlMatchesFileExtension */.urlMatchesFileExtension(sourceURI.sourceURI, closure_6);
    const obj = require(4489) /* urlMatchesFileExtension */;
  }
  if (result) {
    const _URL = URL;
...
export const isAnimatedAvifSource = function isAnimatedAvifSource(sourceURI) {
  let result = null != sourceURI.sourceURI && null != sourceURI.uri;
  if (result) {
    result = require(4489) /* urlMatchesFileExtension */.urlMatchesFileExtension(sourceURI.sourceURI, closure_7);
    const obj = require(4489) /* urlMatchesFileExtension */;
  }
  if (result) {
    const _URL = URL;
...
export const isGIFSource = function isGIFSource(sourceURI) {
  return require(4489) /* urlMatchesFileExtension */.urlMatchesFileExtension(sourceURI.sourceURI, require(691) /* keys */.GIF_RE_IOS);
};
export const isAnimatedImageSource = function isAnimatedImageSource(source) {
  let result = require(4489) /* urlMatchesFileExtension */.urlMatchesFileExtension(source.sourceURI, require(691) /* keys */.GIF_RE_IOS);
  if (!result) {
