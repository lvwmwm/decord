...
// Dependencies: [19, 17, 676, 4377, 1384, 5875, 5878, 1473, 7993, 7873, 4522, 7726, 4411, 1906, 7886, 7902, 1351, 8115, 691, 3889, 8116, 4529, 1236, 8117, 2]
...
// Module 8114 (isValidImageAttachment)
...
function isValidImageAttachment(filename) {
  ...
  if (null == filename) {
    ...
    let tmp = require(4377) /* urlMatchesFileExtension */.isImageFile(filename.filename) && null != height;
    if (tmp) {
      tmp = height > 0;
    }
    if (tmp) {
  ...
  if (null != width.width) {
    if (width.width > 0) {
      if (null != width.height) {
        if (width.height > 0) {
          const enabledHarmTypesForMessage = require(5875) /* getEligibleHarmTypesConfigsForContext */.getEnabledHarmTypesForMessage(closure_0);
          const obj12 = require(5875) /* getEligibleHarmTypesConfigsForContext */;
          let obj = { type: null, media: null };
          obj[0] = require(5878) /* ContentHarmTypeChannel */.ObscuredMediaTypes.Attachment;
          obj[1] = width;
          const result = require(5875) /* getEligibleHarmTypesConfigsForContext */.isMediaObscuredForHarmTypes(obj, enabledHarmTypesForMessage);
          const obj13 = require(5875) /* getEligibleHarmTypesConfigsForContext */;
          const isVideoFileResult = require(4377) /* urlMatchesFileExtension */.isVideoFile(width.filename);
          if (null != width.proxy_url) {
            ...
            if (isVideoFileResult) {
              ...
              obj2.uri = url;
              const formatted = tmp(7993)(width).toLowerCase();
              const str3 = tmp(7993)(width);
              const tmp11 = constants;
              const endsWithResult = formatted.endsWith(".webp");
              const formatted1 = tmp(7993)(width).toLowerCase();
              const endsWithResult1 = formatted1.endsWith(".avif");
              const str5 = tmp(7993)(width);
          ...
          url = width.url;
          const obj15 = require(4377) /* urlMatchesFileExtension */;
        }
      }
    }
  }
...
function extractMediaFromEmbed(image, closure_0, contentMessage, arg3, closure_02) {
  ...
  if (null != thumbnail) {
    if (null == image.video) {
      ...
      const hasSpoilerEmbeds = importDefault(7873)(closure_0, obj).hasSpoilerEmbeds;
      let obj2 = require(5875) /* getEligibleHarmTypesConfigsForContext */;
      const enabledHarmTypesForMessage = obj2.getEnabledHarmTypesForMessage(closure_0);
      let obj3 = require(5875) /* getEligibleHarmTypesConfigsForContext */;
      ...
      const result1 = require(7726) /* frozen */.shouldPlayVideoInline(effectiveVideoProvider);
      let tmp27 = result1;
      if (result1) {
      ...
      tmp13 = importDefault;
      const tmp16Result1 = require(7726) /* frozen */;
    } else {
...
function toMediaSourceFromUnfurledMedia(id, guild_id, media, description, spoiler) {
  ...
  let width;
  let width2;
  let obj = require(7886) /* transformToRowGeneratedComponent */;
  const unfurledMediaItemType = obj.getUnfurledMediaItemType(media);
  if (unfurledMediaItemType === require(7902) /* MediaGalleryItemType */.MediaGalleryItemType.VISUAL_PLACEHOLDER) {
    return null;
  } else {
    ...
    const tmp4 = unfurledMediaItemType === tmp(7902).MediaGalleryItemType.VIDEO;
    let str;
    if (tmp4) {
      str = "png";
    }
...
export const isValidVideoAttachment = function isValidVideoAttachment(filename) {
  ...
  if (tmp) {
    ...
    if (isVideoFileResult) {
      isVideoFileResult = require(4377) /* urlMatchesFileExtension */.isVideoFile(filename.filename);
      const obj = require(4377) /* urlMatchesFileExtension */;
    }
    if (isVideoFileResult) {
...
export const extractMediaFromMessageComponents = function extractMediaFromMessageComponents(getContentMessage, contentMessage, closure_0) {
  if (0 === contentMessage.components.length) {
    ...
    let items = [];
    const obj2 = require(4411) /* flattenComponents */;
    const items1 = [];
    HermesBuiltin.arraySpread(require(4411) /* flattenComponents */.flattenComponents(contentMessage.components).values(), 0);
...
export const extractMediaSourcesFromEmbed = function extractMediaSourcesFromEmbed(arg0, contentMessage, images, index, guild_id) {
  ...
  const hasSpoilerEmbeds = importDefault(7873)(arg0, { contentMessage }).hasSpoilerEmbeds;
      ...
      if (isValidImageAttachment(tmp)) {
        ...
        if (tmp5) {
          ...
          if (isVideoFileResult) {
            let tmp7 = require;
            let tmp8 = dependencyMap;
            let obj = require(4377) /* urlMatchesFileExtension */;
...
export const useSelectedMediaSource = function useSelectedMediaSource(syncer) {
  ...
  const tmp = importDefault(8115)(syncer.index);
  importDefault = tmp;
...
export const getVideoSourceType = function getVideoSourceType(source) {
  ...
  if (null != source.portal) {
    ...
    obj2 = require(8117) /* MediaPlayerManager */;
  }
...
export const isAnimatedWebpSource = function isAnimatedWebpSource(sourceURI) {
  ...
  if (result) {
    result = require(4377) /* urlMatchesFileExtension */.urlMatchesFileExtension(sourceURI.sourceURI, closure_6);
    const obj = require(4377) /* urlMatchesFileExtension */;
  }
  if (result) {
    const _URL = URL;
...
export const isAnimatedAvifSource = function isAnimatedAvifSource(sourceURI) {
  let result = null != sourceURI.sourceURI && null != sourceURI.uri;
  if (result) {
    result = require(4377) /* urlMatchesFileExtension */.urlMatchesFileExtension(sourceURI.sourceURI, closure_7);
    const obj = require(4377) /* urlMatchesFileExtension */;
  }
  if (result) {
    const _URL = URL;
...
export const isGIFSource = function isGIFSource(sourceURI) {
  return require(4377) /* urlMatchesFileExtension */.urlMatchesFileExtension(sourceURI.sourceURI, require(691) /* keys */.GIF_RE_IOS);
};
export const isAnimatedImageSource = function isAnimatedImageSource(source) {
  let result = require(4377) /* urlMatchesFileExtension */.urlMatchesFileExtension(source.sourceURI, require(691) /* keys */.GIF_RE_IOS);
  if (!result) {
