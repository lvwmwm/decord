...
function isValidImageAttachment(filename) {
  ...
  if (null == filename) {
    ...
    let tmp = require(4491) /* urlMatchesFileExtension */.isImageFile(filename.filename) && null != height;
    if (tmp) {
      tmp = height > 0;
    }
    if (tmp) {
  ...
  if (null != width.width) {
    if (width.width > 0) {
      if (null != width.height) {
        if (width.height > 0) {
          const enabledHarmTypesForMessage = require(5909) /* getEligibleHarmTypesConfigsForContext */.getEnabledHarmTypesForMessage(closure_0);
          const obj12 = require(5909) /* getEligibleHarmTypesConfigsForContext */;
          let obj = { type: null, media: null };
          obj[0] = require(5914) /* ContentHarmTypeChannel */.ObscuredMediaTypes.Attachment;
          obj[1] = width;
          const result = require(5909) /* getEligibleHarmTypesConfigsForContext */.isMediaObscuredForHarmTypes(obj, enabledHarmTypesForMessage);
          const obj13 = require(5909) /* getEligibleHarmTypesConfigsForContext */;
          const isVideoFileResult = require(4491) /* urlMatchesFileExtension */.isVideoFile(width.filename);
          ...
          url = width.url;
          const obj15 = require(4491) /* urlMatchesFileExtension */;
        }
      }
    }
  }
...
export const isValidVideoAttachment = function isValidVideoAttachment(filename) {
  ...
  if (tmp) {
    ...
    if (isVideoFileResult) {
      isVideoFileResult = require(4491) /* urlMatchesFileExtension */.isVideoFile(filename.filename);
      const obj = require(4491) /* urlMatchesFileExtension */;
    }
    if (isVideoFileResult) {
...
export const extractMediaFromMessageComponents = function extractMediaFromMessageComponents(getContentMessage, contentMessage, closure_0) {
  if (0 === contentMessage.components.length) {
    ...
    let items = [];
    const obj2 = require(4525) /* flattenComponents */;
    const items1 = [];
    HermesBuiltin.arraySpread(require(4525) /* flattenComponents */.flattenComponents(contentMessage.components).values(), 0);
      ...
      if (isValidImageAttachment(tmp)) {
        ...
        if (tmp5) {
          ...
          if (isVideoFileResult) {
            let tmp7 = require;
            let tmp8 = dependencyMap;
            let obj = require(4491) /* urlMatchesFileExtension */;
...
export const isAnimatedWebpSource = function isAnimatedWebpSource(sourceURI) {
  ...
  if (result) {
    result = require(4491) /* urlMatchesFileExtension */.urlMatchesFileExtension(sourceURI.sourceURI, closure_6);
    const obj = require(4491) /* urlMatchesFileExtension */;
  }
  if (result) {
    const _URL = URL;
...
export const isAnimatedAvifSource = function isAnimatedAvifSource(sourceURI) {
  let result = null != sourceURI.sourceURI && null != sourceURI.uri;
  if (result) {
    result = require(4491) /* urlMatchesFileExtension */.urlMatchesFileExtension(sourceURI.sourceURI, closure_7);
    const obj = require(4491) /* urlMatchesFileExtension */;
  }
  if (result) {
    const _URL = URL;
...
export const isGIFSource = function isGIFSource(sourceURI) {
  return require(4491) /* urlMatchesFileExtension */.urlMatchesFileExtension(sourceURI.sourceURI, require(691) /* keys */.GIF_RE_IOS);
};
export const isAnimatedImageSource = function isAnimatedImageSource(source) {
  let result = require(4491) /* urlMatchesFileExtension */.urlMatchesFileExtension(source.sourceURI, require(691) /* keys */.GIF_RE_IOS);
  if (!result) {
