...
// Module 8282 (useMediaViewerSyncer)
...
let closure_10 = arg1(dependencyMap[3]).isAndroid();
...
const result = arg1(dependencyMap[11]).fileFinishedImporting("modules/media_viewer/native/useMediaViewerSyncer.tsx");

export const useMediaViewerSyncer = function useMediaViewerSyncer(sources) {
  sources = sources.sources;
  const arg1 = sources;
  const initialIndex = sources.initialIndex;
  const importDefault = initialIndex;
  ...
  return React.useMemo(() => {
    ...
    if (0 < sources.length) {
      do {
        ...
        if (null != size) {
          ...
          let tmp7 = closure_8;
          let tmp8 = closure_5;
          let sum = Math.max(Math.min(size.width * (closure_6 / size.height), closure_7), closure_8) + 2 * closure_5;
          let sum1 = num + sum;
          let diff = sum1 - num;
          let num3 = 0;
          if (0 !== num2) {
    ...
    return function makeMediaViewerSyncer(sources) {
      ...
      ({ thumbnailsIndex: closure_3, thumbnailsScrolling: closure_4, thumbnailsAnimateTo: closure_5, viewerPos: closure_6, viewerScrolling: closure_7, zoomed } = animProps);
      ...
      return {
        ...
        useThumbnailsProps(THUMBNAIL_WIDTH_MARGIN, maxIndex) {
          const sources = THUMBNAIL_WIDTH_MARGIN;
          const thumbnailScrollPositions = maxIndex;
          function onSelectedIndexChange() {
            if ("thumbnails" === store.get()) {
              const MediaViewerAnalytics = arg0(animatedRef[7]).MediaViewerAnalytics;
              MediaViewerAnalytics.markActionPerformed(arg0(animatedRef[7]).IncrementableMediaViewerActions.THUMBNAIL_SWIPE);
            } else if ("viewer" === store.get()) {
              const MediaViewerAnalytics3 = arg0(animatedRef[7]).MediaViewerAnalytics;
              MediaViewerAnalytics3.markActionPerformed(arg0(animatedRef[7]).IncrementableMediaViewerActions.VIEWER_SWIPE);
            }
            const MediaViewerAnalytics2 = arg0(animatedRef[7]).MediaViewerAnalytics;
            MediaViewerAnalytics2.markActionPerformed(arg0(animatedRef[7]).IncrementableMediaViewerActions.SELECTED_ITEM_CHANGE);
          }
          const zoomed = onSelectedIndexChange;
          let obj = sources(selectedIndex[4]);
          const animatedRef = obj.useAnimatedRef();
          const selectedIndex = animatedRef;
          let obj1 = sources(selectedIndex[5]);
          const size = obj1.useMediaViewerDimensions();
          const width = size.width;
          const rect = thumbnailScrollPositions(selectedIndex[6])();
          let obj2 = sources(selectedIndex[4]);
          obj = { onBeginDrag: fn };
          const fn = function y() {
          ...
          class E {
            constructor(arg0) {
              result = THUMBNAIL_WIDTH_MARGIN.contentOffset.x / THUMBNAIL_WIDTH_MARGIN;
              if ("thumbnails" === closure_9.get()) {
                tmp3 = globalThis;
                _Math3 = Math;
                _Math4 = Math;
                _Math5 = Math;
                ...
                num2 = 0;
                tmp5 = runOnJS;
                result1 = runOnJS.set(Math.max(0, Math.min(Math.round(result), maxIndex)));
              } else {
                tmp2 = globalThis;
                _Math = Math;
                _Math2 = Math;
              ...
              if (scrollTo.get() >= 0) {
                ...
                if (value === scrollTo.get()) {
                  ...
                  tmp24 = closure_2;
                  tmp25 = runOnJS;
                  result3 = closure_2.set(runOnJS.get());
                }
              } else {
                tmp27 = THUMBNAIL_WIDTH_MARGIN;
                tmp8 = 0 !== THUMBNAIL_WIDTH_MARGIN.get();
                if (!tmp8) {
                  tmp7 = scrollTo;
                  tmp8 = 0 !== scrollTo.get();
                }
                ...
                if (tmp8) {
                  tmp12 = closure_2;
                  tmp13 = runOnJS;
                  result4 = closure_2.set(runOnJS.get());
                  tmp15 = THUMBNAIL_WIDTH_MARGIN;
                  tmp16 = closure_2;
                  num3 = 4;
                  obj = THUMBNAIL_WIDTH_MARGIN(closure_2[4]);
          ...
          obj1 = { variableWidthThumbnailsEnabled: false, thumbnailScrollPositions, thumbnailSize: THUMBNAIL_WIDTH_MARGIN, swipeSource, maxIndex, thumbnailsIndex: mapped, thumbnailsAnimateTo: derivedValue2, selectedIndex, viewerScrolling: diff1, thumbnailsScrolling: mapped1, runOnJS: sources(selectedIndex[4]).runOnJS, onSelectedIndexChange };
          E.__closure = obj1;
          ...
          R.__initData = closure_15;
          obj.onMomentumEnd = R;
          const items = [animatedRef];
          const callback = mapped1.useCallback((arg0) => {
            if (!onSelectedIndexChange.get()) {
              const result = derivedValue2.set(arg0);
              const result1 = store.set("thumbnails");
            }
          }, []);
          const callback1 = mapped1.useCallback((arg0) => {
            arg0(animatedRef[4]).scrollTo(animatedRef, arg1[arg0].scrollStart, 0, true);
          }, items);
          let obj6 = sources(selectedIndex[4]);
          const fn2 = function k() {
            if (derivedValue2.get() >= 0) {
              const obj = arg0(animatedRef[4]);
              obj.scrollTo(animatedRef, derivedValue2.get() * arg0, 0, true);
            }
          };
          obj2 = { thumbnailsAnimateTo: derivedValue2, variableWidthThumbnailsEnabled: false, runOnJS: sources(selectedIndex[4]).runOnJS, scrollVarWidthThumbnails: callback1, scrollTo: sources(selectedIndex[4]).scrollTo, ref: animatedRef, thumbnailSize: THUMBNAIL_WIDTH_MARGIN };
          fn2.__closure = obj2;
          fn2.__workletHash = 1697086875584;
          fn2.__initData = closure_16;
          const derivedValue = obj6.useDerivedValue(fn2);
          const items1 = [animatedRef];
          const callback2 = mapped1.useCallback((arg0) => {
            const result = arg0(animatedRef[8]).lerpVarWidthThumbnailScrollBounds(arg1, arg0);
            const obj = arg0(animatedRef[8]);
            arg0(animatedRef[4]).scrollTo(animatedRef, result, 0, false);
          }, items1);
          let obj8 = sources(selectedIndex[4]);
          class Z {
            constructor() {
              tmp = 0 !== THUMBNAIL_WIDTH_MARGIN.get();
              if (tmp) {
                tmp2 = runOnJS;
                tmp4 = left;
                value = runOnJS.get();
              ...
              if (tmp) {
                tmp5 = THUMBNAIL_WIDTH_MARGIN;
                tmp6 = closure_2;
                num = 4;
                obj = THUMBNAIL_WIDTH_MARGIN(closure_2[4]);
                tmp7 = closure_2;
                tmp8 = left;
                tmp9 = THUMBNAIL_WIDTH_MARGIN;
                flag = false;
                tmp10 = obj;
                num2 = 0;
                scrollToResult = obj.scrollTo(closure_2, left.get() * THUMBNAIL_WIDTH_MARGIN, 0, false);
              }
              return;
            }
          }
          const animatedScrollHandler = obj2.useAnimatedScrollHandler(obj);
          Z.__closure = { viewerScrolling: diff1, thumbnailsIndex: mapped, viewerPos: diff, variableWidthThumbnailsEnabled: false, runOnJS: sources(selectedIndex[4]).runOnJS, lerpScrollVarWidthThumbnails: callback2, scrollTo: sources(selectedIndex[4]).scrollTo, ref: animatedRef, thumbnailSize: THUMBNAIL_WIDTH_MARGIN };
          Z.__workletHash = 10477949154269;
          Z.__initData = closure_17;
          const derivedValue1 = obj8.useDerivedValue(Z);
          const mapped = sources.map((arg0, arg1) => arg1);
          const mapped1 = sources.map((closure_1) => {
            const size = closure_1(animatedRef[9]).flattenSource(closure_1);
            let num = 0;
            if (null != size) {
              const _Math = Math;
              const _Math2 = Math;
              num = (Math.max(Math.min(size.width * (diff / size.height), diff1), onSelectedIndexChange) - onSelectedIndexChange) / 2;
            }
            return num;
          });
          const obj3 = { viewerScrolling: diff1, thumbnailsIndex: mapped, viewerPos: diff, variableWidthThumbnailsEnabled: false, runOnJS: sources(selectedIndex[4]).runOnJS, lerpScrollVarWidthThumbnails: callback2, scrollTo: sources(selectedIndex[4]).scrollTo, ref: animatedRef, thumbnailSize: THUMBNAIL_WIDTH_MARGIN };
          function ee() {
            let num = 0;
            if (mapped1.get() <= 0) {
              const obj = arg0(animatedRef[4]);
          ...
          const obj11 = sources(selectedIndex[4]);
          ee.__closure = { thumbnailsScrolling: mapped1, interpolate: sources(selectedIndex[4]).interpolate, viewerPos: diff, interpolateInput: mapped, interpolateOutput: mapped1 };
          ee.__workletHash = 10097839523885;
          ee.__initData = closure_18;
          const derivedValue2 = obj11.useDerivedValue(ee);
          const diff = (width - THUMBNAIL_WIDTH_MARGIN) / 2 - rect.left;
          const obj4 = { thumbnailsScrolling: mapped1, interpolate: sources(selectedIndex[4]).interpolate, viewerPos: diff, interpolateInput: mapped, interpolateOutput: mapped1 };
          function te() {
            let obj = {};
            if (diff1.get() > 0) {
              let sum = diff + derivedValue2.get();
            } else {
              obj = { overshootClamping: true };
              sum = arg0(animatedRef[10]).withSpring(diff + derivedValue2.get(), obj);
              const obj2 = arg0(animatedRef[10]);
            }
            obj.width = sum;
            return obj;
          }
          const obj13 = sources(selectedIndex[4]);
          ...
          const diff1 = (width - THUMBNAIL_WIDTH_MARGIN) / 2 - rect.right;
          const animatedStyle = obj13.useAnimatedStyle(te);
          const obj5 = { viewerScrolling: diff1, headerBufferSize: diff, margin: derivedValue2, withSpring: sources(selectedIndex[10]).withSpring };
          function ne() {
            let obj = {};
            if (diff1.get() > 0) {
              ...
              obj = { overshootClamping: true };
              sum = arg0(animatedRef[10]).withSpring(diff1 + derivedValue2.get(), obj);
              const obj2 = arg0(animatedRef[10]);
            }
            obj.width = sum;
            return obj;
          }
          obj6 = { viewerScrolling: diff1, footerBufferSize: diff1, margin: derivedValue2, withSpring: sources(selectedIndex[10]).withSpring };
          ne.__closure = obj6;
          ne.__workletHash = 10532164558483;
          ne.__initData = closure_20;
          const animatedStyle1 = sources(selectedIndex[4]).useAnimatedStyle(ne);
          const callback3 = mapped1.useCallback((width, index) => {
            width = index;
            let obj = width(animatedRef[4]);
            const fn = function i() { ... };
            obj = { interpolate: width(animatedRef[4]).interpolate, viewerPos: diff, index };
            fn.__closure = obj;
            fn.__workletHash = 5784737783661;
            fn.__initData = closure_21;
            const derivedValue = obj.useDerivedValue(fn);
            index = derivedValue;
            const bound = Math.max(Math.min(width.width * (diff / width.height), diff1), onSelectedIndexChange);
            const animatedRef = bound;
            const fn2 = function l() { ... };
            obj = { thumbnailsScrolling: mapped1, THUMBNAIL_MIN_WIDTH: onSelectedIndexChange, interpolate: width(animatedRef[4]).interpolate, viewerPos: diff, index, sourceWidth: bound };
            fn2.__closure = obj;
            fn2.__workletHash = 12440745987072;
            fn2.__initData = closure_22;
            const derivedValue1 = width(animatedRef[4]).useDerivedValue(fn2);
            const mapped = derivedValue1;
            const obj3 = width(animatedRef[4]);
            const fn3 = function u() { ... };
            const obj5 = width(animatedRef[4]);
            fn3.__closure = { viewerScrolling: diff1, _width: derivedValue1, withSpring: width(animatedRef[10]).withSpring, THUMBNAIL_HEIGHT: diff, opacity: derivedValue };
            fn3.__workletHash = 513826663139;
            fn3.__initData = closure_23;
            return obj5.useAnimatedStyle(fn3);
          }, []);
          const obj15 = sources(selectedIndex[4]);
          function ie() {
            return !onSelectedIndexChange.get();
          }
          ie.__closure = { zoomed };
          ie.__workletHash = 7667674289153;
          ie.__initData = closure_24;
          obj8 = {
            ref: animatedRef,
            ...
            scrollEnabled: sources(selectedIndex[4]).useDerivedValue(ie),
        ...
        useViewerProps() {
          let obj = sources(selectedIndex[4]);
          const animatedRef = obj.useAnimatedRef();
          const sources = animatedRef;
          let obj1 = sources(selectedIndex[5]);
          const size = obj1.useMediaViewerDimensions();
          const width = size.width;
          const thumbnailScrollPositions = width;
          let obj2 = sources(selectedIndex[4]);
          const fn = function i() {
            if (-1 !== store.get()) {
              const obj = animatedRef(memo[4]);
              obj.scrollTo(animatedRef, store.get() * width, 0, false);
            }
          };
          obj = { thumbnailsAnimateTo: closure_5, scrollTo: sources(selectedIndex[4]).scrollTo, ref: animatedRef, screenWidth: width };
          fn.__closure = obj;
          ...
          obj = { thumbnailsScrolling: React, viewerScrolling: closure_7, thumbnailsAnimateTo: closure_5, scrollTo: sources(selectedIndex[4]).scrollTo, ref: animatedRef, thumbnailsIndex: sharedValue, screenWidth: width };
          fn2.__closure = obj;
          fn2.__workletHash = 16855593341498;
          fn2.__initData = closure_26;
          const derivedValue1 = sources(selectedIndex[4]).useDerivedValue(fn2);
          const memo = React.useMemo(() => {
            ...
            onBeginDrag.__initData = closure_27;
            ...
            onMomentumEnd.__initData = closure_31;
            return { onBeginDrag, onEndDrag, onScroll, onMomentumBegin, onMomentumEnd };
          }, []);
          const selectedIndex = memo;
          const obj5 = sources(selectedIndex[4]);
          const sharedValue = sources(selectedIndex[4]).useSharedValue(0);
          const obj7 = sources(selectedIndex[4]);
                ...
                if (tmp2 < 500) {
                  ...
                  if (0 === arg0.contentOffset.x) {
                    ...
                    if (0 !== closure_2.get()) {
                      ...
                      flag = false;
                      tmp15 = obj;
                      scrollToResult = obj.scrollTo(closure_0, closure_2.get() * width, arg0.contentOffset.y, false);
                    }
                    return;
                  }
                }
              }
              tmp4 = 0 === closure_7.get();
              if (tmp4) {
                tmp5 = closure_4;
                tmp4 = 0 === closure_4.get();
              }
              if (tmp4) {
                tmp6 = closure_5;
                num3 = -1;
                tmp4 = -1 === closure_5.get();
              }
              if (!tmp4) {
                tmp7 = closure_2;
                tmp8 = width;
          ...
          obj2 = { contentSizeLastChangedAt: sharedValue, IS_ANDROID: closure_10, selectedIndex, scrollTo: sources(selectedIndex[4]).scrollTo, ref: animatedRef, screenWidth: width, viewerScrolling: closure_7, thumbnailsScrolling: React, thumbnailsAnimateTo: closure_5, onScrollWorklets: memo };
          S.__closure = obj2;
          ...
          const obj8 = sources(selectedIndex[4]);
          ...
          const animatedScrollHandler = sources(selectedIndex[4]).useAnimatedScrollHandler(obj1);
          return {
            ref: animatedRef,
            onScroll: sources(selectedIndex[4]).useAnimatedScrollHandler(obj1),
            onContentSizeChange: callback,
            useItemVisible: React.useCallback((index) => {
              const animatedRef = index;
              const tmp = sharedValue(React.useState(index === memo.get()), 2);
              const width = tmp2;
