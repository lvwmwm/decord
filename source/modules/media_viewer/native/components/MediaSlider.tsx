...
// Function ID: 8524
// Name: MediaSlider
// Dependencies: [32, 19, 17, 21, 4446, 500, 5906, 12, 689, 5077, 5033, 1236, 8524, 8526, 4442, 8327, 8528, 712, 2]
// Exports: default

// Module 8523 (MediaSlider)
...
export default function MediaSlider(controls) {
  ...
  closure_9 = tmp8[1];
  const tmp11 = paused(5906)(() => paused(closure_1_2[7]).throttle((arg0) => {
    callback(arg0);
  }, 100));
  closure_10 = tmp11;
  closure_11 = paused(5906)(() => paused(closure_1_2[7]).throttle((arg0) => {
    callback(arg0);
    let str = "transparent";
    if (1 === arg0) {
  ...
  if (paused) {
    let PauseIcon = tmp16(8524).PlayIcon;
  } else {
    PauseIcon = tmp16(8526).PauseIcon;
  }
  obj[5] = closure_6(PauseIcon, { size: "md", color: "white" });
  const items4 = [closure_6(controls(5033).PressableOpacity, obj), , , ];
  const items5 = [tmp.centerText, { width: tmp8[0] }];
  items4[1] = closure_6(controls(4442).Text, { style: items5, tabularNumbers: true, lineClamp: 1, color: "text-overlay-light", variant: "text-xs/medium", children: timeFormat });
  ...
  obj5[8] = callback;
  obj5[9] = callback1;
  items7[1] = closure_6(paused(8327), obj5);
  obj1[1] = items7;
  items4[2] = c7(c5, obj1);
  obj6 = {
  ...
  const tmp9Result1 = paused(8327);
  obj6[6] = controls(5077).getTimeFormat(ref.current);
  items4[3] = closure_6(controls(4442).Text, obj6);
  obj[1] = items4;
  return c7(c5, obj);
};
