...
// Function ID: 8900
// Name: MediaSlider
// Dependencies: [32, 19, 17, 21, 4344, 500, 5755, 12, 689, 4930, 4886, 1236, 8900, 8902, 4340, 8030, 8904, 712, 2]
// Exports: default

// Module 8899 (MediaSlider)
...
export default function MediaSlider(controls) {
  ...
  closure_9 = tmp8[1];
  const tmp11 = paused(5755)(() => paused(outer1_2[7]).throttle((arg0) => {
    callback(arg0);
  }, 100));
  c10 = tmp11;
  closure_11 = paused(5755)(() => paused(outer1_2[7]).throttle((arg0) => {
    callback(arg0);
    let str = "transparent";
    if (1 === arg0) {
  ...
  if (paused) {
    let PauseIcon = tmp16(8900).PlayIcon;
  } else {
    PauseIcon = tmp16(8902).PauseIcon;
  }
  obj[5] = closure_6(PauseIcon, { size: "md", color: "white" });
  const items4 = [closure_6(controls(4886).PressableOpacity, obj), , , ];
  const items5 = [tmp.centerText, { width: tmp8[0] }];
  items4[1] = closure_6(controls(4340).Text, { style: items5, tabularNumbers: true, lineClamp: 1, color: "text-overlay-light", variant: "text-xs/medium", children: timeFormat });
  ...
  obj5[8] = callback;
  obj5[9] = callback1;
  items7[1] = closure_6(paused(8030), obj5);
  obj1[1] = items7;
  items4[2] = c7(c5, obj1);
  obj6 = {
  ...
  const tmp9Result1 = paused(8030);
  obj6[6] = controls(4930).getTimeFormat(ref.current);
  items4[3] = closure_6(controls(4340).Text, obj6);
  obj[1] = items4;
  return c7(c5, obj);
};
