...
// Function ID: 9002
// Name: MediaSlider
// Dependencies: [32, 19, 17, 21, 4380, 500, 5822, 12, 689, 4993, 4949, 1236, 9002, 9004, 4376, 8140, 9006, 712, 2]
// Exports: default

// Module 9001 (MediaSlider)
...
export default function MediaSlider(controls) {
  ...
  closure_9 = tmp8[1];
  const tmp11 = paused(5822)(() => paused(closure_1_2[7]).throttle((arg0) => {
    callback(arg0);
  }, 100));
  closure_10 = tmp11;
  closure_11 = paused(5822)(() => paused(closure_1_2[7]).throttle((arg0) => {
    callback(arg0);
    let str = "transparent";
    if (1 === arg0) {
  ...
  if (paused) {
    let PauseIcon = tmp16(9002).PlayIcon;
  } else {
    PauseIcon = tmp16(9004).PauseIcon;
  }
  obj[5] = closure_6(PauseIcon, { size: "md", color: "white" });
  const items4 = [closure_6(controls(4949).PressableOpacity, obj), , , ];
  const items5 = [tmp.centerText, { width: tmp8[0] }];
  items4[1] = closure_6(controls(4376).Text, { style: items5, tabularNumbers: true, lineClamp: 1, color: "text-overlay-light", variant: "text-xs/medium", children: timeFormat });
  ...
  obj5[8] = callback;
  obj5[9] = callback1;
  items7[1] = closure_6(paused(8140), obj5);
  obj1[1] = items7;
  items4[2] = c7(c5, obj1);
  obj6 = {
  ...
  const tmp9Result1 = paused(8140);
  obj6[6] = controls(4993).getTimeFormat(ref.current);
  items4[3] = closure_6(controls(4376).Text, obj6);
  obj[1] = items4;
  return c7(c5, obj);
};
