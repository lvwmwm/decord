...
// Function ID: 12776
// Name: MediaViewer
// Dependencies: [32, 19, 17, 21, 500, 4186, 12776, 12777, 8515, 5617, 7742, 5918, 5938, 8534, 12781, 4447, 8520, 8549, 9469, 4187, 9471, 2]

// Module 12775 (MediaViewer)
import MediaViewerDimensionsProvider from "MediaViewerDimensionsProvider" /* 8534 */;
...
function MediaViewer(arg0) {
  ...
  let value = [height(5938).MEDIA_VIEWER];
  let tmp = height;
  const tmp3 = height(5918);
  [tmp5, tmp6] = sharedValue(sharedValue1.useState(true), 2);
  _require = tmp6;
  let obj = _require(8534);
  const mediaViewerDimensions = obj.useMediaViewerDimensions();
  ({ width, height } = mediaViewerDimensions);
  const tmp9 = height(12781)({ index, onClose, sources, windowHeight: height, windowWidth: width });
  dependencyMap = tmp9;
  obj1 = _require(4186);
  ...
  obj = { absoluteFillObject, windowHeight: height, entranceAnimationDriver: sharedValue, interpolate: _require(4186).interpolate, translatePos, closePosition: tmp9 };
  M.__closure = obj;
  ...
  obj1 = { isClosing, hideRelayoutSharedValue: sharedValue1, overlayEnabled, isInteracting, withTiming: _require(4447).withTiming, Easing: _require(4186).Easing, runOnJS: _require(4186).runOnJS, setShowHeader: tmp6 };
  P.__closure = obj1;
  P.__workletHash = 14874736878670;
  P.__initData = closure_14;
  const animatedStyle1 = _require(4186).useAnimatedStyle(P);
  const obj9 = _require(4186);
  let fn = function x() {
    ...
    if (value) {
      _undefined(4186).runOnJS(_undefined)(true);
      const obj = _undefined(4186);
    }
  };
  obj2 = { overlayEnabled, isInteracting, runOnJS: _require(4186).runOnJS, setShowHeader: tmp6 };
  fn.__closure = obj2;
  fn.__workletHash = 2199049578465;
  fn.__initData = closure_16;
  const derivedValue = _require(4186).useDerivedValue(fn);
  const obj11 = _require(4186);
  function le() {
    let obj = {};
    ...
    if (!sharedValue1.get()) {
      ...
      obj[0] = _undefined(4186).Easing.linear;
      num = _undefined(4447).withTiming(1, obj);
      const obj3 = _undefined(4447);
    }
    obj.opacity = num;
    return obj;
  }
  obj3 = { absoluteFillObject, translatePos, hideRelayoutSharedValue: sharedValue1, withTiming: _require(4447).withTiming, Easing: _require(4186).Easing };
  le.__closure = obj3;
  le.__workletHash = 8244409104374;
  le.__initData = closure_17;
  const animatedStyle2 = _require(4186).useAnimatedStyle(le);
  ...
  items5[3] = translatePos(height(4187), obj6);
  items5[4] = translatePos(tmp(9471), {});
  obj4[3] = items5;
  const children = tmp28(tmp29, obj4);
  return translatePos(tmp7(5918).AnalyticsLocationProvider, { value: tmp3(value).analyticsLocations, children });
}
