...
// Function ID: 12757
// Name: MediaViewer
// Dependencies: [32, 19, 17, 21, 500, 4185, 12757, 12758, 8501, 5604, 7728, 5905, 5925, 8520, 12762, 4446, 8506, 8534, 9451, 4186, 9453, 2]

// Module 12756 (MediaViewer)
import MediaViewerDimensionsProvider from "MediaViewerDimensionsProvider" /* 8520 */;
...
function MediaViewer(arg0) {
  ...
  let value = [height(5925).MEDIA_VIEWER];
  let tmp = height;
  const tmp3 = height(5905);
  [tmp5, tmp6] = sharedValue(sharedValue1.useState(true), 2);
  _require = tmp6;
  let obj = _require(8520);
  const mediaViewerDimensions = obj.useMediaViewerDimensions();
  ({ width, height } = mediaViewerDimensions);
  const tmp9 = height(12762)({ index, onClose, sources, windowHeight: height, windowWidth: width });
  dependencyMap = tmp9;
  obj1 = _require(4185);
  ...
  obj = { absoluteFillObject, windowHeight: height, entranceAnimationDriver: sharedValue, interpolate: _require(4185).interpolate, translatePos, closePosition: tmp9 };
  M.__closure = obj;
  ...
  obj1 = { isClosing, hideRelayoutSharedValue: sharedValue1, overlayEnabled, isInteracting, withTiming: _require(4446).withTiming, Easing: _require(4185).Easing, runOnJS: _require(4185).runOnJS, setShowHeader: tmp6 };
  P.__closure = obj1;
  P.__workletHash = 14874736878670;
  P.__initData = closure_14;
  const animatedStyle1 = _require(4185).useAnimatedStyle(P);
  const obj9 = _require(4185);
  let fn = function x() {
    ...
    if (value) {
      _undefined(4185).runOnJS(_undefined)(true);
      const obj = _undefined(4185);
    }
  };
  obj2 = { overlayEnabled, isInteracting, runOnJS: _require(4185).runOnJS, setShowHeader: tmp6 };
  fn.__closure = obj2;
  fn.__workletHash = 2199049578465;
  fn.__initData = closure_16;
  const derivedValue = _require(4185).useDerivedValue(fn);
  const obj11 = _require(4185);
  function le() {
    let obj = {};
    ...
    if (!sharedValue1.get()) {
      ...
      obj[0] = _undefined(4185).Easing.linear;
      num = _undefined(4446).withTiming(1, obj);
      const obj3 = _undefined(4446);
    }
    obj.opacity = num;
    return obj;
  }
  obj3 = { absoluteFillObject, translatePos, hideRelayoutSharedValue: sharedValue1, withTiming: _require(4446).withTiming, Easing: _require(4185).Easing };
  le.__closure = obj3;
  le.__workletHash = 8244409104374;
  le.__initData = closure_17;
  const animatedStyle2 = _require(4185).useAnimatedStyle(le);
  ...
  const callback2 = sharedValue1.useCallback((orientation, orientation2) => {
    ...
    if (isInteracting) {
      const result = _undefined(8506).unpauseCurrentVideoIfNeeded();
      const obj = _undefined(8506);
    }
  }, items4);
  const obj13 = _require(4185);
  const orientationListener = _require(8534).useOrientationListener(callback2);
  obj4 = { style: closure_6.absoluteFill, onAccessibilityEscape: dismiss, onLayout: callback1, children: null };
  const obj15 = _require(8534);
  obj5 = { barStyle: "light-content", hidden: !tmp5 };
  const items5 = [translatePos(height(9451), obj5), translatePos(height(4185).View, { style: animatedStyle }), translatePos(height(4186), { ref: animatedRef, style: animatedStyle2, children: translatePos(closure_12, { entranceAnimationDriver: sharedValue, onContentSizeChange, onScroll, onLongPress, originLayout, panGestureConfig: mediaViewerPanGestureConfig, ref, renderMedia, sources, useItemVisible, windowHeight: height, windowWidth: width, index, zoomed }) }), , ];
  ...
  items5[3] = translatePos(height(4186), obj6);
  items5[4] = translatePos(tmp(9453), {});
  obj4[3] = items5;
  const children = tmp28(tmp29, obj4);
  return translatePos(tmp7(5905).AnalyticsLocationProvider, { value: tmp3(value).analyticsLocations, children });
}
