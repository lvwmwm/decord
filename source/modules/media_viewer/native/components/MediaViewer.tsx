...
// Function ID: 12323
// Name: MediaViewer
// Dependencies: [32, 19, 17, 21, 500, 4083, 12323, 12324, 8777, 5466, 8049, 5767, 5787, 8910, 12328, 4345, 8896, 8940, 7789, 4084, 10714, 2]

// Module 12322 (MediaViewer)
...
function MediaViewer(arg0) {
  ...
  let value = [height(5787).MEDIA_VIEWER];
  let tmp = height;
  const tmp3 = height(5767);
  [tmp5, tmp6] = sharedValue(sharedValue1.useState(true), 2);
  _require = tmp6;
  let obj = _require(8910);
  const mediaViewerDimensions = obj.useMediaViewerDimensions();
  ({ width, height } = mediaViewerDimensions);
  const tmp9 = height(12328)({ index, onClose, sources, windowHeight: height, windowWidth: width });
  dependencyMap = tmp9;
  ...
  obj1 = { isClosing, hideRelayoutSharedValue: sharedValue1, overlayEnabled, isInteracting, withTiming: _require(4345).withTiming, Easing: _require(4083).Easing, runOnJS: _require(4083).runOnJS, setShowHeader: tmp6 };
  P.__closure = obj1;
  ...
  obj3 = { absoluteFillObject, translatePos, hideRelayoutSharedValue: sharedValue1, withTiming: _require(4345).withTiming, Easing: _require(4083).Easing };
  le.__closure = obj3;
  ...
  items5[4] = translatePos(tmp(10714), {});
  obj4[3] = items5;
  const children = tmp28(tmp29, obj4);
  return translatePos(tmp7(5767).AnalyticsLocationProvider, { value: tmp3(value).analyticsLocations, children });
}
...
export default importAllResult.memo((arg0) => {
  ...
  return callback(require(8910) /* MediaViewerDimensionsProvider */.MediaViewerDimensionsProvider, obj);
});
