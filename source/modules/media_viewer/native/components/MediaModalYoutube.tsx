// Module ID: 12908
// Function ID: 12909
// Dependencies: [32, 19, 673, 21, 8227, 8202, 8196, 8193, 12909, 1234, 2]

// Module 12908
import closure_3 from "_slicedToArray" /* 32 */;
import importAllResult from "noop" /* 19 */;
import { YOUTUBE_EMBED_PAGE_TYPE } from "ME" /* 673 */;
...
const memoResult = importAllResult.memo((visible) => {
  ...
  closure_5 = undefined;
  let ref;
  let tmp4 = callback(importAllResult.useState(visible(8227).PlayerState.UNREADY), 2);
  first = tmp4[0];
  dependencyMap = tmp4[1];
  const tmp6 = callback(importAllResult.useState(undefined), 2);
  callback = tmp6[1];
  const tmp8 = first(8202)(first);
  importAllResult = tmp8;
  const tmp9 = first(8202)(visible);
  closure_5 = tmp9;
  ref = importAllResult.useRef(null);
  const effect = importAllResult.useEffect(() => {
    const MediaViewerAnalytics = visible(8196).MediaViewerAnalytics;
  ...
  callback = importAllResult.useCallback((arg0) => {
    ...
    if ("onReady" === type) {
      if ("-1" === value) {
        let READY = visible(8227).PlayerState.ERRORED;
      } else {
        READY = visible(8227).PlayerState.READY;
      }
      dependencyMap(READY);
      ...
      dependencyMap(visible(8227).PlayerState.ERRORED);
      callback(str6);
      const MediaViewerAnalytics = visible(8196).MediaViewerAnalytics;
      ...
      obj[0] = visible(8227).PlayerState.UNSTARTED;
      obj[0] = visible(8227).PlayerState.ENDED;
      obj[1] = visible(8227).PlayerState.PLAYING;
      obj[2] = visible(8227).PlayerState.PAUSED;
      obj[3] = visible(8227).PlayerState.BUFFERING;
      obj[5] = visible(8227).PlayerState.VIDEO_CUED;
      let tmp4 = null != tmp35;
      if (tmp4) {
        tmp4 = tmp35 in visible(8227).PlayerState;
      }
      if (tmp4) {
        dependencyMap(tmp35);
      }
  ...
  const effect1 = importAllResult.useEffect(() => {
    ...
    if (tmp2) {
      tmp2 = first !== visible(8227).PlayerState.UNREADY;
    }
    if (tmp2) {
      let tmp7 = visible;
      if (visible) {
        tmp7 = closure_4 === visible(8227).PlayerState.UNREADY;
      }
      if (tmp7) {
        tmp7 = first === visible(8227).PlayerState.READY;
      }
      if (tmp7) {
  ...
  if (null == youtubeVideoIdFromURI) {
    ...
    return ref(tmp7(8227), obj, youtubeVideoIdFromURI.videoId);
  }
});
