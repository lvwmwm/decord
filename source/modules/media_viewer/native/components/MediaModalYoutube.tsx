// Module ID: 12318
// Function ID: 12319
// Dependencies: [32, 19, 676, 21, 8913, 8898, 8892, 8891, 12319, 500, 2]
...
const memoResult = importAllResult.memo((visible) => {
  ...
  let c5;
  let ref;
  let tmp4 = callback(importAllResult.useState(visible(8913).PlayerState.UNREADY), 2);
  first = tmp4[0];
  dependencyMap = tmp4[1];
  const tmp6 = callback(importAllResult.useState(undefined), 2);
  callback = tmp6[1];
  const tmp8 = first(8898)(first);
  importAllResult = tmp8;
  const tmp9 = first(8898)(visible);
  c5 = tmp9;
  ref = importAllResult.useRef(null);
  const effect = importAllResult.useEffect(() => {
    const MediaViewerAnalytics = visible(8892).MediaViewerAnalytics;
  ...
  callback = importAllResult.useCallback((arg0) => {
    ...
    if ("onReady" === type) {
      if ("-1" === value) {
        let READY = visible(8913).PlayerState.ERRORED;
      } else {
        READY = visible(8913).PlayerState.READY;
      }
      dependencyMap(READY);
      ...
      dependencyMap(visible(8913).PlayerState.ERRORED);
      callback(str6);
      const MediaViewerAnalytics = visible(8892).MediaViewerAnalytics;
      ...
      obj[0] = visible(8913).PlayerState.UNSTARTED;
      obj[0] = visible(8913).PlayerState.ENDED;
      obj[1] = visible(8913).PlayerState.PLAYING;
      obj[2] = visible(8913).PlayerState.PAUSED;
      obj[3] = visible(8913).PlayerState.BUFFERING;
      obj[5] = visible(8913).PlayerState.VIDEO_CUED;
      let tmp4 = null != tmp35;
      if (tmp4) {
        tmp4 = tmp35 in visible(8913).PlayerState;
      }
      if (tmp4) {
        dependencyMap(tmp35);
      }
  ...
  const effect1 = importAllResult.useEffect(() => {
    ...
    if (tmp2) {
      tmp2 = first !== visible(8913).PlayerState.UNREADY;
    }
    if (tmp2) {
      let tmp7 = visible;
      if (visible) {
        tmp7 = c4 === visible(8913).PlayerState.UNREADY;
      }
      if (tmp7) {
        tmp7 = first === visible(8913).PlayerState.READY;
      }
      if (tmp7) {
  ...
  if (null == youtubeVideoIdFromURI) {
    ...
    return ref(tmp7(8913), obj, youtubeVideoIdFromURI.videoId);
  }
});
