...
// Dependencies: [32, 1390, 4462, 4467, 1218, 5174, 1391, 4534, 1981, 5315, 1922, 9412, 4464, 4466, 676, 3, 9411, 12, 38, 9786, 4511, 589, 709, 2]

// Module 4461 (getParticipants)
import timestampDefault from "timestamp" /* 3 */;
import applyDefault from "apply" /* 12 */;
import initializeDefault from "initialize" /* 589 */;
import dispatcherDefault from "dispatcher" /* 709 */;
import sortKey from "sortKey" /* 9411 */;
import sortKeyDefault from "sortKey" /* 9411 */;
...
function updateParticipant(arg0, arr) {
  const f76234 = (updateParticipant) => updateParticipant.updateParticipant(f76234);
    ...
    if (id(tmp4)) {
      ...
      if (0 !== obj.size()) {
        ...
        if (!Boolean(isGuildVocalOrThreadResult)) {
          let hasEmbeddedActivityResult = obj.size(f76234(closure_1_2[16]).ChannelRTCParticipantsIndexes.STREAM) > 0;
      ...
      if (0 !== obj2.size()) {
        if (closure_1_11.getVoiceChannelId() === arg1) {
          ...
          const found = obj2.toArray(f76234(closure_1_2[16]).ChannelRTCParticipantsIndexes.STREAM).find((type) => {
          ...
          flag = true;
          const toArrayResult = obj2.toArray(f76234(closure_1_2[16]).ChannelRTCParticipantsIndexes.STREAM);
        }
      }
...
function handleEmbeddedActivityChange() {
  const f76236 = (updateEmbeddedActivities) => updateEmbeddedActivities.updateEmbeddedActivities();
...
function handleSpeaking(arg0) {
  const f76237 = (updateParticipantSpeaking) => updateParticipantSpeaking.updateParticipantSpeaking(f76237);
  return closure_25.reduce((arg0, arg1) => {
    let tmp4 = closure_1_27[arg1];
    if (null == tmp4) {
      const tmp10 = new closure_1_1(closure_1_2[16])(arg1);
...
function handleUserUpdate(user) {
  let f76234 = user.user.id;
  f76234 = (updateParticipant) => updateParticipant.updateParticipant(f76234);
...
function handleCallUpdate(channelId) {
  ...
  const f76238 = (rebuild) => rebuild.rebuild();
...
function handleStreamClose(streamKey) {
  obj = f76234(4511);
  const items = [];
  ({ channelId: arr[0], ownerId: f76234 } = f76234(4511).decodeStreamKey(streamKey.streamKey));
  f76234 = (updateParticipant) => updateParticipant.updateParticipant(f76234);
  return items.reduce((arg0, arg1) => {
    let tmp4 = closure_1_27[arg1];
    if (null == tmp4) {
...
obj = {
  ...
  VOICE_CHANNEL_SELECT: function handleVoiceChannelSelect(arg0) {
    ...
    if (channelId !== currentVoiceChannelId) {
      ...
      if (null != currentVoiceChannelId) {
        ...
        const item = guildRingingUsers.forEach((userId) => f76241.updateGuildRingingUsers(userId, false));
        const items = [currentVoiceChannelId];
        f76241 = (rebuild) => rebuild.rebuild();
  ...
  CHANNEL_RTC_SELECT_PARTICIPANT: function handleSelectParticipant(arg0) {
    ...
    if (null == id) {
      const item = obj.toArray(obj(9411).ChannelRTCParticipantsIndexes.STREAM).forEach((user) => {
        if (closure_1_18(user)) {
          obj.updateParticipant(user.user.id);
        }
      });
      const toArrayResult = obj.toArray(obj(9411).ChannelRTCParticipantsIndexes.STREAM);
    }
    let NONE = id;
    if (id == null) {
    ...
    if (obj2.isStreamKey(id)) {
      try {
        const ownerId = obj(4511).decodeStreamKey(id).ownerId;
  ...
  CHANNEL_RTC_POPOUT_PARTICIPANT: function handlePopOutParticipant(arg0) {
    ...
    if (tmp16) {
      ...
      const items1 = [channelId];
      let f76234 = participant.user.id;
      f76234 = (updateParticipant) => updateParticipant.updateParticipant(f76234);
  ...
  STREAM_UPDATE_SELF_HIDDEN: function handleUpdateSelfStreamHidden(channelId) {
    ...
    if (channelId.selfStreamHidden) {
      ...
      const obj3 = f76234(4511);
      if (tmp12) {
        setSelectedParticipantId(channelId, null);
      }
      tmp12 = f76234(4511).isStreamKey(first) && first.includes(id);
    }
    const items1 = [channelId];
    f76234 = (updateParticipant) => updateParticipant.updateParticipant(f76234);
    const reduced = items1.reduce((arg0, arg1) => {
  ...
  RTC_CONNECTION_VIDEO: function handleRTCConnectionVideo(arg0) {
    const items = [];
    ({ channelId: arr[0], userId: f76234 } = arg0);
    f76234 = (updateParticipant) => updateParticipant.updateParticipant(f76234);
    return items.reduce((arg0, arg1) => {
      let tmp4 = closure_1_27[arg1];
      if (null == tmp4) {
  ...
  MEDIA_ENGINE_VIDEO_SOURCE_QUALITY_CHANGED: function handleVideoSourceQuality(channelId) {
    ({ senderUserId: f76244, maxResolution: importDefault, maxFrameRate: dependencyMap } = channelId);
    const items = [channelId.channelId];
    f76244 = (updateParticipantQuality) => updateParticipantQuality.updateParticipantQuality(f76244, closure_1, closure_2);
    return items.reduce((arg0, arg1) => {
      let tmp4 = closure_1_27[arg1];
      if (null == tmp4) {
        const tmp10 = new closure_1_1(closure_1_2[16])(arg1);
  ...
  PUSH_TO_TALK_STATE_CHANGE: function handlePushToTalkStateChange() {
    const f76245 = (updateParticipantSpeaking) => updateParticipantSpeaking.updateParticipantSpeaking(id.getId());
    return closure_25.reduce((arg0, arg1) => {
      let tmp4 = closure_1_27[arg1];
      if (null == tmp4) {
        const tmp10 = new closure_1_1(closure_1_2[16])(arg1);
  ...
  GUILD_RING_START: function handleGuildRingStart(channelId) {
    ...
    if (obj.getCurrentConfig({ guildId, location: "guild_ring_start" }).enabled) {
      ...
      const item = keys.forEach((userId) => f76247.updateGuildRingingUsers(userId, true));
      const items = [channelId];
      f76247 = (rebuild) => rebuild.rebuild();
      return items.reduce((arg0, arg1) => {
        let tmp4 = closure_1_27[arg1];
        if (null == tmp4) {
          const tmp10 = new closure_1_1(closure_1_2[16])(arg1);
  ...
  GUILD_RING_STOP: function handleGuildRingStop(guildId) {
    ...
    if (obj.getCurrentConfig({ guildId: guildId.guildId, location: "guild_ring_start" }).enabled) {
      ...
      f76249 = tmp4;
      const item = ringing.forEach((userId) => f76249.updateGuildRingingUsers(userId, false));
      const items = [channelId];
      f76249 = (rebuild) => rebuild.rebuild();
      return items.reduce((arg0, arg1) => {
        let tmp4 = closure_1_27[arg1];
        if (null == tmp4) {
          const tmp10 = new closure_1_1(closure_1_2[16])(arg1);
