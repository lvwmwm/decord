...
// Dependencies: [5, 17, 9780, 4322, 1218, 4995, 1372, 4351, 9753, 4375, 3938, 4147, 1903, 676, 4364, 3, 10602, 5261, 16580, 5129, 9242, 4513, 9155, 10821, 10825, 1605, 4494, 9779, 2]

// Module 16579 (nativeEventEmitter)
...
prototype["reportIncomingCall"] = function reportIncomingCall(channelId, channelName, arg2) {
  ...
  if (null != channel) {
    ...
    channelName = _require(4494).computeChannelName(channel, mergeGuildAvatar, upsertRelationship);
  ...
  if (channelId === arg0) {
    ...
    if (null != self.pendingMutePreference) {
      ...
      if (isSelfMuteResult !== pendingMutePreference) {
        ...
        importDefault(9155).setSelfMute(MediaEngineContextTypes.DEFAULT, pendingMutePreference, false);
        tmp7 = pendingMutePreference;
        const obj2 = importDefault(9155);
      }
    }
    const result = importDefault(16580).setIncomingCallActive(arg0, tmp7);
    self.lastMuteState = tmp7;
    self.lastScreenShareActive = false;
    const obj3 = importDefault(16580);
  } else {
...
prototype["startRingtone"] = function startRingtone() {
  ...
  if (isMetaQuestResult) {
    ...
    if (!disableSounds) {
      self.ringtone = require(9779) /* createSoundForPack */.createSoundForPack("call_ringing", soundpack.getSoundpack());
      const ringtone = self.ringtone;
      ringtone.loop();
      const tmpResult = require(9779) /* createSoundForPack */;
    }
  }
};
