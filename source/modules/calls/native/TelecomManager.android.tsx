...
// Dependencies: [5, 17, 9666, 4274, 1218, 4946, 1372, 4302, 9639, 4326, 3892, 4095, 1874, 676, 4315, 3, 10489, 5214, 16427, 5081, 9132, 4464, 9049, 10687, 10691, 1577, 4445, 9665, 2]

// Module 16426 (nativeEventEmitter)
...
prototype["reportIncomingCall"] = function reportIncomingCall(channelId, channelName, arg2) {
  ...
  if (null != channel) {
    ...
    channelName = _require(4445).computeChannelName(channel, mergeGuildAvatar, upsertRelationship);
    ...
    const obj4 = self(16427);
    const reportIncomingCallResult = self(16427).reportIncomingCall(channelId, channelName, tmp19);
    self(16427).reportIncomingCall(channelId, channelName, tmp19).then((arg0) => {
      if (!arg0) {
    ...
    const nextPromise = self(16427).reportIncomingCall(channelId, channelName, tmp19).then((arg0) => {
      if (!arg0) {
        outer1_25.warn("Failed to report incoming call: resolved false");
        self.clearCall(closure_0);
      }
  ...
  if (channelId === arg0) {
    ...
    if (null != self.pendingMutePreference) {
      ...
      if (isSelfMuteResult !== pendingMutePreference) {
        ...
        importDefault(9049).setSelfMute(MediaEngineContextTypes.DEFAULT, pendingMutePreference, false);
        tmp7 = pendingMutePreference;
        const obj2 = importDefault(9049);
      }
    }
    const result = importDefault(16427).setIncomingCallActive(arg0, tmp7);
    self.lastMuteState = tmp7;
    self.lastScreenShareActive = false;
    const obj3 = importDefault(16427);
  } else {
...
prototype["startRingtone"] = function startRingtone() {
  ...
  if (isMetaQuestResult) {
    ...
    if (!disableSounds) {
      self.ringtone = require(9665) /* createSoundForPack */.createSoundForPack("call_ringing", soundpack.getSoundpack());
      const ringtone = self.ringtone;
      ringtone.loop();
      const tmpResult = require(9665) /* createSoundForPack */;
    }
  }
};
