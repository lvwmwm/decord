...
// Dependencies: [5, 17, 9713, 4321, 1218, 4993, 1372, 4349, 9686, 4373, 3938, 4141, 1903, 676, 4362, 3, 10535, 5260, 16494, 5128, 9178, 4511, 9095, 10754, 10758, 1605, 4492, 9712, 2]

// Module 16493 (nativeEventEmitter)
...
prototype["reportIncomingCall"] = function reportIncomingCall(channelId, channelName, arg2) {
  ...
  if (null != channel) {
    ...
    channelName = _require(4492).computeChannelName(channel, mergeGuildAvatar, upsertRelationship);
  ...
  if (channelId === arg0) {
    ...
    if (null != self.pendingMutePreference) {
      ...
      if (isSelfMuteResult !== pendingMutePreference) {
        ...
        importDefault(9095).setSelfMute(MediaEngineContextTypes.DEFAULT, pendingMutePreference, false);
        tmp7 = pendingMutePreference;
        const obj2 = importDefault(9095);
      }
    }
    const result = importDefault(16494).setIncomingCallActive(arg0, tmp7);
    self.lastMuteState = tmp7;
    self.lastScreenShareActive = false;
    const obj3 = importDefault(16494);
  } else {
...
prototype["startRingtone"] = function startRingtone() {
  ...
  if (isMetaQuestResult) {
    ...
    if (!disableSounds) {
      self.ringtone = require(9712) /* createSoundForPack */.createSoundForPack("call_ringing", soundpack.getSoundpack());
      const ringtone = self.ringtone;
      ringtone.loop();
      const tmpResult = require(9712) /* createSoundForPack */;
    }
  }
};
