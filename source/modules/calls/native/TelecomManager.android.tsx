...
// Dependencies: [5, 17, 10268, 4208, 1218, 4866, 1372, 4236, 10241, 4261, 3826, 4029, 1874, 676, 4250, 3, 10606, 5134, 16319, 5001, 8913, 4399, 8834, 10737, 10741, 1577, 4380, 10267, 2]

// Module 16318 (nativeEventEmitter)
...
prototype["reportIncomingCall"] = function reportIncomingCall(channelId, channelName, arg2) {
  ...
  if (null != channel) {
    ...
    channelName = _require(4380).computeChannelName(channel, mergeGuildAvatar, upsertRelationship);
    ...
    const obj4 = self(16319);
    const reportIncomingCallResult = self(16319).reportIncomingCall(channelId, channelName, tmp19);
    self(16319).reportIncomingCall(channelId, channelName, tmp19).then((arg0) => {
      if (!arg0) {
    ...
    const nextPromise = self(16319).reportIncomingCall(channelId, channelName, tmp19).then((arg0) => {
      if (!arg0) {
        outer1_25.warn("Failed to report incoming call: resolved false");
        self.clearCall(closure_0);
      }
  ...
  if (channelId === arg0) {
    ...
    if (null != self.pendingMutePreference) {
      ...
      if (isSelfMuteResult !== pendingMutePreference) {
        ...
        importDefault(8834).setSelfMute(MediaEngineContextTypes.DEFAULT, pendingMutePreference, false);
        tmp7 = pendingMutePreference;
        const obj2 = importDefault(8834);
      }
    }
    const result = importDefault(16319).setIncomingCallActive(arg0, tmp7);
    self.lastMuteState = tmp7;
    self.lastScreenShareActive = false;
    const obj3 = importDefault(16319);
  } else {
...
prototype["startRingtone"] = function startRingtone() {
  ...
  if (isMetaQuestResult) {
    ...
    if (!disableSounds) {
      self.ringtone = require(10267) /* createSoundForPack */.createSoundForPack("call_ringing", soundpack.getSoundpack());
      const ringtone = self.ringtone;
      ringtone.loop();
      const tmpResult = require(10267) /* createSoundForPack */;
    }
  }
};
