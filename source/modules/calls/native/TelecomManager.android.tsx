...
// Dependencies: [5, 17, 9785, 4322, 1218, 4995, 1391, 4351, 9758, 4375, 3957, 4149, 1922, 676, 4364, 3, 10607, 5261, 16592, 5129, 9248, 4513, 9161, 10826, 10830, 1624, 4494, 9784, 2]

// Module 16591 (nativeEventEmitter)
...
prototype["reportIncomingCall"] = function reportIncomingCall(channelId, channelName, arg2) {
  ...
  if (null != channel) {
    ...
    channelName = _require(4494).computeChannelName(channel, mergeGuildAvatar, markAllUserIdListsStale);
  ...
  if (channelId === arg0) {
    ...
    if (null != self.pendingMutePreference) {
      ...
      if (isSelfMuteResult !== pendingMutePreference) {
        ...
        importDefault(9161).setSelfMute(MediaEngineContextTypes.DEFAULT, pendingMutePreference, false);
        tmp7 = pendingMutePreference;
        const obj2 = importDefault(9161);
      }
    }
    const result = importDefault(16592).setIncomingCallActive(arg0, tmp7);
    self.lastMuteState = tmp7;
    self.lastScreenShareActive = false;
    const obj3 = importDefault(16592);
  } else {
...
prototype["startRingtone"] = function startRingtone() {
  ...
  if (isMetaQuestResult) {
    ...
    if (!disableSounds) {
      self.ringtone = require(9784) /* createSoundForPack */.createSoundForPack("call_ringing", soundpack.getSoundpack());
      const ringtone = self.ringtone;
      ringtone.loop();
      const tmpResult = require(9784) /* createSoundForPack */;
    }
  }
};
