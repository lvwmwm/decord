...
// Dependencies: [5, 17, 9697, 4304, 1218, 4976, 1372, 4332, 9670, 4356, 3921, 4124, 1903, 676, 4345, 3, 10519, 5244, 16467, 5111, 9161, 4494, 9078, 10742, 10746, 1605, 4475, 9696, 2]

// Module 16466 (nativeEventEmitter)
...
prototype["reportIncomingCall"] = function reportIncomingCall(channelId, channelName, arg2) {
  ...
  if (null != channel) {
    ...
    const obj4 = self(16467);
    const reportIncomingCallResult = self(16467).reportIncomingCall(channelId, channelName, tmp19);
    self(16467).reportIncomingCall(channelId, channelName, tmp19).then((arg0) => {
      if (!arg0) {
    ...
    const nextPromise = self(16467).reportIncomingCall(channelId, channelName, tmp19).then((arg0) => {
      if (!arg0) {
        outer1_25.warn("Failed to report incoming call: resolved false");
        self.clearCall(closure_0);
      }
  ...
  if (channelId === arg0) {
    ...
    if (null != self.pendingMutePreference) {
      ...
      if (isSelfMuteResult !== pendingMutePreference) {
        ...
        importDefault(9078).setSelfMute(MediaEngineContextTypes.DEFAULT, pendingMutePreference, false);
        tmp7 = pendingMutePreference;
        const obj2 = importDefault(9078);
      }
    }
    const result = importDefault(16467).setIncomingCallActive(arg0, tmp7);
    self.lastMuteState = tmp7;
    self.lastScreenShareActive = false;
    const obj3 = importDefault(16467);
  } else {
...
prototype["startRingtone"] = function startRingtone() {
  const self = this;
  let isMetaQuestResult = require(1605) /* isMetaQuest */.isMetaQuest();
  ...
  if (isMetaQuestResult) {
    ...
    if (!disableSounds) {
      self.ringtone = require(9696) /* createSoundForPack */.createSoundForPack("call_ringing", soundpack.getSoundpack());
      const ringtone = self.ringtone;
      ringtone.loop();
      const tmpResult = require(9696) /* createSoundForPack */;
    }
  }
};
