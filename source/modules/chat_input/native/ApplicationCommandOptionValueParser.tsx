...
// Function ID: 11304
// Name: getUsers
// Dependencies: [19, 5206, 1376, 1961, 1971, 1964, 3921, 1903, 4719, 12, 1351, 4475, 5141, 1935, 7063, 2]
// Exports: getRoles, parseOptionValuesForSend, useApplicationCommandOptionValueParser

// Module 11303 (getUsers)
...
function getChannels(getGuildId, arr) {
  ...
  if (null == guildId) {
    ...
    const tmp9Result = guildId(12)(_require(5141).COMMAND_SUPPORTED_CHANNEL_TYPE_KEYS);
    const combined = guildId(12)(_require(5141).COMMAND_SUPPORTED_CHANNEL_TYPE_KEYS).flatMap((arg0) => outer1_6.getChannels(guildId)[arg0].map((channel) => channel.channel)).concat(rebuild.computeAllActiveJoinedThreads(guildId));
...
export const parseOptionValuesForSend = function parseOptionValuesForSend(channel, activeCommand, current) {
  if (null == activeCommand.options) {
    ...
    const parserState = require(7063) /* rebuild */.createParserState(channel);
    ...
    while (iter !== undefined) {
      ...
      if (null != tmp5) {
        ...
        if ("text" === tmp6[0].type) {
          ...
          if (tmp4.type === require(1935) /* PermissionOverwriteType */.ApplicationCommandOptionType.STRING) {
            let tmp7 = nextResult;
