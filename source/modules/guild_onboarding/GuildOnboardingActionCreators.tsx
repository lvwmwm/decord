...
// Dependencies: [5, 32, 1934, 1372, 1942, 1874, 5115, 676, 3805, 4386, 530, 709, 1208, 5121, 12, 5122, 1351, 5125, 698, 4384, 5126, 11, 1384, 5129, 5136, 2]

// Module 5120 (_updateOnboardingResponses)
...
obj[2] = function updateRolesLocal(guildId, items2, differenceResult1) {
  ...
  if (initialize.isViewingRoles(guildId)) {
    const obj5 = require(5121) /* updateImpersonating */;
...
obj[3] = function completeOnboarding(guildId, prompts) {
  ...
  const providerConnectionState = items1(5122).getProviderConnectionState(connections);
  const tmp2Result1 = items1(5122);
  const applicationConnectionState = items1(5122).getApplicationConnectionState(connections);
  const tmp12 = importDefault;
  const tmp2Result2 = items1(5122);
  obj = {};
  const obj8 = importDefault(698);
  const merged = Object.assign(items1(4384).collectGuildAnalyticsMetadata(guildId));
  ...
  const tmp2Result3 = items1(4384);
  const tmp2Result4 = items1(5126);
  ...
  if (initialize.isFullServerPreview(guildId)) {
    const result = tmp2(5121).updateImpersonatedChannels(guildId, items, []);
    const tmp2Result5 = tmp2(5121);
    const result1 = tmp2(5121).updateImpersonatedData(guildId, { optInEnabled: true });
    const tmp2Result6 = tmp2(5121);
    const _Array = Array;
    const result2 = tmp2(5121).updateImpersonatedRoles(guildId, Array.from(selectedRoleIds));
