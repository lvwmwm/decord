...
// Dependencies: [31, 27, 5026, 1838, 4177, 3982, 5093, 5094, 653, 1853, 4361, 33, 4165, 689, 5087, 3875, 4101, 566, 9973, 9974, 1273, 1212, 4161, 5100, 5179, 5495, 5517, 1557, 9975, 5092, 5104, 5098, 5184, 1392, 1327, 3869, 4026, 4166, 9976, 4133, 5189, 1935, 4695, 5487, 5180, 2]
// Exports: default

// Module 9972 (EmptyCustomizeCommunity)
...
function EmptyCustomizeCommunity(setTab) {
  ...
  items1[2] = callback(setTab(4161).Text, obj2);
  obj.children = items1;
...
function PromptTitle(item) {
  ...
  items[1] = tmp4;
  obj.children = items;
  obj.children = callback2(require(4161) /* Text */.Heading, obj);
...
function ConnectionsPrompt(guildId) {
  ...
  if (0 !== stateFromStores.length) {
    ...
    items1[1] = callback(guildId(4161).Text, obj1);
    const obj2 = {
...
function DropdownOption(option) {
  ...
  if (tmp8Result) {
    ...
    obj1.name = str;
    obj.children = callback(importDefault(5184), obj1);
    tmp8Result = callback(closure_4, obj);
    const tmp10 = callback;
    const tmp13 = importDefault(5184);
    const tmp8 = callback;
    const tmp9 = closure_4;
  }
  const items1 = [tmp8Result, callback(option(4161).Text, { variant: "text-md/semibold", children: option.title })];
  obj.children = items1;
  return closure_17(closure_4, obj);
}
...
export default function CustomizeCommunity(guildId) {
  ...
  let tmp4 = stateFromStores(9975)(guildId);
  ...
  if (0 === onboardingPromptsRaw.length) {
    ...
    if (tmp8) {
      ...
      const items5 = [callback(guildId(4161).Text, obj5), newOnboardingPrompts.map(callback), onboardingPromptsWithNewAnswers.map(callback), ];
    ...
    if (tmp16) {
      ...
      const items7 = [callback(guildId(4161).Text, obj9), , ];
      ...
      items7[1] = callback(guildId(4161).Text, obj10);
