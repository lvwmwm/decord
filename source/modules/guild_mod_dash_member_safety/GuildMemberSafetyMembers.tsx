...
// Dependencies: [1874, 3879, 6853, 6854, 6856, 6857, 6863, 4415, 2]
// Exports: hasUnusualDmActivity

// Module 6852 (getGuildMemberSecondaryIndexes)
...
prototype["reset"] = function reset() {
  ...
  _membersMap.clear();
  const secondaryIndexMap = new require(3879) /* version */.SecondaryIndexMap(getGuildMemberSecondaryIndexes, getGuildMemberSecondarySortBy);
...
prototype["enhanceNewMember"] = function enhanceNewMember(trueMember, searchState, isIncludedInSearchResults) {
  ...
  const joinedAtTimestamp = require(6854) /* getJoinedAtDateFormatter */.getJoinedAtTimestamp(trueMember.joinedAt);
  ...
  obj.user = user;
  const obj2 = require(6854) /* getJoinedAtDateFormatter */;
  const tmp = require;
  let ORDER_BY_UNSPECIFIED = searchState.selectedSort;
  if (ORDER_BY_UNSPECIFIED == null) {
    ORDER_BY_UNSPECIFIED = tmp(6853).OrderBy.ORDER_BY_UNSPECIFIED;
  }
  obj.sort = require(6856) /* getSortValueForMember */.getSortValueForMember(trueMember, ORDER_BY_UNSPECIFIED);
...
prototype["_computeMemberSupplementals"] = function _computeMemberSupplementals(userId, unusualDMActivityUntil) {
  let obj = require(6857) /* hasMemberSupplemental */;
  ...
  if (tmp9) {
    ...
    tmp9 = time >= closure_4 - tmp(6853).UNUSUAL_DM_COMPARISON_DELTA;
  }
  obj[6] = tmp9;
  obj[7] = require(6863) /* isSpamSupported */.isSpammer(userId);
  return obj;
};
  ...
  if (null == arg1) {
    ...
    if (keys !== undefined) {
      ...
      while (keys[tmp] !== undefined) {
        ...
        if (importDefault(4415)(tmp15, obj[tmp6])) {
          continue;
        } else {
          obj[tmp6] = tmp15;
          flag = true;
...
export const hasUnusualDmActivity = function hasUnusualDmActivity(arg0) {
  ...
  if (tmp) {
    ...
    const time = date.getTime();
    tmp = time >= closure_4 - require(6853) /* result */.UNUSUAL_DM_COMPARISON_DELTA;
  }
  return tmp;
};
