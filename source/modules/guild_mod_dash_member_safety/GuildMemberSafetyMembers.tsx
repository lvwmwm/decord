...
function createMembersMap() {
  const secondaryIndexMap = new arg1(dependencyMap[3]).SecondaryIndexMap(getGuildMemberSecondaryIndexes, getGuildMemberSecondarySortBy);
...
function hasUnusualDmActivity(arg0) {
  ...
  if (tmp) {
    ...
    tmp = time >= closure_6 - arg1(dependencyMap[4]).UNUSUAL_DM_COMPARISON_DELTA;
  }
  return tmp;
}
...
const obj = { NEW_GUILD_MEMBER: "NEW_GUILD_MEMBER", CURRENT_GUILD_MEMBER: "CURRENT_GUILD_MEMBER", INCLUDED_IN_SEARCH_RESULTS: "INCLUDED_IN_SEARCH_RESULTS" };
const tmp2 = () => {
  class GuildMemberSafetyMembers {
    constructor(arg0) {
      tmp = closure_3(this, GuildMemberSafetyMembers);
      this.newMemberTimestamp = Date.now();
      this.guildId = arg0;
      this._membersMap = closure_10();
      return;
    }
  }
  const arg1 = GuildMemberSafetyMembers;
  let obj = {
    key: "reset",
    value() {
      const _membersMap = this._membersMap;
      _membersMap.clear();
      this._membersMap = callback3();
  ...
  obj = {
    ...
    value(joinedAt, selectedSort) {
      ...
      const joinedAtTimestamp = GuildMemberSafetyMembers(closure_2[5]).getJoinedAtTimestamp(joinedAt.joinedAt);
      ...
      const user = user.getUser(joinedAt.userId);
      ...
      obj["user"] = user;
      const obj2 = GuildMemberSafetyMembers(closure_2[5]);
      let ORDER_BY_UNSPECIFIED = selectedSort.selectedSort;
      if (null == ORDER_BY_UNSPECIFIED) {
        ORDER_BY_UNSPECIFIED = GuildMemberSafetyMembers(closure_2[4]).OrderBy.ORDER_BY_UNSPECIFIED;
      }
      obj["sort"] = GuildMemberSafetyMembers(closure_2[6]).getSortValueForMember(joinedAt, ORDER_BY_UNSPECIFIED);
  ...
  items[3] = {
    ...
    value(id) {
      ...
      let joinSourceType;
      let sourceInviteCode;
      let obj = GuildMemberSafetyMembers(closure_2[7]);
      ...
      obj.hasUnusualDmActivity = callback4(arg1);
      obj.hasUnusualAccountActivity = GuildMemberSafetyMembers(closure_2[8]).isSpammer(id);
      return obj;
    }
  };
  items[4] = {
      ...
      if (null == arg1) {
        ...
        if (keys !== undefined) {
          ...
          while (keys[tmp] !== undefined) {
            ...
            let tmp15 = callback;
            let tmp16 = closure_2;
            if (callback(closure_2[9])(tmp14, obj[tmp5])) {
              continue;
            } else {
              obj[tmp5] = tmp14;
              flag = true;
...
const result = arg1(dependencyMap[10]).fileFinishedImporting("modules/guild_mod_dash_member_safety/GuildMemberSafetyMembers.tsx");
