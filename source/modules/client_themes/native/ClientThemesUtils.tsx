...
// Dependencies: [32, 19, 1302, 4001, 1346, 712, 689, 4033, 4034, 4003, 4035, 4036, 4125, 4122, 2]
...
// Module 4000 (getGradientColorByPercentage)
...
function getGradientColorByPercentage(type, END) {
  let colors;
  let colors2;
  if (type.type === colors(4003).ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
    ...
    if (1 === colors.length) {
      return tmp(4033).hexToRgb(colors[0]);
    } else {
...
function getBottomColorWithOpacity(type, hexToRgbResult, arg2) {
  if (type.type === require(4003) /* ClientThemeType */.ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
  ...
  const tmp7 = new importDefault(4034)(tmpResult.r, tmpResult.g, tmpResult.b, arg2);
  const mixColorsResult = tmp2Result.mixColors(tmp7, new importDefault(4034)(hexToRgbResult.r, hexToRgbResult.g, hexToRgbResult.b, 1 - arg2));
  const tmp8 = new importDefault(4034)(hexToRgbResult.r, hexToRgbResult.g, hexToRgbResult.b, 1 - arg2);
...
function calculateGradientValueWithOpacity(customBackgroundGradient, END, theme) {
  ...
  const hexToRgb = require(4033) /* hexToRgba */.hexToRgb;
  if (isThemeDarkResult) {
    let hexToRgbResult = hexToRgb(tmp5.DARK);
  } else {
    hexToRgbResult = hexToRgb(tmp5.LIGHT);
  }
  if (customBackgroundGradient.type === require(4003) /* ClientThemeType */.ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
  ...
  if (customBackgroundGradient.type === require(4003) /* ClientThemeType */.ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
    if (arg3 != null) {
      let tmp11 = arg3;
    } else {
  ...
  if (tmp10 === obj.START) {
    ...
    const tmp35 = new importDefault(4034)(tmp30.r, tmp30.g, tmp30.b, tmp11);
    const tmp38 = new importDefault(4034)(hexToRgbResult.r, hexToRgbResult.g, hexToRgbResult.b, 1 - tmp11);
    const mixColorsResult = tmp(4033).mixColors(tmp35, tmp38);
...
export const getGradientThemeMetadata = function getGradientThemeMetadata(gradientThemeFromFlags, gradient) {
  if (null != gradientThemeFromFlags) {
    if (null != gradient) {
      ...
      if (gradient.type === require(4003) /* ClientThemeType */.ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
      ...
      if (gradient.type === require(4003) /* ClientThemeType */.ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
...
export const getEmbedScrollGradientBackground = function getEmbedScrollGradientBackground() {
  let customBackgroundGradient = require(4122) /* getCustomThemesName */.getCustomBackgroundGradient();
...
export const getEmbedBackground = function getEmbedBackground() {
  ...
  return require(4033) /* hexToRgba */.hexWithOpacity(tmp4, obj.LEVEL_1);
};
