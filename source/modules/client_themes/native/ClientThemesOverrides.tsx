...
export const useGradientBottom = function useGradientBottom() {
  gradientValue = gradientValue(3942).useGradientValue(gradientValue(3942).GradientPercentage.END);
...
export const useGradientTop = function useGradientTop() {
  gradientValue = gradientValue(3942).useGradientValue(gradientValue(3942).GradientPercentage.START);
  const items = [gradientValue];
  return React.useMemo(() => {
    let tmp;
    if (null != gradientValue) {
...
export const useGradientMidpoint = function useGradientMidpoint() {
  gradientValue = gradientValue(3942).useGradientValue(gradientValue(3942).GradientPercentage.MID);
  const items = [gradientValue];
  return React.useMemo(() => {
    let tmp;
    if (null != gradientValue) {
