...
// Dependencies: [32, 19, 17, 676, 1388, 11631, 21, 4668, 712, 698, 10040, 4739, 1236, 4750, 11632, 5438, 6399, 14853, 7404, 1377, 2]
// Exports: default

// Module 15735 (NotificationCenterBannerImpl)
...
function NotificationCenterBannerImpl(onDismiss) {
  ...
  obj[1] = callback4(onDismiss(10040).BellSlashIcon, obj);
  ...
  const items1 = [callback4(onDismiss(4739).Text, obj2), , ];
  ...
  items1[1] = callback4(onDismiss(4739).Text, obj3);
  ...
  obj5[1] = callback4(onDismiss(4750).Button, obj6);
  ...
  obj7[4] = callback4(onDismiss(6399).XSmallIcon, { size: "sm", color: "icon-strong" });
  items[2] = callback4(onDismiss(5438).PressableHighlight, obj7);
  obj[1] = items;
...
export default function NotificationCenterPermissionNudge() {
  ...
  const canSeePushNotificationNudge = _require(11632).useCanSeePushNotificationNudge();
  const obj2 = _require(11632);
  let prop = null;
  ...
  const tmp5 = callback(_require(7404).useSelectedTimeRecurringDismissibleContent(prop, { cooldownDurationMs: 604800000 }), 2);
  _require = tmp6;
  const items = [tmp5[1]];
  const obj3 = _require(7404);
  let tmp8 = null;
