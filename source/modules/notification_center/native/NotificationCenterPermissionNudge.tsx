...
// Dependencies: [32, 19, 17, 676, 1388, 11974, 21, 4380, 712, 698, 9887, 4376, 1236, 4815, 11975, 4949, 5432, 14883, 6131, 1377, 2]
// Exports: default

// Module 15767 (NotificationCenterBannerImpl)
...
function NotificationCenterBannerImpl(onDismiss) {
  ...
  obj[1] = callback4(onDismiss(9887).BellSlashIcon, obj);
  ...
  const items1 = [callback4(onDismiss(4376).Text, obj2), , ];
  ...
  items1[1] = callback4(onDismiss(4376).Text, obj3);
  ...
  obj5[1] = callback4(onDismiss(4815).Button, obj6);
  ...
  obj7[4] = callback4(onDismiss(5432).XSmallIcon, { size: "sm", color: "icon-strong" });
  items[2] = callback4(onDismiss(4949).PressableHighlight, obj7);
  obj[1] = items;
...
export default function NotificationCenterPermissionNudge() {
  ...
  const canSeePushNotificationNudge = _require(11975).useCanSeePushNotificationNudge();
  const obj2 = _require(11975);
  let prop = null;
  ...
  const tmp5 = callback(_require(6131).useSelectedTimeRecurringDismissibleContent(prop, { cooldownDurationMs: 604800000 }), 2);
  _require = tmp6;
  const items = [tmp5[1]];
  const obj3 = _require(6131);
  let tmp8 = null;
