...
// Dependencies: [19, 7309, 676, 21, 589, 15844, 8599, 4411, 8593, 8597, 5920, 8603, 11643, 11640, 6191, 5936, 5568, 1236, 5938, 4986, 12059, 9352, 15845, 11344, 8604, 4407, 11652, 6186, 2]
// Exports: default

// Module 15843 (NotificationCenterActionSheet)
import closure_3 from "noop" /* 19 */;
import closure_4 from "findOrCreateMessageRecord" /* 7309 */;
...
export default function NotificationCenterActionSheet() {
  ...
  closure_4 = hasForLaterAccess.useCallback((BOOKMARK) => {
    everyoneFilter(4411).hideActionSheet();
    if (hasForLaterAccess) {
      roleFilter(8603).showForLaterModal(BOOKMARK);
      const obj2 = roleFilter(8603);
    } else {
      const items = [tmp(5920).FOR_LATER_ROADBLOCK];
      tmp(8593)(roleFilter(8597).EntitlementFeatureNames.SAVED_MESSAGES, undefined, items);
      const tmpResult = tmp(8593);
    }
  }, items2);
  let obj3 = roleFilter(11643);
  const canUseScheduledMessages = obj3.useCanUseScheduledMessages();
  const callback = hasForLaterAccess.useCallback(() => {
    everyoneFilter(4411).hideActionSheet();
    const obj = everyoneFilter(4411);
    const result = roleFilter(11640).showScheduledMessagesModal();
  }, []);
  const callback1 = hasForLaterAccess.useCallback(() => {
    let obj = everyoneFilter(4411);
    obj.hideActionSheet();
    obj = { screen: constants.NOTIFICATIONS };
    roleFilter(6191).openUserSettings(obj);
  }, []);
  obj = { showGradient: true, header: null, children: null };
  obj = { title: null };
  const intl = roleFilter(1236).intl;
  obj[0] = intl.string(roleFilter(1236).t.HcoRu0);
  obj[1] = callback(roleFilter(5568).BottomSheetTitleHeader, obj);
  ...
  obj3 = { IconComponent: roleFilter(4986).AtIcon, source: everyoneFilter(12059) };
  obj2[3] = callback(roleFilter(5938).ActionSheetRow.Icon, obj3);
  const items3 = [callback(roleFilter(5938).ActionSheetSwitchRow, obj2), ];
  const obj4 = {
    onValueChange() {
  ...
  obj4[4] = callback(roleFilter(5938).ActionSheetRow.Icon, { IconComponent: roleFilter(9352).BellIcon, source: everyoneFilter(15845) });
  items3[1] = callback(roleFilter(5938).ActionSheetSwitchRow, obj4);
  obj1[1] = items3;
  const items4 = [callback2(roleFilter(5938).ActionSheetRow.Group, obj1), ];
  let tmp10Result = null;
  if (isForLaterExperimentOn) {
    const obj6 = { icon: null, label: null, onPress: null, arrow: true };
    const obj7 = { IconComponent: null };
    obj7[0] = tmp(11344).BookmarkIcon;
    obj6[0] = tmp10(tmp(5938).ActionSheetRow.Icon, obj7);
    const intl5 = tmp(1236).intl;
    obj6[1] = intl5.string(tmp(1236).t["2pAkDA"]);
    obj6[2] = function onPress() {
      return lib(roleFilter(8604).SavedMessageSortTypes.BOOKMARK);
    };
    tmp10Result = tmp10(tmp(5938).ActionSheetRow, obj6, "bookmarks");
  }
  ...
  if (isForLaterExperimentOn) {
    ...
    obj9[0] = tmp(4407).ClockIcon;
    obj8[0] = tmp10(tmp(5938).ActionSheetRow.Icon, obj9);
    const intl6 = tmp(1236).intl;
    obj8[1] = intl6.string(tmp(1236).t.aUXxzT);
    obj8[2] = function onPress() {
      return lib(roleFilter(8604).SavedMessageSortTypes.REMINDER);
    };
    tmp10Result = tmp10(tmp(5938).ActionSheetRow, obj8, "reminders");
  }
  ...
  if (canUseScheduledMessages) {
    ...
    obj11[0] = tmp(11652).HourglassIcon;
    obj10[0] = tmp10(tmp(5938).ActionSheetRow.Icon, obj11);
    const intl7 = tmp(1236).intl;
    obj10[1] = intl7.string(tmp(1236).t.SZVs3K);
    obj10[2] = callback;
    tmp10Result1 = tmp10(tmp(5938).ActionSheetRow, obj10, "scheduled-messages");
  }
  const obj12 = { hasIcons: true, children: null };
  items5[2] = tmp10Result1;
  const obj13 = { icon: null, label: null, onPress: null, arrow: true };
  const obj5 = { IconComponent: roleFilter(9352).BellIcon, source: everyoneFilter(15845) };
  obj13[0] = callback(roleFilter(5938).ActionSheetRow.Icon, { IconComponent: roleFilter(6186).SettingsIcon });
  const intl8 = tmp(1236).intl;
  obj13[1] = intl8.string(roleFilter(1236).t.h850Ss);
  obj13[2] = callback1;
  items5[3] = callback(roleFilter(5938).ActionSheetRow, obj13, "settings");
  obj12[1] = items5;
  items4[1] = callback2(roleFilter(5938).ActionSheetRow.Group, obj12);
  obj[2] = items4;
  return callback2(roleFilter(5936).ActionSheet, obj);
};
