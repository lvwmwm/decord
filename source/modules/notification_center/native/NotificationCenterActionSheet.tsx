...
export default function NotificationCenterActionSheet() {
  ...
  findOrCreateMessageRecord = hasForLaterAccess.useCallback((BOOKMARK) => {
    ...
    if (hasForLaterAccess) {
      roleFilter(8190).showForLaterModal(BOOKMARK);
      const obj2 = roleFilter(8190);
    } else {
      everyoneFilter(8181)(roleFilter(8184).EntitlementFeatureNames.SAVED_MESSAGES);
      const tmpResult = everyoneFilter(8181);
    }
  }, items2);
  ...
  obj3[1] = everyoneFilter(10130);
  obj2[3] = callback(roleFilter(5648).ActionSheetRow.Icon, obj3);
  const items3 = [callback(roleFilter(5648).ActionSheetSwitchRow, obj2), ];
  const obj4 = {
    onValueChange() {
  ...
  obj1[1] = items3;
  const items4 = [callback2(roleFilter(5648).ActionSheetRow.Group, obj1), ];
  let tmp8Result = null;
  if (isForLaterExperimentOn) {
    const obj6 = { icon: null, label: null, onPress: null, arrow: true };
    const obj7 = { IconComponent: null };
    obj7[0] = tmp(10191).BookmarkIcon;
    obj6[0] = tmp8(tmp(5648).ActionSheetRow.Icon, obj7);
    const intl5 = tmp(1236).intl;
    obj6[1] = intl5.string(tmp(1236).t["2pAkDA"]);
    obj6[2] = function onPress() {
      return lib(roleFilter(8192).SavedMessageSortTypes.BOOKMARK);
    };
    tmp8Result = tmp8(tmp(5648).ActionSheetRow, obj6, "bookmarks");
  }
  ...
  if (isForLaterExperimentOn) {
    ...
    obj8[0] = tmp8(tmp(5648).ActionSheetRow.Icon, obj9);
    const intl6 = tmp(1236).intl;
    obj8[1] = intl6.string(tmp(1236).t.aUXxzT);
    obj8[2] = function onPress() {
      return lib(roleFilter(8192).SavedMessageSortTypes.REMINDER);
    };
    tmp8Result = tmp8(tmp(5648).ActionSheetRow, obj8, "reminders");
  }
  ...
  obj12[0] = roleFilter(5920).SettingsIcon;
  obj11[0] = callback(roleFilter(5648).ActionSheetRow.Icon, obj12);
  const intl7 = tmp(1236).intl;
  obj11[1] = intl7.string(roleFilter(1236).t.h850Ss);
  obj11[2] = callback;
  items5[2] = callback(roleFilter(5648).ActionSheetRow, obj11, "settings");
  obj10[1] = items5;
  items4[1] = callback2(roleFilter(5648).ActionSheetRow.Group, obj10);
  obj[2] = items4;
  return callback2(roleFilter(5646).ActionSheet, obj);
};
