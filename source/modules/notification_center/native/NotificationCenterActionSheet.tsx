...
// Dependencies: [19, 6992, 676, 21, 589, 15378, 8158, 4223, 8153, 8156, 5615, 8162, 5910, 5631, 5322, 1236, 5633, 4769, 10101, 9030, 15379, 10162, 8164, 4219, 5905, 2]
// Exports: default

// Module 15377 (NotificationCenterActionSheet)
...
export default function NotificationCenterActionSheet() {
  ...
  findOrCreateMessageRecord = hasForLaterAccess.useCallback((BOOKMARK) => {
    ...
    if (hasForLaterAccess) {
      roleFilter(8162).showForLaterModal(BOOKMARK);
      const obj2 = roleFilter(8162);
    } else {
      const items = [tmp(5615).FOR_LATER_ROADBLOCK];
      tmp(8153)(roleFilter(8156).EntitlementFeatureNames.SAVED_MESSAGES, undefined, items);
  ...
  obj3[0] = roleFilter(4769).AtIcon;
  obj3[1] = everyoneFilter(10101);
  obj2[3] = callback(roleFilter(5633).ActionSheetRow.Icon, obj3);
  const items3 = [callback(roleFilter(5633).ActionSheetSwitchRow, obj2), ];
  const obj4 = {
    onValueChange() {
  ...
  obj5[0] = roleFilter(9030).BellIcon;
  obj5[1] = everyoneFilter(15379);
  obj4[4] = callback(roleFilter(5633).ActionSheetRow.Icon, obj5);
  items3[1] = callback(roleFilter(5633).ActionSheetSwitchRow, obj4);
  obj1[1] = items3;
  const items4 = [callback2(roleFilter(5633).ActionSheetRow.Group, obj1), ];
  let tmp8Result = null;
  if (isForLaterExperimentOn) {
    const obj6 = { icon: null, label: null, onPress: null, arrow: true };
    const obj7 = { IconComponent: null };
    obj7[0] = tmp(10162).BookmarkIcon;
    obj6[0] = tmp8(tmp(5633).ActionSheetRow.Icon, obj7);
    const intl5 = tmp(1236).intl;
    obj6[1] = intl5.string(tmp(1236).t["2pAkDA"]);
    obj6[2] = function onPress() {
      return lib(roleFilter(8164).SavedMessageSortTypes.BOOKMARK);
    };
    tmp8Result = tmp8(tmp(5633).ActionSheetRow, obj6, "bookmarks");
  }
  ...
  if (isForLaterExperimentOn) {
    ...
    obj8[0] = tmp8(tmp(5633).ActionSheetRow.Icon, obj9);
    const intl6 = tmp(1236).intl;
    obj8[1] = intl6.string(tmp(1236).t.aUXxzT);
    obj8[2] = function onPress() {
      return lib(roleFilter(8164).SavedMessageSortTypes.REMINDER);
    };
    tmp8Result = tmp8(tmp(5633).ActionSheetRow, obj8, "reminders");
  }
  ...
  obj12[0] = roleFilter(5905).SettingsIcon;
  obj11[0] = callback(roleFilter(5633).ActionSheetRow.Icon, obj12);
  const intl7 = tmp(1236).intl;
  obj11[1] = intl7.string(roleFilter(1236).t.h850Ss);
  obj11[2] = callback;
  items5[2] = callback(roleFilter(5633).ActionSheetRow, obj11, "settings");
  obj10[1] = items5;
  items4[1] = callback2(roleFilter(5633).ActionSheetRow.Group, obj10);
  obj[2] = items4;
  return callback2(roleFilter(5631).ActionSheet, obj);
};
