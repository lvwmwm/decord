...
// Dependencies: [6882, 1862, 3892, 1874, 676, 1236, 4094, 7009, 1856, 38, 2]
// Exports: default

// Module 15383 (getNotificationCenterItemBody)
...
export default function getNotificationCenterItemBody(arg0) {
  ...
  let obj = renderApplication(4094);
  let obj1 = user;
  ...
  if (applicationId(7009).NotificationCenterLocalItems.FRIEND_REQUESTS_GROUPED === type) {
    let tmp2Result = tmp2(4094);
  ...
  } else if (tmp7(7009).NotificationCenterLocalItems.MOBILE_NATIVE_UPDATE_AVAILABLE === type) {
    let str12;
  ...
  } else if (tmp7(7009).NotificationCenterItems.FRIEND_SUGGESTION_CREATED === type) {
    ...
    return str10;
  } else if (tmp7(7009).NotificationCenterItems.GUILD_SCHEDULED_EVENT_STARTED === type) {
    ...
    let tmp7Result = tmp7(1856);
    if (!tmp7Result.isNullOrEmpty(name)) {
      tmp7Result = tmp7(1856);
  ...
  } else if (tmp7(7009).NotificationCenterLocalItems.INCOMING_FRIEND_REQUESTS === type) {
  ...
  } else if (tmp7(7009).NotificationCenterLocalItems.INCOMING_FRIEND_REQUESTS_ACCEPTED === type) {
    if (null != applicationId) {
  ...
  } else if (tmp7(7009).NotificationCenterItems.FRIEND_REQUEST_ACCEPTED === type) {
    if (null != applicationId) {
  ...
  } else if (tmp7(7009).NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS_ACCEPTED === type) {
  ...
  } else if (tmp7(7009).NotificationCenterLocalItems.INCOMING_GAME_FRIEND_REQUESTS === type) {
  ...
  } else if (tmp7(7009).NotificationCenterItems.GAME_FRIEND_REQUEST_ACCEPTED === type) {
    if (null == applicationId) {
      let body = item.body;
    } else {
