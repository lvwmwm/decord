...
// Function ID: 8619
// Name: set
// Dependencies: [19, 8619, 8620, 8622, 8621, 2]
  ...
  return websites.useMemo(() => {
    ...
    if (null != websites) {
      if (null != id1) {
        ...
        if (!someResult) {
          someResult = found.some((category) => category.category === callback(8619).ThirdPartyGameApplicationWebsiteCategory.STEAM);
        }
        if (!someResult) {
        ...
        const sorted = found.sort((category, category2) => {
          let num = -1;
          if (category.category !== callback(8619).ThirdPartyGameApplicationWebsiteCategory.STEAM) {
            let num2 = 0;
            if (category2.category === callback(8619).ThirdPartyGameApplicationWebsiteCategory.STEAM) {
              num2 = 1;
            }
            num = num2;
          }
