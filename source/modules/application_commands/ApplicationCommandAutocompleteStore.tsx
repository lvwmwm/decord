...
set = {
  ...
  APPLICATION_COMMAND_AUTOCOMPLETE_RESPONSE: function handleApplicationCommandAutocompleteResponse(arg0) {
    ...
    if (null == value) {
      ...
      obj[0] = num;
      obj[2] = mapped.length;
      _require(4498).trackWithMetadata(AnalyticEvents.APPLICATION_COMMAND_OPTION_STRING_AUTOCOMPLETE_PERFORMANCE, obj);
      map2.delete(nonce);
  ...
  INTERACTION_FAILURE: function handleInteractionFailure(nonce) {
    ...
    if (null == nonce) {
      ...
      if (null == value) {
        ...
        obj[0] = num;
        require(4498) /* collectGuildAnalyticsMetadata */.trackWithMetadata(AnalyticEvents.APPLICATION_COMMAND_OPTION_STRING_AUTOCOMPLETE_PERFORMANCE, obj);
        map2.delete(nonce);
        const channelId = value.channelId;
        if (!map1.has(channelId)) {
          const activeCommand = store.getActiveCommand(channelId);
