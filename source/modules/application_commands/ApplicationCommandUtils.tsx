...
// Function ID: 7214
// Name: buildCommand
// Dependencies: [1395, 4820, 4917, 676, 505, 7214, 1956, 7215, 506, 12, 38, 14, 4666, 2]
...
function buildCommand(arg0) {
  ...
  if (!tmp) {
    ...
    if (CHAT == null) {
      CHAT = obj(1956).ApplicationCommandType.CHAT;
    }
    obj[6] = CHAT;
    obj[7] = obj(7215).ApplicationCommandInputType.BOT;
    ...
    const keyPermissionsResult = obj(7214).keyPermissions(rootCommand.permissions);
...
function buildSubCommands(arg0) {
  ...
  if (command.hasOwnProperty("id")) {
    ...
    if (command.type !== PermissionOverwriteType.ApplicationCommandOptionType.SUB_COMMAND) {
      if (command.type !== tmp(1956).ApplicationCommandOptionType.SUB_COMMAND_GROUP) {
  ...
  if (null == command.options) {
    ...
    const found = options.filter((type) => type.type === callback(1956).ApplicationCommandOptionType.SUB_COMMAND_GROUP);
    ...
    const found1 = options1.filter((type) => type.type === callback(1956).ApplicationCommandOptionType.SUB_COMMAND);
...
export const getApplicationCommandOptionQueryOptions = function getApplicationCommandOptionQueryOptions(option) {
  ...
  if (!tmp3) {
    tmp3 = option.type === tmp(1956).ApplicationCommandOptionType.MENTIONABLE;
  }
  let tmp4 = option.type === tmp(1956).ApplicationCommandOptionType.ROLE;
  if (!tmp4) {
    tmp4 = option.type === tmp(1956).ApplicationCommandOptionType.MENTIONABLE;
  }
...
export const extractInteractionDataProps = function extractInteractionDataProps(parsed) {
  ...
  if (interactionOptions != null) {
    found = interactionOptions.find((type) => type.type === callback(1956).ApplicationCommandOptionType.SUB_COMMAND_GROUP);
  }
  let sum = id;
  if (null != found) {
  ...
  let found1;
  if (interactionOptions != null) {
    found1 = interactionOptions.find((type) => type.type === callback(1956).ApplicationCommandOptionType.SUB_COMMAND);
  }
  let commandKey = sum;
  if (null != found1) {
