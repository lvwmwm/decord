...
// Function ID: 8093
// Name: getIndexKey
// Dependencies: [32, 5, 19, 8093, 1997, 1218, 1391, 1993, 1910, 1923, 8094, 4917, 676, 3, 8095, 8096, 7213, 7214, 5422, 38, 589, 8097, 698, 1956, 8100, 709, 8101, 8102, 1374, 8376, 7215, 5419, 2]
...
import computeAllowedForUser from "computeAllowedForUser" /* 8376 */;
import computeAllowedForUserAll from "computeAllowedForUser" /* 8376 */;
import closure_4 from "_slicedToArray" /* 32 */;
import closure_5 from "asyncGeneratorStep" /* 5 */;
import closure_6 from "noop" /* 19 */;
import closure_7 from "handleUserSettingsProtoStoreChange" /* 8093 */;
...
import closure_14 from "handleUserSettingsProtoStoreChange" /* 8094 */;
...
function handleFetchSuccess(arg0, flag) {
  ...
  if (null == id) {
    ...
    const applicationCommands = id(7213).buildApplicationCommands(application_commands.map((description_default) => {
      const obj = {};
...
function useQueryState(type, commandTypes, allowFetch) {
  ...
  const permissionContext = applicationId(8097).usePermissionContext(channel, commandTypes.commandTypes);
          ...
          if (tmp13) {
            ...
            if (tmp34) {
              if (null == guild_id.channel.guild_id) {
                ...
                const applicationCommandIndex = applicationId(8096).requestApplicationCommandIndex(obj);
                const obj4 = applicationId(8096);
              }
            }
            obj1 = { type: "guild", guildId: null };
            guild_id = guild_id.channel.guild_id;
            obj1[1] = guild_id;
            const applicationCommandIndex1 = applicationId(8096).requestApplicationCommandIndex(obj1);
            const obj6 = applicationId(8096);
          }
        }
  ...
  const effect1 = stateFromStoresObject1.useEffect(() => {
    if (first2) {
      ...
      if (tmp) {
        const applicationCommandIndex = applicationId(8096).requestApplicationCommandIndex({ type: "user" });
        const obj = applicationId(8096);
      }
      dependencyMap(false);
    }
  }, items3);
...
function queryIndex(allowApplicationCommands) {
  ...
  if (builtIns === undefined) {
    let arr = NONE;
    builtIns = NONE(8100).BuiltInCommandFilter.ALLOW;
  }
  ...
  if (NONE === undefined) {
    NONE = NONE(8100).ScoreMethod.NONE;
  }
  ...
  if (builtIns !== NONE(8100).BuiltInCommandFilter.DENY) {
    const tmp12 = builtIns === NONE(8100).BuiltInCommandFilter.ONLY_TEXT;
    let builtInCommands = NONE(8102).getBuiltInCommands(permissionContext.commandTypes, true, tmp12);
    const tmp9Result = NONE(8102);
  } else {
    builtInCommands = [];
  }
  const items = [];
  ...
  if (builtInCommands.length > 0) {
    const tmp87 = queryIndexSection(NONE(8102).BUILT_IN_SECTIONS[constants.BUILT_IN], builtInCommands, true, true, obj);
    if (null != tmp87) {
      items.push(tmp87);
    }
  }
  ...
  if (NONE === NONE(8100).ScoreMethod.COMMAND_ONLY) {
...
function queryIndexSection(descriptor, builtInCommands, arg2, arg3, arg4) {
  ...
  if (0 !== arr2.length) {
    ...
    obj[1] = arr2;
    let tmp36 = obj;
    tmp39 = scoreMethod !== ScoreMethod.ScoreMethod.NONE && scoreMethod !== tmp27(8100).ScoreMethod.APPLICATION_ONLY;
  } else {
    tmp36 = null;
  }
  return tmp36;
      ...
      if (!tmp14) {
        ...
        if (tmp29) {
          if (null == guild_id.guild_id) {
            ...
            obj1[1] = guild_id.id;
            const applicationCommandIndex2 = tmp8(8096).requestApplicationCommandIndex(obj1);
            flag6 = true;
        ...
        guild_id = guild_id.guild_id;
        obj2[1] = guild_id;
        const applicationCommandIndex3 = tmp8(8096).requestApplicationCommandIndex(obj2);
        flag6 = true;
        const tmp8Result2 = tmp8(8096);
      }
    }
    ...
    if (tmp) {
      if (callback2) {
        ...
        if (tmp15) {
          ...
          obj[1] = callback;
          const applicationCommandIndex = callback(8096).requestApplicationCommandIndex(obj);
          const obj3 = callback(8096);
        }
      }
      dependencyMap(false);
    }
