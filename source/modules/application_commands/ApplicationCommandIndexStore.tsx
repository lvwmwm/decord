...
// Function ID: 9220
// Name: getIndexKey
// Dependencies: [32, 5, 19, 9220, 1995, 1215, 1386, 1991, 1908, 1921, 9221, 4960, 673, 3, 9222, 9223, 7454, 7455, 5475, 38, 586, 9224, 695, 1954, 9227, 706, 9228, 9229, 1369, 9334, 7456, 5472, 2]
...
import computePermissions from "computePermissions" /* 9224 */;
import ScoreMethod from "ScoreMethod" /* 9227 */;
import getOptionValue from "getOptionValue" /* 9229 */;
import computeAllowedForUser from "computeAllowedForUser" /* 9334 */;
import computeAllowedForUserAll from "computeAllowedForUser" /* 9334 */;
import closure_4 from "_slicedToArray" /* 32 */;
import closure_5 from "asyncGeneratorStep" /* 5 */;
import closure_6 from "noop" /* 19 */;
import closure_7 from "handleUserSettingsProtoStoreChange" /* 9220 */;
...
import closure_14 from "handleUserSettingsProtoStoreChange" /* 9221 */;
...
function handleFetchSuccess(arg0, flag) {
  ...
  if (null == id) {
    ...
    const applicationCommands = id(7454).buildApplicationCommands(application_commands.map((description_default) => {
      const obj = {};
...
function useQueryState(type, commandTypes, allowFetch) {
  ...
  const permissionContext = applicationId(9224).usePermissionContext(channel, commandTypes.commandTypes);
          ...
          if (tmp13) {
            ...
            if (tmp34) {
              if (null == guild_id.channel.guild_id) {
                ...
                const applicationCommandIndex = applicationId(9223).requestApplicationCommandIndex(obj);
                const obj4 = applicationId(9223);
              }
            }
            obj1 = { type: "guild", guildId: null };
            guild_id = guild_id.channel.guild_id;
            obj1[1] = guild_id;
            const applicationCommandIndex1 = applicationId(9223).requestApplicationCommandIndex(obj1);
            const obj6 = applicationId(9223);
          }
        }
  ...
  const effect1 = stateFromStoresObject1.useEffect(() => {
    if (first2) {
      ...
      if (tmp) {
        const applicationCommandIndex = applicationId(9223).requestApplicationCommandIndex({ type: "user" });
        const obj = applicationId(9223);
      }
      dependencyMap(false);
    }
  }, items3);
  ...
  stateFromStores2 = applicationId(586).useStateFromStores(items6, () => closure_1_37.getApplicationState(applicationId));
...
function queryIndex(allowApplicationCommands) {
  ...
  if (builtIns === undefined) {
    let arr = NONE;
    builtIns = NONE(9227).BuiltInCommandFilter.ALLOW;
  }
  ...
  if (NONE === undefined) {
    NONE = NONE(9227).ScoreMethod.NONE;
  }
  ...
  if (builtIns !== NONE(9227).BuiltInCommandFilter.DENY) {
    const tmp12 = builtIns === NONE(9227).BuiltInCommandFilter.ONLY_TEXT;
    let builtInCommands = NONE(9229).getBuiltInCommands(permissionContext.commandTypes, true, tmp12);
    const tmp9Result = NONE(9229);
  } else {
    builtInCommands = [];
  }
  const items = [];
  ...
  if (builtInCommands.length > 0) {
    const tmp87 = queryIndexSection(NONE(9229).BUILT_IN_SECTIONS[constants.BUILT_IN], builtInCommands, true, true, obj);
    if (null != tmp87) {
      items.push(tmp87);
    }
  }
  ...
  if (NONE === NONE(9227).ScoreMethod.COMMAND_ONLY) {
...
function queryIndexSection(descriptor, builtInCommands, arg2, arg3, arg4) {
  ...
  if (0 !== arr2.length) {
    ...
    obj[1] = arr2;
    let tmp36 = obj;
    tmp39 = scoreMethod !== ScoreMethod.ScoreMethod.NONE && scoreMethod !== tmp27(9227).ScoreMethod.APPLICATION_ONLY;
  } else {
    tmp36 = null;
  }
  return tmp36;
      ...
      if (!tmp14) {
        ...
        if (tmp29) {
          if (null == guild_id.guild_id) {
            ...
            obj1[1] = guild_id.id;
            const applicationCommandIndex2 = tmp8(9223).requestApplicationCommandIndex(obj1);
            flag6 = true;
            const tmp8Result1 = tmp8(9223);
          }
        }
        obj2 = { type: "guild", guildId: null };
        guild_id = guild_id.guild_id;
        obj2[1] = guild_id;
        const applicationCommandIndex3 = tmp8(9223).requestApplicationCommandIndex(obj2);
        flag6 = true;
        const tmp8Result2 = tmp8(9223);
      }
    }
...
export const useGuildIndexState = function useGuildIndexState(arg0, arg1) {
  ...
  dependencyMap = tmp[1];
  const items = [applicationCommandIndexStore];
  stateFromStoresObject = _require(586).useStateFromStoresObject(items, () => {
    ...
    if (tmp) {
      if (callback2) {
        ...
        if (tmp15) {
          ...
          obj[1] = callback;
          const applicationCommandIndex = callback(9223).requestApplicationCommandIndex(obj);
          const obj3 = callback(9223);
        }
      }
      dependencyMap(false);
    }
...
export const useUserIndexState = function useUserIndexState(arg0, arg1) {
  ...
  dependencyMap = tmp[1];
  const items = [applicationCommandIndexStore];
  stateFromStoresObject = _require(586).useStateFromStoresObject(items, () => authStore.getUserState());
  const items1 = [stateFromStoresObject, arg1, arg0, first];
  const effect = React.useEffect(() => {
    if (first2) {
      let tmp = allowFetch3;
