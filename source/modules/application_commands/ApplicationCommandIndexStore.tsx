...
// Function ID: 8123
// Name: getIndexKey
// Dependencies: [32, 5, 19, 8123, 1946, 1218, 1372, 1942, 1862, 1874, 8124, 4688, 676, 3, 8125, 8126, 6874, 6875, 5185, 38, 589, 8127, 698, 1906, 8130, 709, 8131, 8132, 1355, 9427, 6876, 2]
...
function handleFetchSuccess(arg0, flag) {
  ...
  if (null == id) {
    ...
    if (tmp32) {
      ...
      const membersById = importDefault(5185).requestMembersById(target.guildId, items);
      const obj5 = importDefault(5185);
    }
    const application_commands = index.application_commands;
    const applicationCommands = id(6874).buildApplicationCommands(application_commands.map((description_default) => {
      let name_default;
      const obj = {};
...
function useQueryState(type, commandTypes, allowFetch) {
  ...
  const permissionContext = applicationId(8127).usePermissionContext(channel, commandTypes.commandTypes);
          ...
          if (tmp13) {
            ...
            if (tmp34) {
              if (null == guild_id.channel.guild_id) {
                ...
                const applicationCommandIndex = applicationId(8126).requestApplicationCommandIndex(obj);
                const obj4 = applicationId(8126);
              }
            }
            obj1 = { type: "guild", guildId: null };
            guild_id = guild_id.channel.guild_id;
            obj1[1] = guild_id;
            const applicationCommandIndex1 = applicationId(8126).requestApplicationCommandIndex(obj1);
            const obj6 = applicationId(8126);
          }
        }
  ...
  const effect1 = stateFromStoresObject1.useEffect(() => {
    if (first2) {
      ...
      if (tmp) {
        const applicationCommandIndex = applicationId(8126).requestApplicationCommandIndex({ type: "user" });
        const obj = applicationId(8126);
      }
      dependencyMap(false);
    }
  }, items3);
...
function queryIndex(allowApplicationCommands) {
  ...
  if (builtIns === undefined) {
    let arr = NONE;
    builtIns = NONE(8130).BuiltInCommandFilter.ALLOW;
  }
  ...
  if (NONE === undefined) {
    NONE = NONE(8130).ScoreMethod.NONE;
  }
  ...
  if (builtIns !== NONE(8130).BuiltInCommandFilter.DENY) {
    const tmp12 = builtIns === NONE(8130).BuiltInCommandFilter.ONLY_TEXT;
    let builtInCommands = NONE(8132).getBuiltInCommands(permissionContext.commandTypes, true, tmp12);
    const tmp9Result = NONE(8132);
  } else {
    builtInCommands = [];
  }
  const items = [];
  ...
  if (builtInCommands.length > 0) {
    const tmp87 = queryIndexSection(NONE(8132).BUILT_IN_SECTIONS[constants.BUILT_IN], builtInCommands, true, true, obj);
    if (null != tmp87) {
      items.push(tmp87);
    }
  }
  ...
  if (NONE === NONE(8130).ScoreMethod.COMMAND_ONLY) {
...
function queryIndexSection(descriptor, builtInCommands, arg2, arg3, arg4) {
  ...
  if (null != guild_id1) {
    allowedForChannel = importAll(9427).computeAllowedForChannel(descriptor.permissions, context, context.guild_id);
    const obj2 = importAll(9427);
  }
  const items = [];
  ...
  while (iter !== undefined) {
    ...
    if (hasAccessResult === require(9427) /* computeAllowedForUser */.HasAccessResult.ALLOWED) {
  ...
  let arr2 = items;
  if (scoreMethod !== require(8130) /* ScoreMethod */.ScoreMethod.NONE) {
    arr2 = items;
    if (null != query) {
      arr2 = items;
      if (null != splitQuery) {
  ...
  if (0 !== arr2.length) {
    ...
    obj[1] = arr2;
    let tmp36 = obj;
    tmp39 = scoreMethod !== require(8130) /* ScoreMethod */.ScoreMethod.NONE && scoreMethod !== tmp27(8130).ScoreMethod.APPLICATION_ONLY;
  } else {
    tmp36 = null;
  }
  return tmp36;
...
function toServerPermissions(permissions, id) {
  ...
  if (null != permissions.user) {
    ...
    obj[0] = require(6876) /* ApplicationCommandSectionType */.ApplicationCommandPermissionType.USER;
    obj[1] = id;
  ...
  if (null != permissions.channels) {
    ...
    while (tmp28 !== undefined) {
      ...
      [tmp10, tmp11] = tmp9;
      obj[0] = require(6876) /* ApplicationCommandSectionType */.ApplicationCommandPermissionType.CHANNEL;
      obj[1] = tmp10;
      obj[2] = tmp11;
  ...
  if (null != permissions.roles) {
    ...
    while (tmp32 !== undefined) {
      ...
      [tmp20, tmp21] = tmp19;
      obj[0] = require(6876) /* ApplicationCommandSectionType */.ApplicationCommandPermissionType.ROLE;
      obj[1] = tmp20;
      obj[2] = tmp21;
...
prototype["query"] = function query(type, commandTypes, applicationId) {
  ...
  if (null == authStore2.getCurrentUser()) {
    ...
    let obj1 = require(8127) /* computePermissions */;
      ...
      if (!tmp14) {
        ...
        if (tmp29) {
          if (null == guild_id.guild_id) {
            ...
            obj1[1] = guild_id.id;
            const applicationCommandIndex2 = tmp8(8126).requestApplicationCommandIndex(obj1);
            flag6 = true;
            const tmp8Result1 = tmp8(8126);
          }
        }
        obj2 = { type: "guild", guildId: null };
        guild_id = guild_id.guild_id;
        obj2[1] = guild_id;
        const applicationCommandIndex3 = tmp8(8126).requestApplicationCommandIndex(obj2);
        flag6 = true;
        const tmp8Result2 = tmp8(8126);
      }
    }
...
prototype["queryInstallOnDemandApp"] = function queryInstallOnDemandApp(arg0, id) {
  ...
  if (null != channel) {
    ...
    obj1[1] = require(8130) /* ScoreMethod */.ScoreMethod.COMMAND_ONLY;
    obj1[2] = arg0;
    ...
    if (tmp) {
      if (callback2) {
        ...
        if (tmp15) {
          ...
          obj[1] = callback;
          const applicationCommandIndex = callback(8126).requestApplicationCommandIndex(obj);
          const obj3 = callback(8126);
        }
      }
      dependencyMap(false);
    }
...
export const getSection = function getSection(type, applicationId) {
  if (applicationId === constants.BUILT_IN) {
    ...
    obj[0] = require(8132) /* getOptionValue */.BUILT_IN_SECTIONS[tmp2.BUILT_IN];
    const items = [require(1906) /* PermissionOverwriteType */.ApplicationCommandType.CHAT];
    obj[1] = require(8132) /* getOptionValue */.getBuiltInCommands(items, true, false);
    return obj;
  } else {
