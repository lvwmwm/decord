...
// Dependencies: [19, 21, 13882, 13884, 2]
// Exports: default

// Module 14700 (ActivityPrivacyUpsellActionSheet)
...
export default function ActivityPrivacyUpsellActionSheet(direction) {
  ...
  const upsellStrings = direction(13882).getUpsellStrings(direction === direction(13882).ChangeDirection.RESTRICTING, direction.settingName);
  const items = [direction, affectedGuildIds];
  ({ title, subtitle, confirmText, toastContent } = upsellStrings);
  const onConfirm = React.useCallback(() => {
    const result = direction(outer1_2[2]).applyBulkGuildRestrictionChange(direction, affectedGuildIds);
  }, items);
  return jsx(affectedGuildIds(13884), { direction, affectedGuildIds, title, subtitle, confirmText, toastContent, onConfirm });
};
