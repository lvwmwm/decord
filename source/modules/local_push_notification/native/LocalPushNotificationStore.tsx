...
// Function ID: 13041
// Name: set
// Dependencies: [1910, 5171, 13041, 676, 10832, 3943, 1236, 589, 709, 2]
...
const localPushNotificationStore = new LocalPushNotificationStore(require("dispatcher"), {
  CONNECTION_OPEN: function handleCheckScheduledNotifs() {
    const scheduledLocalNotifications = importDefault(10832).getScheduledLocalNotifications((arr) => {
      ...
      const item = found.forEach((userInfo) => {
        ...
        const result = callback(10832).cancelLocalNotifications(userInfo);
        const obj = callback(10832);
        const result1 = callback(10832).cancelLocalNotifications(userInfo);
        set.delete(userInfo);
      });
    });
  },
        ...
        if (guild.verificationLevel === VerificationLevels.MEDIUM) {
          ...
          if (null != obj) {
            if (!obj.isSameOrBefore(importDefault(3943)(), "minute")) {
              ...
              const result = tmp15(10832).scheduleLocalNotification(obj);
              const tmp15Result = tmp15(10832);
            }
            tmp15 = importDefault;
          }
        }
  ...
  GUILD_DELETE: function handleGuildDelete(guild) {
    ...
    if (set.has(obj)) {
      const result = importDefault(10832).cancelLocalNotifications(obj);
      set.delete(obj);
  ...
  LOGOUT: function handleCancelAll() {
    set.clear();
    const result = importDefault(10832).cancelAllLocalNotifications();
  }
});
