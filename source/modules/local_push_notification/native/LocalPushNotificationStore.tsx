...
// Function ID: 13282
// Name: set
// Dependencies: [1910, 5309, 13282, 676, 9578, 4043, 1236, 589, 709, 2]
  ...
  CONNECTION_OPEN: function handleCheckScheduledNotifs() {
    const scheduledLocalNotifications = NativeModulesDefault.getScheduledLocalNotifications((arr) => {
      ...
      const item = found.forEach((userInfo) => {
        ...
        const result = callback(9578).cancelLocalNotifications(userInfo);
        const obj = callback(9578);
        const result1 = callback(9578).cancelLocalNotifications(userInfo);
        set.delete(userInfo);
      });
    });
  },
        ...
        if (guild.verificationLevel === VerificationLevels.MEDIUM) {
          ...
          if (null != obj) {
            if (!obj.isSameOrBefore(hooksDefault(), "minute")) {
              ...
              const result = tmp15(9578).scheduleLocalNotification(obj);
              const tmp15Result = tmp15(9578);
            }
            tmp15 = importDefault;
          }
        }
