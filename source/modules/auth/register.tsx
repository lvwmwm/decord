      ...
      if (c6 === 2) {
        ...
        try {
          ...
          if (0 === c5) {
            ...
            if (callback instanceof phone(10377).CaptchaCancelError) {
              throw callback;
            } else {
...
function registerFull(giftCodeSKUId) {
  ...
  if (null != birthday) {
    ...
    const diffResult = tmp4(3867)().diff(birthday, "years");
    if (diffResult >= 13) {
      if (diffResult < 13) {
        let str3 = "23+";
        if (diffResult >= 18) {
  ...
  obj3[1] = obj4;
  obj1[2] = obj3;
  const tmp4Result1 = importDefault(5094);
  return importDefault(5094).post(obj1).then((body) => {
    ...
    if (arg0 instanceof callback(10377).CaptchaCancelError) {
      throw arg0;
    } else {
