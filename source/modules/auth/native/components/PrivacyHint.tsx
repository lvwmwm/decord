...
// Dependencies: [19, 17, 5722, 14957, 676, 21, 4255, 4251, 1236, 3973, 7751, 7749, 14996, 2]
// Exports: default

// Module 14995 (PrivacyPolicyDescription)
import "noop";
...
function PrivacyPolicyCheckbox(onToggleConsent) {
  ...
  if (asCheckbox) {
    obj = { checked: null };
    obj[0] = consent;
    let tmp7Result = tmp7(tmp2(7751).FormCheckbox, obj);
    let tmp9 = tmp7;
  } else {
    const obj1 = { selected: null };
    obj1[0] = consent;
    tmp7Result = tmp7(tmp2(7749).FormRow.Radio, obj1);
    tmp9 = tmp7;
  }
...
export default function PrivacyHint(arg0) {
  ...
  if (!consentRequired) {
    if (tmp2) {
      ...
      const items = [callback(importDefault(14996), {}), callback(PrivacyPolicyDescription, {})];
      obj[1] = items;
  ...
  items1 = [callback(importDefault(14996), {}), callback(PrivacyPolicyCheckbox, { consent, onToggleConsent, asCheckbox: true })];
  obj[1] = items1;
