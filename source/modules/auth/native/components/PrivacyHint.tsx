...
// Dependencies: [19, 17, 5749, 14995, 676, 21, 4285, 4281, 1236, 4002, 7892, 7890, 15034, 2]
// Exports: default

// Module 15033 (PrivacyPolicyDescription)
import "noop";
...
function PrivacyPolicyCheckbox(onToggleConsent) {
  ...
  if (asCheckbox) {
    obj = { checked: null };
    obj[0] = consent;
    let tmp7Result = tmp7(tmp2(7892).FormCheckbox, obj);
    let tmp9 = tmp7;
  } else {
    const obj1 = { selected: null };
    obj1[0] = consent;
    tmp7Result = tmp7(tmp2(7890).FormRow.Radio, obj1);
    tmp9 = tmp7;
  }
...
export default function PrivacyHint(arg0) {
  ...
  if (!consentRequired) {
    if (tmp2) {
      ...
      const items = [callback(importDefault(15034), {}), callback(PrivacyPolicyDescription, {})];
      obj[1] = items;
  ...
  items1 = [callback(importDefault(15034), {}), callback(PrivacyPolicyCheckbox, { consent, onToggleConsent, asCheckbox: true })];
  obj[1] = items1;
