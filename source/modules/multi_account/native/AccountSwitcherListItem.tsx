...
// Dependencies: [19, 17, 1930, 4225, 1922, 11977, 21, 4380, 589, 4340, 712, 4335, 4376, 1236, 4949, 4109, 1297, 4224, 2]
// Exports: default

// Module 15312 (AccountStatusIcon)
...
import closure_6 from "initialize" /* 4225 */;
import closure_7 from "mergeGuildAvatar" /* 1922 */;
import { MultiAccountTokenStatus } from "initialize" /* 11977 */;
...
export default function AccountSwitcherListItem(arg0) {
  ...
  if (user.id === id) {
    if (showActiveAccountLabel) {
      ...
      let tmp8 = callback(tmp2(4376).Text, obj);
    }
    if (null == onPressUser) {
      let PressableOpacity = closure_3;
    } else {
      PressableOpacity = tmp2(4949).PressableOpacity;
    }
    obj = { selected: null };
    obj[0] = tmp7;
    const radioA11yNative = tmp2(4109).useRadioA11yNative(obj);
    ...
    if (tmp18Result) {
      ...
      tmp18Result = tmp18(tmp2(4376).Text, obj8);
    }
  ...
  if (user.tokenStatus === MultiAccountTokenStatus.INVALID) {
    ...
    tmp8 = callback(tmp2(4376).Text, obj10);
  }
};
