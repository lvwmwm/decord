...
// Dependencies: [19, 17, 1882, 4095, 1874, 11631, 21, 4255, 589, 4216, 712, 4211, 4251, 1236, 4783, 3973, 1297, 4094, 2]
// Exports: default

// Module 14950 (AccountStatusIcon)
import "noop";
...
export default function AccountSwitcherListItem(arg0) {
  ...
  if (user.id === id) {
    if (showActiveAccountLabel) {
      ...
      let tmp8 = callback(tmp2(4251).Text, obj);
    }
    if (null == onPressUser) {
      let PressableOpacity = closure_3;
    } else {
      PressableOpacity = tmp2(4783).PressableOpacity;
    }
    obj = { selected: null };
    obj[0] = tmp7;
    const radioA11yNative = tmp2(3973).useRadioA11yNative(obj);
    ...
    obj7[1] = str;
    obj6[4] = importDefault(4094).getUserTag(obj2, obj7);
    ...
    if (tmp18Result) {
      ...
      tmp18Result = tmp18(tmp2(4251).Text, obj8);
    }
  ...
  if (user.tokenStatus === MultiAccountTokenStatus.INVALID) {
    ...
    tmp8 = callback(tmp2(4251).Text, obj10);
  }
};
