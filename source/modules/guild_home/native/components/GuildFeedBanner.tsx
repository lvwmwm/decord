...
// Dependencies: [19, 17, 13674, 1909, 16106, 676, 21, 4448, 712, 4187, 1493, 4383, 13675, 500, 1431, 7641, 4449, 4452, 589, 7686, 11089, 1363, 16107, 16108, 5972, 4444, 1297, 5036, 4164, 1236, 16109, 2]

// Module 16105 (GuildFeedBanner)
import ThemesDefault from "Themes" /* 712 */;
import importAllResult from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_6 from "initialize" /* 13674 */;
import closure_7 from "createGuildRecordFromRust" /* 1909 */;
import GUILD_FEED_CARD_MARGIN_HORIZONTAL from "GUILD_FEED_CARD_MARGIN_HORIZONTAL" /* 16106 */;
...
function GuildFeedBanner(guild) {
  ...
  const effect = width.useEffect(() => {
    const guildForPopout = guild(13675).fetchGuildForPopout(guild.id);
  }, items);
  const items1 = [guild];
  ...
  function handleLoad() {
    const result = sharedValue.set(guild(4449).withTiming(1, guild(4452).timingSlow));
  }
  let tmp2Result = tmp2(7641);
  ...
  width = size.width;
  height = size.height;
  const tmp8 = sharedValue(4383)();
  drawerWidth = guild(11089).useDrawerWidth();
  ...
  if (null != memo) {
    ...
    if (tmp2Result2.isThemeDark(tmp8)) {
      let tmp6Result = tmp6(16107);
    } else {
      tmp6Result = tmp6(16108);
    }
    obj2[1] = tmp6Result;
    obj2[2] = handleLoad;
  ...
  tmp6Result = tmp6(5972);
  obj7[2] = guild(5972).GuildIconSizes.XLARGE;
  obj6[1] = closure_11(tmp6Result, obj7);
  const items10 = [closure_11(height, obj6), , , ];
  const obj8 = { style: tmp.textContainer, children: null };
  const items11 = [closure_11(guild(4444).Text, { lineClamp: 1, variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", style: tmp.title, children: guild.name }), ];
  ...
  if (null != description) {
    ...
    obj11[3] = description;
    tmp17Result = tmp17(tmp2(4444).Text, obj11);
  }
  ...
  if (tmp15Result1) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      ...
      items12[1] = tmp17(tmp2(4444).Text, obj15);
      obj13[3] = items12;
      tmp15Result = tmp15(tmp2(5036).PressableOpacity, obj13);
    }
    ...
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = tmp17(tmp2(4444).Text, obj18);
        obj16[0] = items14;
