...
function GuildFeedBanner(guild) {
  ...
  const items11 = [callback(guild(4127).Text, obj9), ];
  let tmp28 = null;
  ...
  if (null != description) {
    ...
    tmp32 = callback(guild(4127).Text, obj11);
  }
  items10[2] = tmp32;
  ...
  if (tmp37Result) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      ...
      tmp40 = callback2(guild(4661).PressableOpacity, obj13);
    }
    const items13 = [tmp40, ];
    let tmp48 = null;
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = callback(guild(4127).Text, obj18);
        class G {
          constructor() {
            obj = { opacity: c1.get() };
            return obj;
