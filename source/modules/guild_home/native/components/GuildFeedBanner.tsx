...
// Dependencies: [19, 17, 13245, 1891, 15585, 676, 21, 4302, 712, 4162, 1474, 4238, 13246, 500, 1416, 8378, 4303, 4306, 589, 7581, 10930, 4147, 15586, 15587, 5719, 4298, 1297, 4844, 4002, 1236, 15588, 2]

// Module 15584 (GuildFeedBanner)
...
function GuildFeedBanner(guild) {
  ...
  const tmp12 = sharedValue(7581)();
  dependencyMap = tmp12;
  const size = tmp6(1474)();
  width = size.width;
  height = size.height;
  const tmp8 = sharedValue(4238)();
  drawerWidth = guild(10930).useDrawerWidth();
  ...
  if (null != memo) {
    ...
    if (tmp2Result2.isThemeDark(tmp8)) {
      let tmp6Result = tmp6(15586);
    } else {
      tmp6Result = tmp6(15587);
    }
    obj2[1] = tmp6Result;
    obj2[2] = handleLoad;
    tmp2Result2 = tmp2(4147);
  }
  ...
  tmp6Result = tmp6(5719);
  obj7[2] = guild(5719).GuildIconSizes.XLARGE;
  obj6[1] = closure_11(tmp6Result, obj7);
  const items10 = [closure_11(height, obj6), , , ];
  const obj8 = { style: tmp.textContainer, children: null };
  const items11 = [closure_11(guild(4298).Text, { lineClamp: 1, variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", style: tmp.title, children: guild.name }), ];
  ...
  if (null != description) {
    ...
    obj11[3] = description;
    tmp17Result = tmp17(tmp2(4298).Text, obj11);
  }
  ...
  if (tmp15Result1) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      ...
      items12[1] = tmp17(tmp2(4298).Text, obj15);
      obj13[3] = items12;
      tmp15Result = tmp15(tmp2(4844).PressableOpacity, obj13);
    }
    ...
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = tmp17(tmp2(4298).Text, obj18);
        obj16[0] = items14;
