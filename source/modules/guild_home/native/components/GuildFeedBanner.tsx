...
// Dependencies: [19, 17, 13707, 1909, 16141, 676, 21, 4478, 712, 4217, 1493, 4413, 13708, 500, 1431, 7673, 4479, 4482, 589, 7718, 11126, 1363, 16142, 16143, 6004, 4474, 1297, 5068, 4194, 1236, 16144, 2]

// Module 16140 (GuildFeedBanner)
import ThemesDefault from "Themes" /* 712 */;
import importAllResult from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_6 from "initialize" /* 13707 */;
import closure_7 from "createGuildRecordFromRust" /* 1909 */;
import GUILD_FEED_CARD_MARGIN_HORIZONTAL from "GUILD_FEED_CARD_MARGIN_HORIZONTAL" /* 16141 */;
...
function GuildFeedBanner(guild) {
  ...
  const effect = width.useEffect(() => {
    const guildForPopout = guild(13708).fetchGuildForPopout(guild.id);
  }, items);
  const items1 = [guild];
  ...
  const tmp12 = sharedValue(7718)();
  dependencyMap = tmp12;
  const size = tmp6(1493)();
  width = size.width;
  height = size.height;
  const tmp8 = sharedValue(4413)();
  drawerWidth = guild(11126).useDrawerWidth();
  ...
  if (null != memo) {
    ...
    if (tmp2Result2.isThemeDark(tmp8)) {
      let tmp6Result = tmp6(16142);
    } else {
      tmp6Result = tmp6(16143);
    }
    obj2[1] = tmp6Result;
    obj2[2] = handleLoad;
  ...
  tmp6Result = tmp6(6004);
  obj7[2] = guild(6004).GuildIconSizes.XLARGE;
  obj6[1] = closure_11(tmp6Result, obj7);
  const items10 = [closure_11(height, obj6), , , ];
  const obj8 = { style: tmp.textContainer, children: null };
  const items11 = [closure_11(guild(4474).Text, { lineClamp: 1, variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", style: tmp.title, children: guild.name }), ];
  ...
  if (null != description) {
    ...
    obj11[3] = description;
    tmp17Result = tmp17(tmp2(4474).Text, obj11);
  }
  ...
  if (tmp15Result1) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      ...
      obj13[3] = items12;
      tmp15Result = tmp15(tmp2(5068).PressableOpacity, obj13);
    }
    ...
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = tmp17(tmp2(4474).Text, obj18);
        obj16[0] = items14;
