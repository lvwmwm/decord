...
// Dependencies: [19, 17, 13521, 1910, 15910, 676, 21, 4380, 712, 4120, 1494, 4315, 13522, 500, 1435, 8595, 4381, 4384, 589, 7785, 11084, 1363, 15911, 15912, 5885, 4376, 1297, 4949, 4098, 1236, 15913, 2]

// Module 15909 (GuildFeedBanner)
import ThemesDefault from "Themes" /* 712 */;
import importAllResult from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_6 from "initialize" /* 13521 */;
import closure_7 from "createGuildRecordFromRust" /* 1910 */;
import GUILD_FEED_CARD_MARGIN_HORIZONTAL from "GUILD_FEED_CARD_MARGIN_HORIZONTAL" /* 15910 */;
...
function GuildFeedBanner(guild) {
  ...
  const effect = width.useEffect(() => {
    const guildForPopout = guild(13522).fetchGuildForPopout(guild.id);
  }, items);
  const items1 = [guild];
  ...
  function handleLoad() {
    const result = sharedValue.set(guild(4381).withTiming(1, guild(4384).timingSlow));
  }
  let tmp2Result = tmp2(8595);
  ...
  const tmp12 = sharedValue(7785)();
  dependencyMap = tmp12;
  const size = tmp6(1494)();
  width = size.width;
  height = size.height;
  const tmp8 = sharedValue(4315)();
  drawerWidth = guild(11084).useDrawerWidth();
  ...
  if (null != memo) {
    ...
    if (tmp2Result2.isThemeDark(tmp8)) {
      let tmp6Result = tmp6(15911);
    } else {
      tmp6Result = tmp6(15912);
    }
    obj2[1] = tmp6Result;
    obj2[2] = handleLoad;
  ...
  tmp6Result = tmp6(5885);
  obj7[2] = guild(5885).GuildIconSizes.XLARGE;
  obj6[1] = closure_11(tmp6Result, obj7);
  const items10 = [closure_11(height, obj6), , , ];
  const obj8 = { style: tmp.textContainer, children: null };
  const items11 = [closure_11(guild(4376).Text, { lineClamp: 1, variant: "heading-xl/extrabold", color: "mobile-text-heading-primary", style: tmp.title, children: guild.name }), ];
  ...
  if (null != description) {
    ...
    obj11[3] = description;
    tmp17Result = tmp17(tmp2(4376).Text, obj11);
  }
  ...
  if (tmp15Result1) {
    ...
    if (features.has(GuildFeatures.DISCOVERABLE)) {
      ...
      items12[1] = tmp17(tmp2(4376).Text, obj15);
      obj13[3] = items12;
      tmp15Result = tmp15(tmp2(4949).PressableOpacity, obj13);
    }
    ...
    if (null != discoverableGuild.presenceCount) {
      ...
      if (null != discoverableGuild.memberCount) {
        ...
        items14[1] = tmp17(tmp2(4376).Text, obj18);
        obj16[0] = items14;
