...
// Dependencies: [4475, 3893, 1882, 1218, 3892, 1874, 676, 4476, 4239, 4483, 11, 4488, 4489, 4586, 3837, 4587, 4589, 2]
// Exports: canEditMessageWithStickers, hasEphemeralAppearance, updateMessageRecord, updateServerMessage

// Module 4474 (createMinimalMessageRecord)
...
function createMinimalMessageRecord(timestamp) {
  ...
  if (null == timestamp.embeds) {
    ...
    items = _require(4587).mergeEmbedsOnURL(mapped);
    const obj2 = _require(4587);
  }
  obj.embeds = items;
  ...
  obj.components = _require(4476).transformComponents(components);
...
function createMessageRecord(message, message) {
  ...
  let obj2 = require(4483) /* useNullableMessageAuthor */;
  ...
  const findGiftCodes = require(4489) /* _resolveGiftCode */.findGiftCodes;
  ...
  if (null == reactions) {
    ...
    if (null != message.poll) {
      tmp50 = importDefault(4586)(message.poll);
    }
    obj.poll = tmp50;
    ...
    if (null == message2.embeds) {
      if (null == message2.message_snapshots) {
        ...
        if (null != message2.components) {
          result8 = result7.set("components", _require(4476).transformComponents(message2.components));
          const obj14 = _require(4476);
        }
        let result9 = result8;
        ...
        if (null == message2.reactions) {
          ...
          if (null != message2.poll) {
            result10 = result9.set("poll", importDefault(4586)(message2.poll));
          }
          let flag = false;
          ...
          if (flag) {
            ...
            obj1[0] = result13;
            obj1[1] = store.getId();
            result14 = result13.set("mentioned", importDefault(4488)(obj1));
            const tmp43 = importDefault(4488);
          }
          return result14;
        } else {
      ...
      if (null == message2.embeds) {
        ...
        const mapped = embeds.map((footer) => message2(outer1_2[15]).sanitizeEmbed(message2.channel_id, message2.id, footer));
        items3 = _require(4587).mergeEmbedsOnURL(mapped);
        const obj9 = _require(4587);
      }
...
export const canEditMessageWithStickers = function canEditMessageWithStickers(content) {
  let tmp = 0 === require(4589) /* getStickerExtensionFromFormatType */.getMessageStickers(content).length;
  if (!tmp) {
    tmp = "" !== content.content;
  }
  return tmp;
