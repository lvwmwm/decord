...
// Dependencies: [1910, 3989, 4337, 676, 688, 689, 4193, 1363, 5927, 589, 5928, 5929, 2]
...
// Module 5926 (useCanManageGuildOfficialMessages)
...
export const getAccessibleGuildOfficialTextColor = function getAccessibleGuildOfficialTextColor(officialMessageColor, merged, arg2, arg3) {
  ...
  return require(4193) /* hexToRgba */.getAccessibleForegroundColor(obj);
};
...
export const useCanToggleGuildOfficialMessages = function useCanToggleGuildOfficialMessages(message, channel, LongPressMessageActionSheet) {
  ...
  let tmp3 = !tmpResult;
  if (tmpResult) {
    tmp3 = importDefault(5929)(message);
  }
  if (tmp3) {
    return !tmp3;
  } else if (message.hasFlag(constants3.IS_GUILD_OFFICIAL)) {
    let isActiveChannelOrUnarchivableThread = require(5928) /* useCanUnarchiveThread */.getIsActiveChannelOrUnarchivableThread(channel);
    const obj2 = require(5928) /* useCanUnarchiveThread */;
  } else {
    isActiveChannelOrUnarchivableThread = null != channel && !channel.isPrivate();
    if (isActiveChannelOrUnarchivableThread) {
      isActiveChannelOrUnarchivableThread = require(5928) /* useCanUnarchiveThread */.getIsActiveChannelOrUnarchivableThread(channel);
...
export const canSendGuildOfficialMessages = function canSendGuildOfficialMessages(guild, channel, _sendMessage) {
  ...
  if (enabled) {
    ...
    if (isActiveChannelOrUnarchivableThread) {
      isActiveChannelOrUnarchivableThread = require(5928) /* useCanUnarchiveThread */.getIsActiveChannelOrUnarchivableThread(channel);
      const obj3 = require(5928) /* useCanUnarchiveThread */;
    }
    if (isActiveChannelOrUnarchivableThread) {
      isActiveChannelOrUnarchivableThread = channel.type !== constants.GUILD_VOICE;
    }
