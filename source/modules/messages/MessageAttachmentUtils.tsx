...
// Dependencies: [3989, 676, 5950, 5955, 5954, 1403, 8454, 647, 4034, 10969, 1236, 2]
...
// Module 11459 (getForumPostShouldObscure)
...
function getForumPostShouldObscure(media, arg1, enabledHarmTypesBitmaskForChannelType) {
  if (null == media) {
    ...
    if (require(8454) /* isMediaAttachment */.ForumPostMediaTypes.EMBED === type) {
      let obj = { type: null, media: null };
      obj[0] = tmp4(5955).ObscuredMediaTypes.Embed;
      obj[1] = media;
      let tmp = obj;
    } else if (tmp4(8454).ForumPostMediaTypes.ATTACHMENT === type) {
      ...
      tmp = null;
      if (tmp4(8454).ForumPostMediaTypes.COMPONENT === type) {
        obj = { type: null, media: null };
    ...
    if (null == tmp) {
      ...
      if (mediaObscuredReasonFromBitmask.length > 0) {
        ...
        if (tmp8) {
          items3[0] = true;
          items3[1] = tmp4(5954).ObscureReason.POTENTIAL_EXPLICIT_CONTENT;
          tmp2 = items3;
        } else if (media.spoiler) {
          items3[0] = arg1;
          items3[1] = tmp4(5954).ObscureReason.SPOILER;
          tmp2 = items3;
        } else {
          items3[0] = false;
          items3[1] = undefined;
...
export const getObscureReasonForAttachment = function getObscureReasonForAttachment(attachment, enabledHarmTypesBitmaskForChannelAndAuthorId) {
  ...
  obj = { type: require(5955) /* ContentHarmTypeChannel */.ObscuredMediaTypes.Attachment, media: attachment };
  ...
  require(5950) /* getEligibleHarmTypesConfigsForContext */;
  obj = { type: require(5955) /* ContentHarmTypeChannel */.ObscuredMediaTypes.Attachment, media: attachment };
  if (mediaObscuredReasonFromBitmask.length > 0) {
    let first = mediaObscuredReasonFromBitmask[0];
  } else if (tmp4) {
    first = tmp(5954).ObscureReason.POTENTIAL_EXPLICIT_CONTENT;
  } else {
    first = null;
    if (tmpResult.hasFlag(num, constants.IS_SPOILER)) {
      first = tmp(5954).ObscureReason.SPOILER;
    }
    tmpResult = tmp(1403);
  }
  return first;
...
export const getObscureReasonForEmbed = function getObscureReasonForEmbed(embed, message, flag2, getUncachedChannelPermissions) {
  let obj = require(5950) /* getEligibleHarmTypesConfigsForContext */;
  obj = { type: require(5955) /* ContentHarmTypeChannel */.ObscuredMediaTypes.Embed, media: embed };
  ...
  if (!bot) {
    obj = { type: null, media: null };
    obj[0] = tmp(5955).ObscuredMediaTypes.Embed;
    obj[1] = embed;
    isMediaScanPendingResult = tmp(5950).isMediaScanPending(obj, getUncachedChannelPermissions);
...
export const getObscureReasonForUnfurledMediaItem = function getObscureReasonForUnfurledMediaItem(media, enabledContentHarmTypeFlags, spoiler, isBot) {
  ...
  let obj = require(5950) /* getEligibleHarmTypesConfigsForContext */;
  obj = { type: require(5955) /* ContentHarmTypeChannel */.ObscuredMediaTypes.GenericMedia, media };
  ...
  if (!flag2) {
    ...
    obj[1] = media;
    isMediaScanPendingResult = tmp(5950).isMediaScanPending(obj, enabledContentHarmTypeFlags);
    const tmpResult = tmp(5950);
  }
  if (mediaObscuredReasonFromBitmask.includes(require(5954) /* ObscureReason */.ObscureReason.EXPLICIT_CONTENT)) {
    let EXPLICIT_CONTENT = tmp(5954).ObscureReason.EXPLICIT_CONTENT;
  } else if (mediaObscuredReasonFromBitmask.includes(tmp(5954).ObscureReason.GORE_CONTENT)) {
    EXPLICIT_CONTENT = tmp(5954).ObscureReason.GORE_CONTENT;
  } else if (mediaObscuredReasonFromBitmask.includes(tmp(5954).ObscureReason.SELF_HARM_CONTENT)) {
    EXPLICIT_CONTENT = tmp(5954).ObscureReason.SELF_HARM_CONTENT;
  } else if (isMediaScanPendingResult) {
    EXPLICIT_CONTENT = tmp(5954).ObscureReason.POTENTIAL_EXPLICIT_CONTENT;
  } else {
    EXPLICIT_CONTENT = null;
    if (flag) {
      EXPLICIT_CONTENT = tmp(5954).ObscureReason.SPOILER;
    }
  }
...
export const useShouldObscure = function useShouldObscure(channel) {
  ...
  const enabledHarmTypesBitmaskForChannelType = channel(5950).getEnabledHarmTypesBitmaskForChannelType(channel(5955).ContentHarmTypeChannel.GUILD);
  return getForumPostShouldObscure(channel.media, !importDefault(10969)(setting, stateFromStores), enabledHarmTypesBitmaskForChannelType);
};
export const getObscuredAlt = function getObscuredAlt(obscureReason) {
  if (require(5954) /* ObscureReason */.ObscureReason.EXPLICIT_CONTENT !== obscureReason) {
    if (tmp(5954).ObscureReason.GORE_CONTENT !== obscureReason) {
      if (tmp(5954).ObscureReason.SELF_HARM_CONTENT !== obscureReason) {
        if (tmp(5954).ObscureReason.SPOILER === obscureReason) {
