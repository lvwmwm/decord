...
function ArchivedLockedThreadChatBanner(channel) {
  ...
  obj[4] = stringResult;
  const items1 = [closure_12(channel(4185).Text, obj), ];
  if (canUnarchiveThread) {
    ...
    obj1[1] = tmp8(tmp3(4600).Button, obj2);
...
function NewMessagesChatBar(channel) {
  ...
  if (obj.useStateFromStores(items, () => connected.isConnected(), [])) {
    if (unreadCount <= 0) {
      ...
      obj[3] = callback(XSmallBoldIcon(4185).Text, obj);
      const items1 = [callback(XSmallBoldIcon(4717).PressableOpacity, obj), ];
      handleScrollToNewMessages = XSmallBoldIcon(4717).PressableOpacity;
