...
// Dependencies: [5379, 1386, 4120, 673, 1233, 1994, 7891, 11, 4675, 7893, 7895, 2]
// Exports: createStageRaiseHandSystemMessage

// Module 7960 (createStageRaiseHandSystemMessage)
...
export const createStageRaiseHandSystemMessage = function createStageRaiseHandSystemMessage(roleStyle) {
  ...
  if (canResult) {
    ...
    canResult = rtsState === tmp(4675).RequestToSpeakStates.REQUESTED_TO_SPEAK;
  }
  if (canResult) {
  ...
  obj = { username: messageAuthorWithProcessedColor.nick, usernameOnClick: tmp6(7893)({ message, author: messageAuthorWithProcessedColor, roleStyle: roleStyle.roleStyle }) };
  ...
  if (message.hasFlag(constants2.EPHEMERAL)) {
    if (message.type === constants3.STAGE_RAISE_HAND) {
      ...
      obj3[1] = message;
      obj2[0] = obj3;
      obj1[0] = intl3.formatToParts(tmp(1233).t["qDAX++"], obj2);
      obj1[1] = tmp6(1994).getArticleURL(constants.EPHEMERAL_MESSAGES);
      const intl4 = tmp(1233).intl;
      obj1[2] = intl4.string(tmp(1233).t.htHOrp);
      tmp10 = obj1;
      const tmp6Result = tmp6(1994);
    }
  }
  obj[3] = tmp10;
  const merged = Object.assign(tmp6(7895)(roleStyle));
  return obj;
};
