...
// Dependencies: [17, 1391, 1990, 3989, 3998, 1922, 676, 12, 1363, 712, 8086, 7009, 4572, 1236, 8139, 3943, 8162, 8163, 1435, 8166, 7836, 1438, 8167, 5791, 8113, 4534, 3978, 4031, 8168, 8169, 2]
// Exports: createAutoModerationActionSystemMessage

// Module 8161 (createAutoModerationActionSystemMessage)
...
export const createAutoModerationActionSystemMessage = function createAutoModerationActionSystemMessage(message) {
  ...
  const tmp = require;
  let obj = require(7009) /* getDecisionOutcomeFromMessage */;
  if (obj.isAutomodMessageRecord(message)) {
    ...
    if (result1) {
      ...
      if (tmp(7009).IS_BACKWARDS_COMPAT_RAID_TYPE !== notificationType) {
        if (tmp(8139).AutomodNotificationEmbedTypeKeys.RAID !== notificationType) {
          if (tmp(8139).AutomodNotificationEmbedTypeKeys.MENTION_RAID === notificationType) {
            ...
            obj[2] = tmp(8086).processColorOrThrow(internal3.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_CRITICAL));
            const tmpResult4 = tmp(8086);
            obj[3] = closure_4.resolveAssetSource(tmp(1435).makeSource(importDefault(7836))).uri;
            const tmpResult5 = tmp(1435);
            const internal4 = importDefault(712).internal;
            obj[4] = tmp(8086).processColorOrThrow(internal4.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_CRITICAL));
            ...
            if (null != avatar) {
              ...
              obj1[1] = tmp(8086).processColorOrThrow(internal.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_POSITIVE));
              const tmpResult10 = tmp(8086);
              obj1[2] = closure_4.resolveAssetSource(tmp(1435).makeSource(importDefault(8167))).uri;
              const tmpResult11 = tmp(1435);
              const internal2 = importDefault(712).internal;
              obj1[3] = tmp(8086).processColorOrThrow(internal2.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_POSITIVE));
            ...
            obj2[1] = tmp(8086).processColorOrThrow(internal7.resolveSemanticColor(theme, importDefault(712).colors.TEXT_SUBTLE));
            const tmpResult13 = tmp(8086);
            obj2[2] = closure_4.resolveAssetSource(tmp(1435).makeSource(importDefault(7836))).uri;
            const tmpResult14 = tmp(1435);
            const internal8 = importDefault(712).internal;
            obj2[3] = tmp(8086).processColorOrThrow(internal8.resolveSemanticColor(theme, importDefault(712).colors.TEXT_SUBTLE));
      ...
      obj5[5] = tmp(8086).processColorOrThrow(internal5.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_CRITICAL));
      ...
      obj5[7] = tmp(8086).processColorOrThrow(internal6.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_CRITICAL));
      intl9 = tmp(1236).intl;
