...
// Dependencies: [17, 1372, 1942, 3913, 3922, 1874, 676, 12, 4131, 712, 7855, 6876, 4513, 1236, 7908, 3867, 7931, 7932, 1416, 7935, 7718, 1419, 7936, 5634, 7882, 4475, 3902, 3955, 7937, 7938, 2]
// Exports: createAutoModerationActionSystemMessage

// Module 7930 (createAutoModerationActionSystemMessage)
...
let closure_11 = apply.memoize((arg0) => {
  ...
  obj[0] = tmp(7855).processColorOrThrow(tmp4);
  return obj;
});
...
export const createAutoModerationActionSystemMessage = function createAutoModerationActionSystemMessage(message) {
  ...
  const tmp = require;
  let obj = require(6876) /* getDecisionOutcomeFromMessage */;
  if (obj.isAutomodMessageRecord(message)) {
    ...
    let str = tmp(6876).getActionHeaderTextMobile(message, author, interactionUserId);
    const intl = tmp(1236).intl;
    const stringResult = intl.string(tmp(1236).t["94JbM3"]);
    const tmpResult1 = tmp(6876);
    const result1 = tmp(6876).isAutomodNotification(message);
    let tmp14 = null;
    let tmp15 = stringResult;
    if (result1) {
      let resolvedReason = tmp(6876).extractAutomodNotificationFields(message);
      const notificationType = resolvedReason.notificationType;
      if (tmp(6876).IS_BACKWARDS_COMPAT_RAID_TYPE !== notificationType) {
        if (tmp(7908).AutomodNotificationEmbedTypeKeys.RAID !== notificationType) {
          if (tmp(7908).AutomodNotificationEmbedTypeKeys.MENTION_RAID === notificationType) {
            let fromNowResult;
            if (null != resolvedReason.raidDatetime) {
              fromNowResult = importDefault(3867)(resolvedReason.raidDatetime).fromNow();
              const obj17 = importDefault(3867)(resolvedReason.raidDatetime);
            }
            ...
            obj[2] = tmp(7855).processColorOrThrow(internal3.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_CRITICAL));
            ...
            obj[4] = tmp(7855).processColorOrThrow(internal4.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_CRITICAL));
            const intl6 = tmp(1236).intl;
            obj[5] = intl6.string(tmp(1236).t.SWIWEV);
            str = "";
            tmp14 = obj;
            tmp15 = stringResult;
            const tmpResult6 = tmp(7855);
          } else if (tmp(7908).AutomodNotificationEmbedTypeKeys.ACTIVITY_ALERTS_ENABLED === notificationType) {
            const userIdOfAutomodAction = tmp(6876).getUserIdOfAutomodAction(message);
            ...
            if (null != avatar) {
              ...
              obj1[1] = tmp(7855).processColorOrThrow(internal.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_POSITIVE));
              const tmpResult10 = tmp(7855);
              obj1[2] = closure_4.resolveAssetSource(tmp(1416).makeSource(importDefault(7936))).uri;
              const tmpResult11 = tmp(1416);
              const internal2 = importDefault(712).internal;
              obj1[3] = tmp(7855).processColorOrThrow(internal2.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_POSITIVE));
              const intl3 = tmp(1236).intl;
              obj1[4] = intl3.string(tmp(1236).t["QV/8u5"]);
              const tmpResult12 = tmp(7855);
              obj1[6] = importDefault(3867)(message.timestamp).fromNow();
              ...
              tmp14 = obj1;
              tmp15 = stringResult;
              const obj16 = importDefault(3867)(message.timestamp);
            }
            ...
            obj2[1] = tmp(7855).processColorOrThrow(internal7.resolveSemanticColor(theme, importDefault(712).colors.TEXT_SUBTLE));
            ...
            obj2[3] = tmp(7855).processColorOrThrow(internal8.resolveSemanticColor(theme, importDefault(712).colors.TEXT_SUBTLE));
            const intl18 = tmp(1236).intl;
            obj2[4] = intl18.string(tmp(1236).t["NxHYX/"]);
            str = "";
            tmp14 = obj2;
            tmp15 = stringResult;
            const tmpResult15 = tmp(7855);
          }
        }
      }
      let fromNowResult1;
      if (null != resolvedReason.raidDatetime) {
        fromNowResult1 = importDefault(3867)(resolvedReason.raidDatetime).fromNow();
        const obj22 = importDefault(3867)(resolvedReason.raidDatetime);
      }
      let str2 = "";
      if (null != resolvedReason.raidDatetime) {
        const _Date = Date;
        const date = new Date(resolvedReason.raidDatetime);
        str2 = date.toLocaleString(tmp(1236).intl.currentLocale, tmp(7932).DATE_CONFIG);
      }
      const tmp42 = resolvedReason.raidType === tmp(7931).AutomodRaidAlertTypes.DM_RAID;
      ...
      obj5[5] = tmp(7855).processColorOrThrow(internal5.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_CRITICAL));
      ...
      obj5[7] = tmp(7855).processColorOrThrow(internal6.resolveSemanticColor(theme, importDefault(712).colors.TEXT_FEEDBACK_CRITICAL));
      intl9 = tmp(1236).intl;
    ...
    if (null == guildId) {
      ...
      if (null != iconRoleId) {
        if (null != guildId) {
          ...
          obj7[0] = guildId;
          obj7[1] = iconRoleId;
          roleIcon = tmp(5634).getRoleIcon(obj7);
          const tmpResult19 = tmp(5634);
        }
      }
      const obj8 = {};
      ...
      obj11[12] = tmp(3902).isMemberCommunicationDisabled(member1);
      const tmpResult24 = tmp(3902);
      obj11[13] = tmp(3955).accessibilityLabelCalendarFormat(message.timestamp);
      obj9[3] = obj11;
      obj9[4] = tmp14;
      ...
      if (null != quarantineType) {
        ...
        obj13[0] = tmp(6876).getQuarantineReasonString(quarantineType);
        formatToPlainStringResult1 = intl15.formatToPlainString(tmp(1236).t["26bB2M"], obj13);
        const tmpResult26 = tmp(6876);
      }
      obj9[6] = formatToPlainStringResult1;
      const tmpResult25 = tmp(3955);
      obj9[7] = closure_4.resolveAssetSource(tmp(1416).makeSource(tmp57(result1 ? 7937 : 7938))).uri;
