...
// Dependencies: [5804, 11200, 4470, 1236, 2]
// Exports: createCtaButton

// Module 12514 (createCtaButton)
...
export const createCtaButton = function createCtaButton(id, channel_id, arg2) {
  let obj = require(11200) /* CtaButtonType */;
  const ctaButtonType = obj.getCtaButtonType(id, channel_id);
  let obj1 = require(4470) /* useAgeVerificationRunner */;
  const isAgeVerifiedResult = obj1.isAgeVerified();
  const result = require(4470) /* useAgeVerificationRunner */.shouldShowTiggerPawtect();
  if (require(11200) /* CtaButtonType */.CtaButtonType.MARK_AS_FALSE_POSITIVE === ctaButtonType) {
    ...
    if (getFpMessageInfo.canSubmitFpReport(id)) {
      prop = tmp(11200).CtaButtonType.MARK_AS_FALSE_POSITIVE;
    }
    obj[3] = prop;
    return obj;
  } else if (tmp(11200).CtaButtonType.AGE_VERIFICATION_RETRY === ctaButtonType) {
    ...
    let prop1;
    if (result) {
      prop1 = tmp(11200).CtaButtonType.AGE_VERIFICATION_RETRY;
    }
    obj[3] = prop1;
    return obj;
  } else if (tmp(11200).CtaButtonType.CONNECT_TO_TEEN === ctaButtonType) {
    ...
    obj1[3] = tmp(11200).CtaButtonType.CONNECT_TO_TEEN;
    return obj1;
  }
  const obj3 = require(4470) /* useAgeVerificationRunner */;
};
