...
export const createCtaButtons = function createCtaButtons(id, channel_id, arg2) {
  ...
  if (CtaButtonType.CtaButtonType.MARK_AS_FALSE_POSITIVE === ctaButtonType) {
    ...
    if (closure_3.canSubmitFpReport(id)) {
      prop = tmp(11477).CtaButtonType.MARK_AS_FALSE_POSITIVE;
    }
    obj = { ctaButton: null };
    obj[3] = prop;
    obj[0] = obj;
    return obj;
  } else if (tmp(11477).CtaButtonType.AGE_VERIFICATION_RETRY === ctaButtonType) {
    ...
    let prop1;
    if (result) {
      prop1 = tmp(11477).CtaButtonType.AGE_VERIFICATION_RETRY;
    }
    ...
    if (tmpResult.isAgeVerificationMessageWithManualReviewCta(channel_id, id)) {
      ...
      let prop2;
      if (result) {
        prop2 = tmp(11477).CtaButtonType.AGE_VERIFICATION_MANUAL_REVIEW;
      }
      obj3[3] = prop2;
      tmp8 = obj3;
    }
    obj2[1] = tmp8;
    return obj2;
  } else if (tmp(11477).CtaButtonType.CONNECT_TO_TEEN === ctaButtonType) {
    ...
    obj5[3] = tmp(11477).CtaButtonType.CONNECT_TO_TEEN;
    obj4[0] = obj5;
    return obj4;
  } else {
    return {};
