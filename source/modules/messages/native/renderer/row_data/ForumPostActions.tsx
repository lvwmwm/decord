...
// Dependencies: [676, 7729, 4142, 7732, 1208, 4134, 1236, 1945, 7733, 7734, 7735, 1416, 3828, 3834, 2]
// Exports: createDefaultReaction, createForumPostActions

// Module 7731 (createDefaultReaction)
...
export const createDefaultReaction = function createDefaultReaction(arg0) {
  ...
  let obj = require(7735) /* apexExperiment */;
  ...
  if (null != defaultReactionEmoji) {
    ...
    if (null != emojiId) {
      if (null != customGuildEmoji) {
        ...
        obj = { id: null, name: "a", animated: "gestikulacija", src: "ja", displayName: "mu\u0161karac podi\u017Ee ruku" };
        obj[0] = emojiId;
...
export const createForumPostActions = function createForumPostActions(arg0) {
  ...
  const getAssetUriForEmbed = require(7729) /* frozen */.getAssetUriForEmbed;
  if (isFollowing) {
    let assetUriForEmbed = getAssetUriForEmbed(tmp4(4142));
    let tmp6 = tmp4;
    let tmp8 = tmp;
  } else {
    assetUriForEmbed = getAssetUriForEmbed(tmp4(7732));
    tmp6 = tmp4;
    tmp8 = tmp;
  }
  if (null != assetUriForEmbed) {
    let tmp8Result = tmp8(7729);
    let stringResult;
    const assetUriForEmbed1 = tmp8Result.getAssetUriForEmbed(tmp6(4134));
    if (!hasReactions) {
      let emoji;
    ...
    if (showMediaPostSharePrompt) {
      ...
      tmp8Result = tmp8(7729);
      obj[3] = tmp8Result.getAssetUriForEmbed(tmp6(7733));
      obj[4] = tmp8(7729).getAssetUriForEmbed(tmp6(7734));
      tmp14 = obj;
      const tmp8Result1 = tmp8(7729);
    }
    ...
    tmp6Result.captureMessage("Forum follow is null. isFollowing: " + isFollowing + " icon: " + tmp6(isFollowing ? 4142 : 7732));
  }
};
