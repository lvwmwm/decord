...
// Dependencies: [676, 7995, 4255, 7998, 1208, 4247, 1236, 1974, 7999, 8000, 8001, 1416, 3940, 3946, 2]
// Exports: createDefaultReaction, createForumPostActions

// Module 7997 (createDefaultReaction)
...
export const createDefaultReaction = function createDefaultReaction(arg0) {
  ...
  let obj = require(8001) /* apexExperiment */;
  ...
  if (null != defaultReactionEmoji) {
    ...
    if (null != emojiId) {
      if (null != customGuildEmoji) {
        ...
        obj = { id: null, name: "a", animated: "<string:4009755510>", src: "isArray", displayName: "accessible" };
        obj[0] = emojiId;
...
export const createForumPostActions = function createForumPostActions(arg0) {
  ...
  const getAssetUriForEmbed = require(7995) /* frozen */.getAssetUriForEmbed;
  if (isFollowing) {
    let assetUriForEmbed = getAssetUriForEmbed(tmp4(4255));
    let tmp6 = tmp4;
    let tmp8 = tmp;
  } else {
    assetUriForEmbed = getAssetUriForEmbed(tmp4(7998));
    tmp6 = tmp4;
    tmp8 = tmp;
  }
  if (null != assetUriForEmbed) {
    let tmp8Result = tmp8(7995);
    let stringResult;
    const assetUriForEmbed1 = tmp8Result.getAssetUriForEmbed(tmp6(4247));
    if (!hasReactions) {
      let emoji;
    ...
    if (showMediaPostSharePrompt) {
      ...
      tmp8Result = tmp8(7995);
      obj[3] = tmp8Result.getAssetUriForEmbed(tmp6(7999));
      obj[4] = tmp8(7995).getAssetUriForEmbed(tmp6(8000));
      tmp14 = obj;
      const tmp8Result1 = tmp8(7995);
    }
    ...
    tmp6Result.captureMessage("Forum follow is null. isFollowing: " + isFollowing + " icon: " + tmp6(isFollowing ? 4255 : 7998));
  }
};
