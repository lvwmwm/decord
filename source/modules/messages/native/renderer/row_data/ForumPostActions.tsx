...
// Dependencies: [676, 8273, 4396, 8276, 1208, 4388, 1236, 1995, 8277, 8278, 1435, 8279, 4100, 4106, 2]
// Exports: createDefaultReaction, createForumPostActions

// Module 8275 (createDefaultReaction)
  ...
  if (null != defaultReactionEmoji) {
    ...
    if (null != emojiId) {
      if (null != customGuildEmoji) {
        ...
        let obj = { id: null, name: "a", animated: true, src: true, displayName: true };
        obj[0] = emojiId;
...
export const createForumPostActions = function createForumPostActions(arg0) {
  ...
  if (isFollowing) {
    let assetUriForEmbed = getAssetUriForEmbed(tmp4(4396));
    let tmp6 = tmp4;
    let tmp8 = tmp;
  } else {
    assetUriForEmbed = getAssetUriForEmbed(tmp4(8276));
    tmp6 = tmp4;
    tmp8 = tmp;
  }
  if (null != assetUriForEmbed) {
    let tmp8Result = tmp8(8273);
    let stringResult;
    const assetUriForEmbed1 = tmp8Result.getAssetUriForEmbed(tmp6(4388));
    if (!hasReactions) {
      let emoji;
    ...
    if (showMediaPostSharePrompt) {
      ...
      tmp8Result = tmp8(8273);
      obj[3] = tmp8Result.getAssetUriForEmbed(tmp6(8277));
      obj[4] = tmp8(8273).getAssetUriForEmbed(tmp6(8278));
      tmp14 = obj;
      const tmp8Result1 = tmp8(8273);
    }
    ...
    tmp6Result.captureMessage("Forum follow is null. isFollowing: " + isFollowing + " icon: " + tmp6(isFollowing ? 4396 : 8276));
  }
};
