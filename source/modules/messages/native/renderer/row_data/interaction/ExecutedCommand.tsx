...
// Dependencies: [17, 1930, 1391, 1922, 676, 1438, 1435, 4699, 8075, 712, 7199, 1955, 7803, 8076, 1236, 8077, 2]
// Exports: createExecutedCommand

// Module 8074 (createExecutedCommand)
...
import canViewInteractionInfo from "canViewInteractionInfo" /* 8075 */;
...
export const createExecutedCommand = function createExecutedCommand(message, channel, roleStyle, theme, defaultUsernameColor) {
  ...
  if (null != message.interaction) {
    ...
    if (null == user1) {
      ...
      if (result1) {
        processColorStringsResult = tmp17(7803).processColorStrings(userAuthor.colorStrings);
        const tmp17Result2 = tmp17(7803);
      }
      let processColorStringsResult1 = null;
      if (result2) {
        processColorStringsResult1 = tmp17(7803).processColorStrings(userAuthor1.colorStrings);
        const tmp17Result3 = tmp17(7803);
      }
      ...
      if (tmp) {
        if (!result) {
          const result3 = tmp17(8076).isActivitiesInTextEnabled(channel);
      ...
      if (result) {
        result4 = tmp17(8077).formatPrimaryEntryPointCommandName(displayName);
        const tmp17Result6 = tmp17(8077);
      }
      ...
      if (null != tmp25) {
        ...
        obj9.targetUsername = tmp17(4699).getUserAuthor(tmp25, channel).nick;
        let id2;
        if (tmp25 != null) {
          id2 = tmp25.id;
        }
