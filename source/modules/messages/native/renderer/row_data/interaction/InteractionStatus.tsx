...
// Dependencies: [8289, 1236, 2]
// Exports: createInteractionStatus

// Module 8386 (createInteractionStatus)
...
export const createInteractionStatus = function createInteractionStatus(message, interaction) {
  let obj = require(8289) /* _executeMessageComponentInteraction */;
  const interactionStatusViewState = obj.getInteractionStatusViewState(message, interaction);
  if (require(8289) /* _executeMessageComponentInteraction */.InteractionStatusViewState.SENDING === interactionStatusViewState) {
    obj = { text: null, state: null };
    const intl4 = tmp(1236).intl;
    obj[0] = intl4.string(tmp(1236).t.RiLfBY);
    obj[1] = constants.LOADING;
    return obj;
  } else if (tmp(8289).InteractionStatusViewState.CREATED === interactionStatusViewState) {
    obj = { text: null, state: null };
    ...
    obj[1] = constants.LOADING;
    return obj;
  } else if (tmp(8289).InteractionStatusViewState.TIMED_OUT === interactionStatusViewState) {
    const obj2 = { text: null, state: null };
    const intl2 = tmp(1236).intl;
    obj2[0] = intl2.string(tmp(1236).t.h8hzPd);
    obj2[1] = constants.FAILED;
    return obj2;
  } else if (tmp(8289).InteractionStatusViewState.FAILED === interactionStatusViewState) {
    ...
    return obj3;
  } else if (tmp(8289).InteractionStatusViewState.EPHEMERAL_SUCCESS === interactionStatusViewState) {
