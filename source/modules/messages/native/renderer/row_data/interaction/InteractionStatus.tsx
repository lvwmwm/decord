...
// Dependencies: [8375, 1236, 2]
// Exports: createInteractionStatus

// Module 8471 (createInteractionStatus)
import set from "set" /* 2 */;
import _executeMessageComponentInteraction from "_executeMessageComponentInteraction" /* 8375 */;
...
export const createInteractionStatus = function createInteractionStatus(message, interaction) {
  ...
  if (_executeMessageComponentInteraction.InteractionStatusViewState.SENDING === interactionStatusViewState) {
    ...
    return obj;
  } else if (tmp(8375).InteractionStatusViewState.CREATED === interactionStatusViewState) {
    obj = { text: null, state: null };
    ...
    obj[1] = constants.LOADING;
    return obj;
  } else if (tmp(8375).InteractionStatusViewState.TIMED_OUT === interactionStatusViewState) {
    const obj2 = { text: null, state: null };
    const intl2 = tmp(1236).intl;
    obj2[0] = intl2.string(tmp(1236).t.h8hzPd);
    obj2[1] = constants.FAILED;
    return obj2;
  } else if (tmp(8375).InteractionStatusViewState.FAILED === interactionStatusViewState) {
    ...
    return obj3;
  } else if (tmp(8375).InteractionStatusViewState.EPHEMERAL_SUCCESS === interactionStatusViewState) {
