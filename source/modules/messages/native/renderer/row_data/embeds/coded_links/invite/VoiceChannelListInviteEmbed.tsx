...
// Dependencies: [17, 4247, 9657, 1415, 1372, 1942, 1862, 3883, 3892, 1874, 4271, 4328, 9658, 676, 7104, 9659, 7851, 7853, 1416, 4094, 4066, 4446, 1236, 9660, 2]
// Exports: canShowVoiceChannelListInviteEmbed, createVoiceChannelListInviteEmbed

// Module 9656 (canShowVoiceChannelListInviteEmbed)
...
export const canShowVoiceChannelListInviteEmbed = function canShowVoiceChannelListInviteEmbed(invite) {
  ...
  if (null != invite.guild) {
    ...
    tmp = null;
    if (null != guild) {
      let channel = importDefault(9659)(invite).channel;
      tmp = null;
      if (null != channel) {
        tmp = null;
...
export const createVoiceChannelListInviteEmbed = function createVoiceChannelListInviteEmbed(invite, closure_2) {
  ...
  if (null != invite.guild) {
    ...
    tmp = null;
    if (null != guild) {
      let channel = displayNameStylesEnabled(9659)(invite).channel;
      tmp = null;
      if (null != channel) {
        tmp = null;
        if (channel.isGuildVocal()) {
  ...
  if (null == tmp) {
    ...
    ({ colors, baseColors } = displayNameStylesEnabled(7851)(closure_2));
    let assetUriForEmbed;
    if (null != guild.icon) {
      assetUriForEmbed = guild(7853).getAssetUriForEmbed(getGuildIconSource(guild, 128, false));
      let obj2 = guild(7853);
    }
    ...
    const mapped = substr.map((member) => {
      ...
      if (typeof guildMemberAvatarURLSimple !== "SENTRY_RELEASE") {
    ...
    const tmp41 = displayNameStylesEnabled(7851)(closure_2);
    ...
    obj.titleText = guild(4446).computeChannelName(channel2, mergeGuildAvatar, upsertRelationship);
