...
// Dependencies: [17, 4185, 9510, 1415, 1372, 1942, 1862, 3821, 3830, 1874, 4209, 4266, 9511, 676, 6025, 9512, 7730, 7734, 1416, 4032, 4004, 4384, 1236, 9513, 2]
...
export const canShowVoiceChannelListInviteEmbed = function canShowVoiceChannelListInviteEmbed(invite) {
  ...
  if (null != invite.guild) {
    ...
    tmp = null;
    if (null != guild) {
      let channel = importDefault(9512)(invite).channel;
      tmp = null;
      if (null != channel) {
        tmp = null;
...
export const createVoiceChannelListInviteEmbed = function createVoiceChannelListInviteEmbed(invite, closure_2) {
  ...
  if (null != invite.guild) {
    ...
    tmp = null;
    if (null != guild) {
      let channel = displayNameStylesEnabled(9512)(invite).channel;
      tmp = null;
      if (null != channel) {
        tmp = null;
        if (channel.isGuildVocal()) {
  ...
  if (null == tmp) {
    ...
    ({ colors, baseColors } = displayNameStylesEnabled(7730)(closure_2));
    let assetUriForEmbed;
    if (null != guild.icon) {
      assetUriForEmbed = guild(7734).getAssetUriForEmbed(getGuildIconSource(guild, 128, false));
      let obj2 = guild(7734);
    }
    ...
    const mapped = substr.map((member) => {
      ...
      if (typeof guildMemberAvatarURLSimple !== "__REMOTEDEV__") {
    ...
    const tmp41 = displayNameStylesEnabled(7730)(closure_2);
    const embedScrollGradientBackground = guild(4004).getEmbedScrollGradientBackground();
    ...
    obj.headerText = "";
    const obj4 = guild(4004);
    obj.titleText = guild(4384).computeChannelName(channel2, mergeGuildAvatar, upsertRelationship);
