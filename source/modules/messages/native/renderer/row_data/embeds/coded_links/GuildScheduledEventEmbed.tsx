...
// Dependencies: [32, 17, 6879, 1415, 1372, 1862, 3892, 1874, 10878, 1378, 7090, 7835, 7837, 8852, 8842, 8836, 9033, 1236, 7840, 8984, 7827, 712, 9764, 4245, 8839, 4445, 8926, 8847, 12481, 8834, 1411, 2]
// Exports: createGuildScheduledEventInviteEmbed, createGuildScheduledEventLinkEmbed

// Module 12480 (createGuildScheduledEventEmbed)
...
function createGuildScheduledEventEmbed(type) {
  ...
  const colors = importDefault(7835)(theme).colors;
  if (channel != null) {
    const id = channel.id;
  }
  let tmp3;
  ...
  if (null != tmp3) {
    let obj = require(7837) /* frozen */;
    assetUriForEmbed = obj.getAssetUriForEmbed(tmp3);
  }
  let obj1 = require(8852) /* canEveryoneRoleViewEvent */;
  ...
  const eventSchedule = require(8842) /* useEventSchedule */.getEventSchedule(guildEvent, recurrenceId);
  const obj3 = require(8842) /* useEventSchedule */;
  let toISOStringResult;
  if (eventSchedule != null) {
    const startTime = eventSchedule.startTime;
    toISOStringResult = startTime.toISOString();
  }
  const eventTimeData = require(8836) /* getRRule */.getEventTimeData(toISOStringResult);
  let tmp7Result = tmp7(9033);
  ...
  tmp7Result = tmp7(7837);
  const assetUriForEmbed1 = tmp7Result.getAssetUriForEmbed(tmp(7840));
  const obj4 = require(8836) /* getRRule */;
  const tmp13 = entity_type === constants.STAGE_INSTANCE;
  const assetUriForEmbed2 = require(7837) /* frozen */.getAssetUriForEmbed(tmp(8984));
  if (null != guild) {
    if (tmp9) {
    ...
    acceptLabelActiveBackgroundColor = tmp7(7827).processColorOrThrow(tmp(712).unsafe_rawColors.GREEN_360);
    const intl2 = tmp7(1236).intl;
    stringResult2 = intl2.string(tmp7(1236).t.XpeFYr);
    acceptLabelGreenColor = colors.acceptLabelGreenColor;
    flag2 = false;
    const tmp7Result3 = tmp7(7827);
  }
  ...
  const tmp7Result1 = require(7837) /* frozen */;
  let locationFromEvent = require(8839) /* getChannelFromEvent */.getLocationFromEvent(guildEvent);
  if (locationFromEvent == null) {
    let channelName;
    if (null != channel) {
      channelName = tmp7(4445).computeChannelName(channel, user, upsertRelationship);
      const tmp7Result5 = tmp7(4445);
    }
    locationFromEvent = channelName;
  }
  const tmp7Result4 = require(8839) /* getChannelFromEvent */;
  const eventLocationIconSource = require(8926) /* getEventLocationIconSource */.getEventLocationIconSource(guildEvent, channel, tmp16);
  ...
  const tmp7Result6 = require(8926) /* getEventLocationIconSource */;
  const guildEventsForCurrentUser = importDefault(8847).getGuildEventsForCurrentUser(guildEvent.guild_id);
  obj1 = {};
  ...
  const tmpResult1 = importDefault(8847);
  obj1.badgeIcon = require(7837) /* frozen */.getAssetUriForEmbed(importDefault(12481));
  let assetUriForEmbed4;
  if (null != eventLocationIconSource) {
    assetUriForEmbed4 = tmp7(7837).getAssetUriForEmbed(eventLocationIconSource);
    const tmp7Result8 = tmp7(7837);
  }
  ...
  if (null != user) {
    assetUriForEmbed5 = tmp7(7837).getAssetUriForEmbed(user.getAvatarSource(guildEvent.guild_id));
    const tmp7Result9 = tmp7(7837);
  }
  ...
  const tmp7Result7 = require(7837) /* frozen */;
  obj1.headerIcon = require(7837) /* frozen */.getAssetUriForEmbed(icon);
...
export const createGuildScheduledEventInviteEmbed = function createGuildScheduledEventInviteEmbed(invite, closure_2) {
  ...
  if (null == guildScheduledEvent) {
    ...
    if (null != guild_id) {
      const guildEventsForGuild = importDefault(8834).fetchGuildEventsForGuild(guild_id);
      tmp11Result = null;
      const obj4 = importDefault(8834);
    }
  } else {
    if (null != guild_id) {
...
export const createGuildScheduledEventLinkEmbed = function createGuildScheduledEventLinkEmbed(code, closure_2) {
  ...
  if (nextRecurrenceIdInEvent == null) {
    nextRecurrenceIdInEvent = first(8836).getNextRecurrenceIdInEvent(guildScheduledEvent);
    const obj2 = first(8836);
  }
  if (null != guildScheduledEvent) {
    if (!callback2(guildScheduledEvent)) {
      ...
      const guildEventUserCounts = importDefault(8847).getGuildEventUserCounts(first, guildScheduledEvent.id, items1);
      let channel_id;
  ...
  if (!table[first]) {
    const guildEventsForGuild = importDefault(8834).fetchGuildEventsForGuild(first);
