...
export const createGuildProfileInvite = function createGuildProfileInvite(invite, closure_2) {
  ...
  const tmp4Result1 = id(9173);
  let memberCount;
  [tmp11, tmp12] = callback(id(9173).getBackgroundForProfile(closure_2, profilePrimaryColor), 2);
  ...
  if (null != guildProfileFromInvite) {
    guildProfileCTAType = tmp4(11103).getGuildProfileCTAType(guildProfileFromInvite, invite.code);
    const tmp4Result3 = tmp4(11103);
  }
  if (id(11103).CTATypes.IS_MEMBER === guildProfileCTAType) {
    const intl7 = tmp4(1236).intl;
    let stringResult = intl7.string(tmp4(1236).t.IRoQXr);
  } else if (tmp4(11103).CTATypes.HAS_APPLICATION === guildProfileCTAType) {
    const intl6 = tmp4(1236).intl;
    stringResult = intl6.string(tmp4(1236).t["4yfIDk"]);
  } else if (tmp4(11103).CTATypes.APPLY_TO_JOIN === guildProfileCTAType) {
    const intl5 = tmp4(1236).intl;
    stringResult = intl5.string(tmp4(1236).t["7XdMW2"]);
  } else if (tmp4(11103).CTATypes.ACCEPT_ROLES === guildProfileCTAType) {
    const intl4 = tmp4(1236).intl;
    stringResult = intl4.string(tmp4(1236).t.MMlhsr);
  } else {
    if (tmp4(11103).CTATypes.LURK_DISCOVERABLE !== guildProfileCTAType) {
      const JOIN_VIA_INVITE = tmp4(11103).CTATypes.JOIN_VIA_INVITE;
    }
  ...
  if (null != fromGuildProfileResult) {
    guildBadgeImageSource = tmp4(9181).getGuildBadgeImageSource(fromGuildProfileResult, closure_2);
    const tmp4Result4 = tmp4(9181);
  }
  let found;
  if (guildProfileFromInvite != null) {
  ...
  const tmp10 = callback(id(9173).getBackgroundForProfile(closure_2, profilePrimaryColor), 2);
  obj4.bannerColor = id(7855).processColorOrThrow(tmp11);
  const tmp4Result6 = id(7855);
  obj4.bannerColorSecondary = id(7855).processColorOrThrow(tmp12);
  if (found == null) {
    found = [];
  }
  ...
  if (null != guildBadgeImageSource) {
    assetUriForEmbed = tmp4(7865).getAssetUriForEmbed(guildBadgeImageSource);
    const tmp4Result8 = tmp4(7865);
  }
