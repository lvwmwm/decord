...
// Dependencies: [19, 17, 1215, 4395, 7669, 21, 7876, 7821, 11412, 11413, 1363, 11740, 11741, 11742, 7877, 11743, 7823, 4448, 4402, 7090, 7089, 11744, 11411, 2]
// Exports: createExperimentEmbed, default

// Module 11739 (ExperimentOverrideActionSheet)
import getEmbedThemeColorsDefault from "getEmbedThemeColors" /* 7876 */;
import closure_3 from "noop" /* 19 */;
import { Image } from "get ActivityIndicator" /* 17 */;
import closure_5 from "fetchFingerprint" /* 1215 */;
import { ExperimentEmbedType } from "ExperimentBuckets" /* 4395 */;
...
export const createExperimentEmbed = function createExperimentEmbed(url, closure_2) {
  ...
  let obj = experimentTreatmentFromEmbedURL(7821);
  const experimentFromEmbedURL = obj.getExperimentFromEmbedURL(url);
  obj1 = experimentTreatmentFromEmbedURL(7821);
  ...
  const legacyExperiments = experimentTreatmentFromEmbedURL(11412).getLegacyExperiments();
  ({ experiments, overridesInfo } = legacyExperiments);
  const obj3 = experimentTreatmentFromEmbedURL(11412);
  const apexExperiments = experimentTreatmentFromEmbedURL(11413).getApexExperiments();
  let tmp10 = null;
  ...
  if (null != experimentFromEmbedURL) {
    if (null != tmp10) {
      ...
      id = id.getId();
      tmp4Result = tmp4(11742);
      const experimentServerAssignment = tmp4Result.getExperimentServerAssignment(tmp10, id);
      const experimentServerAssignmentLabel = tmp4(7821).getExperimentServerAssignmentLabel(tmp10, experimentServerAssignment);
      if (EXPERIMENT === tmp13.EXPERIMENT_TREATMENT) {
        ...
        obj.thumbnailUrl = tmp4(7877).getAssetUriForEmbed(tmp(11743));
  ...
  const obj4 = experimentTreatmentFromEmbedURL(11413);
