...
// Function ID: 8238
// Name: items
// Dependencies: [17, 4341, 1994, 1922, 4328, 8238, 4493, 1438, 7950, 8239, 8240, 4152, 8242, 687, 1236, 7729, 8244, 7693, 8245, 8247, 8248, 8243, 2]
...
export const transformToRowGeneratedContentInventoryEntryComponent = function transformToRowGeneratedContentInventoryEntryComponent(component) {
  ...
  if (require(8238) /* ContentInventoryEntryType */.ContentInventoryEntryType.PLAYED_GAME !== content_type) {
    if (tmp(8238).ContentInventoryEntryType.TOP_GAME !== content_type) {
      if (tmp(8238).ContentInventoryEntryType.WATCHED_MEDIA === content_type) {
        ...
        if (iconURL == null) {
          iconURL = Image.resolveAssetSource(importDefault(7950)).uri;
        }
        let tmp45 = assetImage;
        if (assetImage == null) {
          tmp45 = iconURL;
        }
        if (tmp45 === Image.resolveAssetSource(importDefault(7950)).uri) {
          let tmp47 = items;
        } else {
          tmpResult = tmp(8239);
          if (tmpResult.hasFetchedColors(tmp45)) {
            const heroColors = tmp(8240).getHeroColors(tmp45);
            ({ primaryColor: primaryColor4, secondaryColor: secondaryColor4 } = heroColors);
            const tmpResult1 = tmp(8240);
            const items1 = [tmp(4152).hexToRgba(primaryColor4), ];
            const tmpResult2 = tmp(4152);
            items1[1] = tmp(4152).hexToRgba(secondaryColor4);
            tmp47 = items1;
            const tmpResult3 = tmp(4152);
          }
        }
        let tmp49;
        if (null != tmp47) {
          ...
          obj1[2] = items2;
          obj1[3] = tmp47;
          obj1[4] = obj28.resolveAssetSource(tmp46(8247)).uri;
          tmp49 = obj1;
          const obj33 = /\w+ (\d+), \w+ (\d+)/;
        }
        let tmp20 = tmp49;
        ...
        tmp38 = ImageSizes;
        tmp46 = importDefault;
      } else if (tmp(8238).ContentInventoryEntryType.TOP_ARTIST === content_type) {
        const image_url2 = contentInventoryEntry.extra.media.image_url;
        let tmp30;
        if (null != image_url2) {
          if (tmpResult4.hasFetchedColors(image_url2)) {
            const heroColors1 = tmp(8240).getHeroColors(image_url2);
            ({ primaryColor: primaryColor3, secondaryColor: secondaryColor3 } = heroColors1);
            const tmpResult5 = tmp(8240);
            const items3 = [tmp(4152).hexToRgba(primaryColor3), ];
            const tmpResult6 = tmp(4152);
            items3[1] = tmp(4152).hexToRgba(secondaryColor3);
            const tmpResult7 = tmp(4152);
            const trait = tmp(8242).getTrait(contentInventoryEntry, tmp(8243).ContentInventoryTraitType.AGGREGATE_COUNT);
            let count;
            if (trait != null) {
              count = trait.count;
            }
            if (null != count) {
              ...
              obj3[0] = Image.resolveAssetSource(importDefault(7729)).uri;
              ...
              obj4[2] = items4;
              obj4[3] = items3;
              obj4[4] = Image.resolveAssetSource(importDefault(8248)).uri;
              tmp30 = obj4;
              const formatToPlainStringResult1 = intl.formatToPlainString(tmp(1236).t.HtifnG, obj2);
            }
            const tmpResult8 = tmp(8242);
          }
          tmpResult4 = tmp(8239);
        }
        tmp20 = tmp30;
      } else if (tmp(8238).ContentInventoryEntryType.LISTENED_SESSION === content_type) {
        ...
        if (null != image_url) {
          if (tmpResult9.hasFetchedColors(image_url)) {
            const heroColors2 = tmp(8240).getHeroColors(image_url);
            ({ primaryColor: primaryColor2, secondaryColor: secondaryColor2 } = heroColors2);
            const tmpResult10 = tmp(8240);
            const items5 = [tmp(4152).hexToRgba(primaryColor2), ];
            const tmpResult11 = tmp(4152);
            items5[1] = tmp(4152).hexToRgba(secondaryColor2);
            ...
            obj5[2] = items6;
            obj5[3] = items5;
            obj5[4] = Image.resolveAssetSource(importDefault(8248)).uri;
            tmp25 = obj5;
            const tmpResult12 = tmp(4152);
          }
          tmpResult9 = tmp(8239);
        }
        tmp20 = tmp25;
      } else if (tmp(8238).ContentInventoryEntryType.LAUNCHED_ACTIVITY === content_type) {
        ...
        if (null != application1) {
          ...
          if (uri == null) {
            uri = Image.resolveAssetSource(importDefault(7950)).uri;
          }
          obj = Image;
          if (uri === Image.resolveAssetSource(importDefault(7950)).uri) {
            let tmp7 = items;
          } else {
            if (tmpResult13.hasFetchedColors(uri)) {
              const heroColors3 = tmp(8240).getHeroColors(uri);
              ({ primaryColor, secondaryColor } = heroColors3);
              const tmpResult14 = tmp(8240);
              const items7 = [tmp(4152).hexToRgba(primaryColor), ];
              const tmpResult15 = tmp(4152);
              items7[1] = tmp(4152).hexToRgba(secondaryColor);
              tmp7 = items7;
              const tmpResult16 = tmp(4152);
            }
            tmpResult13 = tmp(8239);
          }
          if (null != tmp7) {
            const items8 = [];
            const obj7 = { badgeUrl: null };
            obj7[0] = obj.resolveAssetSource(tmp6(7693)).uri;
            const timestamp = message.timestamp;
            const time = timestamp.getTime();
            const obj8 = {};
            const merged = Object.assign(obj7);
            obj8.text = tmp(8242).formatEntryTimestamp(contentInventoryEntry, _getSystemLocale.locale, time);
            const tmpResult18 = tmp(8242);
            const obj9 = { formatSet: null };
            obj9[0] = tmp(8242).A11Y_FORMAT_SET;
    ...
    if (null != user) {
      const name1 = importDefault(4493).getName(undefined, undefined, user);
      ...
      const obj53 = importDefault(4493);
  ...
  if (null != application2) {
    ...
    if (game_name == tmp54) {
      game_name = Image.resolveAssetSource(importDefault(7950)).uri;
    }
    if (game_name === Image.resolveAssetSource(importDefault(7950)).uri) {
      let tmp60 = items;
    } else {
      if (tmpResult20.hasFetchedColors(game_name)) {
        const heroColors4 = tmp(8240).getHeroColors(game_name);
        ({ primaryColor: primaryColor5, secondaryColor: secondaryColor5 } = heroColors4);
        const tmpResult21 = tmp(8240);
        const items9 = [tmp(4152).hexToRgba(primaryColor5), ];
        const tmpResult22 = tmp(4152);
        items9[1] = tmp(4152).hexToRgba(secondaryColor5);
        tmp60 = items9;
        const tmpResult23 = tmp(4152);
      }
      tmpResult20 = tmp(8239);
    }
    if (tmp54 != tmp60) {
      const items10 = [];
      if (tmpResult24.isEntryTopGame(contentInventoryEntry)) {
        const entryDuration = tmp(8242).getEntryDuration(contentInventoryEntry);
