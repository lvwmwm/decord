...
// Function ID: 8389
// Name: items
// Dependencies: [17, 4421, 1996, 1922, 4408, 8389, 4573, 1438, 8098, 8390, 8391, 4228, 8393, 687, 1236, 7872, 8395, 7836, 8396, 8398, 8399, 8394, 2]
// Exports: transformToRowGeneratedContentInventoryEntryComponent

// Module 8388 (items)
import set from "set" /* 2 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import items32 from "items3" /* 4408 */;
import getNicknameDefault from "getNickname" /* 4573 */;
import registerAssetDefault from "registerAsset" /* 7872 */;
import registerAssetDefault2 from "registerAsset" /* 8098 */;
import ContentInventoryEntryType from "ContentInventoryEntryType" /* 8389 */;
import registerAssetDefault3 from "registerAsset" /* 8399 */;
...
export const transformToRowGeneratedContentInventoryEntryComponent = function transformToRowGeneratedContentInventoryEntryComponent(component) {
  ...
  if (ContentInventoryEntryType.ContentInventoryEntryType.PLAYED_GAME !== content_type) {
    if (tmp(8389).ContentInventoryEntryType.TOP_GAME !== content_type) {
      if (tmp(8389).ContentInventoryEntryType.WATCHED_MEDIA === content_type) {
        ...
        if (tmp45 === Image.resolveAssetSource(registerAssetDefault2).uri) {
          ...
          if (tmpResult.hasFetchedColors(tmp45)) {
            const heroColors = tmp(8391).getHeroColors(tmp45);
            ({ primaryColor: primaryColor4, secondaryColor: secondaryColor4 } = heroColors);
            const tmpResult1 = tmp(8391);
            const items1 = [tmp(4228).hexToRgba(primaryColor4), ];
            const tmpResult2 = tmp(4228);
            items1[1] = tmp(4228).hexToRgba(secondaryColor4);
            tmp47 = items1;
            const tmpResult3 = tmp(4228);
          }
        }
        let tmp49;
        if (null != tmp47) {
          ...
          obj1[2] = items2;
          obj1[3] = tmp47;
          obj1[4] = obj28.resolveAssetSource(tmp46(8398)).uri;
          tmp49 = obj1;
          const obj33 = /\w+ (\d+), \w+ (\d+)/;
        }
        let tmp20 = tmp49;
        ...
        tmp38 = ImageSizes;
        tmp46 = importDefault;
      } else if (tmp(8389).ContentInventoryEntryType.TOP_ARTIST === content_type) {
        const image_url2 = contentInventoryEntry.extra.media.image_url;
        let tmp30;
        if (null != image_url2) {
          if (tmpResult4.hasFetchedColors(image_url2)) {
            const heroColors1 = tmp(8391).getHeroColors(image_url2);
            ({ primaryColor: primaryColor3, secondaryColor: secondaryColor3 } = heroColors1);
            const tmpResult5 = tmp(8391);
            const items3 = [tmp(4228).hexToRgba(primaryColor3), ];
            const tmpResult6 = tmp(4228);
            items3[1] = tmp(4228).hexToRgba(secondaryColor3);
            const tmpResult7 = tmp(4228);
            const trait = tmp(8393).getTrait(contentInventoryEntry, tmp(8394).ContentInventoryTraitType.AGGREGATE_COUNT);
            let count;
            if (trait != null) {
              count = trait.count;
            }
            ...
            const tmpResult8 = tmp(8393);
          }
          tmpResult4 = tmp(8390);
        }
        tmp20 = tmp30;
      } else if (tmp(8389).ContentInventoryEntryType.LISTENED_SESSION === content_type) {
        ...
        if (null != image_url) {
          if (tmpResult9.hasFetchedColors(image_url)) {
            const heroColors2 = tmp(8391).getHeroColors(image_url);
            ({ primaryColor: primaryColor2, secondaryColor: secondaryColor2 } = heroColors2);
            const tmpResult10 = tmp(8391);
            const items5 = [tmp(4228).hexToRgba(primaryColor2), ];
            const tmpResult11 = tmp(4228);
            items5[1] = tmp(4228).hexToRgba(secondaryColor2);
            ...
            tmp25 = obj5;
            const tmpResult12 = tmp(4228);
          }
          tmpResult9 = tmp(8390);
        }
        tmp20 = tmp25;
      } else if (tmp(8389).ContentInventoryEntryType.LAUNCHED_ACTIVITY === content_type) {
        ...
        if (null != application1) {
          ...
          if (uri === Image.resolveAssetSource(registerAssetDefault2).uri) {
            ...
            if (tmpResult13.hasFetchedColors(uri)) {
              const heroColors3 = tmp(8391).getHeroColors(uri);
              ({ primaryColor, secondaryColor } = heroColors3);
              const tmpResult14 = tmp(8391);
              const items7 = [tmp(4228).hexToRgba(primaryColor), ];
              const tmpResult15 = tmp(4228);
              items7[1] = tmp(4228).hexToRgba(secondaryColor);
              tmp7 = items7;
              const tmpResult16 = tmp(4228);
            }
            tmpResult13 = tmp(8390);
          }
          if (null != tmp7) {
            const items8 = [];
            const obj7 = { badgeUrl: null };
            obj7[0] = obj.resolveAssetSource(tmp6(7836)).uri;
            const timestamp = message.timestamp;
            const time = timestamp.getTime();
            const obj8 = {};
            const merged = Object.assign(obj7);
            obj8.text = tmp(8393).formatEntryTimestamp(contentInventoryEntry, closure_5.locale, time);
            const tmpResult18 = tmp(8393);
            const obj9 = { formatSet: null };
            obj9[0] = tmp(8393).A11Y_FORMAT_SET;
  ...
  if (null != application2) {
    ...
    if (game_name === Image.resolveAssetSource(registerAssetDefault2).uri) {
      ...
      if (tmpResult20.hasFetchedColors(game_name)) {
        const heroColors4 = tmp(8391).getHeroColors(game_name);
        ({ primaryColor: primaryColor5, secondaryColor: secondaryColor5 } = heroColors4);
        const tmpResult21 = tmp(8391);
        const items9 = [tmp(4228).hexToRgba(primaryColor5), ];
        const tmpResult22 = tmp(4228);
        items9[1] = tmp(4228).hexToRgba(secondaryColor5);
        tmp60 = items9;
        const tmpResult23 = tmp(4228);
      }
      tmpResult20 = tmp(8390);
    }
    if (tmp54 != tmp60) {
      const items10 = [];
      if (tmpResult24.isEntryTopGame(contentInventoryEntry)) {
        const entryDuration = tmp(8393).getEntryDuration(contentInventoryEntry);
