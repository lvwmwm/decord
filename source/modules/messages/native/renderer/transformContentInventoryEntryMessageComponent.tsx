...
function getMissingIconURL() {
  return Image.resolveAssetSource(importDefault(7849)).uri;
}
function getGradientColors(game_name) {
  ...
  if (game_name === getMissingIconURL()) {
    ...
    if (obj4.hasFetchedColors(game_name)) {
      const heroColors = require(7851) /* useHeroColors */.getHeroColors(game_name);
      ({ primaryColor, secondaryColor } = heroColors);
      const obj = require(7851) /* useHeroColors */;
      items = [require(3975) /* hexToRgb */.hexToRgba(primaryColor), ];
      const obj2 = require(3975) /* hexToRgb */;
      items[1] = require(3975) /* hexToRgb */.hexToRgba(secondaryColor);
      return items;
    }
    obj4 = require(7850) /* hasFetchedColors */;
  }
}
function buildTimestampBadge(contentInventoryEntry, message, arg2) {
  ...
  obj["text"] = require(7853) /* calculateTimestampDurations */.formatEntryTimestamp(contentInventoryEntry, closure_5.locale, time);
  const obj2 = require(7853) /* calculateTimestampDurations */;
  obj = { formatSet: require(7853) /* calculateTimestampDurations */.A11Y_FORMAT_SET };
  obj["ariaDescription"] = require(7853) /* calculateTimestampDurations */.formatEntryTimestamp(contentInventoryEntry, closure_5.locale, time, obj);
  return obj;
}
...
export const transformToRowGeneratedContentInventoryEntryComponent = function transformToRowGeneratedContentInventoryEntryComponent(component) {
  ...
  if (require(7848) /* ContentInventoryEntryType */.ContentInventoryEntryType.PLAYED_GAME !== content_type) {
    let formatToPlainStringResult = require;
    formatToPlainStringResult = dependencyMap;
    if (require(7848) /* ContentInventoryEntryType */.ContentInventoryEntryType.TOP_GAME !== content_type) {
      formatToPlainStringResult = require;
      formatToPlainStringResult = dependencyMap;
      if (require(7848) /* ContentInventoryEntryType */.ContentInventoryEntryType.WATCHED_MEDIA === content_type) {
        const items = [, ];
        ({ LARGE: arr6[0], LARGE: arr6[1] } = ImageSizes);
        const assetImage = require(7856) /* _createForOfIteratorHelperLoose */.getAssetImage(contentInventoryEntry.extra.application_id, contentInventoryEntry.extra.media_assets_large_image, items);
        ...
        if (null != tmp52) {
          ...
          const obj1 = { imageUrl: iconURL, title: contentInventoryEntry.extra.media_title, subtitles: items1, gradientColors: tmp52, platformIconUrl: Image.resolveAssetSource(importDefault(7858)).uri };
          tmp54 = obj1;
          const obj19 = /\w+ (\d+), \w+ (\d+)/;
        }
        let tmp13 = tmp54;
        const obj17 = require(7856) /* _createForOfIteratorHelperLoose */;
      } else {
        formatToPlainStringResult = require;
        formatToPlainStringResult = dependencyMap;
        if (require(7848) /* ContentInventoryEntryType */.ContentInventoryEntryType.TOP_ARTIST === content_type) {
          const image_url2 = contentInventoryEntry.extra.media.image_url;
          let tmp27;
          if (null != image_url2) {
            let obj8 = require(7850) /* hasFetchedColors */;
            if (obj8.hasFetchedColors(image_url2)) {
              let obj9 = require(7851) /* useHeroColors */;
              const heroColors = obj9.getHeroColors(image_url2);
              ({ primaryColor: primaryColor2, secondaryColor: secondaryColor2 } = heroColors);
              let obj10 = require(3975) /* hexToRgb */;
              const items2 = [obj10.hexToRgba(primaryColor2), ];
              let obj11 = require(3975) /* hexToRgb */;
              items2[1] = obj11.hexToRgba(secondaryColor2);
              const items3 = [];
              let obj12 = require(7853) /* calculateTimestampDurations */;
              const trait = obj12.getTrait(contentInventoryEntry, require(7854) /* ContentInventoryTraitType */.ContentInventoryTraitType.AGGREGATE_COUNT);
              let count;
              if (null != trait) {
                count = trait.count;
              }
              if (null != count) {
                ...
                let obj2 = { count };
                let obj3 = { badgeUrl: Image.resolveAssetSource(importDefault(6593)).uri, text: intl.formatToPlainString(require(1212) /* getSystemLocale */.t.HtifnG, obj2) };
                items3.push(obj3);
                let obj4 = { imageUrl: image_url2, title: contentInventoryEntry.extra.artist.name, subtitles: items3, gradientColors: items2, platformIconUrl: Image.resolveAssetSource(importDefault(7859)).uri };
                tmp27 = obj4;
          ...
          formatToPlainStringResult = require;
          formatToPlainStringResult = dependencyMap;
          if (require(7848) /* ContentInventoryEntryType */.ContentInventoryEntryType.LISTENED_SESSION === content_type) {
            ...
            if (null != image_url) {
              ...
              if (obj2.hasFetchedColors(image_url)) {
                ...
                obj5.platformIconUrl = Image.resolveAssetSource(importDefault(7859)).uri;
                tmp18 = obj5;
              }
            }
            tmp13 = tmp18;
            ...
            formatToPlainStringResult = require;
            formatToPlainStringResult = dependencyMap;
            if (require(7848) /* ContentInventoryEntryType */.ContentInventoryEntryType.LAUNCHED_ACTIVITY === content_type) {
              ...
              if (null != application1) {
                ...
                if (null != tmp5) {
                  const items6 = [];
                  obj = { badgeUrl: Image.resolveAssetSource(importDefault(6557)).uri };
    ...
    if (null != user) {
      const name1 = importDefault(4320).getName(undefined, undefined, user);
      obj8 = { authorId: contentInventoryEntry.author_id };
      const obj30 = importDefault(4320);
    ...
    if (obj14 != tmp66) {
      ...
      if (obj24.isEntryTopGame(contentInventoryEntry)) {
        ...
        if (obj14 != entryDuration) {
          ...
          obj12.badgeUrl = Image.resolveAssetSource(importDefault(6593)).uri;
          obj12.text = combined;
          items7.push(obj12);
        ...
        const obj13 = { badgeUrl: Image.resolveAssetSource(importDefault(7855)).uri };
      ...
      obj24 = require(7853) /* calculateTimestampDurations */;
    }
  }
  tmp13 = tmp62;
};
