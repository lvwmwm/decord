  ...
  if (flag) {
    if ("message_swipe" === source) {
      if (editing.isEditing(isForumPost.id, id.id)) {
        ...
        importDefault(6917).endEditMessage(isForumPost.id);
        if (current != null) {
        ...
        const tmp11Result = importDefault(6917);
      }
    }
  }
...
export const handleCreateThread = function handleCreateThread(guild_id, id, Message) {
  ...
  const result = importDefault(7224).openThreadCreationForMobile(guild_id, id, str);
  let result1 = null == id;
  if (!result1) {
