...
// Dependencies: [5, 32, 19, 8674, 1990, 3989, 4357, 4391, 8108, 676, 21, 9, 10995, 8673, 10977, 8671, 8107, 10998, 10429, 12, 11, 11002, 10438, 10978, 11004, 11005, 6986, 11006, 11424, 4274, 1208, 11000, 10976, 643, 4331, 7262, 11430, 11360, 11010, 500, 1626, 5226, 11431, 2]

// Module 10994 (handleTapShowAltText)
...
function handleTapShowAltText(description) {
  importDefault(10995)({ description: description.nativeEvent.description });
}
function handleMediaPlayFinishedAnalytics(nativeEvent) {
  const result = require(8673) /* getVisibleMessages */.handleMediaPlayFinishedAnalytics(nativeEvent.nativeEvent);
}
...
const forwardRefResult = importAllResult.forwardRef((messages, ref) => {
  ...
  const items4 = [
    ...
    findMessageIndex(_require(11431).ChatTTITracker, { messages: messages.messages })
  ];
  obj3[0] = items4;
