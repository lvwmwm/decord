...
// Dependencies: [5, 32, 19, 17, 5096, 4411, 1874, 21, 4193, 712, 4004, 3897, 4037, 3894, 11438, 11439, 5718, 1236, 589, 686, 4032, 11440, 9632, 4054, 4194, 1297, 9631, 4615, 4721, 9647, 4189, 4604, 2]
// Exports: default
...
export default function DMMessageStickerGreeting(channel) {
  ...
  const showConvoStarterInDM = Text(11440).useShowConvoStarterInDM(channel);
  callback = showConvoStarterInDM;
  const TextResult = Text(11440);
  const tmp2 = END;
  ...
  obj = { styles: toastContent, isRendered: tmp17, hasInputText, hasMessages: stateFromStores, HEIGHT_COMPACT: 72, HEIGHT_FULL: 180, withDelay: Text(4054).withDelay, withTiming: Text(4194).withTiming, STANDARD_EASING: Text(1297).STANDARD_EASING };
  fn.__closure = obj;
  fn.__workletHash = 6327401707106;
  fn.__initData = closure_16;
  const animatedStyle = Text(4054).useAnimatedStyle(fn);
  const TextResult2 = Text(4054);
  let BACKGROUND_BASE_LOWER = Text(4004).useGradientValue(tmp2);
  const TextResult3 = Text(4004);
  if (BACKGROUND_BASE_LOWER == null) {
    BACKGROUND_BASE_LOWER = tmp5(712).colors.BACKGROUND_BASE_LOWER;
  }
  const token = Text(3897).useToken(BACKGROUND_BASE_LOWER);
  const TextResult4 = Text(3897);
  const items8 = [Text(4037).hexWithOpacity(token, 0), token];
  const TextResult5 = Text(4037);
  const shouldAnimateSticker = Text(9631).useShouldAnimateSticker(false);
  if (!showConvoStarterInDM) {
    ...
    obj1[1] = items8;
    const items9 = [callback2(tmp5(4615), obj1), ];
    ...
    if (stateFromStores) {
      ...
      callback[1] = tmp;
      obj3[4] = callback;
      let tmp23Result = tmp23(Text(4721).PressableOpacity, obj3);
    } else {
      tmp24Result = null;
      if (null != stateFromStores2) {
      ...
      obj9[1] = callback;
      items10[1] = tmp24(Text(4604).Button, obj9);
      obj8[0] = items10;
    ...
    items9[1] = obj2;
    obj[2] = items9;
    tmp23Result = tmp23(tmp5(4054).View, obj);
  }
};
