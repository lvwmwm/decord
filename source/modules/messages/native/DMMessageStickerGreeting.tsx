...
// Dependencies: [5, 32, 19, 17, 5396, 4688, 1922, 21, 4445, 712, 4264, 4165, 4293, 4162, 11748, 11749, 7135, 1236, 589, 686, 4289, 11750, 10299, 4185, 4446, 1297, 10298, 4892, 5020, 10181, 4441, 4880, 2]
// Exports: default

// Module 11747 (DMMessageStickerGreeting)
import ThemesDefault from "Themes" /* 712 */;
import closure_3 from "asyncGeneratorStep" /* 5 */;
import closure_4 from "_slicedToArray" /* 32 */;
import closure_5 from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import closure_7 from "loadSavedGuildStickers" /* 5396 */;
...
export default function DMMessageStickerGreeting(channel) {
  ...
  const showConvoStarterInDM = Text(11750).useShowConvoStarterInDM(channel);
  callback = showConvoStarterInDM;
  const TextResult = Text(11750);
  const tmp2 = END;
  ...
  obj = { styles: toastContent, isRendered: tmp17, hasInputText, hasMessages: stateFromStores, HEIGHT_COMPACT: 72, HEIGHT_FULL: 180, withDelay: Text(4185).withDelay, withTiming: Text(4446).withTiming, STANDARD_EASING: Text(1297).STANDARD_EASING };
  fn.__closure = obj;
  fn.__workletHash = 6327401707106;
  fn.__initData = closure_16;
  const animatedStyle = Text(4185).useAnimatedStyle(fn);
  const TextResult2 = Text(4185);
  let BACKGROUND_BASE_LOWER = Text(4264).useGradientValue(tmp2);
  const TextResult3 = Text(4264);
  ...
  const items8 = [Text(4293).hexWithOpacity(token, 0), token];
  const TextResult5 = Text(4293);
  const shouldAnimateSticker = Text(10298).useShouldAnimateSticker(false);
  if (!showConvoStarterInDM) {
    ...
    obj1[1] = items8;
    const items9 = [callback2(tmp5(4892), obj1), ];
    ...
    if (stateFromStores) {
      ...
      callback[1] = tmp;
      obj3[4] = callback;
      let tmp23Result = tmp23(Text(5020).PressableOpacity, obj3);
    } else {
      tmp24Result = null;
      if (null != stateFromStores2) {
      ...
      obj9[1] = callback;
      items10[1] = tmp24(Text(4880).Button, obj9);
      obj8[0] = items10;
    ...
    items9[1] = obj2;
    obj[2] = items9;
    tmp23Result = tmp23(tmp5(4185).View, obj);
  }
};
