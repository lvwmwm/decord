...
// Dependencies: [5, 32, 19, 17, 5158, 4473, 1874, 21, 4255, 712, 4066, 3959, 4099, 3956, 11548, 11549, 6814, 1236, 589, 686, 4094, 11550, 9780, 4116, 4256, 1297, 9779, 4677, 4783, 9794, 4251, 4666, 2]
// Exports: default

// Module 11547 (DMMessageStickerGreeting)
...
export default function DMMessageStickerGreeting(channel) {
  ...
  const showConvoStarterInDM = Text(11550).useShowConvoStarterInDM(channel);
  callback = showConvoStarterInDM;
  const TextResult = Text(11550);
  const tmp2 = END;
  ...
  const shouldAnimateSticker = Text(9779).useShouldAnimateSticker(false);
  if (!showConvoStarterInDM) {
    ...
    obj1[1] = items8;
    const items9 = [callback2(tmp5(4677), obj1), ];
    ...
    if (stateFromStores) {
      ...
      callback[1] = tmp;
      obj3[4] = callback;
      let tmp23Result = tmp23(Text(4783).PressableOpacity, obj3);
    } else {
      tmp24Result = null;
      if (null != stateFromStores2) {
      ...
      obj9[1] = callback;
      items10[1] = tmp24(Text(4666).Button, obj9);
      obj8[0] = items10;
