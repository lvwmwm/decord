...
// Dependencies: [5, 32, 19, 17, 5280, 4584, 1922, 21, 4342, 712, 4162, 4065, 4191, 4062, 11710, 11711, 6986, 1236, 589, 686, 4187, 11712, 10013, 4083, 4343, 1297, 10012, 4788, 4909, 9913, 4338, 4777, 2]
// Exports: default

// Module 11709 (DMMessageStickerGreeting)
...
export default function DMMessageStickerGreeting(channel) {
  ...
  const showConvoStarterInDM = Text(11712).useShowConvoStarterInDM(channel);
  callback = showConvoStarterInDM;
  const TextResult = Text(11712);
  const tmp2 = END;
  ...
  obj = { styles: toastContent, isRendered: tmp17, hasInputText, hasMessages: stateFromStores, HEIGHT_COMPACT: 72, HEIGHT_FULL: 180, withDelay: Text(4083).withDelay, withTiming: Text(4343).withTiming, STANDARD_EASING: Text(1297).STANDARD_EASING };
  fn.__closure = obj;
  fn.__workletHash = 6327401707106;
  fn.__initData = closure_16;
  const animatedStyle = Text(4083).useAnimatedStyle(fn);
  const TextResult2 = Text(4083);
  let BACKGROUND_BASE_LOWER = Text(4162).useGradientValue(tmp2);
  const TextResult3 = Text(4162);
  ...
  const items8 = [Text(4191).hexWithOpacity(token, 0), token];
  const TextResult5 = Text(4191);
  const shouldAnimateSticker = Text(10012).useShouldAnimateSticker(false);
  if (!showConvoStarterInDM) {
    ...
    obj1[1] = items8;
    const items9 = [callback2(tmp5(4788), obj1), ];
    ...
    if (stateFromStores) {
      ...
      callback[1] = tmp;
      obj3[4] = callback;
      let tmp23Result = tmp23(Text(4909).PressableOpacity, obj3);
    } else {
      tmp24Result = null;
      if (null != stateFromStores2) {
      ...
      obj9[1] = callback;
      items10[1] = tmp24(Text(4777).Button, obj9);
      obj8[0] = items10;
