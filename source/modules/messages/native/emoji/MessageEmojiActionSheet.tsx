...
// Dependencies: [19, 17, 676, 21, 4255, 500, 514, 5323, 698, 8629, 8635, 8636, 2]
// Exports: default

// Module 8628 (MessageStandardEmojiActionSheet)
import "noop";
...
function MessageStandardEmojiActionSheet(emojiNode) {
  ...
  obj[1] = jsx(importDefault(8629), { emojiNode: emojiNode.emojiNode, nonce: v4Result });
  obj[2] = <View style={callback().contentWrapper}>{null}</View>;
  return jsx(_require(5323).BottomSheet, { style: callback().contentWrapper, children: null });
}
function MessageCustomEmojiActionSheet(emojiNode) {
  emojiNode = emojiNode.emojiNode;
  let _require;
  let obj = _require(8635);
  ...
  if (emojiAndSource.isFetching) {
    ...
    obj2[5] = tmp8;
    obj2[6] = v4Result;
    obj1[1] = jsx(importDefault(8636), { emojiNode: null, sourceType: null, expressionSourceApplication: null, expressionSourceGuild: null, customEmojiFromJoinedGuild: null, hasJoinedEmojiSourceGuild: null, nonce: null });
    obj[2] = <View style={null}>{null}</View>;
    return jsx(tmp2(5323).BottomSheet, { startExpanded: true, onDismiss: null, children: null });
  }
  tmp = callback();
}
let num = 0;
