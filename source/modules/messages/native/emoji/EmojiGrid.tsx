...
// Function ID: 8801
// Name: Emoji
// Dependencies: [19, 17, 21, 4342, 712, 4006, 1435, 5400, 8785, 8801, 8803, 2]
// Exports: EmojiGrid
...
function Emoji(guildEmoji) {
  ...
  if (null == guildEmoji.id) {
    let tmp3Result = tmp3(4006);
    let uRL = tmp3Result.getURL(guildEmoji.name);
  } else {
    tmp3Result = tmp3(1435);
  ...
  return jsx(importDefault(5400), { src: uRL, fastImageStyle: tmp.gridEmojiFastImage, textEmojiStyle: tmp.gridEmojiText, name: guildEmoji.name }, guildEmoji.id);
}
...
export const EmojiGrid = function EmojiGrid(numberToShow) {
  ...
  let obj;
  obj = {};
  const merged = Object.assign(doNotDisplayEmojiIds(8785).useSharedMessageEmojiStyles());
  ...
  obj[1] = jsx(tmp(8803).GappedList, { gap: 8, children: null });
