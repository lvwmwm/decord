...
// Dependencies: [653, 664, 5904, 21, 3835, 2]
// Exports: isNewGroupItem

// Module 9479 (isNewMessageGroup)
import ME from "ME";
...
function isNewMessageGroup(isForumPost, content, hasFlag) {
  ...
  if (tmp) {
    if (content.blocked !== type.blocked || content.ignored !== type.ignored) {
      ...
      let tmp35 = !importDefault(5904)(content);
      if (!tmp35) {
        type = type.type;
      ...
      const tmp40 = !importDefault(5904)(content);
      tmp32 = !tmp40;
      if (tmp40) {
        ...
        if (!tmp5) {
          ...
          if (!tmp9) {
            ...
            if (!tmp11) {
              ...
              let tmp15 = !tmp12;
              if (tmp12) {
                let tmp18 = !require(3835) /* resetCache */.isSameDay(content.timestamp, type.timestamp);
                if (!tmp18) {
                  let tmp22 = !require(3835) /* resetCache */.isWithinInterval(content.timestamp, type.timestamp, closure_6);
                  if (!tmp22) {
