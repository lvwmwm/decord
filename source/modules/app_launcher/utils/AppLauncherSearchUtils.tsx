...
// Dependencies: [32, 19, 8496, 8494, 8495, 11651, 4838, 4839, 8499, 1955, 8493, 9688, 11622, 11653, 12, 9699, 589, 9691, 11652, 11654, 2]
...
// Module 11650 (useApplicationsInContext)
import getShelfBadgeTypeIfActive from "getShelfBadgeTypeIfActive" /* 8493 */;
import closure_4 from "_slicedToArray" /* 32 */;
import closure_5 from "noop" /* 19 */;
import closure_6 from "handleUserSettingsProtoStoreChange" /* 8496 */;
import getIndexKey from "getIndexKey" /* 8494 */;
import closure_11 from "handleUserSettingsProtoStoreChange" /* 8495 */;
import closure_12 from "getSearchResults" /* 11651 */;
import { FetchState } from "getSearchResults" /* 11651 */;
...
function useApplicationsInContext(allowFetch) {
  ...
  if (includeBuiltIn) {
    items2.push(onlyWithCommands(8493).FAKE_BUILT_IN_APP);
  }
...
export const filterApplicationAllowed = function filterApplicationAllowed(type) {
  ...
  closure_1 = _require(8499).buildPermissionContext(channel, items);
  return (id) => {
...
export const useLocalSearchResults = function useLocalSearchResults(context) {
  ...
  memo1 = obj2.useMemo(() => {
    if (flag) {
      ...
      closure_1 = context2(8499).buildPermissionContext(channel, items);
      closure_2 = {};
      const items1 = [
        (applicationId) => {
            ({ context, userId, roleIds, isImpersonating } = closure_1);
  ...
  const memo3 = obj2.useMemo(() => {
    ...
    closure_1 = context2(8499).buildPermissionContext(channel, items3);
    const items4 = [
      (id) => {
        ({ context, userId, roleIds, isImpersonating } = closure_1);
        const tmp = closure_1_8(descriptor, id.id);
...
export const useGlobalSearchResults = function useGlobalSearchResults(fetches) {
  ...
  const tmp5 = fetches.entrypoint === substr(9699).AppLauncherEntrypoint.VOICE;
  dependencyMap = tmp5;
  guild_id = undefined;
  if ("channel" === context.type) {
    guild_id = context.channel.guild_id;
  ...
  const stateFromStoresObject = substr(589).useStateFromStoresObject(items, () => {
    ...
    obj = { query: substr, guildId: guild_id, page: first, integrationType: substr(9691).ApplicationIntegrationType.USER_INSTALL, minUserInstallCommandCount: 1, excludeAppsWithCustomInstallUrl: true, excludeNonEmbeddedApps: dependencyMap, excludeEmbeddedAppsWithoutPrimaryEntryPointAppCommand: true, source: substr(11652).SearchAppsRequestSource.APP_LAUNCHER };
  ...
  callback1 = first.useCallback((arg0) => {
    ...
    obj = { page, integrationType: substr(9691).ApplicationIntegrationType.USER_INSTALL, minUserInstallCommandCount: 1, excludeAppsWithCustomInstallUrl: true, excludeNonEmbeddedApps: dependencyMap, excludeEmbeddedAppsWithoutPrimaryEntryPointAppCommand: true, source: substr(11652).SearchAppsRequestSource.APP_LAUNCHER };
    obj[2] = obj;
    obj.search(obj);
  }, items4);
