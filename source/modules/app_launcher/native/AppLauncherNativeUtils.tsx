...
// Dependencies: [19, 1391, 1922, 1499, 676, 4904, 4653, 7201, 11541, 7199, 1955, 7455, 1902, 8077, 1435, 8971, 9089, 11546, 5911, 9374, 4413, 2]
...
// Module 11540 (handleApplicationSelected)
...
import Placeholder from "Placeholder" /* 11541 */;
...
export const handleApplicationSelected = function handleApplicationSelected(entrypoint) {
  ...
  if (application.id === BuiltInSectionId.BUILT_IN) {
    let APP = tmp(7201).ApplicationCommandTriggerSections.BUILT_IN;
  } else {
    APP = tmp(7201).ApplicationCommandTriggerSections.APP;
  }
  obj[1] = APP;
...
export const handleApplicationCommandSelected = function handleApplicationCommandSelected(arg0) {
  ...
  let obj = command(7199);
  obj = { command, location: _location, triggerSection: command(7199).getCommandTriggerSection(section), queryLength: query.length, sectionName, query, searchResultsPosition, source: entrypoint };
...
export const useLogAppLauncherEmptyStateView = function useLogAppLauncherEmptyStateView(COMMAND_NOT_FOUND, query) {
  ...
  const entrypoint = _require(8971).useAppLauncherContext().entrypoint;
