...
// Dependencies: [19, 1372, 1874, 1479, 676, 4689, 4450, 6879, 11351, 6877, 1906, 7145, 1855, 8137, 1416, 11060, 9565, 11356, 5587, 10688, 4224, 2]
...
// Module 11350 (handleApplicationSelected)
...
export const handleApplicationSelected = function handleApplicationSelected(entrypoint) {
  ...
  if (application.id === BuiltInSectionId.BUILT_IN) {
    let APP = tmp(6879).ApplicationCommandTriggerSections.BUILT_IN;
  } else {
    APP = tmp(6879).ApplicationCommandTriggerSections.APP;
  }
  obj[1] = APP;
...
export const handleApplicationCommandSelected = function handleApplicationCommandSelected(arg0) {
  ...
  obj[2] = command(6877).getCommandTriggerSection(section);
  obj[3] = query.length;
  obj[4] = sectionName;
  obj[5] = query;
  ...
  if (command.type === command(1906).ApplicationCommandType.PRIMARY_ENTRY_POINT) {
    ...
    if ("channel" === context.type) {
      const result = importAll(7145).setAppLauncherActiveCommand(context.channel.id, command);
...
export const getAppLauncherIconSource = function getAppLauncherIconSource(application) {
  if (null == application) {
    ...
    let obj = require(8137) /* getShelfBadgeTypeIfActive */;
...
export const useLogAppLauncherEmptyStateView = function useLogAppLauncherEmptyStateView(COMMAND_NOT_FOUND, query) {
  ...
  const entrypoint = _require(11060).useAppLauncherContext().entrypoint;
