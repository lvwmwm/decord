...
// Function ID: 11602
// Name: Tag
// Dependencies: [5, 32, 19, 17, 7654, 4368, 676, 21, 712, 4342, 4338, 9611, 11602, 11517, 11511, 7052, 8408, 1236, 698, 9612, 4777, 4355, 10964, 10607, 5789, 5809, 4912, 589, 5393, 5829, 10965, 7856, 5795, 11603, 11604, 11605, 1297, 11543, 10503, 4880, 11607, 2]
// Exports: default
...
function TryItOutButton(botUserId) {
  ...
  callback = React.useCallback(callback(function*() {
    ...
    obj1[0] = outer1_1;
    obj1[1] = outer1_0(9612).EntryPointCommandButtonActions.OPEN_APP_DM;
    ...
    obj2[0] = outer1_0;
    yield outer1_1(4355).openPrivateChannel(obj2);
    if (1 === tmp7) {
      dependencyMap = 0;
  ...
  obj[4] = callback;
  return callback3(botUserId(4777).Button, obj);
}
...
export default function ActivityDetailContent(application) {
  ...
  const tmp = createCacheKey();
  let obj = application(9611);
  const width = obj.useRequiredAppLauncherContext().width;
  let obj1 = application(10607);
  const getPrimaryAppCommand = obj1.useGetPrimaryAppCommand(context, application.id);
  const analyticsLocations = importDefault(5789)(importDefault(5809).APP_DETAIL).analyticsLocations;
  let obj2 = application(4912);
  const isScreenLandscape = obj2.useIsScreenLandscape();
  let detailsContainerLandscape = entrypoint !== application(9612).AppLauncherEntrypoint.VOICE && isScreenLandscape;
  ...
  if (tmp2Result.isRealApplication(application)) {
    ...
    if (isDeveloperOfThisApp) {
      ...
      obj[1] = num2;
      obj[0] = obj;
      const items1 = [callback3(tmp2(5393).TableRowDivider, {}), , ];
      ...
      obj1[2] = importAll(10965).toggleUseActivityUrlOverride;
      items1[1] = callback3(tmp2(5829).TableSwitchRow, obj1);
      ...
      if (useActivityUrlOverride) {
        ...
        obj3[2] = tmp20(10965).setActivityUrlOverride;
        obj2[1] = tmp19(tmp2(7856).TextInput, obj3);
    ...
    const getOrFetchApplication = tmp2(5795).useGetOrFetchApplication(application.id);
    let bot;
    if (getOrFetchApplication != null) {
      bot = getOrFetchApplication.bot;
    }
    const tmp2Result1 = tmp2(5795);
    const obj4 = { context: null, applicationId: null };
    obj4[0] = context;
    obj4[1] = application.id;
    const activityAction = tmp2(11517).useActivityAction(obj4);
    const tmp2Result2 = tmp2(11517);
    const delayedSwapToActivityActionLeave = tmp2(11603).useDelayedSwapToActivityActionLeave(activityAction);
    const obj5 = { context: null, application: null, activityAction: null };
    obj5[0] = context;
    obj5[1] = application;
    obj5[2] = delayedSwapToActivityActionLeave;
    const tmp2Result3 = tmp2(11603);
    ({ reason, disabled } = tmp5(11604)(obj5));
    const obj6 = { context: null, application: null, botUserId: null };
    obj6[0] = context;
    obj6[1] = application;
    let id;
    let tmp5Result = tmp5(11605);
    if (bot != null) {
      id = bot.id;
    }
    obj6[2] = id;
    if (null != getPrimaryAppCommand) {
      ...
      if (tmp36Result) {
        ...
        obj12[2] = reason;
        tmp36Result = tmp36(tmp2(4338).Text, obj12);
      }
      ...
      if (isDeveloperOfThisApp) {
        isDeveloperOfThisApp = tmp2(8408).isEmbeddedApp(application);
        const tmp2Result4 = tmp2(8408);
      }
      if (isDeveloperOfThisApp) {
    ...
    obj22[0] = application;
    const items7 = [callback3(tmp5(10503), obj22), , , , ];
    ...
    obj25[0] = tmp.tagIcon;
    obj24[0] = callback3(tmp2(4880).GroupIcon, obj25);
    const tmp26 = tmp5(11604)(obj5);
    const tmp49 = Tag;
    let num5 = num;
    if (num == null) {
      num5 = 0;
    }
    obj24[1] = tmp2(11607).getItemSubtitleForMaxPlayersShort(num5);
    const tmp2Result5 = tmp2(11607);
    if (num == null) {
      num = 0;
    }
    obj24[2] = tmp5(11607)(num);
