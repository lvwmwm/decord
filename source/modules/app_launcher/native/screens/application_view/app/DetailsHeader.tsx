...
const memoResult = importAllResult.memo(function DetailsHeader(viewContainerStyle) {
  ...
  sharedValue = undefined;
  let obj2 = sharedValue(4050);
  sharedValue = obj2.useSharedValue(null);
  let obj3 = sharedValue(4050);
  let fn = function t() {
  ...
  let tmp27 = first;
  const tmp2Result = sharedValue(4720);
  if (first) {
    tmp27 = !first1;
  }
  ...
  if (!hideName) {
    ...
    obj2[0] = tmp.nameText;
    obj2[4] = sectionName;
    tmp30Result = tmp30(tmp2(4185).Heading, obj2);
  }
  ...
  if (isPartnerApplicationResult) {
    ...
    obj3[1] = tmp30(tmp2(4185).Text, obj4);
  ...
  if (tmp28Result1) {
    ...
    obj5[1] = function onPress() {
      if (first) {
        ...
        if (first1) {
          const obj2 = sharedValue(4190);
          const current = ref.current;
          const fn = function t() {
            outer1_0(outer1_2[6]).runOnJS(closure_14)(true);
          };
          let obj = { runOnJS: null, setShouldLineClamp: null };
          obj[0] = sharedValue(4050).runOnJS;
          obj[1] = _undefined;
          fn.__closure = obj;
          fn.__workletHash = 6050776164847;
          fn.__initData = _undefined;
          const result = sharedValue.set(obj2.withTiming(current, sharedValue(4193).timingStandard, "respect-motion-settings", fn));
        } else {
          _undefined(false);
          obj = sharedValue(4190);
          const result1 = sharedValue.set(obj.withTiming(ref2.current, sharedValue(4193).timingStandard));
        }
        callback(!first1);
        const tmp2 = first1;
      }
    ...
    obj15[2] = num4;
    obj15[3] = memo;
    obj14[1] = tmp30(tmp2(4185).Text, obj15);
    ...
    if (tmp27) {
      ...
      obj17[3] = items11;
      obj16[1] = tmp28(tmp2(4185).Text, obj17);
    ...
    if (first) {
      ...
      if (first1) {
        ...
        tmp30Result3 = tmp30(tmp2(4185).Text, obj18);
      }
    }
  ...
  const children = [num2(first(4050).View, obj), ];
  ...
  if (!first2) {
    ...
    if (!hideName) {
      ...
      obj20[2] = sectionName;
      tmp30Result4 = tmp30(tmp2(4185).Heading, obj20);
    }
    ...
    if (tmp28Result1) {
      ...
      obj22[3] = memo;
      const items15 = [tmp30(tmp2(4185).Text, obj22), ];
      ...
      items15[1] = tmp30(tmp2(4185).Text, obj23);
      obj21[1] = items15;
