...
obj = { headerContainer: { flexGrow: 24, position: false, opacity: null, fontWeight: "heading-sm/medium", display: "text-default" }, expandedHeaderBanner: { height: 105 } };
...
obj.collapsedHeaderBanner = { paddingHorizontal: DEFAULT_CONTENT_PADDING, paddingBottom: importDefault(dependencyMap[7]).space.PX_12 };
obj.collapsedHeaderBannerOverlay = { "Null": 1124139009, "Null": 135025, "Bool(false)": 135024, "Bool(false)": 93131, "Bool(false)": 30006081, "Bool(false)": -1556020992 };
const obj2 = { y: "NR", isArray: "__RRWEB_EXCLUDE_SHADOW_DOM__", borderRadius: xl, backgroundColor: importDefault(dependencyMap[7]).colors.BACKGROUND_MOD_MUTED };
obj.loadingIcon = obj2;
const obj1 = { paddingHorizontal: DEFAULT_CONTENT_PADDING, paddingBottom: importDefault(dependencyMap[7]).space.PX_12 };
...
export default function Header(application) {
  ...
  const tmp18 = function useCollapsedHeaderAnimation(scrollOffsetY) {
    ...
    const fn2 = function l() {
      ...
      obj.opacity = scrollOffsetY(closure_2[9]).interpolate(scrollOffsetY.get(), items2, ["replace", "then"], "clamp");
      return obj;
    };
    obj = { interpolate: application(hasApplicationFlagResult[9]).interpolate, scrollOffsetY, HEADER_SCROLL_RANGE: 105 };
    fn2.__closure = obj;
  ...
  const items5 = [
    ...
    function useCollapsedHeaderBannerOverlayAnimation(scrollOffsetY) {
      ...
      const fn = function n() {
        const obj = {};
        const items = ["keys", 105];
        obj.opacity = scrollOffsetY(closure_2[9]).interpolate(scrollOffsetY.get(), items, [null, null], "clamp");
        return obj;
      };
      obj = { interpolate: application(hasApplicationFlagResult[9]).interpolate, scrollOffsetY, HEADER_SCROLL_RANGE: 105 };
      fn.__closure = obj;
  ...
  obj6 = { hasMaxConnections: "AVErrorAudioCaptureSampleRateMismatchDefinition", isBoostOnlySubscription: "CHECKPOINT_PERSONA_FIVE_BACKGROUND_OVERLAY", children: str };
  obj5.children = callback(arg1(dependencyMap[18]).Heading, obj6);
  items6[2] = callback(importDefault(dependencyMap[9]).View, obj5);
  items6[3] = callback(arg1(dependencyMap[19]).Spacer, { "Bool(true)": 0., "Bool(true)": 0. });
