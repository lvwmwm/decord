...
// Dependencies: [32, 8115, 1387, 1384, 4268, 1373, 589, 11577, 11582, 6219, 2]
// Exports: default

// Module 11576 (useAppLauncherOnboardingContent)
import resultDefault from "result" /* 11582 */;
import closure_3 from "_slicedToArray" /* 32 */;
import closure_4 from "handleUserSettingsProtoStoreChange" /* 8115 */;
import closure_5 from "ensureGuildLoaded" /* 1387 */;
import { DismissibleContentGroupName as closure_6 } from "ContentDismissActionType" /* 1384 */;
...
export default function useAppLauncherOnboardingContent(channelId) {
  ...
  if (tmp3.canShowBotsBanner) {
    items.push(tmp(1373).DismissibleContent.APP_LAUNCHER_ONBOARDING_BOTS_BANNER);
  }
  if (tmp3.canShowAppsOrActivitiesBanner) {
    const push = items.push;
    const DismissibleContent = tmp(1373).DismissibleContent;
    if ((function useHasUsedActivities(channel) {
      channel = channel.channel;
      let obj = channelId(4268);
      let result = obj.useIsDismissibleContentDismissed_UNSAFE(channelId(1373).DismissibleContent.APP_LAUNCHER_ONBOARDING_ACTIVITIES_BANNER);
      const result1 = channelId(4268).useIsDismissibleContentDismissed_UNSAFE(channelId(1373).DismissibleContent.APP_LAUNCHER_ONBOARDING_APPS_BANNER);
      const obj2 = channelId(4268);
      const tmp = channelId;
      ...
      obj[1] = !result;
      const activityApplications = tmp(11577).useActivityApplications(obj);
      let flag = false;
  ...
  let tmp7 = callback(channelId(6219).useSelectedDismissibleContent(items, constants.APP_LAUNCHER_ONBOARDING), 2);
