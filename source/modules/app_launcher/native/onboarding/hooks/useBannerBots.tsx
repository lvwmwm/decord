...
// Function ID: 11723
// Name: useBannerBots
// Dependencies: [19, 1910, 11701, 589, 8570, 11723, 11694, 2]
// Exports: useBannerBots

// Module 11722 (useBannerBots)
import closure_2 from "noop" /* 19 */;
import closure_3 from "createGuildRecordFromRust" /* 1910 */;
import closure_4 from "initialize" /* 11701 */;
...
export const useBannerBots = function useBannerBots(context) {
  ...
  let obj = (function useAppsMap(context) {
    let apps;
    apps = apps(11723).useApplicationsInContext({ context: context.context, onlyWithCommands: true, includeBuiltIn: false, includeEmbeddedApps: false, includeNonEmbeddedApps: true }).apps;
    const items = [apps];
  ...
  let apps = first1(11723).useApplicationsInContext({ context, onlyWithCommands: true, includeBuiltIn: false, includeEmbeddedApps: false, includeNonEmbeddedApps: true }).apps;
  const obj2 = first1(11723);
  const tmp2 = first1;
  const obj3 = (function useCommandsMap(context) {
    let commands;
    commands = commands(11723).useApplicationCommandsInContext({ context: context.context, includeBuiltIn: false }).commands;
  ...
  const tmp4 = (function useFrecencyCommandIds(context) {
    ...
    return context(8570).useTopCommands(obj);
  })({ context });
  ...
  const activityApplications = first1(11694).useActivityApplications({ guildId: guild_id, fetchesShelf: true });
  const obj4 = first1(11694);
  let items = [closure_4];
