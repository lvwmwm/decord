...
// Dependencies: [19, 17, 1372, 1931, 5310, 21, 4255, 712, 5308, 1236, 4666, 8600, 3973, 5286, 7767, 8706, 5281, 3968, 1581, 1474, 5206, 589, 4223, 5309, 5311, 2]
// Exports: default

// Module 11143 (SelectionHeader)
...
function SelectionHeader(renderIcon) {
  ...
  if (selectionActionComponent.maxValues > 1) {
    ...
    tmp5Result = tmp5(tmp6(4666).Button, obj);
  }
  obj[2] = tmp5Result;
  const children = [closure_8(renderIcon(5308).BottomSheetTitleHeader, obj), ];
  ...
  if (null != onQueryChange) {
    ...
    if (null != memo) {
      ...
      tmp5Result1 = tmp5(selectedOptions(8600), obj1);
      const tmp13 = selectedOptions(8600);
    }
  }
...
function SelectionOptionItem(item) {
  ...
  if (clearable) {
    ...
    obj2[0] = selected;
    tmp6Result = tmp6(tmp2(7767).FormCheckbox, obj2);
  } else {
    tmp6Result = null;
    if (true === selected) {
      tmp6Result = tmp6(tmp2(8706).CheckmarkSmallBoldIcon, { color: "text-brand" });
    }
  }
  items1[1] = tmp6Result;
  obj1[1] = items1;
  obj[12] = closure_10(View, obj1);
  return closure_8(item(5286).TableRow, obj);
}
