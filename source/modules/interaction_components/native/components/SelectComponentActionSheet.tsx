...
// Dependencies: [19, 17, 1391, 1979, 5459, 21, 4342, 712, 5457, 1236, 4777, 9072, 4073, 5435, 8035, 9196, 5430, 1351, 1628, 1493, 5328, 589, 4310, 5458, 5460, 2]
// Exports: default

// Module 11288 (SelectionHeader)
...
function SelectionHeader(renderIcon) {
  ...
  if (selectionActionComponent.maxValues > 1) {
    ...
    tmp5Result = tmp5(tmp6(4777).Button, obj);
  }
  obj[2] = tmp5Result;
  const children = [closure_8(renderIcon(5457).BottomSheetTitleHeader, obj), ];
  ...
  if (null != onQueryChange) {
    ...
    if (null != memo) {
      ...
      tmp5Result1 = tmp5(selectedOptions(9072), obj1);
      const tmp13 = selectedOptions(9072);
    }
  }
...
function SelectionOptionItem(item) {
  ...
  let radioA11yNative = item(4073).useRadioA11yNative({ selected: flag2, disabled });
  if (multi) {
  ...
  if (clearable) {
    ...
    obj2[0] = selected;
    tmp6Result = tmp6(tmp2(8035).FormCheckbox, obj2);
  } else {
    tmp6Result = null;
    if (true === selected) {
      tmp6Result = tmp6(tmp2(9196).CheckmarkSmallBoldIcon, { color: "text-brand" });
    }
  }
  items1[1] = tmp6Result;
  obj1[1] = items1;
  obj[12] = closure_10(View, obj1);
  return closure_8(item(5435).TableRow, obj);
}
