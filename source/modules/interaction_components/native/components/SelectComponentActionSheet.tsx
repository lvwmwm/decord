...
function SelectionHeader(renderIcon) {
  ...
  if (selectionActionComponent.maxValues > 1) {
    ...
    tmp5Result = tmp5(tmp6(4600).Button, obj);
  }
  obj[2] = tmp5Result;
  const children = [closure_8(renderIcon(5242).BottomSheetTitleHeader, obj), ];
...
function SelectionOptionItem(item) {
  ...
  let radioA11yNative = item(3907).useRadioA11yNative({ selected: flag2, disabled });
  if (multi) {
  ...
  return closure_8(item(5220).TableRow, obj);
}
