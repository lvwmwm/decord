...
// Dependencies: [5, 19, 17, 1386, 4862, 673, 21, 4481, 4375, 5560, 15462, 1233, 4747, 4441, 10125, 8028, 5635, 8043, 38, 586, 12090, 5140, 5095, 17316, 1954, 4865, 5099, 11929, 9236, 10565, 1890, 10564, 5097, 4934, 709, 5642, 2]
// Exports: default

// Module 17315 (MainAreaCanUpload)
import getSystemLocale from "getSystemLocale" /* 1233 */;
import formatSize from "formatSize" /* 4375 */;
import CircleCheckIcon from "CircleCheckIcon" /* 4441 */;
import flattenComponents from "flattenComponents" /* 4747 */;
import TableRowInner from "TableRowInner" /* 5560 */;
import FileUpIcon from "FileUpIcon" /* 15462 */;
...
function File(upload) {
  ...
  const tmp = callback2();
  obj[1] = callback(upload(10125).AttachmentIcon, { fileName: upload.filename });
  obj[5] = callback(View, obj);
  obj[0] = callback(handleRemoveFile(10125), obj);
  let filename = upload.filename;
  if (filename == null) {
    const intl = tmp3(1233).intl;
    filename = intl.string(tmp3(1233).t.ZMirp0);
  }
  obj[1] = filename;
  const obj2 = {
    variant: "tertiary",
    size: "sm",
    icon: callback(upload(5635).XSmallIcon, { size: "sm" }),
    onPress() {
  ...
  obj[2] = callback(upload(8028).IconButton, obj2);
  return callback(upload(5560).TableRow, obj);
}
...
export default function FileUploadActionComponent(maxValues) {
        ...
        while (true) {
          ...
          if (tmp20 === undefined) {
            ...
            if (closure_1_4.length > 0) {
              ...
              let tmp31 = lib;
              if (!closure_1_5(lib.map((result1) => callback(5099).getFileFromUploadItem(result1).filename))) {
                let tmp32 = closure_4;
                let tmp33 = v0;
                let num6 = 3;
                c8 = 3;
  ...
  const callback2 = allowedExtensions.useCallback(() => {
    ...
    let obj = {
      ...
      onDismissKeyboard() {
        return InteractionModal(10565).hideMediaKeyboardActionSheet();
      },
    ...
    if (mediaFilesAllowed) {
      ...
      obj[10] = function onManageLimited() {
        obj = InteractionModal(10564);
        obj = { onDismissKeyboard: InteractionModal(10565).hideMediaKeyboardActionSheet, onRestoreKeyboard: closure_8 };
