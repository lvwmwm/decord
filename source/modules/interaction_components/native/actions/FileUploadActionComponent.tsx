...
// Dependencies: [5, 19, 17, 1372, 4591, 676, 21, 4255, 4150, 5300, 13761, 1236, 4476, 4216, 9765, 7644, 5255, 7998, 38, 589, 11430, 4848, 4807, 16323, 1906, 4593, 4811, 11272, 8140, 9896, 1844, 9895, 4809, 4663, 712, 5634, 2]
// Exports: default

// Module 16322 (MainAreaCanUpload)
import ME from "ME";
...
function MainAreaCanUpload(arg0) {
  ...
  obj[0] = require(13761) /* FileUpIcon */.FileUpIcon;
  obj[1] = callback(require(5300) /* TableRowInner */.TableRow.Icon, obj);
  ...
  obj[3] = require(4476) /* flattenComponents */.getFileUploadComponentSubtitle(minValues, maxValues, types, formatSizeResult);
...
function MainAreaLimitReached() {
  ...
  obj[0] = callback(require(5300) /* TableRowInner */.TableRow.Icon, obj);
  const intl = require(1236) /* getSystemLocale */.intl;
...
function File(upload) {
  ...
  const tmp = callback2();
  obj[1] = callback(upload(9765).AttachmentIcon, { fileName: upload.filename });
  obj[5] = callback(View, obj);
  obj[0] = callback(handleRemoveFile(9765), obj);
  ...
  obj2[2] = callback(upload(5255).XSmallIcon, { size: "sm" });
  obj2[3] = function onPress() {
    return handleRemoveFile(upload.id);
  };
  const intl2 = tmp3(1236).intl;
  obj2[4] = intl2.string(upload(1236).t.N86XcP);
  obj[2] = callback(upload(7644).IconButton, obj2);
  return callback(upload(5300).TableRow, obj);
}
...
export default function FileUploadActionComponent(maxValues) {
        ...
        while (true) {
          ...
          if (closure_1 === undefined) {
            ...
            if (outer1_4.length > 0) {
              ...
              let tmp30 = outer1_5;
              let tmp31 = lib;
              if (!outer1_5(lib.map((result1) => callback(4811).getFileFromUploadItem(result1).filename))) {
  ...
  const callback2 = allowedExtensions.useCallback(() => {
    ...
    let obj = {
      ...
      onDismissKeyboard() {
        return InteractionModal(9896).hideMediaKeyboardActionSheet();
      },
    ...
    if (mediaFilesAllowed) {
      ...
      obj[10] = function onManageLimited() {
        let obj = InteractionModal(9895);
        obj = { onDismissKeyboard: InteractionModal(9896).hideMediaKeyboardActionSheet, onRestoreKeyboard: closure_8 };
