...
// Dependencies: [5, 19, 17, 1391, 4807, 676, 21, 4445, 4339, 5551, 14190, 1236, 4692, 4405, 10198, 7938, 5503, 7953, 38, 589, 11645, 5075, 5030, 16887, 1955, 4809, 5034, 11490, 8095, 10348, 1892, 10347, 5032, 4878, 712, 5944, 2]
// Exports: default

// Module 16886 (MainAreaCanUpload)
...
function File(upload) {
  ...
  const tmp = callback2();
  obj[1] = callback(upload(10198).AttachmentIcon, { fileName: upload.filename });
  obj[5] = callback(View, obj);
  obj[0] = callback(handleRemoveFile(10198), obj);
  ...
  const obj2 = {
    variant: "tertiary",
    size: "sm",
    icon: callback(upload(5503).XSmallIcon, { size: "sm" }),
    onPress() {
  ...
  obj[2] = callback(upload(7938).IconButton, obj2);
  return callback(upload(5551).TableRow, obj);
}
...
export default function FileUploadActionComponent(maxValues) {
        ...
        while (true) {
          ...
          if (tmp20 === undefined) {
            ...
            if (closure_1_4.length > 0) {
              ...
              let tmp31 = lib;
              if (!closure_1_5(lib.map((result1) => callback(5034).getFileFromUploadItem(result1).filename))) {
                let tmp32 = closure_4;
                let tmp33 = v0;
                let num6 = 3;
                c8 = 3;
  ...
  const callback2 = allowedExtensions.useCallback(() => {
    ...
    let obj = {
      ...
      onDismissKeyboard() {
        return InteractionModal(10348).hideMediaKeyboardActionSheet();
      },
    ...
    if (mediaFilesAllowed) {
      ...
      obj[10] = function onManageLimited() {
        obj = InteractionModal(10347);
        obj = { onDismissKeyboard: InteractionModal(10348).hideMediaKeyboardActionSheet, onRestoreKeyboard: closure_8 };
