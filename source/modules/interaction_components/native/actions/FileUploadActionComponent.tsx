...
// Dependencies: [5, 19, 17, 1391, 4806, 676, 21, 4444, 4338, 5546, 14163, 1236, 4691, 4404, 10096, 7963, 5498, 8438, 38, 589, 11816, 5070, 5025, 16821, 1955, 4808, 5029, 11639, 8579, 10227, 1892, 10226, 5027, 4877, 712, 5939, 2]
// Exports: default

// Module 16820 (MainAreaCanUpload)
...
function File(upload) {
  ...
  const tmp = callback2();
  obj[1] = callback(upload(10096).AttachmentIcon, { fileName: upload.filename });
  obj[5] = callback(View, obj);
  obj[0] = callback(handleRemoveFile(10096), obj);
  ...
  const obj2 = {
    variant: "tertiary",
    size: "sm",
    icon: callback(upload(5498).XSmallIcon, { size: "sm" }),
    onPress() {
  ...
  obj[2] = callback(upload(7963).IconButton, obj2);
  return callback(upload(5546).TableRow, obj);
}
...
export default function FileUploadActionComponent(maxValues) {
        ...
        while (true) {
          ...
          if (tmp20 === undefined) {
            ...
            if (closure_1_4.length > 0) {
              ...
              let tmp31 = lib;
              if (!closure_1_5(lib.map((result1) => callback(5029).getFileFromUploadItem(result1).filename))) {
                let tmp32 = closure_4;
                let tmp33 = v0;
                let num6 = 3;
                c8 = 3;
  ...
  const callback2 = allowedExtensions.useCallback(() => {
    ...
    let obj = {
      ...
      onDismissKeyboard() {
        return InteractionModal(10227).hideMediaKeyboardActionSheet();
      },
    ...
    if (mediaFilesAllowed) {
      ...
      obj[10] = function onManageLimited() {
        obj = InteractionModal(10226);
        obj = { onDismissKeyboard: InteractionModal(10227).hideMediaKeyboardActionSheet, onRestoreKeyboard: closure_8 };
