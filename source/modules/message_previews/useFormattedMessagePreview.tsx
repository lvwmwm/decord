...
// Dependencies: [1218, 3892, 1874, 676, 686, 589, 7870, 4483, 1236, 4442, 5812, 12, 7875, 7953, 2]
// Exports: isMessageContentPreviewable, useFormattedMessagePreview

// Module 9653 (formatMessagePreview)
...
function formatMessagePreview(type, isBlocked) {
  ...
  if (isBlocked.isBlocked) {
    ...
    obj[1] = intl29.string(require(1236) /* getSystemLocale */.t.XAkOo2);
    return obj;
  } else if (tmp) {
    obj = { type: "text", text: null };
    const intl28 = require(1236) /* getSystemLocale */.intl;
    obj[1] = intl28.string(require(1236) /* getSystemLocale */.t["G7p6v/"]);
    return obj;
  } else {
    type = type.type;
    const tmp4 = require;
    if (require(686) /* set */.MessageTypes.DEFAULT !== type) {
            ...
            if (tmp4(686).MessageTypes.CONTEXT_MENU_COMMAND !== type) {
              ...
              if (flag) {
                ...
                if (type.hasFlag(MessageFlags.IS_VOICE_MESSAGE)) {
                  ...
                  const everyResult = attachments.every((filename) => callback(4442).isImageFile(filename.filename));
                  let everyResult1 = !everyResult;
                  if (!everyResult) {
                    const attachments2 = type.attachments;
                    everyResult1 = attachments2.every((filename) => callback(4442).isVideoFile(filename.filename));
                  }
                  ...
                  if (everyResult2) {
                    ...
                    everyResult2 = attachments3.every((filename) => callback(4442).isAudioFile(filename.filename));
                  }
                  if (everyResult) {
                  ...
                  const everyResult3 = embeds.every((url) => {
                    ...
                    if (isImageUrlResult) {
                      isImageUrlResult = callback(4442).isImageUrl(url.url);
                      const obj = callback(4442);
                    }
                  ...
                  if (!everyResult3) {
                    ...
                    everyResult4 = embeds2.every((url) => {
                      ...
                      if (isVideoUrlResult) {
                        isVideoUrlResult = callback(4442).isVideoUrl(url.url);
                        const obj = callback(4442);
                      }
                  ...
                  if (everyResult5) {
                    ...
                    everyResult5 = embeds3.every((url) => {
                      ...
                      if (isAudioFileResult) {
                        isAudioFileResult = callback(4442).isAudioFile(url.url);
                        const obj = callback(4442);
                      }
                  ...
                  tmp10 = obj24;
                } else if (importDefault(5812)(type)) {
                  obj = { type: "text", text: null };
              ...
              if (type.type === tmp4(686).MessageTypes.CALL) {
                ...
                obj52[1] = tmp4(7953).getVoiceSessionMessageContent(type);
                tmp21 = obj52;
                const tmp4Result = tmp4(7953);
              }
            }
          }
        }
