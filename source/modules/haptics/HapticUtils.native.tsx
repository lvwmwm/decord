...
// Dependencies: [4450, 4451, 1234, 4460, 2]
// Exports: triggerHapticFeedback

// Module 4449 (HapticFeedbackTypes)
import set from "set" /* 2 */;
import set2 from "set" /* 1234 */;
import IMPACT_LIGHTDefault from "IMPACT_LIGHT" /* 4450 */;
import useHaptics from "useHaptics" /* 4451 */;
...
export const triggerHapticFeedback = function triggerHapticFeedback(IMPACT_LIGHT) {
  if (IMPACT_LIGHTDefault.IMPACT_LIGHT === IMPACT_LIGHT) {
    ...
    if (obj42.isAndroid()) {
      ...
      str36 = str37;
      tmp69Result = tmp69(4460);
    }
    obj41.trigger(str36);
    obj42 = set2;
  } else if (tmp(4450).IMPACT_MEDIUM === IMPACT_LIGHT) {
    ...
    if (obj39.isAndroid()) {
      ...
      str34 = str35;
      tmp66Result = tmp66(4460);
    }
    obj38.trigger(str34);
    obj39 = set2;
  } else if (tmp(4450).IMPACT_HEAVY === IMPACT_LIGHT) {
    useHaptics.trigger("impactHeavy");
    const obj37 = useHaptics;
  } else if (tmp(4450).NOTIFICATION_ERROR === IMPACT_LIGHT) {
    useHaptics.trigger("notificationError");
    const obj36 = useHaptics;
  } else if (tmp(4450).DRAG_AND_DROP_START === IMPACT_LIGHT) {
    ...
    obj34.trigger(str31);
    obj35 = set2;
  } else if (tmp(4450).DRAG_AND_DROP_END === IMPACT_LIGHT) {
    ...
    if (obj32.isAndroid()) {
      ...
      str29 = str30;
      tmp57Result = tmp57(4460);
    }
    obj31.trigger(str29);
    obj32 = set2;
  } else if (tmp(4450).DRAG_AND_DROP_MOVE === IMPACT_LIGHT) {
    ...
    if (obj29.isAndroid()) {
      ...
      str27 = str28;
      tmp54Result = tmp54(4460);
    }
    obj28.trigger(str27);
    obj29 = set2;
  } else if (tmp(4450).SOFT === IMPACT_LIGHT) {
    useHaptics.trigger("soft");
    const obj27 = useHaptics;
  } else if (tmp(4450).SELECTION === IMPACT_LIGHT) {
