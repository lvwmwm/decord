...
// Dependencies: [1372, 4373, 3938, 12926, 12924, 687, 12927, 691, 12931, 5260, 2]
// Exports: userBlockedWarningInCooldown, voiceBlockedWarningInCooldownForUsers

// Module 12925 (handleChannelSelect)
    ...
    if (null != channel) {
      if (channel.isGroupDM()) {
        ...
        if (tmp) {
          ...
          if (!blockedUserWarningDismissed) {
            let obj = require(12927) /* showGdmBlockedUserModal */;
      ...
      if (blockedUsersForVoiceChannel.size > 0) {
        if (callback4()) {
          ...
          if (!everyResult) {
            const items1 = [];
            HermesBuiltin.arraySpread(ignoredUsersForVoiceChannel, HermesBuiltin.arraySpread(blockedUsersForVoiceChannel, 0));
            const result = require(12931) /* showVoiceChannelBlockedUserWarning */.showVoiceChannelBlockedUserWarning(channelId, items1[0]);
            const tmpResult = require(12931) /* showVoiceChannelBlockedUserWarning */;
          }
        }
      }
      callback5();
...
SharedSpacesWarningManager.prototype["handleBlockedOrIgnoredUserVoiceChannelJoin"] = function handleBlockedOrIgnoredUserVoiceChannelJoin(channelId, id) {
  ...
  if (channelId === channelId) {
    if (null != store.getChannel(channelId)) {
      ...
      if (!tmp6) {
        const result = require(12931) /* showVoiceChannelBlockedUserWarning */.showVoiceChannelBlockedUserWarning(channelId, id);
        const obj = require(12931) /* showVoiceChannelBlockedUserWarning */;
      }
    }
  }
};
