...
// Dependencies: [1372, 4375, 3938, 12996, 12994, 687, 12997, 691, 13001, 5261, 2]
// Exports: userBlockedWarningInCooldown, voiceBlockedWarningInCooldownForUsers

// Module 12995 (handleChannelSelect)
    ...
    if (null != channel) {
      if (channel.isGroupDM()) {
        ...
        if (tmp) {
          ...
          if (!blockedUserWarningDismissed) {
            let obj = require(12997) /* showGdmBlockedUserModal */;
      ...
      if (blockedUsersForVoiceChannel.size > 0) {
        if (callback4()) {
          ...
          if (!everyResult) {
            const items1 = [];
            HermesBuiltin.arraySpread(ignoredUsersForVoiceChannel, HermesBuiltin.arraySpread(blockedUsersForVoiceChannel, 0));
            const result = require(13001) /* showVoiceChannelBlockedUserWarning */.showVoiceChannelBlockedUserWarning(channelId, items1[0]);
            const tmpResult = require(13001) /* showVoiceChannelBlockedUserWarning */;
          }
        }
      }
      callback5();
...
SharedSpacesWarningManager.prototype["handleBlockedOrIgnoredUserVoiceChannelJoin"] = function handleBlockedOrIgnoredUserVoiceChannelJoin(channelId, id) {
  ...
  if (channelId === channelId) {
    if (null != store.getChannel(channelId)) {
      ...
      if (!tmp6) {
        const result = require(13001) /* showVoiceChannelBlockedUserWarning */.showVoiceChannelBlockedUserWarning(channelId, id);
        const obj = require(13001) /* showVoiceChannelBlockedUserWarning */;
      }
    }
  }
};
