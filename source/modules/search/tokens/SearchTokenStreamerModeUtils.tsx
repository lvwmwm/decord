...
function getValidOrderedFilterTokens(closure_0, items) {
  ...
  const items2 = [tmp];
  [tmp5] = items2;
  let result = require(11787) /* SearchTokenTypes */.isGuildLikeSearchContext(closure_0);
  if (!result) {
...
export const isInChannelFilterSupported = function isInChannelFilterSupported(selectedSearchContext) {
  ...
  [tmp3] = tmp;
  let result = require(11787) /* SearchTokenTypes */.isGuildLikeSearchContext(selectedSearchContext);
  if (!result) {
