...
// Function ID: 10257
// Name: getShortcuts
// Dependencies: [32, 1372, 1932, 3892, 1931, 4095, 1874, 10257, 10258, 676, 3837, 12, 1236, 4446, 4094, 10255, 10259, 5168, 5097, 7218, 10260, 10261, 2]
...
function isValidHasAutocomplete(token) {
  return require(10259) /* ANSWER_IN_REGEX */.validateForMapWithNegation("has", getHasMap(), token);
}
function isValidAuthorTypeAutocomplete(token) {
  let obj = require(10259) /* ANSWER_IN_REGEX */;
  obj = {};
  const intl = require(1236) /* getSystemLocale */.intl;
...
function getChannelAutocompletions(arg0) {
  ...
  if (str.startsWith("\"")) {
    ...
    if (obj2.isGuildLikeSearchContext(searchContext)) {
      ...
      obj[7] = tmpResult.getBoosterMap(tmp(7218).AutocompleterResultTypes.TEXT_CHANNEL);
      const obj12 = importDefault(5097);
      ...
      obj[2] = guildId;
      tmpResult = tmp(5097);
      obj[6] = tmpResult.getBoosterMap(tmp(7218).AutocompleterResultTypes.VOICE_CHANNEL);
      const combined = queryChannelsResult.concat(importDefault(5097).queryChannels(obj));
      ...
      importDefault = store2.getTextChannelNameDisambiguations(guildId);
      const obj16 = importDefault(5097);
      ...
      if (searchContext.type === constants.DMS) {
        if (!initialize.hidePersonalInformation) {
          ...
          obj1[3] = tmp(5097).getBoosterMap(tmp(7218).AutocompleterResultTypes.GROUP_DM);
          const tmpResult1 = tmp(5097);
          const queryGroupDMsResult = obj4.queryGroupDMs(obj1);
          obj2 = { query: null, limit: null, boosters: null };
          obj2[0] = substr1;
          obj2[1] = maxResults;
          const obj8 = importDefault(5097);
          obj2[2] = tmp(5097).getBoosterMap(tmp(7218).AutocompleterResultTypes.USER);
          const tmpResult2 = tmp(5097);
          const queryDMChannelsResult = obj8.queryDMChannels(obj2);
          const tmp6 = importDefault(12);
          const sorted = importDefault(12)(queryGroupDMsResult.concat(queryDMChannelsResult)).sort(tmp(7218).sortByMatchScore);
