...
// Function ID: 11717
// Name: getShortcuts
// Dependencies: [32, 1391, 1980, 3957, 1979, 4149, 1922, 11717, 11718, 676, 3902, 12, 1236, 4494, 4148, 11715, 11719, 5229, 5159, 7309, 11720, 11721, 2]
...
function getShortcuts() {
  ...
  obj[intl.string(require(1236) /* getSystemLocale */.t.HYiVEQ)] = () => {
    const obj = callback(3902)();
    const addResult = callback(3902)().startOf("day").add(0, "day");
    const items = [addResult, ];
    const startOfResult = callback(3902)().startOf("day");
  ...
  obj[intl2.string(require(1236) /* getSystemLocale */.t.cu86KC)] = () => {
    const obj = callback(3902)();
    const addResult = callback(3902)().startOf("day").add(-1, "day");
    const items = [addResult, ];
    const startOfResult = callback(3902)().startOf("day");
    items[1] = addResult.clone().add(1, "day");
    return items;
  ...
  obj[intl3.string(require(1236) /* getSystemLocale */.t["FvBj/6"])] = () => {
    const obj = callback(3902)();
    const addResult = callback(3902)().startOf("week").add(0, "week");
    const items = [addResult, ];
    const startOfResult = callback(3902)().startOf("week");
  ...
  obj[intl4.string(require(1236) /* getSystemLocale */.t["20uWCw"])] = () => {
    const obj = callback(3902)();
    const addResult = callback(3902)().startOf("month").add(0, "month");
    const items = [addResult, ];
    const startOfResult = callback(3902)().startOf("month");
  ...
  obj[intl5.string(require(1236) /* getSystemLocale */.t["dXC/hn"])] = () => {
    const obj = callback(3902)();
    const addResult = callback(3902)().startOf("year").add(0, "year");
    const items = [addResult, ];
    const startOfResult = callback(3902)().startOf("year");
...
function dateValidator(getFullMatch) {
  ...
  if (null != tmp2) {
    ...
    const _Set3 = Set;
    const obj20 = importDefault(3902);
    const set = new Set(importDefault(3902).months().map((str) => str.toLowerCase()));
    if (set.has(formatted)) {
      const localResult = tmp32(3902)(formatted, "MMMM").local();
      const items = [localResult, ];
      const obj17 = tmp32(3902)(formatted, "MMMM");
      ...
      const _Set = Set;
      let tmp32Result = tmp32(3902);
      const set1 = new Set(tmp32Result.weekdays().map((str) => str.toLowerCase()));
      if (set1.has(formatted)) {
        const localResult1 = tmp32(3902)(formatted, "dddd").local();
        const items1 = [localResult1, ];
        const obj14 = tmp32(3902)(formatted, "dddd");
        ...
        if (set2.has(formatted)) {
          const localResult2 = tmp32(3902)(formatted, "YYYY").local();
          const items2 = [localResult2, ];
          const obj11 = tmp32(3902)(formatted, "YYYY");
          ...
          const localResult3 = tmp32(3902)(formatted, closure_16).local();
          const items3 = [localResult3, ];
          const obj6 = tmp32(3902)(formatted, closure_16);
    ...
    const monthsResult = importDefault(3902).months();
  }
...
function isValidHasAutocomplete(token) {
  return require(11719) /* ANSWER_IN_REGEX */.validateForMapWithNegation("has", getHasMap(), token);
}
function isValidAuthorTypeAutocomplete(token) {
  let obj = require(11719) /* ANSWER_IN_REGEX */;
  obj = {};
  const intl = require(1236) /* getSystemLocale */.intl;
...
function generateDateAutocompletions() {
  ...
  const monthsResult = importDefault(3902).months();
  const items = [...Array.from(new Set(importDefault(3902).months().map((str) => str.toLowerCase())))];
  const set = new Set(importDefault(3902).months().map((str) => str.toLowerCase()));
  const obj2 = importDefault(3902);
  const weekdaysResult = importDefault(3902).weekdays();
  let arraySpreadResult = HermesBuiltin.arraySpread(Array.from(new Set(importDefault(3902).weekdays().map((str) => str.toLowerCase()))), tmp2);
  const set1 = new Set(importDefault(3902).weekdays().map((str) => str.toLowerCase()));
...
function getChannelAutocompletions(arg0) {
  ...
  if (str.startsWith("\"")) {
    ...
    if (obj2.isGuildLikeSearchContext(searchContext)) {
      ...
      obj[1] = closure_5;
      obj[2] = guildId;
      let tmpResult = tmp(5159);
      obj[7] = tmpResult.getBoosterMap(tmp(7309).AutocompleterResultTypes.TEXT_CHANNEL);
      const obj12 = importDefault(5159);
      ...
      obj[2] = guildId;
      tmpResult = tmp(5159);
      obj[6] = tmpResult.getBoosterMap(tmp(7309).AutocompleterResultTypes.VOICE_CHANNEL);
      const combined = queryChannelsResult.concat(importDefault(5159).queryChannels(obj));
      ...
      importDefault = store2.getTextChannelNameDisambiguations(guildId);
      const obj16 = importDefault(5159);
      ...
      if (searchContext.type === constants.DMS) {
        if (!initialize.hidePersonalInformation) {
          ...
          obj1[3] = tmp(5159).getBoosterMap(tmp(7309).AutocompleterResultTypes.GROUP_DM);
          const tmpResult1 = tmp(5159);
          const queryGroupDMsResult = obj4.queryGroupDMs(obj1);
          obj2 = { query: null, limit: null, boosters: null };
          obj2[0] = substr1;
          obj2[1] = maxResults;
          const obj8 = importDefault(5159);
          obj2[2] = tmp(5159).getBoosterMap(tmp(7309).AutocompleterResultTypes.USER);
          const tmpResult2 = tmp(5159);
          const queryDMChannelsResult = obj8.queryDMChannels(obj2);
          const tmp6 = importDefault(12);
          const sorted = importDefault(12)(queryGroupDMsResult.concat(queryDMChannelsResult)).sort(tmp(7309).sortByMatchScore);
