...
// Function ID: 11789
// Name: getShortcuts
// Dependencies: [32, 1391, 1980, 3998, 1979, 4188, 1922, 11789, 11790, 676, 3943, 12, 1236, 4535, 4187, 11787, 11791, 5290, 5221, 7375, 11792, 11793, 2]
...
function isValidHasAutocomplete(token) {
  return require(11791) /* ANSWER_IN_REGEX */.validateForMapWithNegation("has", getHasMap(), token);
}
function isValidAuthorTypeAutocomplete(token) {
  let obj = require(11791) /* ANSWER_IN_REGEX */;
  obj = {};
  const intl = require(1236) /* getSystemLocale */.intl;
...
function getChannelAutocompletions(arg0) {
  ...
  if (str.startsWith("\"")) {
    ...
    if (obj2.isGuildLikeSearchContext(searchContext)) {
      ...
      const obj12 = importDefault(5221);
      ...
      const combined = queryChannelsResult.concat(importDefault(5221).queryChannels(obj));
      ...
      importDefault = store2.getTextChannelNameDisambiguations(guildId);
      const obj16 = importDefault(5221);
      ...
      if (searchContext.type === constants.DMS) {
        if (!initialize.hidePersonalInformation) {
          ...
          const sorted = importDefault(12)(queryGroupDMsResult.concat(queryDMChannelsResult)).sort(tmp(7375).sortByMatchScore);
...
export const isValidFilterAnswerForSubmit = function isValidFilterAnswerForSubmit(searchTokenType, trimmed) {
  if (null == table[searchTokenType]) {
    ...
    const token = new require(11793) /* getMatch */.Token(items, tmp);
