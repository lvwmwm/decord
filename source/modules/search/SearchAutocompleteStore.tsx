...
// Dependencies: [1391, 1990, 1910, 1979, 4190, 1922, 676, 5266, 11772, 7352, 4189, 4503, 5270, 5199, 11773, 589, 709, 2]

// Module 11774 (handleUserSearchResults)
  ...
  if (null != value) {
    if (null != value) {
      ...
      if (tmp8) {
        ...
        const searchContextId1 = require(11772) /* SearchTokenTypes */.getSearchContextId(c165);
        let value1 = obj2.get(searchContextId1);
        if (value1 == null) {
          obj = { results: null, context: null };
          obj[0] = [];
          obj[1] = importDefault(7352).getUserSearchContext(handleUserSearchResults.bind(null, c165));
          value1 = obj;
          const obj6 = importDefault(7352);
        }
        ...
        const tmpResult = require(11772) /* SearchTokenTypes */;
      }
      tmp19 = constants;
    }
  }
...
function getAutocompleteList(searchContext, autocompleteMode, tokens) {
  ...
  if (constants.FILTER === type) {
    ...
    if (null != filter) {
      ...
      const tmp15 = importDefault(11773)[filter];
...
function handleChannelCreateOrDelete() {
  require(11772) /* SearchTokenTypes */.clearTokenCache();
}
function rebuildAutocompleteResults(c13) {
  ...
  if (null == value) {
    ...
    const searchContextId1 = tmp(11772).getSearchContextId(c13);
    value = map1.get(searchContextId1);
    if (value == null) {
      obj = { results: null, context: null };
      obj[0] = [];
      obj[1] = importDefault(7352).getUserSearchContext(handleUserSearchResults.bind(null, c13));
      value = obj;
      const obj4 = importDefault(7352);
    }
...
const searchAutocompleteStoreClass = new SearchAutocompleteStoreClass(require("dispatcher"), {
  SEARCH_AUTOCOMPLETE_INITIALIZE: function handleSearchAutocompleteInitialize(searchContext) {
    ...
    if (!importDefault(4503)(searchContext, searchContext)) {
      require(11772) /* SearchTokenTypes */.clearTokenCache();
      const obj = require(11772) /* SearchTokenTypes */;
    }
  ...
  SEARCH_AUTOCOMPLETE_QUERY_UPDATE: function handleSearchAutocompleteQueryUpdate(arg0) {
    ...
    if (tmp9) {
      ...
      if (null != trimmed) {
        if (trimmed.length > 0) {
          const guildIdFromSearchContext = tmp5(11772).getGuildIdFromSearchContext(searchContext);
          if (null != guildIdFromSearchContext) {
            const members = tmp(5270).requestMembers(guildIdFromSearchContext, trimmed, c17);
            const tmpResult1 = tmp(5270);
          }
          ...
          const tmp5Result1 = tmp5(11772);
          obj2[2] = tmp5(5199).getBoosterMap(AutocompleterResultTypes.USER);
