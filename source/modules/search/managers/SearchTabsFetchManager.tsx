...
// Dependencies: [109, 8446, 676, 11780, 11781, 2]

// Module 11779 (createRequestPayload)
...
prototype["createWithPayload"] = function createWithPayload(searchTabs) {
    ...
    if (tmp2.GUILD_CHANNEL !== type) {
      if (tmp2.THREAD !== type) {
        if (tmp2.CHANNEL === type) {
          const searchTabFetcherImpl = new require(11781) /* fetch */.SearchTabFetcherImpl(searchContext.channelId, searchContext.type, searchQuery, requestPayload);
          return searchTabFetcherImpl;
        } else if (tmp2.DMS === type) {
          const searchTabFetcherImpl1 = new require(11781) /* fetch */.SearchTabFetcherImpl(searchContext.type, searchContext.type, searchQuery, requestPayload);
  ...
  const searchTabFetcherImpl2 = new require(11781) /* fetch */.SearchTabFetcherImpl(searchContext.guildId, searchContext.type, searchQuery, requestPayload);
