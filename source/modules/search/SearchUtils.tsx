...
// Dependencies: [32, 1372, 1932, 4470, 3892, 1931, 1874, 9281, 676, 3837, 1236, 10256, 11, 10261, 12, 10262, 4446, 4094, 2]
...
export const getAutocompleteMode = function getAutocompleteMode(cursorScope, tokens) {
  ...
  if (0 === tokens.length) {
    ...
    if (obj9.isSearchFilterTokenType(currentToken.type)) {
      if (null != nextToken) {
        if (nextToken.type !== importDefault(10261).NON_TOKEN_TYPE) {
          if (null != nextToken) {
    ...
    if (currentToken.type === importDefault(10261).NON_TOKEN_TYPE) {
    ...
    let tmp4;
    if (currentToken.type === tmp3(10261).NON_TOKEN_TYPE) {
      tmp4 = currentToken;
    }
    obj4 = { type: null, filter: null, token: null };
    obj4[0] = constants2.FILTER_ALL;
    obj4[2] = tmp4;
    obj9 = require(10256) /* getShortcuts */;
    const tmp10 = require;
    tmp3 = importDefault;
  }
};
...
export const refreshSearchTokens = function refreshSearchTokens() {
  const result = require(10256) /* getShortcuts */.rebuildSearchTokenConfigs();
  tmp4.reset();
  const obj = require(10256) /* getShortcuts */;
  const tmp3 = importDefault(12);
  importDefault(12)(importDefault(10256)).forOwn((arg0, type) => {
    const merged = Object.assign(arg0);
    return closure_19.addRule({ type });
  });
  tmp5.reset();
  const tmp3Result = importDefault(12)(importDefault(10256));
  const crossDMSearchTokensConfig = require(10256) /* getShortcuts */.buildCrossDMSearchTokensConfig();
  const obj3 = require(10256) /* getShortcuts */;
  ...
  const result1 = importDefault(10262).markSearchTokensRefreshed();
};
export const getChannelDisplayName = function getChannelDisplayName(isDM) {
  const channelName = require(4446) /* computeChannelName */.computeChannelName(isDM, mergeGuildAvatar, upsertRelationship);
  if (isDM.isDM()) {
...
export const getChannelPlaceholderName = function getChannelPlaceholderName(isGroupDM) {
  if (isGroupDM.isGroupDM()) {
    return require(4446) /* computeChannelName */.computeChannelName(isGroupDM, authStore, upsertRelationship);
    ...
    if (name == null) {
      name = require(4446) /* computeChannelName */.computeChannelName(isGroupDM, authStore, upsertRelationship);
