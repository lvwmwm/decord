...
// Dependencies: [32, 1372, 1932, 4408, 3830, 1931, 1874, 9108, 676, 3775, 1236, 10117, 11, 10122, 12, 10123, 4384, 4032, 2]
...
export const getChannelActiveAgoTimestamp = function getChannelActiveAgoTimestamp(arg0) {
  ...
  const diffResult = obj.diff(importDefault(3775)(arg0), "s");
  if (diffResult > c17) {
    const _Math5 = Math;
...
export const getAutocompleteMode = function getAutocompleteMode(cursorScope, tokens) {
  ...
  if (0 === tokens.length) {
    ...
    if (obj9.isSearchFilterTokenType(currentToken.type)) {
      if (null != nextToken) {
        if (nextToken.type !== importDefault(10122).NON_TOKEN_TYPE) {
          if (null != nextToken) {
    ...
    if (currentToken.type === importDefault(10122).NON_TOKEN_TYPE) {
    ...
    let tmp4;
    if (currentToken.type === tmp3(10122).NON_TOKEN_TYPE) {
      tmp4 = currentToken;
    }
    obj4 = { type: null, filter: null, token: null };
    obj4[0] = constants2.FILTER_ALL;
    obj4[2] = tmp4;
    obj9 = require(10117) /* getShortcuts */;
    const tmp10 = require;
    tmp3 = importDefault;
  }
};
...
export const refreshSearchTokens = function refreshSearchTokens() {
  const result = require(10117) /* getShortcuts */.rebuildSearchTokenConfigs();
  tmp4.reset();
  const obj = require(10117) /* getShortcuts */;
  const tmp3 = importDefault(12);
  importDefault(12)(importDefault(10117)).forOwn((arg0, type) => {
    const merged = Object.assign(arg0);
    return closure_19.addRule({ type });
  });
  tmp5.reset();
  const tmp3Result = importDefault(12)(importDefault(10117));
  const crossDMSearchTokensConfig = require(10117) /* getShortcuts */.buildCrossDMSearchTokensConfig();
  const obj3 = require(10117) /* getShortcuts */;
  ...
  const result1 = importDefault(10123).markSearchTokensRefreshed();
};
export const getChannelDisplayName = function getChannelDisplayName(isDM) {
  const channelName = require(4384) /* computeChannelName */.computeChannelName(isDM, mergeGuildAvatar, upsertRelationship);
  if (isDM.isDM()) {
    const user = mergeGuildAvatar.getUser(isDM.getRecipientId());
    const userTag = importDefault(4032).getUserTag(user);
    let flag = false;
    let str = userTag;
    if (null == userTag) {
      return null;
...
export const getChannelPlaceholderName = function getChannelPlaceholderName(isGroupDM) {
  if (isGroupDM.isGroupDM()) {
    return require(4384) /* computeChannelName */.computeChannelName(isGroupDM, authStore, upsertRelationship);
  } else if (isGroupDM.isDM()) {
    const user = authStore.getUser(isGroupDM.getRecipientId());
    return importDefault(4032).getUserTag(user);
  } else {
    ...
    if (name == null) {
      name = require(4384) /* computeChannelName */.computeChannelName(isGroupDM, authStore, upsertRelationship);
