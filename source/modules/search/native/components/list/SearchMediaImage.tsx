...
// Dependencies: [32, 19, 17, 5793, 21, 4255, 4191, 4101, 5807, 10069, 4761, 4655, 1473, 11287, 500, 1236, 9226, 9734, 8740, 589, 9693, 9765, 2]
...
// Module 15603 (SearchMediaObscurityIcon)
  ...
  if (null != mediaUrl) {
    if (null != mediaHeight) {
      if (null != mediaWidth) {
        ...
        let obj2 = containerHeight(11287);
        ...
        obj5[4] = stringResult;
        const items4 = [callback2(containerHeight(9226).ImageWithPlaceholder, obj5), tmp12];
        obj4[1] = items4;
...
export const SearchAttachmentMediaImage = function SearchAttachmentMediaImage(attachment) {
  ...
  obj = {};
  const attachmentUrl = attachment(8740).getAttachmentUrl(attachment);
...
export const SearchEmbedMediaImage = function SearchEmbedMediaImage(embed) {
  ...
  const tmp2 = embed;
  const size = embed(8740).getEmbedMedia(embed);
  let embedUrl = null;
  if (null != size) {
    embedUrl = tmp2(8740).getEmbedUrl(size);
...
export const SearchSoundMediaImage = function SearchSoundMediaImage(height) {
  ...
  obj[0] = items1;
  obj[1] = callback2(height(9693).CirclePlayIcon, { size: "lg", color: "interactive-text-default" });
...
export const SearchFileMediaImage = function SearchFileMediaImage(height) {
  ...
  obj[0] = items1;
  obj[1] = callback2(height(9765).AttachmentIcon, { fileName });
  return callback2(closure_7, obj);
};
