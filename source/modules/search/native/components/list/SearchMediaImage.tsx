...
// Dependencies: [32, 19, 17, 1386, 6136, 673, 21, 4478, 4413, 1362, 6150, 11099, 5034, 4918, 1491, 11807, 1234, 1233, 8895, 10282, 1398, 6182, 8583, 586, 10235, 10301, 2]
...
// Module 16481 (SearchMediaObscurityIcon)
import closure_3 from "_slicedToArray" /* 32 */;
import importAllResult from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_8 from "ensureGuildLoaded" /* 1386 */;
import closure_9 from "handleReaction" /* 6136 */;
  ...
  if (null != mediaUrl) {
    if (null != mediaHeight) {
      if (null != mediaWidth) {
        ...
        let obj2 = containerHeight(11807);
        ...
        obj5[4] = stringResult;
        const items4 = [callback2(containerHeight(8895).ImageWithPlaceholder, obj5), tmp12];
        obj4[1] = items4;
...
export const SearchEmbedMediaImage = function SearchEmbedMediaImage(embed) {
  ...
  closure_4 = embed(6182).useIsChannelSpoilerGated(store.getChannel(channelId));
  const obj2 = embed(6182);
  const tmp2 = embed;
  ...
  const size = embed(8583).getEmbedMedia(embed);
  let embedUrl = null;
  if (null != size) {
    embedUrl = tmp2(8583).getEmbedUrl(size);
...
export const SearchSoundMediaImage = function SearchSoundMediaImage(height) {
  ...
  obj[0] = items1;
  obj[1] = callback2(height(10235).CirclePlayIcon, { size: "lg", color: "interactive-text-default" });
...
export const SearchFileMediaImage = function SearchFileMediaImage(height) {
  ...
  obj[0] = items1;
  obj[1] = callback2(height(10301).AttachmentIcon, { fileName });
  return callback2(closure_7, obj);
};
