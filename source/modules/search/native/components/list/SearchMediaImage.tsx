...
// Dependencies: [32, 19, 17, 1391, 6080, 676, 21, 4445, 4380, 1363, 6094, 10799, 4982, 4870, 1493, 11505, 500, 1236, 8803, 10180, 1403, 6126, 8500, 589, 10133, 10198, 2]
...
// Module 16162 (SearchMediaObscurityIcon)
import closure_3 from "_slicedToArray" /* 32 */;
import importAllResult from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_8 from "ensureGuildLoaded" /* 1391 */;
import closure_9 from "handleReaction" /* 6080 */;
  ...
  if (null != mediaUrl) {
    if (null != mediaHeight) {
      if (null != mediaWidth) {
        ...
        let obj2 = containerHeight(11505);
        ...
        obj5[4] = stringResult;
        const items4 = [callback2(containerHeight(8803).ImageWithPlaceholder, obj5), tmp12];
        obj4[1] = items4;
...
export const SearchEmbedMediaImage = function SearchEmbedMediaImage(embed) {
  ...
  closure_4 = embed(6126).useIsChannelSpoilerGated(store.getChannel(channelId));
  const obj2 = embed(6126);
  const tmp2 = embed;
  ...
  const size = embed(8500).getEmbedMedia(embed);
  let embedUrl = null;
  if (null != size) {
    embedUrl = tmp2(8500).getEmbedUrl(size);
...
export const SearchSoundMediaImage = function SearchSoundMediaImage(height) {
  ...
  obj[0] = items1;
  obj[1] = callback2(height(10133).CirclePlayIcon, { size: "lg", color: "interactive-text-default" });
...
export const SearchFileMediaImage = function SearchFileMediaImage(height) {
  ...
  obj[0] = items1;
  obj[1] = callback2(height(10198).AttachmentIcon, { fileName });
  return callback2(closure_7, obj);
};
