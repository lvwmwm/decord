...
// Dependencies: [32, 19, 17, 5938, 21, 4344, 4280, 1363, 5952, 10229, 4850, 4744, 1492, 11462, 500, 1236, 9383, 9894, 8891, 589, 9853, 9925, 2]
  ...
  if (null != mediaUrl) {
    if (null != mediaHeight) {
      if (null != mediaWidth) {
        ...
        let obj2 = containerHeight(11462);
        ...
        obj5[4] = stringResult;
        const items4 = [callback2(containerHeight(9383).ImageWithPlaceholder, obj5), tmp12];
        obj4[1] = items4;
...
export const SearchAttachmentMediaImage = function SearchAttachmentMediaImage(attachment) {
  ...
  obj = {};
  const attachmentUrl = attachment(8891).getAttachmentUrl(attachment);
...
export const SearchEmbedMediaImage = function SearchEmbedMediaImage(embed) {
  ...
  const tmp2 = embed;
  const size = embed(8891).getEmbedMedia(embed);
  let embedUrl = null;
  if (null != size) {
    embedUrl = tmp2(8891).getEmbedUrl(size);
...
export const SearchSoundMediaImage = function SearchSoundMediaImage(height) {
  ...
  obj[0] = items1;
  obj[1] = callback2(height(9853).CirclePlayIcon, { size: "lg", color: "interactive-text-default" });
...
export const SearchFileMediaImage = function SearchFileMediaImage(height) {
  ...
  obj[0] = items1;
  obj[1] = callback2(height(9925).AttachmentIcon, { fileName });
  return callback2(closure_7, obj);
};
