...
// Dependencies: [19, 17, 4473, 1389, 1386, 1908, 4709, 673, 21, 4481, 709, 586, 4989, 4681, 1296, 4477, 10074, 10320, 9367, 4325, 8112, 16593, 13215, 16594, 4754, 7892, 7267, 16575, 1233, 10038, 4171, 2]

// Module 16592 (GuildChannelMessageRowHeader)
...
import PollBadgeDefault from "PollBadge" /* 16594 */;
...
function GuildChannelMessageRowHeader(channel) {
  ...
  const items1 = [callback(channel(1296).Icon, obj1), callback(channel(4477).Text, { lineClamp: 1, variant: "text-sm/semibold", color: "interactive-text-default", children: computeChannelNameDefault(channel) }), , , ];
  if (muted) {
  ...
  if (isSystemDMResult) {
    ...
    obj4[0] = tmp6(9367).Types.SYSTEM_DM;
    isSystemDMResult = tmp10(tmp6(9367), obj4);
    const tmp6Result = tmp6(9367);
  }
...
function PrivateChannelMessageRowLabel(message) {
  ...
  const items2 = [callback(message(4477).Text, { lineClamp: 1, variant: "text-md/semibold", color: "interactive-text-active", children: memo }), , ];
  if (muted) {
    obj1 = { source: null, size: null, style: null };
    obj1[0] = channel(10074);
  ...
  if (isSystemDMResult) {
    ...
    obj2[0] = channel(9367).Types.SYSTEM_DM;
    isSystemDMResult = tmp9(channel(9367), obj2);
    const tmp13 = channel(9367);
  }
  items2[2] = isSystemDMResult;
  obj[1] = items2;
  const items3 = [closure_12(closure_4, obj), callback(message(4477).Text, { variant: "text-xs/medium", color: "interactive-text-active", lineClamp: 1, style: tmp.timestamp, accessibilityLabel: timestampAccessibilityLabel, children: timestamp }), , ];
  let tmp9Result = null;
  if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
    const obj4 = { size: "xs", style: null };
    obj4[1] = tmp.suppressNotificationsIcon;
    tmp9Result = tmp9(tmp4(13215).BellZIcon, obj4);
  }
  items3[2] = tmp9Result;
  tmp9Result = null;
  if (message.isPoll()) {
    ...
    tmp9Result = tmp9(channel(16594), obj5);
  }
  items3[3] = tmp9Result;
  obj[1] = items3;
  return closure_12(closure_4, obj);
...
function GuildChannelMessageRowLabel(arg0) {
  ...
  if ("username" === stateFromStores) {
    ...
    const searchMessageTimestamp = tmp2(16593).useSearchMessageTimestamp(message, channel);
    ...
    obj3[4] = tmp22;
    obj3[5] = tmp7;
    items1[1] = callback(tmp2(4477).Text, obj3);
    obj1[1] = items1;
    ...
    obj4[5] = timestamp;
    items2[1] = callback(tmp2(4477).Text, obj4);
    let tmp21Result = null;
    if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
      const obj5 = { size: "xs", style: null };
      obj5[1] = tmp.suppressNotificationsIcon;
      tmp21Result = tmp21(tmp2(13215).BellZIcon, obj5);
    }
