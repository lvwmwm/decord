...
// Dependencies: [19, 17, 5779, 10254, 11585, 9281, 676, 21, 4255, 12, 10253, 15565, 1236, 10255, 15566, 10268, 11576, 15641, 15548, 5339, 4116, 11584, 10262, 10263, 1231, 15642, 15643, 8939, 589, 15644, 15645, 15646, 15647, 2]
// Exports: default

// Module 15564 (NoSearchResultsScreen)
...
import jsxProd from "module_11584";
...
function NoSearchResultsScreen(searchContext) {
  ...
  return callback(importDefault(15565), obj);
}
...
let closure_21 = importAllResult.memo((searchContext) => {
  ...
  const memo = width.useMemo(() => {
    ...
    const item = setActiveIndex.forEach((tab) => {
      ...
      obj[0] = visibleTabs(10255).getTabTitle(tab);
      obj[1] = tab;
      obj = { tab, searchContext: items, width: outer1_3 };
      obj[2] = sharedValue(setActiveIndex(15566), obj);
      let tmp2;
      if (outer1_2 != null) {
        tmp2 = outer1_2[tab];
      }
  ...
  context = width.useContext(visibleTabs(15548).SwipeForMemberListContext);
  obj = context;
  if (context == null) {
    obj = {};
  }
  ...
  if (0 === segmentedControlState.items.length) {
    ...
    const items11 = [sharedValue(setActiveIndex(15642), obj2), ];
    let obj3 = { state: null };
    obj3[0] = segmentedControlState;
    items11[1] = sharedValue(setActiveIndex(15643), obj3);
    obj1[1] = items11;
    ...
    obj6[5] = tmp21;
    obj5[1] = sharedValue(tmp4(8939).SegmentedControlPages, obj6);
    obj4[1] = sharedValue(tmp4(15566).SearchTabsPageContext.Provider, obj5);
...
export default function ConnectedSearchTabsLayout(width) {
  ...
  const autoSearchGuildChannelTab = searchContext(15644).useAutoSearchGuildChannelTab(searchContext, !memo.has(constants.GUILD_CHANNELS));
  const obj3 = searchContext(15644);
  const autoSearchMembersTab = searchContext(15645).useAutoSearchMembersTab(searchContext, !memo.has(constants.MEMBERS));
  const obj4 = searchContext(15645);
  const autoSearchPeopleTab = searchContext(15646).useAutoSearchPeopleTab(searchContext, !memo.has(constants.PEOPLE));
  const obj5 = searchContext(15646);
  const autoTrackSearchTabCountsViewedAnalytics = searchContext(15647).useAutoTrackSearchTabCountsViewedAnalytics({ searchContext, visibleTabCounts, visibleTabs });
