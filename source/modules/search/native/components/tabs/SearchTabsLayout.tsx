...
// Dependencies: [19, 17, 6096, 11873, 11898, 7590, 676, 21, 4448, 12, 11894, 16187, 1236, 11874, 16188, 11895, 11872, 16263, 16172, 5620, 4187, 11897, 11881, 11882, 1231, 16264, 16265, 9229, 589, 16266, 16267, 16268, 16269, 2]
// Exports: default

// Module 16186 (NoSearchResultsScreen)
...
import closure_7 from "handleSearchQuery" /* 11898 */;
...
function NoSearchResultsScreen(searchContext) {
  ...
  return callback(_modDef16187, obj);
}
...
let closure_21 = importAllResult.memo((searchContext) => {
  ...
  const memo = width.useMemo(() => {
    ...
    const item = setActiveIndex.forEach((tab) => {
      let obj = { label: visibleTabs(11874).getTabTitle(tab), id: tab, page: null, count: null };
      obj = { tab, searchContext: items, width: closure_1_3 };
      obj[2] = sharedValue(setActiveIndex(16188), obj);
      let tmp2;
      if (closure_1_2 != null) {
  ...
  context = width.useContext(visibleTabs(16172).SwipeForMemberListContext);
  obj = context;
  if (context == null) {
    obj = {};
  }
  ...
  sharedValue = visibleTabs(4187).useSharedValue(false);
  class D {
    constructor(arg0) {
  ...
  if (0 === segmentedControlState.items.length) {
    ...
    const items11 = [sharedValue(setActiveIndex(16264), obj2), ];
    let obj3 = { state: null };
    obj3[0] = segmentedControlState;
    items11[1] = sharedValue(setActiveIndex(16265), obj3);
    obj1[1] = items11;
    ...
    obj6[5] = tmp21;
    obj5[1] = sharedValue(tmp4(9229).SegmentedControlPages, obj6);
    obj4[1] = sharedValue(tmp4(16188).SearchTabsPageContext.Provider, obj5);
...
export default function ConnectedSearchTabsLayout(width) {
  ...
  const autoSearchGuildChannelTab = searchContext(16266).useAutoSearchGuildChannelTab(searchContext, !memo.has(constants.GUILD_CHANNELS));
  const obj3 = searchContext(16266);
  const autoSearchMembersTab = searchContext(16267).useAutoSearchMembersTab(searchContext, !memo.has(constants.MEMBERS));
  const obj4 = searchContext(16267);
  const autoSearchPeopleTab = searchContext(16268).useAutoSearchPeopleTab(searchContext, !memo.has(constants.PEOPLE));
  const obj5 = searchContext(16268);
  const autoTrackSearchTabCountsViewedAnalytics = searchContext(16269).useAutoTrackSearchTabCountsViewedAnalytics({ searchContext, visibleTabCounts, visibleTabs });
