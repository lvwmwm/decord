...
// Dependencies: [19, 17, 5839, 11644, 11669, 8337, 676, 21, 4302, 12, 11665, 15638, 1236, 11645, 15639, 11666, 11643, 15714, 15621, 5426, 4162, 11668, 11652, 11653, 1231, 15715, 15716, 9497, 589, 15717, 15718, 15719, 15720, 2]
// Exports: default

// Module 15637 (NoSearchResultsScreen)
...
import jsxProd from "module_11668";
...
function NoSearchResultsScreen(searchContext) {
  ...
  return callback(importDefault(15638), obj);
}
...
let closure_21 = importAllResult.memo((searchContext) => {
  ...
  const memo = width.useMemo(() => {
    ...
    const item = setActiveIndex.forEach((tab) => {
      ...
      obj[0] = visibleTabs(11645).getTabTitle(tab);
      obj[1] = tab;
      obj = { tab, searchContext: items, width: outer1_3 };
      obj[2] = sharedValue(setActiveIndex(15639), obj);
      let tmp2;
      if (outer1_2 != null) {
        tmp2 = outer1_2[tab];
      }
  ...
  context = width.useContext(visibleTabs(15621).SwipeForMemberListContext);
  obj = context;
  if (context == null) {
    obj = {};
  }
  ...
  sharedValue = visibleTabs(4162).useSharedValue(false);
  class D {
    constructor(arg0) {
  ...
  if (0 === segmentedControlState.items.length) {
    ...
    const items11 = [sharedValue(setActiveIndex(15715), obj2), ];
    let obj3 = { state: null };
    obj3[0] = segmentedControlState;
    items11[1] = sharedValue(setActiveIndex(15716), obj3);
    obj1[1] = items11;
    ...
    obj6[5] = tmp21;
    obj5[1] = sharedValue(tmp4(9497).SegmentedControlPages, obj6);
    obj4[1] = sharedValue(tmp4(15639).SearchTabsPageContext.Provider, obj5);
...
export default function ConnectedSearchTabsLayout(width) {
  ...
  const autoSearchGuildChannelTab = searchContext(15717).useAutoSearchGuildChannelTab(searchContext, !memo.has(constants.GUILD_CHANNELS));
  const obj3 = searchContext(15717);
  const autoSearchMembersTab = searchContext(15718).useAutoSearchMembersTab(searchContext, !memo.has(constants.MEMBERS));
  const obj4 = searchContext(15718);
  const autoSearchPeopleTab = searchContext(15719).useAutoSearchPeopleTab(searchContext, !memo.has(constants.PEOPLE));
  const obj5 = searchContext(15719);
  const autoTrackSearchTabCountsViewedAnalytics = searchContext(15720).useAutoTrackSearchTabCountsViewedAnalytics({ searchContext, visibleTabCounts, visibleTabs });
