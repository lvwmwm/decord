...
// Dependencies: [19, 17, 6128, 11906, 11931, 7622, 676, 21, 4478, 12, 11927, 16222, 1236, 11907, 16223, 11928, 11905, 16298, 16207, 5652, 4217, 11930, 11914, 11915, 1231, 16299, 16300, 9268, 589, 16301, 16302, 16303, 16304, 2]
// Exports: default

// Module 16221 (NoSearchResultsScreen)
...
function NoSearchResultsScreen(searchContext) {
  ...
  return callback(_modDef16222, obj);
}
...
let closure_21 = importAllResult.memo((searchContext) => {
  ...
  const memo = width.useMemo(() => {
    ...
    const item = setActiveIndex.forEach((tab) => {
      ...
      obj[2] = sharedValue(setActiveIndex(16223), obj);
      let tmp2;
      if (closure_1_2 != null) {
  ...
  context = width.useContext(visibleTabs(16207).SwipeForMemberListContext);
  obj = context;
  if (context == null) {
    obj = {};
  }
  ...
  if (0 === segmentedControlState.items.length) {
    ...
    obj6[5] = tmp21;
    obj5[1] = sharedValue(tmp4(9268).SegmentedControlPages, obj6);
    obj4[1] = sharedValue(tmp4(16223).SearchTabsPageContext.Provider, obj5);
