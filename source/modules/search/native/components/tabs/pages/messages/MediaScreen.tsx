// Module ID: 16186
// Function ID: 16187
// Dependencies: [19, 6127, 1391, 6080, 11823, 7555, 11837, 676, 21, 16182, 16142, 16187, 589, 16188, 11822, 4299, 11824, 8499, 16139, 14376, 8613, 8615, 16189, 16193, 16146, 2]

// Module 16186
import importAllResult from "noop" /* 19 */;
import closure_4 from "initialize" /* 6127 */;
...
const memoResult = importAllResult.memo(function MediaScreen(searchContext) {
  ...
  let obj = searchContext(16182);
  const contentContainerStyles = obj.useContentContainerStyles();
  const tmp2 = tab(16142)(width);
  dependencyMap = tmp2;
  searchMessages = searchContext(16187).useSearchMessages(searchContext, tab);
  const obj2 = searchContext(16187);
  ...
  const searchMessagesLoadingState = searchContext(16188).useSearchMessagesLoadingState(obj);
  ...
  memo = searchMessages.useMemo(() => {
    if (null != searchMessages) {
      let media = searchContext(11822).getMedia(searchContext, tmp);
      const obj = searchContext(11822);
    } else {
      media = closure_1_12;
    }
    return media;
  ...
  const callback = searchMessages.useCallback(() => {
    const nextMessages = tab(11822).fetchNextMessages(searchContext, tab, () => {
  ...
  let obj4 = searchContext(16188);
  onPressMediaItem = searchContext(16139).useOnPressMediaItem({ searchContext, allMediaResults: memo, onEndReached: callback, onEndReachedThreshold: 500 });
  ...
  callback1 = searchMessages.useCallback((media) => {
    ...
    let obj = searchContext(14376);
    if (obj.shouldAgeVerifyForSearchMedia(media, found)) {
      obj = { entryPoint: null };
      obj[0] = tmp2(8615).AgeVerificationModalEntryPoint.SEARCH_MEDIA_PREVIEW;
      const result = tab(8613).showAgeVerificationGetStartedModal(obj);
      const obj4 = tab(8613);
    } else {
      ...
      obj[3] = arg1;
      const result1 = tmp2(16189).trackMessageItemPress(obj);
      onPressMediaItem(media, media.originView);
      const tmp2Result = tmp2(16189);
    }
  }, items4);
  ...
  const memo1 = searchMessages.useMemo(() => {
    ...
    if (placeholderCount > 0) {
      ...
      obj[1] = items.length;
      obj[2] = tmp2;
      const adjustedPlaceholderCount = searchContext(16193).getAdjustedPlaceholderCount(obj);
      ...
      const obj5 = searchContext(16193);
    }
    return items;
  }, items5);
  ...
  const obj6 = searchContext(16139);
  obj[5] = searchContext(16146).MediaVerticalSeparator;
  obj[6] = closure_9;
  obj[7] = isFirstPageLoading;
  obj[8] = isNextPageLoading;
  return jsx(tab(16189), { data: memo1, searchContext, tab, isFocused, contentContainerStyle: contentContainerStyles.mediaContentContainer, ItemSeparatorComponent: null, numColumns: null, isFirstPageLoading: null, isNextPageLoading: null });
});
