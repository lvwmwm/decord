// Module ID: 16238
// Function ID: 16239
// Dependencies: [19, 6143, 1387, 6096, 11873, 7590, 11887, 676, 21, 16234, 16194, 16239, 589, 16240, 11872, 4302, 11874, 8535, 16191, 14394, 8650, 8652, 16241, 16245, 16198, 2]

// Module 16238
import importAllResult from "noop" /* 19 */;
import closure_4 from "initialize" /* 6143 */;
...
import { EMPTY_MEDIA_RESULTS } from "SEARCH_TEXT_INPUT_DEBOUNCE_TIME" /* 11887 */;
...
const memoResult = importAllResult.memo(function MediaScreen(searchContext) {
  ...
  let obj = searchContext(16234);
  const contentContainerStyles = obj.useContentContainerStyles();
  const tmp2 = tab(16194)(width);
  dependencyMap = tmp2;
  searchMessages = searchContext(16239).useSearchMessages(searchContext, tab);
  const obj2 = searchContext(16239);
  ...
  const searchMessagesLoadingState = searchContext(16240).useSearchMessagesLoadingState(obj);
  ...
  memo = searchMessages.useMemo(() => {
    if (null != searchMessages) {
      let media = searchContext(11872).getMedia(searchContext, tmp);
      const obj = searchContext(11872);
    } else {
      media = closure_1_12;
    }
    return media;
  ...
  const callback = searchMessages.useCallback(() => {
    const nextMessages = tab(11872).fetchNextMessages(searchContext, tab, () => {
  ...
  let obj4 = searchContext(16240);
  onPressMediaItem = searchContext(16191).useOnPressMediaItem({ searchContext, allMediaResults: memo, onEndReached: callback, onEndReachedThreshold: 500 });
  ...
  callback1 = searchMessages.useCallback((media) => {
    ...
    let obj = searchContext(14394);
    if (obj.shouldAgeVerifyForSearchMedia(media, found)) {
      obj = { entryPoint: null };
      obj[0] = tmp2(8652).AgeVerificationModalEntryPoint.SEARCH_MEDIA_PREVIEW;
      const result = tab(8650).showAgeVerificationGetStartedModal(obj);
      const obj4 = tab(8650);
    } else {
      ...
      obj[3] = arg1;
      const result1 = tmp2(16241).trackMessageItemPress(obj);
      onPressMediaItem(media, media.originView);
      const tmp2Result = tmp2(16241);
    }
  }, items4);
  ...
  const memo1 = searchMessages.useMemo(() => {
    ...
    if (placeholderCount > 0) {
      ...
      obj[1] = items.length;
      obj[2] = tmp2;
      const adjustedPlaceholderCount = searchContext(16245).getAdjustedPlaceholderCount(obj);
      ...
      const obj5 = searchContext(16245);
    }
    return items;
  }, items5);
  ...
  const obj6 = searchContext(16191);
  obj[5] = searchContext(16198).MediaVerticalSeparator;
  obj[6] = closure_9;
  obj[7] = isFirstPageLoading;
  obj[8] = isNextPageLoading;
  return jsx(tab(16241), { data: memo1, searchContext, tab, isFocused, contentContainerStyle: contentContainerStyles.mediaContentContainer, ItemSeparatorComponent: null, numColumns: null, isFirstPageLoading: null, isNextPageLoading: null });
});
