// Module ID: 16123
// Function ID: 16124
// Dependencies: [19, 6120, 1391, 6072, 11979, 8622, 11993, 676, 21, 16119, 16079, 16124, 589, 16125, 11978, 4298, 11980, 9060, 16076, 14349, 8009, 8011, 16126, 16130, 16083, 2]

// Module 16123
import importAllResult from "noop" /* 19 */;
import closure_4 from "initialize" /* 6120 */;
import closure_5 from "ensureGuildLoaded" /* 1391 */;
import closure_6 from "handleReaction" /* 6072 */;
import closure_7 from "prototype" /* 11979 */;
...
const memoResult = importAllResult.memo(function MediaScreen(searchContext) {
  ...
  let obj = searchContext(16119);
  const contentContainerStyles = obj.useContentContainerStyles();
  const tmp2 = tab(16079)(width);
  dependencyMap = tmp2;
  searchMessages = searchContext(16124).useSearchMessages(searchContext, tab);
  const obj2 = searchContext(16124);
  ...
  const searchMessagesLoadingState = searchContext(16125).useSearchMessagesLoadingState(obj);
  ...
  memo = searchMessages.useMemo(() => {
    if (null != searchMessages) {
      let media = searchContext(11978).getMedia(searchContext, tmp);
      const obj = searchContext(11978);
    } else {
      media = closure_1_12;
    }
    return media;
  ...
  const callback = searchMessages.useCallback(() => {
    const nextMessages = tab(11978).fetchNextMessages(searchContext, tab, () => {
  ...
  let obj4 = searchContext(16125);
  onPressMediaItem = searchContext(16076).useOnPressMediaItem({ searchContext, allMediaResults: memo, onEndReached: callback, onEndReachedThreshold: 500 });
  ...
  callback1 = searchMessages.useCallback((media) => {
    ...
    let obj = searchContext(14349);
    if (obj.shouldAgeVerifyForSearchMedia(media, found)) {
      obj = { entryPoint: null };
      obj[0] = tmp2(8011).AgeVerificationModalEntryPoint.SEARCH_MEDIA_PREVIEW;
      const result = tab(8009).showAgeVerificationGetStartedModal(obj);
      const obj4 = tab(8009);
    } else {
      ...
      obj[3] = arg1;
      const result1 = tmp2(16126).trackMessageItemPress(obj);
      onPressMediaItem(media, media.originView);
      const tmp2Result = tmp2(16126);
    }
  }, items4);
  ...
  const memo1 = searchMessages.useMemo(() => {
    ...
    if (placeholderCount > 0) {
      ...
      obj[1] = items.length;
      obj[2] = tmp2;
      const adjustedPlaceholderCount = searchContext(16130).getAdjustedPlaceholderCount(obj);
      ...
      const obj5 = searchContext(16130);
    }
    return items;
  }, items5);
  ...
  const obj6 = searchContext(16076);
  obj[5] = searchContext(16083).MediaVerticalSeparator;
  obj[6] = closure_9;
  obj[7] = isFirstPageLoading;
  obj[8] = isNextPageLoading;
  return jsx(tab(16126), { data: memo1, searchContext, tab, isFocused, contentContainerStyle: contentContainerStyles.mediaContentContainer, ItemSeparatorComponent: null, numColumns: null, isFirstPageLoading: null, isNextPageLoading: null });
});
