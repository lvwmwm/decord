...
// Function ID: 15796
// Name: SearchTabsPage
// Dependencies: [32, 19, 17, 1391, 8442, 676, 21, 4344, 589, 5985, 4557, 11016, 12082, 15796, 15838, 15840, 15844, 15845, 15853, 15855, 15856, 15862, 15869, 38, 8777, 2]
// Exports: default
    ...
    if (tab !== SearchTabs.MEMBERS) {
      if (searchContext.type === SearchTypes.GUILD_CHANNEL) {
        if (tmp9) {
          ...
          return jsx(importDefault(11016), { guildId: null, channelId: null });
        } else if (isChannelSpoilerGated) {
          obj = { guildId: null, channelId: null };
          ({ guildId: obj13[0], channelId: obj13[1] } = searchContext);
          return jsx(importDefault(12082), { guildId: null, channelId: null });
        }
      }
    }
...
export default function ConnectedSearchTabsPage(tab) {
  ...
  obj = { isFocused: importDefault(8777)(context.selectedTab) === tab, selectMediaTab: context.selectMediaTab, tab, searchContext, width };
  obj[1] = <SearchTabsPage isFocused={importDefault(8777)(context.selectedTab) === tab} selectMediaTab={context.selectMediaTab} tab={tab} searchContext={searchContext} width={width} />;
  return <View isFocused={importDefault(8777)(context.selectedTab) === tab} selectMediaTab={context.selectMediaTab} tab={tab} searchContext={searchContext} width={width} />;
};
