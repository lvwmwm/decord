...
// Dependencies: [1992, 4098, 1922, 11979, 8622, 676, 1236, 11309, 4387, 10679, 10229, 10013, 7926, 4983, 7921, 7918, 11457, 10683, 16157, 11980, 4288, 2]
...
// Module 16156 (getSearchQueryChannelIds)
...
import { SearchListItemTypes } from "MessageEmbedTypes" /* 8622 */;
...
export const getSearchFilterAuthorTypeIcon = function getSearchFilterAuthorTypeIcon(text) {
  ...
  if (intl.string(getSystemLocale.t.tPZo4p) === text) {
    return tmp(11457).UserIcon;
  } else {
    const intl3 = tmp(1236).intl;
    if (intl3.string(tmp(1236).t.JL7sRS) === text) {
      return tmp(10683).RobotIcon;
    } else {
      const intl2 = tmp(1236).intl;
      if (intl2.string(tmp(1236).t.WjkIKU) === text) {
        return tmp(16157).WebhookIcon;
      }
    }
  }
};
