...
// Dependencies: [1992, 4098, 1922, 11823, 7555, 676, 1236, 11187, 4388, 9292, 10350, 10115, 10113, 4988, 12007, 10117, 11312, 9296, 16220, 11824, 4289, 2]
...
// Module 16219 (getSearchQueryChannelIds)
...
import { SearchListItemTypes } from "MessageEmbedTypes" /* 7555 */;
...
export const getSearchFilterAuthorTypeIcon = function getSearchFilterAuthorTypeIcon(text) {
  ...
  if (intl.string(getSystemLocale.t.tPZo4p) === text) {
    return tmp(11312).UserIcon;
  } else {
    const intl3 = tmp(1236).intl;
    if (intl3.string(tmp(1236).t.JL7sRS) === text) {
      return tmp(9296).RobotIcon;
    } else {
      const intl2 = tmp(1236).intl;
      if (intl2.string(tmp(1236).t.WjkIKU) === text) {
        return tmp(16220).WebhookIcon;
      }
    }
  }
};
