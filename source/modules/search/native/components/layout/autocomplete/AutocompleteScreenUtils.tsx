...
// Dependencies: [1942, 3892, 1874, 10254, 9281, 676, 1236, 10058, 4199, 9206, 9834, 9836, 7623, 4767, 7618, 7615, 11146, 9210, 15650, 10255, 4094, 2]
...
// Module 15649 (getSearchQueryChannelIds)
...
export const getSearchFilterAuthorTypeIcon = function getSearchFilterAuthorTypeIcon(text) {
  ...
  if (intl.string(require(1236) /* getSystemLocale */.t.tPZo4p) === text) {
    return tmp(11146).UserIcon;
  } else {
    const intl3 = tmp(1236).intl;
    if (intl3.string(tmp(1236).t.JL7sRS) === text) {
      return tmp(9210).RobotIcon;
    } else {
      const intl2 = tmp(1236).intl;
      if (intl2.string(tmp(1236).t.WjkIKU) === text) {
        return tmp(15650).WebhookIcon;
      }
    }
  }
};
export const toSearchListUserItem = function toSearchListUserItem(items, user, callback2) {
  ...
  if (null == user) {
    ...
    if (nickname == null) {
      nickname = importDefault(4094).getName(user);
