...
// Dependencies: [5899, 11710, 8397, 11724, 676, 11711, 8846, 1384, 6004, 500, 1872, 4247, 1467, 9533, 712, 3974, 11731, 11734, 11718, 12, 643, 2]
...
// Module 11709 (performKeyboardAwareNavigation)
...
function getUrlIcon(target) {
  if (null == target) {
    return require(4247) /* LinkIcon */.LinkIcon;
  } else {
    if (null == obj.safeParseWithQuery(target)) {
      return require(4247) /* LinkIcon */.LinkIcon;
    } else {
      if (tmp10Result.isDiscordUrl(target)) {
        return require(9533) /* ClydeIcon */.ClydeIcon;
      } else {
        let num = 0;
        if (0 < length.length) {
...
function onFetchMessagesStart(searchQueryString) {
  ...
  importDefault(11734).updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
function fetchInitialMessages(closure_0) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    let obj = _require(11711);
    const searchTabFetchId = obj.getSearchTabFetchId(closure_0, tmp3[0], queryString);
    let obj1 = store;
    if (!store.getIsFetching(searchTabFetchId)) {
      const result = queryString(11718).clearAllSearchMesssages();
      const obj3 = queryString(11718);
      obj = { searchContext: null };
      obj[0] = closure_0;
      queryString(11731).trackSearchStarted(obj);
      const obj4 = queryString(11731);
...
function syncAutocomplete(closure_0) {
  ...
  const selectionScope = require(11711) /* SearchTokenTypes */.getSelectionScope(tokenizeQueryResult, queryString.length - 1, queryString.length - 1);
  ...
  const result = importDefault(11718).updateAutocompleteQuery(obj);
}
...
obj[9] = function fetchNextMessages(searchContext, tab, arg2) {
  ...
  const searchTabFetchId1 = _require(11711).getSearchTabFetchId(searchContext, tab, queryString);
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  let obj = importDefault(11718);
  const result = obj.initializeAutocomplete(closure_9);
  const result1 = importDefault(11734).initializeSearchQuery(closure_9);
...
obj[13] = function subscribeSearchQueryState(searchContext, arg1, arg2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f88286(callback.getManager(closure_0));
    if (null == obj) {
      f88287(tmp, tmp);
    }
  }
...
obj[14] = function subscribeTextInputValue(closure_0, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f88286 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
  const f88287 = (textInputValue, textInputValue2) => {
...
export const getMedia = function getMedia(items, items1) {
  guildIdFromSearchContext = guildIdFromSearchContext(11711).getGuildIdFromSearchContext(items);
  items = [];
