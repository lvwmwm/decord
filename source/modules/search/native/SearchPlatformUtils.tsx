...
// Dependencies: [5820, 11629, 8320, 11643, 676, 11630, 8769, 1384, 5925, 500, 1872, 4229, 1467, 9451, 712, 3957, 11650, 11653, 11637, 12, 643, 2]
...
// Module 11628 (performKeyboardAwareNavigation)
...
function onFetchMessagesStart(searchQueryString) {
  ...
  importDefault(11653).updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
function fetchInitialMessages(closure_0) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    if (!store.getIsFetching(searchTabFetchId)) {
      const result = queryString(11637).clearAllSearchMesssages();
      const obj3 = queryString(11637);
      obj = { searchContext: null };
      obj[0] = closure_0;
      queryString(11650).trackSearchStarted(obj);
      const obj4 = queryString(11650);
      ...
      obj1[0] = cursor;
      obj[7] = obj1;
      const obj6 = queryString(11637);
      obj[8] = _require(3957).SearchResultExactCountEnabled.getSetting();
      obj[9] = constants2.NEWEST;
      const tabMessages = obj6.fetchTabMessages(obj);
      const SearchResultExactCountEnabled = _require(3957).SearchResultExactCountEnabled;
    }
  }
}
function syncAutocomplete(closure_0) {
  const queryString = store2.getQueryString(closure_0, true);
  let obj = require(11630) /* SearchTokenTypes */;
  const tokenizeQueryResult = obj.tokenizeQuery(queryString);
  const selectionScope = require(11630) /* SearchTokenTypes */.getSelectionScope(tokenizeQueryResult, queryString.length - 1, queryString.length - 1);
  ...
  const result = importDefault(11637).updateAutocompleteQuery(obj);
}
...
obj[9] = function fetchNextMessages(searchContext, tab, arg2) {
  ...
  const searchTabFetchId1 = _require(11630).getSearchTabFetchId(searchContext, tab, queryString);
  if (!store.getIsInitialFetchComplete(searchTabFetchId1)) {
    ...
    if (!flag) {
      ...
      obj[0] = cursor;
      obj[7] = obj;
      const obj4 = queryString(11637);
      obj[8] = tmp2(3957).SearchResultExactCountEnabled.getSetting();
      obj[9] = constants2.NEWEST;
      tabMessages = obj4.fetchTabMessages(obj);
      const SearchResultExactCountEnabled2 = tmp2(3957).SearchResultExactCountEnabled;
    }
    return tabMessages;
  } else {
    ...
    SearchResultExactCountEnabled = tmp2(3957).SearchResultExactCountEnabled;
  }
};
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  let obj = importDefault(11637);
  const result = obj.initializeAutocomplete(closure_9);
  const result1 = importDefault(11653).initializeSearchQuery(closure_9);
...
obj[13] = function subscribeSearchQueryState(searchContext, arg1, arg2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f87920(callback.getManager(closure_0));
    if (null == obj) {
      f87921(tmp, tmp);
    }
  }
...
obj[14] = function subscribeTextInputValue(closure_0, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f87920 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
  const f87921 = (textInputValue, textInputValue2) => {
...
export const getMedia = function getMedia(items, items1) {
  guildIdFromSearchContext = guildIdFromSearchContext(11630).getGuildIdFromSearchContext(items);
  items = [];
