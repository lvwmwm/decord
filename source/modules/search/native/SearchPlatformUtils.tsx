...
// Dependencies: [6072, 11979, 8622, 11993, 676, 11980, 9061, 1403, 6179, 500, 1892, 4387, 1487, 9751, 712, 4134, 12000, 12003, 11987, 12, 643, 2]
...
// Module 11978 (performKeyboardAwareNavigation)
...
import _modDef12003 from "module_12003" /* 12003 */;
import closure_3 from "handleReaction" /* 6072 */;
import closure_4 from "prototype" /* 11979 */;
import MessageEmbedTypes from "MessageEmbedTypes" /* 8622 */;
import SEARCH_TEXT_INPUT_DEBOUNCE_TIME from "SEARCH_TEXT_INPUT_DEBOUNCE_TIME" /* 11993 */;
...
function onFetchMessagesStart(searchQueryString) {
  ...
  _modDef12003.updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
function fetchInitialMessages(closure_0) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    if (!store.getIsFetching(searchTabFetchId)) {
      const result = queryString(11987).clearAllSearchMesssages();
      const obj3 = queryString(11987);
      obj = { searchContext: null };
      obj[0] = closure_0;
      queryString(12000).trackSearchStarted(obj);
      const obj4 = queryString(12000);
      ...
      obj1[0] = cursor;
      obj[7] = obj1;
      const obj6 = queryString(11987);
      obj[8] = _require(4134).SearchResultExactCountEnabled.getSetting();
      obj[9] = constants2.NEWEST;
      const tabMessages = obj6.fetchTabMessages(obj);
      const SearchResultExactCountEnabled = _require(4134).SearchResultExactCountEnabled;
    }
  }
}
function syncAutocomplete(closure_0) {
  ...
  const result = _modDef11987.updateAutocompleteQuery(obj);
}
...
let obj = {
  ...
  fetchNextMessages(searchContext, tab, arg2) {
    ...
    const searchTabFetchId1 = _require(11980).getSearchTabFetchId(searchContext, tab, queryString);
    if (!store.getIsInitialFetchComplete(searchTabFetchId1)) {
      ...
      if (!flag) {
        ...
        obj[0] = cursor;
        obj[7] = obj;
        const obj4 = queryString(11987);
        obj[8] = tmp2(4134).SearchResultExactCountEnabled.getSetting();
        obj[9] = constants2.NEWEST;
        tabMessages = obj4.fetchTabMessages(obj);
        const SearchResultExactCountEnabled2 = tmp2(4134).SearchResultExactCountEnabled;
      }
      return tabMessages;
    } else {
      ...
      SearchResultExactCountEnabled = tmp2(4134).SearchResultExactCountEnabled;
    }
  },
  syncAutocomplete,
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  let obj = _modDef11987;
  const result = obj.initializeAutocomplete(closure_9);
  const result1 = _modDef12003.initializeSearchQuery(closure_9);
...
obj[13] = function subscribeSearchQueryState(searchContext, arg1, arg2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f89910(callback.getManager(closure_0));
    if (null == closure_3) {
      closure_3 = tmp;
      f89911(tmp, closure_3);
    }
  }
...
obj[14] = function subscribeTextInputValue(closure_0, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f89910 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
  const f89911 = (textInputValue, textInputValue2) => {
...
export const getMedia = function getMedia(items, items1) {
  guildIdFromSearchContext = guildIdFromSearchContext(11980).getGuildIdFromSearchContext(items);
  items = [];
