...
// Dependencies: [5899, 11711, 8397, 11725, 676, 11712, 8846, 1384, 6004, 500, 1872, 4247, 1467, 9533, 712, 3974, 11732, 11735, 11719, 12, 643, 2]
...
// Module 11710 (performKeyboardAwareNavigation)
...
function onFetchMessagesStart(searchQueryString) {
  ...
  importDefault(11735).updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
...
function syncAutocomplete(closure_0) {
  ...
  const selectionScope = require(11712) /* SearchTokenTypes */.getSelectionScope(tokenizeQueryResult, queryString.length - 1, queryString.length - 1);
  ...
  const result = importDefault(11719).updateAutocompleteQuery(obj);
}
...
obj[9] = function fetchNextMessages(searchContext, tab, arg2) {
  ...
  const searchTabFetchId1 = _require(11712).getSearchTabFetchId(searchContext, tab, queryString);
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  let obj = importDefault(11719);
  const result = obj.initializeAutocomplete(closure_9);
  const result1 = importDefault(11735).initializeSearchQuery(closure_9);
