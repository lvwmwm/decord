...
// Dependencies: [7221, 12271, 7810, 12285, 673, 12272, 8193, 1399, 6565, 1234, 1890, 4424, 1485, 10749, 709, 4166, 12290, 12293, 12279, 12, 640, 2]
...
// Module 12270 (performKeyboardAwareNavigation)
...
import closure_3 from "handleReaction" /* 7221 */;
import closure_4 from "prototype" /* 12271 */;
import MessageEmbedTypes from "MessageEmbedTypes" /* 7810 */;
import SEARCH_TEXT_INPUT_DEBOUNCE_TIME from "SEARCH_TEXT_INPUT_DEBOUNCE_TIME" /* 12285 */;
...
function onFetchMessagesStart(searchQueryString) {
  ...
  _modDef12293.updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
function fetchInitialMessages(closure_0) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    let obj = _require(12272);
    const searchTabFetchId = obj.getSearchTabFetchId(closure_0, tmp3[0], queryString);
    obj1 = store;
    if (!store.getIsFetching(searchTabFetchId)) {
      const result = queryString(12279).clearAllSearchMesssages();
      const obj3 = queryString(12279);
      obj = { searchContext: null };
      obj[0] = closure_0;
      queryString(12290).trackSearchStarted(obj);
      const obj4 = queryString(12290);
...
function syncAutocomplete(closure_0) {
  ...
  const result = _modDef12279.updateAutocompleteQuery(obj);
}
...
let obj = {
  ...
  fetchNextMessages(searchContext, tab, arg2) {
    ...
    const searchTabFetchId1 = _require(12272).getSearchTabFetchId(searchContext, tab, queryString);
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  let obj = _modDef12279;
  const result = obj.initializeAutocomplete(closure_9);
  const result1 = _modDef12293.initializeSearchQuery(closure_9);
...
obj[13] = function subscribeSearchQueryState(searchContext, arg1, arg2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f92914(callback.getManager(closure_0));
    if (null == closure_3) {
      closure_3 = tmp;
      f92915(tmp, closure_3);
    }
  }
...
obj[14] = function subscribeTextInputValue(closure_0, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f92914 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
  const f92915 = (textInputValue, textInputValue2) => {
...
export const getMedia = function getMedia(items, items1) {
  guildIdFromSearchContext = guildIdFromSearchContext(12272).getGuildIdFromSearchContext(items);
  items = [];
