...
// Dependencies: [5899, 11714, 8403, 11728, 676, 11715, 8852, 1403, 6006, 500, 1891, 4247, 1486, 9539, 712, 3993, 11735, 11738, 11722, 12, 643, 2]
...
// Module 11713 (performKeyboardAwareNavigation)
  ...
  if (null == target) {
    ...
    if (null == obj.safeParseWithQuery(target)) {
      ...
      if (tmp10Result.isDiscordUrl(target)) {
        return require(9539) /* ClydeIcon */.ClydeIcon;
      } else {
        let num = 0;
        if (0 < length.length) {
...
function onFetchMessagesStart(searchQueryString) {
  ...
  importDefault(11738).updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
function fetchInitialMessages(closure_0) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    if (!store.getIsFetching(searchTabFetchId)) {
      ...
      obj[8] = _require(3993).SearchResultExactCountEnabled.getSetting();
      obj[9] = constants2.NEWEST;
      const tabMessages = obj6.fetchTabMessages(obj);
      const SearchResultExactCountEnabled = _require(3993).SearchResultExactCountEnabled;
    }
  }
}
function syncAutocomplete(closure_0) {
  ...
  const selectionScope = require(11715) /* SearchTokenTypes */.getSelectionScope(tokenizeQueryResult, queryString.length - 1, queryString.length - 1);
  ...
  const result = importDefault(11722).updateAutocompleteQuery(obj);
}
...
obj[9] = function fetchNextMessages(searchContext, tab, arg2) {
  ...
  const searchTabFetchId1 = _require(11715).getSearchTabFetchId(searchContext, tab, queryString);
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  let obj = importDefault(11722);
  const result = obj.initializeAutocomplete(closure_9);
  const result1 = importDefault(11738).initializeSearchQuery(closure_9);
