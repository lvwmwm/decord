...
// Dependencies: [5938, 11771, 8442, 11785, 676, 11772, 8891, 1403, 6045, 500, 1891, 4288, 1486, 9579, 712, 4034, 11792, 11795, 11779, 12, 643, 2]
...
// Module 11770 (performKeyboardAwareNavigation)
...
function getUrlIcon(target) {
  if (null == target) {
    return require(4288) /* LinkIcon */.LinkIcon;
  } else {
    if (null == obj.safeParseWithQuery(target)) {
      return require(4288) /* LinkIcon */.LinkIcon;
    } else {
      if (tmp10Result.isDiscordUrl(target)) {
        return require(9579) /* ClydeIcon */.ClydeIcon;
      } else {
        let num = 0;
        if (0 < length.length) {
...
function onFetchMessagesStart(searchQueryString) {
  ...
  importDefault(11795).updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
function fetchInitialMessages(closure_0) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    let obj = _require(11772);
    const searchTabFetchId = obj.getSearchTabFetchId(closure_0, tmp3[0], queryString);
    let obj1 = store;
    if (!store.getIsFetching(searchTabFetchId)) {
      const result = queryString(11779).clearAllSearchMesssages();
      const obj3 = queryString(11779);
      obj = { searchContext: null };
      obj[0] = closure_0;
      queryString(11792).trackSearchStarted(obj);
      const obj4 = queryString(11792);
...
function syncAutocomplete(closure_0) {
  ...
  const selectionScope = require(11772) /* SearchTokenTypes */.getSelectionScope(tokenizeQueryResult, queryString.length - 1, queryString.length - 1);
  ...
  const result = importDefault(11779).updateAutocompleteQuery(obj);
}
...
obj[9] = function fetchNextMessages(searchContext, tab, arg2) {
  ...
  const searchTabFetchId1 = _require(11772).getSearchTabFetchId(searchContext, tab, queryString);
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  let obj = importDefault(11779);
  const result = obj.initializeAutocomplete(closure_9);
  const result1 = importDefault(11795).initializeSearchQuery(closure_9);
...
obj[13] = function subscribeSearchQueryState(searchContext, arg1, arg2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f88574(callback.getManager(closure_0));
    if (null == obj) {
      f88575(tmp, tmp);
    }
  }
...
obj[14] = function subscribeTextInputValue(closure_0, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f88574 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
  const f88575 = (textInputValue, textInputValue2) => {
...
export const getMedia = function getMedia(items, items1) {
  guildIdFromSearchContext = guildIdFromSearchContext(11772).getGuildIdFromSearchContext(items);
  items = [];
