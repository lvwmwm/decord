...
// Dependencies: [5940, 11770, 8446, 11784, 676, 11771, 8895, 1403, 6047, 500, 1891, 4286, 1486, 9583, 712, 4034, 11791, 11794, 11778, 12, 643, 2]
...
// Module 11769 (performKeyboardAwareNavigation)
...
function onFetchMessagesStart(searchQueryString) {
  ...
  importDefault(11794).updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
function fetchInitialMessages(closure_0) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    let obj = _require(11771);
    const searchTabFetchId = obj.getSearchTabFetchId(closure_0, tmp3[0], queryString);
    let obj1 = store;
    if (!store.getIsFetching(searchTabFetchId)) {
      const result = queryString(11778).clearAllSearchMesssages();
      const obj3 = queryString(11778);
      obj = { searchContext: null };
      obj[0] = closure_0;
      queryString(11791).trackSearchStarted(obj);
      const obj4 = queryString(11791);
...
function syncAutocomplete(closure_0) {
  ...
  const selectionScope = require(11771) /* SearchTokenTypes */.getSelectionScope(tokenizeQueryResult, queryString.length - 1, queryString.length - 1);
  ...
  const result = importDefault(11778).updateAutocompleteQuery(obj);
}
...
obj[9] = function fetchNextMessages(searchContext, tab, arg2) {
  ...
  const searchTabFetchId1 = _require(11771).getSearchTabFetchId(searchContext, tab, queryString);
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  ...
  const result1 = importDefault(11794).initializeSearchQuery(closure_9);
...
obj[13] = function subscribeSearchQueryState(searchContext, arg1, arg2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f88551(callback.getManager(closure_0));
    if (null == obj) {
      f88552(tmp, tmp);
    }
  }
...
obj[14] = function subscribeTextInputValue(closure_0, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f88551 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
  const f88552 = (textInputValue, textInputValue2) => {
...
export const getMedia = function getMedia(items, items1) {
  guildIdFromSearchContext = guildIdFromSearchContext(11771).getGuildIdFromSearchContext(items);
  items = [];
