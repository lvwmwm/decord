...
// Dependencies: [7281, 12339, 7878, 12353, 1074, 12340, 8263, 1384, 6625, 1115, 1874, 4503, 1365, 10818, 576, 1935, 12358, 12361, 12347, 12, 558, 2]
...
// Module 12338 (performKeyboardAwareNavigation)
...
import SEARCH_TEXT_INPUT_DEBOUNCE_TIME from "SEARCH_TEXT_INPUT_DEBOUNCE_TIME" /* 12353 */;
...
function onFetchMessagesStart(searchQueryString) {
  ...
  _modDef12361.updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
...
function syncAutocomplete(closure_0) {
  ...
  const result = _modDef12347.updateAutocompleteQuery(obj);
}
...
let obj = {
  ...
  fetchNextMessages(searchContext, tab, arg2) {
    ...
    const searchTabFetchId1 = _require(12340).getSearchTabFetchId(searchContext, tab, queryString);
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  let obj = _modDef12347;
  const result = obj.initializeAutocomplete(closure_9);
  const result1 = _modDef12361.initializeSearchQuery(closure_9);
...
obj[13] = function subscribeSearchQueryState(searchContext, arg1, arg2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f93178(callback.getManager(closure_0));
    if (null == closure_3) {
      closure_3 = tmp;
...
obj[14] = function subscribeTextInputValue(closure_0, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f93178 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
...
export const getMedia = function getMedia(items, items1) {
  guildIdFromSearchContext = guildIdFromSearchContext(12340).getGuildIdFromSearchContext(items);
  items = [];
