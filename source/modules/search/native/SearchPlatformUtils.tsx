...
// Dependencies: [5940, 11769, 8446, 11783, 676, 11770, 8895, 1403, 6047, 500, 1891, 4286, 1486, 9583, 712, 4034, 11790, 11793, 11777, 12, 643, 2]
...
// Module 11768 (performKeyboardAwareNavigation)
  ...
  if (null == target) {
    ...
    if (null == obj.safeParseWithQuery(target)) {
      ...
      if (tmp10Result.isDiscordUrl(target)) {
        return require(9583) /* ClydeIcon */.ClydeIcon;
      } else {
        let num = 0;
        if (0 < length.length) {
...
function onFetchMessagesStart(searchQueryString) {
  ...
  importDefault(11793).updateSearchQuery(searchQueryString.searchContext, (setSearchResultsQuery) => setSearchResultsQuery.setSearchResultsQuery(searchQueryString));
}
function fetchInitialMessages(closure_0) {
  ...
  if (isInitialSearchQueryResult) {
    ...
    let obj = _require(11770);
    const searchTabFetchId = obj.getSearchTabFetchId(closure_0, tmp3[0], queryString);
    let obj1 = store;
    if (!store.getIsFetching(searchTabFetchId)) {
      const result = queryString(11777).clearAllSearchMesssages();
      const obj3 = queryString(11777);
      obj = { searchContext: null };
      obj[0] = closure_0;
      queryString(11790).trackSearchStarted(obj);
      const obj4 = queryString(11790);
...
function syncAutocomplete(closure_0) {
  ...
  const selectionScope = require(11770) /* SearchTokenTypes */.getSelectionScope(tokenizeQueryResult, queryString.length - 1, queryString.length - 1);
  ...
  const result = importDefault(11777).updateAutocompleteQuery(obj);
}
...
obj[9] = function fetchNextMessages(searchContext, tab, arg2) {
  ...
  const searchTabFetchId1 = _require(11770).getSearchTabFetchId(searchContext, tab, queryString);
...
obj[12] = function navigateToSearchWithPrefetch(closure_8, closure_9) {
  let obj = importDefault(11777);
  const result = obj.initializeAutocomplete(closure_9);
  const result1 = importDefault(11793).initializeSearchQuery(closure_9);
...
obj[13] = function subscribeSearchQueryState(searchContext, arg1, arg2, arg3) {
  ...
  let tmp = arg3;
  function callback() {
    const tmp = f88571(callback.getManager(closure_0));
    if (null == obj) {
      f88572(tmp, tmp);
    }
  }
...
obj[14] = function subscribeTextInputValue(closure_0, debounceResult, arg2) {
  ...
  let flag = arg2;
  const f88571 = (getTextInputValue) => ({ textInputValue: getTextInputValue.getTextInputValue(), textInputChangedFromInput: getTextInputValue.getTextValueChangedFromInput() });
  const f88572 = (textInputValue, textInputValue2) => {
...
export const getMedia = function getMedia(items, items1) {
  guildIdFromSearchContext = guildIdFromSearchContext(11770).getGuildIdFromSearchContext(items);
  items = [];
