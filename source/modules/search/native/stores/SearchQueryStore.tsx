...
// Function ID: 11624
// Name: prototype
// Dependencies: [1372, 3922, 1874, 8208, 8207, 676, 4475, 1236, 11624, 11636, 1941, 589, 709, 2]

// Module 11623 (prototype)
...
const prototype = function SearchQueryStateManager(type) {
  ...
  if (constants3.THREAD !== type) {
    ...
    const tmp12 = new importDefault(11636)();
  ...
  if (isObfuscatedResult) {
    ...
    let channelName;
    if (null != channel) {
      channelName = obj(4475).computeChannelName(channel, mergeGuildAvatar, upsertRelationship);
      const obj2 = obj(4475);
    }
    if (null == channelName) {
      let items1 = [];
      ...
      obj[2] = "" + stringResult + ": " + obj(11624).quoteChannelName(channelName);
      obj[3] = type.channelId;
      obj[4] = SearchFilterAddLocations.CLIENT_AUTO_ADD;
      items1 = [obj];
      const obj4 = obj(11624);
    }
    items = items1;
  }
}.prototype;
...
prototype2["getManager"] = function getManager(closure_0) {
  let value = map.get(require(11624) /* SearchTokenTypes */.getSearchContextId(closure_0));
  if (value == null) {
...
prototype2["getQueryString"] = function getQueryString(searchContext) {
  ...
  let value = map.get(require(11624) /* SearchTokenTypes */.getSearchContextId(searchContext));
  if (value == null) {
...
areArraysShallowlyEqual = {
  SEARCH_QUERY_NATIVE_UPDATE: function handleSearchQueryNativeUpdate(searchContext) {
    ...
    const searchContextId = require(11624) /* SearchTokenTypes */.getSearchContextId(searchContext);
