...
// Dependencies: [17, 21, 4303, 712, 1916, 13925, 4725, 688, 2]
// Exports: default

// Module 13924 (DisplayNameStylesColorSwatch)
...
export default function DisplayNameStylesColorSwatch(colors) {
  ...
  const tmp = callback();
  if (colors.effectId === require(1916) /* DisplayNameEffect */.DisplayNameEffect.GUMMY) {
    if (colors.length > 0) {
      ...
      obj = { colors: null };
      obj[0] = colors;
      obj[1] = jsx(importDefault(13925), { colors: null });
  ...
  if (colors.length >= 2) {
    ...
    return jsx(importDefault(4725), { colors: null, start: null, end: null, style: null });
  } else {
    let str = "#000000";
    if (colors.length > 0) {
