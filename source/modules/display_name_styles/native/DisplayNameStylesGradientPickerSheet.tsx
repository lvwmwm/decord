...
export default function DisplayNameStylesColorPickerSheet(selectedEffectId) {
  ...
  closure_7 = memo.useCallback((arg0) => {
    ...
    onSelectColors(_undefined[13])({
      ...
      onSelect(arg0) {
        let closure_0 = arg0;
        const result = selectedEffectId(4158).triggerHapticFeedback(selectedEffectId(4158).HapticFeedbackTypes.IMPACT_MEDIUM);
  ...
  obj1[3] = callback;
  obj[1] = callback(selectedEffectId(4600).Button, obj1);
  obj[0] = callback(selectedEffectId(5242).BottomSheetTitleHeader, obj);
  ...
  obj4[2] = { x: 0, y: 0 };
  obj4[3] = { x: 1, y: 0 };
  const items4 = [callback(onSelectColors(4611), obj4), ];
  const obj5 = { style: tmp.dropperContainer, children: null };
  const tmp7 = onSelectColors(4611);
  ...
  obj7[1] = callback1;
  items5[2] = callback(selectedEffectId(4600).Button, obj7);
  obj2[1] = items5;
  obj[1] = callback2(first, obj2);
  return callback(selectedEffectId(5243).BottomSheet, obj);
};
