...
// Dependencies: [31, 27, 1850, 653, 1345, 33, 3884, 4011, 4101, 566, 3811, 1212, 2557, 6590, 5617, 3981, 1334, 5221, 5155, 5481, 6763, 477, 5119, 15729, 6187, 1273, 4161, 4578, 4165, 689, 2]
// Exports: default

// Module 15728 (DisplayNameStylesFlywheelMobileActionSheet)
...
export default function DisplayNameStylesFlywheelMobileActionSheet(markAsDismissed) {
  ...
  let enabled = React.useContext(markAsDismissed(3884).AccessibilityPreferencesContext).reducedMotion.enabled;
  let obj = markAsDismissed(4011);
  const isThemeDarkResult = obj.isThemeDark(ref(4101)());
  ...
  const items4 = [callback(markAsDismissed(5481).ActionSheetHeaderBar, obj2), , , , , ];
  const obj3 = { style: tmp3.imageContainer };
  let tmp14 = enabled;
  if (enabled) {
    const obj4 = { resizeMode: "contain" };
    tmp14 = callback(markAsDismissed(6763).DisplayNameStylesV2AbstractUI, obj4);
  }
  const items5 = [tmp14, ];
  if (!!enabled) {
    items5[1] = tmp18;
    ...
    obj6.children = intl2.string(ref(2557).Uzms61);
    items4[3] = callback(markAsDismissed(4161).Text, obj6);
    ...
    items4[4] = callback(markAsDismissed(4161).Text, obj7);
    ...
    const items6 = [callback(markAsDismissed(4578).Button, obj9), ];
    ...
    items6[1] = callback(markAsDismissed(4578).Button, obj10);
    obj8.children = items6;
    items4[5] = callback2(View, obj8);
    obj1.children = items4;
    obj.children = tmp12(tmp13, obj1);
    obj.children = tmp11(markAsDismissed(5155).SafeAreaPaddingView, obj);
    return tmp11(markAsDismissed(5221).BottomSheet, obj);
  } else {
    obj9 = markAsDismissed(477);
    if (obj9.isIOS()) {
      const obj11 = {};
      const obj12 = { uri: ref(15729) };
      obj11.source = obj12;
      obj11.style = tmp3.image;
      obj11.resizeMode = "contain";
      enabled = !enabled;
      obj11.enableAnimation = enabled;
      let tmp21Result = tmp21(ref(5119), obj11);
      const tmp28 = ref(5119);
    } else {
      const obj13 = { url: ref(15729), style: tmp3.image, autoplay: true };
      tmp21Result = tmp21(markAsDismissed(6187).APNGPlayer, obj13);
    }
  }
};
