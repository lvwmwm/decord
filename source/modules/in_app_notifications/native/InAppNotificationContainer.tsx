...
// Dependencies: [32, 19, 17, 9671, 4267, 9655, 676, 21, 4116, 4255, 9672, 7860, 7865, 9673, 9747, 9750, 9751, 9783, 9784, 10089, 10090, 10093, 10094, 9654, 8626, 589, 4071, 1358, 4223, 4449, 4662, 1474, 11, 5206, 4256, 4681, 698, 5353, 4664, 1297, 4653, 1236, 5257, 9702, 2]
// Exports: default

// Module 9670 (NotificationWrapper)
...
function NotificationWrapper(notification) {
  ...
  if (constants.MESSAGE === type) {
    ...
    if (tmp.RESTRICTED_HOURS_WARNING !== type) {
      if (tmp.RESTRICTED_SCHEDULE_UPDATED !== type) {
        if (tmp.MESSAGE_REQUEST === type) {
          obj = { notification: null };
          obj[0] = notification;
          return callback(importDefault(10093), obj);
        } else {
          return null;
        }
      }
    ...
    obj6[0] = notification;
    return callback(importDefault(10090), obj6);
  }
}
function DetachedNotificationContent(notification) {
  ...
  if (notification.type === constants.MESSAGE) {
    ...
    obj[0] = notification;
    tmp = callback(require(10094) /* MessageNotificationDetachedContent */.MessageNotificationDetachedContent, obj);
  }
  return tmp;
}
