...
function Progress(percent) {
  ...
  obj[2] = jsx(first(4050).View, { style: items1 });
...
function RedesignProgress(percent) {
  ...
  items[1] = { opacity: num };
  obj[1] = items;
  obj = { style: tmp.progressTrack, children: tmp7(first(4050).View, { style: items1 }) };
  items1 = [tmp.progressRedesign, "ALERT" === type ? tmp.progressBarWarning : tmp.progressBarBrand, animatedStyle];
  obj[2] = <View style={tmp.progressTrack}>{tmp7(first(4050).View, { style: items1 })}</View>;
  return <View style={tmp.progressTrack}>{tmp7(first(4050).View, { style: items1 })}</View>;
}
