...
// Function ID: 10104
// Name: VideoBadge
// Dependencies: [19, 17, 4473, 10024, 21, 4481, 709, 8204, 4477, 4197, 1234, 4926, 5042, 6910, 644, 7533, 10104, 10060, 8056, 8193, 1491, 5542, 7231, 7236, 8237, 10105, 1296, 8743, 2]
// Exports: MediaPreviewRightAccessory

// Module 10103 (VideoBadge)
...
import PlayIcon from "PlayIcon" /* 8204 */;
import VideoComponentDefault from "VideoComponent" /* 8237 */;
import SolidCutoutDefault from "SolidCutout" /* 8743 */;
import VoiceMessageIcon from "VoiceMessageIcon" /* 10060 */;
import useEnabledHarmTypesBitmaskForChannelAndAuthorId from "useEnabledHarmTypesBitmaskForChannelAndAuthorId" /* 10104 */;
...
function ObscuredMediaOverlay(isSpoiler) {
  ...
  if (isObscured) {
    let tmp14Result = tmp14(tmp2(5042).ImageWarningIcon, { size: "sm", color: "white" });
  } else {
    const obj2 = { style: null, children: null };
    obj2[0] = tmp.spoilerPill;
    obj2[1] = tmp14(tmp2(6910).EyeIcon, { size: "sm", color: "white" });
...
function SinglePreviewableMedia(arg0) {
  ...
  if (VoiceMessageIcon.PreviewableMediaTypes.VOICE_MESSAGE === type) {
    ...
    if (tmp2(10060).PreviewableMediaTypes.AUDIO !== type) {
      if (tmp2(10060).PreviewableMediaTypes.FILE !== type) {
        if (tmp2(10060).PreviewableMediaTypes.IMAGE !== type) {
          if (tmp2(10060).PreviewableMediaTypes.VIDEO !== type) {
            if (tmp2(10060).PreviewableMediaTypes.GIF === type) {
              ...
              if (null == thumbnail) {
                ...
                obj1[0] = tmp2(7236).ObscuredMediaTypes.Embed;
                obj1[1] = media;
            ...
            } else if (tmp2(10060).PreviewableMediaTypes.STICKER === type) {
          ...
          if (width > 0) {
            if (null != height) {
              if (height > 0) {
                ...
                const attachmentObscurityProps = tmp2(8056).getAttachmentObscurityProps(obj12);
                const tmp2Result1 = tmp2(8056);
                const attachmentUrl = tmp2(8193).getAttachmentUrl(media);
                const tmp2Result2 = tmp2(8193);
                ...
                obj13[4] = 2 * size;
                let str3;
                if (type === tmp2(10060).PreviewableMediaTypes.VIDEO) {
                  str3 = "png";
                }
                obj13[6] = str3;
...
function MultiplePreviewableMedia(arg0) {
  ...
  const memo = React.useMemo(() => {
    ...
    const obj = { shape: callback(8743).CutoutShape.RoundedRect, x: 56 - roundToNearestPixelResult + sum, y: -sum, width: roundToNearestPixelResult, height: roundToNearestPixelResult, cornerRadius: closure_4.roundToNearestPixel(roundToNearestPixelResult / 2) };
    return obj;
  }, []);
