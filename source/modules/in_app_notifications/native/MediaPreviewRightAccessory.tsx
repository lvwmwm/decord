...
// Function ID: 10334
// Name: VideoBadge
// Dependencies: [19, 17, 4181, 10262, 21, 4189, 712, 8127, 4185, 3893, 500, 4590, 4696, 8733, 647, 5862, 9888, 10295, 7904, 8117, 1473, 5141, 5877, 5880, 8142, 9635, 1297, 7970, 4050, 2]
// Exports: MediaPreviewRightAccessory

// Module 10333 (VideoBadge)
...
function VideoBadge() {
  ...
  obj = { style: tmp.badge, children: callback(require(8127) /* PlayIcon */.PlayIcon, obj) };
...
function ObscuredMediaOverlay(isSpoiler) {
  ...
  if (isObscured) {
    ...
    obj2[1] = tmp14(tmp2(8733).EyeIcon, { size: "sm", color: "white" });
...
function SinglePreviewableMedia(arg0) {
  ...
  let obj1 = require(5862) /* redactionSettingToRenderedString */;
  const shouldAgeVerifyForExplicitMedia = obj1.useShouldAgeVerifyForExplicitMedia();
  let obj2 = require(9888) /* useEnabledHarmTypesBitmaskForChannelAndAuthorId */;
  const enabledHarmTypesBitmaskForMessage = obj2.useEnabledHarmTypesBitmaskForMessage(message);
  if (require(10295) /* VoiceMessageIcon */.PreviewableMediaTypes.VOICE_MESSAGE === type) {
    ...
    if (tmp2(10295).PreviewableMediaTypes.AUDIO !== type) {
      if (tmp2(10295).PreviewableMediaTypes.FILE !== type) {
        if (tmp2(10295).PreviewableMediaTypes.IMAGE !== type) {
          if (tmp2(10295).PreviewableMediaTypes.VIDEO !== type) {
            if (tmp2(10295).PreviewableMediaTypes.GIF === type) {
                  ...
                  if (null != url) {
                    if ("" !== url) {
                      if (!stateFromStores) {
                        ...
                        obj5[4] = obj6;
                        obj5[5] = url2;
                        let tmp16Result = tmp16(importDefault(8142), obj5);
                      }
                      obj4[2] = tmp16Result;
            ...
            } else if (tmp2(10295).PreviewableMediaTypes.STICKER === type) {
              ...
              obj9[1] = callback(importDefault(9635), obj11);
          ...
          if (width > 0) {
            if (null != height) {
              if (height > 0) {
                ...
                const attachmentObscurityProps = tmp2(7904).getAttachmentObscurityProps(obj12);
                const tmp2Result1 = tmp2(7904);
                const attachmentUrl = tmp2(8117).getAttachmentUrl(media);
                const tmp2Result2 = tmp2(8117);
                ...
                obj13[4] = 2 * size;
                let str3;
                if (type === tmp2(10295).PreviewableMediaTypes.VIDEO) {
                  str3 = "png";
                }
                obj13[6] = str3;
...
function MultiplePreviewableMedia(arg0) {
  ...
  const memo = React.useMemo(() => {
    ...
    obj[0] = callback(7970).CutoutShape.RoundedRect;
  ...
  const items1 = [callback(importDefault(7970), obj), callback(CountBadge, { total: totalMediaCount })];
  obj[1] = items1;
...
export const MediaPreviewRightAccessory = function MediaPreviewRightAccessory(arg0) {
  ...
  let obj = require(10295) /* VoiceMessageIcon */;
