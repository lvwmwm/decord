...
// Dependencies: [31, 27, 1348, 1838, 10200, 653, 33, 4131, 1273, 4095, 689, 10199, 10271, 566, 669, 10275, 10217, 10219, 3809, 44, 1212, 4338, 9874, 9490, 675, 10272, 2]

// Module 10316 (NotificationAvatar)
...
function NotificationBody(channel) {
  ...
  if (!tmp4) {
    ...
    tmp8 = callback(importDefault(10275), obj);
  }
  const items2 = [tmp8, ];
  if (!hasPreviewableMedia) {
    if (null == message.poll) {
      ...
      let tmp15 = callback(channel(10219).ChannelRowPreview, obj1);
    }
    items2[1] = tmp15;
    obj.children = items2;
  ...
  tmp15 = callback(importDefault(10217), obj2);
}
...
let closure_15 = importAllResult.memo((message) => {
  ...
  if (obj.useHasPreviewableMedia(message)) {
    ...
    obj = { message };
    obj.children = callback(require(10271) /* VideoBadge */.MediaPreviewRightAccessory, obj);
...
export default importAllResult.memo(function ReminderNotification(notification) {
  ...
  return callback(notification(10272).NotificationPressable, obj);
});
