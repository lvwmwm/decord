...
// Dependencies: [19, 17, 1372, 1891, 9686, 676, 21, 4285, 1297, 4249, 712, 9685, 9764, 589, 692, 9772, 9705, 9707, 3962, 38, 1236, 4490, 8302, 8304, 698, 9769, 2]

// Module 10120 (NotificationAvatar)
...
function NotificationBody(channel) {
  ...
  let tmp10 = null;
  const messagePreviewTextVariant = channel(9685).getMessagePreviewTextVariant();
  if (!tmp6) {
    obj = { channel: null, parentChannel: null, guild: null, author: null };
    obj[0] = channel;
    obj[1] = stateFromStores1;
    obj[2] = stateFromStores;
    tmp10 = callback(importDefault(9772), obj);
  }
  const items2 = [tmp10, ];
  if (!hasPreviewableMedia) {
    if (null == message.poll) {
      ...
      obj[0] = channel;
      obj[1] = message;
      obj[3] = tmp(3962).ChannelListLayoutTypes.COZY;
      obj[4] = messagePreviewTextVariant;
      obj[6] = num;
      let tmp14 = callback(tmp(9707).ChannelRowPreview, obj);
    }
    obj1 = { children: null };
    items2[1] = tmp14;
    obj1[0] = items2;
  ...
  tmp14 = callback(importDefault(9705), obj2);
}
...
let closure_16 = importAllResult.memo((message) => {
  ...
  let obj = require(9685) /* extractMetadataFromNotification */;
  let tmp4 = null;
  if (obj.useHasPreviewableMedia(message)) {
    ...
    obj = { message: null };
    obj[0] = message;
    obj[1] = callback(require(9764) /* VideoBadge */.MediaPreviewRightAccessory, obj);
...
export default importAllResult.memo(function ReminderNotification(notification) {
  ...
  return callback(notification(9769).NotificationPressable, obj);
});
