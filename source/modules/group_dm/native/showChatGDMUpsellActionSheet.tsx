// Module ID: 11155
// Function ID: 11156
// Name: showChatGDMUpsellActionSheet
// Dependencies: [4415, 11156, 2009, 2]
// Exports: default

// Module 11155 (showChatGDMUpsellActionSheet)
import set from "set" /* 2 */;
import asyncRequireImpl from "asyncRequireImpl" /* 2009 */;
import ACTION_SHEET_HEIGHT_HALFDefault from "ACTION_SHEET_HEIGHT_HALF" /* 4415 */;

const result = set.fileFinishedImporting("modules/group_dm/native/showChatGDMUpsellActionSheet.tsx");

export default function showChatGDMUpsellActionSheet(arg0) {
  ACTION_SHEET_HEIGHT_HALFDefault.openLazy(asyncRequireImpl(11156, dependencyMap.paths), "ChatGDMUpsellActionSheet", arg0);
};
