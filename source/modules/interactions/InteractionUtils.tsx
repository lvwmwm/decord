...
// Dependencies: [5, 1218, 8051, 676, 11, 7225, 8225, 1954, 530, 6920, 8226, 709, 4529, 2, 4527]
...
// Module 8224 (_executeMessageComponentInteraction)
...
function handleInteractionResponse(nonce, ok, applicationId, channelId, guildId) {
  if (!ok.ok) {
    if (ok.hasErr) {
      require(8225) /* _fetchMessageInteractionData */.setFailed(nonce);
      const obj10 = require(8225) /* _fetchMessageInteractionData */;
    } else {
      if (ok.status >= 400) {
        if (ok.status < 500) {
          if (ok.body) {
            ...
            if (ok.body.code === constants.INVALID_FORM_BODY) {
              if (ok.body.errors) {
                const firstSkemaError = require(8226) /* getFirstSkemaFieldError */.getFirstSkemaError(ok.body.errors);
                ...
                const obj6 = require(8226) /* getFirstSkemaFieldError */;
                const tmp24 = require;
                let message;
                if (firstSkemaError != null) {
                  message = firstSkemaError.message;
                }
                require(8225) /* _fetchMessageInteractionData */.setFailed(nonce, undefined, message);
              }
            }
            if (ok.body.code === constants.UNKNOWN_INTEGRATION) {
              ...
              require(8225) /* _fetchMessageInteractionData */.setFailed(nonce, undefined, ok.body.message);
              const obj5 = require(8225) /* _fetchMessageInteractionData */;
            } else {
              const obj2 = require(8225) /* _fetchMessageInteractionData */;
      ...
      obj = require(8225) /* _fetchMessageInteractionData */;
      const body = ok.body;
      let code;
      if (body != null) {
        code = body.code;
