...
// Dependencies: [5, 1218, 8090, 676, 11, 7264, 8264, 1954, 530, 6959, 8265, 709, 4569, 2, 4567]
...
// Module 8263 (_executeMessageComponentInteraction)
...
function mapMessageComponentLocalStateForAPI(type) {
  if (null == type) {
    ...
    type = type.type;
    if (require(1954) /* PermissionOverwriteType */.ComponentType.TEXT_INPUT !== type) {
...
function handleInteractionResponse(nonce, ok, applicationId, channelId, guildId) {
  if (!ok.ok) {
    if (ok.hasErr) {
      require(8264) /* _fetchMessageInteractionData */.setFailed(nonce);
      const obj10 = require(8264) /* _fetchMessageInteractionData */;
    } else {
      if (ok.status >= 400) {
        if (ok.status < 500) {
          if (ok.body) {
            ...
            if (ok.body.code === constants.INVALID_FORM_BODY) {
              if (ok.body.errors) {
                const firstSkemaError = require(8265) /* getFirstSkemaFieldError */.getFirstSkemaError(ok.body.errors);
                ...
                const obj6 = require(8265) /* getFirstSkemaFieldError */;
                const tmp24 = require;
                let message;
                if (firstSkemaError != null) {
                  message = firstSkemaError.message;
                }
                require(8264) /* _fetchMessageInteractionData */.setFailed(nonce, undefined, message);
              }
            }
            if (ok.body.code === constants.UNKNOWN_INTEGRATION) {
              ...
              require(8264) /* _fetchMessageInteractionData */.setFailed(nonce, undefined, ok.body.message);
              const obj5 = require(8264) /* _fetchMessageInteractionData */;
            } else {
              const obj2 = require(8264) /* _fetchMessageInteractionData */;
      ...
      obj = require(8264) /* _fetchMessageInteractionData */;
      const body = ok.body;
      let code;
      if (body != null) {
        code = body.code;
...
export const getInteractionStatusViewState = function getInteractionStatusViewState(state, state2) {
  ...
  if (state.state !== constants2.SENT) {
    if (state.state !== tmp.SEND_FAILED) {
      ...
      const tmp21 = interactionType === require(1954) /* PermissionOverwriteType */.InteractionTypes.APPLICATION_COMMAND;
...
export const canRetryInteractionData = function canRetryInteractionData(interactionData) {
  ...
  if (1 === length) {
    let tmp4 = options;
    if (options[0].type === require(1954) /* PermissionOverwriteType */.ApplicationCommandOptionType.SUB_COMMAND_GROUP) {
      while (true) {
        ...
        if (1 !== length1) {
          ...
          tmp4 = options1;
          if (options1[0].type === require(1954) /* PermissionOverwriteType */.ApplicationCommandOptionType.SUB_COMMAND_GROUP) {
            continue;
          } else {
            let tmp10 = tmp6;
            let tmp11 = tmp8;
  ...
  for (const item10042 of items) {
    ...
    if (item10042.type === require(1954) /* PermissionOverwriteType */.ApplicationCommandOptionType.ATTACHMENT) {
      let tmp16 = obj;
      obj.return();
      let flag = false;
      return false;
