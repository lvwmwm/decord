...
export const interactionCallbackErrorReason = function interactionCallbackErrorReason(reason, applicationId) {
  if (importDefault(4418).ReasonCodes.TIMEOUT === reason) {
  ...
  } else if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_NOT_IN_EXPERIMENT === reason) {
    const intl9 = require(1236) /* getSystemLocale */.intl;
    return intl9.string(require(1236) /* getSystemLocale */.t.GyzcrS);
  } else {
    if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_USER_VERIFICATION_LEVEL !== reason) {
      if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_USER_PERMISSIONS !== reason) {
        if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_UNKNOWN_CHANNEL !== reason) {
          if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_UNKNOWN_GUILD !== reason) {
            if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_CHANNEL_TYPE !== reason) {
              if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_CHANNEL_NO_AFK !== reason) {
                if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_USER_AGE_GATE === reason) {
                  const intl6 = require(1236) /* getSystemLocale */.intl;
                  return intl6.string(require(1236) /* getSystemLocale */.t["4WuFRE"]);
                } else if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_DEV_PREVIEW_GUILD_SIZE === reason) {
                  const intl5 = require(1236) /* getSystemLocale */.intl;
                  return intl5.string(require(1236) /* getSystemLocale */.t.RvkXdb);
                } else {
                  if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_CONFIGURATION_PLATFORM_NOT_SUPPORTED !== reason) {
                    if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_CONFIGURATION_PLATFORM_NOT_RELEASED !== reason) {
                      if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_USER_NO_ACCESS_TO_ACTIVITY === reason) {
                        const intl3 = require(1236) /* getSystemLocale */.intl;
                        return intl3.string(require(1236) /* getSystemLocale */.t.WjNAAA);
                      } else if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_LOCATION_TYPE === reason) {
                        const intl2 = require(1236) /* getSystemLocale */.intl;
                        return intl2.string(require(1236) /* getSystemLocale */.t.PtobXW);
                      } else if (tmp(4418).ReasonCodes.ACTIVITY_LAUNCH_INVALID_USER_REGION_FOR_APPLICATION === reason) {
