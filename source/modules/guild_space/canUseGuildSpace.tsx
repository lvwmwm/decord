...
export const useCanUseGuildSpace = function useCanUseGuildSpace(id, useGuildActionRows) {
  ...
  const guildSpaceExperimentEnabled = _require(5768).useGuildSpaceExperimentEnabled(id, useGuildActionRows);
  _require = id;
  const obj = _require(5768);
  const tmp = _require;
  const items = [getUncachedChannelPermissions];
  const items1 = [id];
