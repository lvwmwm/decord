...
// Dependencies: [32, 19, 17, 14529, 16886, 14506, 21, 4344, 712, 9102, 5307, 16902, 4340, 1236, 1435, 16903, 13312, 14533, 38, 16881, 8008, 9105, 16904, 16905, 16898, 14513, 16877, 14528, 16907, 16908, 16917, 14532, 16918, 16890, 2]
// Exports: GuildRoleSubscriptionTierChannelBenefitsModal, GuildRoleSubscriptionTierIntangibleBenefitsModal

// Module 16901 (AddBenefitButton)
import Row from "Row";
...
function AddBenefitButton(disabled) {
  ...
  const tmp5 = importDefault(9102);
  obj[0] = importDefault(16902);
...
function EmojiRowLabel(emoji) {
  ...
  obj[1] = obj;
  const items = [callback(tmp2, obj), callback(importDefault(16903), { name: emoji.name })];
  obj[0] = items;
...
function ListFooterSection(onChangeTrialInterval) {
  ...
  let obj = onChangeTrialInterval(16881);
  ...
  const items1 = [callback(onChangeTrialInterval(8008).FormSwitchRow, obj), ];
  ...
  const items3 = [callback(onChangeTrialInterval(4340).Text, obj2), , , , , , ];
  ...
  items3[4] = callback(options(9105), obj6);
...
function Content(arg0) {
  ...
  roleSubscriptionSettingsDisabled = _require(16881).useRoleSubscriptionSettingsDisabled();
  const tmp27 = importDefault(16907)(guildId);
  c16 = tmp27;
          ...
          obj = {
            ...
            onDelete() {
              ...
              if (ref_type.ref_type === outer2_10.CHANNEL) {
                outer1_8((arr) => arr.filter(/* F120591 */ function() { ... }));
              } else {
                outer1_10((arr) => arr.filter(/* F120592 */ function() { ... }));
              }
            },
            onSave(ref_type) {
              let closure_0 = ref_type;
  ...
  if (true !== onlyIntangible) {
    if (importDefault(16877)(guildId).isFullServerGating) {
      ...
      tmp32Result = tmp32(tmp10(16918), obj1);
    } else {
      tmp32Result = null;
    }
  }
    ...
    return closure_12(GuildRoleSubscriptionTierBenefitsModalHeader, {
      ...
      onPress() {
        if (type === constants2.EMOJI) {
          ...
          obj[3] = outer1_12;
          obj[4] = outer1_4;
          callback(16908).showEditEmojisModal(obj);
          const obj3 = callback(16908);
        } else {
          if (tmp === tmp2.CHANNEL) {
...
export const GuildRoleSubscriptionTierChannelBenefitsModal = function GuildRoleSubscriptionTierChannelBenefitsModal(arg0) {
  ...
  return callback(importDefault(16890), obj);
};
