...
// Function ID: 16595
// Name: BaseBenefitRow
// Dependencies: [19, 17, 14218, 21, 4189, 14253, 1297, 10733, 4185, 3830, 14246, 4380, 4650, 1236, 2]
// Exports: GuildRoleSubscriptionBenefitPreview

// Module 16594 (BaseBenefitRow)
import "noop";
...
function BaseBenefitRow(isInteractive) {
  ...
  obj = { style: tmp.emojiContainer, children: callback(importDefault(14253), { guildId, id: emoji }) };
...
function DescriptiveBenefitRow(benefit) {
  ...
  if (null != benefit.description) {
    ...
    tmp2 = callback(require(4185) /* Text */.Text, obj);
  }
  let emoji = benefit.emoji_id;
  if (emoji == null) {
    let str = "";
    if (null != benefit.emoji_name) {
      str = importDefault(3830).convertSurrogateToName(benefit.emoji_name, false);
...
function ChannelBenefitRow(benefit) {
  ...
  const tmp = callback3();
  let obj = require(14246) /* getUsedTemplateChannelsForGuild */;
  ...
  if (null != channelWithTemplateFallback) {
    channelIcon = tmp2(4650).getChannelIcon(channelWithTemplateFallback);
    const tmp2Result = tmp2(4650);
  }
  if (null == channelWithTemplateFallback) {
    ...
    let tmp9 = callback(tmp2(4185).Text, obj);
  } else {
    ...
    obj3[2] = tmp5;
    items[1] = callback(tmp2(4185).Text, obj3);
    obj1[1] = items;
...
function IntangibleBenefitRow(benefit) {
  benefit = benefit.benefit;
  obj = { benefit, guildId: benefit.guildId, isInteractive: benefit.isInteractive, children: callback(require(4185) /* Text */.Text, obj) };
...
function EmojiBenefitRow(benefit) {
  ...
  items[2] = callback(require(4185) /* Text */.Text, { style: tmp.emojiColons, variant: "text-md/medium", color: "text-muted", children: ":" });
  obj[4] = items;
