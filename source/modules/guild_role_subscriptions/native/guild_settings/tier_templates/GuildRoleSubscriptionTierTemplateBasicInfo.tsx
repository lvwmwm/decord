...
// Dependencies: [19, 17, 1924, 505, 21, 4344, 712, 5307, 1297, 4340, 1236, 5894, 14532, 4755, 2]
// Exports: GuildRoleSubscriptionTierTemplateBasicInfo

// Module 16942 (GuildRoleSubscriptionTierTemplateBasicInfo)
import "noop";
...
export const GuildRoleSubscriptionTierTemplateBasicInfo = function GuildRoleSubscriptionTierTemplateBasicInfo(template) {
  ...
  const items = [callback(importDefault(5307), obj), callback(template(1297).Spacer, { size: 16 }), ];
  const obj1 = { style: { flexShrink: 1 }, children: null };
  const items1 = [callback(template(4340).Text, { variant: "heading-md/semibold", color: "mobile-text-heading-primary", style: { flexWrap: "wrap" }, children: name }), callback(template(1297).Spacer, { size: 4 }), ];
  ...
  items1[2] = callback(template(4340).Text, obj2);
  obj1[1] = items1;
  ...
  items2[1] = callback(template(4340).Text, obj5);
  ...
  items2[2] = callback(template(4755).BaseTextButton, obj6);
  obj[1] = items2;
