...
// Dependencies: [19, 17, 1924, 505, 21, 4444, 712, 5440, 1297, 4440, 1236, 6028, 14748, 4880, 2]
// Exports: GuildRoleSubscriptionTierTemplateBasicInfo

// Module 17216 (GuildRoleSubscriptionTierTemplateBasicInfo)
...
export const GuildRoleSubscriptionTierTemplateBasicInfo = function GuildRoleSubscriptionTierTemplateBasicInfo(template) {
  ...
  const items1 = [callback(template(4440).Text, { variant: "heading-md/semibold", color: "mobile-text-heading-primary", style: { flexWrap: "wrap" }, children: name }), callback(template(1297).Spacer, { size: 4 }), ];
  ...
  let obj6 = template(6028);
  obj3[0] = obj6.formatPrice(price_tier, CurrencyCodes.USD);
  obj3[1] = template(14748).formatPlanInterval({ interval: SubscriptionIntervalTypes.MONTH, interval_count: 1 });
  obj2[2] = intl.format(template(1236).t.CgmBaG, obj3);
  items1[2] = callback(template(4440).Text, obj2);
  obj1[1] = items1;
  ...
  items2[1] = callback(template(4440).Text, obj5);
  ...
  items2[2] = callback(template(4880).BaseTextButton, obj6);
  obj[1] = items2;
