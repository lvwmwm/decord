...
function BenefitRow(description) {
  ...
  if (null != description) {
    ...
    obj1[3] = description;
    tmp4Result = callback(require(4185) /* Text */.Text, obj1);
  }
  items1[1] = tmp4Result;
  obj[1] = items1;
...
export default function GuildRoleSubscriptionTierTemplateFullCard(template) {
  ...
  const items1 = [callback(_require(4185).Text, obj4), callback(_require(1297).Spacer, { size: 4 }), , , , , , , , , ];
  const obj5 = { variant: "text-sm/normal", color: "text-muted", children: null };
  const intl2 = _require(1236).intl;
  obj5[2] = intl2.string(_require(1236).t.bCb3c8);
  items1[2] = callback(_require(4185).Text, obj5);
  ...
  return callback(_require(5243).BottomSheet, obj);
};
