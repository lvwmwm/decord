...
// Dependencies: [19, 1891, 21, 12, 5180, 16562, 16781, 1236, 16725, 589, 4270, 16782, 1988, 16735, 2]
// Exports: default

// Module 16780 (GuildSettingsRoleSubscriptionEmojisInner)
import "noop";
...
function GuildSettingsRoleSubscriptionEmojisInner(guildId) {
  ...
  if (null == stateFromStores) {
    ...
    obj = { maxSlots: null };
    obj[0] = stateFromStores(16781)(stateFromStores);
    ...
    return jsx(tmp(16562).ManageEmojisModal, { guild: null, headerDescription: null, computeEmojiItems: null, onSelectRolesForEmoji: null, disabled: null });
  }
...
let closure_5 = require("asyncRequireImpl").memoize((arr) => {
  ...
  if (0 === found.length) {
    ...
    const mapped = found.map(_require(16562).computeEmojiItem);
    const reversed = mapped.reverse();
    const tmp4 = importDefault(16781)(arg1);
    const intl = _require(1236).intl;
    const items = [_require(16562).computeSectionItem(intl.string(_require(1236).t.sMOuuS), reversed.length, tmp4)];
...
export default function GuildSettingsRoleSubscriptionEmojis(guildId) {
  ...
  return jsx(importDefault(16735), { guildId, children: null });
};
