...
// Dependencies: [19, 1910, 21, 12, 5221, 16720, 16939, 1236, 16883, 589, 4310, 16940, 2007, 16893, 2]
// Exports: default

// Module 16938 (GuildSettingsRoleSubscriptionEmojisInner)
import "noop";
...
function GuildSettingsRoleSubscriptionEmojisInner(guildId) {
  ...
  if (null == stateFromStores) {
    ...
    obj = { maxSlots: null };
    obj[0] = stateFromStores(16939)(stateFromStores);
    ...
    return jsx(tmp(16720).ManageEmojisModal, { guild: null, headerDescription: null, computeEmojiItems: null, onSelectRolesForEmoji: null, disabled: null });
  }
...
let closure_5 = require("asyncRequireImpl").memoize((arr) => {
  ...
  if (0 === found.length) {
    ...
    const mapped = found.map(_require(16720).computeEmojiItem);
    const reversed = mapped.reverse();
    const tmp4 = importDefault(16939)(arg1);
    const intl = _require(1236).intl;
    const items = [_require(16720).computeSectionItem(intl.string(_require(1236).t.sMOuuS), reversed.length, tmp4)];
...
export default function GuildSettingsRoleSubscriptionEmojis(guildId) {
  ...
  return jsx(importDefault(16893), { guildId, children: null });
};
