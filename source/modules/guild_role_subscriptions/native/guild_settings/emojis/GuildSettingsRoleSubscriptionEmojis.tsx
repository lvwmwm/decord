...
// Dependencies: [19, 1910, 21, 12, 5242, 16736, 16955, 1236, 16899, 589, 4310, 16956, 2007, 16909, 2]
// Exports: default

// Module 16954 (GuildSettingsRoleSubscriptionEmojisInner)
import "noop";
...
function GuildSettingsRoleSubscriptionEmojisInner(guildId) {
  ...
  if (null == stateFromStores) {
    ...
    obj = { maxSlots: null };
    obj[0] = stateFromStores(16955)(stateFromStores);
    ...
    return jsx(tmp(16736).ManageEmojisModal, { guild: null, headerDescription: null, computeEmojiItems: null, onSelectRolesForEmoji: null, disabled: null });
  }
...
let closure_5 = require("asyncRequireImpl").memoize((arr) => {
  ...
  if (0 === found.length) {
    ...
    const mapped = found.map(_require(16736).computeEmojiItem);
    const reversed = mapped.reverse();
    const tmp4 = importDefault(16955)(arg1);
    const intl = _require(1236).intl;
    const items = [_require(16736).computeSectionItem(intl.string(_require(1236).t.sMOuuS), reversed.length, tmp4)];
...
export default function GuildSettingsRoleSubscriptionEmojis(guildId) {
  ...
  return jsx(importDefault(16909), { guildId, children: null });
};
