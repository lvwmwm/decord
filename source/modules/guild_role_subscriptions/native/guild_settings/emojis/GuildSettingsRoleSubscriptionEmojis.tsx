...
// Dependencies: [19, 1862, 21, 12, 5133, 16497, 16721, 1236, 16665, 589, 4223, 16722, 1959, 16675, 2]
// Exports: default

// Module 16720 (GuildSettingsRoleSubscriptionEmojisInner)
import "noop";
...
function GuildSettingsRoleSubscriptionEmojisInner(guildId) {
  ...
  if (null == stateFromStores) {
    ...
    obj = { maxSlots: null };
    obj[0] = stateFromStores(16721)(stateFromStores);
    ...
    return jsx(tmp(16497).ManageEmojisModal, { guild: null, headerDescription: null, computeEmojiItems: null, onSelectRolesForEmoji: null, disabled: null });
  }
...
let closure_5 = require("asyncRequireImpl").memoize((arr) => {
  ...
  if (0 === found.length) {
    ...
    const mapped = found.map(_require(16497).computeEmojiItem);
    const reversed = mapped.reverse();
    const tmp4 = importDefault(16721)(arg1);
    const intl = _require(1236).intl;
    const items = [_require(16497).computeSectionItem(intl.string(_require(1236).t.sMOuuS), reversed.length, tmp4)];
...
export default function GuildSettingsRoleSubscriptionEmojis(guildId) {
  ...
  return jsx(importDefault(16675), { guildId, children: null });
};
