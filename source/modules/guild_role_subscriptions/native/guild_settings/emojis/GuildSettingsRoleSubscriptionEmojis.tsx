...
// Dependencies: [19, 1891, 21, 12, 5163, 16537, 16761, 1236, 16705, 589, 4253, 16762, 1988, 16715, 2]
// Exports: default

// Module 16760 (GuildSettingsRoleSubscriptionEmojisInner)
import "noop";
...
function GuildSettingsRoleSubscriptionEmojisInner(guildId) {
  ...
  if (null == stateFromStores) {
    ...
    obj = { maxSlots: null };
    obj[0] = stateFromStores(16761)(stateFromStores);
    ...
    return jsx(tmp(16537).ManageEmojisModal, { guild: null, headerDescription: null, computeEmojiItems: null, onSelectRolesForEmoji: null, disabled: null });
  }
...
let closure_5 = require("asyncRequireImpl").memoize((arr) => {
  ...
  if (0 === found.length) {
    ...
    const mapped = found.map(_require(16537).computeEmojiItem);
    const reversed = mapped.reverse();
    const tmp4 = importDefault(16761)(arg1);
    const intl = _require(1236).intl;
    const items = [_require(16537).computeSectionItem(intl.string(_require(1236).t.sMOuuS), reversed.length, tmp4)];
...
export default function GuildSettingsRoleSubscriptionEmojis(guildId) {
  ...
  return jsx(importDefault(16715), { guildId, children: null });
};
