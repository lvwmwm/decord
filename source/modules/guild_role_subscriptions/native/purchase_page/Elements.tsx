...
// Function ID: 15512
// Name: TruncatedText
// Dependencies: [32, 19, 17, 3906, 21, 4255, 712, 4251, 1236, 4797, 1297, 15512, 7544, 14338, 647, 7545, 5749, 2]
// Exports: ArrowButton, TruncatedText, useFormattedSubscriptionPlan

// Module 15511 (TruncatedText)
...
export const TruncatedText = function TruncatedText(lineClamp) {
  ...
  if (!first) {
    ...
    const items = [closure_9(num(4251).Text, obj), first];
    obj[3] = items;
...
export const useFormattedSubscriptionPlan = function useFormattedSubscriptionPlan(listingId) {
  const obj = importDefault(7544);
  const _require = callback(importAll(14338).useSubscriptionPlan(listingId), 1)[0];
  ...
  const price = importDefault(7545)(stateFromStores, obj.useNativeIAPPayments().storeFront).price;
  let str = "No Price Available";
  if (null != price) {
    const _HermesInternal = HermesInternal;
    str = "" + tmp2(5749).formatPrice(price.amount, price.currency) + "/mo.";
