...
// Function ID: 14245
// Name: getRoleEmojis
// Dependencies: [5, 32, 19, 5048, 3811, 14245, 14222, 676, 1876, 589, 5503, 4427, 14246, 14247, 688, 3809, 14248, 14229, 14249, 705, 38, 5105, 12618, 12, 9415, 14250, 514, 2]
        ...
        try {
          ...
          if (0 === dependencyMap) {
                  ...
                  closure_16 = closure_14.map((emojiId) => {
                    ...
                    if (null != customEmojiById) {
                      ...
                      if (found.length > 0) {
                        ...
                        obj[1] = customEmojiById.id;
                        obj[2] = found;
                        let updateEmojiResult = _undefined(9415).updateEmoji(obj);
                        const obj2 = _undefined(9415);
                      } else {
                        obj = _undefined(9415);
            ...
            if (tmp25) {
              ...
              obj8[0] = lib(12618).updateSubscriptionTrial(callback, id, obj7);
              return obj8;
            }
          }
        } catch (tmp71) {
            ...
            if (1 === tmp4) {
              if (arg0 === 1) {
                ...
                if (null == id) {
                  ...
                  obj2[0] = callback3(12618).createSubscriptionGroupListing(callback, {});
                  return obj2;
                } else {
...
export const useRole = function useRole(listingId, guildId) {
  ...
  const tmp = importDefault(14246)(guildId, listingId);
  importDefault = tmp;
...
export const useRoleColor = function useRoleColor(editStateId, guildId) {
  const tmp = roleColor(14246)(guildId, editStateId);
  let closure_0 = tmp;
  const items = [tmp];
  let memo = React.useMemo(() => {
    let color;
...
export const useChannelAccessFormat = function useChannelAccessFormat(editStateId, guildId) {
  const tmp = channelAccessFormat(14246)(guildId, editStateId);
  let closure_0 = tmp;
  const items = [tmp];
  let memo = React.useMemo(() => {
    if (null == editStateId) {
...
export const useTierEmojiIds = function useTierEmojiIds(listingId, guildId) {
  ...
  const tmp = tierEmojiIds(14246)(guildId, listingId);
  tierEmojiIds = tmp;
...
export const useTrialInterval = function useTrialInterval(editStateId) {
  const subscriptionTrial = _require(14229).useSubscriptionTrial(editStateId);
  let active_trial;
  const obj = _require(14229);
  ...
  let selectedOption = trialInterval(14249)(active_trial).selectedOption;
...
export const useEditStateIds = function useEditStateIds(groupListingId, guildId, arg2) {
  ...
  let dependencyMap;
  let c4;
  subscriptionListingsForGroup = _require(14229).useSubscriptionListingsForGroup(groupListingId, obj);
