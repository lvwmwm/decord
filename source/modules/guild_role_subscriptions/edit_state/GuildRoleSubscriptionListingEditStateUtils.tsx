...
// Function ID: 15053
// Name: getRoleEmojis
// Dependencies: [5, 32, 19, 5409, 4114, 15053, 15030, 673, 1923, 586, 5949, 4755, 15054, 15055, 685, 4112, 15056, 15037, 15057, 702, 38, 5465, 6110, 12, 9191, 15058, 511, 2]
...
// Module 15052 (getRoleEmojis)
import useSubscriptionRoleDefault from "useSubscriptionRole" /* 15054 */;
...
import { GuildRoleSubscriptionBenefitTypes as closure_11 } from "MAX_SUBSCRIPTION_TIERS" /* 15030 */;
        ...
        try {
          ...
          if (0 === dependencyMap) {
                  ...
                  closure_16 = closure_14.map((emojiId) => {
                    ...
                    if (null != customEmojiById) {
                      ...
                      if (found.length > 0) {
                        ...
                        obj[1] = customEmojiById.id;
                        obj[2] = found;
                        let updateEmojiResult = _undefined(9191).updateEmoji(obj);
                        const obj2 = _undefined(9191);
                      } else {
                        obj = _undefined(9191);
            ...
            if (tmp25) {
              ...
              obj8[0] = lib(6110).updateSubscriptionTrial(callback, id, obj7);
              return obj8;
            }
          }
        } catch (tmp71) {
            ...
            if (1 === tmp4) {
              if (arg0 === 1) {
                ...
                if (null == id) {
                  ...
                  obj2[0] = callback3(6110).createSubscriptionGroupListing(callback, {});
                  return obj2;
                } else {
...
export const useName = function useName(arg0) {
  ...
  const stateFromStores = _require(586).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = name(5949)(() => first);
  dependencyMap = tmp2;
...
export const usePriceTier = function usePriceTier(editStateId) {
  ...
  const stateFromStores = _require(586).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = priceTier(5949)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
...
export const useDescription = function useDescription(arg0) {
  ...
  const stateFromStores = _require(586).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = description(5949)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
  _require = arg0;
...
export const useRole = function useRole(listingId, guildId) {
  ...
  return React.useMemo(() => {
    ...
    if (closure_1 == null) {
      DEFAULT_PREVIEW_ROLE = listingId(15055).DEFAULT_PREVIEW_ROLE;
    }
    const obj = {};
    ...
    if (undefined !== closure_2) {
      obj.color = tmp5;
      obj.colorString = listingId(685).int2hex(tmp5);
      const obj2 = listingId(685);
    }
    return obj;
  }, items);
};
export const useRoleColor = function useRoleColor(editStateId, guildId) {
  const tmp = roleColor(15054)(guildId, editStateId);
  closure_0 = tmp;
  const items = [tmp];
  let memo = React.useMemo(() => {
    let color;
...
export const useChannelAccessFormat = function useChannelAccessFormat(editStateId, guildId) {
  const tmp = channelAccessFormat(15054)(guildId, editStateId);
  closure_0 = tmp;
  const items = [tmp];
  let memo = React.useMemo(() => {
    if (null == editStateId) {
...
export const useChannelBenefits = function useChannelBenefits(listingId) {
  ...
  const stateFromStores = _require(586).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = channelBenefits(5949)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
  _require = listingId;
...
export const useIntangibleBenefits = function useIntangibleBenefits(listingId) {
  ...
  const stateFromStores = _require(586).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = intangibleBenefits(5949)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
  _require = listingId;
...
export const useTierEmojiIds = function useTierEmojiIds(listingId, guildId) {
  ...
  const tmp = tierEmojiIds(15054)(guildId, listingId);
  tierEmojiIds = tmp;
  const items = [closure_7];
  const items1 = [guildId];
  const stateFromStoresArray = _require(586).useStateFromStoresArray(items, () => closure_1_7.getGuildEmoji(closure_0), items1);
...
export const useTrialInterval = function useTrialInterval(editStateId) {
  const subscriptionTrial = _require(15037).useSubscriptionTrial(editStateId);
  let active_trial;
  const obj = _require(15037);
  ...
  let selectedOption = trialInterval(15057)(active_trial).selectedOption;
...
export const useEditStateIds = function useEditStateIds(groupListingId, guildId, arg2) {
  ...
  dependencyMap = undefined;
  closure_4 = undefined;
  subscriptionListingsForGroup = _require(15037).useSubscriptionListingsForGroup(groupListingId, obj);
