...
// Function ID: 14745
// Name: getRoleEmojis
// Dependencies: [5, 32, 19, 5348, 4083, 14745, 14722, 676, 1924, 589, 5888, 4707, 14746, 14747, 688, 4081, 14748, 14729, 14749, 705, 38, 5404, 6048, 12, 8955, 14750, 514, 2]
...
// Module 14744 (getRoleEmojis)
import useSubscriptionRoleDefault from "useSubscriptionRole" /* 14746 */;
...
import { GuildRoleSubscriptionBenefitTypes as closure_11 } from "MAX_SUBSCRIPTION_TIERS" /* 14722 */;
        ...
        try {
          ...
          if (0 === dependencyMap) {
                  ...
                  closure_16 = closure_14.map((emojiId) => {
                    ...
                    if (null != customEmojiById) {
                      ...
                      if (found.length > 0) {
                        ...
                        obj[1] = customEmojiById.id;
                        obj[2] = found;
                        let updateEmojiResult = _undefined(8955).updateEmoji(obj);
                        const obj2 = _undefined(8955);
                      } else {
                        obj = _undefined(8955);
            ...
            if (tmp25) {
              ...
              obj8[0] = lib(6048).updateSubscriptionTrial(callback, id, obj7);
              return obj8;
            }
          }
        } catch (tmp71) {
            ...
            if (1 === tmp4) {
              if (arg0 === 1) {
                ...
                if (null == id) {
                  ...
                  obj2[0] = callback3(6048).createSubscriptionGroupListing(callback, {});
                  return obj2;
                } else {
...
export const usePriceTier = function usePriceTier(editStateId) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = priceTier(5888)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
...
export const useDescription = function useDescription(arg0) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = description(5888)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
  _require = arg0;
...
export const useRole = function useRole(listingId, guildId) {
  ...
  return React.useMemo(() => {
    ...
    if (closure_1 == null) {
      DEFAULT_PREVIEW_ROLE = listingId(14747).DEFAULT_PREVIEW_ROLE;
    }
    const obj = {};
...
export const useRoleColor = function useRoleColor(editStateId, guildId) {
  const tmp = roleColor(14746)(guildId, editStateId);
  closure_0 = tmp;
  const items = [tmp];
  let memo = React.useMemo(() => {
    let color;
...
export const useChannelAccessFormat = function useChannelAccessFormat(editStateId, guildId) {
  const tmp = channelAccessFormat(14746)(guildId, editStateId);
  closure_0 = tmp;
  const items = [tmp];
  let memo = React.useMemo(() => {
    if (null == editStateId) {
...
export const useChannelBenefits = function useChannelBenefits(listingId) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = channelBenefits(5888)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
  _require = listingId;
...
export const useIntangibleBenefits = function useIntangibleBenefits(listingId) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = intangibleBenefits(5888)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
  _require = listingId;
...
export const useTierEmojiIds = function useTierEmojiIds(listingId, guildId) {
  ...
  const tmp = tierEmojiIds(14746)(guildId, listingId);
  tierEmojiIds = tmp;
...
export const useTrialInterval = function useTrialInterval(editStateId) {
  const subscriptionTrial = _require(14729).useSubscriptionTrial(editStateId);
  let active_trial;
  ...
  let selectedOption = trialInterval(14749)(active_trial).selectedOption;
...
export const useEditStateIds = function useEditStateIds(groupListingId, guildId, arg2) {
  ...
  dependencyMap = undefined;
  closure_4 = undefined;
  subscriptionListingsForGroup = _require(14729).useSubscriptionListingsForGroup(groupListingId, obj);
