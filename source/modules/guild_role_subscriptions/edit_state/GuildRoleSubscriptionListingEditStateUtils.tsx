...
// Function ID: 14672
// Name: getRoleEmojis
// Dependencies: [5, 32, 19, 5282, 4019, 14672, 14649, 676, 1924, 589, 5822, 4643, 14673, 14674, 688, 4017, 14675, 14656, 14676, 705, 38, 5338, 5982, 12, 8887, 14677, 514, 2]
...
// Module 14671 (getRoleEmojis)
import useSubscriptionRoleDefault from "useSubscriptionRole" /* 14673 */;
...
import { GuildRoleSubscriptionBenefitTypes as closure_11 } from "MAX_SUBSCRIPTION_TIERS" /* 14649 */;
        ...
        try {
          ...
          if (0 === dependencyMap) {
                  ...
                  closure_16 = closure_14.map((emojiId) => {
                    ...
                    if (null != customEmojiById) {
                      ...
                      if (found.length > 0) {
                        ...
                        obj[1] = customEmojiById.id;
                        obj[2] = found;
                        let updateEmojiResult = _undefined(8887).updateEmoji(obj);
                        const obj2 = _undefined(8887);
                      } else {
                        obj = _undefined(8887);
            ...
            if (tmp25) {
              ...
              obj8[0] = lib(5982).updateSubscriptionTrial(callback, id, obj7);
              return obj8;
            }
          }
        } catch (tmp71) {
            ...
            if (1 === tmp4) {
              if (arg0 === 1) {
                ...
                if (null == id) {
                  ...
                  obj2[0] = callback3(5982).createSubscriptionGroupListing(callback, {});
                  return obj2;
                } else {
...
export const usePriceTier = function usePriceTier(editStateId) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = priceTier(5822)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
...
export const useDescription = function useDescription(arg0) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = description(5822)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
  _require = arg0;
...
export const useRole = function useRole(listingId, guildId) {
  ...
  return React.useMemo(() => {
    ...
    if (closure_1 == null) {
      DEFAULT_PREVIEW_ROLE = listingId(14674).DEFAULT_PREVIEW_ROLE;
    }
    const obj = {};
...
export const useRoleColor = function useRoleColor(editStateId, guildId) {
  const tmp = roleColor(14673)(guildId, editStateId);
  closure_0 = tmp;
  const items = [tmp];
  let memo = React.useMemo(() => {
    let color;
...
export const useChannelAccessFormat = function useChannelAccessFormat(editStateId, guildId) {
  const tmp = channelAccessFormat(14673)(guildId, editStateId);
  closure_0 = tmp;
  const items = [tmp];
  let memo = React.useMemo(() => {
    if (null == editStateId) {
...
export const useChannelBenefits = function useChannelBenefits(listingId) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = channelBenefits(5822)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
  _require = listingId;
...
export const useIntangibleBenefits = function useIntangibleBenefits(listingId) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => closure_1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = intangibleBenefits(5822)(() => first);
  dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => callback(c2), items1);
  _require = listingId;
...
export const useTierEmojiIds = function useTierEmojiIds(listingId, guildId) {
  ...
  const tmp = tierEmojiIds(14673)(guildId, listingId);
  tierEmojiIds = tmp;
...
export const useTrialInterval = function useTrialInterval(editStateId) {
  const subscriptionTrial = _require(14656).useSubscriptionTrial(editStateId);
  let active_trial;
  const obj = _require(14656);
  ...
  let selectedOption = trialInterval(14676)(active_trial).selectedOption;
...
export const useEditStateIds = function useEditStateIds(groupListingId, guildId, arg2) {
  ...
  dependencyMap = undefined;
  closure_4 = undefined;
  subscriptionListingsForGroup = _require(14656).useSubscriptionListingsForGroup(groupListingId, obj);
