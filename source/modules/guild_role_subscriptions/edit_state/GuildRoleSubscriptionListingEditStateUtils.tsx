...
// Function ID: 14462
// Name: getRoleEmojis
// Dependencies: [5, 32, 19, 5176, 3923, 14462, 14439, 676, 1905, 589, 5716, 4541, 14463, 14464, 688, 3921, 14465, 14446, 14466, 705, 38, 5232, 5875, 12, 8740, 14467, 514, 2]
        ...
        try {
          ...
          if (0 === dependencyMap) {
            ...
            if (tmp25) {
              ...
              obj8[0] = lib(5875).updateSubscriptionTrial(callback, id, obj7);
              return obj8;
            }
          }
        } catch (tmp71) {
      ...
      if (1 === tmp4) {
        if (arg0 === 1) {
          ...
          if (null == id) {
            ...
            obj2[0] = callback3(5875).createSubscriptionGroupListing(callback, {});
            return obj2;
          } else {
...
export const usePriceTier = function usePriceTier(editStateId) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => outer1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = priceTier(5716)(() => first);
  let dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => _undefined(c2), items1);
...
export const useDescription = function useDescription(arg0) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => outer1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = description(5716)(() => first);
  let dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => _undefined(c2), items1);
  _require = arg0;
...
export const useRole = function useRole(listingId, guildId) {
  ...
  const tmp = importDefault(14463)(guildId, listingId);
  importDefault = tmp;
...
export const useRoleColor = function useRoleColor(editStateId, guildId) {
  const tmp = roleColor(14463)(guildId, editStateId);
  let closure_0 = tmp;
  const items = [tmp];
  let memo = React.useMemo(() => {
    let color;
...
export const useChannelAccessFormat = function useChannelAccessFormat(editStateId, guildId) {
  const tmp = channelAccessFormat(14463)(guildId, editStateId);
  let closure_0 = tmp;
  const items = [tmp];
  let memo = React.useMemo(() => {
    if (null == editStateId) {
...
export const useChannelBenefits = function useChannelBenefits(listingId) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => outer1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = channelBenefits(5716)(() => first);
  let dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => _undefined(c2), items1);
  _require = listingId;
...
export const useIntangibleBenefits = function useIntangibleBenefits(listingId) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => outer1_8.getSubscriptionListing(closure_0));
  let memo = stateFromStores;
  const tmp2 = intangibleBenefits(5716)(() => first);
  let dependencyMap = tmp2;
  const items1 = [stateFromStores, tmp2];
  memo = React.useMemo(() => _undefined(c2), items1);
  _require = listingId;
...
export const useTierEmojiIds = function useTierEmojiIds(listingId, guildId) {
  ...
  const tmp = tierEmojiIds(14463)(guildId, listingId);
  tierEmojiIds = tmp;
...
export const useTrialInterval = function useTrialInterval(editStateId) {
  const subscriptionTrial = _require(14446).useSubscriptionTrial(editStateId);
  let active_trial;
  ...
  let selectedOption = trialInterval(14466)(active_trial).selectedOption;
...
export const useEditStateIds = function useEditStateIds(groupListingId, guildId, arg2) {
  ...
  let dependencyMap;
  let c4;
  subscriptionListingsForGroup = _require(14446).useSubscriptionListingsForGroup(groupListingId, obj);
