...
// Dependencies: [5, 32, 19, 5057, 3983, 589, 5937, 5390, 14527, 14528, 11661, 1370, 2]
...
// Module 14526 (useFetchListingsForGuild)
...
export const useGroupListingsForGuild = function useGroupListingsForGuild(guildId) {
  const _require = guildId;
  let closure_1 = _require(14527).useGroupListingsFetchContext("useGroupListingsForGuild");
  const obj = _require(14527);
  const items = [makeGroupListingIndexSubscriptionListingTag];
  return _require(589).useStateFromStores(items, () => {
    if (null != closure_0) {
      if (closure_1) {
...
export const useArchiveSubscriptionListing = function useArchiveSubscriptionListing() {
  const tmp2 = callback2(importDefault(11661)(importAll(5937).archiveSubscriptionListing), 2);
