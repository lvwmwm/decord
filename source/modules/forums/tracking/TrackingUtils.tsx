...
// Dependencies: [5097, 3819, 6079, 6064, 1372, 4526, 3817, 4525, 6051, 676, 1379, 1235, 6080, 1381, 6081, 11, 6082, 2]
...
// Module 6078 (collectForumAnalyticsMetadata)
...
function collectForumAnalyticsMetadata(sessionId) {
  ...
  if (null != channel) {
    ...
    if (channel.isForumLikeChannel()) {
      ...
      obj[6] = importDefault(6081).getFilterTagIdsAnalytics();
      const obj3 = importDefault(6081);
      obj[7] = importDefault(6081).getSortOrderAnalytics(channel.id);
      if (sessionId == null) {
        let tmp8Result = tmp8(6080);
        sessionId = tmp8Result.getForumChannelSessionId(channel.id);
      }
      obj[8] = sessionId;
      tmp8Result = tmp8(6081);
      obj[9] = tmp8Result.getLayoutAnalytics(channel.id);
      obj[10] = channel.defaultSortOrder;
      const obj4 = importDefault(6081);
      obj[11] = importDefault(6081).getTagSettingAnalytics(channel.id);
      obj[12] = channel.defaultForumLayout;
      obj[13] = channel.isModeratorReportChannel();
      tmp = obj;
      const tmp8Result1 = importDefault(6081);
    }
  }
  return tmp;
}
...
export const getForumChannelSessionId = function getForumChannelSessionId(arg0) {
  return importDefault(6080).getForumChannelSessionId(arg0);
};
