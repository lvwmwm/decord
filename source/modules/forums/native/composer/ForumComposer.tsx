...
// Function ID: 9730
// Name: Tags
// Dependencies: [5, 32, 19, 17, 4247, 1302, 4592, 1942, 3883, 1931, 7054, 4591, 1874, 676, 1379, 1369, 1338, 1235, 505, 21, 4255, 712, 5581, 589, 4101, 4121, 8152, 4446, 9315, 7050, 5773, 4094, 5281, 4116, 7144, 4263, 6814, 12, 9317, 7814, 9730, 9731, 4595, 1236, 9732, 9734, 500, 9735, 9736, 9739, 1358, 9740, 1959, 9741, 9742, 7765, 1297, 8154, 1579, 1478, 9185, 8668, 9745, 4251, 9746, 9816, 9821, 9822, 7639, 9823, 9827, 5907, 10032, 4767, 9831, 7134, 4223, 10034, 7564, 4666, 4752, 2]
// Exports: default
...
function Tags(tags) {
  ...
  if (0 !== tags.length) {
    ...
    obj[1] = tmp.tagIcon;
    let items = [
      callback(require(7639) /* TagIcon */.TagIcon, obj),
      tags.map((id) => {
          let tmp2 = 0 !== arg1;
          if (tmp2) {
            let obj = { style: null };
...
function ActionBar(channel) {
  ...
  items2 = [tmp.actionsContainer, { marginBottom: importDefault(5281)({ includeKeyboardHeight: true }).insets.bottom }];
  if (isMediaChannelResult) {
    obj = { attachments: null, channelId: null, highlightThumbnails: true };
    obj[0] = stateFromStores;
    obj[1] = channel.id;
    isMediaChannelResult = callback(tmp5(9827), obj);
  }
  ...
  if (!stateFromStores1) {
    ...
    if (tmp10) {
      ...
      tmp10 = callback(tmp2(5907).HeaderActionButton, obj2);
    }
    items4[1] = tmp10;
    let tmp18 = lastInput === tmp2(9734).PostComposerInputs.CONTENT;
    if (tmp18) {
      ...
      obj3[2] = tmp2(7564).ReactionIcon;
      obj3[3] = onShowExpressionPicker;
      tmp18 = callback(tmp2(5907).HeaderActionButton, obj3);
    }
    items4[2] = tmp18;
    ...
    obj5[3] = callback(tmp2(4752).ChatIcon, obj6);
    obj5[4] = function onPress() {
      if (canPost) {
        callback({});
      }
    };
    obj4[1] = callback(tmp2(4666).Button, obj5);
    ...
    obj7[1] = items6;
    if (tmp8) {
      let ImageIcon = tmp2(10032).KeyboardIcon;
    } else {
      ImageIcon = tmp2(4767).ImageIcon;
    }
    obj7[2] = ImageIcon;
    ...
    callback(tmp2(5907).HeaderActionButton, obj7);
