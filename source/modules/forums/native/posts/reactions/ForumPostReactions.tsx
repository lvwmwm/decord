...
export const MostCommonForumPostReaction = function MostCommonForumPostReaction(thread) {
  ...
  const tmp = callback2();
  ({ disableReactionCreates, disableReactionUpdates } = importDefault(10197)(thread));
  ...
  if (null != first) {
    if (!disableReactionUpdates) {
      if (!disableReactionCreates) {
        ...
        let tmp7 = callback(require(9898) /* BurstReactionButton */.ForumPostReactionButton, obj);
      } else {
        tmp7 = null;
      }
      return tmp7;
