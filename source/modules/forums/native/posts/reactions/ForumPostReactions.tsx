...
export const MaxForumPostReactions = function MaxForumPostReactions(thread) {
  ...
  const disableReactionCreates = reactionContainerStyle(10313)(thread).disableReactionCreates;
  let num = 28;
  ...
  ,
    ...
    if (tmp6) {
      ...
      tmp6 = callback(thread(10301).AdditionalReactionCount, obj1);
    }
    items1[1] = tmp6;
    let tmp10 = !disableReactionCreates;
    if (tmp10) {
      const obj2 = { containerStyle: reactionContainerStyle, threadId: thread.id };
      tmp10 = callback(thread(10301).AddReactionButton, obj2);
    }
    items1[2] = tmp10;
    obj.children = items1;
...
export const MostCommonForumPostReaction = function MostCommonForumPostReaction(thread) {
  ...
  const tmp = callback2();
  ({ disableReactionCreates, disableReactionUpdates } = importDefault(10313)(thread));
  ...
  if (null != first) {
    if (!disableReactionUpdates) {
      if (!disableReactionCreates) {
        ...
        let tmp5 = callback(require(10301) /* BurstReactionButton */.ForumPostReactionButton, obj);
      } else {
        tmp5 = null;
      }
      return tmp5;
