...
export const MostCommonForumPostReaction = function MostCommonForumPostReaction(thread) {
  ...
  const tmp = callback2();
  ({ disableReactionCreates, disableReactionUpdates } = importDefault(10198)(thread));
  let obj = require(8403) /* useLoadForumUnreadCounts */;
  ...
  if (null != first) {
    if (!disableReactionUpdates) {
      if (!disableReactionCreates) {
        ...
        let tmp7 = callback(require(9899) /* BurstReactionButton */.ForumPostReactionButton, obj);
      } else {
        tmp7 = null;
      }
      return tmp7;
