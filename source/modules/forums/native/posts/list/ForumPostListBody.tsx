...
// Dependencies: [19, 17, 5845, 21, 4285, 11324, 11314, 11323, 11325, 11333, 11318, 2]
// Exports: default

// Module 11332 (ForumPostListBody)
import "noop";
...
export default function ForumPostListBody(arg0) {
  ...
  if (isNew) {
    ...
    isNew = callback(importDefault(11324), obj1);
  }
  const items1 = [isNew, callback(require(11314) /* ForumPostUsername */.ForumPostAuthor, { thread, hasUnreads }), callback(importDefault(11323), { thread, hasUnreads, format: ForumTimestampFormats.POSTED_DURATION_AGO })];
  obj[1] = items1;
  const items2 = [closure_6(View, obj), callback(importDefault(11325), { title: thread.name, lineClamp: 2, ellipsizeMode: "tail", hasUnreads }), callback(importDefault(11333), { messageContent, message: firstMessage, isMessageDeleted: false, messageLoaded: firstMessageLoaded, hasUnreads, senderModifier })];
  obj[1] = items2;
  ...
  if (!blocked) {
    ...
    if (null != media) {
      ...
      tmp7Result = callback(require(11318) /* ForumPostMediaAndroid */.ForumPostMediaThumbnail, obj4);
    }
  }
  items3[1] = tmp7Result;
  obj[1] = items3;
