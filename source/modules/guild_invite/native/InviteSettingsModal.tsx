...
// Dependencies: [32, 19, 1391, 9830, 1910, 4089, 676, 21, 4445, 712, 1500, 589, 38, 12, 17291, 9835, 4809, 1236, 4897, 709, 6189, 8295, 17292, 9831, 503, 5447, 5960, 2]
// Exports: default

// Module 17290 (AdvancedInstantInviteScreen)
...
function AdvancedInstantInviteScreen() {
  ...
  const effect = obj3.useEffect(() => {
    if (null == channel) {
      ...
      if (null != invitableChannelForGuild) {
        ...
        channel(9835).updateSettings(obj);
        const obj2 = channel(9835);
      } else {
        ...
        obj[2] = channel(9835).close;
        channel(4809).show(obj);
        const obj4 = channel(4809);
      }
    }
  }, items1);
  tmp2Result = tmp2(4897);
  const unmountEffect = tmp2Result.useUnmountEffect(() => {
    channel(709).wait(channel(9835).resetSettings);
  });
  const items2 = [channel];
  callback = obj3.useCallback(() => {
    if (null != channel) {
      if (closure_1_8.can(closure_1_10.CREATE_INSTANT_INVITE, tmp)) {
        const invite = channel(9835).createInvite("IOS Regenerate");
        const obj3 = channel(9835);
        channel(9835).close();
        const obj4 = channel(9835);
      }
    }
    let obj = channel(4809);
    obj = { title: null, body: null, onConfirm: null };
    const intl = navigation(1236).intl;
    obj[0] = intl.string(navigation(1236).t.VINpSK);
    const intl2 = navigation(1236).intl;
  ...
  obj[5] = channel(9831).getMaxUsesOptions;
  ...
  obj[12] = callback4;
  obj[13] = callback5;
  obj[1] = jsx(channel(17292), { style: tmp.formContent, channel: first, guild, maxAge: settings.maxAge, maxUses: settings.maxUses, maxUsesOptions: null, temporary: null, flags: null, roleIds: null, onChangeMaxAge: null, onChangeMaxUses: null, onChangeTemporary: null, onChangeFlags: null, onChangeRoleIds: null });
  return jsx(navigation(8295).Form, { style: tmp.formContent, channel: first, guild, maxAge: settings.maxAge, maxUses: settings.maxUses, maxUsesOptions: null, temporary: null, flags: null, roleIds: null, onChangeMaxAge: null, onChangeMaxUses: null, onChangeTemporary: null, onChangeFlags: null, onChangeRoleIds: null });
}
...
export default function InviteSettingsModal() {
  const memo = React.useMemo(() => {
    ...
    obj[2] = callback(5447).getHeaderCloseButton(callback2(9835).close);
