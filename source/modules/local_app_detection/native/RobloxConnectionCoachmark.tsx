...
function RobloxIcon(theme) {
  ...
  let str = "white";
  const value = importDefault(4871).get(constants.ROBLOX);
  if (isThemeDarkResult) {
    str = "black";
  }
  const obj2 = importDefault(4871);
...
export default function RobloxConnectionActionSheet(markAsDismissed) {
  ...
  obj[1] = callback(markAsDismissed(5552).ActionSheetCloseButton, { onPress: handleCancel });
  obj[2] = callback(markAsDismissed(5242).BottomSheetTitleHeader, obj);
  ...
  obj2[3] = items;
  const items1 = [callback2(markAsDismissed(4598).Stack, obj2), , ];
  ...
  const items2 = [callback(markAsDismissed(4185).Text, obj4), ];
  const obj5 = { variant: "text-md/medium", style: tmp.text, children: null };
  const intl2 = markAsDismissed(1236).intl;
  obj5[2] = intl2.string(markAsDismissed(1236).t.no96NU);
  items2[1] = callback(markAsDismissed(4185).Text, obj5);
  obj3[1] = items2;
  items1[1] = callback2(markAsDismissed(4598).Stack, obj3);
  ...
  const items3 = [callback(markAsDismissed(4600).Button, obj7), ];
  ...
  obj9[3] = handleCancel;
  items3[1] = callback(markAsDismissed(4600).Button, obj9);
  obj6[0] = items3;
  items1[2] = callback2(markAsDismissed(4598).Stack, obj6);
  obj1[2] = items1;
  obj[4] = callback2(markAsDismissed(4598).Stack, obj1);
...
export const useShouldShowRobloxConnectionCoachmark = function useShouldShowRobloxConnectionCoachmark() {
  ...
  const effect = React.useEffect(() => {
    ...
    if (tmp) {
      dependencyMap(true);
      const response = first(4995).fetch();
      const obj = first(4995);
    }
  }, items2);
