...
// Function ID: 16276
// Name: RobloxIcon
// Dependencies: [32, 19, 17, 5106, 1922, 13178, 676, 1388, 21, 4380, 712, 4108, 1629, 4347, 5503, 5502, 5871, 4813, 4376, 1236, 4815, 12440, 10565, 6125, 1364, 5108, 1435, 1297, 7985, 589, 13179, 5231, 4201, 1377, 2]
// Exports: default, useShouldShowRobloxConnectionCoachmark

// Module 16275 (RobloxIcon)
...
import closure_8 from "initialize" /* 13178 */;
...
export default function RobloxConnectionActionSheet(markAsDismissed) {
  ...
  const tmp = callback3();
  let obj = markAsDismissed(4108);
  const theme = obj.useThemeContext().theme;
  obj = { startExpanded: true, contentStyles: tmp.content, header: null, onDismiss: null, children: null };
  obj = { title: null, leading: callback(markAsDismissed(5871).ActionSheetCloseButton, { onPress: handleCancel }) };
  obj[2] = callback(markAsDismissed(5502).BottomSheetTitleHeader, obj);
  ...
  obj2[3] = items;
  const items1 = [callback2(markAsDismissed(4813).Stack, obj2), , ];
  ...
  const items2 = [callback(markAsDismissed(4376).Text, obj4), ];
  const obj5 = { variant: "text-md/medium", style: tmp.text, children: null };
  const intl2 = markAsDismissed(1236).intl;
  obj5[2] = intl2.string(markAsDismissed(1236).t.no96NU);
  items2[1] = callback(markAsDismissed(4376).Text, obj5);
  obj3[1] = items2;
  items1[1] = callback2(markAsDismissed(4813).Stack, obj3);
  ...
  obj7[1] = callback(markAsDismissed(12440).WindowLaunchIcon, { size: "sm", color: ThemesDefault.colors.WHITE });
  ...
  const items3 = [callback(markAsDismissed(4815).Button, obj7), ];
  ...
  obj9[3] = handleCancel;
  items3[1] = callback(markAsDismissed(4815).Button, obj9);
  obj6[0] = items3;
  items1[2] = callback2(markAsDismissed(4813).Stack, obj6);
  obj1[2] = items1;
  obj[4] = callback2(markAsDismissed(4813).Stack, obj1);
...
export const useShouldShowRobloxConnectionCoachmark = function useShouldShowRobloxConnectionCoachmark() {
  ...
  stateFromStores = stateFromStores(589).useStateFromStores(items, () => appInstalled.isAppInstalled(stateFromStores(13179).DetectableAppNames.ROBLOX));
  ...
  const effect = React.useEffect(() => {
    ...
    if (tmp) {
      dependencyMap(true);
      const response = first(5231).fetch();
