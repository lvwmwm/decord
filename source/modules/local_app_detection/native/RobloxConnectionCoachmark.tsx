...
// Function ID: 16357
// Name: RobloxIcon
// Dependencies: [32, 19, 17, 5172, 1922, 13251, 676, 1388, 21, 4444, 712, 4172, 1629, 4411, 5569, 5568, 5937, 4877, 4440, 1236, 4879, 12511, 10633, 6191, 1364, 5174, 1435, 1297, 8052, 589, 13252, 5297, 4265, 1377, 2]
// Exports: default, useShouldShowRobloxConnectionCoachmark

// Module 16356 (RobloxIcon)
...
import closure_8 from "initialize" /* 13251 */;
...
export default function RobloxConnectionActionSheet(markAsDismissed) {
  ...
  const tmp = callback3();
  let obj = markAsDismissed(4172);
  const theme = obj.useThemeContext().theme;
  obj = { startExpanded: true, contentStyles: tmp.content, header: null, onDismiss: null, children: null };
  obj = { title: null, leading: callback(markAsDismissed(5937).ActionSheetCloseButton, { onPress: handleCancel }) };
  obj[2] = callback(markAsDismissed(5568).BottomSheetTitleHeader, obj);
  ...
  obj2[3] = items;
  const items1 = [callback2(markAsDismissed(4877).Stack, obj2), , ];
  ...
  const items2 = [callback(markAsDismissed(4440).Text, obj4), ];
  const obj5 = { variant: "text-md/medium", style: tmp.text, children: null };
  const intl2 = markAsDismissed(1236).intl;
  obj5[2] = intl2.string(markAsDismissed(1236).t.no96NU);
  items2[1] = callback(markAsDismissed(4440).Text, obj5);
  obj3[1] = items2;
  items1[1] = callback2(markAsDismissed(4877).Stack, obj3);
  ...
  obj7[1] = callback(markAsDismissed(12511).WindowLaunchIcon, { size: "sm", color: ThemesDefault.colors.WHITE });
  ...
  const items3 = [callback(markAsDismissed(4879).Button, obj7), ];
  ...
  obj9[3] = handleCancel;
  items3[1] = callback(markAsDismissed(4879).Button, obj9);
  obj6[0] = items3;
  items1[2] = callback2(markAsDismissed(4877).Stack, obj6);
  obj1[2] = items1;
  obj[4] = callback2(markAsDismissed(4877).Stack, obj1);
...
export const useShouldShowRobloxConnectionCoachmark = function useShouldShowRobloxConnectionCoachmark() {
  ...
  stateFromStores = stateFromStores(589).useStateFromStores(items, () => appInstalled.isAppInstalled(stateFromStores(13252).DetectableAppNames.ROBLOX));
  ...
  const effect = React.useEffect(() => {
    ...
    if (tmp) {
      dependencyMap(true);
      const response = first(5297).fetch();
      const obj = first(5297);
    }
  }, items2);
  const items3 = [hasRoloxAccount];
  const effect1 = React.useEffect(() => {
    if (hasRoloxAccount) {
      let obj = stateFromStores(4265);
