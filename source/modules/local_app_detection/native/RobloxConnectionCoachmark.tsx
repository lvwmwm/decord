...
// Function ID: 15844
// Name: RobloxIcon
// Dependencies: [32, 19, 17, 4935, 1874, 12844, 676, 1369, 21, 4255, 712, 3967, 1581, 4223, 5309, 5308, 5618, 4664, 4251, 1236, 4666, 10334, 9151, 5915, 3965, 4937, 1416, 1297, 8196, 589, 12845, 5061, 4071, 1358, 2]
// Exports: default, useShouldShowRobloxConnectionCoachmark

// Module 15843 (RobloxIcon)
...
function RobloxIcon(theme) {
  ...
  let str = "white";
  const value = importDefault(4937).get(constants.ROBLOX);
  if (isThemeDarkResult) {
    str = "black";
  }
  const obj2 = importDefault(4937);
...
export default function RobloxConnectionActionSheet(markAsDismissed) {
  ...
  obj[1] = callback(markAsDismissed(5618).ActionSheetCloseButton, { onPress: handleCancel });
  obj[2] = callback(markAsDismissed(5308).BottomSheetTitleHeader, obj);
  ...
  obj2[3] = items;
  const items1 = [callback2(markAsDismissed(4664).Stack, obj2), , ];
  ...
  obj3[1] = items2;
  items1[1] = callback2(markAsDismissed(4664).Stack, obj3);
  ...
  obj7[1] = callback(markAsDismissed(10334).WindowLaunchIcon, obj8);
  ...
  const items3 = [callback(markAsDismissed(4666).Button, obj7), ];
  ...
  obj9[3] = handleCancel;
  items3[1] = callback(markAsDismissed(4666).Button, obj9);
  obj6[0] = items3;
  items1[2] = callback2(markAsDismissed(4664).Stack, obj6);
  obj1[2] = items1;
  obj[4] = callback2(markAsDismissed(4664).Stack, obj1);
...
export const useShouldShowRobloxConnectionCoachmark = function useShouldShowRobloxConnectionCoachmark() {
  ...
  stateFromStores = stateFromStores(589).useStateFromStores(items, () => appInstalled.isAppInstalled(stateFromStores(12845).DetectableAppNames.ROBLOX));
  ...
  const effect = React.useEffect(() => {
    ...
    if (tmp) {
      dependencyMap(true);
      const response = first(5061).fetch();
      const obj = first(5061);
    }
  }, items2);
