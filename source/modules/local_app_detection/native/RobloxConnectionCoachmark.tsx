...
// Function ID: 16422
// Name: RobloxIcon
// Dependencies: [32, 19, 17, 5177, 1922, 13277, 676, 1388, 21, 4445, 712, 4172, 1629, 4412, 5574, 5573, 5942, 4878, 4441, 1236, 4880, 12733, 9220, 6197, 1364, 5179, 1435, 1297, 8173, 589, 13278, 5302, 4266, 1377, 2]
// Exports: default, useShouldShowRobloxConnectionCoachmark

// Module 16421 (RobloxIcon)
...
import closure_8 from "initialize" /* 13277 */;
...
export default function RobloxConnectionActionSheet(markAsDismissed) {
  ...
  obj = { title: null, leading: callback(markAsDismissed(5942).ActionSheetCloseButton, { onPress: handleCancel }) };
  obj[2] = callback(markAsDismissed(5573).BottomSheetTitleHeader, obj);
  ...
  obj2[3] = items;
  const items1 = [callback2(markAsDismissed(4878).Stack, obj2), , ];
  ...
  const items2 = [callback(markAsDismissed(4441).Text, obj4), ];
  const obj5 = { variant: "text-md/medium", style: tmp.text, children: null };
  const intl2 = markAsDismissed(1236).intl;
  obj5[2] = intl2.string(markAsDismissed(1236).t.no96NU);
  items2[1] = callback(markAsDismissed(4441).Text, obj5);
  obj3[1] = items2;
  items1[1] = callback2(markAsDismissed(4878).Stack, obj3);
  ...
  obj7[1] = callback(markAsDismissed(12733).WindowLaunchIcon, { size: "sm", color: ThemesDefault.colors.WHITE });
  ...
  const items3 = [callback(markAsDismissed(4880).Button, obj7), ];
  ...
  obj9[3] = handleCancel;
  items3[1] = callback(markAsDismissed(4880).Button, obj9);
  obj6[0] = items3;
  items1[2] = callback2(markAsDismissed(4878).Stack, obj6);
  obj1[2] = items1;
  obj[4] = callback2(markAsDismissed(4878).Stack, obj1);
...
export const useShouldShowRobloxConnectionCoachmark = function useShouldShowRobloxConnectionCoachmark() {
  ...
  stateFromStores = stateFromStores(589).useStateFromStores(items, () => appInstalled.isAppInstalled(stateFromStores(13278).DetectableAppNames.ROBLOX));
  ...
  const effect = React.useEffect(() => {
    ...
    if (tmp) {
      dependencyMap(true);
      const response = first(5302).fetch();
      const obj = first(5302);
    }
  }, items2);
  const items3 = [hasRoloxAccount];
  const effect1 = React.useEffect(() => {
    if (hasRoloxAccount) {
      let obj = stateFromStores(4266);
