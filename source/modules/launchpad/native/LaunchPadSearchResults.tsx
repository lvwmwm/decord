...
// Function ID: 15902
// Name: renderItemJSX
// Dependencies: [19, 17, 1946, 6991, 1862, 4451, 21, 4255, 712, 15902, 5851, 589, 4671, 15903, 4797, 15904, 15906, 5646, 15907, 15909, 7202, 15910, 15916, 15917, 15919, 15113, 4251, 1348, 1236, 1474, 15905, 7790, 2]

// Module 15901 (renderItemJSX)
...
import jsxProd from "module_15910";
...
function renderItemJSX(result) {
  ...
  if (null == result) {
    ...
    } else if (tmp14(7202).AutocompleterResultTypes.TEXT_CHANNEL === type) {
      obj = { channel: null, navigationReplace: true, showGuildBadgeIcon: true };
      obj[0] = result.record;
      return callback(importDefault(15910), obj);
    } else if (tmp14(7202).AutocompleterResultTypes.GROUP_DM === type) {
      const obj1 = { channel: null, navigationReplace: true };
      obj1[0] = result.record;
      return callback(importDefault(15916), obj1);
    } else if (tmp14(7202).AutocompleterResultTypes.VOICE_CHANNEL === type) {
      const obj2 = { channel: null };
      obj2[0] = result.record;
      return callback(importDefault(15917), obj2);
    } else if (tmp14(7202).AutocompleterResultTypes.USER === type) {
      ({ record: obj4[0], comparator: obj4[1] } = result);
      return callback(importDefault(15919), { user: null, comparator: null });
    } else if (tmp14(7202).AutocompleterResultTypes.HEADER === type) {
      const obj4 = { name: null, styles: null, isRefreshEnabled: null };
      obj4[0] = result.record.text;
      obj4[1] = tmp;
      obj4[2] = tmp2;
      return tmp14(15113).renderCategoryItem(obj4);
    } else {
      obj = { variant: "text-sm/semibold", children: null };
      obj[1] = result.type;
      return callback(tmp14(4251).Text, obj);
    }
  }
}
...
let closure_14 = importAllResult.memo((guild) => {
  ...
  guild = guild.guild;
  const tmp = callback3();
  const tmp2 = importDefault(15902)();
  ...
  obj = { children: null };
  const tmp7 = importDefault(15903);
  obj1 = { unread, resolvedUnreadSetting: UnreadSetting.ALL_MESSAGES };
  const items4 = [callback(importDefault(15906), obj1), , ];
  obj2 = { size: tmp2.icon.guildIconSize, guild, style: items5 };
  items5 = [tmp.guildIcon, tmp2.icon.margin];
  items4[1] = callback(importDefault(5646), obj2);
  const obj3 = { name: guild.name, mentionBadge: null };
  const tmp8 = importDefault(15904);
  obj3[1] = importDefault(15909)({ mentionCount, locale: stateFromStores, isMentionLowImportance });
  items4[2] = importDefault(15907)(obj3);
  obj[0] = items4;
  obj[3] = tmp8(callback2(closure_10, obj), { fontScale });
  return tmp7(callback(guild(4797).PressableHighlight, obj));
});
let closure_16 = importAllResult.memo((arg0) => {
  ...
  let obj = require(15113) /* renderCategoryItem */;
  const categoryStyles = obj.useCategoryStyles();
  const tmp = callback3();
  const tmp3 = importDefault(1348)("WrappedCategoryItem");
  obj = { style: tmp.categoryWrapper, children: require(15113) /* renderCategoryItem */.renderCategoryItem({ name, onPress, note, styles: categoryStyles, isRefreshEnabled: tmp3 }) };
...
const memoResult1 = importAllResult.memo(function SearchResultsInner(results) {
  ...
  const tmp9 = categoryStyles(15902)();
  c4 = tmp9;
  fontScale = results(4671).useFontScale();
  ...
  if (results.length > 0) {
    ...
    tmp12Result = tmp12(categoryStyles(7790), obj);
  }
  obj[1] = tmp12Result;
