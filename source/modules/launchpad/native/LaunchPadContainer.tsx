...
// Dependencies: [19, 17, 10665, 21, 4661, 10666, 16179, 16181, 11155, 4115, 4744, 4235, 15274, 6377, 16182, 2]
// Exports: default

// Module 16178 (LaunchPadContainer)
import "noop";
...
export default function LaunchPadContainer(children) {
  ...
  const tmp4 = updaters(10666)();
  const tmp5 = updaters(16179)();
  launchPadSharedState = tmp5.launchPadSharedState;
  ({ launchPadPullTabState, launchPadShown, gestureState, updaters } = tmp5);
  ({ gesture, gestureRef } = updaters(16181)({ launchPadType: tmp4, launchPadSharedState, launchPadPullTabState, launchPadShown, gestureState, updaters }));
  const tmp7 = updaters(11155)();
  dependencyMap = tmp7;
  let obj = launchPadSharedState(4115);
  const fn = function o() {
  ...
  let obj1 = launchPadSharedState(4115);
  ...
  obj = { interpolate: launchPadSharedState(4115).interpolate, launchPadSharedState, withSpring: launchPadSharedState(4744).withSpring, windowDimensions: tmp7, LAUNCH_PAD_SPRING_CONFIG: closure_4 };
  fn3.__closure = obj;
  fn3.__workletHash = 13886247172712;
  fn3.__initData = closure_11;
  const animatedStyle = obj1.useAnimatedStyle(fn3);
  const MobileHomeDrawerExperiment = launchPadSharedState(4235).MobileHomeDrawerExperiment;
  ...
  let items1 = [callback(updaters(4115).View, { style: items, children: children.children }), ];
  if (tmp4 !== constants.DISABLED) {
    ...
    obj3[5] = updaters;
    const tmp10Result = tmp10(updaters(16182), obj3);
  }
  items1[1] = tmp10Result;
  obj2[1] = items1;
  obj1[1] = closure_7(closure_3, obj2);
  obj[1] = callback(launchPadSharedState(6377).GestureDetector, obj1);
  return callback(updaters(15274).Provider, obj);
};
