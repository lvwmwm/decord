...
// Dependencies: [19, 17, 10580, 21, 4444, 10581, 16389, 16391, 11626, 4184, 4878, 4304, 15447, 5599, 16392, 2]
// Exports: default

// Module 16388 (LaunchPadContainer)
...
export default function LaunchPadContainer(children) {
  ...
  const tmp = callback2();
  const tmp4 = updaters(10581)();
  const tmp5 = updaters(16389)();
  launchPadSharedState = tmp5.launchPadSharedState;
  ({ launchPadPullTabState, launchPadShown, gestureState, updaters } = tmp5);
  ({ gesture, gestureRef } = updaters(16391)({ launchPadType: tmp4, launchPadSharedState, launchPadPullTabState, launchPadShown, gestureState, updaters }));
  const tmp7 = updaters(11626)();
  dependencyMap = tmp7;
  let obj = launchPadSharedState(4184);
  const fn = function o() {
  ...
  obj1 = launchPadSharedState(4184);
  ...
  obj = { interpolate: launchPadSharedState(4184).interpolate, launchPadSharedState, withSpring: launchPadSharedState(4878).withSpring, windowDimensions: tmp7, LAUNCH_PAD_SPRING_CONFIG: closure_4 };
  fn3.__closure = obj;
  fn3.__workletHash = 13886247172712;
  fn3.__initData = closure_11;
  const animatedStyle = obj1.useAnimatedStyle(fn3);
  const MobileHomeDrawerExperiment = launchPadSharedState(4304).MobileHomeDrawerExperiment;
  ...
  let items1 = [callback(updaters(4184).View, { style: items, children: children.children }), ];
  if (tmp4 !== constants.DISABLED) {
    ...
    obj3[5] = updaters;
    const tmp10Result = tmp10(updaters(16392), obj3);
  }
  items1[1] = tmp10Result;
  obj2[1] = items1;
  obj1[1] = closure_7(closure_3, obj2);
  obj[1] = callback(launchPadSharedState(5599).GestureDetector, obj1);
  return callback(updaters(15447).Provider, obj);
};
