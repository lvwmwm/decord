// Module ID: 16496
// Function ID: 16497
// Dependencies: [19, 17, 5413, 1997, 1391, 1301, 4668, 21, 4446, 16488, 712, 15865, 6081, 589, 4640, 7575, 14846, 6139, 4900, 16497, 4925, 11567, 10130, 4140, 16499, 8090, 15750, 16489, 5033, 16500, 16501, 16492, 16495, 2]
...
const memoResult = importAllResult.memo(function TextChannel(subtitle) {
  ...
  if (null != tmp8ResultResult) {
    if (!isChannelSpoilerGated) {
      ...
      obj[3] = flag;
      obj[4] = tmp2(4140).ChannelListLayoutTypes.COMPACT;
      let result = callback(tmp2(10130).ChannelRowPreview, obj);
    }
    dependencyMap = tmp22;
    const items3 = [arr4.length > 0, arr4];
    const isActivitiesInTextEnabled = tmp2(8090).useIsActivitiesInTextEnabled(channel.id);
    ...
    const tmp2Result3 = tmp2(8090);
    const merged = Object.assign(tmp2(16500).useTextChannelPressEvents(channel, flag2));
    const tmp2Result4 = tmp2(16500);
    obj2 = { channel: null, unread: null, mentionCount: null };
    obj2[0] = channel;
    obj2[1] = unread;
    obj2[2] = mentionCount;
    const merged1 = Object.assign(tmp2(16501).getChannelAccessibilityProps(obj2));
    if (selected) {
    ...
    obj4[5] = tmp8(16495)(obj6);
    obj4[6] = unread;
    obj4[7] = resolvedUnreadSetting;
    obj4[8] = mentionCount;
    obj4[9] = flag;
    obj4[10] = tmp8(4640)(channel);
    obj4[11] = fontScale;
    ...
    obj.children = items5;
    return tmp8Result(tmp26(tmp2(5033).PressableHighlight, obj));
  }
  const tmp18 = arr4(4925)(channel.id);
  result = channel(16499).renderChannelSubtitle({ subtitle: subtitle.subtitle, muted: flag, channelId: channel.id, guildId: channel.guild_id });
});
