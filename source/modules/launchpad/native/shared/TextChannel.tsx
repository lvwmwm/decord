// Module ID: 16566
// Function ID: 16567
// Dependencies: [19, 17, 5448, 1996, 1387, 1301, 4702, 21, 4478, 16558, 712, 15934, 6116, 589, 4674, 7628, 14913, 6174, 4935, 16567, 4960, 11631, 10192, 4171, 16569, 8144, 15820, 16559, 5068, 16570, 16571, 16562, 16565, 2]
...
const memoResult = importAllResult.memo(function TextChannel(subtitle) {
  ...
  if (null != tmp8ResultResult) {
    if (!isChannelSpoilerGated) {
      ...
      let result = callback(tmp2(10192).ChannelRowPreview, obj);
    }
    dependencyMap = tmp22;
    const items3 = [arr4.length > 0, arr4];
    const isActivitiesInTextEnabled = tmp2(8144).useIsActivitiesInTextEnabled(channel.id);
    ...
    const tmp2Result3 = tmp2(8144);
    const merged = Object.assign(tmp2(16570).useTextChannelPressEvents(channel, flag2));
    const tmp2Result4 = tmp2(16570);
    obj2 = { channel: null, unread: null, mentionCount: null };
    obj2[0] = channel;
    obj2[1] = unread;
    obj2[2] = mentionCount;
    const merged1 = Object.assign(tmp2(16571).getChannelAccessibilityProps(obj2));
    if (selected) {
