...
// Dependencies: [1961, 1891, 3912, 11754, 11748, 676, 4253, 11755, 1988, 11756, 589, 11751, 11865, 1236, 11, 2]
// Exports: createGuildProgress, hideActionSheet, openActionSheet, useGuildProgressStep, useIsEligibleForGuildProgress

// Module 11753 (useIOSCompletionStates)
...
function useIOSCompletionStates(guild) {
  ...
  const guildPersonalized = _require(11751).useGuildPersonalized(guild);
  const obj2 = _require(11751);
  const guildPopulated = _require(11751).useGuildPopulated(guild);
  const obj3 = _require(11751);
  ...
  const channelsMessaged = _require(11751).useChannelsMessaged(items3);
...
export const openActionSheet = function openActionSheet(guild) {
  ...
  obj = { guild };
  obj.openLazy(require(1988) /* asyncRequireImpl */(11755, dependencyMap.paths), "guild-progress-" + guild.id, obj);
};
...
export const createGuildProgress = function createGuildProgress(closure_0) {
  if (null != guild.getGuild(closure_0)) {
    const progress = importDefault(11756).createProgress(closure_0);
    const obj = importDefault(11756);
  }
};
