...
// Dependencies: [1932, 1862, 3883, 11720, 11714, 676, 4223, 11721, 1959, 11722, 589, 11717, 11831, 1236, 11, 2]
// Exports: createGuildProgress, hideActionSheet, openActionSheet, useGuildProgressStep, useIsEligibleForGuildProgress

// Module 11719 (useIOSCompletionStates)
...
function useIOSCompletionStates(guild) {
  ...
  const guildPersonalized = _require(11717).useGuildPersonalized(guild);
  const obj2 = _require(11717);
  const guildPopulated = _require(11717).useGuildPopulated(guild);
  const obj3 = _require(11717);
  ...
  const channelsMessaged = _require(11717).useChannelsMessaged(items3);
...
export const openActionSheet = function openActionSheet(guild) {
  ...
  obj = { guild };
  obj.openLazy(require(1959) /* asyncRequireImpl */(11721, dependencyMap.paths), "guild-progress-" + guild.id, obj);
};
...
export const createGuildProgress = function createGuildProgress(closure_0) {
  if (null != guild.getGuild(closure_0)) {
    const progress = importDefault(11722).createProgress(closure_0);
    const obj = importDefault(11722);
  }
};
