...
// Dependencies: [32, 19, 1862, 3817, 5714, 4007, 676, 11, 589, 9152, 2]
// Exports: default

// Module 11721 (useGetJoinRequestAndGuildForInterviewChannel)
...
export default function useGetJoinRequestAndGuildForInterviewChannel(id) {
  ...
  const effect = joinRequest.useEffect(() => {
    if (!tmp) {
      dependencyMap(true);
      const requestToJoinGuilds = first(9152).fetchRequestToJoinGuilds();
      const obj = first(9152);
    }
  }, items1);
  ...
  const effect1 = joinRequest.useEffect(() => {
    if (null == joinRequest) {
      callback(true);
      const joinRequestForInterview = first(9152).fetchJoinRequestForInterview(_slicedToArray);
