...
// Dependencies: [1874, 4007, 3771, 4009, 3813, 4008, 687, 589, 709, 2]

// Module 5714 (updateSubmittedGuildJoinRequestTotal)
...
function updateSubmittedGuildJoinRequestTotal(guildId, DELETED, applicationStatus) {
  if (DELETED !== applicationStatus) {
    if (null != DELETED) {
      if (DELETED === require(4009) /* MAX_RESULTS_PER_PAGE */.GuildJoinRequestApplicationStatuses.SUBMITTED) {
        dependencyMap[guildId] = dependencyMap[guildId] + 1;
        const result = map.set(guildId, importDefault(3771)());
      }
      if (applicationStatus === tmp12(4009).GuildJoinRequestApplicationStatuses.SUBMITTED) {
        const _Math = Math;
        dependencyMap[guildId] = Math.max(0, dependencyMap[guildId] - 1);
        const result1 = map.set(guildId, importDefault(3771)());
      }
      tmp12 = require;
    }
  }
...
function upsert(joinRequestId) {
  ...
  obj = require(4008) /* isActionedApplicationStatus */;
  const tmp2 = require;
...
prototype["getRequests"] = function getRequests(guildId, applicationStatus) {
  ...
  if (obj.isActionedApplicationStatus(applicationStatus)) {
    ...
    tmp2Result = require(4008) /* isActionedApplicationStatus */;
  }
  return values;
};
...
prototype["hasFetched"] = function hasFetched(arg0) {
  if (map.has(arg0)) {
    ...
    let tmp3 = null != value;
    if (tmp3) {
      tmp3 = importDefault(3771)().diff(value, "seconds") < closure_20;
...
prototype["getSelectedApplicationTab"] = function getSelectedApplicationTab(arg0) {
  ...
  if (SUBMITTED == null) {
    SUBMITTED = require(4009) /* MAX_RESULTS_PER_PAGE */.GuildJoinRequestApplicationStatuses.SUBMITTED;
  }
  return SUBMITTED;
};
prototype["getSelectedSortOrder"] = function getSelectedSortOrder(arg0) {
  ...
  if (TIMESTAMP_DESC == null) {
    TIMESTAMP_DESC = require(4009) /* MAX_RESULTS_PER_PAGE */.GuildJoinRequestSortOrders.TIMESTAMP_DESC;
  }
...
const guildJoinRequestStoreV2 = new GuildJoinRequestStoreV2(require("dispatcher"), {
  ...
  GUILD_JOIN_REQUESTS_FETCH_SUCCESS: function handleFetchSuccess(arg0) {
    ...
    let c9 = false;
    ({ status, total } = arg0);
    if (status === require(4009) /* MAX_RESULTS_PER_PAGE */.GuildJoinRequestApplicationStatuses.SUBMITTED) {
      closure_6[guildId] = total;
      let result = map.set(guildId, importDefault(3771)());
    }
  ...
  GUILD_JOIN_REQUESTS_SET_SORT_ORDER: function handleSetSortOrder(arg0) {
    ...
    if (sortOrder !== dependencyMap3[guildId]) {
      ...
      tmpResult = require(4008) /* isActionedApplicationStatus */;
    }
  },
