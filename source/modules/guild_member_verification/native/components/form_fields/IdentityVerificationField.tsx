...
// Dependencies: [19, 17, 21, 4446, 712, 4442, 1236, 4397, 4893, 4271, 7634, 4689, 7715, 2010, 7718, 7751, 7680, 2]
// Exports: default

// Module 7633 (BaseIdentityVerificationField)
...
function BaseIdentityVerificationField(icon) {
  ...
  if (passesVerification) {
    ...
    obj[3] = tmp6(tmp7(4397).CheckmarkLargeIcon, { color: "status-positive" });
    obj1 = obj;
  } else {
    ...
    obj2[4] = onPress;
    obj1[1] = tmp6(tmp7(4893).Button, obj2);
  }
...
export default function IdentityVerificationField(arg0) {
  ...
  if (MAX_RESULTS_PER_PAGE.UserVerificationFieldPlatforms.EMAIL === platform) {
    ...
    if (tmp(4271).UserVerificationFieldPlatforms.PHONE === platform) {
    ...
    if (tmp(4271).UserVerificationFieldPlatforms.EMAIL === platform) {
      let EnvelopeIcon = tmp(7751).EnvelopeIcon;
    } else if (tmp(4271).UserVerificationFieldPlatforms.PHONE === platform) {
      EnvelopeIcon = tmp(7680).MobilePhoneIcon;
    } else {
      EnvelopeIcon = tmp(7751).EnvelopeIcon;
    }
    if (tmp(4271).UserVerificationFieldPlatforms.EMAIL === platform) {
      let fn = () => {
      ...
      fn = tmp(4271).UserVerificationFieldPlatforms.PHONE === platform ? (() => {
