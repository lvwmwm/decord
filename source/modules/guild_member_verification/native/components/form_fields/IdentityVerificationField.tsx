...
// Dependencies: [19, 17, 21, 4302, 712, 4298, 4254, 4712, 1236, 4121, 8391, 4507, 8456, 1988, 8459, 8488, 8427, 2]
// Exports: default

// Module 8390 (BaseIdentityVerificationField)
import "noop";
...
function BaseIdentityVerificationField(icon) {
  ...
  items[1] = callback(require(4298) /* Text */.Text, { style: items1, variant: "text-md/medium", color: "mobile-text-heading-primary", children: label });
  obj = { style: null, children: null };
  if (passesVerification) {
    obj[0] = tmp.verifiedContainer;
    obj[1] = tmp6(tmp7(4254).CheckmarkLargeIcon, { color: "status-positive" });
    let tmp9 = obj;
  } else {
    obj[0] = tmp.ctaButton;
    ...
    obj1[4] = onPress;
    obj[1] = tmp6(tmp7(4712).Button, obj1);
    tmp9 = obj;
  }
...
export default function IdentityVerificationField(arg0) {
  ...
  if (require(4121) /* MAX_RESULTS_PER_PAGE */.UserVerificationFieldPlatforms.EMAIL === platform) {
    ...
    if (tmp(4121).UserVerificationFieldPlatforms.PHONE === platform) {
    ...
    if (tmp(4121).UserVerificationFieldPlatforms.EMAIL === platform) {
      let EnvelopeIcon = tmp(8488).EnvelopeIcon;
    } else if (tmp(4121).UserVerificationFieldPlatforms.PHONE === platform) {
      EnvelopeIcon = tmp(8427).MobilePhoneIcon;
    } else {
      EnvelopeIcon = tmp(8488).EnvelopeIcon;
    }
    if (tmp(4121).UserVerificationFieldPlatforms.EMAIL === platform) {
      let fn = () => {
      ...
      fn = tmp(4121).UserVerificationFieldPlatforms.PHONE === platform ? (() => {
