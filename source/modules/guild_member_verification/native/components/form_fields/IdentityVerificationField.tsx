...
// Dependencies: [19, 17, 21, 4193, 712, 4189, 4145, 4604, 1236, 4013, 9186, 4399, 9241, 1959, 9244, 9271, 9217, 2]
// Exports: default
...
function BaseIdentityVerificationField(icon) {
  ...
  items[1] = callback(require(4189) /* Text */.Text, { style: items1, variant: "text-md/medium", color: "mobile-text-heading-primary", children: label });
  obj = { style: null, children: null };
  if (passesVerification) {
    obj[0] = tmp.verifiedContainer;
    obj[1] = tmp6(tmp7(4145).CheckmarkLargeIcon, { color: "status-positive" });
    let tmp9 = obj;
  } else {
    obj[0] = tmp.ctaButton;
    ...
    obj1[4] = onPress;
    obj[1] = tmp6(tmp7(4604).Button, obj1);
    tmp9 = obj;
  }
...
export default function IdentityVerificationField(arg0) {
  ...
  if (require(4013) /* MAX_RESULTS_PER_PAGE */.UserVerificationFieldPlatforms.EMAIL === platform) {
    ...
    if (tmp(4013).UserVerificationFieldPlatforms.PHONE === platform) {
    ...
    if (tmp(4013).UserVerificationFieldPlatforms.EMAIL === platform) {
      let EnvelopeIcon = tmp(9271).EnvelopeIcon;
    } else if (tmp(4013).UserVerificationFieldPlatforms.PHONE === platform) {
      EnvelopeIcon = tmp(9217).MobilePhoneIcon;
    } else {
      EnvelopeIcon = tmp(9271).EnvelopeIcon;
    }
    if (tmp(4013).UserVerificationFieldPlatforms.EMAIL === platform) {
      let fn = () => {
      ...
      fn = tmp(4013).UserVerificationFieldPlatforms.PHONE === platform ? (() => {
