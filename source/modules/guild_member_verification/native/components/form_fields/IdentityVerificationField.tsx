...
function BaseIdentityVerificationField(icon) {
  ...
  items[1] = callback(require(4185) /* Text */.Text, { style: items1, variant: "text-md/medium", color: "mobile-text-heading-primary", children: label });
  obj = { style: null, children: null };
  if (passesVerification) {
    obj[0] = tmp.verifiedContainer;
    obj[1] = tmp6(tmp7(4141).CheckmarkLargeIcon, { color: "status-positive" });
    let tmp9 = obj;
  } else {
    obj[0] = tmp.ctaButton;
    ...
    obj1[4] = onPress;
    obj[1] = tmp6(tmp7(4600).Button, obj1);
    tmp9 = obj;
  }
...
export default function IdentityVerificationField(arg0) {
  ...
  if (require(4009) /* MAX_RESULTS_PER_PAGE */.UserVerificationFieldPlatforms.EMAIL === platform) {
    ...
    if (tmp(4009).UserVerificationFieldPlatforms.PHONE === platform) {
    ...
    if (tmp(4009).UserVerificationFieldPlatforms.EMAIL === platform) {
      ...
      fn = tmp(4009).UserVerificationFieldPlatforms.PHONE === platform ? (() => {
