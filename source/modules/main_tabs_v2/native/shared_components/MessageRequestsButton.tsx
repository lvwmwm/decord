...
// Dependencies: [19, 17, 6018, 6019, 21, 4445, 589, 15532, 4880, 1236, 7938, 12833, 9894, 2]
// Exports: default

// Module 15531 (MessageRequestAnimation)
...
import closure_6 from "processChannel" /* 6019 */;
...
function MessageRequestAnimation(color) {
  ...
  return callback(ref(15532).MessageRequestLottie, { ref, color: color.color, size: "sm", autoPlay: true });
}
...
export default function MessageRequestsButton(alternateVariant) {
  ...
  if (flag) {
    ...
    if (null != str) {
      ...
      let tmp21 = callback(tmp3(4880).Button, obj);
      let tmp16 = callback;
    } else {
      tmp16 = callback;
      ...
      tmp21 = callback(tmp3(7938).IconButton, obj1);
    }
    const items2 = [tmp21, str > 0 && tmp16(tmp3(12833).ButtonBadge, { badgePosition: "right" })];
    obj[2] = items2;
