...
// Function ID: 8816
// Name: _toPropertyKey
// Dependencies: [109, 32, 19, 5948, 5944, 5945, 1882, 3826, 1874, 676, 3813, 5943, 6167, 709, 5947, 5104, 12, 1236, 8816, 2]
// Exports: default

// Module 8815 (_toPropertyKey)
...
prototype["subscribe"] = function subscribe(onUpdate) {
  ...
  if (flag) {
    userSearchContext = importDefault(6167).getUserSearchContext((results) => {
...
prototype["fetch"] = function fetch(toLocaleLowerCase) {
  const self = this;
  let obj = require(5947) /* cleanString */;
  ...
  if ("" === cleanStringResult) {
    ...
    if (arg1) {
      const members = importDefault(5104).requestMembers(null, cleanStringResult);
      const obj2 = importDefault(5104);
    }
...
prototype["initializeUsersFromCache"] = function initializeUsersFromCache() {
  const self = this;
  const all = importDefault(5943).getAll();
...
prototype["updateUser"] = function updateUser(id) {
  if (importDefault(5943).shouldUseCache) {
    return false;
  } else {
    const self = this;
...
prototype["getItem"] = function getItem(closure_0) {
  ...
  if (null == user) {
    ...
    const self = this;
    names = require(5947) /* cleanString */.getNames(user);
    ({ nick, names } = names);
    ...
    obj[3] = nick;
    const obj4 = require(5947) /* cleanString */;
    const tmp12 = require;
    const relationshipType = require(5947) /* cleanString */.getRelationshipType(user.id);
