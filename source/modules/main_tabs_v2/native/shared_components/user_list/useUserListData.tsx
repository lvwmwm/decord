...
// Function ID: 9542
// Name: _toPropertyKey
// Dependencies: [109, 32, 19, 7044, 7040, 7041, 1911, 3921, 1903, 676, 3908, 7039, 7233, 709, 7043, 5214, 12, 1236, 9004, 2]
// Exports: default
...
prototype["subscribe"] = function subscribe(onUpdate) {
  ...
  if (flag) {
    userSearchContext = importDefault(7233).getUserSearchContext((results) => {
...
prototype["fetch"] = function fetch(toLocaleLowerCase) {
  const self = this;
  let obj = require(7043) /* cleanString */;
  ...
  if ("" === cleanStringResult) {
    ...
    if (arg1) {
      const members = importDefault(5214).requestMembers(null, cleanStringResult);
      const obj2 = importDefault(5214);
    }
...
prototype["initializeUsersFromCache"] = function initializeUsersFromCache() {
  const self = this;
  const all = importDefault(7039).getAll();
...
prototype["updateUser"] = function updateUser(id) {
  if (importDefault(7039).shouldUseCache) {
    return false;
  } else {
    const self = this;
...
prototype["getItem"] = function getItem(closure_0) {
  ...
  if (null == user) {
    ...
    const self = this;
    names = require(7043) /* cleanString */.getNames(user);
    ({ nick, names } = names);
    ...
    obj[3] = nick;
    const obj4 = require(7043) /* cleanString */;
    const tmp12 = require;
    const relationshipType = require(7043) /* cleanString */.getRelationshipType(user.id);
