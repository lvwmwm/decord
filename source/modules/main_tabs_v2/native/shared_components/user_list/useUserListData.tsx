...
// Function ID: 9558
// Name: _toPropertyKey
// Dependencies: [109, 32, 19, 7063, 7059, 7060, 1911, 3938, 1903, 676, 3925, 7058, 7252, 709, 7062, 5230, 12, 1236, 9021, 2]
// Exports: default

// Module 9557 (_toPropertyKey)
...
prototype["subscribe"] = function subscribe(onUpdate) {
  ...
  if (flag) {
    userSearchContext = importDefault(7252).getUserSearchContext((results) => {
...
prototype["fetch"] = function fetch(toLocaleLowerCase) {
  const self = this;
  let obj = require(7062) /* cleanString */;
  ...
  if ("" === cleanStringResult) {
    ...
    if (arg1) {
      const members = importDefault(5230).requestMembers(null, cleanStringResult);
      const obj2 = importDefault(5230);
    }
...
prototype["initializeUsersFromCache"] = function initializeUsersFromCache() {
  const self = this;
  const all = importDefault(7058).getAll();
...
prototype["updateUser"] = function updateUser(id) {
  if (importDefault(7058).shouldUseCache) {
    return false;
  } else {
    const self = this;
...
prototype["getItem"] = function getItem(closure_0) {
  ...
  if (null == user) {
    ...
    const self = this;
    names = require(7062) /* cleanString */.getNames(user);
    ({ nick, names } = names);
    ...
    obj[3] = nick;
    const obj4 = require(7062) /* cleanString */;
    const tmp12 = require;
    const relationshipType = require(7062) /* cleanString */.getRelationshipType(user.id);
