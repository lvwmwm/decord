...
// Dependencies: [19, 17, 14217, 21, 4255, 712, 5245, 4676, 689, 3959, 14300, 4116, 4664, 2]

// Module 15354 (YouBarMaskedBackground)
...
function YouBarMaskedBackground(barWidth) {
  ...
  const tmp3 = importDefault(5245);
  const tmp4 = importDefault(4676);
...
function YouBarAnimatedBackground(arg0) {
  ...
  const tmp3 = importDefault(14300)(token);
  _require = tmp3;
  const tmp4 = importDefault(14300)(token, YOU_BAR_HEIGHT / 2);
  importDefault = tmp4;
  const tmp = callback3();
  const fn = function u() {
  ...
  obj = { withSpring: _require(4664).withSpring, questDockAnimatedBorderRadius: tmp3, YOU_BAR_SPRING_CONFIG, questDockAnimatedBottomLeftRadius: tmp4 };
  fn.__closure = obj;
  ...
  return callback(importDefault(4116).View, { style });
}
