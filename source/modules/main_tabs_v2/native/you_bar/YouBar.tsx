// Module ID: 15884
// Function ID: 15885
// Dependencies: [5, 32, 19, 17, 4266, 4268, 1923, 14632, 676, 1388, 21, 4446, 712, 1494, 14625, 11060, 4303, 14631, 14635, 4186, 4892, 8569, 15885, 15886, 589, 8845, 15888, 1236, 6210, 4301, 4414, 1370, 12, 15891, 2010, 4300, 8992, 6151, 1377, 15532, 5617, 1297, 15902, 15903, 15904, 15909, 15912, 4173, 15914, 15915, 2]
...
import ConnectionBannerIconDefault from "ConnectionBannerIcon" /* 15915 */;
import closure_3 from "asyncGeneratorStep" /* 5 */;
import closure_4 from "_slicedToArray" /* 32 */;
import importAllResult from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_8 from "isSyncedModeThemesEnabled" /* 4266 */;
...
let closure_26 = importAllResult.memo(() => {
  ...
  obj1 = isMobileQuestDockRenderedBase(14625);
  isMobileQuestDockRenderedBase = obj1.useIsMobileQuestDockRenderedBase(mobileQuestDock);
  let obj2 = isMobileQuestDockRenderedBase(11060);
  const drawerWidth = obj2.useDrawerWidth();
  let obj3 = isMobileQuestDockRenderedBase(14631);
  const youBarHorizontalMargin = obj3.useYouBarHorizontalMargin();
  let result = 2 * youBarHorizontalMargin;
  const tmp10 = youBarHorizontalMargin(4303)().isChatBesideChannelList ? drawerWidth - result : youBarHorizontalMargin(1494)().width - result;
  dependencyMap = tmp10;
  let tmp4Result = tmp4(14631);
  const youBarBottomMargin = tmp4Result.useYouBarBottomMargin();
  tmp4Result = tmp4(14635);
  ...
  sharedValue = isMobileQuestDockRenderedBase(4186).useSharedValue(false);
  const tmp4Result1 = isMobileQuestDockRenderedBase(4186);
  let fn = function r() {
    let obj = { marginBottom: isMobileQuestDockRenderedBase(4892).withSpring(youBarBottomMargin + connectionBannerHeight, callback4), transform: null };
    const obj2 = isMobileQuestDockRenderedBase(4892);
    const tmp = callback4;
    let num = 1;
    if (sharedValue.get()) {
      num = 1;
    ...
    obj = { scale: isMobileQuestDockRenderedBase(4892).withSpring(num, tmp) };
    const items = [obj];
    obj[1] = items;
    return obj;
  };
  obj = { withSpring: tmp4(4892).withSpring, barMarginBottom: youBarBottomMargin, connectionBannerHeight, YOU_BAR_SPRING_CONFIG: callback4, isPressedValue: sharedValue, isQuestRendered: isMobileQuestDockRenderedBase };
  fn.__closure = obj;
  fn.__workletHash = 7314807713815;
  fn.__initData = closure_22;
  const animatedStyle = isMobileQuestDockRenderedBase(4186).useAnimatedStyle(fn);
  const tmp4Result2 = isMobileQuestDockRenderedBase(4186);
  const iCYMIEnabled = isMobileQuestDockRenderedBase(8569).useICYMIEnabled("TabsNavigator");
  const tmp4Result3 = isMobileQuestDockRenderedBase(8569);
  const youBarCoachmark = isMobileQuestDockRenderedBase(15885).useYouBarCoachmark({ isQuestRendered: isMobileQuestDockRenderedBase });
  const visibleContent = youBarCoachmark.visibleContent;
  const markAsDismissed = youBarCoachmark.markAsDismissed;
  const tmp4Result4 = isMobileQuestDockRenderedBase(15885);
  const showTinyBroncoPromoSheet = isMobileQuestDockRenderedBase(15886).useShowTinyBroncoPromoSheet({ visibleContent, markAsDismissed });
  ...
  const tmp4Result5 = isMobileQuestDockRenderedBase(15886);
  ...
  const nameplate = isMobileQuestDockRenderedBase(8845).useNameplate({ user: stateFromStores });
  const tmp4Result7 = isMobileQuestDockRenderedBase(8845);
  const youBarAccessibilityLabel = isMobileQuestDockRenderedBase(15888).useYouBarAccessibilityLabel(stateFromStores);
  ...
  const callback = sharedValue.useCallback((nativeEvent) => {
    if ("open-settings" === nativeEvent.nativeEvent.actionName) {
      isMobileQuestDockRenderedBase(6210).openUserSettings();
      const obj = isMobileQuestDockRenderedBase(6210);
    }
  }, []);
  ...
  const effect1 = sharedValue.useEffect(() => {
    const rootNavigationRef = isMobileQuestDockRenderedBase(4301).getRootNavigationRef();
          ...
          if (!someResult) {
            if (null != closure_1_10.current) {
              if ("press" === tmp9.current) {
                const result = isMobileQuestDockRenderedBase(4414).triggerHapticFeedback(isMobileQuestDockRenderedBase(4414).HapticFeedbackTypes.SOFT);
                obj.navigate("you");
                const obj3 = isMobileQuestDockRenderedBase(4414);
              } else {
    ...
    const obj = isMobileQuestDockRenderedBase(4301);
  }, []);
  ...
  callback4 = sharedValue.useCallback(() => {
    const coerceGuildsRouteResult = isMobileQuestDockRenderedBase(4300).coerceGuildsRoute(youBarHorizontalMargin(8992)());
    if (null != coerceGuildsRouteResult) {
      let tmpResult = tmp(4414);
      const result = tmpResult.triggerHapticFeedback(tmp(4414).HapticFeedbackTypes.SOFT);
      ...
      if (guildId === closure_1_17) {
        ...
        tmp(6151).transitionToGuild(tmp5);
        const tmpResult1 = tmp(6151);
      }
    }
  }, []);
  const context = sharedValue.useContext(tmp2(15532));
  const gesture = context.gesture;
  const translateX = context.translateX;
  const tmp4Result8 = isMobileQuestDockRenderedBase(15888);
  closure_16 = isMobileQuestDockRenderedBase(4186).useSharedValue(0);
  const items7 = [callback4, gesture, tmp26[0]];
  const memo3 = sharedValue.useMemo(() => {
    const Gesture = isMobileQuestDockRenderedBase(5617).Gesture;
    ...
    obj = { runOnJS: isMobileQuestDockRenderedBase(4186).runOnJS, handleNavBetweenGuildsAndDMs: callback4 };
    fn3.__closure = obj;
  ...
  const items9 = [callback(youBarHorizontalMargin(15902), { hasNameplate: null != nameplate, isLargeAvatar: !isMobileQuestDockRenderedBase, barWidth: tmp10, isQuestRendered: isMobileQuestDockRenderedBase, avatarSize: tmp37 }), , , ];
  ...
  if (null != nameplate) {
    ...
    obj1[3] = tmp37;
    tmp40Result = tmp40(tmp2(15903), obj1);
  }
  ...
  obj3[10] = callback1;
  obj3[11] = closure_16;
  obj3[12] = callback(youBarHorizontalMargin(15904), { isQuestRendered: isMobileQuestDockRenderedBase, onAvatarPress: memo2 });
  obj2[1] = callback(markAsDismissed, obj3);
  items9[2] = callback(isMobileQuestDockRenderedBase(5617).GestureDetector, obj2);
  const obj4 = { style: tmp.youRowRight, children: null };
  tmp40Result = null;
  if (iCYMIEnabled) {
    const obj5 = { hasNameplate: null };
    obj5[0] = tmp22;
    tmp40Result = tmp40(tmp2(15909), obj5);
  }
  const items10 = [tmp40Result, callback(youBarHorizontalMargin(15912), { hasNameplate: null != nameplate })];
  obj4[1] = items10;
  items9[3] = closure_20(visibleContent, obj4);
  obj[3] = items9;
  return closure_20(youBarHorizontalMargin(4186).View, obj);
});
const memoResult = importAllResult.memo(function YouBarThemed() {
  ...
  const items1 = [callback(_modDef15914, {}), callback(closure_26, {}), callback(ConnectionBannerIconDefault, {})];
  obj[1] = items1;
