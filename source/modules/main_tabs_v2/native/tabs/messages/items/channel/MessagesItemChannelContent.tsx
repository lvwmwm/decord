...
// Dependencies: [19, 17, 4356, 9841, 4540, 21, 4344, 712, 1297, 8632, 8541, 9868, 9770, 1364, 4280, 1367, 589, 14618, 3943, 8981, 15238, 11, 4534, 15239, 4065, 9516, 9517, 4340, 9298, 9177, 9836, 4039, 9683, 1236, 15241, 2]

// Module 15237 (MessagesItemChannelContentIcon)
...
const memoResult = importAllResult.memo(function MessagesItemChannelContent(channel) {
  ...
  const isThemeLightResult = obj.isThemeLight(importDefault(4280)());
  ...
  const tmp10 = callback3(channelSelected, tmp6, tmp8, hasNameplate, importDefault(1367)("MessagesItemChannelContent"));
  ...
  const tmp12 = importDefault(14618)(channel, { unread: hasUnreadMessages });
  ...
  const tmp17 = importDefault(4534)(channel);
  const tmp18 = importDefault(15239)(channel, stateFromStores);
  const waveShouldShow = tmp18.waveShouldShow;
  obj = { variant: null, style: null, lineClamp: 1, ellipsizeMode: "tail" };
  const tmp5 = importDefault(1367)("MessagesItemChannelContent");
  obj[0] = channel(4065).useToken(importDefault(712).modules.mobile.MESSAGES_ITEM_CHANNEL_NAME_TEXT_STYLE);
  const items1 = [, ];
  ...
  if (channel.isDM()) {
    if (null != channel.recipients) {
      if (channel.recipients.length > 0) {
        ...
        obj4[1] = tmp17;
        tmp3Result = tmp3(9516);
        const EffectDisplayType = tmp(9517).EffectDisplayType;
  ...
  obj5.children = tmp17;
  const items2 = [callback(channel(4340).Text, obj5), , ];
  let tmp21Result = null;
  if (tmp16) {
    const obj6 = { userId: null, disabledTooltip: true };
    obj6[0] = channel.recipients[0];
    tmp21Result = tmp21(tmp3(9298), obj6);
  }
  ...
  if (channel.isSystemDM()) {
    ...
    obj7[0] = tmp9.botTag;
    obj7[1] = tmp3(9177).Types.SYSTEM_DM;
    tmp21Result = tmp21(tmp3(9177), obj7);
    const tmp3Result1 = tmp3(9177);
  }
  ...
  if (!hasNameplate) {
    ...
    if (!waveShouldShow) {
      ...
      tmp21Result1 = tmp21(tmp(4340).Text, obj13);
    }
    ...
    if (tmp13) {
      ...
      obj15[4] = muted;
      let tmp21Result2 = tmp21(tmp(9836).ChannelRowPreview, obj15);
      ...
      obj16[2] = guild_id;
      tmp21Result2 = tmp21(tmp3(9683), obj16);
      const tmp3Result2 = tmp3(9683);
    } else {
      tmp21Result2 = null;
      if (tmp3(8981)(channel.id)) {
        ...
        tmp21Result2 = tmp21(tmp(4340).Text, obj17);
      }
    }
