...
// Dependencies: [32, 19, 17, 7554, 1995, 14616, 7569, 1921, 16294, 673, 675, 1383, 7148, 21, 4218, 8176, 4481, 1235, 709, 686, 4336, 644, 1233, 5084, 16327, 8161, 8698, 500, 8117, 1627, 8158, 4416, 8174, 4338, 1492, 4166, 1499, 16706, 1431, 8178, 6885, 8121, 5087, 8132, 8145, 7122, 8155, 1372, 7326, 5542, 16707, 11899, 7096, 16710, 16711, 16712, 4205, 16720, 8152, 8138, 8168, 685, 10580, 8731, 16722, 11828, 8118, 8706, 7320, 7474, 11045, 7408, 2]
// Exports: default

// Module 16705 (handleBackButtonPress)
...
import closure_9 from "zustandStore" /* 14616 */;
import closure_10 from "updateGuildUnreadSentinel" /* 7569 */;
...
function BackButton(paddingTop) {
  ...
  obj = { style: items1, accessibilityRole: "button", accessibilityLabel: formatToPlainStringResult, onPress: handleBackButtonPress, children: callback2(tmp2(16327).CloseIconWithBadgeOnSide, { count: stateFromStores }) };
...
function UnconnectedYouScreen(arg0) {
  ...
  const tmp7Result = navigateToShop(8161)(tmp8);
  callback = tmp7Result;
  const tmp10 = callback3(tmp7Result);
  obj = { type: null, name: null };
  let tmp5Result = tmp5(8698);
  ...
  rect = tmp5(1627)();
  const tmp7 = navigateToShop(8161);
  ({ theme, primaryColor, secondaryColor } = navigateToShop(8158)({ user, displayProfile: obj2 }));
  const tmp14 = navigateToShop(8158)({ user, displayProfile: obj2 });
  const ref = obj.useRef(null);
  let obj3 = navigateToSettings(4218);
  sharedValue = obj3.useSharedValue(0);
  let obj4 = navigateToSettings(4218);
  ...
  const tmp15 = navigateToShop(4416)();
  const animatedScrollHandler = obj4.useAnimatedScrollHandler(fn);
  ({ bannerAnimatedStyle, bannerImageAnimatedStyle, contentAnimatedStyle, blurAnimatedProps, showBlur } = navigateToShop(8174)({ scrollPosition: sharedValue, bannerHeight: tmp7Result }));
  const size = tmp5(1492)();
  const height = size.height;
  let num = 0;
  const diff = size.width - rect.right - rect.left;
  if (navigateToShop(4338)().isChatBesideChannelList) {
    num = 16;
  }
  ...
  tmp12Result = tmp12(16706);
  let tmp28 = !isFocused;
  if (!isFocused) {
    tmp28 = !tmp12Result.useIsProfileModalTransitioning();
  }
  const tmp19 = navigateToShop(8174)({ scrollPosition: sharedValue, bannerHeight: tmp7Result });
  ...
  if (isFocused) {
    ...
    obj2[1] = obj2;
    let tmp37 = null == obj2;
    const userProfileBannerBackgroundColor = tmp12(8178).useUserProfileBannerBackgroundColor(obj2);
    if (!tmp37) {
      tmp37 = !obj2.isLoaded;
    }
    let items = [tmp7Result, bound, rect.bottom];
    const tmp12Result3 = tmp12(8178);
    ...
    const tmp38 = tmp5(6885)();
    obj3 = { layout: "YOU_SCREEN", userId: null };
    obj3[1] = user.id;
    const createUserProfileAnalyticsContext = tmp12(8121).useCreateUserProfileAnalyticsContext(obj3);
    const tmp12Result4 = tmp12(8121);
    const isScreenLandscape = tmp12(5087).useIsScreenLandscape();
    let tmp43;
    tmp5Result = tmp5(8132);
    ...
    obj4[0] = tmp47;
    obj4[1] = ref2.current;
    const items1 = [tmp5(7122).YOU_SCREEN];
    obj4[2] = items1;
    obj4[3] = createUserProfileAnalyticsContext;
    tmp5(8145)(obj4);
    num2 = 0;
    if (null != tmp5Result1Result) {
      num2 = tmp5(8155)(tmp5Result1Result, bound).overflowTop;
    }
    const items2 = [num2];
    if (!tmp38) {
      ...
      tmpResult = tmp(tmp12(7326).useSelectedDismissibleContent(memo1), 2);
      closure_10 = tmp58;
      ...
      const tmp12Result7 = tmp12(7326);
      const obj5 = { disabled: null };
      obj5[0] = tmp63;
      youSettingsCoachmark = tmp12(16707).useYouSettingsCoachmark(obj5);
      let tmp65 = null != youSettingsCoachmark;
      const tmp12Result8 = tmp12(16707);
      const customTypingIndicatorConfig = tmp12(11899).useCustomTypingIndicatorConfig("YouScreen");
        ...
        if (customTypingIndicatorConfig.canSet) {
          if (null != obj2) {
            if (!tmp63) {
              ...
              const tmpResult1 = tmp(tmp12(7326).useSelectedDismissibleContent(items5), 2);
              closure_13 = tmp68;
              ...
              closure_15 = tmp70;
              const tmp12Result10 = tmp12(7326);
              ...
              if (tmp70) {
                ...
                if (tmp72) {
                  if (tmp63) {
                    ...
                    obj6[8] = callback1;
                    tmp63 = callback2(tmp5(16710), obj6);
                  }
                  const items6 = [tmp63, , ];
                  if (tmp65) {
                    ...
                    tmp65 = callback2(tmp5(16707), obj7);
                    const tmp5Result2 = tmp5(16707);
                  }
                  items6[1] = tmp65;
                  let tmp80 = tmp69;
                  if (tmp69) {
                    ...
                    obj8[1] = tmp69;
                    obj8[2] = tmp68;
                    tmp80 = callback2(tmp5(16711), obj8);
                  }
                  const obj9 = { zIndex: 1, children: null };
                  items6[2] = tmp80;
                  obj9[1] = items6;
                  tmp74Result = closure_21(tmp12(7096).LayerScope, obj9);
              ...
              const tmp85 = callback2(tmp5(16712), obj10);
              if (tmp84Result) {
                ...
                obj14[2] = tmp12(8138).ProfileFrameLayerOrder.BACK;
                obj14[3] = bound;
                tmp84Result = tmp84(tmp5(8152), obj14);
                const tmp5Result4 = tmp5(8152);
              }
              ...
              items10[1] = callback2(tmp5(8168), obj15);
              ...
              if (!tmp) {
                ...
                items15[2] = tmp;
                obj19[1] = items15;
                items12[1] = tmp89(tmp5(4218).View, obj19);
                obj17[1] = items12;
                const items16 = [tmp89(tmp5(4218).View, obj17), , , ];
                if (tmp89Result) {
                  ...
                  obj25[0] = skuId2;
                  obj25[3] = tmp28;
                  const items18 = [tmp84(tmp5(8731), obj25), ];
                  tmp = BackButton;
                  const obj26 = { paddingTop: null };
                  obj26[0] = bound1;
                  items18[1] = tmp84(BackButton, obj26);
                  obj23[2] = items18;
                  tmp89Result = tmp89(tmp5(4218).View, obj23);
                }
                ...
                obj27[9] = initialTab;
                obj27[10] = !tmp28;
                items16[2] = tmp84(tmp5(16722), obj27);
                items16[3] = tmp84(tmp12(11828).TTIFirstContentfulPaint, { label: "you_screen" });
                obj16[6] = items16;
                ...
                if (tmp) {
                  ...
                  tmp = tmp5(8152);
                  obj28[2] = tmp12(8138).ProfileFrameLayerOrder.FRONT;
                  obj28[3] = bound;
                  tmp = tmp84(tmp, obj28);
                }
                ...
                obj11[3] = tmp84(tmp12(8121).UserProfileAnalyticsProvider, obj12);
                obj29[0] = tmp84(tmp12(4205).ThemeContextProvider, obj11);
                return tmp84(tmp12(7096).LayerScope, obj29);
                ...
                obj31[3] = source;
                obj31[4] = tmp28;
                const items20 = [tmp84(tmp5(5542), obj31), ];
                tmp = !obj32;
                if (!obj32) {
                  ...
                  tmp = tmp84(tmp12(10580).Caption, obj32);
                }
                items20[1] = tmp;
                obj30[3] = items20;
                tmp = tmp89(tmp12(5084).PressableOpacity, obj30);
              } else {
      ...
      items5 = [];
      const tmp12Result9 = tmp12(11899);
    }
    bound1 = youSettingsCoachmark;
    const tmp5Result1 = tmp5(8145);
  }
  if (!isFocused) {
    ref3.current = false;
  }
...
let closure_22 = importDefaultResult.createAnimatedComponent(_modDef8176);
