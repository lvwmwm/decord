...
// Dependencies: [5, 32, 19, 17, 1376, 1372, 3892, 5720, 8999, 676, 21, 4116, 4251, 4255, 712, 500, 4265, 5723, 709, 589, 698, 15673, 15674, 4256, 9300, 1236, 10566, 15675, 5581, 5601, 5281, 4682, 9309, 1222, 3955, 10376, 10375, 4194, 4193, 10071, 4666, 11589, 2]
// Exports: default

// Module 15672 (isPrivateChannelMatch)
...
function Header(title) {
  ...
  obj = { numInGroup, NUM_IN_GROUP_THRESHOLD: 5, withTiming: numInGroup(4256).withTiming };
  fn.__closure = obj;
  fn.__workletHash = 12426216833792;
  fn.__initData = closure_24;
  obj = { style: callback5().header, children: null };
  const animatedStyle = obj.useAnimatedStyle(fn);
  const items1 = [callback3(numInGroup(9300).GenericHeaderTitle, { title: title.title }), ];
...
function ChatPreview(channelId) {
  ...
  navigateToChannel(15673)(callback);
  ...
  const items2 = [callback3(navigateToChannel(10566), obj), ];
  const obj1 = { portal: null };
  const ref = React.useRef(null);
  obj1[0] = channelId(500).isAndroid();
  items2[1] = callback3(channelId(15675).PortalKeyboardRenderer, obj1);
  obj[1] = items2;
...
let closure_19 = require("module_4116").createAnimatedComponent(require("Text").Text);
...
export default function NewMessageScreen(navigation) {
  ...
  let tmp = callback5();
  dependencyMap = tmp;
  const insets = FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID(5281)({ includeKeyboardHeight: true }).insets;
  stateFromStores = insets;
  FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID(4682)(() => {
  ...
  let tmp4 = FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID(5581);
    ...
    if (null == FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID) {
      if (null == outer1_22(navigation)) {
        ...
        return () => {
          ...
          obj = { id: callback(5723).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID, guild_id: "Array", parent_id: "isArray" };
          obj[1] = obj;
          obj.dispatch(obj);
        };
      }
          ...
          if (0 === c3) {
            if (arg0 === 1) {
              ...
              if (callback instanceof c7) {
                ...
                if (-1 === index) {
                  if (arr.length >= FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID(15674)() - 1) {
                    let obj5 = callback(3955);
                ...
                let obj3 = FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID(4265);
                c3 = 1;
                c4 = 1;
  ...
  if (!tmp8) {
    if (!tmp10) {
      if (first.length > 0) {
        if (null == FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID) {
          FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID = tmp13(5723).FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID;
        }
  ...
  obj = { value: tmp4(FAKE_PLACEHOLDER_PRIVATE_CHANNEL_ID(5601).NEW_MESSAGE_COMPOSER).analyticsLocations, children: null };
  ...
  obj[1] = callback3(tmp2(11589), obj1);
  return callback3(_require(5581).AnalyticsLocationProvider, obj);
};
