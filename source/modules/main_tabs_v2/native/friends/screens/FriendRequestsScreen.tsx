...
// Dependencies: [32, 19, 17, 7393, 4130, 1921, 10853, 673, 9093, 21, 4478, 709, 16593, 2008, 4859, 644, 7394, 4166, 16233, 7395, 11, 5961, 5981, 695, 12718, 5967, 16594, 656, 8911, 1233, 1498, 6000, 5607, 4474, 9273, 5078, 9784, 5076, 10952, 6265, 10859, 2]
// Exports: default

// Module 16592 (compareUserItems)
import DISCORD_EPOCHDefault from "DISCORD_EPOCH" /* 11 */;
import ThemesDefault from "Themes" /* 709 */;
import closure_3 from "_slicedToArray" /* 32 */;
import closure_4 from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import closure_6 from "_validate" /* 7393 */;
import closure_7 from "markAllUserIdListsStale" /* 4130 */;
import closure_8 from "mergeGuildAvatar" /* 1921 */;
import { UserRowModes } from "UserRowModes" /* 10853 */;
import ME from "ME" /* 673 */;
import { MINIMUM_PENDING_INCOMING_COUNT_FOR_CLEAR_ALL as closure_13 } from "RELATIONSHIP_COUNT_ANIMATION_THRESHOLD" /* 9093 */;
...
export default function FriendRequestsScreen() {
  ...
  const analyticsLocations = first(5961)(first(5981).FRIEND_REQUESTS).analyticsLocations;
  let stateFromStoresArray;
  first = undefined;
  let obj = stateFromStoresArray(644);
  ...
  const NotificationCenterAckedBeforeId = stateFromStoresArray(4166).NotificationCenterAckedBeforeId;
  ...
  const effect = gameRelationshipsByType1.useEffect(() => {
    if (stateFromStoresArray.length > 0) {
      ...
      const result = stateFromStoresArray(16233).markNotificationCenterLocalItemsAcked(found);
      const obj = stateFromStoresArray(16233);
      const result1 = stateFromStoresArray(16233).bulkMarkNotificationCenterItemsAcked(arr.filter((addResult) => !closure_1_0(closure_1_2[19]).isRemoteAcked(addResult, closure_1)));
      const obj2 = stateFromStoresArray(16233);
    }
  }, items1);
  ...
  const memo1 = gameRelationshipsByType1.useMemo(() => stateFromStoresArray(16594).getPendingRelationshipIds(first, dependencyMap), items4);
  ...
  let obj4 = stateFromStoresArray(644);
  ...
  callback = gameRelationshipsByType1.useCallback((id) => {
    first(8911)({ userId: id.id, localUser: id, sourceAnalyticsLocations: stateFromStoresArray });
  }, items8);
  const callback1 = gameRelationshipsByType1.useCallback((arg0) => {
    if (1 === arg0) {
      let obj = { type: "section", props: null };
      obj = { title: null };
      const intl = stateFromStoresArray(1233).intl;
      obj[0] = intl.string(stateFromStoresArray(1233).t["NHpP/k"]);
      obj[1] = obj;
      return obj;
    }
  }, []);
  let obj5 = stateFromStoresArray(1498);
  ...
  obj[1] = intl.string(stateFromStoresArray(1233).t.bekioP);
  const items10 = [obj, ];
  obj1 = { id: str2.toString(), label: null, page: null };
  let intl2 = tmp4(1233).intl;
  obj1[1] = intl2.string(stateFromStoresArray(1233).t.tWqcIF);
  items10[1] = obj1;
  obj[3] = items10;
  const segmentedControlState = stateFromStoresArray(9273).useSegmentedControlState(obj);
  obj2 = { value: analyticsLocations, children: null };
  const items11 = [outgoingData(first(5078), { absolute: true }), ];
  obj3 = { style: tmp.container, children: null };
  obj4 = { style: tmp.tabs, children: outgoingData(tmp4(9784).SegmentedControl, { state: segmentedControlState }) };
  ...
  if (tmp28) {
    ...
    obj6[1] = function onPress() {
      ...
      stateFromStoresArray(4859).openAlert("clear-all-incoming-requests", outgoingData(lazyResult, { incomingRequestCount: incoming.length }));
    };
    const obj7 = { variant: "text-sm/semibold", color: "text-brand", children: null };
    const intl3 = tmp4(1233).intl;
    obj7[2] = intl3.string(tmp4(1233).t.O8k7O4);
    obj6[2] = tmp32(tmp4(4474).Text, obj7);
    obj5[1] = tmp32(tmp4(5076).PressableOpacity, obj6);
  ...
  if (tmp26) {
    ...
    if (first1 === tmp25.Incoming) {
      ...
      let stringResult = intl5.string(tmp4(1233).t["7uvAKe"]);
    } else {
      const intl4 = tmp4(1233).intl;
      stringResult = intl4.string(tmp4(1233).t["yvzX/Z"]);
    }
    const obj9 = { title: null, illustration: null, disableBackgroundOverlay: true };
    obj9[0] = stringResult;
    WumpusCouchSpotIllustration = tmp4(6265).WumpusCouchSpotIllustration;
    obj9[1] = WumpusCouchSpotIllustration;
    obj8[1] = tmp32(tmp2(10952), obj9);
    tmp32Result = tmp32(tmp33, obj8);
    const tmp2Result = tmp2(10952);
  } else {
    if (first1 === tmp25.Incoming) {
    ...
    obj10[1] = callback2;
    obj10[2] = callback1;
    items12[2] = tmp32(tmp4(10859).UsersFastList, obj10);
    obj3[1] = items12;
    items11[1] = tmp31(tmp33, obj3);
    obj2[1] = items11;
    return tmp31(tmp4(5961).AnalyticsLocationProvider, obj2);
  }
};
