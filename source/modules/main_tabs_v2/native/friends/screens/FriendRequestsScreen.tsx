...
// Dependencies: [32, 19, 17, 7572, 4130, 1921, 10791, 673, 10805, 21, 4481, 709, 16700, 2008, 4867, 644, 7573, 4166, 16338, 7574, 11, 7102, 7122, 695, 12995, 7108, 16701, 656, 8110, 1233, 1498, 5642, 5560, 4477, 9721, 5086, 9722, 5084, 10925, 5647, 10797, 2]
// Exports: default

// Module 16699 (compareUserItems)
import DISCORD_EPOCHDefault from "DISCORD_EPOCH" /* 11 */;
import ThemesDefault from "Themes" /* 709 */;
import closure_3 from "_slicedToArray" /* 32 */;
import closure_4 from "noop" /* 19 */;
import { View } from "get ActivityIndicator" /* 17 */;
import closure_6 from "_validate" /* 7572 */;
import closure_7 from "markAllUserIdListsStale" /* 4130 */;
import closure_8 from "mergeGuildAvatar" /* 1921 */;
import { UserRowModes } from "UserRowModes" /* 10791 */;
import ME from "ME" /* 673 */;
import { MINIMUM_PENDING_INCOMING_COUNT_FOR_CLEAR_ALL as closure_13 } from "RELATIONSHIP_COUNT_ANIMATION_THRESHOLD" /* 10805 */;
...
export default function FriendRequestsScreen() {
  ...
  const analyticsLocations = first(7102)(first(7122).FRIEND_REQUESTS).analyticsLocations;
  ...
  const effect = gameRelationshipsByType1.useEffect(() => {
    if (stateFromStoresArray.length > 0) {
      ...
      const result = stateFromStoresArray(16338).markNotificationCenterLocalItemsAcked(found);
      const obj = stateFromStoresArray(16338);
      const result1 = stateFromStoresArray(16338).bulkMarkNotificationCenterItemsAcked(arr.filter((addResult) => !closure_1_0(closure_1_2[19]).isRemoteAcked(addResult, closure_1)));
      const obj2 = stateFromStoresArray(16338);
    }
  }, items1);
  ...
  const memo1 = gameRelationshipsByType1.useMemo(() => stateFromStoresArray(16701).getPendingRelationshipIds(first, dependencyMap), items4);
  ...
  callback = gameRelationshipsByType1.useCallback((id) => {
    first(8110)({ userId: id.id, localUser: id, sourceAnalyticsLocations: stateFromStoresArray });
  }, items8);
  ...
  items10[1] = obj1;
  obj[3] = items10;
  const segmentedControlState = stateFromStoresArray(9721).useSegmentedControlState(obj);
  obj2 = { value: analyticsLocations, children: null };
  const items11 = [outgoingData(first(5086), { absolute: true }), ];
  obj3 = { style: tmp.container, children: null };
  obj4 = { style: tmp.tabs, children: outgoingData(tmp4(9722).SegmentedControl, { state: segmentedControlState }) };
  ...
  if (tmp28) {
    ...
    obj6[1] = function onPress() {
      ...
      stateFromStoresArray(4867).openAlert("clear-all-incoming-requests", outgoingData(lazyResult, { incomingRequestCount: incoming.length }));
    };
    ...
    obj6[2] = tmp32(tmp4(4477).Text, obj7);
    obj5[1] = tmp32(tmp4(5084).PressableOpacity, obj6);
  ...
  if (tmp26) {
    ...
    obj9[0] = stringResult;
    WumpusCouchSpotIllustration = tmp4(5647).WumpusCouchSpotIllustration;
    obj9[1] = WumpusCouchSpotIllustration;
    obj8[1] = tmp32(tmp2(10925), obj9);
    ...
    obj10[1] = callback2;
    obj10[2] = callback1;
    items12[2] = tmp32(tmp4(10797).UsersFastList, obj10);
    obj3[1] = items12;
    items11[1] = tmp31(tmp33, obj3);
    obj2[1] = items11;
    return tmp31(tmp4(7102).AnalyticsLocationProvider, obj2);
  }
};
