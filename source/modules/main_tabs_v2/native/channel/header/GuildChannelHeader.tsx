...
// Dependencies: [32, 19, 17, 12717, 4495, 1395, 7249, 1391, 4295, 1910, 4030, 1922, 676, 1388, 21, 4097, 712, 589, 12711, 7187, 9346, 1236, 4984, 12709, 7366, 1377, 1993, 6832, 9314, 1367, 10167, 12705, 2]

// Module 12716 (GuildChannelMemberCount)
import set from "set";
...
function GuildChannelMemberCount(channel) {
  ...
  const token = channel(4097).useToken(importDefault(712).modules.mobile.CHANNEL_HEADER_ICON_SIZE);
  ...
  if (null == total) {
    const tmpResult = tmp(12711);
    return tmpResult.renderMemberCountText(online, total, flag, tmp(7187).ICON_SIZE[token]);
  }
}
function computeVisibleChannelName(channel) {
  ...
  if (obj.shouldNSFWGateGuild(guildId)) {
    ...
    stringResult = tmp(4984).computeChannelName(channel, mergeGuildAvatar, markAllUserIdListsStale);
...
function ChannelLinkedLobbyCoachmark(guild) {
  ...
  const tmp = callback(guild(7366).useSelectedDismissibleContent(items, undefined, true), 2);
  dependencyMap = tmp2;
  ...
  let obj = guild(7366);
  const coachmark = guild(9314).useCoachmark(guild.iconRef, memo);
  return null;
}
