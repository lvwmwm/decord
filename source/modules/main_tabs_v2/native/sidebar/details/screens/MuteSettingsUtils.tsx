...
// Dependencies: [3991, 1391, 1910, 3998, 4539, 1922, 676, 685, 1236, 4534, 7264, 5301, 5296, 9867, 2]
...
// Module 9866 (getMuteSettingLabel)
...
export const handleUnmutePress = function handleUnmutePress(channelId, guildId) {
  ...
  if (null != channel) {
    if (channel.isThread()) {
      let tmp7Result = tmp7(7264);
      const result = tmp7Result.setNotificationSettings(channel, { muted: false });
    } else {
      tmp7Result = tmp7(5301);
      const result1 = tmp7Result.updateChannelOverrideSettings(guildId, channel.id, { muted: false, mute_config: null }, require(5296) /* UserNotificationSettings */.NotificationLabels.Unmuted);
    }
  }
};
export const handleMuteSettingPress = function handleMuteSettingPress(arg0) {
  ...
  const muteSettings = importAll(9867).getMuteSettings(muteDurationSeconds);
  const channel = store.getChannel(channelId);
  ...
  if (null != onOptionPress) {
    ...
    if (channel.isThread()) {
      let tmp4Result = tmp4(7264);
      const result = tmp4Result.setNotificationSettings(channel, muteSettings);
    } else {
      tmp4Result = tmp4(5301);
      const result1 = tmp4Result.updateChannelOverrideSettings(guildId, channel.id, muteSettings, require(5296) /* UserNotificationSettings */.NotificationLabels.Muted);
    }
  } else if (null != guild) {
    const result2 = importDefault(5301).updateGuildNotificationSettings(guild.id, muteSettings, require(5296) /* UserNotificationSettings */.NotificationLabels.Muted);
    const obj5 = importDefault(5301);
  }
};
