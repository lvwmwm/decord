...
// Dependencies: [1340, 4356, 4034, 589, 8447, 4039, 14619, 2]
// Exports: default, useMessagePreviewSetting

// Module 14618 (useMessagePreview)
...
export default function useMessagePreview(guild_id) {
  ...
  if (!disabled) {
    disabled = importDefault(8447)(guild_id);
  }
  if (!disabled) {
    disabled = stateFromStores === tmp(4039).MessagePreviewTypes.NONE;
  }
  if (!disabled) {
    let tmp6 = stateFromStores === tmp(4039).MessagePreviewTypes.UNREADS;
    if (tmp6) {
      if (unread == null) {
  ...
  return importDefault(14619)(guild_id, disabled);
};
