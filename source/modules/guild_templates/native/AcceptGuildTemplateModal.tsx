...
// Dependencies: [5, 32, 19, 7212, 673, 21, 4478, 5500, 586, 5464, 11588, 5561, 11589, 5090, 11601, 11586, 5503, 6016, 2]
// Exports: default

// Module 11587 (ConnectedAcceptGuildTemplate)
import closure_4 from "asyncGeneratorStep" /* 5 */;
import closure_5 from "_slicedToArray" /* 32 */;
import closure_6 from "noop" /* 19 */;
import closure_7 from "handleGuildTemplateResolveSuccess" /* 7212 */;
...
function ConnectedAcceptGuildTemplate(code) {
  ...
  const tmp = callback2();
  const tmp3 = callback(React.useState(first(5464).getGuildNameSuggestion()), 2);
  first = tmp3[0];
  ...
  React = tmp7[1];
  stateFromStores(11588)(stateFromStores);
  ...
  obj[7] = function createServer() {
    if (null != stateFromStores) {
      const obj = stateFromStores(11601);
      stateFromStores(11601).acceptGuildTemplate(tmp.code, first, first1).then(() => callback(table[15]).hideModal(), (arg0) => callback2(arg0));
      const acceptGuildTemplateResult = stateFromStores(11601).acceptGuildTemplate(tmp.code, first, first1);
    }
  };
  obj[2] = jsx(stateFromStores(11589), {
    code,
    guildTemplate: stateFromStores,
    name: first,
    setName(arg0) {
  ...
  return jsx(code(5561).SafeAreaPaddingView, {
    code,
    guildTemplate: stateFromStores,
    name: first,
    setName(arg0) {
...
export default function AcceptGuildTemplateModal(code) {
  ...
  return jsx(code(6016).Navigator, { initialRouteName: ACCEPT_GUILD_TEMPLATE, screens: memo });
};
