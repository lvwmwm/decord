...
// Dependencies: [5, 32, 19, 7391, 673, 21, 4481, 5637, 586, 5474, 11726, 7063, 11727, 5099, 11737, 11724, 5579, 6940, 2]
// Exports: default

// Module 11725 (ConnectedAcceptGuildTemplate)
import closure_4 from "asyncGeneratorStep" /* 5 */;
import closure_5 from "_slicedToArray" /* 32 */;
import closure_6 from "noop" /* 19 */;
import closure_7 from "handleGuildTemplateResolveSuccess" /* 7391 */;
...
function ConnectedAcceptGuildTemplate(code) {
  ...
  const tmp = callback2();
  const tmp3 = callback(React.useState(first(5474).getGuildNameSuggestion()), 2);
  first = tmp3[0];
  ...
  React = tmp7[1];
  stateFromStores(11726)(stateFromStores);
  ...
  obj[7] = function createServer() {
    if (null != stateFromStores) {
      const obj = stateFromStores(11737);
      stateFromStores(11737).acceptGuildTemplate(tmp.code, first, first1).then(() => callback(table[15]).hideModal(), (arg0) => callback2(arg0));
      const acceptGuildTemplateResult = stateFromStores(11737).acceptGuildTemplate(tmp.code, first, first1);
    }
  };
  obj[2] = jsx(stateFromStores(11727), {
    code,
    guildTemplate: stateFromStores,
    name: first,
    setName(arg0) {
  ...
  return jsx(code(7063).SafeAreaPaddingView, {
    code,
    guildTemplate: stateFromStores,
    name: first,
    setName(arg0) {
...
export default function AcceptGuildTemplateModal(code) {
  ...
  return jsx(code(6940).Navigator, { initialRouteName: ACCEPT_GUILD_TEMPLATE, screens: memo });
};
