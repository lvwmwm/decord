...
// Dependencies: [32, 3912, 1903, 676, 505, 686, 5810, 8007, 589, 2]
// Exports: canReplyToMessage, useCanReplyToMessage

// Module 8006 (useCanReplyToMessage)
...
export const useCanReplyToMessage = function useCanReplyToMessage(channel, message) {
  ...
  const canUnarchiveThread = _require(5810).useCanUnarchiveThread(channel);
  const obj = _require(5810);
  let tmp = _require;
  let guildId;
  if (channel != null) {
    guildId = channel.getGuildId();
  }
  const obj2 = _require(8007);
...
export const canReplyToMessage = function canReplyToMessage(channel, message) {
  ...
  const canUnarchiveThreadResult = require(5810) /* useCanUnarchiveThread */.canUnarchiveThread(channel);
  let id;
  if (currentUser != null) {
    id = currentUser.id;
  }
  const guildId = channel.getGuildId();
  const tmpResult = require(8007) /* useUserCommunicationDisabled */;
  const isArchivedThreadResult = channel.isArchivedThread();
  let tmp11 = !isArchivedThreadResult;
  if (isArchivedThreadResult) {
