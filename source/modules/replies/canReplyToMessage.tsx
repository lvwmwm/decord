...
// Dependencies: [32, 3929, 1903, 676, 505, 686, 5829, 8024, 589, 2]
// Exports: canReplyToMessage, useCanReplyToMessage

// Module 8023 (useCanReplyToMessage)
...
export const useCanReplyToMessage = function useCanReplyToMessage(channel, message) {
  ...
  const canUnarchiveThread = _require(5829).useCanUnarchiveThread(channel);
  const obj = _require(5829);
  let tmp = _require;
  let guildId;
  if (channel != null) {
    guildId = channel.getGuildId();
  }
  const obj2 = _require(8024);
...
export const canReplyToMessage = function canReplyToMessage(channel, message) {
  ...
  const canUnarchiveThreadResult = require(5829) /* useCanUnarchiveThread */.canUnarchiveThread(channel);
  let id;
  if (currentUser != null) {
    id = currentUser.id;
  }
  const guildId = channel.getGuildId();
  const tmpResult = require(8024) /* useUserCommunicationDisabled */;
  const isArchivedThreadResult = channel.isArchivedThread();
  let tmp11 = !isArchivedThreadResult;
  if (isArchivedThreadResult) {
