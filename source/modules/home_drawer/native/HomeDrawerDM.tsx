...
function HomeDrawerDMExpandedChildren(channel) {
  ...
  stateFromStores = channel(589).useStateFromStores(items1, () => {
    ...
    if (null != channel) {
      if (stateFromStores1(tmp.type)) {
        let channelName = channel(4380).computeChannelName(tmp, outer1_8, memo);
        const obj = channel(4380);
      } else {
        channelName = null;
      }
      tmp2 = channelName;
  ...
  const title = stateFromStores.useMemo(() => {
    ...
    if (isMuted) {
      ...
      const items = [outer1_10(channel(4185).Text, obj), outer1_10(outer1_9, { size: "xs" })];
      obj[1] = items;
