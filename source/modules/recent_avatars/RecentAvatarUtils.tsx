...
export const generateAvatarDescription = function generateAvatarDescription(maxSettingsForPreset) {
  ...
  if (undefined === assetOrigin) {
    assetOrigin = require(7891) /* AssetOriginTypes */.AssetOriginTypes.NEW_ASSET;
  }
  if (assetOrigin !== require(7891) /* AssetOriginTypes */.AssetOriginTypes.ARCHIVED_ASSET) {
    if (null == filename) {
    ...
    if (assetOrigin === require(7891) /* AssetOriginTypes */.AssetOriginTypes.EDITED_ARCHIVED_ASSET) {
...
export const createPendingImage = function createPendingImage(assetOrigin) {
  ...
  if (NEW_ASSET === undefined) {
    NEW_ASSET = require(7891) /* AssetOriginTypes */.AssetOriginTypes.NEW_ASSET;
  }
  ({ imageUri, description, originalAsset, originalMd5 } = assetOrigin);
  if (require(7891) /* AssetOriginTypes */.AssetOriginTypes.NEW_ASSET === NEW_ASSET) {
    let obj = { assetOrigin: NEW_ASSET, imageUri, description, originalMd5 };
    return obj;
  } else if (require(7891) /* AssetOriginTypes */.AssetOriginTypes.EDITED_ARCHIVED_ASSET === NEW_ASSET) {
    obj = { assetOrigin: NEW_ASSET, imageUri, description, originalAsset, originalMd5 };
    return obj;
  } else if (require(7891) /* AssetOriginTypes */.AssetOriginTypes.ARCHIVED_ASSET === NEW_ASSET) {
...
export const getPendingAvatarSrc = function getPendingAvatarSrc(canAnimate) {
  ...
  if (null != image) {
    ...
    if ("string" !== typeof image) {
      if (image.assetOrigin === require(7891) /* AssetOriginTypes */.AssetOriginTypes.ARCHIVED_ASSET) {
        const obj = { userId };
