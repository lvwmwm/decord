...
// Dependencies: [19, 17, 1905, 8312, 21, 4302, 712, 5668, 10208, 7902, 8670, 4298, 1236, 9447, 4712, 3947, 2]
// Exports: default

// Module 12502 (ForLaterNitroUpsellBar)
...
export default function ForLaterNitroUpsellBar(isReminder) {
  ...
  if (isAtLimit) {
    ...
    obj[1] = tmp.icon;
    let tmp7Result = tmp7(isReminder(7902).WarningIcon, obj);
    let tmp10 = tmp7;
  } else {
  ...
  const items2 = [tmp10(isReminder(4298).Text, obj2), " \u00B7 ", ];
  ...
  if (isAtLimit) {
    ...
    obj1[3] = items2;
    items1[1] = tmp5(isReminder(4298).Text, obj1);
    if (isAtLimit) {
      let Button = tmp2(9447);
    } else {
      Button = tmp12(4712).Button;
    }
