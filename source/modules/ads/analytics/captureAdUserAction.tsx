...
// Dependencies: [5, 5983, 676, 6011, 6000, 5981, 6010, 6017, 698, 514, 500, 5960, 8444, 6012, 5989, 8447, 2]
// Exports: captureAdUserAction

// Module 8443 (emitClickEventWithCreative)
      ...
      if (c8 === 2) {
        ...
        try {
          ...
          if (0 === c7) {
            ...
            constants = arg1;
            if (c1 !== lib(6011).AdCreativeType.QUEST) {
              obj3 = lib(6000);
              ...
              const obj13 = lib(6000);
              const adMetadataSealed = lib(5981).getAdMetadataSealed(c3, adCreativeId);
...
function handleViewImpression(minViewTimeSeconds) {
  ...
  const merged1 = Object.assign(require(8444) /* getBrandSafetyContext */.getBrandSafetyContext(surfaceId));
  obj.impression_id = impressionId;
  obj.is_quest_enrollment_blocked = isQuestEnrollmentBlocked;
  const obj4 = require(8444) /* getBrandSafetyContext */;
  const merged2 = Object.assign(require(6010) /* getQuestContentName */.getContentProperties(surfaceId, questContentPosition, questContentRowIndex));
  if (minViewTimeSeconds.adCreativeType !== require(6011) /* AdCreativeType */.AdCreativeType.QUEST) {
    ...
    let adMetadataSealed = tmp9(5981).getAdMetadataSealed(sourceQuestContent, adCreativeId);
    if (adMetadataSealed == null) {
      adMetadataSealed = null;
    }
    obj2.metadata_sealed = adMetadataSealed;
    const tmp9Result1 = tmp9(5981);
    const currentQuestHomeSearchSession = tmp9(6017).getCurrentQuestHomeSearchSession();
    let uuid;
    if (currentQuestHomeSearchSession != null) {
      uuid = currentQuestHomeSearchSession.uuid;
    }
    ...
    const tmp9Result2 = tmp9(6017);
    let adTrafficMetadataSealed = tmp9(5981).getAdTrafficMetadataSealed(sourceQuestContent, adCreativeId);
    if (adTrafficMetadataSealed == null) {
      ...
      if (constants === 2) {
        ...
        try {
          ...
          if (0 === c4) {
            ...
            const obj2 = {};
            const obj9 = lib2(698);
            const merged = Object.assign(lib(6010).getContentProperties(lib.surfaceId));
            let advertisingId = null;
            if (null != lib2) {
              obj = lib(500);
              advertisingId = null;
...
function reportCaptureAdUserActionError(arg0) {
  const questLogger = require(5989) /* getQuestLogger */.getQuestLogger();
...
export const captureAdUserAction = function captureAdUserAction(type) {
  try {
    type = type.type;
    if (require(8447) /* AdUserActionType */.AdUserActionType.END_CONTENT_LOAD === type) {
    ...
    } else if (tmp2(8447).AdUserActionType.CLICK_INTERNAL === type) {
    ...
    } else if (tmp2(8447).AdUserActionType.CLICK_EXTERNAL_ADVERTISER_CTA === type) {
    ...
    } else if (tmp2(8447).AdUserActionType.VIEW_INTERNAL_SURFACE_IMPRESSION === type) {
    ...
    } else if (tmp2(8447).AdUserActionType.VIEW_EXTERNAL_PAID_AD_PLACEMENT_IMPRESSION === type) {
