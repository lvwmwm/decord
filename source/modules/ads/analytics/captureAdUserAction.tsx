...
// Dependencies: [5, 7161, 676, 7189, 7178, 7159, 7188, 7195, 698, 514, 500, 7138, 9524, 7190, 7167, 9527, 2]
// Exports: captureAdUserAction

// Module 9523 (emitClickEventWithCreative)
      ...
      if (c8 === 2) {
        ...
        try {
          ...
          if (0 === c7) {
            ...
            constants = arg1;
            if (c1 !== lib(7189).AdCreativeType.QUEST) {
              obj3 = lib(7178);
              ...
              const obj13 = lib(7178);
              const adMetadataSealed = lib(7159).getAdMetadataSealed(c3, adCreativeId);
...
function handleViewImpression(minViewTimeSeconds) {
  ...
  const merged1 = Object.assign(require(9524) /* getBrandSafetyContext */.getBrandSafetyContext(surfaceId));
  obj.impression_id = impressionId;
  obj.is_quest_enrollment_blocked = isQuestEnrollmentBlocked;
  const obj4 = require(9524) /* getBrandSafetyContext */;
  const merged2 = Object.assign(require(7188) /* getQuestContentName */.getContentProperties(surfaceId, questContentPosition, questContentRowIndex));
  if (minViewTimeSeconds.adCreativeType !== require(7189) /* AdCreativeType */.AdCreativeType.QUEST) {
    let tmp9Result = tmp9(7178);
    ...
    let adMetadataSealed = tmp9(7159).getAdMetadataSealed(sourceQuestContent, adCreativeId);
    if (adMetadataSealed == null) {
      adMetadataSealed = null;
    }
    obj2.metadata_sealed = adMetadataSealed;
    const tmp9Result1 = tmp9(7159);
    const currentQuestHomeSearchSession = tmp9(7195).getCurrentQuestHomeSearchSession();
    let uuid;
    if (currentQuestHomeSearchSession != null) {
      uuid = currentQuestHomeSearchSession.uuid;
    }
    ...
    const tmp9Result2 = tmp9(7195);
    let adTrafficMetadataSealed = tmp9(7159).getAdTrafficMetadataSealed(sourceQuestContent, adCreativeId);
    if (adTrafficMetadataSealed == null) {
      ...
      if (constants === 2) {
        ...
        try {
          ...
          if (0 === c4) {
            ...
            const obj2 = {};
            const obj9 = lib2(698);
            const merged = Object.assign(lib(7188).getContentProperties(lib.surfaceId));
            let advertisingId = null;
            if (null != lib2) {
              obj = lib(500);
              advertisingId = null;
...
function reportCaptureAdUserActionError(arg0) {
  const questLogger = require(7167) /* getQuestLogger */.getQuestLogger();
...
export const captureAdUserAction = function captureAdUserAction(type) {
  try {
    type = type.type;
    if (require(9527) /* AdUserActionType */.AdUserActionType.END_CONTENT_LOAD === type) {
    ...
    } else if (tmp2(9527).AdUserActionType.CLICK_INTERNAL === type) {
    ...
    } else if (tmp2(9527).AdUserActionType.CLICK_EXTERNAL_ADVERTISER_CTA === type) {
    ...
    } else if (tmp2(9527).AdUserActionType.VIEW_INTERNAL_SURFACE_IMPRESSION === type) {
    ...
    } else if (tmp2(9527).AdUserActionType.VIEW_EXTERNAL_PAID_AD_PLACEMENT_IMPRESSION === type) {
