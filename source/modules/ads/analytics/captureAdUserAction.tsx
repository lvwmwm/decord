...
// Dependencies: [5, 7636, 673, 5411, 7651, 7632, 7661, 7667, 695, 511, 1234, 7610, 11126, 7662, 7642, 11129, 2]
// Exports: captureAdUserAction

// Module 11125 (emitClickEventWithCreative)
...
import getQuestContentName from "getQuestContentName" /* 7661 */;
import getBrandSafetyContext from "getBrandSafetyContext" /* 11126 */;
import AdUserActionType from "AdUserActionType" /* 11129 */;
      ...
      if (c8 === 2) {
        ...
        try {
          ...
          if (0 === c7) {
            ...
            constants = arg1;
            if (c1 !== lib(5411).AdCreativeType.QUEST) {
              obj3 = lib(7651);
              ...
              const obj13 = lib(7651);
              const adMetadataSealed = lib(7632).getAdMetadataSealed(c3, adCreativeId);
...
function handleViewImpression(minViewTimeSeconds) {
  ...
  if (minViewTimeSeconds.adCreativeType !== AdCreativeType.AdCreativeType.QUEST) {
    ...
    let adMetadataSealed = tmp9(7632).getAdMetadataSealed(sourceQuestContent, adCreativeId);
    if (adMetadataSealed == null) {
      adMetadataSealed = null;
    }
    obj2.metadata_sealed = adMetadataSealed;
    const tmp9Result1 = tmp9(7632);
    const currentQuestHomeSearchSession = tmp9(7667).getCurrentQuestHomeSearchSession();
    let uuid;
    if (currentQuestHomeSearchSession != null) {
      uuid = currentQuestHomeSearchSession.uuid;
    }
    ...
    const tmp9Result2 = tmp9(7667);
    let adTrafficMetadataSealed = tmp9(7632).getAdTrafficMetadataSealed(sourceQuestContent, adCreativeId);
    if (adTrafficMetadataSealed == null) {
      ...
      if (constants === 2) {
        ...
        try {
          ...
          if (0 === c4) {
            ...
            const obj2 = {};
            const obj9 = lib2(695);
            const merged = Object.assign(lib(7661).getContentProperties(lib.surfaceId));
            let advertisingId = null;
            if (null != lib2) {
              obj = lib(1234);
              advertisingId = null;
...
export const captureAdUserAction = function captureAdUserAction(type) {
  try {
    ...
    } else if (tmp2(11129).AdUserActionType.CLICK_INTERNAL === type) {
    ...
    } else if (tmp2(11129).AdUserActionType.CLICK_EXTERNAL_ADVERTISER_CTA === type) {
    ...
    } else if (tmp2(11129).AdUserActionType.VIEW_INTERNAL_SURFACE_IMPRESSION === type) {
    ...
    } else if (tmp2(11129).AdUserActionType.VIEW_EXTERNAL_PAID_AD_PLACEMENT_IMPRESSION === type) {
