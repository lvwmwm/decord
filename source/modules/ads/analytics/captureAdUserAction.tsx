...
// Dependencies: [5, 7081, 676, 7109, 7098, 7079, 7108, 7115, 698, 514, 500, 7058, 9436, 7110, 7087, 9439, 2]
// Exports: captureAdUserAction

// Module 9435 (emitClickEventWithCreative)
      ...
      if (c8 === 2) {
        ...
        try {
          ...
          if (0 === c7) {
            ...
            constants = arg1;
            if (c1 !== lib(7109).AdCreativeType.QUEST) {
              obj3 = lib(7098);
              ...
              const obj13 = lib(7098);
              const adMetadataSealed = lib(7079).getAdMetadataSealed(c3, adCreativeId);
...
function handleViewImpression(minViewTimeSeconds) {
  ...
  const merged2 = Object.assign(require(7108) /* getQuestContentName */.getContentProperties(surfaceId, questContentPosition, questContentRowIndex));
  if (minViewTimeSeconds.adCreativeType !== require(7109) /* AdCreativeType */.AdCreativeType.QUEST) {
    let tmp9Result = tmp9(7098);
    ...
    let adMetadataSealed = tmp9(7079).getAdMetadataSealed(sourceQuestContent, adCreativeId);
    if (adMetadataSealed == null) {
      adMetadataSealed = null;
    }
    obj2.metadata_sealed = adMetadataSealed;
    const tmp9Result1 = tmp9(7079);
    const currentQuestHomeSearchSession = tmp9(7115).getCurrentQuestHomeSearchSession();
    let uuid;
    if (currentQuestHomeSearchSession != null) {
      uuid = currentQuestHomeSearchSession.uuid;
    }
    ...
    const tmp9Result2 = tmp9(7115);
    let adTrafficMetadataSealed = tmp9(7079).getAdTrafficMetadataSealed(sourceQuestContent, adCreativeId);
    if (adTrafficMetadataSealed == null) {
      ...
      if (constants === 2) {
        ...
        try {
          ...
          if (0 === c4) {
            ...
            const obj2 = {};
            const obj9 = lib2(698);
            const merged = Object.assign(lib(7108).getContentProperties(lib.surfaceId));
            let advertisingId = null;
            if (null != lib2) {
              obj = lib(500);
              advertisingId = null;
...
function reportCaptureAdUserActionError(arg0) {
  const questLogger = require(7087) /* getQuestLogger */.getQuestLogger();
...
export const captureAdUserAction = function captureAdUserAction(type) {
  try {
    ...
    } else if (tmp2(9439).AdUserActionType.CLICK_INTERNAL === type) {
    ...
    } else if (tmp2(9439).AdUserActionType.CLICK_EXTERNAL_ADVERTISER_CTA === type) {
    ...
    } else if (tmp2(9439).AdUserActionType.VIEW_INTERNAL_SURFACE_IMPRESSION === type) {
    ...
    } else if (tmp2(9439).AdUserActionType.VIEW_EXTERNAL_PAID_AD_PLACEMENT_IMPRESSION === type) {
