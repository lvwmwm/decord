...
// Dependencies: [5, 7054, 676, 7082, 7071, 7052, 7081, 7088, 698, 514, 500, 7031, 9407, 7083, 7060, 9410, 2]
// Exports: captureAdUserAction

// Module 9406 (emitClickEventWithCreative)
      ...
      if (c8 === 2) {
        ...
        try {
          ...
          if (0 === c7) {
            ...
            constants = arg1;
            if (c1 !== lib(7082).AdCreativeType.QUEST) {
              obj3 = lib(7071);
              ...
              const obj13 = lib(7071);
              const adMetadataSealed = lib(7052).getAdMetadataSealed(c3, adCreativeId);
...
function handleViewImpression(minViewTimeSeconds) {
  ...
  const merged1 = Object.assign(require(9407) /* getBrandSafetyContext */.getBrandSafetyContext(surfaceId));
  obj.impression_id = impressionId;
  obj.is_quest_enrollment_blocked = isQuestEnrollmentBlocked;
  const obj4 = require(9407) /* getBrandSafetyContext */;
  const merged2 = Object.assign(require(7081) /* getQuestContentName */.getContentProperties(surfaceId, questContentPosition, questContentRowIndex));
  if (minViewTimeSeconds.adCreativeType !== require(7082) /* AdCreativeType */.AdCreativeType.QUEST) {
    ...
    let adMetadataSealed = tmp9(7052).getAdMetadataSealed(sourceQuestContent, adCreativeId);
    if (adMetadataSealed == null) {
      adMetadataSealed = null;
    }
    obj2.metadata_sealed = adMetadataSealed;
    const tmp9Result1 = tmp9(7052);
    const currentQuestHomeSearchSession = tmp9(7088).getCurrentQuestHomeSearchSession();
    let uuid;
    if (currentQuestHomeSearchSession != null) {
      uuid = currentQuestHomeSearchSession.uuid;
    }
    ...
    const tmp9Result2 = tmp9(7088);
    let adTrafficMetadataSealed = tmp9(7052).getAdTrafficMetadataSealed(sourceQuestContent, adCreativeId);
    if (adTrafficMetadataSealed == null) {
      ...
      if (constants === 2) {
        ...
        try {
          ...
          if (0 === c4) {
            ...
            const obj2 = {};
            const obj9 = lib2(698);
            const merged = Object.assign(lib(7081).getContentProperties(lib.surfaceId));
            let advertisingId = null;
            if (null != lib2) {
              obj = lib(500);
              advertisingId = null;
...
function reportCaptureAdUserActionError(arg0) {
  const questLogger = require(7060) /* getQuestLogger */.getQuestLogger();
...
export const captureAdUserAction = function captureAdUserAction(type) {
  try {
    type = type.type;
    if (require(9410) /* AdUserActionType */.AdUserActionType.END_CONTENT_LOAD === type) {
    ...
    } else if (tmp2(9410).AdUserActionType.CLICK_INTERNAL === type) {
    ...
    } else if (tmp2(9410).AdUserActionType.CLICK_EXTERNAL_ADVERTISER_CTA === type) {
    ...
    } else if (tmp2(9410).AdUserActionType.VIEW_INTERNAL_SURFACE_IMPRESSION === type) {
    ...
    } else if (tmp2(9410).AdUserActionType.VIEW_EXTERNAL_PAID_AD_PLACEMENT_IMPRESSION === type) {
