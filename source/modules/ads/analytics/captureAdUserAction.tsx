...
// Dependencies: [5, 7458, 673, 7456, 7473, 7453, 7483, 7489, 695, 511, 1234, 7431, 9014, 7484, 7464, 9017, 2]
// Exports: captureAdUserAction

// Module 9013 (emitClickEventWithCreative)
...
import getBrandSafetyContext from "getBrandSafetyContext" /* 9014 */;
import AdUserActionType from "AdUserActionType" /* 9017 */;
      ...
      if (c8 === 2) {
        ...
        try {
          ...
          if (0 === c7) {
            ...
            constants = arg1;
            if (c1 !== lib(7456).AdCreativeType.QUEST) {
              obj3 = lib(7473);
              ...
              const obj13 = lib(7473);
              const adMetadataSealed = lib(7453).getAdMetadataSealed(c3, adCreativeId);
...
function handleViewImpression(minViewTimeSeconds) {
  ...
  if (minViewTimeSeconds.adCreativeType !== AdCreativeType.AdCreativeType.QUEST) {
    ...
    let adMetadataSealed = tmp9(7453).getAdMetadataSealed(sourceQuestContent, adCreativeId);
    if (adMetadataSealed == null) {
      adMetadataSealed = null;
    }
    obj2.metadata_sealed = adMetadataSealed;
    const tmp9Result1 = tmp9(7453);
    const currentQuestHomeSearchSession = tmp9(7489).getCurrentQuestHomeSearchSession();
    let uuid;
    if (currentQuestHomeSearchSession != null) {
      uuid = currentQuestHomeSearchSession.uuid;
    }
    ...
    const tmp9Result2 = tmp9(7489);
    let adTrafficMetadataSealed = tmp9(7453).getAdTrafficMetadataSealed(sourceQuestContent, adCreativeId);
    if (adTrafficMetadataSealed == null) {
      ...
      if (constants === 2) {
        ...
        try {
          ...
          if (0 === c4) {
            ...
            const obj2 = {};
            const obj9 = lib2(695);
            const merged = Object.assign(lib(7483).getContentProperties(lib.surfaceId));
            let advertisingId = null;
            if (null != lib2) {
              obj = lib(1234);
              advertisingId = null;
...
export const captureAdUserAction = function captureAdUserAction(type) {
  try {
    ...
    } else if (tmp2(9017).AdUserActionType.CLICK_INTERNAL === type) {
    ...
    } else if (tmp2(9017).AdUserActionType.CLICK_EXTERNAL_ADVERTISER_CTA === type) {
    ...
    } else if (tmp2(9017).AdUserActionType.VIEW_INTERNAL_SURFACE_IMPRESSION === type) {
    ...
    } else if (tmp2(9017).AdUserActionType.VIEW_EXTERNAL_PAID_AD_PLACEMENT_IMPRESSION === type) {
