...
// Dependencies: [5, 32, 19, 17, 6917, 1903, 6918, 21, 4285, 712, 16850, 16849, 11242, 4711, 16439, 14041, 7258, 589, 14039, 16860, 16844, 3985, 1236, 2546, 4253, 14040, 1988, 16853, 11252, 4693, 16861, 10183, 16862, 4281, 2284, 14042, 2]
// Exports: default

// Module 16859 (ParentalConsentConnectionScreen)
...
export default function ParentalConsentConnectionScreen() {
  ...
  const derivedPendingRequests = onTaskComplete(16860).useDerivedPendingRequests(arr, stateFromStores1);
  const tmp2Result6 = onTaskComplete(16860);
  ...
  let tmp5Result = tmp5(16853);
  let intl = tmp2(1236).intl;
  obj[0] = intl.string(getLinkCode(2546).dMMSA0);
  let intl2 = tmp2(1236).intl;
  const format = intl2.format;
  tmp5Result = tmp5(2546);
  if (tmp16) {
  ...
  if (tmp16) {
    ...
    obj3[3] = tmp27;
    obj3[4] = callback1;
    tmp30Result = tmp30(tmp2(10183).ModalActionButton, obj3);
  }
  const tmp14Result = tmp14(obj9.useState(false), 2);
  items8[1] = tmp30Result;
  obj2[1] = items8;
  obj[4] = closure_10(onTaskComplete(11252).ModalFooter, { children: closure_11(onTaskComplete(4693).Stack, obj2) });
  ...
  if (tmp16) {
    ...
    obj6[4] = callback;
    obj6[5] = callback2;
    tmp30Result = tmp30(tmp5(16862), obj6);
  } else {
    ...
    obj8[3] = intl4.string(tmp5(2284).pojgfk);
    const items9 = [tmp30(tmp2(4281).Text, obj8), ];
    obj9 = { shareActions: "full", linkCode: null, expiresAt: null, onRefresh: null };
    obj9[1] = tmp21;
    obj9[2] = parsed;
    obj9[3] = getLinkCode;
    items9[1] = tmp30(tmp2(14042).ConnectGuardianCard, obj9);
    obj7[1] = items9;
    tmp30Result = tmp36(View, obj7);
  }
  obj5[2] = tmp30Result;
  obj[5] = closure_10(onTaskComplete(4693).Stack, obj5);
