...
// Dependencies: [5, 32, 19, 17, 7044, 1922, 7045, 21, 4342, 712, 17028, 17027, 11375, 4771, 16623, 14192, 7381, 589, 14190, 17038, 17022, 4062, 1236, 2565, 4310, 14191, 2007, 17031, 11385, 4753, 17039, 11150, 17040, 4338, 2303, 14193, 2]
// Exports: default

// Module 17037 (ParentalConsentConnectionScreen)
...
export default function ParentalConsentConnectionScreen() {
  ...
  const derivedPendingRequests = onTaskComplete(17038).useDerivedPendingRequests(arr, stateFromStores1);
  const tmp2Result6 = onTaskComplete(17038);
  ...
  let tmp5Result = tmp5(17031);
  let intl = tmp2(1236).intl;
  obj[0] = intl.string(getLinkCode(2565).dMMSA0);
  let intl2 = tmp2(1236).intl;
  const format = intl2.format;
  tmp5Result = tmp5(2565);
  if (tmp16) {
  ...
  if (tmp16) {
    ...
    obj3[3] = tmp27;
    obj3[4] = callback1;
    tmp30Result = tmp30(tmp2(11150).ModalActionButton, obj3);
  }
  const tmp14Result = tmp14(obj9.useState(false), 2);
  items8[1] = tmp30Result;
  obj2[1] = items8;
  obj[4] = closure_10(onTaskComplete(11385).ModalFooter, { children: closure_11(onTaskComplete(4753).Stack, obj2) });
  ...
  if (tmp16) {
    ...
    obj6[4] = callback;
    obj6[5] = callback2;
    tmp30Result = tmp30(tmp5(17040), obj6);
  } else {
    ...
    obj8[3] = intl4.string(tmp5(2303).pojgfk);
    const items9 = [tmp30(tmp2(4338).Text, obj8), ];
    obj9 = { shareActions: "full", linkCode: null, expiresAt: null, onRefresh: null };
    obj9[1] = tmp21;
    obj9[2] = parsed;
    obj9[3] = getLinkCode;
    items9[1] = tmp30(tmp2(14193).ConnectGuardianCard, obj9);
    obj7[1] = items9;
    tmp30Result = tmp36(View, obj7);
  }
  obj5[2] = tmp30Result;
  obj[5] = closure_10(onTaskComplete(4753).Stack, obj5);
