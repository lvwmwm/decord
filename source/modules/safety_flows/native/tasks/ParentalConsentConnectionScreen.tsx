...
// Dependencies: [5, 32, 19, 17, 6905, 1874, 6906, 21, 4285, 712, 16830, 16829, 11236, 4711, 16419, 14031, 7259, 589, 14029, 16840, 16824, 3986, 1236, 2547, 4253, 14030, 1959, 16833, 11246, 4693, 16841, 10181, 16842, 4281, 2285, 14032, 2]
// Exports: default

// Module 16839 (ParentalConsentConnectionScreen)
...
export default function ParentalConsentConnectionScreen() {
  ...
  const derivedPendingRequests = onTaskComplete(16840).useDerivedPendingRequests(arr, stateFromStores1);
  const tmp2Result6 = onTaskComplete(16840);
  ...
  let tmp5Result = tmp5(16833);
  let intl = tmp2(1236).intl;
  obj[0] = intl.string(getLinkCode(2547).dMMSA0);
  let intl2 = tmp2(1236).intl;
  const format = intl2.format;
  tmp5Result = tmp5(2547);
  if (tmp16) {
  ...
  if (tmp16) {
    ...
    obj3[3] = tmp27;
    obj3[4] = callback1;
    tmp30Result = tmp30(tmp2(10181).ModalActionButton, obj3);
  }
  const tmp14Result = tmp14(obj9.useState(false), 2);
  items8[1] = tmp30Result;
  obj2[1] = items8;
  obj[4] = closure_10(onTaskComplete(11246).ModalFooter, { children: closure_11(onTaskComplete(4693).Stack, obj2) });
  ...
  if (tmp16) {
    ...
    obj6[4] = callback;
    obj6[5] = callback2;
    tmp30Result = tmp30(tmp5(16842), obj6);
  } else {
