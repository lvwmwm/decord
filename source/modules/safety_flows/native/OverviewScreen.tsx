...
// Function ID: 17025
// Name: OverviewScreen
// Dependencies: [19, 21, 4344, 17025, 1499, 17026, 7848, 7849, 4752, 712, 4340, 1236, 2565, 5806, 7995, 17020, 11393, 17027, 10312, 2]
// Exports: default

// Module 17024 (OverviewScreen)
import set from "set";
...
export default function OverviewScreen() {
  ...
  const items1 = [callback(task(4340).Text, obj), , ];
  ...
  items1[1] = callback(task(4340).Text, obj1);
  ...
  if (flow_context != null) {
    ...
    if (tasks != null) {
      mapped = tasks.map((task_type) => {
        ...
        obj[0] = task(17020).TASK_TYPE_TO_TITLE[task_type.task_type];
        obj[1] = arg1 + 1;
        return callback(navigation(7995), obj, task_type.task_type);
      });
    }
  }
  const obj3 = { children: null };
  const obj4 = { children: null };
  const items2 = [callback(task(5806).TableRowGroup, { hasIcons: true, children: mapped }), ];
  const obj5 = { variant: "text-xs/medium", color: "text-muted", children: null };
  const intl3 = tmp2(1236).intl;
  obj5[2] = intl3.string(navigation(2565)["0TnUrG"]);
  items2[1] = callback(task(4340).Text, obj5);
  obj2[1] = items2;
  items1[2] = closure_5(task(4752).Stack, obj2);
  obj[2] = items1;
  obj4[0] = closure_5(task(4752).Stack, obj);
  const items3 = [callback(task(7849).ModalContent, obj4), ];
  const obj6 = { children: null };
  const items4 = [callback(navigation(17027), {}), ];
  const obj7 = { variant: "primary", text: null, onPress: null };
  const intl4 = tmp2(1236).intl;
  obj7[1] = intl4.string(navigation(2565).Ks6opt);
  obj7[2] = callback;
  items4[1] = callback(task(10312).ModalActionButton, obj7);
  obj6[0] = items4;
  items3[1] = closure_5(task(11393).ModalFooter, obj6);
  obj3[0] = items3;
  return closure_5(task(7848).ModalScreen, obj3);
};
