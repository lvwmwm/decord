...
// Function ID: 17416
// Name: OverviewScreen
// Dependencies: [19, 21, 4448, 17416, 1499, 17417, 8660, 8661, 4894, 712, 4444, 1236, 2633, 5960, 8690, 17411, 11466, 17418, 10695, 2]
// Exports: default

// Module 17415 (OverviewScreen)
...
export default function OverviewScreen() {
  ...
  obj[2] = intl.string(navigation(2633).RRamMH);
  const items1 = [callback(task(4444).Text, obj), , ];
  obj1 = { variant: "text-md/medium", color: "text-strong", children: null };
  const intl2 = task(1236).intl;
  obj1[2] = intl2.string(navigation(2633).I2Ctk1);
  items1[1] = callback(task(4444).Text, obj1);
  ...
  if (flow_context != null) {
    ...
    if (tasks != null) {
      mapped = tasks.map((task_type) => {
        const obj = { tip: task(17411).TASK_TYPE_TO_TITLE[task_type.task_type], index: arg1 + 1 };
        return callback(navigation(8690), obj, task_type.task_type);
      });
    }
  }
  const obj3 = { children: null };
  const obj4 = { children: null };
  const items2 = [callback(task(5960).TableRowGroup, { hasIcons: true, children: mapped }), ];
  const obj5 = { variant: "text-xs/medium", color: "text-muted", children: null };
  const intl3 = tmp2(1236).intl;
  obj5[2] = intl3.string(navigation(2633)["0TnUrG"]);
  items2[1] = callback(task(4444).Text, obj5);
  obj2[1] = items2;
  items1[2] = closure_5(task(4894).Stack, obj2);
  obj[2] = items1;
  obj4[0] = closure_5(task(4894).Stack, obj);
  const items3 = [callback(task(8661).ModalContent, obj4), ];
  const obj6 = { children: null };
  const items4 = [callback(navigation(17418), {}), ];
  const obj7 = { variant: "primary", text: null, onPress: null };
  const intl4 = tmp2(1236).intl;
  obj7[1] = intl4.string(navigation(2633).Ks6opt);
  obj7[2] = callback;
  items4[1] = callback(task(10695).ModalActionButton, obj7);
  obj6[0] = items4;
  items3[1] = closure_5(task(11466).ModalFooter, obj6);
  obj3[0] = items3;
  return closure_5(task(8660).ModalScreen, obj3);
};
