...
// Function ID: 16869
// Name: OverviewScreen
// Dependencies: [19, 21, 4302, 16869, 1480, 16870, 7747, 7748, 4710, 712, 4298, 1236, 2546, 5707, 7894, 16864, 11267, 16871, 10199, 2]
// Exports: default

// Module 16868 (OverviewScreen)
import set from "set";
...
export default function OverviewScreen() {
  ...
  const items1 = [callback(task(4298).Text, obj), , ];
  ...
  items1[1] = callback(task(4298).Text, obj1);
  ...
  if (flow_context != null) {
    ...
    if (tasks != null) {
      mapped = tasks.map((task_type) => {
        ...
        obj[0] = task(16864).TASK_TYPE_TO_TITLE[task_type.task_type];
        obj[1] = arg1 + 1;
        return callback(navigation(7894), obj, task_type.task_type);
      });
    }
  }
  const obj3 = { children: null };
  const obj4 = { children: null };
  const items2 = [callback(task(5707).TableRowGroup, { hasIcons: true, children: mapped }), ];
  const obj5 = { variant: "text-xs/medium", color: "text-muted", children: null };
  const intl3 = tmp2(1236).intl;
  obj5[2] = intl3.string(navigation(2546)["0TnUrG"]);
  items2[1] = callback(task(4298).Text, obj5);
  obj2[1] = items2;
  items1[2] = closure_5(task(4710).Stack, obj2);
  obj[2] = items1;
  obj4[0] = closure_5(task(4710).Stack, obj);
  const items3 = [callback(task(7748).ModalContent, obj4), ];
  const obj6 = { children: null };
  const items4 = [callback(navigation(16871), {}), ];
  const obj7 = { variant: "primary", text: null, onPress: null };
  const intl4 = tmp2(1236).intl;
  obj7[1] = intl4.string(navigation(2546).Ks6opt);
  obj7[2] = callback;
  items4[1] = callback(task(10199).ModalActionButton, obj7);
  obj6[0] = items4;
  items3[1] = closure_5(task(11267).ModalFooter, obj6);
  obj3[0] = items3;
  return closure_5(task(7747).ModalScreen, obj3);
};
