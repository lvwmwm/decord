...
// Dependencies: [32, 19, 676, 21, 4255, 1236, 6871, 4223, 5631, 5322, 5718, 5634, 8231, 7758, 4665, 4449, 2]
// Exports: default

// Module 11115 (GuildRaidLockdownFeedbackActionSheet)
...
export default function GuildRaidLockdownFeedbackActionSheet(guildId) {
  ...
  obj5[1] = callback(guildId(5322).BottomSheetTitleHeader, obj6);
  ...
  obj8[1] = items.map((value) => {
    ...
    obj[2] = value.text;
    return outer1_6(guildId(8231).TableCheckboxRow, obj, value);
  });
  const items1 = [callback(guildId(5634).TableRowGroup, obj8), , ];
  let hasItem = first.includes(guildId(6871).RaidLockdownFeedbackType.OTHER);
  if (hasItem) {
    ...
    hasItem = tmp7(tmp5(7758).TextArea, obj9);
  }
  items1[1] = hasItem;
  const obj10 = {
    onPress() {
      let obj = guildId(4449);
      ...
      first(4223).hideActionSheet("GuildRaidLockdownFeedbackActionSheet");
    },
    text: null
  };
  const intl9 = tmp5(1236).intl;
  obj10[1] = intl9.string(guildId(1236).t.nAt0rE);
  items1[2] = callback(guildId(4665).Button, obj10);
  obj7[1] = items1;
  obj5[2] = tmp8(first(5718), obj7);
  return callback(guildId(5631).ActionSheet, obj5);
};
