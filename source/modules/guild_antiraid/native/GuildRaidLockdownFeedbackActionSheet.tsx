...
// Dependencies: [32, 19, 676, 21, 4302, 1236, 6917, 4270, 5704, 5395, 5764, 5707, 8388, 7916, 4712, 4496, 2]
// Exports: default

// Module 11164 (GuildRaidLockdownFeedbackActionSheet)
...
export default function GuildRaidLockdownFeedbackActionSheet(guildId) {
  ...
  obj5[1] = callback(guildId(5395).BottomSheetTitleHeader, obj6);
  ...
  obj8[1] = items.map((value) => {
    ...
    obj[2] = value.text;
    return outer1_6(guildId(8388).TableCheckboxRow, obj, value);
  });
  const items1 = [callback(guildId(5707).TableRowGroup, obj8), , ];
  let hasItem = first.includes(guildId(6917).RaidLockdownFeedbackType.OTHER);
  if (hasItem) {
    ...
    hasItem = tmp7(tmp5(7916).TextArea, obj9);
  }
  items1[1] = hasItem;
  const obj10 = {
    onPress() {
      let obj = guildId(4496);
      ...
      first(4270).hideActionSheet("GuildRaidLockdownFeedbackActionSheet");
    },
    text: null
  };
  const intl9 = tmp5(1236).intl;
  obj10[1] = intl9.string(guildId(1236).t.nAt0rE);
  items1[2] = callback(guildId(4712).Button, obj10);
  obj7[1] = items1;
  obj5[2] = tmp8(first(5764), obj7);
  return callback(guildId(5704).ActionSheet, obj5);
};
