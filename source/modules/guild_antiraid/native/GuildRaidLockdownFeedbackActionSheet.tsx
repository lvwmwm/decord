...
// Dependencies: [32, 19, 673, 21, 4478, 1233, 7272, 4445, 5997, 5629, 6057, 6000, 7692, 7817, 4928, 4700, 2]
// Exports: default

// Module 11628 (GuildRaidLockdownFeedbackActionSheet)
...
export default function GuildRaidLockdownFeedbackActionSheet(guildId) {
  ...
  obj5[1] = callback(guildId(5629).BottomSheetTitleHeader, obj6);
  const obj7 = { style: callback2().container, children: null };
  const tmp = callback2();
  const tmp8 = closure_7;
  const tmp9 = first(6057);
  const items1 = [
    callback(guildId(6000).TableRowGroup, {
      hasIcons: false,
      children: items.map((label) => {
        const value = label.value;
        guildId = value;
        return closure_1_6(guildId(7692).TableCheckboxRow, {
          onPress() {
  ...
  let hasItem = first.includes(guildId(7272).RaidLockdownFeedbackType.OTHER);
  if (hasItem) {
    ...
    hasItem = tmp7(tmp5(7817).TextArea, obj9);
  }
  items1[1] = hasItem;
  const obj10 = {
    onPress() {
      let obj = guildId(4700);
  ...
  items1[2] = callback(guildId(4928).Button, obj10);
  obj7[1] = items1;
  obj5[2] = tmp8(tmp9, obj7);
  return callback(guildId(5997).ActionSheet, obj5);
};
