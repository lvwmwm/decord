...
// Dependencies: [32, 19, 676, 21, 4444, 1236, 7190, 4411, 5936, 5568, 5996, 5939, 8673, 8194, 4879, 4652, 2]
// Exports: default

// Module 11464 (GuildRaidLockdownFeedbackActionSheet)
...
export default function GuildRaidLockdownFeedbackActionSheet(guildId) {
  ...
  obj5[1] = callback(guildId(5568).BottomSheetTitleHeader, obj6);
  const obj7 = { style: callback2().container, children: null };
  const tmp = callback2();
  const tmp8 = closure_7;
  const tmp9 = first(5996);
  const items1 = [
    callback(guildId(5939).TableRowGroup, {
      hasIcons: false,
      children: items.map((label) => {
        const value = label.value;
        guildId = value;
        return closure_1_6(guildId(8673).TableCheckboxRow, {
          onPress() {
  ...
  let hasItem = first.includes(guildId(7190).RaidLockdownFeedbackType.OTHER);
  if (hasItem) {
    ...
    hasItem = tmp7(tmp5(8194).TextArea, obj9);
  }
  items1[1] = hasItem;
  const obj10 = {
    onPress() {
      let obj = guildId(4652);
      ...
      first(4411).hideActionSheet("GuildRaidLockdownFeedbackActionSheet");
    },
    text: null
  };
  const intl9 = tmp5(1236).intl;
  obj10[1] = intl9.string(guildId(1236).t.nAt0rE);
  items1[2] = callback(guildId(4879).Button, obj10);
  obj7[1] = items1;
  obj5[2] = tmp8(tmp9, obj7);
  return callback(guildId(5936).ActionSheet, obj5);
};
