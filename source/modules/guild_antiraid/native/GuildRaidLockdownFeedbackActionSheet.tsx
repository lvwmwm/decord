...
// Dependencies: [32, 19, 676, 21, 4285, 1236, 6898, 4253, 5685, 5376, 5745, 5688, 8371, 7899, 4695, 4479, 2]
// Exports: default

// Module 11149 (GuildRaidLockdownFeedbackActionSheet)
...
export default function GuildRaidLockdownFeedbackActionSheet(guildId) {
  ...
  obj5[1] = callback(guildId(5376).BottomSheetTitleHeader, obj6);
  ...
  obj8[1] = items.map((value) => {
    ...
    obj[2] = value.text;
    return outer1_6(guildId(8371).TableCheckboxRow, obj, value);
  });
  const items1 = [callback(guildId(5688).TableRowGroup, obj8), , ];
  let hasItem = first.includes(guildId(6898).RaidLockdownFeedbackType.OTHER);
  if (hasItem) {
    ...
    hasItem = tmp7(tmp5(7899).TextArea, obj9);
  }
  items1[1] = hasItem;
  const obj10 = {
    onPress() {
      let obj = guildId(4479);
      ...
      first(4253).hideActionSheet("GuildRaidLockdownFeedbackActionSheet");
    },
    text: null
  };
  const intl9 = tmp5(1236).intl;
  obj10[1] = intl9.string(guildId(1236).t.nAt0rE);
  items1[2] = callback(guildId(4695).Button, obj10);
  obj7[1] = items1;
  obj5[2] = tmp8(first(5745), obj7);
  return callback(guildId(5685).ActionSheet, obj5);
};
