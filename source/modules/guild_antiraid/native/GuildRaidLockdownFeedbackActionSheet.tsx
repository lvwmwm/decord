...
// Dependencies: [32, 19, 673, 21, 4481, 1233, 7451, 4448, 7138, 7089, 5533, 5642, 5559, 7025, 4936, 4708, 2]
// Exports: default

// Module 11764 (GuildRaidLockdownFeedbackActionSheet)
...
export default function GuildRaidLockdownFeedbackActionSheet(guildId) {
  ...
  obj5[1] = callback(guildId(7089).BottomSheetTitleHeader, obj6);
  const obj7 = { style: callback2().container, children: null };
  const tmp = callback2();
  const tmp8 = closure_7;
  const tmp9 = first(5533);
  const items1 = [
    callback(guildId(5642).TableRowGroup, {
      hasIcons: false,
      children: items.map((label) => {
        const value = label.value;
        guildId = value;
        return closure_1_6(guildId(5559).TableCheckboxRow, {
          onPress() {
  ...
  let hasItem = first.includes(guildId(7451).RaidLockdownFeedbackType.OTHER);
  if (hasItem) {
    ...
    hasItem = tmp7(tmp5(7025).TextArea, obj9);
  }
  items1[1] = hasItem;
  const obj10 = {
    onPress() {
      let obj = guildId(4708);
      ...
      first(4448).hideActionSheet("GuildRaidLockdownFeedbackActionSheet");
    },
    text: null
  };
  const intl9 = tmp5(1233).intl;
  obj10[1] = intl9.string(guildId(1233).t.nAt0rE);
  items1[2] = callback(guildId(4936).Button, obj10);
  obj7[1] = items1;
  obj5[2] = tmp8(tmp9, obj7);
  return callback(guildId(7138).ActionSheet, obj5);
};
