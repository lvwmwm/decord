...
// Dependencies: [32, 4379, 1391, 4623, 4034, 1922, 6017, 6019, 1370, 6021, 1955, 4632, 1403, 1306, 6025, 2]
...
// Module 6016 (getEligibleHarmTypesConfigsForContext)
import create from "create" /* 1306 */;
import ContentHarmType from "ContentHarmType" /* 6019 */;
import ContentHarmTypeChannel from "ContentHarmTypeChannel" /* 6021 */;
import isForwardMessage from "isForwardMessage" /* 6025 */;
...
import closure_9 from "getFpMessageInfo" /* 6017 */;
...
function getEnabledHarmTypesForChannelAndAuthorId(channelId, id) {
  ...
  if (null != currentUser) {
    if (id !== currentUser.id) {
      ...
      if (null == tmp10) {
        let NONE = _require(6019).ContentHarmTypeBitMask.NONE;
      } else {
        _require = tmp10;
        const _Object = Object;
        const values = Object.values(_require(6019).CONTENT_SCAN_TYPE_REGISTRY);
        ...
        if (null == tmp10) {
          NONE = tmp12(6019).ContentHarmTypeBitMask.NONE;
        } else {
  ...
  return _require(6019).ContentHarmTypeBitMask.NONE;
}
function messageHasObscurableMediaForBitmask(firstMessage, EXPLICIT) {
  const _require = EXPLICIT;
  if (EXPLICIT !== _require(6019).ContentHarmTypeBitMask.NONE) {
...
function findComponentMedia(components) {
  ...
  return obj.flatMap((type) => {
    ...
    if ("proxy_url" in media) {
      toUnfurledMediaItemResult = callback(4632).toUnfurledMediaItem(media);
...
function isMediaScanPending(type, enabledContentHarmTypeFlags) {
  if (enabledContentHarmTypeFlags === media(6019).ContentHarmTypeBitMask.NONE) {
    return false;
    ...
    if (0 === arr.length) {
      ...
      type = type.type;
      if (tmp(6021).ObscuredMediaTypes.Embed === type) {
            ...
            if (0 !== arr.filter((arg0) => !closure_1_16(arg0, { type: media(closure_1_2[9]).ObscuredMediaTypes.Embed, media })).length) {
              ...
              if (!("images" in media3)) {
                ...
                if (-1 === content_scan_version) {
                  ...
                  if (!arr.includes(tmp(6019).ContentHarmType.GORE)) {
                    if (!arr.includes(tmp(6019).ContentHarmType.SELF_HARM)) {
        ...
        return flag3;
      } else if (tmp(6021).ObscuredMediaTypes.Attachment === type) {
        ...
        return tmp11;
      } else if (tmp(6021).ObscuredMediaTypes.GenericMedia === type) {
        media = type.media;
        let flag2 = false;
        if (0 !== arr.length) {
          flag2 = false;
...
function isMediaFlaggedForHarmType(EXPLICIT, type) {
  if (null == EXPLICIT) {
    ...
    } else if (tmp3(6021).ObscuredMediaTypes.Attachment === type) {
    ...
    } else if (tmp3(6021).ObscuredMediaTypes.GenericMedia === type) {
...
function contentHarmTypesToFlags(mapped) {
  ...
  while (iter !== undefined) {
    ...
    if (ContentHarmType.ContentHarmType.EXPLICIT === nextResult) {
      ...
      let tmp15 = tmp2;
      let tmp16 = tmp4;
      NONE = NONE | tmp3(6019).ContentHarmTypeBitMask.EXPLICIT;
    } else {
      let tmp17 = tmp2;
      let tmp18 = tmp4;
      if (tmp3(6019).ContentHarmType.GORE === nextResult) {
        let tmp11 = NONE;
        let tmp12 = tmp2;
        let tmp13 = tmp4;
        NONE = NONE | tmp3(6019).ContentHarmTypeBitMask.GORE;
      } else {
        let tmp6 = tmp2;
        let tmp7 = tmp4;
        if (tmp3(6019).ContentHarmType.SELF_HARM === nextResult) {
          let tmp8 = NONE;
          let tmp9 = tmp2;
          let tmp10 = tmp4;
          NONE = NONE | tmp3(6019).ContentHarmTypeBitMask.SELF_HARM;
        }
      }
    }
    continue;
...
function hasUnscannedMedia(message, arg1) {
  ...
  const _require = tmp;
  if (tmp === _require(6019).ContentHarmTypeBitMask.NONE) {
    return false;
  } else {
...
export const getUnscannedMediaIds = function getUnscannedMediaIds(message) {
  ...
  if (null != channelId) {
    ...
    if (NONE === NONE(6019).ContentHarmTypeBitMask.NONE) {
  ...
  NONE = NONE(6019).ContentHarmTypeBitMask.NONE;
};
export const getMediaObscuredReasonFromBitmask = function getMediaObscuredReasonFromBitmask(arg0, enabledContentHarmTypeFlags) {
  const _require = arg0;
  if (enabledContentHarmTypeFlags === _require(6019).ContentHarmTypeBitMask.NONE) {
    return [];
  } else {
    const arr = getHarmTypeFromBitmask(enabledContentHarmTypeFlags);
    if (0 === arr.length) {
...
export const isMediaObscuredForHarmTypes = function isMediaObscuredForHarmTypes(arg0, enabledHarmTypesForMessage) {
  const _require = arg0;
  if (enabledHarmTypesForMessage === _require(6019).ContentHarmTypeBitMask.NONE) {
    return false;
  } else {
