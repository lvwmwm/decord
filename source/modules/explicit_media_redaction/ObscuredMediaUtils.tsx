...
// Dependencies: [32, 4188, 1372, 4407, 3826, 1874, 5863, 5878, 1351, 5880, 1906, 4416, 1384, 1306, 5884, 2]
...
// Module 5877 (getEligibleHarmTypesConfigsForContext)
...
function getEligibleHarmTypesConfigsForContext() {
  const values = Object.values(require(5878) /* ContentHarmType */.CONTENT_SCAN_TYPE_REGISTRY);
...
function getEnabledHarmTypesForMessage(message) {
  ...
  NONE = require(5878) /* ContentHarmType */.ContentHarmTypeBitMask.NONE;
}
function getEnabledHarmTypesForChannelAndAuthorId(channelId, id) {
  ...
  if (null != currentUser) {
    if (id !== currentUser.id) {
      ...
      if (null == tmp10) {
        let NONE = require(5878) /* ContentHarmType */.ContentHarmTypeBitMask.NONE;
      } else {
        require = tmp10;
        ...
        if (null == tmp10) {
          NONE = tmp12(5878).ContentHarmTypeBitMask.NONE;
        } else {
  ...
  return require(5878) /* ContentHarmType */.ContentHarmTypeBitMask.NONE;
}
function messageHasObscurableMediaForBitmask(firstMessage, EXPLICIT) {
  const _require = EXPLICIT;
  if (EXPLICIT !== _require(5878).ContentHarmTypeBitMask.NONE) {
...
function isMediaScanPending(type, enabledContentHarmTypeFlags) {
  if (enabledContentHarmTypeFlags === media(5878).ContentHarmTypeBitMask.NONE) {
    return false;
    ...
    if (0 === arr.length) {
      ...
      type = type.type;
      if (tmp(5880).ObscuredMediaTypes.Embed === type) {
            ...
            if (0 !== arr.filter((arg0) => !outer1_16(arg0, { type: media(outer1_2[9]).ObscuredMediaTypes.Embed, media })).length) {
              ...
              if (!("images" in media3)) {
                ...
                if (-1 === content_scan_version) {
                  ...
                  if (!arr.includes(tmp(5878).ContentHarmType.GORE)) {
                    if (!arr.includes(tmp(5878).ContentHarmType.SELF_HARM)) {
        ...
        return flag3;
      } else if (tmp(5880).ObscuredMediaTypes.Attachment === type) {
        ...
        return tmp11;
      } else if (tmp(5880).ObscuredMediaTypes.GenericMedia === type) {
        media = type.media;
        let flag2 = false;
        if (0 !== arr.length) {
          flag2 = false;
...
function isMediaFlaggedForHarmType(EXPLICIT, type) {
  if (null == EXPLICIT) {
    ...
    const tmp5 = require(5878) /* ContentHarmType */.CONTENT_SCAN_TYPE_REGISTRY[EXPLICIT];
    ...
    type = type.type;
    if (require(5880) /* ContentHarmTypeChannel */.ObscuredMediaTypes.Embed === type) {
    ...
    } else if (tmp3(5880).ObscuredMediaTypes.Attachment === type) {
    ...
    } else if (tmp3(5880).ObscuredMediaTypes.GenericMedia === type) {
...
function contentHarmTypesToFlags(mapped) {
  ...
  while (iter !== undefined) {
    ...
    if (require(5878) /* ContentHarmType */.ContentHarmType.EXPLICIT === nextResult) {
      let tmp14 = NONE;
      let tmp15 = tmp2;
      let tmp16 = tmp4;
      NONE = NONE | tmp3(5878).ContentHarmTypeBitMask.EXPLICIT;
    } else {
      let tmp17 = tmp2;
      let tmp18 = tmp4;
      if (tmp3(5878).ContentHarmType.GORE === nextResult) {
        let tmp11 = NONE;
        let tmp12 = tmp2;
        let tmp13 = tmp4;
        NONE = NONE | tmp3(5878).ContentHarmTypeBitMask.GORE;
      } else {
        let tmp6 = tmp2;
        let tmp7 = tmp4;
        if (tmp3(5878).ContentHarmType.SELF_HARM === nextResult) {
          let tmp8 = NONE;
          let tmp9 = tmp2;
          let tmp10 = tmp4;
          NONE = NONE | tmp3(5878).ContentHarmTypeBitMask.SELF_HARM;
        }
      }
    }
    continue;
...
function getHarmTypeFromBitmask(enabledHarmTypesForMessage) {
  if (enabledHarmTypesForMessage === require(5878) /* ContentHarmType */.ContentHarmTypeBitMask.NONE) {
    return [];
  } else {
    const items = [];
      ...
      if (null != channel) {
        if (!channel.isDM()) {
          if (!channel.isGroupDM()) {
            const GUILD = require(5880) /* ContentHarmTypeChannel */.ContentHarmTypeChannel.GUILD;
          }
        }
        if (null == closure_1) {
          const NON_FRIEND_DM = require(5880) /* ContentHarmTypeChannel */.ContentHarmTypeChannel.NON_FRIEND_DM;
        } else {
          const friendIDs = obj2.getFriendIDs();
        }
        const FRIEND_DM = require(5880) /* ContentHarmTypeChannel */.ContentHarmTypeChannel.FRIEND_DM;
      }
    }
  }
  return tmp6;
...
function getChannelIdAndAuthorIdFromMessage(message) {
  if (null == message) {
    ...
    if ("messageReference" in message) {
      const items = [message.messageReference, importDefault(5884)(message)];
      let items2 = items;
    } else if ("message_reference" in message) {
      const items1 = [message.message_reference, ];
      let obj = require(5884) /* isForwardMessage */;
...
function hasUnscannedMedia(message, arg1) {
  ...
  const require = tmp;
  if (tmp === require(5878) /* ContentHarmType */.ContentHarmTypeBitMask.NONE) {
    return false;
  } else {
...
export const getEnabledHarmTypesBitmaskForChannelType = function getEnabledHarmTypesBitmaskForChannelType(GUILD) {
  ...
  const values = Object.values(_require(5878).CONTENT_SCAN_TYPE_REGISTRY);
  const found = values.filter((isEligible) => {
    let tmp = null == isEligible.isEligible;
    if (!tmp) {
      isEligible = isEligible.isEligible;
...
export const getUnscannedMediaIds = function getUnscannedMediaIds(message) {
  ...
  if (null != channelId) {
    ...
    if (NONE === NONE(5878).ContentHarmTypeBitMask.NONE) {
  ...
  NONE = NONE(5878).ContentHarmTypeBitMask.NONE;
};
export const getMediaObscuredReasonFromBitmask = function getMediaObscuredReasonFromBitmask(arg0, enabledContentHarmTypeFlags) {
  const _require = arg0;
  if (enabledContentHarmTypeFlags === _require(5878).ContentHarmTypeBitMask.NONE) {
    return [];
  } else {
    const arr = getHarmTypeFromBitmask(enabledContentHarmTypeFlags);
    if (0 === arr.length) {
...
export const isMediaObscuredForHarmTypes = function isMediaObscuredForHarmTypes(arg0, enabledHarmTypesForMessage) {
  const _require = arg0;
  if (enabledHarmTypesForMessage === _require(5878).ContentHarmTypeBitMask.NONE) {
    return false;
  } else {
