...
// Dependencies: [32, 4254, 1372, 4472, 3892, 1874, 5804, 5806, 1351, 5808, 1906, 4481, 1384, 1306, 5812, 2]
...
// Module 5803 (getEligibleHarmTypesConfigsForContext)
...
function getEligibleHarmTypesConfigsForContext() {
  const values = Object.values(require(5806) /* ContentHarmType */.CONTENT_SCAN_TYPE_REGISTRY);
...
function getEnabledHarmTypesForMessage(message) {
  ...
  NONE = require(5806) /* ContentHarmType */.ContentHarmTypeBitMask.NONE;
}
function getEnabledHarmTypesForChannelAndAuthorId(channelId, id) {
  ...
  if (null != currentUser) {
    if (id !== currentUser.id) {
      ...
      if (null == tmp10) {
        let NONE = require(5806) /* ContentHarmType */.ContentHarmTypeBitMask.NONE;
      } else {
        require = tmp10;
  ...
  return require(5806) /* ContentHarmType */.ContentHarmTypeBitMask.NONE;
}
function messageHasObscurableMediaForBitmask(firstMessage, EXPLICIT) {
  const _require = EXPLICIT;
  if (EXPLICIT !== _require(5806).ContentHarmTypeBitMask.NONE) {
...
function isMediaFlaggedForHarmType(EXPLICIT, type) {
  if (null == EXPLICIT) {
    ...
    const tmp5 = require(5806) /* ContentHarmType */.CONTENT_SCAN_TYPE_REGISTRY[EXPLICIT];
    ...
    type = type.type;
    if (require(5808) /* ContentHarmTypeChannel */.ObscuredMediaTypes.Embed === type) {
...
function contentHarmTypesToFlags(mapped) {
  ...
  while (iter !== undefined) {
    ...
    if (require(5806) /* ContentHarmType */.ContentHarmType.EXPLICIT === nextResult) {
      let tmp14 = NONE;
      let tmp15 = tmp2;
      let tmp16 = tmp4;
...
function getHarmTypeFromBitmask(enabledHarmTypesForMessage) {
  if (enabledHarmTypesForMessage === require(5806) /* ContentHarmType */.ContentHarmTypeBitMask.NONE) {
    return [];
  } else {
    const items = [];
      ...
      if (null != channel) {
        if (!channel.isDM()) {
          if (!channel.isGroupDM()) {
            const GUILD = require(5808) /* ContentHarmTypeChannel */.ContentHarmTypeChannel.GUILD;
          }
        }
        if (null == closure_1) {
          const NON_FRIEND_DM = require(5808) /* ContentHarmTypeChannel */.ContentHarmTypeChannel.NON_FRIEND_DM;
        } else {
          const friendIDs = obj2.getFriendIDs();
        }
        const FRIEND_DM = require(5808) /* ContentHarmTypeChannel */.ContentHarmTypeChannel.FRIEND_DM;
      }
    }
  }
  return tmp6;
...
function getChannelIdAndAuthorIdFromMessage(message) {
  if (null == message) {
    ...
    if ("messageReference" in message) {
      const items = [message.messageReference, importDefault(5812)(message)];
      let items2 = items;
    } else if ("message_reference" in message) {
      const items1 = [message.message_reference, ];
      let obj = require(5812) /* isForwardMessage */;
...
function hasUnscannedMedia(message, arg1) {
  ...
  const require = tmp;
  if (tmp === require(5806) /* ContentHarmType */.ContentHarmTypeBitMask.NONE) {
    return false;
  } else {
...
export const getEnabledHarmTypesBitmaskForChannelType = function getEnabledHarmTypesBitmaskForChannelType(GUILD) {
  ...
  const values = Object.values(_require(5806).CONTENT_SCAN_TYPE_REGISTRY);
  const found = values.filter((isEligible) => {
    let tmp = null == isEligible.isEligible;
    if (!tmp) {
      isEligible = isEligible.isEligible;
...
export const getMediaObscuredReasonFromBitmask = function getMediaObscuredReasonFromBitmask(arg0, enabledContentHarmTypeFlags) {
  const _require = arg0;
  if (enabledContentHarmTypeFlags === _require(5806).ContentHarmTypeBitMask.NONE) {
    return [];
  } else {
    const arr = getHarmTypeFromBitmask(enabledContentHarmTypeFlags);
    if (0 === arr.length) {
...
export const isMediaObscuredForHarmTypes = function isMediaObscuredForHarmTypes(arg0, enabledHarmTypesForMessage) {
  const _require = arg0;
  if (enabledHarmTypesForMessage === _require(5806).ContentHarmTypeBitMask.NONE) {
    return false;
  } else {
