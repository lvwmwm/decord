...
// Dependencies: [8543, 8540, 8541, 2]
// Exports: computeProfileFrameDimensions

// Module 8542 (computeProfileFrameDimensions)
...
export const computeProfileFrameDimensions = function computeProfileFrameDimensions(closure_4) {
  ...
  const found = closure_4.filter((layer) => {
    layer = layer.layer;
    let tmp3 = layer.type === innerWidth(8540).ProfileFrameLayerType.STAPLE;
    if (tmp3) {
      tmp3 = layer.anchor === innerWidth(8541).ProfileFrameLayerAnchor.TOP;
    }
    return tmp3;
  });
  ...
  const found1 = closure_4.filter((layer) => {
    ...
    let tmp3 = layer.type === innerWidth(8540).ProfileFrameLayerType.STAPLE;
    if (tmp3) {
      tmp3 = layer.anchor === innerWidth(8541).ProfileFrameLayerAnchor.BOTTOM;
    }
    return tmp3;
  });
