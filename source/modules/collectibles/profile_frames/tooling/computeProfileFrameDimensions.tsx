...
// Dependencies: [8807, 8804, 8805, 2]
// Exports: computeProfileFrameDimensions

// Module 8806 (computeProfileFrameDimensions)
...
export const computeProfileFrameDimensions = function computeProfileFrameDimensions(closure_4) {
  innerWidth = innerWidth(8807).DefaultProfileFrameDimensions.INNER_WIDTH;
  ...
  const found = closure_4.filter((layer) => {
    layer = layer.layer;
    let tmp3 = layer.type === innerWidth(8804).ProfileFrameLayerType.STAPLE;
    if (tmp3) {
      tmp3 = layer.anchor === innerWidth(8805).ProfileFrameLayerAnchor.TOP;
    }
    return tmp3;
  });
  const mapped1 = found.map((dims) => Math.max(0, dims.dims.height - (716 - innerWidth(8807).DefaultProfileFrameDimensions.OVERFLOW_TOP)));
  let overflowTop = 0;
  if (mapped1.length > 0) {
    const _Math3 = Math;
    const items1 = [];
  ...
  const found1 = closure_4.filter((layer) => {
    ...
    let tmp3 = layer.type === innerWidth(8804).ProfileFrameLayerType.STAPLE;
    if (tmp3) {
      tmp3 = layer.anchor === innerWidth(8805).ProfileFrameLayerAnchor.BOTTOM;
    }
    return tmp3;
  });
  const mapped2 = found1.map((dims) => Math.max(0, dims.dims.height - (424 - innerWidth(8807).DefaultProfileFrameDimensions.OVERFLOW_BOTTOM)));
