...
// Dependencies: [32, 19, 17, 678, 21, 9262, 712, 4285, 9152, 4804, 6933, 1930, 9292, 9293, 5251, 9334, 679, 9335, 9305, 9295, 9313, 9315, 4128, 3988, 9263, 2]

// Module 9333 (PurchasedAssetOverlay)
...
function PurchasedAssetOverlay() {
  const tmp = callback3();
  obj = { style: tmp.overlayContainer, children: callback2(require(9152) /* CheckmarkLargeBoldIcon */.CheckmarkLargeBoldIcon, obj) };
...
function ProductPreview(arg0) {
  ...
  const tmp = callback3();
  let obj = require(6933) /* getProductOrbPrice */;
...
function ProductPreviewInner(arg0) {
  ...
  if (product.type === cardWidth(1930).CollectiblesItemType.BUNDLE) {
    ...
    return callback2(importDefault(5251), obj);
  } else {
    const ALL = tmp2(679).FractionalPremiumSKUsSets.ALL;
    if (ALL.has(product.skuId)) {
      const obj2 = { skuId: null, width: null, height: null };
      obj2[0] = product.skuId;
      obj2[1] = tmp2(9335).FRACTIONAL_NITRO_COIN_SIZE.CARD;
      obj2[2] = tmp2(9335).FRACTIONAL_NITRO_COIN_SIZE.CARD;
      return callback2(tmp2(9335).FractionalNitroCoinIllustration, obj2);
    } else {
...
function AssetTileInternal(solidBackground) {
  ...
  let obj = require(4128) /* hexToRgba */;
  const tmp = callback3();
  const obj2 = require(4128) /* hexToRgba */;
  let num = 0.8;
  const token = require(3988) /* map */.useToken(importDefault(712).colors.BACKGROUND_BASE_LOW);
  if (flag) {
    num = 1;
  }
...
const memoResult = importAllResult.memo(function AssetTileV2(arg0) {
  ...
  obj = { product: require(6933) /* getProductOrbPrice */.getSelectedProduct(product, defaultVariantIndex), isPurchased, isDisabled, disableBundleStaticBackground, muteBundleStaticBackground, cardWidth };
