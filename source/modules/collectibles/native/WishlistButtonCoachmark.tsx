...
// Dependencies: [32, 19, 1369, 9270, 1358, 5943, 14751, 1236, 8576, 2]
// Exports: default

// Module 14753 (WishlistButtonCoachmark)
...
export default function WishlistButtonCoachmark(anchorRef) {
  ...
  const tmp3 = callback(hasNeverWishlisted(5943).useSelectedDismissibleContent(memo), 2);
  dependencyMap = tmp4;
  const tmp5 = tmp3[0] === hasNeverWishlisted(1358).DismissibleContent.WISHLIST_MOBILE_NUX_PRODUCT_CARD_COACHMARK;
  callback = tmp5;
  const obj2 = hasNeverWishlisted(5943);
  registerDismiss = hasNeverWishlisted(14751).useCollectiblesCoachmarkScrollDismissContext().registerDismiss;
  ...
  const obj3 = hasNeverWishlisted(14751);
  const coachmark = hasNeverWishlisted(8576).useCoachmark(anchorRef.anchorRef, memo1);
  return null;
};
