...
// Dependencies: [32, 19, 1388, 9216, 1377, 7404, 15025, 1236, 9351, 2]
// Exports: default

// Module 15027 (WishlistButtonCoachmark)
...
export default function WishlistButtonCoachmark(anchorRef) {
  ...
  let callback;
  let registerDismiss;
  hasNeverWishlisted = hasNeverWishlisted(9216).useHasNeverWishlisted();
  ...
  let obj = hasNeverWishlisted(9216);
  const tmp3 = callback(hasNeverWishlisted(7404).useSelectedDismissibleContent(memo), 2);
  dependencyMap = tmp4;
  const tmp5 = tmp3[0] === hasNeverWishlisted(1377).DismissibleContent.WISHLIST_MOBILE_NUX_PRODUCT_CARD_COACHMARK;
  callback = tmp5;
  const obj2 = hasNeverWishlisted(7404);
  registerDismiss = hasNeverWishlisted(15025).useCollectiblesCoachmarkScrollDismissContext().registerDismiss;
  ...
  const obj3 = hasNeverWishlisted(15025);
  const coachmark = hasNeverWishlisted(9351).useCoachmark(anchorRef.anchorRef, memo1);
  return null;
};
