...
export const useCardLayout = function useCardLayout() {
  ...
  if (num < 2) {
    ...
    const result = require(9172) /* CollectiblesShopCardInternalV2 */.COLLECTIBLES_SHOP_CARD_WIDTH * num;
    const diff = num - 1;
    const diff1 = width - (result + require(9172) /* CollectiblesShopCardInternalV2 */.COLLECTIBLES_SHOP_CARD_GAP * diff);
    if (diff1 < 2 * require(9172) /* CollectiblesShopCardInternalV2 */.COLLECTIBLES_SHOP_CARD_GAP) {
      const _Math = Math;
    ...
    let tmp12 = num;
    let tmp14 = tmp2;
    if (result1 > require(9172) /* CollectiblesShopCardInternalV2 */.COLLECTIBLES_SHOP_CARD_MAX_WIDTH) {
      let tmp19 = num;
      tmp10 = tmp2;
      tmp11 = result1;
      tmp14 = tmp2;
      ...
      if (num < num2) {
        const sum = tmp19 + 1;
        const result2 = (width - (bound + require(9172) /* CollectiblesShopCardInternalV2 */.COLLECTIBLES_SHOP_CARD_GAP * (sum - 1))) / sum;
        tmp10 = require;
        tmp11 = result2;
        tmp12 = sum;
        tmp14 = require;
        while (result2 > require(9172) /* CollectiblesShopCardInternalV2 */.COLLECTIBLES_SHOP_CARD_MAX_WIDTH) {
          tmp19 = sum;
          let tmp9 = tmp17;
          tmp10 = tmp16;
          tmp11 = result2;
