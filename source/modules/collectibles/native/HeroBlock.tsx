...
// Dependencies: [19, 17, 7043, 678, 676, 21, 9390, 4344, 712, 14389, 14907, 1499, 9393, 589, 10507, 4280, 12238, 4065, 4193, 14381, 14383, 5767, 5787, 14908, 698, 1363, 14910, 14911, 12241, 4765, 4340, 4754, 1236, 11224, 5206, 4886, 5817, 5754, 14912, 7786, 2]
// Exports: default
...
function SkeletonLoading(accessibilityLabel) {
  ...
  obj[5] = Array.from({ length: 10 }).map((arg0, arg1) => {
    ...
    obj[0] = callback(9390).COLLECTIBLES_SHOP_CARD_WIDTH;
...
export default function _default(heroBlock) {
  ...
  const tmp7 = preferVCPrice(4280)();
  const tmp8 = createCacheKey();
  const tmpResult1 = heroBlock(12238);
  const token = heroBlock(4065).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOW);
  const tmpResult2 = heroBlock(4065);
  const tmpResult3 = heroBlock(4193);
  const tmpResult4 = heroBlock(4193);
  const hexToRgbaStringResult = tmpResult3.hexToRgbaString(heroBlock(4193).hexWithOpacity(token, 0));
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      ...
      obj[0] = tmp15(preferVCPrice(5787).COLLECTIBLES_SHOP_HERO).analyticsLocations;
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(4765), obj3);
        }
      ...
      if (tmp4) {
        ...
        if (tmp22Result) {
          ...
          obj10[3] = tmp18;
          tmp22Result = tmp22(tmp(4340).Text, obj10);
        }
        ...
        if (tmp22Result1) {
          ...
          obj11[1] = tmp19;
          tmp22Result1 = tmp22(tmp(4340).Text, obj11);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(4754).Button, obj13);
        }
        ...
        obj21[1] = tmp22(tmp(5817).ChevronSmallRightIcon, { size: "sm", color: "white" });
        items9[1] = tmp22(tmp25, obj21);
        obj17[1] = items9;
        obj14[7] = tmp24(tmp25, obj17);
        tmp24Result = tmp22(tmp(4886).PressableOpacity, obj14, stateFromStores.storeListingId);
      }
      obj8[1] = tmp24Result;
      ...
      if (tmp4) {
        ...
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj27[5] = tmp17;
          obj27[7] = tmp(9390).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(712).space.PX_12;
          ...
          tmp22Result4 = tmp22(tmp(7786).FlashList, obj27);
        }
      ...
      obj22[1] = closure_11(tmp(5754).LayerScope, obj30);
      items6[2] = closure_11(c5, obj22);
      obj1[1] = items6;
      obj[2] = closure_13(c5, obj1);
      obj[1] = closure_11(tmp6Result, obj);
      return closure_11(tmp(5767).AnalyticsLocationProvider, obj);
    } else {
