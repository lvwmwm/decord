...
// Dependencies: [19, 17, 7234, 678, 676, 21, 8827, 4446, 712, 14616, 15350, 1500, 8830, 589, 11022, 4381, 12558, 4166, 4294, 14608, 14610, 5918, 5938, 15351, 698, 1363, 15353, 15354, 12561, 4905, 4442, 4893, 1236, 11268, 5357, 5033, 5968, 5905, 15355, 8051, 2]
// Exports: default

// Module 15349 (SkeletonLoading)
...
function SkeletonLoading(accessibilityLabel) {
  let obj = {
    ...
    children: Array.from({ length: 10 }).map((arg0, arg1) => {
      const obj = { width: callback(8827).COLLECTIBLES_SHOP_CARD_WIDTH };
      return callback3(callback2(14616), obj, arg1);
    })
  };
...
export default function _default(heroBlock) {
  ...
  closure_5 = undefined;
  closure_6 = undefined;
  let obj = heroBlock(15350);
  obj1 = heroBlock(1500);
  dependencyMap = obj1.useNavigation();
  let obj2 = heroBlock(8830);
  ...
  const tmp7 = preferVCPrice(4381)();
  const tmp8 = callback2();
  const tmpResult1 = heroBlock(12558);
  const token = heroBlock(4166).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOW);
  const tmpResult2 = heroBlock(4166);
  const tmpResult3 = heroBlock(4294);
  const tmpResult4 = heroBlock(4294);
  const hexToRgbaStringResult = tmpResult3.hexToRgbaString(heroBlock(4294).hexWithOpacity(token, 0));
  const token1 = heroBlock(4166).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(14608)();
  closure_5 = tmp12;
  const items1 = [heroBlock.rankedSkuIds, tmp12];
  const memo = React.useMemo(() => callback(heroBlock.rankedSkuIds), items1);
  const tmpResult5 = heroBlock(4166);
  const filteredAndSortedProducts = heroBlock(14610).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  closure_6 = tmp14;
  const tmpResult6 = heroBlock(14610);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      ...
      obj[0] = tmp15(preferVCPrice(5938).COLLECTIBLES_SHOP_HERO).analyticsLocations;
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(4905), obj3);
        }
      ...
      if (tmp4) {
        ...
        if (tmp22Result) {
          ...
          obj10[3] = tmp18;
          tmp22Result = tmp22(tmp(4442).Text, obj10);
        }
        ...
        if (tmp22Result1) {
          ...
          obj11[1] = tmp19;
          tmp22Result1 = tmp22(tmp(4442).Text, obj11);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(4893).Button, obj13);
        }
        ...
        obj21[1] = tmp22(tmp(5968).ChevronSmallRightIcon, { size: "sm", color: "white" });
        items9[1] = tmp22(tmp25, obj21);
        obj17[1] = items9;
        obj14[7] = tmp24(tmp25, obj17);
        tmp24Result = tmp22(tmp(5033).PressableOpacity, obj14, stateFromStores.storeListingId);
      }
      obj8[1] = tmp24Result;
      ...
      if (tmp4) {
        ...
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj27[5] = tmp17;
          obj27[7] = tmp(8827).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(712).space.PX_12;
          ...
          tmp22Result4 = tmp22(tmp(8051).FlashList, obj27);
        }
      ...
      obj22[1] = closure_11(tmp(5905).LayerScope, obj30);
      items6[2] = closure_11(closure_5, obj22);
      obj1[1] = items6;
      obj[2] = closure_13(closure_5, obj1);
      obj[1] = closure_11(tmp6Result, obj);
      return closure_11(tmp(5918).AnalyticsLocationProvider, obj);
    } else {
      if (tmpResult7.isThemeDark(tmp7)) {
        let tmp6Result1 = tmp6(15353);
      } else {
        tmp6Result1 = tmp6(15354);
      }
      tmpResult7 = tmp(1363);
    }
  }
