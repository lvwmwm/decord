...
// Dependencies: [19, 17, 6922, 678, 676, 21, 9262, 4285, 712, 14238, 14751, 1480, 9265, 589, 10378, 4221, 12088, 3988, 4128, 14230, 14232, 5649, 5669, 14752, 698, 4130, 14754, 14755, 12091, 4706, 4281, 4695, 1236, 11084, 5148, 4827, 5699, 5636, 14756, 7666, 2]
// Exports: default

// Module 14750 (SkeletonLoading)
...
function SkeletonLoading(accessibilityLabel) {
  ...
  obj[5] = Array.from({ length: 10 }).map((arg0, arg1) => {
    ...
    return callback3(callback2(14238), obj, arg1);
  });
...
export default function _default(heroBlock) {
  ...
  const tmp7 = preferVCPrice(4221)();
  const tmp8 = createCacheKey();
  const tmpResult1 = heroBlock(12088);
  const token = heroBlock(3988).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOW);
  const tmpResult2 = heroBlock(3988);
  const tmpResult3 = heroBlock(4128);
  const tmpResult4 = heroBlock(4128);
  const hexToRgbaStringResult = tmpResult3.hexToRgbaString(heroBlock(4128).hexWithOpacity(token, 0));
  const token1 = heroBlock(3988).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(14230)();
  c5 = tmp12;
  const items1 = [heroBlock.rankedSkuIds, tmp12];
  const memo = React.useMemo(() => _undefined(heroBlock.rankedSkuIds), items1);
  const tmpResult5 = heroBlock(3988);
  const filteredAndSortedProducts = heroBlock(14232).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  c6 = tmp14;
  const tmpResult6 = heroBlock(14232);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      ...
      obj[0] = tmp15(preferVCPrice(5669).COLLECTIBLES_SHOP_HERO).analyticsLocations;
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(4706), obj3);
        }
      ...
      if (tmp4) {
        ...
        if (tmp22Result) {
          ...
          obj10[3] = tmp18;
          tmp22Result = tmp22(tmp(4281).Text, obj10);
        }
        ...
        if (tmp22Result1) {
          ...
          obj11[1] = tmp19;
          tmp22Result1 = tmp22(tmp(4281).Text, obj11);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(4695).Button, obj13);
        }
        ...
        obj21[1] = tmp22(tmp(5699).ChevronSmallRightIcon, { size: "sm", color: "white" });
        items9[1] = tmp22(tmp25, obj21);
        obj17[1] = items9;
        obj14[7] = tmp24(tmp25, obj17);
        tmp24Result = tmp22(tmp(4827).PressableOpacity, obj14, stateFromStores.storeListingId);
      }
      obj8[1] = tmp24Result;
      ...
      if (tmp4) {
        ...
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj27[5] = tmp17;
          obj27[7] = tmp(9262).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(712).space.PX_12;
          ...
          tmp22Result4 = tmp22(tmp(7666).FlashList, obj27);
        }
      ...
      obj22[1] = closure_11(tmp(5636).LayerScope, obj30);
      items6[2] = closure_11(c5, obj22);
      obj1[1] = items6;
      obj[2] = closure_13(c5, obj1);
      obj[1] = closure_11(tmp6Result, obj);
      return closure_11(tmp(5649).AnalyticsLocationProvider, obj);
    } else {
      if (tmpResult7.isThemeDark(tmp7)) {
        let tmp6Result1 = tmp6(14754);
      } else {
        tmp6Result1 = tmp6(14755);
      }
      tmpResult7 = tmp(4130);
    }
  }
