...
// Dependencies: [19, 17, 6941, 678, 676, 21, 9279, 4302, 712, 14252, 14767, 1480, 9282, 589, 10394, 4238, 12103, 4005, 4145, 14244, 14246, 5668, 5688, 14768, 698, 4147, 14770, 14771, 12106, 4723, 4298, 4712, 1236, 11099, 5165, 4844, 5718, 5655, 14772, 7685, 2]
// Exports: default

// Module 14766 (SkeletonLoading)
...
function SkeletonLoading(accessibilityLabel) {
  ...
  obj[5] = Array.from({ length: 10 }).map((arg0, arg1) => {
    ...
    obj[0] = callback(9279).COLLECTIBLES_SHOP_CARD_WIDTH;
    return callback3(callback2(14252), obj, arg1);
  });
...
export default function _default(heroBlock) {
  ...
  const tmp7 = preferVCPrice(4238)();
  const tmp8 = createCacheKey();
  const tmpResult1 = heroBlock(12103);
  const token = heroBlock(4005).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOW);
  const tmpResult2 = heroBlock(4005);
  const tmpResult3 = heroBlock(4145);
  const tmpResult4 = heroBlock(4145);
  const hexToRgbaStringResult = tmpResult3.hexToRgbaString(heroBlock(4145).hexWithOpacity(token, 0));
  const token1 = heroBlock(4005).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(14244)();
  c5 = tmp12;
  ...
  const filteredAndSortedProducts = heroBlock(14246).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  c6 = tmp14;
  const tmpResult6 = heroBlock(14246);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      ...
      obj[0] = tmp15(preferVCPrice(5688).COLLECTIBLES_SHOP_HERO).analyticsLocations;
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(4723), obj3);
        }
      ...
      if (tmp4) {
        ...
        if (tmp22Result) {
          ...
          obj10[3] = tmp18;
          tmp22Result = tmp22(tmp(4298).Text, obj10);
        }
        ...
        if (tmp22Result1) {
          ...
          obj11[1] = tmp19;
          tmp22Result1 = tmp22(tmp(4298).Text, obj11);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(4712).Button, obj13);
        }
        ...
        obj21[1] = tmp22(tmp(5718).ChevronSmallRightIcon, { size: "sm", color: "white" });
        items9[1] = tmp22(tmp25, obj21);
        obj17[1] = items9;
        obj14[7] = tmp24(tmp25, obj17);
        tmp24Result = tmp22(tmp(4844).PressableOpacity, obj14, stateFromStores.storeListingId);
      }
      obj8[1] = tmp24Result;
      ...
      if (tmp4) {
        ...
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj27[5] = tmp17;
          obj27[7] = tmp(9279).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(712).space.PX_12;
          ...
          tmp22Result4 = tmp22(tmp(7685).FlashList, obj27);
        }
      ...
      obj22[1] = closure_11(tmp(5655).LayerScope, obj30);
      items6[2] = closure_11(c5, obj22);
      obj1[1] = items6;
      obj[2] = closure_13(c5, obj1);
      obj[1] = closure_11(tmp6Result, obj);
      return closure_11(tmp(5668).AnalyticsLocationProvider, obj);
    } else {
      if (tmpResult7.isThemeDark(tmp7)) {
        let tmp6Result1 = tmp6(14770);
      } else {
        tmp6Result1 = tmp6(14771);
      }
      tmpResult7 = tmp(4147);
    }
  }
