...
// Dependencies: [19, 17, 7148, 678, 676, 21, 9492, 4380, 712, 14532, 15055, 1500, 9495, 589, 10532, 4315, 12379, 4101, 4228, 14524, 14526, 5834, 5854, 15056, 698, 1363, 15058, 15059, 12382, 4826, 4376, 4815, 1236, 11326, 5273, 4949, 5884, 5821, 15060, 7890, 2]
// Exports: default

// Module 15054 (SkeletonLoading)
...
function SkeletonLoading(accessibilityLabel) {
  let obj = {
    ...
    children: Array.from({ length: 10 }).map((arg0, arg1) => {
      const obj = { width: callback(9492).COLLECTIBLES_SHOP_CARD_WIDTH };
      return callback3(callback2(14532), obj, arg1);
    })
  };
...
export default function _default(heroBlock) {
  ...
  closure_5 = undefined;
  closure_6 = undefined;
  let obj = heroBlock(15055);
  obj1 = heroBlock(1500);
  dependencyMap = obj1.useNavigation();
  let obj2 = heroBlock(9495);
  ...
  const tmp7 = preferVCPrice(4315)();
  const tmp8 = callback2();
  const tmpResult1 = heroBlock(12379);
  const token = heroBlock(4101).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOW);
  const tmpResult2 = heroBlock(4101);
  const tmpResult3 = heroBlock(4228);
  const tmpResult4 = heroBlock(4228);
  const hexToRgbaStringResult = tmpResult3.hexToRgbaString(heroBlock(4228).hexWithOpacity(token, 0));
  const token1 = heroBlock(4101).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(14524)();
  closure_5 = tmp12;
  const items1 = [heroBlock.rankedSkuIds, tmp12];
  const memo = React.useMemo(() => callback(heroBlock.rankedSkuIds), items1);
  const tmpResult5 = heroBlock(4101);
  const filteredAndSortedProducts = heroBlock(14526).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  closure_6 = tmp14;
  const tmpResult6 = heroBlock(14526);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      ...
      obj[0] = tmp15(preferVCPrice(5854).COLLECTIBLES_SHOP_HERO).analyticsLocations;
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(4826), obj3);
        }
      ...
      if (tmp4) {
        ...
        if (tmp22Result) {
          ...
          obj10[3] = tmp18;
          tmp22Result = tmp22(tmp(4376).Text, obj10);
        }
        ...
        if (tmp22Result1) {
          ...
          obj11[1] = tmp19;
          tmp22Result1 = tmp22(tmp(4376).Text, obj11);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(4815).Button, obj13);
        }
        ...
        obj21[1] = tmp22(tmp(5884).ChevronSmallRightIcon, { size: "sm", color: "white" });
        items9[1] = tmp22(tmp25, obj21);
        obj17[1] = items9;
        obj14[7] = tmp24(tmp25, obj17);
        tmp24Result = tmp22(tmp(4949).PressableOpacity, obj14, stateFromStores.storeListingId);
      }
      obj8[1] = tmp24Result;
      ...
      if (tmp4) {
        ...
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj27[5] = tmp17;
          obj27[7] = tmp(9492).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(712).space.PX_12;
          ...
          tmp22Result4 = tmp22(tmp(7890).FlashList, obj27);
        }
      ...
      obj22[1] = closure_11(tmp(5821).LayerScope, obj30);
      items6[2] = closure_11(closure_5, obj22);
      obj1[1] = items6;
      obj[2] = closure_13(closure_5, obj1);
      obj[1] = closure_11(tmp6Result, obj);
      return closure_11(tmp(5834).AnalyticsLocationProvider, obj);
    } else {
      if (tmpResult7.isThemeDark(tmp7)) {
        let tmp6Result1 = tmp6(15058);
      } else {
        tmp6Result1 = tmp6(15059);
      }
      tmpResult7 = tmp(1363);
    }
  }
