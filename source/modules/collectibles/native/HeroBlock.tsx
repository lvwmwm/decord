...
// Dependencies: [19, 17, 7214, 678, 676, 21, 9560, 4444, 712, 14605, 15134, 1500, 9563, 589, 10600, 4379, 12451, 4165, 4292, 14597, 14599, 5900, 5920, 15135, 698, 1363, 15137, 15138, 12454, 4891, 4440, 4879, 1236, 11398, 5339, 5015, 5950, 5887, 15139, 7957, 2]
// Exports: default

// Module 15133 (SkeletonLoading)
...
function SkeletonLoading(accessibilityLabel) {
  let obj = {
    ...
    children: Array.from({ length: 10 }).map((arg0, arg1) => {
      const obj = { width: callback(9560).COLLECTIBLES_SHOP_CARD_WIDTH };
      return callback3(callback2(14605), obj, arg1);
    })
  };
...
export default function _default(heroBlock) {
  ...
  closure_5 = undefined;
  closure_6 = undefined;
  let obj = heroBlock(15134);
  obj1 = heroBlock(1500);
  dependencyMap = obj1.useNavigation();
  let obj2 = heroBlock(9563);
  ...
  const tmp7 = preferVCPrice(4379)();
  const tmp8 = callback2();
  const tmpResult1 = heroBlock(12451);
  const token = heroBlock(4165).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOW);
  const tmpResult2 = heroBlock(4165);
  const tmpResult3 = heroBlock(4292);
  const tmpResult4 = heroBlock(4292);
  const hexToRgbaStringResult = tmpResult3.hexToRgbaString(heroBlock(4292).hexWithOpacity(token, 0));
  const token1 = heroBlock(4165).useToken(preferVCPrice(712).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(14597)();
  closure_5 = tmp12;
  ...
  const filteredAndSortedProducts = heroBlock(14599).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  closure_6 = tmp14;
  const tmpResult6 = heroBlock(14599);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      ...
      obj[0] = tmp15(preferVCPrice(5920).COLLECTIBLES_SHOP_HERO).analyticsLocations;
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(4891), obj3);
        }
      ...
      if (tmp4) {
        ...
        if (tmp22Result) {
          ...
          obj10[3] = tmp18;
          tmp22Result = tmp22(tmp(4440).Text, obj10);
        }
        ...
        if (tmp22Result1) {
          ...
          obj11[1] = tmp19;
          tmp22Result1 = tmp22(tmp(4440).Text, obj11);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(4879).Button, obj13);
        }
        ...
        obj21[1] = tmp22(tmp(5950).ChevronSmallRightIcon, { size: "sm", color: "white" });
        items9[1] = tmp22(tmp25, obj21);
        obj17[1] = items9;
        obj14[7] = tmp24(tmp25, obj17);
        tmp24Result = tmp22(tmp(5015).PressableOpacity, obj14, stateFromStores.storeListingId);
      }
      obj8[1] = tmp24Result;
      ...
      if (tmp4) {
        ...
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj27[5] = tmp17;
          obj27[7] = tmp(9560).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(712).space.PX_12;
          ...
          tmp22Result4 = tmp22(tmp(7957).FlashList, obj27);
        }
      ...
      obj22[1] = closure_11(tmp(5887).LayerScope, obj30);
      items6[2] = closure_11(closure_5, obj22);
      obj1[1] = items6;
      obj[2] = closure_13(closure_5, obj1);
      obj[1] = closure_11(tmp6Result, obj);
      return closure_11(tmp(5900).AnalyticsLocationProvider, obj);
    } else {
      if (tmpResult7.isThemeDark(tmp7)) {
        let tmp6Result1 = tmp6(15137);
      } else {
        tmp6Result1 = tmp6(15138);
      }
      tmpResult7 = tmp(1363);
    }
  }
