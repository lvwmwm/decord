...
// Dependencies: [31, 27, 653, 1852, 33, 4165, 689, 8690, 5119, 11884, 4101, 5828, 4011, 6296, 6297, 4161, 1212, 1921, 3862, 5221, 6206, 4695, 9798, 4578, 4133, 5481, 2]
// Exports: default

// Module 11883 (NitroAcquiredHeader)
...
function NitroAcquiredHeader(skuId) {
  ...
  obj = { source: importDefault(11884) };
  const items = [callback(importDefault(5119), obj), ];
  obj = { style: tmp.fractionNitroIcon };
  const obj1 = { skuId: skuId.skuId, width: require(8690) /* FRACTIONAL_NITRO_COIN_SIZE */.FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET, height: require(8690) /* FRACTIONAL_NITRO_COIN_SIZE */.FRACTIONAL_NITRO_COIN_SIZE.COLLECTED_SHEET };
  obj.children = callback(require(8690) /* FRACTIONAL_NITRO_COIN_SIZE */.FractionalNitroCoinIllustration, obj1);
...
export default function FractionalNitroCollectedActionSheet(skuId) {
  ...
  if (consumed) {
    ...
    let tmp10Result = tmp10(consumed(6206), obj1);
  } else {
    const obj2 = { skuId };
  ...
  obj3.children = callback(skuId(9798).CircleQuestionIcon, obj4);
  items2[1] = callback(skuId(4695).PressableOpacity, obj3);
  obj.children = items2;
  ...
  if (fetchFractionalPremiumInfo.isLoading) {
    ...
    const items5 = [callback(skuId(4578).Button, tmp27), ];
    ...
    items5[1] = callback(skuId(4578).Button, obj10);
  ...
  items3[2] = callback(skuId(5481).ActionSheetHeaderBar, { variant: "floating" });
  obj.children = items3;
  return closure_10(skuId(5221).BottomSheet, obj);
};
