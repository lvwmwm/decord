...
// Function ID: 9616
// Name: PriceTag
// Dependencies: [19, 17, 5899, 678, 676, 21, 4342, 712, 4338, 1949, 7060, 7061, 9458, 1236, 9398, 4007, 9616, 9617, 9469, 9618, 589, 9453, 9478, 500, 7775, 7749, 2]
// Exports: default

// Module 9615 (PriceTag)
...
function PriceTag(accessibilityLabel) {
  ...
  items[1] = callback(require(4338) /* Text */.Text, { variant, style: items1, color: str, accessibilityLabel: accessibilityLabel.accessibilityLabel, children: priceFormatted });
  obj[0] = items;
...
function OrbsPriceTag(arg0) {
  ...
  if (null == vcData.price) {
    ...
    let result = product.type === require(1949) /* CollectiblesItemType */.CollectiblesItemType.BUNDLE;
    if (result) {
      let tmp16Result = tmp16(7060);
      result = tmp16Result.isOrbsExclusiveProduct(product);
    }
    tmp16Result = tmp16(7061);
    ...
    if (result) {
      result = discountPercentage >= tmp16(7061).DISCOUNT_DISPLAY_MINIMUM_THRESHOLD;
    }
    ...
    if (result) {
      ...
      obj1[2] = tmp.orbsIcon;
      obj[3] = callback(tmp16(9458).OrbsIcon, obj1);
    ...
    if (!result) {
      ...
      obj4[2] = tmp.orbsIcon;
      tmp10Result = tmp10(tmp16(9458).OrbsIcon, obj4);
    }
    obj3[2] = tmp10Result;
...
function BundleDiscountV2(discountPercentage) {
  ...
  let tmp4 = null;
  if (discountPercentage >= require(7061) /* getItemRecordsFromPurchases */.DISCOUNT_DISPLAY_MINIMUM_THRESHOLD) {
    let obj = { style: null, children: null };
    obj[0] = tmp.discount;
...
export default function InlinePriceTag(arg0) {
  ...
  let obj1 = require(7061) /* getItemRecordsFromPurchases */;
  ...
  dependencyMap = importDefault(9616)(constants.SHOP_PRODUCT_DETAILS);
  let obj3 = require(9617) /* useProductDisableState */;
  const isDisabled = obj3.useProductDisableState(product.skuId).isDisabled;
  let obj4 = require(9469) /* hasAtLeastOneGPlaySynced */;
  const formattedPriceForCollectiblesProduct = obj4.getFormattedPriceForCollectiblesProduct(product, false, true);
  let obj5 = require(9618) /* useVirtualCurrencyData */;
  ...
  let obj7 = require(9453) /* useCanPurchaseFrames */;
  ...
  if (stateFromStores) {
    if (null == formattedPriceForCollectiblesProduct) {
      return callback(tmp2(9478).CollectiblesShopPricePlaceholder, {});
    }
  }
  if (null == formattedPriceForCollectiblesProduct) {
    let tmp30 = null;
    ...
    if (!(null != formattedPriceForCollectiblesProduct1 && tmp13)) {
      ...
      if (tmp17Result) {
        ...
        obj8[1] = items3;
        const items4 = [tmp17(tmp2(7749).NitroWheelIcon, obj8), ];
        ...
        obj10[1] = function subscribeNowHook(children) {
          return outer1_9(outer1_0(4338).Text, { variant: "text-md/normal", style: _undefined.underline, children }, arg1);
        };
        obj9[3] = intl3.format(tmp2(1236).t.Kxw2LT, obj10);
        items4[1] = tmp17(tmp2(4338).Text, obj9);
        obj7[3] = items4;
      ...
      if (shopDiscountSource === tmp2(7061).ShopDiscountSource.THIRDPARTY) {
        const obj14 = { color: "interactive-text-active", style: null };
        obj14[1] = tmp.nitroIcon;
        let tmp17Result2 = tmp17(tmp2(7775).TagIcon, obj14);
      } else {
        const obj15 = { color: "interactive-text-active", style: null };
        obj15[1] = tmp.nitroIcon;
        tmp17Result2 = tmp17(tmp2(7749).NitroWheelIcon, obj15);
      }
