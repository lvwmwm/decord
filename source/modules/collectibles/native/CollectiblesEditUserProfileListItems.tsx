...
// Dependencies: [19, 17, 1921, 675, 21, 4478, 709, 5077, 4446, 4447, 1296, 8103, 4474, 1233, 5962, 7296, 4445, 8104, 8105, 586, 4139, 8093, 7309, 2]
// Exports: EditCollectiblesListItemNone, EditCollectiblesListItemProduct, EditCollectiblesListItemShop

// Module 8102 (EditCollectibleListItem)
...
import registerAssetDefault from "registerAsset" /* 8103 */;
...
export const EditCollectiblesListItemShop = function EditCollectiblesListItemShop(analyticsSource) {
  ...
  analyticsLocations = analyticsLocations(5962)(analyticsSource).analyticsLocations;
  ...
  obj = { source: analyticsLocations(8104), size: analyticsSource(1296).IconSizes.LARGE };
  ...
  items1[1] = callback(analyticsSource(4474).Text, obj);
  items1[2] = callback(analyticsSource(8105).NewBadge, { style: tmp2.newIcon });
  obj.children = items1;
...
export const EditCollectiblesListItemProduct = function EditCollectiblesListItemProduct(skuId) {
  ...
  if (!result) {
    result = tmp3(7309).isPremiumCollectiblesPurchase(purchase);
    const tmp3Result = tmp3(7309);
  }
  obj = { isSelected };
  const merged1 = Object.assign(merged);
  ...
  if (null == purchase) {
    if (!result) {
      ...
      let tmp14 = callback(tmp3(8105).LockBadge, obj);
    }
    items1[1] = tmp14;
    obj.children = items1;
    return closure_8(EditCollectibleListItem, obj);
  ...
  if (result) {
    ...
    if (!isTryItOut) {
      if (null == purchase) {
        ...
        tmp15 = callback(tmp3(8105).PremiumBadge, obj1);
      } else {
        tmp15 = null;
      }
    }
