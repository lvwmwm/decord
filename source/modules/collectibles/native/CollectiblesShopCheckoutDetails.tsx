...
// Dependencies: [19, 17, 678, 9294, 21, 4285, 712, 8744, 5236, 12051, 8719, 9313, 9292, 9312, 9293, 1898, 9308, 1901, 679, 9335, 9334, 9305, 1236, 4281, 6921, 9337, 12052, 9266, 3931, 9347, 2]
// Exports: default

// Module 12050 (ProfileEffectAssetPreview)
...
function ProfileEffectAssetPreview(item) {
  const tmp = callback4();
  const tmp4 = importDefault(8744)(item.item.skuId);
  let tmp5 = null;
  if (null != tmp4) {
    ...
    const items = [callback(tmp2(5236), obj), ];
    ...
    obj1[1] = obj2;
    obj1[2] = tmp4.title;
    items[1] = callback(tmp2(5236), obj1);
    obj[1] = items;
...
function ProfileFrameAssetPreview(arg0) {
  ...
  const tmp3 = importDefault(8719)(item.skuId, "CollectiblesShopCheckoutDetails");
  let tmp4 = null;
  if (null != tmp3) {
...
function CollectibleProductPreviewContent(userAvatarSource) {
  ...
  if (ALL.has(product.skuId)) {
    ...
    return callback(importDefault(5236), obj);
  } else {
