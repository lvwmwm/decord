...
// Dependencies: [19, 17, 678, 9422, 21, 4344, 712, 8867, 5307, 12201, 8842, 9441, 9420, 9440, 9421, 1946, 9436, 1949, 679, 9463, 9462, 9433, 1236, 4340, 7054, 9465, 12202, 9394, 4007, 9475, 2]
// Exports: default

// Module 12200 (ProfileEffectAssetPreview)
...
function ProfileEffectAssetPreview(item) {
  const tmp = callback4();
  const tmp4 = importDefault(8867)(item.item.skuId);
  let tmp5 = null;
  if (null != tmp4) {
    ...
    const items = [callback(tmp2(5307), obj), ];
    ...
    obj1[1] = obj2;
    obj1[2] = tmp4.title;
    items[1] = callback(tmp2(5307), obj1);
    obj[1] = items;
...
function ProfileFrameAssetPreview(arg0) {
  ...
  const tmp3 = importDefault(8842)(item.skuId, "CollectiblesShopCheckoutDetails");
  let tmp4 = null;
  if (null != tmp3) {
...
function BundleAssetPreview(height) {
  ...
  obj[1] = callback(height(9421), obj1);
...
function NameplateAssetPreview(item) {
  ...
  return callback(importDefault(9436), obj);
}
...
function CollectibleProductPreviewContent(userAvatarSource) {
  ...
  if (ALL.has(product.skuId)) {
    ...
    obj[0] = product.skuId;
    obj[1] = tmp2(9463).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT;
    obj[2] = tmp2(9463).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT;
    return callback(tmp2(9463).FractionalNitroCoinIllustration, obj);
    ...
    return callback(importDefault(5307), obj);
  } else {
    ...
    if (tmp2(1949).CollectiblesItemType.AVATAR_DECORATION === type) {
      ...
      return callback(importDefault(9433), obj2);
...
export default function CollectiblesShopCheckoutDetails(recipientUser) {
  ...
  if (flag3) {
    ...
    obj1[1] = tmp10(tmp3(4340).Text, obj2);
