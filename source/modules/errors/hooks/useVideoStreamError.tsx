...
// Dependencies: [1218, 10744, 4316, 589, 9038, 2]
// Exports: default, useVideoStreamErrorContext

// Module 10743 (useVideoStreamError)
...
export default function useVideoStreamError(arg0, arg1) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => {
    ...
    if (outer1_2.getId() !== dependencyMap) {
      const items = [];
      let arraySpreadResult = HermesBuiltin.arraySpread(outer1_3.getActiveErrorsOfType(callback(9038).AVError.VIDEO_STREAM_RECEIVER_READY_TIMEOUT), 0);
      arraySpreadResult = HermesBuiltin.arraySpread(outer1_3.getActiveErrorsOfType(callback(9038).AVError.VIDEO_STREAM_RECEIVER_READY_TIMEOUT_NO_STREAM), arraySpreadResult);
      items[Symbol.iterator]();
    }
    if (callback === outer1_4.STREAM) {
      let activeErrorsOfType = outer1_3.getActiveErrorsOfType(callback(9038).AVError.SCREENSHARE_OS_ERROR);
    } else {
      activeErrorsOfType = [];
    }
