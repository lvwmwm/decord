...
// Dependencies: [1218, 9564, 4542, 589, 9565, 2]
// Exports: default, useVideoStreamErrorContext

// Module 9563 (useVideoStreamError)
...
export default function useVideoStreamError(arg0, arg1) {
  ...
  const stateFromStores = _require(589).useStateFromStores(items, () => {
    if (closure_1_2.getId() !== dependencyMap) {
      const items = [];
      let arraySpreadResult = HermesBuiltin.arraySpread(closure_1_3.getActiveErrorsOfType(callback(9565).AVError.VIDEO_STREAM_RECEIVER_READY_TIMEOUT), 0);
      arraySpreadResult = HermesBuiltin.arraySpread(closure_1_3.getActiveErrorsOfType(callback(9565).AVError.VIDEO_STREAM_RECEIVER_READY_TIMEOUT_NO_STREAM), arraySpreadResult);
