  ...
  return _require(566).useStateFromStores(items, () => {
    ...
    if (outer1_2.getId() === dependencyMap) {
      if (callback === outer1_4.STREAM) {
        let activeErrorsOfType = outer1_3.getActiveErrorsOfType(callback(8784).AVError.SCREENSHARE_OS_ERROR);
      } else {
        activeErrorsOfType = [];
      }
      const items = [];
      let arraySpreadResult = HermesBuiltin.arraySpread(activeErrorsOfType, 0);
      arraySpreadResult = HermesBuiltin.arraySpread(outer1_3.getActiveErrorsOfType(callback(8784).AVError.VIDEO_STREAM_SENDER_READY_TIMEOUT), arraySpreadResult);
      HermesBuiltin.arraySpread(outer1_3.getActiveErrorsOfType(callback(8784).AVError.VIDEO_STREAM_SENDER_READY_TIMEOUT_NO_STREAM), arraySpreadResult);
    } else {
      const items1 = [];
      HermesBuiltin.arraySpread(outer1_3.getActiveErrorsOfType(callback(8784).AVError.VIDEO_STREAM_RECEIVER_READY_TIMEOUT_NO_STREAM), HermesBuiltin.arraySpread(outer1_3.getActiveErrorsOfType(callback(8784).AVError.VIDEO_STREAM_RECEIVER_READY_TIMEOUT), 0));
      let num6 = 0;
      if (0 < items1.length) {
        while (true) {
          tmp27 = items1[num6];
