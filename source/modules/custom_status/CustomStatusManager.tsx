...
// Function ID: 16152
// Name: timeout
// Dependencies: [4867, 676, 4074, 5134, 3862, 10252, 1355, 1337, 10251, 2]

// Module 16151 (timeout)
...
let prototype = function CustomStatusManager() {
  ...
  applyArgumentsResult.manageExpiringCustomStatus = function manageExpiringCustomStatus() {
    let CustomStatusSetting = applyArgumentsResult(3862).CustomStatusSetting;
      ...
      if (null != setting.expiresAtMs) {
        if ("0" !== setting.expiresAtMs) {
          ...
          if (diff > 0) {
            ...
            const CustomStatusSetting2 = applyArgumentsResult(3862).CustomStatusSetting;
  ...
  applyArgumentsResult.manageExpiringStatus = function manageExpiringStatus() {
    const StatusExpiresAtSetting = applyArgumentsResult(3862).StatusExpiresAtSetting;
  ...
  applyArgumentsResult.lazilyMigrateStatusCreatedAt = function lazilyMigrateStatusCreatedAt() {
    ...
    if (tmp) {
      const StatusCreatedAtSetting = applyArgumentsResult(3862).StatusCreatedAtSetting;
  ...
  applyArgumentsResult.manageExpiringFocusMode = function manageExpiringFocusMode() {
    const FocusModeExpiresAtSetting = applyArgumentsResult(3862).FocusModeExpiresAtSetting;
    ...
    if (null != setting) {
      if ("0" !== setting) {
        ...
        if (diff > 0) {
          ...
          applyArgumentsResult(10251).setFocusMode(false);
          closure_7.stop();
          const tmpResult = applyArgumentsResult(10251);
        }
      }
    }
    if (null != closure_7) {
