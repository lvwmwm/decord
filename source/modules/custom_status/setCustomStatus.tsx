...
// Dependencies: [12080, 676, 3928, 3837, 12084, 698, 2]
// Exports: default

// Module 12083 (setCustomStatus)
...
export default function setCustomStatus(arg0) {
  ...
  if (trimmed.length <= 0) {
    if (null == emojiInfo) {
      const CustomStatusSetting = require(3928) /* explicitContentFromProto */.CustomStatusSetting;
      return CustomStatusSetting.updateSetting(undefined);
    }
  }
  const CustomStatusSetting2 = require(3928) /* explicitContentFromProto */.CustomStatusSetting;
  let str = "";
  if (trimmed.length > 0) {
    str = trimmed;
  }
  ...
  if (null != clearAfter) {
    ...
    if (clearAfter !== ClearAfterValues.DONT_CLEAR) {
      ...
      const addResult = importDefault(3837)().add(importDefault(12084)(clearAfter), "ms");
      str2 = String(importDefault(3837)().add(importDefault(12084)(clearAfter), "ms").toDate().getTime());
      const toDateResult = importDefault(3837)().add(importDefault(12084)(clearAfter), "ms").toDate();
    }
  }
  obj[1] = str2;
  let str4 = "0";
  ...
  obj[3] = str5;
  if (createdAtMs == null) {
    const obj5 = importDefault(3837)();
    createdAtMs = importDefault(3837)().toDate().getTime();
    const toDateResult1 = importDefault(3837)().toDate();
  }
