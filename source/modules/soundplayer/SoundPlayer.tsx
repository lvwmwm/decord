...
// Function ID: 15988
// Name: MuteDeafen
// Dependencies: [19, 1371, 10591, 4317, 15988, 1376, 4322, 1218, 1372, 1891, 4351, 9752, 4375, 1960, 5137, 4319, 4376, 676, 10592, 21, 589, 9778, 4488, 4367, 3919, 1351, 5868, 2]
// Exports: default

// Module 15987 (MuteDeafen)
...
function MuteDeafen() {
  ...
  const f98799 = () => ({ inVoiceChannel: null != voiceChannelId.getVoiceChannelId(), selfMute: _detectH265HardwareDecode.isSelfMute(), selfDeaf: _detectH265HardwareDecode.isSelfDeaf(), audioPermissionReady: _detectH265HardwareDecode.isNativeAudioPermissionReady(), shouldSkipMuteUnmuteSound: _detectH265HardwareDecode.shouldSkipMuteUnmuteSound() });
  const f98800 = (selfDeaf) => {
    let inVoiceChannel;
    let selfMute;
  ...
  const effect = f98800.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f98817[20]).BatchedStoreListener(closure_0, () => {
      ...
      if (!isSoundDisabledResult) {
        items(f98817[21]).playSound(tmp2, 0.4);
        const obj = items(f98817[21]);
      }
      closure_0 = tmp;
    });
...
function Camera() {
  const items = [_detectH265HardwareDecode, handleConnectionOpen];
  const f98801 = () => ({ videoEnabled: videoEnabled.isVideoEnabled(), inVoiceChannel: null != voiceChannelId.getVoiceChannelId() });
  const f98802 = (videoEnabled, videoEnabled2) => {
...
function Speaking() {
  ...
  const f98805 = () => currentUserPTTActive.isCurrentUserPTTActive();
  const f98806 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function SelfMutedTemporarily() {
  ...
  const f98807 = () => _detectH265HardwareDecode.isSelfMutedTemporarily();
  const f98808 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function PriorityVAD() {
  const items = [anyoneHasFlagInContext];
  const f98809 = () => currentUserPrioritySpeaker.isCurrentUserPrioritySpeaker();
  const f98810 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (_detectH265HardwareDecode.getMode() === constants.VOICE_ACTIVITY) {
        if (!isSelfMuteResult) {
          let str = "ptt_stop";
...
function UserHasBeenMoved() {
  ...
  const f98811 = () => obj.userHasBeenMovedVersion;
  const f98812 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      return "user_moved";
    }
  };
  const effect = f98812.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f98817[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function UserInvitedToSpeak() {
  ...
  const f98813 = () => {
    ...
    if (null == voiceChannelId) {
      return items(f98813[22]).RequestToSpeakStates.NONE;
    } else {
      voiceStateForChannel = voiceStateForChannel.getVoiceStateForChannel(voiceChannelId);
      return items(f98813[22]).getAudienceRequestToSpeakState(voiceStateForChannel);
    }
  };
  const f98814 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (arg1 === items(f98813[22]).RequestToSpeakStates.REQUESTED_TO_SPEAK_AND_AWAITING_USER_ACK) {
        return "reconnect";
      }
    }
  };
  const effect = f98814.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f98817[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function VoiceChannel() {
  ...
  const f98815 = () => {
    ...
    if (null != first) {
      const encodeStreamKeyResult = currentUserId(f98815[23]).encodeStreamKey(first);
      ...
      const obj2 = currentUserId(f98815[23]);
    }
  ...
  const f98816 = (voiceChannelId) => {
    ...
    ({ singleActiveStreamKey, singleActiveStreamViewerCount, currentUserId: f98816 } = arg1);
...
function ActivitySounds() {
  ...
  const f98817 = () => {
    ...
    const embeddedActivityLocationChannelId = items(f98817[24]).getEmbeddedActivityLocationChannelId(connectedActivityLocation);
    id = id.getId();
    const obj2 = items(f98817[24]);
  ...
  const f98818 = (voiceChannelActivities) => {
