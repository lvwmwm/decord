...
// Function ID: 15997
// Name: MuteDeafen
// Dependencies: [19, 1371, 10592, 4317, 15997, 1376, 4322, 1218, 1372, 1891, 4351, 9753, 4375, 1960, 5137, 4319, 4376, 676, 10593, 21, 589, 9779, 4488, 4367, 3919, 1351, 5868, 2]
// Exports: default

// Module 15996 (MuteDeafen)
...
function MuteDeafen() {
  ...
  const f98834 = () => ({ inVoiceChannel: null != voiceChannelId.getVoiceChannelId(), selfMute: _detectH265HardwareDecode.isSelfMute(), selfDeaf: _detectH265HardwareDecode.isSelfDeaf(), audioPermissionReady: _detectH265HardwareDecode.isNativeAudioPermissionReady(), shouldSkipMuteUnmuteSound: _detectH265HardwareDecode.shouldSkipMuteUnmuteSound() });
  const f98835 = (selfDeaf) => {
    let inVoiceChannel;
    let selfMute;
  ...
  const effect = f98835.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f98852[20]).BatchedStoreListener(closure_0, () => {
      ...
      if (!isSoundDisabledResult) {
        items(f98852[21]).playSound(tmp2, 0.4);
        const obj = items(f98852[21]);
      }
      closure_0 = tmp;
    });
...
function Camera() {
  const items = [_detectH265HardwareDecode, handleConnectionOpen];
  const f98836 = () => ({ videoEnabled: videoEnabled.isVideoEnabled(), inVoiceChannel: null != voiceChannelId.getVoiceChannelId() });
  const f98837 = (videoEnabled, videoEnabled2) => {
...
function Speaking() {
  ...
  const f98840 = () => currentUserPTTActive.isCurrentUserPTTActive();
  const f98841 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function SelfMutedTemporarily() {
  ...
  const f98842 = () => _detectH265HardwareDecode.isSelfMutedTemporarily();
  const f98843 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function PriorityVAD() {
  const items = [anyoneHasFlagInContext];
  const f98844 = () => currentUserPrioritySpeaker.isCurrentUserPrioritySpeaker();
  const f98845 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (_detectH265HardwareDecode.getMode() === constants.VOICE_ACTIVITY) {
        if (!isSelfMuteResult) {
          let str = "ptt_stop";
...
function UserHasBeenMoved() {
  ...
  const f98846 = () => obj.userHasBeenMovedVersion;
  const f98847 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      return "user_moved";
    }
  };
  const effect = f98847.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f98852[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function UserInvitedToSpeak() {
  ...
  const f98848 = () => {
    ...
    if (null == voiceChannelId) {
      return items(f98848[22]).RequestToSpeakStates.NONE;
    } else {
      voiceStateForChannel = voiceStateForChannel.getVoiceStateForChannel(voiceChannelId);
      return items(f98848[22]).getAudienceRequestToSpeakState(voiceStateForChannel);
    }
  };
  const f98849 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (arg1 === items(f98848[22]).RequestToSpeakStates.REQUESTED_TO_SPEAK_AND_AWAITING_USER_ACK) {
        return "reconnect";
      }
    }
  };
  const effect = f98849.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f98852[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function VoiceChannel() {
  ...
  const f98850 = () => {
    ...
    if (null != first) {
      const encodeStreamKeyResult = currentUserId(f98850[23]).encodeStreamKey(first);
      ...
      const obj2 = currentUserId(f98850[23]);
    }
  ...
  const f98851 = (voiceChannelId) => {
    ...
    ({ singleActiveStreamKey, singleActiveStreamViewerCount, currentUserId: f98851 } = arg1);
...
function ActivitySounds() {
  ...
  const f98852 = () => {
    ...
    const embeddedActivityLocationChannelId = items(f98852[24]).getEmbeddedActivityLocationChannelId(connectedActivityLocation);
    id = id.getId();
    const obj2 = items(f98852[24]);
  ...
  const f98853 = (voiceChannelActivities) => {
