...
// Function ID: 16081
// Name: MuteDeafen
// Dependencies: [19, 1390, 10564, 4359, 16081, 1395, 4364, 1218, 1391, 1910, 4393, 9803, 4416, 1979, 5178, 4361, 4417, 676, 10565, 21, 589, 9829, 4529, 4408, 3979, 1370, 5909, 2]
// Exports: default

// Module 16080 (MuteDeafen)
...
function MuteDeafen() {
  ...
  const f99177 = () => ({ inVoiceChannel: null != voiceChannelId.getVoiceChannelId(), selfMute: _detectH265HardwareDecode.isSelfMute(), selfDeaf: _detectH265HardwareDecode.isSelfDeaf(), audioPermissionReady: _detectH265HardwareDecode.isNativeAudioPermissionReady(), shouldSkipMuteUnmuteSound: _detectH265HardwareDecode.shouldSkipMuteUnmuteSound() });
  const f99178 = (selfDeaf) => {
    let inVoiceChannel;
    let selfMute;
  ...
  const effect = f99178.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f99195[20]).BatchedStoreListener(closure_0, () => {
      ...
      if (!isSoundDisabledResult) {
        items(f99195[21]).playSound(tmp2, 0.4);
        const obj = items(f99195[21]);
      }
      closure_0 = tmp;
    });
...
function Camera() {
  const items = [_detectH265HardwareDecode, handleConnectionOpen];
  const f99179 = () => ({ videoEnabled: videoEnabled.isVideoEnabled(), inVoiceChannel: null != voiceChannelId.getVoiceChannelId() });
  const f99180 = (videoEnabled, videoEnabled2) => {
...
function Speaking() {
  ...
  const f99183 = () => currentUserPTTActive.isCurrentUserPTTActive();
  const f99184 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function SelfMutedTemporarily() {
  ...
  const f99185 = () => _detectH265HardwareDecode.isSelfMutedTemporarily();
  const f99186 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function PriorityVAD() {
  const items = [anyoneHasFlagInContext];
  const f99187 = () => currentUserPrioritySpeaker.isCurrentUserPrioritySpeaker();
  const f99188 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (_detectH265HardwareDecode.getMode() === constants.VOICE_ACTIVITY) {
        if (!isSelfMuteResult) {
          let str = "ptt_stop";
...
function UserHasBeenMoved() {
  ...
  const f99189 = () => obj.userHasBeenMovedVersion;
  const f99190 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      return "user_moved";
    }
  };
  const effect = f99190.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f99195[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function UserInvitedToSpeak() {
  ...
  const f99191 = () => {
    ...
    if (null == voiceChannelId) {
      return items(f99191[22]).RequestToSpeakStates.NONE;
    } else {
      voiceStateForChannel = voiceStateForChannel.getVoiceStateForChannel(voiceChannelId);
      return items(f99191[22]).getAudienceRequestToSpeakState(voiceStateForChannel);
    }
  };
  const f99192 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (arg1 === items(f99191[22]).RequestToSpeakStates.REQUESTED_TO_SPEAK_AND_AWAITING_USER_ACK) {
        return "reconnect";
      }
    }
  };
  const effect = f99192.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f99195[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function VoiceChannel() {
  ...
  const f99193 = () => {
    ...
    if (null != first) {
      const encodeStreamKeyResult = currentUserId(f99193[23]).encodeStreamKey(first);
      ...
      const obj2 = currentUserId(f99193[23]);
    }
  ...
  const f99194 = (voiceChannelId) => {
    ...
    ({ singleActiveStreamKey, singleActiveStreamViewerCount, currentUserId: f99194 } = arg1);
...
function ActivitySounds() {
  ...
  const f99195 = () => {
    ...
    const embeddedActivityLocationChannelId = items(f99195[24]).getEmbeddedActivityLocationChannelId(connectedActivityLocation);
    id = id.getId();
    const obj2 = items(f99195[24]);
  ...
  const f99196 = (voiceChannelActivities) => {
