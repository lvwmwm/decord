...
// Function ID: 15773
// Name: MuteDeafen
// Dependencies: [19, 1371, 5690, 4207, 15773, 1376, 4212, 1218, 1372, 1862, 4240, 10259, 4265, 1931, 5013, 4209, 4266, 676, 1379, 21, 589, 10285, 4378, 4257, 3811, 1351, 5686, 2]
// Exports: default
...
function MuteDeafen() {
  ...
  const f97867 = () => ({ inVoiceChannel: null != voiceChannelId.getVoiceChannelId(), selfMute: _detectH265HardwareDecode.isSelfMute(), selfDeaf: _detectH265HardwareDecode.isSelfDeaf(), audioPermissionReady: _detectH265HardwareDecode.isNativeAudioPermissionReady(), shouldSkipMuteUnmuteSound: _detectH265HardwareDecode.shouldSkipMuteUnmuteSound() });
  const f97868 = (selfDeaf) => {
    let inVoiceChannel;
    let selfMute;
  ...
  const effect = f97868.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f97885[20]).BatchedStoreListener(closure_0, () => {
      ...
      if (!isSoundDisabledResult) {
        items(f97885[21]).playSound(tmp2, 0.4);
        const obj = items(f97885[21]);
      }
      closure_0 = tmp;
    });
...
function Camera() {
  const items = [_detectH265HardwareDecode, handleConnectionOpen];
  const f97869 = () => ({ videoEnabled: videoEnabled.isVideoEnabled(), inVoiceChannel: null != voiceChannelId.getVoiceChannelId() });
  const f97870 = (videoEnabled, videoEnabled2) => {
...
function Speaking() {
  ...
  const f97873 = () => currentUserPTTActive.isCurrentUserPTTActive();
  const f97874 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function SelfMutedTemporarily() {
  ...
  const f97875 = () => _detectH265HardwareDecode.isSelfMutedTemporarily();
  const f97876 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function PriorityVAD() {
  const items = [anyoneHasFlagInContext];
  const f97877 = () => currentUserPrioritySpeaker.isCurrentUserPrioritySpeaker();
  const f97878 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (_detectH265HardwareDecode.getMode() === constants.VOICE_ACTIVITY) {
        if (!isSelfMuteResult) {
          let str = "ptt_stop";
...
function UserHasBeenMoved() {
  ...
  const f97879 = () => obj.userHasBeenMovedVersion;
  const f97880 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      return "user_moved";
    }
  };
  const effect = f97880.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f97885[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function UserInvitedToSpeak() {
  ...
  const f97881 = () => {
    ...
    if (null == voiceChannelId) {
      return items(f97881[22]).RequestToSpeakStates.NONE;
    } else {
      voiceStateForChannel = voiceStateForChannel.getVoiceStateForChannel(voiceChannelId);
      return items(f97881[22]).getAudienceRequestToSpeakState(voiceStateForChannel);
    }
  };
  const f97882 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (arg1 === items(f97881[22]).RequestToSpeakStates.REQUESTED_TO_SPEAK_AND_AWAITING_USER_ACK) {
        return "reconnect";
      }
    }
  };
  const effect = f97882.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f97885[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function VoiceChannel() {
  ...
  const f97883 = () => {
    ...
    if (null != first) {
      const encodeStreamKeyResult = currentUserId(f97883[23]).encodeStreamKey(first);
      ...
      const obj2 = currentUserId(f97883[23]);
    }
  ...
  const f97884 = (voiceChannelId) => {
    ...
    ({ singleActiveStreamKey, singleActiveStreamViewerCount, currentUserId: f97884 } = arg1);
...
function ActivitySounds() {
  ...
  const f97885 = () => {
    ...
    const embeddedActivityLocationChannelId = items(f97885[24]).getEmbeddedActivityLocationChannelId(connectedActivityLocation);
    id = id.getId();
    const obj2 = items(f97885[24]);
  ...
  const f97886 = (voiceChannelActivities) => {
