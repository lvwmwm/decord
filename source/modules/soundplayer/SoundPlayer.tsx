...
// Function ID: 16071
// Name: MuteDeafen
// Dependencies: [19, 1390, 10650, 4358, 16071, 1395, 4363, 1218, 1391, 1910, 4392, 9799, 4415, 1979, 5177, 4360, 4416, 676, 10651, 21, 589, 9825, 4528, 4407, 3979, 1370, 5907, 2]
// Exports: default

// Module 16070 (MuteDeafen)
...
function MuteDeafen() {
  ...
  const f99133 = () => ({ inVoiceChannel: null != voiceChannelId.getVoiceChannelId(), selfMute: _detectH265HardwareDecode.isSelfMute(), selfDeaf: _detectH265HardwareDecode.isSelfDeaf(), audioPermissionReady: _detectH265HardwareDecode.isNativeAudioPermissionReady(), shouldSkipMuteUnmuteSound: _detectH265HardwareDecode.shouldSkipMuteUnmuteSound() });
  const f99134 = (selfDeaf) => {
    let inVoiceChannel;
    let selfMute;
  ...
  const effect = f99134.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f99151[20]).BatchedStoreListener(closure_0, () => {
      ...
      if (!isSoundDisabledResult) {
        items(f99151[21]).playSound(tmp2, 0.4);
        const obj = items(f99151[21]);
      }
      closure_0 = tmp;
    });
...
function Camera() {
  const items = [_detectH265HardwareDecode, handleConnectionOpen];
  const f99135 = () => ({ videoEnabled: videoEnabled.isVideoEnabled(), inVoiceChannel: null != voiceChannelId.getVoiceChannelId() });
  const f99136 = (videoEnabled, videoEnabled2) => {
...
function Speaking() {
  ...
  const f99139 = () => currentUserPTTActive.isCurrentUserPTTActive();
  const f99140 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function SelfMutedTemporarily() {
  ...
  const f99141 = () => _detectH265HardwareDecode.isSelfMutedTemporarily();
  const f99142 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function PriorityVAD() {
  const items = [anyoneHasFlagInContext];
  const f99143 = () => currentUserPrioritySpeaker.isCurrentUserPrioritySpeaker();
  const f99144 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (_detectH265HardwareDecode.getMode() === constants.VOICE_ACTIVITY) {
        if (!isSelfMuteResult) {
          let str = "ptt_stop";
...
function UserHasBeenMoved() {
  ...
  const f99145 = () => obj.userHasBeenMovedVersion;
  const f99146 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      return "user_moved";
    }
  };
  const effect = f99146.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f99151[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function UserInvitedToSpeak() {
  ...
  const f99147 = () => {
    ...
    if (null == voiceChannelId) {
      return items(f99147[22]).RequestToSpeakStates.NONE;
    } else {
      voiceStateForChannel = voiceStateForChannel.getVoiceStateForChannel(voiceChannelId);
      return items(f99147[22]).getAudienceRequestToSpeakState(voiceStateForChannel);
    }
  };
  const f99148 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (arg1 === items(f99147[22]).RequestToSpeakStates.REQUESTED_TO_SPEAK_AND_AWAITING_USER_ACK) {
        return "reconnect";
      }
    }
  };
  const effect = f99148.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f99151[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function VoiceChannel() {
  ...
  const f99149 = () => {
    ...
    if (null != first) {
      const encodeStreamKeyResult = currentUserId(f99149[23]).encodeStreamKey(first);
      ...
      const obj2 = currentUserId(f99149[23]);
    }
  ...
  const f99150 = (voiceChannelId) => {
    ...
    ({ singleActiveStreamKey, singleActiveStreamViewerCount, currentUserId: f99150 } = arg1);
...
function ActivitySounds() {
  ...
  const f99151 = () => {
    ...
    const embeddedActivityLocationChannelId = items(f99151[24]).getEmbeddedActivityLocationChannelId(connectedActivityLocation);
    id = id.getId();
    const obj2 = items(f99151[24]);
  ...
  const f99152 = (voiceChannelActivities) => {
