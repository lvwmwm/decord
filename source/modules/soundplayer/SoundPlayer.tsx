...
// Function ID: 16090
// Name: MuteDeafen
// Dependencies: [19, 1390, 10582, 4359, 16090, 1395, 4364, 1218, 1391, 1910, 4393, 9814, 4416, 1979, 5199, 4361, 4417, 676, 10583, 21, 589, 9840, 4529, 4408, 3979, 1370, 5930, 2]
// Exports: default

// Module 16089 (MuteDeafen)
...
function MuteDeafen() {
  ...
  const f99212 = () => ({ inVoiceChannel: null != voiceChannelId.getVoiceChannelId(), selfMute: _detectH265HardwareDecode.isSelfMute(), selfDeaf: _detectH265HardwareDecode.isSelfDeaf(), audioPermissionReady: _detectH265HardwareDecode.isNativeAudioPermissionReady(), shouldSkipMuteUnmuteSound: _detectH265HardwareDecode.shouldSkipMuteUnmuteSound() });
  const f99213 = (selfDeaf) => {
    let inVoiceChannel;
    let selfMute;
  ...
  const effect = f99213.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f99230[20]).BatchedStoreListener(closure_0, () => {
      ...
      if (!isSoundDisabledResult) {
        items(f99230[21]).playSound(tmp2, 0.4);
        const obj = items(f99230[21]);
      }
      closure_0 = tmp;
    });
...
function Camera() {
  const items = [_detectH265HardwareDecode, handleConnectionOpen];
  const f99214 = () => ({ videoEnabled: videoEnabled.isVideoEnabled(), inVoiceChannel: null != voiceChannelId.getVoiceChannelId() });
  const f99215 = (videoEnabled, videoEnabled2) => {
...
function Speaking() {
  ...
  const f99218 = () => currentUserPTTActive.isCurrentUserPTTActive();
  const f99219 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function SelfMutedTemporarily() {
  ...
  const f99220 = () => _detectH265HardwareDecode.isSelfMutedTemporarily();
  const f99221 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function PriorityVAD() {
  const items = [anyoneHasFlagInContext];
  const f99222 = () => currentUserPrioritySpeaker.isCurrentUserPrioritySpeaker();
  const f99223 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (_detectH265HardwareDecode.getMode() === constants.VOICE_ACTIVITY) {
        if (!isSelfMuteResult) {
          let str = "ptt_stop";
...
function UserHasBeenMoved() {
  ...
  const f99224 = () => obj.userHasBeenMovedVersion;
  const f99225 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      return "user_moved";
    }
  };
  const effect = f99225.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f99230[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function UserInvitedToSpeak() {
  ...
  const f99226 = () => {
    ...
    if (null == voiceChannelId) {
      return items(f99226[22]).RequestToSpeakStates.NONE;
    } else {
      voiceStateForChannel = voiceStateForChannel.getVoiceStateForChannel(voiceChannelId);
      return items(f99226[22]).getAudienceRequestToSpeakState(voiceStateForChannel);
    }
  };
  const f99227 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (arg1 === items(f99226[22]).RequestToSpeakStates.REQUESTED_TO_SPEAK_AND_AWAITING_USER_ACK) {
        return "reconnect";
      }
    }
  };
  const effect = f99227.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f99230[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function VoiceChannel() {
  ...
  const f99228 = () => {
    ...
    if (null != first) {
      const encodeStreamKeyResult = currentUserId(f99228[23]).encodeStreamKey(first);
      ...
      const obj2 = currentUserId(f99228[23]);
    }
  ...
  const f99229 = (voiceChannelId) => {
    ...
    ({ singleActiveStreamKey, singleActiveStreamViewerCount, currentUserId: f99229 } = arg1);
...
function ActivitySounds() {
  ...
  const f99230 = () => {
    ...
    const embeddedActivityLocationChannelId = items(f99230[24]).getEmbeddedActivityLocationChannelId(connectedActivityLocation);
    id = id.getId();
    const obj2 = items(f99230[24]);
  ...
  const f99231 = (voiceChannelActivities) => {
