...
// Function ID: 16014
// Name: MuteDeafen
// Dependencies: [19, 1390, 10597, 4317, 16014, 1395, 4322, 1218, 1391, 1910, 4351, 9758, 4375, 1979, 5137, 4319, 4376, 676, 10598, 21, 589, 9784, 4488, 4367, 3938, 1370, 5868, 2]
// Exports: default

// Module 16013 (MuteDeafen)
...
function MuteDeafen() {
  ...
  const f98916 = () => ({ inVoiceChannel: null != voiceChannelId.getVoiceChannelId(), selfMute: _detectH265HardwareDecode.isSelfMute(), selfDeaf: _detectH265HardwareDecode.isSelfDeaf(), audioPermissionReady: _detectH265HardwareDecode.isNativeAudioPermissionReady(), shouldSkipMuteUnmuteSound: _detectH265HardwareDecode.shouldSkipMuteUnmuteSound() });
  const f98917 = (selfDeaf) => {
    let inVoiceChannel;
    let selfMute;
  ...
  const effect = f98917.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f98934[20]).BatchedStoreListener(closure_0, () => {
      ...
      if (!isSoundDisabledResult) {
        items(f98934[21]).playSound(tmp2, 0.4);
        const obj = items(f98934[21]);
      }
      closure_0 = tmp;
    });
...
function Camera() {
  const items = [_detectH265HardwareDecode, handleConnectionOpen];
  const f98918 = () => ({ videoEnabled: videoEnabled.isVideoEnabled(), inVoiceChannel: null != voiceChannelId.getVoiceChannelId() });
  const f98919 = (videoEnabled, videoEnabled2) => {
...
function Speaking() {
  ...
  const f98922 = () => currentUserPTTActive.isCurrentUserPTTActive();
  const f98923 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function SelfMutedTemporarily() {
  ...
  const f98924 = () => _detectH265HardwareDecode.isSelfMutedTemporarily();
  const f98925 = (arg0, arg1) => {
    if (arg0 !== arg1) {
...
function PriorityVAD() {
  const items = [anyoneHasFlagInContext];
  const f98926 = () => currentUserPrioritySpeaker.isCurrentUserPrioritySpeaker();
  const f98927 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (_detectH265HardwareDecode.getMode() === constants.VOICE_ACTIVITY) {
        if (!isSelfMuteResult) {
          let str = "ptt_stop";
...
function UserHasBeenMoved() {
  ...
  const f98928 = () => obj.userHasBeenMovedVersion;
  const f98929 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      return "user_moved";
    }
  };
  const effect = f98929.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f98934[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function UserInvitedToSpeak() {
  ...
  const f98930 = () => {
    ...
    if (null == voiceChannelId) {
      return items(f98930[22]).RequestToSpeakStates.NONE;
    } else {
      voiceStateForChannel = voiceStateForChannel.getVoiceStateForChannel(voiceChannelId);
      return items(f98930[22]).getAudienceRequestToSpeakState(voiceStateForChannel);
    }
  };
  const f98931 = (arg0, arg1) => {
    if (arg0 !== arg1) {
      if (arg1 === items(f98930[22]).RequestToSpeakStates.REQUESTED_TO_SPEAK_AND_AWAITING_USER_ACK) {
        return "reconnect";
      }
    }
  };
  const effect = f98931.useEffect(() => {
    let closure_0 = batchedStoreListener();
    batchedStoreListener = new items(f98934[20]).BatchedStoreListener(closure_0, () => {
      const tmp = batchedStoreListener();
      const tmp2 = outer1_2(closure_0, tmp);
...
function VoiceChannel() {
  ...
  const f98932 = () => {
    ...
    if (null != first) {
      const encodeStreamKeyResult = currentUserId(f98932[23]).encodeStreamKey(first);
      ...
      const obj2 = currentUserId(f98932[23]);
    }
  ...
  const f98933 = (voiceChannelId) => {
    ...
    ({ singleActiveStreamKey, singleActiveStreamViewerCount, currentUserId: f98933 } = arg1);
...
function ActivitySounds() {
  ...
  const f98934 = () => {
    ...
    const embeddedActivityLocationChannelId = items(f98934[24]).getEmbeddedActivityLocationChannelId(connectedActivityLocation);
    id = id.getId();
    const obj2 = items(f98934[24]);
  ...
  const f98935 = (voiceChannelActivities) => {
