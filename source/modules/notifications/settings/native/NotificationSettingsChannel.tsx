...
// Dependencies: [19, 17, 4667, 21, 4446, 712, 10170, 1236, 4640, 1500, 5460, 5452, 5447, 589, 8309, 10172, 10173, 10179, 10186, 10192, 4893, 2]
// Exports: default

// Module 10169 (NotificationSettingsChannel)
...
export default function NotificationSettingsChannel(channel) {
  ...
  const layoutEffect = React.useLayoutEffect(() => {
    ...
    if (channel.inGuildContext) {
      obj = { headerLeft: null };
      obj[0] = channel(5460).getHeaderBackButton(() => closure_3.popToTop());
      obj.setOptions(obj);
      const obj4 = channel(5460);
    }
  });
  const items = [channel.channel];
  const items1 = [channel.channel];
  const callback = React.useCallback(() => channel(10170).updateChannelToGuildDefault(channel.channel.guild_id, channel.channel.id), items);
  const callback1 = React.useCallback(() => {
    const result = callback(5452).updateChannelOverrideSettings(channel.channel.guild_id, channel.channel.id, { muted: false }, channel(5447).NotificationLabels.Unmuted);
  }, items1);
  ...
  if (muted) {
    ...
    obj[2] = tmp(10172).getMuteBannerSubtitleFromConfig(stateFromStoresObject.config);
    obj[3] = callback1;
    muted = callback(tmp(10172).NotificationSettingsMuteBanner, obj);
    const tmpResult = tmp(10172);
  }
  const items3 = [muted, , , , , ];
  obj1 = { channel: channel.channel };
  items3[1] = callback(_require(10173).NotificationSettingsChannelPresets, obj1);
  obj2 = { style: { marginTop: 24 }, channel: channel.channel };
  items3[2] = callback(_require(10179).NotificationSettingsChannelMessageNotification, obj2);
  items3[3] = callback(_require(10186).NotificationSettingsChannelMessageUnread, { style: { marginTop: 24 }, channel: channel.channel });
  ...
  if (isForumLikeChannelResult) {
    ...
    isForumLikeChannelResult = tmp11(tmp(10192).NotificationSettingsChannelPost, obj4);
  }
  ...
  if (!inherited) {
    ...
    obj5[1] = tmp11(tmp(4893).Button, obj6);
    tmp11Result = tmp11(View, obj5);
  }
  items3[5] = tmp11Result;
  obj[1] = items3;
  return closure_7(_require(8309).Form, obj);
};
