...
// Dependencies: [19, 17, 4450, 21, 4255, 712, 9712, 1236, 4445, 1480, 5223, 5215, 5210, 589, 7749, 9714, 9715, 9719, 9726, 9732, 4665, 2]
// Exports: default

// Module 9711 (NotificationSettingsChannel)
...
export default function NotificationSettingsChannel(channel) {
  ...
  const dependencyMap = importDefault(4445)(channel.channel);
  ...
  const layoutEffect = React.useLayoutEffect(() => {
    ...
    if (channel.inGuildContext) {
      obj = { headerLeft: null };
      obj[0] = channel(5223).getHeaderBackButton(() => NotificationSettingsChannelPost.popToTop());
      obj.setOptions(obj);
      const obj4 = channel(5223);
    }
  });
  const items = [channel.channel];
  const items1 = [channel.channel];
  const callback = React.useCallback(() => channel(9712).updateChannelToGuildDefault(channel.channel.guild_id, channel.channel.id), items);
  const callback1 = React.useCallback(() => {
    const result = callback(5215).updateChannelOverrideSettings(channel.channel.guild_id, channel.channel.id, { muted: false }, channel(5210).NotificationLabels.Unmuted);
  }, items1);
  ...
  if (muted) {
    ...
    obj[2] = tmp(9714).getMuteBannerSubtitleFromConfig(stateFromStoresObject.config);
    obj[3] = callback1;
    muted = callback(tmp(9714).NotificationSettingsMuteBanner, obj);
    const tmpResult = tmp(9714);
  }
  const items3 = [muted, , , , , ];
  obj1 = { channel: channel.channel };
  items3[1] = callback(_require(9715).NotificationSettingsChannelPresets, obj1);
  obj2 = { style: { marginTop: 24 }, channel: channel.channel };
  items3[2] = callback(_require(9719).NotificationSettingsChannelMessageNotification, obj2);
  items3[3] = callback(_require(9726).NotificationSettingsChannelMessageUnread, { style: { marginTop: 24 }, channel: channel.channel });
  ...
  if (isForumLikeChannelResult) {
    ...
    isForumLikeChannelResult = tmp11(tmp(9732).NotificationSettingsChannelPost, obj4);
  }
  ...
  if (!inherited) {
    ...
    obj5[1] = tmp11(tmp(4665).Button, obj6);
    tmp11Result = tmp11(View, obj5);
  }
  items3[5] = tmp11Result;
  obj[1] = items3;
  return closure_7(_require(7749).Form, obj);
};
