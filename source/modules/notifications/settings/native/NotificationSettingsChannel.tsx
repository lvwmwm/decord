...
// Dependencies: [19, 17, 4701, 21, 4478, 709, 10254, 1233, 4674, 1498, 5503, 5495, 5490, 586, 8372, 10256, 10257, 10263, 10270, 10276, 4928, 2]
// Exports: default

// Module 10253 (NotificationSettingsChannel)
...
export default function NotificationSettingsChannel(channel) {
  ...
  const callback = React.useCallback(() => channel(10254).updateChannelToGuildDefault(channel.channel.guild_id, channel.channel.id), items);
  const callback1 = React.useCallback(() => {
    const result = callback(5495).updateChannelOverrideSettings(channel.channel.guild_id, channel.channel.id, { muted: false }, channel(5490).NotificationLabels.Unmuted);
  }, items1);
  ...
  items3[1] = callback(_require(10257).NotificationSettingsChannelPresets, obj1);
  obj2 = { style: { marginTop: 24 }, channel: channel.channel };
  items3[2] = callback(_require(10263).NotificationSettingsChannelMessageNotification, obj2);
  items3[3] = callback(_require(10270).NotificationSettingsChannelMessageUnread, { style: { marginTop: 24 }, channel: channel.channel });
  ...
  obj[1] = items3;
  return closure_7(_require(8372).Form, obj);
};
