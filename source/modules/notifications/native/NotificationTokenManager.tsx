...
// Function ID: 13719
// Name: _initialize
// Dependencies: [17, 1212, 11782, 12983, 1218, 13719, 676, 4482, 709, 10774, 13722, 13723, 1208, 1236, 2578, 500, 13724, 698, 11781, 2]

// Module 13718 (_initialize)
...
prototype["registerListener"] = function registerListener() {
  ...
  if (this.hasRegisterEventListener) {
    ...
    const result = importDefault(10774).addRegisterEventListener(self.handleToken);
  }
};
prototype["registerNotificationCategories"] = function registerNotificationCategories(arg0) {
  const declarativeNotifSettingsExperiment = require(13722) /* apexExperiment */.declarativeNotifSettingsExperiment;
  const config = declarativeNotifSettingsExperiment.getConfig({ location: "registerNotificationCategories" });
  let flag = false;
  try {
    if (config.enabled) {
      flag = importDefault(13723).registerDeclarativeNotificationCategories();
      const obj = importDefault(13723);
    }
    const self = this;
    if (flag) {
      ...
      try {
        if (config.clearDeclarative) {
          ...
          importDefault(13723).clear();
          const obj4 = importDefault(13723);
        }
...
prototype["trackDisabledAndroidNotifChannels"] = function trackDisabledAndroidNotifChannels() {
  if (!this.hasTrackedDisabledAndroidNotifChannels) {
    ...
    let obj = set(500);
    if (obj.isAndroid()) {
      const tmp5 = importDefault(13724);
      let prop;
      if (tmp5 != null) {
