...
// Dependencies: [19, 17, 4363, 676, 4324, 7256, 4394, 9194, 4523, 4407, 9201, 9215, 10884, 10879, 1624, 10895, 10896, 10897, 10898, 10890, 589, 5767, 1236, 2]
// Exports: default, getOSRequirement, getStreamPressHandler, handleCloseScreenshare, tryStartScreenShare

// Module 10883 (stopScreenshare)
...
function stopScreenshare() {
  ...
  importDefault(9201).setGoLiveSource(null);
}
function startStream() {
  if ("android" === obj.getVoiceEngine().platform) {
    const result = importDefault(7256).isForegroundServiceRunning((arg0) => {
      if (arg0) {
...
export default function useScreenshareUtils(arg0) {
  const _require = arg0;
  let tmp = importDefault(10879)(arg0);
  importDefault = tmp;
  const dependencyMap = tmp2;
  const showMobileGoLiveUpsell = importDefault(10890).useConfig({ location: "useScreenshareUtils" }).showMobileGoLiveUpsell;
...
export const handleCloseScreenshare = function handleCloseScreenshare() {
  ...
  if (null != currentUserActiveStream) {
    ...
    obj.stopStream(require(4407) /* isStreamKey */.encodeStreamKey(currentUserActiveStream));
...
export const getStreamPressHandler = function getStreamPressHandler(analyticsLocations) {
  ...
  if (osRequirement) {
    if (hasPermission) {
      ...
      const S = require(9194) /* _handleToggleVideo */.showScreenshareDisabledAlert;
    }
  } else {
    class S {
      constructor() {
...
export const tryStartScreenShare = function tryStartScreenShare(channel) {
  ...
  if (videoPermission) {
    videoPermission = require(10879) /* useHasVideoPermission */.getVideoPermission(channel);
    const obj = require(10879) /* useHasVideoPermission */;
  }
  if (videoPermission) {
    if ("android" === obj2.getVoiceEngine().platform) {
      let result = importDefault(7256).isForegroundServiceRunning((arg0) => {
        if (arg0) {
          let tmpResult = tmp(tmp2[6]);
          const voiceEngine = tmpResult.getVoiceEngine();
          voiceEngine.startBroadcast();
      ...
      const obj3 = importDefault(7256);
    } else {
      BroadcastUploadManager.showPicker();
    }
    obj2 = require(4394) /* inject */;
  }
};
