...
// Dependencies: [5, 32, 19, 17, 10756, 9740, 676, 10621, 21, 4303, 1474, 9748, 4036, 10757, 9738, 9743, 4517, 4509, 10625, 1988, 1231, 10815, 10958, 4154, 10771, 500, 4013, 4184, 5783, 2]
// Exports: default

// Module 10755 (CallWithVoiceChat)
...
function CallWithVoiceChat(channel) {
  ...
  obj = { interpolate: tmp5(4036).interpolate, translateX, width: result };
  fn2.__closure = obj;
  ...
  obj = { runOnJS: tmp5(4036).runOnJS, setShouldRenderChat: tmp12 };
  fn4.__closure = obj;
  fn4.__workletHash = 661145094859;
  fn4.__initData = closure_18;
  const animatedReaction = _require(4036).useAnimatedReaction(fn3, fn4);
  ...
  if (channel.isGuildStageVoice()) {
    ...
    obj3[0] = channel;
    tmp16 = callback2(tmp2(10757), obj3);
  }
  const items1 = [tmp16, ];
  const items2 = [animatedStyle1, StyleSheet.absoluteFill];
  items1[1] = callback2(translateX(4036).View, { pointerEvents: "box-none", style: items2 });
  obj2[1] = items1;
  ...
  if (tmp11[0]) {
    ...
    obj5[0] = channel;
    tmp18Result = tmp18(tmp2(9738), obj5);
  }
  obj4[1] = tmp18Result;
  items3[1] = callback2(closure_6, obj4);
  obj1[1] = items3;
  return closure_13(translateX(4036).View, obj1);
}
function MainCallScreen(channel) {
  channel = channel.channel;
  let isConnectedToVoiceChannel;
  let id;
  let obj = isConnectedToVoiceChannel(9743);
  ...
  let obj1 = isConnectedToVoiceChannel(4517);
  ...
  if (isChannelContentGated) {
    ...
    if (channel.isGuildStageVoice()) {
      ...
      obj2[0] = channel;
      tmp14Result = tmp14(id(10757), obj2);
    }
    obj1[1] = tmp14Result;
...
export default function ChannelCallNavigator(channel) {
  ...
  obj = { value: guild_id, children: tmp2(channel(5783).Navigator, { screens: { [ChannelCallScreens.MAIN_CALL_SCREEN]: obj }, initialRouteName: ChannelCallScreens.MAIN_CALL_SCREEN }) };
  obj = {
    headerShown: false,
    ignoreKeyboard: true,
  ...
  return closure_12(importDefault(4184).Provider, obj);
};
