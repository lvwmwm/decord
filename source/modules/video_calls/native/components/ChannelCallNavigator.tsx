...
// Dependencies: [5, 32, 19, 17, 10689, 9674, 676, 10554, 21, 4302, 1474, 9681, 4162, 10690, 9672, 9677, 4515, 4507, 10558, 1988, 1231, 10748, 10891, 4148, 10704, 500, 4013, 4183, 5723, 2]
// Exports: default

// Module 10688 (CallWithVoiceChat)
...
function CallWithVoiceChat(channel) {
  ...
  obj = { interpolate: tmp5(4162).interpolate, translateX, width: result };
  fn2.__closure = obj;
  ...
  obj = { runOnJS: tmp5(4162).runOnJS, setShouldRenderChat: tmp12 };
  fn4.__closure = obj;
  fn4.__workletHash = 661145094859;
  fn4.__initData = closure_18;
  const animatedReaction = _require(4162).useAnimatedReaction(fn3, fn4);
  ...
  if (channel.isGuildStageVoice()) {
    ...
    obj3[0] = channel;
    tmp16 = callback2(tmp2(10690), obj3);
  }
  const items1 = [tmp16, ];
  const items2 = [animatedStyle1, StyleSheet.absoluteFill];
  items1[1] = callback2(translateX(4162).View, { pointerEvents: "box-none", style: items2 });
  obj2[1] = items1;
  ...
  if (tmp11[0]) {
    ...
    obj5[0] = channel;
    tmp18Result = tmp18(tmp2(9672), obj5);
  }
  obj4[1] = tmp18Result;
  items3[1] = callback2(closure_6, obj4);
  obj1[1] = items3;
  return closure_13(translateX(4162).View, obj1);
}
function MainCallScreen(channel) {
  channel = channel.channel;
  let isConnectedToVoiceChannel;
  let id;
  let obj = isConnectedToVoiceChannel(9677);
  ...
  let obj1 = isConnectedToVoiceChannel(4515);
  ...
  if (isChannelContentGated) {
    ...
    if (channel.isGuildStageVoice()) {
      ...
      obj2[0] = channel;
      tmp14Result = tmp14(id(10690), obj2);
    }
    obj1[1] = tmp14Result;
  ...
  return tmp11(tmp22, { forceHide: isConnectedToVoiceChannel(500).isAndroid() || !isConnectedToVoiceChannel, showWhenParticipantOnScreen: !isConnectedToVoiceChannel, children: tmp11(isConnectedToVoiceChannel(4013).ThemeContextProvider, { gradient: tmp21, children: tmp14Result }) });
}
...
export default function ChannelCallNavigator(channel) {
  ...
  obj = { value: guild_id, children: tmp2(channel(5723).Navigator, { screens: { [ChannelCallScreens.MAIN_CALL_SCREEN]: obj }, initialRouteName: ChannelCallScreens.MAIN_CALL_SCREEN }) };
  obj = {
    headerShown: false,
    ignoreKeyboard: true,
  ...
  return closure_12(importDefault(4183).Provider, obj);
};
