...
// Dependencies: [19, 1415, 1874, 676, 12617, 647, 16545, 16546, 16547, 12610, 1236, 1945, 16548, 2]
// Exports: default

// Module 16544 (useOnboardingMonetizationEnableFlow)
...
export default function useOnboardingMonetizationEnableFlow(features) {
  ...
  const tmp10 = refresh(16545);
  ({ submittedRequest, error, loading, createEnableRequest } = refresh(16545)(id));
  let id1;
  let tmp9Result = tmp9(16546);
  if (features != null) {
    id1 = features.id;
  }
  ...
  const tmp10Result = refresh(16545)(id);
  ({ isApplicationRejected, requestCooldownDuration } = refresh(16547)(eligibility));
  let hasItem2;
  if (features != null) {
  ...
  const tmp16 = refresh(16547)(eligibility);
  ...
  let formatResult3;
  const creatorMonetizationAcceptTermsCheckboxText = _require(16548).getCreatorMonetizationAcceptTermsCheckboxText();
  if (isApplicationRejected) {
    if (tmp35) {
      if (stateFromStores) {
