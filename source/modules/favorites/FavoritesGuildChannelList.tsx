...
// Dependencies: [32, 19, 1386, 5221, 7271, 5448, 4123, 1391, 5485, 1387, 7272, 4121, 4493, 1981, 4701, 1390, 4120, 1425, 7279, 505, 7273, 1306, 6159, 10307, 12, 1471, 2]
// Exports: useFavoritesGuildChannelList

// Module 15863 (getMissingFavoriteThreadIds)
import create from "create" /* 1306 */;
import initializeDefault from "initialize" /* 6159 */;
import closure_3 from "_slicedToArray" /* 32 */;
import closure_4 from "noop" /* 19 */;
import importDefaultResult1 from "_handleConnectionOpen" /* 5221 */;
import importDefaultResult from "participantFromServer" /* 1386 */;
import importDefaultResult3 from "rebuild" /* 5448 */;
import importDefaultResult4 from "storeThread" /* 4123 */;
import createChannelRecord from "createChannelRecord" /* 1391 */;
import importDefaultResult5 from "incrementVersion" /* 5485 */;
import importDefaultResult6 from "ensureGuildLoaded" /* 1387 */;
import closure_13 from "handleConnectionOpen" /* 7272 */;
import importDefaultResult7 from "getUncachedChannelPermissions" /* 4121 */;
...
import importDefaultResult11 from "initializeFromUserSettings" /* 1390 */;
import { createFavoritesGuildChannelRecord as closure_19 } from "createFavoritesGuildChannelRecord" /* 4120 */;
import importDefaultResult2 from "scheduledEventSort" /* 7271 */;
import { ChannelListChannelNoticeRow as closure_22 } from "ChannelListGuildActionRow" /* 7279 */;
...
function computeFavoritesState(favoriteChannels) {
  ...
  Object.defineProperty(obj, "channelList", {
    get: function() {
      if (null == closure_6) {
        ...
        const found = mapped.filter(favoriteChannels(1471).isNotNullish);
  ...
  const favoritesCategories = favoriteChannels(10307).getFavoritesCategories(favoriteChannels);
  ...
  return {
    ...
    getSections() {
      let length;
      items = [];
      items[favoriteChannels(7273).SECTION_INDEX_CHANNEL_NOTICES] = items2.length;
      let num = 0;
      items[favoriteChannels(7273).SECTION_INDEX_GUILD_ACTIONS] = 0;
      items[favoriteChannels(7273).SECTION_INDEX_FAVORITES] = 0;
      items[favoriteChannels(7273).SECTION_INDEX_RECENTS] = 0;
      items[favoriteChannels(7273).SECTION_INDEX_UNCATEGORIZED_CHANNELS] = obj.channelList.length;
      if (0 < found1.length) {
        do {
          ...
          let _Math = Math;
          let tmp4 = found1;
          let sum = favoriteChannels(7273).SECTION_INDEX_FIRST_NAMED_CATEGORY + num;
    ...
    isPlaceholderRow(arg0, arg1) {
      let tmp3 = arg0 < favoriteChannels(7273).SECTION_INDEX_FIRST_NAMED_CATEGORY;
      if (!tmp3) {
        tmp3 = 0 !== arg1;
      }
      let tmp5 = !tmp3;
      if (!tmp3) {
        tmp5 = 0 === found1[arg0 - tmp(undefined, 7273).SECTION_INDEX_FIRST_NAMED_CATEGORY].channelList.length;
      }
      return tmp5;
    },
    getCategoryFromSection(arg0) {
      if (arg0 === favoriteChannels(7273).SECTION_INDEX_UNCATEGORIZED_CHANNELS) {
        let tmp4 = obj;
      } else {
        tmp4 = found1[arg0 - tmp(undefined, 7273).SECTION_INDEX_FIRST_NAMED_CATEGORY];
      }
      return tmp4;
    },
    getNamedCategoryFromSection(arg0) {
      return found1[arg0 - favoriteChannels(undefined, 7273).SECTION_INDEX_FIRST_NAMED_CATEGORY];
    },
      ...
      if (0 < items.length) {
        while (true) {
          ...
          if (0 < items[num].channelList.length) {
            ...
            obj[0] = num + favoriteChannels(7273).SECTION_INDEX_UNCATEGORIZED_CHANNELS;
            obj[1] = num2;
            let items1 = [obj];
            return items1;
          }
...
export const useFavoritesGuildChannelList = function useFavoritesGuildChannelList(arg0) {
  ...
  dependencyMap = undefined;
  let callback;
  hasAccess = _require(10307).useFavoritesAccess("FavoritesGuildChannelList").hasAccess;
  ...
  const effect1 = React.useEffect(() => {
    ...
    if (tmp) {
      ...
      if (0 === arr.length) {
        ...
        const threadsBulk = hasAccess(6159).loadThreadsBulk(arr);
        const obj2 = hasAccess(6159);
      }
    }
  }, items1);
  if (hasAccess) {
  ...
  let flag2 = false;
  if (memo.getSections().length <= _require(7273).SECTION_INDEX_FIRST_NAMED_CATEGORY) {
    _require = false;
