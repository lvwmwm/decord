...
// Dependencies: [19, 1390, 21, 15727, 15728, 15729, 10307, 589, 5989, 5621, 1236, 5991, 5022, 11721, 15730, 5551, 7741, 4436, 2]
// Exports: default

// Module 15726 (FavoritesGuildActionSheet)
import noopAll from "noop" /* 19 */;
import useFavoritesGuildHideActionDefault from "useFavoritesGuildHideAction" /* 15727 */;
import useFavoritesGuildResetActionDefault from "useFavoritesGuildResetAction" /* 15728 */;
import useFavoritesGuildAutoAddedThreadsActionDefault from "useFavoritesGuildAutoAddedThreadsAction" /* 15729 */;
...
export default function FavoritesGuildActionSheet(onClose) {
  ...
  obj[0] = callback(onClose(5621).BottomSheetTitleHeader, obj);
  let tmp8Result = null;
  if (tmp4.isAvailable) {
    ...
    obj3[0] = tmp5(5022).ThreadIcon;
    obj2[2] = tmp8(tmp5(5991).ActionSheetRow.Icon, obj3);
  ...
  if (obj.useFavoritesAccess("FavoritesGuildActionSheet").hasAccess) {
    ...
    if (stateFromStores) {
      ...
      obj6[0] = tmp5(11721).ArrowsUpDownIcon;
      obj5[1] = tmp8(tmp5(5991).ActionSheetRow.Icon, obj6);
      obj5[2] = function onPress() {
        onClose();
        lib(closure_2[14])();
      };
      obj4[1] = tmp8(tmp5(5991).ActionSheetRow, obj5);
      tmp8Result = tmp8(tmp5(5991).ActionSheetRow.Group, obj4);
    }
  }
  items1[1] = tmp8Result;
  const obj7 = { label: tmp2.label, subLabel: tmp2.subLabel, icon: null, variant: null, onPress: null };
  if (tmp2.isPreview) {
    let EyeSlashIcon = tmp5(5551).XSmallIcon;
  } else {
    EyeSlashIcon = tmp5(7741).EyeSlashIcon;
  }
  obj7[2] = callback(onClose(5991).ActionSheetRow.Icon, { IconComponent: EyeSlashIcon });
  let str = "danger";
  if (tmp2.isPreview) {
    str = "default";
  }
  ...
  if (tmp3.isAvailable) {
    ...
    obj8[2] = tmp8(tmp5(5991).ActionSheetRow.Icon, obj9);
    obj8[4] = function onPress() {
      onClose();
      closure_2.perform();
    };
    tmp8Result1 = tmp8(tmp5(5991).ActionSheetRow, obj8);
  }
