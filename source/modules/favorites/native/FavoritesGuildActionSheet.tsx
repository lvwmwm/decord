...
// Dependencies: [19, 1389, 21, 16060, 16061, 16062, 10153, 586, 7138, 7089, 1233, 7140, 5034, 12083, 16063, 5635, 6908, 4439, 2]
// Exports: default
...
export default function FavoritesGuildActionSheet(onClose) {
  ...
  obj[0] = callback(onClose(7089).BottomSheetTitleHeader, obj);
  let tmp8Result = null;
  if (tmp4.isAvailable) {
    ...
    obj3[0] = tmp5(5034).ThreadIcon;
    obj2[2] = tmp8(tmp5(7140).ActionSheetRow.Icon, obj3);
    ({ isEnabled: obj6[3], toggle: obj6[4] } = tmp4);
    obj1[1] = tmp8(tmp5(7140).ActionSheetSwitchRow, obj2);
    tmp8Result = tmp8(tmp5(7140).ActionSheetRow.Group, obj1);
  }
  ...
  if (obj.useFavoritesAccess("FavoritesGuildActionSheet").hasAccess) {
    ...
    if (stateFromStores) {
      ...
      obj6[0] = tmp5(12083).ArrowsUpDownIcon;
      obj5[1] = tmp8(tmp5(7140).ActionSheetRow.Icon, obj6);
      obj5[2] = function onPress() {
        onClose();
        lib(closure_2[14])();
      };
      obj4[1] = tmp8(tmp5(7140).ActionSheetRow, obj5);
      tmp8Result = tmp8(tmp5(7140).ActionSheetRow.Group, obj4);
    }
  }
  items1[1] = tmp8Result;
  const obj7 = { label: tmp2.label, subLabel: tmp2.subLabel, icon: null, variant: null, onPress: null };
  if (tmp2.isPreview) {
    let EyeSlashIcon = tmp5(5635).XSmallIcon;
  } else {
    EyeSlashIcon = tmp5(6908).EyeSlashIcon;
  }
  obj7[2] = callback(onClose(7140).ActionSheetRow.Icon, { IconComponent: EyeSlashIcon });
  let str = "danger";
  if (tmp2.isPreview) {
    str = "default";
  }
  ...
  const items2 = [callback(onClose(7140).ActionSheetRow, obj7), ];
  ...
  if (tmp3.isAvailable) {
    ...
    obj9[0] = tmp5(4439).TrashIcon;
    obj8[2] = tmp8(tmp5(7140).ActionSheetRow.Icon, obj9);
    obj8[4] = function onPress() {
      onClose();
      closure_2.perform();
    };
    tmp8Result1 = tmp8(tmp5(7140).ActionSheetRow, obj8);
  }
  items2[1] = tmp8Result1;
  items1[2] = closure_5(onClose(7140).ActionSheetRow.Group, { hasIcons: true, children: items2 });
  obj[1] = items1;
  return closure_5(onClose(7138).ActionSheet, obj);
};
