...
// Function ID: 8027
// Name: getReportMenu
// Dependencies: [5, 19, 4302, 676, 7781, 530, 8025, 7786, 709, 38, 8023, 4498, 2]
...
function getUnauthenticatedReportNameSafely(name) {
  name = name.name;
  const values = Object.values(require(8023) /* ReportNames */.UnauthenticatedReportNames);
...
function getReportNameSafely(name) {
  name = name.name;
  const values = Object.values(require(8023) /* ReportNames */.ReportNames);
  const hasItem = values.includes(name);
  importDefault(38)(hasItem, "Invalid report type " + name.name);
  return name;
}
function getModeratorReportNameSafely(name) {
  name = name.name;
  const values = Object.values(require(8023) /* ReportNames */.ModeratorReportNames);
  const hasItem = values.includes(name);
  importDefault(38)(hasItem, "Invalid report type " + name.name);
  return name;
}
function genSubmitData(version, name, arr, email_token) {
  ...
  if (name.name !== require(8023) /* ReportNames */.ReportNames.MESSAGE) {
...
export const submitReport = function submitReport(language, name, arr) {
  ...
  if (store.get("iar_skip_api_report_submit")) {
    ...
    const REPORT_TO_MOD = require(8025) /* set */.ReportMenuTypeSets.REPORT_TO_MOD;
...
export const getModeratorReportEndpointSafely = function getModeratorReportEndpointSafely(name) {
  ...
  importDefault(38)(hasItem, "Invalid report type " + name.name);
  if (name.name === require(8023) /* ReportNames */.ModeratorReportNames.MESSAGE) {
...
export const trackCloseReportModalAnalytics = function trackCloseReportModalAnalytics(_onSubmit, c12, first) {
  ...
  if (_onSubmit.name === require(8023) /* ReportNames */.ReportNames.MESSAGE) {
    let id = _onSubmit.record.id;
  }
  obj[3] = id;
  id = undefined;
  if (_onSubmit.name === require(8023) /* ReportNames */.ReportNames.STAGE_CHANNEL) {
    id = _onSubmit.record.id;
  }
  obj[4] = id;
  let id1;
  if (_onSubmit.name === require(8023) /* ReportNames */.ReportNames.GUILD_SCHEDULED_EVENT) {
    id1 = _onSubmit.record.id;
  }
  obj[5] = id1;
  if (_onSubmit.name !== require(8023) /* ReportNames */.ReportNames.GUILD) {
