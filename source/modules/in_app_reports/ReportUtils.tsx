...
// Function ID: 7807
// Name: getReportMenu
// Dependencies: [5, 19, 4254, 676, 7674, 530, 7805, 7679, 709, 38, 7803, 4449, 2]
...
function getUnauthenticatedReportNameSafely(name) {
  name = name.name;
  const values = Object.values(require(7803) /* ReportNames */.UnauthenticatedReportNames);
...
function getReportNameSafely(name) {
  name = name.name;
  const values = Object.values(require(7803) /* ReportNames */.ReportNames);
  const hasItem = values.includes(name);
  importDefault(38)(hasItem, "Invalid report type " + name.name);
  return name;
}
function getModeratorReportNameSafely(name) {
  name = name.name;
  const values = Object.values(require(7803) /* ReportNames */.ModeratorReportNames);
  const hasItem = values.includes(name);
  importDefault(38)(hasItem, "Invalid report type " + name.name);
  return name;
}
function genSubmitData(version, name, arr, email_token) {
  ...
  if (name.name !== require(7803) /* ReportNames */.ReportNames.MESSAGE) {
...
export const submitReport = function submitReport(language, name, arr) {
  ...
  if (store.get("iar_skip_api_report_submit")) {
    ...
    const REPORT_TO_MOD = require(7805) /* set */.ReportMenuTypeSets.REPORT_TO_MOD;
...
export const getModeratorReportEndpointSafely = function getModeratorReportEndpointSafely(name) {
  ...
  importDefault(38)(hasItem, "Invalid report type " + name.name);
  if (name.name === require(7803) /* ReportNames */.ModeratorReportNames.MESSAGE) {
...
export const trackCloseReportModalAnalytics = function trackCloseReportModalAnalytics(_onSubmit, c12, first) {
  ...
  if (_onSubmit.name === require(7803) /* ReportNames */.ReportNames.MESSAGE) {
    let id = _onSubmit.record.id;
  }
  obj[3] = id;
  id = undefined;
  if (_onSubmit.name === require(7803) /* ReportNames */.ReportNames.STAGE_CHANNEL) {
    id = _onSubmit.record.id;
  }
  obj[4] = id;
  let id1;
  if (_onSubmit.name === require(7803) /* ReportNames */.ReportNames.GUILD_SCHEDULED_EVENT) {
    id1 = _onSubmit.record.id;
  }
  obj[5] = id1;
  if (_onSubmit.name !== require(7803) /* ReportNames */.ReportNames.GUILD) {
