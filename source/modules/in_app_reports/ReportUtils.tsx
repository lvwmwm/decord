...
// Function ID: 7694
// Name: getReportMenu
// Dependencies: [5, 19, 4188, 676, 6713, 530, 7692, 6718, 709, 38, 7690, 4384, 2]
...
function getUnauthenticatedReportNameSafely(name) {
  name = name.name;
  const values = Object.values(require(7690) /* ReportNames */.UnauthenticatedReportNames);
...
function getReportNameSafely(name) {
  name = name.name;
  const values = Object.values(require(7690) /* ReportNames */.ReportNames);
  const hasItem = values.includes(name);
  importDefault(38)(hasItem, "Invalid report type " + name.name);
  return name;
}
function getModeratorReportNameSafely(name) {
  name = name.name;
  const values = Object.values(require(7690) /* ReportNames */.ModeratorReportNames);
  const hasItem = values.includes(name);
  importDefault(38)(hasItem, "Invalid report type " + name.name);
  return name;
}
function genSubmitData(version, name, arr, email_token) {
  ...
  obj = { channel_id: "Array", message_id: "call", stage_instance_id: "warn", guild_id: "user", guild_scheduled_event_id: "HermesInternal", user_id: "sa", email_token: "o", application_id: "isArray", entrypoint: "isArray", widget_id: "isArray" };
  obj[2] = str;
  ...
  if (name.name !== require(7690) /* ReportNames */.ReportNames.MESSAGE) {
    if (name.name !== tmp(7690).ReportNames.FIRST_DM) {
      if (name.name !== tmp(7690).ReportNames.GUILD) {
        if (name.name !== tmp(7690).ReportNames.GUILD_DISCOVERY) {
          if (name.name === tmp(7690).ReportNames.GUILD_DIRECTORY_ENTRY) {
            obj = {};
            ...
            return obj;
          } else if (name.name === tmp(7690).ReportNames.STAGE_CHANNEL) {
            const obj1 = {};
            ...
            return obj1;
          } else if (name.name === tmp(7690).ReportNames.GUILD_SCHEDULED_EVENT) {
            const obj2 = {};
            ...
            if (name.name === tmp(7690).ReportNames.USER) {
              const obj3 = {};
              ...
              let tmp3 = obj3;
            } else if (name.name === tmp(7690).UnauthenticatedReportNames.USER) {
              const obj4 = {};
              ...
              tmp3 = obj4;
            } else if (name.name === tmp(7690).UnauthenticatedReportNames.MESSAGE) {
              const obj5 = {};
              ...
              tmp3 = obj5;
            } else if (name.name === tmp(7690).UnauthenticatedReportNames.GUILD) {
              const obj6 = {};
              ...
              tmp3 = obj6;
            } else if (name.name === tmp(7690).ReportNames.APPLICATION) {
              const obj7 = {};
              ...
              tmp3 = obj7;
            } else if (name.name === tmp(7690).ReportNames.WIDGET) {
              const obj8 = {};
              ...
              tmp3 = null;
              if (name.name === tmp(7690).UnauthenticatedReportNames.MEDIA_TAKEDOWN) {
                const obj9 = {};
...
export const submitReport = function submitReport(language, name, arr) {
  ...
  if (store.get("iar_skip_api_report_submit")) {
    ...
    if (REPORT_TO_MOD.has(name.name)) {
      ...
      let tmp15 = null;
      if (name.name === tmp4(7690).ModeratorReportNames.MESSAGE) {
      ...
      const REPORT_TO_MOD2 = tmp4(7692).ReportMenuTypeSets.REPORT_TO_MOD;
      ...
      name = name.name;
      let _Object = Object;
      const values = Object.values(tmp4(7690).ReportNames);
...
export const getModeratorReportEndpointSafely = function getModeratorReportEndpointSafely(name) {
  ...
  importDefault(38)(hasItem, "Invalid report type " + name.name);
  if (name.name === require(7690) /* ReportNames */.ModeratorReportNames.MESSAGE) {
...
export const trackCloseReportModalAnalytics = function trackCloseReportModalAnalytics(_onSubmit, c12, first) {
  ...
  if (_onSubmit.name === require(7690) /* ReportNames */.ReportNames.MESSAGE) {
    let id = _onSubmit.record.id;
  }
  obj[3] = id;
  id = undefined;
  if (_onSubmit.name === require(7690) /* ReportNames */.ReportNames.STAGE_CHANNEL) {
    id = _onSubmit.record.id;
  }
  obj[4] = id;
  let id1;
  if (_onSubmit.name === require(7690) /* ReportNames */.ReportNames.GUILD_SCHEDULED_EVENT) {
    id1 = _onSubmit.record.id;
  }
  obj[5] = id1;
  if (_onSubmit.name !== require(7690) /* ReportNames */.ReportNames.GUILD) {
    if (_onSubmit.name !== tmp2(7690).ReportNames.GUILD_DISCOVERY) {
      if (_onSubmit.name === tmp2(7690).ReportNames.GUILD_DIRECTORY_ENTRY) {
        let id2 = _onSubmit.record.guildId;
      } else if (_onSubmit.name === tmp2(7690).ReportNames.GUILD_SCHEDULED_EVENT) {
        id2 = _onSubmit.record.guild_id;
      }
    }
    obj[6] = id2;
    if (_onSubmit.name === tmp2(7690).ReportNames.GUILD_SCHEDULED_EVENT) {
      let channelId = _onSubmit.record.channel_id;
    } else if (_onSubmit.name === tmp2(7690).ReportNames.GUILD_DIRECTORY_ENTRY) {
      channelId = _onSubmit.record.channelId;
    }
    obj[7] = channelId;
    id2 = undefined;
    if (_onSubmit.name === tmp2(7690).ReportNames.APPLICATION) {
