...
// Dependencies: [19, 17, 21, 4189, 712, 5525, 4185, 2]
// Exports: default

// Module 15197 (ChannelItemEmbeddedActivities)
import "noop";
...
export default function ChannelItemEmbeddedActivities(muted) {
  ...
  if (size === undefined) {
    size = require(5525) /* GameIcon */.GameIconSizes.SIZE_24;
  }
  ...
  if (embeddedApps.length <= 0) {
    ...
    obj[2] = modeMuted;
    return callback(importDefault(5525), obj);
  } else {
    const diff = embeddedApps.length - 1;
    const tmp13 = require(5525) /* GameIcon */.GameIconImageSize[size];
    ...
    const items = [callback(importDefault(5525), obj1), ];
    if (2 === embeddedApps.length) {
      obj = { game: null, size: null };
      obj[0] = embeddedApps[1].application;
      obj[1] = size;
      let tmp16Result = tmp16(importDefault(5525), obj);
    } else {
      ...
      obj4[2] = "+" + diff;
      obj2[1] = tmp16(require(4185) /* Text */.Text, obj4);
