...
// Dependencies: [19, 17, 21, 4344, 712, 5777, 4340, 2]
// Exports: default
...
export default function ChannelItemEmbeddedActivities(muted) {
  ...
  if (size === undefined) {
    size = require(5777) /* GameIcon */.GameIconSizes.SIZE_24;
  }
  ...
  if (embeddedApps.length <= 0) {
    ...
    obj[2] = modeMuted;
    return callback(importDefault(5777), obj);
  } else {
    const diff = embeddedApps.length - 1;
    const tmp13 = require(5777) /* GameIcon */.GameIconImageSize[size];
    ...
    const items = [callback(importDefault(5777), obj1), ];
    if (2 === embeddedApps.length) {
      obj = { game: null, size: null };
      obj[0] = embeddedApps[1].application;
      obj[1] = size;
      let tmp16Result = tmp16(importDefault(5777), obj);
    } else {
      ...
      obj4[2] = "+" + diff;
      obj2[1] = tmp16(require(4340) /* Text */.Text, obj4);
