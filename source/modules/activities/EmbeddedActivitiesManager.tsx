...
// Dependencies: [5, 4396, 1395, 1218, 1391, 4454, 1981, 1922, 10690, 1390, 676, 10691, 500, 530, 4015, 10692, 10693, 698, 10681, 10695, 10696, 4609, 514, 1903, 8499, 4562, 1231, 709, 10475, 10679, 10697, 5835, 10723, 1236, 10731, 10699, 10732, 11061, 1955, 5854, 2]
// Exports: getActiveAnalyticsSessionIDs, trackFrameSessionEnd, trackFrameSessionStart

// Module 10689 (getShelfItemTrackingProperties)
...
import getShelfItemDataDefault from "getShelfItemData" /* 10692 */;
...
import closure_11 from "initialize" /* 10690 */;
            ...
            if (null != closure_5) {
              if (null != premiumType) {
                if (null == closure_5.connectedSince) {
                  ...
                  obj5[0] = callback;
                  obj5[1] = currentUser;
                  activity = lib(10692)(obj5);
                  releasePhase = callback2(activity).releasePhase;
                  constants = lib(10693).getRawThermalState();
                  const obj13 = lib(10693);
...
function handleOpenEmbeddedActivity(applicationId) {
  ...
  obj1 = applicationId(10695);
  if (obj1.tryLaunchAsFrame({ applicationId })) {
    let tmp4Result = tmp4(10696);
    ...
    obj[0] = isStart;
    obj[1] = inviterUserId;
    tmp4Result = tmp4(4015);
    obj[2] = tmp4Result.getEmbeddedActivityLocationChannelId(_location);
    obj[3] = tmp4(4015).getEmbeddedActivityLocationGuildId(_location);
    ...
    const tmp4Result1 = tmp4(4015);
  } else {
    importDefault = id.getId();
    const found = participants.find((userId) => userId.userId === closure_1);
    const embeddedActivityLocationChannelId = tmp4(4015).getEmbeddedActivityLocationChannelId(_location);
    const tmp4Result2 = tmp4(4015);
    const embeddedActivityLocationGuildId = tmp4(4015).getEmbeddedActivityLocationGuildId(_location);
    ...
    if (isFirstActivityInChannel) {
      const participant = _modDef4609.selectParticipant(channel.id, null);
      const obj6 = _modDef4609;
    }
    if (null != found) {
      ...
      if (null != compositeInstanceId) {
        if (!tmp18) {
          ...
          if (null != currentUser) {
            ...
            if (null != channel) {
              commandContextType = tmp4(8499).computeCommandContextType(channel, applicationId);
              const tmp4Result7 = tmp4(8499);
            }
...
export const trackFrameSessionStart = function trackFrameSessionStart(applicationId) {
  ...
  if (null != result) {
    ...
    if (null != currentUser) {
      ...
      let commandContextType = null;
      if (null != channel) {
        commandContextType = tmp(8499).computeCommandContextType(channel, applicationId);
        const tmpResult1 = tmp(8499);
      }
