...
// Dependencies: [5, 4460, 1395, 1218, 1391, 4518, 1981, 1922, 10762, 1390, 676, 10763, 500, 530, 4079, 10764, 10765, 698, 10753, 10767, 10768, 4673, 514, 1903, 8566, 4626, 1231, 709, 10543, 10751, 10769, 5901, 10795, 1236, 10803, 10771, 10804, 11133, 1955, 5920, 2]
// Exports: getActiveAnalyticsSessionIDs, trackFrameSessionEnd, trackFrameSessionStart

// Module 10761 (getShelfItemTrackingProperties)
...
import getShelfItemDataDefault from "getShelfItemData" /* 10764 */;
...
import closure_11 from "initialize" /* 10762 */;
import closure_12 from "participantFromServer" /* 1390 */;
            ...
            if (null != closure_5) {
              if (null != premiumType) {
                if (null == closure_5.connectedSince) {
                  ...
                  obj5[0] = callback;
                  obj5[1] = currentUser;
                  activity = lib(10764)(obj5);
                  releasePhase = callback2(activity).releasePhase;
                  constants = lib(10765).getRawThermalState();
                  const obj13 = lib(10765);
...
function handleOpenEmbeddedActivity(applicationId) {
  ...
  obj1 = applicationId(10767);
  if (obj1.tryLaunchAsFrame({ applicationId })) {
    let tmp4Result = tmp4(10768);
    ...
    obj[0] = isStart;
    obj[1] = inviterUserId;
    tmp4Result = tmp4(4079);
    obj[2] = tmp4Result.getEmbeddedActivityLocationChannelId(_location);
    obj[3] = tmp4(4079).getEmbeddedActivityLocationGuildId(_location);
    ...
    const tmp4Result1 = tmp4(4079);
  } else {
    importDefault = id.getId();
    const found = participants.find((userId) => userId.userId === closure_1);
    const embeddedActivityLocationChannelId = tmp4(4079).getEmbeddedActivityLocationChannelId(_location);
    const tmp4Result2 = tmp4(4079);
    const embeddedActivityLocationGuildId = tmp4(4079).getEmbeddedActivityLocationGuildId(_location);
    ...
    if (isFirstActivityInChannel) {
      const participant = _modDef4673.selectParticipant(channel.id, null);
      const obj6 = _modDef4673;
    }
    if (null != found) {
      ...
      if (null != compositeInstanceId) {
        if (!tmp18) {
          ...
          if (null != currentUser) {
            ...
            if (null != channel) {
              commandContextType = tmp4(8566).computeCommandContextType(channel, applicationId);
              const tmp4Result7 = tmp4(8566);
            }
...
export const trackFrameSessionStart = function trackFrameSessionStart(applicationId) {
  ...
  if (null != result) {
    ...
    if (null != currentUser) {
      ...
      let commandContextType = null;
      if (null != channel) {
        commandContextType = tmp(8566).computeCommandContextType(channel, applicationId);
        const tmpResult1 = tmp(8566);
      }
