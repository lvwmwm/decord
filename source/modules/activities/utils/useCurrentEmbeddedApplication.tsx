...
// Dependencies: [32, 10600, 5601, 2]
// Exports: default

// Module 10599 (useCurrentEmbeddedApplication)
...
export default function useCurrentEmbeddedApplication() {
  ...
  const tmp = importDefault(10600)();
  if (null == tmp) {
    let items = [];
  } else {
    items = [tmp.applicationId];
  }
  const first = callback(importDefault(5601)(items, flag), 1)[0];
  return first;
};
