...
// Function ID: 11211
// Name: getCanJoin
// Dependencies: [676, 11211, 11212, 11213, 11214, 11215, 11216, 11217, 5969, 500, 2]
// Exports: getCanJoin, getCanSync

// Module 11210 (getCanJoin)
import ME from "ME";
...
export const getCanJoin = function getCanJoin(currentUserId) {
  ...
  if (message.author.id === currentUserId.currentUserId) {
    ...
    if (importDefault(11211)(presenceActivity, message, tmp2.id)) {
      let obj = require(11212) /* getPartySize */;
      const partySize = obj.getPartySize(presenceActivity);
      if (obj2.hasPartySize(partySize)) {
        let tmp6Result = tmp6(11214);
        if (!tmp6Result.isPartyFull(partySize)) {
          tmp6Result = tmp6(11215);
            ...
            if (tmp6Result1.getIsAskToJoin(message)) {
              ...
              if (type === constants.JOIN) {
                if (null != presenceActivity) {
                  const remoteJoinableActivityPlatform = tmp6(11217).getRemoteJoinableActivityPlatform(presenceActivity);
                  if (null != remoteJoinableActivityPlatform) {
                    obj = { canJoin: true, remoteJoinPlatform: null };
                    obj[1] = remoteJoinableActivityPlatform;
                    return obj;
                  } else if (tmp13(5969)(presenceActivity, constants2.SUPPORTS_JOIN_URL)) {
                    return { canJoin: true, remoteJoinPlatform: null };
                  }
                  const tmp6Result2 = tmp6(11217);
                }
              }
...
export const getCanSync = function getCanSync(activity, tmp8Result, arg2, id) {
  ...
  if (tmp) {
    let tmp6 = importDefault(11211)(activity, arg2, id.id);
    if (tmp6) {
      let tmp8 = tmp4(5969)(activity, constants2.SYNC);
      if (tmp8) {
        let isPlatformEmbedded = require(500) /* set */.isPlatformEmbedded;
        if (isPlatformEmbedded) {
          isPlatformEmbedded = !tmp9(11215).getIsInParty(tmp8Result, activity);
          const tmp9Result = tmp9(11215);
        }
