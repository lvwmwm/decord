...
// Dependencies: [57, 31, 27, 1347, 5670, 482, 33, 4165, 689, 1557, 4026, 15812, 3877, 5251, 15816, 566, 5503, 15817, 15821, 15822, 15827, 15806, 2]

// Module 15815 (useBaseActivityPanelHeaderContent)
...
function useBaseActivityPanelHeaderContent(landscape) {
  ...
  obj.runOnJS = landscape(4026).runOnJS;
  obj.setMode = setMode;
  ...
  obj = { mode: landscape(15812).MorphablePanelModes.PANEL, panGestureEnabled: true, pipState, swipeRequiresPop: true, wrapperOffset, onPanMinimizeGestureEnd: callback, disableHorizontalSafeAreas: true };
  obj.gesture = setMode(15812)(obj);
...
class BaseActivityPanelContent {
  constructor(arg0) {
    ...
    if (hasConnectedActivity) {
      ...
      tmp9 = View;
      items = [, , ];
      items[0] = jsx(require("module_15816"), {});
      tmp12 = !landscape;
      if (tmp12) {
        tmp13 = jsx;
        tmp14 = View;
...
let closure_17 = importAllResult.memo(function ActivityPanelHeaderContentInner(wrapperOffset) {
  ...
  const first = callback(importDefault(5503)(items1), 1)[0];
  const tmp5 = styles();
  obj = {};
  let id;
  ...
  const tmp6Result = tmp6(importDefault(15817), obj);
  ...
  const tmp7 = importDefault(15817);
  if (!landscape) {
    if (null != first) {
      name = first.name;
    }
  ...
  const items3 = [tmp15(importDefault(15821), obj2), , ];
  let tmp18 = null != applicationId;
  if (tmp18) {
    const obj3 = { applicationId };
    tmp18 = callback2(importDefault(15822), obj3);
  }
  items3[1] = tmp18;
  let tmp22 = null;
  if (landscape) {
  ...
  const obj4 = {};
  let tmp26;
  const tmp16 = importDefault(15821);
  ...
  items4[2] = tmp24(importDefault(15827), obj4);
  obj.children = items4;
  return tmp10(tmp11, obj);
});
const memoResult = importAllResult.memo(() => {
  let obj = { context: importDefault(15806) };
