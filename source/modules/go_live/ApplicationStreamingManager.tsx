...
// Function ID: 16796
// Name: updateRegion
// Dependencies: [4274, 1218, 1372, 4176, 4347, 1931, 4340, 1874, 4319, 676, 12, 4434, 687, 4318, 4140, 709, 5214, 9063, 16775, 2]

// Module 16795 (updateRegion)
...
function updateRegion(encodeStreamKeyResult, preferredRegion) {
  ...
  if (tmp3) {
    require(4434) /* watchStream */.changeStreamRegion(encodeStreamKeyResult, preferredRegion);
...
const prototype = function BaseApplicationStreamingManager() {
  ...
  applyArgumentsResult.handleStreamWatch = function handleStreamWatch(streamKey) {
    ...
    const channel = store.getChannel(streamKey(4318).decodeStreamKey(streamKey).channelId);
    if (channel != null) {
  ...
  applyArgumentsResult.handleStreamCreate = function handleStreamCreate(streamKey) {
    ...
    const obj2 = applyArgumentsResult(4318);
    memberCount = memberCount.getMemberCount(applyArgumentsResult(4318).decodeStreamKey(streamKey).guildId);
  };
            ...
            if (channel.isDM()) {
              if (null == obj3.getActiveStreamForUser(ownerId, channel.getGuildId())) {
                ...
                if (null != streamForUser) {
                  const encodeStreamKeyResult = applyArgumentsResult(4318).encodeStreamKey(streamForUser);
                  if (encodeStreamKeyResult !== c17) {
                    ...
                    if (!isStreamMarkedFullResult) {
                      ...
                      tmp2(4434).watchStream(streamForUser, { noFocus: true });
                      const tmp2Result = tmp2(4434);
                    }
                  }
                  let obj = applyArgumentsResult(4318);
  ...
  applyArgumentsResult.handleCallUpdate = function handleCallUpdate(region) {
    ...
    if (channelId === region.channelId) {
      const encodeStreamKeyResult = applyArgumentsResult(4318).encodeStreamKey(currentUserActiveStream);
      if (region == null) {
      ...
      if (tmp7) {
        tmp3(4434).changeStreamRegion(encodeStreamKeyResult, region);
        const tmp3Result = tmp3(4434);
      }
      const obj = applyArgumentsResult(4318);
