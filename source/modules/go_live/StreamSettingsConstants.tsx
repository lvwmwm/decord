...
// Module 4529 (RESOLUTION_720)
import set from "set" /* 2 */;
import ME from "ME" /* 676 */;
import GuildFeatures from "GuildFeatures" /* 1924 */;

let FPS_602 = require;
let f31526 = dependencyMap;
...
f31526 = () => {
  const intl = FPS_602(f31526[2]).intl;
  return intl.string(FPS_602(f31526[2]).t.XjXqzh);
};
...
f31526 = () => {
  ...
  if (RESOLUTION_480 === obj.RESOLUTION_SOURCE) {
    const intl2 = FPS_602(f31526[2]).intl;
    let stringResult = intl2.string(FPS_602(f31526[2]).t.XjXqzh);
  } else {
    const intl = FPS_602(f31526[2]).intl;
    obj = { resolution: null };
    obj[0] = RESOLUTION_480;
    stringResult = intl.formatToPlainString(FPS_602(f31526[2]).t.TEOC0I, obj);
  }
  return stringResult;
};
...
f31526 = () => {
  ...
  return intl.formatToPlainString(FPS_602(f31526[2]).t["bW+JCW"], obj);
};
