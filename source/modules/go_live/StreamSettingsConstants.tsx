...
// Module 4504 (RESOLUTION_720)
import set from "set" /* 2 */;
import ME from "ME" /* 676 */;
import GuildFeatures from "GuildFeatures" /* 1924 */;

let FPS_602 = require;
let f31667 = dependencyMap;
...
f31667 = () => {
  const intl = FPS_602(f31667[2]).intl;
  return intl.string(FPS_602(f31667[2]).t.XjXqzh);
};
...
f31667 = () => {
  ...
  if (RESOLUTION_480 === obj.RESOLUTION_SOURCE) {
    const intl2 = FPS_602(f31667[2]).intl;
    let stringResult = intl2.string(FPS_602(f31667[2]).t.XjXqzh);
  } else {
    const intl = FPS_602(f31667[2]).intl;
    obj = { resolution: null };
    obj[0] = RESOLUTION_480;
    stringResult = intl.formatToPlainString(FPS_602(f31667[2]).t.TEOC0I, obj);
  }
  return stringResult;
};
...
f31667 = () => {
  ...
  return intl.formatToPlainString(FPS_602(f31667[2]).t["bW+JCW"], obj);
};
