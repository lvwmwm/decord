...
f31690 = () => {
  const intl = FPS_602(f31690[2]).intl;
  return intl.string(FPS_602(f31690[2]).t.XjXqzh);
};
...
f31690 = () => {
  ...
  if (RESOLUTION_480 === obj.RESOLUTION_SOURCE) {
    const intl2 = FPS_602(f31690[2]).intl;
    let stringResult = intl2.string(FPS_602(f31690[2]).t.XjXqzh);
  } else {
    const intl = FPS_602(f31690[2]).intl;
    obj = { resolution: null };
    obj[0] = RESOLUTION_480;
    stringResult = intl.formatToPlainString(FPS_602(f31690[2]).t.TEOC0I, obj);
  }
  return stringResult;
};
...
f31690 = () => {
  ...
  return intl.formatToPlainString(FPS_602(f31690[2]).t["bW+JCW"], obj);
};
