...
f32386 = () => {
  const intl = FPS_602(f32386[2]).intl;
  return intl.string(FPS_602(f32386[2]).t.XjXqzh);
};
...
f32386 = () => {
  ...
  if (RESOLUTION_480 === obj.RESOLUTION_SOURCE) {
    const intl2 = FPS_602(f32386[2]).intl;
    let stringResult = intl2.string(FPS_602(f32386[2]).t.XjXqzh);
  } else {
    const intl = FPS_602(f32386[2]).intl;
    obj = { resolution: null };
    obj[0] = RESOLUTION_480;
    stringResult = intl.formatToPlainString(FPS_602(f32386[2]).t.TEOC0I, obj);
  }
  return stringResult;
};
...
f32386 = () => {
  ...
  return intl.formatToPlainString(FPS_602(f32386[2]).t["bW+JCW"], obj);
};
