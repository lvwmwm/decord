...
// Dependencies: [4269, 1376, 1932, 1862, 3883, 4271, 676, 5086, 1236, 4437, 4468, 589, 2]
// Exports: getStreamCTAAriaLabel, getStreamCTAString, getStreamEligibleChannels, useCanWatchStream

// Module 7079 (canStreamInChannel)
import set from "set";
...
function canStreamInChannel(channel, closure_2, set, arg3) {
  ...
  if (channel.isPrivate()) {
    ...
    if (!isGuildStageVoiceResult) {
      if (flag) {
        flag = !importDefault(5086)(channel, set);
      }
      let tmp9 = !flag;
      if (!flag) {
...
function canWatchStream(basicChannel1, closure_17, closure_12, closure_14, updateVoiceState) {
  if (null == basicChannel1) {
    ...
    let tmp9 = importDefault(5086)(basicChannel1, closure_14);
    const tmp10 = null != updateVoiceState.getAwaitingRemoteSessionInfo() || null != updateVoiceState.getRemoteSessionId();
    let result = require(4468) /* shouldShowAgeGateForVoiceChannel */.shouldAgeVerifyForAgeGate();
    if (result) {
      result = require(4468) /* shouldShowAgeGateForVoiceChannel */.shouldShowAgeGateForChannelId(basicChannel1.id);
      const tmp11Result = require(4468) /* shouldShowAgeGateForVoiceChannel */;
    }
    if (tmp10) {
