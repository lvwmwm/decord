...
// Dependencies: [19, 10109, 21, 11774, 1236, 11579, 13133, 4161, 11778, 1959, 2]
// Exports: default
...
export default function HubSidebarProgressOverview(guild) {
  ...
  const nextHubProgressStep = guild(11774).getNextHubProgressStep(hubProgressBarCompletedSteps);
  if (null == nextHubProgressStep) {
    return null;
  } else {
    const size = hubProgressBarCompletedSteps.size;
    const hubProgressTitleForStep = tmp(11774).getHubProgressTitleForStep(nextHubProgressStep);
    if (size < closure_4) {
    ...
    const _Math = Math;
    const bound = Math.max(tmp(11579).MIN_PROGRESS_PERCENT, 100 * size / tmp11);
    ...
    obj[3] = bound;
    return jsx(tmp(13133).GuildProgressOverviewView, { onPress: null, title: null, subtitle: null, percentComplete: null });
  }
  const obj2 = guild(11774);
};
