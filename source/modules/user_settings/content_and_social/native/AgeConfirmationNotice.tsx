...
// Dependencies: [19, 17, 7820, 21, 5956, 14060, 4058, 1993, 7838, 7840, 712, 1297, 4754, 1236, 4340, 2]
// Exports: default

// Module 14130 (AgeConfirmationNotice)
...
export default function AgeConfirmationNotice() {
  let obj = sensitiveContentFilterHelpArticle(5956);
  sensitiveContentFilterHelpArticle = obj.useSensitiveContentFilterHelpArticle();
  const effect = React.useEffect(() => {
    const result = sensitiveContentFilterHelpArticle(14060).trackSafetySettingsNoticeAnalytics(constants2.AGE_CONFIRMATION_NOTICE, constants.VIEWED);
  }, []);
  ...
  const callback = React.useCallback(() => {
    ...
    obj = { entryPoint: sensitiveContentFilterHelpArticle(7840).AgeVerificationModalEntryPoint.CONTENT_AND_SOCIAL_NOTICE };
    const result = obj.showAgeVerificationGetStartedModal(obj);
    const result1 = sensitiveContentFilterHelpArticle(14060).trackSafetySettingsNoticeAnalytics(constants2.AGE_CONFIRMATION_NOTICE, constants.CONFIRM_AGE);
  }, []);
  ...
  obj2[3] = callback;
  obj1[2] = jsx(sensitiveContentFilterHelpArticle(4754).Button, { variant: "secondary", size: "sm", text: null, onPress: null });
