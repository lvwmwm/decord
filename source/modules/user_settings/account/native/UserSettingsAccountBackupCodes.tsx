...
// Function ID: 14605
// Name: CodeRow
// Dependencies: [19, 17, 13671, 21, 4481, 709, 7130, 4193, 5560, 7073, 1233, 4197, 586, 14605, 4934, 4477, 5642, 2]
// Exports: default

// Module 14604 (CodeRow)
import ThemesDefault from "Themes" /* 709 */;
import closure_3 from "noop" /* 19 */;
import { ScrollView } from "get ActivityIndicator" /* 17 */;
import closure_5 from "getVerificationKey" /* 13671 */;
...
function CodeRow(code) {
  ...
  if (showCheckMark) {
    ...
    tmp2Result = tmp2(code(7073).CheckmarkSmallIcon, obj);
  }
  obj[2] = tmp2Result;
  return closure_6(code(5560).TableRow, obj);
}
...
export default function UserSettingsAccountBackupCodes(headerLabel) {
  ...
  obj[1] = obj;
  const items2 = [headerLabel.map((children) => callback2(stateFromStores(4477).Text, { variant: "text-sm/medium", children }, arg1)), , , ];
  ...
  if (tmp10Result) {
    ...
    tmp10Result = tmp10(tmp3(5642).TableRowGroup, obj1);
  }
  ...
  if (tmp10Result1) {
    ...
    obj4[0] = tmp10(tmp3(4477).Text, obj5);
    obj4[1] = function onPress() {
      const verificationKey = store.getVerificationKey();
      const result = callback(14605).confirmViewBackupCodes(verificationKey, true);
    };
    obj3[1] = tmp10(tmp3(5560).TableRow, obj4);
    tmp10Result1 = tmp10(tmp3(5642).TableRowGroup, obj3);
  }
  ...
  obj[2] = items2;
  return closure_6(tmp11, { children: closure_7(stateFromStores(4934).Stack, obj) });
};
