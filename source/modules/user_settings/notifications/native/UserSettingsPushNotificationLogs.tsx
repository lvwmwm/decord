...
// Dependencies: [5, 32, 19, 17, 676, 21, 4344, 712, 6042, 595, 9918, 1628, 6033, 4886, 1236, 8968, 9920, 12298, 7786, 4340, 2]
// Exports: default

// Module 14822 (UserSettingsPushNotificationLogs)
...
export default function UserSettingsPushNotificationLogs() {
  ...
  let items1 = [callback3(first(6033).SearchField, { size: "md", placeholder: "Filter (regex)", onChange: tmp[1], defaultValue: first }), ];
    ...
    if (c3 === 2) {
      ...
      try {
        ...
        if (0 === dependencyMap) {
          ...
          obj3[0] = outer1_1(9920)(closure_0, false);
  ...
  obj[3] = callback3(first(12298).ShareIcon, {});
  items1[1] = callback3(first(4886).PressableOpacity, obj);
  obj[1] = items1;
  ...
  obj1[2] = function renderItem(item) {
    ...
    const items = [outer1_7(first(4340).Text, obj), ];
    obj = { style: tmp9.code, variant: "text-sm/normal", children: null };
    const items1 = [str, "[", item.type, "]", " "];
    const items2 = [outer1_8(first(4340).Text, { style: tmp9.code, variant: "text-sm/normal", color: "text-brand", children: items1 }), item.title, " - ", item.content];
    obj[2] = items2;
    items[1] = outer1_8(first(4340).Text, obj);
    obj[1] = items;
    return outer1_8(outer1_6, obj, item.index);
  };
  items2[1] = callback3(first(7786).FlashList, obj1);
  obj[1] = items2;
