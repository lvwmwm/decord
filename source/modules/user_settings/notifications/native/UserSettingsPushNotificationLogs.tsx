...
// Dependencies: [5, 32, 19, 17, 676, 21, 4193, 712, 6744, 595, 10364, 1581, 9248, 4721, 1236, 8200, 10366, 12316, 6679, 4189, 2]
// Exports: default
...
export default function UserSettingsPushNotificationLogs() {
  ...
  let items1 = [callback3(first(9248).SearchField, { size: "md", placeholder: "Filter (regex)", onChange: tmp[1], defaultValue: first }), ];
    ...
    if (c3 === 2) {
      ...
      try {
        ...
        if (0 === dependencyMap) {
          ...
          obj3[0] = outer1_1(10366)(closure_0, false);
  ...
  obj[3] = callback3(first(12316).ShareIcon, {});
  items1[1] = callback3(first(4721).PressableOpacity, obj);
  obj[1] = items1;
  ...
  obj1[2] = function renderItem(item) {
    ...
    const items = [outer1_7(first(4189).Text, obj), ];
    obj = { style: tmp9.code, variant: "text-sm/normal", children: null };
    const items1 = [str, "[", item.type, "]", " "];
    const items2 = [outer1_8(first(4189).Text, { style: tmp9.code, variant: "text-sm/normal", color: "text-brand", children: items1 }), item.title, " - ", item.content];
    obj[2] = items2;
    items[1] = outer1_8(first(4189).Text, obj);
    obj[1] = items;
    return outer1_8(outer1_6, obj, item.index);
  };
  items2[1] = callback3(first(6679).FlashList, obj1);
  obj[1] = items2;
