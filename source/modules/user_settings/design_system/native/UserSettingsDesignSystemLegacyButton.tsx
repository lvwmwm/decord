...
// Function ID: 15684
// Name: ComparisonRow
// Dependencies: [19, 17, 21, 1296, 4477, 4936, 4481, 709, 4934, 8523, 2]
// Exports: default

// Module 15683 (ComparisonRow)
...
function ComparisonRow(entry) {
  ...
  if (null != redesignVariant) {
    ...
    const items5 = [tmp9(tmp4(4477).Text, { variant: "text-xxs/medium", color: "text-muted", children: "mana" }), ];
    ...
    items5[1] = tmp9(tmp4(4936).Button, obj5);
    obj4[1] = items5;
    let tmp9Result = tmp7(tmp8, obj4);
  } else {
    const obj6 = { style: null, children: null };
    obj6[0] = tmp.comparisonSide;
    obj6[1] = tmp9(tmp4(4477).Text, { variant: "text-xs/medium", color: "text-muted", children: "no mapping" });
...
export default function UserSettingsDesignSystemLegacyButton() {
  ...
  items1[1] = entries.map((arg0) => {
    [tmp, arr] = arg0;
    return callback2(callback(8523).FormSection, { title: tmp, children: arr.map((entry) => callback(closure_8, { entry }, arg1)) }, "cmp-" + tmp);
  });
  ...
  items1[3] = entries1.map((arg0) => {
    [tmp, arr] = arg0;
    return callback2(callback(8523).FormSection, { title: "" + tmp + " (" + arr.reduce((arg0, count) => arg0 + count.count, 0) + " usages)", children: arr.map((combo) => callback(closure_9, { combo }, arg1)) }, tmp);
  });
  obj[1] = items1;
