...
// Function ID: 15113
// Name: SampleCard
// Dependencies: [19, 17, 21, 4444, 1297, 5951, 4877, 5548, 4440, 12580, 1438, 13707, 12249, 11339, 12250, 8514, 2]
// Exports: default

// Module 15112 (SampleCard)
...
export default function UserSettingsDesignSystemPile() {
  ...
  obj = {
    ...
    children: items.map((children) => {
      ...
      items = [callback2(callback(4440).Text, obj), ];
      ...
      items[1] = callback2(callback(12580).AvatarPile, obj);
      obj[0] = items;
      return callback3(callback(4877).Stack, obj, children);
    })
  };
  ...
  items[2] = callback(SampleCard, {
    ...
    children: items.map((children) => {
      ...
      items[1] = callback2(callback(13707).AvatarDuoPile, obj);
      obj[0] = items;
      return callback3(callback(4877).Stack, obj, children);
    })
  });
  ...
  items[3] = callback(SampleCard, {
    ...
    children: items.map((children) => {
      ...
      obj[2] = items2;
      items[1] = callback3(callback(13707).AvatarDuoPile, obj);
      obj[0] = items;
      return callback3(callback(4877).Stack, obj, children);
    })
  });
  ...
  items[4] = callback(SampleCard, {
    ...
    children: items1.map((size) => {
      ...
      items[1] = callback2(callback(12249).GuildIconPile, obj);
      obj[0] = items;
      return callback3(callback(4877).Stack, obj, size);
    })
  });
  ...
  items1[1] = items2.map((arg0) => {
    ...
    items1[1] = callback2(callback(11339).Pile, obj);
    obj = { "aria-label": null, shape: null, size: 48, gap: 2, depthX: null, depthY: null, children: null };
    tmp4Result = tmp4(12250);
    obj[0] = tmp4Result.getListSummaryLabel(closure_11, closure_11.length);
    obj[1] = callback(8514).CutoutShape.RoundedRect;
    obj[4] = tmp;
    obj[5] = tmp2;
    obj[6] = closure_11.map((value) => {
      const obj = { value, size: callback(5951).GuildIconSizes.LARGE };
      return callback3(callback2(5951), obj, arg1);
    });
    items1[2] = callback2(callback(11339).Pile, obj);
    obj[0] = items1;
    return closure_6(callback(4877).Stack, obj, "" + tmp + "," + tmp2);
  });
  obj5[2] = items1;
