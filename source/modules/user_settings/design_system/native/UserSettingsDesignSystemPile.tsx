...
// Function ID: 14828
// Name: SampleCard
// Dependencies: [19, 17, 21, 4303, 1297, 5779, 4712, 5376, 4299, 12307, 1438, 13438, 11978, 10275, 11979, 8298, 2]
// Exports: default

// Module 14827 (SampleCard)
import "noop";
...
function SampleCard(arg0) {
  ...
  const tmp3 = callback(require(4712) /* Stack */.Stack, { spacing: 16, children });
  let obj = { style: callback3().card, children: null };
  const items = [callback(require(4299) /* Text */.Text, { variant: "text-lg/bold", children: title }), ];
  let tmp2Result = tmp3;
  if (!noScroll) {
  ...
  items[1] = tmp2Result;
  obj[1] = items;
  return closure_6(require(5376) /* PressableCard */.Card, obj);
}
...
export default function UserSettingsDesignSystemPile() {
  ...
  obj = {
    ...
    children: items.map((children) => {
      ...
      items[1] = callback2(callback(12307).AvatarPile, obj);
      obj[0] = items;
      return callback3(callback(4712).Stack, obj, children);
    })
  };
  ...
  items[2] = callback(SampleCard, {
    ...
    children: items.map((children) => {
      ...
      items[1] = callback2(callback(13438).AvatarDuoPile, obj);
      obj[0] = items;
      return callback3(callback(4712).Stack, obj, children);
    })
  });
  ...
  items[3] = callback(SampleCard, {
    ...
    children: items.map((children) => {
      ...
      obj[2] = items2;
      items[1] = callback3(callback(13438).AvatarDuoPile, obj);
      obj[0] = items;
      return callback3(callback(4712).Stack, obj, children);
    })
  });
  ...
  items[4] = callback(SampleCard, {
    ...
    children: items1.map((size) => {
      ...
      items[1] = callback2(callback(11978).GuildIconPile, obj);
      obj[0] = items;
      return callback3(callback(4712).Stack, obj, size);
    })
  });
  const obj5 = { title: "Weird Piles", noScroll: true, children: null };
  items1 = [callback2(require(4299) /* Text */.Text, { variant: "text-md/medium", color: "text-subtle", children: ["These examples explore the edge cases of the underlying ", "<Pile>", " component and aren't recommended uses."] }), ];
  ...
  items1[1] = items2.map((arg0) => {
    ...
    items1[1] = callback2(callback(10275).Pile, obj);
    obj = { "aria-label": null, shape: null, size: 48, gap: 2, depthX: null, depthY: null, children: null };
    tmp4Result = tmp4(11979);
    obj[0] = tmp4Result.getListSummaryLabel(closure_11, closure_11.length);
    obj[1] = callback(8298).CutoutShape.RoundedRect;
    obj[4] = tmp;
    obj[5] = tmp2;
    obj[6] = closure_11.map((value) => {
      const obj = { value, size: null };
      obj[1] = callback(5779).GuildIconSizes.LARGE;
      return callback3(callback2(5779), obj, arg1);
    });
    items1[2] = callback2(callback(10275).Pile, obj);
    obj[0] = items1;
    return closure_6(callback(4712).Stack, obj, "" + tmp + "," + tmp2);
  });
  obj5[2] = items1;
