...
// Function ID: 15034
// Name: SampleCard
// Dependencies: [19, 17, 21, 4380, 1297, 5885, 4813, 5482, 4376, 12509, 1438, 13634, 12176, 11267, 12177, 8447, 2]
// Exports: default

// Module 15033 (SampleCard)
...
export default function UserSettingsDesignSystemPile() {
  ...
  obj = {
    ...
    children: items.map((children) => {
      ...
      items = [callback2(callback(4376).Text, obj), ];
      ...
      items[1] = callback2(callback(12509).AvatarPile, obj);
      obj[0] = items;
      return callback3(callback(4813).Stack, obj, children);
    })
  };
  ...
  items[2] = callback(SampleCard, {
    ...
    children: items.map((children) => {
      ...
      items[1] = callback2(callback(13634).AvatarDuoPile, obj);
      obj[0] = items;
      return callback3(callback(4813).Stack, obj, children);
    })
  });
  ...
  items[3] = callback(SampleCard, {
    ...
    children: items.map((children) => {
      ...
      obj[2] = items2;
      items[1] = callback3(callback(13634).AvatarDuoPile, obj);
      obj[0] = items;
      return callback3(callback(4813).Stack, obj, children);
    })
  });
  ...
  items[4] = callback(SampleCard, {
    ...
    children: items1.map((size) => {
      ...
      items[1] = callback2(callback(12176).GuildIconPile, obj);
      obj[0] = items;
      return callback3(callback(4813).Stack, obj, size);
    })
  });
  ...
  items1[1] = items2.map((arg0) => {
    ...
    items1[1] = callback2(callback(11267).Pile, obj);
    obj = { "aria-label": null, shape: null, size: 48, gap: 2, depthX: null, depthY: null, children: null };
    tmp4Result = tmp4(12177);
    obj[0] = tmp4Result.getListSummaryLabel(closure_11, closure_11.length);
    obj[1] = callback(8447).CutoutShape.RoundedRect;
    obj[4] = tmp;
    obj[5] = tmp2;
    obj[6] = closure_11.map((value) => {
      const obj = { value, size: callback(5885).GuildIconSizes.LARGE };
      return callback3(callback2(5885), obj, arg1);
    });
    items1[2] = callback2(callback(11267).Pile, obj);
    obj[0] = items1;
    return closure_6(callback(4813).Stack, obj, "" + tmp + "," + tmp2);
  });
  obj5[2] = items1;
