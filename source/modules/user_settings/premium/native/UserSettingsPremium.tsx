...
// Dependencies: [32, 19, 17, 1922, 4012, 4013, 6902, 6917, 5897, 676, 1924, 21, 4344, 6918, 1628, 5767, 5787, 6919, 4770, 698, 589, 1499, 1433, 6920, 6922, 709, 6924, 6903, 6952, 7380, 6951, 7632, 7633, 7634, 6054, 4007, 7635, 7637, 7677, 7679, 7680, 8668, 12751, 12799, 7446, 500, 2]
// Exports: default

// Module 6916 (UserSettingsPremium)
...
export default function UserSettingsPremium(applicationId) {
  ...
  if (applicationId.isFromTextSection) {
    let items = [tmp(5787).TEXT_AND_IMAGES];
    let items1 = items;
  } else {
    items1 = [];
  }
  const items2 = [...items1, tmp(5787).PREMIUM_MARKETING];
  analyticsLocations = importDefault(5767)(items2).analyticsLocations;
  let obj = applicationId(6919);
  ...
  const stateFromStores4 = obj7.useStateFromStores(items8, () => product.getProduct(applicationId(6920).ProductIds.PREMIUM_TIER_2_MONTHLY));
  ...
  let obj9 = applicationId(6922);
  const isPaymentsBlocked = obj9.useIsPaymentsBlocked();
  const effect = analyticsLocations.useEffect(() => {
    callback(709).wait(() => callback(table[26]).init());
    const obj = callback(709);
    const userEntitlementsForApplication = applicationId(6903).fetchUserEntitlementsForApplication(closure_18);
  }, []);
  ...
  let obj10 = applicationId(6952);
  premiumTrialOffer = obj10.usePremiumTrialOffer();
  const tmp7 = callback(obj1.useStateFromStoresArray(items3, () => {
    const items = [premiumDiscountOffer.getPremiumTypeSubscription(), premiumDiscountOffer.hasFetchedSubscriptions()];
    return items;
  }), 2);
  premiumDiscountOffer = applicationId(7380).usePremiumDiscountOffer();
  const obj12 = applicationId(7380);
  const premiumTrialOfferPremiumType = applicationId(6951).usePremiumTrialOfferPremiumType();
  ...
  if (isPaymentsBlocked) {
    ...
    if (buyNitroExperiment.showRedesignUpsell) {
      ...
      } else if (premiumFeatureCardOrder === tmp5(8668).PremiumFeatureCardOrder.TIER_0_LEADING) {
      ...
      if (hasTier2Premium) {
        if (null == premiumFeatureCardOrder) {
          obj6 = { onClose: null };
          obj6[0] = onClose;
          tmp31Result1 = tmp31(tmp(12751), obj6);
        }
      }
      ...
      if (null != premiumFeatureCardOrder) {
        ...
        if (tmp31Result3) {
          ...
          obj8[2] = tmp8;
          tmp31Result2 = tmp31(tmp(7637), obj8);
        }
        obj7[2] = tmp31Result2;
        if (tmp31Result3) {
          ...
          obj9[1] = tmp8;
          tmp31Result3 = tmp31(tmp(7677), obj9);
        }
        obj7[3] = tmp31Result3;
        if (stateFromStores3) {
          ...
          stateFromStores3 = tmp31(tmp(7679), obj10);
        }
        obj7[4] = stateFromStores3;
        obj7[5] = onClose;
        if (TIER_2_LEADING == null) {
          TIER_2_LEADING = tmp5(8668).PremiumFeatureCardOrder.TIER_2_LEADING;
        }
        ...
        tmp31Result1 = tmp31(tmp(12799), obj7);
        const tmpResult = tmp(12799);
      } else if (premiumTrialOfferPremiumType === closure_17.TIER_0) {
        let TIER_2_LEADING2 = tmp5(8668).PremiumFeatureCardOrder.TIER_0_LEADING;
      } else {
        TIER_2_LEADING2 = premiumFeatureCardOrder;
        if (premiumTrialOfferPremiumType === tmp33.TIER_2) {
          TIER_2_LEADING2 = tmp5(8668).PremiumFeatureCardOrder.TIER_2_LEADING;
        }
      }
    }
    obj4[1] = tmp31Result1;
    obj2[1] = tmp31(tmp5(5767).AnalyticsLocationProvider, obj4);
