...
// Dependencies: [32, 19, 17, 1922, 4012, 4013, 6908, 6923, 5899, 676, 1924, 21, 4342, 6924, 1628, 5768, 5788, 6925, 4771, 698, 589, 1499, 1433, 6926, 6928, 709, 6930, 6909, 6958, 7384, 6957, 7636, 7637, 7638, 6056, 4007, 7639, 7641, 7681, 7683, 7684, 8672, 12751, 12799, 7450, 500, 2]
// Exports: default

// Module 6922 (UserSettingsPremium)
...
export default function UserSettingsPremium(applicationId) {
  ...
  if (applicationId.isFromTextSection) {
    let items = [tmp(5788).TEXT_AND_IMAGES];
    let items1 = items;
  } else {
    items1 = [];
  }
  const items2 = [...items1, tmp(5788).PREMIUM_MARKETING];
  analyticsLocations = importDefault(5768)(items2).analyticsLocations;
  let obj = applicationId(6925);
  ...
  const stateFromStores4 = obj7.useStateFromStores(items8, () => product.getProduct(applicationId(6926).ProductIds.PREMIUM_TIER_2_MONTHLY));
  ...
  let obj9 = applicationId(6928);
  const isPaymentsBlocked = obj9.useIsPaymentsBlocked();
  const effect = analyticsLocations.useEffect(() => {
    callback(709).wait(() => callback(table[26]).init());
    const obj = callback(709);
    const userEntitlementsForApplication = applicationId(6909).fetchUserEntitlementsForApplication(closure_18);
  }, []);
  ...
  let obj10 = applicationId(6958);
  premiumTrialOffer = obj10.usePremiumTrialOffer();
  const tmp7 = callback(obj1.useStateFromStoresArray(items3, () => {
    const items = [premiumDiscountOffer.getPremiumTypeSubscription(), premiumDiscountOffer.hasFetchedSubscriptions()];
    return items;
  }), 2);
  premiumDiscountOffer = applicationId(7384).usePremiumDiscountOffer();
  const obj12 = applicationId(7384);
  const premiumTrialOfferPremiumType = applicationId(6957).usePremiumTrialOfferPremiumType();
  ...
  if (isPaymentsBlocked) {
    ...
    if (buyNitroExperiment.showRedesignUpsell) {
      ...
      } else if (premiumFeatureCardOrder === tmp5(8672).PremiumFeatureCardOrder.TIER_0_LEADING) {
      ...
      if (null != premiumFeatureCardOrder) {
        ...
        if (tmp31Result3) {
          ...
          obj8[2] = tmp8;
          tmp31Result2 = tmp31(tmp(7641), obj8);
        }
        obj7[2] = tmp31Result2;
        if (tmp31Result3) {
          ...
          obj9[1] = tmp8;
          tmp31Result3 = tmp31(tmp(7681), obj9);
        }
        obj7[3] = tmp31Result3;
        if (stateFromStores3) {
          ...
          stateFromStores3 = tmp31(tmp(7683), obj10);
        }
        obj7[4] = stateFromStores3;
        obj7[5] = onClose;
        if (TIER_2_LEADING == null) {
          TIER_2_LEADING = tmp5(8672).PremiumFeatureCardOrder.TIER_2_LEADING;
        }
        ...
        let TIER_2_LEADING2 = tmp5(8672).PremiumFeatureCardOrder.TIER_0_LEADING;
      } else {
        TIER_2_LEADING2 = premiumFeatureCardOrder;
        if (premiumTrialOfferPremiumType === tmp33.TIER_2) {
          TIER_2_LEADING2 = tmp5(8672).PremiumFeatureCardOrder.TIER_2_LEADING;
        }
      }
    }
    obj4[1] = tmp31Result1;
    obj2[1] = tmp31(tmp5(5768).AnalyticsLocationProvider, obj4);
