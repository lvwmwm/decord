...
// Dependencies: [32, 19, 17, 1903, 3935, 3936, 6782, 6797, 5779, 676, 1905, 21, 4285, 6798, 1609, 5649, 5669, 6799, 4711, 698, 589, 1480, 1414, 6800, 6802, 709, 6804, 6783, 6832, 7261, 6831, 7512, 7513, 7514, 5934, 3930, 7515, 7517, 7557, 7559, 7560, 8546, 12601, 12649, 7326, 500, 2]
// Exports: default

// Module 6796 (UserSettingsPremium)
...
export default function UserSettingsPremium(applicationId) {
  ...
  if (applicationId.isFromTextSection) {
    let items = [tmp(5669).TEXT_AND_IMAGES];
    let items1 = items;
  } else {
    items1 = [];
  }
  const items2 = [...items1, tmp(5669).PREMIUM_MARKETING];
  analyticsLocations = importDefault(5649)(items2).analyticsLocations;
  let obj = applicationId(6799);
  ...
  const stateFromStores4 = obj7.useStateFromStores(items8, () => product.getProduct(applicationId(6800).ProductIds.PREMIUM_TIER_2_MONTHLY));
  ...
  let obj9 = applicationId(6802);
  const isPaymentsBlocked = obj9.useIsPaymentsBlocked();
  const effect = analyticsLocations.useEffect(() => {
    callback(709).wait(() => callback(table[26]).init());
    const obj = callback(709);
    const userEntitlementsForApplication = applicationId(6783).fetchUserEntitlementsForApplication(closure_18);
  }, []);
  ...
  let obj10 = applicationId(6832);
  premiumTrialOffer = obj10.usePremiumTrialOffer();
  const tmp7 = callback(obj1.useStateFromStoresArray(items3, () => {
    const items = [premiumDiscountOffer.getPremiumTypeSubscription(), premiumDiscountOffer.hasFetchedSubscriptions()];
    return items;
  }), 2);
  premiumDiscountOffer = applicationId(7261).usePremiumDiscountOffer();
  const obj12 = applicationId(7261);
  const premiumTrialOfferPremiumType = applicationId(6831).usePremiumTrialOfferPremiumType();
  ...
  if (isPaymentsBlocked) {
    ...
    if (buyNitroExperiment.showRedesignUpsell) {
      ...
      } else if (premiumFeatureCardOrder === tmp5(8546).PremiumFeatureCardOrder.TIER_0_LEADING) {
      ...
      if (hasTier2Premium) {
        if (null == premiumFeatureCardOrder) {
          obj6 = { onClose: null };
          obj6[0] = onClose;
          tmp31Result1 = tmp31(tmp(12601), obj6);
        }
      }
      ...
      if (null != premiumFeatureCardOrder) {
        ...
        if (tmp31Result3) {
          ...
          obj8[2] = tmp8;
          tmp31Result2 = tmp31(tmp(7517), obj8);
        }
        obj7[2] = tmp31Result2;
        if (tmp31Result3) {
          ...
          obj9[1] = tmp8;
          tmp31Result3 = tmp31(tmp(7557), obj9);
        }
        obj7[3] = tmp31Result3;
        if (stateFromStores3) {
          ...
          stateFromStores3 = tmp31(tmp(7559), obj10);
        }
        obj7[4] = stateFromStores3;
        obj7[5] = onClose;
        if (TIER_2_LEADING == null) {
          TIER_2_LEADING = tmp5(8546).PremiumFeatureCardOrder.TIER_2_LEADING;
        }
        ...
        tmp31Result1 = tmp31(tmp(12649), obj7);
        const tmpResult = tmp(12649);
      } else if (premiumTrialOfferPremiumType === closure_17.TIER_0) {
        let TIER_2_LEADING2 = tmp5(8546).PremiumFeatureCardOrder.TIER_0_LEADING;
      } else {
        TIER_2_LEADING2 = premiumFeatureCardOrder;
        if (premiumTrialOfferPremiumType === tmp33.TIER_2) {
          TIER_2_LEADING2 = tmp5(8546).PremiumFeatureCardOrder.TIER_2_LEADING;
        }
      }
    }
    obj4[1] = tmp31Result1;
    obj2[1] = tmp31(tmp5(5649).AnalyticsLocationProvider, obj4);
