...
// Dependencies: [19, 17, 1874, 3903, 4148, 3906, 3907, 676, 1876, 21, 4255, 5110, 3967, 4151, 5771, 4574, 7486, 5907, 4251, 1236, 1945, 12649, 1892, 12663, 12665, 12667, 12671, 12569, 6754, 7948, 12609, 589, 1480, 6760, 1577, 2]
// Exports: default

// Module 12648 (componentDidMount)
...
prototype["componentDidMount"] = function componentDidMount() {
  ...
  if (tmp4) {
    const premiumSubscriptionPlans = require(5771) /* fetchSubscriptionPlansForSKU */.fetchPremiumSubscriptionPlans();
    const obj2 = require(5771) /* fetchSubscriptionPlansForSKU */;
  }
  if (!self.props.isFetchingPaymentSources) {
    const paymentSources = require(4574) /* _deletePaymentSource */.fetchPaymentSources();
    const obj3 = require(4574) /* _deletePaymentSource */;
  }
};
prototype["renderPremiumGuildSubscriptions"] = function renderPremiumGuildSubscriptions() {
  ...
  if (props.hasSlots) {
    ...
    items1[2] = tmp6Result;
    obj[1] = items1;
    const items2 = [closure_15(closure_4, obj), callback(importDefault(12649), {})];
    obj3[0] = items2;
...
prototype["render"] = function render() {
  ...
  if (premiumGroupRole === require(1892) /* create */.PremiumSubscriptionGroupRole.MEMBER) {
    ...
    let tmp5 = callback(importDefault(12663), obj);
  } else {
    tmp5 = null;
    if (fractionalState !== FractionalPremiumStates.NONE) {
      ...
      tmp5 = callback(importDefault(12665), obj);
    }
  }
  ...
  const items1 = [callback(require(12667) /* getTopPatternSource */.TopPattern, { style: tmp.background }), ];
  ...
  if (hasFetchedSubscriptionPlans) {
    ...
    tmp13Result = callback(importDefault(12671), obj3);
  }
...
export default function ConnectedUserSettingsPremiumGuildSubscriptions(route) {
  ...
  ({ fractionalState: c2, endsAt } = flag(6754)({ forceFetch: true }));
  let tmpResult = tmp(7948);
  closure_3 = tmpResult.useIsInReverseTrial();
  const tmp3 = flag(6754)({ forceFetch: true });
  closure_4 = flag(12609)(endsAt, tmp(12609).CountDownMessageTypes.LONG_TIME_LEFT);
  tmpResult = tmp(589);
  ...
  const externalManagementMessage = _require(6760).getExternalManagementMessage(stateFromStores, { shouldAllowExternalManagement: true });
  const tmp10 = UserSettingsPremiumGuildSubscriptions;
  const tmp9 = closure_14;
  const tmpResult3 = _require(6760);
  let tmp11 = null;
