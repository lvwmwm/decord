...
// Dependencies: [19, 17, 1874, 3837, 4082, 3840, 3841, 676, 1876, 21, 4189, 5030, 3901, 4085, 6298, 4509, 6502, 6503, 4185, 1236, 1945, 12525, 1892, 12539, 12541, 12543, 12547, 12441, 5846, 7838, 12485, 589, 1480, 6549, 1577, 2]
// Exports: default

// Module 12524 (componentDidMount)
...
prototype["componentDidMount"] = function componentDidMount() {
  ...
  if (!this.props.hasFetchedSlots) {
    const guildBoostSlots = require(4085) /* _fetchAppliedGuildBoostsForGuild */.fetchGuildBoostSlots();
    const obj = require(4085) /* _fetchAppliedGuildBoostsForGuild */;
  }
  if (tmp4) {
    const premiumSubscriptionPlans = require(6298) /* fetchSubscriptionPlansForSKU */.fetchPremiumSubscriptionPlans();
    const obj2 = require(6298) /* fetchSubscriptionPlansForSKU */;
  }
  if (!self.props.isFetchingPaymentSources) {
    const paymentSources = require(4509) /* _deletePaymentSource */.fetchPaymentSources();
    const obj3 = require(4509) /* _deletePaymentSource */;
  }
};
prototype["renderPremiumGuildSubscriptions"] = function renderPremiumGuildSubscriptions() {
  ...
  if (props.hasSlots) {
    ...
    if (tmp6Result) {
      ...
      tmp6Result = tmp6(tmp7(4185).Text, obj2);
    }
    const obj3 = { children: null };
    items1[2] = tmp6Result;
    obj[1] = items1;
    const items2 = [closure_15(closure_4, obj), callback(importDefault(12525), {})];
    obj3[0] = items2;
...
prototype["render"] = function render() {
  ...
  if (premiumGroupRole === require(1892) /* create */.PremiumSubscriptionGroupRole.MEMBER) {
    ...
    let tmp5 = callback(importDefault(12539), obj);
  } else {
    tmp5 = null;
    if (fractionalState !== FractionalPremiumStates.NONE) {
      ...
      tmp5 = callback(importDefault(12541), obj);
    }
  }
  ...
  const items1 = [callback(require(12543) /* getTopPatternSource */.TopPattern, { style: tmp.background }), ];
  ...
  if (hasFetchedSubscriptionPlans) {
    ...
    tmp13Result = callback(importDefault(12547), obj3);
  }
...
export default function ConnectedUserSettingsPremiumGuildSubscriptions(route) {
  ...
  ({ fractionalState: c2, endsAt } = flag(5846)({ forceFetch: true }));
  let tmpResult = tmp(7838);
  closure_3 = tmpResult.useIsInReverseTrial();
  const tmp3 = flag(5846)({ forceFetch: true });
  closure_4 = flag(12485)(endsAt, tmp(12485).CountDownMessageTypes.LONG_TIME_LEFT);
  tmpResult = tmp(589);
  ...
  const externalManagementMessage = _require(6549).getExternalManagementMessage(stateFromStores, { shouldAllowExternalManagement: true });
  const tmp10 = UserSettingsPremiumGuildSubscriptions;
  const tmp9 = closure_14;
  const tmpResult3 = _require(6549);
  let tmp11 = null;
