...
// Dependencies: [19, 17, 1874, 3837, 4082, 3840, 3841, 676, 1876, 21, 4189, 5030, 3901, 4085, 6301, 4509, 6505, 6506, 4185, 1236, 1945, 12531, 1892, 12545, 12547, 12549, 12553, 12447, 5848, 7841, 12491, 589, 1480, 6552, 1577, 2]
// Exports: default

// Module 12530 (componentDidMount)
...
prototype["componentDidMount"] = function componentDidMount() {
  ...
  if (tmp4) {
    const premiumSubscriptionPlans = require(6301) /* fetchSubscriptionPlansForSKU */.fetchPremiumSubscriptionPlans();
    const obj2 = require(6301) /* fetchSubscriptionPlansForSKU */;
  }
...
prototype["renderPremiumGuildSubscriptions"] = function renderPremiumGuildSubscriptions() {
  ...
  if (props.hasSlots) {
    ...
    items1[2] = tmp6Result;
    obj[1] = items1;
    const items2 = [closure_15(closure_4, obj), callback(importDefault(12531), {})];
    obj3[0] = items2;
...
prototype["render"] = function render() {
  ...
  if (premiumGroupRole === require(1892) /* create */.PremiumSubscriptionGroupRole.MEMBER) {
    ...
    let tmp5 = callback(importDefault(12545), obj);
  } else {
    tmp5 = null;
    if (fractionalState !== FractionalPremiumStates.NONE) {
      ...
      tmp5 = callback(importDefault(12547), obj);
    }
  }
  ...
  const items1 = [callback(require(12549) /* getTopPatternSource */.TopPattern, { style: tmp.background }), ];
  ...
  if (hasFetchedSubscriptionPlans) {
    ...
    tmp13Result = callback(importDefault(12553), obj3);
  }
...
export default function ConnectedUserSettingsPremiumGuildSubscriptions(route) {
  ...
  ({ fractionalState: c2, endsAt } = flag(5848)({ forceFetch: true }));
  let tmpResult = tmp(7841);
  closure_3 = tmpResult.useIsInReverseTrial();
  const tmp3 = flag(5848)({ forceFetch: true });
  closure_4 = flag(12491)(endsAt, tmp(12491).CountDownMessageTypes.LONG_TIME_LEFT);
  tmpResult = tmp(589);
  ...
  const externalManagementMessage = _require(6552).getExternalManagementMessage(stateFromStores, { shouldAllowExternalManagement: true });
  const tmp10 = UserSettingsPremiumGuildSubscriptions;
  const tmp9 = closure_14;
  const tmpResult3 = _require(6552);
  let tmp11 = null;
