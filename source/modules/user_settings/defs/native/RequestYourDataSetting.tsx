...
// Dependencies: [17, 1903, 8022, 676, 21, 700, 8450, 705, 589, 3914, 14034, 1236, 3883, 10380, 14036, 2]
// Exports: fetchHarvestStatus, useIsHarvestRequestDisabled

// Module 14033 (useIsHarvestRequestDisabled)
...
function useIsHarvestRequestDisabled() {
  ...
  const tmp = require;
  let harvestDisabledResult = callback((isRequesting) => isRequesting.isRequesting, require(3914) /* isIterable */.shallow);
...
obj = {
  ...
  useTrailing: function useHarvestRequestSettingTrailing() {
    let tmp = null;
    if (callback((isRequesting) => isRequesting.isRequesting, require(3914) /* isIterable */.shallow)) {
      tmp = <ActivityIndicator />;
    }
    return tmp;
  },
  useDescription: function useRequestYourDataSettingDescription() {
    const tmp3 = callback((harvestRequest) => harvestRequest.harvestRequest, require(3914) /* isIterable */.shallow);
    ...
    if (null == currentUser) {
      ...
      const addResult = importDefault(3883)(tmp3.created_at).add(closure_5, "days");
      let formatToPlainStringResult = null;
      if (!addResult.isBefore(importDefault(3883)())) {
...
export const fetchHarvestStatus = function fetchHarvestStatus() {
  const harvestStatus = require(8450) /* saveProfileAndAccountRequest */.getHarvestStatus();
