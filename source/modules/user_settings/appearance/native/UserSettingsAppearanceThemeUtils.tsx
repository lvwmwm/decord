...
// Dependencies: [1345, 1301, 1304, 673, 1347, 1923, 1348, 1305, 15102, 11880, 9287, 1346, 4328, 15103, 695, 2]
...
export const handleSaveTheme = function handleSaveTheme(found, analyticsLocations, isSynced) {
  if (found.type === ClientThemeType.ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
    ...
    if ("system" === found.theme) {
      ...
      const result1 = tmp(15102).resetBackgroundGradientPreset();
      const tmpResult3 = tmp(15102);
      tmp(11880).updateCustomTheme(found.customThemeSettings, customThemeBaseTheme);
      ...
      return tmp(9287).saveClientTheme(obj1);
    } else {
      if (found.type === tmp(1348).ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
        const result2 = tmp(15102).updateBackgroundGradientPreset(found.id);
        const tmpResult6 = tmp(15102);
        tmp(11880).resetCustomTheme();
        const tmpResult7 = tmp(11880);
        ({ id: obj10[0], theme: obj10[1] } = found);
        let saveClientThemeResult = tmp(9287).saveClientTheme({ backgroundGradientPresetId: null, theme: null });
        const obj2 = { backgroundGradientPresetId: null, theme: null };
        const tmpResult8 = tmp(9287);
      } else {
        const result3 = tmp(15102).resetBackgroundGradientPreset();
        const tmpResult9 = tmp(15102);
        tmp(11880).resetCustomTheme();
        const tmpResult10 = tmp(11880);
        const obj3 = { theme: null };
        obj3[0] = found.theme;
        saveClientThemeResult = tmp(9287).saveClientTheme(obj3);
        const tmpResult11 = tmp(9287);
      }
  ...
  if ("system" !== theme.theme) {
    if (theme.type === ClientThemeType.ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
      ...
      if (theme.type === tmp14(1348).ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
        ...
        const result1 = tmp14(4328).updateSyncedClientTheme(systemTheme, obj1);
        const tmp14Result1 = tmp14(4328);
      } else if (theme.type === tmp14(1348).ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
        const obj2 = { backgroundGradientPresetId: null };
        obj2[0] = theme.id;
        const result2 = tmp14(4328).updateSyncedClientTheme(systemTheme, obj2);
        const tmp14Result2 = tmp14(4328);
      } else {
        const obj3 = { theme: null };
        obj3[0] = theme.theme;
        const result3 = tmp14(4328).updateSyncedClientTheme(systemTheme, obj3);
        const tmp14Result3 = tmp14(4328);
      }
    } else if (theme.type === tmp14(1348).ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
