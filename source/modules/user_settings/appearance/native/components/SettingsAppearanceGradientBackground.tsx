...
const items = [num, num];
let closure_8 = { colors: items, locations: [], startPoint: { 0: "%FunctionPrototype%", 0: "paddingStart" }, endPoint: { 0: "%FunctionPrototype%", 0: "paddingStart" } };
...
const memoResult = importAllResult.memo(function SettingsAppearanceGradientBackground(isDimmed) {
  ...
  StyleSheet = tmp3;
  const items = [tmp3];
  const memo = importAllResult.useMemo(() => tmp3 ? [true] : [], items);
  closure_5 = memo;
  ...
  const sharedValue = obj.useSharedValue({ "Null": "%FunctionPrototype%", "Null": "paddingStart" });
    ...
    constructor() {
      ...
      if (0 !== width) {
        if (0 !== height) {
          ...
          tmp8 = themes;
          num3 = 11;
          diff = 90 - obj.interpolate(tweener.get(), [77601039, 1612144654], items);
          ...
          obj.colors = colors.map((arg0, arg1) => {
            ...
            const processColorResult = obj.processColor(colors(closure_2[4]).interpolateColor(store.get(), [77601039, 1612144654], items));
            let num = 0;
            if (null != processColorResult) {
              num = processColorResult;
            }
          ...
          obj.locations = colors.map((arg0, arg1) => {
            ...
            return colors(closure_2[4]).interpolate(store.get(), [77601039, 1612144654], items);
          });
          obj1 = {};
          num4 = 2;
