...
// Dependencies: [1303, 1921, 673, 4474, 1923, 1945, 7775, 5140, 4708, 9240, 9241, 5090, 5099, 9242, 688, 7122, 1233, 4375, 4865, 2]
// Exports: default

// Module 9239 (showUploadFileSizeError)
...
export default function showUploadFileSizeError(arg0) {
  ...
  const tmp2Result2 = items(9240);
  obj[1] = items(9241).getUploaderChannelId(file);
  obj[2] = baseMaxSize;
  ...
  const tmp2Result3 = items(9241);
  obj[10] = items(5140).getKestrelVariantName(kestrelConfig);
  ...
  if (!tmp22) {
    tmp22 = num > tmp2(5090).MAX_TOTAL_ATTACHMENT_SIZE;
  }
  if (!tmp22) {
    tmp22 = tmp21;
  }
  ...
  const tmp2Result5 = items(4708);
  obj[6] = items(5099).getImageCompressionQuality();
  ...
  if (tmp22) {
    if (errorReason === tmp20.ERROR_SOURCE_UNKNOWN) {
      ...
      if (kestrelConfig.enabled) {
        ...
        if (tmp21) {
          obj1 = { maxSize: null };
          obj1[0] = tmp2(4375).formatSize(tmp2(5090).MAX_TOTAL_ATTACHMENT_SIZE / tmp2(4375).BYTE_IN_KB, { useKibibytes: true });
          stringResult1 = formatToPlainString(t.tUOJdH, obj1);
          const tmp2Result7 = tmp2(4375);
        } else {
          const obj2 = { maxSize: null };
          obj2[0] = tmp2(4375).formatSize(maxSize / tmp2(4375).BYTE_IN_KB, { useKibibytes: true });
          stringResult1 = formatToPlainString(t.fxEKdS, obj2);
          const tmp2Result8 = tmp2(4375);
        }
      }
    ...
    items1(4865).show(obj3);
    const obj20 = items1(4865);
  } else {
    ...
    items4[arraySpreadResult] = items1(7122).FILE_UPLOAD_POPOUT;
    obj4[2] = items4;
    const obj6 = { type: null };
    obj6[0] = constants3.UPLOAD_ERROR_UPSELL;
    obj4[3] = obj6;
    obj4[4] = num;
    const result = items1(9242).handleShowUpsellAlert(obj4);
  }
};
