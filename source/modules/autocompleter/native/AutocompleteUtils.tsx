...
// Dependencies: [19, 17, 1376, 1932, 3892, 1874, 676, 9803, 4689, 21, 4255, 712, 4445, 5111, 4094, 7302, 2]
...
// Module 9802 (AutocompleteFormDivider)
import "noop";
...
function AutocompleteFormDivider() {
  ...
  return jsx(importDefault(7302), { style: createCacheKey().itemDivider });
}
...
export const getAutocompleteResultText = function getAutocompleteResultText(type, channel, channel2) {
  ...
  if (constants.USER === type) {
    ...
    if (channel.isThread()) {
      ...
      return "#\"" + obj3.escapeChannelName(require(4445) /* computeChannelName */.computeChannelName(type.channel, mergeGuildAvatar, upsertRelationship)) + "\"";
    } else {
      ...
      if (null != guildId) {
        if (callback(type.channel.type)) {
          ...
          if (name == null) {
            name = require(4445) /* computeChannelName */.computeChannelName(type.channel, mergeGuildAvatar, upsertRelationship);
      ...
      return "" + closure_11 + require(4445) /* computeChannelName */.computeChannelName(type.channel, mergeGuildAvatar, upsertRelationship);
    }
