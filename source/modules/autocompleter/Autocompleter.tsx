...
// Dependencies: [9887, 9888, 1982, 4100, 1922, 5420, 5353, 9889, 1370, 5424, 9891, 1902, 4431, 4436, 4162, 1486, 12, 5423, 2]

// Module 9886 (getAutocompleterBoosterMap)
...
import closure_3 from "handleUserUpdate" /* 9887 */;
import closure_4 from "fromPath" /* 9888 */;
...
prototype["queryTextChannels"] = function queryTextChannels(closure_1, _limit) {
  ...
  if (this._include(AutocompleterResultTypes.TEXT_CHANNEL)) {
    if (self.options.frecencyBoosters) {
      let boosterMap = blacklist(5353).getBoosterMap(tmp);
      const obj2 = blacklist(5353);
    } else {
      boosterMap = {};
    }
...
prototype["queryGameProfiles"] = function queryGameProfiles(query, _limit) {
  if (this._include(AutocompleterResultTypes.GAME_PROFILE)) {
    let result = _require(9891).queryGamesAutocomplete(query);
    if (result == null) {
      result = [];
    }
