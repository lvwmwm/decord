...
// Dependencies: [7231, 7232, 1961, 3921, 1903, 5210, 5141, 7233, 1355, 5214, 7235, 1884, 4269, 4274, 3983, 1467, 12, 5213, 2]

// Module 7230 (getAutocompleterBoosterMap)
...
function getAutocompleterBoosterMap(USER, options) {
  if (options.frecencyBoosters) {
    let boosterMap = require(5141) /* NOOP */.getBoosterMap(USER);
    const obj2 = require(5141) /* NOOP */;
  } else {
    boosterMap = {};
  }
  return boosterMap;
...
prototype["createSearchContext"] = function createSearchContext() {
  ...
  if (null == this.userSearchContext) {
    self.userSearchContext = importDefault(7233).getUserSearchContext(self.parseUserResults, self._limit);
    const obj = importDefault(7233);
  }
};
...
prototype["queryTextChannels"] = function queryTextChannels(closure_1, _limit) {
  ...
  if (this._include(AutocompleterResultTypes.TEXT_CHANNEL)) {
    if (self.options.frecencyBoosters) {
      let boosterMap = blacklist(5141).getBoosterMap(tmp);
      const obj2 = blacklist(5141);
    } else {
      boosterMap = {};
    }
    ...
    obj[5] = fn;
    obj[6] = boosterMap;
    return importDefault(5141).queryChannels(obj);
  } else {
    return [];
  }
};
prototype["queryVoiceChannels"] = function queryVoiceChannels(closure_1, _limit) {
  ...
  if (this._include(AutocompleterResultTypes.VOICE_CHANNEL)) {
    ...
    if (self.options.frecencyBoosters) {
      let boosterMap = require(5141) /* NOOP */.getBoosterMap(tmp);
      const obj2 = require(5141) /* NOOP */;
    } else {
      boosterMap = {};
    }
...
prototype["queryGuilds"] = function queryGuilds(arg0, arg1) {
  ...
  if (this._include(AutocompleterResultTypes.GUILD)) {
    ...
    return importDefault(5141).queryGuilds(obj);
  } else {
    return [];
  }
};
prototype["queryUsers"] = function queryUsers(arg0, arg1, arg2) {
  ...
  if (null != userSearchContext) {
    if (self._include(AutocompleterResultTypes.USER)) {
      ...
      if (null == thread) {
        if (undefined !== arg1) {
          const members = importDefault(5214).requestMembers(arg1, arg0, 100);
          const obj4 = importDefault(5214);
        }
        ...
        obj[3] = tmp2;
        obj[4] = options.allowSnowflake;
        self._userResults = importDefault(5141).queryUsers(obj);
        const obj2 = importDefault(5141);
      }
    }
...
prototype["queryGroupDMs"] = function queryGroupDMs(arg0, arg1) {
  ...
  if (this._include(AutocompleterResultTypes.GROUP_DM)) {
    ...
    obj[3] = fn;
    obj[4] = boosterMap;
    return importDefault(5141).queryGroupDMs(obj);
  } else {
    return [];
  }
};
...
prototype["queryGameProfiles"] = function queryGameProfiles(query, _limit) {
  if (this._include(AutocompleterResultTypes.GAME_PROFILE)) {
    let result = _require(7235).queryGamesAutocomplete(query);
    if (result == null) {
      result = [];
    }
...
prototype["queryLink"] = function queryLink(query, _refetchForSingleCategoryLimit) {
  ...
  if (this._include(AutocompleterResultTypes.LINK)) {
    ...
    if (type === require(4274) /* CodedLinkType */.CodedLinkType.INVITE) {
      ...
      let tmp3Result = tmp3(5141);
      obj[2] = tmp3Result.calculateScore(11);
      const items = [obj];
      return items;
    } else {
      const sanitizeUrlResult = importDefault(3983).sanitizeUrl(query);
      try {
        const _URL = URL;
