...
// Dependencies: [19, 4469, 5911, 6106, 1370, 589, 2]
// Exports: default, useGetGamesForAppIds

// Module 9790 (useGetGameForAppId)
...
export const useGetGamesForAppIds = function useGetGamesForAppIds(stateFromStoresArray) {
  const tmp = memo(5911)(stateFromStoresArray);
  const _require = tmp;
  const items = [tmp];
  memo = React.useMemo(() => {
  ...
  const games = _require(6106).useGames(memo);
  const obj = _require(6106);
