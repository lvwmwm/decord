...
// Dependencies: [4895, 636, 7382, 4896, 2]
// Exports: queryGamesAutocomplete

// Module 7381 (queryGamesAutocomplete)
import set from "set";
...
let closure_3 = require("debounce")((arg0) => {
  const useGameAutocomplete = require(7382) /* fetchStore */.useGameAutocomplete;
  const items = [arg0];
...
export const queryGamesAutocomplete = function queryGamesAutocomplete(query) {
  const result = require(4896) /* GAME_AUTOCOMPLETE_MAX_QUERY_LENGTH */.normalizeGameAutocompleteQuery(query);
  let found = null;
  if (null != result) {
    callback(result);
    ...
    found = closestResults.filter(require(4896) /* GAME_AUTOCOMPLETE_MAX_QUERY_LENGTH */.isGameAutocompleteResultAllowedInGameWidgets);
  }
  return found;
};
