...
// Dependencies: [7090, 574, 7100, 7091, 2]
// Exports: queryGamesAutocomplete

// Module 7099 (queryGamesAutocomplete)
...
let closure_3 = require("debounce")((arg0) => {
  const useGameAutocomplete = require(7100) /* fetchStore */.useGameAutocomplete;
  const items = [arg0];
...
export const queryGamesAutocomplete = function queryGamesAutocomplete(query) {
  const result = require(7091) /* GAME_AUTOCOMPLETE_MAX_QUERY_LENGTH */.normalizeGameAutocompleteQuery(query);
  let found = null;
  if (null != result) {
    callback(result);
    ...
    found = closestResults.filter(require(7091) /* GAME_AUTOCOMPLETE_MAX_QUERY_LENGTH */.isGameAutocompleteResultAllowedInGameWidgets);
  }
  return found;
};
