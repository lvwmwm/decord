...
// Dependencies: [19, 17, 644, 500, 705, 4235, 8790, 2]
...
// Module 8789 (handleOrientationChange)
  ...
  if (c8) {
    if ("LANDSCAPE" === LANDSCAPE) {
      if ("LANDSCAPE" === orientationLock) {
        ...
        importDefault(8790).ignoreAutoRotate(false);
        const obj9 = importDefault(8790);
        const result = importDefault(8790).unlockAllOrientations();
        const obj10 = importDefault(8790);
      ...
      if ("PORTRAIT" === orientationLock) {
        ...
        importDefault(8790).ignoreAutoRotate(false);
        const obj4 = importDefault(8790);
        const result1 = importDefault(8790).unlockAllOrientations();
        const obj5 = importDefault(8790);
        require(705) /* batchUpdates */.batchUpdates(() => {
          state.setState({ orientationLock: null });
        });
        c8 = false;
...
export const lockOrientation = function lockOrientation(PORTRAIT, flag) {
  ...
  importDefault(8790).ignoreAutoRotate(flag);
  let c8 = false;
  if ("LANDSCAPE" === PORTRAIT) {
...
export const toggleOrientation = function toggleOrientation() {
  ...
  if (orientation === obj.LANDSCAPE) {
    if ("LANDSCAPE" !== orientationLock) {
      importDefault(8790).lockToPortrait();
      const obj4 = importDefault(8790);
      const result = importDefault(8790).unlockAllOrientations();
      const obj5 = importDefault(8790);
...
export const restoreDefaultOrientation = function restoreDefaultOrientation() {
  ...
  importDefault(8790).ignoreAutoRotate(false);
  const obj6 = importDefault(8790);
  const result = importDefault(8790).unlockAllOrientations();
  const obj7 = importDefault(8790);
  require(705) /* batchUpdates */.batchUpdates(() => {
    state.setState({ orientationLock: null });
  });
