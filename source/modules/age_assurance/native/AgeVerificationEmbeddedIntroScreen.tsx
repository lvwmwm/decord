...
// Dependencies: [19, 676, 21, 4302, 712, 7744, 7891, 7899, 7747, 7748, 4710, 5967, 4298, 4517, 7901, 1236, 2802, 7737, 1974, 2]
// Exports: default

// Module 7898 (AgeVerificationEmbeddedIntroScreen)
import set from "set";
...
export default function AgeVerificationEmbeddedIntroScreen(arg0) {
  ...
  const items1 = [callback(navigation(5967).ShieldSpotIllustration, { height: 100, width: 177 }), ];
  ...
  let obj7 = navigation(4517);
  obj4[3] = obj7.getAgeVerificationGetStartedTitle(entryPoint);
  const items2 = [callback(navigation(4298).Text, obj4), ];
  const obj5 = { variant: "text-md/medium", color: "text-strong", style: tmp.header, children: null };
  obj5[3] = navigation(4517).getAgeVerificationGetStartedSubtitle(entryPoint, undefined, isSuspendedUser);
  items2[1] = callback(navigation(4298).Text, obj5);
  obj3[3] = items2;
  items1[1] = callback2(navigation(4710).Stack, obj3);
  obj2[3] = items1;
  const items3 = [callback2(navigation(4710).Stack, obj2), callback(navigation(7901).AgeVerificationMethodsContainer, { ageVerificationMethods: importDefault(7899)({ onClose, classificationId, onGoogleWalletSelect: callback }).ageVerificationMethods, modalSessionId })];
  obj1[3] = items3;
  const items4 = [callback2(navigation(4710).Stack, obj1), ];
  ...
  items4[1] = callback(navigation(4298).Text, obj6);
  obj[0] = items4;
  obj[0] = callback2(navigation(7748).ModalContent, obj);
  return callback(navigation(7747).ModalScreen, obj);
};
