...
// Dependencies: [19, 676, 21, 4661, 712, 5437, 6689, 6697, 5440, 5445, 4733, 5447, 4734, 4992, 6699, 1236, 2853, 5428, 1993, 2]
// Exports: default

// Module 6696 (AgeVerificationEmbeddedIntroScreen)
import set from "set";
...
export default function AgeVerificationEmbeddedIntroScreen(arg0) {
  ...
  const items1 = [callback(navigation(5447).ShieldSpotIllustration, { height: 100, width: 177 }), ];
  ...
  let obj7 = navigation(4992);
  obj4[3] = obj7.getAgeVerificationGetStartedTitle(entryPoint);
  const items2 = [callback(navigation(4734).Text, obj4), ];
  const obj5 = { variant: "text-md/medium", color: "text-strong", style: tmp.header, children: null };
  obj5[3] = navigation(4992).getAgeVerificationGetStartedSubtitle(entryPoint, undefined, isSuspendedUser);
  items2[1] = callback(navigation(4734).Text, obj5);
  obj3[3] = items2;
  items1[1] = callback2(navigation(4733).Stack, obj3);
  obj2[3] = items1;
  const items3 = [callback2(navigation(4733).Stack, obj2), callback(navigation(6699).AgeVerificationMethodsContainer, { ageVerificationMethods: importDefault(6697)({ onClose, classificationId, onGoogleWalletSelect: callback }).ageVerificationMethods, modalSessionId })];
  obj1[3] = items3;
  const items4 = [callback2(navigation(4733).Stack, obj1), ];
  ...
  obj6[3] = intl.format(importDefault(2853).lG69e1, obj7);
  items4[1] = callback(navigation(4734).Text, obj6);
  obj[0] = items4;
  obj[0] = callback2(navigation(5445).ModalContent, obj);
  return callback(navigation(5440).ModalScreen, obj);
};
