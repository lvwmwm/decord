...
// Dependencies: [19, 673, 21, 4481, 709, 8348, 8506, 8514, 8351, 8352, 4934, 5647, 4477, 4741, 8516, 1233, 2918, 8341, 1994, 2]
// Exports: default

// Module 8513 (AgeVerificationEmbeddedIntroScreen)
import ThemesDefault from "Themes" /* 709 */;
import messagesProxyDefault from "messagesProxy" /* 2918 */;
import useAgeVerificationMethodsDefault from "useAgeVerificationMethods" /* 8514 */;
...
export default function AgeVerificationEmbeddedIntroScreen(arg0) {
  ...
  const items1 = [callback(navigation(5647).ShieldSpotIllustration, { height: 100, width: 177 }), ];
  ...
  let obj7 = navigation(4741);
  obj4[3] = obj7.getAgeVerificationGetStartedTitle(entryPoint);
  const items2 = [callback(navigation(4477).Text, obj4), ];
  const obj5 = { variant: "text-md/medium", color: "text-strong", style: tmp.header, children: navigation(4741).getAgeVerificationGetStartedSubtitle(entryPoint, undefined, isSuspendedUser) };
  items2[1] = callback(navigation(4477).Text, obj5);
  obj3[3] = items2;
  items1[1] = callback2(navigation(4934).Stack, obj3);
  obj2[3] = items1;
  const items3 = [callback2(navigation(4934).Stack, obj2), callback(navigation(8516).AgeVerificationMethodsContainer, { ageVerificationMethods: useAgeVerificationMethodsDefault({ onClose, classificationId, onGoogleWalletSelect: callback }).ageVerificationMethods, modalSessionId })];
  obj1[3] = items3;
  const items4 = [callback2(navigation(4934).Stack, obj1), ];
  ...
  items4[1] = callback(navigation(4477).Text, obj6);
  obj[0] = items4;
  obj[0] = callback2(navigation(8352).ModalContent, obj);
  return callback(navigation(8351).ModalScreen, obj);
};
