...
// Dependencies: [109, 1376, 4153, 1882, 1372, 1942, 1874, 4271, 3906, 3907, 676, 3, 12794, 12778, 9, 589, 12823, 1342, 6845, 12824, 709, 12, 7135, 1853, 1885, 8164, 7116, 12900, 4574, 5771, 8091, 8687, 4583, 4476, 12901, 12903, 1852, 12905, 2]
// Exports: default

// Module 12822 (defineSimpleDispatch)
...
function dispatchGuildMemberAdd(guild_id, author, member) {
  ...
  const result1 = obj(1885).parseServerDisplayNameStyles(display_name_styles);
  ...
  if (isEqualResult) {
    ...
    isEqualResult = obj(1852).isEqualAvatarDecoration(avatarDecoration, tmp9);
    const tmp2Result = obj(1852);
  }
  if (isEqualResult) {
...
let result1 = definePreloadableDispatch(["READY"], (guilds) => {
  ...
  const result = importAll(12823).preloadReadyPayloadData();
    ...
    ready.measure(() => {
      ...
      Emitter.batched(() => {
        ...
        const measureResult = hydrateReady.measure(() => {
          const obj = callback(12823);
          return obj.hydrateReadyPayloadPrioritized(obj, obj(12778).socket.identifyStartTime, closure_1);
        });
        let obj = measureResult;
        ...
        outer1_1(outer1_3[18])("AllGatewayConnectionStores", () => obj(12824));
...
defineSimpleDispatch(["RESUMED"], () => {
  const localPresenceState = obj(12778).localPresenceState;
  localPresenceState.forceUpdate();
  const localVoiceState = obj(12778).localVoiceState;
...
const result12 = definePreloadableDispatch(["THREAD_LIST_SYNC"], (guild_id) => {
  ...
  let mapped;
  if (guildId.members) {
    mapped = importDefault(12).map(guildId.members, importDefault(7135));
...
defineSimpleDispatch(["GUILD_BAN_ADD", "GUILD_BAN_REMOVE", "GUILD_MEMBER_ADD", "GUILD_MEMBER_UPDATE", "GUILD_MEMBER_REMOVE"], (guildId, type) => {
  ...
  obj[13] = obj(1853).parseServerUserCollectibles(guildId.collectibles);
  const obj2 = obj(1853);
  obj[14] = obj(1885).parseServerDisplayNameStyles(guildId.display_name_styles);
  const obj3 = obj(1885);
...
defineSimpleDispatch(["PRESENCE_UPDATE"], (hidden_activities) => {
  ...
  const tmpResult = importDefault(12905)(activities, hidden_activities);
  const tmp = importDefault(12905);
...
defineSimpleDispatch(["SAVED_MESSAGE_CREATE"], (body) => {
  ...
  obj[1] = obj(8164).savedMessageCreateObjectToClient(body);
  const obj2 = obj(8164);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["SAVED_MESSAGE_DELETE"], (channelId) => {
  ...
  obj[1] = obj(8164).savedMessageDeleteObjectToClient(channelId);
  const obj2 = obj(8164);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE"], (channelId) => {
  ...
  obj = { type: str, channelId: channelId.channel_id, messageId: channelId.message_id, userId: channelId.user_id, emoji: obj, reactionType: obj(7116).ReactionTypes.VOTE };
...
defineSimpleDispatch(["USER_PAYMENT_SOURCES_UPDATE"], () => {
  if (obj.isUserSettingsOpen()) {
    const paymentSources = require(4574) /* _deletePaymentSource */.fetchPaymentSources();
    const tmpResult = require(4574) /* _deletePaymentSource */;
    const subscriptionPlansBySKUs = importAll(5771).fetchSubscriptionPlansBySKUs(fetchedSKUIDs.getFetchedSKUIDs());
    const obj3 = importAll(5771);
  }
});
defineSimpleDispatch(["USER_SUBSCRIPTIONS_UPDATE"], () => {
  const currentUser = importAll(8091).fetchCurrentUser();
  const obj = importAll(8091);
  const tmp3 = require;
  if (obj2.isUserSettingsOpen()) {
    const subscriptions = tmp3(4574).fetchSubscriptions();
...
defineSimpleDispatch(["USER_BADGE_STATE_UPDATE"], (badge_id) => {
  const badge = require(8687) /* urlUserId */.fetchBadge(badge_id.badge_id);
});
...
defineSimpleDispatch(["USER_PAYMENT_CLIENT_ADD"], (arg0) => {
  const _require = arg0;
  const purchaseTokenHash = _require(4583).getPurchaseTokenHash();
...
defineSimpleDispatch(["INTERACTION_MODAL_CREATE"], (id) => {
  ...
  obj[6] = obj(4476).transformComponents(id.components);
...
defineSimpleDispatch(["GUILD_ROOM_CONNECT"], (body, type) => {
  ...
  obj[1] = obj(12901).serverGuildRoomToClient(body);
  const obj2 = obj(12901);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj(outer1_3[13]).socket;
...
defineSimpleDispatch(["HAVEN_CONNECT"], (roomId, type) => {
  const obj = { type, room: null };
  obj[1] = obj(12903).parseRoom(roomId);
  const obj2 = obj(12903);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj(outer1_3[13]).socket;
