...
// Dependencies: [109, 1376, 4087, 1882, 1372, 1942, 1874, 4205, 3840, 3841, 676, 3, 12701, 12685, 9, 589, 12730, 1342, 5748, 12731, 709, 12, 6091, 1853, 1885, 9550, 6072, 12807, 4509, 6298, 7982, 8109, 4518, 4411, 12808, 12810, 1852, 12812, 2]
// Exports: default

// Module 12729 (defineSimpleDispatch)
...
let result1 = definePreloadableDispatch(["READY"], (guilds) => {
  ...
  const result = importAll(12730).preloadReadyPayloadData();
    ...
    ready.measure(() => {
      ...
      Emitter.batched(() => {
        ...
        const measureResult = hydrateReady.measure(() => {
          const obj = callback(12730);
          return obj.hydrateReadyPayloadPrioritized(obj, obj(12685).socket.identifyStartTime, closure_1);
        });
        let obj = measureResult;
        ...
        outer1_1(outer1_3[18])("AllGatewayConnectionStores", () => obj(12731));
...
defineSimpleDispatch(["RESUMED"], () => {
  const localPresenceState = obj(12685).localPresenceState;
  localPresenceState.forceUpdate();
  const localVoiceState = obj(12685).localVoiceState;
...
const result12 = definePreloadableDispatch(["THREAD_LIST_SYNC"], (guild_id) => {
  ...
  let mapped;
  if (guildId.members) {
    mapped = importDefault(12).map(guildId.members, importDefault(6091));
...
defineSimpleDispatch(["PRESENCE_UPDATE"], (hidden_activities) => {
  ...
  const tmpResult = importDefault(12812)(activities, hidden_activities);
  const tmp = importDefault(12812);
...
defineSimpleDispatch(["SAVED_MESSAGE_CREATE"], (body) => {
  ...
  obj[1] = obj(9550).savedMessageCreateObjectToClient(body);
  const obj2 = obj(9550);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["SAVED_MESSAGE_DELETE"], (channelId) => {
  ...
  obj[1] = obj(9550).savedMessageDeleteObjectToClient(channelId);
  const obj2 = obj(9550);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE"], (channelId) => {
  ...
  obj = { type: str, channelId: channelId.channel_id, messageId: channelId.message_id, userId: channelId.user_id, emoji: obj, reactionType: obj(6072).ReactionTypes.VOTE };
...
defineSimpleDispatch(["USER_PAYMENT_SOURCES_UPDATE"], () => {
  if (obj.isUserSettingsOpen()) {
    const paymentSources = require(4509) /* _deletePaymentSource */.fetchPaymentSources();
    const tmpResult = require(4509) /* _deletePaymentSource */;
    const subscriptionPlansBySKUs = importAll(6298).fetchSubscriptionPlansBySKUs(fetchedSKUIDs.getFetchedSKUIDs());
    const obj3 = importAll(6298);
  }
});
defineSimpleDispatch(["USER_SUBSCRIPTIONS_UPDATE"], () => {
  const currentUser = importAll(7982).fetchCurrentUser();
  const obj = importAll(7982);
  const tmp3 = require;
  if (obj2.isUserSettingsOpen()) {
    const subscriptions = tmp3(4509).fetchSubscriptions();
...
defineSimpleDispatch(["USER_BADGE_STATE_UPDATE"], (badge_id) => {
  const badge = require(8109) /* urlUserId */.fetchBadge(badge_id.badge_id);
});
...
defineSimpleDispatch(["USER_PAYMENT_CLIENT_ADD"], (arg0) => {
  const _require = arg0;
  const purchaseTokenHash = _require(4518).getPurchaseTokenHash();
...
defineSimpleDispatch(["INTERACTION_MODAL_CREATE"], (id) => {
  ...
  obj[6] = obj(4411).transformComponents(id.components);
...
defineSimpleDispatch(["GUILD_ROOM_CONNECT"], (body, type) => {
  ...
  obj[1] = obj(12808).serverGuildRoomToClient(body);
  const obj2 = obj(12808);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj(outer1_3[13]).socket;
...
defineSimpleDispatch(["HAVEN_CONNECT"], (roomId, type) => {
  const obj = { type, room: null };
  obj[1] = obj(12810).parseRoom(roomId);
  const obj2 = obj(12810);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj(outer1_3[13]).socket;
