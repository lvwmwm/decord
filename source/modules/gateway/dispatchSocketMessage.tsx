...
// Dependencies: [109, 1376, 4200, 1911, 1372, 1971, 1903, 4318, 3952, 3953, 676, 3, 12846, 12830, 9, 589, 12875, 1342, 6891, 12876, 709, 12, 7181, 1881, 1914, 8321, 7162, 12951, 4621, 5817, 8249, 8733, 4630, 4523, 12952, 12954, 1880, 12956, 2]
// Exports: default

// Module 12874 (defineSimpleDispatch)
...
let result1 = definePreloadableDispatch(["READY"], (guilds) => {
  ...
  const result = importAll(12875).preloadReadyPayloadData();
    ...
    ready.measure(() => {
      ...
      Emitter.batched(() => {
        ...
        const measureResult = hydrateReady.measure(() => {
          const obj = callback(12875);
          return obj.hydrateReadyPayloadPrioritized(obj, obj(12830).socket.identifyStartTime, closure_1);
        });
        let obj = measureResult;
        ...
        outer1_1(outer1_3[18])("AllGatewayConnectionStores", () => obj(12876));
...
defineSimpleDispatch(["RESUMED"], () => {
  const localPresenceState = obj(12830).localPresenceState;
  localPresenceState.forceUpdate();
  const localVoiceState = obj(12830).localVoiceState;
...
const result12 = definePreloadableDispatch(["THREAD_LIST_SYNC"], (guild_id) => {
  ...
  let mapped;
  if (guildId.members) {
    mapped = importDefault(12).map(guildId.members, importDefault(7181));
...
defineSimpleDispatch(["PRESENCE_UPDATE"], (hidden_activities) => {
  ...
  const tmpResult = importDefault(12956)(activities, hidden_activities);
  const tmp = importDefault(12956);
...
defineSimpleDispatch(["SAVED_MESSAGE_CREATE"], (body) => {
  ...
  obj[1] = obj(8321).savedMessageCreateObjectToClient(body);
  const obj2 = obj(8321);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["SAVED_MESSAGE_DELETE"], (channelId) => {
  ...
  obj[1] = obj(8321).savedMessageDeleteObjectToClient(channelId);
  const obj2 = obj(8321);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE"], (channelId) => {
  ...
  obj = { type: str, channelId: channelId.channel_id, messageId: channelId.message_id, userId: channelId.user_id, emoji: obj, reactionType: obj(7162).ReactionTypes.VOTE };
...
defineSimpleDispatch(["USER_PAYMENT_SOURCES_UPDATE"], () => {
  if (obj.isUserSettingsOpen()) {
    const paymentSources = require(4621) /* _deletePaymentSource */.fetchPaymentSources();
    const tmpResult = require(4621) /* _deletePaymentSource */;
    const subscriptionPlansBySKUs = importAll(5817).fetchSubscriptionPlansBySKUs(fetchedSKUIDs.getFetchedSKUIDs());
    const obj3 = importAll(5817);
  }
});
defineSimpleDispatch(["USER_SUBSCRIPTIONS_UPDATE"], () => {
  const currentUser = importAll(8249).fetchCurrentUser();
  const obj = importAll(8249);
  const tmp3 = require;
  if (obj2.isUserSettingsOpen()) {
    const subscriptions = tmp3(4621).fetchSubscriptions();
    const tmp3Result = tmp3(4621);
  }
});
...
defineSimpleDispatch(["USER_BADGE_STATE_UPDATE"], (badge_id) => {
  const badge = require(8733) /* urlUserId */.fetchBadge(badge_id.badge_id);
});
...
defineSimpleDispatch(["USER_PAYMENT_CLIENT_ADD"], (arg0) => {
  const _require = arg0;
  const purchaseTokenHash = _require(4630).getPurchaseTokenHash();
...
defineSimpleDispatch(["INTERACTION_MODAL_CREATE"], (id) => {
  ...
  obj[6] = obj(4523).transformComponents(id.components);
...
defineSimpleDispatch(["GUILD_ROOM_CONNECT"], (body, type) => {
  ...
  obj[1] = obj(12952).serverGuildRoomToClient(body);
  const obj2 = obj(12952);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj(outer1_3[13]).socket;
...
defineSimpleDispatch(["HAVEN_CONNECT"], (roomId, type) => {
  const obj = { type, room: null };
  obj[1] = obj(12954).parseRoom(roomId);
  const obj2 = obj(12954);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj(outer1_3[13]).socket;
