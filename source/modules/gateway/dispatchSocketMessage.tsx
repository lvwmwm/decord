...
// Dependencies: [109, 1391, 4345, 1930, 1387, 1992, 1922, 4467, 4114, 4115, 676, 3, 13269, 13252, 9, 589, 13298, 1342, 13299, 7205, 13302, 709, 1937, 12, 7494, 1899, 1933, 7572, 7475, 13380, 4805, 6072, 8084, 8531, 4814, 4707, 4650, 1898, 13381, 2]
// Exports: default

// Module 13297 (defineSimpleDispatch)
import timestampDefault from "timestamp" /* 3 */;
import serializeDefault from "serialize" /* 9 */;
import applyDefault from "apply" /* 12 */;
import initializeDefault from "initialize" /* 589 */;
import dispatcherDefault from "dispatcher" /* 709 */;
import _deletePaymentSource from "_deletePaymentSource" /* 4805 */;
import fetchSubscriptionPlansForSKUAll from "fetchSubscriptionPlansForSKU" /* 6072 */;
import convertServerThreadMemberDefault from "convertServerThreadMember" /* 7494 */;
import _fetchProfileAll from "_fetchProfile" /* 8084 */;
import urlUserId from "urlUserId" /* 8531 */;
import hydrateGuildAll from "hydrateGuild" /* 13298 */;
import splitAgeRestrictedActivitiesDefault from "splitAgeRestrictedActivities" /* 13381 */;
import closure_5 from "_objectWithoutProperties" /* 109 */;
import { createChannelRecordFromServer as closure_6 } from "createChannelRecord" /* 1391 */;
import closure_7 from "createFromServer" /* 4345 */;
...
import closure_13 from "updateVoiceState" /* 4467 */;
import closure_14 from "addSubscriptionPlan" /* 4114 */;
import closure_15 from "reset" /* 4115 */;
import { ActivityTypes } from "ME" /* 676 */;
import importDefaultResult from "flush" /* 13269 */;
    ...
    ready.measure(() => {
      ...
      Emitter.batched(() => {
        ...
        const measureResult = hydrateReady.measure(() => {
          obj = callback(13298);
          return obj.hydrateReadyPayloadPrioritized(obj, obj(13252).socket.identifyStartTime, closure_1);
        });
        obj = measureResult;
        ...
        closure_1_1(closure_1_3[19])("AllGatewayConnectionStores", () => obj(13302));
...
defineSimpleDispatch(["RESUMED"], () => {
  const localPresenceState = obj(13252).localPresenceState;
  localPresenceState.forceUpdate();
  const localVoiceState = obj(13252).localVoiceState;
...
defineSimpleDispatch(["SAVED_MESSAGE_CREATE"], (body) => {
  obj = { type: "SAVED_MESSAGE_CREATE", savedMessage: obj(7572).savedMessageCreateObjectToClient(body) };
  const obj2 = obj(7572);
  const obj3 = dispatcherDefault;
  dispatcherDefault.dispatch(obj).catch((error) => {
    closure_1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["SAVED_MESSAGE_DELETE"], (channelId) => {
  obj = { type: "SAVED_MESSAGE_DELETE", savedMessageData: obj(7572).savedMessageDeleteObjectToClient(channelId) };
  const obj2 = obj(7572);
  const obj3 = dispatcherDefault;
  dispatcherDefault.dispatch(obj).catch((error) => {
    closure_1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE"], (channelId) => {
  ...
  obj = { type: str, channelId: channelId.channel_id, messageId: channelId.message_id, userId: channelId.user_id, emoji: obj, reactionType: obj(7475).ReactionTypes.VOTE };
...
defineSimpleDispatch(["USER_PAYMENT_CLIENT_ADD"], (arg0) => {
  const _require = arg0;
  const purchaseTokenHash = _require(4814).getPurchaseTokenHash();
...
defineSimpleDispatch(["INTERACTION_MODAL_CREATE"], (id) => {
  obj = { type: "INTERACTION_MODAL_CREATE", id: id.id, channelId: id.channel_id, customId: id.custom_id, application: id.application, title: id.title, components: obj(4707).transformComponents(id.components), nonce: null, resolved: null };
...
defineSimpleDispatch(["GUILD_ROOM_CONNECT"], (body, type) => {
  obj = { type, room: obj(4650).serverGuildRoomToClient(body) };
  const obj2 = obj(4650);
  const obj3 = dispatcherDefault;
  dispatcherDefault.dispatch(obj).catch((error) => {
    closure_1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
