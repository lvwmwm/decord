...
// Dependencies: [109, 1376, 4087, 1882, 1372, 1942, 1874, 4205, 3840, 3841, 676, 3, 12707, 12691, 9, 589, 12736, 1342, 5748, 12737, 709, 12, 6093, 1853, 1885, 9554, 6074, 12813, 4509, 6301, 7985, 8112, 4518, 4411, 12814, 12816, 1852, 12818, 2]
// Exports: default

// Module 12735 (defineSimpleDispatch)
...
let result1 = definePreloadableDispatch(["READY"], (guilds) => {
  ...
  const result = importAll(12736).preloadReadyPayloadData();
    ...
    ready.measure(() => {
      ...
      Emitter.batched(() => {
        ...
        const measureResult = hydrateReady.measure(() => {
          const obj = callback(12736);
          return obj.hydrateReadyPayloadPrioritized(obj, obj(12691).socket.identifyStartTime, closure_1);
        });
        let obj = measureResult;
        ...
        outer1_1(outer1_3[18])("AllGatewayConnectionStores", () => obj(12737));
...
defineSimpleDispatch(["RESUMED"], () => {
  const localPresenceState = obj(12691).localPresenceState;
  localPresenceState.forceUpdate();
  const localVoiceState = obj(12691).localVoiceState;
...
const result12 = definePreloadableDispatch(["THREAD_LIST_SYNC"], (guild_id) => {
  ...
  let mapped;
  if (guildId.members) {
    mapped = importDefault(12).map(guildId.members, importDefault(6093));
...
defineSimpleDispatch(["PRESENCE_UPDATE"], (hidden_activities) => {
  ...
  const tmpResult = importDefault(12818)(activities, hidden_activities);
  const tmp = importDefault(12818);
...
defineSimpleDispatch(["SAVED_MESSAGE_CREATE"], (body) => {
  ...
  obj[1] = obj(9554).savedMessageCreateObjectToClient(body);
  const obj2 = obj(9554);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["SAVED_MESSAGE_DELETE"], (channelId) => {
  ...
  obj[1] = obj(9554).savedMessageDeleteObjectToClient(channelId);
  const obj2 = obj(9554);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE"], (channelId) => {
  ...
  obj = { type: str, channelId: channelId.channel_id, messageId: channelId.message_id, userId: channelId.user_id, emoji: obj, reactionType: obj(6074).ReactionTypes.VOTE };
...
defineSimpleDispatch(["USER_PAYMENT_SOURCES_UPDATE"], () => {
  if (obj.isUserSettingsOpen()) {
    ...
    const subscriptionPlansBySKUs = importAll(6301).fetchSubscriptionPlansBySKUs(fetchedSKUIDs.getFetchedSKUIDs());
    const obj3 = importAll(6301);
  }
});
defineSimpleDispatch(["USER_SUBSCRIPTIONS_UPDATE"], () => {
  const currentUser = importAll(7985).fetchCurrentUser();
  const obj = importAll(7985);
  const tmp3 = require;
...
defineSimpleDispatch(["USER_BADGE_STATE_UPDATE"], (badge_id) => {
  const badge = require(8112) /* urlUserId */.fetchBadge(badge_id.badge_id);
});
...
defineSimpleDispatch(["GUILD_ROOM_CONNECT"], (body, type) => {
  ...
  obj[1] = obj(12814).serverGuildRoomToClient(body);
  const obj2 = obj(12814);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj(outer1_3[13]).socket;
...
defineSimpleDispatch(["HAVEN_CONNECT"], (roomId, type) => {
  const obj = { type, room: null };
  obj[1] = obj(12816).parseRoom(roomId);
  const obj2 = obj(12816);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj(outer1_3[13]).socket;
