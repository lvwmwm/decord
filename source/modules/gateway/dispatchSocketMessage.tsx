...
// Dependencies: [109, 1395, 4240, 1930, 1391, 1990, 1922, 4361, 4012, 4013, 676, 3, 13000, 12984, 9, 589, 13029, 1342, 7021, 13030, 709, 12, 7308, 1900, 1933, 8450, 7289, 13105, 4686, 5939, 8379, 8856, 4695, 4588, 4542, 1899, 13106, 2]
// Exports: default

// Module 13028 (defineSimpleDispatch)
    ...
    Emitter.batched(() => {
      ...
      let item = found.forEach((presences) => {
        ...
        presences.presences = presences.presences || [].map((activities) => {
          ...
          const tmp2Result = outer1_1(outer1_3[36])(activities, hidden_activities);
          const tmp = id;
          const tmp2 = outer1_1(outer1_3[36]);
...
let result1 = definePreloadableDispatch(["READY"], (guilds) => {
  ...
  const result = importAll(13029).preloadReadyPayloadData();
    ...
    ready.measure(() => {
      ...
      Emitter.batched(() => {
        ...
        const measureResult = hydrateReady.measure(() => {
          const obj = callback(13029);
          return obj.hydrateReadyPayloadPrioritized(obj, obj(12984).socket.identifyStartTime, closure_1);
        });
        let obj = measureResult;
        ...
        outer1_1(outer1_3[18])("AllGatewayConnectionStores", () => obj(13030));
...
defineSimpleDispatch(["RESUMED"], () => {
  const localPresenceState = obj(12984).localPresenceState;
  localPresenceState.forceUpdate();
  const localVoiceState = obj(12984).localVoiceState;
...
const result12 = definePreloadableDispatch(["THREAD_LIST_SYNC"], (guild_id) => {
  ...
  let mapped;
  if (guildId.members) {
    mapped = importDefault(12).map(guildId.members, importDefault(7308));
...
defineSimpleDispatch(["PRESENCE_UPDATE"], (hidden_activities) => {
  ...
  const tmpResult = importDefault(13106)(activities, hidden_activities);
  const tmp = importDefault(13106);
...
defineSimpleDispatch(["SAVED_MESSAGE_CREATE"], (body) => {
  ...
  obj[1] = obj(8450).savedMessageCreateObjectToClient(body);
  const obj2 = obj(8450);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["SAVED_MESSAGE_DELETE"], (channelId) => {
  ...
  obj[1] = obj(8450).savedMessageDeleteObjectToClient(channelId);
  const obj2 = obj(8450);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE"], (channelId) => {
  ...
  obj = { type: str, channelId: channelId.channel_id, messageId: channelId.message_id, userId: channelId.user_id, emoji: obj, reactionType: obj(7289).ReactionTypes.VOTE };
...
defineSimpleDispatch(["USER_PAYMENT_SOURCES_UPDATE"], () => {
  if (obj.isUserSettingsOpen()) {
    const paymentSources = require(4686) /* _deletePaymentSource */.fetchPaymentSources();
    const tmpResult = require(4686) /* _deletePaymentSource */;
    const subscriptionPlansBySKUs = importAll(5939).fetchSubscriptionPlansBySKUs(fetchedSKUIDs.getFetchedSKUIDs());
    const obj3 = importAll(5939);
  }
});
defineSimpleDispatch(["USER_SUBSCRIPTIONS_UPDATE"], () => {
  const currentUser = importAll(8379).fetchCurrentUser();
  const obj = importAll(8379);
  const tmp3 = require;
  if (obj2.isUserSettingsOpen()) {
    const subscriptions = tmp3(4686).fetchSubscriptions();
    const tmp3Result = tmp3(4686);
  }
});
...
defineSimpleDispatch(["USER_BADGE_STATE_UPDATE"], (badge_id) => {
  const badge = require(8856) /* urlUserId */.fetchBadge(badge_id.badge_id);
});
...
defineSimpleDispatch(["USER_PAYMENT_CLIENT_ADD"], (arg0) => {
  const _require = arg0;
  const purchaseTokenHash = _require(4695).getPurchaseTokenHash();
    ...
    let obj = function handleItem(member) {
      if (null != member.member) {
        ...
        if (null != member.presence) {
          ...
          const tmp = outer1_1(outer1_3[36])(activities, hidden_activities);
          ...
          outer1_21.add(obj);
          const tmp9 = outer1_1(outer1_3[36]);
        }
        tmp5 = obj;
      }
    };
...
defineSimpleDispatch(["INTERACTION_MODAL_CREATE"], (id) => {
  ...
  obj[6] = obj(4588).transformComponents(id.components);
...
defineSimpleDispatch(["GUILD_ROOM_CONNECT"], (body, type) => {
  ...
  obj[1] = obj(4542).serverGuildRoomToClient(body);
  const obj2 = obj(4542);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
