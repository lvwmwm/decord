...
// Dependencies: [109, 1376, 4183, 1882, 1372, 1942, 1874, 4301, 3936, 3937, 676, 3, 12822, 12806, 9, 589, 12851, 1342, 6860, 12852, 709, 12, 7161, 1853, 1885, 8192, 7142, 12928, 4604, 5786, 8119, 8715, 4613, 4506, 12929, 12931, 1852, 12933, 2]
// Exports: default

// Module 12850 (defineSimpleDispatch)
...
let result1 = definePreloadableDispatch(["READY"], (guilds) => {
  ...
  const result = importAll(12851).preloadReadyPayloadData();
    ...
    ready.measure(() => {
      ...
      Emitter.batched(() => {
        ...
        const measureResult = hydrateReady.measure(() => {
          const obj = callback(12851);
          return obj.hydrateReadyPayloadPrioritized(obj, obj(12806).socket.identifyStartTime, closure_1);
        });
        let obj = measureResult;
        ...
        outer1_1(outer1_3[18])("AllGatewayConnectionStores", () => obj(12852));
...
defineSimpleDispatch(["RESUMED"], () => {
  const localPresenceState = obj(12806).localPresenceState;
  localPresenceState.forceUpdate();
  const localVoiceState = obj(12806).localVoiceState;
...
const result12 = definePreloadableDispatch(["THREAD_LIST_SYNC"], (guild_id) => {
  ...
  let mapped;
  if (guildId.members) {
    mapped = importDefault(12).map(guildId.members, importDefault(7161));
...
defineSimpleDispatch(["PRESENCE_UPDATE"], (hidden_activities) => {
  ...
  const tmpResult = importDefault(12933)(activities, hidden_activities);
  const tmp = importDefault(12933);
...
defineSimpleDispatch(["SAVED_MESSAGE_CREATE"], (body) => {
  ...
  obj[1] = obj(8192).savedMessageCreateObjectToClient(body);
  const obj2 = obj(8192);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["SAVED_MESSAGE_DELETE"], (channelId) => {
  ...
  obj[1] = obj(8192).savedMessageDeleteObjectToClient(channelId);
  const obj2 = obj(8192);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
...
defineSimpleDispatch(["MESSAGE_POLL_VOTE_ADD", "MESSAGE_POLL_VOTE_REMOVE"], (channelId) => {
  ...
  obj = { type: str, channelId: channelId.channel_id, messageId: channelId.message_id, userId: channelId.user_id, emoji: obj, reactionType: obj(7142).ReactionTypes.VOTE };
...
defineSimpleDispatch(["USER_SUBSCRIPTIONS_UPDATE"], () => {
  const currentUser = importAll(8119).fetchCurrentUser();
  const obj = importAll(8119);
  const tmp3 = require;
...
defineSimpleDispatch(["USER_BADGE_STATE_UPDATE"], (badge_id) => {
  const badge = require(8715) /* urlUserId */.fetchBadge(badge_id.badge_id);
});
...
defineSimpleDispatch(["GUILD_ROOM_CONNECT"], (body, type) => {
  ...
  obj[1] = obj(12929).serverGuildRoomToClient(body);
  const obj2 = obj(12929);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj(outer1_3[13]).socket;
...
defineSimpleDispatch(["HAVEN_CONNECT"], (roomId, type) => {
  const obj = { type, room: null };
  obj[1] = obj(12931).parseRoom(roomId);
  const obj2 = obj(12931);
  const obj3 = importDefault(709);
  importDefault(709).dispatch(obj).catch((error) => {
    outer1_17.error("dispatchOrResetSocket error during " + obj.type + ":", error);
    const socket = obj(outer1_3[13]).socket;
