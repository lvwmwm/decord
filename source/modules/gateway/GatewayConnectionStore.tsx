...
// Dependencies: [5, 1340, 1218, 4995, 1372, 4351, 4375, 4396, 1960, 4996, 4389, 676, 685, 12900, 12944, 3, 595, 5953, 5129, 500, 12936, 12, 4367, 12915, 589, 709, 2]

// Module 4994 (_handleConnectionOpen)
...
function handleClipsFlags() {
  const localVoiceState = require(12900) /* handleIdentify */.localVoiceState;
...
function handleLocalPresenceChange() {
  const localPresenceState = require(12900) /* handleIdentify */.localPresenceState;
...
prototype["getSocket"] = function getSocket() {
  return require(12900) /* handleIdentify */.socket;
};
prototype["isTryingToConnect"] = function isTryingToConnect() {
  const socket = require(12900) /* handleIdentify */.socket;
...
const gatewayConnectionStore = new GatewayConnectionStore(require("dispatcher"), {
  START_SESSION: function handleSessionStart() {
    ...
    if (socket.isClosed()) {
      ...
      const socket2 = require(12900) /* handleIdentify */.socket;
  ...
  CLEAR_CACHES: function handleClearCaches(resetSocket) {
    if (resetSocket.resetSocket) {
      const socket = require(12900) /* handleIdentify */.socket;
      socket.close();
      const dispatcher = require(12900) /* handleIdentify */.socket.dispatcher;
      dispatcher.clear();
      const socket2 = require(12900) /* handleIdentify */.socket;
      socket2.connect();
    }
  ...
  GUILD_DELETE: function handleGuildDelete(guild) {
    if (guild.guild.id === require(12900) /* handleIdentify */.localVoiceState.guildId) {
      const localVoiceState = require(12900) /* handleIdentify */.localVoiceState;
      localVoiceState.setState({ guildId: null, channelId: null });
    }
  },
  CHANNEL_DELETE: function handleChannelDelete(channel) {
    if (channel.channel.id === require(12900) /* handleIdentify */.localVoiceState.channelId) {
      const localVoiceState = require(12900) /* handleIdentify */.localVoiceState;
      localVoiceState.setState({ guildId: null, channelId: null });
    }
  },
  CALL_DELETE: function handleCallDelete(channelId) {
    ...
    if (channelId === require(12900) /* handleIdentify */.localVoiceState.channelId) {
      if (c24 === channelId) {
        return false;
      } else {
        const localVoiceState = require(12900) /* handleIdentify */.localVoiceState;
  ...
  GUILD_MEMBERS_REQUEST: function handleGuildMembersRequest(userIds) {
    ...
    let socket = _require(12900).socket;
    if (socket.isSessionEstablished()) {
      if ("userIds" in userIds) {
        ...
        const socket2 = _require(12900).socket;
  ...
  CALL_CONNECT_MULTIPLE: function handleCallConnectMultiple(channelIds) {
    ...
    let socket = require(12900) /* handleIdentify */.socket;
    if (socket.isSessionEstablished()) {
  ...
  STREAM_WATCH: function handleStreamWatch(arg0) {
    ...
    if (socket.isSessionEstablished()) {
      ...
      let socket2 = _require(12900).socket;
  ...
  RESET_SOCKET: function handleResetSocket(args) {
    if (require(12900) /* handleIdentify */.socket.connectionState !== importDefault(12915).WILL_RECONNECT) {
      const socket = require(12900) /* handleIdentify */.socket;
