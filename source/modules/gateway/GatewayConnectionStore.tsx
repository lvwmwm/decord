...
// Dependencies: [5, 1340, 1218, 4866, 1372, 4236, 4261, 4282, 1931, 4867, 4275, 676, 685, 12685, 12729, 3, 595, 8045, 5001, 500, 12721, 12, 4253, 12700, 589, 709, 2]

// Module 4865 (_handleConnectionOpen)
        ...
        try {
          ...
          if (0 === dependencyMap) {
            if (arg0 === 1) {
              ...
              const localPresenceState = callback(12685).localPresenceState;
              ...
              if (null != voiceChannelId) {
                ...
                if (null != guildId) {
                  ...
                  callback(8045).muteCustomJoinSound(voiceChannelId);
                  const obj5 = callback(8045);
                }
              }
              const localVoiceState = callback(12685).localVoiceState;
...
function handleClipsFlags() {
  const localVoiceState = require(12685) /* handleIdentify */.localVoiceState;
...
function handleLocalPresenceChange() {
  const localPresenceState = require(12685) /* handleIdentify */.localPresenceState;
...
prototype["getSocket"] = function getSocket() {
  return require(12685) /* handleIdentify */.socket;
};
prototype["isTryingToConnect"] = function isTryingToConnect() {
  const socket = require(12685) /* handleIdentify */.socket;
...
const gatewayConnectionStore = new GatewayConnectionStore(require("dispatcher"), {
  START_SESSION: function handleSessionStart() {
    ...
    if (socket.isClosed()) {
      ...
      const socket2 = require(12685) /* handleIdentify */.socket;
  ...
  LOGIN_SUCCESS: function handleSessionRefresh() {
    ...
    if (connectResult) {
      const socket3 = tmp(12685).socket;
      socket3.close();
      const socket4 = tmp(12685).socket;
  ...
  CLEAR_CACHES: function handleClearCaches(resetSocket) {
    if (resetSocket.resetSocket) {
      const socket = require(12685) /* handleIdentify */.socket;
      socket.close();
      const dispatcher = require(12685) /* handleIdentify */.socket.dispatcher;
      dispatcher.clear();
      const socket2 = require(12685) /* handleIdentify */.socket;
      socket2.connect();
    }
  ...
  VOICE_CHANNEL_SELECT: function handleVoiceChannelSelect(guildId) {
    ...
    if (isIOSResult) {
      if (null == guildId.channelId) {
        ...
        const socket = tmp(12685).socket;
        if (socket.isClosed()) {
          importAll(12721).setIsPaused(false);
          const socket2 = tmp(12685).socket;
          socket2.connect();
          const obj3 = importAll(12721);
        }
      }
    }
    return false;
      ...
      if (id.getId() !== userId.userId) {
        ...
        if (userId.sessionId === closure_21) {
          if (null != closure_24) {
            ...
            const localVoiceState2 = callback(12685).localVoiceState;
            const obj = { guildId: null, channelId: null };
            ({ guildId: obj[0], channelId: obj[1] } = userId);
            localVoiceState2.setState(obj);
          }
        } else {
          if (userId.guildId !== callback(12685).localVoiceState.guildId) {
            return arg0;
          } else {
            const localVoiceState = tmp(12685).localVoiceState;
  ...
  GUILD_DELETE: function handleGuildDelete(guild) {
    if (guild.guild.id === require(12685) /* handleIdentify */.localVoiceState.guildId) {
      const localVoiceState = require(12685) /* handleIdentify */.localVoiceState;
      localVoiceState.setState({ guildId: null, channelId: null });
    }
  },
  CHANNEL_DELETE: function handleChannelDelete(channel) {
    if (channel.channel.id === require(12685) /* handleIdentify */.localVoiceState.channelId) {
      const localVoiceState = require(12685) /* handleIdentify */.localVoiceState;
      localVoiceState.setState({ guildId: null, channelId: null });
    }
  },
  CALL_DELETE: function handleCallDelete(channelId) {
    ...
    if (channelId === require(12685) /* handleIdentify */.localVoiceState.channelId) {
      if (c24 === channelId) {
        return false;
      } else {
        const localVoiceState = require(12685) /* handleIdentify */.localVoiceState;
  ...
  APP_STATE_UPDATE: function handleFocus(arg0) {
    ...
    if (obj.isIOS()) {
      if (!fetchFingerprint.isAuthenticated()) {
        ...
        if (isClosedResult) {
          const socket3 = socket(12685).socket;
          isClosedResult = socket3.isClosed();
        }
        if (isClosedResult) {
          importAll(12721).setIsPaused(false);
          const socket4 = socket(12685).socket;
          socket4.connect();
          const obj2 = importAll(12721);
        }
      }
      socket = socket(12685).socket;
      socket.close(true);
    } else {
      if (state.state === constants2.ACTIVE) {
        importAll(12721).setIsPaused(false);
        if (fetchFingerprint.isAuthenticated()) {
          const socket2 = socket(12685).socket;
  ...
  GUILD_MEMBERS_REQUEST: function handleGuildMembersRequest(userIds) {
    ...
    let socket = _require(12685).socket;
    if (socket.isSessionEstablished()) {
      if ("userIds" in userIds) {
        ...
        const socket2 = _require(12685).socket;
  ...
  CALL_CONNECT_MULTIPLE: function handleCallConnectMultiple(channelIds) {
    ...
    let socket = require(12685) /* handleIdentify */.socket;
    if (socket.isSessionEstablished()) {
  ...
  STREAM_WATCH: function handleStreamWatch(arg0) {
    ...
    if (socket.isSessionEstablished()) {
      if (!allowMultiple) {
        ...
        _require = allActiveStreamKeys.find((streamKey) => callback(4253).decodeStreamKey(streamKey).ownerId === id.getId());
        ...
        const item = found.forEach((streamKey) => {
          const socket = callback(12685).socket;
          if (socket.isSessionEstablished()) {
            const socket2 = callback(12685).socket;
            socket2.streamDelete(streamKey);
          }
        });
      }
      let socket2 = _require(12685).socket;
  ...
  RESET_SOCKET: function handleResetSocket(args) {
    if (require(12685) /* handleIdentify */.socket.connectionState !== importDefault(12700).WILL_RECONNECT) {
      const socket = require(12685) /* handleIdentify */.socket;
