...
// Dependencies: [6, 7, 15, 17, 18, 5, 1316, 1194, 4810, 1348, 4178, 4203, 4224, 1907, 4811, 4217, 653, 662, 12626, 12670, 3, 587, 7986, 4945, 477, 12662, 22, 4195, 12641, 566, 686, 2]
...
function deleteStream(streamKey) {
  const socket = require(12626) /* importDefaultResult1 */.socket;
  if (socket.isSessionEstablished()) {
    const socket2 = require(12626) /* importDefaultResult1 */.socket;
...
tmp5 = new tmp5(require("dispatcher"), {
  ...
  LOGIN_SUCCESS: function handleSessionRefresh() {
    ...
    if (!tmp2) {
      const socket3 = require(12626) /* importDefaultResult1 */.socket;
      socket3.close();
      const socket4 = require(12626) /* importDefaultResult1 */.socket;
  ...
  CLEAR_CACHES: function handleClearCaches(resetSocket) {
    if (resetSocket.resetSocket) {
      const socket = require(12626) /* importDefaultResult1 */.socket;
      socket.close();
      const dispatcher = require(12626) /* importDefaultResult1 */.socket.dispatcher;
      dispatcher.clear();
      const socket2 = require(12626) /* importDefaultResult1 */.socket;
      socket2.connect();
    }
  ...
  GUILD_DELETE: function handleGuildDelete(guild) {
    if (guild.guild.id === require(12626) /* importDefaultResult1 */.localVoiceState.guildId) {
      const localVoiceState = require(12626) /* importDefaultResult1 */.localVoiceState;
      localVoiceState.setState({ guildId: null, channelId: null });
    }
  },
  CHANNEL_DELETE: function handleChannelDelete(channel) {
    if (channel.channel.id === require(12626) /* importDefaultResult1 */.localVoiceState.channelId) {
      const localVoiceState = require(12626) /* importDefaultResult1 */.localVoiceState;
      localVoiceState.setState({ guildId: null, channelId: null });
    }
  },
  CALL_DELETE: function handleCallDelete(channelId) {
    ...
    if (channelId === require(12626) /* importDefaultResult1 */.localVoiceState.channelId) {
      if (c29 === channelId) {
        return false;
      } else {
        const localVoiceState = require(12626) /* importDefaultResult1 */.localVoiceState;
  ...
  GUILD_MEMBERS_REQUEST: function handleGuildMembersRequest(userIds) {
    ...
    let socket = _require(12626).socket;
    if (socket.isSessionEstablished()) {
      if ("userIds" in userIds) {
        ...
        const socket2 = _require(12626).socket;
        const obj = {};
  ...
  CALL_CONNECT_MULTIPLE: function handleCallConnectMultiple(channelIds) {
    ...
    let socket = require(12626) /* importDefaultResult1 */.socket;
    if (socket.isSessionEstablished()) {
  ...
  RESET_SOCKET: function handleResetSocket(args) {
    if (require(12626) /* importDefaultResult1 */.socket.connectionState !== importDefault(12641).WILL_RECONNECT) {
      const socket = require(12626) /* importDefaultResult1 */.socket;
