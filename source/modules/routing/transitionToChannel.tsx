...
// Dependencies: [1372, 676, 4198, 4199, 1222, 4383, 38, 2]
...
// Module 4197 (transitionToChannel)
...
export const transitionToChannel = function transitionToChannel(id, openTextInVoiceIfVoiceChannel) {
  ...
  if (null != channel) {
    const guildIdForGenericRedirect = require(4198) /* useGuildIdForChannelRoute */.getGuildIdForGenericRedirect(channel);
    const obj3 = require(4198) /* useGuildIdForChannelRoute */;
    const tmp7 = importDefault;
    importDefault(4199).preload(channel.guild_id, channel.id);
    const obj4 = importDefault(4199);
    ...
    if (prop) {
      tmp7(4383).updateChatOpen(channel.id, true);
      const tmp7Result = tmp7(4383);
    }
...
export const transitionToThread = function transitionToThread(channel, arg1) {
  ...
  let obj = require(4198) /* useGuildIdForChannelRoute */;
...
export const tryTransitionToThreadMessage = function tryTransitionToThreadMessage(parentChannelId, threadId, messageId) {
  ...
  if (null != channel) {
    ...
    if (null != channel1) {
      const guildIdForGenericRedirect1 = require(4198) /* useGuildIdForChannelRoute */.getGuildIdForGenericRedirect(channel1);
      const obj7 = require(4198) /* useGuildIdForChannelRoute */;
      const tmp20 = importDefault;
      importDefault(4199).preload(channel1.guild_id, channel1.id);
      const obj8 = importDefault(4199);
      obj = { openChannel: true };
      ...
      if (prop) {
        tmp20(4383).updateChatOpen(channel1.id, true);
        const tmp20Result = tmp20(4383);
      }
