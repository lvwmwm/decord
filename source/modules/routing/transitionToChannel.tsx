...
function transitionToChannel(id, openTextInVoiceIfVoiceChannel) {
  ...
  if (null != channel) {
    const guildIdForGenericRedirect = require(4174) /* useGuildIdForChannelRoute */.getGuildIdForGenericRedirect(channel);
    const obj3 = require(4174) /* useGuildIdForChannelRoute */;
    importDefault(4175).preload(channel.guild_id, channel.id);
    const obj4 = importDefault(4175);
    ...
    if (tmp) {
      importDefault(4358).updateChatOpen(channel.id, true);
...
function transitionToThreadMessage(channel, id, openTextInVoiceIfVoiceChannel) {
  ...
  let obj = require(4174) /* useGuildIdForChannelRoute */;
