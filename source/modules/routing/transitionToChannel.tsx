...
// Dependencies: [1391, 676, 4353, 4354, 1222, 4537, 38, 2]
...
// Module 4352 (transitionToChannel)
...
export const transitionToChannel = function transitionToChannel(id, openTextInVoiceIfVoiceChannel) {
  ...
  if (null != channel) {
    const guildIdForGenericRedirect = require(4353) /* useGuildIdForChannelRoute */.getGuildIdForGenericRedirect(channel);
    const obj3 = require(4353) /* useGuildIdForChannelRoute */;
    const tmp7 = importDefault;
    importDefault(4354).preload(channel.guild_id, channel.id);
    const obj4 = importDefault(4354);
    ...
    if (prop) {
      tmp7(4537).updateChatOpen(channel.id, true);
      const tmp7Result = tmp7(4537);
    }
...
export const transitionToThread = function transitionToThread(channel, arg1) {
  ...
  let obj = require(4353) /* useGuildIdForChannelRoute */;
...
export const tryTransitionToThreadMessage = function tryTransitionToThreadMessage(parentChannelId, threadId, messageId) {
  ...
  if (null != channel) {
    ...
    if (null != channel1) {
      const guildIdForGenericRedirect1 = require(4353) /* useGuildIdForChannelRoute */.getGuildIdForGenericRedirect(channel1);
      const obj7 = require(4353) /* useGuildIdForChannelRoute */;
      const tmp20 = importDefault;
      importDefault(4354).preload(channel1.guild_id, channel1.id);
      ...
      if (prop) {
        tmp20(4537).updateChatOpen(channel1.id, true);
        const tmp20Result = tmp20(4537);
      }
