...
// Dependencies: [1391, 676, 4769, 4770, 1222, 8666, 38, 2]
...
export const transitionToChannel = function transitionToChannel(id, openTextInVoiceIfVoiceChannel) {
  ...
  if (null != channel) {
    const guildIdForGenericRedirect = require(4769) /* useGuildIdForChannelRoute */.getGuildIdForGenericRedirect(channel);
    const obj3 = require(4769) /* useGuildIdForChannelRoute */;
    const tmp7 = importDefault;
    importDefault(4770).preload(channel.guild_id, channel.id);
    const obj4 = importDefault(4770);
    ...
    if (prop) {
      tmp7(8666).updateChatOpen(channel.id, true);
      const tmp7Result = tmp7(8666);
    }
...
export const transitionToThread = function transitionToThread(channel, arg1) {
  ...
  let obj = require(4769) /* useGuildIdForChannelRoute */;
...
export const tryTransitionToThreadMessage = function tryTransitionToThreadMessage(parentChannelId, threadId, messageId) {
  ...
  if (null != channel) {
    ...
    if (null != channel1) {
      const guildIdForGenericRedirect1 = require(4769) /* useGuildIdForChannelRoute */.getGuildIdForGenericRedirect(channel1);
      const obj7 = require(4769) /* useGuildIdForChannelRoute */;
      const tmp20 = importDefault;
      importDefault(4770).preload(channel1.guild_id, channel1.id);
      const obj8 = importDefault(4770);
      obj = { openChannel: true };
      ...
      if (prop) {
        tmp20(8666).updateChatOpen(channel1.id, true);
        const tmp20Result = tmp20(8666);
      }
