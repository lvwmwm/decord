...
export const transitionToChannel = function transitionToChannel(id, openTextInVoiceIfVoiceChannel) {
  ...
  if (null != channel) {
    const guildIdForGenericRedirect = require(4354) /* useGuildIdForChannelRoute */.getGuildIdForGenericRedirect(channel);
    const obj3 = require(4354) /* useGuildIdForChannelRoute */;
    const tmp7 = importDefault;
    importDefault(4355).preload(channel.guild_id, channel.id);
    const obj4 = importDefault(4355);
...
export const transitionToThread = function transitionToThread(channel, arg1) {
  ...
  let obj = require(4354) /* useGuildIdForChannelRoute */;
...
export const tryTransitionToThreadMessage = function tryTransitionToThreadMessage(parentChannelId, threadId, messageId) {
  ...
  if (null != channel) {
    ...
    if (null != channel1) {
      const guildIdForGenericRedirect1 = require(4354) /* useGuildIdForChannelRoute */.getGuildIdForGenericRedirect(channel1);
      const obj7 = require(4354) /* useGuildIdForChannelRoute */;
      const tmp20 = importDefault;
      importDefault(4355).preload(channel1.guild_id, channel1.id);
      const obj8 = importDefault(4355);
      obj = { openChannel: true };
