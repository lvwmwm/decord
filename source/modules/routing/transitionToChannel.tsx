...
// Dependencies: [1391, 676, 4458, 4459, 1222, 4687, 38, 2]
...
// Module 4457 (transitionToChannel)
import _modDef38 from "module_38" /* 38 */;
import transitionTo from "transitionTo" /* 1222 */;
import useGuildIdForChannelRoute from "useGuildIdForChannelRoute" /* 4458 */;
...
export const transitionToChannel = function transitionToChannel(id, openTextInVoiceIfVoiceChannel) {
  ...
  if (null != channel) {
    ...
    _modDef4459.preload(channel.guild_id, channel.id);
    const obj4 = _modDef4459;
    ...
    if (prop) {
      tmp7(4687).updateChatOpen(channel.id, true);
...
export const tryTransitionToThreadMessage = function tryTransitionToThreadMessage(parentChannelId, threadId, messageId) {
  ...
  if (null != channel) {
    ...
    if (null != channel1) {
      ...
      _modDef4459.preload(channel1.guild_id, channel1.id);
      const obj8 = _modDef4459;
      obj = { openChannel: true };
      ...
      if (prop) {
        tmp20(4687).updateChatOpen(channel1.id, true);
        const tmp20Result = tmp20(4687);
      }
