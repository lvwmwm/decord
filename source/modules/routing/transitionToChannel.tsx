...
function transitionToChannel(id, openTextInVoiceIfVoiceChannel) {
  ...
  if (null != channel) {
    const guildIdForGenericRedirect = require(4140) /* useGuildIdForChannelRoute */.getGuildIdForGenericRedirect(channel);
    const obj3 = require(4140) /* useGuildIdForChannelRoute */;
    importDefault(4141).preload(channel.guild_id, channel.id);
    const obj4 = importDefault(4141);
    ...
    if (tmp) {
      importDefault(4324).updateChatOpen(channel.id, true);
...
function transitionToThreadMessage(channel, id, openTextInVoiceIfVoiceChannel) {
  ...
  let obj = require(4140) /* useGuildIdForChannelRoute */;
