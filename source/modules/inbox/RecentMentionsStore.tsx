...
// Dependencies: [6, 7, 15, 17, 18, 3803, 1194, 1348, 4384, 4177, 3802, 3982, 4360, 1850, 653, 587, 4386, 4380, 4400, 22, 4241, 5904, 5905, 566, 686, 2]

// Module 5903 (_isNativeReflectConstruct)
...
function findOrCreateMessageRecord(channel_id) {
  if (channel_id instanceof _callSuper) {
    ...
    if (null == message) {
      message = require(4386) /* createMinimalMessageRecord */.createMessageRecord(channel_id);
  ...
  if (null != channel) {
    if (channel.type !== constants.DM) {
      ...
      if (!blockedOrIgnoredForMessage.isBlockedOrIgnoredForMessage(message)) {
        if (!importDefault(5905)(message, id)) {
          ...
          if (importDefault(4400)(obj)) {
            ...
            if (tmp24Result) {
              ...
              tmp24Result = importDefault(4400)(obj);
              const tmp24 = importDefault(4400);
            }
            tmp19 = tmp12;
            if (tmp24Result) {
              c31 = false;
...
obj = {
  ...
  LOAD_RECENT_MENTIONS_SUCCESS: function handleLoadMentionsSuccess(arg0) {
    ...
    let closure_30 = require(4241) /* sleep */.now();
    let c29 = true;
  },
    ...
    if (null != id) {
      if (null != dependencyMap[id]) {
        ...
        if (null != substr[findIndexResult]) {
          substr[findIndexResult] = id(4386).updateMessageRecord(tmp7, message.message);
          const obj2 = id(4386);
        }
      }
    }
