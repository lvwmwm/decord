...
// Dependencies: [19, 17, 1372, 1862, 4342, 3892, 1874, 505, 21, 4255, 712, 4446, 9274, 1297, 4251, 589, 4094, 10087, 4599, 4599, 1236, 2]
// Exports: default

// Module 10086 (FailedGroupDMRow)
...
function FailedGroupDMRow(channel) {
  ...
  obj[1] = channel;
  const items = [callback(importDefault(9274), obj), ];
...
function FailedUserRow(user) {
  ...
  if (null != user) {
    ...
    obj[0] = tmp.row;
    obj = { user: null, guildId: "Boolean", status: true, isMobileOnline: "done", isVROnline: true, size: null, avatarDecoration: null, autoStatusCutout: 14.25 };
    obj[0] = user;
    let tmp13 = null;
    ...
    if (stateFromStores == null) {
      stateFromStores = importDefault(4094).getName(user);
      const obj6 = importDefault(4094);
    }
    obj1[4] = stateFromStores;
    items2[1] = closure_10(tmp2(4251).Text, obj1);
    obj[1] = items2;
...
function FailedChannelRow(channel) {
  ...
  obj[3] = channel(10087).GuildIconWithChannelTypeSizes.SMALL_32;
  const items1 = [callback(channel(10087).GuildIconWithChannelType, obj), ];
  const tmp3 = importDefault(4446)(channel);
  items1[1] = callback(channel(4251).Text, { style: tmp.label, variant: "text-md/medium", lineClamp: 1, ellipsizeMode: "tail", children: importDefault(4446)(channel) });
  obj[1] = items1;
  return callback2(View, obj);
}
...
export default function DestinationFailedAlertModal(arg0) {
  ...
  if (null != onRetry) {
    ...
    const items = [tmp2(tmp3(4599).AlertActionButton, obj1, "confirm"), ];
    const obj2 = { variant: "secondary", text: null };
    const intl3 = tmp3(1236).intl;
    obj2[1] = intl3.string(tmp3(1236).t.WAI6xu);
    items[1] = tmp2(tmp3(4599).AlertActionButton, obj2, "cancel");
    obj[0] = items;
    ...
    tmp2Result = tmp2(tmp3(4599).AlertActionButton, obj3, "confirm");
  }
  obj[3] = callback(require(4599) /* getAlertModalItemKey */.AlertActions, { children: tmp2Result });
