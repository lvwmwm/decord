...
// Dependencies: [5, 9884, 1387, 4091, 1922, 10681, 676, 9885, 4461, 9896, 1236, 1471, 12, 2]
...
// Module 10680 (getChannelIdFromDestinationId)
...
import closure_4 from "handleConnectionOpen" /* 9884 */;
import closure_5 from "ensureGuildLoaded" /* 1387 */;
import closure_6 from "getUncachedChannelPermissions" /* 4091 */;
import closure_7 from "mergeGuildAvatar" /* 1922 */;
import { isAllowedType } from "ALLOWED_TYPES" /* 10681 */;
  ...
  if (hasQuery.hasQuery) {
    ...
    if (null != pinnedDestinations) {
        ...
        let mapped = pinnedDestinations.map((type) => {
          if ("user" === type.type) {
            ...
            if (null != user) {
              ...
              obj[0] = includeMissingDMs(9885).AutocompleterResultTypes.USER;
              obj[1] = user;
              tmp6 = obj;
            }
            let tmp3 = tmp6;
          } else {
            tmp3 = callback(9896)(type.id);
          }
          return tmp3;
        });
      }
      ...
      if (channelHistory.length > 0) {
        let mapped1 = channelHistory.map((arg0) => callback(9896)(arg0));
      } else {
        mapped1 = [];
      }
      if (frequentChannels.length > 0) {
        let mapped2 = frequentChannels.map((id) => callback(9896)(id.id));
      } else {
        mapped2 = [];
      }
      const items = [];
...
export const formatResultsWithHeaders = function formatResultsWithHeaders(hasNonEmptyQuery) {
  ...
  const headerResult = queryMode(9885).createHeaderResult(intl.string(queryMode(1236).t.qm9dSj));
  if (hasNonEmptyQuery.hasNonEmptyQuery) {
    ...
    if (null != queryMode) {
      ...
      items4 = [tmp(9885).createHeaderResult(intl2.string(tmp(1236).t["80lOZ1"])), , ];
      let tmp13 = null;
      if (null != selectedChannelId) {
        ...
        if (null != tmp7) {
          ...
          if (isAllowedType(tmp7)) {
            let canResult = tmp7.type === tmp(9885).AutocompleterResultTypes.USER;
            if (!canResult) {
