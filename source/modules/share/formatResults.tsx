...
// Dependencies: [5, 9763, 1386, 4120, 1921, 10913, 673, 9764, 4494, 9773, 1233, 1394, 12, 2]
...
// Module 10912 (getChannelIdFromDestinationId)
import sortByMatchScore from "sortByMatchScore" /* 9764 */;
import createAutocompleterResultForChannelIdDefault from "createAutocompleterResultForChannelId" /* 9773 */;
import closure_3 from "asyncGeneratorStep" /* 5 */;
import closure_4 from "handleConnectionOpen" /* 9763 */;
import closure_5 from "ensureGuildLoaded" /* 1386 */;
import closure_6 from "getUncachedChannelPermissions" /* 4120 */;
import closure_7 from "mergeGuildAvatar" /* 1921 */;
import { isAllowedType } from "ALLOWED_TYPES" /* 10913 */;
  ...
  if (hasQuery.hasQuery) {
    ...
    if (null != pinnedDestinations) {
        ...
        let mapped = pinnedDestinations.map((type) => {
          if ("user" === type.type) {
            ...
            if (null != user) {
              ...
              obj[0] = includeMissingDMs(9764).AutocompleterResultTypes.USER;
              obj[1] = user;
              tmp6 = obj;
            }
            let tmp3 = tmp6;
          } else {
            tmp3 = callback(9773)(type.id);
          }
          return tmp3;
        });
      }
      ...
      if (channelHistory.length > 0) {
        let mapped1 = channelHistory.map((arg0) => callback(9773)(arg0));
      } else {
        mapped1 = [];
      }
      if (frequentChannels.length > 0) {
        let mapped2 = frequentChannels.map((id) => callback(9773)(id.id));
      } else {
        mapped2 = [];
      }
      const items = [];
...
export const formatResultsWithHeaders = function formatResultsWithHeaders(hasNonEmptyQuery) {
  ...
  const headerResult = queryMode(9764).createHeaderResult(intl.string(queryMode(1233).t.qm9dSj));
  if (hasNonEmptyQuery.hasNonEmptyQuery) {
    ...
    if (null != queryMode) {
      ...
      items4 = [tmp(9764).createHeaderResult(intl2.string(tmp(1233).t["80lOZ1"])), , ];
      let tmp13 = null;
      if (null != selectedChannelId) {
        ...
        if (null != tmp7) {
          ...
          if (isAllowedType(tmp7)) {
            let canResult = tmp7.type === tmp(9764).AutocompleterResultTypes.USER;
            if (!canResult) {
