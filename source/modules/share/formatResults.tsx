...
// Dependencies: [5, 9834, 1957, 4199, 1371, 10982, 1074, 9835, 4573, 9844, 1114, 1369, 12, 2]
...
// Module 10981 (getChannelIdFromDestinationId)
...
import closure_4 from "handleConnectionOpen" /* 9834 */;
import closure_5 from "ensureGuildLoaded" /* 1957 */;
import closure_6 from "getUncachedChannelPermissions" /* 4199 */;
import closure_7 from "mergeGuildAvatar" /* 1371 */;
import { isAllowedType } from "ALLOWED_TYPES" /* 10982 */;
  ...
  if (hasQuery.hasQuery) {
    ...
    if (null != pinnedDestinations) {
        ...
        let mapped = pinnedDestinations.map((type) => {
          if ("user" === type.type) {
            ...
            if (null != user) {
              ...
              obj[0] = includeMissingDMs(9835).AutocompleterResultTypes.USER;
              obj[1] = user;
              tmp6 = obj;
            }
            let tmp3 = tmp6;
          } else {
            tmp3 = callback(9844)(type.id);
          }
          return tmp3;
        });
      }
      ...
      if (channelHistory.length > 0) {
        let mapped1 = channelHistory.map((arg0) => callback(9844)(arg0));
      } else {
        mapped1 = [];
      }
      if (frequentChannels.length > 0) {
        let mapped2 = frequentChannels.map((id) => callback(9844)(id.id));
      } else {
        mapped2 = [];
      }
      const items = [];
