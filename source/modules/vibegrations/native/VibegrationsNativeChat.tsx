...
// Function ID: 16101
// Name: ProposalCard
// Dependencies: [19, 17, 7091, 16087, 16090, 21, 4445, 712, 1236, 3469, 4878, 4441, 4435, 4880, 5553, 4158, 16101, 16102, 16104, 16106, 16108, 589, 8037, 16110, 2]
// Exports: default

// Module 16100 (ProposalCard)
...
import VibegrationsMessageAuthorDefault from "VibegrationsMessageAuthor" /* 16102 */;
import VibegrationsClarificationCardDefault from "VibegrationsClarificationCard" /* 16104 */;
import VibegrationsSecretRequestCardDefault from "VibegrationsSecretRequestCard" /* 16106 */;
import VibegrationsSettingsRequestCardDefault from "VibegrationsSettingsRequestCard" /* 16108 */;
import importAllResult from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_6 from "getState" /* 7091 */;
import closure_7 from "newMessage" /* 16087 */;
import { turnSettled } from "newMessage" /* 16087 */;
import rejectPendingPublish from "rejectPendingPublish" /* 16090 */;
import closure_13 from "rejectPendingPublish" /* 16090 */;
...
function ProposalCard(proposal) {
  ...
  if ("" === trimmed) {
    ...
    stringResult = tmp8(4435).parse(trimmed, true, closure_16);
    const tmp8Result = tmp8(4435);
  }
  ...
  if (proposal.changes.length > 0) {
    ...
    const items2 = [tmp3(tmp6(4441).Text, obj1), ];
    ...
    obj[2] = items2;
    tmp5Result = tmp5(tmp6(4878).Stack, obj);
  }
  items1[2] = tmp5Result;
  tmp5Result = null;
  if (bot_permissions.length > 0) {
    ...
    const items3 = [tmp3(tmp6(4441).Text, obj3), ];
    const obj4 = { variant: "text-sm/normal", color: "text-default", children: null };
    obj4[2] = bot_permissions.join(", ");
    items3[1] = tmp3(tmp6(4441).Text, obj4);
    obj2[2] = items3;
    tmp5Result = tmp5(tmp6(4878).Stack, obj2);
  }
  items1[3] = tmp5Result;
  ...
  if (privileged_intents.length > 0) {
    ...
    const items4 = [tmp3(tmp6(4441).Text, obj6), ];
    const obj7 = { variant: "text-sm/normal", color: "text-default", children: null };
    obj7[2] = privileged_intents.join(", ");
    items4[1] = tmp3(tmp6(4441).Text, obj7);
    obj5[2] = items4;
    tmp5Result1 = tmp5(tmp6(4878).Stack, obj5);
  }
  ...
  if (actionable) {
    ...
    obj8[2] = onApprove;
    tmp3Result = tmp3(tmp6(4880).Button, obj8);
  }
  items1[5] = tmp3Result;
  ...
  return callback(closure_5, {
    ...
    children: attachments.map((id) => {
      ...
      if (null != id.id) {
        ...
        obj[2] = id.name;
        obj[3] = closure_1_14(projectId(4441).Text, obj);
        let tmp12 = closure_1_14(projectId(5553).Card, obj, id.id);
      } else {
        ...
        obj1[1] = closure_1_14(projectId(4441).Text, obj2);
...
function ActivityBox(tree) {
  ...
  if (0 !== tree.steps.length) {
    ...
    const items1 = [
      steps.map((id) => {
          const obj = { variant: "text-sm/normal", color: "text-muted", children: callback(16101).describeNode(id) };
          return callback3(callback(4441).Text, obj, id.id);
        }),
  ...
  if ("user" === message.role) {
    if ("" !== trimmed) {
      ...
      if ("" !== trimmed) {
        ...
        obj1[2] = trimmed;
        tmp51Result = tmp51(message(4441).Text, obj1);
      }
  ...
  ,
    ...
    if (null == proposal) {
      ...
      if ("" !== trimmed) {
        ...
        if (!tmp14) {
          ...
          obj4[1] = tmp19(message(4441).Text, obj5);
    ...
    if (null != found) {
      ...
      if ("message" in found) {
        ...
        tmp19Result8 = tmp19(message(4441).Text, obj15);
      }
    }
