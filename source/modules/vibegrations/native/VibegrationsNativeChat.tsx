...
// Function ID: 16119
// Name: ProposalCard
// Dependencies: [19, 17, 7104, 16105, 16108, 21, 4446, 712, 1236, 3470, 4891, 4442, 4436, 4893, 5566, 4159, 16119, 16120, 16122, 16124, 16126, 589, 8051, 16128, 2]
// Exports: default

// Module 16118 (ProposalCard)
...
import VibegrationsMessageAuthorDefault from "VibegrationsMessageAuthor" /* 16120 */;
import VibegrationsClarificationCardDefault from "VibegrationsClarificationCard" /* 16122 */;
import VibegrationsSecretRequestCardDefault from "VibegrationsSecretRequestCard" /* 16124 */;
import VibegrationsSettingsRequestCardDefault from "VibegrationsSettingsRequestCard" /* 16126 */;
import importAllResult from "noop" /* 19 */;
import get_ActivityIndicator from "get ActivityIndicator" /* 17 */;
import closure_6 from "getState" /* 7104 */;
import closure_7 from "newMessage" /* 16105 */;
import { turnSettled } from "newMessage" /* 16105 */;
import rejectPendingPublish from "rejectPendingPublish" /* 16108 */;
import closure_13 from "rejectPendingPublish" /* 16108 */;
...
function ProposalCard(proposal) {
  ...
  if ("" === trimmed) {
    ...
    let stringResult = intl2.string(tmp8(3470).IHCafX);
  } else {
    stringResult = tmp8(4436).parse(trimmed, true, closure_16);
    const tmp8Result = tmp8(4436);
  }
  ...
  if (proposal.changes.length > 0) {
    ...
    obj1[2] = intl3.string(tmp8(3470).KLyB8Y);
    const items2 = [tmp3(tmp6(4442).Text, obj1), ];
    ...
    obj[2] = items2;
    tmp5Result = tmp5(tmp6(4891).Stack, obj);
  }
  items1[2] = tmp5Result;
  tmp5Result = null;
  if (bot_permissions.length > 0) {
    ...
    obj3[2] = intl4.string(tmp8(3470).ieqTtP);
    const items3 = [tmp3(tmp6(4442).Text, obj3), ];
    const obj4 = { variant: "text-sm/normal", color: "text-default", children: null };
    obj4[2] = bot_permissions.join(", ");
    items3[1] = tmp3(tmp6(4442).Text, obj4);
    obj2[2] = items3;
    tmp5Result = tmp5(tmp6(4891).Stack, obj2);
  }
  items1[3] = tmp5Result;
  ...
  if (privileged_intents.length > 0) {
    ...
    obj6[2] = intl5.string(tmp8(3470).Cn9qix);
    const items4 = [tmp3(tmp6(4442).Text, obj6), ];
    const obj7 = { variant: "text-sm/normal", color: "text-default", children: null };
    obj7[2] = privileged_intents.join(", ");
    items4[1] = tmp3(tmp6(4442).Text, obj7);
    obj5[2] = items4;
    tmp5Result1 = tmp5(tmp6(4891).Stack, obj5);
  }
  ...
  if (actionable) {
    ...
    obj8[0] = intl6.string(tmp8(3470).GYoWRk);
    obj8[2] = onApprove;
    tmp3Result = tmp3(tmp6(4893).Button, obj8);
  }
  items1[5] = tmp3Result;
  ...
  return callback(closure_5, {
    ...
    children: attachments.map((id) => {
      ...
      if (null != id.id) {
        ...
        obj = { name: null };
        obj[0] = id.name;
        obj[2] = intl.formatToPlainString(lib(3470).QUFLUq, obj);
        obj = { variant: "text-xs/medium", color: "text-default", children: null };
        obj[2] = id.name;
        obj[3] = closure_1_14(projectId(4442).Text, obj);
        let tmp12 = closure_1_14(projectId(5566).Card, obj, id.id);
      } else {
        ...
        obj3[0] = id.name;
        obj2[2] = intl2.formatToPlainString(lib(3470).OBr7WW, obj3);
        obj1[1] = closure_1_14(projectId(4442).Text, obj2);
...
function ActivityBox(tree) {
  ...
  if (0 !== tree.steps.length) {
    ...
    const items1 = [
      steps.map((id) => {
          const obj = { variant: "text-sm/normal", color: "text-muted", children: callback(16119).describeNode(id) };
          return callback3(callback(4442).Text, obj, id.id);
        }),
  ...
  if ("user" === message.role) {
    if ("" !== trimmed) {
      ...
      if ("" !== trimmed) {
        ...
        obj1[2] = trimmed;
        tmp51Result = tmp51(message(4442).Text, obj1);
      }
  ...
  ,
    ...
    if (null == proposal) {
      ...
      if ("" !== trimmed) {
        ...
        if (!tmp14) {
          ...
          obj4[1] = tmp19(message(4442).Text, obj5);
    ...
    if (null != found) {
      ...
      if ("message" in found) {
        ...
        tmp19Result8 = tmp19(message(4442).Text, obj15);
      }
    }
