...
// Dependencies: [5, 32, 19, 17, 16510, 16513, 16511, 21, 4481, 709, 6921, 586, 1233, 3500, 9721, 13186, 4448, 5640, 5643, 5559, 6547, 4477, 4936, 7138, 7089, 9722, 2]
// Exports: default

// Module 16534 (VibegrationsSettingsSheet)
...
import closure_11 from "rejectPendingPublish" /* 16513 */;
import closure_12 from "isProjectOwner" /* 16511 */;
...
export default function VibegrationsSettingsSheet(projectId) {
  ...
  let tmp3 = map(notifyAgent(6921)({ includeKeyboardHeight: true }).insets.bottom);
  dependencyMap = tmp3;
  ...
  const segmentedControlState = projectId(9721).useSegmentedControlState({ items: memo3, pageWidth: tmp24, defaultIndex: num, onSetActiveIndex: tmp22 });
  if (tmp19) {
    let tmp28 = 1 === tmp21;
  } else {
  ...
  obj[1] = memo(projectId(7089).BottomSheetTitleHeader, obj);
  ...
  if (null != note) {
    ...
    if ("" !== note) {
      ...
      obj2[2] = note;
      tmp32Result = tmp32(tmp4(4477).Text, obj2);
    }
  }
  ...
  if (0 === found.length) {
    ...
    if (0 === memo2.length) {
      ...
      tmp32Result = tmp32(tmp4(4477).Text, obj3);
    }
  }
  ...
  if (found1.length > 0) {
    ...
    if (someResult) {
      ...
      tmp32Result1 = tmp32(tmp4(4477).Text, obj5);
    }
    const items8 = [
      tmp32Result1,
    ...
    if (tmp19) {
      ...
      obj6[1] = tmp32(tmp4(9722).SegmentedControl, obj7);
    ...
    if (tmp28) {
      ...
      const items10 = [tmp32(tmp4(4477).Text, obj9), memo2.map(renderSecret)];
      obj8[1] = items10;
  ...
  if (tmp14[0]) {
    ...
    tmp32Result3 = tmp32(tmp4(4477).Text, obj11);
  }
  ...
  obj12[4] = callback1;
  items7[4] = memo(projectId(4936).Button, obj12);
  obj1[1] = items7;
  obj[2] = memo1(first1, obj1);
  return memo(projectId(7138).ActionSheet, obj);
};
