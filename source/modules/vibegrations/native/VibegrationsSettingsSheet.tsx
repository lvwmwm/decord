...
// Dependencies: [5, 32, 19, 17, 16105, 16108, 16106, 21, 4446, 712, 5559, 589, 1236, 3470, 9199, 12853, 4413, 7639, 7640, 7630, 7652, 4442, 4893, 5954, 5586, 9701, 2]
// Exports: default

// Module 16127 (VibegrationsSettingsSheet)
...
import closure_11 from "rejectPendingPublish" /* 16108 */;
import closure_12 from "isProjectOwner" /* 16106 */;
...
export default function VibegrationsSettingsSheet(projectId) {
  ...
  let tmp3 = map(notifyAgent(5559)({ includeKeyboardHeight: true }).insets.bottom);
  dependencyMap = tmp3;
  ...
  const segmentedControlState = projectId(9199).useSegmentedControlState({ items: memo3, pageWidth: tmp24, defaultIndex: num, onSetActiveIndex: tmp22 });
  if (tmp19) {
    let tmp28 = 1 === tmp21;
  } else {
  ...
  obj[0] = intl.string(notifyAgent(3470).gTMvzD);
  obj[1] = memo(projectId(5586).BottomSheetTitleHeader, obj);
  ...
  if (null != note) {
    ...
    if ("" !== note) {
      ...
      obj2[2] = note;
      tmp32Result = tmp32(tmp4(4442).Text, obj2);
    }
  }
  ...
  if (0 === found.length) {
    ...
    if (0 === memo2.length) {
      ...
      obj3[2] = intl2.string(tmp(3470).URnN4B);
      tmp32Result = tmp32(tmp4(4442).Text, obj3);
    }
  }
  ...
  if (found1.length > 0) {
    ...
    if (someResult) {
      ...
      obj5[2] = intl4.string(tmp(3470)["Hl+eu7"]);
      tmp32Result1 = tmp32(tmp4(4442).Text, obj5);
    }
    const items8 = [
      tmp32Result1,
    ...
    if (tmp19) {
      ...
      obj6[1] = tmp32(tmp4(9701).SegmentedControl, obj7);
    ...
    if (tmp28) {
      ...
      obj9[2] = intl3.string(tmp(3470)["Hl+eu7"]);
      const items10 = [tmp32(tmp4(4442).Text, obj9), memo2.map(renderSecret)];
      obj8[1] = items10;
  ...
  if (tmp14[0]) {
    ...
    obj11[2] = intl5.string(tmp(3470).n02OEo);
    tmp32Result3 = tmp32(tmp4(4442).Text, obj11);
  }
  ...
  obj12[0] = intl6.string(notifyAgent(3470).Tuz9vw);
  obj12[2] = first2;
  obj12[3] = !(null != memo4.values || null != memo4.secrets);
  obj12[4] = callback1;
  items7[4] = memo(projectId(4893).Button, obj12);
  obj1[1] = items7;
  obj[2] = memo1(first1, obj1);
  return memo(projectId(5954).ActionSheet, obj);
};
