...
// Dependencies: [5, 32, 19, 17, 16022, 16025, 16023, 21, 4444, 712, 5541, 589, 1236, 3469, 9171, 12815, 4411, 8203, 8202, 8673, 8002, 4440, 4879, 5936, 5568, 9357, 2]
// Exports: default

// Module 16044 (VibegrationsSettingsSheet)
...
import closure_11 from "rejectPendingPublish" /* 16025 */;
import closure_12 from "isProjectOwner" /* 16023 */;
...
export default function VibegrationsSettingsSheet(projectId) {
  ...
  let tmp3 = map(notifyAgent(5541)({ includeKeyboardHeight: true }).insets.bottom);
  dependencyMap = tmp3;
  ...
  const segmentedControlState = projectId(9171).useSegmentedControlState({ items: memo3, pageWidth: tmp24, defaultIndex: num, onSetActiveIndex: tmp22 });
  if (tmp19) {
    let tmp28 = 1 === tmp21;
  } else {
  ...
  obj[1] = memo(projectId(5568).BottomSheetTitleHeader, obj);
  ...
  if (null != note) {
    ...
    if ("" !== note) {
      ...
      obj2[2] = note;
      tmp32Result = tmp32(tmp4(4440).Text, obj2);
    }
  }
  ...
  if (0 === found.length) {
    ...
    if (0 === memo2.length) {
      ...
      tmp32Result = tmp32(tmp4(4440).Text, obj3);
    }
  }
  ...
  if (found1.length > 0) {
    ...
    if (someResult) {
      ...
      tmp32Result1 = tmp32(tmp4(4440).Text, obj5);
    }
    const items8 = [
      tmp32Result1,
    ...
    if (tmp19) {
      ...
      obj6[1] = tmp32(tmp4(9357).SegmentedControl, obj7);
    ...
    if (tmp28) {
      ...
      const items10 = [tmp32(tmp4(4440).Text, obj9), memo2.map(renderSecret)];
      obj8[1] = items10;
  ...
  if (tmp14[0]) {
    ...
    tmp32Result3 = tmp32(tmp4(4440).Text, obj11);
  }
  ...
  obj12[4] = callback1;
  items7[4] = memo(projectId(4879).Button, obj12);
  obj1[1] = items7;
  obj[2] = memo1(first1, obj1);
  return memo(projectId(5936).ActionSheet, obj);
};
