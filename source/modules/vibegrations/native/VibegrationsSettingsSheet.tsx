...
// Dependencies: [5, 32, 19, 17, 15943, 15946, 15944, 21, 4380, 712, 5475, 589, 1236, 3405, 9103, 12742, 4347, 8136, 8135, 8605, 7935, 4376, 4815, 5870, 5502, 9289, 2]
// Exports: default

// Module 15963 (VibegrationsSettingsSheet)
...
import closure_11 from "rejectPendingPublish" /* 15946 */;
import closure_12 from "isProjectOwner" /* 15944 */;
...
export default function VibegrationsSettingsSheet(projectId) {
  ...
  let tmp3 = map(notifyAgent(5475)({ includeKeyboardHeight: true }).insets.bottom);
  dependencyMap = tmp3;
  ...
  const segmentedControlState = projectId(9103).useSegmentedControlState({ items: memo3, pageWidth: tmp24, defaultIndex: num, onSetActiveIndex: tmp22 });
  if (tmp19) {
    let tmp28 = 1 === tmp21;
  } else {
  ...
  obj[0] = intl.string(notifyAgent(3405).gTMvzD);
  obj[1] = memo(projectId(5502).BottomSheetTitleHeader, obj);
  ...
  if (null != note) {
    ...
    if ("" !== note) {
      ...
      obj2[2] = note;
      tmp32Result = tmp32(tmp4(4376).Text, obj2);
    }
  }
  ...
  if (0 === found.length) {
    ...
    if (0 === memo2.length) {
      ...
      obj3[2] = intl2.string(tmp(3405).URnN4B);
      tmp32Result = tmp32(tmp4(4376).Text, obj3);
    }
  }
  ...
  if (found1.length > 0) {
    ...
    if (someResult) {
      ...
      obj5[2] = intl4.string(tmp(3405)["Hl+eu7"]);
      tmp32Result1 = tmp32(tmp4(4376).Text, obj5);
    }
    const items8 = [
      tmp32Result1,
    ...
    if (tmp19) {
      ...
      obj6[1] = tmp32(tmp4(9289).SegmentedControl, obj7);
    ...
    if (tmp28) {
      ...
      obj9[2] = intl3.string(tmp(3405)["Hl+eu7"]);
      const items10 = [tmp32(tmp4(4376).Text, obj9), memo2.map(renderSecret)];
      obj8[1] = items10;
  ...
  if (tmp14[0]) {
    ...
    obj11[2] = intl5.string(tmp(3405).n02OEo);
    tmp32Result3 = tmp32(tmp4(4376).Text, obj11);
  }
  ...
  obj12[0] = intl6.string(notifyAgent(3405).Tuz9vw);
  obj12[2] = first2;
  obj12[3] = !(null != memo4.values || null != memo4.secrets);
  obj12[4] = callback1;
  items7[4] = memo(projectId(4815).Button, obj12);
  obj1[1] = items7;
  obj[2] = memo1(first1, obj1);
  return memo(projectId(5870).ActionSheet, obj);
};
