...
// Dependencies: [19, 17, 4295, 1971, 1891, 4375, 1903, 1378, 8950, 505, 21, 4303, 712, 5236, 4271, 589, 8941, 8953, 8952, 8957, 9136, 4299, 1236, 1297, 4146, 7796, 5150, 9047, 2]
// Exports: closeGuildEventInfoActionSheet, default

// Module 9143 (EventDetailInfoSheet)
...
export default function EventDetailInfoSheet(guildEvent) {
  ...
  const items8 = [callback(guildEvent(8957).GuildEventCardHeader, obj1), , , , , , , , , ];
  obj2 = { event: guildEvent, textStyle: tmp.eventTitle, style: tmp.eventTitleContainer };
  items8[1] = callback(guildEvent(8957).GuildEventCardTitle, obj2);
  obj3 = { event: guildEvent, textStyle: tmp.guildTextStyle };
  items8[2] = callback(guildEvent(8957).GuildEventCardSimpleGuildInfo, obj3);
  items8[3] = callback(guildEvent(8957).GuildEventSimpleLocation, { event: guildEvent });
  obj4 = { style: tmp.interestedContainer, children: null };
  const items9 = [callback(guildEvent(9136).BellIcon, { size: "sm", style: tmp.interestedIcon }), ];
  const obj6 = { variant: "text-sm/medium", color: "text-default", children: null };
  const intl = guildEvent(1236).intl;
  obj6[2] = intl.format(guildEvent(1236).t["+DLsD8"], { count: importDefault(8952)(guildEvent.guild_id, guildEvent.id, recurrenceId) });
  ...
  items8[6] = callback(guildEvent(8957).GuildEventCardDescription, { event: guildEvent, style: tmp.eventDescriptionContainer });
  ...
  if (hasItem) {
    ...
    const items11 = [tmp2(7796).DividerPosition.TOP, tmp2(7796).DividerPosition.BOTTOM];
    obj12[1] = items11;
  ...
  if (stateFromStores1) {
    ...
    obj15[0] = guildEvent;
    tmp13Result = tmp13(tmp2(8957).GuildEventShareAction, obj15);
  }
  items12[1] = tmp13Result;
  items12[2] = callback(guildEvent(8957).GuildEventModeratorAction, { event: guildEvent, recurrenceId });
  obj13[2] = items12;
  ...
  if (tmp13Result) {
    ...
    tmp13Result = tmp13(tmp7(9047), obj16);
  }
