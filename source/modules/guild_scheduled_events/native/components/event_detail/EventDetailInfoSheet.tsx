...
// Dependencies: [19, 17, 4247, 1942, 1862, 4327, 1874, 1378, 8529, 505, 21, 4255, 712, 5175, 4223, 589, 8520, 8532, 8531, 8536, 10382, 4251, 1236, 1297, 4094, 7707, 5088, 8639, 2]
// Exports: closeGuildEventInfoActionSheet, default

// Module 12393 (EventDetailInfoSheet)
...
export default function EventDetailInfoSheet(guildEvent) {
  ...
  const items8 = [callback(guildEvent(8536).GuildEventCardHeader, obj1), , , , , , , , , ];
  obj2 = { event: guildEvent, textStyle: tmp.eventTitle, style: tmp.eventTitleContainer };
  items8[1] = callback(guildEvent(8536).GuildEventCardTitle, obj2);
  obj3 = { event: guildEvent, textStyle: tmp.guildTextStyle };
  items8[2] = callback(guildEvent(8536).GuildEventCardSimpleGuildInfo, obj3);
  items8[3] = callback(guildEvent(8536).GuildEventSimpleLocation, { event: guildEvent });
  obj4 = { style: tmp.interestedContainer, children: null };
  const items9 = [callback(guildEvent(10382).BellIcon, { size: "sm", style: tmp.interestedIcon }), ];
  const obj6 = { variant: "text-sm/medium", color: "text-default", children: null };
  const intl = guildEvent(1236).intl;
  obj6[2] = intl.format(guildEvent(1236).t["+DLsD8"], { count: importDefault(8531)(guildEvent.guild_id, guildEvent.id, recurrenceId) });
  ...
  items8[6] = callback(guildEvent(8536).GuildEventCardDescription, { event: guildEvent, style: tmp.eventDescriptionContainer });
  ...
  if (hasItem) {
    ...
    const items11 = [tmp2(7707).DividerPosition.TOP, tmp2(7707).DividerPosition.BOTTOM];
    obj12[1] = items11;
    ...
    hasItem = tmp13(tmp7(7707), obj12);
    const tmp7Result = tmp7(7707);
  }
  items8[7] = hasItem;
  const obj13 = { direction: "horizontal", style: tmp.controlsContainer, children: null };
  const items12 = [
    callback(guildEvent(8536).GuildEventCardPrimaryAction, {
      event: guildEvent,
      onCloseAction() {
  ...
  if (stateFromStores1) {
    ...
    obj15[0] = guildEvent;
    tmp13Result = tmp13(tmp2(8536).GuildEventShareAction, obj15);
  }
  items12[1] = tmp13Result;
  items12[2] = callback(guildEvent(8536).GuildEventModeratorAction, { event: guildEvent, recurrenceId });
  obj13[2] = items12;
  items8[8] = callback2(guildEvent(5088).ButtonGroup, obj13);
  ...
  if (tmp13Result) {
    ...
    tmp13Result = tmp13(tmp7(8639), obj16);
  }
