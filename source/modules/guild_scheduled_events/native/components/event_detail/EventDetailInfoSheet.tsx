...
// Dependencies: [19, 17, 4334, 1990, 1910, 4416, 1922, 1397, 9013, 505, 21, 4342, 712, 5297, 4310, 589, 9004, 9016, 9015, 9020, 9200, 4338, 1236, 1297, 4187, 7862, 5212, 9110, 2]
// Exports: closeGuildEventInfoActionSheet, default

// Module 9207 (EventDetailInfoSheet)
...
export default function EventDetailInfoSheet(guildEvent) {
  ...
  const items8 = [callback(guildEvent(9020).GuildEventCardHeader, obj1), , , , , , , , , ];
  obj2 = { event: guildEvent, textStyle: tmp.eventTitle, style: tmp.eventTitleContainer };
  items8[1] = callback(guildEvent(9020).GuildEventCardTitle, obj2);
  obj3 = { event: guildEvent, textStyle: tmp.guildTextStyle };
  items8[2] = callback(guildEvent(9020).GuildEventCardSimpleGuildInfo, obj3);
  items8[3] = callback(guildEvent(9020).GuildEventSimpleLocation, { event: guildEvent });
  obj4 = { style: tmp.interestedContainer, children: null };
  const items9 = [callback(guildEvent(9200).BellIcon, { size: "sm", style: tmp.interestedIcon }), ];
  const obj6 = { variant: "text-sm/medium", color: "text-default", children: null };
  const intl = guildEvent(1236).intl;
  obj6[2] = intl.format(guildEvent(1236).t["+DLsD8"], { count: importDefault(9015)(guildEvent.guild_id, guildEvent.id, recurrenceId) });
  ...
  items8[6] = callback(guildEvent(9020).GuildEventCardDescription, { event: guildEvent, style: tmp.eventDescriptionContainer });
  ...
  if (hasItem) {
    ...
    const items11 = [tmp2(7862).DividerPosition.TOP, tmp2(7862).DividerPosition.BOTTOM];
    obj12[1] = items11;
    ...
    hasItem = tmp13(tmp7(7862), obj12);
    const tmp7Result = tmp7(7862);
  }
  items8[7] = hasItem;
  const obj13 = { direction: "horizontal", style: tmp.controlsContainer, children: null };
  const items12 = [
    callback(guildEvent(9020).GuildEventCardPrimaryAction, {
      event: guildEvent,
      onCloseAction() {
  ...
  if (stateFromStores1) {
    ...
    obj15[0] = guildEvent;
    tmp13Result = tmp13(tmp2(9020).GuildEventShareAction, obj15);
  }
  items12[1] = tmp13Result;
  items12[2] = callback(guildEvent(9020).GuildEventModeratorAction, { event: guildEvent, recurrenceId });
  obj13[2] = items12;
  items8[8] = callback2(guildEvent(5212).ButtonGroup, obj13);
  ...
  if (tmp13Result) {
    ...
    tmp13Result = tmp13(tmp7(9110), obj16);
  }
