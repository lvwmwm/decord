...
// Dependencies: [6925, 8882, 8887, 589, 2]
// Exports: default, getEventSchedule, useEventScheduleById

// Module 8888 (useEventSchedule)
...
export default function useEventSchedule(recurrence_rule, nextRecurrenceIdInEvent) {
  ...
  if (null != recurrence_rule.recurrence_rule) {
    if (null != nextRecurrenceIdInEvent) {
      const baseScheduleForRecurrence = require(8882) /* getRRule */.getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, recurrence_rule);
      const obj2 = require(8882) /* getRRule */;
      const scheduleForRecurrenceWithException = require(8882) /* getRRule */.getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp5);
      ...
      obj[1] = toDateResult;
      const obj3 = require(8882) /* getRRule */;
    }
    return obj;
  }
  obj = { startTime: null, endTime: null };
  ...
  if (tmp5 == stateFromStores) {
    ...
    if (tmp5 != stateFromStores.recurrence_rule) {
      if (tmp5 != nextRecurrenceIdInEvent) {
        tmp2Result = tmp2(8882);
        const baseScheduleForRecurrence = tmp2Result.getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, stateFromStores);
        const scheduleForRecurrenceWithException = tmp2(8882).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp7);
        ({ startDate, endDate } = scheduleForRecurrenceWithException);
        obj = { startTime: null, endTime: null };
        obj[0] = startDate.toDate();
        let toDateResult;
...
export const getEventSchedule = function getEventSchedule(guildEvent, recurrenceId) {
  let endDate;
  let startDate;
  require(8887) /* useEventException */;
