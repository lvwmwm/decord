...
// Dependencies: [6882, 8521, 8526, 589, 2]
// Exports: default, getEventSchedule, useEventScheduleById

// Module 8527 (useEventSchedule)
...
export default function useEventSchedule(recurrence_rule, nextRecurrenceIdInEvent) {
  ...
  if (null != recurrence_rule.recurrence_rule) {
    if (null != nextRecurrenceIdInEvent) {
      const baseScheduleForRecurrence = require(8521) /* getRRule */.getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, recurrence_rule);
      const obj2 = require(8521) /* getRRule */;
      const scheduleForRecurrenceWithException = require(8521) /* getRRule */.getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp5);
      ...
      obj[1] = toDateResult;
      const obj3 = require(8521) /* getRRule */;
    }
    return obj;
  }
  obj = { startTime: null, endTime: null };
  ...
  if (tmp5 == stateFromStores) {
    ...
    if (tmp5 != stateFromStores.recurrence_rule) {
      if (tmp5 != nextRecurrenceIdInEvent) {
        tmp2Result = tmp2(8521);
        const baseScheduleForRecurrence = tmp2Result.getBaseScheduleForRecurrence(nextRecurrenceIdInEvent, stateFromStores);
        const scheduleForRecurrenceWithException = tmp2(8521).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp7);
        ({ startDate, endDate } = scheduleForRecurrenceWithException);
        obj = { startTime: null, endTime: null };
        obj[0] = startDate.toDate();
        let toDateResult;
...
export const getEventSchedule = function getEventSchedule(guildEvent, recurrenceId) {
  let endDate;
  let startDate;
  require(8526) /* useEventException */;
