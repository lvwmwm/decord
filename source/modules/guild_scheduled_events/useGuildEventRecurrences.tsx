...
// Dependencies: [57, 31, 6758, 566, 8510, 8390, 22, 21, 664, 8401, 2]
// Exports: default

// Module 8509 (useGuildEventRecurrences)
...
export default function useGuildEventRecurrences(arg0, arg1, closure_2) {
  ...
  const tmp2 = importDefault(8510)(closure_2);
  const React = tmp2;
  if (null != closure_2) {
    if (null != stateFromStores) {
      let obj2 = _require(8390);
      let _Date = Date;
      let rRule = _require(8390).getRRule(closure_2);
      let date = new Date(stateFromStores.scheduled_start_time);
      const nextRecurrences = obj2.generateNextRecurrences(4, rRule, date);
      let obj3 = _require(8390);
    }
        ...
        if (null != dependencyMap) {
          if (null != stateFromStores) {
            if (!obj3.isEqual(result, dependencyMap)) {
              const rRule = callback(8390).getRRule(dependencyMap);
              const obj = callback(8390);
              const _Date = Date;
              const date = new Date(stateFromStores.scheduled_start_time);
              callback3(callback(8390).generateNextRecurrences(first.length, rRule, date));
              const obj2 = callback(8390);
            }
            obj3 = callback2(22);
          }
        }
    ...
    const effect1 = React.useEffect(() => {
      if (null != callback2) {
        ...
        const guildEventUserCounts = callback2(8401).getGuildEventUserCounts(callback2, closure_0, mapped);
        const obj = callback2(8401);
      }
    }, items2);
          ...
          if (null != dependencyMap) {
            if (0 !== first.length) {
              ...
              if (null != scheduled_start_time) {
                ...
                date.setFullYear(fullYear + callback(8390).MAX_YEARS_AHEAD_RECURRING_EVENT);
                const rRule = callback(8390).getRRule(dependencyMap);
