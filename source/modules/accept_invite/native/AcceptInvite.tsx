...
// Function ID: 11930
// Name: getInviteState
// Dependencies: [32, 19, 17, 676, 21, 4255, 712, 1351, 3959, 5283, 11930, 11933, 1416, 1450, 11938, 1474, 5288, 2]
// Exports: default

// Module 11929 (getInviteState)
...
function AcceptInviteCardComponent(invite) {
  ...
  const effect = React.useEffect(() => {
    ...
    if (tmp !== first) {
      const result = invite(5283).DeprecatedLayoutAnimation();
      dependencyMap(tmp);
      const obj = invite(5283);
    }
  }, items);
  if (null == invite) {
    ...
    obj.invite = invite;
    return callback2(first(11930), obj);
  } else if (tmp22.ERROR === first) {
    obj = {};
    const merged1 = Object.assign(invite);
    obj.invite = invite;
    return callback2(first(11933), obj);
  } else {
    return callback2(InviteResolving, {});
  }
}
...
export default function AcceptInvite(invite) {
  ...
  if (null == splash) {
    let guildSplashSource = tmp3(11938);
  } else {
  ...
  items1[1] = callback2(require(5288) /* PressableCard */.Card, obj2);
  obj[1] = items1;
