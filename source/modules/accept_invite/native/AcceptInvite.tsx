...
// Function ID: 12346
// Name: getInviteState
// Dependencies: [32, 19, 17, 676, 21, 4444, 712, 1370, 4165, 5543, 12346, 12349, 1435, 1469, 12354, 1494, 5548, 2]
// Exports: default

// Module 12345 (getInviteState)
...
function AcceptInviteCardComponent(invite) {
  ...
  const effect = React.useEffect(() => {
    ...
    if (tmp !== first) {
      const result = invite(5543).DeprecatedLayoutAnimation();
      dependencyMap(tmp);
      const obj = invite(5543);
    }
  }, items);
  if (null == invite) {
    ...
    obj.invite = invite;
    return callback2(first(12346), obj);
  } else if (tmp22.ERROR === first) {
    obj = {};
    const merged1 = Object.assign(invite);
    obj.invite = invite;
    return callback2(first(12349), obj);
  } else {
    return callback2(InviteResolving, {});
  }
}
...
export default function AcceptInvite(invite) {
  ...
  if (null == splash) {
    let guildSplashSource = tmp3(12354);
  } else {
