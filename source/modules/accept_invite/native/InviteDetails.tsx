...
// Dependencies: [32, 19, 17, 11842, 1930, 1922, 676, 12134, 21, 4344, 712, 5198, 7235, 1297, 12076, 4189, 1236, 8483, 4340, 1435, 5307, 698, 589, 12135, 9002, 4754, 5190, 2]
// Exports: default

// Module 12133 (shouldShowInviter)
...
function InviteDestinationIcon(invite) {
  ...
  if (null != invite.guild) {
    ...
    obj[0] = tmp.avatar;
    obj[1] = invite.guild;
    obj[2] = importDefault(12076).Sizes.LARGE;
    tmp2Result = tmp2(importDefault(12076), obj);
    const tmp9 = importDefault(12076);
  }
}
function InviteHeader(invite) {
  invite = invite.invite;
  const tmp = createCacheKey();
  let obj = require(7235) /* InviteTypes */;
  ...
  if (tmp2Result.isFriendInvite(invite)) {
    ...
    obj = { username: null };
    obj[0] = importDefault(4189).getFormattedName(invite.inviter);
    name = intl.formatToPlainString(tmp2(1236).t["4aF92R"], obj);
    const obj5 = importDefault(4189);
  }
  let tmp9 = null;
  if (null != name) {
    ...
    const items = [callback2(importDefault(8483), obj1), ];
    ...
    if (!shouldShowInviter(invite, invite.isGuildMember)) {
      ...
      if (id !== closure_11) {
        ...
        if (null != tmp2.memberCount) {
          ...
          items2[1] = callback2(require(4340) /* Text */.Text, obj6);
          obj4[0] = items2;
...
export default function InviteDetails(invite) {
  ...
  items2[4] = callback2(importDefault(12135), { invite, style: tmp.rolesList });
  ...
  if (null != stateFromStores) {
    userAvatarSource = importDefault(1435).getUserAvatarSource(stateFromStores, false, 20);
    const tmp15Result = importDefault(1435);
  }
  let tmp10Result;
