...
// Dependencies: [19, 14616, 11401, 14509, 14618, 1498, 2]
// Exports: useAutoScrollToSearchResultSetting

// Module 14622 (useAutoScrollToSearchResultSetting)
import closure_2 from "noop" /* 19 */;
import closure_3 from "zustandStore" /* 14616 */;
...
export const useAutoScrollToSearchResultSetting = function useAutoScrollToSearchResultSetting(ref, memo, scrollTarget) {
  ...
  if (null != current) {
    flag = false;
    if (tmp(14509).SETTING_RENDERER_CONFIG[current].type !== current.ROUTE) {
      let initialScrollIndex = tmp(14618).getInitialScrollIndex(current, memo);
      let tmp7 = 0 !== initialScrollIndex;
      if (tmp7) {
        tmp7 = 1 !== initialScrollIndex;
      }
      flag = tmp7;
      const tmpResult = tmp(14618);
    }
  }
