...
// Dependencies: [19, 17, 13680, 13681, 10121, 21, 4189, 712, 5554, 4185, 13682, 1581, 13683, 14708, 6676, 14709, 14712, 14713, 1844, 2]

// Module 13679 (SearchListSectionLabel)
...
function SearchListSectionLabel(label) {
  ...
  if (typeof label !== "init") {
    obj = { title: null };
    obj[0] = label;
    tmpResult = tmp(require(5554) /* TableRowGroupTitle */.TableRowGroupTitle, obj);
  }
  obj[1] = tmpResult;
...
function SearchListSectionSubLabel(subLabel) {
  ...
  if (typeof subLabel === "y") {
    ...
    obj[2] = subLabel;
    let tmpResult = tmp(require(4185) /* Text */.Text, obj);
  } else {
    const _Array = Array;
    tmpResult = subLabel;
  }
...
function renderItem(item) {
  ...
  if (ListItemType.SECTION_HEADER === type) {
    ...
    return require(13682) /* GuildSelectDefaultIcon */.renderSettingSearchResultItem(item);
  } else if (tmp.SECTION_ROW === type) {
    return require(13682) /* GuildSelectDefaultIcon */.renderSettingItem(item);
  } else if (tmp.SECTION_ROW_PLACEHOLDER === type) {
    obj = require(13682) /* GuildSelectDefaultIcon */;
...
const memoResult = importAllResult.memo((node) => {
  ...
  obj[6] = memo;
  obj[7] = getItemType;
  obj[1] = jsx(node(6676).FlashList, { ref, ListHeaderComponent: node.ListHeaderComponent, contentContainerStyle: null, scrollIndicatorInsets: null, keyExtractor: null, renderItem: null, data: null, getItemType: null });
