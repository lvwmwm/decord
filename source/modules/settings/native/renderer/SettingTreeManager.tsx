...
// Function ID: 14841
// Name: get
// Dependencies: [10365, 13809, 13808, 2]
...
prototype2["getAncestors"] = function getAncestors(field) {
  const self = this;
  const items = [];
  let transformParentResult = this.transformParent(require(13809) /* frozen */.SETTING_RENDERER_CONFIG[field].parent);
  if (null != transformParentResult) {
    do {
      let arr = items.push(transformParentResult);
      let tmp3 = require;
      let tmp4 = dependencyMap;
      transformParentResult = self.transformParent(require(13809) /* frozen */.SETTING_RENDERER_CONFIG[transformParentResult].parent);
...
prototype2["getNearestRouteAncestorDataOrSelf"] = function getNearestRouteAncestorDataOrSelf(setting) {
  const tmp = require(13809) /* frozen */.SETTING_RENDERER_CONFIG[setting];
  if (tmp.type === NodeType.ROUTE) {
    ...
    for (const item10013 of ancestors) {
      let tmp3 = require;
      let tmp4 = dependencyMap;
      let tmp5 = require(13809) /* frozen */.SETTING_RENDERER_CONFIG[item10013];
      let tmp6 = tmp5;
      let tmp7 = NodeType;
