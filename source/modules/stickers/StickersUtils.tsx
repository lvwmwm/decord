...
// Dependencies: [1340, 1910, 4741, 5092, 4073, 676, 5093, 1435, 1897, 500, 1469, 5095, 2]
...
// Module 4740 (getStickerExtensionFromFormatType)
...
import closure_5 from "getStickerPreview" /* 5092 */;
import STICKER_PICKER_TAB_PANEL_ID from "STICKER_PICKER_TAB_PANEL_ID" /* 4073 */;
import { Endpoints } from "ME" /* 676 */;

require = arg1;
function getStickerExtensionFromFormatType(format_type) {
  if (StickerFormat.StickerFormat.PNG === format_type) {
    const StickerExtensions = tmp(5093).StickerExtensions;
    return tmp(1435).SUPPORTS_WEBP ? StickerExtensions.WEBP : StickerExtensions.PNG;
  } else if (tmp(5093).StickerFormat.APNG === format_type) {
    return tmp(5093).StickerExtensions.APNG;
  } else if (tmp(5093).StickerFormat.LOTTIE === format_type) {
    return tmp(5093).StickerExtensions.LOTTIE;
  } else if (tmp(5093).StickerFormat.GIF === format_type) {
    return tmp(5093).StickerExtensions.GIF;
  } else {
    const _Error = Error;
...
export const getStickerAssetUrl = (format_type) => {
  ...
  if (null == format_type.format_type) {
    ...
    if (tmp) {
      PNG = tmp23(5093).StickerFormat.PNG;
    }
    ...
    try {
      ...
      let str3 = "";
      if (tmp3 === tmp23(5093).StickerExtensions.WEBP) {
        str3 = "&quality=lossless";
      }
      if ("development" !== closure_12) {
        if (format_type.format_type === tmp23(5093).StickerFormat.LOTTIE) {
          const _location3 = location;
          const _HermesInternal4 = HermesInternal;
          return "" + location.protocol + ASSET_ENDPOINT + STICKER_ASSETResult;
        } else {
          let str6 = "";
          if (format_type.format_type === tmp23(5093).StickerFormat.APNG) {
            str6 = "";
            if (flag) {
...
export const createStickerPackCategory = function createStickerPackCategory(id) {
  const obj = { type: _require(5093).StickerCategoryTypes.PACK, id: id.id, name: id.name, stickers: id.stickers, previewSticker: null };
  _require = id;
...
export const shouldAttachSticker = function shouldAttachSticker(arg0, str, channelId, ChannelMessage) {
  if (uploadCount.getUploadCount(channelId, ChannelMessage) > 0) {
    ...
    if (StickerFormat.StickerSelectLocation.STICKER_PICKER === arg0) {
      return "" !== str.trim();
    } else if (tmp3(5093).StickerSelectLocation.AUTOCOMPLETE === arg0) {
      return tmp3(5095).getQueriesFromUserInput(str).length > 1;
    } else {
      const BUILT_IN_INTEGRATION = tmp3(5093).StickerSelectLocation.BUILT_IN_INTEGRATION;
      return false;
    }
  }
};
