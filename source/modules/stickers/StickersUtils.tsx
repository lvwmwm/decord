...
// Dependencies: [1340, 1910, 4819, 5176, 4138, 676, 5177, 1435, 1897, 500, 1469, 5179, 2]
...
// Module 4818 (getStickerExtensionFromFormatType)
...
import closure_5 from "getStickerPreview" /* 5176 */;
import STICKER_PICKER_TAB_PANEL_ID from "STICKER_PICKER_TAB_PANEL_ID" /* 4138 */;
import { Endpoints } from "ME" /* 676 */;

require = arg1;
function getStickerExtensionFromFormatType(format_type) {
  if (StickerFormat.StickerFormat.PNG === format_type) {
    const StickerExtensions = tmp(5177).StickerExtensions;
    return tmp(1435).SUPPORTS_WEBP ? StickerExtensions.WEBP : StickerExtensions.PNG;
  } else if (tmp(5177).StickerFormat.APNG === format_type) {
    return tmp(5177).StickerExtensions.APNG;
  } else if (tmp(5177).StickerFormat.LOTTIE === format_type) {
    return tmp(5177).StickerExtensions.LOTTIE;
  } else if (tmp(5177).StickerFormat.GIF === format_type) {
    return tmp(5177).StickerExtensions.GIF;
  } else {
    const _Error = Error;
...
export const getStickerAssetUrl = (format_type) => {
  ...
  if (null == format_type.format_type) {
    ...
    if (tmp) {
      PNG = tmp23(5177).StickerFormat.PNG;
    }
    ...
    try {
      ...
      let str3 = "";
      if (tmp3 === tmp23(5177).StickerExtensions.WEBP) {
        str3 = "&quality=lossless";
      }
      if ("development" !== closure_12) {
        if (format_type.format_type === tmp23(5177).StickerFormat.LOTTIE) {
          const _location3 = location;
          const _HermesInternal4 = HermesInternal;
          return "" + location.protocol + ASSET_ENDPOINT + STICKER_ASSETResult;
        } else {
          let str6 = "";
          if (format_type.format_type === tmp23(5177).StickerFormat.APNG) {
            str6 = "";
            if (flag) {
...
export const createStickerPackCategory = function createStickerPackCategory(id) {
  const obj = { type: _require(5177).StickerCategoryTypes.PACK, id: id.id, name: id.name, stickers: id.stickers, previewSticker: null };
  _require = id;
...
export const shouldAttachSticker = function shouldAttachSticker(arg0, str, channelId, ChannelMessage) {
  if (uploadCount.getUploadCount(channelId, ChannelMessage) > 0) {
    ...
    if (StickerFormat.StickerSelectLocation.STICKER_PICKER === arg0) {
      return "" !== str.trim();
    } else if (tmp3(5177).StickerSelectLocation.AUTOCOMPLETE === arg0) {
      return tmp3(5179).getQueriesFromUserInput(str).length > 1;
    } else {
      const BUILT_IN_INTEGRATION = tmp3(5177).StickerSelectLocation.BUILT_IN_INTEGRATION;
      return false;
    }
  }
};
