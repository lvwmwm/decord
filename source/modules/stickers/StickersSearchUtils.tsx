...
export const searchAllStickers = function searchAllStickers(arg0) {
  const items = [arg0];
  const obj = importDefault(5031);
  return importDefault(5031).queryStickers(items, true).map((sticker) => sticker.sticker);
};
export const searchSendableStickers = function searchSendableStickers(arg0, arg1) {
  ...
  const obj = importDefault(5031);
  return importDefault(5031).queryStickers(items, true, items1).map((sticker) => sticker.sticker);
};
