...
// Dependencies: [32, 1413, 1863, 1862, 4860, 3834, 1866, 2]
...
function deriveStickerMetadata(arg0, tags) {
  const items = [];
  let obj = { type: items(4860).StickerMetadataTypes.STICKER_NAME, value: null };
  const trimmed = tags.name.trim();
  obj[1] = trimmed.toLocaleLowerCase();
  items.push(obj);
  if (null != tags.tags) {
    obj = { type: null, value: null };
    obj[0] = tmp(4860).StickerMetadataTypes.TAG;
    ...
    if (null != guild) {
      ...
      if (tmp5) {
        obj = { type: null, value: null };
        obj[0] = tmp(4860).StickerMetadataTypes.GUILD_NAME;
        obj[1] = toLocaleLowerCaseResult;
        items.push(obj);
      }
      const str3 = guild.name;
    }
    const byName = importDefault(3834).getByName(str);
    if (null != byName) {
      const obj1 = { type: null, value: null };
      obj1[0] = tmp(4860).StickerMetadataTypes.CORRELATED_EMOJI;
