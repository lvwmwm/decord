...
// Dependencies: [32, 1413, 1863, 1862, 4922, 3896, 1866, 2]

// Module 5159 (parseServerGuildSticker)
...
function deriveStickerMetadata(arg0, tags) {
  const items = [];
  let obj = { type: items(4922).StickerMetadataTypes.STICKER_NAME, value: null };
  const trimmed = tags.name.trim();
  obj[1] = trimmed.toLocaleLowerCase();
  items.push(obj);
  if (null != tags.tags) {
    obj = { type: null, value: null };
    obj[0] = tmp(4922).StickerMetadataTypes.TAG;
    ...
    if (null != guild) {
      ...
      if (tmp5) {
        obj = { type: null, value: null };
        obj[0] = tmp(4922).StickerMetadataTypes.GUILD_NAME;
    ...
    if (null != byName) {
      ...
      obj1[0] = tmp(4922).StickerMetadataTypes.CORRELATED_EMOJI;
