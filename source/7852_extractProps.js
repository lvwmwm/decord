...
// Function ID: 7853
// Name: extractProps
// Dependencies: [7853, 7844, 7854, 7857, 7845, 7843]

// Module 7852 (extractProps)
const require = arg1;
const module = arg2;
...
function extractProps(markerMid) {
  ...
  const obj = {};
  const items = [];
  module(7844)(obj, markerMid, arg1);
  module(7854)(obj, markerMid, items);
  module(7857)(obj, markerMid, items);
  if (markerMid.color) {
  ...
  const tmp6 = module(7845)(markerMid);
  if (null !== tmp6) {
    obj.matrix = tmp6;
  }
  if (null != opacity) {
    obj.opacity = tmp(7843)(opacity);
  }
  if (null != display) {
    let str4;
    if ("none" === display) {
  ...
  if (markerStart) {
    let tmp7;
    if (markerStart) {
      const match = markerStart.match(require(7853) /* pickNotNil */.idPattern);
      let tmp10;
      if (match) {
        tmp10 = match[1];
      }
  ...
  if (str) {
    let tmp11;
    if (str) {
      const match1 = str.match(require(7853) /* pickNotNil */.idPattern);
      let tmp14;
      if (match1) {
        tmp14 = match1[1];
      }
  ...
  if (str2) {
    let tmp15;
    if (str2) {
      const match2 = str2.match(require(7853) /* pickNotNil */.idPattern);
      let tmp18;
      if (match2) {
        tmp18 = match2[1];
      }
  ...
  if (clipPath) {
    const match3 = clipPath.match(require(7853) /* pickNotNil */.idPattern);
    if (match3) {
  ...
  if (mask) {
    const match4 = mask.match(require(7853) /* pickNotNil */.idPattern);
    if (match4) {
      obj.mask = match4[1];
    } else {
  ...
  if (filter) {
    const match5 = filter.match(require(7853) /* pickNotNil */.idPattern);
    if (match5) {
      obj.filter = match5[1];
    } else {
