...
export const useHandler = function useHandler(memoizedGestureCallbacks, items10) {
  ...
  const require = tmp;
  if (null === tmp.current) {
    let obj = require(1683) /* freezeObjectInDev */;
  ...
  for (const key10024 in arg0) {
    ...
    let obj8 = require(1678) /* isWorkletFunction */;
  ...
  const dependencies = require(1799) /* isAnimated */.buildDependencies(items10, memoizedGestureCallbacks);
  const obj3 = require(1799) /* isAnimated */;
  const tmp9 = require;
  tmp.current.savedDependencies = dependencies;
  obj = { context, doDependenciesDiffer: !require(1799) /* isAnimated */.areDependenciesEqual(dependencies, savedDependencies), useWeb: null };
  const obj4 = require(1799) /* isAnimated */;
  let isWebResult = require(1656) /* isJest */.isWeb();
  if (!isWebResult) {
