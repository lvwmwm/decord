...
// Function ID: 13574
// Name: isMatched
// Dependencies: [13574, 1281, 13575]

// Module 13573 (isMatched)
const require = arg1;
  ...
  if (arr) {
    if ("$" === arr[0]) {
      ...
      const mapped = arr2.map((arg0) => {
        let tmp = callback(13575).regions[arg0];
        if (!tmp) {
          const items = [arg0];
          tmp = items;
        }
...
function findMatchingDistance(arg0, arg1) {
  ...
  let tmp7 = obj1;
  if (!obj1) {
    let first = require(13574).data.supplemental.languageMatching["written-new"][0];
    let paradigmLocales;
    if (null !== first) {
      if (undefined !== first) {
    ...
    const prop = tmp8(13574).data.supplemental.languageMatching["written-new"];
    let substr = prop.slice(1, 5);
    const prop1 = tmp8(13574).data.supplemental.languageMatching["written-new"];
