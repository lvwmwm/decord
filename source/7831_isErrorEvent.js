...
// Function ID: 7832
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 7823, 7804, 7776, 7830, 7787, 7783, 7799, 7800, 7832, 7819, 7820, 7803, 7825, 7812, 7833, 7816, 7834, 7781]

// Module 7831 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(7804).DEBUG_BUILD) {
          const logger = tmp(7776).logger;
        ...
        BaseClient(7799).updateSession(release, { init: false });
        const obj = BaseClient(7799);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(7832).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(7832).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(7832);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(7819).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(7804).DEBUG_BUILD) {
        let logger = tmp2(7776).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(7800).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(7832).setupIntegrations(this, integrations);
      const obj = BaseClient(7832);
      const result = BaseClient(7832).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(7816).parseSampleRate(sampleRate);
        let obj = str(7816);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(7833).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(7800).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(7804).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(7776).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(7834);
