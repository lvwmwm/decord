...
// Function ID: 5709
// Name: StyleSheet
// Dependencies: [32, 19, 17, 21, 5659, 5709, 1481, 5689, 5685]

// Module 5708 (StyleSheet)
...
export default importAllResult.memo(function CardContainer(active) {
  ...
  let num = importAllResult.useContext(active(5659).HeaderHeightContext);
  const items = [scene.descriptor];
  const tmp4 = importDefault(5709);
  ({ onPageChangeStart: c9, onPageChangeCancel: c10, onPageChangeConfirm: c11 } = importDefault(5709)(importAllResult.useCallback(() => {
    let navigation;
    let options;
  ...
  let str = "box-none";
  const tmp4Result = importDefault(5709)(importAllResult.useCallback(() => {
    let navigation;
    let options;
    ({ navigation, options } = scene.descriptor);
    let isFocusedResult = navigation.isFocused();
  ...
  if (previousScene) {
    ...
    headerTitle = tmp(5659).getHeaderTitle(options2, route.name);
    tmp10 = headerTitle;
  ...
  if (false === animationEnabled) {
    if (false === isNextScreenTransparent) {
      ...
      obj7[1] = tmp13(tmp(5659).HeaderHeightContext.Provider, obj8);
      obj6[1] = tmp13(tmp(5659).HeaderShownContext.Provider, obj7);
      obj5[1] = tmp13(tmp(5659).HeaderBackContext.Provider, obj6);
      ...
      obj4[1] = items5;
      obj3[1] = scene(tmp3(5685).Provider, obj4);
