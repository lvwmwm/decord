...
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  ...
  sum = typeSize + importDefault(4927).getTypeSize("SHORT");
  const obj2 = importDefault(4927);
  const sum1 = sum + importDefault(4927).getTypeSize("LONG");
  const obj3 = importDefault(4927);
  const shortAt = importDefault(4927).getShortAt(byteLength, sum, byteOrder);
  const obj4 = importDefault(4927);
  const shortAt1 = importDefault(4927).getShortAt(byteLength, sum + typeSize, byteOrder);
  const obj5 = importDefault(4927);
  const longAt = importDefault(4927).getLongAt(byteLength, sum + sum, byteOrder);
  if (undefined !== importDefault(4927).typeSizes[shortAt1]) {
...
function getTagValue(byteLength, sum2, shortAt1, longAt, byteOrder, arg5) {
  ...
  let BYTE = shortAt1;
  if (flag) {
    result = longAt * importDefault(4927).typeSizes[shortAt1];
    BYTE = importDefault(4927).tagTypes.BYTE;
  }
  let sum = sum2;
  const items = [];
  for (let num = 0; num < result; num = num + 1) {
    ...
    sum = sum + importDefault(4927).typeSizes[BYTE];
  }
  if (BYTE === importDefault(4927).tagTypes.ASCII) {
    let asciiValue = importDefault(4927).getAsciiValue(items);
    const tmp9Result = importDefault(4927);
  } else {
    asciiValue = items;
    if (1 === items.length) {
      asciiValue = items[0];
...
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5, arg6) {
  ...
  if (sum + obj2.getTypeSize("SHORT") <= byteLength.byteLength) {
    num = importDefault(4927).getShortAt(byteLength, sum, byteOrder);
    const tmpResult = importDefault(4927);
  }
  obj = {};
  sum = sum + typeSize;
  let tmp5 = sum;
      ...
      while (true) {
        ...
        if (undefined !== tmp13) {
          ...
          if (!tmp19) {
            ...
            let tmp18 = IFD_TYPE_0TH === require(4931) /* importDefaultResult1 */.IFD_TYPE_PENTAX && "LevelInfo" === tmp13.name;
            tmp19 = tmp18;
          }
          if (tmp19) {
  ...
  if (importDefault(4913).USE_THUMBNAIL) {
    ...
    if (tmp5 < byteLength.byteLength - tmp23Result.getTypeSize("LONG")) {
      ...
      let tmp26 = 0 !== longAt;
      if (tmp26) {
        tmp26 = IFD_TYPE_0TH === require(4931) /* importDefaultResult1 */.IFD_TYPE_0TH;
      }
      if (tmp26) {
        obj.Thumbnail = readIfd(byteLength, require(4931) /* importDefaultResult1 */.IFD_TYPE_1ST, sum, sum + longAt, byteOrder, arg5);
      }
    }
  }
  return obj;
...
export const get0thIfdOffset = function get0thIfdOffset(buffer, sum, byteOrder) {
  return sum + importDefault(4927).getLongAt(buffer, sum + 4, byteOrder);
};
export { readIfd };
