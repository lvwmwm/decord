...
// Function ID: 5629
// Name: StyleSheet
// Dependencies: [32, 19, 17, 21, 5579, 5629, 1481, 5609, 5605]

// Module 5628 (StyleSheet)
...
export default importAllResult.memo(function CardContainer(active) {
  ...
  let num = importAllResult.useContext(active(5579).HeaderHeightContext);
  const items = [scene.descriptor];
  const tmp4 = importDefault(5629);
  ({ onPageChangeStart: c9, onPageChangeCancel: c10, onPageChangeConfirm: c11 } = importDefault(5629)(importAllResult.useCallback(() => {
    let navigation;
    let options;
  ...
  let str = "box-none";
  const tmp4Result = importDefault(5629)(importAllResult.useCallback(() => {
    let navigation;
    let options;
    ({ navigation, options } = scene.descriptor);
    let isFocusedResult = navigation.isFocused();
  ...
  if (previousScene) {
    ...
    headerTitle = tmp(5579).getHeaderTitle(options2, route.name);
    tmp10 = headerTitle;
  ...
  if (false === animationEnabled) {
    if (false === isNextScreenTransparent) {
      ...
      obj7[1] = tmp13(tmp(5579).HeaderHeightContext.Provider, obj8);
      obj6[1] = tmp13(tmp(5579).HeaderShownContext.Provider, obj7);
      obj5[1] = tmp13(tmp(5579).HeaderBackContext.Provider, obj6);
      ...
      obj4[1] = items5;
      obj3[1] = scene(tmp3(5605).Provider, obj4);
