...
// Function ID: 7450
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 7441, 7422, 7394, 7448, 7405, 7401, 7417, 7418, 7450, 7437, 7438, 7421, 7443, 7430, 7451, 7434, 7452, 7399]

// Module 7449 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release !== "string") {
        if (BaseClient(7422).DEBUG_BUILD) {
          const logger = tmp(7394).logger;
        ...
        BaseClient(7417).updateSession(release, { init: false });
        const obj = BaseClient(7417);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(7450).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(7450).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(7450);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(7437).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(7422).DEBUG_BUILD) {
        let logger = tmp2(7394).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(7418).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(7450).setupIntegrations(this, integrations);
      const obj = BaseClient(7450);
      const result = BaseClient(7450).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(7434).parseSampleRate(sampleRate);
        let obj = str(7434);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult === "number") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(7451).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(7418).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(7422).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(7394).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tmpResult = tmp(7452);
