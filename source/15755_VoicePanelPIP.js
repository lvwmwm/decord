...
function VoicePanelPIP() {
  ...
  ({ multiPIPWrapperStyles, mainPIPWrapperStyles, secondaryPIPWrapperStyles, mainPIPGesture, secondaryPIPGesture } = function usePIPGesture(mode) {
    ...
    const sharedValue = obj1.useSharedValue({});
    ...
    const callback = safeArea.useCallback(() => {
      ...
      const fn3 = function u(state, activate) {
        ...
        if (state.state === callback(closure_3[17]).State.BEGAN) {
          if (!store.get().active) {
            ...
            if (tmp3) {
              const obj = { absoluteX, absoluteY };
              const result = store.set(obj);
              callback2(closure_3[18])(closure_11, { decimal: "uppercase", numeric: "center", tel: 34 });
              activate.activate();
      ...
      const fn5 = function s(arg0) {
        ...
        callback2(closure_3[18])(closure_14, {});
...
let closure_17 = { "Bool(false)": "ILLO_GREEN_10", "Bool(false)": 0.9, "Bool(false)": "ILLO_GREEN_50" };
...
obj["overflow"] = "hidden";
obj.pipMask = obj;
obj1 = { EHm: 0.2, EHms: null, Ed: 48, Ehm: 2, gap: tmp3.SECONDARY_PIP_TOP_MARGIN };
