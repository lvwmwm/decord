...
// Dependencies: [13271, 13297, 13296, 13295, 13284]

// Module 13299 (__exportStarResult25)
const require = arg1;
...
arg5.PartitionNumberRangePattern = function PartitionNumberRangePattern(arg0, isNaN, isNaN2, getInternalSlots) {
  ...
  items(13271).invariant(tmp4, "Input must be a number", RangeError);
  const internalSlots = getInternalSlots(arg0);
  const result = tmp(13297).PartitionNumberPattern(internalSlots, isNaN);
  const result1 = tmp(13297).PartitionNumberPattern(internalSlots, isNaN2);
  if (FormatNumericResult === items(13296).FormatNumeric(internalSlots, isNaN2)) {
    const FormatApproximatelyResult = tmp(13295).FormatApproximately(internalSlots, result);
    ...
    return tmp(13284).CollapseNumberRange(arg0, items, obj);
  }
  FormatNumericResult = items(13296).FormatNumeric(internalSlots, isNaN);
};
