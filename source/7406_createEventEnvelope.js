...
// Dependencies: [7362, 7363, 7407, 7410, 7399, 7368]
// Exports: createEventEnvelope, createSessionEnvelope, createSpanEnvelope

// Module 7406 (createEventEnvelope)
...
export const createEventEnvelope = function createEventEnvelope(type, arg1, sdk) {
  const sdkMetadataForEnvelopeHeader = require(7407) /* forEachEnvelopeItem */.getSdkMetadataForEnvelopeHeader(sdk);
  let str = "event";
  if (type.type) {
    str = "event";
...
export const createSessionEnvelope = function createSessionEnvelope(toJSON) {
  let obj = require(7407) /* forEachEnvelopeItem */;
...
export const createSpanEnvelope = function createSpanEnvelope(arg0, getDsn) {
  ...
  if (tmp9) {
    const obj1 = { dsn: null };
    obj1[0] = beforeSendSpan(7410).dsnToString(dsn);
    tmp9 = obj1;
    const tmp2Result = beforeSendSpan(7410);
  }
