...
// Function ID: 99963
// Name: items
// Dependencies: [12856, 12855, 12841, 12858]

// Module 12869 (items)
import Comparator from "Comparator";
import Comparator from "Comparator";

Comparator = new Comparator(">=0.0.0-0");
const items = [Comparator];
Comparator = new Comparator(">=0.0.0");
const items1 = [Comparator];
function simpleSubset(semver2, item10021, includePrerelease) {
  let tmp4;
  let tmp5;
  let arr = item10021;
  if (semver2 === item10021) {
    return true;
  } else {
    let tmp = semver2;
    if (1 === semver2.length) {
      ...
      tmp = dependencyMap;
      tmp = semver2;
      if (semver2[0].semver === require(12855) /* Comparator */.ANY) {
        if (1 === arr.length) {
          if (arr[0].semver === require(12855) /* Comparator */.ANY) {
            return true;
          }
        }
      }
    ...
    while (iter !== undefined) {
      ...
      if (">" !== nextResult.operator) {
        ...
        if (">=" !== tmp.operator) {
          ...
          tmp2 = lowerLT(tmp2, tmp, includePrerelease);
        }
        continue;
      }
      let tmp34 = higherGT;
    ...
    if (set.size > 1) {
      ...
      if (tmp) {
        if (tmp2) {
          const tmp45 = require(12841) /* compare */(tmp.semver, tmp2.semver, includePrerelease);
          if (tmp45 > 0) {
            tmp = null;
            return null;
          } else {
      ...
      if (iter2 === undefined) {
        ...
        while (obj2 !== undefined) {
          ...
          let _String3 = String;
          let tmp72 = require(12858) /* satisfies */;
...
function higherGT(semver, semver2, includePrerelease) {
  if (semver) {
    const tmp4 = require(12841) /* compare */(semver.semver, semver2.semver, includePrerelease);
    let tmp5 = semver;
    if (tmp4 <= 0) {
      if (tmp4 < 0) {
        let tmp6 = semver2;
...
export default function subset(arg0, arg1) {
  if (arguments.length > 2) {
    ...
    if (arg0 === arg1) {
      ...
      let tmp19 = require(12856) /* Range */;
      const prototype = tmp19.prototype;
      tmp19 = new tmp19(arg0, obj);
      let tmp27 = require(12856) /* Range */;
      ...
      while (iter !== undefined) {
        ...
        for (const item10021 of set) {
          let tmp8 = simpleSubset;
          let tmp9 = nextResult;
          let tmp10 = obj;
          let tmp11 = simpleSubset(tmp5, item10021, tmp);
          let tmp12 = flag;
          let tmp13 = flag;
          if (!flag) {
            let tmp14 = tmp11;
