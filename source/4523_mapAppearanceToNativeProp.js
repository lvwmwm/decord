...
// Module 4523 (mapAppearanceToNativeProp)
...
export default function TabsScreen(onWillAppear) {
  ...
  if (obj.freezeEnabled()) {
    ...
    if (undefined === freezeContents) {
      if (ref(4492).featureFlags.experiment.controlledBottomTabs) {
        let tmp11 = tmp10;
        if (tmp10) {
          tmp11 = !tmp6;
        }
  ...
  const callback = onDidDisappear.useCallback((arg0) => {
    ref(4517).bottomTabsDebugLog("TabsScreen [" + ref1.current + "] onWillAppear received");
    dependencyMap(true);
  ...
  const callback1 = onDidDisappear.useCallback((arg0) => {
    ref(4517).bottomTabsDebugLog("TabsScreen [" + ref1.current + "] onDidAppear received");
    if (null != onDidAppear) {
      onDidAppear(arg0);
    }
  }, items1);
  ...
  const callback2 = onDidDisappear.useCallback((onDidDisappear) => {
    ref(4517).bottomTabsDebugLog("TabsScreen [" + ref1.current + "] onWillDisappear received");
    if (null != onWillDisappear) {
      onWillDisappear(onDidDisappear);
    }
  }, items2);
  const callback3 = onDidDisappear.useCallback((arg0) => {
    ref(4517).bottomTabsDebugLog("TabsScreen [" + ref1.current + "] onDidDisappear received");
    dependencyMap(false);
  ...
  obj1 = { children: jsx(ref(4490).Freeze, obj2) };
  obj2 = { freeze: flag };
  ({ placeholder: obj6.placeholder, children: obj6.children } = tmp7);
  return tmp18(ref1(4524), Object.assign(obj, tmp17, obj, tmp7, obj1));
};
