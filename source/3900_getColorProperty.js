...
// Dependencies: [57, 31, 3882, 3895, 3890]
// Exports: useRiveColor

// Module 3900 (getColorProperty)
...
export const useRiveColor = function useRiveColor(FillColor, instance) {
  ...
  const tmp2 = callback(require(3895) /* useRiveProperty */.useRiveProperty(instance, FillColor, getColorProperty), 3);
  const first = tmp2[0];
  require = tmp4;
  if (cResult[0] !== first) {
    let fromIntResult;
    if (undefined !== first) {
      let RiveColor = require(3890) /* RiveColor */.RiveColor;
  ...
  if (cResult[2] !== tmp2[1]) {
    function t1(arg0) {
      ...
      if ("string" === typeof arg0) {
        const RiveColor = tmp4(outer1_1[4]).RiveColor;
