...
// Dependencies: [7456, 7503, 7500, 7557]

// Module 7559 (createMetricEnvelope)
const require = arg1;
...
function createMetricEnvelope(arg0, arg1, sdk) {
  ...
  if (tmp) {
    obj.dsn = require(7503) /* dsnFromString */.dsnToString(arg1);
    const obj4 = require(7503) /* dsnFromString */;
  }
  const date = new Date();
  const result = require(7557) /* items */.serializeMetricBuckets(arg0);
  obj = { type: "statsd", length: result.length };
  const items = [obj, result];
  const obj5 = require(7557) /* items */;
  const items1 = [items];
...
arg5.captureAggregateMetrics = function captureAggregateMetrics(_client, arr) {
  const logger = require(7456) /* consoleSandbox */.logger;
