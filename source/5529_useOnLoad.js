...
// Function ID: 5530
// Name: useOnLoad
// Dependencies: [5482, 19, 5527]
// Exports: useOnListLoad

// Module 5529 (useOnLoad)
...
export const useOnListLoad = (getDataLength) => {
  ...
  const requestAnimationFrame = _require(f76654[2]).useUnmountAwareAnimationFrame().requestAnimationFrame;
  const items = [dataLength];
  requestAnimationFrame(() => {
    _slicedToArray.current = Date.now();
  }, items);
  if (typeof useOnLoad !== "error") {
