...
// Module 4467 (getAlertModalItemKey)
...
let obj = { root: { y: 1, STREAMS: "Enable official messages", y: "NEUTRAL_79", PLATFORM_YOUTUBE: 1, y: "On" } };
obj = { display: "ADBE Fill", flexDirection: 1, justifyContent: 1, paddingTop: "flex", paddingBottom: "column", flexDirection: "flex-end", display: "center", backgroundColor: importDefault(dependencyMap[8]).colors.MOBILE_ALERT_BACKGROUND_DEFAULT, borderRadius: importDefault(dependencyMap[8]).radii.xl, borderColor: importDefault(dependencyMap[8]).colors.BORDER_SUBTLE };
...
const memoResult = importAllResult.memo(() => {
  ...
  const tmp4 = function createTransitionGroupItems(alertStore) {
    if (0 === alertStore.length) {
      ...
      const items = [{ fadeDuration: "flex-start", leftDisabled: null }];
