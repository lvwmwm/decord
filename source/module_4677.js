// Module ID: 4677
// Function ID: 4678
// Dependencies: [109, 19, 17, 21, 4675, 4666, 4659, 4678, 4665, 4681, 4654]
...
export default importAllResult.forwardRef(function ScreenStackItem(unstable_sheetFooter, ref) {
  ...
  dependencyMap = flag4.useContext(screenId(4675).RNSScreensRefContext);
  const imperativeHandle = flag4.useImperativeHandle(ref, () => ref.current);
  let obj1 = screenId(4666);
  let flag;
  ...
  importDefault(4659)(flag5, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  ...
  obj[1] = callback(importDefault(4678), { contentStyle, style: container, stackPresentation, children });
  const items1 = [callback(screenId(4666).EdgeInsetApplicationContext.Provider, obj), , ];
  obj = {};
  const merged1 = Object.assign(headerConfig);
  items1[1] = callback(screenId(4665).ScreenStackHeaderConfig, obj);
  let tmp18Result = "formSheet" === stackPresentation && unstable_sheetFooter;
  if (tmp18Result) {
    obj1 = { children: null };
    obj1[0] = unstable_sheetFooter();
    tmp18Result = tmp18(tmp2(4681).FooterComponent, obj1);
  }
  items1[2] = tmp18Result;
  const tmp11 = importDefault(4659);
  ...
  return callback(importDefault(4654), obj2);
});
