...
// Dependencies: [1637, 1638, 1690, 1645, 1720]

// Module 1721 (findDescendantWithExitingAnimation)
const require = arg1;
  ...
  if (isDummy instanceof globalThis.HTMLElement) {
    ...
    if (isDummy) {
      ...
      if (value) {
        ...
        const tmp2Result = tmp2(1720);
      } else {
        const logger = tmp2(1638).logger;
  ...
  if (obj.isWindowAvailable()) {
    ...
    if (element.sheet) {
      ...
      if (1 < arr.length) {
        ...
        const reanimatedError = new require(1645) /* t */.ReanimatedError("Failed to obtain animation index.");
...
arg5.scheduleAnimationCleanup = function scheduleAnimationCleanup(animationName, arg1, arg2) {
  ...
  const timerId = setTimeout(() => {
    if (obj.isWindowAvailable()) {
      ...
      if (undefined === sum) {
        const reanimatedError = new animationName(1645).ReanimatedError("Failed to obtain animation index.");
        throw reanimatedError;
      } else {
        dependencyMap();
        const sheet = element.sheet;
        ...
        if (sum < outer1_5.length) {
          ...
          const reanimatedError1 = new animationName(1645).ReanimatedError("Failed to obtain animation index.");
...
arg5.addHTMLMutationObserver = function addHTMLMutationObserver() {
  ...
  if (!c6) {
    isWindowAvailableResult = require(1637) /* isJest */.isWindowAvailable();
    const obj = require(1637) /* isJest */;
  }
