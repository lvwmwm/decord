...
// Function ID: 4909
// Name: load
// Dependencies: [4909, 4910, 4911, 4912, 4913, 4926, 4928, 4929, 4941, 4945, 4949, 4951, 4953, 4954, 4955, 4956, 4957, 4958, 4959, 4960, 4961, 4962, 4935]
// Exports: load
  ...
  if (typeof response === "string") {
    ...
    if (obj1.test(response)) {
      ...
      if (obj2.test(response)) {
        let resolved = Promise.resolve(response(4910).dataUriToBuffer(response));
        const obj5 = response(4910);
      } else {
        obj2 = then;
        if (then === undefined) {
          obj2 = {};
