...
// Dependencies: [1281, 13549, 13571, 13550, 13564, 13566]
// Exports: PartitionNumberPattern

// Module 13576 (__exportStarResult24)
import e from "e";
...
export const PartitionNumberPattern = function PartitionNumberPattern(internalSlots, isNaN) {
  ...
  if (isNaN.isNaN()) {
    ...
    if (!isNaN.isZero()) {
      require(13550) /* getMultiInternalSlots */.invariant(isNaN.isFinite(), "Input must be a mathematical value");
      let timesResult = isNaN;
      if ("percent" == internalSlots.style) {
        timesResult = isNaN.times(100);
      }
      [tmp7, num4] = require(13564) /* __exportStarResult13 */.ComputeExponent(internalSlots, timesResult);
      timesResult1 = timesResult.times(digitsToString.default.pow(10, -tmp7));
      num3 = tmp7;
      const ComputeExponentResult = require(13564) /* __exportStarResult13 */.ComputeExponent(internalSlots, timesResult);
      const _default = digitsToString.default;
      const tmp3 = require;
    }
    const result = require(13566) /* __exportStarResult21 */.FormatNumericToString(internalSlots, timesResult1);
  ...
  if ("never" !== signDisplay) {
    if ("auto" === signDisplay) {
      ...
      require(13550) /* getMultiInternalSlots */.invariant("negative" === signDisplay, "signDisplay must be \"negative\"");
      let num8 = 0;
      if (roundedNumber.isNegative()) {
        num8 = 0;
        if (!roundedNumber.isZero()) {
