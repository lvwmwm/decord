...
// Module 12967 (getInputRangeFromIndexes)
...
export const defaultScrollInterpolator = function defaultScrollInterpolator(_getCustomIndexResult, props) {
  const items = ["getViewManagerConfig", "hasViewManagerConfig", -1];
  return { inputRange: getInputRangeFromIndexes(items, _getCustomIndexResult, props), outputRange: ["isArray", "\u012E", "isArray"] };
};
export const defaultAnimatedStyles = function defaultAnimatedStyles(index, interpolate, props) {
  if (props.inactiveSlideOpacity < 1) {
    let obj = {};
    obj = { inputRange: [null, null] };
...
export const stackScrollInterpolator = function stackScrollInterpolator(_getCustomIndexResult, props) {
  const items = ["getViewManagerConfig", "hasViewManagerConfig", -1, -2, -3];
...
export const stackAnimatedStyles = function stackAnimatedStyles(index, interpolate, props, layoutCardOffset) {
  ...
  const items = [];
  let obj = {};
  obj = { inputRange: items1, outputRange: items.reverse(), extrapolate: "clamp" };
  const items1 = [-3, -2, -1, 0];
  obj.opacity = interpolate.interpolate(obj);
  obj = { scale: interpolate.interpolate({ inputRange: items2, outputRange: ["opacity", "variant", "formatToPlainString", "LengthDelimited"], extrapolate: "clamp" }) };
  const items2 = [-2, -1, 0, 1];
  const items3 = [obj, ];
  const obj1 = { inputRange: items4 };
  const items4 = [-3, -2, -1, 0, 1];
  const items5 = [getTranslateFromScale(-3, 0.8), getTranslateFromScale(-2, 0.8), getTranslateFromScale(-1, 0.9), 0, 0.5 * tmp];
  obj1.outputRange = items5;
  obj1.extrapolate = "clamp";
  items3[1] = callback({}, str, interpolate.interpolate(obj1));
...
export const tinderAnimatedStyles = function tinderAnimatedStyles(index, interpolate, props, layoutCardOffset) {
  ...
  let obj = { opacity: interpolate.interpolate({ inputRange: items, outputRange: ["hasMore", "displayQuestDismissalToast", "isAffineMatrixFlat", "GRID", "language"], extrapolate: "clamp" }) };
  ...
  const items3 = [obj, , , ];
  obj = { rotate: interpolate.interpolate({ inputRange: [null, null], outputRange: [], extrapolate: "clamp" }) };
  items3[1] = obj;
  const obj1 = { inputRange: items4 };
  const items4 = [-3, -2, -1, 0, 1];
