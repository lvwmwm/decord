...
// Dependencies: [32, 41, 42, 93, 95, 96, 98, 7498, 7513, 7495, 7559, 7523, 7526, 7527, 7499, 7552, 7510, 7560, 7521, 7504, 7535, 7554]

// Module 7558 (_isNativeReflectConstruct)
...
let items = [
  {
    ...
    value: function eventFromException(arg0, arg1) {
      const result = ServerRuntimeClient(7559).eventFromUnknownInput(this, this._options.stackParser, arg0, arg1);
      result.level = "error";
      const obj = ServerRuntimeClient(7559);
      return ServerRuntimeClient(7523).resolvedSyncPromise(result);
    }
  },
  {
    ...
    value: function eventFromMessage(arg0) {
      ...
      const obj = ServerRuntimeClient(7523);
      return obj.resolvedSyncPromise(ServerRuntimeClient(7559).eventFromMessage(this._options.stackParser, arg0, str, arg2, this._options.attachStacktrace));
    }
  },
  {
    ...
    value: function captureException(arg0, arg1, arg2) {
      ...
      if (this._options.autoSessionTracking) {
        if (self._sessionFlusher) {
          const isolationScope = ServerRuntimeClient(7526).getIsolationScope();
          ...
          const obj = ServerRuntimeClient(7526);
        }
      }
  ...
  },
  {
    ...
    value: function initSessionFlusher() {
      ...
      if (release) {
        ...
        obj[0] = release;
        obj[1] = tmp;
        const sessionFlusher = new tmp2(7552).SessionFlusher(self, obj);
        self._sessionFlusher = sessionFlusher;
      } else if (tmp2(7527).DEBUG_BUILD) {
        const logger = tmp2(7499).logger;
    ...
    value: function captureCheckIn(checkInId) {
      ...
      if ("checkInId" in checkInId) {
        ...
        if (this._isEnabled()) {
          ...
          const obj5 = ServerRuntimeClient(7560);
          ...
          if (ServerRuntimeClient(7527).DEBUG_BUILD) {
            const logger2 = ServerRuntimeClient(7499).logger;
          ...
          if (ServerRuntimeClient(7527).DEBUG_BUILD) {
            const logger = ServerRuntimeClient(7499).logger;
            logger.warn("SDK not enabled, will not capture checkin.");
          }
          return checkInId;
        }
      }
      obj = ServerRuntimeClient(7510);
      checkInId = obj.uuid4();
    }
  },
  {
  ...
  },
  {
    ...
    value: function _getTraceInfoFromScope(arg0) {
      if (arg0) {
        const _getSpanForScopeResult = ServerRuntimeClient(7521)._getSpanForScope(arg0);
        ...
        const tmpResult1 = ServerRuntimeClient(7535);
        if (_getSpanForScopeResult) {
