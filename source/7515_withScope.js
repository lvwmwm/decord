...
// Function ID: 7516
// Name: withScope
// Dependencies: [41, 42, 7516, 7502, 7513, 7521]
// Exports: getStackAsyncContextStrategy

// Module 7515 (withScope)
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(7513).getMainCarrier();
  const obj = AsyncContextStack(7513);
  const sentryCarrier = AsyncContextStack(7513).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7513).getMainCarrier();
      let obj = callback(7513);
      let sentryCarrier = callback(7513).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7521);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7513).getMainCarrier();
      const obj = callback(7513);
      const sentryCarrier = callback(7513).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7521);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
