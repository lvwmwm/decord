...
// Function ID: 41592
// Name: fraction
// Dependencies: [57, 4769]

// Module 4784 (fraction)
...
export default {
  ...
  "tiff:ResolutionUnit": function tiffResolutionUnit(replaced) {
    return importDefault(4769).ResolutionUnit(parseInt(replaced, 10));
  },
  "tiff:XResolution": function tiffXResolution(arg0) {
    return fraction(importDefault(4769).XResolution, arg0);
  },
  "tiff:YResolution": function tiffYResolution(arg0) {
    return fraction(importDefault(4769).YResolution, arg0);
  },
  "exif:ApertureValue": function exifApertureValue(arg0) {
    return fraction(importDefault(4769).ApertureValue, arg0);
  },
  "exif:GPSLatitude": calculateGPSValue,
  "exif:GPSLongitude": calculateGPSValue,
  "exif:FNumber": function exifFNumber(arg0) {
    return fraction(importDefault(4769).FNumber, arg0);
  },
  "exif:FocalLength": function exifFocalLength(arg0) {
    return fraction(importDefault(4769).FocalLength, arg0);
  },
  "exif:FocalPlaneResolutionUnit": function exifFocalPlaneResolutionUnit(replaced) {
    return importDefault(4769).FocalPlaneResolutionUnit(parseInt(replaced, 10));
  },
  "exif:ColorSpace": function exifColorSpace(replaced) {
    ...
    return importDefault(4769).ColorSpace(parsed);
  },
  "exif:ComponentsConfiguration": function exifComponentsConfiguration(arg0, str) {
    if (obj.test(str)) {
      ...
      return importDefault(4769).ComponentsConfiguration(mapped);
    } else {
      return str;
    }
    obj = /^\d, \d, \d, \d$/;
  ...
  "exif:Contrast": function exifContrast(replaced) {
    return importDefault(4769).Contrast(parseInt(replaced, 10));
  },
  "exif:CustomRendered": function exifCustomRendered(replaced) {
    return importDefault(4769).CustomRendered(parseInt(replaced, 10));
  },
  "exif:ExposureMode": function exifExposureMode(replaced) {
    return importDefault(4769).ExposureMode(parseInt(replaced, 10));
  },
  "exif:ExposureProgram": function exifExposureProgram(replaced) {
    return importDefault(4769).ExposureProgram(parseInt(replaced, 10));
  },
  "exif:ExposureTime": function exifExposureTime(str) {
    let ExposureTimeResult = str;
    if (isFraction(str)) {
      const parts = str.split("/");
      ExposureTimeResult = importDefault(4769).ExposureTime(parts.map((replaced) => parseInt(replaced, 10)));
      const obj = importDefault(4769);
    }
    return ExposureTimeResult;
  },
  "exif:MeteringMode": function exifMeteringMode(replaced) {
    return importDefault(4769).MeteringMode(parseInt(replaced, 10));
  },
  "exif:Saturation": function exifSaturation(replaced) {
    return importDefault(4769).Saturation(parseInt(replaced, 10));
  },
  "exif:SceneCaptureType": function exifSceneCaptureType(replaced) {
    return importDefault(4769).SceneCaptureType(parseInt(replaced, 10));
  },
  "exif:Sharpness": function exifSharpness(replaced) {
    return importDefault(4769).Sharpness(parseInt(replaced, 10));
  },
  "exif:ShutterSpeedValue": function exifShutterSpeedValue(arg0) {
    return fraction(importDefault(4769).ShutterSpeedValue, arg0);
  },
  "exif:WhiteBalance": function exifWhiteBalance(replaced) {
    return importDefault(4769).WhiteBalance(parseInt(replaced, 10));
  }
};
