...
// Dependencies: [7500, 7508, 7505]

// Module 7510 (addContextToFrame)
const require = arg1;
...
arg5.addContextToFrame = function addContextToFrame(arr, lineno) {
  ...
  if (undefined !== lineno.lineno) {
    ...
    lineno.pre_context = substr.map((arg0) => callback(7508).snipLine(arg0, 0));
    const _Math5 = Math;
    ...
    lineno.context_line = require(7508) /* isMatchingPattern */.snipLine(arr[bound1], num2);
    const _Math = Math;
    const substr1 = arr.slice(Math.min(bound + 1, length), bound + 1 + num);
    lineno.post_context = substr1.map((arg0) => callback(7508).snipLine(arg0, 0));
...
arg5.checkOrSetAlreadyCaught = function checkOrSetAlreadyCaught(__sentry_captured__) {
  if ((function isAlreadyCaptured(__sentry_captured__) {
    ...
    try {
      const result = require(7505) /* addNonEnumerableProperty */.addNonEnumerableProperty(__sentry_captured__, "__sentry_captured__", true);
      return false;
    } catch (err) {
    }
  }
