...
const items = [num, num];
let closure_8 = { colors: items, locations: [], startPoint: { "Null": false, "Null": true }, endPoint: { "Null": false, "Null": true } };
...
const memoResult = importAllResult.memo(function SettingsAppearanceGradientBackground(isDimmed) {
  ...
  StyleSheet = tmp3;
  const items = [tmp3];
  const memo = importAllResult.useMemo(() => tmp3 ? ["Array"] : [], items);
  closure_5 = memo;
    ...
    constructor() {
      ...
      if (0 !== width) {
        if (0 !== height) {
          ...
          tmp8 = themes;
          num3 = 11;
          diff = 90 - obj.interpolate(tweener.get(), [null, null], items);
          ...
          obj.colors = colors.map((arg0, arg1) => {
            ...
            const processColorResult = obj.processColor(colors(closure_2[4]).interpolateColor(store.get(), [null, null], items));
            let num = 0;
            if (null != processColorResult) {
              num = processColorResult;
            }
          ...
          obj.locations = colors.map((arg0, arg1) => {
            ...
            return colors(closure_2[4]).interpolate(store.get(), [null, null], items);
          });
          obj1 = {};
          num4 = 2;
