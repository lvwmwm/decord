...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      ...
      if (handlerTag.oldState === tmp(5494).State.BEGAN) {
        if (handlerTag.state === tmp(5494).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(5494).State.END) {
          if (handlerTag.oldState === tmp(5494).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(5494).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(5494).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(5495).GestureStateManager;
      ...
      if (tmp(5496).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(5496).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(5496).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(5496).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(5492).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
