...
// Dependencies: [7386, 7389, 7402, 7425, 7426, 7412, 7384, 7427, 7420, 7396]

// Module 7424 (applyClientOptions)
const require = arg1;
...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = require(7384) /* isMatchingPattern */.truncate(environment.message, num);
  ...
  if (value) {
    iter.value = require(7384) /* isMatchingPattern */.truncate(iter.value, num);
  ...
  if (url) {
    request.url = require(7384) /* isMatchingPattern */.truncate(request.url, num);
    const obj3 = require(7384) /* isMatchingPattern */;
  }
}
function applyDebugIds(exception) {
  _require = _require(7427).getFilenameToDebugIdMap(arg1);
  try {
          ...
          if (breadcrumbs) {
            ...
            if (breadcrumbs) {
              ...
              obj[0] = breadcrumbs.map((data) => {
                ...
                if (data) {
                  obj = { data: null };
                  obj[0] = dependencyMap(7420).normalize(data.data, dependencyMap, dependencyMap);
                  data = obj;
                  const obj3 = dependencyMap(7420);
                }
