...
// Dependencies: [7400, 7404, 7406, 7408, 7409, 7410, 7411, 7412, 7413, 7416, 7421, 7394]

// Module 7399 (spanTimeInputToSeconds)
const require = arg1;
  ...
  if (typeof num === "number") {
    ...
    if (Array.isArray(num)) {
      ...
      if (num instanceof Date) {
        ...
        sum = require(7408) /* dateTimestampInSeconds */.timestampInSeconds();
        const obj = require(7408) /* dateTimestampInSeconds */;
      }
    }
  }
  return sum;
  ...
  if ((function spanIsSentrySpan(getSpanJSON) {
    ...
    try {
      ...
      if ((function spanIsOpenTelemetrySdkTraceBaseSpan(attributes) {
        ...
        obj[8] = attributes[require(undefined, 7409).SEMANTIC_ATTRIBUTE_SENTRY_OP];
        obj[9] = attributes[require(undefined, 7409).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
        const obj2 = require(7400) /* addNonEnumerableProperty */;
        obj[10] = require(7410) /* getMetricSummaryJsonForSpan */.getMetricSummaryJsonForSpan(getSpanJSON);
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== require(7411) /* getSpanStatusFromHttpCode */.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== tmp(7411).SPAN_STATUS_OK) {
...
arg5.addChildSpanToSpan = function addChildSpanToSpan(arg0, arg1) {
  ...
  const result = require(7400) /* addNonEnumerableProperty */.addNonEnumerableProperty(arg1, _sentryRootSpan, tmp2);
  if (arg0[_sentryChildSpans]) {
    ...
    const result1 = require(7400) /* addNonEnumerableProperty */.addNonEnumerableProperty(arg0, tmp6, set);
    const tmp3Result = require(7400) /* addNonEnumerableProperty */;
  }
};
...
arg5.showSpanDropWarning = function showSpanDropWarning() {
  if (!c2) {
    require(7394) /* consoleSandbox */.consoleSandbox(() => {
...
arg5.spanToTraceContext = function spanToTraceContext(spanContext) {
  ...
  if (isRemote) {
    span_id = require(7404) /* generatePropagationContext */.generateSpanId();
    const obj = require(7404) /* generatePropagationContext */;
  }
  return require(7400) /* addNonEnumerableProperty */.dropUndefinedKeys({ parent_span_id, span_id, trace_id: trace_id.traceId });
};
arg5.spanToTraceHeader = function spanToTraceHeader(spanContext) {
  ...
  return require(7406) /* regExp */.generateSentryTraceHeader(traceId, spanId, 1 === spanContext.spanContext().traceFlags);
};
arg5.spanToTransactionTraceContext = function spanToTransactionTraceContext(spanContext) {
  ...
  return require(7400) /* addNonEnumerableProperty */.dropUndefinedKeys({ parent_span_id, span_id, trace_id, data, op, status, origin });
};
