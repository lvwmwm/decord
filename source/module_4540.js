// Module ID: 4540
// Function ID: 39837
// Dependencies: [29, 31, 27, 33, 4538, 4529, 4522, 4541, 4528, 4544, 4517]
...
export default importAllResult.forwardRef(function ScreenStackItem(unstable_sheetFooter) {
  ...
  const dependencyMap = importAllResult.useContext(screenId(4538).RNSScreensRefContext);
  const imperativeHandle = importAllResult.useImperativeHandle(arg1, () => ref.current);
  let obj = screenId(4529);
  let prop;
  ...
  importDefault(4522)(flag, "[RNScreens] Using both `blurEffect` and `scrollEdgeEffects` simultaneously may cause overlapping effects.");
  if ("formSheet" !== str) {
  ...
  obj.children = callback2(importDefault(4541), obj1);
  const items1 = [callback2(screenId(4529).EdgeInsetApplicationContext.Provider, obj), callback2(screenId(4528).ScreenStackHeaderConfig, Object.assign({}, headerConfig)), ];
  let tmp23 = "formSheet" === str && unstable_sheetFooter;
  if (tmp23) {
    const obj2 = { children: unstable_sheetFooter() };
    tmp23 = callback2(screenId(4544).FooterComponent, obj2);
  }
  items1[2] = tmp23;
  obj.children = items1;
  ...
  let largeTitle;
  const tmp16 = importDefault(4522);
  ...
  return tmp28(importDefault(4517), Object.assign(obj3, tmp, { children }));
});
