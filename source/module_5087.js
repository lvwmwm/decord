...
arg5.default = {
  read(getUint8, sum) {
    ...
    const sum2 = sum1 + 65536 * module(5055).getByteAt(getUint8, sum + 2) + 1;
    obj.ImageWidth = { value: sum2, description: `${tmp9}px` };
    const sum3 = sum + c3;
    const obj2 = { value: sum2, description: `${tmp9}px` };
    const tmpResult1 = module(5055);
    const byteAt2 = module(5055).getByteAt(getUint8, sum3);
    const tmpResult2 = module(5055);
    const sum4 = byteAt2 + 256 * module(5055).getByteAt(getUint8, sum3 + 1);
    const tmpResult3 = module(5055);
    const sum5 = sum4 + 65536 * module(5055).getByteAt(getUint8, sum3 + 2) + 1;
