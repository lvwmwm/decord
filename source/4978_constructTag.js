...
// Function ID: 4979
// Name: constructTag
// Dependencies: [4934, 4950, 4962, 4931, 4938, 4964]

// Module 4978 (constructTag)
const require = arg1;
const module = arg2;
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  const stringValueFromArray = require(4931) /* getDataView */.getStringValueFromArray(items1);
  let combined = stringValueFromArray;
  if (type !== require(4938).TYPE_TEXT) {
    combined = stringValueFromArray;
    if (0 !== items2.length) {
      const _HermesInternal = HermesInternal;
      combined = "" + stringValueFromArray + " (" + tmp4(4931).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(4931);
    }
  }
  obj = { name: combined, value: stringFromDataView, description: null };
  if (type === require(4938).TYPE_ITXT) {
    stringFromDataView = module(4964).decode("UTF-8", decompressResult);
    const obj5 = module(4964);
  }
