...
// Function ID: 5536
// Name: map
// Dependencies: [32, 19, 5533, 5523, 5536, 5509, 5524, 5471]
...
function isGestureEnabled(closure_0) {
  if (obj.isComposedGesture(closure_0)) {
    ...
    someResult = false !== require(5523) /* hash */.maybeUnpackValue(closure_0.config.enabled);
    const tmpResult = require(5523) /* hash */;
  }
  return someResult;
}
...
export const resolveInternalConfigProps = function resolveInternalConfigProps(useAnimated) {
  ...
  if (!useAnimated) {
    useAnimated = require(5536) /* isNativeEvent */.isNativeAnimatedEvent(useAnimated.onUpdate);
    const obj = require(5536) /* isNativeEvent */;
  }
  ...
  if (!disableReanimated) {
    result = undefined !== require(5509).Reanimated;
  }
  if (result) {
    result = require(5523) /* hash */.hasWorkletEventHandlers(useAnimated);
    const obj2 = require(5523) /* hash */;
  }
  if (result) {
    result = !useAnimated.dispatchesAnimatedEvents;
  }
  useAnimated.shouldUseReanimatedDetector = result;
  useAnimated.needsPointerData = require(5536) /* isNativeEvent */.shouldHandleTouchEvents(useAnimated);
};
export const prepareConfigForNativeSide = function prepareConfigForNativeSide(closure_0, closure_1) {
  ...
  const PropsWhiteLists = require(5524) /* items2 */.PropsWhiteLists;
  let EMPTY_WHITE_LIST = PropsWhiteLists.get(closure_0);
  if (EMPTY_WHITE_LIST == null) {
    EMPTY_WHITE_LIST = require(5524) /* items2 */.EMPTY_WHITE_LIST;
  }
  ...
  while (tmp12 !== undefined) {
    ...
    let allowedNativeProps = require(5524) /* items2 */.allowedNativeProps;
