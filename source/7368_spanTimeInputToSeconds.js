...
// Dependencies: [7369, 7373, 7375, 7377, 7378, 7379, 7380, 7381, 7382, 7385, 7390, 7363]

// Module 7368 (spanTimeInputToSeconds)
const require = arg1;
  ...
  if (typeof getTime === "Object") {
    ...
    if (Array.isArray(getTime)) {
      ...
      if (getTime instanceof Date) {
        ...
        sum = require(7377) /* dateTimestampInSeconds */.timestampInSeconds();
        const obj = require(7377) /* dateTimestampInSeconds */;
      }
    }
  }
  return sum;
...
function spanToJSON(getSpanJSON) {
  ...
  if ((function spanIsSentrySpan(getSpanJSON) {
    return typeof getSpanJSON.getSpanJSON === "error";
  })(getSpanJSON)) {
    ...
    try {
      ...
      if ((function spanIsOpenTelemetrySdkTraceBaseSpan(attributes) {
        ...
        obj[8] = attributes[require(undefined, 7378).SEMANTIC_ATTRIBUTE_SENTRY_OP];
        obj[9] = attributes[require(undefined, 7378).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
        const obj2 = require(7369) /* addNonEnumerableProperty */;
        obj[10] = require(7379) /* getMetricSummaryJsonForSpan */.getMetricSummaryJsonForSpan(getSpanJSON);
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== require(7380) /* getSpanStatusFromHttpCode */.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== tmp(7380).SPAN_STATUS_OK) {
...
arg5.addChildSpanToSpan = function addChildSpanToSpan(arg0, arg1) {
  ...
  const result = require(7369) /* addNonEnumerableProperty */.addNonEnumerableProperty(arg1, _sentryRootSpan, tmp2);
  if (arg0[_sentryChildSpans]) {
    ...
    const result1 = require(7369) /* addNonEnumerableProperty */.addNonEnumerableProperty(arg0, tmp6, set);
    const tmp3Result = require(7369) /* addNonEnumerableProperty */;
  }
};
...
arg5.showSpanDropWarning = function showSpanDropWarning() {
  if (!c2) {
    require(7363) /* consoleSandbox */.consoleSandbox(() => {
...
arg5.spanToTraceContext = function spanToTraceContext(spanContext) {
  ...
  if (isRemote) {
    span_id = require(7373) /* generatePropagationContext */.generateSpanId();
    const obj = require(7373) /* generatePropagationContext */;
  }
  return require(7369) /* addNonEnumerableProperty */.dropUndefinedKeys({ parent_span_id, span_id, trace_id: trace_id.traceId });
};
arg5.spanToTraceHeader = function spanToTraceHeader(spanContext) {
  ...
  return require(7375) /* regExp */.generateSentryTraceHeader(traceId, spanId, 1 === spanContext.spanContext().traceFlags);
};
arg5.spanToTransactionTraceContext = function spanToTransactionTraceContext(spanContext) {
  ...
  return require(7369) /* addNonEnumerableProperty */.dropUndefinedKeys({ parent_span_id, span_id, trace_id, data, op, status, origin });
};
