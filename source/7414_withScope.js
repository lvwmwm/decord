...
// Function ID: 7415
// Name: withScope
// Dependencies: [41, 42, 7415, 7401, 7412, 7420]
// Exports: getStackAsyncContextStrategy

// Module 7414 (withScope)
import _classCallCheck from "_classCallCheck";
import _createClass from "_createClass";

let AsyncContextStack = require;
function withScope(arg0) {
  const mainCarrier = AsyncContextStack(7412).getMainCarrier();
  const obj = AsyncContextStack(7412);
  const sentryCarrier = AsyncContextStack(7412).getSentryCarrier(mainCarrier);
...
export function getStackAsyncContextStrategy() {
  return {
    ...
    withSetIsolationScope(arg0, arg1) {
      const callback = arg1;
      let mainCarrier = callback(7412).getMainCarrier();
      let obj = callback(7412);
      let sentryCarrier = callback(7412).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7420);
    ...
    getCurrentScope() {
      const mainCarrier = callback(7412).getMainCarrier();
      const obj = callback(7412);
      const sentryCarrier = callback(7412).getSentryCarrier(mainCarrier);
      let stack = sentryCarrier.stack;
      if (!stack) {
        let tmpResult = tmp(7420);
        const defaultCurrentScope = tmpResult.getDefaultCurrentScope();
