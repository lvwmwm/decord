...
// Function ID: 5724
// Name: memo
// Dependencies: [19, 17, 21, 1654, 5517, 5510, 5506, 5533]
...
const memoResult = memo(function BottomSheetRefreshControlComponent(arg0) {
  ...
  let iter;
  const tmp4 = iter(scrollableGesture(5517).BottomSheetDraggableContext);
  dependencyMap = tmp4;
  let obj = scrollableGesture(5510);
  ...
  const fn = function f() {
    return { enabled: iter.value === scrollableGesture(5506).SCROLLABLE_STATE.UNLOCKED };
  };
  obj = { animatedScrollableState: iter, SCROLLABLE_STATE: tmp2(5506).SCROLLABLE_STATE };
  fn.__closure = obj;
  ...
  const tmp7 = callback(() => {
    let result;
    if (dependencyMap) {
      const Gesture = scrollableGesture(5533).Gesture;
  ...
  if (tmp7) {
    ...
    let tmp8Result = tmp8(tmp2(5533).GestureDetector, obj);
  } else {
    const obj2 = {};
