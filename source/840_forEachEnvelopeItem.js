...
export const createAttachmentEnvelopeItem = function createAttachmentEnvelopeItem(data) {
  ...
  const obj = { type: "attachment", length: data.length };
  ({ filename: obj.filename, contentType: obj.content_type, attachmentType: obj.attachment_type } = data);
...
export const parseEnvelope = function parseEnvelope(arg0) {
  ...
  function readJson() {
    ...
    const sentryCarrier = tmp(readBinary[2]).getSentryCarrier(tmp(readBinary[3]).GLOBAL_OBJ);
    if (sentryCarrier.decodePolyfill) {
      let decodePolyfillResult = sentryCarrier.decodePolyfill(tmp2);
    } else {
