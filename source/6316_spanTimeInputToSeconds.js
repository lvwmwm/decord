...
// Module 6316 (spanTimeInputToSeconds)
  ...
  if ("number" === typeof getTime) {
    ...
    if (Array.isArray(getTime)) {
      ...
      if (getTime instanceof Date) {
        ...
        sum = require(6325) /* dateTimestampInSeconds */.timestampInSeconds();
        const obj = require(6325) /* dateTimestampInSeconds */;
      }
    }
  }
  return sum;
...
function spanToJSON(getSpanJSON) {
  ...
  if ("function" === typeof getSpanJSON.getSpanJSON) {
    ...
    if (!tmp) {
      ...
      obj.op = attributes[require(undefined, 6326).SEMANTIC_ATTRIBUTE_SENTRY_OP];
      obj.origin = attributes[require(undefined, 6326).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
      const obj2 = require(6317) /* addNonEnumerableProperty */;
      const tmp10 = tmp9;
      obj._metrics_summary = require(6327) /* getMetricSummaryJsonForSpan */.getMetricSummaryJsonForSpan(obj);
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== require(6328) /* getSpanStatusFromHttpCode */.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== require(6328) /* getSpanStatusFromHttpCode */.SPAN_STATUS_OK) {
...
function getActiveSpan() {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    const obj4 = require(6333) /* _getSpanForScope */;
    activeSpan = obj4._getSpanForScope(require(6338) /* getCurrentScope */.getCurrentScope());
    const obj5 = require(6338) /* getCurrentScope */;
  }
  return activeSpan;
}
let c3 = false;
...
export const addChildSpanToSpan = function addChildSpanToSpan(_sentryRootSpan) {
  const result = require(6317) /* addNonEnumerableProperty */.addNonEnumerableProperty(arg1, "_sentryRootSpan", _sentryRootSpan._sentryRootSpan || _sentryRootSpan);
  if (_sentryRootSpan._sentryChildSpans) {
    ...
    const result1 = require(6317) /* addNonEnumerableProperty */.addNonEnumerableProperty(_sentryRootSpan, "_sentryChildSpans", set);
    const obj2 = require(6317) /* addNonEnumerableProperty */;
  }
};
...
export const showSpanDropWarning = function showSpanDropWarning() {
  if (!c3) {
    require(6311) /* consoleSandbox */.consoleSandbox(() => {
...
export const spanToTraceContext = function spanToTraceContext(spanContext) {
  ...
  if (isRemote) {
    let obj = require(6321) /* generateTraceId */;
    spanId = obj.generateSpanId();
  }
  obj = { parent_span_id, span_id: spanId, trace_id: spanContextResult.traceId };
  return require(6317) /* addNonEnumerableProperty */.dropUndefinedKeys(obj);
};
export const spanToTraceHeader = function spanToTraceHeader(spanContext) {
  ...
  return require(6323) /* extractTraceparentData */.generateSentryTraceHeader(traceId, spanId, tmp2);
};
export const spanToTransactionTraceContext = function spanToTransactionTraceContext(spanContext) {
  ...
  return require(6317) /* addNonEnumerableProperty */.dropUndefinedKeys({ parent_span_id, span_id, trace_id, data, op, status, origin });
};
export const updateMetricSummaryOnActiveSpan = function updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, tags, bucketKey) {
  ...
  if (tmp) {
    const obj = require(6327) /* getMetricSummaryJsonForSpan */;
...
export const updateSpanName = function updateSpanName(updateName) {
  ...
  updateName.setAttributes(_defineProperty(_defineProperty({}, require(6326).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "custom"), require(6326).SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME, arg1));
};
