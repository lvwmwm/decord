...
// Dependencies: [19, 1678, 1697, 1660]
// Exports: useAnimatedSensor

// Module 1809 (eulerToQuaternion)
...
function adjustRotationToInterfaceOrientation(pitch) {
  ...
  if (interfaceOrientation === require(1678) /* isWorkletFunction */.InterfaceOrientation.ROTATION_90) {
    pitch.pitch = roll;
    pitch.roll = -pitch;
    const _Math2 = Math;
    pitch.yaw = yaw - Math.PI / 2;
  } else if (interfaceOrientation === tmp(1678).InterfaceOrientation.ROTATION_270) {
    pitch.pitch = -roll;
    pitch.roll = pitch;
    const _Math = Math;
    pitch.yaw = yaw + Math.PI / 2;
  } else if (interfaceOrientation === tmp(1678).InterfaceOrientation.ROTATION_180) {
...
export const useAnimatedSensor = function useAnimatedSensor(arg0, adjustToInterfaceOrientation) {
  ...
  obj[0] = _require(1697).initializeSensor(arg0, tmp13);
