...
// Module 13216 (__exportStarResult11)
const require = arg1;
...
arg5.ApplyUnsignedRoundingMode = function ApplyUnsignedRoundingMode(eq, lessThan, ZERO, result) {
  if (eq.eq(lessThan)) {
    ...
    require(13204) /* setInternalSlot */.invariant(lessThan.lessThan(eq) && eq.lessThan(ZERO), combined1.concat(ZERO));
    if ("zero" === result) {
      return lessThan;
      ...
      if (minusResult.lessThan(minusResult1)) {
        ...
        require(13204) /* setInternalSlot */.invariant(minusResult.eq(minusResult1), "d1 should be equal to d2");
        if ("half-zero" === result) {
          return lessThan;
        } else if ("half-infinity" === result) {
          return ZERO;
        } else {
          require(13204) /* setInternalSlot */.invariant("half-even" === result, "unsignedRoundingMode should be half-even");
