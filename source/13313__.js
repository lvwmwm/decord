...
function createIntegerConversion(closure_0, unsigned) {
  const get = tmp;
  if (64 === closure_0) {
    let _Math4 = Math;
    ...
    c0 = 0;
    let _Math = Math;
    closure_1 = Math.pow(2, closure_0) - 1;
  }
  let closure_3 = Math.pow(2, closure_0);
  let closure_4 = Math.pow(2, closure_0 - 1);
  return (arg0, arg1) => {
    let obj = arg1;
    if (undefined === arg1) {
      obj = {};
    ...
    if (obj.enforceRange) {
      if (_Number.isFinite(tmp)) {
        ...
        const typeError = new TypeError(callback("is outside the accepted range of " + c0 + " to " + closure_1 + ", inclusive", obj));
        throw typeError;
      } else {
      ...
      if (!_Number.isNaN(tmp)) {
        if (obj.clamp) {
          const _Math = Math;
          const _Math2 = Math;
          const bound = Math.min(Math.max(tmp, c0), closure_1);
          if (bound > 0) {
            if (bound % 1 === 0.5) {
...
const item = items.forEach((name) => {
  ...
  name[name] = (arg0, arg1) => {
    ...
    const typeError = new TypeError(outer1_3("is not " + str + " " + name + " object", arg1));
    throw typeError;
  };
});
