...
// Function ID: 5525
// Name: hash
// Dependencies: [32, 5511, 5510, 5525, 5534]
...
function maybeUnpackValue(enabled) {
  const Reanimated = require(5510).Reanimated;
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === require(5510).Reanimated) {
    return [];
  } else {
...
export const bindSharedValues = function bindSharedValues(closure_1, _slicedToArray) {
  ...
  if (undefined !== _require(5510).Reanimated) {
...
export const hasWorkletEventHandlers = function hasWorkletEventHandlers(useAnimated) {
  const iter = require(5525) /* items2 */.HandlerCallbacks[Symbol.iterator]();
