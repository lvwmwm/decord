...
// Dependencies: [32, 7421, 7427, 7416, 7429, 7399, 7411, 7412, 7413, 7406, 7404, 7422, 7394, 7426, 7430, 7432, 7423, 7433, 7435, 7409]
// Exports: continueTrace, startInactiveSpan, startNewTrace, startSpan, startSpanManual, suppressTracing, withActiveSpan

// Module 7428 (createChildOrRootSpan)
...
function createChildOrRootSpan(forceTransaction) {
  ...
  let obj = require(7426) /* hasTracingEnabled */;
    ...
    if (parentSpan) {
      if (!forceTransaction.forceTransaction) {
        ...
        if (spanIsSampledResult) {
          ...
          let sentrySpan = new tmp(7435).SentrySpan(obj);
        } else {
          obj = { traceId: null };
          obj[0] = traceId;
          sentrySpan = new tmp(7427).SentryNonRecordingSpan(obj);
        }
        const spanContextResult = parentSpan.spanContext();
        tmp(7399).addChildSpanToSpan(parentSpan, sentrySpan);
        const tmpResult1 = tmp(7399);
        const client = tmp(7421).getClient();
        if (client) {
      ...
      tmp(7432).logSpanStart(sentrySpan);
      const tmpResult4 = tmp(7432);
      const result = tmp(7423).setCapturedScopesOnSpan(sentrySpan, scope, isolationScope);
      return sentrySpan;
    }
    if (parentSpan) {
      const dynamicSamplingContextFromSpan = tmp(7430).getDynamicSamplingContextFromSpan(parentSpan);
      const tmpResult6 = tmp(7430);
      ...
      const tmp42 = _startRootSpan(obj1, scope, tmp(7399).spanIsSampled(parentSpan));
      const spanIsSampledResult1 = tmp(7399).spanIsSampled(parentSpan);
      tmp(7430).freezeDscOnSpan(tmp42, dynamicSamplingContextFromSpan);
      sentrySpan = tmp42;
      const tmpResult8 = tmp(7430);
    } else {
      const obj2 = {};
      ...
      sentrySpan = tmp33;
      if (dsc) {
        tmp(7430).freezeDscOnSpan(tmp33, dsc);
        sentrySpan = tmp33;
        const tmpResult9 = tmp(7430);
      }
    }
  } else {
    const sentryNonRecordingSpan = new tmp(7427).SentryNonRecordingSpan();
...
function _startRootSpan(name) {
  let tmp6;
  let tmp7;
  let obj = require(7421) /* getClient */;
  ...
  if (arg1.getScopeData().sdkProcessingMetadata[__SENTRY_SUPPRESS_TRACING__]) {
    ...
    obj[1] = arg2;
    obj[3] = obj;
    sampleSpanResult = tmp(7433).sampleSpan(options, obj);
  ...
  obj1.attributes = { [require(7409).SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: "custom" };
  obj1.sampled = tmp6;
  const sentrySpan = new tmp(7435).SentrySpan(obj1);
  if (undefined !== tmp7) {
    const attr = sentrySpan.setAttribute(tmp(7409).SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, tmp7);
  }
  if (client) {
    client.emit("spanStart", sentrySpan);
  }
...
export const continueTrace = (arg0, arg1) => {
  ...
  let dependencyMap;
  const _require = arg1;
  const mainCarrier = _require(7412).getMainCarrier();
  const obj = _require(7412);
  const tmp = _require;
  const asyncContextStrategy = _require(7413).getAsyncContextStrategy(mainCarrier);
  if (asyncContextStrategy.continueTrace) {
    ...
    return tmp(7421).withScope((setPropagationContext) => {
...
export const startInactiveSpan = function startInactiveSpan(experimental) {
  ...
  if (asyncContextStrategy.startInactiveSpan) {
    ...
    if (experimental.startTime) {
      ...
      obj.startTimestamp = tmp3(7399).spanTimeInputToSeconds(experimental.startTime);
      delete tmp[tmp2];
      tmp10 = obj;
      const tmp3Result = tmp3(7399);
    }
      ...
      fn = undefined !== parentSpan ? ((arg0) => {
        ...
        if (asyncContextStrategy.withActiveSpan) {
          ...
          withActiveSpanResult = tmp2(tmp3[1]).withScope((arg0) => {
            _null(7416)._setSpanForScope(arg0, _null);
...
export const startNewTrace = function startNewTrace(arg0) {
  const _require = arg0;
  return _require(7421).withScope((setPropagationContext) => {
...
export const suppressTracing = function suppressTracing(arg0) {
  ...
  if (asyncContextStrategy.suppressTracing) {
    ...
    suppressTracingResult = tmp(7421).withScope((setSDKProcessingMetadata) => {
      const result = setSDKProcessingMetadata.setSDKProcessingMetadata({ [outer1_3]: true });
      return callback();
    });
    const tmpResult = tmp(7421);
  }
...
export const withActiveSpan = function withActiveSpan(arg0, arg1) {
  ...
  if (asyncContextStrategy.withActiveSpan) {
    ...
    withActiveSpanResult = tmp(7421).withScope((arg0) => {
      _null(7416)._setSpanForScope(arg0, _null);
      return dependencyMap(arg0);
    });
    const tmpResult = tmp(7421);
