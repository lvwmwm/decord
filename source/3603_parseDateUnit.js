...
let closure_5 = /^-?(?:(\d{3})|(\d{2})(?:-?(\d{2}))?|W(\d{2})(?:-?(\d{1}))?|)$/;
let closure_6 = /^(\d{2}(?:[.,]\d*)?)(?::?(\d{2}(?:[.,]\d*)?))?(?::?(\d{2}(?:[.,]\d*)?))?$/;
let closure_7 = /^([+-])(\d{2})(?::?(\d{2}))?$/;
let closure_8 = [];
...
export default function parseISO(offeredAt) {
  ...
  const defaultResult1 = closure_3.default(num2);
  if (2 !== defaultResult1) {
  ...
  if (!obj.date) {
      ...
      if (!isNaN(undefined.getTime())) {
        ...
        if (obj.time) {
          const tmp19 = function parseTime(time) {
            const match = time.match(closure_6);
            if (match) {
              const tmp4 = callback3(match[1]);
              const tmp5 = callback3(match[2]);
              const tmp6 = callback3(match[3]);
              if (24 === tmp4) {
              ...
              if (tmp7) {
                const result = tmp4 * callback(closure_1[2]).millisecondsInHour;
                return result + tmp5 * callback(closure_1[2]).millisecondsInMinute + 1000 * tmp6;
              } else {
                const _NaN2 = NaN;
                return NaN;
              }
        ...
        if (obj.timezone) {
          const tmp34 = function parseTimezone(timezone) {
            if ("Z" === timezone) {
              ...
              const match = timezone.match(closure_7);
              if (match) {
                let num2 = 1;
                if ("+" === match[1]) {
                  num2 = -1;
                ...
                if (tmp3) {
                  const result = parsed * callback(closure_1[2]).millisecondsInHour;
                  return num2 * (result + num5 * callback(closure_1[2]).millisecondsInMinute);
                } else {
                  const _NaN = NaN;
                  return NaN;
                }
          ...
          if (isNaN(tmp34)) {
            const _NaN2 = NaN;
            const prototype5 = _Date3.prototype;
            _Date3 = new _Date3(NaN);
            return _Date3;
          } else {
            const prototype4 = _Date3.prototype;
            const _Date31 = new _Date3(time + num5 + tmp34);
            return _Date31;
          }
        } else {
          const _Date = Date;
          const date2 = new Date(time + num5);
          const _Date2 = Date;
    ...
    function parseDate(restDateString, year) {
      if (null === year) {
        ...
        const match = restDateString.match(closure_5);
        if (match) {
          const tmp8 = callback2(match[1]);
          const diff = callback2(match[2]) - 1;
          const tmp10 = callback2(match[3]);
          const tmp11 = callback2(match[4]);
          const diff1 = callback2(match[5]) - 1;
          if (tmp6) {
            const _Date3 = Date;
