...
// Function ID: 5551
// Name: useGesture
// Dependencies: [19, 5489, 5472, 5551, 5535, 5511, 5498, 5471]
// Exports: useGesture

// Module 5550 (useGesture)
...
export const useGesture = function useGesture(Fling, clonedAndRemappedConfig) {
  ...
  if (clonedAndRemappedConfig.disableReanimated !== jsEventHandler(() => clonedAndRemappedConfig.disableReanimated, [])) {
    const _Error2 = Error;
    const error = new Error(_require(5472).tagMessage("The \"disableReanimated\" property must not be changed after the handler is created."));
    throw error;
  } else {
    const gestureCallbacks = _require(5551).useGestureCallbacks(tmp2, clonedAndRemappedConfig);
    ...
    if (clonedAndRemappedConfig.shouldUseReanimatedDetector) {
      if (!reanimatedEventHandler) {
        const _Error = Error;
        const error1 = new Error(_require(5472).tagMessage("Failed to create reanimated event handlers."));
        throw error1;
      }
    }
    const items = [tmp2, , , ];
