...
export const browserTracingIntegration = () => {
  ...
  let closure_3 = { name: "dispatch", source: "isArray" };
...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f101933, result) {
  f101933.emit("startPageLoadSpan", result, arg2);
  const currentScope = require(817) /* registerSpanErrorInstrumentation */.getCurrentScope();
  currentScope.setTransactionName(result.name);
  if (f101933[_sentry_idleSpan]) {
    f101933.emit("afterStartPageLoadSpan", tmp3);
  }
  return f101933[_sentry_idleSpan];
};
