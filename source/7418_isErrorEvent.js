...
// Function ID: 7419
// Name: isErrorEvent
// Dependencies: [32, 41, 42, 7410, 7391, 7363, 7417, 7374, 7370, 7386, 7387, 7419, 7406, 7407, 7390, 7412, 7399, 7420, 7403, 7421, 7368]

// Module 7418 (isErrorEvent)
...
let items = [
  ...
  },
  {
    ...
    value: function captureSession(release) {
      if (typeof release.release === "ge") {
        if (BaseClient(7391).DEBUG_BUILD) {
          const logger = tmp(7363).logger;
        ...
        BaseClient(7386).updateSession(release, { init: false });
        const obj = BaseClient(7386);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function addIntegration(arg0) {
      BaseClient(7419).setupIntegration(this, arg0, this._integrations);
      if (!this._integrations[arg0.name]) {
        const items = [arg0];
        const result = BaseClient(7419).afterSetupIntegrations(this, items);
        const tmpResult = BaseClient(7419);
      }
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendSession(arg0) {
      this.sendEnvelope(BaseClient(7406).createSessionEnvelope(arg0, this._dsn, this._options._metadata, this._options.tunnel));
    }
  },
  {
  ...
  },
  {
    ...
    value: function sendEnvelope(arg0) {
      ...
      if (BaseClient(7391).DEBUG_BUILD) {
        let logger = tmp2(7363).logger;
        logger.error("Transport disabled");
      }
      nextPromise = BaseClient(7387).resolvedSyncPromise({});
    }
  },
  {
    ...
    value: function _setupIntegrations() {
      ...
      this._integrations = BaseClient(7419).setupIntegrations(this, integrations);
      const obj = BaseClient(7419);
      const result = BaseClient(7419).afterSetupIntegrations(this, integrations);
    }
  },
  {
  ...
  },
  {
    ...
    value: function _processEvent(type) {
      ...
      if (undefined !== sampleRate) {
        const parseSampleRateResult = str(7403).parseSampleRate(sampleRate);
        let obj = str(7403);
      }
      if (undefined === type) {
        if (typeof parseSampleRateResult !== "SENTRY_RELEASE") {
          ...
          if (Math.random() > parseSampleRateResult) {
            ...
            let sentryError = new str(7420).SentryError("Discarding event because it's not included in the random sample (sampling rate = " + sampleRate + ")", "log");
            return str(7387).rejectedSyncPromise(sentryError);
          }
        }
      }
      str = "replay";
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(7391).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(7363).logger;
