...
export const GestureDetector = function GestureDetector(gesture) {
  ...
  if (gesture.gesture) {
    ...
    const current = callback({ wGiHkK: "You can set exactly one of itemVisiblePercentThreshold or viewAreaCoveragePercentThreshold. Specifying both is not supported.", WIDGET_ENABLED: "FlashList v2 is only supported on new architecture", m8xsti: "LayoutManager is not initialized, layout info is unavailable", display: "LayoutManager is not initialized, child container layout is unavailable" }).current;
    obj = { flexDirection: -1, flexShrink: -1882570463, alignItems: 1360068610, alignSelf: 1361143520, textAlign: 1340580, attachedGestures: [], shouldUseReanimated: someResult };
