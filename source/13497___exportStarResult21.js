...
// Dependencies: [13483, 13481, 13498, 13499, 13500]

// Module 13497 (__exportStarResult21)
const require = arg1;
...
arg5.FormatNumericToString = function FormatNumericToString(roundingType, timesResult) {
  ...
  if (timesResult.isZero()) {
    if (timesResult.isNegative()) {
      let ZERO = require(13483) /* TEN */.ZERO;
      let str2 = "negative";
      let tmp5Result = dependencyMap;
      let tmp5 = require;
    }
    roundingType = roundingType.roundingType;
    const result = tmp5(13498).GetUnsignedRoundingMode(roundingType.roundingMode, tmp9);
    if ("significantDigits" === roundingType) {
      let ToRawPrecisionResult = tmp5(13499).ToRawPrecision(ZERO, roundingType.minimumSignificantDigits, roundingType.maximumSignificantDigits, result);
    } else if ("fractionDigits" === roundingType) {
      ToRawPrecisionResult = tmp5(13500).ToRawFixed(ZERO, roundingType.minimumFractionDigits, roundingType.maximumFractionDigits, roundingType.roundingIncrement, result);
    } else {
      const ToRawPrecisionResult1 = tmp5(13499).ToRawPrecision(ZERO, roundingType.minimumSignificantDigits, roundingType.maximumSignificantDigits, result);
      let ToRawFixedResult = tmp5(13500).ToRawFixed(ZERO, roundingType.minimumFractionDigits, roundingType.maximumFractionDigits, roundingType.roundingIncrement, result);
      if ("morePrecision" === roundingType.roundingType) {
        ...
        tmp5(13481).invariant("lessPrecision" === roundingType.roundingType, "Invalid roundingType");
    ...
    if (integerDigitsCount < minimumIntegerDigits) {
      sum = tmp5(13481).repeat("0", minimumIntegerDigits - integerDigitsCount) + substr;
    }
    if ("negative" !== str2) {
  ...
  require(13481) /* getMultiInternalSlots */.invariant(timesResult.isFinite(), "NumberFormatDigitInternalSlots value is not finite");
  let str = "positive";
