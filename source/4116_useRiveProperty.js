...
// Dependencies: [32, 19, 4117]
// Exports: useRiveProperty

// Module 4116 (useRiveProperty)
...
export const useRiveProperty = function useRiveProperty(instance, FillColor, f29113) {
  const _require = instance;
  const dependencyMap = FillColor;
  const callback = f29113;
  let tmp = first(undefined);
  const callback2 = tmp;
  const items = [instance, FillColor];
  const disposableMemo = _require(4117).useDisposableMemo(() => {
    if (closure_0) {
