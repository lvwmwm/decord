...
// Dependencies: [2043, 4845, 9529, 6531, 6952, 1220, 2044, 2066, 9530, 4844, 2098, 2036, 1372, 1074, 7454, 7452, 4418, 12, 9531, 9533, 6742, 6683, 9534, 8878, 5051, 9542, 9543, 9545, 9538, 9546, 9550, 4983, 2020, 1186, 7270, 9551, 5009, 11, 504, 573, 2]
...
const playInAppMessageSound = tmp(9551);
require = fn;
          ...
          if (tmpResult8.allowInAppNotifications()) {
            if (!tmp3Result.isEnabled()) {
              if (!ChannelRTCStore.getChatOpen(channel_id)) {
                const result = tmp(9534).shouldIncludeSelectedChannel();
                const tmpResult9 = tmp(9534);
                if (tmpResult10.shouldNotify(message, channel_id, result)) {
                  ...
                  if (null == channel) {
                    ...
                    if (tmpResult12.isMessageContentPreviewable(messageRecord)) {
                      ...
                      const notificationDuration = tmp(9543).getNotificationDuration(MESSAGE);
                      ...
                      const tmpResult13 = tmp(9543);
                      obj3.inAppNotificationId = tmp(9543).generateInAppNotificationId();
                    ...
                    tmpResult12 = tmp(9542);
                  }
                  obj7 = ChannelStore;
                } else {
                  return false;
                }
                tmpResult10 = tmp(9534);
              }
            }
                  ...
                  if (null != channel) {
                    if (!obj4.isEnabled()) {
                      if (!ChannelRTCStore.getChatOpen(channelId)) {
                        const result1 = tmp19(9534).shouldIncludeSelectedChannel();
                        const tmp19Result9 = tmp19(9534);
                        ...
                        if (tmp19Result10.shouldNotifyForReaction(obj)) {
                          ...
                          if (tmp19Result12.isMessageContentPreviewable(messageRecord)) {
                            ...
                            const obj3 = {
                              type: REACTION,
                              key: channelId,
                              duration: tmp19(9543).getNotificationDuration(REACTION),
                              onDismiss() {
                                                        InAppNotificationActionCreatorsDefault.clearNotification();
                                                      },
                              channel,
                            ...
                            const tmp19Result13 = tmp19(9543);
                            obj3.inAppNotificationId = tmp19(9543).generateInAppNotificationId();
                            obj2.notification = obj3;
                            handleEnqueueNotification(obj2);
                          } else {
                            return false;
                          }
                          tmp19Result12 = tmp19(9542);
                        } else {
                          return false;
                        }
                        tmp19Result10 = tmp19(9534);
                      }
                    }
                    return false;
                  }
      ...
      if (null != savedMessage.message) {
        ...
        if (null != channel) {
          ...
          const obj3 = {
            ...
            duration: tmp(9543).getNotificationDuration(MESSAGE_REMINDER),
            onDismiss() {
                    InAppNotificationActionCreatorsDefault.clearNotification();
                  },
            channel,
              ...
              if (null == user) {
                ...
                if (null == guild) {
                  ...
                  const obj3 = {
                    ...
                    guild,
                    key: channel.id,
                    duration: tmp(9543).getNotificationDuration(FORUM_THREAD_CREATED),
                    onDismiss() {
                                    InAppNotificationActionCreatorsDefault.clearNotification();
                                  },
                    inAppNotificationId: null
                  };
                  const tmpResult5 = tmp(9543);
                  obj3.inAppNotificationId = tmp(9543).generateInAppNotificationId();
      ...
      if (null != diffSchedulesResult) {
        ...
        if (EnableScreenDowntimeScheduleNotifications.getSetting()) {
          if (tmp2Result7.allowInAppNotifications()) {
            ...
            const notificationDuration = tmp2(9543).getNotificationDuration(RESTRICTED_SCHEDULE_UPDATED);
            const tmp2Result8 = tmp2(9543);
            obj3.key = tmp2(9531).restrictedScheduleNotificationKey(diffSchedulesResult.kind);
            obj3.duration = notificationDuration;
            obj3.onDismiss = function onDismiss() {
              InAppNotificationActionCreatorsDefault.clearNotification();
            };
            const tmp2Result9 = tmp2(9531);
            obj3.title = tmp2(9531).getRestrictedScheduleNotificationTitle(diffSchedulesResult.kind);
            const tmp2Result10 = tmp2(9531);
            obj3.subtitle = tmp2(9531).getRestrictedScheduleNotificationSubtitle(diffSchedulesResult.rule);
            const tmp2Result11 = tmp2(9531);
            obj3.inAppNotificationId = tmp2(9543).generateInAppNotificationId();
            obj2.notification = obj3;
            handleEnqueueNotification(obj2);
            const tmp2Result12 = tmp2(9543);
          }
          tmp2Result7 = tmp2(9534);
        }
      }
    } else {
      return false;
