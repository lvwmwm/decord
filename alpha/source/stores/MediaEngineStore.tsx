...
// Dependencies: [32, 5, 1993, 1998, 4746, 1999, 4874, 1220, 502, 13524, 9094, 2044, 4852, 1372, 13525, 1074, 4871, 5038, 13526, 1084, 13527, 4854, 3, 1091, 1364, 4884, 12821, 2039, 5444, 13528, 13529, 13530, 8963, 12, 13531, 13343, 13532, 13533, 13534, 13535, 13536, 8867, 1241, 573, 9440, 13537, 13538, 13539, 5727, 13540, 13585, 13586, 13587, 13588, 13589, 13590, 4964, 4823, 4447, 510, 13591, 9426, 5872, 13595, 13596, 1115, 5166, 13597, 13598, 13599, 38, 13355, 13354, 13337, 1231, 13202, 8878, 504, 13600, 13601, 13602, 13603, 2]
          ...
          if (null != tmp2.desktopSource) {
            ...
            if (!isWindowsResult) {
              ...
              if (isWindowsResult1) {
                enabled = true === c132;
                if (!enabled) {
                  const WGCDirtyRegionsAllExperiment = tmp68(13533).WGCDirtyRegionsAllExperiment;
              ...
              if (videoHook2) {
                const VideoHookDX12Experiment = tmp68(13534).VideoHookDX12Experiment;
                enabled2 = VideoHookDX12Experiment.getConfig({ location: "updateVideo" }).enabled;
              }
              const UpscaleSmallCapturedFramesExperiment = tmp68(13535).UpscaleSmallCapturedFramesExperiment;
              ...
              obj7.enableGlobalFramePoolLock = tmp68(13536).getGlobalFramePoolLockExperimentConfig({ location: "updateVideo" }).enabled;
              ...
              const tmp68Result7 = tmp68(13536);
            } else {
...
function handleVideoFilterError(code, arg1) {
  ...
  if (arg1 !== constants10.LIVE) {
    ...
    if (constants9.UNSUPPORTED === code) {
      AVUnderlyingError = tmp4(8867).AVUnderlyingError;
      let VideoBackgroundInitFailed = AVUnderlyingError.VideoBackgroundUnsupported;
    } else if (tmp5.INIT_FAILED === code) {
      VideoBackgroundInitFailed = tmp4(8867).AVUnderlyingError.VideoBackgroundInitFailed;
    }
...
function updateConnectionVoiceProcessing(setEchoCancellation) {
  ...
  if (isWindowsResult) {
    if (setEchoCancellation.context === MediaEngineContextTypes.DEFAULT) {
      const systemwideEchoCancellationExperimentConfig = tmp30(13538).getSystemwideEchoCancellationExperimentConfig({ location: "updateConnectionVoiceProcessing" });
...
let closure_157 = async function _maybeProbeAudioEffects(arg0, value) {
  if (c6 === 2) {
    c6 = 3;
    throw new TypeError("Generator functions may not be called on executing generators");
  } else if (tmp6 === 3) {
          ...
          if (obj6.isWindows()) {
            if (obj2.getConfig({ location: "MediaEngineStore.setInputDevice" }).probeAudioEffects) {
              ...
              if (null != guid) {
                ...
                c5 = 2;
                c6 = 1;
                const obj5 = { value: tmp15(tmp30[64])(tmp17.guid, tmp28, constants), done: false };
                return obj5;
              }
            }
            obj2 = require("AudioEffectsExperiment");
            tmp15 = importDefault;
          }
...
prototype["initialize"] = function initialize() {
  ...
  closure_72.on(pollMetrics(4884).MediaEngineEvent.Connection, (setAttenuation) => {
    ...
    if (tmp9Result20.isWindows()) {
      ...
      if (tmp9Result21.isMac()) {
        ...
        if (satisfiesResult) {
          ...
          satisfiesResult = tmp25(13540).satisfies(release, closure_39);
          const tmp25Result = tmp25(13540);
        }
    ...
    if (tmp9Result26.isWeb()) {
      setAttenuation.setExperimentFlag(tmp22.BROWSER_HEVC, tmp25(13586).getConfig({ location: "MediaEngineStore" }).enabled);
      const tmp25Result2 = tmp25(13586);
    }
    ...
    const result2 = closure_72.setHasFullbandPerformance(tmp25(13590)());
  ...
  _modDef12.each(value2, (modeOptions) => {
    ...
    if (tmp8) {
      modeOptions.modeOptions.shortcut = tmp2(13591).toCombo(modeOptions.modeOptions.shortcut);
      const tmp2Result = tmp2(13591);
    }
...
prototype["getVideoEncoderExperiments"] = function getVideoEncoderExperiments(STREAM, streamer) {
  ...
  if (isWindowsResult) {
    ...
    const VideoCaptureDeviceNoReuseExperiment = tmp6(13602).VideoCaptureDeviceNoReuseExperiment;
...
const mediaEngineStore = new MediaEngineStore(DispatcherDefault, {
  ...
  CONNECTION_OPEN: function handleConnectionOpen(sessionId) {
    ...
    if (enabled) {
      enabled = tmp2(13598).getIOSAudioInterruptExperimentConfig("handleConnectionOpen").enabled;
      const tmp2Result3 = tmp2(13598);
    }
    if (enabled) {
    ...
    if (tmp2Result4.isIOS()) {
      ...
      if (setNcModels != null) {
        setNcModels(tmp2(13599).KRISP_NC_MODELS);
      }
      mediaEngineStore.emitChange();
    }
  ...
  AUDIO_SET_LOCAL_VIDEO_DISABLED: function handleSetLocalVideoDisabled(arg0) {
    ...
    if (!tmp22) {
      ...
      if (isAutomatic) {
        tmp3(13355)(userId, tmp12 ? constants12.AUTO_DISABLE : constants12.AUTO_ENABLE, tmp11);
        if (tmp12) {
          obj.add(userId);
        } else {
          obj.delete(userId);
        ...
        if (hasItem) {
          if (!tmp12) {
            ...
            tmp3(13355)(userId, constants12.MANUAL_REENABLE, tmp11);
          }
        }
        tmp3(13355)(userId, tmp12 ? constants12.MANUAL_DISABLE : constants12.MANUAL_ENABLE, tmp11);
      }
      if (tmp19) {
        tmp19 = !tmp12;
      }
  ...
  CLIPS_INIT: function handleClipsInit(applicationName) {
    ...
    if (obj.isClipsEnabled()) {
      if (null != DiscordNativeDefault) {
        ...
        enabled = videoHook2;
        if (videoHook2) {
          const VideoHookDX12Experiment = tmp(13534).VideoHookDX12Experiment;
          enabled = VideoHookDX12Experiment.getConfig({ location: "handleClipsInit" }).enabled;
        }
        const UpscaleSmallCapturedFramesExperiment = tmp(13535).UpscaleSmallCapturedFramesExperiment;
        ...
        ({ minCaptureWidth, minCaptureHeight } = config);
        const obj8 = result;
        const tmpResult4 = tmp(13528);
        let isWindowsResult = tmp(1364).isWindows();
        if (isWindowsResult) {
          const tmp3Result6 = tmp3(4447);
          let release;
          if (tmp3Result6 != null) {
            release = tmp3Result6.os.release;
          }
          isWindowsResult = tmp3(13540).satisfies(release, __initData7);
          const tmp3Result5 = tmp3(13540);
        }
        ...
        if (isMacResult) {
          const tmp3Result8 = tmp3(4447);
          let release1;
          if (tmp3Result8 != null) {
            release1 = tmp3Result8.os.release;
          }
          isMacResult = tmp3(13540).satisfies(release1, __initData4);
          const tmp3Result7 = tmp3(13540);
        }
