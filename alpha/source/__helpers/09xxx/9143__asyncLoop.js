...
// Function ID: 9144
// Name: _asyncLoop
// Dependencies: [41, 42, 5, 9144, 9145]
...
export const concatBytes = function concatBytes() {
  ...
  if (0 < items.length) {
    do {
      let arr2 = items[num];
      let bytesResult = Hash(9144).bytes(arr2);
...
export const randomBytes = function randomBytes(result) {
  ...
  if (Hash(9145).crypto) {
    if (typeof tmp(9145).crypto.getRandomValues === "function") {
      const _crypto2 = tmp(9145).crypto;
      const _Uint8Array = Uint8Array;
  ...
  if (Hash(9145).crypto) {
    if (typeof tmp(9145).crypto.randomBytes === "function") {
      const _crypto = tmp(9145).crypto;
