...
// Function ID: 13185
// Name: _flush
// Dependencies: [5, 13163, 13185, 13164, 13136, 13147, 13150, 13143, 13173, 13137, 13159]
  ...
  if (c1 === 2) {
    ...
    try {
      ...
      if (arg0 === 1) {
        ...
        c1 = 3;
        const obj3 = require("module_13163");
        tmp13 = closure_0;
      }
    } catch (tmp8) {
      c1 = tmp;
...
function endSession() {
  ...
  if (tmp3) {
    tmp(13159).closeSession(tmp3);
    const tmpResult = tmp(13159);
  }
  const isolationScope1 = _mod13163.getIsolationScope();
  const tmpResult4 = _mod13163;
  const currentScope1 = _mod13163.getCurrentScope();
  const tmpResult5 = _mod13163;
...
export const captureException = function captureException(arg0, arg1) {
  const currentScope = _mod13163.getCurrentScope();
  return currentScope.captureException(arg0, _mod13185.parseEventHintOrCaptureContext(arg1));
};
...
export const startSession = function startSession(arg0) {
  ...
  if (undefined === environment) {
    environment = tmp(13173).DEFAULT_ENVIRONMENT;
  }
  const userAgent = _mod13137.GLOBAL_OBJ.navigator || {}.userAgent;
  const tmp3 = _mod13137.GLOBAL_OBJ.navigator || {};
  ...
  if (tmp10) {
    tmp(13159).updateSession(session1, { status: "exited" });
    const tmpResult2 = tmp(13159);
  }
  endSession();
...
export const withMonitor = function withMonitor(monitorSlug, arg1, arg2) {
  ...
  function finishCheckIn(status) {
    const obj = { monitorSlug, status, checkInId: captureCheckInResult, duration: _mod13150.timestampInSeconds() - closure_3 };
    const currentScope = _mod13163.getCurrentScope();
    const client = _mod13163.getClient();
    if (client) {
      if (client.captureCheckIn) {
  ...
  let currentScope = require("module_13163").getCurrentScope();
  let obj = { monitorSlug, status: "in_progress" };
  let obj2 = require("module_13163");
  let client = require("module_13163").getClient();
  if (client) {
    if (client.captureCheckIn) {
      let captureCheckInResult = client.captureCheckIn(obj, arg2, currentScope);
    } else if (tmp(13164).DEBUG_BUILD) {
      let logger2 = tmp(13136).logger;
      logger2.warn("Cannot capture check-in. Client does not support sending check-ins.");
    }
    asyncGeneratorStep = captureCheckInResult;
    closure_3 = tmp(13150).timestampInSeconds();
    const tmpResult = tmp(13150);
    return tmp(13163).withIsolationScope(() => {
      try {
    ...
    let logger = tmp(13136).logger;
    logger.warn("Cannot capture check-in. No client defined.");
  }
  let obj3 = require("module_13163");
  captureCheckInResult = require("module_13147").uuid4();
};
