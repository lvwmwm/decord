...
// Dependencies: [19, 21, 4796, 573, 5198, 1115, 5202, 5293, 5293, 1980, 2]

// Module 5197 (actions/AlertActionCreators)
...
export default {
  ...
  show(hideActionSheet) {
    ...
    if (!tmp7) {
      ...
      if (null != c6) {
        confirmText(5198).dismissAlert(c6);
        const obj5 = confirmText(5198);
      }
      ...
      const tmp31 = c5;
      const tmp32 = c4;
      obj8.variant = confirmText(5293).getAlertButtonVariant(confirmColor);
      ...
      const items = [c3(confirmText(5202).AlertActionButton, obj8), ];
      ...
      if (null != cancelText) {
        ...
        tmp30Result = tmp30(tmp29(5202).AlertActionButton, obj9);
      }
      ...
      obj7.openAlert(text, c3(confirmText(5202).AlertModal, obj3), () => {
        if (!c5) {
          if (c4 != null) {
            tmp();
          }
      ...
      const obj10 = confirmText(5293);
    } else {
      if (null != c6) {
        confirmText(5198).dismissAlert(c6);
        c6 = null;
        const obj2 = confirmText(5198);
      }
      const self = this;
      const obj13 = {
        importer() {
            return asyncRequireImpl(5293, dependencyMap.paths).then((result) => {
