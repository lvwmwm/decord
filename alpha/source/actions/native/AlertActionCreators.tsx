...
// Dependencies: [19, 21, 4800, 573, 5205, 1115, 5209, 5300, 5300, 1981, 2]

// Module 5204 (actions/AlertActionCreators)
...
export default {
  ...
  show(hideActionSheet) {
    ...
    if (!tmp7) {
      ...
      if (null != c6) {
        confirmText(5205).dismissAlert(c6);
        const obj5 = confirmText(5205);
      }
      ...
      const tmp31 = c5;
      const tmp32 = c4;
      obj8.variant = confirmText(5300).getAlertButtonVariant(confirmColor);
      ...
      const items = [c3(confirmText(5209).AlertActionButton, obj8), ];
      ...
      if (null != cancelText) {
        ...
        tmp30Result = tmp30(tmp29(5209).AlertActionButton, obj9);
      }
      ...
      obj7.openAlert(text, c3(confirmText(5209).AlertModal, obj3), () => {
        if (!c5) {
          if (c4 != null) {
            tmp();
          }
      ...
      const obj10 = confirmText(5300);
    } else {
      if (null != c6) {
        confirmText(5205).dismissAlert(c6);
        c6 = null;
        const obj2 = confirmText(5205);
      }
      const self = this;
      const obj13 = {
        importer() {
            return asyncRequireImpl(5300, dependencyMap.paths).then((result) => {
