...
// Dependencies: [19, 21, 4724, 573, 5111, 1115, 5115, 5206, 5206, 1980, 2]

// Module 5110 (actions/AlertActionCreators)
...
export default {
  ...
  show(hideActionSheet) {
    ...
    if (!tmp7) {
      if (flag) {
        merged(4724).hideActionSheet();
        const obj4 = merged(4724);
      }
      if (null != c6) {
        confirmText(5111).dismissAlert(c6);
        const obj5 = confirmText(5111);
      }
      ...
      const tmp31 = c5;
      const tmp32 = c4;
      obj8.variant = confirmText(5206).getAlertButtonVariant(confirmColor);
      ...
      const items = [c3(confirmText(5115).AlertActionButton, obj8), ];
      ...
      if (null != cancelText) {
        ...
        tmp30Result = tmp30(tmp29(5115).AlertActionButton, obj9);
      }
      ...
      obj7.openAlert(text, c3(confirmText(5115).AlertModal, obj3), () => {
        if (!c5) {
          if (c4 != null) {
            tmp();
          }
      ...
      const obj10 = confirmText(5206);
    } else {
      if (null != c6) {
        confirmText(5111).dismissAlert(c6);
        c6 = null;
        const obj2 = confirmText(5111);
      }
      const self = this;
      const obj13 = {
        importer() {
            return asyncRequireImpl(5206, dependencyMap.paths).then((result) => {
