...
// Dependencies: [19, 21, 4796, 573, 5197, 1115, 5201, 5292, 5292, 1980, 2]

// Module 5196 (actions/AlertActionCreators)
...
export default {
  ...
  show(hideActionSheet) {
    ...
    if (!tmp7) {
      if (flag) {
        merged(4796).hideActionSheet();
        const obj4 = merged(4796);
      }
      if (null != c6) {
        confirmText(5197).dismissAlert(c6);
        const obj5 = confirmText(5197);
      }
      ...
      const tmp31 = c5;
      const tmp32 = c4;
      obj8.variant = confirmText(5292).getAlertButtonVariant(confirmColor);
      ...
      const items = [c3(confirmText(5201).AlertActionButton, obj8), ];
      ...
      if (null != cancelText) {
        ...
        tmp30Result = tmp30(tmp29(5201).AlertActionButton, obj9);
      }
      ...
      obj7.openAlert(text, c3(confirmText(5201).AlertModal, obj3), () => {
        if (!c5) {
          if (c4 != null) {
            tmp();
          }
      ...
      const obj10 = confirmText(5292);
    } else {
      if (null != c6) {
        confirmText(5197).dismissAlert(c6);
        c6 = null;
        const obj2 = confirmText(5197);
      }
      const self = this;
      const obj13 = {
        importer() {
            return asyncRequireImpl(5292, dependencyMap.paths).then((result) => {
