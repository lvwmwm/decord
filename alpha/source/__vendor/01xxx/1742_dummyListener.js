...
// Function ID: 1743
// Name: dummyListener
// Dependencies: [41, 42, 1735, 1710, 1734, 1743, 1687, 1641]

// Module 1742 (dummyListener)
      ...
      if ("style" === key10014) {
        ...
        obj[key10014] = flattenArrayResult.map((viewDescriptors) => {
          if (viewDescriptors) {
            if (viewDescriptors.viewDescriptors) {
              if (iter._isFirstRender) {
                ...
                const merged1 = Object.assign(PropsFilter(1710).initialUpdaterRun(viewDescriptors.initial.updater));
                const result = _initialPropsMap.set(viewDescriptors, obj3);
                const obj4 = PropsFilter(1710);
              }
          ...
          if (obj.hasInlineStyles(viewDescriptors)) {
            inlineStyle = PropsFilter(1734).getInlineStyle(viewDescriptors, iter._isFirstRender);
            const obj2 = PropsFilter(1734);
          }
          return inlineStyle;
        });
        continue;
