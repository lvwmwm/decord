      ...
      item = Children.forEach(children, (type) => {
        if (noop.isValidElement(type)) {
          ...
          if (type === Background.Background) {
            closure_13 = type;
          } else if (type === tmp(1859).Content) {
            closure_11 = type;
          } else if (type === tmp(1859).Prev) {
            closure_9 = type;
          } else if (type === tmp(1859).Next) {
            closure_10 = type;
          } else if (type === tmp(1859).Done) {
            closure_12 = type;
          }
        }
      });
...
let rect = { position: "absolute", left: 0, right: 0, bottom: 0, height: fn(1856).KEYBOARD_TOOLBAR_HEIGHT };
obj.sticky = rect;
const size = { position: "absolute", bottom: 0, alignItems: "center", width: "100%", flexDirection: "row", height: fn(1856).KEYBOARD_TOOLBAR_HEIGHT };
obj.toolbar = size;
...
KeyboardToolbar.Background = fn(1859).Background;
KeyboardToolbar.Content = fn(1859).Content;
KeyboardToolbar.Prev = fn(1859).Prev;
KeyboardToolbar.Next = fn(1859).Next;
KeyboardToolbar.Done = fn(1859).Done;
KeyboardToolbar.Group = fn(1628).RCTKeyboardToolbarGroupView;

export default KeyboardToolbar;
export const DefaultKeyboardToolbarTheme = fn(1855).colors;
