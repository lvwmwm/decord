          ...
          if (StringResult in ObjectResult) {
            ...
            if (null == tmp4) {
              ...
              if (typeof tmp4 === "string") {
                ...
                if (isMatch1) {
                  let expVariantDupes = tmp5(1925).expVariantDupes;
                  let isMatch = expVariantDupes.test(str);
                  let tmp9 = !isMatch;
                  if (!isMatch) {
                    let expSingletonDupes = tmp5(1925).expSingletonDupes;
...
fn(obj, "__addLocaleData", {
  value(locale) {
    ...
    if (isMatch1) {
      const expVariantDupes = tmp(1925).expVariantDupes;
      const isMatch = expVariantDupes.test(locale);
      let tmp5 = !isMatch;
      if (!isMatch) {
        const expSingletonDupes = tmp(1925).expSingletonDupes;
