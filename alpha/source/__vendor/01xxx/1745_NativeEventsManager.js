...
// Dependencies: [41, 42, 90, 91, 1746, 1735, 1743]

// Module 1745 (NativeEventsManager)
...
const entry = {
  ...
  value: function attachEvents() {
    ...
    for (const key10010 in props) {
      ...
      let obj = NativeEventsManager(1735);
      let hasItem = obj.has("workletEventHandler", tmp6);
      if (hasItem) {
        hasItem = tmp6.workletEventHandler instanceof tmp7(1743).WorkletEventHandler;
      }
      if (!hasItem) {
        continue;
      } else {
    ...
    value: function updateEvents(current) {
      ...
      if (_classPrivateFieldBase(this, closure_6)[closure_6] !== eventViewTag) {
        for (const key10071 in arg0) {
          ...
          let obj4 = NativeEventsManager(1735);
          let hasItem = obj4.has("workletEventHandler", tmp37);
          if (hasItem) {
            hasItem = tmp37.workletEventHandler instanceof tmp38(1743).WorkletEventHandler;
          }
          if (!hasItem) {
            continue;
          } else {
        ...
        for (const key10010 in arg0) {
          ...
          let obj2 = NativeEventsManager(1735);
          let hasItem1 = obj2.has("workletEventHandler", tmp29);
          if (hasItem1) {
            hasItem1 = tmp29.workletEventHandler instanceof tmp30(1743).WorkletEventHandler;
          }
          if (!hasItem1) {
            continue;
          } else {
            ...
            if (tmp5) {
              let tmp30Result = tmp30(1735);
              let hasItem2 = tmp30Result.has("workletEventHandler", tmp5);
              if (hasItem2) {
                hasItem2 = tmp5.workletEventHandler instanceof tmp30(1743).WorkletEventHandler;
              }
              if (hasItem2) {
        ...
        for (const key10054 in props) {
          ...
          let obj3 = NativeEventsManager(1735);
          let hasItem3 = obj3.has("workletEventHandler", tmp33);
          if (hasItem3) {
            hasItem3 = tmp33.workletEventHandler instanceof tmp34(1743).WorkletEventHandler;
          }
          if (!hasItem3) {
            continue;
          } else {
    ...
    value: function getEventViewTag(arg0) {
      ...
      if (getScrollableNode) {
        ...
        if (typeof scrollableNode !== "number") {
          let num4 = NativeEventsManager(1746).findNodeHandle(scrollableNode);
          if (num4 == null) {
            num4 = -1;
          }
          tmp16 = num4;
          const obj4 = NativeEventsManager(1746);
        }
        return tmp16;
      } else {
        ...
        if (setNativeProps) {
          let num3 = NativeEventsManager(1746).findNodeHandle(tmp(self, tmp2)[tmp2]);
          if (num3 == null) {
            num3 = -1;
          }
          let componentViewTag = num3;
          const obj3 = NativeEventsManager(1746);
        } else if (flag) {
          let __nativeTag;
          if (_componentRef != null) {
          ...
          if (!__nativeTag) {
            ...
            if (!_nativeTag) {
              let num = NativeEventsManager(1746).findNodeHandle(_componentRef);
              if (num == null) {
                num = -1;
              }
              const obj2 = NativeEventsManager(1746);
            }
          }
