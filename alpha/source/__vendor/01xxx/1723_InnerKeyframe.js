...
// Function ID: 1724
// Name: InnerKeyframe
// Dependencies: [41, 42, 1663, 1710, 1690, 1649, 1678]

// Module 1723 (InnerKeyframe)
  ...
  constructor(arg0) {
    ...
    this.build = () => {
      ...
      if (!self.parsedAnimation) {
            ...
            if (0 !== first.length) {
              if (1 === first.length) {
                ...
                if (first[0].easing) {
                  ...
                  linear = self(1690).Easing.linear;
                }
                obj2.easing = linear;
                obj.withTiming(first[0].value, obj2);
              } else {
                const tmp3 = self(1710);
        ...
        let obj = { keyframes, delayFunction, delay: delayV, withTiming: InnerKeyframe(1710).withTiming, Easing: InnerKeyframe(1690).Easing, withSequence: InnerKeyframe(1710).withSequence, initialValues, makeKeyframeKey, callback: callbackV };
        fn.__closure = obj;
            ...
            item = keys.forEach((item) => {
              if ("transform" === item) {
                ...
                if (item in obj) {
                  ...
                  let reanimatedError = new InnerKeyframe(1649).ReanimatedError("Keyframe can contain only that set of properties that were provide with initial values (keyframe 0 or 'from')");
                  throw reanimatedError;
                }
                tmp3 = obj;
              } else {
    ...
    value: function getDelayFunction() {
      ...
      if (this.delayV) {
        const fn2 = function t(arg0, arg1) {
          return InnerKeyframe(1710).withDelay(arg0, arg1, reduceMotionV);
        };
        const obj2 = { withDelay: reduceMotionV(1710).withDelay, reduceMotion: reduceMotionV };
        fn2.__closure = obj2;
        ...
        fn = function n(arg0, arg1) {
          arg1.reduceMotion = InnerKeyframe(1678).getReduceMotionFromConfig(reduceMotionV);
          return arg1;
        };
        const obj = { getReduceMotionFromConfig: reduceMotionV(1678).getReduceMotionFromConfig, reduceMotion: reduceMotionV };
        fn.__closure = obj;
