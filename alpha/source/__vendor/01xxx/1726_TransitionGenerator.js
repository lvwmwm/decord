...
// Dependencies: [109, 32, 1696, 1725, 1694, 1727, 1728, 1729, 1730, 1731, 1732]
// Exports: TransitionGenerator, createAnimationWithInitialValues, createCustomKeyFrameAnimation

// Module 1726 (TransitionGenerator)
import TransitionType from "TransitionType" /* 1694 */;
import convertAnimationObjectToKeyframes from "convertAnimationObjectToKeyframes" /* 1696 */;
...
export const TransitionGenerator = function TransitionGenerator(ENTRY_EXIT, easingY) {
  ...
  if (TransitionType.TransitionType.LINEAR === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1727).LinearTransition(`REA${tmp}`, easingY);
    const tmp3Result = tmp3(1727);
  } else if (tmp3(1694).TransitionType.SEQUENCED === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1728).SequencedTransition(`REA${tmp}`, easingY);
    const tmp3Result10 = tmp3(1728);
  } else if (tmp3(1694).TransitionType.FADING === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1729).FadingTransition(`REA${tmp}`, easingY);
    const tmp3Result11 = tmp3(1729);
  } else if (tmp3(1694).TransitionType.JUMPING === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1730).JumpingTransition(`REA${tmp}`, easingY);
    const tmp3Result12 = tmp3(1730);
  } else if (tmp3(1694).TransitionType.CURVED === ENTRY_EXIT) {
    closure_6 = tmp7 + 1;
    const text1 = `REA${tmp7}`;
    const tmp3Result13 = tmp3(1731);
    ({ firstKeyframeObj, secondKeyframeObj } = tmp3(1731).CurvedTransition(`REA${tmp}`, `REA${+closure_6}`, easingY));
    const CurvedTransitionResult = tmp3(1731).CurvedTransition(`REA${tmp}`, `REA${+closure_6}`, easingY);
    const result = tmp3(1696).convertAnimationObjectToKeyframes(secondKeyframeObj);
    const tmp3Result14 = tmp3(1696);
    tmp3(1725).insertWebAnimation(`REA${+closure_6}`, result);
    const dummyTransitionKeyframeName = text1;
    const tmp3Result15 = tmp3(1725);
  } else if (tmp3(1694).TransitionType.ENTRY_EXIT === ENTRY_EXIT) {
    firstKeyframeObj = tmp3(1732).EntryExitTransition(`REA${tmp}`, easingY);
    const tmp3Result16 = tmp3(1732);
  }
