...
// Function ID: 7248
// Name: JSFPSMonitor
// Dependencies: [7194, 7195, 7182, 7248]

// Module 7247 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(7182).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(7248).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(7248);
      obj.maxFPS = JSFPSMonitor(7248).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(7248);
      obj.averageFPS = JSFPSMonitor(7248).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef7195(JSFPSMonitor, items);
