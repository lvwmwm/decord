...
import append from "append" /* 7910 */;
import peg$SyntaxError from "peg$SyntaxError" /* 7911 */;
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        const parsed = tmp3(7911).parse(arr);
        const tmp3Result5 = tmp3(7910);
        tmp3Result5.append(parsed[0], parsed[3], parsed[1], parsed[4], parsed[2], parsed[5]);
        const tmp3Result4 = tmp3(7911);
      } catch (tmp14) {
...
export const extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "string") {
    transform = _mod7912.parse(transform.transform);
  } else {
    transform = transform.transform;
  }
  return transform;
