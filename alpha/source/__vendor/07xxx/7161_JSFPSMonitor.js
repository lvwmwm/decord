...
// Function ID: 7162
// Name: JSFPSMonitor
// Dependencies: [7108, 7109, 7096, 7162]

// Module 7161 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(7096).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(7162).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(7162);
      obj.maxFPS = JSFPSMonitor(7162).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(7162);
      obj.averageFPS = JSFPSMonitor(7162).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef7109(JSFPSMonitor, items);
