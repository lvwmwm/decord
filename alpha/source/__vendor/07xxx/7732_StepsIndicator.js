...
import StepNumber from "StepNumber" /* 7734 */;
...
export const StepsIndicator = function StepsIndicator(options) {
  ...
  const memo = currentValue.useMemo(() => {
    if (options.length > 9) {
      let fontSize = _mod7730.constants.STEP_NUMBER_TEXT_FONT_SMALL;
    } else {
      fontSize = _mod7730.constants.STEP_NUMBER_TEXT_FONT_BIG;
    }
    return { fontSize };
  }, items);
  ...
  const memo1 = currentValue.useMemo(() => {
    if ("web" === get_ActivityIndicator.Platform.OS) {
      let stepsIndicator = _mod7731.styles.stepsIndicator;
      let tmp6 = require;
    } else {
      const _Object = Object;
      const obj = { marginHorizontal: sliderWidth * _mod7730.constants.MARGIN_HORIZONTAL_PADDING };
      stepsIndicator = Object.assign({}, _mod7731.styles.stepsIndicator, obj);
      tmp6 = require;
    }
    ...
    if ("web" === get_ActivityIndicator.Platform.OS) {
      ...
      let stepIndicatorElement = Object.assign({}, tmp6(7731).styles.stepIndicatorElement, obj3);
    } else {
