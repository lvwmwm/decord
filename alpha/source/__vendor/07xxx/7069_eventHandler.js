...
// Function ID: 7070
// Name: eventHandler
// Dependencies: [7051, 6989, 7004, 6991, 7053]
// Exports: eventHandler

// Module 7069 (eventHandler)
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6989.State.UNDETERMINED) {
    if (state === tmp(6989).State.BEGAN) {
      tmp(7051).runCallback(tmp(7004).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(7051);
    }
  }
  if (oldState === _mod6989.State.BEGAN) {
    if (state === tmp(6989).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(7051).runCallback(tmp(7004).CALLBACK_TYPE.START, arg1, result);
      const tmpResult4 = tmp(7051);
    }
  }
  if (oldState !== state) {
    const obj2 = {};
    const merged = Object.assign(result);
    obj2.canceled = state === tmp(6989).State.FAILED || state === tmp(6989).State.CANCELLED;
    if (oldState === tmp(6989).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(7051).runCallback(tmp(7004).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(7051);
    }
    const tmp7 = state === tmp(6989).State.FAILED || state === tmp(6989).State.CANCELLED;
    tmp(7051).runCallback(tmp(7004).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(7051);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(7051).flattenAndFilterEvent, State: fn(6989).State, runCallback: fn(7051).runCallback, CALLBACK_TYPE: fn(7004).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod7004.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(7051).flattenAndFilterEvent, State: fn(6989).State, runCallback: fn(7051).runCallback, CALLBACK_TYPE: fn(7004).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(7051).flattenAndFilterEvent, runCallback: fn(7051).runCallback, CALLBACK_TYPE: fn(7004).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(7051);
    tmpResult.runCallback(tmp(7051).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(7051);
  }
}
let obj2 = { flattenAndFilterEvent: fn(7051).flattenAndFilterEvent, runCallback: fn(7051).runCallback, CALLBACK_TYPE: fn(7004).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6991).TouchEventType, runCallback: fn(7051).runCallback, touchEventTypeToCallbackType: fn(7051).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6991).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(7051);
            tmpResult7.runCallback(tmp(7051).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(7051);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(7051).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(7051);
          tmp(7051).runCallback(tmp(7004).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(7051);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
...
const obj3 = { TouchEventType: fn(6991).TouchEventType, runCallback: fn(7051).runCallback, touchEventTypeToCallbackType: fn(7051).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(7051).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(7051).isEventForHandlerWithTag, isStateChangeEvent: fn(7053).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(7053).isTouchEvent, handleTouchEvent, handleUpdateEvent };
