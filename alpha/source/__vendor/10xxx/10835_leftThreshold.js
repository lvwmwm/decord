...
// Function ID: 10836
// Name: leftThreshold
// Dependencies: [109, 19, 17, 21, 1638, 10836, 6154, 6086]
// Exports: default

// Module 10835 (leftThreshold)
...
function _default(hitSlop) {
  ...
  let fn = function h() {
    ...
    if (sharedValue5.value > 0) {
      ...
      num7 = tmp8(1638).interpolate(iter2.value, items2, [0, 0, 1]);
      const tmp8Result = tmp8(1638);
    }
    sharedValue7.value = num7;
    let num8 = 0;
    if (sharedValue6.value > 0) {
      const items3 = [-iter3.value, 0, 1];
      num8 = tmp8(1638).interpolate(iter2.value, items3, [1, 0, 0]);
      const tmp8Result2 = tmp8(1638);
    }
  ...
  let fn2 = function p(arg0, arg1) {
    ...
    if (!tmp2) {
      ...
      if (tmp12) {
        if (arg0 > 0) {
          let RIGHT = _mod10836.SwipeDirection.LEFT;
        } else {
          RIGHT = _mod10836.SwipeDirection.RIGHT;
        }
      ...
      if (arg1 > 0) {
        let LEFT = _mod10836.SwipeDirection.RIGHT;
      } else {
        LEFT = _mod10836.SwipeDirection.LEFT;
      }
