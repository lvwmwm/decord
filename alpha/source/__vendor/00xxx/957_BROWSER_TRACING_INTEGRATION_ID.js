...
export const startBrowserTracingPageLoadSpan = function startBrowserTracingPageLoadSpan(f110547, tmp2Result, arg2) {
  f110547.emit("startPageLoadSpan", tmp2Result, arg2);
  const currentScope = _mod682.getCurrentScope();
  currentScope.setTransactionName(tmp2Result.name);
  if (f110547[_sentry_idleSpan]) {
    f110547.emit("afterStartPageLoadSpan", tmp3);
  }
  return f110547[_sentry_idleSpan];
};
