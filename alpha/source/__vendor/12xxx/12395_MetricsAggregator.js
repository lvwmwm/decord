...
// Dependencies: [32, 41, 42, 12392, 12309, 12396, 12397, 12300, 12398]

// Module 12395 (MetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(12309).timestampInSeconds();
      const obj2 = MetricsAggregator(12309);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(12396).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(12396);
    const sanitizeTagsResult = MetricsAggregator(12396).sanitizeTags(obj);
    const obj4 = MetricsAggregator(12396);
    const sanitizeUnitResult = MetricsAggregator(12396).sanitizeUnit(str);
    const obj5 = MetricsAggregator(12396);
    const bucketKey = MetricsAggregator(12396).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(12397).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(12396);
    const result1 = MetricsAggregator(12300).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(12392).MAX_WEIGHT) {
      self.flush();
    }
  }
};
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(12309).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(12392).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(12398).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(12398);
      }
    }
  }
];
