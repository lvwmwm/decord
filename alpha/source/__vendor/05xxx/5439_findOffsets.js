      ...
      if (1835365473 === uint321) {
        ...
        if (uint8 >= 2) {
          ...
          if (obj17.itemType === c4) {
            obj17.contentType = tmp13(5430).getNullTerminatedStringFromDataView(getUint32, sum13);
            const sum14 = sum13 + (obj17.contentType.length + 1);
            parseItemLocationBoxResult = obj17;
            if (sum + length > sum14) {
              obj17.contentEncoding = tmp13(5430).getNullTerminatedStringFromDataView(getUint32, sum14);
              const sum15 = obj17.contentEncoding.length + 1;
              parseItemLocationBoxResult = obj17;
              const tmp13Result3 = tmp13(5430);
            }
            const tmp13Result = tmp13(5430);
          } else {
            parseItemLocationBoxResult = obj17;
            if (obj17.itemType === c5) {
              obj17.itemUri = tmp13(5430).getNullTerminatedStringFromDataView(getUint32, sum13);
              const sum16 = obj17.itemUri.length + 1;
              parseItemLocationBoxResult = obj17;
              const tmp13Result4 = tmp13(5430);
            }
          }
        }
      } else {
