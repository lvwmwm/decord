...
// Function ID: 5946
// Name: Color
// Dependencies: [32, 5946, 5950]

// Module 5945 (Color)
import _mod5946 from "module_5946" /* 5946 */;
...
let obj = {};
let keys1 = Object.keys(_mod5946);
let iter = keys1[Symbol.iterator]();
const nextResult = iter.next();
while (iter !== undefined) {
  let items = [];
  let arraySpreadResult = HermesBuiltin.arraySpread(_mod5946[nextResult].labels, 0);
  class Color {
    constructor(arg0, arg1) {
      self = this;
      str = require;
...
const point = {
  ...
  object() {
    const self = this;
    obj = {};
    const channels = _mod5946[this.model].channels;
    for (let num = 0; num < channels; num = num + 1) {
      obj[_mod5946[this.model].labels[num]] = self.color[num];
    }
    if (1 !== self.valpha) {
  ...
  keyword(color) {
    if (undefined !== color) {
      ...
      const self = this;
      keywordResult = _mod5946[this.model].keyword(this.color);
    }
...
function _loop(item10136) {
  ...
  if (closure_3.includes(item10136)) {
    ...
    channels = require("module_5946")[item10136].channels;
    Color.prototype[item10136] = function() {
      ...
      if (this.model === closure_0) {
        ...
        const rawResult = _mod5946[self.model][tmp].raw(self.color);
        const _Array = Array;
        let tmp2 = rawResult;
