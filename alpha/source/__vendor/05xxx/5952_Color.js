...
// Function ID: 5953
// Name: Color
// Dependencies: [32, 5953, 5957]

// Module 5952 (Color)
import _mod5953 from "module_5953" /* 5953 */;
import _mod5957 from "module_5957" /* 5957 */;
...
let obj = {};
let keys1 = Object.keys(_mod5953);
let iter = keys1[Symbol.iterator]();
const nextResult = iter.next();
while (iter !== undefined) {
  let items = [];
  let arraySpreadResult = HermesBuiltin.arraySpread(_mod5953[nextResult].labels, 0);
  class Color {
    constructor(arg0, arg1) {
      self = this;
      str = require;
...
const point = {
  ...
  object() {
    const self = this;
    obj = {};
    const channels = _mod5953[this.model].channels;
    for (let num = 0; num < channels; num = num + 1) {
      obj[_mod5953[this.model].labels[num]] = self.color[num];
    }
    if (1 !== self.valpha) {
  ...
  keyword(color) {
    if (undefined !== color) {
      ...
      const self = this;
      keywordResult = _mod5953[this.model].keyword(this.color);
    }
...
function _loop(item10136) {
  ...
  if (closure_3.includes(item10136)) {
    ...
    channels = require("module_5953")[item10136].channels;
    Color.prototype[item10136] = function() {
      ...
      if (this.model === closure_0) {
        ...
        const rawResult = _mod5953[self.model][tmp].raw(self.color);
        const _Array = Array;
        let tmp2 = rawResult;
