...
// Dependencies: [41, 42, 5490, 5493, 5499, 5500, 5501, 5502, 5503]

// Module 5492 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5499 */;
import ImageTypes from "ImageTypes" /* 5500 */;
import VideoTypes from "VideoTypes" /* 5501 */;
import CompressedTypes from "CompressedTypes" /* 5502 */;
import OtherTypes from "OtherTypes" /* 5503 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5490).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5490).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5493).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5493).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5493).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5490).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5493).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5490).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
