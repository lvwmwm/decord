...
// Function ID: 5525
// Name: load
// Dependencies: [5525, 5526, 5527, 5528, 5529, 5542, 5544, 5545, 5557, 5561, 5565, 5567, 5569, 5570, 5571, 5572, 5573, 5574, 5575, 5576, 5577, 5578, 5551]
// Exports: load
...
function load(response, response) {
  ...
  if (typeof response === "string") {
    ...
    if (obj2.test(response)) {
      ...
      if (obj3.test(response)) {
        let resolved = Promise.resolve(response(5526).dataUriToBuffer(response));
        const obj5 = response(5526);
      } else {
        let obj7 = then;
        if (then === undefined) {
          obj7 = {};
...
function loadView(byteLength, response) {
  ...
  function addPngTextTags(readTags) {
    if (flag) {
      ...
      if (closure_1.png) {
        objectAssignResult3 = _mod5526.objectAssign({}, closure_1.png, readTags);
      }
      closure_1.png = objectAssignResult3;
      let objectAssignResult4 = readTags;
      if (closure_1.pngText) {
        objectAssignResult4 = _mod5526.objectAssign({}, closure_1.png, readTags);
      }
  ...
  let obj2 = require("module_5528");
  ({ fileType, fileDataOffset, jfifDataOffset, tiffHeaderOffset, iptcDataOffset, xmpChunks, iccChunks, mpfDataOffset, pngHeaderOffset, pngTextChunks, pngChunkOffsets, vp8xChunkOffset, gifHeaderOffset } = require("module_5528").parseAppMarkers(byteLength, flag2));
  let flag4 = false;
  if (require("module_5529").USE_JPEG) {
    flag4 = false;
        ...
        if (readResult11 instanceof Promise) {
          items1.push(readResult11.then(function addIccTags(icc) {
            if (flag) {
              ...
              closure_1 = _mod5526.objectAssign({}, closure_1, icc);
            }
          }));
          flag9 = true;
        } else if (flag) {
  ...
  const parseAppMarkersResult = require("module_5528").parseAppMarkers(byteLength, flag2);
...
export default { load, loadView, errors: _modDef5525 };
export const errors = _modDef5525;
export { load };
export { loadView };
