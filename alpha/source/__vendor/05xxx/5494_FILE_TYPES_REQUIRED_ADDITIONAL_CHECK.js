...
// Dependencies: [41, 42, 5492, 5495, 5501, 5502, 5503, 5504, 5505]

// Module 5494 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5501 */;
import ImageTypes from "ImageTypes" /* 5502 */;
import VideoTypes from "VideoTypes" /* 5503 */;
import CompressedTypes from "CompressedTypes" /* 5504 */;
import OtherTypes from "OtherTypes" /* 5505 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5492).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5492).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5495).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5495).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5495).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5492).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5495).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5492).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
