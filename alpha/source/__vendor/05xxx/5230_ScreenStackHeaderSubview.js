...
// Dependencies: [109, 19, 17, 21, 5231, 5228, 5232, 5233, 5211, 5234]
...
// Module 5230 (ScreenStackHeaderSubview)
...
const forwardRefResult = noop.forwardRef((hidden, ref) => {
  ...
  const edgeInsetApplication = _mod5231.useEdgeInsetApplication(!hidden.hidden, flag, flag2, flag3, flag4);
  ...
  if (headerLeftBarButtonItems) {
    if (tmp(5228).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result = tmp(5232).prepareHeaderBarButtonItems(headerLeftBarButtonItems, "left");
  ...
  if (headerRightBarButtonItems) {
    if (tmp(5228).isHeaderBarButtonsAvailableForCurrentPlatform) {
      result1 = tmp(5232).prepareHeaderBarButtonItems(headerRightBarButtonItems, "right");
      const tmpResult2 = tmp(5232);
    }
  }
  let isHeaderBarButtonsAvailableForCurrentPlatform = tmp(5228).isHeaderBarButtonsAvailableForCurrentPlatform;
...
export const ScreenStackHeaderBackButtonImage = (arg0) => {
  ...
  return jsx(_modDef5234, { type: "back", style: closure_9.headerSubview, synchronousShadowStateUpdatesEnabled: get_synchronousScreenUpdatesEnabledDefault.experiment.synchronousHeaderSubviewUpdatesEnabled, children: null });
};
