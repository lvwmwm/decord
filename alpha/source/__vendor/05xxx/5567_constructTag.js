...
// Function ID: 5568
// Name: constructTag
// Dependencies: [5523, 5539, 5551, 5520, 5527, 5553]
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (type !== PNG_CHUNK_TYPE_SIZE.TYPE_TEXT) {
    ...
    if (0 !== items2.length) {
      ...
      combined = "" + stringValueFromArray + " (" + tmp4(5520).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(5520);
    }
  }
                ...
                try {
                  if (_modDef5523.USE_EXIF) {
                    if (isExifGroupTag(name, value)) {
                      const obj2 = { __exif: tmp2(5539).read(decodeRawData(value), c10, closure_0).tags };
                      return obj2;
                    }
                  }
                  if (_modDef5523.USE_IPTC) {
                    if (isIptcGroupTag(name, value)) {
                      const obj3 = { __iptc: tmp2(5551).read(decodeRawData(value), 0, closure_0) };
                      return obj3;
                    }
                  }
                  if (name) {
