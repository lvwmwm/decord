...
function readTag(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  ...
  if (undefined !== _modDef5533.typeSizes[shortAt1]) {
    if ((function tagValueFitsInOffsetSlot(shortAt1, longAt) {
      ...
      const longAt1 = tmp4(5533).getLongAt(byteLength, sum + sum1, byteOrder);
      str = "<faulty value>";
      tmp21 = longAt1;
      ...
      const tmp4Result = tmp4(5533);
    }
    let tmp34 = str;
    if (shortAt1 === tmp4(5533).tagTypes.ASCII) {
    ...
    if (undefined !== tmp4(5537)[IFD_TYPE_0TH][shortAt]) {
      if (undefined !== tmp4(5537)[IFD_TYPE_0TH][shortAt].name) {
        if (undefined !== tmp4(5537)[IFD_TYPE_0TH][shortAt].description) {
          try {
            descriptionResult = tmp4(5537)[IFD_TYPE_0TH][shortAt].description(tmp34);
            combined = tmp39;
            const obj8 = tmp4(5537)[IFD_TYPE_0TH][shortAt];
          } catch (err) {
      ...
      if (shortAt1 !== tmp4(5533).tagTypes.RATIONAL) {
        if (shortAt1 !== tmp4(5533).tagTypes.SRATIONAL) {
          combined = tmp4(5537)[IFD_TYPE_0TH][shortAt];
...
function readIfd(byteLength, IFD_TYPE_0TH, sum, sum, byteOrder, arg5) {
  ...
  if (_modDef5519.USE_THUMBNAIL) {
    if (tmp5 < byteLength.byteLength - tmp23Result.getTypeSize("LONG")) {
      const longAt = tmp23(5533).getLongAt(byteLength, tmp5, byteOrder);
      let tmp26 = 0 !== longAt;
      if (tmp26) {
      ...
      const tmp23Result2 = tmp23(5533);
    }
    tmp23Result = tmp23(5533);
  }
  return obj3;
}
