...
// Function ID: 5519
// Name: load
// Dependencies: [5519, 5520, 5521, 5522, 5523, 5536, 5538, 5539, 5551, 5555, 5559, 5561, 5563, 5564, 5565, 5566, 5567, 5568, 5569, 5570, 5571, 5572, 5545]
// Exports: load
...
function load(response, response) {
  ...
  if (typeof response === "string") {
    ...
    if (obj2.test(response)) {
      ...
      if (obj3.test(response)) {
        let resolved = Promise.resolve(response(5520).dataUriToBuffer(response));
        const obj5 = response(5520);
      } else {
        let obj7 = then;
        if (then === undefined) {
          obj7 = {};
...
function loadView(byteLength, response) {
  ...
  function addPngTextTags(readTags) {
    if (flag) {
      ...
      if (closure_1.png) {
        objectAssignResult3 = _mod5520.objectAssign({}, closure_1.png, readTags);
      }
      closure_1.png = objectAssignResult3;
      let objectAssignResult4 = readTags;
      if (closure_1.pngText) {
        objectAssignResult4 = _mod5520.objectAssign({}, closure_1.png, readTags);
      }
  ...
  let obj2 = require("module_5522");
  ({ fileType, fileDataOffset, jfifDataOffset, tiffHeaderOffset, iptcDataOffset, xmpChunks, iccChunks, mpfDataOffset, pngHeaderOffset, pngTextChunks, pngChunkOffsets, vp8xChunkOffset, gifHeaderOffset } = require("module_5522").parseAppMarkers(byteLength, flag2));
  let flag4 = false;
  if (require("module_5523").USE_JPEG) {
    flag4 = false;
        ...
        if (readResult11 instanceof Promise) {
          items1.push(readResult11.then(function addIccTags(icc) {
            if (flag) {
              ...
              closure_1 = _mod5520.objectAssign({}, closure_1, icc);
            }
          }));
          flag9 = true;
        } else if (flag) {
  ...
  const parseAppMarkersResult = require("module_5522").parseAppMarkers(byteLength, flag2);
...
export default { load, loadView, errors: _modDef5519 };
export const errors = _modDef5519;
export { load };
export { loadView };
