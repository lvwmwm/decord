...
// Function ID: 5564
// Name: constructTag
// Dependencies: [5519, 5535, 5547, 5516, 5523, 5549]
...
function constructTag(decompressResult, type, items2, items1) {
  ...
  if (type !== PNG_CHUNK_TYPE_SIZE.TYPE_TEXT) {
    ...
    if (0 !== items2.length) {
      ...
      combined = "" + stringValueFromArray + " (" + tmp4(5516).getStringValueFromArray(items2) + ")";
      const tmp4Result = tmp4(5516);
    }
  }
                ...
                try {
                  if (_modDef5519.USE_EXIF) {
                    if (isExifGroupTag(name, value)) {
                      const obj2 = { __exif: tmp2(5535).read(decodeRawData(value), c10, closure_0).tags };
                      return obj2;
                    }
                  }
                  if (_modDef5519.USE_IPTC) {
                    if (isIptcGroupTag(name, value)) {
                      const obj3 = { __iptc: tmp2(5547).read(decodeRawData(value), 0, closure_0) };
                      return obj3;
                    }
                  }
                  if (name) {
