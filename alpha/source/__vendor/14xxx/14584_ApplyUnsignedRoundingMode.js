  ...
  if (eq.eq(timesResult)) {
    ...
    if ("zero" === formattedString) {
      ...
      if (minusResult.lessThan(minusResult1)) {
        ...
        tmp2(14572).invariant(minusResult.eq(minusResult1), "d1 should be equal to d2");
        if ("half-zero" === formattedString) {
          return timesResult;
        } else if ("half-infinity" === formattedString) {
          return roundedNumber;
        } else {
          tmp2(14572).invariant("half-even" === formattedString, "unsignedRoundingMode should be half-even");
