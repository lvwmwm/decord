...
// Dependencies: [377, 41, 42, 9888, 9886, 9890]

// Module 9887 (ReferenceWithTimezone)
  ...
  entry1,
  {
    ...
    value: function fromInput(instant, timezones) {
      if (instant instanceof Date) {
        ...
        const toTimezoneOffsetResult = ParsingComponents(9888).toTimezoneOffset(timezone, instant, timezones);
        const obj = Object.create(ParsingComponents.prototype);
        _classCallCheck(obj, ParsingComponents);
        if (null == instant) {
          const _Date2 = Date;
...
const items2 = [
  ...
  },
  {
    ...
    value: function addDurationAsImplied(EmptyDuration) {
      ...
      const addDurationResult = ParsingComponents(9886).addDuration(result, EmptyDuration);
...
const entry3 = {
  ...
  value: function createRelativeFromReference(reference, reverseDurationResult) {
    ...
    if (reverseDurationResult === undefined) {
      EmptyDuration = ParsingComponents(9886).EmptyDuration;
    }
    const addDurationResult = ParsingComponents(9886).addDuration(reference.getDateWithAdjustedTimezone(), EmptyDuration);
    const obj = Object.create(ParsingComponents.prototype);
    _classCallCheck(obj, ParsingComponents);
    obj._tags = new Set();
    obj.reference = reference;
      ...
      if (!("minute" in EmptyDuration)) {
        if (!("second" in EmptyDuration)) {
          if (!("millisecond" in EmptyDuration)) {
            ParsingComponents(9890).implySimilarTime(obj, addDurationResult);
    ...
    ParsingComponents(9890).assignSimilarTime(obj, addDurationResult);
    ParsingComponents(9890).assignSimilarDate(obj, addDurationResult);
