...
// Dependencies: [13201, 13152, 13190, 13151, 13154, 13173, 13145]

// Module 13235 (extraErrorDataIntegration)
        ...
        if (obj2.isError(obj.originalException)) {
          const tmp7 = (function _extractErrorData(originalException, arg1, maxValueLength) {
            try {
              ...
              if (num(13173).DEBUG_BUILD) {
                const logger = num(13145).logger;
          ...
          if (tmp7) {
            ...
            const normalizer = tmp4(13190);
            const normalizeResult = normalizer.normalize(tmp7, num);
            if (tmp4Result.isPlainObject(normalizeResult)) {
              const result = tmp4(13151).addNonEnumerableProperty(normalizeResult, "__sentry_skip_normalization__", true);
              obj3[tmp6] = normalizeResult;
              const tmp4Result2 = tmp4(13151);
            }
            let obj4 = {};
