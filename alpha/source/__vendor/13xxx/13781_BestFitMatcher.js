...
export const BestFitMatcher = function BestFitMatcher(arg0, arr, fn) {
  ...
  const reduced = arr.reduce((acc, item) => {
    const replaced = item.replace(_mod13777.UNICODE_EXTENSION_SEQUENCE_REGEX, "");
    items.push(replaced);
    acc[replaced] = item;
    return acc;
  }, {});
  const findBestMatchResult = items(13777).findBestMatch(items, arg0);
  let prop;
  if (tmp3) {
