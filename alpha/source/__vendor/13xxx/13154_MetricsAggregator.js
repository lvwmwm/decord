...
// Dependencies: [32, 41, 42, 13151, 13068, 13155, 13156, 13059, 13157]

// Module 13154 (MetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(13068).timestampInSeconds();
      const obj2 = MetricsAggregator(13068);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(13155).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(13155);
    const sanitizeTagsResult = MetricsAggregator(13155).sanitizeTags(obj);
    const obj4 = MetricsAggregator(13155);
    const sanitizeUnitResult = MetricsAggregator(13155).sanitizeUnit(str);
    const obj5 = MetricsAggregator(13155);
    const bucketKey = MetricsAggregator(13155).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(13156).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(13155);
    const result1 = MetricsAggregator(13059).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(13151).MAX_WEIGHT) {
      self.flush();
    }
  }
};
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(13068).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(13151).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(13157).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(13157);
      }
    }
  }
];
