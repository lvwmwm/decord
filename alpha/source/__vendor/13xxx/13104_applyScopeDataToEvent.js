...
export const applyScopeDataToEvent = function applyScopeDataToEvent(extra, arg1) {
  ...
  const dropUndefinedKeysResult = _mod13060.dropUndefinedKeys(extra);
  ...
  const dropUndefinedKeysResult1 = _mod13060.dropUndefinedKeys(tags);
  ...
  const dropUndefinedKeysResult2 = _mod13060.dropUndefinedKeys(user);
  ...
  const tmp3Result7 = _mod13060;
  const dropUndefinedKeysResult3 = _mod13060.dropUndefinedKeys(contexts);
  ...
  if (span) {
    const obj6 = { trace: tmp3(13059).spanToTraceContext(span) };
    const merged8 = Object.assign(extra.contexts);
    extra.contexts = obj6;
    const obj7 = { dynamicSamplingContext: null };
    const tmp3Result9 = tmp3(13059);
    obj7.dynamicSamplingContext = tmp3(13090).getDynamicSamplingContextFromSpan(span);
    const merged9 = Object.assign(extra.sdkProcessingMetadata);
    extra.sdkProcessingMetadata = obj7;
    const tmp3Result10 = tmp3(13090);
    const rootSpan = tmp3(13059).getRootSpan(span);
    const tmp3Result11 = tmp3(13059);
    const description = tmp3(13059).spanToJSON(rootSpan).description;
...
export const mergeAndOverwriteScopeData = function mergeAndOverwriteScopeData(arg0, arg1, arg2) {
  arg0[arg1] = _mod13079.merge(arg0[arg1], arg2, 1);
};
export const mergeScopeData = function mergeScopeData(extra, arg1) {
  ...
  extra.extra = _mod13079.merge(extra.extra, extra, 1);
  extra.tags = _mod13079.merge(extra.tags, tags, 1);
  extra.user = _mod13079.merge(extra.user, user, 1);
  extra.contexts = _mod13079.merge(extra.contexts, contexts, 1);
  extra.sdkProcessingMetadata = _mod13079.merge(extra.sdkProcessingMetadata, sdkProcessingMetadata, 2);
  if (level) {
    extra.level = level;
  }
  if (transactionName) {
