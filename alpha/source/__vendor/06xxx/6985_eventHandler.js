...
// Function ID: 6986
// Name: eventHandler
// Dependencies: [6967, 6905, 6920, 6907, 6969]
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6905.State.UNDETERMINED) {
    if (state === tmp(6905).State.BEGAN) {
      tmp(6967).runCallback(tmp(6920).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6967);
    }
  }
  if (oldState === _mod6905.State.BEGAN) {
    if (state === tmp(6905).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6967).runCallback(tmp(6920).CALLBACK_TYPE.START, arg1, result);
  ...
  if (oldState !== state) {
    ...
    obj2.canceled = state === tmp(6905).State.FAILED || state === tmp(6905).State.CANCELLED;
    if (oldState === tmp(6905).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6967).runCallback(tmp(6920).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6967);
    }
    const tmp7 = state === tmp(6905).State.FAILED || state === tmp(6905).State.CANCELLED;
    tmp(6967).runCallback(tmp(6920).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6967);
  }
}
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6967);
    tmpResult.runCallback(tmp(6967).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6907).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6967);
            tmpResult7.runCallback(tmp(6967).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6967);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6967).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6967);
          tmp(6967).runCallback(tmp(6920).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6967);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
