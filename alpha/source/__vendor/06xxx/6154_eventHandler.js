...
// Function ID: 6155
// Name: eventHandler
// Dependencies: [6136, 6074, 6089, 6076, 6138]
// Exports: eventHandler

// Module 6154 (eventHandler)
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6074.State.UNDETERMINED) {
    if (state === tmp(6074).State.BEGAN) {
      tmp(6136).runCallback(tmp(6089).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6136);
    }
  }
  if (oldState === _mod6074.State.BEGAN) {
    if (state === tmp(6074).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6136).runCallback(tmp(6089).CALLBACK_TYPE.START, arg1, result);
      const tmpResult4 = tmp(6136);
    }
  }
  if (oldState !== state) {
    const obj2 = {};
    const merged = Object.assign(result);
    obj2.canceled = state === tmp(6074).State.FAILED || state === tmp(6074).State.CANCELLED;
    if (oldState === tmp(6074).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6136).runCallback(tmp(6089).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6136);
    }
    const tmp7 = state === tmp(6074).State.FAILED || state === tmp(6074).State.CANCELLED;
    tmp(6136).runCallback(tmp(6089).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6136);
  }
}
handleStateChangeEvent.__closure = { flattenAndFilterEvent: fn(6136).flattenAndFilterEvent, State: fn(6074).State, runCallback: fn(6136).runCallback, CALLBACK_TYPE: fn(6089).CALLBACK_TYPE };
...
function handleUpdateEvent(lastUpdateEvent, arg1, fn, lastUpdateEvent2) {
  ...
  DEFAULT_PROPS_TRANSFORMER.runCallback(_mod6089.CALLBACK_TYPE.UPDATE, arg1, result);
  if (lastUpdateEvent2) {
    lastUpdateEvent2.lastUpdateEvent = lastUpdateEvent;
  }
}
let obj = { flattenAndFilterEvent: fn(6136).flattenAndFilterEvent, State: fn(6074).State, runCallback: fn(6136).runCallback, CALLBACK_TYPE: fn(6089).CALLBACK_TYPE };
handleUpdateEvent.__closure = { flattenAndFilterEvent: fn(6136).flattenAndFilterEvent, runCallback: fn(6136).runCallback, CALLBACK_TYPE: fn(6089).CALLBACK_TYPE };
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6136);
    tmpResult.runCallback(tmp(6136).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    const tmpResult2 = tmp(6136);
  }
}
let obj2 = { flattenAndFilterEvent: fn(6136).flattenAndFilterEvent, runCallback: fn(6136).runCallback, CALLBACK_TYPE: fn(6089).CALLBACK_TYPE };
handleTouchEvent.__closure = { TouchEventType: fn(6076).TouchEventType, runCallback: fn(6136).runCallback, touchEventTypeToCallbackType: fn(6136).touchEventTypeToCallbackType };
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6076).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6136);
            tmpResult7.runCallback(tmp(6136).touchEventTypeToCallbackType(result.eventType), arg2, result);
            const tmpResult8 = tmp(6136);
          }
        } else {
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6136).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6136);
          tmp(6136).runCallback(tmp(6089).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6136);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
...
const obj3 = { TouchEventType: fn(6076).TouchEventType, runCallback: fn(6136).runCallback, touchEventTypeToCallbackType: fn(6136).touchEventTypeToCallbackType };
eventHandler.__closure = { maybeExtractNativeEvent: fn(6136).maybeExtractNativeEvent, isEventForHandlerWithTag: fn(6136).isEventForHandlerWithTag, isStateChangeEvent: fn(6138).isStateChangeEvent, handleStateChangeEvent, isTouchEvent: fn(6138).isTouchEvent, handleTouchEvent, handleUpdateEvent };
