...
// Dependencies: [32, 19, 17, 21, 6917, 6977, 6913, 6972, 6914, 6904, 6942, 6973, 6974, 6966, 6975]
// Exports: InterceptingGestureDetector

// Module 6976 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  gesture(6917).useEnsureGestureHandlerRootView();
  const tmp6 = first1(closure_7(() => new Set()), 2);
  const first = tmp6[0];
  dependencyMap = tmp6[1];
  let items = [first];
  let prop;
  let obj = gesture(6917);
  ...
  if (prop) {
    let REANIMATED = tmp(6977).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
      prop1 = gesture.config.dispatchesAnimatedEvents;
    }
    const InterceptingDetectorMode = tmp(6977).InterceptingDetectorMode;
  ...
  const tmp14 = first1 === gesture(6977).InterceptingDetectorMode.REANIMATED;
  if (first1 === gesture(6977).InterceptingDetectorMode.ANIMATED) {
    let ReanimatedNativeDetector = tmp(6913).AnimatedNativeDetector;
  } else if (tmp14) {
    ReanimatedNativeDetector = tmp(6972).ReanimatedNativeDetector;
  } else {
    ReanimatedNativeDetector = first(6914);
  }
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6977.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6977.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
  if (ReanimatedNativeDetector) {
    ...
    const result = tmp(6973).ensureNativeDetectorComponent(ReanimatedNativeDetector);
    const tmpResult = tmp(6973);
    const gestureRelationsUpdater = tmp(6974).useGestureRelationsUpdater(gesture);
    ...
    const tmpResult4 = tmp(6974);
    const detectorAttachmentGuard = tmp(6975).useDetectorAttachmentGuard(tmp8Result5);
    ...
    obj4.style = tmp(6913).nativeDetectorStyles.detector;
    ...
    return jsx(tmp(6977).InterceptingDetectorContext, obj3);
  } else {
    const _Error = Error;
    let error = new Error(tmp(6904).tagMessage("Gesture expects to run on the UI thread, but failed to create the Reanimated NativeDetector."));
    throw error;
  }
