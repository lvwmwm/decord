...
import ModalPresentationContext from "ModalPresentationContext" /* 6433 */;
import _mod6444 from "module_6444" /* 6444 */;
import CardA11yWrapper from "CardA11yWrapper" /* 6445 */;
...
export const CardContainer = noop.memo(function CardContainerInner(active) {
  ...
  let num = noop.useContext(_mod5943.HeaderHeightContext);
  let tmp4 = focused;
  if (focused) {
    tmp4 = false !== scene.descriptor.options.keyboardHandlingEnabled;
  }
  let obj2 = Link;
  const keyboardManager = _mod6444.useKeyboardManager({ enabled: tmp4, focused });
  ({ onPageChangeStart: c10, onPageChangeCancel: c11, onPageChangeConfirm: c12 } = keyboardManager);
  const tmp2Result = _mod6444;
  ...
  if (previousScene) {
    ...
    headerTitle = tmp2(5943).getHeaderTitle(options, route.name);
    href = tmp2Result5.useLinkBuilder().buildHref(route.name, route.params);
    tmp8 = href;
    tmp9 = headerTitle;
    const tmp2Result6 = tmp2(5943);
  }
  closure_15 = tmp12;
  ...
  obj12.children = React4(_mod5943.HeaderHeightContext.Provider, obj13);
  obj11.children = React4(_mod5943.HeaderShownContext.Provider, obj12);
  obj10.children = React4(_mod5943.HeaderBackContext.Provider, obj11);
  ...
  obj4.children = React4(_mod6446.Card, obj5);
