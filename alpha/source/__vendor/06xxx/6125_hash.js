...
// Function ID: 6126
// Name: hash
// Dependencies: [32, 6112, 6111, 6126, 6135]
...
let c4 = 1.618;
fn(6112).NativeProxy.updateGestureHandlerConfig;
...
maybeUnpackValue.__closure = { Reanimated: fn(6111).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6111.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6111).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6111").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6111).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6111.Reanimated;
          const fn = function t() {
