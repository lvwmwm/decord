...
// Dependencies: [8816, 8817, 8818]
// Exports: default, extractTransformSvgView

// Module 8815 (appendTransformProps)
import append from "append" /* 8816 */;
import peg$SyntaxError from "peg$SyntaxError" /* 8817 */;
import _mod8818 from "module_8818" /* 8818 */;
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        const parsed = tmp3(8817).parse(arr);
        const tmp3Result5 = tmp3(8816);
        tmp3Result5.append(parsed[0], parsed[3], parsed[1], parsed[4], parsed[2], parsed[5]);
        const tmp3Result4 = tmp3(8817);
      } catch (tmp14) {
...
export const extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "string") {
    transform = _mod8818.parse(transform.transform);
  } else {
    transform = transform.transform;
  }
  return transform;
