...
// Function ID: 8631
// Name: StepsIndicator
// Dependencies: [19, 17, 21, 8628, 8629, 8631, 8632]
// Exports: StepsIndicator

// Module 8630 (StepsIndicator)
import _mod8628 from "module_8628" /* 8628 */;
import _mod8629 from "module_8629" /* 8629 */;
import SliderTrackMark from "SliderTrackMark" /* 8631 */;
import StepNumber from "StepNumber" /* 8632 */;
...
export const StepsIndicator = function StepsIndicator(options) {
  ...
  const memo = currentValue.useMemo(() => {
    if (options.length > 9) {
      let fontSize = _mod8628.constants.STEP_NUMBER_TEXT_FONT_SMALL;
    } else {
      fontSize = _mod8628.constants.STEP_NUMBER_TEXT_FONT_BIG;
    }
    return { fontSize };
  }, items);
  ...
  const memo1 = currentValue.useMemo(() => {
    if ("web" === get_ActivityIndicator.Platform.OS) {
      let stepsIndicator = _mod8629.styles.stepsIndicator;
      let tmp6 = require;
    } else {
      const _Object = Object;
      const obj = { marginHorizontal: sliderWidth * _mod8628.constants.MARGIN_HORIZONTAL_PADDING };
      stepsIndicator = Object.assign({}, _mod8629.styles.stepsIndicator, obj);
      tmp6 = require;
    }
    const obj2 = { stepIndicatorContainerStyle: stepsIndicator, stepIndicatorElementStyle: null };
    if ("web" === get_ActivityIndicator.Platform.OS) {
      const _Object2 = Object;
      const obj3 = { width: tmp6(8628).constants.THUMB_SIZE, justifyContent: "space-between" };
      let stepIndicatorElement = Object.assign({}, tmp6(8629).styles.stepIndicatorElement, obj3);
    } else {
      stepIndicatorElement = tmp6(8629).styles.stepIndicatorElement;
    }
