...
// Dependencies: [8812, 8813, 8814]
// Exports: default, extractTransformSvgView

// Module 8811 (appendTransformProps)
import append from "append" /* 8812 */;
import peg$SyntaxError from "peg$SyntaxError" /* 8813 */;
import _mod8814 from "module_8814" /* 8814 */;
...
function transformToMatrix(arg0, arr) {
  ...
  if (arr) {
    ...
    if (Array.isArray(arr)) {
      ...
      try {
        const parsed = tmp3(8813).parse(arr);
        const tmp3Result5 = tmp3(8812);
        tmp3Result5.append(parsed[0], parsed[3], parsed[1], parsed[4], parsed[2], parsed[5]);
        const tmp3Result4 = tmp3(8813);
      } catch (tmp14) {
...
export const extractTransformSvgView = function extractTransformSvgView(transform) {
  if (typeof transform.transform === "string") {
    transform = _mod8814.parse(transform.transform);
  } else {
    transform = transform.transform;
  }
  return transform;
