...
prototype["addReaction"] = function addReaction(emoji) {
  ...
  const mapped = reactions.map((emoji, index) => {
    ...
    if (obj.emojiEquals(emoji.emoji, closure_0)) {
      ...
      if (NORMAL === tmp(7182).ReactionTypes.BURST) {
        if (flag) {
          if (emoji.me_burst) {
            return emoji;
          }
        ...
        burst_colors = colors;
        tmp28 = flag;
      } else if (tmp12 === tmp(7182).ReactionTypes.VOTE) {
...
prototype["removeReaction"] = function removeReaction(arg0) {
  ...
  const mapped = reactions.map((emoji, index) => {
    ...
    if (!obj.emojiEquals(emoji.emoji, closure_0)) {
      ...
      if (NORMAL === tmp2(7182).ReactionTypes.BURST) {
        if (flag) {
          if (!merged1.me_burst) {
      ...
      } else if (tmp4 !== tmp2(7182).ReactionTypes.VOTE) {
        if (flag) {
          if (!merged1.me) {
