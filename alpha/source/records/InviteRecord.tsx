...
// Function ID: 8727
// Name: InviteRecord
// Dependencies: [1387, 1386, 4414, 2]

// Module 8726 (InviteRecord)
...
prototype["isExpired"] = function isExpired() {
  ...
  if (maxAge > 0) {
    const obj = _modDef4414(tmp.createdAt);
    const _Date = Date;
    if (addResult.isBefore(Date.now())) {
      return true;
    }
    addResult = _modDef4414(tmp.createdAt).add(maxAge, "seconds");
  }
  return false;
};
prototype["getExpiresAt"] = function getExpiresAt() {
  ...
  let num = Infinity;
  if (this.maxAge > 0) {
    const obj = _modDef4414(self.createdAt);
    num = _modDef4414(self.createdAt).add(self.maxAge, "seconds").toDate();
    const addResult = _modDef4414(self.createdAt).add(self.maxAge, "seconds");
  }
  return num;
};
