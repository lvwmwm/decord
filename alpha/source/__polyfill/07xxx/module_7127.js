// Module ID: 7127
// Function ID: 7128
// Dependencies: [17, 6958, 6965, 1637, 7128, 7129]
...
export const useGestureEventsHandlersDefault = () => {
  ...
  function handleOnEnd(arg0, translationY) {
    ...
    if (isInTemporaryPosition.value) {
      if (sharedValue.value.initialPosition >= iter.value) {
        if (iter3.value.initialPosition > iter.value) {
          animateToPosition(iter3.value.initialPosition, tmp2(6958).ANIMATION_SOURCE.GESTURE, velocityY / 2);
        }
      }
    }
    if (animatedScrollableType.value !== value22.SCROLLABLE_TYPE.UNDETERMINED) {
      value = iter4.value;
      const VIEW = tmp2(6958).SCROLLABLE_TYPE.VIEW;
    }
    if (tmp5) {
    ...
    if (snapPointResult !== animatedPosition.value) {
      let tmp12 = arg0 === tmp2(6958).GESTURE_SOURCE.CONTENT;
      if (tmp12) {
      ...
      if (!tmp12) {
        animateToPosition(snapPointResult, tmp2(6958).ANIMATION_SOURCE.GESTURE, velocityY / 2);
      }
    }
  }
