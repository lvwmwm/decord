// Module ID: 7071
// Function ID: 7072
// Dependencies: [19, 17, 21, 7072, 7073]
...
const memoResult = noop.memo(function BottomSheetHandleComponent(style) {
  ...
  const obj = {
    style: useMemo(() => {
      const items = [_mod7073.styles.container, React3.flatten(style)];
      return items;
    }, items),
    accessible: null,
  ...
  const items2 = [
    closure_5(closure_4, {
      style: useMemo(() => {
        const items = [_mod7073.styles.indicator, React3.flatten(indicatorStyle)];
        return items;
      }, items1)
    }),
    style.children
