// Module ID: 7010
// Function ID: 7011
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 7011, 6989, 7005, 7012, 7014, 6987, 7016, 7015, 7017, 7018, 6988, 7002]
// Exports: default

// Module 7010
import handlerIDToTag2 from "handlerIDToTag" /* 6987 */;
...
const jsx = fn(21).jsx;
fn(7011).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6989).State.UNDETERMINED]: undefined, [fn(6989).State.BEGAN]: "onBegan", [fn(6989).State.FAILED]: "onFailed", [fn(6989).State.CANCELLED]: "onCancelled", [fn(6989).State.ACTIVE]: "onActivated", [fn(6989).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod7005.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(7016).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(7016).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(7014).scheduleFlushOperations();
          const tmp2Result = tmp2(7014);
          tmp2(7015).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(7016).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(7015).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(7015);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(7014);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(7018)(self.viewNode));
        } else {
          const _Error = Error;
