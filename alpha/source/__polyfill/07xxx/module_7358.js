// Module ID: 7358
// Function ID: 7359
// Dependencies: [32, 19, 17, 21, 7352, 7353, 1506, 7359, 7338, 5945, 7360, 7361, 7362, 7363]
// Exports: Card
...
export const Card = function Card(shadowEnabled) {
  ...
  let tmp11 = num(1506)(() => {
    ...
    if (ref.current) {
      ...
      if (typeof getAnimateToValue === "function") {
        if (tmp16) {
          let num3 = _mod7352.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod7352.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19));
  ...
  const obj8 = {
    ...
    onHandlerStateChange: num(1506)((nativeEvent) => {
      ...
      if (PanGestureHandler.GestureState.ACTIVE === state) {
        ...
        if (tmp(7338).GestureState.CANCELLED !== state) {
          if (tmp(7338).GestureState.FAILED !== state) {
            if (tmp(7338).GestureState.END === state) {
              first3.setValue(0);
              if ("vertical" !== gestureDirection) {
