// Module ID: 7157
// Function ID: 7158
// Dependencies: [19, 17, 21, 7158, 7159]
...
const memoResult = noop.memo(function BottomSheetHandleComponent(style) {
  ...
  const obj = {
    style: useMemo(() => {
      const items = [_mod7159.styles.container, React3.flatten(style)];
      return items;
    }, items),
    accessible: null,
  ...
  const items2 = [
    closure_5(closure_4, {
      style: useMemo(() => {
        const items = [_mod7159.styles.indicator, React3.flatten(indicatorStyle)];
        return items;
      }, items1)
    }),
    style.children
