// Module ID: 7012
// Function ID: 7013
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 7013, 6991, 7007, 7014, 7016, 6989, 7018, 7017, 7019, 7020, 6990, 7004]
// Exports: default

// Module 7012
import handlerIDToTag2 from "handlerIDToTag" /* 6989 */;
...
const jsx = fn(21).jsx;
fn(7013).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6991).State.UNDETERMINED]: undefined, [fn(6991).State.BEGAN]: "onBegan", [fn(6991).State.FAILED]: "onFailed", [fn(6991).State.CANCELLED]: "onCancelled", [fn(6991).State.ACTIVE]: "onActivated", [fn(6991).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod7007.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(7018).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(7018).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(7016).scheduleFlushOperations();
          const tmp2Result = tmp2(7016);
          tmp2(7017).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(7018).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(7017).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(7017);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(7016);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(7020)(self.viewNode));
        } else {
          const _Error = Error;
