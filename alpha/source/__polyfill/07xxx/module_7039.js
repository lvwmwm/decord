// Module ID: 7039
// Function ID: 7040
// Dependencies: [17, 6870, 6877, 1637, 7040, 7041]
...
export const useGestureEventsHandlersDefault = () => {
  ...
  function handleOnEnd(arg0, translationY) {
    ...
    if (isInTemporaryPosition.value) {
      if (sharedValue.value.initialPosition >= iter.value) {
        if (iter3.value.initialPosition > iter.value) {
          animateToPosition(iter3.value.initialPosition, tmp2(6870).ANIMATION_SOURCE.GESTURE, velocityY / 2);
        }
      }
    }
    if (animatedScrollableType.value !== value22.SCROLLABLE_TYPE.UNDETERMINED) {
      value = iter4.value;
      const VIEW = tmp2(6870).SCROLLABLE_TYPE.VIEW;
    }
    if (tmp5) {
    ...
    if (snapPointResult !== animatedPosition.value) {
      let tmp12 = arg0 === tmp2(6870).GESTURE_SOURCE.CONTENT;
      if (tmp12) {
      ...
      if (!tmp12) {
        animateToPosition(snapPointResult, tmp2(6870).ANIMATION_SOURCE.GESTURE, velocityY / 2);
      }
    }
  }
