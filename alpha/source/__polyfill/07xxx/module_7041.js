// Module ID: 7041
// Function ID: 7042
// Dependencies: [32, 19, 7042, 7053, 7044, 7057, 7028]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(7044).SingleGestureName.Tap !== type) {
      if (tmp(7044).SingleGestureName.LongPress !== type) {
        if (tmp(7044).SingleGestureName.Fling !== type) {
          if (tmp(7044).SingleGestureName.Native !== type) {
            if (tmp(7044).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(7042).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_7057").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(7028).Reanimated;
    const enabledSharedValues = gesture(7042).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod7057.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
