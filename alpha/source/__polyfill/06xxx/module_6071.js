// Module ID: 6071
// Function ID: 6072
// Dependencies: [17, 6072, 6074, 6075, 6076]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6074).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6074).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6074).State.BEGAN) {
        if (handlerTag.state === tmp(6074).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6074).State.END) {
          if (handlerTag.oldState === tmp(6074).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6074).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6074).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6075).GestureStateManager;
      ...
      if (tmp(6076).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6076).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6076).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6076).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6072).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
