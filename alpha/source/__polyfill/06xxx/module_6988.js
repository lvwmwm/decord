// Module ID: 6988
// Function ID: 6989
// Dependencies: [17, 6989, 6991, 6992, 6993]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6991).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6991).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6991).State.BEGAN) {
        if (handlerTag.state === tmp(6991).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6991).State.END) {
          if (handlerTag.oldState === tmp(6991).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6991).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6991).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6992).GestureStateManager;
      ...
      if (tmp(6993).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6993).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6993).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6993).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6989).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
