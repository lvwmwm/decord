// Module ID: 6955
// Function ID: 6956
// Dependencies: [32, 19, 6956, 6967, 6958, 6971, 6942]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6958).SingleGestureName.Tap !== type) {
      if (tmp(6958).SingleGestureName.LongPress !== type) {
        if (tmp(6958).SingleGestureName.Fling !== type) {
          if (tmp(6958).SingleGestureName.Native !== type) {
            if (tmp(6958).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6956).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6971").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6942).Reanimated;
    const enabledSharedValues = gesture(6956).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6971.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
