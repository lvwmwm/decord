// Module ID: 6953
// Function ID: 6954
// Dependencies: [32, 19, 6954, 6965, 6956, 6969, 6940]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6956).SingleGestureName.Tap !== type) {
      if (tmp(6956).SingleGestureName.LongPress !== type) {
        if (tmp(6956).SingleGestureName.Fling !== type) {
          if (tmp(6956).SingleGestureName.Native !== type) {
            if (tmp(6956).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6954).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6969").JSResponderContext);
  dependencyMap = tmp;
  ...
  const items = [gesture];
  closure_5(() => {
    const Reanimated = gesture(6940).Reanimated;
    const enabledSharedValues = gesture(6954).getEnabledSharedValues(Reanimated);
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6969.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
