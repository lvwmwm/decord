// Module ID: 6900
// Function ID: 6901
// Dependencies: [17, 6901, 6903, 6904, 6905]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6903).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6903).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6903).State.BEGAN) {
        if (handlerTag.state === tmp(6903).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6903).State.END) {
          if (handlerTag.oldState === tmp(6903).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6903).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6903).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6904).GestureStateManager;
      ...
      if (tmp(6905).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6905).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6905).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6905).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6901).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
