// Module ID: 6441
// Function ID: 6442
// Dependencies: [32, 19, 17, 21, 6435, 6436, 1506, 6442, 6421, 5947, 6443, 6444, 6445, 6446]
// Exports: Card
...
export const Card = function Card(shadowEnabled) {
  ...
  let tmp11 = num(1506)(() => {
    ...
    if (ref.current) {
      ...
      if (typeof getAnimateToValue === "function") {
        if (tmp16) {
          let num3 = _mod6435.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod6435.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19));
  ...
  const obj8 = {
    ...
    onHandlerStateChange: num(1506)((nativeEvent) => {
      ...
      if (PanGestureHandler.GestureState.ACTIVE === state) {
        ...
        if (tmp(6421).GestureState.CANCELLED !== state) {
          if (tmp(6421).GestureState.FAILED !== state) {
            if (tmp(6421).GestureState.END === state) {
              first3.setValue(0);
              if ("vertical" !== gestureDirection) {
