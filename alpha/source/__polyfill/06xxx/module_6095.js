// Module ID: 6095
// Function ID: 6096
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6096, 6074, 6090, 6097, 6099, 6072, 6101, 6100, 6102, 6103, 6073, 6087]
// Exports: default

// Module 6095
import handlerIDToTag2 from "handlerIDToTag" /* 6072 */;
import tagMessage from "tagMessage" /* 6073 */;
import _mod6090 from "module_6090" /* 6090 */;
...
const jsx = fn(21).jsx;
fn(6096).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6074).State.UNDETERMINED]: undefined, [fn(6074).State.BEGAN]: "onBegan", [fn(6074).State.FAILED]: "onFailed", [fn(6074).State.CANCELLED]: "onCancelled", [fn(6074).State.ACTIVE]: "onActivated", [fn(6074).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6090.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6101).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6101).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6099).scheduleFlushOperations();
          const tmp2Result = tmp2(6099);
          tmp2(6100).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6101).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6100).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6100);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6099);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6103)(self.viewNode));
        } else {
          const _Error = Error;
