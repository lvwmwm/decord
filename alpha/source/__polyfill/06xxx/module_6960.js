// Module ID: 6960
// Function ID: 6961
// Dependencies: [32, 19, 17, 21, 1637, 6961, 6962, 6985, 6958, 6974, 38, 6968, 7138, 7140, 7143, 7145, 7149, 7152, 7156]
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6958).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6958).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6958).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6958).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            iter.value = tmp18(6974).animate(obj);
            const tmp18Result2 = tmp18(6974);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6958).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6958).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6985).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6985).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6958).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1637).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6958).ANIMATION_SOURCE.USER, 0, arg1);
