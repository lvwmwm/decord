// Module ID: 6902
// Function ID: 6903
// Dependencies: [17, 6903, 6905, 6906, 6907]
...
function onGestureHandlerEvent(handlerTag) {
  ...
  if (findHandlerResult) {
    if (null != handlerTag.oldState) {
      if (handlerTag.oldState === tmp(6905).State.UNDETERMINED) {
        if (handlerTag.state === tmp(6905).State.BEGAN) {
      ...
      if (handlerTag.oldState === tmp(6905).State.BEGAN) {
        if (handlerTag.state === tmp(6905).State.ACTIVE) {
      ...
      if (handlerTag.oldState !== handlerTag.state) {
        if (handlerTag.state === tmp(6905).State.END) {
          if (handlerTag.oldState === tmp(6905).State.ACTIVE) {
      ...
      let tmp18 = handlerTag.state !== tmp(6905).State.FAILED;
      if (tmp18) {
        tmp18 = handlerTag.state !== tmp(6905).State.CANCELLED;
      }
      if (!tmp18) {
      ...
      if (!map.has(handlerTag.handlerTag)) {
        const GestureStateManager = tmp(6906).GestureStateManager;
      ...
      if (tmp(6907).TouchEventType.TOUCHES_DOWN === eventType) {
      ...
      } else if (tmp(6907).TouchEventType.TOUCHES_MOVE === eventType) {
      ...
      } else if (tmp(6907).TouchEventType.TOUCHES_UP === eventType) {
      ...
      } else if (tmp(6907).TouchEventType.TOUCHES_CANCEL === eventType) {
    ...
    const result2 = tmp(6903).findOldGestureHandler(handlerTag.handlerTag);
    if (result2) {
