// Module ID: 6926
// Function ID: 6927
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6927, 6905, 6921, 6928, 6930, 6903, 6932, 6931, 6933, 6934, 6904, 6918]
// Exports: default

// Module 6926
import handlerIDToTag2 from "handlerIDToTag" /* 6903 */;
...
const jsx = fn(21).jsx;
fn(6927).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6905).State.UNDETERMINED]: undefined, [fn(6905).State.BEGAN]: "onBegan", [fn(6905).State.FAILED]: "onFailed", [fn(6905).State.CANCELLED]: "onCancelled", [fn(6905).State.ACTIVE]: "onActivated", [fn(6905).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6921.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6932).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6932).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6930).scheduleFlushOperations();
          const tmp2Result = tmp2(6930);
          tmp2(6931).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6932).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6931).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6931);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6930);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6934)(self.viewNode));
        } else {
          const _Error = Error;
