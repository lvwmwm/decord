// Module ID: 6100
// Function ID: 6101
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6101, 6079, 6095, 6102, 6104, 6077, 6106, 6105, 6107, 6108, 6078, 6092]
// Exports: default

// Module 6100
import handlerIDToTag2 from "handlerIDToTag" /* 6077 */;
import tagMessage from "tagMessage" /* 6078 */;
import _mod6095 from "module_6095" /* 6095 */;
...
const jsx = fn(21).jsx;
fn(6101).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6079).State.UNDETERMINED]: undefined, [fn(6079).State.BEGAN]: "onBegan", [fn(6079).State.FAILED]: "onFailed", [fn(6079).State.CANCELLED]: "onCancelled", [fn(6079).State.ACTIVE]: "onActivated", [fn(6079).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6095.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6106).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6106).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6104).scheduleFlushOperations();
          const tmp2Result = tmp2(6104);
          tmp2(6105).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6106).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6105).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6105);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6104);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6108)(self.viewNode));
        } else {
          const _Error = Error;
