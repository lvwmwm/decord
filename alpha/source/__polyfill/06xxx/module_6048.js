// Module ID: 6048
// Function ID: 6049
// Dependencies: [32, 19, 17, 21, 1638, 6049, 6050, 6073, 6046, 6062, 38, 6056, 6226, 6228, 6231, 6233, 6237, 6240, 6244]
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6046).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6046).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6046).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6046).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            tmp18(1638).runOnJS(closure_73)(sharedValue4.value, initialPosition, GESTURE);
            const tmp18Result = tmp18(1638);
            ...
            iter.value = tmp18(6062).animate(obj);
            const tmp18Result2 = tmp18(6062);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6046).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6046).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6073).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6073).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6046).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1638).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6046).ANIMATION_SOURCE.USER, 0, arg1);
        const runOnUIResult = tmp(1638).runOnUI(workletCallback2);
      }
    };
