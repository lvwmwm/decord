// Module ID: 6043
// Function ID: 6044
// Dependencies: [32, 19, 17, 21, 1637, 6044, 6045, 6068, 6041, 6057, 38, 6051, 6221, 6223, 6226, 6228, 6232, 6235, 6239]
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6041).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6041).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6041).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6041).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            iter.value = tmp18(6057).animate(obj);
            const tmp18Result2 = tmp18(6057);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6041).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6041).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6068).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6068).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6041).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1637).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6041).ANIMATION_SOURCE.USER, 0, arg1);
