// Module ID: 12418
// Function ID: 12419
// Dependencies: [718, 12309, 12312, 12313, 12327, 12337, 12341, 12345, 12395, 12318, 12347, 12328, 12346, 12384, 12330, 12326, 12320]
...
function _addTracingHeadersToFetchRequest(headers, headers2, span) {
  const traceData = _mod12384.getTraceData({ span });
  ...
  if (tmp4) {
    ...
    if (!headers) {
      ...
      if (typeof Request !== "undefined") {
        ...
        isInstanceOfResult = tmp(12320).isInstanceOf(headers, Request);
        const tmpResult = tmp(12320);
      }
      let headers1;
    ...
    if (headers) {
      ...
      if (typeof Headers !== "undefined") {
        ...
        isInstanceOfResult1 = tmp(12320).isInstanceOf(headers, Headers);
...
export const instrumentFetchRequest = function instrumentFetchRequest(fetchData, fn, fn2, arg3) {
  ...
  if (fetchData.fetchData) {
    ...
    let hasTracingEnabledResult = _mod12345.hasTracingEnabled();
        ...
        if (__span) {
          if (arg3[__span]) {
            if (endResult.response) {
              setHttpStatus = setHttpStatus(12330).setHttpStatus;
              ...
              const setHttpStatusResult = setHttpStatus(12330);
            } else if (endResult.error) {
              const obj2 = { code: setHttpStatus(12330).SPAN_STATUS_ERROR, message: "internal_error" };
    ...
    if (tmp7) {
      const host = setHttpStatus(12395).parseUrl(tmp7).host;
      const setHttpStatusResult2 = setHttpStatus(12395);
    }
    const activeSpan = setHttpStatus(12318).getActiveSpan();
    if (hasTracingEnabledResult) {
      if (activeSpan) {
        ...
        obj4[setHttpStatus(12328).SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = str;
        obj4[setHttpStatus(12328).SEMANTIC_ATTRIBUTE_SENTRY_OP] = "http.client";
        obj3.attributes = obj4;
        let startInactiveSpanResult = setHttpStatus(12347).startInactiveSpan(obj3);
        const setHttpStatusResult4 = setHttpStatus(12347);
      }
      ...
      if (fn2(endResult.fetchData.url)) {
        ...
        setHttpStatusResult5 = setHttpStatus(12345);
      }
      return startInactiveSpanResult;
    }
    startInactiveSpanResult = new setHttpStatus(12346).SentryNonRecordingSpan();
    const setHttpStatusResult3 = setHttpStatus(12318);
  }
};
