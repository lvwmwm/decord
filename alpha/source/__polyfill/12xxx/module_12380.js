// Module ID: 12380
// Function ID: 12381
// Dependencies: [12295, 12296, 12381, 12322, 12351, 12382, 12306, 12304, 12343]
    ...
    setup(arg0) {
      ...
      if ("console" in CONSOLE_LEVELS(handled[1]).GLOBAL_OBJ) {
        let result = CONSOLE_LEVELS(handled[2]).addConsoleInstrumentationHandler((arg0) => {
          ...
          if (hasItem) {
            closure_2 = handled;
            let obj2 = { level: tmp(12382).severityLevelFromString(level), extra: null };
            const obj3 = { arguments: args };
            obj2.extra = obj3;
            const tmpResult = tmp(12382);
            tmp(12322).withScope((addEventProcessor) => {
              ...
              if ("assert" !== level) {
                ...
                if (found) {
                  tmp14(12343).captureException(found, obj2);
                  const tmp14Result = tmp14(12343);
                } else {
                  const tmp14Result2 = tmp14(12304);
                  const safeJoinResult = tmp14(12304).safeJoin(tmp12, " ");
                ...
                const combined = "Assertion failed: " + args(12304).safeJoin(arr.slice(1), " ") || "console.assert";
                addEventProcessor.setExtra("arguments", arr.slice(1));
                obj2 = args(12343);
                obj2.captureMessage(combined, obj2);
                const tmp4 = args(12304).safeJoin(arr.slice(1), " ") || "console.assert";
              }
            });
