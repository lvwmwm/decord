// Module ID: 12333
// Function ID: 12334
// Dependencies: [12323, 12300, 12295]
...
export const logSpanEnd = function logSpanEnd(spanContext) {
  if (_mod12323.DEBUG_BUILD) {
    const spanToJSONResult = tmp(12300).spanToJSON(spanContext);
...
export const logSpanStart = function logSpanStart(spanContext) {
  if (_mod12323.DEBUG_BUILD) {
    ...
    const spanIsSampledResult = tmp(12300).spanIsSampled(spanContext);
    ...
    if (rootSpan !== spanContext) {
      const tmpResult6 = tmp(12300);
      ({ op: op2, description: description2 } = tmp(12300).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(12300).spanToJSON(rootSpan);
    }
    const logger = tmp(12295).logger;
    const _HermesInternal9 = HermesInternal;
    logger.log("" + combined + "\n  " + items.join("\n  "));
    const tmpResult5 = tmp(12300);
  }
};
