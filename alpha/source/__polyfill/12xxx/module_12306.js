// Module ID: 12306
// Function ID: 12307
// Dependencies: [12296, 12304, 12301]
...
export const addContextToFrame = function addContextToFrame(arr, lineno) {
  ...
  if (undefined !== lineno.lineno) {
    ...
    lineno.pre_context = substr.map((item) => _mod12304.snipLine(item, 0));
    const _Math5 = Math;
    ...
    lineno.context_line = _mod12304.snipLine(arr[bound1], num2);
    const _Math = Math;
    const substr1 = arr.slice(Math.min(bound + 1, length), bound + 1 + num);
    lineno.post_context = substr1.map((item) => _mod12304.snipLine(item, 0));
  }
};
...
export const checkOrSetAlreadyCaught = function checkOrSetAlreadyCaught(__sentry_captured__) {
  if ((function isAlreadyCaptured(__sentry_captured__) {
    ...
    try {
      const result = _mod12301.addNonEnumerableProperty(__sentry_captured__, "__sentry_captured__", true);
      return false;
    } catch (err) {
    }
  }
