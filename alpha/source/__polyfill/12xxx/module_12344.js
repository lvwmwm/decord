// Module ID: 12344
// Function ID: 12345
// Dependencies: [12306, 12309, 12322, 12345, 12346, 12332, 12304, 12347, 12340, 12316]
...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = _mod12304.truncate(environment.message, num);
  }
  value = iter;
  ...
  if (value) {
    iter.value = _mod12304.truncate(iter.value, num);
  }
  ...
  if (url) {
    request.url = _mod12304.truncate(request.url, num);
  }
}
function applyDebugIds(exception, arg1) {
  const filenameToDebugIdMap = _mod12347.getFilenameToDebugIdMap(arg1);
  try {
