...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.getScrollResponder = () => f110324;
    tmp3Result.getScrollableNode = () => renderElement.findNodeHandle(f110324.getNativeScrollRef());
    tmp3Result.getInnerViewNode = () => renderElement.findNodeHandle(f110324._innerView.nativeInstance);
    tmp3Result.getInnerViewRef = () => f110324._innerView.nativeInstance;
    tmp3Result.getNativeScrollRef = () => f110324._scrollView.nativeInstance;
    tmp3Result.scrollTo = (num, arg1, arg2) => {
      ...
      const nativeScrollRef = f110324.getNativeScrollRef();
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      f110324._additionalScrollOffset = num;
      f110324._preventNegativeScrollOffset = _preventNegativeScrollOffset;
      if (null != f110324._innerView.nativeInstance) {
    ...
    tmp3Result.scrollResponderZoomTo = (animated, arg1) => {
      ...
      if ("animated" in animated) {
        f110324._animated = animated.animated;
        delete tmp[tmp2];
      ...
      if (null == _keyboardMetrics._keyboardMetrics) {
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f110324._keyboardMetrics) {
              screenY = obj._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + obj._additionalScrollOffset;
            let bound = sum;
            if (true === f110324._preventNegativeScrollOffset) {
              const _Math = Math;
              bound = Math.max(0, sum);
            }
            const point = { x: 0, y: bound, animated: true };
            f110324.scrollTo(point);
            f110324._additionalScrollOffset = 0;
            f110324._preventNegativeScrollOffset = false;
          } else {
    ...
    tmp3Result._handleScroll = (arg0) => {
      f110324._observedScrollSinceBecomingResponder = true;
      if (f110324.props.onScroll) {
        const props = tmp.props;
        props.onScroll(arg0);
      }
    };
    tmp3Result._handleLayout = (nativeEvent) => {
      if (true === f110324.props.invertStickyHeaders) {
        const obj2 = { layoutHeight: nativeEvent.nativeEvent.layout.height };
        obj.setState(obj2);
      }
      if (f110324.props.onLayout) {
        const props = obj.props;
        props.onLayout(nativeEvent);
      }
    };
    tmp3Result._handleContentOnLayout = (arg0) => {
      if (f110324.props.onContentSizeChange) {
        const props = f110324.props;
    ...
    f110324 = (arg0) => Object.assign(arg0, { getScrollResponder: f110324.getScrollResponder, getScrollableNode: f110324.getScrollableNode, getInnerViewNode: f110324.getInnerViewNode, getInnerViewRef: f110324.getInnerViewRef, getNativeScrollRef: f110324.getNativeScrollRef, scrollTo: f110324.scrollTo, scrollToEnd: f110324.scrollToEnd, flashScrollIndicators: f110324.flashScrollIndicators, scrollResponderZoomTo: f110324.scrollResponderZoomTo, scrollResponderScrollNativeHandleToKeyboard: f110324.scrollResponderScrollNativeHandleToKeyboard });
    closure_1 = undefined;
    obj4 = {
      getForwardingRef: c2(closure_4[29])((arg0) => {
            closure_0 = arg0;
    ...
    tmp3Result.scrollResponderKeyboardWillShow = (endCoordinates) => {
      f110324._keyboardMetrics = endCoordinates.endCoordinates;
      if (f110324.props.onKeyboardWillShow) {
        const props = f110324.props;
        props.onKeyboardWillShow(endCoordinates);
      }
    };
    tmp3Result.scrollResponderKeyboardWillHide = (arg0) => {
      f110324._keyboardMetrics = null;
      if (f110324.props.onKeyboardWillHide) {
        const props = tmp.props;
    ...
    tmp3Result.scrollResponderKeyboardDidShow = (endCoordinates) => {
      f110324._keyboardMetrics = endCoordinates.endCoordinates;
      if (f110324.props.onKeyboardDidShow) {
        const props = f110324.props;
    ...
    tmp3Result.scrollResponderKeyboardDidHide = (arg0) => {
      f110324._keyboardMetrics = null;
      if (f110324.props.onKeyboardDidHide) {
        const props = tmp.props;
    ...
    tmp3Result._handleMomentumScrollBegin = (arg0) => {
      ...
      f110324._lastMomentumScrollBeginTime = _performance.now();
      if (f110324.props.onMomentumScrollBegin) {
        const props = f110324.props;
    ...
    tmp3Result._handleMomentumScrollEnd = (arg0) => {
      ...
      const _performance = ScrollView.performance;
      f110324._lastMomentumScrollEndTime = _performance.now();
      if (f110324.props.onMomentumScrollEnd) {
        const props = f110324.props;
    ...
    tmp3Result._handleScrollBeginDrag = (arg0) => {
      ...
      if ("on-drag" === f110324.props.keyboardDismissMode) {
        dismissKeyboardDefault();
      }
      if (f110324.props.onScrollBeginDrag) {
    ...
    tmp3Result._handleScrollEndDrag = (nativeEvent) => {
      ...
      let _isAnimatingResult = f110324._isAnimating();
      ...
      if (f110324.props.onScrollEndDrag) {
        const props = tmp.props;
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return _performance.now() - f110324._lastMomentumScrollEndTime < 16 || f110324._lastMomentumScrollEndTime < f110324._lastMomentumScrollBeginTime;
    };
    tmp3Result._handleResponderGrant = (arg0) => {
      f110324._observedScrollSinceBecomingResponder = false;
      if (f110324.props.onResponderGrant) {
        const props = obj.props;
        props.onResponderGrant(arg0);
      }
      f110324._becameResponderWhileAnimating = f110324._isAnimating();
    };
    tmp3Result._handleResponderReject = () => {

    };
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      f110324._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      if (f110324.props.onResponderRelease) {
        const props = obj.props;
    ...
    tmp3Result._handleResponderTerminationRequest = () => !f110324._observedScrollSinceBecomingResponder;
    tmp3Result._handleScrollShouldSetResponder = () => true !== f110324.props.disableScrollViewPanResponder && f110324._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f110324.props.disableScrollViewPanResponder) {
        return false;
      } else {
    ...
    tmp3Result._softKeyboardIsDetached = () => {
      let tmp = null != f110324._keyboardMetrics;
      if (tmp) {
        tmp = 0 === f110324._keyboardMetrics.height;
      }
      return tmp;
    };
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      f110324._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      const keyboardShouldPersistTaps = f110324.props.keyboardShouldPersistTaps;
      let tmp = !keyboardShouldPersistTaps;
      if (keyboardShouldPersistTaps) {
        tmp = "never" === keyboardShouldPersistTaps;
      }
