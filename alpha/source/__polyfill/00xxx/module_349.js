...
class ScrollView {
  constructor(arg0) {
    ...
    tmp3Result.getScrollResponder = () => f109312;
    tmp3Result.getScrollableNode = () => renderElement.findNodeHandle(f109312.getNativeScrollRef());
    tmp3Result.getInnerViewNode = () => renderElement.findNodeHandle(f109312._innerView.nativeInstance);
    tmp3Result.getInnerViewRef = () => f109312._innerView.nativeInstance;
    tmp3Result.getNativeScrollRef = () => f109312._scrollView.nativeInstance;
    tmp3Result.scrollTo = (num, arg1, arg2) => {
      ...
      const nativeScrollRef = f109312.getNativeScrollRef();
    ...
    tmp3Result.scrollResponderScrollNativeHandleToKeyboard = (measureLayout, arg1, _preventNegativeScrollOffset) => {
      ...
      f109312._additionalScrollOffset = num;
      f109312._preventNegativeScrollOffset = _preventNegativeScrollOffset;
      if (null != f109312._innerView.nativeInstance) {
    ...
    tmp3Result.scrollResponderZoomTo = (animated, arg1) => {
      ...
      if ("animated" in animated) {
        f109312._animated = animated.animated;
        delete tmp[tmp2];
      ...
      if (null == _keyboardMetrics._keyboardMetrics) {
        ...
        const timerId = setTimeout(() => {
          if (typeof scrollTextInputIntoVisibleRect === "function") {
            if (null != f109312._keyboardMetrics) {
              screenY = obj._keyboardMetrics.screenY;
            }
            const sum = closure_0 - screenY + closure_1 + obj._additionalScrollOffset;
            let bound = sum;
            if (true === f109312._preventNegativeScrollOffset) {
              const _Math = Math;
              bound = Math.max(0, sum);
            }
            const point = { x: 0, y: bound, animated: true };
            f109312.scrollTo(point);
            f109312._additionalScrollOffset = 0;
            f109312._preventNegativeScrollOffset = false;
          } else {
    ...
    tmp3Result._handleScroll = (arg0) => {
      f109312._observedScrollSinceBecomingResponder = true;
      if (f109312.props.onScroll) {
        const props = tmp.props;
        props.onScroll(arg0);
      }
    };
    tmp3Result._handleLayout = (nativeEvent) => {
      if (true === f109312.props.invertStickyHeaders) {
        const obj2 = { layoutHeight: nativeEvent.nativeEvent.layout.height };
        obj.setState(obj2);
      }
      if (f109312.props.onLayout) {
        const props = obj.props;
        props.onLayout(nativeEvent);
      }
    };
    tmp3Result._handleContentOnLayout = (arg0) => {
      if (f109312.props.onContentSizeChange) {
        const props = f109312.props;
    ...
    f109312 = (arg0) => Object.assign(arg0, { getScrollResponder: f109312.getScrollResponder, getScrollableNode: f109312.getScrollableNode, getInnerViewNode: f109312.getInnerViewNode, getInnerViewRef: f109312.getInnerViewRef, getNativeScrollRef: f109312.getNativeScrollRef, scrollTo: f109312.scrollTo, scrollToEnd: f109312.scrollToEnd, flashScrollIndicators: f109312.flashScrollIndicators, scrollResponderZoomTo: f109312.scrollResponderZoomTo, scrollResponderScrollNativeHandleToKeyboard: f109312.scrollResponderScrollNativeHandleToKeyboard });
    closure_1 = undefined;
    obj4 = {
      getForwardingRef: c2(closure_4[29])((arg0) => {
            closure_0 = arg0;
    ...
    tmp3Result.scrollResponderKeyboardWillShow = (endCoordinates) => {
      f109312._keyboardMetrics = endCoordinates.endCoordinates;
      if (f109312.props.onKeyboardWillShow) {
        const props = f109312.props;
        props.onKeyboardWillShow(endCoordinates);
      }
    };
    tmp3Result.scrollResponderKeyboardWillHide = (arg0) => {
      f109312._keyboardMetrics = null;
      if (f109312.props.onKeyboardWillHide) {
        const props = tmp.props;
    ...
    tmp3Result.scrollResponderKeyboardDidShow = (endCoordinates) => {
      f109312._keyboardMetrics = endCoordinates.endCoordinates;
      if (f109312.props.onKeyboardDidShow) {
        const props = f109312.props;
    ...
    tmp3Result.scrollResponderKeyboardDidHide = (arg0) => {
      f109312._keyboardMetrics = null;
      if (f109312.props.onKeyboardDidHide) {
        const props = tmp.props;
    ...
    tmp3Result._handleMomentumScrollBegin = (arg0) => {
      ...
      f109312._lastMomentumScrollBeginTime = _performance.now();
      if (f109312.props.onMomentumScrollBegin) {
        const props = f109312.props;
    ...
    tmp3Result._handleMomentumScrollEnd = (arg0) => {
      ...
      const _performance = ScrollView.performance;
      f109312._lastMomentumScrollEndTime = _performance.now();
      if (f109312.props.onMomentumScrollEnd) {
        const props = f109312.props;
    ...
    tmp3Result._handleScrollBeginDrag = (arg0) => {
      ...
      if ("on-drag" === f109312.props.keyboardDismissMode) {
        dismissKeyboardDefault();
      }
      if (f109312.props.onScrollBeginDrag) {
    ...
    tmp3Result._handleScrollEndDrag = (nativeEvent) => {
      ...
      let _isAnimatingResult = f109312._isAnimating();
      ...
      if (f109312.props.onScrollEndDrag) {
        const props = tmp.props;
    ...
    tmp3Result._isAnimating = () => {
      const _performance = ScrollView.performance;
      return _performance.now() - f109312._lastMomentumScrollEndTime < 16 || f109312._lastMomentumScrollEndTime < f109312._lastMomentumScrollBeginTime;
    };
    tmp3Result._handleResponderGrant = (arg0) => {
      f109312._observedScrollSinceBecomingResponder = false;
      if (f109312.props.onResponderGrant) {
        const props = obj.props;
        props.onResponderGrant(arg0);
      }
      f109312._becameResponderWhileAnimating = f109312._isAnimating();
    };
    tmp3Result._handleResponderReject = () => {

    };
    tmp3Result._handleResponderRelease = (nativeEvent) => {
      f109312._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      if (f109312.props.onResponderRelease) {
        const props = obj.props;
    ...
    tmp3Result._handleResponderTerminationRequest = () => !f109312._observedScrollSinceBecomingResponder;
    tmp3Result._handleScrollShouldSetResponder = () => true !== f109312.props.disableScrollViewPanResponder && f109312._isTouching;
    tmp3Result._handleStartShouldSetResponder = (target) => {
      if (true === f109312.props.disableScrollViewPanResponder) {
        return false;
      } else {
    ...
    tmp3Result._softKeyboardIsDetached = () => {
      let tmp = null != f109312._keyboardMetrics;
      if (tmp) {
        tmp = 0 === f109312._keyboardMetrics.height;
      }
      return tmp;
    };
    ...
    tmp3Result._handleTouchEnd = (nativeEvent) => {
      f109312._isTouching = 0 !== nativeEvent.nativeEvent.touches.length;
      const keyboardShouldPersistTaps = f109312.props.keyboardShouldPersistTaps;
      let tmp = !keyboardShouldPersistTaps;
      if (keyboardShouldPersistTaps) {
        tmp = "never" === keyboardShouldPersistTaps;
      }
