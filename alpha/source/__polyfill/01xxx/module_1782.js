...
export const useAnimatedGestureHandler = function useAnimatedGestureHandler(handlers, items10) {
  _require = handlers;
  const handler = require("module_1783").useHandler(handlers, items10);
  ...
  let event = fn;
  if (!useWeb) {
    event = require("module_1785").useEvent(fn, ["onGestureHandlerStateChange", "onGestureHandlerEvent"], tmp4);
    const tmpResult = require("module_1785");
  }
  return event;
};
