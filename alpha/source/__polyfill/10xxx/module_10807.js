...
function getDaysToWeekday(dateWithAdjustedTimezone, sum, next) {
  ...
  if ("this" === next) {
    ...
    if (day == Meridiem.Weekday.SUNDAY) {
      let num12 = 7;
      if (sum != tmp6(10785).Weekday.SUNDAY) {
        num12 = sum;
      }
      let sum3 = num12;
    } else if (day == tmp6(10785).Weekday.SATURDAY) {
      let num9 = 7;
      if (sum != tmp6(10785).Weekday.SATURDAY) {
        let num10 = 8;
        if (sum != tmp6(10785).Weekday.SUNDAY) {
          num10 = 1 + sum;
        }
