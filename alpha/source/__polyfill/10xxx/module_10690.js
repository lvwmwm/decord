...
const regExp = new RegExp("(?:(?:within|in|for)\\s*)?(?:(?:about|around|roughly|approximately|just)\\s*(?:~\\s*)?)?(" + _mod10691.TIME_UNITS_PATTERN + ")(?=\\W|$)", "i");
const regExp1 = new RegExp("(?:within|in|for)\\s*(?:(?:about|around|roughly|approximately|just)\\s*(?:~\\s*)?)?(" + _mod10691.TIME_UNITS_PATTERN + ")(?=\\W|$)", "i");
const regExp2 = new RegExp("(?:within|in|for)\\s*(?:(?:about|around|roughly|approximately|just)\\s*(?:~\\s*)?)?(" + _mod10691.TIME_UNITS_NO_ABBR_PATTERN + ")(?=\\W|$)", "i");
  ...
  entry,
  {
    ...
    value: function innerExtract(reference, arg1) {
      if (str.match(/^for\s*the\s*\w+/)) {
        ...
        const parseDurationResult = ENTimeUnitWithinFormatParser(10691).parseDuration(arg1[1]);
