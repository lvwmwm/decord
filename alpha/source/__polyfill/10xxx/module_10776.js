// Module ID: 10776
// Function ID: 10777
// Dependencies: [41, 42, 10777, 10783]

// Module 10776
import ENDefaultConfiguration2 from "ENDefaultConfiguration" /* 10777 */;
...
const entry1 = {
  ...
  value: function executeParser(debug, pattern) {
    ...
    if (match) {
      ...
      if (extractResult instanceof ParsingContext(10783).ParsingResult) {
      ...
      } else if (!(extractResult instanceof tmp6(10783).ParsingComponents)) {
...
const entry2 = {
  ...
  value: function createParsingComponents(date) {
    ...
    if (!(date instanceof ParsingContext(10783).ParsingComponents)) {
      const self = this;
      parsingComponents = new ParsingContext(10783).ParsingComponents(this.reference, date);
    }
...
const items2 = [
  entry2,
  {
    ...
    value: function createParsingResult(sum, match, extractResult, date) {
      ...
      return new ParsingContext(10783).ParsingResult(self.reference, sum, substr, parsingComponents, parsingComponents1);
    }
  },
  {
    key: "debug",
