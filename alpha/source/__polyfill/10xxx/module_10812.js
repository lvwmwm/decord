// Module ID: 10812
// Function ID: 10813
// Dependencies: [41, 42, 93, 95, 98, 10779, 10782, 10783, 10799]
...
const items = [
  ...
  },
  {
    ...
    value: function mergeResults(arg0, text, start) {
      const parseDurationResult = ENMergeRelativeFollowByDateRefiner(10779).parseDuration(text.text);
      let reverseDurationResult = parseDurationResult;
      if (null != str.match(/\s+(before|from)$/i)) {
        reverseDurationResult = tmp(10782).reverseDuration(parseDurationResult);
      }
      const ParsingComponents = tmp(10783).ParsingComponents;
      const ReferenceWithTimezone = tmp(10783).ReferenceWithTimezone;
      start = start.start;
      ...
      return new ENMergeRelativeFollowByDateRefiner(10783).ParsingResult(start.reference, text.index, "" + text.text + arg0 + start.text, relativeFromReference);
    }
  }
];
