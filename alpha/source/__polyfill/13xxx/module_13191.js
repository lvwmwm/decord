// Module ID: 13191
// Function ID: 13192
// Dependencies: [32, 41, 42, 13183, 13164, 13136, 13190, 13147, 13143, 13159, 13160, 13192, 13179, 13180, 13163, 13185, 13172, 13193, 13176, 13194, 13141]
    ...
    value: function _processEvent(type, arg1, arg2) {
      ...
      return self._prepareEvent(type, arg1, arg2, type.sdkProcessingMetadata || {}.capturedSpanIsolationScope).then((result) => {
        ...
        if (originalException instanceof BaseClient(13193).SentryError) {
          ...
          const _HermesInternal = HermesInternal;
          const sentryError = new tmp(13193).SentryError("Event processing pipeline threw an error, original event will not be sent. Details have been sent as a new event.\nReason: " + originalException);
          throw sentryError;
        }
        tmp = BaseClient;
      });
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(13164).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(13136).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tunnel = self._options.tunnel;
          if (tunnel) {
            tunnel = tmp(13183).dsnToString(self._dsn);
            const tmpResult2 = tmp(13183);
          }
          self.sendEnvelope(tmp(13194).createClientReportEnvelope(_clearOutcomesResult, tunnel));
          const tmpResult = tmp(13194);
        } else if (DEBUG_BUILD) {
