// Module ID: 13185
// Function ID: 13186
// Dependencies: [13147, 13150, 13163, 13186, 13187, 13173, 13145, 13188, 13181, 13157]
...
function applyClientOptions(environment, environment2) {
  ...
  if (environment.message) {
    environment.message = _mod13145.truncate(environment.message, num);
  }
  value = iter;
  ...
  if (value) {
    iter.value = _mod13145.truncate(iter.value, num);
  }
  ...
  if (url) {
    request.url = _mod13145.truncate(request.url, num);
  }
}
function applyDebugIds(exception, arg1) {
  const filenameToDebugIdMap = _mod13188.getFilenameToDebugIdMap(arg1);
  try {
