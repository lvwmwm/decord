// Module ID: 13109
// Function ID: 13110
// Dependencies: [32, 41, 42, 13101, 13082, 13054, 13108, 13065, 13061, 13077, 13078, 13110, 13097, 13098, 13081, 13103, 13090, 13111, 13094, 13112, 13059]
  ...
  },
  {
    ...
    value: function recordDroppedEvent(arg0, arg1, num) {
      ...
      if (this._options.sendClientReports) {
        ...
        if (BaseClient(13082).DEBUG_BUILD) {
          const logger = tmp6(13054).logger;
          let str3 = "";
          if (num2 > 1) {
    ...
    value: function _processEvent(type, arg1, arg2) {
      ...
      return self._prepareEvent(type, arg1, arg2, type.sdkProcessingMetadata || {}.capturedSpanIsolationScope).then((result) => {
        ...
        if (originalException instanceof BaseClient(13111).SentryError) {
          ...
          const _HermesInternal = HermesInternal;
          const sentryError = new tmp(13111).SentryError("Event processing pipeline threw an error, original event will not be sent. Details have been sent as a new event.\nReason: " + originalException);
          throw sentryError;
        }
        tmp = BaseClient;
      });
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(13082).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(13054).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tunnel = self._options.tunnel;
          if (tunnel) {
            tunnel = tmp(13101).dsnToString(self._dsn);
            const tmpResult2 = tmp(13101);
          }
          self.sendEnvelope(tmp(13112).createClientReportEnvelope(_clearOutcomesResult, tunnel));
