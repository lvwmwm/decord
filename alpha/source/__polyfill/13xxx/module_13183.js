// Module ID: 13183
// Function ID: 13184
// Dependencies: [13173, 13150, 13145]
...
export const logSpanEnd = function logSpanEnd(spanContext) {
  if (_mod13173.DEBUG_BUILD) {
    const spanToJSONResult = tmp(13150).spanToJSON(spanContext);
...
export const logSpanStart = function logSpanStart(spanContext) {
  if (_mod13173.DEBUG_BUILD) {
    ...
    const spanIsSampledResult = tmp(13150).spanIsSampled(spanContext);
    ...
    if (rootSpan !== spanContext) {
      const tmpResult6 = tmp(13150);
      ({ op: op2, description: description2 } = tmp(13150).spanToJSON(rootSpan));
      ...
      const spanToJSONResult1 = tmp(13150).spanToJSON(rootSpan);
    }
    const logger = tmp(13145).logger;
    const _HermesInternal9 = HermesInternal;
    logger.log("" + combined + "\n  " + items.join("\n  "));
    const tmpResult5 = tmp(13150);
  }
};
