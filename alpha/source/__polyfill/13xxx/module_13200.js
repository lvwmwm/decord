// Module ID: 13200
// Function ID: 13201
// Dependencies: [32, 41, 42, 13192, 13173, 13145, 13199, 13156, 13152, 13168, 13169, 13201, 13188, 13189, 13172, 13194, 13181, 13202, 13185, 13203, 13150]
    ...
    value: function _processEvent(type, arg1, arg2) {
      ...
      return self._prepareEvent(type, arg1, arg2, type.sdkProcessingMetadata || {}.capturedSpanIsolationScope).then((result) => {
        ...
        if (originalException instanceof BaseClient(13202).SentryError) {
          ...
          const _HermesInternal = HermesInternal;
          const sentryError = new tmp(13202).SentryError("Event processing pipeline threw an error, original event will not be sent. Details have been sent as a new event.\nReason: " + originalException);
          throw sentryError;
        }
        tmp = BaseClient;
      });
  ...
  },
  {
    ...
    value: function _flushOutcomes() {
      ...
      if (0 !== _clearOutcomesResult.length) {
        const DEBUG_BUILD = tmp(13173).DEBUG_BUILD;
        if (self._dsn) {
          if (DEBUG_BUILD) {
            const logger4 = tmp(13145).logger;
            logger4.log("Sending outcomes:", _clearOutcomesResult);
          }
          let tunnel = self._options.tunnel;
          if (tunnel) {
            tunnel = tmp(13192).dsnToString(self._dsn);
            const tmpResult2 = tmp(13192);
          }
          self.sendEnvelope(tmp(13203).createClientReportEnvelope(_clearOutcomesResult, tunnel));
          const tmpResult = tmp(13203);
        } else if (DEBUG_BUILD) {
