// Module ID: 13147
// Function ID: 13148
// Dependencies: [13137, 13145, 13142]
...
export const addContextToFrame = function addContextToFrame(arr, lineno) {
  ...
  if (undefined !== lineno.lineno) {
    ...
    lineno.pre_context = substr.map((item) => _mod13145.snipLine(item, 0));
    const _Math5 = Math;
    ...
    lineno.context_line = _mod13145.snipLine(arr[bound1], num2);
    const _Math = Math;
    const substr1 = arr.slice(Math.min(bound + 1, length), bound + 1 + num);
    lineno.post_context = substr1.map((item) => _mod13145.snipLine(item, 0));
  }
};
...
export const checkOrSetAlreadyCaught = function checkOrSetAlreadyCaught(__sentry_captured__) {
  if ((function isAlreadyCaptured(__sentry_captured__) {
    ...
    try {
      const result = _mod13142.addNonEnumerableProperty(__sentry_captured__, "__sentry_captured__", true);
      return false;
    } catch (err) {
    }
  }
