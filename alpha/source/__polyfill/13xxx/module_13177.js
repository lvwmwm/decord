// Module ID: 13177
// Function ID: 13178
// Dependencies: [41, 42, 13146, 13150, 13151, 13141, 13174, 13142, 13152, 13178, 13164, 13136, 13163, 13179, 13165, 13172]
  ...
  },
  {
    ...
    value: function getSpanJSON() {
      ...
      if (_isStandaloneSpan) {
        _isStandaloneSpan = tmp(13141).getRootSpan(self) === self;
        const tmpResult = tmp(13141);
      }
      obj6.is_segment = _isStandaloneSpan;
      let spanId;
      if (self._isStandaloneSpan) {
        const rootSpan = tmp(13141).getRootSpan(self);
  ...
  },
  {
    ...
    value: function addEvent(name, num, arg2) {
      ...
      if (!isArray) {
        ...
        if (!arg2) {
          timestampInSecondsResult = tmp(13150).timestampInSeconds();
    ...
    value: function _onSpanEnded() {
      ...
      if (self._isStandaloneSpan) {
        if (self._isStandaloneSpan) {
          if (self._sampled) {
            const items = [self];
            const spanEnvelope = tmp(13179).createSpanEnvelope(items, client);
            const tmpResult = tmp(13179);
            const client1 = tmp(13163).getClient();
            if (client1) {
              if (spanEnvelope[1]) {
                if (0 !== arr2.length) {
          ...
          if (result) {
            let scope = tmp(13165).getCapturedScopesOnSpan(self).scope;
            if (!scope) {
              scope = tmp(13163).getCurrentScope();
              const tmpResult7 = tmp(13163);
            }
            scope.captureEvent(result);
            const tmpResult6 = tmp(13165);
          }
        }
      } else {
