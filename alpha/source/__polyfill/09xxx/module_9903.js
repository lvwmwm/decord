...
const regExp = new RegExp("(?:on\\s{0,3})?(" + _mod9894.ORDINAL_NUMBER_PATTERN + ")(?:\\s{0,3}(?:to|\\-|\\\u2013|until|through|till)?\\s{0,3}(" + _mod9894.ORDINAL_NUMBER_PATTERN + "))?(?:-|/|\\s{0,3}(?:of)?\\s{0,3})(" + repeatedTimeunitPattern.matchAnyPattern(_mod9894.MONTH_DICTIONARY) + ")(?:(?:-|/|,?\\s{0,3})(" + _mod9894.YEAR_PATTERN + "(?!\\w)))?(?=\\W|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingResult, index) {
      ...
      const tmp4 = ENMonthNameLittleEndianParser(9894).MONTH_DICTIONARY[index[3].toLowerCase(index[3])];
      const result = ENMonthNameLittleEndianParser(9894).parseOrdinalNumberPattern(index[1]);
      if (result > 31) {
        ...
        if (index[4]) {
          ...
          start2.assign("year", tmp2(9894).parseYear(index[4]));
        } else {
