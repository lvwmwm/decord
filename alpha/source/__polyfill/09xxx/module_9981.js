// Module ID: 9981
// Function ID: 9982
// Dependencies: [41, 42, 93, 95, 98, 9884, 9978, 9885, 9891]
...
const regExp = new RegExp("(" + repeatedTimeunitPattern.matchAnyPattern(_mod9978.MONTH_DICTIONARY) + ")\\s*(?:[,-]?\\s*(" + _mod9978.YEAR_PATTERN + ")?)?(?=[^\\s\\w]|\\s+[^0-9]|\\s+$|$)", "i");
...
const items = [
  entry,
  {
    ...
    value: function innerExtract(createParsingComponents, arg1) {
      ...
      const tmp4 = NLMonthNameParser(9978).MONTH_DICTIONARY[arg1[1].toLowerCase(arg1[1])];
      parsingComponents.assign("month", tmp4);
      if (arg1[2]) {
        parsingComponents.assign("year", tmp2(9978).parseYear(arg1[2]));
      } else {
        parsingComponents.imply("year", tmp2(9885).findYearClosestToRef(createParsingComponents.refDate, 1, tmp4));
      }
