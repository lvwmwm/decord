// Module ID: 9973
// Function ID: 9974
// Dependencies: [41, 42, 93, 95, 98, 9967, 9968]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9968).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9967).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9968).exists(this);
      HashMD(9968).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9967).createView(content);
