// Module ID: 9891
// Function ID: 9892
// Dependencies: [41, 42, 9892, 9898]

// Module 9891
import ENDefaultConfiguration2 from "ENDefaultConfiguration" /* 9892 */;
...
const entry1 = {
  ...
  value: function executeParser(debug, pattern) {
    ...
    if (match) {
      ...
      if (extractResult instanceof ParsingContext(9898).ParsingResult) {
      ...
      } else if (!(extractResult instanceof tmp6(9898).ParsingComponents)) {
...
const entry2 = {
  ...
  value: function createParsingComponents(date) {
    ...
    if (!(date instanceof ParsingContext(9898).ParsingComponents)) {
      const self = this;
      parsingComponents = new ParsingContext(9898).ParsingComponents(this.reference, date);
    }
...
const items2 = [
  entry2,
  {
    ...
    value: function createParsingResult(sum, match, extractResult, date) {
      ...
      return new ParsingContext(9898).ParsingResult(self.reference, sum, substr, parsingComponents, parsingComponents1);
    }
  },
  {
    key: "debug",
