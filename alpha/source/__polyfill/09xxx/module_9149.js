// Module ID: 9149
// Function ID: 9150
// Dependencies: [41, 42, 93, 95, 98, 9143, 9144]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9144).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9143).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9144).exists(this);
      HashMD(9144).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9143).createView(content);
