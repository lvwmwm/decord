// Module ID: 9880
// Function ID: 9881
// Dependencies: [41, 42, 9881, 9887]

// Module 9880
import ENDefaultConfiguration2 from "ENDefaultConfiguration" /* 9881 */;
...
const entry1 = {
  ...
  value: function executeParser(debug, pattern) {
    ...
    if (match) {
      ...
      if (extractResult instanceof ParsingContext(9887).ParsingResult) {
      ...
      } else if (!(extractResult instanceof tmp6(9887).ParsingComponents)) {
...
const entry2 = {
  ...
  value: function createParsingComponents(date) {
    ...
    if (!(date instanceof ParsingContext(9887).ParsingComponents)) {
      const self = this;
      parsingComponents = new ParsingContext(9887).ParsingComponents(this.reference, date);
    }
...
const items2 = [
  entry2,
  {
    ...
    value: function createParsingResult(sum, match, extractResult, date) {
      ...
      return new ParsingContext(9887).ParsingResult(self.reference, sum, substr, parsingComponents, parsingComponents1);
    }
  },
  {
    key: "debug",
