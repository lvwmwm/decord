// Module ID: 9160
// Function ID: 9161
// Dependencies: [41, 42, 93, 95, 98, 9154, 9155]
// Exports: Chi, Maj
...
const entry = {
  ...
  value: function update(B) {
    let tmp8;
    const self = this;
    HashMD(9155).exists(this);
    ({ buffer, blockLen, view } = this);
    const toBytesResult = HashMD(9154).toBytes(B);
    let num = 0;
...
let items = [
  entry,
  {
    ...
    value: function digestInto(content) {
      const self = this;
      HashMD(9155).exists(this);
      HashMD(9155).output(content, this);
      this.finished = true;
      ...
      self.process(view, 0);
      const view1 = HashMD(9154).createView(content);
