// Module ID: 16843
// Function ID: 16844
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16844]
// Exports: captureScreen, ensureModuleIsLoaded

// Module 16843
import _modDef16844 from "module_16844" /* 16844 */;
...
function captureRef(current, options) {
  if (ViewShot(16844)) {
    ...
    return tmp(16844).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16844).releaseCapture(str);
    const obj = ViewShot(16844);
  }
}
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16844)) {
    ...
    return tmp(16844).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
