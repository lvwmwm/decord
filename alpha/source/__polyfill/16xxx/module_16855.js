// Module ID: 16855
// Function ID: 16856
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16856]
// Exports: captureScreen, ensureModuleIsLoaded

// Module 16855
import _modDef16856 from "module_16856" /* 16856 */;
...
function captureRef(current, options) {
  if (ViewShot(16856)) {
    ...
    return tmp(16856).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16856).releaseCapture(str);
    const obj = ViewShot(16856);
  }
}
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16856)) {
    ...
    return tmp(16856).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
