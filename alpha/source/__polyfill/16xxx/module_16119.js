// Module ID: 16119
// Function ID: 16120
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 16120]
...
function captureRef(current, options) {
  if (ViewShot(16120)) {
    ...
    return tmp(16120).captureRef(tmp15, options);
  } else {
    const _Error = Error;
...
function releaseCapture(str) {
  if (typeof str === "string") {
    ViewShot(16120).releaseCapture(str);
    const obj = ViewShot(16120);
  }
}
...
export const captureScreen = function captureScreen(options) {
  if (ViewShot(16120)) {
    ...
    return tmp(16120).captureScreen(options);
  } else {
    const _Error = Error;
    const error = new Error("react-native-view-shot: NativeModules.RNViewShot is undefined. Make sure the library is linked on the native side.");
    throw error;
