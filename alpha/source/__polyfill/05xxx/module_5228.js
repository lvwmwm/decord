// Module ID: 5228
// Function ID: 5229
// Dependencies: [109, 19, 17, 21, 5229, 5230, 5231, 5227, 5232, 5233, 5235, 5218, 5236]
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  if (undefined === enabled) {
    enabled = tmp2(5227).screensEnabled();
    const tmp2Result = tmp2(5227);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5227).freezeEnabled();
    const tmp2Result6 = tmp2(5227);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5232).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5227).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5232).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5232);
      const sheetLargestUndimmedDetent = tmp2(5232).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5232);
      ...
      const sheetInitialDetentIndex1 = tmp2(5232).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5235).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5236).Provider, obj7);
      }
