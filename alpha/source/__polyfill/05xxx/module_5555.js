// Module ID: 5555
// Function ID: 5556
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 5520, 5556, 5557, 5558]
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5557).addMissingNamespaces(tmp), true);
        }
      }
    ...
    for (const key10002 in arg0) {
      ...
      let item = arr2.forEach((attributes) => {
        ...
        if (typeof attributes.value === "object") {
          ParseError(5520).objectAssign(obj, parseNodeChildrenAsTags(attributes.value));
  ...
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      ...
      if (typeof calculateGPSValueDefault[tmp2] === "function") {
        tmp12 = tmp13(5558)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(5558);
      }
      tmp13 = importDefault;
    }
    return tmp12;
