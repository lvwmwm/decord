  ...
  read(byteLength, arg1) {
    ...
    for (let num = 0; num < arg1.length; num = num + 1) {
      ...
      if (stringFromDataView === PNG_CHUNK_TYPE_SIZE.TYPE_PHYS) {
        let tmp22 = arg1[num];
        let tmp23 = 4 <= longAt && tmp22 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 4 <= byteLength.byteLength;
        let tmp24;
        if (tmp23) {
          let tmpResult = tmp(5543);
          let longAt1 = tmpResult.getLongAt(byteLength, tmp22 + tmp3(5533).PNG_CHUNK_DATA_OFFSET);
          let obj4 = { value: longAt1, description: "" + longAt1 };
          tmp24 = obj4;
        }
        obj["Pixels Per Unit X"] = tmp24;
        let tmp26 = arg1[num];
        let tmp27 = 8 <= longAt && tmp26 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 4 + 4 <= byteLength.byteLength;
        let tmp28;
        if (tmp27) {
          let tmpResult9 = tmp(5543);
          let longAt2 = tmpResult9.getLongAt(byteLength, tmp26 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 4);
          let obj5 = { value: longAt2, description: "" + longAt2 };
          tmp28 = obj5;
        }
        obj["Pixels Per Unit Y"] = tmp28;
        let tmp30 = arg1[num];
        let tmp31 = 9 <= longAt && tmp30 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 8 + 1 <= byteLength.byteLength;
        let tmp32;
        if (tmp31) {
          let tmpResult10 = tmp(5543);
          let byteAt = tmpResult10.getByteAt(byteLength, tmp30 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 8);
        ...
        let tmp34 = arg1[num];
        let tmp8 = 7 <= longAt && tmp34 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 7 <= byteLength.byteLength;
        let tmp9;
        if (tmp8) {
          let tmpResult11 = tmp(5543);
          let shortAt = tmpResult11.getShortAt(byteLength, tmp34 + tmp3(5533).PNG_CHUNK_DATA_OFFSET);
          let tmpResult12 = tmp(5543);
          let byteAt1 = tmpResult12.getByteAt(byteLength, tmp34 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 2);
          let tmpResult13 = tmp(5543);
          let byteAt2 = tmpResult13.getByteAt(byteLength, tmp34 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 3);
          let tmpResult14 = tmp(5543);
          let byteAt3 = tmpResult14.getByteAt(byteLength, tmp34 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 4);
          let tmpResult15 = tmp(5543);
          let byteAt4 = tmpResult15.getByteAt(byteLength, tmp34 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 5);
          let tmpResult16 = tmp(5543);
          let byteAt5 = tmpResult16.getByteAt(byteLength, tmp34 + tmp3(5533).PNG_CHUNK_DATA_OFFSET + 6);
