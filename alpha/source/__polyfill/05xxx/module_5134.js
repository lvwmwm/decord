// Module ID: 5134
// Function ID: 5135
// Dependencies: [109, 19, 17, 21, 5135, 5136, 5137, 5133, 5138, 5139, 5141, 5124, 5142]
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  if (undefined === enabled) {
    enabled = tmp2(5133).screensEnabled();
    const tmp2Result = tmp2(5133);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5133).freezeEnabled();
    const tmp2Result6 = tmp2(5133);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5138).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5133).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5138).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5138);
      const sheetLargestUndimmedDetent = tmp2(5138).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5138);
      ...
      const sheetInitialDetentIndex1 = tmp2(5138).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5141).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5142).Provider, obj7);
      }
