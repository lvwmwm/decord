// Module ID: 5218
// Function ID: 5219
// Dependencies: [109, 19, 17, 21, 5219, 5220, 5221, 5217, 5222, 5223, 5225, 5208, 5226]
...
import animatedComponentDefault from "animatedComponent" /* 5220 */;
import _mod5221 from "module_5221" /* 5221 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 5223 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef5219);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod5221.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(5217).screensEnabled();
    const tmp2Result = tmp2(5217);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5217).freezeEnabled();
    const tmp2Result6 = tmp2(5217);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5222).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5217).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5222).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5222);
      const sheetLargestUndimmedDetent = tmp2(5222).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5222);
      ...
      const sheetInitialDetentIndex1 = tmp2(5222).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5225).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5226).Provider, obj7);
      }
