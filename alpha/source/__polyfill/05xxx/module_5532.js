...
export default {
  read(dataView, sum) {
    ...
    let tmp4;
    if (8 <= shortAt) {
      const byteAt = tmp(5533).getByteAt(dataView, sum + 7);
      const obj2 = { value: byteAt, description: "" + byteAt };
      tmp4 = obj2;
      const tmpResult = tmp(5533);
    }
    let tmp6;
    if (3 <= shortAt) {
      const byteAt1 = tmp(5533).getByteAt(dataView, sum + 2);
