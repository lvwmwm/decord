// Module ID: 5221
// Function ID: 5222
// Dependencies: [109, 19, 17, 21, 5222, 5223, 5224, 5220, 5225, 5226, 5228, 5211, 5229]

// Module 5221
import get_synchronousScreenUpdatesEnabledDefault from "get synchronousScreenUpdatesEnabled" /* 5211 */;
import _modDef5222 from "module_5222" /* 5222 */;
import animatedComponentDefault from "animatedComponent" /* 5223 */;
import _mod5224 from "module_5224" /* 5224 */;
import DelayedFreezeDefault from "DelayedFreeze" /* 5226 */;
...
const jsx = fn(21).jsx;
const __INTERNAL_VIEW_CONFIG = Animated.createAnimatedComponent(_modDef5222);
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  const previous = _mod5224.usePrevious(activityState.activityState);
  ...
  if (undefined === enabled) {
    enabled = tmp2(5220).screensEnabled();
    const tmp2Result = tmp2(5220);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5220).freezeEnabled();
    const tmp2Result6 = tmp2(5220);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5225).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5220).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5225).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5225);
      const sheetLargestUndimmedDetent = tmp2(5225).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5225);
      ...
      const sheetInitialDetentIndex1 = tmp2(5225).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5228).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5229).Provider, obj7);
      }
