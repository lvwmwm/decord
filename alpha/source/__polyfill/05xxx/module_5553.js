// Module ID: 5553
// Function ID: 5554
// Dependencies: [32, 42, 41, 93, 95, 98, 158, 5518, 5554, 5555, 5556]
  ...
  try {
    ...
    if ("ParseError" === tmp12.name) {
      if (obj2.isMissingNamespaceError(tmp12)) {
        if (!tmp4) {
          return parseFromString(tmp2, tmp13(5555).addMissingNamespaces(tmp), true);
        }
      }
    ...
    for (const key10002 in arg0) {
      ...
      let item = arr2.forEach((attributes) => {
        ...
        if (typeof attributes.value === "object") {
          ParseError(5518).objectAssign(obj, parseNodeChildrenAsTags(attributes.value));
  ...
  if (Array.isArray(arr)) {
    ...
    if (tmp2) {
      ...
      if (typeof calculateGPSValueDefault[tmp2] === "function") {
        tmp12 = tmp13(5556)[tmp2](arr, tmp11);
        const tmp13Result = tmp13(5556);
      }
      tmp13 = importDefault;
    }
    return tmp12;
