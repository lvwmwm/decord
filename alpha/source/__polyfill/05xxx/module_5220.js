// Module ID: 5220
// Function ID: 5221
// Dependencies: [109, 19, 17, 21, 5221, 5222, 5223, 5219, 5224, 5225, 5227, 5210, 5228]
...
const forwardRefResult = noop.forwardRef(function InnerScreen(activityState, ref) {
  ...
  if (undefined === enabled) {
    enabled = tmp2(5219).screensEnabled();
    const tmp2Result = tmp2(5219);
  }
  let freezeOnBlur = activityState.freezeOnBlur;
  if (undefined === freezeOnBlur) {
    freezeOnBlur = tmp2(5219).freezeEnabled();
    const tmp2Result6 = tmp2(5219);
  }
  ...
  if (undefined === SHEET_DIMMED_ALWAYS) {
    SHEET_DIMMED_ALWAYS = tmp2(5224).SHEET_DIMMED_ALWAYS;
  }
  ...
  if (enabled) {
    if (tmp2(5219).isNativePlatformSupported) {
      const sheetAllowedDetents1 = tmp2(5224).resolveSheetAllowedDetents(sheetAllowedDetents);
      const tmp2Result7 = tmp2(5224);
      const sheetLargestUndimmedDetent = tmp2(5224).resolveSheetLargestUndimmedDetent(SHEET_DIMMED_ALWAYS, sheetAllowedDetents1.length - 1);
      const tmp2Result8 = tmp2(5224);
      ...
      const sheetInitialDetentIndex1 = tmp2(5224).resolveSheetInitialDetentIndex(num3, sheetAllowedDetents1.length - 1);
      ...
      obj4.fullScreenSwipeEnabled = tmp2(5227).parseBooleanToOptionalBooleanNativeProp(fullScreenSwipeEnabled);
      let num7;
      ...
      if (isNativeStack) {
        ...
        tmp31Result = tmp31(tmp32(5228).Provider, obj7);
      }
