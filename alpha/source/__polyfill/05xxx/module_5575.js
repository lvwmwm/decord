...
export default {
  read(getUint8, sum) {
    const byteAt = _modDef5543.getByteAt(getUint8, sum);
    let num = 0;
    if (16 & byteAt) {
      num = 1;
    }
    ...
    obj3.Animation = obj4;
    sum = sum + c2;
    const byteAt1 = _modDef5543.getByteAt(getUint8, sum);
    const tmpResult = _modDef5543;
    const sum1 = byteAt1 + 256 * _modDef5543.getByteAt(getUint8, sum + 1);
    const tmpResult6 = _modDef5543;
    const sum2 = sum1 + 65536 * _modDef5543.getByteAt(getUint8, sum + 2) + 1;
    obj3.ImageWidth = { value: sum2, description: `${tmp9}px` };
    const sum3 = sum + c3;
    const obj5 = { value: sum2, description: `${tmp9}px` };
    const tmpResult7 = _modDef5543;
    const byteAt2 = _modDef5543.getByteAt(getUint8, sum3);
    const tmpResult8 = _modDef5543;
    const sum4 = byteAt2 + 256 * _modDef5543.getByteAt(getUint8, sum3 + 1);
    const tmpResult9 = _modDef5543;
    const sum5 = sum4 + 65536 * _modDef5543.getByteAt(getUint8, sum3 + 2) + 1;
