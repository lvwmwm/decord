...
export default function PremiumUpsellActionSheet(onDismiss) {
  ...
  if (initialUpsellKey == null) {
    initialUpsellKey = tmp2(tmp3[45]).getUpsellType(featureName);
    const tmp2Result = tmp2(tmp3[45]);
  }
  const premiumUpsellConfig = legacyProps(useTier0UpsellContent[44]).usePremiumUpsellConfig(initialUpsellKey, analyticsLocations2);
  ...
  if (!useTier0UpsellContent) {
    trialCtaOverride = tmp2(tmp3[47]).getTrialCtaOverride(tmp10, TIER_2.TIER_2);
    const tmp2Result17 = tmp2(tmp3[47]);
  }
  let obj2 = legacyProps(useTier0UpsellContent[44]);
  const tmp6 = analyticsLocations;
  const items1 = [ThemeStore];
  const stateFromStores1 = tmp2(tmp3[42]).useStateFromStores(items1, () => theme.theme);
  const tmp2Result18 = tmp2(tmp3[42]);
  const items2 = [SelectedGuildStore];
  const stateFromStores2 = tmp2(tmp3[42]).useStateFromStores(items2, () => guildId.getGuildId());
  ...
  obj8.illustration = closure_20(tmp2(tmp3[25]).StickersSpotIllustration, { width: 235, height: 132, accessible: false });
  ...
  obj14.image = analyticsLocations2(tmp3[26]);
  ...
  obj17.image = analyticsLocations2(tmp21 ? tmp3[32] : tmp3[33]);
  ...
  obj22.children = intl19.format(tmp2(tmp3[24]).t["1kFyto"], {
    premiumMax,
    onClick() {
      analyticsLocations2(useTier0UpsellContent[28]).hideActionSheet(legacyProps(useTier0UpsellContent[29]).PREMIUM_UPSELL_ACTION_SHEET_KEY);
      const obj = analyticsLocations2(useTier0UpsellContent[28]);
      const result = legacyProps(useTier0UpsellContent[34]).showScheduledMessagesModal();
    }
  });
  ...
  obj20.image = analyticsLocations2(tmp3[35]);
  ...
  obj24.image = analyticsLocations2(tmp3[36]);
  const obj26 = { colors: null, start: tmp2(tmp3[37]).HorizontalGradient.START, end: tmp2(tmp3[37]).HorizontalGradient.END };
  ...
  const stateFromStores3 = tmp2(tmp3[42]).useStateFromStores(items4, () => useReducedMotion.useReducedMotion);
  ...
  if (!mobileEmojiPickerUpsellRestyleEnabledForFeature) {
    mobileEmojiPickerUpsellRestyleEnabledForFeature = tmp2(tmp3[48]).getMobileStickerPickerUpsellRestyleEnabledForFeature(featureName, "native.PremiumUpsellActionSheet");
  ...
  const tmp36 = analyticsLocations2(tmp3[50])(useTier0UpsellContent, onViewAllPerks, obj3[featureName].analyticsPage, undefined, tmp6);
  ...
  if (null != obj3[featureName]) {
    ...
    if (true === tmp32.showBetaBadge) {
      const obj30 = { size: tmp2(tmp3[53]).BetaSizes.SMALL, gradient: true, style: tmp.betaTag };
      tmp28Result = tmp28(tmp5(tmp3[53]), obj30);
      const tmp5Result = tmp5(tmp3[53]);
    }
    ...
    items7[1] = tmp28(tmp2(tmp3[54]).Text, obj31);
    ...
    items7[2] = tmp28(tmp2(tmp3[54]).Text, obj32);
    ...
    obj34.variant = str3;
    const items9 = [tmp28(tmp2(tmp3[55]).Button, obj34), ];
