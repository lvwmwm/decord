...
// Dependencies: [17, 1074, 21, 4827, 576, 4671, 7781, 1177, 4823, 1115, 5271, 5375, 4794, 4840, 2108, 7481, 7480, 5269, 5995, 2]
// Exports: default

// Module 13804 (ReferralProgramShareConfirmationActionSheet)
...
function SharedUser(user) {
  ...
  const tmp6 = user.trialCreationResult === user(7781).CreateReferralStatus.FAIL;
  ...
  if (tmp6) {
    ...
    const items2 = [tmp9(tmp5(4823).Text, obj5), ];
    ...
    items2[1] = tmp9(tmp5(4823).Text, obj6);
    ...
    tmp9Result = tmp9(tmp5(4823).Text, obj8);
  }
  items1[1] = tmp9Result;
  ...
  obj9.icon = closure_5(user(5375).ChatIcon, { size: "xs", color: nativeDefault.colors.CONTROL_SECONDARY_TEXT_DEFAULT });
  obj9.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.hideActionSheet();
    ChannelActionCreatorsDefault.openPrivateChannel({ recipientIds: user.id });
  };
  items1[2] = closure_5(user(5271).Button, obj9);
...
export default function ReferralProgramShareConfirmationActionSheet(trialCreationResult) {
  ...
  const obj3 = { startExpanded: true, contentStyles: tmp.content, header: closure_5(tmp5(7480).BottomSheetTitleHeader, { title: null }), children: null };
  const obj4 = { children: null };
  const formatResult = intl3.format(tmp5(1115).t.AwGSWl, obj);
  const items = [closure_5(View, { style: tmp.headerAsset, children: closure_5(tmp5(5995).FistBumpSpotIllustration, {}) }), closure_5(tmp5(4823).Text, { variant: "heading-lg/bold", color: "mobile-text-heading-primary", style: tmp.header, children: stringResult }), closure_5(tmp5(4823).Text, { variant: "text-md/medium", color: "text-default", style: tmp.subheader, children: formatResult }), ];
  const obj8 = { style: tmp.recipientContainer, children: null };
  const obj5 = { style: tmp.headerAsset, children: closure_5(tmp5(5995).FistBumpSpotIllustration, {}) };
  ...
  obj4.children = items;
  obj3.children = closure_7(tmp5(5269).Stack, obj4);
  return closure_5(tmp5(7481).BottomSheet, obj3);
};
