...
// Dependencies: [19, 17, 7697, 13711, 1074, 1076, 21, 4757, 576, 1177, 4753, 13712, 13713, 1115, 2108, 6573, 6159, 7698, 504, 7409, 7429, 13714, 1241, 4724, 13715, 1980, 7786, 13722, 13723, 13726, 5187, 2]
// Exports: ReferralProgramPerkCard

// Module 13710 (ReferralProgramPerkCard)
...
export const ReferralProgramPerkCard = function ReferralProgramPerkCard() {
  ...
  let callback = noop.useCallback(() => {
    ...
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(13715, dependencyMap.paths), "referral-program-share-action-sheet");
  }, items2);
  ...
  const item = stateFromStores.forEach((item) => {
    ...
    } else if (item === tmp(7698).ReferralOfferStatus.CONVERTED) {
      closure_1 = closure_1 + 1;
      closure_2 = closure_2 + 1;
    } else if (item === tmp(7698).ReferralOfferStatus.REDEEMED) {
      closure_2 = closure_2 + 1;
    }
  });
  ...
  const shouldShowSpendOrbsCta = tmp2(13722).getShouldShowSpendOrbsCta(obj5, tmp11);
  ...
  if (!useAltReferralCardArt) {
    ...
    if (tmp11 === tmp2(13713).ReferralRewardType.ORBS) {
      const intl4 = tmp2(1115).intl;
      let stringResult = intl4.string(tmp2(1115).t.tAlkl4);
    } else if (tmp11 === tmp2(13713).ReferralRewardType.DISCOUNT) {
    ...
    const items5 = [tmp17(tmp2(4753).Text, obj11), , ];
    ...
    if (null != tmp11) {
      if (obj5.numRewardGranted !== tmp2(13712).MAX_REFERRALS_SENT) {
        if (obj5.numSent === tmp2(13712).MAX_REFERRALS_SENT) {
          if (tmp11 === tmp2(13713).ReferralRewardType.ORBS) {
      ...
      if (obj5.numRedeemed === tmp2(13712).MAX_REFERRALS_SENT) {
      ...
      items5[1] = tmp17(tmp2(4753).Text, obj13);
      if (isEligibleForIncentive) {
        const obj25 = { nRewardsGranted: obj5.numRewardGranted, referralRewardType: tmp11 };
        isEligibleForIncentive = tmp17(tmp5(13726), obj25);
      }
      ...
      if (!shouldShowSpendOrbsCta) {
        if (tmp24) {
          tmp24 = obj5.numSent !== tmp2(13712).MAX_REFERRALS_SENT;
        }
        tmp38 = tmp24;
      }
      ...
      obj26.children = tmp17(tmp2(5187).Button, obj27);
      items4[4] = tmp17(tmp14, obj26);
      obj6.children = items4;
      return tmp13(tmp14, obj6);
    }
    const tmp5Result = tmp5(2108);
  } else if (tmp11 === tmp2(13713).ReferralRewardType.ORBS) {
    let tmp5Result2 = tmp5(6573);
  } else if (tmp11 === tmp2(13713).ReferralRewardType.DISCOUNT) {
    tmp5Result2 = tmp5(6159);
  }
};
