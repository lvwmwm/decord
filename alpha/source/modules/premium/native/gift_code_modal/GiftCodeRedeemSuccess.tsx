...
export default function GiftCodeRedeemSuccess(giftCode) {
  ...
  if (type === giftCode(1974).CollectiblesItemType.BUNDLE) {
    ...
    if (null == stateFromStores) {
      ...
      let tmp31 = closure_8(tmp2(4832).Text, obj16);
    } else {
      if (tmp2Result8.isGameItemSKU(stateFromStores)) {
        ...
        tmp31 = closure_8(tmp2(4832).Text, obj17);
      } else {
        if (giftCode.isSubscription) {
          if (null != getOrFetchSubscriptionPlan) {
            ...
            tmp31 = closure_8(tmp2(4832).Text, obj18);
          }
        }
        if (null != first) {
    ...
    if (tmp2Result9.isGameItemSKU(stateFromStores)) {
      if (null != getOrFetchApplication) {
        ...
        let tmp36Result = tmp42(tmp2(4832).Text, obj21);
        let tmp36 = tmp42;
      }
  ...
  tmp18 = closure_6;
  const match = giftCode(5021).match(first);
  const str = giftCode(5021);
