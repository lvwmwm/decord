...
// Dependencies: [32, 502, 12023, 11, 12011, 1115, 2968, 2]
// Exports: default
...
export default function getRequestToStreamCTAAndIsDisabled(id) {
  ...
  if (tmp9) {
    ...
    text = stringResult;
    if (!tmp3[0]) {
      if (tmp(12023).StreamRequestUnfulfillableReason.ALREADY_STREAMING === tmp4) {
        const intl4 = tmp(1115).intl;
        text = intl4.string(tmp6(2968).P0wwmM);
        isDisabled = true;
      } else if (tmp(12023).StreamRequestUnfulfillableReason.NOT_RUNNING_GAME === tmp4) {
        const intl3 = tmp(1115).intl;
        text = intl3.string(tmp6(2968)["43zohO"]);
        isDisabled = true;
      } else if (tmp(12023).StreamRequestUnfulfillableReason.NOT_IN_VOICE_CHANNEL === tmp4) {
        ...
        isDisabled = false;
        text = stringResult;
        if (tmp(12023).StreamRequestUnfulfillableReason.NO_PERMISSION === tmp4) {
