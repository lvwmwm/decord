...
// Function ID: 17678
// Name: VoicePanelCard
// Dependencies: [32, 19, 17, 4849, 4850, 5722, 12606, 12604, 17633, 12609, 1074, 4848, 12607, 21, 4559, 4823, 5283, 1177, 4827, 576, 4969, 4879, 5892, 1115, 5271, 12605, 504, 9763, 4882, 13418, 9766, 9762, 17678, 9773, 9183, 8595, 4828, 5270, 7404, 5890, 4524, 17679, 11341, 9743, 4533, 7493, 17636, 17649, 17632, 17680, 17681, 8523, 6983, 17682, 12608, 17683, 17684, 17685, 17686, 17687, 17697, 2]

// Module 17677 (VoicePanelCard)
...
import computeCardBorderRadiusDefault from "computeCardBorderRadius" /* 17679 */;
import calculateContentCenterOffsetDefault from "calculateContentCenterOffset" /* 17680 */;
  ...
  const fn4 = function w(mode, transitionState) {
    if (!obj.cheapWorkletShallowEqual(mode, transitionState)) {
      ...
      if (null == transitionState) {
        if (transitionState !== tmp(4533).TransitionStates.YEETED) {
      ...
      if (transitionState === tmp(4533).TransitionStates.YEETED) {
        ...
        tmp(4559).runOnJS(isRTCConnected)();
        ...
        if (transitionState1 === tmp(4533).TransitionStates.YEETED) {
...
let closure_23 = ReanimatedRexport.createAnimatedComponent(fn(4823).Text);
let ReanimatedRexport = ReanimatedRexport_mod;
...
layoutTransitionFunction.__closure = { withSpring: fn(5270).withSpring };
...
let obj9 = { withSpring: fn(5270).withSpring };
size = fn(2);
...
export default noop.memo(function VoicePanelCard(cleanUp) {
  ...
  const obj13 = { isRinging: tmp8, avatarURI: "r", avatarDecoration: "padding", layout: layoutTransition, layoutPhysics: physics };
