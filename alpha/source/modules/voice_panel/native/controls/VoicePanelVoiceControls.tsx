...
// Dependencies: [19, 17, 2045, 11753, 21, 4836, 576, 4566, 11754, 16857, 8800, 2021, 16922, 9437, 13338, 5279, 5999, 17030, 1115, 9448, 504, 11585, 5266, 6073, 1610, 5901, 6544, 11764, 2]
...
import MobileGoLiveEntrypointExperimentDefault from "MobileGoLiveEntrypointExperiment" /* 16922 */;
...
let closure_10 = noop.memo((arg0) => {
  ...
  if (!tmp2) {
    ...
    items[1] = hasOwnProperty(tmp3(17030).GameConsoles, obj4);
    if (nonContextualStreamOutputPresent) {
      nonContextualStreamOutputPresent = tmp12(tmp3(17030).StreamVolumeItem, {});
    }
    ...
    const items1 = [hasOwnProperty(tmp3(17030).DeafenSwitch, {}), , , , ];
    const obj6 = { channel, connected: tmp2 };
    items1[1] = hasOwnProperty(tmp3(17030).AudioRouteButton, obj6);
    const obj7 = { channelId };
    items1[2] = hasOwnProperty(tmp3(17030).HideNonVideoParticipants, obj7);
    items1[3] = hasOwnProperty(tmp3(17030).HideSelfVideo, {});
    let tmp12Result = tmp2;
    if (tmp2) {
      const obj8 = { channel, connected: tmp2 };
      tmp12Result = tmp12(tmp3(17030).InviteButton, obj8);
    }
    ...
    const items2 = [hasOwnProperty(tmp3(17030).VoiceSettingsButton, obj9), ];
    let tmp12Result2 = null != tmp6;
    if (tmp12Result2) {
      const obj10 = { stream: tmp6 };
      tmp12Result2 = tmp12(tmp3(17030).ReportStreamIssueButton, obj10);
    }
    ...
    if (tmp2) {
      ...
      if (setting) {
        ...
        if (embeddedActivityLaunchability === tmp3(8800).EmbeddedActivityLaunchability.CAN_LAUNCH) {
          ...
          const items3 = [tmp12(tmp3(17030).LeaveActivitiesButton, {}), tmp12(tmp3(17030).ShareActivityLogsButton, {}), tmp12(tmp3(17030).ToggleShowActivitiesDebugOverlay, {})];
    ...
    const obj14 = { openTab };
    const items4 = [hasOwnProperty(tmp3(17030).ActivitiesButton, obj14), ];
    if (tmp3(16922).MobileGoLiveEntrypointTreatment.SCREENSHARE_REPLACES_CHAT === treatment) {
      const obj15 = { openTab };
      let tmp9Result = tmp9(tmp3(17030).ChatButton, obj15);
      const obj16 = { hasIcons: true, children: null };
      items4[1] = tmp9Result;
      obj16.children = items4;
      tmp7(tmp3(5999).TableRowGroup, obj16);
    } else if (tmp3(16922).MobileGoLiveEntrypointTreatment.SCREENSHARE_REPLACES_SOUNDBOARD !== treatment) {
      const obj17 = { channel };
      tmp9Result = tmp9(tmp3(17030).ScreenshareButton, obj17);
    }
    const obj18 = { channel };
    tmp9Result = tmp9(tmp3(17030).SoundboardButton, obj18);
  }
});
