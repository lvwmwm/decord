...
// Dependencies: [19, 1074, 1181, 21, 4827, 576, 4676, 12474, 12390, 5892, 17693, 1879, 9655, 7851, 9819, 9209, 5425, 4533, 17692, 12419, 1177, 17696, 17694, 4979, 13117, 4823, 12479, 1115, 2]
// Exports: default

// Module 17695 (ActivityShelfItem)
...
import useLaunchingActivityButtonStateDefault from "useLaunchingActivityButtonState" /* 12474 */;
import _modDef13117 from "module_13117" /* 13117 */;
import ActivityShelfItemBackgroundDefault from "ActivityShelfItemBackground" /* 17692 */;
import ActivityShelfItemSummaryDefault from "ActivityShelfItemSummary" /* 17693 */;
...
function ActivityActionOverlay(arg0) {
  ...
  if (useActivityShelfItem.ActivityAction.JOIN !== action) {
    if (tmp4(12390).ActivityAction.LEAVE !== action) {
      return null;
    }
  }
  let tmp8 = action === tmp4(12390).ActivityAction.LEAVE;
  if (tmp8) {
    const obj = { style: tmp3.ongoingActivityJoinedContainer };
    tmp8 = timestampProducer(tmp(5892), obj);
  }
  const items = [tmp8, ];
  let id1;
...
function ParticipantsText(arg0) {
  ...
  const items1 = [timestampProducer(native.Icon, { source: _modDef13117, size: native.Icon.Sizes.EXTRA_SMALL, color: "white" }), ];
  ...
  if (action === useActivityShelfItem.ActivityAction.START) {
    ...
    let itemSubtitleForMaxPlayersShort = tmp9(12479).getItemSubtitleForMaxPlayersShort(num2);
    const tmp9Result = tmp9(12479);
  } else {
...
export default function ActivityShelfItem(arg0) {
  ...
  if (!disableBadges) {
    const items = [tmp8(12390).ActivityAction.LEAVE, tmp8(12390).ActivityAction.JOIN];
  ...
  if (tmp11) {
    ...
    tmp15Result = tmp15(tmp3(12419), obj8);
  }
  ...
  if (tmp11) {
    ...
    if (isTestModeForApplication) {
      ...
      const obj10 = { size: tmp8(1177).Icon.Sizes.REFRESH_SMALL_16, source: tmp3(17696), color: tmp.developerIconColor.color };
      obj9.children = tmp15(tmp8(1177).Icon, obj10);
      tmp15Result3 = tmp15(tmp3(5892), obj9);
      const tmp3Result4 = tmp3(5892);
    }
  }
  items3[2] = tmp15Result3;
  obj4.children = items3;
  const items4 = [React6(native2.ThemeContextProvider, obj4), ];
  let tmp15Result4 = activityAction === tmp8(12390).ActivityAction.START;
  if (tmp15Result4) {
