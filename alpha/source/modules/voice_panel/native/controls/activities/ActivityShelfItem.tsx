...
// Dependencies: [19, 1074, 1181, 21, 4757, 576, 4606, 12398, 12314, 5808, 17624, 1879, 9582, 7768, 9746, 9138, 5341, 4467, 17623, 12343, 1177, 17627, 17625, 4909, 13035, 4753, 12403, 1115, 2]
// Exports: default

// Module 17626 (ActivityShelfItem)
...
import useLaunchingActivityButtonStateDefault from "useLaunchingActivityButtonState" /* 12398 */;
import _modDef13035 from "module_13035" /* 13035 */;
import ActivityShelfItemBackgroundDefault from "ActivityShelfItemBackground" /* 17623 */;
import ActivityShelfItemSummaryDefault from "ActivityShelfItemSummary" /* 17624 */;
...
function ActivityActionOverlay(arg0) {
  ...
  if (useActivityShelfItem.ActivityAction.JOIN !== action) {
    if (tmp4(12314).ActivityAction.LEAVE !== action) {
      return null;
    }
  }
  let tmp8 = action === tmp4(12314).ActivityAction.LEAVE;
  if (tmp8) {
    const obj = { style: tmp3.ongoingActivityJoinedContainer };
    tmp8 = timestampProducer(tmp(5808), obj);
  }
  const items = [tmp8, ];
  let id1;
...
function ParticipantsText(arg0) {
  ...
  const items1 = [timestampProducer(native.Icon, { source: _modDef13035, size: native.Icon.Sizes.EXTRA_SMALL, color: "white" }), ];
  ...
  if (action === useActivityShelfItem.ActivityAction.START) {
    ...
    let itemSubtitleForMaxPlayersShort = tmp9(12403).getItemSubtitleForMaxPlayersShort(num2);
    const tmp9Result = tmp9(12403);
  } else {
...
export default function ActivityShelfItem(arg0) {
  ...
  if (!disableBadges) {
    const items = [tmp8(12314).ActivityAction.LEAVE, tmp8(12314).ActivityAction.JOIN];
  ...
  if (tmp11) {
    ...
    tmp15Result = tmp15(tmp3(12343), obj8);
  }
  ...
  if (tmp11) {
    ...
    if (isTestModeForApplication) {
      ...
      const obj10 = { size: tmp8(1177).Icon.Sizes.REFRESH_SMALL_16, source: tmp3(17627), color: tmp.developerIconColor.color };
      obj9.children = tmp15(tmp8(1177).Icon, obj10);
      tmp15Result3 = tmp15(tmp3(5808), obj9);
      const tmp3Result4 = tmp3(5808);
    }
  }
  items3[2] = tmp15Result3;
  obj4.children = items3;
  const items4 = [React6(native2.ThemeContextProvider, obj4), ];
  let tmp15Result4 = activityAction === tmp8(12314).ActivityAction.START;
  if (tmp15Result4) {
