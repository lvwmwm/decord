...
// Dependencies: [19, 1074, 1181, 21, 4829, 576, 4680, 11609, 11525, 5896, 16940, 1879, 8757, 6938, 8922, 8311, 5428, 4537, 16939, 11554, 1177, 16943, 16941, 4981, 12276, 4825, 11614, 1115, 2]
// Exports: default
...
function ActivityActionOverlay(arg0) {
  ...
  if (useActivityShelfItem.ActivityAction.JOIN !== action) {
    if (tmp4(11525).ActivityAction.LEAVE !== action) {
      return null;
    }
  }
  let tmp8 = action === tmp4(11525).ActivityAction.LEAVE;
  if (tmp8) {
    const obj = { style: tmp3.ongoingActivityJoinedContainer };
    tmp8 = timestampProducer(tmp(5896), obj);
  }
  const items = [tmp8, ];
  let id1;
...
function ParticipantsText(arg0) {
  ...
  if (action === useActivityShelfItem.ActivityAction.START) {
    ...
    let itemSubtitleForMaxPlayersShort = tmp9(11614).getItemSubtitleForMaxPlayersShort(num2);
    const tmp9Result = tmp9(11614);
  } else {
...
export default function ActivityShelfItem(arg0) {
  ...
  if (!disableBadges) {
    const items = [tmp8(11525).ActivityAction.LEAVE, tmp8(11525).ActivityAction.JOIN];
  ...
  if (tmp11) {
    ...
    tmp15Result = tmp15(tmp3(11554), obj8);
  }
  ...
  if (tmp11) {
    ...
    if (isTestModeForApplication) {
      ...
      const obj10 = { size: tmp8(1177).Icon.Sizes.REFRESH_SMALL_16, source: tmp3(16943), color: tmp.developerIconColor.color };
      obj9.children = tmp15(tmp8(1177).Icon, obj10);
      tmp15Result3 = tmp15(tmp3(5896), obj9);
      const tmp3Result4 = tmp3(5896);
    }
  }
  items3[2] = tmp15Result3;
  obj4.children = items3;
  const items4 = [React6(native2.ThemeContextProvider, obj4), ];
  let tmp15Result4 = activityAction === tmp8(11525).ActivityAction.START;
  if (tmp15Result4) {
