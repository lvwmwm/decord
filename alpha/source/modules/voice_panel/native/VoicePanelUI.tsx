...
// Function ID: 17661
// Name: VoicePanelUI
// Dependencies: [32, 19, 17, 4845, 8640, 5037, 12615, 12613, 4850, 12616, 21, 3, 5272, 4829, 576, 4561, 1609, 12614, 1612, 6985, 9747, 11703, 17661, 17662, 4797, 17655, 4526, 12619, 9851, 17659, 17646, 11347, 9780, 5172, 7406, 7495, 7515, 1248, 17663, 7489, 17664, 17665, 9655, 17666, 17668, 17699, 17728, 17732, 17737, 2]
      ...
      if (modeToSet !== VoicePanelModes.PIP) {
        ...
        if (modeToSet === tmp8.PANEL) {
          if (connected) {
            ...
            const maxPanelWidth = tmp(17646).getMaxPanelWidth(obj3);
            const tmpResult = tmp(17646);
            const panelX = tmp(17646).getPanelX(windowWidth, maxPanelWidth);
            const _Math = Math;
            const tmpResult2 = tmp(17646);
...
export default noop.memo(function VoicePanelUI() {
      ...
      if (mode.connected) {
        if (!mode.gestureActive) {
          if (mode === VoicePanelModes.PANEL) {
            tmp2(4561).runOnJS(closure_2)(true);
            const tmp2Result = tmp2(4561);
          }
          if (mode === VoicePanelModes.PANEL) {
            tmp2(4561).runOnJS(_slicedToArray)(true);
            const tmp2Result6 = tmp2(4561);
          } else {
            tmp2(4561).runOnJS(_slicedToArray)(false);
            const tmp2Result7 = tmp2(4561);
          }
          if (mode === tmp9.PIP) {
            tmp2(4561).runOnJS(noop)(true);
            const tmp2Result8 = tmp2(4561);
          } else {
            tmp2(4561).runOnJS(noop)(false);
            const tmp2Result9 = tmp2(4561);
          }
          tmp9 = VoicePanelModes;
        }
      }
      tmp2(4561).runOnJS(closure_2)(false);
      const tmp2Result10 = tmp2(4561);
    }
  };
          ...
          if (velocityY > 0) {
            if (connected.get()) {
              if (gestureState.get().requiresPop) {
                ...
                channelId(4561).runOnJS(closure_1_15)(tmp2.PIP);
                closure_1(11703)(wrapperOffset, { gestureActive: false, x: 0, y: 0 });
                obj4 = channelId(4561);
              }
            } else {
              ...
              if (wrapperOffset.get().y > 0.2 * diff) {
                closure_1(11703)(tmp12, { gestureActive: false });
                channelId(4561).runOnJS(dismissPanel)();
                const obj3 = channelId(4561);
              } else {
                closure_1(11703)(tmp12, { gestureActive: false, x: 0, y: 0 });
  ...
  if (tmp6[0]) {
    ...
    const items8 = [closure_21(tmp13(7489).LayerScope, obj28), ];
    ...
    obj31.children = closure_21(tmp13(6985).GestureDetector, obj32);
    obj30.children = closure_21(tmp2(7406), obj31);
    items8[1] = closure_21(tmp13(6985).GestureDetector, obj30);
    ...
    items6[2] = closure_21(tmp13(6985).GestureDetector, obj25);
    obj23.children = items6;
    obj22.children = lockScrolling(tmp13(7489).LayerScope, obj23);
    tmp37 = closure_21(tmp13(7495).AnalyticsLocationProvider, obj22);
