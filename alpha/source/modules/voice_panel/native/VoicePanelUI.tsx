...
// Function ID: 17570
// Name: VoicePanelUI
// Dependencies: [32, 19, 17, 4773, 8565, 4965, 12530, 12528, 4778, 12531, 21, 3, 5186, 4757, 576, 4493, 1609, 12529, 1612, 6899, 9670, 11621, 17570, 4725, 17564, 4458, 12534, 9773, 17568, 17555, 11265, 9703, 5086, 7320, 7409, 7429, 1248, 7403, 17571, 17572, 9578, 17573, 17575, 17607, 17636, 17640, 17645, 2]
      ...
      if (modeToSet !== VoicePanelModes.PIP) {
        ...
        if (modeToSet === tmp8.PANEL) {
          if (connected) {
            ...
            const maxPanelWidth = tmp(17555).getMaxPanelWidth(obj3);
            const tmpResult = tmp(17555);
            const panelX = tmp(17555).getPanelX(windowWidth, maxPanelWidth);
            const _Math = Math;
            const tmpResult2 = tmp(17555);
...
export default noop.memo(function VoicePanelUI() {
      ...
      if (mode.connected) {
        if (!mode.gestureActive) {
          if (mode === VoicePanelModes.PANEL) {
            tmp2(4493).runOnJS(closure_2)(true);
            const tmp2Result = tmp2(4493);
          }
          if (mode === VoicePanelModes.PANEL) {
            tmp2(4493).runOnJS(_slicedToArray)(true);
            const tmp2Result6 = tmp2(4493);
          } else {
            tmp2(4493).runOnJS(_slicedToArray)(false);
            const tmp2Result7 = tmp2(4493);
          }
          if (mode === tmp9.PIP) {
            tmp2(4493).runOnJS(noop)(true);
            const tmp2Result8 = tmp2(4493);
          } else {
            tmp2(4493).runOnJS(noop)(false);
            const tmp2Result9 = tmp2(4493);
          }
          tmp9 = VoicePanelModes;
        }
      }
      tmp2(4493).runOnJS(closure_2)(false);
      const tmp2Result10 = tmp2(4493);
    }
  };
          ...
          if (velocityY > 0) {
            if (connected.get()) {
              if (gestureState.get().requiresPop) {
                ...
                channelId(4493).runOnJS(closure_1_15)(tmp2.PIP);
                closure_1(11621)(wrapperOffset, { gestureActive: false, x: 0, y: 0 });
                obj4 = channelId(4493);
              }
            } else {
              ...
              if (wrapperOffset.get().y > 0.2 * diff) {
                closure_1(11621)(tmp12, { gestureActive: false });
                channelId(4493).runOnJS(dismissPanel)();
                const obj3 = channelId(4493);
              } else {
                closure_1(11621)(tmp12, { gestureActive: false, x: 0, y: 0 });
  ...
  if (tmp6[0]) {
    ...
    const items8 = [closure_21(tmp13(7403).LayerScope, obj27), ];
    ...
    obj30.children = closure_21(tmp13(6899).GestureDetector, obj31);
    obj29.children = closure_21(tmp2(7320), obj30);
    items8[1] = closure_21(tmp13(6899).GestureDetector, obj29);
    ...
    obj22.children = lockScrolling(tmp13(7403).LayerScope, obj23);
    tmp35 = closure_21(tmp13(7409).AnalyticsLocationProvider, obj22);
