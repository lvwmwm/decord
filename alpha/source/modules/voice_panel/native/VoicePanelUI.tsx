...
// Function ID: 16885
// Name: VoicePanelUI
// Dependencies: [32, 19, 17, 4845, 7730, 5037, 11741, 11739, 4850, 11742, 21, 3, 5273, 4829, 576, 4563, 1609, 11740, 1612, 6068, 8845, 10883, 16885, 16886, 4797, 16879, 4528, 11745, 8949, 16883, 16870, 10445, 8878, 5173, 6489, 6578, 6598, 1248, 16887, 6572, 16888, 16889, 8753, 16890, 16892, 16923, 16952, 16956, 16961, 2]
      ...
      if (modeToSet !== VoicePanelModes.PIP) {
        ...
        if (modeToSet === tmp8.PANEL) {
          if (connected) {
            ...
            const maxPanelWidth = tmp(16870).getMaxPanelWidth(obj3);
            const tmpResult = tmp(16870);
            const panelX = tmp(16870).getPanelX(windowWidth, maxPanelWidth);
            const _Math = Math;
            const tmpResult2 = tmp(16870);
...
export default noop.memo(function VoicePanelUI() {
      ...
      if (mode.connected) {
        if (!mode.gestureActive) {
          if (mode === VoicePanelModes.PANEL) {
            tmp2(4563).runOnJS(closure_2)(true);
            const tmp2Result = tmp2(4563);
          }
          if (mode === VoicePanelModes.PANEL) {
            tmp2(4563).runOnJS(_slicedToArray)(true);
            const tmp2Result6 = tmp2(4563);
          } else {
            tmp2(4563).runOnJS(_slicedToArray)(false);
            const tmp2Result7 = tmp2(4563);
          }
          if (mode === tmp9.PIP) {
            tmp2(4563).runOnJS(noop)(true);
            const tmp2Result8 = tmp2(4563);
          } else {
            tmp2(4563).runOnJS(noop)(false);
            const tmp2Result9 = tmp2(4563);
          }
          tmp9 = VoicePanelModes;
        }
      }
      tmp2(4563).runOnJS(closure_2)(false);
      const tmp2Result10 = tmp2(4563);
    }
  };
          ...
          if (velocityY > 0) {
            if (connected.get()) {
              if (gestureState.get().requiresPop) {
                ...
                channelId(4563).runOnJS(closure_1_15)(tmp2.PIP);
                closure_1(10883)(wrapperOffset, { gestureActive: false, x: 0, y: 0 });
                obj4 = channelId(4563);
              }
            } else {
              ...
              if (wrapperOffset.get().y > 0.2 * diff) {
                closure_1(10883)(tmp12, { gestureActive: false });
                channelId(4563).runOnJS(dismissPanel)();
                const obj3 = channelId(4563);
              } else {
                closure_1(10883)(tmp12, { gestureActive: false, x: 0, y: 0 });
  ...
  if (tmp6[0]) {
    ...
    const items8 = [closure_21(tmp13(6572).LayerScope, obj28), ];
    ...
    obj31.children = closure_21(tmp13(6068).GestureDetector, obj32);
    obj30.children = closure_21(tmp2(6489), obj31);
    items8[1] = closure_21(tmp13(6068).GestureDetector, obj30);
    ...
    items6[2] = closure_21(tmp13(6068).GestureDetector, obj25);
    obj23.children = items6;
    obj22.children = lockScrolling(tmp13(6572).LayerScope, obj23);
    tmp37 = closure_21(tmp13(6578).AnalyticsLocationProvider, obj22);
