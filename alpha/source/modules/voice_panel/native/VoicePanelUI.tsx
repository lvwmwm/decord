...
// Function ID: 17638
// Name: VoicePanelUI
// Dependencies: [32, 19, 17, 4843, 8636, 5035, 12606, 12604, 4848, 12607, 21, 3, 5270, 4827, 576, 4559, 1609, 12605, 1612, 6983, 9743, 11697, 17638, 17639, 4795, 17632, 4524, 12610, 9846, 17636, 17623, 11341, 9776, 5170, 7404, 7493, 7513, 1248, 17640, 7487, 17641, 17642, 9651, 17643, 17645, 17676, 17705, 17709, 17714, 2]
      ...
      if (modeToSet !== VoicePanelModes.PIP) {
        ...
        if (modeToSet === tmp8.PANEL) {
          if (connected) {
            ...
            const maxPanelWidth = tmp(17623).getMaxPanelWidth(obj3);
            const tmpResult = tmp(17623);
            const panelX = tmp(17623).getPanelX(windowWidth, maxPanelWidth);
            const _Math = Math;
            const tmpResult2 = tmp(17623);
...
export default noop.memo(function VoicePanelUI() {
      ...
      if (mode.connected) {
        if (!mode.gestureActive) {
          if (mode === VoicePanelModes.PANEL) {
            tmp2(4559).runOnJS(closure_2)(true);
            const tmp2Result = tmp2(4559);
          }
          if (mode === VoicePanelModes.PANEL) {
            tmp2(4559).runOnJS(_slicedToArray)(true);
            const tmp2Result6 = tmp2(4559);
          } else {
            tmp2(4559).runOnJS(_slicedToArray)(false);
            const tmp2Result7 = tmp2(4559);
          }
          if (mode === tmp9.PIP) {
            tmp2(4559).runOnJS(noop)(true);
            const tmp2Result8 = tmp2(4559);
          } else {
            tmp2(4559).runOnJS(noop)(false);
            const tmp2Result9 = tmp2(4559);
          }
          tmp9 = VoicePanelModes;
        }
      }
      tmp2(4559).runOnJS(closure_2)(false);
      const tmp2Result10 = tmp2(4559);
    }
  };
          ...
          if (velocityY > 0) {
            if (connected.get()) {
              if (gestureState.get().requiresPop) {
                ...
                channelId(4559).runOnJS(closure_1_15)(tmp2.PIP);
                closure_1(11697)(wrapperOffset, { gestureActive: false, x: 0, y: 0 });
                obj4 = channelId(4559);
              }
            } else {
              ...
              if (wrapperOffset.get().y > 0.2 * diff) {
                closure_1(11697)(tmp12, { gestureActive: false });
                channelId(4559).runOnJS(dismissPanel)();
                const obj3 = channelId(4559);
              } else {
                closure_1(11697)(tmp12, { gestureActive: false, x: 0, y: 0 });
  ...
  if (tmp6[0]) {
    ...
    const items8 = [closure_21(tmp13(7487).LayerScope, obj28), ];
    ...
    obj31.children = closure_21(tmp13(6983).GestureDetector, obj32);
    obj30.children = closure_21(tmp2(7404), obj31);
    items8[1] = closure_21(tmp13(6983).GestureDetector, obj30);
    ...
    items6[2] = closure_21(tmp13(6983).GestureDetector, obj25);
    obj23.children = items6;
    obj22.children = lockScrolling(tmp13(7487).LayerScope, obj23);
    tmp37 = closure_21(tmp13(7493).AnalyticsLocationProvider, obj22);
