...
// Dependencies: [19, 17, 4849, 502, 21, 4559, 5271, 4827, 576, 10404, 504, 4879, 4828, 7404, 4823, 1115, 2]
// Exports: VoicePanelStreamPreview

// Module 13418 (VoicePanelStreamPreview)
...
let ReanimatedRexport = ReanimatedRexport_mod;
let closure_9 = ReanimatedRexport.createAnimatedComponent(fn(5271).Button);
...
export const VoicePanelStreamPreview = function VoicePanelStreamPreview(mode) {
  ...
  const previewUrl = stream(10404)(guildId, channelId, ownerId).previewUrl;
  const tmp4 = stream(10404);
  ...
  let obj2 = mode(4559);
  T.__closure = { mode, withTiming: mode(4828).withTiming, OPACITY_TIMING };
  ...
  if (null != previewUrl) {
    ...
    tmp14 = closure_6(tmp2(4559).Image, obj5);
  }
  const items2 = [tmp14, ];
  const obj7 = { style: animatedStyle, layout, children: null };
  let obj3 = { mode, withTiming: mode(4828).withTiming, OPACITY_TIMING };
  ...
  if (stateFromStores) {
    ...
    obj8.children = tmp16(tmp8(4823).Text, obj9);
  ...
  items2[1] = closure_6(stream(7404), obj7);
