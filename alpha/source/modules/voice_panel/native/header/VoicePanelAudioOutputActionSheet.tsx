...
// Dependencies: [19, 17, 4846, 9992, 2044, 4847, 10019, 1074, 21, 4829, 4796, 563, 10021, 10022, 1115, 5990, 5993, 5916, 9988, 10132, 9853, 10133, 4649, 2028, 10150, 7483, 7482, 5894, 2]

// Module 10020 (VoicePanelAudioOutputActionSheet)
...
import useOnConnectToConsole from "useOnConnectToConsole" /* 10133 */;
  ...
  if (availableDevices.length > 0) {
    ...
    const obj3 = {
      ...
      children: availableDevices.map((deviceId) => {
          const obj = { value: deviceId.deviceId, icon: closure_1_11(availableDevices(5916).TableRowIcon, { source: availableDevices(9988).audioDeviceToIconMap[deviceId.simpleDeviceType] }), label: null, subLabel: null };
          ...
          return tmp(availableDevices(5993).TableRadioRow, obj, deviceId.deviceId);
        })
    };
...
function VoicePanelAudioConsoleSection(channel) {
  ...
  const tmp8 = !channel(4649).useIsDismissibleContentDismissed_UNSAFE(channel(2028).DismissibleContent.DONUT_MOBILE_NUX);
...
export default noop.memo(function VoicePanelAudioOutputActionSheet(arg0) {
  ...
  if (null != stateFromStores) {
    ...
    obj2.header = closure_11(tmp(7482).BottomSheetTitleHeader, obj3);
    ...
    tmp5Result = tmp5(tmp(7483).BottomSheet, obj2);
  }
  return tmp5Result;
});
