...
// Dependencies: [32, 19, 17, 4819, 2041, 4843, 8636, 9825, 9734, 2042, 1992, 4850, 2096, 5035, 12606, 12604, 1074, 2004, 9392, 4848, 12607, 21, 17594, 4559, 9743, 1091, 504, 4521, 9794, 1115, 17595, 17596, 9990, 9672, 17597, 1478, 1612, 17623, 12612, 12608, 11697, 12, 1255, 1110, 1248, 9813, 12367, 5170, 1241, 8678, 9695, 17553, 4533, 17624, 5028, 5007, 4693, 7369, 17625, 17626, 4451, 17627, 17628, 17635, 17558, 17636, 12605, 4710, 2]
// Exports: default

// Module 17593 (VoicePanelController)
...
import trackActivityThermalStateNoticeShown from "trackActivityThermalStateNoticeShown" /* 17596 */;
import VoicePanelFloatingCTAUtils from "VoicePanelFloatingCTAUtils" /* 17597 */;
import useIsVoicePanelParticipantFocusable from "useIsVoicePanelParticipantFocusable" /* 17624 */;
import VoicePanelPIPStateContext from "VoicePanelPIPStateContext" /* 17636 */;
...
const MorphablePanelModes = fn(12607).MorphablePanelModes;
const jsx = fn(21).jsx;
...
export default function VoicePanelController(channelId) {
  ...
  if (stateFromStores) {
    ...
    const fn10 = function f(arg0, arg1) {
      if (!obj.cheapWorkletArrayShallowEqual(arg0, arg1)) {
        [tmp6, tmp7, tmp8] = arg0;
        if (tmp8 === tmp(4533).TransitionStates.YEETED) {
          if (tmp7 !== constants.DISMISSED) {
            tmp(4559).runOnJS(sharedValue)(tmp16.DISMISSED);
          ...
          if (tmp19.PANEL !== PANEL1) {
            ...
            tmp(4559).runOnJS(sharedValue)(PANEL);
            const tmpResult3 = tmp(4559);
          }
          PANEL = PANEL1;
          if (!tmp6) {
            PANEL = tmp19.PANEL;
          ...
          if (!tmp9) {
            tmp(4559).runOnJS(sharedValue)(tmp19.PANEL);
            const tmpResult4 = tmp(4559);
          }
        }
      ...
      if (tmp3) {
        if (result) {
          if (!result1) {
            ...
            if (tmp15) {
              const obj2 = { key: "EMBEDDED_ACTIVITIES_VIDEO_DISABLED_FOR_THERMAL_STATE", icon: _modDef17595, content: null, disableAnimations: true, toastDurationMs: 3000 };
    ...
    const effect8 = obj2.useEffect(() => {
      if (first) {
        ...
        if (performance.now() - guildId.current >= closure_33) {
          ...
          channelId();
          const obj2 = { key: "SPEAKING_WHILE_MUTED", icon: _modDef9794, content: null, toastDurationMs: 3000 };
          const intl = util.intl;
