...
// Dependencies: [19, 17, 7860, 21, 4829, 576, 4545, 8045, 7859, 11345, 573, 4796, 11367, 1980, 1115, 11351, 4825, 2]
// Exports: default

// Module 11366 (AppealIngestionCollectSignal)
...
export default function AppealIngestionCollectSignal(isDsaEligible) {
  ...
  function handleAppealSignalSelect(signal) {
    ...
    if (tmp4) {
      ...
      ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(11367, dependencyMap.paths), "AppealIngestionFreeTextAppealReasonActionSheet", obj3);
  ...
  const items = [closure_6(isDsaEligible(11351).AppealIngestionModalHeader, { headerText: stringResult, subHeaderText: intl2.string(isDsaEligible(1115).t.VEcRhw) }), ];
  let obj3 = { style: tmp.container, children: null };
  const obj4 = { style: tmp.form, children: null };
  const stringResult1 = intl2.string(isDsaEligible(1115).t.VEcRhw);
  const items1 = [closure_6(isDsaEligible(8045).FormSection, { sectionBodyStyle: tmp.formSection, accessibilityRole: "radiogroup", children: closure_5.map((signal, index) => timestampProducer(AppealSignalRadioRow, { signal, selected: signal === closure_2, rowStyle: formRow.formRow, onSelect: handleAppealSignalSelect }, "formrow-" + index)) }), ];
