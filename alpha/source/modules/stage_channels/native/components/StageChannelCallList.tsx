...
// Dependencies: [32, 19, 10308, 5633, 21, 1177, 10309, 5644, 5204, 5650, 38, 10316, 1115, 10317, 10318, 10330, 7319, 10334, 1478, 5344, 2]
// Exports: default

// Module 10307 (StageChannelCallList)
import _modDef38 from "module_38" /* 38 */;
import util from "util" /* 1115 */;
import StageChannelParticipants from "StageChannelParticipants" /* 5644 */;
import SpeakerTile from "SpeakerTile" /* 10309 */;
import StageSectionHeaderDefault from "StageSectionHeader" /* 10316 */;
import StageGridRowDefault from "StageGridRow" /* 10318 */;
import AudienceGridRowDefault from "AudienceGridRow" /* 10330 */;
    ...
    if (stageParticipantsCount.STREAM === arg0) {
      ...
      if (0 !== actualStageSpeakerCount) {
        ...
        if (first1) {
          ...
          tmp21Result = tmp21(tmp22(10317), cutout);
          const tmp22Result = tmp22(10317);
        }
...
export default function StageChannelCallList(channel) {
  ...
  const throttleDurationForChannel = width(10334).useThrottleDurationForChannel(channel.id);
  ...
  const memo = noop.useMemo(() => {
    ...
    if (isScreenLandscape) {
      const _Math = Math;
      const _Math2 = Math;
      num = Math.max(3, Math.floor(width / tmp2(10309).LANDSCAPE_MAX_TILE_WIDTH));
    }
    const obj = {};
  ...
  const tmp4 = _slicedToArray(width(10334).useStageChannelParticipantsListThrottled(channel.id, memo, throttleDurationForChannel, true), 2);
