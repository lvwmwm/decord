...
// Dependencies: [32, 19, 17, 502, 2042, 5717, 21, 4827, 576, 10260, 7531, 1115, 504, 4974, 10261, 10263, 5908, 5725, 8757, 8759, 4794, 8744, 1177, 10265, 4524, 7493, 7513, 5734, 5728, 7481, 6955, 5269, 5990, 4823, 10266, 2]
// Exports: default

// Module 10259 (RequestToSpeakActionSheet)
...
import useRequestToSpeakPermission from "useRequestToSpeakPermission" /* 10260 */;
import _slicedToArray from "module_32" /* 32 */;
import noop from "module_19" /* 19 */;
import AuthenticationStore from "AuthenticationStore" /* 502 */;
import ChannelStore from "ChannelStore" /* 2042 */;

const _modDef10265 = tmp4(10265);
require = fn;
...
function ManageSelfSpeakerRow(channel) {
  ...
  const tmp8 = useAudienceRequestToSpeakStateDefault(stateFromStores, id) === channel(4974).RequestToSpeakStates.ON_STAGE;
  importDefault = tmp8;
  ...
  if (tmp8) {
    let MicrophoneArrowRightIcon = tmp(10261).GroupArrowDownIcon;
  } else {
    MicrophoneArrowRightIcon = tmp(10263).MicrophoneArrowRightIcon;
  }
  let obj2 = {
    onPress() {
      if (!closure_1) {
  ...
  obj2.trailing = closure_9(channel(1177).Icon, { source: _modDef10265 });
  return closure_9(channel(5908).TableRow, obj2);
}
const View = fn(17).View;
let closure_8 = fn(5717).REQUEST_TO_SPEAK_SHEET_KEY;
const jsxProd = fn(21);
...
export default function RequestToSpeakActionSheet(channelId) {
  ...
  let first1;
  noop = undefined;
  const token = channelId(4524).useToken(first(576).modules.mobile.TABLE_ROW_PADDING);
  const obj = channelId(4524);
  const tmp3 = first;
  const items = [];
  const tmp5 = closure_11();
  const tmp6 = first(7493);
  items[HermesBuiltin.arraySpread(channelId.analyticsLocations, 0)] = first(7513).REQUEST_TO_SPEAK;
  ...
  const stageParticipantsCount = channelId(5734).useStageParticipantsCount(channelId, channelId(5728).StageChannelParticipantNamedIndex.ALL_REQUESTED_TO_SPEAK);
  ...
  if (null != stateFromStores) {
    ...
    const items3 = [closure_10(tmp(5990).TableRowGroup, obj9), ];
    ...
    obj12.children = closure_9(tmp(4823).Text, obj14);
    items3[1] = closure_9(View, obj12);
    obj8.children = items3;
    const items4 = [closure_10(tmp(5269).Stack, obj8), ];
    const obj16 = { channel: stateFromStores, height: null };
    const _Math = Math;
    obj16.height = Math.max(first1 - first - 8, 0);
    items4[1] = closure_9(tmp3(10266), obj16);
    obj7.children = items4;
    obj6.children = closure_10(tmp(5269).Stack, obj7);
    obj5.children = closure_9(tmp(6955).BottomSheetScrollView, obj6);
    obj4.children = closure_9(tmp(7481).BottomSheet, obj5);
    tmp14 = closure_9(tmp(7493).AnalyticsLocationProvider, obj4);
    const tmp3Result = tmp3(10266);
  }
  return tmp14;
};
