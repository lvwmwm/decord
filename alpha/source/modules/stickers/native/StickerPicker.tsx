...
// Function ID: 10652
// Name: StickerPicker
// Dependencies: [32, 19, 17, 1372, 5721, 2023, 1074, 21, 4757, 576, 1241, 10652, 10654, 504, 7409, 7429, 9441, 10547, 1248, 10658, 7582, 5104, 10659, 9433, 4455, 1115, 7435, 10668, 7297, 10678, 10679, 10685, 2]

// Module 10651 (StickerPicker)
...
import StickersSearchUtils from "StickersSearchUtils" /* 10658 */;
import openStickerPackDetailActionSheetDefault from "openStickerPackDetailActionSheet" /* 10659 */;
...
let obj = { container: { flex: 1 }, header: { paddingVertical: nativeDefault.space.PX_8 }, loadingIndicator: { alignItems: "center", justifyContent: "center", flex: 1 }, emptyState: { marginTop: fn(2023).STICKER_SEARCH_HEADER_HEIGHT, alignItems: "center", justifyContent: "center", flex: 1 } };
...
export default noop.memo(function StickerPicker(channel) {
  ...
  let tmp = closure_16();
  const fetchStickerPacks = channel(10652).useFetchStickerPacks();
  let obj = channel(10652);
  const stickerCategories = channel(10654).useStickerCategories(channel);
  let obj2 = channel(10654);
  ...
  let mobileStickerPickerUpsellRestyleEnabled = channel(9441).useMobileStickerPickerUpsellRestyleEnabled("native.StickerPicker");
  let obj4 = channel(9441);
  let items1 = [channel, mobileStickerPickerUpsellRestyleEnabled];
  ({ safeAreaStyle, safeAreaBottomKeyboardAware } = onPressSticker(10547)({ hasCategories: true }));
  const items2 = [, ];
  ...
  const callback1 = noop.useCallback((pack_id) => {
    ...
    if (stickerSendability === StickerSendability.StickerSendability.SENDABLE) {
    ...
    } else if (stickerSendability === tmp(7582).StickerSendability.SENDABLE_WITH_PREMIUM) {
      if (tmpResult.isStandardSticker(pack_id)) {
        ...
        if (null != stickerPack) {
          ...
          obj2.analyticsPopoutType = tmp(10659).AnalyticsPopoutType.STICKER_PACK_DETAIL;
      ...
      mobileStickerPickerUpsellRestyleEnabled = stickerSendability === tmp(7582).StickerSendability.SENDABLE_WITH_BOOSTED_GUILD;
      if (mobileStickerPickerUpsellRestyleEnabled) {
        mobileStickerPickerUpsellRestyleEnabled = tmp(9441).getMobileStickerPickerUpsellRestyleEnabled("native.StickerPicker");
        const tmpResult4 = tmp(9441);
      }
      if (mobileStickerPickerUpsellRestyleEnabled) {
  ...
  const tmp13 = onPressSticker(10547)({ hasCategories: true });
  ...
  if (stateFromStores) {
    if (0 === stickerCategories.length) {
      ...
      tmp19Result2 = tmp19(tmp8(10679), obj11);
      const tmp8Result = tmp8(10679);
    }
  } else {
    ...
    items6[2] = tmp19(tmp8(10685), obj13);
