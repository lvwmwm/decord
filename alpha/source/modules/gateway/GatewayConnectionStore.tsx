...
// Dependencies: [5, 1220, 502, 5585, 2044, 1992, 4852, 4879, 2098, 5586, 4868, 1074, 1084, 13156, 13205, 3, 510, 6751, 5718, 1364, 13194, 12, 4881, 13173, 504, 573, 2]

// Module 5584 (GatewayConnectionStore)
import LoggerDefault from "Logger" /* 3 */;
import _modDef12 from "module_12" /* 12 */;
import initializeDefault from "initialize" /* 504 */;
import DispatcherDefault from "Dispatcher" /* 573 */;
import PlatformUtils from "PlatformUtils" /* 1364 */;
import GatewaySocketSingleton from "GatewaySocketSingleton" /* 13156 */;
import ConnectionStateDefault from "ConnectionState" /* 13173 */;
import PauseGatewaySocketAll from "PauseGatewaySocket" /* 13194 */;
import dispatchSocketMessageDefault from "dispatchSocketMessage" /* 13205 */;
...
fn(13156).socket.dispatcher.getDispatchHandler = dispatchSocketMessageDefault;
...
const gatewayConnectionStore = new GatewayConnectionStore(DispatcherDefault, {
  ...
  LOGIN_SUCCESS: function handleSessionRefresh() {
    ...
    if (connectResult) {
      const socket3 = tmp(13156).socket;
      socket3.close();
      const socket4 = tmp(13156).socket;
  ...
  VOICE_CHANNEL_SELECT: function handleVoiceChannelSelect(guildId) {
    ...
    if (isIOSResult) {
      if (null == guildId.channelId) {
        ...
        const socket = tmp(13156).socket;
        if (socket.isClosed()) {
          PauseGatewaySocketAll.setIsPaused(false);
          const socket2 = tmp(13156).socket;
          socket2.connect();
        }
      }
    }
      ...
      if (id.getId() !== userId.userId) {
        ...
        if (userId.sessionId === closure_1_21) {
          ...
          if (userId.guildId !== require("GatewaySocketSingleton").localVoiceState.guildId) {
            ...
            const localVoiceState = tmp(13156).localVoiceState;
    ...
    if (socket.isSessionEstablished()) {
      if (!allowMultiple) {
        ...
        const item = found.forEach((item) => {
          const socket = closure_0(13156).socket;
          if (socket.isSessionEstablished()) {
            const socket2 = closure_0(13156).socket;
