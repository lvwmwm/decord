...
// Dependencies: [5, 1220, 502, 5497, 2042, 1992, 4780, 4807, 2096, 5498, 4796, 1074, 1084, 13901, 13950, 3, 510, 7583, 5630, 1364, 13939, 12, 4809, 13918, 504, 573, 2]
...
import GatewaySocketSingleton from "GatewaySocketSingleton" /* 13901 */;
import ConnectionStateDefault from "ConnectionState" /* 13918 */;
import PauseGatewaySocketAll from "PauseGatewaySocket" /* 13939 */;
import dispatchSocketMessageDefault from "dispatchSocketMessage" /* 13950 */;
...
fn(13901).socket.dispatcher.getDispatchHandler = dispatchSocketMessageDefault;
...
const gatewayConnectionStore = new GatewayConnectionStore(DispatcherDefault, {
  ...
  LOGIN_SUCCESS: function handleSessionRefresh() {
    ...
    if (connectResult) {
      const socket3 = tmp(13901).socket;
      socket3.close();
      const socket4 = tmp(13901).socket;
  ...
  VOICE_CHANNEL_SELECT: function handleVoiceChannelSelect(guildId) {
    ...
    if (isIOSResult) {
      if (null == guildId.channelId) {
        ...
        const socket = tmp(13901).socket;
        if (socket.isClosed()) {
          PauseGatewaySocketAll.setIsPaused(false);
          const socket2 = tmp(13901).socket;
          socket2.connect();
        }
      }
    }
      ...
      if (id.getId() !== userId.userId) {
        ...
        if (userId.sessionId === closure_1_21) {
          ...
          if (userId.guildId !== require("GatewaySocketSingleton").localVoiceState.guildId) {
            ...
            const localVoiceState = tmp(13901).localVoiceState;
  ...
  STREAM_WATCH: function handleStreamWatch(arg0) {
    ...
    if (socket.isSessionEstablished()) {
      if (!allowMultiple) {
        ...
        _require = allActiveStreamKeys.find((item) => closure_0(4809).decodeStreamKey(item).ownerId === id.getId());
        ...
        const item = found.forEach((item) => {
          const socket = closure_0(13901).socket;
          if (socket.isSessionEstablished()) {
            const socket2 = closure_0(13901).socket;
