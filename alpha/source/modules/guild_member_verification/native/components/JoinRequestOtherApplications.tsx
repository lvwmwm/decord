...
// Dependencies: [19, 17, 21, 4827, 576, 4651, 4786, 6944, 16995, 4823, 1115, 16991, 4505, 2]

// Module 16994 (JoinRequestOtherApplications)
...
function ApplicationStatusIcon(status) {
  ...
  if (MemberVerificationTypes.GuildJoinRequestApplicationStatuses.APPROVED === status) {
    ...
    return timestampProducer(tmp(4786).CircleCheckIcon, obj2);
  } else if (tmp(4651).GuildJoinRequestApplicationStatuses.REJECTED === status) {
    const obj = { size: "sm", color: nativeDefault.colors.ICON_FEEDBACK_CRITICAL, secondaryColor: nativeDefault.colors.WHITE };
    return timestampProducer(tmp(6944).CircleXIcon, obj);
  } else {
    return null;
  }
}
...
export default noop.memo((arg0) => {
  ...
  if (0 !== otherGuildJoinRequestsForUser.length) {
    ...
    let items = [closure_6(tmp2(4823).Text, obj3), ];
    let obj4 = {
      style: tmp.container,
