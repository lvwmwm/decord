...
function SelectedServersRow(selectedGuilds) {
  ...
  let num = 3;
  let obj2 = { withTiming: selectedGuilds(4837).withTiming, selectedGuilds, SELECTED_SERVER_SIZE_WITH_BORDER: v50, tokens: first(576) };
  const tmp8 = v50;
...
function FeaturedServer(guild) {
  ...
  const items3 = [closure_12(guild(4832).Text, { maxFontSizeMultiplier: 1, style: tmp.featuredServerTitle, variant: "text-md/semibold", color: "mobile-text-heading-primary", children: guild.name }), closure_12(guild(4832).Text, { maxFontSizeMultiplier: 1, lineClamp: 2, variant: "text-xs/normal", color: "text-default", children: guild.description }), ];
  ...
  if (selected) {
    ...
    tmp7Result = tmp7(tmp2(4792).CircleCheckIcon, obj8);
  }
