...
// Dependencies: [32, 19, 17, 5727, 7473, 11062, 1074, 21, 672, 576, 4756, 5899, 11063, 8578, 5804, 4752, 1115, 11064, 1177, 2009, 4997, 1612, 5342, 9480, 504, 7413, 7410, 7471, 7427, 5203, 1241, 11059, 11060, 1364, 4427, 11065, 9479, 11066, 11075, 7476, 9106, 11079, 3548, 5186, 8179, 8341, 11082, 5848, 5897, 5198, 11084, 2]
// Exports: default

// Module 11061 (SocialLayerStorefrontProductDetailsModal)
...
import carouselMediaItems from "carouselMediaItems" /* 11063 */;
import StorefrontNativeUtils from "StorefrontNativeUtils" /* 11064 */;
import redirectToSlayerStorefrontWebDefault from "redirectToSlayerStorefrontWeb" /* 11065 */;
...
function SocialLayerStorefrontProductDetailsModal(skuId) {
  ...
  const isScreenLandscape = skuId(5342).useIsScreenLandscape();
  let obj = skuId(5342);
  const mobileStoreFront = analyticsLocations(9480).useMobileStoreFront();
  let obj2 = analyticsLocations(9480);
  ...
  const getOrFetchApplication = skuId(7413).useGetOrFetchApplication(applicationId1);
  let tmp12 = getOrFetchApplication;
  const obj5 = skuId(7413);
  if (getOrFetchApplication == null) {
    tmp12 = null;
  }
  const tmp2Result = analyticsLocations(7410);
  ...
  const OTPACOMOrderExperiment = tmp4(9479).OTPACOMOrderExperiment;
  ...
  if (tmp30) {
    ...
    const items10 = [skuId];
    obj6.skuIds = items10;
    obj6.onOrderRetryCancellation = tmp4(11059).closeSocialLayerStorefrontProductDetailsModal;
    ...
    obj6.children = closure_14(tmp4(11075).HeadlessSlayerStorefrontPurchaseRunner, obj9);
    tmp43Result = tmp43(tmp2(11066), obj6, skuId);
    const tmp2Result2 = tmp2(11066);
  }
  const items11 = [skuId, memo1, trackPDPClick];
  if (null == stateFromStores) {
    return null;
  } else {
    let result = tmp4(7476).isSlayerSkuAvailableOnThisPlatform(stateFromStores);
    const intl4 = tmp4(1115).intl;
    const stringResult = intl4.string(tmp4(1115).t.boqtTA);
    const tmp4Result7 = tmp4(7476);
    ...
    if (null != tmp25) {
      ...
      if (null != memo) {
        ...
        tmp49 = closure_14(tmp2(9106), obj11);
      }
    }
    let tmp53 = null;
    if (stateFromStores.exclusive) {
      const obj12 = { style: tmp.exclusiveBadgeContainer, children: closure_14(tmp4(11079).ExclusiveBadge, {}) };
    ...
    if (result1) {
      const obj22 = { icon: tmp2(8341), variant: "primary", size: "lg", disabled: tmp28, accessibilityLabel: null, onPress: null };
      const intl2 = tmp4(1115).intl;
      obj22.accessibilityLabel = intl2.string(tmp4(1115).t.QAZA5f);
      obj22.onPress = tmp48;
      result1 = tmp65(tmp4(8179).IconButton, obj22);
    }
    items14[1] = result1;
    ...
    if (result) {
      ...
      const mobileFinePrintMessageForApplication = tmp4(11082).getMobileFinePrintMessageForApplication(getOrFetchApplication, stringResult, obj24);
    ...
    const obj28 = {
      onPress: tmp4(11059).closeSocialLayerStorefrontProductDetailsModal,
      backImage() {
    ...
    const items17 = [closure_14(tmp4(5848).HeaderBackButton, obj28), ];
...
const SlayerShopPDPCTAType = fn(11062).SlayerShopPDPCTAType;
...
obj.header = { height: fn(5899).NAV_BAR_HEIGHT, flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_8, gap: nativeDefault.space.PX_8, borderBottomWidth: 1, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
obj.headerTitle = { flexShrink: 1 };
let obj2 = { height: fn(5899).NAV_BAR_HEIGHT, flexDirection: "row", alignItems: "center", paddingHorizontal: nativeDefault.space.PX_8, gap: nativeDefault.space.PX_8, borderBottomWidth: 1, borderBottomColor: nativeDefault.colors.BORDER_SUBTLE };
...
obj.hero = { marginHorizontal: nativeDefault.space.PX_16, height: fn(11063).MOBILE_HERO_HEIGHT_PX, borderRadius: nativeDefault.radii.md, overflow: "hidden", backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH };
obj.heroLandscape = { flex: 1, minHeight: 140, height: "alignItems" };
let obj20 = { marginHorizontal: nativeDefault.space.PX_16, height: fn(11063).MOBILE_HERO_HEIGHT_PX, borderRadius: nativeDefault.radii.md, overflow: "hidden", backgroundColor: nativeDefault.colors.BACKGROUND_SURFACE_HIGH };
