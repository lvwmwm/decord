...
// Dependencies: [19, 17, 1074, 21, 4827, 576, 7499, 11146, 1397, 9182, 4823, 2]
// Exports: default

// Module 11353 (SocialLayerStorefrontGiftProductDetails)
...
export default function SocialLayerStorefrontGiftProductDetails(sku) {
  ...
  const tmp = closure_9();
  getOrFetchApplication = getOrFetchApplication(7499).useGetOrFetchApplication(sku.applicationId);
  let obj = getOrFetchApplication(7499);
  const userPrice = getOrFetchApplication(11146).useFormattedSKUPrice({ sku, priceSetAssignmentPurchaseType: constants.GIFT }).userPrice;
  ...
  if (tmp6Result) {
    ...
    items2[1] = tmp8(tmp2(4823).Text, obj9);
    obj6.children = items2;
    tmp6Result = tmp6(tmp7, obj6);
  }
  const items3 = [tmp6Result, closure_7(getOrFetchApplication(4823).Text, { variant: "text-md/semibold", children: sku.name })];
  ...
  if (tmp8Result2) {
    ...
    tmp8Result2 = tmp8(tmp2(4823).Text, obj11);
  }
