...
// Dependencies: [19, 17, 4748, 10352, 21, 4756, 576, 8545, 4752, 4457, 1364, 5174, 5299, 7213, 563, 7842, 10432, 10387, 8394, 8536, 1477, 5804, 7534, 7539, 8578, 10433, 1177, 9094, 2]
...
function ObscuredMediaOverlay(isSpoiler) {
  ...
  if (isObscured) {
    let tmp14Result = tmp14(tmp2(5299).ImageWarningIcon, { size: "sm", color: "white" });
  } else {
    const obj5 = { style: tmp.spoilerPill, children: tmp14(tmp2(7213).EyeIcon, { size: "sm", color: "white" }) };
...
function SinglePreviewableMedia(arg0) {
  ...
  if (usePreviewableMedia.PreviewableMediaTypes.VOICE_MESSAGE === type) {
    ...
    if (tmp2(10387).PreviewableMediaTypes.AUDIO !== type) {
      if (tmp2(10387).PreviewableMediaTypes.FILE !== type) {
        if (tmp2(10387).PreviewableMediaTypes.IMAGE !== type) {
          if (tmp2(10387).PreviewableMediaTypes.VIDEO !== type) {
            if (tmp2(10387).PreviewableMediaTypes.GIF === type) {
              ...
              if (null == thumbnail) {
                ...
                const enabledHarmTypesForChannelAndAuthorId = tmp2(7534).getEnabledHarmTypesForChannelAndAuthorId(message.channel_id, id);
                const tmp2Result = tmp2(7534);
                const obj5 = { type: null, media: null };
                const getMediaObscuredReasonFromBitmask = tmp2(7534).getMediaObscuredReasonFromBitmask;
                obj5.type = tmp2(7539).ObscuredMediaTypes.Embed;
                obj5.media = media;
                ...
                url2 = thumbnail.url;
                const tmp2Result5 = tmp2(7534);
              }
            } else if (tmp2(10387).PreviewableMediaTypes.STICKER === type) {
          ...
          if (width > 0) {
            if (null != height) {
              if (height > 0) {
                ...
                const attachmentObscurityProps = tmp2(8394).getAttachmentObscurityProps(obj13);
                const tmp2Result6 = tmp2(8394);
                const attachmentUrl = tmp2(8536).getAttachmentUrl(media);
                const tmp2Result7 = tmp2(8536);
                ...
                let str3;
                if (type === tmp2(10387).PreviewableMediaTypes.VIDEO) {
                  str3 = "png";
                }
                obj14.format = str3;
