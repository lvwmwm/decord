...
// Dependencies: [19, 17, 4819, 10437, 1074, 1085, 21, 4827, 1365, 576, 2020, 10472, 4823, 1397, 10561, 7461, 1115, 7630, 10436, 10449, 10450, 10478, 11256, 1177, 5887, 12, 5073, 504, 5030, 4838, 10438, 10480, 1980, 10512, 10516, 2]
// Exports: default

// Module 10560 (ReactionNotification)
...
function ReactionNotificationBody(arg0) {
  ...
  if (null != secondaryText) {
    ...
    tmp8Result = tmp8(tmp2(4823).Text, obj3);
  }
  ...
  if (hasMessageContent) {
    ...
    if (null != messagePreview) {
      ...
      tmp8Result2 = tmp8(tmp2(10450).NativeChannelRowPreview, obj4);
    }
  }
...
function ReactionNotificationBodyWrapper(arg0) {
  ...
  const previewableMedia = message(10472).usePreviewableMedia(message);
  ...
  let obj = message(10472);
  const hasPreviewableMedia = message(10436).useHasPreviewableMedia(message);
  let obj2 = message(10436);
...
export default function ReactionNotification(notification) {
  ...
  const callback1 = obj2.useCallback(() => ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(10480, dependencyMap.paths), { channelId: id }, "in-app-notification-settings-modal"), items4);
