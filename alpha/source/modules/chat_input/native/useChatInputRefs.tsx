...
// Dependencies: [19, 8109, 5807, 5192, 8004, 8010, 1372, 9737, 1074, 5298, 4822, 5903, 4695, 12, 12330, 12332, 12333, 12336, 12337, 9499, 8106, 4697, 1610, 1482, 1364, 4698, 1875, 1241, 12005, 7786, 12007, 12338, 8005, 12339, 5190, 12371, 6955, 2]
// Exports: default

// Module 12329 (useChatInputRefs)
...
import ChatInputNativeCommandsDefault from "ChatInputNativeCommands" /* 12330 */;
import ChatInputSendUtils from "ChatInputSendUtils" /* 12339 */;
...
let closure_11 = fn(9737).updateChatInputContainerHeight;
...
export default function useChatInputRefs(chatInputProps) {
  ...
  const tmp2 = chatInputTextFieldHeight(5903)(() => ChatInputUtils.createInputRefTracker(chatInputProps.channel.id, chatInputProps.screenIndex));
  closure_12 = tmp2;
  ...
  const state = noop.useRef(chatInputTextFieldHeight(5903)(() => ({ editId: null, focused: false, selectionStart: 0, selectionEnd: 0, text: chatInputProps.defaultValue, textPrev: chatInputProps.defaultValue, textFieldContentSize: 0, textFieldHeight: chatInputTextFieldHeight })));
    ...
    let obj4 = {
      ...
      handleCancelEditing() {
        ...
        if (null != editingMessage) {
          ...
          tmp6(7786).endEditMessage(channel.id);
          ...
          if (null != threadCreationCallback) {
            ...
            if (applicationCommandManager != null) {
              sendCommandResult = applicationCommandManager.sendCommand(state.current.text, tmp4.current.channel, (command, optionValues) => {
                ...
                obj2.params = obj4;
                const result = threadCreationCallback(12339).chatInputSendApplicationCommand(obj2);
              });
            }
            if (!sendCommandResult) {
              const text1 = chatInputTextFieldHeight(ref[14]).getText(chatInputNativeRef.current, ref3.current, (text) => {
                if (null != props.current.pendingEdit) {
                  ...
                  obj3.chatInputRef = chatInputRef;
                  obj2.params = obj3;
                  const result = chatInputProps(12339).chatInputHandleSendText(obj2);
                  const obj = chatInputProps(12339);
                }
                const keyboardType = chatInputProps(4697).getKeyboardType();
        ...
        if (!SlowmodeStore.isChannelOnCooldown(channel)) {
          ...
          if (null != sticker) {
            if (!obj7.isStandardSticker(current2)) {
              ...
              obj.params = obj2;
              const result = tmp27(12339).chatInputValidateContentLength(obj);
              if (null != result) {
                closure_1.cancel();
                if (null != threadCreationCallback) {
                  ...
                  tmp27(12007).deletePendingReply(channel.id);
                  const tmp27Result2 = tmp27(12007);
                  tmp31(8106).saveDraft(channel.id, "", DraftType.ChannelMessage);
