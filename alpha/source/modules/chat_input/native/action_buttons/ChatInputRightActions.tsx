...
// Dependencies: [32, 19, 17, 12295, 21, 4827, 576, 4524, 4533, 12507, 1610, 12580, 4559, 12579, 12578, 2]

// Module 12770 (ChatInputRightActions)
import nativeDefault from "native" /* 576 */;
import native from "native" /* 4533 */;
import ReanimatedRexportDefault from "ReanimatedRexport" /* 4559 */;
import ChatInputActionButtonGiftOrThreadDefault from "ChatInputActionButtonGiftOrThread" /* 12578 */;
import ChatInputActionButtonTransitionItem from "ChatInputActionButtonTransitionItem" /* 12579 */;
import useChatInputFloatingBounceDefault from "useChatInputFloatingBounce" /* 12580 */;
...
function LeftSlot(state) {
  ...
  obj2.children = React5(ChatInputActionButtonGiftOrThreadDefault, { canStartThreads: false, channel, onPress, styleButton: "flex", shouldShowThread: "comic" });
  return React5(ReanimatedRexportDefault.View, obj2);
}
const View = fn(17).View;
const enterDelayMs = fn(12295).CHAT_INPUT_FLOATING_BOUNCE_ENTER_DELAY_MS;
const jsxProd = fn(21);
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const token = channel(4524).useToken(onPressAction(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_SIZE);
  const obj = channel(4524);
  const tmp3 = onPressAction;
  const sum = token + 2 * channel(4524).useToken(onPressAction(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_MARGIN);
  dependencyMap = sum;
  ...
  if (shouldShowGiftButton) {
    ...
    tmp14Result = closure_7(tmp(4533).TransitionItem, obj4);
  }
  ...
  items1[1] = closure_7(tmp3(12507), obj5);
