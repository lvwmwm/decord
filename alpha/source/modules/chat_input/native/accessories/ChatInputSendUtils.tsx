...
// Dependencies: [5, 5193, 7095, 5192, 1372, 1074, 4822, 1374, 8600, 7191, 1231, 11464, 11459, 7090, 6871, 11150, 8602, 8597, 4485, 8606, 5196, 1115, 1241, 9709, 7318, 4796, 11466, 1980, 5439, 6598, 8783, 8706, 11496, 6938, 2]
...
function chatInputValidateContentLength(arg0) {
  ...
  if (parsed.content.length <= obj3.getMaxMessageLength()) {
    ...
    const tmp7Result4 = tmp7(5196);
    obj5.maxLength = tmp10(8597).getMaxMessageLength();
    obj4.body = intl2.formatToPlainString(tmp10(1115).t.FfjF15, obj5);
    tmp7Result4.show(obj4);
    const tmp10Result = tmp10(8597);
    ...
    const result1 = tmp7(8606).handleShowUpsellAlert(obj7);
    const tmp7Result6 = tmp7(8606);
  }
...
function showFileSizeExceededAlert(c8, largestFileSize) {
  ...
  if (obj.canUploadLargeFiles(currentUser)) {
    ...
    const items = [tmp2(6598).FILE_UPLOAD_POPOUT];
    ...
    const result = tmp2(8606).handleShowUpsellAlert(obj4);
    const tmp2Result2 = tmp2(8606);
  }
}
...
export const chatInputCreateThread = function chatInputCreateThread(text) {
  ...
  if (text.length > obj.getMaxMessageLength()) {
    if (!obj2.canUseIncreasedMessageLength(UserStore.getCurrentUser())) {
      ...
      const result = tmp2(8606).handleShowUpsellAlert(obj3);
    }
