...
// Dependencies: [5, 5190, 8008, 5189, 1372, 1074, 4820, 1374, 9498, 8104, 1231, 12329, 12324, 8003, 7784, 12000, 9500, 9495, 4481, 9504, 5193, 1115, 1241, 10600, 8227, 4794, 12331, 1980, 5436, 7513, 9681, 9604, 12361, 7851, 2]
// Exports: chatInputCreateThread, chatInputHandleSendText, chatInputSendApplicationCommand

// Module 12330 (ChatInputSendUtils)
...
import useMessageMaxLength from "useMessageMaxLength" /* 9495 */;
...
import ChatRestrictions from "ChatRestrictions" /* 10600 */;
...
function chatInputSendMessage(params) {
  ...
  chatInputRef(8104).saveDraft(channel.id, "", scheduledTimestamp.ChannelMessage);
  ...
  let obj3 = chatInputRef(8104);
  const handleLegacyCommandsResult = channel(12329).handleLegacyCommands(text, { channel, isEdit: false });
  let tmp14 = tts;
  ...
  if (null != mentionTimestamps) {
    ...
    if (mentionTimestamps.size > 0) {
      result = tmp12(12324).serializeComposerTimestampMentions(tmp16, mentionTimestamps);
      const tmp12Result = tmp12(12324);
    }
  }
  if (parsed == null) {
    const tmp6Result = tmp6(8003);
    parsed = tmp6Result.parse(channel, result, undefined, mentionGames);
  }
  parsed.tts = tmp14;
  const obj5 = {};
  const obj4 = channel(12329);
  const merged = Object.assign(chatInputRef(7784).getSendMessageOptionsForReply(params.pendingReply));
  ...
  if (hasAttachmentsToUpload) {
    ...
    if (null == uploads) {
      ...
      tmp6(9498).clearAll(id, tmp8.ChannelMessage);
      const tmp6Result6 = tmp6(9498);
    }
  } else {
    items = [];
    if (!hasAttachmentsToUpload) {
    ...
    tmp12(12000).deletePendingReply(channel.id);
    if (applicationCommandManager != null) {
      const result1 = applicationCommandManager.clearTimestampMentions();
    }
    const tmp6Result8 = tmp6(7784);
    ...
    const tmp12Result2 = tmp12(12000);
...
function chatInputValidateContentLength(arg0) {
  ...
  if (parsed.content.length <= obj3.getMaxMessageLength()) {
    ...
    const tmp7Result4 = tmp7(5193);
    obj5.maxLength = tmp10(9495).getMaxMessageLength();
    obj4.body = intl2.formatToPlainString(tmp10(1115).t.FfjF15, obj5);
    tmp7Result4.show(obj4);
    const tmp10Result = tmp10(9495);
    ...
    const result1 = tmp7(9504).handleShowUpsellAlert(obj7);
    const tmp7Result6 = tmp7(9504);
  }
...
function showFileSizeExceededAlert(c8, largestFileSize) {
  ...
  if (obj.canUploadLargeFiles(currentUser)) {
    ...
    const items = [tmp2(7513).FILE_UPLOAD_POPOUT];
    ...
    const result = tmp2(9504).handleShowUpsellAlert(obj4);
    const tmp2Result2 = tmp2(9504);
  }
}
        ...
        while (iter !== undefined) {
          ...
          if (false !== checkResult) {
            ...
            obj3.onConfirm = function onConfirm() {
              const obj = { text, parsedMessage, tts: "padding", source: "redBoxV2Android", params };
        ...
        if (null != uploads) {
          if (obj4.shouldShowAddMediaToOriginalPostModal(uploads, channel.id)) {
            const obj5 = {
              ...
              attachments: uploads,
              sendMessage() {
                          chatInputSendMessage({ text, parsedMessage, tts: "padding", source: null, params });
                        }
            };
            params(4794).openLazy(tmp19(1980)(12331, tmp20.paths), "add-media-to-original-forum-post", obj5);
            const obj6 = params(4794);
          }
          obj4 = ForumPostMediaUtils;
          tmp19 = require;
          tmp20 = dependencyMap;
        }
        const obj7 = { text, parsedMessage: tmp2, tts: "padding", source: "Array", params };
...
export const chatInputCreateThread = function chatInputCreateThread(text) {
  ...
  if (text.length > obj.getMaxMessageLength()) {
    if (!obj2.canUseIncreasedMessageLength(UserStore.getCurrentUser())) {
      ...
      const result = tmp2(9504).handleShowUpsellAlert(obj3);
    }
