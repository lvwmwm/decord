...
// Dependencies: [5, 5192, 8010, 5191, 1372, 1074, 4822, 1374, 9502, 8106, 1231, 12338, 12333, 8005, 7786, 12007, 9504, 9499, 4483, 9508, 5195, 1115, 1241, 10605, 8229, 4796, 12340, 1980, 5438, 7515, 9685, 9608, 12370, 7853, 2]
// Exports: chatInputCreateThread, chatInputHandleSendText, chatInputSendApplicationCommand

// Module 12339 (ChatInputSendUtils)
...
import useMessageMaxLength from "useMessageMaxLength" /* 9499 */;
...
import ChatRestrictions from "ChatRestrictions" /* 10605 */;
...
function chatInputSendMessage(params) {
  ...
  chatInputRef(8106).saveDraft(channel.id, "", scheduledTimestamp.ChannelMessage);
  ...
  let obj3 = chatInputRef(8106);
  const handleLegacyCommandsResult = channel(12338).handleLegacyCommands(text, { channel, isEdit: false });
  let tmp14 = tts;
  ...
  if (null != mentionTimestamps) {
    ...
    if (mentionTimestamps.size > 0) {
      result = tmp12(12333).serializeComposerTimestampMentions(tmp16, mentionTimestamps);
      const tmp12Result = tmp12(12333);
    }
  }
  if (parsed == null) {
    const tmp6Result = tmp6(8005);
    parsed = tmp6Result.parse(channel, result, undefined, mentionGames);
  }
  parsed.tts = tmp14;
  const obj5 = {};
  const obj4 = channel(12338);
  const merged = Object.assign(chatInputRef(7786).getSendMessageOptionsForReply(params.pendingReply));
  ...
  if (hasAttachmentsToUpload) {
    ...
    if (null == uploads) {
      ...
      tmp6(9502).clearAll(id, tmp8.ChannelMessage);
      const tmp6Result6 = tmp6(9502);
    }
  } else {
    items = [];
    if (!hasAttachmentsToUpload) {
    ...
    tmp12(12007).deletePendingReply(channel.id);
    if (applicationCommandManager != null) {
      const result1 = applicationCommandManager.clearTimestampMentions();
    }
    const tmp6Result8 = tmp6(7786);
    ...
    const tmp12Result2 = tmp12(12007);
...
function chatInputValidateContentLength(arg0) {
  ...
  if (parsed.content.length <= obj3.getMaxMessageLength()) {
    ...
    const tmp7Result4 = tmp7(5195);
    obj5.maxLength = tmp10(9499).getMaxMessageLength();
    ...
    const result1 = tmp7(9508).handleShowUpsellAlert(obj7);
    const tmp7Result6 = tmp7(9508);
  }
...
function showFileSizeExceededAlert(c8, largestFileSize) {
  ...
  if (obj.canUploadLargeFiles(currentUser)) {
    ...
    const items = [tmp2(7515).FILE_UPLOAD_POPOUT];
    ...
    const result = tmp2(9508).handleShowUpsellAlert(obj4);
  ...
  if (!SlowmodeStore.isChannelOnCooldown(channel)) {
    if (0 !== text.length) {
      ...
      if (null != tmp2) {
        ...
        while (iter !== undefined) {
          ...
          if (false !== checkResult) {
            ...
            obj3.onConfirm = function onConfirm() {
              const obj = { text, parsedMessage, tts: "padding", source: "<string:1359020034>", params };
        ...
        if (null != uploads) {
          if (obj4.shouldShowAddMediaToOriginalPostModal(uploads, channel.id)) {
            const obj5 = {
              ...
              attachments: uploads,
              sendMessage() {
                          const obj = { text, parsedMessage, tts: "padding", source: "r", params };
                          chatInputSendMessage(obj);
                        }
            };
            params(4796).openLazy(tmp19(1980)(12340, tmp20.paths), "add-media-to-original-forum-post", obj5);
        ...
        const obj7 = { text, parsedMessage: tmp2, tts: "padding", source: 1107361793, params };
...
export const chatInputCreateThread = function chatInputCreateThread(text) {
  ...
  if (text.length > obj.getMaxMessageLength()) {
    if (!obj2.canUseIncreasedMessageLength(UserStore.getCurrentUser())) {
      ...
      const result = tmp2(9508).handleShowUpsellAlert(obj3);
    }
