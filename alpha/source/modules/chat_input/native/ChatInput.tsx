...
// Function ID: 12216
// Name: ChatInput
// Dependencies: [5, 32, 19, 17, 8025, 12216, 9779, 7919, 12217, 5106, 7920, 4396, 5105, 9660, 12219, 1074, 1483, 2039, 1375, 1608, 21, 4757, 576, 1364, 12220, 2020, 4575, 4458, 12221, 504, 7513, 7409, 8006, 4493, 4625, 9606, 12241, 12242, 8091, 12243, 12244, 7468, 1110, 1611, 12287, 12288, 5356, 9427, 5346, 1255, 4725, 1241, 1610, 4937, 9599, 12292, 12453, 10901, 4623, 1874, 12454, 10598, 8179, 12488, 10916, 9424, 12491, 12492, 4428, 12508, 12509, 12510, 12511, 12517, 12518, 1092, 12519, 12520, 12524, 12540, 12545, 12548, 12641, 12643, 12663, 12665, 12666, 12679, 12683, 12687, 12688, 12689, 12690, 12691, 2]

// Module 12215 (ChatInput)
...
import DiceRollStore from "DiceRollStore" /* 12216 */;
...
let useVoiceMessagesUIStore = fn(12217).useVoiceMessagesUIStore;
const DraftType = fn(5106).DraftType;
const useChatBottomManagerUIStore = fn(9660);
...
const forwardRefResult = noop.forwardRef((channel, ref) => {
  ...
  const mobileEmojiSuggestionsConfig = channel(12220).useMobileEmojiSuggestionsConfig({ location: "ChatInput" });
  ...
  let obj = channel(12220);
  const gradientValue = channel(4575).useGradientValue(tmp(4575).GradientPercentage.END);
  let tmpResult = channel(4575);
  const token = channel(4458).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_ACTION_BUTTON_SIZE);
  const tmpResult30 = channel(4458);
  let result = (channel(4458).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_SEND_BUTTON_HEIGHT) - token) / 2;
  const tmpResult31 = channel(4458);
  const token1 = channel(4458).useToken(screenIndex(576).modules.mobile.CHAT_INPUT_FLOATING_SCRIM_GRADIENT_HEIGHT);
  const tmp10 = closure_40(gradientValue, token1);
  const tmpResult32 = channel(4458);
  let token2 = gradientValue;
  if (gradientValue == null) {
    token2 = tmpResult33.useToken(screenIndex(576).colors.BACKGROUND_BASE_LOWER);
  }
  tmpResult33 = channel(4458);
  const token3 = channel(4458).useToken(tmp6(576).modules.mobile.CHAT_INPUT_FLOATING_TYPING_GRADIENT_HEIGHT_REDUCED);
  const tmpResult34 = channel(4458);
  const token4 = channel(4458).useToken(tmp6(576).modules.mobile.CHAT_INPUT_FLOATING_INLINE_FULL_GRADIENT_HEIGHT);
  const tmpResult35 = channel(4458);
  const token5 = channel(4458).useToken(tmp6(576).modules.mobile.CHAT_INPUT_FLOATING_SCRIM_GRADIENT_HEIGHT_AT_BOTTOM);
  const tmpResult36 = channel(4458);
  ...
  const typingUserIdsForDisplay = channel(12221).useTypingUserIdsForDisplay(channel.id, 1);
  const tmp25 = closure_22(screenIndex);
  const tmpResult37 = channel(12221);
  let result1 = channel(12221).hasTypingIndicatorContent(channel, typingUserIdsForDisplay, tmp25);
  ...
  const tmpResult38 = channel(12221);
  ...
  const tmp39 = channel(8006).useCanPostPollsInChannel(channel) && null == threadCreationCallback;
  const tmpResult45 = channel(8006);
  sharedValue = channel(4493).useSharedValue(token);
  const tmpResult46 = channel(4493);
  sharedValue1 = channel(4493).useSharedValue(token);
  ...
  const tmp43 = screenIndex(4625)();
  const tmp44 = useVoiceMessagesUIStore((startTimeMillis) => null != startTimeMillis.startTimeMillis);
  let result3 = !tmp21;
  let isAppLauncherEnabled = result3;
  if (null == threadCreationCallback) {
    isAppLauncherEnabled = tmp(9606).getIsAppLauncherEnabled(channel);
    const tmpResult48 = tmp(9606);
  }
  const tmpResult47 = channel(4493);
  ...
  ({ placeholder, accessibilityLabel } = screenIndex(12241)({ channel, isReadonly: !editable, isCreatingThread: null != threadCreationCallback }));
  let tmp48 = screenIndex(12241)({ channel, isReadonly: !editable, isCreatingThread: null != threadCreationCallback });
  class Ze {
    constructor() {
  ...
  const animatedStyle = channel(4493).useAnimatedStyle(Ze);
  ref = obj10.useRef(null);
  const tmpResult50 = channel(4493);
  const refreshChatInputCoachmark = channel(12242).useRefreshChatInputCoachmark({ disabled: !editable });
  let obj3 = { disabled: !editable };
  const tmpResult51 = channel(12242);
  const canUseScheduledMessages = channel(8091).useCanUseScheduledMessages();
  const tmpResult52 = channel(8091);
  const items10 = [memo1];
  ...
  const scheduledMessageDraftCoachmarkState = channel(12243).useScheduledMessageDraftCoachmarkState(obj4);
  ...
  const tmp56 = screenIndex(12244)({ chatInputProps: { analyticsLocations, canUpload, channel, defaultValue: memo, hasAttachmentsToUpload: stateFromStores2, pendingEdit: stateFromStores, pendingReply: stateFromStores1, screenIndex, secondaryTextFieldRef, threadCreationCallback }, chatInputTextFieldHeight: sharedValue1, ref });
  ...
  memo1 = obj10.useMemo(() => {
    let obj = {
      ...
      handleChangeContentSize(nativeEvent) {
        ...
        if (!obj.getIsChatInputHeightWorkletEnabled()) {
          ...
          const result = textFieldHeight.set(channel(12288).getChatInputHeightAnimationTiming(height, sharedValue.get()));
          const tmp2Result = channel(12288);
        }
      },
      ...
      handleMaxHeightChanged() {
        if (!obj.getIsChatInputHeightWorkletEnabled()) {
          ...
          if (0 !== textFieldContentSize) {
            const textFieldHeight = closure_1_13.state.current.textFieldHeight;
            const result = textFieldHeight.set(channel(12288).getChatInputHeightAnimationTiming(textFieldContentSize, sharedValue.get()));
            const tmpResult = channel(12288);
          }
        }
      },
    ...
    obj.handlePressAction = function handlePressAction(arg0, arg1, current2) {
      if (constants.PHOTOS === arg1) {
        const result = channel(4725).triggerHapticFeedback(channel(4725).HapticFeedbackTypes.IMPACT_LIGHT);
        const obj21 = channel(4725);
        ...
        const keyboardType = channel(4625).getKeyboardType();
        if (keyboardType === channel(1610).KeyboardTypes.APP_LAUNCHER) {
          ...
          const keyboardType1 = tmp42(4625).getKeyboardType();
          ...
          const tmp42Result = tmp42(4625);
        }
        const obj24 = channel(4625);
      } else {
        let tmp35 = current2;
        if (tmp.APPS === arg1) {
          const result1 = channel(4725).triggerHapticFeedback(channel(4725).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj12 = channel(4725);
          channel(4937).trackWithMetadata(constants2.APP_LAUNCHER_ENTRYPOINT_BUTTON_CLICKED);
          const obj13 = channel(4937);
          ...
          const result2 = c2(9599).dismissNewActivityIndicator();
          const obj16 = c2(9599);
          if (tmp35 == null) {
            tmp35 = null;
          }
          const result3 = channel(12292).setAppLauncherA11yFocusReturnRef(tmp35);
          ...
          const obj17 = channel(12292);
          const appDMApplication = channel(12453).getAppDMApplication(tmp30.props.current.channel);
          let name;
          ...
          const tmp23Result = channel(12453);
        } else if (tmp.ALL_PHOTOS === arg1) {
          const result4 = channel(4725).triggerHapticFeedback(channel(4725).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj9 = channel(4725);
          const obj20 = {
            channel: closure_1_13.props.current.channel,
            uploadLimit,
            onDismissKeyboard() {
                  return closure_1_0(4623).dismissKeyboard();
                },
            onRestoreKeyboard() {
                  return closure_1_14.handleToggleKeyboard({ type: closure_0(1610).KeyboardTypes.SYSTEM });
                },
            onSelectFiles(items) {
                  closure_0(10901).addImagesFromPicker(closure_1_13.props.current.channel.id, items, closure_0(5346).UploadOrigin.IMAGE_PICKER);
                },
            draftType: registerViewTag.ChannelMessage
          };
          channel(10901).handleViewAllDialog(obj20);
          const obj10 = channel(10901);
        } else if (tmp.CAMERA === arg1) {
          const result5 = channel(4725).triggerHapticFeedback(channel(4725).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj6 = channel(4725);
          const obj23 = {
            channel: closure_1_13.props.current.channel,
          ...
          channel(10901).handleCameraDialog(obj23);
          const obj7 = channel(10901);
        } else if (tmp.NITRO_GIFT === arg1) {
          const result6 = screenIndex(1874).markPotentialBadState();
          const obj = screenIndex(1874);
          const result7 = channel(4725).triggerHapticFeedback(channel(4725).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj2 = channel(4725);
          if (obj3.isAndroid()) {
            tmp5(4623).dismissKeyboard();
            const tmp5Result = tmp5(4623);
          }
          obj3 = channel(1364);
          channel(10901).handleSelectGift(closure_1_13.props.current.analyticsLocations, closure_1_13.chatInput, tmp35);
          const tmp5Result2 = channel(10901);
        } else if (tmp.THREAD === arg1) {
          const result8 = channel(4725).triggerHapticFeedback(channel(4725).HapticFeedbackTypes.IMPACT_LIGHT);
          const obj28 = channel(4725);
          channel(10901).handleSelectThread(closure_1_13.props.current.channel, closure_1_13.chatInput);
          const obj29 = channel(10901);
        }
      }
    };
    obj.handlePollsPress = function handlePollsPress() {
      const result = channel(4725).triggerHapticFeedback(channel(4725).HapticFeedbackTypes.IMPACT_LIGHT);
      const obj = channel(4725);
      ...
      channel(4623).dismissKeyboard();
      const obj4 = channel(4623);
      channel(12454).openCreatePollModal({
        channel: closure_1_13.props.current.channel,
        onCancel() {
          return closure_1_14.handleToggleKeyboard({ type: closure_0(1610).KeyboardTypes.SYSTEM });
        }
    ...
    obj.handleAttachPress = function handleAttachPress() {
      const result = channel(4725).triggerHapticFeedback(channel(4725).HapticFeedbackTypes.IMPACT_LIGHT);
      const obj = channel(4725);
      channel(10901).handleAttachFile({
        channel: closure_1_13.props.current.channel,
        uploadLimit,
        onDismissKeyboard() {
        ...
        onSelectFiles(items) {
          closure_0(10901).addImagesFromPicker(props.props.current.channel.id, items, closure_0(5346).UploadOrigin.FILE_ATTACHMENT);
        }
      });
    };
    obj.handlePressExpression = function handlePressExpression(context) {
      const result = channel(10598).initiateEmojiInteraction(EmojiInteractionPoint.ChatInputExpressionPressed);
      const obj = channel(10598);
    ...
    obj.handleSelectionOrTextChange = function handleSelectionOrTextChange(nativeEvent) {
      ...
      if (closure_1_13.state.current.editId !== editId) {
        ...
        channel(8179).hideContextMenu();
        const obj3 = channel(8179);
      }
    };
    ...
    obj.handleTextOrFocusChange = function handleTextOrFocusChange(text, focused) {
      if (text.length > 0) {
        if (!focused) {
          const maxMessageLength = channel(9424).getMaxMessageLength();
          if (tmp16) {
        ...
        if (isCoachmarkVisible.isOpen()) {
          screenIndex(10916).hideNativeMenu();
          const obj = screenIndex(10916);
        }
  ...
  const tmp62 = screenIndex(12287)({ textFieldHeight: sharedValue1, textFieldMinHeight: sharedValue });
  ...
  const tmpResult54 = channel(12243);
  if (editable) {
    ...
    let result2 = result3;
    if (!tmp21) {
      result2 = tmp(4428).isPremiumGiftingSupported();
      const tmpResult55 = tmp(4428);
    }
    ...
    tmp66Result = tmp66(tmp6(12492), obj7);
  ...
  const tmp67 = closure_37(screenIndex(12491), { canUpload, channelId: channel.id, screenIndex });
  obj9.customKeyboard = channel(12509).PORTAL_KEYBOARD_PLACEHOLDER_INSTANCE;
  ...
  const items19 = [closure_37(screenIndex(12508), obj9), closure_37(screenIndex(12510), { keyboardType: tmp43, onSelectKeyboard: memo1.handleToggleKeyboard, ref: tmp56.chatInputCover })];
  obj8.children = items19;
  let obj11 = { keyboardType: tmp43, onSelectKeyboard: memo1.handleToggleKeyboard, ref: tmp56.chatInputCover };
  const tmp6Result7 = screenIndex(12508);
  if (editable) {
    ...
    let tmp66Result17 = tmp66(tmp6(12511), obj12);
    const tmp6Result8 = tmp6(12511);
  } else {
    tmp66Result17 = null;
  }
  let obj13 = { collapsable: false, onLayout: callback2, style: null, children: null };
  const items20 = [screenIndex(12517)({ isCreatingThread: null != threadCreationCallback }), tmp10.overflowVisible, ];
  ...
  if (!result1) {
    ...
    tmp66Result18 = tmp66(tmp(12518).ChatInputScrimGradient, obj14);
  }
  ...
  items21[2] = closure_37(channel(12519).ChatInputAccessibilityDivider, {});
  ...
  if (tmp22) {
    ...
    tmp66Result20 = tmp66(tmp6(12520), obj16);
    const tmp6Result9 = tmp6(12520);
  }
  ...
  if (isResourceChannel) {
    let obj21 = { channel };
    tmp66Result24 = tmp66(tmp6(12540), obj21, channel.id);
  }
  items21[5] = tmp66Result24;
  items21[6] = closure_37(channel(12545).MemberActionsChatInputBannerGuardedOuter, { channel });
  items21[7] = closure_37(channel(12548).DoubleTapToReactChatInputBanner, { channel });
  let tmp66Result25 = null;
  if (tmp23) {
    let obj22 = { channelId: channel.id };
    tmp66Result25 = tmp66(tmp6(12641), obj22);
  }
  ...
  if (tmp43 !== channel(1610).KeyboardTypes.EXPRESSION) {
    ...
    tmp66Result26 = tmp66(tmp6(12643), obj23);
  }
  items21[9] = tmp66Result26;
  items21[10] = closure_37(screenIndex(12663), { ref: tmp56.chatInputAppCommandManager, canOnlyUseTextCommands: null != stateFromStores1, channel, chatInputRef: tmp56.chatInput, chatInputStateRef: tmp56.state, commandsDisabled: tmp35 });
  ...
  items24[1] = closure_37(screenIndex(12666), { channel });
  ...
  const items26 = [closure_37(screenIndex(12679), { channel, chatInputRef: tmp56.chatInput, pendingEdit: stateFromStores, pendingReply: stateFromStores1 }), , ];
  ...
  if (tmp66Result27) {
    ...
    tmp66Result27 = tmp66(tmp(12683).EmojiSuggestionBarLarge, obj28);
  }
  ...
  obj32.style = items28;
  const items29 = [tmp74, closure_37(screenIndex(12687), { analyticsLocations, ref: tmp56.chatInputCharCounter })];
  ...
  if (editable) {
    ...
    if (!tmp21) {
      result3 = tmp(4428).isPremiumGiftingSupported();
      const tmpResult58 = tmp(4428);
    }
    ...
    obj35.children = tmp66(tmp6(12688), obj38);
    tmp66Result29 = tmp66(tmp77, obj35);
    const tmp6Result11 = tmp6(12688);
  }
  ...
  if (tmp66Result30) {
    ...
    tmp66Result30 = tmp66(tmp(12689).EmojiSuggestionBarSmall, obj39);
  }
  items24[3] = tmp66Result30;
  obj25.children = closure_38(closure_39, { children: items24 });
  items21[11] = closure_37(screenIndex(12665), obj25);
  ...
  if (null != refreshChatInputCoachmark) {
    ...
    tmp66Result31 = tmp66(tmp6(12242), obj40);
    const tmp6Result12 = tmp6(12242);
  }
  items21[12] = tmp66Result31;
  items21[13] = closure_37(screenIndex(12690), { buttonRef: ref, isVisible: isCoachmarkVisible, onDismiss: dismissCoachmark });
  ...
  if (null == threadCreationCallback) {
    ...
    tmp66Result32 = tmp66(tmp6(12691), obj41);
  }
