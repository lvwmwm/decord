...
// Function ID: 18340
// Name: OverviewScreen
// Dependencies: [19, 21, 4757, 18340, 1484, 18341, 8697, 8698, 5185, 576, 4753, 1115, 2776, 5906, 8859, 18335, 12185, 18342, 11268, 2]
// Exports: default

// Module 18339 (OverviewScreen)
import SafetyFlowsUtils from "SafetyFlowsUtils" /* 18341 */;
...
export default function OverviewScreen() {
  let tmp = closure_6();
  task = task(18340).useSafetyFlowTask().task;
  let obj = task(18340);
  ...
  const items1 = [closure_4(task(4753).Text, obj4), , ];
  ...
  items1[1] = closure_4(task(4753).Text, obj5);
  ...
  if (flow_context != null) {
    ...
    if (tasks != null) {
      mapped = tasks.map((task_type, index) => {
        const obj = { tip: task(18335).TASK_TYPE_TO_TITLE[task_type.task_type], index: index + 1 };
  ...
  const items2 = [closure_4(task(5906).TableRowGroup, { hasIcons: true, children: mapped }), ];
  const obj9 = { variant: "text-xs/medium", color: "text-muted", children: null };
  const intl3 = tmp2(1115).intl;
  obj9.children = intl3.string(navigation(2776)["0TnUrG"]);
  items2[1] = closure_4(task(4753).Text, obj9);
  obj6.children = items2;
  items1[2] = closure_5(task(5185).Stack, obj6);
  obj3.children = items1;
  obj8.children = closure_5(task(5185).Stack, obj3);
  const items3 = [closure_4(task(8698).ModalContent, obj8), ];
  const obj10 = { children: null };
  const items4 = [closure_4(navigation(18342), {}), ];
  const obj11 = { variant: "primary", text: null, onPress: null };
  const intl4 = tmp2(1115).intl;
  obj11.text = intl4.string(navigation(2776).Ks6opt);
  obj11.onPress = callback;
  items4[1] = closure_4(task(11268).ModalActionButton, obj11);
  obj10.children = items4;
  items3[1] = closure_5(task(12185).ModalFooter, obj10);
  obj7.children = items3;
  return closure_5(task(8697).ModalScreen, obj7);
};
