...
// Dependencies: [32, 19, 4744, 4745, 8225, 4749, 8223, 504, 2]
// Exports: getLegacyExperiments, useLegacyExperiments

// Module 11821 (useLegacyExperiments)
  ...
  function _loop(type) {
    ...
    obj.variants = buckets.map((item, index) => {
      ...
      if (typeof type.description === "object") {
        ...
        experimentBucketName = closure_1(8223).getExperimentBucketName(item);
        const obj3 = closure_1(8223);
      }
      obj.label = experimentBucketName;
      obj.shortLabel = closure_1(8223).getExperimentBucketName(item);
      if (item === constants.CONTROL) {
        let TREATMENT = obj(8225).Variation_Type.CONTROL;
      } else if (item === tmp4.NOT_ELIGIBLE) {
        TREATMENT = obj(8225).Variation_Type.UNSPECIFIED;
      } else {
        TREATMENT = obj(8225).Variation_Type.TREATMENT;
      }
      obj.type = TREATMENT;
      return obj;
    });
