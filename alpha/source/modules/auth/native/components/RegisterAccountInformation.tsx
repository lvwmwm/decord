...
// Dependencies: [5, 32, 19, 17, 6922, 4811, 9095, 16341, 16342, 1074, 21, 4827, 576, 4559, 7273, 16338, 504, 16364, 16366, 16349, 1115, 16357, 16340, 16356, 7301, 16367, 16368, 5271, 7270, 5881, 2]
// Exports: default

// Module 16363 (RegisterAccountInformation)
...
import AuthFormViewDefault from "AuthFormView" /* 7301 */;
import useInitialRegistrationStepDefault from "useInitialRegistrationStep" /* 16356 */;
import useAuthFlowBackHandlerDefault from "useAuthFlowBackHandler" /* 16357 */;
...
obj2.layout = LinearTransition.easing(Easing.inOut(fn(4559).Easing.quad)).duration(300);
const size = fn(2);
...
export default function RegisterAccountInformation() {
  ...
  const passwordRegistrationStep = context(16364).usePasswordRegistrationStep();
  ({ password, validatePassword: _slicedToArray, setPassword, passwordScore, preventSubmitPassword } = passwordRegistrationStep);
  let obj4 = context(16364);
  const usernameRegistrationStep = context(16366).useUsernameRegistrationStep(AuthStates.REGISTER_ACCOUNT_INFORMATION);
  ...
  const result = context(16349).hasAllRegistrationFieldsCompleted(obj6, { isConsentRequired: true === stateFromStores1 });
  ConsentStore = result;
  ...
  const tmp5Result3 = context(16349);
  const tmpResult = useAuthFlowBackHandlerDefault;
  tmpResult(context(16340).getPreviousRegistrationTransitionStep(AuthStates.REGISTER_ACCOUNT_INFORMATION));
  ...
  const tmp5Result4 = context(16340);
  ...
  const items4 = [closure_17(closure_6, { style: tmp4.container, children: closure_17(context(16367).RegisterUsernameInput, { username, setUsername, onSubmitEditing: callback, usernameStatus, submitBehavior: "submit", autoFocus: true }) }), , ];
  ...
  obj11.children = closure_17(context(16368).RegisterPasswordInput, obj12);
  ...
  const items5 = [closure_17(context(5271).Button, obj14), ];
  let tmp27Result = null;
  if (null != tmp11.message) {
    tmp27Result = null;
    if ("" !== tmp11.message) {
      const obj15 = { style: tmp4.errors, children: tmp11.message };
      tmp27Result = tmp27(tmp(7270), obj15);
    }
  }
  ...
  if (!tmp3) {
    ...
    tmp27Result4 = tmp27(tmp(5881), obj16);
  }
  return tmp27Result4;
};
