...
// Dependencies: [19, 2044, 4464, 2098, 1074, 21, 13489, 4739, 504, 9267, 9264, 5740, 7358, 1115, 8670, 576, 5009, 4796, 5741, 9480, 7386, 6535, 6794, 9037, 2]
// Exports: default

// Module 13501 (GuildActionSheetTabItems)
...
export default function GuildActionSheetTabItems(guild) {
  guild = guild.guild;
  let stateFromStores;
  let canAccessSettings = guild(13489).useGuildActionSheetPermissions(guild).canAccessSettings;
  const total = stateFromStores(4739)(guild.id).total;
  let obj = guild(13489);
  const items = [GuildChannelStore];
  stateFromStores = guild(504).useStateFromStores(items, () => GuildChannelStore.getChannels(guild.id));
  let obj2 = guild(504);
  let shouldRenderInviteResult = guild(9267).shouldRenderInvite(stateFromStores, guild);
  ...
  obj6.icon = closure_12(guild(8670).BoostGemIcon, { color: stateFromStores(576).unsafe_rawColors.GUILD_BOOSTING_PINK });
  ...
  const items2 = [closure_12(guild(7358).IconButton, obj6), , , ];
  if (shouldRenderInviteResult) {
    ...
    shouldRenderInviteResult = tmp7(tmp(7358).IconButton, obj8);
  }
  ...
  obj9.icon = stateFromStores(7386);
  obj9.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.hideActionSheet();
    NotificationSettingsModalActionCreatorsDefault.open(guild.id);
  };
  items2[2] = closure_12(guild(7358).IconButton, obj9);
  if (canAccessSettings) {
    ...
    obj10.icon = tmp3(6794);
    obj10.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      GuildSettingsActionCreatorsDefault.open(guild.id);
    };
    canAccessSettings = tmp7(tmp(7358).IconButton, obj10);
  }
  items2[3] = canAccessSettings;
  obj4.children = items2;
  return tmp6(guild(5740).ButtonGroup, obj4);
};
