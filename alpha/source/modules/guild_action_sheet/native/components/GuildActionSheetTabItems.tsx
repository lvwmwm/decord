...
// Dependencies: [19, 2044, 4462, 2098, 1074, 21, 14328, 4737, 504, 10170, 10167, 5738, 8269, 1115, 9572, 576, 5009, 4796, 5739, 10378, 8297, 7452, 7711, 9939, 2]
// Exports: default

// Module 14340 (GuildActionSheetTabItems)
...
export default function GuildActionSheetTabItems(guild) {
  guild = guild.guild;
  let stateFromStores;
  let canAccessSettings = guild(14328).useGuildActionSheetPermissions(guild).canAccessSettings;
  const total = stateFromStores(4737)(guild.id).total;
  let obj = guild(14328);
  const items = [GuildChannelStore];
  stateFromStores = guild(504).useStateFromStores(items, () => GuildChannelStore.getChannels(guild.id));
  let obj2 = guild(504);
  let shouldRenderInviteResult = guild(10170).shouldRenderInvite(stateFromStores, guild);
  ...
  let obj3 = guild(10170);
  const tmp6 = closure_13;
  obj6.icon = closure_12(guild(9572).BoostGemIcon, { color: stateFromStores(576).unsafe_rawColors.GUILD_BOOSTING_PINK });
  ...
  const items2 = [closure_12(guild(8269).IconButton, obj6), , , ];
  if (shouldRenderInviteResult) {
    ...
    obj8.icon = tmp3(10378);
    obj8.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      closure_2();
    };
    shouldRenderInviteResult = tmp7(tmp(8269).IconButton, obj8);
  }
  ...
  obj9.icon = stateFromStores(8297);
  obj9.onPress = function onPress() {
    ActionSheetActionCreatorsDefault.hideActionSheet();
    NotificationSettingsModalActionCreatorsDefault.open(guild.id);
  };
  items2[2] = closure_12(guild(8269).IconButton, obj9);
  if (canAccessSettings) {
    ...
    obj10.icon = tmp3(7711);
    obj10.onPress = function onPress() {
      ActionSheetActionCreatorsDefault.hideActionSheet();
      GuildSettingsActionCreatorsDefault.open(guild.id);
    };
    canAccessSettings = tmp7(tmp(8269).IconButton, obj10);
  }
  items2[3] = canAccessSettings;
  obj4.children = items2;
  return tmp6(guild(5738).ButtonGroup, obj4);
};
