...
// Dependencies: [32, 2041, 4774, 4779, 502, 5497, 2042, 4797, 2096, 5638, 1372, 9623, 4776, 4778, 1074, 3, 9622, 12, 38, 10001, 4809, 504, 573, 2]

// Module 4773 (ChannelRTCStore)
...
function updateParticipant(arg0, arr) {
  const f80065 = (dependencyMap) => dependencyMap.updateParticipant(f80065);
      ...
      if (0 !== obj.size()) {
        ...
        if (!Boolean(isGuildVocalOrThreadResult)) {
          ...
          if (!hasEmbeddedActivityResult) {
            hasEmbeddedActivityResult = obj.size(tmp21(9622).ChannelRTCParticipantsIndexes.VIDEO) > 0;
          }
          if (!hasEmbeddedActivityResult) {
      ...
      if (0 !== obj2.size()) {
        if (voiceChannelId.getVoiceChannelId() === item) {
          ...
          if (null != found) {
            ...
            id = id1;
            if (1 !== obj2.size()) {
              if (1 === obj2.size(tmp67(9622).ChannelRTCParticipantsIndexes.VIDEO)) {
                id = _slicedToArray(obj2.toArray(tmp67(9622).ChannelRTCParticipantsIndexes.VIDEO), 1)[0].id;
              } else {
...
function handleEmbeddedActivityChange() {
  const f80067 = (updateEmbeddedActivities) => updateEmbeddedActivities.updateEmbeddedActivities();
...
function handleSpeaking(arg0) {
  const f80068 = (updateParticipantSpeaking) => updateParticipantSpeaking.updateParticipantSpeaking(f80068);
  return closure_25.reduce((acc, item) => {
    let tmp4 = dependencyMap2[item];
    if (null == tmp4) {
      const tmp10 = new ChannelRTCParticipantsDefault(item);
...
function handleCallUpdate(channelId) {
  ...
  const f80069 = (rebuild) => rebuild.rebuild();
...
function handleStreamClose(streamKey) {
  obj = f80065(4809);
  const items = [];
  ({ channelId: arr[0], ownerId: f80065 } = f80065(4809).decodeStreamKey(streamKey.streamKey));
  f80065 = (dependencyMap) => dependencyMap.updateParticipant(f80065);
  return items.reduce((acc, item) => {
    let tmp4 = dependencyMap2[item];
    if (null == tmp4) {
      const tmp10 = new ChannelRTCParticipantsDefault(item);
...
const channelRTCStore = new ChannelRTCStore(DispatcherDefault, {
  ...
  VOICE_CHANNEL_SELECT: function handleVoiceChannelSelect(arg0) {
    ...
    if (channelId !== currentVoiceChannelId) {
      ...
      if (null != currentVoiceChannelId) {
        ...
        const item = guildRingingUsers.forEach((item) => f80072.updateGuildRingingUsers(item, false));
        const items = [currentVoiceChannelId];
        f80072 = (rebuild) => rebuild.rebuild();
    ...
    return voiceStates.reduce((acc, item) => {
      ...
      if (tmp) {
        ...
        if (obj.getCurrentConfig(obj2, { autoTrackExposure: false }).enabled) {
          ...
          if (null == obj3) {
            const tmp8 = new tmp2(9622)(channelId);
            tmp4[channelId] = tmp8;
            obj3 = tmp8;
          }
  ...
  CHANNEL_RTC_SELECT_PARTICIPANT: function handleSelectParticipant(arg0) {
    ...
    if (null == id) {
      const item = obj.toArray(obj(9622).ChannelRTCParticipantsIndexes.STREAM).forEach((user) => {
        if (collapsedCategories(user)) {
          obj.updateParticipant(user.user.id);
        }
      });
      const toArrayResult = obj.toArray(obj(9622).ChannelRTCParticipantsIndexes.STREAM);
    }
    let NONE = id;
    if (id == null) {
    ...
    if (obj2.isStreamKey(id)) {
      try {
        const ownerId = obj(4809).decodeStreamKey(id).ownerId;
  ...
  STREAM_UPDATE_SELF_HIDDEN: function handleUpdateSelfStreamHidden(channelId) {
    ...
    if (channelId.selfStreamHidden) {
      ...
      const obj3 = f80065(4809);
      if (tmp12) {
        setSelectedParticipantId(channelId, null);
      }
      tmp12 = f80065(4809).isStreamKey(first) && first.includes(id);
    }
    const items1 = [channelId];
    f80065 = (dependencyMap) => dependencyMap.updateParticipant(f80065);
    const reduced = items1.reduce((acc, item) => {
  ...
  RTC_CONNECTION_VIDEO: function handleRTCConnectionVideo(arg0) {
    const items = [];
    ({ channelId: arr[0], userId: f80065 } = arg0);
    f80065 = (dependencyMap) => dependencyMap.updateParticipant(f80065);
    return items.reduce((acc, item) => {
      let tmp4 = dependencyMap2[item];
      if (null == tmp4) {
  ...
  MEDIA_ENGINE_VIDEO_SOURCE_QUALITY_CHANGED: function handleVideoSourceQuality(channelId) {
    ({ senderUserId: f80075, maxResolution: importDefault, maxFrameRate: dependencyMap } = channelId);
    const items = [channelId.channelId];
    f80075 = (updateParticipantQuality) => updateParticipantQuality.updateParticipantQuality(f80075, importDefault, dependencyMap);
    return items.reduce((acc, item) => {
      let tmp4 = dependencyMap2[item];
      if (null == tmp4) {
        const tmp10 = new ChannelRTCParticipantsDefault(item);
  ...
  PUSH_TO_TALK_STATE_CHANGE: function handlePushToTalkStateChange() {
    const f80076 = (updateParticipantSpeaking) => updateParticipantSpeaking.updateParticipantSpeaking(id.getId());
    return closure_25.reduce((acc, item) => {
      let tmp4 = dependencyMap2[item];
      if (null == tmp4) {
        const tmp10 = new ChannelRTCParticipantsDefault(item);
  ...
  GUILD_RING_START: function handleGuildRingStart(channelId) {
    ...
    if (obj.getCurrentConfig({ guildId, location: "guild_ring_start" }).enabled) {
      ...
      const item = keys.forEach((item) => f80078.updateGuildRingingUsers(item, true));
      const items = [channelId];
      f80078 = (rebuild) => rebuild.rebuild();
      return items.reduce((acc, item) => {
        let tmp4 = dependencyMap2[item];
        if (null == tmp4) {
          const tmp10 = new ChannelRTCParticipantsDefault(item);
  ...
  GUILD_RING_STOP: function handleGuildRingStop(guildId) {
    ...
    if (obj.getCurrentConfig({ guildId: guildId.guildId, location: "guild_ring_start" }).enabled) {
      ...
      const item = ringing.forEach((item) => f80080.updateGuildRingingUsers(item, false));
      const items = [channelId];
      f80080 = (rebuild) => rebuild.rebuild();
      return items.reduce((acc, item) => {
        let tmp4 = dependencyMap2[item];
        if (null == tmp4) {
          const tmp10 = new ChannelRTCParticipantsDefault(item);
