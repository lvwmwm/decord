...
// Function ID: 12781
// Name: QuestEmbed
// Dependencies: [17, 1182, 7111, 5751, 1074, 7150, 1364, 1115, 7132, 10737, 5754, 4682, 10676, 1609, 12781, 7107, 7126, 7130, 7382, 2]
// Exports: createQuestsEmbed

// Module 12780 (QuestEmbed)
import _mod17 from "module_17" /* 17 */;
import Constants from "Constants" /* 1074 */;
import Constants2 from "Constants" /* 7150 */;
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  if (obj.isMetaQuest()) {
    ...
    obj2.thumbnailUrl = Image.resolveAssetSource(tmp(12781)).uri;
    ...
    const result = tmp2(7107).findQuestOrReplacement(questId, QuestStore.quests, QuestStore.excludedQuests);
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7126).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1115).intl;
        const tmp2Result18 = tmp2(7126);
        ...
        obj8.thumbnailUrl = Image.resolveAssetSource(tmp(12781)).uri;
      ...
      const result2 = tmp2(7126).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1115).intl;
      const tmp2Result20 = tmp2(7126);
      ...
      obj10.thumbnailUrl = Image.resolveAssetSource(tmp(12781)).uri;
      ...
      if (tmp2Result22.isShareableQuest(result.config)) {
        ...
        const tmp2Result23 = tmp2(4682);
        const intl3 = tmp2(1115).intl;
        const obj12 = { questName: result.config.messages.questName };
        const gamePublisher = result.config.messages.gamePublisher;
        const _Date2 = Date;
        const tmp29 = tmp2(4682).isThemeDark(ThemeStore.getState().theme) ? ThemeTypes.DARK : ThemeTypes.LIGHT;
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7132).getQuestTaskDetails(result);
          const tmp2Result24 = tmp2(7132);
          const thirdPartyTaskDetails = tmp2(7132).getThirdPartyTaskDetails(result);
          const tmp2Result25 = tmp2(7132);
          const obj15 = { quest: result, taskDetails: questTaskDetails, location: constants.EMBED_MOBILE, sourceQuestContent: tmp2(5754).QuestContent.QUEST_EMBED_MOBILE, thirdPartyTaskDetails, withoutMarkdown: true, currentUser };
          formatToPlainStringResult1 = tmp2(10737).getQuestsInstructionsToWinReward(obj15);
          const tmp2Result26 = tmp2(10737);
        }
        ...
        obj16.thumbnailUrl = tmp2(10676).getQuestAsset(result, tmp2(10676).QuestAssetType.GAME_TILE, tmp29).url;
        obj16.embedCanBeTapped = true;
        obj16.canBeAccepted = true;
        obj16.type = InviteTypes.GUILD;
        const tmp2Result27 = tmp2(10676);
        const obj17 = {};
        ...
        const result3 = tmp2(7126).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1115).intl;
        const tmp2Result29 = tmp2(7126);
        ...
        obj20.thumbnailUrl = Image.resolveAssetSource(tmp(12781)).uri;
      ...
      tmp2Result22 = tmp2(7130);
    }
    const tmp2Result16 = tmp2(7107);
  }
};
