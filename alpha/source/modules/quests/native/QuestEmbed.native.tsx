...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  if (obj.isMetaQuest()) {
    ...
    const result = tmp2(7112).findQuestOrReplacement(questId, QuestStore.quests, QuestStore.excludedQuests);
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(7131).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1115).intl;
        const tmp2Result18 = tmp2(7131);
      ...
      const result2 = tmp2(7131).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1115).intl;
      const tmp2Result20 = tmp2(7131);
      ...
      if (tmp2Result22.isShareableQuest(result.config)) {
        ...
        const tmp2Result23 = tmp2(4685);
        const intl3 = tmp2(1115).intl;
        const obj12 = { questName: result.config.messages.questName };
        const gamePublisher = result.config.messages.gamePublisher;
        const _Date2 = Date;
        const tmp29 = tmp2(4685).isThemeDark(ThemeStore.getState().theme) ? ThemeTypes.DARK : ThemeTypes.LIGHT;
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(7137).getQuestTaskDetails(result);
          const tmp2Result24 = tmp2(7137);
          const thirdPartyTaskDetails = tmp2(7137).getThirdPartyTaskDetails(result);
          const tmp2Result25 = tmp2(7137);
          const obj15 = { quest: result, taskDetails: questTaskDetails, location: constants.EMBED_MOBILE, sourceQuestContent: tmp2(5759).QuestContent.QUEST_EMBED_MOBILE, thirdPartyTaskDetails, withoutMarkdown: true, currentUser };
          formatToPlainStringResult1 = tmp2(10750).getQuestsInstructionsToWinReward(obj15);
          const tmp2Result26 = tmp2(10750);
        }
        ...
        obj16.thumbnailUrl = tmp2(10689).getQuestAsset(result, tmp2(10689).QuestAssetType.GAME_TILE, tmp29).url;
        obj16.embedCanBeTapped = true;
        obj16.canBeAccepted = true;
        obj16.type = InviteTypes.GUILD;
        const tmp2Result27 = tmp2(10689);
        const obj17 = {};
        ...
        const result3 = tmp2(7131).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1115).intl;
        const tmp2Result29 = tmp2(7131);
      ...
      tmp2Result22 = tmp2(7135);
    }
    const tmp2Result16 = tmp2(7112);
  }
};
