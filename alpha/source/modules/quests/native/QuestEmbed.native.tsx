...
// Function ID: 13627
// Name: QuestEmbed
// Dependencies: [17, 1182, 8026, 5749, 1074, 8065, 1364, 1115, 8047, 12116, 5752, 4680, 11634, 1609, 13627, 8022, 8041, 8045, 8293, 2]
// Exports: createQuestsEmbed

// Module 13626 (QuestEmbed)
...
export const createQuestsEmbed = function createQuestsEmbed(questId) {
  ...
  if (obj.isMetaQuest()) {
    ...
    obj2.thumbnailUrl = Image.resolveAssetSource(tmp(13627)).uri;
    ...
    const result = tmp2(8022).findQuestOrReplacement(questId, QuestStore.quests, QuestStore.excludedQuests);
    ...
    if (null == result) {
      if (null == value) {
        const result1 = tmp2(8041).trackQuestEmbedFallbackViewed(questId, constants2.UNKNOWN_QUEST);
        const intl12 = tmp2(1115).intl;
        const tmp2Result18 = tmp2(8041);
        ...
        obj8.thumbnailUrl = Image.resolveAssetSource(tmp(13627)).uri;
      ...
      const result2 = tmp2(8041).trackQuestEmbedFallbackViewed(questId, constants2.EXCLUDED_QUEST);
      const intl7 = tmp2(1115).intl;
      const tmp2Result20 = tmp2(8041);
      ...
      obj10.thumbnailUrl = Image.resolveAssetSource(tmp(13627)).uri;
      ...
      if (tmp2Result22.isShareableQuest(result.config)) {
        ...
        const tmp2Result23 = tmp2(4680);
        const intl3 = tmp2(1115).intl;
        const obj12 = { questName: result.config.messages.questName };
        const gamePublisher = result.config.messages.gamePublisher;
        const _Date2 = Date;
        const tmp29 = tmp2(4680).isThemeDark(ThemeStore.getState().theme) ? ThemeTypes.DARK : ThemeTypes.LIGHT;
        ...
        if (result.config.expiresAt < date1.toISOString()) {
          ...
          const questTaskDetails = tmp2(8047).getQuestTaskDetails(result);
          const tmp2Result24 = tmp2(8047);
          const thirdPartyTaskDetails = tmp2(8047).getThirdPartyTaskDetails(result);
          const tmp2Result25 = tmp2(8047);
          const obj15 = { quest: result, taskDetails: questTaskDetails, location: constants.EMBED_MOBILE, sourceQuestContent: tmp2(5752).QuestContent.QUEST_EMBED_MOBILE, thirdPartyTaskDetails, withoutMarkdown: true, currentUser };
          formatToPlainStringResult1 = tmp2(12116).getQuestsInstructionsToWinReward(obj15);
          const tmp2Result26 = tmp2(12116);
        }
        ...
        obj16.thumbnailUrl = tmp2(11634).getQuestAsset(result, tmp2(11634).QuestAssetType.GAME_TILE, tmp29).url;
        obj16.embedCanBeTapped = true;
        obj16.canBeAccepted = true;
        obj16.type = InviteTypes.GUILD;
        const tmp2Result27 = tmp2(11634);
        const obj17 = {};
        ...
        const result3 = tmp2(8041).trackQuestEmbedFallbackViewed(questId, constants2.NOT_SHAREABLE_QUEST);
        const intl = tmp2(1115).intl;
        const tmp2Result29 = tmp2(8041);
        ...
        obj20.thumbnailUrl = Image.resolveAssetSource(tmp(13627)).uri;
      ...
      tmp2Result22 = tmp2(8045);
    }
    const tmp2Result16 = tmp2(8022);
  }
};
