...
// Dependencies: [5, 32, 19, 17, 7942, 17507, 5663, 21, 4757, 576, 1365, 4691, 4465, 504, 7415, 7963, 11742, 12032, 5666, 1397, 11546, 7967, 4960, 4475, 12031, 15393, 5185, 5806, 12027, 4753, 1115, 5187, 5836, 7622, 7239, 12035, 11469, 2]
// Exports: default

// Module 17508 (QuestActivityUnenrolledModal)
...
function QuestActivityUnenrolledModalInner(quest) {
  ...
  const theme = quest(4691).useTheme();
  let obj = quest(4691);
  const tmp4 = closure_14(quest(4465).isThemeDark(theme));
  let obj2 = quest(4465);
  ...
  let obj3 = quest(504);
  let obj4 = quest(7415);
  const getOrFetchApplication = obj4.useGetOrFetchApplication(quest(7963).getActivityApplicationId(quest));
  const obj5 = quest(7963);
  const questTaskDetails = quest(11742).useQuestTaskDetails(quest);
  let obj6 = quest(11742);
      ...
      try {
        ...
        if (0 === v1) {
          if (arg0 === 1) {
            ...
            const obj4 = { questContent: tmp4(5666).QuestContent.QUEST_ACTIVITY_UNENROLLED_MODAL, questContentCTA: tmp4(7967).QuestContentCTA.START_QUEST, sourceQuestContent: tmp4(5666).QuestContent.QUEST_ACTIVITY_UNENROLLED_MODAL };
            v1 = 1;
            dependencyMap = 1;
            const obj6 = { value: tmp4(11546).enrollInQuest(quest.id, obj4), done: false };
            return obj6;
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
  ...
  const checkboxA11yNative = quest(4475).useCheckboxA11yNative({ checked });
  ({ accessibilityRole, accessibilityState } = checkboxA11yNative);
  const obj9 = quest(4475);
  const isQuestAccessSuspended = quest(11742).useIsQuestAccessSuspended();
  const obj10 = quest(11742);
  trackQuestContentClickedWithImpression = quest(12031).useTrackQuestContentClickedWithImpression();
  ...
  if (tmp18Result) {
    ...
    obj16.children = tmp18(checked(5806), obj17);
  ...
  obj19.children = closure_11(checked(12027), size);
  ...
  const items6 = [closure_11(quest(4753).Text, obj21), ];
  ...
  items6[1] = closure_12(quest(4753).Text, obj22);
  ...
  const items9 = [closure_11(quest(5187).Button, obj26), ];
  ...
  items9[1] = closure_11(quest(5187).Button, obj27);
  ...
  if (null == stateFromStores) {
    ...
    const obj3 = {
      ...
      headerTitle() {
          ...
          return closure_1_11(questId(4753).Text, obj);
        },
      render() {
    ...
    return closure_11(tmp(11469).Modal, obj4);
  }
  let obj = questId(504);
  tmp = questId;
};
