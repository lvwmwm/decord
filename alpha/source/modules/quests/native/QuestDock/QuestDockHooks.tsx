...
// Function ID: 15360
// Name: QuestDockHooks
// Dependencies: [32, 19, 15360, 5661, 15362, 504, 15363, 15366, 11616, 15364, 15367, 4492, 9664, 15361, 1091, 11741, 15369, 7965, 7966, 7976, 7964, 5664, 5668, 7954, 4723, 15370, 1980, 1478, 11548, 8116, 4457, 576, 2]
  ...
  const fn2 = function t(safeAreaState, restingQuestDockMode) {
    if (!obj.cheapWorkletShallowEqual(safeAreaState, restingQuestDockMode)) {
      ...
      if (constants.RESET_TO_PREVIOUS === restingQuestDockMode) {
        ...
        obj2.x = 0;
        obj2.y = 0;
        obj2.width = tmp(15361).getQuestDockCollapsedWidth(windowWidth, youBarHorizontalMargin, youBarHorizontalMargin);
        obj2.height = height;
        const result = questDockWrapperSpecs.set(obj2);
        const result1 = activeQuestDockMode.set(tmp5.COLLAPSED);
        const tmpResult6 = tmp(15361);
      } else {
        if (tmp5.CLOSED !== restingQuestDockMode) {
          if (tmp5.SOFT_DISMISSED !== restingQuestDockMode) {
            if (tmp5.EXPANDED === restingQuestDockMode) {
              const questDockExpandedHeightLimits = tmp(15361).getQuestDockExpandedHeightLimits(tmp4, safeArea.top, minExpandedContentHeight);
              ...
              if (!tmp7) {
                ...
                if (tmp6) {
                  tmp6 = obj11.get().height === tmp(15361).getQuestDockExpandedHeightLimits(restingQuestDockMode.windowHeight, restingQuestDockMode.safeArea.top, minExpandedContentHeight).maxHeight;
                  const tmpResult8 = tmp(15361);
                }
                tmp7 = tmp6;
              }
              if (!tmp7) {
              ...
              obj3.y = tmp10;
              const tmpResult7 = tmp(15361);
              obj3.width = tmp(15361).getQuestDockExpandedWidth(windowWidth, safeArea.left, safeArea.right);
        ...
        obj4.x = 0;
        obj4.y = 0;
        obj4.width = tmp(15361).getQuestDockClosedWidth(windowWidth, youBarHorizontalMargin, youBarHorizontalMargin);
        obj4.height = height;
        const result4 = questDockWrapperSpecs.set(obj4);
        const result5 = activeQuestDockMode.set(restingQuestDockMode);
        const tmpResult10 = tmp(15361);
      }
    }
  };
...
export const useActionSheetPressHandler = function useActionSheetPressHandler(questCreative) {
  ...
  return noop.useCallback(() => {
    ...
    if (obj2.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_dock_action_sheet")) {
      const obj3 = { type: tmp(7976).AdUserActionType.CLICK_INTERNAL };
      const merged = Object.assign(creativeAnalyticsParams);
      obj3.questContentCTA = tmp(7964).QuestContentCTA.OPEN_CONTEXT_MENU;
      obj3.surfaceId = tmp(5664).QuestContent.QUEST_BAR_MOBILE;
      obj3.sourceQuestContent = tmp(5664).QuestContent.QUEST_BAR_MOBILE;
      obj3.impressionId = questImpressionId;
      tmp(7966).captureAdUserAction(obj3);
      let tmp7 = questImpressionId;
      const tmpResult = tmp(7966);
    } else if (creativeAnalyticsParams.adCreativeType === tmp(5668).AdCreativeType.QUEST) {
      const obj4 = { questId: creativeAnalyticsParams.adCreativeId, questContent: tmp(5664).QuestContent.QUEST_BAR_MOBILE, questContentCTA: tmp(7964).QuestContentCTA.OPEN_CONTEXT_MENU, sourceQuestContent: tmp(5664).QuestContent.QUEST_BAR_MOBILE, impressionId: questImpressionId };
      const result = tmp(7954).trackQuestContentClicked(obj4);
      tmp7 = questImpressionId;
      const tmpResult3 = tmp(7954);
    } else {
      ...
      obj5.questContent = tmp(5664).QuestContent.QUEST_BAR_MOBILE;
      obj5.questContentCTA = tmp(7964).QuestContentCTA.OPEN_CONTEXT_MENU;
      obj5.sourceQuestContent = tmp(5664).QuestContent.QUEST_BAR_MOBILE;
      tmp7 = questImpressionId;
      obj5.impressionId = questImpressionId;
      const result1 = tmp(7954).trackAdContentClicked(obj5);
      const tmpResult4 = tmp(7954);
    }
    obj2 = AdAnalyticsInterfaceExperiment;
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(15370, tmp2.paths), "QuestDockContextMenuActionSheet", { creative: tmp3, impressionId: tmp7 });
  }, items);
};
