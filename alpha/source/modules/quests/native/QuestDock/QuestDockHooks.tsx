...
// Function ID: 15368
// Name: QuestDockHooks
// Dependencies: [32, 19, 15368, 5663, 15370, 504, 15371, 15374, 11620, 15372, 15375, 4493, 9670, 15369, 1091, 11745, 15377, 7979, 7968, 7978, 7967, 5666, 5670, 7957, 4724, 15378, 1980, 1478, 11552, 8119, 4458, 576, 2]
  ...
  const fn2 = function t(safeAreaState, restingQuestDockMode) {
    if (!obj.cheapWorkletShallowEqual(safeAreaState, restingQuestDockMode)) {
      ...
      if (constants.RESET_TO_PREVIOUS === restingQuestDockMode) {
        ...
        tmp(4493).runOnJS(setRestingQuestDockMode)(restingQuestDockMode1);
      } else if (tmp5.COLLAPSED === restingQuestDockMode) {
        const obj2 = {};
        const merged = Object.assign(questDockWrapperSpecs.get());
        obj2.x = 0;
        obj2.y = 0;
        obj2.width = tmp(15369).getQuestDockCollapsedWidth(windowWidth, youBarHorizontalMargin, youBarHorizontalMargin);
        obj2.height = height;
        const result = questDockWrapperSpecs.set(obj2);
        const result1 = activeQuestDockMode.set(tmp5.COLLAPSED);
        const tmpResult6 = tmp(15369);
      } else {
        if (tmp5.CLOSED !== restingQuestDockMode) {
          if (tmp5.SOFT_DISMISSED !== restingQuestDockMode) {
            if (tmp5.EXPANDED === restingQuestDockMode) {
              const questDockExpandedHeightLimits = tmp(15369).getQuestDockExpandedHeightLimits(tmp4, safeArea.top, minExpandedContentHeight);
              ...
              if (!tmp7) {
                ...
                if (tmp6) {
                  tmp6 = obj11.get().height === tmp(15369).getQuestDockExpandedHeightLimits(restingQuestDockMode.windowHeight, restingQuestDockMode.safeArea.top, minExpandedContentHeight).maxHeight;
                  const tmpResult8 = tmp(15369);
                }
                tmp7 = tmp6;
              }
              if (!tmp7) {
              ...
              obj3.y = tmp10;
              const tmpResult7 = tmp(15369);
              obj3.width = tmp(15369).getQuestDockExpandedWidth(windowWidth, safeArea.left, safeArea.right);
        ...
        obj4.x = 0;
        obj4.y = 0;
        obj4.width = tmp(15369).getQuestDockClosedWidth(windowWidth, youBarHorizontalMargin, youBarHorizontalMargin);
        obj4.height = height;
        const result4 = questDockWrapperSpecs.set(obj4);
        const result5 = activeQuestDockMode.set(restingQuestDockMode);
        const tmpResult10 = tmp(15369);
      }
    }
  };
...
export const useActionSheetPressHandler = function useActionSheetPressHandler(questCreative) {
  ...
  return noop.useCallback(() => {
    ...
    if (obj2.shouldMigrateToAdAnalyticsInterface(AdAnalyticsInterfaceExperiment.AdAnalyticsInterfaceExperimentStep.STEP_2_CLICKED_INTERNAL, "quest_dock_action_sheet")) {
      ...
      obj3.questContentCTA = tmp(7967).QuestContentCTA.OPEN_CONTEXT_MENU;
      ...
      const obj4 = { questId: creativeAnalyticsParams.adCreativeId, questContent: tmp(5666).QuestContent.QUEST_BAR_MOBILE, questContentCTA: tmp(7967).QuestContentCTA.OPEN_CONTEXT_MENU, sourceQuestContent: tmp(5666).QuestContent.QUEST_BAR_MOBILE, impressionId: questImpressionId };
      const result = tmp(7957).trackQuestContentClicked(obj4);
      tmp7 = questImpressionId;
      const tmpResult3 = tmp(7957);
    } else {
      ...
      obj5.questContentCTA = tmp(7967).QuestContentCTA.OPEN_CONTEXT_MENU;
      obj5.sourceQuestContent = tmp(5666).QuestContent.QUEST_BAR_MOBILE;
      tmp7 = questImpressionId;
      obj5.impressionId = questImpressionId;
      const result1 = tmp(7957).trackAdContentClicked(obj5);
      const tmpResult4 = tmp(7957);
    }
    obj2 = AdAnalyticsInterfaceExperiment;
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(15378, tmp2.paths), "QuestDockContextMenuActionSheet", { creative: tmp3, impressionId: tmp7 });
  }, items);
};
