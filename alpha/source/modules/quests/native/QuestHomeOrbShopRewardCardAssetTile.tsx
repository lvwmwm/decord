...
// Dependencies: [32, 19, 17, 1076, 21, 9049, 576, 4757, 9096, 38, 1973, 9097, 9098, 8444, 9083, 5806, 9129, 1077, 9130, 9085, 9108, 9110, 9050, 7798, 4606, 4458, 2]

// Module 15362 (QuestHomeOrbShopRewardCardAssetTile)
...
import AvatarDecorationSampleV2 from "AvatarDecorationSampleV2" /* 9096 */;
...
function CompactAvatarDecorationPreview(arg0) {
  ...
  const items = [React6(hasOwnProperty, { style: tmp.avatar, resizeMode: "contain", source: _modDef9097, accessible: false }), ];
  const obj2 = { style: tmp.avatar, resizeMode: "contain", source: _modDef9097, accessible: false };
...
function ProductPreviewInner(cardHeight) {
  ...
  const tmp5 = cardWidth < cardWidth(9049).COLLECTIBLES_SHOP_CARD_WIDTH || cardHeight < cardWidth(9049).COLLECTIBLES_SHOP_CARD_HEIGHT;
  dependencyMap = tmp5;
  ...
  if (product.type === cardWidth(1973).CollectiblesItemType.BUNDLE) {
    ...
    return closure_8(cardHeight(9083), obj2);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj3 = { source: null, style: null };
    const obj4 = { uri: cardHeight(9129) };
    obj3.source = obj4;
    obj3.style = tmp.externalProductImage;
    return closure_8(cardHeight(5806), obj3);
  } else {
    const ALL = tmp2(1077).FractionalPremiumSKUsSets.ALL;
    if (ALL.has(product.skuId)) {
      size = { skuId: product.skuId, width: tmp2(9130).FRACTIONAL_NITRO_COIN_SIZE.CARD, height: tmp2(9130).FRACTIONAL_NITRO_COIN_SIZE.CARD };
      return closure_8(tmp2(9130).FractionalNitroCoinIllustration, size);
    } else {
      ...
      if (tmp2(1973).CollectiblesItemType.AVATAR_DECORATION === type) {
        if (flag) {
          ...
          tmp24Result = tmp24(cardHeight(9096), obj6);
        }
        return tmp24Result;
        ...
        obj7.children = closure_8(cardHeight(9085), obj8);
        ...
        if (flag) {
          ...
          if (prop == null) {
            prop = tmp2(9049).COLLECTIBLES_SHOP_CARD_WIDTH - PX_32;
          }
          ...
          obj9.children = tmp14(cardHeight(9108), obj10);
        ...
        return closure_8(cardHeight(9110), obj11);
      } else {
        return null;
      }
    }
