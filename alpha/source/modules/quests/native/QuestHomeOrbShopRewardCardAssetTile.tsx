...
// Dependencies: [32, 19, 17, 1076, 21, 9044, 576, 4756, 9091, 38, 1973, 9092, 9093, 8440, 9078, 5804, 9124, 1077, 9125, 9080, 9103, 9105, 9045, 7795, 4605, 4457, 2]

// Module 15354 (QuestHomeOrbShopRewardCardAssetTile)
...
import useDefaultVariantIndex from "useDefaultVariantIndex" /* 9045 */;
...
function CompactAvatarDecorationPreview(arg0) {
  ...
  const items = [React6(hasOwnProperty, { style: tmp.avatar, resizeMode: "contain", source: _modDef9092, accessible: false }), ];
  const obj2 = { style: tmp.avatar, resizeMode: "contain", source: _modDef9092, accessible: false };
...
function ProductPreviewInner(cardHeight) {
  ...
  let tmp = closure_16();
  const shopProductItems = cardWidth(8440).useShopProductItems(product);
  ({ firstProfileEffect, firstAvatarDecoration, firstNameplate } = shopProductItems);
  const tmp5 = cardWidth < cardWidth(9044).COLLECTIBLES_SHOP_CARD_WIDTH || cardHeight < cardWidth(9044).COLLECTIBLES_SHOP_CARD_HEIGHT;
  dependencyMap = tmp5;
  ...
  if (product.type === cardWidth(1973).CollectiblesItemType.BUNDLE) {
    ...
    return closure_8(cardHeight(9078), obj2);
  } else if (product.skuId === EXTERNAL_PRODUCT_SKU_IDS.ORB_PROFILE_BADGE) {
    const obj3 = { source: null, style: null };
    const obj4 = { uri: cardHeight(9124) };
    obj3.source = obj4;
    obj3.style = tmp.externalProductImage;
    return closure_8(cardHeight(5804), obj3);
  } else {
    const ALL = tmp2(1077).FractionalPremiumSKUsSets.ALL;
    if (ALL.has(product.skuId)) {
      size = { skuId: product.skuId, width: tmp2(9125).FRACTIONAL_NITRO_COIN_SIZE.CARD, height: tmp2(9125).FRACTIONAL_NITRO_COIN_SIZE.CARD };
      return closure_8(tmp2(9125).FractionalNitroCoinIllustration, size);
    } else {
      ...
      if (tmp2(1973).CollectiblesItemType.AVATAR_DECORATION === type) {
        if (flag) {
          ...
          tmp24Result = tmp24(cardHeight(9091), obj6);
        }
        return tmp24Result;
        ...
        obj7.children = closure_8(cardHeight(9080), obj8);
        ...
        if (flag) {
          ...
          if (prop == null) {
            prop = tmp2(9044).COLLECTIBLES_SHOP_CARD_WIDTH - PX_32;
          }
          ...
          obj9.children = tmp14(cardHeight(9103), obj10);
        ...
        return closure_8(cardHeight(9105), obj11);
      } else {
        return null;
      }
    }
...
let c10 = 100;
const diff = fn(9044).COLLECTIBLES_SHOP_CARD_HEIGHT - 2 * nativeDefault.space.PX_16;
