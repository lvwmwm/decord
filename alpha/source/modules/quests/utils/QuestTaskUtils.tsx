...
export const getQuestTaskDetails = function getQuestTaskDetails(value, DESKTOP) {
  if (arr.some((item) => null != value.config.taskConfigV2.tasks[item])) {
    ...
    if (closure_133_0.some((item) => null != config.config.taskConfigV2.tasks[item])) {
      ...
      if (closure_136_0.some((item) => null != config.config.taskConfigV2.tasks[item])) {
        ...
        const tmp15 = config.taskConfigV2.tasks[tmp(undefined, 5764).FirstPartyQuestTaskTypes.WATCH_VIDEO_ON_MOBILE];
        if (null == tmp14) {
          let tmp18 = tmp15;
          if (tmp15 == null) {
            tmp18 = tmp14;
        ...
        let tmp8 = null != value;
        if (tmp8) {
          tmp8 = null != value.config.taskConfigV2.tasks[tmp(undefined, 5764).FirstPartyQuestTaskTypes.PLAY_ON_DESKTOP];
        }
        if (tmp8) {
          const obj4 = { quest: value, taskType: tmp(5764).FirstPartyQuestTaskTypes.PLAY_ON_DESKTOP };
          tmp13Result = _getTaskDetailsForType(obj4);
        } else if (null != value.config.taskConfigV2.tasks[tmp(undefined, 5764).FirstPartyQuestTaskTypes.PLAY_ACTIVITY]) {
          const obj5 = { quest: value, taskType: tmp(5764).FirstPartyQuestTaskTypes.PLAY_ACTIVITY };
          tmp13Result = _getTaskDetailsForType(obj5);
        } else {
          const obj6 = { quest: value, taskType: tmp(5764).FirstPartyQuestTaskTypes.STREAM_ON_DESKTOP };
  ...
  if (null != tmp13Result) {
    ...
    if (null == tmp26) {
      const obj8 = { progressSeconds: 0, targetSeconds: 1, targetMinutes: 1, percentComplete: 0, taskType: tmp(5764).FirstPartyQuestTaskTypes.STREAM_ON_DESKTOP };
      tmp26 = obj8;
    }
    return tmp26;
  }
