...
// Dependencies: [19, 17, 8639, 21, 4757, 13260, 8609, 11917, 4614, 4493, 4758, 8643, 8644, 1115, 5899, 1177, 576, 4753, 13264, 8178, 1364, 13265, 8185, 2]
// Exports: MediaModalOverlayHeader

// Module 13263 (MediaModalOverlayHeader)
...
import useShouldHideMediaOptionsDefault from "useShouldHideMediaOptions" /* 11917 */;
import noop from "module_19" /* 19 */;

require = fn;
get_ActivityIndicator = fn(17);
({ StyleSheet: c3, View: closure_4 } = get_ActivityIndicator);
const SHARE_PREPARING_MODAL_KEY = fn(8639).SHARE_PREPARING_MODAL_KEY;
const jsxProd = fn(21);
...
export const MediaModalOverlayHeader = function MediaModalOverlayHeader(arg0) {
  ...
  const tmp = closure_8();
  const headerLayoutAnimation = isModalOpen(13260).useHeaderLayoutAnimation(animationDriver);
  let obj = isModalOpen(13260);
  const mediaShareActions = isModalOpen(8609).useMediaShareActions({ source, disableDownload, shareable });
  const tmp7 = useShouldHideMediaOptionsDefault(channelId);
  const obj2 = isModalOpen(8609);
  isModalOpen = isModalOpen(4614).useIsModalOpen(SHARE_PREPARING_MODAL_KEY);
  const obj3 = isModalOpen(4614);
  const fn = function w() {
    let num = 1;
    if (isModalOpen) {
      num = 0;
  ...
  const obj4 = isModalOpen(4493);
  fn.__closure = { withTiming: isModalOpen(4758).withTiming, isPreparing: isModalOpen };
  ...
  const obj5 = { withTiming: isModalOpen(4758).withTiming, isPreparing: isModalOpen };
  let intl = tmp2(1115).intl;
  obj11.accessibilityLabel = intl.string(isModalOpen(1115).t.cpT0Cq);
  obj11.icon = closure_6(isModalOpen(5899).XSmallIcon, { size: "md", color: "interactive-text-active" });
  ...
  if (tmp10Result3) {
    ...
    obj15.children = tmp10(tmp2(4753).Text, obj16);
  ...
  if (!tmp7) {
    ...
    const obj18 = { source };
    const items4 = [tmp10(tmp6(13264), obj18), ];
    ...
    if (!disableMediaOverlayButton) {
      const obj19 = {
        ...
        children(ref) {
              ...
              if (obj2.isAndroid()) {
                let tmp2Result = tmp2(tmp5(13265).MoreVerticalIcon, { size: "md", color: "interactive-text-active" });
              } else {
                tmp2Result = tmp2(tmp5(8185).MoreHorizontalIcon, { size: "md", color: "interactive-text-active" });
              }
              obj.icon = tmp2Result;
              obj.ref = ref.ref;
      ...
      tmp10Result4 = tmp10(tmp2(8178).ContextMenu, obj19);
    }
  ...
  obj8.children = closure_7(isModalOpen(8643).MediaModalOverlayHeaderWrapper, obj9);
