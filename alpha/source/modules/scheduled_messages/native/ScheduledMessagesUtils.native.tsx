...
// Dependencies: [5, 8106, 1115, 4416, 8175, 5032, 12554, 1980, 12564, 8177, 8174, 8178, 7527, 4796, 12054, 2]
...
// Module 12553 (ScheduledMessagesUtils)
...
function openSendTimeActionSheet(title) {
  ...
  const result = title(8175).trackScheduledMessageTimePickerOpened({ entryPoint, isEditing, channelId });
  let obj = title(8175);
  let obj3 = { key, header: { title }, hasIcons: false, options: null };
  let obj2 = title(7527);
  const presetScheduledTimes = title(8175).getPresetScheduledTimes();
  const items = [
  ...
  obj5.onPress = function onPress() {
    ...
    const tmp2 = asyncRequireImpl(12054, dependencyMap.paths);
    ...
    obj3.getError = function getError(isBefore) {
      return title(8175).getScheduledTimeError(isBefore, dependencyMap);
    };
    obj3.createReminder = function createReminder(arg0) {
      return closure_1_3(defaultValue(4416)(arg0));
    };
...
export const openScheduleMessageActionSheet = function openScheduleMessageActionSheet(id, ATTACH_MENU, scheduledTimestamp) {
  ...
  if (null != currentTimestamp) {
    let defaultScheduledTime = _modDef4416(currentTimestamp);
  } else {
...
export const showScheduledMessagesModal = function showScheduledMessagesModal() {
  ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(12554, dependencyMap.paths), {}, "scheduled-messages-modal", { presentation: "modal" });
};
export const openScheduledMessageEditContentModal = function openScheduledMessageEditContentModal(scheduledMessage) {
  ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(12564, dependencyMap.paths), { scheduledMessage }, "scheduled-message-edit-content", { presentation: "modal" });
};
export const openRescheduleMessageActionSheet = function openRescheduleMessageActionSheet(scheduledMessageId, sendAtTimestamp, channelId) {
  ...
  obj.startDate = _modDef4416(sendAtTimestamp);
