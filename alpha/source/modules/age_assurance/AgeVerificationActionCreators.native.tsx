...
// Dependencies: [5, 1372, 8683, 8686, 4965, 8670, 21, 5355, 4959, 8687, 1980, 8675, 8696, 8697, 1609, 5110, 8698, 1380, 1364, 8699, 8700, 4968, 5640, 573, 8701, 8707, 8714, 8850, 8851, 4451, 8684, 8862, 8863, 8864, 2]

// Module 8682 (AgeVerificationActionCreators)
...
import ManualReviewDecidedTeenAlertModalDefault from "ManualReviewDecidedTeenAlertModal" /* 8862 */;
import ManualReviewPendingAlertModalDefault from "ManualReviewPendingAlertModal" /* 8863 */;
import ManualReviewFallbackAlertModalDefault from "ManualReviewFallbackAlertModal" /* 8864 */;
...
function openIncodeAgeVerificationModal(arg0) {
  ...
  if (flag) {
    (async (arg0, value) => {
      closure_0 = tmp3;
      await tmp3(5355).requestPermission(constants.CAMERA, { showAuthorizationError: true });
      if (1 === tmp7) {
        dependencyMap = 0;
        tmp3(8675).showFailedToast(constants2.TIGGER_PAWTECT_ERROR);
        closure_129_0();
        c4 = 3;
        tmp3(8675);
      } else if (2 === tmp7) {
...
const AgeVerificationConstants = fn(8683);
...
let closure_12 = fn(8686).AGE_VERIFICATION_INCODE_PATH;
...
export default {
  ...
  showAgeVerificationGetStartedModal(entryPoint) {
    ...
    UserStore = undefined;
    if (obj.isMetaQuest()) {
      tmp(5110).openAlert(closure_7, jsx(onClose(8698), {}), onClose);
      const tmpResult = tmp(5110);
    } else {
      let isAgeVerifiedResult = tmp(4968).isAgeVerified();
      if (isAgeVerifiedResult) {
        isAgeVerifiedResult = tmp(5640).hasAgeGatedFeatures();
        const tmpResult7 = tmp(5640);
      }
      ...
      if (tmpResult8.isAgeVerificationIncodeEnabled(entryPoint)) {
        ...
        if (tmpResult9.isExpressiveModalV2Enabled(entryPoint)) {
              ...
              try {
                ...
                if (0 === dependencyMap) {
                  if (arg0 === 1) {
                    ...
                    dependencyMap = 1;
                    v3 = 1;
                    const obj5 = { value: entryPoint(8707).shouldShowManualReviewFallback(entryPoint), done: false };
                    return obj5;
                  }
                } else if (arg0 === 1) {
                  v3 = 3;
          ...
          UserStore = tmp(8850).isAgeVerificationExpressiveModalEverywhereEnabled(entryPoint);
          const tmpResult10 = tmp(8850);
