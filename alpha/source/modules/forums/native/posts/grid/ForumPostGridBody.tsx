...
// Dependencies: [32, 19, 17, 21, 4757, 576, 1177, 12264, 12265, 11515, 4753, 1478, 1370, 12266, 7519, 8145, 12270, 2]
// Exports: default

// Module 12263 (ForumPostGridBody)
...
function GIFIcon() {
  const tmp = closure_8();
  return timestampProducer(native.Icon, { size: native.Icon.Sizes.CUSTOM, source: _modDef12264, disableColor: true, style: closure_8().gifIcon });
}
function PlayIcon() {
  return timestampProducer(native.Icon, { size: native.Icon.Sizes.SMALL_20, source: _modDef12265, disableColor: true });
}
function ExtraMediaIcon(extraMediaCount) {
  const tmp = closure_8();
  const obj = { style: tmp.extraMediaCountContainer, children: null };
  items = [timestampProducer(native.Icon, { source: _modDef11515, color: tmp.icon.color, size: native.Icon.Sizes.REFRESH_SMALL_16 }), ];
  const obj2 = { source: _modDef11515, color: tmp.icon.color, size: native.Icon.Sizes.REFRESH_SMALL_16 };
...
export default function ForumPostGridBody(thread) {
  ...
  if (tmp13Result) {
    ...
    obj4.children = tmp13(tmp2(12270).ForumPostAppliedTagPills, obj5);
