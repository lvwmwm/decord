...
// Dependencies: [19, 7550, 2042, 4977, 1372, 1074, 4907, 2020, 1385, 1366, 1370, 4981, 1978, 4987, 11, 2]
...
// Module 8145 (ForumPostMediaUtils)
  ...
  if (null == firstResult) {
    ...
    if (tmp4) {
      if (null != components) {
        ...
        const flattenComponentsResult = tmp(4981).flattenComponents(components);
        const arr = Array.from(tmp(4981).flattenComponents(components).values());
        items1 = Array.from(tmp(4981).flattenComponents(components).values()).flatMap((type) => {
          type = type.type;
        ...
        const flatMapResult = Array.from(tmp(4981).flattenComponents(components).values()).flatMap((type) => {
          type = type.type;
          if (require("Server").ComponentType.THUMBNAIL === type) {
            ({ media, spoiler } = type);
            if (spoiler == null) {
  ...
  if (null == components) {
    ...
    if (tmp3) {
      if (null != components) {
        ...
        const flattenComponentsResult = tmp(4981).flattenComponents(components);
        const arr = Array.from(tmp(4981).flattenComponents(components).values());
        let found = Array.from(tmp(4981).flattenComponents(components).values()).flatMap((type) => {
          type = type.type;
          if (require("Server").ComponentType.THUMBNAIL === type) {
            ({ media, spoiler } = type);
            if (spoiler == null) {
  ...
  if (null == firstMessage) {
    ...
    if (tmp5) {
      if (null != components) {
        ...
        const flattenComponentsResult = tmp(4981).flattenComponents(components);
        const arr = Array.from(tmp(4981).flattenComponents(components).values());
        items = Array.from(tmp(4981).flattenComponents(components).values()).flatMap((type) => {
          type = type.type;
          if (require("Server").ComponentType.THUMBNAIL === type) {
            ({ media, spoiler } = type);
            if (spoiler == null) {
