...
// Dependencies: [5, 32, 2098, 2042, 2105, 1372, 7431, 1074, 4448, 5009, 1271, 573, 1231, 5855, 12, 7437, 1370, 7440, 1241, 5007, 7441, 11, 1385, 7444, 7451, 2]

// Module 7436 (GuildOnboardingActionCreators)
...
obj.completeOnboarding = function completeOnboarding(guildId, prompts) {
  ...
  const selectedRoleIds = items1(7437).getSelectedRoleIds(selectedOptions);
  const obj2 = items1(7437);
  const selectedChannelIds = items1(7437).getSelectedChannelIds(selectedOptions);
  if (GuildOnboardingPromptsStore.getEnabled(guildId)) {
    let defaultChannelIds = obj.getDefaultChannelIds(guildId);
  } else {
    defaultChannelIds = [];
  }
  const obj3 = items1(7437);
  const tmp2Result = items1(7437);
  [arr3, arr4] = items1(7437).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds);
  ...
  const tmp6 = _slicedToArray(items1(7437).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds), 2);
  const tmp2Result11 = items1(7440);
  if (null == tmp) {
    items1 = [];
  } else {
  ...
  const providerConnectionState = items1(7437).getProviderConnectionState(connections);
  const tmp2Result12 = items1(7437);
  const applicationConnectionState = items1(7437).getApplicationConnectionState(connections);
  const tmp2Result13 = items1(7437);
  const obj4 = {};
  const obj8 = AnalyticsUtilsDefault;
  const merged = Object.assign(items1(5007).collectGuildAnalyticsMetadata(guildId));
  ...
  const tmp2Result14 = items1(5007);
  const tmp2Result15 = items1(7441);
  ...
  if (ImpersonateStore.isFullServerPreview(guildId)) {
    const result = tmp2(5855).updateImpersonatedChannels(guildId, items, []);
    const tmp2Result16 = tmp2(5855);
    const result1 = tmp2(5855).updateImpersonatedData(guildId, { optInEnabled: true });
    const tmp2Result17 = tmp2(5855);
    const _Array = Array;
    const result2 = tmp2(5855).updateImpersonatedRoles(guildId, Array.from(selectedRoleIds));
