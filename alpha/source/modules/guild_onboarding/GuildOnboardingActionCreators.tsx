...
// Dependencies: [5, 32, 2098, 2041, 2105, 1372, 7345, 1074, 4382, 4938, 1271, 573, 1231, 5769, 12, 7351, 1370, 7354, 1241, 4936, 7355, 11, 1385, 7358, 7365, 2]

// Module 7350 (GuildOnboardingActionCreators)
...
obj.completeOnboarding = function completeOnboarding(guildId, prompts) {
  ...
  const selectedRoleIds = items1(7351).getSelectedRoleIds(selectedOptions);
  const obj2 = items1(7351);
  const selectedChannelIds = items1(7351).getSelectedChannelIds(selectedOptions);
  if (GuildOnboardingPromptsStore.getEnabled(guildId)) {
    let defaultChannelIds = obj.getDefaultChannelIds(guildId);
  } else {
    defaultChannelIds = [];
  }
  const obj3 = items1(7351);
  const tmp2Result = items1(7351);
  [arr3, arr4] = items1(7351).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds);
  ...
  const tmp6 = _slicedToArray(items1(7351).getChannelCoverageForOnboarding(guildId, prompts, defaultChannelIds), 2);
  const tmp2Result11 = items1(7354);
  if (null == tmp) {
    items1 = [];
  } else {
  ...
  const providerConnectionState = items1(7351).getProviderConnectionState(connections);
  const tmp2Result12 = items1(7351);
  const applicationConnectionState = items1(7351).getApplicationConnectionState(connections);
  const tmp2Result13 = items1(7351);
  const obj4 = {};
  ...
  const tmp2Result15 = items1(7355);
  ...
  if (ImpersonateStore.isFullServerPreview(guildId)) {
    const result = tmp2(5769).updateImpersonatedChannels(guildId, items, []);
    const tmp2Result16 = tmp2(5769);
    const result1 = tmp2(5769).updateImpersonatedData(guildId, { optInEnabled: true });
    const tmp2Result17 = tmp2(5769);
    const _Array = Array;
    const result2 = tmp2(5769).updateImpersonatedRoles(guildId, Array.from(selectedRoleIds));
