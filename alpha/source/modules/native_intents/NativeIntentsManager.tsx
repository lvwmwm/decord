...
// Dependencies: [32, 2042, 2064, 4396, 4406, 2096, 1372, 1074, 18330, 18331, 4910, 13328, 1397, 1370, 4601, 7365, 2]
  ...
  if (obj.getCurrentConfig({ location: "NativeIntentsManager" }, obj2).activityEnabled) {
    ...
    if (null != channel) {
      ...
      if (channel.type === constants.DM) {
        ...
        if (null != first) {
          const globalName = tmp(4601).getGlobalName(first);
          if (null != globalName) {
          ...
          const name = tmp(4601).getName(first);
          if (null != name) {
            items1.push(name);
          }
          const tmpResult4 = tmp(4601);
        }
      }
      ...
      tmp(18331).setActivity(obj4);
      obj6 = RelationshipStore;
      tmp10 = require;
      tmp11 = UserStore;
      const tmpResult5 = tmp(18331);
    } else {
      tmp(18331).resignActivity();
      const tmpResult6 = tmp(18331);
    }
  }
}
...
prototype["handleInit"] = function handleInit() {
  ...
  if (obj.getCurrentConfig({ location: "NativeIntentsManager" }, obj2).clearEnabled) {
    tmp2(18331).clearSearchIndex();
    const tmp2Result = tmp2(18331);
  }
  ...
  if (tmp2Result5.getCurrentConfig({ location: "NativeIntentsManager" }, obj4).searchEnabled) {
    ...
    const tmp2Result7 = tmp2(18330);
    obj5.disable = !tmp2(18331).hasSearch();
    ...
    if (PermissionStore.can(constants3.VIEW_CHANNEL, channel)) {
      ...
      if (null != guild) {
        if (null == guild) {
          ...
          const items1 = [obj4];
          tmp(18331).indexDomains(items1);
          const tmpResult = tmp(18331);
        } else {
...
prototype["handleGuildCreateOrUpdate"] = function handleGuildCreateOrUpdate(guild) {
  ...
  if (obj.getCurrentConfig({ location: "NativeIntentsManager" }, obj2).searchEnabled) {
    ...
    if (null != guild1) {
      ...
      tmp(18331).indexDomains(items);
      const tmpResult = tmp(18331);
    } else {
      const items1 = [guild.id];
      tmp(18331).deleteSearchDomains(items1);
      const tmpResult2 = tmp(18331);
    }
  }
};
