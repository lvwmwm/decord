...
// Dependencies: [5, 5861, 7854, 502, 2042, 1372, 6923, 1074, 4869, 2048, 12754, 3, 4521, 1115, 11599, 4685, 1110, 8739, 4838, 7670, 9966, 4684, 16819, 7962, 8523, 7513, 4840, 10081, 1241, 5007, 1364, 13266, 11, 16817, 7803, 10, 7804, 5580, 13987, 12757, 504, 13121, 10285, 4807, 1094, 11958, 8697, 8694, 573, 5030, 1101, 5029, 14208, 2]
// Exports: default

// Module 18438 (receiveNotification)
      ...
      if (null != data.deeplink) {
        ...
        if ("" !== data.deeplink) {
          ...
          if (payload.type === tmp(1094).LinkingTypes.MESSAGE) {
            ...
            if (tmp8) {
              tmp(11958).receivedNotification(payload.messageId, payload.channelId, tracking_type);
              const tmpResult = tmp(11958);
            }
            ({ guildId: obj8.guildId, channelId: obj8.channelId, messageId: obj8.messageId } = payload);
            const messages1 = tmp80(10285).fetchMessages({ guildId: null, channelId: null, messageId: null, isPreload: true });
            flag = true;
            flag2 = true;
            ...
            const tmp80Result = tmp80(10285);
          } else {
            if (payload.type === tmp(1094).LinkingTypes.ICYMI) {
              if (null != data.channel_id) {
                if (null != data.message_id) {
                  const forNotification = tmp80(8697).fetchForNotification(data.channel_id, data.message_id);
                  flag = false;
                  flag2 = false;
                  const tmp80Result3 = tmp80(8697);
                }
              }
            }
            flag = false;
            ...
            if (payload.type === tmp(1094).LinkingTypes.ICYMI) {
              ...
              if (null != data.user_id) {
                ...
                if (null != data.notification_center_id) {
                  ...
                  const obj5 = { id: data.notification_center_id, type: tmp(8694).ICYMIItemTypes.CUSTOM_STATUS, score: 1000, data: null };
                  ...
                  obj5.data = obj6;
                  const forStatusNotification = tmp80(8697).fetchForStatusNotification(obj5);
                  flag = false;
                  flag2 = false;
                  const tmp80Result4 = tmp80(8697);
                }
              }
            }
          }
    ...
    case "MESSAGE_CREATE":
      ...
      switch (tmp53) {
        ...
        case "GENERIC_PUSH_NOTIFICATION_SENT":
          if (null != data.deeplink) {
            if ("" !== data.deeplink) {
              let tmp19Result7 = tmp19(4807);
              let tmp19Result3Result = tmp19Result7(data.deeplink);
              let tmp19Result8 = tmp19(14208);
    ...
    case "FORUM_THREAD_CREATED":
      ...
      switch (tmp53) {
        ...
        case "GENERIC_PUSH_NOTIFICATION_SENT":
          if (null != data.deeplink) {
            if ("" !== data.deeplink) {
              tmp19Result7 = tmp19(4807);
              tmp19Result3Result = tmp19Result7(data.deeplink);
              tmp19Result8 = tmp19(14208);
...
let closure_17 = fn(12754).MultiAccountSwitchLocation;
    ...
    if (null != data.receiving_user_id) {
      if (null != AuthenticationStore.getId()) {
        if (data.receiving_user_id !== obj.getId()) {
          ...
          receiving_user_id = tmp7(12757).switchAccount(receiving_user_id, false, arg1 ? tmp5.PUSH_NOTIFICATION_INITIAL : tmp5.PUSH_NOTIFICATION);
          receiving_user_id.then(() => {
            const Emitter = initializeDefault.Emitter;
            Emitter.batched(() => receiveNotification_(data));
          });
          const tmp7Result6 = tmp7(12757);
        }
      }
