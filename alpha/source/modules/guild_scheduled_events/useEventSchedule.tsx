...
// Dependencies: [7771, 9759, 9763, 504, 2]
...
export const getEventSchedule = function getEventSchedule(guildEvent, recurrenceId) {
  ...
  if (null != guildEvent.recurrence_rule) {
    if (null != recurrenceId) {
      const baseScheduleForRecurrence = tmp(9759).getBaseScheduleForRecurrence(recurrenceId, guildEvent);
      const tmpResult = tmp(9759);
      const scheduleForRecurrenceWithException = tmp(9759).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp4);
      ({ startDate, endDate } = scheduleForRecurrenceWithException);
