...
// Dependencies: [7856, 9837, 9841, 504, 2]
...
export const getEventSchedule = function getEventSchedule(guildEvent, recurrenceId) {
  ...
  if (null != guildEvent.recurrence_rule) {
    if (null != recurrenceId) {
      const baseScheduleForRecurrence = tmp(9837).getBaseScheduleForRecurrence(recurrenceId, guildEvent);
      const tmpResult = tmp(9837);
      const scheduleForRecurrenceWithException = tmp(9837).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp4);
      ({ startDate, endDate } = scheduleForRecurrenceWithException);
