...
// Dependencies: [7854, 9832, 9836, 504, 2]
...
export const getEventSchedule = function getEventSchedule(guildEvent, recurrenceId) {
  ...
  if (null != guildEvent.recurrence_rule) {
    if (null != recurrenceId) {
      const baseScheduleForRecurrence = tmp(9832).getBaseScheduleForRecurrence(recurrenceId, guildEvent);
      const tmpResult = tmp(9832);
      const scheduleForRecurrenceWithException = tmp(9832).getScheduleForRecurrenceWithException(baseScheduleForRecurrence, tmp4);
      ({ startDate, endDate } = scheduleForRecurrenceWithException);
