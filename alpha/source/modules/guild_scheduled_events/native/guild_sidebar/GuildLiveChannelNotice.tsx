...
// Dependencies: [19, 17, 5723, 2049, 4851, 4464, 4853, 2050, 1085, 21, 576, 1177, 4819, 10474, 8444, 10465, 1364, 5278, 4829, 4825, 12879, 10467, 1875, 5036, 8743, 4763, 8204, 5273, 4680, 4982, 504, 1115, 5327, 9967, 9874, 9884, 9971, 5736, 5730, 5722, 9834, 5403, 16613, 11265, 5912, 2]
  ...
  const callback = noop.useCallback(() => {
    if (null != activeEventOrStageInstanceChannel) {
      ...
      if (obj.isGuildVoice()) {
        ...
        tmp4(8743).connectAndOpen(obj);
        const tmp4Result2 = tmp4(8743);
      }
    }
  }, items);
...
function GuildExternalEventNotice(guildEvent) {
  ...
  if (null != locationFromEvent) {
    ...
    obj2.LocationIcon = tmp(9884).LocationIcon;
    obj2.LiveIcon = tmp(9967).CalendarIcon;
...
export const getScaledLiveChannelNoticeHeight = function getScaledLiveChannelNoticeHeight(fontScale, guildLiveChannelNoticeInfo) {
  ...
  if (!hasSpeakers) {
    ...
    const sum = PX_82 + tmp(10465).scaleTextLineHeight(c22, fontScale);
    const tmp5 = PX_82;
    const tmp8 = PX_4;
    ...
    tmpResult3 = tmp(1364);
    let num3 = 0;
    const sum2 = sum1 + tmp(10465).scaleTextLineHeight(c23, fontScale);
    if (hasButton) {
      num3 = tmp5 + tmp(5278).SMALL_BUTTON_HEIGHT;
    }
