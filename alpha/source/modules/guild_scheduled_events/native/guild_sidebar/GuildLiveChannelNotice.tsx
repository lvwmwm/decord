...
// Dependencies: [19, 17, 5725, 2049, 4851, 4466, 4853, 2050, 1085, 21, 576, 1177, 4819, 9576, 7534, 9567, 1364, 5279, 4829, 4825, 12008, 9569, 1875, 5036, 7833, 4763, 7293, 5274, 4682, 4982, 504, 1115, 5328, 9065, 8972, 8982, 9069, 5738, 5732, 5724, 8932, 5404, 15793, 10363, 5914, 2]
...
function JoinChannelButton(label) {
  ...
  const callback = noop.useCallback(() => {
    if (null != activeEventOrStageInstanceChannel) {
      ...
      if (obj.isGuildVoice()) {
        tmp4(5036).openGuildVoiceModal(obj);
        const tmp4Result = tmp4(5036);
      } else {
        tmp4(7833).connectAndOpen(obj);
        const tmp4Result2 = tmp4(7833);
      }
    }
  }, items);
...
function GuildExternalEventNotice(guildEvent) {
  ...
  if (null != locationFromEvent) {
    ...
    obj2.LocationIcon = tmp(8982).LocationIcon;
    obj2.LiveIcon = tmp(9065).CalendarIcon;
...
let closure_31 = noop.memo((arg0) => {
  ...
  if (tmp2Result) {
    ...
    items4[1] = tmp4(tmp7(4825).Text, obj8);
...
export const getScaledLiveChannelNoticeHeight = function getScaledLiveChannelNoticeHeight(fontScale, guildLiveChannelNoticeInfo) {
  ...
  if (!hasSpeakers) {
    ...
    const sum = PX_82 + tmp(9567).scaleTextLineHeight(c22, fontScale);
    const tmp5 = PX_82;
    const tmp8 = PX_4;
    const tmpResult = tmp(9567);
    let num2 = 0;
    ...
    tmpResult3 = tmp(1364);
    let num3 = 0;
    const sum2 = sum1 + tmp(9567).scaleTextLineHeight(c23, fontScale);
    if (hasButton) {
      num3 = tmp5 + tmp(5279).SMALL_BUTTON_HEIGHT;
    }
