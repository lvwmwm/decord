...
// Dependencies: [19, 17, 5721, 2047, 4849, 4462, 4851, 2048, 1085, 21, 576, 1177, 4817, 10469, 8440, 10460, 1364, 5276, 4827, 4823, 12870, 10462, 1875, 5034, 8739, 4761, 8202, 5271, 4678, 4980, 504, 1115, 5325, 9962, 9869, 9879, 9966, 5734, 5728, 5720, 9829, 5401, 16589, 11259, 5910, 2]
...
function JoinChannelButton(label) {
  ...
  const callback = noop.useCallback(() => {
    if (null != activeEventOrStageInstanceChannel) {
      ...
      if (obj.isGuildVoice()) {
        tmp4(5034).openGuildVoiceModal(obj);
        const tmp4Result = tmp4(5034);
      } else {
        tmp4(8739).connectAndOpen(obj);
        const tmp4Result2 = tmp4(8739);
      }
    }
  }, items);
...
function GuildExternalEventNotice(guildEvent) {
  ...
  if (null != locationFromEvent) {
    ...
    obj2.LocationIcon = tmp(9879).LocationIcon;
    obj2.LiveIcon = tmp(9962).CalendarIcon;
...
let closure_31 = noop.memo((arg0) => {
  ...
  if (tmp2Result) {
    ...
    items4[1] = tmp4(tmp7(4823).Text, obj8);
...
export const getScaledLiveChannelNoticeHeight = function getScaledLiveChannelNoticeHeight(fontScale, guildLiveChannelNoticeInfo) {
  ...
  if (!hasSpeakers) {
    ...
    const sum = PX_82 + tmp(10460).scaleTextLineHeight(c22, fontScale);
    const tmp5 = PX_82;
    const tmp8 = PX_4;
    const tmpResult = tmp(10460);
    let num2 = 0;
    ...
    tmpResult3 = tmp(1364);
    let num3 = 0;
    const sum2 = sum1 + tmp(10460).scaleTextLineHeight(c23, fontScale);
    if (hasButton) {
      num3 = tmp5 + tmp(5276).SMALL_BUTTON_HEIGHT;
    }
