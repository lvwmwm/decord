...
// Dependencies: [19, 17, 4406, 11630, 21, 4757, 576, 11657, 11660, 11661, 504, 11637, 7456, 1115, 9060, 9059, 4724, 11665, 1980, 4452, 7216, 7214, 7215, 7213, 10009, 11668, 6860, 8679, 5110, 11655, 8948, 8947, 8912, 10174, 10173, 4960, 11669, 11673, 11674, 11675, 9521, 9522, 11676, 11666, 5906, 5824, 5830, 2]
// Exports: default

// Module 11659 (SafetyToolsActionSheet)
...
import _modDef8948 from "module_8948" /* 8948 */;
...
import EducationIcon from "EducationIcon" /* 11674 */;
...
export default function SafetyToolsActionSheet(channelId) {
  ...
  const memo = recipientId.useMemo(() => {
    ...
    obj5.icon = importDefault(stateFromStores1 ? 7216 : 7214);
    if (stateFromStores1) {
      let EyeSlashIcon = tmp(7215).EyeIcon;
    } else {
      EyeSlashIcon = tmp(7213).EyeSlashIcon;
    }
  ...
  obj7.children = stateFromStores1(onClose, {
    ...
    children: memo.map((buttons) => {
      ...
      obj2.children = buttons.map((item, index) => {
        ...
        return stateFromStores1(channelId(5824).TableRow, { label, subLabel, onPress, trailing, disabled, icon: stateFromStores1(channelId(5830).TableRowIcon, { source: icon, IconComponent }) }, index);
      });
