...
// Dependencies: [8364, 4416, 8365, 1115, 2]
...
// Module 8363 (GuildAntiRaidUtils)
...
export const hasDetectedActivity = function hasDetectedActivity(incidentsData) {
  ...
  if (tmp) {
    const obj = _modDef4416(incidentsData.dmSpamDetectedAt);
    tmp = _modDef4416(incidentsData.dmSpamDetectedAt).add(React3, "hours") > _modDef4416();
    const addResult = _modDef4416(incidentsData.dmSpamDetectedAt).add(React3, "hours");
  }
  if (!tmp) {
    let tmp6 = null != incidentsData.raidDetectedAt;
    if (tmp6) {
      const obj2 = _modDef4416(incidentsData.raidDetectedAt);
      tmp6 = _modDef4416(incidentsData.raidDetectedAt).add(React3, "hours") > _modDef4416();
      const addResult1 = _modDef4416(incidentsData.raidDetectedAt).add(React3, "hours");
    }
    tmp = tmp6;
  }
  return tmp;
...
export const hasDetectedRaid = function hasDetectedRaid(raidDetectedAt) {
  ...
  if (tmp) {
    const obj = _modDef4416(raidDetectedAt.raidDetectedAt);
    tmp = _modDef4416(raidDetectedAt.raidDetectedAt).add(React3, "hours") > _modDef4416();
    const addResult = _modDef4416(raidDetectedAt.raidDetectedAt).add(React3, "hours");
  }
  return tmp;
};
export const hasDetectedDMRaid = function hasDetectedDMRaid(dmSpamDetectedAt) {
  ...
  if (tmp) {
    const obj = _modDef4416(dmSpamDetectedAt.dmSpamDetectedAt);
    tmp = _modDef4416(dmSpamDetectedAt.dmSpamDetectedAt).add(React3, "hours") > _modDef4416();
    const addResult = _modDef4416(dmSpamDetectedAt.dmSpamDetectedAt).add(React3, "hours");
  }
  return tmp;
};
export const getIncidentAlertType = function getIncidentAlertType(guildIncident) {
  if (null != guildIncident) {
    ...
    if (tmp) {
      const obj = _modDef4416(guildIncident.raidDetectedAt);
      tmp = _modDef4416(guildIncident.raidDetectedAt).add(React3, "hours") > _modDef4416();
      const addResult = _modDef4416(guildIncident.raidDetectedAt).add(React3, "hours");
    }
