...
// Dependencies: [19, 17, 4845, 2044, 1992, 4474, 1372, 4848, 8207, 1074, 4850, 2097, 8208, 4854, 21, 1177, 4829, 576, 504, 11744, 13667, 8329, 13668, 11219, 13531, 5197, 13669, 1115, 13670, 12642, 12701, 5009, 4796, 13685, 1980, 5427, 7384, 13686, 8724, 4520, 1241, 13345, 7710, 5036, 5407, 8213, 8211, 4825, 13687, 10456, 2]
...
export default noop.memo(function PrivateChannelButtons(channelId) {
  ...
  const memo1 = obj9.useMemo(() => {
    if (closure_6) {
      ...
      let tmpResult = tmp(tmp2(13686).SafetyToolsButton, obj3);
    } else {
      ...
      tmpResult = tmp(tmp2(5427).PressableOpacity, obj);
    }
    return tmpResult;
  }, items12);
  if (screenIndex(8724)(channelId)) {
    ...
    if (!tmp15) {
      ...
      if (tmp15) {
        ...
        tmp30(tmp2(5407).VoiceNormalIcon, unsafe_rawColors1);
        ...
        if (stateFromStores1) {
          ...
          let tmp30Result2 = tmp30(tmp2(8213).PhoneHangUpIcon, obj11);
          let tmp33 = tmp30;
        } else {
          let GREEN_360;
          ...
          tmp30Result2 = tmp30(tmp2(8211).PhoneCallIcon, obj12);
          tmp33 = tmp30;
        }
        const items15 = [
          tmp30Result2,
        ...
        if (tmp33Result) {
          ...
          obj13.children = tmp25(tmp2(4825).Text, obj14);
          tmp33Result = tmp33(tmp26, obj13);
        }
        items15[2] = tmp33Result;
        obj8.children = items15;
        const items17 = [tmp25(tmp2(5427).PressableOpacity, obj8), , ];
        let tmp38 = null;
        if (!isMultiUserDMResult) {
          tmp38 = null;
          if (!stateFromStores1) {
