...
// Dependencies: [19, 17, 2042, 11262, 1074, 17185, 21, 8243, 563, 7597, 11568, 8192, 1115, 13112, 1241, 8979, 17357, 7331, 5933, 4685, 1612, 17408, 1364, 17409, 17410, 8253, 8254, 17411, 17412, 17413, 17264, 2]

// Module 17356 (ChannelDetailsNavigator)
...
import ChannelSettingsModal from "ChannelSettingsModal" /* 17357 */;
  ...
  items5[3] = closure_9(Screen.Screen, {
    ...
    options(navigation) {
      ...
      if (DETAILS === navigation.route.name) {
        let renderModalCloseImage = tmp(8192).getRenderModalCloseImage(navigation);
        const tmpResult = tmp(8192);
      } else {
        renderModalCloseImage = tmp(8192).getRenderModalBackImage(navigation);
