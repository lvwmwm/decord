...
export default function UsernameSearchScreen(navigation) {
  ...
  const effect1 = noop.useEffect(() => navigation.addListener("transitionEnd", (data) => {
    ...
    if (!closing) {
      closing = navigation(5266).getIsScreenReaderEnabled();
      const obj = navigation(5266);
    }
    if (!closing) {
  ...
  const clientThemesOverride = navigation(7297).useClientThemesOverride();
  ...
  const items2 = [closure_8(ref(5437), { absolute: true }), ];
