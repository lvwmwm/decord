...
// Dependencies: [32, 19, 17, 1372, 1074, 12935, 21, 4757, 576, 12937, 4601, 1241, 1115, 8636, 8110, 1364, 5343, 4753, 14131, 14133, 2]
// Exports: default

// Module 17244 (AddFriendScreen)
...
export default function AddFriendScreen(navigation) {
  ...
  let callback;
  let tmp = closure_13();
  const contactSyncAccount = navigation(12937).useContactSyncAccount();
  let obj = navigation(12937);
  let obj2 = navigation(12937);
  [tmp5, c2] = callback(noop.useState(!navigation(12937).isContactSyncEnabled(contactSyncAccount)), 2);
  ...
  const layoutEffect = noop.useLayoutEffect(() => {
    navigation.setOptions({
      headerRight(arg0) {
        ...
        const renderHeaderTextButton = navigation(8110).getRenderHeaderTextButton(intl.string(navigation(1115).t.RDE0Sc), callback);
    ...
    result.then((result) => {
      ...
      if (!tmp5) {
        tmp5 = !tmp2(12937).isContactSyncEnabled(contactSyncAccount);
  ...
  const items2 = [closure_10(navigation(4753).Text, obj4), , , ];
  ...
  items2[1] = closure_10(navigation(4753).Text, obj5);
  items2[2] = closure_10(contactSyncAccount(14131), { style: tmp.input, autoFocusInput: false, sourcePage: navigation.route.params.sourcePage });
  ...
  const items3 = [closure_10(navigation(4753).Text, obj8), ];
  let tmp10Result = null;
  if (tmp5) {
    const obj9 = { style: tmp.rowContainer, location: "Add Friend Modal" };
    tmp10Result = closure_10(contactSyncAccount(14133), obj9);
  }
