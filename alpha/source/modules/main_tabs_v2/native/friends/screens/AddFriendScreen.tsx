...
// Dependencies: [32, 19, 17, 1372, 1074, 13017, 21, 4827, 576, 13019, 4671, 1241, 1115, 8707, 8192, 1364, 5427, 4823, 14213, 14215, 2]
// Exports: default

// Module 17313 (AddFriendScreen)
...
export default function AddFriendScreen(navigation) {
  ...
  let callback;
  let tmp = closure_13();
  const contactSyncAccount = navigation(13019).useContactSyncAccount();
  let obj = navigation(13019);
  let obj2 = navigation(13019);
  [tmp5, c2] = callback(noop.useState(!navigation(13019).isContactSyncEnabled(contactSyncAccount)), 2);
  ...
  const layoutEffect = noop.useLayoutEffect(() => {
    navigation.setOptions({
      headerRight(arg0) {
        ...
        const renderHeaderTextButton = navigation(8192).getRenderHeaderTextButton(intl.string(navigation(1115).t.RDE0Sc), callback);
    ...
    result.then((result) => {
      ...
      if (!tmp5) {
        tmp5 = !tmp2(13019).isContactSyncEnabled(contactSyncAccount);
        const tmp2Result = tmp2(13019);
      }
      dependencyMap(tmp5);
    });
  }, items);
  const items1 = [closure_10(contactSyncAccount(5427), { absolute: true }), ];
  ...
  const items2 = [closure_10(navigation(4823).Text, obj4), , , ];
  ...
  items2[1] = closure_10(navigation(4823).Text, obj5);
  items2[2] = closure_10(contactSyncAccount(14213), { style: tmp.input, autoFocusInput: false, sourcePage: navigation.route.params.sourcePage });
  ...
  const items3 = [closure_10(navigation(4823).Text, obj8), ];
  let tmp10Result = null;
  if (tmp5) {
    const obj9 = { style: tmp.rowContainer, location: "Add Friend Modal" };
    tmp10Result = closure_10(contactSyncAccount(14215), obj9);
  }
