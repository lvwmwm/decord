...
// Function ID: 11213
// Name: UserRow
// Dependencies: [19, 17, 4819, 5054, 7983, 2042, 4867, 4472, 11204, 1074, 21, 4827, 576, 4840, 38, 11213, 11214, 4520, 11215, 10082, 7984, 504, 1177, 4823, 4671, 11220, 7493, 8560, 1115, 11239, 4779, 4777, 5271, 2011, 8209, 5375, 8523, 1980, 8561, 11240, 9568, 5300, 8305, 5074, 10075, 11242, 9631, 11250, 10092, 11251, 5907, 11254, 5908, 2]

// Module 11212 (UserRow)
...
import FriendSuggestionActionCreatorsDefault from "FriendSuggestionActionCreators" /* 7984 */;
...
import GameRelationshipActionCreatorsDefault from "GameRelationshipActionCreators" /* 11215 */;
import ActivityStatusDefault from "ActivityStatus" /* 11220 */;
import ActionButtonDefault from "ActionButton" /* 11239 */;
import CrownIcon from "CrownIcon" /* 11240 */;
...
function UserRowSubLabel(arg0) {
  ...
  if (!isGameRelationship) {
    const obj3 = { lineClamp: 1, variant: "text-xs/medium", color: "text-muted", children: tmp(4671).getUserTag(user) };
    closure_14(tmp(4823).Text, obj3);
  ...
  if (null == stateFromStores) {
    ...
    items1[1] = closure_14(tmp(4823).Text, obj8);
      ...
      if (constants.PENDING_INCOMING === type) {
        ...
        obj12.onPress = function onPress() {
          ...
          let result = user(NONE[16]).maybeConfirmFriendRequestAccept({
            ...
            onConfirm() {
              if (null != closure_0) {
                const result = type(4520).presentGameFriendRequestAcceptedToast();
                const obj2 = type(4520);
              } else {
                const result1 = type(4520).presentFriendRequestAcceptedToast();
                const obj = type(4520);
              }
            }
          });
        ...
        obj35.onPress = function onPress() {
          ...
          user(NONE[13]).ensurePrivateChannel(current.id).then((result) => {
            ...
            if (null != channel) {
              ...
              const obj4 = user(11213)(channel, false);
              if (!obj4.inCall) {
                obj4.onPress();
              }
              const tmp6 = user(38);
              const obj = { recipientIds: current.id };
              user(4840).openPrivateChannel(obj);
              const tmp4Result = user(4840);
            }
          });
        };
