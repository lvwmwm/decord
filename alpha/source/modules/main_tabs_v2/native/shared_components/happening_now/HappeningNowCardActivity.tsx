...
// Dependencies: [19, 17, 2049, 1372, 14815, 1074, 1085, 21, 15681, 15682, 4829, 576, 6578, 504, 6584, 1241, 12425, 1980, 7617, 15676, 4981, 15683, 14816, 15677, 1177, 15686, 10339, 15687, 9355, 12541, 8153, 5404, 8527, 1115, 4680, 1364, 9508, 5894, 15678, 15689, 9511, 7588, 15691, 7686, 2]

// Module 15680 (HappeningNowCardActivity)
...
import _modDef15681 from "module_15681" /* 15681 */;
import _modDef15682 from "module_15682" /* 15682 */;
import HappeningNowAvatarStack from "HappeningNowAvatarStack" /* 15689 */;
...
export default noop.memo((userId) => {
  ...
  const callback = activity.useCallback(() => {
    ...
    if (null != stream) {
      asyncRequireImpl(12425, tmp6.paths).then((result) => result.default(channelId.channelId, true));
      const promise2 = asyncRequireImpl(12425, tmp6.paths);
    } else {
      asyncRequireImpl(7617, tmp6.paths).then((result) => result.default({ userId, localUser, sourceAnalyticsLocations }));
      const promise = asyncRequireImpl(7617, tmp6.paths);
    }
  }, items2);
