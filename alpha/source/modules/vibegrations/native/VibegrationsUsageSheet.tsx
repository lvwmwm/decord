...
export default function VibegrationsUsageSheet(projectId) {
  ...
  if (null == stateFromStores) {
    ...
    const tmp2Result7 = tmp2(5371);
    const sumTokenUsageResult = tmp2(5371).sumTokenUsage(stateFromStores.orchestrator, stateFromStores.codegen);
    const tmp2Result8 = tmp2(5371);
    const obj2 = { header: null, children: null };
    const obj3 = { title: null };
    const intl = tmp2(1115).intl;
    obj3.title = intl.string(_modDef3715["9yoLWZ"]);
    obj2.header = closure_5(tmp2(6570).BottomSheetTitleHeader, obj3);
    ...
    const sumTokenUsageResult1 = tmp2Result.sumTokenUsage(sumTokenUsageResult, tmp2(5371).usageOrEmpty(stateFromStores.compaction));
    const tmp2Result9 = tmp2(5371);
    obj7.runes = tmp2(5371).runesFromUsd(stateFromStores.cost_usd).toLocaleString();
    ...
    const runesFromUsdResult = tmp2(5371).runesFromUsd(stateFromStores.cost_usd);
    obj11.usage = tmp2(5371).usageOrEmpty(stateFromStores.compaction);
    items3[2] = closure_5(RoleRow, obj11);
    const obj12 = { label: null, usage: null };
    const intl6 = tmp2(1115).intl;
    obj12.label = intl6.string(_modDef3715.vVUMwj);
    const tmp2Result10 = tmp2(5371);
    obj12.usage = tmp2(5371).usageOrEmpty(stateFromStores.classifier);
    items3[3] = closure_5(RoleRow, obj12);
    obj8.children = items3;
    items2[1] = closure_6(tmp2(5279).Stack, obj8);
    ...
    const _Math = Math;
    const tmp2Result11 = tmp2(5371);
    const _HermesInternal = HermesInternal;
    obj16.children = "" + Math.round(100 * tmp2(5371).cacheHitRate(sumTokenUsageResult1)) + "%";
    items4[1] = closure_5(tmp2(4832).Text, obj16);
    obj13.children = items4;
    items2[2] = closure_6(View, obj13);
    obj5.children = items2;
    obj4.children = closure_6(tmp2(5279).Stack, obj5);
    obj2.children = closure_5(View, obj4);
    return closure_5(tmp2(6618).ActionSheet, obj2);
  }
