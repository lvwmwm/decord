...
// Dependencies: [19, 17, 21, 17032, 4757, 576, 17035, 4601, 16948, 1115, 3678, 1177, 4753, 17039, 2]
// Exports: default

// Module 17034 (VibegrationsRepliedMessage)
import nativeDefault from "native" /* 576 */;
import _modDef3678 from "module_3678" /* 3678 */;
import VibegrationsDesignFeedback from "VibegrationsDesignFeedback" /* 16948 */;
import VibegrationsSelectedMentionDefault from "VibegrationsSelectedMention" /* 17039 */;
...
const diff = fn(17032).MESSAGE_EDGE_INSET + fn(17032).MESSAGE_AVATAR_SIZE / 2 - 1;
const diff1 = fn(17032).MESSAGE_CONTENT_INSET - 4 - diff;
const createStyles = fn(4757);
let obj2 = { root: { marginLeft: diff - fn(17032).MESSAGE_CONTENT_INSET, paddingLeft: diff1 + 4, height: 20, flexDirection: "row", alignItems: "flex-start" }, spine: null, avatar: null, name: null, content: null };
...
export default function VibegrationsRepliedMessage(replied) {
  ...
  const tmp = closure_8();
  const messageAuthorUser = replied(17035).useMessageAuthorUser(replied.userId);
  const obj = replied(17035);
  let str = replied(4601).useName(messageAuthorUser);
  if (str == null) {
    str = "";
  }
  ...
  if (null != messageAuthorUser) {
    ...
    obj5.children = tmp11(tmp2(17035).VibegrationsUserAvatar, obj6);
    tmp11Result = tmp11(closure_5, obj5);
  }
  items1[1] = tmp11Result;
  items1[2] = closure_6(replied(4753).Text, { variant: "text-xs/semibold", color: "text-default", style: tmp.name, lineClamp: 1, children: str });
  ...
  items1[3] = closure_7(replied(4753).Text, obj8);
