...
// Dependencies: [19, 17, 21, 17089, 4827, 576, 17092, 4671, 17000, 1115, 3712, 1177, 4823, 17096, 2]
// Exports: default

// Module 17091 (VibegrationsRepliedMessage)
import nativeDefault from "native" /* 576 */;
import _modDef3712 from "module_3712" /* 3712 */;
import VibegrationsDesignFeedback from "VibegrationsDesignFeedback" /* 17000 */;
import VibegrationsSelectedMentionDefault from "VibegrationsSelectedMention" /* 17096 */;
...
const diff = fn(17089).MESSAGE_EDGE_INSET + fn(17089).MESSAGE_AVATAR_SIZE / 2 - 1;
const diff1 = fn(17089).MESSAGE_CONTENT_INSET - 4 - diff;
const createStyles = fn(4827);
let obj2 = { root: { marginLeft: diff - fn(17089).MESSAGE_CONTENT_INSET, paddingLeft: diff1 + 4, height: 20, flexDirection: "row", alignItems: "flex-start" }, spine: null, avatar: null, name: null, content: null };
...
export default function VibegrationsRepliedMessage(replied) {
  ...
  const tmp = closure_8();
  const messageAuthorUser = replied(17092).useMessageAuthorUser(replied.userId);
  const obj = replied(17092);
  let str = replied(4671).useName(messageAuthorUser);
  if (str == null) {
    str = "";
  }
  ...
  obj3.accessibilityLabel = intl.formatToPlainString(_modDef3712.loFt7s, { name: str, content: trimmed });
  ...
  if (null != messageAuthorUser) {
    ...
    obj5.children = tmp11(tmp2(17092).VibegrationsUserAvatar, obj6);
    tmp11Result = tmp11(closure_5, obj5);
  }
  items1[1] = tmp11Result;
  items1[2] = closure_6(replied(4823).Text, { variant: "text-xs/semibold", color: "text-default", style: tmp.name, lineClamp: 1, children: str });
  ...
  items1[3] = closure_7(replied(4823).Text, obj8);
