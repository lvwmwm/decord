...
// Dependencies: [109, 19, 17, 2048, 2044, 6527, 4750, 2066, 17242, 4476, 5010, 1372, 1074, 21, 4829, 576, 4537, 5009, 4796, 9589, 1980, 6535, 6530, 9599, 9605, 9612, 5992, 1115, 5995, 5994, 6616, 4825, 1177, 4522, 2110, 9598, 5912, 9593, 5918, 12251, 4469, 5013, 5328, 4982, 8045, 5272, 9594, 1484, 504, 6528, 6456, 5931, 17587, 9588, 6416, 2]
// Exports: default
  ...
  constructor() {
    ...
    applyArgumentsResult.handleMutePress = function handleMutePress() {
      ...
      if (muted) {
        ...
        const result = tmp(6535).updateGuildNotificationSettings(guildId, obj, NotificationLabel.muted(!muted));
        const tmpResult = tmp(6535);
      } else {
...
prototype["renderChannel"] = function renderChannel(parent_id) {
  ...
  if (null != parent_id) {
    ...
    if (self.props.shouldUseNewNotificationSystem) {
      const tmp4Result = tmp4(5013);
      const unreadSetting = UserGuildSettingsStore.resolveUnreadSetting(parent_id);
      stringResult = tmp4Result.presetName(tmp4(5013).presetFromSettings(unreadSetting, UserGuildSettingsStore.resolvedMessageNotifications(parent_id)));
      const tmp4Result5 = tmp4(5013);
    }
    ...
    if (null != channel) {
      channelName = tmp4(4982).computeChannelName(channel, UserStore, RelationshipStore);
      const tmp4Result8 = tmp4(4982);
    }
    obj2.subLabel = channelName;
    let tmp10Result;
    if (null != stringResult) {
      const obj4 = { text: stringResult };
      tmp10Result = tmp10(tmp4(5912).TableRow.TrailingText, obj4);
    }
...
prototype["render"] = function render() {
  ...
  if (hasItem) {
    ...
    obj.children = tmp4(tmp5(5272).Stack, obj2);
    ...
    obj3.children = items;
    obj.children = __initData5(tmp5(5272).Stack, obj3);
