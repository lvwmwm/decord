...
// Dependencies: [19, 17, 21, 9026, 9037, 5910, 10107, 4829, 10111, 8518, 1115, 4796, 10098, 1980, 7483, 6957, 5992, 2]
// Exports: default

// Module 10112 (GuildProfileGamesActionSheet)
...
export default function GuildProfileGamesActionSheet(profile) {
  ...
  const tmp = closure_7();
  const allGuildProfileGames = id(10111).useAllGuildProfileGames(profile);
  let obj = id(10111);
  ...
  const callback = noop.useCallback(() => {
    ...
    obj.openLazy(asyncRequireImpl(10098, dependencyMap.paths), "GuildProfileActionSheet:" + id, { guildId: id });
  }, items);
  ...
  obj5.children = jsx(id(5992).TableRowGroup, { title: intl.format(id(1115).t.vuAVo7, { serverName: profile.name }).toString(), hasIcons: true, children: allGuildProfileGames.map((game) => <GuildProfileGameRow key={arg0.id} game={arg0} activityLevel={gameActivity[arg0.id]} />) });
  obj4.children = <View style={tmp.container}>{null}</View>;
  obj3.children = jsx(id(6957).BottomSheetScrollView, { children: null });
  return jsx(id(7483).BottomSheet, { ref: obj2.useBottomSheetRef().bottomSheetRef, scrollable: true, onDismiss: callback, startHeight: 300, children: null });
};
