...
// Function ID: 16694
// Name: HomeDrawerDM
// Dependencies: [19, 17, 2046, 4406, 4938, 1372, 1085, 21, 4757, 504, 4910, 16694, 15600, 13604, 10416, 4753, 10371, 8126, 16656, 4620, 4617, 2]
// Exports: default

// Module 16693 (HomeDrawerDM)
...
function HomeDrawerDMExpandedChildren(channel) {
  ...
  const tmp3 = useMessagePreviewsDefault(channel, { unread: channel(16694).useBaseChannelUnreadBadgeState(channel, false).unread });
  closure_4 = tmp3;
  const obj3 = channel(16694);
  ...
  const title = stateFromStores.useMemo(() => {
    ...
    if (isMuted) {
      ...
      if (isTemporary) {
        tmp5Result = tmp5(13604);
        let BellSlashIcon = tmp5Result.BellZIcon;
      } else {
        BellSlashIcon = tmp5(10416).BellSlashIcon;
      }
    } else {
  ...
  return closure_10(channel(16656).HomeDrawerSharedItem, { title, subtitle });
}
