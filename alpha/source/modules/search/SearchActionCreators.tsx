...
// Dependencies: [12598, 12606, 573, 12, 8027, 12609, 2025, 2]

// Module 12605 (SearchActionCreators)
import _modDef12 from "module_12" /* 12 */;
import UserSettingsProtoActionCreators from "UserSettingsProtoActionCreators" /* 2025 */;
import SearchTabsFetchManagerDefault from "SearchTabsFetchManager" /* 12606 */;
...
export default {
  fetchTabMessages(searchContext) {
    ...
    const tokenizeQueryResult = searchContext(12598).tokenizeQuery(searchQueryString);
    const searchQueryFromTokens = searchContext(12598).getSearchQueryFromTokens(tokenizeQueryResult);
    ...
    let obj2 = searchContext(12598);
    const result = searchContext(12598).searchModeToSearchQueryParams(searchMode);
    let obj3 = {};
    const merged = Object.assign(searchQueryFromTokens);
    const merged1 = Object.assign(result);
    const merged2 = Object.assign(searchAnalyticsIds);
    const tmpResult = searchContext(12598);
    guildIdFromSearchContext = searchContext(12598).getGuildIdFromSearchContext(searchContext);
    if (null != guildIdFromSearchContext) {
      tmp(12598).setIncludeNSFW(obj3, guildIdFromSearchContext);
      const tmpResult5 = tmp(12598);
    }
    const tmpResult4 = searchContext(12598);
    ...
    obj4.id = searchContext(12598).getSearchContextId(searchContext);
    ...
    const tmpResult6 = searchContext(12598);
  ...
  fetchMessages(arg0) {
    ...
    let obj2 = {};
    const obj = guildIdFromSearchContext(12598);
    const tokenizeQueryResult = guildIdFromSearchContext(12598).tokenizeQuery(searchQueryString);
    const merged = Object.assign(guildIdFromSearchContext(12598).getSearchQueryFromTokens(tokenizeQueryResult));
    let obj3 = guildIdFromSearchContext(12598);
    const merged1 = Object.assign(guildIdFromSearchContext(12598).searchModeToSearchQueryParams(searchMode));
    const merged2 = Object.assign(searchAnalyticsIds);
    obj2.offset = pagination.offset;
    const obj4 = guildIdFromSearchContext(12598);
    guildIdFromSearchContext = guildIdFromSearchContext(12598).getGuildIdFromSearchContext(searchContext);
    if (null != guildIdFromSearchContext) {
      tmp(12598).setIncludeNSFW(obj2, guildIdFromSearchContext);
      const tmpResult = tmp(12598);
    }
    if (searchEverywhere) {
      obj2.search_everywhere = true;
    }
    const obj5 = guildIdFromSearchContext(12598);
    searchContextId = guildIdFromSearchContext(12598).getSearchContextId(searchContext);
    const tmp10 = searchContextId;
    const tmpResult2 = guildIdFromSearchContext(12598);
    const obj6 = { id: searchContextId, searchType: searchContext.type, searchQuery: obj2 };
    const obj8 = searchContextId(12609);
    if (onFetchStart != null) {
      const obj9 = { searchContext, searchQueryString, searchQuery: obj2 };
      onFetchStart(obj9);
    }
    const obj7 = searchContextId(12609).create({ id: searchContextId, searchType: searchContext.type, searchQuery: obj2 });
