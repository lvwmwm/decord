...
// Dependencies: [12674, 12682, 573, 12, 8109, 12685, 2025, 2]

// Module 12681 (SearchActionCreators)
import _modDef12 from "module_12" /* 12 */;
import UserSettingsProtoActionCreators from "UserSettingsProtoActionCreators" /* 2025 */;
import SearchTabsFetchManagerDefault from "SearchTabsFetchManager" /* 12682 */;
...
export default {
  fetchTabMessages(searchContext) {
    ...
    const tokenizeQueryResult = searchContext(12674).tokenizeQuery(searchQueryString);
    const searchQueryFromTokens = searchContext(12674).getSearchQueryFromTokens(tokenizeQueryResult);
    ...
    let obj2 = searchContext(12674);
    const result = searchContext(12674).searchModeToSearchQueryParams(searchMode);
    let obj3 = {};
    const merged = Object.assign(searchQueryFromTokens);
    const merged1 = Object.assign(result);
    const merged2 = Object.assign(searchAnalyticsIds);
    const tmpResult = searchContext(12674);
    guildIdFromSearchContext = searchContext(12674).getGuildIdFromSearchContext(searchContext);
    if (null != guildIdFromSearchContext) {
      tmp(12674).setIncludeNSFW(obj3, guildIdFromSearchContext);
      const tmpResult5 = tmp(12674);
    }
    const tmpResult4 = searchContext(12674);
    ...
    obj4.id = searchContext(12674).getSearchContextId(searchContext);
    ...
    const tmpResult6 = searchContext(12674);
  ...
  fetchMessages(arg0) {
    ...
    let obj2 = {};
    const obj = guildIdFromSearchContext(12674);
    const tokenizeQueryResult = guildIdFromSearchContext(12674).tokenizeQuery(searchQueryString);
    const merged = Object.assign(guildIdFromSearchContext(12674).getSearchQueryFromTokens(tokenizeQueryResult));
    let obj3 = guildIdFromSearchContext(12674);
    const merged1 = Object.assign(guildIdFromSearchContext(12674).searchModeToSearchQueryParams(searchMode));
    const merged2 = Object.assign(searchAnalyticsIds);
    obj2.offset = pagination.offset;
    const obj4 = guildIdFromSearchContext(12674);
    guildIdFromSearchContext = guildIdFromSearchContext(12674).getGuildIdFromSearchContext(searchContext);
    if (null != guildIdFromSearchContext) {
      tmp(12674).setIncludeNSFW(obj2, guildIdFromSearchContext);
      const tmpResult = tmp(12674);
    }
    if (searchEverywhere) {
      obj2.search_everywhere = true;
    }
    const obj5 = guildIdFromSearchContext(12674);
    searchContextId = guildIdFromSearchContext(12674).getSearchContextId(searchContext);
    const tmp10 = searchContextId;
    const tmpResult2 = guildIdFromSearchContext(12674);
    const obj6 = { id: searchContextId, searchType: searchContext.type, searchQuery: obj2 };
    const obj8 = searchContextId(12685);
    if (onFetchStart != null) {
      const obj9 = { searchContext, searchQueryString, searchQuery: obj2 };
      onFetchStart(obj9);
    }
    const obj7 = searchContextId(12685).create({ id: searchContextId, searchType: searchContext.type, searchQuery: obj2 });
