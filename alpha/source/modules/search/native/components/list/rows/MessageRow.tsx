...
// Function ID: 17217
// Name: MessageRow
// Dependencies: [19, 17, 4819, 2045, 2042, 2064, 5008, 1074, 21, 4827, 576, 504, 5325, 4980, 1177, 4823, 10485, 10732, 9631, 4671, 8525, 17217, 13686, 17218, 5073, 8305, 7657, 17194, 1115, 10450, 8208, 2]
...
const PollBadgeDefault = tmp5(17218);
require = fn;
function GuildChannelMessageRowHeader(channel) {
  ...
  if (muted) {
    const obj6 = { source: tmp6(10485), size: tmp2(1177).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    muted = tmp10(tmp2(1177).Icon, obj6);
  }
  items1[2] = muted;
  if (isFavorite) {
    const obj7 = { source: tmp6(10732), size: tmp2(1177).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    isFavorite = tmp10(tmp2(1177).Icon, obj7);
  }
  items1[3] = isFavorite;
  let isSystemDMResult = channel.isSystemDM();
  if (isSystemDMResult) {
    const obj8 = { type: tmp6(9631).Types.SYSTEM_DM, verified: true };
    isSystemDMResult = tmp10(tmp6(9631), obj8);
    const tmp6Result = tmp6(9631);
  }
...
function PrivateChannelMessageRowLabel(message) {
  ...
  if (isSystemDMResult) {
    ...
    isSystemDMResult = tmp9(channel(9631), obj5);
  ...
  if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
    ...
    tmp9Result = tmp9(tmp4(13686).BellZIcon, obj7);
  }
  items3[2] = tmp9Result;
  let tmp9Result2 = null;
  if (message.isPoll()) {
    const obj8 = { style: tmp.pollBadge };
    tmp9Result2 = tmp9(channel(17218), obj8);
  }
...
function GuildChannelMessageRowLabel(arg0) {
  ...
  if ("username" === stateFromStores) {
    ...
    const processColorStringsArray = tmp2(8305).useProcessColorStringsArray(colorStrings);
    const tmp2Result3 = tmp2(8305);
    ...
    const tmp2Result = tmp2(8305);
    const searchMessageTimestamp = tmp2(17217).useSearchMessageTimestamp(message, channel);
    ...
    obj6.children = tmp7;
    items1[1] = closure_1_11(tmp2(4823).Text, obj6);
    obj4.children = items1;
    ...
    items2[1] = closure_1_11(tmp2(4823).Text, obj7);
    let tmp21Result = null;
    if (message.hasFlag(MessageFlags.SUPPRESS_NOTIFICATIONS)) {
      const obj8 = { size: "xs", style: tmp.suppressNotificationsIcon };
      tmp21Result = tmp21(tmp2(13686).BellZIcon, obj8);
    }
...
function MessageRowContent(message) {
  ...
  if (isSpoilerHidden) {
    ...
    let tmp4Result = tmp4(tmp5(4823).Text, obj2);
  } else {
    const obj3 = { message, channel, muted: false, layout: tmp5(8208).ChannelListLayoutTypes.COZY, color: "interactive-text-default", lineClamp, messageSizeCacheRef };
    tmp4Result = tmp4(tmp5(10450).NativeMessageChannelRowPreview, obj3);
  }
