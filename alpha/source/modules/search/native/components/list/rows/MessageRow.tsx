...
function GuildChannelMessageRowHeader(channel) {
  ...
  if (muted) {
    const obj6 = { source: tmp6(9603), size: tmp2(1177).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    muted = tmp10(tmp2(1177).Icon, obj6);
  }
  items1[2] = muted;
  if (isFavorite) {
    const obj7 = { source: tmp6(9853), size: tmp2(1177).Icon.Sizes.EXTRA_SMALL, style: tmp.channelStatus };
    isFavorite = tmp10(tmp2(1177).Icon, obj7);
  }
  items1[3] = isFavorite;
  let isSystemDMResult = channel.isSystemDM();
  if (isSystemDMResult) {
    const obj8 = { type: tmp6(8741).Types.SYSTEM_DM, verified: true };
    isSystemDMResult = tmp10(tmp6(8741), obj8);
    const tmp6Result = tmp6(8741);
  }
...
function PrivateChannelMessageRowLabel(message) {
  ...
  if (isSystemDMResult) {
    ...
    isSystemDMResult = tmp9(channel(8741), obj5);
...
function GuildChannelMessageRowLabel(arg0) {
  ...
  if ("username" === stateFromStores) {
    ...
    const processColorStringsArray = tmp2(7403).useProcessColorStringsArray(colorStrings);
    ...
    obj6.children = tmp7;
    items1[1] = closure_1_11(tmp2(4832).Text, obj6);
    obj4.children = items1;
    ...
    items2[1] = closure_1_11(tmp2(4832).Text, obj7);
...
function MessageRowContent(message) {
  ...
  if (isSpoilerHidden) {
    ...
    let tmp4Result = tmp4(tmp5(4832).Text, obj2);
  } else {
    const obj3 = { message, channel, muted: false, layout: tmp5(7304).ChannelListLayoutTypes.COZY, color: "interactive-text-default", lineClamp, messageSizeCacheRef };
    tmp4Result = tmp4(tmp5(9568).NativeMessageChannelRowPreview, obj3);
  }
