...
// Dependencies: [4475, 8209, 17257, 12683, 17258, 12, 2]

// Module 17256 (MessageSearchResultParser)
import _mod12 from "module_12" /* 12 */;
import MessageRecord from "MessageRecord" /* 4475 */;

require = fn;
let closure_3 = fn(8209).EMBED_TYPES_WITHOUT_DESCRIPTION;
fn(17257).CachedSearchResultParser;
const prototype = function SearchResultMessageParser(searchQueryString, lineClamp) {
  ...
  tmp4.truncateMessage = function truncateMessage(content, tokenizedQueryContent, lineClamp) {
    ...
    if (null == content) {
      ...
      const _Set = Set;
      set = new Set(set(17258).analyze(tokenizedQueryContent));
      const parts = str4.split(/(\W+)/g);
      const found = parts.find((item) => set(17258).shouldHighlight(item, set));
      if (null == found) {
        return null;
      } else {
