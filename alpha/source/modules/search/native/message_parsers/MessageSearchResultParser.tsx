...
// Dependencies: [4407, 8125, 17198, 12598, 17199, 12, 2]

// Module 17197 (MessageSearchResultParser)
import _mod12 from "module_12" /* 12 */;
import MessageRecord from "MessageRecord" /* 4407 */;

require = fn;
let closure_3 = fn(8125).EMBED_TYPES_WITHOUT_DESCRIPTION;
fn(17198).CachedSearchResultParser;
const prototype = function SearchResultMessageParser(searchQueryString, lineClamp) {
  ...
  tmp4.truncateMessage = function truncateMessage(content, tokenizedQueryContent, lineClamp) {
    ...
    if (null == content) {
      ...
      const _Set = Set;
      set = new Set(set(17199).analyze(tokenizedQueryContent));
      const parts = str4.split(/(\W+)/g);
      const found = parts.find((item) => set(17199).shouldHighlight(item, set));
      if (null == found) {
        return null;
      } else {
