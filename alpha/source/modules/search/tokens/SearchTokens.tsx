...
// Function ID: 12685
// Name: SearchTokens
// Dependencies: [32, 2044, 4462, 4474, 2098, 4674, 1372, 12685, 12686, 1074, 4416, 12, 1115, 4982, 4673, 12683, 12687, 5822, 5747, 10182, 12688, 12689, 2]
...
function dateValidator(getFullMatch, arg1) {
  ...
  if (null != tmp2) {
    ...
    if (set.has(formatted)) {
      const localResult = tmp32(4416)(formatted, "MMMM").local();
      const items = [localResult, ];
      const obj17 = tmp32(4416)(formatted, "MMMM");
      ...
      const _Set = Set;
      const tmp32Result = tmp32(4416);
      const set1 = new Set(tmp32(4416).weekdays().map((item) => item.toLowerCase()));
      if (set1.has(formatted)) {
        const localResult1 = tmp32(4416)(formatted, "dddd").local();
        const items1 = [localResult1, ];
        const obj14 = tmp32(4416)(formatted, "dddd");
        ...
        if (set2.has(formatted)) {
          const localResult2 = tmp32(4416)(formatted, "YYYY").local();
          const items2 = [localResult2, ];
          const obj11 = tmp32(4416)(formatted, "YYYY");
          items2[1] = localResult2.clone().add(1, "year");
          const cloneResult2 = localResult2.clone();
          [obj9, obj10] = items2;
          const tmp17 = _slicedToArray(items2, 2);
        } else {
          const localResult3 = tmp32(4416)(formatted, value2).local();
          const items3 = [localResult3, ];
          const obj6 = tmp32(4416)(formatted, value2);
      ...
      const weekdaysResult = tmp32(4416).weekdays();
    }
...
export const isValidFilterAnswerForSubmit = function isValidFilterAnswerForSubmit(searchTokenType, trimmed) {
  if (null == closure_37[searchTokenType]) {
    ...
    if (SearchTokenTypes.ANSWER_HAS === tmp) {
      return tmp7(12687).validateForMapWithNegation("has", getHasMap(), token);
      ...
      return tmp7(12687).validateForMapWithNegation("author_type", obj, token);
