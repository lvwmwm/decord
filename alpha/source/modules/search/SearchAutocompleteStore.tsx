...
// Dependencies: [2042, 2105, 2064, 2096, 4602, 1372, 1074, 5734, 12598, 10101, 4601, 4876, 5738, 5661, 12599, 504, 573, 2]

// Module 12600 (SearchAutocompleteStore)
...
import SearchTokensDefault from "SearchTokens" /* 12599 */;
  ...
  if (constants.FILTER === type) {
    ...
    if (null != filter) {
      ...
      if (obj.isGuildLikeSearchContext(searchContext)) {
        ...
        if (tmp8) {
          if (0 !== str.length) {
            const searchContextId = tmp6(12598).getSearchContextId(searchContext);
            value = map1.get(searchContextId);
            if (value == null) {
...
const searchAutocompleteStoreClass = new SearchAutocompleteStoreClass(DispatcherDefault, {
  ...
  SEARCH_AUTOCOMPLETE_QUERY_UPDATE: function handleSearchAutocompleteQueryUpdate(arg0) {
    ...
    if (null != value) {
      ...
      const searchContextId1 = tmp5(12598).getSearchContextId(searchContext);
      let value4 = map1.get(searchContextId1);
      if (value4 == null) {
        const obj6 = { results: [], context: tmp(10101).getUserSearchContext(handleUserSearchResults.bind(null, searchContext)) };
        value4 = obj6;
        const tmpResult = tmp(10101);
      }
    ...
    if (tmp9) {
      const searchContextId2 = tmp5(12598).getSearchContextId(searchContext);
      let value5 = map1.get(searchContextId2);
      if (value5 == null) {
        const obj9 = { results: [], context: tmp(10101).getUserSearchContext(handleUserSearchResults.bind(null, searchContext)) };
        value5 = obj9;
        const tmpResult3 = tmp(10101);
      }
      ...
      if (null != trimmed) {
        if (trimmed.length > 0) {
          const guildIdFromSearchContext = tmp5(12598).getGuildIdFromSearchContext(searchContext);
          if (null != guildIdFromSearchContext) {
            const members = tmp(5738).requestMembers(guildIdFromSearchContext, trimmed, c17);
            const tmpResult4 = tmp(5738);
          }
          ...
          obj10.filters = obj11;
          const tmp5Result5 = tmp5(12598);
          obj10.boosters = tmp5(5661).getBoosterMap(AutocompleterResultTypes.USER);
