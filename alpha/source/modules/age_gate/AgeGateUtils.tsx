...
// Function ID: 5040
// Name: AgeGateUtils
// Dependencies: [2062, 2044, 5040, 2066, 1372, 1099, 1074, 1115, 11, 5041, 5730, 5731, 13292, 504, 7853, 2110, 6627, 6742, 2]
...
function shouldShowAgeGateForVoiceChannel(channelId) {
  ...
  if (tmp4) {
    ...
    let flag = false;
    if (null != currentUser) {
      const result1 = tmp(5041).shouldShowTiggerPawtect();
      const tmpResult = tmp(5041);
      const tmpResult2 = tmp(5730);
      const tmp9 = tmp(5730).isFeatureAgeGated(tmp(5731).AgeGatedFeature.AGE_GATED_SPACES) && result1;
      flag = true !== currentUser.nsfwAllowed || tmp(5730).isFeatureAgeGated(tmp(5731).AgeGatedFeature.AGE_GATED_SPACES) && result1;
      const tmp10 = true !== currentUser.nsfwAllowed || tmp(5730).isFeatureAgeGated(tmp(5731).AgeGatedFeature.AGE_GATED_SPACES) && result1;
    }
    let tmp12 = !flag;
    if (flag) {
...
export const useAgeGateVerifyContentForGuild = function useAgeGateVerifyContentForGuild(stateFromStores) {
  ...
  if (tmpResult.useIsFeatureAgeGated(AgeGatedFeature.AgeGatedFeature.AGE_GATED_SPACES)) {
    ...
    if (tmp3Result) {
      let tinyBroncoWarningDescriptions = tmp(13292).getTinyBroncoWarningDescriptions(tmp3Result, str);
      const tmpResult2 = tmp(13292);
    } else {
  ...
  if (isFeatureAgeGated) {
    if (!isAgeVerified) {
      ...
      obj5.modalType = tmp(7853).NsfwSpaceWarningModalType.NSFW_CHANNEL_AGE_VERIFY;
    }
    ...
    NSFW_CHANNEL_AGE_VERIFY = tmp(7853).NsfwSpaceWarningModalType.NSFW_CHANNEL_AGE_VERIFY;
    ...
    if (stateFromStores) {
      ...
      NSFW_CHANNEL_UNDERAGE = tmp(7853).NsfwSpaceWarningModalType.NSFW_CHANNEL_UNDERAGE;
      ...
      obj7.modalType = tmp(7853).NsfwSpaceWarningModalType.NSFW_CHANNEL_VERIFIED;
      return obj7;
    }
  }
};
export const useAgeGateVerifyContent = function useAgeGateVerifyContent(source) {
  ...
  if (tmpResult3.useIsFeatureAgeGated(AgeGatedFeature.AgeGatedFeature.AGE_GATED_SPACES)) {
    tinyBroncoWarningDescriptions = null;
    if (tmp7) {
      tinyBroncoWarningDescriptions = tmp(13292).getTinyBroncoWarningDescriptions(true, "");
      const tmpResult4 = tmp(13292);
    }
  }
