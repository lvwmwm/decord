...
// Dependencies: [5, 32, 19, 17, 7438, 2109, 1386, 502, 2042, 5584, 2105, 4462, 5711, 1074, 21, 4827, 576, 4761, 5586, 1397, 4678, 1177, 5325, 4980, 4823, 1115, 5710, 11894, 7496, 9631, 11895, 11896, 12, 4524, 11897, 38, 4521, 11599, 5288, 8947, 5709, 504, 5712, 5823, 1241, 5007, 4794, 7710, 11900, 9418, 573, 5030, 9403, 1980, 9395, 11904, 7481, 11905, 7534, 5271, 2]

// Module 11893 (GuildRoleConnectionsConnectAccountsActionSheet)
...
import ConnectionsUtils from "ConnectionsUtils" /* 5710 */;
...
import ConnectionsRoleActionCreators from "ConnectionsRoleActionCreators" /* 11904 */;
...
function ConnectionsChecks(eligibilityStatesGroups) {
  ...
  roleColor = eligibilityStatesGroups(4524).useToken(nativeDefault.unsafe_rawColors.GREEN_330);
  ...
  obj3.children = sorted.map((item) => {
    ...
    const officialApplicationIds = eligibilityStatesGroups(5710).officialApplicationIds;
    let str;
    if (application != null) {
      str = application.id;
    }
    ...
    if (tmp3) {
      const obj3 = { source: tmp5(11895), style: closure_5.connectionsChecksGroupCheckmark };
      ...
      tmp22Result = tmp22(tmp15(4823).Text, obj4);
      tmp24 = tmp22;
    } else if (tmp8) {
      const obj5 = { source: tmp5(11896), style: closure_5.connectionsChecksGroupCaret };
      ...
      tmp22Result = tmp22(tmp15(4823).Text, obj6);
      tmp24 = tmp22;
    }
    let type1;
    if (value != null) {
    ...
    if (hasItem) {
      ...
      let tmp24Result = tmp24(tmp5(11897), obj7);
    } else if (null != tmp11) {
      const obj8 = { style: closure_5.botTag, verified: tmp11.isVerifiedBot() };
      tmp24Result = tmp24(tmp5(9631), obj8);
    ...
    if (!tmp3) {
      ...
      if (tmp20) {
        ...
        obj10.children = tmp24(tmp15(4823).Text, obj11);
    ...
    const items2 = [tmp24(eligibilityStatesGroups(4823).Text, { variant: "text-md/medium", color: "mobile-text-heading-primary", children: name }), tmp24Result, ];
    ...
    if (null != tmp28) {
      const obj16 = {
        onPress() {
            ...
            obj.open({ key: "CONNECTIONS_STEAM_TOOLTIP", icon: _modDef11599, content: stringResult });
          },
        children: null
      };
      const obj17 = { source: tmp5(11599), size: tmp15(1177).Icon.Sizes.SMALL_20, style: tmp38.connectionsChecksGroupTextNameInfoIcon };
...
function ConnectedUserAccountOptions(account) {
  ...
  if (set.has(account.type)) {
    ...
    tmp12 = closure_29(account(8947).FormSwitchRow, obj2);
  }
  let tmp15;
  if (set2.has(account.type)) {
    ...
    tmp15 = closure_29(account(8947).FormSwitchRow, obj3);
  }
  let tmp18;
  if (true === value.hasMetadata) {
    ...
    tmp18 = closure_29(account(8947).FormSwitchRow, obj5);
  }
  ...
  const items = [closure_29(account(8947).FormSwitchRow, obj7), tmp18, tmp15, tmp12];
  obj6.children = items;
          ...
          handlePlatformConnect = function handlePlatformConnect(platformType, role_connections_verification_url) {
            ...
            let obj = {
              ...
              onClose() {
                ...
                let tmp7 = null;
                const guildRoleConnectionsConnectAccountsActionSheetKey = platformType(11900).makeGuildRoleConnectionsConnectAccountsActionSheetKey(role.id);
