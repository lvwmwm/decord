...
// Dependencies: [19, 17, 2109, 502, 21, 4827, 576, 9278, 9279, 9371, 9372, 9373, 9374, 504, 9280, 7499, 7637, 9022, 9033, 9375, 7496, 9384, 13275, 9363, 8685, 7538, 7946, 13279, 4823, 1115, 5425, 10522, 2]
// Exports: default

// Module 9277 (UserProfileApplicationWidgetCard)
...
export default function UserProfileApplicationWidgetCard(userId) {
  ...
  const getOrFetchApplication = userId(7499).useGetOrFetchApplication(widget.applicationId);
  let iconURL;
  if (getOrFetchApplication != null) {
    iconURL = getOrFetchApplication.getIconURL(16);
  }
  const obj3 = userId(7499);
  let canonicalGameId;
  if (getOrFetchApplication != null) {
    canonicalGameId = getOrFetchApplication.getCanonicalGameId();
  }
  const data = userId(7637).useGame(canonicalGameId).data;
  let id;
  const tmp2Result = userId(7637);
  if (data != null) {
    id = data.id;
  }
  const tmp10 = stateFromStores(9022);
  const tmp10Result = tmp10({ location: "UserProfileApplicationWidgetCard", applicationId: id, source: userId(9033).GameProfileSources.UserProfileApplicationWidget, sourceUserId: userId, trackEntryPointImpression: true, stackingBehavior: "stack" });
  dependencyMap = tmp10Result;
  const obj4 = { location: "UserProfileApplicationWidgetCard", applicationId: id, source: userId(9033).GameProfileSources.UserProfileApplicationWidget, sourceUserId: userId, trackEntryPointImpression: true, stackingBehavior: "stack" };
  ({ surfaceConfigs, resolutionContext, isLoading, hasIdentity } = stateFromStores(9375)(userId, widget.applicationId));
  const tmp14 = stateFromStores(7496)(getOrFetchApplication);
  token = tmp14.token;
  ({ fetched, canStartAuthorization } = tmp14);
  const tmp13 = stateFromStores(9375)(userId, widget.applicationId);
  const tmp15 = stateFromStores(9384)(widget.applicationId, stateFromStores1);
  ({ pending, refresh } = stateFromStores(13275)(widget.applicationId));
  const tmp17 = surfaceConfigs[userId(undefined, 9363).ApplicationWidgetConfigSurface.WIDGET_TOP];
  const tmp18 = surfaceConfigs[userId(undefined, 9363).ApplicationWidgetConfigSurface.WIDGET_BOTTOM];
  let tmp19 = null;
  if (null != iconURL) {
  ...
  if (stateFromStores1) {
    if (!isLoading) {
      if (!hasIdentity) {
        ...
        if (tmp22) {
          const _Array = Array;
          let someResult = Array.from(tmp2(8685).OAuth2ScopesSets.APPLICATION_IDENTITIES_SCOPES).some((item) => {
            const scopes = token.scopes;
            return scopes.includes(item);
          });
          if (!someResult) {
            let scopes = token.scopes;
            someResult = scopes.includes(tmp2(8685).OAuth2Scopes.SDK_SOCIAL_LAYER);
          }
          if (!someResult) {
            const scopes2 = token.scopes;
            someResult = scopes2.includes(tmp2(8685).OAuth2Scopes.SDK_SOCIAL_LAYER_PRESENCE);
          }
          tmp22 = someResult;
          const arr = Array.from(tmp2(8685).OAuth2ScopesSets.APPLICATION_IDENTITIES_SCOPES);
        }
        if (fetched) {
        ...
        const tmp9Result = tmp9(7538);
        obj7.title = tmp2(7946).getWidgetTitle(widget);
        ...
        const items3 = [closure_9(tmp2(13279).HourglassIcon, obj9), ];
        const obj10 = { variant: "text-sm/medium", color: "text-muted", children: null };
        const intl = tmp2(1115).intl;
        obj10.children = intl.string(tmp2(1115).t.z5K4Uv);
        items3[1] = closure_9(tmp2(4823).Text, obj10);
        obj8.children = items3;
        obj7.children = closure_10(closure_6, obj8);
        tmp25 = closure_9(tmp9Result, obj7);
        const tmp2Result5 = tmp2(7946);
      }
    }
  }
  const tmp16 = stateFromStores(13275)(widget.applicationId);
  const result = userId(9280).bindResolveFieldValue(resolutionContext);
  const obj11 = {
    style: tmp.header,
    onPress() {
      let tmp;
  ...
  const tmp2Result6 = userId(9280);
  obj11.accessibilityLabel = userId(7946).getWidgetTitle(widget);
  const items4 = [tmp19, ];
  const obj12 = { variant: "text-sm/medium", color: "text-strong", lineClamp: 1, children: null };
  const tmp2Result7 = userId(7946);
  obj12.children = userId(7946).getWidgetTitle(widget);
  items4[1] = closure_9(userId(4823).Text, obj12);
  ...
  if (null != tmp17) {
    if (tmp2(9278).ApplicationWidgetLayoutName.WIDGET_TOP_HERO === layout) {
      const obj13 = { header: tmp33, topConfig: tmp17, resolveFieldValue: result, numberFormat: memo };
      tmp32Result = tmp32(tmp9(9279), obj13);
    } else {
      tmp32Result = null;
      if (tmp2(9278).ApplicationWidgetLayoutName.WIDGET_TOP_CONTAINED === layout) {
        const obj14 = { header: tmp33, topConfig: tmp17, resolveFieldValue: result, numberFormat: memo };
        tmp32Result = tmp32(tmp9(9371), obj14);
      }
    }
  }
  if (tmp18 != null) {
  ...
  if (null != tmp18) {
    if (tmp2(9278).ApplicationWidgetLayoutName.WIDGET_BOTTOM_STATS === layout2) {
      const obj15 = { bottomConfig: tmp18, resolveFieldValue: result, numberFormat: memo };
      tmp32Result2 = tmp32(tmp9(9372), obj15);
    } else if (tmp2(9278).ApplicationWidgetLayoutName.WIDGET_BOTTOM_PROGRESS === layout2) {
      const obj16 = { bottomConfig: tmp18, resolveFieldValue: result };
      tmp32Result2 = tmp32(tmp9(9373), obj16);
    } else {
      tmp32Result2 = null;
      if (tmp2(9278).ApplicationWidgetLayoutName.WIDGET_BOTTOM_COLLECTION === layout2) {
        const obj17 = { bottomConfig: tmp18, resolveFieldValue: result };
        tmp32Result2 = tmp32(tmp9(9374), obj17);
      }
    }
  }
  ...
  if (null != tmp32Result) {
    ...
    if (null != tmp32Result2) {
      ...
      if (true === tmp15) {
        ...
        const items6 = [tmp32(tmp2(10522).RetryIcon, obj21), ];
        const obj22 = { variant: "text-xs/medium", color: "text-muted", children: null };
        const intl3 = tmp2(1115).intl;
        obj22.children = intl3.string(tmp2(1115).t.wzzjk9);
        items6[1] = tmp32(tmp2(4823).Text, obj22);
        obj20.children = items6;
        tmp31Result = tmp31(tmp2(5425).PressableOpacity, obj20);
      }
      items5[3] = tmp31Result;
      obj18.children = items5;
      tmp31Result2 = tmp31(tmp9(7538), obj18);
      const tmp9Result2 = tmp9(7538);
    }
  }
  return tmp31Result2;
};
