...
// Dependencies: [19, 17, 2109, 502, 21, 4756, 576, 9202, 9203, 9295, 9296, 9297, 9298, 504, 9204, 7413, 7551, 8946, 8957, 9299, 7410, 9308, 13189, 9287, 8610, 7452, 7860, 13193, 4752, 1115, 5339, 10437, 2]
// Exports: default

// Module 9201 (UserProfileApplicationWidgetCard)
...
export default function UserProfileApplicationWidgetCard(userId) {
  ...
  const getOrFetchApplication = userId(7413).useGetOrFetchApplication(widget.applicationId);
  let iconURL;
  if (getOrFetchApplication != null) {
    iconURL = getOrFetchApplication.getIconURL(16);
  }
  const obj3 = userId(7413);
  let canonicalGameId;
  if (getOrFetchApplication != null) {
    canonicalGameId = getOrFetchApplication.getCanonicalGameId();
  }
  const data = userId(7551).useGame(canonicalGameId).data;
  let id;
  const tmp2Result = userId(7551);
  if (data != null) {
    id = data.id;
  }
  const tmp10 = stateFromStores(8946);
  const tmp10Result = tmp10({ location: "UserProfileApplicationWidgetCard", applicationId: id, source: userId(8957).GameProfileSources.UserProfileApplicationWidget, sourceUserId: userId, trackEntryPointImpression: true, stackingBehavior: "stack" });
  dependencyMap = tmp10Result;
  const obj4 = { location: "UserProfileApplicationWidgetCard", applicationId: id, source: userId(8957).GameProfileSources.UserProfileApplicationWidget, sourceUserId: userId, trackEntryPointImpression: true, stackingBehavior: "stack" };
  ({ surfaceConfigs, resolutionContext, isLoading, hasIdentity } = stateFromStores(9299)(userId, widget.applicationId));
  const tmp14 = stateFromStores(7410)(getOrFetchApplication);
  token = tmp14.token;
  ({ fetched, canStartAuthorization } = tmp14);
  const tmp13 = stateFromStores(9299)(userId, widget.applicationId);
  const tmp15 = stateFromStores(9308)(widget.applicationId, stateFromStores1);
  ({ pending, refresh } = stateFromStores(13189)(widget.applicationId));
  const tmp17 = surfaceConfigs[userId(undefined, 9287).ApplicationWidgetConfigSurface.WIDGET_TOP];
  const tmp18 = surfaceConfigs[userId(undefined, 9287).ApplicationWidgetConfigSurface.WIDGET_BOTTOM];
  let tmp19 = null;
  if (null != iconURL) {
  ...
  if (stateFromStores1) {
    if (!isLoading) {
      if (!hasIdentity) {
        ...
        if (tmp22) {
          const _Array = Array;
          let someResult = Array.from(tmp2(8610).OAuth2ScopesSets.APPLICATION_IDENTITIES_SCOPES).some((item) => {
            const scopes = token.scopes;
            return scopes.includes(item);
          });
          if (!someResult) {
            let scopes = token.scopes;
            someResult = scopes.includes(tmp2(8610).OAuth2Scopes.SDK_SOCIAL_LAYER);
          }
          if (!someResult) {
            const scopes2 = token.scopes;
            someResult = scopes2.includes(tmp2(8610).OAuth2Scopes.SDK_SOCIAL_LAYER_PRESENCE);
          }
          tmp22 = someResult;
          const arr = Array.from(tmp2(8610).OAuth2ScopesSets.APPLICATION_IDENTITIES_SCOPES);
        }
        if (fetched) {
        ...
        const tmp9Result = tmp9(7452);
        obj7.title = tmp2(7860).getWidgetTitle(widget);
        ...
        const items3 = [closure_9(tmp2(13193).HourglassIcon, obj9), ];
        ...
        const tmp2Result5 = tmp2(7860);
      }
    }
  }
  const tmp16 = stateFromStores(13189)(widget.applicationId);
  const result = userId(9204).bindResolveFieldValue(resolutionContext);
  const obj11 = {
    style: tmp.header,
    onPress() {
      let tmp;
  ...
  const tmp2Result6 = userId(9204);
  obj11.accessibilityLabel = userId(7860).getWidgetTitle(widget);
  const items4 = [tmp19, ];
  const obj12 = { variant: "text-sm/medium", color: "text-strong", lineClamp: 1, children: null };
  const tmp2Result7 = userId(7860);
  obj12.children = userId(7860).getWidgetTitle(widget);
  ...
  if (null != tmp17) {
    if (tmp2(9202).ApplicationWidgetLayoutName.WIDGET_TOP_HERO === layout) {
      const obj13 = { header: tmp33, topConfig: tmp17, resolveFieldValue: result, numberFormat: memo };
      tmp32Result = tmp32(tmp9(9203), obj13);
    } else {
      tmp32Result = null;
      if (tmp2(9202).ApplicationWidgetLayoutName.WIDGET_TOP_CONTAINED === layout) {
        const obj14 = { header: tmp33, topConfig: tmp17, resolveFieldValue: result, numberFormat: memo };
        tmp32Result = tmp32(tmp9(9295), obj14);
      }
    }
  }
  if (tmp18 != null) {
  ...
  if (null != tmp18) {
    if (tmp2(9202).ApplicationWidgetLayoutName.WIDGET_BOTTOM_STATS === layout2) {
      const obj15 = { bottomConfig: tmp18, resolveFieldValue: result, numberFormat: memo };
      tmp32Result2 = tmp32(tmp9(9296), obj15);
    } else if (tmp2(9202).ApplicationWidgetLayoutName.WIDGET_BOTTOM_PROGRESS === layout2) {
      const obj16 = { bottomConfig: tmp18, resolveFieldValue: result };
      tmp32Result2 = tmp32(tmp9(9297), obj16);
    } else {
      tmp32Result2 = null;
      if (tmp2(9202).ApplicationWidgetLayoutName.WIDGET_BOTTOM_COLLECTION === layout2) {
        const obj17 = { bottomConfig: tmp18, resolveFieldValue: result };
        tmp32Result2 = tmp32(tmp9(9298), obj17);
      }
    }
  }
  ...
  if (null != tmp32Result) {
    ...
    if (null != tmp32Result2) {
      ...
      if (true === tmp15) {
        ...
        const items6 = [tmp32(tmp2(10437).RetryIcon, obj21), ];
        ...
        tmp31Result = tmp31(tmp2(5339).PressableOpacity, obj20);
      }
      items5[3] = tmp31Result;
      obj18.children = items5;
      tmp31Result2 = tmp31(tmp9(7452), obj18);
      const tmp9Result2 = tmp9(7452);
    }
  }
  return tmp31Result2;
};
