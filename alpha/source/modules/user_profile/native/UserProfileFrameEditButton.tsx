...
// Dependencies: [32, 19, 17, 6624, 2041, 1085, 21, 576, 4829, 6801, 2028, 7604, 10497, 1973, 4796, 14164, 1980, 1115, 14151, 5884, 8277, 1177, 12729, 2]
// Exports: default

// Module 14163 (UserProfileFrameEditButton)
...
export default function UserProfileFrameEditButton(arg0) {
  ...
  const tmp4 = userProfileFrame(user(6801).useSelectedDismissibleContent(items), 2);
  dependencyMap = tmp5;
  const obj = user(6801);
  ...
  const fetchCollectiblesProduct = user(10497).useFetchCollectiblesProduct(skuId);
  ...
  const tmp2Result = user(10497);
  userProfileFrame = user(7604).useUserProfileFrame({ user, guildId });
  ...
  const callback = noop.useCallback(() => {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14164, dependencyMap.paths), "Profile Frame", { user, currentProfileFrame: userProfileFrame, guildId });
  ...
  if (fetchCollectiblesProduct.isFetching) {
    ...
    obj5.onPress = NOOP;
    obj5.leading = tmp18(tmp2(5884).ActivityIndicator, { animating: true, size: "large" });
    let obj6 = obj5;
  } else {
    ...
    obj6.labelTrailing = tmp18(tmp2(14151).UserProfileEditFormLabelBadges, obj7);
    ...
    if (null != first1) {
      ...
      obj9.children = tmp18(guildId(8277), obj10);
      let tmp18Result = tmp18(View, obj9);
      const tmp23 = guildId(8277);
    } else {
      const obj11 = { source: guildId(12729), style: tmp.noneIcon };
      tmp18Result = tmp18(tmp2(1177).Icon, obj11);
    }
    obj6.leading = tmp18Result;
  }
  return jsx(user(14151).UserProfileEditFormButton, obj6);
};
