...
// Dependencies: [32, 19, 17, 7539, 2039, 1085, 21, 576, 4827, 7716, 2028, 8510, 11393, 1973, 4794, 14993, 1980, 1115, 14980, 5880, 9179, 1177, 13566, 2]
// Exports: default

// Module 14992 (UserProfileFrameEditButton)
...
export default function UserProfileFrameEditButton(arg0) {
  ...
  const tmp4 = userProfileFrame(user(7716).useSelectedDismissibleContent(items), 2);
  dependencyMap = tmp5;
  const obj = user(7716);
  ...
  const fetchCollectiblesProduct = user(11393).useFetchCollectiblesProduct(skuId);
  ...
  const tmp2Result = user(11393);
  userProfileFrame = user(8510).useUserProfileFrame({ user, guildId });
  ...
  const callback = noop.useCallback(() => {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14993, dependencyMap.paths), "Profile Frame", { user, currentProfileFrame: userProfileFrame, guildId });
  ...
  if (fetchCollectiblesProduct.isFetching) {
    ...
    obj5.onPress = NOOP;
    obj5.leading = tmp18(tmp2(5880).ActivityIndicator, { animating: true, size: "large" });
    let obj6 = obj5;
  } else {
    ...
    obj6.labelTrailing = tmp18(tmp2(14980).UserProfileEditFormLabelBadges, obj7);
    ...
    if (null != first1) {
      ...
      obj9.children = tmp18(guildId(9179), obj10);
      let tmp18Result = tmp18(View, obj9);
      const tmp23 = guildId(9179);
    } else {
      const obj11 = { source: guildId(13566), style: tmp.noneIcon };
      tmp18Result = tmp18(tmp2(1177).Icon, obj11);
    }
    obj6.leading = tmp18Result;
  }
  return jsx(user(14980).UserProfileEditFormButton, obj6);
};
