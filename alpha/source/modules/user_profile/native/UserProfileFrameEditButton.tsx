...
// Dependencies: [32, 19, 17, 7455, 2039, 1085, 21, 576, 4757, 7633, 2028, 8439, 11317, 1973, 4724, 14913, 1980, 1115, 14900, 5796, 9108, 1177, 13484, 2]
// Exports: default

// Module 14912 (UserProfileFrameEditButton)
...
export default function UserProfileFrameEditButton(arg0) {
  ...
  const tmp4 = userProfileFrame(user(7633).useSelectedDismissibleContent(items), 2);
  dependencyMap = tmp5;
  const obj = user(7633);
  ...
  const fetchCollectiblesProduct = user(11317).useFetchCollectiblesProduct(skuId);
  ...
  const tmp2Result = user(11317);
  userProfileFrame = user(8439).useUserProfileFrame({ user, guildId });
  ...
  const callback = noop.useCallback(() => {
    ActionSheetActionCreatorsDefault.openLazy(asyncRequireImpl(14913, dependencyMap.paths), "Profile Frame", { user, currentProfileFrame: userProfileFrame, guildId });
  ...
  if (fetchCollectiblesProduct.isFetching) {
    ...
    obj6.labelTrailing = tmp18(tmp2(14900).UserProfileEditFormLabelBadges, obj7);
    ...
    if (null != first1) {
      ...
      obj9.children = tmp18(guildId(9108), obj10);
      let tmp18Result = tmp18(View, obj9);
      const tmp23 = guildId(9108);
    } else {
      const obj11 = { source: guildId(13484), style: tmp.noneIcon };
      tmp18Result = tmp18(tmp2(1177).Icon, obj11);
    }
    obj6.leading = tmp18Result;
  }
  return jsx(user(14900).UserProfileEditFormButton, obj6);
};
