...
function createUserWidgetFromServer(data) {
  ...
  if (WidgetType.WidgetType.CURRENT_GAMES !== type) {
    if (tmp(7036).WidgetType.FAVORITE_GAMES !== type) {
      if (tmp(7036).WidgetType.PLAYED_GAMES !== type) {
        if (tmp(7036).WidgetType.WANT_TO_PLAY_GAMES !== type) {
          if (tmp(7036).WidgetType.APPLICATION === type) {
          ...
          } else if (tmp(7036).WidgetType.PERSONAL === type) {
            ...
            obj3.header = str;
            obj3.sections = tmp(7044).parsePersonalWidgetSections(data.data.sections);
            const userProfilePersonalWidget = new tmp(7044).UserProfilePersonalWidget(obj3);
            return userProfilePersonalWidget;
          } else if (tmp(7036).WidgetType.CLIPS_GALLERY === type) {
  ...
  const baseGameWidget = new tmp(7037).BaseGameWidget({ id: data.id, type, games: _modDef12.uniqBy(mapped1, "gameId") });
...
function createUserWidgetFromSnapshot(type) {
  ...
  const baseGameWidget = new tmp(7037).BaseGameWidget({ id: id4, type: type3, games });
  return baseGameWidget;
}
