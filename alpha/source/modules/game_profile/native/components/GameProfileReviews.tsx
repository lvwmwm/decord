...
// Dependencies: [19, 17, 21, 4757, 576, 4477, 8959, 4452, 9006, 9007, 8962, 1115, 8970, 4753, 2019, 9008, 9014, 8966, 8967, 2]
// Exports: default
...
function SteamReviewRow(url) {
  ...
  const items2 = [closure_7(url(8970).SteamNeutralIcon, { size: "sm", color: tmp4(576).colors.ICON_STRONG }), closure_7(url(4753).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  ...
  if (tmp12Result) {
    ...
    tmp12Result = tmp12(tmp2(4753).Text, obj8);
...
function OpenCriticReview(url) {
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(9014), obj11), ];
        ...
        obj12.children = tmp12(tmp11(4753).Text, obj13);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      ...
      tmp12Result2 = tmp12(tmp11(4753).Text, obj14);
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(8967).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    const calculateSteamReviewScoreDescription = tmp2(9006).calculateSteamReviewScoreDescription;
    if (steam != null) {
    ...
    const result = tmp2(9007).canShowLocalizedSteamReview(steam);
    if (result) {
      let localizedRating;
      if (!tmp12) {
    ...
    const items = [React5(tmp2(4753).Text, obj3), ];
