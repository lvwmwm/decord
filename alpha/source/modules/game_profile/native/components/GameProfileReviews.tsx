...
// Dependencies: [19, 17, 21, 4827, 576, 4543, 9030, 4518, 9077, 9078, 9033, 1115, 9041, 4823, 2019, 9079, 9085, 9037, 9038, 2]
// Exports: default
...
function SteamReviewRow(url) {
  ...
  const items2 = [closure_7(url(9041).SteamNeutralIcon, { size: "sm", color: tmp4(576).colors.ICON_STRONG }), closure_7(url(4823).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  ...
  if (tmp12Result) {
    ...
    tmp12Result = tmp12(tmp2(4823).Text, obj8);
...
function OpenCriticReview(url) {
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(9085), obj11), ];
        ...
        obj12.children = tmp12(tmp11(4823).Text, obj13);
  ...
  if (topCriticRating <= 0) {
    ...
    if (null == tier) {
      ...
      tmp12Result2 = tmp12(tmp11(4823).Text, obj14);
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(9038).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    const calculateSteamReviewScoreDescription = tmp2(9077).calculateSteamReviewScoreDescription;
    if (steam != null) {
    ...
    let tmp10 = tmp6;
    if (tmp6) {
      tmp10 = tmp9 !== tmp2(2019).SteamReviewScoreDescription.NO_USER_REVIEWS;
    }
    const tmp2Result = tmp2(9077);
    const result = tmp2(9078).canShowLocalizedSteamReview(steam);
    if (result) {
      let localizedRating;
      if (!tmp12) {
    ...
    const items = [React5(tmp2(4823).Text, obj3), ];
