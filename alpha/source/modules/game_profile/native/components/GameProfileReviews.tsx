...
// Dependencies: [19, 17, 21, 4829, 576, 4545, 9034, 4520, 9081, 9082, 9037, 1115, 9045, 4825, 2019, 9083, 9089, 9041, 9042, 2]
// Exports: default
...
function SteamReviewRow(url) {
  ...
  const items2 = [closure_7(url(9045).SteamNeutralIcon, { size: "sm", color: tmp4(576).colors.ICON_STRONG }), closure_7(url(4825).Text, { variant: "heading-sm/medium", color: "mobile-text-heading-primary", children: title })];
  ...
  if (null != tier) {
    ...
    if (topCriticRating > 0) {
      ...
      if (num > 0) {
        ...
        const items4 = [tmp12(trackAction(9089), obj11), ];
...
export default function GameProfileReviews(arg0) {
  ...
  if (null == game) {
    ...
    const tmp6 = game.steamReleaseStatus !== tmp2(9042).SteamReleaseStatus.RETIRED_ABANDONED && null != steamWebsiteUrl;
    const reviews = game.reviews;
    let steam;
    if (reviews != null) {
      steam = reviews.steam;
    }
    const calculateSteamReviewScoreDescription = tmp2(9081).calculateSteamReviewScoreDescription;
    if (steam != null) {
    ...
    const result = tmp2(9082).canShowLocalizedSteamReview(steam);
    if (result) {
      let localizedRating;
      if (!tmp12) {
