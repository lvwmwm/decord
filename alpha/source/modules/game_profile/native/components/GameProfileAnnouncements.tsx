...
// Dependencies: [19, 17, 9061, 21, 5291, 1115, 4827, 576, 9089, 7274, 9088, 9107, 9108, 4823, 9111, 4505, 9113, 7493, 9115, 9033, 9027, 9118, 9074, 2]
// Exports: default

// Module 9106 (GameProfileAnnouncements)
...
import AnnouncementMessageUtils from "AnnouncementMessageUtils" /* 9108 */;
import ImageWithPlaceholder from "ImageWithPlaceholder" /* 9111 */;
import navigateToGameAnnouncementDefault from "navigateToGameAnnouncement" /* 9118 */;
...
function EmbedAnnouncementCard(message) {
  ...
  if (null == embedSource) {
    ...
    if (tmp13Result2) {
      ...
      const items6 = [tmp11(tmp34(9113).ReactionIcon, obj23), ];
      ...
      items6[1] = tmp11(tmp34(4823).Text, obj25);
...
function MessageAnnouncementCard(message) {
  ...
  if (tmp10Result) {
    ...
    const items3 = [tmp26(tmp27(9113).ReactionIcon, obj15), ];
    ...
    items3[1] = tmp26(tmp27(4823).Text, obj17);
...
function PollAnnouncementCard(message) {
  ...
  if (null == poll) {
    ...
    const items = [closure_9(message(4823).Text, obj4), , ];
    ...
    if (tmp9Result) {
      ...
      tmp9Result = tmp9(tmp13(4823).Text, obj);
    }
    items1[1] = tmp9Result;
    ...
    const date = new Date(message.timestamp);
    obj9.createdAt = date;
    obj9.expiryLabel = message(9108).getPollExpiryLabel(poll);
    obj8.children = intl2.format(message(1115).t.t0FTsH, obj9);
    obj7.children = closure_9(message(4823).Text, obj8);
...
const MAX_VISIBLE_ANNOUNCEMENTS = fn(9061).MAX_VISIBLE_ANNOUNCEMENTS;
const jsxProd = fn(21);
