...
// Function ID: 10291
// Name: MessageManager
// Dependencies: [32, 4845, 5582, 2048, 7610, 2044, 2066, 4844, 2098, 4650, 1074, 2051, 1091, 3, 5577, 10291, 4759, 8724, 7786, 10292, 510, 4655, 1101, 7612, 5195, 1115, 573, 7451, 2]
      ...
      if (type !== constants.GUILD_STORE) {
        ...
        if (null == type1) {
          ...
          if (orCreate.some(AttachmentUrlUtilsAll.messageHasExpiredAttachmentUrl)) {
            ...
            tmp9(5577).clear(channelId);
            const tmp9Result = tmp9(5577);
            orCreate1 = tmp9(5577).getOrCreate(channelId);
            const tmp9Result9 = tmp9(5577);
          }
          let obj7 = orCreate1;
          if (tmp15) {
            const obj = { jumpTargetId: null, jumped: false, jumpType: Client.JumpType.ANIMATED };
            const mutation = orCreate1.mutate(obj);
            tmp9(5577).commit(mutation);
            obj7 = mutation;
            const tmp9Result10 = tmp9(5577);
          }
          let obj10 = obj7;
          if (tmp19) {
            const mutation1 = obj7.mutate({ focusTargetId: null });
            tmp9(5577).commit(mutation1);
            obj10 = mutation1;
            const tmp9Result11 = tmp9(5577);
          }
          if (isPreload) {
            if (!GatewayConnectionStore.isConnected()) {
              let flag = true;
            }
            let hasUnreadResult = tmp9(8724)(channelId);
            if (hasUnreadResult) {
            ...
            if (flag) {
              tmp9(5577).commit(obj10.mutate({ loadingMore: true }));
              if (null == messageId) {
                let isThreadResult;
                if (channel != null) {
                ...
                tmp9(7786).jumpToMessage(obj12);
                const tmp9Result16 = tmp9(7786);
              }
              const tmp9Result12 = tmp9(5577);
            }
          }
