...
// Dependencies: [7921, 4472, 1372, 1074, 8614, 11875, 1097, 8611, 4518, 5058, 8605, 4980, 5325, 2]
// Exports: handleMessagesTapImage

// Module 11877 (handleMessagesTapImage)
...
export const handleMessagesTapImage = function handleMessagesTapImage(tapImageData) {
  ...
  if (null != portal) {
    embedId(8614).markPortalAlive(portal);
    const obj = embedId(8614);
  }
  if (true === allowWithinModal) {
    ...
    if (type1 !== embedId(1097).MessageReferenceTypes.FORWARD) {
      ...
      const found = attachments.filter((item) => !embedId(8611).isThumbnailAttachment(item));
      if ("attachment" === type) {
        if (index < found.length) {
          const tmp10Result = tmp10(8611);
      ...
      if ("embed" === type) {
        if (null != embedIndex) {
          ...
          if (importDefault < result.length) {
            tmp10(8611).setMediaSourcePortal(result[importDefault], portal);
            found2 = result;
            tmp25 = tmp10;
            const tmp10Result8 = tmp10(8611);
          }
        }
        if (-1 !== importDefault) {
          ...
          let channelName;
          if (showContextName) {
            const tmp25Result3 = tmp25(4980);
            channelName = tmp25Result3.computeChannelName(messageChannel, UserStore, RelationshipStore, false);
          }
          obj3.contextName = channelName;
          let channelIcon;
          if (showContextName) {
            channelIcon = tmp25(5325).getChannelIcon(messageChannel);
            const tmp25Result4 = tmp25(5325);
          }
          obj3.contextIcon = channelIcon;
          tmp25(8605).openMediaModal(obj3);
          const tmp25Result = tmp25(8605);
        }
      }
      if ("component" === type) {
        if (null == componentId) {
          ...
          if (null != embedId) {
            ...
            if (null != components) {
              if (0 !== components.length) {
                const tmp10Result9 = tmp10(8611);
                const result1 = tmp10Result9.extractMediaSourcesFromComponent(tmp7, components, messageChannel.guild_id, tmp10(5058).asComponentId(componentId), componentMediaIndex);
                if (null != result1) {
                  ({ sources, initialIndex: closure_1 } = result1);
                  tmp10(8611).setMediaSourcePortal(sources[importDefault], portal);
                  found2 = sources;
                  tmp25 = tmp10;
                  const tmp10Result11 = tmp10(8611);
                }
                const tmp10Result10 = tmp10(5058);
              }
            }
          }
        ...
        const result2 = tmp10(8611).extractMediaSourcesFromMessage(tmp7, tmp12, messageChannel.guild_id);
        let num2 = 0;
        found2 = result2;
        tmp25 = tmp10;
        if (0 < result2.length) {
          ...
          if (flattenSourceResult.noCarousel) {
            ...
            importDefault = 0;
            tmp20(8611).setMediaSourcePortal(items[0], portal);
            found2 = items;
            tmp25 = tmp20;
            const tmp20Result = tmp20(8611);
          } else {
            importDefault = num2;
            tmp20(8611).setMediaSourcePortal(result2[num2], portal);
            ...
            tmp25 = tmp20;
            const tmp20Result2 = tmp20(8611);
          }
        }
        const tmp10Result12 = tmp10(8611);
      }
    } else {
