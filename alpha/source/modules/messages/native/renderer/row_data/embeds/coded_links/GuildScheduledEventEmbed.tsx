...
// Dependencies: [32, 17, 6941, 2062, 2044, 2066, 4476, 1372, 10838, 2050, 7150, 4819, 7382, 7383, 9052, 8938, 8935, 9062, 1115, 7386, 9301, 7373, 576, 8802, 8972, 4982, 9048, 9061, 12771, 8970, 2058, 2]
// Exports: createGuildScheduledEventInviteEmbed, createGuildScheduledEventLinkEmbed

// Module 12770 (GuildScheduledEventEmbed)
...
import GuildEventUtils from "GuildEventUtils" /* 9048 */;
...
import GuildScheduledEventHeaderUtils from "GuildScheduledEventHeaderUtils" /* 9062 */;
...
function createGuildScheduledEventEmbed(type) {
  ...
  const assetUriForEmbed1 = renderer_EmbedUtils.getAssetUriForEmbed(tmp(7386));
  const tmp7Result12 = renderer_EmbedUtils;
  const assetUriForEmbed2 = renderer_EmbedUtils.getAssetUriForEmbed(tmp(9301));
  if (null != guild) {
    if (tmp9) {
    ...
    acceptLabelActiveBackgroundColor = tmp7(7373).processColorOrThrow(tmp(576).unsafe_rawColors.GREEN_360);
    const intl2 = tmp7(1115).intl;
    stringResult2 = intl2.string(tmp7(1115).t.XpeFYr);
    acceptLabelGreenColor = colors.acceptLabelGreenColor;
    flag2 = false;
    const tmp7Result15 = tmp7(7373);
  }
  ...
  obj8.badgeIcon = renderer_EmbedUtils.getAssetUriForEmbed(_modDef12771);
  let assetUriForEmbed4;
  if (null != eventLocationIconSource) {
    assetUriForEmbed4 = tmp7(7383).getAssetUriForEmbed(eventLocationIconSource);
    const tmp7Result20 = tmp7(7383);
  }
  ...
  if (null != user) {
    assetUriForEmbed5 = tmp7(7383).getAssetUriForEmbed(user.getAvatarSource(guildEvent.guild_id));
    const tmp7Result21 = tmp7(7383);
  }
...
export const createGuildScheduledEventLinkEmbed = function createGuildScheduledEventLinkEmbed(code, theme) {
  ...
  if (nextRecurrenceIdInEvent == null) {
    nextRecurrenceIdInEvent = first(8935).getNextRecurrenceIdInEvent(guildScheduledEvent);
    const obj2 = first(8935);
  }
  if (null != guildScheduledEvent) {
