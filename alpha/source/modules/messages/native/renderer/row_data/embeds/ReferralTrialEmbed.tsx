...
// Dependencies: [2042, 1372, 4421, 7695, 7484, 1074, 1374, 4757, 576, 7487, 8208, 8330, 1115, 8331, 4601, 4415, 7694, 8318, 2108, 1609, 8314, 2]
...
export const createReferralTrialEmbedRedeemable = function createReferralTrialEmbedRedeemable(message, theme, id, relevantUserTrialOffer) {
  if (null != message.author) {
    ...
    if (null != channel) {
      if (channel.isDM()) {
        const obj = { backgroundColor, borderColor: backgroundColor, thumbnailCornerRadius: 3, headerLogoUrl: tmp44(8208).getAssetUriForEmbed(tmp46(8330)), headerText: null, headerColor: null, thumbnailUrl: null };
        const intl = tmp44(1115).intl;
        const tmp44Result = tmp44(8208);
        ...
        obj.thumbnailUrl = tmp44(8208).getAssetUriForEmbed(tmp46(8331));
        let userId;
        ...
        if (null != user) {
          if (null != user1) {
            const name = tmp46(4601).getName(user1);
            const tmp46Result = tmp46(4601);
            const name1 = tmp46(4601).getName(user);
            ...
            if (null == relevantUserTrialOffer) {
              ...
              const isPremiumResult = tmp46(4415).isPremium(user);
              ...
              const tmp46Result7 = tmp46(4415);
              const result = tmp44(7694).hasUserTrialOfferExpired(relevantUserTrialOffer);
              if (!tmp14) {
                const expiresAt = relevantUserTrialOffer.expiresAt;
                const referralTrialOfferExpirationCopy = tmp44(8318).getReferralTrialOfferExpirationCopy(expiresAt.getTime());
                const intl2 = tmp44(1115).intl;
                const tmp44Result9 = tmp44(8318);
                ...
                obj7.duration = tmp44(4415).formatIntervalDuration(obj6);
                const tmp44Result10 = tmp44(4415);
              ...
              if (tmp10) {
                if (!tmp20) {
                  ...
                  if (tmp29) {
                    ...
                    obj11.acceptLabelIconUrl = tmp44(8208).getAssetUriForEmbed(tmp46(8314));
                    tmp33 = obj11;
                    const tmp44Result11 = tmp44(8208);
                  }
                  return tmp33;
                }
              }
              ...
              const tmp44Result8 = tmp44(7694);
            }
            const tmp46Result6 = tmp46(4601);
          }
        }
        const obj17 = {};
