...
// Dependencies: [2042, 1372, 4487, 7778, 7568, 1074, 1374, 4827, 576, 7571, 8290, 8412, 1115, 8413, 4671, 4481, 7777, 8400, 2108, 1609, 8396, 2]
...
export const createReferralTrialEmbedRedeemable = function createReferralTrialEmbedRedeemable(message, theme, id, relevantUserTrialOffer) {
  if (null != message.author) {
    ...
    if (null != channel) {
      if (channel.isDM()) {
        const obj = { backgroundColor, borderColor: backgroundColor, thumbnailCornerRadius: 3, headerLogoUrl: tmp44(8290).getAssetUriForEmbed(tmp46(8412)), headerText: null, headerColor: null, thumbnailUrl: null };
        const intl = tmp44(1115).intl;
        const tmp44Result = tmp44(8290);
        ...
        obj.thumbnailUrl = tmp44(8290).getAssetUriForEmbed(tmp46(8413));
        let userId;
        ...
        if (null != user) {
          if (null != user1) {
            const name = tmp46(4671).getName(user1);
            const tmp46Result = tmp46(4671);
            const name1 = tmp46(4671).getName(user);
            ...
            if (null == relevantUserTrialOffer) {
              ...
              const values = Object.values(tmp44(7571).TrialIdToProductOfferId[closure_9]);
              id = undefined;
              ...
              const isPremiumResult = tmp46(4481).isPremium(user);
              ...
              const tmp46Result7 = tmp46(4481);
              const result = tmp44(7777).hasUserTrialOfferExpired(relevantUserTrialOffer);
              if (!tmp14) {
                const expiresAt = relevantUserTrialOffer.expiresAt;
                const referralTrialOfferExpirationCopy = tmp44(8400).getReferralTrialOfferExpirationCopy(expiresAt.getTime());
                const intl2 = tmp44(1115).intl;
                const tmp44Result9 = tmp44(8400);
                ...
                obj7.duration = tmp44(4481).formatIntervalDuration(obj6);
                const tmp44Result10 = tmp44(4481);
              ...
              if (tmp10) {
                if (!tmp20) {
                  ...
                  if (tmp29) {
                    ...
                    obj11.acceptLabelIconUrl = tmp44(8290).getAssetUriForEmbed(tmp46(8396));
                    tmp33 = obj11;
                    const tmp44Result11 = tmp44(8290);
                  }
                  return tmp33;
                }
              }
              ...
              const tmp44Result8 = tmp44(7777);
            }
            const tmp46Result6 = tmp46(4671);
          }
        }
        const obj17 = {};
