...
// Dependencies: [17, 2060, 4779, 2042, 2105, 2064, 4741, 4406, 1372, 11576, 1074, 7981, 8207, 1115, 8198, 576, 4601, 4608, 12066, 12067, 2056, 2108, 8208, 12982, 12981, 11578, 11577, 13522, 1385, 8667, 1397, 1879, 5241, 9026, 4910, 2]
...
export const createExpiredGuildInvite = function createExpiredGuildInvite(author, arg1, theme) {
  ...
  if (arg1) {
    ...
    if (null != username) {
      ...
      const obj2 = { username: tmp(4601).getFormattedName(author.author) };
  ...
  if (tmp6Result2.isThemeDark(theme)) {
    let tmpResult2 = tmp(12066);
  } else {
    tmpResult2 = tmp(12067);
  }
...
export const createGuildInvite = function createGuildInvite(invite, isOwnInvite, theme) {
  ...
  const channel = tmp(11577)(invite).channel;
    ...
    if (flag2) {
      if (null != target_user) {
        if (tmp12) {
          ...
          obj6.name = tmp(4601).getFormattedName(target_user);
          const tmpResult8 = tmp(4601);
  ...
  if (tmp7) {
    ...
    if (null != guild) {
      guildBadgeImageSource = tmp10(9026).getGuildBadgeImageSource(guild, theme);
      const tmp10Result10 = tmp10(9026);
    }
    const obj11 = {};
    const merged = Object.assign(baseColors);
    let formatted;
    ...
    let channelName;
    if (null != channel) {
      channelName = tmp10(4910).computeChannelName(channel, UserStore, RelationshipStore);
      const tmp10Result11 = tmp10(4910);
    }
    ...
    if (null != guildBadgeImageSource) {
      assetUriForEmbed = tmp10(8208).getAssetUriForEmbed(guildBadgeImageSource);
      const tmp10Result12 = tmp10(8208);
    }
