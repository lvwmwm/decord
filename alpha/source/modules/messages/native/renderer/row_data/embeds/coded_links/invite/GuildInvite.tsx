...
// Dependencies: [17, 2062, 4851, 2044, 2107, 2066, 4813, 4476, 1372, 10838, 1074, 7150, 7382, 1115, 7373, 576, 4675, 4682, 11272, 11273, 2058, 2110, 7383, 12221, 12220, 10840, 10839, 12767, 1385, 7832, 1397, 1879, 5328, 8195, 4982, 2]
...
export const createExpiredGuildInvite = function createExpiredGuildInvite(author, arg1, theme) {
  ...
  if (arg1) {
    ...
    if (null != username) {
      ...
      const obj2 = { username: tmp(4675).getFormattedName(author.author) };
  ...
  if (tmp6Result2.isThemeDark(theme)) {
    let tmpResult2 = tmp(11272);
  } else {
    tmpResult2 = tmp(11273);
  }
...
export const createGuildInvite = function createGuildInvite(invite, isOwnInvite, theme) {
  ...
  const channel = tmp(10839)(invite).channel;
  ...
  if (tmp7) {
    if (flag2) {
      if (null != target_user) {
        if (tmp12) {
          ...
          obj6.name = tmp(4675).getFormattedName(target_user);
          const tmpResult8 = tmp(4675);
    ...
    const assetSource = Image.resolveAssetSource(tmp10(5328).getChannelIcon(channel));
    let uri1;
    if (assetSource != null) {
    ...
    uri = guildIconURL;
    tmp25 = name;
    const tmp10Result7 = tmp10(5328);
  } else {
    if (null != channel) {
      if (channel.type === constants2.GUILD_STAGE_VOICE) {
        const assetSource1 = Image.resolveAssetSource(tmp10(5328).getChannelIcon(channel));
        let uri2;
    ...
    if (null == approximate_member_count) {
      ...
      tmp25 = name;
      if (null != channel) {
        const assetSource2 = Image.resolveAssetSource(tmp10(5328).getChannelIcon(channel));
        let uri3;
  ...
  if (tmp7) {
    ...
    if (null != guild) {
      guildBadgeImageSource = tmp10(8195).getGuildBadgeImageSource(guild, theme);
      const tmp10Result10 = tmp10(8195);
    }
    const obj11 = {};
    const merged = Object.assign(baseColors);
    let formatted;
    ...
    if (null != guildBadgeImageSource) {
      assetUriForEmbed = tmp10(7383).getAssetUriForEmbed(guildBadgeImageSource);
      const tmp10Result12 = tmp10(7383);
    }
