...
// Dependencies: [8195, 1074, 1385, 11545, 4907, 8381, 1364, 8380, 1115, 8400, 8541, 5353, 8398, 2]
// Exports: default
  ...
  return found.map((attachment, index) => {
    ...
    if (isImageFileResult) {
      if (null != width) {
        ...
        const result1 = tmp(8380).sanitizeMediaDimension(num4);
        const tmpResult8 = tmp(8380);
        let num5 = 0;
        if (closure_1_1) {
          num5 = 0;
          if (tmp8) {
        ...
        const result2 = tmp(8380).sanitizeMediaDimension(num5);
        const tmpResult9 = tmp(8380);
        if (flags == null) {
          flags = 0;
        }
        let tmp28;
        ...
        if (tmp30) {
          tmp30 = null != tmp(8400).isSuspiciousDownload(localUri);
          const tmpResult11 = tmp(8400);
        }
        ...
        const merged = Object.assign(tmp(8398).getAttachmentObscurityProps(obj8));
