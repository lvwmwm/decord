...
// Dependencies: [8281, 1074, 1385, 11627, 4979, 8467, 1364, 8466, 1115, 8486, 8616, 5439, 8484, 2]
// Exports: default
  ...
  return found.map((attachment, index) => {
    ...
    if (isImageFileResult) {
      if (null != width) {
        ...
        const result1 = tmp(8466).sanitizeMediaDimension(num4);
        const tmpResult8 = tmp(8466);
        let num5 = 0;
        if (closure_1_1) {
          num5 = 0;
          if (tmp8) {
        ...
        const result2 = tmp(8466).sanitizeMediaDimension(num5);
        const tmpResult9 = tmp(8466);
        if (flags == null) {
          flags = 0;
        }
        let tmp28;
        ...
        if (tmp30) {
          tmp30 = null != tmp(8486).isSuspiciousDownload(localUri);
          const tmpResult11 = tmp(8486);
        }
        ...
        const merged = Object.assign(tmp(8484).getAttachmentObscurityProps(obj8));
