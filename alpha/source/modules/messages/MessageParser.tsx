...
// Function ID: 8006
// Name: MessageParser
// Dependencies: [5764, 5811, 2044, 4462, 2107, 2101, 2066, 4464, 4474, 4674, 1372, 1074, 5298, 1375, 4982, 1929, 5296, 8006, 8007, 2020, 4478, 5304, 4673, 4981, 12, 5310, 1115, 8008, 5747, 1370, 4482, 2]
// Exports: parseAndRebuild

// Module 8005 (MessageParser)
...
import MarkupTextRule2 from "MarkupTextRule" /* 5304 */;
import AutocompleteBoundaryUtils from "AutocompleteBoundaryUtils" /* 8006 */;
    ...
    (function handleEmoji(emojiContext, type, fn) {
      if (null != fn) {
        ...
        if ("emoticon" === type.type) {
          const result = obj(4478).translateSurrogatesToInlineEmoji(type.content);
...
function createParserState(getGuildId, arr) {
  ...
  if (null != guildId) {
    ...
    const found1 = tmp9(12)(guildId(5747).COMMAND_SUPPORTED_CHANNEL_TYPE_KEYS).filter((item) => item !== closure_1_7);
    const tmp9Result1Result = tmp9(12)(guildId(5747).COMMAND_SUPPORTED_CHANNEL_TYPE_KEYS);
...
function unparseWithMeta(content1, id, isNotification) {
  ...
  if (isNotification) {
    ...
    translateSurrogatesToInlineEmoji = translateSurrogatesToInlineEmoji(4478).translateSurrogatesToInlineEmoji;
  }
...
const GAME_MENTION_SENTINEL = fn(5298).GAME_MENTION_SENTINEL;
...
obj9.parse = function parse(arg0, arg1, channelId) {
  ...
  if (null == user) {
    ...
    if (isNotification) {
      ...
      if (nickname == null) {
        nickname = tmp13(4673).getGlobalName(user);
        const tmp13Result3 = tmp13(4673);
      }
      if (null != nickname) {
...
let t = t_mod;
obj13.match = t.anyScopeRegex(fn(5310).soundmojiRawFormatRegex);
...
export default {
  parse(getGuildId, content, arg2, arr) {
    ...
    closure_129_0 = tmp;
    closure_129_1 = obj(4478).translateInlineEmojiToSurrogates;
