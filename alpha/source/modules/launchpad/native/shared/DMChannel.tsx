...
// Dependencies: [19, 4939, 21, 4768, 11183, 4757, 576, 17145, 16694, 15600, 11, 5194, 17458, 5341, 17144, 17459, 10371, 8126, 4910, 2]

// Module 17466 (shared/DMChannel)
...
export default noop.memo(function DMChannel(navigationReplace) {
  ...
  const tmp = closure_6();
  const tmp4 = flag(17145)();
  const baseChannelUnreadBadgeState = channel(16694).useBaseChannelUnreadBadgeState(channel, muted);
  ({ unread, mentionCount } = baseChannelUnreadBadgeState);
  const tmp7 = flag(15600)(channel, { unread });
  ...
  const tmp2Result3 = flag(17458);
  obj4.unreadBadge = jsx(flag(17459), { unread, resolvedUnreadSetting: UnreadSetting.ALL_MESSAGES, muted });
  let tmp11Result = null != tmp7;
  if (tmp11Result) {
    const obj6 = { channel, message: tmp7, color: str, muted, layout: tmp5(8126).ChannelListLayoutTypes.COMPACT };
    tmp11Result = tmp11(tmp5(10371).ChannelRowPreview, obj6);
  }
  obj4.subtitle = tmp11Result;
  obj4.latestMessageTimestamp = extractTimestampResult;
  obj4.channelName = flag(4910)(channel);
  obj4.fontScale = fontScale;
  obj2.children = flag(17144)(obj4);
  return tmp2Result3(jsx(channel(5341).PressableHighlight, { style: null, underlayColor: tmp.pressableUnderlayColor.backgroundColor }));
});
