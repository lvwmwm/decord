...
// Dependencies: [19, 17, 11725, 21, 4757, 11726, 17442, 17444, 12290, 4493, 5186, 4620, 16350, 6899, 17445, 2]
// Exports: default

// Module 17441 (LaunchPadContainer)
...
export default function LaunchPadContainer(children) {
  updaters = undefined;
  const tmp = closure_8();
  const tmp4 = updaters(11726)();
  const tmp5 = updaters(17442)();
  const launchPadSharedState = tmp5.launchPadSharedState;
  ({ launchPadPullTabState, launchPadShown, gestureState, updaters } = tmp5);
  ({ gesture, gestureRef } = updaters(17444)({ launchPadType: tmp4, launchPadSharedState, launchPadPullTabState, launchPadShown, gestureState, updaters }));
  const tmp7 = updaters(12290)();
  dependencyMap = tmp7;
  const tmp2 = updaters;
  const tmp6 = updaters(17444)({ launchPadType: tmp4, launchPadSharedState, launchPadPullTabState, launchPadShown, gestureState, updaters });
  const fn = function o() {
  ...
  const animatedReaction = launchPadSharedState(4493).useAnimatedReaction(fn, fn2);
  let obj = launchPadSharedState(4493);
  ...
  let obj2 = launchPadSharedState(4493);
  fn3.__closure = { interpolate: launchPadSharedState(4493).interpolate, launchPadSharedState, withSpring: launchPadSharedState(5186).withSpring, windowDimensions: tmp7, LAUNCH_PAD_SPRING_CONFIG };
  fn3.__workletHash = 13886247172712;
  fn3.__initData = __initData3;
  const animatedStyle = obj2.useAnimatedStyle(fn3);
  const MobileHomeDrawerExperiment = launchPadSharedState(4620).MobileHomeDrawerExperiment;
  ...
  obj7.style = items;
  let items1 = [closure_6(updaters(4493).View, obj7), ];
  if (tmp4 !== constants.DISABLED) {
    ...
    const tmp10Result = tmp10(tmp2(17445), obj8);
  }
  items1[1] = tmp10Result;
  obj6.children = items1;
  obj5.children = closure_7(closure_3, obj6);
  obj4.children = closure_6(launchPadSharedState(6899).GestureDetector, obj5);
  return closure_6(updaters(16350).Provider, obj4);
};
