...
// Dependencies: [7865, 7866, 1115, 2484, 4414, 7867, 2]
...
export const getEmptyActivityFormatter = function getEmptyActivityFormatter() {
  ...
  obj.today = intl.string(_modDef2484.VjIAQQ);
  const intl2 = util.intl;
  obj.yesterday = intl2.string(_modDef2484["2a8xHY"]);
  obj.days = _modDef2484.Xt6oND;
  return obj;
};
export const getActivityWindowTimestampFormatter = function getActivityWindowTimestampFormatter(arg0) {
  ...
  if (arg0) {
    ...
    obj.yesterday = intl3.string(tmp3(2484).stOECr);
    obj.days = tmp3(2484).n8n5Ba;
    let tmp5 = obj;
  } else {
    obj.today = string(tmp4.g1ZX6m);
    const intl2 = tmp(1115).intl;
    obj.yesterday = intl2.string(tmp3(2484).s3qSVt);
    obj.days = tmp3(2484).f1UJiC;
    tmp5 = obj;
  }
  return tmp5;
};
export const formatUserActivityTimestamp = function formatUserActivityTimestamp(time, timestampFormatter, arg2) {
  const diffResult = _modDef4414().diff(_modDef4414(time), "s");
  const tmp3 = timestampFormatter();
  const obj = _modDef4414();
  _modDef4414(time).format("LL");
  if (diffResult < c10) {
...
export const formatLinkTimestamp = function formatLinkTimestamp(arg0, SENT_TIMESTAMP_FORMATTER) {
  const diffResult = _modDef4414().diff(_modDef4414(arg0), "s");
...
export const getTopUserOrGuildDescription = function getTopUserOrGuildDescription(dms_sent, call_count) {
  if (call_count > 0) {
    if (0 === dms_sent) {
      ...
      let formatToPlainStringResult = intl3.formatToPlainString(_modDef2484["L/Cj7S"], obj2);
    }
  ...
  if (dms_sent > 0) {
    if (0 === call_count) {
      ...
      formatToPlainStringResult = intl2.formatToPlainString(_modDef2484["6X1F0i"], obj3);
    }
  }
  const intl = util.intl;
  formatToPlainStringResult = intl.formatToPlainString(_modDef2484.IYqGMG, { messageCount: dms_sent, callCount: call_count });
};
