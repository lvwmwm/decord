...
// Dependencies: [19, 17, 1372, 7865, 7866, 1074, 21, 4827, 576, 1177, 38, 563, 11, 4823, 4671, 7920, 5887, 5893, 1115, 2484, 15238, 15241, 15242, 2]
// Exports: default

// Module 15237 (FamilyCenterActivityRow)
import _modDef38 from "module_38" /* 38 */;
import nativeDefault from "native" /* 576 */;
import FamilyCenterUtils from "FamilyCenterUtils" /* 7920 */;
import FamilyCenterActivityPurchaseRowDefault from "FamilyCenterActivityPurchaseRow" /* 15238 */;
import FamilyCenterActivityGiftRowDefault from "FamilyCenterActivityGiftRow" /* 15242 */;
...
const ACTION_TO_TEXT = fn(7866).ACTION_TO_TEXT;
...
const memoResult = noop.memo((action) => {
  ...
  if (null == stateFromStores) {
    ...
    obj6.children = tmp3(4671).getName(stateFromStores);
    const items2 = [closure_8(tmp6(4823).Text, obj6), ];
    const obj7 = { variant: "text-xs/medium", color: "channels-default", children: null };
    const tmp3Result2 = tmp3(4671);
    const _Date = Date;
    const date = new Date(extractTimestampResult);
    obj7.children = tmp6(7920).formatUserActivityTimestamp(date.getTime(), value.timestampFormatter);
    items2[1] = closure_8(tmp6(4823).Text, obj7);
...
const memoResult1 = noop.memo((action) => {
  ...
  if (undefined === stateFromStores) {
    ...
    obj4.size = tmp2(5887).GuildIconSizes.NORMAL;
    const items1 = [closure_8(tmp6(5887), obj4), ];
    ...
    if (hasItem) {
      const obj8 = { style: tmp.badge, guild: stateFromStores, size: tmp6(5893).Sizes.SMALL, disableColor: true };
      tmp11Result = tmp11(tmp6(5893), obj8);
      const tmp6Result2 = tmp6(5893);
    }
    ...
    items2[1] = closure_8(tmp2(4823).Text, obj9);
    ...
    if (undefined !== stateFromStores.approximateMemberCount) {
      ...
      obj10.children = intl.format(tmp6(2484)["5JmNgg"], obj19);
      tmp11Result2 = tmp11(tmp2(4823).Text, obj10);
    }
  ...
  if (tmpResult9.isPurchase(action)) {
    ...
    if (tmpResult10.isGift(action)) {
      ...
      if (null == giftInfo) {
        ...
        const giftRowDisplayInfo = tmp(15241).getGiftRowDisplayInfo(giftInfo);
