...
// Dependencies: [19, 17, 1372, 7782, 7783, 1074, 21, 4757, 576, 1177, 38, 563, 11, 4753, 4601, 7837, 5803, 5809, 1115, 2482, 15186, 15189, 15190, 2]
// Exports: default

// Module 15185 (FamilyCenterActivityRow)
import _modDef38 from "module_38" /* 38 */;
import nativeDefault from "native" /* 576 */;
import FamilyCenterUtils from "FamilyCenterUtils" /* 7837 */;
import FamilyCenterActivityPurchaseRowDefault from "FamilyCenterActivityPurchaseRow" /* 15186 */;
import FamilyCenterActivityGiftRowDefault from "FamilyCenterActivityGiftRow" /* 15190 */;
...
const ACTION_TO_TEXT = fn(7783).ACTION_TO_TEXT;
...
const memoResult = noop.memo((action) => {
  ...
  if (null == stateFromStores) {
    ...
    obj6.children = tmp3(4601).getName(stateFromStores);
    const items2 = [closure_8(tmp6(4753).Text, obj6), ];
    const obj7 = { variant: "text-xs/medium", color: "channels-default", children: null };
    const tmp3Result2 = tmp3(4601);
    const _Date = Date;
    const date = new Date(extractTimestampResult);
    obj7.children = tmp6(7837).formatUserActivityTimestamp(date.getTime(), value.timestampFormatter);
    items2[1] = closure_8(tmp6(4753).Text, obj7);
...
const memoResult1 = noop.memo((action) => {
  ...
  if (undefined === stateFromStores) {
    ...
    obj4.size = tmp2(5803).GuildIconSizes.NORMAL;
    const items1 = [closure_8(tmp6(5803), obj4), ];
    ...
    if (hasItem) {
      const obj8 = { style: tmp.badge, guild: stateFromStores, size: tmp6(5809).Sizes.SMALL, disableColor: true };
      tmp11Result = tmp11(tmp6(5809), obj8);
      const tmp6Result2 = tmp6(5809);
    }
    ...
    items2[1] = closure_8(tmp2(4753).Text, obj9);
    ...
    if (undefined !== stateFromStores.approximateMemberCount) {
      ...
      tmp11Result2 = tmp11(tmp2(4753).Text, obj10);
    }
  ...
  if (tmpResult9.isPurchase(action)) {
    ...
    if (tmpResult10.isGift(action)) {
      ...
      if (null == giftInfo) {
        ...
        const giftRowDisplayInfo = tmp(15189).getGiftRowDisplayInfo(giftInfo);
