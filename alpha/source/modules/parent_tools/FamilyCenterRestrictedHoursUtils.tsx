...
function formatDays(days) {
  ...
  if (setsEqual(set, set2)) {
    ...
    return intl3.string(_modDef2487.bPjqd1);
  } else if (tmp2(set, set)) {
    const intl2 = util.intl;
    return intl2.string(_modDef2487["4dr9L9"]);
  } else if (tmp2(set, set1)) {
    const intl = util.intl;
    return intl.string(_modDef2487["6lTTJ+"]);
  } else {
    const _Intl = Intl;
  ...
  if (null != startTime.startTime) {
    if (null != startTime.endTime) {
      ...
      if (60 * startTime2.hours + startTime2.minutes > 60 * endTime2.hours + endTime2.minutes) {
        let OxveI8 = _modDef2487.OxveI8;
      } else {
        OxveI8 = _modDef2487["ERTn+E"];
      }
