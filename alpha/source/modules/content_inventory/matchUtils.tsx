...
// Function ID: 8684
// Name: matchUtils
// Dependencies: [1074, 8684, 8686, 8687, 8689, 8690, 8490, 2]
...
function isMatchingListeningActivity(extra, party) {
  if (obj.isListenedSessionEntry(extra)) {
    ...
    let isTopArtistEntryResult = provider === tmp(8689).ContentInventoryListenedMediaProvider.SPOTIFY;
  } else {
    isTopArtistEntryResult = tmp(8687).isTopArtistEntry(extra);
    if (isTopArtistEntryResult) {
      isTopArtistEntryResult = extra.extra.media.provider === tmp(8689).ContentInventoryListenedMediaProvider.SPOTIFY;
    }
    const tmpResult = tmp(8687);
  }
