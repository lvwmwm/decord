...
// Function ID: 8688
// Name: matchUtils
// Dependencies: [1074, 8688, 8690, 8691, 8693, 8694, 8494, 2]
...
function isMatchingListeningActivity(extra, party) {
  if (obj.isListenedSessionEntry(extra)) {
    ...
    let isTopArtistEntryResult = provider === tmp(8693).ContentInventoryListenedMediaProvider.SPOTIFY;
  } else {
    isTopArtistEntryResult = tmp(8691).isTopArtistEntry(extra);
    if (isTopArtistEntryResult) {
      isTopArtistEntryResult = extra.extra.media.provider === tmp(8693).ContentInventoryListenedMediaProvider.SPOTIFY;
    }
    const tmpResult = tmp(8691);
  }
