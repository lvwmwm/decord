...
// Dependencies: [32, 19, 8203, 4397, 2042, 2105, 5632, 1372, 8386, 21, 1978, 4988, 4986, 504, 8239, 7513, 8388, 8389, 573, 5204, 4981, 2]
...
function getActionComponentState(interaction, id, shouldDisableInteractiveComponents) {
  ...
  if (tmp3) {
    tmp3 = interaction.state !== tmp(4986).InteractionState.FAILED;
  }
  ...
  if (!tmp3) {
    ...
    if (flag) {
      DISABLED = tmp(4988).ActionComponentState.DISABLED;
    }
    return DISABLED;
  } else {
    if (interaction.data.interactionType !== tmp(1978).InteractionTypes.MESSAGE_COMPONENT) {
      if (isInteractionComponent(id)) {
        LOADING = tmp(4988).ActionComponentState.DISABLED;
      }
    }
    LOADING = tmp(4988).ActionComponentState.LOADING;
  }
}
