  ...
  if (null != avatars) {
    mapped = substr.map((source, index) => {
      ...
      if (index !== diff) {
        ...
        const point = { shape: tmp3(8276).CutoutShape.Circle, x: tmp3(1177).AVATAR_SIZE_MAP[tmp3(undefined, 1177).AvatarSizes.XSMALL_20] - 4 - 2, y: -2, size: tmp3(1177).AVATAR_SIZE_MAP[tmp3(undefined, 1177).AvatarSizes.XSMALL_20] + 4 };
        const items = [point];
        obj3.cutouts = items;
        obj3.children = tmp5;
