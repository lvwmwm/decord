...
// Dependencies: [1227, 1182, 1185, 1074, 1229, 1374, 1230, 1186, 14684, 11414, 8651, 1228, 4679, 14685, 1241, 2]
...
export const handleSaveTheme = function handleSaveTheme(mobileThemes, analyticsLocations, isSynced) {
  if (mobileThemes.type === ClientThemesTypes.ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
    ...
    if ("system" === mobileThemes.theme) {
      const result = tmp(14684).resetBackgroundGradientPreset();
      const tmpResult = tmp(14684);
      tmp(11414).resetCustomTheme();
      const tmpResult13 = tmp(11414);
      const obj3 = { theme: mobileThemes.theme };
      return tmp(8651).saveClientTheme(obj3);
      ...
      const result1 = tmp(14684).resetBackgroundGradientPreset();
      const tmpResult16 = tmp(14684);
      tmp(11414).updateCustomTheme(mobileThemes.customThemeSettings, customThemeBaseTheme);
      const tmpResult17 = tmp(11414);
      const obj4 = { customUserThemeSettings: mobileThemes.customThemeSettings, theme: customThemeBaseTheme };
      return tmp(8651).saveClientTheme(obj4);
    } else {
      if (mobileThemes.type === tmp(1230).ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
        const result2 = tmp(14684).updateBackgroundGradientPreset(mobileThemes.id);
        const tmpResult19 = tmp(14684);
        tmp(11414).resetCustomTheme();
        const tmpResult20 = tmp(11414);
        ({ id: obj10.backgroundGradientPresetId, theme: obj10.theme } = mobileThemes);
        let saveClientThemeResult = tmp(8651).saveClientTheme({ backgroundGradientPresetId: null, theme: null });
        const obj5 = { backgroundGradientPresetId: null, theme: null };
        const tmpResult21 = tmp(8651);
      } else {
        const result3 = tmp(14684).resetBackgroundGradientPreset();
        const tmpResult22 = tmp(14684);
        tmp(11414).resetCustomTheme();
        const tmpResult23 = tmp(11414);
        const obj6 = { theme: mobileThemes.theme };
        saveClientThemeResult = tmp(8651).saveClientTheme(obj6);
        const tmpResult24 = tmp(8651);
      }
...
export const handleSaveSyncedModeTheme = function handleSaveSyncedModeTheme(mobileThemes, systemTheme, analyticsLocations) {
  if ("system" !== mobileThemes.theme) {
    if (mobileThemes.type === ClientThemesTypes.ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
      ...
      const result = tmp14(4679).updateThemePreferences(obj3);
      if (mobileThemes.type === tmp14(1230).ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
        const obj4 = { customUserThemeSettings: mobileThemes.customThemeSettings };
        const result1 = tmp14(4679).updateSyncedClientTheme(systemTheme, obj4);
        const tmp14Result6 = tmp14(4679);
        ...
        const result2 = tmp14(4679).updateSyncedClientTheme(systemTheme, obj5);
        const tmp14Result7 = tmp14(4679);
      } else {
        const obj6 = { theme: mobileThemes.theme };
        const result3 = tmp14(4679).updateSyncedClientTheme(systemTheme, obj6);
        const tmp14Result8 = tmp14(4679);
      }
      const tmp14Result5 = tmp14(4679);
    } else if (mobileThemes.type === tmp14(1230).ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
