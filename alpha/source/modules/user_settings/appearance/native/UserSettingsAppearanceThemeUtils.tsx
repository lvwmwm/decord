...
// Dependencies: [1227, 1182, 1185, 1074, 1229, 1374, 1230, 1186, 15514, 12288, 9553, 1228, 4677, 15515, 1241, 2]
...
export const handleSaveTheme = function handleSaveTheme(mobileThemes, analyticsLocations, isSynced) {
  if (mobileThemes.type === ClientThemesTypes.ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
    ...
    if ("system" === mobileThemes.theme) {
      const result = tmp(15514).resetBackgroundGradientPreset();
      const tmpResult = tmp(15514);
      tmp(12288).resetCustomTheme();
      const tmpResult13 = tmp(12288);
      const obj3 = { theme: mobileThemes.theme };
      return tmp(9553).saveClientTheme(obj3);
      ...
      const result1 = tmp(15514).resetBackgroundGradientPreset();
      const tmpResult16 = tmp(15514);
      tmp(12288).updateCustomTheme(mobileThemes.customThemeSettings, customThemeBaseTheme);
      const tmpResult17 = tmp(12288);
      const obj4 = { customUserThemeSettings: mobileThemes.customThemeSettings, theme: customThemeBaseTheme };
      return tmp(9553).saveClientTheme(obj4);
    } else {
      if (mobileThemes.type === tmp(1230).ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
        const result2 = tmp(15514).updateBackgroundGradientPreset(mobileThemes.id);
        const tmpResult19 = tmp(15514);
        tmp(12288).resetCustomTheme();
        const tmpResult20 = tmp(12288);
        ({ id: obj10.backgroundGradientPresetId, theme: obj10.theme } = mobileThemes);
        let saveClientThemeResult = tmp(9553).saveClientTheme({ backgroundGradientPresetId: null, theme: null });
        const obj5 = { backgroundGradientPresetId: null, theme: null };
        const tmpResult21 = tmp(9553);
      } else {
        const result3 = tmp(15514).resetBackgroundGradientPreset();
        const tmpResult22 = tmp(15514);
        tmp(12288).resetCustomTheme();
        const tmpResult23 = tmp(12288);
        const obj6 = { theme: mobileThemes.theme };
        saveClientThemeResult = tmp(9553).saveClientTheme(obj6);
        const tmpResult24 = tmp(9553);
      }
...
export const handleSaveSyncedModeTheme = function handleSaveSyncedModeTheme(mobileThemes, systemTheme, analyticsLocations) {
  if ("system" !== mobileThemes.theme) {
    if (mobileThemes.type === ClientThemesTypes.ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
      ...
      const result = tmp14(4677).updateThemePreferences(obj3);
      if (mobileThemes.type === tmp14(1230).ClientThemeType.CUSTOM_BACKGROUND_GRADIENT) {
        const obj4 = { customUserThemeSettings: mobileThemes.customThemeSettings };
        const result1 = tmp14(4677).updateSyncedClientTheme(systemTheme, obj4);
        const tmp14Result6 = tmp14(4677);
        ...
        const result2 = tmp14(4677).updateSyncedClientTheme(systemTheme, obj5);
        const tmp14Result7 = tmp14(4677);
      } else {
        const obj6 = { theme: mobileThemes.theme };
        const result3 = tmp14(4677).updateSyncedClientTheme(systemTheme, obj6);
        const tmp14Result8 = tmp14(4677);
      }
      const tmp14Result5 = tmp14(4677);
    } else if (mobileThemes.type === tmp14(1230).ClientThemeType.BACKGROUND_GRADIENT_PRESET) {
