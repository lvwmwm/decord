...
// Dependencies: [32, 19, 17, 1372, 4486, 4487, 7722, 1979, 7568, 1074, 1374, 21, 4827, 7742, 1612, 7493, 7513, 7325, 5288, 1241, 504, 1484, 2059, 7743, 7745, 573, 7747, 7728, 9558, 7775, 11048, 7774, 13711, 8407, 7321, 7326, 4481, 4496, 13712, 13752, 13754, 13755, 9553, 13822, 11778, 1364, 2]
// Exports: default

// Module 7741 (UserSettingsPremium)
...
import useStoreConnectionErrorAlertDefault from "useStoreConnectionErrorAlert" /* 7742 */;
...
export default function UserSettingsPremium(applicationId) {
  ...
  if (isFromTextSection) {
    let items = [tmp2(7513).TEXT_AND_IMAGES];
    let items1 = items;
  } else {
    items1 = [];
  }
  const items2 = [...items1, tmp2(7513).PREMIUM_MARKETING];
  analyticsLocations = useAnalyticsLocationsDefault(items2).analyticsLocations;
  navigation = applicationId(7325).useSettingNavigationRoute();
  ...
  const isPaymentsBlocked = applicationId(7745).useIsPaymentsBlocked();
  ...
  const obj11 = applicationId(7745);
  premiumTrialOffer = applicationId(7775).usePremiumTrialOffer();
  const obj12 = applicationId(7775);
  premiumDiscountOffer = applicationId(11048).usePremiumDiscountOffer();
  const obj13 = applicationId(11048);
  const premiumTrialOfferPremiumType = applicationId(7774).usePremiumTrialOfferPremiumType();
  ...
  const obj14 = applicationId(7774);
  const hasTier2Premium = applicationId(4481).useHasTier2Premium();
  ...
  if (tmp28) {
    ...
    const obj16 = { style: tmp.container, children: tmp30(tmp2(11778), {}) };
    ...
    if (hasTier2Premium) {
      if (null == premiumFeatureCardOrder) {
        const obj20 = { onClose };
        let tmp30Result4 = tmp30(tmp2(13755), obj20);
      }
    ...
    if (null != premiumFeatureCardOrder) {
      ...
      if (stateFromStores3) {
        ...
        stateFromStores3 = tmp30(tmp2(13754), obj24);
      }
      obj21.accountCredit = stateFromStores3;
      obj21.onClose = onClose;
      if (TIER_2_LEADING == null) {
        TIER_2_LEADING = tmp6(9553).PremiumFeatureCardOrder.TIER_2_LEADING;
      }
      ...
      tmp30Result4 = tmp30(tmp2(13822), obj21);
      const tmp2Result = tmp2(13822);
    } else if (premiumTrialOfferPremiumType === TIER_0.TIER_0) {
      premiumFeatureCardOrder = tmp6(9553).PremiumFeatureCardOrder.TIER_0_LEADING;
    } else if (premiumTrialOfferPremiumType === tmp33.TIER_2) {
      premiumFeatureCardOrder = tmp6(9553).PremiumFeatureCardOrder.TIER_2_LEADING;
    }
    tmp31 = state;
