...
// Dependencies: [19, 17, 5637, 2046, 2042, 4394, 4781, 10380, 2049, 7779, 21, 16538, 16540, 16544, 576, 7319, 7773, 16455, 1115, 12553, 16533, 16545, 16480, 5192, 16547, 16549, 16551, 16555, 16557, 16558, 16559, 16560, 16561, 16564, 16566, 16567, 16570, 16463, 1095, 16574, 16575, 16585, 16587, 16589, 2067, 16473, 5644, 16471, 16590, 2]
  ...
  if (guildChannels.favoritesSectionNumber === section) {
    ...
    if (obj.isNamedCategorySection(section)) {
      ...
      if (null != namedCategoryFromSection) {
        ...
        tmp5 = collapsedCategories(tmp(16455).CategoryChannel, obj8);
      }
      return tmp5;
    } else {
      return null;
...
export const renderChannelListSectionFooter = function renderChannelListSectionFooter(guildChannels, section, ref, channels) {
  ...
  if (ChannelListState.SECTION_INDEX_CHANNEL_NOTICES === section) {
    return null;
  } else if (tmp5(7773).SECTION_INDEX_GUILD_ACTIONS === section) {
    const guildActionSection = guildChannels.getGuildActionSection();
    let flag = false;
...
export const getChannelListSectionFooterSize = function getChannelListSectionFooterSize(guildChannels, section, result) {
  ...
  if (ChannelListState.SECTION_INDEX_CHANNEL_NOTICES === section) {
    return 0;
  } else if (tmp3(7773).SECTION_INDEX_GUILD_ACTIONS === section) {
    const guildActionSection = guildChannels.getGuildActionSection();
    let flag = false;
    if (!guildActionSection.isEmpty()) {
      const rows = guildActionSection.getRows();
    ...
    let num5 = 0;
    if (flag) {
      num5 = tmp3(12553).DIVIDER_HEIGHT;
    }
    return num5;
  } else {
    if (guildChannels.favoritesSectionNumber !== section) {
      if (guildChannels.recentsSectionNumber !== section) {
        if (guildChannels.voiceChannelsSectionNumber === section) {
          ...
          if (!obj.render) {
            ...
            const sum = num + tmp3(5192).SMALL_BUTTON_HEIGHT;
            if (tmp6) {
    ...
    return tmp3(12553).DIVIDER_HEIGHT;
  }
};
...
export const getChannelListItemSize = function getChannelListItemSize(liveChannelNoticeHeight) {
  ...
  if (ChannelListState.SECTION_INDEX_CHANNEL_NOTICES === section) {
    ...
    if (constants.SPACER === row1) {
      ...
      return tmp(16547).getScaledGuildProgressButtonHeight(fontScale);
    } else if (tmp28.MFA_WARNING === row1) {
      return tmp(16549).getScaledGuildMFAWarningHeight(fontScale);
      ...
      return tmp(16540).getScaledGameClaimNoticeHeight(fontScale);
    } else if (tmp28.APPLICATION_ACCOUNT_LINK === row1) {
      return tmp(16544).getScaledAccountLinkBannerHeight(fontScale);
                ...
                if (tmp26.GUILD_BOOSTS !== row2) {
                  ...
                  if (tmp26.GUILD_VIBEGRATIONS !== row2) {
                    if (tmp26.GUILD_NEW_MEMBER_ACTIONS_PROGRESS_BAR === row2) {
                      num4 = 48 + tmp(12553).DIVIDER_HEIGHT;
                    } else {
                      ...
                      if (tmp26.GUILD_HUB_HEADER_OPTIONS !== row2) {
                        ...
                        if (tmp26.GUILD_SCHEDULED_EVENTS !== row2) {
                          if (tmp26.GUILD_PREMIUM_PROGRESS_BAR === row2) {
                            num4 = tmp(16570).BOOST_PROGRESS_BAR_HEIGHT + tmp(12553).DIVIDER_HEIGHT;
                          } else {
                            num4 = 0;
    ...
    if (null != channelFromSectionRow) {
      ...
      if (type1 === type) {
              ...
              if (tmp(1095).ChannelTypes.GROUP_DM !== type) {
                if (tmp(1095).ChannelTypes.PUBLIC_THREAD !== type) {
                  if (tmp(1095).ChannelTypes.PRIVATE_THREAD !== type) {
                    ...
                    tmpResult11 = tmp(2067);
                  }
                }
                const result = SortedVoiceStateStore.countVoiceStatesForChannel(record.id);
                let sum1 = tmp31;
                if (result > 0) {
                  sum1 = tmp31 + result * tmp(16473).getVoiceUserHeight(fontScale);
                  const tmpResult12 = tmp(16473);
                }
                num = sum1;
              }
            }
        ...
        const voiceUserHeight = tmp(16473).getVoiceUserHeight(fontScale);
        ...
        if (record.type === tmp(1095).ChannelTypes.GUILD_STAGE_VOICE) {
          ...
          if (StageChannelParticipantStore.getParticipantCount(record.id, tmp(5644).StageChannelParticipantNamedIndex.AUDIENCE) > 0) {
            num3 = tmp(16471).getAudienceItemHeight(fontScale);
            const tmpResult14 = tmp(16471);
          }
        }
        let sum4 = tmp31;
...
export const calculateVoiceSummary = function calculateVoiceSummary(arg0) {
  ...
  if (!obj.isVoiceChannelsSection(section, guildChannels)) {
    if (section < tmp(7773).SECTION_INDEX_FIRST_NAMED_CATEGORY) {
      return null;
    }
  }
  obj = channel_list_v2_ChannelListUtils;
