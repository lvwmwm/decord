...
// Dependencies: [19, 17, 1074, 21, 4559, 4827, 576, 16506, 2067, 1115, 16538, 14265, 16536, 5913, 4524, 10578, 4823, 9096, 1177, 7540, 16552, 12631, 16557, 16558, 2]
// Exports: default

// Module 16537 (ChannelListStickyHeader)
import nativeDefault from "native" /* 576 */;
import GuildBadgeV2Default from "GuildBadgeV2" /* 9096 */;
...
export default function ChannelListStickyHeader(guild) {
  ...
  const isFavoritesGuildIdResult = guild(2067).isFavoritesGuildId(guild.id);
  ...
  const obj2 = guild(2067);
  const tmp8 = isFavoritesGuildIdResult ? t.hW8QDk : t["Gpyp/e"];
  const iOSPressEffects = guild(5913).useIOSPressEffects(4);
  ({ onPressIn, onPressOut, pressableStyles } = iOSPressEffects);
  const tmp6Result = guild(5913);
  const favoritesAwareGuildName = guild(2067).getFavoritesAwareGuildName(guild);
  const tmp6Result4 = guild(2067);
  const token = guild(4524).useToken(tmp2(576).modules.mobile.CHANNEL_LIST_TITLE_TEXT_STYLE);
  const tmp6Result5 = guild(4524);
  const token1 = guild(4524).useToken(tmp2(576).modules.mobile.CHANNEL_LIST_SUBTITLE_TEXT_STYLE);
  ...
  if (isFavoritesGuildIdResult) {
    ...
    tmp22 = closure_6(tmp6(10578).StarIcon, obj5);
  }
  const items2 = [tmp22, , , ];
  const tmp16 = closure_8;
  const tmp6Result6 = guild(4524);
  items2[1] = closure_6(closure_4, { ref, collapsable: false, style: tmp.flex, children: closure_6(guild(4823).Text, { experimental_useNativeText: true, color: "mobile-text-heading-primary", variant: token, lineClamp: 1, children: favoritesAwareGuildName }) });
  const obj7 = { guild, size: null, style: null };
  const obj6 = { ref, collapsable: false, style: tmp.flex, children: closure_6(guild(4823).Text, { experimental_useNativeText: true, color: "mobile-text-heading-primary", variant: token, lineClamp: 1, children: favoritesAwareGuildName }) };
  ...
  if (flag2) {
    ...
    tmp24Result = tmp24(tmp6(7540).ChevronSmallRightIcon, obj8);
  }
  ...
  if (null != tmp10) {
    ...
    if (tmp10 > 0) {
      ...
      const items4 = [tmp24(tmp6(4823).Text, obj10), , ];
      ...
      items4[2] = tmp24(tmp6(4823).Text, obj13);
  ...
  if (isFavoritesGuildIdResult) {
    tmp24Result5 = tmp24(tmp6(16552).FavoritesGuildHeaderActionButton, {});
  }
  ...
  if (flag) {
    ...
    tmp24Result6 = tmp24(tmp2(12631), obj16);
  }
  ...
  if (tmp24Result7) {
    ...
    obj17.children = tmp24(tmp2(16557), obj18);
  ...
  if (flag3) {
    ...
    tmp24Result8 = tmp24(tmp2(16558), obj20);
  }
