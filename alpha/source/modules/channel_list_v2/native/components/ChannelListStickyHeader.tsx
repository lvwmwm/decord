...
// Dependencies: [19, 17, 1074, 21, 4493, 4757, 576, 16454, 2067, 1115, 16486, 14183, 16484, 5829, 4458, 10499, 4753, 9025, 1177, 7456, 16500, 12555, 16505, 16506, 2]
// Exports: default

// Module 16485 (ChannelListStickyHeader)
import nativeDefault from "native" /* 576 */;
import GuildBadgeV2Default from "GuildBadgeV2" /* 9025 */;
...
export default function ChannelListStickyHeader(guild) {
  ...
  const iOSPressEffects = guild(5829).useIOSPressEffects(4);
  ({ onPressIn, onPressOut, pressableStyles } = iOSPressEffects);
  const tmp6Result = guild(5829);
  const favoritesAwareGuildName = guild(2067).getFavoritesAwareGuildName(guild);
  const tmp6Result4 = guild(2067);
  const token = guild(4458).useToken(tmp2(576).modules.mobile.CHANNEL_LIST_TITLE_TEXT_STYLE);
  const tmp6Result5 = guild(4458);
  const token1 = guild(4458).useToken(tmp2(576).modules.mobile.CHANNEL_LIST_SUBTITLE_TEXT_STYLE);
  ...
  if (isFavoritesGuildIdResult) {
    ...
    tmp22 = closure_6(tmp6(10499).StarIcon, obj5);
  }
  const items2 = [tmp22, , , ];
  const tmp16 = closure_8;
  const tmp6Result6 = guild(4458);
  items2[1] = closure_6(closure_4, { ref, collapsable: false, style: tmp.flex, children: closure_6(guild(4753).Text, { experimental_useNativeText: true, color: "mobile-text-heading-primary", variant: token, lineClamp: 1, children: favoritesAwareGuildName }) });
  const obj7 = { guild, size: null, style: null };
  const obj6 = { ref, collapsable: false, style: tmp.flex, children: closure_6(guild(4753).Text, { experimental_useNativeText: true, color: "mobile-text-heading-primary", variant: token, lineClamp: 1, children: favoritesAwareGuildName }) };
  ...
  if (null != tmp10) {
    ...
    if (tmp10 > 0) {
      ...
      const items4 = [tmp24(tmp6(4753).Text, obj10), , ];
      ...
      items4[2] = tmp24(tmp6(4753).Text, obj13);
  ...
  if (isFavoritesGuildIdResult) {
    tmp24Result5 = tmp24(tmp6(16500).FavoritesGuildHeaderActionButton, {});
  }
  ...
  if (flag) {
    ...
    tmp24Result6 = tmp24(tmp2(12555), obj16);
  }
  ...
  if (tmp24Result7) {
    ...
    obj17.children = tmp24(tmp2(16505), obj18);
  ...
  if (flag3) {
    ...
    tmp24Result8 = tmp24(tmp2(16506), obj20);
  }
