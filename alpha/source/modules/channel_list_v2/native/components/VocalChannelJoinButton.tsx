...
// Dependencies: [19, 17, 4395, 4775, 1074, 21, 4756, 576, 1364, 8117, 4690, 4607, 9644, 563, 5648, 5642, 5634, 12548, 4752, 1115, 5186, 5315, 5319, 2]

// Module 12547 (VocalChannelJoinButton)
...
export default noop.memo(function VocalChannelJoinButton(channel) {
  ...
  const obj4 = channel(5648);
  const obj5 = channel(5634);
  const tmp8 = channel(5634).useStageHasMedia(channel.id) && isGuildStageVoiceResult;
  ...
  const connectedUserLimit = channel(12548).useConnectedUserLimit({ channel, video: tmp12 });
  const tmp3Result3 = channel(12548);
  let connectedUserLimitFormatted = channel(12548).useConnectedUserLimitFormatted({ channel, video: tmp12, userCount: sum });
    ...
    if (!noIcon) {
      if (!isConnectedToVoiceChannel) {
        ...
        if (isGuildStageVoiceResult) {
          let VoiceNormalIcon = tmp3(5315).StageIcon;
        } else {
          VoiceNormalIcon = tmp3(5319).VoiceNormalIcon;
        }
