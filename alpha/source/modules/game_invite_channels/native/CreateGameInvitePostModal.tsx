...
// Dependencies: [32, 19, 17, 2042, 21, 4827, 576, 7493, 504, 5933, 13109, 7312, 13111, 4838, 4794, 11594, 1980, 1115, 5425, 5983, 4823, 3648, 7416, 7600, 5990, 5908, 7531, 5271, 2]
// Exports: default

// Module 13110 (CreateGameInvitePostModal)
...
function CreateGameInvitePostContent(parentChannel) {
  ...
  const insets = tags(7312)({ includeKeyboardHeight: true }).insets;
  ...
  const createGameInvitePost = parentChannel(13111).useCreateGameInvitePost({
    parentChannel,
    description: tmp5,
    appliedTagIds: memo,
    upload: NOOP_UPLOAD,
  ...
  obj5.children = closure_7(parentChannel(5983).XSmallIcon, {});
  const items4 = [closure_7(parentChannel(5425).PressableOpacity, obj5), ];
  const obj6 = { variant: "text-md/semibold", color: "mobile-text-heading-primary", children: null };
  const intl2 = parentChannel(1115).intl;
  obj6.children = intl2.string(tags(3648).tOsHsu);
  items4[1] = closure_7(parentChannel(4823).Text, obj6);
  ...
  obj8.label = intl3.string(tags(3648)["/mEbGf"]);
  const intl4 = parentChannel(1115).intl;
  obj8.placeholder = intl4.string(tags(3648)["SU/IAE"]);
  obj8.value = tmp5;
  obj8.onChange = tmp6;
  obj8.maxLength = parentChannel(7600).GAME_INVITE_POST_MESSAGE_MAX_LENGTH;
  const items6 = [closure_7(parentChannel(7416).TextArea, obj8), ];
  ...
  if (tmp15Result) {
    ...
    obj10.children = tmp15(tmp9(4823).Text, obj11);
    obj9.trailing = tmp15(tmp14, obj10);
    obj9.onPress = callback1;
    tmp15Result = tmp15(tmp9(5908).TableRow, obj9);
  }
  ...
  obj13.label = intl6.string(tags(3648).Xd2NFi);
  const intl7 = tmp9(1115).intl;
  obj13.subLabel = intl7.string(tags(3648).G91SYQ);
  obj13.value = voiceChatEnabled;
  obj13.onValueChange = callback;
  obj13.disabled = voiceToggleDisabled;
  items7[1] = closure_7(parentChannel(7531).TableSwitchRow, obj13);
  obj12.children = items7;
  items6[1] = closure_8(parentChannel(5990).TableRowGroup, obj12);
  ...
  obj14.children = closure_7(parentChannel(5271).Button, obj15);
...
export default function CreateGameInvitePostModal(parentChannelId) {
  ...
  parentChannelId(5933).useNavigatorBackPressHandler(() => {
  ...
  if (null != stateFromStores) {
    ...
    if (stateFromStores.isGameInvitesChannel()) {
      ...
      tmp4 = closure_7(tmp2(7493).AnalyticsLocationProvider, obj2);
    }
  }
  return tmp4;
};
