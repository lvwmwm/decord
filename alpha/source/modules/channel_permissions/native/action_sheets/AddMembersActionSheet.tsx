...
// Dependencies: [5, 109, 32, 19, 17, 2105, 2099, 2064, 1372, 8676, 1085, 21, 4757, 576, 4401, 7228, 4744, 504, 9829, 4468, 1115, 1177, 4753, 9845, 6871, 9002, 9849, 5738, 9854, 4910, 4902, 9830, 4454, 4724, 7397, 7396, 5187, 2]
// Exports: default

// Module 9844 (AddMembersActionSheet)
...
export default function AddMembersActionSheet(channel) {
  ...
  closure_2 = async function _handleAddPressed() {
    ...
    const item = values.forEach((row) => {
      ...
      if (tmp) {
        if (row.rowType === constants.ROLE) {
          ...
          closure_1_0.push(channel(4902).permissionOverwriteForRole(row.id, closure_0.type));
          const obj = channel(4902);
        } else if (row.rowType === tmp2.MEMBER) {
          closure_1 = closure_1 + 1;
          closure_1_0.push(channel(4902).permissionOverwriteForUser(row.id, closure_0.type));
          const obj2 = channel(4902);
        }
      }
    });
    ...
    if (1 === tmp7) {
      ...
      const result = channel(4454).memberOrRoleAddedToast(closure_128_2, closure_128_1);
      channel(4454);
      tmp3(4724).hideActionSheet();
      dependencyMap = 0;
      tmp3(4724);
    }
    return arg1;
