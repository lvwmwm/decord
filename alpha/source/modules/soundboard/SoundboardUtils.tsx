...
// Dependencies: [5, 1220, 2048, 4466, 1372, 5312, 5314, 1074, 1218, 2020, 4485, 6758, 6751, 6759, 573, 6786, 6787, 6788, 563, 4675, 2028, 2025, 5321, 1241, 5009, 2]
...
import VoiceChannelEffectsActionCreators from "VoiceChannelEffectsActionCreators" /* 6759 */;
import getCurrentVoiceChannelDefault from "getCurrentVoiceChannel" /* 6786 */;
              ...
              if (null != customJoinSound) {
                if (tmp50Result.canUseCustomCallSounds(currentUser)) {
                  if (tmp53Result.canSelectedVoiceChannelUseSoundboard()) {
                    c3 = 1;
                    c4 = 1;
                    const obj4 = { value: tmp53(6751).maybeFetchSoundboardSounds(), done: false };
                    return obj4;
                  }
                  tmp53Result = tmp53(6788);
                }
                tmp50Result = tmp50(4485);
              }
            }
          }
...
export const useSoundBoardDismissContentTypes = function useSoundBoardDismissContentTypes(isSoundboardButtonDisabled) {
  ...
  if (!flag) {
    ...
    if (!values.some((joinSound) => null != joinSound.joinSound)) {
      const result = tmp(4675).ageEligibleForPremiumUpsell(stateFromStores);
