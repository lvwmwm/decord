...
// Function ID: 18108
// Name: HeaderRow
// Dependencies: [5, 32, 19, 17, 18101, 1074, 1375, 21, 4829, 576, 9843, 10682, 1241, 1115, 1255, 5442, 5273, 4825, 504, 2]
// Exports: ConnectedHeaderRow

// Module 18107 (HeaderRow)
...
import GuildSettingsEmojiStore from "GuildSettingsEmojiStore" /* 18101 */;
...
function HeaderRow(guild) {
            ...
            if (arg0 === 1) {
              ...
              if (null != base64) {
                upload = function upload(image, originalMd5, roles) {
                  ...
                  const uploadEmojiResult = closure_1_0(10682).uploadEmoji({ guildId: image.id, image, name: combined, roles, originalMd5 });
                  closure_1_0(10682).uploadEmoji({ guildId: image.id, image, name: combined, roles, originalMd5 }).then(() => closure_2_1(1241).track(constants.EMOJI_UPLOAD_COMPLETED, { guild_id: image.id, upload_id })).catch((error) => {
  ...
  if (canCreateExpressions) {
    ...
    canCreateExpressions = tmp13(tmp7(5273).Button, obj8);
  }
  ...
  if (tmp13Result) {
    ...
    tmp13Result = tmp13(tmp7(4825).Text, obj9);
  }
  const obj10 = { children: null };
  items1[1] = tmp13Result;
  let obj11 = { style: tmp.uploadInstructionsContainer, children: null };
  const items2 = [closure_14(guild(4825).Text, { variant: "text-sm/medium", color: "text-muted", children: description }), , ];
  ...
  items2[1] = closure_14(guild(4825).Text, obj12);
