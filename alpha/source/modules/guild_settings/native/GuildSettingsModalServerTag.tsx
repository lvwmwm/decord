...
// Dependencies: [5, 32, 19, 17, 9841, 2064, 9862, 1074, 8206, 21, 4757, 576, 1484, 9864, 504, 9842, 8438, 9843, 9861, 4454, 1115, 12737, 4649, 5843, 7622, 1485, 5110, 1177, 7286, 10036, 4753, 5906, 7447, 5824, 5185, 14191, 8876, 14189, 18033, 2]
// Exports: default

// Module 18032 (GuildSettingsModalServerTag)
...
let GuildProfileFetchStatus = fn(9841).GuildProfileFetchStatus;
...
export default function GuildSettingsModalServerTag(guildId) {
  ...
  const result = guildId(9864).canUseMobileServerTagSettings(guildId);
  dependencyMap = result;
  let obj2 = guildId(9864);
  const items = [result1];
  ...
  if (guildSupportsTagsResult) {
    guildSupportsTagsResult = tmp2(8438).guildSupportsTags(stateFromStores);
  ...
  result1 = guildId(9864).isServerTagDraftDirty(stateFromStores1, stateFromStores2);
    ...
    if (c3 === 2) {
      ...
      try {
        ...
        if (0 === dependencyMap) {
              ...
              if (!ref.current) {
                if (closure_9) {
                  if (dependencyMap) {
                    ...
                    dependencyMap = 1;
                    c3 = 1;
                    const obj7 = { value: tmp2(9843).saveGuildProfile(guildId, { tag: null, badge: null, badgeColorPrimary: null, badgeColorSecondary: null }), done: false };
                    return obj7;
                  }
                }
              }
          ...
          tmp2(4454).presentError(intl.string(tmp2(1115).t.RTSuVn));
          const obj = tmp2(4454);
        }
        c3 = 3;
      } catch (tmp27) {
  ...
  const effect2 = obj7.useEffect(() => {
    if (c2) {
      if (closure_12) {
        if (c7) {
          let obj = {
            headerRight: first ? (() => closure_1_13(guildId(5843).HeaderSubmittingIndicator, {})) : (() => {
                    const obj = { text: null, onPress: null, disabled: null };
                    const intl = guildId(1115).intl;
                    obj.text = intl.string(guildId(1115).t["R3BPH+"]);
                    obj.onPress = onPress;
                    obj.disabled = !closure_1_9;
                    return closure_13(guildId(7622).HeaderActionButton, obj);
                  })
          };
  ...
  const tmp2Result3 = guildId(9864);
  let tmp26 = result1;
  if (result1) {
    tmp26 = !first;
  }
  ...
  if (result) {
    if (tmp24) {
      if (null == stateFromStores) {
        ...
        return closure_13(navigation(10036), obj8);
      } else if (null == stateFromStores1) {
        return closure_13(tmp2(7286).SceneLoadingIndicator, {});
      } else {
        let tmp29;
        if (tmp14) {
          if (null != stateFromStores1.tag) {
            if ("" !== stateFromStores1.tag) {
              ...
              tmp29 = closure_13(tmp2(4753).Text, obj9);
            }
          }
        }
        ...
        const items11 = [closure_13(tmp2(7447).TableSwitchRow, obj11), ];
        ...
        if (null != tmp29) {
          ...
          if (tmp32Result) {
            ...
            tmp32Result = tmp32(tmp2(14191).GuildBadge, size);
          }
          const items12 = [tmp32Result, tmp29];
          obj13.children = items12;
          tmp31Result = tmp31(tmp2(5185).Stack, obj13);
        }
        obj12.trailing = tmp31Result;
        items11[1] = closure_13(tmp2(5824).TableRow, obj12);
        obj10.children = items11;
        const tmp31Result2 = closure_14(tmp2(5906).TableRowGroup, obj10);
        ...
        if (guildSupportsTagsResult) {
          ...
          const items14 = [tmp32(tmp2(4753).Text, obj17), tmp31Result2, , ];
          ...
          items14[2] = tmp32(tmp2(4753).Text, obj18);
          ...
          items14[3] = tmp32(tmp39(14189), obj19);
          obj15.children = items14;
          let tmp40 = obj15;
        } else {
          const obj20 = { guildId, onUnlockPress: callback2 };
          const items15 = [tmp32(tmp39(18033), obj20), tmp31Result2];
          obj15.children = items15;
          tmp40 = obj15;
        }
        obj14.children = closure_14(tmp2(5185).Stack, tmp40);
