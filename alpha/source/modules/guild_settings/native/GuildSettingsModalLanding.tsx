...
// Dependencies: [19, 4460, 2064, 4462, 1372, 16545, 9935, 1074, 21, 5007, 4827, 5908, 1115, 4781, 4467, 18004, 15291, 9937, 9217, 18006, 4769, 5990, 9113, 10455, 12074, 5393, 9919, 18007, 9628, 5841, 9626, 15937, 10725, 4524, 576, 1484, 504, 9934, 18009, 7588, 7595, 4520, 8947, 5269, 17400, 1397, 18010, 7371, 2]
// Exports: default
...
function SettingsSection(guild) {
  ...
  if (canManageGuild) {
    ...
    obj6.icon = tmp(tmp2(5908).TableRow.Icon, obj7);
    obj6.onPress = function onPress() {
      return importDefault(constants2.INTEGRATIONS);
    };
    items.push(tmp(tmp2(5908).TableRow, obj6, "integrations"));
  }
  ...
  if (tmp2Result.canUseMobileServerTagSettings(guild.id)) {
    ...
    obj8.icon = tmp(tmp2(5908).TableRow.Icon, obj9);
    obj8.onPress = function onPress() {
      return importDefault(constants2.TAG);
    };
    items.push(tmp(tmp2(5908).TableRow, obj8, "server-tag"));
  }
  if (isGuildAdmin) {
  ...
  if (isGuildAdmin) {
    ...
    obj10.icon = tmp(tmp2(5908).TableRow.Icon, obj11);
    obj10.onPress = function onPress() {
      return importDefault(constants2.VANITY_URL);
    };
    items.push(tmp(tmp2(5908).TableRow, obj10, "vanity"));
  }
  let tmpResult = null;
  if (0 !== items.length) {
    ...
    tmpResult = tmp(tmp2(5990).TableRowGroup, obj12);
  }
  return tmpResult;
}
...
function PeopleSection(pushScreen) {
  ...
  if (canManageRoles) {
    ...
    obj3.icon = tmp(tmp2(5908).TableRow.Icon, obj4);
    obj3.onPress = function onPress() {
      return pushScreen(constants2.ROLES);
    };
    items.push(tmp(tmp2(5908).TableRow, obj3, "roles"));
  }
  if (canManageGuild) {
    const obj5 = { label: null, arrow: true, icon: null, onPress: null };
    const intl3 = tmp2(1115).intl;
    ...
    obj5.icon = tmp(tmp2(5908).TableRow.Icon, obj6);
    obj5.onPress = function onPress() {
      return pushScreen(constants2.INSTANT_INVITES);
    };
    items.push(tmp(tmp2(5908).TableRow, obj5, "invites"));
  }
  const obj7 = { title: null, hasIcons: true, children: null };
  const intl4 = tmp2(1115).intl;
...
function CommunitySection(pushScreen) {
  ...
  if (!canManageGuild) {
    ...
    if (hasItem) {
      ...
      obj.icon = tmp2(tmp3(5908).TableRow.Icon, obj6);
      obj.onPress = function onPress() {
...
function GuildSettingsModalLandingInner(guild) {
  ...
  if (canManageGuildRoleSubscriptions) {
    ...
    tmp17Result = tmp17(tmp3(18010), obj11);
  }
