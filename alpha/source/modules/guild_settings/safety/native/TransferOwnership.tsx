...
// Dependencies: [5, 32, 19, 17, 1372, 12189, 1074, 21, 4829, 576, 4526, 1484, 504, 38, 9939, 12187, 4522, 4981, 5889, 4825, 12191, 1177, 1115, 4673, 5992, 5909, 5273, 2]
// Exports: default

// Module 12190 (TransferOwnership)
...
export default function TransferOwnership(guild) {
          ...
          if (arg0 === 1) {
            ...
            if (!mfaEnabled.mfaEnabled) {
              if (null != tmp27.email) {
                v1 = 1;
                dependencyMap = 1;
                const obj6 = { value: v1(9939).sendTransferOwnershipPincode(user.id), done: false };
                return obj6;
              }
            }
            let MFA = null;
            ...
            const obj4 = v1(9939);
            v1(9939).transferOwnership(user.id, id.id, MFA).then(() => {
              v1(12187).close();
              const obj = v1(12187);
              v1(9939).close();
              const obj2 = v1(9939);
              const result = closure_1_0(4522).showTransferOwnershipSuccess();
            }, NOOP);
            dependencyMap = 3;
            const transferOwnershipResult = v1(9939).transferOwnership(user.id, id.id, MFA);
          }
        } else if (arg0 === 1) {
          dependencyMap = 3;
          throw value;
  ...
  const token = guild(4526).useToken(toUser(576).modules.mobile.TABLE_ROW_PADDING);
  let tmp5 = closure_16();
  let obj = guild(4526);
  ...
  const items1 = [closure_13(c7, { source: toUser(12191), style: tmp5.arrow }), , , ];
  const obj10 = { style: tmp5.avatarsWrapper, children: null };
  const obj9 = { source: toUser(12191), style: tmp5.arrow };
  ...
  if (null == nickname) {
    if (!hasAvatarForGuildResult) {
      ...
      const obj15 = { GuildHook: GuildWithSmallIcon, user: tmp3(4673).getUserTag(toUser) };
      let formatResult = intl.format(tmp(1115).t["2XLnG0"], obj15);
      const tmp3Result = tmp3(4673);
    }
    ...
    const obj19 = { username: tmp3(4673).getUserTag(toUser) };
    obj18.label = intl3.format(tmp(1115).t.xm6ACJ, obj19);
    obj18.onPress = function handleConfirmToggle(arg0) {
      _undefined(arg0);
    };
    obj17.children = tmp11(tmp(5909).TableCheckboxRow, obj18);
    items3[1] = tmp11(tmp(5992).TableRowGroup, obj17);
    const obj20 = {
    ...
    obj20.disabled = !tmp8;
    items3[2] = tmp11(tmp(5273).Button, obj20);
  ...
  obj21.user = toUser(4673).getUserTag(toUser);
