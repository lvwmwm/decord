...
function OtaVerificationActionSheet(result) {
  ...
  if (null !== result.totalFileCount) {
    ...
    const items4 = [tmp5(tmp3(4832).Text, { variant: "heading-md/bold", children: "File Counts" }), ];
    ...
    items4[1] = tmp2(tmp3(4832).Text, obj6);
  ...
  if (result.failures.length > 0) {
    ...
    const items6 = [tmp5(tmp3(4832).Text, { variant: "heading-md/bold", children: "Failures" }), ];
...
export default function DevToolsOTATestScreen() {
      ...
      try {
        ...
        if (0 === dependencyMap) {
          if (arg0 === 1) {
            ...
            dependencyMap = 1;
            c3 = 1;
            const obj5 = { value: tmp5(11269).verifyOtaFiles(), done: false };
            return obj5;
          }
        } else if (arg0 === 1) {
          c3 = 3;
          ...
          tmp5(4800).openLazy(Promise.resolve(obj7), "OtaVerificationActionSheet", obj8);
          c3 = 3;
          return { value: "HermesInternal", done: null };
        }
      } catch (tmp10) {
  ...
  obj2.children = items;
  const items1 = [closure_9(subLabel(5999).TableRowGroup, obj2), , , ];
  let str;
  if (tmp5 != null) {
    str = tmp5.source;
  }
  ...
  items1[3] = closure_9(subLabel(5999).TableRowGroup, obj12);
  obj11.children = items1;
  obj.children = closure_9(subLabel(5279).Stack, obj11);
