...
// Dependencies: [32, 19, 17, 5846, 4756, 21, 5736, 4757, 576, 5824, 4455, 7448, 504, 5906, 16016, 7228, 16033, 5185, 16034, 7297, 2]
// Exports: default

// Module 16032 (DevToolsTogglesScreen)
...
function ToggleTableRow(toggleName) {
  ...
  return closure_9(toggleName(5824).TableRow, {
    label: description,
    labelLineClamp: 1,
    subLabel: toggleName,
    subLabelLineClamp: 1,
    ...
    trailing: closure_9(toggleName(7448).FormSwitch, { value, onValueChange })
  }, toggleName);
}
function DevTogglesForCategory(title) {
  ...
  if (0 !== stateFromStores.length) {
    ...
    tmp3 = closure_9(category(5906).TableRowGroup, obj2);
  }
  return tmp3;
}
const ScrollView = fn(17).ScrollView;
const CATEGORY_LABELS = fn(4756).CATEGORY_LABELS;
const jsxProd = fn(21);
...
export default function DevToolsTogglesScreen() {
  ...
  const query = tmp3[0];
  const manaTextMigrationHighlightRestartNotice = query(16033).useManaTextMigrationHighlightRestartNotice();
  let obj = query(16033);
  const tmp5 = query;
  ...
  const items3 = [
    closure_9(query(5824).TableRow, {
      label: "Clear All",
      variant: "danger",
      onPress() {
        first(16034).clearAll();
        const obj = first(16034);
  ...
  items3[1] = closure_9(query(5824).TableRow, { label: closure_9(query(7297).SearchField, { size: "md", placeholder: "Search design toggles", onChange: tmp3[1] }) });
  obj5.children = items3;
  const items4 = [closure_10(query(5906).TableRowGroup, obj5), , ];
  let tmp7Result = null;
    ...
    const obj8 = {
      ...
      children: stateFromStores.map((item) => {
          ...
          return closure_9(closure_13, {
            ...
            value: tmp2,
            onValueChange(arg0) {
              return first(16034).toggle(query, arg0);
            }
          }, tmp);
        })
    };
    tmp7Result = tmp7(tmp5(5906).TableRowGroup, obj8);
  }
  ...
  obj3.children = tmp9(query(5185).Stack, obj9);
