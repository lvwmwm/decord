...
// Function ID: 12400
// Name: useBannerBots
// Dependencies: [19, 2064, 12378, 504, 9490, 12400, 12371, 2]
// Exports: useBannerBots

// Module 12399 (useBannerBots)
import noop from "module_19" /* 19 */;
import GuildStore from "GuildStore" /* 2064 */;
import AppLauncherOnboardingStore from "AppLauncherOnboardingStore" /* 12378 */;
...
export const useBannerBots = function useBannerBots(context) {
  ...
  let obj = (function useAppsMap(context) {
    const apps = first1(12400).useApplicationsInContext({ context: context.context, onlyWithCommands: true, includeBuiltIn: false, includeEmbeddedApps: false, includeNonEmbeddedApps: true }).apps;
    const items = [apps];
  ...
  let apps = first1(12400).useApplicationsInContext({ context, onlyWithCommands: true, includeBuiltIn: false, includeEmbeddedApps: false, includeNonEmbeddedApps: true }).apps;
  let obj2 = first1(12400);
  const tmp2 = first1;
  const obj3 = (function useCommandsMap(context) {
    const commands = first1(12400).useApplicationCommandsInContext({ context: context.context, includeBuiltIn: false }).commands;
  ...
  const tmp4 = (function useFrecencyCommandIds(context) {
    ...
    return context(9490).useTopCommands(obj2);
  })({ context });
  ...
  const activityApplications = first1(12371).useActivityApplications({ guildId: guild_id, fetchesShelf: true });
  const obj4 = first1(12371);
