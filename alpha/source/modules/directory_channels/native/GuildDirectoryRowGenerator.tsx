...
// Dependencies: [12639, 12638, 1115, 2]
// Exports: generateDirectoryRows

// Module 13009 (GuildDirectoryRowGenerator)
...
export const generateDirectoryRows = function generateDirectoryRows(arg0, arr, currentCategoryId) {
  ...
  if (0 === arr.length) {
    ...
    const obj4 = set(12638);
    return set(12638).rankGuildEntries(arr).map((entry) => ({ type: constants.ENTRY, entry }));
  } else {
    const items = [];
    const rankByDateAddedResult = set(12638).rankByDateAdded(arr);
    const _Set = Set;
    ...
    const obj5 = set(12638);
    const result = set(12638).orderByTotalMemberCount(found);
