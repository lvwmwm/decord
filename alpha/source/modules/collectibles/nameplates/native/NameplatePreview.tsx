...
// Dependencies: [19, 17, 4749, 2105, 21, 4757, 576, 1970, 8489, 8432, 504, 4601, 4990, 1177, 9104, 11166, 11167, 4753, 2]
// Exports: NameplatePreview

// Module 11490 (NameplatePreview)
...
export const NameplatePreview = function NameplatePreview(aria_hidden) {
  ...
  const avatarDecoration = user(8489).useAvatarDecoration(user, guildId);
  pendingAvatarDecoration = guildId(8432)({ guildId }).pendingAvatarDecoration;
  const obj2 = user(8489);
  ...
  const obj4 = user(504);
  const name = guildId(4601).useName(user);
  ...
  const tmp15 = guildId(4990)({ userId: user.id, guildId, pendingDisplayNameStyles });
  ...
  const items3 = [closure_7(guildId(9104), { nameplate: nameplateData, style: tmp.nameplate, fullOpacity: true, animate: flag2 }), closure_7(pendingAvatarDecoration, { style: tmp.avatar, children: memo }), ];
  const obj10 = { style: tmp.content, children: null };
  let tmp19Result = null != tmp15;
  if (tmp19Result) {
    const obj11 = { userId: user.id, guildId, userName: pendingGlobalName, variant: "text-md/semibold", effectDisplayType: tmp4(11167).EffectDisplayType.STATIC, lineClamp: 1, pendingDisplayNameStyles };
    tmp19Result = tmp19(tmp7(11166), obj11);
    const tmp7Result = tmp7(11166);
  }
  ...
  if (tmp19Result2) {
    ...
    tmp19Result2 = tmp19(tmp4(4753).Text, obj12);
  }
