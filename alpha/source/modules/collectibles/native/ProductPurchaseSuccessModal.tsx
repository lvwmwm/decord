...
// Dependencies: [32, 718, 19, 17, 4821, 1074, 21, 4829, 576, 1973, 11433, 5936, 5985, 1115, 4561, 5272, 4830, 4797, 7882, 11435, 4526, 5285, 8526, 11437, 504, 11438, 11439, 9210, 8682, 8519, 7456, 11444, 9158, 9171, 11462, 11571, 11572, 4825, 7884, 5273, 2]
// Exports: default

// Module 11434 (ProductPurchaseSuccessModal)
...
function CancelButton(arg0) {
  ...
  return closure_12(_mod5936.HeaderBackButton, obj);
}
...
export default function ProductPurchaseSuccessModal(stageCollectibleChangeForEditProfile) {
  ...
  const backgroundColors = callback(11435)(product.styles).backgroundColors;
  let tertiary;
  if (backgroundColors != null) {
    tertiary = backgroundColors.tertiary;
  }
  ...
  if (useCategoryImage) {
    ...
    if (showOrbBalancePill) {
      ...
      showOrbBalancePill = tmp30(tmp2(11444).BalanceWidgetPill, obj12);
    }
    ...
    if (tmp2(1973).CollectiblesItemType.BUNDLE === type) {
      ...
      if (tmp29Result) {
        ...
        tmp29Result = tmp29(tmp4(9158), obj17);
      }
    ...
    if (null != renderMessages) {
      ...
      const items10 = [tmp30(tmp2(4825).Text, obj23), ];
      const obj25 = { variant: "text-md/medium", color: "text-overlay-light", style: tmp6.title, children: null };
      let result = tmp2(7884).isPremiumCollectiblesProduct(product);
