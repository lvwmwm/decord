...
// Dependencies: [32, 718, 19, 17, 4819, 1074, 21, 4827, 576, 1973, 11427, 5934, 5983, 1115, 4559, 5270, 4828, 4795, 7880, 11429, 4524, 5283, 8522, 11431, 504, 11432, 11433, 9206, 8678, 8515, 7454, 11438, 9154, 9167, 11456, 11565, 11566, 4823, 7882, 5271, 2]
// Exports: default

// Module 11428 (ProductPurchaseSuccessModal)
...
function CancelButton(arg0) {
  ...
  return closure_12(_mod5934.HeaderBackButton, obj);
}
...
export default function ProductPurchaseSuccessModal(stageCollectibleChangeForEditProfile) {
  ...
  const backgroundColors = callback(11429)(product.styles).backgroundColors;
  let tertiary;
  if (backgroundColors != null) {
    tertiary = backgroundColors.tertiary;
  }
  ...
  if (useCategoryImage) {
    ...
    if (showOrbBalancePill) {
      ...
      showOrbBalancePill = tmp30(tmp2(11438).BalanceWidgetPill, obj12);
    }
    ...
    if (tmp2(1973).CollectiblesItemType.BUNDLE === type) {
      ...
      if (tmp29Result) {
        ...
        tmp29Result = tmp29(tmp4(9154), obj17);
      }
    ...
    if (null != renderMessages) {
      ...
      const items10 = [tmp30(tmp2(4823).Text, obj23), ];
      const obj25 = { variant: "text-md/medium", color: "text-overlay-light", style: tmp6.title, children: null };
      let result = tmp2(7882).isPremiumCollectiblesProduct(product);
