...
// Function ID: 16151
// Name: HeroBlock
// Dependencies: [19, 17, 7787, 1076, 1074, 21, 9049, 4757, 576, 9156, 16151, 1484, 9052, 504, 11743, 4691, 16152, 4458, 4606, 16153, 15350, 7409, 7429, 16155, 1241, 4608, 16157, 16158, 16159, 5199, 4753, 5187, 1115, 12024, 5668, 5341, 7456, 7403, 16160, 9002, 2]
// Exports: default

// Module 16150 (HeroBlock)
...
import FeaturedFirstCardCoachmarkAnchorDefault from "FeaturedFirstCardCoachmarkAnchor" /* 16155 */;
...
const result = 0.75 * fn(9049).COLLECTIBLES_SHOP_CARD_WIDTH;
...
export default function _default(heroBlock) {
  ...
  closure_5 = undefined;
  closure_6 = undefined;
  let obj = heroBlock(16151);
  dependencyMap = heroBlock(1484).useNavigation();
  let obj2 = heroBlock(1484);
  noop = heroBlock(9052).useCollectiblesAnalyticsContext();
  ...
  let isEligibleForQuests = heroBlock(11743).getIsEligibleForQuests();
  const tmpResult9 = heroBlock(11743);
  const tmp7 = preferVCPrice(4691)();
  const tmp8 = closure_14();
  const tmpResult10 = heroBlock(16152);
  const token = heroBlock(4458).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOW);
  const tmpResult11 = heroBlock(4458);
  const tmpResult12 = heroBlock(4606);
  const tmpResult13 = heroBlock(4606);
  const hexToRgbaStringResult = tmpResult12.hexToRgbaString(heroBlock(4606).hexWithOpacity(token, 0));
  const token1 = heroBlock(4458).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(16153)();
  closure_5 = tmp12;
  const items1 = [heroBlock.rankedSkuIds, tmp12];
  const memo = noop.useMemo(() => closure_5(heroBlock.rankedSkuIds), items1);
  const tmpResult14 = heroBlock(4458);
  const filteredAndSortedProducts = heroBlock(15350).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  closure_6 = tmp14;
  const tmpResult15 = heroBlock(15350);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      const obj4 = { value: tmp15(preferVCPrice(7429).COLLECTIBLES_SHOP_HERO).analyticsLocations, children: null };
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(5199), obj8);
        }
        ...
        items3[2] = tmp22(tmp6(5199), obj12);
      ...
      if (tmp4) {
        ...
        if (tmp22Result6) {
          ...
          tmp22Result6 = tmp22(tmp(4753).Text, obj15);
        }
        ...
        if (tmp22Result7) {
          ...
          tmp22Result7 = tmp22(tmp(4753).Text, obj16);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(5187).Button, obj19);
        }
        ...
        const obj28 = { style: tmp8.heroViewAllIcon, children: tmp22(tmp(7456).ChevronSmallRightIcon, { size: "sm", color: "white" }) };
        items9[1] = tmp22(tmp25, obj28);
        obj24.children = items9;
        obj20.children = tmp24(tmp25, obj24);
        tmp24Result2 = tmp22(tmp(5341).PressableOpacity, obj20, stateFromStores.storeListingId);
      }
      ...
      if (tmp4) {
        ...
        let tmp22Result9 = tmp22(tmp6(16160), obj30);
        const tmp6Result3 = tmp6(16160);
      } else {
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj34.snapToInterval = tmp(9049).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(576).space.PX_12;
          ...
          tmp22Result10 = tmp22(tmp(9002).FlashList, obj34);
        }
        const obj36 = { children: tmp22Result10 };
        tmp22Result9 = tmp22(closure_12, obj36);
      }
      const obj37 = { children: tmp22Result9 };
      obj29.children = closure_11(tmp(7403).LayerScope, obj37);
      items6[2] = closure_11(closure_5, obj29);
      obj6.children = items6;
      obj5.children = closure_13(closure_5, obj6);
      obj4.children = closure_11(tmp6(16159), obj5);
      return closure_11(tmp(7409).AnalyticsLocationProvider, obj4);
    } else {
      if (tmpResult16.isThemeDark(tmp7)) {
        let tmp6Result4 = tmp6(16157);
      } else {
        tmp6Result4 = tmp6(16158);
      }
      tmpResult16 = tmp(4608);
    }
  }
};
