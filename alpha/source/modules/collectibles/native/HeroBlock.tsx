...
// Function ID: 16204
// Name: HeroBlock
// Dependencies: [19, 17, 7870, 1076, 1074, 21, 9120, 4827, 576, 9227, 16204, 1484, 9123, 504, 11819, 4761, 16205, 4524, 4676, 16206, 15402, 7493, 7513, 16208, 1241, 4678, 16210, 16211, 16212, 5283, 4823, 5271, 1115, 12101, 5752, 5425, 7540, 7487, 16213, 9073, 2]
// Exports: default

// Module 16203 (HeroBlock)
...
import FeaturedFirstCardCoachmarkAnchorDefault from "FeaturedFirstCardCoachmarkAnchor" /* 16208 */;
...
const result = 0.75 * fn(9120).COLLECTIBLES_SHOP_CARD_WIDTH;
...
export default function _default(heroBlock) {
  ...
  closure_5 = undefined;
  closure_6 = undefined;
  let obj = heroBlock(16204);
  dependencyMap = heroBlock(1484).useNavigation();
  let obj2 = heroBlock(1484);
  noop = heroBlock(9123).useCollectiblesAnalyticsContext();
  ...
  let isEligibleForQuests = heroBlock(11819).getIsEligibleForQuests();
  const tmpResult9 = heroBlock(11819);
  const tmp7 = preferVCPrice(4761)();
  const tmp8 = closure_14();
  const tmpResult10 = heroBlock(16205);
  const token = heroBlock(4524).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOW);
  const tmpResult11 = heroBlock(4524);
  const tmpResult12 = heroBlock(4676);
  const tmpResult13 = heroBlock(4676);
  const hexToRgbaStringResult = tmpResult12.hexToRgbaString(heroBlock(4676).hexWithOpacity(token, 0));
  const token1 = heroBlock(4524).useToken(preferVCPrice(576).colors.BACKGROUND_BASE_LOWEST);
  const tmp12 = preferVCPrice(16206)();
  closure_5 = tmp12;
  const items1 = [heroBlock.rankedSkuIds, tmp12];
  const memo = noop.useMemo(() => closure_5(heroBlock.rankedSkuIds), items1);
  const tmpResult14 = heroBlock(4524);
  const filteredAndSortedProducts = heroBlock(15402).useFilteredAndSortedProducts({ products: memo, bypassAndroidUnsyncedFilter: tmp4 });
  closure_6 = tmp14;
  const tmpResult15 = heroBlock(15402);
  let unpublishedAt;
  ...
  if (undefined === stateFromStores) {
    ...
    if (!tmp4) {
      const obj4 = { value: tmp15(preferVCPrice(7513).COLLECTIBLES_SHOP_HERO).analyticsLocations, children: null };
      ...
      if (tmp24Result) {
        ...
        if (tmp4) {
          ...
          tmp22Result = tmp22(tmp6(5283), obj8);
        }
        ...
        items3[2] = tmp22(tmp6(5283), obj12);
      ...
      if (tmp4) {
        ...
        if (tmp22Result6) {
          ...
          tmp22Result6 = tmp22(tmp(4823).Text, obj15);
        }
        ...
        if (tmp22Result7) {
          ...
          tmp22Result7 = tmp22(tmp(4823).Text, obj16);
        }
        ...
        if (isEligibleForQuests) {
          ...
          isEligibleForQuests = tmp22(tmp(5271).Button, obj19);
        }
        ...
        const obj28 = { style: tmp8.heroViewAllIcon, children: tmp22(tmp(7540).ChevronSmallRightIcon, { size: "sm", color: "white" }) };
        items9[1] = tmp22(tmp25, obj28);
        obj24.children = items9;
        obj20.children = tmp24(tmp25, obj24);
        tmp24Result2 = tmp22(tmp(5425).PressableOpacity, obj20, stateFromStores.storeListingId);
      }
      ...
      if (tmp4) {
        ...
        let tmp22Result9 = tmp22(tmp6(16213), obj30);
        const tmp6Result3 = tmp6(16213);
      } else {
        if (0 === filteredAndSortedProducts.length) {
          ...
          obj34.snapToInterval = tmp(9120).COLLECTIBLES_SHOP_CARD_WIDTH + tmp6(576).space.PX_12;
          ...
          tmp22Result10 = tmp22(tmp(9073).FlashList, obj34);
        }
        const obj36 = { children: tmp22Result10 };
        tmp22Result9 = tmp22(closure_12, obj36);
      }
      const obj37 = { children: tmp22Result9 };
      obj29.children = closure_11(tmp(7487).LayerScope, obj37);
      items6[2] = closure_11(closure_5, obj29);
      obj6.children = items6;
      obj5.children = closure_13(closure_5, obj6);
      obj4.children = closure_11(tmp6(16212), obj5);
      return closure_11(tmp(7493).AnalyticsLocationProvider, obj4);
    } else {
      if (tmpResult16.isThemeDark(tmp7)) {
        let tmp6Result4 = tmp6(16210);
      } else {
        tmp6Result4 = tmp6(16211);
      }
      tmpResult16 = tmp(4678);
    }
  }
};
