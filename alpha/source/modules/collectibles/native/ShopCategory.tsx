...
// Function ID: 16147
// Name: ShopCategory
// Dependencies: [19, 17, 1076, 1074, 21, 9049, 4757, 576, 7409, 1484, 15350, 16143, 9002, 16147, 9052, 4724, 8449, 7786, 7429, 5341, 1115, 5806, 7456, 2]
// Exports: ShopCategory

// Module 16146 (ShopCategory)
...
const sum = 100 + fn(9049).COLLECTIBLES_SHOP_CARD_HEIGHT;
...
export const ShopCategory = function ShopCategory(category) {
  ...
  const filteredAndSortedProducts = category(15350).useFilteredAndSortedProducts({ products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive });
  const mobileBannerUrl = category.mobileBannerUrl;
  let obj2 = category(15350);
  let obj3 = { products: category.products, bypassAndroidUnsyncedFilter: category.isOrbsExclusive };
  const collectiblesShopDeepLinkProps = category(16143).useCollectiblesShopDeepLinkProps({ products: filteredAndSortedProducts });
  ...
  const obj4 = category(16143);
  let items = [category.storeListingId];
  const recyclingState = category(9002).useRecyclingState(null, items, () => {
    const current = ref.current;
    if (current != null) {
      current.scrollToOffset({ offset: 0, animated: false });
    }
  });
  const obj6 = category(9002);
  let tmp9 = null != productIndex;
  if (tmp9) {
    tmp9 = productIndex > 0;
  }
  const obj7 = category(16147);
  const scrollToInitialIndexOnce = obj7.useScrollToInitialIndexOnce({ shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(16147).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId });
  const obj8 = { shouldScroll: tmp9, initialScrollIndex: productIndex, flashListRef: ref, afterMs: category(16147).INITIAL_SCROLL_DELAY_MS, resetKey: category.storeListingId };
  collectiblesAnalyticsContext = category(9052).useCollectiblesAnalyticsContext();
  ...
  const tmp4Result = category(9052);
  ...
  if (tmp14Result) {
    ...
    tmp14Result = tmp14(tmp(5806), obj14);
  }
  ...
  obj17.snapToInterval = category(9049).COLLECTIBLES_SHOP_CARD_WIDTH + 12;
  ...
  items5[1] = ref(category(9002).FlashList, obj17);
  obj10.children = items5;
  obj9.children = collectiblesAnalyticsContext(filteredAndSortedProducts, obj10);
  return ref(category(9052).CollectiblesAnalyticsProvider, obj9);
};
