...
// Dependencies: [19, 17, 1076, 9084, 21, 4757, 576, 8499, 5806, 11286, 8474, 9108, 8444, 9083, 1970, 9104, 1973, 1077, 9130, 9129, 9096, 1115, 4753, 7798, 9132, 11287, 8451, 4415, 9148, 2]
// Exports: default
...
function ProfileFrameAssetPreview(arg0) {
  ...
  if (null != tmp3) {
    ...
    tmp4 = React5(tmp(9108), obj);
    const tmpResult = tmp(9108);
  }
  return tmp4;
}
...
function CollectibleProductPreviewContent(userAvatarSource) {
  ...
  if (ALL.has(product.skuId)) {
    const size = { skuId: product.skuId, width: tmp2(9130).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT, height: tmp2(9130).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT };
    return React5(tmp2(9130).FractionalNitroCoinIllustration, size);
...
function ProductPriceAmountTag(product) {
  ...
  const memo = noop.useMemo(() => {
    if (useOrbPrice) {
      ...
      const orbPrice = tmp(7798).getProductOrbPrice(obj);
      const tmpResult = tmp(7798);
    } else {
      const priceText = tmp(9132).getFormattedPriceForCollectiblesProduct(product, hasShopDiscount, true);
...
export default function CollectiblesShopCheckoutDetails(recipientUser) {
  ...
  const memo = noop.useMemo(() => {
    ...
    if (flag2) {
      ...
      const productOrbPrice = tmp(7798).getProductOrbPrice(obj2);
      let str2;
      ...
      str = tmp(9132).getFormattedPriceForCollectiblesProduct(tmp3, hasShopDiscount, true);
      if (str == null) {
        str = "";
      }
      const tmpResult2 = tmp(9132);
  ...
  if (flag3) {
    ...
    obj5.children = tmp10(tmp3(4753).Text, obj6);
