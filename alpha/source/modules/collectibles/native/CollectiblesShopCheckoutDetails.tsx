...
// Dependencies: [19, 17, 1076, 9155, 21, 4827, 576, 8570, 5890, 11362, 8545, 9179, 8515, 9154, 1970, 9175, 1973, 1077, 9201, 9200, 9167, 1115, 4823, 7881, 9203, 11363, 8522, 4481, 9219, 2]
// Exports: default
...
function ProfileFrameAssetPreview(arg0) {
  ...
  if (null != tmp3) {
    ...
    tmp4 = React5(tmp(9179), obj);
    const tmpResult = tmp(9179);
  }
  return tmp4;
}
...
function CollectibleProductPreviewContent(userAvatarSource) {
  ...
  if (ALL.has(product.skuId)) {
    const size = { skuId: product.skuId, width: tmp2(9201).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT, height: tmp2(9201).FRACTIONAL_NITRO_COIN_SIZE.CHECKOUT };
    return React5(tmp2(9201).FractionalNitroCoinIllustration, size);
...
function ProductPriceAmountTag(product) {
  ...
  const memo = noop.useMemo(() => {
    if (useOrbPrice) {
      ...
      const orbPrice = tmp(7881).getProductOrbPrice(obj);
      const tmpResult = tmp(7881);
    } else {
      const priceText = tmp(9203).getFormattedPriceForCollectiblesProduct(product, hasShopDiscount, true);
      const tmpResult2 = tmp(9203);
    }
...
export default function CollectiblesShopCheckoutDetails(recipientUser) {
  ...
  const memo = noop.useMemo(() => {
    ...
    if (flag2) {
      ...
      const productOrbPrice = tmp(7881).getProductOrbPrice(obj2);
      let str2;
      ...
      str = tmp(9203).getFormattedPriceForCollectiblesProduct(tmp3, hasShopDiscount, true);
      if (str == null) {
        str = "";
      }
      const tmpResult2 = tmp(9203);
  ...
  if (flag3) {
    ...
    obj5.children = tmp10(tmp3(4823).Text, obj6);
