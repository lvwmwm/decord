...
// Dependencies: [19, 17, 21, 4757, 11172, 11173, 4753, 2]
// Exports: default

// Module 11171 (PerLetterEffect)
...
export default function PerLetterEffect(name) {
  ...
  const memo = noop.useMemo(() => {
    const regex = colors(11172)();
    closure_1 = 0;
    let obj = name(11173);
    return name(11173).splitGraphemes(regex).map((children, index) => {
      regex.lastIndex = 0;
  ...
  const items2 = [textStyle, { lineHeight: "max" }];
  obj2.style = items2;
  obj2.children = memo;
  obj.children = jsx(name(4753).Text, {});
