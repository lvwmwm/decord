...
// Dependencies: [19, 2044, 2066, 4844, 4848, 4466, 1074, 21, 10591, 1115, 10592, 10594, 7301, 7443, 10570, 4769, 8094, 10379, 8211, 4791, 4781, 10596, 5401, 10598, 8983, 4771, 11309, 10500, 4796, 10487, 1980, 9958, 11315, 11661, 504, 7599, 12, 8235, 4982, 2020, 11328, 5889, 1177, 7530, 11355, 7532, 10977, 7522, 4522, 2]
// Exports: default

// Module 16541 (ThreadLongPressActionSheet)
...
function ThreadLongPressActionSheetConnected(channel) {
  ...
  const canMarkChannelUnread = channel(10594).useCanMarkChannelUnread(channel);
  const obj4 = channel(10594);
  const canManageThread = channel(7599).useCanManageThread(channel);
  const obj5 = channel(7599);
  const isThreadModerator = channel(7599).useIsThreadModerator(channel);
  const obj6 = channel(7599);
  const canUnarchiveThread = channel(7599).useCanUnarchiveThread(channel);
  const obj7 = channel(7599);
  const canJoinThreadVoice = channel(7599).useCanJoinThreadVoice(channel);
  const obj8 = channel(7599);
  ...
  const tmp14 = onClose(8235)(channel);
  const DeveloperMode = channel(2020).DeveloperMode;
  let setting = DeveloperMode.useSetting();
  const tmp15 = onClose(4982)(channel);
  if (null != stateFromStores) {
    const obj11 = { guild: stateFromStores, size: tmp(5889).GuildIconSizes.LARGE };
    let tmp19 = closure_13(tmp13(5889), obj11);
    let tmp18 = closure_13;
    const tmp13Result = tmp13(5889);
  } else {
    tmp18 = closure_13;
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "thread_action_sheet" }).enabled) {
    if (!stateFromStores1) {
      if (canMarkChannelUnread) {
        ...
        obj14.IconComponent = tmp(10592).ChatMarkUnreadIcon;
      ...
      if (hasJoined) {
        ...
        const obj18 = {
          label: stringResult,
          IconComponent: tmp(10379).GroupPlusIcon,
          onPress() {
        ...
        if (null == tmp23) {
          ...
          if (isThreadModerator) {
            ...
            if (isLockedThreadResult) {
              ...
              obj21.IconComponent = tmp(10596).LockUnlockedIcon;
          ...
          if (!isThreadModerator) {
            ...
            if (isMuted) {
              ...
              obj24.IconComponent = tmp(9958).BellIcon;
              ...
              obj24[1] = tmp(10500).BellSlashIcon;
              ...
              obj25.IconComponent = tmp(11315).ChannelNotificationIcon;
              ...
              obj26.header = tmp18(tmp(11355).ActionSheetIconHeader, obj27);
              const items8 = [
                items5.map((buttons) => {
              ...
              if (setting) {
                ...
                const obj30 = { IconComponent: tmp(10977).IdIcon };
                obj29.icon = tmp18(tmp(7532).ActionSheetRow.Icon, obj30);
                ...
                obj28.children = tmp18(tmp(7532).ActionSheetRow, obj29);
                setting = tmp18(tmp(7532).ActionSheetRow.Group, obj28, "developer-actions");
              }
              items8[1] = setting;
              obj26.children = items8;
              return closure_14(tmp(7530).ActionSheet, obj26);
            }
          } else {
            ...
            const obj31 = {
              label: string7Result,
              IconComponent: tmp(10598).PencilIcon,
              onPress() {
          ...
          const obj32 = { label: string3Result, IconComponent: tmp(8211).PhoneCallIcon, onPress: tmp23 };
  ...
  obj33.IconComponent = tmp(7301).EyeIcon;
