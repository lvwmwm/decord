...
function ThreadLongPressActionSheetConnected(channel) {
  ...
  const canMarkChannelUnread = channel(9709).useCanMarkChannelUnread(channel);
  const obj4 = channel(9709);
  const canManageThread = channel(6687).useCanManageThread(channel);
  ...
  const canJoinThreadVoice = channel(6687).useCanJoinThreadVoice(channel);
  ...
  if (null != stateFromStores) {
    ...
    let tmp19 = closure_13(tmp13(5896), obj11);
    let tmp18 = closure_13;
    const tmp13Result = tmp13(5896);
  } else {
    tmp18 = closure_13;
  ...
  if (MarkChannelUnreadExperiment.getConfig({ location: "thread_action_sheet" }).enabled) {
    if (!stateFromStores1) {
      if (canMarkChannelUnread) {
        ...
        obj14.IconComponent = tmp(9707).ChatMarkUnreadIcon;
        ...
        if (null == tmp23) {
          ...
          if (isThreadModerator) {
            ...
            if (isLockedThreadResult) {
              ...
              obj21.IconComponent = tmp(9711).LockUnlockedIcon;
          ...
          if (!isThreadModerator) {
            ...
            if (isMuted) {
              ...
              obj24.IconComponent = tmp(9067).BellIcon;
              ...
              obj24[1] = tmp(9613).BellSlashIcon;
              ...
              obj25.IconComponent = tmp(10424).ChannelNotificationIcon;
              ...
              obj26.header = tmp18(tmp(10464).ActionSheetIconHeader, obj27);
              const items8 = [
                items5.map((buttons) => {
              ...
              if (setting) {
                ...
                const obj30 = { IconComponent: tmp(10092).IdIcon };
                obj29.icon = tmp18(tmp(6620).ActionSheetRow.Icon, obj30);
                ...
                obj28.children = tmp18(tmp(6620).ActionSheetRow, obj29);
                setting = tmp18(tmp(6620).ActionSheetRow.Group, obj28, "developer-actions");
              }
              items8[1] = setting;
              obj26.children = items8;
              return closure_14(tmp(6618).ActionSheet, obj26);
            }
          } else {
          ...
          const obj32 = { label: string3Result, IconComponent: tmp(7305).PhoneCallIcon, onPress: tmp23 };
  ...
  obj33.IconComponent = tmp(6389).EyeIcon;
