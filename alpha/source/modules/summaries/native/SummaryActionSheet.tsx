...
// Dependencies: [19, 17, 2042, 5047, 11688, 1074, 21, 4794, 11983, 1980, 4827, 576, 7597, 11956, 4520, 1115, 4972, 8707, 8092, 8104, 4684, 11, 1101, 7481, 7454, 11984, 4823, 11986, 10199, 11987, 8293, 2]
// Exports: default, openSummaryDividerActionSheet

// Module 11983 (SummaryActionSheet)
...
export default function SummaryActionSheet(summary) {
  ...
  const callback = obj.useCallback(() => {
    ...
    if (null != guild_id) {
      ...
      tmpResult.openLazy(asyncRequireImpl(11956, dependencyMap.paths), "GuildHighlightsNotifications", obj2);
      const tmp7 = asyncRequireImpl(11956, dependencyMap.paths);
    }
  }, items);
  ...
  const callback2 = obj.useCallback(() => {
    ...
    if (null != channel) {
      if (null != message) {
        const result = tmp(8092).openThreadCreationForMobile(tmp4, summary.startId, constants.SUMMARY_ACTION_SHEET);
        const tmpResult = tmp(8092);
        const obj3 = { name: summary.topic };
        tmp(8104).changeThreadSettings(tmp4.id, obj3);
        const tmp11 = require;
...
export const openSummaryDividerActionSheet = function openSummaryDividerActionSheet(channelId, summaryId) {
  ...
  if (null != findSummaryResult) {
    ...
    obj.openLazy(asyncRequireImpl(11983, dependencyMap.paths), "SummaryDivider" + summaryId, obj2);
    const tmp5 = asyncRequireImpl(11983, dependencyMap.paths);
  }
};
