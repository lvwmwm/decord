...
// Dependencies: [19, 17, 21, 4757, 576, 1364, 4575, 672, 1612, 4458, 5199, 5341, 4753, 2]

// Module 10908 (MediaKeyboardBottomSheetActions)
...
export default noop.memo(function MediaKeyboardBottomSheetActions(onHeightChange) {
  ...
  noop = undefined;
  let variant;
  const gradientValue = onHeightChange(4575).useGradientValue(onHeightChange(4575).GradientPercentage.END);
  let hexResult = null;
  ...
  let obj = onHeightChange(4575);
  const tmp6 = closure_8(useSafeAreaInsetsDefault().bottom, onHeightChange(4458).useToken(nativeDefault.colors.MOBILE_FLOATINGBAR_BACKGROUND_HIGHER), hexResult);
  importDefault = tmp6;
  const tmpResult = onHeightChange(4458);
  dependencyMap = onHeightChange(4458).useToken(nativeDefault.modules.mobile.MEDIA_KEYBOARD_BUTTON_ICON_COLOR_ACTIVE);
  const tmpResult4 = onHeightChange(4458);
  noop = onHeightChange(4458).useToken(nativeDefault.modules.mobile.MEDIA_KEYBOARD_BUTTON_TEXT_COLOR_ACTIVE);
  const tmpResult5 = onHeightChange(4458);
  variant = onHeightChange(4458).useToken(nativeDefault.modules.mobile.MEDIA_KEYBOARD_BUTTON_TEXT_VARIANT);
  ...
  const tmpResult6 = onHeightChange(4458);
