...
// Dependencies: [2042, 1074, 4769, 4770, 1101, 4958, 38, 2]
...
export const transitionToChannel = function transitionToChannel(id, openTextInVoiceIfVoiceChannel) {
  ...
  if (null != channel) {
    ...
    if (prop) {
      tmp7(4958).updateChatOpen(channel.id, true);
      const tmp7Result = tmp7(4958);
    }
...
export const tryTransitionToThreadMessage = function tryTransitionToThreadMessage(parentChannelId, threadId, messageId, arg3) {
  ...
  if (null != channel) {
    ...
    if (null != channel1) {
      ...
      if (prop) {
        tmp20(4958).updateChatOpen(channel1.id, true);
        const tmp20Result = tmp20(4958);
      }
