...
// Dependencies: [1074, 4689, 4686, 4687, 4655, 4668, 2]
    ...
    if (rootNavigationRef.isReady()) {
      if (pathname.startsWith("/channels/")) {
        ...
        if (null != tmpResult.matchPath(pathname, obj2)) {
          ...
          const RouteParam6 = tmp(4668).RouteParam;
          const tmpResult15 = tmp(4655);
          const RouteParam7 = tmp(4668).RouteParam;
              ...
              if (null != channelId) {
                if (false === navigationReplace) {
                  ...
                  if (channelId1 === channelId) {
                    ...
                    tmp(4686).navigateToRootTab(obj4);
                    const tmpResult19 = tmp(4686);
                  } else {
                    const obj6 = { channelId, guildId, messageId, replaceChannelAndFixRoot: navigationReplace };
                    tmp(4686).navigateToChannel(obj6);
                    const tmpResult20 = tmp(4686);
                  }
                  const tmpResult18 = tmp(4687);
                }
              }
              const obj7 = { screen: "guilds", guildId, channelId, resetRoot: navigationReplace };
              tmp(4686).navigateToRootTab(obj7);
              const tmpResult21 = tmp(4686);
      ...
      if (pathname.startsWith("/member-verification/")) {
        ...
        const RouteParam5 = tmp(4668).RouteParam;
        obj12.path = Routes.GUILD_MEMBER_VERIFICATION(RouteParam5.guildId());
        const matchPathResult1 = tmp(4655).matchPath(pathname, obj12);
        if (null != matchPathResult1) {
          const result = tmp(4686).navigateToMemberVerification(matchPathResult1.params.guildId, matchPathResult1.params.inviteCode);
          const tmpResult27 = tmp(4686);
        }
        return true;
      } else {
        ...
        tmp(4686).resetToAuthRoute();
        flag = true;
        const tmpResult28 = tmp(4686);
      }
    }
  }
  return true;
