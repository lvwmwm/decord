...
// Dependencies: [32, 19, 17, 10118, 2039, 21, 4827, 576, 2028, 15075, 3068, 8757, 15076, 7716, 4781, 4823, 1115, 5425, 5983, 15087, 5271, 2]
...
function AgeGroupDescription(ageGroup) {
  ...
  if (useAgeGroupPresentation.AgeGroupState.ADULT === ageGroup) {
    ...
    const obj3 = { handleOnAgeGatedContentHook: tmp(15075).handleOpenAgeGatedContentArticle };
    obj2.children = intl3.format(_modDef3068.gi4ulu, obj3);
    return React6(tmp(4823).Text, obj2);
  } else if (tmp(15075).AgeGroupState.TEEN === ageGroup) {
    const obj4 = { variant: "text-sm/normal", color: "text-default", children: null };
    const intl2 = tmp(1115).intl;
    const obj5 = { handleOnAgeGatedContentHook: tmp(15075).handleOpenAgeGatedContentArticle, handleOnConfirmAgeHook: tmp(15075).handleShowAgeVerification };
    obj4.children = intl2.format(_modDef3068["221iML"], obj5);
    return React6(tmp(4823).Text, obj4);
  } else if (tmp(15075).AgeGroupState.UNVERIFIED === ageGroup) {
    const obj = { variant: "text-sm/normal", color: "text-default", children: null };
    const intl = tmp(1115).intl;
    obj6 = { handleOnAgeGatedContentHook: tmp(15087).handleOpenUnconfirmedAgeGroupSupportArticle, handleOnConfirmAgeHook: tmp(15075).handleShowAgeVerification };
    obj.children = intl.format(_modDef3068["W0/7DD"], obj6);
    return React6(tmp(4823).Text, obj);
  }
}
function AgeGroupCallToAction(ageGroup) {
  ...
  if (useAgeGroupPresentation.AgeGroupState.ADULT === ageGroup) {
    ...
    obj2.onPress = tmp(15075).handleOpenAgeGatedContentArticle;
    return React6(tmp(5271).Button, obj2);
  } else if (tmp(15075).AgeGroupState.UNVERIFIED === ageGroup) {
    const obj = { grow: true, variant: "secondary", size: "md", text: null, onPress: null };
    const intl = tmp(1115).intl;
    obj.text = intl.string(_modDef3068["cI+bc/"]);
    obj.onPress = tmp(15075).handleShowAgeVerification;
    return React6(tmp(5271).Button, obj);
  }
}
