...
export default function PremiumBillingInfo(subscription) {
  ...
  if (null == _slicedToArray(obj.useFetchSubscriptionInvoicePreview(obj2), 1)[0]) {
    ...
    const items = [timestampProducer(tmp2(4832).Text, obj6), , ];
    ...
    const items1 = [timestampProducer(tmp2(4832).Text, obj8), , ];
    const obj9 = { style: tmp.billingRenewalInfo, variant: "text-sm/medium", children: tmp6 };
    items1[1] = timestampProducer(tmp2(4832).Text, obj9);
    ...
    if (null != externalManagementMessage) {
      ...
      tmp11Result = timestampProducer(tmp2(4832).Text, obj11);
    }
    items[2] = tmp11Result;
    obj5.children = items;
