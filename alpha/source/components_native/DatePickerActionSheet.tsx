...
// Dependencies: [32, 19, 17, 21, 4757, 576, 4724, 1364, 7396, 7445, 9809, 1115, 5181, 4493, 1177, 4758, 4753, 5188, 4691, 7209, 4348, 7397, 9810, 4608, 2]
// Exports: default
...
function ActionSheetHeader(handleSubmit) {
  ...
  if (isAndroidResult) {
    ...
    obj2.trailing = tmp4(tmp(7445).ActionSheetCloseButton, obj3);
...
function DateRangeError(children) {
  ...
  let obj = show(4493);
  const tmp4 = show;
  A.__closure = { STANDARD_EASING: show(1177).STANDARD_EASING, show, withTiming: show(4758).withTiming };
  ...
  let obj2 = { STANDARD_EASING: show(1177).STANDARD_EASING, show, withTiming: show(4758).withTiming };
  obj3.children = closure_6(View, { ref, accessible: true, accessibilityRole: "alert", style: tmp.rangeError, children: closure_6(tmp4(4753).Text, { variant: "text-md/medium", color: "text-feedback-critical", children: children.errorText }) });
  return closure_6(ref(4493).View, obj3);
}
function ActionSheetFooter(arg0) {
  ...
  if (obj.isAndroid()) {
    ...
    obj3.textElement = timestampProducer(tmp2(4753).Text, obj4);
    const intl2 = tmp2(1115).intl;
    obj3.accessibilityLabel = intl2.string(tmp2(1115).t["ETE/oC"]);
    obj3.style = tmp.actionButton;
    obj3.onPress = handleCancel;
    const items = [timestampProducer(tmp2(5188).BaseTextButton, obj3), ];
...
export default function DatePickerActionSheet(mode) {
  ...
  const tmp27 = minimumDate(7209)((getTime) => {
    if (null != getTime) {
