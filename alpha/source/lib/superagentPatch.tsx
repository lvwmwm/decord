...
// Dependencies: [5, 4744, 1074, 1272, 17802, 1366, 1434, 573, 1271, 502, 1346, 2111, 1372, 1241, 1364, 17803, 1116, 17804, 17805, 7, 1231, 17808, 1980, 5169, 16026, 10090, 5827, 17814, 14197, 17815, 1462, 2]
...
HTTPUtils.setRequestPatch({
  prepareRequest(promise) {
    ...
    if ("/" === promise.url[0]) {
      ...
      const result6 = promise.set("X-Discord-Locale", closure_0(2111).default.locale);
  ...
  interceptResponse(statusCode, arg1, arg2) {
    ...
    if (obj.isLimitedAccessErrorCode(statusCode.statusCode, code1)) {
      tmp7(1980)(5827, tmp8.paths).then((result) => {
        result.default();
      });
      flag = false;
      const promise3 = tmp7(1980)(5827, tmp8.paths);
    } else {
      ...
      if (tmp7Result.isLimitedAccessErrorCode(statusCode.statusCode, code2)) {
        tmp7(1980)(14197, tmp8.paths).then((result) => {
        ...
        flag = false;
        const promise2 = tmp7(1980)(14197, tmp8.paths);
      } else {
        ...
        if (flag) {
          tmp7(1980)(17815, tmp8.paths).then((openRestrictedHoursModal) => {
            const result = openRestrictedHoursModal.openRestrictedHoursModal();
          });
          flag = false;
          const promise = tmp7(1980)(17815, tmp8.paths);
        }
      }
