...
// Function ID: 5194
// Name: ButtonHooks
// Dependencies: [19, 4757, 4467, 576, 4608, 4458, 4493, 5186, 5190, 5192, 5194, 5189, 4753, 1364, 2]
...
export const useIconSizeStyles = (arg0) => {
  ...
  return fontScale.useMemo(() => {
    Icon;
    if ("sm" === closure_0) {
      let iconSize = tmp(5189).getIconSize(tmp(5192).SMALL_BUTTON_ICON_SIZE);
      const tmpResult = tmp(5189);
    } else {
      iconSize = tmp4;
      if ("lg" === tmp5) {
        iconSize = tmp(5189).getIconSize(tmp(5192).LARGE_BUTTON_ICON_SIZE);
        const tmpResult4 = tmp(5189);
      }
    }
    let width = iconSize;
    if (flag) {
      width = iconSize;
      if (fontScale > 1) {
        const tmpResult5 = tmp(5192);
        const tmp13 = tmp(4753).TextStyleSheet[tmpResult5.getButtonDefaultTextVariant(tmpResult5, tmp5)];
