...
// Function ID: 8265
// Name: ContextMenu
// Dependencies: [19, 21, 1364, 4561, 1478, 8265, 5280, 8266, 5267, 8267, 4680, 1115, 8268, 6985, 4797, 5258, 2]
// Exports: ContextMenu

// Module 8264 (ContextMenu)
...
export const ContextMenu = function ContextMenu(triggerOnLongPress) {
  ...
  callback2 = obj3.useCallback((x, y, positionX, positionY, height, width) => {
    ...
    if (obj4.isAndroid()) {
      const AccessibilityAnnouncer = tmp3(4680).AccessibilityAnnouncer;
  ...
  const memo1 = obj3.useMemo(() => {
    ...
    if (flag) {
      const Gesture2 = tmp2(6985).Gesture;
      ...
      const obj2 = { updateContextMenuState: tmp2(8265).updateContextMenuState, state: tmp };
      fn4.__closure = obj2;
      ...
      const Gesture3 = tmp2(6985).Gesture;
      ...
      let result = Gesture3.LongPress().enabled(flag5).minDuration(tmp2(8266).CONTEXT_MENU_LONG_PRESS_DURATION_MS).shouldCancelWhenOutside(false);
      ...
      const obj3 = { runOnJS: tmp2(4561).runOnJS, triggerHapticFeedback: tmp2(4797).triggerHapticFeedback, CONTEXT_MENU_OPEN_HAPTIC: tmp2(8266).CONTEXT_MENU_OPEN_HAPTIC, measureButtonAndShowMenu: callback3 };
      fn5.__closure = obj3;
      fn5.__workletHash = 13919366908951;
      fn5.__initData = __initData3;
      const minDurationResult = Gesture3.LongPress().enabled(flag5).minDuration(tmp2(8266).CONTEXT_MENU_LONG_PRESS_DURATION_MS);
      const Gesture4 = tmp2(6985).Gesture;
      return Gesture4.Simultaneous(result.onStart(fn5), onEndResult);
    } else {
      const Gesture = tmp2(6985).Gesture;
      if (flag2) {
        ...
        const obj5 = { runOnJS: tmp2(4561).runOnJS, triggerHapticFeedback: tmp2(4797).triggerHapticFeedback, CONTEXT_MENU_OPEN_HAPTIC: tmp2(8266).CONTEXT_MENU_OPEN_HAPTIC, measureButtonAndShowMenu: callback3 };
        fn.__closure = obj5;
        ...
        const obj6 = { updateContextMenuState: tmp2(8265).updateContextMenuState, state: tmp };
        fn2.__closure = obj6;
  ...
  const callback5 = obj3.useCallback(() => {
    ...
    if (isAndroidResult) {
      isAndroidResult = tmp(5258).getIsScreenReaderEnabled();
      const tmpResult = tmp(5258);
    }
    if (isAndroidResult) {
      tmp(4561).runOnUI(callback3)();
