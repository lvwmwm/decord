...
function Slot(index) {
  ...
  const glyph = tmp;
  const sharedValue = index(4566).useSharedValue(0);
  ...
  const animatedStyle = index(4566).useAnimatedStyle(T);
  let obj2 = index(4566);
  let obj3 = { trackStepAt, progress: sharedValue, size };
  obj4.children = jsx(size(4566).View, { style: animatedStyle, children: closure_7.map((children) => jsx(AIGlyphText.AIGlyphText, { size, color, allowFontScaling: false, style: glyph.glyph, children }, children)) });
