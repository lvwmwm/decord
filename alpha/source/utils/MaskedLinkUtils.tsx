...
// Dependencies: [8646, 2042, 2064, 8647, 4977, 4406, 2096, 1074, 1929, 5109, 1115, 1241, 8648, 8649, 8650, 8652, 4446, 13246, 13250, 5213, 13251, 13253, 2]
// Exports: handleClick, isLinkTrusted

// Module 8645 (MaskedLinkUtils)
import openURLDefault from "openURL" /* 4446 */;
import LinkAnalyticsUtilsDefault from "LinkAnalyticsUtils" /* 8650 */;
import BlockedDomainStore from "BlockedDomainStore" /* 8646 */;
import ChannelStore from "ChannelStore" /* 2042 */;
import GuildStore from "GuildStore" /* 2064 */;
import MaskedLinkStore from "MaskedLinkStore" /* 8647 */;
...
export const handleClick = function handleClick(href, preventDefault, arg2, contextKey) {
  ...
  if (null == sanitizeUrlResult) {
    ...
    try {
      ...
      tmp3(8650).trackLinkClicked(tmp8);
      if (null == onClick) {
      ...
      if (null !== guild_id1.isBlockedDomain(tmp8)) {
        ...
        if (!tmp55) {
          ...
          if (tmp56) {
            const obj8 = { url: tmp8, trustUrl: tmp53(13253).trustProtocol, onConfirm: handleConfirm, onCancel, isProtocol: true, contextKey };
            tmp3(13251).show(obj8);
            const tmp3Result12 = tmp3(13251);
          } else {
            const punycodeLinkResult = tmp53(5213).punycodeLink(tmp8);
            let displayTarget = tmp8;
            if (null != punycodeLinkResult) {
              displayTarget = punycodeLinkResult.displayTarget;
            }
            const tmp53Result = tmp53(5213);
            const obj9 = { url: displayTarget, trustUrl: tmp53(13253).trustDomain, onConfirm: handleConfirm, onCancel, isProtocol: false, contextKey };
            tmp3(13251).show(obj9);
            const tmp3Result13 = tmp3(13251);
          }
        }
        if (null == preventDefault) {
          handleConfirm();
          ...
          let result = tmp3(8650).trackAnnouncementMessageLinkClicked(obj11);
          const tmp3Result14 = tmp3(8650);
        }
        const obj10 = require("MaskedLinkStoreMethodsAdditional");
      }
      const tmp3Result10 = tmp3(8650);
    } catch (err) {
