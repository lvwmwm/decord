...
// Dependencies: [8717, 2042, 2064, 8718, 5047, 4472, 2096, 1074, 1929, 5193, 1115, 1241, 8719, 8720, 8721, 8723, 4512, 13328, 13332, 5297, 13333, 13335, 2]
// Exports: handleClick, isLinkTrusted

// Module 8716 (MaskedLinkUtils)
import openURLDefault from "openURL" /* 4512 */;
import LinkAnalyticsUtilsDefault from "LinkAnalyticsUtils" /* 8721 */;
import BlockedDomainStore from "BlockedDomainStore" /* 8717 */;
import ChannelStore from "ChannelStore" /* 2042 */;
import GuildStore from "GuildStore" /* 2064 */;
import MaskedLinkStore from "MaskedLinkStore" /* 8718 */;
...
export const handleClick = function handleClick(href, preventDefault, arg2, contextKey) {
  ...
  if (null == sanitizeUrlResult) {
    ...
    try {
      ...
      tmp3(8721).trackLinkClicked(tmp8);
      if (null == onClick) {
      ...
      if (null !== guild_id1.isBlockedDomain(tmp8)) {
        ...
        if (!tmp55) {
          ...
          if (tmp56) {
            const obj8 = { url: tmp8, trustUrl: tmp53(13335).trustProtocol, onConfirm: handleConfirm, onCancel, isProtocol: true, contextKey };
            tmp3(13333).show(obj8);
            const tmp3Result12 = tmp3(13333);
          } else {
            const punycodeLinkResult = tmp53(5297).punycodeLink(tmp8);
            let displayTarget = tmp8;
            if (null != punycodeLinkResult) {
              displayTarget = punycodeLinkResult.displayTarget;
            }
            const tmp53Result = tmp53(5297);
            const obj9 = { url: displayTarget, trustUrl: tmp53(13335).trustDomain, onConfirm: handleConfirm, onCancel, isProtocol: false, contextKey };
            tmp3(13333).show(obj9);
            const tmp3Result13 = tmp3(13333);
          }
        }
        if (null == preventDefault) {
          handleConfirm();
          ...
          let result = tmp3(8721).trackAnnouncementMessageLinkClicked(obj11);
          const tmp3Result14 = tmp3(8721);
        }
        const obj10 = require("MaskedLinkStoreMethodsAdditional");
      }
      const tmp3Result10 = tmp3(8721);
    } catch (err) {
