...
// Function ID: 12214
// Name: MessagesUtils
// Dependencies: [1074, 7571, 4745, 8138, 4905, 2]
      ...
      if (someResult) {
        ...
        someResult = codedLinks.some((item) => {
          ...
          if (CodedLink.CodedLinkType.BUILD_OVERRIDE !== type) {
            if (tmp(4745).CodedLinkType.MANUAL_BUILD_OVERRIDE !== type) {
              if (tmp(4745).CodedLinkType.EXPERIMENT === type) {
                const experimentFromEmbedURL = tmp(8138).getExperimentFromEmbedURL(code);
                ...
                if (tmp(4745).CodedLinkType.EVENT !== type) {
                  if (tmp(4745).CodedLinkType.CHANNEL_LINK !== type) {
                    if (tmp(4745).CodedLinkType.APP_DIRECTORY_PROFILE === type) {
