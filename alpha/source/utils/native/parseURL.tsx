...
// Function ID: 4808
// Name: parseURL
// Dependencies: [32, 1074, 1076, 4808, 4809, 1472, 1929, 1368, 4810, 4815, 13325, 5079, 14206, 1366, 4981, 9406, 7734, 1609, 1364, 10059, 1241, 1254, 14207, 2]
// Exports: default

// Module 4807 (parseURL)
...
import urlPartToSettingsEnumDefault from "urlPartToSettingsEnum" /* 14207 */;
...
export default function parseURL(arg0) {
  ...
  if (null == sanitizeUrlResult) {
    ...
    if (null != findCodedLinkResult) {
      ...
      if (tmp5(4815).CodedLinkType.INVITE === type) {
        ...
        obj5.payload = obj6;
        return obj5;
      } else if (tmp5(4815).CodedLinkType.TEMPLATE === type) {
        ...
        if (tmp5(4815).CodedLinkType.BUILD_OVERRIDE !== type) {
          if (tmp5(4815).CodedLinkType.MANUAL_BUILD_OVERRIDE !== type) {
            if (tmp5(4815).CodedLinkType.EXPERIMENT !== type) {
              if (tmp5(4815).CodedLinkType.EVENT !== type) {
                if (tmp5(4815).CodedLinkType.CHANNEL_LINK !== type) {
                  if (tmp5(4815).CodedLinkType.APP_DIRECTORY_PROFILE !== type) {
                    if (tmp5(4815).CodedLinkType.ACTIVITY_BOOKMARK === type) {
                      ...
                      obj9.payload = obj10;
                      return obj9;
                    } else if (tmp5(4815).CodedLinkType.EMBEDDED_ACTIVITY_INVITE !== type) {
                      if (tmp5(4815).CodedLinkType.GUILD_PRODUCT !== type) {
                        if (tmp5(4815).CodedLinkType.SERVER_SHOP !== type) {
                          if (tmp5(4815).CodedLinkType.SOCIAL_LAYER_STOREFRONT !== type) {
                            if (tmp5(4815).CodedLinkType.SOCIAL_LAYER_STOREFRONT_APP !== type) {
                              if (tmp5(4815).CodedLinkType.QUESTS_EMBED !== type) {
                                if (tmp5(4815).CodedLinkType.GAME_PROFILE === type) {
                                  ...
                                  obj11.payload = obj12;
                                  return obj11;
                                } else if (tmp5(4815).CodedLinkType.APP_DIRECTORY_STOREFRONT !== type) {
                                  if (tmp5(4815).CodedLinkType.APP_DIRECTORY_STOREFRONT_SKU !== type) {
                                    if (tmp5(4815).CodedLinkType.APP_OAUTH2_LINK !== type) {
                                      if (tmp5(4815).CodedLinkType.COLLECTIBLES_SHOP === type) {
                                        const tmp5Result = tmp5(13325);
                                      ...
                                      } else if (tmp5(4815).CodedLinkType.GAME_SERVER_SHARE !== type) {
                                        if (tmp5(4815).CodedLinkType.USER_PROFILE !== type) {
                                          const _Error2 = Error;
    ...
    if (findGiftCodesResult.length > 0) {
      ...
      const result = tmp5(14206).findRemoteAuthFingerprint(host, pathname);
      if (null != result) {
        if (result.length > 0) {
      ...
      if (null != pathname) {
        const tryParseDiceRollLinkResult = tmp5(4981).tryParseDiceRollLink(pathname);
        if (null != tryParseDiceRollLinkResult) {
          ...
          const tryParseChannelPathResult = tmp5(4981).tryParseChannelPath(pathname);
          if (null != tryParseChannelPathResult) {
            ...
            if (null != pathname.match(re27)) {
              ...
              if (null != pathname.match(re16)) {
                ...
                const result1 = tmp5(9406).parseOAuth2AuthorizeProps(str6);
                if (null != result1) {
                  ...
                  if (match5.length > 1) {
                    ...
                    switch (match5[1]) {
                      ...
                      case "mobile-web-redirect-checkout":
                        let result2 = tmp5(7734).isMobileWebRedirectCheckoutEnabled();
                        if (result2) {
          ...
          const tmp5Result11 = tmp5(4981);
        }
        const tmp5Result10 = tmp5(4981);
      }
      tmpResult6 = tmp(1366);
    }
  }
