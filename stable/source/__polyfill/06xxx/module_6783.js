// Module ID: 6783
// Function ID: 6784
// Dependencies: [41, 42, 93, 95, 98, 19, 17, 21, 6784, 6762, 6778, 6785, 6787, 6760, 6789, 6788, 6790, 6791, 6761, 6775]
// Exports: default

// Module 6783
import handlerIDToTag2 from "handlerIDToTag" /* 6760 */;
...
const jsx = fn(21).jsx;
fn(6784).customDirectEventTypes.topGestureHandlerEvent = { registrationName: "onGestureHandlerEvent" };
let closure_10 = { [fn(6762).State.UNDETERMINED]: undefined, [fn(6762).State.BEGAN]: "onBegan", [fn(6762).State.FAILED]: "onFailed", [fn(6762).State.CANCELLED]: "onCancelled", [fn(6762).State.ACTIVE]: "onActivated", [fn(6762).State.END]: "onEnded" };
...
export default function createHandler(name) {
  ...
  class Handler {
    constructor(arg0) {
      ...
      tmp3Result.createGestureHandler = (config) => {
        closure_0.handlerTag = _mod6778.getNextHandlerTag();
          ...
          if (!onHandlerStateChange) {
            if (onGestureEvent) {
              if ("__isNative" in onGestureEvent) {
                let REANIMATED_WORKLET = tmp2(6789).ActionType.NATIVE_ANIMATED_EVENT;
              }
            }
            REANIMATED_WORKLET = tmp2(6789).ActionType.JS_FUNCTION_OLD_API;
          }
          RNGestureHandlerModuleDefault.attachGestureHandler(tmp.handlerTag, viewTag, REANIMATED_WORKLET);
          const result1 = tmp2(6787).scheduleFlushOperations();
          const tmp2Result = tmp2(6787);
          tmp2(6788).ghQueueMicrotask(() => {
            const MountRegistry = Handler(config[16]).MountRegistry;
            const result = MountRegistry.gestureHandlerWillMount(closure_1_0);
          });
        }
        REANIMATED_WORKLET = tmp2(6789).ActionType.REANIMATED_WORKLET;
      };
  ...
  const entry = {
    ...
    value: function componentDidMount() {
      ...
      if (someResult) {
        if (someResult) {
          Handler(6788).ghQueueMicrotask(() => {
            self.update(1);
          });
          const obj = Handler(6788);
        }
        const props2 = self.props;
        const items = [];
        const obj2 = Handler(6787);
        ...
        if (self.viewNode) {
          self.attachGestureHandler(allowedProps(6791)(self.viewNode));
        } else {
          const _Error = Error;
