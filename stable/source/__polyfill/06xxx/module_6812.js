// Module ID: 6812
// Function ID: 6813
// Dependencies: [32, 19, 6813, 6824, 6815, 6828, 6799]
...
function isSupportedGesture(gestures) {
  if (obj.isComposedGesture(gestures)) {
    ...
    if (tmp(6815).SingleGestureName.Tap !== type) {
      if (tmp(6815).SingleGestureName.LongPress !== type) {
        if (tmp(6815).SingleGestureName.Fling !== type) {
          if (tmp(6815).SingleGestureName.Native !== type) {
            if (tmp(6815).SingleGestureName.Hover !== type) {
              return false;
            }
          }
        }
...
let closure_8 = fn(6813).SHARED_VALUE_OFFSET + 0.5;
...
export const useJSResponderHandler = function useJSResponderHandler(gesture) {
  _require = gesture;
  let tmp = closure_3(require("module_6828").JSResponderContext);
  dependencyMap = tmp;
  ...
  let handleStartShouldSetResponder = closure_4(() => {
    if (closure_4()) {
      const result = _mod6828.updateResponderEventValue(closure_1, true);
    }
    return false;
  }, items2);
  if (null == tmp) {
