// Module ID: 6731
// Function ID: 6732
// Dependencies: [32, 19, 17, 21, 1636, 6732, 6733, 6756, 6729, 6745, 38, 6739, 6909, 6911, 6914, 6916, 6920, 6923, 6927]
  ...
  constructor(arg0, arg1) {
      ...
      if (initialPosition !== sharedValue2.value) {
        if (undefined !== initialPosition) {
          if (sharedValue13.value !== value22.ANIMATION_STATE.RUNNING) {
            if (iter2.value === tmp18(6729).ANIMATION_STATE.RUNNING) {
              workletCallback();
            }
            iter2.value = tmp18(6729).ANIMATION_STATE.RUNNING;
            sharedValue14.value = GESTURE;
            sharedValue3.value = initialPosition;
            let tmp8 = state.value === tmp18(6729).KEYBOARD_STATE.SHOWN;
            if (tmp8) {
              tmp8 = keyboardBehavior !== tmp18(6729).KEYBOARD_BEHAVIOR.extend;
            }
            if (tmp8) {
            ...
            iter.value = tmp18(6745).animate(obj);
            const tmp18Result2 = tmp18(6745);
          }
        }
      }
    };
    ...
    getEvaluatedPosition = function getEvaluatedPosition(arg0) {
      ...
      if (arg0 === value22.ANIMATION_SOURCE.KEYBOARD) {
        if (DEFAULT_KEYBOARD_BLUR_BEHAVIOR === tmp(6729).KEYBOARD_BLUR_BEHAVIOR.restore) {
          if (value3 === tmp(6729).KEYBOARD_STATE.HIDDEN) {
            if (sharedValue10.value !== tmp(6756).State.ACTIVE) {
              if (sharedValue11.value !== tmp(6756).State.ACTIVE) {
      ...
      if (keyboardBehavior === value22.KEYBOARD_BEHAVIOR.extend) {
        if (value3 === tmp(6729).KEYBOARD_STATE.SHOWN) {
          return value4;
        }
      }
    ...
    handleSnapToPosition = function handleSnapToPosition(arg0, arg1) {
      ...
      if (tmp4) {
        ...
        tmp(1636).runOnUI(workletCallback2)(normalizeSnapPointResult, tmp(6729).ANIMATION_SOURCE.USER, 0, arg1);
