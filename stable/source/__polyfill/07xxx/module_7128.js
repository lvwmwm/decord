// Module ID: 7128
// Function ID: 7129
// Dependencies: [32, 19, 17, 21, 7122, 7123, 1505, 7129, 7108, 5721, 7130, 7131, 7132, 7133]
// Exports: Card
...
export const Card = function Card(shadowEnabled) {
  ...
  let tmp11 = num(1505)(() => {
    ...
    if (ref.current) {
      ...
      if (typeof getAnimateToValue === "function") {
        if (tmp16) {
          let num3 = _mod7122.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19);
        } else {
          num3 = 0;
        }
        if (tmp7 === num3) {
          if (ref2.current === num3) {
            ...
            if (tmp24) {
              gesture.setValue(_mod7122.getDistanceForDirection(tmp17, tmp18, "rtl" === tmp19));
  ...
  const obj8 = {
    ...
    onHandlerStateChange: num(1505)((nativeEvent) => {
      ...
      if (PanGestureHandler.GestureState.ACTIVE === state) {
        ...
        if (tmp(7108).GestureState.CANCELLED !== state) {
          if (tmp(7108).GestureState.FAILED !== state) {
            if (tmp(7108).GestureState.END === state) {
              first3.setValue(0);
              if ("vertical" !== gestureDirection) {
