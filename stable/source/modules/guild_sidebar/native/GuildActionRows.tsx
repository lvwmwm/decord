...
// Dependencies: [19, 17, 7635, 4651, 10246, 7200, 4818, 21, 4636, 576, 7438, 4457, 1943, 563, 7631, 4839, 11682, 1896, 12512, 12424, 1114, 13932, 2]
...
export const GuildRolesAndChannelsRow = function GuildRolesAndChannelsRow(guild) {
  ...
  const callback = noop.useCallback(() => {
    ModalActionCreatorsDefault.pushLazy(asyncRequireImpl(11682, dependencyMap.paths), { guildId: id }, closure_7);
  }, items2);
  let SELECTED = guild(12512).ChannelModes.DEFAULT;
  if (selected) {
    SELECTED = tmp5(12512).ChannelModes.SELECTED;
  }
  let tmp10 = !result;
  if (result) {
  ...
  let tmp11 = null;
  if (tmp10) {
    const obj4 = { style: tmp.channelInfoContainer, children: jsx(tmp5(12424).NewBadge, {}) };
    tmp11 = <View style={tmp.channelInfoContainer}>{jsx(tmp5(12424).NewBadge, {})}</View>;
  }
  ...
  obj5.name = jsx(guild(12512).BaseChannelName, { name: string2Result, mode: SELECTED });
  const tmp2Result = tmp2(12512);
  obj5.icon = jsx(guild(12512).BaseChannelIcon, { mode: SELECTED, IconComponent: guild(13932).ChannelListMagnifyingGlassIcon });
