...
// Function ID: 12200
// Name: useBannerBots
// Dependencies: [19, 1979, 12178, 504, 9428, 12200, 12171, 2]
// Exports: useBannerBots

// Module 12199 (useBannerBots)
import noop from "module_19" /* 19 */;
import GuildStore from "GuildStore" /* 1979 */;
import AppLauncherOnboardingStore from "AppLauncherOnboardingStore" /* 12178 */;
...
export const useBannerBots = function useBannerBots(context) {
  ...
  let obj = (function useAppsMap(context) {
    const apps = first1(12200).useApplicationsInContext({ context: context.context, onlyWithCommands: true, includeBuiltIn: false, includeEmbeddedApps: false, includeNonEmbeddedApps: true }).apps;
    const items = [apps];
  ...
  let apps = first1(12200).useApplicationsInContext({ context, onlyWithCommands: true, includeBuiltIn: false, includeEmbeddedApps: false, includeNonEmbeddedApps: true }).apps;
  let obj2 = first1(12200);
  const tmp2 = first1;
  const obj3 = (function useCommandsMap(context) {
    const commands = first1(12200).useApplicationCommandsInContext({ context: context.context, includeBuiltIn: false }).commands;
  ...
  const activityApplications = first1(12171).useActivityApplications({ guildId: guild_id, fetchesShelf: true });
  const obj4 = first1(12171);
