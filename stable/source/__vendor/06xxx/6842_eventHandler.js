...
// Function ID: 6843
// Name: eventHandler
// Dependencies: [6824, 6762, 6777, 6764, 6826]
...
function handleStateChangeEvent(result, arg1, lastUpdateEvent, fn) {
  ...
  if (oldState === _mod6762.State.UNDETERMINED) {
    if (state === tmp(6762).State.BEGAN) {
      tmp(6824).runCallback(tmp(6777).CALLBACK_TYPE.BEGAN, arg1, result);
      const tmpResult = tmp(6824);
    }
  }
  if (oldState === _mod6762.State.BEGAN) {
    if (state === tmp(6762).State.ACTIVE) {
      if (fn != null) {
        fn(result);
      }
      tmp(6824).runCallback(tmp(6777).CALLBACK_TYPE.START, arg1, result);
      const tmpResult4 = tmp(6824);
    }
  }
  if (oldState !== state) {
    const obj2 = {};
    const merged = Object.assign(result);
    obj2.canceled = state === tmp(6762).State.FAILED || state === tmp(6762).State.CANCELLED;
    if (oldState === tmp(6762).State.ACTIVE) {
      if (fn != null) {
        fn(obj2);
      }
      tmp(6824).runCallback(tmp(6777).CALLBACK_TYPE.END, arg1, obj2);
      const tmpResult5 = tmp(6824);
    }
    const tmp7 = state === tmp(6762).State.FAILED || state === tmp(6762).State.CANCELLED;
    tmp(6824).runCallback(tmp(6777).CALLBACK_TYPE.FINALIZE, arg1, obj2);
    if (lastUpdateEvent) {
      lastUpdateEvent.lastUpdateEvent = undefined;
    }
    const tmpResult6 = tmp(6824);
  }
}
...
function handleTouchEvent(eventType, arg1) {
  if (eventType.eventType !== TouchEventType.TouchEventType.UNDETERMINED) {
    const tmpResult = tmp(6824);
    tmpResult.runCallback(tmp(6824).touchEventTypeToCallbackType(eventType.eventType), arg1, eventType);
    ...
    if (tmpResult.isStateChangeEvent(result)) {
      ...
      if (tmpResult6.isTouchEvent(result)) {
        if (typeof handleTouchEvent === "function") {
          if (result.eventType !== tmp(6764).TouchEventType.UNDETERMINED) {
            const tmpResult7 = tmp(6824);
            tmpResult7.runCallback(tmp(6824).touchEventTypeToCallbackType(result.eventType), arg2, result);
        ...
        if (typeof handleUpdateEvent === "function") {
          ...
          const result1 = tmp(6824).flattenAndFilterEvent(tmp9);
          const tmpResult9 = tmp(6824);
          tmp(6824).runCallback(tmp(6777).CALLBACK_TYPE.UPDATE, arg2, result1);
          if (lastUpdateEvent) {
            lastUpdateEvent.lastUpdateEvent = result;
          }
          const tmpResult10 = tmp(6824);
        } else {
          throw new TypeError("Trying to call a non-function");
        }
      }
