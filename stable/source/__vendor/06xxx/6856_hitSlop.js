...
// Function ID: 6857
// Name: hitSlop
// Dependencies: [32, 109, 19, 17, 21, 6761, 6828, 6857, 6858, 6859, 6860, 6861, 6854, 6769, 6767]
// Exports: default
...
export default function _default(pressRetentionOffset) {
  ...
  closure_28 = closure_6(() => {
    if (ref5.current == null) {
      tmp.current = _mod6828.isKeyboardDismissingTap(closure_18);
    }
  }, items5);
    ...
    onBegin() {
      ...
      if (!ref5.current) {
        if (Platform.isTV) {
          ...
          if (isScreenReaderEnabled) {
            handleEvent(NATIVE_BEGIN, tmp5(6857).viewCenterToPressableEvent(closure_20.current));
            const tmp5Result = tmp5(6857);
          } else {
