...
// Dependencies: [32, 19, 17, 21, 6774, 6834, 6770, 6829, 6771, 6761, 6799, 6830, 6831, 6823, 6832]
// Exports: InterceptingGestureDetector

// Module 6833 (InterceptingGestureDetector)
...
export const InterceptingGestureDetector = function InterceptingGestureDetector(gesture) {
  ...
  if (prop) {
    let REANIMATED = tmp(6834).InterceptingDetectorMode.REANIMATED;
  } else {
    let prop1;
    if (gesture != null) {
  ...
  register(() => {
    ...
    if (prop) {
      closure_7.setMode(_mod6834.InterceptingDetectorMode.ANIMATED);
    } else {
      let prop1;
      if (tmp != null) {
      ...
      if (prop1) {
        closure_7.setMode(_mod6834.InterceptingDetectorMode.REANIMATED);
      }
    }
  }, items2);
