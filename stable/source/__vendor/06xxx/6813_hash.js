...
// Function ID: 6814
// Name: hash
// Dependencies: [32, 6800, 6799, 6814, 6823]
...
let c4 = 1.618;
fn(6800).NativeProxy.updateGestureHandlerConfig;
...
maybeUnpackValue.__closure = { Reanimated: fn(6799).Reanimated };
...
function getEnabledSharedValues(Reanimated) {
  if (undefined === _mod6799.Reanimated) {
    return [];
  } else {
    if (tmpResult.isComposedGesture(Reanimated)) {
      ...
      Reanimated = tmp(6799).Reanimated;
...
export const bindSharedValues = function bindSharedValues(shouldUseReanimatedDetector, handlerTag) {
  _require = handlerTag;
  if (undefined !== require("module_6799").Reanimated) {
    ...
    while (tmp27 !== undefined) {
      ...
      if (Reanimated.isSharedValue(tmp7)) {
        let Reanimated2 = tmp10(6799).Reanimated;
    ...
    function _loop(maybeSharedValue) {
      ...
      if (Reanimated.isSharedValue(maybeSharedValue)) {
        if (typeof hash === "function") {
          ...
          closure_1 = sum;
          const Reanimated2 = _mod6799.Reanimated;
          const fn = function t() {
