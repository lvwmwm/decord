...
// Dependencies: [12936, 12940, 12942, 12944, 12945, 12946, 12947, 12948, 12949, 12952, 12957, 12930]
...
// Module 12935 (spanTimeInputToSeconds)
  ...
  if (typeof getTime === "number") {
    ...
    if (Array.isArray(getTime)) {
      ...
      if (getTime instanceof Date) {
        ...
        sum = _mod12944.timestampInSeconds();
      }
    }
  }
  return sum;
  ...
  if ((function spanIsSentrySpan(getSpanJSON) {
    ...
    try {
      ...
      if ((function spanIsOpenTelemetrySdkTraceBaseSpan(attributes) {
        ...
        obj3.op = attributes[_mod12945.SEMANTIC_ATTRIBUTE_SENTRY_OP];
        obj3.origin = attributes[_mod12945.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
        const obj2 = _mod12936;
        obj3._metrics_summary = _mod12946.getMetricSummaryJsonForSpan(getSpanJSON);
...
function getStatusMessage(code) {
  if (code) {
    if (code.code !== _mod12947.SPAN_STATUS_UNSET) {
      let str = "ok";
      if (code.code !== tmp(12947).SPAN_STATUS_OK) {
...
export const addChildSpanToSpan = function addChildSpanToSpan(arg0, arg1) {
  ...
  const result = _mod12936.addNonEnumerableProperty(arg1, _sentryRootSpan, tmp2);
  if (arg0[_sentryChildSpans]) {
    ...
    const result1 = _mod12936.addNonEnumerableProperty(arg0, tmp6, set);
...
export const getActiveSpan = function getActiveSpan() {
  ...
  if (asyncContextStrategy.getActiveSpan) {
    ...
    activeSpan = tmpResult._getSpanForScope(tmp(12957).getCurrentScope());
...
export const spanToTraceContext = function spanToTraceContext(spanContext) {
  ...
  return _mod12936.dropUndefinedKeys({ parent_span_id, span_id, trace_id: trace_id.traceId });
};
export const spanToTraceHeader = function spanToTraceHeader(spanContext) {
  ...
  return _mod12942.generateSentryTraceHeader(traceId, spanId, 1 === spanContext.spanContext().traceFlags);
};
export const spanToTransactionTraceContext = function spanToTransactionTraceContext(spanContext) {
  ...
  return _mod12936.dropUndefinedKeys({ parent_span_id, span_id, trace_id, data, op, status, origin });
};
