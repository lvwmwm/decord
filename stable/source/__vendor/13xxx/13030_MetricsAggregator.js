...
// Dependencies: [32, 41, 42, 13027, 12944, 13031, 13032, 12935, 13033]

// Module 13030 (MetricsAggregator)
...
const entry = {
  ...
  value: function add(metricType, arg1, diff, none, tags) {
    ...
    if (arg5 === undefined) {
      timestampInSecondsResult = MetricsAggregator(12944).timestampInSeconds();
      const obj2 = MetricsAggregator(12944);
    }
    const self = this;
    const rounded = Math.floor(timestampInSecondsResult);
    const sanitizeMetricKeyResult = MetricsAggregator(13031).sanitizeMetricKey(arg1);
    const obj3 = MetricsAggregator(13031);
    const sanitizeTagsResult = MetricsAggregator(13031).sanitizeTags(obj);
    const obj4 = MetricsAggregator(13031);
    const sanitizeUnitResult = MetricsAggregator(13031).sanitizeUnit(str);
    const obj5 = MetricsAggregator(13031);
    const bucketKey = MetricsAggregator(13031).getBucketKey(metricType, sanitizeMetricKeyResult, sanitizeUnitResult, sanitizeTagsResult);
    ...
    if (value) {
      ...
      const tmp15 = new tmp5(13032).METRIC_MAP[metricType](diff);
      obj7.metric = tmp15;
    ...
    const obj6 = MetricsAggregator(13031);
    const result1 = MetricsAggregator(12935).updateMetricSummaryOnActiveSpan(metricType, sanitizeMetricKeyResult, diff, sanitizeUnitResult, obj, bucketKey);
    self._bucketsTotalWeight = self._bucketsTotalWeight + obj7.metric.weight;
    if (self._bucketsTotalWeight >= MetricsAggregator(13027).MAX_WEIGHT) {
      self.flush();
    }
  }
};
  ...
  },
  {
    ...
    value: function _flush() {
      ...
      if (this._forceFlush) {
        ...
        const _Math = Math;
        const rounded = Math.floor(MetricsAggregator(12944).timestampInSeconds());
        const _Map = Map;
        const diff = rounded - MetricsAggregator(13027).DEFAULT_FLUSH_INTERVAL / 1000 - self._flushShift;
        const map = new Map();
  ...
  },
  {
    ...
    value: function _captureMetrics(_buckets) {
      if (_buckets.size > 0) {
        ...
        const result = MetricsAggregator(13033).captureAggregateMetrics(this._client, mapped);
        const obj = MetricsAggregator(13033);
      }
    }
  }
];
