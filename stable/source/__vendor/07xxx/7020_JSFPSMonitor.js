...
// Function ID: 7021
// Name: JSFPSMonitor
// Dependencies: [6967, 6968, 6955, 7021]

// Module 7020 (JSFPSMonitor)
...
const items = [
  entry,
  {
    ...
    value: function startTracking() {
      ...
      if (0 !== this.startTime) {
        const _Error = Error;
        const error = new Error(JSFPSMonitor(6955).ErrorMessages.fpsMonitorAlreadyRunning);
        throw error;
      } else {
        const _Date = Date;
  ...
  },
  {
    ...
    value: function stopAndGetData() {
      ...
      const obj = { minFPS: JSFPSMonitor(7021).roundToDecimalPlaces(self.minFPS, 1), maxFPS: null, averageFPS: null };
      const obj2 = JSFPSMonitor(7021);
      obj.maxFPS = JSFPSMonitor(7021).roundToDecimalPlaces(self.maxFPS, 1);
      const obj3 = JSFPSMonitor(7021);
      obj.averageFPS = JSFPSMonitor(7021).roundToDecimalPlaces(self.averageFPS, 1);
      return obj;
    }
  }
];

export const JSFPSMonitor = _modDef6968(JSFPSMonitor, items);
