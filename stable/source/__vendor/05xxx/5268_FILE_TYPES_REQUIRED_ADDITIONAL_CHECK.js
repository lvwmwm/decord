...
// Dependencies: [41, 42, 5266, 5269, 5275, 5276, 5277, 5278, 5279]

// Module 5268 (FILE_TYPES_REQUIRED_ADDITIONAL_CHECK)
import AudioTypes from "AudioTypes" /* 5275 */;
import ImageTypes from "ImageTypes" /* 5276 */;
import VideoTypes from "VideoTypes" /* 5277 */;
import CompressedTypes from "CompressedTypes" /* 5278 */;
import OtherTypes from "OtherTypes" /* 5279 */;
...
const entry = {
  ...
  value: function getInfoByName(key10027) {
    return FileTypes(5266).fetchFromObject(FileTypes, key10027.toUpperCase());
  }
};
let items = [
  entry,
  {
    ...
    value: function getSignaturesByName(item10079) {
      return FileTypes(5266).fetchFromObject(FileTypes, item10079.toUpperCase()).signatures;
    }
  },
  {
    key: "detectSignature",
    ...
    value: function detectTypeByAdditionalCheck(fileChunk, found) {
      ...
      if (mapped.some((item) => {
        ...
        if (!mapped.includes("heic")) {
          let str6 = "flv";
          if (!FileTypes(5269).isFLV(fileChunk)) {
            let str8 = "mp4";
            if (tmp8(5269).isM4V(fileChunk)) {
              str8 = "mp4";
              if (!tmp8(5269).isHEIC(fileChunk)) {
                str8 = "m4v";
              }
            }
            str6 = str8;
        ...
        const result = FileTypes(5266).findMatroskaDocTypeElements(fileChunk);
        let str2 = "mkv";
        if ("mkv" !== result) {
          let str4;
          if ("webm" === result) {
            if (tmp3(5269).isWEBM(fileChunk)) {
              str4 = "webm";
            }
          }
          str2 = str4;
        ...
        if (mapped.some((item) => {
          ...
          if (FileTypes(5266).isAvifStringIncluded(fileChunk)) {
            str = "avif";
          }
        }
        return str;
