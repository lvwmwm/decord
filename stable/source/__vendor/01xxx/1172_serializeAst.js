...
function serializeAst(ast, value) {
  ...
  while (iter !== undefined) {
    ...
    if (typeof nextResult !== "string") {
      ...
      if (InternalIntlMessage(1158).FormatJsNodeType.Argument === first) {
      ...
      } else if (tmp5(1158).FormatJsNodeType.Date === first) {
        value.value = `${value.value}{${tmp2[1]}, date`;
        if (null != tmp2[2]) {
          value.value = `${value.value}, ${tmp2[2]}`;
        }
        value.value = `${value.value}}`;
      } else if (tmp5(1158).FormatJsNodeType.Time === first) {
        value.value = `${value.value}{${tmp2[1]}, time`;
        if (null != tmp2[2]) {
          value.value = `${value.value}, ${tmp2[2]}`;
        }
        value.value = `${value.value}}`;
      } else if (tmp5(1158).FormatJsNodeType.Number === first) {
        value.value = `${value.value}{${tmp2[1]}, number`;
        if (null != tmp2[2]) {
          value.value = `${value.value}, ${tmp2[2]}`;
        }
        value.value = `${value.value}}`;
      } else if (tmp5(1158).FormatJsNodeType.Plural === first) {
        let str = "plural";
        if ("ordinal" == tmp2[4]) {
          str = "selectordinal";
        }
        ...
        value.value = `${value.value}}`;
      } else if (tmp5(1158).FormatJsNodeType.Pound === first) {
        value.value = `${value.value}#`;
      } else if (tmp5(1158).FormatJsNodeType.Select === first) {
        ...
        value.value = `${value.value}}`;
      } else if (tmp5(1158).FormatJsNodeType.Tag === first) {
