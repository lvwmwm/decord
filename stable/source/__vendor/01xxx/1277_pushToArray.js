...
// Function ID: 1278
// Name: pushToArray
// Dependencies: [1276, 1274, 1278]

// Module 1277 (pushToArray)
...
let obj2 = {
  ...
  filter: "r",
  format: _mod1274.default,
  formatter: _mod1274.formatters[_mod1274.default],
  indices: false,
  serializeDate(arg0) {
      ...
      if (undefined === str2) {
        ...
        if ("comma" === fn) {
          if (isArray(str2)) {
            ...
            if (tmp17) {
              maybeMapResult = tmp10(1276).maybeMap(str2, fn2);
              const tmp10Result = tmp10(1276);
            }
            let tmp18;
...
export default (arg0, allowEmptyArrays) => {
  if (allowEmptyArrays) {
    ...
    let format = _mod1274.default;
    if (undefined !== allowEmptyArrays.format) {
      const call = hasOwnProperty.call;
      const formatters = tmp3(1274).formatters;
    ...
    obj.filter = filter;
    obj.format = format;
    obj.formatter = _mod1274.formatters[format];
